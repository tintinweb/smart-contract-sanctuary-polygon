...
contract Box is ERC721Enumerable, RBAC {
    string public defaultURI;

    uint256 private _index;

    event DefaultURISet(string);

    constructor(
        string memory _tokenName,
        string memory _tokenSymbol,
        address _admin
    ) ERC721(_tokenName, _tokenSymbol) RBAC(_admin) {}
    ...
    {
        return
            ERC721.supportsInterface(_interface) ||
            ERC721Enumerable.supportsInterface(_interface) ||
            AccessControlEnumerable.supportsInterface(_interface);
    }
