...
// File contracts/interfaces/IDaoChef.sol
...
// File contracts/farms/DaoZapMMSwap.sol

pragma solidity 0.8.4;

/*
    DaoZap is a ZapperFi's simplified version of zapper contract which will:
    1. use ETH to swap to target token
    2. make LP between ETH and target token
    3. add into DaoChef farm
*/
contract DaoZapMMSwap is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    ...
    ZapInfo[] public zaps;
    IDaoChef public daoChef;
    IUniswapV2Router02 public uniRouter;

    constructor(address _daoChef, address _uniRouter) {
        require(_daoChef != address(0), "Zap::constructor: Invalid address");
        require(_uniRouter != address(0), "Zap::constructor: Invalid address");
        daoChef = IDaoChef(_daoChef);
    ...
    function zap(
        ...
        if (WethUtils.isWeth(address(_info.token0))) {
            _ethIn = msg.value;
            WethUtils.wrap(_ethIn);
        } else IERC20(_info.token0).safeTransferFrom(msg.sender, address(this), _ethIn);

        address _lp = address(daoChef.lpToken(_info.pid));
        ...
        approveToken(_lp, address(daoChef), _liquidity);
        daoChef.deposit(_info.pid, _liquidity, msg.sender);
