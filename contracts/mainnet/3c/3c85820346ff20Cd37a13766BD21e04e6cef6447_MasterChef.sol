...
interface IMigratorChef {
    // Perform LP token migration from legacy Plearn to PlearnSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to Plearn LP tokens.
    // PlearnSwap must mint EXACTLY the same amount of PlearnSwap LP tokens or
    // else something bad will happen. Traditional Plearn does not
    // do that so be careful!
    function migrate(IBEP20 token) external returns (IBEP20);
}

// MasterChef is the master of Plearn. He can make Plearn and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once PLEARN is sufficiently
...
// PLearnToken with Governance.
contract PlearnToken is BEP20("Plearn Token", "PLN") {
    using SafeMath for uint256;
...
contract PlearnEarn is BEP20("PlearnEarn Token", "EARN") {
    ...
    // Safe plearn transfer function, just in case if rounding error causes pool to not have enough PLEARNs.
    function safePlearnTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 plearnBal = plearn.balanceOf(address(this));
        if (_amount > plearnBal) {
            plearn.transfer(_to, plearnBal);
        } else {
