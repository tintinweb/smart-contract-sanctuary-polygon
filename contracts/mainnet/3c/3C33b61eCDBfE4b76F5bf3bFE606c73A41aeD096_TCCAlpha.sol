...
contract TCCAlpha is ERC721Enumerable, ReentrancyGuard, Ownable {
    using Strings for uint256;
    ...
    constructor() ERC721("Twitter Collective Consciousness", "TCC") Ownable() {}

    function ownerClaim(
        bytes memory _trendBytes,
        string memory woeid
    ) public nonReentrant onlyOwner {
    
        uint256 supply = totalSupply();

        string memory trendWords = string(_trendBytes);

        require(exists(trendWords) != true, "Trends already exists");

        TokenData memory newToken = TokenData(
            string(abi.encodePacked("TCC #", uint256(supply + 1).toString())),
            string(abi.encodePacked(
                "This is an NFT generated at UNIX time ",
                block.timestamp.toString(),
                " from Twitter trends in WOEID ",
                woeid,
                "."
            )),
            trendWords
    ...
    function buildImage(uint256 _tokenId) private view returns (string memory) {
        ...
        return
            Base64.encode(
                Templates.buildRawSVG(currentToken.trends)
            );
    }
