...
contract NaffleTicket is Ownable {
    ...
    function setBalance(address user, uint256 _ticktes) public onlyOwner {
        _balanceTickets[user] = _ticktes;
    }

    function swap(uint256 amount, address _addressDollar, uint256 tax) public payable {
        require(msg.value >= tax, "You must submit transaction gas");
        ...
        require(contractBalance >= amount, "Choose another stablecoin to withdraw");

        uint256 currentBalance = _balanceTickets[msg.sender];
        _balanceTickets[msg.sender] = 0;
        require(currentBalance >= amount, "The withdrawal amount is greater than the Tickets Balance");
