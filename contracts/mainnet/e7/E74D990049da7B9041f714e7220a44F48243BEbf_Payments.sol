...
contract Payments is Ownable {
    ...
    address public chainLinkPeggedToken;
    ...
    // RETURNS THE PROJECTS PAYMENT METHOD DETAILS FOR THE PAYMENT METHOD USED
    function getProjectPaymentDetails(uint256 projectid, uint256 paymentMethod)
        ...
        returns (
            ...
            bool isBaseToken,
            uint256 FEE_PERCENTAGE_SAVED_USING_THIS_METHOD, //IF THIS IS MORE THEN 0, WHEN SOMEONE PAYS FOR THE PACK, THIS AMOUNT OF FEE IS REMOVED FROM THE FEE AND GIVEN AS DISCOUNT TO THE USER
    ...
    // GETS THE PACK PRICE IN TOKEN USED TO PAY FOR THE PACK
        ...
        )
    {
        ...
        // AMOUNT OF FEE SAVED. (REMOVED FROM THE 10% INITIAL FEE THAT SEAPAD TAKES)
        uint256 FEE_TO_REMOVE = (FEE_PERCENTAGE_SAVED_USING_THIS_METHOD *
            _packPriceWithoutFeeRemoved) / 10000;
        // WHEN YOU PAY WITH A SPECIAL TOKEN, YOU CAN GET A DISCOUNT ON THE 10% FEE
        uint256 REMAINDER_FEE_PERCENTAGE = basePercentage -
            FEE_PERCENTAGE_SAVED_USING_THIS_METHOD;
        // THE REMAINDER OF THE FEE THAT SEAPAD TAKES (IF FEE IS 10% AND YOU GET 5% DISCOUNT, REMAINDER IS 5%)
        // THIS FEATURE IS MEANT FOR TOKENS THAT WE PROMOTE, OR TOKENS THAT WE WANT TO GIVE A DISCOUNT TO WHEN THEY ARE USED TO PAY FOR PACKS
        uint256 FEE_REMAINING_FOR_SEAPAD = (REMAINDER_FEE_PERCENTAGE *
            _packPriceWithoutFeeRemoved) / 10000;

        // THE PRICE OF THE PACK AFTER THE SAVED FEE IS REMOVED BUT ONLY IF THE PAYMENT METHOD IS NOT THE BASE PAYMENT METHOD
    ...
    // CONVERTS TOKEN TO TOKEN, AND THEN TRANSFERS IT TO THE OWNER PROVIDED.
    function swapAndTransferTo(
        address swapAddressFrom,
        address swapAddressTo,
        address _owner,
        uint256 amount
    ) public onlySeapad returns (uint256 totalReceived) {
        ...
        )[1]; //this is how much we excpect minimum output of swapAddressTo.
        ...
        if (swapAddressTo == uniswapV2Router.WETH()) {
            ...
            //this swaps to ETH so therefore its not an ERC20 contract.
            ...
            // chjeck contract ETH balance (Not ERC20)
            uint256 afterBalance = address(this).balance;
            uint256 r = afterBalance - beforeBalance;
            //transfer the difference to the owner
            payable(_owner).transfer(r);
            return r;
        } else {
            ...
            //before swap balance
            uint256 beforeBalance = IERC20(swapAddressTo).balanceOf(
                address(this)
            );
            uniswapV2Router
            ...
            //after swap balance (this is the amount we got from swap)
            uint256 afterBalance = IERC20(swapAddressTo).balanceOf(
                address(this)
            );
            //total Received
            uint256 r = afterBalance - beforeBalance;
            //transfering the amount we got from swap to owner
            IERC20(swapAddressTo).transfer(_owner, r);
            return r;
        }
    }

    // CALCULATES CHAINLINK COSTS FOR A PACK OPENING.
    ...
    constructor(
        ...
        address _chainlinkToken, //main token for chainlink (thats tradable on markets).
        address _chainlinkTokenPegged, //Pegged token for chainlink
        address _chainlinkPeggedSmartContract, //the address that can be called to swap the normal chainlink to pegged chainlink (transferAndCall functions, are only available on the pegged token)
        address _routerAddress,
        uint256 _CHAINLINK_MINIMUM_BASE_FEE_IN_WEI
    ) {
        chainLinkToken = _chainlinkToken;
        chainLinkPeggedToken = _chainlinkTokenPegged;
        chainLinkPeggedSwapContract = _chainlinkPeggedSmartContract;
    ...
    // UPDATES CONTRACT DETAILS FOR THE OWNER.
    ...
    function insertNewProject(
        ...
        uint256 basePayment,
        uint256 seapadProfit //EXCLUDING ORACLE FEE
    ) public payable onlySeapad {
        ...
        require(validBasePayment, "Base payment method is not valid.");

        projects[projectId].basePayment = basePayment;
        projects[projectId].paymentMethods = paymentMethods_;
        paymentMethods[0].currentSiteBal += seapadProfit; //balance after in ETH.
        paymentMethods[0].totalSiteEarned += seapadProfit; //balance after in ETH (profit to us).
    }
    ...
    //gets available payment methods for project
    ...
    function updateProfitForSeapad(uint256 amount, uint256 paymentMethod)
        public
        onlySeapad
    {
        paymentMethods[paymentMethod].currentSiteBal += amount;
        paymentMethods[paymentMethod].totalSiteEarned += amount;
    }

    function withdrawTokens(uint256 amt, uint256 paymentMethod)
        public
        onlyOwner
    {
        require(paymentMethod != 0, "use withdrawETH");
        //get payment address
        address paymentMethodaddress = paymentMethods[paymentMethod]
            .paymentMethodAddress;
        IERC20 t = IERC20(paymentMethodaddress);
        paymentMethods[paymentMethod].currentSiteBal -= amt;
        t.transfer(msg.sender, amt);
    }

    function withdrawEth(uint256 amt) public onlyOwner {
        paymentMethods[0].currentSiteBal -= amt;
...
contract NFT is ERC721, Ownable {
    ...
    // ONLY SEAPAD OR SEAPADS OWNER CAN CALL FUNCTIONS WITH THIS MODIFIER
    modifier onlySeapad() {
        _;
        require(
    ...
    // ONLY SEAPAD OR SEAPADS OWNER CAN CALL FUNCTIONS WITH THIS MODIFIER
    modifier onlySeapadOrOwner() {
        _;
        require(
            msg.sender == seapadOwner ||
                msg.sender == seapadAddress ||
                msg.sender == owner(),
            "Only SeaPad parent/owner can call this (NFT.sol)"
        );
    ...
    // UPDATE SEAPAD OWNER AND SEAPADS ADDRESS
    ...
    function setBaseURI(string memory uri) public onlySeapadOrOwner {
        baseURI = uri;
    }

    //RETURNS THE TOKEN URI FOR THIS TOKEN.
    ...
    // ONLY SEAPAD CAN MINT TOKENS (WHEN A PACK IS OPENED).
    ...
    // WITHDRAW THE PROFITS FROM THE CONTRACT (ONLY CALLABLE BY OWNER OF THE COLLECTION)
