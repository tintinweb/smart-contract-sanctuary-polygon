  ...
  IKommunitasStakingV3
{
  ...
  function locked(uint16 _lockIndex) external view virtual returns(
    uint128 lockPeriodInDays,
    uint64 apy_d2,
    uint64 feeInPercent_d2,
    uint256 komStaked,
  ...
  function stake(
    ...
    // take out komToken
    IERC20MintableBurnableUpgradeable(komToken).safeTransferFrom(
      sender,
      address(this),
      _amount
    );
  ...
  function unstake(
    ...
    if(isWorker[_msgSender()]){
      require(block.timestamp > stakeDetail.endedAt, "premature");
    } else {
      _staker = _msgSender();
    }
  ...
  function changeCompoundType(
    uint232 _userStakedIndex,
    CompoundTypes _newCompoundType
  ) external virtual {
    // owner validation
  ...
  function _unstake(
    ...
    require(staked[_sender].length > _userStakedIndex,  "bad");
    ...
    IERC20MintableBurnableUpgradeable(komToken).safeTransfer(
      _sender,
      withdrawableAmount
    );
  ...
  function setPeriodInDays(
    ...
    require(
      lockNumber > _lockIndex &&
      _newLockPeriodInDays >= 86400 &&
      _newLockPeriodInDays <= (5 * yearInSeconds),
      "bad"
    );
  ...
  function setPenaltyFee(
    ...
    require(
      lockNumber > _lockIndex &&
      _feeInPercent_d2 >= 100 &&
      _feeInPercent_d2 < 10000,
      "bad"
    );
  ...
  function togglePause() external onlyOwner virtual {
    if(paused()){
      _unpause();
    } else {
      _pause();
