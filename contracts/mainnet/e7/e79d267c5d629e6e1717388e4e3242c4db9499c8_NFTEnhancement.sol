...
contract NFTEnhancement is INFTEnhancement, ERC721 {
        ...
        external
    {
        ...
        uint256 id = composeTokenId(renderer, qualifier);
        names[id] = name;
    ...
    function composeTokenId(address renderer, uint96 qualifier)
        public
        pure
        returns (uint256 tokenId)
    {
        tokenId = (uint256(qualifier) << 160) | uint256(uint160(renderer));
    }

    function decomposeTokenId(uint256 tokenId)
        public
        pure
        returns (address renderer, uint96 qualifier)
    {
        renderer = address(uint160(tokenId & type(uint160).max));
        qualifier = uint96(tokenId >> 160);
    }
