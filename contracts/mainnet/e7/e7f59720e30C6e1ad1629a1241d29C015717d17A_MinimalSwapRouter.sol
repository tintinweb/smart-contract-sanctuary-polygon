...
contract MinimalSwapRouter is Multicall {
    ...
    /// @dev Performs a single exact input swap
    function limitlessExactInputInternal(
        uint256 amountIn,
        address recipient,
        SwapCallbackData memory data
    ) private returns (uint256 amountOut) {
    ...
    function exactInputToSelf(ExactInputToSelfParams memory params) external payable returns (uint256 amountOut) {
        ...
        while (true) {
            ...
            // the outputs of prior swaps become the inputs to subsequent ones
            params.amountIn = limitlessExactInputInternal(
                params.amountIn,
    ...
    /// @dev Performs a single exact output swap
    function limitlessExactOutputInternal(
        uint256 amountOut,
        address recipient,
        SwapCallbackData memory data
    ) private returns (uint256 amountIn) {
    ...
    function exactOutput(ExactOutputParams calldata params) external payable returns (uint256 amountIn) {
        ...
        limitlessExactOutputInternal(params.amountOut, params.recipient, SwapCallbackData({path: params.path, payer: msg.sender}));
