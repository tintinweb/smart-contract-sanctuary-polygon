...
import "@chainlink/contracts/src/v0.8/VRFConsumerBaseV2.sol"; // Import Chainlink VRF
 ...
 *  - anti-botting
 */
contract WorldChallengeGameMagicEden is IERC721M, ERC721AQueryable, Ownable, AccessControl, ReentrancyGuard, VRFConsumerBaseV2 {
    using ECDSA for bytes32;
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    // Variables para Chainlink VRF
    ...
    // WORLD Challenge NFT
    mapping(uint256 => uint256) public idToGenetics;
    mapping(uint256 => uint256) public geneticsToId;

    uint256[] private available;
    uint256[] private used;
    mapping(uint256 => bool) public genetics;

    // Variables de almacenamiento para la información de acuñación
    struct MintData {
        uint32 qty;
        bytes32[] proof;
        uint64 timestamp;
    ...
    mapping(uint256 => MintData) public mintRequests;
    ...
    function grantMinterRole(address account) public onlyOwner {
        // Solo el dueño puede otorgar el rol de minter a otra cuenta
        grantRole(MINTER_ROLE, account);
    }

    function revokeMinterRole(address account) public onlyOwner {
        // Solo el dueño puede revocar el rol de minter a otra cuenta
        revokeRole(MINTER_ROLE, account);
    }
    
    function getGenToId(uint256 _gen) public view returns (uint256) {
        return geneticsToId[_gen];
    }

    function getIdtoGen(uint256 _tokenId) public view returns (uint256) {
        return idToGenetics[_tokenId];
    }
    


    function getGenAvailable() public view returns (uint256[] memory) {
        return available;
    }

    function addGenetic(uint256 _gen) public onlyOwner {
        require(!genetics[_gen], "Genetic already added");
        genetics[_gen] = true;
        available.push(_gen);
    }
    ...
    function addGenetics(uint256[] memory _genetics) public onlyOwner {
        for (uint256 i = 0; i < _genetics.length; i++) {
            addGenetic(_genetics[i]);
        }
    }

    function getGenUsed() public view returns (uint256[] memory) {
        return used;
    }

    function totalGenUsed() public view returns (uint256) {
        return used.length;
    }

    function totalGenAvailable() public view returns (uint256) {
    ...
    function fulfillRandomWords(
        ...
        MintData memory mintData = mintRequests[_requestId];
        preMint(mintData.qty, randomResult);
        _mintInternal(mintData.qty, mintData.to, mintData.proof, mintData.timestamp, mintData.signature);
    }
    ...
    function preMint(uint32 qty, uint256 randomness) internal { 
        for (uint256 i = totalSupply(); i < totalSupply()+qty; i++) {
            uint256 choosenIndex = uint256(
                keccak256(
                    abi.encodePacked(block.timestamp, block.difficulty, msg.sender, available.length, randomness)
                )
            ) % available.length;

            uint256 _gen = available[choosenIndex];
    ...
    function mint(
        ...
        // Antes de la acuñación, realizamos una solicitud al VRF
        uint256 requestId = requestRandomWords();
        // Almacenamos la información de acuñación
        mintRequests[requestId] = MintData(qty, proof, timestamp, signature, msg.sender);
    }
     ...
     * This is typically used for owner to  pre-mint or mint the remaining of the supply.
     */
    function ownerMintWithGen(uint256 gen, address to)
        external
        hasSupply(1)
    {
        require(hasRole(MINTER_ROLE, msg.sender), "YourContract: must have minter role to mint");

        // add preMint world challenge
        // if gen is in genetics search gen in available and save index in choosenIndex and delete from available
        if(genetics[gen] == true) {
            uint256 choosenIndex = 0;
        ...
        idToGenetics[totalSupply()+1] = gen;
        geneticsToId[gen] = totalSupply()+1;
     ...
     * Override by world challenge devs
     */
        ...
        returns (string memory)
    {
        ...
        // World Challenge NFT
        uint256 _gen = getIdtoGen(tokenId);
