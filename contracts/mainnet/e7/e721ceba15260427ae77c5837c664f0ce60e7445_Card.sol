...
contract Card is ERC721URIStorage {
    ...
    function generateImage(uint256 tokenId, address addr)
        public
        view
        returns (string memory)
    {
        bytes memory svg = abi.encodePacked(
            ...
            '<text fill="#000" style="font-family: Arial; font-size: 32px;" x="220" y="120">',
            getName(tokenId),
            '</text><text fill="#000" style="font-family: Arial; font-size: 24px;" x="220" y="176">',
            getAbbrAddress(addr),
            "</text></svg>"
        );
        return
    ...
    function getTokenURI(uint256 tokenId) public view returns (string memory) {
        ...
        bytes memory dataURI = abi.encodePacked(
            ...
            name,
            '","image":"',
            generateImage(tokenId, ownerOf(tokenId)),
            '"',
            "}"
        );
        return
    ...
    function mint(string memory name) public {
        require(checkName(name), "Invalid name");
    ...
    function checkName(string memory name) public view returns (bool) {
        ...
        require(data.length >= 3, "Name too short");
        require(data.length <= 50, "Name too long");
        for (uint256 i = 0; i < data.length; i++) {
            uint8 b = uint8(data[i]);
            if (b < MIN || b > MAX) {
                return false;
            }
            if (b == QUOTE || b == LT || b == GT) {
                return false;
            }
        }
        if (uint8(data[0]) == MIN || uint8(data[data.length - 1]) == MIN) {
    ...
    uint8 constant MIN = 32;
    uint8 constant MAX = 126;
    uint8 constant QUOTE = 34;
    uint8 constant LT = 60;
    uint8 constant GT = 62;
}