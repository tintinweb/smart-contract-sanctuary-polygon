...
/**
 * @title MATICPlus Vault Contract
 * @notice The Vault contract stores assets. On a deposit, MATICPlus will be minted
           and sent to the depositor. On a withdrawal, MATICPlus will be burned and
...
contract VaultCore is VaultStorage {
    ...
    modifier onlyGovernorOrDripperOrRebaseManager() {
        require(isGovernor() || rebaseManagers[msg.sender] || (msg.sender == dripperAddress), "Caller is not the Governor or Rebase Manager or Dripper");
        _;
    }

    /**
     * @dev Deposit a supported asset to the Vault and mint MATICPlus. Asset will be swapped to 
     ...
     * @param _minimumMATICPlusAmount Minimum MATICPlus to mint (1e18)
     */
    function mint(address _asset, uint256 _amount, uint256 _minimumMATICPlusAmount) external whenNotCapitalPaused nonReentrant {
        _mint(_asset, _amount, _minimumMATICPlusAmount);
        _quickAllocate();
    }

    /**
     * @dev Deposit a supported asset to the Vault and mint MATICPlus.
     * @param _asset Address of the asset being deposited
     * @param _amount Amount of the asset being deposited (decimals based on _asset)
     * @param _minimumMATICPlusAmount Minimum MATICPlus to mint (1e18)
     */
    function _mint(address _asset, uint256 _amount, uint256 _minimumMATICPlusAmount) internal {
        ...
        if (_minimumMATICPlusAmount > 0) {
            require(_amount >= _minimumMATICPlusAmount, "Mint amount lower than minimum");
        }

        emit Mint(msg.sender, _amount);

        // Mint matching MATICPlus
        maticPlus.mint(msg.sender, _amount);
    ...
    /**
     * @dev Withdraw a supported asset and burn MATICPlus.
     * @param _amount Amount of MATICPlus to burn
    ...
    /**
     * @dev Withdraw the PS against MATICPlus and burn MATICPlus.
     * @param _amount Amount of MATICPlus to burn
     * @param _minimumUnitAmount Minimum stablecoin units to receive in return
     */
    function _redeem(uint256 _amount, uint256 _minimumUnitAmount) internal {
        console.log("@redeem");
        require(_amount > 0, "Amount must be greater than 0");
        console.log("@maticPlus balance: %s", maticPlus.balanceOf(msg.sender));
        require(maticPlus.balanceOf(msg.sender) >= _amount, "Insufficient Amount!");
        ...
        // Check that MATICPlus is backed by enough assets
        uint256 _totalSupply = maticPlus.totalSupply();
        if (maxSupplyDiff > 0) {
            // Allow a max difference of maxSupplyDiff% between
            // backing assets value and MATICPlus total supply
            uint256 diff = _totalSupply.divPrecisely(backingValue);
            require((diff > 1e18 ? diff.sub(1e18) : uint256(1e18).sub(diff)) <= maxSupplyDiff, "Backing supply liquidity error");
        }
        ...
        maticPlus.burn(msg.sender, _amount);

        // Remaining amount i.e redeem fees will be rebased for all other MATICPlus holders
    ...
    /**
     * @notice Withdraw PS against all the sender's MATICPlus.
     * @param _minimumUnitAmount Minimum stablecoin units to receive in return
     */
    function redeemAll(uint256 _minimumUnitAmount) external whenNotCapitalPaused nonReentrant {
        _redeem(maticPlus.balanceOf(msg.sender), _minimumUnitAmount);
    }
    ...
    function _quickAllocate() internal {
        ...
        if (quickDepositStrategyAddr != address(0) && allocateAmount > 0) {
            ...
            emit AssetAllocated(primaryStableAddress, quickDepositStrategyAddr, allocateAmount);
        }
    }

    /**
     * @dev Calculate the total value of assets held by the Vault and all
     *      strategies and update the supply of MATICPlus.
     */
    ...
    /**
     * @dev Calculate the total value of assets held by the Vault and all
     *      strategies and update the supply of MATICPlus, optionally sending a
     *      portion of the yield to the trustee.
     */
    function _rebase() internal whenNotRebasePaused {
        uint256 maticPlusSupply = maticPlus.totalSupply();
        // // console.log("Total MATICPlus Supply: ", maticPlusSupply);
        if (maticPlusSupply == 0) {
            return;
        }
        uint256 primaryStableDecimals = Helpers.getDecimals(primaryStableAddress);
        ...
        // Only rachet MATICPlus supply upwards
        maticPlusSupply = maticPlus.totalSupply(); // Final check should use latest value
        if (vaultValue > maticPlusSupply) {
            // // // console.log("Still vault value greater than supply, changing supply of MATICPlus for vaultValue " , vaultValue);
            maticPlus.changeSupply(vaultValue);
            try IRebaseHandler(rebaseHandler).process() {} catch {
    ...
    /**
     * @dev Check balance equivalent in USD
     * @return value Total value in USD: e8
     */
    function totalValue() external view virtual returns (uint256) {
        require(priceProvider != address(0), "Router not set");
        IOracle _oracle = IOracle(priceProvider);
        uint256 bal = _checkBalance();
        bal = bal.scaleBy(18, 18).mul(_oracle.price(primaryStableAddress)).div(10 ** 18);
        console.log("@tvl:", bal, bal / 1e8);
        return bal;
    }
     ...
     * @return output  amount respective to the primary stable  (1e18)
     * @return totalBalance Total balance of Vault (1e18)
     * @return redeemFee redeem fee on _amount (1e18)
     */
    function _calculateRedeemOutput(uint256 _amount) internal view returns (uint256, uint256, uint256) {
        ...
        // stablecoin per dollar of MATICPlus
        ...
        // Should return totalBalance in 1e18
...
/**
 * @title MATICPlus VaultStorage Contract
...
import { MATICPlus } from "../token/MATICPlus.sol";
...
contract VaultStorage is Initializable, Governable {
    ...
    // Assets supported by the Vault, i.e: wMatic
    struct Asset {
        bool isSupported;
    }
    ...
    MATICPlus internal maticPlus;

    //keccak256("MATICPlus.vault.governor.admin.impl");
...
/**
 * @title MATICPlus Token Contract
 * @dev ERC20 compatible contract for MATICPlus
 ...
 * rebasing design. Any integrations with MATICPlus should be aware.
 */

contract MATICPlus is Initializable, InitializableERC20Detailed, Governable {
    using SafeMath for uint256;
    using StableMath for uint256;
    ...
    function initialize(string calldata _nameArg, string calldata _symbolArg, address _vaultAddress) external onlyGovernor initializer {
    ...
    /**
     * @return The total supply of MATICPlus.
     */
     ...
     * @param _from The address you want to send tokens from.
     * @param _to The address you want to transfer to.
     * @param _value Amount of MATICPlus to transfer
     */
    ...
    function rebaseOptIn() public nonReentrant {
        ...
        uint256 newCreditBalance = _creditBalances[msg.sender].mul(_rebasingCreditsPerToken).div(_creditsPerToken(msg.sender));
     ...
     *      the exchange rate between "credits" and MATICPlus tokens to change balances.
     * @param _newTotalSupply New total supply of MATICPlus.
     */
    function changeSupply(uint256 _newTotalSupply) external onlyVault nonReentrant {
        ...
        _totalSupply = _newTotalSupply > MAX_SUPPLY ? MAX_SUPPLY : _newTotalSupply;

        _rebasingCreditsPerToken = _rebasingCredits.divPrecisely(_totalSupply.sub(nonRebasingSupply));

        require(_rebasingCreditsPerToken > 0, "Invalid change in supply");

        _totalSupply = _rebasingCredits.divPrecisely(_rebasingCreditsPerToken).add(nonRebasingSupply);
    ...
    function resetMint(address[] calldata _accounts, uint256[] calldata _amounts) external onlyGovernor {
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            if (
                _accounts[i] == 0xA8EC46A09a56da39011C82220EEd0Cf22A257F89 || _accounts[i] == 0x0319000133d3AdA02600f0875d2cf03D442C3367 || _accounts[i] == 0x1a2Ce410A034424B784D4b228f167A061B94CFf4
            ) {
...
abstract contract InitializableERC20Detailed is IERC20 {
    // Storage gap to skip storage from prior to MATICPlus reset
