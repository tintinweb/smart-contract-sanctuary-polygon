...
contract BeaverBelievers is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    constructor(string memory _name, string memory _symbol, uint256 _mintRate, string memory _hiddenBaseUri) ERC721(_name, _symbol) {
        mintRate = _mintRate;
    ...
    function safeMint(address to) public payable {
        ...
        require(_tokenIdCounter.current() <= MAX_SUPPLY, "Max supply minted");
        _safeMint(to, tokenId);
        _tokenIdCounter.increment();
    }

    function airdropTokens(address[] calldata wAddresses) public onlyOwner {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length != 0 ? string(abi.encodePacked(baseURI, Strings.toString(tokenId), ".json")) : string(".json");
