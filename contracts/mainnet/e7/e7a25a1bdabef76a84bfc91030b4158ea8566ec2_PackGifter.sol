...
contract PackGifter is Ownable, IERC721Receiver
{
                     ...
                     string memory jsonReferrer) public onlyOwner
  {
    ...
    /* Build up the move to gift the packs and send the SMC to receiver
       and referrer (if any).  */
    bytes memory toReferrer = "";
    if (bytes (jsonReferrer).length > 0)
      {
        require (
            keccak256 (bytes (jsonReferrer)) != keccak256 (bytes (jsonName)),
            "referrer is the receiver name");
        toReferrer = abi.encodePacked (
          ",", jsonReferrer, ":", Strings.toString (referrer)
        );
      }
    string memory move = string (abi.encodePacked (
      "{\"g\":{\"smc\":{",
        ...
        "\"sc\":{",
        jsonName, ":", Strings.toString (smc),
        toReferrer,
        "}",

        "\"bp\":{",
        jsonName, ":", Strings.toString (packs),
        "}",

      "}}}"
    ));

    /* Send the move to execute all the in-game transfers.  */
