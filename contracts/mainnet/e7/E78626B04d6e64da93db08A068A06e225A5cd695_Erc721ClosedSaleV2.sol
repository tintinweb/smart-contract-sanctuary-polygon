...
contract Erc721ClosedSaleV2 is Erc721SaleV2 {
  ...
  event SaleRevealed(bool isRevealed, uint256 offsetIndex);
    ...
    whenNotPaused
  {
    ...
    baseURI = baseURI_;
    isRevealed = true;
    emit SaleRevealed(isRevealed, offsetIndex);
  }
  ...
  function tokenURI(uint256 tokenId_) public view override(ERC721) returns (string memory) {
    ...
    if (isRevealed) {
      ...
      return string(abi.encodePacked(baseURI, string.concat(offsetTokenId.toString(), '.json')));
    }
    return baseURI;
  }
}
...
import "./IAccessControlV2.sol";

/**
 * @dev This is from an exact copy of OpenZeppelin AccessControl Contract
        Public methods `grantRole`, `revokeRole`, `renounceRole` are removed from here.
        Support for DEFAULT_ADMIN_ROLE removed as its managed by CONTRACT_ADMIN_ROLE
 */
abstract contract AccessControlV2 is Context, IAccessControlV2, ERC165 {
  struct RoleData {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
    return interfaceId == type(IAccessControlV2).interfaceId || super.supportsInterface(interfaceId);
  }
...
abstract contract VerifyV2 is EIP712, RoleV2 {
  ...
  function _verifyVoucher(
    ...
    require(
      params_.issueTime <= (block.timestamp + 60) && block.timestamp <= (params_.issueTime + params_.expirationDuration),
      "35"
    );
...
abstract contract Erc721SaleV2 is ERC721, ERC721Royalty, VerifyV2 {
  ...
  function markMultisigNonceUsed(uint256 nonce_) external {
    require(hasRole(CONTRACT_ADMIN_ROLE, _msgSender()) || hasRole(FINANCE_ADMIN_ROLE, _msgSender()), "10");
