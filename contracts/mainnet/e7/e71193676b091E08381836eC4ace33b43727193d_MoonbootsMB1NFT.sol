  ...
  Claimable
{
    ...
    ERC721A("MB1 Genesis", "MB1") 
  {
    ...
    maxSupply = 10000;
    mintPrice = 250000000000000000000;
    maxQuantity = 3;
  }
    ...
    nonReentrant
  {
    ...
    requireValidQuantity(_quantity);
  ...
  */
  ...
  function batchMint(
    ...
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    if(_to.length != _quantities.length) {
      revert InvalidQuantity();
    }
    uint256 total = 0;
    for(uint256 i = 0; i < _to.length; i++) {
      _safeMint(_to[i], _quantities[i]);
      total += _quantities[i];
    }
    ...
    override
  {
    if(blockList[_msgSender()]) {
      revert AddressIsBlocked(_msgSender());
    }
    if(blockList[_from]) {
      revert AddressIsBlocked(_from);
    }
    if(blockList[_to]) {
      revert AddressIsBlocked(_to);
    }
    if(_quantity == 1)
      if(address(0) != _from)
  ...
  error AddressIsBlocked(address target);
}
