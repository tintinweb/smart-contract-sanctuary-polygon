...
contract GuildFacet is Modifiers{
    function modifyGuildSettings(
        ...
        s.guilds[_guild].backupProfitSplitter = _backupSplitter;
    }
    ...
    function setGuildBackupSplitter(uint256 _guild, address _backupSplitter) public onlyApproved{
        s.guilds[_guild].backupProfitSplitter = _backupSplitter;
    }

    function getGuildBackupSplitter(uint256 _guild) public view returns(address){
        return s.guilds[_guild].backupProfitSplitter;
    }

    function getGuildBackupSplitterByTokenId(uint256 _tokenId) public view returns(address){
        return getGuildBackupSplitter(s.gotchiMapping[_tokenId].assignedGuild);
    }

    function getGuild(uint256 _tokenId) public view returns(uint256){
        return s.gotchiMapping[_tokenId].assignedGuild;
    }
}
