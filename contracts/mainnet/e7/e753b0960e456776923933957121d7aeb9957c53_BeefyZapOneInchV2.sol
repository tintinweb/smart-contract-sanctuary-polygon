...
contract BeefyZapOneInchV2 {
    ...
    constructor(address _oneInchRouter, address _router, address _WETH) {
    ...
    function beefInETH (address beefyVault, bytes memory token0, bytes memory token1) external payable {
        ...
        _swapAndStake(beefyVault, WETH, token0, token1);
    }
    ...
    function beefIn (address beefyVault, address inputToken, uint256 tokenInAmount, bytes memory token0, bytes memory token1) external {
        ...
        _swapAndStake(beefyVault, inputToken, token0, token1);
    }
    ...
    function _swapAndStake(address beefyVault, address inputToken, bytes memory token0, bytes memory token1) private {
        (IBeefyVault vault, IUniswapV2Pair pair) = _getVaultPair(beefyVault);
