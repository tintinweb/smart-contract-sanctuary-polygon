...
contract CreativezVoting is Ownable {
    ...
    mapping(uint256 => bool) public NFTTokenIDs;   
    ...
    function initiateAVote(string memory _option1, address _option1StakingAddress, string memory _option2, address _option2StakingAddress, string memory _option3, address _option3StakingAddress) public onlyOwner{
        ...
        for (uint256 i = 1; i == Creativez(CreativezNFTAddrs).getSupp(); i++){
            NFTTokenIDs[i] = true;
        } 
        emit votingBegins (
    ...
    function vote(address option) public nonReentrant{
        ...
        if (block.timestamp < voteLength){
            ...
            for (uint256 i = 0; i==Creativez(CreativezNFTAddrs).balanceOf(msg.sender)-1; i++) {
                uint256 tokenID = Creativez(CreativezNFTAddrs).tokenOfOwnerByIndex(msg.sender, i);
                if (NFTTokenIDs[tokenID]=true){
                  count++;} 
                NFTTokenIDs[tokenID]=false;
                }             
    ...
    //this function will check whether the NFT TokenID has voted or not (true = can still vote & false = already voted)
    function checkIfTokenIDHasVoted (uint256 _tokenID) public view returns (bool){
        return NFTTokenIDs[_tokenID];
    }
}