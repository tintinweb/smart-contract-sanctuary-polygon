...
contract TradeNFT is ERC721Enumerable, MetaContext  {
    ...
    mapping(address => bool) private _isMinter; // Options contract should be minter
    mapping(uint256 => Trade) private _trades; // NFT id to Trade

    uint256[] private _openTrades;
    mapping(uint256 => uint256) private _openTradesIndexes;

    mapping(uint256 => uint256[]) private _assetOpenTrades;
    mapping(uint256 => mapping(uint256 => uint256)) private _assetOpenTradesIndexes;
    ...
    function ownerOf(uint256 _id) public view override(ERC721, IERC721) returns (address) {
    ...
    function openTrades() public view returns (uint256[] memory) { return _openTrades; }
    function openTradesIndexes(uint256 _id) public view returns (uint256) { return _openTradesIndexes[_id]; }
    function assetOpenTrades(uint256 _asset) public view returns (uint256[] memory) { return _assetOpenTrades[_asset]; }
    function assetOpenTradesIndexes(uint256 _asset, uint256 _id) public view returns (uint256) { return _assetOpenTradesIndexes[_asset][_id]; }
    ...
    */
    function mint(
        ...
        newTrade.collateral = _mintTrade.collateral;
        ...
        newTrade.trader = _mintTrade.account;
        newTrade.id = newTokenID;
        newTrade.tigAsset = _mintTrade.tigAsset;
        newTrade.receivedAsset = _mintTrade.receivedAsset;
        newTrade.stableVault = _mintTrade.stableVault;
        ...
        if (_mintTrade.orderType > 0) {
            ...
            _openTrades.push(newTokenID);
            _openTradesIndexes[newTokenID] = _openTrades.length-1;

            _assetOpenTrades[_mintTrade.asset].push(newTokenID);
            _assetOpenTradesIndexes[_mintTrade.asset][newTokenID] = _assetOpenTrades[_mintTrade.asset].length-1;
        }
        _tokenIds.increment();
    ...
    /**
     * @param _id id of the trade NFT
     * @param _price price used for execution
     */
    function executeLimitOrder(uint256 _id, uint256 _price, uint256 _collateral) external onlyMinter {
        ...
        _trade.collateral = _collateral;
        _trade.expires = _trade.duration + block.timestamp;
        ...
        _openTrades.push(_id);
        _openTradesIndexes[_id] = _openTrades.length-1;
        _assetOpenTrades[_asset].push(_id);
        _assetOpenTradesIndexes[_asset][_id] = _assetOpenTrades[_asset].length-1;
    }
    ...
    */
    function burn(uint256 _id) external onlyMinter {
        ...
        if (_trades[_id].orderType > 0) {
            ...
            _assetOpenTradesIndexes[_asset][_assetOpenTrades[_asset][_assetOpenTrades[_asset].length-1]] = _assetOpenTradesIndexes[_asset][_id];
            _assetOpenTrades[_asset][_assetOpenTradesIndexes[_asset][_id]] = _assetOpenTrades[_asset][_assetOpenTrades[_asset].length-1];
            delete _assetOpenTradesIndexes[_asset][_id];
            _assetOpenTrades[_asset].pop();  

            _openTradesIndexes[_openTrades[_openTrades.length-1]] = _openTradesIndexes[_id];
            _openTrades[_openTradesIndexes[_id]] = _openTrades[_openTrades.length-1];
            delete _openTradesIndexes[_id];
            _openTrades.pop();              
        }
        delete _trades[_id];
    }

    function assetOpenTradesLength(uint256 _asset) external view returns (uint256) {
        return _assetOpenTrades[_asset].length;
    }
    ...
    function openTradesSelection(uint256 _from, uint256 _to) external view returns (uint[] memory) {
        uint[] memory _ids = new uint[](_to-_from);
        for (uint256 i=0; i<_ids.length; i++) {
            _ids[i] = _openTrades[i+_from];
        }
        return _ids;
    }
