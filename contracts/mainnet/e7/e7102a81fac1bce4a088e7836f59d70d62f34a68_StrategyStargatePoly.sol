...
contract StrategyStargatePoly is StratManager, FeeManager, GasThrottler {
    ...
    ITridentRouter.ExactInputSingleParams public outputToInputParams;
    ...
    constructor(
        ...
        output = _routes.outputToInputRoute[0];
        input = _routes.outputToInputRoute[_routes.outputToInputRoute.length - 1];
        native = _routes.inputToNativeRoute[_routes.inputToNativeRoute.length - 1];

        require(_routes.inputToNativeRoute[0] == input, 'inputToNativeRoute[0] != input');
        outputToInputRoute = _routes.outputToInputRoute;
        inputToNativeRoute = _routes.inputToNativeRoute;

        outputToInputParams = ITridentRouter.ExactInputSingleParams(
            0,
            1,
            _routes.outputToInputPool,
            output,
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        ITridentRouter(unirouter).exactInputSingleWithNativeToken(outputToInputParams);

        uint256 toNative = IERC20(input).balanceOf(address(this)).mul(45).div(1000);
        IUniswapRouterETH(quickRouter).swapExactTokensForTokens(toNative, 0, inputToNativeRoute, address(this), now);
    ...
    function callReward() external view returns (uint256) {
        ...
        if (outputBal > 0) {
            ...
            uint256 inputBal = IBentoPool(outputToInputParams.pool).getAmountOut(data);
            if (inputBal > 0) {
                uint256[] memory amountOut = IUniswapRouterETH(quickRouter).getAmountsOut(inputBal, inputToNativeRoute);
    ...
    function _giveAllowances() internal {
        ...
        IERC20(input).safeApprove(quickRouter, uint256(-1));
    ...
    function _removeAllowances() internal {
        ...
        IERC20(input).safeApprove(quickRouter, 0);
        IERC20(input).safeApprove(stargateRouter, 0);
    }
