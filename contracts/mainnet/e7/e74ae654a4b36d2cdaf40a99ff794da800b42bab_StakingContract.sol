...
constructor(uint256 _rewardPerTokenPerHour, address _rewardTokenAddress) {
    depositFee = 1;
    withdrawFee = 1;
...
function stake(uint256 _amount) external notContract() {
    ...
    require(rewardToken.transferFrom(msg.sender, address(this), _amount), "StakingContract: failed to transfer tokens");
...
function withdraw() external nonReentrant notContract() {
    ...
    require(rewardToken.transfer(msg.sender, totalWithdrawAmount), "StakingContract: failed to transfer tokens");
