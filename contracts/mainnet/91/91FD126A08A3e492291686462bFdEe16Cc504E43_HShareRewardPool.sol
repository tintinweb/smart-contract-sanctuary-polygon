...
// File contracts/distribution/HShareRewardPool.sol
...
contract HShareRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accHSharePerShare; // Accumulated tSHAREs per share, times 1e18. See below.
    ...
    uint256 public tSharePerSecond = 0.000744488 ether; // 23800 hshare / (370 days * 24h * 60min * 60s)
    uint256 public runningTime = 370 days; // 370 days
    uint256 public constant TOTAL_REWARDS = 23800 ether;
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "HShareRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "HShareRewardPool: existing pool?");
        }
    }
    ...
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            accHSharePerShare = accHSharePerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
        return user.amount.mul(accHSharePerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accHSharePerShare = pool.accHSharePerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount.mul(pool.accHSharePerShare).div(1e18).sub(user.rewardDebt);
            if (_pending > 0) {
    ...
    function safeHShareTransfer(address _to, uint256 _amount) internal {
