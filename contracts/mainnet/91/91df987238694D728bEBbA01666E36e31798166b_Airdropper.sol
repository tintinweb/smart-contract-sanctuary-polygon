...
contract Airdropper is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(address payable _VILTokenAddress) {
    ...
    function setWhitelist(address[] memory _addresses, uint256[] memory _amounts) public onlyOwner {
    ...
    function checkWhitelist(address _address) public view returns (uint256 _amount) {
    ...
    function airdrop() public nonReentrant whenNotPaused returns (bool result) {
        require(whitelist[msg.sender] > 0, "Unable to claim airdrop");

        uint8 decimals = token.decimals();
        uint256 numVILTokens = whitelist[msg.sender] * 10 ** decimals;
