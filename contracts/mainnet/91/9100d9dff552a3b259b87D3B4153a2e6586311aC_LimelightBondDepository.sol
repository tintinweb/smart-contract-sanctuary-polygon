...
contract LimelightBondDepository is Ownable {
  ...
  address public immutable CLOUT; // token given as payment for bond
  address public immutable principle; // token used to create bond
  address public immutable treasury; // mints CLOUT when receives principle
  ...
  struct Bond {
    uint256 payout; // CLOUT remaining to be paid
    uint256 vesting; // Blocks left to vest
    uint256 lastBlock; // Last interaction
    uint256 pricePaid; // In BCT, for front end viewing
  }
  ...
  function deposit(
    ...
    require(payout >= 10000000, "Bond too small"); // must be > 0.01 CLOUT ( underflow protection )
    ...
    /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) CLOUT
         */
    ...
    if (fee != 0) {
      ...
      IERC20(CLOUT).safeTransfer(DAO, fee);
    }
  ...
  function stakeOrSend(
    ...
    if (!_stake) {
      ...
      IERC20(CLOUT).transfer(_recipient, _amount); // send payout
    } else {
      // if user wants to stake
      if (useHelper) {
        // use if staking warmup is 0
        IERC20(CLOUT).approve(stakingHelper, _amount);
  ...
  function maxPayout() public view returns (uint256) {
    return IERC20(CLOUT).totalSupply().mul(terms.maxPayout).div(100000);
  }
  ...
  /**
   *  @notice calculate current ratio of debt to CLOUT supply
   *  @return debtRatio_ uint
   */
  function debtRatio() public view returns (uint256 debtRatio_) {
    uint256 supply = IERC20(CLOUT).totalSupply();
  ...
  /**
   *  @notice calculate amount of CLOUT available for claim by depositor
   *  @param _depositor address
  ...
  /**
   *  @notice allow anyone to send lost tokens (excluding principle or CLOUT) to the DAO
   *  @return bool
   */
  function recoverLostToken(address _token) external returns (bool) {
    require(_token != CLOUT);
