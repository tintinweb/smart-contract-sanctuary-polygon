...
contract AutoUnstake {
  FarmStaker staker;
  address buildings;

  constructor(address _staker, address _buildings) {
    staker = FarmStaker(_staker);
    buildings = _buildings;
  }

  function autoUnstake(uint256 landId, uint256 farmId) public {
    FarmStaker.Asset[] memory assets = staker.allAssetsOnFarm(msg.sender, landId, farmId);
    uint256[] memory buildingTokenIds = new uint256[](100);
    for (uint256 i = 0; i < buildingTokenIds.length; i++) {
      buildingTokenIds[i] = 100;
    }

    uint256 btIndex = 0;
    for (uint256 i = 0; i < assets.length; i++) {
      address asset = assets[i].staking.asset;
      uint256 tokenId = uint256(assets[i].staking.tokenId);
      if (asset == buildings) {
        buildingTokenIds[btIndex++] = tokenId;
      } else {
        staker.unstake(asset, msg.sender, landId, farmId, tokenId);
      }
    }

    for (uint256 i = 0; i < btIndex; i++) {
      staker.unstake(buildings, msg.sender, landId, farmId, buildingTokenIds[i]);
    }
  }

  function manualUnstake(uint256 landId, uint256 farmId, address[] calldata assets, uint256[] calldata tokenIds) public {
    require(assets.length == tokenIds.length, "Wrong arrays");
    for (uint256 i = 0; i < assets.length; i++) {
      staker.unstake(assets[i], msg.sender, landId, farmId, tokenIds[i]);
    }
  }
}
