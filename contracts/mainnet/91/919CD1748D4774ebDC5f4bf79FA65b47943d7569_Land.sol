    ...
    ReentrancyGuard
{
    ...
    uint256 public constant MAX_SUPPLY = 13333;
    ...
    event PresaleAmountUpdated(uint256 amount);
    ...
    constructor(
        ...
        uint256 _price
    ) ERC721("Chicken Derby Land NFT", "LND") RandomlyAssigned(MAX_SUPPLY, 0) {
    ...
    function setPresaleAmount(uint256 _amount) public onlyOwner {
        if (!_isPresaleActive()) revert Errors.PreSaleNotActive();
        if (_amount > MAX_SUPPLY) revert Errors.ExceedMaxSupply();
        presaleAmount = _amount;
        emit PresaleAmountUpdated(_amount);
    }

    function startPresale(uint256 _amount, uint256 _daysAfter)
        public
        onlyOwner
    {
        if (_isSaleStarted) revert Errors.SaleStarted();
        _presaleStart = block.timestamp;
        _publicSaleStart = _presaleStart + _daysAfter * 1 days;
        if (_amount > MAX_SUPPLY) revert Errors.ExceedMaxSupply();
        presaleAmount = _amount;
        _isSaleStarted = true;
        emit PresaleStart(_presaleStart, _amount, _publicSaleStart);
    }
    ...
    function _mintRandomId(address to) internal {
        ...
        assert(id > 0 && id <= MAX_SUPPLY);
        _safeMint(to, id);
    }

    function _mintLoop(uint256 count) internal {
    ...
    // presale
    function whiteListedMint(uint256 _mintAmount, Coupon memory coupon)
        public
        nonReentrant
        ensureAvailabilityFor(_mintAmount)
    {
        if (!_isPresaleActive()) revert Errors.PreSaleNotActive();
        if (_mintAmount <= 0) revert Errors.MintNoNFT();
        if (_mintAmount > maxMintAmount) revert Errors.ExceedMaxMintAmount();
        if (tokenCount() + _mintAmount > presaleAmount)
            revert Errors.ExceedPresaleSupply();
        bytes32 digest = keccak256(abi.encode(_msgSender()));
        if (!_isVerifiedCoupon(digest, coupon)) revert Errors.NotWhiteListed();
        if (
            IERC20(wethContract).allowance(_msgSender(), address(this)) <
            price.mul(_mintAmount)
    ...
    function mint(uint256 _mintAmount)
        ...
        ensureAvailabilityFor(_mintAmount)
    {
        if (!_isPublicSaleActive()) revert Errors.PublicSaleNotActive();
        if (_mintAmount <= 0) revert Errors.MintNoNFT();
        if (_mintAmount > maxMintAmount) revert Errors.ExceedMaxMintAmount();
        if (
            IERC20(wethContract).allowance(_msgSender(), address(this)) <
    ...
    function tokensOwnedByAddress(address _owner)
        public
        view
    ...
    function withDraw() public onlyOwner {
        uint256 balance = IERC20(wethContract).balanceOf(address(this));
        IERC20(wethContract).transfer(_msgSender(), balance);
    }
}
