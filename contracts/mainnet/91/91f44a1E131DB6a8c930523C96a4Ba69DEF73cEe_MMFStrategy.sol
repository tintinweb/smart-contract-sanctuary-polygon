...
contract MMFStrategy is IMmfStrategy, Operator {
    ...
    function getPendingReward(uint256 _rwPid) public view returns (uint256 _earned, address _earnToken) {
        RewardPool memory pool = rewardPools[_rwPid];
        _earnToken = pool.earnToken;
        if (_earnToken == mmf) {
            _earned = IMmfRewardPool(pool.masterChef).pendingMeerkat(mmfRewardPoolId, address(this));
        } else {
            _earned = IMmfRewardPool(pool.masterChef).pendingReward(address(this));
        }
    }
    ...
    //TODO fix deposit functions => done
    ...
    // TODO fix bugs => done, bugs is due to wrong reward update operation
