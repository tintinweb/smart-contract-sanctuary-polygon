...
contract Pay is IPay, PayConfig {
    ...
    function pay(
        ...
        if(address(0) != settleCurrency) {
            ...
            paidAmount = _paiAmount;

            if (iMerchant.getAutoSettle(_merchant)) {
                fee = _autoWithdraw(_merchant, _currency, _paiAmount);
            } else {
                fee = getFee(paidAmount);
    ...
    function payWithETH(
        ...
        if (iMerchant.getAutoSettle(_merchant)) {
            if (WETH9 == settleCurrency) {
                fee = _autoWithdraw(_merchant, settleCurrency, msg.value);
            } else {
                fee = _autoWithdraw(_merchant, settleCurrency, _orderAmount);
            }
        } else {
    ...
    function _autoWithdraw(
        ...
        if(address(0) != _settleCurrency && WETH9 != _settleCurrency) {
