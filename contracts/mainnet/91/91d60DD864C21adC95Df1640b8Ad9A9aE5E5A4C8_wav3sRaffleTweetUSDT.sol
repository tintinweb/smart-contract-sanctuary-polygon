...
contract wav3sRaffleTweetUSDT is VRFConsumerBaseV2{
    ...
    function withdrawMirrorBudget(
        ...
        require(block.timestamp > postDataByPublicationIndex[nextId].raffleTime+1, "Funds can be withdrawn 1 hour after the raffle time is over");
        require(postDataByPublicationId[pubId].retwitters >= postDataByPublicationIndex[nextId].budget/postDataByPublicationIndex[nextId].reward, "Funds can be withdrawn if the participants are not enough");


        // Get the post budget and currency for the given post
        budget = postDataByPublicationIndex[nextId].budget;
