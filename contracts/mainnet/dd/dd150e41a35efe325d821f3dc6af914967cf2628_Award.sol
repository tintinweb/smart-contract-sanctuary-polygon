...
contract Award is Permission {
    ...
    function CreateAwardPools(string memory _appId, uint256 _id, IAwardData.AwardPool[] memory _pools, uint256 _preGas) public payable onlyAdmin(_appId) {
        ...
        for (uint i = 0; i < _pools.length; i++) {
            if (_pools[i].isNFT) {
                ...
                if (_pools[i].isETH) {
                    ethCount = ethCount.add(_pools[i].award_count);
                } else {
                    _pools[i].token.transferFrom(msg.sender, address(this), _pools[i].award_count);
    ...
    function ExcuteAward(string memory _appId, uint256 _id, address[][] memory _receivers) public payable {
        require(isOperator(_appId), "Only operator");
        ...
        for (uint i = 0; i < pools.length; i++) {
            require(_receivers[i].length <= pools[i].drop_count, "Receivers exceeded");
            if (pools[i].isNFT) {
                ...
                if (pools[i].isETH) {
                    for (uint j = 0; j < _receivers[i].length; j++) {
                        payable(_receivers[i][j]).transfer(pools[i].award_count.div(_receivers[i].length));
                    }
                } else {
                    for (uint j = 0; j < _receivers[i].length; j++) {
                        pools[i].token.transfer(_receivers[i][j], pools[i].award_count.div(_receivers[i].length));
                    }
                }
            }
        }
