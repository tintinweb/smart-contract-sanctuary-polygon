...
contract NFTCollection is ERC721, Ownable, ERC721Burnable, ERC721Enumerable, ERC2981Collection {
 ...
 mapping(uint256 => string) private _tokenURIs;
  ...
  constructor(address _marketplaceAddress) ERC721("GIBOOME Collections", "MARKETSTORE") {
  ...
  function revokeCreator(address account) public onlyOwner {    
     registCreators[account] = false;
  }

  function registCreator(address account) public onlyOwner {    
  ...
  function getTokensList() public view returns (uint256[] memory) {
     ...
     uint256 numberOfTokensListed = 0;

     for (uint256 i = 0; i < numberOfExistingTokens; i++) {
          uint256 tokenId = i + 1;
          if(_exists(tokenId) != true)  continue; 
          numberOfTokensListed += 1;
      }

     uint256[] memory allTokenIds = new uint256[](numberOfTokensListed);
