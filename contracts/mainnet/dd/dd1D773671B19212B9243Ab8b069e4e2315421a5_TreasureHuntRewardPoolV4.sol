...
contract TreasureHuntRewardPoolV4 is Initializable, OwnableUpgradeable {
    ...
    mapping(address => uint16) raidRoll;
    mapping(address => bool) public noFirstRaid;
    mapping(address => bool) isReleaser;

    uint8 public islandNum;
    mapping(uint8 => uint256) islandsReward;
    mapping(uint8 => uint16) winPercentage; //depend on island num
    mapping(address => bool) playerDoubleWin;
    mapping(address => bool) playerRaidState;
    mapping(address => uint8) playerWinIsland;
    mapping(address => uint256) playerWinFleet;

    mapping(address => uint256) public totalEarningOfSeaportOwner;
    mapping(address => uint256) public earningOnCurrentSeaport;

    mapping(address => uint8) public playerLevel;
    mapping(address => uint256) public playerExp;
    mapping(uint8 => uint32) public levelExp;
    mapping(uint8 => uint8) public bonus;
    ...
    address public treasureHuntStore;
    ...
    address public treasureHuntReferral;
    ...
    modifier onlyWinner() {
        require(playerRaidState[msg.sender], "You are not winner!");
        _;
    }
    ...
    modifier enoughFund(uint256 tokenID, uint8 islandNumber) {
        ...
        require(ITreasureHunt(treasureHuntFleet).getFleetInfo(tokenID).Rank >= getRankOfIsland(islandNumber), "Your fleet has low rank!");
        _;
    }

    modifier onSeaport(address player) {
        require(ITreasureHunt(treasureHuntSeaport).isOnSeaport(player), "You are not on seaport!");
        _;
    }

    event OccupyIsland(address player, uint8 islandNum, address seaportOwner);
    event ReleaseReward(address player);
    function initialize(address _treasureHuntFleet, address _treasureHuntSeaport, address _treasureHuntPicker) public initializer {
        __Ownable_init();
        islandNum = 50;
        ...
        devWallet = 0xcD0bb04F1821d519DC772a65a1f85159E57981b4;
    ...
    function initWinPercentage(
        uint16[] memory percent
    ) public onlyOwner{
        require(percent.length == 5, "Invalid Input!");        
        for (uint8 i; i < percent.length; i++) {
            winPercentage[i + 1] = percent[i];
        }
    }
    ...
    function initIslandRewards(uint16[] memory rewards) public onlyOwner {
        ...
        for(uint8 i; i < islandNum; i++) {
            islandsReward[i + 1] = rewards[i] * (10 ** decimal);
        }
    }
    ...
    function getRankOfIsland(uint8 _islandNum) public pure returns(uint8) {
        if (_islandNum % 10 == 0) {
            return _islandNum / 10;
        } else {
            return _islandNum / 10 + 1;
        }
    }

    function readRoll() public view returns(uint16) {
        return raidRoll[msg.sender];
    }
    ...
    function raidIsland(uint256 tokenID, uint8 islandNumber) public enoughFund(tokenID, islandNumber) onSeaport(msg.sender) {
        ...
        if (!noFirstRaid[msg.sender]) {
            noFirstRaid[msg.sender] = true;
        ...
        uint16 percentage = winPercentage[getRankOfIsland(islandNumber)];

        uint16 chance = uint16(random % 1000);
        raidRoll[msg.sender] = chance;

        uint256 fuel = islandsReward[islandNumber] * fuelPercent / 100;
        uint256 energy = islandsReward[islandNumber] * fuelPercent / 100;
        ITreasureHunt(treasureHuntFleet).updateFleetFund(tokenID, fuel, energy);

        if(chance <= percentage) {
            playerRaidState[msg.sender] = true;
            playerWinIsland[msg.sender] = islandNumber;
            playerWinFleet[msg.sender] = tokenID;
        } else {
            playerRaidState[msg.sender] = false;
        }
        uint256 repairCost = islandsReward[islandNumber] * repairPercent / 100;

        ITreasureHunt(treasureHuntFleet).setRepairCost(tokenID, repairCost);
    ...
    function repairFleet(uint256 tokenID, bool isDirect) public {
        ...
        uint256 repairCost = ITreasureHunt(treasureHuntFleet).getFleetInfo(tokenID).RepairCost;
        if (isDirect) {
            transferCost(repairCost, true);
        } else {
            payUsingReward(msg.sender, repairCost, true);
        }
        fleetRaidNum[tokenID] = 0;
        ITreasureHunt(treasureHuntFleet).setFleetDurability(tokenID, true);
        ITreasureHunt(treasureHuntFleet).resetRepairCost(tokenID);
    }
    ...
    function normalReward() public onlyWinner(){
        address acc = msg.sender;
        uint256 nReward = islandsReward[playerWinIsland[acc]] * (100 + bonus[playerLevel[acc]]) / 100; 
        playerRaidState[acc] = false;
        reward[acc] += nReward;
        totalReward[acc] += nReward;
        updateRaidReward(acc, nReward);
        address seaportOwner = ITreasureHunt(treasureHuntSeaport).getSeaportOwnerByPlayer(acc);
        emit OccupyIsland(acc, playerWinIsland[acc], seaportOwner);
    }

    function doubleReward() public onlyWinner(){
        address acc = msg.sender;
        ...
        ITreasureHunt(treasureHuntFleet).reduceDoubleOrNothingLifeCycle(playerWinFleet[acc], acc);
    }
    ...
    function payUsingReward(address acc, uint256 amt, bool isReal) public {
        uint256 realAmt = amt * (100 + rewardFeePercent) / 100;
        if (!isReal) {
            realAmt *= (10 ** decimal);
        } 
    ...
    function transferBurnReward(address acc, uint256 amt) public onlyReleaser(msg.sender){
        uint256 realAmt = amt * (10 ** decimal);
        reward[acc] += realAmt;
    }

    function transferCost(uint256 amt, bool isReal) public {
        uint256 realAmt = amt;
        if (!isReal) {
            realAmt = amt * (10 ** decimal);
        }
        require(IERC20(tokenUSDT).balanceOf(tx.origin) >= realAmt, "Insuffient USDC!");

        IERC20(tokenUSDT).transferFrom(tx.origin, treasureHuntStore, realAmt);
        devAmt += realAmt * rewardPercentForDev / 100;
    }
    ...
    function buyNFT(uint256 amt, address to) public onlyMarket {
        ...
        IERC20(tokenUSDT).transferFrom(tx.origin, treasureHuntStore, fee);
        IERC20(tokenUSDT).transferFrom(tx.origin, to, realAmt - fee);
    }

    function buySeaportFromAdmin(uint256 amt) public onlyMarket {
        uint256 realAmt = amt * (10 ** decimal);
        require(IERC20(tokenUSDT).balanceOf(tx.origin) >= realAmt, "Insuffient USDC!");

        uint256 fee = realAmt * rewardPercentForDev / 100;
        devAmt += fee;
        IERC20(tokenUSDT).transferFrom(tx.origin, treasureHuntStore, realAmt);
    }
    ...
    function updateRaidReward(address acc, uint256 amt) private onSeaport(acc){
        if (!ITreasureHunt(treasureHuntSeaport).isSeaportOwner(acc)) {
            address ownerOfSeaport = ITreasureHunt(treasureHuntSeaport).getSeaportOwnerByPlayer(acc);
            if (!seaportActivate || (seaportActivate && seaportPassiveIncome(ownerOfSeaport))) {
                uint256 seaportFee = amt * dailyPercentForSeaport / 100; 
                if (ownerOfSeaport == address(this)) {
                    devAmt += seaportFee;
                    totalEarningOfSeaportOwner[address(this)] += seaportFee;
                } else {
                    reward[ownerOfSeaport] += seaportFee;
                    totalReward[ownerOfSeaport] += seaportFee;
                    totalEarningOfSeaportOwner[ownerOfSeaport] += seaportFee;
                }
                earningOnCurrentSeaport[msg.sender] += seaportFee;
            } 
    ...
    function withdrawReward() public onSeaport(msg.sender) {
        require(block.timestamp >= timeToWithdraw[msg.sender], "Can't withdraw now!");
        require(reward[msg.sender] > 0, "No reward now!");
        timeToWithdraw[msg.sender] = block.timestamp + withdrawInterval;
        if(ITreasureHunt(treasureHuntSeaport).isSeaportOwner(msg.sender)) {
    ...
    function setWithdrawInterval(uint8 interval) public onlyOwner {
        withdrawInterval = interval * 1 days;
    }

    function resetTotalEarning(address acc) public onlyMarket {
        totalEarningOfSeaportOwner[acc] = 0;
    }

    function resetEarningOnSeaport() public {
        earningOnCurrentSeaport[tx.origin] = 0;
    }

    function readEarningOnSeaport(address acc) public view returns(uint256) {
        return earningOnCurrentSeaport[acc];
    }

    function readTotalEarningOfSeaportOwner(address acc) public view returns(uint256) {
        return totalEarningOfSeaportOwner[acc];
    }
    ...
    function setTreasureHuntStore(address _store) public onlyOwner {
        treasureHuntStore = _store;
    }
    ...
    function releaseFee() public onlyOwner {
        uint256 amt = devAmt * 37 / 100;
        IERC20(tokenUSDT).transfer(devWallet, devAmt - amt);
        IERC20(tokenUSDT).transfer(devWallet1, amt);
        devAmt = 0;
    }

    function getLastRaidResult(address acc) public view returns (bool, uint16, uint8, uint256){
        return (playerRaidState[acc], raidRoll[acc], playerWinIsland[acc], playerWinFleet[acc]);
    } 

    function readLastRaidTime(address acc) public view returns (uint256) {
        return lastRaidTime[acc];
    }

    function setTreasureHuntReferral(address referral) public onlyOwner {
        treasureHuntReferral = referral;
    }

    function referralReward(address codeOwner, uint256 price) public {
        require(msg.sender == treasureHuntReferral, "You are not Referral Contract!");
        reward[codeOwner] += price * (10 ** decimal);
    }

    function raidIslandV1(uint256 tokenID, uint8 islandNumber) public enoughFund(tokenID, islandNumber) onSeaport(msg.sender) {
        require( ITreasureHunt(treasureHuntFleet).canRaid(tokenID) , "You have to wait to raid!");
        if (!noFirstRaid[msg.sender]) {
            noFirstRaid[msg.sender] = true;
            ITreasureHunt(treasureHuntReferral).setReferral(msg.sender, true);
            timeToWithdraw[msg.sender] = block.timestamp + withdrawInterval;
        } else {
            ITreasureHunt(treasureHuntReferral).setReferral(msg.sender, false);
        } 
        ITreasureHunt(treasureHuntFleet).setFleetRaidTime(tokenID);
        uint256 random = ITreasureHunt(treasureHuntPicker).random(string(abi.encodePacked(tokenID)), islandNumber);
    ...
    function raidIslandV2(uint256 tokenID, uint8 islandNumber) public enoughFund(tokenID, islandNumber) onSeaport(msg.sender) {
        require( ITreasureHunt(treasureHuntFleet).canRaid(tokenID) , "You have to wait to raid!");
        require(ITreasureHunt(treasureHuntFleet).getFleetInfo(tokenID).Durability, "You should repair fleet before raid!");
        if (!noFirstRaid[msg.sender]) {
            noFirstRaid[msg.sender] = true;
            ITreasureHunt(treasureHuntReferral).setReferral(msg.sender, true);
            timeToWithdraw[msg.sender] = block.timestamp + withdrawInterval;
    ...
    function TotalEarningsBySeaport(uint256 tokenID) public view returns(uint256) {
        address[] memory players = ITreasureHunt(treasureHuntSeaport).playersOfSeaport(tokenID);
        uint256 amount;
        for(uint i; i < players.length; i++) {
            amount += reward[players[i]];
        }
        return amount;
    }
}
...
interface ITreasureHunt is ITreasureHuntTrait {
    ...
    function playersOfSeaport(uint256) external view returns(address[] memory);
    //referral
