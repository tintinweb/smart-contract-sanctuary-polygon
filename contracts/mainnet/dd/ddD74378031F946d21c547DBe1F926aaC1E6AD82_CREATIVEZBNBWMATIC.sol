...
interface External{    
    ...
    function deposit(uint256 _pid, uint256 _amount, address to) external; //for BNB-Matic _pid = 6
    function harvest(uint256 _pid, address to) external;
    function withdraw(uint256 _pid, uint256 _amount, address to) external; //to withdraw the LP from the staking contract. _pid is 6 amount is found with userInfo read function
...
contract CREATIVEZBNBWMATIC is Ownable{
      ...
      constructor(){
        ...
        External(BNBToken).approve(ApeRouter, approvalAmount); //approve BNBToken on ApeRouter
...
// Variables //
  ...
  address BNBToken = 0xA649325Aa7C5093d12D6F98EB4378deAe68CE23F;
  address BananaToken = 0x5d47bAbA0d66083C52009271faF3F50DCc01023C;
  address ApeLP = 0x0359001070cF696D5993E0697335157a6f7dB289;
...
// reinvestment functions //                                                                                    
    ...
    function withdrawFromStakingPool() external onlyOwner {
        (uint x,) = External(ApeSwap).userInfo(6, address(this));
        External(ApeSwap).withdraw(6, x, address(this));
    }

    //remove the Banana-WMATIC liqudity and seperate in BNB & Matic tokens
    function removeBNBWMATICLiquidity() external onlyOwner {
      External(ApeRouter).removeLiquidityETH(BNBToken, External(ApeLP).balanceOf(address(this)), 100, 100, address(this), block.timestamp+3000);      
    } 
    ...
    //swap BNB for MATIC on Apeswap
    function BNBToMatic() public payable onlyOwner {
                address[] memory path;
                path = new address[](2);
                path[0] = BNBToken;
                path[1] = WMATICToken;
                External(ApeRouter).swapExactTokensForETH(External(BNBToken).balanceOf(address(this)), 1, path, address(this), block.timestamp+1800); 
    }
    ...
    function returnBNBBalance() public view onlyOwner returns (uint256) {
      return External(BNBToken).balanceOf(address(this));
    }
...
////////////////////////////////////////////////////// End Reentrancy Guard //////////////////////////////////////////////////////
  ...
  function pushTheButton() public payable nonReentrant {
        ...
        start();          
          if (External(WMATICToken).balanceOf(address(this)) > threshold && I > II) { //change this balance - remember that 1 matic = 1 * 10 ** 18 - for now it is at a good value for testing
              //swap half WMATIC to BNB
              ...
              //add the WMATIC and BNB to WMATIC-BNB liquidity
              uint256 BNBBal = External(BNBToken).balanceOf(address(this));
              uint256 WMATICBal = External(WMATICToken).balanceOf(address(this));
              External(ApeRouter).addLiquidity(WMATICToken, BNBToken, WMATICBal, BNBBal, uint(WMATICBal*3/5), uint(BNBBal*3/5), address(this), block.timestamp+1800);              
              III++;
              _start -= (_timelocked-120);
              if(buttonPushTimer < External(CreativezButtonPushRewardsAddrs).deployerTimeStamp()){
                delete pushAddrs;
              ...
              External(ApeSwap).deposit(6, External(ApeLP).balanceOf(address(this)), address(this)); 
              I++;
              IV++;
              _start -= (_timelocked-120);
              if(buttonPushTimer < External(CreativezButtonPushRewardsAddrs).deployerTimeStamp()){
              ...
              External(ApeSwap).harvest(6, address(this)); 
              b++; 
                emit BananaClaim(
