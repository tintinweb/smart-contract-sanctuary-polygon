...
//Note Assumes that all swap actions can happen in one swap
/**
 * @dev used to convert up to 4 input tokens for 4 output tokens, useful for DCAing
 */
 ...
 /*     
        ...
        Token B->[              ]->Token F
        Token C->[              ]->Token G
        Token D->[______________]->Token H

Note: Token[A,B,C,D] can all be the same token
Note: if Token B is 0x0000....0000 then Token B' needs to be 0x0000....0000, same with all other Tokens
Note if the input token[i] == output token[i], no swap is made and the amount is carried through 
Note Assumes there is a valid swap pair between the input and outputs

*/
contract MultiSwapAction is BaseSiloAction{
    ...
    function enter(address implementation, bytes memory configuration, bytes memory inputData) public override returns(uint[4] memory outputAmounts){
        ...
        (input, output, exchangeRateOracle) = abi.decode(configuration, (address[4],address[4],address));
        IExchangeRateOracle oracle = IExchangeRateOracle(exchangeRateOracle);
        uint[] memory amounts = new uint[](2);
        uint[4] memory inputAmount = abi.decode(inputData, (uint[4]));
        Quote memory bestQuote;
        for(uint i=0; i<4; i++){
            if(input[i] == output[i]){
                outputAmounts[i] = inputAmount[i];//carry through the input amount
                continue;
            }
            else{
                path[0] = input[i];
                path[1] = output[i];
                if(inputAmount[i] > 0){
                    bestQuote = oracle.findOptimalSwap(path[0], path[1], inputAmount[i]);
                    if(bestQuote.amountOut > 0){//trade at best quote if amount is greater than zero
                        //aprove set to zero https://docs.google.com/document/d/1YLPtQxZu1UAvO9cZ1O2RPXBbT0mooh4DYKjA_jp-RLM/edit
                        //https://etherscan.io/address/0xdac17f958d2ee523a2206206994597c13d831ec7#code
                        SafeERC20.safeApprove(IERC20(path[0]), bestQuote.router, 0);
                        SafeERC20.safeApprove(IERC20(path[0]), bestQuote.router, inputAmount[i]);
                        amounts = IUniswapV2Router02(bestQuote.router).swapExactTokensForTokens(
                            inputAmount[i],
                            bestQuote.amountOut,
                            path,
                            address(this),
    ...
    function exit(address implementation, bytes memory configuration, bytes memory outputData) public override returns(uint[4] memory outputAmounts){
        ...
        IExchangeRateOracle oracle = IExchangeRateOracle(exchangeRateOracle);
        uint[] memory amounts = new uint[](2);
        uint[4] memory inputAmount = abi.decode(outputData, (uint[4]));
        Quote memory bestQuote;
        for(uint i=0; i<4; i++){
            if(input[i] == output[i]){
                outputAmounts[i] = inputAmount[i];//carry through the input amount
    ...
    function createConfig(address[4] memory _inputs, address[4] memory _outputs, address _exchangeRateOracle) public pure returns(bytes memory configData){
        configData = abi.encode(_inputs, _outputs, _exchangeRateOracle);
    }
    ...
    function validateConfig(bytes memory configData) public view override returns(bool){
        ...
        address exchangeRateOracle;
        address[4] memory output;
        (input, output, exchangeRateOracle) = abi.decode(configData, (address[4],address[4],address));
        IExchangeRateOracle oracle = IExchangeRateOracle(exchangeRateOracle);
        address[] memory exchanges;
        for (uint i=0; i<4;i++){
            if(output[i] != address(0) && input[i] != output[i]){
                exchanges = oracle.getExchanges(input[i], output[i]);
                for(uint j=0; j<exchanges.length; j++){
                    if(exchanges[j] != address(0)){
                        break;
                    }
                    if(j == exchanges.length-1){//no valid exchange found
                        return false;
                    }
                }
            }
