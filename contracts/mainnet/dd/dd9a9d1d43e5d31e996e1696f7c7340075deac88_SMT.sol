...
contract SMT {
    ...
    uint256 private constant referDepth = 100000;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _defaultLeaders.length; i++) {
            if(i < 2){feeReceivers.push(_defaultLeaders[i]);}
            else if(i == 2){smtToken = IERC20(_defaultLeaders[i]);}
            /*else if(i > 2 && i < 27) { setleaders(_defaultLeaders[i])) }*/
    ...
    function _deposit(
        ...
        if(_amount == 4030e18 && nodeUsers.length < 500 && userInfo[_userAddr].tjDeposit >= 5000e18 && userInfo[_userAddr].tjnum >= 5){
    ...
    function _updateUplineReward(address _userAddr, uint256 _amount) private {
        ...
        for (uint256 i = 0; i < 15; i++) {  //referDepth
            if (upline != address(0)) {
