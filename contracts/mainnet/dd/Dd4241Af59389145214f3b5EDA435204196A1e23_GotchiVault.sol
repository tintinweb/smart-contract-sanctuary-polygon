   ...
   /* function spendSkillPoints(uint256 _tokenId, int16[4] calldata _values) public whenNotPaused{ 
    ...
    function depositERC721(address _tokenAddress, uint256[] calldata _tokenId) public whenNotPaused {
        ...
        if(numFee > 0 && s.fee721 > 0){
            ...
            IERC20(im_ghstAddress).transfer(s.contractOwner, feeAmount.mul(100).div(400));
            IERC20(im_ghstAddress).transfer(s.contractCreator, feeAmount.mul(100).div(400));
            IERC20(im_ghstAddress).transfer(s.vGHSTAddress, feeAmount.mul(200).div(400));
        }
    ...
    function setRentalSettings(
        ...
        IERC20(im_ghstAddress).transferFrom(msg.sender, address(this), s.customSettingsFee.mul(_tokenIds.length));
        s.totalFeesCollected += s.customSettingsFee.mul(_tokenIds.length);
    }
    ...
    function withdrawProfits(address[] calldata _revenueTokens) public onlyApproved{
        ...
        for(uint256 i = 0; i < _revenueTokens.length; i++){
            ...
            //only for GHST do we want to send 10% to the gas multisig
            if(_revenueTokens[i] == im_ghstAddress){
                token.transfer(0xCc05eE5c2526A61f01eEe705b37FF78c95446521,balance.mul(100).div(1000));
