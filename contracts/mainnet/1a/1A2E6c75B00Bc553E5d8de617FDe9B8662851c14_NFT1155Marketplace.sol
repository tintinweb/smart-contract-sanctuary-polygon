...
contract NFT1155Marketplace is ERC1155 {
    ...
    function mint(string memory tokenURI, uint256 amount) public payable returns (uint) {
        _tokenIds.increment();
        uint256 newTokenId = _tokenIds.current();

//        _owner = payable(msg.sender);
        _mint(_owner, newTokenId, amount, "");
        _uriMapping[newTokenId] = tokenURI;
        return newTokenId;
    }
    ...
    function buy(BuyRequest[] memory requestList) public payable {
        for (uint i=0; i < requestList.length; i++) {
            _safeTransferFrom(_owner, requestList[i].toAddress, requestList[i].tokenId, requestList[i].amount, "");
        }
    }
