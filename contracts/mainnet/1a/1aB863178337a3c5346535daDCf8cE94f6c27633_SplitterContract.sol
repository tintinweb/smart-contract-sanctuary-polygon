...
contract SplitterContract is OwnableUpgradeable {
    ...
    /// @notice RedKite Main Marketplace
    ...
    /// @notice Swap path for the uniswapV2 router interaction
    address[] public path;

    /// @notice 90% always sent to the token seller 
    uint256 public constant SELLER_SHARE = 90;

    /// @notice Other privileged callers
    mapping(address => bool) public otherMarkets;


    modifier onlyAdmin() {
        require(
            msg.sender == admin || tx.origin == admin,
    ...
    /// @notice Distribution shares structure
    struct ArtistDistribution {
        address[] addresses;
        uint256[] shares;
    }

    /// @notice primary and secondary distribution data
     ...
     * @dev Add new market
     */
    function addMarket(address newMarket) external onlyAdmin {
        require(newMarket != address(0), "wrong address");
    ...
    function setPath(address[] calldata _path) external onlyOwner {
        ...
        require(_path.length == 2, "wrong path length");
        path = _path;
