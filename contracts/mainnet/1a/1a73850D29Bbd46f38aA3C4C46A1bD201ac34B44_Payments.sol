...
// SPDX-License-Identifier: UNLICENSED
// Smart Contract v1.0.0 - (May 9, 2023).
...
import "../Lending/IVaultLendersRewards.sol";
...
contract Payments is ReentrancyGuard, AccessControl, IMortgageInterest {
    ...
    address internal relayAddress = 0x858e4C659f2A23D944304365249215eAc5b60433;
    address public walletPanoram;

    IMortgageControl private mortgageControl;
    TokenInfo public tokenInfo;
    uint16 private percentagePanoram = 0; // 1000;  0 % para Panoram por el momento, todo se va a lender rewards para pagar intereses.
    uint16 private percentagePool = 10000; // 9000=90% , 100 % para Lender Rewards Para pagar los intereses por el momento.
    uint16 private percentagePanoramDelayedPay = 0; // 7000;  0 % para Panoram por el momento, todo se va a lender rewards para pagar intereses.
    uint16 private percentagePoolDelayedPay = 10000;  //3000=30% , 100 % para Lender Rewards Para pagar los intereses por el momento.
    ...
    function liquidationDebt(uint256 _IdMortgage, address _token) external validToken(_token) isPaused nonReentrant {
        ...
        if (delayed) {
            ...
            IVaultLendersRewards(vaultRewardsLenders).deposit(mortgage.amountToPoolDelayed, _token);
            IERC20(_token).safeTransfer(walletPanoram, mortgage.amountToPanoramDelayed);
        } else {
            IVaultLenders(vaultLenders).depositCapital(mortgage.totalDebt, _token);
            IVaultLendersRewards(vaultRewardsLenders).deposit(mortgage.amountToPool, _token);
        ...
        nonReentrant
    {
        ...
        IVaultLendersRewards(VaultLendersRewards).deposit(payForLenderRewards, _token);
    ...
    function handlePayment(
        ...
        if (delayedPayment) {
            ...
            IVaultLendersRewards(vaultRewardsLenders).deposit(mortgageInt.totalPoolLiquidation, _token);
            IERC20(_token).safeTransfer(walletPanoram, mortgageInt.totalPanoramLiquidation);
        } else {
            IVaultLenders(vaultLenders).depositCapital(mortgageInt.amountToVault, _token);
            IVaultLendersRewards(vaultRewardsLenders).deposit(mortgageInt.amountToPool, _token);
