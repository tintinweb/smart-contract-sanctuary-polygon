...
contract NFTMintSaleMultiple is Ownable {
    ...
    constructor (address masterNFT_) {
        masterNFT = masterNFT_;
    }

    function initialize(string memory symbol, string memory name, string memory baseURI, address royaltyReceiver_, uint256 royaltyRate_, uint32 beginTime_, uint32 endTime_, TierInfo[] calldata tiers_, IERC20 paymentToken_) external {
        require(nft == VibeERC721(address(0)), "Already initialized");

        _transferOwnership(msg.sender);

        address proxy = Clones.clone(masterNFT);
        VibeERC721(proxy).initialize(name, symbol, baseURI);

        nft = VibeERC721(proxy);
        nft.setRoyalty(royaltyReceiver_, royaltyRate_);
    ...
    function removeTokensAndReclaimOwnership(address proceedRecipient) external onlyOwner {
        ...
        paymentToken.safeTransfer(proceedRecipient, paymentToken.balanceOf(address(this)));
...
contract VibeERC721 is Ownable, ERC721 {
    using Strings for uint256;
    event LogSetRoyalty(uint256 royaltyRate, address indexed royaltyReceiver_);
    ...
    function initialize(string memory _name, string memory _symbol, string memory baseURI_) public {
    ...
    function burn(uint256 id) external {
        ...
        emit Transfer(oldOwner, address(0), id);
    }
    ...
    function royaltyInfo(
        ...
        return (royaltyReceiver, _salePrice * royaltyRate / 1e18);
    }

    function setRoyalty(address royaltyReceiver_, uint256 royaltyRate_) external onlyOwner {
        require(royaltyReceiver_ != address(0) && royaltyReceiver_ != address(this));
