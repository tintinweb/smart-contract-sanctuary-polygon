...
// @author Wivern for Dopepool.Finance
...
contract ZapUniswapV2 {
    ...
    IUniswapRouterETH public immutable router;
    ...
    function beefInETH(
        ...
        require(
            ...
            "Dopepool: Insignificant input amount"
        );
    ...
    function beefIn(
        ...
        require(
            IERC20(tokenIn).allowance(msg.sender, address(this)) >=
                tokenInAmount,
            "Dopepool: Input token is not approved"
        );
    ...
    function beefOut(address beefyVault, uint256 withdrawAmount) external {
        (IVault vault, IUniswapV2Pair pair) = _getVaultPair(beefyVault);
    ...
    function beefOutAndSwap(
        ...
        require(
            ...
            "Dopepool: desired token not present in liquidity pair"
        );
    ...
    function _getVaultPair(
        address beefyVault
    ) private view returns (IVault vault, IUniswapV2Pair pair) {
        vault = IVault(beefyVault);
        ...
        require(
            ...
            "Dopepool: Incompatible liquidity pair factory"
        );
    }
    ...
    function _swapAndStake(
        ...
        require(
            ...
            "Dopepool: Liquidity pair reserves too low"
        );

        bool isInputA = pair.token0() == tokenIn;
        require(
            isInputA || pair.token1() == tokenIn,
            "Dopepool: Input token not present in liquidity pair"
        );
        ...
        for (uint256 i; i < tokens.length; i++) {
            ...
            if (balance > 0) {
                if (tokens[i] == WETH) {
                    ...
                    require(success, "Dopepool: ETH transfer failed");
                } else {
