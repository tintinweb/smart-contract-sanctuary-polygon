...
contract ERC20 is Context, IERC20 {
    ...
    string private _name = "AmorToken";
...
interface IController {
    function addSet(address _AmorToken) external;
    function feeRecipient() external view returns(address);
    function getModuleFee(address _module, uint256 _feeType) external view returns(uint256);
    function isModule(address _module) external view returns(bool);
    function isSet(address _AmorToken) external view returns(bool);
...
interface IModule {
    /**
     * Called by a AmorToken to notify that this module was removed from the Set token. Any logic can be included
...
// File contracts/interfaces/IAmorToken.sol
 ...
 * @author Set Protocol
 *
 * Interface for operating with AmorTokens.
 */
interface IAmorToken is IERC20 {
    ...
    /* ============ Structs ============ */
    /**
     * The base definition of a AmorToken Position
     *
     * @param component           Address of token in the Position
     * @param module              If not in default state, the address of associated module
     * @param unit                Each unit is the # of components per 10^18 of a AmorToken
 ...
 * @author Set Protocol
 *
 * Collection of helper functions for handling and updating AmorToken Positions
 *
 * CHANGELOG:
...
library Position {
    ...
    /**
     * Returns whether the AmorToken has a default position for a given component (if the real unit is > 0)
     */
    function hasDefaultPosition(IAmorToken _AmorToken, address _component) internal view returns(bool) {
        return _AmorToken.getDefaultPositionRealUnit(_component) > 0;
    }

    /**
     * Returns whether the AmorToken has an external position for a given component (if # of position modules is > 0)
     */
    function hasExternalPosition(IAmorToken _AmorToken, address _component) internal view returns(bool) {
        return _AmorToken.getExternalPositionModules(_component).length > 0;
    }
    
    /**
     * Returns whether the AmorToken component default position real unit is greater than or equal to units passed in.
     */
    function hasSufficientDefaultUnits(IAmorToken _AmorToken, address _component, uint256 _unit) internal view returns(bool) {
        return _AmorToken.getDefaultPositionRealUnit(_component) >= _unit.toInt256();
    }

    /**
     * Returns whether the AmorToken component external position is greater than or equal to the real units passed in.
     */
    function hasSufficientExternalUnits(
        IAmorToken _AmorToken,
        address _component,
        ...
        returns(bool)
    {
       return _AmorToken.getExternalPositionRealUnit(_component, _positionModule) >= _unit.toInt256();    
    }

    /**
     * If the position does not exist, create a new Position and add to the AmorToken. If it already exists,
     * then set the position units. If the new units is 0, remove the position. Handles adding/removing of 
     * components where needed (in light of potential external positions).
     *
     * @param _AmorToken           Address of AmorToken being modified
     * @param _component          Address of the component
     * @param _newUnit            Quantity of Position units - must be >= 0
     */
    function editDefaultPosition(IAmorToken _AmorToken, address _component, uint256 _newUnit) internal {
        bool isPositionFound = hasDefaultPosition(_AmorToken, _component);
        if (!isPositionFound && _newUnit > 0) {
            // If there is no Default Position and no External Modules, then component does not exist
            if (!hasExternalPosition(_AmorToken, _component)) {
                _AmorToken.addComponent(_component);
            }
        } else if (isPositionFound && _newUnit == 0) {
            // If there is a Default Position and no external positions, remove the component
            if (!hasExternalPosition(_AmorToken, _component)) {
                _AmorToken.removeComponent(_component);
            }
        }

        _AmorToken.editDefaultPositionUnit(_component, _newUnit.toInt256());
    }
     ...
     *    external position.
     *
     * @param _AmorToken         AmorToken being updated
    ...
    {
        if (_newUnit != 0) {
            if (!_AmorToken.isComponent(_component)) {
                _AmorToken.addComponent(_component);
                _AmorToken.addExternalPositionModule(_component, _module);
            } else if (!_AmorToken.isExternalPositionModule(_component, _module)) {
                _AmorToken.addExternalPositionModule(_component, _module);
            }
            _AmorToken.editExternalPositionUnit(_component, _module, _newUnit);
            _AmorToken.editExternalPositionData(_component, _module, _data);
        } else {
            require(_data.length == 0, "Passed data must be null");
            // If no default or external position remaining then remove component from components array
            if (_AmorToken.getExternalPositionRealUnit(_component, _module) != 0) {
                address[] memory positionModules = _AmorToken.getExternalPositionModules(_component);
                if (_AmorToken.getDefaultPositionRealUnit(_component) == 0 && positionModules.length == 1) {
                    require(positionModules[0] == _module, "External positions must be 0 to remove component");
                    _AmorToken.removeComponent(_component);
                }
                _AmorToken.removeExternalPositionModule(_component, _module);
            }
        }
    }
     ...
     *
     * @param _AmorTokenSupply     Supply of AmorToken in precise units (10^18)
     * @param _positionUnit       Quantity of Position units
     *
     * @return                    Total notional amount of units
     */
    function getDefaultTotalNotional(uint256 _AmorTokenSupply, uint256 _positionUnit) internal pure returns (uint256) {
        return _AmorTokenSupply.preciseMul(_positionUnit);
    }
     ...
     */
    function getDefaultPositionUnit(uint256 _AmorTokenSupply, uint256 _totalNotional) internal pure returns (uint256) {
        return _totalNotional.preciseDiv(_AmorTokenSupply);
    }

    /**
     * Get the total tracked balance - total supply * position unit
     *
     * @param _AmorToken           Address of the AmorToken
     * @param _component          Address of the component
     * @return                    Notional tracked balance
     */
    function getDefaultTrackedBalance(IAmorToken _AmorToken, address _component) internal view returns(uint256) {
        int256 positionUnit = _AmorToken.getDefaultPositionRealUnit(_component); 
        return _AmorToken.totalSupply().preciseMul(positionUnit.toUint256());
    }
     ...
     * @param _AmorToken                 Address of the AmorToken
     * @param _component                Address of the component
     * @param _setTotalSupply           Current AmorToken supply
    ...
    {
        uint256 currentBalance = IERC20(_component).balanceOf(address(_AmorToken));
        uint256 positionUnit = _AmorToken.getDefaultPositionRealUnit(_component).toUint256();
        ...
        editDefaultPosition(_AmorToken, _component, newTokenUnit);

        return (currentBalance, positionUnit, newTokenUnit);
    }

    /**
     * Calculate the new position unit given total notional values pre and post executing an action that changes AmorToken state
     * The intention is to make updates to the units without accidentally picking up airdropped assets as well.
     *
     * @param _AmorTokenSupply     Supply of AmorToken in precise units (10^18)
     * @param _preTotalNotional   Total notional amount of component prior to executing action
     * @param _postTotalNotional  Total notional amount of component after the executing action
     * @param _prePositionUnit    Position unit of AmorToken prior to executing action
     * @return                    New position unit
     */
    function calculateDefaultEditPositionUnit(
        uint256 _AmorTokenSupply,
        ...
        returns (uint256)
    {
        ...
        uint256 airdroppedAmount = _preTotalNotional.sub(_prePositionUnit.preciseMul(_AmorTokenSupply));
        return _postTotalNotional.sub(airdroppedAmount).preciseDiv(_AmorTokenSupply);
    }
}
...
// File contracts/protocol/AmorToken.sol
...
contract AmorToken is ERC20 {
    ...
    /*
        The PositionState is the status of the Position, whether it is Default (held on the AmorToken)
    ...
    */
    ...
    /**
     * Throws if the sender is not a AmorToken's module or module not enabled
     */
    modifier onlyModule() {
    ...
    /**
     * Throws if the sender is not the AmorToken's manager
     */
    ...
    /**
     * Throws if AmorToken is locked and called by any account other than the locker.
     */
    ...
    // List of initialized Modules; Modules extend the functionality of AmorTokens
    ...
    mapping(address => IAmorToken.ModuleState) public moduleStates;
    ...
    mapping(address => IAmorToken.ComponentPosition) private componentPositions;
    ...
    /**
     * When a new AmorToken is created, initializes Positions in default state and adds modules into pending state.
     * All parameter validations are on the AmorTokenCreator contract. Validations are performed already on the 
     * AmorTokenCreator. Initiates the positionMultiplier as 1e18 (no adjustments).
     *
     * @param _components             List of addresses of components for initial Positions
     * @param _units                  List of units. Each unit is the # of components per 10^18 of a AmorToken
     * @param _modules                List of modules to enable. All modules must be approved by the Controller
     * @param _controller             Address of the controller
     * @param _manager                Address of the manager
     * @param _name                   Name of the AmorToken
     * @param _symbol                 Symbol of the AmorToken
     */
    constructor(
        ...
        ERC20(_name, _symbol)
    {
        ...
        for (uint256 i = 0; i < _modules.length; i++) {
            moduleStates[_modules[i]] = IAmorToken.ModuleState.PENDING;
        }
    ...
    /**
     * PRIVELEGED MODULE FUNCTION. When a AmorToken is locked, only the locker can call privileged functions.
     */
    ...
    /**
     * PRIVELEGED MODULE FUNCTION. Unlocks the AmorToken and clears the locker
     */
    ...
    function addModule(address _module) external onlyManager {
        require(moduleStates[_module] == IAmorToken.ModuleState.NONE, "Module must not be added");
        require(controller.isModule(_module), "Must be enabled on Controller");

        moduleStates[_module] = IAmorToken.ModuleState.PENDING;

        emit ModuleAdded(_module);
    }

    /**
     * MANAGER ONLY. Removes a module from the AmorToken. AmorToken calls removeModule on module itself to confirm
    ...
    function removeModule(address _module) external onlyManager {
        ...
        require(moduleStates[_module] == IAmorToken.ModuleState.INITIALIZED, "Module must be added");

        IModule(_module).removeModule();

        moduleStates[_module] = IAmorToken.ModuleState.NONE;
    ...
    /**
     * MANAGER ONLY. Removes a pending module from the AmorToken.
     */
    function removePendingModule(address _module) external onlyManager {
        require(!isLocked, "Only when unlocked");
        require(moduleStates[_module] == IAmorToken.ModuleState.PENDING, "Module must be pending");
    ...
    function initializeModule() external {
        require(!isLocked, "Only when unlocked");
        require(moduleStates[msg.sender] == IAmorToken.ModuleState.PENDING, "Module must be pending");
        
        moduleStates[msg.sender] = IAmorToken.ModuleState.INITIALIZED;
    ...
    /**
     * MANAGER ONLY. Changes manager; We allow null addresses in case the manager wishes to wind down the AmorToken.
     ...
     */
    function isInitializedModule(address _module) external view returns (bool) {
        return moduleStates[_module] == IAmorToken.ModuleState.INITIALIZED;
    }
     ...
     */
    function isPendingModule(address _module) external view returns (bool) {
        return moduleStates[_module] == IAmorToken.ModuleState.PENDING;
    }
     ...
     */
    function getPositions() external view returns (IAmorToken.Position[] memory) {
        IAmorToken.Position[] memory positions = new IAmorToken.Position[](_getPositionCount());
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            if (_defaultPositionVirtualUnit(component) > 0) {
                positions[positionCount] = IAmorToken.Position({
                    component: component,
                    module: address(0),
     ...
     *
     * Module must be initialized on the AmorToken and enabled by the controller
     */
