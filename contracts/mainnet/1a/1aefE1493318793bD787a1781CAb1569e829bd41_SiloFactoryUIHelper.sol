...
contract SiloFactoryUIHelper is Ownable {
    ...
    function setCategoryInputTypes(uint256 _category, string[5] memory _types)
        external
        onlyOwner
    {
    ...
    function getSiloInputAndOutput(uint256 siloId)
        public
        view
        returns (address[5] memory input, address[5] memory output)
    {
    ...
    function getExtraSiloInfo(uint256 siloId)
        external
        view
        returns (
            bool isLimit,
    ...
    function getVaultInfo(uint256 siloId)
        external
        view
        returns (
            uint256 vaultId,
        ...
        )
    {
        ...
        for (uint256 i; i < actions.length; ) {
            action = IAction(actions[i]);
            (vaultId,collateral, debt, cdr, debtCeil) = action.vaultInfo(
                address(silo),
                configData[i]
            );
            if(vaultId > 0 ){
                return (vaultId,collateral, debt, cdr, debtCeil);
            }
            unchecked {
                i++;
            }
    ...
    function getSiloTokenMinimums(uint256 siloId)
        public
        view
