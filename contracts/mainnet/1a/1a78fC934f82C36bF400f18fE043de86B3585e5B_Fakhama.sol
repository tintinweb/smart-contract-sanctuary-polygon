...
contract Fakhama is IERC20
{
    ...
    mapping(uint=>uint) DirectPercentage;
...
uint _referId = map_Users[memberId].SponsorId;
        while(_spId>0)
        {
            if(level==1) map_Users[_spId].Business+=amount;
            ...
            if(level>=1 && level<=16)
            {
                if(currentRank>=level)
                {
                    ...
                    if(_referId == _spId)
                    {
                    _levelIncome = (amount*DirectPercentage[currentRank])/(100*100);
                    }
                    else
    ...
    constructor(address)
    {
        ...
        DirectPercentage[1] = 900;
        DirectPercentage[2] = 1200;
        DirectPercentage[3] = 1400;
        DirectPercentage[4] = 1550;
        DirectPercentage[5] = 1700;
        DirectPercentage[6] = 1825;
        DirectPercentage[7] = 1950;
        DirectPercentage[8] = 2050;
        DirectPercentage[9] = 2150;
        DirectPercentage[10] = 2250;
        DirectPercentage[11] = 2350;
        DirectPercentage[12] = 2400;
        DirectPercentage[13] = 2450;
        DirectPercentage[14] = 2500;
        DirectPercentage[15] = 2600;
        DirectPercentage[16] = 2700;
