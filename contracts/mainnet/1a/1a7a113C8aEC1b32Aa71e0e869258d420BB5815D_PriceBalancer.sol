...
contract PriceBalancer is Ownable {
    ...
    function execute() external onlyOwners {
        ...
        bool sold;
        bool updated;
        uint256 balanceDiff = usdcBalanceRef >= limit
            ? usdcBalanceRef - limit
            : 0;

        if (
            currentPrice <= buyPrice &&
            IERC20(usdcAddress).balanceOf(address(this)) >= balanceDiff
        ) bought = _buyTokens();
        if (currentPrice >= sellPrice) {
            sold = _sellTokens();
        }
        updated = _updateBalanceRef(
    ...
    modifier withdrawable() {
        require(
            ...
            "Withdrawable: tokens are still vested"
        );
        _;
    }
}
