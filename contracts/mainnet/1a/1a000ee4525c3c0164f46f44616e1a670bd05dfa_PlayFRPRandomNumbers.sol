...
contract PlayFRPRandomNumbers is Ownable
{
    ...
    event WheelGameSucceeded(uint256 indexed requestId, uint256 result);
    event RouletteGameSucceeded(uint256 indexed requestId, uint256 result);
    event GamesSucceeded(uint256 indexed requestId, uint256[] result);
    //
    ...
    function fulfillRandomWords(
        ...
        //WheelGameNumber
        vrfRequests[requestId].randomWord = randomWords;

        emit GamesSucceeded(requestId, randomWords);

        //emit WheelGameSucceeded(requestId, vrfRequests[requestId].randomWord[0]);
        //emit RouletteGameSucceeded(requestId, vrfRequests[requestId].randomWord[1]);
        //emit CardGameSucceeded(requestId, cardGame);
    }
    //
