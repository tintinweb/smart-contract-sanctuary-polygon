...
import {ITimeswapV2PeripheryCollectTransactionFees} from "@timeswap-labs/v2-periphery/contracts/interfaces/ITimeswapV2PeripheryCollectTransactionFees.sol";

import {INativeWithdraws} from "./INativeWithdraws.sol";
import {IMulticall} from "./IMulticall.sol";

import {TimeswapV2PeripheryUniswapV3CollectTransactionFeesParam} from "../structs/Param.sol";
...
interface ITimeswapV2PeripheryUniswapV3CollectTransactionFees is
  ITimeswapV2PeripheryCollectTransactionFees,
  ...
  IMulticall
{
  ...
  function collectTransactionFees(
    TimeswapV2PeripheryUniswapV3CollectTransactionFeesParam calldata param
  )
    external
...
import {TimeswapV2PeripheryCollectTransactionFees} from "@timeswap-labs/v2-periphery/contracts/TimeswapV2PeripheryCollectTransactionFees.sol";
...
import {ITimeswapV2PeripheryUniswapV3CollectTransactionFees} from "./interfaces/ITimeswapV2PeripheryUniswapV3CollectTransactionFees.sol";
...
/// @title Capable of collecting transaction fees from a Timeswap V2 Protocol
/// @author Timeswap Labs
contract TimeswapV2PeripheryUniswapV3CollectTransactionFees is
  ITimeswapV2PeripheryUniswapV3CollectTransactionFees,
  TimeswapV2PeripheryCollectTransactionFees,
  ...
  Multicall
{
  ...
  /// @inheritdoc ITimeswapV2PeripheryUniswapV3CollectTransactionFees
  function collectTransactionFees(
    TimeswapV2PeripheryUniswapV3CollectTransactionFeesParam calldata param
  )
    external
    returns (uint256 tokenAmount, uint256 excessLong0Amount, uint256 excessLong1Amount, uint256 excessShortAmount)
  {
    ...
    CacheFees memory cache;
    (cache.long0Fees, cache.long1Fees, cache.shortFees) = ITimeswapV2LiquidityToken(liquidityTokens).feesEarnedOf(
      msg.sender,
    ...
    ITimeswapV2LiquidityToken(liquidityTokens).transferFeesFrom(
      ...
      param.long0Requested.min(cache.long0Fees),
      param.long1Requested.min(cache.long1Fees),
      param.shortRequested.min(cache.shortFees)
    );
...
interface ITimeswapV2PeripheryCollectTransactionFees is ITimeswapV2TokenMintCallback {
...
import {ITimeswapV2PeripheryCollectTransactionFees} from "./interfaces/ITimeswapV2PeripheryCollectTransactionFees.sol";
...
/// @title Abstract contract which specifies functions that are required for  collect transaction fees which are to be inherited for a specific DEX/Aggregator implementation

abstract contract TimeswapV2PeripheryCollectTransactionFees is ITimeswapV2PeripheryCollectTransactionFees {
  using Math for uint256;
  /* ===== MODEL ===== */
  /// @inheritdoc ITimeswapV2PeripheryCollectTransactionFees
  ...
  /// @notice the abstract implementation for collect transaction fees function
  /// @param param params for  collectTransactionFees as mentioned in the TimeswapV2PeripheryCollectTransactionFeesParam struct
  ...
  {
    if (param.maturity <= block.timestamp) Error.alreadyMatured(param.maturity, uint96(block.timestamp));
    ...
    ITimeswapV2LiquidityToken(liquidityTokens).collect(
      TimeswapV2LiquidityTokenCollectParam({
        ...
        maturity: param.maturity,
        to: address(this),
        long0FeesDesired: param.long0Requested,
        long1FeesDesired: param.long1Requested,
    ...
    uint256 shortAmount;
    (token0Amount, token1Amount, shortAmount) = ITimeswapV2Pool(poolPair).collectTransactionFees(
    ...
    if (shortAmount >= longAmount) {
      ...
      if (token0Amount != 0 && token1Amount != 0) {
        (, , shortAmountBurnt, ) = ITimeswapV2Option(optionPair).burn(
          TimeswapV2OptionBurnParam({
            strike: param.strike,
            maturity: param.maturity,
            token0To: param.token0To,
  ...
  /// @notice the implementation which is to be overriden for DEX/Aggregator specific logic for TimeswapV2CollectTransactionFeesExcessLongChoice
  /// @param param params for calling the implementation specfic collectTransactionFeesExcessLongChoice to be overriden
