...
/**
 * @title DForce Strategy
 * @notice Investment strategy for investing stablecoins via DForce Strategy
 */
...
import "../interfaces/dForce/IGauge.sol";
import "../interfaces/dForce/ILendingToken.sol";
...
contract DForceSingleStrategy is InitializableAbstractStrategy {
    using StableMath for uint256;
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    IERC20 public token0;
    ILendingToken public _iToken;

    IGauge public gauge;

    function initialize(
        address _platformAddress, // provider
        address _vaultAddress, // VaultProxy address
        address[] calldata _rewardTokenAddresses, // Reward tokens
        address[] calldata _assets, // assets
        address[] calldata _pTokens, // pool tokens
        // address _router, // uniswap router
        address _gauge
        ...
        _iToken = ILendingToken(_pTokens[0]);
    ...
    function _deposit(uint256 _amount) internal {
        _unwrap(_amount);
        _iToken.mint{ value: address(this).balance }(address(this));
    }

    /**
     * @dev function to unwrap to wToken0 to token0 {i.e: wmatic->matic}
     * @param _amount amount to wrap
     */
    function _unwrap(uint256 _amount) internal {
        IWrappedToken(address(token0)).withdraw(_amount);
    }

    /**
     * @dev function to wrap to token0 to wToken0 {i.e: matic->wmatic}
     * @param _amount amount to wrap
     */
    function _wrap(uint256 _amount) internal {
        IWrappedToken(address(token0)).deposit{ value: _amount }();
    }
    ...
    function depositAll() external override onlyVault nonReentrant {
        _deposit(token0.balanceOf(address(this)));
    }

    /**
     * @dev function to withdraw all token0 balance from strategy
     */
    function _withdrawAll() internal {
        uint256 _bal = _iToken.balanceOf(address(this));
        if (_bal > 0) {
            _iToken.redeem(address(this), _bal);
        }
    ...
    function withdraw(address _beneficiary, address _asset, uint256 _amount) external override onlyVaultOrGovernor nonReentrant {
        require(_asset == address(token0), "Token not compatible.");
        uint256 _closeBal = (_iToken.balanceOf(address(this)) * 99) / 100;
        if (_amount >= _closeBal) {
            // console.log("withdrawAll");
            _withdrawAll();
            token0.safeTransfer(vaultAddress, _amount);
            _deposit(token0.balanceOf(address(this)));
            return;
        }
        _iToken.redeemUnderlying(address(this), _amount);
        _wrap(_amount);
        token0.safeTransfer(_beneficiary, _amount);
    }
    ...
    function _checkBalance() internal view returns (uint256) {
        return address(this).balance + _iToken.balanceOf(address(this)) + token0.balanceOf(address(this));
    }
    ...
    //    return token0.balanceOf(address(this)) + _iToken.balanceOf(address(this));
    // }
...
interface IGauge {
    function claimRewards(address[] memory, address[] memory, address[] memory) external;
}
...
interface ILendingToken is IERC20 {
    ...
    function redeemUnderlying(address _reserve, uint256 _amount) external;

    function redeem(address _reserve, uint256 _amount) external;
}
