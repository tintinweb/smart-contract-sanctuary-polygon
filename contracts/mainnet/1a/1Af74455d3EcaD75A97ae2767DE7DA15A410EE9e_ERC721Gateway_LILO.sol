...
abstract contract ERC721Gateway is IERC721Gateway, AnyCallApp {
    ...
    function _swapout(uint256 tokenId, uint256 chainID)
        internal
        virtual
    ...
    function _anyFallback(bytes calldata data) internal override {
        (
            ...
            uint256 swapoutSeq,
            bytes memory extraMsg
        ) = abi.decode(data, (uint256, address, address, uint256, bytes));
...
contract ERC721Gateway_LILO is ERC721Gateway, IERC721Receiver {
    ...
    // allow refunds of fees in the case of flag 2 excess fee
    ...
    constructor(address anyCallProxy, address token)
        ERC721Gateway(anyCallProxy, 2, token)
        onlyAdmin
    {}
