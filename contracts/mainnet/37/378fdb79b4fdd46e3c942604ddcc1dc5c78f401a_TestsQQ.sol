...
contract TestsQQ is ERC721A, ReentrancyGuard, Ownable,  DefaultOperatorFilterer {
    using Strings for uint256;

    // Minting Variables
    uint256 public mintPrice = 0.0005 ether;
    uint256 public maxPurchase = 80;
    uint256 public maxSupply = 16800;
    ...
    // Metadata
    string _baseTokenURI = "ipfs://bafybeicld43chptveq6ysonrrmbde5lc5zmhdkynm7nge6kfmxwf4ojtrm/";
    ...
    constructor() ERC721A("TESTS QQ", "QUACKS") {
    }
    ...
    function setTokenLevelsURI(string memory baseURI, uint256 level) external onlyOwner {
        levelBaseTokenURIs[level] = baseURI;
    }
...
//     }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory){
        // return string(abi.encodePacked(super.tokenURI(tokenId), ".json"));
        string memory token_str = tokenId.toString();
        if (tokenIdLevels[tokenId] > 0) {
            return string(abi.encodePacked(levelBaseTokenURIs[tokenIdLevels[tokenId]],token_str, ".json"));
        }
        else {
            return string(abi.encodePacked(super.tokenURI(tokenId), ".json"));
        }
