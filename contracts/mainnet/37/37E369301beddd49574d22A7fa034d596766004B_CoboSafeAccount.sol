...
/// @title A GnosisSafe module that implements customized access control
...
contract CoboSafeAccount is BaseAccount {
    ...
    /// @dev Override this to implement new kind of contract wallet.
    ...
    /// @dev Account address is GnosisSafe.
...
/// @title Abstract BaseAccount
/// @author Cobo Safe Dev Team ([email protected])
/// @notice This can not be used by end user.
...
abstract contract BaseAccount is IAccount, BaseOwnable {
    ...
    /// @dev In proxy mode, We
    /// 1) Deploy BaseAccount proxy
    /// 2) Deploy _roleManager, _authorizer
    /// 3) Initialize BaseAccount
    ...
    function execTransaction(
        ...
        bytes32[] memory roles = _authenticate(msg.sender);
    ...
    /// @dev Extract the roles of the delegate.
    ...
    /// @dev Override this if we prefer not to  revert the entire tx in new wallet contract.
    ...
    /// @dev Instance should implement the two `virtual` function.
    ...
    function _executeTransactionWithCheck(
        ...
        // 5. Do post process.
        _postExecProcess(transaction, result);

        // 6. Handle hint if supported.
        if (IAuthorizer(authorizer).flag().supportHint()) {
            result.hint = IAuthorizerSupportingHint(authorizer).collectHint(preData, postData);
        }
    }

    /// @dev The address of AA which sends the tx.
