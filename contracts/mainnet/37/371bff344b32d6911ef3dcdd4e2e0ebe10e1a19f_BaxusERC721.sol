...
import "./SellableERC721.sol";

contract BaxusERC721 is SellableERC721, ExtendedAccessControl {
    ...
    ) SellableERC721(name, symbol, royaltyReceiver, 10, 100) {
        _grantRole(DEFAULT_ADMIN_ROLE, admin);
        _grantRole(MINTER_ROLE, minters);
    }
    ...
    function mint(
        ...
        _mintAndSell(to, tokenId, tokenURI, initialTokenPrice);
    }

    function setRoyaltyData(RoyaltyData calldata _royaltyData)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        royaltyData = _royaltyData;
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(SellableERC721, AccessControl)
        returns (bool)
    {
...
contract SellableERC721 is ERC721IPFS, ReentrancyGuard, IERC2981 {
    ...
    RoyaltyData public royaltyData;
    ...
    event CancelSell(address indexed seller, uint256 indexed tokenId);
    event Buy(
        address indexed seller,
    ...
    constructor(
        ...
        uint128 royaltyDenumerator
    ) ERC721IPFS(name, symbol) {
        royaltyData = RoyaltyData({
            receiver: royaltyReceiver,
            numerator: royaltyNumerator,
            denumerator: royaltyDenumerator
        });
    }

    modifier onlyTokenHolder(uint256 tokenId) {
        if (msg.sender != ownerOf(tokenId)) {
            revert OnlyTokenHolder();
        }
        _;
    }
    ...
    {
        RoyaltyData memory r = royaltyData;
        uint256 royaltyAmount = (_salePrice * r.numerator) / r.denumerator;
    ...
    function _mintAndSell(
        ...
        _setIpfsUri(tokenId, tokenURI);
        _sell(tokenId, initialTokenPrice);
    }

    function sell(uint256 tokenId, uint256 tokenPrice)
        external
        onlyTokenHolder(tokenId)
    {
        _sell(tokenId, tokenPrice);
    }

    function _sell(uint256 tokenId, uint256 tokenPrice) internal {
        if (tokenPrice == 0) {
            revert PriceCannotBeZero();
        }
        tokenPrices[tokenId] = tokenPrice;
        emit Sell(msg.sender, tokenId, tokenPrice);
    }

    function cancelSell(uint256 tokenId) external onlyTokenHolder(tokenId) {
    ...
    function buy(uint256 tokenId) external payable nonReentrant {
        ...
        if (msg.value != tokenPrice) {
            revert PriceDoesNotMatch();
        }
        (address royaltyReceiver, uint256 royaltyAmount) = royaltyInfo(
            tokenId,
            tokenPrice
        );
        uint256 transferAmount = tokenPrice - royaltyAmount;
        // Effects
        tokenPrices[tokenId] = 0;
        // Interactions
        address seller = ownerOf(tokenId);
        payable(seller).sendValue(transferAmount);
        payable(royaltyReceiver).sendValue(royaltyAmount);
        _safeTransfer(seller, msg.sender, tokenId, "");
        emit Buy(seller, msg.sender, tokenId, msg.value);
    }
}
