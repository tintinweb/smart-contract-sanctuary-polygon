...
contract ERC721Sale is SaleRoles {
    ...
    function setFreeMintMerkleRoot(
        bytes32 newMerkleRoot
    ) external onlyOperator {
        _freeSecretSaleData.merkleRoot = newMerkleRoot;
        emitFreeMintData();
    ...
    function freeMintIssuedAmountOf(
        address minter
    ) external view returns (uint256) {
        return _freeSecretSaleData.issuedAmountPerAddress[minter];
    }

    function freeMintRemainAmountOf(
        address minter,
        uint256 maxAmountPerAddress
    ) external view returns (uint256) {
        require(_freeSecretSaleData.isSale == true, "not sale");
    ...
    function preMintData()
        external
        view
        returns (bytes32, uint256, uint256, uint256, bool)
    {
        return (
    ...
    function preMintIssuedAmountOf(
        address minter
    ) external view returns (uint256) {
        return _preSecretSaleData.issuedAmountPerAddress[minter];
    }

    function preMintRemainAmountOf(
        address minter,
        uint256 maxAmountPerAddress
    ) external view returns (uint256) {
        require(_preSecretSaleData.isSale == true, "not sale");
    ...
    function setPublicMintMaxAmountPerAddress(
        uint256 newMaxAmountPerAddress
    ) external onlyOperator {
        _publicSaleData.maxAmountPerAddress = newMaxAmountPerAddress;
        emitPublicMintData();
    ...
    function publicMintData() external view returns (uint256, uint256, bool) {
        return (
            _publicSaleData.maxAmountPerAddress,
            _publicPrice,
    ...
    function publicMintIssuedAmountOf(
        address minter
    ) external view returns (uint256) {
        return _publicSaleData.issuedAmountPerAddress[minter];
    }

    function publicMintRemainAmountOf(
        address minter
    ) external view returns (uint256) {
        require(_publicSaleData.isSale == true, "not sale");
        require(
    ...
    function coreSupplies() external view returns (uint256, uint256, uint256) {
        IERC721Core erc721Core = IERC721Core(_mintAddress);
    ...
    function setEarningAddress(
        address newEarningAddress
...
contract SaleRoles is Pausable, AdminRole, OperatorRole, FinancialRole {
    ...
    function transferFinancial(
        address newFinancial
...
contract Sports3PlusCollectionSale is ERC721Sale {
