...
import "IERC721.sol";
import "VxPolygon.sol";

contract KongSafetyModule {
	KongzVXPoly public constant VX = KongzVXPoly(0x05df72d911e52AB122f7d9955728BC96A718782C);
	...
	function renounce(address _vxOwner) external {
		require(guardians[_vxOwner] == msg.sender, "!guardian");
		guardians[_vxOwner] = address(0);
	}

	function lockMany(uint256[] calldata _vxs) external {
		require(_vxs.length > 0, "At least 1");
		address owner = VX.ownerOf(_vxs[0]);
		require(guardians[owner] == msg.sender, "!guardian");
		for (uint256 i = 0; i < _vxs.length; i++) {
			require(VX.ownerOf(_vxs[i]) == owner, "!owner");
			VX.lockId(_vxs[i]);
		}
	}

	function unlockMany(uint256[] calldata _vxs) external {
		require(_vxs.length > 0, "At least 1");
		address owner = VX.ownerOf(_vxs[0]);
		require(guardians[owner] == msg.sender, "!guardian");
		for (uint256 i = 0; i < _vxs.length; i++) {
	...
	function unlockManyAndTransfer(uint256[] calldata _vxs, address _recipient) external {
		require(_vxs.length > 0, "At least 1");
		address owner = VX.ownerOf(_vxs[0]);
		require(guardians[owner] == msg.sender, "!guardian");
		for (uint256 i = 0; i < _vxs.length; i++) {
			require(VX.ownerOf(_vxs[i]) == owner, "!owner");
			VX.unlockId(_vxs[i]);
			VX.safeTransferFrom(owner, _recipient, _vxs[i]);
		}
	}
}
