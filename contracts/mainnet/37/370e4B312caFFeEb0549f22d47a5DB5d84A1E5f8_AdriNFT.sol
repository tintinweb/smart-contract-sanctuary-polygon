...
contract AdriNFT is ERC721A, ERC2981, DefaultOperatorFilterer, Ownable {
    bool public contractSealed = false;
    string public baseURI;

    bool public mintActive;
    uint256 public constant TOKEN_MAX_SUPPLY = 4636;
    uint88 public tokenPrice = 40 ether;
    ...
    function crossmint(address to, uint256 quantity) external payable {
        ...
        _baseMint(to, quantity);
    }

    function approvedClaim(address to) external {
        // Revert if caller is not approved minter
        if(msg.sender != approvedMinter) revert InvalidCaller();
    ...
    function setTokenPrice(uint88 _tokenPrice) external onlyOwner {
    ...
    function setApprovedMinter(address _approvedMinter) external onlyOwner {
    ...
    function _baseMint(address to, uint256 quantity) internal {
