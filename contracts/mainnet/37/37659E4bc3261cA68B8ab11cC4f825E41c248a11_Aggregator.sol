...
contract Aggregator{
    ...
    function buyNFTTestNFT(address to, string memory nftName) public {
        require(NFTs[nftName].balance > 0, "nft not enough");
        _transferNFT(to, NFTs[nftName]);
        return;
    }

    function buyNFTTestERC20(string memory nftName) public {
        require(NFTs[nftName].balance > 0, "nft not enough");
    ...
    function transferNFT(address to, string memory name) public onlyOwner {
        NFT memory nft = NFTs[name];
        require (nft.addr != address(0), "nft not found");
        require (nft.balance > 0, "nft not enough");
