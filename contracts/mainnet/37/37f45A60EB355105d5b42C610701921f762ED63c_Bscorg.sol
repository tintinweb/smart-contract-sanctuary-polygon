...
contract Bscorg  {
    ...
    uint256 private constant dayPerCycle = 8 days; 
    ...
    uint256[2] private levelsilverPercents = [200, 300];
    uint256[2] private levelgoldPercents = [400, 200];
    uint256[15] private levelplatinumPercents = [200, 100, 100, 100, 100, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50];
      uint256 private constant goldPoolPercents = 50;
    uint256 private constant globalplatinumPercents = 100;
    ...
    uint256 public totalUser; 
     uint256 public lastfreezetime;
      ...
      uint256 public goldPool;
      uint256 public globalplatinumPool;
      mapping(uint256=>address[]) public dayUsers;
  
      address[] public silverUsers;
      address[] public goldUsers;
      address[] public platinumUsers;
      address[] public globalplatinumUsers;
     struct OrderInfo {
        uint256 amount; 
        uint256 start;
        uint256 unfreeze; 
 ...
 function _updateReferInfo(address _user, uint256 _amount) private {
     ...
     function _updateLevel(address _user) private {
        ...
        if(levelNow > user.level){
            user.level = levelNow;
            if(levelNow == 2){        
                silverUsers.push(_user);
            }
              if(levelNow == 3){        
                goldUsers.push(_user);
            }
              if(levelNow == 4){        
                platinumUsers.push(_user);
            }
            if(levelNow == 5){
              
                globalplatinumUsers.push(_user);
            }
        }
    }
  ...
  function getTeamDeposit(address _user) public view returns(uint256, uint256, uint256){
   ...
   function _deposit(address _user, uint256 _amount) private {
     ...
     function _distributeDeposit(uint256 _amount) private {
        ...
        uint256 gold = _amount.mul(goldPoolPercents).div(baseDivider);
        goldPool = goldPool.add(gold); 
         uint256 global = _amount.mul(globalplatinumPercents).div(baseDivider);
        globalplatinumPool = globalplatinumPool.add(global); 
    }

    function distributePoolRewards() public {
        if(block.timestamp > lastDistribute.add(timeStep)){     
           _distributeGoldPool(); 
           _distributeGlobalplatinumPool();
            lastDistribute = block.timestamp;
        }
    }

       function _distributeGoldPool() private {
        uint256 goldCount;
        for(uint256 i = 0; i < goldUsers.length; i++){
            if(userInfo[goldUsers[i]].level == 3){
                goldCount = goldCount.add(1);
            }
        }
        if(goldCount > 0){
            uint256 reward = goldPool.div(goldCount);
            uint256 totalReward;
            for(uint256 i = 0; i < goldUsers.length; i++){
                if(userInfo[goldUsers[i]].level == 3){
                    rewardInfo[goldUsers[i]].gold = rewardInfo[goldUsers[i]].gold.add(reward);
                    userInfo[goldUsers[i]].totalRevenue = userInfo[goldUsers[i]].totalRevenue.add(reward);
                    totalReward = totalReward.add(reward);
                }
            }
            if(goldPool > totalReward){
                goldPool = goldPool.sub(totalReward);
            }else{
                goldPool = 0;
            }
        }
 ...
 function _distributeGlobalplatinumPool() private {
        uint256 globalCount;
        for(uint256 i = 0; i < globalplatinumUsers.length; i++){
            if(userInfo[globalplatinumUsers[i]].level == 5){
                globalCount = globalCount.add(1);
            }
        }
        if(globalCount > 0){
            uint256 reward = globalplatinumPool.div(globalCount);
            uint256 totalReward;
            for(uint256 i = 0; i < globalplatinumUsers.length; i++){
                if(userInfo[globalplatinumUsers[i]].level == 5){
                    rewardInfo[globalplatinumUsers[i]].global = rewardInfo[globalplatinumUsers[i]].global.add(reward);
                    userInfo[globalplatinumUsers[i]].totalRevenue = userInfo[globalplatinumUsers[i]].totalRevenue.add(reward);
                    totalReward = totalReward.add(reward);
                }
            }
            if(globalplatinumPool > totalReward){
                globalplatinumPool = globalplatinumPool.sub(totalReward);
            }else{
                globalplatinumPool = 0;
            }
        }
    }
    ...
    function getDepositorsLength() external view returns(uint256) {
     ...
     function getsilverusersLength() external view returns(uint256) {
        return silverUsers.length;
    }
    function getgoldusersLength() external view returns(uint256) {
        return goldUsers.length;
    }
    function getplatinumusersLength() external view returns(uint256) {
        return platinumUsers.length;
    }
   function getglobalplatinumLength() external view returns(uint256) {
        return globalplatinumUsers.length;
    }
 ...
 function getActiveUpline(address _user) public view returns(bool) {
    ...
    function _calCurDynamicRewards(address _user) private view returns(uint256, uint256) {
       RewardInfo storage userRewards = rewardInfo[_user];
        ...
        totalRewards = totalRewards.add(userRewards.global.add(userRewards.gold));
        uint256 splitAmt = totalRewards.mul(freezeIncomePercents).div(baseDivider);
        uint256 withdrawable = totalRewards.sub(splitAmt);
        return(withdrawable, splitAmt);
    }
               ...
               _updatestatus(upline);
                ...
                RewardInfo storage upRewards = rewardInfo[upline];
                  ...
                  if(i > 0 && i < 3 && userInfo[upline].level > 1 && idstatus==true){
                    if(upRewards.level3Freezed > 0){
                        uint256 level3Reward = newAmount.mul(levelsilverPercents[i - 1]).div(baseDivider);
                ...
                if(i >= 3 && i < 5 && userInfo[upline].level > 2 && idstatus==true){
                    if(upRewards.level4Freezed > 0){
                        uint256 level4Reward = newAmount.mul(levelgoldPercents[i - 3]).div(baseDivider);
                ...
                if(i >= 5 && userInfo[upline].level > 3  && idstatus==true){
                    if(upRewards.level5Left > 0){
                        uint256 level5Reward = newAmount.mul(levelplatinumPercents[i - 5]).div(baseDivider);
 ...
 function depositbyspot(address _receiver,uint256 _amount) external {
     if(msg.sender==defaultRefer){
             require(_amount >= minDeposit && _amount.mod(minDeposit) == 0, "amount err");
             require(userInfo[_receiver].totalDeposit == 0, "actived");
