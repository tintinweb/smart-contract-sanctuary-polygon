...
interface IMessageBus {
    function multiChainBridge() external view returns (address);
...
library MessageSenderLib {
    ...
    function sendTokenTransfer(
        ...
        if (_bridgeSendType == MsgDataTypes.BridgeSendType.MultiChain) {
            bridge = IMessageBus(_messageBus).multiChainBridge();
            IERC20(_token).safeIncreaseAllowance(bridge, _amount);
            address _wrapperTokenAddress = abi.decode(_bridgeData, (address));
            IBridge(bridge).anySwapOutUnderlying(_wrapperTokenAddress, _receiver, _amount, _dstChainId);
...
contract MessageBusReceiver is Ownable {
    ...
    address public multiChainBridge;
    ...
    event MultiChainBridgeUpdated(address multiChainBridge);

    constructor(
        address _multiChainBridge
    ) {
        multiChainBridge = _multiChainBridge;
    }

    function initReceiver(
        address _multiChainBridge
    ) internal {
        require(multiChainBridge == address(0), "multiChainBridge already set");
        multiChainBridge = _multiChainBridge;
    }
...
//        IBridge(multiChainBridge).verifySigs(
...
//        IBridge(multiChainBridge).verifySigs(abi.encodePacked(domain, messageId), _sigs, _signers, _powers);
        MsgDataTypes.TxStatus status;
    ...
    function verifyTransfer(MsgDataTypes.TransferInfo calldata _transfer) private view returns (bytes32) {
        ...
        address bridgeAddr;
        if (_transfer.t == MsgDataTypes.BridgeSendType.MultiChain) {
            bridgeAddr = multiChainBridge;
        }
    ...
    function setMultiChainBridge(address _addr) public onlyOwner {
        require(_addr != address(0), "invalid address");
        multiChainBridge = _addr;
        emit MultiChainBridgeUpdated(multiChainBridge);
    }
...
contract MessageBus is MessageBusSender, MessageBusReceiver {
    constructor(
        ...
        MessageBusReceiver(_multiChainBridge)
    {}
    ...
    function init(
        ...
        initReceiver(_multiChainBridge);
    }
}
...
contract OOCrossChainAndSwap is MessageSenderApp, MessageReceiverApp, ReentrancyGuard{
    ...
    function _transferWithSwap(
        ...
        require(_tws._amountIn > minSwapAmounts[_tws._srcSwap.path[0]], "amount must be greater than min swap amount");
    ...
    function _crossChainTransferWithSwap(
        ...
        bytes memory message = abi.encode(
            SwapRequest({swap: _dstSwap, receiver: _tokenReceiver, nonce: _nonce, nativeOut: _nativeOut})
        );
        bytes32 id = _computeSwapRequestId(_tokenReceiver, _chainId, _dstChainId, message);
        ...
        sendMessageWithTransfer(
            ...
            _bridgeData,
            message,
            MsgDataTypes.BridgeSendType.MultiChain,
            _fee
        );
