...
[DONE implemented in opensea]implement royalties
[DONE removed this already] implement og sale 
*/
...
contract NFT is ERC721Enumerable, Ownable { // inherit from erc721enumerable and ownable, meaning our contract would have the access into their built in methods
  ...
  uint256 public whitelistCost = 0.1 ether; // price of minting during whitelist sale - modifiable by only owner 
  uint256 public cost = 1 ether; // price of minting during public sale - modifiable by only owner 
  uint256 public maxSupply = 1000; // max supply to be mintable - modifiable by only owner
  uint256 public maxMintAmount = 20; // max mint amount per transaction - modifiable by only owner 
  uint256 public whitelistNftPerAddressLimit = 10; // max cyber outlaws nft per address during whitelist - modifiable by only owner 
  ...
  bool public isMerkleActivate = false;
  bytes32 public merkleRoot = "asdfafda";
  address[] public whitelistedAddresses;
  mapping(address => uint256) public whitelistAddressMintedBalance; // determines the amount of mint done by the user 
  ...
  // whitelist 
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable { // public - outside of the contract have access to the function | payable - allows a function to process transaction
    ...
    if (msg.sender != owner()) { // if the caller of the mint function is not the owner then do everything within the scope, else if the owner then proceed to minting without paying any cost 
      ...
      if (isMerkleActivate == true){
        // verify the given merkle proof 
  ...
  function setIsMerkleActivate(bool _state) public onlyOwner {
      isMerkleActivate = _state;
  }
