...
/**
 * NFT implementation for one million pixel project
 */
contract NFT is
    ERC721,
    ERC721Royalty,
    ...
    MintableNFT
{
    ...
    // Total supply for the NFT, 0 means infinity
    uint256 public maxSupply;

    // Stores address for proxies registry, those proxies can transfer tokens in the name of the owners
    address public proxyRegistryAddress;

    // Base token URI used as a prefix by tokenURI().
    string public baseURI;

    // Counts minted tokens
    ...
    function mintTo(address to, uint256 tokenId) external onlyMinter {
        if (tokenId == 0 || (maxSupply != 0 && tokenId > maxSupply)) {
    ...
    function mintTo(address to, uint256[] memory tokenIds) external onlyMinter {
        mintedCounter += tokenIds.length;
    ...
    function canMint(uint256[] memory tokenIds) external view returns (bool) {
        for (uint i = 0; i < tokenIds.length; i++) {
            if (
                tokenIds[i] == 0 ||
                (maxSupply != 0 && tokenIds[i] > maxSupply) ||
                _exists(tokenIds[i])
            ) {
                return false;
            }
    ...
    function deleteDefaultRoyalty() external onlyOperator {
    ...
    function resetTokenRoyalty(uint256 tokenId) external onlyOperator {
        _resetTokenRoyalty(tokenId);
    }
     ...
     * but using meta transactions. It enables gass-less transactions.
     */
