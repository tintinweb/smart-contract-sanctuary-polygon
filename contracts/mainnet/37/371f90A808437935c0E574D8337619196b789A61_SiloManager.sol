...
contract SiloManager is Initializable, KeeperCompatibleInterface {
  ...
  modifier onlyAdmin() {
    require(
      msg.sender == managerFactory || msg.sender == owner,
  ...
  function initialize(address _mangerFactory, address _owner)
    external
    initializer
  {
    ...
    returns (bool upkeepNeeded, bytes memory performData)
  {
    if (upkeepId > 0) {
      if (!upkeepNeeded) {
        if (autoTopup) {
          if (
            ManagerFactory.getUpkeepBalance(owner) <
            ManagerFactory.getMinimumUpkeepBalance(owner)
          ) {
            // not healthy status
          ...
          for (uint256 i; i < SiloFactory.balanceOf(owner); ) {
            ...
            if (Silo.highRiskAction()) {
              ...
              uint96 minBalance = (getRiskBuffer() *
                ManagerFactory.getMinBalance(upkeepId)) / uint96(10000);
              if (balance < minBalance) {
                if (Silo.getStatus() == Statuses.MANAGED && Silo.deposited()) {
  ...
  function performUpkeep(bytes calldata performData) external override {
    if (customRegistry != address(0)) {
      ...
      require(
        msg.sender == ManagerFactory.alphaRegistry() ||
          msg.sender == ManagerFactory.betaRegistry(),
    ...
    if (silo != address(this)) {
      ...
      uint256 task = abi.decode(siloPerformData, (uint256));
      // if(task == 0 || task == 1){
      ...
      if (task == 2) {
        ...
        if (
          ERC20LinkBal > 0 &&
          ERC20LinkBal <=
          PegSwap.getSwappableAmount(address(ERC20Link), address(ERC677Link))
        ) {
        ...
        if (balance >= addFundsThreshold) {
          ...
          uint256 addAmount = ManagerFactory.getMinimumUpkeepBalance(owner) -
            ManagerFactory.getUpkeepBalance(owner);
          addAmount = (addAmount * 110) / 100;

          uint256 topupAmount = addAmount > balance ? balance : addAmount;
          ERC677Link.approve(ManagerFactory.getKeeperRegistry(), topupAmount);

          IKeepersRegistry(ManagerFactory.getKeeperRegistry()).addFunds(
            upkeepId,
            uint96(topupAmount)
  ...
  function setUpkeepId(uint256 id) external {
    require(
      msg.sender == address(ManagerFactory),
      "Only factory can set upkeep id"
    );
    upkeepId = id;
  }
  ...
  function setCustomRiskBuffer(uint96 _buffer) external onlyAdmin {
    ...
    uint96 risk = uint96(10000) + uint96(siloBalance * 500);
    ...
    returns (uint96 minRisk, uint96 minRejoin)
  {
    ...
    minRisk = uint96(10000) + uint96(siloBalance * 500);
