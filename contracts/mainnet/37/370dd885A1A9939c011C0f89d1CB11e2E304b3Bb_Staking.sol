...
contract Staking is Ownable, ReentrancyGuard {
	...
	INUGS public nugs; //0x4F9410d01A5eBAc79dE3B777eaf63E6a0b8E15d3
	...
	function claim(
		...
		require(hashTransaction(to, nonce, _tokenId, _nftContract) == hash, "invalid hash");
	...
	function unclaimedRewards(uint256 _tokenId, address _nftContract) public view returns (uint256) {
		...
		if (lastClaimDate == uint256(0)) {
			return 300000000000000000000; // 300 $NUGS initially
		}
	...
	//hashTransaction(to, nonce, _tokenId, _nftContract)
	function hashTransaction(
		...
		bytes32 hash = keccak256(
			abi.encodePacked(
				...
				keccak256(abi.encodePacked(param1, param2, param3, param4))
			)
		);
		return hash;
	}
	...
	function setNugsToken(address _contract) external onlyOwner {
		nugs = INUGS(_contract);
	}

	//stake only from this tokens
	function setAllowedNFTs(address _contractAddress, uint256 _emissionRate) external onlyOwner {
		require(_emissionRate <= 1000000000000000000000, "no more than 1000 per day");
