...
contract ORL_StakingEngine is Ownable {
    ...
    function createEvent(uint16 _id, uint8 _size, address _conCheck, uint8 _conValue, uint _matP, uint _nutP) public {
    ...
    function signUp (uint16[] calldata _racers, address _con, uint8[] calldata _event) external payable {

    if (paused) revert StakingIsPaused();
    funchecks memory _funchecks;
     //    if (walletToID[msg.sender] == 0){
      //      walletID.push(msg.sender);
     //       walletToID[msg.sender] = walletID.length;
      //  }
        ...
        do {
            ...
            if (_con == erc721Contract){
                _funchecks.own = stakeableNFT.ownerOf(_racers[i]);
            } else if (_con == customAddress) {
                _funchecks.own = customContract.ownerOf(_racers[i]);
            }
        ...
        _funchecks.tempstaked = currentlyStaked;
        delete i;
        do{


            _temp.id = _racers[i];
            _temp.nftContract = _con;
            _temp.eventWanted = _event[i];
            _temp.owner = msg.sender;
            uint16 teehee = slotByRacer[currentWeek][_con][_temp.id];

            if (teehee != 0)  revert alreadyRegistered();
            
                teehee = _funchecks.tempstaked;
                ++_funchecks.tempstaked;
                slotByRacer[currentWeek][_con][_temp.id] = teehee;
            

            _funchecks.newn = uint8(randomNumber(_funchecks.num, 3, 10));  
            _temp.leg1 = _funchecks.newn;
            _funchecks.num += _funchecks.newn;
            _funchecks.newn = uint8(randomNumber(_funchecks.num, 3, 10));  
            _temp.leg2 = _funchecks.newn;
            _funchecks.num += _funchecks.newn;
            _funchecks.newn = uint8(randomNumber(_funchecks.num, 6, 12));  
            _temp.leg3 = _funchecks.newn;
            perfByRacer[currentWeek][teehee] = _temp;
            ...
            unchecked {
                ++i;
                 ...
                 currentlyStaked = _funchecks.tempstaked;
    }

     function returnRacers() external view returns(performance[] memory){
        performance[] memory _racers = new performance[](currentlyStaked);

        for (uint16 i; i < currentlyStaked;){
    ...
    function setBoost(uint8 _boost, uint16 _tokenID) external {
       // if (walletToID[msg.sender] == 0){
    ...
    function getOwnerUnstaked(address _owner) external view returns ( uint [] memory){
        uint[] memory ownersTokens = RacerInterface(erc721Contract).tokensOfOwner(_owner);
        uint cachedLength = ownersTokens.length;
