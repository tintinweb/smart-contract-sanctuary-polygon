...
contract KannaSaleL2 is Ownable, AccessControl {
    ...
    bytes32 public constant CLAIM_MANAGER_ROLE =
        keccak256("CLAIM_MANAGER_ROLE");
    ...
    constructor(
        ...
        require(
            address(_priceAggregator) != address(0),
    ...
    function convertToWEI(
        uint256 amountInKNN
    ) public view positiveAmount(amountInKNN) returns (uint256, uint256) {
        ...
        return (
            (amountInKNN * knnPriceInUSD) / maticPriceInUSD,
            maticPriceInUSD
        );
    }
    ...
    function convertToKNN(
        uint256 amountInWEI
    ) public view positiveAmount(amountInWEI) returns (uint256, uint256) {
        (, int256 answer, , , ) = priceAggregator.latestRoundData();
        ...
        return (
            (amountInWEI * maticPriceInUSD) / knnPriceInUSD,
            maticPriceInUSD
        );
    }
    ...
    function buyTokens() external payable {
        ...
        (uint256 finalAmount, uint256 maticPriceInUSD) = convertToKNN(
            msg.value
        );
