...
contract FactoryERC721 is Ownable {
    ...
    ERC721Token[] public tokens;
    event Deployed(address addr, uint256 salt);
    event ERC721Created(address owner, address tokenContract); 

    function deployERC721(string[] memory _categories, string[] memory _baseUri, uint[] memory _price, uint[] memory _maxSupply, uint[] memory _percentages, address _usdc, address _investor, address _artist, uint[] memory _percentagesPrimary,  uint[] memory _percentagesSecondary ,uint _startDate) public returns (address) {
        ERC721Token t = new ERC721Token( _categories,  _baseUri,  _price,  _maxSupply,  _percentages, _usdc, _investor, _artist, _percentagesPrimary, _percentagesSecondary , _startDate);
        tokens.push(t);
    ...
    function transferOwnershipFactory(address payable _contract, address _newOwner) public payable onlyOwner {
        ERC721Token(_contract).transferOwnership(_newOwner);
    }
}
...
contract ERC721Token is ERC721Enumerable, Pausable, Ownable {
    ...
    address investor;
    uint256[] percentagesPrimary;
    uint256[] percentageSecondary;
    uint256 startDate;
    
    //Constructor
    constructor(string[] memory _categories, string[] memory _baseUri, uint[] memory _price, uint[] memory _maxSupply, uint[] memory _percentages, address _usdc,  address  _artist, address _investor, uint[] memory _percentagesPrimary,  uint[] memory _percentagesSecondary, uint _startDate)
    ERC721("test", "TT") {
        ...
        require(getSum(_percentagesPrimary) == 100, "Percentage must be 100");
        require(getSum(_percentagesSecondary) == 100, "Percentage must be 100");
        ...
        investor = _investor;
        artist = _artist;
        percentagesPrimary = _percentagesPrimary;
        percentageSecondary = _percentagesSecondary;
    ...
    */
    receive() external payable {
        (bool success1, ) = payable(investor).call{value: ((msg.value * percentageSecondary[0]) / 100)}("");
		require(success1);
        (bool success2, ) = payable(artist).call{value: (msg.value * percentageSecondary[1]) / 100}("");
		require(success2);
    ...
    */
    function FundRoyalties(uint amount) public onlyOwner{
    ...
    */
    function withdraw() external payable onlyOwner {
        (bool success1, ) = payable(investor).call{value: ((msg.value * percentagesPrimary[0]) / 100)}("");
		require(success1);
        (bool success2, ) = payable(artist).call{value: (msg.value * percentagesPrimary[1]) / 100}("");
		require(success2);
    }
    ...
    *
    */
    function emergencyWithdraw(address erc20Token) external onlyOwner {
        contractERC20(erc20Token).transfer(investor, contractERC20(erc20Token).balanceOf(address(this)) * percentagesPrimary[0] / 100); 
        contractERC20(erc20Token).transfer(artist, contractERC20(erc20Token).balanceOf(address(this)) * percentagesPrimary[1] / 100);
    } 
    ...
    */
    function setArtistPercentage(uint _percentageArtist) external onlyOwner {
        percentagesPrimary[1] = _percentageArtist;
    }

    /**
    * @notice set the investor percentage
    */
    function setInvestorPercentage(uint _percentageInvestor) external onlyOwner {
        percentagesPrimary[0] = _percentageInvestor;
    }

    /**
    * @notice set artist percentage for opensea
    */
    function setArtistPercentageOpensea(uint _percentageArtistOpensea) external onlyOwner {
        percentageSecondary[1] = _percentageArtistOpensea;
    }

    /**
    * @notice set investor percentage for opensea
    */
    function setInvestorPercentageOpensea(uint _percentageInvestorOpensea) external onlyOwner {
        percentageSecondary[0] = _percentageInvestorOpensea;
    }

    /**
    * @notice set the investor address
    */
    function setOnlyInvestor(address _investor) external onlyOwner {
        investor = _investor;
    }
