...
contract rePoint is DataStorage {
    ...
    constructor(
        ...
        maxOld = _maxOld;
        lastReward24h = 1678449600;
        lastReward7d = 1678276675;
        lastReward30d = 1677843900;
    }
    ...
    function _addMonthDirectLeft(address userAddr, uint256 directLeft) internal {
        uint256 userLeftDirect = _monthDirectLeft[monthCounter][userAddr];

        if(userLeftDirect >= 50) {
            _monthDirectLeft[monthCounter][userAddr] += directLeft;
        } else {
            uint256 neededDirectLeft = 50 - userLeftDirect;
    ...
    function _addMonthDirectRight(address userAddr, uint256 directRight) internal {
        uint256 userRightDirect = _monthDirectRight[monthCounter][userAddr];

        if(userRightDirect >= 50) {
            _monthDirectRight[monthCounter][userAddr] += directRight;
        } else {
            uint256 neededDirectRight = 50 - userRightDirect;
    ...
    function register (
        ...
        address childLeft,
        address childRight,
        uint256 monthDirectLeft,
        uint256 monthDirectRight
    ) public {
        ...
        _monthDirectLeft[monthCounter][userAddr] = monthDirectLeft;
        _monthDirectRight[monthCounter][userAddr] = monthDirectRight;
    }
