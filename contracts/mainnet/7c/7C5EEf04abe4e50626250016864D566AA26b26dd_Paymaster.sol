...
contract Paymaster is Ownable, BasePaymaster {
  ...
  ICloud9Escrow public cloud9Escrow;

  address public targetContract;
  string private constant version = "2.2.4+nft.claim";

  constructor(
    address _trustedForwarder,
    IRelayHub _relayHub,
    ICloudKey _cloudKey,
    ICloud9Escrow _cloud9Escrow
  ) {
    // Zero address check all address parameters
    require(
      _trustedForwarder != address(0),
      "CloudKey : Invalid Forwarder Addresss"
    );
    require(
      address(_relayHub) != address(0),
      "CloudKey : Invalid Relayhub Address"
    );
    require(
      address(_cloudKey) != address(0),
      "CloudKey : Invalid CloudKey Address"
    );
    require(
      address(_cloud9Escrow) != address(0),
      "CloudKey : Invalid Escrow Address"
    );
    ...
    returns (bytes memory context, bool revertOnRecipientRevert)
  {
    ...
    handlePreRelayedCall(relayRequest.request.to, relayRequest.request.data);

    return ("", false);
  }

  /// @notice This method is mostly for executing effects after a relayed call has been forwarded. Empty implementation here because we have no requirement for logic after the relay call, we strictly enforce verification in the preRelayedCall method.
  ...
  // @note Requires public visibility for this as it overrides the implementation from @opengsn which specifies public visibility
    ...
    onlyOwner
  {
    ...
    super.setTrustedForwarder(_trustedForwarder);
  }

  /**
      Private Methods
    */
  function getMethodSignatureFromData(bytes memory _data)
    private
    pure
    returns (bytes4 methodSig)
  {
    assembly {
      methodSig := mload(add(_data, add(0x20, 0)))
    }
  }

  /// @notice Method used to handle a preRelayedCall
  /// @notice This method doesn't validate the forwarding request, this is because before GSN forwards a transaction they will first try and estimate the gas of the original method and if that fails they won't forward the transaction
  function handlePreRelayedCall(address _to, bytes calldata _data) private {
    if (_to == address(cloudKey)) {
      bytes4 methodSig = getMethodSignatureFromData(_data);

      require(
        methodSig == bytes4(keccak256("claimToken(address,uint256,bytes)")),
        "Paymaster: Invalid method specified"
      );

      return;
    } else if (_to == address(cloud9Escrow)) {
      bytes4 methodSig = getMethodSignatureFromData(_data);

      require(
        methodSig == bytes4(keccak256("claimCarbonOffset(uint256)")) ||
          methodSig == bytes4(keccak256("claimMatic(uint256)")),
        "Paymaster: Invalid method specified"
      );
    ...
    revert("Paymaster: Invalid method specified");
  }
}
