...
contract Evoldefi {
	...
	constructor(address payable wallet, address payable dWallet, uint256 startDate) {
		...
		devWallet = dWallet;
		startUNIX = startDate;
        plans.push(Plan(12, 121)); // 12.1% per day for 12 days
        plans.push(Plan(18, 113)); // 11.3% per day for 18 days
        plans.push(Plan(25, 101)); // 10.1% per day for 25 days
        plans.push(Plan(12, 166)); // 16.6% per day for 12 days
        plans.push(Plan(18, 180)); // 18.0% per day for 18 days
        plans.push(Plan(25, 181)); // 18.1% per day for 25 days
	}

	function invest(address referrer, uint8 plan) public payable {
		require(block.timestamp > startUNIX, "We are not live yet!");
		require(msg.value >= INVEST_MIN_AMOUNT, "Minimum investment is 10");
