...
contract SelfieZone is StandardZone {
  constructor() StandardZone(
    msg.sender,
    hex"e009e41b87d5aa830d4a8e8c4752d9e8cb4009c691b8b3550bb5d5c8de92c2c1",
    ".selfie",
    "SELFIE",
    "ipfs://"
  ) {}
  ...
  function register(
    ...
    namehash = super.register(to, parent, label);
  }
  ...
  function registerWithMeta(
    ...
    require(_isApprovedOrOwner(_msgSender(), uint256(getOrigin())), "must own zone");
    namehash = register(to, parent, label);
    _setTokenURI(uint256(namehash), _tokenURI);
  }
...
contract BaseZone is ZoneInterface, StandardNFT {
  ...
  function register(
    ...
    require(_isApprovedOrOwner(_msgSender(), uint256(parent)), "must own parent");
  ...
  function setRecord(
    ...
    require(_isApprovedOrOwner(_msgSender(), uint256(resource)), "must own resource");
    require(locks[key] < block.timestamp , "record locked");

    records[key] = Record(header, data);

    emit RecordSet(resource, label, header, data);
  }

  function getRecord(
    bytes32 resource,
    string calldata label
  ) public view virtual override returns (bytes32 header, bytes32 data) {
    bytes32 key = _getKey(resource, label);
    Record memory record = records[key];
    return(record.header, record.data);
  }
  ...
  function setLock(
    ...
    bytes32 key = _getKey(resource, label);
    unlocks = Math.max(locks[key], block.timestamp).add(time);
    locks[key] = unlocks;
  ...
  function getLock(
    bytes32 resource,
    string calldata label
  ) public view override returns (uint256 unlocks) {
    bytes32 key = _getKey(resource, label);
    unlocks = locks[key];
  }
