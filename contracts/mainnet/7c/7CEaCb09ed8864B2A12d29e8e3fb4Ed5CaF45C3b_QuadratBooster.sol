...
contract QuadratBooster is QuadratBoosterStorage {
        ...
        returns (uint256 _rewardAmount)
    {
        ...
        if (_rewardAmount > 0) {
            ...
            rewardToken.safeTransfer(user, _rewardAmount);
        }
    }
...
import {InitializePayload, DepositInfo} from "../structs/SQuadratBooster.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    // Current cumulative rewardPerShare * 1e12.
    uint256 public cumulativeRewardPerShare;
    // Current cumulative blockNumber.
    uint64 public cumulativeRewardBlockNumber;
    ...
    uint64 public minimalWithdrawBlocks;
    // Timed deposit bonus
    mapping(uint32 => uint16) public timedBonus;
    // Info of each user that stakes token.
    mapping(address => DepositInfo) public deposits;
    // Cumulative rewardPerShares * 1e12.
    mapping(uint64 => uint256) public cumulativeRewardPerShares;
     ...
     * @param to address
     */
    function transferDeposit(address to) external override {
        address sender = _msgSender();
        DepositInfo storage deposit = deposits[sender];
        require(deposit.amount > 0, "UA");
        require(sender != to && deposits[to].amount == 0, "TA");
        deposits[to] = deposit;
        _depositors.add(to);
        _depositors.remove(sender);
        delete deposits[sender];
        emit DepositTrasfered(sender, to, deposit.amount);
    }
    ...
    function subReward(address token, address treasure)
        ...
        onlyOwner
    {
        require(token != address(0) && treasure != address(0), "ZTA");
        require(token != address(depositToken), "DTA");
        uint256 amount;
    ...
    function setMinimalWithdrawBlocks(uint64 blocks)
        ...
        onlyOwner
    {
        require(minimalWithdrawBlocks != blocks, "RMWB");
     ...
     * @param _payload InitializePayload data
     * @return bytes array
     */
    function transformPayloadToBytes(InitializePayload memory _payload)
        external
        pure
        override
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < blocks.length; ) {
            require(blocks[i] > 0, "TBN");
            timedBonus[blocks[i]] = bonuses[i];
            unchecked {
                i++;
            }
        }
        ...
        returns (uint256 _cumulativeRewardPerShare)
    {
        ...
        if (_blockNumber > _fromBlock) {
            ...
            ) = _calculateCumulativeRewardPerShare(
                cumulativeRewardPerShare,
                rewardPerBlock,
                virtualTotalDeposit,
                _fromBlock,
            ...
            cumulativeRewardBlockNumber = Math
                .min(toBlock, _blockNumber)
                .toUint64();
    ...
    function _rewardUnlocked() internal view returns (uint256) {
        return rewardToken.balanceOf(address(this)) - _rewardLocked();
    }

    // solhint-disable-next-line function-max-lines
    function _calculateCumulativeRewardPerShare(
        uint256 _cumulativeRewardPerShare,
...
import {InitializePayload} from "../structs/SQuadratBooster.sol";
