...
contract DexInvestorFacet is Modifiers, ReentrancyGuard {
    ...
    */
    function issueFromDexCalldata(
        ...
        require(!LibReferralToken.isReferralToken(_addr) || LibReferralToken.isAllowedInvestor(_addr, msg.sender), "Investor not alowed");
        ...
        emit IssuedUsingDex(msg.sender, _addr, _amount, _call.tokenAddress, fromAmount);
        ...
        if (LibReferralToken.isReferralToken(_addr) && LibReferralToken.getFeeValue(_addr) > 0) {
            ...
            emit LibReferralToken.ReferralFeeCharged(_addr, msg.sender, feeDistributor, _feeAmount);
        }

        // finally mint port token
        IPortToken(_addr).controllerMint(msg.sender, _amount);
