 ...
 *   2) Choose one of the tariff plans, enter the MATIC amount (15 MATIC minimum) using our website "Invest" button
 *   3) Wait for your earnings
 *   4) Withdraw earnings any time using our website "Withdraw" button
 *
 *   [INVESTMENT CONDITIONS]
 *
 *   - Basic interest rate: +4% every 24 hours - only for new deposits
 *   - Minimal deposit: 15 matic, no maximal limit
...
contract BullStaker {
	...
	uint256 constant public INVEST_MIN_AMOUNT = 15 ether;
	...
	constructor(address payable marketing, address payable dev, uint256 startDate)  {
		require(!isContract(dev));
		require(startDate > 0);
		devWallet = dev;
        marketingWallet = marketing;

		startUNIX = startDate; // 1650043800 
