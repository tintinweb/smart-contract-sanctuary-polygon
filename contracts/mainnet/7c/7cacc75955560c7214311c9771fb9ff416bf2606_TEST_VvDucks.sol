...
contract TEST_VvDucks is ERC721A, ReentrancyGuard, Ownable {
    using Strings for uint256;

    // Minting Variables
    uint256 public mintPrice = 0.001 ether;
    uint256 public maxPurchase = 200;
    ...
    mapping(uint256 => uint256[]) public tokenIdBurnLog;
    mapping(uint => string) public levelBaseTokenURIs;


    // Metadata
    string _baseTokenURI = "ipfs://QmQkakuXHuqXyb5N5oMfi4s2YMFGv7Lk8ahwtyCD12w5vi/";
    ...
    constructor() ERC721A("TESTINGVvDucks ", "QUACKS") {
    }
    ...
    external payable  {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            if (tokenIds[i] != _tokenID) {
                ...
                tokenIdBurnLog[_tokenID].push(tokenIds[i]);
            }
            else {
