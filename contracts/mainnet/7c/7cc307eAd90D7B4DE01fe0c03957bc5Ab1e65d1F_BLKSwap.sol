...
contract BLKSwap is Ownable {
    ...
    event SwapIntoBLK(address indexed account, address tokenIn, uint256 depositedAmount, int256 price);
    ...
    function changeEthOracle(address _newEthOracle) public onlyOwner() {
        ethOracle = _newEthOracle;
    }
    ...
    function getPriceFromOracle(address _token) public view returns (int256) {
    ...
    function estimateTokenIn(address _token, uint256 _amount) public view returns (uint256) {
        require(tokenOracles[_token] != address(0), "token not supported by swap");
        int256 price = getPriceFromOracle(_token);
        return _amount.mul(uint256(price));
    }

    function estimateEthIn(uint256 _amount) public view returns (uint256) {
        int256 price = getPriceFromOracle(ethOracle);
        return _amount.mul(uint256(price));
    }
    ...
    function swapTokenIn(address _token, uint256 _amount) public {
        require(tokenOracles[_token] != address(0), "token not supported by swap");
        int256 price = getPriceFromOracle(_token);
        emit SwapIntoBLK(msg.sender, _token, _amount, price);
    }

    function swapEthIn(uint256 _amount) public payable {
        require(msg.value == _amount, "incorrect amount sent");
        int256 price = getPriceFromOracle(ethOracle);
        emit SwapIntoBLK(msg.sender, address(0), _amount, price);
    }
