...
contract UniswapHelper is IUniswapV3MintCallback, IUniswapV3SwapCallback {
    ...
    int24 public tickRebalanceDeviation = 800; // 1000 from bounds
    ...
    function getPrincipal() external view returns (Position memory basePosition) {
        basePosition = _getPosition(baseTicks);
    }
    ...
    function getBalance(uint256 _fees, bool _oracle) external view returns (uint256) {
        ...
        uint256 token0In1 = token0To1(token0Bal, _oracle);
    ...
    function estimatePrincipalFees() public view returns (uint256) {
        (, int24 tick, , , , , ) = uniV3Pool.slot0();
        (uint256 token0, uint256 token1) = _estimateFees(baseTicks, tick);
        // Compute current fees earned
        return token1.add(token0To1(token0, false));
    }
    ...
    function token0Price(bool _oracle) public view returns (uint256) {
        (, int24 tick, , , , , ) = uniV3Pool.slot0();
        uint256 price =
    ...
    function token1To0(uint256 _token_1, bool _oracle) public view returns (uint256) {
        if (_token_1 == 0) return 0;
        return _token_1.mul(1e18).div(token0Price(_oracle));
    }

    function token0To1(uint256 _token_0, bool _oracle) public view returns (uint256) {
        if (_token_0 == 0) return 0;
        return token0Price(_oracle).mul(_token_0).div(1e18);
    }
    ...
    function uniswapV3SwapCallback(
        ...
        if (amount0Delta > 0)
            IERC20(TOKEN_0).safeTransfer(address(uniV3Pool), uint256(amount0Delta));
        if (amount1Delta > 0)
            IERC20(TOKEN_1).safeTransfer(address(uniV3Pool), uint256(amount1Delta));
        swapStorage = false;
    }
    ...
    function _unwindAllPositions() internal returns (uint256, uint256) {
    ...
    function _setOrders() internal {
        ...
        _balanceProportion(baseTicks); // NOTE here we check amounts and swap accordingly
        uint256 wantAmount = balanceOfToken0();
        uint256 wethAmount = balanceOfToken1();
    ...
    {
        (uint256 base0, uint256 base1) = _unwindAllPositions();
    ...
    function _calcPrincipalInWant() public view returns (uint256) {
        ...
        uint256 token0In1 = token0To1(base.token0, false);
    ...
    function _sendFundsToStrategy() internal {
        uint256 token0 = balanceOfToken0();
        uint256 token1 = balanceOfToken1();
        if (token0 > 0) {
    ...
    function _balanceProportion(Ticks memory _ticks) internal {
        uint256 amount0Desired = balanceOfToken0();
        uint256 amount1Desired = balanceOfToken1();

        // Get max liquidity for amounts available
        uint128 _liquidity = _liquidityForAmounts(_ticks, amount0Desired, amount1Desired);
        ...
        uint256 _amountSpecified =
            _zeroForOne
                ? (amount0Desired.sub(amount0.mul(basisOne + uniPoolFee / 2).div(basisOne)).div(2))
                : (amount1Desired.sub(amount1.mul(basisOne + uniPoolFee / 2).div(basisOne)).div(2));

        if (_amountSpecified > 0) {
            _swap(_amountSpecified, _zeroForOne);
        }
    }
    ...
    function _swap(uint256 _amountIn, bool _in) internal returns (int256, int256) {
        ...
        return
            uniV3Pool.swap(
                ...
                int256(_amountIn), // amountSpecified: the amount of the swap
    ...
    function _getPositionsTotalAssets() public view returns (uint256, uint256) {
        Position memory basePos = _getPosition(baseTicks);
        return (basePos.token0, basePos.token1);
    }
    ...
    function setTickWidth(int24 _tickWidth, int24 _maxTickDeviation) external onlyAuthorized {
