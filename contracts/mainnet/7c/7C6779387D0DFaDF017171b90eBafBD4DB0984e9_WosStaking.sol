...
contract WosStaking is OnlyAuthorized, CollateralWrapper {
    ...
    event sentToTheStaker(address staker, uint256 collateral);
    event StakeResult(string msg, uint256 distributedCollateral);
    event ERC20Change(address before, address newAddress);
    ...
    constructor(
        ...
        locked = false;
        emit ERC20Change(address(0), address(_erc20_contract_address));
    }
    ...
    function stakeTokens(
        IERC20 token,
        uint256 amount
    ) external timestampIsSet noReentrant {
        uac.verifyAll(msg.sender, 2);

        require(
            token == erc20Contract,
            "You are only allowed to stake the official WOS token address which was passed into this contract's constructor"
        );
        require(
            amount <= token.balanceOf(msg.sender),
            "Not enough Wos tokens in your wallet, please try lesser amount"
        );
        token.safeTransferFrom(msg.sender, address(this), amount);
        balances[msg.sender] = balances[msg.sender].add(amount);
        totalStakingBalance.add(amount);
    ...
    function unstakeTokens(
        ...
        uac.verifyGameStatus(2);
        signature.verifyMessage(
            abi.encodePacked(amount, staker).hash(),
            nonce,
            timestamp,
            signatures
        );
        _unstakeTokens(staker, token, amount);
    }
    ...
    function unstakeTokens(
        ...
        uac.verifyAll(msg.sender, 2);
        _unstakeTokens(msg.sender, token, amount);
    }
    ...
    function _unstakeTokens(
        ...
        IERC20 token,
        uint256 amount
    ) private timestampIsSet noReentrant {
        require(
        ...
        require(
            token == erc20Contract,
            "Token parameter must be the same as the WOS contract address which was passed into the constructor"
        );
        if (block.timestamp >= timePeriod) {
            alreadyWithdrawn[staker] = alreadyWithdrawn[staker].add(amount);
            balances[staker] = balances[staker].sub(amount);
            totalStakingBalance.sub(amount);
            ...
            emit TokensUnstaked(staker, amount);
        } else {
            revert(
    ...
    function removeStaker(address addr) private {
        if (stakers.length == 1 || stakersIndex[addr] == stakers.length) {
            stakers.pop();
            stakersIndex[addr] = 0;
            return;
        }

        uint256 index = stakersIndex[addr];
        address swapStaker = stakers[stakers.length - 1];
        stakers[index - 1] = swapStaker;
        stakers.pop();
        stakersIndex[addr] = 0;
        stakersIndex[swapStaker] = index;
    }

    function setERC20Contract(address addr) public onlyOwner {
        require(
            addr != address(0),
            "_erc20_contract_address address can not be zero"
        );

        emit ERC20Change(address(erc20Contract), addr);
    ...
    function distribute() external {
        ...
        if (availableCollateral < minBalance) {
            emit StakeResult(
        ...
        if (wagered == 0) {
            emit StakeResult("There is no staked wos", 0);
            return;
        }
        ...
        for (uint256 index = 0; index < stakers.length; index++) {
            ...
            uint256 percentageDistribute = (stakerBalance * 100) / wagered;
            uint256 toTransfer = (percentageDistribute * availableCollateral) /
                100;

            uint256 remaining = _collateralBalanceOf(address(this));
            _ctSign(
                string.concat(
                    "WS_RC_T_LSTEP_",
                    Strings.toString(index),
                    ";toTransfer=",
                    Strings.toString(toTransfer),
                    ";remaining=",
                    Strings.toString(remaining)
                )
            );
            _collateralTransfer(staker, toTransfer);

            collateralSent[staker] = collateralSent[staker] + toTransfer;
            distributed += toTransfer;
            emit sentToTheStaker(staker, toTransfer);
        }

        emit StakeResult(
            "Balances have been distributed to stakers",
            distributed
        );
    }

    function getStakedBalance(address staker) external view returns (uint256) {
        uint256 balance = balances[staker];
        return balance;
    }
