...
contract GiddyFarm is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  function initialize(address trustedForwarder, address token) public initializer {
    __Ownable_init();
    __Pausable_init();
  ...
  function withdraw(uint256 amount, uint256 fap) external whenNotPaused nonReentrant {
    ...
    updateFarm();
    amount = deductFee(GIDDY_TOKEN, amount, fap);
    SafeERC20Upgradeable.safeTransfer(IERC20Upgradeable(GIDDY_TOKEN), _msgSender(), amount);
    user.pending += ((user.amount * accumulatedRewards) / 1e12) - user.debt;
    user.amount -= amount;
    user.debt = (((user.amount) * accumulatedRewards) / 1e12);
