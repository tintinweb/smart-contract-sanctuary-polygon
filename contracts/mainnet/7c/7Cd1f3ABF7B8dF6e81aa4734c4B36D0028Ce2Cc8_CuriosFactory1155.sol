    ...
    CantBeEvilCuriosUpgradeable
{
    ...
    uint256 tokenTypesCount;
    mapping(uint256 => Initialization.TokenType) private tokenTypes;

    string baseURI;

    constructor(Initialization.Initialization1155 memory init) {
        initialize(init);
    }

    function initialize(
        Initialization.Initialization1155 memory init
    ) public initializer {
        __Ownable_init();
        __Pausable_init();

        //setting token URI is currently not supported
        __ERC1155_init(init.baseURI);
        ...
        for (uint256 i = 0; i < init.tokenCreators.length; i++) {
            _grantRole(TOKEN_CREATOR_ROLE, init.tokenCreators[i]);
        }
    ...
    //add role for only a token creator
    function createNewTokenType(
        uint256 maxSupply,
        uint256 mintPrice,
        bool isPublicMint
    ) public onlyRole(TOKEN_CREATOR_ROLE) {
        _createNewTokenType(maxSupply, mintPrice, isPublicMint);
    }
    ...
    function _createNewTokenType(
        ...
        Initialization.TokenType memory newGivingCircle = Initialization
            .TokenType(maxSupply, mintPrice, isPublicMint);
        tokenTypes[tokenTypesCount] = newGivingCircle;
        tokenTypesCount++;
    }

    function publicMint(uint256 id, uint256 amount) external payable {
        require(tokenTypes[id].isPublicMint, "PM1");
        require(msg.value >= tokenTypes[id].mintPrice, "PM2");

        _mintTo(msg.sender, id, amount);
    }
    ...
    function _mintTo(
        ...
        require(tokenTypesCount != 0, "MT1");
        require(id < tokenTypesCount);

        if (tokenTypes[id].maxSupply == 0) {
            _mint(addr, id, amount, "");
        } else {
            require(totalSupply(id) < tokenTypes[id].maxSupply, "M1");
            _mint(addr, id, amount, "");
        }
    }
    ...
    function publicMintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(tokenTypes[i].isPublicMint, "PM1");
        }

        uint256 amount = 0;
        for (uint256 i = 0; i < ids.length; i++) {
            amount += tokenTypes[ids[i]].mintPrice;
        }
        require(msg.value >= amount, "PM2");

        _mintToBatch(msg.sender, ids, amounts, data);
    }
    ...
    function _mintToBatch(
        ...
        for (uint256 i = 0; i < tokenTypesCount; i++) {
            require(ids[i] < tokenTypesCount);
        }

        for (uint256 i = 0; i < ids.length; i++) {
            if (tokenTypes[ids[i]].maxSupply == 0) {
                _mintBatch(to, ids, amounts, data);
            } else {
                require(
                    totalSupply(ids[i]) < tokenTypes[ids[i]].maxSupply,
                    "M1"
                );
                _mintBatch(to, ids, amounts, data);
            }
...
import "./Initialization/Initialization.sol";
import "./Curios1155.sol";

//add pausable
contract CuriosFactory1155 is AccessControl {
    ...
    mapping(uint256 => Curios1155) public instances;

    event CreatedNewInstance(address);

    constructor(address[] memory _admins, address[] memory _controllers) {
    ...
    function createNewInstance(
        ...
        Curios1155 instance = Curios1155(clone);
