    ...
    ITFM
{
    ...
    */
    function strategiesCompatible(
        ...
        require(
            strategies[_thisStrategyID].expiry > block.timestamp,
            "S1" // "strategies must be active"
        );
    }
    ...
    */
    ...
    function transfer(
        ...
        address initiator = Utils.checkTransferUserSignatures(
            _params,
            strategy.alpha,
            strategy.omega,
    ...
    */
    function combine(CombineParams calldata _params) external {
        strategiesCompatible(_params.thisStrategyID, _params.targetStrategyID);
    ...
    */
    function novate(NovateParams calldata _params) external {
        ...
        Utils.checkNovationSignatures(
            ...
            thisStrategy.transferable && targetStrategy.transferable,
            strategyNonce[_params.thisStrategyID],
            strategyNonce[_params.targetStrategyID]
        );
    ...
    */
    ...
    /**
        ...
        @param _strategyID the ID of the strategy to liquidate
        @param _newAmplitude the amplitude the strategy should have after liquidation
        @param _transferredCollateralAlpha collateral to be transferred from alpha to omega
        @param _transferredCollateralOmega collateral to be transferred from omega to alpha
        @param _confiscatedCollateralAlpha collateral to be confiscated from alpha (sent to treasury)
        @param _confiscatedCollateralOmega collateral to be confiscated from omega (sent to treasury)
    */
    function liquidate(
        uint256 _strategyID,
        int256 _newAmplitude,
        ...
        strategy.amplitude = _newAmplitude;
        ...
        collateralManager.liquidateExecute(
            LiquidateRequest(
                ...
                strategy.alpha,
                strategy.omega,
                _transferredCollateralAlpha,
                _transferredCollateralOmega,
                _confiscatedCollateralAlpha,
                _confiscatedCollateralOmega,
                strategy.basis
            )
        );
    }
...
interface ITFM {
    ...
    function liquidate(
        uint256 _strategyID,
        int256 _newAmplitude,
        uint256 _transferredCollateralAlpha, // one of these will always be zero; meaning -> collat trans from alpha to omega
        uint256 _transferredCollateralOmega, // one of these will always be zero
        uint256 _confiscatedCollateralAlpha, // meaning -> collat conf from alpha and put in the treasury
        uint256 _confiscatedCollateralOmega
    ) external;
}
    ...
    ICollateralManager
{
    ...
    */
    function withdraw(address _basisAddress, uint256 _amount) external {
        require(
            _amount <= unallocatedCollateral[msg.sender][_basisAddress],
            "C11" // "amount greater than unallocated collateral"
        );
        unallocatedCollateral[msg.sender][_basisAddress] -= _amount;
    ...
    /**
        ...
        @param _isEmergency true for an emergency update, false for a normal (scheduled) update
    */
    function updateCollateralNonce(bool _isEmergency) external isRelay {
        collateralNonce += (_isEmergency ? 2 : 1);
    }
    ...
    */
    ...
    /**
        ...
        @param _basis basis of the strategy
    */
    function allocateCollateral(
        ...
        _allocateCollateralUser(msg.sender, _toStrategyID, _amount, _basis);
    }
    ...
    */
        ...
        isTFM
    {
        ...
        // Allocate required collateral to newly minted strategy for msg.sender.
        ...
        //TODO: RENAME function, such termines create negative
    ...
    */
    function peppermintExecute(PeppermintRequest memory _req) external isTFM {
        ...
        uint256 unsignedPremium = _req.premium < 0
            ...
            : uint256(_req.premium);
        uint256 alphaCollateralRequirement = _req.alphaRequiredCollateral +
            (_req.premium < 0 ? unsignedPremium : 0) +
            _req.particleMass;
        uint256 omegaCollateralRequirement = _req.omegaRequiredCollateral +
            (_req.premium > 0 ? unsignedPremium : 0) +
            _req.particleMass;
    ...
    */
    function liquidateExecute(LiquidateRequest memory _req) external isTFM {
        ...
        if (_req.confiscatedCollateralAlpha > 0) {
            confiscateCollateral(
                _req.alpha,
                _req.strategyID,
        ...
        if (_req.confiscatedCollateralOmega > 0) {
            confiscateCollateral(
                _req.omega,
                _req.strategyID,
...
interface ICollateralManager {
     ...
     *  Cryptographic Collateral Verification
     ***********************************************/
    function updateCollateralNonce(bool _isEmergency) external;
    ...
    function combineExecute(CombineRequest memory _req)
        external;
