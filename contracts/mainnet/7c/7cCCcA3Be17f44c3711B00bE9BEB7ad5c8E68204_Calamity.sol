...
contract Calamity is Ownable{
    ...
    address public constant RELAYER = 0x725ce73ea2D3f666a4BF1b034CE72330b03Ab933;
        ...
        returns (bytes32, uint256)
    {
        ...
        bytes32 digest = keccak256(
                abi.encodePacked(
                "\x19\x01",
                skyToken.DOMAIN_SEPARATOR(),
                keccak256(abi.encode(skyToken.PERMIT_TYPEHASH(), user, RELAYER, amount, skyToken.nonces(user), deadline))
            )
        );
        return (digest, deadline);
    }

    function verifySignature(address user, uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public view returns(address) {
        return _verifySignature(user, amount, deadline, v, r, s);
    }
    ...
    function bless(uint256 amount, address user, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public notPaused notCapped {
        ...
        skyToken.permit(user, RELAYER, amount, deadline, v, r, s);
        skyToken.transferFrom(user, address(this), amount);
    }

    function _verifySignature(address user, uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) internal view returns(address) {
        unchecked {
            bytes32 digest = keccak256(
                abi.encodePacked(
                    "\x19\x01",
            ...
            require(recoveredAddress != address(0) && recoveredAddress == user, "INVALID_SIGNER");
