...
contract Staking is Ownable
    ...
    function stakeUnlock( uint256 unlockAmount, uint256 durationInDays )
        ...
        uint256 startingDay = ( block.timestamp - startTime + ONE_DAY ) / ONE_DAY;
    ...
    function setXUnlockForDayToTempSumAndSpecifyRewards( uint256 day, uint256 rewardAmount )
        ...
        require( day == ( lastDayWithRewardsSet + 1 ),
            "Staking::setRewardsForDay: days must be provided in sequence" );
    ...
    function getXUnlockAmountForStakeOnDay( Stake memory stake, uint256 onDay )
        private pure returns (uint256)
		{
        if ( stake.endingDay < onDay || onDay < stake.startingDay )
            return 0;
