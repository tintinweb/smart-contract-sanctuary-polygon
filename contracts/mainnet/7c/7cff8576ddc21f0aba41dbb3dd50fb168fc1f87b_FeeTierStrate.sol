...
contract FeeTierStrate is Ownable {
  ...
  function setMaxFee(uint256 newFee) public onlyOwner {
    MAX_FEE = newFee;
  }

  function setTotalFee(uint256 newFee) public onlyOwner {
    totalFee = newFee;
  }
  ...
  function insertTier(string memory title, address account, uint256 fee) public onlyOwner {
  ...
  function updateTier(uint256 index, string memory title, address account, uint256 fee) public onlyOwner {
    require(_feeTier[index].exist, "Only existing tier can be loaded");
    require(fee < MAX_FEE, "Fee tier value is overflowed");
