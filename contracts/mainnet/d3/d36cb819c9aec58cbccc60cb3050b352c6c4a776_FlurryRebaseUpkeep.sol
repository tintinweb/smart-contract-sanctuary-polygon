...
contract FlurryRebaseUpkeep is AccessControlEnumerableUpgradeable, IFlurryUpkeep {
    ...
    bool public pauseOnRebaseError;
    ...
    function checkPauseOnError(address underlying) internal {
        if (pauseOnRebaseError && address(pauser) != address(0)) {
            // Pause contract group if rebase operation is reverted
            pauser.pauseGroup(underlying);
        }
    }

    function setLastTimeStamp(uint256 _lastTimeStamp) external onlyRole(DEFAULT_ADMIN_ROLE) {
        lastTimeStamp = _lastTimeStamp;
    }

    function setRebaseInterval(uint256 interval) external onlyRole(DEFAULT_ADMIN_ROLE) {
        rebaseInterval = interval;
    }

    function registerVault(address vaultAddr) external onlyRole(DEFAULT_ADMIN_ROLE) notZeroAddress(vaultAddr) {
    ...
    function getPauser() external view returns (IPauser) {
        return pauser;
    }

    function setPauser(address addr) external onlyRole(DEFAULT_ADMIN_ROLE) notZeroAddress(addr) {
        pauser = IPauser(addr);
        emit PauserChanged(addr);
    }

    function setPauseOnRebaseError(bool value) external onlyRole(DEFAULT_ADMIN_ROLE) {
        pauseOnRebaseError = value;
    }
