...
contract ComplexRewarder is IRewarder, Ownable, ReentrancyGuard {
    ...
    event LogUpdatePool(uint indexed pid, uint lastRewardBlock, uint lpSupply, uint accRewardPerShare);
    ...
    function createChild(IERC20Ext _rewardToken, uint _rewardPerBlock) external onlyOwner {
        IRewarder child = new ChildRewarder(_rewardToken, _rewardPerBlock, MASTERCHEF_V2, address(this));
    ...
    /// @param _rewardPerBlock The amount of token to be distributed per second.
    ...
    function pendingToken(uint _pid, address _user) public view returns (uint pending) {
        ...
        if (block.timestamp > pool.lastRewardBlock && lpSupply != 0) {
            uint time = block.timestamp - pool.lastRewardBlock;
...
contract ChildRewarder is IRewarder, Ownable, ReentrancyGuard {
    ...
    constructor (IERC20Ext _rewardToken, uint _rewardPerBlock, address _MASTERCHEF_V2, address _PARENT) {
        uint decimalsRewardToken = _rewardToken.decimals();
        require(decimalsRewardToken < 30, "Token has way too many decimals");
        ACC_TOKEN_PRECISION = 10**(30 - decimalsRewardToken);
        rewardToken = _rewardToken;
