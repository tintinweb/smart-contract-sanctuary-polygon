...
contract GammaWbtcGiddyStrategy is GiddyStrategyV2, Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function withdraw(uint256 staked) external override nonReentrant onlyVault returns (uint256[] memory amounts) {
    ...
    SafeERC20Upgradeable.safeTransfer(IERC20Upgradeable(WBTC_TOKEN), address(vault), amounts[0]);
    SafeERC20Upgradeable.safeTransfer(IERC20Upgradeable(GIDDY_TOKEN), address(vault), amounts[1]);
  }
  ...
  function depositLP(uint256[] memory amounts) private returns (uint256 staked) {
    SafeERC20Upgradeable.safeApprove(IERC20Upgradeable(WBTC_TOKEN), POS, amounts[0]);
    SafeERC20Upgradeable.safeApprove(IERC20Upgradeable(GIDDY_TOKEN), POS, amounts[1]);
