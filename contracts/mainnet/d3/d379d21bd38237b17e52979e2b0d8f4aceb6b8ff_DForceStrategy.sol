...
contract DForceStrategy is UpgradeableBase, IStrategy {
    ...
    function setBLID(address _blid) external onlyOwner {
        require(_blid != ZERO_ADDRESS, "DF5");
    ...
    function setMultiLogicProxy(address _multiLogicProxy) external onlyOwner {
        require(_multiLogicProxy != ZERO_ADDRESS, "DF5");
    ...
    function setStrategyStatistics(address _strategyStatistics)
        ...
        onlyOwner
    {
        require(_strategyStatistics != ZERO_ADDRESS, "DF5");
     ...
     */
    function setSwapRewardsToBLID(
        SwapInfo memory swapInfo,
        uint256 _minimumBLIDPerRewardToken
    ) external onlyOwner {
        require(swapInfo.swapRouters.length == swapInfo.paths.length, "DF6");
        require(swapInfo.paths[0][0] == rewardsToken, "DF7");
        require(
     ...
     * @param swapInfo : addresses of swapRouter and path
     */
    function setSwapRewardsToStrategyToken(SwapInfo memory swapInfo)
        external
        onlyOwner
    {
        require(swapInfo.swapRouters.length == swapInfo.paths.length, "DF6");
    ...
    /**
     * @notice Set StrategyToken -> BLID swap information
     * @param swapInfo : addresses of swapRouter and path
     */
    function setSwapStrategyTokenToBLID(SwapInfo memory swapInfo)
        external
        onlyOwner
    {
        require(swapInfo.swapRouters.length == swapInfo.paths.length, "DF6");
        require(swapInfo.paths[0][0] == strategyToken, "DF7");
        require(
            swapInfo.paths[swapInfo.paths.length - 1][
                swapInfo.paths[swapInfo.paths.length - 1].length - 1
            ] == blid,
    ...
    function setStrategyXToken(address _xToken)
        ...
        onlyStrategyPaused
    {
        require(_xToken != ZERO_ADDRESS, "DF5");
    ...
    function rebalance() external override {
        ...
        if (borrowRate < _borrowRateMin) {
            ...
            uint256 buildAmount = ((((((totalSupply * targetBorrowRate) /
                BASE) * collateralFactor) / BASE) - borrowAmount) * BASE) /
        ...
        if (borrowRate > _borrowRateMax) {
            ...
            uint256 destroyAmount = ((borrowAmount -
                (((totalSupply * targetBorrowRate) / BASE) * LTV) /
        ...
        onlyMultiLogicProxy
    {
        ...
        uint256 destroyAmount = (borrowAmount * amount) /
    ...
    function _rewardsPriceKillSwitch(
        ...
        int256 delta = (_rewardsTokenPriceInfo.latestAnswer).toInt256() -
        ...
        if (
            ...
            delta =
                (delta * (1 ether)) /
                ((_rewardsTokenPriceInfo.latestAnswer).toInt256() *
                    ((block.timestamp).toInt256() -
                        (_rewardsTokenPriceInfo.timestamp).toInt256()));
        }
        ...
        if (_amountRewardsToken <= minRewardsSwapLimit) killSwitch = true;
    }
