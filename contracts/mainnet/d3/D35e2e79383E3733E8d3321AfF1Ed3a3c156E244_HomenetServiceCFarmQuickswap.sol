...
contract HomenetServiceCFarmQuickswap is HomenetService {
    ...
    {
        // swap bridged token to main inside a closed scope - protect stack.
        {
        ...
        IUniswapV2Pair pair = IUniswapV2Pair(CFarmQuickswap(cFarm).pair());

        CollectTokens._collect(pair.token0(), protocolFeeCollector);
        CollectTokens._collect(pair.token1(), protocolFeeCollector);
    }
        ...
        returns (QueueEntry[] memory outgoingQueue, uint256 quoteTokenAmount)
    {
        ...
        // swap main token to withdraw inside a closed scope - protect stack.
        {
            address tokenMain = CFarmQuickswap(cFarm).tokenMain();
            address tokenOther = CFarmQuickswap(cFarm).tokenOther();

            if (tokenMain == quoteToken) {
                quoteTokenAmount = tokenMainAmount;
                CollectTokens._collect(tokenOther, protocolFeeCollector);

            } else {
                IERC20Upgradeable(tokenMain).safeTransfer(swapHelper, tokenMainAmount);

                CollectTokens._collect(tokenMain, protocolFeeCollector);
                CollectTokens._collect(tokenOther, protocolFeeCollector);
