...
contract SwapRouter02 is V2SwapRouter, V2DfynRouter, V3SwapRouter, ApproveAndCall, MulticallExtended, SelfPermit {
    constructor(
        address _factoryV2,
        address factoryV3,
...
abstract contract V2DfynRouter is IDfynRouterV2, DfynImmutable {
    ...
    function payToken(
        ...
        if (token == wETH && address(this).balance >= value) {
            // pay with WETH
            vault.deposit{value: value}(address(0), payer, recipient, value, 0);
    ...
    /// @inheritdoc IDfynRouterV2
    function exactOutputSingle(ExactOutputSingleParamsDfyn calldata params)
        external
        payable
        override
        returns (uint256 amountIn)
    {
        ...
        amountIn = exactOutputInternal(
            ...
            params.recipient,
            params.unWrapVault,
            SwapCallbackDataDfyn({path: abi.encodePacked(params.tokenOut, params.tokenIn), payer: msg.sender})
        );
    ...
    /// @inheritdoc IDfynRouterV2
    function exactOutput(ExactOutputParamsDfyn calldata params) external payable override returns (uint256 amountIn) {
        ...
        exactOutputInternal(
            ...
            params.recipient,
            params.unWrapVault,
            SwapCallbackDataDfyn({path: params.path, payer: msg.sender})
        );
