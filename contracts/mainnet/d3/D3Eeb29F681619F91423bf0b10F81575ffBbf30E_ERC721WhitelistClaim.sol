...
contract ERC721WhitelistClaim is App,
__with_semver(
    uint8(0),
    uint8(0),
    uint8(0)
) {
    string constant public CLAIM_LIMIT_ERROR = "CLAIM_LIMIT_ERROR";
    uint8 public immutable CONTROLLER = _initNextRole();

    bool public isActive;

    bytes public merkleTreeURI;
    ...
    function claim(address to, bytes32[] calldata merkleProof, bytes32 tierValue) external {
        _requireActive();
        require(
            (++claims[to][tierValue]) <= claimLimitByTier[tierValue],
            CLAIM_LIMIT_ERROR
        );
        require(
            MerkleProof.verify(
                merkleProof,
                merkleTreeRoot,
                keccak256(abi.encodePacked(to, tierValue))
            ),
            Exceptions.VALIDATION_ERROR
        );
        ERC721Properties(_kernel.getAppAddress(AppsIds.ERC721)).mint(
            to, tierValue
        );
    }
