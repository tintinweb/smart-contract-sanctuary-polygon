...
contract IxsSale is Context, AccessControl, IIxsSale {
    ...
    function setNewDeadline(uint256 _saleId, uint8 index, uint256 newTimestamp) 
        external 
        onlyRole(OPERATOR_ROLE) 
    {
        require(index < 3 && index >= 0, "IxsSale: UNBOUND_INDEX");
        LaunchSale memory oldSale = onSales[_saleId];
        uint256 oldDeadline = oldSale.deadlines[index];
        uint256 prevDeadline = index > 0 ? oldSale.deadlines[index-1] : oldSale.deadlines[0];
        require(oldDeadline < newTimestamp && prevDeadline < newTimestamp, "IxsSale: INVALID_TIMESTAMP");
        LaunchSale storage sale = onSales[_saleId];
        sale.deadlines[index] = newTimestamp;
    }
        ...
        onlyRootUser(_saleId, _msgSender())
    {
        ...
        for (uint256 i; i < _receiver.length; i++) {
            ...
            if (isSuccessSale) {
                uint256 amountOut = getAmountOut(_saleId, amount);
        ...
        onlyRootUser(_saleId, _msgSender())
    {
        ...
        if (argsSale.dealBalance != sale.caps[2] && sale.tokenMinted) {
            ...
            uint256 secAmountOut = getAmountOut(
                _saleId, 
