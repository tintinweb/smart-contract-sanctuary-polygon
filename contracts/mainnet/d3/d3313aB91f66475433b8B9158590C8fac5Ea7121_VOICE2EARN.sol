...
contract VOICE2EARN is ERC721Enumerable {
    ...
    event SetTransferable(string videoId, uint256 price, bool status);
    event Acquire(
        string videoId,
        address seller,
    ...
    constructor(address _treasury, address _token)
        ERC721("VOICE2EARN Social Content", "VTESC")
    {
    ...
    function setTransferableStatus(
        ...
        require(
            _price >= 500000 && _price <= 2000000,
            "Price should be between 0.5 and 2"
        );

        NFTdetails[_tokenId].transferable = _status;
        NFTdetails[_tokenId].price = _price;
        NFTdetails[_tokenId].transferTimestamp = block.timestamp;

        emit SetTransferable(NFTdetails[_tokenId].videoId, _price, _status);
    }
    ...
    function acquire(
        ...
        uint256 buyingPrice = (_price * NFTdetails[_tokenId].price) / 1000000;

        acquire(_tokenId, buyingPrice, msg.sender);
    }

    function systemAcquire(
        uint256 _tokenId,
        uint256 _price,
        bytes32 hash,
        bytes memory signature
        ...
        require(matchAddresSigner(hash, signature), "WRONG_SIGNATURE");
        require(
            NFTdetails[_tokenId].transferTimestamp + 7 days < block.timestamp,
            "Not ready for system acquire"
        ); 

        acquire(_tokenId, _price, treasuryWallet);
    }
    ...
    function acquire(
        ...
        if (buyingPrice < 1 ether) {
            buyingPrice = 1 ether;
        }

        uint256 creatorFee = (buyingPrice * 10) / 100;
        uint256 adminFee = (buyingPrice * 10) / 100;
        ...
        IERC20(TPLToken).transferFrom(buyer, treasuryWallet, adminFee);
        IERC20(TPLToken).transferFrom(
            buyer,
            prevOwner,
        ...
        _transfer(prevOwner, buyer, _tokenId);
    ...
    function deleteComment(uint256 _tokenId, uint256 index) external {
        require(
            msg.sender == ownerOf(_tokenId) ||
                msg.sender == NFTdetails[_tokenId].minter,
    ...
    function getComments(uint256 tokenId)
        external
        view
        returns (Comments[] memory comments)
    {
    ...
    function getNftsOnSale()
        external
        view
        returns (uint256[] memory tokensId, uint256 itemsOnSale)
    {
        uint256 tokenCount = totalSupply();

        for (uint256 i = 1; i <= tokenCount; i++) {
            if (NFTdetails[i].transferable) {
                itemsOnSale++;
            }
        }

        uint256 currentIndex;
        tokensId = new uint256[](itemsOnSale);
    ...
    function _afterTokenTransfer(
        ...
        NFTdetails[tokenId].commentsActive = true;
        NFTdetails[tokenId].transferable = false;

        emit CommentStatusChanged(
            from,
            NFTdetails[tokenId].videoId,
            true,
            block.timestamp
        );
    }
