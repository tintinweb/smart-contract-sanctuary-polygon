...
function add(uint256[8] memory num1, uint256[8] memory num2) public pure returns(uint256[8] memory sum) { 
    uint256[8] memory intermediaries;
...
interface IERC20 {
    ...
    function totalSupply() external view returns (uint256[8] memory);
    function balanceOf(address account) external view returns (uint256[8] memory);
    function allowance(address owner, address spender) external view returns (uint256[8] memory);

    function transfer(address recipient, uint256[8] memory amount) external returns (bool);
    function approve(address spender, uint256[8] memory amount) external returns (bool);

    event Transfer(address indexed from, address indexed to, uint256[8] value);
    event Approval(address indexed owner, address indexed spender, uint256[8] value);
}
...
contract SampleToken is IERC20 {
    ...
    mapping(address => uint256[8]) balances;
    mapping(address => mapping (address => uint256[8])) allowed;

    uint256[8] totalSupply_;
    uint256[8] total;

    constructor(uint256[8] memory total) public {
        totalSupply_ = total;
        balances[msg.sender] = totalSupply_;
    }

    function totalSupply() public override view returns (uint256[8] memory) {
        return totalSupply_;
    }

    function balanceOf(address tokenOwner) public override view returns (uint256[8] memory) {
        return balances[tokenOwner];
    }

    function transfer(address receiver, uint256[8] memory numTokens) public override returns (bool) {
...
//        balances[receiver] = balances[receiver].add(numTokens);
    ...
    function approve(address delegate, uint256[8] memory numTokens) public override returns (bool) {
        allowed[msg.sender][delegate] = numTokens;
        emit Approval(msg.sender, delegate, numTokens);
        return true;
    }

    function allowance(address owner, address delegate) public override view returns (uint256[8] memory) {
