...
import {NFT, ERC2771Context, Ownable} from "./OwnerMint.sol";
import "@sbinft/contracts/access/Admin.sol";

contract NFTFactory is ERC2771Context, Admin,Ownable {
  constructor(address trustedForwarder) ERC2771Context(trustedForwarder) {
    _setChildTrusedForwarder(trustedForwarder);
  }

  //コントラクト情報格納配列
  mapping(address => address[]) private _NFTContracts;
   ...
   * TrustedForwarderも設定
   */
  function createContract(string calldata name, string calldata symbol)
    external
  {
    NFT nftContract = new NFT(name, symbol, _TrustedForwarder);
    nftContract.transferOwnership(_msgSender());

    address[] storage ownerContracts = _NFTContracts[_msgSender()];
    ownerContracts.push(address(nftContract));

    emit Created(address(nftContract), name, symbol, _msgSender());
  }

  function setChildTrustedforwarder(address ForwarderAddress) public onlyAdmin {
   ...
   */
  function getContractsByOwner(address owner_)
    public
    view
    returns (address[] memory)
  {
    return _NFTContracts[owner_];
  }
...
contract NFT is ERC721URIStorage, ERC2771Context, Ownable {
