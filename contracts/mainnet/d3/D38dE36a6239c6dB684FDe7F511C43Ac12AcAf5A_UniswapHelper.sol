...
contract UniswapHelper is IUniswapV3MintCallback {
    ...
    int24 public tickWidth = 1800; // for 2200 r = 1.2461 for 4000 r = 1.4918 for 1800 r = 1.1972
    int24 public limitTickWidth = 0;
    int24 public tickRebalanceDeviation = 400; // 10% of the width meanwhile
    int24 public tickLORebalanceDeviation = 200;
    uint16 public limitOrderMaxSize = 50;
    uint16 public limitOrderMinSize = 0;

    // Only set this to true externally when we TOKEN_0 to trigger our keepers to harvest for us
    uint256 public wantDust = 1e6;
    uint256 public wethDust = 1e6;
    uint256 internal factor = 500; // eta / 500
    ...
    address public TOKEN_0 = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; // Matic
    ...
    function name() external pure returns (string memory) {
        return 'Strategy Uniswap Helper v0.4';
    }

    function balanceOfToken0() public view returns (uint256) {
        return IERC20(TOKEN_0).balanceOf(address(this));
    }

    function balanceOfToken1() public view returns (uint256) {
        return IERC20(TOKEN_1).balanceOf(address(this));
    }
    ...
    function _calcLimitOrderRebalance(uint256 _eta) internal view returns (bool) {
        ...
        uint256 token_1_in_0 = _token_1_to_0(limitOrder.token1.add(strategy.balanceOfWeth()), true);
        uint256 token_0 = limitOrder.token0.add(token_1_in_0).add(strategy.balanceOfWant());
        // TODO: change this to make it more efficient and dynamic without he max Limit order size
        if (token_0 > _eta.div(factor)) {
            if (
    ...
    function getBalance(uint256 _fees, bool _oracle) external view returns (uint256) {
        (uint256 token0Bal, uint256 token1Bal) = _getPositionsTotalAssets();
        if (token0Bal == 0) return token1Bal;
        uint256 token0In1 = _token_0_to_1(token0Bal, _oracle);
        uint256 wethInUsdcMinusFees = token0In1.mul(_fees).div(basisOne);
        return wethInUsdcMinusFees.add(token1Bal);
    }
    ...
    function estimateLimitOrderFees() public view returns (uint256) {
        ...
        return wantFee.add(_token_1_to_0(wethFee, false));
    }
    ...
    function calcTickPrice(int24 _tick) public pure returns (uint256) {
        ...
        return (uint256(sqrtPrice).mul(uint256(sqrtPrice))).mul(1e18).div(uint256(2**192));
    }
    ...
    function uniswapV3MintCallback(
        ...
        mintStorage = false;
        if (amount0Owed > 0) IERC20(TOKEN_0).safeTransfer(address(uniV3Pool), amount0Owed);
        if (amount1Owed > 0) IERC20(TOKEN_1).safeTransfer(address(uniV3Pool), amount1Owed);
    }
    ...
    function _setOrders() internal {
        ...
        uint256 wantAmount = _balanceOfToken0();
        uint256 wethAmount = _balanceOfToken1();
    ...
    function _openPositions() public view returns (Position memory, Position memory) {
        return (_getPosition(baseTicks), _getPosition(limitTicks));
    }

    function _calcPrincipalInWant() public view returns (uint256) {
        Position memory base = _getPosition(baseTicks);
        if (base.liquidity == 0) return 0;
        uint256 token0In1 = _token_0_to_1(base.token0, false);
        return token0In1.add(base.token1);
    }

    function _balanceOfToken1() public view returns (uint256) {
        return IERC20(TOKEN_1).balanceOf(address(this));
    }

    function _balanceOfToken0() public view returns (uint256) {
        return IERC20(TOKEN_0).balanceOf(address(this));
    }
    ...
    function _sendFundsToStrategy() internal {
        uint256 token0 = _balanceOfToken0();
        uint256 token1 = _balanceOfToken1();
        if (token0 > 0) {
            IERC20(TOKEN_0).safeTransfer(address(strategy), token0);
        }
        if (token1 > 0) {
            IERC20(TOKEN_1).safeTransfer(address(strategy), token1);
        }
    ...
    {
        token0Balance = balanceOfToken0();
        token1Balance = balanceOfToken1();

        uint256 limitOrderSize = token0Balance.add(_token_1_to_0(token1Balance, false));
        uint16 divideBy = 1;

        uint256 eta = strategy.estimatedTotalAssets();

        uint256 maxOrderSize = eta.mul(limitOrderMaxSize).div(basisOne);

        if (limitOrderSize > maxOrderSize) divideBy = 2;

        if (token0Balance < wantDust) return (token0Balance, token1Balance.div(divideBy));
        if (token1Balance < wethDust) return (token0Balance.div(divideBy), token1Balance);
    }
    ...
    {
        (uint256 _token0Balance, uint256 _token1Balance) = _getLimitOrderAmounts();
        (Ticks memory bid, Ticks memory ask) = _getBidAsk();

        uint128 bidLiquidity = _liquidityForAmount0(bid, _token0Balance);
        uint128 askLiquidity = _liquidityForAmount1(ask, _token1Balance);
    ...
    function _token_0_Price(bool _oracle) internal view returns (uint256) {
        (, int24 tick, , , , , ) = uniV3Pool.slot0();
        uint256 price =
            _oracle ? aaveOracle.getAssetPrice(address(TOKEN_0)).div(100) : calcTickPrice(tick);
        return price;
    }

    function _token_1_to_0(uint256 _token_1, bool _oracle) internal view returns (uint256) {
        if (_token_1 == 0) return 0;
        return _token_1.mul(1e18).div(_token_0_Price(_oracle));
    }

    function _token_0_to_1(uint256 _token_0, bool _oracle) public view returns (uint256) {
        if (_token_0 == 0) return 0;
        return _token_0_Price(_oracle).mul(_token_0).div(1e18);
    }

    /**
     * @dev Returns the information about a position by the position's key
     */
    function _getPosition(Ticks memory _ticks) public view returns (Position memory) {
    ...
    function _getPositionsTotalAssets() public view returns (uint256, uint256) {
    ...
    function _getBidAsk() internal view returns (Ticks memory bid, Ticks memory ask) {
        (, int24 tick, , , , , ) = uniV3Pool.slot0();
        // NOTE The higher the tick the lower the price of TOKEN_1
        int24 tickFloor = _floor(tick);
        int24 tickCeil = tickFloor + tickSpacing;

        bid = Ticks({lowerTick: tickCeil, upperTick: tickCeil + tickSpacing + limitTickWidth});
        ask = Ticks({lowerTick: tickFloor - tickSpacing - limitTickWidth, upperTick: tickFloor});
    }
    ...
    function setLimitOrderSize(
        uint16 _limitOrderMaxSize,
        uint16 _limitOrderMinSize,
        uint256 _factor
    ) external onlyAuthorized {
        limitOrderMaxSize = _limitOrderMaxSize;
        limitOrderMinSize = _limitOrderMinSize;
        factor = _factor;
    }
}
