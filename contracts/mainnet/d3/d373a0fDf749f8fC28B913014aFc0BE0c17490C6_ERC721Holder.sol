...
/**
 * @dev Distribute a fixed reward in tokens to holder of ERC721 NFT, each NFT can claim the reward once.
 */
contract ERC721Holder is AbstractConditionOracle {
     ...
     * @param _defaultReward Default reward, unless overriden by adminSetReward.
     * @param _defaultConsumer address of contract which is authorized to call `consumeClaim`, pass address(0) to configure this later.
     */
    constructor(
        address _nftToken,
...
/**
 * @dev Shared configuration settings for condition oracles.
 */
abstract contract AbstractConditionOracle is IConditionOracle, Ownable {
     ...
     * @param _consumer Consumer address to authorize / revoke auth.
...
/**
 * @dev API of conditional oracle.
 */
interface IConditionOracle {
    /**
     * @dev Check if address is an authorized consumer for conditional distributor.
     * @param _consumer Address to check for consumer authorization.
     * @return true if authorized consumer.
     */
    function canConsumeClaims(address _consumer) external view returns (bool);

    /**
     * @dev Check if claim is valid for account.
     * @param _account Account owning the claim.
     * @param _claim ABI encoded claim with integrity hash.
     * @return true if claim is valid for account.
     */
    function hasClaim(address _account, bytes calldata _claim) external view returns (bool);

    /**
     * @dev Use claim, invalidating it in a process.
     *      Can be called only by conditional distributor contract.
     * @param _account Account owning the claim.
     * @param _claim ABI encoded claim with integrity hash.
