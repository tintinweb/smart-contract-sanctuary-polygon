...
// File contracts/SphereBondingCalculator.sol
...
contract SphereBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == SPHERE) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20(SPHERE).decimals() ) ).div( getTotalValue( _pair ) );
    }
}