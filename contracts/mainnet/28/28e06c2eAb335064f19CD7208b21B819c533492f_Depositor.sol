...
contract Depositor is Ownable {
	...
	event Deposit(address depositor, uint256 amount, address token, uint256 deadline);
	...
	function deposit(
		...
		IERC20(token).safeTransferFrom(msg.sender, _receiver, amount);
		emit Deposit(msg.sender, amount, token, deadline);
	}
