...
contract CustomBond is Ownable {
    ...
    IERC20 immutable public principalToken; // inflow token
    ITreasury immutable public customTreasury; // pays for and receives principal
    address public feeTreasury; //treasury trat receives any fees
    ...
    bool immutable public feeInPayout;
    ...
    /**
     *  @notice change address of the fee treasury
     *  @param _feeTreasury address
     */
    function changeFeeTreasury(address _feeTreasury) external onlyPolicy() {
        feeTreasury = _feeTreasury;
    }
    ...
    function deposit(uint _amount, uint _maxPrice, address _depositor) external returns (uint) {
        ...
        if(feeInPayout) {
            (payout, fee) = _payoutFor( value ); // payout and fee is computed
        } else {
            (payout, fee) = _payoutFor( _amount ); // payout and fee is computed
        ...
        if(feeInPayout) {
            ...
            if(fee != 0) { // if fee, send to the treasury
                payoutToken.safeTransfer(feeTreasury, fee);
            }
        } else {
            customTreasury.sendPayoutTokens( payout );
            if(fee != 0) { // if fee, send to the treasury
                principalToken.safeTransferFrom( msg.sender, feeTreasury, fee );
            }
        }
    ...
    function trueBondPrice() public view returns ( uint price_ ) {
        price_ = bondPrice().add(bondPrice().mul( currentFee() ).div( 1e6 ) );
    }
    ...
    /**
     *  @notice calculate user's interest due for new bond, without dividing by 1e11
     *  @param _value uint
     *  @return _payout uint
     *  @return _fee uint
     */
    function _payoutFor( uint _value ) public view returns ( uint _payout, uint _fee) {
        if(feeInPayout) {
            uint total = FixedPoint.fraction( _value, bondPrice() ).decode112with18();
            _fee = 0;
            _payout = total.sub(_fee);
        } else {
            _fee = 0;
            _payout = FixedPoint.fraction( customTreasury.valueOfToken(address(principalToken), _value.sub(_fee)), bondPrice() ).decode112with18();
        }
    }
     ...
     */
    function currentFee() public pure returns( uint currentFee_ ) {
        return 0;
    }
}