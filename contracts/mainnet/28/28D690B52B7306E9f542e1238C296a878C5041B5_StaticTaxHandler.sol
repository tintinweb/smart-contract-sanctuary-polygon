...
contract StaticTaxHandler is ITaxHandler, ExchangePoolProcessor {
    ...
    event BuyTaxBasisPointsUpdated(uint256 oldBasisPoints, uint256 newBasisPoints);

    /// @notice Emitted when the tax basis points number is updated.
    event SellTaxBasisPointsUpdated(uint256 oldBasisPoints, uint256 newBasisPoints);
    ...
    constructor(uint256 initialTaxBasisPoints) {
        buyTaxBasisPoints = initialTaxBasisPoints;
        sellTaxBasisPoints = initialTaxBasisPoints;
    }
    ...
    /**
     * @notice Set new number for buy tax basis points.
     * @param newBasisPoints New tax basis points number to set for calculations.
     */
    function setBuyTaxBasisPoints(uint256 newBasisPoints) external onlyOwner {
        uint256 oldBasisPoints = buyTaxBasisPoints;
        buyTaxBasisPoints = newBasisPoints;

        emit BuyTaxBasisPointsUpdated(oldBasisPoints, newBasisPoints);
    }

    /**
     * @notice Set new number for sell tax basis points.
     * @param newBasisPoints New tax basis points number to set for calculations.
     */
    function setSellTaxBasisPoints(uint256 newBasisPoints) external onlyOwner {
        uint256 oldBasisPoints = sellTaxBasisPoints;
        sellTaxBasisPoints = newBasisPoints;

        emit SellTaxBasisPointsUpdated(oldBasisPoints, newBasisPoints);
    }
