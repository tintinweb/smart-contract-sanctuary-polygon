...
contract ShredsToken is Ownable, ERC20 {

    uint256 public constant MAX_SHREDS = 1e26;
    ...
    /// @notice accounts that are allowed to mint or burn SHREDS
    mapping(address => bool) public isMinterBurner;

    /// @notice Construct ShredsToken contract for the in-game utility token for the Sheet Fighter game
    /// @dev    implicitly execute Ownable contructor - owner of this impl contract is the deployer
    constructor() ERC20('Shreds', 'SHREDS') Ownable() {}

    /// @notice Update the address of the PolygonSheetFighterToken contract
    /// @dev only relevant on Polygon
    ...
    function setMinterBurner(address account, bool status) external onlyOwner {
        isMinterBurner[account] = status;
    }

    /// @notice Generic SHREDS mint function
    /// @param to Account which receives the minted SHREDS
    /// @param value SHREDS amount to mint
    /// @dev The caller must have authorization (see `setMinterBurner`)
    function mint(address to, uint256 value) external {
        require(isMinterBurner[msg.sender], "Forbidden to mint");
        _mint(to, value);
    }
    ...
    /// @param value SHREDS amount to burn
    /// @dev The caller must have allowance for `from`'s tokens of at least `value`.
    /// @dev The caller must have authorization (see `setMinterBurner`)
    function burnFrom(address from, uint256 value) external {
        require(isMinterBurner[msg.sender], "Forbidden to burn");
    ...
    function _getMaximumRewards(uint256 calculatedRewards) internal view returns(uint256) {
        uint256 totalCellAvailable = MAX_SHREDS - totalSupply();
