...
// File: contracts_POL/main/QuickSwapStaking.sol
...
/** 
* @author Formation.Fi.
* @notice Implementation of the contract QuickSwapStaking;
*/
...
interface IStakingRewardsFactory {
    ...
    function stakingRewardsInfoByStakingToken(address key) view external 
    returns (StakingRewardsInfo memory);
}

contract QuickSwapStaking is Ownable {
    using SafeERC20 for IERC20;
    address public safeHouse;
    IStakingRewardsFactory public stakingRewardsFactory;
    Admin public admin;
    constructor (address _admin, address _safeHouse, address _stakingRewardsFactory) payable {
        ...
        require(
            _stakingRewardsFactory != address(0),
            "Formation.Fi: zero address"
        );
        safeHouse = _safeHouse;
        admin = Admin(_admin);
    ...
    function stake(
        ...
        stakingRewardsFactory.stakingRewardsInfoByStakingToken( _stakingToken).stakingRewards;
        IStakingRewards _stakingContract =  IStakingRewards(_stakingRewards);
        _stakingContract.stake(_amount);
    }
...
// **** UNSTAKE LP TOKEN ****
    function unstake(
        ...
        stakingRewardsFactory.stakingRewardsInfoByStakingToken( _stakingToken).stakingRewards;
        IStakingRewards _stakingContract =  IStakingRewards(_stakingRewards);
        _stakingContract.withdraw(_amount);
    }
...
// **** CLAIM REWARD ****
    function claimReward(
        ...
        stakingRewardsFactory.stakingRewardsInfoByStakingToken( _stakingToken).stakingRewards;
        IStakingRewards _stakingContract =  IStakingRewards(_stakingRewards);
        _stakingContract.getReward( );
    }

// **** GET REWARD ****
    function getReward(
        address _stakingToken
    ) public view  onlyManager {

        address _stakingRewards = 
        stakingRewardsFactory.stakingRewardsInfoByStakingToken( _stakingToken).stakingRewards;
        IStakingRewards _stakingContract =  IStakingRewards(_stakingRewards);
        _stakingContract.earned(address(this));
    }
