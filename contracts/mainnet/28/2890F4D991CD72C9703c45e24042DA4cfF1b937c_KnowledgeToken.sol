...
/**
 * Users can mint Knowledge Token NFT-s.
 * They can mint them only for existing Publications.
 */
contract KnowledgeToken is ERC721Enumerable, Ownable {
    event KnowledgeTokenMinted(uint256 indexed tokenId, uint256 indexed publicationId);
    event Withdraw(address indexed account, uint256 indexed amount);

    bytes32 public constant WRITER_ROLE = keccak256("WRITER_ROLE");

    uint256 public s_tokenCounter;
    // tokenId => publicationId
    mapping(uint256 => uint256) public s_tokenToPublication;
    mapping(uint256 => string) public s_tokenUri;

    // fees
    uint16 public s_adminFee = 50;
    mapping(address => uint256) public s_writerFeesAccrued;
    mapping(address => uint256) public s_writerFeesWithdrawn;

    // private
    IPublication private s_publication;
    ...
    function initialize(
        ...
        _initialized = true;
        s_tokenCounter = 0;
        s_publication = IPublication(publication);
    ...
    /**
     * Mint a new knowledge token for the reader.
     */
    function mint(uint256 publicationId) external payable returns (uint256) {
        require(s_publication.exists(publicationId), "Nonexistent publication");
        IPublication.PublicationInfo memory pub = s_publication.getPublicationInfo(publicationId);
        if (msg.value < pub.mintFee) revert Errors.NeedMoreETHSent();

        address pubOwner = s_publication.ownerOf(publicationId);

        s_writerFeesAccrued[pubOwner] += (msg.value * (1000 - s_adminFee)) / 1000;
        s_writerFeesAccrued[this.owner()] += (msg.value * s_adminFee) / 1000;

        uint256 tokenId = ++s_tokenCounter;
        s_tokenToPublication[tokenId] = publicationId;
        s_tokenUri[tokenId] = pub.uri;

        _mint(msg.sender, tokenId);
        emit KnowledgeTokenMinted(tokenId, publicationId);

        return tokenId;
    }

    function getWithdrawableAmount() public view returns (uint256) {
        uint256 accrued = s_writerFeesAccrued[msg.sender];
        uint256 withdrawn = s_writerFeesWithdrawn[msg.sender];
        uint256 amountToWithDraw = accrued - withdrawn;
    ...
    function withdraw() external {
        ...
        s_writerFeesWithdrawn[msg.sender] += amountToWithDraw;
    ...
    /**
     * 0-1000. Default is 5.0.
     */
    function setAdminFee(uint16 adminFee) external onlyOwner {
        require(adminFee < 1000, "Admin fee should be between 0 and 1000");
        s_adminFee = adminFee;
    }
