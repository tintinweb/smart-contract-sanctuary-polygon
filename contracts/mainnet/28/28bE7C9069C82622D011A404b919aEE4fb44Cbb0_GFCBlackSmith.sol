...
contract GFCBlackSmith is Ownable, Whitelist, VRFConsumerBase {
    ...
    function forgeAsteroids() public {
        ...
        chargeGCOIN();
        getRandNum4Forge();
        addressForgeInfo[msg.sender] = ForgeInfo(1, 6, 3);
    }
    ...
    function smith(uint256 transactionId) public {
        ...
        if(fInfo.forgeType == 1) {
            ...
            emit WeaponForged(msg.sender, tokenId, transactionId);
        }else if(fInfo.forgeType == 3) {
            smithAsteroids(transactionId);
        }else {
            revert();
        }
    ...
    function smithAsteroids(uint256 transactionId) internal{
        uint256 tokenId = mintNextTier(getResult(msg.sender)%2, 6);
        emit WeaponForged(msg.sender, tokenId, transactionId);
        mysteryItem.devMint(msg.sender, asteroidsForgeRewardId, 1);
        emit RewardMinted(msg.sender, asteroidsForgeRewardId, transactionId);
    }
