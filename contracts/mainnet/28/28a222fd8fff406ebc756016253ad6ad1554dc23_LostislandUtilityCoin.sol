...
// File: contracts/LostislandUtility.sol


pragma solidity ^0.8.2;


contract LostislandUtilityCoin is ERC20, AccessControl, ERC721Holder {
    bytes32 public constant CONTRACT_ADMIN_ROLE = keccak256("CONTRACT_ADMIN_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    address public collectionAddressLostislandClubNFT;
    address public collectionAddressNFTv2;

    uint256 public numberOfBlocksPerRewardUnit;

    uint256 public coinAmountPerRewardUnitLostislandClubNFT;
    uint256 public coinAmountPerRewardUnitNFTv2;

    uint256 public welcomeBonusAmountLostislandClubNFT;
    uint256 public welcomeBonusAmountNFTv2;

    uint256 public amountOfStakersLostislandClubNFT;
    uint256 public amountOfStakersNFTv2;

    uint256 public tokensStakedLostislandClubNFT;
    ...
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogLostislandClubNFT;

        /// owner => tokenId => StakeInfo
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogNFTv2;

    /// owner => #NFTsStaked
    mapping (address => uint256) public tokensStakedByUserLostislandClubNFT;
    ...
    mapping (uint256 => bool) public welcomeBonusCollectedLostislandClubNFT;

        /// tokenId => true/false (true if welcome bonus has been collected for a specific tokenId)
    mapping (uint256 => bool) public welcomeBonusCollectedNFTv2;

    /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserLostislandClubNFT;

        /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserNFTv2;

    /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioLostislandClubNFT;

        /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioNFTv2;

    event RewardsHarvestedLostislandClubNFT (address owner, uint256 amount);
    event NFTStakedLostislandClubNFT (address owner, uint256 tokenId);
    event NFTUnstakedLostislandClubNFT (address owner, uint256 tokenId);
    ...
    constructor(address owner, address _collectionAddressLostislandClubNFT, address _collectionAddressNFTv2) ERC20("LostislandUtility", "LIU") AccessControl(){
        _mint(owner, 100000 * 10 ** 18);
        _setupRole(DEFAULT_ADMIN_ROLE, owner);
        _setupRole(CONTRACT_ADMIN_ROLE, owner);
        _setupRole(BURNER_ROLE, owner);
        _setupRole(MINTER_ROLE, owner);
        collectionAddressLostislandClubNFT = _collectionAddressLostislandClubNFT;
        collectionAddressNFTv2 = _collectionAddressNFTv2;
        contractCreationBlock = block.number;
        coinAmountPerRewardUnitLostislandClubNFT = 15 * 10 ** 18; // 5 ERC20 coins per rewardUnit, may be changed later on
        ...
        welcomeBonusAmountLostislandClubNFT = 300 * 10 ** 18; // 300 tokens welcome bonus, only paid once per tokenId
        welcomeBonusAmountNFTv2 = 600 * 10 ** 18; // 600 tokens welcome bonus, only paid once per tokenId
    }

    function stakedNFTSByUserLostislandClubNFT(address owner) external view returns (uint256[] memory){
        return stakePortfolioByUserLostislandClubNFT[owner];
    }
    ...
    function pendingRewardsLostislandClubNFT(address owner, uint256 tokenId) public view returns (uint256){
        StakeInfo memory info = stakeLogLostislandClubNFT[owner][tokenId];
        ...
        return rewardAmount * coinAmountPerRewardUnitLostislandClubNFT;
    ...
    function stakeLostislandClubNFT(uint256 tokenId) public {
        IERC721(collectionAddressLostislandClubNFT).safeTransferFrom(_msgSender(), address(this), tokenId);
        require(IERC721(collectionAddressLostislandClubNFT).ownerOf(tokenId) == address(this),
            "LostislandUtility: Error while transferring token");
        StakeInfo storage info = stakeLogLostislandClubNFT[_msgSender()][tokenId];
        info.stakedAtBlock = block.number;
        info.lastHarvestBlock = block.number;
        info.currentlyStaked = true;
        if(tokensStakedByUserLostislandClubNFT[_msgSender()] == 0){
            amountOfStakersLostislandClubNFT += 1;
        }
        tokensStakedByUserLostislandClubNFT[_msgSender()] += 1;
        tokensStakedLostislandClubNFT += 1;
        stakePortfolioByUserLostislandClubNFT[_msgSender()].push(tokenId);
        uint256 indexOfNewElement = stakePortfolioByUserLostislandClubNFT[_msgSender()].length - 1;
        indexOfTokenIdInStakePortfolioLostislandClubNFT[tokenId] = indexOfNewElement;
        if(!welcomeBonusCollectedLostislandClubNFT[tokenId]) {
            _mint(_msgSender(), welcomeBonusAmountLostislandClubNFT);
            welcomeBonusCollectedLostislandClubNFT[tokenId] = true;
        }

        emit NFTStakedLostislandClubNFT(_msgSender(), tokenId);
    }
    ...
    function stakeBatchLostislandClubNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            stakeLostislandClubNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakeBatchLostislandClubNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            unstakeLostislandClubNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakeLostislandClubNFT(uint256 tokenId) public {
        if(pendingRewardsLostislandClubNFT(_msgSender(), tokenId) > 0){
            harvestLostislandClubNFT(tokenId);
        }
        StakeInfo storage info = stakeLogLostislandClubNFT[_msgSender()][tokenId];
        info.currentlyStaked = true;
        IERC721(collectionAddressLostislandClubNFT).safeTransferFrom(address(this), _msgSender(), tokenId);
        require(IERC721(collectionAddressLostislandClubNFT).ownerOf(tokenId) == _msgSender(),
            "LostislandUtility: Error while transferring token");
        if(tokensStakedByUserLostislandClubNFT[_msgSender()] == 1){
            amountOfStakersLostislandClubNFT -= 1;
        }
        tokensStakedByUserLostislandClubNFT[_msgSender()] -= 1;
        tokensStakedLostislandClubNFT -= 1;
        stakePortfolioByUserLostislandClubNFT[_msgSender()][indexOfTokenIdInStakePortfolioLostislandClubNFT[tokenId]] = 0;
        emit NFTUnstakedLostislandClubNFT(_msgSender(), tokenId);
    }
    ...
    function harvestLostislandClubNFT(uint256 tokenId) public {
        StakeInfo storage info = stakeLogLostislandClubNFT[_msgSender()][tokenId];
        uint256 rewardAmountInERC20Tokens = pendingRewardsLostislandClubNFT(_msgSender(), tokenId);
        if(rewardAmountInERC20Tokens > 0) {
            info.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInERC20Tokens);
            emit RewardsHarvestedLostislandClubNFT(_msgSender(), rewardAmountInERC20Tokens);
        }
    }
    ...
    function harvestBatchLostislandClubNFT(address user) external {
        uint256[] memory tokenIds = stakePortfolioByUserLostislandClubNFT[user];

        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            harvestLostislandClubNFT(tokenIds[currentId]);
        }
    }
    ...
    function setCoinAmountPerRewardUnitLostislandClubNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        coinAmountPerRewardUnitLostislandClubNFT = coinAmount;
    }
    ...
    function setWelcomeBonusAmountLostislandClubNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        welcomeBonusAmountLostislandClubNFT = coinAmount;
    }
    ...
    function setCollectionAddressLostislandClubNFT(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "VIBE: update to zero address not possible");
        collectionAddressLostislandClubNFT = newAddress;
    }
