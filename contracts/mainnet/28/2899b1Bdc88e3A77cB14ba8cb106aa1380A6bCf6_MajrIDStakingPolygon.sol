 ...
 * @title MajrIDPolygon
 * @author MAJR, Inc.
 * @notice This contract is used to stake MAJR IDs and earn rewards in the form of MAJR's native ERC2O tokens
 */
contract MajrIDStakingPolygon is Ownable, ReentrancyGuard, Pausable {
  ...
  uint256 public rewardsDuration = 2922 days; // ~ 8 years, accounting for the leap years of 2024 & 2028
  ...
  function notifyRewardAmount(
    ...
    require(
      ...
      "MajrStaking: Provided reward too high"
    );
  ...
  function setRewardsDuration(uint256 _rewardsDuration) external onlyOwner {
    require(
      ...
      "MajrStaking: Previous rewards period must be complete before changing the duration for the new period."
    );
  ...
  /**
   * @notice Removes staking tokens and transfers them back to the staker
   * @notice Users can only withdraw while the voting in the MAJR Canon (governance) contract is not active
   * @param _amount uint
   * @dev Updates rewards on call
   */
  function withdraw(
    ...
    require(
      ...
      "MajrStaking: Cannot withdraw more tokens than staked."
    );
    require(
      isVotingActive() == false,
      "MajrStaking: Cannot withdraw staked MAJR IDs while voting is active."
    );
    ...
    for (uint256 i = 0; i < _amount; i++) {
      uint256 tokenId = userStakedTokenIds[msg.sender][
        userStakedTokenIds[msg.sender].length - 1
      ];
  ...
  /**
   * @notice Returns whether or not the voting is currently active in the MAJR Canon contract
   * @notice Voting is considered to be active only if the MAJR Canon contract is not paused
   * @return bool
   */
