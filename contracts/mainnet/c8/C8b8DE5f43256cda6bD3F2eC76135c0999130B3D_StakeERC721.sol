...
contract StakeERC721 is Owner {
    ...
    function unstakeAll(address tokenAddress) public {
        ...
        for (
            ...
            uint256 bt = block.timestamp - tokenIdToTimeStamp[tokenAddress][tokenId];
            if (bt >= 86400) bt = 86400;
            totalRewards =
                totalRewards +
                (bt * tokenAddressEmissionsRate[tokenAddress]);
        ...
        if (
            ...
            IERC20(pdToken).transfer(inviter, (fi * 60) / 100);
            IERC20(ngcToken).transfer(inviter, (fi * 40) / 100);

            templar.setUserReward(msg.sender, (fi * 60) / 100, (fi * 40) / 100);

            rate += inviteefee;
        }

        uint256 ff = (totalRewards * rate) / 100;
        IERC20(pdToken).transfer(feeAccount, (ff * 60) / 100);
        IERC20(ngcToken).transfer(feeAccount, (ff * 40) / 100);

        uint256 fm = (totalRewards * (100 - rate)) / 100;
        IERC20(pdToken).transfer(msg.sender, (fm * 60) / 100);
        IERC20(ngcToken).transfer(msg.sender, (fm * 40) / 100);
    }
        ...
        public
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            uint256 bt = block.timestamp - tokenIdToTimeStamp[tokenAddress][tokenIds[i]];
            if (bt >= 86400) bt = 86400;
            totalRewards =
                totalRewards +
                (bt * tokenAddressEmissionsRate[tokenAddress]);
    ...
    function claimByTokenId(address tokenAddress, uint256 tokenId) public {
        ...
        uint256 bt = block.timestamp - tokenIdToTimeStamp[tokenAddress][tokenId];
        if (bt >= 86400) bt = 86400;
        uint256 totalRewards = (bt * tokenAddressEmissionsRate[tokenAddress]);
    ...
    function getAllRewards(address staker) public view returns (uint256) {
        ...
        for (uint256 ind = 0; ind < stakeNFTList.length; ind++) {
            ...
            for (uint256 i = 0; i < tokenIds.length; i++) {
                 uint256 bt = block.timestamp - tokenIdToTimeStamp[stakeNFTList[ind]][tokenIds[i]];
                if (bt >= 86400) bt = 86400;
                totalRewards =
                    totalRewards +
                    (bt * tokenAddressEmissionsRate[stakeNFTList[ind]]);
            }
        }
        return totalRewards;
    }
        ...
        returns (uint256)
    {
        ...
        if (secondsStaked >= 86400) secondsStaked = 86400;
