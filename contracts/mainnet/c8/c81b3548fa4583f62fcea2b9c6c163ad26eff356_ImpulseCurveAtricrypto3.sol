...
// File: ImpulseCurveAtricrypto3.sol
...
contract ImpulseCurveAtricrypto3 is BaseImpulseMultiStrategy {
    ...
    function wantPriceInUsd(uint256 _wantAmt) external view virtual override returns (uint256) {
        ...
        uint256 priceWbtcInUsdc = getWbtcPriceInUsd(underlyingAmounts[3]);
        uint256 priceWethInUsdc = getWethPriceInUsd(underlyingAmounts[4]);
        wantPrice += priceWbtcInUsdc;
        wantPrice += priceWethInUsdc;
        return wantPrice;
    }

    function getWbtcPriceInUsd(uint256 underlyingAmountWbtc) public view returns (uint256) {
        address[] memory path = new address[](3);
        path[0] = address(underlyingTokens[3]);
        path[1] = address(underlyingTokens[4]);
        path[2] = address(underlyingTokens[1]);
        uint256 priceWbtcInUsdc = IUniswapV2Router01(router).getAmountsOut(underlyingAmountWbtc, path)[2];
        priceWbtcInUsdc *= 10**12; // Since USDC has 6 decimals
        return priceWbtcInUsdc;
    }

    function getWethPriceInUsd(uint256 underlyingAmountWeth) public view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = address(underlyingTokens[4]);
        path[1] = address(underlyingTokens[1]);
        uint256 priceWethInUsdc = IUniswapV2Router01(router).getAmountsOut(underlyingAmountWeth, path)[1];
        priceWethInUsdc *= 10**12; // Since USDC has 6 decimals
        return priceWethInUsdc;
    }
    ...
    function _swapRewardsToUnderlings(uint256[] memory _rewardAmts) internal virtual override {
        uint256 u = 4; //WETH in underlying list
