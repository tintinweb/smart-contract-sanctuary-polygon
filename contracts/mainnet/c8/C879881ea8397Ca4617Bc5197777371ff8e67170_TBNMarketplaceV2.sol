...
struct PriceCalculation {
  uint256 tbnReservePrice;
  uint256 paymentReservePrice;
...
contract TBNMarketplaceV2 is ERC721Holder, AccessControl, ReentrancyGuard {
  ...
  event ListingSold(uint256 listingId, address soldTo);
  ...
  */
    ...
    nonReentrant
  {
    require(listingIds.contains(listingId), "Listing does not exist.");
    /*
          calculate price according to oracle
          expected to return price in uint256

          use safe math for below as not solidity version 8
          listing.priceBandBasisPoint would be basisPoints compared to market price from oracle
          get percentage with listing.priceBandBasisPoint.mul(${oraclePrice}).div(10000)

          getting oracle price will cost more gas

          take max between reserve price or price bound amount
          use amount for rest of calculations

          ie. 10% below market price - price of token = 1$
              (90%) * 1BNB / 10000 basis point
              9000 * 10^18 / 10000 = (9 * 10^17)
              13% above market price - price of token = 21$
              11,300 * (21*10^18) / 10,000 = 23.73 * 10^18

          for testing - create dummy contract stupid oracle - hardcoded value
        */
    ...
    uint256 fullCost;
    /** 
      If token address associated with valid chainlink token symbol
      ~~~~~ BOTH PAYMENT AND TBN TOKEN FOR ABOVE~~~~~~
      Get token price from chainlink oracle
      Use chainlink oracle instead of uniswap
      fullPrice = price from oracle * # of tokens in TBN
    */
    if (
      ...
      fullCost =
        (paymentPrice * TBNAmount * listing.priceBandBasisPoint) /
        (tbnPrice * 10000);
      ...
      require(
        fullCost <
          ((expectedPaymentAmount * (10000 + errorMarginBasisPoint)) / 10000),
        "chain Oracle price above error margin"
      );
      require(
        fullCost > ((expectedPaymentAmount * errorMarginBasisPoint) / 10000),
        "chain Oracle price below error margin"
      );
    } else if (listing.isPriceBound) {
      ...
      // mul by 1000000000000000000 to prevent underflow or overflow
      uint256 tbnReservePrice = (tbnTokenReserves * 1 ether) / tbnWETHReserves;
      uint256 paymentReservePrice = (paymentTokenReserves * 1 ether) /
        paymentWETHReserves;
      fullCost =
        (paymentReservePrice * TBNAmount * listing.priceBandBasisPoint) /
        (tbnReservePrice * 10000);
    ...
    uint256 payoutToSeller = (fullCost.mul((10000 - basisPointFee))).div(10000);
