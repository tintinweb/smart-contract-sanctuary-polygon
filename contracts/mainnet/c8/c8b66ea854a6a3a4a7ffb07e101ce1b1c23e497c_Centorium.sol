...
contract Centorium {
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		require(msg.value >= INVEST_MIN_AMOUNT, "min amount is 0.5 matic");
	...
	function getUserHoldBonus(address userAddress) public view returns(uint256) {
		uint256 bonus = 0;
		if(block.timestamp > users[userAddress].checkpoint && users[userAddress].deposits.length > 0){
	...
	function getSiteInfo() public view returns(uint256 _totalInvested, uint256 _totalBonus, uint256 _totalWithdraw, uint256 _contractBalance) {
		return(totalInvested, totalReferral, totalWithdraw, getContractBalance());
	}
