...
contract MyToken is ERC20Drop {
    ...
    function getQuickswapPair() external view returns (address) {
        return quickswapPair;
    }

    function setQuickswapPair(address _quickswapPair) external onlyOwner {
        require(quickswapPair == address(0), "Quickswap Pair address already set");
        quickswapPair = _quickswapPair;
    }

    function setSellFees(uint256 _sellFeePercentage, bool _sellFeesEnabled) external onlyOwner {
        require(_sellFeePercentage <= 100, "Sell fee percentage cannot exceed 100");
        sellFeePercentage = _sellFeePercentage;
        sellFeesEnabled = _sellFeesEnabled;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        if (recipient == quickswapPair && !isExcludedFromFee[sender] && sellFeesEnabled) {
