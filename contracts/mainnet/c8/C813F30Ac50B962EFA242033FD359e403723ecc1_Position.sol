...
contract Position is ERC721Enumerable, MetaContext, IPosition {
    ...
    mapping(uint256 => mapping(address => mapping(bool => int256))) private totalAccInterest;
    ...
    function trades(uint _id) public view returns (Trade memory) {
        ...
        uint256 _sideOi;
        int256 _pendingFunding = int256(block.timestamp-lastUpdate[_trade.asset][_trade.tigAsset])*fundingDeltaPerSec[_trade.asset][_trade.tigAsset];
        if (_trade.direction) {
            _pendingFunding = -_pendingFunding;
            _sideOi = longOi[_trade.asset][_trade.tigAsset];
        } else {
            _sideOi = shortOi[_trade.asset][_trade.tigAsset];
        }
        if (_sideOi > 0) {
            _trade.accInterest = ((totalAccInterest[_trade.asset][_trade.tigAsset][_trade.direction] - initId[_id]) + _pendingFunding)*int256(_trade.margin*_trade.leverage)/int256(_sideOi*1e18);
        }
        return _trade;
    }
    ...
    function updateFunding(uint256 _asset, address _tigAsset, uint256 _longOi, uint256 _shortOi, uint256 _baseFundingRate) external onlyMinter {
        totalAccInterest[_asset][_tigAsset][true] -= int256(block.timestamp-lastUpdate[_asset][_tigAsset])*fundingDeltaPerSec[_asset][_tigAsset];
        totalAccInterest[_asset][_tigAsset][false] += int256(block.timestamp-lastUpdate[_asset][_tigAsset])*fundingDeltaPerSec[_asset][_tigAsset];
        lastUpdate[_asset][_tigAsset] = block.timestamp;
        int256 _oiDelta = int256(_longOi)-int256(_shortOi);
        fundingDeltaPerSec[_asset][_tigAsset] = (_oiDelta*int256(_baseFundingRate)/int256(DIVISION_CONSTANT))/int256(31536000);
    ...
    */
    function mint(
        ...
        if (_mintTrade.orderType > 0) {
            ...
            initId[newTokenID] = totalAccInterest[_mintTrade.asset][_mintTrade.tigAsset][_mintTrade.direction];
    ...
    function executeLimitOrder(uint256 _id, uint256 _price, uint256 _newMargin) external onlyMinter {
        ...
        _trade.orderType = 0;
        _trade.price = _price;
        initId[_id] = totalAccInterest[_trade.asset][_trade.tigAsset][_trade.direction];
    ...
    function reducePosition(uint256 _id, uint256 _percent) external onlyMinter {
        ...
        initId[_id] += initId[_id]*int(_percent)/int(DIVISION_CONSTANT);
    }
