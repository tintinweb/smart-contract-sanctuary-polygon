    ...
    CommonConstants
{
    ...
    */
    ...
    {
        require(
            tx.origin == getContractAddress(1002),
            "Unauthorized non-meta deposit"
        );
        require(
    ...
    /**
        ...
        On-chain randomness:
        A Serran sends the sig (serranSignature, unique) of his input data. (The increasing nonce makes the sig always unique)
        ...
        Additionally, the randomness function combines seedSignature with gasleft() so that assigning multiple positions within one tx is still random.
        @param  userAddress     The user's address to be assigned
        @param  amount          The amount of crates
        @param  serranSignature The signature used for the crate transfer
        @param  seedSignature   The seed which the randomness is based on
    */
    function assignRandomPosition(
        address userAddress,
        uint256 amount,
        bytes memory serranSignature,
        bytes memory seedSignature
    ) external isActive isAuthorizedCrateOpener {
        bytes32 dataHash = _getSeedHash(userAddress, amount, serranSignature);
        ...
        require(amount % DECIMALS == 0, "Keine halben Sachen");

        uint256 crateAmount = amount / DECIMALS;
        for (uint256 i = 0; i < crateAmount; i++) {
            uint256 randomNumber = _getRandomUnit256(seedSignature);
    ...
    */
    function _getSeedHash(
        ...
        return keccak256(abi.encodePacked(signer, amount, signature));
    }
    ...
    */
    function _getRandomUnit256(bytes memory seed)
        private
        view
        returns (uint256)
    {
        return uint256(keccak256(abi.encodePacked(seed, gasleft())));
    }
