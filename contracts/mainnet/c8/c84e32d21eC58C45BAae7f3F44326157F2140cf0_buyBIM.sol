...
contract buyBIM is Ownable {
        ...
        internal
    {
        ...
        uint256 BIMamount = (_USDamount*1000000000000000000)/rateUSDtoBIM;
        require(BIM.balanceOf(address(this)) >= BIMamount, "not enough BIM on the contract");
        require(USD.transferFrom(msg.sender, receiverWalletAddress, _USDamount), "USD transaction failed");
        require(BIM.transfer(msg.sender, BIMamount), "BIM transaction failed");
    }
    ...
    function setMinUSD(uint256 _minUSD)
        external
        onlyOwner
    {
        minUSD = _minUSD;
    ...
    function setMaxUSD(uint256 _maxUSD)
        external
        onlyOwner
    {
        maxUSD = _maxUSD;
