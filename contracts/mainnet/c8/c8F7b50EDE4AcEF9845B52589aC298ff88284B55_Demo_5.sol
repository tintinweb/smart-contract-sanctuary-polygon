...
contract Demo_5 is Ownable, ChainlinkClient {
    using Chainlink for Chainlink.Request;
    using CBORChainlink for BufferChainlink.buffer;

    /* ========== usage: record everything on chain ========== */
    // mapping(bytes32 => string) public stakeGame
    // mapping(bytes32 => string) public stakeSelection

    string[] verifiedGameId;
    uint256 date_event;
    mapping(bytes32 => address) public stakeHash; // off-chain hash value on chain
    bool public homeTeamWin; // update home/away winning`
    
    // mapping(address => string) public stakeSelection; // record the side user bets
    ...
    mapping(address => uint256) public grossWin; // record winning by each
    ...
    event NewGame(address sender, bytes32 stakeGame); // store user's bet by event
    event NewWin(address sender, string stakeSelection); // // store user's bet by event
    event Resolved(bytes32 gameID, uint8 homeScore, uint8 awayScore);
    // varaible for addStake
    ...
    constructor() {
        ...
        setChainlinkOracle(0xA98FEd1e6E59fB7Cf6b35fD240Cd72EBf2DA7d66); // polygon
        // setChainlinkToken(0x326C977E6efc84E512bB9C30f76E30c160eD06FB); // goerli
        // setChainlinkOracle(0xB9756312523826A566e222a34793E414A81c88E1); // goerli
        verifiedGameId = ["1a99710c4b40b07390e390222dd33aee"];
        date_event = 1675382400;
        homeTeamWin = false;
        // isAdmin = msg.sender;
    }
    
    /* ========== STAKE STAKE STAKE STAKE STAKE STAKE STAKE STAKE STAKE STAKE STAKE STAKE ========== */
    // 1. checkBalance()
    // 2. addStake()
    // 3. settleStakes()
    // 4. releasing()
    ...
    function addStake(bytes32 _hash, string memory _win) public payable {
        ...
        //* 這裡要改成傳值
        // bytes32 _game = 0x3434313463323763613161363636653739646635643263383464393466373362;
        // store on chain
        stakeHash[_hash] = msg.sender;
        // stakeSelection[msg.sender] = _win; /* 這個不用存儲應該沒事吧 */
        // trigger event
        
        /* ========== usage: record user's stake to home/away prize pool ========== */
        /* ========== usage: add user to home/away address list ========== */
    ...
    /* ========== 3. settleStakes ========== */
    function settleStakes() public onlyOwner returns (uint256[] memory, uint256, uint256) {
        uint256 _totalPrize = address(this).balance * 95 / 100; // 95% of all stakes
        
        if ( homeTeamWin == true ) {
            uint256 _size = punterHome.length;
            uint256[] memory result = new uint256[](_size);
            for ( uint256 i = 0; i < _size; i++) {
                uint256 _shareByAddress = ( stakeHome[punterHome[i]] * 1000000000000000000 ) / stakesHome;
                uint256 _winningByAddress = _shareByAddress * _totalPrize / 1000000000000000000;
                
                result[i] = _winningByAddress;
                grossWin[punterHome[i]] = _winningByAddress;
                // punterHome[i].transfer(_winningsByEach);
            }
            return (result, _size, _totalPrize);
        } else {
            uint256 _size = punterAway.length;
            uint256[] memory result = new uint256[](_size);
            for ( uint256 i = 0; i < _size; i++) {
                uint256 _shareByAddress = ( stakeAway[punterAway[i]] * 1000000000000000000 ) / stakesAway;
                uint256 _winningByAddress = _shareByAddress * _totalPrize / 1000000000000000000;
                
                result[i] = _winningByAddress;
                grossWin[punterAway[i]] = _winningByAddress;
                // punterHome[i].transfer(_winningsByEach);
            }
            return (result, _size, _totalPrize);
        }
    ...
    /* ========== 4. releaseing() ========== */
    function releasing() public onlyOwner {
        // require(msg.sender == isAdmin);
        // punterHome[i].transfer(_winningsByEach);
        uint _size;
        if ( homeTeamWin == true ) {
            _size = punterHome.length;
            for ( uint i = 0; i < _size; i++ ) {
                punterHome[i].transfer(grossWin[punterHome[i]]);
            }
        } else {
            _size = punterAway.length;
            for ( uint i = 0; i < _size; i++ ) {
                punterAway[i].transfer(grossWin[punterAway[i]]);
            }
        }
    }

    /* ========== GET DATA FROM CHAINLINK ========== */
    // 1. requestGamesFiltering()
    // 2. getGamesResolved()

    /* ========== 1. send request to Chainlink ========== */
    function requestGamesFiltering(
        ...
        string[] memory _eventIds, // assign this value in constructor or this function
        uint256[] memory _statusIds
    ) external {
        bytes32 _specId = 0x3233663930383335623264313463663938383339386337333833306139323837; // LINK Jobs on polygon
        ...
        bytes32 _verifiedValue = keccak256(abi.encodePacked(verifiedGameId[0]));

        if ( _inputInBytes32 == _verifiedValue ) {

            Chainlink.Request memory req = buildOperatorRequest(_specId, this.fulfillGames.selector);
            req.add("market", _market);
            req.addUint("sportId", _sportId);
            req.addUint("date", date_event);
            req.addStringArray("gameIds", _eventIds); // NB: optional filter
    ...
    /* ========== 2. get response from oracle - game resolve ========== */
    function getGamesResolved(bytes32 _requestId, uint256 _idx) external returns ( GameResolve memory ) {
        ...
        // bytes32 _gameId = game.gameId;
        // uint8 _homeScore = game.homeScore;
        // uint8 _awayScore = game.awayScore;
        ...
        emit Resolved(game.gameId, game.homeScore, game.awayScore);
        return ( game );
    }

    // utility: view oracle's address
    ...
    /*
    // get response from oracle - game created
    ...
    // require direct response on specific game result
    function requestGames(
        ...
        uint256 _payment = 100000000000000000; // 0.1 LINK
        uint256 _sportId = 4; // NBA
        uint256 _date = 1674777600; // Friday, January 27, 2023 12:00:00 AM
        Chainlink.Request memory req = buildOperatorRequest(_specId, this.fulfillGames.selector);
