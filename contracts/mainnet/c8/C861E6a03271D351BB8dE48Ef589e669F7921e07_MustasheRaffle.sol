...
pragma solidity ^0.8.7;
    ...
    // Raffle contract using $AMBER token for tickets, and ERC721 NFTs for prizes.
    ...
    uint256 private constant MAX_ENDED_RAFFLES = 50;
    Raffle[] private endedRafflesBuffer;
    uint256 private endedRafflesIndex;
   ...
   function endRaffleAfterExpiry(uint256 raffleId) external {
    ...
    function getEndedRaffles() public view returns (Raffle[] memory) {
        return endedRafflesBuffer;
    }

    function endRaffle(uint256 raffleId) external {
        _endRaffle(raffleId);
        addEndedRaffleToBuffer(raffles[raffleId]);
    }

    function addEndedRaffleToBuffer(Raffle memory raffle) private {
        if (endedRafflesBuffer.length < MAX_ENDED_RAFFLES) {
            endedRafflesBuffer.push(raffle);
        } else {
            endedRafflesBuffer[endedRafflesIndex] = raffle;
            endedRafflesIndex = (endedRafflesIndex + 1) % MAX_ENDED_RAFFLES;
        }
    }
