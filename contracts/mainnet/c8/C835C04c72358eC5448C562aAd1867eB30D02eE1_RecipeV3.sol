...
contract RecipeV3 is IRecipe, Ownable {
    ...
    function swap(address _inputToken, address _outputToken, uint256 _outputAmount) internal {
        console.log("Input Token: ",_inputToken);
        console.log("Output Token: ",_outputToken);
    ...
    function dexSwap(address _assetIn, address _assetOut, uint _amountOut, uint _ammIndex) public {
        console.log("Amount out we ware buying: ",_amountOut);
        console.log("at Amm: ",_ammIndex);
        //Uni1
        if(_ammIndex == 0){
            ...
            console.log("We are doing UniV3 trade 500");
            console.log("_assetIn Balance: ",IERC20(_assetIn).balanceOf(address(this)));
            console.log("amount Swapping out: ",_amountOut);
        ...
        if(_ammIndex == 1){
            ...
            console.log("We are doing UniV3 trade 3000");
            console.log("_assetIn Balance: ",IERC20(_assetIn).balanceOf(address(this)));
            console.log("amount Swapping out: ",_amountOut);
    ...
    function swapPie(address _pie, uint256 _outputAmount) internal {
        ...
        for(uint256 i = 0; i < tokens.length; i ++) {
            console.log("Amounts we plan to buy: ",amounts[i]+1);
            swap(address(WETH), tokens[i], amounts[i]+1);
            IERC20 token = IERC20(tokens[i]);
            token.approve(_pie, 0);
            token.approve(_pie, amounts[i]+1);
            console.log("We are trying to deposit: ",amounts[i]);
            console.log("We have: ",token.balanceOf(address(this)));
    ...
    function getBestPrice(address _assetIn, address _assetOut, uint _amountOut) public returns (BestPrice memory){  
        ...
        if(balancerViable[_assetOut]!= ""){
            ...
            console.log("balancerAmount: ",balancerAmount);
