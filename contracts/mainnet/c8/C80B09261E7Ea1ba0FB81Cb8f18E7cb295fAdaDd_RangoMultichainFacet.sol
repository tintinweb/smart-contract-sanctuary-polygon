...
contract RangoMultichainFacet is IRango, ReentrancyGuard, IRangoMultichain {
    ...
    /// @param _addresses The address of whitelist contracts for bridge
    function initMultichain(address[] calldata _addresses) external { 
    ...
    function addMultichainRouters(address[] calldata _addresses) public {
        LibDiamond.enforceIsContractOwner();
        addMultichainRoutersInternal(_addresses);
    }
    ...
    function removeMultichainRouters(address[] calldata _addresses) external {
        LibDiamond.enforceIsContractOwner();
        MultichainStorage storage s = getMultichainStorage();
        for (uint i = 0; i < _addresses.length; i++) {
            delete s.multichainRouters[_addresses[i]];
        }
    ...
    function doMultichainBridge(
        ...
        if (request.actionType == MultichainBridgeType.OUT_NATIVE) {
            require(msg.value >= inputAmount, 'Insufficient ETH sent for OUT_NATIVE action');
            require(fromToken == LibSwapper.ETH, 'Invalid fromToken, it must be equal to null address');
        }
        ...
        if (request.actionType == MultichainBridgeType.OUT) {
            ...
            router.anySwapOutNative{value: msg.value}(request.underlyingToken, request.receiverAddress, request.receiverChainID);
        } else {
            revert();
        }
