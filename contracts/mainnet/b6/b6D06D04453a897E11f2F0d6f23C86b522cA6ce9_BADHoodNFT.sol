...
contract BADHoodNFT is Ownable, ERC721Enumerable, ERC721URIStorage {
    ...
    mapping(uint => uint) public token2Card; // tokenId  => cardId
    ...
    function burn(uint tokenId_) public returns (bool) {
        ...
        uint cardId = token2Card[tokenId_];
    ...
    function tokenURI(uint tokenId_) public view override(ERC721URIStorage, ERC721) returns (string memory) {
        require(_exists(tokenId_), "K: nonexistent token");

        string memory tURI = super.tokenURI(tokenId_);
        string memory cURI = cardInfoes[token2Card[tokenId_]].tokenURI;
        string memory base = _myBaseURI();

        return  bytes(tURI).length > 0 ? string(abi.encodePacked(base, cURI, "/", tURI)) : string(abi.encodePacked(base, cURI));
    }
