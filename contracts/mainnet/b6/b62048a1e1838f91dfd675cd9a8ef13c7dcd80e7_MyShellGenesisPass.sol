...
// File: contracts/MyShellGenesisPass.sol
...
contract MyShellGenesisPass is
    ...
    OwnableUpgradeable
{
    ...
    // Mapping of whitelisted address and allowed mint amount
    mapping(address => uint256) private _mintAllowance;

    // level of NFT
    mapping(uint256 => uint256) private _passLevel;
    ...
    /**
     * @dev Returns the tokenID's vip level
     */
    function level(uint256 tokenID) external view returns (uint256) {
        return _passLevel[tokenID];
    }

    /**
     * @dev Returns the NFT totalSupply
     */
    ...
    /**
     * @dev Returns the NFT whitlist amount
     */
    function totalAllowance() external view returns (uint256) {
    ...
    /**
     * @dev Add multiple addresses from the whitelist
     */
    function addMintAllowance(
        address[] calldata accounts,
        uint256[] calldata amounts
    ) external onlyOwner {
        require(
            accounts.length == amounts.length,
            "Number of accounts and amounts mismatched."
        );
        for (uint256 i = 0; i < accounts.length; i++) {
            require(
                _totalAllowance + amounts[i] <= _totalSupply,
                "Can't add more whitelist"
            );
            _mintAllowance[accounts[i]] += amounts[i];
            _totalAllowance += amounts[i];
        }
    }

    /**
     * @dev Mints NFTs for the callers if they are whitelisted
     */
    function mintBatch(
        address[] memory accounts,
        uint256[] memory amounts
    ) external {
        require(amounts.length == accounts.length, "illegal array length");

        uint256 loopTimes = amounts.length;

        for (uint256 i = 0; i < loopTimes; i++) {
            uint256 mintAmount = amounts[i] < _mintAllowance[accounts[i]]
                ? amounts[i]
                : _mintAllowance[accounts[i]];
            if (mintAmount == 0) {
                continue;
            } else {
                mint(accounts[i], mintAmount);
            }
        }
    }

    /**
     * @dev Mints NFTs for the whitelisted caller
     */
    function mint(address account, uint256 amount) public {
        require(
            amount <= _mintAllowance[account],
            "Mint amount exceeds allowance"
        );
        mintLoop(account, amount);
    }

    /**
     * @dev Mints NFTs
     */
    function mintLoop(address account, uint256 amount) internal {
        for (uint256 i = 0; i < amount; i++) {
            _tokenIdCounter++;
            _safeMint(account, _tokenIdCounter);
            _passLevel[_tokenIdCounter] = 1;
        }
        _mintAllowance[account] -= amount;
    }
