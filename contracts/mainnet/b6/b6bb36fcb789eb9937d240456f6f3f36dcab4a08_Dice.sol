...
abstract contract Manager is Ownable {
    ...
    mapping(address => uint[]) public userBetMap;
    uint[] public validBetAmount;
    ...
    function setBetAmount (uint[] memory _array) external onlyOwner{
        uint[] memory vba = new uint[](_array.length);
    ...
    function getBetAmountLength () external view returns(uint) {
        return validBetAmount.length;
    }

    function getUserBetMap(uint _checkNumbers) external view returns(uint[] memory betList) {
        uint length = userBetMap[msg.sender].length;
        _checkNumbers = _checkNumbers > length ? length : _checkNumbers;
        uint[] memory vba = new uint[](_checkNumbers);
        uint j;
        for( uint i = length ; i > (length - _checkNumbers); i-- ){
    ...
    function betAmountValid(uint amount) internal view returns(bool) {
        for(uint i = 0; i < validBetAmount.length ; i ++ ){
...
interface IHouse {
    function placeBet(address player, uint amount,uint rollUnder, uint modulo, address refAddr) payable external;
...
contract Dice is ReentrancyGuard,Manager, IGame{
    ...
    function placeBet(uint betChoice,address refAddr) external payable nonReentrant {
        ...
        house.placeBet{value: msg.value}(msg.sender,amount, rollUnder, MODULO,refAddr);
