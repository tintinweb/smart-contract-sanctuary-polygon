...
contract SBTFacet is IERC1155MetadataURI, Modifiers {
    ...
    function mintBatch(address[] calldata _tos, uint256 cardId_) external payable onlyTrustedForwarder {
        uint256 count = _tos.length;
        require(count != 0, "SBTFacet: _tos must be longer than 1");

        address msgSender = LibERC2771Context.msgSender();
        s.userExecutionCount[msgSender]["mint"] += count;

        uint256 projectId = s.cardMetadata[cardId_].projectId;

        require(LibProjects.getAdminByProjectId(projectId, msgSender) == 1, "SBTFacet: Not authorized");

        for (uint256 i; i < count; ) {
            address to = _tos[i];
            require(to != address(0), "SBTFacet: Mint to the zero address");

            require(s.mintDetails[to][cardId_].isMinted != 1, "SBTFacet: Already minted to this user");

            if (s.mintDetails[to][cardId_].index == 0) {
                s.userCardsIds[to].push(cardId_);
                s.mintDetails[to][cardId_] = MintDetailStruct({
                    isMinted: 1,
                    index: s.userCardsIds[to].length,
                    mintedAt: block.timestamp,
                    mintedBy: msgSender
                });
                s.cardUsers[cardId_].push(to);
            } else {
                s.userCardsIds[to][s.mintDetails[to][cardId_].index - 1] = cardId_;
                s.mintDetails[to][cardId_].isMinted = 1;
                s.mintDetails[to][cardId_].mintedAt = block.timestamp;
                s.mintDetails[to][cardId_].mintedBy = msgSender;
            }

            emit TransferSingle(msgSender, address(0), to, cardId_, 1);
            unchecked {
                ++i;
            }
        }
    ...
    function burn(address _from, uint256 cardId_) external payable onlyTrustedForwarder {
        require(_from != address(0), "SBTFacet: Burn from the zero address");

        address msgSender = LibERC2771Context.msgSender();

        require(_from == msgSender || s.cardMetadata[cardId_].createdBy == msgSender, "SBTFacet: Not authorized");

        require(s.mintDetails[_from][cardId_].isMinted == 1, "SBTFacet: Not minted to this user");

        ++s.userExecutionCount[msgSender]["burn"];

        delete s.userCardsIds[_from][s.mintDetails[_from][cardId_].index - 1];
        s.mintDetails[_from][cardId_].isMinted = 2;

        uint256 projectId = s.cardMetadata[cardId_].projectId;
        uint256[] memory cardIds = s.projectCardIds[projectId];

        if (s.cardMetadata[cardId_].createdBy != _from) {
            bool ownership;
        ...
        emit TransferSingle(msgSender, _from, address(0), cardId_, 1);
    }
    ...
    function uri(uint256 _id) external view returns (string memory) {
        require(s.cardMetadata[_id].createdAt != 0, "SBTFacet: URI query for nonexistent card");

        MetadataStruct memory metadata = s.cardMetadata[_id];
        uint256 projectId = metadata.projectId;
        ReturnProjectStruct memory project = LibProjects.getProjectById(projectId);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        bytes(metadata.imageCID).length != 0 ? metadata.imageCID : s.cardImageCID,
                        '",',
                        ...
                        '"value": "',
                        Strings.toHexString(metadata.createdBy),
                        '"',
                        "}",
                        "]",
                        "}"
