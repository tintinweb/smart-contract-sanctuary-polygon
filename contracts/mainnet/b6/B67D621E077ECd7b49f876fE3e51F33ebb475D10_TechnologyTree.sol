...
contract TechnologyTree is OwnableUpgradeable {
    ...
    function buyTec(uint ID, uint amount) external {
        ...
        if(userTec[msg.sender][ID] + amount >= 1000){
            amount = 1000 - userTec[msg.sender][ID];
        }
    ...
    function buyTecBatch(uint[] memory ids, uint[] memory amounts) external {
        ...
        for (uint i = 0; i < ids.length; i ++) {
            ...
            if(userTec[msg.sender][ids[i]] + tempAmount >= 1000){
                tempAmount = 1000 - userTec[msg.sender][ids[i]];
            }
            total += tempAmount;
