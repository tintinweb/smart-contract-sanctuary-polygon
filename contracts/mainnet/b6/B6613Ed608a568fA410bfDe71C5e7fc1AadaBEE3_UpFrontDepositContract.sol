...
contract UpFrontDepositContract is ReentrancyGuard {
  ...
  event WithdrawnFunds(address indexed to, uint256 amount, address executor);
  ...
  modifier isInvitation(address _invitation) {
    if (INVITATION_ONLY && !investorData[_msgSender()].exists) {
      ...
      require(investorData[_invitation].invited.length < INVITATION_MAX, "Invitation code usage exceeded.");
  ...
  function ownerWithdraw(address payable _to, uint256 _amount) external payable isOwner isInitialized nonReEntrant {
    ...
    emit WithdrawnFunds(_to, _amount, msg.sender);
  }

  function ownerWithdrawToken(address _token, address _to, uint256 _amount) external isOwner isInitialized isTokenInterface(_token) nonReEntrant {
  ...
  function getContractInfo() external view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, bool) {
    return (REWARD_RATE, DEPOSIT_WINDOW_START, DEPOSIT_WINDOW_LENGTH, DEPOSIT_WINDOW_CLOSED, DEPOSIT_MIN_AMOUNT, DEPOSIT_MAX_AMOUNT, WITHDRAWAL_LOCK_TIME, WITHDRAWAL_WAIT_TIME, INVITATION_ONLY ? INVITATION_MAX : 0, CYCLE_LENGTH, PROOF_OF_LIFE, MAX_INVESTORS, REWARDS_PROCESSED);
  }
  ...
  function getInvestorInfo(address _investor) external view returns (investorDataStruct memory) {
    require(investorData[_investor].exists, "Unknown investor.");

    return investorData[_investor];
  }
  ...
  function newDeposit(uint256 _amount, address _invitation) external isInitialized isInvitation(_invitation) nonReEntrant {
    ...
    if (!investorData[msg.sender].exists) {
      ...
      if (INVITATION_ONLY) {
        ...
        investorData[_invitation].invited.push(invitedDataStruct(getCurrentTime(), msg.sender));
      }
  ...
  function setDepositCap(uint256 _cap) external isOwner isInitialized {
    DEPOSIT_CAP = _cap;
  }
  ...
  function setMaxInvestors(uint256 _value) external isOwner isInitialized {
    ...
    require(_value >= cnt, "Maximum number of investors must be equal to or greater than the current number of investors.");
  ...
  function setInvitationOnly(bool _state) external isOwner isInitialized {
    if (_state == true) { require(INVITATION_MAX > 0, "A maximum number of invitations per investor must be defined."); }
