...
contract HeroesV1 is CosTokenUpgradeable {
    ...
    function initialize() public initializer {
        __CosToken_init("Clash of Streamers - Heroes", "HERO");
    }

    function _baseURI() internal pure override returns (string memory) {
        return "https://api.cos-nft.com/api/opensea/v2/heroes/";
    }
    ...
    function _applyMerge(
        ...
        if (
            ...
            unpackedTarget.faction != unpackedSource.faction
        ) {
            unpackedTarget.modelVariation = unpackedSource.modelVariation;
        }
    ...
    function _packCollectibleSpecific(
        ...
        assert(unpacked.modelVariation <= type(uint8).max);
        assert(unpacked.skinDivision <= type(uint16).max);
        assert(unpacked.skinSlot <= type(uint16).max);
        assert(unpacked.class <= type(uint8).max / 2);

        // 1) Set next 8 bits to modelVariation
        packedData |= uint256(unpacked.modelVariation) << offset;
        offset += 8;

        // 1) Set next 16 bits to skinDivision
    ...
    function _unpackCollectibleSpecific(
        ...
        // 1) Read modelVariation from the next 8 bits
        unpacked.modelVariation = uint8(packedData >> offset);
        offset += 8;

        // 1) Read skinDivision from the next 16 bits
        unpacked.skinDivision = uint16(packedData >> offset);
        offset += 16;
