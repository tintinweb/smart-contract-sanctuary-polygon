...
contract Mines is IMines, VRFV2WrapperConsumerBase {
    ...
    function _reveal(uint256 gameId, uint8 selectionIndex) private {
        ...
        require(!isBetInFlight(gameId), "Mines: wait till the previous bet's result comes through.");
        require(balance(gameId) < prize(games[gameId].wager, games[gameId].revealedNum), "Mines: not enough balance to pay the next prize");
        requestRandomness(gameId);
        games[gameId].selectionIndex = selectionIndex;
    }
