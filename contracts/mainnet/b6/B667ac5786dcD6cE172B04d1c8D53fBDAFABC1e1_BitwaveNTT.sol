...
contract BitwaveNTT is ERC721, AccessControl {
    ...
    uint256 internal _currentTokenIndex;

    /// @notice the index of mapped URIs.
    uint256 internal _currentUriIndex;

    /// @notice the mapping of _currentUriIndex to BaseUri.
    mapping (uint256 => BaseUri) public baseUriMap;
    ...
    function mint(address[] memory to, string memory baseUri, string memory suffix) public onlyRole(MINTER_ROLE) {
        uint256 startTokenId = _currentTokenIndex + 1;
        for (uint i = 0; i < to.length; i++) {
            _currentTokenIndex += 1;
            _mint(to[i], _currentTokenIndex);
        }
        uint256 endTokenId = _currentTokenIndex;
        _currentUriIndex += 1;
        baseUriMap[_currentUriIndex] = BaseUri(baseUri, suffix, startTokenId, endTokenId);
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        for (uint256 i = 1; i < 256; i++) {
            require(tokenId > 0, "UN1");
            if (baseUriMap[i].startTokenId <= tokenId && baseUriMap[i].endTokenId >= tokenId) {
                return string(abi.encodePacked(baseUriMap[i].uri, "/", Strings.toString(tokenId), baseUriMap[i].suffix));
            }
        }
        revert ("UN1");
    }
