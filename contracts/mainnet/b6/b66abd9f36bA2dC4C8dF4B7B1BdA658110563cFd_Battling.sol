...
contract Battling is Ownable, VRFConsumerBase, Set, SignerVerifiable {
    ...
    function initiateBattle(uint256 _amount, string memory _message, uint256 _battle_id, uint256 _deadline, bytes memory _signature) external frozen callerVerified(_amount, _message, _battle_id, _deadline, _signature) {
        require(_amount <= balances[msg.sender], "Player does not have enough balance");
        Set.add(msg.sender);
        balances[msg.sender] -= _amount;
    }
    
    function claimWinnings(uint256 _amount, string memory _message, uint256 _battle_id, uint256 _deadline, bytes memory _signature) external frozen callerVerified(_amount, _message, _battle_id, _deadline, _signature) {
    ...
    function returnWager(uint256 _amount, string memory _message, uint256 _battle_id, uint256 _deadline, bytes memory _signature) external frozen callerVerified(_amount, _message, _battle_id, _deadline, _signature) {
