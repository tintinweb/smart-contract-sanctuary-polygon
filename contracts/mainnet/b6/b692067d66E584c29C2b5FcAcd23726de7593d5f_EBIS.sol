...
contract EBIS is ERC20, Ownable, Pausable {
    ...
    function releaseTimeLock(address _holder) internal returns (bool) {
        ...
        if (
            lockupInfo[_holder].lockupBalance <=
            lockupInfo[_holder].unlockAmountPerMonth
        ) {
            ...
            lockupInfo[_holder].releaseTime = lockupInfo[_holder]
                .releaseTime
                .add(MONTH);
            lockupInfo[_holder].lockupBalance = lockupInfo[_holder]
                .lockupBalance
                .sub(releaseAmount);
        }
        emit Unlock(_holder, releaseAmount);
