...
contract Followers {
    ...
    */
    function getFollowers(address usrAddress, uint256 startFrom) public view returns(address[] memory) {
        ...
        string memory usrAddressStr = KUtils._toLower(KUtils.addressToString(usrAddress));
    ...
    function removeFollower(address addressRequester, address addressTarget) public onlyAdmins {
        ...
        string memory addressRequesterStr = KUtils._toLower(KUtils.addressToString(addressRequester));
        string memory addressTargetStr = KUtils._toLower(KUtils.addressToString(addressTarget));

        // Make sure it's not themselves
        require(!KUtils.stringsEqual(addressRequesterStr, addressTargetStr), "Not a thing");
        ...
        for (uint i=0; i < followingMap[addressRequester].length; i++) {
            if (KUtils.stringsEqual(KUtils._toLower(KUtils.addressToString(followingMap[addressRequester][i])), KUtils._toLower(KUtils.addressToString(addressTarget)))){
                place = i;
                break;
            }
        }
...
interface IUserProfiles {
    ...
    function logPost(address posterAddress, address contractAddress, uint256 tipPerTag, address[] calldata tipReceivers) external;

    // Update a user profile
    function updateProfile(string calldata handle, string calldata location, string calldata avatar, string calldata _uri, string calldata _bio) external;
