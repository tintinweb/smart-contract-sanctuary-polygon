...
interface ILongStaking {
    function add(uint _rootPid, bool _withUpdate) external;
...
interface IMasterChef {
    function updateMultiplier(uint multiplierNumber) external;
    ...
    function setMaxMint(uint _nsdxMaxMint) external;

    function setPerBlock(uint _nsdxPerBlock, bool _withUpdate) external;
...
interface IFactory {
    function createPair(address _nToken, address swapToToken) external returns (address _pair);
}

contract Admin is Initializable, OwnableUpgradeable {
    IShortStaking public shortStaking;
    ILongStaking public longStaking;
    IShortLock public shortLock;
    IAsset public asset;
    IMint public mint;
    ...
    function initialize(
        ...
        shortStaking = IShortStaking(shortStaking_);
        longStaking = ILongStaking(longStaking_);
        shortLock = IShortLock(shortLock_);
        asset = IAsset(asset_);
        mint = IMint(mint_);
    ...
    function transferAllOwnership(address newOwner) external onlyOwner {
        ...
        shortStaking.transferOwnership(newOwner);
        longStaking.transferOwnership(newOwner);
        shortLock.transferOwnership(newOwner);
        asset.transferOwnership(newOwner);
        mint.transferOwnership(newOwner);
    ...
    function whiteList(
        ...
        IAsset.IPOParams memory ipoParams
    ) external onlyOwner {
        AssetToken nToken = new AssetToken();
        nToken.initialize(tokenParams.nTokenName, tokenParams.nTokenSymbol);
        nToken.transferOwnership(address(mint));

        StakingToken sToken = new StakingToken();
        sToken.initialize(tokenParams.sTokenName, tokenParams.sTokenSymbol);
        sToken.transferOwnership(address(shortStaking));

        address swapToToken = mint.swapToToken();
        ...
        masterChef.add(lAllocPoint_, pair, false);
        uint rootPoolId = masterChef.poolLength() - 1;

        // add a pool in LongStaking
        longStaking.add(rootPoolId, false);

        // add a short farm pool in MasterChef
        masterChef.add(sAllocPoint_, address(sToken), false);
        rootPoolId = IMasterChef(masterChef).poolLength() - 1;

        // add a pool in ShortStaking
        shortStaking.add(rootPoolId, address(sToken), false);
        uint poolId = shortStaking.poolLength() - 1;
    ...
    function asset_registerCollateral(
        ...
        asset.registerCollateral(cAssetToken, oracle, multiplier);
    }
    ...
    function asset_setCollateralInPreIPO(address cAssetToken, bool value)
        ...
        onlyOwner
    {
        asset.setCollateralInPreIPO(cAssetToken, value);
    }
    ...
    function masterChef_setMaxMint(uint256 _nsdxMaxMint) external onlyOwner {
        masterChef.setMaxMint(_nsdxMaxMint);
    }
    ...
    function masterChef_setPerBlock(uint256 _nsdxPerBlock, bool _withUpdate)
        ...
        onlyOwner
    {
        masterChef.setPerBlock(_nsdxPerBlock, _withUpdate);
    }
    ...
    function masterChef_transferNSDXOwnership(address _newOwner)
        ...
        onlyOwner
    {
        masterChef.transferNSDXOwnership(_newOwner);
    }
    ...
    function shortStaking_setMasterChef(address _masterChef)
        ...
        onlyOwner
    {
        shortStaking.setMasterChef(_masterChef);
    }
    ...
    function setAsset(IAsset asset_) external onlyOwner {
        asset = asset_;
    }

    function setLongStaking(ILongStaking staking_) external onlyOwner {
        longStaking = staking_;
    }

    function setMasterChef(IMasterChef masterChef_) external onlyOwner {
        masterChef = masterChef_;
    }

    function setMint(IMint mint_) external onlyOwner {
        mint = mint_;
    }

    function setShortLock(IShortLock shortLock_) external onlyOwner {
        shortLock = shortLock_;
    }

    function setShortStaking(IShortStaking staking_) external onlyOwner {
