...
import "./BitwaveNTT721Roles.sol";

/// @title A factory contract for NTT contracts with roles.
/// @author Bitwave
/// @author Inish Crisson
contract BitwaveNTT721RolesFactory {
    ...
    event BitwaveNTTContractCreated(address indexed contractAddress);

/// @notice Deploys a new Bitwave URI based NTT contract. 
/// @return newContractAddress The address of the deployed contract.
    function deployNewCert(string memory _name, string memory _symbol) public returns (address) {
        BitwaveNTT721Roles newBitwaveCert = new BitwaveNTT721Roles(
            _name,
            _symbol,
            msg.sender
        );
        address newContractAddress = address(newBitwaveCert);
        emit BitwaveNTTContractCreated(newContractAddress);
...
contract BitwaveNTT721Roles is ERC721, AccessControl {
    ...
    /// @notice A mapping of token URIs to tokenIds.
    ...
    function bulkMint(address[] memory to, string[] memory tokenURI_) public onlyRole(MINTER_ROLE) {
        for (uint i = 0; i < to.length; i++) {
            uint256 tokenId = currentTokenId();
            _mint(to[i], tokenId);
            _tokenURI[tokenId] = tokenURI_[i];
        }
    }
    ...
    function mint(address to, string memory tokenURI_) public onlyRole(MINTER_ROLE) {
        uint256 tokenId = currentTokenId();
        _mint(to, tokenId);
