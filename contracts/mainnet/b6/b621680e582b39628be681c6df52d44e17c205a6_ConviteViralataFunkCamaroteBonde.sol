...
contract ConviteViralataFunkCamaroteBonde is ERC721, Ownable {
  using Strings for uint256;
...
struct Whitelist {
    ...
    uint256 public cost = 5.075 ether;
    ...
    constructor() ERC721("Ingresso VIP ViralataFunk", "VIP") {
        setHiddenMetadataUri(
            "ipfs://bafybeifjqfrzypfrgzi4plcsuoyijqmn45xkh3ythtamevwf4d63fj4jtm/ticket.json"
        );
    }
...
function mintForAddresses(uint256[] memory _mintAmounts, address[] memory _receivers) public onlyOwner {
    require(_mintAmounts.length == _receivers.length, "Arrays must be of same length");
