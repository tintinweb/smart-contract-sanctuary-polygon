...
contract PremiumPack is ERC1155, Ownable, AccessControl, ReentrancyGuard {
    ...
    string public constant name =
        "Cross the Ages - Arkhante Booster Premium Pack";
    ...
    constructor(address sig, address withdraw) ERC1155("") {
        ...
        apprentice = Pack(73000000000000000000, 0, 0);
        disciple = Pack(182000000000000000000, 1, 0);
        primus = Pack(365000000000000000000, 2, 0);
    }
    ...
    function uri(uint256 tokenId) public view override returns (string memory) {
        ...
        return (
            string(
                abi.encodePacked(
                    "https://ipfs.io/ipfs/QmduQbcV4RmeJQ6eiEkPxBRuHTaacA6VoBpmQUwtcRXHJ3/",
                    id,
                    ".json"
                )
            )
    ...
    function getSigner(bytes32 messageHash, uint8 v, bytes32 r, bytes32 s) private pure returns (address) {
    ...
    function airdrop(uint8 packType, address toAirdropAddress, uint256 tokenId) public onlyOwner {
        Pack memory pack = getPackByPackType(packType);
        ...
        emit AirdroppedPackMinted(msg.sender, toAirdropAddress, tokenId, packType);
    }
