...
contract WiredMotnka is ERC721 {
    ...
    /**
     * @dev Create collectible with count.
     */
    function createCollectible(uint256 count) public payable returns (uint256) {
        uint256 tokenId = _nextTokenIdToMint;
        require(
            tokenId + count - 1 <= _maxTokenId,
            "Not enough tokens available to mint."
        );
        require(msg.value >= _minMintPrice * count, "Not enough funds.");
        for (uint256 i = 0; i < count; i++) {
            _safeMint(msg.sender, tokenId + i);
        }
        payable(_withdrawAddress).transfer(msg.value);
        _nextTokenIdToMint += count;
        return tokenId;
    }
    ...
    /**
     * @dev Create collectible for specific address with count.
     */
    function createCollectibleFor(
        address to,
        uint256 count
