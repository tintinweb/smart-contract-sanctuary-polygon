 ...
 */
 contract Presale is Owned{
    ...
    uint256 public minDaiLimit = 50 * (10 ** 18);
    uint256 public maxDaiLimit = 1500 * (10 ** 18);
    ...
    constructor(uint256 _rate, address _wallet, ERC20 _dai, ERC20 _pToken, uint _closingTime, uint _vestedTime) {
        require(_rate > 0);
    ...
    // allows buyers to put their dai to get some pToken once the presale will closes
    function buy(uint _amount) public {
        require(isPresaleOpen, "Presale is not open yet");
        require(isList == false || whiteListed[msg.sender] == true);
        require(_amount >= minDaiLimit, "You need to sell at least some min amount");
        require(paidTotal[msg.sender] + _amount <= maxDaiLimit);
        ...
        // update state
        daiRaised = daiRaised.add(_amount);
        ...
        preBuys[msg.sender].daiAmount = preBuys[msg.sender].daiAmount.add(_amount);
    ...
    function getClaimableAmount(address _account) public view returns (uint) {
        ...
        uint totalpTokenToClaim = preBuys[_account].daiAmount.mul(rate).div(RATE_DECIMALS).mul(percentReleased).div(VESTING_TIME_DECIMALS);
    ...
    function claim() public {
        ...
        uint totalpTokenToClaim = preBuys[msg.sender].daiAmount.mul(rate).div(RATE_DECIMALS).mul(percentReleased).div(VESTING_TIME_DECIMALS);
    ...
    // allows operator wallet to get the dai deposited in the contract
    function retreiveDAI() public {
        require(msg.sender == wallet);
        require(!isPresaleOpen, "Presale is not over yet");
        dai.safeTransfer(wallet, dai.balanceOf(address(this)));
    }
