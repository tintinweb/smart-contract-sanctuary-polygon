...
// File: contracts\interfaces\IStakingDualRewards.sol
...
// File: contracts\StakedLPToken07.sol
...
contract StakedLPToken07 is IUniswapV2Pair, PoolToken {
    using SafeToken for address;
	
	bool public constant isStakedLPToken = true;
	string public constant stakedLPTokenType = "07";
	...
	address public constant rewardsToken = 0x831753DD7087CaC61aB5644b308642cc1c33Dc13;
	...
	function _update() internal {
		totalBalance = IStakingDualRewards(stakingRewards).balanceOf(address(this));
		emit Sync(totalBalance);
	}
	...
	function mint(address minter) external nonReentrant update returns (uint mintTokens) {
		...
		IStakingDualRewards(stakingRewards).stake(mintAmount);
		mintTokens = mintAmount.mul(1e18).div(exchangeRate());
	...
	function redeem(address redeemer) external nonReentrant update returns (uint redeemAmount) {
		...
		_burn(address(this), redeemTokens);
		IStakingDualRewards(stakingRewards).withdraw(redeemAmount);
		_safeTransfer(redeemer, redeemAmount);
		emit Redeem(msg.sender, redeemer, redeemAmount, redeemTokens);		
	}
	...
	function _getReward() internal returns (uint256) {
		IStakingDualRewards(stakingRewards).getReward();
		IDragonLair(DQUICK).leave(DQUICK.myBalance());
		uint256 rewardsTokenBBalance = rewardsTokenB.myBalance();
		if (rewardsTokenBBalance > 0) {
			swapExactTokensForTokens(rewardsTokenB, rewardsToken, rewardsTokenBBalance);
		}
	...
	function reinvest() external nonReentrant update {
		...
		if (token0 == rewardsToken || token1 == rewardsToken) {
			...
			swapExactTokensForTokens(rewardsToken, WETH, rewardsToken.myBalance());
		...
		IStakingDualRewards(stakingRewards).stake(liquidity);
...
// File: contracts\StakedLPTokenFactory07.sol
