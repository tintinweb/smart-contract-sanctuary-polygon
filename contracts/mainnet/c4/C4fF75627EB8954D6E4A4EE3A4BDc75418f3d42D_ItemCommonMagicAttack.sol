...
abstract contract AttackItemBase is ItemBase, IAttackItem {

  // ---- CONSTANTS ----

  /// @notice Version of the contract
  /// @dev Should be incremented when contract changed
  string public constant ATTACK_ITEM_BASE_VERSION = "1.0.0";


  // ---- VARIABLES ----

  bool internal _attackItemInited;
  mapping(uint => IStatController.MagicAttack) _itemsAttackAttributes;
  GenerateAttackInfo internal _generateAttackInfo;

  event AttackInfoChanged(GenerateAttackInfo attackInfo);

  // ---- INITIALIZER ----

  // init implementation
  constructor() {_attackItemInited = true;}

  // ---- VIEWS ----

  function isAttackItem() external pure override returns (bool) {
    return true;
  }

  function attackAttributes(uint tokenId) external view override returns (IStatController.MagicAttack memory) {
    // we need to generate a weapon for monsters
    if (tokenId == 0) {
      IAttackItem.GenerateAttackInfo memory _info = _generateAttackInfo;
      return IStatController.MagicAttack({
      aType : _info.attackType,
      min : _info.damageMin,
      max : _info.damageMax,
      attributeFactors : _info.attributeFactors,
      manaConsume : _info.manaConsumeMax
      });
    }
    return _itemsAttackAttributes[tokenId];
  }

  // ---- GOV ACTIONS ----

  function setAttackInfo(GenerateAttackInfo memory _attackInfo) external {
    require(isGovernance(msg.sender) || !_attackItemInited, "!gov");

    _generateAttackInfo = _attackInfo;

    _attackItemInited = true;
    emit AttackInfoChanged(_attackInfo);
  }
  ...
  function _afterMint(uint tokenId) internal override {
    ...
    _itemsAttackAttributes[tokenId] = _generateAttack();
  }
  ...
  function _additionalAugment(uint tokenId) internal override {
    ...
    _itemsAttackAttributes[tokenId] = itemCalculator.augmentMagicAttack(_itemsAttackAttributes[tokenId]);
  }

  function _generateAttack() internal virtual returns (IStatController.MagicAttack memory) {
    IItemCalculator itemCalculator = IItemCalculator(IController(controller()).itemCalculator());
    return itemCalculator.generateAttack(_generateAttackInfo);
  }
...
import "../base/item/AttackItemBase.sol";

contract ItemCommonMagicAttack is AttackItemBase {
  ...
  function itemMetaType() external pure override returns (ItemMetaType) {
    return ItemMetaType.ATTACK_ITEM;
  }
}
