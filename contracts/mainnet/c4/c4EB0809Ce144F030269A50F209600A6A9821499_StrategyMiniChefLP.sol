...
contract StrategyMiniChefLP is StratManager, FeeManager {
    ...
    function addLiquidity() internal {
        ...
        if (outputHalf < 0.1 ether) return;
        if (lpToken0 != output) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(
                outputHalf,
                0,
    ...
    function callReward() public view returns (uint256) {
        ...
        if (rewarder != nullAddress) {
            pendingNative = IRewarder(rewarder).pendingToken(
                poolId,
                address(this)
            );
        }
        ...
        return
            pendingNative.add(nativeOut).mul(45).div(1000).mul(callFee).div(
                MAX_FEE
            );
    }
...
contract StratManager is Ownable, Pausable {
     ...
     * @param _beefyFeeRecipient address where to send Dopepool's fees.
     */
    constructor(
        address _keeper,
        address _strategist,
