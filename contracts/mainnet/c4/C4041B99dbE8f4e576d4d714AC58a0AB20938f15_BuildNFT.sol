...
//Polygon Mainnet 0xC4041B99dbE8f4e576d4d714AC58a0AB20938f15
contract BuildNFT is ERC721A, Ownable {
    ...
    constructor(
        string memory _uri
    ) ERC721A("JourneyNFT Token", "Journy 721AT") {
        _baseTokenURI = _uri;
        _mint(msg.sender, 2);
    }
    ...
    function mint(address to, uint256 quantity) external payable onlyOwner {
        ...
        require(totalSupply() + quantity <= MAX_SUPPLY, "Mint is paused");
        _mint(to, quantity);
    }

    function mintLote(address[] memory froms) external payable onlyOwner {
        require(!mintPaused, "Mint is paused");
