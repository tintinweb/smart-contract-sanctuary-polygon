...
contract RaffleSharks is Ownable, ReentrancyGuard, AccessControl {
    ...
    uint256 public perTicketCost = 10;
    ...
    function raffleCounter(uint256 _tokenAmount) internal view returns (uint256) {
         require(_tokenAmount%perTicketCost==0, "Amount Should be multiple of perTicketCost");
         return _tokenAmount.div(perTicketCost);
    }
    ...
    function buyRaffleTicket(uint256 _raffleId, uint256 _amount, address _address) external nonReentrant {
        require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not a admin");
        Raffle storage _raffle = getRaffleDetails[_raffleId];
        require(_amount > 0, "Need to pass more tokens");
        require(IAlphaSharkRewards(AlphaSharkRewards).availableTokens(_address) >= _amount.mul(10**18), "Insufficient Balance");
        ...
        {
            _raffle.raffleContenders.push(_address);
        }
        
        _tokenOwner.ticketForRaffle[_raffleId] += tokenPurchase;
        _raffle.totalRaffleTicket += tokenPurchase;

        IAlphaSharkRewards(AlphaSharkRewards).updateAddLockTokens(_amount.mul(10**18), _address, true);
        User memory user;
        user.owner = _address;
        user.raffleId = _raffleId;
        user.rafflePurchased = tokenPurchase;
        userRaffleDetails[_address].push(user);
    }

    function refundAllRaffleTokens(uint256 _raffleId) external nonReentrant {
        require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not a admin");
        Raffle storage _raffle = getRaffleDetails[_raffleId];
        ...
        for(uint256 i=0; i < listOfAddresses.length; i++){
            ...
            if(_tokenOwner.ticketForRaffle[_raffleId] > 0)
            {
                IAlphaSharkRewards(AlphaSharkRewards).updateAddLockTokens(_tokenOwner.ticketForRaffle[_raffleId].mul(10**18).mul(perTicketCost), _tokenOwner.owner, false);
    ...
    function getSingleRaffleData(uint256 _raffleId) view external returns (Raffle memory) {
        return getRaffleDetails[_raffleId];
    } 

    function updatePerTicketPrice(uint256 _price) external onlyOwner {
        perTicketCost = _price;
    }
