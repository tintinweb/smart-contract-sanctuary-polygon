...
contract LendingHelper {
    ...
    uint256 private constant DEFAULT_COLLAT_MAX_MARGIN = 2000;
    ...
    uint256 public targetCollatRatio = 5385; // The LTV we are levering up to
    uint256 public maxCollatRatio; // Closest to liquidation we'll risk
    uint256 public minRatio = 50;
    uint256 public lendingRatio = 6500; // In bps 65% of total want

    uint256 internal minRebalanceRatio = 5200;
    uint256 internal maxRebalanceRatio = 5600;
    ...
    IPDataProvider private constant dataProvider =
        IPDataProvider(0x7551b5D2763519d4e37e8B81929D336De671d46d);
    ...
    constructor(address _vault) public {
        ...
        maxBorrowCollatRatio = ltv.sub(DEFAULT_COLLAT_MAX_MARGIN); // 8000 - 2000 = 6000 -> 60%
    ...
    {
        (, ltv, liquidationThreshold, , , , , , , ) = dataProvider.getReserveConfigurationData(
            token
        ); // In bps ltv = 8000 & liquidationThreshold = 8500 for USDC
    }

    function getBorrowFromDepositInWant(uint256 deposit, uint256 collatRatio)
        public
        pure
        returns (uint256)
    {
    ...
    function triggerLend() public view returns (bool) {
        ...
        if (currentCollatRatio != 0 && currentCollatRatio < minRebalanceRatio) return true;
        if (currentCollatRatio > maxRebalanceRatio) return true;
        if (currentCollatRatio >= maxBorrowCollatRatio) return true; // 6000 currently (redundant but who knows)
    }
    ...
    function balanceLending() external onlyAuthorized {
        ...
        if (targetCollAmount > currentCollAmount) {
            ...
            _withdrawCollateral(currentCollAmount.sub(targetCollAmount)); // TODO check this, to avoid withdrawing more than allowed by borrows
        }
        _rebalanceLoan();
    ...
    function rebalanceLoan() external onlyAuthorized {
        _rebalanceLoan();
        _sendFundsToStrategy(); // NOTE added transfer here
    }
    ...
    function unwindPosition(uint256 _repayAmount) external onlyAuthorized {
        ...
        uint256 collatToWithdraw = strategy.wethToWant(_repayAmount, true).mul(basisOne).div(targetCollatRatio);
        _withdrawCollateral(collatToWithdraw);
    }

    function withdrawExcessCollateral(uint256 collatRatio)
        external
        onlyAuthorized
    ...
    function _unwindPartialPosition(uint256 _amountInWeth) internal {
        ...
        if (toRepayInWeth > wethDust) {
            ...
            // In this case we withdraw till the targetCollatRatio
    ...
    function _repayWeth(uint256 amount) internal onlyAuthorized returns (uint256) {
        ...
        return lendingPool.repay(address(wETH), amount, 2, address(this));
    }
    ...
    function _withdrawExactCollateral(uint256 _toWithdrawInWant) internal returns (uint256 amount) {
        (, uint256 borrows) = getCurrentPositionInWant();
        uint256 maxWithdrawAmount = getDepositFromBorrow(borrows, targetCollatRatio);
        if (_toWithdrawInWant < maxWithdrawAmount) {
            amount = _withdrawCollateral(_toWithdrawInWant);
        }
    }
    ...
    function _withdrawCollateral(uint256 amount) internal returns (uint256) {
        if (amount == 0) return 0;
        uint256 borrowCollValue =
            _wethToWant(balanceOfDebtToken()).mul(basisOne).div(targetCollatRatio);
        uint256 maxWithdraw = balanceOfAToken().sub(borrowCollValue);
    ...
    function _borrowWeth(uint256 amount) internal returns (uint256) {
        if (amount == 0) return 0;
        lendingPool.borrow(address(wETH), amount, 2, 0, address(this)); // 0 for referral
        return amount;
    }
    ...
    function setLendingRatio(uint256 _lendingRatio) external onlyAuthorized {
        require(_lendingRatio < basisOne);
        lendingRatio = _lendingRatio;
    }
