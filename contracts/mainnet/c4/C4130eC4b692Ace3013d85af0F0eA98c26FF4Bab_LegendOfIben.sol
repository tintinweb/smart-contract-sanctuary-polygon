...
//create method create collection( count, baseUri)
// store collection in array[Collection]
// override tokenUri 

contract LegendOfIben is ERC721,
    ...
    modifier onlyMinter() {
        require(isMinter(_msgSender()), "Restricted to minters.");
        _;
    }

    function grantMinterRole(address to) public onlyAdmin {
    ...
    */
    ...
    function _collectionURI(uint256 tokenId) internal view returns(string memory){
        require(_collections.length > 0, "Collections should be at least one");
        uint256 count = 0;
        for (uint256 i; i < _collections.length; i++) {
            count += _collections[i].count;
            if(tokenId <= count){
                return _collections[i].uri;
            }
        }
        return _collections[0].uri;
    }

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");

        return string(abi.encodePacked(_baseURIPath, _collectionURI(tokenId), "/", tokenId.toString()));
    }

    function createCollection(uint256 count, string calldata uri) public onlyAdmin {
        _collections.push(Collection({ count: count, uri: uri }));
        _maxSupply += count;
    }
    ...
    function mint(
        ...
        require(_nextTokenId <= _maxSupply, "Current collection is already sold out");
...
/// @notice Mint several tokens at once
    /// @param recipient recipient for each token
    /// @param royaltyRecipient recipient for royalties (if royaltyValues[i] > 0)
    /// @param royaltyValue royalties asked for (EIP2981)
