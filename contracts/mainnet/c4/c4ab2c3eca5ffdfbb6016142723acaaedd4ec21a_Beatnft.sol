...
contract Beatnft is ERC721URIStorage, Ownable {
    ...
    event CreateNFT(address indexed user, uint256 types, uint256 tokenId, uint256 number);

    event MergeNFT(address indexed user, uint256 typeId, uint256 tokenId, uint256 burnNum);
    ...
    address private controllerAddress = 0x49a7477719B0F23C291915C1c6Ad43f8F9431897;
    ...
    function addGoodsNum(uint256 NFTType, uint256 amount) public onlyController {
        types[NFTType].amount = types[NFTType].amount + amount;
	}

    function mergeNFT(address user, uint256 typeId, uint256[] memory tokenIds) public onlyController returns (uint256) {

        uint256 fragmentNum = tokenIds.length;

        batchBurn(user, tokenIds);

        uint256 tokenId = createNFT(user, typeId);

        emit MergeNFT(user, typeId, tokenId, fragmentNum);
    ...
    function createNFT(address user, uint256 NFTType) public onlyController returns (uint256){
        ...
        emit CreateNFT(user, NFTType, tokenId, types[NFTType].amount);
	...
	function burn(address user, uint256 tokenId) public virtual onlyController returns (bool) {
    ...
    function batchBurn(address user, uint256[] memory tokenIds) public virtual onlyController returns (bool) {
        for(uint256 i = 0; i < tokenIds.length; i++){
            require(_isApprovedOrOwner(user, tokenIds[i]), "You are not the owner nor approved!");
