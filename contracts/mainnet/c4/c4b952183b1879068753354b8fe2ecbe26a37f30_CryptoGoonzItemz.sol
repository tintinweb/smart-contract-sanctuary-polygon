...
// File: contracts/CryptoGoonzItemz.sol
...
contract CryptoGoonzItemz is ERC1155, Ownable, PaymentSplitter {
    ...
    function mint(uint256 id, uint256 amount) external payable {
        require(msg.value >= (amount * cost), "ERR:IF");
        require(!paused, "ERR:CP");
        _mint(msg.sender, id, amount, "");
    }

    function mintToOwner(address to, uint256 id, uint256 amount) external onlyOwner{
        _mint(to, id, amount, "");
    } 

    function changePermissionGiver(address _new) external onlyOwner {
        permissionGiver = _new;
    }

    function addNewAcheivement(string memory _new) external onlyOwner {
    ...
    function setCost(uint256 _new_cost) external onlyOwner{
        cost = _new_cost;
    }
