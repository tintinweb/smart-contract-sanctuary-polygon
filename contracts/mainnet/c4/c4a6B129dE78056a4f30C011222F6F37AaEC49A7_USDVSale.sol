...
contract USDVSale is Ownable, Pausable {
    IERC20 public USDC;
	IERC20 public USDV;
    ...
    event Buy(address indexed buyer, uint256 amount, uint256 chainId);
    ...
    function changeUSDV(address _usdv) public onlyOwner{
        USDV = IERC20(_usdv);
    }
    ...
    function changeSender(address _sender) public onlyOwner {
        receiver = _sender;
    }

	function buy(address account, uint256 amount, uint256 chainId) public whenNotPaused onlyWhitelisted(msg.sender) {
        require(account != address(0) && amount > 0, "Incorrect params");
		USDC.transferFrom(msg.sender, receiver, amount);

        emit Buy(account, amount, chainId);
	}
}