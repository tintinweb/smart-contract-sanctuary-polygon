...
interface IVAULT{
	function add(IERC20 _lpToken, address _platformAddress, uint _platformPoolId, uint _poolType) external returns(uint pid);
...
contract ORCFarm is Ownable {
    ...
    function startFarming(uint256 _sushiPerBlock) public{
        ...
        bonusEndBlock = startBlock + 40000 * 30;
    }
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate, address _platformAddress, uint _poolType, uint _platformPoolId) public {
        require(msg.sender == owner() || msg.sender == devaddr, "!dev addr");
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        if(_poolType > 0){
            ...
            _vaultPid = IVAULT(vaultAddress).add( _lpToken, _platformAddress, _platformPoolId, _poolType);
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public payable {
        ...
        if (address(pool.lpToken) == WMATIC) {
            if(_amount > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
			...
			if (address(pool.lpToken) == WMATIC) {
                TransferHelper.safeTransfer(WMATIC, address(wethelper), _amount);
		        wethelper.withdraw(WMATIC, address(msg.sender), _amount);
