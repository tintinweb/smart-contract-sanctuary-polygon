 ...
 */
contract ERC11550xtoweryInternal is Ownable, AccessControl, ERC1155Supply {
    ...
    modifier onlyMinter() {
        ...
        require(
            canIMint(),
            "ERC11550xtowery: must have minter role to mint"
        );
        _;
    }
...
/**
 * @title ERC11550xtower
 * ERC-721 Marketplace with tokens and royalties support
 */
contract ERC1155Marketplace is ERC11550xtoweryInternal, ReentrancyGuard {
    ...
    constructor(
        ...
        ERC11550xtoweryInternal(_owner, uri, name, symbol, _anyoneCanMint)
    {
        require(_commissionRate + _royaltiesCommissionRate < 100, "0xtower: total commissions should be lower than 100");
        admin = _admin;
    ...
    function sell(uint256 tokenId, uint256 qty, uint256 price, address wallet) external {
        ...
        // Limit the sell to owned items, the user cannot sell items do not have
        require(balanceOf(_msgSender(), tokenId) >= qty, "0xtower: you do not have enough tokens to sell");
    ...
    function buy(uint256 tokenId, uint256 index, uint256 qty) external nonReentrant {
        ...
        // validate user
        require(openSales[tokenId][index].user != _msgSender() , "0xtower: the user cannot buy his own offer");
    ...
    function cancelSale(uint256 tokenId, uint256 index) external {
        ...
        require(openSales[tokenId][index].user == _msgSender(), "0xtower: only the original seller can cancel his sales");
    ...
    function bid(uint256 tokenId, uint256 qty, uint256 price) external nonReentrant {
        ...
        require(qty>0, "0xtower: qty has to be positive");
        require(price>0, "0xtower: price has to be positive");
        require(qty <= totalSupply(tokenId), "0xtower: not enough items for sale");
    ...
    function cancelBid(uint256 tokenId, uint256 index) external nonReentrant {
        ...
        // Only the original bidder can cancel his bids
        require(openOffers[tokenId][index].user == _msgSender(), "0xtower: only the original bidder can cancel his bids");
    ...
    function acceptBid(uint256 tokenId, uint256 index, uint256 qty) external nonReentrant {
        ...
        // validate user
        require(openOffers[tokenId][index].user != _msgSender() , "0xtower: the user cannot accept his own bid");
    ...
    function callOptionalReturn(IERC1155 token, bytes memory data) private {
        ...
        require(address(token).isContract(), "0xtower: call to non-contract");

        // solhint-disable-next-line avoid-low-level-calls
        (bool success, bytes memory returndata) = address(token).call(data);
        require(success, "0xtower: low-level call failed");

        if (returndata.length > 0) { // Return data is optional
            // solhint-disable-next-line max-line-length
            require(abi.decode(returndata, (bool)), "0xtower: ERC20 operation did not succeed");
        }
    }
 ...
 */
contract ERC11550xtower is ERC1155Marketplace, EIP712 {
    string private constant SIGNING_DOMAIN = "ERC11550xtower-Voucher";
    ...
    function redeem(
        ...
        require(chainId == getChainId(), "0xtower: Wrong chain");
        require(vouchersMintedQty[id] + qtyToMint <= qty, "0xtower: the qty exceeds the amount available for minting");
        require(nftId == 0 || exists(nftId), "0xtower: Wrong NFT ID");
    ...
    function verify(
        ...
        require(recoveredSigner == signer, "0xtower: wrong signature");
    }
