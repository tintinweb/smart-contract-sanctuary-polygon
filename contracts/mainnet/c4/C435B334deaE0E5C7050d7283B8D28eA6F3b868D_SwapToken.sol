...
contract SwapToken {
    ...
    event OneToMultiSwaps( address _tokenIn, address[] _tokenAddresses, uint[] _tokenAmounts, uint[] amountsOut );
    ...
    function addInDefinedTokenWithAggregator(address _tokenAddress, address _tokenAggregatorAddress) external onlyOwner {
    ...
    function removeFromDefinedTokenWithAggregator(address _tokenAddress) external onlyOwner {
        require(definedTokens[_tokenAddress],"CLAMP: TOKEN ADDRESS DOES NOT EXISTS"); // Check that the address is in the mapping
    ...
    function isDefinedToken( address _tokenAddress ) external view returns (bool) {
        return definedTokens[_tokenAddress];
    }

    function getAggregatorAddress( address _tokenAddress ) external view returns (address) {
    ...
    function MaticToMultiSwap( address[] memory _tokenAddresses, uint256[] memory _tokenAmounts, uint256 _amountTotal ) external payable {
        require(_tokenAddresses.length == _tokenAmounts.length, "Mismatched token addresses and amounts");
        MaticToMultiSwapWithReceiver( _tokenAddresses, _tokenAmounts, _amountTotal, msg.sender );
    }

    // need receiver if we want to swap from wallet contract
    function MaticToMultiSwapWithReceiver( address[] memory _tokenAddresses, uint256[] memory _tokenAmounts, uint256 _amountTotal, address _receiver ) public payable {
        require(msg.value > 0, "MSG.VALUE CONDITION FAILED - 1"); //will change msg.value in future
        require( msg.value == amountWithFee(_amountTotal), "MSG.VALUE CONDITION FAILED - 2" );

        totalValueCheck(_tokenAmounts, _amountTotal);

        require( areAddressesUniqueAndDefined(_tokenAddresses), "NOT DEFINED AND UNIQUE" );

        // Convert received MATIC to WMATIC
        _wmatic.deposit{value: _amountTotal}();
        _wmatic.approve(address(routerAddress), _amountTotal);

        multiSwapHelper( _wmaticAddress, _tokenAddresses, _tokenAmounts, _receiver );
    }
    ...
    function ERC20ToMultiSwap( address _tokenIn, address[] memory _tokenAddresses, uint256[] memory _tokenAmounts, uint256 _amountTotal, uint256 _amountWithFee ) external {
        require(_tokenAddresses.length == _tokenAmounts.length, "Mismatched token addresses and amounts");
        ERC20ToMultiSwapWithReceiver( _tokenIn, _tokenAddresses, _tokenAmounts, _amountTotal, _amountWithFee, msg.sender );
    }

    // need receiver if we want to swap from wallet contract
    function ERC20ToMultiSwapWithReceiver( address _tokenIn, address[] memory _tokenAddresses, uint256[] memory _tokenAmounts, uint256 _amountTotal, uint256 _amountWithFee, address _receiver ) public payable {
        require( _amountWithFee == amountWithFee(_amountTotal), "MSG.VALUE CONDITION FAILED - 2" );
        totalValueCheck(_tokenAmounts, _amountTotal);
        require( areAddressesUniqueAndDefined(_tokenAddresses), "NOT DEFINED AND UNIQUE" );
    ...
    //HELPERS
    // Private functions
    function multiSwapHelper( address _tokenIn, address[] memory _tokenAddresses, uint256[] memory _tokenAmounts, address _receiver) private {
        ...
        for (uint i = 0; i < _tokenAddresses.length; i++) {
            if (ERCIndex != i) {
                ...
                IERC20(_tokenIn).safeTransfer(wallets[_receiver], _tokenAmounts[i]);
                amountsOut[i] = _tokenAmounts[i];
            }
        }

        walletAddress.update( _tokenIn, _tokenAddresses, _tokenAmounts, amountsOut );
        emit OneToMultiSwaps( _tokenIn, _tokenAddresses, _tokenAmounts, amountsOut );
    }

    function areAddressesUniqueAndDefined( address[] memory _tokenAddresses ) internal view returns (bool) {
    ...
    function totalValueCheck( uint256[] memory _tokenAmounts, uint256 _amountTotal ) private pure {
        uint256 total = 0;
    ...
    function swapExactInputSingle( address _recipient, address _tokenIn, address _tokenOut, uint256 _amountIn ) internal returns (uint256 amountOut) {
    ...
    function getAmountOutMin( address _tokenIn, address _tokenOut, uint256 _amountIn ) internal view returns (uint256 amountOutMin) {
    ...
    function getDerivedPrice( address _tokenIn, address _tokenOut, uint8 _decimals ) internal view returns (int256) {
        require(
