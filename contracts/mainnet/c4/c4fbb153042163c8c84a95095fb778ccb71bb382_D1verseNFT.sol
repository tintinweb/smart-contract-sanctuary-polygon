...
contract D1verseNFT is IERC721, IERC721Metadata, IERC2981, ILazyMint {
    ...
    mapping(address => bool) public nftDEX;
    ...
    modifier onlyNftDex {
        require(nftDEX[msg.sender], "Caller is not NFT Dex Contract");
        _;
    }
    ...
    function setNftDao(address nft_dao) public onlyDAO {
        nftDAO = nft_dao;
    }
    ...
    function setNftName(string memory nft_name) public onlyOperator {
        _name = nft_name;
    }

    function setNftSymbol(string memory nft_symbol) public onlyOperator {
        _symbol = nft_symbol;
    }

    function setNftDex(address addr, bool flag) public onlyOperator {
        if(!flag) {
            delete nftDEX[addr];
        } else {
            nftDEX[addr] = true;
    ...
    function setContractURI(address contract_uri) public onlyOperator {
        contractURI = contract_uri;
    }

    function setMigration(address migration_contract) public onlyOperator {
        migration = migration_contract;
    }

    function migrateFrom(bool burn_old, uint256 begin_id, uint96 royaltyRate, uint256[] memory new_id) public onlyOperator {
        for(uint256 i = 0; i < new_id.length; i++) {
            uint256 old_id = begin_id + i;
            address owner = Burnable(migration).ownerOf(old_id);
            _mint(owner, new_id[i]);
            if(burn_old) {
                Burnable(migration).burn(old_id);
            }
            _royaltyInfo[new_id[i]].royaltyRate = royaltyRate;
            _royaltyInfo[new_id[i]].receiver = address(uint160(new_id[i] >> 96));
        }
    }
    ...
    function mintToRecipients(address[] memory recipients, uint256[] memory tokenIds, uint96 royaltyRate) public onlyOperator {
        ...
        for (uint i = 0; i < recipients.length; ++i) {
            ...
            _royaltyInfo[tokenIds[i]].royaltyRate = royaltyRate;
            _royaltyInfo[tokenIds[i]].receiver = address(uint160(tokenIds[i] >> 96));
        }
    }

    function mintToCreators(uint256[] memory tokenIds, uint96 royaltyRate) public onlyOperator {
        for (uint i = 0; i < tokenIds.length; ++i) {
            address to = address(uint160(tokenIds[i] >> 96));
            _mint(to, tokenIds[i]);
            _royaltyInfo[tokenIds[i]].royaltyRate = royaltyRate;
            _royaltyInfo[tokenIds[i]].receiver = to;
        }
    }
    ...
    function migrate(uint256[] memory tokenIds) public onlyOperator {
    ...
    function lazyMint(address to, uint256 tokenId, address receiver, uint96 royaltyRate) public override onlyNftDex {
        // _mint(to, tokenId);
        require(to != address(0), "ERC721: mint to the zero address");
        require(!_exists(tokenId), "ERC721: token already minted");
        ...
        _royaltyInfo[tokenId].receiver = receiver;
        _royaltyInfo[tokenId].royaltyRate = royaltyRate;
    ...
    function setRoyaltyInfo(uint256 tokenId, address receiver, uint96 royaltyRate) public override onlyNftDex {
        _royaltyInfo[tokenId].royaltyRate = royaltyRate;
        _royaltyInfo[tokenId].receiver = receiver;
    }
