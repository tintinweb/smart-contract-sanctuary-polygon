...
contract StrategyAm3Crv {
    ...
    address public gauge =
        address(0x20759F567BB3EcDB55c817c9a1d13076aB215EdC);
        ...
        external
    {
        ...
        // Swap on Sushi
        if (_crv > 0) {
            _swapOnSushi(crvToUsdcPath, _crv, maxSlippage);
            _zap();
        }
    }
    ...
    function harvestExtra(
        ...
        if (Gauge(gauge).reward_tokens(0) != address(0)) {
            Gauge(gauge).claim_rewards();
            _swapOnSushi(_pathToUsdc, _amount, maxSlippage);
            _zap();
        }
    }
    ...
    function _zap() internal {
        ...
        if (_usdc > 0) {
            ...
            ICurveFi(pool).add_liquidity([0, _usdc, 0], 0, true);
        }
	...
	function _swapOnSushi(address[] memory path, uint256 _amount, uint256 _maxSlippage) internal {
		IERC20(path[0]).safeApprove(sushiRouter, 0);
		IERC20(path[0]).safeApprove(sushiRouter, _amount);

		uint256[] memory amounts = ISushiSwapRouter(sushiRouter).getAmountsOut(_amount, path);

		uint256 minAmount = amounts[path.length - 1].mul(10000 - _maxSlippage).div(10000);

		ISushiSwapRouter(sushiRouter).swapExactTokensForTokens(
			_amount,
			minAmount,
			path,
			address(this),
    ...
    function setMaxSlippage(uint256 _maxSlippage) external onlyGovernance {
        maxSlippage = _maxSlippage;
    }

    function setCrvToUsdcPath(address[] calldata _crvToUsdcPath) external onlyGovernance {
        crvToUsdcPath = _crvToUsdcPath;
    }
}