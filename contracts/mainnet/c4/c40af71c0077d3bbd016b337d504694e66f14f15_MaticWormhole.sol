...
contract MaticWormhole is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    using AddressUpgradeable for address;
    using StringsUpgradeable for uint256;

    address public fxChild;
    address public ethPortal;
    IExoAvatar public exoAvatar;

    mapping(address => bool) public auth;

    event MessageSent(bytes message);
    event CallMade(address target, bool success, bytes data);
    event MessageReceived(address sender, address from, bytes data);
    event MessageReceived2(address sender, uint256[] ids);
    ...
    function initialize(
        ...
        fxChild = _fxChild;
        ethPortal = _ethPortal;
        exoAvatar = IExoAvatar(_exoAvatar);
    }
    ...
    function setEthPortal(address _ethPortal) external onlyOwner {
        ethPortal = _ethPortal;
    }

    function setAvatar(address _avatar) external onlyOwner {
        exoAvatar = IExoAvatar(_avatar);
    }
    ...
    function processMessageFromRoot(
        ...
        emit MessageReceived(rootMessageSender, msg.sender, data);
        require(msg.sender == fxChild, "INVALID_SENDER");
        require(rootMessageSender == ethPortal, "INVALID_PORTAL");

        _processMessageFromRoot(data);
    }

    function _processMessageFromRoot(bytes memory data) internal {
        (address initiator, uint256[] memory planetIds) = abi.decode(data, (address, uint256[]));
        exoAvatar.activateAvatars(initiator, planetIds);
    }
}
