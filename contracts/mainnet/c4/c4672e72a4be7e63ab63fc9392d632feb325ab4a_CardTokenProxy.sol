    ...
    OpenSeaCompatForLSP8
{
    ...
    error UnpackCardBatchDifferentArgumentLenght();
    ...
    function unpackCardBatch(address[] memory to, bytes32[] memory tokenId)
        public
        onlyRole(getRoleAdmin(DEFAULT_ADMIN_ROLE))
        returns (uint256)
    {
        ...
        if (to.length != tokenId.length) {
            revert UnpackCardBatchDifferentArgumentLenght();
        }

        for (uint256 i = 0; i < to.length; i++) {
            _mintableSupply = unpackCard(to[i], tokenId[i]);
        }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(AccessControl, IERC165, LSP8CompatibilityForERC721InitAbstract)
        returns (bool)
    {
