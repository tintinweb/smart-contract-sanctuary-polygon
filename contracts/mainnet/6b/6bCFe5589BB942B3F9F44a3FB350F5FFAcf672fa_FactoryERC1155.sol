...
contract FactoryERC1155 {
    ERC1155PresetMinterPauser[] public tokens; //an array that contains different ERC1155 tokens deployed
    ...
    function deployERC1155(string memory _contractName, string memory _uri, uint[] memory _ids, string[] memory _names) public returns (address) {
        ERC1155PresetMinterPauser t = new ERC1155PresetMinterPauser(_contractName, _uri, _names, _ids);
        tokens.push(t);
    ...
    function mintERC1155(uint _index, string memory _name, uint256 amount) public {
        require(msg.sender == tokens[_index].owner(), "not owner");
        uint id = tokens[_index].nameToId(_name);
        tokens[_index].mint(indexToOwner[_index], id, amount, "");
        emit ERC1155Minted(tokens[_index].owner(), address(tokens[_index]), amount);
    }

    function setURI(uint256 _index, uint256 _id, string memory _uri) public virtual {
        require(msg.sender == tokens[_index].owner(), "not owner");
        tokens[_index]._setURI(_id, _uri);
    }

    function burn(uint256 _index, uint256 _id, uint256 value) public virtual {
        require(msg.sender == tokens[_index].owner(), "not owner");
        tokens[_index]._burn(tokens[_index].owner(), _id, value);
    }
    ...
    function getERC1155byIndexAndId(uint256 _index, uint256 _id) public view returns (address _contract, address _owner, string memory _uri, uint supply) {
        ERC1155PresetMinterPauser token = tokens[_index];
 ...
 */
contract ERC1155PresetMinterPauser is ERC1155, AccessControlEnumerable, Ownable {
    ...
    function mint(address to, uint256 _id, uint256 amount, bytes memory data) public virtual returns (uint) {
     ...
     */
    function mintBatch(address to, uint256[] memory _ids, uint256[] memory amounts, bytes memory data) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "ERC1155PresetMinterPauser: must have minter role to mint");
        _mintBatch(to, _ids, amounts, data);
    }

    function _setURI(uint256 _id, string memory _uri) public virtual {
        emit URI(_uri, _id);
    }

    function _burn(address from, uint256 _id, uint256 amount) public virtual {
        ...
        _beforeTokenTransfer(operator, from, address(0), _ids, amounts, "");
        ...
        emit TransferSingle(operator, from, address(0), _id, amount);
    }
    ...
    function _beforeTokenTransfer(address operator, address from, address to, uint256[] memory _ids, uint256[] memory amounts, bytes memory data) internal virtual override(ERC1155) {
...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    function _asSingletonArray(uint256 element) public pure returns (uint256[] memory) {
