...
interface IRelayImplementation {
    /// @notice Executes a relayed call with given params
    /// @param actor Actor
...
import "../interfaces/IRelayImplementation.sol";

contract ViaRelay is Ownable, EIP712 {
    using Address for address;

    // CONSTANTS

    /// @notice EIP712 typehash used for relays
    bytes32 public constant RELAY_TYPEHASH =
        keccak256("Relay(address implementation,bytes data,uint256 nonce)");
    ...
    constructor() EIP712("Via Relay", "1.0.0") {}
    ...
    function relay(
        ...
        // Obtain actor (signature author if present, sender otherwise)
        address actor;
        if (signature.length > 0) {
            ...
            actor = ECDSA.recover(digest, signature);
        } else {
            actor = msg.sender;
        }
        ...
        if (nonce > 0) {
            ...
            require(!nonceUsed[actor][nonce], Errors.NONCE_ALREADY_USED);

            // Mark nonce as used
            nonceUsed[actor][nonce] = true;
        }

        // Relay to implementation
        implementation.functionDelegateCall(
            abi.encodeWithSelector(
                IRelayImplementation.relay.selector,
                actor,
                signedData,
                extraData,
                nonce
