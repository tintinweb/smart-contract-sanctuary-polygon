...
contract VoteEscrow is ERC20Votes, ReentrancyGuard, Ownable {
	...
	function setMultiFeeDistribution(address _multiFeeDistribution)
		external
		onlyOwner
	{
		require(
	...
	function create_lock(uint256 _value, uint256 _days) external {
		...
		require(
			multiFeeDistribution != address(0),
			"VoteEscrow: need to be set a multi fee distribution"
		);
	...
	function withdraw() external nonReentrant {
		 ...
		 */
		IMultiFeeDistribution(multiFeeDistribution).withdraw(
			mintedForLock[_msgSender()],
			_msgSender()
		);
	...
	function voting_power_unlock_time(uint256 _value, uint256 _unlockTime)
		public
		view
		returns (uint256)
	{
