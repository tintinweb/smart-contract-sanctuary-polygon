...
import "./lib/EIP712BaseUpgradeable.sol";
  ...
  EIP712BaseUpgradeable
{
  ...
  function __EIP712MetaTransactionUpgradeable_init(
    string memory name,
    string memory version
  ) internal onlyInitializing {
    __EIP712MetaTransactionUpgradeable_init_unchained(name, version);
  }

  function __EIP712MetaTransactionUpgradeable_init_unchained(
    string memory name,
    string memory version
  ) internal onlyInitializing {
    EIP712BaseUpgradeable._initialize(name, version);
  }
  ...
  EIP712MetaTransactionUpgradeable
{
  ...
  function mint(bytes32 r, bytes32 s, uint8 v) external {
    ...
    bytes32 digest = _hashTypedDataV4(
      keccak256(abi.encode(_MINT_TYPEHASH, msgSender()))
    );
