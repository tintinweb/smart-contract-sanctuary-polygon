...
// File: contracts/helpers/interfaces/ITDAIERC20.sol



pragma solidity ^0.8.4;

interface ITDAIERC20 {
...
// File: contracts/regular/TDAITreasury.sol
...
contract TDAITreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, STDAI }
    ...
    address public sTDAI;
    uint public sTDAIQueue; // Delays change to sTDAI address
    ...
    /**
        @notice allow approved address to deposit an asset for TDAI
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint TDAI needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( TDAI ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn TDAI for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ITDAIERC20( TDAI ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sTDAI ).balanceOf( msg.sender ); // Can only borrow against sTDAI held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with TDAI
        @param _amount uint
     */
    function repayDebtWithTDAI( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved as a Debtor" );

        ITDAIERC20( TDAI ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, TDAI, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( TDAI ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( TDAI ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns TDAI valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match TDAI decimals
            value_ = _amount.mul( 10 ** IERC20( TDAI ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.STDAI ) { // 9
            sTDAIQueue = block.number.add( blocksNeededForQueue );
        } else return false;
