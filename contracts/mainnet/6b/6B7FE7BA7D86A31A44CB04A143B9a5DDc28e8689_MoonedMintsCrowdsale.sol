...
// File contracts/interfaces/IMoonedMintsCrowdsale.sol


pragma solidity ^0.8.0;


interface IMoonedMintsCrowdsale {
    function initialize(
        address erc20_,
        address settings_,
        uint256 price_,
        address person_) external;
    function buy() payable external;

    event Bought(address who, uint256 amount);
}
...
// File contracts/interfaces/IMoonedMintsERC20.sol


pragma solidity ^0.8.0;
interface IMoonedMintsERC20 {
    function initialize(
        string memory name_,
...
// File contracts/MoonedMintsCrowdsale.sol


pragma solidity ^0.8.0;
contract MoonedMintsCrowdsale is
    Context,
    Initializable,
    ReentrancyGuard,
    IMoonedMintsCrowdsale {
    using SafeERC20 for IERC20;

    address public erc20;
    uint256 public price;
    IMoonedMintsSettings public settings;
    uint256 public crowdsaleTreasurePercent;
    ...
    function initialize(
        ...
        address settings_,
        uint256 price_,
        address person_) external override initializer {
        __MoonedMintsCrowdsale_init(erc20_, settings_, price_, person_);
    }

    function __MoonedMintsCrowdsale_init(
        address erc20_,
        address settings_,
        uint256 price_,
        address person_) internal onlyInitializing {
        require(price_ > 0, "MMC: bad price");
        erc20 = erc20_;
        settings = IMoonedMintsSettings(settings_);
        price = price_;
        person = person_;
        crowdsaleTreasurePercent = settings.crowdsaleTreasurePercent();
    }

    /*
     * @dev Buy MoonedMints ERC20 in exchange of ETH
     */
    function buy() external payable virtual override nonReentrant {
        require(msg.value > 0, "MMC: bad sent value");
        uint256 toSend = msg.value * crowdsaleTreasurePercent / 100;
        _safeTransferETH(settings.treasure(),  toSend);
        _safeTransferETH(person,  msg.value - toSend);
        toSend = msg.value * 10**IERC20Meta(erc20).decimals() / price;
        IERC20(erc20).safeTransfer(_msgSender(), toSend);
        emit Bought(_msgSender(), toSend);
    }
