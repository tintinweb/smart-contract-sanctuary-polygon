...
// Creating Rafflee 
  ...
  function fulfillRandomWords( uint256 _requestId, uint256[] memory _randomWords) internal override 
  {
    ...
    if (_raffle.entryFeesCollected > 0) {
        ...
        if (_prizePerEachWinner > 0) {
            ...
            uint256[] memory randomArray = _randomIndexArray(randomValue, _numberOfwinners, _entries.length);
  ...
  function _randomIndexArray(uint256 randomValue, uint256 length, uint256 range) internal pure returns (uint256[] memory randomArray) {
    randomArray = new uint256[](length);
    for (uint256 i = 0; i < length; i++) {
        randomArray[i] = uint256((uint256(keccak256(abi.encode(randomValue, i))) % range));
    }
    return randomArray;
  }
