...
contract MEGAMIFourSeasonsAndFlowers is ERC1155URIStorage, Ownable, RoyaltiesV2 {

    /**
     * @dev The name of the token
     */
    string private _name = "MEGAMI Four Seasons and Flowers";
    
    /**
     * @dev The symbol of the token
     */
    string private _symbol = "FSAF";
    ...
    /**
     @dev The struct used for airdropping tokens     */
    struct AirDropInfo {
    ...
    constructor() 
        ERC1155("") 
    {
        defaultRoyaltyInfo.recipientAddress = payable(address(this));
     ...
     * @param airdropInfo the information used for airdropping tokens
     */
    function airdrop(uint256 tokenId, AirDropInfo[] calldata airdropInfo)
        external
        onlyOwner
    {
        uint256 recipientsLength = airdropInfo.length;
        require(recipientsLength > 0, "no recipients");
        require(exists(tokenId), "token doesn't exist");

        for(uint256 i = 0; i < recipientsLength;) {
            mint(airdropInfo[i].recipientAddress, tokenId, airdropInfo[i].numberOfTokens, "");
    ...
    /**
     * @dev Allow owner to send funds directly to recipient.
