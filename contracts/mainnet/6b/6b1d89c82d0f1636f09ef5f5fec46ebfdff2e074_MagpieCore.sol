...
contract MagpieCore is ReentrancyGuardUpgradeable, OwnableUpgradeable, UUPSUpgradeable, Pausable, IMagpieCore {
    using LibAssetUpgradeable for address;
    using LibBytes for bytes;
    using LibSwap for IMagpieRouter.SwapArgs;
    ...
    mapping(uint8 => mapping(bytes32 => mapping(uint64 => mapping(address => uint256)))) public deposits;
    ...
    modifier onlyStargate() {
        require(msg.sender == config.stargateAddress, "MagpieCore: only stargate allowed");
        _;
    }
    ...
    function updateConfig(Config calldata _config) external override onlyOwner {
        ...
        require(_config.stargateAddress != address(0), "MagpieCore: invalid hyphenLiquidityPoolAddress");
    ...
    function _getWrapSwapConfig(
        ...
        } else if (fromAssetAddress == config.weth && toAssetAddress.isNative()) {
            wrapSwapConfig.prepareFromAsset = false;
            wrapSwapConfig.prepareToAsset = true;
            wrapSwapConfig.swap = false;
            wrapSwapConfig.unwrapToAsset = true;
        } else if (fromAssetAddress == toAssetAddress && swapArgs.assets.length == 1) {
            wrapSwapConfig.prepareFromAsset = false;
            wrapSwapConfig.prepareToAsset = false;
            wrapSwapConfig.swap = false;
            wrapSwapConfig.unwrapToAsset = false;
        ...
        wrapSwapConfig.transferFromSender = !fromAssetAddress.isNative() && transferFromSender;
    }
    ...
    function _wrapSwap(
        ...
        if (wrapSwapConfig.swap) {
            swapArgs.getFromAssetAddress().transfer(payable(config.magpieRouterAddress), amountIn);
            amountOuts = IMagpieRouter(config.magpieRouterAddress).swap(swapArgs);
    ...
    function swap(
        IMagpieRouter.SwapArgs calldata swapArgs
    )
        external
        payable
        onlyOwner
    ...
    {
        WrapSwapConfig memory wrapSwapConfig = _getWrapSwapConfig(swapArgs, true);
    ...
    function swapIn(
        ...
        whenNotPaused
        nonReentrant
        returns (uint256[] memory amountOuts, uint256 depositAmount, uint64 coreSequence, uint64 tokenSequence)
    {
        ...
        WrapSwapConfig memory wrapSwapConfig = _getWrapSwapConfig(args.swapArgs, true);
        ...
        if (msg.value > 0 && msg.value != args.swapArgs.getAmountIn()) {
            ...
            if (args.swapArgs.getFromAssetAddress().isNative()) {
                bridgeFee = msg.value > args.swapArgs.getAmountIn() ? msg.value - args.swapArgs.getAmountIn() : 0;
            }
        }

        (depositAmount, coreSequence, tokenSequence) = IMagpieBridge(config.magpieBridgeAddress).bridgeIn{
            value: bridgeFee
        }(args.bridgeType, args.payload, amountOut, toAssetAddress, msg.sender);
    ...
    function swapOut(
        ...
        returns (uint256[] memory amountOuts)
    {
        (IMagpieBridge.ValidationOutPayload memory payload, uint64 coreSequence) = IMagpieBridge(
            config.magpieBridgeAddress
        ).getPayload(args.bridgeArgs.encodedVmCore);

        require(!sequences[payload.senderNetworkId][coreSequence], "MagpieCore: already used sequence");
        ...
        uint256 depositAmount = payload.bridgeType == IMagpieBridge.BridgeType.Wormhole
            ? 0
            : deposits[payload.senderNetworkId][payload.senderAddress][coreSequence][fromAssetAddress];
        ...
        if (amountIn == 0) {
            amountIn = deposits[payload.senderNetworkId][payload.senderAddress][coreSequence][fromAssetAddress];
        }
        ...
        if (payload.to == msg.sender) {
            ...
            swapArgs.amountOutMin = 0;
        }

        require(payload.fromAssetAddress == fromAssetAddress, "MagpieCore: invalid fromAssetAddress");
        // if (msg.sender != payload.to) {
        //     require(payload.toAssetAddress == toAssetAddress, "MagpieCore: invalid toAssetAddress");
        // }
        require(payload.to == swapArgs.to && payload.to != address(this), "MagpieCore: invalid to");
        require(payload.recipientCoreAddress == address(this), "MagpieCore: invalid recipientCoreAddress");
        require(uint256(payload.recipientNetworkId) == config.networkId, "MagpieCore: invalid recipientChainId");
        // require(swapArgs.amountOutMin >= payload.amountOutMin, "MagpieCore: invalid amountOutMin");
        // require(swapArgs.routes[0].amountIn > payload.swapOutGasFee, "MagpieCore: invalid amountIn");
        require(swapArgs.getAmountIn() <= amountIn, "MagpieCore: invalid amountIn");

        swapArgs.routes[0].amountIn =
            swapArgs.routes[0].amountIn +
            (amountIn > swapArgs.getAmountIn() ? amountIn - swapArgs.getAmountIn() : 0) -
            payload.swapOutGasFee;

        WrapSwapConfig memory wrapSwapConfig = _getWrapSwapConfig(swapArgs, false);

        amountOuts = _wrapSwap(swapArgs, wrapSwapConfig);

        if (payload.swapOutGasFee > 0) {
            gasFeeAccumulatedByToken[fromAssetAddress] += payload.swapOutGasFee;
            gasFeeAccumulated[fromAssetAddress][msg.sender] += payload.swapOutGasFee;
        }

        emit SwappedOut(args, amountOuts, payload.senderNetworkId, coreSequence, payload.senderAddress, msg.sender);
    }
    ...
    function withdrawFunds(address tokenAddress) external onlyOwner {
        tokenAddress.transfer(payable(owner()), tokenAddress.getBalance());
    }
    ...
    function sgReceive(
        ...
        (uint8 networkId, bytes32 senderAddress, uint64 coreSequence) = payload.parseSgPayload();
