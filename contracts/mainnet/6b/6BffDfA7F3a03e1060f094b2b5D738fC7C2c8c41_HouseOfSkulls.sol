...
contract HouseOfSkulls is Ownable, ERC721Enumerable, ReentrancyGuard, ERC2981 {
    string private baseTokenURI;

    // Minting
    bool public paused;

    uint256 public whitelistStartTimestamp = 1683988800;
    uint256 public publicStartTimestamp = 1683990000;
    ...
    function addWhitelistDefault(
        address[] memory addresses_
    ) external onlyOwner {
        for (uint256 i = 0; i < addresses_.length; ) {
            whitelist[addresses_[i]] = 8;
            unchecked {
                ++i;
            }
        }
    ...
    function addWhitelist(
        ...
        for (uint256 i = 0; i < addresses_.length; ) {
            whitelist[addresses_[i]] = amount_[i];
            unchecked {
                ++i;
            }
        }
    ...
    function mint(uint256 amount) external payable nonReentrant {
        ...
        bool whitelisted = isWhitelisted(_msgSender());
        if (!whitelisted && block.timestamp < publicStartTimestamp)
            revert MintNotOpen();
        ...
        uint256 payout = totalPrice - launchpadRoyalties - royalties;
    ...
    function whitelistCount(address _address) public view returns (uint256) {
