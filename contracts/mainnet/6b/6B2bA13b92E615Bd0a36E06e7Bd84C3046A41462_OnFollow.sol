...
import {IPurchaseModule} from './interfaces/IPurchaseModule.sol';
 ...
 * @author Starit
 *
 * @notice This contract adds a public `HUB` immutable to inheriting modules, as well as an
 * `onlyHub` modifier.
 */
contract OnFollow {
    ...
    // status
    //   0: not-exits
    //   1: not-whitelisted / not-approved
    //   2: whitelisted / not-approved
    //   3: not-whitelisted / approved
    //   11: approved and whitelisted
    // status < 10 un-listed
    // status > 10 listed
    struct ModuleInfo {
        uint256 category;
        ...
        uint256 feeAmount;
        address receipt;
        address purchaseModule; // purchase strategy
    }

    address public governance;

    ModuleInfo[] public modules; //starts from zero // can be array?
    uint256 public modulesLength;
    mapping(address => uint256) public status; // contract address => status
    mapping(uint256 => mapping(uint256 => bool)) public paid; // Todo:: use nft to replace

    // modified
    modifier onlyGovernance() {
        require(msg.sender == governance, 'governance-address-only');
        _;
    }

    modifier onlyModuleOwner(uint256 _moduleId) {
        require(msg.sender == modules[_moduleId].owner, 'module-owner-only');
        _;
    }
    ...
    function addModule(
        ...
        uint256 _feeAmount,
        address _receipt,
        address _purchaseModule
    ) public onlyGovernance {
        if (_moduleAddress == address(0)) revert Errors.NoModuleContractSet();
        if (_receipt == address(0)) revert Errors.Invalid();

        // check is whitelisted 
        uint256 moduleId = modulesLength;
        
        ModuleInfo storage m = modules[moduleId];
        m.category = _category; // Todo:: limit category input
        m.contractAddress = _moduleAddress;
        m.feeToken = _feeToken;
        m.feeAmount = _feeAmount;
        m.receipt = _receipt;
        m.purchaseModule = _purchaseModule;

        bool whitelisted = isModuleWhitelistedByLens(moduleId);
        status[_moduleAddress] = whitelisted ? 2 : 1;
        emit ModuleAdded(_moduleAddress, _category, _feeToken, _feeAmount, _receipt, _purchaseModule);
    }

    function setModuleStatus(uint256 _moduleId, uint256 _status) public onlyGovernance {
        ModuleInfo storage m = modules[_moduleId];
        if (m.contractAddress == address(0)) revert Errors.NoModuleContractSet();
        status[m.contractAddress] = _status;
        emit ModuleStatusSet(_moduleId, _status);
    }

    function setModuleFeeAmount(uint256 _moduleId, uint256 _feeAmount) public onlyModuleOwner(_moduleId) {
        ModuleInfo storage m = modules[_moduleId];
        m.feeAmount = _feeAmount;
        emit ModuleFeeUpdate(_moduleId, m.feeToken, m.feeAmount);
    }

    function purchase(
        uint256 _moduleId,
        uint256 _profileId,
        bytes calldata purchaseModuleDatas
    ) external {
        _purchase(_moduleId, msg.sender, _profileId, purchaseModuleDatas);
    }
    ...
    function _purchase(
        ...
        address _payer,
        uint256 _profileId,
        bytes calldata _purchaseModuleData
    ) internal {
        ModuleInfo storage m = modules[_moduleId];
        // check if the module listed
        if (m.contractAddress == address(0)) revert Errors.NoModuleContractSet();
        // check if paid already
        if (paid[_moduleId][_profileId] == true) revert Errors.AlreadyPaid();
        // pay

        if (m.feeToken != address(0) && m.feeAmount != 0) {
            IERC20(m.feeToken).safeTransferFrom(_payer, address(this), m.feeAmount);
            // Todo:: make sure it fails if transfer fails
        }

        // execute purchase module strategy
        if (m.purchaseModule != address(0)) {
            IPurchaseModule(m.purchaseModule).processPurchase(
                _payer,
                _profileId,
                _purchaseModuleData
            );
        ...
        // if transfer failed then should revert
        // record the payment
        paid[_moduleId][_profileId] = true;
        // emit event
        emit ModulePurchased(_payer, _profileId, _moduleId);
    }

    function setLensModule(
        uint256 _profileId,
        uint256 _moduleId,
        ...
        ModuleInfo storage m = modules[_moduleId];
        // if not paid, then fail
        require(paid[_moduleId][_profileId] == true, 'not-paid-module');

        if (m.category == 0) {
            setFollowModule(_profileId, _moduleId, _data, _sig);
        } else if (m.category == 1) {
            setCollectModule(_moduleId);
        } else if (m.category == 2) {
            setReferenceModule(_moduleId);
        }
        revert Errors.InvalidModuleCategory();
    }

    function setFollowModule(
        uint256 _profileId,
        uint256 _onFollowModuleId,
        bytes calldata _followModuleInitData,
        DataTypes.EIP712Signature calldata _sig
    ) internal {
        ModuleInfo storage m = modules[_onFollowModuleId];

        ILensHub(HUB).setFollowModuleWithSig(
            DataTypes.SetFollowModuleWithSigData(
                _profileId,
                m.contractAddress,
                _followModuleInitData,
                _sig
            )
        );
        emit UserLensFollowModuleSet(_profileId, _onFollowModuleId, m.contractAddress);
    }

    function setCollectModule(uint256 _moduleId) internal {
        ModuleInfo storage m = modules[_moduleId];
    }

    function setReferenceModule(uint256 _moduleId) internal {
        ModuleInfo storage m = modules[_moduleId];
    }

    function isModuleWhitelistedByLens(uint256 _moduleId) internal returns (bool) {
        ModuleInfo storage m = modules[_moduleId];
        if (m.category == 0) {
            return isFollowModuleWhitelistedByLens(m.contractAddress);
        } else if (m.category == 1) {
            return isCollectModuleWhitelistedByLens(m.contractAddress);
        } else if (m.category == 2) {
            return isReferenceModuleWhitelistedByLens(m.contractAddress);
        }
        return false;
    }

    function isFollowModuleWhitelistedByLens(address followModule) internal view returns (bool) {
        return ILensHub(HUB).isFollowModuleWhitelisted(followModule);
    }

    function isCollectModuleWhitelistedByLens(address collectModule) internal view returns (bool) {
        return ILensHub(HUB).isCollectModuleWhitelisted(collectModule);
    }

    function isReferenceModuleWhitelistedByLens(address referenceModule) internal view returns (bool) {
        return ILensHub(HUB).isReferenceModuleWhitelisted(referenceModule);
    }


    event ModulePurchased(address, uint256, uint256);
    event ModuleAdded(address, uint256, address, uint256, address, address);
    event ModuleStatusSet(uint256, uint256);
    event ModuleFeeUpdate(uint256 moduleId, address feeToken, uint256 feeAmount);
    event UserLensFollowModuleSet(uint256 profileId, uint256 moduleId, address followModuleContract);
}
...
library Errors {
    error NotHub();
    error Invalid();      // general ,will be changed later
    error InitParamsInvalid();
    error NoModuleContractSet();
    error AlreadyPaid();
    error InvalidModuleCategory();
}
...
interface IPurchaseModule {
    ...
    /**
     * @notice Processes a given module purchase strategy, this can only be called from the onFollow contract.
     *
     * @param payer The payer address.
     * @param profileId The token ID of the profile receiving purchased module.
     * @param data Arbitrary data passed by the payer.
     */
