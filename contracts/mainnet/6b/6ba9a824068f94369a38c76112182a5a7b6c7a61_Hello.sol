...
contract Hello is AccessControl, ERC5633 {
    ...
    function airdrop(
        ...
        require(beneficiaries.length == amounts.length, "Hello: length mismatch");

        for (uint256 i = 0; i < beneficiaries.length; i++) {
            require(amounts[i] != 0, "Hello: zero amount");

            _mint(beneficiaries[i], id, amounts[i], "");
        }
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view override(AccessControl, ERC5633) returns (bool) {
        return super.supportsInterface(interfaceId);
    }

    function setSoulbound(uint256 id, bool soulbound) external onlyRole(DEFAULT_ADMIN_ROLE) {
