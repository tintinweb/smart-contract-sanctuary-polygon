...
import "../core/CozyCoin.sol";

contract BubbleWorldStore is Ownable {
  struct User {
    address account;
    string handle;
  }
  ...
  CozyCoin private coin;
  CozyRegistry private registry;

  // Set of all accounts that have minted
  mapping(address => bool) private accounts;

  // Set of all discord handles that have minted
  mapping(string => bool) private handles;

  error Locked();
  error IndexOutOfBound(uint index);
  error HandleMinted(string handle);
  error AccountMinted(address account);
  error RegistryIdInvalid();
  ...
  function getUsers(uint index, uint size) external view returns (User[] memory) {
    if (index >= users.length) {
      revert IndexOutOfBound(index);
    }

    uint remaining = users.length - index;
    uint total = remaining < size ? remaining : size;

    User[] memory result = new User[](total);

    for (uint i = 0; i < total; i++) {
      result[i] = users[i + index];
    }

    return result;
  }

  function hasAccountBought() external view returns (bool) {
    return accounts[msg.sender];
  }

  function hasHandleBought(string memory handle) external view returns (bool) {
    return handles[handle];
  }

  // ------------ User -------------

  function purchase(uint registryId, string calldata handle) external notLocked {
    if (msg.sender != registry.ownerOf(registryId)) {
      revert RegistryIdInvalid();
    }

    if (accounts[msg.sender]) {
      revert AccountMinted(msg.sender);
    }

    if (handles[handle]) {
      revert HandleMinted(handle);
    }
    ...
    User memory user = User(msg.sender, handle);
    users.push(user);
    accounts[msg.sender] = true;
    handles[handle] = true;
    supply += 1;

    bool sent = coin.transferFrom(msg.sender, address(this), price);

    if (!sent) {
      revert TransferFailed();
    }

    emit Purchased(msg.sender, registryId, handle, supply);
  }

  // ------------ Admin ------------

  function setPrice(uint _price) external onlyOwner notLocked {
    price = _price;
  }

  function setCap(uint _cap) external onlyOwner notLocked {
    cap = _cap;
  }
...
contract CozyCoin is ERC20PresetMinterPauser {
  string private constant _name = "Cozy Coin";
  string private constant _symbol = "COZY";

  error CapExceeded(uint amount, uint cap);
  error CapInvalid(uint prev, uint next);
  error Locked();

  uint public cap;
  bool public locked;

  constructor(uint _cap) ERC20PresetMinterPauser(_name, _symbol) {
    cap = _cap;
  }

  // ----------- Setters -----------

  function setCap(uint _cap) public onlyRole(DEFAULT_ADMIN_ROLE) notLocked {
    if (totalSupply() > _cap) {
      revert CapInvalid(cap, _cap);
    }
