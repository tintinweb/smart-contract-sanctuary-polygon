    ...
    ISecAccount
{
    ...
    bool public isRegist = false;

    function owner() public view returns (address) {
        if (isRegist) {
            try sec.owner() returns (address caOwner) {
                return caOwner;
            } catch {
                return address(0);
            }
    ...
    function _onlyOwner() internal view {
        ...
        if (msg.sender != address(this)) {
            try sec.isOwner(msg.sender) returns (bool result) {
    ...
    function _validateSignature(
        ...
        if (!isRegist) {
            ...
            try sec.owner() returns (address result) {
                if (result != signer) {
                    return SIG_VALIDATION_FAILED;
                }
            } catch {
        ...
        returns (bytes4)
    {
        ...
        try sec.isOwner(signer) returns (bool result) {
            if (result) {
                return 0x1626ba7e;
            } else {
                return 0xffffffff;
    ...
    function registerSuccess() public override {
        require(msg.sender == address(sec) && !isRegist, "account registed");
        isRegist = true;
