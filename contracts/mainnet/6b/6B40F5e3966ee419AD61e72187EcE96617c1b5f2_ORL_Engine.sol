...
contract ORL_Engine is Ownable, Concatenate {
    ...
    IERC721 public customContract = IERC721(customAddress);
    ...
    function createEvent(uint _id, address _conCheck, uint _conValue) public {
    ...
    function returnRacers(uint _event) public view 
    returns(uint[] memory _racersReal,
    address[] memory _contracts,
    uint[] memory _flow1,
    uint[] memory _flow2,
    uint[] memory _flow3){
        uint[] memory _racers = racersByEvent[_event];
        uint _len = _racers.length;
        uint[] memory _racersReal = new uint[](_len);
        address[] memory _contract = new address[](_len);
        uint[] memory _flow1 = new uint[](_len);
        uint[] memory _flow2 = new uint[](_len);
        uint[] memory _flow3 = new uint[](_len);
        for(uint i = 0; i < _len; i++){
            uint _slot = _racers[i];
            _racersReal[i] = perfByRacer[0][_slot].id;
            _contract[i] = perfByRacer[0][_slot].nftContract;
            _flow1[i] = perfByRacer[0][_slot].leg1;
            _flow2[i] = perfByRacer[0][_slot].leg2;
            _flow3[i] = perfByRacer[0][_slot].leg3;
        }
        return (_racersReal, _contracts, _flow1, _flow2, _flow3);
    }
