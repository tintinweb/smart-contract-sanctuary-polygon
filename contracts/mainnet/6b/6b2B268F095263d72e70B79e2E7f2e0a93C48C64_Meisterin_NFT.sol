...
contract Meisterin_NFT is ERC721Enumerable, Ownable {
    using Strings for uint256;
  ...
  modifier onlySoulboundOwner(uint256 tokenId) {
    require(originalOwners[tokenId] == msg.sender, "Only original owner can transfer soulbound token");
    _;
  }
  ...
  function _transfer(address from, address to, uint256 tokenId) internal virtual override onlySoulboundOwner(tokenId) {
    super._transfer(from, to, tokenId);
  }

  function _approve(address to, uint256 tokenId) internal virtual override onlySoulboundOwner(tokenId) {
    super._approve(to, tokenId);
  }

  // public
  function mint(address _to, uint256 _mintAmount, string memory code) public payable onlyOwner {
    uint256 supply = totalSupply();
    require(!paused);
    require(_mintAmount > 0);
    require(_mintAmount <= maxMintAmount);
    require(supply + _mintAmount <= maxSupply);
    require(usedCodes[code] == false, "Already minted");

    for (uint256 i = 1; i <= _mintAmount; i++) {
      usedCodes[code] = true;
      uint256 tokenId = supply + i;
      _safeMint(_to, tokenId);
      originalOwners[tokenId] = _to;
    }
  }
