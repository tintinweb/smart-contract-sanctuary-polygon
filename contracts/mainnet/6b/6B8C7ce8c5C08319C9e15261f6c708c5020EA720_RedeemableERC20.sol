...
// File: contracts/RedeemableERC20.sol
 ...
 * <3
 */
contract RedeemableERC20 is EIP712MetaTransaction("RedeemableERC20", "1") {
    /**
     * @dev Emitted when token `tokenId` in ERC20-compliant contract `tokenContract` is deposited into slot `slotId`.
     */
    event Deposit(
    ...
    function depositERC20(
        address tokenContract, /* The contract of erc20 tokens being recieved */
        uint256 value, /* The amount of the tokens being recieved */
        bytes calldata data
    ) external {
        // Parse the redeemer key from the data
        require(
            data.length == 32,
            "RedeemableERC20: Token received data must be a 32-byte public key"
        );
        ...
        require(
            ...
            "RedeemableERC20: A slot with that ID/redeemer key has already been used. Generate a new key to deposit this token."
        );
        IERC20 tokenContractInterface = IERC20(tokenContract);
        tokenContractInterface.transferFrom(msgSender(), address(this), value);
        ...
        emit Deposit(redeemerKey, tokenContract, value);
    }
    ...
    function redeem(address slotId, address to) external {
        require(
            isSlotActive(slotId),
            "RedeemableERC20: Requested slot is not active."
        );
        ...
        require(
            msgSender() == slotId,
            "RedeemableERC20: This caller is not authorized to redeem this slot."
        );
        ...
        IERC20 tokenContract = IERC20(slotCopy.tokenContract);
        // transfer can fail and revert.
        tokenContract.transfer(to, slotCopy.tokenAmount[0]);
