...
contract DragonTigerBase is Manager {
    mapping(address => uint16[]) cards;
    uint256 randNonce = 0;
    uint16 tieRate = 5000;
    uint40[] public odds = [2, 9, 2];
    ...
    function updateCards(address player) internal {
        uint16[] memory card = cards[player];
        if (card.length == 0 || card.length <= 138) {
            uint16[] memory newCard = new uint16[](418);
            for (uint16 i = 0; i < 418; i++) {
                newCard[i] = i;
            }
            for (uint8 i = 0; i < 8; i++) {
                uint256 index = randMod(newCard.length);
                delete newCard[index];
            }

            cards[player] = newCard;
        }
    }

    function changeTieRate(uint16 rate) external onlyOwner {
        tieRate = rate;
    }

    /** @param numChoice number bet choice, eg: "100" means player choose bet dragon, numChoice will be 4;
     * @param betAmount uint array, eg: [40000000000000000,0,0];
    ...
    function placeBetBase(
        ...
        (totalBetAmount, totalWinnableAmount) = checkBetAmount(
            betAmount,
            agentID,
            token
        );
        updateCards(msg.sender);
        ...
        virtual 
    {
        ...
        uint16[] memory card = new uint16[](2);

        (card[0], card[1]) = calcCard(
            bet.player,
            randomNumber
        );
        ...
        uint128 charge = 0;
        (bet.winAmount, charge, bet.outcome) = calc(
            bet.agentID,
            bet.betAmount,
            bet.choice,
            card
        ...
        )
    {
        ...
        uint16 dragonOut = (((card[0] % 52) + 1) / 4 + 1);
        uint16 tigerOut = (((card[1] % 52) + 1) / 4 + 1);
        uint16 outcome;

        if (dragonOut > tigerOut) {
            outcome = 0;
            if (choice & (1 << outcome) > 0) {
                winAmount =
                    (betAmount[outcome] * (10000 - chargeRate)) /
                    10000 +
                    betAmount[outcome];
                charge += (betAmount[outcome] * chargeRate) / 10000;
            }
        }
        ...
        if (dragonOut == tigerOut) {
            ...
            if (choice & (1 << outcome) > 0) {
              ...
              winAmount += (betAmount[0] * tieRate) / 10000;
              winAmount += (betAmount[2] * tieRate) / 10000;
            }
        }
        return (uint128(winAmount), uint128(charge), outcome);
    }
    ...
    function calcCard(
        ...
        uint256 randomNumber1 = 0;
        uint256 randomNumber2 = 0;
        (randomNumber1, randomNumber2) = getRandoms(randomNumber);
        uint16[] memory card = cards[_player];
        uint256 indexDragon = randomNumber1 % card.length;
        uint16 dragon = card[indexDragon];
        delete card[indexDragon];

        uint256 indexTiger = randomNumber2 % card.length;
        uint16 tiger = card[indexTiger];
        delete card[indexTiger];

        return (dragon, tiger);
    }

    function getRandoms(uint256 randomWords)
        public
        pure
        returns (uint256, uint256)
    {
        string memory s = Strings.toString(randomWords);
        uint256 length = bytes(s).length;
        uint256 firstEnd = length / 2;
        uint256 secondStart = firstEnd + 1;
        string memory firstStr = getSlice(1, firstEnd, s);
        string memory secondStr = getSlice(secondStart, length, s);

        return (st2num(firstStr), st2num(secondStr));
    }
...
contract DragonTiger is DragonTigerBase, ReentrancyGuard, Random {
    /** @param numChoice number bet choice, eg: "100" means player choose bet dragon, numChoice will be 4;
     * @param betAmount uint array, eg: [40000000000000000,0,0];
     * @param agentID which agent player belongs;
     * @param tokenString bet token, eg: "MATIC" or "USDT";
    ...
    function placeBet(
        ...
        uint256 betId = placeBetBase(numChoice, betAmount, agentID, tokenString);
