...
contract PhaverIssuer is Pausable, Ownable {
    ...
    /*                          STORAGE                           */
    /*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/
    uint256 public constant TOKEN_ID = 1; // force id to be 1
    AccountBoundToken public immutable _ABT;
    ...
    /// @notice SBT address is set in the constructor.
    /// @param ABTAddress The address of the SBT contract.
    /// @param relayer The address of the relayer.
    constructor(address ABTAddress, address relayer) {
        _ABT = AccountBoundToken(ABTAddress);
        _relayer = relayer;
    }
    ...
    /// @notice Issues a Phaver SBT to the provided receiver address.
    ...
    function issueCredential(
        ...
        bytes32 socialsHash = keccak256(
            abi.encodePacked(receiver, twitterIdHash, discordIdHash)
        );
        // Store hash of socials for verification in updateCredential
        _socialsHash[receiver] = socialsHash;

        _ABT.issue(receiver, TOKEN_ID, credentialURL);
    }

    /// @notice Airdrops SBTs to a list of addresses.
    /// @param batch List of BatchData for SBTs to be airdropped to.
    function batchIssueCredentials(BatchData[] memory batch)
        external
        whenNotPaused
        onlyRelayer
    {
        for (uint256 i; i < batch.length; ++i) {
            BatchData memory item = batch[i];
            address receiver = item.receiver;
            bytes32 twitterIdHash = item.twitterIdHash;
            bytes32 discordIdHash = item.discordIdHash;
            string memory credentialURL = item.credentialURL;
    ...
    function updateCredential(
        ...
        if (issuer != _relayer) revert InvalidProof();

        // check the provided socialsHash against the stored one.
        if (_socialsHash[receiver] != socialsHash)
            revert InvalidSocialsHash(socialsHash);

        _ABT.update(receiver, TOKEN_ID, credentialURL);
    }
    ...
    /// @param twitterIdHash Hash of the caller's twitter credential.
    /// @param discordIdHash Hash of the caller's discord credential.
    function _deduplicateCredential(
        bytes32 twitterIdHash,
        bytes32 discordIdHash
    ) internal {
