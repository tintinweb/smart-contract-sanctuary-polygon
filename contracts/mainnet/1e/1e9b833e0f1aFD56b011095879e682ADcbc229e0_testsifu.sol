...
/**
 * Regal Sifu token smart contract
 * @author Regal Sifu Team
 */

contract testsifu is ERC20, Ownable {
    ...
    bool public transferRestEnabled = true;
    ...
    address private constant reserveWallet =
        0x3B2db4d00851bf070495cc257376BE5f76103B23;
    address private liquidityWallet;

    uint256 public maxSellTransactionAmount = 5000000 * 1e18; // 0.5% of total supply 1B
    uint256 public maxBuyTransactionAmount = 20000000 * 1e18; // 2% of total supply 1B

    uint256 private feeUnits = 1000; // 1000 represents 100%
    uint256 public standardFee = 200; // 4% buy fees, 20% at launch antibot
    uint256 public liquidityFee = 100; // 2% to liquidity, 10% at launch
    uint256 public burnFee = 100; // 2% burn, 10% at launch

    uint256 public antiDumpFee = 300; // 2% extra for sell transactions; at launch 30% for 30 mins
    uint256 public antiDumpBurn = 150; // extra 2% is burnt; at launch 15% for 30 mins
    uint256 public antiDumpLiq = 150; // 15% for first 30mins of launch will be changed to 0

    uint256 private liquidityBalance;
    uint256 public swapTokensAtAmount = 1000000 * (10 ** 18); // Contract accrues token which are swapped and liquified and burned
    ...
    constructor() ERC20("test", "test") {
        //  Deployer is a liquidity provider
        liquidityWallet = owner();

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506 // UniswapRouter
        );
    ...
    function setTransferRestEnabled(bool _enabled) external onlyOwner {
        transferRestEnabled = _enabled;
    }
    ...
    function updateLiquidityWallet(
        ...
        require(
            ...
            "Regal Sifu: The liquidity wallet is already this address"
        );
    ...
    function updateStandardFees(
        ...
        burnFee = _burnFee;
        standardFee = _liquidityFee + _burnFee;
        require(
            standardFee <= 300,
            "Regal Sifu: Standard fee should be less than or equal 30%"
        );
    }

    function updateAntiDumpFees(
        uint256 _antiDumpBurn,
        uint256 _antiDumpLiq
    ) external onlyOwner {
        antiDumpBurn = _antiDumpBurn;
        antiDumpLiq = _antiDumpLiq;
        antiDumpFee = _antiDumpBurn + _antiDumpLiq;

        require(
            antiDumpFee <= 300,
            "Regal Sifu: Antidump fee should be less than or equal 30%"
        );
    }
     ...
     */
    function excludeFromWhiteList(address _address) external onlyOwner {
     ...
     * Function that allows only the owner to exclude an address from whitelist
     */
    function excludeAddressFromBlackList(address _address) external onlyOwner {
    ...
    function removeBadToken(IERC20 Token) external onlyOwner {
        require(
            ...
            "Regal Sifu: You cannot remove this Token"
        );
        Token.transfer(owner(), Token.balanceOf(address(this)));
    }

    /**
     * Function that allows only the owner to remove SIFU tokens, ETH from the contract
     */
    function _removeDust() private {
        ...
        require(success, "Regal Sifu: ETH TRANSFER FAILED");
    }
    ...
    function updateMaxSellAmount(uint256 _max) external onlyOwner {
        require(_max >= 2500000 * 1e18 && _max <= 30000000 * 1e18);
    ...
    function updateMaxBuyAmount(uint256 _max) external onlyOwner {
        require(_max >= 2500000 * 1e18 && _max <= 50000000 * 1e18);
    ...
    function _transfer(
        ...
        if (
            ...
            require(tradingEnabled, "Regal Sifu: Trading Disabled");
            require(
                block.timestamp >= tradingEnabledTimestamp ||
                    whiteList[from] ||
                    whiteList[to],
                "Regal Sifu: Trading Still Not Enabled"
            );
            ...
            uint256 burnAmount = (amount * (burnFee)) / (feeUnits);
            uint256 liquidityAmount = (amount * (liquidityFee)) / (feeUnits);

            if (automatedMarketMakerPairs[from]) {
                require(
                    amount <= maxBuyTransactionAmount,
                    "Regal Sifu: Max Buy Amount Error"
                );
                if (transferRestEnabled && !whiteList[to]) {
                    require(
                        lastTransactionTime[to] == 0 ||
                            block.timestamp - lastTransactionTime[to] >= 600,
                        "Regal Sifu: Only one transaction allowed within 10 minutes"
                    );
            ...
            if (automatedMarketMakerPairs[to]) {
                require(
                    ...
                    "Regal Sifu: Max Sell Amount Error"
                );
                if (transferRestEnabled && !whiteList[from]) {
                    require(
                        lastTransactionTime[from] == 0 ||
                            block.timestamp - lastTransactionTime[from] >= 600,
                        "Regal Sifu: Only one transaction allowed within 10 minutes"
                    );
                    lastTransactionTime[from] = block.timestamp;
                }

                fees = fees + ((amount * (antiDumpFee)) / (feeUnits));
                burnAmount =
                    burnAmount +
                    ((amount * (antiDumpBurn)) / (feeUnits));
                liquidityAmount =
                    liquidityAmount +
                    ((amount * (antiDumpLiq)) / (feeUnits));
            }
            _burn(from, burnAmount);
            super._transfer(from, address(this), fees - burnAmount);
            liquidityBalance = liquidityBalance + (liquidityAmount);
    ...
    /**
     * Function: To swap accrued fees and add to liquidity
     */
    ...
    function setAutomatedMarketMakerPair(
        ...
        require(
            pair != uniswapV2Pair,
            "Regal Sifu: The Uniswap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "Regal Sifu: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
