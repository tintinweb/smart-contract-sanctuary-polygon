...
contract SplitterContract is AccessControl, SplitterInterface {
    ...
    mapping(address => uint) private _participantAddressToIdMap;

    string[] private _variableNameArray;
    ...
    constructor() {
        ...
        _variableNameArray = new string[](2);
        _variableNameArray[0] = "weight";
        _variableNameArray[1] = "destinationAddress";

        _variableHashArray = new bytes8[](2);
        _variableHashArray[0] = bytes8(keccak256(bytes(_variableNameArray[0])));
        _variableHashArray[1] = bytes8(keccak256(bytes(_variableNameArray[1])));
    }
    ...
    //* PUBLIC REGION *//
    /////////////////////
    function splitFunds() public payable override returns (bool) {
        for (uint i = 0; i < _participantCount; i++) {
            ParticipantData memory currentParticipant = getParticipantData(i);
            uint amountToClaim = (msg.value * currentParticipant.weight) / _combinedWeight;
            require(amountToClaim > 0, "no funds to claim");
            payable(currentParticipant.destinationAddress).transfer(amountToClaim);
            emit FundsSent(currentParticipant.destinationAddress, amountToClaim);
        }
        return true;
    }
    ...
    //* MANAGER_ROLE REGION *//
    ///////////////////////////
    function addParticipant(uint weight_, address destinationAddress_) public onlyRole(MANAGER_ROLE) override returns (bool) {
        ParticipantData memory newParticipant = ParticipantData(0, weight_, destinationAddress_);
    ...
    function addParticipantWeight(uint participantId_, uint weight_) public onlyRole(MANAGER_ROLE) override returns (bool) {
        return editParticipant(participantId_, _variableHashArray[0], bytes32(_participantMap[participantId_].weight + weight_));
    }
    ...
    function editParticipant(uint participantId_, bytes8 variableHash_, bytes32 value_) public onlyRole(MANAGER_ROLE) override returns (bool) {
        if (variableHash_ == _variableHashArray[0]) {
            ...
            _participantAddressToIdMap[newDestinationAddress] = participantId_;
    ...
    function _appendParticipant(ParticipantData memory participant_) internal {
        ...
        _participantAddressToIdMap[participant_.destinationAddress] = getParticipantCount();

        _participantCount += 1;
        _combinedWeight += participant_.weight;

        emit ParticipantAdded(participant_.id, participant_.weight, _combinedWeight, participant_.destinationAddress);
    }
    ...
    function _removeParticipant(uint participantId_) internal {
        ...
        emit ParticipantRemoved(currentParticipant.id, currentParticipant.weight, _combinedWeight, currentParticipant.destinationAddress);
    ...
    function _switchParticipants(uint firstTokenId_, uint secondTokenId_) internal {
        ...
        _participantAddressToIdMap[firstParticipant.destinationAddress] = secondTokenId_;

        secondParticipant.id = firstTokenId_;
        _participantMap[firstTokenId_] = secondParticipant;
        _participantAddressToIdMap[secondParticipant.destinationAddress] = firstTokenId_;
    ...
    function getVariables() public view override returns (string[] memory, bytes8[] memory) {
        return (_variableNameArray, _variableHashArray);
    }
}
...
interface SplitterInterface is IAccessControl {
    ...
    //* PUBLIC REGION *//
    /////////////////////
    function splitFunds() payable external returns (bool);

    ///////////////////////////
    //* MANAGER_ROLE REGION *//
    ///////////////////////////
    function addParticipant(uint weight_, address destinationAddress_) external returns (bool);
    ...
    function addParticipantWeight(uint participantId_, uint weight_) external returns (bool);
    ...
    event ParticipantAdded(uint id, uint weight, uint combinedWeight, address destinationAddress);
    ...
    event ParticipantRemoved(uint id, uint weight, uint combinedWeight, address destinationAddress);
