    ...
    Pausable
{
    ...
    /**
     * Override isApprovedForAll to whitelist user's CsPort proxy accounts to enable gas-free listings.
     */
    ...
    {
        // Whitelist CsPort proxy contracts for easy trading.
    ...
    function mint(
        ...
        uint256 _quantity,
        bytes memory _data
    ) internal virtual onlyOwnerOrProxy {
    ...
    /**
     * This is used instead of msg.sender as transactions won't be sent by the original token owner, but by CsPort.
     */
...
/**
 * @title AssetContract
 * AssetContract - A contract for easily creating non-fungible assets on CsPort.
 */
contract AssetContract is ERC1155Tradable {
    ...
    /**
     * Compat for factory interfaces on CsPort
    ...
    function safeTransferFrom(
        ...
        if (mintedBalance < _amount) {
            ...
            mint(_from, _to, _id, _amount - mintedBalance, _data);
            if (mintedBalance > 0) {
...
contract CSNFT1155 is AssetContract, ReentrancyGuard {
    ...
    function mint(
        ...
        uint256 _quantity,
        bytes memory _data
    ) internal override nonReentrant creatorOnly(_id) {
        _mint(_from, _to, _id, _quantity, _data);
    }
