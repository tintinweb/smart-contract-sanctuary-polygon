...
contract WEB3AM_FlashCrossDexArbitrage is IUniswapV3FlashCallback, PeripheryImmutableState, PeripheryPayments {
 ...
 /* ----------------------------------------------------State Variable Definition----------------------------------------------------------- */
    ...
    address public constant USDC = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;          // USDC contract address on polygon mainnet
    address public constant WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;        // WMATIC contract address on polygon mainnet
    address public constant WETH = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619;          // WETH contract address on polygon mainnet
   ...
   */
    ...
    function updateAdmin (address _walletaddrs) external onlyAdmin {
        require(_walletaddrs != address(0) , "IA");
        admin = _walletaddrs ;
        emit UpdateAdminEvent (msg.sender , _walletaddrs , "Admin Updated to new address");
 ...
 /* ------------------------------------------------------------External Function - updateProfitCollector---------------------------------------------------------------------------- */
   ...
   */
    ...
    function updateProfitCollector (address _walletaddrs) external onlyAdmin {
        ...
        emit UpdateProfitCollectorEvent (msg.sender , _walletaddrs , "Profit Collector Updated to new address");
 ...
 /* ---------------------------------------------------- Defining Events ----------------------------------------------------------- */
       ...
       event FlashLoanSuccess( string logInfo , uint256 date , uint256 wmatictokenAmount , uint256 usdctokenAmount);
       event FlashLoanStart(string logInfo);
       event UniswapV3SwapSuccessful(string logInfo);
       event SushiSwapV2SwapSuccessful(string logInfo);
       event TransferPoolFeeToken(string indexed logInfo, uint indexed amout);
       event WithdrawPoolFeeToken (string indexed logInfo, uint indexed amout);
       event UpdateAdminEvent(address sender , address newWalletAddress , string logInfo );
       event UpdateProfitCollectorEvent(address sender , address newWalletAddress , string logInfo);
...
/* ----------------------------------------------------External Function - uniswapV3FlashCallback----------------------------------------------------------- */
   ...
   */  
    ...
    function uniswapV3FlashCallback(
        ...
        emit FlashLoanStart("Flash Swap Start");
       ...
       uint swap_out = transfer_wrapper_uniV3(WMATIC,USDC, amount_swap);     
        ...
        emit UniswapV3SwapSuccessful("Successfuly Bought USDC on UNISWAPV3");
        ...
        emit SushiSwapV2SwapSuccessful("Successfuly Bought back WMATIC on UNISWAPV3");
        ...
        uint256 amount1Owed = LowGasSafeMath.add(decoded.amount1, fee1);
         ...
         if (amount0Owed < 0 && amount1Owed < 0 ) {
            revert("Transaction Reverted due to loss");
        }
    ...
    */
    ...
    function initFlash(FlashParams memory params) external {
        ...
        emit FlashLoanSuccess('flash Loan Success', block.timestamp ,(IERC20(WMATIC).balanceOf(address(this))), IERC20(USDC).balanceOf(address(this)));
    }

    

   /* ---------------------------------------------------- External Function | Access Controlled By Admin-------------------------------------------------------------------------------*/
     ...
     function transferTokenForFee(uint amount) external onlyAdmin returns (bool) {
       IERC20(WMATIC).transferFrom(msg.sender,address(this), amount);
       emit TransferPoolFeeToken("Transfer Success", amount);
       return true;
    }
      ...
      */
      ...
      function withdrawTokenOfFee(uint amount)external onlyAdmin returns(bool) {
       IERC20(WMATIC).transfer(msg.sender, amount);
        emit WithdrawPoolFeeToken("Transfer Success", amount);
        return true;
   ...
   */  
    ...
    function sendMaticToContract() public payable {}
    ...
    /* ----------------------------------------------------Public Function | Access Controlled By Admin-------------------------------------------------------- */
   
       function withdrawNativeCurrency(address payable _to) public onlyAdmin {
...
contract UniswapV3Swap {
    ...
    function swapTokenMax(address tokenIn, address tokenOut, uint256 amountIn) external returns (uint256 amountOut) {
        ...
        // The call to `exactInputSingle` executes the swap given the route.  // Multihop ?
