...
contract PlanetNewRefer is OwnableUpgradeable {
    ...
    mapping(address => bool) public isUserAdd;
    uint public totalWhite;
    ...
    function initialize() public initializer {
        ...
        reward = [100e6, 500e6, 1000e6, 2000e6];
        referLimit = [10, 60, 150, 500];

    }

    function addRefer(address[] memory addrs, bool b) external onlyOwner {
        for (uint i = 0; i < addrs.length; i++) {
            if(userInfo[addrs[i]].isRefer){
                continue;
            }
            totalWhite += 1;
    ...
    function setReward(uint[] memory rew) external onlyOwner {
        reward = rew;
    }

    function setReferLimit(uint[] memory referList_) external onlyOwner {
        referLimit = referList_;
    }

    function addReferList(address refer, address[] memory addrs) external onlyOwner {
        require(userInfo[refer].isRefer, 'not refer');
        userInfo[refer].referAmount += addrs.length;
        for (uint i = 0; i < addrs.length; i++) {
            require(!isUserAdd[addrs[i]], 'already add');
            isUserAdd[addrs[i]] = true;
        }
    }

    function getUserToClaim(address addr) public view returns (uint){
        if (!userInfo[addr].isRefer) {
            return 0;
        }
        uint amount = userInfo[addr].referAmount;
        if (amount < referLimit[0]) {
            return 0;
        }
        if (amount >= referLimit[3]) {
            return reward[3];
        } else if (amount >= referLimit[2]) {
            return reward[2];
        } else if (amount >= referLimit[1]) {
            return reward[1];
        } else {
            return reward[0];
        }
    ...
    function claimReward(uint index) external {
        require(userInfo[msg.sender].isRefer, 'not refer');
        require(index < reward.length, 'wrong index');
        require(userInfo[msg.sender].referAmount >= referLimit[index], 'not enough refer amount');
        require(userInfo[msg.sender].claimed < reward[index]);
        uint rew = reward[index] - userInfo[msg.sender].claimed;
        require(rew != 0, 'no reward');
        usdt.transfer(msg.sender, rew);
        userInfo[msg.sender].claimed = reward[index];
