...
contract LuarWorldCardholder is ERC721A, IERC2981, Admin {
    ...
    function mint(address to, uint256 qty, bool limitMint) public onlyAdmins {
        _mintCheck(qty);
        require(_paused == false, 'Contract is paused');
        if (limitMint) {
            require(!didMint[to], 'Already claimed for user');
            didMint[to] = true;
        }
        _mint(to, qty);
    }

    function _mintCheck(uint256 qty) internal view {
        require(_totalMinted() + qty <= _maxSupply, 'Max supply reached');
    }
    ...
    function setRoyaltyPercentage(uint256 _royaltyPercentage) public onlyOwner {
        require(
            ...
            'Royalty percentage must not exceed 100'
        );
