...
contract BulkGrinder is Ownable, IERC721Receiver {
    ...
    constructor (address _grinder, address _comethItems, address[] memory _resources, uint256 _fees) Ownable() {
        grinder = _grinder;
    ...
    function setFees (uint256 feesRate) public onlyOwner {
        require(feesRate < 100, "Fees too high");
        fees = feesRate;
    }
    ...
    function bulkGrind (uint256[] memory itemIDs, IGrindHelper.Ingredient[] memory ingredients) external {
        ...
        for (uint i = 0; i < resources.length; i++) {
            uint256 profit = IERC20(resources[i]).balanceOf(address(this));
            uint256 taxes = profit * fees / 1000;
