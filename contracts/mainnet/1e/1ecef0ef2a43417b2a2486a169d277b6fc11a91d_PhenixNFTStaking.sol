...
contract PhenixNFTStaking is Ownable {
    ...
    uint256 public dailyRewardRate = (3 * FXP_BASE) / 100;
    uint256 public maxNFTsPerTransaction = 100;
    ...
    constructor(address _rewardToken, address _erc721Token) {
        // PHNX TOKEN (Mainnet): 0x57d06bB1e3B60C875cD3A4445a53217F9B44d390;
        // FPC NFT Token (Mainnet): 0xCf185Cb08c62Ff969D258FBA18F92dabb278A8A9;
        ...
        _setStakingTimeCommitment(7 * 86400, 100, 100); // 7 Days (1.0x Multiplier)
        _setStakingTimeCommitment(15 * 86400, 105, 100); // 15 Days (1.05x Multiplier)
        _setStakingTimeCommitment(30 * 86400, 110, 100); // 30 Days (1.10x Multiplier)
        _setStakingTimeCommitment(45 * 86400, 115, 100); // 45 Days (1.15x Multiplier)
        _setStakingTimeCommitment(60 * 86400, 120, 100); // 60 Days (1.20x Multiplier)
    }
    ...
    function rewardOf(uint256 _tokenId) public view returns (uint256) {
        if (
            ...
            uint256 totalTokenAllocation = IERC20(rewardToken).balanceOf(
                address(this)
            );
