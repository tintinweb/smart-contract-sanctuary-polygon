...
contract LuxyStake is Pausable, Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    /* ========== STATE VARIABLES ========== */
    IERC20 public luxyToken;
    uint256 public periodFinish;
    uint256 public rewardRate;
    uint256 public constant rewardsDuration = 270 days;
    ...
    event Composed(address indexed user, uint256 reward);
    ...
    /* ========== CONSTRUCTOR ========== */
    /**
     * @param _luxyToken Address of the Luxy token
     */
    constructor(address _luxyToken) {
        luxyToken = IERC20(_luxyToken);
    }
    ...
    {
        require(amount > 0, "LuxyStake::CANNOT_STAKE_ZERO");
        _totalSupply += amount;
        _balances[_msgSender()] += amount;
        luxyToken.safeTransferFrom(_msgSender(), address(this), amount);
    ...
    function withdraw(uint256 amount)
        ...
        updateReward(_msgSender())
    {
        require(amount > 0, "LuxyStake::CANNOT_WITHDRAW_ZERO");
        require(
            _balances[_msgSender()] >= amount,
            "LuxyStake::INSUFFICIENT_TOKEN_AMOUNT"
        );
        _totalSupply -= amount;
        _balances[_msgSender()] -= amount;
        luxyToken.safeTransfer(_msgSender(), amount);
    ...
    function getReward() public nonReentrant updateReward(_msgSender()) {
        ...
        if (reward > 0) {
            ...
            luxyToken.safeTransfer(_msgSender(), reward);
        ...
        updateReward(_msgSender())
    {
        ...
        require(reward > 0, "LuxyStake::ZERO_REWARD_AMOUNT");
        rewards[_msgSender()] = 0;
        _totalSupply += reward;
        _balances[_msgSender()] += reward;

        emit Composed(_msgSender(), reward);
    }
        ...
        updateReward(address(0))
    {
        ...
        */
        uint256 balance = luxyToken.balanceOf(address(this)) - _totalSupply;
        require(
            rewardRate <= balance / rewardsDuration,
            "LuxyStake::PROVIDED_REWARD_TOO_HIGH"
        );
    ...
    {
        require(
            tokenAddress != address(luxyToken),
            "LuxyStake::CANNOT_WITHDRAW_STAKING_TOKEN"
        );
