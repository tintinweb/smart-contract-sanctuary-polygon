...
contract VentureCapital is IVentureCapital, Pausable, ExternallyCallable {
    ...
    /** @notice This function is use for manager transfers with no tax */
    ...
    /** @notice when transferring a stake token the # of shares is decreased */
    function transferSharesAndRebalance(
        address from,
        address to,
        uint256 oldShares,
        ...
        subTotalSharesOfAndRebalanceInternal(from, oldShares);
        addTotalSharesOfAndRebalanceInternal(to, newShares);
    }
    ...
    // @param tokenAddress {address} - the divident token address
    // @param amountBought {uint} - the amount in ETH that was bid in the auction
    function updateTokenPricePerShareAxn(uint256 amountBought)
        external
        override
        onlyExternalCaller
    {
        tokenPricePerShare[axion] =
            tokenPricePerShare[axion] + //increase the token price per share with the amount bought divided by the total Vca registered shares
            (amountBought * (1e36)) /
            (contracts.stakeManager.getTotalShares() + 1e12);
    }
