...
import {CanvasManagerV2} from "./CanvasManagerV2.sol";

/**
 * @title ContributionViewer
 * @author Matrix
 */
contract ContributionViewer {
    // ==================== Variables ====================

    CanvasManagerV2 internal immutable _newCanvasManager;

    // ==================== Constructor function ====================

    constructor(address newCanvasManager) {
        _newCanvasManager = CanvasManagerV2(newCanvasManager);
    }

    // ==================== External functions ====================

    function getNewCanvasManager() external view returns (address) {
        return address(_newCanvasManager);
    }

    /**
     * @dev return all token ids of PixNft which contribution > 0
     */
    function getContributionsByRange(
        address contributor,
        uint256 beginArtId,
        uint256 endArtId
    ) external view returns (uint256[] memory tokenIds) {
        require(beginArtId > 0, "beginArtId == 0");
        require(endArtId >= beginArtId, "beginArtId > endArtId");

        uint256 resultCount = 0;
        uint256[] memory tempTokenIds = new uint256[](endArtId - beginArtId + 1);

        for (uint256 artId = beginArtId; artId <= endArtId; ++artId) {
            try _newCanvasManager.getShares(artId, contributor) returns (uint256 individualShares, uint256) {
                if (individualShares > 0) {
                    tempTokenIds[resultCount++] = artId;
                }
            } catch {}
        }
     ...
     * @dev return all token ids of PixNft which contribution > 0
     */
    function getContributionsByArray(address contributor, uint256[] calldata artIds) external view returns (uint256[] memory tokenIds) {
        uint256 queryCount = artIds.length;
        require(queryCount > 0, "empty array");

        uint256 resultCount = 0;
        uint256[] memory tempTokenIds = new uint256[](queryCount);

        for (uint256 i = 0; i < queryCount; ++i) {
            try _newCanvasManager.getShares(artIds[i], contributor) returns (uint256 individualShares, uint256) {
                if (individualShares > 0) {
                    tempTokenIds[resultCount++] = artIds[i];
                }
            } catch {}
        }
