...
contract superbot {
    ...
    uint256 _expectedProfit
        ...
        IERC20(_baseCurrency).approve(quickswapRouter,IERC20(_baseCurrency).balanceOf(address(this)));
    ...
    uint256 _expectedProfit
        ...
        path1[1] = _qouteCurrency;
        uint256 deadline = block.timestamp + 720000;
        IERC20(_baseCurrency).approve(sushiswapRouter,IERC20(_baseCurrency).balanceOf(address(this)));
        IsushiswapRouter(sushiswapRouter).swapExactTokensForTokens(
            IERC20(_baseCurrency).balanceOf(address(this)),
            _amountOutMin1,
            path1,
    ...
    uint256 _expectedProfit
        ...
        IuniswapRouter.ExactInputSingleParams memory params = IuniswapRouter
            .ExactInputSingleParams({
                ...
                deadline: deadline,
                amountIn: IERC20(_baseCurrency).balanceOf(address(this)),
                amountOutMinimum: _amountOutMin1,
                sqrtPriceLimitX96: 0
            });
        IERC20(_baseCurrency).approve(uniswapRouter,IERC20(_baseCurrency).balanceOf(address(this)));
