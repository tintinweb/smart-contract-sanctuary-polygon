...
contract MasterChefNFT is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    uint256 public theEndlessPID = 0;
    ...
    function _payPendingUSDReward() internal {
        ...
        uint256 usdPending = ((user.amount * accDepositUSDRewardPerShare) / 1e18) - user.usdRewardDebt;

        if (usdPending > 0) {
            // send rewards
            treasuryDAO.transferUSDToUser(msg.sender, usdPending);
        }
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (poolInfo[theEndlessPID].totalLocked > 0) {
            uint256 usdRelease = treasuryDAO.getUSDDrip();

            accDepositUSDRewardPerShare = accDepositUSDRewardPerShare + ((usdRelease * 1e18) / poolInfo[0].totalLocked);
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                // treasuryDAO.convertDepositFeesToUSD(address(pool.lpToken), pool.tokenType);
                
                user.amount = user.amount + _amount - totalDepositFee;
                pool.totalLocked = pool.totalLocked + _amount - totalDepositFee;
            } else {
...
contract TreasuryDAO is ReentrancyGuard, AccessControl {
    ...
    IERC20 public usdRewardCurrency;
    IUniswapV2Router02 public sandManSwapRouter;
    // The trading pair
    address public sandManSwapPair;
    ...
    uint256 public lastUSDDistroTime = type(uint256).max;
     ...
     */
    constructor(address _usdCurrency, IUniswapV2Router02 _sandManSwapRouter) {
        sandManSwapRouter = _sandManSwapRouter;
        usdRewardCurrency = IERC20(_usdCurrency);
    ...
    function autoLiquidity() external payable nonReentrant {
    ...
    function getUSDDripRate() external view returns (uint256) {
        return usdRewardCurrency.balanceOf(address(this)) / distributionTimeFrame;
    }
     ...
     * Can only be called by the current owner.
     */
    function getUSDDrip() external onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256) {
        uint256 usdBalance = usdRewardCurrency.balanceOf(address(this));
        if (pendingUSD > usdBalance)
            return 0;

        uint256 usdAvailable = usdBalance - pendingUSD;
     ...
     */
    function convertDepositFeesToUSD(address token, uint256 tokenType) public  onlyRole(DEFAULT_ADMIN_ROLE) {
        // shouldn't be trying to sell TheEndless
        if (token == address(usdRewardCurrency))
            return;

        if (tokenType == 1) {
            _removeLiquidity(token);
            convertDepositFeesToUSD(IUniswapV2Pair(token).token0(), 0);
            convertDepositFeesToUSD(IUniswapV2Pair(token).token1(), 0);
            return;
        }

        uint256 totalTokenBalance = IERC20(token).balanceOf(address(this));

        // uint256 usdValue = getTokenUSDValue(totalTokenBalance, token, tokenType, false, address(usdRewardCurrency));

        if (totalTokenBalance == 0)
            return;
        // if (usdValue < usdSwapThreshold)
        //     return;
        ...
        require(IERC20(token).approve(address(sandManSwapRouter), totalTokenBalance), 'approval failed');
    ...
    /**
     * @dev send usd to a user
     * Can only be called by the current operator.
     */
    function transferUSDToUser(address recipient, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
       uint256 usdBalance = usdRewardCurrency.balanceOf(address(this));
       ...
       pendingUSD -= amount;
        ...
        emit USDTransferredToUser(recipient, amount);
    }

    /**
     * @dev set the number of blocks we should use to calculate the USD drip rate.
     * Can only be called by the current operator.
     */
    function setUSDDistributionTimeFrame(uint256 _usdDistributionTimeFrame) external onlyRole(OPERATOR_ROLE) {
        require(_usdDistributionTimeFrame > 1600 && _usdDistributionTimeFrame < 70080000 /* 5 years */, "_usdDistributionTimeFrame out of range!");
     ...
     * Can only be called by the current operator.
     */
    function updateSandManSwapRouter(address _router) external onlyRole(OPERATOR_ROLE) {
