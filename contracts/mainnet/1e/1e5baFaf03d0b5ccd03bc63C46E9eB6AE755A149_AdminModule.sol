...
contract AdminModule is Internals {
    ...
    modifier onlyOwner {
        require(IProxy(address(this)).getAdmin() == msg.sender, "P1:M1 not-an-admin");
        _;
    }
    ...
    */
    function newMarket(address token_, string memory name_, string memory symbol_) external onlyOwner {
        require(_itokens[token_] == address(0), "P1:M1: itoken-already-exist");
    ...
    /**
    * @dev Enables the market. Market needs to already exist in order to enable.
    * @param token_ address of token.
    */
    function enableMarket(address token_) external onlyOwner {
        require(_itokens[token_] != address(0), "P1:M1: market-does-not-exist");
        _tokenEnabled[token_] = true;
    ...
    */
    function setupRewards(address token_, address rewardtoken_, uint128 rewardAmount_, uint64 startTime_, uint64 duration_) external onlyOwner {
        ...
        require(startTime_ >= block.timestamp, "P1:M1: start-time-should-be-more-than-current-time");
...
contract IToken is ERC20 {
    ...
    modifier onlyLendingPool {
        require(msg.sender == _lendingPool, "P1:M1: not-lending-pool");
        _;
    }
