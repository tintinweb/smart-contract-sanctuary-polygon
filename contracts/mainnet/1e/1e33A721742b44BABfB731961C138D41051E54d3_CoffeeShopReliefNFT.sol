...
contract Named is INameable, ERC165 {
    ...
    function setName(uint256 id, string memory name) public onlyNameManager {
        names[id] = name;
    }

    function setNames(uint256[] memory ids, string[] memory _names)
        public
        onlyNameManager
    {
...
// File: CoffeeShopReliefNFT.sol

/**
 * @title Coffee Shop Relief NFT
 * 
 * The mission of the Coffee Shop Support DAO (decentralized autonomous
 * organization) is to directly support local coffee shops and provide
 * financial relief to those determined by the NFT holding community.

 * Many coffee shops are still struggling post-pandemic with spiking energy
 * costs, staffing shortages, and compounding debt, so a DAO was created
 * (https://coffeeshopdao.com) to raise funds and distribute them according
 * to community votes.

 * 80% of net proceeds go into Coffee Shop Support Fund (CSSF) controlled
 * by the DAO. When the CSSF reaches $100,000, the community (NFT owners)
 * will vote on which two shops will each receive $50,000. Coffee shops
 * will also receive $2,500 out of the fund for every 100 nominations.

 * Thank you for your support!
 */
contract CoffeeShopReliefNFT is ReliefDAONFT {
    constructor() ViciERC721v4("Coffee Shop Relief DAO", "CSR", 100000) {
        setContractURI(
            "https://storage.googleapis.com/arrival-shadow-motion-newman/vici/coffee-shop/CoffeeShop.json"
        );
    }
}