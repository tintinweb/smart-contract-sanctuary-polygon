...
contract Erc20_Token is ERC20, AccessControl, ERC20Burnable, Pausable {
  using SafeMath for uint256;
  uint256 public _maxAmountMintable = 50_000_000e18;

  constructor(
    string memory _name,
    string memory _symbol,
    uint8 _decimals,
    uint256 maxAmountMintable,
    address creator
  ) ERC20(_name, _symbol) {
    _setupRole(DEFAULT_ADMIN_ROLE, creator);
    _maxAmountMintable = maxAmountMintable * (10 ** _decimals);
  ...
  function burn(address _to, uint256 _amount) public onlyAdminRole whenNotPaused {
    require(balanceOf(_to) >= _amount, "Not enough balance");
  ...
  function unpasue() external onlyAdminRole {
    super._unpause();
  }
}
