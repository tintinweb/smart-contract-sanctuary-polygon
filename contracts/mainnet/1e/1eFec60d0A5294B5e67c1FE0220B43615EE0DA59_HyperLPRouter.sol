...
contract HyperLPRouter is IUniswapV3SwapCallback, GasStationRecipient, Ownable {
    ...
    function uniswapV3SwapCallback(
        ...
        if (amount0Delta > 0) {
            // address transfer_from = IUniswapV3Pool(pool).token0() == address(wETH) ? address(this) : payer;
            // IERC20(IUniswapV3Pool(pool).token0()).safeTransferFrom(transfer_from, pool, uint256(amount0Delta));
            IERC20(IUniswapV3Pool(pool).token0()).safeTransferFrom(payer, pool, uint256(amount0Delta));
        } 
        else {
            // address transfer_from = IUniswapV3Pool(pool).token1() == address(wETH) ? address(this) : payer;
            // IERC20(IUniswapV3Pool(pool).token1()).safeTransferFrom(transfer_from, pool, uint256(amount1Delta));
