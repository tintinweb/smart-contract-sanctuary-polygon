...
contract Stop {
    ...
    event OrderCalled(address, address, address, uint);
    ...
    * @param _address address that should be the new owner
    ...
    function transferOwnership(address payable _address) external onlyOwner returns(bool){
        owner = _address;
    }
    ...
    * @param _feeTo address to split the fees to
    ...
    function setBuyOrder(address _router, address _token0, address _token1, uint _amount, uint _trigger, address _feeTo) external payable {
        ...
        emit BuyOrderSet(_router, _token0, _token1, chargedAmount, _trigger, msg.sender);
    }     
    ...
    * @param _amount of the order  
    ...
    function triggerBuyOrder(address _router, address _token, address _trader, uint _amount) external payable onlyOwner {
        IERC20 token = IERC20(_token);
        address WETH =  IUniswapV2Router02(_router).WETH();    
        address[] memory path = new address[](2);
        path[0] = WETH;
        ...
        uint chargedAmount = _amount - fees;
        token.approve(_router, chargedAmount);
        IUniswapV2Router02(_router).swapExactTokensForTokensSupportingFeeOnTransferTokens(chargedAmount, 0, path, _trader, ttl);
        emit OrderCalled(_router, _token, _trader, _amount);
    }
