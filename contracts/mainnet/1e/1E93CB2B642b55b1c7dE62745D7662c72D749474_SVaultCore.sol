...
contract SVaultCore is SVaultStorage  {
    ...
    /**
     * @dev Deposit a supported asset to the Vault and mint Share.
     * @param _amount Amount of the asset being deposited (decimals based on _asset)
     */
    function _deposit(
        uint256 _amount
        ...
        // Collect All rewards from startegies
        _claimRewards();
        // Get the total asset in the Vault and strategies
     ...
     **/
    function _quickDeposit() internal {
        ...
        uint256 allocateAmount = available();
        if (quickDepositStrategyAddr != address(0)   && allocateAmount > 0 ) {
            ISingleStrategy strategy = ISingleStrategy(quickDepositStrategyAddr);
            _asset().safeTransfer(address(strategy), allocateAmount);
            strategy.deposit(asset, allocateAmount); // TODO: Possibly new startegy interface for not asking asset
            emit QuickDeposited(
                asset,
    ...
    function withdraw(uint256 _shares) public {
        uint256 _give = (balance() * _shares) / totalSupply();
        _burn(msg.sender, _shares);
        uint _before = available();

        if (_before < _give) {
            uint _withdrawable = _give - _before;
            _strategyWithdraw(_withdrawable);
            uint _after = available();
            uint _diff = _after - _before;
            if (_diff < _withdrawable) {
                _give = _before + _diff;
            }
        }
        _asset().safeTransfer(msg.sender, _give);
        emit Withdraw(msg.sender, _give, _shares);
    }
    ...
    function _strategyWithdraw(uint256 _withdrawable) internal {
        ...
        while((totalAmount < _withdrawable) && (strategyIndex < strategyWithWeights.length)) {
            uint256 currentStratBal = ISingleStrategy(strategyWithWeights[strategyIndex].strategy).checkBalance();
        ...
        for (uint8 i = 0; i < strategyWithWeights.length; i++) {
            ...
            if (amountsToWithdraw[i] > 0) {
                ISingleStrategy(strategiesToWithdrawFrom[i]).withdraw(address(this), asset, amountsToWithdraw[i]);
            } else {
                ISingleStrategy(strategiesToWithdrawFrom[i]).withdrawAll();
            }
            
        }
        require(_asset().balanceOf(address(this)) >= _withdrawable, "Not enough funds after withdrawl.");
    }


    /**
    * Function to claim rewards from all the strategies linked
    */
    function _claimRewards() internal {
        for (uint256 i = 0; i < allStrategies.length; i++) {
            ISingleStrategy(allStrategies[i]).collectRewardTokens();
        }
    ...
    /**
     * @notice Get the balance of an asset held in Vault and all strategies.
     * @return fbalance Balance of asset in decimals of asset
     */
    function _balance()
        internal
        view
        ...
        returns (uint256 fbalance)
    {
        ...
        for (uint256 i = 0; i < allStrategies.length; i++) {
            ...
            fbalance = fbalance.add(strategy.checkBalance());
        }
    }
    /**
     * @notice Check balance equivalent in USD
     * @return uint256, uint256 - Balance equivalent in USD. E8
     **/
    function tvl() public view returns (uint256) {
        require(oracleRouter != address(0), "Router not set");
        IOracle _oracle = IOracle(oracleRouter);
        uint256 _b0 = _balance();
        _b0 = _b0.scaleBy(18, Helpers.getDecimals(asset)).mul(_oracle.price(asset)).div(10**18);
        return _b0;
    }
