...
abstract contract MiniCurveExchange {
    ...
    function swap(
        ...
        console.log("Slippage:", _convert(IOracle(_oracleRouter), _fromToken, _toToken, _amount).subBasisPoints(100));
        return IStableSwapPool(_curvePool).exchange_underlying(
            _getTokenIndex(_fromToken),
            _getTokenIndex(_toToken),
            _amount,
            _convert(IOracle(_oracleRouter), _fromToken, _toToken, _amount).subBasisPoints(100) // 1% slippage
        );
    }
