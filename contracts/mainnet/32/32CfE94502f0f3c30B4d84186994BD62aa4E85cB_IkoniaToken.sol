...
contract IkoniaToken is ERC1155, AccessControl, Adminable, ContractMetadata, IMintable, Pausable {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant PAUSE_MANAGER_ROLE = keccak256("PAUSE_MANAGER_ROLE");
    bool public immutable fungible;

    BitMaps.BitMap mintedTokens;

    constructor(address[] memory admins, bool _fungible)
        Adminable(admins)
        ERC1155("")
    {
        fungible = _fungible;
        ...
        whenNotPaused
    {
        if (!fungible) {
            for (uint index = 0; index < tokenIDs.length; index++) {
                uint256 quantity = quantities[index];
                require(quantity == 1, "Invalid quantity");

                uint256 tokenID = tokenIDs[index];
                require(!BitMaps.get(mintedTokens, tokenID), "Token has already been minted");
                BitMaps.set(mintedTokens, tokenID);
            }
        }

        _mintBatch(receiver, tokenIDs, quantities, new bytes(0));
    }

    /**
     * @dev Overriding to auto-approve OpenSea's proxy contract.
     */
    ...
    {
        // Always approve if OpenSea's ERC-1155 proxy address is detected.
...
abstract contract Adminable is AccessControl {
    constructor(address[] memory admins) {
        ...
        for (uint index = 0; index < admins.length; index++) {
            _grantRole(DEFAULT_ADMIN_ROLE, admins[index]);
        }
    }
}
...
interface IMintable {
    function mintBatch(address receiver, uint256[] memory tokenIDs, uint256[] memory quantities) external;
}
