...
abstract contract DePOCountryDictionary is Ownable {
    ...
    function getCountries(uint256 page, uint256 pageSize) public view returns (Country[] memory) {
        uint256 from = pageSize * page - 1;
        uint256 to = pageSize * page;

        Country[] memory countries = new Country[](pageSize);

        uint256 index = 0;
        for (from; from < to; from++) {
            Country storage country = _countries[from];
...
abstract contract DePOCurrencyDictionary is DePOCountryDictionary {
    ...
    function getCountryCurrencies(uint256 countryId, uint256 page, uint256 pageSize) public view returns (Currency[] memory) {
        uint256 from = pageSize * page - 1;
        uint256 to = pageSize * page;

        Currency[] memory currencies = new Currency[](pageSize);

        uint256 index = 0;
        for (from; from < to; from++) {
            Currency storage currency = _currencies[countryId][from];
...
abstract contract DePOPaymentMethodsDictionary is DePOCountryDictionary {
    ...
    function getCountryPaymentMethods(uint256 countryId, uint256 page, uint256 pageSize) public view returns (PaymentMethod[] memory) {
        uint256 from = pageSize * page - 1;
        uint256 to = pageSize * page;

        PaymentMethod[] memory paymentMethods = new PaymentMethod[](pageSize);

        uint256 index = 0;
        for (from; from < to; from++) {
            PaymentMethod storage paymentMethod = _paymentMethods[countryId][from];
...
abstract contract DePOTokenDictionary is Ownable {
    ...
    function getTokens(uint256 page, uint256 pageSize) public view returns (Token[] memory) {
        uint256 from = pageSize * page - 1;
        uint256 to = pageSize * page;

        Token[] memory tokens = new Token[](pageSize);

        uint256 index = 0;
        for (from; from < to; from++) {
            Token storage token = _tokens[from];
            tokens[index] = token;
            index++;
        }
