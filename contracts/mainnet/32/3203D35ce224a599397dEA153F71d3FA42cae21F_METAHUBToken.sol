  ...
  function _calculateTax(address _from, address _to, uint _value) private returns (uint) {
    ...
    if (isBuyingToken) {
      ...
      if(lsd.isQualifiedForTaxDiscount(_to) && lsdDiscountTaxPercentages[TaxType.Buy] > 0) {
        return baseTax * (100 * DECIMAL3 - lsdDiscountTaxPercentages[TaxType.Buy]) / DECIMAL3 / 100;
      }
    }
    if (_from == address(vault)) {
      ...
      if(lsd.isQualifiedForTaxDiscount(_to) && lsdDiscountTaxPercentages[TaxType.Claim] > 0) {
        return baseTax * (100 * DECIMAL3 - lsdDiscountTaxPercentages[TaxType.Claim]) / DECIMAL3 / 100;
      }
    }
    ...
    if (isSellingToken) {
      ...
      if(lsd.isQualifiedForTaxDiscount(_from) && lsdDiscountTaxPercentages[TaxType.Sell] > 0) {
        baseTax = baseTax * (100 * DECIMAL3 - lsdDiscountTaxPercentages[TaxType.Sell]) / DECIMAL3 / 100;
      }
    ...
    if (_from != address(vault) && lsd.isQualifiedForTaxDiscount(_from) && lsdDiscountTaxPercentages[TaxType.Transfer] > 0) {
      baseTax = baseTax * (100 * DECIMAL3 - lsdDiscountTaxPercentages[TaxType.Transfer]) / DECIMAL3 / 100;
    }
    return baseTax;
  }
