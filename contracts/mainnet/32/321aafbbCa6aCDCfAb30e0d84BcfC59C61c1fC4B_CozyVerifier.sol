...
import "./CozyRegistry.sol";

contract CozyVerifier is ChainlinkClient, AccessControl {
  using Chainlink for Chainlink.Request;
  using Strings for uint;

  bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");

  error PaymentIncorrect(uint payment);
  error VerificationConfigNotFound(address partnerAddress);
  error InvalidVerificationConfig();
  error RequestPending(bytes32 requestId);
  error RequestNotFound(bytes32 requestId);
  error InvalidOracle(address sender, address oracleAddress);
  error LinkTransferFailed(address sender);
  ...
  // Mapping from partner address to verification config
  mapping(address => VerificationConfig) _configs;

  // Mapping from request ID to request information
  mapping(bytes32 => Request) _requests;

  // Mapping from partner token contents to request ID
  mapping(address => mapping(uint => bytes32)) _requestIdForPartnerToken;

  uint public verificationFee;
  address public linkAddress;
  CozyRegistry public registry;

  event VerificationRequested(
    bytes32 indexed requestId,
    address caller,
    address indexed partnerAddress,
    uint indexed partnerTokenId
  );
  ...
  constructor(
    uint _verificationFee,
    address _linkAddress,
    address _cozyRegistryAddress
  ) {
    verificationFee = _verificationFee;
    linkAddress = _linkAddress;
    registry = CozyRegistry(_cozyRegistryAddress);
  ...
  function setVerificationFee(uint _verificationFee) external onlyRole(DEFAULT_ADMIN_ROLE) {
    verificationFee = _verificationFee;
  }

  function setLinkAddress(address _linkAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
    linkAddress = _linkAddress;
    setChainlinkToken(linkAddress);
  }

  function setCozyRegistry(address _cozyRegistryAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
    registry = CozyRegistry(_cozyRegistryAddress);
  }
  ...
  /**
   * Returns the request for the provided request ID.
   *
   * The request will contain an zero partner address if it does not exist.
   */
  ...
  /**
   * Returns the request ID for the provided partner token information.
   *
   * The request ID will be a zero value if it does not exist.
   */
  function getRequestIdForPartnerToken(address partnerAddress, uint partnerTokenId)
    external
    view
    returns (bytes32)
  {
    return _requestIdForPartnerToken[partnerAddress][partnerTokenId];
  }

  /**
   * Returns the verification config for the provider partner address.
   *
   * The Verification Config will have a zero Oracle address if it does not exist.
   */
  function getVerificationConfig(address partnerAddress)
    external
    view
    returns (VerificationConfig memory)
  {
    return _configs[partnerAddress];
  }

  // --------- Verification --------

  function verify(address partnerAddress, uint partnerTokenId) public payable virtual {
    if (msg.value != verificationFee) {
      revert PaymentIncorrect(msg.value);
    }

    VerificationConfig storage config = _configs[partnerAddress];
    if (config.oracleAddress == address(0)) {
      revert VerificationConfigNotFound(partnerAddress);
    }

    bytes32 requestId = _requestIdForPartnerToken[partnerAddress][partnerTokenId];
    delete _requests[requestId];
    delete _requestIdForPartnerToken[partnerAddress][partnerTokenId];
    ...
    string memory tokenId = partnerTokenId.toString();
    string memory contractAddress = Strings.toHexString(uint(uint160(partnerAddress)), 20);
    string memory requesterAddress = Strings.toHexString(uint(uint160(msg.sender)), 20);

    chainlinkRequest.add("tokenId", tokenId);
    chainlinkRequest.add("contractAddress", contractAddress);
    chainlinkRequest.add("requesterAddress", requesterAddress);

    requestId = sendChainlinkRequestTo(config.oracleAddress, chainlinkRequest, config.jobFee);
    ...
    _requestIdForPartnerToken[partnerAddress][partnerTokenId] = requestId;

    emit VerificationRequested(requestId, msg.sender, partnerAddress, partnerTokenId);
  }

  function safeVerify(address partnerAddress, uint partnerTokenId) public payable {
    bytes32 requestId = _requestIdForPartnerToken[partnerAddress][partnerTokenId];
    ...
    verify(partnerAddress, partnerTokenId);
  }

  function fulfill(bytes32 requestId, bytes32 owner) public virtual {
    Request storage request = _requests[requestId];
    if (request.partnerAddress == address(0)) {
      revert RequestNotFound(requestId);
    }

    VerificationConfig storage config = _configs[request.partnerAddress];
    if (config.oracleAddress != msg.sender) {
      revert InvalidOracle(msg.sender, config.oracleAddress);
    }

    request.status = RequestStatus.FULFILLED;

    address ownerAddress = address(uint160(uint(owner)));
    // Never registers an NFT to the zero address
    if (ownerAddress != address(0)) {
      registry.register(ownerAddress, request.partnerAddress, request.partnerTokenId);
    }

    emit VerificationFulfilled(
      requestId,
      ownerAddress,
      request.partnerAddress,
      request.partnerTokenId
    );
  }

  // -------- Administration -------

  function addVerificationConfig(address partnerAddress, VerificationConfig memory config)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    if (config.oracleAddress == address(0)) {
      revert InvalidVerificationConfig();
    }
    ...
    _configs[partnerAddress] = config;
  }

  function removeVerificationConfig(address partnerAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
    delete _configs[partnerAddress];
  }
  ...
  function clearRequest(bytes32 requestId) external onlyRole(OPERATOR_ROLE) {
    delete _requests[requestId];
  }
}
