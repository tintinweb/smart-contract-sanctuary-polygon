...
contract BitcoinGrowsOnTrees is ERC721 {
    ...
    constructor()
        ERC721("Bitcoin Grows on Trees", "BGOT")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 47) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeic3g5ojpdgzgoso7bqcnn6eccda2aumb7mdyolxiqnjzzyikzpazm/", Strings.toString(tokenId), ".json")
            );
    }
