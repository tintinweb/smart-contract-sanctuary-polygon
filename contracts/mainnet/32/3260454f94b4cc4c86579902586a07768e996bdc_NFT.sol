...
contract NFT is ERC721Enumerable, Ownable { // inherit from erc721enumerable and ownable, meaning our contract would have the access into their built in methods
  ...
  uint256 public maxSupply = 4000; // max supply to be mintable - modifiable by only owner
  ...
  bool public pausedMint = false; // ability to stop the minting sale - modifiable by only owner
  ...
  bool public onlyWhitelisted = false; // determines whether minting is available for whitelists only, if false then it's for public - modifiable by only owner
  ...
  constructor(
    ...
    setBaseURI("ipfs://QmZNoidNTW4h2Szu8J9Nfe33jNocrhQUoNfh2bXcTcS7CF/"); // uri for the characters | DO NOT HARDCODE ON REAL DEPLOYMENT 
  ...
  // og
  function ogMint(uint256 _mintAmount) public payable { // public - outside of the contract have access to the function | payable - allows a function to process transaction
    require(!pausedMint, "the contract is paused"); // if not paused then proceed to next line else reject the transaction with an error message 
    ...
    if (msg.sender != owner()) { // if the caller of the mint function is not the owner then do everything within the scope, else if the owner then proceed to minting without paying any cost 
      ...
      require(ownerMintedCount + _mintAmount <= ogNftPerAddressLimit, "max mint per address during og sale exceeded");
      require(balanceOf(msg.sender) + _mintAmount <= ogNftPerAddressLimit, "max NFT per address during og sale exceeded");
  ...
  // whitelist 
  function whitelistMint(uint256 _mintAmount) public payable { // public - outside of the contract have access to the function | payable - allows a function to process transaction
    require(!pausedMint, "the contract is paused"); // if not paused then proceed to next line else reject the transaction with an error message 
    uint256 supply = totalSupply(); // totalSupply() returns the current supply then assigns its value into supply variable 
    require(_mintAmount > 0, "need to mint at least 1 NFT"); // mint amount must be greater than 0 else reject the transaction with an error message
    require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded"); // mint amount for this transaction should not exceed the max mint amount (default: 20) else reject the transaction with an error message
  ...
  function isOg(address _user) public view returns (bool) { // // public - outside of the contract have access to the function | view - cannot do any modifications inside the function
    for (uint i = 0; i < ogAddresses.length; i++) { // loop thru all of the values inside whitelistedAddresses 
      if (ogAddresses[i] == _user) { // if the _user matches one of them meaning they are included in the whitelisted list then return true 
          return true;
      }
    }
