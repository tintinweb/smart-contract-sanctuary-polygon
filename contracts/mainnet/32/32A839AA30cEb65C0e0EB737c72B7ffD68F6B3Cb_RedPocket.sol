...
interface IRedPocket {
    ...
    function createRedPocket(bytes32 id, uint256 splitNumber, string memory message) external payable;

    function createWhitelistedRedPocket(bytes32 id,uint256 splitNumber, address[] calldata whitelist, string memory message) external payable;
...
contract RedPocket is IRedPocket {
    ...
    uint256 randomnessUpperBound = 1500;
    uint256 randomnessLowerBound = 500;
    uint256 baseDenominator = 1000;

    function createRedPocket(bytes32 id, uint256 splitNumber, string memory message) public payable {
        createRedPocketInternal(id, splitNumber, false, new address[](0), message);
    }

    function createWhitelistedRedPocket(bytes32 id, uint256 splitNumber, address[] calldata whitelist, string memory message) public payable {
        createRedPocketInternal(id, splitNumber, true, whitelist, message);
    }
    ...
    function claim(bytes32 id) public {
        ...
        if (detail.claimants.length == detail.split - 1) {
            ...
            uint256 fairShare = detail.remainingBalance.div(detail.split - detail.claimants.length );
            uint256 multiplier = generatePseudoRandomNumber(randomnessUpperBound - randomnessLowerBound) + randomnessLowerBound;
            valueClaimed = fairShare.mul(multiplier).div(baseDenominator);
        }
    ...
    function generateID(uint256 value, uint256 splitNumber) public view returns (bytes32) {
        return keccak256(abi.encodePacked(msg.sender, value, splitNumber, block.timestamp));
    }

    // Internals

    function createRedPocketInternal(bytes32 id, uint256 splitNumber, bool isWhitelisted, address[] memory whitelist, string memory message) internal {
        ...
        require(redPockets[id].creator == ZERO_ADDRESS, "ID already used");
    ...
    function generatePseudoRandomNumber(uint256 modulo) internal view returns(uint256) {
        return uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender))) % modulo;
    }
