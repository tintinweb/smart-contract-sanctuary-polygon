...
interface IKyberPool {
    ...
    function intializedTicks(int24 tick) external view returns (int24 previous, int24 next);
}
...
contract StrategyKyberCLA is StratFeeManagerInitializable, IERC721Receiver {
    ...
    function _init(uint256 _amount0, uint256 _amount1) private returns (uint256 actual0, uint256 actual1) {
        ...
        uint128 liquidity;
        (ticksPrevious[0],) = pool.intializedTicks(ticks.lowerTick);
        (ticksPrevious[1],) = pool.intializedTicks(ticks.upperTick);
