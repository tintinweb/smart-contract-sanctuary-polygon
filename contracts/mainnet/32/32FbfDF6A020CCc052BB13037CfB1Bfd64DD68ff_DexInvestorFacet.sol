...
contract DexInvestorFacet is Modifiers, ReentrancyGuard {
    ...
    */
    ...
    function _issueFromDexCalldata(
        ...
        if (LibReferralToken.isReferralToken(_addr) && LibReferralToken.getFeeValue(_addr) > 0 && msgSender!=IPortToken(_addr).manager()) {
            uint256 _feeAmount;
...
contract FeeFacet is Modifiers, ITransferHookReceiver {
    ...
    function getTokensWithPendingFees(
        uint256 mintInterval,
        uint256 minSupply,
        uint256 from,
        uint256 to
    ) external view returns (address[] memory pendingTokens) {
        address[] storage tokens = LibPortTokenFactory.diamondStorage().portTokens;
        uint256 l = tokens.length;
        require(from < l, "out of range");

        if (to >= l || to == 0) {
            to = l - 1;
        }
        require(from <= to, "invalid arguments");

        LibFee.FeeData storage d;
        pendingTokens = new address[](to - from + 1);

        for (uint256 i = from; i <= to; i++) {
            address portTokenAddr = tokens[i];
            if (IERC20(portTokenAddr).totalSupply() <= minSupply) {
                continue;
            }

            d = LibFee.feeDataForToken(portTokenAddr);
            
            if (d.streamingFee > 0 && LibFee.feeDataForToken(portTokenAddr).lastMintedAt < block.timestamp - mintInterval) {
                pendingTokens[from - i] = portTokenAddr;
                continue;
            }
            ...
            (uint256 pendingSuccessFee, ) = LibFee.getPendingSuccessFee(portTokenAddr);
            if (pendingSuccessFee>0) {
                pendingTokens[from - i] = portTokenAddr;    
            }
        }
    }

    function batchAccumulateAndMintFees(address[] memory addrs) external whenNotPaused {
        for (uint256 i = 0; i < addrs.length; i++) {
            LibFee.accumulateAndMintFees(addrs[i]);
        }
    }
}
