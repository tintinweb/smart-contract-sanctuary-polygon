...
contract Ticket is ERC721URIStorage, Governable, ReentrancyGuard {
    ...
    constructor(address _governance, uint256 _maxTicketCount, string memory _eName) Governable(_governance) ERC721("Is Sanat Ticket", "ISSANAT") {
        require(_maxTicketCount > 0, "invalid ticket count");
        require(bytes(_eName).length > 0, "invalid event name");
     ...
     * @param _user The user to mint ticket.
     * @param _tokenURI The token uri.
     * @return ticketId The ticket id.
    */
    function createTicket(address _user, string memory _tokenURI) external nonReentrant onlyGovernance returns (uint256 ticketId) {
        require(soldTicketCount < maxTicketCount, "tickets sold out");
        require(_tickets[_user] == 0, "user has ticket");
        require(_user != address(0x0), "zero address user");

        soldTicketCount++;
        _ticketId++;
        _safeMint(_user, _ticketId);
        _setTokenURI(_ticketId, _tokenURI);
        _tickets[_user] = _ticketId;
        emit TicketSold(_user, _ticketId);
        return _ticketId;
    }
    ...
    function getTicketId(address user) external view returns (uint256 ticketId) {
        return _tickets[user];
    }    

    function setEventName(string memory name) external onlyGovernance {
         _eventName = name;
     }
    ...
    function burnTicket(address[] memory users) external onlyGovernance {
        for (uint256 i = 0; i < users.length; i++) {
            if (_tickets[users[i]] > 0) {
                _burn(_tickets[users[i]]);
                delete _tickets[users[i]];
            }
        }
    }
}
