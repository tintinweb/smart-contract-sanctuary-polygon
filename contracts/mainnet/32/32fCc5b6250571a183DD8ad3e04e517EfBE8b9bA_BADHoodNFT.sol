...
contract BADHoodNFT is RandomGenerator, Ownable, ERC721Enumerable, ERC721URIStorage {
    ...
    function batchTokenURI(address account_) public view returns (uint[] memory, string[] memory) {
        uint amount = balanceOf(account_);
        uint tokenId;
        uint[] memory idInfo = new uint[](amount);
        string[] memory uriInfo = new string[](amount);
        for (uint i = 0; i < amount; i++) {
            tokenId = tokenOfOwnerByIndex(account_, i);
            idInfo[i] = tokenId;
            uriInfo[i] = tokenURI(tokenId);
        }
        return (idInfo, uriInfo);
    }
}
