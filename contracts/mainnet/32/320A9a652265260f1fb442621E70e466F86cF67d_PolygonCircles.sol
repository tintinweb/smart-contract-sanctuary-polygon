...
contract PolygonCircles is DynamicNftUriGetter {
    ...
    function _shape(bytes32 seed) private pure returns(string memory) {
        ...
        return string(
            abi.encodePacked(
                "<circle fill='hsla(",(uint256(seed) % 360).toString(),",60%,55%,.95)' r='512' cx='",(uint128(xy[0]) % 2048).toString(),"' cy='",(uint128(xy[1]) % 2048).toString(),"'/>"
            )
        );
    }
        ...
        return string(
                abi.encodePacked(
                    ...
                    Base64.encode(string(abi.encodePacked(
                        '{"name":"Polygon Circles",', 
                        '"description":"A dynamic NFT, changing dynamically based on the 16 most recent blocks on the chain.\\n',
                            'Every circle is representing a block, positioned and colored based on the block hash.\\n\\n',
