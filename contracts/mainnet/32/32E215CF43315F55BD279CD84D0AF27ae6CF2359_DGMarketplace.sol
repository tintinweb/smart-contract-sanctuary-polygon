    ...
    OrderManagement
{
    ...
    function sell(
        ...
        require(
            IERC721(_nftAddress).isApprovedForAll(sender, address(this)),
            "Token not approved"
        );

        require(
            IERC165(_nftAddress).supportsInterface(0x80ac58cd),
            "DGMarketplace#sell: NOT_ERC721"
        );

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(
                IERC721(_nftAddress).ownerOf(_tokenIds[i]) == sender,
                "Not the owner of NFTs"
            );
    ...
    function processPurchase(
        ...
        if (IERC165(_nftAddress).supportsInterface(0x2a55205a)) {
            (royaltyBeneficiary, royaltyFee) = IERC2981(_nftAddress)
                .royaltyInfo(_tokenId, order.price);
        } else {
            try IDCL721(_nftAddress).creator() {
                royaltyBeneficiary = IDCL721(_nftAddress).creator();
    ...
    function registerPaperKey(address _paperKey) external onlyAdmin {
        require(paperKeyManager.register(_paperKey), "Error registering key");
    }

    function setFee(uint32 _newFee) external onlyAdmin {
        require(
            _newFee < BASE_FEE,
    ...
    function setFeeOwner(address _newFeeOwner) external onlyAdmin {
        require(
    ...
    function setFeeTips(uint256 _newFee) external onlyAdmin {
        require(
            _newFee < 1000000,
    ...
    function withdrawERC20(address _token, uint256 _amount) external onlyAdmin {
...
contract DGMarketplaceConfig is Initializable {
    ...
    modifier onlyAdmin() {
        require(getMsgSender() == adminWallet, "Not admin");
        _;
    }

    function getMsgSender() internal view returns (address sender) {
