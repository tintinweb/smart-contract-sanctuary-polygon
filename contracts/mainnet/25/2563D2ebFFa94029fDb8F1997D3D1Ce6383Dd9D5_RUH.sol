...
contract RGCHS is ERC721URIStorage, Ownable, ReentrancyGuard, IERC2981, ContextMixin, RoyaltiesV2, RGCDelegate, ERC721OperatorFilterV2
{
    using Strings for *;
    ...
    string public collectionNameUriEncoded;
    ...
    address private royaltyReceiver = 0x22fC8cde25e2e576e87311Ff712288342BCc0F13;
    uint256 private royaltySize = 1000;
    mapping (uint256 => LibPart.Part[]) public royalties;

    address public signingAddress = 0xC705e0fDcf62e9091B8075BF662093c96336a87A;

    address public _delegate = 0xC705e0fDcf62e9091B8075BF662093c96336a87A;

    event _setRoyaltySize(uint256 _royaltySize);
    event _setRoyaltyReceiver(address _royaltyReceiver);
    event _setSigningAddress(address _address);
    event _setDelegate(address delegate);
    event _unioTransfer(address from,address to,uint256 tokenId);
    event _setBaseURI(string URI);
    ...
    {
        return string(abi.encodePacked(baseURI, collectionNameUriEncoded, "/", tokenId.toString()));
    }
    ...
    ) external nonReentrant()
    {
        ...
        emit _unioTransfer(from,to,tokenId);
    }

    function unioBulkMintV1(address[] calldata _address) external delegateOnly
    {
        uint256 currentID = _tokenIdCounter;
        _tokenIdCounter += _address.length;
    ...
    function unioBulkMintV2(address[] calldata _address, uint256[] memory _ids) external delegateOnly
    {
        _tokenIdCounter += _address.length;
        for (uint256 i = 0; i < _address.length; i++)
        {
            _safeMint(_address[i], _ids[i]);
        }
    }
...
import "./openzeppelin-contracts/contracts/utils/Counters.sol";
import "./721_HeroesShared_MKIII.sol";

contract RUH is RGCHS
{
    constructor() RGCHS("UNIOVERSE UNCOMMON HEROES", "UNIOH4")
    {
        collectionNameUriEncoded = "UNIOVERSE%20UNCOMMON%20HEROES";
    }
}
...
abstract contract ERC721OperatorFilterV2 is ERC721URIStorage, Ownable, RGCDelegate
{
    ...
    */
    ...
    {
        require(msg.sender == _delegate0, 'Only Delegated wallets are allowed.');
        _;
    }
...
{
    address public _delegate0 = 0xB825642D6E731cB0ed2cE2b27760A87Cf1a04516;
}