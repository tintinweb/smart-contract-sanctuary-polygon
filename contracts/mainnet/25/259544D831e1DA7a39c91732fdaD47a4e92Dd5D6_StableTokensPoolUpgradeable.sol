    ...
    UUPSUpgradeable
{
    ...
    uint256 public tokenAMinSwapAmount;
    uint256 public tokenAMaxSwapAmount;

    struct SwapVariable {
        IERC20Upgradeable erc20TokenIn;
        IERC20Upgradeable erc20TokenOut;
        uint256 amountIn;
        uint256 amountOut;
        uint256 totalFee;
    ...
    event ExchangeRateWithFee(uint256 exchangeRate, uint256 fee);
    ...
    modifier onlyTrustedForwarder() {
        require(
            ...
            "Caller is not trustedForwarder"
        );
        _;
    }
    ...
    modifier onlyEmergencyRole() {
        require(
            emergencyOperator == _msgSender(),
            "Caller does not have emergency role"
        );
        _;
    }

    modifier onlyExchangeOracle() {
        require(
            msg.sender == exchangeRateOracle,
            "Caller is not exchangeRateOracle"
        );
        _;
    }
    ...
    function initialize(
        ...
        // TODO: this should be based on token
        tokenAMaxSwapAmount = 100000000000;
        txFee = 8;
        emergencyOperator = _multiSigEmergency;
    ...
    function getTokenADecimal() private pure returns (uint256) {
        // TODO: this should be dynamic based on token
        return 6;
    }

    function addLiquidity(address tokenIn, uint256 amountIn) external {
    ...
    function swapWithAuthorization(
    ...
    ) external whenNotPaused onlyTrustedForwarder {
        SwapVariable memory swapVariable;

        uint256 _txFee = txFee;
        IERC20Upgradeable _tokenA = tokenA;
        IERC20Upgradeable _tokenB = tokenB;
        uint256 _exchangeRate = exchangeRate;

        uint256 len = senders.length;
        uint256 tokenADecimal = getTokenADecimal();
        for (uint256 i = 0; i < len; ) {
            swapVariable.amountIn = amountIns[i];
            swapVariable.totalFee = (swapVariable.amountIn * _txFee) / 10000;
            swapVariable.amountInWithFee =
                swapVariable.amountIn -
                swapVariable.totalFee;

            if (IERC20Upgradeable(tokenIns[i]) == _tokenA) {
                swapVariable.amountOut =
                    (swapVariable.amountInWithFee * _exchangeRate) /
                    10 ** tokenADecimal;
                swapVariable.erc20TokenIn = _tokenA;
                swapVariable.erc20TokenOut = _tokenB;
            } else {
                swapVariable.amountOut =
                    (swapVariable.amountInWithFee * 10 ** tokenADecimal) /
                    _exchangeRate;
                swapVariable.erc20TokenIn = _tokenB;
                swapVariable.erc20TokenOut = _tokenA;
            }
            // swap
            ERC20Upgradeable erc20TokenIn = ERC20Upgradeable(
                address(swapVariable.erc20TokenIn)
            );
            ERC20Upgradeable erc20TokenOut = ERC20Upgradeable(
                address(swapVariable.erc20TokenOut)
            );

            if (
                erc20TokenOut.balanceOf(address(this)) >= swapVariable.amountOut
            ) {
                try
                    erc20TokenIn.transferFrom(
                        senders[i],
                        address(this),
                        swapVariable.amountIn
                    )
        ...
        emit ExchangeRateWithFee(_exchangeRate, txFee);
    }

    function setExchangeRate(
        uint256 _exchangeRate
    ) external onlyExchangeOracle {
    ...
    function setTxFee(
        ...
        txFee = _txFee;
        tokenAMinSwapAmount = _minAmount;
        tokenAMaxSwapAmount = _maxAmount;
    }

    function pause() external {
        require(
            isMultiSigOwner(msg.sender) || emergencyOperator == _msgSender(),
            "Not governance or emergency operator"
        );
        _pause();
    }
    ...
    function unpause() external onlyMultiSig {
        require(
            isMultiSig(msg.sender) || emergencyOperator == _msgSender(),
            "Not governance or emergency operator"
        );
        _unpause();
    }
    ...
    {
        return
            _getAmountOut(amountIn, IERC20Upgradeable(tokenIn), exchangeRate);
    }
    ...
    function verifyMinMaxSwapAmount(
        address tokenIn,
        uint256 amountIn,
        uint256 amountOut
    ) internal view {
        if (tokenIn == address(tokenA)) {
            require(
                amountIn >= tokenAMinSwapAmount &&
                    amountIn <= tokenAMaxSwapAmount
            );
            return;
        }

        require(
            amountOut >= tokenAMinSwapAmount && amountOut <= tokenAMaxSwapAmount
        );
    }
    ...
    {
        IERC20Upgradeable erc20TokenIn = IERC20Upgradeable(tokenIn);
        (amountOut, tokenOut, totalFee) = _getAmountOut(
            amountIn,
            erc20TokenIn,
            exchangeRate
        );

        erc20TokenIn.transferFrom(msg.sender, address(this), amountIn);
        IERC20Upgradeable(tokenOut).transfer(to, amountOut);
        verifyMinMaxSwapAmount(tokenIn, amountIn, amountOut);
        ...
        returns (uint256 amountOut, address tokenOut, uint256 totalFee)
    {
        ...
        if (erc20TokenIn == tokenA) {
            amountOut =
                (amountInWithFee * _exchangeRate) /
                (10 ** tokenADecimal);
            tokenOut = address(tokenB);
        } else {
            amountOut = (amountInWithFee * 10 ** tokenADecimal) / _exchangeRate;
        ...
        returns (uint256 amountInWithFee, address tokenIn, uint256 totalFee)
    {
        ...
        verifyMinMaxSwapAmount(tokenIn, amountInWithFee, amountOut);
        ...
        returns (uint256 amountInWithFee, address tokenIn, uint256 totalFee)
    {
        ...
        if (erc20TokenOut == tokenB) {
            amountInWithFee =
                (amountOut * 10 ** tokenADecimal * 10000) /
                (_exchangeRate * 10000 - exchangeRate * txFee);
            tokenIn = address(tokenA);
        } else {
            amountInWithFee =
                ((amountOut * _exchangeRate) * 10000) /
                (10 ** tokenADecimal * 10000 - 10 ** tokenADecimal * txFee);
            tokenIn = address(tokenB);
        }

        totalFee = (amountInWithFee * txFee) / 10000;
    }

    function changeOperator(address _newOperator) public onlyEmergencyRole {
        require(_newOperator != address(0), "invalid address");
        emergencyOperator = _newOperator;
    }

    function _authorizeUpgrade(address) internal override onlyEmergencyRole {}
