...
contract CockyDocky is ERC721, PaymentSplitter {
    uint256 public totalSupply;
    uint256 private cost = 4 ether;
    ...
    constructor(address[] memory _payees, uint256[] memory _shares)
        ERC721("Cocky Docky", "DCOCK")
        PaymentSplitter(_payees, _shares)
    {}

    function mint() external payable {
        if (totalSupply + 1 > 9999) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    "ipfs://QmNmrcpUVkDEPQhRmyPoxBqwkY4BkghMJSVgM84KwnmpYg/",
