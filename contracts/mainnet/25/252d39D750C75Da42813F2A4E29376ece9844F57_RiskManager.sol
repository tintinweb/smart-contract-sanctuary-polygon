...
import "../vendor/LiquidityMath.sol";
...
contract RiskManager is IRiskManager, BaseLogic {
    ...
    constructor(bytes32 moduleGitCommit_, RiskManagerSettings memory settings) BaseLogic(MODULEID__RISK_MANAGER, moduleGitCommit_) {
        ...
        quadratFactory = settings.quadratFactory;
    }
    ...
    function getNewMarketParameters(address underlying) external override returns (NewMarketParameters memory p) {
        ...
        } else if (quadratFactory != address(0) && IQuadratV1Factory(quadratFactory).isTrustedPool(underlying)) {
            p.pricingType = PRICINGTYPE__FORWARDED__QUADRAT;
            p.pricingParameters = uint32(0);

            p.config.collateralFactor = underlyingLookup[underlying].collateralFactor;
        } else {
            // Uniswap3 TWAP
    ...
    function getPriceInternalQuadrat(address underlying, uint32 /*pricingParameters*/) internal view returns (uint twap, uint twapPeriod) {
            
            uint totalSupply = IERC20(underlying).totalSupply();

            if (totalSupply > 0) {
                IQuadratV1Pool quadrat = IQuadratV1Pool(underlying);
                uint twap0; uint twap1;
                address token0 = quadrat.token0();
                address token1 = quadrat.token1();
                {
                    uint twapPeriod0; uint twapPeriod1;
                    (twap0, twapPeriod0) = getPrice(token0);
                    (twap1, twapPeriod1) = getPrice(token1);
                    twapPeriod = twapPeriod0 > twapPeriod1 ? twapPeriod0 : twapPeriod1;
                }

                IUniswapV3Pool pool = quadrat.pool();
                uint amount0; uint amount1;
                {
                    (uint128 liquidity,,,,) = pool.positions(keccak256(abi.encodePacked(underlying, quadrat.lowerTick(), quadrat.upperTick())));
                    (uint160 sqrtPriceX96,,,,,,) = IUniswapV3Pool(pool).slot0();
                    (amount0, amount1) = LiquidityMath.getAmountsDelta(sqrtPriceX96, TickMath.getSqrtRatioAtTick(quadrat.lowerTick()), TickMath.getSqrtRatioAtTick(quadrat.upperTick()), liquidity);
                }
                twap = (twap0 * amount0 / 10**IERC20(token0).decimals() + twap1 * amount1 / 10**IERC20(token1).decimals()) * 1e18 / totalSupply;
            }
    }
    ...
    function getPriceInternal(AssetCache memory assetCache, AssetConfig memory config) public view FREEMEM returns (uint twap, uint twapPeriod) {
        ...
        } else if (pricingType == PRICINGTYPE__FORWARDED__QUADRAT) {
            (twap, twapPeriod) = getPriceInternalQuadrat(underlying, pricingParameters);
        } else {
            revert("e/unknown-pricing-type");
        }
    }

    function getPrice(address underlying) public view override returns (uint twap, uint twapPeriod) {
...
// Updated to Solidity 0.8 by Quadrat:
...
/// @title Liquidity math functions
...
// Updated to Solidity 0.8 by Quadrat:
//   * Removed getNextSqrtPriceFromAmount0RoundingUp, getNextSqrtPriceFromAmount1RoundingDown,
