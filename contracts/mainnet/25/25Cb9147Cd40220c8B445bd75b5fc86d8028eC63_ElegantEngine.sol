...
interface StakingInterface {
    ...
    function getBoost(uint _boost, address _wallet) external view returns(uint);
...
contract ElegantEngine is Ownable, Concatenate {
    ...
    function addRacers(address _sender, address _con, uint[] calldata _id, uint[] calldata _ev) external { 
        ...
        for(uint i=0; i<_count; i++){
            ...
            perfByRacer[teehee].id = _temp.tempID;
            perfByRacer[teehee].nftContract = _temp.tempCon;
            perfByRacer[teehee].eventWanted = _temp.tempEvent;
    ...
    function getOwnerStaked(address _owner) public view returns ( uint [] memory){
        ...
        for ( uint i = 0; i < ownersTokens.length; i ++ ) {
            ...
            uint _slot = slotByRacer[_owner][t];
    ...
    function tierBoost(uint _id) public returns(uint) {
        address ruleBook = rulesByRace[1].conCheck;
        RulesInterface _rules = RulesInterface(ruleBook);
        address _con = perfByRacer[_id].nftContract;
        uint _tokenID = perfByRacer[_id].id;
        
        bool isBoosted;
        uint i = 1;
        for (i; i < 19; i ++){
            isBoosted = _rules.tokenCheck(_tokenID,i,_con);
            if (isBoosted == true){
    ...
    function buildRace(uint _event) internal {
        ...
        for(i; i<racing; i++){ //go through every racer
            ...
            _temp.tempRID = perfByRacer[_temp.tempID].id;
            racersByRace[_week][raceNum].push(_temp.tempID);   
            
            _temp.tempOwner = stakeableNFT.ownerOf(_temp.tempRID);

            uint boost = _staking.getBoost(_temp.tempRID, _temp.tempOwner);
            boost = boost + tierBoost(_temp.tempID);
            uint _nonce = block.timestamp.sub(deployT)+racersByEvent[_event].length;
            
            perfByRacer[_temp.tempID].firstLeg = randomNumber(_nonce, 7, 10) + boost;
            _nonce = _nonce + 1;
            perfByRacer[_temp.tempID].secondLeg = randomNumber(_nonce, 8, 16) + boost;
            perfByRacer[_temp.tempID].steps = returnSteps(perfByRacer[_temp.tempID].firstLeg,perfByRacer[_temp.tempID].secondLeg);
            perfByRacer[_temp.tempID].raceIn = raceNum;
            uint lMinus = (racersByEvent[_event].length - 1);
            racersByEvent[_event][_r] = racersByEvent[_event][lMinus];
            racersByEvent[_event].pop();
        }
        racesByEvent[_week][raceNum] = _event;
