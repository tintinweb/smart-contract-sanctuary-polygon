...
contract SiloReferral is Ownable {
    ...
    event RegisterCode(bytes32 code, address recipient, uint256 share);
    ...
    event UpdateShare(bytes32 code, uint256 oldShare, uint256 newShare);
    ...
    function registerCode(
        ...
        emit RegisterCode(_code, _recipient, _share);
    }

    function udpateRecipient(bytes32 _code, address _recipient)
        external
        onlyOwner
    {
        require(_code != bytes32(0), "invalid code");
        ...
        emit UpdateRecipient(_code, oldRecipient, _recipient);
    }

    function udpateCodeShare(bytes32 _code, uint256 _share) external onlyOwner {
        require(_code != bytes32(0), "invalid code");
        ...
        emit UpdateShare(_code, oldShare, _share);
    }
    ...
    function setSiloReferrer(address _silo, bytes32 _code)
        external
        onlySilo
    {
        if (_code != bytes32(0)) {
