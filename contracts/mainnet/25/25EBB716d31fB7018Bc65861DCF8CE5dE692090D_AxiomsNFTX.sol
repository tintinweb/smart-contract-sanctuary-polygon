...
contract AxiomsNFTX is Ownable, ERC721Min, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) IdToMultiplier; // 1.5x multipler = 150
    mapping(uint256 => string) TokenURIMap;
    uint256 public defaultMultiplier = 150; // 200 = 2x
    
    constructor(uint256 price, uint256 maxMint, uint256 increaseVal) 
        ERC721Min("AXIOMS X REDEMPTION", "AXI-X") 
    {
        PRICE = price;
        MAX_MINT = maxMint + 1;
    ...
    function getMultiplierAndPrice(uint256 id) external view returns(uint256, uint256) {
        return (IdToMultiplier[id] == 0 ? defaultMultiplier : IdToMultiplier[id], 
    ...
    function redeem(uint256 id) external payable {
        ...
        uint256 multiplier = IdToMultiplier[id] == 0 ? defaultMultiplier : IdToMultiplier[id];
    ...
    function setRedeemedURI(string calldata URI) external onlyOwner {
        _redeemedURI = URI;
    }    
