...
import { ERC1155Upgradeable } from "../dependencies/openzeppelin/v4_7_0/ERC1155Upgradeable.sol";
...
import { ILdaTransferHook } from "../interfaces/ILdaTransferHookFungible.sol";
import { RoyalUtil } from "../shared/RoyalUtil.sol";
import { ERC1155PermitUpgradeable } from "./ERC1155PermitUpgradeable.sol";
import { IFungibleLda } from "./IFungibleLda.sol";
    ...
    IFungibleLda
{
    ...
    /// @dev Maximum supply of all tokens
    ...
    /// @dev Mapping (tokenId) => RightsInfo for this token.
    ///
    ///  DEPRECATED
    ///
    /// @custom:oz-renamed-from _RIGHTS_INFO_
    mapping(uint256 => RightsInfo) internal _RIGHTS_INFO__DEPRECATED_;

    /// @dev Mapping (tokenId) => current supply for this token.
    mapping(uint256 => uint256) internal _CURRENT_SUPPLY_;

    /// @dev Address of the royalties contract.
    address internal _ROYALTIES_CONTRACT_;

    /// @dev Next tokenId to be created.
        ...
        initializer
    {
        ...
        __EIP712_init_unchained("Royal Fungible LDAs", "1");
    ...
    function setRoyaltiesContract(
        ...
        onlyOwner
    {
        _ROYALTIES_CONTRACT_ = royaltiesContract;
        emit SetRoyaltiesContract(royaltiesContract);
    }
    ...
    /**
     * @notice Register a Token. In order for LDAs to be minted, they must
     *  belong to a registered Token that has not yet reached its max supply.
     */
    ...
    {
        require(
            tokenId < _NEXT_TOKEN_ID_,
            "Token is not registered"
        );

        uint256 currentSupply = _CURRENT_SUPPLY_[tokenId];
        uint256 updatedSupply = currentSupply + amount;

        require(
            updatedSupply <= MAX_SUPPLY,
            "Amount exceeds max supply"
        );

        // Update current supply before minting to prevent reentrancy attacks
        _CURRENT_SUPPLY_[tokenId] = updatedSupply;
    ...
    /**
     * @notice Casing of URI is to match the OpenSea spec
    ...
    function getCurrentSupply(
        ...
        returns (uint256)
    {
        return _CURRENT_SUPPLY_[tokenId];
    }
        ...
        override
    {
        ...
        for (uint256 i; i < ids.length; i++) {
            ...
            address contractAddress = _ROYALTIES_CONTRACT_;
            if (contractAddress != address(0)) {
                ILdaTransferHook(contractAddress).beforeLdaTransfer(
                    from,
                    to,
                    tokenId
                );
...
interface IFungibleLda is IERC1155Upgradeable {
