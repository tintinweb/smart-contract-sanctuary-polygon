...
contract WAGMIMusicToken1155 is ERC1155Upgradeable, IERC2981Upgradeable, OwnableUpgradeable {
  ...
  function createAlbum(
    ...
    for(uint256 i=0; i<album._quantities.length; ){
      musics[newTokenId.current()] =
      Music({
  ...
  function getTokenIdsOfAlbum(
      ...
      uint256 index = 0;
      for (uint256 id = 1; id < newTokenId.current(); ){
  ...
  function _getDistribution(
    ...
    uint256 _share = 0;
    for(uint32 i=0; i < musics[_tokenId].stakeHolders.length; ){
  ...
  */
  ...
  function _validateShare(
    ...
    uint32 s;
    for(uint256 i=0; i<_share.length; ){
      s += _share[i];
    }
