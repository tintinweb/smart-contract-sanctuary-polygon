...
interface IBatcher {
  function batchTEL(uint256 totalWithdrawl, address[] memory wallets, uint256[] memory amounts) external returns (bool);
  function batchERC20(IERC20 token, uint256 totalWithdrawl, address[] memory wallets, uint256[] memory amounts) external returns (bool);
}

/**
 * @title Batcher
 * @author Amir Shirif, Telcoin, LLC.
 * @notice groups multiple erc20 transfers together
 */
contract Batcher is IBatcher, TieredOwnership {
  using SafeERC20 for IERC20;
  IERC20 public constant TELCOIN = IERC20(0xdF7837DE1F2Fa4631D716CF2502f8b230F1dcc32);

  /**
   * @notice sends TEL in batches
   * @dev must first approve contract for balance
   * @dev if there is not a zero balance at the end of the transaction the transaction will revert
   ...
   */
  function batchTEL(uint256 totalWithdrawl, address[] memory wallets, uint256[] memory amounts) external override onlyOwner() returns (bool) {
    TELCOIN.safeTransferFrom(_msgSender(), address(this), totalWithdrawl);
    for (uint i = 0; i < wallets.length; i++) {
      TELCOIN.safeTransfer(wallets[i], amounts[i]);
    }
    require(TELCOIN.balanceOf(address(this)) == 0, "Batcher: must have zero balance after execution");
    return true;
  }

  /**
   * @notice sends ERC20 in batches
   * @dev must first approve contract for balance
   * @dev if there is not a zero balance at the end of the transaction the transaction will revert
   ...
   * @return a boolean value indicating whether the operation succeeded.
   */
  function batchERC20(IERC20 token, uint256 totalWithdrawl, address[] memory wallets, uint256[] memory amounts) external override onlyOwner() returns (bool) {
    token.safeTransferFrom(_msgSender(), address(this), totalWithdrawl);
    for (uint i = 0; i < wallets.length; i++) {
      token.safeTransfer(wallets[i], amounts[i]);
    }
    require(token.balanceOf(address(this)) == 0, "Batcher: must have zero balance after execution");
    return true;
  }

  /**
   * @notice retrieves entire amount of desired token
   * @param token the token to be withdrawn
   * @return a boolean value indicating whether the operation succeeded.
   */
  function rescueERC20(IERC20 token) external onlyOwner() returns (bool) {
