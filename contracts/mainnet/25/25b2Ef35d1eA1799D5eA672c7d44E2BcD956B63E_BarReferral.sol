...
// File: ./interfaces/IBarReferral.sol

pragma solidity ^0.8.0;

interface IBarReferral {
    /**
     * @dev Record referral.
     */
...
// File: ./BarReferral.sol


pragma solidity ^0.8.0;

contract BarReferral is IBarReferral, Ownable {
    ...
    /// @notice Record referrer of user
        ...
        override
    {
        ...
        if (
            ...
            referredCounts[_referrer] += 1;
            emit NewReferrer(_user, _referrer);
        }
    }

    /// @notice Add referral commission amount to the referrer of the "account"
    function addReferralReward(address _account, uint256 _reward)
        external
        override
    {
        require(operators[_msgSender()], "Caller is not the operator");
        address referrer = referrers[_account];
        if (referrer != address(0) && _reward > 0) {
            referrerRewards[referrer] += _reward;
            totalReferralRewards += _reward;
            emit ReferrerRewarded(_account, referrer, _reward);
        }
    }
    ...
    /// @notice withdraw wrong tokens sent to this contract
    function recoverWrongTokens(IERC20 _token, uint256 _amount)
        external
        onlyOwner
    {
