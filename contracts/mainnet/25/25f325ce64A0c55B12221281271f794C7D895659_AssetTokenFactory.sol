...
contract AssetToken is ERC20, ReentrancyGuard {
    ...
    /** @dev 
        The enum represents functions on the aset token data contract these functions 
        work like modifiers to check the condition with a require statement 
    */
    enum Modifiers {
        noModifier,
        onlyActiveContract,
        onlyUnfreezedContract,
        onlyIssuer,
    ...
    /** @dev
        To implement this "kind of" modifiers the function have calls to this functiom like this:
        checkAccessToFunction([
            uint256(Modifiers.onlyActiveContract),
            uint256(Modifiers.onlyUnfreezedContract),
            uint256(Modifiers.onlyIssuer),
            uint256(0),
            uint256(0),
            uint256(0)]);

        Which means only 3 modifiers are needed and the rest is filled with zeros to 
        prevent sending a dynamic array
        This can be improved. The approach works to avoid individual calls
        and variable instantiation of the IAssetTokenData interface
    */
    ...
    function checkAccessToFunction(uint256[6] memory modifiers) internal view {
        IAssetTokenData assetTknDtaContract = IAssetTokenData(assetTokenDataAddress);
        uint256 i = 0;
        bool found = false;
        while (i < modifiers.length && modifiers[i] != 0) {
            if (modifiers[i] == uint256(Modifiers.onlyActiveContract)) {
                // slither-disable-next-line calls-loop
                assetTknDtaContract.onlyActiveContract(address(this));
                found = true;
            }
            if (modifiers[i] == uint256(Modifiers.onlyUnfreezedContract)) {
                // slither-disable-next-line calls-loop
                assetTknDtaContract.onlyUnfreezedContract(address(this));
                found = true;
            }
            if (modifiers[i] == uint256(Modifiers.onlyIssuer)) {
                // slither-disable-next-line calls-loop
                assetTknDtaContract.onlyIssuer(address(this), _msgSender());
                found = true;
            }
            if (modifiers[i] == uint256(Modifiers.onlyIssuerOrGuardian)) {
                // slither-disable-next-line calls-loop
                assetTknDtaContract.onlyIssuerOrGuardian(address(this), _msgSender());
                found = true;
            }
            if (modifiers[i] == uint256(Modifiers.onlyIssuerOrAgent)) {
                // slither-disable-next-line calls-loop
    ...
    function _beforeTokenTransfer(
        ...
        /// @dev modifier onlyUnfreezedContract
        ...
        checkAccessToFunction(
            [uint256(Modifiers.onlyUnfreezedContract), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)]
        );
        IAssetTokenData assetTknDtaContract = IAssetTokenData(assetTokenDataAddress);
    ...
    /// @param _newAddress value to be set
    function setAssetTokenData(address _newAddress) external {
        /// @dev modifier onlyUnfreezedContract onlyIssuerOrGuardian
        checkAccessToFunction(
            [
                uint256(Modifiers.onlyUnfreezedContract),
                uint256(Modifiers.onlyIssuerOrGuardian),
                uint256(0),
                uint256(0),
                uint256(0),
                uint256(0)
    ...
    function setKya(string memory _kya) external {
        /// @dev modifier onlyActiveContract onlyUnfreezedContract onlyIssuer
        checkAccessToFunction(
            [
                uint256(Modifiers.onlyIssuerOrGuardian),
                uint256(Modifiers.onlyUnfreezedContract),
    ...
    function freezeContract() external {
        /// @dev modifier onlyIssuerOrGuardian
        checkAccessToFunction(
            [uint256(Modifiers.onlyIssuerOrGuardian), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)]
        );
    ...
    function _requestMint(uint256 _amount, address _destination) private returns (uint256) {
        /// @dev modifier onlyActiveContract onlyUnfreezedContract onlyIssuerOrAgent
        checkAccessToFunction(
            [
                uint256(Modifiers.onlyActiveContract),
                uint256(Modifiers.onlyUnfreezedContract),
                uint256(Modifiers.onlyIssuerOrAgent),
                uint256(0),
                uint256(0),
                uint256(0)
            ]
    ...
    function approveMint(uint256 _mintRequestID, string memory _referenceTo) public nonReentrant {
        /// @dev modifier onlyActiveContract onlyIssuer
        checkAccessToFunction(
            [
                uint256(Modifiers.onlyActiveContract),
                uint256(Modifiers.onlyIssuer),
    ...
    function safeguardStake(uint256 _amount, string memory _receipt) external nonReentrant {
        /// @dev modifier onlyActiveContract
        checkAccessToFunction(
            [uint256(Modifiers.onlyActiveContract), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)]
        );
        ...
        approve(assetTknDtaContract.getGuardian(address(this)), redemptionRequests[_requestID].assetTokenAmount);
    }
    ...
    function _safeguardUnstake(uint256 _amount) private {
        /// @dev modifier onlyActiveContract onlyUnfreezedContract
        checkAccessToFunction(
            [
                uint256(Modifiers.onlyActiveContract),
                uint256(Modifiers.onlyUnfreezedContract),
