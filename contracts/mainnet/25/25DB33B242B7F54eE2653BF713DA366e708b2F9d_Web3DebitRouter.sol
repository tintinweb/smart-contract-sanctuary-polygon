...
contract Web3DebitRouter is ReentrancyGuard {
...
function _swapExactOutputSingleAndPayOnChainERC20(DataSwap memory _dataswap) internal {
    ...
    ISwapRouterUniswapV3.ExactOutputSingleParams memory params =
            ISwapRouterUniswapV3.ExactOutputSingleParams({
                tokenIn: _dataswap.tokenIn,
                tokenOut: _dataswap.tokenOut,
                fee: _dataswap.poolFee,
    ...
    uint amountIn = swapRouterUniswapV3.exactOutputSingle(params);
...
function _swapExactOutputSingleAndPayOnChainNATIVE(DataSwap memory _dataswap) internal {
    ...
    uint amountIn = swapRouterUniswapV3.exactOutputSingle{ value: msg.value }(params);
    ...
    if (amountIn < _dataswap.amountInMaximum) {
        ...
        if (!success) {
            gasdust += _dataswap.amountInMaximum - amountIn;
        }
    }
...
function _swapExactOutputSingleAndPayCrossChainERC20(DataToStargate memory _datastargate, bytes memory _datauniswap) internal {
    ...
    require(ERC20(thetokenIn).approve(address(swapRouterUniswapV3), theamountInMaximum));
    ...
    ISwapRouterUniswapV3.ExactOutputSingleParams memory params =
            ISwapRouterUniswapV3.ExactOutputSingleParams({
                tokenIn: thetokenIn,
                tokenOut: _datastargate.tokenincross_,
                fee: thepoolFee,
                recipient: address(this),
                deadline: thetimeswap,
                amountOut: _datastargate.amountLD_,
                amountInMaximum: theamountInMaximum,
                sqrtPriceLimitX96: 0
            });
    ...
    if (amountIn < theamountInMaximum) {
        require(ERC20(thetokenIn).approve(address(swapRouterUniswapV3), 0));
        require(ERC20(thetokenIn).transfer(msg.sender, theamountInMaximum - amountIn));
    }
      
    require(ERC20(_datastargate.tokenincross_).approve(address(stargateRouter), _datastargate.amountLD_));
    
    _swapToStargateFromERC20(_datastargate, amountIn, thetokenIn, theamountpay);
...
function _swapExactOutputSingleAndPayCrossChainNATIVE(DataToStargate memory _datastargate, bytes memory _datauniswap) internal {
    ...
    uint amountIn = swapRouterUniswapV3.exactOutputSingle{ value: theamountInMaximum }(params);

    require((ERC20(_datastargate.tokenincross_).balanceOf(address(this)) - balancestart) == _datastargate.amountLD_);

    if (amountIn < theamountInMaximum) {
        swapRouterUniswapV3.refundETH();
        (bool success,) = msg.sender.call{ value: theamountInMaximum - amountIn }("");

        if (!success) {
            gasdust += theamountInMaximum - amountIn;
        }
    }
    
    require(ERC20(_datastargate.tokenincross_).approve(address(stargateRouter), _datastargate.amountLD_));
    
    _swapToStargateFromNATIVE(_datastargate, _dataswap, amountIn);
