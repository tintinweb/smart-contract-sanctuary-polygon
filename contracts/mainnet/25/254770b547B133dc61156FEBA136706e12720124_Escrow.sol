...
contract Escrow {
    ...
    uint256 public constant price = 100000000;
    ...
    mapping(uint256 => NFTInfo) public depositedNFTs;

    event NFTDeposited(address indexed user, uint256 indexed nftId);
    event NFTSold(address indexed user, uint256 indexed nftId);
    event NFTWithdrawn(address indexed user, uint256 indexed nftId);

    constructor(address _nftContract, address _stablecoinContract, address _uniswapHandlerAddress) {
        nftContract = IERC721(_nftContract);
        stablecoinContract = IERC20(_stablecoinContract);
        contractOwner = msg.sender;
         uniswapHandler = UniswapV2Handler(_uniswapHandlerAddress);
    }
function setUniswapHandler(UniswapV2Handler _uniswapHandler) public {
    ...
    function deposit(uint256 _nftId) public {
        ...
        depositedNFTs[_nftId] = NFTInfo(msg.sender, block.timestamp);
        emit NFTDeposited(msg.sender, _nftId);
    }
  
    function buyBackUSDT(uint256 _nftId ) public  {
        
        require(msg.sender == contractOwner, "Only Owner");
        require(depositedNFTs[_nftId].depositor != address(0), "NFT not deposited");
        
        nftContract.transferFrom(address(this), contractOwner, _nftId);
        stablecoinContract.transferFrom(contractOwner, depositedNFTs[_nftId].depositor, price);

        emit NFTSold(depositedNFTs[_nftId].depositor, _nftId);
     
    }

     function buyBack(uint256 _nftId ) public  {
        require(msg.sender == contractOwner, "Only Owner");
        require(depositedNFTs[_nftId].depositor != address(0), "NFT not deposited");

        uniswapHandler.swapTokensWithFees(price, depositedNFTs[_nftId].depositor);
        nftContract.transferFrom(address(this), contractOwner, _nftId);
   ...
   function  setcontractOwner(address _Owner) external {
    ...
    function withdraw(uint256 _nftId) public {
        require(msg.sender == depositedNFTs[_nftId].depositor, "Only depositor can withdraw the NFT");
        require(block.timestamp >= depositedNFTs[_nftId].depositTime + 7 days, "7 days have not passed since deposit");

        nftContract.transferFrom(address(this), msg.sender, _nftId);
        emit NFTWithdrawn(msg.sender, _nftId);
