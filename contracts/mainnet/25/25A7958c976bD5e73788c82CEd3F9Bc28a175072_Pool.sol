...
contract Pool is ReentrancyGuard, IPool {
    ...
    function mint(
        ...
        try ITreasury(treasury).allocateSeigniorage() {} catch {}
        ...
        uint256 _actual_10MB_amount = _total_10MB_value.sub((_total_10MB_value.mul(_minting_fee)).div(PRICE_PRECISION));
    ...
    function redeem(
        ...
        if (_effective_collateral_ratio > 0) {
            uint256 _collateral_output_value = _10MB_amount_post_fee.div(10**missing_decimals).mul(_effective_collateral_ratio).div(PRICE_PRECISION);
