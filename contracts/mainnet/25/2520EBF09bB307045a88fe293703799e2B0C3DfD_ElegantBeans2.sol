...
contract ElegantBeans2 is Ownable, AccessControl {
    ...
    function buildRace(uint[] calldata _racers) public {
        ...
        for(i; i<racing; i++){ //go through every racer
            ...
            perfByRacer[rHack].id = _racer;
            racersByRace[raceNum].push(rHack);  
            perfByRacer[rHack].firstLeg = randomNumber(_nonce, 4, 11);
            _nonce = _nonce + 1;
            perfByRacer[rHack].secondLeg = randomNumber(_nonce, 6, 13);
            perfByRacer[rHack].steps = returnSteps(perfByRacer[rHack].firstLeg,perfByRacer[rHack].secondLeg);
