...
contract AssetTokenIssuer is AccessControlUpgradeable {
    using SafeMath for uint256;

    uint256 public constant BPS = 10000;
    uint256 public constant DECIMALS = 27;
    ...
    function authorizeAsset(address _assetAddress, address _priceFeedAddress)
        external
        requireAdmin
        requireNonEmptyAddress(_assetAddress)
        requireNonEmptyAddress(_priceFeedAddress)
    {
        authorizedAssetsPriceFeedAddresses[_assetAddress] = _priceFeedAddress;
    }
    ...
    function setAssetTokenPriceFeedAddress(address _priceFeedAddress)
        external
        requireAdmin
        requireNonEmptyAddress(_priceFeedAddress)
    {
        assetTokenPriceFeedAddress = _priceFeedAddress;
    }
    ...
    function setAssetTokenAddress(address _assetTokenAddress)
        external
        requireAdmin
    ...
    function getAmountToMint(address asset, uint256 amount) external view returns (uint256) {
        ...
        return amountToRequest.mul(10**DECIMALS).div(currentRate);
    }
    ...
    function calculateAmountToRequestMint(address asset, uint256 amount) public view returns (uint256) {
        ...
        uint256 amountToRequest = assetValue.mul(10**DECIMALS).div(uint256(assetTokenPrice));
    ...
    function mint(
        ...
        uint256 amountToRequest = calculateAmountToRequestMint(asset, amount);
        uint256 amountToMint = amountToRequest.mul(10**DECIMALS).div(getCurrentRate());
