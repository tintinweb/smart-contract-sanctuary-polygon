    ...
    Metadata
{
    ...
    function init(bytes memory _encodedData) external {
        ...
        (
            ...
            _encodedData,
            (IERC20, uint256, IERC20, address, string)
        );

        (, , , , , rewardTokenUrl, routerAddress, rewardsDuration) = abi.decode(
            _encodedData,
            (IERC20, uint256, IERC20, address, string, string, address, uint256)
        );

        notifyRewardAmountInternal(rewardAmount, rewardsDuration);
        owner = rewardsDistribution;

        _initMetaOwner(owner);
        _updateMeta(address(stakingToken), routerAddress, stakingTokenUrl);
        _updateMeta(address(rewardsToken), address(0), rewardTokenUrl);
    ...
    {
        notifyRewardAmountInternal(reward, rewardsDuration);
    }

    /* ========== RESTRICTED FUNCTIONS ========== */

    function notifyRewardAmountInternal(uint256 reward, uint256 rewardsDuration)
        internal
