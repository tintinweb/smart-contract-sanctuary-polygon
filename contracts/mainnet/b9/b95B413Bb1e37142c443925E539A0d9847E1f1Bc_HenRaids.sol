...
contract HenRaids is Ownable, VRFConsumerBase {
  ...
  function raid(address account, uint256 landId, uint256 farmId) public payable {
    ...
    RaidCount storage r = farmRaids[account][landId][farmId];
    if (block.timestamp - r.lastRaidTimestamp > 24 hours) {
      farmRaids[account][landId][farmId] = RaidCount(uint128(block.timestamp), uint8(0));
    }
    (, uint256 eth, uint256 matic,) = raidCost(account, landId, farmId);
    ...
    farmRaids[account][landId][farmId].amount++;
