    ...
    SBalanceSheetV2 // no dependency
{
    ...
    function depositCollateral(IErc20 collateral, uint256 depositAmount) external override {
        // Checks: the Fintroller allows this action to be performed.
        if (msg.sender != owner) {
            revert BalanceSheet__DepositCollateralNotAuthorized(msg.sender);
        }
...
interface IBalanceSheetV2 is IOwnableUpgradeable {
    ...
    /// @notice Emitted when account calling to deposit collateral is not authorized.
    error BalanceSheet__DepositCollateralNotAuthorized(address account);
