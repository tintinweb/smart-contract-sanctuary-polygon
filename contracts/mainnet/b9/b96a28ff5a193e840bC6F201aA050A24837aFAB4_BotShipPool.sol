...
contract BotShipPool is Managable, Pausable, ReentrancyGuard, ERC721Holder {
    ...
    event ItemAddedToPool(address indexed _nft, address indexed _owner, uint256 indexed _tokenId, address payToken, uint256 price);
    event ItemUpdated(address indexed _nft, address indexed _owner, uint256 indexed _tokenId, address payToken, uint256 price);
    event ItemRemovedFromPool(address indexed _nft, address indexed _owner, uint256 indexed _tokenId);
    event ItemSold(address indexed nft, address indexed _owner, address indexed buyer, uint256 tokenId, address payToken, uint256 price);
    ...
    event AllAllowedToAdd(bool _allAllowedToAdd);
    ...
    /// @notice Platform fee (will be saved in the % * 1000 form, e.g. 2.755% = 2755)
    uint256 public platformFee;

    bool public allAllowedToAdd;
    ...
    function addItem(
        ...
        if(allAllowedToAdd == false){
            _addItemWithManager(_nftAddress, _tokenId, _payToken, _price);        
        } else {
    ...
    function addManyItems(
        ...
        if(allAllowedToAdd == false){
            _addManyItemsWithManager(_nftAddress, _payToken, _tokenId, _price);        
        } else {
            _addManyItems(_nftAddress, _payToken, _tokenId, _price);        
        }
    }
    ...
    function removeItem(
        ...
        _removeItem(_nftAddress, _tokenId, false);
        IERC721(_nftAddress).safeTransferFrom(address(this), _msgSender(), _tokenId);

        emit ItemRemovedFromPool(_nftAddress, msg.sender, _tokenId);
    }
    ...
    function removeManyItems(
        ...
        for (uint i = 0; i < _tokenId.length; i++){
            _removeItem(_nftAddress, _tokenId[i], false);
            IERC721(_nftAddress).safeTransferFrom(address(this), _msgSender(), _tokenId[i]);

            emit ItemRemovedFromPool(_nftAddress, msg.sender, _tokenId[i]);
        }
    }
    ...
    function buyItem(
        ...
        Listing memory _listing = _removeItem(_nftAddress, _tokenId, true);
    ...
    function buyTeam(
        ...
        address[3] memory _owner;
        for(uint i = 0; i < 3; i++){
            Listing memory _listing = _removeItem(_nftAddress, _tokenId[i], true);
            require(_listing.payToken == _payToken[i], "invalid pay token");
            _price[i] = _listing.price;
            _owner[i] = _listing.owner;
            require(_price[i] > 0, "price is zero");
        }

        _buyTheTeam(_buyTeam.buyer, _nftAddress, _payToken, _price, _tokenId, _owner);
    ...
    function getBotsInPool() public view returns(ExtendedListing[] memory) {
        ...
        for (uint i = 0; i < pooledBotIds.length; i++){
            ...
            _listing.owner = pooledBots[pooledBotIds[i]].owner;
    ...
    function getShipsInPool() public view returns(ExtendedListing[] memory) {
        ...
        for (uint i = 0; i < pooledShipIds.length; i++){
            ...
            _listing.owner = pooledShips[pooledShipIds[i]].owner;
    ...
    /**
     ...
     @dev Only Manager
     @param _platformFee the platform fee to set  !!! (MUST be saved in the % * 1000 form, e.g. 2.755% = 2755)
     */
    function updatePlatformFee(uint256 _platformFee) external onlyManager {
        platformFee = _platformFee;
        emit UpdatePlatformFee(_platformFee);
    }

    function allowAllToAdd() external onlyManager {
         allAllowedToAdd = true;
         emit AllAllowedToAdd(true);
    }

    function forbidAllToAdd() external onlyManager {
         allAllowedToAdd = false;
         emit AllAllowedToAdd(false);
    }
    ...
    function _addManyItemsWithManager(
        address _nftAddress,
        address _payToken,
        uint256[] calldata _tokenId,
        uint256[] calldata _price
    ...
    function _updateItem(
        ...
        if(_nftAddress == _botAddress){
            ...
            require(_existingListing.owner == msg.sender, "not owner");
    ...
    function _buyItem(Listing memory _listing, address _nftAddress, uint _tokenId, address _buyer) private {
       ...
       if(_listing.payToken == BITS_ADDRESS){
            ...
            uint256 feeAmount = (_listing.price * platformFee) / 100000;
            if(feeAmount > 0){
                IERC20(_listing.payToken).approve(treasuryAddress, feeAmount);
                ITokenPaymentSplitter(treasuryAddress).split(BITS_ADDRESS, _buyer, feeAmount);
            }

            require(IERC20(BITS_ADDRESS).transfer(_listing.owner, (_listing.price-feeAmount)));
            ...
            if(feeAmount > 0){
                (bool success,) = treasuryAddress.call{value: feeAmount}(
                    abi.encodeWithSignature("split(address,address,uint256)", ZERO_ADDRESS, _buyer, feeAmount)
                );
                require(success, "Splitter call fail");
            }
            {
                (bool success,) = _listing.owner.call{value: (_listing.price-feeAmount), gas: 50000}("");
                require(success, "Owner call fail");
            }
    ...
    function _buyTheTeam(address _buyer, address _nftAddress, address[3] memory _payToken, uint256[3] memory _price, uint[3] memory _tokenId, address[3] memory _owner) private {
       ...
       for (uint i = 0; i < 3; i++){
            if(_payToken[i] == BITS_ADDRESS){
                ...
                require(IERC20(BITS_ADDRESS).transferFrom(_msgSender(), address(this), _price[i]));

                uint256 feeAmount = (_price[i] * platformFee) / 100000;
                if(feeAmount > 0){
                    IERC20(BITS_ADDRESS).approve(treasuryAddress, feeAmount);
                    ITokenPaymentSplitter(treasuryAddress).split(BITS_ADDRESS, _buyer, feeAmount);
                }

                require(IERC20(BITS_ADDRESS).transfer(_owner[i], (_price[i]-feeAmount)));
                ...
                {
                    (bool success,) = _owner[i].call{value: (_price[i]-feeAmount), gas: 50000}("");
                    require(success, "Owner call fail");
                }
            }
       }
