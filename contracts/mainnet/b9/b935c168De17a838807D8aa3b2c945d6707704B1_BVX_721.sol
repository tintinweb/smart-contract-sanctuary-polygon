...
contract BVX_721 is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721('BovineVerse x XCabin ', 'BVX') {
        myBaseURI = 'https://ipfs.io/ipfs/';
        superMinter = _msgSender();
        newTypes(1,1000,'QmULKovRXw788Nw9LcA1SQpB8Sy7YuymcPaCxZ1a6n1An2');
    }
    ...
    function mintBatch(address addr,uint ID,uint amount) public{
        if (_msgSender() != superMinter) {
            require(minters[_msgSender()] >= amount, 'no mint amount');
            minters[_msgSender()] -= amount;
        }
        require(types[ID].currentAmount + amount <= types[ID].maxAmount,'out of limit');
        types[ID].currentAmount += amount;
