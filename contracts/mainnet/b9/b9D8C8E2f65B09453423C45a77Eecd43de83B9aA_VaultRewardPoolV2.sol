...
contract VaultRewardPoolV2 is permission {
    ...
    function depositFor(address recipient) external payable returns (bool) {
        IWMATIC(factory.getAddr("wmatic")).deposit{ value: msg.value }();
        uint256 wrappedAmount = IERC20(factory.getAddr("wmatic")).balanceOf(address(this));
        IERC20(factory.getAddr("wmatic")).transfer(recipient,wrappedAmount);
        return true;
    }
