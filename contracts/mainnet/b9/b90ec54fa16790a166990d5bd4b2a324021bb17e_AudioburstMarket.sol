...
contract AudioburstMarket is Ownable, ERC721Holder {
    ...
    function buy(uint256 _id) payable external ItemExists(_id) {
        ...
        require(paidAmount >= totalAmount, "Amount is not enough!");
        
        uint256 feeAmount = paidAmount.mul(swapFee).div(PERCENTS_DIVIDER);
        uint256 creatorAmount = paidAmount.mul(pair.royalty).div(PERCENTS_DIVIDER);
        uint256 sellerAmount = paidAmount.sub(feeAmount).sub(creatorAmount);
        payable(swapFeeAddress).transfer(feeAmount);
        payable(pair.creator).transfer(creatorAmount);
        payable(pair.owner).transfer(sellerAmount);
