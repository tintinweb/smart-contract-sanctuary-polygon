...
contract FlowZeroAnimationCollabNFT20230701 is BaseSBT {
    ...
    constructor(
        uint256 maxSupply_
    ) BaseSBT("FLOW zeroanimation collab NFT 20230701", "FLZA20230701") {
    ...
    function airdrop(address to_) external onlyMinter whenNotPaused {
        require(
            _mintedAmount() + 1 <= maxSupply,
            "FLZA20230701: minted amount exceeds the upper limit"
        );
        require(!_isAirdroppeds[to_], "FLZA20230701: already airdropped");
    ...
    function bulkAirdrop(
        ...
        require(
            _mintedAmount() + tos_.length <= maxSupply,
            "FLZA20230701: minted amount exceeds the upper limit"
        );

        for (uint256 i = 0; i < tos_.length; i++) {
            require(
                !_isAirdroppeds[tos_[i]],
                "FLZA20230701: already airdropped"
            );
