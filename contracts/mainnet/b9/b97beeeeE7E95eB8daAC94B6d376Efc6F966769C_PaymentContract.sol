    ...
    UUPSUpgradeable
{
    ...
    function splitSignedData(bytes memory signedData) private pure returns (uint256 amount) {
        // require(signature.length == 65, "sig length should be 65");

        assembly {
            amount := mload(add(signedData, 0))
            // s := mload(add(signedData, 64))
            // v := byte(0, mload(add(signedData, 96)))
        }

        return (amount);
    }

    function claimEx(uint256 amount, uint256 nonce, bytes memory signedData) external {
        bytes memory sig = bytes(signedData);

        claim(amount, nonce, sig);

        // console.log(111, signedData);
        // uint256 amount = splitSignedData(signedData);
