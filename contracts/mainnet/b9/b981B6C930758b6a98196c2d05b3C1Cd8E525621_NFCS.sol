    ...
    Version
{
    ...
    function initialize(IAxelarGateway _axelarGateway) public initializer {
        __ERC721_init("TEST AXELAR", "NTA");
        __Pausable_init();
        __Ownable_init();
        __ERC721Enumerable_init();
        __UUPSUpgradeable_init();
        addressBook = IAddressBook(address(0));
    ...
    event AxelarCall(string sourceChain, string sourceAddress, bytes payload);
    ...
    function setAxelarGateway(IAxelarGateway _gateway) external onlyOwner {
        gateway = _gateway;
    }
    ...
    function _execute(
        ...
        (
            ...
            address b,
            bytes memory s
        ) = abi.decode(payload, (string, uint256, string, address, bytes));
        address[] memory bundle;
        bytes[] memory signatures;
        bundle[0] = b;
        signatures[0] = s;
        mintToken(bundle, signatures, message, nonce, version);
        emit AxelarCall(sourceChain, sourceAddress, payload);
    }
...
string constant CONFIGURATOR_VERSION = "1.0.0";
string constant LIMIT_MANAGER_VERSION = "1.0.1";
string constant LOAN_MANAGER_VERSION = "1.0.1";
string constant NFCS_VERSION = "1.0.7";
string constant POOL_VERSION = "1.0.0";
string constant PRICE_FEED_VERSION = "1.0.0";
string constant SCORE_DB_VERSION = "1.0.1";
string constant SETTINGS_PROVIDER_VERSION = "1.0.1";
...
library Errors {
    ...
    string public constant NFCS_SENDER_IS_NOT_IN_BUNDLE = "7"; //  Ensure the caller of NFCS mint method is some addresses from bundle
    string public constant NFCS_UPDATE_WITHOUT_POPULATE = "11"; //  Data was not populated

    string public constant POOL_TOTAL_SUPPLY_ZERO = "104"; // Zero totalSupply() pool

    string public constant LOAN_MANAGER_COLLATERAL_PAST_LTV = "204"; //Collateral value past ltv
    string public constant LOAN_MANAGER_ZERO_REPAY = "205"; //Loan repay zero
    string public constant LOAN_MANAGER_LOAN_AMOUNT_ZERO = "206"; //Loan amount is zero
    string public constant LOAN_MANAGER_LOAN_IS_LIQUENT = "207"; //Loan amount is zero
    ...
    string public constant ARGUMENTS_LENGTH = "1004"; // Zero value
