...
contract StoryController is Controllable, IStoryController, ERC721Holder {
  ...
  mapping(address => uint) public storyIds;
  /// @dev Store used ids for stories.
  mapping(uint => bool) internal _usedStoryIds;
  /// @dev story => pageId => nextChamber
  mapping(address => mapping(uint => address)) internal nextChamberRewrite;
  /// @dev story => pageId => heroClass (zero is default answers) => answers IDs array (keccak of pageId+heroClassId+answerId), where array index = num of answer
  mapping(address => mapping(uint => mapping(uint => bytes32[]))) public answers;

  /// @dev story => answerId => answerResultId => nextPageIds (will be chosen randomly from this array), where answerResultId is:
  ///      0 - unknown,
  ///      1 - success,
  ///      2 - attr fail
  ///      3 - random fail
  ...
  mapping(address => mapping(bytes32 => mapping(AnswerResultId => uint[]))) public nextPageIds;
  /// @dev story => answerId => requirements array
  mapping(address => mapping(bytes32 => AttributeRequirements[])) public attributeRequirements;
  /// @dev story => answerId => requirements array
  mapping(address => mapping(bytes32 => ItemRequirements[])) public itemRequirements;
  /// @dev story => answerId => requirements array
  mapping(address => mapping(bytes32 => TokenRequirements[])) public tokenRequirements;
  /// @dev story => answerId => random, 1 - 99% success of this action, zero means no check
  mapping(address => mapping(bytes32 => uint)) internal randomRequirement;
  /// @dev story => answerId => delay, if time since the last call more than this value the check is fail, zero means no check
  mapping(address => mapping(bytes32 => uint)) internal delayRequirement;
  /// @dev story => answerId => custom data for hero
  mapping(address => mapping(bytes32 => CustomDataRequirement)) internal heroCustomDataRequirement;
  /// @dev story => answerId => global custom data
  mapping(address => mapping(bytes32 => CustomDataRequirement)) internal globalCustomDataRequirement;

  /// @dev story => answerId => info
  mapping(address => mapping(bytes32 => StoryResultInfo)) internal successInfo;
  /// @dev story => answerId => info
  mapping(address => mapping(bytes32 => StoryResultInfo)) internal failInfo;
  /// @dev story => answerId => resultType => custom data, where resultType is
  ///      0 - hero success
  ///      1 - hero fail
  ///      2 - global success
  ///      3 - global fail
  mapping(address => mapping(bytes32 => mapping(CustomDataResult => CustomData[]))) internal customDataResult;
  /// @dev story => Custom hero data requirements for a story. If exist and hero is not eligible should be not chose in a dungeon.
  mapping(address => CustomDataRequirementRange[]) internal storyRequiredHeroData;
  /// @dev story => Minimal level for the history. 0 means no requirements.
  mapping(address => uint) internal storyRequiredLevel;
  /// @dev story => answer = Burn random equipped item
  mapping(address => mapping(bytes32 => BurnInfo)) internal burnItem;

  // hero states

  /// @dev story => hero => heroId => pageId
  mapping(address => mapping(address => mapping(uint => uint))) public heroPage;
  /// @dev story => hero => heroId => timestamp
  mapping(address => mapping(address => mapping(uint => uint))) public heroLastActionTS;
  /// @dev story => answerIdHash, this answer will immediately finish the story
  mapping(address => mapping(bytes32 => bool)) internal finalAnswers;
  /// @dev Prevent register the story twice
  mapping(address => bool) public registeredStories;

  // ---- EVENTS ----

  event SetBurnItemsMeta(address story, uint storyId, bytes32[] answerBurnRandomItemAnswerId, uint[] answerBurnRandomItemSlot, uint[] answerBurnRandomItemChance);
  event SetNextChambersRewriteMeta(address story, uint storyId, NextChambersRewriteMeta meta);
  event SetAnswersMeta(address story, uint storyId, AnswersMeta meta);
  event SetAnswerNextPageMeta(address story, uint storyId, AnswerNextPageMeta meta);
  event SetAnswerAttributeRequirements(address story, uint storyId, AnswerAttributeRequirementsMeta meta);
  event SetAnswerItemRequirements(address story, uint storyId, AnswerItemRequirementsMeta meta);
  event SetAnswerTokenRequirementsMeta(address story, uint storyId, AnswerTokenRequirementsMeta meta);
  event SetAnswerRandomRequirementMeta(address story, uint storyId, AnswerRandomRequirementMeta meta);
  event SetAnswerDelayRequirementMeta(address story, uint storyId, AnswerDelayRequirementMeta meta);
  event SetAnswerHeroCustomDataRequirementMeta(address story, uint storyId, AnswerCustomDataMeta meta);
  event SetAnswerGlobalCustomDataRequirementMeta(address story, uint storyId, AnswerCustomDataMeta meta);
  event SetSuccessInfo(address story, uint storyId, AnswerResultMeta meta);
  event SetFailInfo(address story, uint storyId, AnswerResultMeta meta);
  event SetCustomDataResult(address story, uint storyId, AnswerCustomDataResultMeta meta, CustomDataResult _type);
  event StoryFinalAnswers(address story, uint storyId, bytes32[] finalAnswers_);
  event StoryCustomDataRequirements(address story, uint storyId, bytes32 requiredCustomDataIndex, uint requiredCustomDataMinValue, uint requiredCustomDataMaxValue, bool requiredCustomDataIsHero);
  event StoryRequiredLevel(address story, uint requiredLevel);
  event StoryFinalized(address story, uint storyId);
  event StoryRemoved(address story);
  ...
  function currentHeroAnswers(
    address story,
    address hero,
    uint heroId
  ) public view returns (bytes32[] memory) {
    uint heroClass = IHero(hero).heroClass();
    mapping(uint => bytes32[]) storage a1 = answers[story][heroPage[story][hero][heroId]];
    bytes32[] storage heroAnswers = a1[heroClass];
  ...
  function isStoryAvailableForHero(address story, address heroToken, uint heroTokenId) external view override returns (bool) {
    return StoryLib.isStoryAvailableForHero(
      story,
      heroToken,
      heroTokenId,
      IController(controller()).statController(),
      storyRequiredHeroData,
      storyRequiredLevel
    );
  }

  function _isStoryValid(address story, uint storyId) internal view {
    require(storyId != 0, "zero storyId");
    require(!_usedStoryIds[storyId], "used storyId");
    require(!registeredStories[story], "story registered");
  }
  ...
  function setBurnItemsMeta(
    ...
    address story = msg.sender;
    _isStoryValid(story, storyId);

    unchecked {
      uint length = answerBurnRandomItemAnswerId.length;
      if (length != 0) {
        mapping(bytes32 => BurnInfo) storage answersBurn = burnItem[story];
        for (uint i; i < length; ++i) {
          answersBurn[answerBurnRandomItemAnswerId[i]] = BurnInfo(answerBurnRandomItemSlot[i], answerBurnRandomItemChance[i]);
        }
      }
    }

    emit SetBurnItemsMeta(story, storyId, answerBurnRandomItemAnswerId, answerBurnRandomItemSlot, answerBurnRandomItemChance);
  }

  function setNextChambersRewriteMeta(uint storyId, NextChambersRewriteMeta memory meta) external override {
    address story = msg.sender;
    _isStoryValid(story, storyId);

    unchecked {
      for (uint i; i < meta.nextChamberPageIds.length; ++i) {
        nextChamberRewrite[story][meta.nextChamberPageIds[i]] = meta.nextChamberAddresses[i];
      }
    }

    emit SetNextChambersRewriteMeta(story, storyId, meta);
  }

  function setAnswersMeta(uint storyId, AnswersMeta memory meta) external override {
    address story = msg.sender;
    _isStoryValid(story, storyId);
    unchecked {

    // remove all answers if exist
      for (uint i; i < meta.answerPageIds.length; ++i) {
        delete answers[story][meta.answerPageIds[i]][meta.answerHeroClasses[i]];
      }

      for (uint i; i < meta.answerPageIds.length; ++i) {
        bytes32[] storage answersHashes = answers[story][meta.answerPageIds[i]][meta.answerHeroClasses[i]];
        bytes32 answerHash = keccak256(abi.encodePacked(meta.answerPageIds[i], meta.answerHeroClasses[i], meta.answerIds[i]));
        answersHashes.push(answerHash);
      }
    }

    emit SetAnswersMeta(story, storyId, meta);
  }

  function setAnswerNextPageMeta(uint storyId, AnswerNextPageMeta memory meta) external override {
    address story = msg.sender;
    _isStoryValid(story, storyId);

    unchecked {
      for (uint i; i < meta.answerIndexes.length; ++i) {
        nextPageIds[story][meta.answerIndexes[i]][AnswerResultId(meta.answerResultIds[i])] = meta.answerNextPageIds[i];
      }
    }

    emit SetAnswerNextPageMeta(story, storyId, meta);
  }

  function setAnswerAttributeRequirements(uint storyId, AnswerAttributeRequirementsMeta memory meta) external override {
    address story = msg.sender;
    _isStoryValid(story, storyId);
    unchecked {
      for (uint i; i < meta.answerIndexes.length; ++i) {
        delete attributeRequirements[story][meta.answerIndexes[i]];
        AttributeRequirements[] storage attrs = attributeRequirements[story][meta.answerIndexes[i]];
        for (uint j; j < meta.cores[i].length; ++j) {
          attrs.push(AttributeRequirements({
            core: meta.cores[i][j],
            id: meta.ids[i][j],
            value: meta.values[i][j]
          }));
        }
      }
    }

    emit SetAnswerAttributeRequirements(story, storyId, meta);
  }

  function setAnswerItemRequirements(uint storyId, AnswerItemRequirementsMeta memory meta) external override {
    address story = msg.sender;
    _isStoryValid(story, storyId);
    unchecked {
      for (uint i; i < meta.answerIndexes.length; ++i) {
        delete itemRequirements[story][meta.answerIndexes[i]];
        ItemRequirements[] storage attrs = itemRequirements[story][meta.answerIndexes[i]];
        for (uint j; j < meta.requireItems[i].length; ++j) {
          attrs.push(ItemRequirements({
            requireItem: meta.requireItems[i][j],
            requireItemBurn: meta.requireItemBurn[i][j],
            requireItemEquipped: meta.requireItemEquipped[i][j]
          }));
        }
      }
    }

    emit SetAnswerItemRequirements(story, storyId, meta);
  }

  function setAnswerTokenRequirementsMeta(uint storyId, AnswerTokenRequirementsMeta memory meta) external override {
    address story = msg.sender;
    _isStoryValid(story, storyId);
    unchecked {
      for (uint i; i < meta.answerIndexes.length; ++i) {
        delete tokenRequirements[story][meta.answerIndexes[i]];
        TokenRequirements[] storage attrs = tokenRequirements[story][meta.answerIndexes[i]];
        for (uint j; j < meta.requireToken[i].length; ++j) {
          attrs.push(TokenRequirements({
            requireToken: meta.requireToken[i][j],
            requireAmount: meta.requireAmount[i][j],
            requireTransfer: meta.requireTransfer[i][j]
          }));
        }
      }
    }

    emit SetAnswerTokenRequirementsMeta(story, storyId, meta);
  }

  function setAnswerRandomRequirementMeta(uint storyId, AnswerRandomRequirementMeta memory meta) external override {
    address story = msg.sender;
    _isStoryValid(story, storyId);
    unchecked {
      for (uint i; i < meta.answerIndexes.length; ++i) {
        randomRequirement[story][meta.answerIndexes[i]] = meta.randomRequirements[i];
      }
    }

    emit SetAnswerRandomRequirementMeta(story, storyId, meta);
  }

  function setAnswerDelayRequirementMeta(uint storyId, AnswerDelayRequirementMeta memory meta) external override {
    address story = msg.sender;
    _isStoryValid(story, storyId);
    unchecked {
      for (uint i; i < meta.answerIndexes.length; ++i) {
        delayRequirement[story][meta.answerIndexes[i]] = meta.delayRequirements[i];
      }
    }

    emit SetAnswerDelayRequirementMeta(story, storyId, meta);
  }

  function setAnswerHeroCustomDataRequirementMeta(uint storyId, AnswerCustomDataMeta memory meta) external override {
    address story = msg.sender;
    _isStoryValid(story, storyId);
    unchecked {
      for (uint i; i < meta.answerIndexes.length; ++i) {
        heroCustomDataRequirement[story][meta.answerIndexes[i]] = CustomDataRequirement({
          index: meta.dataIndexes[i],
          valueMin: meta.dataValuesMin[i],
          valueMax: meta.dataValuesMax[i],
          mandatory: meta.mandatory[i]
        });
      }
    }

    emit SetAnswerHeroCustomDataRequirementMeta(story, storyId, meta);
  }

  function setAnswerGlobalCustomDataRequirementMeta(uint storyId, AnswerCustomDataMeta memory meta) external override {
    address story = msg.sender;
    _isStoryValid(story, storyId);
    unchecked {
      for (uint i; i < meta.answerIndexes.length; ++i) {
        globalCustomDataRequirement[story][meta.answerIndexes[i]] = CustomDataRequirement({
          index: meta.dataIndexes[i],
          valueMin: meta.dataValuesMin[i],
          valueMax: meta.dataValuesMax[i],
          mandatory: meta.mandatory[i]
    ...
    emit SetAnswerGlobalCustomDataRequirementMeta(story, storyId, meta);
  }

  function setSuccessInfo(uint storyId, AnswerResultMeta memory meta) external override {
    address story = msg.sender;
    _isStoryValid(story, storyId);
    unchecked {
      for (uint i; i < meta.answerIndexes.length; ++i) {

        successInfo[story][meta.answerIndexes[i]] = StoryResultInfo({
          positiveAttributes: AttributesInfo(meta.positiveIds[i], meta.positiveValues[i]),
          negativeAttributes: AttributesInfo(meta.negativeIds[i], meta.negativeValues[i]),
          experience: meta.experience[i],
          heal: meta.heal[i],
          manaRegen: meta.manaRegen[i],
          lifeChancesRecovered: meta.lifeChancesRecovered[i],
          damage: meta.damage[i],
          manaConsumed: meta.manaConsumed[i],
          mintItems: meta.mintItems[i],
          mintItemsChances: meta.mintItemsChances[i]
        });

      }
    }
    emit SetSuccessInfo(story, storyId, meta);
  }

  function setFailInfo(uint storyId, AnswerResultMeta memory meta) external override {
    address story = msg.sender;
    _isStoryValid(story, storyId);
    unchecked {
      for (uint i; i < meta.answerIndexes.length; ++i) {

        failInfo[story][meta.answerIndexes[i]] = StoryResultInfo({
          positiveAttributes: AttributesInfo(meta.positiveIds[i], meta.positiveValues[i]),
          negativeAttributes: AttributesInfo(meta.negativeIds[i], meta.negativeValues[i]),
          experience: meta.experience[i],
          heal: meta.heal[i],
    ...
    emit SetFailInfo(story, storyId, meta);
  }
  ...
  function setCustomDataResult(
    ...
    unchecked {
      for (uint i; i < meta.answerIndexes.length; ++i) {
        bytes32 answerIndex = meta.answerIndexes[i];
        bytes32[] memory dataIndexes = meta.dataIndexes[i];
        uint[] memory dataValues = meta.dataValues[i];
        bool[] memory increase = meta.increase[i];

        delete customDataResult[story][answerIndex][_type];
        CustomData[] storage d = customDataResult[story][answerIndex][_type];
    ...
    emit SetCustomDataResult(story, storyId, meta, _type);
  }

  function setStoryFinalAnswers(uint storyId, bytes32[] memory finalAnswers_) external override {
    address story = msg.sender;
    _isStoryValid(story, storyId);

    unchecked {
      for (uint i; i < finalAnswers_.length; ++i) {
        finalAnswers[story][finalAnswers_[i]] = true;
      }
    }

    emit StoryFinalAnswers(story, storyId, finalAnswers_);
  }
  ...
  function setStoryCustomDataRequirements(
    ...
    storyRequiredLevel[story] = minLevel;
    emit StoryRequiredLevel(story, minLevel);

    delete storyRequiredHeroData[story];
    CustomDataRequirementRange[] storage allData = storyRequiredHeroData[story];

    for (uint i; i < requiredCustomDataIndex.length; ++i) {
      allData.push(CustomDataRequirementRange({
        index: requiredCustomDataIndex[i],
        minValue: requiredCustomDataMinValue[i],
        maxValue: requiredCustomDataMaxValue[i],
        isHeroData: requiredCustomDataIsHero[i]
      }));

      emit StoryCustomDataRequirements(story, storyId, requiredCustomDataIndex[i], requiredCustomDataMinValue[i], requiredCustomDataMaxValue[i], requiredCustomDataIsHero[i]);
    }
  }

  function finalizeStoryRegistration(uint storyId) external override {
    address story = msg.sender;
    _isStoryValid(story, storyId);

    registeredStories[story] = true;
    // remove old id
    delete _usedStoryIds[storyIds[story]];
    // store new used id
    _usedStoryIds[storyId] = true;
    // register new id for story
    storyIds[story] = storyId;

    emit StoryFinalized(story, storyId);
  }

  // ---- CHANGE META ----

  function removeStory(address story) external {
    require(isGovernance(msg.sender), "!gov");

    delete _usedStoryIds[storyIds[story]];
    delete storyIds[story];
    delete registeredStories[story];
  ...
  function storyAction(
    ...
    uint heroTokenId,
    bytes memory data
  ) external override returns (IChamber.ChamberResult memory) {
    require(msg.sender == story, "INVALID");

    IController _controller = IController(controller());

    require(IChamberController(_controller.chamberController()).validChambers(story), "INVALID");

    StoryActionContext memory context = StoryActionContext({
      sender: sender,
      dungeon: dungeon,
      story: story,
      stageId: stageId,
      controller: _controller,
      statController: IStatController(_controller.statController()),
      heroToken: heroToken,
      heroTokenId: heroTokenId,
      answerIdHash: _decodeAnswerId(data)
    });

    IChamber.ChamberResult memory result = _getEmptyResult();
    result.chamber = story;
    result.heroToken = heroToken;
    result.heroTokenId = heroTokenId;


    uint pageId = heroPage[story][heroToken][heroTokenId];
    bytes32[] memory heroAnswers = currentHeroAnswers(story, heroToken, heroTokenId);

    if (heroAnswers.length == 0) {
      // end this chamber
      result.rewriteNextChamber = nextChamberRewrite[story][pageId];
      heroPage[story][heroToken][heroTokenId] = 0;
      result.completed = true;
    } else {
      (bool finalAnswer, IChamber.ChamberResult memory returnResult) = _handleAnswer(context, result, heroAnswers);
      result = returnResult;
      if (finalAnswer) {
        result.rewriteNextChamber = nextChamberRewrite[story][pageId];
        heroPage[story][heroToken][heroTokenId] = 0;
    ...
    heroLastActionTS[story][heroToken][heroTokenId] = block.timestamp;
    return result;
  }

  function _decodeAnswerId(bytes memory data) internal pure returns (bytes32 answerIdHash) {
    (answerIdHash) = abi.decode(data, (bytes32));
  }

  function _handleAnswer(
    StoryActionContext memory context,
    IChamber.ChamberResult memory result,
    bytes32[] memory heroAnswers
  ) internal returns (bool finalAnswer, IChamber.ChamberResult memory returnResult) {

    bytes32 answerIndex = StoryLib.findAnswerIndex(heroAnswers, context.answerIdHash);
    finalAnswer = finalAnswers[context.story][answerIndex];

    AnswerResultId answerResult = StoryLib.checkAnswer(
      context,
      answerIndex,
    ...
    returnResult = StoryLib.handleAnswer(
      answerIndex,
      answerResult,
      heroPage,
      customDataResult,
    ...
    StoryLib.burn(context, burnItem[context.story][answerIndex]);
  }

  function _getEmptyResult() internal pure returns (IChamber.ChamberResult memory result) {
    IChamber.ChamberResult[] memory emptyResult = new IChamber.ChamberResult[](1);
