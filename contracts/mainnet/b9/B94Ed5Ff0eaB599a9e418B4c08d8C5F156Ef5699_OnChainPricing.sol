...
contract OnChainPricing is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function findRouter(
        address[] memory path
    ) public pure returns (address router, address pool) {
        ...
        if (path[0] == KNC_ADDRESS || path[pathLength - 1] == KNC_ADDRESS) {
            address knc_weth = 0xd8B9E9444fCBF26BEA4BAdd6142dD6a962BCA86A;
            address knc_wmatic = 0x37e6449B0e99BeFD2A708eA048d970F4FF4dC65d;
            address knc_wmatic2 = 0xfd3b7305693956994Fc0cC973F4Dd046fc3500AC;

            address wamtic = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
            address weth = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619;

            if (path[1] == wamtic || path[pathLength - 2] == wamtic) {
                return (KYBER_DMM_ROUTER, knc_wmatic);
            } else if (path[1] == weth || path[pathLength - 2] == weth) {
                return (KYBER_DMM_ROUTER, knc_weth);
            }
        } else {
            return (QUICK_ROUTER, address(0));
        }
    ...
    function findOptimalRouter(
        ...
        // uint256 kyberQuote = getUniPrice(KYBER_DMM_ROUTER, path, amountIn);
        // quotes[1] = Quote(KYBER_DMM_ROUTER, kyberQuote);
