...
import "./utils/CrossMultiplier.sol";
...
contract EthRateKeeperV3 is KeeperCompatibleInterface, Ownable, CrossMultiplier {
    ...
    */
    ...
    uint256 crossMultiplier_numerator;
    ...
    */
    ...
    event IntervalUpdated(uint _oldInterval, uint _newInterval);
    event CrossMultiplierNumeratorUpdated(uint256 _oldValue, uint256 _newValue);
    ...
    constructor(uint _updateInterval, address payable _crowdsale, uint256 _crossMultiplier_numerator) {
      crowdsale = _crowdsale; //Crowdsale to perform upkeep
      priceFeed = AggregatorV3Interface(0xAB594600376Ec9fD91F8e885dADF0CE036862dE0);
      crossMultiplier_numerator = _crossMultiplier_numerator;
    ...
    function checkUpkeep(bytes calldata /* checkData */) external view override returns (bool upkeepNeeded, bytes memory performData ) {
        ...
        int maticPrice = _getLatestPrice();

        //3-. Do conversion
        uint256 result = _convert(maticPrice);
    ...
    function performUpkeep(bytes calldata performData ) external override {
        ...
        //2-. Get current matic price
        int maticPrice = _getLatestPrice();
        require(maticPrice != 0, "performUpkeep: Matic price can't be 0");

        //3-. Do conversion
        uint256 result = _convert(maticPrice);

        require(result != 0, "performUpkeep: conversion result can't be 0");

        //Load the smart contract
        PausableCrowdsale c = PausableCrowdsale(crowdsale); 
        
        //Update new rate
        bool success = c.setNewEthRate(result);
        require(success, "performUpkeep: setNewEthRate failed");
    }
     ...
     */
    function _convert(int _maticPrice) internal view returns (uint256 result){
        result = _crossMul(_maticPrice.toUint256(), crossMultiplier_numerator);
    }
    ...
    function updateInterval(uint _newInterval) public onlyOwner {
        ...
        uint _oldInterval = _interval;
        _interval = _newInterval;
        emit IntervalUpdated( _oldInterval,  _newInterval);
    }

     /**
     * @dev Sets a new bc value for the cross multiplier
     * @param _newValue New value, can't be 0
     */
    function setCrossMultiplier_numerator(uint256 _newValue) public onlyOwner {
        require(_newValue != 0, "setCrossMultiplier_numerator: new value can not be 0");
        uint256 _oldValue = crossMultiplier_numerator;
        crossMultiplier_numerator = _newValue;
        emit CrossMultiplierNumeratorUpdated( _oldValue,  _newValue);
    }
...
contract CrossMultiplier {
     /**
     * @dev Given two fractions a/b = c/d => d = (b * c) / a.
     * reverting on division by zero. The result is rounded 
     * towards zero
     * @param a numerator of left-side fraction
     * @param b denominator of left-side fraction
     * @param c numerator of right-side fraction
     * @return d denominator of right-side fraction
     */
    function _crossMul(uint256 a, uint256 b, uint256 c) internal pure returns (uint256 d){
        d = (b * c) / a;
        return d;
    }
     ...
     * reverting on division by zero. The result is rounded 
     * towards zero
     * Sometimes b and c will never change, in this case we can store
     * (b * c) and only calculate bc / a
     * @param a numerator of left-side fraction
     * @param bc precalculated (b * c) operation
     * @return d denominator of right-side fraction
     */
    function _crossMul(uint256 a, uint256 bc) internal pure  returns (uint256 d){
        d = (bc) / a;
        return d;
    }
