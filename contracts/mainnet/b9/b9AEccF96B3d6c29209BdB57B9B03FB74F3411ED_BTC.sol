...
interface IBORROW {
    ...
    function borrows(uint256 btcWei) external returns (bool);
...
contract BTC is IBTC, IOWNER {
    ...
    function borrows(uint256 btcWei) external virtual override returns (bool) {
    ...
    function payments(uint256 usdaWei)
        external
        virtual
        override
        returns (bool)
    ...
    function _mintBy(uint256 usdaWei) internal returns (bool) {
        return USDA.borrow(usdaWei) == true;
    }

    function _burnBy(uint256 usdaWei) internal returns (bool) {
        return USDA.payment(usdaWei) == true;
    }
    ...
    function _burn(address from, uint256 btcWei_) internal returns (bool) {
        uint256 btcWei = _ParseWei(btcWei_);
    ...
    function _transfer(
        ...
        require(
            _hasWei(from, btcWei_) == true && !_isBlack(from),
            "Not balance"
        );
        uint256 btcWei = _ParseWei(btcWei_);
    ...
    function _reward1Second(uint256 time_) internal view returns (uint256) {
        ...
        uint256 _variable_ = time_ * _maxCap - _totalSupply() / 10;
        uint256 _secondTen = 365 * 24 * 3600;
        return (_variable_ / _secondTen);
    }
