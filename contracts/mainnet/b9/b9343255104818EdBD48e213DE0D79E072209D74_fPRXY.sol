...
contract fPRXY is Ownable, ERC20Burnable {
    ...
    mapping(address => mapping(uint256 => uint256)) public userRewardPerTokenPaidById;
    ...
    modifier updateReward(address account) {
        ...
        if (block.timestamp >= stakingStartDate) {
            ...
            for (uint256 i = 0; i < _stakeId; i++) {
                ...
                uint256 _apyIndex = _stakeAmount < 1000
                    ? 0
                    : (_stakeAmount > 10000 ? apy[2] : apy[1]);
                lastUpdateTime = lastTimeRewardApplicable();
                if (account != address(0)) {
                    rewards[account] = earned(account, _apyIndex, _stakeAmount);
                    userRewardPerTokenPaidById[account][i] = rewardPerTokenStored[_apyIndex];
                }
            }
        }
        _;
    ...
    function earned(address account, uint256 _apyIndex, uint256 _stakeAmount)
        public
        view
        ...
        returns (uint256 _rewardEarned)
    {
        for (uint256 i = 0; i < _stakeId.length; i++) {
            uint256 _stake = userStakeById[account][_stakeId[i]];
            _rewardEarned += _stake
                .mul(
                    rewardPerToken(_stake).sub(
                        userRewardPerTokenPaidById[account][_stakeId[i]]
                    )
                )
                .div(1e18)
                .add(rewards[account]);
    ...
    function getTotalRewardForYearByAmount(uint256 _amount)
        ...
        returns (uint256)
    {
        uint256 apyInfo = _amount < 1000
            ? apy[0]
            : (_amount > 10000 ? apy[2] : apy[1]);
        return
            apyInfo
                .mul(totalSupply())
                .mul(getPrxyPriceInUsd())
    ...
    function rewardRate(uint256 _amount) public view returns (uint256) {
        return getTotalRewardForYear(_amount).div(31536000);
    }
