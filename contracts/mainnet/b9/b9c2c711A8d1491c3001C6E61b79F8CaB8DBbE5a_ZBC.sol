...
contract SwapWallet {
    ...
    function withdrawBatch(address token, address[] memory tos, uint256[] memory amounts) external {
        require(msg.sender == zbc, "RewardPool: FORBIDDEN"); // sufficient check
        require(tos.length == amounts.length, "RewardPool: FORBIDDEN"); // sufficient check
...
contract ZBC is Ownable, ZBCCompound {

    using SafeMath for uint256;

    uint256 constant private initialSupply = 10000000000 ether;
    ...
    constructor(string memory name_, string memory symbol_,
        address mint_, address ownerAdmin,address v2Router_, address usdt_, address ido_) ERC20(name_,symbol_) {
        if (ownerAdmin == address(0)) {
            ownerAdmin = 0xeB5F3c0c3AD43421710B969e2857F4231E32adbf;
        }
        _transferOwnership(ownerAdmin);

        _mint(mint_, initialSupply);
        ...
        if (v2Router_ != address(0)) {
            sys.v2Router = v2Router_;
        } else {
            sys.v2Router = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506;
        }
        if (usdt_ != address(0)) {
            sys.usdt = usdt_;
        }else {
            sys.usdt = 0xc2132D05D31c914a87C6611C10748AEb04B58e8F;
        }
        _initTax();
        sys.zbcWallet = _createAddr("ZBCSwapWallet");

        sys.zbcIDO = ido_;
        isExcluded[sys.zbcIDO] = true;
        isTaxExcluded[sys.zbcIDO] = true;

        isTaxExcluded[msg.sender] = true;
        isTaxExcluded[mint_] = true;
        isTaxExcluded[sys.zbcWallet] = true;
        isTaxExcluded[sys.zbcIDO] = true;
    }

    function setNFTPool(address nftPool_) external onlyOwner {
        sys.zbcNFTPool = nftPool_;
        feeAccounts[6] = sys.zbcNFTPool;
    }

    function setMutualPool(address mutualPool_) external onlyOwner {
        sys.zbcMutual = mutualPool_;
        feeAccounts[7] = sys.zbcMutual;
    }

    function mintFromIDO(address addr,uint256 amount) external {
        require(msg.sender == sys.zbcIDO,"ZBC: mintFromIDO not allowed");
        _mint(addr,amount);
    }
    ...
    function _initTax() private {
        ...
        isExcluded[sys.zbcMutual] = true;
        isExcluded[sys.zbcNFTPool] = true;
        isExcluded[sys.zbcWallet] = true;
        isExcluded[sys.v2Router] = true;
        isExcluded[address(0)] = true;
    }

    function _beforeTransfer() private {
        if (sys.v2USDTPool == address(0)) {
            IRouter router = IRouter(sys.v2Router);
            address pairUsdt = IRouter(router.factory()).getPair(address(this), sys.usdt);
        ...
        if (sys.v2MaticPool == address(0)) {
            IRouter router = IRouter(sys.v2Router);
            address weth = IRouter(sys.v2Router).WETH();
    ...
    function sellToUSDT(uint256 _sellAmount,address[] memory path) private{
        if (super.allowance(address(this),sys.v2Router) < 100000000e18) {
            super._approve(address(this),sys.v2Router, 1000000000000e18);
        }
        uint256[] memory amountsOut = IRouter(sys.v2Router).swapExactTokensForTokens(
            _sellAmount,0,path,sys.zbcWallet,block.timestamp+10000);
        require(amountsOut[amountsOut.length - 1] >0,"swap err");
        _distribute(amountsOut[amountsOut.length - 1]);
    }

    function _distribute(uint256 swapOutAmount) private {

        uint256 ecoTotalAmount = swapOutAmount.mul(defaultFees.ecoSystemFee).div(100);
        uint256 nftPoolAmount = swapOutAmount.mul(defaultFees.nftPoolFee).div(100);
        uint256 mutualAmount = swapOutAmount.mul(defaultFees.mutualPoolFee).div(100);

        uint256[] memory amounts = new uint256[](8);
        for (uint i=0; i<6; i++) {
            amounts[i] = ecoTotalAmount.div(6);
        }
        amounts[6] = nftPoolAmount;
        amounts[7] = mutualAmount;

        IWallet(sys.zbcWallet).withdrawBatch(sys.usdt,feeAccounts,amounts);
    }
...
abstract contract ZBCCompound is ERC20{
    ...
    uint256 constant  private epoch = 90;
    ...
    function balanceOf(address account) public view virtual override returns (uint256) {
        ...
        return rewardAmount(user.activeBlockNumber,user.lastRewardBlock,block.number,balance);
    }
    function _updateReward(address account) internal {
        ...
        uint256 balance = rewardAmount(user.activeBlockNumber,
    ...
    function rewardAmount(uint256 activeBlock,uint256 lastRewardBlock,
        ...
        uint256 blocks = nowBlock.sub(activeBlock);
        ...
        for (uint256 i=1; i <= nums+1; i++) {
            uint256 point = activeBlock.add(epochBlocks * i);
            nSubsidy = baseProfit >> (i-1);

            if (lastRewardBlock >= point) {
                continue;
            }
            if (nowBlock >= point) {
