...
/**
 * @dev used to split up 1 input amount into 4 output amounts, and send amounts to recipients
 */
...
/*            
 ...
 0x0000....0000->[              ]->X2
 0x0000....0000->[              ]->X3
 0x0000....0000->[______________]->X4

Note: The bottom two Token A's can be 0x0000....0000, but if they are, then the ratio for them must be zero
Note: Ratio summed up must equal 10,000
Note: X 1,2,3,4 are up to 4 recipient addresses
*/
...
contract AdvancedTransfer is BaseSiloAction {
  ...
  constructor(string memory _name, address _siloFactory) {
    name = _name;
    metaData = "address[4],address[4],uint[4],address[4],address[],address";
  ...
  function enter(
    ...
    outputAmounts = transferAction(implementation, configuration, inputData);
  }
  ...
  function exit(
    ...
    outputAmounts = transferAction(implementation, configuration, outputData);
  }
  ...
  function transferAction(
    ...
    bytes memory configuration,
    bytes memory inputData
  ) private returns (uint256[4] memory outputAmounts) {
    uint256 baseDecimals = 10000;
    ActionInfo memory info;
    ...
    {
      uint256[4] memory ratio;
      address[4] memory input;
      address[4] memory output;
      address[4] memory recipients;
      address[] memory _path;
      address oracle;
      (input, output, ratio, recipients, _path, oracle) = abi.decode(
        configuration,
        (address[4], address[4], uint256[4], address[4], address[], address)
      );
      info = ActionInfo({
        inputs: input,
        outputs: output,
        ...
        ratio: ratio,
        path: _path,
        Oracle: IExchangeRateOracle(oracle),
        alpha: 0,
        beta: 0,
        upkeep: 0,
        siloManager: address(0)
      });

      uint256 balance = IERC20(input[0]).balanceOf(address(this));
      if(inputAmounts[0] > balance){
        inputAmounts[0] = balance;
      }
    }

    {
      bool noConvert;
      address[4] memory siloTokens;
      address potentialSilo;

      bool isSiloManager;
      for (uint256 i = 0; i < 4; i++) {
        potentialSilo = address(0); //reset it
        noConvert = false; //reset it
        if (info.ratio[i] == 0) {
          continue;
        } //skip
        if (info.recipients[i] == address(0)) {
          //compounding
          potentialSilo = address(this);
        } else if (IAction(implementation).getIsSilo(info.recipients[i])) {
          //sending to a silo
          potentialSilo = info.recipients[i];
        }

        if (potentialSilo == address(0)) {
          isSiloManager = IAction(implementation).getIsSiloManager(
            address(this),
            info.recipients[i]
          );
          if (isSiloManager) {
            //sending to a non silo
          
            if (
              info.inputs[0] == ERC20_LINK_ADDRESS ||
              info.inputs[0] == ERC677_LINK_ADDRESS
            ) {
              noConvert = true;
            }else{
              info.siloManager = info.recipients[i];
            }
          } else {
            noConvert = true;
          }
        } else {
          siloTokens = ISilo(potentialSilo).getInputTokens();
          for (uint256 j = 0; j < 4; j++) {
            if (siloTokens[j] == info.inputs[0]) {
              //see if receiving silo can accept this reward token
              noConvert = true;
              break;
            }
          }
        ...
        if (noConvert) {
          info.outputs[i] = info.inputs[0]; //set output[i] to be whatever the reward token is
          info.alpha += info.ratio[i];
        } else {
          if (isSiloManager) {
            info.outputs[i] = ERC20_LINK_ADDRESS; //set output[i] to whatever path converts it into
            info.upkeep += info.ratio[i];
          } else {
            info.outputs[i] = info.path[info.path.length - 1]; //set output[i] to whatever path converts it into
            info.beta += info.ratio[i];
          }
        }
      }
    }
    //make a swap if beta > 0
    uint256[] memory amounts = new uint256[](info.path.length);
    // address[] memory path ;
    bool result;
    ///
    if (
      info.upkeep > 0 && info.path.length > 1 && info.siloManager != address(0)
    ) {
      address[] memory path = new address[](info.path.length + 1);

      for (uint256 index = 0; index < info.path.length; ) {
        path[index] = info.path[index];
        unchecked {
          index++;
        }
      }

      path[info.path.length] = ERC20_LINK_ADDRESS;

      uint256 amount = (inputAmounts[0] * info.upkeep) / baseDecimals;

      Quote memory best = info.Oracle.findOptimalRouter(path, amount);

      if (best.amountOut > 0) {
        // SafeERC20.safeApprove(IERC20(info.path[0]), best.router, 0);
        SafeERC20.safeApprove(IERC20(info.path[0]), best.router, amount);
        (amounts, result) = _swap(
          best.router,
          path,
          amount,
          best.amountOut,
      ...
      inputAmounts[0] = inputAmounts[0] - amount; //remaining input[0]
    }
    ///
    if (info.beta > 0 && info.path.length > 1) {
      Quote memory best;
      uint256 amount = (inputAmounts[0] * info.beta) / baseDecimals;

      best = info.Oracle.findOptimalRouter(info.path, amount);
      ...
      amounts[0] = inputAmounts[0] - amount; //remaining input[0]
    } else {
      amounts[0] = inputAmounts[0];
    }
    for (uint256 i = 0; i < 4; i++) {
      //now set outputAmounts
      if (info.ratio[i] == 0) {
        continue;
      }
      if (
        info.siloManager != address(0) && info.siloManager == info.recipients[i] && info.upkeep > 0
      ) {
        outputAmounts[i] = 0;
      } else {
        if (info.outputs[i] == info.inputs[0]) {
          //use amounts[0] and alpha
          outputAmounts[i] = (amounts[0] * info.ratio[i]) / info.alpha;
        } else {
          //use amounts[1] and beta
          outputAmounts[i] =
            (amounts[amounts.length - 1] * info.ratio[i]) /
            info.beta;
        }
      }
    }
    for (uint256 i = 0; i < 4; i++) {
      if (
        info.recipients[i] != address(0) &&
        info.recipients[i] != address(this) &&
        outputAmounts[i] > 0
      ) {
        SafeERC20.safeTransfer(
          IERC20(info.outputs[i]),
          info.recipients[i],
          outputAmounts[i]
        );
      }
  ...
  {
    (inputs, outputs, ratio, recipients, path, oracle) = abi.decode(
      configData,
      (address[4], address[4], uint256[4], address[4], address[], address)
    );
  }
    ...
    returns (bool)
  {
    ...
    Quote memory best = IExchangeRateOracle(oracle).findOptimalRouter(
      path,
      1000000
    );
...
abstract contract BaseSiloAction is Ownable {
    ...
    function getIsSiloManager(address _silo,address _manager) public view returns(bool){
...
interface IAction{
    ...
    function getIsSiloManager(address _silo,address _manager) external view returns(bool);
