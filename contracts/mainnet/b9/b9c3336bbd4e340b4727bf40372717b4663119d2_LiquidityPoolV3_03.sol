...
// 
contract LiquidityPoolV3_03 is ReentrancyGuardUpgradeable, AccessControlUpgradeable, PausableUpgradeable, ERC20Upgradeable {
	...
	uint256 constant public contractVersion = 3 << 8 + 3;

	uint256 public N_TOKENS;
	IERC20Upgradeable[] public TOKENS;
	uint256[] public TOKENS_MUL;
	...
	event WithdrawAdminFee(address indexed addressTo, uint256[] tokenAmounts, uint256 totalAmount);
	event Deposit(address indexed user, uint256[] tokenAmounts, uint256 totalAmount, uint256 fee, uint256 mintedAmount);
	event Withdraw(address indexed user, uint256[] tokenAmounts, uint256 burnedAmount);
	event Borrow(address indexed user, uint256[] tokenAmounts, uint256[] balances, uint256 totalAmount, uint256 fee, uint256 adminFee);
	...
	****************************************/
	...
	function _totalBalanceWithAdminFee ()
		internal
		view
		returns (uint256 totalBalanceWithAdminFee_, uint256[] memory _balances)
	{
		_balances = _balancesWithAdminFee();
		for (uint256 i = 0; i < N_TOKENS; i++) {
			totalBalanceWithAdminFee_ = totalBalanceWithAdminFee_.add(
				_balances[i].mul(TOKENS_MUL[i])
			);
		}
	}
	...
	function _balancesWithAdminFee ()
		internal
		view
		returns (uint256[] memory balancesWithAdminFee_)
	{
		balancesWithAdminFee_ = new uint256[](N_TOKENS);
		for (uint256 i = 0; i < N_TOKENS; i++) {
		...
		internal
	{
		...
		for (uint256 i = 0; i < N_TOKENS; i++) {
			if (outAmounts_[i] != 0)
				TOKENS[i].safeTransfer(msg.sender, outAmounts_[i]);
		}
	...
	****************************************/
	...
	function withdrawAdminFee ()
		external
		nonReentrant
		returns (uint256[] memory outAmounts_)
	{
		...
		for (uint256 i = 0; i < N_TOKENS; i++) {
			if(outAmounts_[i] != 0){
				// There is a calculation error, probably due to evm specifics / safemath
				// To cope with it there is a workaround:
				// If totalSupply is zero (there is no user's liquidity in pool)
				// We withdraw remaining amount of tokens (which will be less than admin balance by calculation errors)
				if (_totalSupply != 0) {
				...
				TOKENS[i].safeTransfer(adminFeeAddress, outAmounts_[i]);
			}
		}
		...
		returns (uint256 mintAmount_)
	{
		...
		for (uint256 i = 0; i < N_TOKENS; i++) {
			if (amounts_[i] != 0) {
				TOKENS[i].safeTransferFrom(msg.sender, address(this), amounts_[i]);
	...
	/*
	 * Withdraw functions can only be called when totalSupply is greater that zero
	 * It will help to stop execution earlier (less gas is used) and provide human-readable error
	 * Another reason to check - we can skip protecting calculation functions with nonZeroTotalSupply modifier
	 * So there is no error raised when calling those functions
	 */
	 ...
	 * @return outAmounts_ array of tokens amounts that were withdrawn
	 */
	function withdrawUnbalanced (
		uint256 amount_,
		uint256[] calldata outAmountPCTs_
	)
		external
		nonReentrant
		nonZeroTotalSupply
		validArrayLength(outAmountPCTs_)
		returns (uint256[] memory outAmounts_)
	{
		outAmounts_ = calcWithdrawUnbalanced(amount_, outAmountPCTs_);
		_withdraw(amount_, outAmounts_);
	}
	 ...
	 */
	function withdrawUnbalancedExactOut (
		uint256[] calldata outAmounts_
	)
		external
		nonReentrant
		nonZeroTotalSupply
		validArrayLength(outAmounts_)
		returns (uint256 amount_)
	{
		
		amount_ = calcWithdrawUnbalancedExactOut(outAmounts_);
		_withdraw(amount_, outAmounts_);
	}
		...
		validArrayLength(amounts_)
	{
		...
		for (uint256 i = 0; i < N_TOKENS; i++) {
			...
			if(amounts_[i] != 0) {
				_totalAmount = _totalAmount.add(amounts_[i].mul(TOKENS_MUL[i]));
				TOKENS[i].safeTransfer(msg.sender, amounts_[i]);
			}
		}
		...
		(
			uint256 _totalBalanceAfter,
			uint256[] memory balances
		) = _totalBalanceWithAdminFee();
		require(
			_totalBalanceAfter >= _totalBalance.add(_fee),
		...
		emit Borrow(msg.sender, amounts_, balances, _totalAmount, _fee.sub(_adminFee), _adminFee);
	}
	...
	****************************************/
	 ...
	 * @return _totalBalance uint256 balance
	 */
	function totalBalance ()
		public
		view
		returns (uint256 _totalBalance)
	{
		if (totalSupply() == 0) return 0;
		(_totalBalance, ) = _totalBalanceWithAdminFee();
		return _totalBalance.sub(adminBalance);
	}
	...
	/**
	 * Withdraw calculation functions return 0 if total supply is zero
	 * There is no modifier, because they may be used for calculating values on servers
	 * So we provide them zeros (because user can withdraw 0 with any amount of vTokens)
	 * Also, we don't need require() here because functions that perorm withdraw 
	 * Are already protected by nonZeroTotalSupply modifier
	 */
	...
	function calcWithdraw (
		...
		returns (uint256[] memory outAmounts_)
	{
		outAmounts_ = new uint256[](N_TOKENS);
		uint256 _totalSupply = totalSupply();
	 ...
	 * @return outAmounts_ array of token amounts will be returned on withdraw
	 */
	function calcWithdrawUnbalanced (
		uint256 amount_,
		uint256[] calldata outAmountPCTs_
	)
		public
	 ...
	 */
	function calcWithdrawUnbalancedExactOut (
		uint256[] calldata outAmounts_
	)
		public
		view
	...
	{
		uint256 _totalSupply = totalSupply();
		return _totalSupply != 0 ? (totalBalance()).mul(10 ** NORM_BASE).div(totalSupply()) : 0;
	}
	...
	function getTokenIdByAddress(
		...
		returns (uint256 id)
	{
		for (id = 0; id < TOKENS.length; id++) {
			if (address(TOKENS[id]) == address_) return id;
		}
		require(
			false,
			"Token is not presented in pool"
		);
	}
