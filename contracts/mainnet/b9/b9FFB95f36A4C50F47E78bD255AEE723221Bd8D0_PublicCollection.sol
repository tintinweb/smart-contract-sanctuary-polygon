...
import "./Common/ICollection.sol";
import "./Common/ICollectionConfig.sol";

import "./Common/StrLib.sol";

//-----------------------------------
// PublicCollection(ERC721)
//-----------------------------------
contract PublicCollection is Ownable, ERC721, ICollection {
    //--------------------------------------
    // 定数
    //--------------------------------------
    string private constant TOKEN_NAME = "Lead Edge Market Public Collection";
    string private constant TOKEN_SYMBOL = "LEMPC";
    ...
    //-----------------------------------------------------------------------------------------------
    // [external/onlyOwner] 設定（ここは初期化のイメージ）
    // 通常のコレクションがコントラクトでやっている設定の代替窓口（イベントを発火しない＝サーバーが追う必要はない）
    //-----------------------------------------------------------------------------------------------
    function setCollectionId(uint256 id) external onlyOwner {
        _collectionId = id;
    }

    function setCollectionConfig(address target) external onlyOwner {
        _collectionConfig = ICollectionConfig(target);
    }
        ...
        returns (string memory)
    {
        ...
        return (
            string(
                abi.encodePacked(
                    "ipfs://",
                    _meta_hashes[tokenId - TOKEN_ID_OFFSET]
                )
            )
        );
    }
    ...
    //------------------------------------------------------------------------------
    // [external] トークンの作成（誰でも呼べる＆呼び出しを制限する場合は署名で行う）
    //------------------------------------------------------------------------------
    function createToken(string calldata metaHash, string calldata signature)
        external
        override
    {
        // 署名確認（公開コレクション）
        if (_collectionConfig.signRequiredForPublic()) {
            string memory strContract = StrLib.numToStrHex(
                uint256(uint160(address(this))),
                40
            );
            string memory strSender = StrLib.numToStrHex(
                uint256(uint160(msg.sender)),
                40
            );
            string memory message = string(
                abi.encodePacked(strContract, strSender, metaHash)
            );
            require(
                _collectionConfig.checkSignature(signature, message),
                "invalid signature"
            );
        }
    ...
    //------------------------------------------------------------------------------
    // [external] メタデータのハッシュの更新（誰でも呼べる＆呼び出しを制限する場合は署名で行う）
    //------------------------------------------------------------------------------
    function updateMetaHash(
        ...
        if (_collectionConfig.signRequiredForPublic()) {
            ...
            string memory message = string(
                abi.encodePacked(strContract, strSender, strId, metaHash)
            );
            require(
                _collectionConfig.checkSignature(signature, message),
                "invalid signature"
    ...
    //-------------------------------------------------------------------
    // [external/onlyOwner] 管理者によるミント
    //-------------------------------------------------------------------
    function createTokenWithAdmin(address creator, string calldata metaHash)
        external
        override
        onlyOwner
    {
        ...
        // event
        emit TokenCreated(address(this), creator, metaHash, tokenId);
    ...
    //-------------------------------------------------------------------
    // [external/onlyOwner] 管理者によるメタデータの更新
    //-------------------------------------------------------------------
    function updateMetaHashWithAdmin(
        uint256 tokenId,
        address creator,
        string calldata metaHash
        ...
        // event
        emit MetaHashUpdated(address(this), tokenId, creator, metaHash);
    }
}
...
interface ICollection {
  ...
  function createTokenWithAdmin(address creator, string calldata metaHash) external;
...
interface ICollectionConfig {
    ...
    function checkSignature(string calldata signature, string calldata message)
        external
        view
        returns (bool);
}
