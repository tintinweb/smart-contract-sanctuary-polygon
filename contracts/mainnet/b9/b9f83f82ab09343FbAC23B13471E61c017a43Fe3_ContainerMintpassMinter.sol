...
contract ContainerMintpassMinter is AccessControl, Withdrawable {
    ...
    function safeMintAll(uint256 start) external onlyRole(STAFF_ROLE) {
        if(!mintActive) revert MintNotActive();

        // Loop through Mintpass holders
        uint256 mintpasses = _mintpass.totalSupply();
        for (uint256 i = start; i <= mintpasses; i++) {
            address _owner = _mintpass.ownerOf(i);
            _container.safeMint(_owner);
        }
    }
    
    function safeMintLimit(uint256 limit) external onlyRole(STAFF_ROLE) {
        if(!mintActive) revert MintNotActive();
