...
contract Semantic is ISemantic, AccessControl, Semantic721 {
    ...
    mapping(uint256 => mapping(uint256 => mapping(address => bool))) public subjectOwner;
    ...
    //token
    mapping(uint256 => mapping(uint256 => mapping(string => uint256))) public tokenDataForId;
        ...
        onlyRole(getRoleAdmin(DEFAULT_ADMIN_ROLE))
    {
        for (uint256 i = 0; i < operateDatas.length; i++) {
            ...
            for (
                ...
                require(
                    tokenDataForId[subjectId][predicateAndObject.predicateId][predicateAndObject.object] == 0,
                    "already exist"
                );
            }
            _mintTurtle(
    ...
    function _mintTurtle(
        ...
        string memory rdf = "";
        Turtle storage p = tokenIdForTurtleData[tokenId];
        p.subjectId = subjectId;
        for (uint256 i = 0; i < predicateAndObjects.length; i++) {
            PredicateAndObject memory predicateAndObject = predicateAndObjects[i];
            p.predicateAndObjects.push(predicateAndObject);
            tokenDataForId[subjectId][predicateAndObject.predicateId][predicateAndObject.object] = tokenId;
    ...
    function _burn(
        ...
        Turtle memory turtle = tokenIdForTurtleData[tokenId];
        Subject memory  subject = subjectIdForData[turtle.subjectId];
        string memory rdf = "";
        for (uint256 i = 0; i < turtle.predicateAndObjects.length; i++) {
            PredicateAndObject memory predicateAndObject = turtle.predicateAndObjects[i];
            delete tokenDataForId[turtle.subjectId][predicateAndObject.predicateId][predicateAndObject.object];
            string memory singleRdf = generateRdf(
                subject.classId,
                subject.value,
                predicateAndObject.predicateId,
        ...
        if (subjectId > 0) {
            require(
                _isSubjectOwner(subjectId) ||
                    tokenDataForId[subjectId][OWNER_PREDICATE_ID][
                        ALL_PERMISSION
                    ] >
                    0,
                "not have permission"
    ...
    function _setSubjectOwner(uint256 subjectId) internal virtual {  
        subjectOwner[subjectId][OWNER_PREDICATE_ID][msg.sender] = true;
    }
    ...
    {
        return
            subjectOwner[subjectId][OWNER_PREDICATE_ID][msg.sender] 
                || tokenDataForId[subjectId][OWNER_PREDICATE_ID][Strings.toHexString(msg.sender)] > 0;
    }
