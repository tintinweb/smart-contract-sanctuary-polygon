...
contract Luckyredenvelope {
    ...
    constructor() {
        owner = msg.sender;
        bool[] memory isplay = new bool[](4);
        isplay[0]=false;
        isplay[1]=false;
        isplay[2]=false;
        isplay[3]=false;
        user[msg.sender]=Investor(msg.sender,0,0,0,0,0,isplay,playtype,msg.sender);
        for (uint i = 0; i < bonustype.length; i++) {
            ...
            if (i > 0) {
                bonustype[i].Amount = i == 1 ? 2e16 : (i == 2 ? 3e16 : 4e16);
                //bonustype[i].Amount = i == 1 ? 100 ether : (i == 2 ? 200 ether : 500 ether);
                bonustype[i].number = 10;
            } else {
                bonustype[i].Amount =  1e16;
                //bonustype[i].Amount =  50 ether;
                bonustype[i].number = 10;
            }
        }
    }
    ...
    function Deposit(address ref) payable public {
        ...
        if(userer.addr== address(0)){
            ...
            isplay[2]=false;
            isplay[3]=false;
            user[msg.sender]=Investor(msg.sender,msg.value,0,0,0,0,isplay,playtype,ref);
        }else{
    ...
    function Start(uint index) public restricted {
        if(bonustype[index].isActive){
            ...
            for (uint i = 0; i < bonustype[index].redAddr.length; i++) {
                user[bonustype[index].redAddr[i]].isplay[index]=false;
    ...
    function Joinin(uint index) public {
        ...
        require(!user[msg.sender].isplay[index], "user false");
        require(user[msg.sender].amount>=bonustype[index].Amount, "no amount");
        Investor storage  userer = user[msg.sender];
        userer.isplay[index]=true;
    ...
    function settlement(uint index) private  {
        ...
        for (uint i = 0; i < number; i++) {
            ...
            if (resultID == i) {
                user[redAddr[i]].isplay[index] = false;
    ...
    function userisplay(address addr) public view returns(bool[] memory) {
        return user[addr].isplay;
    }
