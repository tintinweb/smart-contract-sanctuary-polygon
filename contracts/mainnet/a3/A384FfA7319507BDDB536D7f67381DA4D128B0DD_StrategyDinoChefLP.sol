...
import "../Common/StratManager2.sol";
import "../Common/FeeManager2.sol";
import "../../utils/StringUtils.sol";

contract StrategyDinoChefLP is StratManager2, FeeManager2 {
    ...
    constructor(
    ...
    ) public StratManager2(_keeper, _strategist, _unirouter, _vault, _treasury) {
        want = _want;
        poolId = _poolId;
        chef = _chef;
        ...
        usdc = _outputToUSDCRoute[_outputToUSDCRoute.length - 1];
    ...
    function chargeFees() internal {
        // uint256 toNative = IERC20(output).balanceOf(address(this)).mul(treasuryFee).div(TREASURY_MAX);
        // IUniswapRouterETH(unirouter).swapExactTokensForTokens(toNative, 0, outputToNativeRoute, address(this), now);
        // uint256 nativeBal = IERC20(native).balanceOf(address(this));

        uint256 toUSDC = IERC20(output).balanceOf(address(this)).mul(treasuryFee).div(TREASURY_MAX);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toUSDC, 0, outputToUSDCRoute, address(this), now);

        uint256 usdcBal = IERC20(native).balanceOf(address(this));

        IERC20(usdc).safeTransfer(treasury, usdcBal);
    }
...
contract StratManager2 is Ownable, Pausable {
    /**
     * @dev Beefy Contracts:
     ...
     * @param _treasury address where to send Beefy's fees.
     */
    constructor(
        address _keeper,
        address _strategist,
     ...
     * @param _treasury new beefy fee recipient address.
     */
...
import "./StratManager2.sol";

abstract contract FeeManager2 is StratManager2 {
    uint256 public constant STRATEGIST_FEE = 0;
    uint256 public constant MAX_FEE = 1000;
    uint256 public constant MAX_CALL_FEE = 0;

    uint256 public constant WITHDRAWAL_FEE_CAP = 0;
    uint256 public constant WITHDRAWAL_MAX = 10000;

    uint256 public constant TREASURY_MAX = 1000;
    ...
    function setTreasuryFee(uint256 _fee) public onlyManager {
        require(_fee <= TREASURY_MAX, "!cap");
