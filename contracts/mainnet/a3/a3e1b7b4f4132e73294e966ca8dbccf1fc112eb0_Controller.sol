...
contract Controller {
    ...
    function initApprovals() external {
        if (PRICEFACT[USDT] < 1) {
            PRICEFACT[USDT] = 1000000000000000000;
        }

        if (PRICEFACT[USDC] < 1) {
            PRICEFACT[USDC] = 1000000000000000000;
        }

        if (PRICEFACT[DAI] < 1) {
            PRICEFACT[DAI] = 1000000000000000000;
        }
    ...
    function protocol_in(address _usddollar, uint256 _amount) internal {
    ...
    function protocol_out(address _usddollar, address _aave) internal {
        require(
    ...
    function deposit(
        ...
        // deposit to AAVE
        protocol_in(_usddollar, _amount);

        pendTime[msg.sender] = block.timestamp + (120000 * MULT);

        uint256 _tokenamount = ((_amount * 100000000000000) *
            PRICEFACT[_usddollar]) / 1e18;

        // user balance add
        pendBalance[msg.sender] = pendBalance[msg.sender] + _tokenamount;

        pendBalance[_refer] = pendBalance[_refer] + (_tokenamount / 10);
    }

    // only withdraws the interest
    function withdraw(address _usddollar, address _aave) internal {
        if (IERC20(_usddollar).balanceOf(address(this)) > 1000000) {
            //get money from aave
            protocol_out(_usddollar, _aave);
            ...
            PRICEFACT[_usddollar] = IERC20(TOKEN).balanceOf(address(this));
    ...
    function runUSDT(uint256 _amount, address _refer) external {
        deposit(USDT, _amount, _refer);
        withdraw(USDT, aUSDT);
    }

    function runUSDC(uint256 _amount, address _refer) external {
        deposit(USDC, _amount, _refer);
        withdraw(USDC, aUSDC);
    }

    function runDAI(uint256 _amount, address _refer) external {
