...
contract SolaceStrategy is BaseStrategy, IERC721Receiver {

    uint256 internal constant keepMax = 10000;
    uint256 public xsLockID; //given: other users have already deposited before us

    address internal constant SOLACE = 0x501acE9c35E60f03A2af4d484f49F9B1EFde9f40;
    address internal constant WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;

    address internal constant SOLACE_REWARDS = 0x501ace3D42f9c8723B108D4fBE29989060a91411;

    //0x85Efec4ee18a06CE1685abF93e434751C3cb9bA9 is the Solace/FRAX Uniswap v3 pool
    ...
    constructor()
        ...
        BaseStrategy(
            ...
            SOLACE,
            0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff, //QS router (not used)
            0x501Ace47c5b0C2099C4464f681c3fa2ECD3146C1 //xS Locker
        )
    {
        //Note: `rewards` is the xS Locker contract that SOLACE is deposited in
        //SOLACE_REWARDS is the contract that rewards are claimed from
    ...
    function balanceOfPool() public override view returns (uint256) {
        return ISolaceLocker(rewards).stakedBalance(address(this));
    }

    function getHarvestable() external override view returns (uint256) {
        return 0; //ISolaceRewards(rewards).pendingRewardsOfLock(address(this));
    }
    ...
    function harvest() public override nonReentrant {
        ...
        //Deduct perf fee from harvested SOLACE
    ...
    function _getReward() internal {
        ISolaceRewards(SOLACE_REWARDS).harvestLock(xsLockID);
    }
    ...
    function _withdrawSome(uint256 _amount)
        ...
        returns (uint256)
    {
        ISolaceLocker(rewards).withdrawInPart(xsLockID, address(this), _amount);
        return _amount;
    }
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            if(xsLockID == 0) {
                xsLockID = ISolaceLocker(rewards).createLock(address(this), _want, 0);
            }
            else {
                ISolaceLocker(rewards).increaseAmount(xsLockID, _want);
            }
        }
    }

    //don't need to care about NFT if we're not voting
    function onERC721Received(
        address operator,
        address from,
        uint256 tokenId,
    ...
    function emergencyWithdraw(uint256 _xsLockID, uint256 _amount) public onlyOwner {
        ISolaceLocker(rewards).withdrawInPart(_xsLockID, address(this), _amount);
    }
}