...
contract OddzPredictionOps is KeeperCompatibleInterface, IResolver, Ownable {
    ...
    mapping(OddzPredictionStorage => IOddzPrediction) public storagePrediction;
    OddzPredictionStorage[] public storages;


    constructor(OddzPredictionStorage[] memory _oddzPredictionStorage, IOddzPrediction[] memory _oddzPrediction) {
        for(uint i = 0 ; i < _oddzPredictionStorage.length; i++) {
                storages.push(_oddzPredictionStorage[i]);
               
                storagePrediction[_oddzPredictionStorage[i]] = _oddzPrediction[i];
                currentRoundId.push(_oddzPrediction[i].currentRoundId());
        }
        ...
        )
    {
        for(uint i = 0 ; i< storages.length; i++ ){
          IOddzPrediction.Round memory round = storages[i].getPredictionRound(currentRoundId[i]);

        upkeepNeeded = block.timestamp > round.betEndTimestamp && round.roundState != IOddzPrediction.RoundState.ENDED;
    ...
    function performUpkeep(bytes calldata) external override {
        for(uint i = 0 ; i< storages.length; i++ ){
            ...
            if (block.timestamp > round.betEndTimestamp && round.roundState != IOddzPrediction.RoundState.ENDED) {
                storagePrediction[storages[i]].executeRound();
    ...
    function checker() external view override returns (bool canExec, bytes memory execPayload) {
        ...
        for(uint i = 0 ; i< storages.length; i++ ){
           ...
           canExec = block.timestamp > round.betEndTimestamp && round.roundState != IOddzPrediction.RoundState.ENDED;

           execPayload = abi.encodeWithSignature("executeRound()", storagePrediction[storages[i]]);
    ...
    function setPredicitionMarket(OddzPredictionStorage _oddzPredicitionStorage, IOddzPrediction _oddzPredicition) public onlyOwner {
        require(address(_oddzPredicitionStorage) == address(0), "Invalid storage");
        require(address(_oddzPredicition) == address(0), "Invalid prdicitions");

        if(address(storagePrediction[_oddzPredicitionStorage]) != address(0)){
            storages.push(_oddzPredicitionStorage);
            currentRoundId.push(_oddzPredicition.currentRoundId());
        }
            
            storagePrediction[_oddzPredicitionStorage] = _oddzPredicition;
