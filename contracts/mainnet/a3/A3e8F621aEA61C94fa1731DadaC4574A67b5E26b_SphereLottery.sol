...
contract SphereLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInSphere = 50 ether;
    uint256 public minPriceTicketInSphere = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInSphere;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] spherePerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInSphere;
        uint32 finalNumber;
    }
     ...
     * @param _sphereTokenAddress: address of the SPHERE token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _sphereTokenAddress, address _randomGeneratorAddress) {
        sphereToken = IERC20(_sphereTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of SPHERE to this contract
        uint256 amountSphereToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInSphere,
            _ticketNumbers.length
        );

        // Transfer sphere tokens to this contract
        sphereToken.safeTransferFrom(address(msg.sender), address(this), amountSphereToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInSphere += amountSphereToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInSphereToTransfer
        uint256 rewardInSphereToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInSphereToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        sphereToken.safeTransfer(msg.sender, rewardInSphereToTransfer);

        emit TicketsClaim(msg.sender, rewardInSphereToTransfer, _lotteryId, _ticketIds.length);
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in SPHERE per group, and make lottery claimable
     * @param _lotteryId: lottery id
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInSphere) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in SPHERE for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].spherePerBracket[j] =
                ...
                // A. No SPHERE to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].spherePerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInSphere - amountToShareToWinners);

        // Transfer SPHERE to treasury address
        sphereToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in SPHERE token
     * @dev Callable by owner or injector address
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        sphereToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInSphere += _amount;
     ...
     * @param _priceTicketInSphere: price of a ticket in SPHERE
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInSphere >= minPriceTicketInSphere) && (_priceTicketInSphere <= maxPriceTicketInSphere),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInSphere: _priceTicketInSphere,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            spherePerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInSphere: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(sphereToken), "Cannot be SPHERE token");
    ...
    /**
     * @notice Set SPHERE price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInSphere: minimum price of a ticket in SPHERE
     * @param _maxPriceTicketInSphere: maximum price of a ticket in SPHERE
     */
    function setMinAndMaxTicketPriceInSphere(uint256 _minPriceTicketInSphere, uint256 _maxPriceTicketInSphere)
        external
        onlyOwner
    {
        require(_minPriceTicketInSphere <= _maxPriceTicketInSphere, "minPrice must be < maxPrice");

        minPriceTicketInSphere = _minPriceTicketInSphere;
        maxPriceTicketInSphere = _maxPriceTicketInSphere;
    }
     ...
     * @param _priceTicket price of a ticket (in SPHERE)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].spherePerBracket[_bracket];
        } else {
            return 0;
        }
    }
