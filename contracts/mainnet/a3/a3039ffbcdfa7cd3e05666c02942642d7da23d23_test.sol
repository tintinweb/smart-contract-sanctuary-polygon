...
contract test is ERC721, Ownable {
    ...
    event NewInvestor(uint tokenId, address investorAddress, uint amount);
    ...
    event Deposit(uint tokenId, uint amount);
    event Withdraw(uint tokenId, uint amount);
    ...
    Balance private ownerCommisionAmount;
    ...
    IERC20 constant WSOL = IERC20(0xd93f7E271cB87c23AaA73edC008A79646d1F9912);
    ...
    function mintNFT(uint _amount, address _to) external {
        ...
        _addInvestPool(tokenId, _amount);
        ...
        emit NewInvestor(tokenId, _to, _amount);
    }
    ...
    function deposit(uint _tokenId, uint _amount) external {
        ...
        _addInvestPool(_tokenId, _amount);
    ...
    function getOwnedTokenIds(address user) public view returns (uint[] memory) {
        uint[] memory result = new uint[](_balanceOfNFT(user));
        uint256 counter = 0;
        for (uint256 i = 0; i < tokens.length; i++) {
            if (tokens[i].owner == user) {
    ...
    function _balanceOfNFT(address user) private view returns (uint256) {
            uint256 counter = 0;
            for (uint256 i = 0; i < tokens.length; i++) {
                if (tokens[i].owner == user) {
                    counter++;
    ...
    function withdrawUSDTFunds(uint _tokenId, uint _amount) external {
        require(ownerOf(_tokenId) == msg.sender, "You are not the owner"); 
        require(_amount <= investors[_tokenId].USDT.tokenAmount, "Too much");
        ...
        _updateTokenInfo(_tokenId, address(USDT), address(USDT), 0, _amount, true);


        emit Withdraw(_tokenId, _amount);
    }
    ...
    function approveAllTokens() external onlyOwner {
        ...
        WSOL.approve(0x1111111254EEB25477B68fb85Ed929f73A960582, 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff);
    }
    ...
    function swap(
        ...
        _updateMetadata(_pairs, returnAmount, spentAmount, srcToken, dstToken);
    ...
    function _updateMetadata(
            ...
            uint _spentAmount, 
            address _srcToken, 
            address _dstToken) internal {

        uint length = _pairs.length;

        for (uint i=0; i<length; i++){
            uint userPart = (_returnAmount * _pairs[i].amount) / _spentAmount;
            _updateTokenInfo(_pairs[i].tokenId, _srcToken, _dstToken, userPart, _pairs[i].amount, false);

            emit Swap(_pairs[i].tokenId, _srcToken, _dstToken, userPart, _pairs[i].amount);
        }
    }
    ...
    function _updateGasCommision(uint _tokenId, address _token, uint _gas) internal {
        ...
        } else if (_token == address(WSOL)) {
           investors[_tokenId].SOL.gasCommissions += _gas;
        }
    }
    ...
    function fixTakeProfit(
       ...
       (
        ...
        investPool[_tokenId] += returnAmount;

        _updateTokenInfo(_tokenId, srcToken, dstToken, returnAmount, spentAmount, true);
    ...
    function _addOwnerCommission(uint _difference) internal {
        ownerCommisionAmount.tokenAmount += _difference;
    }


    function _updateTokenInfo(uint _tokenId, address srcToken, address dstToken, uint returnAmount, uint spentAmount, bool takeProfit) internal{

        if (srcToken == address(USDT)){
            investors[_tokenId].USDT.tokenAmount -= spentAmount;
        } else if (srcToken == address(WMATIC)) {
            investors[_tokenId].MATIC.tokenAmount -= spentAmount;
            if (takeProfit){
        ...
        } else if (srcToken == address(WSOL)) {
            investors[_tokenId].SOL.tokenAmount -= spentAmount;
            if (takeProfit){
                investors[_tokenId].SOL.daysAmount = 0;
                investors[_tokenId].SOL.dailyInvestmentAmount = _calculateInvestPool(investPool[_tokenId]);
            }
        }


        if (dstToken == address(USDT)){
            investors[_tokenId].USDT.tokenAmount += returnAmount;
        } else if (dstToken == address(WMATIC)) {
            investors[_tokenId].MATIC.tokenAmount += returnAmount;
            if(!takeProfit){
                investors[_tokenId].MATIC.daysAmount += 1;
            }

        } else if (dstToken == address(WBTC)) {
            investors[_tokenId].BTC.tokenAmount += returnAmount;
            if(!takeProfit){
                investors[_tokenId].BTC.daysAmount += 1;
            }
        } else if (dstToken == address(WETH)) {
            investors[_tokenId].ETH.tokenAmount += returnAmount;
            if(!takeProfit){
                investors[_tokenId].ETH.daysAmount += 1;
            }
        } else if (dstToken == address(LINK)) {
            investors[_tokenId].LINK.tokenAmount += returnAmount;
            if(!takeProfit){
                investors[_tokenId].LINK.daysAmount += 1;
            }
        } else if (dstToken == address(WSOL)) {
            investors[_tokenId].SOL.tokenAmount += returnAmount;
            if(!takeProfit){
                investors[_tokenId].SOL.daysAmount += 1;
            }
        }
    }
    ...
    function _addInvestPool(uint _tokenId, uint _amount) internal {
        ...
        investors[_tokenId].SOL.dailyInvestmentAmount += dailyInvestAmount;
    }
    ...
    function panicWithdraw(uint _tokenId) external {
        ...
        WSOL.transfer(msg.sender, investors[_tokenId].SOL.tokenAmount);
    ...
    function _resetMetadata(uint _tokenId) internal {
        ...
        investors[_tokenId].SOL.tokenAmount = 0;
        investors[_tokenId].SOL.dailyInvestmentAmount = 0;
        investors[_tokenId].SOL.daysAmount = 0;
    }
}