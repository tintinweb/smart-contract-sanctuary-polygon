...
interface IReward {
     ...
     */
    function setReward(uint newReward) external;
...
// File contracts/Reward/TradeReward.sol
 ...
 * @title Reward V2
 */
contract TradeReward is IReward, AccessControl {
     ...
     * Emits {NewReward} event
     */
    function setReward(uint newReward) external onlyRole(OWNER) {
    ...
    function claimReward(address lender) external onlyRole(REWARD_MANAGER) {
        ...
        _token.safeTransfer(lender, totalReward);
     ...
     * Emits {NewReward} event
     */
    function _setReward(uint newReward) private {
        if (currentRound > 0) {
        ...
        uint oldReward = round[currentRound].apy;
    ...
    function _calculateReward(
        ...
        uint oneYear = (365 days);
        return ((((end - start) * reward * amount) * 1E7) / oneYear);
    }
