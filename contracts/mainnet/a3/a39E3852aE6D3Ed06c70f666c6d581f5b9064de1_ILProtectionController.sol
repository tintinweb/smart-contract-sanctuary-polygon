...
contract ILProtectionController is ILProtectionControllerInterface, BaseController {
  ...
  function closeProtections(uint256[] memory _protectionsIds) public override {
    ...
    for (uint256 i; i < _protectionsIds.length; i++) {
      ...
      if (protectionDetails.protectionEndTimestamp <= block.timestamp) {
        ...
        (uint16 impermanentLoss, bool isBelowMin) = calculateIL(
          protectionWithMetadata.token1EntryPriceUSD,
          protectionWithMetadata.token2EntryPriceUSD,
          token1Price,
          token2Price
        );

        if (!isBelowMin) {
          amountToBePaid = calcAmountToBePaid(
            impermanentLoss,
            protectionDetails.lpTokensWorthAtBuyTimeUSD,
            protectionWithMetadata.token1EntryPriceUSD,
            protectionWithMetadata.token2EntryPriceUSD,
            token1Price,
            token2Price
          ...
          if (amountToBePaid > protectionWithMetadata.maxAmountToBePaid) {
            amountToBePaid = protectionWithMetadata.maxAmountToBePaid;
          }
  ...
  function checkUpkeep(
    ...
    if (finalizedProtectionsIds.length > 0) {
      upkeepNeeded = true;
      performData = abi.encode(finalizedProtectionsIds);
    } else {
      upkeepNeeded = false;
    }
  }
  ...
  function calcAmountToBePaidWithProtectionId(uint256 _protectionId) external view override returns (uint256) {
    require(openProtectionsWithMetadata[_protectionId].exists, "Not an open protection or doesn't exist");
    ...
    return
      calcAmountToBePaid(
        impermanentLoss,
        protection.lpTokensWorthAtBuyTimeUSD,
        protectionWithMetadata.token1EntryPriceUSD,
        protectionWithMetadata.token2EntryPriceUSD,
        token1Price,
        token2Price
  ...
  function calcPolicyPeriodEnd(uint256 _policyPeriod) internal view returns (uint256) {
    return block.timestamp + _policyPeriod * 24 * 60 * 60;
  }
  ...
  function calculateIL(
  ...
  ) internal view returns (uint16 impermanentLoss, bool isBelowMin) {
    impermanentLoss = ILUtils.calculateIL(
    ...
    if (impermanentLoss < protectionConfig.minILProtectedPercentage()) {
      isBelowMin = true;
    } else {
      isBelowMin = false;
    }
