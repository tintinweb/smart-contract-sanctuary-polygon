...
contract BondingCurveOffering is BondingCurve {
    ...
    modifier validBuy(uint256 _amount) {
        ...
        uint256 _aftertax_amount = aftertax(_amount);
        uint256 _tokens = calculateBuyAmount(_aftertax_amount);
    ...
    function Purchase(uint256 _amount) public validBuy(_amount) {
        ...
        uint256 _aftertax_amount = aftertax(_amount);
        uint256 _tax = tax(_amount);
        uint256 _tokens = _purchase(_aftertax_amount);
        token.transfer(msg.sender, _tokens);
        usdt.transfer(wallet, _tax);
    }
    ...
    function aftertax(uint256 _amount)
        public
        view
    ...
    function tax(uint256 _amount) public view returns (uint256 _tax) {
        return _amount * rate - aftertax(_amount);
    }
}
