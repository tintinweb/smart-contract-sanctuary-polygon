...
contract NFTMarketplace is Ownable, ReentrancyGuard {
    struct Auction {
        ...
        uint256 startPrice;
        uint256 endBlock;
        uint256 bidExtensionBlock;  // 新增：出价后的拍卖延时
    ...
    event AuctionCreated(uint256 id, address seller, uint256 tokenId, uint256 startPrice, uint256 endBlock, uint256 bidExtensionBlock, uint256 feePercentage);
    ...
    // 新增：批量创建拍卖
    function createAuctions(uint256[] calldata tokenIds, uint256 startPrice, uint256 endBlock, uint256 bidExtensionBlock, uint256 feePercentage) external {
        for (uint i = 0; i < tokenIds.length; i++) {
            createAuction(tokenIds[i], startPrice, endBlock, bidExtensionBlock, feePercentage);
        }
    }

    function createAuction(uint256 tokenId, uint256 startPrice, uint256 endBlock, uint256 bidExtensionBlock, uint256 feePercentage) public {
        ...
        auctions.push(Auction({
            ...
            endBlock: endBlock,
            bidExtensionBlock: bidExtensionBlock,  // 新增：出价后的拍卖延时
        ...
        emit AuctionCreated(_nextAuctionId, msg.sender, tokenId, startPrice, endBlock, bidExtensionBlock, feePercentage);
    ...
    function endAuction(uint256 id) external nonReentrant onlyOwner {
        ...
        if (auction.highestBid >= auction.startPrice) {
            ...
            // 使用 'transferFrom' 安全地将 NFT 从卖家转移到买家
            ...
            // 如果没有人出价，那么就退回 NFT 给卖家
            token.safeTransferFrom(auction.seller, auction.seller, auction.tokenId);
