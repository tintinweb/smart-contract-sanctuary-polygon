...
contract LotteryPool {
    ...
    function addLotteryInductees(uint256 _lotteryId, address[] memory _inductees) public notOverLottery(_lotteryId) onlyOperator {
    ...
    function drawALottery(uint256 _lotteryId) public notOverLottery(_lotteryId) onlyOperator {
    ...
    function drawALotteryByIndex(uint256 _lotteryId, uint256 _index) public notOverLottery(_lotteryId) onlyOperator {
        lotteries[_lotteryId].over = true;
        if (lotteries[_lotteryId].fixedNum.length > 0) {
    ...
    function _drawALotteryByIndex(uint256 _lotteryId, uint256 _index, bool isFixNum) internal {
        ...
        if (isFixNum) {
            ...
            lotteryNum = remainLotteryInductees[_lotteryId].length.mul(lotteries[_lotteryId].proportionNum[_index]).div(100);
            if (lotteryNum == 0) {
                lotteryNum = 1;
            }
        }
