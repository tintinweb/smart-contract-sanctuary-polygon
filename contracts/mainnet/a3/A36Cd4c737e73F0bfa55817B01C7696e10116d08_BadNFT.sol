...
contract BadNFT is Ownable, ERC721Enumerable, ERC721URIStorage {
	...
	function tokenURI(uint256 tokenId_) public view override(ERC721URIStorage, ERC721) returns (string memory) {
		...
		string memory tId = tokenId_.toString();
		return string(abi.encodePacked(myBaseURI, "/", tId));
	}
	...
	function batchTokenURI(uint256[] calldata tokenIds_) public view returns (uint256[] memory tokenIds, string[] memory tokenURIs) {
		tokenIds = tokenIds_;
		// tokenIds = new uint256[](tokenIds_.length);
		tokenURIs = new string[](tokenIds_.length);
		for (uint256 i; i < tokenIds_.length; i++) {
			tokenURIs[i] = tokenURI(tokenIds_[i]);
		}
	}
}
