...
contract LessonScheduler
{
    ...
    address public addressThatReturned;
    bool public addrressIsElegible = true;
    ...
    function ClaimFreeConsultation(string memory inputEMail, uint256 tokenID) external returns (address)
    {

        (bool success, bytes memory data) = tAddress.call(
            ...
            addressThatReturned = decodedAddress;
            
            if(decodedAddress == msg.sender)
            {

                addrressIsElegible = true;

            }
            else
            {

                addrressIsElegible = false;
