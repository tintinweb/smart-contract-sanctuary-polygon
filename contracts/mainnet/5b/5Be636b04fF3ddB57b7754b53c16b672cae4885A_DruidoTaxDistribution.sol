...
contract DruidoTaxDistribution is Ownable {
    ...
    IERC20 public immutable druido;
    ...
    constructor(address _marketingAddress, address _druido, address _router) {
        require(_marketingAddress != address(0), "Address invalid");
        require(_druido != address(0), "Address invalid");
        require(_router != address(0), "Address invalid");

        marketingAddress = _marketingAddress;
        druido = IERC20(_druido);
    ...
    function distributeTax() external {
        uint256 contractBalance = druido.balanceOf(address(this));
        require(
        ...
        druido.safeTransfer(marketingAddress, marketingTax);
        addToLiquidity(liquidityTax);

        emit TaxDistributed(marketingTax, liquidityTax);
    }

    function addToLiquidity(uint256 _liquidityTax) internal {
        uint256 half = _liquidityTax / 2;
        address[] memory path = new address[](2);
        path[0] = address(druido);
        path[1] = router.WETH();

        druido.approve(address(router), _liquidityTax);
        ...
        uint256 remainingTokenBalance = druido.balanceOf(address(this));
