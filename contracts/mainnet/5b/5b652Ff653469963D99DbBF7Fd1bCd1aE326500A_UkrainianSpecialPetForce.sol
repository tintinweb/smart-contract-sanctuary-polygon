...
contract UkrainianSpecialPetForce is ERC721, Ownable {
    uint256 public constant MAX_SUPPLY = 1500;
    uint16 private mintCount = 0;

    uint256 public price = 300000000000000000000;
    string baseTokenURI;
    bool public saleOpen = true;

    event Minted(uint256 totalMinted);

    constructor(string memory baseURI) ERC721("Ukrainian Special Pet Force", "USPF") {
        setBaseURI(baseURI);
    }
    ...
    function mint(address _to,uint16 _count) external payable {
        ...
        if (msg.sender != owner()) {
            ...
            require(
                _count <= 3,
                "Maximum 3 NFTs can be minted per transaction"
            );
            require(
