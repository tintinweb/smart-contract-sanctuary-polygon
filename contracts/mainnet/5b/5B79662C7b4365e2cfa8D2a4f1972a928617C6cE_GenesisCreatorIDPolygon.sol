 ...
 */
contract GenesisCreatorIDPolygon is ERC20Splitter, ERC721A, Pausable {
    ...
    ERC721A(_name, _symbol)
  {
    require(
      _price > 0,
      "GenesisCreatorIDPolygon: Mint price must be greater than zero."
    );
  ...
  /**
   * @notice Updates the mint price of a Genesis Creator ID
   * @param _price uint256
   * @dev Only owner can call it and only when the contract is paused
   * @dev The main Creator ID contract won't have a function to update the mint price, as it will be set during the deployment
   */
  ...
  function mint(address to, uint256 quantity) external whenNotPaused {
    require(
      ...
      "GenesisCreatorIDPolygon Max supply reached."
    );
    require(
      IERC20(token).balanceOf(msg.sender) >= price * quantity,
      "GenesisCreatorIDPolygon Not enough tokens to mint."
    );
    ...
    require(sent, "GenesisCreatorIDPolygon Failed to send tokens.");
  ...
  function mintWithReferrer(
    ...
    require(
      to != referrer,
      "GenesisCreatorIDPolygon Cannot mint with the recipient as the referrer."
    );
  ...
  function burn(uint256 _tokenId) external whenNotPaused {
    require(
      ...
      "GenesisCreatorIDPolygon You do not own this token."
    );
