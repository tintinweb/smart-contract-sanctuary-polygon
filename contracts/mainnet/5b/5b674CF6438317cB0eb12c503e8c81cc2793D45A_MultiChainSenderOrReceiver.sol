...
contract MultiChainSenderOrReceiver is BoringOwnable, IReceiver {
    ...
    // instead of using the more complicated minimal proxy pattern to redeploy a unique contract
    // in order to ensure the same address of adapters accross networks instead of assuming the deployment using create
    // adds a bit of gas
    function addNewAdapter (address adapterMaster) external onlyOwner {
        address proxy = factory.deploy(adapterMaster, bytes(""), true);
        adapter = MultiChainAdapter(proxy);
        emit LogAddNewAdapter(proxy);
    }
...
contract MultiChainAdapter is IAdapter, BoringOwnable, ILayerZeroReceiver {
    ...
    function addConnection(uint256 chainId, uint16 chainIdLZ) external {
