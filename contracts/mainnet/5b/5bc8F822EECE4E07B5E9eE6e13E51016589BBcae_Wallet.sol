...
contract Wallet is AccessControl {
    ...
    // earnDayDecrease[day] = decrease
    mapping(uint256 => uint256) public earnDayDecrease;
    uint256 public currentIndex;
    uint256 public currentEarnDay;
    ...
    function updateTokens() external onlyRole(OPERATE_ROLE) {
        uint256 day = _day();
        uint256 totalSupply = RS.totalSupply();
        require(currentDay <= day, "try tomorrow!");
        for (uint i = currentDay; i < day; ++i) {
            currentEarnDay -= earnDayDecrease[i];
            maxMint += currentEarnDay;
        }
        for (uint i = currentIndex; i < totalSupply; ++i) {
             uint256 tokenId = RS.tokenByIndex(i);
             if (tokenId !=0 && _gets(tokenId) == 0) {
                 IRunbitShoes.MetaData memory meta = RS.tokenMetaData(tokenId);
                 uint256 earnDay = uint256(meta.specialty + meta.comfort + meta.aesthetic) * rate;
                 tokens[tokenId] = earnDay * (meta.durability / 100);
                 currentEarnDay += earnDay;
                 earnDayDecrease[day + (meta.durability / 100)] += earnDay;
             }
        }

        currentEarnDay -= earnDayDecrease[day];
        maxMint += currentEarnDay;
    ...
    function mint(address to, uint256 amount) external onlyRole(OPERATE_ROLE) {
        require(minted <= maxMint, "Exceed max mint");
