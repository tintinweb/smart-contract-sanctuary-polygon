...
contract WPNS is ERC721,ERC721Enumerable,Ownable,VRFConsumerBase {
    ...
    event AddPartner(address account);
    event RemovePartner(address account);
    ...
    event SetLogging(bool creation, bool _attribs, bool _extraAttribs, bool _partnerAttribs);

    /// @notice Struct for returning token data
    struct TokenData {
        uint256 weaponTierLvl;
        uint256 weaponType;
        uint256 weaponSubtype;
    ...
    uint32 constant MAX_EXTRA_ATTRIB_LVL = 4294967295;
    ...
    modifier onlyBurner {
        require(msg.sender == burner, "BRN-00");
        _;
    }
    modifier onlyMinter {
        require(msg.sender == minter, "MNT-00");
        _;
    }
        ...
        onlyMinter
    {
        ...
        require(recipientsLength_ <= maxMintBatch,"CRE-10");
        ...
        require(wpnTTSMintIndex[_tierLvl]+recipientsLength_ <= weaponTiers[_tierLvl-1].ttsMintCap,"CRE-50");
        ...
        for (uint256 i=0; i < recipientsLength_; i++) {
            ...
            randomNumber_ = uint256(keccak256(abi.encodePacked(_rndNumVrf, tokenId_, recipient_)));
    ...
    /// @notice Set partner token attribs
    ...
    /// @notice Get partner token attribs
    ...
    function setPartnerTokenAttribByIndex(uint256 _tokenId, uint256 _index, uint256 _value) external {
    ...
    function setPartnerTokenAttribByIndex_Batch(uint256[] calldata _tokenId, uint256 _index, uint256[] calldata _value) external {
        require(isPartner[msg.sender], "PAR-00");
        require(_index > 0 && _index <= 8, "PAR-02");
        uint256 tokenIdLength_ = _tokenId.length;
    ...
    function setPartnerTokenAttribs(uint256 _tokenId, uint256[8] memory _values) external {
        require(isPartner[msg.sender], "PAR-00");
        require(_tokenId <= maxTokenId,"PAR-01");
        _setPartnerTokenAttribs(msg.sender, _tokenId, _values);
    }

    function setPartnerTokenAttribs_Batch(uint256[] calldata _tokenId, uint256[8][] memory _values) external {
        require(isPartner[msg.sender], "PAR-00");
        uint256 tokenIdLength_ = _tokenId.length;
    ...
    function setLogging(bool _creation, bool _attribs, bool _extraAttribs, bool _partnerAttribs) external onlySuperAdmin {
        emitCreation = _creation;
        emitAttribs = _attribs;
        emitExtraAttribs = _extraAttribs;
        emitPartnerAttribs =  _partnerAttribs;
        emit SetLogging(_creation, _attribs, _extraAttribs, _partnerAttribs);
    }
