...
contract Semantic is ISemantic, AccessControl,ERC721 {
    ...
    function claim(ClaimOperateData[] memory claimOperateDatas) external {
     ...
     function testSign(ClaimOperateData memory operateData) external  view returns(string memory rdf,string memory signOriginalData,address singer){
            require(
                operateData.expireTime > block.timestamp,
                "signData has expired"
            );
            rdf = generateRdf(1,Strings.toHexString(msg.sender),operateData.predicateId,operateData.object);
