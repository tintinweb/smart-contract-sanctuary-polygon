    ...
    ERC20VotesUpgradeable
{
    ...
    function getRewards(
        ...
        int256 accumulatedReward = int256(
            (balanceOf(msg.sender) * accRewardPerShare) / ACC_REWARD_PRECISION
        );
        int256 rewardDebt = user.rewardDebt;
        if (userCleanMapping[msg.sender] != true) {
            rewardDebt = 0;
        }
        uint256 pending = uint256(accumulatedReward - rewardDebt) +
    ...
    function setFarm(
        ...
        farmInfo.accRewardsPerShare = _rewardPerSecond;
    }
