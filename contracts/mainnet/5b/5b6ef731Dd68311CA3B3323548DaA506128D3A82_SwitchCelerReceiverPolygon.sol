...
contract SwitchCelerReceiverPolygon is SwitchPolygon {
    ...
    ) SwitchPolygon(_switchViewAddress, _switchEventAddress, _paraswapProxy, _augustusSwapper)
        public
    {
        ...
        )
    {
        ...
        if (parts == 0) {
            if (IERC20(m.bridgeToken).isETH()) {
                payable(m.recipient).transfer(srcAmount);
                _emitCrosschainSwapDone(m, srcAmount, 0, DataTypes.SwapStatus.Failed);
            } else {
                IERC20(m.bridgeToken).universalTransfer(m.recipient, srcAmount);
                _emitCrosschainSwapDone(m, srcAmount, 0, DataTypes.SwapStatus.Failed);
            }
        }
