...
// File contracts/Uniswap_V2_TWAMM/core/libraries/SafeMath.sol
...
library LongTermOrdersLib {
    ...
    function executeVirtualOrdersUntilBlock(LongTermOrders storage longTermOrders, uint256 blockTimestamp, ExecuteVirtualOrdersResult memory reserveResult) internal {
        ...
        while (nextExpiryBlockTimestamp < blockTimestamp) {
            ...
            if (orderPool0.salesRateEndingPerBlock[nextExpiryBlockTimestamp] > 0
                ...
                uint256 blockTimestampEllapsed = nextExpiryBlockTimestamp - longTermOrders.lastVirtualOrderTimestamp;
    ...
    function executeVirtualOrdersUntilBlockView(LongTermOrders storage longTermOrders, uint256 blockTimestamp, ExecuteVirtualOrdersResult memory reserveResult) internal view {
        uint256 nextExpiryBlockTimestamp = longTermOrders.lastVirtualOrderTimestamp - (longTermOrders.lastVirtualOrderTimestamp % longTermOrders.orderTimeInterval) + longTermOrders.orderTimeInterval;
        //iterate through time intervals eligible for order expiries, moving state forward
        ...
        while (nextExpiryBlockTimestamp < blockTimestamp) {
            ...
            if (orderPool0.salesRateEndingPerBlock[nextExpiryBlockTimestamp] > 0
                ...
                //amount sold from virtual trades
                uint256 blockTimestampEllapsed = nextExpiryBlockTimestamp - longTermOrders.lastVirtualOrderTimestamp;
                uint256 token0SellAmount = currentSalesRate0 * blockTimestampEllapsed / SELL_RATE_ADDITIONAL_PRECISION;
                uint256 token1SellAmount = currentSalesRate1 * blockTimestampEllapsed / SELL_RATE_ADDITIONAL_PRECISION;

                (uint256 token0Out, uint256 token1Out) = executeVirtualTradesAndOrderExpiries(reserveResult, token0SellAmount, token1SellAmount);
                currentSalesRate0 -= orderPool0.salesRateEndingPerBlock[nextExpiryBlockTimestamp];
                currentSalesRate1 -= orderPool1.salesRateEndingPerBlock[nextExpiryBlockTimestamp];
            }
            nextExpiryBlockTimestamp += longTermOrders.orderTimeInterval;
        }
        //finally, move state to current blockTimestamp if necessary
...
contract UniV2TWAMMPair is IUniswapV2PairPartialV5, UniV2TWAMMERC20 {
    ...
    function withdrawProceedsFromLongTermSwap(uint256 orderId) external lock execVirtualOrders {
    ...
    function getReserveAfterTwamm(uint256 blockTimestamp) public view returns (
        ...
        longTermOrders.executeVirtualOrdersUntilBlockView(blockTimestamp, result);
