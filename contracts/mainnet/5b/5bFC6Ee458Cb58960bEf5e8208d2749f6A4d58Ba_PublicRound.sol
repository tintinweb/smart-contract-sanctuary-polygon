...
contract PublicRound is AccessControl, VestingAbstract {
    ...
    modifier isFinish(Status status) {
        require(status == Status.Sale, "The sale is completed");
        _;
    }
     ...
     */
    function purchase(uint128 amountUSD) external isFinish(_infoRound.status) {
        address msgSender = msg.sender;
        IERC20(_infoRound.acceptedToken).safeTransferFrom(
            msgSender,
            address(this),
            amountUSD
        );
        ...
        require(
            _infoRound.amountPurchased <= _infoRound.tokenDistribution,
            "Maximum tokens sold"
        );
    ...
    function finishSale() external onlyRole(ADMIN_ROLE) {
        address msgSender = msg.sender;
        uint256 balance = IERC20(_infoRound.acceptedToken).balanceOf(
            address(this)
        );
        IERC20(_infoRound.acceptedToken).safeTransfer(msgSender, balance);

        IERC20(_basicInfo.token).safeTransfer(
            msgSender,
            _infoRound.tokenDistribution - _infoRound.amountPurchased
        );
        _infoRound.status = Status.Finish;
    ...
    /**
     * @notice - Returns information about the round
     * @return - structure InfoRound
     */
    function getInfoRound() external view returns (InfoRound memory) {
        return _infoRound;
    }
