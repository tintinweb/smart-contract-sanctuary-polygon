...
contract AxoseaRoyalties {
    using SafeERC20 for IERC20;

    address public xlt = 0x19072f56Af0a815d62B1B4651eECC82200eb922B;
    uint public immutable devCut;
    IUniswapV2Pair public pair = IUniswapV2Pair(0xa129ffC5b6C32f94FbaBBF89c85c4226A927Ae7C);
    address public wmatic = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address public devAddr = 0x000004380841636dAB6f0988F496683A915d5EF7;
    ...
    function distribute(address stuckToken) public onlyEOA{
        ...
        if (stuckToken != address(0)) {
            require(stuckToken != wmatic, "distribute, cant directly withdraw wmatic");
    ...
    function _distribute() internal {   
        uint maticBal = address(this).balance;

        if (maticBal > 0) {
            IWMATIC(wmatic).deposit{value: maticBal}();
        }

        uint wmaticBal = IERC20(wmatic).balanceOf(address(this));

        if(wmaticBal > 0){
            IERC20(wmatic).transfer(devAddr, wmaticBal * devCut / 10000); // send devcut
            wmaticBal = IERC20(wmatic).balanceOf(address(this));

            _swap(wmatic, wmaticBal, xlt);
        }
    }
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        ...
        require(reserveIn > 0 && reserveOut > 0, 'Summoning: INSUFFICIENT_LIQUIDITY');
    ...
    function safeTransferMATIC(address to, uint value) private {
