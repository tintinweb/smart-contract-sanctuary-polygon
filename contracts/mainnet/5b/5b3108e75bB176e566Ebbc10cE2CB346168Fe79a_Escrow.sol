...
contract Escrow is AccessControl, IERC721Receiver {
    ...
    uint8 constant PRE_SALE_DAYS = 2; // In days ; DEFAULT MUST BE IN 15 DAYS
    ...
    uint256 public AUCTION_BASE_PRICE = 3 * 10 ** 6; // 10k USDT (since USDT has 6 decimals)
    ...
    modifier whilePreSale() {
        require(
            actualStage == Stages.PreSale && !preSaleEnd(),
            "Pre-Sale period is not available"
        );
        _;
    }
    ...
    modifier documentExists(uint256 _id) {
        require(
            _id < documents[actualStage].length && _id >= 0,
    ...
    modifier onlyAdminOrNotary() {
        require(
            hasRole(ADMIN_ROLE, _msgSender()) || isNotary(),
            "Only admin or notary can do this"
        );
        _;
    }
    ...
    function addDocument(string calldata _hash, bool _visible) public {
        ...
        if (actualStage == Stages.Building) {
            ...
            documents[actualStage].push(
                Document(_hash, DocumentStatus.Valid, _visible, Stages.Building)
            );
        } else if (actualStage == Stages.Compliance) {
            require(buyer == _msgSender(), "Only winner allowed");
            require(
                auction.auctionPaidAndNFTApproved(),
                "Auction must be paid and NFT approved to burn"
            );
            _visible = false;
        ...
        emit DocumentUploaded(
            uint8(documents[actualStage].length - 1),
            _hash,
            _visible,
            status,
            actualStage
    ...
    function setFees(
        uint256 _taxesFee,
        uint256 _techFee
    ) public onlyAdminOrNotary {
        require(_taxesFee > 0 && _techFee > 0, "Taxes must be greater than 0");
        require(
            actualStage == Stages.Building,
            "Fees can only be modified in Building stage"
        );
        TAXES_FEE = _taxesFee;
        TECH_FEE = _techFee;
    ...
    function startPreSale() public onlyAdminOrNotary {
        ...
        require(
            actualStage == Stages.Building,
            "Pre-sale can only be started in Building stage"
        );
    ...
    function _dispatchPreSale() internal {
        endPreSaleTime = block.timestamp + PRE_SALE_DAYS * 1 days; // ---> !!! ESTO VA EN DIAS... SE CAMBIÓ A HORAS PARA TESTEAR !!! <--
    ...
    function addToWhitelist(
        address _user
    ) public whilePreSale onlyAdminOrNotary {
    ...
    function onERC721Received(
        ...
        require(
            ...
            "Operator only can be this contract"
        );

        return IERC721Receiver.onERC721Received.selector;
    }

    function transferNFTOwnershipToAdmin(
        address _to
    ...
    function _dispatchSale() internal {
        ...
        require(
            auction.addListToWhitelist(whitelist.values()),
            "There was a problem sending whitelist to Auction contract"
        );
    ...
    function setSaleEnds(address _winner) public returns (bool success) {
        ...
        require(
            _msgSender() == address(auction),
            "Only Auction can call this function"
        );
    ...
    function setComplianceEnds() public onlyNotary returns (bool success) {
        require(
            actualStage == Stages.Compliance,
            "Compliance Stage must be actual stage"
        );
    ...
    // Transfering Stage
    function setTransferingEnds(
        uint256 _transferResultOption
        ...
        if (_transferResultOption == 1) {
            // If this option was received then continue with normal flow
            require(
                actualStage == Stages.Transfering,
                "Transfering Stage must be actual stage"
            );
            require(
                documents[actualStage].length > 0,
                "No deed documents uploaded, can't continue"
            );
            nft.safeTransferFrom(address(this), buyer, finalTokenId);
            setNextStage(Stages.Done);
            success = true;
        } else if (_transferResultOption == 2) {
            // Notary cancelled
            require(
                actualStage == Stages.Compliance ||
                    actualStage == Stages.Transfering,
                "Compliance OR Transfering Stage must be actual stage"
            );
            setNextStage(Stages.NotaryCancelled);
            success = true;
        } else if (_transferResultOption == 3) {
            // Winner cancelled
            require(
                actualStage == Stages.Compliance ||
                    actualStage == Stages.Transfering,
