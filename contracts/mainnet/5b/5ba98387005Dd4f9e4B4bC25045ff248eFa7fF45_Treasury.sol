...
contract Treasury is Ownable {
    ...
    address public BANG;
    uint public BANGQueue; // Delays change to BANG address
    ...
    /**
        @notice allow approved address to deposit an asset for BIG
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint BIG needed and store amount of rewards for distribution

        send_ = value.sub( _profit );
        IERC20Mintable( BIG ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn BIG for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IBIGERC20( BIG ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( BANG ).balanceOf( msg.sender ); // Can only borrow against BANG held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with BIG
        @param _amount uint
     */
    function repayDebtWithBIG( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IBIGERC20( BIG ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, BIG, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( BIG ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( BIG ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns BIG valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match BIG decimals
            value_ = _amount
                .mul(10**IERC20(BIG).decimals())
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.BANG ) { // 9
            BANGQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
