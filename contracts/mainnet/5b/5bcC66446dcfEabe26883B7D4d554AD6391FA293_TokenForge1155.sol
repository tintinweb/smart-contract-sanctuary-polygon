...
contract TokenForge1155 is ERC1155Tradable {
    ...
    constructor()
        ERC1155Tradable("TokenForge", "TOKENFORGE1", "https://accessories.token-forge.io/nft/{id}")
    {
    ...
    modifier whenMintingNotPaused() {
        require(!mintingPaused(), "TokenForge1155: paused");
        _;
    }
    ...
    modifier whenMintingPaused() {
        require(mintingPaused(), "TokenForge1155: not paused");
        _;
    }
    ...
    function create(
        ...
        string memory _uri,
        bytes memory _data
    ) public onlyRole(MINTER_ROLE) whenMintingNotPaused returns (uint256) {
    ...
    function mint(
        ...
        uint256 _quantity,
        bytes memory _data
    ) public virtual override whenMintingNotPaused creatorOnly(_id) {
     ...
     * - The contract must not be paused.
     */
    function pauseMinting() public whenMintingNotPaused {
        require(hasRole(PAUSER_ROLE, _msgSender()), "TokenForge1155: must have pauser role to unpause");
     ...
     * - The contract must be paused.
     */
    function unpauseMinting() public whenMintingPaused {
        require(hasRole(PAUSER_ROLE, _msgSender()), "TokenForge1155: must have pauser role to unpause");
 ...
 */
contract ERC1155Tradable is ContextMixin, ERC1155, NativeMetaTransaction, Ownable, AccessControlEnumerable {
