...
contract SNAKE is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public PRICE = 13 * 10**18; //13 matic
    ...
    ) ERC721A("Snake Club", "SNAKE", _maxBatchSize, _collectionSize) {
        baseURI = initBaseURI;
        _signer = signer;
    ...
    function adminMint(address to) external nonReentrant onlyOwner {
        require(
            totalSupply() + 1 + reserveAmount - tokensReserved <=
                collectionSize,
            "Max supply exceeded."
    ...
    function withdraw(address to_address) external nonReentrant onlyOwner {
        uint256 balance = address(this).balance;
        (bool success1, ) = payable(to_address)
            .call{value: balance}("");
