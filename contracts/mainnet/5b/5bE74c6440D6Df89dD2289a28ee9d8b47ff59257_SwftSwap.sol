...
contract SwftSwap is ReentrancyGuard, Ownable, EIP712 {
    ...
    struct Order {
        string partnerNO; // 三方渠道编号
        address fromToken; // 源币地址
        uint256 destination; // 目标链chainId
        address receiptAddress; // 目标链接收地址
        address toToken; // 目标币地址
        uint256 depositAmount; // 存入数量
        uint256 minReturnAmount; // 最小接收数量
        address _dex; // 兑换dex合约地址
        bytes _data; // dex合约的calldata
    }
    ...
    event SwapTokenToToken(
        ...
        address toToken, // 目标币地址
        uint256 minReturnAmount, // 最小接收数量
        address receiptAddress, // 接收地址
        address _dex, // dex合约地址
        uint256 returnAmount // dex合约执行calldata
    );
    ...
    function swapTokenToToken(Order memory order, bytes memory _sign) external {
        require(
            agent == _hashTypedDataV4(_hashOrder(order)).recover(_sign),
        ...
        uint256  baseFee ;
        if(fee[order.partnerNO] == 0){
            baseFee = 2_000_000_000_000_000;
        }else{
            baseFee = fee[order.partnerNO];
        }
        uint256 feeAmount = _inputAmount.mul(baseFee).div(10**18);
        ...
        payable
    {
        ...
        require(
            msg.value >= order.depositAmount && msg.value > 0,
            "TOKEN_AMOUNT_MUST_NOT_SMALL_THAN_DEPOSITAMOUNT"
        );
        uint256  baseFee ;
        if(fee[order.partnerNO] == 0){
        ...
        uint256 feeAmount = msg.value.mul(baseFee).div(10**18);
    ...
    function withdraw(
        ...
        require(_dex != address(0), "DEX_CAN_NOT_BE_0");
        require(dexWhiteList[_dex] == true, "DEX_NOT_IN_WHITE_LIST");
        require(toToken != usdtAddress, "TOKEN_CAN_NOT_BE_USDT_ADDRESS");
        uint256 usdtBalance = IERC20(usdtAddress).balanceOf(address(this));
        ...
        (bool success, ) = _dex.call(data);
        require(success, "DEX_SWAP_FAILED");
        uint256 _inputToken = IERC20(toToken).balanceOf(address(this)).sub(
            _tokenBalanceOrigin
        );
