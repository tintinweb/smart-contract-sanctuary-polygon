...
contract Force1_Qore_USDT is owned
{
    ...
    function subCore_(address msgsender, address _refAddress,bool pay, uint prc) internal returns(bool)
    {
        ...
        (uint user3rdParent, ) = getPosition(msgsender, 1);
        require(user3rdParent<6, "no place under this referrer");
        ...
        uint userPosition;
        (userPosition, user3rdParent) = getPosition(msgsender, 1);
        (,treeComplete) = findFreeParentInDown(user3rdParent, 1);
        ...
        if(treeComplete)
        {
            recyclePosition(user3rdParent,1, pay );
        }
    ...
    function findFreeParentInDown(uint  refID_ , uint _level) public view returns(uint parentID, bool noFreeReferrer)
    {
        ...
        address[6] memory childss;
        uint[6] memory parenT;
    ...
    function buyCore_(address msgsender, uint _level, bool pay,  uint prc) internal returns(bool)
    {
        ...
        (userPosition, user3rdParent) = getPosition(msgsender, _level);
        (,treeComplete) = findFreeParentInDown(user3rdParent, _level); 
        ...
        {           
            ...
            recyclePosition(user3rdParent, _level, pay);
   ...
   function splidStack_New(address msgsender, uint userPosition, uint prc, uint tempPosition, uint _referrerID, uint _level) internal returns(bool)
    ...
    event debugEv(address _user, bool treeComplete,uint user3rdParent,uint _level,uint userPosition);
