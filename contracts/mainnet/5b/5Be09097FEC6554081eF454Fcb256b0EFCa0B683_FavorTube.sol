...
contract FavorTube is IERC223Recipient, TokenTax, Ownable, LockBalance, ReentrancyGuard {
    ...
    event $subscribe(address owner, address licensee, uint32 expire);
    event $setUserConfig(uint256 price, uint mode);
    ...
    constructor(uint8 taxRate, uint256 price, uint32 subscribeBlock_, address ERC223Token, uint interval) TokenTax(taxRate) LockBalance(interval) {
    ...
    function tokenReceived(address _from, uint _value, bytes memory _data) external override {
        ...
        (owner) = decodeCallbackData(_data);

        UserConfig memory config = userConfig(owner);
        ...
        address lockAccount = owner;

        if (config.mode == 1) lockAccount = _from;

        _lock(lockAccount, remainValue);

        uint32 expire = computeExpire(subscribeBlock);

        emit $subscribe(owner, _from, expire);
    }
    ...
    function decodeCallbackData(bytes memory data) internal pure returns (address _owner) {
    ...
    function setTaxRate(uint8 newRate) external override onlyOwner returns (bool success) {
        return _setTaxRate(newRate);
    }
...
abstract contract TokenTax {
    ...
    function _setTaxRate(uint8 newRate) internal returns (bool success) {
    ...
    function setTaxRate(uint8 newRate) external virtual returns (bool success);
...
abstract contract LockBalance {
    ...
    mapping(address => uint) lockTotal;
    ...
    function _lock(address addr, uint amount) internal {
        ...
        if (data.current.round == n) {
            data.current.amount += amount;
        } else {
            if (data.current.round + 1 == n) {
                data.last = data.current;
            } else {
                data.last = Data(n - 1, 0);
            }
            data.current.round = n;
            data.current.amount = amount;
        }
