...
/**
 * @title The PalletLocker contract acts as a holding contract for GET/xGET deposited as collatearl for inventoryPallets.
...
import "./PalletLocker.sol";
import "./interfaces/IPalletLockerFactory.sol";

/**
 * @title The PalletLockerFactory contract deploys palletLockerrs for EFF Actors. 
 * @author https://github.com/kasper-keunen
 */
contract PalletLockerFactory is IPalletLockerFactory {
    mapping(address => address) public accountToPalletLocker;
    mapping(address => bool) public isPalletLocker;
    address public immutable chamberOfCommerce;
    ...
    modifier isChamberOfCommerce() {
        require(
            msg.sender == chamberOfCommerce, 
            "PalletLockerFactory:Caller not ChamberOfCommerce"
        );
        _;
    }

    /**
     * Deploys a PalletLocker for a certain safeAddress 
     * @param _safeAddressAccount the gnosis safe address of the actro
     */
    function newPalletLocker(
        address _safeAddressAccount
    ) external isChamberOfCommerce returns(address palletLocker_) {
        palletLocker_ = address(
            new PalletLocker(
                _safeAddressAccount,
                msg.sender // chamberOfCommerce
        ));
        accountToPalletLocker[_safeAddressAccount] = palletLocker_;
        isPalletLocker[palletLocker_] = true;
        emit PalletLockerCreated(
            _safeAddressAccount, 
            palletLocker_
        );
    }
