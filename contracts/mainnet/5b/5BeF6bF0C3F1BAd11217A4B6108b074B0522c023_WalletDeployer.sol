...
contract WalletDeployer is ModuleProxyFactory {
    ...
    address public immutable authorities;
    ...
    function deployTrustedTxModifierAndSafe(uint256 _saltNonce)
        public
        returns (address trustedTxModifier, address payable safe)
    {
        safe = payable(safeFactory.createProxy(safeSingleton, hex"00"));

        trustedTxModifier = deployTrustedTxModifier(safe, _saltNonce);
    ...
    function deployAndSetupSafesMultiple(uint256 numSafes, uint256 _saltNonce)
        external
    {
        for (uint256 i = 0; i < numSafes; i++) {
            deployTrustedTxModifierAndSafe(_saltNonce);
        }
    }

    function deployTrustedTxModifier(address _safe, uint256 _saltNonce)
        public
        returns (address)
    {
