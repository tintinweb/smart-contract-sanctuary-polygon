...
import "./IQuickLPMining.sol";
import "./rStrategy.sol";
import "./IrOracle.sol";
import "./IDQuick.sol";
import "./IQUICKMigration.sol";

contract rCompoundLPERC20QuicSwap02Strategy is rStrategy {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    //  -------------------- CONTRACT VARIABLES --------------------
    // Tokens and Contracts to Interface with
    IQuickLPMining internal _stakingContract; // The staking contract that the vault will compound into
    ...
    address internal _dquickAddress;
    ...
    address internal _dquickAddr = 0xf28164A485B0B2C90639E47b0f377b4a438a16B1;
    address internal quickmigration = 0x333068D06563a8DfDBF330A0e04A9d128e98bf5a;
    address internal OldQuick = 0x831753DD7087CaC61aB5644b308642cc1c33Dc13;
        ...
        )
    {
        ...
        _dquickAddress = _dquickAddr;
        _stakingContract = IQuickLPMining(_stakingContractAddr);
        _stakingToken = IUniswapV2Pair(_stakingTokenAddr);
        _pairedToken = IERC20(_stakingToken.token0());
        _stakingLpBaseToken = IERC20(_stakingToken.token1());
        _rewardToken = IERC20(_rewardTokenAddr);
        _rewardLpToken = IUniswapV2Pair(_rewardTokenLpAddr);
        _rewardLpBaseToken = IERC20(_rewardLpToken.token1());

        _router = IUniswapV2Router02(_routerAddr); // quickswap router mainnet polygon 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff
    ...
    function _leave() internal {
        uint _amount = IERC20(_dquickAddress).balanceOf(address(this));
        IDQuick(_dquickAddress).leave(_amount);
        uint _quickToMigrationAmount = IERC20(OldQuick).balanceOf(address(this));
        IQUICKMigration(quickmigration).quickToQuickX(_quickToMigrationAmount);
    }
        ...
        onlyRole(MANAGER_ROLE)
    {
        ...
        if (address(_rewardToken) == address(_pairedToken)) {
            ...
            minAmountBaseToken = _router.getAmountOut(
            ...
            desiredAmountPairedToken = amounts[2].mul(50 * (10**16)).div(
                BIG_UNIT
            );
            delete amounts;
            delete _swapPath;
            ...
            // Get the current price from the exchange
            (reserveA, reserveB, ) = _stakingToken.getReserves(); // Token A is the base token in all of the Flag Pairs and Token B is the _pairedToken
            minAmountBaseToken = _router.getAmountOut(
                desiredAmountPairedToken,
                reserveB,
                reserveA
            ...
            desiredAmountBaseToken = _stakingLpBaseToken.balanceOf(
                address(this)
            );
        }

        {
            ...
            minAmountPairedToken = _router.getAmountOut(
                desiredAmountBaseToken,
                reserveA,
                reserveB
            );
            minAmountBaseToken = _router.quote(
                minAmountPairedToken,
                reserveB,
                reserveA
            );
