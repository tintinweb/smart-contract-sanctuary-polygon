...
contract OPTFund is Ownable, ReentrancyGuard {
    ...
    function getRealOPT3Price() public view returns (uint256) {
        uint256 realOPT3Price = getOPT3Price().div(priceDivisor);
        return realOPT3Price;
    }
    ...
    function getUserOPT3Balance(address _address) public view returns (uint256) {
        return IBEP20(OPT3).balanceOf(_address);
    }

    function depositOPT3(uint256 _amount) external nonReentrant {
        require(depositingOpen = true);
        require(getUserOPT3Balance(msg.sender) >= _amount, "OPT3 Balance smaller than amount");

        if(depositedBefore[msg.sender] = false) {
            uint256 newDepositID = totalDeposits + 1;
            ...
            stakeStruct memory newStake = stakeStruct(newDepositID, depositAfterFees, msg.sender, block.timestamp);
            idToDeposit[newDepositID] = newStake;
            deposits.push(newStake);
    ...
    function buyDiscount(uint256 _USDTAmount) public nonReentrant {
    ...
    function calculateBorrowableAmount(uint256 _coll) public view returns (uint256) {
        uint256 OPTPrice = getRealOPT3Price();
    ...
    function borrowUsingOPT3(uint256 _OPT3Amount) public nonReentrant {
        ...
        uint256 liquidPrice = (getRealOPT3Price().div(100)).mul(60);
        ...
        if(borrowedBefore[msg.sender] = true) {
            ...
            idToLoan[loanID].OPT3PriceAtBorrowing = getRealOPT3Price();
            idToLoan[loanID].liquidationPrice = liquidPrice;
            idToLoan[loanID].paidOff = false;

            IERC20(USDT).transfer(msg.sender, loanAmount);

        }

        if(borrowedBefore[msg.sender] = false) {
            uint256 newLoanID = totalLoans +1;
            ...
            idToLoan[newLoanID].OPT3PriceAtBorrowing = getRealOPT3Price();
    ...
    function repayLoan(uint256 _id) external nonReentrant {
        require(idToLoan[_id].taker == msg.sender);
        require(getRealOPT3Price() > idToLoan[_id].liquidationPrice, 'Loan is currently locked (price too low)');
