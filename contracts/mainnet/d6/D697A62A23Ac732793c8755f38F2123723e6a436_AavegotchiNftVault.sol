...
contract AavegotchiNftVault is OriumNftVault {
    ...
    function createRentalOffer(
        ...
        address _nftAddress,
        bytes memory data
    ) external onlyScholarshipManager {
     ...
     * @param _nftAddress address of the NFT contract
     */
    function cancelRentalOffer(uint256 _tokenId, address _nftAddress)
        external
        onlyScholarshipManager
    {
     ...
     * @param _tokenId uint256 id of the NFT
     */
    function endRental(address _nftAddress, uint32 _tokenId) external onlyScholarshipManager {
        LibAavegotchiNftVault.endRental(_nftAddress, _tokenId, address(factory), address(this));
    }
     ...
     * @param _tokenId uint256 id of the NFT
     */
    function claimTokensOfRental(address _nftAddress, uint256 _tokenId) external onlyOwner {
        LibAavegotchiNftVault.claimTokensOfRental(_nftAddress, _tokenId, address(factory));
    }
     ...
     */
    function withdrawNfts(address[] memory _nftAddresses, uint256[] memory _tokenIds)
        public
        virtual
        override
        onlyOwner
     ...
     * @notice look parent function for more details
     */
    function getNftState(address _nftAddress, uint256 _tokenId)
        public
        view
        virtual
    ...
    {
        uint256 nftTypeId = factory.getPlatformNftType(platform, _nftAddress);
        address _rentalImplementation = factory.rentalImplementationOf(_nftAddress);

        if (nftTypeId == LibAavegotchiNftVault.NFT_TYPE_AAVEGOTCHI) {
            _nftState = LibAavegotchiNftVault.getAavegotchiState(
                _rentalImplementation,
                uint32(_tokenId)
            );
        } else if (nftTypeId == LibAavegotchiNftVault.NFT_TYPE_LAND) {
            _nftState = LibAavegotchiNftVault.getLandState(
                _rentalImplementation,
                uint256(_tokenId)
            );
        } else {
     ...
     * @notice look parent function for more details
     */
    function pauseListing(address _nftAddress, uint256 _tokenId)
        public
        virtual
        override
        onlyOwner
...
contract OriumNftVault is IOriumNftVault, Initializable, OwnableUpgradeable {
    ...
    function _depositNfts(address _nftAddress, uint256 _tokenId)
        internal
        onlyTrustedNFT(_nftAddress)
    {
    ...
    function _withdrawNfts(address _nftAddress, uint256 _tokenId)
        internal
        onlyTrustedNFT(_nftAddress)
    {
        require(
    ...
    /**
     * @notice unpause NFTs from the contract
     * @param _nftAddress address of NFT
     * @param _tokenId token id of NFT
     */
    function unPauseListing(address _nftAddress, uint256 _tokenId)
        external
        virtual
        override
        onlyOwner
    ...
    function _unDelegateScholarshipProgram(address _nftAddress, uint256 _tokenId)
        internal
        onlyTrustedNFT(_nftAddress)
    {
        require(
     ...
     * @param _maxAllowedRentalPeriod is the max allowed period
     */
    function setMaxAllowedRentalPeriod(
        address _nftAddress,
        uint256 _tokenId,
        uint256 _maxAllowedRentalPeriod
    ) external onlyOwner onlyTrustedNFT(_nftAddress) {
        require(
            getNftState(_nftAddress, _tokenId) != NftState.NOT_DEPOSITED,
            "OriumNftVault:: Token is not deposited"
        );
...
interface IOriumAavegotchiSplitter {
    function withdrawNfts(WithdrawVoucher calldata voucher, SignatureData calldata signature)
        external;
}
...
library LibAavegotchiNftVault {
    ...
    function claimTokens(
        ...
        IOriumAavegotchiSplitter(splitter).withdrawNfts(_voucher, _signature);
    }

    function claimTokensOfRental(
        address _nftAddress,
        uint256 _tokenId,
        address _factory
    ) external {
    ...
    function _createLandRentalOffer(
        ...
        (_actionRight, _accessRight, _whitelistId) = abi.decode(data, (uint256, uint256, uint32));
    ...
    function _createLandRental(
        ...
        _validateCreateLandRental(_actionRight, _accessRight, _whitelistId);

        uint256[] memory parcelId = new uint256[](1);
        parcelId[0] = _parcelId;

        uint256[] memory actionRight = new uint256[](1);
        actionRight[0] = _actionRight;

        uint256[] memory accessRight = new uint256[](1);
        accessRight[0] = _accessRight;

        uint32[] memory whitelistId = new uint32[](1);
        whitelistId[0] = _whitelistId;
    ...
    function getValidRevenueSplit(uint256[] memory shares, address _factory)
        public
        view
        returns (uint8[3] memory _revenueSplit)
    {
    ...
    function convertSharesToAavegotchi(uint256[] memory shares)
        public
        pure
        returns (uint8[3] memory _revenueSplit)
    {
    ...
    function recalculateShare(uint256 _share, uint256 _totalSharesWithoutOriumFee)
        public
        pure
        returns (uint256 _newShare)
    {
    ...
    function _isLendingClaimable(uint32 tokenId, address _rentalImplementation)
        private
        view
        returns (bool)
    {
    ...
    function _validateCreateLandRental(
        ...
        require(_accessRight != 0, "LibAavegotchiNftVault:: Wrong access right");
        require(
            _actionRight == 0 || _actionRight == 1,
            "LibAavegotchiNftVault:: Wrong action right"
        );

        if (_accessRight == 2) {
            require(_whitelistId != 0, "LibAavegotchiNftVault:: Whitelist id cannot be 0");
        }
    }

    //Getters
    function getAavegotchiState(address _rentalImplementation, uint32 _tokenId)
        external
        view
        returns (NftState)
    {
    ...
    function getLandState(address _rentalImplementation, uint256 _parcelId)
        external
        view
        returns (NftState)
    {
