...
contract CMBalanceMonitor is KeeperCompatibleInterface, Ownable {
  ...
  event MinGasForTransferUpdated(uint256 oldMinGasForTransfer, uint256 newMinGasForTransfer);
  ...
  uint256 private s_minGasForTransfer;
  ...
  constructor(address keeperRegistryAddress, uint256 minWaitPeriodSeconds) {
    ...
    paused = true;
    s_minGasForTransfer = 55_000; // observed limit of 45K + 10k buffer
  }
  ...
  function topUp(address[] memory needsFunding) public onlyWhenNotPaused {
    require(needsFunding.length <= s_watchList.length, "Too many addresses for topUp");
    uint256 minWaitPeriodSeconds = s_minWaitPeriodSeconds;
    uint256 minGasForTransfer = s_minGasForTransfer;
    Target memory target;
  ...
  /**
   * @notice Sets the min gas for transfer
   */
  function setMinGasForTransfer(uint256 newMinGasForTransfer) external onlyAdmin {
    require(newMinGasForTransfer > 0);
    emit MinGasForTransferUpdated(s_minGasForTransfer, newMinGasForTransfer);
    s_minGasForTransfer = newMinGasForTransfer;
  }
  ...
  /**
   * @notice Gets the minGasForTransfer
   */
  function getMinGasForTransfer() external view returns (uint256) {
    return s_minGasForTransfer;
  }
