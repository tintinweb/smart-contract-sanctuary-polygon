...
contract RewardDistributor is CollateralWrapper {
    using ECDSALib for bytes;
    using SafeERC20 for IERC20;
    ...
    event Distributed(
        address indexed to,
        uint256 amount,
        bytes32 uniqueRewardIdentifier
    );

    event BatchDistributionCompleted(
        bytes32 uniqueBatchRewardIdentifier,
        uint256 totalAmount
    );
    ...
    mapping(bytes32 => bool) private batchDistributionCompleted;
    mapping(bytes32 => bool) private distributionPerRecipientCompleted;
    ...
    function distribute(
        ...
        signature.verifyMessage(
            abi
                .encodePacked(to, amount, uniqueRewardIdentifier, msg.sender)
                .hash(),
            nonce,
            timestamp,
            signatures
        );
        _distribute(to, amount, uniqueRewardIdentifier);
    }

    function distributeByBatch(
        ...
        require(
            batchDistributionCompleted[uniqueBatchRewardIdentifier] == false,
            "This batch distribution has already been executed."
        );

        signature.verifyMessage(
            abi.encodePacked(uniqueBatchRewardIdentifier, msg.sender).hash(),
            nonce,
            timestamp,
            signatures
        );
        ...
        for (uint256 i = 0; i < payments.length; i++) {
            ...
            _distribute(
                payment.recipient,
                payment.amount,
                payment.uniqueRewardIdentifier
            );
        }

        batchDistributionCompleted[uniqueBatchRewardIdentifier] = true;

        emit BatchDistributionCompleted(
            uniqueBatchRewardIdentifier,
            totalAmount
        );
    }
    ...
    function transferAccidentallyLockedTokens(
        ...
        signature.verifyMessage(
            abi.encodePacked(address(token), amount, msg.sender).hash(),
            nonce,
            timestamp,
            signatures
        );
        ...
        require(
            token != _usdt,
            "The token address cannot be the ERC20 USDT address"
        );
    ...
    function _distribute(
        ...
        require(
            distributionPerRecipientCompleted[uniqueRewardIdentifier] == false,
            "This distribution has already been executed."
        );
        _collateralTransfer(to, amount);
        distributionPerRecipientCompleted[uniqueRewardIdentifier] = true;
        emit Distributed(to, amount, uniqueRewardIdentifier);
    }

    function getBatchDistributionCompleted(
        bytes32 hash
    ) external view returns (bool) {
        return batchDistributionCompleted[hash];
    }

    function getDistributionPerRecipientCompleted(
        bytes32 hash
    ) external view returns (bool) {
        return distributionPerRecipientCompleted[hash];
    }
}
