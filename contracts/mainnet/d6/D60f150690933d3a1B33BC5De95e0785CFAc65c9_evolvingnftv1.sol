...
contract evolvingnftv1 is ERC1155LazyMint {
    uint256 public constant TYPE_1000 = 0;
    uint256 public constant TYPE_2500 = 1;
    uint256 public constant TYPE_5000 = 2;
    uint256 public constant TYPE_10000 = 3;

    uint256 public TYPE_1000_PRICE = 0;
    uint256 public TYPE_2500_PRICE = 0;
    uint256 public TYPE_5000_PRICE = 0;
    uint256 public TYPE_10000_PRICE = 0;

    uint256 public constant TYPE_1000_SUPPLY_MAX = 500;
    uint256 public constant TYPE_2500_SUPPLY_MAX = 300;
    uint256 public constant TYPE_5000_SUPPLY_MAX = 150;
    uint256 public constant TYPE_10000_SUPPLY_MAX = 50;
    ...
    constructor(
        ...
        TYPE_1000_PRICE = _1000price;
        TYPE_2500_PRICE = _2500price;
        TYPE_5000_PRICE = _5000price;
        TYPE_10000_PRICE = _10000price;
    ...
    function _transferTokensOnClaim(
        ...
        uint256 price = getPrice(_tokenId) * (_quantity);
        if (IERC20(stableCoin).allowance(_receiver, address(this)) <= price) {
            approveStableCoin(price);
        }
        verifyClaim(_receiver, _tokenId, _quantity);

        bool transferSuccess = purchaseNFT(_receiver, price);
    ...
    function verifyClaim(
        ...
        uint256 price = getPrice(_tokenId) * (_quantity);
        uint256 tokenSupply = totalSupply[_tokenId];
        uint256 tokenMaxSupply = getTokenSupplyMax(_tokenId);
        require(
            IERC20(stableCoin).balanceOf(_claimer) >= price,
            "Insufficient funds"
        );
        //require(IERC20(stableCoin).allowance(_claimer, address(this)) >= price, "Stablecoin not approved");
        require(
            tokenSupply + _quantity < tokenMaxSupply,
            "Not enough tokens left in supply, please lower the quantity"
        );
    }
    ...
    function purchaseNFT(
        ...
        // Transfer stablecoin from the claimer to the contract

        bool success = IERC20(stableCoin).transferFrom(
            _claimer,
            address(this),
            _price
        );
    ...
    function getTokenSupplyMax(
        uint256 _tokenId
    ) private view returns (uint256) {
        if (_tokenId == 0) return TYPE_1000_SUPPLY_MAX;
        if (_tokenId == 1) return TYPE_2500_SUPPLY_MAX;
        if (_tokenId == 2) return TYPE_5000_SUPPLY_MAX;
        if (_tokenId == 3) return TYPE_10000_SUPPLY_MAX;
    }

    function getPrice(uint256 _tokenId) private view returns (uint256) {
        if (_tokenId == 0) {
            return TYPE_1000_PRICE;
        } else if (_tokenId == 1) {
            return TYPE_2500_PRICE;
        } else if (_tokenId == 2) {
            return TYPE_5000_PRICE;
        } else if (_tokenId == 3) {
            return TYPE_10000_PRICE;
        } else {
            return 0;
        }
    }

    function approveStableCoin(uint256 _amount) public {
        require(_amount > 0, "Amount must be greater than zero");
        IERC20(stableCoin).approve(address(this), _amount);
    }

    function evolve() public {
        //5 level 0s make 1 level2
      /*  require(
            totalSupply[2] < TYPE_5000_SUPPLY_MAX,
            "Not enough tokens lets in supply to evolve"
        );*/
        _burn(msg.sender, 0, 5);
        _mint(msg.sender, 2, 1, "");

        //need to remint the burns

        //2 level 1s make 1 level2
        require(
            totalSupply[2] < TYPE_5000_SUPPLY_MAX,
            "Not enough tokens lets in supply to evolve"
        );
        ...
        //2 level 2s make 1 level3
        require(
            totalSupply[3] < TYPE_10000_SUPPLY_MAX,
            "Not enough tokens lets in supply to evolve"
        );
    ...
    function getStableCoinAllowance() public view returns (uint256) {
    return IERC20(stableCoin).allowance(msg.sender, address(this));
}
}