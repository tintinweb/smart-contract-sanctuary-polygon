...
contract LionTron is owned
{
    ...
    mapping(address => rdata[]) public recycleWait;
    mapping(address => mapping(uint => bool)) public autoLevelBuy; 
    ...
    function regUser_(address msgsender, address _refAddress,bool pay, uint prc) internal returns(bool)
    {
        ...
        //emit directPaidEv(userInfos[msgsender].id,userInfos[origRef].id,prc*5/10, 1,now);
        uint userPosition;
        ...
        else if(userPosition >= 16 && userPosition <= 18 && userInfos[msgsender].levelBought < 2)
        {
            payForLevel(msgsender, 1, false, pay, false);
            autoLevelBuy[msgsender][2] = true;
        }
        else
        {
        ...
        if(treeComplete)
        {
            //if(userInfos[msgsender].levelBought >= 2)
            //{
            ...
            //{
            //    rdata memory tmp;
            //    tmp.user4thParent = user4thParent;
            //    tmp.level = 1;
            //    tmp.pay = pay;
            //    recycleWait[msgsender].push(tmp);
            //}

        }
        splitPart(lastIDCount,_referrerID,msgsender,userPosition,prc,temp.position,temp.currentParent );
        
        if (userPosition == 19 && autoLevelBuy[msgsender][2] )
        {
            buyLevel_(msgsender,2,true, levelPrice[2]);  
            autoLevelBuy[msgsender][2] = false;          
        }
        return true;
    }

    function manualRecycle(uint recycleWaitIndex) public returns(bool)
    {
        require(recycleWaitIndex < recycleWait[msg.sender].length, "Invalid index");
        if(recycleWaitIndex > 0) require(recycleWait[msg.sender][recycleWaitIndex-1].processed, "please process previous index");
        rdata memory temp = recycleWait[msg.sender][recycleWaitIndex];
        require(!temp.processed, "already processed");
        require(userInfos[msg.sender].levelBought >= temp.level, "next level buy pending");
        recyclePosition(temp.user4thParent,temp.level, true );
        recycleWait[msg.sender][recycleWaitIndex].processed = true;
        return true;
    }
    ...
    {
        emit regLevelEv(lastIDCount_,_referrerID,now, msgsender,userAddressByID[_referrerID]);
        emit treeEv(lastIDCount_,userPosition,prc,tempPosition, now,  tempCurrentParent, 1 );
        return true;
    }
    ...
    function buyLevel(uint _level) public returns(bool)
    {
        ...
        require(!autoLevelBuy[msg.sender][_level], "entered auto mode");
    ...
    function buyLevel_(address msgsender, uint _level,bool pay,  uint prc) internal returns(bool)
    {
        ...
        //emit directPaidEv(userInfos[msgsender].id,userInfos[origRef].id,prc*5/10, _level,now);
        uint userPosition;
        ...
        else if(userPosition >= 16 && userPosition <= 19 && userInfos[msgsender].levelBought < _level + 1 && _level < 12)
        {
            payForLevel(msgsender, _level, true, pay, false);
            autoLevelBuy[msgsender][_level +1] = true;
        }        
        else
        {
        ...
        if(treeComplete)
        {           
            //if(userInfos[msgsender].levelBought >= _level + 1 || _level == 12)
            //{
        ...
        emit levelBuyEv(prc, userInfos[msgsender].id,_level, now);
        splidStack( msgsender,userPosition,prc,temp.position,_referrerID,_level);

        if (userPosition == 19 && autoLevelBuy[msgsender][_level + 1] )
        {
            buyLevel_(msgsender,_level + 1,true, levelPrice[_level + 1]);  
            autoLevelBuy[msgsender][_level + 1] = false;          
        }
    ...
    {
        emit treeEv(userInfos[msgsender].id,userPosition,prc,tempPosition,now,_referrerID, _level );
        return true;
    }
    ...
    function recyclePosition(uint _userID, uint _level, bool pay)  internal returns(bool)
    {
        ...
        if(_userID == 1 ) 
        {
            ...
            emit treeEv(_userID,0,levelPrice[_level],0,now,1, _level );
            return true;
        }
