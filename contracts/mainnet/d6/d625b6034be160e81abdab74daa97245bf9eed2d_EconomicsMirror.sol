...
contract EconomicsMirror is Ownable, IEconomicsDataTypes {
    ...
    function topUpIntegrator(
        ...
        uint256 _total,
        uint256 _price
    ) external isDAOController returns(uint256) {
...
interface IChamberOfCommerce is ITellerV2DataTypes, IEconomicsDataTypes {
    ...
    // function returnStakeLocker(address _safeAddress) external view returns(address _stakingLocker);
    ...
    //     uint32 _integratorIndex
    // ) external view returns(IntegratorData memory _integratorData);
    ...
    // function returnAccount(address _accountAddress) external view returns(ActorAccount memory account_);
...
interface ITrancheBucketDataTypes is IEconomicsDataTypes {
    ...
    // add the balance at that moment
    // add maybe reputation
    // add the GET/USD price at the moment
    struct BackingStruct {
        address relayerAttestation;
        BackingVerification verification;
        uint32 integratorIndex;
        // IntegratorData integratorData;
        uint256 usdBalance;
        uint256 timestampBacking;
    }
