...
// File contracts/exp/ATriCryptoConvexFarm.sol
...
contract ATriCryptoConvexFarm is ERC20, ReentrancyGuard, Ownable {
    ...
    uint public defaultDepositIndex = 1;
    address public convexRewardPool;
    uint public convexPoolId = 3;
    ...
    constructor(
    ...
    ) ERC20("ATriCryptoCvxFarm", "ATRI-CVX-FRM") {
        ...
        _addRewardV3(_crvToNativePath, 1e17);
    ...
    function resetRewardsV3() external onlyOwner {
        delete rewardsV3;
    }
    ...
    function addRewardV3(bytes memory _rewardToNativePath, uint _minAmount) external onlyOwner {
        ...
        require(_rewardToNativeRoute[_rewardToNativeRoute.length - 1] == native, "Last element of path must be native");
        _addRewardV3(_rewardToNativePath, _minAmount);
    }

    function _addRewardV3(bytes memory _rewardToNativePath, uint _minAmount) internal {
        address[] memory _rewardToNativeRoute = pathToRoute(_rewardToNativePath);
    ...
    function swapNativeToDeposit() internal {
        uint256 nativeBal = IERC20(native).balanceOf(address(this));
        UniV3Actions.swapV3WithDeadline(unirouterV3, nativeToDepositPath, nativeBal);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(curveLpToken).approve(curvePool, type(uint).max);
