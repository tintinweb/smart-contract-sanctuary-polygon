...
function verified(bytes32 merkleRoot1, address to, uint256 amount, bytes32[] calldata proof) pure external returns (bytes32, bool) {

        // Verify merkle proof, or revert if not in tree
        bytes32 leaf = keccak256(abi.encodePacked(to, amount));
        bool isValidLeaf = MerkleProof.verify(proof, merkleRoot1, leaf);
        return (leaf, isValidLeaf);
}
