...
// File: contracts/PSCUtility.sol


pragma solidity ^0.8.2;


contract LionCubUtility is ERC20, AccessControl, ERC721Holder {
    bytes32 public constant CONTRACT_ADMIN_ROLE = keccak256("CONTRACT_ADMIN_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    address public collectionAddressLionCubNFT;
    ...
    uint256 public welcomeBonusAmountLionCubNFT;
    uint256 public welcomeBonusAmountNFTv2;

    uint256 public amountOfStakersLionCubNFT;
    uint256 public amountOfStakersNFTv2;

    uint256 public tokensStakedLionCubNFT;
    ...
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogLionCubNFT;

        /// owner => tokenId => StakeInfo
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogNFTv2;

    /// owner => #NFTsStaked
    mapping (address => uint256) public tokensStakedByUserLionCubNFT;
    ...
    mapping (uint256 => bool) public welcomeBonusCollectedLionCubNFT;

        /// tokenId => true/false (true if welcome bonus has been collected for a specific tokenId)
    mapping (uint256 => bool) public welcomeBonusCollectedNFTv2;

    /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserLionCubNFT;

        /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserNFTv2;

    /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioLionCubNFT;

        /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioNFTv2;

    event RewardsHarvestedLionCubNFT (address owner, uint256 amount);
    event NFTStakedLionCubNFT (address owner, uint256 tokenId);
    event NFTUnstakedLionCubNFT (address owner, uint256 tokenId);
    ...
    constructor(address owner, address _collectionAddressLionCubNFT, address _collectionAddressNFTv2) ERC20("LionCubUtility", "LCU") AccessControl(){
        _mint(owner, 100000 * 10 ** 18);
        _setupRole(DEFAULT_ADMIN_ROLE, owner);
        _setupRole(CONTRACT_ADMIN_ROLE, owner);
        _setupRole(BURNER_ROLE, owner);
        _setupRole(MINTER_ROLE, owner);
        collectionAddressLionCubNFT = _collectionAddressLionCubNFT;
        collectionAddressNFTv2 = _collectionAddressNFTv2;
        contractCreationBlock = block.number;
        coinAmountPerRewardUnitLionCubNFT = 15 * 10 ** 18; // 5 ERC20 coins per rewardUnit, may be changed later on
        ...
        welcomeBonusAmountLionCubNFT = 300 * 10 ** 18; // 300 tokens welcome bonus, only paid once per tokenId
        welcomeBonusAmountNFTv2 = 600 * 10 ** 18; // 600 tokens welcome bonus, only paid once per tokenId
    }

    function stakedNFTSByUserLionCubNFT(address owner) external view returns (uint256[] memory){
        return stakePortfolioByUserLionCubNFT[owner];
    }
    ...
    function pendingRewardsLionCubNFT(address owner, uint256 tokenId) public view returns (uint256){
        StakeInfo memory info = stakeLogLionCubNFT[owner][tokenId];
        ...
        return rewardAmount * coinAmountPerRewardUnitLionCubNFT;
    ...
    function stakeLionCubNFT(uint256 tokenId) public {
        IERC721(collectionAddressLionCubNFT).safeTransferFrom(_msgSender(), address(this), tokenId);
        require(IERC721(collectionAddressLionCubNFT).ownerOf(tokenId) == address(this),
            "PSCUtility: Error while transferring token");
        StakeInfo storage info = stakeLogLionCubNFT[_msgSender()][tokenId];
        info.stakedAtBlock = block.number;
        info.lastHarvestBlock = block.number;
        info.currentlyStaked = true;
        if(tokensStakedByUserLionCubNFT[_msgSender()] == 0){
            amountOfStakersLionCubNFT += 1;
        }
        tokensStakedByUserLionCubNFT[_msgSender()] += 1;
        tokensStakedLionCubNFT += 1;
        stakePortfolioByUserLionCubNFT[_msgSender()].push(tokenId);
        uint256 indexOfNewElement = stakePortfolioByUserLionCubNFT[_msgSender()].length - 1;
        indexOfTokenIdInStakePortfolioLionCubNFT[tokenId] = indexOfNewElement;
        if(!welcomeBonusCollectedLionCubNFT[tokenId]) {
            _mint(_msgSender(), welcomeBonusAmountLionCubNFT);
            welcomeBonusCollectedLionCubNFT[tokenId] = true;
        }

        emit NFTStakedLionCubNFT(_msgSender(), tokenId);
    }
    ...
    function stakeBatchLionCubNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            stakeLionCubNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakeBatchLionCubNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            unstakeLionCubNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakeLionCubNFT(uint256 tokenId) public {
        if(pendingRewardsLionCubNFT(_msgSender(), tokenId) > 0){
            harvestLionCubNFT(tokenId);
        }
        StakeInfo storage info = stakeLogLionCubNFT[_msgSender()][tokenId];
        info.currentlyStaked = true;
        IERC721(collectionAddressLionCubNFT).safeTransferFrom(address(this), _msgSender(), tokenId);
        require(IERC721(collectionAddressLionCubNFT).ownerOf(tokenId) == _msgSender(),
            "PSCUtility: Error while transferring token");
        if(tokensStakedByUserLionCubNFT[_msgSender()] == 1){
            amountOfStakersLionCubNFT -= 1;
        }
        tokensStakedByUserLionCubNFT[_msgSender()] -= 1;
        tokensStakedLionCubNFT -= 1;
        stakePortfolioByUserLionCubNFT[_msgSender()][indexOfTokenIdInStakePortfolioLionCubNFT[tokenId]] = 0;
        emit NFTUnstakedLionCubNFT(_msgSender(), tokenId);
    }
    ...
    function harvestLionCubNFT(uint256 tokenId) public {
        StakeInfo storage info = stakeLogLionCubNFT[_msgSender()][tokenId];
        ...
        if(rewardAmountInERC20Tokens > 0) {
            ...
            emit RewardsHarvestedLionCubNFT(_msgSender(), rewardAmountInERC20Tokens);
        }
    }
    ...
    function harvestBatchLionCubNFT(address user) external {
        uint256[] memory tokenIds = stakePortfolioByUserLionCubNFT[user];

        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            harvestLionCubNFT(tokenIds[currentId]);
        }
    }
    ...
    function setCoinAmountPerRewardUnitLionCubNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        coinAmountPerRewardUnitLionCubNFT = coinAmount;
    }
    ...
    function setWelcomeBonusAmountLionCubNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        welcomeBonusAmountLionCubNFT = coinAmount;
    }
    ...
    function setCollectionAddressLionCubNFT(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "VIBE: update to zero address not possible");
        collectionAddressLionCubNFT = newAddress;
    }
