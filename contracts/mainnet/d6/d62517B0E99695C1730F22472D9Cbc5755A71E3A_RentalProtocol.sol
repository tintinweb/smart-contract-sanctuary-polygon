...
contract BorrowedNFT is IRewardsSplitter, ERC721 {
    ...
    uint64 public constant MAX_DISTRIBUTED_REWARDS = 100_00;
    ...
    function onERC20Received(
        ...
        for (uint256 i = 0; i < fees.length; i++) {
            ...
            uint256 reward = (remaining * fee.percentage) / MAX_DISTRIBUTED_REWARDS;
            remaining -= reward;
...
interface IRentalProtocol {
    ...
    struct RentalOffer {
        ...
        address taker;
        /// NFTs exposed in this rental offer
        NFTRentalSettings[] nftSettings;
    ...
    struct NFTRentalSettings {
        ...
        uint64 duration;
        /// percentage of rewards for this user (10000 for 100%)
        uint64 basisPoints;
    }
    ...
    event RentalEnded(address indexed lender, address indexed tenant, address token, uint256 tokenId, uint256 end);
    ...
    function setFeesCollector(address _feesCollector) external;

    function setFeesPercentage(uint64 _feesPercentage) external;
}
...
contract RentalProtocol is IRentalProtocol, AccessControl, ERC721Holder, ReentrancyGuard, EIP712 {
    ...
    uint64 public protocolFeeBasisPoints;
    ...
    constructor(address _feesCollector, uint64 _feesPercentage) EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION) {
    ...
    function rent(
        ...
        if (offer.feeAmount > 0) {
            ...
            // taker pays rental cost to lender
            feeToken.safeTransferFrom(msg.sender, offer.maker, offer.feeAmount);
            // taker pays the fees to feesCollector
        ...
        // process each NFT
        for (uint256 i = 0; i < offer.nftSettings.length; i++) {
            NFTRentalSettings memory nftSettings = offer.nftSettings[i];
            address token = nftSettings.token;
            uint256 tokenId = nftSettings.tokenId;
            uint64 basisPoints = nftSettings.basisPoints;
            ...
            IERC721(nftSettings.token).safeTransferFrom(offer.maker, address(this), tokenId);
            // mint LendNFT token
            ...
            Rental memory rental = Rental({
                end: block.timestamp + nftSettings.duration,
                lenderFee: basisPoints,
                sublenderFee: 0
            });

            rentals[nftSettings.token][tokenId] = rental;
    ...
    function getFeesTable(address token, uint256 tokenId) external view override returns (Fee[] memory) {
        ...
        fees[0] = Fee({to: lentNFT.ownerOf(tokenId), percentage: rentals[token][tokenId].lenderFee});

        if (isSublet) {
            SubLentNFT subLentNFT = originalToSubLendNFT[token];
            fees[1] = Fee({to: subLentNFT.ownerOf(tokenId), percentage: rentals[token][tokenId].sublenderFee});
        }
    ...
    function setFeesPercentage(uint64 _protocolFeeBasisPoints) public override {
        require(hasRole(FEES_MANAGER_ROLE, msg.sender), "FEES_MANAGER_ROLE only");
    ...
    function hashRentalOffer(RentalOffer memory offer) public view returns (bytes32) {
        bytes32 nftSettingsHash = hashNFTSettings(offer.nftSettings);
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "RentalOffer(address maker,address taker,NFTRentalSettings[] nftSettings,address feeToken,uint256 feeAmount,uint256 nonce,uint256 deadline)NFTRentalSettings(address token,uint256 tokenId,uint64 duration,uint64 basisPoints)"
                        ),
                        offer.maker,
                        offer.taker,
                        nftSettingsHash,
    ...
    function _endRental(address token, uint256 tokenId) internal {
        ...
        emit RentalEnded(lender, tenant, token, tokenId, rentals[token][tokenId].end);
    ...
    function _endSublet(address token, uint256 tokenId) internal {
        ...
        BorrowedNFT borrowedNFT = originalToBorrowedNFT[token];
        borrowedNFT.transferFrom(borrowedNFT.ownerOf(tokenId), subLender, tokenId);
    ...
    function hashNFTSettings(NFTRentalSettings[] memory nftRentalSettings) internal pure returns (bytes32 hash) {
        uint256 arrayLength = nftRentalSettings.length;
        if (arrayLength == 0) {
        ...
        for (uint256 i = 0; i < arrayLength; i++) {
            hashArray[i] = keccak256(
                abi.encode(
                    keccak256("NFTRentalSettings(address token,uint256 tokenId,uint64 duration,uint64 basisPoints)"),
                    nftRentalSettings[i].token,
                    nftRentalSettings[i].tokenId,
                    nftRentalSettings[i].duration,
                    nftRentalSettings[i].basisPoints
                )
            );
        }
