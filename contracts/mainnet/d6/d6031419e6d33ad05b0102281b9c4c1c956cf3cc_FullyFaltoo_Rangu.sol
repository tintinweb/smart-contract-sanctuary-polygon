...
contract ERC721 is ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    uint256 public MAX_SUPPLY = 1379;

    /**
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
     */
    constructor() {
        _name = "Fully Faltoo_Rangu";
        _symbol = "Rangu";
        royaltyFeeInfo = RoyaltyInfo(msg.sender, 1000);
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(_tokenURIs[tokenId]).length > 0 ? string(abi.encodePacked(_baseURI, _tokenURIs[tokenId])) : string(abi.encodePacked(_baseURI,tokenId.toString()));
    }
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view virtual returns (bool) {
        require(_exists(tokenId), "ERC721: _operator query for nonexistent token");
        address owner = ERC721.ownerOf(tokenId);
    ...
    function _setApprovalForAll(
        ...
        require(owner != _operator, "ERC721: approve to caller");
     ...
     * @param _tokenURI string memory URI of the token to be minted.
     */

    function createCollectible(address creator, string memory _tokenURI) external virtual override returns (uint256) {
        require(msg.sender == operator, "ERC721: caller doesn't have a role");
        require(tokenCounter <= MAX_SUPPLY, "ERC721: Minting limit exceeds the MAX_SUPPLY");
...
contract FullyFaltoo_Rangu is ERC721 {
