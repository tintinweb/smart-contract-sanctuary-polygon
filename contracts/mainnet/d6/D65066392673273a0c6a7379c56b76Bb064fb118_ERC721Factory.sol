...
contract ERC721Factory is IERC721Factory, Allocator, Whitelist {
    ...
    constructor(address collection_, uint256 fee_, uint256 baseMaxAllocation_, Phase[] memory phases_) {
        setCollection(collection_);
        setFactoryValues(false, true, fee_, baseMaxAllocation_);
    ...
    function mintAdmin(address to, uint256 amount) public payable override onlyOwner {
        (bool allowed, ) = canAllocate(to, amount);
        if (!allowed) setAllocation(to, currentAllocation(to) + amount);
        mint(to, amount);
    }
    ...
    function canMint(address minter, uint256 amount) public override view returns (bool, string memory) {
        ...
        (allowed, message) = canAllocate(minter, amount);
        if (!allowed) {
        ...
        if (_msgSender() != owner()) {
            ...
            if (isWhitelistActive() && !isWhitelisted(minter)) {
    ...
    // Setters
    function setFactoryValues(bool enableMinting_, bool enableWhitelist_, uint256 fee_, uint256 baseMaxAllocation_)
    internal onlyOwner {
        setMintingActive(enableMinting_);
        initializeWhitelist();
        setWhitelistActive(enableWhitelist_);
        initializeAllocator();
        setFee(fee_);
    ...
    function setCollection(address collection_) internal {
    ...
    function transferCollectionOwnership(address factory_) external override onlyOwner {
        require(factory_ != address(0) && factory_ != address(this),
            'Factory: new factory address needs to be different than zero and current address!');

        ERC721Wrapper(collection).transferOwnership(factory_);
        setCollection(address(0));

        emit FactoryTransferred(factory_);
    }
}
...
interface IERC721Wrapper {
    ...
    event ReservedUrisDeleted();
    event ReservedUriAdded(uint indexed forTokenId, string uri);
...
contract ERC721Wrapper is Ownable, ContextMixin, ERC721URIStorage, IERC721Wrapper, ERC2981 {
    ...
    // Core functions
    function mintTo(address to) external override {
        mintTo(to, 1);
    }
    ...
    function delReservedTokenURIs() external onlyOwner {
        require(reservedURICounter == 0, 'ERC721: no longer can delete reserved token URIs, minting already started!');
        delete reservedURIs;
        emit ReservedUrisDeleted();
    }

    function addReservedTokenURIs(string[] memory _tokenURIs) external onlyOwner {
        for (uint i=0; i<_tokenURIs.length; i++) {
            reservedURIs.push(_tokenURIs[i]);
            emit ReservedUriAdded(i+1, _tokenURIs[i]);
        }
    }
...
contract Whitelist is Ownable, IWhitelist {
    ...
    function addToWhitelist(address[] memory wallets) external override onlyOwner {
        for (uint i=0; i < wallets.length; i++) {
            _addAddress(wallets[i]);
        }
    }

    function deleteFromWhitelist(address[] memory wallets) external override onlyOwner {
        for(uint i=0; i < wallets.length; i++) {
            _removeAddress(wallets[i]);
        }
    }
    ...
    function _removeAddress(address wallet) internal {
    ...
    function _addAddress(address wallet) internal {
        if (!isWhitelisted(wallet)) {
...
interface IWhitelist { 
    ...
    event AddressAdded(address wallet);
    event AddressRemoved(address wallet);
...
contract Allocator is Ownable, IAllocator {
    ...
    function setAllocations(Allocation[] memory allocs) external override onlyOwner {
        for (uint i=0; i < allocs.length; i++) {
            setAllocation(allocs[i].allocator, allocs[i].amount);
        }
    }

    function setAllocation(address allocator, uint256 amount) public override onlyOwner {
    ...
    function allocate(address allocator, uint256 amount) internal onlyWhenAllocatable(allocator, amount) {
    ...
    function updatePhase(uint256 phaseId, uint64 blockNumber, uint256 minLimit) external override onlyOwner {
