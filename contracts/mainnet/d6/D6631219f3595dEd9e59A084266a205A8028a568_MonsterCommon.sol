...
abstract contract MonsterBase is ChamberBase, IMonster {
  ...
  struct AdrContext {
    ...
    IFightCalculator fightCalculator;
  }
  ...
  /// @notice Version of the contract
  /// @dev Should be incremented when contract changed
  string public constant MONSTER_BASE_VERSION = "1.0.0";

  // ---- VARIABLES ----

  MonsterGenerateInfo internal _monsterGenerateInfo;
  address[] public mintItems;
  uint[] public mintItemsChances;
  uint public override race;
  uint internal maxDropItems;

  // fight specific

  mapping(address => mapping(uint => GeneratedMonster)) internal _generatedMonsters;

  // ---- EVENTS ----

  event MintItemsChanged(address[] mintItems_, uint[] mintItemsChances_);
  event MonsterInfoChanged(MonsterGenerateInfo info);
  ...
  function __MonsterBase_init(
    ...
    MonsterGenerateInfo memory monsterInfo_,
    address[] memory mintItems_,
    uint[] memory mintItemsChances_,
    uint _maxDropItems,
    uint _race
    ...
    _monsterGenerateInfo = monsterInfo_;
    _checkItem(mintItems_, mintItemsChances_);
    mintItems = mintItems_;
    mintItemsChances = mintItemsChances_;
    maxDropItems = _maxDropItems;
    race = _race;
  }

  // ---- VIEWS ----

  function monsterInfo(address dungeonLogic, address heroToken, uint heroTokenId) public view returns (
    uint[] memory attributes,
    uint level,
    uint experience,
    IFightCalculator.AttackInfo memory attackInfo
  ) {
    AdrContext memory context = _context(msg.sender, heroToken, heroTokenId);
    (attributes,
      level,
      experience,
      attackInfo) = _generateMonsterInfo(
      context.statController,
      _generatedMonsters[heroToken][heroTokenId].amplifier,
      monsterMultiplier(context.controller, dungeonLogic, biome),
      1e18,
      IHero(heroToken).stats(heroTokenId).level
    );
  }

  function monsterMultiplier(IController _controller, address dungeonLogic, uint biome) public view returns (uint) {
    return _controller.dungeonMultiplier(
      dungeonLogic,
      address(this)
    ) + _controller.globalBiomeMonsterMultiplier(biome);
  }

  function monsterGenInfo(address heroToken_, uint heroTokenId_) external view returns (GeneratedMonster memory) {
    return _generatedMonsters[heroToken_][heroTokenId_];
  }

  // ---- GOV ACTIONS ----

  function setMonsterInfo(MonsterGenerateInfo memory info_) external {
    require(isGovernance(msg.sender), "!gov");
    _monsterGenerateInfo = info_;
    emit MonsterInfoChanged(info_);
  }
  ...
  function setMintItems(
    ...
    require(isGovernance(msg.sender), "!gov");
    require(mintItems_.length == mintItemsChances_.length, "Wrong input");
    _checkItem(mintItems_, mintItemsChances_);
    mintItems = mintItems_;
    mintItemsChances = mintItemsChances_;
  ...
  function open(address heroToken, uint heroTokenId) public override returns (uint iteration) {
    AdrContext memory context = _context(msg.sender, heroToken, heroTokenId);

    require(context.controller.validDungeons(msg.sender), "Not dungeon");

    GeneratedMonster memory gen = GeneratedMonster({
      generated: true,
      amplifier: context.oracle.getRandomNumber(_MAX_AMPLIFIER, uint(uint160(msg.sender))),
      hp: 0,
      turnCounter: 0
    });

    (uint[] memory attributes,,,,) = _generateMonster(gen, context, context.statController.heroStats(heroToken, heroTokenId).level);

    gen.hp = attributes[uint(IStatController.ATTRIBUTES.LIFE)];

    _generatedMonsters[heroToken][heroTokenId] = gen;

    return super.open(heroToken, heroTokenId);
  }
  ...
  function _action(
    ...
    AdrContext memory context = _context(sender, heroToken_, heroTokenId_);
    GeneratedMonster memory gen = _generatedMonsters[heroToken_][heroTokenId_];

    FightInternalInfo memory fInfo = _fight(stageId, data_, gen, context);

    uint monsterBiome = biome;
    ...
    _bossDefeated(isMonsterDead, context, heroToken_, heroTokenId_);

    if (isMonsterDead || isHeroDead) {
      if (gen.generated) {
        delete _generatedMonsters[heroToken_][heroTokenId_];
      }
      context.statController.clearUsedConsumables(heroToken_, heroTokenId_);
    } else {
      if (!gen.generated) {
        _generatedMonsters[heroToken_][heroTokenId_] = GeneratedMonster({
          generated: true,
        ...
        GeneratedMonster storage genStorage = _generatedMonsters[heroToken_][heroTokenId_];
        genStorage.hp = fInfo.monsterHp;
        genStorage.turnCounter = gen.turnCounter + 1;
      }
    }
    ...
    return ChamberResult({
      ...
      kill: isHeroDead,
      experience: isMonsterDead ? StatLib.expPerMonster(
      ...
      heal: fInfo.heroLifeRegen,
      manaRegen: isMonsterDead ? fInfo.heroFightInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.MANA_AFTER_KILL)] : 0,
      lifeChancesRecovered: 0,
      damage: fInfo.damage,
      manaConsumed: fInfo.manaConsumed,
      mintItems: isMonsterDead ? _mintRandomItems(fInfo, monsterBiome) : new address[](0),
      completed: isMonsterDead || isHeroDead,
      rewriteNextChamber: address(0),
      iteration: 0 // should be rewrite later in ChamberBase
    });
  }

  function _bossDefeated(bool isMonsterDead, AdrContext memory context, address heroToken_, uint heroTokenId_) internal {
    if (!isMonsterDead) {
      return;
    }
    if (chamberType == uint(IChamberController.ChamberType.BOSS_3)) {
      IDungeonFactory(context.controller.dungeonFactory()).setBossCompleted(heroToken_, heroTokenId_);
    }
  }
  ...
  function _collectHeroFighterInfo(
  ...
  ) internal view returns (IFightCalculator.FighterInfo memory fInfo, uint manaConsumed) {

    IStatController.ChangeableStats memory heroStats = IHero(context.heroToken).stats(context.heroTokenId);

    (uint[] memory heroAttributes, uint _manaConsumed) = _buffAndGetHeroAttributes(
      context.heroToken,
      context.heroTokenId,
      heroStats.level,
      attackInfo,
    ...
    if (attackInfo.attackType == IFightCalculator.AttackType.MAGIC) {
      manaConsumed += IAttackItem(attackInfo.attackToken).attackAttributes(attackInfo.attackTokenId).manaConsume;
    }
  ...
  function _buffAndGetHeroAttributes(
    ...
    IFightCalculator.AttackInfo memory attackInfo,
    AdrContext memory context
  ) internal view returns (uint[] memory heroAttributes, uint manaConsumed) {
  ...
  function _debuff(
    uint[] memory attributes,
    address[] memory debuffTokens,
    uint[] memory debuffTokenIds,
    AdrContext memory context
  ) internal view {
    uint length = debuffTokens.length;
    for (uint i; i < length; ++i) {
      address token = debuffTokens[i];
      uint id = debuffTokenIds[i];

      require(context.controller.validItems(token), "Item not registered");
      require(IItem(token).equipped(id), "Buff item not equipped");
      require(IItem(token).equippedOn(id, context.heroToken) == context.heroTokenId, "Not your buff item");

      (uint[] memory debuff,uint[] memory buff) = IBuffItem(token).debuff(id);
      StatLib.attributesRemove(attributes, debuff);
      StatLib.attributesAdd(attributes, buff);
    }
  }

  function _collectMonsterFighterInfo(
    GeneratedMonster memory gen,
    IFightCalculator.AttackInfo memory heroAttackInfo,
    IFightCalculator.FighterInfo memory heroFightInfo,
    AdrContext memory context
  ) internal returns (IFightCalculator.FighterInfo memory fighterInfo, uint rarity) {

    (uint[] memory attributes, uint level, uint amplifier, uint experience, IFightCalculator.AttackInfo memory attackInfo) = _generateMonster(gen, context, heroFightInfo.fighterStats.level);

    _debuff(attributes, heroAttackInfo.skillTokens, heroAttackInfo.skillTokenIds, context);

    uint hp;
    if (gen.generated) {
      hp = gen.hp;
    } else {
      hp = Math.max(attributes[uint(IStatController.ATTRIBUTES.LIFE)], 1);
    }
    ...
    IStatController.ChangeableStats memory stats = IStatController.ChangeableStats({
      ...
      life: hp,
      mana: attributes[uint(IStatController.ATTRIBUTES.MANA)],
      lifeChances: 0
    });

    rarity = amplifier;

    fighterInfo = IFightCalculator.FighterInfo({
  ...
  function _fight(
    ...
    GeneratedMonster memory gen,
    AdrContext memory context
  ) internal returns (FightInternalInfo memory){
    IFightCalculator.FighterInfo memory heroFightInfo;
    uint manaConsumed;
    IFightCalculator.FighterInfo memory monsterFightInfo;
    uint monsterRarity;
    {
      IFightCalculator.AttackInfo memory heroAttackInfo = context.fightCalculator.decodeAndCheckAttackInfo(
        data_,
        context.heroToken,
        context.heroTokenId
      );
      (heroFightInfo, manaConsumed) = _collectHeroFighterInfo(heroAttackInfo, context);
      (monsterFightInfo, monsterRarity) = _collectMonsterFighterInfo(gen, heroAttackInfo, heroFightInfo, context);
    }

    // >>> FIGHT!
    IFightCalculator.FightResult memory fightResult = context.fightCalculator.fight(IFightCalculator.FightCall({
      fighterA: heroFightInfo,
      fighterB: monsterFightInfo,
      dungeon: msg.sender,
      heroAdr: context.heroToken,
      heroId: context.heroTokenId,
      stageId: stageId
    }));

    return FightInternalInfo({
      manaConsumed: manaConsumed + fightResult.manaConsumedA,
      monsterRarity: monsterRarity,
      damage: _calcDmg(heroFightInfo.fighterStats.life, fightResult.healthA),
      heroFightInfo: heroFightInfo,
      monsterFightInfo: monsterFightInfo,
      heroLifeRegen: fightResult.healthA > heroFightInfo.fighterStats.life ? fightResult.healthA - heroFightInfo.fighterStats.life : 0,
      heroHp: fightResult.healthA,
      monsterHp: fightResult.healthB
    });
  }

  function _calcDmg(uint heroLifeBefore, uint heroLifeAfter) internal pure returns (uint) {
    uint damage;
    if (heroLifeAfter != 0) {
      damage = heroLifeBefore - Math.min(heroLifeAfter, heroLifeBefore);
    } else {
      damage = heroLifeBefore;
    }
    return damage;
  }

  function _generateMonster(GeneratedMonster memory gen, AdrContext memory context, uint heroLevel) internal returns (
    uint[] memory attributes,
    uint level,
    uint amplifier,
    uint experience,
    ...
    if (gen.generated) {
      amplifier = gen.amplifier;
    } else {
      amplifier = context.oracle.getRandomNumber(_MAX_AMPLIFIER, uint(uint160(msg.sender)));
    }

    uint _biome = biome;
    uint multiplier = monsterMultiplier(context.controller, IProxyControlled(msg.sender).implementation(), _biome);
    ...
    (attributes,
      ...
      amplifier,
      multiplier,
      context.oracle.getRandomNumber(1e18, amplifier),
      heroLevel
    );
  }

  function _generateMonsterInfo(IStatController statController, uint amplifier, uint dungeonMultiplier, uint rnd, uint heroLevel) internal view returns (
    uint[] memory attributes,
    uint level,
    uint experience,
    IFightCalculator.AttackInfo memory attackInfo
  ) {
    MonsterGenerateInfo storage info = _monsterGenerateInfo;

    level = info.level;


    if (level < heroLevel + 1) {
      level = Math.min(level + ((heroLevel - level) * 10 / 15), biome * 5);
    }

    if (info.attackInfo.attackType == IFightCalculator.AttackType.MAGIC) {
      if (rnd > 5e17) {
        IFightCalculator.AttackInfo[] memory tmp = new IFightCalculator.AttackInfo[](1);
        attackInfo = tmp[0];
    ...
    (attributes, experience) = statController.generateMonsterAttributes(
      info.ids,
      info.values,
      amplifier,
      dungeonMultiplier,
  ...
  function _mintRandomItems(
    ...
    return ChamberLib.mintRandomItems(ChamberLib.MintItemInfo({
      ...
      heroCurrentLvl: fInfo.heroFightInfo.fighterStats.level,
      ...
      maxItems: maxDropItems
    }));
  }

  function _context(address sender, address heroToken, uint heroTokenId) internal view returns (AdrContext memory context) {
    context = AdrContext({
      sender: sender,
      heroToken: heroToken,
      ...
      statController: IStatController(address(0)),
      fightCalculator: IFightCalculator(address(0))
    });
    context.oracle = IOracle(context.controller.oracle());
    context.statController = IStatController(context.controller.statController());
    context.fightCalculator = IFightCalculator(context.controller.fightCalculator());
  }
...
import "../interfaces/IChamberController.sol";
import "../base/chamber/MonsterBase.sol";

contract MonsterCommon is MonsterBase {
  ...
  function init(
    address controller_,
    ChamberMeta memory meta,
    MonsterGenerateInfo memory info,
    address[] memory mintItems_,
    uint[] memory mintItemsChances_,
    uint _maxDropItems,
    uint _race
