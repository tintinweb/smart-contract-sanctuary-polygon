...
import "../interfaces/ICollateralPolicy.sol";
import "../Operator.sol";
import "./../interfaces/IAssetController.sol";
import "./../interfaces/IMultiAssetTreasury.sol";

contract CollateralPolicy is ICollateralPolicy, Operator {
    ...
    function assetTcr(uint256 _assetId) public override view returns (uint256) {
        return assetCollateralPolicy[_assetId].target_collateral_ratio;
    }

    function assetEcr(uint256 _assetId) public override view returns (uint256) {
        return assetCollateralPolicy[_assetId].effective_collateral_ratio;
    }

    function getMissingDecimal(uint256 _assetId) public override view returns (uint256) {
        return assetCollateralPolicy[_assetId].missing_decimals;
    }

    function assetPriceBand(uint256 _assetId) public override view returns (uint256) {
    ...
    function addCollateralPolicy(
        ...
        uint256 _init_tcr,
        uint256 _init_ecr
    ) external override onlyAssetControllerOrOperator {
    ...
    function calcEffectiveCollateralRatio(uint256 _assetId) public view returns (uint256 _ecr) {
        ...
        uint256 total_collateral_value = IMultiAssetTreasury(treasury).collateralValue(_assetId);
    ...
    function refreshCollateralRatio() public override {
        ...
        for (uint256 aid = 0; aid < assetCount; aid++) {
            ...
            uint256 current_asset_price = IAssetController(assetController).getAssetPrice(aid);
    ...
    function setRefreshCooldown(uint256 _refresh_cooldown) public onlyOperator {
        require(_refresh_cooldown >= MIN_REFRESH_COOLDOWN && _refresh_cooldown <= MAX_REFRESH_COOLDOWN, "Invalid refresh cooldown");
...
interface ICollateralPolicy {
    ...
    function assetTcr(uint256 _aid) external view returns (uint256);

    function assetEcr(uint256 _aid) external view returns (uint256);

    function getMissingDecimal(uint256 _aid) external view returns (uint256);

    function assetPriceBand(uint256 _aid) external view returns (uint256);
