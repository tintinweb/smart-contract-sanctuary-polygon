...
contract UniswapHelper is IUniswapV3MintCallback {
    ...
    int24 public tickWidth = 2200; // r = 1.2461
    ...
    function calcSecondRebalance(LimitOrderStatus limit) external view returns (bool) {
        ...
        if (usdcValue > strategy.estimatedTotalAssets().div(10)) {
            if (limit == LimitOrderStatus.Bid && limitOrder.token0 == 0) {
                return true;
            }
            if (limit == LimitOrderStatus.Ask && limitOrder.token1 == 0) {
                return true;
            }
        }
        return false;
        ...
        )
    {
        ...
        estimatedFees = calcEstimatedFees();
        estimatedPrincipalFees = estimatePrincipalFees();
        estimatedLimitOrderFees = estimateLimitOrderFees();
        estimatedHarvest = int256(estimatedFees - _estimatedImpermanentLoss());
    }

    function calcEstimatedFees() public view returns (uint256) {
        return estimatePrincipalFees().add(estimateLimitOrderFees());
    }
    ...
    function unwindAllPositions() external onlyAuthorized returns (uint256, uint256) {
    ...
    function setOrders() external onlyAuthorized returns (LimitOrderStatus, bool) {
        (uint256 liquidityL, , ) = _position(limitTicks);
        (uint256 liquidity, , ) = _position(baseTicks);
        if (liquidityL > 0) {
            _closeLimitOrderPosition();
        }
        if (liquidity > 0) {
        ...
        (wantAmount, wethAmount, baseTicks);
        (LimitOrderStatus order, bool orderSet) = _setLimitOrder();
        _sendFundsToStrategy();
        return (order, orderSet);
    }

    // In order to be flexible, repay weth for example to reduce collateralRatio and put usdc to work
    function setLimitOrder() external onlyAuthorized returns (LimitOrderStatus, bool) {
        (LimitOrderStatus order, bool orderSet) = _setLimitOrder();
        _sendFundsToStrategy();
        return (order, orderSet);
    }
    ...
    function _unwindPosition(Ticks memory _ticks) internal returns (uint256, uint256) {
        (uint128 _liquidityLO, , ) = _position(_ticks);
        if (_liquidityLO > 0) {
            return _burnAndCollectLiquidity(_liquidityLO, _ticks);
        }
    }

    function _setLimitOrder() internal returns (LimitOrderStatus order, bool orderSet) {
    ...
    function _closeLimitOrderPosition() internal {
        (, Position memory limitOrder) = _openPositions();
        if (limitOrder.liquidity == 0) return;
        _burnAndCollectLiquidity(limitOrder.liquidity, limitTicks);
    }
        ...
        returns (uint256 wantBalance, uint256 wethBalance)
    {
        ...
        if (wantBalance < strategy.wantDust()) return (wantBalance, wethBalance.div(2));
        if (wethBalance < strategy.wethDust()) return (wantBalance.div(2), wethBalance);
    }
    ...
    function _estimatedImpermanentLoss() internal view returns (uint256) {
        uint256 debt = strategy.strategyDebt();
        uint256 currentBalance = strategy.estimatedTotalAssets();
        if (currentBalance < debt) return debt.sub(currentBalance);
    }
...
interface IStrategy {
    ...
    function estimatedTotalAssetsOracle() external view returns (uint256);
    ...
    function tickWidth() external returns (int24);
