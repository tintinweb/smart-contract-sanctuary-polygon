...
contract CompoundLens {
    ...
    function cTokenMetadata(OToken cToken) public returns (OTokenMetadata memory) {
        ...
        if (compareStrings(cToken.symbol(), "cETH")) {
            ...
            OErc20 cErc20 = OErc20(address(cToken));
            underlyingAssetAddress = cErc20.underlying();
            // Hacky, may be wrong?
            underlyingDecimals = OErc20(cErc20.underlying()).decimals();
        }
    ...
    function cTokenMetadataAll(OToken[] calldata cTokens) external returns (OTokenMetadata[] memory) {
        uint cTokenCount = cTokens.length;
        OTokenMetadata[] memory res = new OTokenMetadata[](cTokenCount);
    ...
    function cTokenBalances(OToken cToken, address payable account) public returns (OTokenBalances memory) {
    ...
    function cTokenBalancesAll(OToken[] calldata cTokens, address payable account) external returns (OTokenBalances[] memory) {
        uint cTokenCount = cTokens.length;
        OTokenBalances[] memory res = new OTokenBalances[](cTokenCount);
        for (uint i = 0; i < cTokenCount; i++) {
    ...
    function cTokenUnderlyingPrice(OToken cToken) public returns (OTokenUnderlyingPrice memory) {
        ComptrollerLensInterface comptroller = ComptrollerLensInterface(address(cToken.comptroller()));
    ...
    function cTokenUnderlyingPriceAll(OToken[] calldata cTokens) external returns (OTokenUnderlyingPrice[] memory) {
        uint cTokenCount = cTokens.length;
        OTokenUnderlyingPrice[] memory res = new OTokenUnderlyingPrice[](cTokenCount);
        for (uint i = 0; i < cTokenCount; i++) {
