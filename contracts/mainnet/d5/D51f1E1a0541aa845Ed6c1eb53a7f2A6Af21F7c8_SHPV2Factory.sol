...
contract SHPV2Factory is Ownable {
    ...
    function deployPool(
        ...
        uint256[MAX_UNLOCKS] calldata _distributionEnd,
        ...
        returns (address pool)
    {
        ...
        ISHP(pool).initialize(token, _initialBalance, _unlocksDate, _distributionEnd, _rewards, _accounts);
    ...
    function transferPoolOwnership(address _pool, address _newOwner) external onlyOwner {
        require(IOwnable(_pool).owner() == address(this), "Factory has no pool ownership");
...
contract StrongHoldersPoolV2 is Ownable, ReentrancyGuard, ISHP {
    ...
    struct GeneralPoolInfo {
        ...
        uint256[MAX_UNLOCKS] distributionEnds;
    }
    ...
    function initialize(
        ...
        uint256 _initialBalance,
        uint256[MAX_UNLOCKS] calldata _unlocksDate,
        uint256[MAX_UNLOCKS] calldata _distributionEnds,
        uint256[MAX_UNLOCKS] calldata _rewards,
        address[MAX_POOL_USERS] calldata _accounts
    )
        external
        ...
        onlyOwner
    {
        generalPoolInfo = GeneralPoolInfo({
            ...
            distributionEnds: _distributionEnds
        });
        ...
        nonReentrant
    {
        ...
        require(!pools[_poolId].disabled, "SHP: pool is disabled");
    ...
    function emergencyStart(uint256 _poolId) external onlyOwner {
        pools[_poolId].disabled = false;
    }

    function emergencyStop(uint256 _poolId) external onlyOwner {
        pools[_poolId].disabled = true;
    }

    function emergencyWithdraw(address _to, uint256 _balance) external onlyOwner {
        _withdraw(_to, _balance);
    }
    ...
    function calculateReward(uint8 _poolId, address _account) public view returns (uint256 reward) {
        ...
        if (block.timestamp >= distributionEnd(_poolId)) {
            ...
            if (block.timestamp >= generalPoolInfo.unlocks[_poolId]) {
                reward = generalPoolInfo.rewards[pool.lastWithdrawPosition];
            }
        }
    }
    ...
    function distributionEnd(uint _poolId) public view returns (uint256 timestamp) {
        ...
        timestamp = generalPoolInfo.distributionEnds[_poolId];
    }
    ...
    function _isParticipant(address _account) internal view returns (bool status) {
        GeneralPoolInfo memory pool = generalPoolInfo;
        for (uint8 i; i < MAX_POOL_USERS; i++) {
            if (_account == pool.accounts[i]) {
                status = true;
            }
        }
    }
    ...
    modifier checkAccess() {
        bool accessGranted = _isParticipant(msg.sender);
...
interface ISHP {
	function initialize(
		...
		uint256[10] calldata _distributionEnd,
