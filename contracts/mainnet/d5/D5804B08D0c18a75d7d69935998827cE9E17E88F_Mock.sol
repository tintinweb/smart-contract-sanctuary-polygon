...
contract Mock {
    ...
    event DecodedArgs(_ILiFi.LiFiData argsLiFiData, ITransactionManager.PrepareArgs argsPrepareArgs);
    event Address3(address a, address b, address c);
    event String(string msg);
    ...
    function check(
        ...
        ) = abi.decode(sendArgs.lifiTxData, (_ILiFi.LiFiData, ITransactionManager.PrepareArgs));

        emit DecodedArgs(argsLiFiData, argsPrepareArgs);
        emit Address3(argsLiFiData.sendingAssetId, argsPrepareArgs.invariantData.sendingAssetId, tokenIn);
        if(argsLiFiData.sendingAssetId != argsPrepareArgs.invariantData.sendingAssetId){
            emit String("LiFiAdapter: NOT argsLiFiData.sendingAssetId == argsPrepareArgs.invariantData.sendingAssetId");
        }
        if (argsLiFiData.sendingAssetId != tokenIn) {
            emit String("LiFiAdapter: NOT sendingAssetId == tokenIn");
        }
    }
}
