...
contract UniV3Rebalancer is OwnableUpgradeable, IUniV3Rebalancer {
  ...
  int24 thresholdMulti;
  int24 constant THRESHOLD_MULTI_MAX = 100;
  ...
  function thresholdRebalancePools() public onlyBenevolent {
    ...
    for (uint256 i = 0; i < dysonPools.length; i++) {
      ...
      if (!_inThresholdCalc(dysonStrategy, uniPool)) {
        dysonStrategy.rebalance();
        k++;
      }
    }
   ...
   * @return  bool  if in range, pool gets skipped
   */
  function _inThresholdCalc(IStrategyRebalanceStakerUniV3 dysonStrategy, IUniswapV3Pool uniPool)
    internal
    view
    returns (bool)
  {
    ...
    return
      currentTick > ((dysonStrategy.tick_lower() * (thresholdMulti)) / THRESHOLD_MULTI_MAX) &&
      currentTick < ((dysonStrategy.tick_upper() * thresholdMulti) / THRESHOLD_MULTI_MAX);
  }

  /**
   * @notice  the threshold calcuation
   * @dev     Simsala
   * @param   _dysonStrategy  dyson strategy to check
   * @return  _tickLower  the new threshold tick lower
   * @return  _tickUpper  the new threshold tick upper
   */
  function thresholdTick(IStrategyRebalanceStakerUniV3 _dysonStrategy)
    public
    view
    returns (int24 _tickLower, int24 _tickUpper)
  {
    _tickLower = (_dysonStrategy.tick_lower() * (thresholdMulti)) / THRESHOLD_MULTI_MAX;
    _tickUpper = (_dysonStrategy.tick_upper() * thresholdMulti) / THRESHOLD_MULTI_MAX;
  }
