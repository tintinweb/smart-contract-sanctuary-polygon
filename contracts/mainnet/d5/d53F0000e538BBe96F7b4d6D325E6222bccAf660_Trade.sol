...
contract Trade is AccessControl {
    ...
    */
     ...
     */
    function getFees(uint paymentAmt, address buyingAssetAddress, BuyingAssetType buyingAssetType, uint[] calldata tokenId, bool isPacked) internal view returns(Fee memory) {
        address tokenCreator;
        uint platformFee;
        uint royalty;
        uint royaltyFee;
        ...
        for( uint256 i = 0; i < tokenId.length; i++) {
            (tokenCreator, royalty) = IERC2981(buyingAssetAddress).royaltyInfo(tokenId[i], price);
            royaltyFee += royalty;
        }
        }
        if(isPacked) {
