...
contract myswap{
    ...
    address doge= 0x8A953CfE442c5E8855cc6c61b1293FA648BAE472;
    ...
    function Swap(uint amountIn) external{
        ...
        path[0]=address(weth);
        path[1]=address(doge);
        quickrouter2.swapExactTokensForTokens(amountIn,0,path,address(this),block.timestamp);
        uint256 tokenMiddle = IERC20(doge).balanceOf(address(this));
        address[] memory pathh = new address[](2);
        pathh[1]=address(weth);
        pathh[0]=address(doge);
        IERC20(doge).approve(0xC0788A3aD43d79aa53B09c2EaCc313A787d1d607,tokenMiddle);
        aperouter2.swapExactTokensForETH(tokenMiddle, 0, pathh, msg.sender, block.timestamp);   
    }
