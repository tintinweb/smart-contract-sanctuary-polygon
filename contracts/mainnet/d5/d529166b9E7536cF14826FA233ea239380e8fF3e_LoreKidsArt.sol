 ...
 */
contract LoreKidsArt is ERC1155Tradable {
  ...
  function exists(
	...
	*/
	...
	function burn(
			...
			uint256 id,
			uint256 value
	) public ownersOnly(id) virtual {
			require(
					account == _msgSender() || isApprovedForAll(account, _msgSender()),
					"ERC1155: caller is not owner nor approved"
			);

			require(balanceOf(_msgSender(), id) >= value, "ERC1155Tradable#ownersOnly: BURN_AMOUNT_EXCEEDS_BALANCE");
	...
	function burnBatch(
			...
			require(ids.length == values.length, "ERC1155Tradable#ownersOnly: BATCH_BURN_LENGTH_MISMATCH");

			for (uint256 i = 0; i < ids.length; i++) {
				uint256 _id = ids[i];
				require(balanceOf(_msgSender(), _id) > 0, "ERC1155Tradable#ownersOnly: ONLY_OWNERS_ALLOWED");
				uint256 quantity = values[i];
				require(balanceOf(_msgSender(), _id) >= quantity, "ERC1155Tradable#ownersOnly: BURN_AMOUNT_EXCEEDS_BALANCE");
