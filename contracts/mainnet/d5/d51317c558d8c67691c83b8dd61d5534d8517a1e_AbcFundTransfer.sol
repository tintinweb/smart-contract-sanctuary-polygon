...
contract AbcFundTransfer is Ownable{
    ...
    event MTransfer(uint256 id, address from, address to, address currency, uint256 amount, string memo, uint256 timestamp);
    ...
    function mTransfer(Order[] memory orders) public {
        for(uint256 i = 0; i < orders.length; i++) {
            ...
            IERC20(order.currency).safeTransferFrom(msg.sender, order.to, order.amount);
            emit MTransfer(order.id, msg.sender, order.to, order.currency, order.amount, order.memo, block.timestamp);
        }
    }

    function mTransferMatic(Order[] memory orders) public payable {
        for(uint256 i = 0; i < orders.length; i++) {
            Order memory order = orders[i];
            require(order.from == msg.sender, "order.from not same with sender.");        
            require(order.currency == address(0), "wrong currency.");
            require(address(this).balance >= order.amount, "Insufficient matic.");        
            payable(order.to).transfer(order.amount);
            emit MTransfer(order.id, msg.sender, order.to, order.currency, order.amount, order.memo, block.timestamp);
        }
    }
