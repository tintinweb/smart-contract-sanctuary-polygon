...
contract MagpieCore is ReentrancyGuard, Ownable, Pausable, IMagpieCore {
  ...
  function _wrapAssets(IMagpieRouter.SwapArgs memory swapArgs) internal returns (IMagpieRouter.SwapArgs memory newSwapArgs) {
  ...
  function _swap(IMagpieRouter.SwapArgs memory swapArgs, bool transferFromSender) internal returns(uint256[] memory amountOuts) {
    ...
    uint256 startingBalance = swapArgs.getToAssetId().getBalance();

    if (!fromAssetId.isNative() && transferFromSender) {
      fromAssetId.transferFrom(msg.sender, address(this), amountIn);
    }

    swapArgs.getFromAssetId().increaseAllowance(config.routerAddress, amountIn);
    ...
    require(wrappedToAssedId.getBalance() == startingBalance + amountOut, 'MagpieCore: invalid amountOut');
