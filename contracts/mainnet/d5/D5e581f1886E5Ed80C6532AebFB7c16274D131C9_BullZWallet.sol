...
contract BullZWallet is Ownable {
    ...
    function makeRequest(string memory reason, address token, uint amount) public {
        ...
        voteInfo[voteNum].votesFor.push(msg.sender);
    }
    ...
    function submitVote(uint voteID, bool myVote) public {
        ...
        //update tally
        if (myVote == true) {
            voteInfo[voteID].votesFor.push(msg.sender);
        } else {
            voteInfo[voteID].votesAgainst.push(msg.sender);
        }
    ...
    function withdraw(uint voteID) public {
        ...
        uint count = voteInfo[voteID].votesFor.length;
