...
// File: contracts/vaults/INonfungiblePositionManager.sol
...
// File: contracts/vaults/TransferHelper.sol
...
contract Management is Ownable {
    ...
    */
    function getDepositFee(uint256 _value) public view    
        ...
        _fee = Math.max((depositFeeRate * _value) /FACTOR_FEES_DECIMALS, minDepositFee);
    ...
    function calculateperformanceFee() external {
        ...
        if (tokenPrice > tokenPriceMean) {
            ...
            performanceFee = (token.totalSupply() *
    ...
    function calculatemanagementFee() external {
        ...
        if (managementFeeTime!= 0){
           ...
           managementFee = (token.totalSupply() * managementFeeRate * _deltaTime) 
...
// File: contracts/vaults/VaultUniswapV3.sol
...
/** 
* @author Formation.Fi.
* @notice Implementation of the contract VaultUniswapV3;
*/

contract VaultUniswapV3 is IERC721Receiver, Ownable {
    ...
    constructor(INonfungiblePositionManager _nonfungiblePositionManager, address _management,
        address _safeHouse) {
        require(
    ...
    modifier onlyManager() {
        address _manager = management.manager();
    ...
    function mintPosition(INonfungiblePositionManager.MintParams memory params, bool _isDefault) 
        ...
        params.deadline = block.timestamp + params.deadline;
    ...
    function collectAllFees(uint256 _tokenId, uint128 _amount0Max, 
        uint128 _amount1Max, bool _isDefault) external onlyManager 
    ...
    function decreaseLiquidity(uint256 _tokenId, uint128 _amount, 
        uint256 _amount0Min, uint256 _amount1Min, uint256 _deltaTime) external onlyManager 
        returns(uint256 _amount0, uint256 _amount1) {
        require(_amount<= deposits[_tokenId].liquidity, 
            "Formation.Fi: amount exccedes liquidity");
        INonfungiblePositionManager.DecreaseLiquidityParams memory params =
            INonfungiblePositionManager.DecreaseLiquidityParams({
                ...
                deadline: block.timestamp + _deltaTime
            });
    ...
    function increaseLiquidityCurrentRange(uint256 _tokenId, uint256 _amountAdd0,
        uint256 _amountAdd1,  uint256 _amount0Min, uint256 _amount1Min,
        uint256 _deltaTime) external onlyManager returns 
        (uint128 _liquidity, uint256 _amount0, uint256 _amount1) {
        TransferHelper.safeApprove(deposits[_tokenId].token0, address(nonfungiblePositionManager), _amountAdd0);
        TransferHelper.safeApprove(deposits[_tokenId].token1, address(nonfungiblePositionManager), _amountAdd1);
        ...
        (_liquidity, _amount0, _amount1) = nonfungiblePositionManager.increaseLiquidity(params);
        _createDeposit(_tokenId);
    }
    ...
    function sendNFTPositionToSafeHouse(uint256 _tokenId) onlyManager external {
        require (safeHouse!=address(0), 
            "Formation.Fi: zero address");
        nonfungiblePositionManager.safeTransferFrom(address(this), safeHouse, _tokenId);
        delete deposits[_tokenId];
    }


    function sendAssetToSafeHouse( address _asset,  uint256 _amount) external onlyManager {
        require(_amount > 0 , 
           "Formation.Fi: amount is zero");
        if (_asset == address(0)){
           require(_amount <= address(this).balance , 
           "Formation.Fi: balance limit");
           TransferHelper.safeTransferETH(safeHouse, _amount);
           
        }
        else{   
            require(_amount <= IERC20(_asset).balanceOf(address(this)) , 
           "Formation.Fi: balance limit");
           TransferHelper.safeTransfer(_asset, safeHouse, _amount);   
        }
    }
    ...
    function onERC721Received(
        ...
        address,
        uint256 tokenId,
        bytes calldata) external override returns (bytes4) {
    ...
    function _createDeposit(uint256 _tokenId) internal {
        ...
        deposits[_tokenId] = Deposit({liquidity: _liquidity, token0: _token0, token1: _token1});
