...
contract PortTokenFactoryFacet is Modifiers {
    ...
    function createPortToken(string calldata _name, string calldata _symbol) external whenNotPaused returns (address) {
...
library LibPriceInterface {
    ...
    function getTokenPrice(address _addr, uint256 _amount) internal view returns (uint256 price) {
        ...
        if (os.feedType == 1) {
            ...
            price = IOffchainOracle(os.feed).getRate(IERC20(_addr), IERC20(ds.baseToken), true)
                * _amount 
                / (10**ERC20(_addr).decimals());
        }
    }
    ...
    function getTokenForPrice(address _addr, uint256 _amount) internal view returns (uint256 price) {
        ...
        if (os.feedType == 1) {
            ...
            price = IOffchainOracle(os.feed).getRate(IERC20(ds.baseToken), IERC20(_addr), true)
                * _amount
                / 10**ERC20(_addr).decimals();
