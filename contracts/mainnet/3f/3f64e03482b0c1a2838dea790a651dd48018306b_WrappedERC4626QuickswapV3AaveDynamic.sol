...
contract WrappedERC4626QuickswapV3AaveDynamic is
...
abstract contract BaseDynamicallyHedgedConcentratedLiquidityStrategy is
    BaseHedgedConcentratedLiquidityStrategy
{
    ...
    function rehedge() public {
        ...
        uint256 borrowTokenAmount = (aave.tokenToBorrow == token0())
            ? amount0
            : amount1;
        ...
        if (borrowTokenAmount > currentDebt) {
            ...
            uint256 amountToWithdraw = pricesOracle.convert(
                aave.tokenToBorrow,
                aave.collateral,
                amountToRepay
            );
...
import "../_lib/quickswap/contracts/interfaces/IAlgebraPool.sol";
import "../_lib/quickswap/contracts/interfaces/INonfungiblePositionManager.sol";
import "../_lib/quickswap/contracts/interfaces/IFarmingCenter.sol";
import "../_lib/quickswap/contracts/interfaces/IIncentiveKey.sol";
    ...
    BaseConcentratedLiquidityStrategy
{
    ...
    function _readdLiquidity() internal virtual override {
        ...
        uint256 amountToRepay = (currentDebt > neededDebt)
            ? currentDebt - neededDebt
            : 0;
        uint256 amountToWithdraw = (currentCollateral > neededCollateral)
            ? currentCollateral - neededCollateral
            : 0;
        uint256 amountToBorrow = (neededDebt > currentDebt)
            ? neededDebt - currentDebt
            : 0;
        uint256 amountToSupply = (neededCollateral > currentCollateral)
            ? neededCollateral - currentCollateral
            : 0;

        uint256 amountToSwap;
        if (amountToRepay > 0) {
            if (tokenToBorrowBalance < amountToRepay) {
                amountToSwap = pricesOracle.convert(
                    aave.tokenToBorrow,
                    aave.collateral,
        ...
        if (tokenToBorrowBalance > neededDebt) {
            ...
            amountToSwap = Math.min(pricesOracle.convert(aave.tokenToBorrow, aave.collateral, neededDebt - tokenToBorrowBalance), collateralBalance);
            assetConverter.safeSwap(
                aave.collateral,
                aave.tokenToBorrow,
                amountToSwap
