...
contract KDKDTokenTEST is ERC2771Context, Ownable, Pausable, ERC1155Burnable, ERC1155URIStorage {
    string private _name;
    string private _symbol;

    constructor(string memory name_, string memory symbol_, string memory uri_, MinimalForwarder forwarder) ERC1155(uri_) ERC2771Context(address(forwarder)) {
        _name = name_;
        _symbol = symbol_;
    }
    ...
    function mint(address account, uint256 id, string memory tokenURI, uint256 amount, bytes memory data)
    public
    onlyOwner
    {
    ...
    function mintBatch(address to, uint256[] memory ids, string[] memory tokenURIs, uint256[] memory amounts, bytes memory data)
    public
    onlyOwner
    {
