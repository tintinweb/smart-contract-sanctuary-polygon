...
contract CMF is AccessControlEnumerable,Pausable,ICMF {
    ...
    function recharge(uint256 types,uint256 price,uint256 orderId) public{
        ...
        uint256 oId = userRecharge.length + 1;
        uint256 beiDay = userRecharge.length/10;
        if(beiDay>6){
            beiDay = 6;
        }
        uint256 endtime = block.timestamp + 6*Config.timeStep + beiDay*2*Config.timeStep;
        uint256 profit = price * Config.rechargePercent / 10000;
        userRecharge.push(RechargeStruct(oId,types,_msgSender(),price,block.timestamp,endtime,profit,1));
        if(types == 3){
   ...
   function rechargeDay(uint256 price) public{
    ...
    function redeem(uint256 orderId) private{
        ...
        require(userRecharge.status == 1, "Redeemed");
        require(userRecharge.endtime < block.timestamp, "Redeem endtime");
        userRecharge.status = 2;
        //赎回返佣 
        //利息的30% 进积分账户
    ...
    function withdrawCredit(address owner,bytes memory credittype,uint256 price) public{
        ...
        require(keccak256(CREDIT1_NAME) != keccak256(credittype), "PRICE error");
        //分2笔
        //30 % 到 积分账户
    ...
    function payLevelEnd() public{
        ...
        payUserInfo[_msgSender()].endtime = block.timestamp + day * Config.timeStep;
    ...
    * 获取充值记录
    ...
    function _profit(address owner,uint256 price) private{
        ...
        while(true){
            level++;
            address parentAddress = userReferrer[curAddress];
            if (parentAddress == address(0) || userStatusValid[parentAddress] == 0) {
                break;
            }
            curAddress = parentAddress;
