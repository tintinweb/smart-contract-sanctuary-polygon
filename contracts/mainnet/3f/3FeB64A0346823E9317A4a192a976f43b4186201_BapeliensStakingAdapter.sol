...
contract BapeliensStakingAdapter is Ownable2Step {
    event BapeliensStakingSet(address indexed bapeliensStaking);

    StakingPoolsERC721 public bapeliensStaking;

    constructor(address _bapeliensStaking) {
        bapeliensStaking = StakingPoolsERC721(_bapeliensStaking);
    }

    function setBapeliensStaking(address _bapeliensStaking) external onlyOwner {
        if (_bapeliensStaking == address(0)) revert Errors.ZeroAddress();
        bapeliensStaking = StakingPoolsERC721(_bapeliensStaking);
        emit BapeliensStakingSet(_bapeliensStaking);
    }

    function _getStakedTokenBalance(address _owner) internal view returns (uint256) {
        if (address(bapeliensStaking) == address(0)) revert Errors.ZeroAddress();
        return bapeliensStaking.totalStakedForOwner(_owner);
    }

    function getStakedTokenIds(address _owner) external view returns (uint256[] memory) {
        uint256 balance = _getStakedTokenBalance(_owner);
        uint256[] memory stakedTokens = new uint256[](balance);
        for (uint256 i = 0; i < balance; ) {
            (stakedTokens[i],,,) = bapeliensStaking.stakedTokens(_owner, i);
    ...
    function getStakedTokenBalance(address _owner) external view returns (uint256) {
        return _getStakedTokenBalance(_owner);
    }
}