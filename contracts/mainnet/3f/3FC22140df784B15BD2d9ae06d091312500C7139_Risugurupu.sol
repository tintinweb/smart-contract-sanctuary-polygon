// SPDX-License-Identifier: MIT
    ...
    pragma abicoder v2;
      ...
      // @title:      Risugurūpu NFT
      // @twitter:    https://twitter.com/risugurupu
...
───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
    ...
    contract Risugurupu is ERC721, Ownable, ERC721Enumerable {
      using SafeMath for uint256;
      using Strings for uint256;
    
      uint256 public tokenPrice = 2000000000000000; // 0.02 ETH 
      ...
      event DickMinted(uint256 tokenId, address owner);
    
      constructor() ERC721("Risugurupu", "Risugurupu") {}
      ...
      function risuMint(uint256 numberOfTokens) external payable {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
          ...
          if (totalSupply() < MAX_TOKENS) {
            ...
            emit DickMinted(id, msg.sender);
          }
        }
      }
    
      function presaleRisu(uint256 numberOfTokens) external payable {
        ...
        returns (string memory)
      {
        ...
        if (isRevealed == false) {
          return
            "ipfs://QmfMSVmsUj4vfcKHc4MA8uqwUBV1a7rwjFBVf9zizhigNB/hidden.json";
        }
