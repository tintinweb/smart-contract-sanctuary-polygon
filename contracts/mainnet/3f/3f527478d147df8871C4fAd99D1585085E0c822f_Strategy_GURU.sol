...
/*
    ...
    GURU token uses for staking.
    MATIC -> WMATIC -> WETH -> DAI -> GURU routing
    Token Helper = "Staked GURU (sGURU)"
*/
contract Strategy_GURU is Ownable {
    ...
    address public lpToken = 0x5Cf66CeAf7F6395642cD11b5929499229edEF531; // GURU - 9 dec : Dai - 18 dec
    address public vault;
    address public mainToken = 0x04568467f0AAe5fb85Bf0e031ee66FF2C200a6Fb; // sGURU decimals 9
    ...
    address public GURU = 0x057E0bd9B797f9Eeeb8307B35DbC8c12E534c41E; // token0 - decimals 9
    ...
    address public constant stakingHelper = 0x14d9E4bCc1791aEf83170c6876629F7E60c9Ba1c; // stake
    address public constant staking = 0x4Eef9cb4D2DA4AB2A76a4477E9d2b07f403f0675; // unstake
    // ------------>        END          <--------------
    ...
    function earn() public returns(uint256 _balance) {
        _balance = getBalanceOfToken(GURU);
        if(_balance > 0) {
            _approveToken(GURU, stakingHelper);
    ...
    function emergencyWithdraw(address _receiver) public onlyVaultOrOwner {
        ...
        _transfer(GURU, _receiver, getBalanceOfToken(GURU));
    ...
    function deposit() public payable onlyVault returns (uint256) {
        ...
        _swapExactTokenForToken(router, DAI, GURU, getBalanceOfToken(DAI));
        _updateLastPrices();
    ...
    function getBalanceOfToken(address _token) public view returns (uint256) {
        ...
        if(_token == GURU) {
            return IERC20(_token).balanceOf(address(this)) - pendingFeeGURU;
        }
        if(_token == DAI) {
    ...
    function setLP(address _address) public onlyVaultOrOwner {
        require(_address != address(0), "The address can not be zero address");
        require(GURU == IPair(_address).token0() && DAI == IPair(_address).token1(), "Tokens don't match");
    ...
    function takeFee(uint256 _total) external onlyVault {
        ...
        _amount = getBalanceOfToken(GURU) + pendingFeeGURU;
        bool result = _swapExactTokenForToken(router, GURU, DAI, _amount);
        if(result) {
            emit TakeFee(_amount);
            pendingFeeGURU = 0;
        } else {
            pendingFeeGURU = _amount;
        }
    ...
    function _swapTokenToWrapped(uint256 _total, uint256 _percent) internal {
        ...
        _swapExactTokenForToken(router, GURU, DAI, getBalanceOfToken(GURU));
    ...
    function _getWethPerDaiPrice() internal view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = WETH;
        path[1] = DAI;
    ...
    function _getWrappedPerWethPrice() internal view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = wrappedToken;
        path[1] = WETH;
