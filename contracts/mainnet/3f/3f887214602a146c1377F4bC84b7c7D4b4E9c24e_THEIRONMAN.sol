...
contract THEIRONMAN is ERC721  {
    ...
    constructor()
        ERC721("IRON MAN FANMADE", "IMFM")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 4001) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeif7sompmffqgde7pzdvcfrhp22qpcabvl6e5ogalswqfrf3lhktne/", Strings.toString(tokenId), ".json")
            );
    }
