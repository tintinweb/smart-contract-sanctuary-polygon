 ...
 * @title Crowdsale
 */
contract CrowdsaleTFTToken is ICrowdsale, Ownable, Pausable, ReentrancyGuard {
    ...
    /**
     * @dev Getter for the total TFT sold.
     */
    ...
    /**
     * @dev Getter for bought TFT amounts by user.
     */
     ...
     * @param TFT_ The selling TFT address;
     ...
     */
    constructor(address BUSD_, address USDT_, address USDC_, address TFT_, address pancakeRouter_, address walletFactory_) {
        BUSD = BUSD_;
        USDT = USDT_;
        USDC = USDC_;
        TFT = TFT_;
     ...
     *
     * @param token For what token user want buy TFT. Can be BUSD\USDT\USDC\0x0. Use 0x0 and send value to buy TFT for BNB.
     *  USDT\USDC\BNB will be changed to BUSD 'on the fly';
     * @param amountIn Amount for which user want to buy TFT;
     ...
     * @param referrer The referrer, if present. If possible will be set in the TFT token too, to get rewards from future
     *  transfers.
     *
    ...
    function _buy(address token, uint256 amountIn, uint256 minAmountOut, address referrer) internal {
        ...
        IBEP20 erc20Impl = IBEP20(TFT);
        uint256 decimals = erc20Impl.decimals();

        uint256[] memory amountTFTOuts = new uint256[](4);
        for (uint256 i = 0; i < _vestingManagers.length; ++i) {
            ...
            amountTFTOuts[0] = amountOut;
            amountTFTOuts[1] = (amountOut * 500) / PERCENTAGE_DENOM;
            amountTFTOuts[2] = (amountOut * 300) / PERCENTAGE_DENOM;
            amountTFTOuts[3] = (amountOut * 200) / PERCENTAGE_DENOM;

            _execute(_vestingManagers[i].vestingManager, msg.sender, amountTFTOuts);
        }
    }
    ...
    function _execute(address vestingManager, address beneficiary, uint256[] memory amountTFTOuts) private {
        ...
        totalSold += amountTFTOuts[0];
        emit TokenTransferred(allLevelsVestingWallets[0], amountTFTOuts[0]);
        emit TokenSold(beneficiary, amountTFTOuts[0]);

        IERC20 erc20Impl = IERC20(TFT);

        bought[beneficiary] += amountTFTOuts[0];
        
        erc20Impl.safeTransfer(allLevelsVestingWallets[0], amountTFTOuts[0]);
        for (uint256 i = 1; i < allLevelsVestingWallets.length; ++i) {
            if (allLevelsVestingWallets[i] == address(0)) {
                break;
            }
            totalEarned += amountTFTOuts[i];
            emit RewardEarned(allLevelsVestingWallets[i], amountTFTOuts[i], i);
            rewards[allLevelsReferrers[i]] += amountTFTOuts[i];
            erc20Impl.safeTransfer(allLevelsVestingWallets[i], amountTFTOuts[i]);
        }
    }
...
interface ICrowdsale {
    ...
    function TFT() external view returns (address);
