...
import "../interfaces/IPairVaultOperatorActionsV2.sol";

import "./UniversePairVaultStorage.sol";
import "./UniswapHelper.sol";


contract UniversePairVaultV2 is
    IPairVaultOperatorActionsV2,
    IVaultEvents,
    ERC721Holder,
    Ownable,
    UniversePairVaultStorage
{

    using SafeERC20 for IUToken;
    using SafeMath for uint256;
    using PositionNFTForPair for PositionNFTForPair.Position;
    ...
    function initialize(
        ...
        token0 = IUToken(pool.token0());
        token1 = IUToken(pool.token1());
        poolMap[_poolAddress] = true;

        config = IUniversePairVaultConfig(_config);
        nftManager = IPositionNFTManager(_nftManager);

        token0.approve(_nftManager, uint(- 1));
        token1.approve(_nftManager, uint(- 1));

        setInitPos(_poolAddress, pool.tickSpacing());
    }
    ...
    // To reduce the contract size
    function operate(uint8 otype, address p0, address p1) external onlyOwner {
        require(p0 != address(0));
        if(otype == 0){  //changeManager
            operator = p0;
            emit ChangeManger(operator);
        }else if(otype == 1){ // setUTokens
            require(address(shareToken) == address(0), "2");
            shareToken = IUToken(p0);
        }else if(otype < 4){ // 2 add whiteList; 3 del whiteList
            bool status = (otype == 2 ? true : false);
            contractWhiteLists[p0] = status;
            emit UpdateWhiteList(p0, status);
        }else if(otype == 4){  //withdraw fee
            withdrawPerformanceFee(p0);
        }else if(otype == 5){  //claim v3Stake Reward
            claimStakeReward(IERC20Minimal(p0), p1);
        }

    }

    function withdrawPerformanceFee(address to) internal{
        ProtocolFees memory pf = protocolFees;
        if (pf.fee0 > 1) {
            token0.safeTransfer(to, pf.fee0 - 1);
            pf.fee0 = 1;
        }
        if (pf.fee1 > 1) {
            token1.safeTransfer(to, pf.fee1 - 1);
            pf.fee1 = 1;
        }
        protocolFees = pf;
    }
    ...
    function claimStakeReward(
        ...
        PositionNFTForPair.Position memory _pos;
        for (uint i = 0; i < positionList.length; i++) {
            _pos = positionList[i];
            if(_pos.tokenId > 0){
                PositionNFTForPair.nftUnStakeAll(config, _pos.tokenId);
                PositionNFTForPair.nftStakeAll(config, address(nftManager), _pos);
            }
        }
        PositionNFTForPair.v3Staker.claimReward(rewardToken, to, 0);
    }
    ...
    function _sub128(uint128 a, uint128 b) internal pure returns (uint128) {
        require(b <= a, "5");
        return a - b;
    }
    ...
    function _quantityTransform(
        ...
        if (newAmt != 0) {
            newShare = (totalShare == 0 ? newAmt :  FullMath.mulDiv(newAmt, totalShare, totalAmt));
        }
    }
    ...
    function _balance0() internal view returns (uint256) {
        return token0.balanceOf(address(this)).sub(protocolFees.fee0);
    }

    /// @dev Get the pool's balance of token1 Belong to the user
    function _balance1() internal view returns (uint256) {
        return token1.balanceOf(address(this)).sub(protocolFees.fee1);
    }
    ...
    function _calcShare(
        ...
        if (totalShare == 0) {// First Time
            ...
            if(total0 == 0){
                amount0Desired = 0;
                share = FullMath.mulDiv(totalShare, amount1Desired, total1);
            } else if(total1 == 0){
                amount1Desired = 0;
                share = FullMath.mulDiv(totalShare, amount0Desired, total0);
            } else if(amount0Desired.mul(total1) > amount1Desired.mul(total0)){
    ...
    function _calcBal(
        ...
        bal0 = _quantityTransform(share, total0, totalShare);
        bal1 = _quantityTransform(share, total1, totalShare);
    }
    ...
    function _getTotalAmounts() internal view returns (
        ...
        (free0, free1) = (_balance0(), _balance1());
        (total0, total1) = (free0, free1);
        uint256 amount0;
        uint256 amount1;
        PositionNFTForPair.Position memory _pos;
        for (uint256 i = 0; i < positionList.length; i++) {
            _pos = positionList[i];
            if (_pos.liquidity > 0) {
                (amount0, amount1) = nftManager.getTotalAmounts(_pos.tokenId, config.performanceFee());
                (total0, total1) = (total0.add(amount0), total1.add(amount1));
            }
        }
    }
    ...
    function getShares(
        uint256 amount0Desired,
        uint256 amount1Desired
    ) external view returns (uint256 share0, uint256) {
        (share0,,) = _calcShare(amount0Desired, amount1Desired);
    }

    function getBalancedAmount(
        uint256 amount0Desired,
        uint256 amount1Desired
    ) external view returns (uint256 share, uint256 amount0, uint256 amount1) {
    ...
    /// @dev For Frontend
    function getUserShares(address user) external view returns (uint256 share0, uint256) {
        share0 = shareToken.balanceOf(user);
    }
    ...
    /// @dev For Frontend
    function getUserBals(address user) external view returns (uint256, uint256) {
        return _calcBal(shareToken.balanceOf(user));
    }
    ...
    function _collectPerformanceFee(
        ...
        uint256 rate = config.performanceFee();
        if (rate != 0) {
            ProtocolFees memory pf = protocolFees;
            if (feesFromPool0 > 0) {
    ...
    function _swapAndAddAll(PositionNFTForPair.Position memory _pos, uint256 idx) internal{
        if(_pos.tokenId > 0){
            (uint256 reserve0, uint256 reserve1) = _mockReserve(_pos);
            _swap(_balance0(), _balance1(), reserve0, reserve1, _pos.poolAddress);
            //unstake nft from v3 staker
            PositionNFTForPair.nftUnStakeAll(config, _pos.tokenId);
            // add liquidity
            (,, uint128 liquidity) = nftManager.addAll(_pos.tokenId, _balance0(), _balance1());
            // update pos
            _pos.liquidity =  _add128(_pos.liquidity, liquidity);
            positionList[idx] = _pos;
            //stake nft again
            PositionNFTForPair.nftStakeAll(config, address(nftManager), _pos);
        }
    }

    function _swap(uint256 bal0, uint256 bal1, uint256 reserve0, uint256 reserve1, address _swapPool) internal {
        (uint160 sqrtRatioX96, , , , , ,) = IUniswapV3Pool(_swapPool).slot0();
        uint256 priceX96 = FullMath.mulDiv(sqrtRatioX96, sqrtRatioX96, FixedPoint96.Q96);
        uint256 dividend; uint256 divisor;
        uint256 amt; bool zeroForOne;
        if (bal0.mul(reserve1) >= bal1.mul(reserve0)) {
            dividend = bal0.mul(reserve1) - bal1.mul(reserve0);
            divisor = FullMath.mulDiv(priceX96, reserve0, FixedPoint96.Q96).add(reserve1);
            amt = dividend.div(divisor);
            zeroForOne = true;
        } else {
            dividend = bal1.mul(reserve0) - bal0.mul(reserve1);
            divisor = FullMath.mulDiv(reserve1, FixedPoint96.Q96, priceX96).add(reserve0);
            amt = dividend.div(divisor);
        }
        if (amt > 0) {
            uint160 sqrtPriceLimitX96 = (zeroForOne ? TickMath.MIN_SQRT_RATIO + 1 : TickMath.MAX_SQRT_RATIO - 1);
            IUniswapV3Pool(_swapPool).swap(address(this), zeroForOne, int256(amt), sqrtPriceLimitX96, '');
        }
    }

    function _mockReserve(PositionNFTForPair.Position memory _pos) internal view returns (uint256 reserve0, uint256 reserve1){
        // calculate token0/token1
        (uint160 sqrtPrice, , , , , ,) = IUniswapV3Pool(_pos.poolAddress).slot0();
        (reserve0, reserve1) = LiquidityAmounts.getAmountsForLiquidity(
            sqrtPrice,
            TickMath.getSqrtRatioAtTick(_pos.lowerTick),
            TickMath.getSqrtRatioAtTick(_pos.upperTick),
            1E20);
    }
    ...
    function _deposit(
        ...
        // add share
        shareToken.mint(to, share);
        // Invest All
        PositionNFTForPair.Position memory _pos = positionList[0];
        _swapAndAddAll(_pos, 0);
        // EVENT
        emit Deposit(to, share, 0, amount0Desired, amount1Desired);
    }

    /// @dev deposit all and get a nft
    function _mintAll(PositionNFTForPair.Position memory _pos, int24 _lowerTick, int24 _upperTick, uint256 idx) internal {
        (_lowerTick, _upperTick) = UniswapHelper.tickRegulate(_lowerTick, _upperTick, _pos.tickSpacing);
        _pos.lowerTick = _lowerTick;
        _pos.upperTick = _upperTick;
        address poolAddress = _pos.poolAddress;
        (uint256 reserve0, uint256 reserve1) = _mockReserve(_pos);
        _swap(_balance0(), _balance1(), reserve0, reserve1, _pos.poolAddress);
        (uint256 tokenId, uint256 amount0, uint256 amount1, uint128 liquidity)
                        = nftManager.mintAll(poolAddress, _balance0(), _balance1(), _lowerTick, _upperTick, address(this));
        require(tokenId > 0, "0l");
        _pos.tokenId = tokenId;
        _pos.liquidity = liquidity;
        _pos.positionTick = UniswapHelper.currentTick(poolAddress);
        positionList[idx] = _pos;
        PositionNFTForPair.nftStakeAll(config, address(nftManager), _pos);
    }
    ...
    function _multiStopMining(
        ...
        for (uint i = 0; i < positionList.length; i++) {
            ...
            if (_pos.liquidity > 0){
                    PositionNFTForPair.nftUnStakeAll(config, _pos.tokenId);
                    uint128 liq = _toUint128(FullMath.mulDiv(uint256(_pos.liquidity), share, totalShare));
                    (uint256 _amt0, uint256 _amt1, uint256 _fee0, uint256 _fee1) = nftManager.burn(_pos.tokenId, liq);
                    amt0 = amt0.add(_amt0);
                    amt1 = amt1.add(_amt1);
                    fee0 = fee0.add(_fee0);
                    fee1 = fee1.add(_fee1);
                    _pos.liquidity = _sub128(_pos.liquidity, liq);
                    positionList[i] = _pos;
                    PositionNFTForPair.nftStakeAll(config, address(nftManager), _pos);
            }
        }
        (amt0, amt1) =  (amt0.sub(fee0), amt1.sub(fee1));
        // collect fee
        (uint256 feesToProtocol0, uint256 feesToProtocol1) = _collectPerformanceFee(fee0, fee1);
        (fee0, fee1) = (fee0.sub(feesToProtocol0), fee1.sub(feesToProtocol1));
        (amt0, amt1) = (
            amt0.add(FullMath.mulDiv(fee0, share, totalShare)),
            amt1.add(FullMath.mulDiv(fee1, share, totalShare))
        );
    }
    ...
    function setInitPos(
        ...
        PositionNFTForPair.Position memory  _pos = PositionNFTForPair.Position({
            tokenId: 0,
        ...
        positionList.push(_pos);
    }

    function burnAndCollectFee(PositionNFTForPair.Position memory _pos, uint128 liq) internal returns(uint256 fee0, uint256 fee1) {
        if(_pos.tokenId > 0){
            PositionNFTForPair.nftUnStakeAll(config, _pos.tokenId);
            (, , fee0, fee1) = nftManager.burn(_pos.tokenId, liq);
            PositionNFTForPair.nftStakeAll(config, address(nftManager), _pos);
    ...
    function burnAllAndCollectFee(PositionNFTForPair.Position memory _pos) internal returns(uint256 fee0, uint256 fee1) {
        if(_pos.tokenId > 0){
            PositionNFTForPair.nftUnStakeAll(config, _pos.tokenId);
            (, , fee0, fee1) = nftManager.burnAll(_pos.tokenId);
            _collectPerformanceFee(fee0, fee1);
            _pos.tokenId = 0;
            _pos.liquidity = 0;
        }
    ...
    function withdraw(uint256 share0, uint256 share1) external returns (uint256 withdraw0, uint256 withdraw1) {
        return withdraw(share0);
    }

    /// @notice Withdraw token by user
    /// @param share The share amount of user
    function withdraw(uint256 share) public returns (uint256 amount0, uint256 amount1){
        // get user share
        uint256 maxShare = shareToken.balanceOf(msg.sender);
        if (share > maxShare) {
            share = maxShare;
        }
        require(share > 0, "0");
    ...
    /// @inheritdoc IPairVaultOperatorActionsV2
    function initPosition(
        int24 _lowerTick,
        int24 _upperTick
    ) external override onlyManager {
        PositionNFTForPair.Position memory _pos  = positionList[0];
        require(_pos.tokenId == 0, 'p');
        // add liquidity
        _mintAll(_pos, _lowerTick, _upperTick, 0);
    }
    ...
    function addPool(uint24 _poolFee) external override onlyManager {
        address poolAddress = PoolAddress.computeAddress(
            ...
            PoolAddress.getPoolKey(address(token0), address(token1), _poolFee)
        );
    ...
    function avoidRisk(
        ...
        for (uint256 i = 0; i < idxs.length; i++) {
            _pos = positionList[i];
            burnAllAndCollectFee(_pos);
            positionList[i] = _pos;
        }
        if (r0 !=0 || r1 != 0) {
            _swap(_balance0(), _balance1(), r0, r1, positionList[0].poolAddress);
        }
    }
    ...
    function changePool(
        ...
        // read in memory
        PositionNFTForPair.Position memory _pos = positionList[idx];
        // check attack
        UniswapHelper.checkDiffTick(_pos.poolAddress, _spotTick, config.diffTick());
        burnAllAndCollectFee(_pos);
        // new pool info
        ...
        // add liquidity
        _mintAll(_pos, _lowerTick, _upperTick, idx);
    }
    ...
    function reBalance(
        ...
        PositionNFTForPair.Position memory _pos = positionList[idx];
        require(band > 0 && reBalanceThreshold > 0 && _pos.tickSpacing > 0, "b");
        // Read status in uniPool
        int24 tickNow = UniswapHelper.currentTick(_pos.poolAddress);
        if (_pos.liquidity == 0 ||
            _pos.positionTick - tickNow >= reBalanceThreshold ||
            tickNow - _pos.positionTick >= reBalanceThreshold) {
            tickNow = UniswapHelper.floor(tickNow, _pos.tickSpacing);
            forceReBalance(idx, tickNow - band, tickNow + band, _spotTick);
        }
    }
    ...
    function adjustMining(
        ...
        PositionNFTForPair.Position memory fromP = positionList[fromIdx];
        require(liq > 0 && fromP.liquidity >= liq, "fp");
        // Withdraw First
        burnAndCollectFee(fromP, liq);
        positionList[fromIdx] = fromP;
        // Deposit
        PositionNFTForPair.Position memory toPos = positionList[toIdx];
        UniswapHelper.checkDiffTick(toPos.poolAddress, _spotTick, config.diffTick());
        if (toPos.tokenId > 0) {
            burnAllAndCollectFee(toPos);
        }
        _mintAll(toPos, lowerTick, upperTick, toIdx);
    }

    /// @inheritdoc IPairVaultOperatorActionsV2
    function addPosition(address poolAddress) external override onlyManager {
        require(poolMap[poolAddress], "p");
        setInitPos(poolAddress, IUniswapV3Pool(poolAddress).tickSpacing());
    }

    /// @inheritdoc IPairVaultOperatorActionsV2
    function reInvest(uint256 idx) external override onlyManager {
        PositionNFTForPair.Position memory _pos = positionList[idx];
        burnAndCollectFee(_pos, 0);
        //swap add liquidity
        _swapAndAddAll(_pos, idx);
    }
    ...
    function nftStake(IUniswapV3Staker.IncentiveKey memory key) external {
        ...
        for (uint256 i = 0; i < positionList.length; i++) {
            _pos = positionList[i];
            if (_pos.tokenId > 0 && _pos.poolAddress == address(key.pool) && block.timestamp >= key.startTime && block.timestamp < key.endTime) {
                PositionNFTForPair.nftStake(config, key, address(nftManager), _pos);
            }
        }
    }
    ...
    function nftUnStake(IUniswapV3Staker.IncentiveKey memory key) external {
        ...
        for (uint256 i = 0; i < positionList.length; i++) {
            _pos = positionList[i];
            if (_pos.tokenId > 0) {
                PositionNFTForPair.nftUnStake(key, _pos.tokenId);
            }
        }
    ...
    function onERC721Received(
        ...
        PositionNFTForPair.positionManager.approve(address(nftManager), tokenId);
...
interface IPairVaultOperatorActionsV2 {

    function initPosition(int24, int24) external;
    ...
    function reInvest(uint256 idx) external;
...
library UniswapHelper {
    ...
    function checkDiffTick(address poolAddress, int24 _spotTick, uint24 _diffTick) internal view {
        ...
        require(tick - _spotTick < int24(_diffTick) && _spotTick - tick < int24(_diffTick), "DF");
    }

    function floor(int24 tick, int24 _tickSpacing) internal pure returns (int24) {
    ...
    function tickRegulate(
        ...
        lowerTick = floor(_lowerTick, tickSpacing);
        upperTick = floor(_upperTick, tickSpacing);
...
interface IUniversePairVaultConfig {
    ...
    function pairVaultParam(address vaultAddress) external view returns(PairVaultParam memory param);
...
library PositionNFTForPair {
    ...
    function nftStakeAll(IUniversePairVaultConfig config,  address nftManagerAddress, Position memory pos) internal{
        //get active Incentive List
        IUniswapV3Staker.IncentiveKey[] memory activeList = config.findActiveIncentive(address(this), pos.poolAddress);
        if(activeList.length > 0){
            if(pos.liquidity > 0 ){
                positionManager.safeTransferFrom(address(this), address(v3Staker), pos.tokenId, abi.encode(activeList));
            }else{
    ...
    // @dev stake to Incentive
    function nftUnStakeAll(IUniversePairVaultConfig config, uint256 tokenId) internal{
...
interface IPositionNFTManager {
    ...
    function mintAll(
        ...
        int24 upperTick,
        address to
    ) external returns(uint256 tokenId, uint256 amount0, uint256 amount1, uint128 liquidity);
