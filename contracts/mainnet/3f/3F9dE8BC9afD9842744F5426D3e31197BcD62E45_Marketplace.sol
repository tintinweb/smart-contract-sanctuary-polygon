    ...
    BasicMetaTransaction
{
    ...
    IERC20Upgradeable public usdcContractAddress;
    ...
    function setUSDCAddress(address _usdcAddress) external onlyOwner {
        require(
            ...
            "setUSDCAddress: USDC address can not be zero address"
        );
        usdcContractAddress = IERC20Upgradeable(_usdcAddress);
    }

    /**@dev switch flag to enable/disable use of crypto */
    function switchCryptoFlag() external onlyOwner {
        cryptoFlag = !cryptoFlag;
    }

    /**@notice fallback function for receiving unkown funds */
    ...
    function listProperty(
        ...
        require(amount > 0, "listProperty : Amount must be greater than 0!");
        require(
            mogul.balanceOf(_msgSender(), tokenId) >= amount,
            "listProperty: Caller must own given token!"
        );
        require(
            fundReceiverAddress != address(0),
            "list: fund receiver address cannot be zero address"
        );

        if (
            cryptoFlag == false &&
            (mode == PaymentOptions.MATIC || mode == PaymentOptions.USDC)
        ) {
            revert ListProperty__CryptoPaymentDisabled();
        }
    ...
    function buyProperty(
        ...
        if (idToProperty[listingId].mode == PaymentOptions.MATIC) {
            require(
                msg.value >= (idToProperty[listingId].price * amount),
                "buy: Insufficient matic funds provided"
            );
            payable(idToProperty[listingId].fundReceiverAddress).transfer(
                msg.value
            );
        } else if (idToProperty[listingId].mode == PaymentOptions.USDC) {
            require(
                usdcContractAddress.transferFrom(
                    _msgSender(),
                    idToProperty[listingId].fundReceiverAddress,
                    idToProperty[listingId].price * amount
                ),
                "buy: usdc transfer failed"
            );
        }
        ...
        emit PropertySold(
            ...
            idToProperty[listingId].listingId,
            idToProperty[listingId].mode
        );
     ...
     */
    function deListProperty(bytes32[] memory proof, uint256 _listingId)
        external
        nonReentrant
    {
        require(
