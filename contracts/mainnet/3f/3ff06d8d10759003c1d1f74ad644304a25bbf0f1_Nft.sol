...
contract Nft is ERC721, ERC721URIStorage, Ownable {
    ...
    /// NftAlreadyOwned error
    error NftAlreadyOwned(address receiver);
    /// NftMinted event
    ...
    function tokenIdOf(address _owner) external view returns (uint256 tokenId) {
        tokenId = _tokenIdOf[_owner];
    }
    ...
    function safeMint(address _to) external onlyOwner returns (uint256 tokenId) {
    ...
    function safeMintBatch(address[] calldata _to) external onlyOwner returns (uint256[] memory tokenIds) {
        ...
        for (uint256 i = 0; i < _to.length; i++) {
            ...
            emit NftMinted(_to[i], tokenIds[i]);
        }
    }
    ...
    {
        if (balanceOf(_to) > 0) {
            revert NftAlreadyOwned(_to);
        }
        _tokenIdOf[_to] = _tokenId;
