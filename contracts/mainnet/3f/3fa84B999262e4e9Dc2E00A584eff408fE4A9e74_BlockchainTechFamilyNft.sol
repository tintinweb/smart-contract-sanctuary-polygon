...
contract BlockchainTechFamilyNft is Initializable, ERC721Upgradeable, OwnableUpgradeable {
    using StringsUpgradeable for uint256;
    ...
    function initialize(string memory baseUri_, string memory name_, string memory symbol_) public initializer {
    ...
    modifier onlyContractOwnerOrNftOwnerTransferringToContractOwner(address to, uint256 tokenId) {
        require(
            msg.sender == owner() || (to == owner() && (ERC721Upgradeable.ownerOf(tokenId) == msg.sender)),
            "Transfer possible only by contract owner or to contract owner by NFT owner"
        );
        _;
    }
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public override(ERC721Upgradeable) onlyContractOwnerOrNftOwnerTransferringToContractOwner(to, tokenId) {
        _transfer(from, to, tokenId);
