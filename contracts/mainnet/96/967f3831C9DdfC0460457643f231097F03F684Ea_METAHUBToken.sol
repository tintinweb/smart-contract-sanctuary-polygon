...
contract METAHUBToken is IBEP20, TokenAuth {
  ...
  mapping (TaxType => uint) public lsdDiscountTaxPercentages; // decimal 3
  struct Config {
    uint secondsInADay;
    uint hardCapCashOut;
...
//  }
  ...
  function updateTaxWhitelist(address _user, TaxType _type, bool _whitelisted) external onlyMn {
    whitelistTax[_user][_type] = _whitelisted;
  }

  function updateLSDTaxDiscount(TaxType _type, uint _percentage) external onlyMn {
  ...
  function _calculateTax(address _from, address _to, uint _value) private returns (uint) {
    ...
    if (isBuyingToken) {
      if (whitelistTax[_to][TaxType.Buy]) {
        return 0;
      }
      if(lsd.isQualifiedForTaxDiscount(_to) && lsdDiscountTaxPercentages[TaxType.Buy] > 0) {
        return baseTax * (DECIMAL3 - lsdDiscountTaxPercentages[TaxType.Buy]) / DECIMAL3;
      }
    }
    if (_from == address(vault)) {
      if (whitelistTax[_to][TaxType.Claim]) {
        return 0;
      }
      if (config.sharkCheckpoints.length > 0) {
        ...
        if (checkpointIndex > 0) {
          return baseTax * (DECIMAL3 - config.sharkTaxPercentages[checkpointIndex]) / DECIMAL3;
        }
      }

      if(lsd.isQualifiedForTaxDiscount(_to) && lsdDiscountTaxPercentages[TaxType.Claim] > 0) {
        return baseTax * (DECIMAL3 - lsdDiscountTaxPercentages[TaxType.Claim]) / DECIMAL3;
      }
    }
    ...
    if (isSellingToken) {
      ...
      if(lsd.isQualifiedForTaxDiscount(_from) && lsdDiscountTaxPercentages[TaxType.Sell] > 0) {
        baseTax = baseTax * (DECIMAL3 - lsdDiscountTaxPercentages[TaxType.Sell]) / DECIMAL3;
      }
      (uint deposited,) = vault.getUserInfo(_from);
      if (_value >= deposited * config.cashOutTaxPercentage / 100) {
        baseTax = _value / 2;
      }
    } else if (_from != address(vault) && lsd.isQualifiedForTaxDiscount(_from) && lsdDiscountTaxPercentages[TaxType.Transfer] > 0) {
      baseTax = baseTax * (DECIMAL3 - lsdDiscountTaxPercentages[TaxType.Transfer]) / DECIMAL3;
    }
    return baseTax;
  }
