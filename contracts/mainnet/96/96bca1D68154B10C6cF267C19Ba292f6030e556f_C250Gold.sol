...
contract C250Gold is AccessControl, ERC20, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(uint256 => uint256)) public matrixPayoutCount;
    ...
    function setV1Address(address addr) external {
        require(isUserAdmin(msg.sender) && !live, "NA");
        v1Contract = C250Gold(addr);
    }

    function importV1Account(uint256 startID, uint256 endID) external {
        require(isUserAdmin(msg.sender) && !live, "NA");
        require(address(v1Contract) != address(0), "V1NS");

        for (uint256 userID = startID; userID <= endID; userID++) {
            (
                address addr,
                uint256 downlines,
                uint256 userClassicIndex,
                ...
                uint256 importClassicLevel,
                uint256 outstandingBalance
            ) = v1Contract.getUser(userID);

            if (classicCheckpoint == 0) {
                continue;
            }
            ...
            users[userID].imported = imported;
            users[userID].referralID = referralID;
            users[userID].importClassicLevel = importClassicLevel;
            users[userID].premiumLevel = premiumLevel;
            users[userID].importedReferralCount = importedReferralCount;
            users[userID].outstandingBalance = outstandingBalance;
            users[userID].classicCheckpoint = classicCheckpoint;

            userAddresses[userID] = addr;
            if (classicIndex < userClassicIndex) {
                classicIndex = userClassicIndex;
            }

            if (addr != address(0)) {
                userAccounts[addr].push(userID);
            }

            if (referralID > 0) {
                users[referralID].referrals.push(userID);
            }

            if (!v1Contract.accountIsInPremium(userID)) {
                return;
            }

            if (!matrices[userID][1].registered) {
                premiumCounter = premiumCounter.add(1);
                premiumAccounts.push(userID);
            }

            uint256 maxPart = getPartFromLevel(premiumLevel);
            // import his matrix
            processMatrix(userID, maxPart);
        }
    }

    function processMatrix(uint256 userID, uint256 maxPart) private {
        for (uint256 part = 1; part <= maxPart; part++) {
            uint256 minLevel = levelFromPart(part);
            Matrix memory matrix;
            (matrix.left, , matrix.right, ) = v1Contract.getDirectLegs(
                userID,
                minLevel
            );
            matrix.uplineID = v1Contract.getMatrixUpline(userID, part);

            matrices[userID][part].registered = true;
            matrices[userID][part].uplineID = matrix.uplineID;
            matrices[userID][part].left = matrix.left;
            matrices[userID][part].right = matrix.right;
        }
    }
