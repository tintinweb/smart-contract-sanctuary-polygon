...
contract FeeRecipient is Ownable, ReentrancyGuard {
    ...
    constructor(FeeDistributor feeDistributor_) {
        feeDistributor = feeDistributor_;
    }
    
    function _fallback() internal virtual {
        if (msg.value > 0) {
            feeDistributor.deposit{value: msg.value}();
        }
    }
    ...
    function emergencyWithdrawERC721(
        ...
        require(tokens.length == tokenIds.length, 'length check');
        for (uint256 i = 0; i < tokens.length; i++) {
