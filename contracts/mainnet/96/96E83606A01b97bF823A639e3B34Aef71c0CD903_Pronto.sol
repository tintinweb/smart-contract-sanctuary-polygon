...
contract Pronto is ERC20, AccessControl {
    ...
    event MiningPoolAddressUpdated(address indexed updater, address indexed _newaddress, string indexed _type);
    event MaxBurnUpdated(address indexed updater, uint256 newAmount);
    event CommissionPercentageUpdated(address indexed updater, uint256 percentage);
    ...
    constructor(
        ...
        require(_commissionPercentage >= 0 && _commissionPercentage <= 10, "Percentage must be between 0 and 10");
        ...
        onlyRole(UPDATER_ROLE)
    {
        ...
        miningPoolAddress = _miningPoolAddress;
        emit MiningPoolAddressUpdated(msg.sender, _miningPoolAddress, "Mining Pool Address");
    }
    ...
    function setCommissionPercentage(uint256 percentage)
        ...
        onlyRole(UPDATER_ROLE)
    {
        require(percentage >= 0 && percentage <= 10, "Percentage must be between 0 and 100");
        commissionPercentage = tP - percentage;

        emit CommissionPercentageUpdated(msg.sender, percentage);
    }

    function setMaxBurn(uint256 amount)
        external
        virtual
        onlyRole(UPDATER_ROLE)
    {
        ...
        emit MaxBurnUpdated(msg.sender, amount);
    }
    ...
    function currentCommissionPercentage() public view virtual returns (uint256) {
