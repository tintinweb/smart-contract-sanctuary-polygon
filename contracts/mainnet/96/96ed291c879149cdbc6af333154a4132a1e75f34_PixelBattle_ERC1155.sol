...
// File: contracts/hyperplay/MintRoundManager.sol

pragma solidity ^0.8.0;

contract MintRoundManager {

    string public _name; // 라운드 명
    string public _image; // 대표 이미지

    bool public _pause; // 활성 모드
    bool public _wlMode; // 화이티리스트 모드
    bool public _packMode; // Pack 판매 모드

    uint public _startBlockNumber; // 블록 높이 (Start)
    uint public _endBlockNumber; // 블록 높이 (End)

    uint256 public _totalSupply; // 판매 수량
    uint256 public _maxMintAmountPerWallet; // 지갑 당 최대 구매 수량
    uint256 public _maxMintAmountPerTx; // 트랜젝션 당 최대 구매 수량
    uint256 public _mintIndexForSale; // 판매된 수량
    uint256 public _amountPack; // 판매된 수량
    uint256 public _mintPrice; // 판매 가격 matic
    uint256 public _originPrice; // 원 가격 matic

    // White List
    mapping(address => uint256) internal _allowList;

    // == 화이트 리스트 모드
    function setInfo(string memory name, string memory image)
    external
    {
        _name = name;
        _image = image;
    ...
    {
        string[] memory _result = new string[](2);
        _result[0] = _name;
        _result[1] = _image;
        return _result;
    }
    ...
    function setPause(bool isPause)
    external
    {
        _pause = isPause;
    }
    ...
    function setWLMode(bool isWLMode)
    external
    {
        _wlMode = isWLMode;
    }
    ...
    function setPackMode(bool isPackMode)
    external
    {
        _packMode = isPackMode;
    }

    function resetIndexForSale()
    external
    {
        _mintIndexForSale = 0;
    }

    function setMetaData(uint256 maxMintAmountPerWallet,
    ...
    external
    {
        _maxMintAmountPerWallet = maxMintAmountPerWallet;
        _maxMintAmountPerTx = maxMintAmountPerTx;
        _mintPrice = mintPrice;
        _originPrice = originPrice;
        _amountPack = amountPack;
    ...
    returns (uint[] memory)
    {
        uint[] memory _result = new uint[](6);
        _result[0] = _maxMintAmountPerWallet;
        _result[1] = _maxMintAmountPerTx;
        _result[2] = _mintPrice;
        _result[3] = _originPrice;
        _result[4] = _amountPack;
        _result[5] = _totalSupply;
        return _result;
    }

    // == Set MintedAmountInfo
    function setMintedAmountInfo(uint256 mintIndexForSale)
    external
    {
        _mintIndexForSale = mintIndexForSale;
    }
    // == Get MintedAmountInfo
    function getMintedAmountInfo()
    external
    view
    returns (uint256)
    {
        return _mintIndexForSale;
    }

    // == Allow List
    function getAllowList(address _address)
    external
    view
    returns (uint256)
    {
    ...
    external
    {
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            _allowList[addresses[i]] = _maxMintAmountPerWallet;
        }
    }

    function deleteAllowList(address[] calldata addresses)
    external
    {
    ...
    function setMintingStartBlock(uint startBlockNumber)
    external
    {
        _startBlockNumber = startBlockNumber;
    }

    function setMintingEndBlock(uint endBlockNumber)
    external
    {
        _endBlockNumber = endBlockNumber;
    }

    function getMintingStartBlock()
    external
    view
    returns (uint)
    {
        return _startBlockNumber;
    }

    function getMintingEndBlock()
    external
    view returns (uint)
    {
        return _endBlockNumber;
    }

    function hasAddress(address _address)
    external
    view
    returns (bool)
    {
    ...
    // == 민팅 후처리
    function decreaseAmount(address _address, uint qty)
    external
    {
        _allowList[_address] -= qty;
    }
    function minted(uint qty)
    external
    {
        _mintIndexForSale = SafeMath.add(_mintIndexForSale, qty);
    }
    ...
    returns (bool)
    {
        return _mintIndexForSale >= _totalSupply;
    }

//    function isQuantityExceed(address _address, uint qty)
//    external
//    view
//    returns (bool)
//    {
//        return qty > _allowList[_address];
//    }
}

// File: contracts/PixelBattle_ERC1155.sol
...
/**
  @title Creating PixelBattle_ERC1155
  @dev Contract for creating nfts using ERC1155. Unlike ERC20, ERC1155 lacks a decimals field,
       since each token is distinct and cannot be partitioned
*/
contract PixelBattle_ERC1155 is ERC1155URIStorage, Ownable, Pausable, ERC1155Burnable {

    // init
    string public name = "PixelBattle by Pixelplay";
    string public symbol = "PixelMecha";
    ...
    constructor() ERC1155(
    //        "ipfs://QmQZD1NtvKj2aqo6zCUN6ko3wXALGqy8NV4vhvooNpSSdB/{id}.json"
    //        "https://ipfs.io/ipfs/QmQNc9TRBLrntBDmK6a5CR9eVgjH37xDsAwV7nfXFg1APC/{id}.json"
    //        "https://turquoise-bitter-scorpion-130.mypinata.cloud/ipfs/QmQNc9TRBLrntBDmK6a5CR9eVgjH37xDsAwV7nfXFg1APC/{id}.json"
    //        "https://turquoise-bitter-scorpion-130.mypinata.cloud/ipfs/QmQZD1NtvKj2aqo6zCUN6ko3wXALGqy8NV4vhvooNpSSdB/{id}.json"
        "https://api.nftvisualization.io/api/v1/hyper-play/contract/0x0d278F3209628298271Cc50FE53556381698d0F3/token/{id}/metadata"
    ) {
    }

    // ======================== MINT by payable ========================
    function mint(uint round, uint256[] memory ids)
    public
    payable
    {
        basicCheck(ids);
        ...
        if (isPackMode) {
            require(qty == amountPack, "Mint Quantity per tx exceed (1)");
        } else {
            require(qty <= maxMintAmountPerTx, "Mint Quantity per tx exceed (2)");
        }
        ...
        if (isPackMode) {
            mintPack(msg.sender, ids);

            _roundManager.minted(amountPack);
            if (isWLMode) {
                _roundManager.decreaseAmount(msg.sender, amountPack);
            }
        }  else {
            mintPack(msg.sender, ids);
    ...
    // ======================== MINT/BURN by Owner ========================
    function mintPack(address recepient, uint256[] memory ids)
    private
    returns (uint256[] memory)
    {
        basicCheck(ids);
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            uint256 _itemId = ids[i];
            amounts[i] = 1;

            _tokenURIs[_itemId] = uri(_itemId);
            _totalSupply[_itemId] = amount;
        }
        _mintBatch(recepient, ids, amounts, "");
        return ids;
    }

    function mintFixed(address recepient, uint tokenId)
    public
    onlyOwner
    returns (uint256)
    {
    ...
    onlyOwner
    {
        for (uint256 i = 0; i < ids.length; i++) {
            uint256 _itemId = ids[i];
            _totalSupply[_itemId] = 1;
            _tokenURIs[_itemId] = uri(_itemId);
        }
        return _mintBatch(to, ids, amounts, "");
    }
    ...
    function basicCheck(uint256[] memory ids)
    internal
    view
    {
        require(!paused(), "Contract is not active");
        require(ids.length > 0, "Invalid Quantity");
    ...
    function exists(uint256 id) public view returns (bool) {
        return PixelBattle_ERC1155.totalSupply(id) > 0;
    }
