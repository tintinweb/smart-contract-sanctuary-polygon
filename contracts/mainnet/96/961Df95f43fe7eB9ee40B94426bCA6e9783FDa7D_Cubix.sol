...
contract Cubix is Ownership {
    ...
    constructor(address _feeAddress, address _companyAddress, address _cubixaddress){
    ...
    function updateCubixPrice(uint256 _cubixperusd, uint256[] calldata packIds) external onlyAuthorized
    {
        cubixperusd = _cubixperusd; 
        for (uint256 index = 0; index < packIds.length; index++) {
            PKGDetail[packIds[index]].pkgprice = PKGDetail[packIds[index]].pkgAmount * _cubixperusd  * decimalsValue;
        }
