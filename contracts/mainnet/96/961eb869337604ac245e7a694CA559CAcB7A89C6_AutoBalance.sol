...
import { BhavishPredictionStorage, Address } from ".././Impl/BhavishPrediction/BhavishPredictionStorage.sol";
...
contract AutoBalance is Ownable{
    ...
    constructor(IBhavishNativeSDK _nativeSDK, bytes32 _underlying, bytes32 _strike, BhavishPredictionStorage _bhavishPredictionStorage) {
    ...
    function getPredictionRoundStatus(uint256 _roundId) public view returns (IBhavishPrediction.RoundState roundStatus) {
        IBhavishPrediction.Round memory roundDetails = bhavishPredictionStorage.getPredictionRound(_roundId);
    ...
    function withdraw() external onlyOwner {
        _safeTransfer(address(msg.sender), totalAssets());
    }

    function performPredict(uint256 _roundId) external onlyOwner {   
        uint256 _predictAmount = totalAssets() / 2;
    ...
    function performClaimAndRefund(uint256 _roundId) external onlyOwner {
        ...
        roundIds[0] = _roundId;
        IBhavishPrediction.RoundState rStatus = getPredictionRoundStatus(_roundId);
        if (rStatus == IBhavishPrediction.RoundState.ENDED) {
            nativeSDK.claim(
                IBhavishSDK.PredictionStruct(underlying, strike, 0, false),
                roundIds,
                swapParams
                );
        }
