    ...
    OwnableUpgradeable
{
    ...
    IERC721 public monkeysContract;

    uint256[] public lockupArray;

    mapping(uint256 => uint256) public tokenLastStakedAt;
    mapping(address => uint256[]) public stakedTokensOfWallet;
    mapping(uint256 => uint256) public tokenStakedAt;
    mapping(uint256 => uint256) public tokenLockup;

    function initialize(address _monkeysContract) external initializer {
        __ReentrancyGuard_init();
        __ERC721Holder_init();
        __Ownable_init();
        monkeysContract = IERC721(_monkeysContract);
    }
    ...
    function stakeToken(
        ...
        require(includesLockupArray(lockup), "Element not found in the array.");

        for (uint256 i; i < tokenIDs.length; ) {
            uint256 currentToken = tokenIDs[i];

            if (monkeysContract.ownerOf(currentToken) == msg.sender) {
                monkeysContract.transferFrom(
                    msg.sender,
                    address(this),
                    currentToken
                );
    ...
    function isStakeMature(
        uint256 tokenId,
        uint256 lockup
    ) public view returns (bool) {
        uint256 stakeTime = tokenStakedAt[tokenId];
        uint256 currentTime = block.timestamp;
        return (currentTime >= stakeTime + (lockup * 1 days));
    }

    function unstakeToken(uint256[] calldata tokenIDs) external {
        for (uint256 i; i < tokenIDs.length; ) {
            uint256 currentToken = tokenIDs[i];
            uint256 currentLockup = tokenLockup[currentToken];
            require(isStakeMature(currentToken, currentLockup), "Locked");
    ...
    function includesLockupArray(uint256 element) public view returns (bool) {
        for (uint256 i = 0; i < lockupArray.length; i++) {
            if (lockupArray[i] == element) {
                return true;
            }
        }
        return false;
    ...
    function addItem(uint256 item) public onlyOwner {
        lockupArray.push(item);
    }

    function removeItem(uint256 index) public onlyOwner {
        require(index < lockupArray.length, "Invalid index.");
        lockupArray[index] = lockupArray[lockupArray.length - 1];
        lockupArray.pop();
    }
}