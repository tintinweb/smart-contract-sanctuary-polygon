...
contract SERA {
    ...
    uint256 public exchangeRate = 1; // 1:1 exchange rate
    uint256 public transactionFeePercentage = 200; // 2% fee
    ...
    function sendTether(uint256 _amount) public {
        require(_amount > 0, "Amount must be greater than zero");

        IERC20 tetherToken = IERC20(tetherAddress);

        // Transfer USDT tokens from sender to the contract
        require(tetherToken.transferFrom(msg.sender, address(this), _amount), "USDT transfer failed");

        // Calculate the equivalent amount of 'SERA' tokens based on the exchange rate
        uint256 serAmount = _amount * exchangeRate;

        // Mint 'SERA' tokens and store them in the contract
        balanceOf[msg.sender] += serAmount;

        emit Transfer(address(0), msg.sender, serAmount);
    }
    ...
    function redeemStablecoin(uint256 _amount) external {
        ...
        uint256 tetherAmount = _amount / exchangeRate;
