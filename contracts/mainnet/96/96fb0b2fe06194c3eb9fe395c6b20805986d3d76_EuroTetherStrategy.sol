...
interface ICurveOuterPool_euroTether {
...
contract EuroTetherStrategy is BaseStrategy {
    ...
    address public pool = 0x225FB4176f0E20CDb66b4a3DF70CA3063281E855; //outer pool contract
    //address public inner_pool = 0xB446BF7b8D6D4276d0c75eC0e3ee8dD7Fe15783A; //inner pool contract

    address public crvEURTUSD = 0x600743B1d8A96438bD46836fD34977a00293f6Aa;
    address public crvEURTUSD_deposit = 0x40c0e9376468b4f257d15F8c47E5D0C646C28880;
    ...
    constructor() public BaseStrategy(crvEURTUSD, FUND, WMATIC, sushiRouter, crvEURTUSD_deposit) {
        multiHarvest = 0x3355743Db830Ed30FF4089DB8b18DEeb683F8546;
        
        wmatic_usdc_path = [WMATIC, USDC];

        crv_usdc_path = [CRV, WETH, USDC];
        crv_wmatic_path = [CRV, WETH, WMATIC];

        IERC20(crvEURTUSD).safeApprove(crvEURTUSD_deposit, uint256(-1));
        IERC20(crvEURTUSD_deposit).safeApprove(crvEURTUSD_deposit, uint256(-1));
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        //Deposit USDC in pool to get LP tokens
        uint256 _usdcBalance = IERC20(USDC).balanceOf(address(this));
        if (_usdcBalance > 0) {
            ICurveOuterPool_euroTether(pool).add_liquidity([0, 0, _usdcBalance, 0], 0, address(this));
        }
    ...
    function salvage(address recipient, address token, uint256 amount) public onlyOwner {
        //Note: No CRV or WMATIC should be in this contract after harvesting, if there is, something went wrong or someone manually sent it here
