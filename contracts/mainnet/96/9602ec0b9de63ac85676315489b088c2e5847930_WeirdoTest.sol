...
pragma solidity ^0.8.0;
contract WeirdoTest is ERC721, ERC721Enumerable, Ownable, Pausable {
  ...
  uint256 public mintPriceMATICUWU = 0.0001 ether;
  uint256 public mintPriceUWU = 0.0001 ether;
...
IERC20 public TokenUWU;
  ...
  constructor(string memory baseURIVal_, IERC20 _TokenUWU)
    ERC721("test", "test")
  {
      ...
      doesNotExceedMaxPurchaseCount(numberOfTokens)
  {
    ...
    function mintTokensUWU(uint256 numberOfTokens)
      public
      payable
      whenNotPaused
      whenMintingNotComplete
      ...
      doesNotExceedMaxPurchaseCount(numberOfTokens)
  {
      uint256 amount = calculatePriceMATICUWU(numberOfTokens);
      uint256 amountUWU = calculatePriceUWU(numberOfTokens);

    if (msg.sender != owner()) {
      uint256 balance = TokenUWU.balanceOf(msg.sender);
      require(balance >= amountUWU, "insufficient UWU");
      ...
      doesNotExceedMaxPurchaseCount(1)
  {
    ...
    require(info.itChanged == true, "you already made the change");
...
function calculatePriceMATICUWU(uint256 numberOfTokens)
    internal
    view
    returns (uint256)
  {
      return mintPriceMATICUWU.mul(numberOfTokens);
  }

function calculatePriceUWU(uint256 numberOfTokens)
    internal
    view
    returns (uint256)
  {
      return mintPriceUWU.mul(numberOfTokens);
  }
...
function setMintPriceUWUMATIC(uint256 newPrice) public onlyOwner {
    mintPriceMATICUWU = newPrice;
  }
function setMintPriceUWU(uint256 newPrice) public onlyOwner {
    mintPriceUWU = newPrice;
  }
