...
contract VaultCore is VaultStorage {
    ...
    function _rebase() internal whenNotRebasePaused {
        ...
        uint256 cashSupplyNew = cASH.totalSupply();

        // autobribe
        address _autobribeAddress = autobribeAddress; // gas savings
        if(_autobribeAddress != address(0) && vaultValue > cashSupplyNew){
            cASH.mint(_autobribeAddress, IAutobribe(_autobribeAddress).getCashForBribes());
...
interface IAutobribe {
    function getCashForBribes() external view returns (uint256);
