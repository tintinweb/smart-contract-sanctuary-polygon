...
contract Mai3CrvStrategy is BaseStrategyMasterChef {

    uint256 public constant keepMax = 10000;

    address public MAI3CRV_POOL = 0x447646e84498552e62eCF097Cc305eaBFFF09308;
    ...
    address[] public qi_mai_path;
    address[] public reward_matic_path;

    constructor()
        public
        BaseStrategyMasterChef(
            0x0635AF5ab29Fc7bbA007B8cebAD27b7A3d3D1958, //Chefv2
            MAI3CRV_POOL,
            0x01fE07ce760DA7a025e4Cf3f950aE236F8e62120, //fund
            0,
            QI,
    ...
    {
        qi_mai_path = [QI, MAI];
        reward_matic_path = [QI, WMATIC];

        IERC20(want).safeApprove(rewards, uint256(-1));

        IERC20(MAI).safeApprove(MAI3CRV_POOL, uint256(-1));
        IERC20(QI).safeApprove(currentRouter, uint256(-1));
    }
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        //Swap remaining QI for MAI
        uint256 remainingHarvested = IERC20(harvestedToken).balanceOf(address(this));
        if (remainingHarvested > 0) {
            _swapUniswapWithPathPreapproved(qi_mai_path, remainingHarvested);
        }

        //Deposit MAI in pool to get want
        uint256 maiBalance = IERC20(MAI).balanceOf(address(this));
        if (maiBalance > 0) {
            ICurvePool_Mai(MAI3CRV_POOL).add_liquidity([maiBalance, 0], 0);
        }
    }
