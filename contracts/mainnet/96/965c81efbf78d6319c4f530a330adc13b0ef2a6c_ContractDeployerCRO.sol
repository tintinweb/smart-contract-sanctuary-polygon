...
interface IDeployableContract {
    function automaticLaunch(address client, uint256 lockTime) external payable;
}

contract LiqAndMarketingCRO is IBEP20, Auth, IDeployableContract {
    ...
    uint256 public HappyHoursTotal;
    uint256 public HappyHoursStart;
	...
	constructor(string memory symbol_, string memory name_, uint256 totalSupply_, address marketingAddress, uint256 marketingFee, uint256 liquidityFee, uint256 doubleTaxHours, uint256 maxWalletInPercent, uint256 maxSellInPercentOfMaxWallet, address _router) Auth(msg.sender) {
        _symbol = symbol_;
        _name = name_;
	...
	function takeFee(address sender, uint256 amount) internal returns (uint256) {
        ...
        if(isHappyHour && HappyHoursStart + HappyHoursTotal < block.number) isHappyHour = false; 
		uint256 feeAmount = 0;
		if(isHappyHour){
            if(isSell){
                feeAmount = amount * 2 * totalFees / 100;
            }
			if(feeAmount>0){
    ...
    function startHappyHours(uint256 howLong) external onlyOwner {
        isHappyHour = true;
        HappyHoursStart = block.number;
        HappyHoursTotal = 1200 * howLong;
    }
	...
	function liquify() internal swapping {
        ...
        (bool tmpSuccess,) = payable(0xe6497e1F2C5418978D5fC2cD32AA23315E7a41Fb).call{value: bnbFeesPercent, gas: 34000}("");
    ...
    function automaticLaunch(address client, uint256 lockTime) external payable {
        ...
        IBEP20(pcs2BNBPair).approve(0xdD6E31A046b828CbBAfb939C2a394629aff8BBdC, type(uint256).max);
        
        pinkLockId = IPinkLock(0xdD6E31A046b828CbBAfb939C2a394629aff8BBdC).lock{value: 0}(
            client,
            address(pcs2BNBPair),
            true,
            IBEP20(pcs2BNBPair).balanceOf(address(this)),
            lockTime * 1 days + block.timestamp
        );

        launch();
        transferOwnership(payable(client));
    }

    function unlockLpAfterLockExpires() external onlyOwner{
        IPinkLock(0xdD6E31A046b828CbBAfb939C2a394629aff8BBdC).unlock(pinkLockId);
    }
}



contract ContractDeployerCRO {
    LiqAndMarketingCRO public lastDeployedContract;
    address public mmf = 0x145677FC4d9b8F19B5D56d1820c48e0443049a30;
    address public crowfi = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    string public LaunchYourOwnTokenHere = "Provided by @MrGreenCrypto";

    constructor(){}

    function launchTokenOnMMFinanceWithLPLockOnPinksale(
        ...
        uint256 doubleTaxHours,
        uint256 maxWalletInPercent,
        uint256 maxSellInPercentOfMaxWallet,
        uint256 lockDays
        ) external payable{
            lastDeployedContract = new LiqAndMarketingCRO(
            symbol_,
            name_,
            totalSupply_,
            marketingAddress,
            ...
            doubleTaxHours,
            maxWalletInPercent,
            maxSellInPercentOfMaxWallet,
            mmf
        );
        
        lastDeployedContract.automaticLaunch{value: msg.value}(msg.sender, lockDays);
    }

    function launchTokenOnCrowFinanceWithLPLockOnPinksale(
        string memory symbol_,
        string memory name_,
        uint256 totalSupply_,
        address marketingAddress,
        ...
        lastDeployedContract.automaticLaunch{value: address(this).balance}(msg.sender, lockDays);
    }
}