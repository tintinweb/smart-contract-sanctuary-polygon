...
error Structured10MRouterFakeUpgrade__UndefinedRouteForItemIndex(uint256 itemIndex, uint totalRoutes, uint256 itemsPerRoute);
error Structured10MRouterFakeUpgrade__NotRouteRegisterer();
error Structured10MRouterFakeUpgrade__InvalidArg_AddressZero();
error Structured10MRouterFakeUpgrade__InvalidArg_ItemsPerRouteZero();

contract Structured10MRouterFakeUpgrade is OwnableUpgradeable {
  ...
  function registerRoute(uint256 index, string calldata cid) public {
    if (_msgSender() != routeRegisterer) {
      revert Structured10MRouterFakeUpgrade__NotRouteRegisterer();
    }
  ...
  function lookup(uint256 itemIndex) public view returns (string memory) {
    ...
    if (bytes(cid).length == 0) {
      revert Structured10MRouterFakeUpgrade__UndefinedRouteForItemIndex({itemIndex: itemIndex, totalRoutes: routes.length, itemsPerRoute: itemsPerRoute});
    }
    return cid;
  }
  ...
  function setRouteRegisterer(address account) public onlyOwner {
    if (account == address(0)){
      revert Structured10MRouterFakeUpgrade__InvalidArg_AddressZero();
    }
  ...
  function setItemsPerRoute(uint256 _itemsPerRoute) public onlyOwner {
    if (_itemsPerRoute == 0){
      revert Structured10MRouterFakeUpgrade__InvalidArg_ItemsPerRouteZero();
    }
