...
contract TEST_VvDucks is ERC721A, ReentrancyGuard, Ownable {
    ...
    mapping(uint => string) levelBaseTokenURIs;
    ...
    function setTokenLevelsURI(string memory baseURI, uint256 tokenId) external onlyOwner {
        levelBaseTokenURIs[tokenId] = baseURI;
    }
...
//     }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory){
        ...
        if (tokenIdLevels[tokenId] > 0) {
            return string(abi.encodePacked(levelBaseTokenURIs[tokenIdLevels[tokenId]],tokenId, ".json"));
        }
        else {
            return string(abi.encodePacked(super.tokenURI(tokenId), ".json"));
        }
