...
contract ConversionV1 is Initializable, Ownable {
    ...
    function getTokenPrice(
        ...
        if (priceFeed[targetToken][address(0)] != address(0)) {
            ...
            if (pairedWith == Trace) {
                ...
                _price = getSwapFromUSD(targetToken);
            }
            return _price;
        }
    }
    ...
    function getSwapFromTrace(address tokenA) public view returns(uint256) {
        ...
        uint256 price;
        uint256 decimals;
        uint256 usdTokenPrice = getSwapPrice(USD, Trace); 
        ...
        if(token0 == Trace) {
            if(decimals == 18) {
                price = (reserves1 * 10 ** 36) / (reserves0 * 10 ** 18);
                ...
                price = (reserves1 * 10 ** 18 ) / (reserves0 * 10 ** decimals);
                if(price != 0) {
            ...
            if(decimals == 18) {
                ...
                if(price != 0) {
                 ...
                 price = (reserves0 * 10 ** 18 ) / (reserves1 * 10 ** decimals); 
                 if(price != 0) {
                    toUSD = (usdTokenPrice/price);
                }
            }
    ...
    function getSwapFromUSD(address tokenA) public view returns(uint256) {
        ...
        if(token0 == USD) {
            price = (reserves0 * 10 ** decimals * 10 ** 8) / (reserves1 * 10 ** 6); 
