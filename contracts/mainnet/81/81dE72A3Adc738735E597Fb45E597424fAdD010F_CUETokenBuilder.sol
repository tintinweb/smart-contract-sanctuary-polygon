...
contract CUETokenBuilder is Context, Ownable {
    ...
    function getPrice(address sender) public view returns (uint256) {
        ...
        int price_eth_usd = get_feed_price() / 10 ** 8;

        uint256 eth_per_usd = uint256(1 ether / price_eth_usd);

        uint256 builder_price_eth = eth_per_usd * builder_price_usd;
    ...
    function setBuilderPrice(uint256 _price) external onlyOwner {

        builder_price_usd = _price;
    }

    function setBuilderPriceOracle(uint256 _price) external {
