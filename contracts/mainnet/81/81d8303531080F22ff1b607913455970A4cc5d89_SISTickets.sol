...
/**
 * @title SISTickets
 * SISTickets - Base smart contract using ERC-721 Standard
 */
contract SISTickets is ERC721, ERC721URIStorage, Ownable {
    ...
    /*
        This method will mint allow owner fix the base uri
    */
    function fixBaseURI(string memory _newURI) external onlyOwner {
        baseURI = _newURI;
    }
    ...
    */
        ...
        returns (uint256)
    {
        require(
            !nftExists(_tokenURI),
            "SISTickets: Trying to mint existent nft"
        );
        if (hardCap > 0) {
            uint256 reached = _tokenIdCounter.current() + 1;
            require(reached <= hardCap, "SISTickets: Hard cap reached");
        }
