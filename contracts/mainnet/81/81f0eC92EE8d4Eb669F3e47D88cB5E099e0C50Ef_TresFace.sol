...
AuxHelperFourInto256
{
    ...
    // Using int packing to map this to 4 64bit values.
    mapping(string => uint256) private _uriFragmentInfo;
    ...
    function getUriFragmentInfo(string memory uriFragment) external view returns (uint256) {
        return _uriFragmentInfo[uriFragment];
    }

    function getUriFragmentInfo_TokenId(string memory uriFragment) external view returns (uint64) {
        (uint64 tokenId, , , ) = _unpack64(_uriFragmentInfo[uriFragment]);
        return tokenId;
    }

    function getUriFragmentInfo_Status(string memory uriFragment) external view returns (uint256) {
        (, uint64 status, , ) = _unpack64(_uriFragmentInfo[uriFragment]);
        return status;
    }

    function getUriFragmentInfo_Generation(string memory uriFragment) external view returns (uint256) {
        (, , uint64 generation, ) = _unpack64(_uriFragmentInfo[uriFragment]);
        return generation;
    }
    ...
    function bindToUser(uint256 tokenId, string memory userUri) external {
        ...
        require(_uriFragmentInfo[userUri] == 0, 'Must be new fragment');
    ...
    function _setBindToUser(uint256 __tokenId, string memory __userUri) internal {
        // Look up the previous Uri Fragment.
        string memory previousFragment = _userUriFragments[__tokenId];

        // Look up the previous Uri Fragment Info [tokenId, status, generation, unused]
        (uint64 boundTokenId, , uint64 boundGeneration, ) = _unpack64(_uriFragmentInfo[previousFragment]);

        // Set the previous fragment to retired. Make sure it is a real fragment first.
        if(!_compareStrings(previousFragment, '')){
            _uriFragmentInfo[previousFragment] = _pack64(uint64(__tokenId), 2, boundGeneration, 0);
        }

        // Update the token's uri fragment to the new one
        _userUriFragments[__tokenId] = __userUri;

        // Set the current fragment to active
        _uriFragmentInfo[__userUri] = _pack64(uint64(__tokenId), 1, boundGeneration + 1, 0);
    }
