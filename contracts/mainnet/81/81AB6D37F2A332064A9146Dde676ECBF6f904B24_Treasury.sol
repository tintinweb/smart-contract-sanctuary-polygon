...
contract Treasury is ContractGuard {
    ...
    uint256 public price_target; // The price of 10MB at which the collateral ratio will respond to; this value is only used for the collateral ratio mechanism and not for minting and redeeming which are hardcoded at $1
    ...
    {
        return (get10MBPrice(), get10SHAREPrice(), IERC20(_10MB).totalSupply(), target_collateral_ratio, effective_collateral_ratio, globalCollateralValue(), minting_fee, redemption_fee);
    }
    ...
    function globalCollateralValue() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < pools_array.length; i++) {
            ...
            if (pools_array[i] != address(0)) {
                total_collateral_value = total_collateral_value.add(IPool(pools_array[i]).collateral10MBBalance());
            }
        }
    ...
    function calcEffectiveCollateralRatio() public view returns (uint256) {
        ...
        uint256 total_supply_10MB = IERC20(_10MB).totalSupply();
        uint256 ecr = total_collateral_value.mul(PRICE_PRECISION).div(total_supply_10MB);
    ...
    function refreshCollateralRatio() external {
        ...
        uint256 current_10MB_price = get10MBPrice();

        // Step increments are 0.25% (upon genesis, changable by setRatioStep())
        if (current_10MB_price > price_target.add(price_band)) {
        ...
        else if (current_10MB_price < price_target.sub(price_band)) {
    ...
    function calcCollateralBalance() public view returns (uint256 _collateral_value, bool _exceeded) {
        uint256 total_collateral_value = globalCollateralValue();
        uint256 target_collateral_value = IERC20(_10MB).totalSupply().mul(target_collateral_ratio).div(PRICE_PRECISION);
    ...
    function get10SHAREPrice() public view returns (uint256 _10SHAREPrice) {
        try IOracle(_10SHAREOracle).consult(_10SHARE, 1e18) returns (uint144 price) {
    ...
    function getRedeemableBonds() public view returns (uint256 _redeemableBonds) {
        ...
        if (__10MBPrice > _10MBPriceCeiling) {
            ...
            if (_rate > 0) {
                _redeemableBonds = _total10MB.mul(1e5).div(_rate);
            }
        }
    }
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (__10MBPrice <= _10MBPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = _10MBPriceOne.mul(1e5).div(__10MBPrice); // to burn 1 10MB
    ...
    function initialize(
        ...
        // TODO: update time
        refresh_cooldown = 10;//3600; // Refresh cooldown period is set to 1 hour (3600 seconds) at genesis
        price_target = 1e5; // = $0.1. (6 decimals of precision). Collateral ratio will adjust according to the $0.1 price target at genesis
        price_band = 500;
        redemption_fee = 8000;
        minting_fee = 4000;
    ...
    function buyBonds(uint256 __10MBAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 _bondAmount = __10MBAmount.mul(_rate).div(1e5);
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 __10MBAmount = _bondAmount.mul(_rate).div(1e5);
    ...
    function get10MBExpansionRate() public view returns (uint256 _rate) {
        if (epoch < bootstrapEpochs) { // 21 first epochs with 3.5% expansion
            ...
            if (_twap >= _10MBPriceCeiling) {
                uint256 _percentage = _twap.sub(_10MBPriceOne); // 1% = 1e3
                uint256 _mse = maxSupplyExpansionPercent.mul(1e1);
                if (_percentage > _mse) {
                    _percentage = _mse;
                }
                _rate = _percentage.div(1e1);
            }
        }
    }
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            if (previousEpoch10MBPrice >= _10MBPriceCeiling) {
                ...
                uint256 _mse = _calculateMaxSupplyExpansionPercent(_10MBSupply).mul(1e1);
                if (_percentage > _mse) {
                    _percentage = _mse;
                }
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    // saved enough to pay debt, mint as usual rate
                    _savedForBoardroom = _10MBSupply.mul(_percentage).div(1e5);
                } else {
                    // have not saved enough to pay debt, mint more
                    uint256 _seigniorage = _10MBSupply.mul(_percentage).div(1e5);
