    ...
    UUPSUpgradeable
{
    ...
    function getStakingRewardClaimTimeRemaining(uint256 _stakingID) external view returns (uint256) {
        ...
        if(userStakingInfo.lastTimeRewardClaimed > 0) {
            return userStakingInfo.lastTimeRewardClaimed + _stakingRewardClaimTimeLimit < _currentTime ? 0 : userStakingInfo.lastTimeRewardClaimed - _currentTime;
        }

        return userStakingInfo.startTime + _stakingRewardClaimTimeLimit < _currentTime ? 0 : userStakingInfo.lastTimeRewardClaimed - _currentTime;
    }
