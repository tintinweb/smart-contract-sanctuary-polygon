...
contract ERC721PaymentOpenSale is ERC721BaseSale, __with_semver(uint8(1), uint8(0), uint8(0)) {
    ...
    function setTokenSaleAddress(address tokenSaleAddress_) external requirePermission(CONTROLLER) {
        require(
            tokenSaleAddress_ != address(0x00) && tokenSaleAddress_.code.length > 0, 
            Exceptions.NULL_TOKEN_ADDRESS
        );
    ...
    function _tokenTransferFromToTreasury(address from, address tokenBuyAddress, uint256 costInBuyTokens_) internal {
