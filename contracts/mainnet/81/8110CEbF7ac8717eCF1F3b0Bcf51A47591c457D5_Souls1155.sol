...
contract Souls1155 is ERC1155, Ownable,TokenUriSupplier {
    using Strings for uint256;

    uint256 public mintBurnTokenId = 1;
    uint256 public mintLimit = 2;
    uint256 public maxSupplyPerTokenId = 1000;
    bool public hasMintLimit = true;
    bool public mintPaused = true;
    bool public burnMintPaused = true;
    string public name = "DAO Heaven Souls";
    string public symbol = "DHS";

    mapping(address => mapping(uint256 => uint256)) public mintedBalance;
    mapping(uint256 => uint256) public currentSupply;

    constructor()
        ERC1155("")
    {
        baseURI = "https://data.dao-heaven.com/dhvfm/metadata/";
        _mint(msg.sender, mintBurnTokenId, 1, "");
        currentSupply[mintBurnTokenId] += 1;
    }

    // onlyOwner
    function setMintBurnTokenId(uint256 _mintBurnTokenId) external onlyOwner {
        mintBurnTokenId = _mintBurnTokenId;
    }
    ...
    function enableMintLimit() external onlyOwner {
        hasMintLimit = true;
    }

    function disableMintLimit() external onlyOwner {
        hasMintLimit = false;
    }
    ...
    function pauseBurnMint() external onlyOwner {
        burnMintPaused = true;
    }

    function unpauseBurnMint() external onlyOwner {
        burnMintPaused = false;
    }

    // override TokenUriSupplier for ERC1155
    ...
    // external for minting
    function mint(uint256 id, uint256 amount) external {
        require(!mintPaused, "Minting is paused");
        require(id == mintBurnTokenId, "You can only mint the specified token");
        require(
            currentSupply[id] + amount <= maxSupplyPerTokenId,
            "Exceeds max supply per token ID"
        );
        if (hasMintLimit) {
            require(
                mintedBalance[msg.sender][id] + amount <= mintLimit,
                "Mint limit exceeded"
            );
        }
        mintedBalance[msg.sender][id] += amount;
    ...
    function burnMint(
        ...
        require(!burnMintPaused, "Burn and Mint is paused");
        require(
            burnId == mintBurnTokenId,
            "You can only burn the specified token"
        );
        require(
            balanceOf(msg.sender, burnId) >= amount,
            "Not enough tokens to burn"
        );

        _burn(msg.sender, burnId, amount);
        uint256 mintId = _randomRange(startId, endId);
    ...
    function _randomRange(
        ...
        return start + (uint256(random) % range);
    }
}
