...
contract CMF is AccessControlEnumerable,Pausable,ICMF {
   ...
   function rechargeDay(uint256 price) public{
    ...
    function payLevel() public{
        ...
        payUserInfo[_msgSender()] = block.timestamp;
    ...
    function payLevelEnd() public{
        require(payUserInfo[_msgSender()] >0, "payLevel error");
        require(payUserInfo[_msgSender()] + Setting.LeveEndDay * Config.timeStep < block.timestamp, "payLevel endtime error");
        payUserInfo[_msgSender()] = 0;
        userLevel[_msgSender()] = 0;
        IERC20(USDT).transfer(_msgSender(), Setting.LevelPrice3);
        _upgrade(_msgSender(),0);
        emit Paylevelend(_msgSender(), Setting.LevelPrice3);
    }
    /**
     * 节点分红
     * Node divvy
    ...
    * 获取充值记录
    ...
    function _upgrade(address owner,uint256 price) private{
        ...
        while(true){
            ...
            teamRechargePrice[parentAddress] += price;
            for(uint256 i=0;i<teamUpNum.length;i++){
                uint256 level = i+1;
                if(userLevel[parentAddress]>=level){
                    break;
                }
                if(userRechargePrice[parentAddress]<userUpPrice[i] || teamRechargePrice[parentAddress]<teamUpPrice[i]){
                    break;
                }
                if((level == 1 && userZhiValid[parentAddress]>=teamUpNum[i]) || (level>1 && userLevelNum[parentAddress][level]>=teamUpNum[i])){
                    userLevel[parentAddress] = level;
                    //s6 最高分红
                    if(userLevel[parentAddress] == 6){
                        levelMaxUser.push(parentAddress);
                    }
                    //三级团队计算人数
                    userTeamLevelNum(parentAddress,level);
                }
            }
        }
    }
...
abstract contract ICMF is CMFDATA {
    ...
    //付费等级信息
    mapping(address => uint256) public payUserInfo;
    //付费等级信息
