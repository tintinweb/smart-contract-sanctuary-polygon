...
contract NFTMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function listItem(
        ...
        // TODO: Add listing price if needed
        // require(
        //     msg.value == listingPrice,
        //     "Price must be equal to listing price"
        // );

        address fromOwner = IERC721(tokenAddress).ownerOf(tokenId);
        IERC721(tokenAddress).transferFrom(fromOwner, address(this), tokenId);
     ...
     * Buyer must approve the marketplace to spend the payment token (increaseAllowance or approve)
     * @param listingId marketplace item listing id
     * @param recipientAddress address that receives the NFT
     */
    function buyItem(uint256 listingId, address recipientAddress)
        public
        nonReentrant
    {
        ...
        IERC20(item.paymentAddress).transferFrom(
            msg.sender,
            item.seller,
            item.price
        );
