...
contract TokenBatchTransfer is Ownable {
    ...
    event BatchTransfer(address lastAddress, uint256 tokenAmount, uint256 lastIndex);
    ...
    onlyOperator
    {
        ...
        uint256 tokenHolders_length = tokenHolders.length;
        for(uint256 indx = 0; indx < tokenHolders_length; indx++) {
            require(token.transfer(tokenHolders[indx], amounts[indx]), "Unable to transfer token to the account");
        }
        totalTransferred = totalTransferred + tokenHolders_length;
        emit BatchTransfer(tokenHolders[tokenHolders_length-1], amounts[tokenHolders_length-1], totalTransferred-1);
    }
