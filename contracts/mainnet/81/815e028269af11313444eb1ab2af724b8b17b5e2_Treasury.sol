...
interface IBoardroom {
    ...
    function getBeePrice() external view returns (uint256);
...
contract Treasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        address(0xf9ef7991bFb684F997010e6868bE55202BF48A61), // BeeGenesisPool
        // address(), // BeeRewardPool
        address(0)
    ];
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of BEE price
    ...
    function getBurnableBeeLeft() public view returns (uint256 _burnableBeeLeft) {
        ...
        if (_beePrice <= beePriceOne) {
            ...
            if (_bondMaxSupply > _bondSupply) {
                ...
                uint256 _maxBurnableBee = _maxMintableBond.mul(_beePrice).div(beePriceOne);
    ...
    function getRedeemableBonds() public view returns (uint256 _redeemableBonds) {
        ...
        if (_beePrice > beePriceCeiling) {
            ...
            if (_rate > 0) {
                _redeemableBonds = _totalBee.mul(beePriceOne).div(_rate);
            }
        }
    }
    ...
    function initialize(
        ...
        beePriceOne = 25 * 1e13; // This is to allow a PEG of 4000 BEE per ETH
    ...
    function buyBonds(uint256 _beeAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        require(
            beePrice < beePriceOne, // price < $1
            "Treasury: beePrice not eligible for bond purchase"
        );
        ...
        uint256 _bondAmount = _beeAmount.mul(_rate).div(beePriceOne);
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        require(beePrice == targetPrice, "Treasury: BEE price moved");
        require(
            beePrice > beePriceCeiling, // price > $1.01
            "Treasury: beePrice not eligible for bond purchase"
        );
        ...
        uint256 _beeAmount = _bondAmount.mul(_rate).div(beePriceOne);
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            if (previousEpochBeePrice > beePriceCeiling) {
                // Expansion ($BEE Price > 1 $ETH): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(bbond).totalSupply();
                uint256 _percentage = previousEpochBeePrice.sub(beePriceOne).mul(1000);
                uint256 _savedForBond;
                uint256 _savedForBoardroom;
                uint256 _mse = _calculateMaxSupplyExpansionPercent(beeSupply).mul(1e14);
                if (_percentage > _mse) {
                    _percentage = _mse;
                }
    ...
    function _simulateMaxSupplyExpansionPercent(uint256 _beeSupply) internal view returns (uint256) {
        for (uint8 tierId = 8; tierId >= 0; --tierId) {
            if (_beeSupply >= supplyTiers[tierId]) {
    ...
    function simulateSeigniorage() public view returns (uint256 _savedForBond, uint256 _savedForBoardroom) {
        uint256 _previousEpochBeePrice = getBeePrice();
        uint256 beeSupply = getBeeCirculatingSupply().sub(seigniorageSaved);
        if (epoch < bootstrapEpochs) {
            // 28 first epochs with 4.5% expansion
            _savedForBoardroom = beeSupply.mul(bootstrapSupplyExpansionPercent).div(10000);
        } else {
            if (_previousEpochBeePrice > beePriceCeiling) {
                // Expansion ($TOMB Price > 1 $FTM): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(bbond).totalSupply();
                uint256 _percentage = _previousEpochBeePrice.sub(beePriceOne).mul(1000);
                uint256 _mse = _simulateMaxSupplyExpansionPercent(beeSupply).mul(1e14);
                if (_percentage > _mse) {
                    _percentage = _mse;
                }
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
