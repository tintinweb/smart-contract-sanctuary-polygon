...
interface IMain {
    ...
    struct PlanStruct {
        ...
        uint256 index;
        string uuid; // to help mapping plans in web2 and web3
        uint256 budget;
     ...
     * @param _uuid the id of the plan in web2 database
    ...
    /**
     * @dev to Migrate Stakeholder(s) current address to a new one in some specific plans
     * @dev new whitelistInput must be generated and it's root hash calculated.
     * @param _planIds the index of the plans to update their whitelist context
     ...
     */
    function migrateStakeholderAddressInSpecificPlans(
    ...
    Config
{
    ...
    function getPlanDetails(uint256 _index) external view override returns (PlanStruct memory) {
        require(plans.length > 0, "no plan is deployed yet!");
        require(_index <= plans.length, "plan index out of bounds!");
        return plans[_index];
    }

    function getPlanMapping() external view returns (PlanMapping[] memory _result) {
        uint256 _count = plans.length;
        _result = new PlanMapping[](_count);
        for (uint i = 0; i < _count; i++) {
            _result[i] = PlanMapping(i, plans[i].uuid);
        }
        return _result;
    }
    ...
    function cancelStakeholders(
        ...
        for (uint i = 0; i < _stakeholdersAddress.length; i++) {
            require(getStakeholderStatus(_planIndex, _stakeholdersAddress[i]).cancelledAt == 0, "stakeholder is already cancelled!");

            // keep already vested tokens to help compute withdrawable tokens later
            ...
            // @dev then we reduce the amount of claimed tokens by the cancelled stakeholder to keep the top-up computer correct.
            ...
            // require(_p.merkleRootHash != _newRootHash, "Root hash is the same!");
            // _p.merkleRootHash = _newRootHash;
    ...
    function cancelPlansInBulk(uint256[] calldata _plansIndex) external override onlyOwner {
        for (uint i = 0; i < plans.length; i++) {
            ...
            require(_p.cancelledAt == 0, "Plan is already canceled!");
            require(!_p.isTrustless, "Forbidden!");
    ...
    function _verify(bytes32 _rootHash, uint256 _planIndex, address _addr, uint256 _allocation, bytes32[] calldata _proof) private pure {
        bytes32 _leaf = keccak256(bytes.concat(keccak256(abi.encode(_addr, _allocation))));
