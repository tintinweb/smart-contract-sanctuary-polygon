...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (tokenId >= 1 && tokenId <= 1000) {
            return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, uint256(1).toString())) : "";
        } else if(tokenId > 1000 && tokenId <= 2000){
            return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, uint256(2).toString())) : "";
        } else if(tokenId > 2000 && tokenId <= 3000){
            return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, uint256(3).toString())) : "";
        } else if(tokenId > 3000 && tokenId <= 4000){
            return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, uint256(4).toString())) : "";
        } else if(tokenId > 4000 && tokenId <= 5000){
            return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, uint256(5).toString())) : "";
        } else if(tokenId > 5000 && tokenId <= 6000){
            return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, uint256(6).toString())) : "";
        } else if(tokenId > 6000 && tokenId <= 7000){
            return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, uint256(7).toString())) : "";
        } else if(tokenId > 7000 && tokenId <= 8000){
            return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, uint256(8).toString())) : "";
        }
    }
...
contract DZPAllExcessPass is ERC721, ERC721Enumerable, Ownable {
    string public BASE_URI = "";
    uint256 public constant MAX_SUPPLY = 8000;

    mapping(address => bool) public mintApproved;

    // Provenance hash for all PPC
    // string public constant DZP_PROVENANCE =
    //     "3395fb80ea0913bfff22eb9093910885dfbf082da8c2376f3a03ce380ceee517";

    constructor() ERC721("DZP All Exce$$ Pass", "DZP") {
