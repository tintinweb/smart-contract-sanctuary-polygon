...
contract AdvancedTransfer is BaseSiloAction {
    ...
    function transferAction(
...

        {
            ...
            address[] memory path;
            address oracle;
            (input, ratio, recipients, path, oracle) = abi.decode(
                configuration,
            ...
            for (uint256 i; i < 4; ) {
                ...
                if (noConvert) {
                    ...
                    if (isSiloManager) {
                        ...
                        info.outputs[i] = info.path[info.pathLength - 1]; //set output[i] to whatever path converts it into
        ...
        uint256[] memory amounts = new uint256[](info.pathLength);
        // address[] memory path ;
        bool result;
        ...
        if (
            ...
            if (result) {
                ...
                path[0] = info.path[info.pathLength - 1];
                path[1] = ERC20_LINK_ADDRESS;

                uint256[] memory secondAmounts = new uint256[](2);

                // SafeERC20.safeIncreaseAllowance(
                //     IERC20(path[0]),
                //     QUICK_V2_ROUTER,
                //     amounts[info.pathLength - 1]
                // );

                (secondAmounts, result) = _swap(
                    info.oracle,
                    path,
                    amounts[info.pathLength - 1],
                    0,
        ...
        ///
        if (info.beta > 0 && info.pathLength > 1) {
            // Quote memory best;
    ...
    function dmmSwap(
        ...
        try
            IDMMExchangeRouter(router).swapExactTokensForTokens(
                amountIn,
                0,
                pools,
                tokenPath,
    ...
    function validateConfig(
        ...
        // uint256 amountIn = 10 ** IERC20Decimals(path[0]).decimals();
