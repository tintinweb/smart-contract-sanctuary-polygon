...
contract TestContract is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  event TestSwap(SwapInfo swap, bytes data);
  ...
  function doSwaps(SwapInfo[] calldata swaps) private {
    for (uint8 i; i < swaps.length; i++) {
      if (!IERC20Upgradeable(swaps[i].srcToken).approve(ONE_INCH_ROUTER, swaps[i].amount)) {
        revert();
      }
      (bool swapResult, bytes memory swaptData) = address(ONE_INCH_ROUTER).call(swaps[i].data);
      if (!swapResult) {
        revert("SWAP_FAILED");
      }
      else {
        emit TestSwap(swaps[i], swaptData);
      }
    }
  }
}
