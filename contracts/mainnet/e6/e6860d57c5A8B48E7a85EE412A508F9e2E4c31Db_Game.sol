...
contract GameBase is Ownable {
    ...
    {
        fund = (_liquidity.mul(address(_vault).balance)).div(_totalSupply);
        return fund;
    }
...
contract Game is ReentrancyGuard, GameBase {
    constructor(
        ...
        _name = "MATIC DICE - Crystal.Network";
        _symbol = "LP";
        _decimals = 18;
        _totalSupply = 0;
        _vault = IVAULT(vault);
    ...
    function initialze(uint256 _fund) external payable onlyOwner {
        ...
        require(
            msg.value == uint256(_fund + 1),
            "Need 1Ether and 1 Wei for initialization"
        );

        _totalInvested += uint256(_fund + 1);
        _receiveETH(uint256(_fund + 1));
        ...
        onlyRand
    {
        ...
        if (bonus > (address(_vault).balance) / 2) {
            result = isHigher
                ? result % target
    ...
    function _receiveETH(uint256 _value) internal {
        require(_vault.receiveETH{value: _value}(), "Receive ETH Error");
    }

    function _sendETH(address _user, uint256 _value) internal {
        if (_value > uint256(0)) {
            require(_vault.transfer(address(0), _user, _value), "Send ETH Error");
        }
    }
}