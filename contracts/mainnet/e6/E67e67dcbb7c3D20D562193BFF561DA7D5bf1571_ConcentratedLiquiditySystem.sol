...
contract ConcentratedLiquiditySystem is BaseContract {
  ...
  function addMenAndStMenLiquidity(uint _amount) external onlyContractCall {
    ...
    if (menToken.balanceOf(address(this)) < _amount) {
  ...
  function lock(uint _tokenAmount, uint _duration) external onlyMn {
    menToken.releaseCLSAllocation(_tokenAmount);
    lsd.mint(_tokenAmount, _duration);
  }

  function unlock(uint _stAmount) external onlyMn {
    lsd.burn(_stAmount);
  }
