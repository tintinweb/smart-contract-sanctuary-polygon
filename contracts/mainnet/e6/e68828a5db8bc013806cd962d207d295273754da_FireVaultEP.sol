...
contract FireVaultEP is ERC20 {
    ...
    address public constant FBX_VAULT = 0x960d43BE128585Ca45365CD74a7773B9d814dfBE;

    address[] public sushiPath;

    event Deposit(address indexed account, uint256 amountEP);
    event Withdraw(address indexed account, uint256 amountEP);
    ...
    // Returns the sum of EP staked and EP owned by the contract
    function totalEPBalance() public view returns (uint256) {
        (uint256 stakedEP,,) = EP.userInfos(address(this));
        return stakedEP.add(EP.balanceOf(address(this)));
    }

    // Returns how many EPs can be withdrawn per fireEP (before claiming rewards)
    function withdrawalRate() public view returns (uint256) {
        if (totalSupply() == 0) {
            return 1e18;
        }
        return totalEPBalance().mul(1e18).div(totalSupply());
    }
    
    // Claim FBX rewards and buy/mint new EPs
    function claimSwap() public {
        // Claim rewards
        uint256 rewards = EP.claimRewards();
        if (rewards > 0) {
            // Send 5% fee to the FBX Vault
            uint256 fee = rewards.mul(5).div(100);
            FBX.safeTransfer(FBX_VAULT, fee);
            // Buy or mint EP
            uint256 toSpend = FBX.balanceOf(address(this));
            if (toSpend > 0) {
                uint256 buyableAmount = SUSHI_ROUTER.getAmountsOut(toSpend, sushiPath)[1];
                if (buyableAmount > toSpend.div(300)) {
                    FBX.approve(address(SUSHI_ROUTER), toSpend);
                    SUSHI_ROUTER.swapExactTokensForTokens(toSpend, buyableAmount.mul(995).div(1000), sushiPath, address(this), block.timestamp + 1);
                }
                else {
                    FBX.approve(address(EP), toSpend);
                    EP.mintEP(toSpend.div(300));
                }
            }
        }
    }
    ...
    function claimSwapStake() public {
        claimSwap();
        stake();
    }

    // Deposit EP, in exchange for fireEP
    function deposit(uint256 amountEP) external returns (uint256 amountFireEP) {
        ...
        // claim reward to calculate updated withdrawal rate
        claimSwap();
        ...
        emit Deposit(msg.sender, amountEP);
    }

    // Redeem fireEP for underlying EP
    function withdraw(uint256 amountFireEP) external returns (uint256 amountEP) {
        ...
        emit Withdraw(msg.sender, amountEP);
    }
}