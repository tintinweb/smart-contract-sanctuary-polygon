...
contract No1 is ReentrancyGuard, Bucket {
    ...
    uint256 public day_number = 9600*10**18;
    ...
    address public TokenAddress=0xb7Dd19490951339fE65E341Df6eC5f7f93FF2779;
    ...
    address public fundAddress=0xf3Ba842b90DC7cD526aaebd60dc96Bf443a5f648;
    struct FundTarget {
    ...
    function setPause(bool _paused) external {
     ...
     function setplatformAddress(address _platformAddress) external {
        require(msg.sender == tempAdmin, "Only operator");
        platformAddress = _platformAddress;
    }
  ...
  function setTempAdmin(address _tempAdmin) external {
   ...
   function hour_limit_number()public view returns(uint256){
    ...
    function appointment(uint256 ledgerType,uint256 _number,address _referrer)external notContract nonReentrant{
        ...
        uint i = get_hourEach() ;
        uint256 _hourEach = 0;
        for(i;i<get_hourEach() + 20;i++){
            if( hourLimitNumber[i]+_number < hour_limit_number()){
                 _hourEach = i;
                break;
            }
        }
        require(_hourEach >=0,"not hour each");
   ...
   uint256 counter = 0;
    ...
    function _getSalesToLevel(uint256 amount) internal pure virtual returns (uint8) {
        ...
        if (amount < 10000*10**18) {
            ...
            return 1;
        } else if (amount < 400000*10**18) {
            return 2;
        } else if (amount < 800000*10**18) {
            return 3;
        } else if (amount < 1500000*10**18) {
            return 4;
        } else if (amount < 3000000*10**18) {
            return 5;
        } else if (amount < 6000000*10**18) {
            return 6;
        } else if (amount < 12000000*10**18) {
            return 7;
        } else if (amount < 30000000*10**18) {
            return 8;
        } else if (amount < 60000000*10**18) {
            return 9;
        } else if (amount < 120000000*10**18) {
            return 10;
        } else if (amount < 240000000*10**18) {
            return 11;
        }
        return 12;
    }
