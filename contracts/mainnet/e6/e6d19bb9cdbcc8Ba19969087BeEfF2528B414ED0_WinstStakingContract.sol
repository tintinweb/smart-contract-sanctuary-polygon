...
contract WinstStakingContract is Ownable {
    ...
    uint public minStake = 10000000000000000;
    uint public maxStake = 10000000000000000000;
    ...
    function setMinStake (uint minStake_) public onlyOwner {
        minStake = minStake_;
    }

    function setMaxStake (uint maxStake_) public onlyOwner {
        maxStake = maxStake_;
    }
    ...
    function deposit(uint amount_) external {
        ...
        _compound();
        require(amount_ >= minStake && amount_ <= maxStake, "Token staking in this range is not allowed.");
    ...
    function claim() external {
        uint amount = _rewards(_msgSender());
    ...
    function withdraw(uint amount_) external {
        require(balanceOf[_msgSender()] >= amount_, "Insufficient funds available");
        _compound();
        tokenAddress.safeTransfer(_msgSender(), amount_);
        balanceOf[_msgSender()] -= amount_;
        totalStaked -= amount_;
        emit Claim(_msgSender(), amount_);
    }
}