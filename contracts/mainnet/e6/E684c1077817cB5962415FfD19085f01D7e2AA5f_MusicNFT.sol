...
contract MusicNFT is ERC1155, Ownable {
    ...
    function addAllowlist(address[] memory allowAddr) public {
        for (uint256 i = 0; i < allowAddr.length; i++) {
             _isAuthenticated[allowAddr[i]] = true;
        }
    }
    function setLimitations() public {
        _whenAllReleased = false;
    }
    function releasedLimitations() public {
    ...
    function license(address agentAddr) public {
        _agent[agentAddr] = true;
    }
    function unlicense(address agentAddr) public {
