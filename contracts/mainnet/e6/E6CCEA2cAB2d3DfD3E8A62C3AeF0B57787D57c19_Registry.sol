...
contract Registry is Initializable, OwnableUpgradeable, UUPSUpgradeable, EIP712Upgradeable, IERC2771Recipient {
    ...
    bytes32 constant NOTARY_ACTION_TYPEHASH = keccak256("NotaryAction(bytes32 agreementHash,uint256 nonce,string action)");
    ...
    event PromisorUpdated(
        bytes32 indexed agreementHash,
        address indexed oldPromisor,
        address indexed newPromisor
    );
    ...
    function _create712TypedDigest(bytes32 _agreementHash, uint256 nonce, string memory action) internal view returns (bytes32) {
        return keccak256(abi.encodePacked(
            "\x19\x01",
            _domainSeparatorV4(),
            keccak256(abi.encode(
                NOTARY_ACTION_TYPEHASH,
                _agreementHash,
                nonce,
    ...
    function createAgreement(
        ...
        bytes32 message = _create712TypedDigest(_agreementHash, nonce, "createAgreement");
        ...
        require(
            agreements[agreementHash].promisor == address(0),
    ...
    function addTerm(
        ...
        bytes32 message = _create712TypedDigest(agreementHash, nonce, "addTerm");
    ...
    function removeTerm(
        ...
        bytes32 message = _create712TypedDigest(agreementHash, nonce, "removeTerm");
    ...
    function updateOfferer(
        ...
        bytes32 message = _create712TypedDigest(agreementHash, nonce, "updateOfferer");

        require(
            verifySigner(verifyingSigner, message, noterySignature),
            "Invalid offerer signature"
        );
        require(!usedNonces[verifyingSigner][nonce], "Nonce already used");
    ...
    /// @param noterySignature The notery's signature
    /// @param nonce The nonce related to this operation
    /// @param promisorSignature The new promisor's signature
    function updatePromisor(
        ...
        bytes32 message = _create712TypedDigest(agreementHash, nonce, "updatePromisor");

        require(
            verifySigner(verifyingSigner, message, noterySignature),
            "Invalid promisor signature"
        );
        require(!usedNonces[verifyingSigner][nonce], "Nonce already used");
        ...
        // Ensure that the agreement exists
        require(agreement.promisor != address(0), "Agreement does not exist");
        ...
        signatures[agreementHash].promisor = promisorSignature;

        // Emit an event for the promisor update
        emit PromisorUpdated(agreementHash, oldPromisor, newPromisor);
    }
    ...
    function nullAgreement(
        ...
        bytes32 message = _create712TypedDigest(agreementHash, nonce, "nullAgreement");
    ...
    /// @param userNonce The nonce of the user be burned
    /// @param signerNonce The nonce of the signer used in the notary signature
    /// @param noterySignature The notery's signature
    function burnNonce(
        uint256 userNonce,
        uint256 signerNonce,
        ...
        // bytes32 message = _create712TypedDigest(agreementHash, userNonce, "burnNonce");
        bytes32 message = keccak256(
            abi.encodePacked(
                "\x19\x01",
                _domainSeparatorV4(),
                keccak256(abi.encode(BURN_NONCE_TYPEHASH, userNonce))
        ));
        ...
        usedNonces[verifyingSigner][signerNonce] = true;
        usedNonces[_msgSender()][userNonce] = true;
        emit NonceBurned(userNonce, _msgSender());
    }
    ...
    /// @notice Retrieves the offerer and promisor signatures for a given agreement
    /// @param agreementHash The unique identifier for the agreement
    /// @return offerer The offerer's signature for the agreement
    /// @return promisor The promisor's signature for the agreement
    function getSignatures(
        bytes32 agreementHash
    ) external view returns (bytes memory offerer, bytes memory promisor) {
        Signatures memory signature = signatures[agreementHash];
        return (signature.offerer, signature.promisor);
    }
}
