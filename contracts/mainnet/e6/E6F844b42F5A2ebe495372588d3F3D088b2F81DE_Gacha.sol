    ...
    UUPSUpgradeable
{
                    ...
                    if (currentRewardToken.typeToken == TypeToken.ERC721) {
                        ...
                        if (currentRewardToken.isMintNft) {
                            ...
                            for (uint256 j = 0; j < currentIndexNFT; j++) {
                                ...
                                if (
                                    IChallenge(currentTokenAddress).ownerOf(
                                        j
                                    ) == address(this)
                                ) {
    ...
    /**
     * @dev Update reward rate and maximum number allowed for a reward token.
     * @param _indexOfReward The index of the reward token to update.
     * @param _rateOfLost The new unlock rate for the reward token.
     * @param _maxNumberAllowed The new maximum number allowed for the reward token.
     */
    function updateRewardRateAndMaxAllowed(
    ...
    /**
     * @dev Update the challenge information and Gacha contract information.
     * @param _challengeInfo The updated challenge information.
     * @param _isDefaultGachaContract A boolean value indicating whether it is the default Gacha contract.
     * @param _receiveAdminWallet The address of the receive admin wallet.
     * @param _returnedNFTWallet The address of the returned NFT wallet.
     * @param _gachaName The name of the Gacha.
     * @param _gachaSponsor The sponsor of the Gacha.
     */
    ...
    /**
     * @dev Check if the given index of token reward exists in the list of token IDs.
     * @param _indexOfTokenReward The index of the token reward to check.
     */
    function checkIndexOfTokenReward(uint256 _indexOfTokenReward) private view {
