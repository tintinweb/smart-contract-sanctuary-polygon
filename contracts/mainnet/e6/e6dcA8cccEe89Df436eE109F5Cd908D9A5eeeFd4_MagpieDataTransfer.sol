...
import "./interfaces/IMagpieDataTransfer.sol";
import "./interfaces/IWormholeCore.sol";
import "./interfaces/ILayerZero.sol";
import "./interfaces/ILayerZeroReceiver.sol";

contract MagpieDataTransfer is ReentrancyGuardUpgradeable, OwnableUpgradeable, UUPSUpgradeable, IMagpieDataTransfer {
    using LibBytes for bytes;

    address public magpieCoreAddress;
    address public magpieBridgeAddress;
    mapping(DataTransferType => mapping(uint8 => mapping(uint64 => Sequence))) public payloads;
    mapping(uint8 => uint64) private counter;
    ...
    modifier onlyMagpieCore() {
        require(msg.sender == magpieCoreAddress, "MagpieDataTransfer: only magpieCore allowed");
        _;
    }

    modifier onlyMagpieBridge() {
        require(msg.sender == magpieBridgeAddress, "MagpieDataTransfer: only magpieBridge allowed");
        _;
    }

    modifier onlyLayerZero() {
        require(msg.sender == config.layerZeroAddress, "MagpieCore: only layerZero allowed");
        _;
    }
    ...
    function updateMagpieBridge(address _magpieBridgeAddress) external {
        magpieBridgeAddress = _magpieBridgeAddress;
    }

    function dataTransferWormhole(bytes memory payload) external onlyMagpieBridge returns (uint64) {
        return
            IWormholeCore(config.wormholeAddress).publishMessage(
    ...
    function dataTransferLayerZero(
        IMagpieBridge.ValidationInPayload memory validationInPayload,
        bytes memory payload,
        address payable refundAddress
    ) external onlyMagpieBridge returns (uint64 coreSequence) {
        ILayerZero(config.layerZeroAddress).send(
            uint16(validationInPayload.layerZeroRecipientChainId),
            abi.encodePacked(validationInPayload.recipientCoreAddress),
            payload,
            refundAddress,
            address(0x0),
            "0x"
    ...
    function getVM(bytes memory encodedVm) public view onlyMagpieBridge returns (IWormholeCore.VM memory) {
        (IWormholeCore.VM memory vm, bool valid, string memory reason) = IWormholeCore(config.wormholeAddress)
    ...
    function lzReceive(
        ...
        ValidationOutPayload memory payloadOut = payload.parse();
        Sequence memory sequence = payloads[payloadOut.dataTransferType][payloadOut.senderNetworkId][
            payloadOut.coreSequence
        ];
        sequence.isUsed = false;
        sequence.payload = payloadOut;
    }

    function getCoreSequence() internal returns (uint64 coreSequence) {
        counter[config.networkId] += 1;
        coreSequence = counter[config.networkId];
    }
...
import "./IMagpieDataTransfer.sol";
...
interface IMagpieBridge {
    ...
    struct BridgeConfig {
        ...
        address magpieDataTransferAddress;
    ...
    function bridgeIn(
        ...
        IMagpieDataTransfer.DataTransferType dataTransferType,
    ...
    function bridgeOut(
        IMagpieDataTransfer.ValidationOutPayload memory payload,
...
library LibBytes {
    ...
    function toDataTransferType(bytes memory self, uint256 start)
        internal
        pure
        returns (IMagpieDataTransfer.DataTransferType)
    {
        return
            self.toUint8(start) == 0
                ? IMagpieDataTransfer.DataTransferType.Wormhole
                : IMagpieDataTransfer.DataTransferType.LayerZero;
    }

    function parse(bytes memory self) internal pure returns (IMagpieDataTransfer.ValidationOutPayload memory payload) {
        uint256 i = 0;
        ...
        payload.coreSequence = self.toUint64(i);
        i += 8;
        ...
        payload.dataTransferType = self.toDataTransferType(i);
        i += 1;
...
interface IMagpieDataTransfer {
    ...
    function dataTransferLayerZero(
        ...
        bytes memory payload,
        address payable refundAddress
    ) external returns (uint64 counter);

    function dataTransferWormhole(bytes memory payload) external returns (uint64);

    function getVM(bytes memory encodedVm) external view returns (IWormholeCore.VM memory);
