...
contract QuickMaiLpFarm is BaseSiloAction, IIncentiveKey {
    ...
    function _act(
        ...
        if (
            ...
            protocol.farm.collectRewards(key, tokenId);

            uint256 reward = protocol.farm.eternalFarming().rewards(
                address(this),
                IERC20Minimal(dQUICK_ADDRESS)
            );

            console.log("collect reward :  %s", reward);
            ...
            console.log(
                "############## QuickMaiLpFarm mai balance: %s  inputAmounts[0]: %s",
                balance,
                inputAmounts[0]
            );
            ...
            if (inputAmounts[0] > 0) {
                ...
                console.log(
                    "############## QuickMaiLpFarm _getSwapAmountForV3Lp swapAmount: %s ",
                    swapAmount
                );

                if (swapAmount > 0) {
                    // uint256 totalSupply = protocol.quickMaiLP.totalSupply();
                    //Only do the swap if the the swap amount out is non zero, and if the liquidity minted is non zero
                    // uint256 amountOut = protocol.quickRouter.getAmountsOut(
                    //     swapAmount,
                    //     path
                    // )[1];
                    // uint256 potentialLiquidityA = ((swapAmount * totalSupply) /
                    //     IERC20(path[0]).balanceOf(
                    //         address(protocol.quickMaiLP)
                    //     ));
                    // uint256 potentialLiquidityB = ((amountOut * totalSupply) /
                    //     IERC20(path[1]).balanceOf(
                    //         address(protocol.quickMaiLP)
                    //     ));
                    // if (
                    //     amountOut > 0 &&
                    //     potentialLiquidityA != 0 &&
                    //     potentialLiquidityB != 0
                    // )
                    id = getUserLastFarm(address(this));

                    {
                        ...
                        console.log(
                            "############## QuickMaiLpFarm swapExactTokensForTokens amounts[0]: %s  amounts[1]: %s ",
                            amounts[0],
                            amounts[1]
                        );
                        ...
                        if (id == 0) {
                            ...
                            console.log(
                                "add the liquidity to the original position mananger: %s: liquidity: %s",
                                tokenId,
                                liquidity
                            );
                        }
                ...
                //     // amountToRemove = protocol.quickLpFarm.balanceOf(
                //     //     address(this)
                //     // );
                // } else {
                //     //want to remove some of the stake
                //     // amountToRemove = _getLpAmount(
                //     //     address(protocol.quickMaiLP),
                //     //     address(protocol.MAI),
                //     //     inputAmounts[3]
                //     // );
                //     // if (
                //     //     protocol.quickLpFarm.balanceOf(address(this)) <
                //     //     amountToRemove
                //     // ) {
                //     //     amountToRemove = protocol.quickLpFarm.balanceOf(
                //     //         address(this)
                ...
                // console.log(
                //     " remove v3 lp from stake v3 inputAmounts[3]  %s",
                //     inputAmounts[3]
                // );

                {
                    ...
                    if (id != 0) {
                        (, , tokenId) = protocol.farm.l2Nfts(id);
                        console.log(
                            " remove v3  farm nft id :  %s position nft id: %s",
                            id,
                            tokenId
                        );
                        ...
                        if (inputAmounts[3] == 2**256 - 1) {
                            //want to remove entire stake
                            IIncentiveKey.IncentiveKey
                                memory key = IIncentiveKey.IncentiveKey({
                                    rewardToken: IERC20Minimal(dQUICK_ADDRESS),
                                    bonusRewardToken: IERC20Minimal(QI_ADDRESS),
                                    pool: protocol.pool,
                                    startTime: 1665193197,
                            ...
                            protocol.farm.exitFarming(key, tokenId, false);
                            ...
                            console.log(
                                " remove v3  claim dQUICK reward: %s",
                                reward
                            );
                            ...
                            // console.log(" remove v3 claim  QI reward  start ");
                            ...
                            console.log(
                                " remove withdrawToken tokenId: %s",
                                tokenId
                            );

                            (, , , , , , removeLiquidity, , , , ) = protocol
                                .positionManager
                                .positions(tokenId);
                        } else {
                            //want to remove some of the stake
                        ...
                        // uint256 posBalance = protocol.positionManager.balanceOf(
                        //     address(this)
                        // );

                        // console.log(
                        //     " remove v3  protocol.farm.withdrawTokens  posBalance: %s",
                        //     posBalance
                        // );

                        if (removeLiquidity > 0) {
                            console.log(
                                " removeLiquidity liquidity: %s",
                                removeLiquidity
                            );
                            (
                                uint256 amount0,
        ...
        )
    {
        ...
        console.log("mint new position liquiidty");
        ...
        protocol.farm.enterFarming(key, tokenId, 0, false);
    }
        ...
        returns (ActionBalance memory)
    {
        ...
        (
            ...
            uint256 amount0,
            uint256 amount1
        ) = _getFarmLiquidityAmount(_silo, protocol);
        ...
        return
            ActionBalance({
                ...
                debt: 0,
                collateralToken: address(protocol.pool),
                debtToken: address(0),
                ...
                lpUnderlyingTokens: string(
                    abi.encodePacked(
                        Strings.toHexString(uint160(address(protocol.MAI)), 20)
                    )
                )
            });
    }
        ...
        returns (bool)
    {
        ...
        uint256 reward = farm.eternalFarming().rewards(
            address(this),
            IERC20Minimal(dQUICK_ADDRESS)
        );
    ...
    {
        IFarmingCenter farm = IFarmingCenter(QUICK_V3_FARM);
        pendingReward = farm.eternalFarming().rewards(
            address(this),
            IERC20Minimal(dQUICK_ADDRESS)
        );
    }
    ...
    {
        if (protocol.farm.balanceOf(silo) > 0) {
            uint256 id = getUserLastFarm(silo);
            (, , uint256 tokenId) = protocol.farm.l2Nfts(id);
            (
                ,
                ,
                ,
            ...
            ) = protocol.positionManager.positions(tokenId);
