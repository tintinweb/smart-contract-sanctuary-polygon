...
contract DDAOClaimDBayc is AccessControl
{
    using SafeMath  for uint256;
    using SafeERC20 for IERC20;

    event eClaim(uint256 grp,address addr,uint256 amount,uint256 payed);

    uint48  constant public TimeStart   = 1649203200; // 6 Apr 2022
    uint256 constant public AmountMax   = 860000;
    uint48  constant public TimeEnd     = TimeStart + (180+27) * 86400;

    address public AddrDDAO = 0x90F3edc7D5298918F7BB51694134b07356F7d0C7;

    mapping(uint256 => uint256)public Group;
    mapping(uint256 => uint256)public GroupLen;
    mapping(uint256 => address[])public MemberAddr;
    mapping(uint256 => uint256[])public MemberKoef;
    mapping(uint256 => mapping(address => uint256))	public MemberNum;
    ...
    mapping(uint256 => mapping(address => uint256))public MemberEpoch;

    mapping(uint256 => mapping(address => uint256))public Payed;
    ...
    mapping(uint256 => mapping(address => uint256))public Member;
   ...
   struct history
	...
	uint256 grp_sum;
        ...
        Group[4] = 0;
            ...
            if(block.chainid == 137)
            {
                ...
                GroupMemberAdd(1,0x04Dfb309d28b7eFb116Cc97A1385c22815B9d3f8,17954936170);
                GroupMemberAdd(1,0xa542e3CDd21841CcBcCA70017101eb6a2fc68723,17954936170);
                GroupMemberAdd(1,0xc7B5D7057BB3A77d8FFD89D3065Ad14E1E9deD7c,11566106000);
                GroupMemberAdd(1,0x8ac51a5d5921Bf5640ddc6633f13FcD0d8e19be6,8614500358);
                GroupMemberAdd(1,0x3ef7Bf350074EFDE3FD107ce38e652a10a5750f5,7054935757);
                GroupMemberAdd(1,0x237b3c12D93885b65227094092013b2a792e92dd,5793523218);
                GroupMemberAdd(1,0xBD0aa1CF9FB2af52d6E81ef95828B0F54baDf343,5641954280);
                GroupMemberAdd(1,0x98BCE99aa50CB33eca0dDcb2a04404B80dEd3F3E,5509331459);
                GroupMemberAdd(1,0x3BC782A05f09e015d96337a5184F143c956F6aA3,3525972134);
                GroupMemberAdd(1,0x8DcAC18996dC58b5eebfDC5D0A7b22E0B951A4b7,3524974970);
                GroupMemberAdd(1,0xA54d34374d4FEb2583BE4A7278e875Fd773764Fa,3524974970);
                GroupMemberAdd(1,0x256b09f7Ae7d5fec8C8ac77184CA09F867BbBf4c,2857872210);
                GroupMemberAdd(1,0x7AE29F334D7cb67b58df5aE2A19F360F1Fd3bE75,2264559592);
                GroupMemberAdd(1,0x3cB704A5FB4428796b728DF7e4CbC67BCA1497Ae,2106010505);
                GroupMemberAdd(1,0xa486fEBD45dBb3ac859Ff85B7039D7327C012bc1,1406998496);
                GroupMemberAdd(1,0x7d2D2E04f1Db8B54746eFA719CB62F32A6C84a84,562400533);
                GroupMemberAdd(1,0x2A77484F4cca78a5B3f71c22A50e3A1b8583072D,108690883);
                GroupMemberAdd(1,0xC9A9943A2230ae6b3423F00d1435f96950f82B23,27322295);
    ...
    function GroupMemberAdd(uint256 grp,address addr, uint256 koef)public onlyAdmin
    {
    ...
    function GroupMemberChange(uint256 grp,address addr, uint256 koef)public onlyAdmin
    {
	uint256 i;
    ...
    function GroupKoefShow(uint256 grp)public view returns(uint256[] memory)
    {
	return MemberKoef[grp];
    }
    function GroupKoefSum(uint256 grp)public view returns(uint256 sum)
    {
	for(uint256 i = 0;i < MemberKoef[grp].length;i++)
	{
    ...
    function RewardCalc(uint256 grp, address addr, uint48 time)public view returns(uint256 amount)
    {
	amount = 0;
	reward_calc memory temp;
	bool flag = false;
	uint256 num = MemberNum[grp][addr];
    ...
    function PartAmount(uint256 val, uint256 sum)public pure returns(uint256 out)
    {
    ...
    function Claim(uint256 grp,address addr,uint256 amount)public
    {
    ...
    function ClaimAmount(uint256 grp, address addr)public view returns(uint256 amount)
    {
	if(MemberGrp[addr].enable)
	{
    ...
    function ClaimAmount(address addr)public view returns(uint256 amount)
    {
	amount = ClaimAmount(MemberGrp[addr].grp,addr);
    }
    function balanceOf(address addr)public view returns(uint256 amount)
    {
	amount = ClaimAmount(MemberGrp[addr].grp,addr);
    }
    function GroupMemberShow(uint256 grp)public view returns(address[] memory out)
    {
        out = MemberAddr[grp];
    }
    function EpochViewArr(uint256 epoch_num,uint256 grp)public view returns(uint256[] memory out)
    {
	out = Epoch[epoch_num].arr[grp];
    }
    function EpochViewSum(uint256 epoch_num,uint256 grp)public view returns(uint256 out)
    {
	out = Epoch[epoch_num].sum[grp-1];
    }
    function EpochViewGrp(uint256 epoch_num,uint256 grp)public view returns(uint256 out)
    {
	out = Epoch[epoch_num].grp[grp-1];
    }
    function Claimed(address addr)public view returns(uint256 out)
    {
	if(MemberGrp[addr].enable)
	out = Payed[MemberGrp[addr].grp][addr];
	else
	out = 0;
    }
}
