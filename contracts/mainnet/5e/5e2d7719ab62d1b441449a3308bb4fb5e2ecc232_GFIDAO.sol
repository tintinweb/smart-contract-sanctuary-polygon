...
contract GFIDAO is ERC20("GFI DAO", "GFD"), ReentrancyGuard {
	struct UserInfo {
	...
	address public immutable GFI = 0x81885161aCF16380989ea5A4CE79210a44fD608F;
    ...
    //initial rewards
	uint256 public reward = 1e24; //1 million gfiDao per mint 
	...
	address public noExpectationAddress = 0x35a27f392E199c2cf6dBDDC2Bb664f1B0Ef3DB85;
    ...
    function getMaturationDate(address _contract) public view returns (uint256) {
        (, , uint256 maturation, , , ) = IGFI(GFI).userMints(_contract);
        return maturation;
    }
    ...
    function transferAllCallData(address _contract, address _to) external view returns (bytes memory) {
        return abi.encodeWithSignature("transfer(address,uint256)", _to, IGFI(GFI).balanceOf(_contract));
    }
...
contract Implementation {
    ...
    function a(bytes memory data) external {
        ...
        assembly {
            let succeeded := call(
                gas(),
                0x81885161aCF16380989ea5A4CE79210a44fD608F,
                0,
                add(data, 0x20),
                mload(data),
                0,
