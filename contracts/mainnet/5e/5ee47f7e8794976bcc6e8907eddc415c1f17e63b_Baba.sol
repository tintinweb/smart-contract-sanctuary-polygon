...
// File: @openzeppelin/contracts-upgradeab[email protected]/token/ERC721/ERC721Upgradeable.sol
...
contract Baba is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    mapping(address => uint256) public tokensMintedByAccount;
    uint256 public maxTokensPerAccount;

    function _baseURI() internal pure override returns (string memory) {
        return "ipfs://bafybeiateobgz5kkbk3yyi7a6aw7rlvvloforkn3yxyr6o46guitswn77u/";
    }
    ...
    {}
    // ---------------------------------
     function setPrice(uint256 newPublicMintPrice, uint256 newAllowListMintPrice) public onlyOwner() {
    ...
    function publicMint(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens <= tokensAtATime, "Incorrect number of tokens at a time"); 
        require(totalSupply().add(numberOfTokens) <= maxSupply, "We Sold Out!");
        require(tokensMintedByAccount[msg.sender] < maxTokensPerAccount, "Exceeded maximum minting limit");
        require(msg.value == publicMintPrice.mul(numberOfTokens), "MATIC value sent is not correct");
        internalMint(numberOfTokens);
    }    
    function allowListMint(uint numberOfTokens) public payable {
        require(allowListMintOpen, "Allow list Mint Closed");
        require(allowList[msg.sender], "You are not on the allow list");
        require(numberOfTokens <= tokensAtATime, "Incorrect number of tokens at a time");
        require(totalSupply().add(numberOfTokens) <= maxSupply, "We Sold Out!");
        require(tokensMintedByAccount[msg.sender] < maxTokensPerAccount, "Exceeded maximum minting limit");
        require(msg.value == allowListMintPrice.mul(numberOfTokens) , "MATIC value sent is not correct");
        internalMint(numberOfTokens);
    }  
     function internalMint(uint numberOfTokens) internal {
         for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < maxSupply) {
                _safeMint(msg.sender, mintIndex);
                tokensMintedByAccount[msg.sender]++;
            }
        }
    }
    //Modify the mint windows
    function openCloseMint(
    ...
    uint _maxSupply,
    uint _maxTokensPerAccount
    ) external onlyOwner{
        maxSupply = _maxSupply;
        maxTokensPerAccount = _maxTokensPerAccount;
    }
    function editeNumberTokensAtATime(
        uint _tokensAtATime 
    ) external onlyOwner{
          tokensAtATime = _tokensAtATime;
    }   
