...
contract ApeSwapZapFullV5 is
    ...
    Multicall
{
    constructor(ITreasury goldenBananaTreasury) ApeSwapZapPools(goldenBananaTreasury) {}
}
...
abstract contract ApeSwapZapTBills {
    ...
    function zapTBill(zapTBillParams memory params) external {
        ...
        if (params.inputAmount == Constants.CONTRACT_BALANCE) {
            params.inputAmount = inputToken.balanceOf(address(this));
        } else {
            uint256 balanceBefore = inputToken.balanceOf(address(this));
            IERC20(address(inputToken)).safeTransferFrom(msg.sender, address(this), params.inputAmount);
            params.inputAmount = inputToken.balanceOf(address(this)) - balanceBefore;
        }
...
abstract contract ApeSwapZapMiniApeV2 {
    using SafeERC20 for IERC20;
...
abstract contract ApeSwapZapPools {
    using SafeERC20 for IERC20;
...
abstract contract ApeSwapZapVaults {
    using SafeERC20 for IERC20;
...
interface IZapLiquidity {
    struct LiquidityV2Params {
        address lpRouter;
        address token0;
        address token1;
        uint256 amount0Desired;
...
interface IZapSwap {
    ...
    function swap(SwapParams memory zapParams) external payable;
...
contract ZapAnalyzer is IZapAnalyzer {
    ...
    function estimateSwapReturns(
        ...
        if (params.liquidityPath.liquidityType == LiquidityType.V2) {
            ...
            // vars.factory = IApeFactory(router.factory());
            // vars.inputAmountHalf = params.inputAmount / 2;
            // returnValues.minAmountSwap0 = vars.inputAmountHalf;
            // if (params.path0.path.length > 1) {
            //     uint256[] memory amountsOut0 = router.getAmountsOut(vars.inputAmountHalf, params.path0.path);
            //     returnValues.minAmountSwap0 = amountsOut0[amountsOut0.length - 1];
            // }
            // returnValues.minAmountSwap1 = vars.inputAmountHalf;
            // if (params.path1.path.length > 1) {
            //     uint256[] memory amountsOut1 = router.getAmountsOut(vars.inputAmountHalf, params.path1.path);
            //     returnValues.minAmountSwap1 = amountsOut1[amountsOut1.length - 1];
            // }

            // //V2 LP
            // IApePair lp = IApePair(vars.factory.getPair(vars.token0, vars.token1));
            ...
            // }
            // returnValues.minAmountLP1 = IApeRouter02(params.path0.swapRouter).quote(
            //     returnValues.minAmountSwap0,
            //     vars.reserveA,
            //     vars.reserveB
            // );
...
contract ZapLiquidity is IZapLiquidity {
    using SafeERC20 for IERC20;

    function addLiquidityV2(LiquidityV2Params memory params) external returns (uint256 amount0Lp, uint256 amount1Lp) {
        if (params.amount0Desired == Constants.CONTRACT_BALANCE) {
        ...
        if (params.amount1Desired == Constants.CONTRACT_BALANCE) {
        ...
        console.log("desired0", params.amount0Desired, IERC20(params.token0).balanceOf(address(this)));
        console.log("desired1", params.amount1Desired, IERC20(params.token1).balanceOf(address(this)));
        ...
        TransferDustBack(IERC20(params.token0), IERC20(params.token1));
    }

    function addLiquidityV3(
        LiquidityV3Params memory params
    ...
    function addLiquidityArrakis(
        LiquidityArrakisParams memory params
    ) external returns (uint256 amount0Lp, uint256 amount1Lp) {
        if (params.amount0Desired == Constants.CONTRACT_BALANCE) {
            params.amount0Desired = IERC20(params.token0).balanceOf(address(this));
        } else {
            IERC20(params.token0).safeTransferFrom(msg.sender, address(this), params.amount0Desired);
    ...
    function TransferDustBack(IERC20 token0, IERC20 token1) internal {
        //TODO just transfer difference?
...
contract ZapSwap is IZapSwap, ReentrancyGuard {
    using SafeERC20 for IERC20;
    ...
    function swap(SwapParams memory params) external payable override nonReentrant {
        ...
        if (msg.value > 0) {
            require(
                address(params.inputToken) == address(0),
                "ApeSwapZap: Set inputToken to address(0) for native zap"
            );
        } else if (params.inputAmount == Constants.CONTRACT_BALANCE) {
            params.inputAmount = IERC20(params.inputToken).balanceOf(address(this));
        } else {
            uint256 balanceBefore = _getBalance(params.inputToken);
            IERC20(params.inputToken).safeTransferFrom(msg.sender, address(this), params.inputAmount);
            params.inputAmount = _getBalance(params.inputToken) - balanceBefore;
        }
        ...
        if (params.swapType == SwapType2.MultiSwapRouter) {
            IApeSwapMultiSwapRouter(params.caller).multicall{value: params.inputAmount}(params.swapData);
        } else {
