  ...
  Ownable
{
  ...
  // reward token accepted for contributions
  // and paid out on burn
   ...
   * @param _mintEndTime block time before which minting will be allowed
   */
  constructor(
   ...
   * @return _mintEndTime may also be retrieved by querying mintEndTime directly
   */
  function state()
    external
    view
  ...
  /**
   * @notice number of contributorsbuil
   ...
   * @param _nft address of the token balance to withdraw
...
interface IeQI {
  function userInfo(address user) external view returns (uint256 amount, uint256 endBlock);
}
  ...
  GamifiedNFT
{
    ...
    returns (uint256 _amount)
  {
    ...
    // 1. sum of sequence [S = n/2(a+l)] is used to create a number of mint/burn shares.
    //    the number of shares is such that there is a predetermined ratio between the number
    //    of shares that the best and worst case minters/burners will receive. for example
    ...
    // 2. the mint/burn reward amount (half of the total reward) is divided by the total number of
    //    shares to determine a value per share
    // 3. determine the incremental number of shares rewarded for each mint/burn order
    // 4. calculate the total number of shares rewarded for the given mint/burn order
    // 5. give the payout for the given mint/burn order
  ...
  /**
   * @notice convenience function to get all nfts ids owned by an address at once
