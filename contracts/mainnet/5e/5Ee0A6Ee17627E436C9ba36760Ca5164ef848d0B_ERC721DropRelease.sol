...
import "./forked/ERC721Drop.sol";

/**
 *  ERC721DropRelease is an implementation of the ERC721 Drop.
 *      - Claim Overrides - Once Allowed claimer's max claim is met, the claimer is moved to general claim settings.
 *
 *  Special thank you to Nacho Lacovino for helping with the claims override logic. The logic can be found in https://thirdweb.com/nach.eth/DynamicFreeMint
 */
contract ERC721DropRelease is ERC721Drop {
    constructor(
        string memory _name,
...
import "@thirdweb-dev/contracts/extension/interface/IDropSinglePhase.sol";
...
abstract contract DropSinglePhase is IDropSinglePhase {
    ...
    function claim(
        ...
        uint256 claimLimit;
        uint256 claimPrice;
        uint256 _supplyClaimedByWallet;

        (, isActiveOverride, claimLimit, claimPrice, _supplyClaimedByWallet) = verifyClaim(
            _dropMsgSender(),
            _quantity,
            _currency,
            _pricePerToken,
        ...
        )
    {
        ...
        claimPrice = currentClaimPhase.pricePerToken;
        address claimCurrency = currentClaimPhase.currency;

       _supplyClaimedByWallet = supplyClaimedByWallet[conditionId][ _claimer]; 
        ...
        isActiveOverride = (isOverride && _supplyClaimedByWallet < _allowlistProof.quantityLimitPerWallet)
        ...
        if ( _currency != claimCurrency || (_pricePerToken != claimPrice && !isActiveOverride) ) {
            revert("!PriceOrCurrency");
        }
           
        if ( _quantity == 0 || (_quantity + _supplyClaimedByWallet > claimLimit && !isActiveOverride)
        ) {
            revert("!Qty");
        }
    ...
    function _getTotalPrice(
        ...
        uint256 _quantityAlreadyClaimed,
        bool    _isActiveOverride,
        uint256 _pricePerToken,
        uint256 _claimPrice
    ) internal pure returns (uint256 totalPrice) {
        if (_isActiveOverride) {
            uint256 discountAllowance = (_quantityForFree > 0) && (_quantityForFree > _quantityAlreadyClaimed) ? _quantityForFree - _quantityAlreadyClaimed : 0;
            uint256 discountQuantity = _quantityToClaim > discountAllowance ? discountAllowance : _quantityToClaim;
            uint256 paidQuantity = _quantityToClaim > discountQuantity ? (_quantityToClaim - discountQuantity) : 0;
            totalPrice = (paidQuantity * _pricePerToken) + (discountQuantity *  _claimPrice);
        } else {
