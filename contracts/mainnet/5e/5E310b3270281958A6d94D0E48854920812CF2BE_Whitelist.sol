...
contract Whitelist is IWhitelist, AccessControlEnumerable {
    ...
    event Whitelist_add(address[] users);
    event Whitelist_remove(address[] users);

    constructor() {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    function whitelistCheck(address _address) view external override returns(bool) {
    ...
    function addToWhitelist(address[] memory _addresses) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i; i < _addresses.length; i++) {
            _whitelistAddresses[_addresses[i]] = true; 
        }
        emit Whitelist_add(_addresses);
    }

    function removeFromWhitelist(address[] memory _addresses) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i; i < _addresses.length; i++) {
            _whitelistAddresses[_addresses[i]] = false;
        }
        emit Whitelist_remove(_addresses);
    }

    function setupAdmin(address account) public onlyRole(DEFAULT_ADMIN_ROLE) {
