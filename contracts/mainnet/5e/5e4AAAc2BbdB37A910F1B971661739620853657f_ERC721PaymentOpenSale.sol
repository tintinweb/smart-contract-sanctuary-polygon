...
contract ERC721PaymentOpenSale is ERC721BaseSale, __with_semver(uint8(1), uint8(0), uint8(0)) {
    ...
    function buy(
        ...
        if (nativeTokenWrapperAddress != tokenAddress) {
            ...
            _tokenTransferFromToTreasury(tokenAddress, costInTokens);
        } else {
            _transferToTreasury();
        }
    ...
    function _tokenTransferFromToTreasury(address tokenAddress_, uint256 costInTokens_) internal {
        address wallet = kernel.getAppAddress(AppsIds.TREASURY);
        require(wallet != address(0x00), Exceptions.NO_TREASURY_ADDRESS);
        
        IERC20 token = IERC20(tokenAddress_);

        uint256 currentAllowance = token.allowance(msg.sender, wallet);
        require(currentAllowance >= costInTokens_, Exceptions.INSUFFICIENT_ALLOWANCE);

        token.safeTransferFrom(msg.sender, wallet, costInTokens_);
    }
}
    ...
    __with_semver(uint8(1), uint8(0), uint8(0))
{
    ...
    function _getPrice(address poolAddress, uint32 twapInterval) internal view returns (uint256 priceX96) {
        if (twapInterval == 0) {
            ...
            return UniswapMath.mulDiv(sqrtPriceX96, sqrtPriceX96, FixedPoint96.Q96) >> (96 * 2);
        }
    }
}