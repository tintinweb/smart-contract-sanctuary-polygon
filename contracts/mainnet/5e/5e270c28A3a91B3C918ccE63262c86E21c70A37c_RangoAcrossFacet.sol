...
contract RangoAcrossFacet is IRango, ReentrancyGuard, IRangoAcross {
    ...
    function acrossBridge(
        ...
        // todo: should we check request.toToken == bridgeRequest.originToken?
    ...
    /// @param fromToken address of token on source chain
    /// @param amount the amount of token to be deposited to bridge
    function acrossBridge(
        AcrossBridgeRequest memory request,
        address fromToken, // todo do we need this when we have originToken on AcrossBridgeRequest?
        uint256 amount
    ) external payable nonReentrant {
    ...
    function doAcrossBridge(
        AcrossBridgeRequest memory request,
        address fromToken,// todo: do we need this param? we have request.originToken
        uint256 amount
    ) internal {
        AcrossStorage storage s = getAcrossStorage();
