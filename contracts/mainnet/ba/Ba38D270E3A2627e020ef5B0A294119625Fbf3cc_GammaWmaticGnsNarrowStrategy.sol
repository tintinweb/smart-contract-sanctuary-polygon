...
contract GammaWmaticGnsNarrowStrategy is GiddyStrategyV2, Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  uint256 constant private BASE_PERCENT = 1e6;
  uint256 constant private COMPOUND_THRESHOLD_QUICK = 1e18;
  uint256 constant private PID = 44;
  ...
  address constant private POS = 0x688cb9492bd2C72016f1765d813B2D713aa1F4C7;
  address constant private WMATIC_TOKEN = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
  address constant private GNS_TOKEN = 0xE5417Af564e4bFDA1c483642db72007871397896;
  ...
  function getContractRewards() public view override returns (uint256[] memory amounts) {
    ...
    amounts[0] += IERC20(DQUICK_TOKEN_AND_LAIR).balanceOf(address(this));
    amounts[0] = IDragonsLair(DQUICK_TOKEN_AND_LAIR).dQUICKForQUICK(amounts[0]);
    amounts[0] += IERC20(QUICK_TOKEN).balanceOf(address(this));
    amounts[1] = COMPOUND_THRESHOLD_QUICK;
  }
  ...
  function compound(SwapInfo[] calldata swaps) external override onlyVault returns (uint256 staked) {
    ...
    if (swaps[0].amount > 0) {
      amounts[0] += GiddyLibraryV2.routerSwap(router, swaps[0], address(this), address(this), WMATIC_TOKEN);
    }
    if (swaps[1].amount > 0) {
      amounts[1] += GiddyLibraryV2.routerSwap(router, swaps[0], address(this), address(this), GNS_TOKEN);
    }
    amounts[0] = deductEarningsFee(WMATIC_TOKEN, amounts[0]);
    amounts[1] = deductEarningsFee(GNS_TOKEN, amounts[1]);
  ...
  function withdraw(uint256 staked) external override nonReentrant onlyVault returns (uint256[] memory amounts) {
    ...
    SafeERC20Upgradeable.safeTransfer(IERC20Upgradeable(WMATIC_TOKEN), address(vault), amounts[0]);
    SafeERC20Upgradeable.safeTransfer(IERC20Upgradeable(GNS_TOKEN), address(vault), amounts[1]);
  }
  ...
  function depositLP(uint256[] memory amounts) private returns (uint256 staked) {
    SafeERC20Upgradeable.safeApprove(IERC20Upgradeable(WMATIC_TOKEN), POS, amounts[0]);
    SafeERC20Upgradeable.safeApprove(IERC20Upgradeable(GNS_TOKEN), POS, amounts[1]);
