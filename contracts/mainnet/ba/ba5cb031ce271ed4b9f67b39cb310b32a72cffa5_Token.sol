...
contract Token is ERC721Enumerable, Ownable {
    ...
    uint256 public maxPerTransactionGeneralSale = 10000;
    uint256 public maxPerUserPreSale = 1;
    uint256 public maxTokensGeneralSale = 10000;
    uint256 public maxTokensPreSale = 1;
    ...
    function mintGeneralSale(uint256 _amountToMint) public payable mintPriceCompliance(_amountToMint) {
        require(generalSaleIsActive, "General sale is not active yet.");
        require(
            _amountToMint <= maxPerTransactionGeneralSale,
            "Exceeds buyer address limit for sale."
        );
        require(
            (totalSupply() + _amountToMint) <= (maxTokensGeneralSale - reservedTokens),
            "Exceeds supply for sale."
        );
    ...
    function mintPreSale() public payable mintPriceCompliance(1){
        uint256 numberOfTokensCurrentlyOwned = balanceOf(msg.sender);

        // Potentially remove this if statement
        require(preSaleIsActive, "Pre-sale is not active.");
        require(
            !generalSaleIsActive,
            "General sale must be inactive throughout the pre-sale."
        );
        require(
            numberOfTokensCurrentlyOwned < maxPerUserPreSale,
            "You can only mint one per address during the pre-sale."
        );
        require(
            (totalSupply() + 1) <= maxTokensPreSale,
            "Exceeds token supply for pre-sale."
        );
    ...
    function giveAway(address _to, uint256 _amount) external onlyOwner {
        require(
            _amount <= reservedTokens,
            "Request exceeds reserved supply of our legends."
        );
    ...
    function numbersOfTokensOwnedBy(address _owner)
        public
        view
        returns (uint256)
    {
    ...
    {
        require(_exists(_tokenId), "Query made for nonexistent token.");

        string memory tokenURISuffix = ".json";
        string memory tokenIdentifier = string(
            abi.encodePacked(Strings.toString(_tokenId), tokenURISuffix)
        );
        string memory base = baseURI();

        // If no base URI, then we actually have not revealed the collection.
        if (bytes(base).length == 0) {
            return string(abi.encodePacked(loadingURI, Strings.toString(_tokenId)));
        }
