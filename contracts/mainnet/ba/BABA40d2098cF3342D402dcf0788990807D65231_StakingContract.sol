...
contract StakingContract is ReEntrancyGuard{
    ...
    address public _link = 0x53E0bca35eC356BD5ddDFebbD1Fc0fD03FaBad39; //Chainlink
    IERC20 token = IERC20(_link);
    address public owner;
    address payable public LiquiditywalletAddress = payable(0x79bDB6A591F30e81d8Db033cfd3B6a509A35c294);   
    uint256 public customAPY = 1000;
    uint256 public LiquidityFee = 650;
    uint256 public refFee = 250;
    ...
    constructor() public {
     ...
     function set_linkAddress(address payable _linkAddress) external {
        require(msg.sender == owner , "Caller must be Owner!");
        _link = _linkAddress;
    }

    function setLiquidityAddress(address payable LiquidityAddress) external {
        require(msg.sender == owner , "Caller must be Owner!");
        LiquiditywalletAddress = LiquidityAddress;
    }

    function setLiquidityFeePercent(uint256 Fee) external {
        require(msg.sender == owner , "Caller must be Owner!");
        LiquidityFee = Fee;
    }
    ...
    function customStaking(uint256 _amount) external noReentrant {
        ...
        uint256 feeAmount = _amount * LiquidityFee / 1000;
        ...
        token.transfer(LiquiditywalletAddress, feeAmount  );
    ...
    function changeAPY(uint256 _value) external {
        ...
        require(
            _value > 0,
            "APY value has to be more than 0, try 1000 for (1.00% daily) instead"
        );
        customAPY = _value;
    }
    ...
    function claimRewards() noReentrant external {
            ...
            uint256 numdays = (block.timestamp - lastRewardTime[recipient]) / 259200;
            require(numdays > 0 , "Reward Already Claimed in Last 72 Hours!");
    ...
    {
        uint256 nextclaimTime = lastRewardTime[account] + 259200;
