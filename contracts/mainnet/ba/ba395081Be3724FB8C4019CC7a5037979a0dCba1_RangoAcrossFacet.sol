...
contract RangoAcrossFacet is IRango, ReentrancyGuard, IRangoAcross {
    ...
    /// @notice An event showing that an Across bridge call happened
    ...
    function acrossBridge(
        ...
        if (request.toToken != LibSwapper.ETH)
            LibSwapper.approve(bridgeRequest.originToken, bridgeRequest.spokePoolAddress, out);
        doAcrossBridge(bridgeRequest, out);
    }

    /// @notice Sends money to Across bridge
    /// @param request required data for the bridging step, including the destination chain and recipient wallet address
    /// @param amount the amount of token to be deposited to bridge
    function acrossBridge(
        AcrossBridgeRequest memory request,
        uint256 amount
    ) external payable nonReentrant {
        // transfer tokens if necessary
        // request.originToken can be address(0) and will be replaced in doAcrossBridge
        if (request.originToken == LibSwapper.ETH) {
            require(msg.value >= amount, "Insufficient ETH sent for bridging");
        } else {
            SafeERC20.safeTransferFrom(IERC20(request.originToken), msg.sender, address(this), amount);
            LibSwapper.approve(request.originToken, request.spokePoolAddress, amount);
        }
        doAcrossBridge(request, amount);
    }
    ...
    function doAcrossBridge(
        ...
        IAcrossSpokePool spoke = IAcrossSpokePool(request.spokePoolAddress);

        address token = request.originToken;
        if (token == LibSwapper.ETH) token = LibSwapper.getBaseSwapperStorage().WETH;

        spoke.deposit{value : request.originToken == LibSwapper.ETH ? amount : 0}(
            request.recipient,
            token,
            amount,
