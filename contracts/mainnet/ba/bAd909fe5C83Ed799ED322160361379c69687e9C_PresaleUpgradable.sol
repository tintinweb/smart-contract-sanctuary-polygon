    ...
    UUPSUpgradeable
{
    ...
    uint256 private USDContractDecimals;
    ...
    //Referral Function & Variables
    receive() external payable {
        BuyWithETH(defaultReferrer);
    }
    ...
    function getTotalReferralBonus() external view returns (uint256) {
        ...
        for (uint8 i; i < levelLength; i++) {
            referralBonus += uint8(levelRate[i]);
        }
    ...
    function setUSDContractAdmin(
        ...
        USDContractDecimals = IERC20_EXTENDED(USDContract).decimals();
        return true;
    }

    function getUSDContractDecimals() public view returns (uint256) {
        return USDContractDecimals;
    }

    function getRewardTokenContractInfo() external view returns (address) {
    ...
    function setRewardPerUSD(uint256 _value) external returns (bool) {
        rewardPerUSD = _value;
        return true;
    }
        ...
        returns (uint256 minETHRequired)
    {
        if (minContributionUSD == 0) {
            ...
            minETHRequired = (1 * 10 ** tokenDecimals) / ratio;
        }
    }
    ...
    function getMinContributionETH()
        ...
        returns (uint256 minETHRequired)
    {
        minETHRequired = _getMinContributionETH();
    }
    ...
    function getUserRefereeCount(
        ...
        Account storage userAccount = accounts[_address];
        userRefereeCount = userAccount.referredAddresses.length;
    }

    function getUserTeamRefereeAddress(
        address _address
    ) external view returns (address[] memory userTeamReferees) {
        Account storage userAccount = accounts[_address];
        userTeamReferees = userAccount.teamAddress;
    }

    function getUserTeamRefereeCount(
        address _address
    ) external view returns (uint256 userTeamRefereeCount) {
        Account storage userAccount = accounts[_address];
        userTeamRefereeCount = userAccount.teamAddress.length;
    }

    function getUserTotalBusinessETH(
        address _address
    ) external view returns (uint256 userTotalBusiness) {
        Account storage userAccount = accounts[_address];
        userTotalBusiness = userAccount.totalBusinessETH;
    }

    function getUserTotalBusinessUSD(
        address _address
    ) external view returns (uint256 userTotalBusiness) {
        Account storage userAccount = accounts[_address];
        userTotalBusiness = userAccount.totalBusinessUSD;
    }

    function getUserTotalRewardPaidETH(
        address _address
    ) external view returns (uint256 userTotalRewardPaid) {
        Account storage userAccount = accounts[_address];
        uint256 length = userAccount.rewardPaidETH.length;
        for (uint256 i; i < length; i++) {
            userTotalRewardPaid += userAccount.rewardPaidETH[i];
        }
    }

    function getUserTotalRewardPaidUSD(
        address _address
    ) external view returns (uint256 userTotalRewardPaid) {
        Account storage userAccount = accounts[_address];
        uint256 length = userAccount.rewardPaidUSD.length;
        for (uint256 i; i < length; i++) {
            userTotalRewardPaid += userAccount.rewardPaidUSD[i];
        }
    }
    ...
    function getTeamCount(
        ...
        referredCount = accounts[_address].teamAddress.length;
    }
    ...
    function _payReferralInETH(uint256 value, address _referee) private {
        ...
        for (uint256 i; i < levelRate.length; i++) {
            ...
            emit RewardPaid(_referee, referrer, c, i + 1, "ETH");

            userAccount = referrerAccount;
        }

        totalRewardDistributedETH += totalReferal;
    }
    ...
    function _payReferralInUSD(uint256 value, address _referee) private {
        ...
        for (uint256 i; i < levelRate.length; i++) {
            ...
            emit RewardPaid(_referee, referrer, c, i + 1, "USD");
            userAccount = referrerAccount;
        }

        totalRewardDistributedUSD += totalReferal;
    }
    ...
    function _getTokensValueUSD(
        ...
        tokenValue = (_USDValue * _price) / 10 ** USDContractDecimals;
    }

    function _BuyWithETH(address _address, uint256 _value) private {
    ...
    function _BuyWithUSD(
        ...
        uint256 tokenValue = _getTokensValueUSD(_value, pricePerUSD);
        IERC20Upgradeable(tokenContract).transfer(_address, tokenValue);

        totalTokenSold += tokenValue;
        totalUSDRaised += _value;
        emit TokenPurchased(_address, tokenValue, _value, "USD");
    }

    function _payRewardTokensETH(address _address, uint256 _value) private {
        uint256 rewardValue = _getTokensValueETH(_value, rewardPerUSD);
    ...
    function _payRewardTokensUSD(address _address, uint256 _value) private {
        uint256 rewardValue = _getTokensValueUSD(_value, rewardPerUSD);
        IERC20Upgradeable(rewardTokenContract).transferFrom(
            rewardOwner,
            _address,
            rewardValue
    ...
    function BuyWithETH(address _referrer) public payable whenNotPaused {
        if (!hasReferrer(msg.sender) && _referrer != address(0)) {
            _addReferrer(msg.sender, _referrer);
        }

        _BuyWithETH(msg.sender, msg.value);
        _payRewardTokensETH(msg.sender, msg.value);

        if (isPayReferral) {
            _payReferralInETH(msg.value, msg.sender);
        }
    ...
    function BuyWithUSD(
        ...
        _BuyWithUSD(msg.sender, _value);
        _payRewardTokensUSD(msg.sender, _value);

        if (isPayReferral) {
            _payReferralInUSD(_value, msg.sender);
        }

        uint256 balanceOfThisContract = IERC20Upgradeable(USDContract)
    ...
    function initialize() external initializer {
        ...
        // tokenContract = 0x4194f5990704DAAc8690834d24863aAEaeFaFDa9;
        // rewardTokenContract = 0x7F9fD63932babC508FAD2f324EB534D09cfE86F0;

        // tokenSeller = 0x4a3E55b9C8e2C7278D3D8F7E0e524Be3Cd76BA8f;
        // rewardOwner = 0xf91e1eD00f015c6Be3FBc6b0d84fBaEF005ec5B5;
        ...
        tokenContract = 0x0706DCf3DcC08BBcaCbE0B4ff8e9A9E10552A778;
        rewardTokenContract = 0x0F0EC170DEAF700CAf78aA12806A22E3c8f7621a;

        tokenSeller = 0x7a0DeC713157f4289E112f5B8785C4Ae8B298F7F;
        rewardOwner = 0x7a0DeC713157f4289E112f5B8785C4Ae8B298F7F;

        pricePerUSD = 85000000000000000000;
        rewardPerUSD = 1000000000000000000;

        minContributionUSD = 0;
        USDContractDecimals = 6;
        ...
        isPayReferral = true;
        defaultReferrer = payable(0x7a0DeC713157f4289E112f5B8785C4Ae8B298F7F);
        levelDecimals = 100;
        levelRate = [10, 5, 5];
