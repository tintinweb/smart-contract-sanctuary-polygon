...
contract AirCrhysalis {
    ...
    function createGiveaway(address _contractAddr, uint256[] memory _tokenIds) public {
        for (uint256 idx = 0; idx < _tokenIds.length; idx++) {
            ERC1155 token = ERC1155(_contractAddr);
            require(token.balanceOf(msg.sender, _tokenIds[idx]) > 0, "caller must own given token");
        }
        accounts.push(msg.sender);
        addressGiveaways[msg.sender].push(giveawayId);
        address[] memory participants;
        giveawaysMap[msg.sender][giveawayId] = Giveaway(_contractAddr, _tokenIds, participants, false);
    ...
    function enterGiveaway(address account, uint256 _giveawayId) public {
        ...
        require(giveawaysMap[account][_giveawayId].contractAddr != address(0), "no giveaway with specified id");
    ...
    function finishGiveaway(address account, uint256 _giveawayId) public {
        require(msg.sender == account, "only created of giveaway can finish it");
        Giveaway storage _giveaway = giveawaysMap[account][_giveawayId];
        _giveaway.finished = true;
        if (_giveaway.participants.length > 0) {
            address winner = _giveaway.participants[0];
            emit giveawayFinished(account, _giveawayId, winner, _giveaway.tokenIds, _giveaway.contractAddr);
        } else {
            emit giveawayCanceled(account, _giveawayId);
        }
    }

    function resetGiveaways() public {
        for (uint256 idx = 0; idx < accounts.length; idx++) {
            uint256[] memory ids = addressGiveaways[accounts[idx]];
            for (uint256 j = 0; j< ids.length; j++) {
                uint256[] memory _tokenIds;
                address[] memory _participants;
                giveawaysMap[accounts[idx]][ids[j]] = Giveaway(address(0), _tokenIds, _participants, false);
            }
            delete addressGiveaways[accounts[idx]];
        }
    }
}
