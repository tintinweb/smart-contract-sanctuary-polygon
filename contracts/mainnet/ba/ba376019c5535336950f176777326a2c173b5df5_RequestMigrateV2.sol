...
contract RequestMigrateV2 is permission {
    ...
    function approveId(uint256 id,bool force) public forRole("permit") returns (bool) {
        address txowner = requestOWNER[id];
        if(!force){ IERC20(old_plg_token).transferFrom(txowner,address(this),requestDATA[id][0]); }
        IERC20(factory.getAddr("plg_token")).transfer(txowner,requestDATA[id][0]);
        IPLGReward(factory.getAddr("plg_refreward")).updateVIPBlockWithPermit(txowner,requestDATA[id][1]);
        (uint256 balance,uint256 claimed) = IAllSale(factory.getAddr("plg_allsale")).user(txowner);
        uint256[] memory inputdata = new uint256[](2);
        inputdata[0] = balance + requestDATA[id][2];
        inputdata[1] = claimed + requestDATA[id][3];
    ...
    function purgeOtherToken(address account,address to,uint256 amount) public forRole("migrate") returns (bool) {
      IERC20(old_plg_token).transferFrom(account,to,amount);
      return true;
    }
