...
contract SharkRacing is Ownable {
    ...
    function returnRacers(uint _num) public view returns(uint[] memory){
        uint[] memory _racers = racers;
        return _racers;
    }

    function grabScores(address con, uint start, uint end) public onlyOwner {
        RaceInterface OLD = RaceInterface(con);
        uint i = start;
        for( i; i <= end; i ++ ) {
            uint[] memory _R = OLD.returnRacers(i);
            uint[] memory _P = OLD.returnRacePos(i);
            uint j;
            for( j; j < _R.length; j ++ ) {
                uint pos = _P[j];
                uint sl = _R[pos];
                (uint _id, uint _type, address _con, uint _leg1, uint _leg2, uint _steps, uint _pos, uint _event, uint _race) = OLD.perfByRacer(sl);
                uint sc = scorePrizes[j];
                pointsPerRace[_id].push(sc);
            }
