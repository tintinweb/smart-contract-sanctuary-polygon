...
contract BulkGrinderComethFLAG is Ownable, IERC721Receiver {
    ...
    address cardAddress;
    address moneyAddress = 0x9111D6446Ac5b88A84cf06425c6286658368542F;
    address[] resources = [
    ...
    uint256 public looterTax = 2500000000000000;
    uint256 public fleetTax = 1000000000000000;
    uint256 public freshTax = 5000000000000000;
    bool public IsActive = false;

    constructor(address _grinderAddress, address _cardAddress, address dev, address _royalty) Ownable() {
    ...
    function BulkGrindLooter(
        ...
        uint256 price = cardId.length.mul(looterTax);
        ...
        for (uint256 i = 0; i < resources.length; i++) {
            ...
            IERC20(resources[i]).transfer(msg.sender, amount);
        }
        money().safeTransferFrom(msg.sender, Royalty, price);
    }
    ...
    function BulkGrindFleet(
        ...
        uint256 price = cardId.length.mul(fleetTax);
        for (uint256 i = 0; i < cardId.length; i++) {
            comethCard().safeTransferFrom(msg.sender, address(this), cardId[i]);
            IGrind(grinderAddress).grind(cardId[i], ingredients);
        }
    ...
    function BulkGrindFresh(
        ...
        uint256 price = cardId.length.mul(freshTax);
        for (uint256 i = 0; i < cardId.length; i++) {
            comethCard().safeTransferFrom(msg.sender, address(this), cardId[i]);
            IGrind(grinderAddress).grind(cardId[i], ingredients);
        }
