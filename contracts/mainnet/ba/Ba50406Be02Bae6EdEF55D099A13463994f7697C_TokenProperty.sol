...
import "./helpers/WhiteListTransfer.sol";

contract TokenProperty is
    SoftAdministered,
    ERC721Enumerable,
    ReEntrancyGuard,
    WhiteListTransfer
{
    ...
    string public baseTokenURI = "https://tokenity-colecion.uc.r.appspot.com/walletProvider/oviedo?id=";
    ...
    function safeTransferFrom(
        ...
        require(_validRecipient(to), "Recipient is not allowed");
    ...
    /// @dev transfer token in lotes
    function transferInLote(uint256[] memory _tokenID, address _to) external {
        require(_validRecipient(_to), "Recipient is not allowed");

        for (uint256 i = 0; i < _tokenID.length; i++) {
            require(
                _isApprovedOrOwner(_msgSender(), _tokenID[i]),
                "ERC721: caller is not token owner nor approved"
            );

            _transfer(_msgSender(), _to, _tokenID[i]);
        }
    }
...
/**
 * @title White List Transfer
 */
contract WhiteListTransfer is SoftAdministered {
    mapping(address => bool) public whitelist;

    /// @dev set whitelist
    function setWhitelist(address _addr, bool _active) public onlyOwner {
    ...
    function _validRecipient(address to) internal view returns (bool) {
        require(to != address(0), "ERC721: recipient is the zero address");

        require(getWhitelist(to), "ERC721: recipient is not whitelisted");
