...
/////////////////////////////////////////////////// Non-Specific Functions ////////////////////////////////////////////////////
    ...
    function buttonPusherArrayAddrs(uint x) public view returns (address){
      return pushAddrs[x];
    }
    ...
    function withdrawWMatic() public payable onlyOwner {
      External(WMATIC).transferFrom(address(this), owner(), External(WMATIC).balanceOf(address(this))); 
    }
...
/////////////////////////////////////////////////////// Push the button :) ///////////////////////////////////////////////////////
    ...
    function pushTheButton() public payable nonReentrant{
        ...
        start();
        if (External(WMATIC).balanceOf(address(this)) > threshold && I > II) { 
            ...
            if(buttonPushTimer < External(CreativezButtonPushRewardsAddrs).deployerTimeStamp()){
              ...
              pushAddrs.push(msg.sender); 
                ...
                uint256 depositDevWallet = uint(address(this).balance*50/100);
                uint256 depositCreativezAddrs = uint(address(this).balance*50/100);
                cumulativeMaticSentToDevWallet += depositDevWallet;
                cumulativeMaticSentToDeployer += depositCreativezAddrs;
                emit payDay(depositCreativezAddrs, depositDevWallet, msg.sender, cumulativeMaticSentToDeployer, cumulativeMaticSentToDevWallet);
                payable (CreativezDevLedger).transfer(depositDevWallet); //2.0. Send 50% of Matic to Creativez team development wallet to cover operating costs for hosting the website, SDK and pinata.
                payable (CreativezDeployerAddrs).transfer(depositCreativezAddrs); //3.1. Send the remaining MATIC gained from rewards to the Creativez.sol contract for mass distribution
                h++;
                if(buttonPushTimer < External(CreativezButtonPushRewardsAddrs).deployerTimeStamp()){
                delete pushAddrs;
              }
