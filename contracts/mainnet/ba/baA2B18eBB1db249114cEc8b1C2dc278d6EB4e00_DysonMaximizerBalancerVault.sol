...
import "../../interfaces/balancer/Maximizer/IBoostPoolBalancer.sol";


/**
 * @dev Implementation of a vault to deposit funds for yield optimizing.
 * This is the contract that receives funds and that users interface with.
 * The yield optimizing strategy itself is implemented in a separate 'MaximizerBalancer.sol' contract.
 */
contract DysonMaximizerBalancerVault is Initializable, OwnableUpgradeable, ERC20Upgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // The strategy currently in use by the vault.
    IMaximizerBalancer public strategy;
    bool private _isStrategyInitialized;

    IBoostPoolBalancer public boostPool;
     ...
     */
    function __DysonMaximizerBalancerVault_init(
        string memory _name,
        string memory _symbol) public initializer {
        __Ownable_init_unchained();
        __ERC20_init_unchained(_name, _symbol);
        __ReentrancyGuard_init_unchained();
        __DysonMaximizerBalancerVault_init_unchained();
    }

    function __DysonMaximizerBalancerVault_init_unchained() internal initializer {}

    function want() public view returns (IERC20Upgradeable) {
        return strategy.primaryVault().want();
    }
    ...
    /**
     * calculates the total vault-receipt balance of the given address.
       This total balance includes the receipt tokens that the user has staked in the Boost Pool.
     * @param address_ address for which the this balance is to be calculated
     * @return userBalance
     */
    function balanceBelongTo(address address_) public view returns (uint256){
        uint256 userBalance = balanceOf(address_);
        if (address(boostPool) != address(0)) {
            uint256 pid1 = boostPool.poolId1(address(this));
            if (pid1 != 0) {
                uint256 boostAmount;
                (boostAmount,) = boostPool.userInfo(pid1 - 1, address_);
                userBalance += boostAmount;
            }
        }
        return userBalance;
    }
     ...
     */
    function earn(uint256 shares) public {
        uint _bal = available();
        want().safeTransfer(address(strategy), _bal);
        strategy.deposit(msg.sender, shares);
    }
    ...
    function withdraw(uint256 shares) public nonReentrant {
        require(shares <= balanceOf(msg.sender), "DysonMaximizerBalancerVault::amount exceeds balance!");
        uint256 amount = strategy.withdraw(msg.sender, shares);
    ...
    function inCaseTokensGetStuck(address token) external onlyOwner {
        require(token != address(want()), "!token");

        uint256 amount = IERC20Upgradeable(token).balanceOf(address(this));
        IERC20Upgradeable(token).safeTransfer(msg.sender, amount);
    }

    // @dev non-transferable vault receipts
    function _transfer(address from, address to, uint256 amount) internal override {
        if (msg.sender != address(boostPool)) revert('token is non-transferable');
        super._transfer(from, to, amount);
    }

    /**
     * updates the Maximizer contract connected to this vault
     * @param strategy_ Address of the new Maximizer contract
     */
    function setStrategy(IMaximizerBalancer strategy_) public onlyOwner {
        require(!_isStrategyInitialized, 'strategy already initialized');
        require(address(strategy_.maximizerVault()) == address(this), "new maximizer doesn't have this vault set as its vault!");
        strategy = strategy_;
    }

    /**
     * updates the BoostPool contract connected to this vault
     * @param address_ Address of the new BoostPool contract
     */
    function setBoostPool(address address_) public onlyOwner {
        boostPool = IBoostPoolBalancer(address_);
    }
}
...
interface IBoostPoolBalancer {
  function add ( uint256 allocPoint, address vault ) external;
  function allocPointUpdate ( uint256 pid, uint256 allocPoint, bool withUpdate ) external;
  function claimAllReward (  ) external;
  function claimPoolReward ( uint256 pid ) external;
  function endBlock (  ) external view returns ( uint256 );
  function endBlockUpdate ( uint256 _newEndBlock ) external;
  function founderAddress (  ) external view returns ( address );
  function getAllocPoint ( uint256 pid ) external view returns ( uint256 );
  function getLastRewardBlock ( uint256 pid ) external view returns ( uint256 );
  function getMultiplier ( uint256 _from, uint256 _to ) external view returns ( uint256 );
  function getPoolReward ( uint256 pid ) external view returns ( uint256 forUsers, uint256 forFounders );
  function getUserAmount ( uint256 pid, address userAddress ) external view returns ( uint256 );
  function initReward ( uint256 amount, uint256 _startBlock, uint256 _endBlock ) external;
  function massUpdatePools (  ) external;
  function notifyReward ( uint256 amount ) external;
  function owner (  ) external view returns ( address );
  function pendingReward ( uint256 pid, address userAddress ) external view returns ( uint256 );
  function percentForFounders (  ) external view returns ( uint256 );
  function poolId1 ( address ) external view returns ( uint256 );
  function poolInfo ( uint256 ) external view returns ( address vault, uint256 allocPoint, uint256 lastRewardBlock, uint256 accRewardPerShare );
