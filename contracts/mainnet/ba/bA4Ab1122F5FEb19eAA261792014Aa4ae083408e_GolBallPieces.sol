    ...
    OwnableUpgradeable
{
    ...
    constructor(MinimalForwarder _forwarder)
    ...
    function mint(
        ...
        // require(tokenId <= maxSupply && tokenId > 0, "tokenId out of range"); // Just in case - Fede
        _safeMint(to, tokenId);
        whitelist[to] = _whitelist;
    }
    ...
    function setTokenURI(string memory baseURI)
        external
    ...
    function setGeneralURI(string memory generalURI_)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        generalURI = generalURI_;
    ...
    function setSelectedNumber(uint256 selectedNumber_)
        external
        onlyRole(ORACLE_ROLE)
    {
        pieceSelected = true;
        selectedNumber = selectedNumber_;
        selectedTokenId = tokenByIndex(selectedNumber);
    ...
    function getSelectedAddress() public view returns (address) {
        require(pieceSelected, "there is no selected piece");
        require(selectedAddress != address(0), "there is no selected address");
...
contract MinimalForwarder is EIP712, Ownable {
    ...
    function checkExecutor(address executor)
        external
        view
        onlyOwner
        returns (bool)
