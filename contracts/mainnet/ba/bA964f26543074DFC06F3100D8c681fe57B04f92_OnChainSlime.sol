...
contract OnChainSlime is ERC721Enumerable, Ownable {
    using Strings for uint256;

    uint256 public constant MAX_TOKEN_ID = 999;

    uint256 public nextTokenId = 1;
    Layer[6] private layers;

    mapping(uint256 => uint256) public dnas; // tokenId => DNA
    mapping(uint256 => bool) public dnaExists; // DNA/100(except background) => exists

    constructor() ERC721("OnChainSlime", "SLIME") {
    ...
    function getLayer(uint256 layerIndex) external view returns (Layer memory) {
        return layers[layerIndex];
    }

    function getPartsCounts() external view returns (uint8[] memory result) {
        result = new uint8[](layers.length);
        for (uint256 i = 0; i < layers.length; i++) {
            result[i] = uint8(layers[i].parts.length);
        }
    }

    event MintSlime(uint256 indexed tokenId, uint256 indexed dna);

    function getParamsByDna(uint256 dna) private pure returns (uint8[6] memory params) {
        // DNA = background + body*100 + mouth*100**2 + cheek*100**3 + eyes*100**4 + head*100**5
