...
contract DGN  is ERC721Enumerable, Ownable, DefaultOperatorFilterer {
    using Strings for uint256;
    ...
    address[] public whitelistedOGAddresses;
    address[] public whitelistedAddresses;

    bool public RevealedActive = false;
    bool public PresaleMode = true;
    bool public PublicMode = false;
    bool public blockTransfers = true;

    uint256 public OGPrice = 147 ether;
    uint256 public WLPrice = 163 ether;
    uint256 public PublicPrice = 200 ether;
    uint256 public MaxSupply = 1000;
    uint256 public MaxPresaleSupply = 100;
    ...
    function PresaleMint(uint256 _Amount) public payable {
        require(PresaleMode == true, "PreSale not started");
        require(isOGWhitelisted(msg.sender) || isWhitelisted(msg.sender), "User is not whitelisted");
        ...
        require(supply + _Amount <= MaxPresaleSupply, "Sold Out");
    
        if (isOGWhitelisted(msg.sender)) {
            require(msg.value >= OGPrice * _Amount, "OG: Balance Insufficient");
        }
        if (isWhitelisted(msg.sender)) {
            require(msg.value >= WLPrice * _Amount, "WL: Balance Insufficient");
        }
    ...
    delete whitelistedOGAddresses;
    whitelistedOGAddresses = _users;
    }
 
    // Verify Whitelist Users
    function isOGWhitelisted(address _user) public view returns (bool) {
    for (uint i = 0; i < whitelistedOGAddresses.length; i++) {
      if (whitelistedOGAddresses[i] == _user) {
          return true;
      }
    }
    return false;
    ...
    function TurnPresaleMode(bool _state) public onlyOwner {
        PresaleMode = _state;
    }
    ...
    // Opensea royalties and limit transfers/sales

    function blockTransfer() public onlyOwner {
        blockTransfers = true;
    }

    function unblockTransfer() public onlyOwner {
    ...
    function transferFrom(
        ...
        require(!blockTransfers, "Selling is currently blocked");
