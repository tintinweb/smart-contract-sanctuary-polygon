...
contract LimitOrders is EIP712("FBS Token", "1"){
    ...
    bytes32 LIMIT_ORDER_TYPEHASH = keccak256(
        "Order(address owner,uint256 status,address pair,address tokenIn,address tokenOut,uint256 amountIn,uint256 priceExecute, uint256 minAmountOut,uint256 deadline,uint256 salt)"
        );
    ...
    function recoverSigner(Order memory ord, bytes memory _signature)
        ...
        returns (address)
    {
        bytes32 _ordHash = getMessageHash(ord);
        bytes32 _hash = getEthSignedMessageHash(_ordHash);
        (bytes32 r, bytes32 s, uint8 v) = splitSignature(_signature);
        return ecrecover(_hash, v, r, s);
    }

    function getMessageHash(Order memory ord)
        public
        view
        returns (bytes32)
    {
        ...
        )
    {
        ...
        function shouldFulfilOrder(Order memory ord, bytes memory _signature) public view returns (bool) {
            bool _isEnoughToken = false;
            if(IERC20(address(ord.tokenIn)).balanceOf(address(ord.owner)) >= ord.amountIn) _isEnoughToken = true;

            bytes32 _hashOrder = getMessageHash(ord);
            
            bool _isNotCancelledOrder = true;
            for(uint i=0; i<=msgHashOrderCancel[ord.owner].length; i++){
                if(_hashOrder == msgHashOrderCancel[ord.owner][i]) {
            ...
            if(_signer != ord.owner) _isSigner = false;

            bool _isFulfiledOrder = false;
            if(getAmountOut(ord.amountIn,ord.pair, ord.tokenIn, ord.tokenOut) >= ord.priceExecute) _isFulfiledOrder = true;

            return _isEnoughToken && _isNotCancelledOrder && _isPendingOrder && _isSigner && _isFulfiledOrder;
        }


        function ExecuteCustomOrder(Order memory ord, bytes memory _signature) public reentrancyGuard returns (bool result, string memory message){
            require(block.timestamp <= ord.deadline, "SPACE: Expired deadline");
            
            address signer = recoverSigner(ord, _signature);
            require(signer == ord.owner, "SPACE: invalid signature");

            //msgHash
            bytes32 _hashOrd = getMessageHash(ord);

            bool _isCancelledOrder = false;
            for(uint i=0; i<=msgHashOrderCancel[ord.owner].length; i++){
                if(_hashOrd == msgHashOrderCancel[ord.owner][i]) {
                    _isCancelledOrder = true;
                }
            }
            require(_isCancelledOrder == false, "SPACE: Can't executed cancelled order!!!");

            require(IERC20(ord.tokenIn).balanceOf(ord.owner) >= ord.amountIn, "SPACE: Not enough token!!!!");

            require(ord.status == 0, "Can't executed non-pending order!!!");
            result = false;

            if(execute(ord)) {
                return (result = true, message = "Execute Order Success!!!");
            } else {
                return (result = false, message = "Failed to execute order!!!");
                }
        }
    ...
    function execute(Order memory ord) internal returns (bool filled) {
        require(ord.salt >= 1000, "failed with err code 100");
