...
contract AxoToken is ERC20Permit, Ownable {
    ...
    */
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s ) external {
        ...
        require(signatory != address(0), "AXO::delegateBySig: invalid signature");
        require(nonce == _useNonce(signatory), "AXO::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "AXO::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "AXO::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying AXOs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "AXO::_writeCheckpoint: block number exceeds 32 bits");
...
contract MasterChef is Ownable {
    ...
    uint256 public axoPerSecond;

    // set a max axo per second, which can never be higher than 1 per second
    uint256 public constant maxAxoPerSecond = 1e18;
    ...
    constructor(
        AxoToken _axo,
        address _devaddr,
        uint256 _axoPerSecond,
        uint256 _startTime
    ) {
        axo = _axo;
        devaddr = _devaddr;
        axoPerSecond = _axoPerSecond;
    ...
    function setAxoPerSecond(uint256 _axoPerSecond) external onlyOwner {
        require(_axoPerSecond <= maxAxoPerSecond, "setAxoPerSecond: too many axos!");
    ...
    function pendingAXO(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 axoReward = multiplier.mul(axoPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
