...
contract LandTest is Ownable, ERC721A {
    using Address for address;
    using Strings for uint256;

    enum Status {Stop, Land1x1Mint, Land2x2Mint, Land3x3Mint}
    ...
    mapping(address => WLLandAmount) private addressLandAmount;
    mapping(address => WLLandMintedAmount) private addressLandMintedAmount;
    ...
    function land1x1Mint(uint256 _quantity) external payable callerIsUser onlyStatus(Status.Land1x1Mint) {
        require(_totalMinted() < land1x1Supply, "Reached land1x1 max supply");
        require(addressLandAmount[msg.sender].wlLand1x1Amount - addressLandMintedAmount[msg.sender].Land1x1MintedAmount >= _quantity, "Reached the number of mintable");
        _safeMint(msg.sender, _quantity);
        addressLandMintedAmount[msg.sender].Land1x1MintedAmount += _quantity;
    }

    function land2x2Mint(uint256 _quantity) external payable callerIsUser onlyStatus(Status.Land2x2Mint) {
        require(_totalMinted() >= land1x1Supply, "Land1x1 has not finished mint");
        require(_totalMinted() < land1x1Supply + land2x2Supply, "Reached land2x2 max supply");
        require(addressLandAmount[msg.sender].wlLand2x2Amount - addressLandMintedAmount[msg.sender].Land2x2MintedAmount >= _quantity, "Reached the number of mintable");
        _safeMint(msg.sender, _quantity);
        addressLandMintedAmount[msg.sender].Land2x2MintedAmount += _quantity;
    }

    function land3x3Mint(uint256 _quantity) external payable callerIsUser onlyStatus(Status.Land3x3Mint) {
        require(_totalMinted() >= land1x1Supply+land2x2Supply, "Land2x2 has not finished mint");
        require(_totalMinted() < land1x1Supply + land2x2Supply + land3x3Supply, "Reached land3x3 max supply");
        require(addressLandAmount[msg.sender].wlLand3x3Amount - addressLandMintedAmount[msg.sender].Land3x3MintedAmount >= _quantity, "Reached the number of mintable");
        _safeMint(msg.sender, _quantity);
    ...
    function setMaxSupply(uint256 _maxSupply) external onlyOwner {
        require(_maxSupply > land1x1Supply + land2x2Supply + land3x3Supply, "Quantity less than land1,2,3");
        maxSupply = _maxSupply;
    }
    ...
    function setLand3x3Supply(uint _land3x3Supply) external onlyOwner {
        land3x3Supply = _land3x3Supply;
    }

    function teamMint(uint256 _amount, address _receiver) public onlyOwner {
        require(totalSupply() + _amount <= maxSupply, "All token have already been minted!");
