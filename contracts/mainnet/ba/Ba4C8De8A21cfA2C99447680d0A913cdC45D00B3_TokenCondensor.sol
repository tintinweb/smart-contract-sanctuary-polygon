...
contract TokenCondensor is BaseSiloAction {
    constructor(string memory _name, address _siloFactory) {
        name = _name;
        metaData = "address[5],address[5],address,address[][5]";
    ...
    address private constant QUICK_ROUTER =
        0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;

    address private constant DMM_ROUTER =
    ...
    function tokenCondense(
        bytes memory configuration,
        bytes memory inputData
    ) private returns (uint256[5] memory outputAmounts) {
        bool original;
        uint256[5] memory inputAmounts;
        (outputAmounts, inputAmounts, original) = directTokenCondense(
            configuration,
            inputData
        );
    }
    ...
    function directTokenCondense(
        ...
        internal
        returns (
            uint256[5] memory outputAmounts,
            uint256[5] memory afterInputs,
            bool original
        )
    {
        ...
        address[][5] memory paths;
        (inputs, outputs, exchangeRateOracle, paths) = abi.decode(
            configuration,
            (address[5], address[5], address, address[][5])
        );
        ...
        for (uint256 i; i < 5; ) {
            ...
            for (uint256 j; j < 5; ) {
                ...
                if (
                    paths[i].length > 0 &&
                    paths[i][0] == inputs[i] &&
                    paths[i][paths[i].length - 1] == outputs[j]
                ) {
    ...
    function swapTokens(
        ...
        (address oracleRouter, address pool) = oracle.findRouter(path);
        ...
        if (pool == address(0)) {
            ...
            for (uint256 index; index < pathLength - 1; ) {
                ...
                (oracleRouter, pool) = oracle.findRouter(subPath);
                ...
                if (pool == address(0)) {
                    outAmounts = IUniswapV2Router02(oracleRouter)
                    ...
                    outAmounts = IDMMExchangeRouter(oracleRouter)
                        .swapExactTokensForTokens(
                            nextAmountIn,
                            0,
                            pools,
    ...
    function swapTokenWithBridge(
        ...
        path2[1] = WETH;
        path2[2] = tokenOut;
        uint256 amountOut = oracle.getUniPrice(router, path2, amountIn);
        if (amountOut > 0) {
            (amounts, result) = swapTokens(oracle, path2, amountIn, amountOut);
        } else {
            path2[0] = tokenIn;
            path2[1] = USDC;
            path2[2] = tokenOut;

            (amounts, result) = swapTokens(oracle, path2, amountIn, 0);
    ...
    function createConfig(
        ...
        configData = abi.encode(_inputs, _outputs, _exchangeOracle, paths);
    }

    function validateConfig(bytes memory) public pure override returns (bool) {
        // address[5] memory inputs;
        // address exchangeRateOracle;
        // address[5] memory outputs;
        // address[][5] memory paths;
        ...
        //             uint256 amountOut = oracle.getUniPrice(
        //                 QUICK_ROUTER,
        //                 paths[i],
        //                 1 ether
        //             );
