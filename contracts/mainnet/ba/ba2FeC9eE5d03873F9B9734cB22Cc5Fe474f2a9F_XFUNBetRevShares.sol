...
contract XFUNBetRevShares is IXFUNBetRevShares, Ownable {
	...
	// NEW! Layer Burn Percent
	uint64 public layerBurnPercent; // Decimals 8

	function updateLayerBurnPercent(uint64 _percent) external onlyOwner { 
		require(_percent >= 0 && _percent < 10000000000);
		layerBurnPercent = _percent;

		emit UpdateLayerBurnPercent(_percent);
	}
	...
	function updateLayerAffiliatePercent(uint64 _percent) external onlyOwner { 
		require(_percent >= 0 && _percent < 10000000000);
