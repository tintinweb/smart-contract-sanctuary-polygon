...
contract MXE_Presale_USDT is ReentrancyGuard, Context, Ownable {
    ...
    constructor (uint256 rate, address wallet, IERC20 token, IERC20 usdttoken)  {
        require(rate > 0, "Pre-Sale: rate is 0");
        require(wallet != address(0), "Pre-Sale: wallet is the zero address");
        require(address(token) != address(0), "Pre-Sale: token is the zero address");
        require(address(usdttoken) != address(0), "Pre-Sale: token is the zero address");
    ...
    function buyTokens(uint256 amount) public nonReentrant icoActive{
        ...
        require(_usdttoken.allowance(msg.sender,address(this))>=amount,"Allowance not given for Buying Token");
        require(_usdttoken.transferFrom(msg.sender,address(this),amount),"Couldnt Transfer Amount");
