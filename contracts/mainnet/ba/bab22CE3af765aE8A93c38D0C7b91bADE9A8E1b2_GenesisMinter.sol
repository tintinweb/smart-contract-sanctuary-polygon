...
contract GenesisMinter is Managable {
    ...
    function mintMany(uint256[] calldata _ids) external onlyManager {
        for (uint256 i = 0; i < _ids.length; i++) {
            IBot(botAddress).mintTokenId(tokenHolder, _ids[i]);
        }
    }

    function mint(uint256 _id) external onlyManager {
        IBot(botAddress).mintTokenId(tokenHolder, _id);
    }

    function mintManyWithMeta(uint256[] calldata _ids, uint256[] calldata _dna) external onlyManager {
        require(_ids.length == _dna.length, "GenesisMinter: ids and dna arrays must be equal length");

        for(uint256 i = 0; i < _ids.length; i++) {
            IBot(botAddress).mintTokenId(tokenHolder, _ids[i]);
            LibBot.Bot memory _bot = zeroBot(_ids[i], _dna[i]);
            IBotMetadata(botMetadataAddress).setBot(_ids[i], _bot);
        }
    }

    function mintWithMeta(uint256 _id, uint256 _dna) external onlyManager {
        IBot(botAddress).mintTokenId(tokenHolder, _id);
        LibBot.Bot memory _bot = zeroBot(_id, _dna);
        IBotMetadata(botMetadataAddress).setBot(_id, _bot);        
    }

    function zeroBot(uint256 _id, uint256 _dna) internal view returns (LibBot.Bot memory){
        return LibBot.Bot({
            id: _id,
            genes: _dna,
