...
contract MagpieCore{
    ...
    function swapInToken(uint256 amountOutMin, uint256 targetAmountOutMin, uint256 amountIn, address[] calldata path, uint16 recipientChainId, bytes32 recipient,   uint32 nonceValue) external payable returns(uint64 tokenSequence, uint64 coreSequence){
        uint[] memory amounts;
        {
        address _origToken = path[0];
        IERC20 originalToken = IERC20(_origToken);
        require(originalToken.transferFrom(msg.sender, address(this), amountIn), 'transferFrom failed.');
        require(originalToken.approve(address(uniswap), amountIn), 'approve failed.');
        amounts = uniswap.swapExactTokensForTokens(amountIn,amountOutMin,path,address(this),nonceValue+50000);
        }
        ...
        tokenSequence = bridge.transferTokens{value: msg.value}(swapTokenAddress, amounts[amounts.length-1], recipientChainId, recipient, arbiterFeeValue,nonceValue);
        }
        
        Payload memory payload = Payload({targetPath: path, 
                                          targetAmountIn: amounts[amounts.length-1],
                                          targetAmountOutMin: targetAmountOutMin,
