    ...
    DropSinglePhase
{
    ...
    uint256 tokenCount = balanceOf(owner); // เรียกใช้ฟังก์ชัน balanceOf เพื่อหาจำนวน token ที่เจ้าของครอบครอง
    if (tokenCount == 0) {
        // ถ้าไม่มี token ที่เจ้าของครอบครอง ให้ return array ว่างๆ
        return new uint256[](0);
    } else {
        uint256[] memory result = new uint256[](tokenCount); // สร้าง array ขนาดเท่ากับจำนวน token ที่เจ้าของครอบครอง
        uint256 index = 0;
        for (uint256 tokenId = 0; tokenId < totalSupply(); tokenId++) {
            if (ownerOf(tokenId) == owner) {
                result[index] = tokenId; // ถ้า token นี้เป็นของเจ้าของ ให้เพิ่ม tokenId ลงใน array result
                index++;
            }
        }
        return result;
