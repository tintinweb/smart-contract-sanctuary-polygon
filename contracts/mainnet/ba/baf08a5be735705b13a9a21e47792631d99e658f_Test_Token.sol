...
contract Test_Token is ERC20, Ownable {
    ...
    TEST_TOKENDividendTracker public dividendTracker;
    ...
    uint256 private numberOfBuyTax;
    ...
    constructor(address charity_, address treasury_, address marketing_, address creator_) ERC20("TEST_TOKEN", "TEST") {
    	...
    	dividendTracker = new TEST_TOKENDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        TEST_TOKENDividendTracker newDividendTracker = TEST_TOKENDividendTracker(payable(newAddress));
    ...
    function addOnBlacklist(address account) public onlyOwner {
        require(!isBlacklisted[account], "TEST: Already added..");
        require(isContract(account), "TEST: Only bot can blacklist..");
        isBlacklisted[account] = true;
    }
    
    // function to allow admin to remove an address from blacklist..
    function removeFromBlacklist(address account) public onlyOwner {
        require(isBlacklisted[account], "TEST: Already removed..");
    ...
    function updatelpTokenRecipient(address newlpTokenRecipient) public onlyOwner {
        require(newlpTokenRecipient != lpTokenRecipient, "TEST: The liquidity wallet is already this address");
    ...
    function updateMaxSellBuyAmount(uint256 maxBuy, uint256 maxSell) public onlyOwner {
        require(maxBuy >= totalSupply().mul(1e5).div(1e2) && maxSell >= totalSupply().mul(1e5).div(1e2), "TEST: You cannot set less than 0.001% of totalSupply..");
        maxBuyAmount = maxBuy;
    ...
    function _transfer(
        ...
        require(!isBlacklisted[from] && !isBlacklisted[to], "TEST: to or from addres blacklisted...");
        ...
        if(takeFee) {
            ...
            if (automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to]) {
                ...
                if (numberOfBuyTax == 10) {
                    super._transfer(address(this), to, _luckyBuyerFeeTotal);
    ...
    function collectFeeOnBuy(address account, uint256 amount) private returns (uint256) {
        ...
        if(buyLuckyBuyerFee != 0) {
            ...
            numberOfBuyTax = numberOfBuyTax.add(1);
            super._transfer(account, address(this), Fee);
        }
    ...
    function enabledTrading() public onlyOwner {
        require(!tradingIsEnabled, "TEST: Trading already enabled..");
...
contract TEST_TOKENDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() DividendPayingToken("TEST_TOKEN_Dividend_Tracker", "VMDT") {
    	claimWait = 3600;
