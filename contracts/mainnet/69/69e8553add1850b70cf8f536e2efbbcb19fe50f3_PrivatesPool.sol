...
// File: contracts/PrivatesPool.sol
...
interface IReward {
    ...
    function totalPRIVATESInContract() view external returns(uint256);
}

contract PrivatesPool is Ownable {
    ...
    uint256 public initialPRIVATESPerBlock;
    ...
    constructor(
        ...
        poolInfo.push(PoolInfo({
            stakedToken: _PRIVATES,
            allocPoint: 1000,
            lastRewardBlock: startBlock,
            accPRIVATESPerShare: 0,
            totalTokensInPool: 0
        }));
        totalAllocPoint = 1000;
        initialPRIVATESPerBlock = PRIVATESPerBlock;
    }
    ...
    function pendingPRIVATES(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accPRIVATESPerShare = pool.accPRIVATESPerShare;
        ...
        if (block.number > pool.lastRewardBlock && stakedSupply != 0) {
            ...
            accPRIVATESPerShare = accPRIVATESPerShare.add(PRIVATESReward.mul(1e12).div(stakedSupply));
        }
        return user.amount.mul(accPRIVATESPerShare).div(1e12).sub(user.rewardDebt);
    }   
    ...
    function rewardsPerDay(uint256 _pid, address _address) public view returns (uint){      
        ...
        return  userInfo[_pid][_address].amount.mul(28800).div(poolInfo[_pid].totalTokensInPool).mul(PRIVATESPerBlock); 
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if(PRIVATES.totalSupply() < 1000000 ether - PRIVATESReward.mul(11).div(10)){
            PRIVATES.mint(devaddr, PRIVATESReward.div(10));
            PRIVATES.mint(address(rewardContract), PRIVATESReward);
        }

        pool.accPRIVATESPerShare = pool.accPRIVATESPerShare.add(PRIVATESReward.mul(1e12).div(stakedSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if(rewardsActive){
            if (user.amount > 0) {
                uint256 pending = user.amount.mul(pool.accPRIVATESPerShare).div(1e12).sub(user.rewardDebt);
                if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accPRIVATESPerShare).div(1e12);     

        emit Deposit(msg.sender, _pid, _amount);
    }

    function withdraw(uint256 _pid, uint256 _amount) public {
        require(rewardsActive == true, "The rewards are not ready yet");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];        
    ...
    function dev(address _devaddr) public {
        require(msg.sender == devSetter, "devSetter: error");
        devaddr = _devaddr;
    }

    function free(uint256 _pid) public{
        require(rewardsActive == true, "The rewards are not ready yet");
        updatePool(_pid);
        ...
        uint256 pending = user.amount.mul(accPRIVATESPerShare).div(1e12).sub(user.rewardDebt);
       
        require(pending > 0, "No pending to free");
        rewardContract.payTo(msg.sender, pending);
        user.rewardDebt = user.amount.mul(accPRIVATESPerShare).div(1e12);
    }
