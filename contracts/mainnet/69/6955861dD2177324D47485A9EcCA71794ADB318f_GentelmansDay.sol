...
// File: erc721l/contracts/Linkable.sol
...
// File: erc721l/contracts/IERC721Linkable.sol
...
// File: erc721l/contracts/ERC721Linkable.sol
...
contract GentelmansDay is ERC721Linkable {
    ...
    address EddieRivers = 0xC89391dE1bDDAC8cA554c6298b0b22F7C5EBc871;
    ...
    function mint(uint256 tokenId) public onlyHolder notPaused {
        ...
        _safeMint(msg.sender, participants.length);

        approve(address(this), participants.length);
        this.linkToken(participants.length, tokenId);

        emit NewParticipant(msg.sender, tokenId);
    }

    function freeMint(uint256 tokenId) public onlyOwner {
        require(registered[tokenId] == false, "Token already registered");

        registered[tokenId] = true;
        participants.push(tokenId);

        address holder = MRC.ownerOf(tokenId);

        _safeMint(holder, participants.length);

        approve(address(this), participants.length);
        this.linkToken(participants.length, tokenId);

        emit NewParticipant(holder, tokenId);
    }
