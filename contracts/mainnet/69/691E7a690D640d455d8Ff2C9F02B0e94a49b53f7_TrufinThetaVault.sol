...
.
    ...
    {
        require(numShares > 0, "numShares should be greater than zero");
        require(WHITELIST.isLawyer(msg.sender), "Lawyer is not whitelisted");
    ...
    function _completeWithdraw() internal returns (uint256) {
        ...
        if (IMasterWhitelist(WHITELIST).isVaultWhitelisted(msg.sender)) {
            ...
            //feesNotSentToRecipient was fixed in the previous round
            uint256 totalFeeRebate = vaultFee.feesNotSentToRecipient[round - 1];

            // Total deposit amount by whitelisted vaults in the previous epoch
            // but deposit for previous epoch was made before this epoch, so in the "previous-previous epoch"
            ...
            uint256 totalWhitelistedVaultsShares = ShareMath.assetToShares(
                totalVaultDeposit,
                //and share price was fixed in the "previuos-previous epoch" too
                roundPricePerShare[round - 2],
                vaultParams.decimals
            );
        ...
        return (withdrawAmount - vaultFeeToCredit);
    }
        ...
        )
    {
        ...
        uint256 performanceFeeInAsset;
        uint256 totalVaultFee;
        uint256 feesNotSentToRecipient;
        uint256 newPricePerShare;
        (
            lockedBalance,
            queuedWithdrawAmount,
        ...
        feesNotSentToRecipient = calcFeesNotSentToRecipient(totalVaultFee);
