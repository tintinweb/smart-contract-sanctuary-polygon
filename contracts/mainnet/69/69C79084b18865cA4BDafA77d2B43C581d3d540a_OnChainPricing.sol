...
contract OnChainPricing {
    ...
    function findOptimalSwap(address tokenIn, address tokenOut, uint256 amountIn) external view returns (Quote memory) {
        ...
        uint256 uniQuote = getUniPrice(QUICK_ROUTER, tokenIn, tokenOut, amountIn);
        quotes[1] = Quote(QUICK_ROUTER, uniQuote);

        uint256 sushiQuote = getUniPrice(SUSHI_ROUTER, tokenIn, tokenOut, amountIn);
        quotes[2] = Quote(SUSHI_ROUTER, sushiQuote);

        uint256 gravityQuote = getGravityPrice(GRAVITY_ROUTER, tokenIn, tokenOut, amountIn);
    ...
    function getGravityPrice(address router, address tokenIn, address tokenOut, uint256 amountIn) public view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = address(tokenIn);
        path[1] = address(tokenOut);
