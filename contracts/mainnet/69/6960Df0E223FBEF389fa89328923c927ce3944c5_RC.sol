...
contract RC is Ownable {
    ...
    modifier validSubscriptionID(uint256 _sid) {
        require(_sid < subscriptions.length 
        && subscriptions[_sid].active, "subscription is not valid");
        _;
    }
    ...
    event ValidatorsChanged(address user, uint256 newVoteQuorum, address[] newValidators);
    ...
    event UpgradeInsurancePlan(address user, uint256 newSubscriptionID,uint256 expirationTime,uint256 priceInPaymentToken);
    event BillPayment(
        address payer,
    ...
    event InsuranceEvent(address insuranceOwner, address backupWallet, uint256 recoveryTokensCount);
    ...
    constructor(address _feeAddress, address _paymentAdmin,address _quoteTokenAddress) {
        
        Subscription memory freePlan = Subscription(
            true,
            30 days,
            0,
            address(0),
            address(0),
            0,
            10000, // feeAmountBP +100%
            2000 // erc20AmountBP 20%
        );
        subscriptions.push(freePlan);
    ...
    function addSubscription(
        ...
        require(
            _subscription.erc20AmountBP <= MAX_ERC20_PENALTY_BP,
            "erc20AmountBP is too large"
        );
        require(
            _subscription.feeAmountBP <= MAX_FEE_PENALTY_BP,
            "feeAmountBP is too large"
        );
    ...
    function createInsurance(
        ...
        uint256 prevPrice=subscriptions[userInsurance.subscriptionID].priceInQuoteToken;
        // create new or up free(only once)
        require(
            userInsurance.backupWallet == address(0) ||
                (prevPrice == 0 && paymentPlan.priceInQuoteToken > 0),// after trial
            "already created"
        );
        require(
        ...
        if (userInsurance.backupWallet != address(0)) {
            // after trial period
            delete proposals[msg.sender];
        } 
    ...
    function upgradeInsurancePlan(uint256 _subscriptionID) external validSubscriptionID(_subscriptionID) {
        ...
        require(
            paymentPlan.priceInQuoteToken > 0,
            "cannot be upgraded to free plan"
        );

        userInsurance.subscriptionID = _subscriptionID;
        userInsurance.expirationTime = block.timestamp + paymentPlan.validityPeriod;
        ...
        emit UpgradeInsurancePlan(msg.sender, _subscriptionID, userInsurance.expirationTime, priceInPaymentToken);
    }
    ...
    function _billPayment(
        ...
        Subscription memory userPaymentPlan = subscriptions[userInsurance.subscriptionID];
        require(
            userPaymentPlan.priceInQuoteToken > 0,
            "not allowed for a trial subscription"
        );
    ...
    function insuranceEvent(
        ...
        if (userPaymentPlan.recoveryTokenLimit > 0) {
            ...
            require(
                userInsurance.rcCount <=
                    userPaymentPlan.recoveryTokenLimit,
        ...
        if (block.timestamp > userInsurance.expirationTime) {
            ...
            _billPayment(msg.sender, insuranceOwner, userInsurance, penalty);
        }
        ...
        for (uint256 i = 0; i < erc20Tokens.length; i++) {
            ...
            if (balance > 0) {
                erc20PenaltyAmount = penalty
                    ? (balance * userPaymentPlan.erc20AmountBP) /
                        BASE_POINT
                    : 0;
