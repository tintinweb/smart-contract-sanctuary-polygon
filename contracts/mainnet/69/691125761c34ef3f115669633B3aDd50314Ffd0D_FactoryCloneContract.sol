...
contract FactoryCloneContract is Ownable {
    ...
    function updateEmitterAddress(
        ...
        emitterAddress = _emitterContractAddress;
    }
    ...
    ReentrancyGuard
{
    ...
    //@dev struct for NFT transfer details
    struct NftDetails {
        address nftAddress;
        address receiver;
        uint256 tokenId;
    ...
    //@dev struct for external contract calls details
    struct ContractCallDetails {
        address to;
        bytes data;
    }
    ...
    address public gatingTokenAddress;
    uint256 public gatingTokenBalanceRequired;
    ...
    // bool public isTokenGatingApplied;
    ...
    /// @dev onlyAdmin modifier to allow only Admin access to functions
    modifier onlyAdmin() {
    ...
    function updateMinMaxDeposit(
        ...
        if (isTemplateErc721) revert NotERC20Template();
    ...
    function updateOwnerFee(
        ...
        if (!(_ownerFeePerDeposit < 100))
            revert AmountInvalid("ownerFeePerDeposit", ownerFeePerDeposit);
    ...
    /// @dev Function to enable token gating
    function enableTokenGating(
        address _address,
        uint256 _amount
    ) external onlyAdmin {
        if (_address == address(0)) revert AddressInvalid("_address", _address);
        if (_amount == 0) revert AmountInvalid("_amount", _amount);
        gatingTokenAddress = _address;
        gatingTokenBalanceRequired = _amount;
    }

    /// @dev Function to disable token gating
    function disableTokenGating() external onlyAdmin {
        gatingTokenAddress = address(0);
        gatingTokenBalanceRequired = 0;
    }

    /// @dev Function to close deposit
    function closeDeposit() external payable onlyAdmin {
        if (depositCloseTime < block.timestamp) revert DepositClosed();
    ...
    function checkDeposit() external view returns (bool) {
        return block.timestamp < depositCloseTime ? true : false;
    }

    /// @dev Function to start deposit
    /// @param _days New close date
    function startDeposit(uint256 _days) external payable onlyAdmin {
        if (_days == 0) revert AmountInvalid("_days", _days);
        if (depositCloseTime > block.timestamp) revert DepositStarted();
    ...
    function deposit(
        ...
        if (depositCloseTime < block.timestamp) revert DepositClosed();
        if (_usdcTokenAddress != usdcTokenAddress)
            revert AddressInvalid("usdcAddress", _usdcTokenAddress);

        if (!isTemplateErc721) {
            if (!(_totalAmount >= minDepositPerUser))
                revert AmountInvalid("_totalAmount", _totalAmount);

            if (!(_totalAmount <= maxDepositPerUser))
                revert AmountInvalid("_totalAmount", _totalAmount);
        }

        if (gatingTokenAddress != address(0)) {
            require(
                IERC20(gatingTokenAddress).balanceOf(msg.sender) >
        ...
        if (isTemplateErc721) {
            if (priceOfNft != _totalAmount)
                revert AmountInvalid("priceOfNft", priceOfNft);
        }

        daoBalance += _totalAmount;

        if (!isTemplateErc721) {
            if (daoBalance > totalRaiseAmount)
    ...
    function updateProposalAndExecution(
        ...
        if (params.executionIds[6] == true) {
            transferNft(
                params.nftDetails.nftAddress,
                params.nftDetails.receiver,
                params.nftDetails.tokenId
            );
        }
        if (params.executionIds[7] == true) {
            contractCalls(
                params.contractCallDetails.to,
                params.contractCallDetails.data
            );
        }
    ...
    function transferNft(address _nft, address _to, uint256 _tokenId) internal {
    ...
    function contractCalls(address _to, bytes memory _data) internal {
        if (_to == address(0)) revert AddressInvalid("_to", _to);
    ...
    function airDropToken(
        ...
        uint256 leng = members.length;
        for (uint256 i = 0; i < leng; ) {
            uint256 _dropAmount;
            if (isTemplateErc721) {
                _dropAmount = (balanceOf(members[i]).mul(_updatedAmount)).div(
                    totalNFTsMinted
                );
            } else {
                _dropAmount = (balanceOf(members[i]).mul(_updatedAmount)).div(
    ...
    function stableCoinDecimals() external view returns (uint8) {
...
contract Nft is ERC721Enumerable, Ownable {
    ...
    bool public isNftTransferable;

    bool public isNftTotalSupplyUnlimited;
    ...
    function setOnlyAllowWhitelist() external payable onlyAdmins {
        onlyAllowWhitelist = true;
    }

    function unsetOnlyAllowWhitelist() external payable onlyAdmins {
    ...
    function startMinting() external payable onlyAdmins {
    ...
    function stopMinting() external payable onlyAdmins {
        isMintingOpen = false;
    }

    function whitelistAddress(address _address) external onlyAdmins {
        if (!onlyAllowWhitelist) revert NotWhitelisted();
        allowMint[_address] = true;
        Emitter(emitterAddress).whitelistAddress(
            implementationAddress,
            address(this),
            _address
        );
        ...
        onlyAdmins
    {
        ...
        Emitter(emitterAddress).removeWhitelistAddress(
            implementationAddress,
            address(this),
            _address
        );
    ...
    function mintNft(address _to, string memory _tokenURI) external {
        if (msg.sender != implementationAddress) revert NoAccess(msg.sender);

        if (!isMintingOpen) revert MintingNotOpen();

        if (balanceOf(_to) == maxTokensPerUser)
            revert MaxTokensMintedForUser(_to);

        if (!isNftTotalSupplyUnlimited) {
            if (_tokenIdTracker == totalSupplyOfToken) revert MaxTokensMinted();
        }

        if (onlyAllowWhitelist) {
            if (!allowMint[_to]) revert NotWhitelisted();
        }
