...
import {LibOrders, AuctionListOrder, AuctionBidOrder, AuctionSetOwnerOrder} from "../../shared/libraries/LibOrders.sol"; 

contract AuctionListFacet is Modifiers {
    AppStorage internal s;

    event AuctionBundleListed1(
    ...
    function auctionListBundle(
        AuctionListOrder calldata order
    ) public {

        if (!order.deferred) {
          require(
            order.owner == LibCommon.msgSender(),
            "auctionListBundle: SENDER_MUST_BE_OWNER"
          );
        } else {
          require(
            order.owner == address(0),
            "auctionListBundle: VIRTUAL_OWNER_MUST_BE_0x0"
          );
        }

        LibAuction.checkAuctionListOrder(order);

        LibAuction.auctionList(order);
    }
}
...
import {
    ...
    AuctionBidOrder,
    AuctionResolveOrder,
    AuctionSetOwnerOrder
} from "../../shared/libraries/LibOrders.sol";

enum AuctionResult { AuctionPreserved, AuctionCancelled, BidAccepted }
...
library LibAuction {
    ...
    function emitAuctionBundleListed(
        uint256 bundleId,
        uint8[] memory standards,
        AuctionListOrder calldata order
    ) internal {

        Bundle storage bundle = LibAppStorage.diamondStorage().market.bundles[order.bundleId];

        emit AuctionBundleListed1(
            bundleId,
            bundle.owner,
            order.paymentToken,
            order.tokens,
            order.ids,
            order.amounts,
            order.startingPrices,
            standards
        );

        emit AuctionBundleListed2(
            bundleId,
            bundle.reservePrice,
            order.duration, 
            uint8(bundle.state),
    ...
    function checkAuctionListOrder (
        ...
        require(
          ...
          "checkAuctionListOrder: INVALID_BUNDLE_SIZE"
        );

        require(
            bundleSize > 0,
            "checkAuctionListOrder: ZERO_SIZE"
        );
        require(
            order.ids.length == bundleSize &&
            order.amounts.length == bundleSize &&
            order.startingPrices.length == bundleSize,
            "checkAuctionListOrder: SIZE_MISMATCH"
        );
        if (!order.deferred) {
          require(
              order.reservePrice > s.marketConfig.minReservePriceTotal[order.paymentToken],
              "checkAuctionListOrder: INVALID_RESERVE_PRICE"
          );
        }
        require(
            order.duration > s.marketConfig.minAuctionDuration && order.duration < s.marketConfig.maxAuctionDuration,
            "checkAuctionListOrder: INVALID_DURATION"
        );
        if (order.deferred && order.minted) {
            revert("checkAuctionListOrder: VIRTUAL_BUT_MINTED");
        }

        LibMarketplace.checkPaymentSupport(order.paymentToken);
        LibMarketplace.checkTargetSupport(order.targetToken);

        bool isLazy = !order.minted || order.deferred;

        for (uint256 i = 0; i < bundleSize; i++) {

            Asset storage asset = s.market.assets[order.owner][order.tokens[i]][order.ids[i]];

            LibMarketplace.checkAssetIsIdle(asset);

            require(
                order.amounts[i] > 0, 
                "checkAuctionListOrder: ZERO_AMOUNT"
            );
            require(
                order.startingPrices[i] >= s.marketConfig.minPrices[order.paymentToken], 
                "checkAuctionListOrder: INVALID_PRICE"
            );
            require(
                order.standards[i] == AssetStandard.ERC721 ||
                order.standards[i] == AssetStandard.ERC1155, 
                "checkAuctionListOrder: INVALID_STANDARD"
            );

            if (!order.minted && !order.deferred) {
                if (bytes(order.uris[i]).length == 0) {
                    revert("checkAuctionListOrder: MISSING_TOKEN_URI");
                }
                if (order.royalties.length > 0 && order.royalties.length != bundleSize) {
                    revert("checkAuctionListOrder: INVALID_ROYALTIES");
                }
            }

            if (isLazy) {
                AssetVersion version = s.marketConfig.assetTokenVersions[order.tokens[i]];
                LibNFT.checkAssetVersionIsDefault(order.standards[i], version);

                if (order.deferred) {
                    if (order.standards[i] == AssetStandard.ERC721) {
                        require(
                            IFERC721V1(order.tokens[i]).creatorOf(order.ids[i]) == address(0),
                            "checkAuctionListOrder: TOKEN_ID_EXISTS"
                        );
                    } else if (order.standards[i] == AssetStandard.ERC1155) {
                        require(
                            IFERC1155V1(order.tokens[i]).creatorOf(order.ids[i]) == address(0), 
                            "checkAuctionListOrder: TOKEN_ID_EXISTS"
                        );
                    }
                }
    ...
    function auctionList (
        ...
        if (!order.deferred) {
            bundle.reservePrice = order.reservePrice;
        }
        bundle.listingTime = uint64(block.timestamp);
        bundle.duration = order.duration;
        bundle.state = order.minted ? BundleState.OnSale : BundleState.Pending;
        bundle.listingType = order.deferred ? ListingType.VirtualAuction : ListingType.Auction;

        bool lazy = !order.minted || order.deferred;
        bool needRoyalties = lazy && !order.deferred && order.royalties.length > 0;
    ...
    function checkAuctionBidOrder(
        AuctionBidOrder calldata order
    ) internal view {
        ...
        require(
            bundle.listingType == ListingType.Auction || bundle.listingType == ListingType.VirtualAuction,
            "checkAuctionBidOrder: WRONG_LISTING_TYPE"
        );
        require(
            bundle.state == BundleState.OnSale || bundle.state == BundleState.Pending,
            "checkAuctionBidOrder: WRONG_BUNDLE_STATE"
        );
        require(
            bundle.listingTime + bundle.duration > block.timestamp,
            "checkAuctionBidOrder: AUCTION_EXPIRED"
        );

        if (bundle.bid.active) {
            require(
                bundle.bid.bidder != order.bidder,
                "checkAuctionBidOrder: BID_IS_ACTIVE"
            );
            require(
                bundle.bid.value + s.marketConfig.auctionSteps[bundle.paymentToken] <= order.value,
                "checkAuctionBidOrder: LOW_BID_VALUE"
            );
        } else {
            // new bid value must be above starting price
            require(
                LibMarketplace.getBundlePriceTotal(bundle) <= order.value,
                "checkAuctionBidOrder: BID_IS_BELOW_STARTING_PRICE"
            );
        }
    }
    ...
    function makeBid(
        ...
        if (bundle.state == BundleState.Pending && bundle.listingType == ListingType.Auction) {
            LibNFT.mintBundle(bundle);
            bundle.state = BundleState.OnSale;
        }

        IERC20(bundle.paymentToken).transferFrom(order.bidder, address(this), order.value);

        Bid storage bid = bundle.bid;

        if (bid.active) {
            // return current bid
            IERC20(bundle.paymentToken).safeTransfer(bid.bidder, bid.value);
            // each overbid increases acution duration
            bundle.duration = bundle.duration + s.marketConfig.auctionProlongation;
        ...
        bid.bidder = order.bidder;
        bid.value = order.value;
        bid.active = true;
    ...
    function resolveAuction(
        AuctionResolveOrder calldata order
    ) internal {
        ...
        require(
            bundle.listingType == ListingType.Auction || bundle.listingType == ListingType.VirtualAuction,
            "resolveAuction: WRONG_LISTING_TYPE"
        );
        require(
            bundle.state == BundleState.OnSale || bundle.state == BundleState.Pending,
            "resolveAuction: WRONG_BUNDLE_STATE"
        );

        AuctionResult result;

        uint256 relaxationTime = bundle.listingType == ListingType.VirtualAuction ? s.marketConfig.auctionRelaxationTime : 0;

        if (LibCommon.msgSender() == bundle.owner) {
            // owner can resolve at any time
            result = auctionExecute(
                order.bundleId,
                order.accept
            );
        } else if (block.timestamp > bundle.listingTime + bundle.duration + relaxationTime) {
            // resolver can resolve after relaxation time has passed
            // resolver can accept only if bid value is above reserve price
            // resolver cannot accept if auction is virtual
            result = auctionExecute(
                order.bundleId,
                bundle.listingType == ListingType.VirtualAuction ? false : bundle.bid.value >= bundle.reservePrice
            );
        }
    ...
    function auctionExecute(
        uint256 bundleId,
        bool accept
    ) internal returns (AuctionResult) {

        AppStorage storage s = LibAppStorage.diamondStorage();

        Bundle storage bundle = s.market.bundles[bundleId]; 

        AuctionResult result;

        Bid storage bid = bundle.bid;

        if (bid.active && accept && bundle.state == BundleState.OnSale) {
            ...
            for (uint256 i = 0; i < bundleSize; i++) {
                ...
                uint256 id = bundle.ids[i];

                Asset storage asset = s.market.assets[bundle.owner][token][id];

                uint256 cost = bid.value;
    ...
    function setAuctionOwner(
        AuctionSetOwnerOrder calldata order
    ) internal {
        ...
        LibNFT.validateRoyalty(order.royalty);  

        for (uint256 i = 0; i < bundleSize; i++) {
            if (bytes(order.uris[i]).length == 0) {
                revert("auctionSetOwner: MISSING_TOKEN_URI");
            }

            address token = bundle.tokens[i];
            uint256 id = bundle.ids[i];

            Asset storage virtualAsset = s.market.assets[bundle.owner][token][id];
            Asset storage asset = s.market.assets[order.owner][token][id];

            asset.bundleId = virtualAsset.bundleId;
            asset.amount = virtualAsset.amount;
            asset.price = virtualAsset.price;
            asset.uri = order.uris[i];
            asset.standard = virtualAsset.standard;
            asset.version = virtualAsset.version;
            asset.royalty = order.royalty;

            virtualAsset.bundleId = 0;
        }
