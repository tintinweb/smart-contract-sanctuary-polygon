...
import './ERC721EFactory.sol';

import "../nfts/ERC721EW.sol";

contract ERC721EWFactory is ERC721EFactory {

    using EnumerableSet for EnumerableSet.AddressSet;

    constructor(address forwarder) ERC721EFactory(forwarder) {
...
contract ERC721E is IERC721E, ERC721AMarketplace, Ownable, AccessControl {
    ...
    VenueRegistar private _venueRegistar;
    ...
    constructor(
        ...
        _venueRegistar = VenueRegistar(payable(venueRegistar));
    ...
    function buyTickets(
        ...
        if (amount != sellingPrices.length) revert ERC721WrongBatchLengths();
        ...
        //Implement royalties
        if (price > 0 && _msgSender() != _venueRegistar.tokenCreator()) {
...
contract ERC721EW is IERC721EW, ERC721AMarketplace, Ownable, AccessControl {
    ...
    VenueRegistar internal _venueRegistar;
    ...
    function editMerkleRoot(uint256 entryPos, bytes32 newMerkleRoot) external virtual override onlyRole(CREATOR_ROLE) {
        if (entryPos > entriesCount) revert ERC721WrongEntryPos();
        EntryType storage entry = _entryByPos[entryPos];
    ...
    function buyTickets(
        ...
        address tokenCreator = _venueRegistar.tokenCreator();

        //For merkle verification
        if(merkleRoot != "") {
            address wallet = _msgSender();
            if(_msgSender() == tokenCreator) wallet = receiver;
            if(!_verifyWhitelist(merkleRoot, merkleProof, wallet)) revert ERC721EWUnableToVerifyMerkleProof();
        } 
        
        if (price > 0 && _msgSender() != tokenCreator) {
            uint256 totalPrice = amount * price;
            (address receiverRoyalty, uint256 amountRoyalty) = _getDefaultRoyaltyBatch(totalPrice);
            IERC20(paymentToken).transferFrom(_msgSender(), venue, totalPrice - amountRoyalty);
            IERC20(paymentToken).transferFrom(_msgSender(), receiverRoyalty, amountRoyalty);
    ...
    function _verifyWhitelist(bytes32 merkleRoot, bytes32[] calldata merkleProof, address addr) private pure returns(bool) {
    ...
    ERC165
{
    ...
    address private _venueFactory;

    address private _forwarder;

    address private _tokenReviewPaymaster; // address of GSN Paymaster
    address private _tokenReview;
    address public tokenCreator; //EOA which can mint on behalf of stripe credit card payment

    uint256 private _tokenReviewMinBalance; // min balance VenueRegistar should have to pay GSN reviews in ETH, MATIC

    uint256 public price; //in ETH, cost to deploy a venue when sender is not registar owner

    string private _versionVenueSBT;

    mapping(address => address) public splitterByVenue;

    EnumerableSet.AddressSet private _venues;
    ...
    /// @inheritdoc IVenueRegistar
    function versionRegistar()
        external
        pure
        virtual
        override
        returns (string memory)
    {
        return "1.0.0-beta.0+fob.rsv.iVenueRegistar";
    }
    ...
    /// @inheritdoc IVenueRegistar
    function whitelistPaymentToken(address token, bool whitelist) external virtual override onlyOwner {
        if(whitelist) _whitelistedPaymentTokens.add(token);
        else _whitelistedPaymentTokens.remove(token);
    }

    /// @inheritdoc IVenueRegistar
    function setVenueFactory(address venueFactory) external virtual override onlyOwner {
        if(venueFactory == address(0)) revert RegistarWrongVenueFactory();
        _venueFactory = venueFactory;
    }

    /// @inheritdoc IVenueRegistar
    function setTokenCreator(address newTokenCreator) external virtual override onlyOwner {
    ...
    function setTokenReviewParams(
        ...
        _tokenReview = tokenReview;
        _tokenReviewPaymaster = tokenReviewPaymaster;
        _tokenReviewMinBalance = minBalance;
    }

    /// @inheritdoc IVenueRegistar
    function setPrice(uint256 publicPrice) external virtual override onlyOwner {
        price = publicPrice;
    }
    ...
    /// @inheritdoc IVenueRegistar
    function setVenueSBTVersion(string memory newVersion)
        external
        virtual
        override
        onlyOwner
    {
        _versionVenueSBT = newVersion;
    }
    ...
    function deployVenue(
        ...
        if(_venueFactory == address(0)) revert RegistarUndefinedVenueFactory(); 
        if (msg.value != price) revert RegistarInvalidPrice();
        _deployVenue(name, symbol, baseUri, _msgSender());
    }
    ...
    function ownerDeployVenue(
        ...
        _deployVenue(name, symbol, baseUri, owner);
    }

    function _deployVenue(
        string memory name,
        string memory symbol,
        string memory baseUri,
        address owner
        ...
        (address venueAddress, address splitter) = IVenueFactory(_venueFactory).deployVenue(
            name,
            symbol,
            baseUri,
            owner
        );
        _venues.add(venueAddress);
        splitterByVenue[venueAddress] = splitter;
        emit NewVenue(venueAddress, splitter, owner, block.number);
    }

    /// @inheritdoc IVenueRegistar
    function removeVenue(address venue) external virtual override onlyOwner {
        if (!_venues.contains(venue)) revert RegistarVenueNotFound();
        _venues.remove(venue);
    }

    /// @inheritdoc IVenueRegistar
    function registerVenue(address venue) external virtual override onlyOwner {
        IVenueSBT venueSBT = IVenueSBT(venue);

        if (
            keccak256(abi.encodePacked(venueSBT.versionVenueSBT())) !=
            keccak256(abi.encodePacked(_versionVenueSBT))
        ) revert RegistarSBTVersionNotSupported();

        if (venueSBT.getVenueRegistar() != address(this))
            revert RegistarWrongVenueRegistar();

        if (_venues.contains(venue)) revert RegistarVenueAlreadyRegistered();

        _venues.add(venue);
    }

    /// @inheritdoc IVenueRegistar
    function getWhitelistedPaymentTokens() external virtual view override returns(address[] memory) {
        return _whitelistedPaymentTokens.values();
    }

    /// @inheritdoc IVenueRegistar
    function isPaymentTokenWhitelisted(address token) external virtual view override returns(bool) {
        return _whitelistedPaymentTokens.contains(token);
    }
    ...
    function getTokenReviewParams()
        ...
        override
        returns (address, address, uint256) {
        return (_tokenReview, _tokenReviewPaymaster, _tokenReviewMinBalance);
    }

    /// @inheritdoc IVenueRegistar
    function getMinBalance() external view virtual override returns(uint256) { 
        return _tokenReviewMinBalance;
    }

    /// @inheritdoc IVenueRegistar
    function isVenue(address venue)
        external
        view
        virtual
        override
    ...
    function withdrawToken(address token, address to)
        ...
        onlyOwner
    {
        if (to == address(0)) revert RegistarInvalidReceiver();
        IERC20 erc20 = IERC20(token);
        erc20.transfer(to, erc20.balanceOf(address(this)));
    }

    /// @inheritdoc IVenueRegistar
    function withdrawETH(address receiver) external virtual override onlyOwner {
        (bool success, ) = receiver.call{value: address(this).balance}("");
        if (!success) revert RegistarUnableToWithdrawETH();
    }
    ...
    {
        return
            interfaceId == type(IVenueRegistar).interfaceId ||
            super.supportsInterface(interfaceId);
    }
