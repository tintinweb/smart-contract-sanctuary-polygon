...
contract DexGoNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    function getTokenIdCounterCurrent() public view returns (uint) {
    ...
    function setLatestPurchaseTime(address wallet, uint timestamp) public  {
        require(msg.sender == gameServer || msg.sender == owner() || msg.sender == rentAndKm,'DexGoNFT: only admin accounts can change it');
    ...
    function isApprovedOrOwner(address sender, uint256 tokenId) public view returns(bool) {
...
//    }
    ...
    function _safeMintType(address to, string memory uri, uint8 typeNft, bool needURI) private {
        ...
        IDexGoRentAndKm(rentAndKm).setKmForId(tokenId, priceInitialForType[typeNft]);
    }
    ...
    function distributeMoney(address sender, uint value) public {
        require(msg.sender == rentAndKm, "only rent contract can distribute");
        _distributeMoney(sender, value);
    }
    ...
    function purchaseBatchValue(uint8[] memory typesNft) public view returns (uint256) {
        ...
        for(uint256 x=0;x<typesNft.length;x++) {
            if (_priceForType[typesNft[x]] == 0) _priceForType[typesNft[x]] = priceForType[typesNft[x]];
    ...
    returns (string memory)
    {
        ...
        if (IDexGoRentAndKm(rentAndKm).getKmLeavesForId(tokenId) * 100 / priceInitialForType[typeNft] < 25) image = imageForType25PercentKm[typeNft];
        if (IDexGoRentAndKm(rentAndKm).getKmLeavesForId(tokenId) * 100 / priceInitialForType[typeNft] < 50) image = imageForType50PercentKm[typeNft];
        if (IDexGoRentAndKm(rentAndKm).getKmLeavesForId(tokenId) * 100 / priceInitialForType[typeNft] < 75) image = imageForType75PercentKm[typeNft];
