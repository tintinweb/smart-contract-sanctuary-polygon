...
contract MagpieCore is ReentrancyGuard, Ownable, Pausable, IMagpieCore {
  ...
  function updateConfig(Config calldata _config) external override onlyOwner {
    ...
    require(_config.hyphenLiquidityPool != address(0), 'MagpieCore: invalid hyphenLiquidityPool');
    ...
    returns (uint256[] memory amountOuts)
  {
    ...
    if (to != address(this)) {
      toAssetId.transfer(to, amountOut); // the amount while transferring will be less than the actual amount out. ~amountOut/ amountOut
    }
  }
    ...
    )
  {
    ...
    uint256 beforeSwapBal = args.swapArgs.getToAssetId().getBalance();
    uint256 amountOut;
    ...
    uint256 afterSwapBal = args.swapArgs.getToAssetId().getBalance();

    require(
      afterSwapBal - beforeSwapBal > args.payload.swapOutGasFee,
      'MagpieCore: invalid swapOutGasFee'
    );

    if (useTokenBridge) {
      args.swapArgs.getToAssetId().increaseAllowance(
        config.tokenBridgeAddress,
        afterSwapBal - beforeSwapBal
      );
      tokenSequence = IWormhole(config.tokenBridgeAddress).transferTokens(
        args.swapArgs.getToAssetId(),
        afterSwapBal - beforeSwapBal,
        args.payload.recipientBridgeChainId,
        args.payload.recipientCoreAddress,
        0,
        uint32(block.timestamp % 2**32)
      );
    } else {
      args.swapArgs.getToAssetId().increaseAllowance(
        config.hyphenLiquidityPool,
        afterSwapBal - beforeSwapBal
      );
      tokenSequence = 0;
      IHyphenLiquidityPool(config.hyphenLiquidityPool).depositErc20(
        args.payload.recipientChainId,
        args.swapArgs.getToAssetId(),
        address(uint160(uint256(args.payload.recipientCoreAddress))),
        afterSwapBal - beforeSwapBal,
        'test'
      );
    }
    {
      bytes memory payloadOut = bytes.concat(
        abi.encodePacked(
          ...
          args.payload.to,
          args.payload.recipientPoolAddress,
          args.payload.recipientCoreAddress,
          args.payload.recipientChainId,
      ...
      emit EmitSequence(amountOuts, coreSequence, tokenSequence, msg.sender);
  ...
  function swapOut(SwapOutArgs calldata args)
    ...
    returns (uint256[] memory amountOuts)
  {
    (IWormholeCore.VM memory vm, bool valid, ) = IWormholeCore(config.coreBridgeAddress)
    ...
    if (payload.useTokenBridge == true) {
      IWormhole(config.tokenBridgeAddress).completeTransfer(args.encodedVmBridge);
    } else {
      (, bool status) = IHyphenLiquidityPool(config.hyphenLiquidityPool).checkHashStatus(
        payload.fromAssetId,
        payload.amountIn,
        payload.recipientCoreAddress,
        args.depositHash
      );
      require(status, 'MagpieCore: transaction not processed');
    }
    ...
    swapArgs.routes[0].amountIn = swapArgs.routes[0].amountIn - (payload.destGasTokenAmount + payload.swapOutGasFee);
    ...
    if (payload.destGasTokenAmount > 0) {
      ...
      _wrapSwap(args.gasTokenSwapArgs, false);
    }

    gasFeeAccumulatedByToken[payload.fromAssetId] += payload.swapOutGasFee;
    gasFeeAccumulated[payload.fromAssetId][payload.recipientCoreAddress] += payload.swapOutGasFee;

    emit SwappedOut(args, amountOuts, msg.sender);
  }

  function withdrawGasFee(address tokenAddress) external whenNotPaused nonReentrant {
...
interface IMagpieCore {
  ...
  struct ValidationInPayload{
    ...
    bytes32 toAssetId;
    bytes32 to;
    bytes32 recipientPoolAddress; 
    uint256 amountOutMin;
    bytes32 recipientCoreAddress; 
    uint256 recipientChainId;
    uint16 recipientBridgeChainId; /* while using wormhole for swapping in and out */
  ...
  struct ValidationOutPayload {
    ...
    address toAssetId;
    address to;
    address recipientPoolAddress; /* user address */
    address recipientCoreAddress; /* Magpie core address of the recipient chain */
    uint256 recipientChainId;
    uint256 amountOutMin;
    uint256 swapOutGasFee;
    uint256 destGasTokenAmount;
...
library LibBytes {
  ...
  function parse(bytes memory self)
    internal
    pure
    returns (IMagpieCore.ValidationOutPayload memory payload)
  {
    uint256 i = 0;
    ...
    payload.recipientPoolAddress = self.toAddress(i);
    i += 32;
    ...
    payload.useTokenBridge = self.toUint8(i) == 1 ? true : false;
    i += 1;
