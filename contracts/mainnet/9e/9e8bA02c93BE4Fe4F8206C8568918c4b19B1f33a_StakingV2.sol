...
contract StakingV2 is Ownable, AccessControl {
    ...
    mapping(address => bool) public allowedStakingAncestors;
    ...
    function addStakingAncestors(address[] memory stakingInstances, bool status) public onlyOwner {
        for (uint i=0; i<stakingInstances.length; ++i) {
            allowedStakingAncestors[stakingInstances[i]] = status;
        }
        emit StakingAncestorChanged();
    }
    ...
    function deposit(uint256 pid, address addr, uint256 amount, uint256 timerange) external {
        require(msg.sender == addr || isMaintainer(msg.sender) || allowedStakingAncestors[msg.sender],
            'Staking: unable to stake funds to specified address');
