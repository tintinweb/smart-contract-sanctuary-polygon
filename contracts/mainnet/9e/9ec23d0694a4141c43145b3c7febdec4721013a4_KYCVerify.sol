 ...
 * Platform.
 */
contract KYCVerify is Initializable, ContextUpgradeable {
    ...
    error KYCVerify_NotKYCVerifier();
    ...
    modifier onlyKyCVerifierRole() {
        if (!MarketRegistry(_marketRegistry).hasKYCVerifierRole(_msgSender())) {
            revert KYCVerify_NotKYCVerifier();
        }
        _;
    }

    function initialize(string calldata kycProvider_, address marketRegistry_)
        public
        initializer
    {
        if (marketRegistry_ == address(0)) {
            revert KYCVerify_ZeroAddress();
        }
