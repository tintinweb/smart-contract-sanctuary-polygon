...
contract EthermonTradeDebug is EthermonEnum, BasicAccessControl, SafeMathEthermon {
    ...
    return requestPrice;
     ...
     function buyItemEmon13(uint64 _objId) external   view  returns (uint256) {
        return emon.balanceOf(msgSender());     
      
    }
 

    
    function buyItemEmon1(uint64 _objId) external   view returns (uint256) {
        // check item is valid to sell
        EthermonTradeData monTradeData = EthermonTradeData(
            tradingMonDataContract
        );
  ...
  function buyItemEmon2(uint64 _objId) external   view returns (uint256) {
        // check item is valid to sell
        EthermonTradeData monTradeData = EthermonTradeData(
            tradingMonDataContract
        );
 ...
 return requestPrice;
  ...
  function buyItemEmon3(uint64 _objId) external   view returns (uint256) {
        // check item is valid to sell
        EthermonTradeData monTradeData = EthermonTradeData(
            tradingMonDataContract
        );
