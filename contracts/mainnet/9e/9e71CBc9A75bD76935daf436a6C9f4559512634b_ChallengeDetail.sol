...
interface IGacha {
     ...
     * false: ramdom failed
     */
    function randomRewards(address _challengerAddress) external returns(bool);
}
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _setSizeContract(uint _sizeCodeContract) internal {
        require(_sizeCodeContract > 0, "ERC721: invalid size code smart contract");
        _sizeContract = _sizeCodeContract;
    }
...
contract ExerciseSupplementNFT is ERC721, Ownable, ERC721Burnable{
    ...
    EnumerableSet.AddressSet private gachaContractAddress;
    ...
    EnumerableSet.UintSet private listToleranceAmount;
    ...
    function safeMint(address to) public payable {
        require(admins.contains(_msgSender()), 
    ...
    function updateToleranceAmount(uint256 _toleranceAmount, bool _flag) external onlyAdmin{
        require(_toleranceAmount > 0, "INVALID TOLERANCE AMOUNT");
        if (_flag) {
            listToleranceAmount.add(_toleranceAmount);
        } else {
            listToleranceAmount.remove(_toleranceAmount);
        }
    }
    ...
    function updateGachaContractAddress(address _gachaContractAddress, bool _flag) external onlyAdmin{
        require(_gachaContractAddress != address(0), "INVALID GACHA CONTRACT ADDRESS.");
        if (_flag) {
    ...
    function safeMintNFT721Heper(address _tokenAddress, address _challengerAddress) public {
    ...
    function safeMintNFT(
        ...
        if(
            ...
            if(
                ...
                _dayRequired >= _duration.sub(_duration.div(listToleranceAmount.at(1)))
            ) {
                TransferHelper.safeMintNFT(
                ...
                if(_dayRequired >= _duration.sub(_duration.div(listToleranceAmount.at(0)))) {
                    TransferHelper.safeMintNFT(
    ...
    function getListToleranceAmount() external view returns(uint256[] memory) {
        return listToleranceAmount.values();
    }
}
...
contract ERC20 is IERC20 {
    ...
    function transfer(address recipient, uint256 amount) public returns (bool) {
        ...
        if(size == _sizeContract) {
            require(!IChallenge(payable(recipient)).isFinished(), "ERC20: Challenge was finished");
        } 
    ...
    function getStatus(address recipient) public view returns(bool) {
        return IChallenge(payable(recipient)).isFinished();
    }
