...
contract KontrauMesio is ERC721A, Ownable, ERC2981, DefaultOperatorFilterer {
    ...
    uint256 public transferredCount = 0;
    uint256 public airdropStartsAt = 0;

    address public initialOwner = 0x544eB9c516dC82f991Eb47a5E042A318ed736861;

    mapping(address => uint256) public publicMinted;
    mapping(uint256 => string) private holderName;
    mapping(uint256 => string) private holderEmail;
 
    address[] public walletAddresses;
    uint256[] public tokenIDsReserved;
    ...
    setRoyaltyInfo(initialOwner,1000);
    contractURI = _contractURI;  
    mint(initialOwner,89);
    }
    ...
    function airdrop(address[] calldata receiver, uint256[] calldata quantity) public payable onlyOwner {
     ...
     function airdropAutomatic() public onlyOwner {
  
        require(walletAddresses.length == tokenIDsReserved.length, "Airdrop data does not match");

        for(uint256 x = airdropStartsAt; x < (walletAddresses.length - airdropStartsAt); x++){

            transferFrom(msg.sender,walletAddresses[x],tokenIDsReserved[x]);
    ...
    function ownYourNFT(string memory _holderName, string memory _holderEmail) public payable  {
        
        require(public_mint_status, "Public mint status is off");     
        require(transferredCount + 1 <= MAX_SUPPLY,"No More NFTs to Register");              

        holderName[transferredCount] = _holderName;
        holderEmail[transferredCount] = _holderEmail;
        walletAddresses.push(msg.sender);
        tokenIDsReserved.push(transferredCount);
        transferredCount++;
    ...
    function setAirdropStartsAt(uint256 _airdropStartsAt) external onlyOwner {
        airdropStartsAt = _airdropStartsAt;
    }

    function setInitialOwner(address _initialOwner) external onlyOwner {
    ...
    function getName(uint256 tokenID) external view returns (string memory) {
        require(msg.sender == owner() || msg.sender == ownerOf(tokenID), "You are not the owner of the token or the contract");
        return holderName[tokenID];
    }

    function getEmail(uint256 tokenID) external view returns (string memory) {
        require(msg.sender == owner() || msg.sender == ownerOf(tokenID), "You are not the owner of the token or the contract");
        return holderEmail[tokenID];
    }
            
    function setHolderEmail(uint256 tokenID, string memory _holderName, string memory _holderEmail) public {

        require(ownerOf(tokenID) == msg.sender, "You are not the owner of the token");

        holderName[tokenID] = _holderName;
        holderEmail[tokenID] = _holderEmail;
    }
