...
interface IPcvStorage is PcvStruct{
    ...
    function getMaxInputPercent() external view returns(uint);
    ...
    function setMaxInputPercent(uint percent) external;
    ...
    function _callMethod(PcvStrategy memory strgy,StrategyExecVariable memory vrb, uint step) internal
        ...
        for(uint i = 0 ; i < strgy.inputTokens[step].length; i++){
            ...
            if(minfo.needInvest[i]){
                ...
                inAmount[inIdx] = vrb.initAmount * strgy.inputPercent[step][i] / strgy.percentBase;
                inIdx += 1;
            }
        }
...
contract PcvProxy is PcvStruct ,IERC20{
    ...
    function lg_addStrategy(
        ...
        uint maxInputPercent =  IPcvStorage(_pcvStorage).getMaxInputPercent();
        ...
        for(uint m = 0; m < methods.length; m++){
           ...
           needInvest[m] = minfo.needInvest;
            ...
            for(uint p = 0; p < inputPercent[m].length;p++){
                require(maxInputPercent >= inputPercent[m][p],"inputPercent is out of range");
            ...
            // unclose strategy, the first step need invest
            if(!closePosition  && (m == 0) ){
                require(hasInvest,"First step input error");
            }
        }

        // unclose strategy, verify the each input asset whether can get from previous output or init asset
        if(!closePosition){
            for(uint s = methods.length-1; s >= 1 ; s--){
                for(uint i = 0;i < inputTokens[s].length;i++){
                    if(needInvest[s][i]){
                        bool hasOutPut = false;
                        if(inputTokens[s][i] == initToken ){
                            continue;
                        }
                        for(uint bs = s-1;bs >= 0; bs--){
                            for(uint out = 0; out < outputTokens[bs].length;out++){
                                if(outputTokens[bs][out] == inputTokens[s][i]){
                                    hasOutPut = true;
                                    break;
                                }
                        }
                    ...
                    require(hasOutPut , "Input asset cannot find source");
                    }
    ...
    function _checkToken(address [] memory protocols,address [][] memory inTokens,address [][] memory outTokens) public view {
        for(uint p = 0 ; p < protocols.length;p++){
            for(uint i = 0; i< inTokens[p].length;i++){      
                require(
                    IPcvStorage(_pcvStorage).isSupportAsset(protocols[p],inTokens[p][i]), 
                    string(abi.encodePacked("protocol:",protocols[p]," not support token:",inTokens[p][i]))
                );
            }

            for(uint out = 0; out < outTokens[p].length;out++){
                require(
                    IPcvStorage(_pcvStorage).isSupportAsset(protocols[p],outTokens[p][out]), 
                    string(abi.encodePacked("protocol:",protocols[p]," not support token:",outTokens[p][out]))
                );
            }
        }
    }
    ...
    function invest(uint256 amount) external payable{
        ...
        // mint pcvToken
        uint8 decimal = IERC20(settleAsset).decimals();

        uint8 decimalDiff = IPcvStorage(_pcvStorage).decimals() - decimal;
        _mint(msg.sender,amount/netWorth * 10 ** decimalDiff);
    ...
    function redeem(uint256 amount) external{
        ...
        uint8 decimal = IERC20(settleAsset).decimals();
        uint8 decimalDiff = IPcvStorage(_pcvStorage).decimals() - decimal;
        uint256 redeemAmount = amount / 10**decimalDiff;
        redeemAmount = redeemAmount * netWorth / decimal;
    ...
    function _recordAssets(address [][] memory outputTokens) internal {
        ...
        address [] memory newOutTokens = new address[](allOutTokens+1);
        uint idx = 0;
        ...
        newOutTokens[idx] = getSettleAsset();
...
contract PcvFactory is Ownable{
   ...
   // build a new PcvProxy
    function buildPcvProxy(address pcvOwner,address settleAsset,uint256 minInvest,uint256 maxInvest) external onlyOwner returns(address){
    ...
    function _storePcv(address pcvOwner,address pcv,address settleAsset,uint256 minInvest,uint256 maxInvest) internal onlyOwner{
