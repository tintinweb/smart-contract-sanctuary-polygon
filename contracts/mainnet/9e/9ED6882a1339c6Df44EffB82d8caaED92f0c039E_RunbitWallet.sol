...
contract RunbitWallet is AccessControl {
    ...
    constructor(address _rs, address _rc, address _rt) {
    ...
    function _getMinEarnPrice(IRunbitCollection.ShoesCollection memory colection) private pure returns (uint256) {
        uint256 maxEarn = uint256(colection.meta.specialty 
                            + colection.meta.comfort 
                            + colection.meta.aesthetic 
                            + colection.adjust1 
                            + colection.adjust2 
                            + colection.adjust3)
                        * colection.meta.durability;
        return  colection.price1 / maxEarn;
    }
    ...
    function addShoesCollection(IRunbitCollection.ShoesCollection memory colection) external onlyRole(OPERATE_ROLE) {
        ...
        uint256 minEarnPrice = _getMinEarnPrice(colection);
        require(minEarnPrice >= minPrice, "price too small!");
        require(colection.meta.durability >= minDuration, "duration too small!");
    ...
    function editShoesCollection(uint256 collectionId, uint256 price0, uint256 price1, uint256 status) external onlyRole(OPERATE_ROLE) {
        IRunbitCollection.ShoesCollection memory colection = RC.shoesCollections(collectionId);
        uint256 minEarnPrice = _getMinEarnPrice(colection);
        require(minEarnPrice >= minPrice, "price too small!");
