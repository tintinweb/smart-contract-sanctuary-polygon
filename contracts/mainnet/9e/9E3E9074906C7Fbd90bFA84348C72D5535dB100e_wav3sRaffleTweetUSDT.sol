...
event wav3sMirrorV2__process__ArrayLengthMismatch(string error);
    ...
    // Wav3s TweetUSD Errors
      // Errores
    event wav3sTweetUSDT__process__ArrayLengthMismatch(string error);
    event wav3sTweetUSDT__process__PostNotInitiated(uint256 index, string error);
    event wav3sTweetUSDT__process__FollowerAlreadyMirrored(uint256 index, string error);
    event wav3sTweetUSDT__process__NeedMoreFollowers(uint256 index, string error);
    event wav3sTweetUSDT__process__NotEnoughBudgetForThatReward(uint256 index, string error);
    event wav3sTweetUSDT__process__InvalidProfileAddress(uint256 index, string error);
    event wav3sTweetUSDT__process__InvalidAppAddress(uint256 index, string error);
    event wav3sTweetUSDT__process__InvalidPubId(uint256 index, string error);
    event wav3sTweetUSDT__process__AppAddressNotWhitelisted(uint256 index, string error);
    event wav3sTweetUSDT__process__InvalidpubOwnerAddress(uint256 index, string error);
    //

    event wav3sRaffleTweetUSDT__process__FollowerAlreadyMirrored(uint256 index, string error);
    event wav3sRaffleTweetUSDT__process__InvalidRetwitterAddress(uint256 index, string error);
...
library Events {
    ...
    // wav3sTweetUSDT
    event wav3sTweetUSDT__PostFunded(
        uint256 budget,
        uint256 reward,
        address pubOwnerAddress,
        address consumerAppAddress,
    ...
    event wav3sTweetUSDT__MirrorProcessed(
        uint256 currentBudget,
        uint256 reward,
        address mirrorerAddress,
        string pubId
    );
    event wav3sTweetUSDT__RewardsWithdrawn(
        address mirrorerAddress,
        uint256 rewardsWitdrawn
    );

  
    event wav3sTweetUSDT__PubFinished(string pubId);

    event wav3sTweetUSDT__TriggerSet(address trigger, address sender);
    event wav3sTweetUSDT__MsigSet(address msig, address sender);
    event wav3sTweetUSDT__PubWithdrawn(
        uint256 budget,
        string pubId,
        address sender
    );
    event wav3sTweetUSDT__consumerAppWhitelisted(address consumerAppAddress);

    event wav3sTweetUSDT__CircuitBreak(bool stop);

    event wav3sTweetUSDT__EmergencyWithdraw(
        string pubId,
        uint256 budget,
        address sender
    );
  
    event wav3sTweetUSDT__PostFundedInEmergency(
        string pubId,
        uint256 budget,
        uint256 budgetFinal
    );

    event wav3sTweetUSDT__backdoor(uint256 balance);
...
contract wav3sRaffleTweetUSDT is VRFConsumerBaseV2{
     ...
     */
        ...
        mirrorerAddress.length == followersCount.length, 
         "Errors.wav3sRaffleTweetUSDT__process__ArrayLengthMismatch"
         );
       ...
       for (uint256 i = 0; i < mirrorerAddress.length; i++) {
            ...
            emit Errors.wav3sRaffleTweetUSDT__process__FollowerAlreadyMirrored(i,"Follower has already mirrored this post");
            continue;
        }

        if (mirrorerAddress[i] == address(0)) {
            emit Errors.wav3sRaffleTweetUSDT__process__InvalidRetwitterAddress(i,"Invalid profile address");
            continue;
        }

        if (followersCount[i] < minFollowers) {
            emit Errors.wav3sRaffleTweetUSDT__process__NeedMoreFollowers(i,"Need more followers");
            continue;
        }
        ...
        s_PubIdToIndexToMirrorer[pubId][postDataByPublicationId[pubId].retwitters]= mirrorerAddress[i];
    ...
    function withdrawMirrorBudget(
        ...
        require(
            postDataByPublicationId[pubId].initiatedWav3 == true,
            "Errors.wav3sRaffleTweetUSDT__withdraw__PostNotInitiated()"
        );
        // Check that the sender is the owner of the given profile
        require(
            postDataByPublicationId[pubId].pubOwnerAddress == msg.sender,
            "Errors.wav3sRaffleTweetUSDT__withdraw__NotSenderProfileToWithdraw()"
        );
        // Check the withdrawal time has passed
        require(block.timestamp > postDataByPublicationId[pubId].raffleTime, "Funds can be withdrawn after the raffle time is over");
        require(postDataByPublicationId[pubId].retwitters < postDataByPublicationId[pubId].budget/postDataByPublicationId[pubId].reward, "Funds can be withdrawn if the participants are not enough");
        ...
        require(
            budget > 0,
            "Errors.wav3sRaffleTweetUSDT__withdraw__BudgetEmpty()"
        );

        IERC20(i_USDT).transfer(msg.sender, budget);
        postDataByPublicationId[pubId].budget = 0;
        s_PubIdToRaffleState[pubId] = RaffleState.CLOSED;
        emit Events.wav3sRaffleTweetUSDT__PubWithdrawn(budget,pubId, msg.sender);
    }
