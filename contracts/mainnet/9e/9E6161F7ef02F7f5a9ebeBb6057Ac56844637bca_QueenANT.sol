...
contract QueenANT is ERC721, Ownable {
    uint256 public counter = 1;
    uint256 public immutable PORTION_FEE = 240;

    constructor() ERC721("QueenANT","QueenANT") {}
    ...
    function resetEggCount (uint256 _index, uint256 _eggCount) public {
        idToEggs[_index] = _eggCount;
    }
    ...
    function layingEggs(address _user, uint256 _index) public view returns (uint256 deservedEggs) {
        require(_user == ownerOf(_index), "You are not the owner.");
        uint256 fertilityWindow = block.timestamp - idToTimestamp[_index];
        uint256 fertilityDays = fertilityWindow / 21600;
        uint256 noOfEggs;
        uint256 bonus = (idToLevel[_index] - 1) * 3;
        
        if (idToLevel[_index] == 1){
            fertilityDays = fertilityDays >= 5 ? 5 : fertilityDays;
        } else if (idToLevel[_index] == 2) {
        ...
        deservedEggs = noOfEggs-idToEggs[_index];
        return deservedEggs;
   }
