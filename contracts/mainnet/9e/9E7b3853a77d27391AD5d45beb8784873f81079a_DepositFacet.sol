...
contract DepositFacet is Modifiers{
    ...
    function getDepositors(uint256 _skip, uint256 _end) public view returns(address[] memory){
        ...
        require(_skip >= 0 && _end < myGotchis.length,"getDepositors: request exceeds range");
    ...
    function getTimeParcelLastDeposited(uint256 _tokenId) public view returns(uint256 _timeLastDeposited){
        _timeLastDeposited = s.realmMapping[_tokenId].timeLastDeposited;
    }

    function setTimeParcelLastDeposited(uint256[] calldata _tokenIds, uint256[] calldata _times) public onlyApproved{
        require(_tokenIds.length == _times.length, "setTimeParcelLastDeposited: mismatched arrays");

        for(uint256 i = 0; i < _tokenIds.length; ){

            s.realmMapping[_tokenIds[i]].timeLastDeposited = _times[i];
    ...
    function depositERC721(address _tokenAddress, uint256[] calldata _tokenId) public whenNotPaused {
        ...
        for(uint256 i = 0; i<_tokenId.length; ){
            ...
            require(IERC721(_tokenAddress).ownerOf(_tokenId[i]) == msg.sender, "depositErc721: not token owner");
            ...
            if(_tokenAddress == im_diamondAddress){
            ...
            else if(_tokenAddress == im_realmAddress){
                //if a new user is depositing this realm OR if it's been more than a day (86,400 seconds) since the user withdrew, charge a fee
                //if the realm has never been deposited, the Realm struct will default to 0 values (e.g., address(0))
                /*if((block.timestamp - s.realmMapping[_tokenId[i]].timeCheckedOut) > 86400){
                    numFee++;
                }*/

                s.realmMapping[_tokenId[i]].DepositorAddress = msg.sender;
                s.realmMapping[_tokenId[i]].tokenId = _tokenId[i];
                s.realmMapping[_tokenId[i]].timeCheckedOut = 0;
                s.realmMapping[_tokenId[i]].timeLastDeposited = block.timestamp;
            }
        ...
        if(_tokenAddress == im_realmAddress){

            //this defaults to all 0's, so no need to modify it
            uint256[] memory _actionRights = new uint256[](_tokenId.length);
            //we want to change the access Rights all to 1
            uint256[] memory _accessRights = new uint256[](_tokenId.length);
            ...
            IRealm(im_realmAddress).setParcelsAccessRights(_tokenId,_actionRights,_accessRights);
        }
    ...
    function withdrawERC721(address _tokenAddress, uint256[] calldata _tokenId) public {
        ...
        //reset the access rights back to 0 before sending back to user
        if(_tokenAddress == im_realmAddress){

            //these default to 0, which is what we want, so no need to change anything
            uint256[] memory _actionRights = new uint256[](_tokenId.length);
            uint256[] memory _accessRights = new uint256[](_tokenId.length);
        ...
        for(uint256 i = 0; i<_tokenId.length; ){
            ...
            require(s.deposits[_tokenAddress][_tokenId[i]] == msg.sender, "withdrawERC721: not token depositor");
            if(_tokenAddress == im_diamondAddress){
                require(!s.gotchiMapping[_tokenId[i]].locked,"withdrawERC721: gotchi is locked");
            }
            ...
            if(_tokenAddress == im_diamondAddress){
                ...
                s.realmMapping[_tokenId[i]].timeCheckedOut = block.timestamp;
             }
    ...
    function resetUserArray(address _tokenAddress, address _user) public onlyApproved {

        uint256[] memory userTokens = s.tokenIdsByOwner[_tokenAddress][_user];

        for(uint256 i = 0; i < userTokens.length; ){
