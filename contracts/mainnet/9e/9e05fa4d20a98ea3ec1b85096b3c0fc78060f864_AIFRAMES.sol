...
contract AIFRAMES is ERC721  {
    uint256 public totalSupply;
    uint256 public cost = 0 ether;
    uint256 public maxMints = 2267;
    ...
    constructor()
        ERC721("AI-FRAMES", "AIFRS")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 2267) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeifg42lzagk5opmzcmqsr4wux6mwr2ucxein3l4gwoaiuqmoc33o7e/", Strings.toString(tokenId), ".json")
            );
    }
