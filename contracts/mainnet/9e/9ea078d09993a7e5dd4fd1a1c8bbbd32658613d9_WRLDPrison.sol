...
contract WRLDPrison is ERC721A, Ownable {
    using Strings for uint256;
    IERC20 public WRLDTokenContract;

    uint256 MINT_LIMIT_PER_WALLET = 5; //5 on public mint
    uint256 MINT_PRICE; //how much in WRLD each NFT will cost
    uint256 MINT_LIMIT; //how many mints there can be (max. supply)
    ...
    function whitelistMint(uint256 _mintAmount) external {
        require(mintActive, "minting is not available yet.");
        ...
        require(whitelistMintCount[msg.sender] + _mintAmount <= 5, "max whitelist mints per wallet reached");
