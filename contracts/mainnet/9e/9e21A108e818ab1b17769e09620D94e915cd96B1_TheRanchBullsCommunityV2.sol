...
error Buddy_NotAllowed();
error Buddy_MutlipleBuddySwitchesNotAllowed();
error Address_CantBeAddressZero();



/// @custom:security-contact [email protected]
contract TheRanchBullsCommunityV2 is 
    Initializable,
    ...
    mapping(address => bool) public whitelistMintAddresses;     // all addresses that are given to me via Buddyship deals with other projects
    ...
    mapping(uint256 => bool) public allowedMintingQuantity;    // used to make 1,2,3,5,7,9 the only allowed minting numbers in the normalBulls mint function
    ...
    mapping(address => address) public myBuddy;   // Buddy mapping; msg.sender  ==> who referred them
    mapping(address => uint256) public myParnterNetworkTeamCount;  // Keeps track of how many people are currently using an address as their Buddy 
    ...
    mapping(address => bool) public buddyAlreadyUpdated;
    ...
    */
    function mint(uint256 _tokenQuantity) public payable {
        ...
        // Buddy update count check 
        if (myBuddy[msg.sender] != address(0)){
            if ( buddyAlreadyUpdated[msg.sender] == false) {
                // set flag to true 
                buddyAlreadyUpdated[msg.sender] = true;

                address currentBuddy = myBuddy[msg.sender];
                myParnterNetworkTeamCount[currentBuddy] += 1;

                address _topNetworker_1 = topNetworker_1;
                address _topNetworker_2 = topNetworker_2;

                if (myParnterNetworkTeamCount[currentBuddy] > myParnterNetworkTeamCount[_topNetworker_1]){
                    topNetworker_1 = currentBuddy;
                    topNetworker_2 = _topNetworker_1;
                } else if (myParnterNetworkTeamCount[currentBuddy] > myParnterNetworkTeamCount[_topNetworker_2]) {
                    topNetworker_2 = currentBuddy;
                }
            }   
        } 
    ...
    */
    ...
    function alphaBullsMintGoldPaper(address _address, uint256 _tokenQuantity) public payable {
        ...
        for(uint256 i = 0; i < _tokenQuantity; i++) {
            goldTokenSupply.increment();
            _safeMint(_address, goldTokenSupply.current());
        }
    ...
    function checkClaimEligibilityGold(uint256 quantity) external view returns (string memory){
        if (!alphaBullsMintLive) {
            return "Minting is not live";
        } else if (quantity > 5) {
            return "max mint amount per transaction exceeded";
        } else if (goldTokenSupply.current() + quantity > mintStoppingPoint[MintType.GOLD]) {
            return "not enough supply left";
        }
        return "";
    }
   ...
   // METADATA
    ...
    /**
     * @dev allow a user to set their buddy address. If alice sets bob as her buddy, Bob gets blocked from setting alice. 
    */
    function setBuddyAddress(address _newBuddy)  public {
        if (address(_newBuddy) == address(0)) { revert Buddy_NotAllowed();}
        if (address(_newBuddy) == msg.sender) { revert Buddy_NotAllowed();}
        if (myBuddy[_newBuddy] == msg.sender) { revert Buddy_NotAllowed();}

        address currentBuddy = myBuddy[msg.sender];
    
        if (currentBuddy == address(0)){
            myBuddy[msg.sender] = _newBuddy;
        } else {
            revert Buddy_MutlipleBuddySwitchesNotAllowed();
        }
    }
    ...
    */
    ...
    function addOrRemoveAddressesFreeMint(address[] calldata _users, bool _bool) external onlyOwner {
       ...
       // Free function 
    function updateMintBonuses() external onlyOwner {
