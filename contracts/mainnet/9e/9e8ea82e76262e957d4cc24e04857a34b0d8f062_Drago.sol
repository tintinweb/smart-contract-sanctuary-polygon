...
contract Membership is Context {
    ...
    function transferOwner(address next) public onlyOwner {
        address prev = _owner;
        setMembership(next, 1);
        setMembership(prev, 0);
        _owner = next;
        emit OwnerTransferred(prev, next);
    }
...
contract Drago is ERC721, Membership {
    ...
    function setBreed(uint256 tokenId, uint256 value) public onlyMinter {
        _breeds[tokenId] = value;
    }
