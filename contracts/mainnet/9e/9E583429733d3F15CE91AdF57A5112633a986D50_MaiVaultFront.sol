...
contract MaiVaultFront is BaseSiloAction{
    ...
    function enter(address implementation, bytes memory configuration, bytes memory inputData) public override returns(uint[4] memory outputAmounts){
        ...
        uint vaultId;
        {
            ...
            (input,output,maiVault, ratioRange, position) = abi.decode(configuration, (address[4],address[4],address, uint[3],uint));
            //carry through 1,2
            if(position == 1){
            ...
            outputAmounts[position] = IERC20Decimals(output[position]).balanceOf(address(this));
            outputAmounts[position] = _takeFee(implementation, outputAmounts[position], output[position]);
        }
    ...
    function exit(address implementation, bytes memory configuration, bytes memory outputData) public override returns(uint[4] memory outputAmounts){
        ...
        if(inputAmounts[0] == 0 || inputAmounts[0] >= vault.vaultCollateral(vaultId)){//trying to remove ALL collateral
            outputAmounts[3] = 2**256 - 1;//ask next action to remove ALL stake
        }
        else{
            //check the Collateral to Debt Ratio
            IPriceFeed oracle = IPriceFeed(vault.ethPriceSource());
            uint collateralValue = ( (vault.vaultCollateral(vaultId) - inputAmounts[0]) * oracle.latestAnswer()) / (10**oracle.decimals());//in terms of collateral decimals
            collateralValue = ((10**miMatic.decimals()) * collateralValue) / (10**collateral.decimals()); //convert to miMatic decimals
            uint idealDebt = 10**miMatic.decimals() * collateralValue/ratioRange[1];
            uint debt = vault.vaultDebt(vaultId); //in terms of miMatic decimals
            if(idealDebt > debt){//want to take out more of a loan
                outputAmounts[0] = idealDebt - debt;
            ...
            else if(idealDebt < debt){//need to repay some debt
                outputAmounts[3] = debt - idealDebt;
            }
        }
    }
    ...
    function showBalances(address _silo, bytes memory _configurationData) external view override returns(ActionBalance memory){
        ...
        uint vaultId;
        address[4] memory output;
        (input,output,maiVault,,) = abi.decode(_configurationData, (address[4],address[4],address, uint[3],uint));
...
abstract contract BaseSiloAction is Ownable {
    ...
    function withdrawLimit(bytes memory _configurationData) public view virtual returns(uint,uint){
        return (0,0);
    }
