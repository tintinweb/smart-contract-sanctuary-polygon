...
contract XONE is ERC20Burnable, Operator {
    using SafeMath for uint256;

    mapping(address => bool) private _isExcludedFromFee;
    mapping(address => bool) private _isExcludedToFee;
    uint256 public taxRate;
    uint256 public taxCorrectionRate; 
    ...
    constructor(
        ...
        uint initialMint_ // Initial mint for airdrops and forming liquidity
        ) 
    ...
    // List of all additional write functions
    ...
    function getPegPrice() public view returns (uint256 pegPrice) {
        try IOracle(Oracle).consult(address(this), 1e18) returns (uint144 price) {
            ...
            revert("Failed to consult Peg price from the Oracle");
        }
    }

    function getPegUpdatedPrice() public view returns (uint256 _XONEPrice) {
        try IOracle(Oracle).twap(address(this), 1e18) returns (uint144 price) {
            return uint256(price);
        } catch {
            revert("Failed to consult Peg price from the Oracle");
        }
    ...
    function setTaxRate(uint256 _taxRate) external onlyOwner {
        require(_taxRate <= 1500, "too high"); // Max tax rate 15%
        taxRate = _taxRate;
    }
    ...
    // Override Transfer function to include taxes 
