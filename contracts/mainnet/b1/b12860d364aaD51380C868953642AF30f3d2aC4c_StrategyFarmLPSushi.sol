...
contract StrategyFarmLPSushi is StrategyFarmLP {
    ...
    function initialize(
        ...
        super.initialize(_safeFarm, _masterchef, _poolId);

        reward = IMasterChefSushi(masterchef).SUSHI();
        if (output == wbnb) {
            rewardToOutputRoute = [reward, wbnb];
        } else if (reward != output) {
            rewardToOutputRoute = [reward, wbnb, output];
        }

        _giveRewardAllowances();
    }
    ...
    function _poolDeposit(uint256 _amount) internal override {
        if (_amount > 0) {
            // deposit
            IMasterChefSushi(masterchef).deposit(poolId, _amount, address(this));
        }
        else {
            // harvest & swap reward to output
            IMasterChefSushi(masterchef).harvest(poolId, address(this));
    ...
    function _poolWithdraw(uint256 _amount) internal override {
        IMasterChefSushi(masterchef).withdraw(poolId, _amount, address(this));
    }

    function _emergencyWithdraw() internal override {
        uint256 poolBal = balanceOfPool();
        if (poolBal > 0) {
            IMasterChefSushi(masterchef).emergencyWithdraw(poolId, address(this));
        }
    }
    ...
    function _giveRewardAllowances() private {
        if (reward != address(0)
            && reward != output
            && reward != lpToken0
            && reward != lpToken1
...
// INTERNAL FUNCTIONS
    ...
    function _removeAllowances() internal override virtual {
        ...
        if (output != want)     IERC20(output).safeApprove(unirouter, 0);
        if (lpToken0 != output) IERC20(lpToken0).safeApprove(unirouter, 0);
        if (lpToken1 != output) IERC20(lpToken1).safeApprove(unirouter, 0);
    }
