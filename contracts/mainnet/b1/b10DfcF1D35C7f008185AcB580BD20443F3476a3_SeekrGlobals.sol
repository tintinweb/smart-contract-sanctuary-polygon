...
import "./interfaces/ISeekrGlobals.sol";
import "../Authorizer/interfaces/ISeekrAuthorizer.sol";
import "../Diamond/interfaces/ISeekrProxyDiamondConfiguration.sol";

/// @title SeekrGlobals
/// @author Cory Cherven (@Animalmix55)
/// @notice Manages the globals shared across the Seekr application
/// @dev Should be behind a transparent proxy
contract SeekrGlobals is Initializable, ISeekrGlobals {
    string private _tokenSymbol;

    ISeekrAuthorizer private _authorizer;

    address private _projectImplementation;

    ISeekrProxyDiamondConfiguration private _projectDiamondConfig;

    address private _companyImplementation;

    ISeekrProxyDiamondConfiguration private _companyDiamondConfig;

    address private _seekrHub;

    function initialize(
        string calldata tokenSymbol,
        address authorizer,
        address projectImplementation,
        address companyImplementation
    ) public initializer {
        _setTokenSymbol(tokenSymbol);
        _setAuthorizer(authorizer);
        _setProjectImplementation(projectImplementation);
        _setCompanyImplementation(companyImplementation);

        // TO BE SET LATER: hub address, project diamond configuration, company diamond configuration
    }
    ...
    function getAuthorizer() external view override returns (ISeekrAuthorizer) {
        return _authorizer;
    }
    ...
    function setCompanyDiamondConfiguration(
        address configAddress
    ) external override adminOnly(msg.sender) {
        _setCompanyDiamondConfiguration(configAddress);
    }

    function getCompanyDiamondConfiguration()
        external
        view
        override
        returns (ISeekrProxyDiamondConfiguration)
    {
        return _companyDiamondConfig;
    }

    function getCompanyImplementation()
        external
        view
        override
        returns (address)
    {
        return _companyImplementation;
    }

    function setCompanyImplementation(
        address companyImplementationAddress
    ) external override adminOnly(msg.sender) {
        _setCompanyImplementation(companyImplementationAddress);
    }

    function setProjectDiamondConfiguration(
        address configAddress
    ) external override adminOnly(msg.sender) {
        _setProjectDiamondConfiguration(configAddress);
    }

    function getProjectDiamondConfiguration()
        external
        view
        override
        returns (ISeekrProxyDiamondConfiguration)
    {
        return _projectDiamondConfig;
    }
    ...
    function setProjectImplementation(
        address projectImplementationAddress
    ) external override adminOnly(msg.sender) {
        _setProjectImplementation(projectImplementationAddress);
    }

    function getSeekrHub() external view override returns (address) {
        return _seekrHub;
    }

    function setSeekrHub(
        address seekrHubAddress
    ) external override adminOnly(msg.sender) {
        // no internal setter as the constructor doesn't need to use this
        _seekrHub = seekrHubAddress;
    }

    // ------------------------------ INTERNAL -----------------------------

    function _setTokenSymbol(string calldata symbol) internal {
        _tokenSymbol = symbol;
    }

    function _setAuthorizer(address authorizerAddress) internal {
        _authorizer = ISeekrAuthorizer(authorizerAddress);
    }

    function _setProjectDiamondConfiguration(address configAddress) internal {
        _projectDiamondConfig = ISeekrProxyDiamondConfiguration(configAddress);
    }


    function _setProjectImplementation(address piAddress) internal {
        _projectImplementation = piAddress;
    }

    function _setCompanyDiamondConfiguration(address configAddress) internal {
        _companyDiamondConfig = ISeekrProxyDiamondConfiguration(configAddress);
    }

    function _setCompanyImplementation(address ciAddress) internal {
        _companyImplementation = ciAddress;
    }
    ...
    modifier adminOnly(address user) {
        require(
            _authorizer.hasRole(_authorizer.SUPER_ADMIN_ROLE(), user),
            "Not admin"
        );
        _;
    }
