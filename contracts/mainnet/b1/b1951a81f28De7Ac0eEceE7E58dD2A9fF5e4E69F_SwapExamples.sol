...
contract SwapExamples is Ownable {
    ...
    function swapNativeAssetForTokens() external payable {
        WMATIC(WMATIC_ADDRESS).deposit{value: msg.value}();
        emit LogDeposit(msg.value);

        // uint256 halfMaticAmount = msg.value / 2;

        WMATIC(WMATIC_ADDRESS).approve(address(swapRouter), msg.value);
        ...
        emit LogSwapExecution(msg.value, token1Out, WMATIC_ADDRESS, USDC);
