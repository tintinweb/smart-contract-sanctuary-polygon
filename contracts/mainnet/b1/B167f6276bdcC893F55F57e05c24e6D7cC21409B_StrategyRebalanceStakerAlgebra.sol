...
interface IControllerV7 {
  ...
  function setAddress(uint8 name, address _address) external;
...
contract StrategyRebalanceStakerAlgebra is Initializable, OwnableUpgradeable {
  ...
  function determineTicks() public view returns (int24, int24) {
    ...
    return algebraCalculator.determineTicksCalc(_cumulativeTicks, tickSpacing, tickRangeMultiplier, twapTime);
  }
  ...
  function _lpUnderlyingAmounts(uint128 liquidity_) internal view returns (uint256 amount0_, uint256 amount1_) {
    (uint160 sqrtRatioX96, , , , , , ) = pool.globalState();
    (int24 tl, int24 tu, , , ) = lpPos();
    (amount0_, amount1_) = algebraCalculator.amountsForLiquidity(sqrtRatioX96, liquidity_, (tl), (tu));
  }
  ...
  {
    if (tokenId > 0)
      (, , , , tickLower_, tickUpper_, liquidity_, , , tokensOwed0_, tokensOwed1_) = nftManager.positions(tokenId);
  }
