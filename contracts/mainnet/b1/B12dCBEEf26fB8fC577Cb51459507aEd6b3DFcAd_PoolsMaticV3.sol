...
contract PoolsMaticV3 is Ownable, ReentrancyGuard {
    ...
    function tokenPrice(address token) public view returns (uint[] memory amounts){
        ...
        path[1] = wBnb;
        path[2] = usd;
        amounts = IPancakeRouter(pancakeRouter).getAmountsOut(1 ether, path);
        if(getChainID() == 137) amounts[2] = amounts[2] * 10**12;
    }
    function tokenPriceWithBNBPair(address token) public view returns (uint[] memory amounts){
        address[] memory path = new address[](2);
        path[0] = token;
        path[1] = usd;
        amounts = IPancakeRouter(pancakeRouter).getAmountsIn(1 ether, path);
    ...
    function currentRewardTREND(address user) public view returns(uint) {
        ...
        uint trendPrice = tokenPrice(TREND)[2];
        uint radix = trendPrice >= 1 ether ? 1 ether * 10000 / trendPrice : 10000;
        uint rw = currentSpeed * u.remain * radix / 100 / multiTimeInterest / 10000;
        if(rw > u.remain) rw = u.remain;
        return rw;
    }

    function claimRewardTREND() external nonReentrant {
        Mine memory u = usersMine[_msgSender()];
        uint reward = currentRewardTREND(_msgSender());
        if(reward > u.remain) reward = u.remain;
        if(reward > 0) {
            IERC20(TREND).transfer(_msgSender(), reward);
            userClaimMined[_msgSender()].push(Claim(getDays(), reward, u.remain, u.mineSpeed + u.mineSpeedLevel));
            u.claimed += reward;
            u.remain -= reward;
        }
    }
