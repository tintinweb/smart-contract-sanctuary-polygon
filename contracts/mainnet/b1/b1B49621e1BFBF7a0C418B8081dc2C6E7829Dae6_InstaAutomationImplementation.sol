...
contract InstaAutomationImplementation is Helpers {
    ...
    function executeAutomation(
        ...
        uint256 route_,
        Swap memory swap_
    ) external onlyDSA(user_) onlyOwner {
        Automation memory currentUserMap = automationMap[userMap[user_]];
        ...
        require(
            cast(AccountInterface(user_), spells),
            "automation execution failed"
        );

        uint256 currentHf = _getHealthFactor(user_);
...
contract Helpers is Events {
    ...
    struct BuildSpellsVariables {
        bool isSameToken;
        uint256 id;
        uint256 swapSetID;
        uint256 withdrawSetId;
        uint256 paybackGetId;
    ...
    function _buildSpell(
        ...
        uint256 route_,
        Swap memory swap_
    ) internal pure returns (Spell memory spells) {

        BuildSpellsVariables memory buildSpellsVariables;
        uint8 ctr;

        buildSpellsVariables.isSameToken = collateralToken_ == debtToken_;

        buildSpellsVariables.id = 87562384628;
        buildSpellsVariables.swapSetID = buildSpellsVariables.isSameToken ? 0 : buildSpellsVariables.id;
        buildSpellsVariables.withdrawSetId = buildSpellsVariables.isSameToken ? buildSpellsVariables.id :  0;
        buildSpellsVariables.paybackGetId = 
            loanAmtWithFee_ > 0 && buildSpellsVariables.isSameToken ? 0 : buildSpellsVariables.id;

        buildSpellsVariables.spellsLength = 0;

        if (loanAmtWithFee_ > 0) {
            /**
             ...
             * swap: swap the collateral token into the debt token (skip if sellToken === buyToken)
            ...
            buildSpellsVariables.spellsLength = (buildSpellsVariables.isSameToken) ? 3 : 4;
    
            (flashloanSpell._targets, flashloanSpell._datas) = (
                new string[](buildSpellsVariables.spellsLength),
                new bytes[](buildSpellsVariables.spellsLength)
            );

            (spells._targets, spells._datas) = (new string[](1), new bytes[](1));

            if (!buildSpellsVariables.isSameToken) {
                (flashloanSpell._targets[ctr], flashloanSpell._datas[ctr++]) = (
                    "1INCH-A",
                    abi.encodeWithSignature(
                        ...
                        swap_.unitAmt,
                        swap_.callData,
                        buildSpellsVariables.swapSetID
                    )
                );
            }
            ...
            (flashloanSpell._targets[ctr], flashloanSpell._datas[ctr++]) = (
                ...
                abi.encodeWithSignature(
                    ...
                    loanAmtWithFee_, // the amount of collateral token to withdraw
                    0,
                    0
                )
            );
            ...
            buildSpellsVariables.spellsLength = (buildSpellsVariables.isSameToken) ? 2 : 3;
             ...
             */
            (spells._targets[ctr], spells._datas[ctr++]) = (
                "AAVE-V3-A",
                abi.encodeWithSignature(
                    "withdraw(address,uint256,uint256,uint256)",
                    collateralToken_, // collateral token to withdraw
                    collateralAmount_, // amount to withdraw
                    0,
                    buildSpellsVariables.withdrawSetId
                )
            );
