...
// File: contracts/lib/IMintAndBurnable.sol
...
// File: contracts/OperatorWallet.sol
    ...
    PausableUpgradeable
{
    ...
    function initialize(address initialOwner, address initialOperator)
        external
        initializer
    {
        __Pausable_init();
    ...
    function __ContractWallet_init(
    ...
    function __ContractWallet_init_unchained(
        address initialOwner,
        address initialOperator
    ) internal {
        require(
            initialOwner != address(0),
            "OperatorWallet: owner is a zero address"
        );
        require(
            initialOperator != address(0),
            "OperatorWallet: operator is a zero address"
        );
    ...
    modifier onlyOwner() {
        require(
            _owner == msg.sender,
            "OperatorWallet: caller is not the owner"
        );
        _;
    }
    ...
    modifier onlyOperatorAndOwner() {
        require(
            _operator == msg.sender || _owner == msg.sender,
            "OperatorWallet: caller is not operator or owner"
        );
        _;
    }
        ...
        onlyOwner
    {
        require(
            newOwner != address(0),
            "OperatorWallet: new owner is a zero address"
        );
        require(
            newOwner != _owner,
            "OperatorWallet: new owner is the same as the current owner"
        );
    ...
    function acceptOwnership() external whenNotPaused {
        require(
            ...
            "OperatorWallet: no authorization"
        );
    ...
    function addHotWallet(address hotwallet) external whenNotPaused onlyOwner {
        require(
            hotwallet != address(0),
            "OperatorWallet: hotwallet is a zero address"
        );
        require(
            hotWalletsMap[hotwallet] == 0,
            "OperatorWallet: hotwallet already exist"
        );
        require(
            hotwallet != address(this),
            "OperatorWallet: operatorWallet itself can not be a hotwallet"
        );
    ...
    function removeHotWallet(address hotwallet)
        external
        whenNotPaused
        onlyOwner
    {
        ...
        require(
            hotWalletsMap[hotwallet] > 0,
            "OperatorWallet: hotwallet do not exist"
        );
        ...
        onlyOwner
    {
        ...
        require(
            ...
            "OperatorWallet: already an operator"
        );
    ...
    function mint(
        ...
        address recipient,
        uint256 amount
    ) external whenNotPaused onlyOperatorAndOwner nonReentrant {
        require(
            isHotWallet(recipient) || address(this) == recipient,
            "OperatorWallet: recipient is not a hot wallet or self"
        );
    ...
    function transfer(
        ...
        require(
            isHotWallet(recipient),
            "OperatorWallet: recipient is not a hotwallet"
        );
        IERC20Upgradeable(token).safeTransfer(recipient, amount);
    }
