...
contract ERC721WhitelistSale is ERC721BaseSale, __with_semver(uint8(2), uint8(0), uint8(0)) {
    ...
    function initialize(
        ...
        setTimeOfSale(options.start, options.end);
        setTierPerWalletLimits(options.tierValues, options.tierLimits);
        _setMerkleTree(options.merkleRoot, options.merkleProofIpfsHash);
        setTiersActive(options.tierValues, options.tiersActive);
    }

    function setMerkleTree(bytes32 merkleRoot, bytes memory merkleProofIpfsHash_) external onlySalesController {
        _setMerkleTree(merkleRoot, merkleProofIpfsHash_);
    }

    function buy(bytes32[] calldata proof, bytes32 tierValue) external payable saleTime {
        ...
        address erc721 = ISalesController(salesController).mint(_msgSender(), tierValue);
        _afterBuy(_msgSender(), erc721, tierValue, cost, address(0), "Whitelist", __semver());
    }
...
import "../App.sol";
import "../../libraries/Semver.sol";
import "../ERC721Sale/ERC721BaseSale.sol";
import "../../kernel/interfaces/IKernel.sol";
import "../../libraries/AppsIds.sol";
import "../AppProxy.sol";
...
import "./ISalesController.sol";
import "../../updateManager/IUpdateManager.sol";

contract SalesController is ISalesController, App, __with_semver(uint8(1), uint8(0), uint8(0)) {
    uint8 public immutable CONTROLLER = _initNextRole();

    mapping(SaleType => UpgradeableBeacon) internal beacons;

    using EnumerableMap for EnumerableMap.AddressToUintMap;
    EnumerableMap.AddressToUintMap private _sales;

    event AddSale(uint256 index, address app, SaleType saleType);

    modifier onlyApp() {
        require(_sales.contains(_msgSender()), Exceptions.ADMIN_SALES_CONTROLLER_ERROR);
        _;
    }
    ...
    function initialize(IKernel _kernel) external initializer {
        __App_init(_kernel);
        beacons[SaleType.Open] = UpgradeableBeacon(IUpdateManager(kernel.getUpdateManager()).getBeacon(AppsIds.ERC721_OPEN_SALE));
        beacons[SaleType.Whitelist] = UpgradeableBeacon(IUpdateManager(kernel.getUpdateManager()).getBeacon(AppsIds.ERC721_WHITELIST_SALE));
    }

    function deploySale(bytes calldata options, SaleType saleType) external requirePermission(CONTROLLER) {
        address sale = address(new BeaconProxy(address(beacons[saleType]), ""));
        ERC721BaseSale(sale).initialize(address(kernel), address(this), options);

        _sales.set(sale, uint256(saleType));

        emit AddSale(_sales.length() - 1, address(sale), saleType);
    }

    function mint(address to, bytes32 tierValue) external onlyApp returns (address erc721) {
        erc721 = kernel.getAppAddress(AppsIds.ERC721);
        ERC721Properties(erc721).mint(to, tierValue);
    }

    function executeSingle(address app, bytes memory data) public requirePermission(CONTROLLER) returns (bytes memory) {
        require(_sales.contains(app), Exceptions.ADMIN_SALES_CONTROLLER_ERROR);
        return Address.functionCall(app, abi.encodePacked(data), Exceptions.ADMIN_SALES_CONTROLLER_ERROR);
    }

    function executeBatch(address[] calldata apps, bytes[] calldata data)
        external
        requirePermission(CONTROLLER)
        returns (bytes[] memory returnData)
    {
        ...
        for (uint256 i = 0; i < data.length; i++) {
            returnData[i] = executeSingle(apps[i], data[i]);
        }
    }

    function getSaleType(address sale) public view returns (SaleType) {
        (bool contains, uint256 value) = _sales.tryGet(sale);
        if(contains){
    ...
    function getTreasury() external view returns (address) {
        return kernel.getTreasury();
    }

    function getSale(uint256 index) public view returns (Sale memory) {
        (address app, uint256 saleType) = _sales.at(index);
        return Sale(SaleType(saleType), app);
    }

    function getAllSales() public view returns (Sale[] memory) {
        uint256 length = _sales.length();
        Sale[] memory result = new Sale[](length);
        for(uint i=0; i<length; i++){
            (address app, uint256 saleType) = _sales.at(i);
            result[i] = Sale(SaleType(saleType), app);
        }
        return result;
    }
    // TODO: remove after migration
    function addSale(address sale, uint8 saleType) external {
        require(msg.sender==address(kernel));
        _sales.set(sale, saleType);
    }
}
...
import "@openzeppelin/contracts/utils/StorageSlot.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "../apps/SalesController/SalesController.sol";
...
contract Kernel is Initializable, AppManager, IKernel, __with_semver(uint8(1), uint8(1), uint8(0)) {
    ...
    //TODO remove after migration
    function migrateSales() external requirePermission(KERNEL_ADMIN){

        address beacon = _updateManager.getBeacon(AppsIds.SALES_CONTROLLER);
        address salesController = address(new AppBeaconProxy(beacon, abi.encodeCall(SalesController.initialize, (Kernel(this)))));
        _setApp(AppsIds.SALES_CONTROLLER, salesController, true);

        _addPermission(
            AppsIds.SUDO,
            AppsIds.SALES_CONTROLLER,
            SalesController(salesController).CONTROLLER()
        );
        _addPermission(
        ...
        address openSale = _appInfo[AppsIds.ERC721_OPEN_SALE].addr;
        if(openSale!=address(0)){
            SalesController(salesController).addSale(openSale, 1);
        }

        address whiteList = _appInfo[AppsIds.ERC721_WHITELIST_SALE].addr;
        if(whiteList!=address(0)){
            SalesController(salesController).addSale(whiteList, 2);
        }
...
contract DAOConstructor is
    ...
    OwnableUpgradeable,
    IDAOConstructor,
    __with_semver(uint8(1), uint8(5), uint8(0))
{
    ...
    function deploy(
        ...
        _deployERC721(kernel, deploymentSettings.nftSettings);

        _deploySalesController(kernel);
    ...
    function _deploySalesController(Kernel kernel) internal {
        SalesController salesController = SalesController(
            kernel.deployApp(
                AppsIds.SALES_CONTROLLER,
                abi.encodeCall(SalesController.initialize, (kernel))
            )
        );
