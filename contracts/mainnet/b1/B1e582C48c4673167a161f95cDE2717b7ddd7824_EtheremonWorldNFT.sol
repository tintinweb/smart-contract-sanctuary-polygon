...
contract EtheremonWorldNFT is BasicAccessControl {
    ...
    mapping(uint32 => uint8) public STAT_MAX_CLASS;
    ...
    function mintMonster(
        ...
        uint8 maxStat = (STAT_MAX_CLASS[_classId] > 0)
        ...
        for (uint256 i = 0; i < STAT_COUNT; i += 1) {
            ...
            rand = uint8(seed % maxStat);
            minStat = data.getElementInArrayType(
                    EthermonEnum.ArrayType.STAT_START,
                    classId64,
                    i
                );
            // formula to find a number between min and max:
            value = rand % ((minStat+maxStat) - minStat + 1) + minStat;
    ...
    function catchMonsterNFTemon(
        ...
        for (uint256 i = 0; i < STAT_COUNT; i += 1) {
            ...
            // formula to find a number between find min and max with rand:
            value = uint8(seed % maxStat) % ((minStat+maxStat) - minStat + 1) + minStat;
