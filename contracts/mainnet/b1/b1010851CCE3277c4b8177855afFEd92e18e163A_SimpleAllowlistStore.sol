...
contract SimpleAllowlistStore is AccessControl {
  ...
  // Mapping from Id to Allowlist
  mapping(uint => Allowlist) private lists;
  ...
  error IndexOutOfBound(uint id, uint index);
  error HandleMinted(uint id, string handle);
  error AccountMinted(uint id, address account);
  error RegistryIdInvalid(uint id);
  error TransferFailed(uint id);
  error CapExceeded(uint id);

  event Purchased(uint indexed id, address indexed owner, uint indexed registryId, uint supply);
  event Registered(uint indexed id, address indexed owner, string indexed handle);

  constructor(address _coin, address _registry) {
    coin = CozyCoin(_coin);
  ...
  function getUsers(uint id) external view exists(id) returns (User[] memory) {
    return lists[id].users;
  }
  ...
  function getUsersBatch(
    ...
    uint index,
    uint size
  ) external view exists(id) returns (User[] memory) {
    Allowlist storage list = lists[id];

    if (index >= list.users.length) {
      revert IndexOutOfBound(id, index);
    }

    uint remaining = list.users.length - index;
    uint total = remaining < size ? remaining : size;

    User[] memory result = new User[](total);

    for (uint i = 0; i < total; i++) {
      result[i] = list.users[i + index];
    }

    return result;
  }

  function getList(uint id) external view exists(id) returns (AllowlistView memory) {
    Allowlist storage list = lists[id];
    return
      AllowlistView(list.id, list.price, list.users.length, list.cap, list.locked, list.exists);
  }

  function getLists(uint[] calldata ids) external view returns (AllowlistView[] memory) {
    AllowlistView[] memory views = new AllowlistView[](ids.length);

    for (uint i = 0; i < ids.length; i++) {
      Allowlist storage list = lists[ids[i]];
      views[i] = AllowlistView(
        list.id,
        list.price,
        list.users.length,
        list.cap,
  ...
  function hasAccountBought(uint id) external view exists(id) returns (bool) {
    return lists[id].accounts[msg.sender];
  }

  function hasAccountBoughtAny(uint[] memory ids) external view returns (bool[] memory) {
    bool[] memory purchases = new bool[](ids.length);

    for (uint i = 0; i < ids.length; i++) {
      purchases[i] = lists[ids[i]].accounts[msg.sender];
    }

    return purchases;
  }

  function hasHandleBought(uint id, string memory handle) external view exists(id) returns (bool) {
    return lists[id].handles[handle];
  }

  function hasHandleBoughtAny(uint[] memory ids, string memory handle)
    external
    view
    returns (bool[] memory)
  {
    ...
    for (uint i = 0; i < ids.length; i++) {
      purchases[i] = lists[ids[i]].handles[handle];
    }
  ...
  ) external exists(id) notLocked(id) {
    Allowlist storage list = lists[id];

    if (msg.sender != registry.ownerOf(registryId)) {
      revert RegistryIdInvalid(id);
    }

    if (list.accounts[msg.sender]) {
      revert AccountMinted(id, msg.sender);
    }

    if (list.handles[handle]) {
      revert HandleMinted(id, handle);
    }

    if (list.users.length >= list.cap) {
      revert CapExceeded(id);
    }

    User memory user = User(msg.sender, handle);
    list.users.push(user);
    list.accounts[msg.sender] = true;
    list.handles[handle] = true;

    bool sent = coin.transferFrom(msg.sender, address(this), list.price);

    if (!sent) {
      revert TransferFailed(id);
    }

    emit Purchased(id, msg.sender, registryId, list.users.length);
    emit Registered(id, msg.sender, handle);
  }

  // ----------- Operator --------

  function setPrice(uint id, uint _price)
    external
    onlyRole(OPERATOR_ROLE)
    exists(id)
    notLocked(id)
  {
    lists[id].price = _price;
  }

  function setCap(uint id, uint _cap) external onlyRole(OPERATOR_ROLE) exists(id) notLocked(id) {
    if (lists[id].users.length > _cap) {
      revert CapExceeded(id);
    }
  ...
  function lock(uint id) external onlyRole(OPERATOR_ROLE) {
    lists[id].locked = true;
  }

  function burn() external onlyRole(OPERATOR_ROLE) {
