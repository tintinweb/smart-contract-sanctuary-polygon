 ...
 * behavior.
 */
 contract RajuPresale is Owned{
    using SafeMath for uint256;
    using SafeERC20 for ERC20;

    // unix timestamp datas
    uint256 public closingTime; // time once the presale will close
    ...
    uint256 public minUsdcLimit;
    uint256 public maxUsdcLimit;
    uint256 public minpTokenLimit;
    uint256 public maxpTokenLimit;

    uint256 public startTime1;
    uint256 public startTime2;
    uint256 public startTime3;
    uint256 public period1;
    uint256 public period2;
    uint256 public period3;
    uint256 public rebase1;
    uint256 public rebase2;
    uint256 public rebase3;
    ...
    event SetRebase(uint rebase);
    event SetStartTime(uint starttime);
    event SetPeriod(uint period);
    event SetDailyRate(uint dailyRate);
    event SetUsdcLimit(uint limit);
    event SetpTokenLimit(uint limit);

    constructor(uint256 _rate1, uint256 _rate2, address _wallet, ERC20 _usdc, ERC20 _pToken, uint _closingTime) {
        require(_rate1 > 0);
        require(_rate2 > 0);
    ...
    function buy(uint _amount) public {
        ...
        require(_amount >= minUsdcLimit, "You need to buy at least some min amount");
        require(_amount < maxUsdcLimit, "Selected amount exceeds maxLimit.");
        require(_amount <= usdc.balanceOf(msg.sender), "There is no enough USDC in your wallet.");
        require(paidTotal[msg.sender] + _amount <= maxUsdcLimit);
        require((block.timestamp >= startTime1 && block.timestamp < (startTime1 + period1)) || (block.timestamp >= startTime2 && block.timestamp < (startTime2 + period2)), "Presale has not yet opened or is already closed.");


        // calculate pToken amount to be created
        uint256 pTokenAmount;
        if(block.timestamp >= startTime1 && block.timestamp < (startTime1 + period1))
        {
            pTokenAmount = _amount.mul(RATE_DECIMALS).div(rate1).mul(10 ** 12);
        }
        else if(block.timestamp >= startTime2 && block.timestamp < (startTime2 + period2))
        {
    ...
    function sell(uint _amount) public {
        require(isList == false || whiteListed[msg.sender] == true);
        require(_amount >= minpTokenLimit, "You need to sell at least some min amount");
        require(_amount < maxpTokenLimit, "Selected amount exceeds maxLimit.");
        require(_amount <= pToken.balanceOf(msg.sender), "There is no enough pToken in your wallet.");
        require((block.timestamp >= startTime2 && block.timestamp < (startTime2 + period2)), "You can't sell at this step.");

        // calculate pToken amount to be created
        uint256 usdcAmount;
        if(block.timestamp >= startTime2 && block.timestamp < (startTime2 + period2))
        {
    ...
    function getPercentReleased() public view returns (uint released) {
        ...
        if (block.timestamp <= startTime3) {
        return 30;
        } 
        else if (block.timestamp > startTime3) { // already 100% released
            released = 30 + dailyRate.mul(block.timestamp - startTime3).div(86400);
            if (released >= 100) 
                return 100;
            return released;
        } 
    ...
    function getRebaseRate(uint time) public view returns (uint rebaseRate) {
        if((time >= startTime1) && (time < startTime1 + period1)) {
            return rebase1;
        } else if((time >= startTime2) && (time < startTime2 + period2)) {
            return rebase2;
        } else if((time >= startTime3) && (time < startTime3 + period3)) {
            return rebase3;
        } else {
            return 1;
        }
    ...
    function setstartTime1(uint256 _startTime1) external onlyOwner {
        require(_startTime1 >= block.timestamp);
        startTime1 = _startTime1;
        emit SetStartTime(_startTime1);
    }

    function setstartTime2(uint256 _startTime2) external onlyOwner {
        require(_startTime2 >= block.timestamp);
        startTime2 = _startTime2;
        emit SetStartTime(_startTime2);
    }

    function setstartTime3(uint256 _startTime3) external onlyOwner {
        require(_startTime3 >= block.timestamp, "Starttime3 should not less than current time.");
        startTime3 = _startTime3;
        emit SetStartTime(_startTime3);
    }

    function setPeriod1(uint256 _period1) external onlyOwner {
        require(block.timestamp < startTime1);
        period1 = _period1;
        emit SetPeriod(_period1);
    }

    function setPeriod2(uint256 _period2) external onlyOwner {
        require(block.timestamp < startTime2);
        period2 = _period2;
        emit SetPeriod(_period2);
    }

    function setPeriod3(uint256 _period3) external onlyOwner {
        require(block.timestamp < startTime3);
        period3 = _period3;
        emit SetPeriod(_period3);
    }

    function setRebase1(uint256 _rebase1) external onlyOwner {
        require(block.timestamp < startTime1);
        rebase1 = _rebase1;
        emit SetRebase(_rebase1);
    }

    function setRebase2(uint256 _rebase2) external onlyOwner {
        require(block.timestamp < startTime2);
        rebase2 = _rebase2;
        emit SetRebase(_rebase2);
    }

    function setRebase3(uint256 _rebase3) external onlyOwner {
        require(block.timestamp < startTime3);
        rebase3 = _rebase3;
        emit SetRebase(_rebase3);
    }

    function setDailyRate(uint256 _dailyRate) external onlyOwner {
        require(startTime1 > block.timestamp, "Presale has already started.");
    ...
    function setMinUsdc(uint256 _amount) external onlyOwner {
        require(_amount > 0);
        minUsdcLimit = _amount;
        emit SetUsdcLimit(_amount);
    }

    function setMaxUsdc(uint256 _amount) external onlyOwner {
        require(_amount > 0);
        maxUsdcLimit = _amount;
        emit SetUsdcLimit(_amount);
    }

    function setMinpToken(uint256 _amount) external onlyOwner {
        require(_amount > 0);
        minpTokenLimit = _amount;
        emit SetpTokenLimit(_amount);
    }

    function setMaxpToken(uint256 _amount) external onlyOwner {
        require(_amount > 0);
        maxpTokenLimit = _amount;
        emit SetpTokenLimit(_amount);
    }
}