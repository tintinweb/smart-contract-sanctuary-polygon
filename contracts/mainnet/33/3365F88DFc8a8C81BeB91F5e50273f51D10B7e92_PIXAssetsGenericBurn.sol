...
contract PIXAssetsGenericBurn is IGenericBurn, OwnableUpgradeable {
    ...
    mapping(uint256 => uint256[]) public s_tokenCategoryIds;
    ...
    mapping(uint256 => ContentCategory[]) public s_tokenCategories;
    ...
    mapping(uint256 => mapping(uint256 => uint256[])) largeContentWeights;
    mapping(address => uint256) userAvatarSeed;
    mapping(address => RequestInputs) userVRFRequestParams;
    address avatarPack;

    mapping(uint256 => uint256) tokenBurnLimit;
    ...
    function setContentLarge(
        ...
        largeContentWeights[_tokenId][index].push(_weights);

        s_tokenCategories[_tokenId][index].tokens.push(_tokens);
        s_tokenCategories[_tokenId][index].tokenIds.push(_tokenIds);
        s_tokenCategories[_tokenId][index].tokenAmounts.push(_amounts);

        // emit ContentsUpdated(_tokenId, _categoryId, _tokens, _tokenIds, _amounts, _weights);
    }

    function setContentLargeWeights(uint256 _tokenId, uint256 _categoryId) external onlyGov {
        if (!s_tokenCategoryActive[_tokenId][_categoryId])
            revert GB__InvalidCategoryId(_categoryId);
        uint256 index = s_tokenCategoryIndex[_tokenId][_categoryId];

        uint256 sum;
        for (uint256 i = 0; i < largeContentWeights[_tokenId][index].length; i++) {
            sum += largeContentWeights[_tokenId][index][i];
        }

        s_tokenCategories[_tokenId][index].tokenWeights = _arrayToCumulative(
            largeContentWeights[_tokenId][index]
        );
        s_tokenCategories[_tokenId][index].contentsTotalWeight = sum;
    }
    ...
    function burnPack(
        ...
        require(_amount <= tokenBurnLimit[_tokenId], "PIX BURN: BURN_LIMIT_EXCEEDED");
    ...
    function claimAvatar() external {
        ...
        uint256 contentAmountsTotalWeight = s_tokenCategories[req.tokenId][0]
            .contentAmountsTotalWeight;
        uint256 contentsTotalWeight = s_tokenCategories[req.tokenId][0].contentsTotalWeight;

        uint256[] memory contentAmounts = s_tokenCategories[req.tokenId][0].contentAmounts;
        uint256[] memory contentAmountsWeights = s_tokenCategories[req.tokenId][0]
            .contentAmountsWeights;
        uint256[] memory tokenWeights = s_tokenCategories[req.tokenId][0].tokenWeights;
        uint256[] memory tokenIds = s_tokenCategories[req.tokenId][0].tokenIds;
