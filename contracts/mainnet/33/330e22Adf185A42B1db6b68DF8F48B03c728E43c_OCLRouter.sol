...
import "./../core/interfaces/ITokenWhitelist.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        DFX_ROUTER = _router;
        JARVIS_DEX = _jarvisDex;
        PERCENTAGE_BASED_POINT = 10000; // 100 is 1%
        tokenWhitelist = _tokenWhitelist;
    }

    // Approve and Sell with DFX or Jarvis
    function sellPropertyWithPermit(
        ...
        require(tryB != _swapArgs.to, "Same token");
        require(
            ITokenWhitelist(tokenWhitelist).isOnWhitelist(_swapArgs.to),
            "Token not supported"
        );
    ...
    // Approve and Sell on Marketplace with Native Asset
    ...
    function swapOnDfx(
        ...
        require(
            ITokenWhitelist(tokenWhitelist).isOnWhitelist(_swapArgs._origin),
            "Token not supported"
        );
        require(
            ITokenWhitelist(tokenWhitelist).isOnWhitelist(_swapArgs._target),
            "Token not supported"
        );
        ...
        return _dfxSwapHelper(_swapArgs);
    }

    function _dfxSwapHelper(
        DexSwapArgs memory _swapArgs
    ) private returns (uint swappedAmount) {
    ...
    function approveAndBuyPropertyWithMetaTx(
        ...
        require(
            ITokenWhitelist(tokenWhitelist).isOnWhitelist(_swapArgs.from),
            "Token not supported"
        );
        ...
        buyProperty(_swapArgs, _marketplace, _isFeeInXeq, _inputAmount);
    }
    ...
    function buyProperty(
        ...
        // now buying property
        IMarketplace(_marketplace).swap(_swapArgs, _isFeeInXeq);
    }
    ...
    function getPropertyPriceWithFees(
        ...
        uint feePercentage;
        uint propertyPrice = amountOfTokens * _property.price;
    ...
    function updateJarvisDexAddress(address _jarvisDex) external onlyAdmin {
        JARVIS_DEX = _jarvisDex;
    }
