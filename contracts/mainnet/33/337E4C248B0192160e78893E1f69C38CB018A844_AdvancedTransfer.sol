...
contract AdvancedTransfer is BaseSiloAction{
    ...
    function enter(address implementation, bytes memory configuration, bytes memory inputData) public override returns(uint[4] memory outputAmounts){
...

        {
            ...
            address[] memory path;
            address oracle;
            (input, output, ratio,recipients,path, oracle) = abi.decode(configuration, (address[4],address[4],uint[4],address[4],address[],address));
            info = ActionInfo({
                 inputs: input,
                 outputs: output,
        ...
        
        {
            ...
            for(uint i=0; i<4; i++){
                ...
                if(info.ratio[i] == 0){continue;}//skip
                if(info.recipients[i] == address(0)){//compounding
                    potentialSilo = address(this);
                }
                else if(IAction(implementation).getIsSilo(info.recipients[i])){//sending to a silo
                ...
                if(potentialSilo == address(0)){
                    ...
                    for(uint j=0; j<4; j++){
                        if(siloTokens[j] == info.inputs[0]){//see if receiving silo can accept this reward token
                            noConvert = true;
                            break;
                        }
                    }
        ...
        if(info.beta > 0){
            Quote memory best;
            uint amount = inputAmounts[0] * info.beta / baseDecimals;
            for(uint i=0; i<(info.path.length-1); i++){
                path[0] = info.path[i];
                path[1] = info.path[i+1];
                best = info.Oracle.findOptimalSwap(info.path[i], info.path[i+1], amount);
                if(best.amountOut > 0){
                    SafeERC20.safeApprove(IERC20(info.path[i]), best.router, 0);
                    SafeERC20.safeApprove(IERC20(info.path[i]), best.router, amount);
                    amounts = IUniswapV2Router02(best.router).swapExactTokensForTokens(
                            amount,
                            best.amountOut,
                            path,
                            address(this),
            ...
            amounts[0] = inputAmounts[0] - (inputAmounts[0] * info.beta / baseDecimals);//remaining input[0]
            amounts[1] = amount;//converted token
        }
        else{
            amounts[0] = inputAmounts[0];
        }
        for(uint i=0; i<4; i++){//now set outputAmounts
            if(info.ratio[i] == 0){continue;}
            if(info.outputs[i] == info.inputs[0]){//use amounts[0] and alpha
                outputAmounts[i] = amounts[0] * info.ratio[i] / info.alpha; 
            }
            else{//use amounts[1] and beta
                outputAmounts[i] = amounts[1] * info.ratio[i] / info.beta;
            }
        }
        for(uint i=0; i<4; i++){
            if(info.recipients[i] != address(0) && info.recipients[i] != address(this) && outputAmounts[i] > 0){
                SafeERC20.safeTransfer(IERC20(info.outputs[i]), info.recipients[i], outputAmounts[i]);
            }
        }
    }
    ...
    function createConfig(address[4] memory _inputs, address[4] memory _outputs, uint[4] memory _ratio, address[4] memory _recipients, address[] memory _path, address _oracle) public pure returns(bytes memory configData){
        configData = abi.encode(_inputs, _outputs, _ratio, _recipients, _path, _oracle);
    }

    function decodeConfig(bytes memory configData) public pure returns(address[4] memory inputs, address[4] memory outputs, uint[4] memory ratio, address[4] memory recipients, address[] memory path, address oracle){
        (inputs,  outputs, ratio, recipients, path, oracle) = abi.decode(configData, (address[4], address[4], uint[4], address[4], address[], address));
    }
    ...
    function validateConfig(bytes memory configData) public view override returns(bool){
        ...
        address[4] memory output;
        address[4] memory recipients;
        (input, output, ratio,recipients) = abi.decode(configData, (address[4],address[4],uint[4],address[4]));
        uint sum;
        for (uint i=0; i<4; i++){
            sum += ratio[i];
        }
...
interface IExchangeRateOracle {
  ...
  function getExchanges(address tokenIn, address tokenOut) external view returns(address[] memory);
}
