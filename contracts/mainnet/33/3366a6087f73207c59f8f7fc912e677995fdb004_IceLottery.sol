...
contract IceLottery is VRFConsumerBaseV2{
    ...
    uint64 s_subscriptionId; //390 for polygon
    ...
    mapping(uint256 => mapping(uint256 => RequestStatus)) public s_requests;
        ...
        VRFConsumerBaseV2(vrfCoordinator)
    {
        ...
        changeConfig(_ticketPrice, _lotteryToken, address(0), _winnersShare, _costAndDevShare, _burningShare);
    ...
    function changeConfig(uint _ticketPrice, address _lotteryToken, address _burningAddress, uint8 _winnersShare, uint8 _costAndDevShare, uint8 _burningShare) public onlyManager{
    ...
    function requestRandomWords() private returns(uint requestId){
       requestId = COORDINATOR.requestRandomWords(
        ...
        s_requests[currentLotteryId][requestId] = RequestStatus({randomWords: new uint256[](0), exists: true, fulfilled: false});
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override{
        require(s_requests[currentLotteryId][_requestId].exists, "request not found");
        s_requests[currentLotteryId][_requestId].fulfilled = true;
        s_requests[currentLotteryId][_requestId].randomWords = _randomWords;
        emit RequestFulfilled(_requestId, _randomWords);
        distributePrize(_randomWords[0]);
    }    

    function getRequestStatus(uint256 _lotteryId, uint256 _requestId) external view returns (bool fulfilled, uint256[] memory randomWords) {
        require(s_requests[_lotteryId][_requestId].exists, "request not found");
        RequestStatus memory request = s_requests[_lotteryId][_requestId];
