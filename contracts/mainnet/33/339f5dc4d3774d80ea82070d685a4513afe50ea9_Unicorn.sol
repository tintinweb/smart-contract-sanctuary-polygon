 ...
 * is concerned).
 *
 * This contract is only required for intermediate, library-like contracts.v
 */
...
contract Unicorn is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 12000 ether;
       // Distribution for airdrops wallet
    uint256 public constant INITIAL_AIRDROP_WALLET_DISTRIBUTION = 1200 ether;

    // Have the rewards been distributed to the pools
    bool public rewardPoolDistributed = false;

    address public unicornOracle;
    ...
    mapping (address => bool) private mintOperators;        // Contracts that will be able to mint OBOL

    modifier onlyMintOperator() {
        require( mintOperators[_msgSender()] == true, "Ownable: mint operator only");
        _;
    }

    /**
     * @notice Constructs the UNICORN ERC-20 contract.
     */
    constructor() public ERC20("Fairy Finance", "UNICORN") {
        // Mints 1 UNICORN to contract creator for initial pool setup
        _mint(msg.sender, 1 ether);   
        IUniswapV2Router _dexRouter = IUniswapV2Router(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
    ...
    function _getUnicornPrice() internal view returns (uint256 _unicornPrice) {
        try IOracle(unicornOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Unicorn: failed to fetch UNICORN price from Oracle");
        }
    }

    function setUnicornOracle(address _unicornOracle) public onlyOperator {
        require(_unicornOracle != address(0), "oracle address cannot be 0 address");
        unicornOracle = _unicornOracle;
    }

    /**
     * @notice Operator mints UNICORN to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of UNICORN to mint to
