...
contract StakingFactory is AccessControl, Ownable {
    ...
    function createStakingContract(
        ...
        _addStakingContractPoolReward(address(staking));
    ...
    /**
     * @dev Function for calculate pool reward amount from args.
     * @param _poolStakeAmount - pool stake amount.
     */
    function calculatePoolRewardAmount(uint256 _poolStakeAmount, uint256 _percentage, uint256 _duration) public view returns (uint256) {
        return (_poolStakeAmount * _percentage * _duration) / 100 / 365;
    }

    /**
     * @dev Call add reward on  staking contract.
     * @param _stakingAddress - Staking contract address.
     */
    function _addStakingContractPoolReward(address _stakingAddress) internal {
        IStakingV2 staking = IStakingV2(_stakingAddress);
        staking.addPoolReward();
    }
}
...
contract StakingV2 is AccessControl, Ownable, Pausable, ReentrancyGuard {
     ...
     */
    function addPoolReward() external onlyOwner {
        uint256 maxPoolReward = calculatePoolRewardAmount(poolStakeLimit);
        stakingFactory.vaultPayWithToken(
            stakingOwner,
            tokenAddress,
            maxPoolReward
        );
        ...
        afterStakeStarted
    {
        ...
        stakingFactory.vaultTransferTokenToAddress(
            msg.sender,
            tokenAddress,
            exceedAmount
        );
    ...
    function claimReward() external nonReentrant afterStakeStarted {
        ...
        require(
            unclaimAmount > 0,
            "[StakingV2.claimReward] No claimable reward"
        );
        ...
        returns (uint256)
    {
        ...
        if (currentDay > startDay) {
            dayPassed = (currentDay - startDay) > duration
                ? duration
                : (currentDay - startDay);
        }
     ...
     * @param _poolStakeAmount - pool stake amount.
     */
    function calculatePoolRewardAmount(uint256 _poolStakeAmount)
        public
        view
        returns (uint256)
    {
...
interface IStakingV2 {
    function addPoolReward() external;
