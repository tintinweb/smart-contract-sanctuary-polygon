...
contract TidePool is ITidePool, IUniswapV3MintCallback, IUniswapV3SwapCallback, ERC20 {
    ...
    function withdraw() external override {
        ...
        if(liquidity > 0) {
            ...
            liquidity = pool.getPosition(tickLower, tickUpper);

            // user liquidity is a simple percentage of total supply
            userLiquidityShare = uint256(liquidity) * balance / total;

            // burn liquidity proportial to user's share
            (amount0, amount1) = pool.burnManagedLiquidity(userLiquidityShare.toUint128(), tickLower, tickUpper);
        }
    ...
    function rerange() external override {
        ...
        require(totalSupply() > 0 && diff > 1 days && !needsRebalance(),"1D");
        ...
        // recalculate the range with the new window size, (roughly) same ratio of assets  
    ...
    function needsRebalance() public view returns (bool) {
        (, int24 tick,,,,,) = pool.slot0();
        return totalSupply() > 0 && (tick > tickUpper || tick < tickLower);
    }
