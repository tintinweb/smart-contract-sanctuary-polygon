 ...
 */
import { SVaultInitializer } from "./SVaultInitializer.sol";
import { SVaultAdmin } from "./SVaultAdmin.sol";


contract SVault is SVaultInitializer, SVaultAdmin {}
...
contract SVaultInitializer is SVaultStorage {
    function initialize(string memory _name, string memory _symbol, address _asset)
        external
        onlyGovernor
        initializer
    {
        ...
        // Set intitial withdraw lock to 0
        withdrawLockup = 0;

        emit VaultInitialized(msg.sender, _name, _symbol, _asset);
    }
}
...
contract SVaultAdmin is SVaultStorage {
    ...
    **********************************/
    ...
    function setCXVaults(CXVault[] calldata _childXVaults) external onlyGovernor {
        ...
        for (uint8 i = 0; i < _childXVaults.length; i++) {
            CXVault memory childXVault = _childXVaults[i];
            ...
            cXVaults[childXVault.chainId] = childXVault;
        }
    }
    ...
    function _balance() internal {
        ...
        for (uint8 i; i < stratsWithWeights.length; i++) {
            ...
            if(stratsWithWeights[i].targetWeight == 0){
                ...
                totalAssetInStrat += ISingleStrategy(stratsWithWeights[i].strategy).checkBalance();
        ...
        for (uint8 i; i < stratsWithWeights.length; i++) {
            ...
            uint256 currentLiquidity = ISingleStrategy(stratsWithWeights[i].strategy).checkBalance();
...
contract SVaultStorage is ERC20Upgradeable, Governable {
    ...
    mapping(uint8 => CXVault) public cXVaults;
