...
contract AaveV3Vault is TokenizedVaultV1 {
    ...
    IAaveRewardControllerV3 public immutable rewardController;
...
abstract contract TokenizedVaultV1 is ERC4626, Ownable, ReentrancyGuard {
    ...
    /// @notice Emitted when idleBetweenCompounds changed by owner
    event IdleBetweenCompoundsSet(uint256 idleBetweenCompounds);
    ...
    function setIdleBetweenCompounds(uint256 idleBetweenCompounds_) public onlyOwner {
        idleBetweenCompounds = idleBetweenCompounds_;
        emit IdleBetweenCompoundsSet(idleBetweenCompounds_);
    }
    ...
    function _compound() internal {
        ...
        for (uint256 i = 0; i < rewardAssetsLength; i++) {
            ...
            if (rewardAsset == address(0) || rewardAsset == vaultAsset) continue;

            if (rewardAsset == address(1)) {
                uint256 rewardBalance = address(this).balance;
                if (swapper.canSwap(wrapAsset, vaultAsset, rewardBalance)) {
                    ...
                    swapper.swap(wrapAsset, vaultAsset);
                }
            } else {
                uint256 rewardBalance = IERC20(rewardAsset).balanceOf(address(this));

                if (swapper.canSwap(rewardAsset, vaultAsset, rewardBalance)) {
                    SafeERC20.safeTransfer(IERC20(rewardAsset), address(swapper), rewardBalance);
                    swapper.swap(rewardAsset, vaultAsset);
                }
            }
        }
    }
