...
import "./GiddyVault.sol";
import "./libraries/GiddyLibrary.sol";

contract SushiUsdcEthQuery is GiddyQuery, Initializable, OwnableUpgradeable {
  ...
  address constant private USDC_ETH_PAIR = 0x34965ba0ac2451A34a0471F04CCa3F990b8dea27;
  address constant private ETH_TOKEN = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619;

  GiddyVault public vault;

  function initialize(address vaultAddress) public initializer {
    __Ownable_init();
    vault = GiddyVault(vaultAddress);
  }

  function deposit(uint256 inBefore, uint256 inAfter) external override view returns (uint256 outBefore, uint256 outAfter) {
    outBefore = usdcToUsdcEthLP(inBefore);
    outAfter = usdcToUsdcEthLP((inAfter - (inAfter * 10015) / 20000) * 2);
  }
  function withdraw(uint256 inBefore, uint256 inAfter) external override view returns (uint256 outBefore, uint256 outAfter) {
    outBefore = usdcEthLPToUsdc(inBefore);
    outAfter = usdcEthLPToUsdc(inAfter);
    uint ethAmount = usdcToEth(outAfter / 2);
    (, uint usdcAfter) = GiddyLibrary.calcSwap(SUSHI_ROUTER, ETH_TOKEN, USDC_TOKEN, ethAmount, ethAmount);
    outAfter = outAfter / 2 + usdcAfter;
  }

  function usdcToUsdcEthLP(uint256 usdcAmount) private view returns (uint256) {
    (uint256 resUsdc,,) = IUniswapV2Pair(USDC_ETH_PAIR).getReserves();
    return (IUniswapV2Pair(USDC_ETH_PAIR).totalSupply() * (usdcAmount / 2)) / resUsdc;
  }

  function usdcEthLPToUsdc(uint256 usdcEthLP) private view returns (uint256) {
    (uint256 resUsdc,,) = IUniswapV2Pair(USDC_ETH_PAIR).getReserves();
    return (resUsdc * usdcEthLP * 2) / IUniswapV2Pair(USDC_ETH_PAIR).totalSupply();
  }

  function usdcToEth(uint256 amount) private view returns(uint256)
  {
    (uint256 resUsdc, uint256 resEth,) = IUniswapV2Pair(USDC_ETH_PAIR).getReserves();
    require(resUsdc > 0 && resEth > 0);
    return (amount * resEth) / resUsdc;
  }
}
...
import "./GiddyStrategy.sol";
import "./GiddyQuery.sol";
import "./GiddyConfigYA.sol";
import "./libraries/GiddyLibrary.sol";

contract GiddyVault is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  GiddyStrategy public strategy;
  GiddyQuery public query;
  ...
  function initialize(address _trustedForwarder, address configAddress) public initializer {
    __Ownable_init();
    __Pausable_init();
  ...
  function getContractShares() public view returns (uint256) {
  ...
  function getContractRewards() public view returns (uint256) {
  ...
  function emitCompoundEvent(uint256 value, uint256 lastCompoundTime, uint256 lastContractBalance) external {
    require(_msgSender() == address(strategy), "SENDER_NOT_STRATEGY");
    emit Compound(value, lastCompoundTime, lastContractBalance);
  }

  function depositCalc(uint256 value, uint256 fap) external returns (uint256 amountOut, uint256 priceImpact) {
    (priceImpact, amountOut) = query.deposit(value - fap, value - fap);
    priceImpact = GiddyLibrary.calcPriceImpact(priceImpact, amountOut);
  }

  function deposit(bytes calldata auth, uint256 fap) external whenNotPaused nonReentrant () {
    ...
    require(spender == address(this), "AUTH_SPENDER_CHECK");
    require(owner == _msgSender(), "AUTH_OWNER_CHECK");
    ...
    joinStrategy(owner, value, fap);
  }

  function depositGiddyCalc(uint256 value, uint256 fap) external returns (uint256 amountOut, uint256 priceImpact) {
    fap = giddyToUsdc(fap);
    (priceImpact, amountOut) = GiddyLibrary.calcSwap(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, value, value);
    (priceImpact, amountOut) = query.deposit(priceImpact - fap, amountOut - fap);
    priceImpact = GiddyLibrary.calcPriceImpact(priceImpact, amountOut);
  }

  function depositGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap) external whenNotPaused nonReentrant {
    require(req.spender == address(this), "AUTH_SPENDER_CHECK");
    require(req.owner == _msgSender(), "AUTH_OWNER_CHECK");
    ...
    uint256 usdcValue = GiddyLibrary.swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, address(this), req.value);
    joinStrategy(req.owner, usdcValue, giddyToUsdc(fap));
  }

  function withdrawCalc(uint256 shares, uint256 fap) external returns (uint256 amountOut, uint256 priceImpact) {
    amountOut = sharesToValue(shares);
    (priceImpact, amountOut) = query.withdraw(amountOut, amountOut);
    priceImpact = GiddyLibrary.calcPriceImpact(priceImpact, amountOut);
    amountOut -= fap;
  }

  function withdraw(uint256 shares, uint256 fap) external whenNotPaused nonReentrant {
    uint256 usdcAmount = leaveStrategy(_msgSender(), shares, fap);
    if (!IERC20(USDC_TOKEN).transfer(_msgSender(), usdcAmount)) {
      revert("ACCOUNT_TRANSFER");
    }
  }

  function withdrawGiddyCalc(uint256 shares, uint256 fap) external returns (uint256 amountOut, uint256 priceImpact) {
    fap = giddyToUsdc(fap);
    amountOut = sharesToValue(shares);
    (priceImpact, amountOut) = query.withdraw(amountOut, amountOut);
    (priceImpact, amountOut) = GiddyLibrary.calcSwap(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, priceImpact - fap, amountOut - fap);
    priceImpact = GiddyLibrary.calcPriceImpact(priceImpact, amountOut);
  }

  function withdrawGiddy(uint256 shares, uint256 fap) external whenNotPaused nonReentrant {
    uint256 usdcAmount = leaveStrategy(_msgSender(), shares, giddyToUsdc(fap));
    GiddyLibrary.swapTokensSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, _msgSender(), usdcAmount);
  }

  function joinStrategy(address user, uint256 usdcAmount, uint256 fap) private {
    require(usdcAmount > 0, "INVALID_USDC_AMOUNT");
    strategy.compound();
    usdcAmount = deductFeeUsdc(usdcAmount, fap);
    if (!IERC20(USDC_TOKEN).transfer(address(strategy), usdcAmount)) {
      revert("STRATEGY_TRANSFER");
    }
    uint256 tokenValue = strategy.deposit(usdcAmount);
    uint256 shares = contractShares == 0 ? tokenValue * INIT_SHARES : tokenValue * contractShares / (getContractBalance() - tokenValue);
    userShares[user] += shares;
    contractShares += shares;
    emit Deposit(user, usdcAmount);
  }

  function leaveStrategy(address user, uint256 shares, uint256 fap) private returns (uint256 usdcAmount) {
    require(shares > 0, "ZERO_SHARES");
    require(shares <= userShares[user], "SHARES_EXCEEDS_OWNED");
    strategy.compound();
    uint256 tokenAmount = getContractBalance() * shares / contractShares;
    userShares[user] -= shares;
    contractShares -= shares;
    usdcAmount = strategy.withdraw(tokenAmount);
    usdcAmount = deductFeeUsdc(usdcAmount, fap);
    emit Withdraw(user, usdcAmount);
  }

  function deductFeeUsdc(uint256 amount, uint256 fap) private returns (uint256) {
    if (fap > 0) {
      if (!IERC20(USDC_TOKEN).transfer(config.feeAccount(), fap)) {
        revert("DEDUCT_FEE_USDC");
      }
    }
    return amount - fap;
  }

  function giddyToUsdc(uint256 amount) private view returns(uint256)
  {
    (uint256 resUsdc, uint256 resGiddy,) = IUniswapV2Pair(GIDDY_USDC_PAIR).getReserves();
    require(resUsdc > 0 && resGiddy > 0, "INVALID_PRICE_DATA");
    return amount * resUsdc / resGiddy;
  }

  function setStrategy(address strategyAddress) public onlyOwner {
    strategy = GiddyStrategy(strategyAddress);
  }

  function setQuery(address queryAddress) public onlyOwner {
    query = GiddyQuery(queryAddress);
  }
...
abstract contract GiddyStrategy {
  ...
  function deposit(uint256 usdcAmount) external virtual returns (uint256 tokenAmount);
  function withdraw(uint256 tokenAmount) external virtual returns (uint usdcAmount);
...
abstract contract GiddyQuery {
  function deposit(uint256 inBefore, uint256 inAfter) external virtual returns (uint256 outBefore, uint256 outAfter);
  function withdraw(uint256 inBefore, uint256 inAfter) external virtual returns (uint256 outBefore, uint256 outAfter);
}
