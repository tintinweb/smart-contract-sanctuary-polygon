...
/* dmagic:
A transfer gate (GatedERC20) for use with Dark_Magic tokens
...
struct DmagicTransferGateParameters
{
    address dev;
...
contract DMagicTransferGate is Owned, TokensRecoverable, IDarkMagicTransferGate, ReentrancyGuard
{   
    using Address for address;
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    DmagicTransferGateParameters public parameters;
    IDmagic immutable dmagic ;
    ...
    constructor(address _dMagic) {
        dmagic=IDmagic(_dMagic);
    }
    ...
    function setParameters(address _dev, address _stake, uint16 _stakeRate, uint16 _burnRate, uint16 _devRate) public override ownerOnly()
    {
        ...
        DmagicTransferGateParameters memory _parameters;
    ...
    function allowPool(IUniswapV2Factory _uniswapV2Factory, IERC20 token) public override ownerOnly()
    {
        ...
        address pool = _uniswapV2Factory.getPair(address(dmagic), address(token));
        if (pool == address(0)) {
            pool = _uniswapV2Factory.createPair(address(dmagic), address(token));
        }
    ...
    function safeAddLiquidity(IUniswapV2Router02 _uniswapRouter02, IERC20 token, uint256 tokenAmount, uint256 dmagicAmount) public nonReentrant override
        returns (uint256 dMagicUsed, uint256 tokenUsed, uint256 liquidity)
    {
        require(allowedFactoryAddress[address(_uniswapRouter02.factory())],"This _uniswapV2Factory not allowed");

        address pool = IUniswapV2Factory(_uniswapRouter02.factory()).getPair(address(dmagic), address(token));
        require (pool != address(0) && addressStates[pool] == AddressState.AllowedPool, "Pool not approved");
        unrestricted = true;

        uint256 tokenBalance = token.balanceOf(address(this));
        uint256 dmagicBalance = dmagic.balanceOf(address(this));

        token.safeTransferFrom(msg.sender, address(this), tokenAmount);
        dmagic.transferFrom(msg.sender, address(this), dmagicAmount);
        dmagic.approve(address(_uniswapRouter02), dmagicAmount);
        ...
        uint256 dmagicAmountReceived=dmagic.balanceOf(address(this)).sub(dmagicBalance);

        // uint256 tokenMinAmount = estimateBuy(_uniswapRouter02, token,tokenAmount).mul((100000-slippageBuy)/1000)/100;
        // uint256 dMagicMinAmount = estimateSell(_uniswapRouter02, token,dmagicAmount).mul((100000-slippageSell)/1000)/100;

        (dMagicUsed, tokenUsed ,liquidity ) = 
        _uniswapRouter02.addLiquidity(
            address(dmagic), 
            address(token), 
            dmagicAmountReceived, 
            tokenAmountReceived,
            dmagicAmountReceived.mul(100000-slippage).div(100000), 
        ...
        if (dMagicUsed < dmagicAmount) {
            dmagic.transfer(msg.sender, dmagicAmount.sub(dMagicUsed));
        }
 ...
 function handleTransfer(address, address from, address to, uint256 amount) external override
        ...
        DmagicTransferGateParameters memory params = parameters;
