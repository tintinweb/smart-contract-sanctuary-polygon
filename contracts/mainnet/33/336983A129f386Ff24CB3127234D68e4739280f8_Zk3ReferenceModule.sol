 ...
 * by a follower.
 */
contract Zk3ReferenceModule is ModuleBase, IReferenceModule {
    address public immutable semaphoreZk3;
    Zk3InitData public zk3InitData;
    ...
    constructor(address hub, address zk3) ModuleBase(hub) {
        semaphoreZk3 = zk3;
    }
    ...
    function initializeReferenceModule(
        ...
        zk3InitData = abi.decode(data, (Zk3InitData));
        ISemaphoreZk3 semaphore = ISemaphoreZk3(semaphoreZk3);
        semaphore.broadcastSignal(zk3InitData.signal, zk3InitData.nullifierHash, zk3InitData.circleId, zk3InitData.externalNullifer, zk3InitData.proof);
        return new bytes(0);
    }

    /**
     * @dev Validates that the comment has a valid zk proof.
     */
    function _checkZkProof(uint256 profileId, address user, bytes calldata data) internal {
        Zk3Metadata memory zk3Metadata = abi.decode(data, (Zk3Metadata));
        require(zk3Metadata.circleId == zk3InitData.circleId, 'Zk3ReferenceModule: circle ID mismatch');
        ISemaphoreZk3 semaphore = ISemaphoreZk3(semaphoreZk3);
        semaphore.broadcastSignal(zk3Metadata.signal, zk3Metadata.nullifierHash, zk3Metadata.circleId, zk3Metadata.externalNullifer, zk3Metadata.proof);
    ...
    function processComment(
        ...
        if (keccak256(data) == keccak256(new bytes(0))) {
            revert("Zk3ReferenceModule: no proof");
        }

        _checkZkProof(profileIdPointed, commentCreator, data);
    }
    ...
    function processMirror(
        ...
        _checkZkProof(profileIdPointed, mirrorCreator, data);
    }
}
