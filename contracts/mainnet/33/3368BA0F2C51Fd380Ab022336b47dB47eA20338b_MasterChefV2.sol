...
/// @notice The (older) MasterChef contract gives out a constant number of AXO tokens per second.
/// It is the only address with minting rights for AXO.
...
contract MasterChefV2 is Ownable {
    ...
    /// `rewardDebt` The amount of AXO entitled to the user.
    struct UserInfo {
        uint amount;
        uint rewardDebt;
    }
    ...
    /// Also known as the amount of AXO to distribute per second.
    struct PoolInfo {
    ...
    /// @param _MASTER_CHEF The AxoSwap MCV1 contract address.
    ...
    function pendingAXO(uint _pid, address _user) external view validatePid(_pid) returns (uint pending) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint axoReward = totalAllocPoint == 0 ? 0 : ((multiplier * axoPerSecond() * pool.allocPoint) / totalAllocPoint);
    ...
    function axoPerSecond() public view returns (uint amount) {
        amount = MASTER_CHEF.axoPerSecond() * MASTER_CHEF.poolInfo(MASTER_PID).allocPoint / MASTER_CHEF.totalAllocPoint();
    }
...
interface IMasterChef {
    ...
    struct PoolInfo {
        ...
        uint256 accAxoPerShare; // Accumulated BOO per share, times 1e12. See below.
    }

    function poolInfo(uint256 pid) external view returns (IMasterChef.PoolInfo memory);
    function totalAllocPoint() external view returns (uint256);
    function axoPerSecond() external view returns (uint256);
