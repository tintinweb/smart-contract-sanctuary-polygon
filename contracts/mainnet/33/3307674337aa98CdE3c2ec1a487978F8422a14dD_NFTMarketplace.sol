...
contract NFTMarketplace is INFTMarketplace, Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function initialize(address payable _theosProtocolAddress, uint _theosProtocolFee) public initializer {
    ...
    function setTheosProtocolAddress(address payable _newProtocolAddress) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function setTheosProtocolFee(uint _newProtocolFee) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function listItem(address tokenAddress, uint tokenId, uint price) public {
    ...
    /// @param tokenAddress NFT Collection address
    /// @param tokenId token id
    function unlistItem(address tokenAddress, uint tokenId) public {
    ...
    function unlistItems(ItemToUnlist[] calldata items) public onlyRole(MARKETPLACE_WALLET) {
    ...
    function getItemPriceWithFee(address tokenAddress, uint tokenId) public view returns (uint) {
    ...
    /// @param tokenAddress NFT Collection address
    /// @param tokenId token id
    function buyItem(address tokenAddress, uint tokenId) public payable virtual {
        (Item memory item, bytes32 orderHash) = _getOrder(tokenAddress, tokenId);

        // we are taking fee from both buyer and seller
        // because of that value for the item must be different
        uint fee = _calculateFee(item.price, protocolFee);
        ...
        if (token.supportsInterface(ROYALTIES_INTERFACE_ID)) {
            ...
            if (royaltiesPercentage > 0) {
                ...
                uint royaltyValue = _calculateFee(item.price, royaltiesPercentage);
    ...
    function addMarketplaceWalletRole(address account) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _grantRole(MARKETPLACE_WALLET, account);
    }

    /// @dev Reverts if order is not found
    function _getOrder(address tokenAddress, uint tokenId) internal view returns (Item memory, bytes32) {
    ...
    function _generateUniqueOrderHash(address tokenAddress, uint tokenId) internal pure returns (bytes32) {
    ...
    function _calculateFee(uint price, uint fee) internal pure returns (uint) {
...
interface INFTMarketplace {
    ...
    event TheosProtocolFeeChanged(uint newFee);
    /// @dev Emitted when owner or marketplace unlists the item
    event ItemUnlisted(address indexed owner, address indexed tokenAddress, uint indexed tokenId);
    /// @dev Emitted when owner lists his item for sale
    event ItemListed(address indexed owner, address indexed tokenAddress, uint indexed tokenId, uint price);
    /// @dev Emitted when the listing got updated (e.g. price changed)
    event ItemUpdated(address indexed owner, address indexed tokenAddress, uint indexed tokenId, uint price);
    /// @dev Emitted when the item is sold
    event ItemSold(address indexed newOwner, address indexed tokenAddress, uint indexed tokenId, uint price);
    ...
    /// @notice Changes Theos protocol fee
    /// @param _newProtocolFee new protocol fee
    function setTheosProtocolFee(uint _newProtocolFee) external;
    ...
    /// @param tokenId token id
    /// @param price ask price in wei
    function listItem(address tokenAddress, uint tokenId, uint price) external;

    /// @notice Used to buy the item 
    /// @dev Make sure to send correct amount of ETH
    /// @param tokenAddress NFT Collection address
    /// @param tokenId token id
    function buyItem(address tokenAddress, uint tokenId) external payable;

    /// @notice Used to unlist the item from a marketplace
    /// @dev Only NFT owner / Marketplace can unlist the item
    /// @param tokenAddress NFT Collection address
    /// @param tokenId token id
    function unlistItem(address tokenAddress, uint tokenId) external;

    /// @notice Used to check what is the buy item price
    function getItemPriceWithFee(address tokenAddress, uint tokenId) external view returns (uint);
}
...
library LibOrder {
    bytes32 constant public ORDER_TYPEHASH = keccak256("Order(address maker,Asset makeAsset,Asset takeAsset,uint256 salt,uint256 deadline)Asset(uint256 typ,bytes data)");
    bytes32 constant public ASSET_TYPEHASH = keccak256("Asset(uint256 typ,bytes data)");
