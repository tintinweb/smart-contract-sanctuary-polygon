...
/// @title Canonical SMBSwap V3 factory
/// @notice Deploys SMBSwap V3 pools and manages ownership and control over pool protocol fees
contract SMBV3Factory is ISMBV3Factory {
    /// @inheritdoc ISMBV3Factory
    ...
    function createPool(
        ...
        pool = ISMBV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        ISMBV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return ISMBV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        ISMBV3Pool(pool).setLmPool(lmPool);
    }
}