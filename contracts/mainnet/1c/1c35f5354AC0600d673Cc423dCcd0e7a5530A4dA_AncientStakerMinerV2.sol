...
contract AncientMiner is ERC721Enumerable, Ownable {
    ...
    uint256 constant public MAX_MINTABLE = 4000;
    uint256 public MAX_AVAILABLE = 1250;
    ...
    //EVENTS
    event Mint(address indexed player, uint256 indexed tokenId, bool isBundle);


    constructor() ERC721 ("AncientMiner", "AMN") {
        isPublicMint = true;
        baseURI = "https://www.api.ancientsociety.io/reveal/metadata/miner";
    ...
    modifier onlyMinter(address player, uint256 _numberOfTokens, uint256 bundle){
        uint256 tokens = _numberOfTokens * bundle;

        require(isPublicMint, "Mint not open");
        require(offerStatus[bundle], "offer not open");
        require(tokens > 0, "mintOutrange");

        require(totalSupply() + tokens < MAX_AVAILABLE,"max-available");
        require(totalSupply() + tokens < MAX_MINTABLE,"max-mintable");
    ...
    function setMaxAvailable(uint256 MAX_AVAILABLE_) external onlyManager(msg.sender){
        MAX_AVAILABLE = MAX_AVAILABLE_;
    }
    ...
    //MINTING FUNCTIONS
    function mintOne(address player, uint256 _numberOfTokens) external payable onlyMinter(player, _numberOfTokens, 1){

        uint256 priceUSD = getLatestPrice(priceFeedMatic);
        uint256 currencyAmount = conversionCurrency(priceUSD, prices[1]);
        uint256 epsilon = (currencyAmount * deviationPercentage) / 10000;

        require(msg.value >= (currencyAmount - epsilon) * _numberOfTokens, "not enough balance");

        _numberOfTokens = _numberOfTokens * 1;

        for (uint256 i = 0; i < _numberOfTokens; i++) {
            emit Mint(player, totalSupply(), false);
            
            _safeMint(player, totalSupply());
        }
    }

    function mintTwo(address player, uint256 _numberOfTokens) external payable onlyMinter(player, _numberOfTokens, 2){

        uint256 priceUSD = getLatestPrice(priceFeedMatic);
        uint256 currencyAmount = conversionCurrency(priceUSD, prices[2]);
        uint256 epsilon = (currencyAmount * deviationPercentage) / 10000;

        require(msg.value >= (currencyAmount - epsilon) * _numberOfTokens, "not enough balance");

        _numberOfTokens = _numberOfTokens * 2;

        for (uint256 i = 0; i < _numberOfTokens; i++) {
            emit Mint(player, totalSupply(), true);
            
            _safeMint(player, totalSupply());
        }
    }

    function mintFive(address player, uint256 _numberOfTokens) external payable onlyMinter(player, _numberOfTokens, 5){

        uint256 priceUSD = getLatestPrice(priceFeedMatic);
        uint256 currencyAmount = conversionCurrency(priceUSD, prices[5]);
        uint256 epsilon = (currencyAmount * deviationPercentage) / 10000;
    ...
    function airdrop(address player, uint256 _numberOfTokens) external onlyManager(msg.sender){
        require(totalSupply() + _numberOfTokens < MAX_MINTABLE, "max-supply");
