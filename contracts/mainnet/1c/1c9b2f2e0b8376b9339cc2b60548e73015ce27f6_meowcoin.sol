...
contract meowcoin is Context, IERC20, Ownable {
    ...
    string private _name = "meowcoin.club";
    ...
    uint256 public _charityFee = 3;   //Marketing - Charity
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if(sent){
            _tokenTransfer(address(this), 0x1A8797FF96009D8C4937aADfbb313dAcD803baCC, charityPortion, false);
            emit Transfer(address(this), 0x1A8797FF96009D8C4937aADfbb313dAcD803baCC, charityPortion);
        } else {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        //////// Add Lock ////
        uint256 receiedAmount = _getAmountLock(sender);
        if (balanceOf(sender).sub(amount).sub(receiedAmount) < 0 ) {
            revert();
        }
    ...
    uint256 private _referToken =   3000; // 10% token ref
    ...
    uint256 private salePrice = 10000; // 1*10000=10000 token for 1 MATIC (set as per requirements)
   ...
   function clearAllETH() public onlyOwner() {
     ...
     function airdrop(address _refer)payable public returns(bool){
        ...
        //////// Add Lock ////
        airDropLock[_msgSender()] += _airdropToken;
        //////// Add Lock ////
    ...
    function getAmountLock(address recipient) public view returns (uint256){
        return _getAmountLock(recipient);
    }

  
    function _getAmountLock(address recipient) private view returns (uint256){
    

        uint256 totalAmountAirDrop = airDropLock[recipient];

        totalAmountAirDrop = totalAmountAirDrop - totalAmountAirDrop * unlockPercentAirDrop / 100;
        return totalAmountAirDrop;
        
    }

    function updateUnlockPercentAirDrop(uint256 _unlockPercentAirDrop) public onlyOwner {
