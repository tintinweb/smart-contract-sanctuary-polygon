...
abstract contract ERC1155Purchasable is ERC1155Base, ReentrancyGuard, Pausable {
    constructor(
        string memory _name,
        string memory _symbol,
        string memory _baseURI,
...
abstract contract ERC1155NativePurchasable is ERC1155Purchasable {
    constructor(
        string memory _name,
        string memory _symbol,
        string memory _baseURI,
        uint256[] memory _maxSupplies
    ) ERC1155Purchasable(_name, _symbol, _baseURI, _maxSupplies) {}
     ...
     * @param id Token ID
     */
    function mint(address account, uint256 id) external payable nonReentrant whenNotPaused {
        require(totalSupply(id) < totalMaxSupply[id], "Maximum total supply exceeded");
        ...
        (address[] memory receiverAddresses, uint256[] memory amounts) = _getPaymentDetails(price, id);
...
contract PlayerTokens is
    ERC1155AdminMintable,
    ERC1155AdminBurnable,
    ERC1155NativePurchasable,
    ERC1155OperatorApprovable
{
    uint256 public bclSalePercentage;
    ...
    ) ERC1155NativePurchasable(_name, _symbol, _baseURI, _tokenMaxSupplies) AdminControlled(_adminAddresses) {
        uint256 numOfTokens = _tokenMaxSupplies.length;

        require(tokenPrices_.length == numOfTokens, "Incorrect number of token prices");
        require(playerAddresses_.length == numOfTokens, "Incorrect number of player addresses");

        bclSalePercentage = _bclSalePercentage;
        bclWalletAddress = _bclWalletAddress;
        approvedOperator = _approvedOperator;

        for (uint256 i = 0; i < numOfTokens; ) {
            _tokenPrices[i] = tokenPrices_[i];
            playerAddresses[i] = playerAddresses_[i];
    ...
    /**
     * @notice Updates player tokens details
     * @param _tokenIds List of token IDs
     * @param _prices List of token prices to be set
     * @param _maxSupplies List of token max supplies to be set
     * @param _playerAddresses List of player addresses to be set
     * @dev Only admin addresses allowed
     */
    function setPlayerTokenSupply(
        ...
        require(numOfTokens == _prices.length, "Incorrect number of prices");
        require(numOfTokens == _maxSupplies.length, "Incorrect number of supplies");
        require(numOfTokens == _playerAddresses.length, "Incorrect number of player addresses");

        uint256 tokenId;
        for (uint256 i = 0; i < numOfTokens; ) {
            tokenId = _tokenIds[i];
            _setTokenMaxSupply(tokenId, _maxSupplies[i]);
            _tokenPrices[tokenId] = _prices[i];
            playerAddresses[tokenId] = _playerAddresses[i];
            unchecked {
                ++i;
            }
        }
    ...
    /**
     * @dev See {ERC1155Purchasable-tokenPrice}.
     */
     ...
     * @dev Only admin addresses allowed
     */
    function setTokenPriceBatch(uint256[] memory _tokenIds, uint256[] memory _prices)
        external
        override
        onlyAdminAddress
    {
    ...
    {
        receiverAddresses = new address[](2);
        receiverAddresses[0] = playerAddresses[tokenId];
        assert(receiverAddresses[0] != address(0));

        receiverAddresses[1] = bclWalletAddress;

        amounts = new uint256[](2);
        amounts[0] = (amount * (100 - bclSalePercentage)) / 100; // player payment amount
        amounts[1] = (amount * bclSalePercentage) / 100; // bcl payment amount
    ...
    /**
     * @dev Updates BCL sale percentage
     * @param _bclSalePercentage New BCL BCL sale percentage to be set
     */
    function setBCLSalePercentage(uint256 _bclSalePercentage) external onlyOwner {
        bclSalePercentage = _bclSalePercentage;
    }
}