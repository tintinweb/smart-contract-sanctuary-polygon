...
library VaultLifecycle {
    ...
    function getNextDay(uint256 timestamp, uint256 _dayOfWeek) internal pure returns (uint256) {
        // dayOfWeek = 0 (sunday) - 6 (saturday)
        uint256 dayOfWeek = ((timestamp / 1 days) + 4) % 7;
        uint256 nextDay = timestamp + ((7 + _dayOfWeek - dayOfWeek) % 7) * 1 days;
    ...
    //actually used only by tests, maybe we can remove it
    function getNextFriday(uint256 timestamp) internal pure returns (uint256) {
        // dayOfWeek = 0 (sunday) - 6 (saturday)
        uint256 nextMonday = getNextDay(timestamp, 1);
        uint256 nextWednesday = getNextDay(timestamp, 3);

        return nextMonday > nextWednesday ? nextWednesday : nextMonday;
    }
