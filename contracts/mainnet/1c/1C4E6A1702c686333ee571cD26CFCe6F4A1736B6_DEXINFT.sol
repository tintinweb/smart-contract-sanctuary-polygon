...
contract DEXINFT is ERC721URIStorage, ERC721Enumerable, IERC721TokenCreator, Ownable {
     ...
     */
    event Minted(address indexed minter, uint256 nftID, string uri, uint256 royalty, address indexed creator);
     ...
     */
    constructor() ERC721("DEXI CreatorHub", "DEXICH") {}
     ...
     */
    function bulkMint(address _to, string memory _tokenURI, uint256 _numberOfToken, uint256 _royalty, address _creator) external {
        require(marketContractAddress != address(0), "Market contract is not set");

        for(uint i = 0; i < _numberOfToken; i++) {
            mint(_to, _tokenURI, _royalty, _creator);
        }
    }
     ...
     * Emits a {Transfer} event.
     */
    function mint(address to, string memory _tokenURI, uint256 _royalty, address _creator) public returns (uint256) {
        require(marketContractAddress != address(0), "Market contract is not set");
        ...
        _creators[mintIndex] = _creator;
        _royalties[mintIndex] = _royalty;

        emit Minted(to, mintIndex, _tokenURI, _royalty, _creator);
