...
contract CreditCardCheckoutFacet {
  function claimTo(address to, uint256 quantity) external payable returns (uint256) {
    return LazyMintLib.publicMintTo(quantity, to);
  }
}
...
library LazyMintLib {
    ...
    function publicMint(uint256 quantity) internal returns (uint256) {
        return publicMintTo(quantity, msg.sender);
    }

    function publicMintTo(uint256 quantity, address _to) internal returns (uint256) {
        ...
        if (
            ...
            (ERC721ALib._numberMinted(_to) + quantity) >
            s.maxMintsPerWallet
        ) {
        ...
        return BaseNFTLib._safeMint(_to, quantity);
    }
}
