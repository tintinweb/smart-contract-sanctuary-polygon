...
contract DeliveryStorkClub is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string baseURI;
  string public baseExtension = ".json";
  uint256 public cost = 25 ether; //Matic
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        if(onlyWhitelisted == true){
            require(verifyWhitelist(msg.sender), "user is not whitelisted");
        }
    ...
    returns (uint256[] memory)
  {
    ...
    function verifyWhitelist(address _whitelist) public view returns(bool) {
      bool userIsWhitelisted = whitelist[_whitelist];
