...
contract KyberStrategy is InitializableAbstractStrategy, UniswapV2Exchange, IERC721Receiver {
    ...
    function getRatios() public view override returns (uint256, uint256) {
        ...
        (uint256 reserve0, uint256 reserve1) = _locked();
    ...
    function _locked() public view returns (uint256 _tAmountLiq0, uint256 _tAmountLiq1) {
        (uint160 sqrtPriceX96,  int24 currentTick ,  , ) = uniswapV3Pool.getPoolState();
        (INonfungiblePositionManager.Position memory position, ) = uniswapPositionManager.positions(tokenId);
        if (currentTick < position.tickLower) {
            ...
            return (
                QtyDeltaMath.calcRequiredQty0(TickMath.getSqrtRatioAtTick(position.tickLower), TickMath.getSqrtRatioAtTick(position.tickUpper), position.liquidity, false).revToUint256(),
                0
            );
        }
        if (currentTick >= position.tickUpper) {
            ...
            return (
                0,
                QtyDeltaMath.calcRequiredQty1(TickMath.getSqrtRatioAtTick(position.tickLower), TickMath.getSqrtRatioAtTick(position.tickUpper), position.liquidity, false).revToUint256()
            );
        }
        _tAmountLiq0 = QtyDeltaMath.calcRequiredQty0(sqrtPriceX96, TickMath.getSqrtRatioAtTick(position.tickUpper), position.liquidity, false).revToUint256();
        _tAmountLiq1 = QtyDeltaMath.calcRequiredQty1(TickMath.getSqrtRatioAtTick(position.tickLower),sqrtPriceX96, position.liquidity, false).revToUint256();
    ...
    function _checkBalance() internal view returns (uint256, uint256) {
        ...
        (uint256 _tAmountLiq0, uint256 _tAmountLiq1) = _locked();
