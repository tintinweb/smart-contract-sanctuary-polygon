...
/**
 * @title TokenStorageFactory - single token pool deployer.
 */
contract TokenStorageFactory is Ownable {
	bytes32 public constant INIT_CODE_HASH = keccak256(abi.encodePacked(type(TokenStorage).creationCode));

	uint256 private _index;

	IERC20 public immutable token;

	event CreatePool(uint id, address pool);

	constructor(IERC20 _token) {
		token = _token;
	}

	// @dev Deploy new token storage.
	function deploy() external onlyOwner {
		bytes32 salt = keccak256(abi.encodePacked(_index));
		bytes memory bytecode = type(TokenStorage).creationCode;
		address pool = Create2.deploy(0, salt, bytecode);
		IOwnable(pool).transferOwnership(msg.sender);
		ITokenStorage(pool).initialize(token);
		_index++;
		emit CreatePool(_index - 1, pool);
	}

	// @dev Get last deployed storage id.
	function getLastPool() external view returns (uint256 poolId) {
		require(_index != 0, "TokenStorageFactory: EMPTY_STORAGE");

		poolId = _index - 1;
	}

	// @dev Get storage address by {_poolId}.
	function getStorageAddress(uint256 _poolId) external view returns (address pool) {
		require(_poolId < _index, "TokenStorageFactory: NOT_EXIST");

		bytes32 salt = keccak256(abi.encodePacked(_poolId));
		pool = Create2.computeAddress(salt, INIT_CODE_HASH);
	}
}
...
contract TokenStorage is Ownable {
	...
	function initialize(IERC20 _token) external {
		require(address(token) == address(0), "TokenStorage: INITIALIZED");
		token = _token;
	}
