...
contract CoinMarket is Pausable {
    ...
    function setSell(
        ...
        Orders.push(_Order);
        emit OrderDone(orderCount,coinType,msg.sender,amount,price,unit,true);
        orderCount++;
    ...
    function setBuy(
        ...
        Orders.push(_Order);
        emit OrderDone(orderCount,coinType,msg.sender,amount,price,unit,false);
        orderCount++;
        if (msg.value > chargeAmount) {
           payable(msg.sender).transfer(msg.value - chargeAmount);
        }
    ...
    function removeOrder(uint256 id) internal {
        ...
        if (_Order.label) {
            ...
            payable(_Order.launcher).transfer(amount*_Order.price * 10525 / 10000);
        }
    }
}