...
library LibHashflow {
    ...
    function swapHashflow(Hop memory h) internal {
        ...
        assembly {
            ...
            mstore(add(quote, 192), mload(add(poolData, 52))) // effectiveBaseTokenAmount
            mstore(add(quote, 224), mload(add(poolData, 52))) // maxBaseTokenAmount
            mstore(add(quote, 256), mload(add(poolData, 84))) // maxQuoteTokenAmount
        ...
        if (quote.effectiveBaseTokenAmount > h.amountIn) {
            revert HashflowInvalidAmountIn();
        }

        quote.trader = h.recipient;
        quote.baseToken = h.path[0];
        quote.quoteToken = h.path[1];
        quote.signature = poolData.slice(220, poolData.length - 220);

        h.path[0].approve(h.addr, quote.effectiveBaseTokenAmount);

        IQuote(h.addr).tradeSingleHop(quote);
    }
}
...
library LibSaddleBase {
    ...
    function swapSaddleBase(Hop memory h) internal {
        h.enforceSingleHop();
...
library LibSaddleMeta {
    ...
    function swapSaddleMeta(Hop memory h) internal {
        h.enforceSingleHop();
        address poolAddress = getPoolAddress(h.poolDataList[0]);
        h.path[0].approve(poolAddress, h.amountIn);
        uint8 tokenIndexFrom = ISwap(poolAddress).getTokenIndex(h.path[0]);
        uint8 tokenIndexTo = ISwap(poolAddress).getTokenIndex(h.path[1]);
        ISwap(poolAddress).swapUnderlying(tokenIndexFrom, tokenIndexTo, h.amountIn, 0, block.timestamp);
        h.enforceTransferToRecipient();
    }
}
...
library LibTrident {
    ...
    function swapTrident(Hop memory h) internal {
        ...
        for (uint256 i = 0; i < pl - 1; ) {
            ...
            if (i == 0) {
                ITridentBentoBoxV1(h.addr).deposit(h.path[i], h.addr, getPoolAddress(h.poolDataList[i]), h.amountIn, 0);
            }

            ITridentPool(poolAddress).swap(
                encodeTridentData(h.path[i], isLast ? h.recipient : getPoolAddress(h.poolDataList[i + 1]), isLast)
            );
...
import {LibSaddleBase} from "../LibSaddleBase.sol";
import {LibSaddleMeta} from "../LibSaddleMeta.sol";
...
contract Router1Facet is IRouter1 {
    ...
    function swapSaddleBase(Hop calldata h) external payable {
        LibSaddleBase.swapSaddleBase(h);
    }

    function swapSaddleMeta(Hop calldata h) external payable {
        LibSaddleMeta.swapSaddleMeta(h);
    }
}
