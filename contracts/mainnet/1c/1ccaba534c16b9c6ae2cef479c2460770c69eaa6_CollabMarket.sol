...
contract CollabMarket is Initializable, OwnableUpgradeable, UUPSUpgradeable, IERC721Receiver {
    ...
    function claim() external {
        require(price > 0, "not started");
        IERC20(tokenAddress).transferFrom(msg.sender, address(this), price);

        uint256 tokenId = _extractRandomTokenId();
        IERC721(beastAddress).safeTransferFrom(address(this), msg.sender, tokenId, "");
        IRagnarokConsumable(consumableAddress).mint(msg.sender, EXPEDITION_CHEST, 1);
    }
    ...
    function setAddresses(address _tokenAddress, address _beastAddress) external onlyOwner {
    ...
    function withdrawBeasts(address to, uint256 amount) external onlyOwner {
        for (uint256 index = 0; index < amount; index++) {
            uint256 tokenId = _extractRandomTokenId();
            IERC721(beastAddress).safeTransferFrom(address(this), to, tokenId, "");
        }
    }
    ...
    function _extractRandomTokenId() internal returns (uint256) {
        uint256 randomIndex = _getRandomNumber(_tokenIds.length);
        uint256 maxIndex = _tokenIds.length - 1;
        uint256 tokenId = _tokenIds[randomIndex];
        if (randomIndex == maxIndex) {
            _tokenIds.pop();
            return tokenId;
        }
        uint256 lastItem = _tokenIds[maxIndex];
        _tokenIds[randomIndex] = lastItem;
        _tokenIds.pop();
        return tokenId;
    }

    function _getRandomNumber(uint256 limit) internal returns (uint256) {
        seedNonce++;

        return uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender, seedNonce))) % limit;
    }
    ...
    function onERC721Received(
        ...
        require(msg.sender == beastAddress, "only beasts");
