    ...
    ReentrancyGuardUpgradeable
{
    ...
    enum AuctionCancelErrorType {
        ...
        AUCTION_NOT_YET_COMPLETED,
        AUCTION_NOT_YET_EXPIRED,
        NOT_PERMITTED,
        AUCTION_ENDED
    }
    ...
    Auction[] private _auctions; // All auctions
    PaymentToken[] private _paymentTokens; // All payment tokens available
    MarketFeeTier[] private _feeTiers;
    ...
    uint256 private _buyerFeePercent; // Buyer Fee percent when collect fees
    mapping(uint256 => ClaimState) private _claimStatePerAuctionId; // Claim state per auction ID
    mapping(uint256 => Bid[]) private _bidsPerAuctionId; // All bids per auction ID
    ...
    modifier isValidAuction(uint256 auctionId_) {
        _isValidAuction(auctionId_);
        _;
    }

    modifier onlyMarketOwnerRole() {
        _onlyMarketOwnerRole();
        _;
    }
     ...
     * @param feeTiers_ (MarketFeeTier[] calldata) Auction creation fee
     * @param minAuctionListingPriceUSD_ (uint256) Minimum auction listing price
     */
    function initialize(
        address marketRegistry_,
        MarketFeeTier[] calldata feeTiers_,
        uint256 minAuctionListingPriceUSD_,
        uint256 buyerFeePercent_,
        uint256 sellerFeePercent_
    ) public initializer {
        if (marketRegistry_ == address(0)) {
            revert Market_ZeroAddress();
        }

        if (minAuctionListingPriceUSD_ == 0) {
            revert Market_InvalidMinAuctionPrice();
        }
        ...
        for (uint256 i; i < feeTiersLength; ) {
            if (feeTiers_[i].feeRecipient == address(0)) {
                revert Market_InvalidFeeTierRecipient();
            }

            if (feeTiers_[i].feeTier == 0) {
                revert Market_InvalidFeeTier();
            }
        ...
        if (feeTiersSum > 9999) {
            revert Market_InvalidFeeTiersSum();
        }

        for (uint256 i; i < feeTiersLength; ) {
            _feeTiers.push(
                MarketFeeTier(feeTiers_[i].feeRecipient, feeTiers_[i].feeTier)
            );
        ...
        if (buyerFeePercent_ + sellerFeePercent_ != 10000) {
            revert Market_InvalidFeeTiersSum();
        }
        ...
        _buyerFeePercent = buyerFeePercent_; // We dont need seller fee because the rest of buyer fee is always seller fee

        _auctionIdCounter.increment(); // Starts from 1

        emit MarketFeeTierChanged(feeTiers_);
        emit MinAuctionListingPriceUSDChanged(minAuctionListingPriceUSD_);
        emit MinBidIncrementPercentChanged(500);
    }
     ...
     */
    function _activeAuctionsLength(
    ...
    function _returnPayment(
        ...
        if (buyer_ != address(0)) {
            if (_paymentTokens[paymentTokenIndex_].native) {
                payable(buyer_).transfer(paymentAmount_);
            } else {
                IERC20Upgradeable(_paymentTokens[paymentTokenIndex_].token)
                    .transferFrom(address(this), buyer_, paymentAmount_);
            }
        }
    }

    function _isValidAuction(uint256 auctionId_) private view {
        if (auctionId_ > getAuctionsCount()) {
            revert Market_InvalidAuctionID();
        }
    }

    function _onlyMarketOwnerRole() private view {
        if (!MarketRegistry(_marketRegistry).hasMarketOwnerRole(msg.sender)) {
            revert MarketRegistry_NotMarketOwner();
        }
    }
    ...
    function addPaymentToken(
        ...
        if (!native_ && token_ == address(0)) {
            revert Market_PaymentTokenAddFailed();
        }
        ...
        for (uint256 i; i < length; ) {
            if (
                ...
                PaymentToken storage paymentToken = _paymentTokens[i];
    ...
    function removePaymentToken(
        ...
        if (index_ >= _paymentTokens.length) {
            revert Market_PaymentTokenRemoveFailed(
                uint8(PaymentTokenRemoveErrorType.INVALID_PAYMENT_TOKEN_INDEX)
            );
        }

        if (_serviceFeesPerPaymentTokenId[index_] > 0) {
            revert Market_PaymentTokenRemoveFailed(
                uint8(PaymentTokenRemoveErrorType.WITHDRAW_TOKENS_FIRST)
            );
        }

        if (_activeAuctionsLength(index_) > 0) {
            revert Market_PaymentTokenRemoveFailed(
                uint8(PaymentTokenRemoveErrorType.ACTIVE_AUCTIONS_EXIST)
            );
        }

        delete _paymentTokens[index_].enabled;

        emit PaymentTokenRemoved(index_);
    }
     ...
     */
    function setMinBidIncrementPercent(
        uint256 percent_
    ) external payable onlyOwner onlyMarketOwnerRole {
        _minBidIncrementPercent = percent_ > 10000 ? 10000 : percent_;

        emit MinBidIncrementPercentChanged(_minBidIncrementPercent);
    }
    ...
    function createAuction(
        ...
        if (startTime_ < block.timestamp || endTime_ <= startTime_) {
            revert Market_AuctionCreateFailed(
                uint8(AuctionCreateErrorType.INVALID_TIME_RANGE)
            );
        }
        ...
        if (
            ...
            revert Market_AuctionCreateFailed(
                uint8(AuctionCreateErrorType.INVALID_START_PRICE)
            );
        }

        if (reservedPrice_ < startPrice_) {
            revert Market_AuctionCreateFailed(
                uint8(AuctionCreateErrorType.INVALID_RESERVED_PRICE)
            );
        }
        ...
        if (
            ...
            revert Market_AuctionCreateFailed(
                uint8(AuctionCreateErrorType.INVALID_PAYMENT_TOKEN_INDEX)
            );
        }
        ...
        if (
            ...
            revert Market_AuctionCreateFailed(
                uint8(AuctionCreateErrorType.INVALID_NFT_CONTRACT)
            );
        }
        ...
        _auctions.push(
            Auction(
                ...
                tokenId_,
                paymentTokenIndexes,
                Bid(address(0), 0, paymentTokenIndex_),
                AuctionState.STARTED,
                false
            )
        );

        _claimStatePerAuctionId[auctionId] = ClaimState.NONE;
    ...
    function createInstantSale(
        ...
        if (priceUSD_ < _minAuctionListingPriceUSD) {
            revert Market_AuctionCreateFailed(
                uint8(AuctionCreateErrorType.INVALID_PRICE_USD)
            );
        }
    ...
    function buyNow(
        ...
        Auction storage auction = _auctions[auctionId_ - 1];
        ...
        if (!auction.instantSale) {
            revert Market_AuctionBuyNowFailed(
                uint8(AuctionBuyNowErrorType.INSTANTSALE_NOT_ENABLED)
            );
        }
        ...
        if (!enabled) {
            revert Market_AuctionBuyNowFailed(
                uint8(AuctionBuyNowErrorType.INVALID_PAYMENT_TOKEN_INDEX)
            );
        }
        ...
        if (
            ...
            auction.priceUSD +
                (auction.priceUSD * getServiceFeePercent() * _buyerFeePercent) /
                (10 ** 8)
        ) {
            revert Market_AuctionBuyNowFailed(
                uint8(AuctionBuyNowErrorType.INVALID_PAYMENT_AMOUNT)
            );
        }

        if (msg.sender == auction.creator) {
            revert Market_AuctionBuyNowFailed(
                uint8(AuctionBuyNowErrorType.CREATOR_CANNOT_BUY)
            );
        }
        ...
        if (
            ...
            revert Market_AuctionBuyNowFailed(
                uint8(AuctionBuyNowErrorType.AUCTION_COMPLETED_OR_CANCELLED)
            );
        }
        ...
        if (
            ...
            revert Market_AuctionBuyNowFailed(
                uint8(AuctionBuyNowErrorType.AUCTION_ENDED_OR_NOT_STARTED)
            );
        }
        ...
        emit BoughtNFT(msg.sender, paymentAmountWithFee, paymentTokenIndex_);
    }
     ...
     * @param publicKey (string calldata)
     */
    function placeBid(
        uint256 auctionId_,
        uint256 bidAmountWithFee_,
        ...
        if (auction.instantSale) {
            revert Market_AuctionBidFailed(
                uint8(AuctionBidErrorType.CANNOT_BID_TO_INSTANT_SALE_AUCTION)
            );
        }
        ...
        if (
            bidAmountWithFee <
            minBidAmount + (minBidAmount * _buyerFeePercent) / (10 ** 8)
        ) {
            revert Market_AuctionBidFailed(
                uint8(AuctionBidErrorType.INVALID_BID_AMOUNT)
            );
        }

        if (msg.sender == auction.creator) {
            revert Market_AuctionBidFailed(
                uint8(AuctionBidErrorType.CREATOR_CANNOT_BID)
            );
        }
        ...
        if (
            ...
            revert Market_AuctionBidFailed(
                uint8(AuctionBidErrorType.AUCTION_COMPLETED_OR_CANCELLED)
            );
        }
        ...
        if (
            ...
            revert Market_AuctionBidFailed(
                uint8(AuctionBidErrorType.AUCTION_ENDED_OR_NOT_STARTED)
            );
        }
        ...
        if (
            ...
            auction.reservedPrice +
                (auction.reservedPrice * _buyerFeePercent) /
                (10 ** 8)
        ) {
            auction.endTime = block.timestamp;
        }
    ...
    function redeemNFT(
        ...
        if (msg.sender != highestBid.bidder) {
            revert Market_AuctionRedeemNFTFailed(
                uint8(AuctionRedeemNFTErrorType.NOT_AUCTION_WINNER)
            );
        }
        ...
        if (
            ...
            revert Market_AuctionRedeemNFTFailed(
                uint8(AuctionRedeemNFTErrorType.AUCTION_COMPLETED_OR_CANCELLED)
            );
        }

        if (block.timestamp < auction.endTime) {
            revert Market_AuctionRedeemNFTFailed(
                uint8(AuctionRedeemNFTErrorType.AUCTION_NOT_ENDED)
            );
        }

        if (_claimStatePerAuctionId[auctionId_] == ClaimState.REDEEMED) {
            revert Market_AuctionRedeemNFTFailed(
                uint8(AuctionRedeemNFTErrorType.NFT_ALREADY_REDEEMED)
            );
        }

        _claimStatePerAuctionId[auctionId_] = ClaimState.REDEEMED;
        ...
        emit NFTRedeemed(highestBid.bidder);
    }
    ...
    function claimPayment(
        uint256 auctionId_,
        string calldata encryptionKey_
    ) external nonReentrant whenNotPaused isValidAuction(auctionId_) {
        Auction storage auction = _auctions[auctionId_ - 1];
        Bid memory highestBid = auction.highestBid;

        if (msg.sender != auction.creator) {
            revert Market_AuctionClaimPaymentFailed(
                uint8(AuctionClaimPaymentErrorType.NOT_AUCTION_CREATOR)
            );
        }
        ...
        if (
            ...
            revert Market_AuctionClaimPaymentFailed(
                uint8(
                    AuctionClaimPaymentErrorType.AUCTION_COMPLETED_OR_CANCELLED
                )
            );
        }
        ...
        if (_claimStatePerAuctionId[auctionId_] != ClaimState.REDEEMED) {
            revert Market_AuctionClaimPaymentFailed(
                uint8(AuctionClaimPaymentErrorType.NFT_NOT_REDEEMED)
            );
        }
        ...
        auction.auctionState = AuctionState.COMPLETED;
        _claimStatePerAuctionId[auctionId_] = ClaimState.CLAIMED;
        ...
        if (_paymentTokens[highestBid.paymentTokenIndex].native) {
            ...
            MarketFeeTier[] storage feeTiers = _feeTiers;
        ...
        emit PaymentClaimed(msg.sender, highestBid.bidAmount);
    }
    ...
    function cancelAuction(
        uint256 auctionId_
    ) external nonReentrant isValidAuction(auctionId_) {
        Auction storage auction = _auctions[auctionId_ - 1];
        Bid memory highestBid = auction.highestBid;
        AuctionState auctionState = auction.auctionState;
        ...
        if (msg.sender == auction.creator) {
            if (
                ...
                revert Market_AuctionCancelFailed(
                    uint8(AuctionCancelErrorType.AUCTION_ALREADY_COMPLETED)
                );
            }
            ...
            if (
                ...
                revert Market_AuctionCancelFailed(
                    uint8(AuctionCancelErrorType.AUCTION_NOT_YET_COMPLETED)
                );
            }

            if (block.timestamp - auction.endTime < 7 days) {
                revert Market_AuctionCancelFailed(
                    uint8(AuctionCancelErrorType.AUCTION_NOT_YET_EXPIRED)
                );
            }
        } else if (msg.sender == owner()) {} else {
            revert Market_AuctionCancelFailed(
                uint8(AuctionCancelErrorType.NOT_PERMITTED)
            );
        }

        if (auctionState == AuctionState.CANCELLED) {
            revert Market_AuctionCancelFailed(
                uint8(AuctionCancelErrorType.AUCTION_ALREADY_CANCELLED)
            );
        }
    ...
    /**
     * @dev Delist auction (onlyOwner)
     * @param auctionId_ (uint256) Auction ID
     */
    function delistAuction(
        uint256 auctionId_
    )
        external
        onlyOwner
    ...
    /**
     * @dev Refund NFT purchase and cancel auction, only buyer can call
     * @param auctionId_ (uint256) Auction ID
     */
    function refund(
        uint256 auctionId_
    ...
    function withdrawMATIC(
        uint256 amount_
    ) external payable nonReentrant onlyOwner onlyMarketOwnerRole {
        if (amount_ > address(this).balance) {
            revert Market_InvalidWithdrawAmount();
        }
     ...
     * @param feeTiers_ (MarketFeeTier[] calldata ) New service fee percent
     * @param buyerFeePercent_ (uint256)
     * @param sellerFeePercent_ (uint256)
     */
    function setFeeTiers(
        ...
        _buyerFeePercent = buyerFeePercent_;
     ...
     */
    function setMinAuctionListingPriceUSD(
        uint256 newMinAuctionListingPriceUSD_
    ) external payable onlyOwner onlyMarketOwnerRole {
        _minAuctionListingPriceUSD = newMinAuctionListingPriceUSD_;

        emit MinAuctionListingPriceUSDChanged(newMinAuctionListingPriceUSD_);
    }
     ...
     */
    function getAuctionsForTokenId(
        address nftContract_,
        uint256 tokenId_
    ...
    function getAuctions(
        uint256 offset_,
        uint256 length_
    ) external view returns (uint256[] memory auctionIds, uint256 nextOffset) {
        uint256 length = getAuctionsCount();
     ...
     * @return (MarketFeeTier[] memory) Fee tiers
     */
    function getFeeTiers() external view returns (MarketFeeTier[] memory) {
        return _feeTiers;
    }
     ...
     */
    function getPaymentTokens() external view returns (PaymentToken[] memory) {
    ...
    function getPrice(
        uint256 usdAmount_,
        uint256 paymentTokenIndex_
    ) external view returns (uint256 pricePaymentToken) {
        PaymentToken storage paymentToken = _paymentTokens[paymentTokenIndex_];

        if (!paymentToken.enabled) {
            revert Market_TokenNotEnabled();
        }
    ...
    function isPaymentToken(
        address token_,
        bool native_
    ) public view returns (bool result, uint256 paymentTokenIndex) {
        uint256 length = _paymentTokens.length;
    ...
    AccessControlUpgradeable
{
    ...
    MarketContract[] private _marketContracts;
    ...
    bytes32 public constant LOCK_PROVIDER = keccak256("LOCK_PROVIDER");
        ...
        onlyRole(AMBERFI_OWNER)
    {
        ...
        MarketContract memory marketContract = MarketContract(
            id,
            market_,
            marketOwner_,
            true
        ...
        emit MarketRegistered(id, market_, marketOwner_, true);
    }
    ...
    function unregisterMarket(uint256 marketId_)
        ...
        onlyRole(AMBERFI_OWNER)
    {
        MarketContract storage marketContract = _marketContracts[marketId_];
        marketContract.enabled = false;

        _marketIds[marketContract.marketContract] = 0;

        emit MarketUnregistered(marketId_);
    }
    ...
    function hasLockProviderRole(address account_) public view returns (bool) {
        return hasRole(LOCK_PROVIDER, account_);
    }
