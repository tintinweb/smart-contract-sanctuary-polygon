...
contract CurveATriCrypto3YA is BaseYA, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  address constant private CRV_USD_BTC_ETH_TOKEN = 0xdAD97F7713Ae9437fa9249920eC8507e5FbB23d3;
  ...
  function getContractBalance() public view override returns (uint256) {
    return IERC20(CRV_USD_BTC_ETH_TOKEN).balanceOf(address(this)) - pendingTriFees;
  }

  function getContractRewards() public view override returns (uint256) {
    return getContractBalance() - ((lastVirtualPrice * getContractBalance()) / virtualPrice());
  }
  ...
  function sharesToValue(uint256 shares) public view override returns (uint256) {
    ...
    uint256 pendingFees = getContractRewards() * EARNINGS_FEE / BASE_PERCENT;
  ...
  function deposit(bytes calldata auth, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    uint256 triValue = getContractBalance() - contractBalance;

    uint256 shares = contractShares == 0 ? triValue * INIT_SHARES : triValue * contractShares / contractBalance;
  ...
  function depositGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    value = swapGiddyForUsdc(address(this), value);
  ...
  function withdraw(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    if (!IERC20(CRV_USD_BTC_ETH_TOKEN).approve(CURVE_POOL, value)) {
      revert();
    }
    uint256 contractBalance = IERC20(USDC_TOKEN).balanceOf(address(this));
    ICurveCryptoFiveWaySwap(CURVE_POOL).remove_liquidity_one_coin(value, 1, 0);
    value = IERC20(USDC_TOKEN).balanceOf(address(this)) - contractBalance;
  ...
  function withdrawGiddy(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    value = swapUsdcForGiddy(address(this), value);
  ...
  function getConfiguration() public pure override returns (uint256, uint256) {
    return (EARNINGS_FEE, COMPOUND_USDC_THRESHOLD);
  }
  ...
  modifier deductEarningsFee() {
    ...
    if (earnings > 0) {
      ...
      uint256 usdcBalance = IERC20(USDC_TOKEN).balanceOf(address(this));
      if (usdcBalance >= COMPOUND_USDC_THRESHOLD) {
        if (!IERC20(USDC_TOKEN).transfer(feeAccount, usdcBalance)) {
          revert();
        }
      }
      emit Compound(earnings - feeTri, lastCompoundTime, lastVirtualPrice);
      lastCompoundTime = block.timestamp;
    }
    _;
    lastVirtualPrice = virtualPrice();
