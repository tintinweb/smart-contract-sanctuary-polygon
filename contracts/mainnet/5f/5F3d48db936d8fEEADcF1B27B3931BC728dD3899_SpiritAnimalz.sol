...
contract SpiritAnimalz is ERC721 , DefaultOperatorFilterer {
    ...
    constructor()
        ERC721("SpiritAnimalz1", "SAZO")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 70) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeiehhneepztwesvvxfro67mdwg6yo2fu5a6t27r3zd5lrw77qw7so4/", Strings.toString(tokenId), ".json")
            );
    }
