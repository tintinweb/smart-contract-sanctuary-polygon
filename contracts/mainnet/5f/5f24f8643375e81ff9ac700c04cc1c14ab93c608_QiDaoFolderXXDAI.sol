...
import "./QiDaoFolderV2.sol";
import "./TetuMiddleware.sol";
// import "forge-std/console.sol";

contract QiDaoFolderXXDAI is QiDaoFolderV2 {
  ...
  address internal constant XXDAI_VAULT_ADDRESS = 0xaa19d0e397c964a35e6e80262c692dbfC9C23451;
  address internal constant XXDAI_ADDRESS = 0xf52B3250E026E0307d7d717AE0f331baAA4F83a8;
  ...
  address internal middlewareAddress;

  constructor() QiDaoFolderV2(MAI_ADDRESS, MAI_PAIR_ADDRESS, XXDAI_VAULT_ADDRESS, XXDAI_ADDRESS, QI_ADDRESS, GELATO_OPS_ADDRESS) {
  }

  function setMiddlewareAddress(address _middlewareAddress) external onlyOwner {
    middlewareAddress = _middlewareAddress;
    IERC20(XXDAI_ADDRESS).safeApprove(_middlewareAddress, type(uint).max);
    IERC20(DAI_ADDRESS).safeApprove(_middlewareAddress, type(uint).max);
  }
  ...
  function _approveSwaps() internal override {
    ...
    IERC20(DAI_ADDRESS).safeApprove(CURVE_AAVE_POOL_ADDRESS, type(uint).max);
  }

  function _swapMaiForToken(uint _amountMai, uint _minAmountTokenReceived) internal override {
    require(middlewareAddress != address(0), "middleware not set");
    ...
    // AM3CRV -> DAI
    uint gotDai = IERC20(DAI_ADDRESS).balanceOf(address(this));
    IAPool(CURVE_AAVE_POOL_ADDRESS).remove_liquidity_one_coin(gotAm3crv, 0, 0, true);
    gotDai = IERC20(DAI_ADDRESS).balanceOf(address(this)) - gotDai;

    // DAI -> XXDAI
    uint gotXxdai = IERC20(XXDAI_ADDRESS).balanceOf(address(this));
    TetuMiddleware(middlewareAddress).depositToVault(XXDAI_ADDRESS, gotDai);
    gotXxdai = IERC20(XXDAI_ADDRESS).balanceOf(address(this)) - gotXxdai;

    require(
      gotXxdai >= _minAmountTokenReceived,
      "too much slippage"
    );
    emit SwappedMaiForToken(_amountMai, gotXxdai);
  }
  ...
  function _swapTokenForMai(uint _amountToken, uint _minAmountMaiReceived) internal override {
    ...
    // XXDAI -> DAI
    uint gotDai = IERC20(DAI_ADDRESS).balanceOf(address(this));
    TetuMiddleware(middlewareAddress).withdrawFromVault(XXDAI_ADDRESS, _amountToken);
    gotDai = IERC20(DAI_ADDRESS).balanceOf(address(this)) - gotDai;
...
import "./interfaces/IQiDaoVaultV2.sol";
...
abstract contract QiDaoFolderV2 is UUPSUpgradeable {
  using SafeERC20 for IERC20;
  ...
  function initialize(
    ...
    vaultId = IQiDaoVaultV2(vaultAddress).createVault();
  }
  ...
  function checkRebalanceGelato() external view returns (bool canExec, bytes memory execPayload) {
    ...
    if (rt != RebalanceType.NONE) {
      canExec = true;
      execPayload = abi.encodeWithSelector(QiDaoFolderV2.rebalance.selector, 0, 0); // TODO: set min amounts
    }
  }
  ...
  function uniswapV2Call(address _sender, uint _amount0, uint _amount1, bytes calldata _params) external {
    ...
    IQiDaoVaultV2(vaultAddress).payBackToken(vaultId, amtReceived, 0);
    _unfold(type(uint).max, _minMaiReceivedPerToken);
  ...
  function bailout() external onlyOwner onlyInitialized {
    IQiDaoVaultV2(vaultAddress).safeTransferFrom(address(this), owner, vaultId);
  ...
  function availableBorrows() view public returns (uint) {
    ...
    uint borrowsBasedOnMai = IQiDaoVaultV2(vaultAddress).getDebtCeiling();
  ...
  function vaultValue() view public returns (uint) {
    uint debt = vaultDebt();
    uint repaymentFees = debt * IQiDaoVaultV2(vaultAddress).closingFee() / 10000;
  ...
  function vaultDebt() view public returns (uint) {
    return IQiDaoVaultV2(vaultAddress).vaultDebt(vaultId);
  }

  /// @return amount of collateral locked in the QiDao vault
  function vaultCollateral() view public returns (uint) {
    return IQiDaoVaultV2(vaultAddress).vaultCollateral(vaultId);
  }
  ...
  function _fold(uint _maxNewDebtAmount, uint _minTokenReceivedPerMai) internal {
    ...
    for (uint i = 0; i < maxFoldsPerCall; i++) {
      ...
      // borrow MAI
      IQiDaoVaultV2(vaultAddress).borrowToken(vaultId, borrowAmount, 0);
  ...
  function _depositCollateral(uint _amount) internal {
    ...
    IQiDaoVaultV2(vaultAddress).depositCollateral(vaultId, _amount);
  }

  function _withdrawCollateral(uint _amount) internal {
    IQiDaoVaultV2(vaultAddress).withdrawCollateral(vaultId, _amount);
  }
  ...
  function _repayAvailableMaiDebt() internal {
    ...
    if (amt > 0) {
      IQiDaoVaultV2(vaultAddress).payBackToken(vaultId, amt, 0);
    }
  }
  ...
  function _collateralPrice() view internal returns (uint) {
    return IQiDaoVaultV2(vaultAddress).getEthPriceSource();
  }
  ...
  function _vaultMinimumCdr() view internal returns (uint) {
    return IQiDaoVaultV2(vaultAddress)._minimumCollateralPercentage() * CDR_PRECISION / 1e2;
  }
  ...
  function onERC721Received(address, address, uint256, bytes calldata) pure external returns (bytes4) {
    return QiDaoFolderV2.onERC721Received.selector;
  }
