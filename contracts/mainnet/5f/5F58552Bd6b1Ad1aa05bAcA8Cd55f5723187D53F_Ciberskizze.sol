...
contract Ciberskizze is ERC721 , DefaultOperatorFilterer {
    ...
    constructor()
        ERC721("Ciberskizze", "CIBER")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 20000) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeigaampnoxpactozrsxks7lwfwsjp6mjwkxzfruygbrc5vhvfj3tsi/", Strings.toString(tokenId), ".json")
            );
    }
