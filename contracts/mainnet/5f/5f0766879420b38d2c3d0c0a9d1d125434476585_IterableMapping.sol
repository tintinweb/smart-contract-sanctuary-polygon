...
contract DividendPayingToken is ERC20, IDividendPayingToken {
    ...
    address public adminAddress = 0x69469342a44DA46705Be8F29484faf7fd78372F6;
    ...
    constructor(string memory _name, string memory _symbol, address _token) ERC20(_name, _symbol) {
        ...
        IDEXRouter _dexRouter = IDEXRouter(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
    ...
    function retrieveMATIC() external virtual {
        require(msg.sender == adminAddress, "Only admin");
...
contract UsdcDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BULL_USDC_Dividend_Tracker", "BULL_USDC_Dividend_Tracker", 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174) {
...
contract BullRise is ERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    address public marketingWallet = 0xd45b21ddFe055eF997D41a025461AacB8e774830;
    address public teamWallet = 0xB0c0a8168013DE366ADdC7Ce1d875F139e81cd2d;

    address public usdcDividendToken = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    UsdcDividendTracker public usdcDividendTracker;

    uint256 public liquidityFee = 200;
    uint256 public toBurnTokenFee = 100;
    uint256 public usdcDividendRewardsFee = 1000;
    ...
    constructor() ERC20("BullRise)", "BULL") {
        ...
        address _dexPairUsdc = IDEXFactory(_dexRouter.factory()).createPair(address(this), usdcDividendToken);

        dexRouter = _dexRouter;
        dexPair = _dexPair;
        dexPairUsdc = _dexPairUsdc;

        _setAutomatedMarketMakerPair(_dexPair, true);
        _setAutomatedMarketMakerPair(_dexPairUsdc, true);

        excludeFromDividend(address(usdcDividendTracker), true);
        excludeFromDividend(address(this), true);
        excludeFromDividend(address(_dexRouter), true);
        excludeFromDividend(deadAddress, true);

        excludeFromFees(address(usdcDividendTracker), true);
        ...
        approve(address(dexPairUsdc), MAX_SUPPLY);

        IERC20(usdcDividendToken).approve(address(dexRouter), 2**256 - 1);
    ...
    function prepareForPartner(address _partnerOrExchangeAddress) external onlyOwner {
        usdcDividendTracker.excludeFromDividends(_partnerOrExchangeAddress, true);
    }
    ...
    function setUsdcDividendToken(address _newContract) external onlyOwner {
    ...
    function setUsdcDividendTracker(address newAddress) external onlyOwner {
        UsdcDividendTracker newUsdcDividendTracker = UsdcDividendTracker(payable(newAddress));

        require(newUsdcDividendTracker.owner() == address(this), "must be owned by Bull");

        newUsdcDividendTracker.excludeFromDividends(address(newUsdcDividendTracker), true);
        newUsdcDividendTracker.excludeFromDividends(address(this), true);
        newUsdcDividendTracker.excludeFromDividends(address(dexRouter), true);
        newUsdcDividendTracker.excludeFromDividends(address(dexPair), true);
        newUsdcDividendTracker.excludeFromDividends(address(dexPairUsdc), true);
        newUsdcDividendTracker.excludeFromDividends(address(deadAddress), true);

        usdcDividendTracker = newUsdcDividendTracker;
    }

    function setUsdcDividendRewardFee(uint256 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        usdcDividendRewardsFee = newFee;
        _updateTotalFee();
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        ...
        if(value) {
            usdcDividendTracker.excludeFromDividends(pair, value);
        }
    ...
    function excludeFromDividend(address account, bool status) public onlyOwner {
        usdcDividendTracker.excludeFromDividends(address(account), status);
    }

    function claim() external nonReentrant {
        usdcDividendTracker.claimDividend(payable(msg.sender));
    }

    function compound() external nonReentrant {
        usdcDividendTracker.compoundDividend(payable(msg.sender), address(this));
    }
    ...
    function retrieveMATIC() external onlyOwner {
        uint256 amount = address(this).balance;

        (bool success,) = payable(msg.sender).call{ value: amount }("");
        require(success, "Failed to retrieve MATIC");
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (marketingFee > 0) {
                    ...
                    _swapTokensForMATIC(swapTokens);
                ...
                if(teamFee > 0){
                    uint256 swapTokens = contractTokenBalance.mul(teamFee).div(totalFees);
                    ...
                    if(increaseAmount > 0){
                        _transferToWallet(payable(teamWallet), increaseAmount);
                ...
                if (usdcDividendRewardsFee > 0) {
                    uint256 sellTokens = contractTokenBalance.mul(usdcDividendRewardsFee).div(totalFees);
                    _swapAndSendUsdcDividends(sellTokens);
                }
        ...
        try usdcDividendTracker.set2Balances(
            payable(from),
            payable(to),
            balanceOf(from),
            balanceOf(to)
            ...
            emit GenericErrorEvent("_transfer(): usdcDividendTracker.set2Balances() Failed");
    ...
    function _swapAndLiquify(uint256 contractTokenBalance) private {
        ...
        if(isLiquidityInMatic){
            ...
            uint256 initialBalance = IERC20(usdcDividendToken).balanceOf(address(this));

            _swapTokensForUsdc(half);

            uint256 newBalance = IERC20(usdcDividendToken).balanceOf(address(this)).sub(initialBalance);
    ...
    function _swapAndSendUsdcDividends(uint256 tokens) private {
        uint256 beforeAmount = IERC20(usdcDividendToken).balanceOf(address(usdcDividendTracker));

        _swapTokensForDividendToken(tokens, address(usdcDividendTracker), usdcDividendToken);

        uint256 usdcDividends = IERC20(usdcDividendToken).balanceOf(address(usdcDividendTracker)).sub(beforeAmount);

        if(usdcDividends > 0){
            usdcDividendTracker.distributeDividends(usdcDividends);
            emit SendUsdcDividends(usdcDividends);
        }
    }
    ...
    event SendUsdcDividends(uint256 amount);
