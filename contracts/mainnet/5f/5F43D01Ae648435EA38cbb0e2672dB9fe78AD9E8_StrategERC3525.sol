...
import "@murphy-labs/erc-3525/contracts/ERC3525Upgradeable.sol";
...
contract StrategERC3525 is ERC3525Upgradeable, ReentrancyGuard{
    ...
    uint256 immutable RATIO = 1000;
    uint256 public totalRewards;
    uint256 immutable public MAX_CLAIM_DELAY = 180 days; // max period to claim token

    address public vault;
    address public TREASURY; // à spécifier en dur l'adresse TREASURY
    mapping(uint256 => TokenIdInfo) public tokenIdInfo;

    event ReceiveRewards(address token, uint256 value, uint256 timestamp);
    event Redeem(uint256 indexed tokenId, address indexed owner, uint256 value);
    ...
    */
    ...
    function addRewards(uint256 _amount) external {
        ...
        tokenFee.transferFrom(vault, address(this), _amount);
        totalRewards += _amount;

        emit ReceiveRewards(address(tokenFee), _amount, block.timestamp);
    }
    ...
    */
    function pullUnclaimedToken(uint256 _tokenId) public {
        ...
        if(block.timestamp - info.timeClaimed <= MAX_CLAIM_DELAY) revert ClaimDelayNotReached();
        ...
        if (feeClaimable > 0) {
            tokenFee.transfer(TREASURY, feeClaimable);
            info.timeClaimed = block.timestamp;
            info.lastClaimTotalRewards = totalRewards;

            emit Redeem(_tokenId, TREASURY, feeClaimable);
        }
    }
    ...
    function _claimRewards(uint256 _tokenId) internal returns (bool) {
        ...
        if (feeClaimable > 0) {
            tokenFee.transfer(owner, feeClaimable);
            info.timeClaimed = block.timestamp;
            info.lastClaimTotalRewards = totalRewards;

            emit Redeem(_tokenId, owner, feeClaimable);
        }
