...
// contracts/IVoucherAggregator.sol
// SPDX-License-Identifier: MIT
...
interface IVoucherAggregator {
    ...
    function batchMint721(
        ...
        address payable[] calldata to_addresses,
        VoucherParams[] calldata params_
    ) external payable;

    /// @notice DEBUG: will fail completely if any mint fails. Gets the number of tokens the specified wallet is able to purchase currently.
    ///
    /// @param contracts       A list of ERC721 smart contracts that have the address allowed to mint
    /// @param amounts         A list of the quantity/amounts of each token to mint
    function debugBatchMint721(
        address payable[] calldata contracts,
        uint256[] calldata amounts,
        address payable[] calldata to_addresses,
        VoucherParams[] calldata params_
...
// contracts/VoucherAggregator.sol
// SPDX-License-Identifier: MIT

pragma solidity ^0.8.19;

import "./IVoucherAggregator.sol";
...
interface MintInterface721 {
    function mintTokensTo(uint256 tokenCount_, address to_, VoucherParams calldata params_) external payable;
}

contract VoucherAggregator is IVoucherAggregator, AccessControl, IERC721Receiver {
    event batchMint721Response(bool success, bytes data, string msg, address contract_address, uint256 amount);
    ...
    function batchMint721(
        ...
        require(
            ...
            contracts.length == params_.length &&
        ...
        for (uint i=0; i<contracts.length; i++) {
            ...
            try underlying_contract.mintTokensTo(amounts[i], to_addresses[i], params_[i]) {
                emit batchMint721Response(true, "", "", contracts[i], amounts[i]);
            } catch Error(string memory _err) {
                emit batchMint721Response(false, "", _err, contracts[i], amounts[i]);
    ...
    function debugBatchMint721(
        ...
        for (uint i=0; i<contracts.length; i++) {
            MintInterface721 underlying_contract = MintInterface721(contracts[i]);
            underlying_contract.mintTokensTo(amounts[i], to_addresses[i], params_[i]);
        }
    }
}