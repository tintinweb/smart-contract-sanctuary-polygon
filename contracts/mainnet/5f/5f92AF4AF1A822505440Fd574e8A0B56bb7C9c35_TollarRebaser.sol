...
contract TollarRebaser is Ownable {
    ...
    constructor(ITollar _tollar) Ownable() {
        tollar = _tollar;

        _priceTarget = 10**USD_DECIMALS;
        _priceThresholdMax = _priceTarget + PRICE_THRESHOLD_DEVIATION;
        _priceThresholdMin = _priceTarget - PRICE_THRESHOLD_DEVIATION;
    }
     ...
     */
    function addTollarTransaction(address destination, bytes memory data)
        external
        onlyOwner
    {
    ...
    function setPairUSD(
        ...
        pairUSD = IUniswapV2Pair(
            IUniswapV2Factory(factory).getPair(token0, token1)
        );
        require(address(pairUSD) != address(0), "Pair does not exist");
    }
        ...
        if (price < _priceThresholdMin || price > _priceThresholdMax) {
            ...
            supplyDelta =
                (totalSupply.toInt256() *
                    (price.toInt256() - _priceTarget.toInt256())) /
                (_priceTarget.toInt256());
    ...
    function getPriceTLR_USD() internal view returns (uint256) {
        ...
        (uint256 reserve0, uint256 reserve1, ) = pairUSD.getReserves();
        (uint256 reserveDAI, uint256 reserveTLR) = address(tollar) != token0
            ? (reserve0, reserve1)
            : (reserve1, reserve0);

        uint256 price = (reserveDAI * PRICE_PRECISION) / reserveTLR;
