...
contract BSBFactoryUpgradeable is Initializable, OwnableUpgradeable, IBSBTypes, BSBBaseUpgradeable, VRFConsumerBaseUpgradeable {
    ...
    uint[] public reRollTokenLevelPriceTiersMilestones;
    mapping(uint => uint) public tokenUpgradeLevelPriceTier;
    ...
    function setLevelPriceTiers(
        uint[] memory milestones_,
        uint[] calldata levelPriceTier_
    ) external onlyOwner {

        require(milestones_.length == levelPriceTier_.length, "FAC:13");

        reRollTokenLevelPriceTiersMilestones = milestones_;

        for (uint i = 0; i < milestones_.length; i++) {
            tokenUpgradeLevelPriceTier[milestones_[i]] = levelPriceTier_[i];
        }
    }
    ...
    function _calculatePrice(uint tokenId_, uint tokenLevel_, uint additionalTax_) internal view returns(uint) {
        return gen1ReRollStartingPrice + gen1ReRollAdditionalTax[tokenId_] + additionalTax_ + _getLevelPriceTier(tokenLevel_);
    }
    ...
    function _getLevelPriceTier(uint level_) internal view returns (uint) {
        for (uint i = 0; i < reRollTokenLevelPriceTiersMilestones.length; ++i) {
            if (level_ >= reRollTokenLevelPriceTiersMilestones[reRollTokenLevelPriceTiersMilestones.length - 1 - i]) {
                return tokenUpgradeLevelPriceTier[reRollTokenLevelPriceTiersMilestones[reRollTokenLevelPriceTiersMilestones.length - 1 - i]];
            }
        }

        return tokenUpgradeLevelPriceTier[0];
    }
