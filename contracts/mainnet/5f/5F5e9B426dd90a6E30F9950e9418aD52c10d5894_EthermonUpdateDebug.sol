    ...
    SafeMathEthermon
{
    ...
    mapping(string => uint256 ) reRollPriceEmon;
    ...
    function tok_class(uint64 _tokenId) public view returns (address add, address add0)  {
  ...
  function restat() public view returns (uint256 price )  {
        EtheremonUpdateDataInterface ud = EtheremonUpdateDataInterface(updateData);

        uint256 price = ud.getRerollPrice(1010);
...
function class(uint64 _tokenId) public view returns (uint32 classId)  {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        MonsterObjAcc memory obj;
        (
            obj.monsterId,
    ...
    */
    function reRollMonsterStatsEMON(uint64 _tokenId) public view returns (uint256 price_in_emon)  {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        MonsterObjAcc memory obj;
        (
            obj.monsterId,
      ...
      //  require(obj.trainer != address(0));
        ...
        uint256 price = ud.getRerollPrice(uint32(class.classId));

        return price;
        // if (tradeData.isOnTrading(obj.monsterId)) revert();

        // require(obj.trainer == msgSender());
        // uint256 finalPrice = getRerollPrice(class.classId);
