...
contract GotchiManagementFacet is Modifiers{
    ...
    event UnstakedLending(address _owner, uint256 _tokenId, bool _lending);

    //callable by gotchi owner -- allow owner to pause lending
    function pauseLending(uint256[] calldata _tokenIds,bool _pause) public whenNotPaused{
        for(uint256 i = 0; i < _tokenIds.length; ){
            require(s.deposits[im_diamondAddress][_tokenIds[i]] == msg.sender, "pauseLending: not gotchi depositor");
    ...
    function setUnstakedLending(uint256[] calldata _tokenIds, bool setLending) public{

        for(uint256 i = 0; i < _tokenIds.length; ){

            if(setLending){
                //confirm that the sender is the owner of the gotchi
                require(IAavegotchi(im_diamondAddress).ownerOf(_tokenIds[i]) == msg.sender, "startRentals: not Gotchi owner");

                //confirm that we are a lending operator
                require(IAavegotchi(im_diamondAddress).isLendingOperator(msg.sender,address(this),uint32(_tokenIds[i])), "startRentals: must authorize as lendingOperator");

                //add this gotchi to our list of unstaked gotchis we're managing
                EnumerableSet.add(s.unstakedLendingGotchis,_tokenIds[i]);
            }
            else{
                //confirm that the sender is the owner of the gotchi, or approved
                require(
                    (IAavegotchi(im_diamondAddress).ownerOf(_tokenIds[i]) == msg.sender) ||
                    (s.approvedUsers[msg.sender]), "startRentals: not Gotchi owner");

                //remove the gotchi from our list of unstaked gotchis we're managing
                EnumerableSet.remove(s.unstakedLendingGotchis,_tokenIds[i]);
            }

            emit UnstakedLending(msg.sender,_tokenIds[i],setLending);
            unchecked{
                i++;
            }
        }
    ...
    function lockGotchis(uint256[] calldata _tokenIds,bool _lock, uint256 _lockPeriod) public whenNotPaused{

        for(uint256 i = 0; i < _tokenIds.length; ){

            require(s.deposits[im_diamondAddress][_tokenIds[i]] == msg.sender, "lockGotchis: not gotchi depositor");

            Gotchi storage thisGotchi = s.gotchiMapping[_tokenIds[i]];

            //if we're not locking, then the lockup period must have expired
            require(_lock || (block.timestamp - thisGotchi.lastLocked) > thisGotchi.lockPeriod,"lockGotchis: lockup period has not expired");
            ...
            //if we're locking, need to unpause
            if(_lock){
    ...
    function unstakedReadyToRent(uint32 _tokenId) public view returns(bool _ready){

        //an aavegotchi is ready to rent if it's  both not lent AND not listed
        _ready =
        !(IAavegotchi(im_diamondAddress).isAavegotchiLent(_tokenId)) &&
        (IAavegotchi(im_diamondAddress).getGotchiLendingIdByToken(_tokenId) == 0);
    }
