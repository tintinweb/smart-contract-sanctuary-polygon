...
contract GarageArtist is ERC721A, Ownable, ReentrancyGuard {
    bool public frozenMetadata;
    bool public frozenCollectionSize;
    ...
    mapping(address => uint256) public isAllowlistAddress;
    ...
    function mint(
        ...
        require(
            msg.value >= pricePerMint * _amount,
            "Not enough ETH sent"
        );
        require(
    ...
    function mintAllowlist(
        ...
        require(
            issued + _amount <= collectionSize,
            "Minting limit reached"
        );
        require(
            isAllowlistAddress[_msgSender()] >= _amount, 
    ...
    function mintOwner(
        ...
        for (uint256 i = 0; i < _addresses.length; i++) {
            _mintCount += _amounts[i];
        }
        require(
            issued + _mintCount <= collectionSize,
            "Minting limit reached"
        );
        for (uint256 i = 0; i < _addresses.length; i++) {
    ...
    function allowlistAddresses(
        ...
        uint256[] calldata wAmounts
    ) external onlyOwner {
        for (uint i = 0; i < wAddresses.length; i++) {
            isAllowlistAddress[wAddresses[i]] = wAmounts[i];
        }
    }
    ...
    function setCollectionSize(
        ...
        require(
            !frozenCollectionSize, 
            "Collection Size is frozen"
        );
    ...
    function _mint(
        ...
        for (uint256 j = 0; j < _amount / maxBatchSize; j++) {
            _safeMint(_address, maxBatchSize);
        }
        _safeMint(_address, _amount % maxBatchSize);
    }
