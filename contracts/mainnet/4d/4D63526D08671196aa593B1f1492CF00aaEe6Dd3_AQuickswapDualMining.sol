...
// File contracts/actions/quickswap/IStakingDualRewards.sol
...
// File contracts/actions/quickswap/IStakingDualRewardsFactory.sol
...
interface IStakingDualRewardsFactory {
    function stakingRewardsInfoByStakingToken(address token)
        external
        view
        returns (StakingRewardsInfo memory);
...
// File contracts/actions/quickswap/IDQuick.sol
...
// File contracts/actions/quickswap/AQuickswapDualMining.sol
    ...
    ErrorMsg
{
    ...
    function stake(address token, uint256 amount)
        ...
        delegateCallOnly
    {
        IStakingDualRewards stakingDualRewards = _getStakingDualRewardsContract(
            token
        );
        ...
        )
    {
        ...
        uint256 rewardTokenAAmountBefore = rewardTokenA.balanceOf(
            address(this)
        );

        uint256 rewardTokenBAmountBefore = rewardTokenB.balanceOf(
            address(this)
        );
    ...
    function _getReward(address token) private returns (uint256, uint256) {
        ...
        (
            IERC20 rewardsTokenA,
            IERC20 rewardsTokenB
        ) = _getStakingDualRewardsTokens(token);

        // reward token amount before harvest
        uint256 rewardsTokenAAmountBefore = rewardsTokenA.balanceOf(
            address(this)
        );
        uint256 rewardsTokenBAmountBefore = rewardsTokenB.balanceOf(
            address(this)
        );
        ...
        uint256 rewardsTokenAAmountAfter = rewardsTokenA.balanceOf(
            address(this)
        );
        uint256 rewardsTokenBAmountAfter = rewardsTokenB.balanceOf(
            address(this)
        );
    ...
    {
        StakingRewardsInfo memory info = stakingDualRewardsFactory
            .stakingRewardsInfoByStakingToken(token);
