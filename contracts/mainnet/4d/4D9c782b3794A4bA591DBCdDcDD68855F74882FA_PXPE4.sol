...
contract PXPE4 is ERC721r, Ownable, PullPayment, ReentrancyGuard {
  uint public cost;
  ...
  constructor(
    ...
    uint pricePerUnitInUSD
  ) ERC721r('Patronus x Pablo Echeverry Collection', 'PXPE4', supply, hiddenUri, baseUri, extension) {
    cost = pricePerUnitInUSD;
  ...
  function getPriceInWei(uint amount) public view returns (uint256 weiPrice) {
    return (cost * amount * 1e26) / uint256(getLatestPrice());
  }
  ...
  function setBaseExtension(string memory newExtension) public onlyOwner {
    _setBaseExtension(newExtension);
  }

  function buy(uint amount) public payable nonReentrant {
    require(msg.value >= getPriceInWei(amount), 'transaction underpriced');
    // Send the founds to the escrow contract.
...
contract ERC721r is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // Unrevealed NFT's URI
    string public _notRevealedUri;

    // Unrevealed NFT's URI
    string private _currentBaseURI;

    // NFT's Uri after reveal.
     ...
     */
    constructor(string memory name_, string memory symbol_, uint256 maxSupply_, string memory notRevealedUri_, string memory baserUri_, string memory baseExtension_) {
        _name = name_;
        _symbol = symbol_;
        _maxSupply = maxSupply_;
        _numAvailableTokens = maxSupply_;
        _notRevealedUri = notRevealedUri_;
        _currentBaseURI = baserUri_;
    ...
    function _setHiddenURI(string memory newuri) internal virtual {
    ...
    function _setBaseExtension(string memory newExtension) internal virtual {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(_currentBaseURI).length > 0 ? string(abi.encodePacked(_currentBaseURI, tokenId.toString(), _baseExtension)) : "";
    }
