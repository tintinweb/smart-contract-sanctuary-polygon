...
contract DUNDEAD is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("Ducks Undead", "Ducks Undead",69,6969)
    {
        baseURI = "ipfs://QmfGaHiDNYHvzMiwM4WJwcL8gftWQ7SvihhkNVUbKWK4LG/";
        MintStartEnabled = true;
    }

    function mint(uint256 qty) external payable
    {
        require(MintStartEnabled , "Duck Info:  Minting Public Pause");
        require(qty <= maxMintPerTx, "Duck Info:  Limit Per Transaction");
        require(totalSupply() + qty <= maxSupply,"Duck Info:  Soldout");
        _safemint(qty);
    }
    ...
    function _safemint(uint256 qty) internal
    {
        if(WalletMint[msg.sender] < freeMint) 
        {
           ...
           require(msg.value >= (qty - freeMint) * MintPrice,"Duck Info:  Fund not enough");
            WalletMint[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
        else
        {
           require(msg.value >= qty * MintPrice,"Duck Info:  Fund not enough");
            WalletMint[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
    }
