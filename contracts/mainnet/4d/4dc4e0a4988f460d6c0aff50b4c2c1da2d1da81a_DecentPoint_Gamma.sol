...
contract DecentPoint_Gamma is ERC20, Ownable{
    ...
    //function mint(address to, uint256 amount) public OnlyRightAddress {
        //_mint(to, amount);
    //}
...
function withdrawFee(uint _amount)OnlyRightAddress public noReentrant returns(uint){
...
function userDeposit(uint _projectID, uint _amount)public returns(uint,uint) {
    require(projectOwner[_projectID]!=msg.sender);//project owner cannot be the user.
...
function UserWithdraw(uint _projectID, uint _pointAmount)public noReentrant
returns(uint,uint){
    require(projectOwner[_projectID]!=msg.sender);//project owner cannot be the user.
    require(projectDestiWallet[_projectID]!=msg.sender);
    adjust(_projectID);
    ...
    transfer(address(this),_pointAmount*10**18);//
    logs.push(Log(Action_time,_projectID,msg.sender,5,_pointAmount,1));
...
function userUsePoint_inPlatform (uint _projectID, uint _pointAmount)public returns(uint){
    require(projectOwner[_projectID]!=msg.sender);//project owner cannot be the user.
    require(projectDestiWallet[_projectID]!=msg.sender);
    adjust(_projectID);
     require(projectUserPointRemain[msg.sender][_projectID]>= _pointAmount*10**18);
      uint Action_time = block.timestamp; uint p = _pointAmount*10**18;
      projectPointRemain[_projectID] +=  p;

      transfer(projectOwner[_projectID], p);
      projectUserPointRemain[msg.sender][_projectID]-= p;   
      logs.push(Log(Action_time,_projectID,msg.sender,3,_pointAmount,1));
      return (logs.length-1);
}


function userBuyItemFromUser (uint _projectID, uint _pointAmount, address _seller)public returns(uint,uint){
    require(projectOwner[_projectID]!=msg.sender);//project owner cannot be the user.
    require(projectDestiWallet[_projectID]!=msg.sender);
    adjust(_projectID);
     require(projectUserPointRemain[msg.sender][_projectID]>= _pointAmount*10**18);
      ...
      projectUserPointRemain[msg.sender][_projectID]-= p;
      projectUserPointRemain[_seller][_projectID]+= p;
      transfer(_seller,p);

      logs.push(Log(Action_time,_projectID,msg.sender,3,_pointAmount,1));
      logs.push(Log(Action_time,_projectID,_seller,7,_pointAmount,1));
      return ((logs.length-2),(logs.length-1));
}
...
returns(uint,uint){
   ...
   logs.push(Log(Action_time,_projectID,msg.sender,5,_pointAmount,1));
         ...
         logs.push(Log(Action_time,_projectID,msg.sender,3,currencyBack/(10**18),3));
...
function mintPointByProjectOwner(uint _projectID,uint amount) public noReentrant returns(uint){
         ...
         logs.push(Log(Action_time,9999999,msg.sender,5,amount/(10**18),1));
        
        return (logs.length-1);
    }

   function promotionPointByProjectOwner(uint _projectID, address _user, uint _amount)public noReentrant returns(uint){
        require(specialAuthorized[_projectID]==1);
        require(projectOwner[_projectID]==msg.sender);         
        ...
        uint amount = _amount*10**18;
         ...
         transfer(_user,amount);
         logs.push(Log(Action_time,9999999,_user,6,amount/(10**18),1));
         return (logs.length-1);

   }

   function adjust(uint _projectID)internal{
       uint b = balanceOf(msg.sender);uint x;uint y;
       if(projectUserPointRemain[msg.sender][_projectID]>b){
          x = projectUserPointRemain[msg.sender][_projectID];
          y = x-b;
          projectUserPointRemain[msg.sender][_projectID]-=y;
       }
