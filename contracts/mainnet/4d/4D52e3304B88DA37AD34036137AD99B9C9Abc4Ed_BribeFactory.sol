...
contract InternalBribe is IBribe {
    ...
    uint public constant DURATION = 3 days; // rewards are released over 3 days
...
contract ExternalBribe is IBribe {
    ...
    uint internal constant DURATION = 3 days; // rewards are released over the voting period
    uint internal constant MAX_REWARD_TOKENS = 16;
    ...
    function _bribeStart(uint timestamp) internal pure returns (uint) {
        return timestamp - (timestamp % (3 days));
    }
    ...
    function getEpochStart(uint timestamp) public pure returns (uint) {
        ...
        return timestamp < bribeEnd ? bribeStart : bribeStart + 3 days;
    }
