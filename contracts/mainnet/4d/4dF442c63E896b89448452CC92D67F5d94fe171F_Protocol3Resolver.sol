...
contract Protocol3Resolver is Helpers {
    ...
    struct Position {
        ...
        address pool;
        address[] tokens;
        uint256[2] collateralWithoutFeeAccrued;
        uint256[2] collateral;
        uint256[] debts;
        bool isStaked;
        BorrowLimit borrowLimit;
        uint256 status;
    ...
    function getPositionInfo(uint256[] memory NFTIDs_) public view returns (Position[] memory positions_) {
        ...
        for(uint256 i = 0; i < NFTIDs_.length; i++) {
            ...
            positions_[i].owner = protocolModule.nftOwner(uint96(NFTIDs_[i]));
            positions_[i].pool = getPoolAddress(NFTIDs_[i]);
            positions_[i].tokens = protocolModule.poolMarkets(positions_[i].pool);
            (
                positions_[i].collateralWithoutFeeAccrued[0],
                positions_[i].collateral[0],
                positions_[i].collateralWithoutFeeAccrued[1],
                positions_[i].collateral[1]
            ) = getCollateral(NFTIDs_[i], positions_[i].pool);
    ...
    function getUserInfo(address user_) public view returns (Position[] memory) {
        uint256[] memory NFTIDs_ = getUsersDepositedNfts(user_);
        return getPositionInfo(NFTIDs_);
    }

    constructor(address liquidityAddr_, address protocolAddr_) Helpers(liquidityAddr_, protocolAddr_) {}
}
...
contract Constants {
    ...
    IProtocolModule internal immutable protocolModule;
    ...
    constructor(address liquidityAddr_, address protocolAddr_) {
        liquidity = ILiquidity(liquidityAddr_);
        protocolModule = IProtocolModule(protocolAddr_);
    }
...
contract UniswapHelpers is Constants {
    ...
    function _computeAddress(address factory_, PoolKey memory key_)
        internal
        pure
    ...
    function _getPoolAddress(
        ...
        poolAddr_ = _computeAddress(
            nftManager.factory(),
            PoolKey({token0: token0_, token1: token1_, fee: fee_})
        );
    }

    constructor(address liquidityAddr_, address protocolAddr_) Constants(liquidityAddr_, protocolAddr_) {}
...
contract Helpers is UniswapHelpers {
    ...
    function getCollateralWithoutFee(uint256 NFTID_, address pool_) internal view returns (uint256 amount0WithoutFeeAccrued_, uint256 amount1WithoutFeeAccrued_) {
    ...
    function getCollateral(uint256 NFTID_, address pool_) internal view returns (uint256 amount0WithoutFeeAccrued_, uint256 amount0_, uint256 amount1WithoutFeeAccrued_, uint256 amount1_) {
        (uint256 fee0_, uint256 fee1_) = protocolModule.getFeeAccruedWrapper(NFTID_, pool_);
        (amount0WithoutFeeAccrued_, amount1WithoutFeeAccrued_) = getCollateralWithoutFee(NFTID_, pool_);
    ...
    function getUsersDepositedNfts(address user_) internal view returns (uint256[] memory NFTIDs) {
        (uint current_,, uint count_) = protocolModule.nftLink(user_);
        NFTIDs = new uint256[](count_);
        for (uint i = 0; i < count_; i++) {
            NFTIDs[i] = current_;
            (, current_,) = protocolModule.nftList(uint96(current_));
        }
    }

    constructor(address liquidityAddr_, address protocolAddr_) UniswapHelpers(liquidityAddr_, protocolAddr_) {}
