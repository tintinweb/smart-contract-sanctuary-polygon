            ...
            for (uint i; i < rewards.length; ++i) {
                ...
                if (rewardBal > 0) {
                    if (rewards[i].rewardSwapPoolId != bytes32(0)) {
                        ...
                        uint256[] memory amountOut = IUniswapRouterETH(
                            unirouter
                        ).getAmountsOut(rewardBal, rewards[i].routeToNative);
...
contract StratFeeManager is Ownable, Pausable {
    ...
    event SetFeeRecipient(address beefyFeeRecipient);
    event SetFeeConfig(address beefyFeeConfig);
    ...
    function setFeeRecipient(address _beefyFeeRecipient) external onlyOwner {
        ...
        emit SetFeeRecipient(_beefyFeeRecipient);
    }

    // set new fee config address to fetch fees
    function setFeeConfig(address _beefyFeeConfig) external onlyOwner {
        beefyFeeConfig = IFeeConfig(_beefyFeeConfig);
        emit SetFeeConfig(_beefyFeeConfig);
    }
