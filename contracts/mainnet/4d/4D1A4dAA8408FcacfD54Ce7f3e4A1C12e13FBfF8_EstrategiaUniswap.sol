 ...
 *         La estrategia interacciona con la Pool de Liquidez USDC-USDT en UniswapV3.
...
contract EstrategiaUniswap is Ownable {
	 ...
	 */
	function gananciaEstimada() public view returns (uint256 usdcFee, uint256 usdtFee) {
		(
			,
			,
			,
	...
	function balanceDesplegado() public view returns (uint256 balance) {
		...
		// Calculamos las cantidades asociadas a la liquidez
		...
		// Calculamos las fees generadas
		uint256 usdcFee = _calcularFeesGeneradas(true, feeGrowthInside0Last, tick, liquidity) +
			uint256(tokensOwed0);
		uint256 usdtFee = _calcularFeesGeneradas(false, feeGrowthInside1Last, tick, liquidity) +
			uint256(tokensOwed1);

		balance = cantidad0 + cantidad1 + usdcFee + usdtFee;
	}
	...
	function _calcularFeesGeneradas(
		...
		unchecked {
			// Calculamos crecimiento de fees por debajo
			...
			// Calculamos crecimiento de fees por encima
