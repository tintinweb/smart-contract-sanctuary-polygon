...
contract MagpieCore is ReentrancyGuard, Ownable, IMagpieCore {
  ...
  mapping(bytes => bool) private vaas;
  ...
  function _swap(IMagpieRouter.SwapArgs memory swapArgs) internal returns(uint256[] memory amountOuts) {
    ...
    require(address(swapArgs.to).getBalance() == amountOut, 'MagpieCore: invalid amountOut');
  ...
  function swapIn(SwapInArgs calldata args) external payable override returns(uint256[] memory amountOuts, uint64, uint64) {
    ...
    amountOuts = _swap(args.swapArgs);

    toAssetId.increaseAllowance(config.tokenBridgeAddress, amountIn);
  ...
  function swapOut(
    ...
    require(vaas[vm.payload] != true, 'MagpieCore: already used vaa');

    vaas[vm.payload] = true;
