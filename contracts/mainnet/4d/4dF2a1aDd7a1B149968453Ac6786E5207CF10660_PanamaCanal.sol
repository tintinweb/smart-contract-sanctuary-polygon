...
contract GameManager {
    uint256 public constant MAX_NUMBER_OF_HANDS = 2;
    ...
    constructor(GameManagerConstructorArgs memory _gameManagerConstructorArgs) {
        uint256 expectedLength = _gameManagerConstructorArgs
            .approvedTokens
            .length;

        require(
            _gameManagerConstructorArgs.minimumAnte.length == expectedLength &&
                _gameManagerConstructorArgs.maximumAnte.length ==
                expectedLength,
        ...
        for (; i < expectedLength; ) {
            minimumAnte[
                _gameManagerConstructorArgs.approvedTokens[i]
            ] = _gameManagerConstructorArgs.minimumAnte[i];
            maximumAnte[
                _gameManagerConstructorArgs.approvedTokens[i]
            ] = _gameManagerConstructorArgs.maximumAnte[i];
    ...
    function getRequestIdsByGame(uint256 _gameId)
        public
        view
        returns (uint256[] memory)
    {
    ...
    function getDoubleDownHand(uint256 _gameId)
        public
        view
        returns (bool[] memory)
    {
    ...
    function getDealerCards(uint256 _gameId)
        public
        view
        returns (Card[] memory)
    {
    ...
    function getPlayerDealtCards(uint256 _gameId, uint256 _handPointer)
        public
        view
        returns (Card[] memory)
    {
    ...
    function setMinimumAnte(address _token, uint256 _minimumAnte)
        public
        virtual
    {
    ...
    function setMaximumAnte(address _token, uint256 _maximumAnte)
        public
        virtual
    {
