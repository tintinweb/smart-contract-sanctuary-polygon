...
contract Gachapon is PrizePoolManager, StateManager, IGachapon, VRFConsumerBase {
  ...
  function initialize(string memory _gachaponTitle, address _currency) external {
    require(msg.sender == address(_factory), "The gachapon can only be initialized by the factory, after its creation");
...
abstract contract StateManager is Ownable, IDeployerControlledState {
  ...
  /// @dev the address of the factory that created this gachapon
