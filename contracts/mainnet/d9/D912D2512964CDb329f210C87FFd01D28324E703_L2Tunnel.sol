...
contract L2Tunnel is FxBaseChildTunnel, Tunnel {
	constructor(address _fxChild, address _Svg721Address)
		FxBaseChildTunnel(_fxChild)
	{
		Svg721Address = _Svg721Address;
	}

	function transferToL1(uint256 tokenId) external whenNotPaused {
	...
	function _processMessageFromRoot(
		...
		if (ISVG721(Svg721Address).exists(tokenId)) {
			IERC721(Svg721Address).safeTransferFrom(address(this), to, tokenId);
		} else {
			IL2_SVG721(Svg721Address).mint(to, tokenId);
		}
