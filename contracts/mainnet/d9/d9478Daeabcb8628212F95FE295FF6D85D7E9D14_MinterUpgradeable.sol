...
contract MinterUpgradeable is ERC1271, OwnableUpgradeable, ERC165Upgradeable, IMinterUpgradeable, RolesValidator {
    ...
    event NewSplitter(LibPart.Part[] payees, bytes32 splitterBytes, address indexed splitterAddress);
    ...
    // Implementation of the payment splitter contract to be deployed for eah unique royalty definition
    address public splitterImplementationContract;

    // Reciever of the royalty funds and stake manager
    address public royaltyStaker;
    bool public isRoyaltyStakerLocked;

    // Maps royalty definition hash to royalty splitter address and %
    ...
    function setSplitterImplementationContract(address _splitterImplementationContract) public initializer {
        splitterImplementationContract = _splitterImplementationContract;
    }

    function setRoyaltyStakingContract(address _royaltyStaker) public onlyOwner {
        require(!isRoyaltyStakerLocked, "Royalty staker set");
        royaltyStaker = _royaltyStaker;
    }

    function lockRoyaltyStakingContract() public onlyOwner {
        isRoyaltyStakerLocked = true;
    }
    ...
    function getOrCreateSplitterUsingSplit(LibPart.Part[] memory payees) internal virtual returns (bytes32){
        ...
        if (payees.length == 1) {
            ...
            if (splitterAddress == address(0x0)){
                ...
                SplitPayments(splitterAddress).setRoyaltyStaker(royaltyStaker);

                // Save the data to be retrieved for storing stakes and fetching royalty info
    ...
    function getWithdrawSplits(address splitterAddress) external view override returns (LibPart.Part[] memory) {
        require(withdrawSplits[splitterAddress].length > 0, "Splits not defined");
        return withdrawSplits[splitterAddress];
    }
...
import "./../exchange/IRoyaltyStaker.sol";

contract SplitPayments is Initializable {
    IRoyaltyStaker staker;
    
    constructor() public {
    }

    function setRoyaltyStaker(address _staker) external initializer {
        staker = IRoyaltyStaker(_staker);
    }

    receive() external payable {
        staker.recieveRoyalty{value: address(this).balance}();
    }
