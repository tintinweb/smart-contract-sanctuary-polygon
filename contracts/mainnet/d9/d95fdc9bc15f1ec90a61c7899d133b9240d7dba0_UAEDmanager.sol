...
contract UAEDmanager {
    address public immutable usdc;
    UAED public immutable uaed;
    uint public immutable USDC2UAEDratio;   // 8 decimals  

    constructor(address _usdc) {
        usdc = _usdc;
        uaed = new UAED();
        USDC2UAEDratio = 27000000;
    }

    function mint(address _user, uint _amount) public {
        IERC20(usdc).transferFrom(msg.sender, address(this), (_amount * USDC2UAEDratio) / 1e8);    // 8 decimals  
        uaed.mint(_user, _amount);
    }

    function burn(address _owner, uint _amount) public {
        uaed.burn(msg.sender, _owner, _amount);
        IERC20(usdc).transfer(msg.sender, (_amount * USDC2UAEDratio) / 1e8);     // 8 decimals  
    }
}