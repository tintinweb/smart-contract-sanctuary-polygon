...
contract Vault is IVault, AccessControlEnumerableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function underlying() public view override returns (IERC20MetadataUpgradeable) {
    ...
    function _mintAfterTransfer(uint256 amount) internal {
    ...
    function _rebalance() internal {
        ...
        if (deployAmount != 0) {
            ...
            uint256 nativePrice = IPriceOracle(config.underlyingNativePriceOracle()).priceByQuoteSymbol(
                address(underlying())
            );
    ...
    function mintWithDepositToken(uint256 amount, address depositToken) external override whenNotPaused nonReentrant {
        ...
        IERC20MetadataUpgradeable(depositToken).safeTransfer(unwinder, amount);
    ...
    function withdrawFromAllStrategies() external override nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
        IVaultConfig.Strategy[] memory strategies = config.getStrategiesList();

        // try to withdraw all cash regardless of whether its locked
        for (uint256 i = 0; i < strategies.length; i++) {
...
import "../interfaces/IKyberTokenExchange.sol";
...
interface IVault {
    ...
    // /**
    //  * @dev IMPORTANT: Called by Flurry DApp only. All arguments should be determined off-chain
    //  * @param optPoolsPath array of addresses of the KMM pools used
    //  * @param optPath array of IERC20MetadataUpgradeable tokens in the trading path
    //  * @param amountIn_ exact amount of input underlying
    //  * @param amountOutMin_ minimum amount of output rhoToken desired
    //  */
    // function mintViaKyber(
    //     address[] calldata optPoolsPath,
    //     IERC20MetadataUpgradeable[] calldata optPath,
    //     uint256 amountIn_,
...
interface IVaultConfig {
    ...
    function kyberTokenExchange() external view returns (address);

    function setKyberTokenExchange(address addr) external;
}
...
/**
 * @title Kyber Token Exchange Interface
 * @notice Interface for Kyber Token Exchange, which implements ITokenExchange
 *
 */
interface IKyberTokenExchange is ITokenExchange {
    /**
     * @dev IMPORTANT: Called by Vault only. All arguments should be determined off-chain
     * @param optPoolsPath array of addresses of the KMM pools used
     * @param optPath array of IERC20MetadataUpgradeable tokens in the trading path
     * @param amountIn_ exact amount of input underlying
     * @param amountOutMin_ minimum amount of output underlying desired
     * @return uint256 ouptut token received
     */
    function sellExactInputWithPath(
        address[] calldata optPoolsPath,
        IERC20MetadataUpgradeable[] calldata optPath,
        uint256 amountIn_,
        uint256 amountOutMin_
    ...
    function getPairPoolsPath(address input_, address output_) external view returns (address[] memory);

    function getPools(address token0, address token1) external view returns (address[] memory _tokenPools);

    // function getAmountOut(uint256 amountIn, address[] calldata poolsPath, IERC20MetadataUpgradeable[] calldata path) external view returns (uint256 amount);
    // function getAmountIn(uint256 amountIn, address[] calldata poolsPath, IERC20MetadataUpgradeable[] calldata path) external view returns (uint256 amount);

    /**
     * @return _governmentFeeBps amount of fees taken by KyberSwap from each liquidity addition
     * NOTE: it does not affect swapping (no fee is charged)
     */
    function getKyberFeeBps() external view returns (uint16 _governmentFeeBps);
}
