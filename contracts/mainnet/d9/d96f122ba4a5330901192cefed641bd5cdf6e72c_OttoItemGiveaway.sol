...
contract OttoItemGiveaway is BaseContract {
    ...
    function setAdventure(address adv_) external onlyAdmin {
        ADV = IAdventure(adv_);
    }
    ...
    function airdropToOttoOwner(
        ...
        for (uint256 i = 0; i < ottoIds_.length; i++) {
            ...
            if (owner_ == address(ADV)) {
                owner_ = ADV.rootOwnerOf(ottoId_);
            }
    ...
    function transferItem(
        ...
        require(
            to.length == ids.length && to.length == amount.length,
            'to, ids and amount must be same length'
        );
        for (uint256 i = 0; i < to.length; i++) {
            ITEM.safeTransferFrom(msg.sender, to[i], ids[i], amount[i], '');
        }
    }
...
interface IOttoItem {
    ...
    struct ItemInfo {
        EquipableGender equibableGender;
    }
    ...
    function equipable(uint256 itemId_, uint256 ottoId_)
        external
        view
        returns (bool);
    ...
    function item(uint256 itemId_) external view returns (ItemInfo memory);
}
...
interface IAdventure {
    ...
    function rootOwnerOf(uint256 ottoId_) external view returns (address);
}
