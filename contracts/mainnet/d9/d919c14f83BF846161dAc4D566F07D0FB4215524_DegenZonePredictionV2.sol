...
contract DegenZonePredictionV2 is  ReentrancyGuard {
    ...
    AggregatorV3Interface public oracle= AggregatorV3Interface(0xc907E116054Ad103354f2D350FD2514433D57F6f);

    IERC721 public y = IERC721(0x1629932BBBBB7BDA156EFecF92D06c612e2C9708);  //define NFT token
...
/*     function claimNFTBull() public nonReentrant notContract{
    ...
    function NFTPrizes() public view returns(uint256,uint256){
        return (NFTbull,NFTbear);
    }
    ...
    function claim(uint256 epoch) public nonReentrant notContract{
        ...
        if (ledger[epoch][msg.sender].betType == 1){
            ...
            if (rounds[epoch].closePrice > rounds[epoch].lockPrice ){
                ...
                address NFTOwner = y.ownerOf(tokenId);
                _safeTransferMatic(msg.sender,reward);
                _safeTransferMatic(NFTOwner,NFTreward);
            ...
            if (rounds[epoch].closePrice < rounds[epoch].lockPrice ){
                ...
                address NFTOwner = y.ownerOf(tokenId+1);
