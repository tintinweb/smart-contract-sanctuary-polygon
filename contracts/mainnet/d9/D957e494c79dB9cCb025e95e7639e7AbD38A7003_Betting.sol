...
contract Betting is Ownable, Pausable, IBettable, INicknameStorage, IBetResolvable {
    ...
    event BetPlaced(uint256 _betId, address _player, address _token, uint256 _amount);
    event BetRefunded(uint256 _betId, address _player, address _token, uint256 _amount);
    ...
    function enterWithBet(address _token, uint256 _amount) whenNotPaused external returns (uint256) {
        ...
        uint256 betIdentifier = currentBetNumber;
        currentBetNumber += 1;
        ...
        emit BetPlaced(betIdentifier, msg.sender, _token, _amount);
    ...
    function refund(uint256 _betIdentifier) onlyOwner internal {
        ...
        emit BetRefunded(_betIdentifier, player, token, amount);
    }
