...
import "contracts/utils/PartyVotingWindowStorage.sol";
...
import "contracts/NFTNomineeForPartyLeader.sol";


/**
 * @title ERC721 token party leader.
 */
contract NFTPartyLeader is
    Base,
    NFTPartyMembershipOwnershipPauserMintPriceTransferTaxBase,
    MintPowerPointsStorage,
    PartyVotingWindowStorage
{
    using Utils for address;
    address public nftNomineeForPartyLeader;
    ...
    /// @notice fet token info
    /// @param tokenId token id
    /// @return windowId window id
    /// @return nflTokenId NfL token id
    /// @return isPartyLeaderNow is party leader now
    function getTokenInfo(uint256 tokenId) external view returns(
        uint256 windowId,
        uint256 nflTokenId,
        bool isPartyLeaderNow
    ) {
        TokenInfo memory o = tokenInfo[tokenId];
        windowId = o.windowId;
        nflTokenId = o.nflTokenId;
        isPartyLeaderNow = tokenId == _lastTokenId;
    }

    /// @notice get currentPartyLeaderTokenId
    /// @return token id
    function getCurrentPartyLeaderTokenId() public view returns(uint256) {
        return _lastTokenId;
    }

    /// @notice check if is currentPartyLeader
    /// @param wallet some address
    /// @return flag
    function isCurrentPartyLeader(address wallet) external view returns(bool) {
        uint256 _lastTokenIdValue = _lastTokenId;
        return _lastTokenIdValue > 0 && ownerOf(_lastTokenIdValue) == wallet;
    }
    ...
    function initialize2(
        ...
        __PartyVotingWindowStorage_init_unchained();
    }

    /// @notice 3rd initialization
    /// @param mintPowerPointsValue mintPowerPoints value
    /// @param nftNomineeForPartyLeaderAddress nftNomineeForPartyLeader address
    function initialize3(
        uint256 mintPowerPointsValue,
        address nftNomineeForPartyLeaderAddress
    ) external virtual initializer3 {
        __MintPowerPointsStorage_init_unchained(mintPowerPoints);
        nftNomineeForPartyLeader = nftNomineeForPartyLeaderAddress.ensureNotZero();
    }

    /// @notice inauguration
    /// @param nflTokenId NfL token id
    function mint(uint256 nflTokenId) external {
        uint256 tokenId = ++_lastTokenId;
        uint256 passportId = getPassportId();
        checkPartyMembership();

        // Msg.Sender pays MintPrice to Coin, which is transferred to the Treasury Account.
        // MintPrice.sol

        // Msg.Sender has NFT Status = Accepted
        require(NFTNomineeForPartyLeader(nftNomineeForPartyLeader).ownerPartyMembershipOf(nflTokenId) ==
            getPartyMembershipId(), "NOT_OWNER");
        (
            uint256 partyVotingWindowTokenId,
            NFTNomineeForPartyLeader.Status NFLstatus,
            /* uint256 NFLvotes */,
            /* uint256 NFLpartyLeaderTokenId */,
            /* uint256 NFLelectedAt */
        ) = NFTNomineeForPartyLeader(nftNomineeForPartyLeader).getTokenInfo(nflTokenId);
        require(
            NFLstatus == NFTNomineeForPartyLeader.Status.Elected,
            "BAD_STATUS"
        );

        // check partyVotingWindowTokenId is the last finished partyVotingWindow
        if (PartyVotingWindow(partyVotingWindow).getStatus(partyVotingWindowTokenId+1) >= PartyVotingWindow.Status.Voting){
            NFTNomineeForPartyLeader(nftNomineeForPartyLeader).setStatusExpired(nflTokenId);
            return;
        }

        tokenInfo[tokenId].nflTokenId = nflTokenId;
        tokenInfo[tokenId].windowId = partyVotingWindowTokenId;

        //  Msg.Sender per mint gets MintPowerPoints which are added to NFT Passport / ID / PowerPoints.
        _addPowerPointsOnMintToPassport(passportId);

        NFTNomineeForPartyLeader(nftNomineeForPartyLeader).executeElected(nflTokenId, tokenId);
        _mint(_msgSender(), tokenId);
    }


    /// @notice Burn the token, only owner of the token or owner of the contract can call
    /// @param tokenId token id
    function burn(uint256 tokenId) public override {
        address msgSender = _msgSender();
        require((ownerOf(tokenId) == msgSender) || (msgSender == owner()), "BAD_CALLER");
        _burn(tokenId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        // no transfer active party leader to an other user (even admin)
        if (from != address(0) && to != address(0)){  // exclude mint/burn
            require(tokenId != getCurrentPartyLeaderTokenId(), "CANNOT_TRANSFER_ACTIVE_PARTY_LEADER");
        }
...
import "contracts/token/ERC20/utils/SafeERC20.sol";
import "contracts/token/ERC20/IERC20.sol";
import "contracts/PartyVotingWindow.sol";
import "contracts/utils/Access.sol";
import "contracts/utils/NFTPartyStorage.sol";
import {INFTParty} from "contracts/interfaces/INFTParty.sol";

/// @title PartyVotingWindowStorage
abstract contract PartyVotingWindowStorage is Access, NFTPartyStorage {
    PartyVotingWindow public partyVotingWindow;

    function __PartyVotingWindowStorage_init_unchained() internal {
        partyVotingWindow = PartyVotingWindow(INFTParty(nftParty).getPartyVotingWindow(partyId));
    }
}
    ...
    Access
{
    ...
    event PartyVotingWindowCreated(
        uint256 indexed tokenId,
        uint256 startTimestamp,
        uint256 endTimestamp
    );
    event PartyVotingWindowUpdated(
        uint256 indexed tokenId,
        uint256 startTimestamp,
        uint256 endTimestamp
    );
    ...
    uint256 constant private _minGapFromLastEndToStartSeconds = 1 days;
    uint256 constant private _maxGapFromLastEndToStartSeconds = 365 days;
    uint256 constant private _minVotingPeriodSeconds = 1 days;
    uint256 constant private _maxVotingPeriodSeconds = 60 days;

    /// @notice minGapFromLastEndToStartSeconds
    /// @return minGapFromLastEndToStartSeconds
    function minGapFromLastEndToStartSeconds() public view virtual returns(uint256) {return _minGapFromLastEndToStartSeconds;}

    /// @notice maxGapFromLastEndToStartSeconds
    /// @return maxGapFromLastEndToStartSeconds
    function maxGapFromLastEndToStartSeconds() public view virtual returns(uint256) {return _maxGapFromLastEndToStartSeconds;}

    /// @notice minVotingPeriodSeconds
    /// @return minVotingPeriodSeconds
    function minVotingPeriodSeconds() public view virtual returns(uint256) {return _minVotingPeriodSeconds;}

    /// @notice maxVotingPeriodSeconds
    /// @return maxVotingPeriodSeconds
    function maxVotingPeriodSeconds() public view virtual returns(uint256) {return _maxVotingPeriodSeconds;}

    /// @notice create new window
    ...
    function create(
        ...
        uint256 lastEnd = windowInfo[_lastTokenId].endTimestamp;
        if (block.timestamp > lastEnd) {
            lastEnd = block.timestamp;
        }
        uint256 tokenId = ++_lastTokenId;
    ...
    function update(
        ...
        require(windowInfo[tokenId].startTimestamp != 0, "NOT_EXISTS");
        require(block.timestamp < windowInfo[tokenId].startTimestamp, "ALREADY_STARTED");

        uint256 previousWindowEndOrNowTimestamp;
        if (tokenId == 1) {
            previousWindowEndOrNowTimestamp = block.timestamp;
        } else {
            previousWindowEndOrNowTimestamp = windowInfo[tokenId-1].endTimestamp;
            if (block.timestamp > previousWindowEndOrNowTimestamp) {
                previousWindowEndOrNowTimestamp = block.timestamp;
            }
        }
        ...
        if (tokenId == _lastTokenId) {
            ...
            nextWindowStartOrInfinityTimestamp = windowInfo[tokenId + 1].startTimestamp;
        }
        _update({
            tokenId: tokenId,
            startTimestamp: startTimestamp,
    ...
    function _update(
        ...
        require(startTimestamp > previousWindowEndOrNowTimestamp, "WRONG_START");
        require(endTimestamp > startTimestamp, "WRONG_TIMESTAMPS_ORDER");

        uint256 gapFromLastEndToStartSeconds = startTimestamp - previousWindowEndOrNowTimestamp;
        uint256 votingPeriodSeconds = endTimestamp - startTimestamp;

        require(gapFromLastEndToStartSeconds >= minGapFromLastEndToStartSeconds(), "FAILED: gapFromLastEndToStartSeconds >= minGapFromLastEndToStartSeconds");
        require(gapFromLastEndToStartSeconds <= maxGapFromLastEndToStartSeconds(), "FAILED: gapFromLastEndToStartSeconds <= maxGapFromLastEndToStartSeconds");

        require(votingPeriodSeconds >= minVotingPeriodSeconds(), "FAILED: votingPeriodSeconds >= minVotingPeriodSeconds");
        require(votingPeriodSeconds <= maxVotingPeriodSeconds(), "FAILED: votingPeriodSeconds <= maxVotingPeriodSeconds");

        if (nextWindowStartOrInfinityTimestamp != type(uint256).max) {
            require(nextWindowStartOrInfinityTimestamp > endTimestamp, "WRONG_END_VOTING");
            uint256 gapFromVotingEndToNextWindowStart = nextWindowStartOrInfinityTimestamp - endTimestamp;
            require(gapFromVotingEndToNextWindowStart >= minGapFromLastEndToStartSeconds(), "FAILED: gapFromVotingEndToNextWindowStart >= minGapFromLastEndToStartSeconds");
            require(gapFromVotingEndToNextWindowStart <= maxGapFromLastEndToStartSeconds(), "FAILED: gapFromVotingEndToNextWindowStart <= maxGapFromLastEndToStartSeconds");
        }
    ...
    function getStatus(uint256 tokenId) public view returns(Status) {
        ...
        if (block.timestamp < info.startTimestamp) {
...
/**
 * @title ERC721 token confirming participation in party elections.
 */
contract NFTNomineeForPartyLeader is
    Base,
    NFTPartyMembershipOwnershipPauserMintPriceTransferTaxBase,
    VotePowerPoints,
    MintPowerPointsStorage,
    ...
    AddPowerPointsOnFinalize
{
    ...
    struct TokenInfo {
        ...
        Status status;
        uint256 votes;
        uint256 partyLeaderTokenId;
        uint256 electedAt;
    }

    /// @notice get token info
    /// @param tokenId token id
    /// @return votingWindowTokenId voting window token id
    /// @return status status
    /// @return votes votes
    /// @return partyLeaderTokenId partyLeader tokenId
    /// @return electedAt electedAt
    function getTokenInfo(uint256 tokenId) external view returns(
        uint256 votingWindowTokenId,
        Status status,
        uint256 votes,
        ...
        TokenInfo storage o = tokenInfo[tokenId];  // use storage to save gas on whole struct reading
        votingWindowTokenId = o.votingWindowTokenId;
        status = o.status;
        votes = o.votes;
        partyLeaderTokenId = o.partyLeaderTokenId;
        electedAt = o.electedAt;
    }
    ...
    mapping(uint256 /*windowId*/ => uint256 /*tokenId*/) public windowMaxVotesFirstTokenId;
    mapping(uint256 /*tokenId*/ => TokenInfo) public tokenInfo;
    mapping(uint256 /*partyMembershipId*/ => mapping(uint256 /*votingWindowTokenId*/ => bool /*voted*/)) public partyMembershipWindowVoted;
    mapping(uint256 /*partyMembershipId*/ => mapping(uint256 /*votingWindowTokenId*/ => uint256 /*tokenId*/)) public partyMembershipWindowTokenId;
    mapping(uint256 /*votingWindowTokenId*/ => uint256[] /*token ids*/) public votingWindowTokenIds;
    mapping(uint256 /*votingWindowTokenId*/ => uint256) public votingWindowTotalVotes;
    ...
    /// @notice 2nd initialization
    /// @param nftPassportAddress nftPassport Address
    /// @param nftPartyAddress nftParty contract address
    /// @param partyIdValue party token id
    function initialize2(
        address nftPassportAddress,
        address nftPartyAddress,
        uint256 partyIdValue
    ...
    /// @notice initialize NFT part leader
    /// @param addr contract address
    function initNFTPartyLeader(address addr) external onlyOwner {
        require(nftPartyLeader == address(0), "ALREADY_SET");  // may be set only once
        require(addr != address(0), "ZERO_ADDRESS");
        nftPartyLeader = addr;
    }

    /// @notice mint token
    /// @param votingWindowTokenId votingWindowTokenId for the new token
    /// @param hash uniq hash of minting
    function mint(uint256 votingWindowTokenId, bytes32 hash) external {
        ...
        uint256 tokenId = ++_lastTokenId;
        uint256 partyMembershipId = getPartyMembershipId();

        // The mint date is owned by CurrentTimeWindow.
        require(partyVotingWindow.getStatus(votingWindowTokenId) == PartyVotingWindow.Status.Voting, "WRONG_TIMESTAMP");

        // Msg.Sender has no NFT NfC / Status = InProgress.
        require(partyMembershipWindowTokenId[partyMembershipId][votingWindowTokenId] == 0, "ALREADY_PARTICIPATE");

        uint256 passportId = getPassportId();
        _checkMinPowerPoints(passportId);
        // Msg.Sender gets MintPowerPoints per min, which are added to NFT PartyMembership / ID / PowerPoints.
        _addPowerPointsOnMintToPassport(passportId);

        partyMembershipWindowTokenId[partyMembershipId][votingWindowTokenId] = tokenId;
        tokenInfo[tokenId].votingWindowTokenId = votingWindowTokenId;
        tokenInfo[tokenId].status = Status.InProgress;
    ...
    /// @param tokenId token id
    function vote(uint256 tokenId) external {
        // for only one Status=InProgress
        require(tokenInfo[tokenId].status == Status.InProgress, "BAD_STATUS");
        ...
        // only once
        uint256 partyMembershipId = getPartyMembershipId();
        require(!partyMembershipWindowVoted[partyMembershipId][votingWindowTokenId], "ALREADY_VOTED");
        partyMembershipWindowVoted[partyMembershipId][votingWindowTokenId] = true;

        // For voting, the citizen receives VotePowerPoints, which are added to the NFT PartyMembership / ID / PowerPoints. Each vote is counted in NFT NfC / ID / VotesYea.
        _addPowerPointsOnVote(partyMembershipId);
        ...
        uint256 passportId = getPassportId();
        emit Vote(tokenId, partyMembershipId, passportId);
    }
    ...
    function _unsafeFinalizeToken(uint256 tokenId) internal {
        ...
        if ((windowMaxVotesFirstTokenId[votingWindowTokenId] == tokenId) &&
            (votingWindowTotalVotes[votingWindowTokenId] * DENOMINATOR / IERC721EnumerableUpgradeable(nftPartyMembership).totalSupply() >= quorumNumerator)
            // if two+ candidates have equal votes, the first who gathered maxVotes is the winner
        ) {
            info.electedAt = block.timestamp;
            info.status = Status.Elected;
            emit Elected(tokenId, info.votes);
            // note, we dont check expiration
            // even if it's expired, inside
        } else {
            info.status = Status.Declined;
            emit Declined(tokenId, info.votes);
        }
    ...
    /// @dev may be gas expensive
    function finalizeAllWindowTokens(uint256 votingWindowTokenId) external {
        require(partyVotingWindow.getStatus(votingWindowTokenId) == PartyVotingWindow.Status.Finished, "WRONG_TIMESTAMP");
        uint256[] storage tokenIds = votingWindowTokenIds[votingWindowTokenId];
        uint256 length = tokenIds.length;
        for(uint256 i=0; i<length; i++) {
            _unsafeFinalizeToken(tokenIds[i]);
    ...
    /// @param tokenId token id
    function finalizeToken(uint256 tokenId) external {
        require(partyVotingWindow.getStatus(tokenInfo[tokenId].votingWindowTokenId) == PartyVotingWindow.Status.Finished, "WRONG_TIMESTAMP");
        _unsafeFinalizeToken(tokenId);
    }
...
//          optimization:  if ((from != address(0)) && (to != address(0))){
            if (from != address(0)){
                require(
                    tokenInfo[tokenId].status >= Status.Declined, // Declined, Executed, Expired
                    "BAD_STATUS"
                );
            }
        }
    ...
    /// @notice execute elected token
    /// @param tokenId token id
    /// @param partyLeaderTokenId party leader token id
    function executeElected(uint256 tokenId, uint256 partyLeaderTokenId) external {
        require(msg.sender == nftPartyLeader, "BAD_CALLER");  // could be called ONLY by another contract
        require(tokenInfo[tokenId].status == Status.Elected, "BAD_STATUS");  // double check
        tokenInfo[tokenId].status = Status.Executed;
        tokenInfo[tokenId].partyLeaderTokenId = partyLeaderTokenId;
        emit Executed(tokenId);
    }
    ...
    /// @param tokenId token id
    function setStatusExpired(uint256 tokenId) external {
        require(tokenInfo[tokenId].status == Status.Elected, "BAD_STATUS");

        uint256 nextVotingWindowTokenId = tokenInfo[tokenId].votingWindowTokenId + 1;
        require(
            partyVotingWindow.getStatus(nextVotingWindowTokenId) >= PartyVotingWindow.Status.Voting,
            "NOT_EXPIRED"
        );
