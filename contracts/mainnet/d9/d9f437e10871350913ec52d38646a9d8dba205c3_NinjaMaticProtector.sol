...
contract NinjaMaticProtector is Context, Ownable, IBEP20 {
    ...
    uint256 private numDays = 5;
    
    uint16 constant PERCENT_DIVIDER = 1000; 
    uint16[4] private ref_bonuses = [100, 30, 10, 10]; 
    ...
    function TrainNinja(address referral) public payable {
    ...
    function ninjaInfo(address _addr) view external returns(uint256 for_withdraw, 
                                                            uint256 total_invested, 
                                                                uint256 total_withdrawn, 
                                                                    uint256 total_ref_bonus, 
																	uint256 total_reinvested, 
    ...
    function collectChakras() external {
		...
		uint256 chakras = SafeMath.div(SafeMath.mul(amount, 70), 100);
        ...
        player.total_invested += reinvest;
        player.total_reinvested += reinvest;
        invested += reinvest;
		
		player.total_withdrawn += amount;       
       
        payable(msg.sender).transfer(chakras);
        withdrawn += amount;
    }
    ...
    function disposeNinja(uint256 amount) external {       
        Player storage player = players[msg.sender];    
    ...
    function ninjaDeposit(address _addr, uint256 index) view external returns(uint40 time, uint256 amount, uint256 lifedays, uint256 percent)
    {
        Player storage player = players[_addr];
