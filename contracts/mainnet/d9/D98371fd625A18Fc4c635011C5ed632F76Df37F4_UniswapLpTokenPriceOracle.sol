...
contract UniswapLpTokenPriceOracle {
    ...
    function _price(address token) internal view virtual returns (uint256) {
        ...
        return
            sqrtK.mul(2).mul(sqrt(price0)).div(2**56).mul(sqrt(price1)).div(
                2**56
            );
    }
