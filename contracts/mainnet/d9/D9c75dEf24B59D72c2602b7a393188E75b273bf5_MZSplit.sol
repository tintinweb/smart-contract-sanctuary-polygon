...
contract MZSplit is Admin {
    ...
    function depositWithAffiliate(string memory tokenSymbol, uint256 value, string memory sku, address payable affiliate, uint256 affiliateCut) public payable {
        require(affiliate != address(0), "Affilate address not provided");
        // Check is ETH
        bool isETH = keccak256(bytes(tokenSymbol)) == keccak256(bytes("ETH"));
        require(isETH || _supportedTokens[tokenSymbol].deployed != address(0), "Token not supported");
        require(value > 0, "No amount provided");
        require(affiliateCut <= _affiliateCutMax, "Invalid affiliate cut");
        ...
        for(uint8 i=0; i<_splitCount; i++) {
            ...
            uint256 valueCut = value.mul(uint256(_splits[i].cut).sub(affiliateCut.div(_splitCount))).div(_sumCuts);
            // Increment recipient balance
            _splits[i].balance[tokenSymbol] += valueCut;
            // Track used up value
            valueUsed += valueCut;
        ...
        uint256 affiliateBalance = value - valueUsed;
        ...
        // ERC20Token
            ...
            erc20.transfer(affiliate, affiliateBalance);
        }
        // Fire withdraw event
        emit Withdraw(affiliate, tokenSymbol, affiliateBalance);
    }
