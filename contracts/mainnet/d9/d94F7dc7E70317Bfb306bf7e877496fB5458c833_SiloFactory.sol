...
contract SiloFactory is Ownable, ERC721Enumerable{
    ...
    event NewSilo(address owner, uint siloID, string  name, string  strategyName, uint category,  uint delay);
    event UpdateSiloStrategy(address owner, uint siloID, uint index,bytes configurationData,address implementation);

    // event SetStrategy( uint siloID, string  strategyName, uint category);
    
    event SiloDeposit(uint siloID,uint[4] amount);
    
    event SiloManualHarvest( uint siloID);
    event SiloWithdraw(uint siloID);
    
    event SiloWithdrawToken(uint siloID, address[] tokens);

    event SiloExit(uint siloID,uint amount,uint8 exitType);

    event SiloStatusUpdate(uint siloID, uint8 status);


    event SiloName(uint siloID,string name);
    event SiloDelayUpdate(uint siloID,uint delay);
    ...
    function withdrawTokensFromSilo( uint siloID, address[] memory tokens) external isOwner(siloID){
        ...
        emit SiloWithdrawToken(siloID, tokens);
    }
    ...
    function adjustSiloStrategy(uint siloID, uint _index, bytes memory _configurationData, address _implementation) external isOwner(siloID){
        ...
        ISiloSubFactory(subFactory).checkActionLogicValid(msg.sender,_implementation,_configurationData);

        ISilo silo = ISilo(siloMap[siloID]);
        silo.adjustStrategy(_index,  _configurationData, _implementation);

        emit UpdateSiloStrategy(msg.sender, siloID, _index,_configurationData,_implementation);
    }
    ...
    function deploySiloAndSetStrategy(string memory _name, string memory _strategyName, uint _category,  uint _delay, address[4] memory _inputs, address[] memory _actions, bytes[] memory _configurationData) external{
        ISiloSubFactory(subFactory).checkActionsLogicValid(msg.sender,_actions,_configurationData);
        ...
        emit NewSilo(msg.sender, siloId, _name, _strategyName,_category,_delay);
    ...
    function _setActionStack(uint siloID, string memory _strategyName, uint _category, address[4] memory _inputs, address[] memory _actions, bytes[] memory _configurationData) internal{
        ...
        require(factory.managerApproved(msg.sender), "Gravity Finance: no approved manager");
        ...
        if(silo.highRiskAction()){//if silo is running a high risk strategy then make sure the recievers manager has enough Link
            require(factory.getUpkeepBalance(msg.sender) >= factory.getMinimumUpkeepBalance(msg.sender), "small upkeep balance for high risk strategies");
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        if(enforceBeforeTransfer && to != address(0)){//if we aren't burning
            ...
            require(factory.managerApproved(to), "Gravity Finance: no approved manager");
            require(balanceOf(to) < maxSilosPerUser, "Gravity Finance: over max number of silos");
            if(ISilo(siloMap[tokenId]).highRiskAction()){//if silo is running a high risk strategy then make sure the recievers manager has enough Link
                require(factory.getUpkeepBalance(to) >= factory.getMinimumUpkeepBalance(to), "small upkeep balance for high risk strategies");
            }
        }
    }
