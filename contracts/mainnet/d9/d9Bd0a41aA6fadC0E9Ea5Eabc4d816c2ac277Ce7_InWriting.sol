...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    uint256 royalty = 0; // "0%", if it was 10 it would be "10%, 20 would be 20%, etc..."
        ...
        returns (bool)
    {
        ...
        payable(seller).transfer(seller_price);
    ...
    function update_royalty(uint256 newRoyalty) public onlyOwner {
        royalty = newRoyalty;
    }
...
contract InWriting is ERC721 {
        ...
        returns (uint256)
    {
        ...
        if (msg.value > cost) {
            payable(addr).transfer(msg.value - cost);
        }
