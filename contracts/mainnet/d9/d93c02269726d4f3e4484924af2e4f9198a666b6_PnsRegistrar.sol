...
contract Computation {
    function computeNamehash(string memory _name) public pure returns (bytes32 namehash) {
        ...
        namehash = keccak256(
        abi.encodePacked(namehash, keccak256(abi.encodePacked('pol')))
        );
        namehash = keccak256(
...
contract PnsRegistrar is Computation, PnsAddressesImplementation {
    ...
    function _register(Register memory register) internal {
        ...
        string memory name = toLowerCase(register.name);

        require(register.expiration != 0, "Invalid duration.");
        require(checkString(name) == true, "Invalid string.");
        bytes32 _hash = computeNamehash(name);
        ...
        if (currentTokenID == 0) {
            ...
            pnsRegistry.setRecord(_hash, _tokenId, name, register.expiration);
        } else {
