...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function deposit(
        ...
        for (uint i=0; i<users[msg.sender].userContList.length;i++) {
            ...
            if (collaAddr==contList[contId].collaAddr && USDAddr==contList[contId].USDAddr){
                //有欠款的先还款
                if(contState==22){
                    uint overdraft = contList[contId].USDAmount;
                    if (toAssetAmount>=overdraft){
                        ...
                        //还一部分欠款
                        contList[contId].USDAmount -= toAssetAmount;
                        toAssetAmount = 0;
                        emit overdraftLog(contId,overdraft,contList[contId].contState);
                    }
                }
    ...
    function withdraw(address USDAddr,address stableAddr,uint USDAmount)public {
        ...
        for (uint i=0; i<users[msg.sender].userContList.length;i++) {
            ...
            if (USDAddr==contList[contId].USDAddr){
                ...
                if(contState==22){
                    collaAmount +=  contList[contId].USDAmount;
                }
            }
        }
    ...
    function tradToProperty(uint orderId,address userAddr,address tokenAddr,uint amount) public {
        ...
        for (uint i=0;i<lengthNum;i++){
            ...
            if (contList[contId].contState==22 && contList[contId].USDAddr == tokenAddr){
                uint overdraft = contList[contId].USDAmount;
                if(amount>=overdraft){
                    amount -= overdraft;
                    contList[contId].contState = 23;
   ...
   function getUserAsset(address userAddr) public view returns(string memory){
    ...
    function getUserCont(address userAddr) public view returns(string memory){
        ...
        for (uint i=0;i<lengthNum;i++){
            ...
            if (USDAmount > 0 || collaAmount > 0){
                ...
                conList = string(abi.encodePacked(conList,uint2str(contList[contId].contState)));
                conList = string(abi.encodePacked(conList,','));
                ...
                if (USDAmount==0 || collaRate==liquRate){
                    ...
                    uint nowRate = USDAmountEst * deno/USDAmount;
                    if (nowRate >= liquRate && collaRate > liquRate){
                        factorH = (nowRate - liquRate) * deno / (collaRate - liquRate);
    ...
    function collaToValue(address collaAddr, uint amount) public view returns(string memory){
        ...
        uint collaRate = collaList[collaAddr].collaRate;
        uint value = (USDAmount * deno / collaRate) * deno / (10**decimals);
    ...
    //合同差的抵押物
    function contToColla(uint contId) public view returns(uint){
        uint liquiAmount = contList[contId].USDAmount + contList[contId].inteConf + getInterest(contId,0);
        uint userAsset = users[contList[contId].userAddr].asset[contList[contId].USDAddr];
        uint collaAmount;
    ...
    function liquiContCollaAmount(address userAddr,uint contId) public view returns(string memory){
        ...
        require(contList[contId].userAddr == userAddr,"CONTID_ERROR_2");

        uint amount = contToColla(contId);
        uint collaAmount = (((amount * 103) /100) * deno) / (10 ** collaList[contList[contId].collaAddr].decimals);
    ...
    function liquiContColla(uint contId, uint amount) public {
        ...
        uint collaAmount = contToColla(contId);
        require(amount <= (collaAmount*110/100),"colla_amount_too_hight");
        ...
        uint userAsset = users[msg.sender].asset[contList[contId].USDAddr];
        setUserAsset(msg.sender,contList[contId].USDAddr,amountOut,0,8);
        
        repayment(contId,amountOut+userAsset);
    }

    event liquiContractLog(uint contId, uint collaAmount, uint amountOut);
    //清算
    function liquiContract(uint contId) public {
        require(contList[contId].contState == 0,"donnt_liqui");
        ...
        uint userAsset = users[userAddr].asset[USDAddr];
        uint amountOut = sushiSwapper(getPath(collaAddr,USDAddr),collaAmount);
        if ((amountOut + userAsset) >= (USDAmount+inteAmount)){
            //剩余的转入资金帐户
            users[userAddr].asset[USDAddr] = amountOut + userAsset - USDAmount - inteAmount;
            contList[contId].USDAmount = 0;
            contList[contId].contState = 21;//清算完成
        }else{
            contList[contId].USDAmount = USDAmount + inteAmount - amountOut - userAsset;
            contList[contId].contState = 22;//清算完成并欠款
            users[userAddr].asset[USDAddr] = 0;
        }
    ...
    function updateCont(uint contId,uint overdraft) public {
        contList[contId].USDAmount =  overdraft;
