...
contract Mail is OwnableUpgradeable {
    ...
    function _processMail(uint types, uint rewardId, uint amount, bool isTax) internal {
        if (types == 1) {
            ...
            urBox.mint(msg.sender, rewardId);
        }
    }
}
...
contract UrBox is OwnableUpgradeable{
    ...
    function setMinters(address addr,uint allowance) external onlyOwner{
        minters[addr] = allowance;
    }
    
    function balanceOf(address addr,uint boxId) public view returns(uint){
        return _balance[addr][boxId];
    }
    
    function checkBoxItemInfo(uint boxId) public view returns(uint[] memory,uint[] memory){
        return(boxInfo[boxId].itemList,boxInfo[boxId].itemAmount);
    }
    
    function balanceOfBatch(address addr,uint[] memory boxIds) public view returns(uint[] memory){
        uint[] memory list = new uint[](boxIds.length);
        for(uint i = 0; i < boxIds.length; i++){
            list[i] = balanceOf(addr,boxIds[i]);
        }
        return list;
    }
    
    function mint(address addr,uint boxID) external{
        if(msg.sender != superMinter){
            require(minters[msg.sender] > 0,'not mitnter');
            minters[msg.sender] --;
        }
        _balance[addr][boxID] ++;
        emit Mint(addr,boxID);
    ...
    function openBox(uint boxId,uint amount) external{
        require(_balance[msg.sender][boxId] >= amount,'out of balance');
