...
contract VRFv2Consumer is VRFConsumerBaseV2 {
  ...
  function getSeed(uint256 _s) external view returns(uint256){
    require(allowMap[msg.sender] == true, "Permission Denied");
    uint256 salt = uint256(keccak256(abi.encode(block.difficulty, block.timestamp)));
    uint256 s;
    if (s_randomWords[_s] >= s_randomWords[_s+1]) {
      s = s_randomWords[_s+1];
    } else {
      s = s_randomWords[_s];
    }
    if (s >= salt) {
