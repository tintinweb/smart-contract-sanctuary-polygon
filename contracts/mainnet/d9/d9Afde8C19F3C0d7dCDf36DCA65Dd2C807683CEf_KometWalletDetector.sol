...
contract KometWalletDetector {
	SmartWalletFactory factory;

	constructor(address _factory) {
		factory = SmartWalletFactory(_factory);
	}
	
	function isKometWallet(address _wallet) public view returns (bool) {
		require(factory.contracts(_wallet), "FINT: This wallet is not a Komet Wallet");
		return true;
 ...
 * @notice Factory Contract to deploy new KometWallets (BaseWallet)
 */
contract SmartWalletFactory {
	...
	constructor (address _kometWallet) {
		kometWallet = _kometWallet;
	}

	function createWallet(string memory name) public {
		BaseWallet smartWallet = new BaseWallet(name, msg.sender, kometWallet);
		contracts[address(smartWallet)] = true;
		vaults[msg.sender].push(address(smartWallet));
		emit WalletCreated(msg.sender, address(smartWallet));
	}

	function fetchWallets(address owner) public view returns (address[] memory) {
        return vaults[owner];
    }
}
