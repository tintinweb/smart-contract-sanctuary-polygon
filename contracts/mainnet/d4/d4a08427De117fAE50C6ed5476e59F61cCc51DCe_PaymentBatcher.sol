...
contract PaymentBatcher is Initializable, IPaymentBatcher, WhitelistConsumer {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    bytes1 public constant OPERATOR_WALLETS_WHITELIST = 0x01;
    ...
    function initialize(
        address _operatorWalletsWhitelistAddress
    ) public initializer {
        _setWhitelistAddress(
            _operatorWalletsWhitelistAddress,
            OPERATOR_WALLETS_WHITELIST
        );
    }
    ...
    function executeBatchPayment(
    ...
    ) external isWhitelistedOn(OPERATOR_WALLETS_WHITELIST) {
        require(
            receivers.length == amounts.length,
            'RoyaltyPaymentExecutor: addresses and amounts length mismatch'
        );
        ...
        emit BatchPaymentExecuted(msg.sender, token, receivers, amounts);
    }

    function setOperationalWhitelistAddress(
        address _operationalWhitelistAddress
    ) external isInitialized isWhitelistedOn(OPERATOR_WALLETS_WHITELIST) {
        _setWhitelistAddress(
    ...
    ) public isWhitelistedOn(OPERATOR_WALLETS_WHITELIST) {
