...
contract MusicNFT is ERC1155, Ownable {
    ...
    string private _uri =
        "ipfs://QmahQFdHvMRZqR8HQYwFsWNHc6ASS2whgW5PT8nYyvfHUT/metadata/{id}.json";
    ...
    modifier onlyCreatorOrAgent() {
        require(
            creator == _msgSender() || _agent[_msgSender()],
            "This is not allowed except for creator or agent"
        );
        _;
    }
    modifier supplyCheck(uint256 _tokenId, uint256 _amount) {
        require(
            _supplyOfEach[_tokenId] + _amount <= _AMOUNT_OF_MAX_MINT[_tokenId],
            "Max supply reached"
        );
        _;
    }
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(
                _supplyOfEach[_tokenIds[i]] + _amounts[i] <=
                    _AMOUNT_OF_MAX_MINT[_tokenIds[i]],
                "Max supply reached"
            );
        }
        _;
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(
                _tokenIds[i] >= minMintId && _tokenIds[i] <= maxMintId,
                "tokenId is not allowed on this chain"
            );
            _supplyOfEach[_tokenIds[i]] += _amounts[i];
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0) || to == address(0) || from != creator) {
            return;
        }
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            if (creator == _msgSender() || _agent[_msgSender()]) {
                ...
                require(
                    _isAuthenticated[to],
                    "This address is not authenticated"
                );
                if (ids[i] == 3) {
                    require(
                        balanceOf(to, ids[i]) + amounts[i] <= 1,
                        "Can't buy same songs more than two record"
                    );
                } else {
                    require(
                        balanceOf(to, ids[i]) + amounts[i] <= 2,
                        "Can't buy same songs more than two record"
                    );
                }
    ...
    function addAllowlist(address[] memory allowAddr)
        public
        onlyCreatorOrAgent
    {
    ...
    function EMGreveal(string memory _EMGuri) public onlyCreatorOrAgent {
        _setURI(_EMGuri);
    }
