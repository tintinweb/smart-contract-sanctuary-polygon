//  _____             _   _                _____ _
// |_   _|___ _ _ ___| |_| |___ ___ ___   |     |_|___ ___ ___
//   | | |  _| | |_ -|  _| | -_|_ -|_ -|  | | | | |   | -_|_ -|
//   |_| |_| |___|___|_| |_|___|___|___|  |_|_|_|_|_|_|___|___|
//
// Trustless Mines by ChainHackers
// Experience the Thrill of Trustless Betting. Welcome to the Future.
...
contract Mines is IMines, VRFV2WrapperConsumerBase, Ownable {
    struct Game {
        address player;
        uint256 wager;
        address wagerToken;
    ...
    function startAndReveal(uint8 minesNum, uint256 wager, uint8 selectionIndex, address tokenAddress) external payable {
        ...
        require(minesNum > 0 && minesNum < CELLS_NUM, "Mines: minesNum must be between 1 and 24");
    ...
    function cashout(uint256 gameId) external {
        require(msg.sender == games[gameId].player, "Mines: you can do this only in games you started!");
        uint256 prize = prize(games[gameId].wager, games[gameId].revealedNum);
    ...
    function collapse() external onlyOwner {
        // it's OK to have the address hardcoded,
        // this function should go before any production deploy happens
    ...
    function gameStatus(uint256 gameId) external view returns (GameStatus){
        if (games[gameId].wager == 0) {
            return GameStatus.None;
        }
        if (isBetInFlight(gameId)) {
            return GameStatus.BetInFlight;
        }
    ...
    function _reveal(uint256 gameId, uint8 selectionIndex) private {
        require(selectionIndex < CELLS_NUM, "Mines: selectionIndex must be between 0 and 24");
        //TODO not completely safe, reserve prize money for the bet-in-flight period
        require(balance(gameId) >= prize(games[gameId].wager, games[gameId].revealedNum + 1), "Mines: not enough balance to pay the next prize");
    ...
    function isBetInFlight(uint256 gameId) private view returns (bool){
        return games[gameId].pendingRequest > 0;
    }

    function balance(uint256 gameId) private view returns (uint256){
        if (games[gameId].wagerToken == address(0)) {
            return address(this).balance;
        }
        return IERC20(games[gameId].wagerToken).balanceOf(address(this));
    }

    function prize(uint256 wager, uint8 revealedNum) private pure returns (uint256) {
        return wager * 99 * CELLS_NUM / (CELLS_NUM - revealedNum) / 100;
    }
}
