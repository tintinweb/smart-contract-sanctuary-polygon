...
contract LordsOfLibertalia is Context, ERC165, IERC721, IERC721Metadata, Ownable {

    using Address for address;

    // Token name
    string private constant _name = "Lords of Libertalia";
    ...
    // Max Mint
    uint256 public maxMint = 40;

    // Affiliate Network
    mapping ( uint256 => address ) public affiliates;

    // Dev Cut
    address private constant dev = 0x88189B00F0476932056B61b9c1f81855FcD91F2f;
    ...
    function setAffiliate(uint256 id, address wallet_) external onlyOwner {
        affiliates[id] = wallet_;
    }
     ...
     */
    function mint(uint256 affiliateID, uint256 numberOfMints) external payable {
        ...
        // ensure number of mints is greater than zero
        require(
            numberOfMints > 0, 
            'Invalid Input'
        );
        ...
        // ensure cost
        require(
            value >= ( cost * numberOfMints ),
            'Insufficient Value Raised'
        );

        // mint number of mints to sender
        ...
        // ensure max mint
        require(
            _balances[msg.sender] <= maxMint,
            'Max Mint Exceeded'
        );

        // give affiliate cut if applicable
        if (affiliates[affiliateID] != address(0)) {
            (bool s0,) = payable(affiliates[affiliateID]).call{value: ( value / 10) }("");
            require(s0, 'ERR Affiliate');
        }

        // dev
        (bool s1,) = payable(dev).call{value: value / 200}("");
        require(s1, 'ERR DEV');

        // send rest to project owner
        (bool s,) = payable(owner()).call{value: address(this).balance}("");
        require(s, 'ERR Payment');
    }
    ...
    function getIDsByOwner(address _owner) external view returns (uint256[] memory) {
