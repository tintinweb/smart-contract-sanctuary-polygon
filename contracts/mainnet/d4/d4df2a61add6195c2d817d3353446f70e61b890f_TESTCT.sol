...
contract TESTCT is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
    ...
    function mintWhitelistSale(uint256 _mintAmount, bytes32[] calldata _merkleProof) external payable nonReentrant{
        require(!globalPause,"123: contract is paused");
        require((block.timestamp >= startWLMintTime && block.timestamp <= endWLtMintTime) || !pauseWLSale, "123: whitelistSale closed");
        require(totalSupply() + _mintAmount <= maxSupply, "123: maxSupply exceeded");
        require(_mintAmount == 1 || _mintAmount == 2, "123: mintAmount must be 1 or 2");
            
        bytes32 leaf = keccak256(bytes.concat(keccak256(abi.encode(msg.sender))));
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "123: Invalid proof");
        require(_mintAmount + addressMintedAmount[msg.sender] <= 2, "123: user cannot mint more then 2 NFTs" );
        ...
        if(addressMintedAmount[msg.sender] == 0 && _mintAmount == 1 ){
            ...
            require(msg.value == priceWhitelistSale,"123: user must send the exact price");
    ...
    function mintPublicSale(uint256 _mintAmount) external payable nonReentrant{
        require(!globalPause,"123: contract is paused");
        require((block.timestamp >= startPBMintTime && block.timestamp <= endPBMintTime) || !pausePBSale ,"123: publicSale closed");
        require(totalSupply() + _mintAmount <= maxSupply,"123: maxSupply exceeded");
        require(_mintAmount== 1 || _mintAmount == 2, "123: mintAmount must be 1 or 2");    
        require(_mintAmount + addressMintedAmount[msg.sender] <= 2,"123: user cannot mint more then 2 NFTs" );
        ...
        if(addressMintedAmount[msg.sender] == 0 && _mintAmount == 1 ){
            ...
            require(msg.value == pricePublicSale,"123: user must send the exact price");
    ...
    function mintBatch(address[] memory addresses, uint256 _mintAmounts) external onlyOwner{
        require(totalSupply() + addresses.length * _mintAmounts <= maxSupply,"123: maxSupply exceeded");
    ...
    function withdraw() public nonReentrant {
        require(msg.sender == owner() || msg.sender == payerAccount, "123: can be called only with the owner or payerAccount");
        uint256 balance = address(this).balance;
        require(balance != 0, "123: contract balance is zero");
    ...
    function sendViaCall(address payable _to, uint256 _amount) private {
        ...
        require(sent, "123: failed to send amount");
    }
