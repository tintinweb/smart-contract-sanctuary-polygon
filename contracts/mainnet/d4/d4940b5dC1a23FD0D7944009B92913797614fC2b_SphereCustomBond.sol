...
contract SphereCustomBond is Ownable {
    ...
    address immutable private sphereFinance;
    address immutable private subsidyRouter; // pays subsidy in OHM to custom treasury
    address private sphereTreasury; // receives fee
    ...
    constructor(
        ...
        address _sphereTreasury,
        address _subsidyRouter,
        address _initialOwner,
        address _sphereFinance,
        ...
        require( _sphereTreasury != address(0) );
        sphereTreasury = _sphereTreasury;
        require( _subsidyRouter != address(0) );
        subsidyRouter = _subsidyRouter;
        require( _initialOwner != address(0) );
        policy = _initialOwner;
        require( _sphereFinance != address(0) );
        sphereFinance = _sphereFinance;
    ...
    /**
     *  @notice change address of Sphere Treasury
     *  @param _sphereTreasury address
     */
    function changeSphereTreasury(address _sphereTreasury) external {
        require( msg.sender == sphereFinance, "Only Sphere Finance" );
        sphereTreasury = _sphereTreasury;
    }
    ...
    function deposit(uint _amount, uint _maxPrice, address _depositor) external returns (uint) {
        ...
        if(feeInPayout) {
            ...
            if(fee != 0) { // if fee, send to Sphere treasury
                payoutToken.safeTransfer(sphereTreasury, fee);
            }
        } else {
            customTreasury.sendPayoutTokens( payout );
            if(fee != 0) { // if fee, send to Sphere treasury
                principalToken.safeTransferFrom( msg.sender, sphereTreasury, fee );
            }
        }
    ...
    function trueBondPrice() public view returns ( uint price_ ) {
        price_ = bondPrice().add(bondPrice().mul( currentSphereFee() ).div( 1e6 ) );
    }
    ...
    /**
     *  @notice calculate user's interest due for new bond, accounting for Sphere Fee. 
    ...
    function payoutFor( uint _value ) public view returns ( uint _payout, uint _fee) {
        if(feeInPayout) {
            uint total = FixedPoint.fraction( _value, bondPrice() ).decode112with18().div( 1e18 );
            _fee = total.mul( currentSphereFee() ).div( 1e6 );
            _payout = total.sub(_fee);
        } else {
            _fee = _value.mul( currentSphereFee() ).div( 1e6 );
            _payout = FixedPoint.fraction( customTreasury.valueOfToken(address(principalToken), _value.sub(_fee)), bondPrice() ).decode112with18().div( 1e18 );
        }
    }

    /**
     *  @notice calculate current ratio of debt to payout token supply
     *  @notice protocols using Sphere Finance should be careful when quickly adding large %s to total supply
    ...
    /**
     *  @notice current fee Sphere takes of each bond
     *  @return currentFee_ uint
     */
    function currentSphereFee() public view returns( uint currentFee_ ) {
