...
contract Pandas is ERC721, Ownable, RandomlyAssigned {
  using Strings for uint256;

  uint256 public currentSupply = 0; //@dev do not change value here
  uint256 public _totalSupply = 1500;  //@dev total number of collection tokens (Change or remove)
  uint256 public baseCost = 5 ether; //@dev base minting cost (Change) "ether" represents "Matic"

  string public baseURI = "ipfs://QmUtQqNCaPkDymJCpY244HbGcvL9N6mtGPhXpzPUCsv4FM/";

  constructor() 
    ERC721("Panda~Monium", "PANDAS")
    RandomlyAssigned(1500,1) // Max. # of NFTs available; Start counting from 1 (instead of 0) "1500"change, "1" Do Not Change
    {
       for (uint256 a = 1; a <= 20; a++) {  //@dev allocates a set number of tokens for developer
            mint();
        }
    }
  ...
  function mint()//@dev if using hashlips minting dapp remove "totalSupply" call from mint function
      public
      payable
  {
