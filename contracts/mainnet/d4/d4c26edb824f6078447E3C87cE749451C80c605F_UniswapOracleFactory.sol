...
import "../interface/UniI.sol";
import "../interface/TokenI.sol";
import "./UniswapOracleInstance.sol";

contract UniswapOracleFactory is AccessControl {
  ...
  /**
    ...
    @param _uniFactoryAdd  as a usable contract instance within the UniswapOracleFactory
    @param _router is the address of the uniswap factory contract
    @param _wETH is the address of the native wrapped token
    **/
  constructor(
  ...
  **/
  function createNewOracle(address token) external onlyAdmin returns (address) {
  ...
  /**
    ...
    @param _token is the address of the MoneyMarketInstance whos asset price is being retrieved
    @param _amount u
    @return returns the price of the asset
    */
  function getUnderlyingPriceofAsset(address _token, uint256 _amount)
    external
    returns (uint256)
  {
    UniswapOracleInstance oracle = UniswapOracleInstance(
      instanceTracker[_token]
    );
  ...
  /**
    ...
    @param _token is the address of the MoneyMarketInstance whos asset price is being retrieved
    @param _amount is the address of the MoneyMarketInstance whos asset price is being retrieved
    @return returns the price of the asset
    **/
  function viewUnderlyingPriceofAsset(address _token, uint256 _amount)
    external
    view
    returns (uint256)
  {
...
@return returns the price of the asset
**/
  function getUnderlyingAssetPriceOfwETH(address _token, uint256 _amount)
    external
    returns (uint256)
  {
    UniswapOracleInstance oracle = UniswapOracleInstance(
  ...
  /**
    ...
    @return returns the price of the asset
    **/
  function viewUnderlyingAssetPriceOfwETH(address _token, uint256 _amount)
    external
    view
    returns (uint256)
  {
...
**/
  ...
  {
    address[] memory path;
    if (_tokenA == wETH_add || _tokenB == wETH_add) {
      path = new address[](2);
      path[0] = _tokenA;
      path[1] = _tokenB;
    } else {
...
**/
  ...
  constructor(
    ...
    if (_tokenA != _tokenB) {
      IUniswapV2Factory _fac = IUniswapV2Factory(_factory);
      IUniswapV2Pair _pair = IUniswapV2Pair(_fac.getPair(_tokenA, _tokenB));
      tokenA = _tokenA;
      pair = _pair;
      token0 = _pair.token0();
      token1 = _pair.token1();
  ...
  function update() public {
    ...
    if (timeElapsed >= PERIOD || firstRun == true) {
      ...
      if (token0 == tokenA) {
        ...
        price0Average = FixedPoint.uq112x112(uint224((diff0) / timeElapsed));

        uint256 diff1 = price1Cumulative.sub(price1CumulativeLast);
        price1Average = FixedPoint.uq112x112(uint224((diff1) / timeElapsed));
