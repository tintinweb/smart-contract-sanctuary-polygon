...
contract AaveWmaticYA is BaseYA, UniswapV2Giddy, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  address[] private usdcToToken;
  address[] private tokenToUsdc;
  address[] private giddyToToken;
  address[] private tokenToGiddy;
  address private token;
  address private amToken;

  function initialize(
    address _trustedForwarder,
    address[] calldata _usdcToToken,
    address[] calldata _tokenToUsdc,
    address[] calldata _giddyToToken,
    address[] calldata _tokenToGiddy,
    address _amToken,
    ...
    usdcToToken = _usdcToToken;
    tokenToUsdc = _tokenToUsdc;
    giddyToToken = _giddyToToken;
    tokenToGiddy = _tokenToGiddy;
    token = _tokenToUsdc[0];
    amToken = _amToken;
  ...
  function depositCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = calcPrice(SUSHI_ROUTER, usdcToToken, value - fap);
  }
  ...
  function deposit(bytes calldata auth, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    uint256 tokenValue = swapTokens(SUSHI_ROUTER, usdcToToken, address(this), value);
    ...
    if (!IERC20(token).approve(LENDING_POOL, tokenValue)) {
      revert();
    }
    ILendingPool(LENDING_POOL).deposit(token, tokenValue, address(this), 0);
  ...
  function depositGiddyCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = calcPrice(SUSHI_ROUTER, giddyToToken, value - fap);
  }
  ...
  function depositGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    uint256 tokenValue = swapTokens(SUSHI_ROUTER, giddyToToken, address(this), giddyValue);
  ...
  function withdrawCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = calcPrice(SUSHI_ROUTER, tokenToUsdc, sharesToValue(shares));
    amountOut -= fap;
  }
  ...
  function withdraw(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    ILendingPool(LENDING_POOL).withdraw(token, value, address(this));
    value = swapTokens(SUSHI_ROUTER, tokenToUsdc, address(this), value);
    value = deductFeeUsdc(value, fap);
  ...
  function withdrawGiddyCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = calcPrice(SUSHI_ROUTER, tokenToGiddy, sharesToValue(shares));
    amountOut -= fap;
  }
  ...
  function withdrawGiddy(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    ILendingPool(LENDING_POOL).withdraw(token, value, address(this));
    value = swapTokens(SUSHI_ROUTER, tokenToGiddy, address(this), value);
  ...
  modifier deductEarningsFee() {
    ...
    if (earnings > 0) {
      ...
      if (fee > 0) {
        ILendingPool(LENDING_POOL).withdraw(token, fee, address(this));

        uint256 tokenBalance = IERC20(token).balanceOf(address(this));
        if (tokenBalance >= COMPOUND_THRESHOLD) {
          swapTokens(SUSHI_ROUTER, tokenToUsdc, GIDDY_CONFIG.feeAccount(), tokenBalance);
        }
      }
