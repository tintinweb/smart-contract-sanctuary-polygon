...
contract Minati is ERC20, ERC20Burnable, Pausable, AccessControl {
    ...
    function BurnIT(address addr ,uint256 amt) external onlyRole(MINTER_ROLE) {
    ...
    function blacklistUser(address _target) external onlyRole(BLACKLIST_ROLE) {
        _blacklisted[_target] = true;
    }

    function removeBlacklist(address _target) external onlyRole(BLACKLIST_ROLE) {
        _blacklisted[_target] = false;
    }

 function addWhitelist(address[] calldata whitelist) external onlyRole(BLACKLIST_ROLE) {
    require(whitelist.length > 0, "Invalid Input");
    for(uint256 i = 0; i < whitelist.length; i++) {
        whitelisted[whitelist[i]] = true;
    }
}
function removeWhitelist(address[] calldata whitelist) external onlyRole(BLACKLIST_ROLE) {
    require(whitelist.length > 0, "Invalid Input");
    for(uint256 i = 0; i < whitelist.length; i++) {
    ...
    function _beforeTokenTransfer(
        ...
        require(
            !_blacklisted[to],
            "You cannot transfer beacause you have been black listed"
        );
        super._beforeTokenTransfer(from, to, amount);
        }
    }
