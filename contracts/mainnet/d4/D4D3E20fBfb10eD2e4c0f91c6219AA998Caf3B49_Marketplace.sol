...
/**
 * @title Marketplace
 * @notice Provides buying and selling of property tokens against baseCurreny and other configureable tokens
 * @dev Buying can be done with base and allowed currency, in order to sell, we need to have ERC4626 pool of that sell currency for liquidity
 */
    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// @notice to make sure that the delay has passed and maintainer can withdrawLiquidity
    modifier delayedLiquidityWithdrawal() {
        require(
            storageParams.pendingWithdrawalRequestTimestamp != 0 &&
                block.timestamp >=
                storageParams.pendingWithdrawalRequestTimestamp,
            "WithdrawLiquidity not requested yet or it is too early to WithdrawLiquidity"
        );
        storageParams.pendingWithdrawalRequestTimestamp = 0;
        _;
    }
     ...
     * @param sender address to be assigned the role of Maintainer
     * @param manager address of Manager.sol to grant roles
     */
    function initialize(
    ...
    /**
     * @param legalToken Address of Legal token
     * @return detaiks of Legal Property, see IMarketplace.sol
     */
     ...
     * @return liquidity of _token collected against _wLegalToken
     */
     ...
     * @return available asked WLegalToken in Marketplace + ERC4626Vault
     */
     ...
     * @return address of WLegal against LegalToken
     */
    ...
    function updateBuyFeePercentage(
        ...
        _require(
            _newPercentage > 0 &&
                _newPercentage <= storageParams.maxFeePercentage,
            Errors.INVALID_FEE_PERCENTAGE
        );
        storageParams.buyFeePercentage = _newPercentage;

        emit BuyFeesUpdated(_newPercentage);
    }
    ...
    function updateSellFeePercentage(
        ...
        emit SellFeesUpdated(_newPercentage);
    }

    /**
     * @return fees percentage on buy
     */
    function getBuyFeePercentage() external view returns (uint256) {
    ...
    /**
     * @return fees percentage on sell
     */
    function getSellFeePercentage() external view returns (uint256) {
    ...
    function setFeeReceiverAddress(
        ...
        _require(_feeReceiver != address(0), Errors.ZERO_ADDRESS);
        storageParams.feeReceiver = _feeReceiver;
        emit FeeReceiverUpdated(_feeReceiver);
    }

    /**
     * @return address of fees reciever
     */
    ...
    function callIdentity(
        ...
        _require(success, Errors.CALL_IDENTITY_FAILED);
    }
    ...
    /**
     * @notice if admin want to withdraw liquidity of _token against _wLegalToken
     * @param _wLegalToken address of the wrapped legal token
     * @param _token address of token pair against _wLegalToken
    ...
    function withdrawLiquidity(
        ...
        address _token,
        address _to
    ) external onlyMaintainer delayedLiquidityWithdrawal nonReentrant {
        if (storageParams.wLegalToTokens[_wLegalToken][_token] == 0) {
            _revert(Errors.ZERO_BALANCE);
        }
        ...
        emit LiquidityWithdrawed(_to, _wLegalToken, amountToSend);
    }

    /**
     * @notice for maintainer to request the withdrawLiquidity
     */
    function requestLiquidityWithdrawal() external onlyMaintainer {
        require(
            storageParams.pendingWithdrawalRequestTimestamp == 0,
    ...
    /**
     * @notice for maintainer to cancel the request of liquidity withdrawal
     */
    function cancelLiquidityWithdrawalRequest() external onlyMaintainer {
        storageParams.pendingWithdrawalRequestTimestamp = 0;
    }
    ...
    ) external onlyMaintainer nonReentrant returns (address) {
        address WLegalShares = storageParams.addProperty(_propertyParams);
        IPriceFeed.Property memory _property = _propertyParams.propertyDetails;
        _require(
            _property.currency == storageParams.baseCurrency,
            Errors.INVALID_BASE_CURRENCY
        );
    ...
    /**
     * @notice - updates MarketplaceBorrower that will borrow from a Pool to fill user property SELL operations
    ...
    function setMarketplaceBorrower(
        ...
        emit MarketplaceBorrowerUpdated(_mpBorrower);
    }
     ...
     * @param isFeeInXEQ true: fees of buy/sell to be paid in XEQ, false:fees to be paid in tradeCurrency
     */
    function swap(
        swapArgs memory args,
        bool isFeeInXEQ
        ...
        if (storageParams.tokenExisits[args.to]) {
            // checking if buy state is active or not
            if (
                storageParams.buyState == State.Paused ||
                storageParams.propertyToBuySellState[args.to][
                    BuySellState.BuyState
                ] ==
                State.Paused
            ) {
                _revert(Errors.BUY_PAUSED);
            } else {
                return
                    _swap(
                        args.from,
            ...
            //selling WLegal token
        } else if (storageParams.tokenExisits[args.from]) {
            // checking if sell state is active or not
            if (
                storageParams.sellState == State.Paused ||
                storageParams.propertyToBuySellState[args.from][
                    BuySellState.SellState
                ] ==
                State.Paused
            ) {
                _revert(Errors.SELL_PAUSED);
            } else {
                // checking if there exists priceFeed of asked curency
                if (
                    !(IPriceFeed(
    ...
    function _swap(
        ...
        (
            ...
            address _priceFeed, // address of PriceFeed.sol
            address _currencyToFeed // currency to feed address in Pricefeed.sol
        ) = storageParams._getPropertyPrice(_from, _to);

        // checking if user is trying to buy/sell in property's base currency
        if (
        ...
        // means user is not using property's base currency for buy/sell
        else {
     ...
     * @param _from from address can be if(_isBuying) ? buyCurrency : propertyToken
     * @param _to to address can be if(!_isBuying) ? propertyToken : sellCurrency
    ...
    /**
     * @notice to calculate the price it takes to buy asked amount of property tokens in asked currency.
     *  @param _quoteParams details of property
     *  @return quotePrice no. of tokens required to buy property without fees in asked currency
     */
    function propertyQuotePrice(
        IMarketplace.QuotePriceParams memory _quoteParams
    ...
    /**
     * @notice  calculate the property price.
     * @param _from the currency to be used for buy/sell
     * @param _to address of WrappedLegal.
     * @return  _property details about the property, see IPriceFeed.sol
     * @return  _priceFeed address of PriceFeed.sol
     * @return  _currencyToFeed address of chainlink feed of the _from
     */
    function getPropertyPrice(
        address _from,
        address _to
    ...
    /**
     * @notice - Address of currency in which marketplace is supposed to buy and sell
     * @return main currency in which buying and selling will be done mostly
     */
    ...
    /**
     * @notice changes the property buy status to Active or Paused
     * @param _wrappedLegal addres of WLegalToken
     * @param _newState , Active of Paused
     */
    function updatePropertyBuyState(
        address _wrappedLegal,
        State _newState
    ) external onlyMaintainer {
        storageParams.propertyToBuySellState[_wrappedLegal][
            BuySellState.BuyState
        ] = _newState;
    }

    /**
     * @notice changes the property sell status to Active or Paused
     * @param _wrappedLegal addres of WLegalToken
     * @param _newState , Active of Paused
     */
    function updatePropertySellState(
        address _wrappedLegal,
        State _newState
    ) external onlyMaintainer {
        storageParams.propertyToBuySellState[_wrappedLegal][
    ...
    /**
     * @param _wrappedLegal addres of WLegalToken
     * @return State.active is buy is active and State.Paused is buy is paused
     */
    function getPropertyBuyState(
        address _wrappedLegal
    ...
    /**
     * @param _wrappedLegal addres of WLegalToken
     * @return State.active is sell is active and State.Paused is sell is paused
     */
    function getPropertySellState(
        address _wrappedLegal
    ) external view returns (State) {
        return
            storageParams.propertyToBuySellState[_wrappedLegal][
