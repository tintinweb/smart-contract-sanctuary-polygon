...
contract halolos is ERC721  {
    ...
    constructor()
        ERC721("halolos", "halolos")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 125) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeihj5pwcbeoprv5ftlbjutfsuhdq77znt5pis55rhbxxrdpjkwdfry/", Strings.toString(tokenId), ".json")
            );
    }
