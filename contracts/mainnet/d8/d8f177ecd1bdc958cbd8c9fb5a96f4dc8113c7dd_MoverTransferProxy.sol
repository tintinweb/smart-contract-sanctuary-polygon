...
*/
contract MoverTransferProxy is AccessControlUpgradeable, SafeAllowanceResetUpgradeable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    ///////////////////////////////////////////////////////////////////////////
    event BridgeTx(address indexed beneficiary, address token, uint256 amount, address target);
    ...
    function bridgeAsset(address _token, uint256 _amount, bytes memory _bridgeTxData) public {
        IERC20Upgradeable(_token).safeTransferFrom(msg.sender, address(this), _amount);

        bridgeAssetDirect(_token, _amount, _bridgeTxData);
    }

    function bridgeAssetDirect(address _token, uint256 _amount, bytes memory _bridgeTxData) internal {
        ...
        emit BridgeTx(msg.sender, _token, _amount, targetAddress);
    }

    function swapBridgeAsset(address _tokenFrom, address _tokenTo, uint256 _amountFrom, uint256 _expectedMinimumReceived, bytes memory _convertData, bytes memory _bridgeTxData) public payable {
        require(_tokenFrom != _tokenTo, "same tokens provided");
        ...
        bridgeAssetDirect(_tokenTo, amountReceived, _bridgeTxData);
    }
}