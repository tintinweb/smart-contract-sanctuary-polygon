...
contract BorrowedNFT is IRewardsSplitter, ERC721 {
    using SafeERC20 for IERC20;

    uint64 public constant MAX_BASIS_POINTS = 100_00;

    address public immutable rentalProtocol;
    ERC721 public immutable originalNFT;

    constructor(address _rentalProtocol, address _originalNFT) ERC721("Borrowed NFT", "bNFT") {
        rentalProtocol = _rentalProtocol;
        originalNFT = ERC721(_originalNFT);
    }
    ...
    function onERC20Received(
        ...
        IRentalProtocol.Fee[] memory fees = IRentalProtocol(rentalProtocol).getFeesTable(address(originalNFT), tokenId);
...
contract LentNFT is ERC721 {
    address public immutable rentalProtocol;
    ERC721 public immutable originalNFT;

    constructor(address _rentalProtocol, address _originalNFT) ERC721("Lent NFT", "lNFT") {
        rentalProtocol = _rentalProtocol;
        originalNFT = ERC721(_originalNFT);
    }
...
contract RentalProtocol is IRentalProtocol, AccessControl, ERC721Holder, ReentrancyGuard, EIP712 {
    using SafeERC20 for IERC20;
    ...
    constructor(address _feesCollector, uint16 feesBasisPoints) EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION) {
    ...
    function preSignRentalOffer(RentalOffer calldata offer) external override {
    ...
    function cancelRentalOffer(uint256 nonce) external override {
    ...
    function rent(
        ...
        if (offer.feeAmount > 0) {
            IERC20 feeToken = IERC20(offer.feeToken);
    ...
    function endRental(address token, uint256 tokenId) external override nonReentrant {
        require(block.timestamp > rentals[token][tokenId].end, "Rental hasn't ended");
        _endRental(token, tokenId);
    }

    function endRentalPrematuraly(address token, uint256 tokenId) external override nonReentrant {
        LentNFT lentNFT = originalToLentNFT[token];
    ...
    function endSublet(address token, uint256 tokenId) external override nonReentrant {
...
contract SubLentNFT is ERC721 {
    address public immutable rentalProtocol;
    ERC721 public immutable originalNFT;

    constructor(address _rentalProtocol, address _originalNFT) ERC721("SubLent NFT", "slNFT") {
        rentalProtocol = _rentalProtocol;
        originalNFT = ERC721(_originalNFT);
    }
