...
contract Presale is Ownable {
    ...
    event Exchange(address indexed buyer, uint256 paid, uint256 tokenAmount);

    constructor(address token, uint256 initialExchangeRate) {
        require(token != address(0), "Invalid token address");
        require(initialExchangeRate > 0, "Invalid exchange rate");
        _token = ERC20(token);
        _exchangeRate = initialExchangeRate;
        _minimum = 10000000000000000000;  // 10 Matic
        _maximum = 100000000000000000000000; // 100000 Matic
    ...
    function getContribution(address wallet) external view returns (uint256) {
