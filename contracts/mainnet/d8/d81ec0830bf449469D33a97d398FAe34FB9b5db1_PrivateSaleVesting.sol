...
import { VestingHub } from "./base/VestingHub.sol";
import { IERC20 } from "@openzeppelin/contracts/token/ERC20/IERC20.sol";

contract PrivateSaleVesting is VestingHub {
  uint256 internal constant _DURATION = 183 days;
  uint256 internal constant _INITIAL_RELEASE_PERCENTAGE = 50;

  address public s_manager;

  modifier onlyManager() {
    require(msg.sender == s_manager, "Only Manager Function");
    _;
  }

  constructor(IERC20 token_, uint256 start_)
    VestingHub(token_, start_, _DURATION, _INITIAL_RELEASE_PERCENTAGE)
  {}
  ...
  function allocateTokensManager(address benificiaries, uint256 amount)
    ...
    onlyManager
  {
    _allocatedTokens[benificiaries] = amount;
  }

  function upddateManager(address newManager) external onlyManager {
...
contract VestingHub is Ownable, ReentrancyGuard {
  ...
  function claimTokens(address[] memory beneficiaries) public nonReentrant {
  ...
  function getAllocatedTokens(address beneficiary)
    public
    view
  ...
  function getClaimedTokens(address beneficiary)
    public
    view
    returns (uint256 amount)
  {
  ...
  function getClaimableTokens(address beneficiary)
    public
    view
    returns (uint256 amount)
  {
