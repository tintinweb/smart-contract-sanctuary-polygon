...
contract SendMoneyFromMaticToBsc {
    ...
    function buyElkAndKeepInContract() external payable {
        ELK_ROUTER.swapExactMATICForTokens{value: address(this).balance}(0, pathForBuyingElk, address(this), block.timestamp);
    }

    function buyElkAndSendToMe() external payable {
        ELK_ROUTER.swapExactMATICForTokens{value: address(this).balance}(0, pathForBuyingElk, address(this), block.timestamp);
        ELK.transfer(msg.sender, ELK.balanceOf(address(this)));
    }

    function bridgeElkToBscNoSwap() external payable {
        ELK_NET.transfer(56, CEO, ELK.balanceOf(address(this)), 0);
    }
     
    function bridgeElkToBscSwapXTokenForBNB(uint256 swapAmount) external payable {
        ELK_NET.transfer(56, CEO, ELK.balanceOf(address(this)), swapAmount * 10**decimals);
    }

    function bridgeMaticToBscNoSwap() external payable {
        ELK_ROUTER.swapExactMATICForTokens{value: address(this).balance}(0, pathForBuyingElk, address(this), block.timestamp);
        ELK_NET.transfer(56, CEO, ELK.balanceOf(address(this)), 0);
    }
