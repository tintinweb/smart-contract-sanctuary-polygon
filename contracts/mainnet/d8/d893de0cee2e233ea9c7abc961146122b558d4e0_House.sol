...
contract House is ReentrancyGuard, Ownable {
    ...
    function placeBet(address player, uint amount, uint rollUnder, uint MODULO, address refAddr) payable external isHouseLive admin nonReentrant {
        ...
        amountArray[2] = amountToTreasurePoolConverter(profitAmount);
        amountArray[3] = amountToPrizePoolConverter(profitAmount);
        amountArray[4] = this.amountToWinnableAmount(amount, rollUnder,MODULO);
        amountArray[5] = amountToRefConverter(profitAmount);
        ...
        if(user[player].refAddr != address(0)){
            user[user[player].refAddr].refProfit += amountArray[5];
        }else{
