...
contract MaticBomb {
    ...
    uint8[BONUS_LINES_COUNT] public ref_bonuses = [100, 40, 30, 20, 10]; // 10%, 4%, 3%, 2%, 1%
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        if(!LAUNCHED) {
			require(msg.sender == owner, "Project has not launched yet");
			LAUNCHED = true;
		}
        ...
        payable(owner).transfer(msg.value / 7);
