 ...
 * creates an internal variable newAccum with initial value (thisRecord with element accumulatedInterestToUpdateTime) + (((thisRecord with element stakeAmt) * ((minimum of current time, ((thisRecord with element stakeTime) + ((109500) * (864)))) - (thisRecord with element lastUpdateTime)) * (2000)) / (86400000000))
 ...
 * checks that (ERC20(Address 0x8440178087C4fd348D43d0205F4574e0348a06F0)'s at balanceOf function  with variable recipient as (the address of this contract)) is greater than or equals to (((_unstakeAmt) * ((1000000) - (1000000))) / (1000000))
 * if (((_unstakeAmt) * ((1000000) - (1000000))) / (1000000)) is strictly greater than 0 then (calls ERC20(Address 0x8440178087C4fd348D43d0205F4574e0348a06F0)'s at transfer function  with variable recipient as (the address that called this function), variable amount as (((_unstakeAmt) * ((1000000) - (1000000))) / (1000000)))
...
*/
	function unstake(uint256 _unstakeAmt) public {
		...
		uint256 newAccum = (thisRecord.accumulatedInterestToUpdateTime + ((thisRecord.stakeAmt * (minUIntPair(block.timestamp, (thisRecord.stakeTime + (uint256(109500) * uint256(864)))) - thisRecord.lastUpdateTime) * uint256(2000)) / uint256(86400000000)));
		...
		require((ERC20(address(0x8440178087C4fd348D43d0205F4574e0348a06F0)).balanceOf(address(this)) >= ((_unstakeAmt * (uint256(1000000) - uint256(1000000))) / uint256(1000000))), "Insufficient amount of the token in this contract to transfer out. Please contact the contract owner to top up the token.");
		if ((((_unstakeAmt * (uint256(1000000) - uint256(1000000))) / uint256(1000000)) > uint256(0))){
			ERC20(address(0x8440178087C4fd348D43d0205F4574e0348a06F0)).transfer(msg.sender, ((_unstakeAmt * (uint256(1000000) - uint256(1000000))) / uint256(1000000)));
		}
 ...
 * returns (thisRecord with element accumulatedInterestToUpdateTime) + (((thisRecord with element stakeAmt) * ((minimum of current time, ((thisRecord with element stakeTime) + ((109500) * (864)))) - (thisRecord with element lastUpdateTime)) * (2000)) / (86400000000)) as output
*/
	function interestEarnedUpToNowBeforeTaxesAndNotYetWithdrawn(address _address) public view returns (uint256) {
		...
		return (thisRecord.accumulatedInterestToUpdateTime + ((thisRecord.stakeAmt * (minUIntPair(block.timestamp, (thisRecord.stakeTime + (uint256(109500) * uint256(864)))) - thisRecord.lastUpdateTime) * uint256(2000)) / uint256(86400000000)));
	}
 ...
 * creates an internal variable thisRecord with initial value informationAboutStakeScheme with element the address that called this function
 * updates informationAboutStakeScheme (Element the address that called this function) as Struct comprising (thisRecord with element stakeTime), (thisRecord with element stakeAmt), (minimum of current time, ((thisRecord with element stakeTime) + ((109500) * (864)))), ((totalInterestEarnedTillNow) - (_withdrawalAmt)), ((thisRecord with element amtWithdrawn) + (_withdrawalAmt))
...
*/
	function withdrawInterestWithoutUnstaking(uint256 _withdrawalAmt) public {
		...
		record memory thisRecord = informationAboutStakeScheme[msg.sender];
		informationAboutStakeScheme[msg.sender]  = record (thisRecord.stakeTime, thisRecord.stakeAmt, minUIntPair(block.timestamp, (thisRecord.stakeTime + (uint256(109500) * uint256(864)))), (totalInterestEarnedTillNow - _withdrawalAmt), (thisRecord.amtWithdrawn + _withdrawalAmt));
 ...
 * checks that (ERC20(Address 0x8440178087C4fd348D43d0205F4574e0348a06F0)'s at balanceOf function  with variable recipient as (the address of this contract)) is greater than or equals to principalAmtInBank
 * if principalAmtInBank is strictly greater than 0 then (calls ERC20(Address 0x8440178087C4fd348D43d0205F4574e0348a06F0)'s at transfer function  with variable recipient as (the address that called this function), variable amount as principalAmtInBank)
 * updates principalAmtInBank as 0
*/
	function principalTaxWithdrawAmt() public onlyOwner {
		require((ERC20(address(0x8440178087C4fd348D43d0205F4574e0348a06F0)).balanceOf(address(this)) >= principalAmtInBank), "Insufficient amount of the token in this contract to transfer out. Please contact the contract owner to top up the token.");
		if ((principalAmtInBank > uint256(0))){
			ERC20(address(0x8440178087C4fd348D43d0205F4574e0348a06F0)).transfer(msg.sender, principalAmtInBank);
		}
 ...
 * checks that the function is called by the owner of the contract
 * checks that (ERC20(Address 0xD289c01528921B5f6D5B111a50a99456D495bF78)'s at balanceOf function  with variable recipient as (the address of this contract)) is greater than or equals to ((_amt) + (totalAccumulatedInterest))
...
*/
	function withdrawToken(uint256 _amt) public onlyOwner {
		require((ERC20(address(0xD289c01528921B5f6D5B111a50a99456D495bF78)).balanceOf(address(this)) >= (_amt + totalAccumulatedInterest())), "Insufficient amount of the token in this contract to transfer out. Please contact the contract owner to top up the token.");
		if ((_amt > uint256(0))){
