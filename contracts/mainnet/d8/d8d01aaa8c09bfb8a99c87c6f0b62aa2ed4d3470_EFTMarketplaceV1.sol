 ...
 */
contract EFTMarketplaceV1 is  Initializable, ERC721Holder, OwnableUpgradeable {
   ...
   */
  uint8 minOfferGuaranteeHours;

  //coupon signer address 
  address public couponSigner;


  // Eternity Foundation Token 
  IERC20 eft;
   ...
   */
  function initialize (address _card, address payable _daoAddress, uint8 _minOfferGuaranteeHours ,address _couponSigner
  ) public initializer {
...
* @dev Fired in cancelOfferOnBuyOrder()
  ...
  modifier validCardType(uint8 _cardType) {
    require(_cardType >= 0 && _cardType <= 53, "card type must be in range [0, 53]");
    _;
  }
  ...
  function setMinOfferGuaranteeHours(uint8 _hours) external onlyOwner {
    ...
    emit MinOfferGuaranteeHoursChanged(msg.sender,  minOfferGuaranteeHours, _hours);
...
/**
   ...
   *
   */
  function _transferFees(uint256 _feeValue, address _collectionAddress, address _collectionPartnerDaoAddress) internal {
    uint256 feeValue = _feeValue;
    uint partnerMarketplaceFeePercent = ICollection(_collectionAddress).partnerMarketplaceFeePercent();
    bool success = false;
    if (partnerMarketplaceFeePercent > 0) {
        //Send % to partner Dao
        uint256 partnerMarketplaceFeePercentValue = feeValue * partnerMarketplaceFeePercent / 100_000;
        success = eft.transfer(_collectionPartnerDaoAddress, partnerMarketplaceFeePercentValue);
        require(success, "Send fees to partner failed.");
        feeValue = feeValue - partnerMarketplaceFeePercentValue;
      }

      success = eft.transfer(daoAddress, feeValue);
...
/**
   ...
   *
   */
  function _getSellerFeeWithDiscount(uint256 _amount, uint8 _collectionId, Coupon memory _coupon, uint _discount) internal view returns (uint marketplaceFee, uint256 marketplaceFeeValue, uint discount){
    ...
    if(_discount > 0){
        bytes32 digest = keccak256(
            abi.encode(_collectionId, _discount, msg.sender)
        );
    ...
    (address collectionAddress,,,,,,) = card.cardCollectionInfo(_collectionId);

    marketplaceFee = ICollection(collectionAddress).marketplaceSellerFeePercent();
    marketplaceFeeValue = _amount * marketplaceFee / 100;
...
/**
   ...
   *
   */
  function _getBuyerFeeWithDiscount(uint256 _amount, uint8 _collectionId, Coupon memory _coupon, uint _discount) internal view returns (uint marketplaceFee, uint256 marketplaceFeeValue, uint discount){
    ...
    marketplaceFee = ICollection(collectionAddress).marketplaceBuyerFeePercent();
    marketplaceFeeValue = _amount * marketplaceFee / 100;
 ...
 function _isVerifiedCoupon(bytes32 _digest, Coupon memory _coupon) internal view returns (bool) {
  ...
  function createSellOrder(
    ...
    orderId = keccak256(abi.encodePacked(block.timestamp, msg.sender, _assetIds, _ask, _name));
    
    (uint marketPlaceFee, uint256 marketplaceFeeValue, uint discount)  = _getSellerFeeWithDiscount(_ask, _collectionId, _coupon, _discount);
  ...
  */
  function cancelSellOrder(bytes32 _orderId) external sellOrderExists(_orderId) senderOwnsSellOrder(_orderId) {
    ...
    if(sellOrder.highestOfferor != address(0) && !sellOrder.highestOfferor.isContract()) {
      // Refund offeror
        bool success = eft.transfer(sellOrder.highestOfferor, (sellOrder.highestOffer + sellOrder.highestOfferBuyerFee));
  ...
  */
  function placeOfferOnSellOrder(
    ...
    (,uint256 marketplaceFeeValue,)  = _getBuyerFeeWithDiscount(_offerValue, sellOrder.collectionId, _coupon, _discount);
    
    require(_value == _offerValue + marketplaceFeeValue, "incorrect amount provided");
   
    // If the sell order is not open to offers without an ask price
    if(sellOrder.ask != 0) {
      // Ensure the offer value is less than the order ask
      require(_offerValue < sellOrder.ask, "offer value must be less than order ask");
    }
    if(sellOrder.ask == 0 && sellOrder.offerMinPrice > 0){
      require(_offerValue >= sellOrder.offerMinPrice, "offer value must be higher than offer min price");
    }

    // Ensure the value of the new offer is higher than the current
    require(_offerValue > sellOrder.highestOffer, "offer value must be higher than current");
    ...
    require(_offerGuarantee >= block.timestamp + uint256(minOfferGuaranteeHours) * 60 * 60, "you need to increase offer guarantee");
    ...
    if(!sellOrder.owner.isContract()) {
      // safe offer money in contract
      bool success = eft.transferFrom(msg.sender, address(this), _value);
      require(success, "Transfer for sell order offer.");
    }
 ...
 * @notice Cancelles an offer on a sell order
   ...
   */
  function cancelOfferOnSellOrder (bytes32 _orderId) external onlyIfSenderNotContract sellOrderExists(_orderId) {
    SellOrder storage sellOrder = sellOrders[_orderId];
    ...
    if(!sellOrder.highestOfferor.isContract() && sellOrder.highestOffer > 0) {
      // Refund offeror
      bool success = eft.transfer(sellOrder.highestOfferor, (sellOrder.highestOffer + sellOrder.highestOfferBuyerFee));
  ...
  */
  function acceptSellOrderHighestOffer(bytes32 _orderId) external sellOrderExists(_orderId) senderOwnsSellOrder(_orderId) {
    ...
    // offer)
    if(!sellOrder.owner.isContract() && sellOrder.highestOffer > 0) {
      // Transfer EFT amount without fees to sell order owner
      // (fees are paid to DAO)
    uint256 marketplaceFeeValue = sellOrder.highestOffer * sellOrder.marketPlaceFee / 100;
     if(sellOrder.discount > 0){
        marketplaceFeeValue = marketplaceFeeValue - (marketplaceFeeValue * sellOrder.discount / 100);
     }
     
     bool success = eft.transfer(sellOrder.owner, sellOrder.highestOffer - marketplaceFeeValue);
    require(success, "Transfer to sell order owner failed.");
    uint256 totalFee = marketplaceFeeValue + sellOrder.highestOfferBuyerFee;
   ...
   */
function fulfillSellOrder(bytes32 _orderId, uint256 _value, Coupon memory _coupon, uint _discount) external onlyIfSenderNotContract sellOrderExists(_orderId) {
    // Copy sell order in memory
    SellOrder memory sellOrder = sellOrders[_orderId];
    ...
    (,uint256 marketplaceBuyerFeeValue,)  = _getBuyerFeeWithDiscount(sellOrder.ask, sellOrder.collectionId, _coupon, _discount);
    // Ensure EFT sent is equal to sell order ask value with fee
    require(sellOrder.ask + marketplaceBuyerFeeValue == _value, "incorrect amount provided");
    ...
    // offer)
    if(!sellOrder.owner.isContract()) {
      // Transfer EFT to marketplace 
      
      bool success = eft.transferFrom(msg.sender, address(this), _value);
      require(success, "Transfer for sell order failed.");

      success = eft.transfer(sellOrder.owner, sellOrder.ask - sellOrder.sellerFee);
      require(success, "Transfer to sell order owner failed.");
      
      // (fees are paid to DAO and Partnern)
      _transferFees(sellOrder.sellerFee + marketplaceBuyerFeeValue, collectionAddress, partnerDaoAddress);
    }
  ...
  function createBuyOrder(
    ...
    uint _discount,
    uint256 _value
  ) external  validCardType(_cardType) returns (bytes32 orderId) {
   
    // Generate pseudorandom order ID
    orderId = keccak256(abi.encodePacked(block.timestamp, msg.sender, _cardType, _bid));

    (,uint256 marketplaceFeeValue,) = _getBuyerFeeWithDiscount(_bid, _cardCollection, _coupon, _discount);
   
    require(_value == _bid + marketplaceFeeValue, "incorrect amount provided");

    //save buyOrder money in contract
    bool success = eft.transferFrom(msg.sender, address(this), _value);
  ...
  * @notice Cancel buy order
  ...
  function cancelBuyOrder(bytes32 _orderId) external buyOrderExists(_orderId) senderOwnsBuyOrder(_orderId) {
    ...
    //Refund Buy order
    bool success = eft.transfer(buyOrder.owner, buyOrder.bid + buyOrder.buyerFee);
    require(success, "Refund previous offeror failed.");
  ...
  function placeOfferOnBuyOrder(
    ...
    (uint256 marketplaceFeeValue,,) = _getSellerFeeWithDiscount(_offer, cardCollection, _coupon, _discount);

    // If the buy order is not open to offers without a bid price
    if(buyOrder.bid != 0) {
      // Ensure the offer value is more than the order bid
      require(_offer - marketplaceFeeValue > buyOrder.bid, "offer value must be more than order bid");
    }
 ...
 * @notice Cancelles an offer on a buy order
  ...
  function acceptBuyOrderLowestOffer(bytes32 _orderId) external buyOrderExists(_orderId) senderOwnsBuyOrder(_orderId) {
    ...
    if(!buyOrder.lowestOfferor.isContract()) {
      ...
      // Transfer EFT amount without fees to sell
      // (fees are paid to DAO and Partner)
      bool success = eft.transfer(buyOrder.lowestOfferor, buyOrder.lowestOffer - buyOrder.lowestOfferSellerFee);

      _transferFees(buyOrder.lowestOfferSellerFee + buyOrder.buyerFee, collectionAddress, partnerDaoAddress);
   ...
   */
  function fulfillBuyOrder(bytes32 _orderId, uint256 _cardId, Coupon memory _coupon, uint _discount) external onlyIfSenderNotContract buyOrderExists(_orderId) {
    // Copy buy order in memory
    BuyOrder memory buyOrder = buyOrders[_orderId];
    ...
    if(!buyOrder.owner.isContract()) {
     ...
     (,uint256 marketplaceSellerFeeValue,) = _getSellerFeeWithDiscount(buyOrder.bid, cardCollection, _coupon, _discount);
    
    // Transfer EFT amount without fees to seller
     bool success = eft.transfer(msg.sender, buyOrder.bid - marketplaceSellerFeeValue);
     require(success, "Transfer to sell order owner failed.");

    // (fees are paid to DAO )
    _transferFees(marketplaceSellerFeeValue + buyOrder.buyerFee, collectionAddress, partnerDaoAddress);
    ...
    if(buyOrder.lowestOfferor != address(0) && !buyOrder.lowestOfferor.isContract()) {
      // Refund previous offeror
      success = eft.transfer(buyOrder.lowestOfferor,  buyOrder.lowestOffer + buyOrder.lowestOfferSellerFee);
      require(success, "Refund previous offeror failed.");
    }
...
/**
  * @notice Collection interface for getting collection fees data
   ...
   * 
   * uint public marketplaceBuyerFeePercent = 5_000;
   * 
...
interface ICollection {
    ...
    function marketplaceSellerFeePercent() external view returns(uint);
    function marketplaceBuyerFeePercent() external view returns(uint);
