...
contract Surrivatives is ERC2771Context, ERC1155, Ownable, Pausable{
  uint public collectionAvailable = 4;
  string public name = "Surrivatives";
  ...
  constructor(string memory userUri, bytes32 merkleroot_, address forwarder) ERC1155(userUri) ERC2771Context(forwarder) {
    _setURI(userUri);
  ...
  function _msgSender() internal override(Context, ERC2771Context) virtual view returns (address ret) {
    return ERC2771Context._msgSender();
  }
  //gas less transactions requirement
  function _msgData() internal override(Context, ERC2771Context) virtual view returns (bytes calldata ret) {
    return ERC2771Context._msgData();
  }

  function _isValidProof(bytes32[] calldata _proof, address user, uint[] calldata ids, uint[] calldata amounts) view private returns(bool) {
  ...
  function claim (uint[] calldata ids, uint[] calldata amounts, bytes32[] calldata _proof) external {
    require(_isValidProof(_proof, _msgSender(), ids, amounts), "Invalid proof");
    require(ids.length == amounts.length, "Error in the data");
    require(_claimed[_msgSender()]== false, "Already Claimed");
    _claimed[_msgSender()] = true;
    _mintBatch(_msgSender(), ids, amounts, "0x0");
  }
  ...
  modifier isWhitelisted {
    require(whitelist[_msgSender()] == true, "You are not allowed to add a collection");
    _;
  }
