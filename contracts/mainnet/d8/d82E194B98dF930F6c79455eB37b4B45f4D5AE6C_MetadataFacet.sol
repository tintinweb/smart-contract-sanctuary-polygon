...
contract MetadataFacet {
    ...
    function getUnicornParents(uint256 tokenId) public view returns (uint256[2] memory parentIds) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        parentIds[0] = ds.unicornParents[tokenId][0];
        parentIds[1] = ds.unicornParents[tokenId][1];
    }

    function getParentMetadata(uint256 tokenId) public view returns(
        ...
        parentBreedingPoints[0] = LibUnicornDNA._getBreedingPoints(firstParentDNA);
        parentBreedingPoints[1] = LibUnicornDNA._getBreedingPoints(secondParentDNA);
        parentNames[0] = LibUnicornNames._getFullName(parentTokenIds[0]);
        parentNames[1] = LibUnicornNames._getFullName(parentTokenIds[1]);
        parentTokenURIs[0] = LibERC721.getTokenURI(parentTokenIds[0]);
        parentTokenURIs[1] = LibERC721.getTokenURI(parentTokenIds[1]);
        (firstParentBodyPartIds, firstParentGeneIds) = LibUnicornDNA._getGeneMapFromDNA(firstParentDNA);
        (secondParentBodyPartIds, secondParentGeneIds) = LibUnicornDNA._getGeneMapFromDNA(secondParentDNA);
    }
}
