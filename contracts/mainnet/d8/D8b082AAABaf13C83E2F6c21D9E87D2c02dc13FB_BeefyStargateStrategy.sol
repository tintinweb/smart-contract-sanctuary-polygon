...
contract BeefyStargateStrategy is
    Initializable,
    ...
    IFeedStrategy
{
    ...
    function initialize(
        ...
        lpToken = IStargateLp(IBeefyVault(_token).want());
        referenceFeed = IFeedStrategy(_referenceFeed);

        mooToken = IBeefyVault(_token);
        lpDecimals = IBeefyVault(_token).decimals();
    ...
    function getPriceOfAmount(
        ...
        uint256 totalOneSideWithdraw = lpToken.amountLPtoLD(amount);
        (int256 usdPrice, uint8 usdDecimals) = referenceFeed.getPrice();

        require(usdPrice > 0, "BeefyStarPriceStrat: feed lte 0");
