...
contract GeneticViewFacet {
    function getGeneTierById(uint256 _geneId) external view returns (uint256) {
        return LibDiamond.diamondStorage().geneTierById[_geneId];
    }

    function getGeneTierUpgradeById(uint256 _geneId)
        external
        view
        returns (uint256)
    {
        return LibDiamond.diamondStorage().geneTierUpgradeById[_geneId];
    }

    function getGeneApplicationById(uint256 _geneId)
        external
        view
        returns (uint256)
    {
        return LibDiamond.diamondStorage().geneApplicationById[_geneId];
    }

    function getGeneBuckets(uint256 _classId)
        external
        view
        returns (uint256[] memory)
    {
        return LibDiamond.diamondStorage().geneBuckets[_classId];
    }

    function getGeneBucketSumWeights(uint256 _classId)
        external
        view
        returns (uint256)
    {
        return LibDiamond.diamondStorage().geneBucketSumWeights[_classId];
    }

    function getGeneWeightById(uint256 _geneId)
        external
        view
        returns (uint256)
    {
        return LibDiamond.diamondStorage().geneWeightById[_geneId];
    }

    function getGeneBonusStatByGeneId(uint256 _geneId, uint256 _geneBonusSlot)
        external
        view
        returns (uint256)
    {
        return
            LibDiamond.diamondStorage().geneBonusStatByGeneId[_geneId][
                _geneBonusSlot
            ];
    }

    function getGeneBonusValueByGeneId(uint256 _geneId, uint256 _geneBonusSlot)
        external
        view
        returns (uint256)
    {
        return
            LibDiamond.diamondStorage().geneBonusValueByGeneId[_geneId][
                _geneBonusSlot
            ];
    }

    function getBodyPartLocalIdFromGlobalId(uint256 _globalPartId)
        external
        view
        returns (uint256)
    {
        return
            LibDiamond.diamondStorage().bodyPartLocalIdFromGlobalId[
                _globalPartId
            ];
    }

    function getBodyPartIsMythic(uint256 _globalPartId)
        external
        view
        returns (bool)
    {
        return LibDiamond.diamondStorage().bodyPartIsMythic[_globalPartId];
    }

    function getBodyPartInheritedGene(uint256 _globalPartId)
        external
        view
        returns (uint256)
    {
        return LibDiamond.diamondStorage().bodyPartInheritedGene[_globalPartId];
    }

    function getBodyPartBuckets(uint256 _classId, uint256 _partSlotId)
        external
        view
        returns (uint256[] memory)
    {
        return
            LibDiamond.diamondStorage().bodyPartBuckets[_classId][_partSlotId];
    }
    ...
    function getBodyPartGlobalIdFromLocalId(
        ...
        return
            LibDiamond.diamondStorage().bodyPartGlobalIdFromLocalId[_classId][
                _partSlotId
            ][_localPartId];
    }

    function getBodyPartWeight(uint256 _globalPartId)
        external
        view
        returns (uint256)
    {
        return LibDiamond.diamondStorage().bodyPartWeight[_globalPartId];
    }

    function getBaseStats(uint256 _classId, uint256 _statId)
        external
        view
        returns (uint256)
    {
        return LibDiamond.diamondStorage().baseStats[_classId][_statId];
    }
    ...
    function getUnicornGeneMap(uint256 tokenId) external view returns (uint256[6] memory bodyPartIds, uint256[18] memory geneIds) {
    ...
    function getGeneMapFromDNA(uint256 dna)  external view returns (uint256[6] memory bodyPartIds, uint256[18] memory geneIds) {
