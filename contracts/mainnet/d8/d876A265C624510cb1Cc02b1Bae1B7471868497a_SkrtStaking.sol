...
contract SkrtStaking is Ownable {
    ...
    struct StakingPenalties {
        uint256 firstPenaltyPeriodSeconds;
        uint256 firstUnStakePenalty;
        uint256 secondUnStakePenalty;
    }
    ...
    constructor(address _tokenAddress, address _skrtVerificationAddress, StakingParameters memory _stakingParameters, StakingPenalties memory _stakingPenalties, RewardPercentage memory _rewardPercentage, 
    ...
    **/
    ...
    external 
    returns (bool) {
        require(allowStaking, "Staking is not allowed");
    ...
    returns (bool){
        ...
        if (_stakeReward > 0) {
            ...
            require(token.transfer(msg.sender, _stakeReward), "[UnStake] Something went wrong while transferring your reward");
        }
    ...
    returns (bool) {
        ...
        //transfer tokens
        require(token.transfer(msg.sender, _stakeReward), "[Claim] Something went wrong while transferring your reward");
     ...
     **/ 
    function setAllowStaking(bool _allowStaking) 
    whenPaused
    external onlyOwner() 
    returns (bool) {
    ...
    function _checkForUnStakePenalties(uint256 _totalStakeSeconds, uint256 _stakeReward) internal view returns (uint256) {
        ...
        if (_totalStakeSeconds <= stakingPenalties.firstPenaltyPeriodSeconds) {
            return stakingPenalties.firstUnStakePenalty;
        } else if (_totalStakeSeconds > stakingPenalties.firstPenaltyPeriodSeconds && _totalStakeSeconds < stakingParameters.stakingPeriodSeconds) {
            return stakingPenalties.secondUnStakePenalty;
        } else {
            return 0;
        }
    }
