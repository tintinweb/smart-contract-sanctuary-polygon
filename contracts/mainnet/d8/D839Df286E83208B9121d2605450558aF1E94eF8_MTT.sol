...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 IWBNB = IBEP20(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270);
    ...
    uint256 public minPeriod = 3 minutes;               //=> 30 minutes;  
    uint256 public minDistribution = 1 * (10 ** 15);    //=> 1 * (10 ** 18);
    ...
    constructor (address _router) {
        router = _router != address(0)
            ...
            : IPancakeSwapRouter(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
        _token = msg.sender;
    }
...
contract MTT is BEP20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public constant _name = "MTT TOKEN";
    string public constant _symbol = "MTT";
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 200 * 10**6 * 10**DECIMALS;

    uint256 public compoundingTime;

    uint256 private liquidityFee = 20;
    uint256 private treasuryFee = 25;
    uint256 private reflectionFee = 30;
    ...
    uint256 private constant MAX_SUPPLY = 1e11 * 10**DECIMALS;
    ...
    constructor() BEP20Detailed(_name, _symbol, uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0x23CEffc66f3243aef06279d47c9b6D1C4D2860f4;
        treasuryReceiver = 0xcFb8F05d995DacbA0C31efF485A0875B26fE5129;
        marketingPool = 0x23CEffc66f3243aef06279d47c9b6D1C4D2860f4;
        ...
        _isFeeExempt[marketingPool] = true;
    ...
    function rebase() internal {
        ...
        uint256 times = deltaTime.div(compoundingTime);
        ...
        _gonsPerFragment = TOTAL_GONS.div(_totalSupply);
        _lastRebasedTime = _lastRebasedTime.add(times.mul(compoundingTime));
    ...
    function _transferFrom(
        ...
        if(!isDividendExempt[sender]){ try distributor.setShare(sender, _gonBalances[sender]) {} catch {} }
        if(!isDividendExempt[recipient]){ try distributor.setShare(recipient, _gonBalances[recipient]) {} catch {} }
    ...
    function takeFee(
        ...
        if (liquidityPool[sender] == true) {
            ...
            feeAmount = gonAmount.div(feeDenominator).mul(buyTax);    
        } else if (liquidityPool[recipient] == true) {
            //It's an LP Pair and it's a sell
            feeAmount = gonAmount.div(feeDenominator).mul(sellTax);
        } else {
            feeAmount = gonAmount.div(feeDenominator).mul(buyTax);
        }

        uint256 _lqfee = feeAmount.div(feeDenominator).mul(liquidityFee);
        _gonBalances[address(this)] = _gonBalances[address(this)].add(feeAmount.sub(_lqfee));
        _gonBalances[autoLiquidityReceiver] = _gonBalances[autoLiquidityReceiver].add(_lqfee);
    ...
    function swapBack() internal swapping {
        ...
        emit DividendReceiver(amountETHToTreasuryAndDIS.sub(amountETHToTreasury.add(amountETHToMarketing)));             
    }
    ...
    function shouldRebase() internal view returns (bool) {
        return
            ...
            msg.sender != pair &&
            !inSwap &&
            block.timestamp >= (_lastRebasedTime + compoundingTime);
    }
    ...
    function maxTxnFee() public view returns (uint256) {
        return buyTax > sellTax ? buyTax : sellTax;
    }
    ...
    function setIsDividendExempt(address holder, bool exempt) external onlyOwner {
        ...
        if(exempt){
            ...
            distributor.setShare(holder, _gonBalances[holder]);
        }
    }

    function changeCompoundingTime(uint256 _compoundingTime) external onlyOwner {
        require(_compoundingTime > 1 minutes && _compoundingTime <= 24 hours);
    ...
    event DividendReceiver(uint256 amount);
