...
contract Chests is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  modifier mintPriceComplianceETH(uint256 _mintAmount) {
    require(msg.value >= costETH * _mintAmount || msg.sender == owner(), 'Insufficient funds!');
    _;
  }

  modifier mintPriceComplianceToken(uint256 _mintAmount, uint256 _tokenAmount) {
    require(address(token)!= address(0),"token not set");
    require(_tokenAmount >= costToken * _mintAmount || msg.sender == owner(), 'Insufficient funds!');
    _;
  }

 

  function mintWithETH(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceComplianceETH(_mintAmount) {
    require(!paused, 'The contract is paused!');

    _safeMint(_msgSender(), _mintAmount);
  }

  function mintWithToken(uint256 _mintAmount, uint256 _tokenAmount) public  mintCompliance(_mintAmount) mintPriceComplianceToken(_mintAmount, _tokenAmount) {
    require(!paused, 'The contract is paused!');
...
function setIMPAddress(address _impAddress) public onlyOwner{
   token = IERC20(_impAddress);
}
...
function setMaxSupply(uint256 _newMaxSupply) public onlyOwner{
  ...
  function setCostETH(uint256 _costETH) public onlyOwner {
    costETH = _costETH;
  }

    function setCostToken(uint256 _costToken) public onlyOwner {
  ...
  function withdrawETH() public onlyOwner nonReentrant {
   ...
   function withdrawToken() public onlyOwner nonReentrant {
    ...
    uint256 withdrawableByOwner = token.balanceOf(address(this));
    token.transfer(owner(), withdrawableByOwner);
  }
