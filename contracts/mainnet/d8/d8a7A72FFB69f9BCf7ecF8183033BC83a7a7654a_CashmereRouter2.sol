...
contract CashmereRouter2 is Ownable {
    using SafeERC20 for IERC20;

    event TestSwap(uint256 returnAmount, uint256 unspentAmount);

    function swap(IERC20 srcToken, IERC20 trgToken, uint256 srcAmount, address callAddress, bytes memory data) external payable {
        uint256 srcBefore = srcToken.balanceOf(address(this));
        uint256 trgBefore = trgToken.balanceOf(address(this));

        srcToken.safeTransferFrom(msg.sender, address(this), srcAmount);
        srcToken.approve(callAddress, srcAmount);
        (bool success, bytes memory returnData) = callAddress.call{ value: msg.value }(data);
        require(success, "!success");
        uint256 unspentAmount = srcToken.balanceOf(address(this)) - srcBefore;
        uint256 returnAmount = trgToken.balanceOf(address(this)) - trgBefore;
        if (unspentAmount > 0) {
            srcToken.safeTransfer(msg.sender, unspentAmount);
        }
        if (returnAmount > 0) {
            trgToken.safeTransfer(msg.sender, returnAmount);
        }
        emit TestSwap(returnAmount, unspentAmount);
    }
}
