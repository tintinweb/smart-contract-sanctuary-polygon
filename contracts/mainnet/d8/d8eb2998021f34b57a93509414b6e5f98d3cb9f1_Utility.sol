...
abstract contract UtilityStorage {
...
import "../storage/UtilityStorage.sol";
import { UUPSUpgradeable } from "openzeppelin-contracts/contracts/proxy/utils/UUPSUpgradeable.sol";
import "openzeppelin-contracts/contracts/utils/Strings.sol";

contract Utility is ERC1155Base, UtilityStorage, UUPSUpgradeable, IUtility, IUpgradeable {
    constructor() {
       _disableInitializers();
    }
    ...
    function initialize(address _owner, string memory _name, string memory _symbol, string memory _uri) initializer public {
        _initialize(_owner, _uri);
        name = _name;
        symbol = _symbol;
    }
    ...
    function ifOnboardingCharacter(uint256[] memory ids) private pure returns(bool) {
        for (uint i = 0; i < ids.length; i++) {
            if (ids[i] == Types.ONBOARDING_CHAR) {
                return true;
            }
        }
        return false;
    ...
    function mint(address to, uint256 id, uint256 amount, bytes memory data) public override onlyRole(GOVERNOR_ROLE) {
        require(id != Types.ONBOARDING_CHAR || balanceOf(to, Types.ONBOARDING_CHAR) == 0, "Player already claimed onboarding pass");
    ...
    function burn(
        ...
        uint256 id,
        uint256 value
    ) public override(IUtility, ERC1155Base) onlyRole(GOVERNOR_ROLE) {
    ...
    function mintBatch(
        ...
        bool onboardingCharacter = ifOnboardingCharacter(ids);
        require(!onboardingCharacter, "Onboarding Character Pass cannot be batch minted");
        _mintBatch(to, ids, amounts, data);
    }

    function balanceOf(address account, uint256 id) public view override(ERC1155Upgradeable, IUtility) returns (uint256) {
        return ERC1155Upgradeable.balanceOf(account, id);
    }

    function balanceOfBatch(address[] memory accounts, uint256[] memory ids) public view override(ERC1155Upgradeable, IUtility) returns (uint256[] memory) {
    ...
    function _beforeTokenTransfer(
        ...
        require(numOfBasic == 0 || from == address(0), "Cannot transfer onboarding character NFT");
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155Base) returns (bool) {
        return interfaceId == type(IUtility).interfaceId || super.supportsInterface(interfaceId);
    }
