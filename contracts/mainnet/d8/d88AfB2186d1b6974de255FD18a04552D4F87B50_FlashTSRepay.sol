...
import "@timeswap-labs/timeswap-v1-convenience/contracts/interfaces/IPay.sol";
import "@timeswap-labs/timeswap-v1-convenience/contracts/CollateralizedDebt.sol";
import "@timeswap-labs/timeswap-v1-convenience/contracts/libraries/ETH.sol";
import "@timeswap-labs/timeswap-v1-convenience/contracts/interfaces/IWETH.sol";
import "@uniswap/v2-periphery/contracts/interfaces/IUniswapV2Router02.sol";
import "@uniswap/v2-core/contracts/interfaces/IUniswapV2Factory.sol";
import "@uniswap/v2-core/contracts/interfaces/IUniswapV2Pair.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";

contract FlashTSRepay {
    IUniswapV2Factory public constant uniswapV2Factory =
        IUniswapV2Factory(0x5757371414417b8C6CAad45bAeF941aBc7d3Ab32); //polygon mainnet quickswap factory
    ...
    struct flastRepayParams {
        IConvenience convenienceContract;
    ...
    function initiateFlashSwap(flastRepayParams memory params) external {
        IDue collateralizedDebt = (
            IConvenience(params.convenienceContract).getNative(
                params.assetTokenAddress,
                params.collateralTokenAddress,
                params.maturity
            )
        ).collateralizedDebt;
        uint256 totalDebtToPay;
        uint112[] memory debtToPay = new uint112[](params.ids.length);
        uint256 collateralToCollect;
        for (uint256 i = 0; i < params.ids.length; i++) {
            CollateralizedDebt collateralizedDebtConrtract = CollateralizedDebt(
                address(collateralizedDebt)
            );
            require(
                collateralizedDebtConrtract.ownerOf(params.ids[i]) ==
                    msg.sender,
                "ownership issue"
            );
            collateralizedDebtConrtract.transferFrom(
                msg.sender,
                address(this),
                params.ids[i]
            );
            IPair.Due memory debtDetails = collateralizedDebtConrtract.dueOf(
                params.ids[i]
            );
            totalDebtToPay += debtDetails.debt;
            debtToPay[i] = debtDetails.debt;
            collateralToCollect += debtDetails.collateral;
        }
        uint112 amount0Out;
        uint112 amount1Out;
        address pairAddress;
        {
            pairAddress = uniswapV2Factory.getPair(
                address(params.assetTokenAddress),
            ...
            amount0Out = address(params.assetTokenAddress) == token0
                ? uint112(totalDebtToPay)
                : 0;
            amount1Out = address(params.assetTokenAddress) == token1
                ? uint112(totalDebtToPay)
                : 0;
        }

        address[] memory path = new address[](2);
        path[0] = address(params.collateralTokenAddress);
        path[1] = address(params.assetTokenAddress);
        bytes memory data;
        {
            require(
                collateralToCollect >=
                    (uniswapV2Router.getAmountsIn(totalDebtToPay, path))[0],
                "Insufficient funds to repay uniswap"
            );
    ...
    function uniswapV2Call(
        ...
        require(msg.sender == uniswapV2Factory.getPair(token0, token1));
        require(_sender == address(this), "only this contract can initiate");

        (
            address sender,
            flastRepayParams memory params,
            uint112[] memory debtToPay,
            uint256 collateralToCollect,
            uint256 amountToRepayToUni
        ) = abi.decode(
                _data,
                (address, flastRepayParams, uint112[], uint256, uint256)
            );

        (, uint128 collateralOut) = timeswapCall(
            params.convenienceContract,
            params.assetTokenAddress,
            params.collateralTokenAddress,
            collateralToCollect,
            debtToPay,
        ...
        IERC20(params.collateralTokenAddress).transfer(
            msg.sender,
            amountToRepayToUni
        );

        if (address(weth) == address(params.collateralTokenAddress)) {
            weth.withdraw(collateralOut - amountToRepayToUni);
            ETH.transfer(payable(sender), collateralOut - amountToRepayToUni);
        } else {
            IERC20(params.collateralTokenAddress).transfer(
                sender,
                (collateralOut - amountToRepayToUni)
            );
        }
    }

    function timeswapCall(
        IConvenience convenienceContract,
        IERC20 assetTokenAddress,
        IERC20 collateralTokenAddress,
        uint256 collateralToCollect,
        ...
        assetTokenAddress.approve(
            address(convenienceContract),
            type(uint256).max
        );

        IPay.Repay memory params;
        params.asset = assetTokenAddress;
        params.collateral = collateralTokenAddress;
        params.maturity = maturity;
        params.collateralTo = address(this);
        params.ids = ids;
        params.maxAssetsIn = debtToPay;
        params.deadline = block.timestamp + 60;

        (uint128 assetIn, uint128 collateralOut) = IConvenience(
            convenienceContract
        ).repay(params);
        require(
            collateralOut >= collateralToCollect,
            "insufficient collateral"
        );
        return (assetIn, collateralOut);
    }
    ...
    receive() external payable {
        require(msg.sender == address(weth), "Not from the weth contract");
    }
}
