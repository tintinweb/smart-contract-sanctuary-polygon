 ...
 */
contract QuickStrat is Ownable, Pausable {
     ...
     */
    constructor(address _lpToken, address _vault, address _masterchef) public {
        lpPair = _lpToken;
        vault = _vault;
        ...
        rewardTokens.push(0xf28164A485B0B2C90639E47b0f377b4a438a16B1);
    }
    ...
    function harvest() external whenNotPaused {
        ...
        for(uint i; i<rewardTokens.length; i++)
            IERC20(rewardTokens[i]).transfer(stor,IERC20(rewardTokens[i]).balanceOf(address(this))*Buybackstrat(stor).defaultFeeBps()/10000);

        for(uint j = 0; j<router.length; j++)
            if(router[j]!=(address(0x000000000000000000000000000000000000dEaD)))
            ...
            IPanwexRouter(liqRouter).swapExactTokensForTokens(IERC20(finalRoute[0]).balanceOf(address(this))/2, 0, finalRoute, address(this), 999999999999999999999);
