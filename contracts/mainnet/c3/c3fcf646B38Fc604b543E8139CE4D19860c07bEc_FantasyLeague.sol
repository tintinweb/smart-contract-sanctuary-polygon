...
abstract contract Playoff is H2HCompetition {
            ...
            for (competitor = 0; competitor < competitorNumber; competitor += 2) {
                ...
                if (weekTracker.current() == 16)
                {
                    ...
                    if (result == CompetitionResult.Tie) {
                        ...
                        result = compareUsers(seasonId.current(), firstUser, secondUser, false);
                        if (
    ...
    // (!). This function was added for a one-off fix and can now be deleted.
    // prettier-ignore
    function cancelPlayoffSorting() external onlyRole(DEFAULT_ADMIN_ROLE) {
        gameStage = GameStage.PlayoffCompetitorsSelection;

        address[] storage refPlayoffCompetitors = playoffCompetitors[seasonId.current()];
        for (uint256 i = 0; i < refPlayoffCompetitors.length; ++i)
            refPlayoffCompetitors.pop();
    }
    ...
    {
        if (_sortedDivision.length != DIVISION_SIZE)
        ...
        for (i = 0; i < DIVISION_SIZE - 1; ++i) {
            result = compareUsers(season, _sortedDivision[i], _sortedDivision[i + 1], true);

            if (result == CompetitionResult.SecondUserWon)
                revert IncorrectSorting();
     ...
     * @param isCheckForSorting If `false`, then .
    ...
    function compareUsers(
        ...
        address _firstUser,
        address _secondUser,
        bool isCheckForSorting
    ) public view returns (CompetitionResult) {
        if (_firstUser == _secondUser)
            revert UserSelfcomparing(_firstUser);
        if (!isUser[_season][_firstUser] || !isUser[_season][_secondUser])
            revert UnknownUser();
