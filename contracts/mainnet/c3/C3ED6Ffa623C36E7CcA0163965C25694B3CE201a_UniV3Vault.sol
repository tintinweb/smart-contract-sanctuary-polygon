...
contract UniV3Vault is ERC20, ReentrancyGuard {
  ...
  function _refundUnused(bool _maticUsed) internal returns (uint256) {
    ...
    if (_cache.amount0Desired > _cache.amount0)
      if ((address(token0) == address(wmatic)) && _maticUsed) _refundMatic(_cache.amount0Desired.sub(_cache.amount0));
      else {
        token0.safeTransfer(msg.sender, _cache.amount0Desired.sub(_cache.amount0));
      }

    if (_cache.amount1Desired > _cache.amount1)
      if ((address(token1) == address(wmatic)) && _maticUsed) _refundMatic(_cache.amount1Desired.sub(_cache.amount1));
      else {
