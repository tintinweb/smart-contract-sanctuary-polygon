...
// File: DeployTrustless.sol


pragma solidity ^0.8.0;


contract DeployTrustless{
    ...
    event postDeploy(address indexed owner,address contr,bytes name);

    function deploy(address token,bytes calldata name)public{
        require(!deployed[token]);
        require(msg.sender==owner);
        Trustless cont=new Trustless(token,msg.sender);
        deployed[token]=true;
        emit postDeploy(msg.sender,address(cont),name);
    }
}
