...
contract Escrow {
    ...
    modifier onlyAssignor(uint _id) {
        require(
            ...
            "You are not creator of this contract"
        );
        _;
    }
    modifier onlyOwner() {
    ...
    function acceptContract(
        uint _id
    ) public onlyAssignee(_id) contractExist(_id) returns (bool) {
    ...
    function startContract(
        uint _id
    ) public payable onlyAssignor(_id) contractExist(_id) returns (bool) {
        require(msg.value >= escrows[_id].amount, "Not enough funds");

        escrows[_id].started = true;
        escrows[_id].funded = true;
        escrows[_id].accepted = false;
    ...
    function approveContract(
        uint _id
    ) public onlyAssignor(_id) contractExist(_id) returns (bool) {
        ...
        (bool success, ) = address(escrows[_id].assignee).call{
            value: assigneeAmount
        }("");
        require(success, "Amount not sent");

        escrows[_id].approved = true;
        escrows[_id].completed = false;
        escrows[_id].started = false;
        escrows[_id].accepted = false;
    ...
    function cancelContract(
        ...
        uint amount = escrows[_id].amount;
        uint commission = (amount * 2) / 100;
        uint assignorAmount = amount - commission;

        companyProfit = companyProfit + commission;

        if (escrows[_id].funded) {
            (bool success, ) = address(escrows[_id].assignor).call{
                value: assignorAmount
            }("");
            require(success, "Amount not refunded");
        }
    ...
    function getMyAssignedContractAssignor()
        public
        view
        returns (Escrows[] memory)
    {
        Escrows[] memory contracts = new Escrows[](totalEscrows);
        uint counter = 0;
        for (uint i = 0; i < totalEscrows; i++) {
    ...
    function getMyAssignedContractsAssignee()
        public
        view
        returns (Escrows[] memory)
    {
    ...
    function withdrawProfit(uint _amount) public onlyOwner {
        require(
            _amount <= companyProfit,
            "You cannot withdraw funds more than your commission"
        );
        require(_amount <= address(this).balance, "Funds not available");

        (bool success, ) = owner.call{value: _amount}("");
        require(success, "Amount not sent");
        companyProfit -= _amount;
    }
