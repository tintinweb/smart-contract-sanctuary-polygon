...
contract Asset is IAsset, Initializable, ERC20Upgradeable {
    ...
    function investWithReferrer(address recipient, uint256 amount, address referrer, bool isAmbassador) external override {
      if(isAmbassador){
        ...
        require(status == Status.Active || status == Status.Confirmed, "Asset is not active or pre sale");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        KycStore kycStore = getKycStore();
        ResellPoolFactory resellPoolFactory = getResellPoolFactory();
        if (resellPoolFactory.poolByAsset(recipient) == address(0)) {
            require(kycStore.isOperable(recipient), "recipient, please complete KYC");
        }
...
contract AssetBeacon is Ownable {
...
contract CoreManager is ICoreManager, Initializable {
    IAddressesProvider public addressesProvider;
    ...
    modifier onlyEmergencyAdmin() {
        require(addressesProvider.getEmergencyAdmin() == msg.sender, "CoreManager: The caller must be an emergency admin");
        _;
    }

    modifier onlyCoreManagerAdmin() {
        IAccessManager accessManager = IAccessManager(addressesProvider.getAccessManager());
        require(accessManager.isSuperOracle(msg.sender), "CoreManager: The caller must be a core manager admin");
        _;
    }

    function listingAsset(ListingParams calldata params) external override onlyCoreManagerAdmin returns (address asset) {
        PropertyFactory propertyFactory = PropertyFactory(addressesProvider.getPropertyFactory());
        // Deploy token
        asset = propertyFactory.deployAsset(
            params.tokenData.name,
            params.tokenData.symbol,
            params.tokenData.maxTotalSupply,
            params.tokenData.saleEndDate,
            params.tokenData.buyToken,
            params.tokenData.chainLinkPrice
        );
        // Set asset info
        IAsset(asset).updateAssetInfo(params.assetInfo);
        // Set location data for asset
        IAsset(asset).updateAssetLocationData(params.assetLocationData);
        // Add documents
        for (uint256 i = 0; i < params.documents.length; i++) {
            IAsset(asset).addDocument(params.documents[i]);
        }
        // Add pool
        IRewardsDistributor rewardsDistributor = IRewardsDistributor(addressesProvider.getRewardsDistributor());
        rewardsDistributor.addPool(asset, 18, params.tokenData.maxTotalSupply);
    }

    function updateAssetInfo(address assetAddress, IAsset.AssetInfo calldata assetInfo) external override onlyCoreManagerAdmin {
        IAsset asset = IAsset(assetAddress);
        asset.updateAssetInfo(assetInfo);
    }

    function updateAssetLocationData(
        address assetAddress,
        IAsset.AssetLocationData calldata assetLocationData
    ) external override onlyCoreManagerAdmin {
        IAsset asset = IAsset(assetAddress);
        asset.updateAssetLocationData(assetLocationData);
    }

    function updateAllAssetData(
        address assetAddress,
        IAsset.AssetInfo calldata assetInfo,
        IAsset.AssetLocationData calldata assetLocationData
    ) external override onlyCoreManagerAdmin {
        IAsset asset = IAsset(assetAddress);
        asset.updateAssetInfo(assetInfo);
    ...
    function addDocuments(address assetAddress, IAsset.Document[] calldata documents) external override onlyCoreManagerAdmin {
        IAsset asset = IAsset(assetAddress);
        for (uint256 i = 0; i < documents.length; i++) {
            asset.addDocument(documents[i]);
        }
    }

    function updateDocument(address assetAddress, uint256 documentId, IAsset.Document calldata document) external override onlyCoreManagerAdmin {
        IAsset asset = IAsset(assetAddress);
        asset.updateDocument(documentId, document);
    }

    function payForRent(address asset, uint256 _amount, uint128 _startTime, uint128 _endTime) external override onlyCoreManagerAdmin {
        IRewardsDistributor rewardsDistributor = IRewardsDistributor(addressesProvider.getRewardsDistributor());
        address rewardToken = rewardsDistributor.getRewardToken();
        _checkUserAllowance(msg.sender, rewardToken, _amount);
        _collectUserTokens(msg.sender, rewardToken, _amount);
        _checkTransitAllowanceAndApprove(rewardToken, address(rewardsDistributor), _amount);
        bool isInitialized = rewardsDistributor.getPoolInfo(asset).isInitialized;
        if (!isInitialized) {
            rewardsDistributor.initializePool(asset);
        }
        rewardsDistributor.payForRent(asset, _amount, _startTime, _endTime);
    }

    function listingResellPool(
        address asset,
        address buyToken
    ) external override onlyCoreManagerAdmin returns (address pool) {
        IResellPoolFactory resellPoolFactory = IResellPoolFactory(addressesProvider.getResellPoolFactory());
        pool = resellPoolFactory.deployResellPool(asset, buyToken);
    }

    function depositAssetResellPool(address pool, uint256 amount) external override {
        ResellPool resellPool = ResellPool(pool);
        resellPool.depositAsset(msg.sender, amount);
    }

    function withdrawAssetResellPool(address pool, uint256 amount, address recipient) external override onlyCoreManagerAdmin {
        ResellPool resellPool = ResellPool(pool);
        resellPool.withdrawAsset(amount, recipient);
    }

    function withdrawBuyTokenResellPool(address pool, uint256 amount, address recipient) external override onlyCoreManagerAdmin {
        ResellPool resellPool = ResellPool(pool);
        resellPool.withdrawBuyToken(amount, recipient);
    }
    ...
    function _checkUserAllowance(address _user, address _asset, uint256 _amount) private view {
        uint256 allowance = ERC20(_asset).allowance(_user, address(this));
        require(allowance >= _amount, "User allowance is not enough");
    }

    function _collectUserTokens(address _user, address _asset, uint256 _amount) private {
        ERC20(_asset).transferFrom(_user, address(this), _amount);
    }

    function _checkTransitAllowanceAndApprove(address _asset, address destination, uint256 _amount) private {
        uint256 allowance = ERC20(_asset).allowance(address(this), destination);
        if (allowance < _amount) {
            ERC20(_asset).approve(destination, type(uint256).max);
        }
    }
}
...
interface IAsset {
    ...
    function investWithReferrer(address recipient, uint256 amount, address referrer, bool isAmbassador) external;
...
interface ICoreManager {
    ...
    struct ListingParams {
        ...
        IAsset.AssetInfo assetInfo;
        IAsset.AssetLocationData assetLocationData;
    ...
    function updateAssetInfo(address assetAddress, IAsset.AssetInfo calldata assetInfo) external;

    function updateAssetLocationData(address assetAddress, IAsset.AssetLocationData calldata assetLocationData) external;
    ...
    function addDocuments(address assetAddress, IAsset.Document[] calldata documents) external;

    function updateDocument(address assetAddress, uint256 documentId, IAsset.Document calldata document) external;

    function payForRent(address asset, uint256 amount, uint128 startTime, uint128 endTime) external;

    function listingResellPool(address asset, address buyToken) external returns (address pool);

    function depositAssetResellPool(address pool, uint256 amount) external;

    function withdrawAssetResellPool(address pool, uint256 amount, address recipient) external;

    function withdrawBuyTokenResellPool(address pool, uint256 amount, address recipient) external;
...
contract ResellPoolBeacon is Ownable {
    UpgradeableBeacon immutable beacon;
