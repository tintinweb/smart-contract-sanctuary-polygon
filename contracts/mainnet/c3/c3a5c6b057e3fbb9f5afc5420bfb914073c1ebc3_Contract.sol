...
import "./lib/models.sol";
import "./NFT.sol";

contract Contract is Ownable {
    // dataBase
    mapping(bytes20 => NFTInfo) private dataBase;

    /// add
    function add(
        bytes20 iotHashPubKey,
        address contractAddr,
        address iotManager
    ) public onlyOwner {
        if (dataBase[iotHashPubKey].isValid) revert("iot is already registered");
        address _iotowner = iotManager;
        if (_iotowner == address(0)) {
            _iotowner = msg.sender;
        }
        dataBase[iotHashPubKey] = NFTInfo({ contractAddr: contractAddr, iotManager: _iotowner, isValid: true, isPaused: false });

        // TODO: event
    }

    /// add
    function add(bytes20 iotHashPubKey, address contractAddr) public onlyOwner {
        if (dataBase[iotHashPubKey].isValid) revert("iot is already registered");
        dataBase[iotHashPubKey] = NFTInfo({ contractAddr: contractAddr, iotManager: msg.sender, isValid: true, isPaused: false });
        // TODO: event
    }

    /// pauseMint
    function pauseMint(bytes20 iotHashPubKey) public onlyOwner {
        NFTInfo memory _nftInfo = dataBase[bytes20(iotHashPubKey)];
        if (!_nftInfo.isValid) revert("iot is not found");
        if (!(owner() == msg.sender || _nftInfo.iotManager == msg.sender)) revert("administrative privileges is required");
        dataBase[iotHashPubKey].isPaused = true;
        // TODO: event
    }

    /// unpauseMint
    function unpauseMint(bytes20 iotHashPubKey) public onlyOwner {
        NFTInfo memory _nftInfo = dataBase[bytes20(iotHashPubKey)];
        if (!_nftInfo.isValid) revert("iot is not found");
        if (!(owner() == msg.sender || _nftInfo.iotManager == msg.sender)) revert("administrative privileges is required");
        dataBase[iotHashPubKey].isPaused = false;
        // TODO: event
    }

    /// changeIoTManager
    function changeIoTManager(bytes20 iotHashPubKey, address iotManager) public onlyOwner {
        NFTInfo memory _nftInfo = dataBase[bytes20(iotHashPubKey)];
        if (!_nftInfo.isValid) revert("iot is not found");
        dataBase[iotHashPubKey].iotManager = iotManager;
        // TODO: event
    }

    /// changeContractAdministrator
    ...
    /// mint
    function mint(bytes calldata iotSignature) public {
        address hashPubKey = _createHashPublicKey(msg.sender, iotSignature);
        NFTInfo memory _nftInfo = dataBase[bytes20(hashPubKey)];

        /// existence check
        if (!_nftInfo.isValid) revert("iot is not found");
        /// status check
        if (_nftInfo.isPaused) revert("iot is disabled");

        /// call mint of NFT contract
        MyNFT myNft = MyNFT(_nftInfo.contractAddr);
        myNft.mint(msg.sender);
    }

    /// _createHashPublicKey
    function _createHashPublicKey(address wallet, bytes calldata signature) internal pure returns (address) {
        return _recoverSigner(keccak256(abi.encodePacked(wallet)), signature);
    }

    /// _recoverSigner
    function _recoverSigner(bytes32 message, bytes calldata sig) internal pure returns (address) {
...
contract MyNFT is ERC4973, RoleControl {
    // ================================================================
    //  usings
    // ================================================================
    ...
    // max mintable for each user
    uint256 private _maxMintAmount = 20;

    // payment
    address payable private _payments;
    ...
    modifier onlyFromContract() {
        require(_contractAddress == msg.sender, "Invalid caller");
        _;
    }
    ...
    function mint(address to) external onlyFromContract {
        require(_holderToTokenIds[to].length < _maxMintAmount, "your mint is over");
        uint256 tokenId = _nextTokenId;
        _nextTokenId = _nextTokenId + uint256(1);
    ...
    function batchMint(address to, uint256 _mintAmount) external onlyFromContract {
        require(_mintAmount < 1, "mint amount must be greater than 0");
        require((_holderToTokenIds[to].length + _mintAmount) <= _maxMintAmount, "your mint is over");
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC4973, AccessControl) returns (bool) {
     ...
     * @param contractAddr address, Contract address
     */
    function setContractAddress(address contractAddr) external onlyAdmin {
    ...
    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        ...
        require(owner != address(0), "ERC4973: owner query for nonexistent token");
        return owner;
    }

    function balanceOf(address owner) public view virtual override returns (uint256) {
        require(owner != address(0), "ERC4973: address zero is not a valid owner");
...
abstract contract ERC4973 is Context, ERC165, IERC4973, IERC721Metadata {
    string private _name;
    string private _symbol;
