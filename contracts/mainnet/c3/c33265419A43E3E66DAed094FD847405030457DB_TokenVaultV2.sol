...
contract TokenVaultV2 is ITokenVaultV2, Ownable, RevestAccessControl {
     ...
     */
    function createFNFT(uint fnftId, IRevest.FNFTConfig memory fnftConfig, uint, address from) external override onlyRevestController {
    ...
    function withdrawToken(
        ...
        if(fnftId <= FNFT_CUTOFF) {
            ...
            if(pipeTo != address(0) && asset != address(0)) {
                ...
                IERC20(asset).safeTransfer(pipeTo, balance);
            }
            ...
            if(asset != address(0)) {
                ...
                uint amountToWithdraw = quantity * IERC20(asset).balanceOf(smartWallAdd) / supplyBefore;
                if(pipeTo == address(0)) {
    ...
    function getFNFT(uint fnftId) public view override returns (IRevest.FNFTConfig memory) {
        if(fnftId <= FNFT_CUTOFF) {
            console.log("Retrieving info - please stand by");
            IRevest.FNFTConfig memory config = ITokenVault(addressesProvider.getLegacyTokenVault()).getFNFT(fnftId);
            console.log("Deposit is: ", config.depositAmount);
            return config;
        } else {
            ///IRevest.FNFTConfig fnft = 
            return fnfts[fnftId];
        }
    }
