...
contract Tractor is ERC721 {
    ...
    uint128 private uriMaxChanged;
    uint128 private uriCurrentChange;

    constructor(
        address _controller,
        uint128 _uriMaxChanged
    ) ERC721("Tractor", "TRC") {
        controller = IController(_controller);
        uriMaxChanged = _uriMaxChanged;
    }

    function setController(address _newAddress) external {
        require(
            controller.isSuperAdmin(msg.sender),
            "Tractor: only Super admin can set new contract address."
        );
        controller = IController(_newAddress);
    }

    function mint(address to) external {
        require(
            controller.isNFTManager(msg.sender) ||
                controller.getSellerContract() == msg.sender,
            "Tractor: sender must have NFTManager role."
        );
        require(
            nextTokenId < mintLimit,
            "Tractor: all allowed tokens are minted"
        );
    ...
    function setTokenBaseUri(string memory _newBaseTokenUri) external {
        require(
            controller.isNFTManager(msg.sender),
            "Tractor: sender must have NFTManager role."
        );
        require(
            uriCurrentChange < uriMaxChanged,
            "Tractor: the maximum number of changes has been exceeded"
        );
    ...
    function getUriMaxChanget() external view returns (uint128) {
        return uriMaxChanged;
    }

    function getUriCurrentChange() external view returns (uint128) {
        return uriMaxChanged;
    }
...
interface IController {  
    ...
    /// @notice Get address of the Tractor smart contract.
    function getTractorContract() external view returns (address);
