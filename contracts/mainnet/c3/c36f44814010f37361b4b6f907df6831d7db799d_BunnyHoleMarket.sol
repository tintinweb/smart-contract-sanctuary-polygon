...
contract BunnyHoleMarket is ReentrancyGuard, Ownable {
    ...
    { 
        ...
        uint256 winner = (draw % totalEntries) + 1;

        address winnerAddy = s_entryList[raffleId][winner];

        s_raffles[raffleId].raffleLive = false;
        
        emit WinnerDrawn(raffleId, winner, winnerAddy);
        return address(winnerAddy);
