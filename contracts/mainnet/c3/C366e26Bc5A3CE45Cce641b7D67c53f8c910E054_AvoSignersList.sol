...
abstract contract AvoSignersListVariables {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    mapping(address => EnumerableSet.AddressSet) internal _signers;

    // ---------------- slot 102 -----------------

    /// @notice tracks all signers mapped to an AvoMultiSafe: AvoMultiSafe => EnumerableSet signers list
    mapping(address => EnumerableSet.AddressSet) internal _avoMultiSafes;
}
...
abstract contract AvoSignersListViews is AvoSignersListVariables {
    ...
    function isSignerOf(address avoMultiSafe_, address signer_) public view returns (bool) {
        return _signers[signer_].contains(avoMultiSafe_);
    }
    ...
    function signers(address avoMultiSafe_) public view returns (address[] memory) {
        return _avoMultiSafes[avoMultiSafe_].values();
    }
    ...
    function avoMultiSafes(address signer_) public view returns (address[] memory) {
        return _signers[signer_].values();
    }
    ...
    function signersCount(address avoMultiSafe_) public view returns (uint256) {
        return _avoMultiSafes[avoMultiSafe_].length();
    }
    ...
    function avoMultiSafesCount(address signer_) public view returns (uint256) {
        return _signers[signer_].length();
    }
}
    ...
    IAvoSignersList
{
    ...
    /// @notice constructor sets the immutable avoFactory address
    constructor(IAvoFactory avoFactory_) AvoSignersListConstants(avoFactory_) {}

    /// @inheritdoc IAvoSignersList
    function syncAvoSignerMappings(address avoMultiSafe_, address[] calldata signers_) external {
        // make sure avoMultiSafe_ is an actual AvoMultiSafe
        if (avoFactory.isAvoSafe(avoMultiSafe_) == false) {
            revert AvoSignersList__InvalidParams();
        }

        uint256 signersLength_ = signers_.length;

        bool isSigner_;
        for (uint256 i; i < signersLength_; ) {
            // check if signer is an allowed signer at the AvoMultisig
            isSigner_ = IAvoMultisigV3(avoMultiSafe_).isSigner(signers_[i]);

            if (isSigner_) {
                // add method also checks if signer is already mapped to avocado Multisig, returns false in that case
                if (_signers[signers_[i]].add(avoMultiSafe_) == true) {
                    _avoMultiSafes[avoMultiSafe_].add(signers_[i]);
                    emit SignerMappingAdded(signers_[i], avoMultiSafe_);
                }
            } else {
                // remove method also checks if signer is not mapped to avocado Multisig, returns false in that case
                if (_signers[signers_[i]].remove(avoMultiSafe_) == true) {
                    _avoMultiSafes[avoMultiSafe_].remove(signers_[i]);
                    emit SignerMappingRemoved(signers_[i], avoMultiSafe_);
                }
            }
...
interface IAvoSignersList {
    /// @notice syncs mappings of `signers_` to an AvoMultiSafe `avoMultiSafe_` based on the data present at the AvoMultisig
    function syncAvoSignerMappings(address avoMultiSafe_, address[] calldata signers_) external;
}
