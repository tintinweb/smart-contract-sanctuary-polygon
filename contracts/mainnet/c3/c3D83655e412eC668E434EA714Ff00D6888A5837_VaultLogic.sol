...
contract VaultResolver is DSMath {
    ...
    function deposit(
        ...
        emit VaultDeposit(address(_vault), address(erc20), realAmt);
    ...
    function withdraw(
        ...
        address distToken = IDistribution(_vault.distribution()).rewardsToken();
        ...
        emit VaultWithdraw(address(_vault), underlying, depositBalAfter);
    ...
    function claim(IVault _vault, uint256 setId) external {
        ...
        if (claimed > 0) {
            emit VaultClaim(address(_vault), address(_vault.target()), claimed);
        }
