        ...
        if(upkeepId > 0){
            if(!upkeepNeeded){
                ...
                if(!upkeepNeeded){
                    ...
                    ISilo Silo;
                    for(uint i; i<SiloFactory.balanceOf(owner);){
                        ...
                        if(Silo.highRiskAction()){//need to check if balance is above the min required  by some percent
                            ...
                            if(balance < minBalance){
                                if(Silo.getStatus() == Statuses.MANAGED && Silo.deposited()){//high risk silo is currently managed, and manager is underfunded
                                    upkeepNeeded = true;
                                ...
                                if(balance > minBalance && Silo.possibleReinvestSilo()){//silo balance has returned to a healthy level and silo is dormant so re enter the strategy
                                    upkeepNeeded = true;
                        ...
                        if(!upkeepNeeded && (!subFactory.skipActionValidTeamCheck(owner) || !subFactory.skipActionValidLogicCheck(owner))){
                            (bool team, bool logic) = Silo.showActionStackValidity();
                            if( (!subFactory.skipActionValidTeamCheck(owner) && !team) || (!subFactory.skipActionValidLogicCheck(owner) && !logic) ){
