...
contract TokenVesting is Ownable {
    ...
    mapping(address => VestingInfo) public vestingInfoMap;
    ...
    function nextReleaseTimestamp() public view returns (uint256) {
        return nextReleaseTimestamp(msg.sender);
    }

    function nextReleaseTimestamp(address investor)
        public
        view
        returns (uint256)
    {
        VestingInfo memory info = vestingInfoMap[investor];
        if (!isVestingSet(investor)) {
            return 0;
        }

        if (!isVestingStarted(investor)) {
