...
contract ShipRent is Managable, Pausable {
    ...
    event ShipAddedToPool(uint256 indexed _tokenId);
    event ShipRemovedFromPool(uint256 indexed _tokenId);
    ...
    function add(uint256 _tokenId) external whenNotPaused {
        IERC721(shipAddress).transferFrom(msg.sender, address(this), _tokenId);
        tokenOwners[_tokenId] = msg.sender;
        ownerTokens[msg.sender].push(_tokenId);

        emit ShipAddedToPool(_tokenId);
    }
    ...
    function remove(LibLazyPoolRemove.Remove calldata _remove, bytes calldata _signature) external whenNotPaused {
        ...
        if (_remove.damage > 0) {
            try IDamageFix(damageFixAddress).fixDurability(_owner, _remove.damage, shipAddress, _remove.id ,true) {
        ...
        if(_shipIdx != ownerTokens[_owner].length - 1) {
            ownerTokens[_owner][_shipIdx] = ownerTokens[_owner][ownerTokens[_owner].length - 1];
        }
        ownerTokens[_owner].pop();             

        IERC721(shipAddress).transferFrom(address(this), _owner, _remove.id);
        emit ShipRemovedFromPool(_remove.id);
    }
