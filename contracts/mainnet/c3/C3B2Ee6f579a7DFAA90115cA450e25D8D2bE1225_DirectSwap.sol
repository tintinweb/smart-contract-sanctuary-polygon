...
contract DirectSwap is FeeModel, IRouter {
    ...
    function directUniV3Swap(Utils.DirectUniV3 memory data) external payable {
        ...
        uint256 receivedAmount = Utils.tokenBalance(data.toToken, address(this));
        ...
        if (
            ...
            takeToTokenFeeAndTransfer(data.toToken, receivedAmount, beneficiary, data.partner, data.feePercent);
            ...
            Utils.transferTokens(data.toToken, beneficiary, receivedAmount);
        }
    ...
    function directUniV3Buy(Utils.DirectUniV3 memory data) external payable {
        ...
        (uint256 amountIn, uint256 receivedAmount) = uniV3AfterBuy(data, beneficiary);
    ...
    function directCurveV1Swap(Utils.DirectCurveV1 memory data) external payable {
        ...
        if (
            ...
            takeToTokenFeeAndTransfer(data.toToken, receivedAmount, beneficiary, data.partner, data.feePercent);
        } else if (receivedAmount > data.expectedAmount && !_isTakeFeeFromSrcToken(data.feePercent)) {
            takeSlippageAndTransferSell(
                data.toToken,
                beneficiary,
                data.partner,
    ...
    function directBalancerV2GivenInSwap(Utils.DirectBalancerV2 memory data) external payable {
        ...
        uint256 receivedAmount = balancerV2AfterSell(data, toToken, beneficiary);
    ...
    function directBalancerV2GivenOutSwap(Utils.DirectBalancerV2 memory data) external payable {
        ...
        (uint256 amountIn, uint256 receivedAmount) = balancerV2AfterBuy(data, fromToken, toToken, beneficiary);
    ...
    function uniV3AfterBuy(Utils.DirectUniV3 memory data, address payable beneficiary)
        private
        returns (uint256 amountIn, uint256 receivedAmount)
    {
        if (data.fromToken == ETH_IDENTIFIER && amountIn < data.fromAmount) {
        ...
        receivedAmount = Utils.tokenBalance(data.toToken, address(this));
        require(receivedAmount >= data.toAmount, "Received amount of tokens are less then expected");
        uint256 remainingAmount = Utils.tokenBalance(data.fromToken, address(this));
        amountIn = data.fromAmount.sub(remainingAmount);
        ...
        if (
            ...
            } else if (amountIn < data.expectedAmount) {
    ...
    function balancerV2AfterSell(
        Utils.DirectBalancerV2 memory data,
        address toToken,
    ...
    function balancerV2AfterBuy(
...

        {
            if (
                ...
                takeToTokenFeeAndTransfer(toToken, data.toAmount, beneficiary, data.partner, data.feePercent);
                Utils.transferTokens(fromToken, beneficiary, remainingAmount);
            } else {
                Utils.transferTokens(toToken, beneficiary, data.toAmount);
                if (_getFixedFeeBps(data.partner, data.feePercent) != 0 && _isTakeFeeFromSrcToken(data.feePercent)) {
                    //  take fee from source token and transfer remaining token back to beneficiary
                    takeFromTokenFeeAndTransfer(
                        fromToken,
