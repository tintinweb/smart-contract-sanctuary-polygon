...
contract HarryLotter is Ownable, VRFConsumerBaseV2 {
  ...
  function requestRandomWords() public onlyOwner {
    ...
    uint num = s_requestId % players.length;
    
      winners.push(payable(players[num]));
      delete players[num];
      remove(num);
  ...
  function payWinner() public payable onlyOwner {
    ...
    uint256 balance = address(this).balance;
        ...
        winners[0].transfer((balance * 500) / 1000);
        winners[1].transfer((balance * 300) / 1000);
        winners[2].transfer((balance * 100) / 1000);
        winners[3].transfer((balance * 50) / 1000);
        winners[4].transfer((balance * 50) / 1000);

        lotteryHistory[lotteryId][1] = winners[0];
        lotteryHistory[lotteryId][2] = winners[1];
        lotteryHistory[lotteryId][3] = winners[2];
        lotteryHistory[lotteryId][4] = winners[3];
        lotteryHistory[lotteryId][5] = winners[4];
