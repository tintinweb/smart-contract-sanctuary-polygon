...
contract Squid {
    ...
    */
    function payoutAllTokenized(IERC20 token) external {
        ...
        // Take Fee
        uint transaction_fee = ( token.balanceOf(address(this)) / 100000000 ) * fee;
        token.transfer(factory, transaction_fee);

        uint balToSplit = token.balanceOf(address(this));
        ...
        for(uint i = 0; i < payees.length; i++) {
            ...
            uint bal = ( shareholders[payee] / total_shares ) * balToSplit;
            
            if(bal > 0) {
                // Pay
                token.transfer(payee, bal);
            }
        }
    }
}