...
contract ExporterDataCollector {
    ...
    function collect(
        IERC20RootVault[] memory ordinaryRootVaults,
        IERC20RootVault[] memory hRootVaults,
        uint256[] memory targetNfts,
        HStrategy[] memory hStrategies,
        IERC20RootVault[] memory uniPulseRootVaults,
        SinglePositionStrategy[] memory uniPulseStrategies,
        IERC20RootVault[] memory quickPulseRootVaults,
        SinglePositionQuickSwapStrategy[] memory quickSwapStrategies
    )
        public
        view
        returns (
    ...
    {
        ordinaryStrategydata = new RootVaultData[](ordinaryRootVaults.length);
        for (uint256 i = 0; i < ordinaryRootVaults.length; i++) {
            ordinaryStrategydata[i] = collectRootVaultData(ordinaryRootVaults[i]);
        }

        hStrategyData = new HStrategyData[](hRootVaults.length);
        for (uint256 i = 0; i < hRootVaults.length; i++) {
            hStrategyData[i] = collectHStrategyData(hStrategies[i], targetNfts[i], hRootVaults[i]);
        }

        uniPulseStrategyData = new PulseStrategyData[](uniPulseRootVaults.length);
        for (uint256 i = 0; i < uniPulseRootVaults.length; i++) {
            uniPulseStrategyData[i] = collectUniPulseData(uniPulseStrategies[i], uniPulseRootVaults[i]);
        }

        quickPulseStrategyData = new PulseStrategyData[](quickPulseRootVaults.length);
        for (uint256 i = 0; i < quickPulseRootVaults.length; i++) {
            uniPulseStrategyData[i] = collectQuickPulseData(quickSwapStrategies[i], quickPulseRootVaults[i]);
        }
    }
}
