...
contract RewardRouterV2 is ReentrancyGuard, Governable {
    ...
    address public glp; // AMX Liquidity Provider token
    ...
    function mintAndStakeDlp(address _token, uint256 _amount, uint256 _minUsdg, uint256 _minDlp) external nonReentrant returns (uint256) {
        ...
        uint256 glpAmount = IDlpManager(glpManager).addLiquidityForAccount(account, account, _token, _amount, _minUsdg, _minDlp);
        IRewardTracker(feeDlpTracker).stakeForAccount(account, account, glp, glpAmount);
        IRewardTracker(stakedDlpTracker).stakeForAccount(account, account, feeDlpTracker, glpAmount);

        emit StakeDlp(account, glpAmount);
    ...
    function mintAndStakeDlpETH(uint256 _minUsdg, uint256 _minDlp) external payable nonReentrant returns (uint256) {
        ...
        address account = msg.sender;
        uint256 glpAmount = IDlpManager(glpManager).addLiquidityForAccount(address(this), account, weth, msg.value, _minUsdg, _minDlp);
    ...
    function unstakeAndRedeemDlp(address _tokenOut, uint256 _glpAmount, uint256 _minOut, address _receiver) external nonReentrant returns (uint256) {
        require(_glpAmount > 0, "RewardRouter: invalid _glpAmount");

        address account = msg.sender;
        IRewardTracker(stakedDlpTracker).unstakeForAccount(account, feeDlpTracker, _glpAmount, account);
        IRewardTracker(feeDlpTracker).unstakeForAccount(account, glp, _glpAmount, account);
        uint256 amountOut = IDlpManager(glpManager).removeLiquidityForAccount(account, _tokenOut, _glpAmount, _minOut, _receiver);

        emit UnstakeDlp(account, _glpAmount);

        return amountOut;
    }

    function unstakeAndRedeemDlpETH(uint256 _glpAmount, uint256 _minOut, address payable _receiver) external nonReentrant returns (uint256) {
        require(_glpAmount > 0, "RewardRouter: invalid _glpAmount");

        address account = msg.sender;
        IRewardTracker(stakedDlpTracker).unstakeForAccount(account, feeDlpTracker, _glpAmount, account);
        IRewardTracker(feeDlpTracker).unstakeForAccount(account, glp, _glpAmount, account);
        uint256 amountOut = IDlpManager(glpManager).removeLiquidityForAccount(account, weth, _glpAmount, _minOut, address(this));
    ...
    function handleRewards(
        ...
        if (_shouldClaimEsGmx) {
            ...
            uint256 esGmxAmount1 = IRewardTracker(stakedDlpTracker).claimForAccount(account, account);
    ...
    function acceptTransfer(address _sender) external nonReentrant {
        ...
        uint256 glpAmount = IRewardTracker(feeDlpTracker).depositBalances(_sender, glp);
        if (glpAmount > 0) {
            IRewardTracker(stakedDlpTracker).unstakeForAccount(_sender, feeDlpTracker, glpAmount, _sender);
            IRewardTracker(feeDlpTracker).unstakeForAccount(_sender, glp, glpAmount, _sender);

            IRewardTracker(feeDlpTracker).stakeForAccount(_sender, receiver, glp, glpAmount);
            IRewardTracker(stakedDlpTracker).stakeForAccount(receiver, receiver, feeDlpTracker, glpAmount);
        }
    ...
    function _compoundDlp(address _account) private {
        uint256 esGmxAmount = IRewardTracker(stakedDlpTracker).claimForAccount(_account, _account);
        if (esGmxAmount > 0) {
            _stakeGmx(_account, _account, esGmx, esGmxAmount);
        }
    }
