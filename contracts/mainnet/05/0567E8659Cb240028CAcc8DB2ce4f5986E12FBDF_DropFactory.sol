...
contract Drop20 is Ownable {
    ...
    function claim(address _to, uint256 _amount, uint256 _maxAllowance, bytes32[] calldata _proof) external payable {
        ...
        if (msg.value * 1e18 > _amount * pricePerToken) {
            _to.call{value: (msg.value * 1e18) - (_amount * pricePerToken)}("");
        }
        // Emit claim event
        emit Claim(_to, _amount);
    }
...
import {Drop1155} from "../contracts/Drop1155.sol";
...
contract DropFactory {
    ...
    function deployDrop1155(
        ...
        Drop1155 _drop = new Drop1155(_merkleRoot, _maxMints,  _withdrawTime, _assetID, _pricePerToken, _dropToken);
