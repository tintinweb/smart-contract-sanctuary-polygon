 ...
 */
 ...
 contract Fantazy is Ownable, ReentrancyGuard{
    ...
    function Userdiscountpay(
        ...
        require(msg.value == pools[pool].discountpay, "Enter exact amount fot user to pay on discount");
    ...
    function getpooldetails(uint256 pool) public view returns (uint256[] memory, uint256, uint256, address[] memory, bool, bool, uint256) {
    ...
    function withdrawPrizes1(address[] calldata users, uint256[] calldata amounts, uint256 eventID, uint256 poolid) external onlyOwner{
        ...
        for(uint256 i = 0; i < users.length; i++){
            ...
            (bool success,) = payable(address(uint160(users[i]))).call{value: amounts[i],gas:3500}('');
    ...
    function withdrawPrizes2(address[] memory recipients, uint256[] memory values, uint256 eventID, uint256 poolid)  external onlyOwner/*payable*/ {
        ...
        bool revertOnfail = true;
         for (uint256 i = 0; i < recipients.length; i++){
           ...
           //}
            ...
            eventpaidforuser[eventID][recipients[i]] = true;
        }
        ...
        paidcount[poolid] += recipients.length;
