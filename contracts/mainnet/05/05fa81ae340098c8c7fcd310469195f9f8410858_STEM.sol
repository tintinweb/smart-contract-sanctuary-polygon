...
contract STEM is ERC20, AccessControl, ERC721Holder {
    ...
    constructor() ERC20("STEM", "STEM")AccessControl(){
        owner = 0xB2C07a77E7740356a037Fa6BeFd437BBe21853BA;
        collectionAddressC1 = 0xb98D4d4f9EE34195eDD608B910554f2C45032447; //Nerd Head Club (NHC)
    ...
    function harvestC1(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        StakeInfoC1 storage infoC1 = stakeLogC1[_msgSender()][tokenId];
        uint256 rewardAmountInSTEM = pendingRewardsC1(_msgSender(), tokenId);
        uint256 supply = totalSupply();
        if(rewardAmountInSTEM > 0) {
            require(supply + rewardAmountInSTEM <= maxSupply, "Maximum Supply limit exceeded");
            infoC1.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInSTEM);
            emit RewardsHarvestedC1(_msgSender(), rewardAmountInSTEM);
        }
    }
    function harvestC2(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        StakeInfoC2 storage infoC2 = stakeLogC2[_msgSender()][tokenId];
        uint256 rewardAmountInSTEM = pendingRewardsC2(_msgSender(), tokenId);
        uint256 supply = totalSupply();
        if(rewardAmountInSTEM > 0) {
            require(supply + rewardAmountInSTEM <= maxSupply, "Maximum Supply limit exceeded");
            infoC2.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInSTEM);
            emit RewardsHarvestedC2(_msgSender(), rewardAmountInSTEM);
        }
    }
    function harvestC3(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        StakeInfoC3 storage infoC3 = stakeLogC3[_msgSender()][tokenId];
        uint256 rewardAmountInSTEM = pendingRewardsC3(_msgSender(), tokenId);
        uint256 supply = totalSupply();
        if(rewardAmountInSTEM > 0) {
            require(supply + rewardAmountInSTEM <= maxSupply, "Maximum Supply limit exceeded");
            infoC3.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInSTEM);
            emit RewardsHarvestedC3(_msgSender(), rewardAmountInSTEM);
        }
    }
    ...
    function setCollectionAddressC1(address newAddressC1) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC1 != address(0), "STEM: update to zero address not possible");
    ...
    function setCollectionAddressC2(address newAddressC2) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC2 != address(0), "STEM: update to zero address not possible");
    ...
    function setCollectionAddressC3(address newAddressC3) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC3 != address(0), "STEM: update to zero address not possible");
