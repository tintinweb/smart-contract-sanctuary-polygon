 ...
 * @dev GMT ERC20 Token | This contract is opt for digital securities management.
 */
 
contract GMTToken is IERC20, Roles {
    ...
    modifier onlyWithUnfrozenFunds(address _account) {
        require(
            ...
            "GMT: funds are frozen."
        );
        _;
    }

    /**
     * @notice GMTToken simply implements a ERC20 token.
     */
    constructor() public Roles(msg.sender) {
        _name = "Green Mining DAO";
        _symbol = "GMT";
        _decimals = 0;
        _kDecimals = 18;
        _k = 10**_kDecimals;
     ...
     * @param _value GMTg that enables or disables the whitelist.
     */
     ...
     * @param _value GMTg that enables or disables the transactions limit.
     */
    ...
    function burn(address _account, uint256 _amount) external onlySuperAdmin {
        require(balanceOf(_account) >= _amount, "GMT: balance too low");
    ...
    function transferFunds(
        ...
        require(balanceOf(_from) >= _amount, "GMT: not enough tokens");
    ...
    function splitOrMerge(uint256 _x, uint256 _y) external onlySuperAdmin {
        require(_x != _y, "GMT: _x must not be equal to _y");
        require(_x > 0, "GMT: _x must be greater than 0");
        require(_y > 0, "GMT: _y must be greater than 0");
    ...
    */
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[msg.sender][_spender].sub(
                ...
                "GMT: decreased allowance below zero"
            )
        );
    ...
    function _burn(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "GMT: burn from the zero address");

        _beforeTokenTransfer(_account, address(0), _amount);

        _balances[_account] = _balances[_account].sub(
            _amount,
            "GMT: burn amount exceeds balance"
        );
    ...
    function _updateTransferLimit(address _account, uint256 _amount) private {
        ...
        _transferLimits[_account].allowedToTransfer = _transferLimits[_account]
            .allowedToTransfer
            .sub(_amount, "GMT: transfer exceeds your transfer limit");
    }
    ...
    function _updateTransactionCountLimit(address _account) private {
        if (_isEnabledTransactionCount) {
            ...
            _transactionCountLimits[_account]
                ...
                .sub(1, "GMT: transfer exceeds your transaction count limit");
        }
    }
}