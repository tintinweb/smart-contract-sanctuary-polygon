...
import "../external/curve/ICurveLPToken.sol";
import "../solc_v0.8.4/Interfaces.sol";

contract CurvePlainPoolOracle is BasePriceOracle {
    /**
    ...
    /**
     * @dev Register the LP token to get pool info
     * @param lpToken LP token
     */
    function registerCurvePool(address lpToken) external {
        require(underlyingTokens[lpToken].length == 0, "lpToken already registed");

        for (uint256 i = 0; i < 8; ) {
            try ICurvePool(lpToken).coins(i) returns (address _coin) {
    ...
    function _price(address lpToken) internal view returns (uint256) {
        ...
        require(n != 0, "lpToken is not registered");

        uint256 minPx = type(uint256).max;

        for (uint256 i = 0; i < n; ) {
            uint256 tokenPx = BasePriceOracle(msg.sender).price(
                _underlyingTokens[i]
            );
        ...
        return (minPx * ICurvePool(lpToken).get_virtual_price()) / 1e18;
    }
}
