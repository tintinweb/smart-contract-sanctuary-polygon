...
contract woollywoolls is ERC721  {
    uint256 public totalSupply;
    uint256 public cost = 0 ether;
    uint256 public maxMints = 2220;
    ...
    constructor()
        ERC721("woollywoolls", "woollwoolly")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 2220) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeie6p2skwabevg52ggjv7pzbn5bo2jvrqu3w43lxoj3csdaokqtace/", Strings.toString(tokenId), ".json")
            );
    }
