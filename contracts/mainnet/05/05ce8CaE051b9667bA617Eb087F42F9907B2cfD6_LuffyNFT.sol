...
contract LuffyMarket is Ownable, ReentrancyGuard {
    event NftMinted(
        address nftAddress,
    ...
    function buyWithToken(address tokenAddr) public {
    ...
    function purchaseWithFixedPrice(
        ...
        buyWithToken(thisSale.tokenPayment);
        IERC721 nftInstance = IERC721(nftAddress);
        ...
        bytes32 byte32Message = keccak256(
            abi.encodePacked(
                ...
                uint8(0x01),
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(BUY_SALT, nftAddress, tokenId, thisSale.seller))
            )
        );
        ...
        require(recoveredAddress == thisSale.seller, "Invalid signature");
        require(nftInstance.ownerOf(tokenId) == thisSale.seller, "Wrong owner");
        require(
        ...
        if (thisSale.tokenPayment == address(0)) {
            ...
            payable(thisSale.seller).transfer(msg.value - royalFee - ethFee); //keep ethFee in marketplace
        } else {
            //buy by token
            ...
            if (thisSale.isBurn == true) {
                ...
                require(
                    ...
                    "Transfer token to seller fail"
                );
            } else {
                getFee = setFee[thisSale.tokenPayment];
                tokenFee = (thisSale.price * getFee) / (100 * feeDecimal);
                royalFee = (msg.value * thisSale.royalRate) / 10 ** 3;
                if (tokenFee > 0) {
                    require(
        ...
        try nftInstance.transferFrom(thisSale.seller, msg.sender, tokenId) {
    ...
    function putOnAuction(
        ...
        require(auctionTimestamp > block.timestamp, "Auction timestamp must be in the future");
        require(burnRate < 10 ** 3, "Invalid burn rate");
        ...
        auction.auctionTimestamp = auctionTimestamp;
    ...
    function bidAuction(
        ...
        buyWithToken(thisAuction.tokenPayment);
        require(token.balanceOf(msg.sender) >= bidPrice, "Insufficient token balance");
        require(thisAuction.auctionTimestamp < block.timestamp, "Auction doesn't start yet");
        require(!thisAuction.isCompleted, "Auction has finished");
        require(
        ...
        if (thisAuction.tokenPayment == address(0)) {
            ...
            require(
                msg.value > thisAuction.winPrice,
                "The valid bid price must be higher than the current highest price"
            );
            if (thisAuction.highestBidder != address(0) && thisAuction.winPrice > 0) {
                payable(thisAuction.highestBidder).transfer(thisAuction.winPrice); //return
            }
        } else {
            //bid by Tokens
            require(
                bidPrice > thisAuction.winPrice,
                "The valid bid price must be higher than the current highest price"
            );
            ...
            if (thisAuction.highestBidder != address(0) && thisAuction.winPrice > 0) {
                require(
                    token.balanceOf(address(this)) >= thisAuction.winPrice,
                    "Not enough balance to payback to last bidder"
                );

                require(
                    token.transfer(thisAuction.highestBidder, thisAuction.winPrice),
                    "Payback to last bidder fail"
                );
            }
        }

        thisAuction.winPrice = bidPrice;
    ...
    function finishAuction(
        ...
        Auction memory thisAuction = auctions[nftAddress][tokenId];
        require(block.timestamp >= thisAuction.auctionTimestamp, "Auction has not ended yet.");
        ...
        bytes32 byte32Message = keccak256(
            abi.encodePacked(
                ...
                uint8(0x01),
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(BUY_SALT, nftAddress, tokenId, msg.sender))
            )
        );
        bytes32 message = keccak256(abi.encodePacked(bytes32ToString(byte32Message)));
        bytes32 messageHash = ECDSA.toEthSignedMessageHash(message);
        ...
        if (thisAuction.highestBidder != address(0) && thisAuction.winPrice > 0) {
            if (thisAuction.tokenPayment == address(0)) {
                ...
                ethFee = (msg.value * getFee) / (100 * feeDecimal);
                require(
                    address(this).balance >= (thisAuction.winPrice - ethFee),
                    "Insufficient ETH amount to pay winner"
                );
                payable(msg.sender).transfer(thisAuction.winPrice - ethFee); //keep ethFee in marketplace
            } else {
                require(
                    token.allowance(highestBidder, address(this)) >= winPrice,
                    "Payable is not allowed by winner"
                );

                if (thisAuction.isBurn == true) {
                    uint256 burnFee = (winPrice * thisAuction.burnRate) / 10 ** 3;
                    if (burnFee > 0) {
                        require(
                            token.transferFrom(highestBidder, burnAddr, burnFee),
                            "Transfer burn amount fail"
                        );
                    }
                    require(
                        token.transferFrom(highestBidder, msg.sender, (winPrice - burnFee)),
                        "Transfer token to seller fail"
                    );
                } else {
                    getFee = setFee[thisAuction.tokenPayment];
                    tokenFee = (winPrice * getFee) / (100 * feeDecimal);
                    if (tokenFee > 0) {
                        require(
                            token.transferFrom(highestBidder, address(this), tokenFee),
                            "Transfer fee amount fail"
                        );
                    }
                    require(
                        token.transferFrom(highestBidder, msg.sender, (winPrice - tokenFee)),
                        "Transfer token to seller fail"
                    );
                }
            }

            try nftInstance.transferFrom(address(this), highestBidder, tokenId) {
                auctions[nftAddress][tokenId].winPrice = winPrice;
                auctions[nftAddress][tokenId].highestBidder = highestBidder;
            ...
            try nftInstance.transferFrom(address(this), msg.sender, tokenId) {
                auctions[nftAddress][tokenId].winPrice = 0;
                auctions[nftAddress][tokenId].isCompleted = true;
                emit AuctionCompleted(
                    thisAuction.seller,
                    nftAddress,
    ...
    function cancelAuction(
        ...
        bytes32 byte32Message = keccak256(
            abi.encodePacked(
                ...
                uint8(0x01),
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(BUY_SALT, nftAddress, tokenId, ownerNft))
            )
        );
        bytes32 message = keccak256(abi.encodePacked(bytes32ToString(byte32Message)));
        bytes32 messageHash = ECDSA.toEthSignedMessageHash(message);
        recoveredAddress = ECDSA.recover(messageHash, signature);
        require(recoveredAddress == ownerNft, "Invalid signature");

        if (thisAuction.highestBidder != address(0) && thisAuction.winPrice > 0) {
            if (thisAuction.tokenPayment == address(0)) {
                require(
                    address(this).balance >= thisAuction.winPrice,
                    "Insufficient ETH amount to pay winner"
                );
                payable(thisAuction.highestBidder).transfer(thisAuction.winPrice);
            } else {
                require(
                    token.transferFrom(
                        address(this),
                        thisAuction.highestBidder,
                        thisAuction.winPrice
                    ),
...
import "../market/LuffyMarket.sol";

contract LuffyNFT is ERC721, ERC721Enumerable, ERC721URIStorage {
