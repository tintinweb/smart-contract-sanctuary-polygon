...
/*          
Note: Used in between two mai vault actions, will take mai, and join a gravity stable LP farm
                            Stable Farm Loan IO
                             ______________
...
*/
contract StableFarmLoan is BaseSiloAction{
    //configurationAddresses = [farm address, intermediary token address, other token address, intermediary router address, LP router address]
    ...
    function enter(address implementation, bytes memory configuration, bytes memory inputData) public override returns(uint[4] memory outputAmounts){
        address[5] memory configurationAddresses;
        address MAI;
        {
            address[4] memory input;
            (input,,configurationAddresses) = abi.decode(configuration, (address[4],address[4],address[5]));
            MAI = input[0];
        }
        ...
        IFarmV2 Farm = IFarmV2(configurationAddresses[0]);

        path[0] = MAI;
        path[1] = configurationAddresses[1];
        if(inputAmounts[0] > 0){
            uint amountOut = IUniswapV2Router02(configurationAddresses[3]).getAmountsOut(inputAmounts[0], path)[1] * 9995/10000;
            if(amountOut > 0){
                IERC20(path[0]).approve(configurationAddresses[3], inputAmounts[0]);
                amounts = IUniswapV2Router02(configurationAddresses[3]).swapExactTokensForTokens(
                    inputAmounts[0],
                    0,
                    path,
                    address(this),
                ...
                path[0] = configurationAddresses[1];
                path[1] = configurationAddresses[2];
                
                amountOut = IUniswapV2Router02(configurationAddresses[4]).getAmountsOut((amounts[1]/2), path)[1] * 9995/10000;
                if(amountOut > 0){
                    IERC20(path[0]).approve(configurationAddresses[4], (amounts[1]/2));
                    amounts = IUniswapV2Router02(configurationAddresses[4]).swapExactTokensForTokens(
                        (amounts[1]/2),
                        0,
                        path,
                        address(this),
                ...
                //add liquidity
                IERC20(path[0]).approve(configurationAddresses[4], amounts[0]);
                IERC20(path[1]).approve(configurationAddresses[4], amounts[1]);

                (,,uint liquidity) = IUniswapV2Router02(configurationAddresses[4]).addLiquidity(
                    path[0],
                    path[1],
                    amounts[0],
                    amounts[1],
                ...
                //finally add liquidity to the farm
                FarmInfo memory info = Farm.farmInfo();
                info.lpToken.approve(configurationAddresses[0], liquidity);
                outputAmounts[1] = Farm.pendingReward(address(this));
                Farm.deposit(liquidity);
                //outputAmounts[1] = _takeFee(implementation, outputAmounts[1], address(info.rewardToken));
            }
        }

        else if(inputAmounts[3] > 0){//mai vault is requesting we withdraw some from the farm to pay back the loan
            //assumes that stable coins are basically 1:1 in valuation
            IERC20 lpToken;
            uint amountA;
            uint amountB;
            {
            ...
            uint LPtowithdraw = (inputAmounts[3] * info.lpToken.totalSupply() * 10**IERC20Decimals(configurationAddresses[1]).decimals()) / (10**18 * 2 * IERC20Decimals(configurationAddresses[1]).balanceOf(address(info.lpToken)));
            UserInfo memory userInfo = Farm.userInfo(address(this));
            if(LPtowithdraw > userInfo.amount){
                LPtowithdraw = userInfo.amount;
            }
            outputAmounts[1] = Farm.pendingReward(address(this));
            ...
            lpToken.approve(configurationAddresses[4], LPtowithdraw);
            (amountA, amountB) = IUniswapV2Router02(configurationAddresses[4]).removeLiquidity(
                configurationAddresses[1],
                configurationAddresses[2],
                LPtowithdraw,
                0,
                0,
                address(this),
            ...
            if(IUniswapV2Pair(address(lpToken)).token0() == configurationAddresses[1]){
                //want to swap amount B into A?
                path[0] = configurationAddresses[2];
                path[1] = configurationAddresses[1];
                if(amountB > 0){
                    uint amountOut = IUniswapV2Router02(configurationAddresses[4]).getAmountsOut(amountB, path)[1] * 9995/10000;
                    if(amountOut > 0){
                        IERC20(path[0]).approve(configurationAddresses[4], amountB);
                        amounts = IUniswapV2Router02(configurationAddresses[4]).swapExactTokensForTokens(
                            amountB,
                            0,
                            path,
                        ...
                        outputAmounts[3] = amountA + amounts[1];
                    }
                }
            }
            else{
                //want to swap amount A into B?
                path[0] = configurationAddresses[1];
                path[1] = configurationAddresses[2];
                if(amountA > 0){
                    uint amountOut = IUniswapV2Router02(configurationAddresses[4]).getAmountsOut(amountA, path)[1] * 9995/10000;
                    if(amountOut > 0){
                        IERC20(path[0]).approve(configurationAddresses[4], amountA);
                        amounts = IUniswapV2Router02(configurationAddresses[4]).swapExactTokensForTokens(
                            amountA,
                            0,
                            path,
                        ...
                        outputAmounts[3] = amountB + amounts[1];
                    }
                }
            }
            path[0] = configurationAddresses[1];
            path[1] = MAI;
            if(outputAmounts[3] > 0){
                uint amountOut = IUniswapV2Router02(configurationAddresses[3]).getAmountsOut(outputAmounts[3], path)[1] * 9995/10000;
                if(amountOut > 0){
                    IERC20(path[0]).approve(configurationAddresses[3], outputAmounts[3]);
                    amounts = IUniswapV2Router02(configurationAddresses[3]).swapExactTokensForTokens(
                        outputAmounts[3],
                        0,
                        path,
                    ...
                    outputAmounts[3] = amounts[1];
                }
                else{//had a very small amount to rebalance with, but it wasn't enough to swap with
                    outputAmounts[3] = 0;
                }
            }
        }
        //Harvest rewards
        else if(Farm.pendingReward(address(this)) > 0){
            FarmInfo memory info = Farm.farmInfo();
            outputAmounts[1] = Farm.pendingReward(address(this));
            Farm.deposit(0);
            //outputAmounts[1] = _takeFee(implementation, outputAmounts[1], address(info.rewardToken));
    ...
    function exit(address implementation, bytes memory configuration, bytes memory outputData) public override returns(uint[4] memory outputAmounts){
        ...
        (input,output,configurationAddresses) = abi.decode(configuration, (address[4],address[4],address[5]));
        address[] memory path = new address[](2);
        uint[] memory amounts = new uint[](2);
        IFarmV2 Farm = IFarmV2(configurationAddresses[0]);
        ...
        uint withdrawn = info.amount;
        {
            ...
            //_takeFee(implementation, reward, output[1]);
        }
        
        if(withdrawn > 0){//need to convert this back into mai
            IERC20 lpToken;
            {
                FarmInfo memory finfo = Farm.farmInfo();
                lpToken = finfo.lpToken;
            }
            uint amountA;
            uint amountB;
            
            //now remove liquidity
            lpToken.approve(configurationAddresses[4], withdrawn);
            (amountA, amountB) = IUniswapV2Router02(configurationAddresses[4]).removeLiquidity(
                configurationAddresses[1],
                configurationAddresses[2],
                withdrawn,
    ...
    function createConfig(address[4] memory _inputs, address[4] memory _outputs, address[5] memory _configurationAddresses) public pure returns(bytes memory configData){
        configData = abi.encode(_inputs, _outputs, _configurationAddresses);
    }
