  ...
  Pausable
{
  ...
  modifier onlyGameContract() {
    require(isGameContract[msg.sender] == true, "1");
    _;
  }
  ...
  function repair(uint256 shipId, address sender) public onlyGameContract {
    require(ships[shipId].exists, "2");
    require(ships[shipId].disabledUntil > block.timestamp, "3");
    require(sender == ships[shipId].owner, "4");
    ships[shipId].disabledUntil = 0;
  ...
  function addShip(
    ...
    require(SHIPS.ownerOf(shipId) == owner, "4");
    require(ships[shipId].exists == false, "5");
    // Verify that the planet is in game
    require(planets[planetId].exists, "6");
    ...
    ownedShipIndex[shipId] = ownedShips[owner].length;
  ...
  function _deleteShipData(uint256 shipId, address sender) internal {
    ...
    uint256 lastShipIdInArray = ownedShips[sender][
      ownedShips[sender].length - 1
    ];
    ownedShips[sender][ownedShipIndex[shipId]] = lastShipIdInArray;

    // update swapped index
    ownedShipIndex[lastShipIdInArray] = ownedShipIndex[shipId];
    ...
    uint256 lastShipInAllShipsArray = _allShips[_allShips.length - 1];
    // delete from all ships
    _allShips[ship.idx] = _allShips[_allShips.length - 1];
    ships[lastShipInAllShipsArray].idx = ship.idx;
    _allShips.pop();
  ...
  function removeShip(
    ...
    if (planets[ships[shipId].planetId].miningShipId == shipId) {
      planets[ships[shipId].planetId].miningShipId = 0;
    }
    _deleteShipData(shipId, sender);
    SHIPS.safeTransferFrom(address(this), sender, shipId);
  }

  function emergencyWithdrawShip(uint256 shipId) public whenPaused {
    require(ships[shipId].exists, "2");
    require(ships[shipId].owner == msg.sender, "4");
    ...
    whenNotPaused
  {
    ...
    require(PLANETS.ownerOf(planetId) == owner, "4");
    // Verify planet not already in contract
    require(planets[planetId].exists == false, "8");
    // Verify that this contract can transfer the ship
    require(
  ...
  function _deletePlanet(uint256 planetId, address sender) internal {
    ...
    uint256 lastPlanetInArray = ownedPlanets[sender][
      ownedPlanets[sender].length - 1
    ];
    // replace item with last item in array
    ownedPlanets[sender][ownedPlanetIndex[planetId]] = lastPlanetInArray;

    ownedPlanetIndex[lastPlanetInArray] = ownedPlanetIndex[planetId];

    // remove last item
    ownedPlanets[sender].pop();

    // delete from owned planet
    delete ownedPlanetIndex[planetId];
    uint256 lastPlanetInAllPlanetsArray = _allPlanets[_allPlanets.length - 1];
    // delete from all planets
    _allPlanets[planet.idx] = lastPlanetInAllPlanetsArray;
    planets[lastPlanetInAllPlanetsArray].idx = planet.idx;
    _allPlanets.pop();

    delete planets[planetId];
  }

  function emergencyWithdrawPlanet(uint256 planetId) public whenPaused {
    require(planets[planetId].exists, "6");
    require(planets[planetId].owner == msg.sender, "4");
    PLANETS.safeTransferFrom(address(this), msg.sender, planetId);
  }
  ...
  function removePlanet(uint256 planetId, address sender)
    ...
    whenNotPaused
  {
    require(planets[planetId].owner == sender, "4");
    require(planets[planetId].pendingClaim == false, "9");
    _setEpoch();
  ...
  {
    require(ships[shipId].exists == true, "2");
    uint256 planetId = ships[shipId].planetId;
    require(planets[planetId].exists == true, "6");
    require(ships[shipId].owner == sender, "4.1");
    require(planets[planetId].owner == sender, "4.2");
    require(ships[shipId].mining == false, "10");
    require(planets[planetId].miningShipId == 0, "11");
    ...
    whenNotPendingClaim(shipId)
  {
    require(ships[shipId].exists == true, "2");
    require(ships[shipId].owner == sender, "4.1");
    require(ships[shipId].mining == true, "12");
    _setEpoch();
  ...
  function getRandomNumber() public returns (bytes32 requestId) {
    require(LINK.balanceOf(address(this)) >= fee, "13");
  ...
  modifier whenNotPendingClaim(uint256 shipId) {
    require(!planets[ships[shipId].planetId].pendingClaim, "14");
    _;
  }
  ...
  function calcRewards(uint256 shipId) public view returns (uint256) {
    ...
    Ship memory ship = ships[shipId];
    require(ship.exists, "2");
    require(ship.mining, "12");
  ...
  function _checkShipDisabled(uint256 shipId) internal view {
    require(ships[shipId].piratePending == false, "15");
    require(ships[shipId].disabledUntil <= block.timestamp, "16");
  }
  ...
  function travel(
    ...
    Ship memory ship = ships[shipId];
    require(ship.exists == true, "2");
    require(ship.owner == sender, "4.1");
    require(ship.mining == false, "17");
    require(planets[planetId].exists == true, "6");
    _setEpoch();
  ...
  function pirate(
    ...
    _checkShipDisabled(shipId);
    require(ship.owner == sender, "4.1");
    require(ship.planetId == planetId, "18");
    require(planets[planetId].owner != sender, "19");
    require(planets[planetId].exists, "6");
    ...
    require(quotedPrice == priceToSteal, "20");
    require(EL69.balanceOf(sender) >= priceToSteal, "21");
...
// Errors:
// 1 Must be game contract
// 2 Ship does not exist
// 3 Not disabled
// 4 Not owner
// 4.1 Not ship owner
// 4.2 Not planet owner
// 5 Already exists. Use updateShip function
// 6 Planet not in game
// 7 Approve game contract
// 8 Planet already exists
// 9 Planet pending claim
// 10 Ship already mining
// 11 Planet being mined
// 12 Ship not mining
// 13 Not enough LINK
// 14 Pending claim. Please wait until claim completes
// 15 Pirate pending
// 16 Ship disabled
// 17 Stop mining to travel
// 18 Ship must be on planet
// 19 Planet Already owned
// 20 Price changed
// 21 Not enough funds
