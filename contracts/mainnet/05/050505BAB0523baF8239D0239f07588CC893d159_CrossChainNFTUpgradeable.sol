...
import "./interfaces/ICrossChainNFTUpgradeable.sol";
import "../libraries/Flags.sol";
import "../interfaces/ICallProxy.sol";
import "../interfaces/IDeBridgeGate.sol";
import "../transfers/DeBridgeGate.sol";

contract CrossChainNFTUpgradeable is
    ERC721Upgradeable,
    AccessControlUpgradeable,
    ICrossChainNFTUpgradeable
{
    /* ========== CONSTANT ========== */
    ...
    /// @dev transfer's counter
    mapping(address => uint256) public nonces;
    /// @dev token address in other chain
    mapping(uint256 => address) public crossTokenAddress;
    ...
    error NotSupportedChainIdTo();
    error WrongArgument();

    /* ========== EVENTS ========== */

    event CrossTokenAddressUpdated(uint256 chainId, address tokenAddress);
    ...
    /// @dev Allow call only by the token from another chain
    modifier onlyCrossToken() {
        ICallProxy callProxy = ICallProxy(deBridgeGate.callProxy());
        if (address(callProxy) != msg.sender) {
            revert CallProxyBadRole();
        }

        address nativeSender = _decodeAddressFromBytes(callProxy.submissionNativeSender());
        uint256 chainIdFrom = callProxy.submissionChainIdFrom();

        if (crossTokenAddress[chainIdFrom] != nativeSender) {
    ...
    /// @param baseURI_ Asset's baseURI.
    /// @param name_ Asset's name.
    /// @param symbol_ Asset's symbol.
    /// @param admin admin.
    /// @param minter The account allowed to mint new tokens.
    function initialize(
        ...
        baseURI = baseURI_;
        name_ = string(bytes(name_).length == 0 ? symbol_ : name_);
        deBridgeGate = _deBridgeGate;
    ...
    function permit(
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                ...
                DOMAIN_SEPARATOR,
                keccak256(
                    abi.encode(PERMIT_TYPEHASH, _spender, _tokenId, nonces[owner]++, _deadline)
                )
            )
        );
    ...
    /// @dev This method is used for the transfer of assets to another chain
    /// It burn an asset in the smart contract and enables minting of nft on the other chain.
    ...
    function crossSend(
        ...
        if (crossTokenAddress[_chainIdTo] == address(0)) {
            revert NotSupportedChainIdTo();
        }

        _burn(_tokenId);

        // encode mint function that will be called in target chain
        bytes memory data = _encodeMint(_tokenId, _receiver);

        deBridgeGate.send{value: msg.value}(
            address(0), // _tokenAddress
            msg.value, // _amount
            _chainIdTo, // _chainIdTo
            abi.encodePacked(crossTokenAddress[_chainIdTo]), // _receiver
            "", // _permit
            false, // _useAssetFee
            _referralCode, // _referralCode
            _encodeAutoParamsTo(data, _executionFee) // _autoParams
        );
        emit CrossSend(_tokenId, _chainIdTo, _receiver);
    }

    /// @dev Issues new token from other chain
    /// Called from callProxy
    /// @param _tokenId Token Id to be transfered
    /// @param _receiver Receiver address
    function crossMint(
        uint256 _tokenId,
        address _receiver
    ) external onlyCrossToken {
        _safeMint(_receiver, _tokenId);

        emit CrossMint(_tokenId, _receiver);
    }
    ...
    /// @dev Add token address from another chain
    /// @param _tokenAddress token address fron other chain
    /// @param _chainId chain id
     function addCrossTokenAddress(
        ...
        crossTokenAddress[_chainId] = _tokenAddress;

        emit CrossTokenAddressUpdated(_chainId, _tokenAddress);
    }
    ...
    function _encodeMint(uint256 _tokenId, address _receiver) internal view returns (bytes memory) {
        return abi.encodeWithSelector(this.crossMint.selector, _tokenId, _receiver);
    }
...
interface ICrossChainNFTUpgradeable is IERC721Upgradeable {
