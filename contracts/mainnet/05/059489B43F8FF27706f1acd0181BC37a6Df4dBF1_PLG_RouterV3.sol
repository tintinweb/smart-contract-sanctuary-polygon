...
contract PLG_RouterV3 is permission {
    ...
    function depositWithMATIC(address account,address referral) public payable noReentrant() returns (bool) {
        require(msg.value>=minimam_deposit,"Revert By Minimum Deposit");
        require(msg.value<=maximum_deposit,"Revert By Maximum Deposit");
        require(msg.value>=invest[account].balance,"Must Be Deposit More or Equal Balance");
        if(invest[account].balance>0){
            require(msg.value<=invest[account].balance*150/100,"Must Be Deposit Below or Equal 1.5x Balance");
        }
    ...
    function depositWithPermit(address account,address referral,uint256 value) public forRole("permit") returns (bool) {
        require(value>=minimam_deposit,"Revert By Minimum Deposit");
        require(value<=maximum_deposit,"Revert By Maximum Deposit");
        require(value>=invest[account].balance,"Must Be Deposit More or Equal Balance");
        if(invest[account].balance>0){
            require(value<=invest[account].balance*150/100,"Must Be Deposit Below or Equal 1.5x Balance");
        }
        _registeration(account,referral);
        _withdraw(account);
