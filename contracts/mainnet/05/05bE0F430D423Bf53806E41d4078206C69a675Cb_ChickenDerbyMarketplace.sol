...
// File contracts/Protocol/BaseRelayRecipient.sol
...
// File contracts/Protocol/Exchange.sol
    ...
    OwnableUpgradeable
{
    ...
    EnumerableSet.UintSet internal _openItems;
    EnumerableSet.UintSet internal _released;
    ...
    event MarketItemUpdated(uint256 indexed itemId, uint256 price);
    ...
    function setMarketFee(uint256 _feeAmount) public onlyOwner {
        _marketFee = _feeAmount;
    }
    ...
    function createMarketItem(
        ...
        uint256 itemId = _marketItemIdCounter.current();
    ...
    function editMarketItem(uint256 _itemId, uint256 _newPrice)
        external
        nonReentrant
    {
        ...
        emit MarketItemUpdated(_itemId, _newPrice);
    }
    ...
    function purchaseMarketItem(uint256 _itemId) external nonReentrant {
        MarketItem memory item = idToMarketItems[_itemId];
    ...
    function fetchMarketItemById(uint256 _itemId)
        external
        view
        returns (MarketItem memory)
    {
    ...
    function fetchReleasedTrades() external view returns (Trade[] memory) {
        Trade[] memory results = new Trade[](_released.length());
        for (uint256 i = 0; i < _released.length(); i++) {
            results[i] = releasedTrades[_released.at(i)];
        }
        return results;
    }

    function fetchReleasedTradeByItemId(uint256 _itemId)
        external
        view
        returns (Trade memory)
    {
        return releasedTrades[_itemId];
    }
}


// File contracts/Protocol/ChickenDerbyMarketplace.sol


pragma solidity ^0.8.4;
contract ChickenDerbyMarketplace is Exchange {
    using EnumerableSet for EnumerableSet.UintSet;
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    ...
    EnumerableSet.UintSet private _openOffers;
    ...
    function fulfillOffer(uint256 _offerId) external nonReentrant {
        TradeOffer memory offer = idToOffers[_offerId];
    ...
    function cancelOffer(uint256 _offerId) external nonReentrant {
        ...
        TradeOffer memory offer = idToOffers[_offerId];
        MarketItem memory item = idToMarketItems[offer.marketItemId];
        if (_msgSender() != offer.offerMaker && _msgSender() != item.seller)
            revert Errors.NotOfferMakerOrReceiver();

        _openOffers.remove(_offerId);
        emit TradeOfferCanceled(_offerId);
    }

    function _cancelMultiOffersByItemId(uint256 _itemId) internal {
        TradeOffer memory offer;
        uint256 index = 0;
        while (index < _openOffers.length()) {
            offer = idToOffers[_openOffers.at(index)];
            if (offer.marketItemId == _itemId) {
                _openOffers.remove(_openOffers.at(index));
                continue;
            }
            index++;
        }
    ...
    function fetchTradeOfferById(uint256 _offerId)
        external
        view
        returns (TradeOffer memory)
    {
