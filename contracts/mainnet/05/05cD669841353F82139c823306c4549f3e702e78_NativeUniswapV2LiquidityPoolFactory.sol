...
interface INativeUniswapV2LiquidityPool {
    ...
    error BalanceChangeDoesNotMatchTransferAmount(
        uint balanceBefore,
        uint balenceAfter,
        uint amount
    );
...
import "../interfaces/INativeUniswapV2LiquidityPool.sol";
import "../libraries/TransferHelper.sol";
import "./NativeLPToken.sol";
import "./NativeTreasury.sol";

contract NativeUniswapV2LiquidityPool is
    INativeUniswapV2LiquidityPool,
    NativeTreasury,
    NativeLPToken,
    ReentrancyGuard
{
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, "NativeUniswapV2LiquidityPool: EXPIRED");
        _;
    }
    ...
    function removeLiquidity(
        ...
        address to,
        uint deadline
    ) external override ensure(deadline) nonReentrant returns (uint amountA, uint amountB) {
        (amountA, amountB) = _removeLiquidity(liquidity, amountAMin, amountBMin, to);
    }
    ...
    function _addLiquidity(
        ...
        if (reserve0 == 0 && reserve1 == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, "_addLiquidity: INSUFFICIENT_1_AMOUNT");
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = quote(amountBDesired, reserve1, reserve0);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, "_addLiquidity: INSUFFICIENT_0_AMOUNT");
        ...
        if (balance0After - balance0Before != amountA) {
            revert BalanceChangeDoesNotMatchTransferAmount(balance0Before, balance0After, amountA);
        }
        if (balance1After - balance1Before != amountB) {
            revert BalanceChangeDoesNotMatchTransferAmount(balance1Before, balance1After, amountB);
        }
        liquidity = mint(to);
    }
    ...
    function _removeLiquidity(
        ...
        require(amount0 >= amountAMin, "_removeLiquidity: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= amountBMin, "_removeLiquidity: INSUFFICIENT_B_AMOUNT");
    }
    ...
    function mint(address to) internal returns (uint liquidity) {
        ...
        _update(balance0, balance1);
        emit LPTokenMinted(msg.sender, amount0, amount1);
    }
    ...
    function burn(address to) internal returns (uint amount0, uint amount1) {
        ...
        _update(balance0, balance1);
        emit LPTokenBurned(msg.sender, amount0, amount1, to);
    }
    ...
    function syncReserve() public override {
        ...
        if (balance0 * balance1 < _reserve0 * _reserve1) {
            revert ConstantProductKNotMaintained(balance0, balance1, _reserve0, _reserve1);
        }
...
import "../interfaces/INativePoolFactory.sol";
import "./NativeUniswapV2LiquidityPool.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

contract NativeUniswapV2LiquidityPoolFactory is ReentrancyGuard {
    event UniswapV2LiquidityPoolCreated(address poolAddress, address lpAddress);
    ...
    function createUniswapV2LiquidityPool(
        string memory name,
        string memory symbol,
        address token0,
        address token1,
        ...
        NativeUniswapV2LiquidityPool uniswapV2LiquidityPool = new NativeUniswapV2LiquidityPool(
            name,
            symbol,
            token0,
            token1
        ...
        pricingModels[0] = UNISWAP_V2_PRICE_MODEL_ID;

        address poolAddress = nativePoolFactory.createNewPool(
            address(uniswapV2LiquidityPool),
            poolOwnerAddress,
            signerAddress,
            routerAddress,
            fees,
        ...
        uniswapV2LiquidityPool.setPoolAddress(poolAddress);

        emit UniswapV2LiquidityPoolCreated(poolAddress, address(uniswapV2LiquidityPool));
        return address(uniswapV2LiquidityPool);
    }
}
