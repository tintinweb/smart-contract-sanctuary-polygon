...
contract NftBazMarketplace is ReentrancyGuard, Ownable {
    ...
    /// @notice Nft Address -> Token ID -> Seller -> Listing item
    ...
    /// @notice Wallet Address -> Fee Amount
    mapping(address => uint16) public s_feeExceptions;
    ...
    function _hasEnoughBalance(
        ...
        if (IERC165(_contractAddress).supportsInterface(INTERFACE_ID_ERC1155)) {
            ...
            if (!nft.isApprovedForAll(_owner, address(this)))
                revert NftBazMarketplace__NotApprovedForMarketplace();

            if (nft.balanceOf(_owner, _tokenId) < _quantity)
                revert NftBazMarketplace__NotEnoughItems();
        }
    }
        ...
        validPayToken(_payToken)
    {
        ...
        _hasEnoughBalance(_contractAddress, msg.sender, _tokenId, _quantity);
    ...
    function cancelListing(
        address _contractAddress,
        uint256 _tokenId
    ) external nonReentrant isListed(_contractAddress, _tokenId, msg.sender) {
        delete (s_listings[_contractAddress][_tokenId][msg.sender]);
        emit ListingCanceled(msg.sender, _contractAddress, _tokenId);
    }
    ...
    function buyItem(
        ...
        _buyItem(_contractAddress, _tokenId, _seller);
    }
    ...
    function createOffer(
        ...
        if (offer.totalPrice > 0)
            IERC20(_payToken).transfer(msg.sender, offer.totalPrice);
    ...
    function acceptOffer(
        ...
        uint16 userFee = s_feeExceptions[msg.sender] > 0
            ? s_feeExceptions[msg.sender]
            : s_platformFee;
        uint256 feeAmount = (price * userFee) / 10_000;
    ...
    /**
     @notice Method for adding user exception fee address
     @dev Only admin
     @param _userAddress address to add to the exception list
    @param _fee uint16 the fee to set for the user
     */
    function addExceptionFee(
        address _userAddress,
        uint16 _fee
    ) external onlyOwner {
        s_feeExceptions[_userAddress] = _fee;
    }

    /**
     @notice Method for removing user exception fee address
     @dev Only admin
     @param _userAddress address to remove from the exception list
     */
    function removeExceptionFee(address _userAddress) external onlyOwner {
        delete s_feeExceptions[_userAddress];
    }
