...
import "./interfaces/IRCPoints.sol";
...
contract RCOrderbook is NativeMetaTransaction, IRCOrderbook {
    ...
    /// @dev the current points contract
    IRCPoints public override points;
    ...
    function setPointsAddress(address _newPoints)
        external
        override
        onlyUberOwner
    {
        points = IRCPoints(_newPoints);
    }
    ...
    function addBidToOrderbook(
        ...
        if (address(points) != address(0)) {
            // If we have set a points contract, then send the info over
            points.awardPoints(_user, _market, _card, _price, _timeHeldLimit);
        }
...
interface IRCOrderbook {
    ...
    function points() external view returns (IRCPoints);
    ...
    function setPointsAddress(address _newPoints) external;
