    ...
    __with_semver(uint8(1), uint8(0), uint8(2))
{
    ...
    function isInWhitelistTokenAddress(address tokenAddress_) 
    ...
    function getTradePrice(address tokenSalePrice, address tokenBuyPrice) public view returns (uint256 price) {
        ...
        if (tokenSalePrice == tokenBuyPrice) {
            require(isInWhitelistTokenAddress(tokenSalePrice) && isInWhitelistTokenAddress(tokenSalePrice),
    ...
    function _fee(uint i) private pure returns (uint24) {
          uint24[3] memory _fees = [FEE_100, FEE_500, FEE_3000];
          return _fees[i];
    }

     function increasePoolHistory(address tokenSalePrice, address tokenBuyPrice, uint16 size) external {
        address poolAddress = _getTargetPool(tokenSalePrice, tokenBuyPrice);
