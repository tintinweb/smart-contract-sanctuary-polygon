...
// File: contracts/BangMarketplacev2.sol
...
contract NftTraderV2 {
    ...
    function purchase(address contractAddr, uint256 tokenId, uint256 amount) public payable {
        ...
        require(msg.value >= item.price * amount, "insufficent funds sent");
        balances[item.seller] <= msg.value;

        ERC1155 token = ERC1155(contractAddr);
        token.safeTransferFrom(item.seller, msg.sender, 5, amount, ""); //hard coded token ID
