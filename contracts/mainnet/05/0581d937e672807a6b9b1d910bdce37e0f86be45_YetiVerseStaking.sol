...
pragma solidity ^0.8.0;
contract YetiVerseStaking is Ownable, Pausable, IERC721Receiver {
    ...
    function harvest(uint256 tokenId) public {
        ...
        if(rewardAmountInIERC20Tokens > 0) {
            info.lastHarvestBlock = block.number;
            safeTransferToken(_msgSender(), rewardAmountInIERC20Tokens);
            emit Rewarded(_msgSender(), rewardAmountInIERC20Tokens);
        }
    }
    ...
    function setTokenPerReward(uint256 TokenAmount) external onlyOwner {
        TokenPerReward = TokenAmount;
    }
