...
contract ANT {
    ...
    function feedLarva(uint256 _larvaAmount) public {
        ...
        require(larvaeList.length > 0, "You don't have any larva");
        ...
        for (uint i = 0; i<_larvaAmount; i++){
            ...
            larva.feedingLarva(msg.sender, 1);
    ...
    function maxHatch () public view returns(uint256 _maxPossible) {
        ...
        if (_userBuildings.length > 0 && buildingblock.idToActive(_userBuildings[0])){
...
interface ILarvaANT is IERC721 {
    ...
    //functions
    function feedingLarva(address _user, uint256 _larvaAmount) external;
...
interface IBuildingBlock {
    ...
    function getMergedBuildingBlockCount(address _user) external view returns(uint256 _count);
