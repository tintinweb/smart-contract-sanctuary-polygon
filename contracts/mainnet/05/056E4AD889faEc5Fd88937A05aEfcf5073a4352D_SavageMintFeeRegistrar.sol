...
contract SavageMintFeeRegistrar is Operator {
  ...
  mapping(address => EnumerableSet.UintSet) private _paidIds;
  uint256 private _fee;
  ISavageStock private _stock;
  ...
  function paymentTokens() external view returns (address[] memory) {
    return _stock.paymentTokens();
  }

  function paymentTokensLength() external view returns (uint256) {
    return _stock.paymentTokensLength();
  }

  function stock() external view returns (ISavageStock) {
    return _stock;
  }

  function paymentToken(uint256 tokenId) external view returns (address) {
    return _stock.paymentToken(tokenId);
  }

  function isIdPaid(address account, uint256 paymentId) external view returns (bool) {
    return _paidIds[account].contains(paymentId);
  }

  function intermediateStablecoin() public view returns (address) {
    return _stock.intermediateStablecoin();
  }

  function router() public view returns (IUniswapV2Router02) {
    return _stock.router();
  }

  function savageToken() public view returns (address) {
    return _stock.savageToken();
  }

  function stripeToken() public view returns (address) {
    return _stock.stripeToken();
  }

  function wrappedToken() public view returns (address) {
    return _stock.wrappedToken();
  }

  function isPaymentTokenExist(address token) public view returns (bool) {
    return _stock.isPaymentTokenExist(token);
  }

  function getFeeTokenAmount(address tokenIn) public view returns (uint256) {
    require(tokenIn != stripeToken(), "Stripe token is unsupported");
    require(isPaymentTokenExist(tokenIn), "TokenIn is unsupported");
    address intermediateStablecoin_ = intermediateStablecoin();
    if (tokenIn == intermediateStablecoin_) return _fee;
    return router().getAmountsIn(_fee, _getInPath(tokenIn, intermediateStablecoin_))[0];
  }

  event FeeUpdated(uint256 fee_);
  event MintFeePaid(
    uint256 paymentId,
  ...
  event StockUpdated(address stock_);

  constructor(address stock_, uint256 fee_) {
    _updateStock(stock_);
    _updateFee(fee_);
  }

  function payFee(uint256 paymentId) external payable returns (bool) {
    uint256 value = msg.value;
    address wrappedToken_ = wrappedToken();
    require(isPaymentTokenExist(wrappedToken_), "WrappedToken is unsupported");
    IWETH(wrappedToken_).deposit{value: value}();
    return _payFee(wrappedToken_, value, paymentId, true);
  }

  function payFee(address tokenIn, uint256 amountIn, uint256 paymentId) external returns (bool) {
    require(isPaymentTokenExist(tokenIn), "TokenIn is unsupported");
    return _payFee(tokenIn, amountIn, paymentId, false);
  }
  ...
  function updateStock(address stock_) external onlyOwner returns (bool) {
    _updateStock(stock_);
    return true;
  }

  function withdraw(address tokenIn) external onlyOwner returns (uint256 balance) {
    address this_ = address(this);
    address caller = msg.sender;
    if (tokenIn != address(0)) {
      IERC20 tokenIn_ = IERC20(tokenIn);
      balance = tokenIn_.balanceOf(this_);
      require(balance > 0, "No tokens to withdraw");
      tokenIn_.safeTransfer(caller, balance);
    } else {
      balance = this_.balance;
      require(balance > 0, "No token to withdraw");
      payable(caller).transfer(balance);
    }
  }

  function _payFee(address tokenIn, uint256 amountIn, uint256 paymentId, bool isNative) private returns (bool success) {
    address caller = msg.sender;
    address this_ = address(this);
    uint256 requiredStableAmount = _fee;
    address intermediateStablecoin_ = intermediateStablecoin();
    address savageToken_ = savageToken();
    require(tokenIn != stripeToken(), "Stripe token is unsupported");
    require(!_paidIds[caller].contains(paymentId), "PaymentId already paid");
    if (!isNative) IERC20(tokenIn).safeTransferFrom(caller, this_, amountIn);
    uint256 stableAmount = amountIn;
    if (tokenIn != intermediateStablecoin_) {
      uint256[] memory amounts = router().getAmountsOut(amountIn, _getInPath(tokenIn, intermediateStablecoin_));
      stableAmount = amounts[amounts.length - 1];
    }
    success = stableAmount >= requiredStableAmount;
    require(success, "TokenIn amount not cover price");
    uint256 amountOut = amountIn;
    if (tokenIn != savageToken_) amountOut = _swap(_getInPath(tokenIn, savageToken_), amountOut, this_);
    _paidIds[caller].add(paymentId);
    emit MintFeePaid(paymentId, caller, tokenIn, isNative, amountIn, stableAmount);
  }

  function _getInPath(address tokenFrom, address tokenTo) private view returns (address[] memory path) {
    address wrappedToken_ = wrappedToken();
    uint256 length = tokenFrom == wrappedToken_ ? 2 : 3;
    path = new address[](length);
    path[0] = tokenFrom;
    path[length - 1] = tokenTo;
    if (length > 2) path[1] = wrappedToken_;
  }

  function _swap(address[] memory path, uint256 amount, address to) private returns (uint256) {
    IUniswapV2Router02 router_ = router();
    IERC20(path[0]).safeApprove(address(router_), amount);
    return router_.swapExactTokensForTokens(
      amount,
      1,
      path,
      to,
  ...
  function _updateFee(uint256 fee_) private {
    require(fee_ > 0, "Fee is zero");
    _fee = fee_;
    emit FeeUpdated(fee_);
  }

  function _updateStock(address stock_) private {
    require(stock_ != address(0), "Stock is zero address");
    _stock = ISavageStock(stock_);
    emit StockUpdated(stock_);
  }
}
