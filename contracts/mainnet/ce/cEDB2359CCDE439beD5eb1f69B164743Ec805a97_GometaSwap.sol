...
import "./IGometaSwap.sol";

/**
 * @title GometaSwap
 * @author Kamanchi
 * @notice This SC using for exchange ERC20 stablecoins
 * on 'GOMETA WAYS' token
 */
contract GometaSwap is IGometaSwap, ReentrancyGuard, Ownable {
    using Strings for uint256;

    uint256 public gometaPrice;
    ...
    constructor(
        uint256 gometaPrice_,
        address owner_,
        address stablecoinAddress_
    ) {
        gometaPrice = gometaPrice_;
        tokenStorage = owner_;
     ...
     * @param buyToken_ - address of 'GOMETA WAYS'
    ...
    function swapTokens(
        ...
        require(
            sellToken_ != address(0) && buyToken_ != address(0),
            "GometaSwap: contract address can't be 0"
        );
        require(
            amountTokensToSwap_ != 0,
            "GometaSwap: you can't exchange 0 tokens"
        );
        require(
            checkSellTokenAddress(sellToken_),
            "GometaSwap: 'sellToken_' , not matched !"
        );
        IERC20 sellToken = IERC20(sellToken_);
        IERC20 buyToken = IERC20(buyToken_);
        uint256 amountTokensToBuy;
        if (stableCoinAddresses[0] == sellToken_) {
            require(
                bytes(amountTokensToSwap_.toString()).length >= 6,
                "GometaSwap: min value of 'USDT' is 10 cents"
            );
            amountTokensToBuy = (amountTokensToSwap_ * 1e30) / gometaPrice;
        } else {
            amountTokensToBuy = (amountTokensToSwap_ * 1e18) / gometaPrice;
        }
        ...
        require(
            ...
            "GometaSwap : approve more tokens to success swap"
        );
        require(
            buyToken.allowance(tokenStorage, address(this)) >=
                amountTokensToBuy,
            "GometaSwap : not enougth tokens for transfer"
        );
        sellToken.transferFrom(msg.sender, tokenStorage, amountTokensToSwap_);
        buyToken.transferFrom(tokenStorage, msg.sender, amountTokensToBuy);
        return true;
    }

    /**
     * @dev function allows owner change 'GOMETA WAYS' price
     */
    function setGometaTokenPrice(uint256 newPrice_)
        external
        onlyOwner
        returns (uint256)
    {
        gometaPrice = newPrice_;
        return gometaPrice;
    }

    /**
     * @dev functions allows change 'GOMETA WAYS' storage
     *
     * NOTE: by default storage == 'owner' address
     */
    function setNewStorage(address newStorage_)
        external
        onlyOwner
        returns (address)
    {
        tokenStorage = newStorage_;
        return tokenStorage;
    }
        ...
        onlyOwner
    {
        require(
            ...
            "GometaSwap : 'sellToken_' , can't be 0"
        );
...
interface IGometaSwap {
     ...
     * @dev function allows owner change 'GOMETA WAYS' price
     */
    function setGometaTokenPrice(uint256 newPrice_) external returns (uint256);

    /**
     * @dev functions allows owner address
     */
    function setNewStorage(address newStorage_) external returns (address);
