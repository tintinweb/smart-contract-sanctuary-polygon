...
/// @title Council NFT
/// @notice An ERC721 contract with restrictions on who can move tokens. Owners are not allowed to transfer tokens they own.
/// @notice There are spenders approved to move all token ID's, and spenders approved to move specific token ID's
contract CouncilNFT is ERC721Enumerable, Ownable {
    /// @notice Maps a token ID and address to a boolean indicating an account's ability to transfer the token ID
    mapping(uint256 => mapping(address => bool)) public isApprovedSpenderForTokenId;

    /// @notice Maps an account to a boolean indiciating an account's ability to transfer ANY token ID
    mapping(address => bool) public isApprovedSpenderForAllTokens;

    constructor(string memory name_, string memory symbol_) ERC721(name_, symbol_) {}

    /// @notice Allows spender to transfer ANY token ID
    /// @notice Callable only by contract owner
    /// @param spender The spender who can transfer any token ID
    function addApprovedSpenderForAllTokens(address spender) external onlyOwner {
        isApprovedSpenderForAllTokens[spender] = true;
    }

    /// @notice Revokes spender's ability to transfer ANY token ID
    /// @notice Callable only by contract owner
    /// @param spender Account to revoke transfer rights for all token IDs
    function removeApprovedSpenderForAllTokens(address spender) external onlyOwner {
        isApprovedSpenderForAllTokens[spender] = false;
    }

    /// @notice Grants transfer rights for specific token ID to given account
    /// @notice Callable only by contract owner
    /// @param spender Account to grant transfer rights for a specific token ID
    /// @param tokenId The specific token ID to grant transfer rights for
    ...
    /// @notice Revokes transfer rights for specific token ID from given account
    /// @notice Callable only by contract owner
    /// @param spender Account to revoke transfer rights for a specific token ID
    /// @param tokenId The specific token ID to revoke transfer rights for
    ...
    /// @notice Determines if the spender is approved for the specific token ID or is approved for ANY token ID
    /// @dev Does NOT throw an error if the token ID is not valid.
    /// @param spender Address of the spender to check approval for
    /// @param tokenId The specific token ID to check approval for
    /// @return Returns true if the spender is approved for the specific token ID or is approved for ANY token ID, false otherwise.
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view override returns (bool) {
        return isApprovedSpenderForAllTokens[spender] || isApprovedSpenderForTokenId[tokenId][spender];
    }

    /// @notice Mints a new token ID to the specified account
    /// @dev Throws an error if the caller is not approved to mint the token ID.
    /// @param to Address of the account to mint the new token ID to
    ...
    /// @notice Burns a specific token ID
    /// @dev Throws an error if the caller is not approved to burn the token ID.
...
/// @title Streamable Council NFT
/// @notice Contract inheriting CouncilNFT that can split incoming superfluid streams among token holders
/// @notice The flow rate to each token holder is always inFlowRate / totalSupply
/// @notice Every time there is a mint/burn/transfer of a token, outbound streams are updated
/// @notice Every time a superfluid stream to this app is created/updated/deleted, outbound streams are updated
contract StreamableCouncilNFT is SuperAppBase, CouncilNFT {
    using SuperTokenV1Library for ISuperToken;
    using SafeERC20 for IERC20;

    /// @notice The maximum supply of StreamableCouncilNFT's is 8
    uint256 constant public maxSupply = 8;

    /// @dev Superfluid Constant Flow Agreement ID
    bytes32 public constant CFA_ID =
        keccak256("org.superfluid-finance.agreements.ConstantFlowAgreement.v1");

    /// @dev Superfluid host contract
    ISuperfluid private _host;
    /// @dev Superfluid CFA
    ISuperToken private _acceptedToken; // accepted token

    /// @dev The account that is sending a stream to this app
    /// @dev There can only be one inbound stream to this app at one time
    address public streamOrigin;

    /// @dev Whitelist of allowed stream senders
    /// @dev Only accounts in this whitelist are allowed to send streams to this app
    mapping(address => bool) public senderWhitelist;

    /// @dev Emitted when an account is whitelisted to send streams to this app
    event Whitelisted(address account);
    /// @dev Emitted when an account is de-whitelisted
    event DeWhitelisted(address account);

    /// @notice Constructor for StreamableCouncilNFT
    /// @param _name Name of the NFT
    /// @param _symbol Symbol of the NFT
    /// @param host Superfluid host contract
    /// @param acceptedToken Accepted supertoken address
    /// @param registrationKey SuperApp Registration Key
    constructor(
        string memory _name,
        ...
        // APP_LEVEL_FINAL means we cannot send streams to other SuperApps
        uint256 configWord = SuperAppDefinitions.APP_LEVEL_FINAL |
            SuperAppDefinitions.BEFORE_AGREEMENT_UPDATED_NOOP |
            SuperAppDefinitions.BEFORE_AGREEMENT_TERMINATED_NOOP;
        
        // register app with superfluid
        _host.registerAppWithKey(configWord, registrationKey);
    }

    /// @notice Admin function to recover any stuck ERC20 tokens
    /// @notice Callable only by contract owner
    ...
    /// @notice Admin function to whitelist a superfluid stream sender
    /// @param sender The account to whitelist
    function whitelistSender(address sender) external onlyOwner {
        senderWhitelist[sender] = true;
        emit Whitelisted(sender);
    }

    /// @notice Admin function to remove an account from the superfluid stream sender whitelist
    /// @param sender The account to remove from the whitelist
    function deWhitelistSender(address sender) external onlyOwner {
        senderWhitelist[sender] = false;
        emit DeWhitelisted(sender);
    }
    ...
    /// @dev Create/update/delete flows to all token holders with a rate of "incoming flow rate" / totalSupply
    /// @dev Called after mint, burn, agreementCreated, agreementUpdated, and agreementTerminated
    /// @dev When called after burn, the flow to the holder of the burned token has already been deleted by _changeReceiver
    /// @param ctx Context struct (can be empty bytes if useContext is false)
    /// @param useContext Boolean indicating whether to use and update the context struct. (will be true if called in SuperApp callback, otherwise false)
    /// @return newCtx New context struct. If useContext is false, then it will be equal to the ctx argument
    function _updateOutflow(bytes memory ctx, bool useContext) private returns (bytes memory newCtx) {
        newCtx = ctx;

        // if the streamOrigin is address(0), we have no incoming streams, so we do nothing
        // if the totalSupply is 0, then we have no nft holder to send streams to, so we do nothing
        if (streamOrigin == address(0) || totalSupply() == 0) {
            return newCtx;
        }

        // get the flowrate into the app
        int96 inFlowRate = _acceptedToken.getFlowRate(streamOrigin, address(this));

        // calculate the flow rate we should send to each nft holder
        int96 newOutflowRate = inFlowRate / int96(int256(totalSupply()));

        // if the flow rate to each nft holder is 0, then we have to delete the outgoing streams to all holders if they exist
        if (newOutflowRate == 0) {
            for (uint256 i = 0; i < totalSupply(); i++) {
                address tokenHolder = ownerOf(tokenByIndex(i));

                // get current outflow rate to tokenHolder
                int96 ofr = _acceptedToken.getFlowRate(address(this), tokenHolder);

                // if if outflow exists, we have to delete it
                if (ofr != int96(0)) {
                    if (useContext) {
                        // if we are using context we have to update it
                        newCtx = _acceptedToken.deleteFlowWithCtx(address(this), tokenHolder, newCtx);
                    }
                    else {
                        // if we aren't using context we do not pass it along/update it
                        _acceptedToken.deleteFlow(address(this), tokenHolder);
                    }
                }
            }
        }
        else {
            // if the calculated flow rate to token holders is nonzero
            for (uint256 i = 0; i < totalSupply(); i++) {
                address tokenHolder = ownerOf(tokenByIndex(i));
                ...
                // if current outflow rate to tokenHolder is 0, then we create a flow to them
                if (ofr == int96(0)) {
                    if (useContext) {
                        // if we are using context we have to update it
                        newCtx = _acceptedToken.createFlowWithCtx(tokenHolder, newOutflowRate, newCtx);
                    }
                    else {
                        // if we aren't using context we do not pass it along/update it
                        _acceptedToken.createFlow(tokenHolder, newOutflowRate);
                    }
                }
                // else if current outflow rate to tokenHolder is nonzero and is not equal to what it should be, we update the flow to them
                else if (ofr != newOutflowRate) {
                    if (useContext) {
                        // if we are using context we have to update it
                        newCtx = _acceptedToken.updateFlowWithCtx(tokenHolder, newOutflowRate, newCtx);
                    }
                    else {
                        // if we aren't using context we do not pass it along/update it
                        _acceptedToken.updateFlow(tokenHolder, newOutflowRate);
                    }
                }
            }
        }
    ...
    /// @dev Called in _beforeTokenTransfer hook. (called on mint, burn, normal transfer)
    /// @dev If the transfer is a mint or burn, _updateOutflow is called after this function.
    /// @dev If the transfer is a mint, do nothing. _updateOutflow will do the work of creating/updating streams to all token holders.
    /// @dev If the transfer is a burn and oldReceiver has some outgoing flow to it, delete the stream to oldReceiver. _updateOutflow will update the streams of the remaining token holders.
    /// @dev If the transfer is a normal transfer between accounts and oldReceiver has some outgoing flow to it, delete the flow to oldReceiver and create one of equal flow rate to newReceiver.
        ...
        internal
    {
        ...
        // if newReceiver is the same as oldReceiver, we don't need to do anything
        if (newReceiver == oldReceiver) return;

        // get outflow rate to oldReceiver
        int96 oldOutFlowRate = _acceptedToken.getFlowRate(address(this), oldReceiver);

        // if the outflow rate to the oldReceiver is nonzero, we have to delete the flow to oldReceiver
        if (oldOutFlowRate > 0) {
            _acceptedToken.deleteFlow(address(this), oldReceiver);

            // if this is a burn we only delete the old stream
            // if it is not a burn or mint, then we create/update a flow of equal rate to newReceiver
            if (newReceiver != address(0)) {
                // get current outflow rate to newReceiver
                int96 newOutFlowRate = _acceptedToken.getFlowRate(address(this), newReceiver);

                // if current outflow rate to newReceiver is 0, then we create a flow to them
                if (newOutFlowRate == 0) {
                    _acceptedToken.createFlow(newReceiver, oldOutFlowRate);
                }
                // if current outflow rate to newReceiver is nonzero, then we update the flow to them
                else if (newOutFlowRate != oldOutFlowRate) {
                    _acceptedToken.updateFlow(newReceiver, oldOutFlowRate);
                }
            }
        }
    }
    ...
    /// @dev Called before token transfer (this includes mint/burn)
    /// @param from Account the token is being transferred from (0 if mint)
    /// @param to Account the token is being transferred to (0 if burn)
    function _beforeTokenTransfer(
        address from, /*from*/
        address to,
        uint256 tokenId,
        ...
        // because our app is registered as final, we can't take downstream apps
        require(!_host.isApp(ISuperApp(to)), "New receiver can not be a superApp");
    ...
    /// @dev Mint a token
    /// @dev Reverts if tokenId is greater than or equal to maxSupply
    /// @param to Account to mint the token to
    /// @param tokenId Token ID to mint
    function _mint(address to, uint256 tokenId) internal override {
        require(tokenId < maxSupply, "StreamableCouncilNFT: Cannot exceed maximum supply");
        super._mint(to, tokenId);
        // we don't use context because we were not passed a context
    ...
    // This is a test event. Before deployment, remove this.
    event Gas(uint256 x);

    /// @dev Called before an agreement is created
    /// @dev Reverts if there already exists an inbound stream to this app
    function beforeAgreementCreated(
        ISuperToken,
        address,
        bytes32,
    ...
    /// @dev Called after an agreement is created
    /// @dev Reverts if agreement is of incorrect type or is not the correct token
    /// @dev Sets streamOrigin to the sender of the flow
    /// @dev Calls _updateOutflow with context
    function afterAgreementCreated(
        ...
        returns (bytes memory newCtx)
    {
        // remove gasleft() stuff before deployment
        uint256 g = gasleft();

        streamOrigin = _host.decodeCtx(_ctx).msgSender;
        require(senderWhitelist[streamOrigin], "StreamableCouncilNFT: Stream sender is not whitelisted");
    ...
    /// @dev Called after an agreement is updated
    /// @dev This function CANNOT REVERT! https://docs.superfluid.finance/superfluid/developers/super-apps/super-app#heres-an-overview-of-each-rule
    /// @dev Calls _updateOutflow with context
    function afterAgreementTerminated(
        ISuperToken _superToken,
        address _agreementClass,
    ...
    /// @dev Helper function to check if a SuperToken is the correct one
    /// @param superToken Address of super token contract
    /// @return superToken == _acceptedToken 
    function _isSameToken(ISuperToken superToken) private view returns (bool) {
        return address(superToken) == address(_acceptedToken);
    }

    /// @dev Helper function that checks if agreementClass is ConstantFlowAgreement.v1
    /// @return true if agreementClass is ConstantFlowAgreement.v1
    ...
    /// @dev Modifier that reverts if msg.sender is not the Superfluid host contract
    modifier onlyHost() {
        require(
    ...
    /// @dev Modifier that reverts if superToken or agreementClass are not what is expected
...
contract StreamableCouncilNFTFactory {
    address constant deployer = 0xBFb1c7BE402eC0D289fA25db4C544718D5CF1Df2;
    address constant owner = 0x583D596b0a79C0e83C87851eA9FB1A91e80290B2;

    ISuperfluid constant sfHost = ISuperfluid(0x3E14dC1b13c488a8d5D310918780c983bD5982E7);
    ISuperToken constant telx = ISuperToken(0x84fd3836C7E8f03b3c946DE6e9FF9BdbE80b5916);
    string constant registrationKey = "k1";

    event Deployed(address indexed contractAddress, string name, string symbol);

    function create(string calldata name_, string calldata symbol_) external {
        require(msg.sender == deployer);
        StreamableCouncilNFT nft = new StreamableCouncilNFT(name_, symbol_, sfHost, telx, registrationKey);
        nft.transferOwnership(owner);
        emit Deployed(address(nft), name_, symbol_);
    }
}