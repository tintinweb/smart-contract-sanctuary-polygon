...
contract ChickenFarm {
    ...
    event NewIncomeFromReferrals(address indexed referral, address indexed referrer, uint256 amountOfChickens);
    event OneEggPriceHasChanged(uint256 indexed oldPrice, uint256 newPrice, uint256 eggsOnMarket);
    event DevFeePayment(uint256 indexed amount);
    ...
    function buyChickens(address _ref) external payable {
        ...
        emit NewIncomeFromReferrals(msg.sender, checkedRef, (msg.value * ReferralsFeePercent) / 100);
    ...
    function sellEggs() external {
        ...
        emit DevFeePayment(developersFee);
