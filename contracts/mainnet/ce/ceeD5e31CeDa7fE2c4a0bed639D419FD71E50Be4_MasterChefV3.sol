...
interface ITakoToken is IERC20 {
...
// contract MasterChefV3 is ReentrancyGuard, Ownable, SelfPermit, Multicall {
contract MasterChefV3 is ReentrancyGuard, Ownable {
    ...
    /// Also known as the amount of reward to distribute per second.
    struct PoolInfo {
    ...
    // mapping(uint256 => uint256) public lpSupplies;
    ...
    /// @notice Reward tokens created per second.
    ...
    uint256 public treasuryPercent = 0;
    uint256 public investorPercent = 0;
    ...
    constructor(
        ...
        rewardToken = ITakoToken(_rewardToken);
        devAddr = _devAddr;
        ...
        returns (uint256 pending)
    {
        ...
        // uint256 lpSupply = lpSupplies[_pid];
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            uint256 lpPercent = 1000 -
                ...
                treasuryPercent -
                investorPercent;
            uint256 rewardRewardsForPool = totalAllocPoint == 0
                ? 0
                : (((multiplier * rewardPerSec * pool.allocPoint) /
                    totalAllocPoint) * lpPercent) / 1000;

            accRewardPerShare =
                accRewardPerShare +
                ((rewardRewardsForPool * ACC_REWARDS_PRECISION) / lpSupply);
        }
        pending =
            ((user.amount * accRewardPerShare) / ACC_REWARDS_PRECISION) -
            user.rewardDebt;
    }
        ...
        returns (PoolInfo memory pool)
    {
        ...
        // uint256 lpSupply = lpSupplies[pid];

        uint256 multiplier = block.timestamp - pool.lastRewardTimestamp;
        uint256 reward = totalAllocPoint == 0
            ? 0
            : ((multiplier * rewardPerSec * pool.allocPoint) / totalAllocPoint);
        ...
        rewardToken.mint(devAddr, (reward * devPercent) / 1000);
        rewardToken.mint(treasuryAddr, (reward * treasuryPercent) / 1000);
        rewardToken.mint(investorAddr, (reward * investorPercent) / 1000);
        rewardToken.mint(address(this), (reward * lpPercent) / 1000);

        pool.accRewardPerShare = (pool.accRewardPerShare +
            (((reward * ACC_REWARDS_PRECISION) / lpSupply) * lpPercent) /
            1000).toUint128();
    ...
    function deposit(
        ...
        uint256 amount,
        address to
    ) external nonReentrant validatePid(pid) {
    ...
    /// @notice Deposit LP tokens to MC for reward allocation.
    /// @param pid The index of the pool. See `poolInfo`.
    ...
    function _deposit(
        ...
        // Effects
        uint256 _pendingRewards = ((user.amount * pool.accRewardPerShare) /
            ACC_REWARDS_PRECISION) - user.rewardDebt;
        ...
        if (_pendingRewards != 0) {
            safeRewardTransfer(to, _pendingRewards);
        }

        IRewarder _rewarder = rewarder[pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onReward(pid, to, to, _pendingRewards, user.amount);
        }
    ...
    function harvestMultiple(uint256[] calldata pids) external nonReentrant {
    ...
    function _emergencyWithdraw(uint256 pid, address to)
        internal
        validatePid(pid)
    {
        UserInfo storage user = userInfo[pid][msg.sender];
    ...
    function add(
        ...
        // uint64 lastRewardTimestamp = uint64(block.timestamp);
        uint64 lastRewardTimestamp = block.timestamp > startTimestamp
        ...
        // poolinfo.lastRewardTimestamp = uint64(lastRewardTimestamp);
    ...
    // /// @notice Batch update the given pool's reward allocation point and `IRewarder` contract. Can only be called by the owner.
    // /// @param _pid The index of the pool. See `poolInfo`.
    // /// @param _allocPoint New AP of the pool.
    ...
    //     uint64[] calldata _allocPoint,
    //     IRewarder[] calldata _rewarders,
    //     bool[] calldata overwrite,
    //     bool update
    // ) external onlyOwner {
    //     require(
    ...
    //         "MC: all arrays need to be the same length"
    //     );
    ...
    function updateEmissionRate(uint256 _rewardPerSec) public onlyOwner {
        rewardPerSec = _rewardPerSec;
        emit UpdateEmissionRate(msg.sender, _rewardPerSec);
    }
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > rewardsBalance) {
            rewardToken.transfer(_to, rewardsBalance);
        } else {
            rewardToken.transfer(_to, _amount);
        }
    }

    // TODO: check whether it's better to use onlyOwner?
    ...
    function setTreasuryAddr(address _treasuryAddr) public {
        ...
        treasuryAddr = _treasuryAddr;
        emit SetTreasuryAddress(msg.sender, treasuryAddr);
    }
