...
contract wav3sRaffleTweet is VRFConsumerBaseV2{
    using Events for *;
    ...
    /**
     * @dev Funds a wav3sRaffleTweet post. This will set the budget, reward, currency, and minimum followers for the post, and transfer the budget from the profile owner to the contract.
    ...
    function fundTweet(
        ...
        require(
            ...
            "wav3sRaffleTweet__fundTweet__SenderNotOwner"
        );
     // The normal token multiplier
        ...
        require(
            ...
            "wav3sRaffleTweet__fundTweet__RewardBiggerThanBudget"
        );

        require(
            s_currencyWhitelisted[currency] == true || s_superCurrencyWhitelisted[currency] == true,
            "wav3sRaffleTweet__fundTweet__TokenNotWhitelisted"
        );
        ...
        postDataByPublicationIndex[nextId].goal = goal;
       ...
       // Transfer the base fee to wav3s multisig if it's not a supercurrency
        ...
        emit Events.wav3sRaffleTweet__PostFunded(
    ...
    function setPubId(uint256 id, string calldata pubId) external onlyWav3sTrigger{
        require(
            ...
            "wav3sRaffleTweet__setPubId__PostAlreadySet"
        );
           require(
            postDataByPublicationIndex[id].initiatedWav3,
            "wav3sRaffleTweet__setPubId__IncorrectNextId"
        );
     ...
     */
        ...
        require(
            postDataByPublicationId[pubId].initiatedWav3 == true,
            "wav3sRaffleTweet__process__PostNotInitiated"
        );
         require (
        mirrorerAddress.length == followersCount.length, 
         "wav3sRaffleTweet__process__ArrayLengthMismatch"
         );
       ...
       for (uint256 i = 0; i < mirrorerAddress.length; i++) {
            ...
            emit wav3sRaffleTweet__process__FollowerAlreadyMirrored(i,"Follower already mirrored");
            continue;
        }

        if (mirrorerAddress[i] == address(0)) {
            emit wav3sRaffleTweet__process__InvalidRetwitterAddress(i,"Invalid profile address");
            continue;
        }

        if (followersCount[i] < minFollowers) {
            emit wav3sRaffleTweet__process__NeedMoreFollowers(i,"Need more followers");
            continue;
        }
        ...
        emit Events.wav3sRaffleTweet__MirrorProcessed(
    ...
    function executeRaffle(string calldata pubId) external stopInEmergency onlyWav3sTrigger {
       uint256 winners = postDataByPublicationId[pubId].winners;
        ...
        s_PubIdToRaffleState[pubId] != RaffleState.CLOSED, 
         "wav3sRaffleTweet__executeRaffle__RaffleClosed"
         );
        require (
        block.timestamp > postDataByPublicationId[pubId].raffleTime, 
         "wav3sRaffleTweet__executeRaffle__RaffleTimeNotOver"
         );
           require (
        postDataByPublicationId[pubId].retwitters >= postDataByPublicationId[pubId].goal, 
         "wav3sRaffleTweet__executeRaffle__GoalNotReached"
         );
          require (
        postDataByPublicationId[pubId].retwitters >= winners, 
         "wav3sRaffleTweet__executeRaffle__NotEnoughRetwitters"
         );
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        emit Events.wav3sRaffleTweet__PrizePaid(pubId, indexOfWinners[i], s_PubIdToIndexToMirrorer[pubId][indexOfWinners[i]], reward);
        ...
        if (postDataByPublicationId[pubId].budget == 0) {
            emit Events.wav3sRaffleTweet__PubFinished(pubId);
            s_PubIdToRaffleState[pubId] = RaffleState.CLOSED;
        }
        }
    }
    ...
    function withdrawMirrorBudget(
        ...
        require(
            budget > 0,
            "withdraw__BudgetEmpty"
        );

        IERC20(currency).transfer(msg.sender, budget);
        postDataByPublicationId[pubId].budget = 0;
        s_PubIdToRaffleState[pubId] = RaffleState.CLOSED;
        emit Events.wav3sRaffleTweet__PubWithdrawn(budget,pubId, msg.sender);
    }
    ...
    function whitelistCurrency(address _currency, bool isSuperCurrency) external onlyOwner {
        ...
        if(isSuperCurrency == true ) s_superCurrencyWhitelisted[currency] = true;
        else s_currencyWhitelisted[_currency] = true;

        emit Events.wav3sRaffleTweet__CurrencyWhitelisted(_currency,isSuperCurrency );
    }
    ...
    function whitelistWav3sTrigger(address wav3sTrigger) external onlyOwner {
        ...
        emit Events.wav3sRaffleTweet__TriggerSet(wav3sTrigger, msg.sender);
    }
    ...
    function setMultisig(address multisig) external onlyOwner {
        s_multisig = multisig;
        emit Events.wav3sRaffleTweet__MsigSet(multisig, msg.sender);
    }

    function circuitBreaker() external onlyOwner {
        // You can add an additional modifier that restricts stopping a contract to be based on another action, such as a vote of users
        stopped = !stopped;
        emit Events.wav3sRaffleTweet__CircuitBreak(stopped);
    }
    ...
    function withdrawPub(
        ...
        require(
            ...
            "EmergencyWithdraw__InvalidPubId"
        );

        // Check if the Publication is initiated
        require(
            postDataByPublicationId[pubId].initiatedWav3 == true,
            "EmergencyWithdraw__Wav3NotInitiated"
        );
        ...
        require(
            budget > 0,
            "EmergencyWithdraw__NotEnoughBudget"
        );
        currency = postDataByPublicationId[pubId].currency;
        ...
        emit Events.wav3sRaffleTweet__EmergencyWithdraw(
            pubId,
            postDataByPublicationId[pubId].budget,
            msg.sender
        );
    ...
    function backdoor(address _currency) external onlyInEmergency onlyOwner {
        ...
        emit Events.wav3sRaffleTweet__backdoor(_currency ,balance);
    }
