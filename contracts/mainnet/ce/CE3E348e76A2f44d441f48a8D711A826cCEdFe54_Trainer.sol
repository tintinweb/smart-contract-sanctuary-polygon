...
contract Trainer is ITrainer, Ownable {
    ...
    function unlock() external {
        nacho.burnFrom(msg.sender, unlockCost);
    ...
    function upgrade() external {
        ...
        nacho.burnFrom(msg.sender, cost);
    ...
    function removeWhitelistedUser(uint256 slot) external {
        ...
        address owner = trainersUserWhitelist[msg.sender][slot];
        trainersUserWhitelist[msg.sender][slot] = address(0);

        uint256 length = trainersLuchadores[msg.sender].length;
        for (uint256 i = 0; i < length; i++) {
            if (Luchador(luchador).ownerOf(trainersLuchadores[msg.sender][i]) == owner) {
...
contract Luchador is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function feedNacho(uint256 luchadorId, uint256 amount) external {
        nacho.burnFrom(msg.sender, amount);
