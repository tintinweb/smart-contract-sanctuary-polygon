...
contract BSBBattleUpgradeable is Initializable, BSBBaseGameModeUpgradeable, VRFConsumerBaseUpgradeable {
    ...
    uint public yieldMultiplierPercentage;
    uint public metaPassYieldMultiplierPercentage;
    uint public baseRisk;
    uint public bloodShardDropChancePercentage;
    uint public metaPassRiskReductionPercentage;
    uint public metaPassBloodShardDropBoostPercentage;
    uint public minimumYieldRisk;

    IBSBBloodShard BSBBloodShardCaller;

    mapping(address => mapping(uint256 => uint256)) public tokenSquadMapping;
    mapping(address => mapping(uint256 => bool)) public tokenSquadAppartenance;
    mapping(uint256 => mapping(address => EnumerableSetUpgradeable.UintSet)) squadEnrollments;

    mapping(address => mapping(uint => uint)) public claimableLevelPercentageForBear;

    event SeedFulfilled();
    event BloodShardWon(address by, address collection, uint tokenId);
    event BloodShardStolen(address from, address to, address collection, uint eliteTokenId);
    ...
    function __BSBBattle_init(
        ...
        staminaBuff = 1;
        levelDaysRate = 2;
        yieldMultiplierPercentage = 30;
        metaPassYieldMultiplierPercentage = 10;
        baseRisk = 50;
        bloodShardDropChancePercentage = 1;
        metaPassRiskReductionPercentage = 5;
        metaPassBloodShardDropBoostPercentage = 2;
        minimumYieldRisk = 10;
    }

    function enroll(CollectionItems[] memory collectionItems_) public whenNotPaused {
        for (uint i = 0; i < collectionItems_.length; ++i) {
            _enrollCollectionAssets(collectionItems_[i]);
        }
    ...
    function stackInSquad(CollectionItems memory collectionItems_, uint leaderId_) external whenNotPaused {
        require(collectionItems_.collection == contracts["gen1"], "BAT:1");
        _checkIfOwnsItem(contracts["gen0"], leaderId_, _msgSender());

        BearStats memory bearStats = BSBStorageCaller.getBearStats(contracts["gen0"], leaderId_);
        require(
            collectionItems_.assets.length + getLeaderSquadSize(leaderId_) <= bearStats.leadership.value,
            "BAT:2"
        );
        delete bearStats;

        CollectionItems[] memory collectionItemsArray = new CollectionItems[](1);
        collectionItemsArray[0] = collectionItems_;
        enroll(collectionItemsArray);
        delete collectionItemsArray;

        for (uint i = 0; i < collectionItems_.assets.length; ++i) {
            _stackToken(collectionItems_.collection, collectionItems_.assets[i], leaderId_);
    ...
    function claimRewards(CollectionItems[] memory collectionItems_) public override whenNotPaused {
        ...
        for (uint i = 0; i < collectionItems_.length; ++i) {
            ...
            accumulatedYield += _claimRewards(collectionItems_[i], applyBuff_);
        }

        delete applyBuff_;

        BSBBloodTokenCaller.mint(_msgSender(), accumulatedYield * ETHER);
    }
    ...
    function calculateRemainingStamina(
        ...
        if (!_shouldBeRewarded(collection_, tokenId_)) {
            return bearStamina_.value - bearStamina_.min;
        }

        bool applyBuff_ = BSBMintPassBuffCaller.getBuffBalance(tokenOwners[collection_][tokenId_]) != uint(0);

        uint spentStamina_ = _calculateSpentStamina(
            _getMaxSecondsLifetimeUntilBlock(collection_, tokenId_, bearStamina_, applyBuff_), applyBuff_, bearStamina_
        );

        return bearStamina_.value - spentStamina_ - bearStamina_.min;
    }
    ...
    function getSquadEnrolments(uint tokenId_) external view returns(uint[] memory, uint[] memory) {
        address gen0Collection = contracts["gen0"];
        address gen1Collection = contracts["gen1"];

        uint[] memory gen0Enrolments = new uint[](
            EnumerableSetUpgradeable.length(squadEnrollments[tokenId_][gen0Collection])
        );
        uint[] memory gen1Enrolments = new uint[](
            EnumerableSetUpgradeable.length(squadEnrollments[tokenId_][gen1Collection]));

        for (uint i = 0; i < EnumerableSetUpgradeable.length(squadEnrollments[tokenId_][gen0Collection]); ++i){
            gen0Enrolments[i] = EnumerableSetUpgradeable.at(squadEnrollments[tokenId_][gen0Collection], i);
        }

        for (uint i = 0; i < EnumerableSetUpgradeable.length(squadEnrollments[tokenId_][gen1Collection]); ++i){
            gen1Enrolments[i] = EnumerableSetUpgradeable.at(squadEnrollments[tokenId_][gen1Collection], i);
        }

        return (gen0Enrolments, gen1Enrolments);
    }

    function calculateReward(address collection_, uint tokenId_) public view returns (uint, uint) {
        require(EnumerableSetUpgradeable.contains(allowedStakeCollections, collection_), "BAT:3");
        ...
        BearStats memory bearStats = BSBStorageCaller.getBearStats(collection_, tokenId_);

        uint secondsLifetime_ = _getMaxSecondsLifetimeUntilBlock(collection_, tokenId_, bearStats.stamina, applyBuff_);
        uint levelReward_ = _calculateLevelReward(secondsLifetime_, bearStats.level);
        uint fractionalClaimableLevelPercentage = _calculateClaimableFractionalLevelPercentage(
            secondsLifetime_, levelReward_
        );
        uint extraLevel_ = (claimableLevelPercentageForBear[collection_][tokenId_] + fractionalClaimableLevelPercentage >= 100) ? 1 : 0;

        return (
            levelReward_ + extraLevel_,
            _calculateYieldReward(secondsLifetime_, collectionsYield[collection_], bearStats.level.value, applyBuff_)
        );
    }

    function calculateRewards(address collection_, uint[] calldata tokenIds_) external view returns (uint[] memory, uint[] memory) {
        uint[] memory levelRewards = new uint[](tokenIds_.length);
        uint[] memory yieldRewards = new uint[](tokenIds_.length);
        for (uint64 i = 0; i < tokenIds_.length; ++i) {
            (levelRewards[i], yieldRewards[i]) = calculateReward(collection_, tokenIds_[i]);
        }
        return (levelRewards, yieldRewards);
    }
    ...
    function calculateDailyYield(address collection_, uint tokenId_) public view returns (uint) {
        bool applyBuff = BSBMintPassBuffCaller.getBuffBalance(tokenOwners[collection_][tokenId_]) != uint(0);
        BearStats memory bearStats = BSBStorageCaller.getBearStats(collection_, tokenId_);
        uint collectionYield = collectionsYield[collection_];
        return _calculateDailyYieldWithBonus(collectionYield, bearStats.level.value, applyBuff);
    }

    function calculateDailyYieldForMany(address collection_, uint[] calldata tokenIds_) external view returns (uint[] memory) {
        uint[] memory dailyYields = new uint[](tokenIds_.length);
        for (uint i = 0; i < tokenIds_.length; i++) {
            dailyYields[i] = calculateDailyYield(collection_, tokenIds_[i]);
        }
        return dailyYields;
    }
    ...
    function getLeaderSquadSize(uint tokenId_) public view returns (uint) {
        return EnumerableSetUpgradeable.length(squadEnrollments[tokenId_][contracts["gen0"]]) +
        EnumerableSetUpgradeable.length(squadEnrollments[tokenId_][contracts["gen1"]]);
    }

    function setLevelImpacts(uint[] memory milestones_, ImpactType[] calldata impacts_) external onlyOwner {

        require(milestones_.length == impacts_.length, "BAT:4");
    ...
    function setBloodShardAddressCaller(address address_) external onlyOwner {
        BSBBloodShardCaller = IBSBBloodShard(address_);
    }

    function setStaminaDecreaseRate(uint value_) external onlyOwner {
        staminaDecreaseRate = value_;
    }

    function setStaminaBuff(uint value_) external onlyOwner {
        staminaBuff = value_;
    }

    function setLevelDaysRate(uint value_) external onlyOwner {
        levelDaysRate = value_;
    }

    function setYieldMultiplierPercentage(uint value_) external onlyOwner {
        yieldMultiplierPercentage = value_;
    }

    function setMetaPassYieldMultiplierPercentage(uint value_) external onlyOwner {
        metaPassYieldMultiplierPercentage = value_;
    }

    function setBaseRisk(uint value_) external onlyOwner {
        baseRisk = value_;
    }

    function setBloodShardDropChancePercentage(uint value_) external onlyOwner {
        bloodShardDropChancePercentage = value_;
    }

    function setMetaPassRiskReductionPercentage(uint value_) external onlyOwner {
        metaPassRiskReductionPercentage = value_;
    }

    function setMetaPassBloodShardDropBoostPercentage(uint value_) external onlyOwner {
        metaPassBloodShardDropBoostPercentage = value_;
    }

    function setMinimumYieldRisk(uint value_) external onlyOwner {
        minimumYieldRisk = value_;
    }

    function initSeedGeneration() public onlyOwner returns (bytes32 requestId) {
        require(LINK.balanceOf(address(this)) >= vrfFee, "BAT:5");
    ...
    function _getMaxSecondsLifetimeUntilBlock(
        address collection_,
        uint tokenId_,
        Stat memory stamina_,
        bool applyBuff_
    ) internal view returns(uint) {
        uint remainingSecondsLifetime = (stamina_.value - stamina_.min) * DAYS / _calculateActualDecreaseRate(applyBuff_);

        uint secondsElapsedUntilBlock = block.timestamp - stateChangeDates[collection_][tokenId_];

        return secondsElapsedUntilBlock < remainingSecondsLifetime ? secondsElapsedUntilBlock : remainingSecondsLifetime;
    }

    function _calculateLevelReward(uint secondsLifetime_, Stat memory level_) internal view returns(uint) {
        uint accumulatedLevel_ = secondsLifetime_ / (levelDaysRate * DAYS);
    ...
    function _calculateClaimableFractionalLevelPercentage(uint secondsLifetime_, uint levelReward_) internal view returns(uint) {
        uint secondsInClaimablePeriod_ = levelDaysRate * DAYS;
        return (secondsLifetime_ - levelReward_ * secondsInClaimablePeriod_) * 100 / secondsInClaimablePeriod_;
    }

    function _calculateSpentStamina(uint secondsLifetime_, bool applyBuff_, Stat memory stamina_) internal view returns(uint) {
        uint spentStamina = (secondsLifetime_ / DAYS) * _calculateActualDecreaseRate(applyBuff_);
    ...
    function _calculateYieldReward(
        uint secondsLifetime_,
        uint yield_,
        uint level_,
        bool applyBuff_
    ) internal view returns (uint) {
        return (secondsLifetime_ / DAYS) * _calculateDailyYieldWithBonus(yield_, level_, applyBuff_);
    }
    ...
    function _calculateDailyYieldWithBonus(
        ...
        return yield_ + (
        yieldMultiplierPercentage +
        (applyBuff_ ? metaPassYieldMultiplierPercentage : uint(0)) +
        getLevelImpact(level_).yieldBoost
        ) * yield_ / 100;
    }

    function _shouldBeRewarded(address collection, uint tokenId) internal view returns(bool) {
        return collection != contracts["gen1"] || tokenSquadAppartenance[collection][tokenId];
    }
    ...
    function _applyRisk(
        ...
        delete levelImpactRiskReduction_;
        ...
        _rewardBloodShard(collection_, tokenId_, level_, amount_, faction_, risk_, applyBuff_);

        return wonAmount;
    }

    function _rewardBloodShard(
        address collection_,
        uint tokenId_,
        uint level_,
        uint amount_,
        ...
        if (!_pickPortionOutOfAmountWithRange(
            _getBloodShardWinChancePercentage(level_, applyBuff_), tokenId_, amount_, 10000
        )) {
            return;
        }

        address winner_ = _msgSender();
        emit BloodShardWon(winner_, collection_, tokenId_);

        if (_pickPortionOutOfAmount(risk_, tokenId_, level_)) {
            (address eliteWinner_, address winnerCollection_, uint winnerTokenId_) = _pickWinnerFromElites(faction_, tokenId_);

            if (eliteWinner_ != address(0)) {
                emit BloodShardStolen(winner_, eliteWinner_, winnerCollection_, winnerTokenId_);
                winner_ = eliteWinner_;
            }

            delete winnerCollection_;
            delete winnerTokenId_;
        }

        BSBBloodShardCaller.mint(winner_, 1, "");
        delete winner_;
    }

    function _getBloodShardWinChancePercentage(
        uint level_,
        bool applyBuff_
    ) internal view returns (uint) {
        return bloodShardDropChancePercentage +
        getLevelImpact(level_).bloodShardBoost +
        (applyBuff_ ? metaPassBloodShardDropBoostPercentage : 0);
    }
    ...
    function _pickWinnerFromElites(
        ...
        uint opposingFaction_ = 1 - faction_;

        uint gen0ElitesWeight_ = EnumerableSetUpgradeable.length(elites[opposingFaction_][gen0Address]) * 3;
        uint elitesPoolSize_ = gen0ElitesWeight_ + EnumerableSetUpgradeable.length(elites[opposingFaction_][gen1Address]);
        ...
        if (winnerWeightedIndex_ >= gen0ElitesWeight_) {
            winnerTokenId_ = EnumerableSetUpgradeable.at(
                elites[opposingFaction_][gen1Address], winnerWeightedIndex_ - gen0ElitesWeight_
            );
        ...
        winnerTokenId_ = EnumerableSetUpgradeable.at(
            elites[opposingFaction_][gen0Address], winnerWeightedIndex_ / 3
        );

        delete opposingFaction_;
    ...
    function _removeFromElitesAndUnstackIfNecessary(CollectionItems memory collectionItem_) internal {
        if (collectionItem_.collection == contracts["gen0"] || collectionItem_.collection == contracts["gen1"]) {
            ...
            for (uint i = 0; i < collectionItem_.assets.length; ++i) {
                if (collectionItem_.collection == gen0Address) {
                    require(getLeaderSquadSize(collectionItem_.assets[i]) == 0, "BAT:6");
                }
                if (tokenSquadAppartenance[collectionItem_.collection][collectionItem_.assets[i]]) {
                    _unstackToken(collectionItem_.collection, collectionItem_.assets[i]);
                }
                if (bearsStats[i].isElite) {
                    _removeEliteFromPool(collectionItem_.collection, collectionItem_.assets[i], bearsStats[i].faction);
                }
            }
    ...
    function _stackToken(address collection_, uint tokenId_, uint leaderId_) internal {
        require(!tokenSquadAppartenance[collection_][tokenId_], "BAT:7");
        tokenSquadAppartenance[collection_][tokenId_] = true;
        tokenSquadMapping[collection_][tokenId_] = leaderId_;
        EnumerableSetUpgradeable.add(squadEnrollments[leaderId_][collection_], tokenId_);
    }

    function _unstackToken(address collection_, uint tokenId_) internal {
        require(tokenSquadAppartenance[collection_][tokenId_], "BAT:8");
        EnumerableSetUpgradeable.remove(squadEnrollments[tokenSquadMapping[collection_][tokenId_]][collection_], tokenId_);
        delete tokenSquadMapping[collection_][tokenId_];
        delete tokenSquadAppartenance[collection_][tokenId_];
    }

    function _claimRewards(CollectionItems memory collectionItems_, bool applyBuff_) internal returns (uint) {
        uint collectionAccumulatedRewards_;

        BearStats[] memory bearsStats_ = BSBStorageCaller.getBearsStats(
            collectionItems_.collection, collectionItems_.assets
        );

        uint collectionYield_ = collectionsYield[collectionItems_.collection];

        for (uint i = 0; i < collectionItems_.assets.length; ++i) {
            if (!_shouldBeRewarded(collectionItems_.collection, collectionItems_.assets[i])) {
                continue;
            }

            uint secondsLifetime_ = _getMaxSecondsLifetimeUntilBlock(
            ...
            uint yieldReward_ = _calculateYieldReward(
                secondsLifetime_,
                collectionYield_,
                bearsStats_[i].level.value,
                applyBuff_
            );

            uint yieldRewardAfterRisk_ = yieldReward_ > 0 ? _applyRisk(
                collectionItems_.collection,
                collectionItems_.assets[i],
                bearsStats_[i].faction,
                yieldReward_,
                bearsStats_[i].level.value,
                applyBuff_
            ) : 0;

            if(yieldRewardAfterRisk_ > 0) {
                emit BloodTokensClaimed(collectionItems_.collection, collectionItems_.assets[i], yieldRewardAfterRisk_);
            }

            collectionAccumulatedRewards_ += yieldRewardAfterRisk_;

            uint levelReward_ = _calculateLevelReward(secondsLifetime_, bearsStats_[i].level);
            uint fractionalClaimableLevelPercentage = _calculateClaimableFractionalLevelPercentage(
                secondsLifetime_, levelReward_
            );
            claimableLevelPercentageForBear[collectionItems_.collection][collectionItems_.assets[i]] += fractionalClaimableLevelPercentage;

            if (claimableLevelPercentageForBear[collectionItems_.collection][collectionItems_.assets[i]] >= 100) {
                levelReward_ += 1;
                claimableLevelPercentageForBear[collectionItems_.collection][collectionItems_.assets[i]] -= 100;
            }
            uint resultingLevel_ = bearsStats_[i].level.value + levelReward_;
            ...
            bearsStats_[i].stamina.value -= _calculateSpentStamina(secondsLifetime_, applyBuff_, bearsStats_[i].stamina);

            BSBStorageCaller.setBearStats(collectionItems_.collection, collectionItems_.assets[i], bearsStats_[i]);

            stateChangeDates[collectionItems_.collection][collectionItems_.assets[i]] = block.timestamp;

            delete secondsLifetime_;
            delete yieldReward_;
            delete resultingLevel_;
        }

        delete bearsStats_;
        delete collectionYield_;

        return collectionAccumulatedRewards_;
    }
    ...
    function _pickPortionOutOfAmount(uint portion_, uint tokenId_, uint amount_) internal view returns (bool) {
        return _pickPortionOutOfAmountWithRange(portion_, tokenId_, amount_, 100);
    }

    function _pickPortionOutOfAmountWithRange(
        uint portion_, uint tokenId_, uint amount_, uint range_
    ...
    function _enrollCollectionAssets(CollectionItems memory collectionItems_) internal {
        _checkIfAllowedToStake(collectionItems_, _msgSender());
        _addToElitesIfNecessary(collectionItems_);
        _stakeItems(collectionItems_, _msgSender());
    }
