...
contract KoreanPunksXD is ERC721 {
    ...
    constructor()
        ERC721("Korean Punks XD", "KPXD")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 1945) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeibs3c7hyjzzgq3qqfxiprvzjo2qjiqzyt4yizmvx25pjjkxbksayu/", Strings.toString(tokenId), ".json")
            );
    }
