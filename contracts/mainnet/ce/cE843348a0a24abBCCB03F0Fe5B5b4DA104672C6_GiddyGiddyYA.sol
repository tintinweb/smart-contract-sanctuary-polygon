...
contract GiddyGiddyYA is BaseYA, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  uint256 constant private PID = 0;
  ...
  address constant private SUSHI_ROUTER = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506;
  ...
  address constant private GIDDY_USDC_PAIR = 0xDE990994309BC08E57aca82B1A19170AD84323E8;
  uint256 constant private EARNINGS_FEE = 48500;
  uint256 constant private COMPOUND_THRESHOLD = 10e18;
  ...
  function swapGiddyForUsdc(address receiver, uint256 amount) private returns (uint256) {
    address[] memory route = new address[](2);
    route[0] = GIDDY_TOKEN;
    route[1] = USDC_TOKEN;
    if (!IERC20(GIDDY_TOKEN).approve(SUSHI_ROUTER, amount)) {
      revert("Failed giddy swap approval");
    }
    return IUniswapV2Router02(SUSHI_ROUTER).swapExactTokensForTokens(amount, 0, route, receiver, block.timestamp + 1200)[1];
  }

  function swapUsdcForGiddy(address receiver, uint256 amount) private returns (uint256) {
    address[] memory route = new address[](2);
    route[0] = USDC_TOKEN;
    route[1] = GIDDY_TOKEN;
    if (!IERC20(USDC_TOKEN).approve(SUSHI_ROUTER, amount)) {
      revert("Failed usdc swap approval");
    }
    return IUniswapV2Router02(SUSHI_ROUTER).swapExactTokensForTokens(amount, 0, route, receiver, block.timestamp + 1200)[1];
  }

  function giddyToUsdc(uint256 amount) public view returns(uint256)
  {
  ...
  function sharesToValue(uint256 shares) public view override returns (uint256) {
    ...
    uint256 rewards = getContractRewards();
    uint256 pending = rewards > COMPOUND_THRESHOLD ? rewards - (rewards * EARNINGS_FEE / BASE_PERCENT) : 0;
    return (getContractBalance() + pending) * shares / contractShares;
  }

  function compound() public override whenNotPaused {
    IGiddyChef(GIDDY_CHEF).harvestAll();
    uint256 earnings = IERC20(GIDDY_TOKEN).balanceOf(address(this));
    if (earnings > COMPOUND_THRESHOLD) {
      earnings = deductFeeGiddy(earnings, earnings * EARNINGS_FEE / BASE_PERCENT);
      if (!IERC20(GIDDY_TOKEN).approve(GIDDY_CHEF, earnings)) {
        revert("Failed compound approval");
      }
      emit Compound(earnings, lastCompoundTime, getContractBalance());
      IGiddyChef(GIDDY_CHEF).deposit(PID, earnings);
      lastCompoundTime = block.timestamp;
    }
  }

  function calcPrice(address router, address src, address dst, uint256 value) internal view returns (uint256 amountOut, uint256 priceImpact) {
    address[] memory route = new address[](2);
    route[0] = src;
    route[1] = dst;
    amountOut = IUniswapV2Router01(router).getAmountsOut(value, route)[0];
    uint256 dstReserve = IUniswapV2Router01(router).getAmountsOut(10000000000000000000000000000000000000000000000000000, route)[0];
    route[0] = dst;
    route[1] = src;
    uint256 srcReserve = IUniswapV2Router01(router).getAmountsOut(10000000000000000000000000000000000000000000000000000, route)[0];
    amountOut = dstReserve;
    priceImpact = srcReserve;
    // uint256 newDstReserve = (srcReserve * dstReserve) / (srcReserve + value);
    // priceImpact = ((srcReserve + value) * dstReserve * BASE_PERCENT) / (srcReserve * newDstReserve);
  }

  function depositCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = calcPrice(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, value - fap);
  }
  ...
  function deposit(bytes calldata auth, uint256 fap) external override whenNotPaused nonReentrant {
    ...
    if (!IERC20(USDC_TOKEN).transferFrom(owner, address(this), value)) {
      revert("Failed transfer to contract");
    }
    value = deductFeeUsdc(value, fap);
    uint256 giddyValue = swapUsdcForGiddy(address(this), value);

    uint256 shares = contractShares == 0 ? giddyValue * INIT_SHARES : giddyValue * contractShares / getContractBalance();
    userShares[owner] += shares;
    contractShares += shares;

    if (!IERC20(GIDDY_TOKEN).approve(GIDDY_CHEF, giddyValue)) {
      revert("Failed giddy chef approval");
    }
    IGiddyChef(GIDDY_CHEF).deposit(PID, giddyValue);

    emit Deposit(owner, value);
  }

  function depositGiddyCalc(uint256 value, uint256 fap) external pure override returns (uint256 amountOut, uint256 priceImpact) {
    amountOut = value - fap;
    priceImpact = 0;
  }
  ...
  function depositGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap) external override whenNotPaused nonReentrant {
    ...
    emit Deposit(req.owner, giddyToUsdc(giddyValue));
  }

  function withdrawCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = calcPrice(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, sharesToValue(shares));
    amountOut -= fap;
  }
  ...
  function getConfiguration() public pure override returns (uint256, uint256) {
    return (EARNINGS_FEE, COMPOUND_THRESHOLD);
  }
  ...
  function getFeeAccount() public view override returns (address) {
    return feeAccount;
  }

  function unstickMoney() public override onlyOwner {
    uint256 usdcBalance = IERC20(USDC_TOKEN).balanceOf(address(this));
    if (!IERC20(USDC_TOKEN).transfer(_msgSender(), usdcBalance)) {
      revert("Usdc transfer failed");
    }
    uint256 giddyBalance = IERC20(GIDDY_TOKEN).balanceOf(address(this));
    if (!IERC20(GIDDY_TOKEN).transfer(_msgSender(), giddyBalance)) {
      revert("Giddy transfer failed");
    }
  }

  function deductFeeUsdc(uint256 amount, uint256 fap) internal returns (uint256) {
    if (fap > 0) {
      if (!IERC20(USDC_TOKEN).transfer(feeAccount, fap)) {
  ...
  function deductFeeGiddy(uint256 amount, uint256 fap) internal returns (uint256) {
    if (fap > 0) {
      swapGiddyForUsdc(feeAccount, fap);
    }
    return amount - fap;
  }
}
...
abstract contract BaseYA {
  ...
  function depositCalc(uint256 value, uint256 fap) external view virtual returns (uint256 amountOut, uint256 priceImpact);
  function deposit(bytes calldata auth, uint256 fap) external virtual;
  function depositGiddyCalc(uint256 value, uint256 fap) external view virtual returns (uint256 amountOut, uint256 priceImpact);
  ...
  function withdrawCalc(uint256 shares, uint256 fap) external view virtual returns (uint256 amountOut, uint256 priceImpact);
  function withdraw(uint256 shares, uint256 fap) external virtual;
  function withdrawGiddyCalc(uint256 shares, uint256 fap) external view virtual returns (uint256 amountOut, uint256 priceImpact);
  function withdrawGiddy(uint256 shares, uint256 fap) external virtual;
  
  function getConfiguration() public pure virtual returns (uint256, uint256);
  function setFeeAccount(address account) public virtual;
  function getFeeAccount() public view virtual returns (address);
  function unstickMoney() public virtual;
