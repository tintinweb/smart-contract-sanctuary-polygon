...
/// @title VaultRouter
/// @notice Used for staking/unstaking in Vaults with Permit extension to improve UX
    ...
    ReentrancyGuardUpgradeable
{
    ...
    /**
     * @notice Stakes jtry and and gets Xjtry and BB-JTRY even deposits BB-JTRY to gauge if said to.
     * @param assets number of Jtry to stake
     * @param depositSTokensToGauge true: deposit BB-JTRY to gauge, false:send BB-JTRY to _owner
     * @param _stakeToken address of token to stake (jTRY)
     * @param _customVaultAddress address of CustomVault.sol used to convert jTRY -> xJTRY and vice versa
     * @param _xToken address of xJTRY
     * @param _bbVault address of ERC4626Vault
     * @param _gauge CustomGauge.sol address to deposit BB-JTRY tokens to
     * @param _owner owner of JTRY and receover of BB-JTRY in case of depositSTokensToGauge is 'false'
     */
    function stake(
        uint256 assets,
        ...
        require(msg.sender == _owner, "Caller and Owner miss-match");
        return
            _stakeHelper(
                assets,
                depositSTokensToGauge,
                _stakeToken,
                _customVaultAddress,
                _xToken,
                _bbVault,
    ...
    /**
     * @notice Stakes jtry with Permit and and gets Xjtry and BB-JTRY and deposits BB-JTRY to gauge
     * @param _metaTxArgs To perform allowance in JTRY using permit. See IMetatx for details
     * @param assets number of Jtry to stake
     * @param _stakeToken address of token to stake (jTRY)
     * @param _customVaultAddress address of CustomVault.sol used to convert jTRY -> xJTRY and vice versa
     * @param _xToken address of xJTRY
     * @param _bbVault address of ERC4626Vault
     * @param _gauge CustomGauge.sol address to deposit BB-JTRY tokens to
     * @return amount of BB-JTRY minted
     */
    ...
    function _stakeHelper(
        ...
        // transferFrom JTRY token from caller
        ...
        // giving allowance of JTRY to CustomVault
        ...
        // approving xJTRY to ERC4626Vault
        IERC20Upgradeable(_xToken).safeIncreaseAllowance(
            _bbVault,
            xTokensReceived
        );
        // now staking to the ERC4626Vault vault using xToken and receiving BB-JTRY
        uint shares = IERC4626StakingPool(_bbVault).stake(
            xTokensReceived,
            address(this)
        );

        // true means deposit BB-JTRY to gauge
        if (depositSTokensToGauge) {
            IERC20Upgradeable(_bbVault).safeIncreaseAllowance(_gauge, shares);
            IGauge(_gauge).depositFor(shares, 0, _owner);
        }
        // false means transfer BB-JTRY to _owner
        else {
            // now transferring sTokens to user
            IERC20Upgradeable(_bbVault).safeTransfer(_owner, shares);
        }
        return shares;
    }

    /**
     * @notice unstakes using sending BB-JTRY to ERC4626Vault and getting X-jTRY and then converting X-Jtry to JTRY in CustomVault.sol.
     * @param _metaTxArgsUnStake To perform allowance in BB-JTRY using permit. See IMetatx for details
     * @param _bbToken address of BB-JTRY
     * @param _amount amount of BB-JTRY tokens willing to spend for unstaking
     * @param _tokenOut address of JTRY
     * @param _customVault address of CustomVault.sol used to convert jTRY -> xJTRY and vice versa
     * @param _xToken address of xJTRY
     */
    ...
    function unstakeBuyBackToken(
        ...
        // transferring BB-JTRY from caller
        ...
        // giving allowance of xJTRY to CustomVault
        IERC20Upgradeable(_xToken).safeIncreaseAllowance(
            _customVault,
            receiveTokens
        );

        // getting JTRY from CustomVault.sol against deposited xJTRY
        uint256 receivedTOkenOut = IERC4626StakingPool(_customVault).withdraw(
            receiveTokens / (10 ** (18 - IERC20Metadata(_tokenOut).decimals())),
            address(this)
        );

        // transferring JTRY to caller
