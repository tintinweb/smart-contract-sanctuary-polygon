    ...
    function swap(
        ...
        if (inputToken != address(0) && outputToken != address(0)) { //first approve token0 to v2
            ...
            //} else if (inputToken == address(0) && amountIn == 0) {
                ...
                if (msg.value > amounts[0]) { //refund dust eth, if any
                    IERC20(address(0)).uniTransfer(payable(msg.sender), msg.value - amounts[0]);
                }
...
contract UniswapV2Aggergator {
    ...
    function addRouter(string memory name, address payable uniswapV2Address) external {
        require(_owner == msg.sender, "only owner");
        require(address(_routers[name]) == address(0), "name existed");
        _names.push(name);
        _routers[name] = UniswapV2(uniswapV2Address);
    }
    ...
    function swap(
        ...
        if (inputToken != address(0)) {
            IERC20(inputToken).approve(address(_routers[name]), amountIn);
        }
        IERC20(inputToken).uniTransferFrom(payable(msg.sender), address(_routers[name]), amountIn);
        _routers[name].swap(
            inputToken, outputToken, amountIn, amountOut, path, to, slippageAmount
        );
    }
    ...
    function getBestPath(BestPathParam memory param) 
        external
        view
    ...
    {   
        ...
        for (uint256 i; i < _names.length; i++) {
            (address[] memory path, uint256 result) = _routers[_names[i]].getBestPath(
                pathToken0, 
                pathToken1, 
                param.amountIn, 
                param.amountOut);
        ...
        if (bestResult > 0) {
            data = abi.encodePacked(bestName, param.token0, param.token1, param.amountIn, param.amountOut, bestPath, param.to, param.slippage);
        }
    }
