...
contract Cubix is Ownership {
    ...
    function register(address referrerAddress, address placement, uint256 usdAmount, uint pkgIndex, uint _position, bool isUSDT) external returns(bool)
    {
        ...
        require(UserInfo[placement].uID > 0 || placement == owner(),"Incorrect placement");
        ...
        if(placement == address(0) || placement == msg.sender || UserInfo[placement].uID == 0)
        {
            placement = owner() ;
        }               
    ...
    function registerOldUsers(address _user, uint256 userid,address referrerAddress, address placement, uint256 usdAmount, uint pkgIndex, uint _position, bool isUSDT) external onlyOwner returns(bool)
    {      
        ...
        if(placement == address(0) || placement == _user || UserInfo[placement].uID == 0)
        {
            placement = owner() ;
        }
        UserInfo[_user].referrer = referrerAddress;
        UserInfo[_user].placement = placement;
        UserInfo[_user].uID = userid;
        UserInfo[referrerAddress].directusers += 1;
        UserInfo[referrerAddress].teamCount += 1;
