...
contract GenericHandlerUpgradeable is Initializable, AccessControlUpgradeable {
    ...
    **/
    function genericDeposit(
        ...
        uint256 fees = deductFee(_destChainID, _feeToken, _gasLimit, _gasPrice);
    ...
    **/
    function replayGenericDeposit(
        ...
        uint256 fee = deductFeeWithoutBaseFee(_destChainID, record._feeToken, _gasLimit, _gasPrice);
        record._gasLimit = _gasLimit;
        record._gasPrice = _gasPrice;
        record._totalFeesInTokens += fee;
        bridge.replayGenericDeposit(_destChainID, record._resourceID, _depositNonce);
    }
    ...
    **/
    function decodeData(bytes calldata _data) internal pure returns (DepositRecord memory) {
        ...
        (
            ...
            depositData.data,
            depositData.hash,
            depositData._gasLimit
        ) = abi.decode(_data, (uint8, uint64, address, address, bytes4, bytes, bytes32, uint256));
        (, , , , , , , , depositData._gasPrice) = abi.decode(
            _data,
            (uint8, uint64, address, address, bytes4, bytes, bytes32, uint256, uint256)
        );
        return depositData;
    }
    ...
    **/
    function setFees(
        ...
        feeManager.setFee(destinationChainID, feeTokenAddress, feeFactor, bridgeFee, decimals, accepted);
    }
    ...
    **/
    function calculateFees(
        ...
        return (feeFactor * gasLimit * gasPrice) + bridgeFees;
    }
    ...
    **/
    function deductFee(
        ...
        uint256 fee = ((feeFactor * gasLimit * gasPrice) + bridgeFees);
        token.safeTransferFrom(msg.sender, address(feeManager), fee);
        return fee;
    }

    /**
        @notice Function deducts fees for a cross chain Call for replay transactions.
        @param destinationChainID id of the destination chain.
        @param feeTokenAddress Address fee token.
        @param gasLimit Gas limit required for cross chain call.
        @param gasPrice Gas Price for the transaction.
    **/
    function deductFeeWithoutBaseFee(
        uint8 destinationChainID,
        address feeTokenAddress,
        uint256 gasLimit,
        uint256 gasPrice
        ...
        IERC20Upgradeable token = IERC20Upgradeable(feeTokenAddress);
        uint256 fee = feeFactor * gasLimit * gasPrice;
        token.safeTransferFrom(msg.sender, address(feeManager), fee);
        return fee;
    }
...
interface iFeeManagerGeneric {
    ...
    function getFee(uint8 destinationChainID, address feeTokenAddress)
        external
        view
        returns (
            uint256,
