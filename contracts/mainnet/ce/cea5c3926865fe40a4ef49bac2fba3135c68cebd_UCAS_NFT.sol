...
contract UCAS_NFT is ERC721Enumerable, Ownable {
    ...
    // 设置单次最大的mint数量
    function setmaxMintAmountOneTime(uint256 _newmaxMintAmount)
        public
        onlyOwner
    {
    ...
    function mintDrop(address[] calldata addrList) public onlyOwner {
        ...
        require(
            totalSupply() + addrList.length <= maxSupply,
            "Send over total supply"
        );
    ...
    function bulkTransfer(
        ...
        for (uint256 i = 0; i < addrList.length; i++) {
            safeTransferFrom(msg.sender, addrList[i], nftlist[i]);
        }
    }
    ...
    function setMaxTokenAmountForEachAddress(uint256 _MaxAmount)
        public
        onlyOwner
    {
