...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
  ...
  // mapping for token creator
    ...
    */
    ...
    function _safeMint(address to, uint256 tokenId, uint256 fee, address creator) internal virtual {
        _safeMint(to, tokenId, fee,creator, "");
    }

    function _safeMint(address to, uint256 tokenId, uint256 fee, address creator, bytes memory _data) internal virtual {
        _mint(to, tokenId, fee, creator);
        require(_checkOnERC721Received(address(0), to, tokenId, _data), "ERC721: transfer to non ERC721Receiver implementer");
    }

    function _mint(address to, uint256 tokenId, uint256 fee, address creator) internal virtual {
...
contract Fantically is ERC721 {
    ...
    function Minting(string memory tokenURI, uint256 fee, address creator) public returns (uint256) {
        uint256 newItemId = tokenCounter;
        // verifySign(tokenURI, sign);
        _safeMint(msg.sender, newItemId, fee, creator);
