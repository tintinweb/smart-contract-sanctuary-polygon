...
contract Stakers is IStakers {
    ...
    /// @inheritdoc IStakers
    uint256 public nTrustedParties;
    /// @inheritdoc IStakers
    uint256 public nStakers;
    /// @inheritdoc IStakers
    address[] public registeredParty;
    ...
    constructor(address _storageAddress, uint256 _stake, uint256 _toWei) {
        sto = _storageAddress;
    ...
    function removeTrustedParty(address _addr) external onlySuperUser {
        _removeTrustedParty(_addr);
    }

    function _removeTrustedParty(address _addr) private {
        require(
    ...
    function enrol() external payable {
        ...
        require(
            msg.value == requiredStake / TO_WEI,
    ...
    function unEnroll() external {
        ...
        _unEnroll(msg.sender);
    }

    /**
     @notice Forces the unenroll of a staker by governance decision,
       followed by removing it from list of trustedParties
     */
    function forceUnEnroll(address _staker) external onlySuperUser {
        _unEnroll(_staker);
        _removeTrustedParty(_staker);
    }

    function _unEnroll(address _staker) private {
        require(removeStaker(_staker), "failed to unenroll");
        uint256 amount = pendingWithdrawals[_staker] + stakes[_staker];
        pendingWithdrawals[_staker] = 0;
        stakes[_staker] = 0;
        if (amount > 0) {
            payable(_staker).transfer(amount);
        }
        emit NewUnenrol(_staker);
    }
        ...
        onlyLogicOwner
    {
        ...
        require(isStaker[_staker] || isStakerGovernance(_staker), "failed to update: staker not registered");
        require(
    ...
    function finalize() external onlyLogicOwner {
        ...
        if (level() == 1) {
            address addressToReward = popUpdaters();
            if (!isStakerGovernance(addressToReward)) {
                addRewardToUpdater(addressToReward);
            }
        }
        require(
            level() == 0,
    ...
    /**
     @dev a staker can only be removed via slash, governance,
    ...
    /// @inheritdoc IStakers
    function trustedParty(uint256 idx) public view returns (address) {
        // go through all registered parties ever,
        // and count only those that are trusted parties
        require(idx < nTrustedParties, "trusted party queried beyond array length");
        uint256 i;
        uint256 validPartiesBelowIdx;
        address candidate;
        while (validPartiesBelowIdx < idx + 1) {
            candidate = registeredParty[i];
            if (isTrustedParty[candidate]) validPartiesBelowIdx++;
            i++;
        }
        return candidate;
    }
    ...
    /// @inheritdoc IStakers
    function staker(uint256 idx) public view returns (address) {
        // go through all registered parties ever,
        // and count only those that are stakers.
        require(idx < nStakers, "staker queried beyond array length");
        uint256 i;
        uint256 validStakersBelowIdx;
        address candidate;
        while (validStakersBelowIdx < idx + 1) {
            candidate = registeredParty[i];
            if (isStaker[candidate]) validStakersBelowIdx++;
            i++;
        }
        return candidate;
    }
    ...
    /**
     @notice Returns true if the address provided corresponds to a governance call
     @param _staker the address to be queried
     @return true if the provided address corresponds to a governance call
     */
    function isStakerGovernance(address _staker) private pure returns (bool) {
        return _staker == address(0);
    }
}
