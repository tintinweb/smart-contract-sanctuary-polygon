...
import "../interfaces/maximizer/IMaximizer.sol";
 ...
 */
contract DysonMaximizerVault is Initializable, OwnableUpgradeable, ERC20Upgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // The strategy currently in use by the vault.
    IMaximizer public strategy;
    bool private _isStrategyInitialized;
    uint256 private _totalLock;

    mapping(address => uint256) private _locks;

    // tokenManager addresses can send/receive DysonMaximizerVault receipts and lock/unlock tokens
    mapping(address => bool) private _tokenManager;
     ...
     */
    function __DysonMaximizerVault_init(
        string memory _name,
        string memory _symbol) public initializer {
        __Ownable_init_unchained();
        __ERC20_init_unchained(_name, _symbol);
        __ReentrancyGuard_init_unchained();
        __DysonMaximizerVault_init_unchained();
    }

    function __DysonMaximizerVault_init_unchained() internal initializer {}
     ...
     */
    function earn(uint256 _shares) public {
        uint _bal = available();
        want().safeTransfer(address(strategy), _bal);
        strategy.deposit(msg.sender, _shares);
    }
    ...
    function withdraw(uint256 _shares) public nonReentrant {
        require(_shares <= balanceOf(msg.sender), "DysonMaximizerVault::amount exceeds balance!");
        require(_shares <= unlockOf(msg.sender), "DysonMaximizerVault::can't withdraw locked receipt tokens!");
        uint256 amount = strategy.withdraw(msg.sender, _shares);
        _burn(msg.sender, _shares);
        want().safeTransfer(msg.sender, amount);
    }

    function setStrategy(IMaximizer _strategy) public onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        if (!_tokenManager[msg.sender]) revert('token is non-transferable');
        super._transfer(from, to, amount);
    }

    function _burn(address account, uint256 amount) internal override {
        require(unlockOf(account) >= amount, "Vault: burn amount exceeds unlocked balance");
        super._burn(account, amount);
    }

    function lock(address _account, uint256 _amount) public {
        require(_tokenManager[msg.sender], 'caller not tokenManager');
        require(_account != address(0), "Cannot lock to the zero address");
        require(_amount <= unlockOf(_account), "amount over unlocked balance");
    ...
    function unlock(address _account, uint256 _amount) public {
        require(_tokenManager[msg.sender], 'caller not tokenManager');
        require(_account != address(0), "Cannot unlock to the zero address");
        require(_amount <= lockOf(_account), "amount over locked balance");
    ...
    function balanceOf(address account) public view override returns (uint256) {
        return super.balanceOf(account) + _locks[account];
    }
    ...
    function unlockOf(address account) public view returns (uint256) {
    ...
    function addTokenManager(address _address) public onlyOwner {
        _tokenManager[_address] = true;
    }

    function removeTokenManager(address _address) public onlyOwner {
        _tokenManager[_address] = false;
    }

    function isTokenManager(address _address) public view returns (bool) {
        return _tokenManager[_address];
    }
}
