...
import "./V3Router/libraries/IncentiveId.sol";
...
contract QuickMaiLpFarm is BaseSiloAction, IIncentiveKey {
    address public constant QUICK_V3_POSITION_MANAGER =
        0x8eF88E4c7CfbbaC1C163f7eddd4B578792201de6;

    // address public constant PARASWAP_ROUTER_ADDRESS =
    //     0xDEF171Fe48CF0115B1d80b88dc8eAB59176FEe57;

    address public constant QUICK_V3_ROUTER =
        0xf5b509bB0909a69B1c207E495f687a596C168E12;

    // address public constant V3_CORE_FACTORY_ADDRESSES =
    //     0x411b0fAcC3489691f28ad58c47006AF5E3Ab3A28;

    address public constant QUOTER_ADDRESSES =
        0xa15F0D7377B2A0C0c10db057f641beD21028FC89;

    address public constant QUICK_V3_FARM =
        0x7F281A8cdF66eF5e9db8434Ec6D97acc1bc01E78;

    address public constant QUICK_V2_ROUTER =
        0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;

    address public constant MAI_ADDRESS =
        0xa3Fa99A148fA48D14Ed51d610c367C61876997F1;

    address public constant USDT_ADDRESS =
        0xc2132D05D31c914a87C6611C10748AEb04B58e8F;

    address public constant dQUICK_ADDRESS =
        0x958d208Cdf087843e9AD98d23823d32E17d723A1;

    address public constant QI_ADDRESS =
        0x580A84C73811E1839F75d86d75d88cCa0c241fF4;

    uint128 public constant MAX_INT = 2**128 - 1;
    ...
    function _act(
    ...
    ) internal returns (uint256[4] memory outputAmounts) {
        ...
        console.log(
            "############## QuickMaiLpFarm  inputAmounts[0]: %s  inputAmounts[3]: %s",
            inputAmounts[0],
            inputAmounts[3]
        );
        ...
        if (
            ...
            (uint256 reward, uint256 bonusReward) = protocol
                .farm
                .collectRewards(protocol.key, tokenId);

            console.log(
                "collect reward :  %s  bonusReward : %s",
                reward,
                bonusReward
            );

            if (reward > 0) {
                reward = protocol.farm.claimReward(
            ...
            if (inputAmounts[0] > 0) {
                ...
                if (id != 0) {
                    ...
                    uint256 reward = protocol.farm.claimReward(
                        IERC20Minimal(address(protocol.dQUICK)),
                        address(this),
                        0,
                        MAX_INT
                    ); //harvest rewards

                    console.log(" remove v3  claim dQUICK reward: %s", reward);

                    if (reward > 0) {
                        outputAmounts[1] = _takeFee(
                            implementation,
                            reward,
                            address(protocol.dQUICK)
                        );
                    }
                    outputAmounts[1] += inputAmounts[1];

                    // claim external bonus Qi reward
                    reward = protocol.farm.claimReward(
                        IERC20Minimal(QI_ADDRESS),
                        address(this),
                        0,
                    ...
                    console.log(" remove v3  claim qi reward: %s", reward);
                    ...
                    if (removeLiquidity > 0) {
                        ...
                        console.log(
                            " remove v3  _removeLiquidityPosition : amount0: %s amount1:%s",
                            amount0,
                            amount1
                        );

                        uint256 output = _swapTokens(
                            USDT_ADDRESS,
                            MAI_ADDRESS,
                            amount1
                        );

                        outputAmounts[3] = amount0 + output;

                        // console.log(
                        //     " remove v3  _swapTokens output:%s ",
                        //     output
                        // );

                        if (removeLiquidity < liquidity) {
                            // console.log("re enter farm ");
        ...
        console.log(
            "############## QuickMaiLpFarm  outputAmounts[0]: %s  outputAmounts[2] :%s outputAmounts[3]: %s",
            outputAmounts[0],
            outputAmounts[2],
            outputAmounts[3]
        );
        ...
        )
    {
        ...
        // console.log("############## QuickMaiLpFarm getTickLowerAndUpper tickLower: %s  tickUpper: %s ", tickLower,tickUpper);

        (tokenId, liquidity, amount0, amount1) = protocol.positionManager.mint(
        ...
        private
    {
        ...
        // IAlgebraEternalFarming externalFarm = protocol.farm.eternalFarming();
        // IFarmingCenterVault farmCenter = protocol.farm.farmingCenterVault();
        ...
        // (address incentive, address eternal) = protocol.farm.virtualPoolAddresses(address(protocol.pool));
        ...
        returns (int24 tickLower, int24 tickUpper)
    {
        ...
        // int24 tickLowerCount = 3;
        // int24 tickUpperCount = 3;
    ...
    // function getUserLastPosition(address user, ProtocolHelper memory protocol)
    //     internal
    //     view
    //     returns (uint256 tokenId)
    // {
    //     uint256 tokenCount = protocol.positionManager.balanceOf(user);
    //     if (tokenCount > 0) {
    //         tokenId = protocol.positionManager.tokenOfOwnerByIndex(
    //             user,
    //             tokenCount - 1
    //         );
    //     } else {
        ...
        returns (ActionBalance memory)
    {
        ...
        path[0] = USDT_ADDRESS;
        path[1] = MAI_ADDRESS;
        uint256 amountOut = amount1 > 0
            ? getUniPrice(QUICK_V2_ROUTER, path, amount1)
            : 0;
        uint256 convertedBal = amount0 + amountOut;
        ...
        return
            ActionBalance({
                ...
                collateralConvertedToken: MAI_ADDRESS,
                lpUnderlyingBalances: string(
                    abi.encodePacked((Strings.toString(convertedBal)))
                ),
                lpUnderlyingTokens: string(
                    abi.encodePacked(
                        Strings.toHexString(uint160(MAI_ADDRESS), 20)
                    )
                )
            });
    }
        ...
        )
    {
        ...
        reward += farm.eternalFarming().rewards(msg.sender,IERC20Minimal(dQUICK_ADDRESS));
    ...
    // function _getSwapAmt(uint256 amtA, uint256 resA)
    //     internal
    //     pure
    //     returns (uint256 amount)
    // {
    //     amount =
    //         (sqrt((1997**2 * resA**2) + (4 * 997 * 1000 * amtA * resA)) -
    //             (1997 * resA)) /
    //         1994;
    // }
    ...
    //  */
    // function _getOptimal(
    //     address _lp,
    //     address _single,
    //     uint256 _amount
    // ) internal view returns (uint256 optimalAmountA) {
    ...
    //     (uint112 resA, uint112 resB, ) = tokenPair.getReserves();
    //     if (tokenPair.token0() == _single) {
    //         lpAmount = (totalSupply * _amount) / 2 / resA;
    //     } else {
    //         lpAmount = (totalSupply * _amount) / 2 / resB;
    //     }
    // }

    // function _getTokenAmountfromLP(
    //     address _lp,
    //     address _single,
    //     uint256 _amount
    ...
    //     uint256 amountA = (_amount * resA) / totalSupply;
    //     uint256 amountB = (_amount * resB) / totalSupply;

    //     address[] memory path = new address[](2);
    //     path[0] = tokenPair.token1();
    //     path[1] = tokenPair.token0();

    //     if (tokenPair.token0() == _single) {
    //         uint256 amountOut = getUniPrice(QUICK_V2_ROUTER, path, amountB);
    //         tokenAmount = amountA + amountOut;
    //     } else {
    //         uint256 amountOut = getUniPrice(QUICK_V2_ROUTER, path, amountA);
    //         tokenAmount = amountB + amountOut;
    //     }
    // }

    // function _getV3PoolInputAmount(
    //     uint256 input0,
    //     uint256 price,
    //     uint256 highPrice,
    //     uint256 lowPrice
    // ) internal pure returns (uint256 input1) {
    //     uint256 liquidity0 = (input0 * sqrt(price) * sqrt(highPrice)) /
    //         (sqrt(highPrice) - sqrt(price));
    //     input1 = liquidity0 * (sqrt(price) - sqrt(lowPrice));
    // }
    ...
    function _getSwapAmountForV3Lp(
        ...
        // if (protocol.quickMaiLP.token0() == _single) {
        //     v2SwapAmount = calculateSwapAmount(
        //         resA,
        //         resB,
        //         amount0,
