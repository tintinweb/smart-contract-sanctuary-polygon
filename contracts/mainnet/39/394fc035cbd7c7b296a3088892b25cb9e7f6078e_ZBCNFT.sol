...
contract ZBCNFT is Setting, ERC721 {

    /**
     * @dev Throws if called by any account other than the owner.
     */
    modifier onlyIDO() {
        require(msg.sender == sysAddr.ido, "Ownable: caller is not the ido");
        _;
    }
    ...
    constructor(string memory url_, string memory name_, string memory symbol_) ERC721(name_,symbol_) {
        url = url_;
    }

    function mint(address to, uint256 tokenId) external onlyIDO {
        require(currentSupply < maxSupply, "ZBCNFT: max supply");
        _safeMint(to, tokenId);
        currentSupply++;
        INFTPool(sysAddr.nftPool).updateUserNFT(address(0), to);
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        return "https://www.zephyrloops.com/Pic.jpg";
    }

    function _transfer(address from, address to, uint256 tokenId) internal override {
        INFTPool(sysAddr.nftPool).updateUserNFT(from, to);
