...
contract MarketPlaceBE is MarketPlaceBEStorage, AccessControlUpgradeable, ReentrancyGuardUpgradeable, IMarketPlaceBE {
    ...
    function buy721(Order calldata _order, bytes memory _signature) external payable nonReentrant {
        require(!usedOrderIds[_order.id], "Order ID already used");
        ...
        // mark this order ID as used
        usedOrderIds[_order.id] = true;
        emit NewNFTBuyOrder(_order.collection, _order.tokenId, _msgSender());
    }

    /**
     * @notice buy a NFT (ERC1155), not checking if a collection is whitelisted, checking the buy amount requested
     * @param _order sell order
     * @param _signature message signature
    ...
    function buy1155(Order calldata _order, bytes memory _signature,  uint256 _buyAmount) external payable nonReentrant {
        require(!usedOrderIds[_order.id], "Order ID already completely filled");
        require(_buyAmount <= _order.amount - totalFilledAmountOrderIds[_order.id], "Not enough tokens to buy");
        string memory originalMsg = restoreMsg(_order);
        bytes32 originalPrefixedHash = ethMsgHash(msgHash(originalMsg));
        ...
        // update total amount already filled
        totalFilledAmountOrderIds[_order.id] += _buyAmount;
        if (_order.amount - totalFilledAmountOrderIds[_order.id] == 0) {
            usedOrderIds[_order.id] = true;
        }
...
contract MarketPlaceBEStorage {
    ...
    mapping(uint256 => uint256) public totalFilledAmountOrderIds;   // for erc1155 only
}
