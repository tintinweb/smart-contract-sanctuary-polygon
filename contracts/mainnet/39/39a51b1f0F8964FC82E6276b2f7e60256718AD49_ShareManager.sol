...
contract ShareManager is BaseContract {
  ...
  function checkin() external {
    ...
    uint nftBalance = rankingNft.balanceOf(msg.sender);
  ...
  function claim(uint _round) external {
    ...
    require(!rankingNft.waitingList(msg.sender), "ShareManager: you can not do this now");
    ...
    if (user.nftStocked > 0) {
      ...
      for (uint i = 0; i < user.listNftStocked.length; i++) {
        rankingNft.transferFrom(address(this), msg.sender, user.listNftStocked[i]);
      }
    }
    if (claimable > 0) {
  ...
  function _takeNftToken(User storage _user) private {
    require(rankingNft.isApprovedForAll(msg.sender, address(this)), "ShareManager: please call setApprovalForAll() first");
    uint[] memory userNFTs = rankingNft.getOwnerNFTs(msg.sender);
    for (uint i = 0; i < userNFTs.length; i++) {
      _user.listNftStocked.push(userNFTs[i]);
      rankingNft.transferFrom(msg.sender, address(this), userNFTs[i]);
    }
  }
