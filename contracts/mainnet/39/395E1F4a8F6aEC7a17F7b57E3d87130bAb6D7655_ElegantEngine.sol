...
contract ElegantEngine is Ownable, Concatenate {
    ...
    address public coachIntAddress;
    ...
    function tierBoost(uint _id) public returns(uint) {
        ...
        address _con = perfByRacer[_week][_id].nftContract;
        uint _tokenID = perfByRacer[_week][_id].id;
    ...
    function buildRace(uint _event) internal {
        ...
        for(i; i<racing; i++){ //go through every racer
            ...
            racersByRace[0][raceNum].push(_temp.tempID);   
            ...
            boosts.boost = boosts.boost + tierBoost(_temp.tempID);
            if (boosts.coach != 0){
