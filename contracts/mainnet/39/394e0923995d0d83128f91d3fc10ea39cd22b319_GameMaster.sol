...
contract GameMaster is Initializable, AccessControlUpgradeable, PausableUpgradeable, ICharacter, IGameMaster {
    ...
    function performFight(
        ...
        uint16 xpRewardsToClaim = uint16(fightMultiplier.mul(getXpGainForFight(playerPower, targetPower)));
        ...
        if (playerRoll > monsterRoll) {
            ...
            tokens = handleFightWonOverflow(char,seed,tokens);
            ...
            tokens = handleFightLostOverflow(char,tokens);
            handleStake(tokens);
        }
    ...
    function handleFightWonOverflow(uint256 char, uint256 seed, uint256 tokens) private returns(uint256) {
    ...
    function handleFightLostOverflow(uint256 char, uint256 tokens) private returns(uint256) { 
        CharacterType cType = characters.getCharacterType(char);
         if (cType == CharacterType.Elite) {
    ...
    function harvestRewardsAndAddToTotalEpochRewards() external {
        // pull rewards from Core
        uint256 balanceBefore = tagToken.balanceOf(address(this));
        cipherCore.harvestYieldInTags();
        uint256 harvested = tagToken.balanceOf(address(this)).sub(balanceBefore);
     ...
     function injectRewardsAndAddToTotalEpochRewards(uint256 amount) external {
        require(tagToken.transferFrom(_msgSender(), address(this), amount), 'Unable to pull funds from the sender');
        injectedRewards = injectedRewards.add(amount);
        // add injected rewards
        totalEpochRewards = totalEpochRewards.add(injectedRewards);
    ...
    ICharacter
{
    ...
    function lootItems(
        ...
        address owner = characters.ownerOf(character);
        
        require(amount > 0, 'amount > 0 needed');
        require(owner == user || owner == msg.sender, 'Not the character owner');
