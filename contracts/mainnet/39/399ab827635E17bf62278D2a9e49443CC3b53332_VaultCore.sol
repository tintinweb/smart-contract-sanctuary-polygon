...
contract VaultCore is VaultStorage {
    ...
    function _mint(address _asset, uint256 _amount, uint256 _minimumMATICPlusAmount) internal {
        ...
        uint256 _toMintMaticPlus = _amount;

        // Transfer the deposited coins to the vault
        // Mint fee
        if (mintFeeBps > 0 && treasuryAddress != address(0)) {
            console.log("FEE: ", _toMintMaticPlus.mul(mintFeeBps).div(10000));
            uint256 mintFee = _toMintMaticPlus.mul(mintFeeBps).div(10000);
            _toMintMaticPlus = _toMintMaticPlus.sub(mintFee);
            IERC20(primaryTokenAddress).safeTransfer(treasuryAddress, mintFee);
        ...
        emit Mint(msg.sender, _toMintMaticPlus);

        // Mint matching MATICPlus
        maticPlus.mint(msg.sender, _toMintMaticPlus);
    ...
    function _redeem(uint256 _amount, uint256 _minimumUnitAmount) internal {
        ...
        // Check that MATIC+ is backed by enough assets
        uint256 _totalSupply = maticPlus.totalSupply();
        if (maxSupplyDiff > 0) {
            // Allow a max difference of maxSupplyDiff% between
            // backing assets value and MATIC+ total supply
        ...
        IERC20 primaryToken = IERC20(primaryTokenAddress);
        ...
        uint256 totalAmount = primaryToken.balanceOf(address(this));
        ...
        for (uint8 i = 0; i < strategyWithWeights.length; i++) {
            ...
            if (amountsToWithdraw[i] > 0) {
                IStrategy(strategiesToWithdrawFrom[i]).withdraw(address(this), primaryTokenAddress, amountsToWithdraw[i]);
            } else {
                IStrategy(strategiesToWithdrawFrom[i]).withdrawAll();
            }
        }
        require(primaryToken.balanceOf(address(this)) >= (output + redeemFee), "Not enough funds after withdrawl.");
        primaryToken.safeTransfer(msg.sender, output);
        maticPlus.burn(msg.sender, _amount);

        // Remaining amount i.e redeem fees will be rebased for all other MATIC+ holders
    ...
    function _quickAllocate() internal {
        ...
        uint256 allocateAmount = IERC20(primaryTokenAddress).balanceOf(address(this));
        if (quickDepositStrategyAddr != address(0) && allocateAmount > 0) {
            IStrategy strategy = IStrategy(quickDepositStrategyAddr);
            IERC20(primaryTokenAddress).safeTransfer(address(strategy), allocateAmount);
            console.log("@deposit: depositing in: %s", address(strategy));
            strategy.deposit(primaryTokenAddress, allocateAmount);
            console.log("@deposit: deposited");
            emit AssetAllocated(primaryTokenAddress, quickDepositStrategyAddr, allocateAmount);
        }
    }
    ...
    function _rebase() internal whenNotRebasePaused {
        ...
        uint256 vaultValue = _checkBalance(true);
        ...
        if (vaultValue > maticPlusSupply) {
            ...
            try IRebaseHandler(rebaseHandler).process() {} catch {
                console.log("@deposit: HANDLER_FAILED");
            }
        }
    }
    ...
    function _checkBalance(bool _nav) internal view virtual returns (uint256 balance) {
        balance = IERC20(primaryTokenAddress).balanceOf(address(this));
    ...
    function totalValue() external view virtual returns (uint256) {
        ...
        uint256 bal = _checkBalance(false);
        console.log("tvl: bal", bal);
        return bal.mul(_oracle.price(primaryTokenAddress)).div(1e18);
    }
    ...
    function _calculateRedeemOutput(uint256 _amount) internal view returns (uint256, uint256, uint256) {
        uint256 _psBalance = _checkBalance(true);
        uint256 _psDecimals = Helpers.getDecimals(primaryTokenAddress);
        uint256 _psBalanceIn18Decs = _psBalance.scaleBy(18, _psDecimals);
        uint256 _maticPlusTS = maticPlus.totalSupply();
        uint256 _give = (_amount.mul(_psBalanceIn18Decs)).div(_maticPlusTS);
    ...
    function available() public view returns (uint256) {
        return IERC20(primaryTokenAddress).balanceOf(address(this));
    }
