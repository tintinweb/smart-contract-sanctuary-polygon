...
contract OnlySwapV5 {
    ...
    function payfee(address token,uint amount,IRouter.Swap_router[] memory feeSwap) internal returns(uint fee){
        fee=amount*3/1000;
        uint feeOnTokenPair=fee;
        if(token!=_pairFeeToken){
            if(feeSwap.length==0){
                uint price;
                (price,feeSwap) = getBestSwapsByTokens(token,_pairFeeToken,fee);
                require(price>0,"No route for swap");
            }
            feeOnTokenPair=internalSwapTokensForTokensByPath(feeSwap,fee,false,new IRouter.Swap_router[](0));
        }
        if(promo){
    ...
    function internalSwapTokensForTokensByPath(IRouter.Swap_router[] memory bestSwap,uint amountA,bool takefee,IRouter.Swap_router[] memory feeSwap) internal returns(uint amount) {
        bool hasUSDC=false;
        amount=amountA;
        if(takefee && !hasUSDC && bestSwap[0].tokenA==_pairFeeToken){
            amount=amountA-payfee(bestSwap[0].tokenA,amountA,feeSwap);
            hasUSDC=true;
        }
        for(uint32 i;i<bestSwap.length;i=unsafe32_inc(i)){
            ...
            if(takefee && !hasUSDC && bestSwap[i].tokenB==_pairFeeToken){
                amount=amount-payfee(bestSwap[i].tokenB,amount,feeSwap);
                hasUSDC=true;
            }
        }
        if(takefee && !hasUSDC){
            amount=amount-payfee(bestSwap[bestSwap.length-1].tokenB,amount,feeSwap);
        }
    }
    function bestSwapTokensForTokensByPath(uint amountA,uint minAmountTokenB,IRouter.Swap_router[] memory path,IRouter.Swap_router[] memory feepath) public payable {
        ...
        internalSwapTokensForTokensByPath(path,amountA,true,feepath);
    ...
    function bestPathSwapTokensForTokens(address tokenA,address tokenB,uint amountA,uint slippage) public payable {
        ...
        (uint price,IRouter.Swap_router[] memory path) = getBestSwapsByTokens(tokenA,tokenB,amountA);
        require(price>0,"No route for swap");
        internalSwapTokensForTokensByPath(path,amountA,true,new IRouter.Swap_router[](0));
        uint amount=IERC20(tokenB).balanceOf(address(this));
    ...
    function refund(IRouter.Swap_router[] memory path) internal {
        //refund matic
        ...
        for(uint32 i;i<path.length;i=unsafe32_inc(i)){
            uint tokenBalance=IERC20(path[i].tokenA).balanceOf(address(this));
            if(tokenBalance>0){
                require(IERC20(path[i].tokenA).transfer(msg.sender,tokenBalance),"Error on transfer");  
            }
        }
    }
