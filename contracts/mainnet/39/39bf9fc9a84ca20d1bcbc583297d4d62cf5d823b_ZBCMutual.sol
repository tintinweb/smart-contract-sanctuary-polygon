...
contract ZBCMutual is Setting,IMutual, RiskOrder{
    ...
    function setFeeWallet(address lenderFee) external {
    ...
    function depositByBorrow(uint256 _amount, address lender_) external onlyRegister {
        ...
        if (lenders[lender_].addr != lender_) {
            lenders[lender_].addr = lender_;
        }

        IERC20(sysAddr.usdt).transfer(feeWallet,profitAmt);
    ...
    function _withdrawByLender(address addr,address to) private {
        ...
        IWallet(feeWallet).withdraw(sysAddr.usdt,to,pendingAmt);
    ...
    function _isLending(address addr) private view returns(bool) {
        return _isLender(addr);
    }
    ...
    function _luckPoolRewards() private {
        ...
        for (uint i=0; i<todayLuckAddrs.length; i++) {
            rewardInfo[todayLuckAddrs[i]].luck += avgAmount * realPercents / base100;
            rewardInfo[todayLuckAddrs[i]].split += avgAmount * splitPercents / base100;
    ...
    function getUser(address _user) public view returns(UserInfo memory,RewardInfo memory) {
        ...
        if (isExit) {
            (uint256 lastTime, uint256 avgAmount) = _pendingLuck();
            if (sys.luckLastTime != lastTime) {
                ri.luck += avgAmount * realPercents / base100;
                ri.split += avgAmount * splitPercents / base100;
                ri.totalRevenue += avgAmount;
            }
        }
    ...
    function getLeader(address addr) external view returns(Lender memory) {
        ...
        lender.isLender = _isLender(addr);
        return lender;
    }
    ...
    function getLender10() external view returns(address[] memory lenders10) {
        IWallet.UserInfo[] memory lenderArr = getAllNFTOwners();

        Lender[] memory arr = new Lender[](lenderArr.length);
        uint256 allLender = 0;
        for (uint i=0; i<lenderArr.length; i++) {
            Lender memory lender = Lender({
                addr:  lenderArr[i].addr,
                nftNums: lenderArr[i].nftNums,
                isLender: false,
                balance: 0
            });
            arr[i] = lender;
            if (_isLender(lender.addr)) {
    ...
    function getAllNFTOwners() public view returns(IWallet.UserInfo[] memory) {
        return IWallet(sysAddr.nftPool).getNFTOwners();
    }
...
contract RiskOrder {
    ...
    function initRiskLevel() private {
        RiskLevel memory rl1 = RiskLevel(700000e6,500000e6,300000e6,1050000e6);
        riskLevels.push(rl1);
        RiskLevel memory rl2 = RiskLevel(3500000e6,2100000e6,1050000e6,3500000e6);
        riskLevels.push(rl2);
        RiskLevel memory rl3 = RiskLevel(7000000e6,3500000e6,1400000e6,7000000e6);
        riskLevels.push(rl3);
    }
