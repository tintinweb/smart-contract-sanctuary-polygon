...
contract LuffyMarket is Ownable, ERC721Holder, ReentrancyGuard {
    ...
    function finishAuction(
        ...
        require(
            thisAuction.ownerNft == msg.sender,
            "You have no permission to finish this auction"
        );
        ...
        bytes32 byte32Message = keccak256(
            abi.encodePacked(
                ...
                uint8(0x01),
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(BUY_SALT, nftAddress, tokenId, thisAuction.ownerNft))
            )
        );
        bytes32 message = keccak256(abi.encodePacked(bytes32ToString(byte32Message)));
        bytes32 messageHash = ECDSA.toEthSignedMessageHash(message);
        recoveredAddress = ECDSA.recover(messageHash, signature);
        require(recoveredAddress == thisAuction.ownerNft, "Invalid signature");
        ...
        if (thisAuction.highestBidder != address(0) && thisAuction.highestBidPrice > 0) {
            if (thisAuction.tokenPayment == address(0)) {
                ...
                payable(msg.sender).transfer(thisAuction.highestBidPrice - royalFee - ethFee); //keep ethFee in marketplace
            } else {
                if (thisAuction.isBurn == true) {
                    ...
                    require(
                        token.transfer(msg.sender, (thisAuction.highestBidPrice - tokenFee)), //keeping tokenFee in Contract
                        "Transfer token to seller fail"
                    );
                }
            }
