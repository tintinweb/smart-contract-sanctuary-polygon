...
contract WPNS is ERC721,ERC721Enumerable,Ownable,VRFConsumerBase {
    ...
    /// @notice add weapon tier
    function deleteLastWeaponTier() external onlyAdmin {
        uint256 tierLvl_ = weaponTiers.length;
        require(weaponTiers[tierLvl_-1].mintIndex == 0,"DWT-01");
        weaponTiers.pop();
        ...
        onlyAdmin
    {
        ...
        require(weaponCount_ <= maxMintBatch,"CRE-10");
        ...
        require(weaponTiers[_tierLvl-1].mintIndex+weaponCount_ <= weaponTiers[_tierLvl-1].mintCap,"CRE-50");
        for (uint256 i = 0; i < weaponCount_; i++) {
            require(_recipient[i] != address(0),"CRE-60");
        }
        ...
        uint256 serialNo_ = weaponTiers[_tierLvl-1].mintIndex;
        address recipient_;
        for (uint256 i = 0; i < weaponCount_; i++) {
            tokenId_++;
            serialNo_++;
        ...
        weaponTier_.mintIndex += uint32(weaponCount_);
    }

    /// @notice Burn weapon token
    function burnWeapon(uint256 _tokenId)
        external
        onlyBurner
    {
        // !!! require allowance ???
        ...
        require(
            ...
            isApprovedForAll(tokenOwner_, msg.sender),"BRN-02");
        // !!!
    ...
    (
        ...
        private        
    {
        require(_sec <= MAX_ATTRIB_LVL && _ano <= MAX_ATTRIB_LVL && _inn <= MAX_ATTRIB_LVL, "ATR-01");
    ...
    function changeMintCap(uint256 _tierLvl, uint256 _newCap) external onlyOwner{
    ...
    function increaseWeaponTypeCount(uint256 _tierLvl) external onlyAdmin{
    ...
    function increaseWeaponSubtypeCount(uint256 _tierLvl) external onlyAdmin{
    ...
    function increaseWeaponCategoryCount(uint256 _tierLvl) external onlyAdmin{
    ...
    function toggleSafeMinting() external onlyOwner {
