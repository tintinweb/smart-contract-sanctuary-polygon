...
contract StrategyDualRewardsMaticX is BaseStrategyStakingRewards {
    ...
    address private constant STADER = 0x1d734A02eF1e1f5886e66b0673b71Af5B53ffA94; //STADER -> USDC -> MATIC
    ...
    constructor()
        ...
        BaseStrategyStakingRewards(
            0x1E16eCc4F912d8dB04b8177b4186bb597267fc25,
        ...
        )
    {
        ...
        a_b_path = [WMATIC, MATICx];
        reward_matic_path = [STADER, USDC, WMATIC];
        quick_matic_path = [QUICK, WMATIC];

        IERC20(WMATIC).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
        IERC20(MATICx).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
        IERC20(QUICK).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
        IERC20(STADER).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        uint256 _rewardBal = IERC20(STADER).balanceOf(address(this));
        if(_rewardBal > 0) {
            IERC20(STADER).safeTransfer(strategist, _rewardBal);
        }
        ...
        uint256 ldoHarvested = IERC20(STADER).balanceOf(address(this));
        if (ldoHarvested > 0) {
