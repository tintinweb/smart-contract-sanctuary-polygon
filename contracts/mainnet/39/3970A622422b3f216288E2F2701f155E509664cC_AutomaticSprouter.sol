...
contract AutomaticSprouter is Ownable, IERC165 {
    ...
    address public mojoSeedAddress;
    address public mojoAddress;
    address public sprouterAddress;

    constructor(
        address _owner,
        address _mojoSeedAddress,
        address _mojoAddress,
        address _sprouterAddress
    ) {
        _transferOwnership(_owner);

        mojoSeedAddress = _mojoSeedAddress;
        mojoAddress = _mojoAddress;
        sprouterAddress = _sprouterAddress;
    }

    function plant(uint256[] memory seedIds) public onlyOwner {
        MojoSeedERC721 mojoSeed = MojoSeedERC721(mojoSeedAddress);

        for (uint256 i = 0; i < seedIds.length; i++) {
            uint256 seedId = seedIds[i];

            mojoSeed.approveAndCall(sprouterAddress, seedId);
        }
    }

    function sprout(uint256[] memory seedIds) public onlyOwner {
        MojoERC721 mojo = MojoERC721(mojoAddress);
        Sprouter sprouter = Sprouter(sprouterAddress);

        for (uint256 i = 0; i < seedIds.length; i++) {
            uint256 seedId = seedIds[i];

            sprouter.sprout(seedId, address(this));
            assert(mojo.ownerOf(seedId) == address(this));
        }
    }
