...
// File: contracts/PreorderTest.sol
...
contract PreorderTest is ERC721, AccessControl {
  using Strings for uint256;
  bytes32 public constant ADMIN_ROLE = keccak256("ADMIN");

  //PRICING
  uint public preorderPrice = 0.019 ether;
  uint public mintingPrice =  0.029 ether;

  //STAGES CONFIG
  uint256 maxSupply = 10000;
  bool public hidden = false;
  bool public paused = false;
  bool public preorderFinished = false;

  //NFT CONFIG
  string public hiddenUrl = 'https://preorder-test.vercel.app/assets/metadata/hidden.json';
  string public currentBaseURI = 'https://preorder-test.vercel.app/assets/metadata/';
  string public baseExtension = ".json";

  mapping(uint => PreorderOwner) public preorders; // { tokenId => PreorderOwner }
  mapping(address => bool) public whitelist;

  constructor() ERC721("PreorderToken", "PRT") {
  ...
  function processPreorder(uint256 _tokenId) public payable {
    ...
    require(preorders[_tokenId].preordered != true, "Token already preordered");
    require(preorders[_tokenId].reserved != true, "Token already reserved");
    require(whitelist[msg.sender], "Not in whitelist");
    require(msg.value >= preorderPrice, "Not enough ether");


    preorders[_tokenId].owner = msg.sender;
    preorders[_tokenId].preordered = true;
  }

  function reserveToken(uint256 _tokenId) public payable {
    require(!paused, "Contract is temporary paused");
    ...
    preorders[_tokenId].owner = msg.sender;
    preorders[_tokenId].reserved = true;
  }
  //-----------------------------------------
  //-------------END PREORDER----------------
  //-----------------------------------------
  ...
  function mint(uint256 _tokenId) public payable {
    ...
    require(preorders[_tokenId].owner == msg.sender || preorders[_tokenId].owner == 0x0000000000000000000000000000000000000000, 'Token already booked');

    if(preorders[_tokenId].owner == msg.sender && preorders[_tokenId].preordered){
      require(msg.value == 0 ether, "Too much ether");
    }
    else {
...
function tokenURI(uint256 tokenId)
  ...
  function setCurrentBaseURI(string calldata value) public onlyAdmin{
    currentBaseURI = value;
  }
