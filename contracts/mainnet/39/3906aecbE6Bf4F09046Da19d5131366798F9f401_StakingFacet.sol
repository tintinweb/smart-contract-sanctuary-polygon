...
contract StakingFacet {
    ...
    event Staked(address indexed user, Nfts selectedNft, uint256 tokenId);
    event Unstaked(address indexed user, Nfts selectedNft, uint256 tokenId);
    ...
    function stake(Nfts _selectedNft, uint256 _tokenId) external {
        s.nftContract[_selectedNft].transferFrom(
            msg.sender,
            s.nftStorage,
            _tokenId
        );
        ...
        emit Staked(msg.sender, _selectedNft, _tokenId);
    }
    ...
    function unstake(Nfts _selectedNft, uint256 _tokenId) external {
        ...
        emit Unstaked(msg.sender, _selectedNft, _tokenId);
    }
