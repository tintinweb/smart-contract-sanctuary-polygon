...
contract CC_Vehicles is ERC721, ERC2981{
    ...
    function mintTo(address to, address royaltyReceiver, bytes memory nftRawData) public onlyOwnerOrAuthorizedUser{
        tokenIdCounter.increment();
        _mint(to, tokenIdCounter.current());
        (uint32 vehicleType) = abi.decode(nftRawData,(uint32));
    ...
    function initialMintBatch(uint256 count, address royaltyReceiver, bytes memory nftRawData) public onlyOwner{
        for(uint i=0;i<count;i++){
            mintTo(msg.sender, royaltyReceiver, nftRawData);
        }
    }
