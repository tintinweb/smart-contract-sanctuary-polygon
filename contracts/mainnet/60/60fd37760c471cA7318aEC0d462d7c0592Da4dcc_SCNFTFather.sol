...
contract NFT is Ownable, ERC721A, ReentrancyGuard, DefaultOperatorFilterer {

    using Strings for uint256;

    address public treasuryAddr;
    address public nftMinting;
    ...
    constructor(
        ...
        uint256 maxBatchSize_,
        uint256 collectionSize_
    ) ERC721A(nftName, nftSymbol, maxBatchSize_, collectionSize_) {
        treasuryAddr = msg.sender;
    }

    function setNftMinting(address _nftMinting) external onlyOwner {
        nftMinting = _nftMinting;
    }
    ...
    function setBaseURI(string calldata baseURI) external onlyOwner {
        require(mintFinished, "NFT: minting must be completed first");
    ...
    function buyWithCrossmint(uint256 _quantity, address _to, bytes32[] calldata _merkleProof) external payable {
        INFTMinting(nftMinting).buy{value:msg.value}(_quantity, _to, _merkleProof);
    }
...
contract SCNFTFather is Ownable {
    ...
    event ContractNFTCreated(address creator, address nftAddress);
    ...
    returns (address)
    {
        ...
        emit ContractNFTCreated(_msgSender(), address(nft));

        smartContracts.push(SmartContractsCreateds({
            smartContractAddress: address(nft),
