...
// File: contracts\DAOSwapPoly.sol
...
contract DAOHqSwapPoly is Ownable, ReentrancyGuard, DaoHqRouter {
    using SafeERC20 for IERC20;

    uint256 public fees;

    event MaticTransfer(address indexed from, address indexed to, uint256 value);
    ...
    function transfer(uint amount, address to) internal {
        ...
        emit MaticTransfer(address(this), to, amountToTransfer);
    }
    ...
    external nonReentrant returns (uint amountOut) {
        ...
        require(initialBalance + amountOut == address(this).balance, "Matic was not returned correctly");
    ...
    /**
     * @dev to recieve Matic from DaoHqRouter when swaping
     */
