...
abstract contract PrizePool is PrizePoolInterface, OwnableUpgradeable, ReentrancyGuardUpgradeable, TokenControllerInterface, IERC721ReceiverUpgradeable {
  ...
  /// Stores whitelisted users that are allowed to transfer S.P.H.E.R.E Games tokens.
  /// _whitelisted An array of addresses that are allowed to transfer S.P.H.E.R.E. Games tokens.
  ...
  /// Checks if an address is whitelisted and adds it to the whitelist array if not
  /// _address address to add to the whitelist
  /// return True if the user was added to the whitelist, false otherwise
  function addToWhitelist(address _address) external onlyOwner {
    require(_whitelisted[_address] == false, "Address is already whitelisted");
    _whitelisted[_address] = true;
  }

  /// Checks if an address is whitelisted and remove it from the whitelist array if the address is whitelisted
  /// _address The address to remove
  /// Return True if the user was removed from to the whitelist, false otherwise
