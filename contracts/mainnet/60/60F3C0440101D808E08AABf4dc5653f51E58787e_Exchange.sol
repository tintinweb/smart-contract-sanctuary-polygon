...
import {IERC2981} from "../lib/openzeppelin/contracts/interfaces/IERC2981.sol";
...
contract Exchange is Owners, ReentrancyGuard {
    ...
    function updateRoyalty(address collection, address setter, address recipient, uint fee) external {
      ...
      require(owners[msg.sender] || msg.sender == royaltiesSetters[collection] || msg.sender == collectionSetter);
        ...
        nonReentrant
    {
        ...
        // Execution part 1/2
        if (msg.value > 0) {
            require(makerAsk.currency == address(weth), "Order: ETH given but currency not WETH");
            require(msg.value == takerBid.price, "Order: ETH amount not equal to price");
    ...
    function matchAskWithTakerBidMulti(Orders.TakerOrder[] calldata takerBids, Orders.MakerOrder[] calldata makerAsks)
        external
        nonReentrant
    {
      require(takerBids.length == makerAsks.length, "not the same length");
      for (uint256 i = 0; i < takerBids.length; i++) {
          matchAskWithTakerBid(takerBids[i], makerAsks[i]);
      }
    }
...
abstract contract Owners {
    ...
    function setOwner(address owner, bool active) public virtual onlyOwner {
