...
contract AaveActions is Base {
    ...
    function healthFactorStatus() public view returns (bool needsRebalancing, uint256 factor, uint256 ethAmount) {
        ...
        factor = (healthFactor == type(uint256).max ? 1 : 1e18) * healthFactor / targetHealthFactor;
        needsRebalancing = factor > 1e18 + allowOverHealthFactor || factor < 1e18 - allowUnderHealthFactor; // ± 5%
    ...
    function setHealthFactorParameters(uint256 over, uint256 under) external onlyAuthorised {
        allowOverHealthFactor = over;
        allowUnderHealthFactor = under;
    }

    function setTargetHealthFactor(uint256 _targetHealthFactor) external onlyAuthorised {
...
contract AaveUniStrategy is UniswapActions, AaveActions {
    ...
    uint256 allowOverHedge = 2e16; // 1e16/1e18 ~ 1% // We are long.
    uint256 allowUnderHedge = 1e16; // 1e16/1e18 ~ 1% // We are short.
    ...
    function hedgeStatus() public view returns (bool needsRebalancing, uint256 factor, uint256 ethAmount) {
        ...
        needsRebalancing = factor > 1e18 + allowOverHedge || factor < 1e18 - allowUnderHedge; // ± 1%
        ethAmount = factor > 1e18 ? (available - debt) : (debt - available);
    }

    function setHedgeParameters(uint256 over, uint256 under) external onlyAuthorised {
        allowOverHedge = over;
    ...
    function maintainHedge(int24 priceTick, uint24 maxDifference) external onlyAuthorised returns (int256 liquidityChange, int256 collateralChange, int256 debtChange) {
        ...
        if (hedgeFactor > 1e18 && healthFactor > 1e18) { // borrow weth, sell to usdc, lp
        ...
        } else if (hedgeFactor > 1e18 && healthFactor <= 1e18) { // remove lp, sell weth, deposit usdc
        ...
        } else if (hedgeFactor < 1e18 && healthFactor > 1e18) { // withdraw usdc, buy weth, lp
        ...
        } else if (hedgeFactor < 1e18 && healthFactor <= 1e18) { // remove lp, buy weth, repay        
