...
/*
Badass Bunniez Smart Contract by Cahit Karahan
...
interface UtilityTokenIERC20 is IERC20, IERC20Metadata {
  ...
  function contractTransfer(address _from, address _to, uint256 _amount) external returns (bool);
}

contract BadassBunniez is ERC721, Ownable, ReentrancyGuard {
  int private supply;
  ...
  string public hiddenMetadataUri = "ipfs://QmWkGP99gpVmRysEVQ6QaZXLTxcYYmZF3t18xTtXpTXvVT";
  
  uint256 private basePoints = 1 ether;
  uint256 private _numHolders = 0;
  uint256 public cost = 1 ether;
  uint256 public maxSupply = 1111;
  uint256 public maxMintAmountPerTx = 10;
  uint256 public topTransferOfTokens = 0;
  uint256 public totalTransferOfTokens = 0;
  uint256 public burnMultiplier = 1111 ether;
  ...
  bool public holdScoreBasedDAO = true;
  ...
  event DividendsDistributed(uint256 _income, address indexed _from);
  event DividendsDistributedERC20(uint256 _income, IERC20 indexed _token, address indexed _from);
  event DividendsDistributedUtilityToken(uint256 _income, UtilityTokenIERC20 indexed _token, address indexed _from);
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    if (_msgSender() != owner()) {
      ...
      require(_msgValue() >= cost * _mintAmount, "Insufficient funds!");
    }
  ...
  function deposit() public payable {
    require(_msgValue() > 0, "The amount must be greater than 0!");
  }
  ...
  function distributeDividendsAll() public onlyOwned {
    uint256 totalAmount = address(this).balance;
    uint256 remaining = _distributeDividends(totalAmount, address(0), address(0));

    emit DividendsDistributed(totalAmount - remaining, address(this));
  }

  function distributeDividends(uint256 _amount) public onlyOwned {
    uint256 remaining = _distributeDividends(_amount, address(0), address(0));

    emit DividendsDistributed(_amount - remaining, address(this));
  }

  function distributeFromUser() public payable {
    uint256 totalAmount = _msgValue();
    uint256 remaining = _distributeDividends(totalAmount, address(0), _msgSender());

    emit DividendsDistributed(totalAmount - remaining, _msgSender());
  }

  function distributeDividendsAll(address _token) public onlyOwned {
    require(_token.code.length > 0, "The address is not a contract!");
    uint256 totalAmount = IERC20(_token).balanceOf(address(this));
    uint256 remaining = _distributeDividends(totalAmount, _token, address(0));

    emit DividendsDistributedERC20(totalAmount - remaining, IERC20(_token), address(this));
  }

  function distributeDividends(address _token, uint256 _amount) public onlyOwned {
    require(_token.code.length > 0, "The address is not a contract!");
    uint256 remaining = _distributeDividends(_amount, _token, address(0));

    emit DividendsDistributedERC20(_amount - remaining, IERC20(_token), address(this));
  }

  function distributeFromUser(address _token, uint256 _amount) public {
    require(_token.code.length > 0, "The address is not a contract!");
    uint256 totalAmount = _amount;
    require(IERC20(_token).balanceOf(_msgSender()) >= _amount, "You don't have enough tokens!");
    require(IERC20(_token).allowance(_msgSender(), address(this)) >= _amount, "Not enough allowance! Use approve method of the token contract!");
    uint256 remaining = _distributeDividends(totalAmount, _token, _msgSender());

    emit DividendsDistributedERC20(totalAmount - remaining, IERC20(_token), _msgSender());
  }

  function distributeDividendsUtilityTokenMint(uint256 _totalAmount) public onlyOwned {
    uint256 remaining = _distributeDividends(_totalAmount, utilityToken, address(0));

    emit DividendsDistributedUtilityToken(_totalAmount - remaining, UtilityTokenIERC20(utilityToken), address(this));
  }

  function distributeFromUserUtilityToken(uint256 _totalAmount) public {
    require(IERC20(utilityToken).balanceOf(_msgSender()) >= _totalAmount, "You don't have enough tokens!");
    uint256 remaining = _distributeDividends(_totalAmount, utilityToken, _msgSender());

    emit DividendsDistributedUtilityToken(_totalAmount - remaining, UtilityTokenIERC20(utilityToken), _msgSender());
  }

  function _distributeDividends(uint256 _totalAmount, address _token, address _user) internal nonReentrant returns (uint256) {
      ...
      if (amount <= remaining && amount > 0) {
        if (_token == address(0)) {
          ...
          if(_user == address(0)) {
            ...
            res = UtilityTokenIERC20(utilityToken).contractTransfer(_user, owner, amount);
          }
        } else {
          if(_user == address(0)) {
            res = IERC20(_token).transfer(owner, amount);
          } else {
            res = IERC20(_token).transferFrom(_user, owner, amount);
          }
        }
        if (res) {
          remaining -= amount;
