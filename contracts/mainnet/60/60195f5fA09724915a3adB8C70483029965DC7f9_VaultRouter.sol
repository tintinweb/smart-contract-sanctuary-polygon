    ...
    ReentrancyGuardUpgradeable
{
    ...
    function updateStakeToken(address _stakeToken) external onlyAdmin {
        stakeToken = _stakeToken;
    }

    function updatecustomVaultAddress(
        address _customVaultAddress
    ) external onlyAdmin {
        customVaultAddress = _customVaultAddress;
    }

    function updateBBVault(address _bbVault) external onlyAdmin {
        bbVault = _bbVault;
    }

    function updatexToken(address _xToken) external onlyAdmin {
        xToken = _xToken;
    }

    function updateGauge(address _gauge) external onlyAdmin {
        gauge = _gauge;
    }

    function unstakeBuyBackTokenWithPermit(
        IMetaTx.MetaTxPermitArgs memory _metaTxArgsUnStake,
        address _bbToken,
        uint256 _amount,
        address _tokenOut,
        ...
        IMetaTx(_bbToken).permit(
            _metaTxArgsUnStake.owner,
            _metaTxArgsUnStake.spender,
            _metaTxArgsUnStake.value,
            _metaTxArgsUnStake.deadline,
            _metaTxArgsUnStake.v,
            _metaTxArgsUnStake.r,
            _metaTxArgsUnStake.s
        );
        unstakeBuyBackToken(
            _bbToken,
            _amount,
    ...
    function unstakeBuyBackToken(
        ...
        uint256 receivedTOkenOut = IERC4626StakingPool(_customVault).withdraw(
            receiveTokens / (10 ** (18 - IERC20Metadata(_tokenOut).decimals())),
            address(this)
        );
