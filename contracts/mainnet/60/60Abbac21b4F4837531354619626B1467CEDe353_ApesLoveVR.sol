...
contract ApesLoveVR is ERC721 {
    uint256 public totalSupply;
    uint256 public cost = 0.2 ether;
    uint256 public maxMints = 35;
    ...
    constructor()
        ERC721("Apes Love VR", "ALV")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 35) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeialsz6357r4etydrdmeslyr3ftgxjacuxfupit2bwxjcamj32j3w4/", Strings.toString(tokenId), ".json")
            );
    }
