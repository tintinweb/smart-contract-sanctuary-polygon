...
contract Trading is MetaContext, ITrading {
    ...
    uint public liqPercent = 8000; // Default 80%
    uint public maxWinPercent = 100000;
    ...
    mapping(uint => uint) private blockDelayPassed; // id => block.number
    uint private blockDelay;
        ...
        external
    {
        ...
        if (trade.direction && trade.orderType == 1) {
            require(trade.price >= _price, "BuyLimit");
        } else if (!trade.direction && trade.orderType == 1) {
            require(trade.price <= _price, "SellLimit");      
        } else if (!trade.direction && trade.orderType == 2) {
            require(trade.price >= _price, "SellStop");
        } else {
            require(trade.price <= _price, "BuyStop");
        }
    ...
    function getLiqPrice(uint _id) external view returns (uint256) {
        IPosition.Trade memory _trade = position.trades(_id);
        return TradingLibrary.liqPrice(_trade.direction, _trade.price, _trade.leverage, _trade.margin, _trade.accInterest, liqPercent);
    }
...
library TradingLibrary {
    ...
    function liqPrice(bool _direction, uint _tradePrice, uint _leverage, uint _margin, int _accInterest, uint _liqPercent) external pure returns (uint256 _liqPrice) {
        if (_direction) {
