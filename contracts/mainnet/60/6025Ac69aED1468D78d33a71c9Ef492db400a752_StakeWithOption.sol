...
contract StakeWithOption is Ownable, ReentrancyGuard {
    ...
    * if the pool owner withdrawn the pool, the pool status will be set to Finished
    * and normal user should withdraw token and usdt in the `withdrawToken` function
    */
    ...
    /**
    * if active and after unlock time, or the pool is stopped or finished,
    * a user can withdraw all tokens at any condition
    */
    function withdrawToken(uint256 poolId) nonReentrant public {
        if (pools[poolId].status == PoolStatus.Active) {
            _ensurePoolUnlock(poolId);
        }
