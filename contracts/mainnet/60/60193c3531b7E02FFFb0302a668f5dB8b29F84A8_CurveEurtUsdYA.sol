...
contract CurveEurtUsdYA is BaseYA, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  uint256 constant private COMPOUND_USDC_THRESHOLD = 1e6;
  ...
  function depositGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    value = GiddyUniswapV2.swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, address(this), value);
  ...
  function withdrawGiddyCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    (amountOut, priceImpact) = GiddyUniswapV2.calcPriceSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, amountOut);
    amountOut -= fap;
  }
  ...
  function withdrawGiddy(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    uint256 value = leave(shares);
    value = GiddyUniswapV2.swapTokensSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, address(this), value);
  ...
  function postOwnerUpgrade() public onlyOwner {
    uint256 balance = IERC20(CURVE_EURT_3CRV_TOKEN).balanceOf(address(this));
    if (!IERC20(CURVE_EURT_3CRV_TOKEN).approve(REWARD_POOL, balance)) {
      revert();
    }
  ...
  function deductFeeGiddy(uint256 amount, uint256 fap) private returns (uint256) {
    if (fap > 0) {
      GiddyUniswapV2.swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, feeAccount, fap);
    }
    return amount - fap;
  }
  ...
  modifier deductEarningsFee() {
    ...
    if (earnings > 0) {
      ...
      if (value >= COMPOUND_CRV_THRESHOLD) {
        ...
        route[1] = WETH_TOKEN;
        route[1] = USDC_TOKEN;
        value = GiddyUniswapV2.swapTokens(SUSHI_ROUTER, route, address(this), value);
