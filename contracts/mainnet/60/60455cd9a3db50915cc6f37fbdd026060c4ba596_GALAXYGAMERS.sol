...
// File: contracts/GalaxyGamers.sol
...
contract GALAXYGAMERS is ERC721A, ERC2981, RevokableDefaultOperatorFilterer, Ownable, ReentrancyGuard, PaymentSplitter {   

    using Strings for uint; // Pour la fx qui retourne l'URI d'un token
    uint256 public supply = 2500; // Supply total
    ...
    event newMint(address indexed owner, uint256 startId, uint256 number); // Event de mint

    constructor(string memory defaultUri, address[] memory teamMembers, uint[] memory teamShares) ERC721A("Galaxy Gamers", "GG")
