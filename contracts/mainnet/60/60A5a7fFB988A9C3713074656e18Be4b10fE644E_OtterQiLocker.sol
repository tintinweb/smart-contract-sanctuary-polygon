...
import '../interfaces/IQiFarm2.sol';
...
contract OtterQiLocker is LockerOwnedUpgradeable, UUPSUpgradeable, IStrategy {
    ...
    function convertTetuQiToQi(
        uint256 amountIn_,
        uint256 amountOutMin_,
        bool xTetu
    ) external onlyLocker {
        if (xTetu) {
            treasury.manage(address(xTetuQi), amountIn_);
            // xTetuQi -> tetuQi
            xTetuQi.withdraw(amountIn_);
        } else {
            treasury.manage(address(tetuQi), amountIn_);
        }
        address[] memory tetuQiPath = new address[](2);
        tetuQiPath[0] = address(tetuQi);
        tetuQiPath[1] = address(qi);
        tetuQi.approve(address(tetuRouter), amountIn_);
        uint256[] memory swapResult = tetuRouter.swapExactTokensForTokens(
            amountIn_,
            amountOutMin_,
            tetuQiPath,
            address(this),
        ...
        uint256 qiAmount = swapResult[1];
        qi.transfer(address(treasury), qiAmount);
        emit ConvertToQi(address(tetuQi), amountIn_, qiAmount);
    }

    // ===== Qi Staking ====

    address public constant qiFarm = 0x574Fe4E8120C4Da1741b5Fd45584de7A5b521F0F;

    /// @notice Stake lp to Qi farm
    /// @param amount_ the amount of lp
    function stake(uint256 pid_, uint256 amount_) public onlyLocker {
        IERC20 lp = IQiFarm(qiFarm).poolInfo(pid_).lpToken;

        treasury.manage(address(lp), amount_);
        lp.approve(address(qiFarm), amount_);
        IQiFarm(qiFarm).deposit(pid_, amount_);
    ...
    function unstake(uint256 pid_, uint256 amount_) external onlyLocker {
        IERC20 lp = IQiFarm(qiFarm).poolInfo(pid_).lpToken;
        IQiFarm(qiFarm).withdraw(pid_, amount_);
    ...
    function harvestStaking(uint256 pid_) internal returns (uint256) {
        IQiFarm(qiFarm).deposit(pid_, 0);
        uint256 amount = qi.balanceOf(address(this));
        // qi.transfer(address(treasury), amount);
        ...
        returns (uint256)
    {
        ...
        uint256 reinvestAmount = (qiAmount * reinvestRate) / 10000;
        uint256 rewardAmount = qiAmount - reinvestAmount;
        ...
        if (reinvestAmount > 0) {
            ...
            emit Invest(amountsOut[2]);
        }

        // swap Qi -> USDC
        qi.approve(QuickSwapRouter, rewardAmount);
    ...
    /// @notice Reinvest Qi into Qi/WMATIC
    /// @param amount_ the reinvest amount of Qi
    function reinvest(uint256 amount_) internal returns (uint256) {
        IERC20 matic = IERC20(WMATIC);

        IUniswapV2Router02 quickRouter = IUniswapV2Router02(QuickSwapRouter);

        // Swap half Qi -> WMATIC
        uint256 amountIn = amount_ / 2;
        qi.approve(QuickSwapRouter, amountIn);
        address[] memory path = new address[](2);
        path[0] = address(address(qi));
        path[1] = address(address(matic));
        ...
        IQiFarm(qiFarm).deposit(4, lpAmount);
