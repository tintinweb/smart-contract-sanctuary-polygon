...
contract MemoCreatorKeyMetadataGeneratorV1 is IDynamicMetadata, Ownable {
    ...
    mapping(uint16 => string) private _imageBaseUrls; //version to baseUrls

    mapping(uint16 => uint16) private _attrCounts;

    constructor() {
        //_imageBaseUrl = imageBaseUrl_;
    }

    function setImageBaseUrl(uint16 version_, string memory imageBaseUrl_)
        external
        onlyOwner
    {
        _imageBaseUrls[version_] = imageBaseUrl_;
    }

    function getImageBaseUrl(uint16 version_)
        public
        view
        returns (string memory)
    {
        return _imageBaseUrls[version_];
    }

    function setAttrCounts(uint16 version_, uint16 attrCount_)
        external
        onlyOwner
    {
        _attrCounts[version_] = attrCount_;
    }

    function getAttrCounts(uint16 version_) public view returns (uint16) {
        return _attrCounts[version_];
    }
    ...
    function tokenMetadata(
        ...
        require(
            bytes(_imageBaseUrls[creatorKeyNFT._version]).length > 0,
            "image url not set"
        );
        ...
        string memory base64Json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        _getJson(tokenId, creatorKeyNFT, creatorkeyAttributes)
                    )
                )
            )
        );
    ...
    function _getJson(
        ...
        string memory nftName = string(
            abi.encodePacked(creatorKeyNFT._creatorName)
        );

        /** Description string */
        bytes memory descString;
        descString = abi.encodePacked(
            descString,
            abi.encodePacked(
                "MemoWorld Creator Key Holders can create NFT collections in memoworld.xyz"
            )
        );
        ...
        imageString = string(
            abi.encodePacked(
                _imageBaseUrls[creatorKeyNFT._version],
        ...
        */
        ...
        if (creatorkeyAttributes._isInited) {
            if (bytes(creatorkeyAttributes._imageUrl).length > 0) {
                imageString = creatorkeyAttributes._imageUrl;
            }
        }
        ...
        ERC721MetadataStructure memory metadata = ERC721MetadataStructure({
            ...
            image: imageString,
            animation_url: imageString,
            external_link: "https://memoworld.xyz",
            image_url: string(imageString),
            keyStatus: Strings.toString(creatorKeyNFT._passStatus),
            keyLevel: Strings.toString(creatorKeyNFT._passLevel),
            keyVersion: Strings.toString(creatorKeyNFT._version),
            attributes: _getJsonAttributes(
                tokenId,
                creatorKeyNFT,
                creatorkeyAttributes
            )
        });
...
*/
    function _getJsonAttributes(
        ...
        uint256 attrSize = _attrCounts[creatorKeyNFT._version];
        ...
        ERC721MetadataAttribute[]
            memory metadataAttributes = new ERC721MetadataAttribute[](
                3 + attrSize
            );
        ...
        metadataAttributes[0] = _getERC721MetadataAttribute(
            ...
            "",
            "Level",
            creatorKeyNFT._passLevel == 99
                ? "Genesis"
                : Strings.toString(creatorKeyNFT._passLevel),
            ""
        );
        ...
        metadataAttributes[1] = _getERC721MetadataAttribute(
            ...
            "",
            "Version",
            Strings.toString(creatorKeyNFT._version),
            ""
        );
        ...
        metadataAttributes[2] = _getERC721MetadataAttribute(
            ...
            "",
            "Status",
            creatorKeyNFT._passStatus == 1 ? "Activated" : "Unactivated",
            ""
        );

        if (creatorkeyAttributes._isInited) {
            for (uint8 i = 0; i < attrSize; i++) {
                metadataAttributes[i + 3] = _getERC721MetadataAttribute(
                    false,
                    true,
                    true,
                    false,
                    "",
                    creatorkeyAttributes._attrNames[i],
                    creatorkeyAttributes._attrValues[i],
                    ""
                );
            }
        }
