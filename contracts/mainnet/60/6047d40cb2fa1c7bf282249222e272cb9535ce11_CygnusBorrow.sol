...
contract CygnusBorrowControl is ICygnusBorrowControl, CygnusTerminal {
     ...
     *  @inheritdoc ICygnusBorrowControl
     */
    uint256 public override reserveFactor = 0; // Default is reserve-less
...
contract CygnusBorrowModel is ICygnusBorrowModel, CygnusBorrowControl {
     ...
     */
    mapping(address => mapping(address => BorrowSnapshot)) internal borrowBalances;
    ...
    function getBorrowBalance(
        ...
        BorrowSnapshot storage borrowSnapshot = borrowBalances[collateral][borrower];
    ...
    function _updateBorrow(
        ...
        // Track borrower
        _trackRewards(borrower, borrowSnapshot.principal, collateral);
    }
     ...
     */
    function _trackRewards(address account, uint256 balance, address collateral) internal {
     ...
     *  @inheritdoc ICygnusBorrowModel
     */
    function trackBorrower(address collateral, address borrower) external override {
        // Latest borrow balance (with interest)
        (uint256 principal, ) = getBorrowBalance(collateral, borrower);

        // Pass borrower info to the Rewarder (if any)
        _trackRewards(borrower, principal, collateral);
    }
    ...
    function trackLender(address lender) external override {
        ...
        // If user transfered CygUSD tokens to another account, then the _afterTokenTransfer hook tracks the sender and receiver
        // so lender trackings are always accurate.
        _trackRewards(lender, balance, address(0));
    }
}
...
contract CygnusBorrowVoid is ICygnusBorrowVoid, CygnusBorrowModel {
     ...
     *  @custom:security only-admin 👽
     */
    function addBonusTokenHarvester(address _reward) external override cygnusAdmin {
        /// @custom:error TokenIsUnderlying Harvester cannot move underlying
        if (_reward == underlying || _reward == address(COMET_USDC)) revert CygnusBorrowVoid__TokenIsUnderlying();

        // Allow harvester to access an extra reward token
        approveTokenPrivate(_reward, harvester, type(uint256).max);

        /// @custom:error NewBonusHarvesterToken
        emit NewBonusHarvesterToken(_reward, harvester);
    }
}
...
interface ICygnusBorrowVoid is ICygnusBorrowModel {
    ...
    /**
     *  @dev Reverts if the token we are allowing the harvester is underlying
     *
    ...
    /**
     *  @notice Admin 👽
     *  @notice Manually allow the harvester to access a reward token (reverts if _reward is underlying)
     *
     *  @param _reward The address of the token we are allowing the harvester to move
     *
     *  @custom:security only-admin
     */
    function addBonusTokenHarvester(address _reward) external;
}
