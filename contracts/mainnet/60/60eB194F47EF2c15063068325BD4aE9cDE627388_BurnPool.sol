...
contract BurnPool is Ownable {
    ...
    constructor(IERC20 _GREEN, IERC20 _REWARD, uint256 _rewardPerBlock, uint256 _startBlock, uint256 _lockBlock, uint256 _endBlock) public {
    ...
    /**
     * @dev Deposit GREEN tokens to Faucet for rewards allocation and/or withdraw outstanding rewards.
     * @param _amount Amount of GREEN tokens to deposit.
     */
    function transact(uint256 _amount) public {
        ...
        if (block.number < lockBlock && _amount != 0) {
            GREEN.safeTransferFrom(address(msg.sender), address(this), _amount);
            stakedGreen = stakedGreen.add(_amount);
            user.amount = user.amount.add(_amount);
            user.rewardDebt = user.amount.mul(poolInfo.accRewardPerShare).div(1e12);
            emit Deposit(msg.sender, _amount);
        }
        if (block.number >= endBlock) {
            GREEN.safeTransfer(burnAddress, user.amount);
