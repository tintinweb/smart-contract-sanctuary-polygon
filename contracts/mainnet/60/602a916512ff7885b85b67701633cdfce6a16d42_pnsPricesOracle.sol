...
contract pnsPricesOracle is PnsAddressesImplementation {
    ...
    constructor(address addresses_,address maticOracleAddress_,address ethOracleAddress_, uint256[] memory costs_) PnsAddressesImplementation(addresses_) {
        maticPriceFeed = AggregatorV3Interface(maticOracleAddress_);
        ethPriceFeed = AggregatorV3Interface(ethOracleAddress_);
        usdCosts = costs_;
    }

    function setMaticOracle(address oracleAddress) public {
        require(msg.sender == owner(), "Not authorized.");
        maticPriceFeed = AggregatorV3Interface(oracleAddress);
    }
    ...
    function getMaticCost(string memory _name, uint256 expiration) public view returns (uint256) {
        uint256 _usdCost = getUsdCost(_name);
        uint256 _maticPrice = uint256(getMaticPrice());
        uint256 cost = _usdCost * ( 1e18 / _maticPrice ) * expiration * 1e8;
        return cost;
    }
