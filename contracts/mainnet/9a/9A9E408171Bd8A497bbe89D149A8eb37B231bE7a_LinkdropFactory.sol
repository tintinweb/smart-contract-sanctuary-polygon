...
contract LinkdropFactoryCommon is LinkdropFactoryStorage {
    ...
    * @param _campaignId Campaign id
    * @param _transferPattern which pattern the campaign will use (mint on claim, transfer pre-minted tokens, etc)
    * @return proxy Proxy contract address
    */
    function deployProxy(uint _campaignId, uint _transferPattern)
    public
    payable
    returns (address payable proxy)
    {
      proxy = _deployProxy(msg.sender, _campaignId, _transferPattern);
    }
    ...
    * @return proxy Proxy contract address
    */
    function deployProxyWithSigner(uint _campaignId, address _signer, uint _transferPattern)
    public
    payable
    returns (address payable proxy)
    {
    ...
    * @return proxy Proxy contract address
    */
    function _deployProxy(address payable _linkdropMaster, uint _campaignId, uint _transferPattern)
    internal
    returns (address payable proxy)
    {
