...
contract RUNA is Context, IERC20, Ownable {
    ...
    string private _name = "Runa";
    string private _symbol = "XR";
    uint8 private _decimals = 18;
    uint256 public _bReward = 0; 
    uint256 public _bDevfee = 0; 
    uint256 public _bRfuel = 50;
    uint256 public _sReward = 0; 
    uint256 public _sDevfee = 0; 
    uint256 public _sRfuel = 80; 
    ...
    uint256 public DeFiBoost = 500;
    
    address public _DeFi = payable(0x81723063eaD5c32C64ED419a95dc916D6B71AaDA); 
    address public _DeFiCore = payable(0x81723063eaD5c32C64ED419a95dc916D6B71AaDA); 
    ...
    modifier onlyDev() {
        require(_DeFi == _msgSender(), "Caller is not the Dev");
        _;
    }
    ...
    function excludeFromReward(address account) public onlyDev() {
    ...
    function includeInReward(address account) external onlyDev() {
        require(_excludedFromReward[account], "Account is already excluded");
    ...
    function set_DeFi(address newDeFi) external onlyDev {
        require(newDeFi != address(0), "DeFi can not be a zero address");
        _DeFi = (newDeFi);
    }
   
    function set_DeFiCore(address newDeFiCore) external onlyDev {
        require(newDeFiCore != address(0), "DeFiCore can not be a zero address");
        _DeFiCore = (newDeFiCore);
    }
    
    function setDeFiBoost(uint256 Inject_Number) external onlyDev() {
       DeFiBoost = Inject_Number;
       numTokensSellToAddToLiquidity = DeFiBoost * 10**18;
   }
    
    function setBuyFeePercent(uint256 reflect, uint256 dev, uint256 liquid) public onlyDev {
        require(reflect.add(dev).add(liquid) <= 100, "Fee limitation is 10%");
        _bReward = reflect;
        _bDevfee = dev;
        _bRfuel = liquid;
    }

    function setSellFeePercent(uint256 reflect, uint256 dev, uint256 liquid) public onlyDev {
    ...
    function withdrawMatic(uint256 amount) public onlyDev() {
        if(amount == 0) payable(_DeFi).transfer(address(this).balance);
        else payable(_DeFi).transfer(amount);
    }
    
    function withdrawToken(address _tokenContract, uint256 _amount) external onlyDev {
    IERC20 tokenContract = IERC20(_tokenContract);
    tokenContract.approve(address(this), _amount);
    tokenContract.transferFrom(address(this), _DeFi, _amount);
}
    ...
    function _transfer(
     ...
     // Divine balance into two parts, manually add to liquidity with left over tokens
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
         uint256 tokenAmount = contractTokenBalance.mul(2).div(3); // tokens  div 3 portions, swap 2 leave 1 behide
                uint256 slices = contractTokenBalance.div(3); // 1 portion of token left over for automatic liquidity 
                     uint256 initialBalance = address(this).balance; // total balance 
                         swapTokensForEth(tokenAmount); // swap 2 portions
                              uint256 newBalance = address(this).balance.sub(initialBalance); // new swap balance 
                           uint256 pizza = newBalance.div(2); // div 2 portions
                       addLiquidity(slices, pizza); // add 1 portion of token and 1 portion of blance
                   uint256 pizzaslices = address(this).balance.sub(pizza); // 1 portion of blance left over
              payable(_DeFi).transfer(pizzaslices); // send to marketing wallet for manually liquidity and other promotion 
        emit SwapAndLiquify(tokenAmount, pizza, slices);
    }
