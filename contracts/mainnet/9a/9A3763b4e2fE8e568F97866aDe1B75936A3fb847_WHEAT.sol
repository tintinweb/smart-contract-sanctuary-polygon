...
contract WHEAT is ERC20, Ownable, ReentrancyGuard {
    ERC20 public constant token_USDC = ERC20(0x1168Ac314Ef5AB1D0F3fB89Fd110564f237FF22C);
    ...
    function set_lottery_share_percentage(uint _lottery_share_percentage) external onlyOwner() {
        // Max 2%
        require(_lottery_share_percentage <= 200);
        emit LotteryShareSet(lottery_share_percentage, _lottery_share_percentage);

        lottery_share_percentage = _lottery_share_percentage;
    }

    // 0.5% of lobby entried goto lottery pot
    uint public lottery_share_percentage = 50;
    ...
    function set_lastLobbyPool(uint _lastLobbyPool) external onlyOwner() {
        require(_lastLobbyPool > lastLobbyPool, "Invalid value");
        ...
        if (currentDay != _currentDay) {
            if (currentDay < dividendsPoolCapDays) {
                for (uint _day = currentDay + 1; _day <= (currentDay * 2 + 1); _day++) {
                    dayUSDCPool[_day] += currentDay > 0 ? (lobbyEntry[currentDay] * percentOfLobbyToBePooled) / (currentDay * 10000) : 0;
                }
            } else {
            ...
            // total of 10.5% from every day's lobby entry goes to:
    ...
    function _clcNFTBoost(uint amount, uint ability /* basis points 1e4 */) internal pure returns (uint) {
        return (amount * (1e12 * 110 / 100 + (((1e12  * ability) / 100) / 1e4))) / 1e12;
    }
    ...
    function DoEndStake(uint stakeId, bool doNotSendDivs) internal {
        ...
        if (stakeReturn != 0) {
            ...
            overall_collectedBonusTokens += getNFTRoostingBoostedAmount(bonusAmount);

            uint endStakeWHEATMintAmount = getNFTRoostingBoostedAmount(stakeReturn + bonusAmount);

            _mint(msg.sender, endStakeWHEATMintAmount);

            if (nftMasterChefAddress != address(0) && endStakeWHEATMintAmount > 0 && masterchefWHEATShare > 0)
                _mint(nftMasterChefAddress, (endStakeWHEATMintAmount * masterchefWHEATShare) /10000);
            if (daoAddress != address(0) && endStakeWHEATMintAmount > 0 && daoWHEATShare > 0)
                _mint(daoAddress, (endStakeWHEATMintAmount * daoWHEATShare) /10000);
        }
...
contract FarmerLandNFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint public constant MAX_LEVEL = 150;
    uint public constant MAX_ABILITY = 100e4;
    ...
    constructor(uint  _startTime, uint _nftType, uint _MAX_ELEMENTS, WHEAT _WHEAT, string memory name1, string memory name2) ERC721(name1, name2) {
