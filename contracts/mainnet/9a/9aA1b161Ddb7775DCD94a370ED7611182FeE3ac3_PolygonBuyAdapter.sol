 ...
 */
contract PolygonBuyAdapter is IBuyAdapter, NewUniswapV2, UniswapV3, AugustusRFQ {
...
abstract contract AugustusRFQ is WethProvider {
    ...
    struct AugustusRFQData {
        IAugustusRFQ.Order order;
        bytes signature;
        bytes makerAssetPermit;
    }
    ...
    function swapOnAugustusRFQ(
        ...
        address userAddress = address(uint160(data.order.nonceAndMeta));
...
interface IAugustusRFQ {
    ...
    function batchFillOrderWithTargetNFT(OrderInfoNFT[] calldata orderInfos, address target) external;
}