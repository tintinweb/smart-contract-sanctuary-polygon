...
contract BalancerStrategy is InitializableAbstractStrategy, BalancerExchange, UniswapV2Exchange {
    ...
    function initialize(
        ...
        maticXToken = IERC20(_rewardTokenAddresses[3]);

        super._initialize(_platformAddress, _vaultAddress, _rewardTokenAddresses, _assets, _pTokens);
    }

    /**
     * @dev function to set requisites required for swapping, setting balancer vault & uniswap router
     */
    ...
    /**
     * @dev function to _stake LP
     * @param _amount lp to stake
     */
    ...
    /**
     * @dev function to arrange LP;
     *      if not available then withdraw from gauge
     * @param _lp lp to get
     */
    ...
    function deposit(address _asset, uint256 _amount) external override onlyVaultOrGovernor nonReentrant {
        ...
        console.log("staking", lp.balanceOf(address(this)));
    ...
    function _withdraw(address _beneficiary, address _asset, uint256 _amount) internal {
        ...
        uint256 _lpBalance = lpBalance();
        uint256 minBpt = lp.getMinimumBpt();
        uint256 lpToWithdraw = _tokensToLP(_amount);

        uint256 closeAmount = ((_lpBalance * 90) / 100);
        if (lpToWithdraw >= closeAmount || _lpBalance.sub(lpToWithdraw) <= minBpt) {
            _arrangeLp(_lpBalance);
            _withdrawAll();
        } else {
        ...
        _sendBack(_beneficiary, token0.balanceOf(address(this)));
    ...
    /**
     * query pool to calculate LP
     * @param _amount amount to convert
     * @return _lp lp can get
     */
    ...
    function _checkBalance() internal view returns (uint256) {
        ...
        uint256 _lpToToken0 = lpBalance().mul(lp.getRate()).div(1e18);
        uint256 _swapFee = _lpToToken0.mul(lp.getSwapFeePercentage()).div(1e18);
        return token0Balance.add(_lpToToken0).sub(_swapFee).subBasisPoints(80);
    }
    ...
    /**
    @dev Function to send balance of token0 to _ben
     */
    ...
    function _collectRewards() internal {
        ...
        if (_staderBalance > 1e7) {
            ...
            total += batchSwap(
                ...
                IAsset(address(staderToken)),
                IAsset(address(maticXToken)),
