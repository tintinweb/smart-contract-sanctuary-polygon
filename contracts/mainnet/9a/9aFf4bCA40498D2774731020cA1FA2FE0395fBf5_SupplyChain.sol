...
contract SupplyChain is Ownable {
    ...
    function addCompany(string memory _name, string memory _domain, uint _industry) external payable {
        ...
        for (uint i = 0; i < shareholders.length; i++) {
            payable(shareholders[i].addr).transfer(msg.value * shareholders[i].shares / 100);
        }
    ...
    function setShareholders(address[] memory _shareholders, uint[] memory _shares) external onlyOwner {
        delete shareholders;
        for (uint i = 0; i < _shareholders.length; i++) {
            shareholders.push(Shareholder({
                addr: _shareholders[i],
                shares: _shares[i]
            }));
        }
    }
