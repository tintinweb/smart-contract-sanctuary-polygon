...
contract UserProxyInterface {
    ...
    function setZapperContract(address _zapperContract) public {
        require(zapperContract == address(0), "Already set");
     ...
     */
    function depositLpAndStake(address dystPoolAddress, uint256 amount) public {
        // Fetch user proxy
        IUserProxy userProxy = createAndGetUserProxy();
        address userProxyOwnerAddress = msg.sender;
    ...
    function depositLpAndStakeFromZapper(
        address dystPoolAddress,
        address recipient,
        uint256 amount
    ) external {
        require(msg.sender == zapperContract, "Only accessible by zapper");
        // Fetch user proxy
        IUserProxy userProxy = createAndGetUserProxy(recipient);
        address userProxyOwnerAddress = recipient;

        // Receive LP from UserProxy owner
        IERC20(dystPoolAddress).transferFrom(msg.sender, address(this), amount);
    ...
    function _amountStaked(address dystPoolAddress) internal returns (uint256) {
        // Fetch UserProxy
        IUserProxy userProxy = createAndGetUserProxy();
    ...
    function withdrawLp(address dystPoolAddress) external {
        ...
        uint256 amount = IERC20(penPoolAddress).balanceOf(
            userProxyOwnerAddress
        );
    ...
    function unstakePenLp(address penPoolAddress) public {
        ...
        address stakingAddress = penLens.stakingRewardsByPenPool(
            penPoolAddress
        );
        uint256 amount = IERC20(stakingAddress).balanceOf(address(userProxy));
    ...
    function createAndGetUserProxy(address user) internal returns (IUserProxy) {
        return
            IUserProxy(
                IUserProxyFactory(userProxyFactoryAddress)
                    .createAndGetUserProxy(user)
            );
    }
