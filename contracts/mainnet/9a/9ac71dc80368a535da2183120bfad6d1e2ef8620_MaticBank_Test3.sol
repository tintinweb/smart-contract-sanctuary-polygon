...
contract MaticBank_Test3 {
	using SafeMath for uint256;

	uint256 constant public INVEST_MIN_AMOUNT = 0.1 ether;
	uint256[] public REFERRAL_PERCENTS = [80, 40, 20];
	uint256 constant public ProjectFee1 = 250; 
	uint256 constant public ProjectFee2 = 250; 
	uint256 constant public ProjectFee3 = 250; 
	...
	event FeePayed(address indexed user, uint256 totalAmount);
		...
		constructor(address payable wallet, address payable wallet2, address payable wallet3) public {
		...
		commissionWallet = wallet2;
		commissionWallet = wallet3;
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		uint256 fee = msg.value.mul(ProjectFee1).div(PERCENTS_DIVIDER);
		fee = msg.value.mul(ProjectFee2).div(PERCENTS_DIVIDER);
		fee = msg.value.mul(ProjectFee3).div(PERCENTS_DIVIDER);
