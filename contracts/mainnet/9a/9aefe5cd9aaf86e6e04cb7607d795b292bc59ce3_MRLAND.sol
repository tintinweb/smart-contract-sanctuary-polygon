...
contract MRLAND is IMNFT, ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    address public wrapToken = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    ...
    function mint(uint256 _qty, address _to) external payable override   {
        uint256 totalPrice = mintPrice * _qty;
        require(totalSupply() + _qty <= MAX_SUPPLY, "NFT: out of stock");
        require(_to != address(0), "NFT: invalid address");
        require(IERC20(wrapToken).allowance(msg.sender, address(this)) >= totalPrice, "Not enough allowance to spend Wrapped Token");
        IERC20(wrapToken).transferFrom(msg.sender, revenueAddress, totalPrice);
    ...
    function setWrappedToken(address _wrapToken) public onlyOwner {
        wrapToken = _wrapToken;
    }
