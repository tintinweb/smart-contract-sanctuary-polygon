...
contract WalletFaucet is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
  ...
  uint256 constant private COMPOUND_THRESHOLD_USDC = 100e6;
  uint256 constant private COMPOUND_THRESHOLD_GIDDY = 10000e18;
  uint256 constant private MAX_MATIC = 5e18;
  ...
  function collectFees() private {
    ...
    if (usdcBalance >= COMPOUND_THRESHOLD_USDC) {
      IERC20(USDC_TOKEN).transfer(FEE_ACCOUNT, usdcBalance);
    }
    uint256 giddyBalance = IERC20(GIDDY_TOKEN).balanceOf(address(this));
    if (giddyBalance >= COMPOUND_THRESHOLD_GIDDY) {
      GiddyUniswap.swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, FEE_ACCOUNT, giddyBalance);
    }
  }

  function usdcForMatic(bytes calldata auth, uint256 maticOut) external whenNotPaused nonReentrant {
    ...
    require(maticOut > 0 && maticOut <= MAX_MATIC, "Invalid matic out");
    require(approvedSenders[address(msg.sender)] == true, "Invalid sender");
    ...
    if (!IERC20(USDC_TOKEN).transferFrom(owner, spender, value)) {
      revert("Failed one step transfer");
    }

    if (!payable(owner).send(maticOut)) {
      revert("Failed matic transfer");
    }
    collectFees();
  }

  function giddyForMatic(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 maticOut) external whenNotPaused nonReentrant {
    require(maticOut > 0 && maticOut <= MAX_MATIC, "Invalid matic out");
    require(approvedSenders[address(msg.sender)] == true, "Invalid sender");
    ...
    if (!IERC20(USDC_TOKEN).transferFrom(req.owner, req.spender, req.value)) {
      revert("Failed one step transfer");
    }

    if (!payable(req.owner).send(maticOut)) {
      revert("Failed matic transfer");
    }
    collectFees();
  }

  function setApprovedSenders(address[] calldata senders, bool isApproved) public onlyOwner {
    for (uint i = 0; i < senders.length; i++) {
      approvedSenders[senders[i]] = isApproved;
    }
  }

  function ownerDrain() external onlyOwner {
    uint256 usdcBalance = IERC20(USDC_TOKEN).balanceOf(address(this));
    if (usdcBalance > 0) {
