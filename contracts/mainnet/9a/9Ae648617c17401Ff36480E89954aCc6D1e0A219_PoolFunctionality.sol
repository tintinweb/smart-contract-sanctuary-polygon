...
contract PoolFunctionality is Ownable, IPoolFunctionality {
    ...
    function setLendingPool(address[] calldata pools, bool[] calldata values) public onlyOwner {
        require(pools.length == values.length, "PoolFunctionality: WL");
        require(pools.length != 0, "PoolFunctionality: FL");

        for (uint i = 0; i < pools.length; ++i) {
            isLendingPool[pools[i]] = values[i];
        }
    }
    ...
    function isLending(address pool) external view override returns (bool) {
        return isLendingPool[pool];
    }
    ...
    function _swapCurve(
        ...
        for (uint256 i; i < path.length - 1; ++i) {
            ...
            bool is_meta = ICurveRegistry(curFactory).is_meta(pool) || isLendingPool[pool];
            ...
            if (is_meta) 
                ICurvePool(pool).exchange_underlying(inputInd, outputInd, amounts[amountsIndex], 0);
            else
...
library OrionMultiPoolLibrary {
    ...
    function get_xp(address factory, address pool) internal view returns(uint256[] memory xp) {
        ...
        bool is_meta = ICurveRegistry(factory).is_meta(pool);
        bool isLending = IPoolFunctionality(address(this)).isLending(pool);

        address[MAX_COINS] memory coins;
        uint256[MAX_COINS] memory balances;
        if (is_meta) {
            coins = ICurveRegistry(factory).get_underlying_coins(pool);
            balances = ICurveRegistry(factory).get_underlying_balances(pool);
        } else if (isLending) {
            coins = ICurveRegistry(factory).get_underlying_coins(pool);
    ...
    function getAmountOutCurve(address factory, address from, address to, uint256 amount) view internal returns(uint256) {
        ...
        bool is_meta = ICurveRegistry(factory).is_meta(pool);
        bool isLending = IPoolFunctionality(address(this)).isLending(pool);
        is_meta = is_meta || isLending;
        uint256 dy;
        if (is_meta) {
            dy = ICurvePool(pool).get_dy_underlying(i, j, amount);
        } else {
            dy = ICurvePool(pool).get_dy(i, j, amount);
        }
...
interface ICurveRegistry {
    ...
    function is_meta(address pool) view external returns(bool);
    ...
    function get_underlying_balances(address _pool) view external returns (uint256[MAX_COINS] memory);
    ...
    function get_underlying_coins(address _pool) view external returns (address[MAX_COINS] memory);
}