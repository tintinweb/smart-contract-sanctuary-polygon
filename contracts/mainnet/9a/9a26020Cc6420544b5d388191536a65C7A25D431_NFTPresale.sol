...
contract NFTPresale is Ownable, Pausable {
    ...
    function redeem(address _account, address _token)
        external
        payable
        whenNotPaused
    {
        nativeAmountCheck(1, _token);
    ...
    function _redeemWithProof(
        ...
        require(_verify(_leafHash(_account), _proof), "Invalid merkle proof");
    ...
    function batchRedeem(
        ...
        nativeAmountCheck(_totalMinting, _token);
    ...
    function nativeAmountCheck(uint256 _totalMinting, address _buyToken)
        private
        view
    {
        require(_totalMinting > 0, "Invalid nft number to be minted");
        TokenInfo memory buyTokenInfo = allowedBuyTokenInfo[_buyToken];

        if (_buyToken == address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)) {
            uint256 totalAmount = _totalMinting * buyTokenInfo.amount;
