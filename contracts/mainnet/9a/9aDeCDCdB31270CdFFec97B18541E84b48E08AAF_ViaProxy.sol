...
// Interfaces
import '../interfaces/IViaRegistry.sol';
import '../interfaces/IWETH.sol';

contract ViaProxy is ReentrancyGuard, AccessControl, Pausable {
    using SafeERC20 for IERC20;

    // Via cross-chain address in source chain
    address public viaRegistry;
    ...
    modifier onlyManager() {
        require(hasRole(MANAGER, msg.sender) || hasRole(DEFAULT_ADMIN_ROLE, msg.sender), 'Caller is not in manager');
        _;
    }
    ...
    constructor(address _nativeWrap, address _viaRegistry) {
        viaRegistry = _viaRegistry;
        RubicFee = 3000;
     ...
     */
    receive() external payable {} // TODO add require for native

    function viaCall(IViaRegistry.UserRequest memory _userRequest, address _integrator)
        public
        payable
        nonReentrant
        whenNotPaused
    {
        if (_userRequest.bridgeRequest.inputToken == nativeWrap) {
            require(msg.value == _userRequest.amount, ViaErrors.VALUE_NOT_EQUAL_TO_AMOUNT);

            uint256 _amountWithFee = _calculateFee(
                _integrator,
                _userRequest.bridgeRequest.inputToken,
                _userRequest.amount
            );
            _userRequest.amount = _amountWithFee;

            uint256 _balanceBefore = IERC20(_userRequest.bridgeRequest.inputToken).balanceOf(address(this));
            IViaRegistry(viaRegistry).outboundTransferTo(_userRequest);
            require(
                _balanceBefore - IERC20(_userRequest.bridgeRequest.inputToken).balanceOf(address(this)) ==
                    _amountWithFee,
                ViaErrors.DIFFERENT_AMOUNT_SPENT
            );
        } else {
            require(msg.value == 0, ViaErrors.VALUE_SHOULD_BE_ZERO);

            IERC20(_userRequest.bridgeRequest.inputToken).safeTransferFrom(
                msg.sender,
                address(this),
                _userRequest.amount
            );
            smartApprove(IERC20(_userRequest.bridgeRequest.inputToken), _userRequest.amount, viaRegistry);
    ...
    function changeRegistry(address _viaRegistry) external onlyManager {
        viaRegistry = _viaRegistry;
    }
    ...
    function integratorCollectFee(
        ...
        require(integratorCollectedFee[_integrator][_token] >= _amount, 'amount too big');
    ...
    function collectIntegratorFeeManual(
        ...
        require(integratorCollectedFee[_token][_integrator] >= _amount, 'amount too big');
        _sendToken(_token, _amount, _integrator, _nativeOut);
...
library ViaErrors {
    ...
    string internal constant MIDDLEWARE_ACTION_FAILED = 'MIDDLEWARE_ACTION_FAILED';
    ...
    string internal constant DIFFERENT_AMOUNT_SPENT = 'DIFFERENT_AMOUNT_SPENT';
}
