...
interface ERC20MinterBurner is IERC20 {
    function poolBurnFrom(address from, uint256 amount) external;

    function poolMint(address from, uint256 amount) external;
}
...
contract FlashMinter is IFlashLender, Pausable, Ownable {
    ...
    function flashLoan(
        ...
        token.poolMint(address(receiver), amount);
        require(
            receiver.onFlashLoan(msg.sender, amount, _fee, data) ==
                CALLBACK_SUCCESS,
            "FlashMinter: Callback failed"
        );
        token.poolBurnFrom(address(receiver), amount + _fee);
        return true;
    }
