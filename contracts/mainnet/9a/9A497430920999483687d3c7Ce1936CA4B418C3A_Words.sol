...
contract Words is ERC721BaseInternal, WordsInternal, VRFInternal {
    ...
    function cardDetails(uint256 tokenId) public view returns(
        string[] memory word,
...
abstract contract WordsInternal {
    ...
    function _cardDetails(uint256 tokenId) internal view returns(
        string[] memory word,
        uint256 initialPower,
        uint256 power,
        uint256 value,
        ...
        WordsStorage.Layout storage l = WordsStorage.layout();
        WordsStorage.Word storage card = l.words[tokenId];
