...
contract DoubleUp is Ownable, ReentrancyGuard {
    ...
    uint256 public constant entryToCost = 10 * 10**uint(18); //1 => 10 Matic, 10=> 100 Matic for fees
    ...
    event Prize(address indexed prizeAddress, uint earnings, uint jackpotEarnings, uint currentWinnerCounter);
    event Eliminated(address indexed eliminatedAddress, uint collateral, uint currentEliminationCounter);
    ...
    function accumulatedBalance() view public returns(uint256) {
        return address(this).balance - devFunds - bonusPrize;
    }
    ...
    function enterImplementation(uint256 entry, uint256 immunityLevel, string calldata message, address msgSender) private {
        ...
        for(uint max = 0;max < 6 && head != address(0) && (currentActionableBalance >= currentWinTarget);++max){
            uint256 amountWon = currentWinTarget;
            uint256 totalAmountWon = amountWon;
            if(eliminationCounter >= eliminationsUntilBonus){
                emit Prize(head, amountWon, bonusPrize, winnerCounter + 1);
                totalAmountWon+=bonusPrize;
                eliminationCounter = 0;
                bonusPrize = 0;
            }else{
                emit Prize(head, amountWon, 0, winnerCounter + 1);
            }
        ...
        if(entriesLength >= 50){
            ...
            eliminationCounter++;
            emit Eliminated(current.next, next.collateral, eliminationCounter);
        ...
        emit BalanceUpdate(accumulatedBalance(), bonusPrize, currentWinTarget);
    }

    function withdrawDevFundBalance() external onlyOwner {
        if(refundsActive){
            //Sorry, the bonus prize is impossible to refund properly, so just letting the dev team capture it.
            //Ideally we won't ever use the refund self-destruct, but if we must, then this at least ensures funds aren't trapped.
            devFunds += bonusPrize;
            bonusPrize = 0;
        }
    ...
    //This is a nuclear option. Once triggered, we cannot re-enable the contract.
