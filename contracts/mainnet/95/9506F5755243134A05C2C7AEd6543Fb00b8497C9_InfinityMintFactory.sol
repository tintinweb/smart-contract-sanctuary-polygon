...
contract InfinityMintStickers is Stickers, ERC721 {
	...
	function beforeTransfer(
		...
		if (registeredOwners[from].length - 1 <= 0)
			...
			uint256 index = 0;
			for (uint256 i = 0; i < registeredOwners[from].length; i++) {
				if (index == temp.length) break;
				if (registeredOwners[from][i] == tokenId) continue;
				temp[index++] = tokenId;
			}
		...
		onlyOnce
	{
		...
		//don't take price if it is less than 10 wei as will make splits impossible
		if (price > 10 && valuesController.tryGetValue("stickerSplit") >= 2) {
			...
			(bool success, ) = address(erc721).call{ value: cut }(
				abi.encodeWithSignature(
			...
			require(
				success,
				"failed to drop off sticker royalty to main ERC721, sticker contract probably isnt linked to token id"
			);
		...
		returns (string memory)
	{
		...
		require(
			isSafe(stickers[uint64(stickerId)], erc721TokenId),
	...
	function addRequest(bytes memory packed) public payable override onlyOnce {
		...
		address sender = (msg.sender);
		require(
			registeredOwners[sender].length < 100,
...
abstract contract Stickers is StickerInterface {
	string public stickerType = "eads1";
	address public erc721;
	uint64 public currentTokenId;
