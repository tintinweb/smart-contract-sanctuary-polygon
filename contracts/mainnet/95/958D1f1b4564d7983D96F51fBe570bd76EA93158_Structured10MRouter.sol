...
contract Structured10MRouter is OwnableUpgradeable {
  ...
  address public _routeRegisterer;
   ...
   */
  function initialize(uint256 initialRoutesCount, uint256 itemsPerRoute, address routeRegisterer, address owner) public initializer {
    _routes = new string[](initialRoutesCount);
    _itemsPerRoute = itemsPerRoute;
    _routeRegisterer = routeRegisterer;
   ...
   */
  function registerRoute(uint256 index, string memory cid) public {
    require(_msgSender() == _routeRegisterer, "Only the `routeRegisterer` wallet can register routes");
