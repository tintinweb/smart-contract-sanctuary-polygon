    ...
    function buyfromToken(uint _pid,address ref,uint _amount) public {
        ...
        if(_pid == 1){
         ...
         //  uint multiplier = temp * 10 **(token.decimals()) / 50000 ;
           uint multiplier = temp*perDollarPrice;

            token.transfer(msg.sender,multiplier);

        }
        else if(_pid == 2){
            // uint256 _amount =  usdc.allowance(msg.sender , address(this)) ;
           // usdc.transferFrom(msg.sender,address(this),_amount);
            ...
            uint temp = _amount;
            uint multiplier = temp*perDollarPrice; //(token.decimals() - usdt.decimals());
            token.transfer(msg.sender,multiplier);
        }
        else {
