...
import "./SphereBondFactoryMiscellaneous.sol";

contract SphereBondFactory {

    /* ======== STATE VARIABLES ======== */

    address immutable private sphereProFactoryStorage;
    address immutable private sphereProSubsidyRouter;
    address immutable public sphereMultisig;
    ...
    modifier onlyMultisig() {
        require( sphereMultisig == msg.sender, "caller is not Sphere MSig" );
        _;
    }

    /* ======== CONSTRUCTION ======== */

    constructor(address _sphereTreasury, address _sphereProFactoryStorage, address _sphereSubsidyRouter, address _sphereMultisig) {
        require( _sphereTreasury != address(0) );
        sphereTreasury = _sphereTreasury;
        require( _sphereProFactoryStorage != address(0) );
        sphereProFactoryStorage = _sphereProFactoryStorage;
        require( _sphereSubsidyRouter != address(0) );
        sphereProSubsidyRouter = _sphereSubsidyRouter;
        require( _sphereMultisig != address(0) );
        sphereMultisig = _sphereMultisig;
    }
    ...
    /**
        @notice chnage Sphere Treasury address
        @param _sphereTreasury address
     */
    function changeTreasuryAddress(address _sphereTreasury) external onlyMultisig() {
        sphereTreasury = _sphereTreasury;
    }
     ...
     */
    function createBondAndTreasury(address _payoutToken, address _principalToken, address _initialOwner, uint[] calldata _tierCeilings, uint[] calldata _fees, bool _feeInPayout) external onlyMultisig() returns(address _treasury, address _bond) {

        CustomTreasury treasury = new CustomTreasury(_payoutToken, _initialOwner);
        CustomBond bond = new CustomBond(address(treasury), _principalToken, sphereTreasury, sphereProSubsidyRouter, _initialOwner, sphereMultisig, _tierCeilings, _fees, _feeInPayout);

        return ISphereProFactoryStorage(sphereProFactoryStorage).pushBond(
    ...
    /**
        ...
        @return _bond address
     */
    function createBond(address _principalToken, address _customTreasury, address _initialOwner, uint[] calldata _tierCeilings, uint[] calldata _fees, bool _feeInPayout ) external onlyMultisig() returns(address _treasury, address _bond) {

        CustomBond bond = new CustomBond(_customTreasury, _principalToken, sphereTreasury, sphereProSubsidyRouter, _initialOwner, sphereMultisig, _tierCeilings, _fees, _feeInPayout);
...
contract Ownable {
    ...
    function transferManagement(address _newOwner) external onlyPolicy() {
...
contract CustomBond is Ownable {
     ...
     *  @notice protocols using Sphere Pro should be careful when quickly adding large %s to total supply
...
interface ISphereProFactoryStorage {
