    ...
    ReentrancyGuard
{
    ...
    function _addBuyFee(uint256 price) internal view returns (uint256) {
        uint256 weiPrice = price * 1e18;
        uint256 newPrice = weiPrice + (weiPrice * config.buyFee()) / 10000;
        return newPrice;
    }
    ...
    function _payFee(
        ...
        // Cut buy fee
        uint256 listedPriceInWei = (weiPrice * 10000) / ((10000 + config.buyFee()) * 100) * 100;
        uint256 buyFeeInWei = weiPrice - listedPriceInWei;
        uint256 buyFeeInEth = buyFeeInWei / 1e18;
        ...
        if (checkRoyalties(token)) {
            (address creator, uint256 royaltyAmount) = IERC2981(token)
                .royaltyInfo(tokenId, listedPriceInWei);
        ...
        uint256 sellFeeInWei = (listedPriceInWei * config.sellFee()) / 10000;
        uint256 sellFeeInEth = sellFeeInWei / 1e18;
        uint256 restInWei = listedPriceInWei - sellFeeInWei - royaltyFee;
        rest = restInWei / 1e18;

        if (config.treasury() != address(0)) {
            payable(config.treasury()).transfer(buyFeeInEth + sellFeeInEth);
        } else {
            feesCollected += (buyFeeInEth + sellFeeInEth);
        }
    }
