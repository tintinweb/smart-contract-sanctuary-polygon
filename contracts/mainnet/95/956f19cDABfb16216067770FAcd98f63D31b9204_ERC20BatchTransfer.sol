...
contract ERC20BatchTransfer {
    ...
    function withdrawToken(uint256 value, ERC20 token) public onlyOperator {
        require(token.transfer(msg.sender, value), "Unable to transfer token to the owner account");
        emit WithdrawToken(msg.sender, value);
    }

    function batchTransfer(address[] calldata tokenHolders, uint256[] calldata amounts, ERC20 token) external onlyOperator {
        require(tokenHolders.length == amounts.length, "Invalid input parameters");
        for(uint256 indx = 0; indx < tokenHolders.length; ) {
