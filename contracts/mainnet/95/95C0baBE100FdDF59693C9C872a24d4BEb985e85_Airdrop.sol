...
contract Airdrop is AccessControl {
    ...
    function batchAirDrop(address[] memory _addresses)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint16 i = 0; i < _addresses.length; i++) {
            if (!Address.isContract(_addresses[i])) {
                payable(_addresses[i]).transfer(69000000000000000);
                console.log("Sent to ", i + 1);
            }
        }
    }
    ...
    function claimMatic() external onlyRole(DEFAULT_ADMIN_ROLE) {
