...
contract DexGoRentAndKm is Ownable {
    ...
    function setRentPercentToShareAndRentable(uint256 _tokenId, uint256 _percentToShareWei, bool _rentable) public {
        require(IDexGoNFT(nftContract).isApprovedOrOwner(msg.sender, _tokenId), "Caller is not token owner nor approved");
        require(!IDexGoNFT(nftContract).getInAppPurchaseBlackListWallet(msg.sender) && !IDexGoNFT(nftContract).getInAppPurchaseBlackListTokenId(_tokenId), "wallet or tokenId blacklisted");
    ...
    function allRentablesRentable() public view returns (RentableItem [] memory, uint) {
        RentableItem [] memory rentablesReturn = new RentableItem[](IDexGoNFT(nftContract).getTokenIdCounterCurrent());
        uint256 rentablesReturnCount;
        for (uint i; i < IDexGoNFT(nftContract).getTokenIdCounterCurrent(); i++) {
    ...
    function rent(uint256 _tokenId) public payable {
        ...
        require(!IDexGoNFT(nftContract).getInAppPurchaseBlackListWallet(msg.sender) && !IDexGoNFT(nftContract).getInAppPurchaseBlackListTokenId(_tokenId), "wallet or tokenId blacklisted");
        payable(IERC721(nftContract).ownerOf(_tokenId)).transfer(fixedAmountOwner);

        Address.sendValue(payable(nftContract), fixedAmountProject);
        IDexGoNFT(nftContract).distributeMoney(msg.sender, fixedAmountProject);
    ...
    function rentReturn(uint256 _tokenId) public {
        require(IDexGoNFT(nftContract).isApprovedOrOwner(msg.sender, _tokenId) || msg.sender == rentables[_tokenId].borrower, "Caller is not token owner nor approved or not borrower");
    ...
    function setKmForId(uint256 tokenId, uint256 km) public {
        require(msg.sender == nftContract || msg.sender == IDexGoNFT(nftContract).getGameServer() || msg.sender == owner(),'only admin accounts can change km');
    ...
    function repair(uint256 _tokenId, bool isSpeedUp) public payable {
        if (isSpeedUp) {
            ...
            Address.sendValue(payable(nftContract), msg.value);
            IDexGoNFT(nftContract).distributeMoney(msg.sender, msg.value);
        ...
        kmLeavesForId[_tokenId] = IDexGoNFT(nftContract).getPriceInitialForType(IDexGoNFT(nftContract).getTypeForId(_tokenId)) * repairCount[_tokenId] / 100;
        IDexGoNFT(nftContract).setLatestPurchaseTime(msg.sender, block.timestamp);
    }
