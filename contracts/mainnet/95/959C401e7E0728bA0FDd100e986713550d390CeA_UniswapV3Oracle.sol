...
library Exceptions {
    ...
    string constant NULL_STABLECOIN_ADDRESS = "NULL_STABLECOIN_ADDRESS";
    ...
    __with_semver(uint8(1), uint8(0), uint8(0))
{
    ...
    function getPrice(address tokenAddressA, address tokenAddressB) external view returns (uint256 price) {
        uint24 basicFeeA = _whitelistTokens[tokenAddressA];
        uint24 basicFeeB = _whitelistTokens[tokenAddressB];
        ...
        return _getPrice(poolAddress, _twapInterval);
    }

    function setFactoryAddress(IUniswapV3Factory factoryAddress_) external onlyOwner {
    ...
    function _getPrice(address poolAddress, uint32 twapInterval) internal view returns (uint256 priceX96) {
        if (twapInterval == 0) {
            (uint160 sqrtPriceX96, , , , , , ) = IUniswapV3Pool(poolAddress).slot0();
            return UniswapMath.mulDiv(sqrtPriceX96, sqrtPriceX96, FixedPoint96.Q96);
        } else {
            uint32[] memory secondsAgos = new uint32[](2);
            secondsAgos[0] = twapInterval;
            secondsAgos[1] = 0;
            (int56[] memory tickCumulatives, ) = IUniswapV3Pool(poolAddress).observe(secondsAgos);
            uint160 sqrtPriceX96 = UniswapMath.getSqrtRatioAtTick(
