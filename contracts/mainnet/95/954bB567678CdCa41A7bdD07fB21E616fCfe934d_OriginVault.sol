...
// File contracts/OriginVault.sol
...
contract OriginVault is Ownable {
    ...
    modifier onlyRelayer() {
        require(
            msg.sender == relayer,
            "OriginVault: only relayer can trigger this method!"
        );
        _;
    }
    ...
    // allow depositor deposit supported token {emits Deposit}
        ...
        isvalidTxAmount(_token, _amount)
    {
        ...
        if (feeTo != address(0) && feeAmount > 0) {
            ERC20(_token).transfer(feeTo, feeAmount);
    ...
    // send tokens to depositor (relayer only)
    function withdraw(
        address _depositor,
        uint256 _amount,
        address _token
