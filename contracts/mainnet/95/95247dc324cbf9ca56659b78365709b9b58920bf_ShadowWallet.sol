...
// File contracts/simple-contract-wallet/ShadowWallet.sol
...
contract ShadowWallet is Initializable, ERC721Holder, ERC1155Holder, ReentrancyGuard {
    ...
    modifier isActive {
        require(active == true, "SW: Contract Inactive");
        _;
    }

    modifier onlyOwner {
        if (deployer == address(0)) {
            require(owner == msg.sender, "SW: Unauthorised Access");
            _;
        } else {
            require(deployer == msg.sender, "SW: Unauthorised Access");
            _;
        }
    }
    ...
    function updateDeployer(address _deployer) external onlyDeployer isActive {
        deployer = _deployer;
    }

    function swapOwner(address _owner) external onlyOwner isActive {
        require(owner != address(0), "SW: Zero Address Provided");
        owner = _owner;
        emit SwapOwner(msg.sender, owner, block.timestamp);
    }

    function switchState() external onlyDeployer {
        active = !active;
    }
    ...
    function claimNativeToken() external onlyOwner nonReentrant() {
        ...
        if (contractBalance > 0) {
            ...
            require(success, "SW: Claim Error");
        }
    }
