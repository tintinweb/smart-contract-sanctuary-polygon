...
contract TidePoolFactory {
    ...
    function deploy(address _pool) public returns (address validatedPool) {
...
contract TidePool is ITidePool, IUniswapV3MintCallback, IUniswapV3SwapCallback, ERC20 {
    ...
    function swapIntoRatio(uint256 amount0, uint256 amount1) private returns (uint256 desired0, uint256 desired1) {
        ...
        if(pool.getPosition(tickLower, tickUpper) == 0)
            (tickUpper, tickLower) = TidePoolMath.calculateWindow(pool.getTick(), pool.tickSpacing(), tickWindow, 50);
...
library TidePoolMath {
    ...
    // window size grows by 4 ticks every rebalance, but shrinks 1 tick per day.
    function getTickWindowSize(int24 _previousWindow, uint256 _lastRebalance) public view returns (int24 window) {
        uint256 diff = block.timestamp - _lastRebalance;
        window = _previousWindow + 4 - int24(diff / 1 days);
        window = window < 2 ? 2 : window;
    }
}
