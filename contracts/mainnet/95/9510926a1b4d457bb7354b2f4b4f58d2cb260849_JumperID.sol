...
abstract contract ERC5192 is ERC721AQueryable, IERC5192, IERC721ABurnable {
  ...
  function safeTransferFrom(
    ...
    uint256 tokenId,
    bytes memory data
  ) public override(ERC721A,IERC721A) checkLock {
  ...
  function approve(address approved, uint256 tokenId) public override(ERC721A,IERC721A) checkLock {
...
contract JumperID is ERC5192, Context, OwnableRoles {

    constructor(
        string memory name,
        string memory symbol,
        bool initialLockStatus,
        string memory metadata
    ) ERC5192(name, symbol, initialLockStatus) {
        _normalUri = metadata;
    ...
    //@dev FACTORY_ROLE  for performing metadata update interactions.
    //@dev SUPPORT_ROLE for authorized contract interactions.

    uint256 public constant SUPPORT_ROLE = _ROLE_3;
    ...
    bool public distributionIsLive;
    ...
    // Flexible URI controls that defaults to base token
    // Allows for indiviudal token specific modification
    ...
    function resetURI(uint256 tokenId) external onlyRolesOrOwner(ADMIN_ROLE | FACTORY_ROLE) {
    customURIs[tokenId] = "";
    }
    ...
    // 
    function adminMint(address to) external onlyRolesOrOwner(ADMIN_ROLE) {
        require(totalSupply() <= _maxSupply, "Quantity exceeds supply");
        _safeMint(to,1);
    }
    ...
    function batchDistribute(address[] memory recipients) external onlyRolesOrOwner(ADMIN_ROLE | SUPPORT_ROLE) {
    ...
    function toggleDistributionStatus() public onlyRolesOrOwner(ADMIN_ROLE) {
        ...
        returns (bool)
    {
        ...
        // - IERC2981: 0x2a55205a
        return ERC721A.supportsInterface(interfaceId) || ERC5192.supportsInterface(interfaceId);
    }
