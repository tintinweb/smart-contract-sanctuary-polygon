...
contract TimeContent is Ownable,ReentrancyGuard{
    uint256 private pointId; //supervisor Id
    uint256 private _contentId; //contents Id
    uint256 private passVote=3; //pass vote
    uint256 private punishPassVote=3; //Punish pass vote
    uint256 private contractLock=1;  //1 unlock,other lock
    uint256 public totalVote; //all vote
    uint256 public validAllVote;  //Total number of review missions in force
    using SafeERC20 for IERC20;

    IERC20[] public allowAllToken;  
    //if supervisor 
    mapping(address=>bool)public addressToBool;
    //Record the number of votes cast by the supervisor
    mapping(address=>uint256)private alreadyVoteAmount;
    //Record whether the supervisor has withdrawn the money
    mapping(address=>uint256)private ifWithdrawMoney;
  
    mapping(string=>uint256)public contentNameToId; 
    mapping(uint256=>address)public contentIdToAddress; 
    mapping(string=>bool)public contentNameToBool;  

    mapping(string=>uint256)public nameToPunishId;
    
    //The supervisor certificate of a vote on a matter
    mapping(address=>mapping(uint256=>bool))public supervisorVoteProof;
    //Proof that the supervisor has voted on the disciplinary matter
    ...
    struct contentThing{
        uint256 contentId;        //Id
        string contentName;       //name 
        string contentWillThing;  //The receiver submitted by the sender needs to transfer the token in detail
        uint256 tradeMoney;       //trade money
        uint256 startTime;        //start time
        uint256 endTime;          //end time
        address sourceAddress;    //sender
        address pointAddress;     //receiver
        bool receiverContentWhether;   //The receiver confirms that the agreed token has been transferred to the correct address
        string receiverBlockProof;  //The hash record submitted by the recipient
        uint256 votedSum; //The current number of votes for the transaction
        address choosedToken;  //choose token
    }
    contentThing[] public _waitContentThing;

    struct punishment{
        uint256 punishId;
        uint256 allPunishVotes; //The disciplinary matter now counts votes
        string senderProof;  //sender proof
        string receiverProof; //receiver proof
    }
    ...
    //change lock
    function changeLock(uint256 contractState)external onlyOwner{
    ...
    //create a contents
    function createThing(string calldata _contentName,string memory _willThing,uint256 _tradeMoney,uint256 _endTime,address _pointAddress,address token)external{   
        uint256 _thisContentName=StringUtils.strlen(_contentName);
        uint256 transferMoney=_tradeMoney*1 ether;
        uint256 getEndTime=_endTime+block.timestamp+3600;
        require(checkToken(IERC20(token))); 
        require(getEndTime>=block.timestamp+3600,"End time need >= 60min");
        require(_pointAddress!=msg.sender && _pointAddress!=address(0));
        require(_thisContentName>=1 && _thisContentName<=100);  
        require(_tradeMoney>=100 && _tradeMoney<=10000000000);  
        require(contentNameToBool[_contentName]!=true,"This contentName already existed!"); 
        contentNameToBool[_contentName]=true;     
        contentNameToId[_contentName]=_contentId;   
        contentIdToAddress[_contentId]=msg.sender;
        nameToPunishId[_contentName]=_contentId;  
       
        _waitContentThing.push(
        contentThing(
        _contentId,_contentName,_willThing,transferMoney,block.timestamp,getEndTime,msg.sender,_pointAddress,false,"",0,token
        ));
    ...
    function addSupervisors(address waitAddress,string calldata _supervisorName)external onlyOwner{
        ...
        addressToBool[waitAddress]=true;
    ...
    function voteContent(string calldata contentName,bool judge)external userState nonReentrant{
        ...
        require(doTimeLock(contentName)==false,"Contents had pass");
    ...
    function votePunish(string calldata contentName,bool _punishJudge)external userState nonReentrant{
        ...
        require(addressToBool[msg.sender]);
        require(punishState(contentName)==false,"Punish contents had pass");
    ...
    //Get how many of these tokens the supervisor can retrieve
    ...
    //Whether the receiver is sure to initiate a transaction and submit a transaction confirmation record,
    // otherwise the review will fail (very important)
    function judgeReceiverContent(string calldata contentName,string calldata blockProof)external nonReentrant{
        require(getThisPointAddress(contentName)==msg.sender);  
        _waitContentThing[contentNameToContentId(contentName)].receiverContentWhether=true; 
        _waitContentThing[contentNameToContentId(contentName)].receiverBlockProof=blockProof;
    }

    //The initiator reviews the block hash evidence submission
    ...
    //The designee reviews the block hash evidence submission
    ...
    //punish contents state
    function punishState(string calldata contentName)public view returns(bool){
        uint256 thisPunishId=nameToPunishId[contentName];
        return _punishment[thisPunishId].allPunishVotes>=punishPassVote?true:false;
    }

    //Whether contents pass
    function doTimeLock(string calldata contentName)public view returns(bool){
        uint256 totalVotes = _waitContentThing[contentNameToContentId(contentName)].votedSum;  
    ...
    function contentNameToContentId(string calldata name)public view returns(uint256){
        require(contentNameToBool[name],"Content name error");
    ...
    //get this contents trade money amout
    ...
    //The number of times the supervisor has voted
    ...
    function getAllNoPunishs()public view returns(punishment[] memory){
        ...
        for(uint256 j;j<_punishment.length;j++){
            if(a<newFinishId){
                if(_punishment[j].allPunishVotes<punishPassVote){
    ...
    //Get all the transactions of the user's individual
