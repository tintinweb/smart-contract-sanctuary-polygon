...
contract iGold is ERC20, Ownable {
    ...
    int256 pricePerGram = pricePerOunce * 10000 / 311035; // Multiplied by 10^4 to handle decimals
    ...
    function buy(uint256 usdtAmount) public notPaused{
        uint256 _totalSupply = totalSupply();
        require(_totalSupply <= goldReserve.mul(1e8).div(10), "gold reserve reached");
        int256 goldPrice = getLatestGoldPriceGram();
        require(goldPrice > 0, "Invalid gold price");

        uint256 goldAmount = usdtAmount.mul(1e2).mul(1e1).mul(1e8).div(uint256(goldPrice)); // 0.1g per token
        uint256 islamiFee = getIslamiPrice(usdtAmount.div(100)); // 1% fee

        emit trade("Buy", goldAmount, goldPrice, usdtAmount, islamiFee);
        ...
        require(
            islamiToken.transferFrom(msg.sender, address(this), islamiFee),
            "Check ISLAMI allowance or user balance"
        );

        usdtVault = usdtVault.add(usdtAmount);

        feesCollected = feesCollected.add(islamiFee);
        islamiVault = islamiVault.add(islamiFee);

        _mint(msg.sender, goldAmount);
    }

    function sell(uint256 goldAmount) public notPaused{
        int256 goldPrice = getLatestGoldPriceGram();
        require(goldPrice > 0, "Invalid gold price");

        uint256 usdtAmount = goldAmount.mul(uint256(goldPrice)).div(1e4).div(1e1).div(1e6); // 0.1g per token
        uint256 islamiFee = getIslamiPrice(usdtAmount.div(100)); // 1% fee
        ...
        returns (uint256 goldValue, uint256 usdtInVault)
    {
        ...
        goldValue = totalMintedGold.mul(uint256(goldPrice)).div(1e4).div(1e1).div(1e6); // Calculate the value of minted iGold tokens in USDT
    ...
    function withdrawTokens(IERC20 token, uint256 amount) external onlyOwner {
        require(token == islamiToken || token == usdtToken, "Invalid token");
        require(
            token.balanceOf(address(this)) >= amount,
            "Insufficient balance"
        );

        if (token == islamiToken) {
            islamiVault -= amount;
