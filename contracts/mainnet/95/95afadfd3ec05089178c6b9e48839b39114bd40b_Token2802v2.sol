...
interface iTest2802v2 {
    function balanceOf(address owner) external view returns (uint256);
}

contract Token2802v2 is ERC20, Ownable {

    iTest2802v2 public Test2802v2;
    ...
    constructor(address Test2802v2Address) ERC20("Test2802v2", "T2802V2") {
        Test2802v2 = iTest2802v2(Test2802v2Address);
        START = block.timestamp;
    }

    function updateReward(address from, address to) external {
        require(msg.sender == address(Test2802v2));
        if(from != address(0)){
    ...
    function burn(address user, uint256 amount) external {
        require(allowedAddresses[msg.sender] || msg.sender == address(Test2802v2), "Address does not have permission to burn");
        _burn(user, amount);
    }
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        return Test2802v2.balanceOf(user) * BASE_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 86400;
    }
