...
contract NFT is ERC721URIStorage, Ownable {
	...
	function mintNft(string memory _tokenURI) public onlyOwner {
		_mintNftTo(msg.sender, _tokenURI);
	}

	function mintNftTo(address toAddress, string memory _tokenURI) public onlyOwner {
		_mintNftTo(toAddress, _tokenURI);
	}

	function mintManyNftTo(address toAddress, string[] memory _tokenURIArray) public onlyOwner {
		uint256 count = _tokenURIArray.length;
		for (uint256 i=0; i<count; i++){
			_mintNftTo(toAddress, _tokenURIArray[i]);
		}
	}

	function _mintNftTo(address toAddress, string memory _tokenURI)  internal {
		_tokenIds.increment();
