...
contract PetoNewBetContract is
    Initializable,
    UUPSUpgradeable,
    ...
    AccessControlUpgradeable
{
    ...
    function _addTransaction(bytes32 transactionIdHash) private {
        require(_transactions[transactionIdHash] == false, "This transactionId was used before");
        _transactions[transactionIdHash] = true;
    }
    ...
    function withdrawSig(
        ...
        _addTransaction(transactionIdHash);
        ...
        require(balanceOf(to) >= amount, "User must have funds in contract");

        _usdtToken.transfer(to, amount);
        _balances[to] -= amount;
        emit Withdraw(to, amount, transactionIdHash, uint32(block.timestamp));
    }
}
