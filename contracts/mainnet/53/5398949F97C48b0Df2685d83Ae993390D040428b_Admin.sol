...
contract Admin is BaseAdmin {
    ...
    function openSale__setPaymentPolicy(
    ...
    function whitelistSale__setPaymentPolicy(
        bytes32[] calldata tierValues,
        uint256[] calldata tierPrices_
    )
        external
    ...
    {
        ERC721WhitelistSale(
            _kernel.getAppAddress(AppsIds.ERC721_WHITELIST_SALE)
        ).setPaymentPolicy(tierValues, tierPrices_);
    }

    function whitelistSale__setMerkleTree(
        bytes32 merkleRoot,
        bytes calldata merkleProofIpfsHash
    )
        external
        onlyAdminOrSudo 
    {
        ERC721WhitelistSale(
            _kernel.getAppAddress(AppsIds.ERC721_WHITELIST_SALE)
        ).setMerkleTree(merkleRoot, merkleProofIpfsHash);
    }
...
contract ERC721BaseSale is App {
    ...
    function setIsActive(bool isActive_)
        external
        requirePermission(ADMIN) 
    {
        isActive = isActive_;
