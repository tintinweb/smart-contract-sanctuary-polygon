...
contract QiDaoFolderSDAM3 is QiDaoFolder {
  ...
  constructor() QiDaoFolder(MAI_ADDRESS, MAI_PAIR_ADDRESS, SDAM3_VAULT_ADDRESS, SDAM3_STAKEDAO_VAULT_ADDRESS) {
  }

  function _approveSwaps() internal override {
    IERC20(maiAddress).approve(CURVE_MAI_3POOL_ADDRESS, type(uint).max);
    IERC20(AM3CRV_ADDRESS).approve(CURVE_MAI_3POOL_ADDRESS, type(uint).max);
    IERC20(AM3CRV_ADDRESS).approve(SDAM3_STAKEDAO_VAULT_ADDRESS, type(uint).max);
  }
...
abstract contract QiDaoFolder {
  ...
  uint constant DEFAULT_REBALANCE_TRIGGER_DEVIATION = 100; // e.g. rebalance at 116% or 114%
  ...
  uint public rebalanceTriggerDeviation; // rebalance when CDR deviates by this amount
  ...
  constructor(address _maiAddress, address _maiPairAddress, address _vaultAddress, address _tokenAddress) {
    owner = msg.sender;
  ...
  function initialize(string memory _name, string memory _symbol, uint _targetCdr) external onlyOwner {
    ...
    rebalanceTriggerDeviation = DEFAULT_REBALANCE_TRIGGER_DEVIATION;
  ...
  /// @param _rebalanceTriggerDeviation rebalance when CDR deviates by this amount
  function setRebalanceTriggerDeviation(uint _rebalanceTriggerDeviation) external onlyOwner onlyInitialized {
    rebalanceTriggerDeviation = _rebalanceTriggerDeviation;
  }
  ...
  // TODO: replace with vault / tokenization stuff
  ...
  function vaultValue() view public returns (uint) {
    ...
    return _vaultCollateralValue() - debt - repaymentFees;
  }
  ...
  function _getRebalanceType() view internal returns (RebalanceType rt) {
    ...
    } else if (cdr > targetCdr + rebalanceTriggerDeviation) {
