...
contract ExchangeItGateway is BaseRelayRecipient, Ownable{
    ...
    event ListingWithdrawn(address seller, address listing);
    ...
    constructor() {
        ...
        //address mtrustedForwarder = 0xF82986F574803dfFd9609BE8b9c7B92f63a1410E; // Kovan
        address mtrustedForwarder = 0x9399BB24DBB5C4b782C70c2969F58716Ebbd6a3b; // Matic Mumbai
    ...
    function testBiconomy() external payable {
        testAddresses.push(_msgSender());
    }
    ...
    /*
     * MVP Seller poaching methods
     */
    function claimListing(
        bytes32 hashUnlockCodeBuyer,
        bytes32 hashUnlockCodeSeller,
        address sellerAddress
    ) external payable {
        // Check to make sure that values are valid
       require(hashUnlockCodeBuyer.length > 0, "Seller's hash code must not be empty");

        // Create a Listing
        Listing listing = new Listing(
            sellerAddress,
            hashUnlockCodeSeller,
            hashUnlockCodeBuyer
        );
        ...
        sellerListings[sellerAddress].push(_numOfListings.current());
        ...
        // Emit event
        emit ListingCreated(sellerAddress, address(listing));
    }

    function unlockListingAndWithdraw(address listingAddress, string memory buyerUnlockCode, address sellerAddress) external {
        Listing targetListing = Listing(listingAddress);
        targetListing.unlockListingAndWithdraw(buyerUnlockCode, sellerAddress);
        emit ListingWithdrawn(sellerAddress, listingAddress);
    }
...
/*
    function createListing(
        ...
        // TODO: removed restrictions for now
        ...
        require(hashSellerCode.length > 0, "Seller's hash code must not be empty");
        ...
        // Make sure we track the sellers listings
        sellerListings[_msgSender()].push(_numOfListings.current());
        ...
        // Emit event
        emit ListingCreated(_msgSender(), address(listing));
    }
*/
    /*
    ...
    function cancelOffer(address listingAddress) external {
        ...
        // Cancel the offer
        targetListing.cancelOffer(payable(msg.sender));
...
*/
    ...
    constructor(
        ...
        bytes32 _hashUnlockCodeSeller,
        bytes32 _hashUnlockCodeBuyer
    ) {
        ...
        hashSellerCode = _hashUnlockCodeSeller;
        hashBuyerCode = _hashUnlockCodeBuyer;
    ...
    function deposit_approve() public payable {
        uint256 amount = 1;
        wETH.approve(address(this), amount);
    }

    /**
     * @dev Anybody can deposit into this contract, unprotected but nobody but us would do that
     */
    ...
    /**
     * @dev Withdraw everything from this contract, private function
     *
     ...
     */
    function _withdraw(address payable payee) private {
        uint256 payment = deposits;

        deposits = 0;

        wETH.transfer(payee, payment);
        emit Withdrawn(payment);
    }

    function unlockListingAndWithdraw(string memory buyerUnlockCode, address senderAddress) external {
        ...
        _withdraw(payable(senderAddress));
    }
