...
contract HopProvider is ReentrancyGuard {
	...
	function transferNative(address _from, address _receiver, uint256 toChainId, address tokenAddress, uint _amount, address _hopBridge) external payable nonReentrant {
		if(tokenAddress == NATIVE_TOKEN_ADDRESS) {
			...
			L2AMMSwapHop(_hopBridge).swapAndSend{value: _amount}(toChainId, _receiver, _amount, 28578266354135306662, 0, 0, 0, 0);
		} else {
			...
			IERC20(tokenAddress).safeIncreaseAllowance(address(L2AMMSwapHop(_hopBridge)), _amount);

			L2AMMSwapHop(_hopBridge).swapAndSend(toChainId, _receiver, _amount, 28578266354135306662, 0, 0, 0, 0);
		}
