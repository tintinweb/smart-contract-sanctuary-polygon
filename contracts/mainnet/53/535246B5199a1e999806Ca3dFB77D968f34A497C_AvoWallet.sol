...
abstract contract AvoCoreConstantsOverride is AvoCoreErrors {
    ...
    /// @dev minimum fee or fee charged via `castAuthorized()` to charge if `AvoVersionsRegistry.calcFee()` would fail
    ...
    /// then MAX_AUTHORIZED_FEE is charged as fee instead
    uint256 public immutable AUTHORIZED_MAX_FEE;
    /// @dev address that the fee charged via `castAuthorized()` is sent to
    ...
    AvoWalletCastAuthorized
{
    ...
    /// @param authorizedMinFee_       minimum fee or fee charged via `castAuthorized()` to charge if `AvoVersionsRegistry.calcFee()` would fail.
    /// @param authorizedMaxFee_       global maximum for fee charged via `castAuthorized()`. If AvoVersionsRegistry
