...
contract Writer is IWriter, StorageExtensionConstants {
    ...
    function _stakers() external view returns (address) {
        return IStorageGetters(_sto).stakers();
    }
        ...
        returns (uint256 ownVerse)
    {
        if (IStorageGetters(_sto).challengesLevel() > 0) {
            ...
            IStorage(_sto).pushOwnershipRoot(bytes32(0x0), timestamp);
        }
    ...
    /// @inheritdoc IWriter
    function setProposedOwnershipToPrevVerse(bytes32 ownershipRootPrevVerse)
        external
        onlyUpdates
    {
        IStorage(_sto).deleteChallenges();
        IStorage(_sto).pushChallenge(
            ownershipRootPrevVerse,
            bytes32(0),
            bytes32(0x0),
            0
        );
    ...
    /// @inheritdoc IWriter
    function storeBypassedVerse(uint256 verse)
        external
        onlyUpdates
    {
        IStorage(_sto).setClaim(BYPASSED_CLAIM_IDX, verse, 1, '');
    }
    ...
    /// @inheritdoc IWriter
    function activateGovernance() external onlyUpdates {
        IStorage(_sto).setClaim(GOVERNANCE_CLAIM_IDX, 0, 1, 'awaitingGovernance');
    }

    /// @inheritdoc IWriter
    function deactivateGovernance() external onlyUpdates {
        IStorage(_sto).setClaim(GOVERNANCE_CLAIM_IDX, 0, 0, 'awaitingGovernance');
    }
}
