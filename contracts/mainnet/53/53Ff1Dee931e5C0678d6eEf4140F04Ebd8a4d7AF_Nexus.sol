...
/**
    #Nexus Transaction tax:
    33.3% Nexus Rewards
    33.3% Staking Rewards
    33.3% Locked to Liquidity
*/
...
contract Nexus is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1000 * 10**6 * 10**uint256(_decimals);
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    uint256 public _reflectioinFee = 0;
    uint256 private _previousReflectionFee = _reflectioinFee;

    uint256 public _stakingFee = 333;
    uint256 private _previousStakingFee = _stakingFee;

    uint256 public _nexusFee = 333;
    uint256 private _previousNexusFee = _nexusFee;

    uint256 public _liquidityFee = 333;
    uint256 private _previousLiquidityFee = _liquidityFee;

    address public stakingWallet = 0xcead128aD3F487509ded6940A052C0e9156f6666;
    address public nexusWallet = 0x9eC4feCF71BE4c3548fa07D5bE21b5Bd041648c7;
    ...
    constructor() {
        ...
        _isExcludedFromFee[0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff] = true;
    ...
    function excludeFromReward(address account) public onlyOwner {
        require(
            account != 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff,
            "We can not exclude Quick router."
        );
    ...
    function _distributeFee(
        ...
        _rOwned[stakingWallet] += rStaking;
        if (_isExcluded[stakingWallet]) {
            _tOwned[stakingWallet] += tStaking;
        }
        _rOwned[nexusWallet] += rNexus;
        if (_isExcluded[nexusWallet]) {
            _tOwned[nexusWallet] += tNexus;
        }
    }
        ...
        returns (ValuesStruct memory)
    {
        ...
        RValuesStruct memory rvs = _getRValues(
            tAmount,
            tvs.tFee,
            tvs.tStaking,
            tvs.tNexus,
    ...
    function _getTValues(uint256 tAmount)
        private
        view
        returns (TValuesStruct memory)
    {
        uint256 tFee = calculateReflectionFee(tAmount);
        uint256 tStaking = calculateStakingFee(tAmount);
        uint256 tLiquidity = calculateLiquidityFee(tAmount);
        uint256 tNexus = calculateNexusFee(tAmount);
        uint256 tTransferAmount = tAmount -
            ...
            tLiquidity;
        return
            TValuesStruct(tTransferAmount, tFee, tStaking, tNexus, tLiquidity);
    }
    ...
    function _getRValues(
        ...
        uint256 tLiquidity,
        uint256 currentRate
    ) private pure returns (RValuesStruct memory) {
        uint256 rAmount = tAmount * currentRate;
        uint256 rFee = tFee * currentRate;
        uint256 rStaking = tStaking * currentRate;
        uint256 rNexus = tNexus * currentRate;
    ...
    function calculateReflectionFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(_reflectioinFee).div(10**2);
    }
    ...
    function calculateStakingFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(_stakingFee).div(10**7);
    }

    function calculateNexusFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_nexusFee).div(10**7);
    }
    ...
    function calculateLiquidityFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(_liquidityFee).div(10**7);
    }
    ...
    function restoreAllFee() private {
        _reflectioinFee = _previousReflectionFee;
        _liquidityFee = _previousLiquidityFee;
        _stakingFee = _previousStakingFee;
        _nexusFee = _previousNexusFee;
    }
    ...
    function setStakingWallet(address newWallet) external onlyOwner {
        stakingWallet = newWallet;
    }

    function setNexusWallet(address newWallet) external onlyOwner {
        nexusWallet = newWallet;
    }
    ...
    function setNexusFee(uint256 nexusFee) external onlyOwner {
        _nexusFee = nexusFee;
    }
    ...
    event UpdateSellFeeMultiplier(
        uint256 oldSellFeeMultiplier,
        uint256 newSellFeeMultiplier
    );
}
