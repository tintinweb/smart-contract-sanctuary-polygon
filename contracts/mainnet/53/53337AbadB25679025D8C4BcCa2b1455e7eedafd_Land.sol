    ...
    Ownable
{
    ...
    address private immutable chickenContract;
    ...
    function _isChickenOwnerSaleActive() private view returns (bool) {
        return (block.timestamp >= _chickenOwnerSaleStart &&
            block.timestamp <= _chickenOwnerSaleEnd);
    }

    // chicken owner sale
    function chickenOwnerMint(uint256 _mintAmount)
        public
        nonReentrant
        ensureAvailabilityFor(_mintAmount)
    {
        if (!_isChickenOwnerSaleActive())
            revert Errors.ChickenOwnerSaleNotActive();
        if (IERC721(chickenContract).balanceOf(_msgSender()) == 0)
            revert Errors.NotOwnerOfChicken();
        if (_mintAmount == 0) revert Errors.MintNoNFT();
        if (_mintAmount > maxMintAmount) revert Errors.ExceedMaxMintAmount();
        if (tokenCount() + _mintAmount > MAX_SUPPLY)
