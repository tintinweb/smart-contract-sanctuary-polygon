...
library VaultLifecycleWithSwap {
    ...
    function getNextFriday(uint256 timestamp) internal pure returns (uint256) {
        ...
        uint256 nextFriday = timestamp + ((7 + 1 - dayOfWeek) % 7) * 1 days;
    ...
    ERC20Upgradeable
{
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        if (depositFee > 0) {
            uint256 fee = amount.mul(depositFee).div(
                100 * Vault.FEE_MULTIPLIER
            );
            amount += fee;
        }
    ...
    function _completeWithdraw() internal returns (uint256) {
        ...
        if (withdrawalFee > 0) {
            uint256 fee = withdrawAmount.mul(withdrawalFee).div(
                100 * Vault.FEE_MULTIPLIER
            );
            withdrawAmount -= fee;
        }
    ...
    function mintPending() external onlyOwner {
        _mint(address(this), 31500169057462173950556);
