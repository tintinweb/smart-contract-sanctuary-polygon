    ...
    UUPSUpgradeable
{
    ...
    mapping(uint256 => bool) public claimedById;

    event LandFuturesClaimUpdate(
        uint256[] landIds
    );
    ...
    function setClaimed(uint256[] memory _ids, bool _b) public onlyRole(GM_ROLE) {
        for (uint i = 0; i < _ids.length; i++) {
            claimedById[_ids[i]] = _b;
        }
        
        emit LandFuturesClaimUpdate(_ids);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0)) {
            for (uint256 i = 0; i < ids.length; i++) {
                require(claimedById[ids[i]] == false, "The box is claimed");
            }
        }
