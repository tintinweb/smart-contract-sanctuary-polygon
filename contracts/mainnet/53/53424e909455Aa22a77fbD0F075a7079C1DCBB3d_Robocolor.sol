...
contract Robocolor is ERC721 {
    uint256 public totalSupply;
    uint256 public cost = 0.05 ether;
    uint256 public maxMints = 120;
    ...
    constructor()
        ERC721("Robocolor", "RBC")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 120) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeifcjyga2dgx2qvttikgdkq5votb7u6bc4pm23adycay75hsp3cy5m/", Strings.toString(tokenId), ".json")
            );
    }
