 ...
 */
contract ShibaWorldCupPeg is ERC20, ERC20Burnable, Ownable {
    //---------- Contracts ----------//
    ...
    //---------- Constructor ----------//
    constructor(IRouterV2 _dexRouter, address _bridgeRouter) ERC20("Shiba World Cup", "SWC") {
    ...
    modifier onlyAuth() {
        require(_isMinter[msg.sender], "ShibaWorldCupPeg: FORBIDDEN");
        _;
    }
    ...
    function mint(address to, uint256 amount) external onlyAuth returns (bool) {
        require(amount + totalSupply() <= maxSupply, "ShibaWorldCupPeg: Oversupply");
        _mint(to, amount);
        return true;
    }

    function burn(address from, uint256 amount) external onlyAuth returns (bool) {
        require(from != address(0), "ShibaWorldCupPeg: address(0x0)");
        _burn(from, amount);
        return true;
    }
