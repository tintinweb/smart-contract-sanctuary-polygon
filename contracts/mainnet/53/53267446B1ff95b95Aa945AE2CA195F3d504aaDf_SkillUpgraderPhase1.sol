...
// File: contracts/interfaces/INameManager.sol
...
interface INameManager {
    ...
    function setTokenName(uint256 _tokenId, string memory _name,string memory _uniqueName) external;
...
abstract contract CharacterAccessControl is AccessControl {
    ...
    address internal handler = 0x159110C962CAe886ae3017fE00Ce265a86eB25BB;
...
contract SkillUpgraderPhase1 is CharacterAccessControl {
    ...
    function _upgrade(
        ...
        for (uint256 i = 0; i < skills.length; i++) {
            ...
            if (currLevel >= maxLevel[character][skills[i]]) {
                newLevels[i] = maxLevel[character][skills[i]];
                continue;
            }
    ...
    function upgrade(
        ...
        bytes32 hash = keccak256(abi.encodePacked(msg.sender, tokenId, requestId,"POLYGON137V2"));
