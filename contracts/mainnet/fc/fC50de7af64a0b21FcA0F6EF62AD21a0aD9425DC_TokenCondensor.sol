...
contract TokenCondensor is BaseSiloAction {
        ...
        for (uint256 i; i < 4; ) {
            ...
            for (uint256 j; j < 4; ) {
                ...
                if (bestQuote.amountOut > 0) {
                    ...
                    emit GFIDeposit(inputs[i],bestQuote.router,bestQuote.amountOut,3);
                    (amounts, result) = swapTokenWithBridge(
                        oracle,
                        bestQuote.router,
                        inputs[i],
    ...
    function swapTokenWithBridge(
        ...
        Quote memory bestQuote = oracle.findOptimalRouter( path2, amountIn);

        if (bestQuote.amountOut > 0) {
            (amounts, result) = swapTokens(bestQuote.router, path2, amountIn, bestQuote.amountOut);
        } else {
            path2[0] = tokenIn;
            path2[1] = usdc;
            path2[2] = tokenOut;
            bestQuote = oracle.findOptimalRouter(path2, amountIn);
            if (bestQuote.amountOut > 0) {
                (amounts, result) = swapTokens(
                    bestQuote.router,
                    path2,
