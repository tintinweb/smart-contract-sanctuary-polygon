...
contract DAOFactory is ERC721 {
    ...
    constructor(IUpdateManager updateManagerProxy_, address sudo_) ERC721("Super DAO Factory", "SDF") {
        updateManager = updateManagerProxy_;
        sudo = sudo_;
    }

    function mint(Deployment calldata deployment) external {
        ...
        ERC721OpenSale erc721OpenSale = ERC721OpenSale(
            kernel.deployApp(
                ...
                erc721OpenSaleCode,
                abi.encodeCall(ERC721OpenSale(erc721OpenSaleCode).initialize, (kernel))
            )
        );
        ...
        ERC721WhitelistSale erc721WhitelistSale = ERC721WhitelistSale(
            kernel.deployApp(
                ...
                abi.encodeCall(ERC721WhitelistSale.initialize, (kernel))
            )
        );
        ...
        kernel.addPermission(AppsIds.SUDO, AppsIds.ERC721_OPEN_SALE, ERC721OpenSale(erc721OpenSale).ADMIN());
        kernel.addPermission(
            AppsIds.SUDO,
            AppsIds.ERC721_WHITELIST_SALE,
            ERC721WhitelistSale(erc721WhitelistSale).ADMIN()
        );

        kernel.addPermission(AppsIds.ADMIN, AppsIds.ERC721, ERC721Properties(erc721).ADMIN());
        kernel.addPermission(AppsIds.ADMIN, AppsIds.ERC721_OPEN_SALE, ERC721OpenSale(erc721OpenSale).ADMIN());
        kernel.addPermission(
            AppsIds.ADMIN,
            AppsIds.ERC721_WHITELIST_SALE,
            ERC721WhitelistSale(erc721WhitelistSale).ADMIN()
        ...
        kernel.addPermission(AppsIds.ERC721_OPEN_SALE, AppsIds.ERC721, ERC721Properties(erc721).ADMIN());
        kernel.addPermission(AppsIds.ERC721_WHITELIST_SALE, AppsIds.ERC721, ERC721OpenSale(erc721OpenSale).ADMIN());
