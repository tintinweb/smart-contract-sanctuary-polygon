    ...
    BaseConcentratedLiquidityStrategy
{
        ...
        )
    {
        ...
        if (!_isPositionExists()) {
            ...
            uint256 delta = Math.mulDiv(
                amountForDebt - (aave.ltv * amountForDebt) / 1000000,
                amountForCollateral,
                amountForDebt + (aave.ltv * amountForCollateral) / 1000000
            );
    ...
    function _deposit(uint256 assets) internal virtual override {
        ...
        uint256 tokenToBorrowAmount = aave.supplyAndBorrow(amountToSupply);
    ...
    function _readdLiquidity() internal virtual override {
        ...
        (uint256 neededCollateral, , , ) = getAmounts(assets);

        uint256 neededDebt = pricesOracle.convert(
            aave.collateral,
            aave.tokenToBorrow,
            (neededCollateral * aave.ltv) / 1000000
        );
        ...
        } else if (collateralBalance > neededCollateral) {
            assetConverter.safeSwap(
                aave.collateral,
                aave.tokenToBorrow,
                collateralBalance - neededCollateral
            );
        }
