...
contract BondingCurveOffering is BondingCurve {
    ...
    modifier validBuy(uint256 _amount) {
        ...
        uint256 _taxed_rated = afterTaxedRated(_amount);
        uint256 _tokens = calculateBuyAmount(_taxed_rated);
    ...
    function Purchase(uint256 _amount) public validBuy(_amount) {
        ...
        uint256 _tokens = _purchase(afterTaxedRated(_amount));
        emit TradePrice(1,_tokens * (10 ** USDT_DIGITS) / _amount);
    ...
    function Reimbursement(uint256 _tokens) public validSell(_tokens) {
        ...
        emit TradePrice(0,_tokens * (10 ** USDT_DIGITS) / _amount);
        usdt.transfer(msg.sender, _amount);
    }

    function afterTaxedRated(uint256 _amount)
        public
        view
    ...
    function tax(uint256 _amount) public view returns (uint256 _tax) {
        return (_amount * rate - afterTaxedRated(_amount)) / rate;
    }
}
