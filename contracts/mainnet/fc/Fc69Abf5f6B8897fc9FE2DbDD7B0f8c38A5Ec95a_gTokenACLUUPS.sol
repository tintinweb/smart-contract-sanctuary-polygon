// ce7076bdd96db7c02e2c388f14ba2b558a40aea0
pragma solidity 0.8.17;
import "ACLBase.sol";


contract gTokenACLUUPS is ACLBase {
    string public constant NAME = "gTokenACLUUPS";

    function checkReceiver(address _receiver) internal {
        require(_receiver == safeAddress, "receiver not safe address");
    }

    function deposit(uint, address _receiver) external onlySelf {
        checkReceiver(_receiver);
    }

    function depositWithDiscountAndLock(uint, uint, address _receiver) external onlySelf{
        checkReceiver(_receiver);
    }

    function makeWithdrawRequest(uint, address _owner) external onlySelf{
        checkReceiver(_owner);
    }

    function redeem(uint,address _receiver,address _owner) external onlySelf{
        checkReceiver(_receiver);
        checkReceiver(_owner);
    }

    function unlockDeposit(uint, address receiver) external onlySelf{
