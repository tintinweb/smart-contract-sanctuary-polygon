...
contract DCAAction is BaseSiloAction {
    ...
    constructor(string memory _name, address _siloFactory) {
        name = _name;
        metaData = "address[5],address[5],uint[5],bytes[]";
    ...
    function swapWithUniRoutes(
        SubRoute[] memory subRoutes,
        uint256 inputAmount
    ) private returns (uint256 outputAmount) {
    ...
    function swapAction(
        ...
        (_inputs, _outputs, _investAmounts, subRoutesData) = abi.decode(
            configuration,
            (address[5], address[5], uint256[5], bytes[])
        );
    ...
    function createConfig(
        ...
        configData = abi.encode(_inputs, _outputs, _investAmounts, subRoutes);
    }
    ...
    {
        (_inputs, _outputs, _investAmounts, subRoutes) = abi.decode(
            configData,
            (address[5], address[5], uint256[5], bytes[])
        );
    }
    ...
    function extraInfo(
        ...
        info[0] = gfiGov.pendingEarnings(msg.sender);
        info[3] = MAX_UINT256;
    }
}
