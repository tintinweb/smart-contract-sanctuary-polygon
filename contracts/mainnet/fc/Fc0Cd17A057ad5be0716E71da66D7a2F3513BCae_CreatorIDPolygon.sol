 ...
 */
contract CreatorIDPolygon is ERC20Splitter, ERC721A, Pausable {
    ...
    ERC721A(_name, _symbol)
  {
    require(
      _price > 0,
      "CreatorIDPolygon: Mint price must be greater than zero."
    );
  ...
  /**
   * @notice Mints the desired amount of MAJR IDs to the `to` address and splits the mint fee to the split addresses
   * @param to address
   * @param quantity uint256
   * @dev Can only be called if the max supply has not been reached
   * @dev Can only be called while the contract is not paused
   * @dev User must have enough tokens to mint the desired amount of MAJR IDs
   */
  function mint(address to, uint256 quantity) external whenNotPaused {
    require(
      totalSupply() + quantity <= maxSupply,
      "CreatorIDPolygon Max supply reached."
    );
    require(
      IERC20(token).balanceOf(msg.sender) >= price * quantity,
      "CreatorIDPolygon Not enough tokens to mint."
    );

    bool approved = IERC20(token).approve(address(this), price * quantity);
    require(
      approved,
    ...
    require(sent, "CreatorIDPolygon Failed to send tokens.");

    _safeMint(to, quantity);
    this.split(price * quantity);
  }

  /**
   * @notice Mints the desired amount of MAJR IDs to the `to` address and splits the mint fee to the split addresses & the referrer's address
   * @param to address
   * @param quantity uint256
   ...
   * @dev Can only be called while the contract is not paused
   * @dev User must have enough tokens to mint the desired amount of MAJR IDs
   * @dev The referrer address cannot be the same as the mint recipient address
   */
  function mintWithReferrer(
    ...
    require(
      to != referrer,
      "CreatorIDPolygon Cannot mint with the recipient as the referrer."
    );
    ...
    _safeMint(to, quantity);
    this.referralSplit(price * quantity, referrer);
  }
  ...
  function burn(uint256 _tokenId) external whenNotPaused {
    require(
      ...
      "CreatorIDPolygon You do not own this token."
    );
   ...
   * @dev This function shouldn't encounter the out-of-gas error up to a certain point
   * @dev When the collection grows too big, this function should be replaced by the multiple calls of the tokensOfUserIn method
   */
