...
library SBTLib {
    ...
    function getCommunityToId(
        ...
        if (_storageParams.communityToId[community] == 0) {
            _revert(Errors.COMMUNITY_DOES_NOT_EXIST);
        }
    ...
    function addCommunity(
        ...
        if (_storageParams.nameExist[name] || _storageParams.idExist[id]) {
            _revert(Errors.ALREADY_APPROVED_COMMUNITY);
        }
    ...
    function addApprovedCommunity(
        ...
        if (!_storageParams.nameExist[community]) {
            _revert(Errors.WRONG_COMMUNITY_NAME);
        }
    ...
    function bulkRemoveCommunities(
        ...
        if (communities.length > _communities.length) {
            _revert(Errors.INPUT_LENGTH_IS_GREATER_THAN_TOTAL);
        }
        //
    ...
    function mint(
        ...
        uint id = _storageParams.communityToId[communityName];
        if (id == 0) {
            _revert(Errors.WRONG_COMMUNITY_NAME);
        }
        return id;
...
/**
 * @title OxEQUITYKYC
 * @notice SBT for KYC to allow user to have Legal Property when enabled
 */

contract OxEQUITYKYC is ISBT, ERC1155, AccessControl {
    ...
    string public name = "0xEQUITY KYC SBT";
    string public symbol = "XEQKYC";
    ...
    /**
     * @notice to get the balanceOf user against specific community/Id.
     * @param user address to check the balance
     * @param community name/symbol
     * @return balance of user against a community
     */
    function getBalanceOf(
        address user,
        string memory community
     ...
     * @param _name of the community.
     * @return true: community exist. false: does not exist
     */

    function DoesCommunityExist(
        string memory _name
    ) external view returns (bool) {
        return storageParams.nameExist[_name];
    }

    /**
     * @notice get approved communities for wrapped Legal property
     * @param _symbol of the wrapped Legal property.
     * @return Names of approved communities against Wrapped Legal Property
     */

    function getApprovedSBTCommunities(
        string memory _symbol
    ) external view returns (string[] memory) {
        return storageParams.approvedSBTCommunities[_symbol];
    }
    ...
    function addCommunity(
        ...
        storageParams.addCommunity(_name, id);
        emit CommunityAdded(_name);
    }
    ...
    function removeCommunity(
        ...
        storageParams.removeCommunity(_name);
        emit CommunityRemoved(_name);
    }
    ...
    function addApprovedCommunity(
        ...
        emit ApprovedCommunityAdded(wrappedProperty, community);
    }
    ...
    function bulkApproveCommunities(
        ...
        emit BulkApprovedCommunities(wrappedProperty, communties);
    }
     ...
     * @return true: community exists agaisnt the Wrapped Legal, false:community does not exist agaisnt the Wrapped Legal
     */
    ...
    function bulkRemoveCommunities(
        ...
        emit BulkRemoveCommunities(wrappedProperty, communities);
    }
    ...
    function mint(
        ...
        if (balanceOf(to, id) == 1) {
            _revert(Errors.CANT_MINT_TWICE);
        }
        _mint(to, id, 1, "");
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (!(from == address(0) || to == address(0))) {
            _revert(Errors.TRANSFER_NOT_ALLOWED);
        }
        (operator);
    }
