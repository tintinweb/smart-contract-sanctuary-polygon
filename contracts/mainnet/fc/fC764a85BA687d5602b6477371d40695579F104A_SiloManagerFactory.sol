...
contract SiloManagerFactory is Ownable {
    ...
    address public REGISTRAR_ADDRESS = 0xDb8e8e2ccb5C033938736aa89Fe4fa1eDfD15a1d;//Mainnet
    ...
    //     bytes memory data = abi.encodeWithSelector(FUNC_SELECTOR, name, hex"", address(this), 200000, msg.sender, hex"", amount, SOURCE,address(this));
    ...
    function migrationCancel() external{
        ...
        KeeperRegistryInterface(getKeeperRegistry()).addFunds(id, uint96(migrationBorrow));
    }
    ...
    function migrationWithdraw() external{
        ...
        uint id = ISiloManager(msg.sender).upkeepId();
        require(id != 0, "Manager not approved");
        (,,,uint96 balance,,,uint maxValidBlocknumber,) = KeeperRegistryInterface(getOldKeeperRegistry()).getUpkeep(id);
        uint bal = uint(balance);
        require(block.number > maxValidBlocknumber, "Gravity Finance: Max Valid Block not reached yet");
        if(alphaOrBeta){
        ...
        KeeperRegistryInterface(getOldKeeperRegistry()).withdrawFunds(id, address(this));
        ...
        if(bal > migrationBorrow){//users could technically have their upkeep burn a ton of Link while the migration is happening, but that only benefits keepers
            ...
            KeeperRegistryInterface(getKeeperRegistry()).addFunds(id, uint96(fundsToReturn));
        }
    }
    ...
    function createSiloManager(uint _amount) external{
        ...
        bytes memory data = abi.encodeWithSelector(FUNC_SELECTOR, name, hex"", manager, GAS_LIMIT, address(this), hex"", amount, SOURCE,address(this));
        ERC677Link.transferAndCall(REGISTRAR_ADDRESS, _amount, data);
        managerCount+=1;

        (State memory _state,,) = Registry.getState();

        uint256 id = uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), address(Registry), _state.nonce)));
    ...
    function sponsorUpkeep() external{
        ...
        (,,,uint96 balance,,,uint maxValidBlock,) = KeeperRegistryInterface(getOldKeeperRegistry()).getUpkeep(id);
    ...
    function managerApproved(address _user) external view returns(bool){
        ...
        ISiloManager Manager = ISiloManager(manager);
        uint id = Manager.upkeepId(); 
        (address target,,,,,,uint64 maxValidBlocknumber,) = Registry.getUpkeep(id);
    ...
    function managerCanceled(address _user) external view returns(bool){
        ...
        bool canceled = maxValidBlocknumber != UINT64_MAX && maxValidBlocknumber != 0;
        return canceled;
    }
    ...
    function cancelUpkeep() external{
        ...
        for(uint i=0; i<SiloFactory.balanceOf(msg.sender); i++){
            siloID = SiloFactory.tokenOfOwnerByIndex(msg.sender, i);
            Silo = ISilo(SiloFactory.siloMap(siloID));
            if(Silo.highRiskAction()){
                require(Silo.getStatus() == Statuses.PAUSED, "Call exitSiloStrategy(id,0) before canceling manager");
            }
        }
    ...
    function getOldMaxValidBlockAndBalance(uint _id) external view returns(uint mvb, uint96 bal){
        (,,,bal,,, mvb,) = KeeperRegistryInterface(getOldKeeperRegistry()).getUpkeep(_id);

    }

    function getUsersUpkeepId(address _user) external view returns(uint id) {
        address manager = userToManager[_user];
        require(manager != address(0), "Gravity Finance: _user does not own a manger");
        ISiloManager Manager = ISiloManager(manager);
        id = Manager.upkeepId();
    ...
    function adminCancelManagerUpkeep(uint256 id) external onlyOwner{
        (,,,,,,uint maxValidBlock,) = Registry.getUpkeep(id);
        require(maxValidBlock == UINT64_MAX, "Gravity Finance: Upkeep already cancelled");
        //remove upkeep from sets if in sets
        if(upkeepSetAlpha.contains(id)){upkeepSetAlpha.remove(id);}
        else if(upkeepSetBeta.contains(id)){upkeepSetBeta.remove(id);}
        if(needSponsor.contains(id)){needSponsor.remove(id);}
    ...
    //have it swap ERC677 Link to ERC20
    function adminWithdrawFundsOfManager(bool _linkType,uint256 id, address to) external onlyOwner{
        (,,,uint96 balance,,,uint maxValidBlock,) = Registry.getUpkeep(id);
        require(block.number > maxValidBlock, "Gravity Finance: Max Valid Block not reached yet");
        //remove upkeep from sets if in sets
        if(upkeepSetAlpha.contains(id)){upkeepSetAlpha.remove(id);}
        ...
        if(_linkType){//user wants erc677 link
            ...
            ERC677Link.approve(PEGSWAP_ADDRESS, uint(balance));
            PegSwap.swap(uint(balance), ERC677_LINK_ADDRESS, ERC20_LINK_ADDRESS);
            SafeERC20.safeTransfer(ERC20Link,to, uint(balance));
        }
    }

    function setSiloManagerKeepId(address manager,uint256 id) external onlyOwner{
        ISiloManager(manager).setUpkeepId(id);
    }
...
enum Statuses{ PAUSED, DORMANT, MANAGED }
