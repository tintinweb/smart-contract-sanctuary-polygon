...
contract YamaBridge is NonblockingLzApp {
    ...
    function handleDeposit(
      ...
      if (stargateMsg.endpointId >= router.endpointsLength()) {
        ...
        stargateMsg.amount -= swapper.swapToNative(token, sgFee,
          stargateMsg.amount);
    ...
    function sendStargate(
      ...
      bytes memory payload = generateLzPayload(
        ...
        uint256(layerZero.getOutboundNonce(dstChainId, stargate.bridge()) - 1),
        0,
        0
      );
    ...
    function processLzParams(bytes memory payload) internal returns (
...
contract YamaRouter is Ownable {
  ...
  function deposit(uint256 id, uint256 amount) public {
    endpoints[id].token.transferFrom(msg.sender, address(this), amount);
  ...
  function withdraw(uint256 id, uint256 LPTokenAmount) public returns (
    ...
    uint256 depositorToEndpointRatio = (
      depositorRatio(id) * precision) / endpointRatio(id);
    endpointWithdrawAmount = endpoints[id].endpoint.withdraw(
      endpointWithdrawAmount);
    depositorPay = (
      depositorToEndpointRatio * endpointWithdrawAmount) / precision;
