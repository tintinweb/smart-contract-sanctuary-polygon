...
contract LotteryController is TimelockController, Pausable, ReentrancyGuard {
  ...
  constructor(
      ...
      address[] memory proposers,
      address[] memory executors)
      TimelockController(7 days, proposers, executors)
  {
    token = _token;
    lottery = _lottery;
  ...
  {
    require(block.timestamp >= lastDraw + 6 days + 23 hours);
  ...
  function getUnclaimedRevenue(address account) public view returns (uint256) {
    uint256 cumulatedShares = token.getCumulatedBalance(account);
    uint256 cumulatedTotalSupply = token.getCumulatedTotalSupply();
    uint256 totalValue = address(this).balance + _totalWithdrawn;
    return cumulatedShares * totalValue / cumulatedTotalSupply - _withdrawn[account];
  }

  function withdraw(address payable account, uint256 amount) public whenNotPaused nonReentrant {
    uint256 unclaimed = getUnclaimedRevenue(account);
    require(amount <= unclaimed, 'the requested amount is not available');
...
contract LotteryToken is Ownable, ERC20, ERC20Votes {
  struct UserState {
    uint256 fromBlock;
    uint256 cumulatedBalance;
  }

  uint256 private _cumulatedTotalSupply = 0;
  uint256 private _lastUpdateBlock = block.number;
  ...
  function _updateCumulatedBalance(address account) private {
    UserState storage state = _userStates[account];
    state.cumulatedBalance += balanceOf(account) * (block.number - state.fromBlock);
    state.fromBlock = block.number;
  }

  function _updateCumulatedTotalSupply() private {
    _cumulatedTotalSupply += totalSupply() * (block.number - _lastUpdateBlock);
      ...
      internal override (ERC20)
  {
    ...
    _updateCumulatedBalance(from);
    _updateCumulatedBalance(to);
    _updateCumulatedTotalSupply();
  }
  ...
  function getCumulatedTotalSupply() external view returns (uint256) {
    return _cumulatedTotalSupply + totalSupply() * (block.number - _lastUpdateBlock);
  }

  function getCumulatedBalance(address account) external view returns (uint256) {
    UserState storage state = _userStates[account];
    return state.cumulatedBalance + balanceOf(account) * (block.number - state.fromBlock);
  }
