...
interface ISavingsContractV2 {
  function depositSavings(uint256 _amount, address _beneficiary, address _referrer)
    external
...
contract MStableIntegration is IntegrationInterface, CoinStatsBaseV1 {
  ...
  constructor(uint256 _goodwill, uint256 _affiliateSplit)
    CoinStatsBaseV1(_goodwill, _affiliateSplit)
  {
    // 0x exchange
  ...
  */
  ...
  function _deposit(
    ...
    uint256 imTokensReceived = ISavingsContractV2(savingContract)
      .depositSavings(depositTokenAmount, msg.sender, address(this));
  ...
  */
  function withdraw(
    ...
    _pullTokens(savingsContractAddress, savingsTokenAmount);
...
abstract contract FeesV1 is Ownable {
  ...
  function setNewGoodwill(uint256 _newGoodwill) public onlyOwner {
    require(_newGoodwill >= 0 && _newGoodwill <= 100, "Invalid goodwill value");
  ...
  function setShouldResetAllownace(
    ...
    for (uint256 i = 0; i < tokens.length; i++) {
      approvedTargets[tokens[i]] = statuses[i];
    }
  }
