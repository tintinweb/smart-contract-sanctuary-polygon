...
contract Game1Manager is KeeperCompatibleInterface, AccessControl {
  ...
  uint72[5] private TOKENS_FOR_PERIOD = [
    0,
    10 * (10**18),
    25 * (10**18),
    50 * (10**18),
  ...
  bool public locked;
  uint public cadenceSecs;
  uint public startTimestamp;
  uint public lastStartTimestamp;
  GameStartArgs public args;

  mapping(uint => mapping(uint => uint)) private redeemedRewards;

  constructor(
    address _gameAddress,
    address _cozyCoinAddress,
    address _cozyRegistryAddress
  ) {
    game = Game1(_gameAddress);
    coin = CozyCoin(_cozyCoinAddress);
  ...
  function setLastStartTimestamp(uint _lastStartTimestamp)
    public
  ...
  function setSchedule(uint _startTimestamp, uint _cadenceSecs)
    public
    onlyRole(OPERATOR_ROLE)
    notLocked
  {
    ...
    cadenceSecs = _cadenceSecs;
  }

  function setGameStartArgs(GameStartArgs memory _args) public onlyRole(OPERATOR_ROLE) notLocked {
    args = _args;
  }

  function getWindowForTimestamp(
    uint timestamp,
    uint start,
    uint cadence
    ...
    uint window = (timestamp - start) / cadence;
    return window * cadence + start;
  }

  function isScheduleReadyForTimestamp(
    uint timestamp,
    uint lastStart,
    uint start,
    uint cadence
  ) public pure returns (bool) {
    uint windowStart = getWindowForTimestamp(timestamp, start, cadence);
    return lastStart < windowStart && timestamp >= windowStart;
  }

  function getNextWindowTimestamp() public view returns (uint) {
    return getWindowForTimestamp(block.timestamp, startTimestamp, cadenceSecs);
  }

  function isGameReady() public view returns (bool) {
    GameState memory gs = game.getGameState();
    return gs.phase == Phase.NONE || gs.phase == Phase.END;
  }

  function isScheduleReady() public view returns (bool) {
    return
      isScheduleReadyForTimestamp(block.timestamp, lastStartTimestamp, startTimestamp, cadenceSecs);
  }

  function canStartGame() public view returns (bool) {
    return isScheduleReady() && isGameReady();
  }

  function startGame() internal notLocked {
    game.startGame(args);
    lastStartTimestamp = block.timestamp;
  }
  ...
  struct GameResult {
    uint gameID;
    uint redeemableTokens;
    bool redeemed;
    bool redeemable;
  }

  function getGameHistory(uint tokenID) public view returns (GameResult[] memory) {
    uint maxGameID = game.getCurrentGameID();
    GameResult[] memory result = new GameResult[](maxGameID);

    for (uint i = 1; i <= maxGameID; i++) {
      result[i - 1] = getGameResult(tokenID, i);
    }
    return result;
  }

  function getGameResult(uint tokenID, uint gameID)
    public
    view
    returns (GameResult memory gameResult)
  {
    PlayerState memory playerState = game.getPlayerStatForGame(tokenID, gameID);
    if (playerState.culledOnSweep == 0 && playerState.wonAtSweep == 0) {
      return gameResult;
    }
    gameResult.redeemable = true;
    gameResult.redeemed = redeemedRewards[tokenID][gameID] > 0;
    gameResult.gameID = gameID;

    GameState memory gs = game.getGameStateForGame(gameID);
    uint deathPeriod = playerState.culledOnSweep / gs.sweepsPerPeriod;
    if (playerState.wonAtSweep > 0) {
      gameResult.redeemableTokens = TOKENS_FOR_PERIOD[TOKENS_FOR_PERIOD.length - 1];
    } else if (deathPeriod < TOKENS_FOR_PERIOD.length) {
      gameResult.redeemableTokens = TOKENS_FOR_PERIOD[deathPeriod];
    } else {
      gameResult.redeemableTokens = TOKENS_FOR_PERIOD[TOKENS_FOR_PERIOD.length - 1];
    }
  }

  function redeemRewards(uint tokenID, uint gameID) public notLocked {
    if (msg.sender != registry.ownerOf(tokenID)) {
      revert InvalidToken();
    }
    // ensure the player hasn't already redeemed
    GameResult memory result = getGameResult(tokenID, gameID);
    ...
    if (result.redeemableTokens <= 0) {
      revert NothingToRedeem();
    }

    // redeem
    coin.mint(msg.sender, result.redeemableTokens);

    // mark redeemed
    redeemedRewards[tokenID][gameID] = result.redeemableTokens;
  }

  function lock() public onlyRole(OPERATOR_ROLE) {
    locked = true;
  }
