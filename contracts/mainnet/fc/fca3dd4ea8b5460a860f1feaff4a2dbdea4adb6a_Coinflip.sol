...
contract Coinflip is Ownable, VRFConsumerBase {
    ...
    uint public BraverfeeBalance;
    uint public StarfeeBalance;
    uint public feeBalance;

    uint public minBetAmount = 2 ether;
    uint public betFee = 5;
    address star = 0x4fd917bB549dA0201cb146636cd2a4Dc1C305Fe5;
    ...
    function changeFees(uint feechange) public payable {
    ...
    *@notice The following functions are reserved for the owner of the contract.
    ...
    function withdrawFees() public onlyOwner {
        uint toTransfertoOwner = SafeMath.div(SafeMath.mul(feeBalance,60),100);
        uint toTransfertoStar = SafeMath.div(SafeMath.mul(feeBalance,40),100);

        feeBalance = 0;
        payable(star).transfer(toTransfertoStar);

        payable(msg.sender).transfer(toTransfertoOwner);
    }
