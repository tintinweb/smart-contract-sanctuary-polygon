...
contract LaunchPad is Operator, ILaunchPad {
    using SafeMath for uint256;
    using SafeERC20 for ERC20;

    struct InvestorInfo {
        uint256 sTokenAllocation;
        uint256 rTokenToBeSpent;
        uint256 rTokenUnSpent;
    }
    ...
    function getInvestorInfo(address investor) external view returns (
        ...
        return (
        _staked = stakedBalances[investor],
        _sTokenAllocation = getAllocation(investor),
    ...
    function initialize(
        ...
        fundToRaise = _sTokenAmount.mul(priceNumerator).div(priceDenominator);
    ...
    function commitToken(uint256 _amount) public override checkLaunchPadRunning initialized {
    ...
    function claimToken() public override checkTimeEnd initialized {
        require(stakedBalances[msg.sender] > 0, "You were not participated in launchpad");
    ...
    function useRaisedFund() public override checkTimeEnd initialized onlyOperator {
    ...
    function updateInvestorInfo(address investor) internal {
        ...
        investorInfo.sTokenAllocation = getAllocation(investor);
        ...
        investorInfo.rTokenUnSpent = stakedBalances[investor].sub(investorInfo.rTokenToBeSpent);
        investorInfos[investor] = investorInfo;
    }

    function getAllocation(address investor) internal view returns (uint256) {
        uint256 investorCommitted = stakedBalances[investor];
        uint256 allocation = investorCommitted.mul(allocationPerCommit).div(1e18).mul(10 ** missing_decimal);
        return allocation;
    }
