...
contract PepeKing is ERC20, Ownable {
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (_isAutomatedMarketMakerPair[from]) {
                ...
                fees =
                    (amount *
                        (devTaxSell + marketingTaxSell + liquidityTaxSell)) /
                    denominator;
            } else {
                fees =
                    (amount *
    ...
    function swapBack() internal inSwap {
        ...
        uint256 liquifyToken = (contractTokenBalance *
            ((liquidityTaxSell + liquidityTaxTransfer) / 2)) / totalTax;

        uint256 swapBackAmount = contractTokenBalance - liquifyToken;

        totalTax -= (liquidityTaxSell + liquidityTaxTransfer) / 2;
        ...
        uint256 devBNB = (newBalance * (devTaxSell + devTaxTransfer)) /
            totalTax;
        uint256 marketingBNB = (newBalance *
            (marketingTaxSell + marketingTaxTransfer)) / totalTax;
