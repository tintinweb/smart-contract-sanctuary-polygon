...
import "../../../../interfaces/balancer/Maximizer/IMaximizerBalancer.sol";

import "./StrategyBalancerAC.sol";

/**
 * @dev Called by the Maximizer contract; contains functions for swapping, zapping,
        removing liquidity, and used to store swapping paths and other router-related info
 */
contract BalancerRouterUtils is Initializable, OwnableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    address public secondaryRouter;
    IMaximizerBalancer public maximizer;
    ...
    // Routes
    address[] public primaryLp0ToNativeRoute;
    address[] public primaryLp1ToNativeRoute;
    address[] public nativeToSecondaryLpToken0Route;
    address[] public nativeToSecondaryLpToken1Route;

    // poolId of each pool in the route - Balancer
    bytes32[] public primaryLp0ToNativeRoutePids;
    bytes32[] public primaryLp1ToNativeRoutePids;

    // pool type: stable/volatile - Dystopia
    bool[] public isStableNativeToSecondaryLpToken0;
    bool[] public isStableNativeToSecondaryLpToken1;
    bool public isStableSecondaryLp0LP1;

    /**
     * @param _secondaryRouter Secondary Router's Address
     * @param _primaryLp0ToNativeRoute Array of address. Path along which to swap through
     * @param _primaryLp1ToNativeRoute Array of address. Path along which to swap through
     * @param _primaryLp0ToNativeRoutePids Array of bytes32 pool ids. A pool id for each swap,
                                           thus its length would be one less than the swapping path's length
     * @param _primaryLp1ToNativeRoutePids Array of bytes32 pool ids. A pool id for each swap,
                                           thus its length would be one less than the swapping path's length
     * @param _nativeToSecondaryLpToken0Route Array of address. Path along which to swap through
     * @param _nativeToSecondaryLpToken1Route Array of address. Path along which to swap through
     * @param _isStableNativeToSecondaryLpToken0 Array of bool. bool indicates whether the pool,
                                                 through which the swapping is taking place, is stable or not.
                                                 Its length would be one less than the swapping path's length
     * @param _isStableNativeToSecondaryLpToken1 Array of bool. bool indicates whether the pool,
                                                 through which the swapping is taking place, is stable or not.
                                                 Its length would be one less than the swapping path's length
     * @param _isStableSecondaryLp0LP1 Array of bool. bool indicates whether the pool, through which the swapping is taking place,
                                       is stable or not. Its length would be one less than the swapping path's length
     */
    function __BalancerRouterUtils_init(
        address _secondaryRouter,
        address[] memory _primaryLp0ToNativeRoute,
        address[] memory _primaryLp1ToNativeRoute,
        bytes32[] memory _primaryLp0ToNativeRoutePids,
        bytes32[] memory _primaryLp1ToNativeRoutePids,
        address[] memory _nativeToSecondaryLpToken0Route,
        address[] memory _nativeToSecondaryLpToken1Route,
        bool[] memory _isStableNativeToSecondaryLpToken0,
        bool[] memory _isStableNativeToSecondaryLpToken1,
        bool[] memory _isStableSecondaryLp0LP1
    ) public initializer {
        __Ownable_init_unchained();
        __BalancerRouterUtils_init_unchained(_secondaryRouter,
            _primaryLp0ToNativeRoute, _primaryLp1ToNativeRoute,
            _primaryLp0ToNativeRoutePids, _primaryLp1ToNativeRoutePids, _nativeToSecondaryLpToken0Route, _nativeToSecondaryLpToken1Route,
            _isStableNativeToSecondaryLpToken0, _isStableNativeToSecondaryLpToken1, _isStableSecondaryLp0LP1);
    }

    function __BalancerRouterUtils_init_unchained(
        address _secondaryRouter,
        address[] memory _primaryLp0ToNativeRoute,
        address[] memory _primaryLp1ToNativeRoute,
        bytes32[] memory _primaryLp0ToNativeRoutePids,
        ...
        secondaryRouter = _secondaryRouter;

        require(_primaryLp0ToNativeRoute.length == _primaryLp0ToNativeRoutePids.length + 1, "route length != routePids length + 1");
        require(_primaryLp1ToNativeRoute.length == _primaryLp1ToNativeRoutePids.length + 1, "route length != routePids length + 1");
        require(_isStableNativeToSecondaryLpToken0.length == _nativeToSecondaryLpToken0Route.length - 1,
            '_isStableNativeToSecondaryLpToken0.length != _nativeToSecondaryLpToken0Route.length - 1');
        require(_isStableNativeToSecondaryLpToken1.length == _nativeToSecondaryLpToken1Route.length - 1,
            '_isStableNativeToSecondaryLpToken1.length != _nativeToSecondaryLpToken1Route.length - 1');
        require(_isStableSecondaryLp0LP1.length == 1, '_isStableSecondaryLp0LP1.length != 1');

        primaryLp0ToNativeRoute = _primaryLp0ToNativeRoute;
        primaryLp1ToNativeRoute = _primaryLp1ToNativeRoute;
        primaryLp0ToNativeRoutePids = _primaryLp0ToNativeRoutePids;
        primaryLp1ToNativeRoutePids = _primaryLp1ToNativeRoutePids;

        nativeToSecondaryLpToken0Route = _nativeToSecondaryLpToken0Route;
        nativeToSecondaryLpToken1Route = _nativeToSecondaryLpToken1Route;
        isStableNativeToSecondaryLpToken0 = _isStableNativeToSecondaryLpToken0;
        isStableNativeToSecondaryLpToken1 = _isStableNativeToSecondaryLpToken1;

        isStableSecondaryLp0LP1 = _isStableSecondaryLp0LP1[0];
    }

    /**
     * To perform swapping through Balancer exchange
     * @param amount The amount to be swapped
     * @param route Array of address; routing path along which to swap
     * @param routePids Array of bytes32; pool id for each pair in `route`; its length is one less than that of `route`
     */
    function swapBalancer(uint256 amount, address[] memory route, bytes32[] memory routePids) public {
        address primaryRouter = maximizer.primaryVault().strategy().router();
        ...
        if (route.length == 2) {
            ...
            IRouterVault(primaryRouter).swap(singleSwap, funds, 0, type(uint256).max);
        } else {
            ...
            IRouterVault(primaryRouter).batchSwap(IRouterVault.SwapKind.GIVEN_IN, swaps, route, funds, limits, type(uint256).max);
        }
    }

    /**
     * To remove liquidity from a Balancer pool
     * @param primaryWantAmount number of balancer pool receipts to remove liquidity for
     * @dev userData [EXACT_BPT_IN_FOR_ONE_TOKEN_OUT, bptAmountIn, exitTokenIndex]
     */
    function exitPoolBalancer(uint256 primaryWantAmount) public {
        bytes32 poolId = maximizer.primaryVault().strategy().poolId();
        address primaryRouter = maximizer.primaryVault().strategy().router();

        address[] memory assets;
        (assets,,) = IRouterVault(primaryRouter).getPoolTokens(poolId);

        uint256[] memory minAmountsOut = new uint256[](assets.length);

        uint256 nativeIndex;
        for (uint256 j = 0; j < assets.length;) {
            if (assets[j] == address(native)) {
                nativeIndex = j;
                break;
            }
        unchecked {
        ...
        bytes memory userData = abi.encode(0, primaryWantAmount, nativeIndex);

        IRouterVault.ExitPoolRequest memory request = IRouterVault.ExitPoolRequest({
        assets : assets,
        minAmountsOut : minAmountsOut,
        userData : userData,
        toInternalBalance : false
        });
        IRouterVault(primaryRouter).exitPool(poolId, address(this), payable(address(this)), request);
    }

    /**
     * To zap all primary want present in this contract to the native token
     * @param primaryWantAmount amount of primary want token that is to be zapped
     */
    function zapPrimaryWantToNative(uint256 primaryWantAmount) public {
        uint256 nativeBalanceBefore = native.balanceOf(address(this));
        maximizer.primaryVault().want().safeTransferFrom(msg.sender, address(this), primaryWantAmount);
        exitPoolBalancer(primaryWantAmount);
        if (primaryLpToken0 != address(native)) {
            uint256 primaryLpToken0Bal = IERC20Upgradeable(primaryLpToken0).balanceOf(address(this));
            if (primaryLpToken0Bal > 0) {
                swapBalancer(primaryLpToken0Bal, getPrimaryLp0ToNativeRoute(), getPrimaryLp0ToNativeRoutePids());
            }
        }

        if (primaryLpToken1 != address(native)) {
            uint256 primaryLpToken1Bal = IERC20Upgradeable(primaryLpToken1).balanceOf(address(this));
            if (primaryLpToken1Bal > 0) {
                swapBalancer(primaryLpToken1Bal, getPrimaryLp1ToNativeRoute(), getPrimaryLp1ToNativeRoutePids());
            }
        }
        native.safeTransfer(address(maximizer), native.balanceOf(address(this)) - nativeBalanceBefore);
    }

    function getPrimaryLp0ToNativeRoute() public view returns (address[] memory) {
        return primaryLp0ToNativeRoute;
    }

    function getPrimaryLp1ToNativeRoute() public view returns (address[] memory) {
        return primaryLp1ToNativeRoute;
    }

    function getNativeToSecondaryLpToken0Route() public view returns (address[] memory) {
        return nativeToSecondaryLpToken0Route;
    }

    function getNativeToSecondaryLpToken1Route() public view returns (address[] memory) {
        return nativeToSecondaryLpToken1Route;
    }

    function getPrimaryLp0ToNativeRoutePids() public view returns (bytes32[] memory) {
        return primaryLp0ToNativeRoutePids;
    }

    function getPrimaryLp1ToNativeRoutePids() public view returns (bytes32[] memory) {
        return primaryLp1ToNativeRoutePids;
    }

    function getIsStableNativeToSecondaryLpToken0() public view returns (bool[] memory) {
        return isStableNativeToSecondaryLpToken0;
    }

    function getIsStableNativeToSecondaryLpToken1() public view returns (bool[] memory) {
        return isStableNativeToSecondaryLpToken1;
    }

    function getIsStableSecondaryLp0LP1() public view returns (bool) {
        return isStableSecondaryLp0LP1;
    }

    /**
     * To update the maximizer contract address that this contract depends on
     * @param maximizerAddress Address of the new maximizer contract
     */
    function setMaximizer(address maximizerAddress) public onlyOwner {
        maximizer = IMaximizerBalancer(maximizerAddress);
        native = IERC20Upgradeable(maximizer.native());
        primaryLpToken0 = maximizer.primaryLpToken0();
        primaryLpToken1 = maximizer.primaryLpToken1();

        _giveAllowances();
    }

    function setIsStableSecondaryLp0LP1(bool isStable) public onlyOwner {
        isStableSecondaryLp0LP1 = isStable;
    }

    /**
     * @dev primaryRouter already has the allowance for primary want
     */
...
contract StrategyBalancerAC is Initializable, StratManagerUpgradeableCommon, DynamicFeeManager {
    ...
    function __StrategyBalancerAC_init(
        ...
        __StrategyBalancerAC_init_unchained(_want, _rewardPool, _outputToNativeRoute, _outputToLp0Route, _outputToLp1Route, _routePids);
    }
    ...
    /// @dev assuming last elm of maxAmountsIn corresponds to the pool receipt token
