...
contract NFT is ERC1155 ,Ownable{
    ...
    uint256 constant ownerRoyaltyRate = 300;
    ...
    mapping(uint256=>address) exist;
    mapping(address=>uint256) balances;
    mapping(address=>uint256[]) createTokenId;
    mapping(address=>string) baseMetadataURIPrefix;
    mapping(address=>mapping(uint256=>uint256)) tokenExpirationTimestamp;
    ...
    function getOwnNFTList(address _creater) external view returns(uint256 [] memory , uint256 [] memory, uint256 [] memory){
        ...
        for (uint256 _i = 0; _i < _length; _i++){
            ...
            if(_NFTData[_id].expiration == 0){
                _tokenExpirationTimestamp[_i] = 9999999999999;
            }else{
                _tokenExpirationTimestamp[_i] = tokenExpirationTimestamp[msg.sender][_id];
            }
        }
    ...
    function mint(uint256 _quantity,uint256 _cost,uint256 _expiration) external{
        ...
        tokenExpirationTimestamp[msg.sender][_id] = 0;
    ...
    function setURI(string memory _uri) external {
        require(keccak256(abi.encodePacked(_uri)) != keccak256(abi.encodePacked("")),unicode"URIが入力されていません。");
    ...
    function getExpiration(uint256 _id) external view returns (uint256) {
        uint256 _expiration = tokenExpirationTimestamp[msg.sender][_id];
        return _expiration;
    }
    ...
    function transferFromCreater(
        ...
        require(_creater != _to ,"The delivery address is invalid.");
        ...
        if(_expiration != 0){
            _expirationTimestamp = _getTokenExpirationTimestamp(_id,_quantity,_expiration);
            tokenExpirationTimestamp[_to][_id] = _expirationTimestamp;
        }
    ...
    function _getTokenExpirationTimestamp(uint256 _id,uint256 _quantity,uint256 _expiration) private view returns (uint256) {
        ...
        uint256 _existingExpiration = tokenExpirationTimestamp[msg.sender][_id];
        uint256 baseTimestamp = block.timestamp;
        if(_existingExpiration > baseTimestamp){
            baseTimestamp = _existingExpiration;
        }

        uint256 addTime = _quantity * _expiration * 24 * 60 * 60;
