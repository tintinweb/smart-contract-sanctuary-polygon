...
contract MaticHeavy {
	...
	uint constant public			INVEST_MIN_AMOUNT				= 10 ether;				// 10 MATIC
	uint[] public					REFERRAL_PERCENTS				= [60, 30, 10];			// 6% 3% 1%
	...
	constructor(address payable _walletProject) {
		...
		startUNIX = 1651413600; // project will launch on Sun, May 1 2022 14:00:00 GMT+0000
		
		PLANS.push( TPlan(14,90) );
		PLANS.push( TPlan(21,85) );
	...
	function claim() public {
		...
		uint claimAmount = _getUserDividends(msg.sender) + user.refDividends;
	...
	function getUserInfo(address _user) public view returns (TPlanInfo memory o_planInfo, TRefInfo memory o_refInfo, TUserInfo memory o_userInfo, uint o_timestamp) {
		...
		//user stats
		o_userInfo.claimable = o_planInfo.dividends + o_refInfo.dividends;
