    ...
    Ownable
{
    ...
    function setApprovalForAll(
        address operator,
        bool approved
    ) public override checkLock onlyAllowedOperatorApproval(operator) {
    ...
    function approve(
        address operator,
        uint256 tokenId
    ) public payable override checkLock onlyAllowedOperatorApproval(operator) {
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public payable override checkLock onlyAllowedOperator(from) {
        ...
        if (from == address(0)) {
            if (quantity > 1) {
                for (uint256 i = 0; i < quantity; i++) {
                    if (isLocked) {
                        emit Locked(startTokenId + i);
                    } else {
                        emit Unlocked(startTokenId + i);
                    }
                }
            } else {
                if (isLocked) {
