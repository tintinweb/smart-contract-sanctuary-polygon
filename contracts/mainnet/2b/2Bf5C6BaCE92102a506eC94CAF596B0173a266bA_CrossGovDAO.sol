...
contract CrossGovDAO is OwnableUpgradeable, IDAOCrossGovInterface, IERC165Upgradeable {
    ...
    uint public constant PRICE_DECIMALS = 1e12;

    address public voting;// contract address of voting
    address public snapshotTokenAddress;
    string public name;
    string public desc;
    string public baseNetwork;
    string public website;
    string public twitter;
    string public discord;
    string public tokenLogo;

    IDAOCrossGovInterface.DAORule public rule;

    event UpdateDaoRule(uint minimumVote, uint minimumCreateProposal, uint minimumValidVotes, uint communityVotingDuration, uint contractVotingDuration, string content);

    modifier onlyVoting() {
        require(voting == msg.sender, "caller is not the voting");
        _;
    }

    function initialize(IDAOCrossGovInterface.DAOBasic memory basic,
        IDAOCrossGovInterface.DAORule memory _rule, address _voting) initializer override public {
        desc = basic.daoDesc;
        website = basic.website;
        twitter = basic.twitter;
        discord = basic.discord;
        name = basic.name;
        snapshotTokenAddress = basic.contractAddress;
        tokenLogo = basic.tokenLogo;
        rule = _rule;
        voting = _voting;
        __Ownable_init();
    }

    function getDaoRule() view external override returns (IDAOCrossGovInterface.DAORule memory) {
        return rule;
    }

    //update dao rule
    function updateDaoRule(DAORule memory _rule) override public onlyVoting {
        rule.minimumVote = _rule.minimumVote;
        rule.minimumCreateProposal = _rule.minimumCreateProposal;
        rule.minimumValidVotes = _rule.minimumValidVotes;
        rule.communityVotingDuration = _rule.communityVotingDuration;
        rule.contractVotingDuration = _rule.contractVotingDuration;
        rule.content = _rule.content;
        emit UpdateDaoRule(_rule.minimumVote, _rule.minimumCreateProposal, _rule.minimumValidVotes,
            _rule.communityVotingDuration, _rule.contractVotingDuration, _rule.content
        );
    }

    //*****  Only through the proposal can the following operations be performed.*****
    function supportsInterface(bytes4 interfaceId) override pure public returns (bool) {
        return interfaceId == IDAOCrossGovInterface.updateDaoRule.selector;
    }

    function getDaoToken() override pure external returns (address) {
        return address(0);
    }
}
