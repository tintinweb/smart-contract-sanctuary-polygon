...
contract DexGoNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, ERC721Burnable {
    ...
    constructor(uint256 networkId, address _storageContract) ERC721("dexGoNFT", "DEXGO") {
        ...
        if (networkId == 137 || networkId == 1313161554) startPrice = 10 ether;
    ...
    function _safeMintType(address to, string memory uri, uint8 typeNft, bool needURI) private {
        ...
        IDexGoStorage(storageContract).setKmForId(tokenId, IDexGoStorage(storageContract).getKmForType(typeNft));
    }
    ...
    function convertFromPastContracts(address pastContract, uint[] memory tokenIds) public nonReentrant {
    ...
    function getIsIGO() public view returns (bool) {
        return isIGO;
    }

    event Purchase(address indexed from, uint256 value, uint8 typeNft, uint8 finalTypeNft, address referrer, uint valueForOwner, address token);
    function _purchaseFinalize(address sender, address referrer) private {
        if (referrer == address (0)) require(sender == IDexGoStorage(storageContract).getAccountTeam1() || sender == IDexGoStorage(storageContract).getAccountTeam2() || sender == owner(), "No referrer allwed only for top accounts");
    ...
    function purchaseToken(uint8 typeNft, address referrer, string memory _inAppPurchaseInfo, address token) public nonReentrant {
        require(IDexGoStorage(storageContract).isTokenAllowed(token) == true, "NA");
        ...
        uint valueForOwner = IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).distributeMoney(msg.sender, IDexGoStorage(storageContract).getPriceForTypeToken(finalTypeNft, token), bytes(_inAppPurchaseInfo).length > 0, token);
        ...
        emit Purchase(msg.sender, IDexGoStorage(storageContract).getPriceForTypeToken(finalTypeNft, token), typeNft, finalTypeNft, referrer, valueForOwner, token);
        _purchaseFinalize(msg.sender, referrer);
    }
    function purchaseBatchToken(uint8[] memory typesNft, address referrer, string[] memory _inAppPurchaseInfo, address token) public nonReentrant {
        require(IDexGoStorage(storageContract).isTokenAllowed(token) == true, "NA");
        uint value;
        for(uint256 y=0;y<typesNft.length;y++) {
            ...
            value = value + IDexGoStorage(storageContract).getPriceForTypeToken(finalTypeNft, token);
        }
//        require(IERC20(token).allowance(msg.sender, IDexGoStorage(storageContract).getHandshakeLevels()) >= value, "WV");
        uint valueForOwner = IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).distributeMoney(msg.sender, value, bytes(_inAppPurchaseInfo[0]).length > 0, token);
        for(uint256 y=0;y<typesNft.length;y++) {
            uint8 finalTypeNft = typesNft[y];
            if (typesNft[y] == MAGIC_BOX) {
                finalTypeNft = uint8(random());
            ...
            emit Purchase(msg.sender, value,  typesNft[y], finalTypeNft, referrer, valueForOwner, token);
        }
        _purchaseFinalize(msg.sender, referrer);
    }


    function purchase(uint8 typeNft, address referrer, string memory _inAppPurchaseInfo) payable public nonReentrant {
        ...
        uint valueForOwner = IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).distributeMoney(msg.sender, msg.value, bytes(_inAppPurchaseInfo).length > 0, address(0));
        IDexGoStorage(storageContract).setInAppPurchaseData(_inAppPurchaseInfo, _tokenIdCounter.current());
        _safeMintType(msg.sender, "", finalTypeNft, false);
        emit Purchase(msg.sender, msg.value, typeNft, finalTypeNft, referrer, valueForOwner, address(0));
        _purchaseFinalize(msg.sender, referrer);
    }
    ...
    function purchaseBatch(uint8[] memory typesNft, address referrer, string[] memory _inAppPurchaseInfo) payable public nonReentrant {
        require(msg.value >= purchaseBatchValue(typesNft).sub(IDexGoStorage(storageContract).getValueDecrease()), "WV");
        Address.sendValue(payable(IDexGoStorage(storageContract).getHandshakeLevels()), msg.value);
        uint valueForOwner = IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).distributeMoney(msg.sender, msg.value, bytes(_inAppPurchaseInfo[0]).length > 0, address(0));
        for(uint256 y=0;y<typesNft.length;y++) {
            uint8 finalTypeNft = typesNft[y];
            uint256 price = IDexGoStorage(storageContract).valueInMainCoin(finalTypeNft);
            if (typesNft[y] == MAGIC_BOX) {
                finalTypeNft = uint8(random());
            }
            _safeMintType(msg.sender, "", finalTypeNft, false);
            IDexGoStorage(storageContract).setInAppPurchaseData(_inAppPurchaseInfo[y], _tokenIdCounter.current());
            emit Purchase(msg.sender, price,  typesNft[y], finalTypeNft, referrer, valueForOwner, address(0));
        }
        _purchaseFinalize(msg.sender, referrer);
    }
