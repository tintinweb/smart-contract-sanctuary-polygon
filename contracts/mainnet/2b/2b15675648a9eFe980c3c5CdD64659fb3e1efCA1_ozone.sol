...
contract ozone is ERC20BurnableUpgradeable, Operator {
    using SafeMath8 for uint8;
    using SafeMathUpgradeable for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 EART-WETH LP -> ozone Pool
    uint256 public constant INITIAL_LP_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the EARHT ERC-20 contract.
     */

    function _init_Token(uint256 _taxRate, address _taxCollectorAddress)
        public
        initializer
    {
        ...
        createOperator();
        __ERC20_init("ozone Token", "OZONE");
    ...
    function _getOzonePrice() internal view returns (uint256 _ozonePrice) {
        try IOracle(_ozoneOracle).consult(address(this), 1e18) returns (
            uint144 _price
        ) {
            return uint256(_price);
        } catch {
            revert("ozone: failed to fetch ozone price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _ozonePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (
                ...
                if (_ozonePrice >= taxTiersTwaps[tierId]) {
                    require(
    ...
    function __init_oracle(address _evmoon_Oracle)
        public
        onlyOperatorOrTaxOffice
    {
        require(
            _evmoon_Oracle != address(0),
            "oracle address cannot be 0 address"
        );
        _ozoneOracle = _evmoon_Oracle;
    }
    ...
    /**
     * @notice Operator mints ozone to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of ozone to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentozonePrice = _getOzonePrice();
            currentTaxRate = _updateTaxRate(currentozonePrice);
            if (currentozonePrice < burnThreshold) {
                burnTax = true;
            }
        }
     ...
     */
    function __distriuteAirdrop(address _airdropRewardsAddress)
        external
        onlyOperator
    {
        require(!rewardPoolDistributed, "only can distribute once");
        require(
            _airdropRewardsAddress != address(0),
            "!_airdropRewardsAddress"
        );
        rewardPoolDistributed = true;
    }

    function __distributPoolGenesis(address _rewardsPoolAddress)
        external
        onlyOperator
    {
        require(_rewardsPoolAddress != address(0), "!_rewardsPoolAddress");
        rewardPoolDistributed = true;
        _mint(_rewardsPoolAddress, INITIAL_GENESIS_POOL_DISTRIBUTION);
    }

    function __distributePoolLp(address _rewardsPoolLPAddress)
        external
        onlyOperator
    {
        require(_rewardsPoolLPAddress != address(0), "!_rewardsPoolLPAddress");
        rewardPoolDistributed = true;
        _mint(_rewardsPoolLPAddress, INITIAL_LP_POOL_DISTRIBUTION);
    }
