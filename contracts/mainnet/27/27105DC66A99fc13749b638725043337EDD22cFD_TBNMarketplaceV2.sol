...
contract TBNMarketplaceV2 is ERC721Holder, AccessControl, ReentrancyGuard {
  ...
  EnumerableSet.AddressSet private tbnNfts;
  ...
  */
  ...
  //Add to list of valid TBN NFTs
  function addTbnNftAddress(address _tbnNFTAddress) external onlyAdmin {
    tbnNfts.add(_tbnNFTAddress);
  }

  //Remove from list of valid TBN NFTs
  function removeTbnNftAddress(address _tbnNFTAddress) external onlyAdmin {
    tbnNfts.remove(_tbnNFTAddress);
  }

  //Check if address is a valid TBN NFT
  function containsTbnNftAddress(address _tbnNFTAddress)
    external
    returns (bool isContained)
  {
    isContained = tbnNfts.contains(_tbnNFTAddress);
  }
  ...
  function listTBNTokens(
    ...
    require(tbnNfts.contains(nftTokenAddress), "Only TBN NFTs can be listed");
  ...
  function buyToken(uint256 listingId) external payable nonReentrant {
    ...
    if (
      ...
      /** Calculation of price
                get reserves of tbn and payment tokens
                tbnReservePrice = tbnReserve / WETH reserve 
                paymentReservePrice = paymentReserve / WETH reserve
                exchangeRate = paymentReservePrice / tbnReservePrice
                fullPrice = exchangeRate * # of tokens in TBN
            */
