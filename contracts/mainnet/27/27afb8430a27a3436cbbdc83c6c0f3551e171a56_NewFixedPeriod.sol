...
contract NewFixedPeriod is
  ...
  function mintEth() public payable nonReentrant returns (uint256 tokenId) {
    ...
    // refund
    if (msg.value - rate > 0) {
      (bool success, ) = payable(_msgSender()).call{value: msg.value - rate}(   // update
        ""
      );
    ...
    if (platform != address(0)) {
      ...
      // Accept the assets
      (bool result, ) = payable(receivingAddress).call{value: rate - ((rate * (platformRate)) / 100)}("");
      require(result, "Failed to send Ether");
    } else {
      // Accept the assets
      (bool result, ) = payable(receivingAddress).call{value: rate}("");
      require(result, "Failed to send Ether");
    }
  ...
  function withdraw() public nonReentrant  {
    if (address(erc20) == address(0)) {
      ...
      (bool success, ) = payable(receivingAddress).call{value: amount}(""); // withdraw ETH to platform account
      require(success, "Failed to send Ether");

      emit Withdraw(receivingAddress, amount);
    } else {
      uint256 amount = IERC20Upgradeable(erc20).balanceOf(address(this));
      IERC20Upgradeable(erc20).safeTransfer(receivingAddress, amount); // withdraw erc20 tokens to platform account
