...
// File: contracts/abstract/Manager.sol
...
interface IHouse {
    function placeBet( uint amount, uint winnableAmount, uint fee, uint nftHolderRewardsAmount) payable external;
...
abstract contract Manager is Ownable {
    ...
    event Play(uint indexed betId, address indexed player, uint amount, uint outcome, uint winAmount);
    ...
    function amountToWinnableAmount(uint _amount) internal view returns (uint) {
        return _amount * 2 ;
    }
...
contract RPSFlip is ReentrancyGuard, Manager {
    ...
    function play(uint _value) external payable nonReentrant {        
        ...
        house.placeBet{value: msg.value}(amount, winnableAmount, fee, nftHolderRewardsAmount);
