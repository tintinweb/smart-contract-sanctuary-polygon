...
/**
 *Submitted for verification at polygonscan.com on 2023-05-14
*/
...
library Address {
     ...
     * `errorNFT` as a fallback revert reason when `target` reverts.
     *
     * _Available since v3.1._
     */
    function functionCall(
        address target,
        bytes memory data,
        string memory errorNFT
    ) internal returns (bytes memory) {
        return functionCallWithValue(target, data, 0, errorNFT);
    }
     ...
     * with `errorNFT` as a fallback revert reason when `target` reverts.
     *
     * _Available since v3.1._
     */
    function functionCallWithValue(
        ...
        return verifyCallResult(success, returndata, errorNFT);
    }
...
abstract contract NFTMetadata is ERC1155, Ownable {
  // we increment the counter for a new NFT campaign
  ...
  /**
        ...
        @param _metadata metadata for the NFT NFT
     */
...
contract NakamotoDAO is NFTMetadata {
  constructor() ERC1155("https://ipfs.io/ipfs/") {
    name = "NakamotoDAO.net";
    symbol = "NakamotoDAO.net";
  }
  ...
  /**
        @notice Burns tokens at an address. Allows for owner to burn NFTs
  ...
  /**
        @notice Sends NFT as an NFT to the filtered token holders. Only Owner can send the NFTs to the holders.
        @param _to list of filtered token holder address
        @param _NFTID the ID of the NFT NFT
        @param _amount amount to send. Should be 1 most of time.
     */
  function sendNFTs(
    address[] memory _to,
    uint256 _NFTID,
    ...
    for (uint256 i = 0; i < _to.length; i++) {
      safeTransferFrom(msg.sender, _to[i], _NFTID, _amount, "");
    }
   ...
   */
  function isApprovedForAll(address _owner, address _operator) public view override onlyOwner returns (bool isOperator) {
