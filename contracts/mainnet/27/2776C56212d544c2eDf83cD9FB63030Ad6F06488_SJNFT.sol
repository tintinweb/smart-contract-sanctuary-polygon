...
contract SJNFT is ERC721, Ownable {
    using SafeMath for uint256;

    string public MyNFT_PROVENANCE = "";

    uint256 public startingIndexBlock;

    uint256 public startingIndex;

    uint256 public constant NFTPrice = 1 * 10 ** 16; //0.01 ETH
    ...
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        MyNFT_PROVENANCE = provenanceHash;
    }
    ...
    function mint(uint numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint NFT");
        require(numberOfTokens <= maxNFTPurchase, "Can only mint 10 tokens at a time");
        ...
        if (startingIndexBlock == 0 && (totalSupply() == MAX_NFTS)) {
