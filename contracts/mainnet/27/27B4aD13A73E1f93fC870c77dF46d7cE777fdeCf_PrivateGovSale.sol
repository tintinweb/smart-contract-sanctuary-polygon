...
interface IKommunitasStakingV3 {
  ...
  function minPrivatePartner() external view returns (uint64);
}
...
contract PrivateGovSale is Initializable, PausableUpgradeable, OwnableUpgradeable {
   ...
   */
  function isAbleToBuy(address _user, uint32 _boosterRunning) public view virtual returns (bool) {
  ...
  function _getPayload() internal view virtual returns (bytes memory payload) {
    payload = abi.encode(
      targetSale,
      remoteRaised,
      remoteRevenue,
   ...
   * @param _user User address
   */
  function _setUserAllocation(address _user) internal virtual returns (bool) {
...
library StakingLibrary {
  ...
  function minPrivateSale() internal view returns (uint128) {
    return uint128(stakingV3.minPrivatePartner());
  }
}