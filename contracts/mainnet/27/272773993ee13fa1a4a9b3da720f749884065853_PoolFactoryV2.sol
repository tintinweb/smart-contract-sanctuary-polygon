...
// File contracts/PoolFactoryV2.sol
...
contract PoolFactoryV2 is Initializable, OwnableUpgradeable {
    ...
    */
    ...
    function createPool(
        ...
        Pool storage pool = pools[poolCount];
            ...
            emit NewPool(poolCount, pool.game, pool.gameMode, pool.currentPrize, pool.entryFee, pool.startOn, pool.finishOn, pool.firstFee, pool.secondFee, pool.thirdFee, pool.tokenAddr);
            return poolCount;
    }
    ...
    */
    ...
    function finishPool(uint256 _poolId, address payable _winner1, address payable _winner2, address payable _winner3) public onlyOwner {
        ...
        if (poo.tokenAddr == address(0)) {
        treasury.transfer(poo.currentPrize.mul(bbFee).div(100));
        ...
        _winner3.transfer((poo.currentPrize).mul(poo.thirdFee).div(100));
            ...
            IERC20Upgradeable token = IERC20Upgradeable(poo.tokenAddr);
            token.safeTransfer(treasury, poo.currentPrize.mul(bbFee).div(100));
            poo.currentPrize = (poo.currentPrize).sub(poo.currentPrize.mul(bbFee).div(100));

            token.safeTransfer(_winner1, (poo.currentPrize).mul(poo.firstFee).div(100));
            token.safeTransfer(_winner2, (poo.currentPrize).mul(poo.secondFee).div(100));
            token.safeTransfer(_winner3, (poo.currentPrize).mul(poo.thirdFee).div(100));
        }
