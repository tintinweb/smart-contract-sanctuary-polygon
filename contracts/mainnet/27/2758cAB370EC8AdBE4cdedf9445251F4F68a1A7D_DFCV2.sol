    ...
    ERC721EnumerableUpgradeable
{
    ...
    function mint(uint256 _mintAmount, address _to) public payable {
        ...
        require(_to == _msgSender(), "You must be the recipient");

        if (whitelistOnly == true) {
            require(isWhitelisted(_to), "Address is not whitelisted!");
        }
    ...
    /// @dev change the status from hidden to reveal. This is a one way function, meaning it can't go from revealed to hidden again.
    ...
    /// @dev Toggles the whitelistOnly status
