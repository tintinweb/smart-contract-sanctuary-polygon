...
contract BAYCPolygon is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public publicSupply = 9950;
  ...
  function burnToken(uint256 _tokenId) public payable returns(bool){
    ...
    require(msg.value >= burnCost,"not enough matic");

    bool isTokenOwner = ERC721.ownerOf(_tokenId) == msg.sender;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(supply + _mintAmount <= publicSupply,"exceeded max supply");
    require(msg.value >= cost * _mintAmount,"not enough matic");
  ...
  function setBurnCost(uint256 _newCost) external onlyOwner {
    burnCost = _newCost;
  }

  function setBurnEnabled(bool _value) external onlyOwner {
