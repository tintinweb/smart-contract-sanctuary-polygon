...
contract Sunnylady is ERC721AQueryable, ERC721ABurnable, Ownable {
  using EnumerableSet for EnumerableSet.UintSet;

  event ForceSUNNY(uint256 phase);
  ...
  /// @notice the moooon
  uint256 public constant NEW_SUNNY = 592500; // 07/01/1970 : 21h35
  uint256 public constant SUNNY_CYCLE = (29530 * 86400) / 1000; // 29.53 days // 2551392
  uint256 public constant SUNNY_CYCLE_1_8 = SUNNY_CYCLE / 8; //  3.69 days //  318924
  uint256 public constant NEW_SUNNY_START = NEW_SUNNY - (SUNNY_CYCLE_1_8 / 2);
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    uint256 currentSUNNYPhase = getSUNNYPhase(block.timestamp);

    if (bytes(baseURI).length == 0) {
      return _toString(tokenId);
    }

    if (usedPhases.contains(currentSUNNYPhase)) {
      return string(abi.encodePacked(baseURI, _toString(tokenId), "_", _toString(currentSUNNYPhase), uriSuffix));
    }
  ...
  function getSUNNYPhase(uint256 timestamp) public view returns (uint256 SUNNYPhase) {
    if (forcedPhase >= 0 && forcedPhase <= 7) {
      return forcedPhase;
    }
    uint256 cycle_offset = (timestamp - NEW_SUNNY_START) % SUNNY_CYCLE;
    return ((cycle_offset * 10**18 * 8) / (SUNNY_CYCLE)) / 10**18;
  }

  function getSUNNYPhaseName(uint256 timestamp) public view returns (string memory phase) {
    uint256 SUNNYPhase = getSUNNYPhase(timestamp);

    if (SUNNYPhase == 0) return "0 - New SUNNY";
    if (SUNNYPhase == 1) return "1 - Waxing Crescent SUNNY";
    if (SUNNYPhase == 2) return "2 - First Quarter SUNNY";
    if (SUNNYPhase == 3) return "3 - Waxing Gibbous SUNNY";
    if (SUNNYPhase == 4) return "4 - Full SUNNY";
    if (SUNNYPhase == 5) return "5 - Waning Gibbous SUNNY";
    if (SUNNYPhase == 6) return "6 - Last Quarter SUNNY";
    if (SUNNYPhase == 7) return "7 - Waning Crescent SUNNY";
  }

  function getCurrentSUNNYPhase() external view returns (uint256 SUNNYPhase) {
    return getSUNNYPhase(block.timestamp);
  }

  function getCurrentSUNNYPhaseName() external view returns (string memory) {
    return getSUNNYPhaseName(block.timestamp);
  }
  ...
  /// @notice 0-7: force a phase (0:new SUNNY, 4:full SUNNY), > 7: default phase
  function forceSUNNY(uint256 phase) external onlyOwner {
    forcedPhase = phase;
    emit ForceSUNNY(phase);
  }
}