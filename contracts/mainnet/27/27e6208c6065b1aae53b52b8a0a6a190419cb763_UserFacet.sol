...
contract UserFacet is Base {
    ...
    function createUser(bytes calldata data_)
        external
        onlyEoA()
    {
        (bytes memory signature,  address wallet, string memory discordID) = abi.decode(
            data_,
            (bytes, address, string)
        );
        ISignatureFacet(address(this)).isValidSignature(
            s.createUserSigner,
            keccak256(abi.encodePacked(msg.sender, discordID)),
            signature
        );

        if (s.users[msg.sender].wallet == msg.sender)
            revert UserAlreadyExists();
        s.users[msg.sender].wallet = wallet;
        s.users[msg.sender].discordId = discordID;
        s.usersCount += 1;
    }

    function updateUser(
        address wallet_,
        string memory discordId_,
        bool frozen_
    ...
    function isUser(address wallet_)
        public
        view 
        returns (bool)
    {
        return s.users[wallet_].wallet != address(0);
    }
