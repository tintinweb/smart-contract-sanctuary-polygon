...
contract XVMCmigrator {
	...
	address public immutable token = 0x970ccEe657Dd831e9C37511Aa3eb5302C1Eb5EEe;
	
	address public immutable oldPool;
	address public immutable newPool;

   constructor(address _oldPool, address _newPool) {
		IERC20(0x6d0c966c8A09e354Df9C48b446A474CE3343D912).safeApprove(0x970ccEe657Dd831e9C37511Aa3eb5302C1Eb5EEe, type(uint256).max); //to exchange(swap) old for new token
		oldPool = _oldPool;
		newPool = _newPool;
    }
