...
contract CandleGenieMemeRage is Ownable, Pausable, ReentrancyGuard 
{
    ...
    event TicketBuyed(address indexed sender, uint256 indexed epoch, uint256 slot, uint256 amount);
    ...
    function BuyTicket(uint256 epoch, uint256 slot) external payable whenNotPaused nonReentrant notContract 
    {
        ...
        require(msg.value >= ticketPrice, "Amount lower than ticket price");
        ...
        emit TicketBuyed(msg.sender, currentEpoch, slot, amount);
    }
        ...
        for (uint256 i = 0; i < epochs.length; i++) 
        {
            ...
            if (Rounds[epochs[i]].closed) 
            {
                ...
                if (claimable(epochs[i], msg.sender))
                {
                    ...
                    // Determining Winner Count
                    uint256 winnerCount = 0;
                    for (uint256 j = 0; j < round.bets.length; j++) 
                    {
                        Bet memory bet = round.bets[j];
                        if (bet.position == round.result)
                        {
                            winnerCount++;
                        }
                    }
                    ...
                    if (winnerCount > 0)
                    {
                        // Calc
                        uint256 roundTotal = round.slotAmounts[0] + round.slotAmounts[1] + round.slotAmounts[2];
                        uint256 rewardBaseAmount = roundTotal / 100 * rewardRate;
                        uint256 rewardAmount = rewardBaseAmount / winnerCount;
                        // Mark
    ...
    function getRoundBets(uint256 epoch) external view returns (Bet[] memory) {
        return Rounds[epoch].bets;
    }
    ...
    function getSlotItems() external view returns (SlotItem[] memory) {
     ...
     function getStats() external view returns (uint256, uint256, uint256, uint256) {
        return (betsCount, betsTotal, paidBetsCount, paidBetsTotal);
    }
