...
contract Arbitrage {
    ...
    function approveToSushiSwap(address token, uint256 amount) public {
        IERC20(token).approve(address(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506),amount);
    }

    function getSwapTokenAmount(address token1, address token2) public view virtual returns (uint) {
    ...
    function getSwapTokenAmount1(address token1, address token2,uint amountOut) public view virtual returns (uint) {
        address[] memory path = new address[](2);
        path[0] = token1;
        path[1] = token2;
        uint amountIn = sushiRouter.getAmountsIn(
