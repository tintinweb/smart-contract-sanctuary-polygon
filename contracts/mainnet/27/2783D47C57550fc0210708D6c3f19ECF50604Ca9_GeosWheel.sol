...
abstract contract Authorizables is Ownable {
...
contract GeosVault is GeosContracts, Authorizables, GeosTokens {
    ...
    function _sendTokens(address to, uint256 amount, uint8 packageType) internal {
        send20Token(geos20, to, amount);
        emit RewardGiven(to, uint8(packageType), 0, amount);
    }

    function _sendNFTs(address to, uint256 tokenId, uint8 packageType) internal {
        send1155Token(geos1155, to, tokenId);
        emit RewardGiven(to, uint8(packageType), tokenId, 0);
    }
...
contract GeosWheel is VRFConsumerBase, GeosVault {
    ...
    function _getRandomRewardIncrementer(uint256 _index, uint256 _total) internal view returns(uint256){

        return(((uint256(keccak256(abi.encode(mainRandomness, _index)))) % _total) + 1);
    }
    ...
    function giveReward(address to) internal  {
        ...
        if(uint8(packageType) < uint8(6)){
            _sendTokens(to, packageVar, uint8(packageType));
            
        }else {
            _sendNFTs(to, packageVar, uint8(packageType));
        }
    ...
    function createRandomArr() internal view returns(uint256[] memory _out){
        _out = new uint256[](packagesLength);
        for (uint256 i=0; i<packagesLength; i++){
            _out[i] = _packageChance[uint8(i)];
        }
    }

    function _getReward() internal view returns(RewardPackage _rewardPackage, uint256 _rewardVar){
        uint256[] memory _randomArr = createRandomArr();
        uint256 _totalSum = GeosCheckers.sumArray(_randomArr);
        uint256 _rr = _getRandomRewardIncrementer(rewardTracker, _totalSum);
        uint8 packageId = GeosCheckers.getIdFromFlat(_randomArr, _rr);
        require(packageId != uint8(12), "GeosWheel: Not existant");
        _rewardPackage = RewardPackage(packageId);
