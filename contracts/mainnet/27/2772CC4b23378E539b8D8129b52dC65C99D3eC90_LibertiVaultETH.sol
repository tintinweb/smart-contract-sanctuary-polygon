...
abstract contract LibertiAggregationRouterV4 {
    ...
    function swap(
        ...
        require(desc.dstReceiver == receiver, "!badrecv");
        require(desc.amount <= maxAmount, "!badamnt");
    ...
    OwnableUpgradeable
{
    ...
    ILibertiPriceFeed internal immutable priceFeed; //feedDirectory -- directory
    ...
    constructor(address _delegatedPriceFeed) {
        _disableInitializers();
    ...
    /// @notice function.
    function setMinDepositAmount(uint256 assets) external onlyOwner {
        minDepositAmount = assets;
    }
    ...
    function setMaxDepositAmount(uint256 assets) external onlyOwner {
    ...
    function rebalance(
        ...
        returnAmount = swap(data, address(this), type(uint256).max);
    ...
    function getValueInNumeraire(
        ...
        uint256 tokenPrice = priceFeed.getPrice(token);
    ...
    function getNavInNumeraire(MathUpgradeable.Rounding rounding) public view returns (uint256) {
        uint256 navToken0 = getValueInNumeraire(
            asset(),
            IERC20Metadata(asset()).balanceOf(address(this)),
            rounding
        );
    ...
    function deposit(
        ...
        emit Deposit(_msgSender(), receiver, assets, shares); //FIXME: all shares, or shares minted to sender?
    }
    ...
    function _deposit(
        ...
        require(assets >= minDepositAmount, "ERC4626: deposit less than min");
        require(assets <= maxDeposit(receiver), "ERC4626: deposit more than max");
        uint256 returnAmount = 0;
        uint256 swapAmount = 0;
        if (10_000 > invariant) {
            swapAmount = assetsToToken1(assets);
            returnAmount = swap(data, address(this), swapAmount);
        }
        uint256 supply = totalSupply();
        if (0 < supply) {
        ...
        uint256 feeAmount = shares.mulDiv(entryFeeValue, 10_000, MathUpgradeable.Rounding.Down);
        _mint(receiver, shares - feeAmount);
        _mint(owner(), feeAmount); // mint(feeTo)
    }

    /// @notice Given an amount of shares, returns the precise corresponding amount of asset
    /// @notice after exit fee.
    function sharesToToken0(uint256 shares) external view returns (uint256) {
        uint256 totalToken0 = IERC20Metadata(asset()).balanceOf(address(this));
        if (0 < totalToken0) {
            shares -= shares.mulDiv(exitFeeValue, 10_000, MathUpgradeable.Rounding.Down);
            return totalToken0.mulDiv(shares, totalSupply(), MathUpgradeable.Rounding.Down);
        }
        return 0;
    }
    ...
    function sharesToToken1(uint256 shares) external view returns (uint256) {
        ...
        if (0 < totalToken1) {
            shares -= shares.mulDiv(exitFeeValue, 10_000, MathUpgradeable.Rounding.Down);
            return totalToken1.mulDiv(shares, totalSupply(), MathUpgradeable.Rounding.Down);
        }
        return 0;
    }
    ...
    function assetsToToken1(uint256 assets) public view returns (uint256) {
        if (10_000 > invariant) {
            return assets - assets.mulDiv(invariant, 10_000);
        }
        return 0;
    }
    ...
    function _redeem(
        ...
        uint256 feeAmount = shares.mulDiv(exitFeeValue, 10_000, MathUpgradeable.Rounding.Down);
        shares -= feeAmount;
        ...
        if (0 < totalToken1) {
            ...
            assets += swap(data, receiver, amount);
        }
    }

    //
    // ERC-4626 unimplemented functions
    //
    ...
    function setEntryFeeValue(uint256 value) external onlyOwner {
        require(100 >= value, "!max"); // max entry fee: 100 bps (1 percent)
        entryFeeValue = value;
    }
    ...
    /// @notice is to let a shareholder to redeem thei shares without relying on any external
...
contract LibertiVaultETH is LibertiVault {
    ...
    constructor(address _delegatedPriceFeed, address _weth) LibertiVault(_delegatedPriceFeed) {
        weth = Weth9(_weth);
    }
    ...
    function depositEth(
        ...
        emit Deposit(_msgSender(), receiver, msg.value, shares); //FIXME: all shares, or shares minted to sender?
    }

    /// @notice Allows the redeem of the blockchain's native currency from a vault. Unwraps the
    /// @notice wrapped token before transfering the value to the receiver.
    function redeemEth(
        uint256 shares,
        address receiver,
        address _owner,
