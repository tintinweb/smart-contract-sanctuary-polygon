...
contract FablesOfCrypto is ERC1155Supply, Ownable {
    ...
    uint256 public constant IKMaxSupply = 333;
    ...
    constructor() ERC1155("https://thecryptotribe.xyz/{id}.json") {
        ...
        //Sets Max Supply of main assets
    ...
    function mintFireKEY(uint256 amount) public payable {
        require(
            totalSupply(FireKEY) < FKMaxSupply,
            "Max Supply reached. No more Fire KEYs can be minted"
        );
        uint256 feeToPay;
        if (presaleActive) {
    ...
    function mintIceKEY(uint256 amount) public payable {
        uint256 feeToPay;
        if (presaleActive) {
            feeToPay = amount * IKFeePre;
        } else {
    ...
    function mintWoodenKEY(uint256 amount) public payable {
        uint256 feeToPay;
        if (presaleActive) {
            feeToPay = amount * WKFeePre;
        } else {
    ...
    function createSeries(
        ...
        seriesFee[_id] = _mintFee;

        if (_initialSupply != 0) _mint(msg.sender, _id, _initialSupply, "");
        tokenMaxSupply[_id] = _maxSupply;
        return _id;
    }

    //Activates/Deactivates presale
    function activatePresale(bool _val) public onlyOwner {
        presaleActive = _val;
    }
