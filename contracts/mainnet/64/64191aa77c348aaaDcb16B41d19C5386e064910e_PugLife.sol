...
abstract contract Auth {
     ...
     */
    modifier authorized() {
        require(isAuthorized(owner), "!AUTHORIZED"); _;
    }
...
contract PugLife is IERC20, Auth {
    ...
    constructor () Auth(msg.sender) {
        ...
        taxReceiver = 0xa0bf58e71a54b092Cf6185CC1660916b4c94Cd90;
    ...
    function setTaxFree(address adr, bool isFree) external onlyOwner() {
    ...
    function takeTax(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        _balances[address(taxReceiver)] = _balances[address(taxReceiver)].add(tax);
        emit Transfer(sender, address(taxReceiver), tax);

        return amount.sub(tax);
    }

         function setTaxDenominator(uint256 _amount) external onlyOwner {
        taxDenominator = _amount;
    }
    
    function setReceivers(address _taxReceiver) external onlyOwner {
