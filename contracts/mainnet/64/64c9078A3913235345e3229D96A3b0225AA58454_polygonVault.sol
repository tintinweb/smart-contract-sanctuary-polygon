...
interface IUniswapRouter is ISwapRouter {
  function refundMatic() external payable;
}

// Add deposit function for MATIC
...
contract polygonVault {
  ...
  address public usdcAddress = 0xe6b8a5CF854791412c1f6EFC7CAf629f5Df1c747;
  address public maticAddress = 0x0000000000000000000000000000000000001010;
  ...
  address public chainLinkMATICUSDAddress = 0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada;
...
/* Polygon Mainnet Addresses 
  ...
  address public uinswapV3QuoterAddress = 0xb27308f9F90D607463bb33eA1BeBb41C27CE5AB6;
  address public uinswapV3RouterAddress = 0xE592427A0AEce92De3Edee1F18E0157C05861564;
  address public chainLinkMATICUSDAddress = 0xAB594600376Ec9fD91F8e885dADF0CE036862dE0;
*/
  uint public maticPrice = 0;
  ...
  DepositableERC20 maticToken = DepositableERC20(maticAddress);
  IQuoter quoter = IQuoter(uinswapV3QuoterAddress);
  IUniswapRouter uniswapRouter = IUniswapRouter (uinswapV3RouterAddress);

  event polygonVaultLog(string msg, uint ref);

  constructor() {
    console.log('Deploying polygonVault Version:', version);
  ...
  function getTotalBalance() public view returns(uint) {
    require(maticPrice > 0, 'MATIC price has not been set');
    uint usdcBalance = getUSDCBalance();
    uint maticBalance = getMaticBalance();
    uint maticUSD = (maticBalance * maticPrice)/eth; // assumes both assets have 18 decimals
    uint totalBalance = maticUSD + usdcBalance;
    return totalBalance;
  }

  function updateMaticPriceUniswap() public returns(uint) {
    uint maticPriceRaw = quoter.quoteExactOutputSingle(usdcAddress,maticAddress,3000,100000,0);
    maticPrice = maticPriceRaw;
    return maticPrice;
  }

  function updateMaticPriceChainlink() public returns(uint) {
    int256 chainLinkMaticPrice = EACAggregatorProxy(chainLinkMATICUSDAddress).latestAnswer();
    maticPrice = uint(chainLinkMaticPrice*10000000000);
    return maticPrice;
  }

  function buyMatic(uint amountUSD) internal {
    ...
    emit polygonVaultLog('amountIn', amountIn);
    require(usdcToken.approve(address(uinswapV3RouterAddress), amountIn), 'USDC approve failed');
    ...
    uniswapRouter.refundMatic();
  }

  function sellMatic(uint amountUSD) internal {
    uint256 deadline = block.timestamp + 15;
    uint24 fee = 3000;
    address recipient = address(this);
    ...
    uint160 sqrtPriceLimitX96 = 0;
    require(maticToken.approve(address(uinswapV3RouterAddress), amountOut), 'MATIC approve failed');
  ...
  function closeAccount() public {
    ...
    uint maticBalance = getMaticBalance();
    if (maticBalance > 0) {
      maticToken.safeTransfer(owner, maticBalance);
    }
  }
