...
contract HorsemanLite is ReentrancyGuard {
    ...
    uint256 private constant RoundTime = 172800;//48 hour
    uint256 public constant HeroPrice = 10 ether;

    uint256 private constant CntDivide = 10000;
    uint256 private constant Ecosystem = 200;
    uint256 private constant GrandPot = 100;
    uint256 private constant MaxGrandPot = 500000 ether;
    uint256[4] private Dividend = [1200, 1, 100, 400]; //[init, 0.1%update, drop1%, min3%]
    uint256[2] private UnitInvite = [200, 100];
    ...
    uint256 private constant NeedMul = 100;
    ////////////////////////////////////////////// var ////////////////////////////////////////////////////////////
    ...
    uint256[4] private ArrWager = [1, 5, 25, 125];
    mapping(uint256 => mapping(uint256=>uint256)) mapIndexPower;//rountId =>(wagerMul=>index)
    mapping(uint256 => mapping(uint256=>address)) mapPower1;//rountId =>(index=>addres)
    mapping(uint256 => mapping(uint256=>address)) mapPower5;
    mapping(uint256 => mapping(uint256=>address)) mapPower25;
    mapping(uint256 => mapping(uint256=>address)) mapPower125;
     ...
     */
    function Duel(address _tokenAddress, uint256 wager, address inviter)
        external
        payable
        isActivated
        isInTime
        ...
        nonReentrant
    {
        ...
        if(inviter != address(0) && mapPlay[roundIndex][inviter].wager != 0){
            DataPlay storage play =  mapPlay[roundIndex][msg.sender];
            if(play.wager == 0 && play.myInviters[0] == address(0)){
                play.myInviters[0] = inviter;
                mapPlay[roundIndex][inviter].inviters.push(msg.sender);

                address parent = mapPlay[roundIndex][inviter].myInviters[0];
                if(parent != address(0)){
                    play.myInviters[1] = parent;
                    mapPlay[roundIndex][parent].inviters.push(msg.sender);
                }
            }
        }
        ...
        internal
    {
        ...
        if (r <= Dividend[1] && curDividend > Dividend[3]) {
        ...
        if (r >= Probability[0]) {
            play.lastResultType = GameResultType.Rugpulled;
            income.reward = (Multiplier[0] * dataRandom.wager) / 100;

            round.addr = addr;
            round.endTime = block.timestamp + RoundTime;
        } else if (r >= Probability[1]) {
            play.lastResultType = GameResultType.Steamrolled;
            income.reward = (Multiplier[1] * dataRandom.wager) / 100;
        } else if (r >= Probability[2]) {
            play.lastResultType = GameResultType.OverKill;
            income.reward = (Multiplier[2] * dataRandom.wager) / 100;
        } else if (r >= Probability[3]) {
            play.lastResultType = GameResultType.Win;
            income.reward = (Multiplier[3] * dataRandom.wager) / 100;
        } else if(r >= Probability[4]){
            play.lastResultType = GameResultType.Tie;
            income.reward = (Multiplier[4] * dataRandom.wager) / 100;
        } else {
            play.lastResultType = GameResultType.Lose;
        }

        play.wager += dataRandom.wager;

        uint256 wagerMul = dataRandom.wager / HeroPrice;
        mapping(uint256=>address) storage mapPower = mapPower1[roundIndex];

        if(wagerMul >= ArrWager[3]){
            wagerMul = ArrWager[3];
            mapPower = mapPower125[roundIndex];
        }else if(wagerMul >= ArrWager[2]){
            wagerMul = ArrWager[2];
            mapPower = mapPower25[roundIndex];
        }else if(wagerMul >= ArrWager[1]){
            wagerMul = ArrWager[1];
            mapPower = mapPower5[roundIndex];
        }else{
            wagerMul = ArrWager[0];
        }

        address addrDividend;
        uint256 indexPower = mapIndexPower[roundIndex][wagerMul];

        if (income.reward == 0) {
            play.defeat +=  wagerMul;
            round.defeatCnt += wagerMul;

            mapPower[indexPower] = addr;
            mapIndexPower[roundIndex][wagerMul]++;
        } else {
            uint256 unit = income.reward / CntDivide;

           DataInfo memory data = DataInfo(
                unit * Ecosystem,
                round.bounty >= MaxGrandPot ? 0 : unit * GrandPot,
               0,0
            );

            round.bounty += data.bounty;
            if(round.bounty > MaxGrandPot){
                round.bounty = MaxGrandPot;
            }

            uint256 intDividend = curDividend;

            if (indexPower > 0){
                for (uint256 i=0; i<play.myInviters.length; i++) 
                {
                    address addrInvite = play.myInviters[i];
                    if(addrInvite != address(0)){
                        intDividend -= UnitInvite[i];
                        mapPlay[roundIndex][addrInvite].inviterFee += unit * UnitInvite[i];
                        data.invite += unit * UnitInvite[i];
                        Bankroll.addPayAddrTemp(addrInvite, unit * UnitInvite[i]);
                    }
                }
            }

            data.dividend = indexPower > 0 ? unit * intDividend : 0;
            ...
            //transfer dividend
            if(indexPower > 0){
                r = indexPower == 1 ? 0 : randomWords[0]/10000%(indexPower-1);
            
                addrDividend = mapPower[r];
                mapPlay[roundIndex][addrDividend].dividend += data.dividend;
                Bankroll.addPayAddrTemp(addrDividend, data.dividend);
            }

          
            income.reward = income.reward - data.ecosystem - data.bounty - data.dividend - data.invite;
            play.wining += income.reward;

            //transfer ecosystem
            Bankroll.addEcosystem(data.ecosystem);
            //transfer reward
            Bankroll.addPayAddrTemp(addr, income.reward);

            income = DataIncome(dataRandom.wager, income.reward, data.ecosystem, data.bounty, data.dividend, data.invite);
        }
        ...
        emit Duel_Outcome_Event(addr, play, round, income, addrDividend);
    ...
    function _kellyWager(uint256 wager, address tokenAddress) private view {
        ...
        uint256 maxWager = getBankBalance(tokenAddress) / mul;
        if (wager > maxWager) {
    ...
    function getIndexPower(uint256 wagerMul) external view returns (uint256) {
        return mapIndexPower[roundIndex][wagerMul];
    }
