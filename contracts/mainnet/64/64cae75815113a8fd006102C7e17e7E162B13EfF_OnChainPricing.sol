...
contract OnChainPricing {
    ...
    // UniV2
    address public constant QUICK_ROUTER =
    ...
    function findOptimalSwap(
        ...
        uint256 length = 2; // Add length you need
        ...
        quotes[1] = Quote(GRAVITY_ROUTER, gravityQuote);
    ...
    function getUniPrice(
        ...
        try IUniswapRouterV2(router).getAmountsOut(amountIn, path) returns (
    ...
    function getGravityPrice(address[] memory path, uint256 amountIn)
        public
        view
        returns (uint256)
    {
        ...
        try
            IUniswapRouterV2(GRAVITY_ROUTER).getAmountsOut(amountIn, path)
        ...
        returns (address[] memory)
    {
        ...
        for (uint256 i = 0; i < 3; i++) {
            //if pair doesn't exist then remove router from array
            if (
                IUniswapV2Factory(IUniswapRouterV2(exchanges[i]).factory())
                    .getPair(tokenIn, tokenOut) == address(0)
            ) {
