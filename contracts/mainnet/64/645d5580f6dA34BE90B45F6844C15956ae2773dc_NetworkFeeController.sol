...
contract NetworkFeeController is INetworkFeeController, Ownable {
    ...
    constructor(
        ...
        address _networkFeeVault,
        address _maticEthAggregator
    ) {
    ...
    function setMethodNetworkFees(bytes4 method, NetworkFee memory _networkFee)
        public
        onlyOwner
    {
    ...
    function getNetworkFee(
        ...
        if (methodNetworkFee.callback != address(0)) {
            return
                INetworkFeeController(methodNetworkFee.callback).getNetworkFee(
                    sender,
                    method,
                    asset,
                    amount
        ...
        uint256 need_gas = methodNetworkFee
            .gasUsed
            .mul(tx.gasprice)
            .mul(methodNetworkFee.fee)
            .div(100); // Matic/token, token/Matic
        uint256 matic_eth_price = IAggregator(maticEthAggregator)
            .latestAnswer();
        uint256 price = IPriceOracleGetter(priceOracleGetter).getAssetPrice(
            asset
        );
        return (
            need_gas.mul(matic_eth_price).div(price).div(
