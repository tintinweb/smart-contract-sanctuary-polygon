 ...
 */
contract TokenFactory is IOmniApp, Ownable {
    ...
    function processMintPayment(uint256 price, address spender, address receiver, bool isLock, Asset memory asset) internal {
        ...
        token.transferFrom(spender, receiver, inWei * 98 / 100);
        token.transferFrom(spender, _feeManager, inWei * 2 / 100);
        emit Paid(receiver);
    }
     ...
     */
    function withdraw(address refundee, address creator, uint256 price, string memory assetName, bool isPayout) private {
        ...
        if (isPayout) {
            token.transfer(creator, inWei * 98 / 100);
            token.transfer(_feeManager, inWei * 2 / 100);
            emit Paid(creator);
            return;
        }
