...
contract Executor is Bidder {
    ...
    function purchase(uint256 tokenId) public payable {
        ...
        require(value >= ask.value, "Polygods: didn't send enough ETH");
        ...
        emit Bought(tokenId, value, seller, to);

        Heap.Node memory floor = _getFloor();
        int256 baseline = _baseline[tokenId];

        if(value <= floor.priority || value < baseline){
    ...
    function acceptBid(uint256 tokenId, int256 minPriceInWei) public {       
        ...
        require(value != 0, "Polygods: bid.value == 0");
        require(value <= minPriceInWei, "Polygods: bid.value > minPrice");
        ...
        emit Bought(tokenId, value, accepter, bidder);
...
contract Bidder is VRF {
    ...
    function enterBid(uint256 tokenId) public payable {
        ...
        int256 value = int256(msg.value);
        require(value != 0, "Polygods: insufficient bid value");
    ...
    function withdrawBid(uint256 tokenId) public {
        ...
        uint256 value = uint256(bid.value);
...
contract MintBurn is Core {
    ...
    constructor(string memory name_, string memory symbol_, string memory baseURI_) Core(name_, symbol_, baseURI_) {
        ...
        //countTotal = 10001; // (production)
        countTotal = 301; // (testing)

        active = false;
        assigned = false;

        //mintPrice = 169000000000000000000; //169 MATIC (production)
        _mintPrice = 1000000000000000000; //1 MATIC (testing)

        /* * */
        uint reserve = 240; 
        for(uint tokenId = 1; tokenId <= reserve; tokenId++) {
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        _baseline[tokenId] = int256(_mintPrice);
