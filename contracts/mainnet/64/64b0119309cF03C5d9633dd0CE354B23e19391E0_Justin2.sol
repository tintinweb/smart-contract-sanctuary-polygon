...
/// @title Sassy Express Term
/// @notice This contract represents a specific term implementation for an agreement.
/// @dev This contract inherits from the TermBase and implements the ExpressTerms interface.
contract Justin2 is TermBase, ExpressTerms {
    bytes4 private constant _INTERFACE_ID_TERM =
        bytes4(keccak256("term(Agreement)"));

    /// @notice Registers the interface id for this contract during the construction phase
    constructor() {
    ...
    function term(
        ...
        return
            '\n\n### 1. RECITALS\n\nThe Company has selected **Pudgy Penguin NFT number ${TOKEN_ID}** for use in a product development,\npromotion and/or sales Project relating to or concerning Food company (the Project). Penguin\nHolder has personal rights and certain commercial rights to their ++Pudgy Penguin NFT++, which the\nCompany desires to license. Specifically, the Company has requested, and Penguin Holder has\nagreed to make Pudgy Penguin NFT number ${TOKEN_ID} available for use in the Project.';
    }
}
...
struct Agreement {
    ...
    bytes32[] signers; // Array of additional promisors
    string dynamicData; // CID
    uint256 nonce;
    uint256 chainId;
}
