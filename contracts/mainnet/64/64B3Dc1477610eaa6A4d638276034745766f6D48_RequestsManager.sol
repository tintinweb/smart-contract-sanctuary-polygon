...
contract RequestsManager is RequestsManagerStorage {
  ...
  function createRequest(uint32 countHours_, string calldata descriptionUrl_) external override {
    ...
    dm.addRequest(requestUid, worker, countHours_, descriptionUrl_);
  ...
  function _approve(RequestUid requestUid_, bool approveValue_, string memory explanation_) internal {
  ...
  function approve(RequestUid requestUid, bool approveValue_, string calldata explanation_) external override {
  ...
  function disapproveBatch(RequestUid[] calldata requestUids, string[] calldata explanations) external override {
    uint lenRequests = requestUids.length;
  ...
  function _refreshRequestStatus(
  ...
  // *****************************************************
  function decodeRequestStatusValue(RequestStatusValue status) public pure returns (
    RequestStatus requestStatus,
    uint32 countPositive,
  ...
  function extractRequestStatus(RequestStatusValue status) external pure returns (RequestStatus requestStatus) {
    return _extractRequestStatus(status);
  }

  function _extractRequestStatus(RequestStatusValue status) internal pure returns (RequestStatus requestStatus) {
    requestStatus = RequestStatus(uint256(uint32(RequestStatusValue.unwrap(status))));
  }

  function encodeRequestStatusValue(
    RequestStatus requestStatus,
    uint32 countPositive,
    uint32 countNegative
  ) public pure returns (
    RequestStatusValue status
  ) {
    uint256 encoded;
  ...
  function getRequestsForEpoch(EpochType epoch) external view returns (RequestUid[] memory requests) {
    WorkerUid[] memory workerUids = requestsForEpoch[epoch];
    uint len = workerUids.length;
    requests = new RequestUid[](len);
    for (uint i = 0; i < len; i = _uncheckedInc(i)) {
      requests[i] = _getRequestUid(epoch, workerUids[i]);
    }
  }
  ...
  function getRequestsStatuses(RequestUid[] calldata requests) external view returns (
    ...
    statuses = new RequestStatus[](len);
    for (uint i = 0; i < len; i = _uncheckedInc(i)) {
      RequestStatusValue value = requestsStatusValues[requests[i]];
      (statuses[i],,) = decodeRequestStatusValue(value);
    }
  }
}
...
/// @author dvpublic
abstract contract RequestsManagerStorage is Initializable, Controllable, IRequestsManager {
  // *****************************************************
  //                  Constants
  // *****************************************************
