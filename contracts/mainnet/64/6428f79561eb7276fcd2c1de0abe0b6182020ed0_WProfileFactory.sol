...
// File contracts/interfaces/IBBPermissionsV01.sol
...
// Wraps all necessary BackedBy write functions with a custom owner check.
// Ownership is determined by the parent contract's ERC721 token of the same id as the BB profileId.
// IBBPermissionsV01 is implemented so the current owner can view locked content.
// The owner can also set an accessOverride address to extend default permissions with custom logic.

contract WProfile is IBBPermissionsV01 {

    WProfileFactory factory;
    ...
    address accessOverride = address(0);
    uint256 profileId;

    constructor(address initialReceiver, string memory cid, bool fromExisting, uint256 existingProfileId) {
        factory = WProfileFactory(msg.sender);
        if (fromExisting) {
            profileId = existingProfileId;
        } else {
            profileId = profiles.createProfile(address(this), initialReceiver, cid);
        }
    }

    function owner() public view returns (address) { return factory.ownerOf(profileId); }
    function getProfileId() public view returns (uint256) { return profileId; }
    ...
    function setOverride(address newOverride) external {
        require(msg.sender == owner(), "not owner");
        accessOverride = newOverride;
    }

    // BBProfile
    function editProfile(address receiver, string calldata cid) external {
        require(owner() == msg.sender, "not owner");
        profiles.editProfile(profileId, address(this), receiver, cid);
    }

    // BBTiers
    function createTiers(uint256[] calldata prices, string[] calldata cids, bool[] memory deprecated, address[] calldata supportedCurrencies, uint256[] calldata priceMultipliers) external returns(uint256 tierSetId) {
        require(owner() == msg.sender, "not owner");
        return tiers.createTiers(profileId, prices, cids, deprecated, supportedCurrencies, priceMultipliers);
    }

    function editTiers(uint256 tierSetId, uint256[] calldata prices, string[] calldata cids, bool[] memory deprecated) external {
        require(owner() == msg.sender, "not owner");
        tiers.editTiers(profileId, tierSetId, prices, cids, deprecated);
    }

    function setSupportedCurrencies(uint256 tierSetId, address[] calldata supportedCurrencies, uint256[] calldata priceMultipliers) external {
        require(owner() == msg.sender, "not owner");
        tiers.setSupportedCurrencies(profileId, tierSetId, supportedCurrencies, priceMultipliers);
    }

    // BBSubscriptionFactory
    function createSubscriptionProfile(uint256 tierSetId, uint256 contribution) external {
        require(owner() == msg.sender, "not owner");
        subFactory.createSubscriptionProfile(profileId, tierSetId, contribution);
    }

    function setContribution(uint256 contribution) external {
        require(owner() == msg.sender, "not owner");
        subFactory.setContribution(profileId, contribution);
    }

    // BBPosts
    function createPost(string calldata cid) external returns(uint256 postId) {
        require(owner() == msg.sender, "not owner");
        return posts.createPost(profileId, cid);
    }

    function editPost(uint256 postId, string calldata cid) external {
        require(owner() == msg.sender, "not owner");
        posts.editPost(profileId, postId, cid);
    }

    // This function is always called when a user attempts to view a locked post.
    // accessOverride can grant additional access using custom logic, but never revoke it.
    function canViewSubscription(address account) external view returns(bool) {
        if (owner() == account) return true;

        if(accessOverride.code.length > 0) {
            try IBBPermissionsV01(accessOverride).canViewSubscription(account) returns (bool success) {
                if(success)
                    return success;
            } catch { }
        }
...
// File contracts/WProfileFactory.sol

pragma solidity ^0.8.0;
// NFT wrapped BackedBy profiles.

// The createProfile function deploys a new WProfile contract and mints an ERC721 token to the caller.
// The deployed WProfile contract owns an underlying BackedBy profile of the same id.
// It contains all the necessary write functions but defers to the ERC721 for ownership checks.

contract WProfileFactory is ERC721, ERC721Burnable {
    ...
    mapping(uint256 => address) public tokenIdToWProfile;

    constructor() ERC721("BackedBy Profile", "BBP") {
        factoryOwner = msg.sender;
    }

    function setBaseURI(string calldata newURI) public {
        require(msg.sender == factoryOwner, "Only the owner of the factory contract can change the base URI");
        baseURI = newURI;
    }
    ...
    function createProfile(address initialReceiver, string memory cid) public returns (address) {
        WProfile newWProfile = new WProfile(initialReceiver, cid, false, 0);
        uint256 id = newWProfile.getProfileId();
        _safeMint(msg.sender, id);
        tokenIdToWProfile[id] = address(newWProfile);
        return(address(newWProfile));
    }


    function mintForExistingProfile(uint256 profileId) external returns(address) {
        (address expectedAddr, address receiver, string memory cid) = profiles.getProfile(profileId);
        require(msg.sender == expectedAddr, "You do not own this profile");

        // If it is the real owner, deploy a new WProfile and mint the ERC721 token to them.
        WProfile newWProfile = new WProfile(receiver, cid, true, profileId);
        _safeMint(msg.sender, profileId);
        tokenIdToWProfile[profileId] = address(newWProfile);
        return(address(newWProfile));
    }
