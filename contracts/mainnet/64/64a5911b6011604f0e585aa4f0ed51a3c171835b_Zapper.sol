...
contract Zapper {
    ...
    address public uniRouterV3 = 0xE592427A0AEce92De3Edee1F18E0157C05861564; // UniswapV3 on Matic Mainnet!
    address public uniRouter = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // QuickSwap - MaticMain
    address public uniFactory = 0x5757371414417b8C6CAad45bAeF941aBc7d3Ab32; // QuickSwap Factory - MaticMain

    address public sushiRouter = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506; // sushiswapV2 Factory - MaticMain
    address public sushiFactory = 0xc35DADB65012eC5796536bD9864eD8773aBc74C4; //sushiswap v2 - MaticMain
    ...
    function ZapInSingleToken(address inToken, address outToken) internal returns (uint256) {
        ...
        _swapUniswap(inToken, outToken, _inBalance);
								// user's token, Output LP token, balance of users token held within contract

        uint256 _outBalance = IERC20(outToken).balanceOf(address(this));
        IERC20(outToken).safeTransfer(msg.sender, _outBalance);

        return _outBalance; // lp token balance
    }

// User deposit is wrong LP, split tokens into their halves.
