...
contract Hosterizing_Test is ERC721URIStorage {
    ...
    constructor(
        ...
        tld = _tld;
        console.log("%s name service for Hosterizing Domains deployed", _tld);
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        require(len > 0);
        if (len == 3) {
            return 50 * 10**18; // 5 MATIC = 5 000 000 000 000 000 000 (18 decimals). We're going with 0.5 Matic because the faucets don't give a lot
        } else if (len == 4) {
            return 30 * 10**18; // To charge smaller amounts, reduce the decimals. This is 0.3
        } else {
            return 10 * 10**18;
        }
    }
