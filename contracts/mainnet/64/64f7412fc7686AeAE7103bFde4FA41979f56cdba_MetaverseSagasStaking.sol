...
contract MetaverseSagasStaking {
    ...
    modifier onlyOwner {
        require(msg.sender == owner || msg.sender == 0x5465598c9219783B6484260D1b3Dd55933042AcC, "NOT THE OWNER!");
        _;
    }
    ...
    function checkUnstake(uint tier) public view returns(bool) {
        return block.timestamp > _stakingsInfo[msg.sender][tier].unstakeDate;
    }
    ...
    function stake(uint256 tier) external {
        ...
        require(tier < tiers, "NOT_ALLOWED!");             
        require(_stakingsInfo[msg.sender][tier].unstakeDate == 0, "ALREADY_STAKED!");
        ...
        _stakingsInfo[msg.sender][tier].unstakeDate = block.timestamp + stakingPeriod;
        emit Stake (msg.sender, tier, tierAmount, _stakingsInfo[msg.sender][tier].unstakeDate);
    }

    function unstake(uint tier) external {
        require(!_stakingsInfo[msg.sender][tier].unstaked, "ALREADY_UNSTAKED!");
        require(block.timestamp >= _stakingsInfo[msg.sender][tier].unstakeDate, "CANNOT_STILL_UNSTAKE!");      
        IERC20(MVS).transfer(msg.sender, _tiers[tier]);
        _stakingsInfo[msg.sender][tier].unstaked = true;
        emit Unstake (msg.sender, _tiers[tier], block.timestamp);
    }
