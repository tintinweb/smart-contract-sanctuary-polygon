...
contract StupidSpermSwimmingClub is Context, ERC721Enumerable, Ownable {
    ...
    * Whitelist Mint
    */
    function mint(uint max, bytes memory signature) external {
        bytes32 hash;
        require(signer != address(0), "Pre-Mint is not available yet");
        require(saleIsActive == true, "Impossible Pre-Mint when sale is active");   
        ...
        require(whitelistMinted[_msgSender()].add(1) <= max, "Max mint reached");

        /*
         * Update total minted in preMint state
         */
        whitelistMinted[_msgSender()] = whitelistMinted[_msgSender()].add(1);
