...
contract AkaLottery is ReentrancyGuard, Ownable, VRFConsumerBaseV2 {
    ...
    mapping(address => uint256) private _userFreeTicket;
        ...
        nonReentrant
    {
        ...
        uint256 count = _ticketNumbers.length;

        uint256  freeTickets = _userFreeTicket[msg.sender];
        while (freeTickets != 0) {
            _userFreeTicket[msg.sender] = _userFreeTicket[msg.sender] - 1;
            count = count - 1;
        ...
        betToken.transferFrom(address(msg.sender), address(this), amountTokenToTransfer);
    ...
    function claimTickets(
        ...
        betToken.safeTransfer(msg.sender, rewardInTokenToTransfer);
    ...
    function fulfillRandomWords(
        ...
        latestLotteryId = currentLotteryId;
    }
        ...
        nonReentrant
    {
        ...
        require(_lotteryId == latestLotteryId, "Numbers not drawn");

        // Calculate the finalNumber based on the randomResult generated by ChainLink's fallback
        uint32 finalNumber = randomResult;
        ...
        betToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external onlyOwnerOrInjector {
        ...
        betToken.transferFrom(address(msg.sender), address(this), _amount);
    ...
    function airdropFreeTickets(address _user, uint256 amount) external onlyOwner {
        _userFreeTicket[_user] = _userFreeTicket[_user] + amount;
    }

    function viewUserFreeTickets(address _user) external view returns (uint256) {
        return _userFreeTicket[_user];
    }
