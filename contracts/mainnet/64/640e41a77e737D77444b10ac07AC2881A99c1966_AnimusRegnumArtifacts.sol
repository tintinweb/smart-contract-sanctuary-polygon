...
import "../Common/PaymentSplitterMod.sol";

contract AnimusRegnumArtifacts is Delegated, ERC1155, PaymentSplitterMod{
  struct Token{
    uint burnPrice;
    uint mintPrice;
    uint balance;
  ...
  constructor()
    ...
    ERC1155("")
  {
    _addPayee(0x608D6C1f1bD9a99565a7C2ED41B5E8e1A2599284, 95);
    _addPayee(0xB7edf3Cbb58ecb74BdE6298294c7AAb339F3cE4a, 5);
  }
  ...
  /** BEGIN: OS required **/
  //external
  function create(address, uint supply, string calldata uri_, bytes calldata)
  ...
  function mintBatch( uint[] calldata ids, uint[] calldata quantities, bytes calldata data) external payable{
    ...
    for(uint i; i < ids.length; ++i){
      ...
      require( token.isMintActive,                       "Sale is not active"        );
      require( token.balance + quantity <= token.supply, "Not enough supply"         );
      require( balanceOf(msg.sender, ids[i] ) + quantity < token.maxWallet, "Don't be greedy" );
      require( msg.value >= token.mintPrice * quantity,  "Ether sent is not correct" );
    ...
    _mintBatch(msg.sender, ids, quantities, data );
  }
