...
import {IERC20Vault} from "../interfaces/vaults/IERC20Vault.sol";
...
contract ExporterDataCollector {
    ...
    function getUniV3Vault(address y, address z) public view returns (IUniV3Vault) {
        if (IUniV3Vault(y).supportsInterface(type(IUniV3Vault).interfaceId)) {
            return IUniV3Vault(y);
        } else {
            return IUniV3Vault(z);
        }
    }
