...
contract cannibal0x is ERC20("Cannibal0x DarkX", "dark0x"), Auth {
    ...
    address public WMATIC; // = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; // main net wmatic address
    ...
    uint256 public baseReclaimed;
    uint256 public _totalSupply = 10;
    uint256 public _denomToken; // whole value of tokenBase held in reserve 
    uint256 public _numerRate; // fractional amount of cannibal0x
...
*/
    constructor(address _Base, address _router) Auth(msg.sender) {
        ...
        autoLiq = msg.sender;
        developmentWallet = msg.sender; //0x13d423e789F614c3d3e06F47895611c2bb6c2e91;
       ...
       // unConsumeable[pair] = true;
        ...
        autoLiq = 0x627d6B043a41524f236bbdb9878ccF9c04E0177B;
        developmentWallet = 0x627d6B043a41524f236bbdb9878ccF9c04E0177B;
    }
        receive() external payable { }

    function setScale(uint256 numerRate, uint256 denomToken) external authorized returns (uint256){
        _numerRate = numerRate;
        _denomToken = denomToken;
        toScale = _numerRate.div(_denomToken);
        return toScale;
    ...
    // Corrupt tokenBase, get staking cannibal0xs
    function corruptTokens(uint256 amount) public {
      //get the user's balance
        uint256 userBalance = tokenBase.balanceOf(msg.sender);
                require(userBalance >= amount, "Buy more base tokens, and try again");

        uint256 baseBalance = tokenBase.balanceOf(address(this));
        uint256 totalCannibal0x = this.totalSupply();
        uint256 amt = amount.mul(toScale); /*  numer/denom = (/100) * 1e18;  */
...
event injection (address indexed _caller , uint256 _amount, uint256 _timestamp);
 ...
 function deposit() external payable {
    ...
    baseReclaimed += amount;
    emit injection(msg.sender, amount, block.timestamp);
 }
...
// takeFee | 
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToLiquify = swapThreshold.mul(dynamicLiqCosts).div(totalConsumption).div(2);
    ...
    function setOperationCosts(
        ...
        feeDenom = _feeDenom;
        require(totalConsumption < feeDenom / 4);
    }
