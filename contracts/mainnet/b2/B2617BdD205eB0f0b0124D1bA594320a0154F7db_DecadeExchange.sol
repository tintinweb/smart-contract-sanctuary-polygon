...
//        // usdc-eth
        ...
        // Matic
        _oracles[0x1BFD67037B42Cf73acF2047067bd4F2C47D9BfD6] = 0x19b0F0833C78c0848109E3842D34d2fDF2cA69BA;
        // wbtc-eth
    ...
    function acceptance(LibTrade.Execution[] memory chunk, uint gasprice) public view returns (LibTrade.Acceptance[] memory) {
        ...
        for (uint i = 0; i < chunk.length; i++) {
            ...
            if (exec.ask.amount < filledAsk + exec.amount) continue;
            if (exec.bid.amount < filledBid + exec.amount) continue;

            (uint reserveAsk, uint reserveBid) = _reservesOf(exec.ask.account, exec.bid.account, exec.amount, exec.reserve);
            (,int _price,,,) = IFeedRegistry(_oracles[exec.base]).latestRoundData();
            uint txCost = gasprice * GAS_EXPECTATION * 1e18 / uint(_price) / (10 ** (18 - IERC20Metadata(exec.base).decimals())) / 2;
    ...
    function _txCostOf(address base, uint gasUsed) private view returns (uint) {
        (,int _price,,,) = IFeedRegistry(_oracles[base]).latestRoundData();
        return tx.gasprice * gasUsed * 1e18 / uint(_price) / (10 ** (18 - IERC20Metadata(base).decimals()));
    }
