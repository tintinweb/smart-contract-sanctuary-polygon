...
contract StrategyOtherPair is BaseStrategyStakingRewards {
    ...
    address[] private reward_matic_path;
        ...
        )
    {
        ...
        reward_matic_path[0] = QUICK;
        reward_matic_path[1] = WMATIC;

        IERC20(_tokenA).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
        IERC20(_tokenB).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
        IERC20(QUICK).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        uint256 _dQuick = IERC20(DQUICK).balanceOf(address(this));
        if(_dQuick > 0) {
            IERC20(DQUICK).safeTransfer(strategist, _dQuick);
        }
        uint256 matic_bal = IERC20(WMATIC).balanceOf(address(this));
        if(matic_bal > 0) {
            IERC20(WMATIC).safeTransfer(strategist, matic_bal);
        }
        uint256 _quick = IERC20(QUICK).balanceOf(address(this));
        if(_quick > 0) {
            IERC20(QUICK).safeTransfer(strategist, _quick);
        }
        ...
        //Convert some QUICK to WMATIC and distribute fee
        ...
        //Swap remaining QUICK for tokenA
...
abstract contract VaultBase is IVault, Ownable, ReentrancyGuard {
    ...
    //Allowed amount of the token sent to the fee dist each vault can mint ADDY rewards for, default 100000 WMATIC
    uint256 public override rewardAllocation = 100000;
