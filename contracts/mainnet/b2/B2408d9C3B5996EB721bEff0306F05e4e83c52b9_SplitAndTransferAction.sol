...
contract SplitAndTransferAction is BaseSiloAction{
    constructor(string memory _name, address _siloFactory, address[4] memory _inputs, address[4] memory _outputs, uint[4] memory _ratio, address[4] memory _recipients) {
        name = _name;
        metaData = "address[4],address[4],uint[4],address[4]";
        factory = _siloFactory;
        //configurationData = abi.encode(_inputs, _outputs, _ratio, _recipients);
    }
    ...
    function enter(address implementation, bytes memory configuration, bytes memory inputData) public override returns(uint[4] memory outputAmounts){
        ...
        if(storedConfig.length > 0){
            (input, output, ratio, recipients) = abi.decode(storedConfig, (address[4],address[4],uint[4],address[4]));
        }
        else{
            (input, output, ratio,recipients) = abi.decode(configuration, (address[4],address[4],uint[4],address[4]));
        }
        uint ratioSum;
        uint outputSum;
        ...
        for(uint i=0; i<4; i++){
            ratioSum += ratio[i];
            outputAmounts[i] = (inputAmounts[0] * ratio[i]) / baseDecimals;
            outputSum += outputAmounts[i];
        }
        if(outputSum < inputAmounts[0]){ //add any dust to the 0 output
            outputAmounts[0] += (inputAmounts[0] - outputSum);
        }
        require(ratioSum == baseDecimals, "Invalid Ratios");
        for(uint i=0; i<4; i++){
            if(recipients[i] != address(0) && recipients[i] != address(this) && outputAmounts[i] > 0){
                SafeERC20.safeTransfer(IERC20(output[i]), recipients[i], outputAmounts[i]);
            }
        }
    }
    ...
    function createConfig(address[4] memory _inputs, address[4] memory _outputs, uint[4] memory _ratio, address[4] memory _recipients) public pure returns(bytes memory configData){
        configData = abi.encode(_inputs, _outputs, _ratio, _recipients);
    }

    function decodeConfig(bytes memory configData) public pure returns(address[4] memory inputs, address[4] memory outputs, uint[4] memory ratio, address[4] memory recipients){
        (inputs,  outputs, ratio, recipients) = abi.decode(configData, (address[4], address[4], uint[4], address[4]));
    }
    ...
    function showBalances(address _silo, bytes memory _configurationData) external view override returns(uint, uint, address, address){}
}
...
abstract contract BaseSiloAction {
    ...
    function showFee(address _action) external view returns(string memory name, uint[4] memory fees){
        name = feeName;
        if(usesTakeFee){
    ...
    function showBalances(address _silo, bytes memory _configurationData) external view virtual returns(uint, uint, address, address);
