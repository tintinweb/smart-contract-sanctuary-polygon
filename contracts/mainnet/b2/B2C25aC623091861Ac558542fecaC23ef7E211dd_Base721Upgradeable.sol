    ...
    ERC721WithPermitUpgradable
{
    ...
    function buy(
        ...
        (, referralBonus, ) = _processPayment(typeInfo.paymentToken, total, _msgSender(), referral_);
    ...
    function batchMint(uint256 typeNFT_, uint256 quantity_, address recipient_) external onlyRole(MINTER_ROLE) {
        _setSold(typeNFT_, quantity_);
        uint256 tokenId = _idCounter;
        emit Registered(recipient_, typeNFT_, tokenId, quantity_, _sold[typeNFT_], address(0), address(0), 0);
        _batchProcess(
            recipient_,
            tokenId,
            quantity_,
    ...
    FeeDistributorUpgradeable
{
    ...
    function _calculateAmounts(
        ...
        if (affiliate_ == address(0)) {
            primarySaleAmount = (total_ * (_clientInfo.percentageInBps + affiliatePercentageInBps));
            affiliateAmount = 0;
            systemAmount = value_ - primarySaleAmount;
        } else {
