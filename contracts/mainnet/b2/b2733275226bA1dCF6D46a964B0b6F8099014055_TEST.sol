    ...
    ERC721PausableUpgradeable
{
    ...
    function initialize(uint256 mintFee, address withdrawAddress, uint256 maxSupply) public initializer {
    ...
    function mint(string memory uri, bytes32[] memory proof) external virtual payable{
        if(owner() != msg.sender){
            ...
            require(supply.add(1) <= _maxSupply, "NFT supply exceeded");
            require(msg.value >= _mintFee, "mint fee is not enough");
        }
    ...
    function withdraw() external onlyOwner{
        (bool sent, ) = _withdrawAddress.call{value: address(this).balance}("");
