...
abstract contract LibertiSwap {
    ...
    function swap(uint256 minOut, bytes calldata data) public returns (bytes memory) {
    ...
    OwnableUpgradeable
{
    ...
    ILibertiPriceFeed public immutable priceFeed;
    ...
    function _previewDeposit(uint256 assets) internal view returns (uint256, uint256) {
        ...
        return (
            supply.mulDiv(valueToken0 + valueToken1, nav, MathUpgradeable.Rounding.Down),
            sellToken0
        );
    }
    ...
    function deposit(
        ...
        require(assets <= maxDeposit(receiver), "ERC4626: deposit more than max");
        (uint256 shares, uint256 sellToken0) = _previewDeposit(assets);
        ...
        )
    {
        ...
        if (0 < totalToken1) {
            ...
            return (amountToken0 + amountsOut[1], amountToken0, amountToken1);
        }
        return (amountToken0, amountToken0, 0);
    }
    ...
    function _redeem0(
        ...
        (uint256 assets, uint256 amountToken0, uint256 amountToken1) = _previewRedeem(shares);
    ...
    function redeem(
        ...
        (uint256 assets, uint256 amountToken0) = _redeem0(shares, receiver, amountOutMin);
        _withdraw(_msgSender(), receiver, owner_, amountToken0, shares);
        return assets;
    }
    ...
    function totalAssets() public view override returns (uint256) {
        ...
        if (0 < otherBalance) {
            uint256[] memory amountsOut = getAmountsOut(otherBalance, other, asset());
            return (IERC20(asset()).balanceOf(address(this)) + amountsOut[1]);
        }
        return IERC20(asset()).balanceOf(address(this));
    }

    function convertToShares(uint256 assets) public view override returns (uint256 shares) {
        return assets.mulDiv(totalSupply(), totalAssets(), MathUpgradeable.Rounding.Down);
    }

    function convertToAssets(uint256 shares) public view override returns (uint256 assets) {
        return shares.mulDiv(totalAssets(), totalSupply(), MathUpgradeable.Rounding.Down);
    }
...
contract LibertiVaultETH is LibertiVault {
    ...
    function depositEth(address receiver, uint256 amountOutMin) public payable returns (uint256) {
        ...
        (uint256 shares, uint256 sellToken0) = _previewDeposit(msg.value);
        require(shares >= amountOutMin, "!min");
    ...
    function redeemEth(
        ...
        (uint256 assets, ) = _redeem0(shares, address(this), amountOutMin);
