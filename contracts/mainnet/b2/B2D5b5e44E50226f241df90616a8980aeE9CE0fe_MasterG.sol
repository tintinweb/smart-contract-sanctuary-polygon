...
contract MasterG is ERC1155, Ownable {
    ...
    mapping(uint256 => Signature[] ) private signaturesOfId;
    mapping(uint256 => mapping(address => bool)) private onSignatureWhitelist;

    //events
    event NftSigned(address indexed user_wallet, uint256 indexed token_id, string name, string description);
    
    constructor(string memory _symbol, string memory _name) ERC1155("") {
        name = _name;
        symbol = _symbol;
        highestId = 0;
    ...
    function addSignature(uint256 _id, string memory _name, string memory _description) public {
        require(onSignatureWhitelist[_id][msg.sender], "You do not have permission to sign this NFT or has already signed this NFT.");
        onSignatureWhitelist[_id][msg.sender] = false;
        signaturesOfId[_id].push(Signature(msg.sender,_name,_description));
        emit NftSigned(msg.sender, _id, _name, _description);
    }

    function getSignatures(uint256 id) public view returns(Signature[] memory) {
        return signaturesOfId[id];
    }

    // Signature Whitelist Functions

    function addToSignatureWhitelist(uint256 _id, address[] calldata addresses) external  onlyOwner {
      
      for (uint256 i = 0; i < addresses.length; i++) {
      require(addresses[i] != address(0), "Can't add the null address");

      onSignatureWhitelist[_id][addresses[i]] = true;
    }
  }

  function isUserOnSignatureWhitelist(address addr, uint256 _id) external view  returns (bool) {
    return onSignatureWhitelist[_id][addr];
  }

  function removeFromAllowList(uint256 _id, address[] calldata addresses) external  onlyOwner {
    for (uint256 i = 0; i < addresses.length; i++) {
      require(addresses[i] != address(0), "Can't remove the null address");

      /// @dev We don't want to reset possible _allowListClaimed numbers.
      onSignatureWhitelist[_id][addresses[i]] = false;
    }
  }
