...
contract Forwarder is IERC165, IForwarder, Pausable {
  ...
  function onlyRelayer() internal view {
    require(_msgSender() == IForwarderFactory(factoryAddress).relayerAddress(), "relayer only");
  }
  ...
  function _approve(address _tokenAddress) internal {
    SafeERC20.safeApprove(IERC20(_tokenAddress), IForwarderFactory(factoryAddress).handlerAddress(), type(uint256).max);
  }
  ...
  function _deposit(
    ...
    IBridge(IForwarderFactory(factoryAddress).bridgeAddress()).deposit(_destinationDomainID, _resourceID, _depositData, _feeData);
  }
  ...
  function _wrapAndDeposit(uint256 _ethBalance) internal whenNotPaused {
    ...
    _deposit(
      ...
      keccak256(bytes("WETH")), // resourceID
      abi.encode( // depositData
        address(this), // cVA address
        uint8(18), // weth decimal
  ...
  function setFactory(address _factoryAddress) external whenPaused {
    // owner check
    onlyOwner();
...
contract ForwarderFactory is Ownable, Pausable, IForwarderFactory {
  ...
  function owner() public view override(IForwarderFactory, Ownable) returns (address) {
    return super.owner();
  }
  ...
  function isForwarderDeployed(address _forwarderAddress) external view returns(bool){
