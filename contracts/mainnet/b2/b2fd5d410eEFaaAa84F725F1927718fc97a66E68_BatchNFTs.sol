...
contract BatchNFTs is Ownable, ERC721AQueryable, ReentrancyGuard {
    uint256 public immutable maxAmountPerMint;
    uint256 public mintPrice;
    bool public mintPaused;
    uint256 public current_supply = 0;
    string public _baseTokenURI;
    uint256 public zeroNFT = 0;
    address payable private ETHaccount;
    ...
    function mint(uint256 quantity) external payable nonReentrant {
        ...
        require(
            current_supply + quantity <= maxAmountPerMint,
            "Exceeds maximum supply"
        );
        require(quantity > 0, "Must mint at least one token");
        require(quantity <= 5, "Your minting quantity has been exceeded!");
        require(msg.value == quantity * mintPrice, "Incorrect Ether value");
        _mint(msg.sender, quantity);
        ETHaccount.transfer(msg.value);
    ...
    function setBaseURI(string calldata baseURI) external onlyOwner {
        require(bytes(baseURI).length > 0, "Base URI must be non-empty");
    ...
    function zeroBurn() external onlyOwner nonReentrant {
        require(zeroNFT == 0, "you not to zero");
        _mint(owner(), 1);
        zeroNFT = zeroNFT + 1;
    }

    function zeroCheck() public view returns (uint256) {
        return zeroNFT;
    }
}
...
import "./Queryable/IERC721ABurnable.sol";
import "./ERC721A.sol";
...
import "./Queryable/IERC721AQueryable.sol";
import "./ERC721A.sol";
