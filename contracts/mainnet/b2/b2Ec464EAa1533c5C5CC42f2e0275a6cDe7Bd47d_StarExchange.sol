    ...
    EIP712Upgradeable
{
    ...
    function initialize(string calldata name_, string calldata version_) external initializer {
        ...
        _grantRole(collectionRole, 0x040e646c218F593215e080bE9265E7f3087B82BE);

        _grantRole(operatorRole, 0xeB994692B35f54d979c8eE50Cc1040F9286E5954);
    ...
    function cancelAllOrders(uint256 minNonce_) external {
        ...
        if (minNonce_ <= nonce || minNonce_ >= nonce + 10000) revert Exchange__InvalidNonce();
    ...
    function cancelSellOrders(uint256[] calldata nonces_) external {
        ...
        if (length == 0) revert Exchange__ZeroValue();

        address sender = _msgSender();

        uint256 _minNonce = minNonce[sender]; // cache storage to memory
        BitMapsUpgradeable.BitMap storage map = _isNonceExecutedOrCancelled[sender];

        for (uint256 i; i < length; ) {
            if (nonces_[i] < _minNonce) revert Exchange__InvalidNonce();
    ...
    function _validateOrder(
        ...
        if (order.price == 0) revert Exchange__ZeroValue();
        ...
        if (!hasRole(CURRENCY_ROLE, order.currency)) revert Exchange__InvalidCurrency();
        if (!hasRole(COLLECTION_ROLE, order.collection)) revert Exchange__InvalidCollection();
        ...
        if (recoveredAddress == address(0) || recoveredAddress != order.signer)
