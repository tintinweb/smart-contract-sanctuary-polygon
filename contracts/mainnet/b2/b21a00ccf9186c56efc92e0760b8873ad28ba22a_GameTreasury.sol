...
contract GameTreasury is Treasury {
    ...
    */
    function initialize(address registry_,uint256 nonce_, bytes memory signature_) public initializer {
        __Treasury_init(Constants.GAME_TREASURY, registry_, nonce_, signature_);
        address cryptoRunnerState = registry.getEntry(Constants.CRYPTO_RUNNER_STATE);
        operators[cryptoRunnerState] = cryptoRunnerState != address(0);
    }
    /**
    @notice Setting the maximum reward threshold safeguard
    @param maximumReward_ The new threshold
    */
    function setMaximumReward(uint256 maximumReward_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 previousThreshold = maximumReward;
        if(maximumReward_ > data.balanceOf(address(this)))
            revert ExceedsCurrentBalance();
        maximumReward = maximumReward_;
        emit MaximumRewardChanged(msg.sender, previousThreshold, maximumReward_);
    }
    /**
    @notice Transfer an amount of DATA to an address
    @dev Only callable from approved operators
    @param to_ recipient
    @param amount_ the DATA
    */
    function transferRewards(address to_, uint256 amount_) external onlyApproved {
        if(to_ == address(0))
            revert CannotTransferToZeroAddress();
        if(maximumReward > 0 && amount_ > maximumReward)
            revert QuantityExceedsMaximumReward();
        data.transfer(to_, amount_);
    ...
    */
    modifier onlyApproved() override {
        address cryptoRunnerState = registry.getEntry(Constants.CRYPTO_RUNNER_STATE);
        address airdrops = registry.getEntry(Constants.AIRDROPS);
        if(!operators[msg.sender] && (msg.sender == cryptoRunnerState || msg.sender == airdrops)) {
            operators[msg.sender] = true;
            emit OperatorApproval(address(this), msg.sender, true);
        }
