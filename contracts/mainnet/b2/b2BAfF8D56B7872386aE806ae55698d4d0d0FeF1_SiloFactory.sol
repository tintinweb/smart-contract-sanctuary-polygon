...
contract SiloFactory is Ownable, ERC721Enumerable, ERC721Holder{
    ...
    function viewSiloStrategyMetaData(uint siloID) external view returns(string memory, uint, address[] memory actions, bytes[] memory configData){
        ISilo silo = ISilo(siloMap[siloID]);
        (actions, configData) = silo.viewStrategy();
        return (silo.strategyName(), silo.strategyCategory(), actions, configData);
    }
