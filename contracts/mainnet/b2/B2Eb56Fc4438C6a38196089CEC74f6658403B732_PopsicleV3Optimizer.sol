...
library PoolVariables {
    ...
    /// @return tickLowerA The lower tick of the range
    /// @return tickUpperA The upper tick of the range
    function getPositionTicks(IUniswapV3Pool pool, uint256 amount0Desired, uint256 amount1Desired, int24 baseThreshold, int24 tickSpacing, int24 tickLower, int24 tickUpper) internal view returns(int24 tickLowerA, int24 tickUpperA) {
        Info memory cache = 
        ...
        bool zeroGreaterOne = amountsDirection(currentTick, tickLower, tickUpper);
        ...
        tickLowerA = cache.tickLower;
        tickUpperA = cache.tickUpper;
    }
    ...
    function amountsDirection(int24 currentTick, int24 tickLower, int24 tickUpper) internal pure returns (bool zeroGreaterOne) {
        zeroGreaterOne =  abs(currentTick - tickLower) <= abs(currentTick - tickUpper);
    }
...
contract PopsicleV3Optimizer is ERC20Permit, ReentrancyGuard, IPopsicleV3Optimizer {
     ...
     */
     constructor(
        address _pool,
        address _strategy
    ) ERC20("Popsicle LP V3 MATIC / USDT", "PLP") ERC20Permit("Popsicle LP V3 MATIC / USDT") {
    ...
    function rerange() external override nonReentrant checkDeviation {
        ...
        (tickLower, tickUpper) = pool.getPositionTicks(balance0, balance1, baseThreshold, tickSpacing, tickLower, tickUpper);
    ...
    function rebalance() external override nonReentrant checkDeviation {
        ...
        // Get imbalanced token
        bool zeroForOne = PoolVariables.amountsDirection(currentTick, tickLower, tickUpper);
        ...
        (tickLower, tickUpper) = pool.getPositionTicks(cache.amount0Desired, cache.amount1Desired, baseThreshold, tickSpacing, tickLower, tickUpper);
