...
contract RelativeTestUSD is ERC20, ERC20Burnable, Pausable, AccessControl {
  ...
  function setWaitTime(uint256 _waitTime) external onlyRole(DEFAULT_ADMIN_ROLE) {
    waitTime = _waitTime;
  }

  function setDripAmount(uint256 _dripAmount) external onlyRole(DEFAULT_ADMIN_ROLE) {
    dripAmount = _dripAmount;
  }

  function faucetDrip() external whenNotPaused {
    require(allowedToDrip(msg.sender), "not allowed to drip yet");
  ...
  function allowedToDrip(address _address) public view returns (bool) {
    if (block.timestamp >= lastDripTime[_address]) {
      return true;
    }
    return false;
  }
  ...
  function _beforeTokenTransfer(
    ...
    require(
      hasRole(TRANSFER_ROLE, _from) || hasRole(TRANSFER_ROLE, _to),
