...
contract MagpieCore{
    ...
    function SwapInEth(uint256 amountOutMin,address _origToken, address swapTokenAddress, address currentContractAddress,  uint16 recipientChainId, bytes32 recipient,   uint32 nonceValue) external payable returns(uint64 tokenSequence, uint64 coreSequence){
            ...
            SafeERC20.safeIncreaseAllowance(originalToken, uniswapAddress, amountOutMin);
    ...
    function SwapInToken(uint256 amountOutMin, uint256 amountIn, address[] calldata path, address currentContractAddress,  uint16 recipientChainId, bytes32 recipient,   uint32 nonceValue) external payable returns(uint64 tokenSequence, uint64 coreSequence){
        {
        address _origToken = path[0];
        IERC20 originalToken = IERC20(_origToken);
        SafeERC20.safeIncreaseAllowance(originalToken, uniswapAddress, amountIn);
        uniswap.swapExactTokensForTokens(amountIn,amountOutMin,path,currentContractAddress,nonceValue+50000);
        }

        // {
        // address swapTokenAddress = path[path.length-1];
        // IERC20 token = IERC20(swapTokenAddress);
        // SafeERC20.safeIncreaseAllowance(token, bridgeAddress, amountOutMin);
        // tokenSequence = bridge.transferTokens{value: msg.value}(swapTokenAddress, amountOutMin, recipientChainId, recipient, arbiterFeeValue,nonceValue);
        // }
