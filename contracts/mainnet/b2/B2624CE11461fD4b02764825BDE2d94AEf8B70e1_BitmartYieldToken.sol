...
contract BitmartYieldToken is ERC721Enumerable, Ownable, ERC2981 {
    ...
    uint256 private _expiryTime = 365 days;
    ...
    string private _baseURI_ = "https://nftupload.infura-ipfs.io/ipfs/";

     modifier onlyAdmin() {
        require(_admin[_msgSender()] || _msgSender()==owner(), "Caller is not the an admin");
        _;
    }
    ...
    function setExpiredFundsAddress(address expiredFundsAddress) public onlyAdmin{
        _expiredFundsAddress = expiredFundsAddress;
    }

    function setbaseURI(string calldata baseURI) public onlyAdmin{
        _baseURI_ = baseURI;
    }

    function mint(string calldata cid, uint8 weight) public onlyAdmin{
        _metadata[_nextToken] = cid;
    ...
    function getWeight(uint256 tokenId) public view returns (uint256){
        return _yieldInfo[tokenId].weight;
    }

    function setProcessTimePeriod(uint256 period) public onlyAdmin{
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        return string(abi.encodePacked(_baseURI(), _metadata[tokenId]));
    }

    function purchaseTransfer(uint256 tokenId, address to) public onlyAdmin{
        require(ownerOf(tokenId)==owner(),"Token not available for purchase");
