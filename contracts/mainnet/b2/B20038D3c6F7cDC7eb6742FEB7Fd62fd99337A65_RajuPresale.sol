 ...
 */
 contract RajuPresale is Owned{
    ...
    function buy(uint _amount) public {
        ...
        {
            pTokenAmount = _amount.div(rate1).mul(RATE_DECIMALS).mul(10 ** 18);
        }
        else if(block.timestamp >= startTime2 && block.timestamp < (startTime2 + period2))
        {
            pTokenAmount = _amount.div(rate2).mul(RATE_DECIMALS).mul(10 ** 18);
        }
    ...
    function sell(uint _amount) public {
        ...
        {
            usdcAmount = _amount.mul(rate2).div(RATE_DECIMALS).mul(10 ** 6);
        }
