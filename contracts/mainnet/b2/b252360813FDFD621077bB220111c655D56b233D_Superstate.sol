...
contract Superstate is UniswapV2ERC20, IUniswapV2Pair {
    ...
    function getBalances0() public view returns (uint256) {
        return token0s.convertToAssets(token0s.balanceOf(address(this)));
    }

    function getBalances1() public view returns (uint256) {
        return token1s.convertToAssets(token1s.balanceOf(address(this)));
    }
    ...
    constructor(
        ...
        token0s = IERC4626(s0);
        token1s = IERC4626(s1);
        IERC20(token0).approve(s0,9999999999999999999999999999999999999999999999);
        IERC20(token1).approve(s1,9999999999999999999999999999999999999999999999);//)
    ...
    function swap(
       ...
       bytes calldata data
        ...
        if(balance0>getBalances0()){deposit0(IERC20(token0).balanceOf(address(this)));}
        else{deposit1(IERC20(token1).balanceOf(address(this))); }
        {
    ...
    function skim(address to) external lock override {
        address _token0 = token0; // gas savings
        address _token1 = token1; // gas savingssB\\ltt
