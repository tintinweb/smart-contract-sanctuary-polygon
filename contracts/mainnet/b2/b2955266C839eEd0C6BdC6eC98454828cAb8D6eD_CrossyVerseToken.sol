...
contract CrossyVerseToken is Context, IBEP20, Ownable {
    ...
    string private _name = "Crossy Verse";
    string private _symbol = "CROSSY";
    ...
    uint256 liquidityFeeCounter = 833;
    uint256 marketFeeCounter = 2499;
    uint256 dev1FeeCounter = 833;
    uint256 dev2FeeCounter = 833;
    uint256 dev3FeeCounter = 833;
    uint256 game1FeeCounter = 833;
    uint256 game2FeeCounter = 837;
    uint256 communityFeeCounter = 2499;
    ...
    constructor() {
        ...
        dev1 = payable(0xe8527846778Bc603956b087AAA164CD36fcbeB9F);
        dev2 = payable(0xe8527846778Bc603956b087AAA164CD36fcbeB9F);
        dev3 = payable(0xe8527846778Bc603956b087AAA164CD36fcbeB9F);
        game1Reward = payable(0xc8eF939e866d0686fA4ba6711f7c0cCB3CBbcc78);
        game2Reward = payable(0xC2CA36b800DE71dBc246b07039a42ed3E75D2605);
        marketWallet = payable(0x60ACbca99dbb2Cc4A07bF3eE59B62Fd9888dF0DB);
        communityReward = payable(0x6C327db008899FE169988439a0ABfe70b72B80fA);
        tokenFundWallet = payable(0xa3968Fba9D45D52b6b978f575934364ac2d5774c);
    ...
    function setDistributionPercent(
        ...
        uint256 _dev1FeeCounter,
        uint256 _dev2FeeCounter,
        uint256 _dev3FeeCounter,
        uint256 _game1FeeCounter,
        uint256 _game2FeeCounter,
        uint256 _communityFeeCounter,
        uint256 _divider
    ) external onlyOwner {
        liquidityFeeCounter = _liquidityFeeCounter;
        marketFeeCounter = _marketFeeCounter;
        dev1FeeCounter = _dev1FeeCounter;
        dev2FeeCounter = _dev2FeeCounter;
        dev3FeeCounter = _dev3FeeCounter;
        game1FeeCounter = _game1FeeCounter;
        game2FeeCounter = _game2FeeCounter;
        communityFeeCounter = _communityFeeCounter;
        divider = _divider;
    }
    ...
    // to move stuck matic
    function removeStuckMatic(address payable _wallet,uint256 _matic)external onlyOwner{
    ...
    function _takeDevFeesInMATIC(address sender, uint256 amount) internal {
        ...
        _balances[address(this)] = _balances[address(this)].add(
            _dev1Fee.add(_dev2Fee).add(_dev3Fee)
        );
        emit Transfer(
            sender,
            address(this),
    ...
    function _takeReaminingFeeInMATIC(address sender, uint256 amount) internal {
        ...
        _balances[address(this)] = _balances[address(this)].add(
            _lpFee.add(_marketFee).add(_game1Fee).add(_game2Fee).add(
                _communityFee
            )
        );
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            uint256 liquidityDivison = contractTokenBalance
                .mul(liquidityFeeCounter)
                .div(10000);

            uint256 halfLiquidity = liquidityDivison.div(2);
            uint256 otherHalfLiquidity = liquidityDivison.sub(halfLiquidity);
            ...
            uint256 bnbForDev1 = deltaBalance.mul(dev1FeeCounter).div(divider);
            uint256 bnbForDev2 = deltaBalance.mul(dev2FeeCounter).div(divider);
            uint256 bnbForDev3 = deltaBalance.mul(dev3FeeCounter).div(divider);
