...
contract Reward is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
  ...
  event TokensTransferred(address indexed user, uint256 _amount, uint256 indexed vehicleNodeId);

  event AdminWithdrawal(address indexed user, uint256 _amount);

  event DidntQualify(address indexed user, uint256 _amount, uint256 indexed vehicleNodeId);
   ...
   * @param tokenAddress The contract address for the DIMO token
   * @param vehicleNftAddress The contract address for the DIMO vehicle NFT
   * @param sanctionsContractAddress The contract address for the Chainalysis sanctions oracle
   */
  function initialize(address tokenAddress, address vehicleNftAddress, address sanctionsContractAddress) external initializer {
    ...
    require(vehicleNftAddress != address(0), "vehicleNftAddress is an invalid zero address");
    ...
    vehicleNft = INft(vehicleNftAddress);
    ...
    // currentWeek and currentWeekSpent start at 0.
    currentWeekLimit = initialWeeklyLimit;
  }

  /* ========== MUTATIVE FUNCTIONS ========== */

  /** @dev Advances the current week if necessary. If unnecessary then this is a no-op.
   */
  function updateWeek() private {
  ...
  /** @dev Admin function to manage genesis time if deployed sooner
   */
  function resetRewardsGenesisTime() external onlyRole(DEFAULT_ADMIN_ROLE) {
    rewardsGenesisTime = block.timestamp;
  }

  /** @dev administrative function to manage genesis time if it needs to be set to a specific time.
   * @param timestamp timestamp to set as the genesis time, past or future.
   */
  function manuallySetRewardsGenesisTime(uint256 timestamp) external onlyRole(DEFAULT_ADMIN_ROLE) {
    rewardsGenesisTime = timestamp;
  }
   ...
   */
  function batchTransfer(address[] calldata users, uint256[] calldata values, uint256[] calldata vehicleIds) external onlyRole(ORACLE_ROLE) {
    // Advance the week if necessary.
    updateWeek();
    ...
    for (uint256 i = 0; i < arrayLength; i++) {
      vehicleOwner = vehicleNft.ownerOf(vehicleIds[i]);

      // check to see user address is sanctioned or not
      if (vehicleOwner == users[i] && minimumRequiredTimeHasPassed(vehicleIdLastRewardTime[vehicleIds[i]]) && !sanctionsList.isSanctioned(vehicleOwner)) {
