...
contract Marketplace {
    ...
    function listForSale(uint256 itemId, uint256 priceInMatic) external onlyValidCollection {
        ...
        items[itemId] = Item(msg.sender, priceInMatic, true, false, 0, address(0));
        emit ItemListed(itemId, msg.sender, priceInMatic, false);
    }
    ...
    function endAuction(uint256 itemId) external onlyMarketplaceOwner {
        ...
        if (winner != address(0)) {
            collection.transferFrom(item.seller, winner, itemId);
