...
contract TestingNFT is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    constructor(string memory name, string memory symbol, uint256 _nftPrice) public ERC721(name, symbol) {
        nftPrice = _nftPrice;
    }
    ...
    function mint(uint numberOfTokens) public payable {
        ...
        require(nftPrice.mul(numberOfTokens) <= msg.value, "MATIC value sent is not correct");
        
        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < MAX_SUPPLY) {
                if (totalSupply()==4) {
                    changeNftPrice(nftPrice*2);
                }
