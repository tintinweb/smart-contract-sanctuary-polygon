...
contract TidePool is ITidePool, IUniswapV3MintCallback, IUniswapV3SwapCallback, ERC20 {
    ...
    function deposit(uint256 amount0, uint256 amount1) external override returns (uint128 liquidity) {
        ...
        if(rewards0 > 0 || rewards1 > 0) pool.mintManagedLiquidity(rewards0, rewards1, tickLower, tickUpper);
        ...
        if(leftover0 > 0) pay(token0, address(this), msg.sender, leftover0);
        if(leftover1 > 0) pay(token1, address(this), msg.sender, leftover1);
    ...
    function withdraw() external override {
        ...
        uint256 userLiquidityShare = balance * uint256(pool.getPosition(tickLower, tickUpper)) / totalSupply();

        // burn liquidity proportial to user's share
        (uint256 amount0, uint256 amount1) = pool.burnManagedLiquidity(userLiquidityShare.toUint128(), tickLower, tickUpper);
        _burn(msg.sender, balance);

        // compound any leftover rewards back into the pool
        pool.mintManagedLiquidity(rewards0 - userRewards0Share, rewards1 - userRewards1Share, tickLower, tickUpper);
    ...
    function harvest() private returns (uint256 rewards0, uint256 rewards1, uint256 fees0, uint256 fees1) {
        ...
        if(fees0 > 0) pay(token0, address(this), treasury, fees0);
        if(fees1 > 0) pay(token1, address(this), treasury, fees1);
    }
    ...
    function swapIntoRatio(uint256 amount0, uint256 amount1) private returns (uint256 desired0, uint256 desired1) {
        ...
        (uint256 useable0, uint256 useable1) = TidePoolMath.getUsableLiquidity(sqrtRatioX96, amount0, amount1, tickLower, tickUpper);
        ...
        if(TidePoolMath.zeroIsLessUsed(useable0, amount0, useable1, amount1)) {
            ...
            if(amountIn > 0) {
                (amountOut, leftover) = pool.swapWithLimit(token0, token1, amountIn, slippageBps);
            ...
            // corner case where we already have correct ratio and don't need to swap
            if(amountIn > 0)  {
                (amountOut, leftover) = pool.swapWithLimit(token1, token0, amountIn, slippageBps);
                locked = leftover > amountIn * slippageBps / 1e5 ? true : false;
            }
    ...
    function rerange() external override {
        ...
        require(totalSupply() > 0,"S");
        require(diff > 1 days,"1D");
    ...
    function emergencyWithdraw() public {
        pool.burnManagedLiquidity(pool.getPosition(tickLower, tickUpper), tickLower, tickUpper);
        pay(token0, address(this), treasury, IERC20(token0).balanceOf(address(this)));
        pay(token1, address(this), treasury, IERC20(token1).balanceOf(address(this)));
    }
}
...
interface ITidePool {
    ...
    event Deposited(address owner, uint256 amount0, uint256 amount1);
...
library TidePoolMath {
    ...
    function getUsableLiquidity(uint160 sqrtRatioX96, uint256 amount0, uint256 amount1, int24 tickLower, int24 tickUpper) public pure returns (uint256, uint256) {
