...
contract Treasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        ...
        address(0x8E879afEB733c760C7E0095236Dba5B3b0fD9A9e),
        address(0x1873e05454d0A7d81648638f127BA3d85F77E8dA)
    ];
    ...
    function getExpansionPercent() public view returns (uint256) {
        uint256 previousEpochWlrsPrice = getWlrsPrice();
        uint256 _percentage = previousEpochWlrsPrice.sub(wlrsPriceOne);
        uint256 _mse = maxSupplyExpansionPercent.mul(1e14);
        if (fixedExpansion != 0) {
            _percentage = fixedExpansion;
        } else if (expansionFactor != 0) {
            _percentage = _percentage.mul(expansionFactor).div(10000);
        }

        if (fixedExpansion == 0 && minExpansion > _percentage) { // if min expansion threshold enabled then set lower limit
