...
contract UniswapPairOracle is Ownable {
    ...
    function twap(address token, uint256 pricePrecision) external view returns (uint256 amountOut) {
        ...
        if (token == token0) {
            amountOut = price0Average.mul(pricePrecision).decode144() * (10**_token1MissingDecimals);
        } else {
            require(token == token1, "PairOracle: INVALID_TOKEN");
            amountOut = price1Average.mul(pricePrecision).decode144() * (10**_token0MissingDecimals);
        }
    }
