...
// File: contracts/Admin/data/GachaData.sol
...
contract DspGachaData is Ownable {
    ...
    function setGachaInfo(GachaInfo memory _gachaInfo) external onlyOwner {
        require(_gachaInfo.tokenId != 0, "gacha id not valid");
    ...
    function setGachaInfos(GachaInfo[] memory _gachaInfo) external onlyOwner {
        for (uint256 i = 0; i < _gachaInfo.length; i++) {
            require(_gachaInfo[i].tokenId != 0, "gacha id not valid");
            uint256 sumTierRatio = 0;
            for (uint256 j = 0; j < _gachaInfo[i].tierRatio.length; j++) {
                sumTierRatio += _gachaInfo[i].tierRatio[j];
            }
            require(sumTierRatio != 0, "gacha tier ratio sum 0");

            for (uint256 j = 0; j < _gachaInfo[i].gachaGradeRatio.length; j++) {
                if (_gachaInfo[i].tierRatio[j] != 0) {
                    uint256 sumGachaGradeRatio = 0;
                    for (uint256 k = 0; k < _gachaInfo[i].gachaGradeRatio[j].length; k++) {
                        sumGachaGradeRatio += _gachaInfo[i].gachaGradeRatio[j][k];
                    }
                    require(sumGachaGradeRatio != 0, "gacha gacha grade ratio sum 0");
                }
            }
            if (!gachaInfo[_gachaInfo[i].tokenId].isValid) {
                gachaCount++;
            }
            gachaInfo[_gachaInfo[i].tokenId] = _gachaInfo[i];

            emit SetGachaInfo(_gachaInfo[i].tokenId, _gachaInfo[i].name, _gachaInfo[i].tierRatio, _gachaInfo[i].gachaGradeRatio, _gachaInfo[i].isValid);
        }
    }
...
// File: contracts/Admin/AdminGachaTicketAirdropMachine.sol
...
contract AdminGachaTicketAirdropMachine is LuxOnSuperOperators, LuxOnLive {
    event SetMintAddress(address indexed mintAddress);
    event SetGachaDataAddress(address indexed gachaDataAddress);
    event Airdrop(address indexed to, address indexed mintAddress, uint256 indexed tokenId, uint256 amount);
    ...
    constructor(
        ...
        string memory operator,
        address luxOnAdmin
    ) LuxOnLive(luxOnService) LuxOnSuperOperators(operator, luxOnAdmin) {
    ...
    function airdrop(AirdopInfo memory airdopInfo) external onlySuperOperator {
        ...
        emit Airdrop(airdopInfo.to, mintAddress, airdopInfo.tokenId, airdopInfo.amount);
    }

    function airdropMany(AirdopInfo[] memory airdopInfo) external onlySuperOperator {
        for (uint256 i = 0; i < airdopInfo.length; i++) {
            ...
            emit Airdrop(airdopInfo[i].to, mintAddress, airdopInfo[i].tokenId, airdopInfo[i].amount);
        }
    }
}