...
contract AddressesProvider is IAddressesProvider, Initializable, OwnableUpgradeable {
    mapping(bytes32 => address) private _addresses;

    bytes32 private constant REWARDS_DISTRIBUTOR = "REWARDS_DISTRIBUTOR";
    bytes32 private constant COMMISSION_DISTRIBUTOR = "COMMISSION_DISTRIBUTOR";
    ...
    bytes32 private constant KYC_STORE = "KYC_STORE";
    bytes32 private constant RESELL_POOL_FACTORY = "RESELL_POOL_FACTORY";
    bytes32 private constant ORACLE_MANAGER = "ORACLE_MANAGER";
    ...
    function setAccessManager(address _accessManager) external override onlyOwner {
        _addresses[ACCESS_MANAGER] = _accessManager;
    ...
    function getCommissionsDistributor() external view override returns (address) {
        return getAddress(COMMISSION_DISTRIBUTOR);
    }

    function setCommissionsDistributor(address _commissionDistributor) external override onlyOwner {
        _addresses[COMMISSION_DISTRIBUTOR] = _commissionDistributor;
        emit CommissionsDistributorUpdated(_commissionDistributor);
    }

    function getKycStore() external view override returns (address) {
        return getAddress(KYC_STORE);
    }

    function setKycStore(address _address) external override onlyOwner {
        _addresses[KYC_STORE] = _address;
        emit KycStoreUpdated(_address);
    }

    function getResellPoolFactory() external view override returns (address) {
        return getAddress(RESELL_POOL_FACTORY);
    }

    function setResellPoolFactory(address _address) external override onlyOwner {
        _addresses[RESELL_POOL_FACTORY] = _address;
        emit ResellPoolFactoryUpdated(_address);
    }

    function getOracleManager() external view override returns (address) {
        return getAddress(ORACLE_MANAGER);
    }

    function setOracleManager(address _address) external override onlyOwner {
        _addresses[ORACLE_MANAGER] = _address;
        emit OracleManagerUpdated(_address);
    }
}
