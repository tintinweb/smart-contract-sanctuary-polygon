...
contract NaffleTicket is Ownable {
    ...
    mapping(address => uint256) _balanceTickets;
    ...
    function swap(address _addressDollar, uint256 tax) public payable {
        ...
        uint256 currentBalance = _balanceTickets[msg.sender] * (10**decimals);
        _balanceTickets[msg.sender] = 0;
        
        IERC20 contractDollar = IERC20(_addressDollar);
        uint256 contractBalance = contractDollar.balanceOf(address(this));
        require(contractBalance >= currentBalance , "Choose another stablecoin to withdraw");
 
        (bool sent) = contractDollar.transfer(msg.sender, currentBalance);
        require(sent, "Failed to transfer tickets");

        emit SwapTickets(msg.sender, currentBalance);
    }
}