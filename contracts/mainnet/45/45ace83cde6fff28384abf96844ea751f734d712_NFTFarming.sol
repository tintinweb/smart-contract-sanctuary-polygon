...
contract NFTFarming is IERC777Recipient {
    ...
    constructor() payable {
        artisToken = IERC777(0x8EC776d8Eda7275aa566794D6578eF607C32a02C);
        nftMinter = ArtisLifeNFT(0xe974Fcf6dfB733bEf5319D866c9d0F1e372C25a7);
    ...
    */
    function getFee(bytes calldata nftID) external view returns (uint256) {
    ...
    */
    function withdraw(
        bytes calldata nftID,
        uint256 editionNum
    ) external poolExist(nftID) withdrawLocked(nftID, msg.sender) {
        require(
            (allNFTs[nftID].stakingPool[msg.sender] > 0),
        ...
        nftMinter.mintFor(msg.sender, allNFTs[nftID].unftId, 1, editionNum, "");
    ...
    */
    function createNFTPool(
        ...
        uint256 maxPayout,
        string memory uri
    ) external onlyOwner poolNotExist(nftId) {
    ...
    */
    function artistPayout(bytes calldata nftID) external {
        require(
        ...
        The payout is calculated by max payout multiplied by [ NFT minted / NFT total supply ]
    */
    function artistPayoutAdmin(bytes calldata nftID) external onlyOwner(){
        require(allNFTs[nftID].settled == false, "Already paid out.");
        require(canDeposit(nftID) == false, "Payout not available yet.");
        uint256 payout = allNFTs[nftID].maxPayout.mul(
            (allNFTs[nftID].amountLeftToClaim.div(allNFTs[nftID].TOTAL_SUPPLY))
    ...
    */
    function collectLeftoverFunds(bytes calldata nftID) external onlyOwner {
    ...
    */
    function setERC777Address(address artisAddress) external onlyOwner {
        artisToken = IERC777(artisAddress);
    }
    ...
    */
    function setFee(bytes calldata nftID, uint256 _fee) external onlyOwner {
