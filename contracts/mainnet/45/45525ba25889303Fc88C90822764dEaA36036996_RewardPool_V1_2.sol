...
// File: https://raw.githubusercontent.com/OpenZeppelin/openzeppelin-contracts/master/contracts/token/ERC20/utils/SafeERC20.sol
...
contract RewardPool_V1_2 is Ownable {
    ...
    constructor (
        ...
        require(_PENT != address(0), "Zero Address");
        PENT = _PENT;
    ...
    function setPENTAddress(address _PENT) external onlyOwner {
        require(_PENT != address(0), "Zero Address");
        PENT = _PENT;
    }
    ...
    function rewardTo(address _account, uint256 _rewardAmount) external onlyManager {
        require(IERC20(PENT).balanceOf(address(this)) > _rewardAmount, "Insufficient Balance");

        SafeERC20.safeTransfer(IERC20(PENT), _account, _rewardAmount);
    }

    function withdrawToken() external onlyManager {
        uint256 balance = IERC20(PENT).balanceOf(address(this));

        require (balance > 0, "Insufficient Balance");
        SafeERC20.safeTransfer(IERC20(PENT), treasury, balance);
    }
}