...
abstract contract Supervisable is Context {
    ...
    event SupervisorOwnershipTransferred(
        address indexed previouSupervisor,
    ...
    modifier onlySupervisor() {
        require(
            supervisor() == _msgSender(),
            "Supervisable: caller is not the supervisor"
        );
        _;
    }
     ...
     * Internal function without access restriction.
     */
    function _transferSupervisorOwnership(address newSupervisor)
        internal
        virtual
    {
...
contract Lockable is Context {
    ...
    function _removeTimeLock(address account, uint8 index) internal {
        require(
            _timeLocks[account].length > index && index >= 0,
            "Time Lock: invalid index"
        );
     ...
     */
    function getTimeLock(address account, uint8 index)
        public
        view
        returns (uint256, uint256)
    {
        ...
        return (
            _timeLocks[account][index].amount,
     ...
     */
    function getTimeLockedAmount(address account)
        public
        view
        returns (uint256)
    {
    ...
    function _addVestingLock(
        ...
        require(
            account != address(0),
            "Vesting Lock: lock from the zero address"
        );
    ...
    {
        return (
            _vestingLocks[account].amount,
            _vestingLocks[account].startsAt,
            _vestingLocks[account].period,
            _vestingLocks[account].count
        );
    }
     ...
     */
    function getVestingLockedAmount(address account)
        public
        view
        returns (uint256)
    {
        ...
        if (amount > 0) {
            ...
            if (timestamp < startsAt) {
                ...
                vestingLockedAmount =
                    (amount * ((expiresAt - timestamp) / period + 1)) /
                    count;
            }
        }
...
contract MNA is
    ...
    ERC20
{
    uint256 private constant _initialSupply = 4_000_000_000e18;

    constructor() ERC20("MNA TOKEN", "MNA") {
    ...
    function _beforeTokenTransfer(
        ...
        require(
            !isFreezed(from),
            "Freezable: token transfer from freezed account"
        );
        require(!isFreezed(to), "Freezable: token transfer to freezed account");
        require(
            !isFreezed(_msgSender()),
            "Freezable: token transfer called from freezed account"
        );
        require(!paused(), "Pausable: token transfer while paused");
        require(
            balanceOf(from) - getAllLockedAmount(from) >= amount,
            "Lockable: insufficient transfer amount"
        );
    }
     ...
     */
    function transferSupervisorOwnership(address newSupervisor)
        public
        onlySupervisor
        whenNotPaused
    {
        require(
            newSupervisor != address(0),
            "Supervisable: new supervisor is the zero address"
        );
     ...
     */
    function removeTimeLock(address account, uint8 index)
        public
        onlySupervisor
        whenNotPaused
    {
     ...
     */
    function removeVestingLock(address account)
        public
        onlySupervisor
        whenNotPaused
    {
