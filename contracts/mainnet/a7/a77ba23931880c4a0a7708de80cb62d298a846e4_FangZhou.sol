...
/**
 * @title 方舟数藏
 * website https://fangzhou.allen3.cn
 * deploy on matic
 */
                                                                                                     
                                                                                            
 
contract FangZhou is ERC721A, Ownable {
        
    bool public saleIsActive = true;
    uint256 public Price = 1000 ether;
    uint256 public totalNum = 12000;
    string private _baseURIextended = "https://lq-nft-fangzhou.oss-cn-hongkong.aliyuncs.com/nft/metadata/fangzhou-metadata-";	     
    address private proxyRegistryAddress;    
	
    constructor() ERC721A("FangZhou", "FZ",500) {       
    }
    ...
    function gift(address[] memory _tos,uint256 numberOfTokens) external onlyOwner {            
        uint256 num = _tos.length;
        require(totalSupply() + num * numberOfTokens <= totalNum, "already mint out");     
        for(uint256 i = 0;i < num ; i++){       
            _safeMint(_tos[i], numberOfTokens);
        }
    }
