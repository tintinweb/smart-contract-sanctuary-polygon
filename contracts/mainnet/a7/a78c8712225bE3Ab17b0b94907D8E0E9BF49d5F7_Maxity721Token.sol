...
contract Maxity721Token is Ownable,Pausable ,ERC721URIStorage,IMaxityMetadata,IMaxity721Token{
    ...
    function mint(address to) public onlyWhiteList whenNotPaused {
    ...
    function mint(address designer,address to,uint count,uint _futureRoyalty,string memory uri) external override onlyWhiteList whenNotPaused returns (uint256[] memory tokenids)  {
        ...
        for(uint256 i; i < count; i++) {
            ...
            _disigners[newItemId] = designer;
            new_tokenids[i]=newItemId;
            futureRoyalty[newItemId] = _futureRoyalty;
            emit TokenMint(newItemId,designer,to,uri);
        }
        return new_tokenids;
    }
    ...
    function burn(uint256 tokenId) public onlyWhiteList whenNotPaused {
        ...
        if (_disigners[tokenId] != address(0x0)) {
            delete _disigners[tokenId];
        }
    ...
    function addWhiteList(address _addWhitelist) public onlyOwner returns (bool) {
        require(_addWhitelist != address(0), "_addMinter is the zero address");
    ...
    function delWhiteList(address _delWhiteList) public onlyOwner returns (bool) {
        require(_delWhiteList != address(0), "_delMinter is the zero address");
