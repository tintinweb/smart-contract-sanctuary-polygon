...
contract Superstate is UniswapV2ERC20, IUniswapV2Pair {
    ...
    //  if (flag == 0){ all deposits on

    ///  }else if(flag==1){ asset 1 on 2 off

    //   }else if (flag==2){asset 1 off 2 on

    //  }
    // else{ all deposits off
    ...
    function getBalances0() public view returns (uint256) {
        if (flag == 0 || flag == 1) {
            ...
            return IERC20(token0).balanceOf(address(this)); // token0s.convertToAssets(token0s.balanceOf(address(this)));
            // balance balance
        } else if (flag == 2) {} else {}
        //}//
    }
