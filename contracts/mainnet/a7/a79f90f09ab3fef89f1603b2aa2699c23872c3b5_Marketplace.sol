...
contract Marketplace is IMarketplace, Ownable {
    uint256 private immutable HUNDRED = 100 ether;

    /// 1% platform charges to buy a NFT
    uint256 private SELLER_PERCENTAGE;
    ...
    mapping(uint256 => IMarketplace.OrderData) public Order;
    ...
    event Unlisted(uint256 indexed listId);
    event Bought(uint256 indexed listId);
    event OfferExecuted(
        address nft,
        uint256 tokenId,
        uint256 price,
    ...
    event PriceUpdated(uint256 indexed listId, uint256 price);
    ...
    constructor(
        ...
        WETH = _WETH;
        SELLER_PERCENTAGE = _charges;
    }

    /**
     * @notice update royaltyEngine address.
    ...
    /**
     * @notice update rosolver address.
     * @dev onlyOwner can call this
     */
    ...
    /**
     * @notice update Platform charges address.
     * @dev onlyOwner can call this
     */
    function updateCharges(uint256 _charges) external onlyOwner {
        require(
            _charges <= 100 ether && _charges > 90 ether,
            "Invalid Charges"
        );
        SELLER_PERCENTAGE = _charges;
    }
    ...
    /**
     * @notice update NFT sell price.
     * @dev only NFT owner can update price
     * Emits {PriceUpdated} event.
     */

    function updateSellPrice(uint256 _listId, uint256 _price) external {
        require(_price != 0, "Invalid Price");
        if (Order[_listId].ownerAddress == msg.sender) {
            Order[_listId].sellPrice = _price;
        } else {
            revert("Invalid-_listId");
        }
        emit PriceUpdated(_listId, _price);
    }

    /**
     * @notice Can bulk List NFTs for sell.
     * @dev list details stored in "Order" mapping
     * Emits {listed} event.
     */

    function list(ListRequest[] calldata data) external {
        /// This variables could be declared inside the loop, but that causes the compiler to allocate memory on each
        ...
        for (uint256 i; i < data.length; ) {
            ...
            _list.sellPrice = _listData.sellPrice;
            console.log("sellP",_list.sellPrice);
    ...
    /**
     * @notice buyNFT
     * -check royalties info from royaltyEigne
     * -deduct 1% as protocol charges
     */
    ...
    function buyNft(BuyRequest[] memory data) external payable {
        ...
        bool success;
        console.log("SHHHHH",address(this).balance);
        for (uint256 i; i < data.length; ) {
            _buyData = data[i];
            _id = _buyData.id;
            _sell = Order[_id];
            console.log(_sell.sellPrice);

            uint256 abc = (_sell.sellPrice * SELLER_PERCENTAGE) / HUNDRED;
            console.log("SHHHHH",address(this).balance);
            console.log("abc",abc);
            ///  tranfer 99% of buy price to seller
            (success, ) = payable(_sell.ownerAddress).call{
                value: (_sell.sellPrice * SELLER_PERCENTAGE) / HUNDRED
            }("");
            require(success, "Transfer Failed");
            console.log(IERC721(_sell.nft).ownerOf(_sell.tokenId));
            _transferToken(_sell.nft,address(this),msg.sender,_sell.tokenId, _sell.lendAmount);
            console.log(IERC721(_sell.nft).ownerOf(_sell.tokenId));
            console.log(address(this));
            console.log(msg.sender);
            checkAndDeductRoyalties(_sell.nft, _sell.tokenId, _sell.sellPrice);
            _total = _total + _buyData.charges;
        ...
        ///sum of charges is equal to msg.value
        require(_total == _msgvalue, "Invalid ETH Amount");
     ...
     * -validate offer
     * -transfer NFT
     * -calculate 1% as protocol charges
     * -transfer respective amount
     *  Emits {OfferExecuted}
     */
    ...
    function executeOffer(
        ...
        _transferToken(_nft, msg.sender, _recevier, _tokenId, _amount);
        uint256 sellerAmount = (_price * SELLER_PERCENTAGE) / HUNDRED;
        IERC20(WETH).transferFrom(_recevier, msg.sender, sellerAmount);
        IERC20(WETH).transferFrom(_recevier, devAdd, _price - sellerAmount);
        emit OfferExecuted(_nft, _tokenId, _price, _recevier);
    }
    ...
    */
    ...
    function checkAndDeductRoyalties(
        ...
        if (recipients.length > 0) {
            ...
            for (uint256 i; i < recipients.length; ) {
                ...
                if (!success) require(success, "Tranfer Failed");
    ...
    function toUint96(uint256 y) internal pure returns (uint96 z) {
        require((z = uint96(y)) == y, "Invalid_TypeCasting");
    }
...
interface IMarketplace {
    ...
    struct ListRequest {
        address nft;
        uint256 tokenId;
        uint256 lendAmount; // copies/ quantity
        uint256 sellPrice; // what the name suggests -,-
    }
