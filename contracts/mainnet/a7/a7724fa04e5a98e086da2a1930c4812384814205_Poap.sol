...
contract Poap is ERC1155URIStorage, Ownable, ERC2981 {
    //@param flg of pause
    bool public paused;

    //@param tokenToLimitNum: minting limitation by user for each tokenid
    mapping(address => mapping(uint256 => uint256)) public mintLimitByUserForTokneId;
    ...
    function mint(
        ...
        require(
            mintLimitByUserForTokneId[_receiver][_tokenId] < tokenToLimitNum[_tokenId],
            "You have already received"
        );
        require(
            (mintLimitByUserForTokneId[_receiver][_tokenId] + _amount) <= tokenToLimitNum[_tokenId],
            "Mints num exceeded limit"
        );

        //effect
        mintLimitByUserForTokneId[_receiver][_tokenId] += _amount;
