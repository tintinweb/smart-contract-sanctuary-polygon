...
contract Marketplace is BaseContract {
  ...
  function takeOrder(address _nftContractAddress, uint _tokenId, address _paymentAddress, uint _weekNumber) external validOrder(_nftContractAddress, _tokenId) {
    ...
    order.takeOrderPrice = isInstallmentRentalOrder ? currentPrice / _weekNumber : currentPrice;
  ...
  function extendOrder(address _nftContractAddress, uint _tokenId, address _paymentAddress, uint _weekNumber) external {
    ...
    uint currentPrice = _discountPrice(order, order.takeOrderPrice * _weekNumber, _paymentAddress);
  ...
  function _discountPrice(Order storage _order, uint _currentPrice, address _paymentAddress) private view returns(uint) {
    ...
    if (_order.discounts[indexOrderPayment] > 0) {
      return _currentPrice * (oneHundredPercentageDecimal3 - _order.discounts[indexOrderPayment]);
    }

    return _currentPrice;
  }

  function _validateDiscounts(uint32[] calldata _discounts) private {
    for (uint i = 0; i < _discounts.length; i += 1) {
      require(_discounts[i] <= oneHundredPercentageDecimal3, "Marketplace: discount invalid");
    }
  }
  ...
  function _getCurrentPrice(address _nftContractAddress, uint _tokenId, uint _weekNumber, address _paymentAddress) private view returns (uint) {
    ...
    currentPrice = _discountPrice(order, currentPrice, _paymentAddress);
