...
abstract contract Token is IERC20, Ownable {
    ...
    constructor (
        ...
        _feeConfigs.push(FeeConfig(900000000 * tokenDecimals, 0, 0, 50));//if burn to 0xdEaD ,0.5% Fee LP
        _feeConfigs.push(FeeConfig(800000000 * tokenDecimals, 100, 100, 0));//if burn to 0xdEaD ,2% Fee
        _feeConfigs.push(FeeConfig(700000000 * tokenDecimals, 200, 200, 0));//if burn to 0xdEaD ,4% Fee
        _feeConfigs.push(FeeConfig(600000000 * tokenDecimals, 300, 200, 100));//if burn to 0xdEaD ,6% Fee
        _feeConfigs.push(FeeConfig(500000000 * tokenDecimals, 300, 400, 100));//if burn to 0xdEaD ,8% Fee
        _feeConfigs.push(FeeConfig(0, 500, 400, 100));//if burn to 0xdEaD ,10% Fee
...
contract Contract is Token {
    constructor(address SwapRouter,address USDT,string memory tokenName,
        string memory tokenSymbol, address Fund, address buy, address sale, address ai) Token(
