...
// File: contracts/Degenerates.sol
...
contract Degenerates is Ownable, ERC721, DefaultOperatorFilterer {
    using Strings for uint256;
    using Counters for Counters.Counter;
    string public baseURI;
    uint256 public MAX_SUPPLY = 7777;
    bool public SaleActive = false;
    uint256 public publicSalePrice = 50 ether;
    uint256 public MAX_PER_WALLET_PUBLIC = 500;
    ...
    ) ERC721("Degenerates", "Degenerates") {
        baseURI = _baseURI;
    ...
    function batchGift(address[] memory _to, uint256[] memory _tokens)
        external
        onlyOwner
    {
        require(_to.length == _tokens.length, "Mismatched arrays");
        require(
            totalSupply() + _tokens.length <= MAX_SUPPLY,
            "Exceeds max Supply"
        );
        for (uint256 i = 0; i < _to.length; i++) {
            _safeMint(_to[i], _tokens[i]);
            supply.increment();
        }
    }
