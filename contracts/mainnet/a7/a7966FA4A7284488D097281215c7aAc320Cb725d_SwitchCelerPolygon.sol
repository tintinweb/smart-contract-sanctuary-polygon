...
contract SwitchCelerPolygon is SwitchPolygon {
    ...
    ) SwitchPolygon(_switchViewAddress, _switchEventAddress)
        public
    {
        ...
        returns (IMessageReceiverApp.ExecutionStatus)
    {
        ...
        if (m.bridgeToken == m.dstToken) {
            ...
            (status, dstAmount) = _swapInternalForCeler(_amount, parts, lastNonZeroIndex, m);
        }
    ...
    function _swapBeforeCeler(TransferArgsCeler calldata transferArgs) private returns (uint256 returnAmount, uint256 parts) {
        ...
        returnAmount = _swapInternalForSingleSwap(transferArgs.srcDistribution, transferArgs.amount, parts, lastNonZeroIndex, IERC20(transferArgs.srcSwap.srcToken), IERC20(transferArgs.srcSwap.dstToken));
        if (returnAmount == 0) {
...
contract SwitchPolygon is SwitchRootPolygon {
...
abstract contract SwitchRootPolygon is ISwitchView {
