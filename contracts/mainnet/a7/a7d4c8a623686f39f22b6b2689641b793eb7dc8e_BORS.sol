...
contract BORS is Context, IMRC20, Ownable {
    ...
    string private _name = "BORS";
    string private _symbol = "BORS";
    ...
    function getEquivalentInMATIC(uint256 amountOfToken) public view returns (uint equivalentInMATIC) {
        (uint borsInPool, uint maticInPool) = getReservesInPool();
        return ((amountOfToken.mul(maticInPool)).div(borsInPool));
    }

    function getEquivalentInToken(uint256 amountOfMATIC) public view returns (uint equivalentInToken) {
        (uint borsInPool, uint maticInPool) = getReservesInPool();
        return ((amountOfMATIC.mul(borsInPool)).div(maticInPool));
    }

    function getReservesInPool() public view returns (uint borsInPool, uint maticInPool) {
     ...
     * Vorz Team cannot transfer more than 8000 MATIC / week
    ...
    function getMarketingAddress() public view returns (address marketingAddress) {
        ...
        // prevent dumps
        require(getEquivalentInMATIC(tokenAmount) < max_MATIC, "VorzShop cannot sell amounts higher than 8000 MATIC");
