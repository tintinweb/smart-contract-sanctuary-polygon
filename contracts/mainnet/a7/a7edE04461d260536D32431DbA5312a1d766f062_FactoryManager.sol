...
contract FactoryManager is Owned, TokensRecoverable {
    ...
    function setFactories(address _tokenFactory, address _mgeFactory, address _calculatorFactory, address _feeSplitterFactory, address _controllerFactory) public onlySignedAdmin(){
        tokenFactory = TokenFactory(_tokenFactory);
        mgeFactory = MGEFactory(_mgeFactory);
        calculatorFactory = CalculatorFactory(_calculatorFactory);
        feeSplitterFactory = FeeSplitterFactory(_feeSplitterFactory);
        controllerFactory = ControllerFactory(_controllerFactory);
    }
