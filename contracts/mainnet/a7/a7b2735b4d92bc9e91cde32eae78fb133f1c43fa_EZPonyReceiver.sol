...
contract EZPonyReceiver is IERC721Receiver, Ownable {
    ...
    modifier onlySeller() { 
        require(sellers[msg.sender], "Caller is not the seller");
        _; 
    }
    ...
    function onERC721Received(
        ...
        if (nftAddresses[msg.sender] && sellers[from]) {
            tokenIds.increment();
    ...
    function adminAddSeller(address _address) external onlyOwner {
        sellers[_address] = true;
    }
    function adminRemoveSeller(address _address) external onlyOwner {
        sellers[_address] = false;
    }
    function adminAddNFTAddress(address _address) external onlyOwner {
        nftAddresses[_address] = true;
    }
    function adminRemoveNFTAddress(address _address) external onlyOwner {
        nftAddresses[_address] = false;
    }
    function adminSetWethAddress(address _address) external onlyOwner {
