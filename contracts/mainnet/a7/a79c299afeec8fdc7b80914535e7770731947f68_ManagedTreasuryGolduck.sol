...
contract ManagedTreasuryGolduck is AccessControl,Pausable,ReentrancyGuard {
    ...
    uint256 public maximumTokenToSwap = 100000e18;
    uint256 public maximumEthToSwap = 2e18;
    ...
    function setMaxSwapTokensETHLimit(uint256 newMaximumTokenToSwap,uint256 newMaximumEthToSwap) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function payServiceFee(address to,uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        withdrawCoin(to,amount);
    }

    function withdrawFundsForExpenses(address to,uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        withdrawCoin(to,amount);
    }
    ...
    function buyBackGolduck(uint256 amountEth) external onlyCaller nonReentrant whenNotPaused returns (uint256 amountToken) {
        if(!(amountEth <= maximumEthToSwap)) revert InvalidAmount();

        return swapEthForTokens(amountEth,address(managedToken));
    }

    function addLiquidityToGolduck(uint256 ethAmount,uint256 tokenAmount) external onlyCaller nonReentrant whenNotPaused {
        addLiquidity(tokenAmount, ethAmount);
    }

    function sellTreasuryTokens(uint256 tokenAmount) external onlyCaller nonReentrant whenNotPaused {
        swapTokensForEth(managedToken,tokenAmount);
    }

    /**
     * @dev from token -> eth/bnb/matic/metis.
     * @notice Ensure you have enough of other project tokens before running this function.
     */
    function sellOtherProjectToken(ERC20 otherToken,uint256 tokenAmount) external onlyCaller nonReentrant whenNotPaused {
        swapTokensForEth(otherToken,tokenAmount);
    }

    /**
     * @dev from eth/bnb/matic/metis -> otherProject token.       
     * @notice you have enough balance of ETH/BNB etc., before running this function
     */
    function buyOtherProjectToken(uint256 ethAmount,address toAddress) external onlyCaller nonReentrant whenNotPaused {
        swapEthForTokens(ethAmount,toAddress);
    }

    function buyBackAndBurnGolduck(uint256 amountEth) external onlyCaller nonReentrant whenNotPaused returns (bool) {
        if(!(amountEth <= maximumEthToSwap)) revert InvalidAmount();
