...
contract getRandoms {
    ...
    function changeTryLimit(uint256 amount, uint256 _starter) private {
        uint256 tmpnum = Try_limited + amount;
    ...
    function _getRand() private returns (uint256 _rand_id) {
        ...
        uint256 randomId = _tryGenerateRandomId();
        total_tryed++;
    ...
    function changeTryLimit2(uint256 amount, uint256 _starter) private {
        uint256 tmpnum = Try_limited2 + amount;
    ...
    function _getRand2() private returns (uint256 _rand_id) {
        ...
        uint256 randomId = _tryGenerateRandomId2();
        total_tryed2++;
    ...
    function changeTryLimit3(uint256 amount, uint256 _starter) private {
        uint256 tmpnum = Try_limited3 + amount;
    ...
    function _getRand3() private returns (uint256 _rand_id) {
        ...
        uint256 randomId = _tryGenerateRandomId3();
        total_tryed3++;
    ...
    function changeTryLimit4(uint256 amount, uint256 _starter) private {
        uint256 tmpnum = Try_limited4 + amount;
    ...
    function _getRand4() private returns (uint256 _rand_id) {
        ...
        uint256 randomId = _tryGenerateRandomId4();
        total_tryed4++;
    ...
    function changeTryLimit5(uint256 amount, uint256 _starter) private {
        uint256 tmpnum = Try_limited5 + amount;
    ...
    function _getRand5() private returns (uint256 _rand_id) {
        ...
        uint256 randomId = _tryGenerateRandomId5();
        total_tryed5++;
...
contract DoubleOrNothing is ERC721TokenReceiver, Ownable {
    ...
    function ApeNoLongerForSale(uint256 apeIndex) public reentrancyGuard {
    ...
    function tryChance5() public payable {
        ...
        require(balance5 > value, "try another time balnace 5 matic");
    ...
    function tryChance100() public payable {
        ...
        require(balance100 > value, "try another time balnace 100");
    ...
    function find(address _wallet, uint256 value)
        private
        view
        returns (uint256)
    {
    ...
    function _sendValue(address _to, uint256 _value) private {
    ...
    function withdrawUserCommission() public payable {
        ...
        require(userTokens[msg.sender].commission > 0, "commmision 0 !");

        uint256 commission = userTokens[msg.sender].commission  ;
    ...
    function calc(uint256 _paymenID) public onlyDeployer {
        Payment memory p = payments[_paymenID];

        if (p.balance == 5) {
            p.status = 'paying';
            payments[_paymenID] = p;
            calc5(p);
        }
    ...
    function calc5(Payment memory _payment) private onlyDeployer {
        uint256 random = randomfn.random1();
        if (random > 0) {
            Double++;
            _payment.status = 'paid';
            payments[_payment.paymenID] = _payment;

            successfulAttempt += _payment.feeValue;

            UserToken storage user = userTokens[_payment.wallet_address];

            user.userDoubleMATIC += _payment.feeValue;

            _sendValue(_payment.wallet_address, _payment.feeValue);
            balance5 -= _payment.feeValue;
        }

        if (random == 0) {
            _payment.status = 'reject';
            payments[_payment.paymenID] = _payment;

            UserToken storage user = userTokens[_payment.wallet_address];

            user.userNothingMATIC += _payment.value;

            Nothing++;
        }

        History storage txHistory = txHistorys[_payment.wallet_address];

        totalTransaction storage totaltx = listTX[totaltried];
        totaltx.userAmount = _payment.value;

        totaltx.userAdress = _payment.wallet_address;
        totaltx.userTxSuccess = random;

        txHistory.deposits.push(
            PlayerDeposit({
                amount: _payment.value,
                totalWithdraw: 0,
                wasSuccess: random,
        ...
        txHistory.owner = _payment.wallet_address;
        txHistory.price = _payment.value;
    }

    function calc10(Payment memory _payment) private onlyDeployer {
    ...
    function calc25(Payment memory _payment) private onlyDeployer {
    ...
    function calc50(Payment memory _payment) private onlyDeployer {
    ...
    function calc100(Payment memory _payment) private onlyDeployer {
    ...
    modifier onlyDeployer() {
        require(deployer == msg.sender, "just deployer");
        _;
    }
