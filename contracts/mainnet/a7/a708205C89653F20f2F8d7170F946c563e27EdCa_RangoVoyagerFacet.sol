...
contract RangoVoyagerFacet is IRango, ReentrancyGuard, IRangoVoyager {
    ...
    address private nativeWrappedAddress;
    ...
    constructor(address _weth, address _address) {
    ...
    function doVoyagerBridge(
        ...
        if (fromToken != LibSwapper.ETH) {
            LibSwapper.approve(fromToken, request.reserveContract, amount);
            SafeERC20.safeTransferFrom(IERC20(fromToken), msg.sender, address(this), amount);
        }
        LibSwapper.approve(request.feeTokenAddress, request.reserveContract, msg.value);

        IVoyager voyager = IVoyager(routerAddress);

        callVoyager(voyager, request, amount);
    ...
    function voyagerBridge(
        ...
        require(bridgeRequest.feeTokenAddress == baseSwapperStorage.WETH,
            "Fee is only acceptable in native token"
        );

        (uint out, uint value) = LibSwapper.onChainSwapsPreBridge(request, calls, bridgeRequest.feeAmount);

        doVoyagerBridge(bridgeRequest, request.toToken, out);
    }
