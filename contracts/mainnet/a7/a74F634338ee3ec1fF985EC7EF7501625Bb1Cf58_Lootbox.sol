...
contract Lootbox is VRFConsumerBaseV2, Governable {
    ...
    function open(uint containerId) external payable {
        require(
            rollCount[msg.sender][containerId] == 0,
            "Already opening container"
        );
    ...
    function roll(uint containerId) external payable {
        ...
        require(
            rollCount[msg.sender][containerId] > 0,
            "This container is not opening now"
        );
    ...
    function _roll(uint containerId) internal {
        ...
        require(
            viableContainers.contains(containerId),
            "This container are not viable"
        );
        ...
        uint32[] memory counts = getAvailableItemsNftCount(
            containersInfo[containerId].itemIds
        );
        if (
     ...
     */
    function setViableContainers(
        uint[] calldata newViableContainers
    ...
    function setContainer(
        ...
        require(
            itemIds.length == weights.length,
            "items and weights are not same length"
        );
    ...
    function getRollPrice(uint rollIndex) public view returns (uint) {
        return
            rollIndex >= rollPrices.length
                ? rollPrices[rollPrices.length - 1]
                : rollPrices[rollIndex];
    }

    function getNextRollPrice(
        address user,
        uint containerId
    ...
    function getItemNftIds(
        uint itemId
    ) external view returns (uint16[] memory) {
        return nftIds[itemId];
    }

    function getItemsNftCount(
    ...
    function getAvailableItemsNftCount(
        uint32[] memory itemIds
    ) public view returns (uint32[] memory result) {
        result = new uint32[](itemIds.length);
        uint len = itemIds.length;

        for (uint i = 0; i < len; ) {
            result[i] = uint32(
                nftIds[itemIds[i]].length - pendingNftCount[itemIds[i]]
            );
            unchecked {
                i++;
            }
    ...
    function getContainerInfo(
        uint id
    ) external view returns (uint32[] memory ids, uint16[] memory weights) {
    ...
    function fulfillRandomWords(
        ...
        for (; selectedSlot < containerInfo.weights.length; selectedSlot++) {
            ...
            accumulatedWeight += (nftIds[itemId].length -
                pendingNftCount[itemId] >
                0)
                ? containerInfo.weights[selectedSlot]
                : 0;
