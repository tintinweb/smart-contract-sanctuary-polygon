...
contract xIndexMinter is Operator, ReentrancyGuard {
    ...
    function getNativePrice(uint256 _cid) public view returns (uint256 _nativePrice) {
        ...
        if (_native == usdc) {
            ...
            _nativePrice = amounts[amounts.length - 1].mul(10 ** collateral.nativeMissingDecimals);
        }
    }
    ...
    function getLpPrice(uint256 _cid) public view returns (uint256 _lpPrice) {
        ...
        _lpPrice = nativeReserve.mul(2).mul(10 ** collateral.nativeMissingDecimals).mul(10 ** 18).div(lpSupply);
    }
    ...
    function calcMint(uint256 _cid, uint256 _amount, bool _useNative) public view returns (uint256 _xIndex_amount) {
        ...
        uint256 input_amount_value = _amount.mul(collateral_price).div(PRICE_PRECISION);
        _xIndex_amount = input_amount_value.mul(PRICE_PRECISION).div(xINDEX_PRICE);
    }
