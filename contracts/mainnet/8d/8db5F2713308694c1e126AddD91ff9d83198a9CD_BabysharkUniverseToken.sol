...
contract BabysharkUniverseToken is ERC20, Pausable, AccessControlEnumerable {
    ...
    constructor() ERC20("Baby Shark Universe Token", "BSU") {
        ...
        // roleAdmin 설정이 없을 경우, grantRole이 제대로 동작하지 않음
        _setRoleAdmin(OWNER_ROLE, OWNER_ROLE);        

        _mint(msg.sender, 1500000000 * 10**uint(decimals()));
    }

    // 토큰 생성후, 언락 대상 계정에 토큰을 배분하는 코드
    ...
    // pause에 따라 전송하기전에 해당 유무를 판단하기 위한 코드
    ...
    //---------------------------------------------------------
    // owner 관리 관련
    // owner_role이 숨겨져 있기 때문에, 외부에서 호출해서 renounceRole, grantRole, revokeRole을 호출하기는 어렵다.
    // 이를 통해서, 노출은 되었지만 사용하기 힘들게 만들고, 이와 관련된 로직은 addOwner와 removeOwner 2개의 함수만을 통해서 접근하는 것으로 한다.
