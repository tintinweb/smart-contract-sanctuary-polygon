...
library ChunkStorage {
    ...
    function write(
        ...
        assembly {
            ...
            let dataoffsetp := add(_data, 0x20)
            ...
            for { let data_c := 0 } lt(data_c, data_size) { } {
                ...
                mstore(bytecode_pointer, mload(add(dataoffsetp, data_c)))
...
// File contracts/BAStorageT.sol


/*
 * Data Storage for Bansho
 * @nuko973663
 */

pragma solidity ^0.8.4;
contract BAStorageT is ERC721, Ownable, AccessControlEnumerable {
    using Strings for uint256;
    
    using ChunkStorage for ChunkStorage.Storage;

    ChunkStorage.Storage private _store;

    string private _svg;
    string private _description;
    mapping(uint256=>string) private _tokenNames;
    mapping(uint256=>bytes[40]) private _dataSet;
    mapping(uint256=>uint256) private _dataSetLength;
    ...
    function tokenURI(uint256 tokenId_) public view override returns (string memory) {
        ...
        uint256 id = tokenId_ >> 160;
        uint256 addr = uint160(tokenId_);
    ...
    function tokenId(address address_, uint256 tokenId_) public pure returns (uint256) {
        return uint256(uint160(address_) + (tokenId_ << 160));
    }

    function tokenData(address address_, uint256 tokenId_) public view returns (bytes memory) {
        uint256 id =uint256(uint160(address_) + (tokenId_ << 160));
        require(_exists(id), "ERC721Metadata: URI query for nonexistent token");
        bytes32 key0;
        bytes32 key1;
        bytes32 key2;
        bytes memory data;
        for(uint256 i = 0; i <= _dataSetLength[id]/3; i++){
            key0 = keccak256(abi.encodePacked(id, i*3));
            key1 = keccak256(abi.encodePacked(id, i*3 + 1));
            key2 = keccak256(abi.encodePacked(id, i*3 + 2));
            data = abi.encodePacked(data, _store.read(key0), _store.read(key1), _store.read(key2));
        }

        return data;
    }

    function setData(address address_, uint256 tokenId_, uint256 dataId_, bytes memory data_) public {
        require(hasRole(MINTER_ROLE, _msgSender()), "must have minter role");
        require( (dataId_ >= 0)&&(dataId_ < 40), "dataId must be between 0 and 9" );
        uint256 id = uint256(uint160(address_) + (tokenId_ << 160));
        
        bytes32 key = keccak256(abi.encodePacked(id, dataId_));
        _store.write(key, data_);
        _dataSetLength[id] = dataId_ + 1;

        if(dataId_==0)_safeMint(_msgSender(), id);
    }
