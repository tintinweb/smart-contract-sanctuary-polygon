...
import "../../contracts/libraries/OSBLimitedToken.sol";

contract $OSBLimitedToken is OSBLimitedToken {
...
contract OSBLimitedToken is ERC20Upgradeable, OwnableUpgradeable {
    uint8 private _decimals;

    /**
     * @dev The maximum total supply of tokens that can be stored by the contract,
     * effectively limiting the amount of tokens that can be created.
     */
    ...
    error ExceededMaximumTotalSupply();
    ...
    /// @dev Emit an event when the mint is successfully.
    event Mint(address indexed to, uint256 indexed amount);

    /// @dev Emit an event when burn successfully.
     ...
     * @param _maxTotalSupply The maximum total supply of tokens that can be stored by the contract.
    ...
    function initialize(
        ...
        if (_maxTotalSupply == 0) revert InvalidMaxTotalSupply();
        ...
        if (_issueAmount > 0) _mint(_owner, _issueAmount);
        emit ContractDeployed(_owner, _name, _symbol, decimals_, _issueAmount, _maxTotalSupply);
    }

    /**
     * @dev Mint a specific amount of tokens and assign them to a specified address.
    ...
    function mint(address _to, uint256 _amount) external onlyOwner {
        ...
        if (totalSupply() + _amount > maxTotalSupply) revert ExceededMaximumTotalSupply();
        _mint(_to, _amount);
        emit Mint(_to, _amount);
    }

    /**
     * @dev Burn a specific amount of tokens from the caller's balance.
