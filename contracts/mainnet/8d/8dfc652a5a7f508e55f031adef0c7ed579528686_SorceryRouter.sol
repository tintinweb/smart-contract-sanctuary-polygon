...
/**
 *Submitted for verification at snowtrace.io on 2023-05-09
*/
...
contract SorceryRouter is IUniswapV2Router02, Ownable {
    using SafeMathUniswap for uint;
    ...
    constructor(address _factory, address _WETH, address treasuryAddress) public {
        factory = _factory;
        WETH = _WETH;
    ...
    function swapTokensForExactTokens(
        ...
        require((amounts[0] + feeAmount) <= amountInMax, 'UniswapV2Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]
        );
        TransferHelper.safeTransferFrom(
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = UniswapV2Library.getAmountsOut(factory, (msg.value - feeAmount), path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
        ...
        returns (uint[] memory amounts)
    {
        ...
        require((amounts[0] + feeAmount) <= msg.value, 'UniswapV2Router: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0] + feeAmount}();
        assert(IWETH(WETH).transfer(UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]));
        assert(IWETH(WETH).transfer(treasury, feeAmount));
        _swap(amounts, path, to);
        // refund dust eth, if any
        if (msg.value > (amounts[0]+feeAmount)) TransferHelper.safeTransferETH(msg.sender, msg.value - (amounts[0]+feeAmount));
    }
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(UniswapV2Library.pairFor(factory, path[0], path[1]), (amountIn - feeAmount)));
        assert(IWETH(WETH).transfer(treasury, feeAmount));
        uint balanceBefore = IERC20Uniswap(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
