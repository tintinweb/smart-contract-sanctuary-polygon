...
import "./../../connectors/kyber/v3/libraries/LiquidityMath.sol";
...
import "./../../connectors/kyber/IKyberSwapElasticLM.sol";
import "./../../connectors/kyber/v3/interfaces/INonfungiblePositionManager.sol";
import "./../../connectors/kyber/v3/interfaces/IUniswapV3Pool.sol";
import "./../../connectors/kyber/v3/libraries/QtyDeltaMath.sol";
import "./../../connectors/kyber/v3/libraries/OracleLibrary.sol";
import "./../../connectors/kyber/v3/libraries/TickMath.sol";

import "hardhat/console.sol";

contract KyberStrategy is InitializableAbstractStrategy, UniswapV2Exchange, IERC721Receiver {
    ...
    using OvnMath for int256;

    IERC20 public token0;
    IERC20 public token1;
    IERC20 public vaultTokenA;
    IERC20 public vaultTokenB;
    IERC20 public KNC;
    ...
    uint256 public elasticMinerPoolId;
    IKyberSwapElasticLM public elasticMiner;
    ...
    function initialize(
        ...
        uint256 _elasticMinerPoolId,
        address _elasticMiner,
        ...
        vaultTokenA = IERC20(vault.token0());
        vaultTokenB = IERC20(vault.token1());

        KNC = IERC20(_rewardTokenAddresses[0]);

        swapper = ISwapper(_essentials[0]);
        oracleRouter = IOracle(_essentials[1]);

        uniswapV3Pool = IUniswapV3Pool(_pToken);
        uniswapPositionManager = INonfungiblePositionManager(_nftPositionManager);

        elasticMinerPoolId = _elasticMinerPoolId;
        elasticMiner = IKyberSwapElasticLM(_elasticMiner);

        decimals.push(Helpers.getDecimals(_assets[0]));
        decimals.push(Helpers.getDecimals(_assets[1]));

        vDecimals.push(Helpers.getDecimals(address(vaultTokenA)));
        vDecimals.push(Helpers.getDecimals(address(vaultTokenB)));
    ...
    /**
    @dev Function to return the current reserve ratio in form 100:X
    @return (uint256,uint256) - ratios of reserves
     */
    function getRatios() public view override returns (uint256, uint256) {
        if (tokenId == 0) {
            (uint160 sqrtPriceX96, int24 currentTick, , ) = uniswapV3Pool.getPoolState();
            uint256 token1Equivalent = OracleLibrary.getQuoteAtTick(currentTick, sqrtPriceX96, 10 ** 20, address(token0), address(token1));
            return (10 ** 20, token1Equivalent.scaleBy(18, decimals[1]));
        }
        (int256 reserve0, int256 reserve1) = _locked();
        return (10 ** 20, uint(reserve1).scaleBy(18, decimals[1]).mul(10 ** 20).div(uint(reserve0).scaleBy(18, decimals[0])));
    }

    function _alignFundsWithRatios(uint256 _amount0, uint256 _amount1) internal view returns (uint256, uint256) {
        uint256 _newAmount0 = _amount0;
        uint256 _newAmount1 = _amount1;
        (uint256 _ratio0, uint256 _ratio1) = getRatios();
        if (_amount0.scaleBy(18, decimals[0]) <= _amount1.scaleBy(18, decimals[1])) {
            _newAmount1 = (_amount0.scaleBy(decimals[1], decimals[0]) * _ratio1) / _ratio0;
            if (_newAmount1 > _amount1) {
                _newAmount1 = _amount1;
                _newAmount0 = (_newAmount1.scaleBy(decimals[0], decimals[1]) * _ratio0) / _ratio1;
            }
        } else if (_amount0.scaleBy(18, decimals[0]) > _amount1.scaleBy(18, decimals[1])) {
            _newAmount0 = (_amount1.scaleBy(decimals[0], decimals[1]) * _ratio0) / _ratio1;
            if (_newAmount0 > _amount0) {
                _newAmount0 = _amount0;
                _newAmount1 = (_newAmount0.scaleBy(decimals[1], decimals[0]) * _ratio1) / _ratio0;
            }
        }
        return (_newAmount0, _newAmount1);
    }

    function _deposit(address[] memory _assets, uint256[] memory _amounts, bool _useUnderlying) internal {
        (uint256 _amount0, uint256 _amount1) = _alignFundsWithRatios(_amounts[0], _amounts[1]);
        uint128 _liquidity;
        if (tokenId == 0) {
            // create NFT in UniswapV3
            _liquidity = _mint(_amount0, _amount1);
            uniswapPositionManager.approve(address(elasticMiner), tokenId);
            try elasticMiner.deposit(_getId()) {} catch Error(string memory reason) {
                console.log("D_FAIL", reason);
            }
        } else {
            _liquidity = _addLiquidity(_amount0, _amount1);
        }
        console.log("L_ADDED");
        if (_liquidity > 0) {
            _stake(_liquidity);
    ...
    function _unstake() internal {
        ...
        (, _liquidities[0]) = elasticMiner.positions(tokenId);
        if (_liquidities[0] > 0) {
            elasticMiner.exit(elasticMinerPoolId, _getId(), _liquidities);
            elasticMiner.withdraw(_getId());
        }
    }

    function _stake(uint128 _liquidity) internal {
        uint256[] memory _liquidities = new uint256[](1);
        _liquidities[0] = _liquidity;
        try elasticMiner.join(elasticMinerPoolId, _getId(), _liquidities) {} catch Error(string memory reason) {
            console.log("J_FAIL", reason);
        }
    }

    function _mint(uint256 _amount0, uint256 _amount1) internal returns (uint128) {
        (, , int24 nearestCurrentTick, ) = uniswapV3Pool.getPoolState();
        int256 diff = (((int256(nearestCurrentTick) * 250) / 10000) / uniswapV3Pool.tickDistance()) * uniswapV3Pool.tickDistance();
        diff = (diff < 0) ? diff * -1 : diff;
        int256 range0 = nearestCurrentTick - diff;
        int256 range1 = nearestCurrentTick + diff;
        (int24 _previousTick, int24 _nextTick) = uniswapV3Pool.initializedTicks(nearestCurrentTick);
        INonfungiblePositionManager.MintParams memory params = INonfungiblePositionManager.MintParams(
            address(token0),
            address(token1),
            uniswapV3Pool.swapFeeUnits(),
            int24(range0),
            int24(range1),
            [_previousTick, _nextTick],
            _amount0,
            _amount1,
            OvnMath.subBasisPoints(_amount0, 500), // slippage 5%
            OvnMath.subBasisPoints(_amount1, 500), // slippage 5%
            address(this),
            block.timestamp + 600
        );
        token0.approve(address(uniswapPositionManager), _amount0);
        token1.approve(address(uniswapPositionManager), _amount1);
        (uint256 _tokenId, uint128 _liquidity, , ) = uniswapPositionManager.mint(params);
        uniswapPositionManager.positions(tokenId);
        tokenId = _tokenId;
        return _liquidity;
    }

    function _addLiquidity(uint256 _amount0, uint256 _amount1) internal returns (uint128) {
        ...
        token0.approve(address(uniswapPositionManager), _amount0);
        token1.approve(address(uniswapPositionManager), _amount1);
        (uint128 liquidity, , , ) = uniswapPositionManager.addLiquidity(params);
        return (liquidity);
    }

    /*
    @dev Function to deposit the assets to the sPools
    @param _assets Assets which are to be supported by the strategy. Ex: MATIC-USDC
    @param _amounts Corresponding amounts of each asset
     */
    function deposit(address[] calldata _assets, uint256[] calldata _amounts) external override onlyVault nonReentrant {
        _deposit(_assets, _amounts, false);
    }

    /**
    @dev Function to withdraw the assets from the sPools
    @param _assets Assets which are to be supported by the strategy. Ex: MATIC-USDC
    @param _amounts Corresponding amounts of each asset
     */
    function withdraw(address _beneficiary, address[] calldata _assets, uint256[] calldata _amounts) external override onlyVaultOrGovernor nonReentrant {
        _withdraw(_beneficiary, _assets, _amounts);
    }

    function _alignFundsWithRatiosForWithdrawl(uint256 _amount0, uint256 _amount1) internal view returns (uint256, uint256) {
        uint256 _newAmount0 = _amount0;
        uint256 _newAmount1 = _amount1;
        (uint256 _ratio0, uint256 _ratio1) = getRatios();
        if (_amount0.scaleBy(18, decimals[0]) <= _amount1.scaleBy(18, decimals[1])) {
    ...
    function _sendBack(address _ben, uint256 _amount0, uint256 _amount1) internal {
        token0.safeTransfer(_ben, _amount0);
        token1.safeTransfer(_ben, _amount1);
    }

    /**
    @dev Internal - Function to withdraw the assets from the sPools
    @param _assets Assets which are to be supported by the strategy. Ex: MATIC-USDC
    @param _amounts Corresponding amounts of each asset
     */
    function _withdraw(address _beneficiary, address[] memory _assets, uint256[] memory _amounts) internal {
        require(supportsAsset(_assets), "TNS");
        uint256[2] memory withdrawable = [_amounts[0].subOrZero(token0.balanceOf(address(this))), _amounts[1].subOrZero(token1.balanceOf(address(this)))];
        if (withdrawable[0] == 0 && withdrawable[1] == 0) {
            _sendBack(_beneficiary, _amounts[0], _amounts[1]);
            return;
        }
        _unstake();
        (uint256 _amount0, uint256 _amount1) = _alignFundsWithRatiosForWithdrawl(withdrawable[0], withdrawable[1]);
        console.log("W:", _amount0, _amount1);
        (INonfungiblePositionManager.Position memory position, ) = uniswapPositionManager.positions(tokenId);
        (, int24 currentTick, , ) = uniswapV3Pool.getPoolState();
        uint128 liquidity = LiquidityMath.getLiquidityFromQties(
            TickMath.getSqrtRatioAtTick(currentTick),
            TickMath.getSqrtRatioAtTick(position.tickLower),
            TickMath.getSqrtRatioAtTick(position.tickUpper),
            _amount0.addBasisPoints(1000), // slippage 10%
            _amount1.addBasisPoints(1000) // slippage 10%
        );
        // Adding 3%
        liquidity = (liquidity * (10 ** 4 + 300)) / 10 ** 4;
        ...
        INonfungiblePositionManager.RemoveLiquidityParams memory params = INonfungiblePositionManager.RemoveLiquidityParams(tokenId, liquidity, _amount0, _amount1, block.timestamp + 600);
        (uint256 returnedAmount0, uint256 returnedAmount1, ) = uniswapPositionManager.removeLiquidity(params);
        INonfungiblePositionManager.BurnRTokenParams memory burnParams = INonfungiblePositionManager.BurnRTokenParams(tokenId, 0, 0, block.timestamp + 600);
        // Try catch
        try uniswapPositionManager.burnRTokens(burnParams) {} catch Error(string memory reason) {
            console.log("BRN FAIL", reason);
        }
        uniswapPositionManager.transferAllTokens(address(token0), returnedAmount0, address(this));
        uniswapPositionManager.transferAllTokens(address(token1), returnedAmount1, address(this));
        _sendBack(_beneficiary, _amounts[0], _amounts[1]);
    }

    /**
    @dev Internal - Function to withdraw all the assets from the sPools to the Vault
     */
    function _withdrawAll() internal {
        (uint256 _bal0, uint256 _bal1) = _checkBalance();
        (INonfungiblePositionManager.Position memory position, ) = uniswapPositionManager.positions(tokenId);
        if (position.liquidity == 0) {
            _sendBack(vaultAddress, _bal0, _bal1);
        }
        _unstake();

        INonfungiblePositionManager.RemoveLiquidityParams memory params = INonfungiblePositionManager.RemoveLiquidityParams(
            tokenId,
            position.liquidity,
            _bal0.subBasisPoints(300), // 3% Slippage
            _bal1.subBasisPoints(300), // 3% Slippage
            block.timestamp + 600
        );
        (uint256 returnedAmount0, uint256 returnedAmount1, ) = uniswapPositionManager.removeLiquidity(params);
        INonfungiblePositionManager.BurnRTokenParams memory burnParams = INonfungiblePositionManager.BurnRTokenParams(tokenId, 0, 0, block.timestamp + 600);
    ...
    /**
    @dev Function to withdraw all the assets from the sPools to the Vault
     */
    function withdrawAll() external override onlyVaultOrGovernor nonReentrant {
        _withdrawAll();
        // Reusing the previous bal
        (uint256 _bal0, uint256 _bal1) = _available();
        _sendBack(vaultAddress, _bal0, _bal1);
        // emit WithdrawAll(tokenId, _bal0, _bal1, position.liquidity);
    }

    function _locked() public view returns (int256 _tAmountLiq0, int256 _tAmountLiq1) {
        (uint160 sqrtPriceX96,  int24 currentTick ,  , ) = uniswapV3Pool.getPoolState();
        (INonfungiblePositionManager.Position memory position, ) = uniswapPositionManager.positions(tokenId);
        if (currentTick < position.tickLower) {
            ...
            return (
                QtyDeltaMath.calcRequiredQty0(TickMath.getSqrtRatioAtTick(position.tickLower), TickMath.getSqrtRatioAtTick(position.tickUpper), position.liquidity, false),
                0
            );
        }
        if (currentTick >= position.tickUpper) {
            ...
            return (
                0,
                QtyDeltaMath.calcRequiredQty1(TickMath.getSqrtRatioAtTick(position.tickLower), TickMath.getSqrtRatioAtTick(position.tickUpper), position.liquidity, false)
            );
        }
        _tAmountLiq0 = QtyDeltaMath.calcRequiredQty0(sqrtPriceX96, TickMath.getSqrtRatioAtTick(position.tickUpper), position.liquidity, false);
        _tAmountLiq1 = QtyDeltaMath.calcRequiredQty1(sqrtPriceX96, TickMath.getSqrtRatioAtTick(position.tickLower), position.liquidity, false);
    }

    /**
    @dev Internal - Function to check the current balance of assets in this contract
                    and staked in the sPool.
    @return (uint256,uint256) - (Balance of token0, Balance of token1)
     */
    function _checkBalance() internal view returns (uint256, uint256) {
        (uint256 _bal0, uint256 _bal1) = _available();
        if (tokenId == 0) {
            return (_bal0, _bal1);
        }
        (int256 _tAmountLiq0, int256 _tAmountLiq1) = _locked();
        return (_bal0 + uint(_tAmountLiq0), _bal1 + uint(_tAmountLiq1));
    }
    ...
    @return (uint256,uint256) - (Balance of token0, Balance of token1)
     */
    function checkBalance() external view override returns (uint256, uint256) {
        return _checkBalance();
    }

    /**
    @dev Internal - Function to collect rewards from the sPool and send it to Harvestor when invoked.
    */
    function _collectRewards() internal returns (uint256, uint256) {
        uint256[] memory _pIds = new uint256[](1);
        _pIds[0] = elasticMinerPoolId;
        bytes[] memory _datas = new bytes[](1);
        _datas[0] = abi.encode(IKyberSwapElasticLM.HarvestData({ pIds: _pIds }));

        try elasticMiner.harvestMultiplePools(_getId(), _datas) {} catch Error(string memory reason) {
            console.log("H_FAIL", reason);
        }
        uint256 rewardBalance = KNC.balanceOf(address(this));
        if (rewardBalance <= minThresholds[0]) {
            return (0, 0);
        }

        uint256 swappedT1amount = 0;
        KNC.safeApprove(address(swapper), rewardBalance);
        uint256 rewardBalanceInVaultTokenA = swapper.swapCommon(address(KNC), address(vaultTokenA), rewardBalance.subBasisPoints(100));
        (, uint256 _swapPart1) = _rewardSwapDistribution(rewardBalanceInVaultTokenA);
        if (_swapPart1 > minThresholds[0]) {
            vaultTokenA.safeApprove(address(swapper), _swapPart1);
            swappedT1amount = swapper.swapCommon(address(vaultTokenA), address(vaultTokenB), _swapPart1);
        }
        if (vaultTokenA.balanceOf(address(this)) > minThresholds[1] && vaultTokenB.balanceOf(address(this)) > minThresholds[2]) {
            _deposit();
        } else {
            // console.log("NETTD"); // Not enough tokens to deposit
        }
        return (0, swappedT1amount);
    }

    /**
    @dev Internal function to the calculate the ratio in which the reward needs to be swapped
    @return (uint256,uint256) - (Part for token0, Part for token1)
     */
    function _rewardSwapDistribution(uint256 _reward) internal view returns (uint256, uint256) {
        (uint256 _leftover0, uint256 _leftover1) = _available();
        _leftover0 = _leftover0.scaleBy(18, decimals[0]);
        _leftover1 = _leftover1.scaleBy(18, decimals[1]);
        uint256 _LDH = uint256(Helpers.abs(int256(_leftover0) - int256(_leftover1))) / 2;
        uint256 _part0 = 0;
        uint256 _part1 = 0;
        if (_LDH < _reward / 2) {
            _part0 = _reward / 2 - _LDH;
            _part1 = _reward / 2 + _LDH;
            return (_part0, _part1);
        }
        if (_leftover0 < _leftover1) {
            _part0 = _reward;
        } else {
            _part1 = _reward;
        }
    ...
    /**
    @dev Function to collect rewards from the sPool and send it to Harvestor when invoked.
    */
    function collectRewardTokens() external override onlyVault nonReentrant returns (uint256, uint256) {
        return _collectRewards();
    }

    /**
    @dev Function to set the thresholds for various conditions used in the Strategy
    @param _minThresholds array containing the thresholds vaules
    */
    function setThresholds(uint256[] calldata _minThresholds) external onlyVaultOrGovernor nonReentrant {
        require(_minThresholds.length == 3);
        minThresholds = _minThresholds;
    }

    /**
    @dev Function to check whether an asset it is supported or not by the Strategy
    @param _assets Assets to check whether they are supported or not.
    @return bool whether the asset is supported or not
    */
    function supportsAsset(address[] memory _assets) internal view returns (bool) {
        require(_assets.length == 2, "NS");
        return _assets[0] == address(vaultTokenA) && _assets[1] == address(vaultTokenB);
    }

    /**
    @dev Function to get the equivalent LP tokens for the given assets
    @return uint256 - LP tokens
     */
    ...
    /**
    @dev Return the health factor of the Strategy
    @return uint256 Average health factor of the Strategy
    */
    function health() external view returns (uint256, uint256) {
        (uint256 balance0, uint256 balance1) = _checkBalance();
        if (balance0 == 0 && balance1 == 0) {
            return (0, 0);
        }
        return (100 - ((token0.balanceOf(address(this)) * 100) / balance0), 100 - ((token1.balanceOf(address(this)) * 100) / balance1));
    }

    /**
     * @dev Deposit whatever present in the contract to the sPool
     */
    function _deposit() internal {
        uint256[] memory _amounts = new uint256[](2);
        (_amounts[0], _amounts[1]) = _available();
        address[] memory _assets = new address[](2);
        _assets[0] = address(vaultTokenA);
        _assets[1] = address(vaultTokenB);
        _deposit(_assets, _amounts, false);
    }

    function _getId() internal returns (uint256[] memory) {
        uint256[] memory _ids = new uint256[](1);
        _ids[0] = tokenId;
        return _ids;
    }

    function _available() internal view returns (uint256, uint256) {
        uint256[] memory _amounts = new uint256[](2);
        return (vaultTokenA.balanceOf(address(this)), vaultTokenB.balanceOf(address(this)));
    }
...
abstract contract InitializableAbstractStrategy is Initializable, Governable {
    ...
    */
    function collectRewardTokens() external virtual returns (uint256, uint256);    

    /**
     * @dev Deposit an amount of asset into the platform
     * @param _assets               Addresses for the asset
     * @param _amounts              Units of asset to deposit
     */
    function deposit(address[] calldata _assets, uint256[] calldata _amounts) external virtual;
     ...
     * @param _assets             Address of the assets
    ...
    /**
     * @dev Get the Ratio of Assets in the third party sPool
     * @return uint256,uint256     - Ratio of the sPools
     */
    function getRatios() external view virtual returns (uint256,uint256);
...
library LiquidityMath {
    ...
    function getLiquidityFromQty0(uint160 lowerSqrtP, uint160 upperSqrtP, uint256 qty0) internal pure returns (uint128) {
    ...
    function getLiquidityFromQty1(uint160 lowerSqrtP, uint160 upperSqrtP, uint256 qty1) internal pure returns (uint128) {
        unchecked {
    ...
    function getLiquidityFromQties(uint160 currentSqrtP, uint160 lowerSqrtP, uint160 upperSqrtP, uint256 qty0, uint256 qty1) internal pure returns (uint128) {
...
interface IVault {
    ...
    event AssetCombinationAdded(address[] _assets);
    ...
    function setOracleRouter(address _oracleRouter) external;
    ...
    function supportAsset(address _asset, uint8 _kind) external;
    ...
    function claimReward(address asset) external view returns (uint256, uint256);
    function claimRewards() external view returns (uint256, uint256);
    ...
    function avgStrategyRatio() external view returns (uint256, uint256);
    ...
    function isBluechipAsset(address _asset) external view returns (bool);
    function isUSDAsset(address _asset) external view returns (bool);
    ...
    function addMintableCombination(address[] calldata _assets) external;
    function isMintableCombination(bytes32 _hash) external view returns (bool);
    function getMintablesCount() external view returns (uint256);
...
interface IKyberSwapElasticLM is IKyberSwapElasticLMEvents {
  ...
  function positions(uint256 nftId) external view returns (address,uint256);
...
interface INonfungiblePositionManager is  IERC721 {
    ...
    view
    returns (
       Position memory, PoolInfo memory
    );
    ...
    function unwrapWeth(uint256 _amount,address _a) external payable;
    function transferAllTokens(address _token1, uint256 _amount1, address recipient) external payable;
}
    ...
    IERC721
{
    ...
    function initializedTicks (int24 nearestCurrentTick) external view returns (int24, int24);
