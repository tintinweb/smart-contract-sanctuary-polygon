...
contract TDTNft1 is ERC721URIStorage, Whitelist {
  string public s_token_uri =
        "ipfs://QmYZw9p9FzFXNzUFZ2oCTtxrM7buk3trHx4qeXQVSiU1Kq";
  ...
  /* Set the URI of the NFT to a new one.
     @param new value of the URI
     @dev only the owner can set the uri
     */
  function setTokenURI(
    string memory newTokenUri /* tokenId */
  ) external onlyOwner {
    s_token_uri = newTokenUri;
  }

  /* Return the URI of the NFT.
     @param tokenId the value of the NFT (not used)
     @dev retrieves the value of the `s_token_uri`
     @return the stored value */
  function tokenURI(
    uint256 /* tokenId */
...
contract Whitelist is Ownable {
  ...
  function addAddressesToWhitelist(address[] memory addrs) external onlyOwner {
