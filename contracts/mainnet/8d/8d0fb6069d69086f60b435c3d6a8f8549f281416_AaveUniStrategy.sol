...
contract UniswapActions is Base {
    ...
    IMiniChef internal minichef = IMiniChef(0x0769fd68dFb93167989C6f7254cd0D766Fb2841F);
    int24 internal constant tickSpacing = 10;
    uint24 public buffer = 350; // 5%
    int24 public distanceLower = 2250;
    int24 public distanceUpper = 2250;
    int24 public tickLower = 200_250; // 202580
    int24 public tickUpper = 204_750;
    uint32 public maDurationRangeShift = 43200;
    uint32 public maDurationHedgeStatus = 43200;
    ...
    function getIdealRange() public view returns (int24 lower, int24 upper) {
        int24 currentTick = getAveragePriceTick(maDurationRangeShift);
        currentTick = currentTick - (currentTick % tickSpacing);
        lower = currentTick - distanceLower;
        upper = currentTick + distanceUpper;
    }

    function getAveragePriceTick(uint32 duration) public view returns (int24 tick) {
        uint32[] memory secondsAgo = new uint32[](2);
        secondsAgo[0] = 0;
        secondsAgo[1] = duration;
        (int56[] memory tickCumulatives,) = pool.observe(secondsAgo);
        tick = int24((tickCumulatives[0] - tickCumulatives[1]) / int56(uint56(duration)));
    }
    ...
    function assetsInV3Average() public view returns (uint256 amount0, uint256 amount1) {
        int24 tick = getAveragePriceTick(maDurationHedgeStatus);
...
contract AaveUniStrategy is UniswapActions, AaveActions {
        ...
        returns (int256 liquidityChange, int256 collateralChange, int256 debtChange)
    {
        ...
        if (hedgeFactor > targetHedgeFactor && healthFactor > targetHealthFactor) {
            // borrow weth, sell to usdc, lp
            _borrow(weth, amount);
            ...
            return (int256(uint256(liquidity)), 0, -int256(amount + extraBorrow));
        } else if (hedgeFactor > targetHedgeFactor && healthFactor <= targetHealthFactor) {
    ...
    function setHedgeFees(bool _hedgeFees) external onlyAuthorised {
