...
import "@metacrypt/contracts/erc721/MetacryptERC721.sol";

/// @title ERC721 Contract for Tribul Access Pass
/// @author Akshat Mittal ([email protected])
contract TribulAccessPass is MetacryptERC721, Ownable, Pausable, AccessControl {
    ...
    constructor(
        ...
        MetacryptERC721(
            "Tribul Access Pass",
            "Tribul-PASS",
            "https://tribul-metadata.metacrypt.org/api/access-pass/",
            _royaltyReceiver,
            _royaltyNumerator
        )
    {
    ...
    function appMint(address to, uint256 quantity) public onlyRole(CONTROLLER_ROLE) {
        _mint(to, quantity);
    }

    function appBurn(uint256 tokenId) public onlyRole(CONTROLLER_ROLE) {
        _burn(tokenId);
    }
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(MetacryptERC721, AccessControl) returns (bool) {
        return MetacryptERC721.supportsInterface(interfaceId) || AccessControl.supportsInterface(interfaceId);
    }
}
...
import "./TribulAccessPass.sol";

/// @title ERC721 Contract for Tribul Curators
/// @author Akshat Mittal ([email protected])
contract TribulCurator is MetacryptERC721, Ownable, AccessControl {
    bytes32 public constant CONTROLLER_ROLE = keccak256("CONTROLLER_ROLE");

    TribulAccessPass public accessPass;
    ...
    constructor(
        ...
        MetacryptERC721(
            "Tribul Curator",
            "Tribul-CURATOR",
            "https://tribul-metadata.metacrypt.org/api/curator/",
            _royaltyReceiver,
            _royaltyNumerator
        )
    {
        ...
        accessPass = TribulAccessPass(_accessPass);
    }
    ...
    function mintAccessPass(address to) external {
        if (balanceOf(msg.sender) > 0) {
            accessPass.appMint(to, 1);
        }
    }
