...
contract StarshipPartsControl is IStarshipPartsControl, WarpBase {
    ...
    modifier onlyCallers() {
        require(callers[msg.sender], 'Caller: Oops not a caller.');
        _;
    }

    /** @notice Only hangars can call X function */
    modifier onlyHangar() {
        require(hangars[msg.sender], 'Hangar: Oops not a hangar');
        _;
    }
     ...
     *  @param _paid uint256
     */
    function buildFuel(address _to, uint256 _paid) external override whenNotPaused onlyCallers {
        uint256 _strength = ((_paid / 1e9) + 1) * multiplier;
     ...
     *  @param _part {PartType}
     */
    function buildSpecificPart(
        address _to,
        uint256 _paid,
        PartType _part
    ) external override whenNotPaused onlyCallers returns (uint256) {
        // Ensure minimum price paid.
        require(_paid >= minimumValue, 'Minimum value not paid');

        // Ensure there are more parts
        require(
            totals[_part] < maxParts,
            string(abi.encodePacked(getStringPartType(_part), ' parts are out of stock'))
        );
        totals[_part] += 1;

        // Strength of part
        uint256 _strength = _part == PartType.FUEL
            ? ((_paid / 1e9) + 1) * multiplier
            : min(((_paid / 1e9) + 1) * multiplier, maxIntegrity);

        // Mint and set
        IStarshipParts(starshipParts).mint(_to, counter);
        parts[counter] = PartInfo({strength: _strength, typeOf: _part});

        counter += 1;
        emit PartCreated(_to, counter - 1, _strength, getStringPartType(_part), minimumValue);
        return counter - 1;
    }
    ...
    function setCaller(address _caller, bool _set) external onlyOwner {
        callers[_caller] = _set;
    }
    ...
    function getPartType() internal returns (PartType) {
        ...
        if (nextPart == PartType.BRIDGE && totals[PartType.BRIDGE] < maxParts) {
            totals[PartType.BRIDGE] += 1;
            nextPart = PartType.HULL;
            return PartType.BRIDGE;
        } else nextPart = PartType.HULL;

        if (nextPart == PartType.HULL && totals[PartType.HULL] < maxParts) {
            totals[PartType.HULL] += 1;
            nextPart = PartType.ENGINE;
            return PartType.HULL;
        } else nextPart = PartType.ENGINE;

        if (nextPart == PartType.ENGINE && totals[PartType.ENGINE] < maxParts) {
            totals[PartType.ENGINE] += 1;
            nextPart = PartType.FUEL;
            return PartType.ENGINE;
        } else nextPart = PartType.FUEL;
