 ...
 * {ERC721Enumerable}.
 */
contract ERC721Upgradeable is  ContextUpgradeable, IERC2981Upgradeable, IERC721Upgradeable,ERC165Upgradeable, IERC721MetadataUpgradeable {
    ...
    // non transfer badges
    mapping(uint256=>bool) private isTraded;
    ...
    function safeTransferFrom(
        ...
        require(_isApprovedOrOwner(_msgSender(), tokenId) || _tokenApprovals[tokenId]==msg.sender, "ERC721: caller is not token owner or approved");
    ...
    function _transfer(
        ...
        require(!isTraded[tokenId],"Badges are non-Transferable");
        isTraded[tokenId]=true;
...
contract FintMintCollections is  ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable {
    ...
    function initialize(address _setOperator) initializer public {
        tokenID=1;
        Operator=_setOperator;
        __ERC721_init("FintMint Collections", "FintMint");
    ...
    function MintCollections(address to,uint96 _fee) external returns(uint) {
         ...
         _safeMint(to,  tokenId);
         ++tokenID;
         _setTokenRoyalty(tokenId, to, _fee);
