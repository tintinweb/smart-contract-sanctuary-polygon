...
contract CircleofDreamssRavenDruids is ERC721 , DefaultOperatorFilterer {
    ...
    constructor()
        ERC721("Circle of Dreams's Raven Druids", "CDRD")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 102) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeig3sy3auplxfkorobmtq5hn6mmxhgpwkjn56po3yvdfva7vd4hs44/", Strings.toString(tokenId), ".json")
            );
    }
