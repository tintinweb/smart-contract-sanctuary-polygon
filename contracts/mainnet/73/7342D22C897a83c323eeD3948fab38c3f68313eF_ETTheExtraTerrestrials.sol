...
/// @title Contract of E.T. The Extra-Terrestrials collection
/// @author Mestoph
...
//contract ETTheExtraTerrestrials is ERC721Enumerable, PaymentSplitter, Ownable, ReentrancyGuard {
contract ETTheExtraTerrestrials is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint public max_mint_allowed = 15;
    //Maximum number of NFTs an address can mint in preseal
    uint public max_mint_presale = 3;
    //Price of one NFT in presale
    uint public pricePresale = 0.005 ether;
    //Price of one NFT in sale
    uint public priceSale = 0.006 ether;
    //Max min ammount to mint per session
    ...
    //Presale only for whitelist ?
    bool presaleOnlyForWhiteListed = false;

    //Mint 0 index to sample ?
    bool mintZeroIndex = true;
    ...
    //    0x53de9022c995C6F39255458f275bc9F1242Af5Cb
    //];
    ...
    //constructor(string memory _name, string memory _symbol, string memory _theBaseURI, string memory _notRevealedURI, bytes32 _merkleRoot) ERC721(_name, _symbol) PaymentSplitter(_team, _teamShares) {
    constructor(string memory _name, string memory _symbol, string memory _theBaseURI, string memory _notRevealedURI, bytes32 _merkleRoot) ERC721(_name, _symbol) {
        _supply.reset();
        transferOwnership(msg.sender);
        sellingStep = Steps.Presale; //Before;
        baseURI = _theBaseURI;
    ...
    *
    * @param _account The account of the wallet
    *
    ...
    **/
    function presaleMint(address _account, bytes32[] calldata _proof) external payable nonReentrant {
        ...
        require(nftsPerWallet[_account] < max_mint_presale, "You can only get 3 NFT on the Presale");
        //Is this user on the whitelist ?
        if (presaleOnlyForWhiteListed) {
        ...
        _supply.increment();
        //Mint the user NFT
        _safeMint(_account, _supply.current());
    }
    ...
    **/
    function saleMint(address _account, uint256 _ammount) external payable nonReentrant {
        ...
        require(_ammount > 0, "need to mint at least 1 NFT");
        require(_ammount <= maxMintAmount, "max mint amount per session exceeded");
        require(_supply.current() + _ammount <= MAX_SUPPLY, "Sale is almost done and we don't have enought NFTs left.");
        ...
        if(_supply.current() + _ammount == MAX_SUPPLY) {
    ...
    **/
    function gift(address _account) external onlyOwner {
        require(_supply.current() + 1 <= MAX_SUPPLY, "Sold out");
        //Increment the Id of the next NFT to mint
        _supply.increment();
        //Mint the user NFT
        _safeMint(_account, _supply.current());
    ...
    *
    * @param _account The account to hash
    *
    ...
    **/
    ...
    /** 
    * @notice Returns all ID of an Owner
    *
    * @param _account The account to hash
    *
    * @return Array of nfts IDs of an owner
    **/
