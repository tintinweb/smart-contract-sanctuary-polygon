...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender() || addressAdmin[msg.sender] == true, "Ownable: caller do not has auth");
        _;
    }
...
interface IDiceWarNft {
    ...
    function burn(address _userAddr, uint8 tokenId, uint256 amount) external;
    function balanceOfNft(address account, uint id) external returns(uint256);
    function getLvList (uint8 tokenId) external  returns(address[] memory _addresses);
    function nftStakingRewards (address userAddr) external returns(uint);
...
contract House is ReentrancyGuard, Ownable {
    ...
    uint public lev1Token = 1000000000000000000000 ;
    uint public lev2Token = 10000000000000000000000 ;
    uint public lev3Token = 100000000000000000000000 ;
    uint public nftHoldersRewardsBP = 200;
    uint public projectRewardsBP = 80;
    uint public treasurePoolBP = 20;
    uint public jackpotBP = 20;
    uint public refBP = 80;
    uint public allBetAmount;
    ...
    event HousePlaceBet(address player, uint amount);
    event BalanceClaimed(address indexed player, uint amount);
    event RefRewardsClaimed(address indexed player, uint amount);
    ...
    function getHouseEdgeBP () external view returns(uint) {
        return houseEdgeBP;
    }
    ...
    // Converters
    function amountToWinnableAmount(uint _amount, uint rollUnder, uint MODULO) public view returns (uint) {
        require(0 < rollUnder && rollUnder <= MODULO, "Win probability out of range");
        uint bettableAmount = this.amountToBettableAmountConverter(_amount);
        return bettableAmount * MODULO / rollUnder;
    }

    function amountToBettableAmountConverter(uint amount) public view returns(uint) {
    ...
    function amountToJackPotConverter(uint _amount) internal view returns (uint) {
    ...
    function placeBet(address player, uint amount, uint rollUnder, uint MODULO, address refAddr) payable external isHouseLive admin nonReentrant {
        if(this.isNew(player)){
            register(player, refAddr);
        }
        uint bettableAmount = amountToBettableAmountConverter(amount);
        uint profitAmount = amount - bettableAmount;
        ...
        uint dicewarRewards = bettableAmount;
    ...
    function claimBalance(address player) external isHouseLive nonReentrant {
        ...
        uint gBalance = playerBalance[player];
    ...
    function claimRefRewards(address player) external isHouseLive nonReentrant {
        require(player == msg.sender , "Can only withdraw your own referral rewards");
        uint gBalance = user[player].refProfit;
        require(gBalance > 0, "No funds to claim");
        payable(player).transfer(gBalance);
     ...
     *Upgrade user's NFT
     */
    function upgradeNft(uint8 level) external {
        require(level == 1 || level == 2 || level == 3, "Can not upgrade this level");
        require(dicewarNft.balanceOfNft(msg.sender, level) >= 1, "You do not have NFT to upgrade");
        if(level == 1){
    ...
    function register (address _addr, address _refAddr) internal returns(bool){
        ...
        user[_addr] = userDetail(userSum+1,_addr,_refAddr,0,0,0,0);
        userSum ++;
    ...
    event stakingRewardsClaim(address userAddress, uint amount);
     function claimNftStakingRewards() public returns(bool){
        uint rewardAmount = dicewarNft.nftStakingRewards(msg.sender);
        require(rewardAmount > 0 , "No amount can be claimed");
        require(dicewarNft.unstakeAllToken(msg.sender), "unstake all token error");
