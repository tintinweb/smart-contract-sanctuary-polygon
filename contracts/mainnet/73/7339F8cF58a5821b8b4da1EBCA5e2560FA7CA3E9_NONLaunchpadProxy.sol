...
contract NONLaunchpadProxy is ILaunchpadProxy, Ownable, ReentrancyGuard {
    ...
    function updateLaunchpadNftTypeAndSourceAddress(
        bytes4 launchpadId,
        uint256 nftType,
        uint256 sourceAddress
    ) external {
        onlyLPADController(
            _msgSender(),
            launchpads[launchpadId].controllerAdmin
...
library LaunchpadBuy {
    ...
    function processBuy(
        ...
        uint256 nftType = lpad.roundss[roundsIdx].nftType;
        address sourceAddress = lpad.roundss[roundsIdx].sourceAddress;
        transferIncomes(
            lpad,
            sender,
        ...
        if (nftType == 0) {
            ...
            uint256 tokenId = lpadRounds.tokenId;
            callLaunchpadBuy(lpad, sender, quantity, sourceAddress, tokenId);
            lpad.roundss[roundsIdx].saleQuantity = uint32(
                lpad.roundss[roundsIdx].saleQuantity + quantity
            );
        }
        return shouldPay;
    }
