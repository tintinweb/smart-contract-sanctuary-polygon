...
interface IDataStore {
    function balance(address _xvault) external view returns (uint256);
    function tvl(address _xvault) external view returns (uint256);
...
import "./SVaultStorage.sol";
import "../utils/Sort.sol";
import "../utils/Array.sol";
import "../datastore/interfaces/IDataStore.sol";
import "../bridge/interfaces/IBridge.sol";
import "hardhat/console.sol";

contract SVaultAdmin is SVaultStorage {
    ...
    **********************************/
    ...
    /*********************************
                CHILD XVAULTS
    **********************************/
    function setIsParent(bool _isParent) external onlyGovernor {
        isParent = _isParent;
    }
    function setCXVaults(ChildXVault[] calldata _childXVaults) external onlyGovernor {
        // Pop out everthing from childXVaults

        // TODO: Currently expecting the allocations are sorted. May need to put sorting later
        uint256 totalTarget = 0;
        for (uint8 i = 0; i < _childXVaults.length; i++) {
            ChildXVault memory childXVault = _childXVaults[i];
            require(childXVault.xvault != address(0) && childXVault.chainId != 0 && childXVault.allocation != 0, "0x0 childXVault");
            totalTarget += childXVault.allocation;
            children[childXVault.chainId] = childXVault;
        }
    }
...
|    * @param _payoutTimeRange duration to honor payout time. Ex: 15 * 60;
    */
    ...
    /***************************
            REBALANCE
    ****************************/
    function intraRebalance() external onlyGovernor {
        _intraRebalance();
    }
     /**
    * @dev Balance the Vault with predefined weights
    */
    function _intraRebalance() internal {
    ...
    {
        fbalance = IERC20(asset).balanceOf(address(this));
        for (uint256 i = 0; i < allStrategies.length; i++) {
            ISingleStrategy strategy = ISingleStrategy(allStrategies[i]);
            fbalance = fbalance + strategy.balance();
        }
    }
    ...
    function setBridgingParams(uint256 _bridgingMinimumThreshold, uint256 _bridgingEpsilonBps) public onlyGovernor {
        bridgingMinimumThreshold = _bridgingMinimumThreshold;
        bridingEpsilonBps = _bridgingEpsilonBps;
    }
        
    function rebalance(uint256[] memory _chainIds, uint256[] memory _amounts, uint256[] memory _minAmounts) public onlyGovernor {
        require(_chainIds.length == _amounts.length && _chainIds.length == childChainIds.length, "Invalid input");
        uint256[] memory _calculatedBalances = getRebalanceData();
        for (uint i = 0; i < _calculatedBalances.length; i++) {
            // Check if the _amounts[i] and transferToChildren[i] are within bridgingEpsilon %
            require(_checkDiffBps(_amounts[i], _calculatedBalances[i], bridingEpsilonBps), "AMT_N_ALIGNED"); 
            if (_amounts[i] < bridgingMinimumThreshold) {
                console.log("Skip: %s %s", _chainIds[i], _amounts[i]);
                continue;
            }
             
            ChildXVault memory c = children[childChainIds[i]];
            if (isAssetNative) {
                IBridge(bridge).send{value:  _amounts[i]}(asset, childChainIds[i], c.xvault, _amounts[i], _minAmounts[i], isAssetNative);
            } else {
                IBridge(bridge).send(asset, childChainIds[i], c.xvault, _amounts[i], _minAmounts[i], isAssetNative);
            }
        }
    ...
    function getRebalanceData() internal view returns (uint256[] memory transferToChildren) {

        uint256 _totalChildBalance = 0;
        // loop through children
        for (uint i = 0; i < childChainIds.length; i++) {
            ChildXVault memory c = children[childChainIds[i]];
            _totalChildBalance += IDataStore(datastore).balance(c.xvault);
        }

        uint256 _totalBalance = _balance() + _totalChildBalance;

        // Based on allocation distrubte the _totalBalance in childrens
        for (uint i = 0; i < childChainIds.length; i++) {
            ChildXVault memory c = children[childChainIds[i]];
            transferToChildren[i] = (_totalBalance * c.allocation) / 10000;
        }
    }
}
...
import { ISingleStrategy } from "../interfaces/ISingleStrategy.sol";
import { Governable } from "../governance/Governable.sol";
import "../utils/Helpers.sol";
import { StableMath } from "../utils/StableMath.sol";
import { ISwapper } from "../swapper/ISwapper.sol";

contract SVaultStorage is ERC20Upgradeable, Governable {
    using SafeMath for uint256;
    ...
    //keccak256("sxvault.governor.admin.impl");
    bytes32 constant adminImplPosition = 0xdc24ef6d7bb0275dd264ec21353e4438b041bbf8e216b339c77dfc8a82029bb0;
    ...
    mapping(uint8 => CXVault) public cXVaults; // TO BE REMOVED
    ...
    mapping(uint256 => ChildXVault) public children;
    uint256[] public childChainIds;
    
    bool public isAssetNative;

    address public datastore;
    uint256 public bridingEpsilonBps;
    uint256 public bridgingMinimumThreshold;
