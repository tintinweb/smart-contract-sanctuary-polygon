...
contract PET is ERC721Base {
    ...
    // events
    // event PublicSaleMint(address indexed sender, uint256 indexed numPets);
    event OperatorMint(address indexed sender, uint256 indexed numPets);
    event OperatorMintTo(
        address indexed sender,
        address indexed to,
        uint256 indexed numPets
    );
    ...
    /**
     * @dev override tokenURI to support batch minting
     */
    function tokenURI(
        uint256 _tokenId
    ...
    function setMintPrice(uint256 _publicSalePrice) external onlyOwner {
    ...
    /**
     * @dev Public sale minting functions
     */
    function mintTo(
        address _to,
        string memory _tokenURI
    ) public override whenPublicSaleActive {
        ERC20(tokenContract).transfer(address(owner()), publicSalePrice);
        _safeMint(_to, 1, "");
    }
    ...
    function batchMintTo(
        ...
        require(_quantity > 0, "Must mint at least one pet");
        require(
            _quantity <= maxMintPerTx,
            "_quantity should not exceed maxMintPerTx"
        );

        ERC20(tokenContract).transfer(
            address(owner()),
            publicSalePrice * _quantity
        );
        _safeMint(_to, _quantity, "");
    }

    /**
     * @dev operator minting function
     */
    function setMerkleRoot(bytes32 _merkleRoot) external onlyOwner {
        merkleRoot = _merkleRoot;
    }

    function operatorMintPet(
        uint256 _numPets,
        ...
        require(isValid, "Sender is not eligible to mint");
        require(_numPets > 0, "Must mint at least one pet");

        emit OperatorMint(msg.sender, _numPets);
        _safeMint(msg.sender, _numPets);
    }
    ...
    function operatorMintTo(
        ...
        emit OperatorMintTo(msg.sender, _to, _numPets);
        _safeMint(_to, _numPets);
    }
}