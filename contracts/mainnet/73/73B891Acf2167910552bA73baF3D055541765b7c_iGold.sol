...
contract iGold is ERC20, Ownable {
    ...
    function buy(uint256 usdtAmount) public {
        ...
        uint256 goldAmount = usdtAmount.mul(1e2).div(uint256(goldPrice)).mul(1e1); // 0.1g per token
    ...
    function sell(uint256 goldAmount) public {
        ...
        uint256 usdtAmount = goldAmount.mul(uint256(goldPrice)).div(1e4).div(1e1); // 0.1g per token
        ...
        returns (uint256 goldValue, uint256 usdtInVault)
    {
        ...
        goldValue = totalMintedGold.mul(uint256(goldPrice)).div(1e4).div(1e1); // Calculate the value of minted iGold tokens in USDT
