...
/_.___/\___/_.___/\____/   /_/   /_/_/ /_/\__,_/_/ /_/\___/\___/
    ...
    http://bebu.finance
*/
contract BEARETH1X is ERC20Burnable, Operator {
     ...
     */
    constructor() public ERC20("BEARETH1X", "BEARETH1X") {
    ...
    function _isPair(address _address) public view returns (bool) {
        try  IUniswapV2Pair(_address).factory() {
            return true;
        } catch {
            return false;
        }    
    ...
    function _getAfterSwapPrice(uint amountIn) internal view returns (uint) {
        (uint256 amount0, uint256 amount1, ) = IUniswapV2Pair(pair).getReserves();
    ...
    //calculate tax 
    function _calculateTax(uint amount, uint _AfterSwapPrice, uint _IndexPrice) internal pure returns (uint tax) {
        ...
        tax = amount.mul(numerator).div(denominator);
    }
    ...
    //calculate reward
    function _calculateReward(uint amount, uint _RealtimePrice, uint _IndexPrice) internal pure returns (uint reward) {
        uint diff = _IndexPrice - _RealtimePrice;
        uint numerator = diff.mul(2);
        uint denominator = Babylonian.sqrt(diff.mul(diff).mul(16) + _IndexPrice.mul(_IndexPrice));
        reward = amount.mul(numerator).div(denominator);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        // tokens transfered from EOA to pair address
        if(_isPair(to)){
