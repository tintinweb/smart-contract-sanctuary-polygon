...
contract ERC721Facet is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        return LibERC721.getTokenURI(tokenId);
    }
    ...
    function getDNA(uint256 _tokenId) external view returns (uint256) {
        return LibERC721.getDNA(_tokenId);
    }
        ...
        external
    {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            LibERC721.setDNAForGenesisToken(_tokenIds[i], _class);
        }
    }
    ...
    function lockUnicornIntoGame(uint256 tokenId) external {
        LibPermissions.enforceCallerOwnsNFTOrHasPermission(tokenId, IPermissionProvider.Permission.UNICORN_AIRLOCK_IN_ALLOWED);
    ...
    function unlockUnicornOutOfGameGenerateMessageHash(
        ...
        return LibAirlock.unlockUnicornOutOfGameGenerateMessageHash(tokenId, tokenURI, requestId, blockDeadline);
    }
    ...
    function unlockUnicornOutOfGameWithSignature(
        ...
        require(
            SignatureChecker.isValidSignatureNow(LibDiamond.gameServer(), hash, signature),
        ...
        LibPermissions.enforceCallerOwnsNFTOrHasPermission(tokenId, IPermissionProvider.Permission.UNICORN_AIRLOCK_OUT_ALLOWED);
        LibAirlock.unlockUnicornOutOfGame(tokenId, tokenURI);
    }

    function forceUnlockUnicornOutOfGame(uint256 tokenId) external {
    ...
    function setForceUnlockUnicornCooldown(uint256 cooldownSeconds) external {
        ...
        LibDiamond.diamondStorage().erc721_forceUnlockUnicornCooldown = cooldownSeconds;
    }

    function getForceUnlockUnicornCooldown() external view returns (uint256) {
        return LibDiamond.diamondStorage().erc721_forceUnlockUnicornCooldown;
    }

    function unicornLastForceUnlock(uint256 tokenId) external view returns (uint256) {
        return LibDiamond.diamondStorage().erc721_unicornLastForceUnlock[tokenId];
    }
    ...
    function unicornIsTransferable(uint256 tokenId) external view returns (bool) {
        return LibERC721.unicornIsTransferable(tokenId);
    }

    //  TODO: Deprecate this for TokenAddressInitializerFacet
    ...
    function getPaymentAmount() internal view returns (uint256) {
        // Raw value * 10^18 (weth decimals)
        return 0.1 * 10**18;
    }

    function mint(address _to) public {
        LibDiamond.enforceIsContractOwner();
        LibERC721.mintNextToken(_to);
    }
    
    function getMintAllowance(address _address) public view returns (uint256) {
        return LibDiamond.diamondStorage().erc721_mint_whitelist[_address];
    }

    function setTokenURI(uint256 tokenId, string memory _tokenURI) public {
        LibDiamond.enforceIsOwnerOrGameServer();
        require(
            LibDiamond.diamondStorage().erc721_owners[tokenId] != address(0),
            LibUtil.concat("ERC721: Cannot set tokenURI for unminted token ", LibUtil.uintToString(tokenId))
        );
    ...
    function claimGenesisTokens(address _to, uint256 _numTokens)
        external
        virtual
    {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        require(
            block.timestamp >=
                LibDiamond.diamondStorage().erc721_genesisEggPresaleUnlockTime,
            "ERC721Facet: Pre-sale is not unlocked yet."
        );

        require(
            ds.erc721_mint_whitelist[_to] >= _numTokens,
            "ERC721Facet: Insufficient allowance in mint whitelist for _to address"
        );

        // Soldity >0.8.0 reverts on overflow, making the addition operation safe.
        require(
            ds.erc721_current_token_id + _numTokens <=
                LibDiamond.ERC721_GENESIS_TOKENS,
            "ERC721Facet: genesis token limit reached"
        );
        ...
        require(
            wethToken.allowance(msg.sender, address(this)) >=
                getPaymentAmount() * _numTokens,
            "ERC721Facet: Insufficient allowance to transfer Weth"
        );

        for (uint256 i = 0; i < _numTokens; i++) {
            LibERC721.mintNextToken(_to);
            ds.erc721_mint_whitelist[_to] -= 1;
        }

        // Making tranfer after minting
    ...
    // It will override any existing mintAllowance for the given addresses.
    function addWhiteListAddresses(
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            ds.erc721_mint_whitelist[addresses[i]] = mintAllowance;
        }
    }
}
