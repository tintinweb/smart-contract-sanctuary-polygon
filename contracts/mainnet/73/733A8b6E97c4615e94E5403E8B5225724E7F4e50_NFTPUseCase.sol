...
interface ITwapGetter {
    function calculateTwapPriceFromLiquidity(
        uint32 twapInterval
    ) external view returns (uint256);
}

contract NFTPUseCase is Ownable, ReentrancyGuard {

    address public nftp;
    address public admin;
    address public rechargeAdmin;
    IERC20 public paidWith;
    ITwapGetter public twapGetter;
    uint256 public useCaseId = 1;
    uint32 public twapInterval;

    mapping (uint256 => bool) private useCaseSpent;
    mapping (uint256 => mapping(uint256 => uint256)) private collectionRarityPrice;

    event UseCaseSpent(uint256 indexed tokenId, address indexed owner, uint256 timestamp, uint256 indexed useCaseId);
    event UseCaseRecharged(uint256 indexed tokenId, address indexed owner, uint256 timestamp);
    ...
    constructor(address nftp_, address token_, address twapGetter_, uint32 twapInterval_) {
        if (nftp_ == address(0)) revert ZeroAddress();
        if (token_ == address(0)) revert ZeroAddress();
        if (twapGetter_ == address(0)) revert ZeroAddress();

        nftp = nftp_;
        paidWith = IERC20(token_);
        twapGetter = ITwapGetter(twapGetter_);
        twapInterval = twapInterval_;
        admin = msg.sender;
        rechargeAdmin = msg.sender;
    }
    ...
    modifier onlyRechargeAdmin() {
        require(msg.sender == rechargeAdmin, "Not authorized to perform the action");
        _;
    }
    ...
    */
        ...
        onlyOwner
    {
        ...
        require(_adminAddress != address(0), "Account cannot be 0");
        admin = _adminAddress;
    }

    function setTwapInterval(uint32 _twapInterval) external onlyOwner {
        require(_twapInterval >= 0, "Cannot be negative!");
        twapInterval = _twapInterval;
    }

    /**
    * @notice Sets the recharge admin address
    * @param _adminAddress address of the recharge admin
    */
    function setRechargeAdmin(address _adminAddress)
        external
        onlyOwner
    {
        require(
            _adminAddress != rechargeAdmin,
            "Account already configured"
        );
        require(_adminAddress != address(0), "Account cannot be 0");
        rechargeAdmin = _adminAddress;
    }

    /**
    * @notice Allows NFT owner to use it for an off-chain usecase
    * @param tokenId Id of NFT token to be spent
    */
    function spend(uint256 tokenId) external {
        if (msg.sender != IERC721(nftp).ownerOf(tokenId)) revert NotNFTOwner();
        if (useCaseSpent[tokenId]) revert AlreadySpent();

        useCaseSpent[tokenId] = true;

        emit UseCaseSpent(tokenId, msg.sender, block.timestamp, useCaseId);
        useCaseId++;
    }

    function authorizedSpend(uint256 _tokenId, address _addr) external onlyAdmin {
        if (_addr != IERC721(nftp).ownerOf(_tokenId)) revert NotNFTOwner();
        if (useCaseSpent[_tokenId]) revert AlreadySpent();
        useCaseSpent[_tokenId] = true;

        emit UseCaseSpent(_tokenId, _addr, block.timestamp, useCaseId);
        useCaseId++;
    }

    /**
    * @notice  */
    function rechangeByOwner(uint256 tokenId) external onlyOwner {
        _recharge(tokenId);
    }

    function authorizedRecharge(uint256 _tokenId, address _address) external onlyRechargeAdmin {
        if (_address != IERC721(nftp).ownerOf(_tokenId)) revert NotNFTOwner();
        _recharge(_tokenId);
    }

    function rechargeWithTokens(uint256 tokenId) external {      
        uint256 cost = getRechargeCost(tokenId);
    ...
    function _recharge(uint256 tokenId) internal nonReentrant {
        useCaseSpent[tokenId] = false;

        emit UseCaseRecharged(tokenId, msg.sender, block.timestamp);
    }

    function setRechargePrice(uint256 collectionId, uint256 rarity, uint256 price) external onlyOwner {
        collectionRarityPrice[collectionId][rarity] = price;
    }

    function getRechargePriceByCollectionRarity(
        uint256 id,
        uint256 rarity
    ) 
        external 
        ...
        returns(uint256) 
    {
        return collectionRarityPrice[id][rarity];
    }

    function getRechargePrice(uint256 tokenId) public view returns(uint256) {
        (uint256 id, uint256 rarity,) = IPussiesNFT(nftp).getInfo((tokenId));
        return collectionRarityPrice[id][rarity];
    }

    function getRechargeCost(uint256 tokenId) public view returns(uint256) {
        uint256 price = getRechargePrice(tokenId);
        uint256 paidWithPrice = twapGetter.calculateTwapPriceFromLiquidity(twapInterval);
        return paidWithPrice == 0 ? 0 : price * 10**18 / paidWithPrice;
    }
}