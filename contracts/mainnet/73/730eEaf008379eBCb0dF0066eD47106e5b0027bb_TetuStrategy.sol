    ...
    BalancerExchange
{
    ...
    function withdraw(
        ...
        uint256 numberOfShares = (OvnMath.addBasisPoints(
            _amount,
            BASIS_POINTS_FOR_SLIPPAGE
    ...
    function checkBalance() external view override returns (uint256) {
        ...
        if (
            ...
            balanceWithInvestments =  IStableSwapPool(curvePool).get_dy_underlying(curvePoolIndices[address(token0)], curvePoolIndices[address(primaryStable)], balanceWithInvestments);
        }
    ...
    function setBalancer(
        ...
        require(
            _balancerPoolIdUsdcTusdDaiUsdt != "",
            "Empty pool id not allowed"
        );
    ...
    function _swapAssetToPrimaryStable() internal {
        if (
            ...
            IStableSwapPool(curvePool).exchange_underlying(
                curvePoolIndices[address(token0)],
                curvePoolIndices[address(primaryStable)],
                token0.balanceOf(address(this)),
                token0.balanceOf(address(this)).scaleBy(Helpers.getDecimals(address(primaryStable)), Helpers.getDecimals(address(token0))).subBasisPoints(100)
            );
    ...
    function _swapPrimaryStableToToken0() internal {
        ...
        if (address(primaryStable) != address(token0)) {
            primaryStable.safeIncreaseAllowance(
                curvePool,
            ...
            IStableSwapPool(curvePool).exchange_underlying(
                ...
                curvePoolIndices[address(token0)],
                primaryStableBalance,
                primaryStableBalance.scaleBy(Helpers.getDecimals(address(token0)), Helpers.getDecimals(address(primaryStable))).subBasisPoints(100)
            );
