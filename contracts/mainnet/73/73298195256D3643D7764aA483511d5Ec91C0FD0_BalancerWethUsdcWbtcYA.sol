...
contract BalancerWethUsdcWbtcYA is BaseYA, UniswapV2Giddy, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  address constant private BPTC_TOKEN = 0x03cD191F589d12b0582a99808cf19851E468E6B5;
  address constant private BPTCG_TOKEN = 0x0F09F70Ed59202c77aC667f574A5f79bC65CeA48;
  ...
  address constant private BALANCER_REWARDS = 0x0F09F70Ed59202c77aC667f574A5f79bC65CeA48;
  bytes32 constant private POOL_ID = 0x03cd191f589d12b0582a99808cf19851e468e6b500010000000000000000000a;
  ...
  function getContractBalance() public view override returns (uint256) {
    return IERC20(BPTCG_TOKEN).balanceOf(address(this));
  }
  ...
  function compound() public override whenNotPaused {
    ...
    if (earnings > COMPOUND_THRESHOLD) {
      ...
      singleSwap.poolId = 0xf461f2240b66d55dcf9059e26c022160c06863bf000100000000000000000006;
      singleSwap.kind = IBalancer.SwapKind.GIVEN_IN;
      singleSwap.assetIn = BALANCER_TOKEN;
      singleSwap.assetOut = WMATIC_TOKEN;
      singleSwap.amount = earnings;
      ...
      if (!IERC20(BALANCER_TOKEN).approve(BALANCER_POOL, earnings)) {
        revert();
      }
      earnings = IBalancer(BALANCER_POOL).swap(singleSwap, fundManagement, 0, block.timestamp + 1200);
      earnings = swapTokensSimple(SUSHI_ROUTER, WMATIC_TOKEN, USDC_TOKEN, address(this), earnings);
  ...
  function depositCalc(uint256 value, uint256 fap) external override returns (uint256 amountOut, uint256 priceImpact) {
    amountOut = joinPool(value - fap, true);
    priceImpact = 0;
  }
  ...
  function depositGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap) external override whenNotPaused nonReentrant() {
    ...
    value = swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, address(this), value);
  ...
  function withdrawCalc(uint256 shares, uint256 fap) external override returns (uint256 amountOut, uint256 priceImpact) {
    amountOut = exitPool(sharesToValue(shares), true) - fap;
    priceImpact = 0;
  }
  ...
  function withdrawGiddyCalc(uint256 shares, uint256 fap) external override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    (amountOut, priceImpact) = calcPriceSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, amountOut);
    amountOut -= fap;
  }
  ...
  function deductFeeGiddy(uint256 amount, uint256 fap) internal returns (uint256) {
    if (fap > 0) {
      ...
      route[0] = GIDDY_TOKEN;
      route[1] = USDC_TOKEN;
      swapTokens(SUSHI_ROUTER, route, GIDDY_CONFIG.feeAccount(), fap);
    }
    return amount - fap;
  }

  function joinPool(uint256 usdcValue, bool query) private returns(uint256 amountOut) {
    ...
    joinRequest.userData = abi.encode(IBalancer.JoinKind.EXACT_TOKENS_IN_FOR_BPT_OUT, amounts, 0);
    if (query) {
      ...
      amountOut = IERC20(BPTC_TOKEN).balanceOf(address(this));
    }
  }

  function join(uint256 usdcValue) private returns(uint256 shares) {
    if (!IERC20(USDC_TOKEN).approve(BALANCER_POOL, usdcValue)) {
      revert();
    }

    uint256 bptcValue = joinPool(usdcValue, false);
    shares = contractShares == 0 ? bptcValue * INIT_SHARES : bptcValue * contractShares / getContractBalance();

    if (!IERC20(BPTC_TOKEN).approve(BALANCER_REWARDS, bptcValue)) {
      revert();
    }
    IRewards(BALANCER_REWARDS).deposit(bptcValue);
  }

  function exitPool(uint256 bptcValue, bool query) private returns (uint256 usdcValue) {
    ...
    exitRequest.userData = abi.encode(IBalancer.ExitKind.EXACT_BPT_IN_FOR_ONE_TOKEN_OUT, bptcValue, 1);
...
contract UniswapV2Giddy {
  uint256 constant private BASE_PERCENT = 1e6;
  uint256 constant internal SWAP_FEE = 3000;
  ...
  function calcSwap(address router, address src, address dst, uint256 inBefore, uint256 inAfter) internal view returns (uint256 outBefore, uint256 outAfter) {
    ...
    outAfter = (outAfter * (BASE_PERCENT - SWAP_FEE)) / BASE_PERCENT;
  }
  ...
  function calcPrice(address router, address[] memory route, uint256 value) internal view returns (uint256 amountOut, uint256 priceImpact) {
    ...
    priceImpact = outBefore * BASE_PERCENT / amountOut;
