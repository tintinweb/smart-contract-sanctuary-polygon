...
contract MoaMarketplaceDonor is IMoaMarketplaceDonor, ReentrancyGuard, Ownable {
    ...
    function batchListForSale(
        ...
        require(price > 0);
        require(donationDays > 0 && donationDays <= 60);

        for (uint256 i = 0; i < tokenIds.length; i++) {
            listForSale(nftContract, tokenIds[i], price, donationDays);
        }
    }
...
interface IMoaMarketplaceDonor {
    ...
    /// @dev create a batch of market item for sale of as Support MoA.
