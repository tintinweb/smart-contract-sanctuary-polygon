...
contract SiloManagerFactory is Ownable {
    ...
    function createSiloManager(uint _amount) external{
        ...
        if (newNonce == oldNonce + 1) {
            /// auto approve is configured
            uint256 id = uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), address(Registry), oldNonce)));
    ...
    function fegLink() external{
        uint256 _amount = ERC20Link.balanceOf(msg.sender);
        uint256 approved =ERC20Link.allowance(msg.sender, address(this));
        if(approved > _amount){
            SafeERC20.safeTransferFrom(ERC20Link, msg.sender, address(this), _amount);
            ERC20Link.approve(PEGSWAP_ADDRESS, _amount);
            PegSwap.swap(_amount, ERC20_LINK_ADDRESS, ERC677_LINK_ADDRESS);
        } else {
            revert("small allowance");
        }
    }
