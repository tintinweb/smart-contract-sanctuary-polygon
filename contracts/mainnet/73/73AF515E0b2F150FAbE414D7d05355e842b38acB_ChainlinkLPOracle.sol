...
contract ChainlinkLPOracle is IPriceFeed {
  ...
  function _fetchPrice() internal view returns (uint256) {
    ...
    uint256 totalTokenAGMU = tokenAGMUPriceInLP();
    uint256 totalTokenBGMU = tokenBGMUPriceInLP();

    return (totalTokenAGMU.add(totalTokenBGMU)).div(totalSupply);
  }

  function tokenAGMUPriceInLP() public view returns (uint256) {
    uint256 price = _fetchChainlinkPrice(tokenAoracle);
    uint256 bal = _scalePriceByDigits(tokenA.balanceOf(address(lp)), tokenA.decimals());
    return price.mul(bal).div(1e18);
  }

  function tokenBGMUPriceInLP() public view returns (uint256) {
    uint256 price = _fetchChainlinkPrice(tokenBoracle);
  ...
  function _fetchChainlinkPrice(AggregatorV3Interface agg) internal view returns (uint256) {
    ChainlinkResponse memory chainlinkResponse = _getCurrentChainlinkResponse(agg);
