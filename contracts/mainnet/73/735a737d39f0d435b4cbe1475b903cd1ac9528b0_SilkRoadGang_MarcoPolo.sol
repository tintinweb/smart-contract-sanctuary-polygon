...
contract SilkRoadGang_MarcoPolo is ERC721, ERC721Enumerable, Ownable {
    ...
    mapping(address => uint256) public addressMintedBalance;    //presale whitelisted

    uint96 royalityFeeInBips;
    string public contractUri;
    address RoyalityReciever;

    constructor(uint96 _royalityFeeInBips, string memory _contractUri,string memory _uriPrefix) ERC721("Silk Road Gang MARCO POLO", "MAPO") {
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if (msg.sender != owner()) {
            if(onlyWhitelisted == true) {
                ...
                uint256 totalNFT = balanceOf(msg.sender);
                require( totalNFT != nftPerAddressLimit && totalNFT <= nftPerAddressLimit , "Max Wallet NFT Limit Reach!!");
