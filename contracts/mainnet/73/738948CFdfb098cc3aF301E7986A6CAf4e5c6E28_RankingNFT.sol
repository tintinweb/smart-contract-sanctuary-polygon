...
contract RankingNFT is NFTAuth, ERC721Upgradeable {
  using ArrayUtil for uint[];
  uint public totalSupply;
  uint private tokenCounter;
  IVault public vault;
  bool public freeTransferable;
  mapping (address => uint[]) private ownerNFTs;
  mapping (address => bool) private exclusives;

  function initialize(
    string calldata _name,
    string calldata _symbol
  ) public initializer {
    NFTAuth.init(msg.sender);
    ERC721Upgradeable.__ERC721_init(_name, _symbol);
    freeTransferable = true;
  }

  function updateFreeTransferable(bool _freeTransferable) external onlyMn {
    freeTransferable = _freeTransferable;
  }
  ...
  function mint(address _owner, uint _quantity) external onlyMintRight {
  ...
  function _beforeTokenTransfer(address _from, address _to, uint _tokenId) internal override {
    require(!waitingList[_from], "You can not do this at the moment");
    require(freeTransferable, "NFT: transfer is not allowed");
    if (_from == address(0)) {
      totalSupply++;
    } else if (!exclusives[_from]) {
      ownerNFTs[_from].removeElementFromArray(_tokenId);
    }
    if (_to == address(0)) {
      totalSupply--;
    } else if (!exclusives[_to]) {
      ownerNFTs[_to].push(_tokenId);
    }
    _tokenId;
  }

  function _afterTokenTransfer(address _from, address _to, uint _tokenId) internal override {
    vault.updateQualifiedLevel(_from, _to);
    _tokenId;
  }

  function _initDependentContracts() override internal {
    mintAble[addressBook.get("swap")] = true;
    address vaultAddress = addressBook.get("vault");
    require(vaultAddress != address(0), "RankingNFT: vault address invalid");
...
abstract contract NFTAuth is Auth, ContextUpgradeable {
  ...
  function init(address _mn) virtual override public {
    Auth.init(_mn);
  }

  modifier onlyMintRight() {
    require(_isMintRight() || _isMn(), "NFTAuth: Only mint right");
    _;
  }

  modifier onlyTransferable() {
    require(_isTransferAble() || _isMn(), "NFTAuth: Only transferAble");
    _;
  }
  ...
  function updateMintAble(address _address, bool _mintAble) external onlyMn {
    require(_address != address(0), "NFTAuth: Address invalid");
    mintAble[_address] = _mintAble;
  }

  function updateTransferable(address _address, bool _transferable) external onlyMn {
    require(_address != address(0), "NFTAuth: Address invalid");
