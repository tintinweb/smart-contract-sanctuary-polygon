 ...
 */
 contract RajuPresale is Owned{
    ...
    function buy(uint _amount) public {
        ...
        {
            pTokenAmount = _amount.div(rate1).mul(RATE_DECIMALS);
        }
        else if(block.timestamp >= startTime2 && block.timestamp < (startTime2 + period2))
        {
            pTokenAmount = _amount.div(rate2).mul(RATE_DECIMALS);
        }
    ...
    function sell(uint _amount) public {
        ...
        {
            usdcAmount = _amount.mul(rate2).div(RATE_DECIMALS);
        }
    ...
    function getPercentReleased() public view returns (uint released) {
        ...
        return 30;
            ...
            released = 30 + dailyRate.mul(startTime3 - block.timestamp).div(86400);
            if (released >= 100) 
                return 100;
            return released;
        } 
    ...
    function update() public onlyOwner {
        ...
        for (uint i;i < buyerAccounts.length; i++) {
            preBuys[buyerAccounts[i]].pTokenAmount *= rebaseRate.div(REBASE_DECIMALS);
            preBuys[buyerAccounts[i]].pTokenClaimedAmount *= rebaseRate.div(REBASE_DECIMALS);
        }
    }
    ...
    function claim() public {
        ...
        totalpTokenAmountToDistribute -= pTokenToClaim;
