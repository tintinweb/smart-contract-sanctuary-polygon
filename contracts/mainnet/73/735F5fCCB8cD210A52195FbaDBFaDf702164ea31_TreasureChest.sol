...
contract TreasureChest is ERC721URIStorage, Ownable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(_baseURI(), "gem", "/", tokenId.toString()));
    }
    ...
    function updatePublicMintingEnabled(bool enabled) external onlyAdmin {
        isPublicMintingEnabled = enabled;
    }

    function updateWhitelistMintingEnabled(bool enabled) external onlyAdmin {
        isWhitelistMintingEnabled = enabled;
    }

    function updateOpenEnabled(bool enabled) external onlyAdmin {
