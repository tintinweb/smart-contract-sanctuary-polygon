...
contract MaltWhitelist is Permissions {
    ...
    onlyRoleMalt(ADMIN_ROLE, "Must have admin privs")
  {
    ...
    malt.mint(address(this), requirement);

    // Just using Malt here is safe as no malt exists in circulating
    // so a griefing attack of sending Malt here is not possible
    ...
    // Renounce role now that we have minted
    // To avoid any future issues with the dangling permission
    malt.renounceRole(keccak256("MONETARY_MINTER_ROLE"), address(this));
    ...
    collateralToken.safeApprove(address(router), maltBalance);
    ...
    uint256 rewardBalance = collateralToken.balanceOf(address(this));

    // LE gets 25%
    uint256 liquidityExtensionCut = rewardBalance * 25 / 100;
    uint256 swingTraderCut = rewardBalance - liquidityExtensionCut;

    collateralToken.safeTransfer(liquidityExtension, liquidityExtensionCut);
    collateralToken.safeTransfer(swingTrader, swingTraderCut);
  ...
  function setCollateralToken(address _collateralToken)
    ...
    onlyRoleMalt(ADMIN_ROLE, "Must have admin privs")
  {
    require(_collateralToken != address(0), "Cannot be addr(0)");
    collateralToken = IERC20(_collateralToken);
  }
