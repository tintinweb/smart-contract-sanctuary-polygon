...
contract MutualAccess is ERC1155, Ownable, ReentrancyGuard, IERC2981 {
    ...
    mapping(uint256 => RoyaltyRecipient) private erc2981royalties;
    // Allowed address to call set approval for all
    mapping(address => bool) private allowedMarketplaces;
    ...
    constructor(
        ...
        allowedMarketplaces[address(this)] = true;
    }
    ...
    {
        if (erc2981royalties[tokenId].recipient == address(0)) {
            revert InvalidArgument();
        }
        receiver = erc2981royalties[tokenId].recipient;
        royaltyAmount =
            (erc2981royalties[tokenId].feePercentage * salePrice) /
            PERCENTAGE_DIVIDER;
    }
    ...
    function setRoyaltyInfo(
        ...
        erc2981royalties[tokenId].recipient = receiver;
        erc2981royalties[tokenId].feePercentage = percentage;
    }
        ...
        nonReentrant
    {
        ...
        if (content[contentId].primarySaleEnded == false) {
            ...
            delete secondaryTicketPrices[contentId][ticketSeller];
        }
    }
    ...
    // Override the ERC1155 setApprovalForAll to allow setting only this contract to be the operator
    function setApprovalForAll(address operator, bool approved)
        public
        override(ERC1155)
    {
        // after marketplace was removed, ok to clear but illegal to set
        if (allowedMarketplaces[operator] == false && approved) {
            revert InvalidArgument();
        }
    ...
    function setMarketApprovedAddress(address approvedContract, bool approval)
        external
        onlyOwner
    {
        allowedMarketplaces[approvedContract] = approval;
    }
}
