...
// File: TreePoetsDonations.sol


pragma solidity ^0.8.4;

contract TreePoetsDonations is ERC721A, Ownable, ERC2981, DefaultOperatorFilterer {
    using Strings for uint256;
    ...
    constructor(string memory _initBaseURI, string memory _initNotRevealedUri, string memory _contractURI) ERC721A("Tree Poets Donations", "TPD") {
    ...
    function mint(uint256 ID) public payable {
        ...
        if (msg.sender != owner()) {
            require(
                block.timestamp - mintedTime[ID] >= timeGap,
                "Too many request per day"
            );
