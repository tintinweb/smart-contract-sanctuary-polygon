...
contract Stable is OwnableUpgradeable , ERC721HolderUpgradeable {
   ...
   // ------------------upgrade
    ...
    mapping(uint => mapping(uint => uint)) public growAmountItem;
    NewUserRefer public newRefer;
    mapping(address => uint) public growUpAmount;
    mapping(uint => uint) public cattlePutOutTime;
    ...
    function setMaxGrowAmount(uint amount) external onlyOwner{
        maxGrowAmount = amount;
    }
    ...
    function putIn(uint tokenId) external {
        ...
        if(cattle.isCreation(tokenId)&&cattle.getGender(tokenId) == 1){
            cattlePutOutTime[tokenId] = block.timestamp + 86400;
        }
    ...
    function isUsing(uint tokenId_) public view returns(bool){
        if(block.timestamp >= cattle.deadTime(tokenId_)){
            return false;
        }
    ...
    function putOut(uint tokenId) external {
        ...
        require(block.timestamp > cattlePutOutTime[tokenId],'too early to put out');
...
//        require(energy[tokenId]<= energyLimit,'out of energyLimit');
        energy[tokenId] += amount_ * effect[0];
        feeding[tokenId] = block.timestamp + (feedingTime[types - 1] * amount_);
        if(!cattle.getAdult(tokenId)){
            require(growAmount[refreshTime][tokenId]<= maxGrowAmount,'out limit');
            growAmount[refreshTime][tokenId] += amount_ * effect[0];
            grow[tokenId] += amount_ * effect[0];
            if(growAmount[refreshTime][tokenId] >= maxGrowAmount){
                grow[tokenId] -= growAmount[refreshTime][tokenId] - maxGrowAmount;
                growAmount[refreshTime][tokenId] = maxGrowAmount;
            ...
            if (grow[tokenId] >= 30000){
                ...
                growUpAmount[msg.sender]++;
                if(address(newRefer) != address(0) && growUpAmount[msg.sender] >= 2){
                    newRefer.finishTask(msg.sender,2);
                }
                emit GrowUp(msg.sender,tokenId);
                _addStableExp(msg.sender,20);
            }
            emit Grow(tokenId,amount_ * effect[0]);
        }
    ...
    function setGrow(uint tokenId,uint amount) external onlyOwner{
        grow[tokenId] = amount;
    }
...
//            require(growAmount[refreshTime][tokenId] <= maxGrowAmount,'out limit');
            uint temp = effect[1] * amount_;

            grow[tokenId] += temp;
//            if(growAmountItem[refreshTime][tokenId] + temp >= 5000){
//                temp = growAmountItem[refreshTime][tokenId] + temp - 5000;
//                growAmountItem[refreshTime][tokenId] = 5000;
//                growAmount[refreshTime][tokenId] += temp;
//            }else{
                growAmountItem[refreshTime][tokenId] += temp;
//            }
...
//            }
    ...
    function _addStableExp(address addr,uint amount) internal{
        ...
        if(address(newRefer) != address(0) && stableLevel[addr] >= 1){
            newRefer.finishTask(msg.sender,1);
        }
