...
contract ERC721Properties is ERC721Upgradeable, OwnableUpgradeable, App {
    ...
    uint8 immutable public ADMIN = _initNextRole();

    string constant INVALID_TIER_AMOUNT_ERROR = "INVALID_TIER_AMOUNT_ERROR";
    
    string constant PROPERTY_TIER = "TIER";

    string constant ATTRIBUTE_TIER_HAS_NO_COMMON_ARTWORK = "TIER_HAS_NO_COMMON_ARTWORK";
    ...
    uint32 public transferUnlocksAtHours;
    ...
    mapping(uint32 => bytes) internal _tokenIpfsHashOverride;
    bytes[] internal _tierIpfsHashes;
    ...
    function initialize(Initialization calldata init) external initializer {
        _transferOwnership(init.owner_);

        __ERC721_init(init.name, init.symbol);
        __App_init(init.kernel);

        isTransferable = init.isTransferable;
        if (isTransferable) {
            transferUnlocksAtHours = init.transferUnlocksAtHours;
        }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        bool isCommonArtwork = _getAttribute(PROPERTY_TIER, tier, ATTRIBUTE_TIER_HAS_NO_COMMON_ARTWORK) != bytes32(0);
        if (isCommonArtwork) {
            return string(abi.encodePacked(baseURI, tier));
        } else {
            return string(abi.encodePacked("ipfs://", _tokenIpfsHashOverride[tokenIdU32]));
        }
    }

    function setBaseURI(string calldata uri) external requirePermission(ADMIN) {
        baseURI = uri;
    }

    function mint(address to, bytes32 tierValue) external requirePermission(ADMIN) {
        ...
        bytes32[] memory propValues = new bytes32[](1);
    ...
    function burn(uint32 tokenId) external requirePermission(ADMIN) {
        _burn(tokenId);
    ...
    function getProperty(uint32 tokenId, string calldata key) external view returns (bytes32) {
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && to != address(0)) {
            ...
            require(block.timestamp / 1 hours > transferUnlocksAtHours);
        }
    }
    ...
    function _setAttribute(string memory propKey, bytes32 propValue, string memory attrKey, bytes32 attrValue) internal {
        _attributesByPropertyKey[_calculateKeyHash(propKey)][propValue].attributes[_calculateKeyHash(attrKey)] = attrValue;

        emit SetAttribute(propKey, propValue, attrKey, attrValue);
    }

    function _getAttribute(string memory propKey, bytes32 propValue, string memory attrKey) internal view returns (bytes32) {
