...
abstract contract ExternalSwapRouterUpgradeable is Initializable {
    ...
    address public constant PANCAKESWAP_ROUTER_ADDRESS = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
...
contract NativePoolFactory is
    ...
    ReentrancyGuardUpgradeable
{
    // bytes4(keccak256(bytes("initialize((address,address,address,address,bool,bool,uint256[],address[],address[],uint256[]),address)")));
    ...
    function createNewPool(
        ...
        address pool = address(
            new ERC1967Proxy(
                poolImplementation,
                abi.encodeWithSelector(INIT_SELECTOR, poolConfig, registry)
            )
        );
