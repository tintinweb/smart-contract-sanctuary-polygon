...
contract vesexyangel is ERC721  {
    uint256 public totalSupply;
    uint256 public cost = 0 ether;
    uint256 public maxMints = 591;
    ...
    constructor()
        ERC721("ve-sexyangel", "vesexan")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 591) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeihh5hcofkgtnuvpjnclt6v4m63ctj7ftg4v3gqnttlhwu2zpaxshy/", Strings.toString(tokenId), ".json")
            );
    }
