...
contract Multicall {
    ...
    function uniV2Swap(address pool, bool direc) public {
        ...
        uint amIn = (amOut * 997) / 1000;
        amOut = direc ? (amIn * reserve1) / (reserve0 + amIn) : (amIn * reserve0) / (reserve1 + amIn);
        transfer(direc ? IUniV2Pool(msg.sender).token0() : IUniV2Pool(msg.sender).token1(), pool, (amIn*1000)/997);
    ...
    function quoteRoute(Route calldata _route) public returns(uint){
        amOut = _route.amIn;
