...
contract LPPool is BaseLPPool {
    ...
    constructor() BaseLPPool("PELAGOLAB LP", "ASLP") {}
...
contract TokenStake is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    IERC20Upgradeable public pelago;
    ...
    uint public rewardPerBlock; //for pelago reward
    ...
    function initialize(IERC20Upgradeable pelago_,
        uint rewardPerBlock_) external initializer {
        __PelagoFarming_init(pelago_, rewardPerBlock_);
    }

    function __PelagoFarming_init(IERC20Upgradeable pelago_,
        uint rewardPerBlock_) internal  {
        __Ownable_init();
        __Pausable_init_unchained();
        require(address(pelago_) != address(0),"pelago_ address is null");
    ...
    function depositLP(uint _pid, uint _amount) external virtual onlyEOA whenNotPaused {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                pelago.safeTransfer(_msgSender(), pending);
            }
        }
        if (_amount > 0) {
    ...
    function updateLPPool(uint _pid) public whenNotPaused {
        ...
        uint pelagoReward = multiplier * rewardPerBlock * lpPool.allocPoint / totalLPAllocPoint;
        lpPool.accRewardPerShare += pelagoReward * 1e12 / lpSupply;
        lpPool.lastRewardBlock = block.number;
    }
    ...
    function pendingLPReward(uint _pid, address _user) public view returns (uint[] memory pending) {
        ...
        if (block.number > lpPool.lastRewardBlock && lpSupply != 0) {
            uint multiplier = block.number - lpPool.lastRewardBlock;
            uint pelagoReward = multiplier * rewardPerBlock * lpPool.allocPoint / totalLPAllocPoint;
            accRewardPerShare = accRewardPerShare + (pelagoReward * 1e12 / lpSupply);
        }
        pending = new uint[](1);
    ...
    function setPelagoAddress(IERC20Upgradeable token_) onlyOwner external {
