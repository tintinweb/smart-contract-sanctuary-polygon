...
contract AirdropOperatorDeployer {

    constructor() public { }

    event OperatorDeployed(address operator, address owner, address token, address lp);

    function deployAirdropOperator(address _token, address _lp) external returns (address addr){
        AirdropOperator operator = new AirdropOperator(msg.sender, _token, _lp);

        emit OperatorDeployed(address(operator), msg.sender, _token, _lp);
