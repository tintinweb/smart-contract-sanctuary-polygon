...
contract MultiAssetTreasury is IMultiAssetTreasury, Operator, ReentrancyGuard {
    ...
    ICollateralPolicy public collateralPolicy; // Store Collateral reserve
    address public daoFund; // Dao Fund smart contract
    IAssetController public assetController; // Smart-Contract to add/update Synthetic Assets
    ...
    modifier onlyAssetControllerOrOperator() {
        require(msg.sender == address(assetController) || msg.sender == operator(), "!AssetController");
        _;
    }
    ...
    function initializing(
        ...
        daoFund = _daoFund;
        assetController = IAssetController(_assetController);
    ...
    function assetPrice(uint256 _assetId) public view returns (uint256) {
        return assetController.getAssetPrice(_assetId);
    }

    function sharePrice() public view returns (uint256) {
        return assetController.getXSharePrice();
    }
    ...
    function info(uint256 _assetId) external view override returns (
        ...
        _assetSupply = assetController.getAssetTotalSupply(_assetId);
        _tcr = collateralPolicy.assetTcr(_assetId);
        _ecr = collateralPolicy.assetEcr(_assetId);
    ...
    function globalCollateralValue() public view returns (uint256 _global_collateral_value) {
        uint256 _assetCount = assetController.assetCount();
        _global_collateral_value = 0;
        for (uint256 aid = 0; aid < _assetCount; aid++) {
            uint256 collateral_price = assetController.getCollateralPriceInDollar(aid);
    ...
    function globalCollateralBalance(uint256 _assetId) public view override returns (uint256) {
        address _collateral = assetController.getCollateral(_assetId);
    ...
    function collateralValue(uint256 _assetId) public view override returns (uint256) {
        uint256 _missing_decimals = collateralPolicy.getMissingDecimal(_assetId);
        return
    ...
    function excessCollateralBalance(uint256 _assetId) public view returns (uint256 _excess) {
        uint256 _tcr = collateralPolicy.assetTcr(_assetId);
        uint256 _ecr = collateralPolicy.assetEcr(_assetId);
        if (_ecr <= _tcr) {
            _excess = 0;
        } else {
    ...
    function calcCollateralBalance(uint256 _assetId) public view returns (uint256 _collateral_value, bool _exceeded) {
        ...
        uint256 asset_total_supply = assetController.getAssetTotalSupply(_assetId);
        uint256 target_collateral_value = asset_total_supply.mul(collateralPolicy.assetTcr(_assetId)).div(PRICE_PRECISION);
    ...
    function calcCollateralValue(uint256 _collateral_amount, uint256 _assetId) internal view returns (uint256 _collateral_value) {
        uint256 _missing_decimals = collateralPolicy.getMissingDecimal(_assetId);
        bool isStable = assetController.isAssetStable(_assetId);
        _collateral_value = _collateral_amount.mul(10 ** _missing_decimals);

        if (!isStable) {
            uint256 _collateral_exchange_price = assetController.getCollateralPriceInDollar(_assetId);
    ...
    function setCollateralPolicy(address _collateralPolicy) public onlyOperator {
        require(_collateralPolicy != address(0), "invalidAddress");
        collateralPolicy = ICollateralPolicy(_collateralPolicy);
    }
