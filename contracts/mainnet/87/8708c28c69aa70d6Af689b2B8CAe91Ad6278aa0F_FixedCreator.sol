...
contract FixedVesting{
     ...
     */
    function vestingRunning() public view returns(uint8 round, uint16 totalPercent_d2){
        uint16 total;
    ...
    function claimToken() external {
        (uint8 round, uint16 totalPercent_d2) = vestingRunning();
        
        require(!isPaused && round > 0 && token != address(0) && IERC20(token).balanceOf(address(this)) > 0, "!started");
        require(invoice[msg.sender].purchased > 0, "!buyer");
        require(invoice[msg.sender].completed_d2 < totalPercent_d2, "claimed");
        
        uint128 amountToClaim;
        if(invoice[msg.sender].completed_d2 == 0){
            amountToClaim = (invoice[msg.sender].purchased * totalPercent_d2) / 10000;
        } else{
            amountToClaim = ((invoice[msg.sender].claimed * totalPercent_d2) / invoice[msg.sender].completed_d2) - invoice[msg.sender].claimed;
        }
     ...
     */
    function updateVestingDatetimes(uint8[] calldata _vestingRound, uint32[] calldata _newDatetime) external onlyOwner {
        require(_vestingRound.length == _newDatetime.length, "!good");

        (uint8 round, ) = vestingRunning();
     ...
     */
    function updateVestingRatios(uint8[] calldata _vestingRound, uint16[] calldata _newRatio_d2) external onlyOwner {
    ...
    function removeLastVestingRound() external onlyOwner {
        delete vesting[vestingLength-1];
