...
contract CurveEurtUsdYA is BaseYA, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  uint256 constant private COMPOUND_TRI_THRESHOLD = 1e16;
  ...
  address constant private CURVE_POOL = 0x225FB4176f0E20CDb66b4a3DF70CA3063281E855;
  address constant private CURVE_POOL_BASE = 0x445FE580eF8d70FF569aB36e80c647af338db351;
  address constant private CURVE_EURT_3CRV_TOKEN = 0x600743B1d8A96438bD46836fD34977a00293f6Aa;
  address constant private CURVE_EURT_3CRVG_TOKEN = 0x8b397084699Cc64E429F610F81Fac13bf061ef55;
  address constant private REWARD_POOL = 0x8b397084699Cc64E429F610F81Fac13bf061ef55;
  ...
  function getContractBalance() public view override returns (uint256) {
    return IERC20(CURVE_EURT_3CRVG_TOKEN).balanceOf(address(this)) - pendingTriFees;
  }
  ...
  function depositCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    amountOut = ICurveCryptoFourWaySwap(CURVE_POOL).calc_token_amount([0,0,value - fap,0]);
    priceImpact = 0;
  }
  ...
  function depositGiddyCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    amountOut = ICurveCryptoFourWaySwap(CURVE_POOL).calc_token_amount([0,0,amountOut,0]);
  }
  ...
  function withdrawCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    amountOut = ICurveCryptoFourWaySwap(CURVE_POOL).calc_withdraw_one_coin(sharesToValue(shares), 2) - fap;
    priceImpact = 0;
  }
  ...
  function withdrawGiddyCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    amountOut = ICurveCryptoFourWaySwap(CURVE_POOL).calc_withdraw_one_coin(sharesToValue(shares), 2);
  ...
  function join(uint256 usdcValue) private returns (uint256 shares) {
    ...
    ICurveCryptoFourWaySwap(CURVE_POOL).add_liquidity([0,0,usdcValue,0], 0);
    uint256 triValue = IERC20(CURVE_EURT_3CRV_TOKEN).balanceOf(address(this));
    shares = contractShares == 0 ? triValue * INIT_SHARES : triValue * contractShares / getContractBalance();
    if (!IERC20(CURVE_EURT_3CRV_TOKEN).approve(REWARD_POOL, triValue)) {
      revert();
    }
  ...
  function leave(uint256 shares) private returns (uint256 usdcValue) {
    ...
    if (!IERC20(CURVE_EURT_3CRV_TOKEN).approve(CURVE_POOL, value)) {
      revert();
    }
    ICurveCryptoFourWaySwap(CURVE_POOL).remove_liquidity_one_coin(value, 2, 0);
  ...
  modifier deductEarningsFee() {
    ...
    if (earnings > 0) {
      ...
      if (pendingTriFees >= COMPOUND_TRI_THRESHOLD) {
        ...
        if (!IERC20(CURVE_EURT_3CRV_TOKEN).approve(CURVE_POOL, pendingTriFees)) {
          revert(); 
        }
        ICurveCryptoFourWaySwap(CURVE_POOL).remove_liquidity_one_coin(pendingTriFees, 2, 0);
      ...
      if (value >= COMPOUND_CRV_THRESHOLD) {
        ...
        ICurveCryptoFourWaySwap(CURVE_POOL).add_liquidity([0,0,value,0], 0);

        value = IERC20(CURVE_EURT_3CRV_TOKEN).balanceOf(address(this));
        if (!IERC20(CURVE_EURT_3CRV_TOKEN).approve(REWARD_POOL, value)) {
          revert();
        }
