...
abstract contract OwnableUpgradeable is /* Initializable, */ ContextUpgradeable {
     ...
     */
    function __Ownable_init(address _admin_) internal onlyInitializing {
        __Ownable_init_unchained(_admin_);
    }

    function __Ownable_init_unchained(address _admin_) internal onlyInitializing {
...
interface IERC721Mintable {
    ...
    function router() external view returns (IRouter router);
    ...
    OwnableUpgradeable
{
    ...
    function toggleOpenForAll() external authorized {
        openForAll = !openForAll;
    }

    function whitelistUsers(address[] memory users) external authorized {
        for (uint256 index = 0; index < users.length; index++) {
            whitelistedUsers[users[index]] = true;
        }
    }

    function removeWhitelistUsers(address[] memory users) external authorized {
        for (uint256 index = 0; index < users.length; index++) {
    ...
    function setRoyaltyLimit(uint96 _royaltyLimit) external authorized {
        require(
            _royaltyLimit <= _feeDenominator(),
            "ERC721Base: Royalty limit must be below 100%"
        );
    ...
    function setBaseURI(string memory _baseURI_) external authorized {
        baseURI_ = _baseURI_;
    }

    function setContractURI(string memory _contractURI) external authorized {
        contractURI = _contractURI;
    }
    ...
    function withdrawERC20(address _erc20) external authorized {
...
contract ERC721Mintable is UUPSUpgradeable, ERC721BaseUpgradeable {
    using SharedDefinitions for SharedDefinitions.ERC721MintableParams;
    // mintingCharges in wei, Will be controlled by authorized wallets
    ...
    // Method to set minting charges per NFT
    function setBrokerage(uint256 _brokerage) external authorized {
        require(
    ...
    function setRouter(IRouter _router) external authorized {
        router = _router;
    }
  ...
  //  }
    ...
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor(
        SharedDefinitions.ERC721MintableParams memory params
    ) initializer {
        __ERC721Base_init(
        params.name,
        params.symbol,
    ...
    _mintingCharge = params.mintingCharge;
    chargeInETH = params.chargeInETH;
    openForAll = true;
    brokerage = params.brokerage;
    router = params.router;
    WETH = params.WETH;
    USDC = params.USDC;
    paths.push(params.USDC);
    paths.push(params.WETH);
    authorizations[params.AADMIN] = true;
    }
...
contract ERC721Factory {
     ...
     */
    constructor(/*  address payable _mintableToken, address _admin */) {
        /*  mintableToken = _mintableToken;
        ADMIN = _admin; */ 
    }

    function initialize(
        address payable _mintableToken, address _admin
    ) external {
        mintableToken = _mintableToken;
        ADMIN = _admin;
    }

    function setMintableAddress(address payable _mintableToken) external {
     ...
     * @param params.name Display name for collection contract
     * @param params.symbol Symbol for collection contract
     * @param params.contractURI Collection description URI which contains display image and other necessery details.
     * @param params.tokenURIPrefix prefix for tokenURI of NFT contract
     */
    function createCollection(
        CollectionParams memory params
    ) public returns (address collectionAddress){
        SharedDefinitions.ERC721MintableParams memory params721m = SharedDefinitions.ERC721MintableParams(
            params.name,
            params.symbol, 
            '', 
            '',
            params.royaltyLimit,
            IERC721Mintable(mintableToken).mintingCharge(),
            false,
            IERC721Mintable(mintableToken).brokerage(),
            IERC721Mintable(mintableToken).router(),
            IERC721Mintable(mintableToken).WETH(),
            IERC721Mintable(mintableToken).USDC(),
            ADMIN
        );
        
        // Create a new instance of the Collection contract
        ERC721Mintable collection = new ERC721Mintable(params721m);
        ...
        _collectionCreators[collectionAddress] = msg.sender;
    ...
    function getCollectionCreator(address _collection) public view returns (address) {
        return _collectionCreators[_collection];
    }
