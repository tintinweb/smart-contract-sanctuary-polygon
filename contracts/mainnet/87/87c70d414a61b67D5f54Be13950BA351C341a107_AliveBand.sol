...
contract AliveBand is Initializable, AccessControlUpgradeable {
    ...
    function createDutchSong(
        ...
        string memory _contractURI,
        address _artist,
        uint256[8] memory _songInfos ) public onlyGovernor returns (address) {
        uint256 songId = allSongs.length + 1;
        bytes memory data = abi.encodeWithSelector(
...
contract AliveRegistry is Initializable, ProxyAdmin {
    ...
    mapping(address => bool) public isAliveAlbum;
    ...
    function initialize(
        ...
        address _admin,
        IERC20 usdcAdd, 
        AggregatorV3Interface aggregatorUsdcToNative
    ) public initializer {
        ...
        usdcAddress = usdcAdd;
        priceFeed = aggregatorUsdcToNative;
 ...
 function convertToNative(uint256 usdcAmountToConvert)
        public
        view
        returns (uint256)
    {
        ...
        return uint256(usdcAmountToConvert* 10e7/uint(answer));
    }
 function convertToUSD(uint256 nativeAmountToConvert)
        public
        view
        returns (uint256)
    {
        ...
        return uint256(nativeAmountToConvert* uint(answer)/10e7);
    }
    ...
    function addAliveAlbums(address[] calldata _newAliveAlbum)
        external
        onlyAdmin
    {
        for (uint256 i = 0; i < _newAliveAlbum.length; i++) {
            isAliveAlbum[_newAliveAlbum[i]] = true;
        }
        // emit AddManagers(_newManagerWallets);
    }

    function removeAliveAlbums(address[] calldata _newAliveAlbum)
        external
        onlyAdmin
    {
        for (uint256 i = 0; i < _newAliveAlbum.length; i++) {
            isAliveAlbum[_newAliveAlbum[i]] = false;
        }
        // emit RemoveManagers(_removeManagerWallets);
    }
...
contract AliveBondedSong is AliveCommonSong {
    ...
    //     init_cantBeEvil(_licenseVersion);
...
function getMaxMintAmount() public view virtual returns(uint){
 ...
 return maxMintAmount;
    ...
    function _onlyAdmin() internal view{
           require(
            AliveRegistry(aliveRegistry).isManagerWallet(msg.sender) == true, "only manager"
        );
    }
 
   function _onlyAlbum() internal view {
    
          require(
            AliveRegistry(aliveRegistry).isAliveAlbum(msg.sender) == true ,
            "only album"
        );
    }
...
contract AliveDutchSong is AliveCommonSong {
    ...
    function getPrice(uint256 _quantity) public virtual view returns (uint256 price) {
    ...
    {  require(block.timestamp > startedAt);
        require(
            _quantity <
                AliveRegistry(aliveRegistry).getAliveSongMaxMintPerTxByType(
                    songType),
            "You can't mint Songs more than Limit per mint!"
        );
        require(
            totalSupply() + _quantity <= maxMintAmount,
            "Can't mint more than Limit!"
...
contract AliveFixedSong is AliveCommonSong {
    ...
    uint256 public initialPrice;
    uint256 public startedAt;////
    mapping(address => uint) public priceUser;
    mapping (address => bool) public isWhiteListed; // is wl?
    
function whitelistBatch(uint price, address[] memory _wlAdd) public onlyAdmin {
  unchecked {
    for (uint i = 0; i < _wlAdd.length; i++) {
        isWhiteListed[_wlAdd[i]] = true;
        priceUser[_wlAdd[i]] = price;
    }
  }
}
...
function removeWhitelistBatch(address[] memory _wlRem) public onlyAdmin {
 ...
 function getPrice(uint256 _quantity, address _to) virtual public view returns (uint256 price) {
     if(isWhiteListed[_to]) {
         return priceUser[_to]*_quantity;
     }
     else {
...
contract AliveBondedSongV2 is AliveBondedSong {
        ...
        initializerERC721A
    {
        ...
        // init_cantBeEvil(_licenseVersion);
        band = msg.sender;
        usdc = AliveRegistry(aliveRegistry).usdcAddress();
        ...
        maxMintAmount = _songInfos[2];
        initialPrice = _songInfos[3];
        currentPrice = _songInfos[3];
        upcountRate = _songInfos[4];
        startedAt = _songInfos[5];
        mintStopTime = _songInfos[6];

        aliveRoyaltyFee = AliveRegistry(aliveRegistry).feePercent();
        ...
        if (bal == 0 && price != 0) {
            if (isUSDCMint) {
                require(
                    ...
                    "Insufficient allowance of usdc!"
                );
                ...
                revert(
                    "Does Not Support USDC Mints, inssuficent matic value passed"
                );
            }
        } else {
            uint256 priceInMatic = getPriceMatic(_quantity, _to);
            require(msg.value >= priceInMatic, "Insufficient funds in matic!");
            if (msg.value > priceInMatic) {
                payable(msg.sender).transfer(msg.value - priceInMatic);
            }
            uint256 feePercent = aliveRoyaltyFee;
            uint256 feePrice = (priceInMatic * feePercent) / 100;
            payable(AliveRegistry(aliveRegistry).aliveHouseRoyaltyReceiver())
                .transfer(feePrice);
            payable(mintRoyaltyReceiver).transfer(priceInMatic - feePrice);
    ...
    function changeFeePercentage(uint8 _feePercent) public onlyAdmin {
        aliveRoyaltyFee = _feePercent;
    }

    function getNFTData(uint256 amount, address _of)
        public
        view
        returns (
            uint256 _price,
        ...
        )
    {
        return (
            getPrice(amount),
            AliveRegistry(aliveRegistry).convertToNative(getPrice(amount)),
            totalSupply(),
            getMaxMintAmount(),
            upcountRate,
            balanceOf(_of),
        ...
        returns (uint256 totalPrice)
    {
        ...
        if (isUSDDominant) {
            ...
            return AliveRegistry(aliveRegistry).convertToUSD(price);
        }
    }

    function getPriceMatic(uint256 _quantity, address _to)
        public
        view
        returns (uint256 price)
    {
        if (isUSDDominant) {
            return
                AliveRegistry(aliveRegistry).convertToNative(
                    getPrice(_quantity)
                );
        } else {
    ...
    function withdrawAndDistribute(address tokenAdd) public nonReentrant {
        uint256 feePercent = aliveRoyaltyFee;
        ...
        if (tokenAdd == address(0)) {
            ...
            payable(mintRoyaltyReceiver).transfer(price - feePrice);
            if (usdc.balanceOf(address(this)) >= 100) {
                usdc.transferFrom(
                    msg.sender,
                    AliveRegistry(aliveRegistry).aliveHouseRoyaltyReceiver(),
                    feePrice
                );
                usdc.transferFrom(
                    msg.sender,
            ...
            require(
                IERC20(tokenAdd).balanceOf(address(this)) >= 100,
                "Not enough to withdraw"
            );

            IERC20(tokenAdd).transferFrom(
                msg.sender,
                AliveRegistry(aliveRegistry).aliveHouseRoyaltyReceiver(),
                feePrice
            );
...
contract AliveDutchSongV2 is AliveDutchSong {
        ...
        initializerERC721A
    {
        ...
        maxMintAmount = _songInfos[2];
        ceilPrice = _songInfos[3];
        floorPrice = _songInfos[4];
        startedAt = _songInfos[5];
        discountRate = _songInfos[6];
        mintStopTime = _songInfos[7];
        ...
        nonReentrant
    {
        ...
        if (bal == 0) {
            ...
            uint256 priceInMatic = AliveRegistry(aliveRegistry).convertToNative(
                price
            );
            require(msg.value >= priceInMatic, "Insufficient funds in matic!");
            if (msg.value > priceInMatic) {
    ...
    function getNFTData(uint256 amount, address _address)
        public
        view
        returns (
            uint256 _price,
        ...
        if (block.timestamp > startedAt) {
            ...
            if (discount > ceilPrice || floorPrice > ceilPrice - discount) {
                if (isUSDDominant) {
                    ...
                    return
                        AliveRegistry(aliveRegistry).convertToUSD(
                            floorPrice * _quantity
                        );
                }
            } else {
...
contract AliveFixedSongV2 is AliveFixedSong {
        ...
        nonReentrant
    {
        ...
        require(_quantity > 0, "mint more than 0!");
        // if user tries to mint with no matic, check if usdcMint enabled, if so check user balance and price
        uint256 price = getPrice(_quantity, _to);
        uint256 bal = msg.value;
