...
interface IManageTHSC {
    ...
    function addSigner(address adminAddress) external;

    function isSigner(address adminAddress) external view returns (bool);

    function removeSigner(address adminAddress) external;
}
...
import "./interfaces/IManageTHSC.sol";

contract ManageTHSC is IManageTHSC, AccessControl {
    ...
    function changeOwner(address _newOwner) external onlyOwner {
        ...
        _revokeRole(SIGNER_ROLE, _msgSender());
        _setupRole(SIGNER_ROLE, _newOwner);
    }
    ...
    function addSigner(address _adminAddress) external onlyOwner {
        _setupRole(SIGNER_ROLE, _adminAddress);
    }

    function removeSigner(address _adminAddress) external onlyOwner {
        _revokeRole(SIGNER_ROLE, _adminAddress);
    }

    function isOwner(address _ownerAddress) external view returns (bool) {
        return hasRole(OWNER_ROLE, _ownerAddress);
    }

    function isAdmin(address _adminAddress) external view returns (bool) {
        return hasRole(ADMIN_ROLE, _adminAddress);
    }

    function isSigner(address _adminAddress) external view returns (bool) {
        return hasRole(SIGNER_ROLE, _adminAddress);
    }
}