...
/**
 * @title FuelDistributor Contract
 * @author GET Protocol
 * @notice Contract responsible distribution of spent fuel
 * @dev In production, fuel strictly refers to $GET
 *
...
import "./interfaces/IFuelDistributor.sol";
import "./interfaces/IRegistry.sol";

contract FuelDistributor is
    IFuelDistributor,
    AuthModifiers,
    OwnableUpgradeable,
    ...
    UUPSUpgradeable
{
    ...
    Destination[] public destinationsProtocol;
    Destination[] public destinationsRemainder;
    Destination[] public destinationsCredit;
     ...
     * @param _destinationsProtocol Array of Destination structs for protoocol fuel collection
     * @param _destinationsRemainder Array of Destination structs for collection of remaining fuel
     */
    // solhint-disable-next-line func-name-mixedcase
    function __FuelDistributor_init(
        address _registry,
        Destination[] calldata _destinationsProtocol,
        Destination[] calldata _destinationsRemainder,
        Destination[] calldata _destinationsCredit
    ) public initializer {
        __Ownable_init();
        __AuthModifiers_init(_registry);
        __FuelDistributor_init_unchained(_registry, _destinationsProtocol, _destinationsRemainder, _destinationsCredit);
    }

    // solhint-disable-next-line func-name-mixedcase
    function __FuelDistributor_init_unchained(
        address _registry,
        Destination[] calldata _destinationsProtocol,
        Destination[] calldata _destinationsRemainder,
        Destination[] calldata _destinationsCredit
        ...
        _setDestinationsProtocol(_destinationsProtocol);
        _setDestinationsCredit(_destinationsCredit);
        _setDestinationsRemainder(_destinationsRemainder);
    }

    /**
     * @notice Internal function to process fuel transfer
     * @param _total Total fuel to be distributed
     * @param _destinations Destination struct designating address and amount to be transferred
     */
    function _distribute(uint256 _total, Destination[] storage _destinations) internal {
        for (uint256 i = 0; i < _destinations.length; i++) {
            uint256 _amount = (_total * _destinations[i].percentage) / 1e6;
            require(
                registry.economics().fuelToken().transfer(_destinations[i].destination, _amount),
                "Economics: fuel token transfer failed"
            );
            emit Distribute(_amount, _total, _destinations[i].destination);
        }
    }

    /**
     * @notice Collects and distributes spent fuel from the Economics contract
     */
    function collect() public nonReentrant {
        (uint256 _amountTotal, uint256 _amountProtocol, uint256 _amountCredit) = registry
            .economics()
            .collectSpentFuel();
        require(_amountTotal > 0, "FuelDistributor: nothing to collect");
        uint256 _amountRemainder = _amountTotal - _amountCredit - _amountProtocol;
        _distribute(_amountProtocol, destinationsProtocol);
        _distribute(_amountCredit, destinationsCredit);
        _distribute(_amountRemainder, destinationsRemainder);
    }

    /**
     * @notice Sets the destinationsProtocol global variable
     * @dev the destinationsProtocol variable holds the destinations protocol fuel is transferred to
     * @param _destinationsProtocol value to set
     */
    function setDestinationsProtocol(Destination[] calldata _destinationsProtocol) public onlyIntegratorAdmin {
        _setDestinationsProtocol(_destinationsProtocol);
    }

    /**
     * @notice Internally sets the destinationsProtocol global variable
     * @param _destinationsProtocol value to set
     */
    function _setDestinationsProtocol(Destination[] calldata _destinationsProtocol) internal {
        emit UpdateDestinationsProtocol(destinationsProtocol, _destinationsProtocol);

        delete destinationsProtocol;
        for (uint256 i = 0; i < _destinationsProtocol.length; i++) {
            destinationsProtocol.push(_destinationsProtocol[i]);
        }
    }

    /**
     * @notice Sets the destinationsRemainder global variable
     * @dev the _destinationsRemainder variable holds the destinations remaining fuel is transferred to
     * @param _destinationsRemainder value to set
     */
    function setDestinationsRemainder(Destination[] calldata _destinationsRemainder) public onlyIntegratorAdmin {
        _setDestinationsRemainder(_destinationsRemainder);
    }

    /**
     * @notice Internally sets the destinationsRemainder global variable
     * @param _destinationsRemainder value to set
     */
    function _setDestinationsRemainder(Destination[] calldata _destinationsRemainder) internal {
        emit UpdateDestinationsRemainder(destinationsRemainder, _destinationsRemainder);

        delete destinationsRemainder;
        for (uint256 i = 0; i < _destinationsRemainder.length; i++) {
            destinationsRemainder.push(_destinationsRemainder[i]);
        }
    }

    /**
     * @notice Sets the destinationsCredit global variable
     * @dev the _destinationsCredit variable holds the destinations credit fuel is transferred to
     * @param _destinationsCredit value to set
     */
    function setDestinationsCredit(Destination[] calldata _destinationsCredit) public onlyIntegratorAdmin {
        _setDestinationsCredit(_destinationsCredit);
    }

    /**
     * @notice Internally sets the destinationsCredit global variable
     * @param _destinationsCredit value to set
     */
    function _setDestinationsCredit(Destination[] calldata _destinationsCredit) internal {
        emit UpdateDestinationsCredit(destinationsCredit, _destinationsCredit);

        delete destinationsCredit;
        for (uint256 i = 0; i < _destinationsCredit.length; i++) {
            destinationsCredit.push(_destinationsCredit[i]);
        }
    }
...
interface IFuelDistributor {
    ...
    function destinationsProtocol(uint256 _index)
        external
        returns (
            address payable,
            uint24,
    ...
    function destinationsRemainder(uint256 _index)
        external
        returns (
            address payable,
            uint24,
    ...
    function destinationsCredit(uint256 _index)
        external
        returns (
            address payable,
            uint24,
