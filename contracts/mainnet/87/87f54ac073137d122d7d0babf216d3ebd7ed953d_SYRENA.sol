...
contract SYRENA is IBEP20, Ownable {
    ...
    string private constant _symbol = "SYRENA";
    string private constant _name = "SYRENA";
    ...
    function setMinter(address account, bool _is) external onlyOwner {
        _isMinter[account] = _is;
    }
    ...
    function increaseAllowance(address s, uint256 v) external returns (bool) {
        _approve(msg.sender, s, _allowances[msg.sender][s].add(v));
        return true;
    }
    ...
    function decreaseAllowance(address s, uint256 v) external returns (bool) {
        _approve(
            msg.sender,
            s,
            _allowances[msg.sender][s].sub(v, "Decreased allowance below zero")
        );
        return true;
    }
    ...
    function _transfer(
        ...
        _balances[s] = _balances[s].sub(a, "Transfer amount exceeds balance");
