...
contract Contract is ERC721A, Ownable {
    ...
    uint256 public constant RESERVED_TOKENS = 400;
    uint256 public constant FOR_SALE_TOKENS = 1400 - RESERVED_TOKENS;
    address public constant RESERVED_TOKENS_ADDRESS =  0x26d4EC389D825d47f9d1610C5fD9CD8ef225C1c7;
    ...
    uint256 public tokenPricePresale = 400.45 ether; //400.45
    uint256 public tokenPricePublicSale = 400.45 ether; //400.45

    uint256 public maxTokensPerAddressPresale = 200;
    uint256 public maxTokensPerAddress = 200;

    uint256 public presaleTotalLimit = FOR_SALE_TOKENS;

    mapping(address => bool) public whitelisted;

    string public tokenBaseURI = "ipfs://QmUZNw4f1hYQxVMJn7444ZNEnva7AZCzQDGiuJcKLUVrt4/";

    uint256 public soldAmount = 0;
    mapping(address => uint256) public purchased;

    constructor() ERC721A("Day Trading NFT", "DTN") {
    ...
    function payout(uint256 payoutAmount) external onlyOwner {
        ...
        require(payoutAmount <= address(this).balance, "Not enough balance in contract");
        address currentOwner = _ownerships[_startTokenId()].addr;
        uint256 batchStart = _startTokenId();
        for(uint256 i = _startTokenId() + 1; i < _startTokenId() + supply; i++) {

            if (_ownerships[i].addr == address(0)) {
                continue;
            } else {
                payable(currentOwner).transfer((payoutAmount * (i - batchStart)) / supply); 
                batchStart = i;
                currentOwner = _ownerships[i].addr;
            }
        }
        payable(currentOwner).transfer((payoutAmount * (supply - batchStart)) / supply); 
    }

    function payoutInRange(uint256 payoutAmount, uint256 start, uint256 end) external onlyOwner {
        uint256 supply = totalSupply();
        require(start < end, "Start must be less than end");
        require(start >= _startTokenId(), "Start must be greater than or equal to the start token id");
        require(end < _startTokenId() + supply, "End must be less than or equal to the last token id");
        require(payoutAmount <= address(this).balance, "Not enough balance in contract");

        uint256 range = _startTokenId() + end - start;
        address currentOwner = ownerOf(start);
        uint256 batchStart = start;
        
        for(uint256 i = start + 1; i < _startTokenId() + end; i++) {
            if (_ownerships[i].addr == address(0)) {
                continue;
            } else {
                payable(currentOwner).transfer((payoutAmount * (i - batchStart)) / range); 
                batchStart = i;
                currentOwner = _ownerships[i].addr;
            }
        }
        payable(currentOwner).transfer((payoutAmount * (end - batchStart)) / range); 
    }
    ...
    function getMaxTokensForPhase(address target) public view returns (uint256) {
    ...
    function getMaxTokensAllowed(address target) public view returns (uint256) {
        uint256 maxAllowedTokens = getMaxTokensForPhase(target);
    ...
    function getContractInfo(address target) external view returns (
        ...
        _isWhitelisted = whitelisted[target];
        _totalForSale = FOR_SALE_TOKENS;
    }

    function mint(uint256 _amount, address _recipient) external payable {
        require(msg.value >= tokenPrice() * _amount, "Incorrect ETH sent");
        require(_amount <= getMaxTokensAllowed(_recipient), "Cannot mint more than the max allowed tokens");
