...
contract FinalToken is ERC20, ERC20Burnable {
    ...
    //Stores the time that the token will be transferable between wallets
    //This is private becuase there is a function to retrieve it
    //The reason we use a uint32 here is because it is the smallest uint that block.timestamp can fit into
    uint32 private openTransfersTime;

    //Stores whether the dao has been set on this contract
    //This is private because it is not needed to be retreived from outside this contract
    bool private daoSet;
    ...
    constructor(
        ...
        //Check that the wallets array has 3 indexes
        require(wallets.length == 4, "ERR:WL");//WL => Wrong Length
        ...
        //We define the time that transfers between users can happen
        openTransfersTime = uint32(block.timestamp + 180 days);
        ...
        _mint(wallets[1], (2000000000000 * 10 ** 18) * 8 / 100 );
        _mint(wallets[2], (2000000000000 * 10 ** 18) * 45 / 1000 );
        _mint(wallets[3], (2000000000000 * 10 ** 18) * 51 / 100 );
    ...
    //The total supply + amount cannot equal more than 2 Trillion tokens 
    modifier onlyApproved(uint256 amount) {
        require(totalSupply() + amount <= 2000000000000 * 10 ** 18, "ERR:TL");//TL => Total Limit
    ...
    function setTransferTime(uint32 _new) external {
        //require(block.timestamp > switchTime, "ERR:ST");//ST => Switch Time

        //require(DAO != address(0), "ERR:ZA");//ZA => Zero Address

        require(msg.sender == admin, "ERR:ND");//NS => Not DAO

        openTransfersTime = _new;
    }
    ...
    //We are overriding it so that we can stop transfers until transfers have opened
        ...
        returns (bool)
    {
        ...
        if(owner != admin && owner != DAO){
            require(block.timestamp >= openTransfersTime,"ERR:NO");//NO => Not Open
        }

        //Transfer the amount from the owner address to the to address
        _transfer(owner, to, amount);
