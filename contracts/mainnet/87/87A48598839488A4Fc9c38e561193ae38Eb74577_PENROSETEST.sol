...
// **********added pair interface for price calculation
...
//can't add initial liquidity through the smart contract with swapAndLiquify on.
contract PENROSETEST is ERC20, Ownable {
    ...
    uint256 public liquidityPercent = 50; // 33% of total collected tax
    uint256 public marketingPercent = 30; // 34% of total collected tax
    uint256 public devPercent = 20; // 33% of total collected tax

    // **** ath amount and timestamp trackers from the original DCA contract, non essential feature, can easily remove without breaking the code
    uint256 public ath;
    uint256 public athTimestamp;

    address payable public marketingWallet =
        payable(0x944BFE4ca3e585bd9d3A8b6ed879756f37eE8016);
    address payable public devWallet =
        payable(0x8089b7a835768FcCc568EE506722d8cd191A2aa5);
    ...
    constructor() ERC20("PENROSETEST", "PENROSETEST") {
        ...
        setWhitelist(address(uniswapV2Router), true);
    ...
    // ****added modifier from restricting transfers between wallets
    ...
    function setWhitelist(address target, bool value) public {
        require(msg.sender == owner() || msg.sender == address(this));
        isWhitelisted[target] = value;
        emit Whitelisted(target, value);
    }
        ...
        returns (bool)
    {
        ...
        // ***must update whitelist for router change
        setWhitelist(address(uniswapV2Router), false);
        setWhitelist(newAddress, true);
    ...
    // **********added explicit transfer and transferFrom overrides to add onlyDex modifier
    function transfer(address recipient, uint256 amount)
        public
        override
        onlyDex(recipient)
    ...
    function transferFrom(
        ...
        address recipient,
        uint256 amount
    ) public override onlyDex(recipient) returns (bool) {
        _transfer(sender, recipient, amount);
    ...
    // **************added price calculator for DCA
    ...
    function _transfer(
        ...
        // ****** this is where DCA logic is implemented
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if (automatedMarketMakerPairs[to]) {
                ...
                require(
                    dollarCostAverage[from] <= tokenPrice,
                    "DCA too high to sell"
                );
            }
        }
    ...
    function setMaxSellTransaction(uint256 _maxSellTxAmount) public onlyOwner {
        ...
        require(
            maxSellTransactionAmount > totalSupply().div(1000),
            "value is too low"
        );
    }
    ...
    function setMaxBuyTransaction(uint256 _maxBuyTxAmount) public onlyOwner {
        ...
        require(
            maxBuyTransactionAmount > totalSupply().div(1000),
            "value is too low"
        );
    }
    ...
    // for quicker testing, remove before mainnet launch
