...
contract MiladyRaveMaker is ERC721A, ERC721AQueryable, Ownable {
    ...
    constructor() ERC721A("MiladyRave", "MIR") {
        ...
        miladyPrice = 0.03 ether;
        maxSupply = 5000;
        paused = true; // Must be paused at first.
    }
    ...
    function mint(uint256 quantity, bytes calldata signature) external payable {
        ...
        uint256 mintPrice = publicPrice;

        // If there is a non-empty signature provided,
        // it means that the minter is trying to do a discounted mint.
        if (signature.length > 1) {
            ...
            mintPrice = miladyPrice;
        }

        require(msg.value == mintPrice * quantity, "Wrong Ether value.");
    ...
    function _checkMint(uint256 quantity) private view {
        unchecked {
            ...
            require(quantity < type(uint16).max, "Overflow.");
