...
contract Stake is Ownable {
    ...
    address public solisiumVaultAddress;

    ITokenMint private tokenMint;
    IToken private solisiumToken;
    ISolisiumVault private solisiumVault;
    ...
    uint256 private  minimumInitial = 10;
    uint256 private minimumAmount = 1;

    uint256 public deposit_bracket_size = 10000e18;     // @BB 5% increase whale tax per 10000 tokens... 10 below cuts it at 50% since 5 * 10
    uint256 public max_payout_cap = 100000e18;          // 100k SLSM or 10% of supply
    ...
    constructor(address _mintAddress, address _solisiumTokenAddress, address _fountainAddress, address _vaultAddress) public Ownable() {

        //Only the mint should own its paired token
        tokenMint = ITokenMint(_mintAddress);

        //Solisium
        solisiumToken = IToken(_solisiumTokenAddress);

        //ISolisiumVault
        solisiumVaultAddress = _vaultAddress;
        solisiumVault = ISolisiumVault(_vaultAddress);
    ...
    //@dev Default payable is empty since Stake executes trades and recieves MATIC
    function() external payable  {
        //Do nothing, MATIC will be sent to contract when selling tokens
    }
    ...
    //@dev Deposit specified SLSM amount supplying an upline referral
    function deposit(address _upline, uint256 _amount) external {

        address _addr = msg.sender;

        (uint256 realizedDeposit, uint256 taxAmount) = solisiumToken.calculateTransferTaxes(_addr, _amount);
        uint256 _total_amount = realizedDeposit;

        //Checkin for custody management.  If a user rolls for themselves they are active
        checkin();

        require(_amount >= minimumAmount, "Minimum deposit of 1 SLSM");
        ...
        //Transfer SLSM to the contract
        require(
            solisiumToken.transferFrom(
                _addr,
                address(solisiumVaultAddress),
                _amount
            ),
            "SLSM token transfer failed"
        );
        /*
        User deposits 10;
    ...
    function _claim_out(address _addr) internal {
        ...
        uint256 vaultBalance = solisiumToken.balanceOf(solisiumVaultAddress);
        if (vaultBalance < to_payout) {
            uint256 differenceToMint = to_payout.sub(vaultBalance);
            tokenMint.mint(solisiumVaultAddress, differenceToMint);
        }

        solisiumVault.withdraw(to_payout);

        uint256 realizedPayout = to_payout.mul(SafeMath.sub(100, ExitTax)).div(100); // 10% tax on withdraw
        require(solisiumToken.transfer(address(msg.sender), realizedPayout));
    ...
    function contractInfo() external view returns(uint256 _total_users, uint256 _total_deposited, uint256 _total_withdraw, uint256 _total_matic, uint256 _total_txs, uint256 _total_airdrops) {
        return (total_users, total_deposited, total_withdraw, total_matic, total_txs, total_airdrops);
    }

    /////// Airdrops ///////


    function airdrop(address[] _to, uint256 _amountPerUser) external pure {

    }



    //@dev Send specified SLSM amount supplying an upline referral
    function airdrop(address _to, uint256 _amount) external {

        address _addr = msg.sender;

        (uint256 _realizedAmount, uint256 taxAmount) = solisiumToken.calculateTransferTaxes(_addr, _amount);
        ...
        require(
            ...
            "SLSM to contract transfer failed; check balance and allowance, airdrop"
        );
