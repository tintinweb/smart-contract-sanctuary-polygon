...
contract APunks is Ownable, ERC721Enumerable {
    ...
    // Public constants
    uint256 public constant BUYABLE_SUPPLY = 15;
    uint256 public constant MAX_NFTS = 30;
    ...
    uint256 public punkReserve = 100;
    ...
    constructor() ERC721("A Punks", "APNK") {
    ...
    function mintFree() public {
     ...
     function crossmint(address _to, uint256 quantity) public payable {
        ...
        if (remaining > 0) {
            (bool success, ) = _to.call{value: remaining}("");
            require(success);
        }
    ...
    function setFrame(IERC721 _external_nft, uint256 _content, uint256 _punk, uint256 _frame_index)
        external
    {
        require(msg.sender == ownerOf(_punk), "Must Own punk to Set Frames");
        ...
        frames[_punk][_frame_index] = Frame(_external_nft, _content);
    ...
    function reservePunks(address _to, uint256 _reserveAmount) public onlyOwner {
        require(_reserveAmount > 0 && _reserveAmount <= punkReserve, "Not Enough reserves left for team");
