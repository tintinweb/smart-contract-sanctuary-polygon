...
import {ICommunityPassV1} from 'medallion/ICommunityPassV1.sol';
...
import {INFTMetadataV1} from 'medallion/brevity/token/INFTMetadataV1.sol';
    ...
    IRoyaltySubscriberV1
{
    ...
    /// @dev Reverts attempt to issue duplicate pass to recipient
    /// @dev DEPRECATED: use `issue(address)` instead
    ...
    function privateIssueToRecipient(address recipient)
        ...
        returns (uint256)
    {
        (uint256 tokenId, bool wasIssued) = issue(recipient);
        require(wasIssued, 'CommunityPass: duplicate');

        return tokenId;
    }

    /// @inheritdoc ICommunityPassV1
    function issue(address recipient) public onlyRole(DELEGATE_ISSUER_ROLE) returns (uint256, bool) {
        if (balanceOf(recipient) > 0) {
            return (tokenOfOwnerByIndex(recipient, 0), false);
        }

        return (_issueTo(recipient), true);
    }
    ...
    function setRoyaltyProvider(address newRoyaltyProvider) public onlyRole(DELEGATE_ADMIN_ROLE) {
        if (newRoyaltyProvider != address(0)) {
            require(
                IERC165(newRoyaltyProvider).supportsInterface(type(IRoyaltyProviderV1).interfaceId),
                "CommunityPass: invalid royalty provider"
            );
        }
    ...
    function issuedAt(uint256 tokenId) public view returns (uint256) {
        require(numTransfers(tokenId) > 0, 'CommunityPass: out of bounds');
    ...
    function transferredAt(uint256 tokenId, int256 index) public view returns (uint256) {
        require(index >= -1, 'CommunityPass: out of bounds');
        require(numTransfers(tokenId) > 0, 'CommunityPass: out of bounds');
        ...
        require(uIndex < _transferredAt[tokenId].length, 'CommunityPass: out of bounds');
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(AccessControlEnumerableUpgradeable, ERC721Upgradeable, ERC721EnumerableUpgradeable, IERC165)
        returns (bool)
    {
        return
            interfaceId == type(ICommunityPassV1).interfaceId ||
...
interface ICommunityPassV1 {
    /// @notice Issues a new token
    /// @dev Duplicate calls succeed returning the previously issued token id
    /// @param recipient The token recipient
    /// @return uint256 The new tokenId
    /// @return bool Whether then token was created
    function issue(address recipient) external returns (uint256, bool);
}
