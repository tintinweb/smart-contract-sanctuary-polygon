...
contract SwapAxiomsToDMagic is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    address private depositToken = 0x01fA5b3A5d77BcF705DD505bBcBb34bce310E7FE;  // AXIOMS
    ...
    function depositTokens(uint256 _amount) external nonReentrant {
        ...
        // send all axiom tokens. No conversion
        if (outputAddress != address(0)) {
            IERC20(depositToken).transfer(outputAddress, IERC20(depositToken).balanceOf(address(this)));
        }
    ...
    function recoverChainTokenInEmergency() external onlyOwner nonReentrant {
