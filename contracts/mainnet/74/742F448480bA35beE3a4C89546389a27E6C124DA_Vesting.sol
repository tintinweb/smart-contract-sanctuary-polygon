...
contract Vesting is
    ...
    function returnVesting(address _vestingContractAddress, address _returnAddress, uint256 _amount) external whenNotPaused nonReentrant onlyOwner {
        require(isVesting[_vestingContractAddress], "The address is not a vesting deposit");
        VestingDeposit vestingDeposit = VestingDeposit(_vestingContractAddress);        
        vestingDeposit.returnVesting(_returnAddress, _amount);
    }
...
contract VestingDeposit is ReentrancyGuard {
    ...
    function returnVesting(address _returnAddress, uint256 _amount) external  nonReentrant onlyVestingContract {
        token.transfer(_returnAddress, _amount);
    }
