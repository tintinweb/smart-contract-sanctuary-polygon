    ...
    IERC2771Recipient
{
    ...
    /// @param signers An array containing the addresses of the signers of the agreement
    /// @param dynamicData A string containing CID
    /// @param nonce An integer representing nonce value
    /// @param chainId An integer representing the chainId
    struct Agreement {
        string contractName;
        uint128 id;
        address offerer;
    ...
    /// @notice Signatures struct to manage signatures related to an agreement
    /// @param offerer The bytes representing the offerer's signature
    /// @param promisor The bytes representing the promisor's signature
    struct Signatures {
        bytes offerer;
        bytes promisor;
    }
    ...
    /// @notice A mapping to keep track of used nonces for each address
    ...
    /// @notice A mapping to store all signatures by their hash
    mapping(bytes32 => Signatures) public signatures;
    ...
    event OffererUpdated(
        ...
        address indexed oldOfferer,
        address indexed newOfferer
    );
    ...
    event TermAdded(bytes32 indexed agreementHash, address indexed term);
    ...
    event TermRemoved(
        bytes32 indexed agreementHash,
        address indexed removedTerm
    );

    /// @notice Creates a new agreement
    /// @dev This function requires a valid notery signature and unused nonces. It also verifies the offchain agreement hash.
    /// @param agreement The agreement details
    /// @param _agreementHash The hash of the agreement
    /// @param noterySignature The notery's signature
    ...
    function createAgreement(
        ...
        bytes memory noterySignature,
        uint256 nonce,
        bytes memory offererSignature,
        bytes memory promisorSignature
    ) external {
        bytes32 message = keccak256(
            abi.encodePacked(_agreementHash, nonce, "createAgreement")
        );
        require(
            verifySigner(verifyingSigner, message, noterySignature),
            "Invalid notery signature"
        );
        require(!usedNonces[verifyingSigner][nonce], "Nonce already used");

        bytes32 agreementHash = keccak256(abi.encode(agreement));
        require(_agreementHash == agreementHash, "offchain hash invalid");
        require(agreements[agreementHash].promisor == address(0), "agreement already exists");

        usedNonces[verifyingSigner][nonce] = true;
        usedNonces[agreement.promisor][agreement.nonce] = true;
        agreementsStartDate[agreementHash] = block.timestamp;

        // Store the pending agreement
        agreements[agreementHash] = agreement;
        signatures[agreementHash] = Signatures(
            offererSignature,
            promisorSignature
        );
    ...
    /// @notice Adds a new term to an existing agreement
    /// @dev This function requires a valid notery signature and unused nonces. It also verifies the existence of the agreement.
    /// @param agreementHash The hash of the agreement to which a new term is being added
    /// @param term The address of the new term
    /// @param noterySignature The notery's signature
    /// @param nonce The nonce related to this operation
    function addTerm(
        ...
        bytes32 message = keccak256(
            abi.encodePacked(agreementHash, nonce, "addTerm")
        );
        require(
            verifySigner(verifyingSigner, message, noterySignature),
            "Invalid notery signature"
        ...
        agreement.terms.push(term);

        // Emit an event for the term added
        emit TermAdded(agreementHash, term);
    }

    /// @notice Removes a term from an existing agreement
    /// @dev This function requires a valid notery signature and unused nonces. It also verifies the existence of the agreement and that the index is within bounds.
    /// @param agreementHash The hash of the agreement from which a term is being removed
    ...
    function removeTerm(
        ...
        bytes32 message = keccak256(
            abi.encodePacked(agreementHash, nonce, "removeTerm")
        );
        require(
            verifySigner(verifyingSigner, message, noterySignature),
            "Invalid notery signature"
        ...
        require(index < agreement.terms.length, "Invalid index");

        usedNonces[verifyingSigner][nonce] = true;

        // Remove the term at the specified index
        address removedTerm = agreement.terms[index];
        agreement.terms[index] = agreement.terms[agreement.terms.length - 1];
        agreement.terms.pop();

        // Emit an event for the term removed
        emit TermRemoved(agreementHash, removedTerm);
    }
    ...
    /// @param noterySignature The notery's signature
    /// @param nonce The nonce related to this operation
    /// @param offererSignature The new offerer's signature
    function updateofferer(
        ...
        bytes32 message = keccak256(
            abi.encodePacked(agreementHash, nonce, "updateOfferer")
        );
        require(
            verifySigner(verifyingSigner, message, noterySignature),
            "Invalid notery signature"
        ...
        // Ensure that the agreement exists
        require(agreement.offerer != address(0), "Agreement does not exist");
        ...
        signatures[agreementHash].offerer = offererSignature;

        // Emit an event for the offerer update
        emit OffererUpdated(agreementHash, oldOfferer, newOfferer);
    }

    /// @notice Nullifies an existing agreement
    /// @dev This function requires a valid notery signature and an unused nonce
    ...
    function nullAgreement(
        ...
        bytes32 message = keccak256(
            abi.encodePacked(agreementHash, nonce, "nullAgreement")
        );
        require(
            verifySigner(verifyingSigner, message, noterySignature),
            "Invalid notery signature"
        ...
        Agreement storage agreementToRemove = agreements[agreementHash];
        delete agreements[agreementHash];
        emit AgreementRemoved(agreementToRemove);
    }

    /// @notice Validates that the provided terms contracts support the required interface
    /// @param terms The array of terms contracts to validate
    function validateTerms(address[] memory terms) internal view {
        bytes4 termInterfaceId = bytes4(keccak256("term(Agreement)"));

        for (uint256 i = 0; i < terms.length; i++) {
            require(
                IERC165Upgradeable(terms[i]).supportsInterface(termInterfaceId),
                "Term contract does not support term interface"
            );
        }
    }
    ...
    {
        Agreement storage agreement = agreements[agreementHash];
        require(agreement.offerer != address(0), "Agreement not found");
