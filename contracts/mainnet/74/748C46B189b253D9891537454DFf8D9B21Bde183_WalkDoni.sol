...
contract WalkDoni is ERC721, ERC721Metadata, Ownable {
    ...
    // public
    function mint(address _user, uint256 amount) external payable {
        require(paused);
        uint256 mintCount = mintingSupplyCount;
        require(mintingTotalSupply >= mintCount + amount);
        uint256 mintPrice = tokenPrice * amount;
        if (msg.sender != owner) {
        ...
        for (uint256 i = 1; i <= amount; i++) {
            _safeMint(_user, mintCount + i + 10000);
    ...
    function reveal(uint256 tokenId) external {
        require(ownerOf(tokenId) == msg.sender && !revealed[tokenId]);
    ...
    function multipleReveal(uint256[] calldata tokenId) external {
        for (uint256 i; i < tokenId.length; i++) {
            require(ownerOf(tokenId[i]) == msg.sender && !revealed[tokenId[i]]);
    ...
    function EmergencyMintPause(bool check) external onlyOwner {
        paused = check;
    }
