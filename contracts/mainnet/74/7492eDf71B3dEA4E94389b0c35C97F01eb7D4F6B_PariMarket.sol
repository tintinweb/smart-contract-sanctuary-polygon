...
contract PariPoolContract is Context {
  ...
  uint8 constant private POOL_PARIS_ID = 0;
  uint8 constant private POOL_CLAIMED_PARIS_ID = 0;
    ...
    )
  {
    ...
    uint parisSize = _poolParis[poolid][POOL_PARIS_ID].length();
    if (parisSize == 0) return (new Pari[](0), parisSize);
    if (offset >= parisSize) return (new Pari[](0), parisSize);

    uint restSize = parisSize - offset;
    uint limit = 20;
    uint lastIdx = 0;
    if (restSize > limit) {
      lastIdx = restSize - limit;
    }
    uint resultSize = restSize - lastIdx;
    Pari[] memory paris = new Pari[](resultSize);

    uint idx = 0;
    while (idx != resultSize) {
      bytes32 pariid = _poolParis[poolid][POOL_PARIS_ID].at(restSize - 1 - idx);
      paris[idx] = _paris[pariid];
      idx++;
    }

    return (paris, parisSize);
  }
    ...
    internal
  {
    ...
    if (pari.pariid == 0x0) {
      ...
      _poolParis[poolid][POOL_PARIS_ID].add(pariid);
  ...
  function _claimPari(
    ...
    if (!_poolParis[pool.poolid][POOL_PARIS_ID].contains(pariid)) {
    ...
    if (_claimedParis[pool.poolid][POOL_CLAIMED_PARIS_ID].contains(pari.pariid)) {
      revert("CannotClaimClaimedPari");
    }
    ...
    private
  {
    ...
    _claimedParis[pari.poolid][POOL_CLAIMED_PARIS_ID].add(pariid);
