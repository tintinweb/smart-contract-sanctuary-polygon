...
// File: contracts/TheMinkeWhales.sol
...
contract TheMinkeWhales is ERC721, ReentrancyGuard, ERC2771Context, Ownable {
  using Counters for Counters.Counter;

  constructor(
    string memory _customBaseURI,
    address _trustedForwarder
  ) ERC721("The Minke Whales", "WHALE") ERC2771Context(_trustedForwarder)
  {
    customBaseURI = _customBaseURI;
    allowedMintCountMap[0x667fc4B1eDc5ff96F45Bc382cBfB60b51647948d] = 1500;
  }
  ...
  function allowlistMint(uint256 _count) public nonReentrant {
    if (allowedMintCount(_msgSender()) >= _count) {
      updateMintCount(_msgSender(), _count);
    } else {
      revert("Minting limit exceeded");
    }
