...
contract RewardPool is VRFConsumerBaseV2, Ownable {
  ...
  // mapping(uint => uint) public lastPizzeriaReward; // TODO: delete if not needed
  ...
  struct Request {
    uint pizzeriaId;
    uint reward;
    // uint unusedMultiplierTokens; TODO: delete if not needed
    uint betMultiplier;
    uint buyin;
    address user;
    bool isMultiplier;
  ...
  function play(uint pizzeriaId, uint buyin) external payable {
    ...
    require(pizzeriaInstance.isOwner(pizzeriaId, msg.sender), "PVE: this pizzeria does not belong to you");
    // TODO: UNCOMMENT !!! require(lastDeliveryTime[pizzeriaId] < (block.timestamp - 86400) , "PVE: pizzeria has already delivered in last 24h");
  ...
  function multiplier(uint betinRdk, uint pizzeriaId, uint8 _multiplier) external payable {
    ...
    // uint _lastPizzeriaReward = lastPizzeriaReward[pizzeriaId]; TODO: delete if not needed
    ...
    require(pizzeriaInstance.isOwner(pizzeriaId, msg.sender), "PVE: this pizzeria does not belong to you");
    // require(betinRdk > 0 && betinRdk <= _lastPizzeriaReward, "PVE: Insufficient funds from last race/multiplier"); TODO: delete if not needed
  ...
  bool isMultiplier, uint buyin) internal {
    ...
    // requestToBuyin[s_requestId] = buyin;
    requests[s_requestId] = Request(
      _pizzeriaId,
      _rdkReward,
      // unusedMultiplierTokens, TODO: delete if not needed
      _betRdk,
      buyin,
      _user,
      isMultiplier,
  ...
  // }
    ...
    uint randNumber = 1 + randomWords[0] % 10000;
      ...
      if(_request.isMultiplier) {
        // uint betInRdk = lastPizzeriaReward[_request.pizzeriaId] - _request.unusedMultiplierTokens; // TODO: delete if not needed
  ...
  //   event RewardPool_IngredientsPurchased(address indexed user, uint indexed pizzeriaId, uint boughtDays);
  //   event Played(address indexed user, uint indexed pizzeriaId, uint reward, uint speed, uint minSpeed, uint time, uint buyin);
    // lastPizzeriaReward[_request.pizzeriaId] = _request.unusedMultiplierTokens + _request.reward * winnerFlag; TODO: DELETE IF NOT NEEDED
