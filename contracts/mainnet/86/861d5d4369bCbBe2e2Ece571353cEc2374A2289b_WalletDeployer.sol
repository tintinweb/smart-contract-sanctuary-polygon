...
import "./interfaces/ITrustedTxModifier.sol";
import "./interfaces/IGnosisSafeL2.sol";
import "./Authorities.sol";

contract WalletDeployer is ModuleProxyFactory {
    MultiSend private immutable gnosisMultisendLibrary;
    GnosisSafeProxyFactory private immutable safeFactory;
    GnosisSafeL2 private immutable safeSingleton;
    ITrustedTxModifier private immutable moduleSingleton;
    CompatibilityFallbackHandler private immutable fallbackHandler;
    ModuleProxyFactory private immutable moduleProxyFactory;

    event WalletSetup(
        ITrustedTxModifier moduleAddress,
        GnosisSafeProxy safeAddress,
        Authorities authorities
    );
    event TrustedTxModifierSetup(
        GnosisSafeProxy avatar,
        GnosisSafeProxy target,
        Authorities authorities
    );

    constructor(
        MultiSend _gnosisMultisendLibrary,
        GnosisSafeProxyFactory _safeFactory,
        GnosisSafeL2 _safeSingleton,
        CompatibilityFallbackHandler _fallbackHandler,
        ITrustedTxModifier _moduleSingleton,
        ...
        moduleSingleton = _moduleSingleton;
        moduleProxyFactory = _moduleProxyFactory;
    }

    function deployTrustedTxModifierAndSafe(
        uint256 _saltNonce,
        Authorities authorities
    )
        public
        returns (ITrustedTxModifier trustedTxModifier, GnosisSafeProxy safe)
    {
        safe = safeFactory.createProxy(address(safeSingleton), hex"00");

        trustedTxModifier = ITrustedTxModifier(
            deployTrustedTxModifier(safe, _saltNonce, authorities)
        );

        bytes memory multisendAction = _generateMultisendAction(
            trustedTxModifier,
            safe
        );

        address[] memory owners = new address[](1);
        owners[0] = address(authorities);

        IGnosisSafeL2(address(safe)).setup(
            owners,
            1,
        ...
        emit WalletSetup(trustedTxModifier, safe, authorities);
    }

    function deployAndSetupSafesMultiple(
        uint256 numSafes,
        uint256 _saltNonce,
        Authorities authorities
    ) external {
        for (uint256 i = 0; i < numSafes; i++) {
            deployTrustedTxModifierAndSafe(_saltNonce, authorities);
        }
    }

    function deployTrustedTxModifier(
        GnosisSafeProxy _safe,
        uint256 _saltNonce,
        Authorities authorities
    ) public returns (address) {
        bytes memory initializeParams = abi.encode(_safe, _safe, authorities);

        address trustedTxModifier = moduleProxyFactory.deployModule(
            address(moduleSingleton),
            abi.encodeWithSignature("setUp(bytes)", initializeParams),
            _saltNonce
        );

        emit TrustedTxModifierSetup(_safe, _safe, authorities);

        return trustedTxModifier;
    }

    function _generateMultisendAction(
        ITrustedTxModifier trustedTxModifier,
        GnosisSafeProxy safe
    ) internal pure returns (bytes memory _action) {
        bytes memory enableTrustedTxModifier = abi.encodeWithSignature(
            "enableModule(address)",
            trustedTxModifier
        );

        bytes memory setGuard = abi.encodeWithSignature(
            "setGuard(address)",
            trustedTxModifier
        );
        ...
        bytes memory packedCalls = abi.encodePacked(
            ...
            safe,
            uint256(0),
            uint256(enableTrustedTxModifier.length),
            bytes(enableTrustedTxModifier),
            uint8(0),
            safe,
            uint256(0),
            uint256(setGuard.length),
            bytes(setGuard)
        );
...
contract Authorities is Ownable, IERC1271 {
    ...
    {
        return (
            isSigningAuthority(ECDSA.recover(messageHash, signature))
                ? true
                : false
        );
    }
...
interface ITrustedTxModifier {
    event AvatarSet(address indexed previousAvatar, address indexed newAvatar);
    event ChangedGuard(address guard);
    event DisabledModule(address module);
    event Ejected();
    event EjectingAuthorityChanged(address _ejectingAuthority);
    ...
    event SigningAuthorityChanged(address _signingAuthority);
    event TargetSet(address indexed previousTarget, address indexed newTarget);

    function DOMAIN_SEPARATOR() external view returns (bytes32);

    function authorities() external view returns (address);

    function avatar() external view returns (address);

    function changeEjectingAuthority(
        address _ejectingAuthority,
        bytes memory data,
