...
// File: contracts/libs/ML_AdminEmergencyWithdraw.sol
...
contract ML_AdminEmergencyWithdraw
{
    //========================
    // LIBS
    //========================
    ...
    function _adminEmergencyWithdrawETH(uint256 _amount, address _to, uint256 _gas) internal
    {
        (bool success, ) = payable(_to).call{ value: _amount, gas: _gas }("");
        success; //prevent warning
    }

    function _adminEmergencyWithdrawETH(uint256 _amount, address _to) internal
    {
        _adminEmergencyWithdrawETH(_amount, _to, 30000);
    }

    function _adminEmergencyWithdrawToken(IERC20 _token, uint256 _amount, address _to) internal
    {
...
// File: contracts/Tools/Escrow.sol
...
contract Escrow_ML is
    Ownable,
    Pausable,
    ML_AdminEmergencyWithdraw
{
    //========================
    // LIBS
    //========================
    ...
    struct Offer
    {
        ...
        uint256 revision;           //revision of offer
    ...
    //fees
    uint256 public feesInPeggedAmount;
    ITokenPair public oraclePair;
    IUniRouterV2 oracleRouter;
    address public feeWallet;

    //trades
    uint256 public tradeCounter;
    mapping(uint256 => Trade) public trades;
    mapping(address => mapping(address => uint256)) public tradePartnerMap;
    ...
    function setOracle(IUniRouterV2 _router, ITokenPair _pair) external onlyOwner
    {
        require(
            _pair.token0() == _router.WETH()
                || _pair.token1() == _router.WETH(),
            "OraclePair doesn't contain ETH");
    ...
    function getFeePeggedToken() public view returns (address)
    {
        return oraclePair.token0() != oracleRouter.WETH() ? oraclePair.token0() : oraclePair.token1(); 
    }

    function getFeeAmountInETH() public view returns (uint256)
    {
        if (feesInPeggedAmount == 0
            || address(oracleRouter) == address(0)
            || address(oraclePair) == address(0))
        {
            return 0;
        }
        address[] memory path = new address[](2);
        path[1] = getFeePeggedToken();  
        path[0] = oracleRouter.WETH(); 
        uint256[] memory out = oracleRouter.getAmountsOut(feesInPeggedAmount, path);          
    ...
    function getTrade(uint256 _tradeId) external view returns (
        ...
        Offer memory offer1, 
        Offer memory offer2, 
        OfferItem[] memory items1, 
        OfferItem[] memory items2
    )
    {
        status = trades[_tradeId].status;
        offer1 = trades[_tradeId].offer1;
        items1 = offer1.items;
        offer2 = trades[_tradeId].offer2;
        items2 = offer2.items;
    }    
    ...
    //========================
    // OFFER INFO FUNCTIONS
    //========================

    function getOffer(uint256 _tradeId, bool _offer1) public view returns (Offer memory)
    {
        return (_offer1
    ...
    function getOfferItems(uint256 _tradeId, bool _offer1) public view returns (OfferItem[] memory)
    {
        return getOffer(_tradeId, _offer1).items;
    }
    ...
    function setOffer(uint256 _tradeId, uint256 _coinAmount, OfferItem[] memory _items) external payable
    {
        //check
        ...
        {
            (bool success, ) = payable(msg.sender).call{ value: offer.balance - _coinAmount }("");
            success;            
    ...
    function executeTrade(uint256 _tradeId) external payable whenNotPaused
    {
        //check
        requireTradeOwner(_tradeId);
        requireTradeActive(_tradeId);
        require(getFeeAmountInETH() <= msg.value, "Insufficent payment");
    ...
    function executeTradeOffer(uint256 _tradeId, bool _offer1) private
    {
        ...
        Offer storage offer = (_offer1
            ? trades[_tradeId].offer1
            : trades[_tradeId].offer2);
        address partner = (_offer1
            ? trades[_tradeId].offer2.owner
            : trades[_tradeId].offer1.owner);

        //send balance
        if (offer.balance > 0)
        {
            (bool success, ) = payable(partner).call{ value: offer.balance }("");
            success;
        }        
        ...
        {
            require(
                executeTradeOfferItem(offer.owner, partner, offer.items[n]),
                "Trade failed");
        }
    ...
    function cancelTrade(uint256 _tradeId) external payable
    {
        //check
        requireTradeOwner(_tradeId);
        requireTradeActive(_tradeId);
    ...
    function createTrade(address _partner) external whenNotPaused returns (uint256)
    {
        ...
        //create trade
        tradeCounter += 1;
        trades[tradeCounter].id = tradeCounter;
        trades[tradeCounter].status = TradeStatus.ONGOING;
        trades[tradeCounter].offer1.owner = msg.sender;
        trades[tradeCounter].offer2.owner = _partner;
    ...
    function adminEmergencyWithdrawETH(uint256 _amount, address _to) external onlyOwner
    {
        super._adminEmergencyWithdrawETH(_amount, _to);        
    }

    function adminEmergencyWithdrawToken(IERC20 _token, uint256 _amount, address _to) external onlyOwner
    {
        super._adminEmergencyWithdrawToken(_token, _amount, _to);
    }  
}