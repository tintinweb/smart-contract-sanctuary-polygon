...
contract Sniper is AccessControlUpgradeable {
	...
	function paisChecked() public view returns (uint) {
	...
	{
		for (uint256 i = lastPairChecked; i < totalPairs(); i++) {
			...
			bool exists = tokens.contains(tokenAddress);

			if (reservesGood && !exists) {
				canExec = true;
