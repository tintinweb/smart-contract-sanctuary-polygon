...
/// @notice Based on https://docs.gelato.network/developer-products/gelato-ops-smart-contract-automation-hub/guides/writing-a-resolver/smart-contract-resolver
contract GelatoAMBRebalancer is PokeMeReady {

  string public constant VERSION = '1.0.2';
  address public immutable owner;
  mapping (address => bool) public manualRebalance;

  event GelatoRebalanceAllPipes(address AMBStrategy);
  event GelatoRebalanceAllPipesStub(address AMBStrategy);
  ...
  external onlyPokeMe {
    manualRebalance[AMBStrategy] = false;
  ...
  /// @notice Stub function for integration testing purposes
  function rebalanceAllPipesStub(address AMBStrategy)
  external onlyPokeMe {
    manualRebalance[AMBStrategy] = false;
    emit GelatoRebalanceAllPipesStub(AMBStrategy);
  }
  ...
  external view returns (bool canExec, bytes memory execPayload) {
    canExec = manualRebalance[AMBStrategy] || ILinearPipeline(AMBStrategy).isRebalanceNeeded();
  ...
  function setManualRebalance(address AMBStrategy, bool value)
  external {
    require(owner == msg.sender, 'GR: not owner');
    manualRebalance[AMBStrategy] = value;
  }
}
