...
contract LuchadorDataGetter {
    Luchador public luchador = Luchador(0xcC792E8B52b7A6717253B3260408953aC892539c);

    function getLuchadoresData(uint256 startIndex, uint256 endIndex) public view returns (Luchador.LuchadorData[] memory) {
        uint256 range = endIndex - startIndex;
        Luchador.LuchadorData[] memory luchadores = new Luchador.LuchadorData[](range);

        for (uint i = startIndex; i < endIndex; i++) {
            luchadores[i - startIndex].tokenURI = luchador.tokenURI(i);
            luchadores[i - startIndex].attributes = luchador.getAllLuchadorAttributes(i);
            luchadores[i - startIndex].location = luchador.luchadoresLocation(i);
            luchadores[i - startIndex].currentTrainerOwner = luchador.luchadoresCurrentTrainerOwner(i);
            luchadores[i - startIndex].locationStartTime = luchador.luchadoresLocationStartTime(i);
            luchadores[i - startIndex].weight = luchador.luchadoresWeight(i);
            luchadores[i - startIndex].wins = luchador.luchadoresWins(i);
            luchadores[i - startIndex].losses = luchador.luchadoresLosses(i);
            luchadores[i - startIndex].name = luchador.luchadoresName(i);
            luchadores[i - startIndex].bio = luchador.luchadoresBio(i);
            luchadores[i - startIndex].price = luchador.luchadoresPrice(i);
            luchadores[i - startIndex].index = luchador.luchadoresIndex(i);
            luchadores[i - startIndex].lastSalePrice = luchador.luchadoresLastSalePrice(i);
            luchadores[i - startIndex].saleStatus = luchador.luchadoresSaleStatus(i);
        }
