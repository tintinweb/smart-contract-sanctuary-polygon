...
contract wav3sRaffleTweetUSDT is VRFConsumerBaseV2{
    ...
    // Winners of pubId 
    mapping(string => address[]) public wav3Winner;
    ...
    function fundTweet(
        ...
        IERC20(i_USDT).transfer(
            s_multisig,
            ((fees_amount * i_wav3s_fee) / (total_fees)+i_basefee)
        );
    ...
    function executeRaffle(string calldata pubId) external stopInEmergency onlyWav3sTrigger {
       uint256 winners = postDataByPublicationId[pubId].winners;
        ...
        s_PubIdToRaffleState[pubId] != RaffleState.CLOSED, 
         "Errors.wav3sRaffleTweetUSDT__executeRaffle__RaffleClosed"
         );
        require (
        block.timestamp > postDataByPublicationId[pubId].raffleTime, 
         "Errors.wav3sRaffleTweetUSDT__executeRaffle__RaffleTimeNotOver"
         );
          require (
        postDataByPublicationId[pubId].retwitters >= winners, 
         "Errors.wav3sRaffleTweetUSDT__executeRaffle__NotEnoughRetwitters"
         );
    ...
    // mapping para mostrar los winners de la raffle
    ...
    function getRetwitters(
        string calldata pubId
    ) external view returns (uint256) {
        // Get amount of retwitters for this Publication
        return postDataByPublicationId[pubId].retwitters; 
    }
        ...
        // Get PostData for this Publication
        return s_PubIdToRaffleState[pubId];
    }
