...
contract BotSuperTrendDividendToken is Ownable, ReentrancyGuard, ERC20 {
    ...
    function withdraw() external nonReentrant isUnlocked {
        ...
        uint256 holderBalance = balanceOf(account_);
        require(holderBalance != 0, "BotSuperTrendDividendToken: caller possess no shares");

        uint256 amount = ( (dividendPerToken - xDividendPerToken[account_]) * holderBalance / MULTIPLIER);
        amount += credit[account_];
        credit[account_] = 0;
        xDividendPerToken[account_] = dividendPerToken;

        (bool success, ) = payable(account_).call{value: amount}("");
        require(success, "BotSuperTrendDividendToken: Could not withdraw eth");
    }

    function getDividentOwner(address account_) public view onlyOwner returns (uint256) {
        uint256 holderBalance = balanceOf(account_);
        uint256 amount = 0;
        if (holderBalance != 0){
            amount = ( (dividendPerToken - xDividendPerToken[account_]) * holderBalance / MULTIPLIER);
            amount += credit[account_];
            }
        return amount;
    }
