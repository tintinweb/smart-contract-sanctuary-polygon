...
contract BullManager {
    ...
    IERC20 private USDC = IERC20(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174); //polygon
    ...
    // Cost to buy a tree 
    uint costPerTree = 1*10**18;
    // Cost to refresh
    uint refreshCost = 1*10**4;
    uint claimFee = 1*10**4;
    ...
    function createTreeWithTokens(string memory _treeName) external NotBlacklist checkNameConflict(msg.sender, _treeName) {
        ...
        splitToken();
        Tree memory newTree = Tree(_treeName, block.timestamp, block.timestamp + 10 minutes, block.timestamp, rewardPerTree);
