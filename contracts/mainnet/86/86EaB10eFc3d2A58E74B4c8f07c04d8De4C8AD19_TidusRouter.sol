...
contract BaseAggregator {
    ...
    function _fillQuoteTokenToEth(
        ...
        uint256 initialEthAmount = address(this).balance - msg.value;
        require(msg.value >= feeAmount, "INSUFFICIENT_FEE");
        ...
        if (feeAmount > 0) {
            uint256 fees = (ethDiff * feeAmount) / 1e18;
    ...
    function _fillQuoteTokenToToken(
        ...
        require(msg.value > 0 && feeAmount > 0 && msg.value >= feeAmount, "INSUFFICIENT_FEE_AMOUNT");
        ...
        // 8 - Send the rest of the ETH after fees to the user
        uint256 fees = (msg.value * feeAmount) / 1e18;
        uint256 amountMinusFees = msg.value - fees;
...
contract TidusRouter is BaseAggregator {
    ...
    constructor(address payable _tidusTreasury, address _0xProxyAddress) BaseAggregator(_tidusTreasury) {
        require(_0xProxyAddress != address(0), "ZERO_ADDRESS");
        swapTargets[_0xProxyAddress] = true;
        owner = _tidusTreasury;
        status = 1;
    }
