    ...
    UUPSUpgradeable
{
    ...
    modifier onlySufficentLocks(address account, uint256 amount) {
        FundItem storage fund = _balances[account];
        require(fund.locked >= amount, "Insufficent locks");
        _;
    }
    ...
    function unlock(address account, uint256 amount) private onlySufficentLocks(account, amount) {
        FundItem storage fund = _balances[account];
        fund.free += amount;
        fund.locked -= amount;
    }
