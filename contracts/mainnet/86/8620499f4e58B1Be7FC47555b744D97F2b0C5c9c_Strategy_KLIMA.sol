...
/*
    ...
    KLIMA token uses for staking.
    MATIC -> WMATIC -> USDC -> KLIMA routing
    Token Helper = "sKLIMA"
*/
contract Strategy_KLIMA is Ownable {
    ...
    uint256 public pendingFeeKLIMA;
    ...
    address public lpToken = 0x5786b267d35F9D011c4750e0B0bA584E1fDbeAD1;
    address public vault;
    address public mainToken = 0xb0C22d8D350C67420f06F48936654f567C73E8C8; // sKLIMA decimals 9
    address public KLIMA = 0x4e78011Ce80ee02d2c3e649Fb657E45898257815; // token1 - decimals 9
    address public USDC = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174; // token0 - decimals 6
    ...
    address public constant stakingHelper = 0x4D70a031Fc76DA6a9bC0C922101A05FA95c3A227; // stake
    address public constant staking = 0x25d28a24Ceb6F81015bB0b2007D795ACAc411b4d; // unstake
    // ------------>        END          <--------------
    ...
    function earn() public returns(uint256 _balance) {
        _balance = getBalanceOfToken(KLIMA);
        if(_balance > 0) {
            _approveToken(KLIMA, stakingHelper);
    ...
    function emergencyWithdraw(address _receiver) public onlyVaultOrOwner {
        ...
        _transfer(KLIMA, _receiver, getBalanceOfToken(KLIMA));
        _transfer(USDC, _receiver, getBalanceOfToken(USDC));
    ...
    function deposit() public payable onlyVault returns (uint256) {
        ...
        _approveToken(KLIMA, router);
        _approveToken(USDC, router);
        _checkPrices();
        _swapExactETHForTokens(USDC, address(this).balance);
        _swapExactTokenForToken(router, USDC, KLIMA, getBalanceOfToken(USDC));
        _updateLastPrices();
    ...
    function depositAsMigrate() public onlyVault {
        ...
        _approveToken(wrappedToken, router);
        _approveToken(USDC, router);
        _swapExactTokenForToken(router, wrappedToken, USDC, getBalanceOfToken(wrappedToken));
        _swapExactTokenForToken(router, USDC, KLIMA, getBalanceOfToken(USDC));
        earn();
    }
    ...
    function getBalanceOfToken(address _token) public view returns (uint256) {
        ...
        if(_token == KLIMA) {
            return IERC20(_token).balanceOf(address(this)) - pendingFeeKLIMA;
        }
        if(_token == mainToken) {
    ...
    function setLP(address _address) public onlyVaultOrOwner {
        require(_address != address(0), "The address can not be zero address");
        require(wrappedToken == IPair(_address).token0() && KLIMA == IPair(_address).token1(), "Tokens don't match");
    ...
    function _takeFee(uint256 _total) internal {
        ...
        _amount = getBalanceOfToken(KLIMA) + pendingFeeKLIMA;
        bool result = _swapExactTokenForToken(router, KLIMA, USDC, _amount);
        if(result) {
            emit TakeFee(pendingFeeKLIMA);
            pendingFeeKLIMA = 0;
        } else {
            pendingFeeKLIMA = _amount;
        }

        _amount = getBalanceOfToken(USDC) + pendingFeeUSDC;
        result = _swapExactTokenForToken(router, USDC, wrappedToken, _amount);
        if(result) {
            emit TakeFee(pendingFeeUSDC);
            pendingFeeUSDC = 0;
        } else {
            pendingFeeUSDC = _amount;
        }
    ...
    function _swapTokenToWrapped(uint256 _total, uint256 _percent) internal {
        ...
        _swapExactTokenForToken(router, KLIMA, USDC, getBalanceOfToken(KLIMA));
        _swapExactTokenForToken(router, USDC, wrappedToken, getBalanceOfToken(USDC));
    }
    ...
    function _getToken0Price() public view returns (uint256) {
        ...
        path[0] = KLIMA;
        path[1] = USDC;
        return 1e12 / (_getAmountsOut(router, 100 * 1e9, path) / 100);
    }
    ...
    function _getToken1Price() public view returns (uint256) {
        ...
        path[0] = USDC;
        path[1] = KLIMA;
        return 1e18 / (_getAmountsOut(router, 100 * 1e6, path) / 100);
    }

    function _getUSDCToWrappedPrice() public view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = wrappedToken;
        path[1] = USDC;
        return 1e12 / (_getAmountsOut(router, 100 * 1e18, path) / 100);
    }

    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
    	uint256 amountUSDC = _getToken1Price() * _amount / 1e9;
        tCI += _getUSDCToWrappedPrice() * amountUSDC / 1e6;
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if (_balance > 0) {
            path[0] = KLIMA;
            path[1] = USDC;
            amountUSDC = _getAmountsOut(router, _balance, path);
        }

        _balance = getBalanceOfToken(USDC) + amountUSDC;
        if (_balance > 0) {
	        path[0] = USDC;
	        path[1] = wrappedToken;
