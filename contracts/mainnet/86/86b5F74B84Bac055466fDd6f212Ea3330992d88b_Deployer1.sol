...
contract Deployer1 {
    ...
    function deploy(
        ...
        MojoSeedERC721(mojoSeed).mintTo(owner);
        MojoSeedERC721(mojoSeed).setMinter(address(this), false);

        MojoERC721(mojo).setMinter(address(this), true);
        MojoERC721(mojo).mintTo(owner);
        MojoERC721(mojo).setMinter(address(this), false);
...
abstract contract PlanetMojoERC721Base is ERC721MintManagement, CustomRoyalties, OwnableWithTempOwnership {
    uint256 private constant OPEN_SEA_WORKAROUND = 1; 
