...
contract Strategy_KLIMA is Ownable {
    ...
    function getBalanceOfToken(address _token) public view returns (uint256) {
        ...
        if(_token == USDC) {
            return IERC20(_token).balanceOf(address(this)) - pendingFeeUSDC;
        }
        if(_token == mainToken) {
    ...
    function _getToken0Price() public view returns (uint256) {
        ...
        path[0] = KLIMA;
        path[1] = USDC;
        return 1e15 / (_getAmountsOut(router, 100 * 1e9, path) / 100);
    }
    ...
    function _getToken1Price() public view returns (uint256) {
        ...
        path[0] = USDC;
        path[1] = KLIMA;
        return 1e15 / (_getAmountsOut(router, 100 * 1e6, path) / 100);
    }
    ...
    function _getUSDCToWrappedPrice() public view returns (uint256) {
        ...
        path[0] = wrappedToken;
        path[1] = USDC;
        return 1e24 / (_getAmountsOut(router, 100 * 1e18, path) / 100);
    }
