...
contract ChanceVault is Ownable, Pausable {
    ...
    function withdrow(address _to, uint256 amount, uint256 c_amount) external {
        ...
        uint256 balance = IWrappedToken(WTOKEN).getBalanceOf(address(this));
        require(balance >= amount + c_amount, "NotEnough");
        if (c_amount > 0){
            IWrappedToken(WTOKEN).transfer(COMMUNITY_ADDR, c_amount);
        }
        IWrappedToken(WTOKEN).transfer(_to, amount);
    }
