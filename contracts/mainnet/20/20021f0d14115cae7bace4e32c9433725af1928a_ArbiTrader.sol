 ...
 */
contract ArbiTrader is IFlashLoanSimpleReceiver, OwnableUpgradeable, AbstractTrader{
    ...
    event LogOperationEvent(Operation currentOperation);
     ...
     */
    function performStrategy(Strategy memory _strategy) external {
        // console.log('performStrategy...');
    ...
    function executeOperation(
        ...
        uint256 _repayLoanAmount = _loanAmount + _aavePool.FLASHLOAN_PREMIUM_TO_PROTOCOL();
        uint256 borrowed = IERC20(_asset).balanceOf(address(this));

        // console.log('Initial loan:; USDC ', borrowed);
        require(borrowed >= _loanAmount,"AT: Loan Transfer Fail");
        Strategy memory strategy = abi.decode(_data, (Strategy));

        uint256 _swapResult1 = 0;

        for(uint8 i = 0; i < strategy._strategyLength; i++){
            Operation memory _operation = _decodeOperation(strategy._ops[i]);

            if(_swapResult1 != 0){  
                _operation._amountIn = _swapResult1;
                // _operation._amountOut = _repayLoanAmount; // min to repay
            }
            emit LogOperationEvent(_operation);
            _swapResult1 = _swap(_operation);
        }
        uint256 finalBalance = IERC20(_asset).balanceOf(address(this));
        uint256 pAndL = _repayLoanAmount > finalBalance ? _repayLoanAmount - finalBalance : finalBalance - _repayLoanAmount;
        
        require( finalBalance >= _repayLoanAmount, "Not enought profit for repay loan");

        IERC20(_asset).approve(address(_aavePool), _repayLoanAmount);
        ...
        IERC20(_asset).transfer(_treassury,IERC20(_asset).balanceOf(address(this)));
        return true;
    }

    /* 
         @dev Supports multihop for uniswap3 and all v2 swaps
    */
    function _swap(Operation memory operation) internal returns(uint256 _amountOut){
        ...
        if(keccak256(bytes(operation._dexSymbol)) == keccak256(bytes("UNIV3"))){
            if(operation._poolFees.length > 1){
                _amountOut = ISwapRouter(operation._dexAddress).exactInput(
                                ISwapRouter.ExactInputParams({
                                    path: _defineExactInputPath( operation._paths, operation._poolFees),
    ...
    function _decodeOperation(bytes memory operation) internal returns(Operation memory _operation){
       (
        ...
        _operation._spotPrice,
        _operation._paths,
        _operation._poolFees) = abi.decode(operation,(string, address, uint256, uint256,uint256, address[], uint24[]));
    }
    
    function _withdrawProfit(address _asset) external onlyOwner {
    ...
    function _withdrawNativeProfit() external onlyOwner {
...
abstract contract AbstractTrader {
    struct Pool {
        address token0;
        address token1;
        uint8 poolFee;
