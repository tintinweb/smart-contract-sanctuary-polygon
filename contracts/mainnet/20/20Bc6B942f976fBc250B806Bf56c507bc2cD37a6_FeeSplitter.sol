...
contract FeeSplitter is TokensRecoverable
{
    ...
    SignedStake public signedStake;
    ...
    constructor(address _devAddress, IPancakeRouter02 _router, SignedStake _signedStake) 
    {
    ...
    function setSignStaking(SignedStake _signedStake) public onlyRoyaltyCollector
    {
        signedStake = _signedStake;
    }
    ...
    function setFees(IGatedERC20 token, uint256 burnRate, uint256 sellRate, uint256 keepRate) public onlyRoyaltyCollector() // 100% = 10000
    {
        ...
        token.approve(address(signedStake), uint256(-1)); 
    }
    //todo update below
    ...
    {
        //require (collectors.length == rates.length, "Fee Collectors and Rates must be the same size");
        //collectors[collectors.length] = address(signedStake);
        uint256 totalRate = 0;
        for (uint256 i = 0; i < rates.length; i++)
        {
            totalRate = totalRate + rates[i];
        ...
        rootedTokenFeeCollectors[token].push(address(signedStake));
    ...
    function distribute(IERC20 token, uint256 amount, address[] memory collectors, uint256[] memory rates) private
    {
        for (uint256 i = 0; i < collectors.length; i++)
        {
            ...
            if (rate > 0)
            {
                ...
                uint256 fee = rate * amount / 10000;
                signedStake.depositTokens(address(token), fee);
            } else{
