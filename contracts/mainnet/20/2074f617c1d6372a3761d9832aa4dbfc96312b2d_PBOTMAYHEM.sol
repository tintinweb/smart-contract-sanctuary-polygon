...
contract PBOTMAYHEM is ERC1155, Ownable {
	...
	function airdrop(address[] calldata addr, uint token) external onlyOwner {	
		for(uint256 x = 0; x < addr.length; x++) {
        _mint(addr[x], token, 1, "");
		}
	}
