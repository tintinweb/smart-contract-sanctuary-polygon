...
contract PLG_RouterV5 is permission {
    ...
    function migrate(address[] memory accounts,address oldRouter) public forRole("owner") returns (bool) {
        for(uint256 i=0; i<accounts.length; i++){
            uint256[] memory data = IPLGData(oldRouter).invest(accounts[i]);
            invest[accounts[i]].balance = data[0];
            invest[accounts[i]].repeat = data[1];
            invest[accounts[i]].block_deposit = data[2];
            invest[accounts[i]].block_withdraw = data[3];
            invest[accounts[i]].block_break = data[4];
            invest[accounts[i]].cycle = data[5];
            invest[accounts[i]].recycle = data[6];
        }
        return true;
    }
