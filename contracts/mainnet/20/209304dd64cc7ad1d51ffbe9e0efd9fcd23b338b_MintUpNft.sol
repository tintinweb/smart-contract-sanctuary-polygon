...
contract MintUpNft is ERC721A, ERC2981, Ownable, ERC20Payement {
    ...
    event NewPremintSequentialInRandom();
    ...
    constructor(
        ...
        premintRandom = initParams.random;
        ...
        _setDefaultRoyalty(
            initParams.royaltiesRecipient,
            initParams.royaltiesAmount
        );
    }
    ...
    function _performPayment(uint256 _price) internal {
        if (paymentMethod) {
            ...
            success = _ERC20Payment(msg.sender, address(this), _price);
            if (!success) revert failTransfer();
        } else {
            if (msg.value < _price) revert amountSendIncorrect();
        }
        mintUpBalance += (_price * mintUpPart) / 10000;
    }
        ...
        verify(msg.sender, _quantitySignature, currentPhase, _signature)
    {
        ...
        if (_quantity + quantityPremint[msg.sender] > _quantitySignature)
            revert quantityExceed();
    ...
    {
        if (_quantity > _quantitySignature) revert quantityExceed();
        if (_quantity + quantityWhitelist[_to] > _quantitySignature)
            revert quantityExceed();
        if (_quantity == 0) revert quantityZero();
        if (_nextTokenId() + _quantity > maxSupply + 1) revert maxSupplyReach();
    ...
    function publicMint(
        ...
        if (_quantity == 0) revert quantityZero();
        if (_nextTokenId() + _quantity > maxSupply + 1) revert maxSupplyReach();
        if (quantityPublic[_to] + _quantity > maxPerAddress)
            revert quantityExceed();
        ...
        unchecked {
            for (uint256 i; i < _quantity; ++i) {
                ...
                if (random && !premintRandom) {
                    indexerLength -= 1;
                    indexer[nextId + i] = indexerLength;
                }
    ...
    function withdrawOwner() external {
        ...
        if (paymentMethod) {
            ...
            success = _withdrawERC20(owner, _balance - mintUpBalance);
            if (!success) revert failTransfer();
        } else {
            (success, ) = payable(owner).call{
                value: address(this).balance - mintUpBalance
            }("");
            if (!success) revert failTransfer();
        }
    }

    function withdrawMintUp() external {
        if (msg.sender != mintUp) revert notAuthorized();
        bool success;
        if (paymentMethod) {
            success = _withdrawERC20(mintUp, mintUpBalance);
            if (!success) revert failTransfer();
        } else {
            (success, ) = payable(mintUp).call{value: mintUpBalance}("");
            if (!success) revert failTransfer();
        }
        mintUpBalance = 0;
    }
    ...
    function setPremintSequentialInRandom() external onlyOwner {
        if (!random) revert onlyRandomCollection();
        premintRandom = false;
        emit NewPremintSequentialInRandom();
    }
