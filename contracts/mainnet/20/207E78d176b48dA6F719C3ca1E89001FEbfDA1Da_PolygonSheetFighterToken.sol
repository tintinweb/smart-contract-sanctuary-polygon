...
import "ERC721A.sol";
import "Base64.sol";
import "SheetFighterUtilities.sol";
import "IBridge.sol";
...
contract PolygonSheetFighterToken is Ownable, ERC721A {
    ...
    /// @notice Contracts allowed to initiate transfers
    mapping(address => bool) addressCanTransfer;

    /// @notice Address of the Polygon bridge
    address public bridge;

    /// @notice Address of the Upgrade contract
    ...
    constructor() ERC721A("Sheet Fighter", "SHEET", 8888) Ownable() {}
    ...
    /// @dev Mint sheets initially
    /// @param _startId Start token id, inclusive
    /// @param _amount # of tokens being minted
    /// @param _flavorTexts Array of pipe-concatonated strings
    /// @param _fighterStats Array of sheet stats
    function initMint(
        uint256 _startId,
        uint256 _amount,
        string[] calldata _flavorTexts, 
        FighterStats[] calldata _fighterStats
    ) 
        external 
        onlyOwner 
    {
        require(bridge != address(0), "Bridge must be set");
        require(_flavorTexts.length == _amount,  "Invalid _flavorTexts length");
        require(_fighterStats.length == _amount,  "Invalid _fighterStats length");

        // Mint
        _safeMint(bridge, _amount);

        // Set flavor texts, paper stocks, and sheet colors
        for(uint256 i = 0; i < _amount; i++) {
            flavorTexts[_startId + i] = _flavorTexts[i];
            tokenStats[_startId + i] = _fighterStats[i];
        }
    }

    /// @dev Set transfer permission for an address
    /// @param initiator Address that may attempt to initiate a transfer
    /// @param canTransfer Boolean indicating whether or not the initiator can initiate a transfer
    function setCanTransfer(address initiator, bool canTransfer) external onlyOwner {
        addressCanTransfer[initiator] = canTransfer;
    }
    ...
    /// @dev Used for authorization
    /// @param  _upgradeContract New address for the bridge
    function setUpgradeContract(address _upgradeContract) external onlyOwner {
        upgradeContract = _upgradeContract;
    }

    /// @notice Update the address of the battle contract
    /// @dev Used for authorization
    /// @param  _battleContract New address for the bridge
    ...
    function bridgeSheets(address tokenOwner, uint256[] calldata tokenIds) external onlyBridge {
    ...
    function updateStats(
        ...
        external 
    {
        require(msg.sender == upgradeContract, "Only the upgrade contract can upgrade a sheet");

        // Update stats
        if(attributeNumber == 0){
            tokenStats[tokenId].HP = value;
        }
        else if(attributeNumber == 1) {
            tokenStats[tokenId].critical = value;
        }
        else if(attributeNumber == 2) {
            tokenStats[tokenId].heal = value;
        }
        else if(attributeNumber == 3) {
            tokenStats[tokenId].defense = value;
        }
        else if(attributeNumber == 4) {
            tokenStats[tokenId].attack = value;
        }
    }
    ...
    function isRareStat(uint8 stat, uint8 min, uint8 max) public pure returns(bool) {
        return uint256(stat) * 100 >= (RARE_MOVE_THRESHOLD * (uint256(max) - uint256(min))) + (100 * uint256(min));
    }

    /// @dev Before token transfer hook -- runs on mint, too
    /// @dev Prevents non-permissioned addresses from initiating any transfers
    /// @param from Address owning the token(s)
    /// @param to Address receiving the token(s)
    /// @param startTokenId Id of the first sequential token being transfered
    /// @param quantity Number of sequential tokens being transfered
    function _beforeTokenTransfers(
        ...
        require(addressCanTransfer[msg.sender], "Not allowed to transfer");
    ...
    function _getAttributes(FighterStats memory _stats) internal pure returns(string memory) {
