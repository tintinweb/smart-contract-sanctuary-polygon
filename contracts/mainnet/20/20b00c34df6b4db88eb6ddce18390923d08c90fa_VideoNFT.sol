 ...
 */
contract VideoNFT is ERC721PresetMinterPauserAutoId {
    mapping(uint256=>string) private _tokenURI;
    Counters.Counter private _tokenIdTracker;
    using Counters for Counters.Counter;


    constructor(string memory tokenName, string memory tokenSymbol) ERC721PresetMinterPauserAutoId(tokenName, tokenSymbol,"") {}
    ...
    function mint(address to, string memory _uri) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "ERC721PresetMinterPauserAutoId: must have minter role to mint");
        require(bytes(_uri).length > 0 ,"Token URI can't be empty");
        // We cannot just use balanceOf to create the new tokenId because tokens
        // can be burned (destroyed), so we need a separate counter.
