...
contract FUNDToken is IERC1404,Ownable,ERC20 {

    using SafeERC20 for IERC20;
    uint8 public constant SUCCESS_CODE = 0;
    uint8 public constant SEND_NOT_ALLOWED_CODE = 1;
    uint8 public constant SENDER_NOT_ALLOWED_CODE = 3;
    uint8 public constant RECEIVE_NOT_ALLOWED_CODE = 2;
    string public constant SUCCESS_MESSAGE = "SUCCESS";
    string public constant SEND_NOT_ALLOWED_ERROR = "SENDING_ACCOUNT_NOT_WHITELISTED";
    string public constant SENDER_NOT_ALLOWED_ERROR = "SENDER_ACCOUNT_NOT_WHITELISTED";
    string public constant RECEIVE_NOT_ALLOWED_ERROR = "RECEIVING_ACCOUNT_NOT_WHITELISTED";
    mapping (uint8 => string) public messages;
    mapping (address => bool) public kyc;
    mapping(address => bool) public whitelistedAddresses;

    IERC1404 private transferRestrictions;

    event RestrictionsUpdated (address newRestrictionsAddress, address updatedBy);
    event RestrictionsDetectTransfer (address from, address to, uint value);
    ...
    modifier notRestricted (address from, address to, uint256 value) {
     ...
     modifier notRestrictedTransferFrom (address sender,address from, address to, uint256 value) {
        uint8 restrictionCode = detectTransferFromRestriction(sender,from, to, value);
        require(restrictionCode == SUCCESS_CODE, messageForTransferRestriction(restrictionCode));
        _;
    }

    constructor () public ERC20("FUND Token", "FNT"){
        messages[0] = SUCCESS_MESSAGE;
        messages[1] = SEND_NOT_ALLOWED_ERROR;
        messages[2] = RECEIVE_NOT_ALLOWED_ERROR;
        messages[3] = SENDER_NOT_ALLOWED_ERROR;
        _mint(msg.sender, 50000000000000000000000000);
    }

    function messageForTransferRestriction (uint8 restrictionCode) public view virtual override returns (string memory message)
    {
        message = messages[restrictionCode];
       //z return transferRestrictions.messageForTransferRestriction(restrictionCode);
    }
    function updateTransferRestrictions(address _newRestrictionsAddress)
        ...
        returns (bool)
    {
        transferRestrictions = IERC1404(_newRestrictionsAddress);
    ...
    //         require(
    //             !whitelist[to],
    //             "receiver are not whitelisted"
    //         );
    //     } else {
    //         restrictionCode = SUCCESS_CODE; // successful transfer (required)
           
    //     }
    //     emit RestrictionsDetectTransfer(address(msg.sender),address(to),value);
    // }
        ...
        returns (uint8 restrictionCode)
    {
           ...
           if (!kyc[from]) {
            ...
            // require(whitelist[msg.sender],"sender whitelisted");
        ...
        returns (uint8 restrictionCode)
    {
         ...
         if (!kyc[from]) {
            ...
            // );
             ...
             restrictionCode = SENDER_NOT_ALLOWED_CODE; 
            //  require(
            //     !whitelist[msg.sender],
            //     "you are not whitelisted"
            // );
    ...
    public onlyOwner
    {
     ...
     function addMultipleWhitelistedAddresses(address[] calldata _operator)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < _operator.length; i++) {
            kyc[_operator[i]] = true;
        }
    }
