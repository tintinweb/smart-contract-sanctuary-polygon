...
contract Bankroll {
    ...
    function updateAdmin(address addr) isAdmin isHuman external {
        currentCorpBank = addr;
    }


    function setHorsemanLite(address addr) isAdmin isHuman external {
        HorsemanLite = addr;
    }

    function tranform() isAdmin isHuman external payable {
        Wager += msg.value;
    }
    ...
    function withdrawFunds() external isAdmin isHuman{
        Withdraw += this.totalBalance();
        payable(msg.sender).transfer(this.totalBalance());
    }

    function withdrawFees(uint256 val) external isAdmin isHuman{
