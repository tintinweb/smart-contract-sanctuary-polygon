...
contract Liquidator is Initializable, AccessControlUpgradeable, UUPSUpgradeable, Version {
        ...
        onlyRole(Roles.LIQUIDATION_BOT)
    {
        ...
        //We create liquidity record only in case of collateral and underlying token are different
        if (collateralToken != underlyingToken) {
            liquidity.push(
                LiquidityToSwap(collateralToken, underlyingToken, collateralAmount, pool)
            );
        }
