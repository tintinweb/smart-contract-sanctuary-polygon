...
contract OddzSwap {
    ...
    // Get tge amounts in for the specified path
    function getAmountsIn(uint256 amountOut, address[] memory path) external view returns (uint256[] memory amounts) {
        require(path.length >= 2, "INVALID_PATH");
        amounts = new uint256[](path.length);
        amounts = IUniswapV2Router02(UNISWAP_ROUTER).getAmountsIn(amountOut, path);
    }
    ...
    function swapExactTokensForETH(
        ...
        require(ERC20(path[0]).transferFrom(msg.sender, address(this), amountIn), "TransferFrom failed!");
        amounts = IUniswapV2Router02(UNISWAP_ROUTER).swapExactTokensForETH(amountIn, amountOutMin, path, to, deadline);
    }
    ...
    function getWETHAddress() public view returns (address wethAddress) {
        wethAddress = IUniswapV2Router02(UNISWAP_ROUTER).WETH();
    }
}
