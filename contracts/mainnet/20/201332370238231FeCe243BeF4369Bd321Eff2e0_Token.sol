...
contract Token is ERC20 {
    string public constant name = "TokenV2";
    string public constant symbol = "TV2";
    uint256 public constant initialSupply = 1000000000 * (10 ** uint256(decimals));
    uint256 private constant unixDayTime = 86400;
    ...
    */
    ...
    function transfer(address _to, uint256 _value) public whenNotFrozen whenNotPaused returns (bool) {
    ...
    function burn(address _who, uint256 _value) public onlyOwner {
        require(_value <= super.balanceOf(_who), "Not enough balance");
    ...
    function detailBalance(address _holder) public view returns (uint256 totalBalance, uint256 lockedBalance) {
        uint256 lockedValue;
        uint256 length = lockInfo[_holder].length;
        for(uint256 i = 0; i < length; i++ ) {
            LockInfo memory acc = lockInfo[_holder][i];
            ...
            if (acc.releaseStartTime > block.timestamp){
                lockedValue = lockedValue.add(acc.releaseDays.mul(acc.unitValue).add(acc.extraValue));
                continue;
            }

            uint256 pastDays = block.timestamp.sub(acc.releaseStartTime).div(unixDayTime).add(1);

            if(acc.releaseDays > pastDays){
                uint256 leftDays = acc.releaseDays.sub(pastDays);
                lockedValue = lockedValue.add(acc.unitValue.mul(leftDays).add(acc.extraValue));
            }
        }
        return (balanceOf(_holder), lockedValue);
    }
    ...
    function releaseLock(address _holder) internal {
        ...
        for(uint256 i = 0; i < lockInfo[_holder].length; i++) {
            ...
            uint256 pastDays = block.timestamp.sub(acc.releaseStartTime).div(unixDayTime).add(1);
            if(acc.releaseDays > pastDays){
                lockInfo[_holder][i].releaseStartTime = lockInfo[_holder][i].releaseStartTime.add(pastDays.mul(unixDayTime));
    ...
    function lockState(address _holder, uint256 _idx) public view returns (uint256, uint256, uint256, uint256) {
        return (lockInfo[_holder][_idx].releaseStartTime, lockInfo[_holder][_idx].releaseDays, lockInfo[_holder][_idx].unitValue, lockInfo[_holder][_idx].extraValue);
    }

    function lock(address _holder, uint256 _value, uint256 _releaseTime) public onlyOwner {
        require(super.balanceOf(_holder) >= _value, "Not enough balance");
    ...
    function dailyLock(address _holder, uint256 _totalValue, uint256 _releaseDays, uint256 _releaseStartTime) public onlyOwner {
        require(_totalValue > 0, "Invalid totalValue");
        require(_totalValue <= super.balanceOf(_holder), "Not enough balance");
        require(_releaseDays > 0 && _releaseDays <= 400, "Invalid releaseDays (0 < releaseDays <= 400");
    ...
    function transferWithDailyLock(address _to, uint256 _totalValue, uint256 _releaseDays, uint256 _releaseStartTime) public onlyOwner returns (bool) {
        require(_to != address(0), "Invalid address");
        require(_totalValue > 0, "Invalid totalValue");
        require(_totalValue <= super.balanceOf(owner), "Not enough balance");
        require(_releaseDays > 0 && _releaseDays <= 400, "Invalid releaseDays (0 < releaseDays <= 400)");
