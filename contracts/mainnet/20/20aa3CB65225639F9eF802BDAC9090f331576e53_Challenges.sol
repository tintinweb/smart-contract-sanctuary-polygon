...
contract Challenges is
    ...
    SerializeBuyNowGet
{
    // Addresses of the external contracts that this contract uses:
    address private immutable _sto;
    address private immutable _writer;
    address private immutable _info;
    address private immutable _freezelib;
    address private immutable _completelib;
    address private immutable _buyNowlib;
    ...
    function challVerifiableOnChain(
        ...
        // or by the fact that the asset belongs to a different blackholeId
        // or by an untradable mark set on asset creation
        isTXValid =
            isTXValid &&
            IInfo(_info).hasCorrectBlackholeId(assetId) &&
