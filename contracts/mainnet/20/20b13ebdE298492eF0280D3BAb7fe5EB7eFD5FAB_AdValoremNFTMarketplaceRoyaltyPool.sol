...
contract AdValoremNFTMarketplaceRoyaltyPool is ReentrancyGuard, OwnableUpgradeable {
    ...
    function getInfoFromMarket(
        ...
        require(
            marketPlaceContract == _marketOwner,
            "RoyaltyPool: marketowner not authorized"
        );

        totalRewardSupply = totalRewardSupply + _royaltyAmount;
        uint256 businessOwnerAmount = (_royaltyAmount * businessOwnerPercent) /
            10000;
    ...
    function pendingReward(address _address) public view returns (uint256) {
        ...
        uint256 pendingUserAdValorem = (user.amount * (rewardDuration) * apr) /
            PRECISION /
            YEAR_TIME;
    ...
    function withdraw() external nonReentrant {
        ...
        require(
            block.timestamp >= user.timeDeposited + lockDuration,
            "withdraw: Time locked"
        );
        require(
            user.amount >= 0,
            "Royalty Pool withdraw: don't have vlr token"
        );
        require(
            totalRewardSupply >= user.amount + rewardAmount,
            "withdraw: Should charge reward token"
        );

        if (user.amount > 0) {
            totalRewardSupply = totalRewardSupply - user.amount - rewardAmount;
            _claimPendingReward(address(msg.sender));
            IBEP20(vlrTokenContract).safeTransfer(
                address(msg.sender),
                user.amount
            );
        }
    ...
    function emergencyWithdraw() external nonReentrant {
        RoyaltyPoolInfo storage user = userPoolInfo[msg.sender];
        require(
            totalRewardSupply >= user.amount,
            "emergencyWithdraw: Should charge total reward token"
        );
    ...
    function setRoyaltiesPercent(
        uint256 _businessOwnerPercent,
        uint256 _buyerPercent,
        uint256 _marketOwnerPercent
    ) public onlyOwner {
        require(
            _businessOwnerPercent + _buyerPercent + _marketOwnerPercent ==
                10000,
            "Royalty Pool: The sum of all percentages should be 100%"
        );
    ...
    function setLockDuration(uint256 _lockDuration) public onlyOwner {
        require(
            _lockDuration >= 24 * 60 * 60,
            "Royalty Pool: Minium should be greater than one day"
        );
