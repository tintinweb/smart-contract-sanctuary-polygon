...
contract DIARequestBaseOracle is Ownable {
    ...
    mapping(string => Message) public values; // call using symbol
    mapping(string => Message) public timebasedvalues; // call using asset-blockchain-timestamp
    ...
    function setValue(Message memory value) external {
        require(getSigner(value) == signer);
        values[value.Symbol] = value;

        string memory timeString = value.TimeRequest.toString();

        string memory timekey = string(
            abi.encodePacked(
                value.Blockchain,
                "-",
                _toString(value.Address),
                "-",
                timeString
            )
        );
        ...
        timebasedvalues[timekey] = value;
        timebasedvalues[value.Symbol] = value;
    ...
    // Internal function to convert address to string
