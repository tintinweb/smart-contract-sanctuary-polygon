 ...
 */
contract OddzPrediction is IOddzPrediction, AccessControl, Pausable, ReentrancyGuard, KeeperCompatibleInterface {
     ...
     * @dev Callable by Operator
     */
    function executeRound() public override whenNotPaused {
        require(
    ...
    /*
    * Implement Keeper compatible methods
    * to check/execute rounds
    */
    function checkUpkeep(bytes calldata) external view override 
            returns (bool upkeepNeeded, bytes memory /* performData */) {
            
            Round memory round = oddzPredictionStorage.getPredictionRound(currentRoundId);
            upkeepNeeded = (block.timestamp - round.betEndTimestamp) > round.betEndTimestamp;
    }

     function performUpkeep(bytes calldata) external override {
        Round memory round = oddzPredictionStorage.getPredictionRound(currentRoundId);
        //We highly recommend revalidating the upkeep in the performUpkeep function
        if ((block.timestamp - round.betEndTimestamp) > round.betEndTimestamp) {
            executeRound();
        }
    }
}
