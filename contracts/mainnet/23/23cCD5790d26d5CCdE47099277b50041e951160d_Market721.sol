...
contract Market721 is OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    //upgrade
    mapping(uint => uint) public lowerLimit;
    mapping(uint => mapping(uint => uint)) public lowerLimits;
    ...
    function setLowerLimit(uint types_, uint tradeType_, uint limit_) external onlyOwner {
        lowerLimits[types_][tradeType_] = limit_;
    }
    ...
    function sell(uint goodsType_, uint tokenId_, uint tradeType_, uint price_) public {
        ...
        if (goodsType_ == 1) {
            bool isCreation = ICOW(address(goodsInfos[goodsType_].place)).isCreation(tokenId_);
            bool isAdult = ICOW(address(goodsInfos[goodsType_].place)).getAdult(tokenId_);
            require(isCreation || !isAdult, 'not allowed');
            if (!isAdult) {
                require(price_ >= 150e6, 'Price');
            } else {
                require(lowerLimits[goodsType_][tradeType_] <= price_, "Price");
            }
        } else {
            require(lowerLimits[goodsType_][tradeType_] <= price_, "Price");
        }
