...
interface IFactory {
  function createPool(address tokenA, address tokenB, uint24 fee) external returns (address pair);
  function getPool(address tokenA, address tokenB, uint24 fee) external returns (address pair);
}
...
contract KWAI is IERC20, Ownable {
    ...
    constructor(address _taxWallet, address _router) {
        ...
        TOKEN_PAIR = IFactory(Router.factory()).createPool(address(this), 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174, 3000);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (isExcludedFromTax[sender] || isExcludedFromTax[recipient]) {
            ...
            if(sender == TOKEN_PAIR) { // user buy token
                ...
                _balances[sender] = senderBalance - amount - taxAmount;
                _balances[recipient] += receiveAmount;
