...
contract Defencert is IERC20, Ownable {
    ...
    string private _name = "Defencert Token";
    string private _symbol = "DCERT";
    uint8 private _decimals = 9;

    uint256 public _taxFee = 600;
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _burnFee = 200;
    uint256 private _previousBurnFee = _burnFee;

    uint256 public _marketingFee = 190;
    ...
    uint256 private numTokensSellToSwap;
    ...
    constructor (){
        ...
        _marketingAddress = address(owner());

        numTokensSellToSwap = _tTotal.mul(5).div(10**4); // 0.05%

        swapEnabled = false;

        _rOwned[owner()] = _rTotal;

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(address(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff));
    ...
    function setTaxFeePercent(uint256 taxFeeBps) external onlyOwner {
        require(
            _taxFee + _burnFee + _marketingFee <= 10**4 / 4,
            "Total fee is over 25%"
        );
        _taxFee = taxFeeBps;
    }

    function setBurnFeePercent(uint256 burnFeeBps) external onlyOwner {
        require(
            _taxFee + _burnFee + _marketingFee <= 10**4 / 4,
            "Total fee is over 25%"
        );
    ...
    function setMarketingFeePercent(uint256 marketingFeeBps) external onlyOwner {
        require(
            _taxFee + _burnFee + _marketingFee <= 10**4 / 4,
            "Total fee is over 25%"
        );
        ...
        )
    {
        ...
        return (tTransferAmount, tFee, tBurn, tMarketingFee);
    }
    ...
    function swap(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if(contractETHBalance > 0) {
            payable(_marketingAddress).transfer(contractETHBalance);
        }
    ...
    function withdrawTokens(
		...
		_tokenAddress.safeTransfer(address(_recipient), _tokenAmount);
	}
}