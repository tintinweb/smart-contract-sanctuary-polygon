...
contract PLGDepositorV3 is permission {
    ...
    function unlockLevelWithPLG(address account) public returns (bool) {
        ...
        IUnlocker(factory.getAddr("plg_refreward")).updateVIPBlockWithPermit(account,type(uint256).max);
        return true;
    }
    ...
    function depositWithPLG(address account,address referral,uint256 amount,uint256 slippage,uint256 denominator) public noReentrant() returns (bool) {
        ...
        IDepositor(factory.getAddr("plg_depositETH")).depositWithMATIC{ value: amount }(account,referral);
