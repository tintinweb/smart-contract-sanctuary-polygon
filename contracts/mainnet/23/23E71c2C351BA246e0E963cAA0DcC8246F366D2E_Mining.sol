...
contract Mining is Ownable, ReentrancyGuard {
    ...
    function getBaseInfos() external view returns(uint256[] memory, string[] memory, address[] memory){
        uint256[] memory array = new uint256[](7);
        array[0] = stakedTokenSupply;
        array[1] = intervalPerBlock;
        array[2] = rewardPerBlock;
        array[3] = startTime;
        array[4] = endTime;
        array[5] = stakedToken.decimals();
        array[6] = rewardToken.decimals();
        string[] memory strs = new string[](2);
        strs[0] = stakedToken.symbol();
        strs[1] = rewardToken.symbol();
        address[] memory addresses = new address[](2);
        addresses[0] = address(stakedToken);
        addresses[1] = address(rewardToken);
        return (array, strs, addresses);
    }
    ...
    function getUserInfos(address account) external view returns(uint256[] memory){
        ...
        array[0] = user.amount;
        array[1] = pendingReward(account);
        array[2] = stakedTokenSupply > 0 ? rewardPerBlock.mul(user.amount).div(stakedTokenSupply) : 0;
        array[3] = stakedToken.balanceOf(account);
        array[4] = stakedToken.allowance(account, address(this));
        return array;
    }
}