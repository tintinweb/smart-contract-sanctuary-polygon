...
abstract contract VaultConfigAccessRequest
{
    ...
    function getAccessManger() internal virtual view returns (IMoonAccessManager);
    ...
    {
        getAccessManger().requireAdmin(msg.sender);
    }

    function requireManager() internal view
    {
        getAccessManger().requireManager(msg.sender);
    }

    function requireDeployer() internal view
    {
        getAccessManger().requireDeployer(msg.sender);
    }

    function requireSecurityAdmin() internal view
    {
        getAccessManger().requireSecurityAdmin(msg.sender);
    }

    function requireSecurityMod() internal view
    {
        getAccessManger().requireSecurityMod(msg.sender);
    }
}
...
{    
    ...
    ///@notice total alloc points of the pool's chef
    ...
    ///@notice reward emission of the pool's chef
    ...
    function getAccessManger() internal override view returns (IMoonAccessManager)
    {
...
{    
    ...
    //shares
    uint256 public override totalPayoutShares;
    uint256 public override totalCompoundShares;
    ...
    function getPayoutAndCompoundShare(uint256 _layer) private view returns (uint256 compoundShares, uint256 payoutShares)
    {
    ...
    function withdrawFromLayer(address _user, uint256 _shares) internal
    {
        ...
        UserInfo storage user = users[_user];
        LayerInfo storage layer = layers[user.layer];        
        uint256 amount = layerShareToAmount(user.layer, _shares);
    ...
    {   
        ...
        for (uint256 n = layerStepSize; n <= 100; n += layerStepSize)
        {
            ...
            if (shares != 0)
            {
                layerSharesGained = (sharesGained * shares) / totalCompoundShares;
...
///@notice manage user balances and payouts
contract VaultV2 is
