...
contract CharacterMinter  is ICharacter,  ERC721Enumerable, Ownable, VRFConsumerBase, Pausable {
  ...
  */
  ...
  function mint(uint256 amount) external payable whenNotPaused{
    ...
    require(LINK.balanceOf(address(this)) >= 0.0001 ether, "Not enough LINK - fill contract with faucet");
    require(minted + amount <= MAX_TOKENS, "All tokens minted");
    require(amount > 0 && amount <= 10, "Invalid mint amount");
    if(_msgSender() == owner()) 
    {
      // Free mint for owner/ Ensuring giveaways and giving tokens to old contract owner. Salvage contract to be deployed also. Only available during gen.0 // first 10k
    }
  ...
  // Change generator
  function changeGenerator(address payable _generator) external onlyOwner{
    generator = Generator(_generator);
  }
  ...
  function revealToken(uint16 tokenId) public whenNotPaused returns (Character memory t) {
    ...
    t = generator.selectTraits(tokenSeeds[tokenId], tokenId);

    //For preventing overlaping between gen.0 and gen.1. Since gen.0 has 100 bonus point, this may show same traits different since there is a inherited gen. bonus.
    if(tokenId <= 10000)
    {
      t.charisma -= 100;
    }
