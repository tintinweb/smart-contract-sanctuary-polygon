...
contract DefinitiveStakingManager is Ownable {
  ...
  uint256 public feePct = 0; // Fee percentage to be payed to the fee account
   ...
   */
  function deposit(uint256 _underlyingAmount, uint8 _index) external {
    require(_underlyingAmount > 0, "Deposit amount can't be zero");
    ...
    uint256 newLPTokens = depositIntoDefinitive(_underlyingAmount, _index);
  ...
  function withdraw(uint256 _lpTokenAmount, uint8 _index) public {
    ...
    // Update strategy
    // TODO @cstoneham - double check this
   ...
   */
  function withdrawAll(uint8 _index) external {
    uint256 lpTokenAmount = strategyStakers[msg.sender].amount;
    withdraw(lpTokenAmount, _index);
  }
