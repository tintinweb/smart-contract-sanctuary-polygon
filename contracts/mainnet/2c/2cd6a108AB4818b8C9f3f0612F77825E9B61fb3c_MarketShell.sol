...
// File: interfaces/IMarketApproval.sol


pragma solidity ^0.8.11;

interface IMarketApproval {
    function safeTransfer_721(address tokenAddress, address from, address to, uint256 tokenId) external;

    function safeTransfer_1155(
...
interface IMarketAdvance {
    function setCurrentAdvanceByToken(address user, address tokenAddress, uint256 tokenId, uint256 value) external;
    function getCurrentAdvanceByToken(address user, address tokenAddress, uint256 tokenId) external view returns(uint256);
}
...
contract MarketShell is AccessControlEnumerable {
    ...
    IMarketApproval private _marketApproval;
    bool private _canMint = true;

    string public constant VERSION = "2";

     constructor(address _paymentAccount_address, address _mys_address, address _advance_address, address _whitelist_address, address _itemCollection_address, address _marketAdmin_address, address _marketApproval_address) {
         ...
         _marketApproval = IMarketApproval(_marketApproval_address);
         _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    function getAddresses() view public onlyRole(DEFAULT_ADMIN_ROLE) returns (address, address, address, address, address, address, address) {
        return (address(_paymentAccount), address(_mys_nft), address(_marketAdvance), address(_whitelistCollectors), address(_itemCollection), address(_marketAdmin), address(_marketApproval));
    }

    function updateAddresses(address paymentAccount, address mys_nft, address marketAdvance, address whitelist, address itemCollection, address marketAdmin, address marketApproval) public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (marketApproval != address(0)) {
            _marketApproval = IMarketApproval(marketApproval);
        }
    }

    function canMint(bool _minting) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _canMint = _minting;
        emit MintingAvailable(_minting, _msgSender());
    }
    ...
    function mint(string calldata _uri, uint256 _amount, address payable[] calldata receivers, uint16[] calldata basisPoints, uint256 releaseDate) public {
         require(!_marketAdmin.checkBlacklist(_msgSender()), "MarketAdmin: Blacklisted user");
        _mint(_msgSender(), _uri, _amount, receivers, basisPoints, releaseDate);
    }

    function mint_admin(address _user, string calldata _uri, uint256 _amount, address payable[] calldata receivers, uint16[] calldata basisPoints, uint256 releaseDate) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _mint(_user, _uri, _amount, receivers, basisPoints, releaseDate);
    }

    function _mint(address _user, string calldata _uri, uint256 _amount, address payable[] calldata receivers, uint16[] calldata basisPoints, uint256 releaseDate) private {
        require(_canMint, "Minting not available");
        uint256 tokenId = _mys_nft.mint(_user, _uri, _amount, receivers, basisPoints, releaseDate, "0x00");
        emit MintedToken(tokenId, address(_mys_nft), _user);
    }
    ...
    function addToken_admin(address _seller, address _tokenAddress, uint256 _tokenId, uint256 _price, address erc20, uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256) {
        return _addToken(_seller, _tokenAddress, _tokenId, _price, erc20, amount);
    }
    ...
    function addToken(address _tokenAddress, uint256 _tokenId, uint256 _price, address erc20, uint256 amount) public returns (uint256) {
        require(!_marketAdmin.checkBlacklist(_msgSender()), "MarketAdmin: Blacklisted user");
        return _addToken(_msgSender(), _tokenAddress, _tokenId, _price, erc20, amount);
    }

    function _addToken(address _seller, address _tokenAddress, uint256 _tokenId, uint256 _price, address erc20, uint256 amount) private returns (uint256) {
        require(amount > 0, "Amount value must be > 0");
        if (IERC165(_tokenAddress).supportsInterface(type(IERC1155).interfaceId)) {
            return _add_erc_1155(_tokenAddress, _tokenId, _price, erc20, _seller, amount);

        } else if (IERC165(_tokenAddress).supportsInterface(type(IERC721).interfaceId)) {
            require(amount == 1, "ERC721: amount must be = 1");
            return _add_erc_721(_tokenAddress, _tokenId, _price, erc20, _seller);
        }
    ...
    function buyERC20_admin(address _buyer, uint256 itemId, uint256 exchangeRate) public onlyRole(DEFAULT_ADMIN_ROLE) {
         _buyERC20(_buyer, itemId, exchangeRate);
    }

    function buyERC20(uint256 itemId, uint256 exchangeRate) public {
        _buyERC20(_msgSender(), itemId, exchangeRate);
    }

    function _buyERC20(address _buyer, uint256 itemId, uint256 exchangeRate) private {
        (ItemInfo memory item, ) = _itemBy(itemId);
        require (IERC20(item.erc20Address).allowance(_buyer, address(this)) >= item.price, "Market: allowance too low");
        userPurchaseAbilityCheck(_buyer, item, 1);
        _buy(item, _buyer, 1, item.price, exchangeRate);
    }

    function buy(uint256 itemId, uint256 exchangeRate) public payable {
        (ItemInfo memory item, ) = _itemBy(itemId);
        ...
        _buy(item, _msgSender(), 1, msg.value, exchangeRate);
    }

    function buyTokens(uint256 itemId, uint256 amount, uint256 exchangeRate) public payable {
        (ItemInfo memory item, ) = _itemBy(itemId);
        require(item.erc20Address == address(0), "Market: Invalid currency");
        ...
        _buy(item, msg.sender, amount, msg.value, exchangeRate);
    }
    ...
    function _buy(ItemInfo memory item, address _sender, uint256 _tokenCount, uint256 _amount, uint256 exchangeRate) private {
        require(item.amount >= _tokenCount, "Market: Not enough tokens");

        if (IERC165(item.tokenAddress).supportsInterface(type(IMYS_ERC1155).interfaceId)) {
            _buy_MYS_ERC_1155(item, IERC1155(item.tokenAddress), _amount, _tokenCount, _sender, exchangeRate);
        } else if (IERC165(item.tokenAddress).supportsInterface(type(IERC1155).interfaceId)) {
            _buy_ERC1155(item, IERC1155(item.tokenAddress), _amount, _tokenCount, _sender, exchangeRate);
        } else if (IERC165(item.tokenAddress).supportsInterface(type(IERC721).interfaceId)) {
            require(item.price <= _amount, "Market: Too low price");
            _buy_ERC721(item, IERC721(item.tokenAddress), _amount, _sender, exchangeRate);
        } else {
            revert("Unsupported token");
        }
    }

    function calculateAndUpdateAdvance(address user, address tokenAddress, uint256 tokenId, uint256 amountInEth, uint256 exchangeRate) private returns (uint256 toPlatform, uint256 toUser) {
        uint256 advanceInDollars = _marketAdvance.getCurrentAdvanceByToken(user, tokenAddress, tokenId);
        if (advanceInDollars == 0) {
            return (0, amountInEth);
        }
        uint256 advanceInEth = advanceInDollars / exchangeRate;

        if (advanceInEth < amountInEth) {
            uint256 ethToUser = amountInEth - advanceInEth;
            _marketAdvance.setCurrentAdvanceByToken(user, tokenAddress, tokenId, 0);
            return (advanceInEth, ethToUser);
        }
        uint256 _newAdvance = (advanceInEth - amountInEth) * exchangeRate;
        _marketAdvance.setCurrentAdvanceByToken(user, tokenAddress, tokenId, _newAdvance);
        return (amountInEth, 0);
    }

    function _buy_ERC721(ItemInfo memory item, IERC721 nft, uint256 _amount, address _buyer, uint256 exchangeRate) private {
        address payable platformAddress =  _paymentAccount.getPlatformRecipient();
       
        (uint256 toPlatform, uint256 toUser) = calculateAndUpdateAdvance(item.owner, item.tokenAddress, item.tokenId, _amount, exchangeRate);

        if (toUser > 0) {
            uint256 platformFee = toUser * uint256(_paymentAccount.getPrimaryPlatformFee()) / 10_000;
            payment(item, _buyer, platformAddress, toPlatform + platformFee, "Market: Platform - failed to send ETH");
            payment(item, _buyer, payable(nft.ownerOf(item.tokenId)), toUser - platformFee, "Market: Seller - failed to send ETH");
        } else {
             payment(item, _buyer, platformAddress, toPlatform, "Market: Platform - failed to send ETH");
        }

        if(nft.isApprovedForAll(item.owner, address(_marketApproval))) {
            _marketApproval.safeTransfer_721(item.tokenAddress, item.owner, _buyer, item.tokenId);
        } else {
    ...
    function _buy_ERC1155(ItemInfo memory item, IERC1155 nft, uint256 _amount, uint256 _tokenCount, address _buyer, uint256 exchangeRate) private {
        
        (uint256 toPlatform, uint256 toUser) = calculateAndUpdateAdvance(item.owner, item.tokenAddress, item.tokenId, _amount, exchangeRate);

        if (toUser > 0) {
            uint256 platformFee = toUser * uint256(_paymentAccount.getPrimaryPlatformFee()) / 10_000;
            payment(item, _buyer,  _paymentAccount.getPlatformRecipient(), toPlatform + platformFee, "Market: Platform - failed to send ETH");
            payment(item, _buyer, payable(item.owner), toUser - platformFee, "Market: Seller - failed to send ETH");
        } else {
            payment(item, _buyer,  _paymentAccount.getPlatformRecipient(), toPlatform, "Market: Platform - failed to send ETH");
        }
     
        require(nft.balanceOf(item.owner, item.tokenId) >= _tokenCount, "ERC1155: insufficient funds");
        _itemCollection.changeItemAmount(item.itemId, item.amount - _tokenCount);

        if (nft.isApprovedForAll(item.owner, address(_marketApproval))) {
            _marketApproval.safeTransfer_1155(item.tokenAddress, item.owner, _buyer, item.tokenId, _tokenCount, "0x00");
        } else {
    ...
    function _buy_MYS_ERC_1155(ItemInfo memory item, IERC1155 nft, uint256 _amount, uint256 _tokenCount, address _buyer, uint256 exchangeRate) private {
        ...
        (uint256 toPlatform, uint256 toUser) = calculateAndUpdateAdvance(item.owner, item.tokenAddress, item.tokenId, _amount, exchangeRate);   
        if (toUser > 0) {
            (, uint256 platformRoyaltyAmount) = SystemWideRoyalties_ERC1155(item.tokenAddress).platformRoyaltyInfo(item.owner, item.tokenId, toUser);
            ...
            _mysteriousPaymentShareOut(item, SystemWideRoyalties_ERC1155(item.tokenAddress), toUser - platformRoyaltyAmount, _buyer);
        } else {
            payment(item, _buyer, _paymentAccount.getPlatformRecipient(), toPlatform, "Market: Platform - failed to send ETH");
        }

        require(nft.balanceOf(item.owner, item.tokenId) >= _tokenCount, "Market: insufficient funds");
        if (nft.isApprovedForAll(item.owner, address(_marketApproval))) {
            _marketApproval.safeTransfer_1155(item.tokenAddress, item.owner, _buyer, item.tokenId, _tokenCount, "0x00");
        } else {
            nft.safeTransferFrom(item.owner, _buyer, item.tokenId, _tokenCount, "0x00");
    ...
    function _removeItem(uint256 _itemId, address _sender) private {
     ...
     function _add_erc_1155(address _tokenAddress, uint256 _tokenId, uint256 _price, address erc20, address sender, uint256 amount) private returns (uint256) {
        ...
        require(nft.isApprovedForAll(sender, address(this)) || nft.isApprovedForAll(sender, address(_marketApproval)), "ERC1155: isApprovedForAll error");
    ...
    function _add_erc_721(address _tokenAddress, uint256 _tokenId, uint256 _price, address erc20, address sender) private returns (uint256) {
        ...
        require(nft.getApproved(_tokenId) == address(this) || 
                ...
                nft.getApproved(_tokenId) == address(_marketApproval) ||
                nft.isApprovedForAll(msg.sender, address(_marketApproval))
