...
contract NUGFT is ERC721Enumerable, Ownable {
    ...
    uint256 public cost = 75 ether; // cost for other than owners to mint change using setCost fn for presale
    ...
    address[] public airgrabAddresses;
    mapping(address => uint8) private _airgrabCount;
    /**
     * @dev Initializes the contract setting the deployer as the initial owner.
     */
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        require(msg.value >= cost * _mintAmount, "pay for it please");

        if(presaleLive == true) {
            require(supply < 501, "Presale sold out");
    ...
    function mint2(uint256 _mintAmount) public onlyOwner {
      uint256 supply = totalSupply();
      require(!paused);
      require(_mintAmount > 0);
      require(supply + _mintAmount <= maxSupply);
    ...
    function airgrab() public {
      uint256 supply = totalSupply();
      require(!paused);
      require(supply + 1 <= maxSupply);
      require(isEligible(msg.sender), "user is not eligible for airgrab");
      require(_airgrabCount[msg.sender] == 0 , "Airgrab already availed");

      _safeMint(msg.sender, supply + 1);
      _airgrabCount[msg.sender] += 1;
     }
         

    function isEligible(address _user) public view returns (bool) {
        for (uint i = 0; i < airgrabAddresses.length; i++) {
            if (airgrabAddresses[i] == _user) {
            return true;
            }
        }
        return false;
  ...
  function AirgrablistUsers(address[] calldata _users) public onlyOwner {
    delete airgrabAddresses;
    airgrabAddresses = _users;
  }
    /**
     * @dev add address to presale whitelist.
     */
  function addToAirgrabList(address _user) public onlyOwner {
    require(_user != address(0), "Cannot add zero address");
    airgrabAddresses.push(_user);
  }
