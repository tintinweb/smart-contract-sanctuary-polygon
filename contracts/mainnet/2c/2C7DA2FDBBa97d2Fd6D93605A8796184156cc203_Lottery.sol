...
contract Lottery is VRFConsumerBaseV2, ILottery, Roles {
    ...
    uint8 decimals = 6;
    address private constant OFFICIAL_AGENCY = address(1989);
    ...
    mapping(address => bool) private _double;
    ...
    function _updateRecommender(address _binder, address _recommender) internal {
        ...
        _double[_binder] = true;

        UserAccount storage alice = _accounts[_binder];
        if (_recommender != OFFICIAL_AGENCY) {
            alice.recommenders.push(_recommender);

            UserAccount memory boba = _accounts[_recommender];
            for (uint i; i < boba.recommenders.length; i++) {
                address addr = boba.recommenders[i];
                alice.recommenders.push(addr);
            }
        } else {
            alice.topAgent = true;
        }
    ...
    function _instillPool(uint _value) private {
        ...
        if (len != 0 && !account.topAgent) {
            uint base = 4 * 10 ** (decimals - 3);
            uint step = 1 * 10 ** (decimals - 3);
            for (uint i; i < account.recommenders.length; i++) {
                UserAccount storage ua = _accounts[account.recommenders[i]];
                uint rate = Utils.getPromoteRate(ua.accAgencyIncome);
                uint profit = (_value / ONE_UINTS * base * rate) / 10;
...
library Utils {
    ...
    function getPromoteRate(uint _value) internal pure returns (uint) {
        if (_value < 500 * 10 ** 6) return 10;
        return _value >= 1_000 * 10 ** 6 ? 30 : 15;
    }
