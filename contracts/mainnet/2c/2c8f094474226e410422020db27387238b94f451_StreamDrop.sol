...
contract StreamDrop is AccessControl {
    ...
    function changeTreasuryStream(address newAddress) public onlyCREATOR {
        Stream memory stream;
        stream.id = numStreams;
        stream.owner = newAddress;
        stream.balance = streamForAddress[treasuryAddress].balance;
        stream.initialBalance = streamForAddress[treasuryAddress].initialBalance;
        stream.startTime = streamForAddress[treasuryAddress].startTime;
        stream.endTime = streamForAddress[treasuryAddress].endTime;
        allStreams.push(stream);
        streamForAddress[newAddress] = stream;
        numStreams++;
        streamForAddress[treasuryAddress].balance = 0;
    }
