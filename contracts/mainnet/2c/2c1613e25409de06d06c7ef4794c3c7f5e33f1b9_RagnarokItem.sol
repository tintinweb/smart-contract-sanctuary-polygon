    ...
    UUPSUpgradeable
{
    ...
    mapping(uint16 => uint256) private _itemTypes;
    mapping(uint16 => uint16[]) private _itemsTypesByRarity;
    mapping(address => EnumerableSetUpgradeable.UintSet) private _ownerItemIds;
    address public randomOracleAddress;
    address public skullAddress;
    mapping(address => CombineEvent) public combineEvents;

    function combineItems(uint16[4] memory ids) external {
        require(combineEvents[msg.sender].rarity == 0, "already combining");
        uint256 baseRarity = SharedLib.representationToItemType(_itemTypes[ids[0]]).rarity;
        require(baseRarity < 4, "not legendaries");

        uint256[] memory _ids = new uint256[](4);
        uint256[] memory amounts = new uint256[](4);
        for (uint256 index = 0; index < ids.length; index++) {
            uint16 id = ids[index];
            require(baseRarity == SharedLib.representationToItemType(_itemTypes[id]).rarity, "invalid rarities");
            _ids[index] = id;
            amounts[index] = 1;
        }

        _burnBatch(msg.sender, _ids, amounts);
        IRagnarokERC20(skullAddress).burnFrom(msg.sender, baseRarity * 300 ether);

        bytes32 requestId = IRandomOracle(randomOracleAddress).requestRandomNumber();
        combineEvents[msg.sender] = CombineEvent({
            rarity: uint16(baseRarity + 1),
            revealBlock: block.number + 60,
            requestId: requestId
        });
    }

    function claimCombine() external {
        CombineEvent memory combineEvent = combineEvents[msg.sender];
        require(combineEvent.rarity != 0 && combineEvent.revealBlock < block.number, "not allowed");
        uint256 randomSeed = IRandomOracle(randomOracleAddress).getRandomNumber(combineEvent.requestId);
        delete combineEvents[msg.sender];
        _mintItem(msg.sender, _getRandomItemId(combineEvent.rarity, randomSeed), 1);
    }
    ...
    function mintRandom(
        ...
        _mintItem(to, _getRandomItemId(rarity, randomSeed), 1);
    }

    function _getRandomItemId(uint16 rarity, uint256 randomSeed) internal view returns (uint16) {
        uint256 index = _getRandomNumber(randomSeed, _itemsTypesByRarity[rarity].length);
        return _itemsTypesByRarity[rarity][index];
    }
    ...
    function _mintItem(
        ...
        require(_itemTypes[id] != 0, "item does not exist");
    ...
    function registerItemType(
        ...
        uint256[6] memory classes
    ) external {
        require(_auth[msg.sender], "not allowed");
        SharedLib.ItemType memory itemType = SharedLib.ItemType({
            rarity: rarity,
        ...
        if (_itemTypes[id] != 0) {
            require(rarity == SharedLib.representationToItemType(_itemTypes[id]).rarity, "can't change rarity");
        }

        _itemTypes[id] = SharedLib.itemTypeToRepresentation(itemType);
        _itemsTypesByRarity[uint16(rarity)].push(id);
    }

    function _getRandomNumber(uint256 seed, uint256 limit) internal view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(seed, msg.sender))) % limit;
    }
    ...
    function getItemsByRarity(uint16 rarity) external view returns (uint16[] memory) {
        ...
        return _itemsTypesByRarity[rarity];
    }

    function allBalancesOf(address owner) external view returns (uint256[] memory, uint256[] memory) {
        uint256[] memory ids = _ownerItemIds[owner].values();
    ...
    function previewCombine() external view returns (uint16) {
        CombineEvent memory combineEvent = combineEvents[msg.sender];
        uint256 randomSeed = IRandomOracle(randomOracleAddress).getRandomNumber(combineEvent.requestId);
        require(combineEvent.revealBlock < block.number, "not ready");
        require(randomSeed > 0, "random seed not ready");
        return _getRandomItemId(combineEvent.rarity, randomSeed);
    }

    // OWNER FUNCTIONS

    function setAuth(address addr, bool isAuth) external onlyOwner {
        _auth[addr] = isAuth;
    }
    ...
    function setSkullAddress(address _skullAddress) external onlyOwner {
