...
contract Goblintown is ERC721A, Ownable {
  ...
  uint256 public MaxWhitelist = 2000;
  uint256 public amountForDevs = 200;
  uint256 public WhitelistSaleStartTime;
  uint256 public publicSaleStartTime;
  uint256 public publicPrice = 12e16;   //0.12 ETH
  uint256 public whitelistPrice = 8e16; //0.08 ETH
  ...
  constructor(
    ...
    uint256 collectionSize_
  ) ERC721A("Goblintown", "GOBLIN", maxPerBatchMintSize_, collectionSize_) {
  ...
  function CheckWhitelistPrice() public view returns (uint256) {
  ...
  function devMintBatch(address[] memory _Addrs, uint numberOfTokens) public onlyOwner {
    uint _addressAmount = _Addrs.length;
    for(uint i = 0; i < _addressAmount; i++) {
      _safeMint(_Addrs[i], numberOfTokens);
    }
  }
  ...
  function setTimes(uint timeWhitelist, uint timePublic) external onlyOwner {
    ...
    onlyOwner
  {
    ...
    require(
      amountForWhitelist + numSlots[i] <= MaxWhitelist,
  ...
  function CheckamountForWhitelist() public view returns (uint256) {
    return amountForWhitelist;
  }
