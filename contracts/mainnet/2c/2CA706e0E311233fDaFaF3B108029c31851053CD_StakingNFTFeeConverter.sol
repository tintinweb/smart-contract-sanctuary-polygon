...
contract StakingNFTFeeConverter  {
    ...
    event StakingReward(uint256 _timestamp, uint256 _wmaticAmount);
    ...
    function setDistribution() external keeper {
        ...
        _safeTransfer(wmatic, masterchef, _balance);
    ...
    function swap() external keeper {
        ...
        _balance = IERC20(wmatic).balanceOf(address(this));
        if(_balance > 0){
            _safeTransfer(wmatic, masterchef, _balance);
            IMasterchef(masterchef).setDistributionRate(_balance);
        }
    ...
    function setToken(address pair) internal {
        ...
        if(_token0 != wmatic && isToken[_token0] == false){
        ...
        if(_token1 != wmatic && isToken[_token1] == false){
