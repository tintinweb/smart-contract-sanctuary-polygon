    ...
    Pausable
{
    ...
    //---------- Contracts ----------//
    IERC20Burnable public immutable SWC;
    ...
    constructor(
        ...
        address _swcVault,
        address _dexRouter,
        address _keeperRegistery
    ) {
        SWC = IERC20Burnable(_swc);
        ROUTER = IJamonRouter(_dexRouter);
        BANK = IShibaBank(_bank);
        SWC.approve(address(ROUTER), ~uint256(0));
    ...
    function _processRewards(uint256 amount) internal {
        uint256 toBurn = amount.mul(burnFee).div(10000);
    ...
    function _distributeRewards() internal {
        ...
        uint256 swcBalance = SWC.balanceOf(address(this));
        if (swcBalance >= 100) {
            _processRewards(swcBalance);
        }
    }

    //----------- External Functions -----------//
    function canDistribute() public view returns (bool) {
        return BANK.hasProfits() && !paused();
    }
    ...
    function setInterval(uint256 _interval) external onlyOwner {
        require(_interval >= 1 days && _interval <= 14 days);
        interval = _interval;
    }

    function setBurnFee(uint256 _burnFee) external onlyOwner {
        require(_burnFee > 0 && _burnFee < 10000);
        burnFee = _burnFee;
    }

    function setSwcVault(address _newSwcVault) external onlyOwner {
        require(_newSwcVault != address(0));
        swcVault = _newSwcVault;
    }
