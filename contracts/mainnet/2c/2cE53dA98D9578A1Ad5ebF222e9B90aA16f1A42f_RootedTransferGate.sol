...
{   
    ...
    constructor(address _dev, ILiquidityLockedERC20 _rootedToken, IPancakeRouter02 _pancakeRouter)
    {
        dev = _dev;
    ...
    modifier isController(){
        require(msg.sender == owner || msg.sender == dev, "Only controller or owner can do this");
        _;
    }

    function setUnrestrictedController(address unrestrictedController, bool allow) public isController()
    {
        unrestrictedControllers[unrestrictedController] = allow;
    }

    function setFreeParticipantController(address freeParticipantController, bool allow) public isController()
    {
        freeParticipantControllers[freeParticipantController] = allow;
    }

    function setFeeControllers(address feeController, bool allow) public isController()
    {
        feeControllers[feeController] = allow;
    }

    function setFeeSplitter(address _feeSplitter) public isController()
    {
    ...
    {
        require (msg.sender == owner || freeParticipantControllers[msg.sender] || msg.sender == dev , "Not an owner or free participant controller");
        freeParticipant[participant] = free;
    }

    function setUnrestricted(bool _unrestricted) public
    {
        require (unrestrictedControllers[msg.sender] || msg.sender == dev, "Not an unrestricted controller");
        unrestricted = _unrestricted;
        rootedToken.setLiquidityLock(mainPool, !_unrestricted);
    }    

    function setMainPool(IPancakePair _mainPool) public isController()
    {
        mainPool = _mainPool;
    }

     function setPoolTaxRate(address pool, uint16 taxRate) public isController()
    {
    ...
    {
        require (feeControllers[msg.sender] || msg.sender == owner || msg.sender == dev, "Not an owner or fee controller");
