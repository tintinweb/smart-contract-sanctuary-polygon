...
interface IStakingCore {
    ...
    event StakingDeposit(address from, uint256 amount);
    event StakingWithdrawal(address to, uint256 amount, uint256 fee);
    event StakingDelayedWithdrawal(
        address to,
        uint256 amount,
        uint256 fee,
        uint64 until
    );
    event StakingClaim(address to, uint256 amount);
    ...
    AccessControl
{
    ...
    function statsOf(
        ...
        return
            StakingStatistics(
                _impactOf(investor),
                _debt(investor, _now() > _endsAt ? _endsAt : _now()),
                pending,
                ready
            );
    }
    ...
    function deposit(
        ...
        if (amount == 0) revert DepositIsEmpty();
    ...
    function withdraw(uint256 amount, bool delayed) external override {
        ...
        if (delayed) {
            ...
            emit StakingDelayedWithdrawal(_msgSender(), amount, fee, unlockAt);
        } else {
            uint256 fee;
            if (_now() < _endsAt)
            ...
            emit StakingWithdrawal(_msgSender(), amount, fee);
        }
    ...
    function reward(address to) external override onlyRole(MANAGER_ROLE) {
        ...
        uint256 prize = _debt(to, _now());
        _withdrawal(to);
        emit StakingWithdrawal(to, prize, 0);
    ...
    function refund(uint256 amount) external override onlyRole(PARTNER_ROLE) {
        ...
        uint256 tokensToGive = _totalDelayed() + _totalDebt(_now());
    ...
    function claim(address recipient) external override {
        ...
        emit StakingClaim(recipient, amount);
