...
interface ITimerPool {    /*  TIME POOL BONUS interface   */
    function update(
        uint256 _amounts,
        uint256 _time,
        address _user
...
contract BUSDoilfield {
    ...
    IERC20 public token                       = IERC20(0x6c11E96fE8E0e78995a5E901eFAf24BE72051EDE);/*  Token = BSC-BUSD  */
	uint256 constant public INVEST_MIN_AMOUNT = 1e19; /*  Min  10 BUSD  */
	...
	function invest(address referrer, uint8 plan, uint256 amounts) public  {
        ...
        token.transfer(address(timer),fee);                          /*  (invest = Time pool 1% fee)   */
	...
	function withdraw() public {
        ...
        token.transfer(address(timer),fee);                           /*  (withdraw = Time pool 1% fee)  */
        token.transfer(msg.sender,(totalAmount = (totalAmount-fee)));
