...
contract Silo is Initializable, KeeperCompatibleInterface, IERC721Receiver {
    ...
    function initialize(
        ...
        SILO_ID = siloID;
        isNew = true;
        minGravityGovernance = 6000000000000000;
        mainAction = main;
    }
        ...
        if (status == Statuses.UNWIND) {
            ...
            if (!upkeepNeeded) {
                ...
                for (uint256 i; i < strategy.length; ) {
                    ...
                    if (upkeepNeeded) {
                        performData = abi.encode(false, i * 100 + autoType);
    ...
    function checkAutoMaintain()
        public
        view
        returns (bool upkeepNeeded, uint256 index, uint256 autoType)
    {
        for (uint256 i; i < strategy.length; ) {
            if (i == 0 || (i != 0 && deposited)) {
                (upkeepNeeded, autoType) = IAction(strategy[i].implementation)
                    .checkMaintain(strategy[i].configurationData);
            }
    ...
    ) external override onlySiloManager {
    ...
    {
        uint256[4] memory info = IAction(strategy[mainAction].implementation)
            .extraInfo(strategy[mainAction].configurationData);
