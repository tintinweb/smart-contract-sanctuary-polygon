...
contract AI is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string baseURI;
  string public baseExtension = ".json";
  uint256 public mintAmount=6;
  uint256 public cost = mint(mintAmount);
  ...
  function _baseURI() internal view virtual override returns (string memory) {
   ...
   function needToUpdateCost (uint256 _tokenId) public pure returns(uint256 _cost) {
    if( _tokenId > 0 && _tokenId <=10 ){
        return 0.00002 ether;
    }
    if( _tokenId > 10 && _tokenId <=16){
        return 0.00004 ether;
    }
}
  function mint(uint256 _tokenId) public payable returns (uint256) {
    uint256 costs = needToUpdateCost(cost);
