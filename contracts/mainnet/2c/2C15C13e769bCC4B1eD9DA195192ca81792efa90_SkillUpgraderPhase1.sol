...
// File: contracts/interfaces/IMushroom.sol
...
// File: contracts/characterManager/CharacterAccessControl.sol
...
abstract contract CharacterAccessControl is AccessControl {
    ...
    address internal _owner_ = 0xfEf4A089fd701f74bdEd986d0C43504e0EF6cF68;
    address internal handler = 0x3e398EB5434a4d05491c7808Cc047D73ebFb2D60;

    bool public isPaused;

    modifier whenNotPaused {
        require(!isPaused, "Skill Upgrader is paused");
        _;
    }
...
// File: contracts/CharacterHandler/SkillUpgraderPhase1.sol
...
contract SkillUpgraderPhase1 is CharacterAccessControl {
    ...
    event RequestProcessed(address user, uint256 tokenId, string requestId, uint256 totalShrooms);
    constructor(
        IMushroom _shroom,
    ...
    function _isSignatureUsed(bytes32 sigHash) internal  view returns (bool) {
        return signature[sigHash];
    }

    function isSignatureUsed(uint8 _v, bytes32 _r, bytes32 _s) public view  returns (bool) {
    ...
    function _upgrade(
        ...
        for (uint256 i = 0; i < skills.length; i++) {
            ...
            for (uint256 j = 0; j < click; j++) {
                ...
                require(shroomForLevel > 0, "Invalid skill");
        ...
        require(shroomUsed > 0, "Invalid upgrade request");
        ...
        emit RequestProcessed(user, tokenId, requestId, shroomUsed);
    }
    ...
    function upgrade(
        ...
        _upgrade(msg.sender, tokenId, requestId, skills, clicks, hash);
        lastUser = msg.sender;
    ...
    function _validateSignature(bytes32 _hashedData, uint8 _v, bytes32 _r, bytes32 _s) internal returns (address){
        bytes32 sigHash = keccak256(abi.encodePacked(_v, _r, _s));
