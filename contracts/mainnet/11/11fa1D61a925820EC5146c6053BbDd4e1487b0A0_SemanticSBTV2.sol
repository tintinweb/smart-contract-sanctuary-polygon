...
/**
 * @dev Fork https://github.com/generalgalactic/ERC721S and implement ISemanticSBT interface
 */
contract SemanticSBTV2 is Ownable, Initializable, ERC165, IERC721, IERC721Metadata, ISemanticSBT, ISemanticSBTMetadata, ISemanticSBTOperator, SemanticBaseStruct {
    ...
    // Array of all tokens storing the owner's address and the predicateIndex and objectIndex
    SPO[] private _tokens;
    ...
    // Mapping of classId to (Mapping of subject value to subjectIndex)
    ...
    // All object value of string type
    string[] _stringO;
    // Mapping of blankNode index of BlankNodeO
    ...
    function className(uint256 cIndex) public view returns (string memory name_) {
        require(cIndex > 0 && cIndex < _classNames.length, "class not exist");
    ...
    function predicate(uint256 pIndex) public view returns (string memory name_, FieldType fieldType) {
        require(pIndex > 0 && pIndex < _predicates.length, "predicate not exist");
    ...
    function subjectIndex(string memory subjectValue, string memory className_) public view returns (uint256){
        ...
        require(sIndex > 0, "does not exist");
        return sIndex;
    }
    ...
    function subject(uint256 index) public view returns (string memory subjectValue, string memory className_){
        require(index > 0 && index < _subjects.length, "does not exist");
    ...
    function _addClass(string[] memory classList) internal {
        for (uint256 i = 0; i < classList.length; i++) {
            ...
            require(_classIndex[className_] == 0, "already added");
    ...
    function _addPredicate(Predicate[] memory predicates) internal {
        for (uint256 i = 0; i < predicates.length; i++) {
            ...
            require(_predicateIndex[predicate_.name] == 0, "already added");
    ...
    function _addSubjectPO(uint256[] storage pIndex, uint256[] storage oIndex, SubjectPO[] memory subjectPOList) internal {
        for (uint256 i = 0; i < subjectPOList.length; i++) {
            ...
            require(subjectPO.oIndex > 0 && subjectPO.oIndex < _subjects.length, "subject not exist");
    ...
    function _addBlankNodePO(uint256[] storage pIndex, uint256[] storage oIndex, BlankNodePO[] memory blankNodePOList) internal {
        for (uint256 i = 0; i < blankNodePOList.length; i++) {
            ...
            require(blankNodePO.pIndex <= _predicates.length, "predicate not exist");
    ...
    function _checkPredicate(uint256 pIndex, FieldType fieldType) internal view {
        require(pIndex > 0 && pIndex < _predicates.length, "predicate not exist");
        require(_predicates[pIndex].fieldType == fieldType, "predicate type error");
    }
    ...
    function addSubject(string memory value, string memory className_) external onlyMinter returns (uint256 sIndex) {
        ...
        require(cIndex > 0, "param error");
        require(_subjectIndex[cIndex][value] == 0, "already added");
    ...
    function mint(address account, uint256 sIndex, IntPO[] memory intPOList, StringPO[] memory stringPOList,
        ...
        require(sIndex <= _subjects.length, "param error");
        ...
        require(pIndex.length > 0, "param error");
...
/**
 * @dev The base struct for semantic contract
 */
interface SemanticBaseStruct {
        enum FieldType{
        INT,
        STRING,
        ADDRESS,
