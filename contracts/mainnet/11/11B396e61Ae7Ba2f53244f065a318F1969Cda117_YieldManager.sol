...
contract YieldManager is ReentrancyGuard {
    ...
    mapping(address => address) affiliateLookup;
    ...
    configStruct[] clientLevels;
    configStruct[] sponsorLevels;

    address[] stakingAddresses;
    address[] lpStakingAddresses;
    ...
    constructor() {
        ...
        clientLevels.push(
            configStruct({level: 0, val1: 19 * 10 ** 18, val2: 150, val3: 100})
        );
        clientLevels.push(
            configStruct({
                level: 500 * 10 ** 18,
        ...
        sponsorLevels.push(configStruct({level: 0, val1: 0, val2: 0, val3: 0}));
        sponsorLevels.push(
            configStruct({
                level: 500 * 10 ** 18,
                val1: 100,
    ...
    function setClientLevels(uint[] memory levels, uint[] memory val1s, uint[] memory val2s, uint[] memory val3s) public onlyOwner {
    ...
    //updates client levels
    function setSponsorLevels(uint[] memory levels, uint[] memory val1s, uint[] memory val2s, uint[] memory val3s) public onlyOwner {
        require(levels.length == val1s.length, "length mismatch");
        require(val1s.length == val2s.length, "length mismatch");
        require(val2s.length == val3s.length, "length mismatch");
