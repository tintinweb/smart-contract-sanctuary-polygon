...
contract MultiTransferERC20 {
     ...
     */
    function golemTransferIndirectPacked(bytes32[] calldata payments) external {
        uint256 totalAmount = 0;
        for (uint i = 0; i < payments.length; ++i) {
            bytes32 payment = payments[i];
			totalAmount += uint(payment) % 2**96;
        }
        require(GLM.transferFrom(msg.sender, address(this), totalAmount), "transferFrom failed");
    ...
    /**
     * @dev Packed version of batchTransferIndirect to save some gas on arguments. GLM limited to balance stored in contract.
     */
    function golemTransferIndirectPackedLimit(bytes32[] calldata payments) external {
        uint256 totalAmount = 0;
        for (uint i = 0; i < payments.length; ++i) {
            // A payment contains compressed data:
            // first 96 bits (12 bytes) is a value,
