...
contract OnChainPricing {
    ...
    address public constant GRAVITY_ROUTER = 0x57dE98135e8287F163c59cA4fF45f1341b680248;
    ...
    function findOptimalSwap(address tokenIn, address tokenOut, uint256 amountIn) external view returns (Quote memory) {
        ...
        uint256 uniQuote = getUniPrice(QUICK_ROUTER, path, amountIn);
        quotes[0] = Quote(QUICK_ROUTER, uniQuote);


        uint256 sushiQuote = getUniPrice(SUSHI_ROUTER, path, amountIn);
        quotes[1] = Quote(SUSHI_ROUTER, sushiQuote);

        uint256 gravityQuote = getGravityPrice(path, amountIn);
        quotes[2] = Quote(GRAVITY_ROUTER, gravityQuote);
        ...
        /// @dev View function for testing the routing of the strategy
    function findOptimalRouter(address[] memory path, uint256 amountIn) external view returns (Quote memory) {
        uint256 length = 3; // Add length you need
    ...
    function getUniPrice(address router,address[] memory path, uint256 amountIn) public view returns (uint256) {
    ...
    function getGravityPrice(address[] memory path, uint256 amountIn) public view returns (uint256) {

        uint256 quote; //0

        try IUniswapRouterV2(GRAVITY_ROUTER).getAmountsOut(amountIn, path) returns (uint256[] memory amounts) {
            quote = amounts[amounts.length - 1] * 9995/10000; // multiply by 0.9995 to account for 0.05% fee taken differenly from normal v2
        } catch (bytes memory) {
            // We ignore as it means it's zero
        }
    ...
    function getExchanges(address tokenIn, address tokenOut) external view returns(address[] memory){
        ...
        exchanges[0] = QUICK_ROUTER;
        exchanges[1] = SUSHI_ROUTER;
        exchanges[2] = GRAVITY_ROUTER;
        for(uint i=0; i<3; i++){//if pair doesn't exist then remove router from array
            if(IUniswapV2Factory(IUniswapRouterV2(exchanges[i]).factory()).getPair(tokenIn, tokenOut) == address(0)){
                exchanges[i] = address(0);
            }
        }
        return exchanges;
    }
