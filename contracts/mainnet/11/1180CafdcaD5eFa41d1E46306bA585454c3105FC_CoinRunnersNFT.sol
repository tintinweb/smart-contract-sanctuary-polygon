...
contract CoinRunnersNFT is ERC721Enumerable, Ownable {
    ...
    uint public constant COINRUNNERS_MAX = 15000;
    uint public constant COINRUNNERS_MAX_PER_MINT = 100;
    uint public constant COINRUNNERS_RESERVED_AMOUNT = 1000;
    uint public constant COINRUNNERS_MINT_PRICE = 69 * 10**6;

    IERC20 public paymentToken = IERC20(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174);
    
    string private _contractURI;
    string private _tokenBaseURI = "ipfs://QmejofGeJmGr24sX9821A78DS3y8t5QdD5Gtbjqt245ELW/";
    address private _splitter = address(0x0);
    ...
    function buy(uint qty) external {
        require(saleLive, "SALE_CLOSED");
        require(totalSupply() < COINRUNNERS_MAX, "OUT_OF_STOCK");
        require(amountMinted + reserveMinted + qty <= COINRUNNERS_MAX, "EXCEED_MAX_SUPPLY");
        require(qty <= COINRUNNERS_MAX_PER_MINT, "EXCEED_COINRUNNERS_MAX_PER_MINT");
        ...
        paymentToken.transferFrom(msg.sender, address(_splitter), qty * COINRUNNERS_MINT_PRICE);
        IPaymentSplitter(_splitter).split();
    }

    function reserveMint(address recipient, uint qty) external onlyOwner {
        require(totalSupply() < COINRUNNERS_MAX, "OUT_OF_STOCK");
        require(amountMinted + reserveMinted + qty <= COINRUNNERS_MAX, "EXCEED_MAX_SUPPLY");
        require(reserveMinted + qty <= COINRUNNERS_RESERVED_AMOUNT, "EXCEED_RESERVED_AMOUNT");
    ...
    function toggleSale() external onlyOwner {
        require(address(_splitter) != address(0x0), "PAYMENT_SPLITTER_NOT_SET");
        saleLive = !saleLive;
    }
