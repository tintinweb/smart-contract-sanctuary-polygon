...
contract RubicLiFiProxy is MultipleTransitToken, OnlySourceFunctionality {
    ...
    {
        accrueFixedCryptoFee(_params.integrator, integratorToFeeInfo[_params.integrator]); // collect fixed fee
        ...
        EventEmitter(_params)
    {
        uint256 _amountIn = accrueTokenFees(
            _params.integrator,
            integratorToFeeInfo[_params.integrator],
            accrueFixedCryptoFee(_params.integrator, integratorToFeeInfo[_params.integrator]), // amountIn - fixedFee - commission
            0,
            address(0)
        );
