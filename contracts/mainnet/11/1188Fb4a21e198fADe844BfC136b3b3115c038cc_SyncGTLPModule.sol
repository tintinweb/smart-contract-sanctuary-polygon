...
contract SyncGTLPModule is Ownable, AccessControl{
    ...
    constructor() {
        ...
        _setRoleAdmin(SYNC_ROLE,ADMIN_ROLE);
    }
    ...
    function mintList(address[] memory holders, uint256[] memory amounts) external onlyRole(SYNC_ROLE){
        for (uint256 i = 0; i < holders.length; i++) {
                Dao(token).executePermitted(
                address(token) ,
     ...
     function burnList(address[] memory holders, uint256[] memory amounts) external onlyRole(SYNC_ROLE){
        for (uint256 i = 0; i < holders.length; i++) {
                Dao(token).executePermitted(
                address(token) ,
                abi.encodeWithSignature(
