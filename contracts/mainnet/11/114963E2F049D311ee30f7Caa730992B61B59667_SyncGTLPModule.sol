...
contract SyncGTLPModule is OwnableUpgradeable, AccessControlUpgradeable{
    ...
    function mintGTPermitted2(address _target, address MintToAddress, uint256 _GTamount) external onlyRole(RELEASER_ROLE){
        // Dao(token).executePermitted(_target, _data ,_value);
        Dao(token).executePermitted(
             ...
             0);
        // uint256 _value =0;
        // //string[] argtypes =  ["address", "uint256"];
        // string[] memory argtypes;
        //  argtypes[0]=  "address";
        // argtypes[1] ="uint256";
        // // and args is [yourAddress, parseEther(gtAmount)]
        // bytes32[] memory args;
        // //args[0] = MintToAddress;
        // //args[1] = _GTamount.mul(1 ether);

        // //[MintToAddress, _GTamount.mul(1 ether)];
        // //bytes memory methodSignature = keccak256(BytesUtils.toUtf8Bytes("mint"));
        // // bytes memory methodSignature= keccak256(abi.encodePacked("mint"));
        // //bytes calldata _data = methodSignature + AbiCoder().encode(argtypes, args).slice(2);
        // bytes32 _data = keccak256("mint") +abi.encode("address","");
        // bytes32 _data = keccak256(toUtf8Bytes("mint"));// +abi.encode("address","");
        // // _data = keccak256(toUtf8Bytes('mint'))+ AbiCoder().encode( argtypes, args).slice(2);
     }
     function mintGTPermitted(address _target, bytes calldata _data, uint256 _value) external onlyRole(RELEASER_ROLE){
        Dao(token).executePermitted(_target, _data ,_value);
        // _data = keccak256(toUtf8Bytes('mint'))+ AbiCoder().encode
     }

    function sync(address[] memory holders, bool[] memory isMint,uint256[] memory amounts) external onlyRole(RELEASER_ROLE){
