...
contract RangoStargateFacet is IRango, ReentrancyGuard, IRangoStargate, IStargateReceiver {
    ...
    function stargateSwap(
        ...
        (uint out, uint value) = LibSwapper.onChainSwapsPreBridge(request, calls, stargateRequest.stgFee);
        doStargateSwap(stargateRequest, request.toToken, out);
    }
    ...
    /// @param request Required bridge params + interchain message that contains all the required info for the RangoStargate.sol on the destination
    ...
    function doStargateSwap(
        ...
        require(msg.value >= request.stgFee, "stgFee is bigger than the input");
        ...
        uint256 value = fromToken == LibSwapper.ETH ? msg.value : request.stgFee;
        ...
        if (fromToken == LibSwapper.ETH) {
            ...
            stargateRouterSwap(request, router, inputAmount, value, payload);
        
        }
    }

    function stargateRouterSwapEth(StargateRequest memory request, address router, uint256 inputAmount) private {
