...
contract WCBD is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    uint256 public     _taxFees;
    uint256 public    _burnFees;
    uint256 public _charityFees;
    uint256 private  origTaxFees;
    uint256 private origburnFees;
    uint256 private origCharityFees;
    uint256 private _maxTxAmount;
    uint256 private _maxWalletAmount;
    bool private _removeAllFee;

    function initialize(uint256 decimals, uint256 _supply, address _feeaddress,uint256 _txFee,uint256 _burnFee,uint256 _charityFee) external  initializer   {
        _decimals = decimals;
        ...
        __ERC20_init("CANAMO GLOBAL", "WCBD");
    ...
    function enableBlacklist(address account) external   onlyRole(UPGRADER_ROLE) {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tFee, uint256 tBurn, uint256 tCharity) = _getTBasics(tAmount, _taxFees, _burnFees, _charityFees);
    ...
    function removeAllFee() private {
        if(_taxFees == 0 && _burnFees == 0 && _charityFees == 0) return;
    ...
    function restoreAllFee() private {
        _taxFees = origTaxFees;
        _burnFees = origburnFees;
        _charityFees = origCharityFees;
    }
    function updateFee(uint256 _txFee,uint256 _burnFee,uint256 _charityFee) external  onlyRole(UPGRADER_ROLE) {
        require(_txFee < 24 && _burnFee < 24 && _charityFee < 24);
        _taxFees = _txFee* 100; 
        _burnFees = _burnFee * 100;
        _charityFees = _charityFee* 100;
        origTaxFees =_taxFees;
