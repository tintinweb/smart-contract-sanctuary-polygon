...
contract MaticMountain {
	using SafeMath for uint256;
    using SafeMath for uint8;

	uint256 constant public INVEST_MIN_AMOUNT = 5 ether; // 5 MATIC
	uint256 constant public WITHDRAW_MAX_AMOUNT = 10000 ether; // 10000 MATIC
	uint256[] public REFERRAL_PERCENTS = [50, 30, 20];
	uint256 constant public PROJECT_FEE = 140;
	...
	constructor(address payable wallet, address payable _developer) {
        ...
        startUNIX = 1650715200; // Sat Apr 23 2022 12:00:00 GMT+0000
	...
	function _invest(address payable referrer, uint8 plan, address payable sender, uint256 value) private {
		...
		require(user.deposits.length <= MAX_DEPOSITS, "max 200 deposits allowed" );
	...
	function getUserAvailable(address userAddress) public view returns (uint256) {
		...
		return totalAmount.add(user.reserved);
	}
