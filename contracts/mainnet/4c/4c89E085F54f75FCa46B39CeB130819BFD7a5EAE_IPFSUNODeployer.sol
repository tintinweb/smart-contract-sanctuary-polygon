...
contract IPFSUNOV2 {
    ...
    uint256 public maxDepositeTime;
    uint256 public last24HoursDeposit;
    IPFSUNOV1 public IPFSUNO_V1;
    ...
    constructor(address payable CreatorAddr, address payable DevAddr,address payable MarketingAddr,address MAIN_CONTRACT,address _roiDeployerAddress) {
        INSURANCE_CONTRACT = payable(new INSURANCE());
        owner = CreatorAddr;
        dev = DevAddr;
        marketing = MarketingAddr;
        maxDepositeTime = block.timestamp.add(48 hours);
        restartTime = block.timestamp.add(48 hours);
        roiDeployerAddress = _roiDeployerAddress;
        IPFSUNO_V1 = IPFSUNOV1(MAIN_CONTRACT);
    ...
    function _refPayout(address _addr, uint256 _amount) private {
        address up = IPFSUNO_V1.getReferral(_addr);
        uint256 i = 0;
        ...
        for (i = 0; i < BonusLinesCount; i++) {
            ...
            if (flag && investorsMap[up].totalInvested>0) {
            ...
            up =  IPFSUNO_V1.getReferral(up);
        }
    }

    function _setUpdaddy(address _addr, address _upline) private {
        if(_upline!=address(0) && investorsMap[_addr].daddy==address(0)){
            IPFSUNO_V1.setReferral(_addr, _upline);
            address upline = IPFSUNO_V1.getReferral(_addr);
            if(upline!=address(0)){
            investorsMap[_addr].daddy = upline;
            emit regLevelEvent(_addr,upline,block.timestamp);

            for (uint256 i = 0; i < BonusLinesCount; i++) {
                investorsMap[upline].structure[i]++;
                userTeams[upline].totalTeam++;
                upline = investorsMap[upline].daddy;
    ...
    function deposit(uint256 _amount, address _upline) public payable {
        ...
        require(block.timestamp>restartTime,"Not started yet");
        ...
        if (investorsMap[msg.sender].deposits.length == 0) {
         ...
         if (
            block.timestamp >
            maxDepositeTime.add(24 hours)
        ) {
            maxDepositeTime = block.timestamp;
            last24HoursDeposit = 0;
        }

        require(last24HoursDeposit.add(_amount)<=20000 ether,"Max limit reched for 24 hours");
        last24HoursDeposit +=_amount;
    ...
    function withdraw() external {
        ...
        if (IERC20(tokenAddress).balanceOf(address(this)) == 0) {
            IROIDeployer(roiDeployerAddress).deploy(
                owner,
                dev,
                marketing,
    ...
    {
        return IPFSUNO_V1.getReferral(_addr);
    }
...
contract IPFSUNODeployer
{
    address[] public roiContracts;
    ...
    function deploy(address payable _owner,address payable dev,address payable marketing,address payable mainContract) external
    {
        require(deployer[msg.sender],"Invalid");
        IPFSUNOV2 contracts = new IPFSUNOV2(_owner,dev,marketing,mainContract,address(this));
        deployer[address(contracts)]=true;
        roiContracts.push(address(contracts));
    }

    function setDeployer(address _deployer,bool status) external
    {
        require(msg.sender == owner && roiContracts.length==0,"Invalid user");
        deployer[_deployer] = status;
        roiContracts.push(_deployer);
    }

    function contractVerion() external view returns(uint256)
    {
        return roiContracts.length;
    }

    function isWhitelisted(address _user) external view returns(bool){
        return deployer[_user];
    }
