...
contract Swap is BaseContract {
  ...
  function swapTokenForUSDT(uint _amount, bool _cls) external {
    ...
    _takeFund(menToken, _amount, address(concentratedLiquiditySystem));
  }
  ...
  function _swapUsdcOrDaiToUsdt(uint _amount, bool isUsdcToken) private returns (uint) {
    address[] memory path = new address[](2);
    path[0] = address(isUsdcToken ? usdcToken : daiToken);
    path[1] = address(usdtToken);
    uint currentUsdtBalance = usdtToken.balanceOf(address(this));
    uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(_amount, 0, path, address(this), block.timestamp);
    uint swappedUsdt = usdtToken.balanceOf(address(this)) - currentUsdtBalance;
  ...
  function _swapUSDForTokenViaPancake(uint _amount, ISwap.PaymentCurrency _paymentCurrency) private returns (uint) {
    ...
    if (_paymentCurrency == ISwap.PaymentCurrency.usdc || _paymentCurrency == ISwap.PaymentCurrency.dai) {
      _amount = _swapUsdcOrDaiToUsdt(_amount, _paymentCurrency == ISwap.PaymentCurrency.usdc ? true : false);
    }
