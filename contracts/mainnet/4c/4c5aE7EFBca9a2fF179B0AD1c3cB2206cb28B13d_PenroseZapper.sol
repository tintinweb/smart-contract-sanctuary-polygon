...
contract PenroseZapper is 
    ...
    function initializeProxyStorage(address _penroseInterface, address _WNATIVE, address _dystRouter)
        public
    ...
    function tokenToPenroseZap(
        ...
        if (specialTokensWithRouters[_from] != address(0)) {
            ...
            uint256 nativeAmount = _swapTokenForNative(_from, amount, stability.tokenToNative, address(this), specialRouterAddr);
            // then swap native to LP
            poolAmount = _swapNativeToLP(_to, nativeAmount, stability, address(this), routerAddr);
        } else {
            // Otherwise, can directly use 1 router to do entire swap
            // Approve this contract to be able to transact the given ERC20 token with the router
            _approveTokenIfNeeded(_from, routerAddr);
            poolAmount = _swapTokenToLP(_from, amount, stability, _to, address(this), routerAddr);
        }
        IUserProxyInterface(penroseInterface).depositLpAndStake(_to, poolAmount);
    }
    ...
    function tokenToLP(
        ...
        if (specialTokensWithRouters[_from] != address(0)) {
            ...
            uint256 nativeAmount = _swapTokenForNative(_from, amount, stability.tokenToNative, address(this), specialRouterAddr);
            // then swap native to LP
            poolAmount = _swapNativeToLP(_to, nativeAmount, stability, _recipient, routerAddr);
        } else {
            // Otherwise, can directly use 1 router to do entire swap
            // Approve this contract to be able to transact the given ERC20 token with the router
            _approveTokenIfNeeded(_from, routerAddr);
            poolAmount = _swapTokenToLP(_from, amount, stability, _to, _recipient, routerAddr);
        }
    }
    ...
    function zapInToLP(
        ...
        uint256 poolAmount = _swapNativeToLP(_to, msg.value, stability, _recipient, routerAddr);
        emit ZapAmount(poolAmount);
    }

    function nativeToPenroseZap(
        address _to,
        OptimumStability calldata stability,
        address routerAddr,
        address _recipient
        ...
        // from Native to an LP token through the specified router
        uint256 poolAmount = _swapNativeToLP(_to, msg.value, stability, address(this), routerAddr);
        emit ZapAmount(poolAmount);
    ...
    function _swapTokenToLP(
        ...
        if (_from == IDystPair(_to).token0() || _from == IDystPair(_to).token1()) {
            ...
            addLiquidityArguments.token1 = _from == IDystPair(_to).token0() ? IDystPair(_to).token1() : IDystPair(_to).token0();
            _approveTokenIfNeeded(addLiquidityArguments.token1, routerAddr);
            // calculate amount of _from to sell
            uint256 sellAmount = amount.div(2);
            // execute swap
            addLiquidityArguments.token1Amount = _swap(_from, sellAmount, addLiquidityArguments.token1, stability.tokenToNative, _from == IDystPair(_to).token0() ? stability.nativeToToken1 : stability.nativeToToken0, address(this), routerAddr);
            addLiquidityArguments.stable = IDystPair(_to).stable();
            uint256 liquidity;
            (, , liquidity) = IDystRouter01(routerAddr).addLiquidity(_from, addLiquidityArguments.token1, addLiquidityArguments.stable, amount.sub(sellAmount), addLiquidityArguments.token1Amount, 0, 0, recipient, block.timestamp);
            return liquidity;
        } else {
            // go through native token for highest liquidity
            uint256 nativeAmount = _swapTokenForNative(_from, amount, stability.tokenToNative, address(this), routerAddr);
            return _swapNativeToLP(_to, nativeAmount, stability, recipient, routerAddr);
        }
    }
    ...
    function _swapNativeToLP(
        ...
        if (token0 == WNATIVE || token1 == WNATIVE) {
            address token = token0 == WNATIVE ? token1 : token0;
            bool swapStability = token0 == WNATIVE ? stability.nativeToToken1 : stability.nativeToToken0;
            (, , liquidity) = _swapHalfNativeAndProvide(token, amount, swapStability, stable,  routerAddress, recipient);
        } else {
            (, , liquidity) = _swapNativeToEqualTokensAndProvide(token0, token1, stability, stable, amount, routerAddress, recipient);
        }
        return liquidity;
    }
        ...
        )
    {
        ...
        uint256 tokenAmount = _swapNativeForToken(token, swapValue, swapStability, address(this), routerAddress);
        _approveTokenIfNeeded(token, dystRouter);
        IDystRouter01 router = IDystRouter01(dystRouter);
        return router.addLiquidityMATIC{value: amount.sub(swapValue)}(token, poolStability, tokenAmount, 0, 0, recipient, block.timestamp);
        ...
        )
    {
        ...
        AddLiquidityArgs memory addLiquidityArguments;
        uint256 swapValue = amount.div(2);
        addLiquidityArguments.token0Amount = _swapNativeForToken(token0, swapValue, stability.nativeToToken0, address(this), routerAddress);
        addLiquidityArguments.token1Amount = _swapNativeForToken(token1, amount.sub(swapValue), stability.nativeToToken1, address(this), routerAddress);
        
        _approveTokenIfNeeded(token0, dystRouter);
        _approveTokenIfNeeded(token1, dystRouter);
        IDystRouter01 router = IDystRouter01(dystRouter);
        return router.addLiquidity(token0, token1, poolStability, addLiquidityArguments.token0Amount, addLiquidityArguments.token1Amount, 0, 0, recipient, block.timestamp+100);
    }
    ...
    function _swapNativeForToken(
        ...
        if (routerAddr != dystRouter) {
            ...
            uint256[] memory amounts = router.swapExactMATICForTokens{value: value}(0, path, recipient, block.timestamp);
            return amounts[amounts.length - 1];
        }
    }
    ...
    function _swapTokenForNative(
        ...
        if (routerAddr != dystRouter) {
            ...
            uint256[] memory amounts = router.swapExactTokensForMATIC(amount, 0, path, recipient, block.timestamp);
            return amounts[amounts.length - 1];
        }
    }
    ...
    function setAllowedTokenAddress(address _token, bool _isAllowed) external onlyGovernance {
        tokenIsAllowedMap[_token] = _isAllowed;
    }

    function setSpecialTokenWithRouter(address _token, address _router) external onlyGovernance {
    ...
    function setPenroseInterface(address _penroseInterface) external onlyGovernance {
        penroseInterface = _penroseInterface;
    }
    ...
    function withdraw(address token) external onlyGovernance {
        ...
        IERC20(token).transfer(governanceAddress(), IERC20(token).balanceOf(address(this)));
    }
}
