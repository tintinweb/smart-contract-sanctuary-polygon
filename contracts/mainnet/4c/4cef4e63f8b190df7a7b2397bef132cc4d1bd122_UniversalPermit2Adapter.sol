...
import { BasePermit2Adapter, IPermit2 } from "./base/BasePermit2Adapter.sol";
import { ArbitraryExecutionPermit2Adapter } from "./base/ArbitraryExecutionPermit2Adapter.sol";
import { SwapPermit2Adapter } from "./base/SwapPermit2Adapter.sol";
...
abstract contract BasePermit2Adapter is IBasePermit2Adapter {
   ...
   */
  function _batchPermit2TakeFromCaller(
   ...
   * @param _signature The signature that allows the transfer
   */
  function _permit2TakeFromCaller(
    address _token,
    uint256 _amount,
    uint256 _nonce,
    uint256 _deadline,
   ...
   */
  function _maxApprove(address _token, address _allowanceTarget) internal {
    // This helper should handle cases like USDT. Thanks OZ!
    IERC20(_token).forceApprove(_allowanceTarget, type(uint256).max);
  }

  /**
   * @notice Calls the target contract with the given data and value
   * @param _target The contract to call
   * @param _data The call's data
   * @param _value The value to send
   * @return The raw return data from the contract's execution
   */
  function _callContract(address _target, bytes calldata _data, uint256 _value) internal returns (bytes memory) {
  ...
  function _distributeShares(
    address _token,
    TargetShare[] calldata _distribution
  )
    internal
    returns (uint256 _available)
  {
    _available = _balanceOf(_token);
    ...
    for (uint256 i; i < _distribution.length - 1;) {
      ...
      _sendToRecipient(_token, _toSend, _distribution[i].recipient);
      _amountLeft -= _toSend;
      unchecked {
        ++i;
      }
    ...
    _sendToRecipient(_token, _amountLeft, _distribution[_distribution.length - 1].recipient);
  }
   ...
   * @return _balance The current balance held by the contract
   */
  function _sendBalanceOnContractToRecipient(address _token, address _recipient) internal returns (uint256 _balance) {
    _balance = _balanceOf(_token);
   ...
   */
  function _sendToRecipient(address _token, uint256 _amount, address _recipient) internal {
    if (_amount > 0) {
...
import { BasePermit2Adapter, IPermit2 } from "./BasePermit2Adapter.sol";
...
abstract contract ArbitraryExecutionPermit2Adapter is BasePermit2Adapter, IArbitraryExecutionPermit2Adapter {
  ...
  {
    _permit2TakeFromCaller(_permit.token, _permit.amount, _permit.nonce, _deadline, _permit.signature);
  ...
  function executeWithBatchPermit(
    ...
    returns (bytes[] memory _executionResults, uint256[] memory _tokenBalances)
  {
    _batchPermit2TakeFromCaller(_batchPermit.tokens, _batchPermit.nonce, _deadline, _batchPermit.signature);
    return _approveExecuteAndTransfer(_allowanceTargets, _contractCalls, _transferOut);
  }
    ...
    returns (bytes[] memory _executionResults, uint256[] memory _tokenBalances)
  {
    ...
    for (uint256 i; i < _allowanceTargets.length;) {
      _maxApprove(_allowanceTargets[i].token, _allowanceTargets[i].allowanceTarget);
      unchecked {
        ++i;
      }
    }
    ...
    for (uint256 i; i < _transferOut.length;) {
      _tokenBalances[i] = _distributeShares(_transferOut[i].token, _transferOut[i].distribution);
      unchecked {
        ++i;
      }
    }
 ...
 * some extra checks to guarantee
 *         that the minimum amounts are respected
 * @dev It's important to note that this contract should never hold any funds outside of the scope of a transaction,
 *      nor should it be granted "regular" ERC20 token approvals. This contract is meant to be used as a proxy, so
 *      the only tokens approved/transferred through Permit2 should be entirely spent in the same transaction.
 *      Any unspent allowance or remaining tokens on the contract can be transferred by anyone, so please be careful!
...
abstract contract SwapPermit2Adapter is BasePermit2Adapter, ISwapPermit2Adapter {
  ...
  {
    // Take from caller
    _permit2TakeFromCaller(_params.tokenIn, _params.amountIn, _params.nonce, _params.deadline, _params.signature);

    // Max approve token in
    _maxApproveIfNecessary(_params.tokenIn, _params.allowanceTarget);

    // Execute swap
    _callContract(_params.swapper, _params.swapData, msg.value);

    // Check min amount
    _amountOut = _balanceOf(_params.tokenOut);
    if (_amountOut < _params.minAmountOut) revert ReceivedTooLittleTokenOut(_amountOut, _params.minAmountOut);

    // Distribute token out
    _distributeShares(_params.tokenOut, _params.transferOut);
  ...
  {
    // Take from caller
    _permit2TakeFromCaller(_params.tokenIn, _params.maxAmountIn, _params.nonce, _params.deadline, _params.signature);
    ...
    uint256 _unspentTokenIn = _sendBalanceOnContractToRecipient(_params.tokenIn, _params.unspentTokenInRecipient);
  ...
  function _maxApproveIfNecessary(address _token, address _allowanceTarget) internal {
    if (_allowanceTarget != address(0)) {
      _maxApprove(_token, _allowanceTarget);
    }
  }
}
...
interface IBasePermit2Adapter {
  /// @notice A specific target of part of the balance on the contract
  struct TargetShare {
    address recipient;
    uint256 shareBps;
  }
...
interface ISwapPermit2Adapter is IBasePermit2Adapter {
  ...
  /// @notice Data about fee collection
  struct FeeData {
    address recipient;
    uint256 feeBps;
  }
