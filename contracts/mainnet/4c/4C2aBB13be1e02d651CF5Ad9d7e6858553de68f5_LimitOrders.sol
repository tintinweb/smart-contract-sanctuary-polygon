...
contract LimitOrders is EIP712("FBS Token", "1"){
    ...
    bytes32 LIMIT_ORDER_TYPEHASH = keccak256(
        "Order(address owner,uint256 status,uint256 swapType,address pair,address tokenIn,address tokenOut,uint256 amountIn,uint256 priceExecute, uint256 minAmountOut,uint256 deadline,uint256 salt)"
        );
        ...
        returns (address)
    {
        ...
        bytes32 _hash = getEthSignedMessageHash(_hashTypedDataV4(structHash));
    ...
    function shouldFulfilOrder(Order memory ord, bytes memory _signature) public view returns (bool) {
        ...
        bool isEnoughToken = false;
        if(IERC20(ord.tokenIn).balanceOf(ord.owner) >= ord.amountIn) return isEnoughToken = true;
        ...
        if(ord.status == 0) return isPendingOrder = true;
        bool isSigner = false;
        if(_signer == ord.owner) return isSigner = true;
        bool isFulfiledOrder = false;
        if(getAmountOut(ord.amountIn,ord.pair, ord.tokenIn, ord.tokenOut) >= ord.priceExecute) return isFulfiledOrder = true;

        return isEnoughToken && isCancelledOrder && isPendingOrder && isSigner && isFulfiledOrder;
    }
