...
// File contracts/EvanPunks.sol


pragma solidity ^0.8.4;
//10000 = 100%, 1000 = 10%, 100 = 1%
contract EvanPunks is ERC721Enumerable, PlatziPunksDNA, ERC2981, Ownable {
    using Strings for uint256;
    uint public maxSupply;
    mapping(uint => uint) public tokenDNA;
    mapping(uint => string) public tokenHash;

    constructor(uint _maxSuppy) ERC721("EvanPunks", "EPKS") {
        maxSupply = _maxSuppy;
    ...
    //denomitator = 10000
    function mint(string calldata _hash, uint96 _fraction) public {
        uint _current = totalSupply();
        require(_current < maxSupply, "No PlatziPunks left :(");
        tokenDNA[_current] = deterministicPseudoRandomDNA(_current, msg.sender);
        tokenHash[_current] = _hash;
        _setTokenRoyalty(_current, msg.sender, _fraction);
    ...
    function tokenURI(uint tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI_).length > 0 ? string(abi.encodePacked(baseURI_, tokenHash[tokenId])): "";
    }
...
// File contracts/EvanPunks1155.sol


pragma solidity ^0.8.4;
contract EvanPunks1155 is ERC1155, Ownable, ERC1155Supply, ERC2981 {
    string constant public baseURI = "https://gateway.pinata.cloud/ipfs/";
    mapping(uint => string) public tokenHash;
    constructor() ERC1155("") {
    ...
    //10000 = 100%, 1000 = 10%, 100 = 1%
    function mint(address account, uint256 id, uint256 amount, bytes memory data, string calldata _hash)
        public
        onlyOwner
    {
        _mint(account, id, amount, data);
        tokenHash[id] = _hash;
        _setTokenRoyalty(id, msg.sender, 500);
    }

    function mintBatch(address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data, string calldata _hash)
        public
        onlyOwner
    {
        for(uint i; i < ids.length; i++) {
            tokenHash[ids[i]] = _hash;
            _setTokenRoyalty(ids[i], msg.sender, 500);
        }
...
pragma solidity ^0.8.4;
contract PlatziPunks is ERC721Enumerable, PlatziPunksDNA {
    using Strings for uint256;
    uint public maxSupply;
    mapping(uint => uint) public tokenDNA;
