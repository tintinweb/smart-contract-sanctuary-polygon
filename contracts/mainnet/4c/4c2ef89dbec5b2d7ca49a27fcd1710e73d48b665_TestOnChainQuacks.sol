...
contract TestOnChainQuacks is ERC721A, ReentrancyGuard, Ownable,  DefaultOperatorFilterer {
    using Strings for uint256;

    // Minting Variables
    uint256 public mintPrice = 0.0005 ether;
    uint256 public maxPurchase = 80;
    uint256 public maxSupply = 16027;
    ...
    external payable  {
        ...
        tokenHue[_tokenID] = ((_tokenID * tokenIdLevels[_tokenID] ) % 360);
        tokenSat[_tokenID] = 30 + (_tokenID % 50);
        tokenLum[_tokenID] = 80 - (_tokenID % 50);
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                image,
                                '", "description": "An on Chain which can Burn and Combine, change colors, and gain levels by owner on chain actions","attributes": [{"trait_type": "Metadata","value": "OnChain"},{"trait_type": "Utility","value": "Combine And Burn"},{"trait_type": "Token Level","value":',
                                Strings.toString(tokenIdLevels[id]),
                                " }]}"
                            )
                        )
