    ...
    Administrated
{
    ...
    function presaleBuy(
        ...
        uint256 _tokenId = _buy(_signature, _uuid, _tokenURI, presalePrice);
    ...
    function publicBuy(
        ...
        uint256 _tokenId = _buy(_signature, _uuid, _tokenURI, price);
    ...
    function presaleBuyBatches(
        ...
        for (uint256 _i = 0; _i < _signatures.length; _i++) {
            uint256 _tokenId = _buy(_signatures[_i], _uuids[_i], _tokenURIs[_i], presalePrice);
            emit PresaleBuy(_tokenId, msg.sender);
        }
    }
    ...
    function publicBuyBatches(
        ...
        for (uint256 _i = 0; _i < _signatures.length; _i++) {
            uint256 _tokenId = _buy(_signatures[_i], _uuids[_i], _tokenURIs[_i], price);
            emit Buy(_tokenId, msg.sender);
        }
    }
     ...
     * @param _uuid Universal Unique Identifier
     * @param _tokenURI Token URI String
     * @param _tokenPrice Price of a token on sale or presale
    ...
    function _buy(
        ...
        // Splitting payment
        if (_tokenPrice > 0) {
            IERC20(weth).safeTransferFrom(msg.sender, splitter, _tokenPrice);
