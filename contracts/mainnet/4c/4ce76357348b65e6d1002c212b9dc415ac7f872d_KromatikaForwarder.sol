...
// File: contracts/metaswap/LibERC20Adapter.sol
...
/// @notice A transformation callback used in `TransformERC20.transformERC20()`.
...
// File: contracts/metaswap/KromatikaForwarder.sol
...
contract KromatikaForwarder is IForwarder {
    ...
    function executeCall(
        ...
        require(validators[request.validator], 'KF_VAUTH');
        require(request.validTo == 0 || block.timestamp + 20 <= request.validTo, 'KF_VD');
        ...
        require(signer != address(0) && signer == request.validator, 'KF_NV');
        require(!messageDelivered[digest], 'KF_MD');
        messageDelivered[digest] = true;

        uint256 paymentFeesBefore = LibERC20Adapter.getTokenBalanceOf(
            IERC20(request.paymentToken), request.validator
        );
        ...
        require(paymentFeesAfter.sub(request.paymentFees) == paymentFeesBefore, 'KF_FNM');
        require(request.tokenGasPrice.mul(initialGas.sub(gasleft())) <= request.paymentFees, 'KF_OC');
    }
