...
interface ITournamentRouter {
  function verifier() external view returns (address);

  function services(uint64) external view returns (address);
  ...
  function joinTournament(
    ...
    bytes memory _verifyParams,
    bytes memory _verifyConfigs
  ) external;
...
import { TournamentTypes } from "../types/Types.sol";
import { ITournamentRouter } from "../interfaces/ITournamentRouter.sol";
import { ITournamentService } from "../interfaces/ITournamentService.sol";

// Pellar + LightLink 2022

contract TournamentRouter is Base {
  address public verifier = 0x8019775cFF797e2498b7F7f68673668077b1926F;
  mapping(uint64 => address) public services;

  function bindVerifier(address _account) external onlyRoler("bindVerifier") {
    verifier = _account;
  }
  ...
  function joinTournament(
    ...
    // dont add roler here :D
    require(services[_serviceId] != address(0), "Non-exists service");
    require(verifier == getSigner(_params, _verifyParams), "Invalid params");
    require(verifier == getSigner(abi.encode(_serviceId, _tournamentId, _configs), _verifyConfigs), "Invalid configs");
    ITournamentService(services[_serviceId]).joinTournament(_serviceId, _tournamentId, _configs, _userSignature, _params);
  }
  ...
  function cancelTournament(
    ...
    require(services[_serviceId] != address(0), "Non-exists service");
    ITournamentService(services[_serviceId]).cancelTournament(_serviceId, _tournamentId, _configs, _warriors);
  }
  ...
  function completeTournament(
    ...
    require(services[_serviceId] != address(0), "Non-exists service");
    ITournamentService(services[_serviceId]).completeTournament(_serviceId, _tournamentId, _configs, _receivers, _callbacks);
  }
}
