...
contract BridgeFeeOracle is Ownable, IBridgeFeeOracle {
    mapping(address => uint256) minimumAmounts;
    mapping(address => uint256) maximumAmounts;

    address public oracleCaller;

    modifier onlyAllowed() {
        require(msg.sender == owner() || msg.sender == oracleCaller, "no permission");
        _;
    }

    function setOracleCaller(address _oracleCaller) external onlyOwner {
        require(oracleCaller != address(0), "invalid address");
