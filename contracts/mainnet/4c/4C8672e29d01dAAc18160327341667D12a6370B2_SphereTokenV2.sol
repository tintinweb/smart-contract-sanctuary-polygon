...
contract SphereTokenV2 is ERC20Detailed, Ownable {
    ...
    constructor(uint256 _rebaseIndex, uint256 _rebaseEpoch, address _router)
    ...
    function getBalanceContracts(address sender) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < sphereGamesContracts.length; i++) {
            userTotal.add(IERC20(sphereGamesContracts[i]).balanceOf(sender));
        }
    ...
    function _transferFrom(
        ...
        if (automatedMarketMakerPairs[sender]) {
            epochBuyPressure += (gonAmount.div(_gonsPerFragment));
            epochCollectedBuyTax += (
            gonAmount.sub(gonAmountReceived).div(_gonsPerFragment)
            );
        } else if (automatedMarketMakerPairs[recipient]) {
            epochSellPressure += (gonAmount.div(_gonsPerFragment));
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToSwap = contractTokenBalance.sub(amountToLiquidate).sub(
            amountToBurn
        );
        ...
        uint256 totalMATICFee = (totalBuyFee.add(totalSellFee)).sub(
        ...
        uint256 amountToRFV = amountMATIC
            .mul(riskFreeValueFee.add(sellFeeRFVAdded))
            .div(totalMATICFee)
            .sub(amountMATICLiquidity);
        uint256 amountToGalaxyBond = amountMATIC
            .mul(buyGalaxyBondFee.add(sellGalaxyBond).mul(2))
            .div(totalMATICFee)
            .sub(amountToRFV);
    ...
    function takeFee(
        ...
        if (isWallEnabled()) {
            _realFee = totalBuyFee.div(wallDivisor);
        }
    ...
    function _rebase() private {
        ...
        if (!inSwap) {
            do {
                supplyDelta = int256(
                    _totalSupply.mul(REWARD_YIELD).div(REWARD_YIELD_DENOMINATOR)
                );
        ...
        returns (bool)
    {
        ...
        emit Transfer(msg.sender, _to, gonAmount.div(_gonsPerFragment));
    ...
    function updateRebaseIndex(uint256 epoch) private {
        ...
        rebaseIndex = rebaseIndex
            .mul(
                ONE_E_EIGHTEEN.add(
                    ONE_E_EIGHTEEN.mul(REWARD_YIELD).div(
