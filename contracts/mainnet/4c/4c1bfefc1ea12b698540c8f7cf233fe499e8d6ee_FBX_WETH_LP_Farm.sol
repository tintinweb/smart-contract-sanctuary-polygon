...
// File: contracts/FBX_WETH_LP_Farm.sol
...
contract FBX_WETH_LP_Farm is Ownable {

    address public constant LP_TOKEN = 0xcfFbFA978Ac3fB10e829cA6b763c307daAFe8a77;
    address public constant REWARD_TOKEN = 0xD125443F38A69d776177c2B9c041f462936F8218;

    using SafeERC20 for IERC20;
    IERC20 public constant SAFE_LP_TOKEN = IERC20(LP_TOKEN);
    IERC20 public constant SAFE_REWARD_TOKEN = IERC20(REWARD_TOKEN);
    ...
    function claimableRewards(address account) public view returns (uint256) {
        ...
        if (start > lastClaim[account]) {
            elapsedTime = block.timestamp - start;
        }
        if (elapsedTime > 15 * 24 * 3600) {
            elapsedTime = 15 * 24 * 3600; // Max reward period is 15 days
        }
        uint256 rewards = dailyRewards * stakedTokens[account] / totalStakedTokens * elapsedTime / 24 / 3600;
        uint256 contractBalance = SAFE_REWARD_TOKEN.balanceOf(address(this));
    ...
    function claimRewards() public returns (uint256) {
        ...
        if (rewards > 0) {
            SAFE_REWARD_TOKEN.approve(address(this), rewards);
            SAFE_REWARD_TOKEN.safeTransfer(msg.sender, rewards);
        }
        return rewards;
    }
    ...
    function stake(uint256 amount) public {
        ...
        claimRewards();
        SAFE_LP_TOKEN.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function unstake(uint256 amount) public {
        ...
        stakedTokens[msg.sender] -= amount;
        totalStakedTokens -= amount;
        SAFE_LP_TOKEN.safeTransfer(msg.sender, amount);
    }
}