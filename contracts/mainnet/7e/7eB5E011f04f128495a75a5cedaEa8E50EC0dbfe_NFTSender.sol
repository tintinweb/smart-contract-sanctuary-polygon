...
contract NFTSender is AccessControlEnumerable {
  ...
  function send(address _receiver, address _nft, uint256[] calldata _tokenIds) external {
    for (uint256 i = 0; i < _tokenIds.length; i++) {
      IERC721(_nft).safeTransferFrom(_msgSender(), _receiver, _tokenIds[i]);
    }
  }
