...
contract RewardControl is RoleControl {
  ...
  function storeReward(RewardArgs memory args) private {
    Reward memory r;
    bytes32 hashedWallet = hashAddressAndString(args.receiver, args.rewardId);
  ...
  function renewReward(string memory rewardId, address receiver) public onlyAdminOrOperator {
    bytes32 hashedWallet = hashAddressAndString(receiver, rewardId);

    reward[hashedWallet].claimableAt = block.timestamp;
  }

  function storeRewards(RewardArgs[] memory rewards) public onlyAdminOrOperator {
  ...
  function hashAddressAndString(address _address, string memory rewardId) public pure returns (bytes32) {
    bytes memory addressBytes = abi.encodePacked(_address);
    bytes memory strBytes = bytes(rewardId);
...
contract SnackclubTournamentsPayout is RewardControl {
  ...
  function claimReward(string memory rewardId) public {
    bytes32 hashedWallet = hashAddressAndString(msg.sender, rewardId);
