...
contract DebtRepaymentGRcoinToken is Ownable, ReentrancyGuard, ERC20 {
    ...
    modifier mintable(uint256 amount) {
        require(amount + totalSupply() <= MAX_SUPPLY, "DebtRepaymentGRcoinToken: amount surpasses max supply");
        _;
    }
    modifier isUnlocked() {
        require(!locked, "DebtRepaymentGRcoinToken: contract is currently locked");
        _;
    }

    receive() external payable {
        require(totalSupply() != 0, "DebtRepaymentGRcoinToken: No tokens minted");
    ...
    function withdrawDividends() external nonReentrant isUnlocked {
        ...
        require(holderBalance != 0, "DebtRepaymentGRcoinToken: caller possess no shares");
        ...
        require(success, "DebtRepaymentGRcoinToken: Could not withdraw eth");
    }
