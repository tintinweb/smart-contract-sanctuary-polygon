...
contract LootBoxFacet {
    ...
    function StashLootBoxGenerateMessageHash(
        ...
        bytes32 structHash = keccak256(
            abi.encode(
                keccak256(
                    "LootBoxPayload(address player, uint256 poolId, uint256 amount, uint256 roundTripId, uint256 bundleId, uint256 blockDeadline)"
                ),
                player,
                poolId,
                amount,
    ...
    function StashLootBoxWithSignature(
        ...
        LibTerminusToken.stashLootBox(msg.sender, poolId, roundTripId, amount);
    }
...
library LibTerminusToken {
    ...
    bytes32 private constant TERMINUS_STORAGE_POSITION =
        keccak256("CryptoUnicorns.Terminus.storage");

    struct TerminusStorage {
        address terminusTokenAddress;
        mapping(uint256 => bool) keystonePoolIds;
        mapping(uint256 => bool) lootboxPoolIds;
    }

    function terminusStorage() internal pure returns (TerminusStorage storage ts) {
    ...
    function setTerminusTokenAddress(address _terminusTokenAddress) internal {
        // We double-enforce contract ownership here because
        // this functionality needs to be highly protected.
        LibDiamond.enforceIsContractOwner();
        terminusStorage().terminusTokenAddress = _terminusTokenAddress;
    }

    function getTerminusTokenAddress() internal view returns (address) {
        return terminusStorage().terminusTokenAddress;
    }
    ...
    function stashLootBox(
        ...
        require(isLootboxValid(poolId), "LibTerminusToken: Desired pool is not a lootbox pool.");
        address tokenAddress = terminusStorage().terminusTokenAddress;
    ...
    function _unlockKeystonesFromGame(uint256 poolId, uint256 amount, uint256 roundTripId) internal {
        require(keystonePoolIsValid(poolId), "LibTerminusToken: Desired pool is not a keystone pool.");

        TerminusFacet terminusContract = TerminusFacet(terminusStorage().terminusTokenAddress);
        
        require(terminusContract.terminusPoolSupply(poolId) + amount <= terminusContract.terminusPoolCapacity(poolId), "LibTerminusToken: Desired amount is lower than the pool capacity allows.");

        terminusContract.mint(msg.sender, poolId, amount, "");
...
library LibDiamond {
    ...
    //  TODO: De-duplicate this event with IERC173
    ...
    //  TODO: De-duplicate this event with IDiamondCut
