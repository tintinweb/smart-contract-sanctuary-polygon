...
contract Invitation is ERC721Enumerable, Ownable {

    string public baseURI;
    uint256 public minted = 0;

    constructor(string memory _newBaseURI) ERC721("Invitation", "INV") {
        baseURI = _newBaseURI;
    }

    function airdrop(address[] memory _wallets) external onlyOwner {
        for(uint256 i; i < _wallets.length; i++){
            _safeMint(_wallets[i], minted + 1);
            minted++;
        }
    }
