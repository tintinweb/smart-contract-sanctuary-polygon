...
contract MegaArb {
  ...
  uint256 private constant _FLAG_NONE = 0;
  uint256 private constant _FLAG_ALL = _FLAG_BREAK_IF_ZERO | _FLAG_REEVALUATE_LEFT | _FLAG_REEVALUATE_RIGHT;
  ...
  function optimize(
    ...
    unchecked {
      ...
      while (_right - _left >= _precision) {
        ...
        if (_getFlag(_flags, _FLAG_REEVALUATE_LEFT)) {
          // Calls the arb function of the MegaArb contract
          try MegaArb(this).arb(_tokenIn, _left, _funding, _swaps, address(this)) returns (uint256) {
            revert('SR'); // should always revert
          } catch (bytes memory _result) {
            assembly {
              if iszero(eq(mload(_result), 32)) {
          ...
          if (_getFlag(_flags, _FLAG_BREAK_IF_ZERO)) {
            if (_leftAmountOut == 0) {
              _left = _right = 0;
              break;
            }
            _flags = _setFlags(_flags, _FLAG_NONE, _FLAG_BREAK_IF_ZERO);
          }
        }
        // Evaluates the right third
        if (_getFlag(_flags, _FLAG_REEVALUATE_RIGHT)) {
          // Calls the arb function of the MegaArb contract
          try MegaArb(this).arb(_tokenIn, _right, _funding, _swaps, address(this)) returns (uint256) {
            revert('SR'); // should always revert
          } catch (bytes memory _result) {
            assembly {
              if iszero(eq(mload(_result), 32)) {
        ...
        if (_leftAmountOut <= _rightAmountOut) {
          _left = _left + _third;
          _flags = _setFlags(_flags, _FLAG_REEVALUATE_LEFT, _FLAG_REEVALUATE_RIGHT);
          _amountIn = _right;
          _amountOut = _rightAmountOut;
        } else {
          _right = _right - _third;
          _flags = _setFlags(_flags, _FLAG_REEVALUATE_RIGHT, _FLAG_REEVALUATE_LEFT);
          _amountIn - _left;
  ...
  function _getFlag(uint256 _flags, uint256 _flag) internal pure returns (bool) {
    return 0 != _flags & _flag;
  }

  function _setFlags(uint256 _current, uint256 _on, uint256 _off) internal pure returns (uint256 _flag) {
    _flag = (_current | _on) & (~_off);
  }
