...
contract Staker is IERC721Receiver {
    ...
    function unstake(uint[] memory _id) public {
        for(uint i; i < _id.length; i++) {
            ...
            for(uint e; e < ownerOfTokens[msg.sender].length; e++) {
                if(ownerOfTokens[msg.sender][e] == _tempID){
                ownerOfTokens[msg.sender][e] = ownerOfTokens[msg.sender][(ownerOfTokens[msg.sender].length - 1)];
    ...
    function viewTokens(address _address) public view returns(uint) {
        uint _tempTokens;
        _tempTokens += viewAllocatedTokens(_address);
        _tempTokens += viewActiveTokens(_address);
        _tempTokens -= viewsubtractedTokens(_address);
        return _tempTokens;
    }
