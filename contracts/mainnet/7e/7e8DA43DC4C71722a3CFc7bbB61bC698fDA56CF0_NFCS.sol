...
import {ROLE_PAYMENT_CONTRACT, ROLE_ORACLE} from "../Globals.sol";
    ...
    Version
{
    ...
    function mintToken(
        ...
        for (uint256 i = 0; i < bundle.length; i++) {
            ...
            secondaryToPrimary[bundle[i]] = msg.sender;
            ...
            if (bundle[i] == msg.sender) {
                isValidMsgSender = true;
            }
        }
        require(isValidMsgSender, Errors.NFCS_SENDER_IS_NOT_IN_BUNDLE);
        // Mint new token
    ...
    function addAddressToBundle(
        ...
        uint256 _nonce,
        string memory version
    ) external whenNotPaused checkVersion(version) {
        require(_mintedNonce[msg.sender] == true, Errors.NFCS_TOKEN_NOT_MINTED);
        require(secondaryToPrimary[msg.sender] == msg.sender, Errors.NFCS_UPDATE_WITHOUT_POPULATE);
        ...
        emit BundleUpdate(block.timestamp, msg.sender, tokenId, bundle);
    }

    function populateSecondaryToPrimary(uint256 tokenId) external {
        address[] memory bundle = _tokenBundle[tokenId];
        require(bundle.length > 0, Errors.NFCS_NOTHING_TO_POPULATE);
        address owner = ownerOf(tokenId);
        for (uint256 i = 0; i < bundle.length; i++) {
            require(secondaryToPrimary[bundle[i]] == address(0), Errors.NFCS_ALREADY_POPULATED);
            secondaryToPrimary[bundle[i]] = owner;
        }
        emit Migration(block.timestamp, tokenId, msg.sender);
    }
...
string constant NFCS_VERSION = "1.0.2";
...
library Errors {
    ...
    string public constant NFCS_NO_PRIMARY_ADDRESS = "8"; //  No primary address found
    string public constant NFCS_ALREADY_POPULATED = "9"; //  Mapping of secondary addresses to primary is already performed
    string public constant NFCS_NOTHING_TO_POPULATE = "10"; //  There is nothing to transfer
