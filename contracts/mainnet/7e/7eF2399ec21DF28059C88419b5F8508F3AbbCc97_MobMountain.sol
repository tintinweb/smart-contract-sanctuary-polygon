    ...
    ConfirmedOwner(msg.sender)
{
  ...
  function beginRun(uint256 tokenId) external whenNotPaused  {
    ...
    require(weeksToRaidersToRunState[thisWeek][tokenId] == RunStates.NOT_RUN, "Already began run");
  ...
  function finishRun(RequestStatus memory request) internal {
    ...
    if (IERC721(raidersAddress).isApprovedForAll(raiderOwner, address(this))) {
      ...
      uint8 finalRunResultIndex = uint8((request.randomWords[0] % randomResultPool.length) + 1);
      uint8 finalRunResult = randomResultPool[finalRunResultIndex];


      uint pickedMobTokenIdIndex = (request.randomWords[0] % stakedTokenIds.length) + 1;
