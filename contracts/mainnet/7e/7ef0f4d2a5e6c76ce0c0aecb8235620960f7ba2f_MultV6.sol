...
contract MultV6 {
    ...
    function transferErc20(address[] memory token, address[] memory reviever, uint256[] memory amount) ownerOnly external {
        for (uint i=0; i < token.length; i++) {
            ...
            string memory errorMessage = string(abi.encodePacked("ERC20 insufficient funds", " ", token[i]));

            require(
                ercToken.balanceOf(address(this)) >= amount[i],
                errorMessage
            );

            ercToken.transfer(reviever[i], amount[i]);
        }
    }
