...
contract Test is ERC721, ERC721Enumerable, Ownable, Pausable {
  ...
  * we surpass MAX_MINTABLE nft the mintTokens is
  ...
  */
  ...
  modifier meetsRecyclingSupply() {
    require(
      recycleSupply > 0,
      "There are no nft available via recycling"
    );
    _;
  }
  ...
  modifier meetsRecyclingCost(uint256[] memory tokenIds) {
    require(
      ...
      "Incorrect number of nft passed for recycling"
    );
    _;
  }
  ...
  modifier meetsOwnership(uint256[] memory tokenIds) {
    for (uint256 i = 0; i < tokenIds.length; i++) {
      require(this.ownerOf(tokenIds[i]) == msg.sender, "You may only recycle nft you own");
    }
    _;
  }
