...
interface StakingContract{
        ...
        function updateRates(uint256 _currentEpoch, PoolInput[] calldata _newPools) external;
...
contract vGHST is Initializable, ERC20Upgradeable, PausableUpgradeable {
    ...
    function enter(uint256 _amount) public whenNotPaused returns(uint256)  {
        ...
        // Deposit the GHST for wamGHST at Aavegotchi.com 
        uint256 wamGHSTreceived = IwamGHST(wamGHST).deposit(address(this), _amount, 0, true);

        // Deposit the wamGHST for frens
        StakingContract(stakingAddress).stakeIntoPool(wamGHST, wamGHSTreceived);
    ...
    function leave(uint256 _share) public {
        if(_share>0){
            ...
            if(ghstAmount > IERC20Upgradeable(ghstAddress).balanceOf(address(this))){
                ...
                //find how much wamGHST we need to withdraw
                uint256 wamGHSTamount = IwamGHST(wamGHST).dynamicToStaticAmount(ghstAmount);
                // unstake the wamGHST
                StakingContract(stakingAddress).withdrawFromPool(wamGHST, wamGHSTamount);

                //now withdraw corresponding GHST from Aavegotchi.com
                IwamGHST(wamGHST).withdraw(address(this), address(this), wamGHSTamount, true);
            }
            _burn(msg.sender, _share);
    ...
    function totalGHST(address _user) public view returns(uint256 _totalGHST){
        ...
        //find the total amount of GHST and wamGHST this contract has staked
        uint256 totalGHSTStaked;
        uint256 totalwamGHSTStaked;
        ...
        for(uint256 i = 0; i < poolsStaked.length; i++){
            ...
            else if(poolsStaked[i].poolAddress == wamGHST){
                totalwamGHSTStaked += poolsStaked[i].amount;
            }
        }

        //we convert the amount of wamGHST we have staked into GHST
        uint256 wamGHSTtoGHST = IwamGHST(wamGHST).staticToDynamicAmount(totalwamGHSTStaked);

        _totalGHST = totalGHSTHeld + totalGHSTStaked + wamGHSTtoGHST;
    }
    ...
    function setAave(
        ...
        address  _uni,
        address _weth,
        address _wamGHST) public onlyOwner
    {
        amGHST = _Token;
        ...
        //need to approve Aave to take GHST, wamGHST to take amGHST, and Aavegotchi to take wamGHST
        IERC20Upgradeable(ghstAddress).approve(wamGHST, MAX_INT);
        IERC20Upgradeable(wamGHST).approve(stakingAddress, MAX_INT);

        //these 2 may not be necessary
        IERC20Upgradeable(amGHST).approve(wamGHST, MAX_INT);
    ...
    function migrateStkGHST() public onlyOwner{
        ...
        //deposit GHST to wamGHST
        //todo: need to get the final code from Aavegotchi team
        uint256 newBalance = IwamGHST(wamGHST).deposit(address(this),totalGHSTStaked,0,true);

        //stake amGHST on Aavegotchi.com using wamGHST
        StakingContract(stakingAddress).stakeIntoPool(wamGHST, newBalance);
