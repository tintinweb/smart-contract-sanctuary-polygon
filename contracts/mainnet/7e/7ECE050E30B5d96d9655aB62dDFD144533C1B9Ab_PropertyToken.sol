...
/// @title Investline Property Token Contract
    ...
    BasicMetaTransaction
{
    ...
    */
    function uri(
        ...
        require(exists(_tokenId), "uri: tokenId does not exist");
    ...
    function mintNewPropertyToken(
        ...
        require(bytes(__uri).length > 0, "mintNewPropertyToken: URI not found");
        require(
            _amount != 0,
            "mintNewPropertyToken: _amount must be greater than zero"
        );
        require(
            _propOwnerAddress != address(0),
            "mintNewPropertyToken: _propOwnerAddress cannot be zero address"
        );
        emit MintedNewPropertyToken(_currentTokenId, _amount, __uri);
        _setURI(_currentTokenId, __uri);
        _mint(_propOwnerAddress, _currentTokenId, _amount, "");
    }
    ...
    function markPropertyOffset(uint256 _tokenId) external onlyOwner {
        require(exists(_tokenId), "markPropertyOffset: tokenId does not exist");
    ...
    function recoverPropertyFromOffset(uint256 _tokenId) external onlyOwner {
        require(
            exists(_tokenId),
            "recoverPropertyFromOffset: tokenId does not exist"
        );
    ...
    function updateTokenURI(
        ...
        require(exists(_tokenId), "updateTokenURI: tokenId does not exist");
        require(bytes(__uri).length > 0, "updateTokenURI: URI not found");
    ...
    */
    ...
    function _safeTransferFrom(
        ...
        require(
            offsetStatus[id] == false,
            "_safeTransferFrom: Token is marked to offset, can not transfer"
        );
