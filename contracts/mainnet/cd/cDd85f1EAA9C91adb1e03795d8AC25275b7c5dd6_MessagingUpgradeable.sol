...
contract MessagingUpgradeable is Initializable, OwnableUpgradeable {
    ...
    function _createMessageRecord(
        ...
        emit MessageSent(_from, _to, messageId, _textMessageReceiver, _textMessageSender, attachmentIds, _isEncrypted, block.timestamp, ++messagesCount);
    } 
...
contract UnifarmAccountsUpgradeable is Initializable, OwnableUpgradeable {
    ...
    bytes32 public constant SUBSC_PERMIT_TYPEHASH = keccak256("SubscPermit(address user,bytes32 appID,bool subscriptionStatus,uint256 nonce,uint256 deadline)");
    ...
    event AppSubscribed(bytes32 appID, address subscriber, uint256 count, uint256 totalCount);

    event AppUnSubscribed(bytes32 appID, address subscriber, uint256 count, uint256 totalCount);
        ...
        GasNotZero(_msgSender(), isOauthUser) 
    {
        ...
        _updateDappTextInfo(_appId, _appName, _appUrl, _appDesc, _appCategory, _appTags, _appSocial);
    ...
    function _subscribeToDapp(
        ...
        if (subscriptionStatus)
            emit AppSubscribed(appID, user, ++subscriberCount[appID], ++noOfSubscribers);
        else
            emit AppUnSubscribed(appID, user, --subscriberCount[appID], --noOfSubscribers);
    ...
    function subscribeWithPermit(
        ...
        abi.encodePacked(
                "\x19\x01",
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(SUBSC_PERMIT_TYPEHASH, user, appID, subscriptionStatus, nonce[user]++, deadline))
            )
        );

        address recoveredUser = ecrecover(digest, v, r, s);
        require(recoveredUser != address(0) && 
            (recoveredUser == user || recoveredUser == getSecondaryWalletAccount(user)), "INVALID_SIGN");
    ...
    function createWallet(
        ...
        emit WalletCreated(_msgSender(), _account, isOauthUser, oAuthEncryptedUserId, ++noOfWallets);
    }
    ...
    function _updateGaslessData(
        uint256 _gasLeftInit
    ) internal {
        if (msg.sender == trustedForwarder) 
