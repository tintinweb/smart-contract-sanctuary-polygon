...
{    
    ...
    function getPayoutAndCompoundShare(uint256 _layer) internal view returns (uint256 compoundShares, uint256 payoutShares)
    {
    ...
    ML_TransferHelper
{
    ...
    //========================
    // PAYOUT INFO FUNCTIONS
    //========================

    function pendingPayouts(address _user) public view returns (uint256)
    {
        //get data
        UserInfo storage user = users[_user];
        LayerInfo storage layer = layers[user.layer];        
    ...
    {   
        ...
        //claim or index
        uint256 pending = pendingPayouts(_user);
        if (pending > 0)
        {
