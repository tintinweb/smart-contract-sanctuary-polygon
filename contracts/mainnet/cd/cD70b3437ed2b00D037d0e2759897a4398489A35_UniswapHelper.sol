...
contract UniswapHelper is IUniswapV3MintCallback {
    ...
    int24 public tickRebalanceDeviation = 1000; // +/- 150 USD from the initial tick
    ...
    uint256 internal factor = 100; // eta / 100 -> 0,1%
        ...
        returns (uint256 wantBalance, uint256 wethBalance)
    {
        ...
        uint256 permanentLimitOrderAmount = eta.mul(limitOrderMinSize).div(basisOne);

        if (limitOrderSize < permanentLimitOrderAmount) {
            return (wantBalance, wethBalance);
        }

        if (limitOrderSize > maxOrderSize) divideBy = 2;

        if (wantBalance < wantDust) {
            uint256 spareAmount = _wantToWeth(limitOrderSize.sub(permanentLimitOrderAmount), false);
            uint256 toInvest = wethBalance.sub(spareAmount.div(divideBy));
            return (wantBalance, toInvest);
        }
        if (wethBalance < wethDust) {
            uint256 spareAmount = limitOrderSize.sub(permanentLimitOrderAmount);
            uint256 toInvest = wantBalance.sub(spareAmount.div(divideBy));
            return (toInvest, wethBalance);
        }
    }
    ...
    function _wantToWeth(uint256 _wantAmount, bool _oracle) public view returns (uint256) {
