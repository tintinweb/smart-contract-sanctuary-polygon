...
contract LicenseSale is ERC721Holder, Ownable, Pausable {
    ...
    uint256 public MAX_TRADABLE_TOKEN_ID = 1000000;
    NFT721 public nft;
    ...
    function buyToken(uint256 _tokenId) public payable whenNotPaused {
        ...
        (bool sent, ) = payable(_tokenSellers[_tokenId]).call{value: msg.value}("");
...
contract NFT721 is ERC721, ERC721URIStorage {
    ...
    function safeMint(
      address to,
      string memory uri,
      uint256 _licenseToken,
      string memory category) external {
        require(!license.isExpired(_licenseToken), "NFT721: expired");
        require(!license.isValidCategory(_licenseToken, category), "NFT721: not a valid category");
...
contract LicenseNFT is ERC721, ERC721URIStorage, AccessControl {
    using Counters for Counters.Counter;
    ...
    function isValidCategory(uint256 tokenId, string memory cat) public view returns (bool) {
      string[] memory userCategory = userInfo[tokenId].categories;

      for (uint i; i < userCategory.length; i++) {
        if (keccak256(bytes(userCategory[i])) == keccak256(bytes(cat))) {
            return true;
        }
      }
      return false;
    ...
    function getDetails(uint256 tokenId) external view returns (uint256, string[] memory, string memory, string memory, bool){
      return (
        expiry[tokenId],
        userInfo[tokenId].categories,
        userInfo[tokenId].name,
        userInfo[tokenId].bio,
        isExpired(tokenId)
      );
    }
}
