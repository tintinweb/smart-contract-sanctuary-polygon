...
contract BTPT is ERC20, Ownable {
    ...
    /// @notice Sets owner and mint tokens once
    /// @param newOwner A new owner address
    /// @dev You can change the owner with another function, but this one can be used once
    function setOwnerOnce(address newOwner) public onlyOwner {
        require(!isOwnerSet, "Assigning an owner with a mint can be done once");

        transferOwnership(newOwner);
        /// Mint 1M BTPT to the contract
        _mint(
            address(this), 
            1 * 1000 * 1000 ether
        );
        /// Mint 100K BTPT to the owner
        _mint(
            newOwner,
            100 * 1000 ether
        );
