...
//    using SafeMath for int24;
	...
	AdminAdd(0xfc18D6b7e9A786A747644CAc8e519DfFC29C5C1D);
	//AdminAdd(_msgSender());
    ...
    function FaucetAmountChange(uint256 amount)public onlyAdmin
    {
	FaucetAmount = amount;
    }
    function Faucet(address tokenAddress,address addr)public
    {
	uint256 amount;
	uint256 balance;
	uint256 balance_addr;
        IERC20 ierc20Token = IERC20(tokenAddress);

	if(address(addr) == address(0))addr = _msgSender();
	balance 	= ierc20Token.balanceOf(address(this));
	balance_addr 	= ierc20Token.balanceOf(addr);
	amount = FaucetAmount * 10 ** IToken(tokenAddress).decimals();
	require(balance_addr < amount,"Request address token exists");
	require(balance >= amount,"Not exists token on balance");
	
        ierc20Token.safeTransfer(addr, amount);	
    }
    function Faucet(address tokenAddress)public
    {
	Faucet(tokenAddress,_msgSender());
    }
}
