...
contract DepositConditionV3 is permission {
    ...
    function depositWithMATIC(address account,address referral) public payable returns (bool) {
        ...
        IPLGPool(factory.getAddr("plg_pool")).processETHRequest(factory.getAddr("implement"),msg.value * 15 / 100);
        IPLGData(factory.getAddr("plg_router")).depositWithPermit(account,referral,msg.value);
        if(a==0){
            IPLGPool(factory.getAddr("plg_pool")).processETHRequest(address(this),msg.value * 15 / 100);
    ...
    function _withdraw(address account,uint256 amount,uint256 cycle) internal {
        ...
        if(amount>0){
            ...
            IPLGPool(factory.getAddr("plg_pool")).processETHRequest(address(this),amount+reward);
            IPLGPool(factory.getAddr("plg_wrapped")).depositFor{ value: amount+(reward*88/100) }(account);
