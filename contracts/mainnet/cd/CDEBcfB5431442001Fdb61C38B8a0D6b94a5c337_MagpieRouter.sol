...
contract MagpieRouter is ReentrancyGuard, Ownable, IMagpieRouter {
  ...
  function addAmms(Amm[] calldata amms) external override onlyOwner {
    require(amms.length > 0, 'MagpieRouter: invalid amms');
    for (uint256 i = 0; i < amms.length; i++) {
      require(amms[i].id != address(0), 'MagpieRouter: invalid amm address');
      require(amms[i].index > 0, 'MagpieRouter: invalid amm index');
      require(amms[i].protocolIndex > 0, 'MagpieRouter: invalid amm protocolIndex');

      _amms[amms[i].index] = amms[i];
    }
    
    emit AmmsAdded(amms, msg.sender);
  }

  function removeAmm(uint16 ammIndex) external override onlyOwner {
    _amms[ammIndex].id = address(0);

    emit AmmRemoved(_amms[ammIndex], msg.sender);
  }
  ...
  function _swapHop(uint256 amountIn, Hop memory hop, address[] memory assets, uint256 deadline) internal returns (uint256) {
    ...
    if (amm.protocolIndex == 2) {
      return _swapUniswapV2(amountIn, hop, assets, deadline);
    } else if (amm.protocolIndex == 1 || amm.protocolIndex == 5) {
