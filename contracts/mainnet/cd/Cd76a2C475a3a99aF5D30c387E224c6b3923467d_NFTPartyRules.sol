...
/**
 * @title ERC721 for tokens confirming the release of PartyRules.
 */
contract NFTPartyRules is
    Base,
    NFTPartyMembershipOwnershipPauserMintPriceTransferTaxBase,
    MintPowerPointsStorage,
    MinPowerPointsStorage
{
    NFTPartyDraftRules public nftPartyDraftRules;

    struct TokenInfo {
        uint256 mintedTimestamp;
        uint256 partyDraftRulesTokenId;
    }

    mapping(uint256 /*tokenId*/ => TokenInfo) internal tokenInfo;

    event NoMintBecauseExpired(uint256 indexed draftTokenId);

    /// @notice get token info
    /// @param tokenId token id
    /// @return mintedTimestamp mintedTimestamp
    /// @return partyDraftRulesTokenId partyDraftRulesTokenId
    function getTokenInfo(uint256 tokenId) external view returns(
        ...
        partyDraftRulesTokenId = o.partyDraftRulesTokenId;
    }
    ...
    /// @notice 3rd initalization
    /// @param mintPowerPointsValue mintPowerPoints value
    /// @param nftPartyDraftRulesAddress nftPartyDraftRules address
    function initialize3(
        uint256 mintPowerPointsValue,
        address nftPartyDraftRulesAddress
    ) external virtual initializer3 onlyNotZeroAddress(nftPartyDraftRulesAddress) {
        __MintPowerPointsStorage_init_unchained(mintPowerPointsValue);
        nftPartyDraftRules = NFTPartyDraftRules(nftPartyDraftRulesAddress);
    }

    /// @notice mint new token
    /// @param draftTokenId draft token id
    function mint(uint256 draftTokenId) external {
        uint256 tokenId = ++_lastTokenId;
        require(nftPartyDraftRules.ownerOf(draftTokenId) == _msgSender(), "NOT_OWNER");

        (
        /* uint256 draft_mintedTimestamp */,
        NFTPartyDraftRules.Status draft_status,
        /* uint256 draft_votesYes */,
        /* uint256 draft_votesNo */,
        /* uint256 draft_rulesTokenId */
        ) = nftPartyDraftRules.getTokenInfo(draftTokenId);
        require(draft_status == NFTPartyDraftRules.Status.Accepted, "NOT_ACCEPTED");

        NFTPartyDraftRules.Status draftStatus = nftPartyDraftRules.executeAccepted({
            tokenId: draftTokenId,
            rulesTokenId: tokenId
        });
        if (draftStatus == NFTPartyDraftRules.Status.Expired) {
            emit NoMintBecauseExpired(draftTokenId);
            return;
        }
        ...
        tokenInfo[tokenId].partyDraftRulesTokenId = draftTokenId;
    }
