...
import { ERC4907ProfitShareUpgradeable } from "@oriumnetwork/orium-contracts/contracts-upgradeable/ERC4907ProfitShareUpgradeable/ERC4907ProfitShareUpgradeable.sol";
import { Initializable } from "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import { AccessControlUpgradeable } from "@openzeppelin/contracts-upgradeable/access/AccessControlUpgradeable.sol";
import { IERC721Metadata } from "@openzeppelin/contracts/token/ERC721/extensions/IERC721Metadata.sol";
import { IERC721 } from "@openzeppelin/contracts/token/ERC721/IERC721.sol";

contract ProfitShareImpl is ERC4907ProfitShareUpgradeable, AccessControlUpgradeable {
    address public originalNft;
    bytes32 private constant WRAPPER_ROLE = keccak256("WRAPPER_ROLE");

    function initialize(address _orginalNft, address _operator) public initializer {
        __ERC721_init("", "");
        __AccessControl_init();

        originalNft = _orginalNft;

        _setupRole(DEFAULT_ADMIN_ROLE, _operator);
        _setupRole(WRAPPER_ROLE, msg.sender);
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC4907ProfitShareUpgradeable, AccessControlUpgradeable) returns (bool) {
        return super.supportsInterface(interfaceId);
    }

    function mint(address to_, uint256 id_) public onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256) {
        _mint(to_, id_);
        return id_;
    }

    function burn(uint256 id_) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _burn(id_);
    }

    function tokenURI(uint256 id_) public view override returns (string memory _tokenURI) {
        if (_hasMetadata()) {
            _tokenURI = IERC721Metadata(originalNft).tokenURI(id_);
        } else {
            _tokenURI = "";
        }
    }
    ...
    function name() public view override returns (string memory _name) {
        if (_hasMetadata()) {
            _name = string(abi.encodePacked("Wrapped ", IERC721Metadata(originalNft).name()));
        } else {
            _name = "Wrapped NFT";
        }
    }
    ...
    function symbol() public view override returns (string memory _symbol) {
        if (_hasMetadata()) {
            _symbol = string(abi.encodePacked("W", IERC721Metadata(originalNft).symbol()));
        } else {
            _symbol = "WNFT";
        }
    }

    function _hasMetadata() internal view returns (bool) {
        return IERC721(originalNft).supportsInterface(type(IERC721Metadata).interfaceId);
    }
    ...
    function batchSetUserProfitShare(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            setUserProfitShare(tokenIds[i], user, expires, beneficiaries, split);
        }
    }
   ...
   function setUserProfitShare(
    ...
    ) public virtual override(ERC4907ProfitShareUpgradeable) {
        ...
        require(_isApprovedOrOwner(msg.sender, tokenId) || hasRole(WRAPPER_ROLE, msg.sender), "ERC4907ProfitShare: transfer caller is not owner nor approved or wrapper");
...
import { ERC4907Upgradeable } from "./test/ERC4907Upgradeable.sol";
