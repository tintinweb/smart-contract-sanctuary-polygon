    ...
    IERC721ReceiverUpgradeable
{
    ...
    IERC20Upgradeable public salesToken;
    ...
    function listItemMint(
        ...
        require(
            !auctionHandler.auctionExists(collection, tokenID),
            "auction already exists"
        );
        listItem(collection, tokenID, price, true);
    }
    ...
    function listItemSecondary(
        ...
        listItem(collection, tokenID, price, false);
    }
    ...
    function listItem(
        ...
        uint256 listingID = getListingID(collection, tokenID);
        require(
        ...
        emit ListingCreated(collection, tokenID, seller, price, mint);
    ...
    function buyItem(
        ...
        if (listing.mint) {
            listing.collection.mintItem(listing.tokenID, buyer);
        } else {
    ...
    function cancelListing(ICollection collection, uint256 tokenID) external {
        ...
        emit ListingCancelled(collection, tokenID, seller, listing.price);
    ...
    function makeOffer(
        ...
        address buyer = _msgSender();
        require(buyer != collection.ownerOf(tokenID), "buyer is owner");

        uint256 listingID = getListingID(collection, tokenID);

        Offer memory prevOffer = offers[listingID];
        address prevBuyer = prevOffer.buyer;
        uint256 prevPrice = prevOffer.price;

        require(price > prevPrice, "price < prevPrice");
        ...
        emit OfferCreated(collection, tokenID, buyer, price);
        ...
        if (prevBuyer != address(0)) {
            CheckedTransfers.checkTransfer(salesToken, prevBuyer, prevPrice);
        }
    }
    ...
    function acceptOffer(
        ...
        address seller = _msgSender();
        require(collection.ownerOf(tokenID) == seller, "incorrect owner");

        uint256 listingID = getListingID(collection, tokenID);

        Offer memory offer = offers[listingID];
        require(offer.buyer != address(0), "offer not found");
        ...
        collection.safeTransferFrom(seller, offer.buyer, tokenID);

        if (fee > 0) {
            CheckedTransfers.checkTransfer(salesToken, feeCollector, fee);
        }

        CheckedTransfers.checkTransfer(salesToken, seller, sellerTransfer);
    }
    ...
    function cancelOffer(ICollection collection, uint256 tokenID) external {
        ...
        emit OfferCancelled(collection, tokenID, buyer, offer.price);

        CheckedTransfers.checkTransfer(salesToken, buyer, offer.price);
    }
    ...
    function getListingID(ICollection collection, uint256 tokenID)
        internal
        pure
        returns (uint256)
    {
