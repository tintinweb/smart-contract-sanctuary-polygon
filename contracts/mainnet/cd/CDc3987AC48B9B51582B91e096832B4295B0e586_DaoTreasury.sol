...
// File contracts/treasury/DaoTreasury.sol
...
/*  DaoTreasury.sol ============================================
    The contract is to store the reserve of FrontlineDAO Protocol
...
contract DaoTreasury is Ownable {
    ...
    IDaoStaking public staking;

    constructor(IDaoStaking _staking) {
        staking = _staking;
    }

    // ========== PUBLIC VIEW FUNCTIONS ============

    /// @notice Return ERC-20 balance of DaoTreasury
    /// @param _token Address of the ERC-20 token
    /// @return Balance of the DaoTreasury
    ...
    function requestFund(address _token, uint256 _amount) external {
        require(strategies[msg.sender], "DaoTreasury::requestFund: Only strategies can request fund");
        require(_amount <= balanceOf(_token), "DaoTreasury::requestFund: Request more fund than balance");
    ...
    function addStrategy(address _strategy) external onlyOwner {
        require(_strategy != address(0), "DaoTreasury::addStrategy: invalid address");
        require(!strategies[_strategy], "DaoTreasury::addStrategy: strategy was previously added");
    ...
    function removeStrategy(address _strategy) external onlyOwner {
        require(strategies[_strategy], "DaoTreasury::removeStrategy: strategy not found");
    ...
    function allocateFee(address _token, uint256 _amount) external onlyOwner {
        require(address(staking) != address(0), "DaoTreasury::allocateFee:Fee distributor not set");
        require(_amount > 0, "DaoTreasury::allocateFee: invalid amount");
