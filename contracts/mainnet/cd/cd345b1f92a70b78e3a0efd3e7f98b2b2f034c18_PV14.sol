...
contract PV14 is ERC20Interface, Owned {
    ...
    function balanceOf(address tokenOwner) public view returns (uint balance) {
     ...
     function GiveBonus(address to, uint tokens) public onlyOwner returns(bool success){
    ...
    function SetSystemLock(uint _Lock)public onlyOwner{
   ...
   function SetminBigPoint(uint _minBIGP)public onlyOwner{
       MinBigPoint=_minBIGP;     
       }
   function BIGPSupply(uint _BIGtotalSupply)public onlyOwner{
   ...
   function SetPercentToMem(uint _percent)public onlyOwner{
   ...
   function SetStartPoint(uint _point, string memory _note)public onlyOwner returns(uint,string memory){
       PointSpent = _point;
    ...
    function TogglePoolPoint() public onlyOwner{
        PoolPoint = PoolPointRemain;
    }

    function ViewPoolPoint()public view returns(uint,uint){
    ...
    function SetInitPoolPoint(uint _point)public onlyOwner{
       PoolPoint = _point;
