...
contract Strategy is BaseStrategy, IUniswapV3SwapCallback {
    ...
    // True if we choose to reinvest profits following each harvest
    // bool public reInvest = false;
    ...
    IUniswapHelper.LimitOrderStatus public limit;
    ...
    function adjustPosition(uint256 _debtOutstanding) internal override {
        ...
        if (wantBal > wantDust) {
            ...
            lendHelper.initiateLending();
            // TODO: add check to make sure the money went well
            want.safeTransfer(address(uniHelper), balanceOfWant());
            wETH.safeTransfer(address(uniHelper), balanceOfWeth());
            (IUniswapHelper.LimitOrderStatus _order, bool _isLimitOrderSet) = uniHelper.setOrders();
            limit = _order;
            orderSet = _isLimitOrderSet;
        }
    }

    function rebalance(int24 _tickLower, int24 _tickUpper) public onlyKeepers {
        _rebalance(_tickLower, _tickUpper);
    }

    function autoRebalance() external onlyKeepers {
        (int24 lowerTick, int24 upperTick) = uniHelper.getAutoRebalanceTicks();
        _rebalance(lowerTick, upperTick);
    }

    function secondRebalance() public onlyKeepers {
        if (orderSet == false) return;
        bool shouldRebalance = uniHelper.calcSecondRebalance(limit);
        (int24 baseLower, int24 baseUpper) = uniHelper.getBaseTicks();
        if (shouldRebalance) {
            _rebalance(baseLower, baseUpper);
            orderSet == false;
        }
    }

    function renewLimitOrder() public onlyKeepers {
        uniHelper.closeLimitOrderPosition();
        want.safeTransfer(address(uniHelper), balanceOfWant());
        wETH.safeTransfer(address(uniHelper), balanceOfWeth());
        (IUniswapHelper.LimitOrderStatus _order, bool _isLimitOrderSet) = uniHelper.setLimitOrder();
        limit = _order;
        orderSet = _isLimitOrderSet;
    }
    ...
    function _rebalance(int24 _tickLower, int24 _tickUpper) internal {
        ...
        (uint256 fees0, uint256 fees1) = uniHelper.unwindAllPositions();

        // Update state variables to add liquidity in new range
        uniHelper.setTickParams(_tickLower, _tickUpper);
        ...
        // initiateLending has _adjustToTargetRatio() included inside
        lendHelper.initiateLending();
    ...
    function _withdrawSome(uint256 _initialWantNeeded) internal {
        ...
        if (amountNeeded == 0) return;
        _withdrawFromPrincipal(amountNeeded); // TODO after this executes no funds are sent back to strat and next amount needed is poor calculated
...
interface IUniswapHelper {
    ...
    function unwindAllPositions() external returns (uint256, uint256);

    function setOrders() external returns (LimitOrderStatus order, bool orderSet);

    function setLimitOrder() external returns (LimitOrderStatus order, bool orderSet);
...
interface ILendingHelper {
    ...
    function initiateLending() external;
