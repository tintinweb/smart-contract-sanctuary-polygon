...
contract RedemptionsFunctionsDeploy1155 is
  ...
  Ownable
{
  ...
  constructor(address _storageContract, address _functionsContract, address _xpContract, MinimalForwarder _trustedForwarder)
  ...
  function createRedeemableDeploy1155(
    ...
    if (_requiresPurchase == true) {
      ...
      actions[0] = IXP.Action({
        name: _action.name, points: _action.points, direction: IXP.Direction.decrease, isValid: true
      });

      IXP(xpContract).addActions(_projectId, actions);
    }

    Redemptions1155 new1155Contract = new Redemptions1155(_uri, _royaltyReceiver, _royaltyFee, _name, _symbol);

    new1155Contract.mint(_msgSender(), address(functionsContract), _amount);
    new1155Contract.transferOwnership(_msgSender());
