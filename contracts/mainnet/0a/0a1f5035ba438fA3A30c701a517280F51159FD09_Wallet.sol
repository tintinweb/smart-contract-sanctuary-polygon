...
interface IRunbitCollection {
    struct ShoesCollection {
        ...
        uint8  status; //0 不可购买及兑换 1 可购买 2 可兑换 3 可购且可兑换
        uint64 startId;
        uint16 stock;  // 库存
        uint16 sales;  // 销量
        uint32 price0; // 兑换价格
        uint120 price1; // RT 价格
        uint32 adjust1;
        uint32 adjust2;
        uint32 adjust3;
        uint32 adjust4;
...
contract Wallet is AccessControl {
   ...
   mapping(bytes32 => uint256) public role_count;
    ...
    function setContracts(IERC20Burnable _rt, IRunbitCollection _rc) external onlyRole(DEFAULT_ADMIN_ROLE) {
        RT = _rt;
        RC = _rc;
    }

    function mint(address to, uint256 amount) external onlyRole(OPERATE_ROLE) {
    ...
    function addShoesCollection(IRunbitCollection.ShoesCollection memory colection) external onlyRole(OPERATE_ROLE) {
        RC.addShoesCollection(colection);
        emit DelegateAddCollection(msg.sender, colection);
    }
