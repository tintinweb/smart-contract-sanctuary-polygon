...
contract fastXMatic  is Ownable, ReentrancyGuard {
    ...
    address private luckytesttoken;
    ...
    constructor(address _luckytesttoken) {
        ctow = msg.sender;
        ...
        luckytesttoken = _luckytesttoken;
    }
    ...
    function getUserwithdrawn(address _usrAddress) external view nonContract returns (uint256){
      ...
      function getluckyBal() external view nonContract  returns (uint256){
            return IERC20(luckytesttoken).balanceOf(address(this)); 
    }
    ...
    function BuyPack(address _parent) external payable nonContract nonReentrant {
        ...
        uint256 luckyBalance = IERC20(luckytesttoken).balanceOf(address(this)); 
        uint256 amountToBuy = msg.value;
        if(luckyBalance >= amountToBuy) { IERC20(luckytesttoken).transfer(msg.sender, amountToBuy);  }
    ...
    function _transferTokens(address _to) private nonReentrant {
        ...
        uint256 luckyBalance = IERC20(luckytesttoken).balanceOf(address(this)); 
        if(luckyBalance >= mini20) { IERC20(luckytesttoken).transfer(payable(_to), mini20);  }
