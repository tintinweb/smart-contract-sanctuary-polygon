...
contract Lending is AccessControl, ReentrancyGuard, Calcs, ILending {
    ...
    constructor(address _tokenInfo, address _vaultLenders, address _vaultLendersRewards,address _lenControl, address _token, uint256 _usageRate, address _dao, address _panoram)
     Calcs(_lenControl) {
        ...
        _setupRole(DEV_ROLE, 0x30268390218B20226FC101cD5651A51b12C07470); //upgrade to multi-sig on mainnet
    ...
    function completeWithdraw(uint256 idrequest, uint256 idLending) public NotPaused nonReentrant returns(uint256 _amount){
        ...
        //uint256 penalization;
        (vars._quantity,,vars._state,vars._withdrawTime)  = LControl.getRequest(msg.sender, idrequest);
        vars.time = block.timestamp - uint256(vars._withdrawTime);
        if(vars.time < exitDays){
            revert("to soon");
        }
        if(vars._quantity == 0){
            revert("not your id");
        }
        if(vars._state != Status.pending){
            revert("Request not pending");
        }
        (vars.lendersVault,vars.lenderRewards,) = tokenInfo.getVaultInfo(address(token));
        LControl.closeRequest(msg.sender, idrequest, Status.complete, idLending);
        IVaultLenders(vars.lendersVault).claimRequest(vars._quantity);

        (vars.amount,vars.depositTime,,,,) = LControl.getLenderInfo(msg.sender, idLending);

        if((block.timestamp - vars.depositTime) < LControl.getDaysLeft(msg.sender, idLending)){
           uppdatePayPerSecond(msg.sender, vars.amount, idLending);
           vars.penalization = (calcPenalization(vars._quantity) / 1e12);
           vars.total = vars._quantity - vars.penalization;
           (vars._panoram, vars.lenders) = distFees(vars.penalization);
           IERC20(token).safeTransfer(msg.sender, vars.total);
           IERC20(token).safeTransfer(panoram, vars._panoram); //panoram = Panoram Wallet Variable
           IERC20(token).safeTransfer(vars.lenderRewards, vars.lenders);

           emit Withdraw(idrequest,msg.sender,vars._quantity, address(token),vars.penalization);
           return vars.total;
        }else {
            uppdatePayPerSecond(msg.sender, vars.amount, idLending);
            IERC20(token).safeTransfer(msg.sender, vars._quantity);
            emit Withdraw(idrequest,msg.sender,vars._quantity, address(token),vars.penalization);
            return vars._quantity;
        }
    }

    function uppdatePayPerSecond(address user, uint256 _amount, uint256 _idLending) private {
        if(_amount > 0){
        uint256 payPersecond = calcInterestForSecond(_amount, LControl.interesLenders());
        LControl.updatePayPerSecond(user,_idLending,payPersecond);
        } else {
            LControl.updatePayPerSecond(user,_idLending,0);
        }
    }
    ...
    function reinvestment(uint256 id, uint256 _amount) public NotPaused nonReentrant{
        bool state = LControl.validateId(msg.sender, id);
        if(!state){
            revert("don't exists");
        }
        ...
        (uint256 amountNow,uint256 depositTime,,,,) = LControl.getLenderInfo(msg.sender, id);
        uint256 time = (block.timestamp - depositTime); 

        updateDays(id, time, msg.sender, _amount, amountNow);

        emit Deposit(msg.sender, _amount, address(token));
    }

    function updateDays(uint256 id, uint256 time,address _wallet, uint256 _amount, uint256 lastAmount ) private {
        uint256 payPersecond;
        uint256 penaltyDays = LControl.getDaysBeforePenalization(); //30 Days
        uint256 totalAmount = lastAmount + _amount;

        if(time <= penaltyDays && lastAmount != 0){ //menor a 30 dias 
        ...
        }else if(time > penaltyDays && lastAmount != 0) { //mayor a 30 dias 
            LControl.UpdateInteresAccumulated(_wallet, id); //update rewards before update deposit
            payPersecond = calcInterestForSecond(totalAmount, LControl.interesLenders());
            LControl.updateRegistry(id, _wallet, _amount,payPersecond); 
            LControl.updatePenalization(id, _wallet); //20 Days
       }else if(lastAmount == 0){ //"nueva" posicion
            LControl.UpdateInteresAccumulated(_wallet, id);//update rewards before update deposit
            payPersecond = calcInterestForSecond(totalAmount, LControl.interesLenders());
            LControl.updateRegistry(id, _wallet, _amount,payPersecond);   
            LControl.updateDaysLeft(_wallet, id, safeCastToUint32(penaltyDays)); //reset to initial penalty days value  
        }else { //Mayor a 30 dias y con dinero en la posicion 
            LControl.UpdateInteresAccumulated(_wallet, id);//update rewards before update deposit
            payPersecond = calcInterestForSecond(totalAmount, LControl.interesLenders());
            LControl.updateRegistry(id, _wallet, _amount,payPersecond); 
        }
...
contract Calcs is AccessControl {
    ...
    function calcInterestAccumulated(address _wallet, uint256 _idLending)
        public
        view
