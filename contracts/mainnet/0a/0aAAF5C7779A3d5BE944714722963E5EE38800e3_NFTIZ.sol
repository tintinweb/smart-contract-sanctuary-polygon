...
contract NFTIZ is Ownable {
    ...
    /* Events */
    event CreateOffer(string indexed indexedOfferId, string offerId, uint256 budget);
    event CloseOffer(string indexed indexedOfferId, string offerId, uint256 percent, uint256 date);

    event AddBudget(string indexed indexedOfferId, string offerId, uint256 amount, uint256 budget);
    event WithdrawBudget(string indexed indexedOfferId, string offerId, uint256 amount, address wallet);

    event AddProfit(string indexed indexedOfferId, string offerId, address wallet, uint256 amount, uint256 fee);
    event WithdrawProfit(string indexed indexedOfferId, string offerId, address wallet, uint256 amount);
    ...
    function createOffer(string memory _offerId, address _token, uint256 _amount, uint256 _vesting, bool _recover) public {
        ...
        emit CreateOffer(_offerId, _offerId, _amount);
    }
    ...
    function addBudget(string memory _offerId, uint256 _amount) public {
        ...
        emit AddBudget(_offerId, _offerId, _amount, offers[_offerId].budget);
    }
    ...
    function _withdrawOfferBudget(string memory _offerId, uint256 _amount, address _wallet) internal {
        ...
        emit WithdrawBudget(_offerId, _offerId, _amount, _wallet);
    }
    ...
    function closeOffer(string memory _offerId, uint256 _percent, uint256 _date, address _wallet) public onlyOwnerOrSigner {
        ...
        emit CloseOffer(_offerId, _offerId, _percent, _date);
    }
    ...
    function addProfit(string memory _offerId, address _wallet, uint256 _amount, uint256 _fee) public onlyOwnerOrSigner {
        ...
        emit AddProfit(_offerId, _offerId, _wallet, _amount, _fee);
    }
    ...
    function _withdrawOfferProfit(string memory _offerId, address _from, address _to, uint256 _amount) internal {
        ...
        emit WithdrawProfit(_offerId, _offerId, _from, _amount);
    }
