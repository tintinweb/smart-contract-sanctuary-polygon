    ...
    function exit(
        ...
        if (inputAmounts[0] == 0) {
            ...
            if(block.number >= user.withdrawBlock + DaiVault.withdrawTimelock()){
                uint currentDai = DaiVault.currentBalanceDai();

                uint256 maxAmount = (user.maxDaiDeposited * DaiVault.maxWithdrawP()) / 100;
                ...
                if(currentDai > 0){
                    uint256 reward = (user.daiDeposited * DaiVault.accDaiPerDai()) /1e18 -  user.debtDai;
