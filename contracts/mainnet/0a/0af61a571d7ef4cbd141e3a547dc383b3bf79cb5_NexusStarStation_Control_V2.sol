...
contract NexusStarStation_Control_V2 is Context, Ownable{
    ...
    uint256 public totalMinted = 0;        // Total number of nfts minted. Used as a tracker for all space station classes and used for calculating station upgrades.
    address public rewardPool;             // Address for reward pool. Set at deployment    
    ...
    address public StarStationAddress;     // Address of the starstation nft contract
    spaceStationInterface spaceStationContract;//interface to Stations NFT contract
    uint256 public burnRate = 15;            // % of purchase cost burnt
    ...
    function updateAPR(uint256 start,uint256 end) public {
        require(start < totalMinted,"Not enough stations");
        if(end < totalMinted){
            end = totalMinted;
        }
        for (uint256 pid = start; pid <end; pid++) {                                                    // Cycle through all Spacestations
            ...
            if(SpaceStation.lastUpdate < totalMinted){                                                  // Check that new Nfts have been minted since stations last update
                if(SpaceStation.APR < stationClass.maxAPR){                                             // Check that station weight is less then max weight
                    ...
                    if(newAPR <= stationClass.maxAPR){
                        SpaceStation.APR = newAPR;                                                      // Update the ships weight.
                    }
                    else{
    ...
    function payReward() external {
        ...
        for (uint256 i = 0; i < totalMinted; i++) {
            SpaceStations memory spaceStation = spaceStations[i];
            StationClass memory stationClass = stationClasses[spaceStation.class];
            address to = spaceStationContract.ownerOf(i);
            uint256 amount = (stationClass.price * (stationClass.startingAPR+(stationClass.upgradeRate*(totalMinted-(i+1)))));
            IERC20 token = IERC20(star);
            token.safeTransferFrom(rewardPool,to, amount);
        }
    ...
    function payBonus(uint256 numDays) external onlyOwner{
        for (uint256 i = 0; i < totalMinted; i++) {
            ...
            StationClass memory stationClass = stationClasses[spaceStation.class];
            address to = spaceStationContract.ownerOf(i);
            uint256 amount = numDays*(stationClass.price * (stationClass.startingAPR+(stationClass.upgradeRate*(totalMinted-(i+1)))));
            IERC20 token = IERC20(star);
            token.safeTransferFrom(rewardPool,to,amount);
        }
    }
