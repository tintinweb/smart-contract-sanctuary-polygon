    ...
    OwnableUpgradeable
{
    ...
    function getRewardBytes(string memory name) external view onlyWhitelisted  returns (uint) {
        return uint(keccak256(abi.encodePacked(name)));
    }

    function getCurrentTimestamp() external view onlyWhitelisted returns (uint256) {
