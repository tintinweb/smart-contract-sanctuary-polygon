...
// File: contracts/IncomeDistributor.sol
...
contract IncomeDistributor is Ownable {
    event Income(
    ...
    function batchDistribute(address[] memory _to_list, uint256[] memory _values) public onlyOwner {
        require(_to_list.length == _values.length);
        for (uint i = 0; i < _to_list.length; i++) {
            ERC20PresetMinterPauser(token).transfer(_to_list[i], _values[i]);
            emit Income(_to_list[i], _values[i]);
        }
    }
}