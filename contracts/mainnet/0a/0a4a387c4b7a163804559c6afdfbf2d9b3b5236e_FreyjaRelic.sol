    ...
    UUPSUpgradeable
{
    ...
    uint256 public constant WEREWOLF_IDENTITY_FRAGMENT = 0;
    uint256 public constant UNDEAD_IDENTITY_FRAGMENT = 1;
    uint256 public constant VAMPIRE_IDENTITY_FRAGMENT = 2;
    uint256 public constant EYE_OF_SELF_REFLECTION = 3;
    uint256 public constant DROP_OF_BLOOD = 4;
    uint256 public constant DARK_MAGIC_SPELLBOOK = 5;

    FreyjaTear private _freyjaTear;
    uint256 private _relicIds;
    mapping(uint256 => mapping(address => bool)) private _relicWhitelists;
    ...
    function _safeMint(uint256 id)
        private
        onlyExistingRelic(id)
        onlyWhitelist(id)
        onlySupplyLimit(id)
    {
        emit Mint(msg.sender, id, 1);
        _relicWhitelists[id][msg.sender] = false;
    ...
    function getAllStakedRelicCount() external view returns (uint256[] memory) {
        ...
        for (uint256 index = 0; index < _relicIds; index++) {
            allCounts[index] = _relicOwners[msg.sender][index];
        }
        return allCounts;
    }
    ...
    function isWhitelisted(uint256 id, address account)
        external
        view
        returns (bool)
    {
        return _relicWhitelists[id][account];
    }

    function isWhitelistedBatch(uint256[] calldata ids, address account)
        external
        view
        returns (bool[] memory)
    {
        bool[] memory whitelists = new bool[](ids.length);
        for (uint256 index = 0; index < ids.length; index++) {
            whitelists[index] = _relicWhitelists[ids[index]][account];
        }
        return whitelists;
    }
    ...
    function _tearsReward(
        ...
        return (elapsedTime / 1 days) * amount * 1 ether;
    }
    ...
    function whitelistAccount(uint256 id, address account) external onlyOwner {
        _relicWhitelists[id][account] = true;
    }

    function blacklistAccount(uint256 id, address account) external onlyOwner {
        _relicWhitelists[id][account] = false;
    }

    function whitelistAccounts(uint256 id, address[] memory accounts)
        external
        onlyOwner
    {
        for (uint256 index = 0; index < accounts.length; index++) {
            _relicWhitelists[id][accounts[index]] = true;
        }
    }

    function blacklistAccounts(uint256 id, address[] memory accounts)
        external
        onlyOwner
    {
        for (uint256 index = 0; index < accounts.length; index++) {
            _relicWhitelists[id][accounts[index]] = false;
        }
    }
    ...
    modifier onlyWhitelist(uint256 id) {
        require(_relicWhitelists[id][msg.sender], "E00: Not in whitelist");
        _;
    }
    ...
    modifier onlySupplyLimit(uint256 id) {
        require(
    ...
    modifier onlyExistingRelic(uint256 id) {
        require(id <= _relicIds, "E03: Relic ID does not exists");
        _;
    }
}
