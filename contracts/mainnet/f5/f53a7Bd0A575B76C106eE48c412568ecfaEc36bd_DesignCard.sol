...
contract DesignCard {
  ...
  function buildMetadata(uint256 _tkId, bool _noENS, string memory _ethName, string memory _ethNickName, string memory _Edition,
    uint _NumCardCollected, uint _totalSupply) public pure returns (string memory) {
      string memory tokenID = uint256(_tkId).toString();
      string memory NumCardCollected = uint256(_NumCardCollected).toString();
      string memory totalSupply=uint256(_totalSupply).toString();
      ...
      if (_noENS==true){
        ...
        if (bytes(_ethName).length<=5){
          WordSize='font-size="35"';
        } else if (bytes(_ethName).length<=8){
          WordSize='font-size="32"';
        } else if (bytes(_ethName).length<15){
          WordSize='font-size="29"';
        } else {
          WordSize='font-size="25"';
        }
        ethName=toUpper(_ethName);
        ethNameInName=string(abi.encodePacked(toUpper(_ethName)," ",nickName));
      }
      ...
      return string(abi.encodePacked(
              'data:application/json;base64,', Base64.encode(bytes(abi.encodePacked(
                          '{"name":"',
                          string(abi.encodePacked('Bcard ID ', tokenID,' ', ethNameInName)),
                          '","description":"', 
                          "You are you",
                          '", "image": "', 
                          'data:image/svg+xml;base64,', 
                          buildImage(ethName,nickName,WordSize),
                          '","attributes":[{ "trait_type": "Series","value": "',_Edition,'"},{ "display_type": "number", "max_value":"',totalSupply,'", "trait_type": "Number of Bcards Collected","value":"',NumCardCollected,'"}]}')))));
  }
