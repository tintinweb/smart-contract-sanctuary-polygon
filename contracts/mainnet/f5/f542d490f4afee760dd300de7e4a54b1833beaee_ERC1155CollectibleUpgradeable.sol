    ...
    ERC1155CollectibleHelper
{
     ...
     *
     * bronze: 340282366920938463463374607431768211456
    ...
    // max tier info
    // collectibleId => maxTier
    mapping(uint256 => uint8) public maxTierInfo;
    ...
    function initialize(ICommonGameData _commonGameStorage) public initializer {
        ...
        baseUri = "https://prod-api.isekaiverse.io/api/metadata/erc1155Collectibles?token_id=";
    ...
    /**
     * @dev returns the max tier limit of a character
     * @param _id can be token ID, or just the collectible ID of the character you want the max tier value for
     */
    function getMaxTier(uint256 _id) public view returns (uint256) {
        return (maxTierInfo[_id & collectibleIdMask]);
    }
    ...
    function totalCollectibleSupply(uint256 _id) public view returns (uint256) {
        ...
        require(_exists(_id), "collectible ID does not exist");
        return tokenSupply[_id];
    }
     ...
     * @param _maxTier max number of tiers for this character
    ...
    function createNFT(
        ...
        // check parameters
        require(
            _season != 0 && _maxTier != 0 && _type != 0,
            "incorrect values"
        );

        for (uint8 _tier = 1; _tier <= _maxTier; _tier++) {
            ...
            // the character of each tier will have the same maxTier
            maxTierInfo[collectibleId] = _maxTier;
            ...
            emit ERC1155NFTCreated(
                ...
                string(
                    abi.encodePacked(
                        baseUri,
                        StringsUpgradeable.toString(collectibleId)
                    )
                ),
                msg.sender
            );
    ...
    function createShard(
        ...
        bytes memory _data
    ) public onlyRole(CREATOR_ROLE) whenNotPaused returns (uint256) {
        uint256 collectibleId = create(
            0,
            0,
            0,
    ...
    function create(
        ...
        // Sukumi, Prince Ao or Aloysia in case of Champions and swords, hammers in items or shard type in case of shards (bronze, silver, gold, platinum)
     ...
     * @param _maxTiers max tiers of the characters being created for a season
        ...
        returns (uint256[] memory, uint256)
    {
        ...
        for (
            ...
            uint256 id = collectibleId +
                tokenSupply[collectibleId] +
                collectibleIndex;
            _mint(
                account,
    ...
    function burn(
        ...
        uint256 id,
        uint256 value
    ) public override(ERC1155BurnableUpgradeable) whenNotPaused {
        require(
