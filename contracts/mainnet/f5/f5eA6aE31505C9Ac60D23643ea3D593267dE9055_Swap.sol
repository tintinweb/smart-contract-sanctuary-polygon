...
contract Swap {
    ...
    function ballance() public view OnlyOwner returns(uint, uint){   
        return(usdc_token.balanceOf(address(this)), link_token.balanceOf(address(this)));
     
    }
    function swapUniV3(uint256 amount) public OnlyOwner returns (uint256 amountOut) {
    ...
    function SwapSushi() public OnlyOwner returns(uint[] memory){
    ...
    function callSwap(uint amount) public OnlyOwner{
        swapUniV3(amount);
