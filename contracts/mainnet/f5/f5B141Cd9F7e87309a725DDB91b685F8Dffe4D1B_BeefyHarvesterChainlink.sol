...
import {KeeperCompatibleInterface} from "./interfaces/external/KeeperCompatibleInterface.sol";

import {IKeeperRegistry} from "./interfaces/external/IKeeperRegistry.sol";

import {BeefyHarvesterBase, IBeefyRegistry, IUpkeepRefunder} from "./BeefyHarvesterBase.sol";

contract BeefyHarvesterChainlink is BeefyHarvesterBase, KeeperCompatibleInterface {
        ...
        )
    {
        ...
        if (!isAnyVaultHarvestable) {
            performData_ = bytes("BeefyAutoHarvester: No vaults to harvest");
        } else {
            upkeepNeeded_ = true;
            performData_ = abi.encode(vault, newStartIndex, estimatedTxCost, estimatedCallRewards, tx.gasprice);
        }
    }
    ...
    function performUpkeep(bytes calldata performData_) external override {
        ...
        ) = abi.decode(performData_, (address, uint256, uint256, uint256, uint256));

        _performUpkeep(vault, newStartIndex, estimatedTxCost, estimatedCallRewards, checkUpkeepGasprice);
    }
}
...
import {IBeefyRegistry} from "./interfaces/IBeefyRegistry.sol";
import {IUpkeepRefunder} from "./interfaces/IUpkeepRefunder.sol";

import {UpkeepLibrary} from "./libraries/UpkeepLibrary.sol";

abstract contract BeefyHarvesterBase is ManageableUpgradeable, PausableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    uint256 public constant KEEPER_REGISTRY_GAS_OVERHEAD = 80_000; // Gas cost of upstream contract that calls performUpkeep(). This is a private variable on KeeperRegistry.
    uint256 public constant GELATO_REGISTRY_GAS_OVERHEAD = 10_000; // Probably not needed, but serves as extra buffer.
    uint256 public constant HARVEST_GAS_PROFITABILITY_BUFFER = 10_000;

    // Contracts.
    IBeefyRegistry public vaultRegistry;
    IKeeperRegistry public keeperRegistry; // only used in chainlink child contract, must keep same storage layout for proxy upgrade.
    IUpkeepRefunder public upkeepRefunder;

    uint256 public chainlinkUpkeepTxPremiumFactor; // deprecated, must keep same storage layout for proxy upgrade.

    // State variables that will change across upkeeps.
    uint256 public startIndex;

    uint256 public upkeepTxPremiumFactor; // Tx premium factor/multiplier scaled by 1 gwei (10**9).

    // Whther using chainlink keepers or gelato.
    bool public isGelato;
    bool public useUpkeepRefunder;
    ...
    {
        // Get vault addresses to iterate over.
        address[] memory vaults = vaultRegistry.allVaultAddresses();

        (isAnyVaultHarvestable_, vault_, newStartIndex_, estimatedTxCost_, estimatedCallRewards_) = _findVaultToHarvest(
            vaults
        );
    }

    function _findVaultToHarvest(address[] memory vaults_)
        internal
        view
        returns (
            bool isAnyVaultHarvestable_,
        ...
        )
    {
        ...
        isAnyVaultHarvestable_ = false;
        vault_ = address(0);
        newStartIndex_ = startIndex;
        estimatedTxCost_ = 0;
        estimatedCallRewards_ = 0;
        ...
        for (uint256 offset; offset < vaults_.length; ++offset) {
            ...
            vaultIndexToCheck = UpkeepLibrary._getCircularIndex(startIndex + offset, vaults_.length);
        ...
        )
    {
        ...
        uint256 rawVaultHarvestOverhead = vaultRegistry.harvestFunctionGasOverhead(vaultAddress_);
        uint256 vaultHarvestGasOverhead = _estimateSingleVaultHarvestGasOverhead(rawVaultHarvestOverhead);
        ...
        bool isProfitableHarvest = callRewardAmount_ >= harvestThreshold;
    ...
    function _performUpkeep(
        ...
        // TODO: validate that vault is in registry.
        ...
        uint256 oldStartIndex = startIndex;
        _updateStartIndex(newStartIndex_);
        ...
        if (useUpkeepRefunder) {
            upkeepRefunder.notifyRefundUpkeep();
        }
    }

    function _reportHarvestSummary(
        uint256 oldStartIndex_,
        uint256 newStartIndex_,
        uint256 checkUpkeepGasPrice_,
        ...
        // Calculate profit.
        uint256 estimatedProfit = UpkeepLibrary._calculateProfit(estimatedCallRewards_, estimatedTxCost_);
    ...
    function _harvestVault(address vault_) internal returns (bool didHarvest_, uint256 callRewards_) {
        ...
        address callFeeRecipient = address(upkeepRefunder) != address(0) ? address(upkeepRefunder) : address(this);
    ...
    function _updateStartIndex(uint256 newStartIndex_) internal {
        // ensure newStartIndex_ is valid and set startIndex
    ...
    function setUpkeepRefunder(address upkeepRefunder_) external onlyManager {
        upkeepRefunder = IUpkeepRefunder(upkeepRefunder_);
    }

    function setStartIndex(uint256 startIndex_) external onlyManager {
        address[] memory vaults = vaultRegistry.allVaultAddresses();
        startIndex = UpkeepLibrary._getCircularIndex(startIndex_, vaults.length);
    }
    ...
    function _calculateTxCostWithPremium(uint256 gasOverhead_) internal view returns (uint256 txCost_) {
        return UpkeepLibrary._calculateUpkeepTxCost(tx.gasprice, gasOverhead_, upkeepTxPremiumFactor);
    }

    function _calculateTxCostWithOverheadWithPremium(uint256 totalVaultHarvestOverhead_)
        internal
        view
        returns (uint256 txCost_)
    {
        return
            UpkeepLibrary._calculateUpkeepTxCostFromTotalVaultHarvestOverhead(
                ...
                _getUpstreamGasOverhead(),
                upkeepTxPremiumFactor
            );
    }

    function _getUpstreamGasOverhead() internal view returns (uint256 gasOverhead_) {
        gasOverhead_ = isGelato ? GELATO_REGISTRY_GAS_OVERHEAD : KEEPER_REGISTRY_GAS_OVERHEAD;
    }
    ...
    {
        totalGasOverhead_ = vaultHarvestFunctionGasOverhead_ + _getUpstreamGasOverhead();
    }
...
interface IBeefyRegistry {
    function allVaultAddresses() external view returns (address[] memory vaultAddresses_);

    function getVaultCount() external view returns (uint256 count_);

    function setHarvestFunctionGasOverhead(IBeefyVault vault_, uint256 gasOverhead_) external;

    function harvestFunctionGasOverhead(address vault_) external view returns (uint256 gasOverhead_);
}
...
library UpkeepLibrary {
    ...
    function _calculateUpkeepTxCost(
        ...
        upkeepTxCost_ = (gasprice_ * gasOverhead_ * upkeepTxPremiumFactor_) / UPKEEPTX_PREMIUM_SCALING_FACTOR;
    }
