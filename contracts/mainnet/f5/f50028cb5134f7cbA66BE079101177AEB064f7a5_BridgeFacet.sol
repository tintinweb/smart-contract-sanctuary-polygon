...
contract BridgeFacet {
    bytes32 internal constant NAMESPACE =
        keccak256("com.cicleo.facets.bridge");
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(user, subscriptionManagerId, subscriptionId, user, price, nonce));
    }
    ...
    function verify(
        ...
        bytes32 messageHash = getMessageHash(subscriptionManagerId, subscriptionId, user, price, nonce);
    ...
    function bridgeSubscription(
        ...
        require(verify(subscriptionManagerId, subscriptionId, user, price, s.userNonce[user], signature), "Invalid signature");
        ...
        LibPayment.redistributeToken(price, manager, subscriptionManagerId, user);
...
library LibAdmin {
    ...
    function isContractOwner(address user, uint256 subscriptionManagerId) internal view returns (bool isOwner) {
        isOwner = diamondStorage().factory.verifyIfOwner(user, subscriptionManagerId);
    }

    function enforceIsOwnerOfSubManager(uint256 subscriptionManagerId) internal view {
        require(isContractOwner(msg.sender, subscriptionManagerId), "LibAdmin: Must hold ownerpass for this submanager");
    }
...
library LibPayment {
    bytes32 constant DIAMOND_STORAGE_POSITION = keccak256("com.cicleo.facets.payment");
    ...
    function referralPercent(uint256 id) view internal returns (uint16) {
    ...
    function redistributeToken(
        ...
        if (
            ...
            uint256 referral = (toOwner * diamondStorage().referralPercent[id]) / 1000;
            toOwner -= referral;
