...
contract OrdinalMarketplace is IOrdinalMarketplace, Ownable, ReentrancyGuard, Pausable {
    ...
    function listOrdinal(
        ...
        if (isSignRequired) {
            if (_scriptType == ScriptTypes.P2TR) {
                require(
                    _verifySchnorr(_convertToBytes32(scriptHash), txId, _signature),
                ...
                require(
                    _bytesToAddress(_sliceBytes(abi.encodePacked(keccak256(_bitcoinPubKey)), 12, 31)) == 
                        ecrecover(txId, _signature.v, _signature.r, _signature.s),
                    "OrdinalMarketplace: not ordinal owner"
                );
            }
        }
    ...
    function sellOrdinal(
        ...
        _checkOrdinalTransferAndSendFunds(
            _txId, 
            _seller, 
            _bidIdx, 
            _transferTx.vout, 
    ...
    function _checkOrdinalTransferAndSendFunds(
        ...
        if (_ordinalOutputIdx != 0) {
            require(
                ...
                "OrdinalMarketplace: not transferred"
            );
        }
