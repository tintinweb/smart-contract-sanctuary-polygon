...
contract GovStakingV2 is Ownable, ReentrancyGuard, Pausable {
    uint256 public opt1;
    uint256 public opt2; // 1 week
    uint256 public opt3; // 1 month
    uint256 public opt4; // 6 month
    uint256 public opt5; // 1 year
    uint256 public opt6; // 3 years
    ...
    function init(
        ...
        require(!isInit, "already initialzied");
        isInit = true;
        gogo = IERC20(gogoAddress);
        ...
        opt1 = 0;
        opt2 = 1 weeks; // 1 week
        opt3 = 2629746; // 1 month
        opt4 = 15778476; // 6 month
        opt5 = 31556952; // 1 year
        opt6 = 94608000; // 3 years
        ...
        return
            extendingPeriod > 0
                ? calcExtendRate( // extend lock period (0 amount is possible)
                    ...
                    oldRate,
                    getRewardRateBasedOnOldRate(oldRate, extendingPeriod)
                )
    ...
    function getRewardRateBasedOnOldRate(
        uint256 oldRate,
        uint256 extendingPeriod
    ) internal view returns (uint256) {
        // derive the corresponding period
        uint256 periodMin;
        uint256 periodMax;

        if (oldRate >= rewardRates[opt1] && oldRate <= rewardRates[opt2]) {
            periodMin = opt1;
            periodMax = opt2;
        }
        if (oldRate >= rewardRates[opt2] && oldRate <= rewardRates[opt3]) {
            periodMin = opt2;
            periodMax = opt3;
        }
        if (oldRate >= rewardRates[opt3] && oldRate <= rewardRates[opt4]) {
            periodMin = opt3;
            periodMax = opt4;
        }
        if (oldRate >= rewardRates[opt4] && oldRate <= rewardRates[opt5]) {
            periodMin = opt4;
            periodMax = opt5;
        }
        if (oldRate >= rewardRates[opt5] && oldRate <= rewardRates[opt6]) {
            periodMin = opt5;
            periodMax = opt6;
        }

        uint256 oldPeriod = (((oldRate - rewardRates[periodMin]) *
            (periodMax - periodMin)) /
            (rewardRates[periodMax] - rewardRates[periodMin])) + periodMin;

        // get the new period for rate calculation
        uint256 newPeriod = oldPeriod + extendingPeriod;

        if (newPeriod > opt6) return rewardRates[opt6]; // return maximum rate

        uint256 max;

        if (newPeriod >= opt1 && newPeriod <= opt2) {
            max = opt2;
        }
        if (newPeriod >= opt2 && newPeriod <= opt3) {
            max = opt3;
        }
        if (newPeriod >= opt3 && newPeriod <= opt4) {
            max = opt4;
        }
        if (newPeriod >= opt4 && newPeriod <= opt5) {
            max = opt5;
        }
        if (newPeriod >= opt5 && newPeriod <= opt6) {
            max = opt6;
        }

        return
            (((rewardRates[max] - oldRate) * (newPeriod - oldPeriod)) /
                (max - oldPeriod)) + oldRate;
    }

    function earned(address _user) external view returns (uint256) {
        GovStakingStorage.UserInfo memory user = GovStakingStorage(
            rewards.getStoreAddress()
        ).getUserInformation(_user);
        uint256 qualifiedUntil = (
            user.lockStart + (user.lockPeriod) < block.timestamp
                ? user.lockStart + user.lockPeriod
                : block.timestamp
    ...
    function setGovGogo(address _newGovGogo) external onlyOwner {
        govGogo = govERC20(_newGovGogo);
    }
