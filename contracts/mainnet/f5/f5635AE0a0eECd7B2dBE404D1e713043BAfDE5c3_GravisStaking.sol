...
contract GravisStaking is Ownable {
    ...
    IMintableToken public shipBuildingParts;

    uint256 public shipBuildingPartsPerGrvxPerSecond;
    ...
    constructor(
        IERC20 grvx_,
        IMintableToken shipBuildingParts_,
        uint256 rewardStart_,
        uint256 shipBuildingPartsPerGrvxPerYear_,
        uint256 lockPeriod_
    ) {
        if (rewardStart_ != 0) {
        ...
        grvx = grvx_;
        shipBuildingParts = shipBuildingParts_;
        lastRewardDistribution = rewardStart_;
        shipBuildingPartsPerGrvxPerSecond = shipBuildingPartsPerGrvxPerYear_ / (365 days);
    ...
    function claimReward() external {
        ...
        shipBuildingParts.mint(msg.sender, reward);
    }

    // RESTRICTED FUNCTIONS

    function setShipBuildingPartsPerGrvxPerYear(uint256 shipBuildingPartsPerGrvxPerYear_, uint256 lockPeriod_)
        external
        onlyOwner
    {
        _distributeRewards();
    ...
    function rewardOf(address account) public view returns (uint256) {
        ...
        if (block.timestamp > lastRewardDistribution && totalStakes > 0) {
            currentRewardPerShare +=
                (shipBuildingPartsPerGrvxPerSecond *
