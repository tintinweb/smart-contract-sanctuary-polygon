...
contract Hosterizing_Test is ERC721URIStorage {
    ...
    constructor(
        string memory _tld, EIP20 _payment_token) 
        payable ERC721("Hosterizing_Test Name Service", "HTZT") 
        {
        owner = payable(msg.sender);
        tld = _tld;
        console.log("%s name service for Hosterizing_Test Domains deployed", _tld);
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        require(len > 0);
        if (len == 3) {
            return 50 * 1; // 5 MATIC = 5 000 000 000 000 000 000 (18 decimals). We're going with 0.5 Matic because the faucets don't give a lot
        } else if (len == 4) {
            return 30 * 1; // To charge smaller amounts, reduce the decimals. This is 0.3
        } else {
            return 10 * 1;
        }
    }
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        _name,
                        '", "description": "A domain on the HTZ name service", "image": "data:image/svg+xml;base64,',
