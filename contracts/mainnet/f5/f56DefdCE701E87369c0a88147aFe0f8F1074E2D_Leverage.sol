...
contract Leverage is Ownable {
    ...
    function leverage(
        ...
        if (_isETH) {
            ...
            _asset = ADDR_weth;
            _amount = msg.value;
            IWETH(_asset).deposit{value: _amount}();
        } else {
            currentLeverageAsset = _asset;
    ...
    function deleverage(
        ...
        if (
            ...
            _asset = payable(currentLeverageAsset);
        }
        uint256 aTokenBalance = getAtokenBalanceForReserve(_asset);
        require(aTokenBalance > 0, "Leveraged balance is zero.");
        uint256 withdrawn = _withdrawMaxFromPool(_asset);
        if (_isETH) {
            require(_asset == ADDR_weth, "Invaild asset.");
            _amount = msg.value;
            IWETH(_asset).deposit{value: _amount}();
        } else {
            IERC20Detailed(_asset).transferFrom(
        ...
        while (getAtokenBalanceForReserve(_asset) > 0) {
            _amount = _deleverage(_asset, _amount, _paramsForDeleverage);
        }
