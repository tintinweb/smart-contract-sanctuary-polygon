...
contract PresaleRidersFactory is VRFConsumerBaseV2, Ownable  
{
  ...
  // Events
  event MysteryBoxOpened(address indexed user, uint indexed mysteryBoxId, uint topMysteryBoxType);
  event NftRevealed(address indexed user, uint indexed mysteryBoxId, ICollectionRiders.RidersAttributes[] riderAttributes, uint topMysteryBoxType);
  event Minted(address indexed user, ICollectionRiders.RidersAttributes[] riderAttributes);
  ...
  function openMysteryBox(uint _mysteryBoxId) external payable {
    ...
    uint32 numWords =  uint32(_quantity) * 3;
    ...
    internal override 
  {
    ...
    emit MysteryBoxOpened(_user, mysteryBoxId, presaleInstance.getMysteryBoxIdToTopTypeRider(_user,mysteryBoxId));
  }
  ...
  function revealNFT(uint _mysteryBoxId) external{
    ...
    ICollectionRiders.RidersAttributes[] memory eventAttributes = new ICollectionRiders.RidersAttributes[](_randomWordsLength/4);
    for(uint i = 0; i < _randomWordsLength; i += 3) {
      ...
      (string memory currentBaseURI, string memory currentImageURI) = presaleInstance.getRidersURIs(attributes.pizzaQuantity);
      // Gets the final IPFs URI of the image and the motorbike of the rider using the third random
      (string memory _tokenURI, string memory _imageURI, bytes12 motorBike) = _getTokenURIandBike(
      ...
      eventAttributes[((_randomWordsLength - i)/4) - 1] = _presaleMint(msg.sender, _randomWords[i + 2], motorBike, _tokenURI, _imageURI, attributes);
    }
    emit NftRevealed(msg.sender, _mysteryBoxId, eventAttributes, presaleInstance.getMysteryBoxIdToTopTypeRider(msg.sender, _mysteryBoxId));
  ...
  function _getTokenURIandBike(bytes12[] memory _motorBikes, string memory _currentBaseURI, string memory _currentImageURI, uint _tokenURILength,  uint _randNumber3)
    internal
    pure
    returns (string memory, string memory, bytes12)
  {
    ...
    return (
        bytes(_currentBaseURI).length > 0
          ...
          : "",
        bytes(_currentImageURI).length > 0
          ? string(abi.encodePacked(_currentImageURI, "/", randURIString, ".png"))
          : "",        
        motorBike
    );
  }
  ...
  function _presaleMint(address user, uint _randonNumber4, bytes12 _motorBike, string memory _tokenURI, string memory _imageURI, IPresale.RiderAttributes memory attributes) internal returns(ICollectionRiders.RidersAttributes memory riderAttributes) {
    ...
    riderAttributes = ICollectionRiders.RidersAttributes(
      0,
    ...
    riderAttributes.riderId = ridersInstance.mint(user, riderAttributes);
  }
  ...
  /// @param _boxType { RiderLEG = 0, RiderMYT = 1, RiderSP = 2 } 
  /// @param _isPromotionalA defines if its type A or TypeB riders
  /// @param _receiver address of user who will receive the riders to be minted
  function promotionalMint(uint _mintQuantity, uint _boxType, bool _isPromotionalA, address _receiver) external onlyOwner { 
    require(_boxType < 3, "Incorrect boxType");
    ICollectionRiders.RidersAttributes[] memory eventAttributes = new ICollectionRiders.RidersAttributes[](_mintQuantity);
    for(uint i = 0; i < _mintQuantity; i++) {
      uint16 rangeResult = uint16(_getProbabilityRange(uint16((uint(keccak256(abi.encodePacked(i,block.timestamp, 'First Random'))) % 1000) + 1), presaleInstance.getTypeProbability(_boxType)));
    IPresale.RiderAttributes memory attributes  = presaleInstance.getRiderAttributes(_boxType,rangeResult);
    // Get the IPFs folder for the NFT being minted using the second random
    (string memory currentBaseURI, string memory currentImageURI) = presaleInstance.getRidersURIs(attributes.pizzaQuantity);
    // Gets the final IPFs URI of the image and the motorbike of the rider using the third random
    (string memory _tokenURI, string memory _imageURI, bytes12 _motorBike) = _getTokenURIandBike(
        presaleInstance.getMotorbikes(),
        currentBaseURI, 
        currentImageURI,
        presaleInstance.getBaseURILength(currentBaseURI),
    ...
    // Information is ready and sent to mint the NFT in the CollectionRiders contract
    eventAttributes[i].riderId = ridersInstance.mint(_receiver, riderAttributes);
    eventAttributes[i] = riderAttributes;
    }
    emit Minted(_receiver, eventAttributes);
  }
