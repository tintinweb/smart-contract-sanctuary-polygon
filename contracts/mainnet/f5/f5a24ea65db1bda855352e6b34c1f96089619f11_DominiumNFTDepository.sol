...
contract DominiumNFTDepository is Ownable, IERC721Receiver {
  ...
  mapping(address => uint256[]) public deposits; // Each address mapped to all the deposited token IDs
  ...
  function deposit(uint256 tokenID) public {
    ...
    IERC20(initialFeeCoin).safeTransferFrom(msg.sender, treasury, getStakingCost(tokenID));
  ...
  function payFee(uint256 tokenID) public {
