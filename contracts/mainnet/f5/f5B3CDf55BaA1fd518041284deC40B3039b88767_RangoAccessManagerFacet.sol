...
library LibSwapper {
    ...
    /// @notice Fee received for execution on destination chain
    /// @param token The address of received token, ZERO address for native
    /// @param wallet The address of receiver wallet
    /// @param amount The amount received as fee
    event DestinationFee(address token, address wallet, uint amount);
    ...
    /// @param swapRequest The SwapRequest received by the contract
    /// @param outputAmount The output amount of the swap, measured by the balance change before and after the swap
    event RangoSwap(SwapRequest swapRequest, uint256 outputAmount);
    ...
    /// @param destinationFee The amount of fee required for relayer execution on the destination
    ...
    function onChainSwapsPreBridge(
        ...
        uint minimumRequiredValue = (isNative ? request.feeIn + request.affiliateIn + request.amountIn + request.destinationFee : 0) + extraFee;
    ...
    function onChainSwapsInternal(
        ...
        emit RangoSwap(request, secondaryBalance);
    ...
    function callSwapsAndFees(SwapRequest memory request, Call[] calldata calls) private returns (bytes[] memory) {
        ...
        // Get Platform fee
        if (request.feeIn > 0 || request.destinationFee >0) {
            require(baseSwapperStorage.feeContractAddress != ETH, "Fee contract address not set");
            _sendToken(request.fromToken, request.feeIn + request.destinationFee, baseSwapperStorage.feeContractAddress, isSourceNative, false);
            emit FeeReward(request.fromToken, baseSwapperStorage.feeContractAddress, request.feeIn);
            emit DestinationFee(request.fromToken, baseSwapperStorage.feeContractAddress, request.feeIn);
        }
