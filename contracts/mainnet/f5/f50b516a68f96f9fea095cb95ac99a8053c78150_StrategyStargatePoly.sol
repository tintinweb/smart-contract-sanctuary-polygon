    ...
    constructor(
        ...
        for (uint256 i; i < routeLength; i++) {
            outputToNativeParams.path.push(ITridentRouter.Path(
                _routes.outputToNativePool[i], abi.encode(outputToNativeRoute[i], address(this), true)
            ));
        }
        ...
        for (uint256 i; i < routeLength; i++) {
            outputToInputParams.path.push(ITridentRouter.Path(
                _routes.outputToInputPool[i], abi.encode(outputToInputRoute[i], address(this), true)
            ));
        }
