...
import "./token/onft/extension/OkayBearsNFT721.sol";

contract OkayBearsNFT is OkayBearsNFT721 {
    constructor(address _layerZeroEndpoint, uint _startMintId, uint _endMintId, string memory _notRevealedUri) OkayBearsNFT721("OkayBearsNFT", "OkayBearsNFT", _layerZeroEndpoint, _startMintId, _endMintId, _notRevealedUri) {}
}
...
contract OkayBearsNFT721 is ONFT721 {
    uint public nextMintId;
    uint public maxMintId;

    uint public tokenPrice = 0.75 ether;
    uint public cost = 0.75 ether;
    ...
    function mint(uint8 numTokens) external payable {
        require(saleIsActive, "OkayBearsNFT : Sale must be active to mint token");
        require(numTokens < 100, "OkayBearsNFT : Max 30 NFTs per transaction");
        require(nextMintId + (numTokens - 1) <= maxMintId, "OkayBearsNFT : Max Mint limit reached");
