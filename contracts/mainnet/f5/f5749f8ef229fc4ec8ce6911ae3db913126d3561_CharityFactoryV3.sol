...
import "./interfaces/ICharityFactoryV3.sol";
import "./Organization.sol";

contract CharityFactoryV3 is Initializable, ContextUpgradeable, ICharityFactoryV3, OwnableUpgradeable {
    ...
    uint256 public totalOrganizationsToken;
        ...
        validateAmount(_amount)
    {
        ...
        totalOrganizationsToken += _amount;
    ...
    function alocateFreeTokens() external {
        uint256 currentbalance = charityToken.balanceOf(address(this));
        uint256 balanceDelta = currentbalance.sub(totalOrganizationsToken);

        if (balanceDelta > 0) {
            _updateTokenBalances(balanceDelta);
            totalOrganizationsToken += balanceDelta;
        }
    }
...
interface ICharityFactoryV3 {
    ...
    function totalOrganizationsToken() external view returns (uint256);
