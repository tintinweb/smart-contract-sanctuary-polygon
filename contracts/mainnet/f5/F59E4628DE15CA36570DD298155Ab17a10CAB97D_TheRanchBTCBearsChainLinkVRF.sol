...
import "Ownable.sol";
import "IERC20.sol";
import "TheRanchBTCBearsCommunity.sol";



contract TheRanchBTCBearsChainLinkVRF is VRFConsumerBaseV2, Ownable {


    address public TheRanchBTCBearsProxy;
    ...
    modifier TRBC_contract_only {
        require(msg.sender ==  TheRanchBTCBearsProxy, "Not authorized to call");
        _;
    }
    ...
    */
    function fulfillRandomWords(
        ...
        TheRanchBTCBearsCommunity TRBC = TheRanchBTCBearsCommunity(TheRanchBTCBearsProxy);
        ...
        for( uint256 i; i < randomWords.length ; i++ ) {
            ...
            if (_winnerIndex <= _GiveawayPlayersCount) {
                ...
                // get address of this winner from the proxy contract
                address _winnersAddress = TRBC.ownerOf(_winnerIndex);

                // don't let the same address win twice
                if(addressAlreadyWonGiveaway[_winnersAddress] == false ){
                    winningAddresses.push(_winnersAddress);
                    winningIndexes.push(_winnerIndex);
                    _winnersFoundCount += 1;
                    addressAlreadyWonGiveaway[_winnersAddress] = true;
                }
        ...
        TRBC.mintingGiveaway(winningAddresses, winningIndexes, amoeWinningAddresses, amoeWinningIndexes, _winningAmount );
    ...
    */
    function setTheRanchBTCBearsProxy(address _address) public onlyOwner {
        if (address(_address ) == address(0)) { revert Address_CantBeAddressZero();}
        TheRanchBTCBearsProxy = _address;
    }
...
import "TheRanchBTCBearsChainLinkVRF.sol";


error Minting_ExceedsTotalBears();
...
contract TheRanchBTCBearsCommunity is 
    Initializable,
    ...
    uint256 public USDCReserveBalance;    // amount held within contract for referrals and Giveaway balance 
    ...
    uint256 public monthlyHostingFee;   
    ...
    */
    ...
    // Stockyard allows the rewardBears function to be more modular. 
    ...
    // BTC Bear Owners information 
    struct BTCBearOwner {
        uint256 USDC_Balance;
        uint256 WBTC_Balance;
        uint256 hostingFeeBalance;     // totaly hosting fees due from BTC Bear owner address  
        uint256 hostingFeeReserve;     // amount of money a user has deposited into account to pay auto-pay for hosting fees.
        uint256 hostingClock;          // how many months, pay periods, are they behind on the maintenance fees? 0 means all paid up, 3 gets liquidated. 
    ...
    mapping(address => BTCBearOwner) public  btcBearOwners;


    // Monthly WBTC rewarding variables 
    uint256 public currentRewardingDate;        // This date is set when we send WBTC into the contract to reward the BTC Bears to confirm who has been paid out. 
    ...
    bool public readyToReward;   // bool to confirm we have met all the requirements and are good to go to call the rewardBears function 
    ...
    */
    ...
    event NewBearsEnteringRanch(
        address indexed NewBearOwner,
        uint256  BearsPurchased,
        uint256 _NFTCount
    );
    ...
    function initialize() public initializer {
        __ERC721_init("TheRanch_BTC_Bears_Community", "TRBC");
    ...
    */
    function mint(uint256 _tokenQuantity) public payable {
        ...
        if (_tokenSupply.current() + _tokenQuantity  > maxSupply) {revert Minting_ExceedsTotalBears();}


        IERC20Upgradeable usdcToken = IERC20Upgradeable(usdcTokenContract);
        uint256 minting_cost_per_Bear = mintingCost * 10 ** usdcTokenDecimals;
        uint256 totalTransactionCost = minting_cost_per_Bear * _tokenQuantity;
        ...
        USDCRewardsBalance += (referralFundAmt + GiveawayFundAmt);
        ...
        if(referrer != address(0) &&  balanceOf(referrer) > 0){
            btcBearOwners[referrer].USDC_Balance += referralFundAmt;
        }
        else
        {
            uint256 splitReferralAmt = referralFundAmt * 50 / 100;
            btcBearOwners[coreTeam_1].USDC_Balance += splitReferralAmt;
            btcBearOwners[coreTeam_2].USDC_Balance += splitReferralAmt;
        }


        // update Early Supporter Flag
        if( btcBearOwners[msg.sender].earlySupporter == false){
            btcBearOwners[msg.sender].earlySupporter = true;
        }

        emit NewBearsEnteringRanch(msg.sender, _tokenQuantity, _tokenSupply.current());
    ...
    function startGiveaway() internal {
        ...
        TheRanchBTCBearsChainLinkVRF CHAINLINKVRF = TheRanchBTCBearsChainLinkVRF(chainLinkVRFContractMintAddress);
    ...
    */
    ...
    function mintingGiveaway(
        ...
        require(isApprovedCaller[msg.sender] == true, "must be the chainlinkVRFBears contract to interact");

        for( uint256 i; i < _winningAddresses.length ; i++ ) {
            // update the daily Giveaway winnners USDC balance
            btcBearOwners[_winningAddresses[i]].USDC_Balance += _winningAmount;
        ...
        emit mintingGiveawayEvent(_winningAddresses, _winningIndexes,_amoeWinningAddresses,_amoeWinningIndexes, _winningAmount);
    ...
    */
    function setReadyToReward() external ADMIN_APPROVED {
        ...
        if (monthlyHostingFee == 0) { revert Rewarding_NotReady();}
    ...
    */
    ...
    /**@dev do this transaction 1 time, it covers all the stockyards for mutliple rewards Bears function call
    **/ 
    ...
    function setPayPerNftForTheMonthAndCurrentRewardingDate(uint256 _totalAmountToDeposit, uint256 _dateOfRewarding) public onlyOwner {
        ...
        btcBearOwners[coreTeam_1].WBTC_Balance += coreTeam_1_amt;
        btcBearOwners[coreTeam_2].WBTC_Balance += coreTeam_2_amt;
    ...
    */
    ...
    function rewardBears(uint256 _stockyardNumber) public ADMIN_APPROVED {
        ...
        for( uint256 i = startingIndex; i <= endingIndex; i++) {
            address BearOwnerAddress = ownerOf(i);
            
            if (BearOwnerAddress != address(0)){

                // have we checked them this month, if lastRewardDate == currentRewardingDate then skip them  
                if (btcBearOwners[BearOwnerAddress].lastRewardDate != currentRewardingDate) {


                    // Have we ever rewarded them before, if not, add them into the rewarded address array. 
                    if (btcBearOwners[BearOwnerAddress].lastRewardDate == 0) {
                        rewardedAddresses.push(BearOwnerAddress);
                    }

                    BTCBearOwner storage _BearOwner = btcBearOwners[BearOwnerAddress];

                    // update lastRewardDate for this address 
                    _BearOwner.lastRewardDate = currentRewardingDate;
        

                    // get the amount of NFTs this address owns
                    uint256 _nftCount = walletOfOwner(BearOwnerAddress).length;

                    uint256 _nftCountForMaintenance = maintenanceFeeCount(BearOwnerAddress);

                    // get the total payout amount
                    uint256 totalPayoutForTheBearOwner = _nftCount * payPerNftForTheMonth;
                    
                    // get the referrer and the referral amount 
                    address referrer = myPartner[BearOwnerAddress];
                    uint256 referralAmt = totalPayoutForTheBearOwner * 1 / 100;

                    // update the wbtc balances accordingly with their partner 
                    if(referrer != address(0) &&  balanceOf(referrer) > 0){
                        btcBearOwners[referrer].WBTC_Balance += (referralAmt * 2);
                        _BearOwner.WBTC_Balance += (totalPayoutForTheBearOwner - (referralAmt * 2));
      
                    } else {
                        btcBearOwners[coreTeam_1].WBTC_Balance += referralAmt;
                        btcBearOwners[coreTeam_2].WBTC_Balance += referralAmt;
                        _BearOwner.WBTC_Balance += (totalPayoutForTheBearOwner - (referralAmt * 2));
                    }

                    //  update the maintenance Fees due from the _BearOwner

                    uint256 _maint_fee_to_add = (_nftCountForMaintenance * monthlyHostingFee);
                    _BearOwner.hostingFeeBalance += _maint_fee_to_add;
         

                    // update the hostingClock for the _BearOwner
                    if (_maint_fee_to_add > 0) {
                        _BearOwner.hostingClock += 1;
                    } else if (_BearOwner.hostingFeeBalance > 0){
                        _BearOwner.hostingClock += 1;
                    } 


                    // Check if _BearOwner is 3 rewards cycles behind on the account 
                    if (_BearOwner.hostingClock == 3){
                        upForLiquidation.push(BearOwnerAddress);
                    }
                }
            }
        }

        emit rewardEvent(payPerNftForTheMonth, monthlyHostingFee, startingIndex, endingIndex);
    }
    ...
    * to update there hostingClock by 1 and add to liquidation list if they reach 3 in that category 
    */
    function updateMaintenanceStandingOfThoseNotRewardedThisMonth() external ADMIN_APPROVED {
        for( uint256 i; i < rewardedAddresses.length; i++) {
            address _address = rewardedAddresses[i];
            if (btcBearOwners[_address].WBTC_Balance > 0){
                if (btcBearOwners[_address].hostingFeeBalance != 0){
                    if (btcBearOwners[_address].lastRewardDate != currentRewardingDate) {
                        
                        // take action and add one to hostingClock
                        btcBearOwners[_address].hostingClock += 1;
                        
                        if (btcBearOwners[_address].hostingClock == 3){
    ...
    **/
    function liquidateOutstandingAccounts() external ADMIN_APPROVED {
        ...
        for( uint256 i; i < upForLiquidation.length; i++) {
            ...
            BTCBearOwner storage _BearOwner = btcBearOwners[_culprit];

            // Does the _culprit have 45 dollars in their USDC.e rewards account? 
            // If they do, subtract it out and avoid liquidation. 
            uint256 _neededAmountToAvoidLiquidation = _BearOwner.hostingFeeBalance;

            if (_BearOwner.hostingFeeReserve >= _neededAmountToAvoidLiquidation ) {
                _BearOwner.hostingFeeReserve -= _neededAmountToAvoidLiquidation;
                hostingSafeBalance += _neededAmountToAvoidLiquidation;
                USDCReserveBalance -= _neededAmountToAvoidLiquidation; 
                _BearOwner.hostingFeeBalance = 0;
                _BearOwner.hostingClock = 0;
                continue;
            } else {
                uint256 _amount = _BearOwner.WBTC_Balance;
                _BearOwner.WBTC_Balance = 0;
                totalAmountLiquidated += _amount; 

                // reset fees and months behind increment 
                _BearOwner.hostingFeeBalance = 0;
                _BearOwner.hostingClock = 0;  
                _BearOwner.liquidationCount += 1; 
                // emit event
    ...
    function getLiquidatedArrayLength() external view ADMIN_APPROVED returns (uint) {
        return upForLiquidation.length;
    }


    /**
    * @dev When any other contract in our ecosystem checks the owner of the BTC Bears, it will update the USDC amount for the 
    * BTC Bears owner on this contract. It incentives ownership of both NFTS this way: 
    * In this example, lets assume we have a HayBale NFT on another smart contract, 
    * --------------------------------------------------------------------------------------------------------------------------------------------------------
    * - own Bear with and have an active partner: 98 / 2
    * - else: 98 / 1 / 1  
    */
    function royaltyGiveaway(address[] memory _winningAddresses,  uint256[] memory _winningIndexes, uint256 _winningAmount) payable external {
        ...
        for( uint256 i; i < _winningAddresses.length; i++) {
            ...
            // get the referrer of this particular BTC Bear owner  
            ...
            if(referrer != address(0) &&  balanceOf(referrer) > 0 && balanceOf(_ownerOfNFT) > 0){
                btcBearOwners[referrer].USDC_Balance += (referralAmt * 2);
                btcBearOwners[_ownerOfNFT].USDC_Balance += (_winningAmount - (referralAmt * 2));
            } else {
                btcBearOwners[coreTeam_1].USDC_Balance += referralAmt;
                btcBearOwners[coreTeam_2].USDC_Balance += referralAmt;
                btcBearOwners[_ownerOfNFT].USDC_Balance += (_winningAmount - (referralAmt * 2));
            }
        }
    ...
    **/
    function payHostingFees() external nonReentrant {

        uint256 _feesDue = btcBearOwners[msg.sender].hostingFeeBalance;
        uint256 _balance = btcBearOwners[msg.sender].hostingFeeReserve;
        
        if (_feesDue == 0) { revert Maintenance_NoMaintenanceFeesRequired();}

        if (_balance >= _feesDue){

            btcBearOwners[msg.sender].hostingFeeReserve -= _feesDue;
            hostingSafeBalance += _feesDue; 

            emit payHostingFeesEvent(msg.sender, _feesDue, 0);
        } else {
            ...
            if(_balance == 0){
                ...
                btcBearOwners[msg.sender].hostingFeeReserve -= _balance; 
            ...
            emit payHostingFeesEvent(msg.sender, _balance, amt_needed);
        }
        // reset fees and months behind. 
        btcBearOwners[msg.sender].hostingFeeBalance = 0;
        btcBearOwners[msg.sender].hostingClock = 0;
    }
    ...
    function depositIntoHostingReserve (uint256 _amountToDeposit) public payable nonReentrant {
        IERC20Upgradeable usdcToken = IERC20Upgradeable(usdcTokenContract);
        usdcToken.safeTransferFrom(msg.sender, address(this), (_amountToDeposit));

        btcBearOwners[msg.sender].hostingFeeReserve += _amountToDeposit;
        
        USDCReserveBalance += _amountToDeposit;

        // emit event
        emit addedToHostingReserve(_amountToDeposit, msg.sender);
    }



    function withdrawHostingReserve() external nonReentrant {
        if (paused) { revert Contract_CurrentlyPaused_CheckSocials();}

        // Get hostingFeeReserve balance for msg.sender
        uint256 myBalance = btcBearOwners[msg.sender].hostingFeeReserve;
        ...
        USDCReserveBalance -= myBalance;

        btcBearOwners[msg.sender].hostingFeeReserve = 0  ;
        
        // emit event
        emit withdrawHostingReserveForAddressEvent(msg.sender, myBalance);
    ...
    function withdrawToken(address _tokenContract) external onlyOwner {
        ...
        if (_tokenContract == usdcTokenContract){
            _amt = tokenContract.balanceOf(address(this)) - (btcMinersSafeBalance + hostingSafeBalance + USDCRewardsBalance + marketingSafeBalance + USDCReserveBalance);
        } else if (_tokenContract == wbtcTokenContract){
            _amt = btcBearOwners[msg.sender].WBTC_Balance; 
        } else {
   ...
   function withdrawSafe(uint _safeToTarget) external ADMIN_APPROVED {
    ...
    // function liquidationRedemption() external {
    //     if (btcBearOwners[msg.sender].liquidationCount == 0){revert Redemption_NotAllowedWhenZero();}
    //     IERC20Upgradeable tokenContract = IERC20Upgradeable(usdcTokenContract);
    //     uint256 _amount = 125 * 10 ** usdcTokenDecimals;
    //     tokenContract.safeTransferFrom(msg.sender, address(this),_amount);
    //     btcBearOwners[msg.sender].liquidationCount -= 1;
    //     hostingSafeBalance += _amount;
    //     emit liquidationRedemptionEvent(msg.sender, _amount);
    // }
    ...
    function withdrawWbtcBalance() external nonReentrant {
        ...
        require(btcBearOwners[msg.sender].hostingFeeBalance == 0, "You must pay maintenance fee balance before WBTC withdrawal is allowed");

        // Get the total Balance to award the owner of the NFT(s)
        uint256 myBalance = btcBearOwners[msg.sender].WBTC_Balance; 
        if (myBalance == 0) { revert Rewarding_NoBalanceToWithdraw();}
        ...
        btcBearOwners[msg.sender].WBTC_Balance = 0;
    ...
    function withdrawUsdcBalance() external nonReentrant {
        ...
        uint256 myBalance = btcBearOwners[msg.sender].USDC_Balance;
        if (myBalance == 0) { revert Rewarding_NoBalanceToWithdraw();}
        ...
        // update mapping on contract 
        btcBearOwners[msg.sender].USDC_Balance = 0  ;
    ...
    /**
     * @dev returns how many people have ever been rewarded from owning a BTC Bear
     */
   ...
   // METADATA
    ...
    function setMonthlyHostingFee(uint256 _newHostingFee) external onlyOwner {
        if (_newHostingFee > 15 ) { revert Hosting_FeeOverCeiling();}
        monthlyHostingFee = _newHostingFee * 10 ** usdcTokenDecimals;
    }
    ...
    function setExcludedHostingFeesNFTs(uint256 _exemptIndex, bool _bool) external onlyOwner {

        if ( excludedNftIndexes[_exemptIndex] == true && _bool == false) {
            excludedNftIndexesCount -= 1;
            excludedNftIndexes[_exemptIndex] = _bool;
        } else if (excludedNftIndexes[_exemptIndex] == false && _bool == true) {
            excludedNftIndexesCount += 1;
            excludedNftIndexes[_exemptIndex] = _bool;
        }
    ...
    **/
    ...
    function getliquidationCountForAddress(address _addressToCheck) external view returns (uint256) {
        return btcBearOwners[_addressToCheck].liquidationCount;
    }   
