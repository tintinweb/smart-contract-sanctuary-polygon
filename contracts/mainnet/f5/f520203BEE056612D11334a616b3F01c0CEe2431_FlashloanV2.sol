...
contract FlashloanV2 is FlashLoanReceiverBaseV2, Withdrawable {
    ...
    address DAI;
    constructor(address _addressProvider, IUniswapV2Router02 router1, IUniswapV2Router02 router2, address DAI_)
        public
        FlashLoanReceiverBaseV2(_addressProvider)
    {
        DAI = DAI_;
        router1 = IUniswapV2Router02(router1);
        router2 = IUniswapV2Router02(router2);
    }
    ...
    function executeOperation(
        ...
        path[0] = assets[0];
        path[1] = DAI;
        router1.swapExactTokensForTokens(amounts[0], 0, path, address(this), block.number);

        path[0] = DAI;
        path[1] = assets[0];
        router2.swapExactTokensForTokens(amounts[0], 0, path, address(this), block.number);
     ...
     */
    function myflashloan(address _asset, uint256 amount) public onlyOwner {
        bytes memory data = "";
