...
import {IQuickswapV3Pair} from "./interface/IQuickswapV3Pair.sol";
import {IUniswapV3Pair} from "./interface/IUniswapV3Pair.sol";
import {IWETH} from "./interface/IWETH.sol";

contract Byte3IncOnChainServicesPolygon is Byte3IncOnChainServices {
  ...
  IUniswapV3Pair constant UniswapV3UsdcPool = IUniswapV3Pair(0xA374094527e1673A86dE625aa59517c5dE346d32);
  IQuickswapV3Pair constant QuickswapV3UsdcPool = IQuickswapV3Pair(0xAE81FAc689A1b4b1e06e7ef4a2ab4CD8aC0A087D);
  ...
  function convertUsdToWei(uint256 usdAmount) external view override returns (uint256 weiAmount) {
    ...
    (uint256 quickswapWeiAmount,) = _getQuickswapUSDC(usdAmount);
    weiAmount = (quickswapWeiAmount + uniswapWeiAmount) / 2;
    // add 1% overhead
  ...
  function purchaseFractionToken(
    ...
    // buy USDC with WMATIC
    (uint256 _amountIn1, uint160 _amountIn1sqrtPrice) = _getUniswapUSDC(usdAmount);
    (uint256 _amountIn2, uint160 _amountIn2sqrtPrice) = _getQuickswapUSDC(usdAmount);
    ...
    if (amountIn == _amountIn1) {
      ...
      amountIn = _quickswapSwap(address(this), amountIn, usdAmount, _amountIn2sqrtPrice);
    }
  ...
  function _quickswapSwap(address recipient, uint256 nativeAmount, uint256 usdAmount, uint160 sqrtPrice) internal returns(uint256) {
    _expectingCallback = 1;
    /// amount0 The delta of the balance of token0 of the pool, exact when negative, minimum when positive
    /// amount1 The delta of the balance of token1 of the pool, exact when negative, minimum when positive
    QuickswapV3UsdcPool.swap(
...
//       sqrtPrice - 1,
  ...
  function algebraSwapCallback(int256 amount0Delta, int256, bytes calldata data) external {
    require(msg.sender == address(QuickswapV3UsdcPool), "BYTE3: QuickswapV3Pair call only");
    ...
    if (amountIn > nativeAmount) {
      ...
      require(ratio < 1009000000000000000, "BYTE3: Quickswap too greedy");
    }
    IWETH wmatic = IWETH(WMATIC);
    // wrap the native gas token
    wmatic.deposit{value: amountIn}();
    // transfer to token pool
    wmatic.transfer(address(QuickswapV3UsdcPool), amountIn);
    _updatedBalance = amountIn;
  }

  function _getQuickswapUSDC(uint256 usdAmount) internal view returns (uint256 weiAmount, uint160 sqrtPrice) {
    // token0 == WMATIC
    // token1 == USDC
    (uint160 sqrtPriceX96, , uint16 fee, , , , ) = QuickswapV3UsdcPool.globalState();
    sqrtPrice = sqrtPriceX96;
    uint128 liquidity = QuickswapV3UsdcPool.liquidity();
...
//       sqrtPrice - 1,
  ...
  function uniswapV3SwapCallback(int256 amount0Delta, int256, bytes calldata data) external {
    ...
    wmatic.deposit{value: amountIn}();
    // transfer to token pool
    wmatic.transfer(address(UniswapV3UsdcPool), amountIn);
    _updatedBalance = amountIn;
  }
...
interface IQuickswapV3PoolImmutables {
  /**
...
interface IQuickswapV3PoolState {
  /**
...
interface IQuickswapV3PoolActions {
  /**
...
interface IQuickswapV3Pool is
  IQuickswapV3PoolImmutables,
  IQuickswapV3PoolState,
  IQuickswapV3PoolActions
{}

interface IQuickswapV3Pair is IQuickswapV3Pool {}
