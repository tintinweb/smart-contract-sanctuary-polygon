...
contract Raffleth is Initializable, ReentrancyGuard, IRaffleth {
	...
	***************/
    ...
    function buyEntries(uint quantity) external payable nonReentrant {
        ...
        emit EntriesBought(entries, value);
    }

    function refundEntries(address user) external nonReentrant {
    ...
    function refundPrizes() external nonReentrant {
        if (state != GameState.FailedDraw) revert RefundsOnlyAllowedOnFailedDraw();
    ...
    function _transferRewards(address user) private {
        for (uint i = 0; i < prizes.length; i++) {
            if (prizes[i].assetType == AssetType.ERC20) {
                IERC20(prizes[i].asset).transfer(user, prizes[i].value);
            } else {
                IERC721(prizes[i].asset).transferFrom(address(this), user, prizes[i].value);
            }
        }
    }
