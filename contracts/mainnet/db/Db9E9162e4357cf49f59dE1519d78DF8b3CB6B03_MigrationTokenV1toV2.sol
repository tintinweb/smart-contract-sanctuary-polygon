...
interface IRoyaltyStaking {
    ...
    function setDailyEmissions(uint256 _amount) external;
...
contract MigrationTokenV1toV2 is Ownable, ReentrancyGuard, Pausable {
    ...
    function V1Token() internal view returns (IERC20) {
        return IERC20(v1Token);
    }

    function V2Token() internal view returns (IERC20) {
        return IERC20(v2Token);
    }
    ...
    function RoyaltySwapV1Lock() public nonReentrant whenNotPaused {
        require(stakers[msg.sender] == false, "Already Withdraw your lock!");
        uint256 balance = royalty().addressStakedBalance(msg.sender);
        V2Token().safeTransfer(msg.sender, balance);
        stakers[msg.sender] = true;
    }

    function SwapV1toV2(uint256 _amount) public nonReentrant whenNotPaused {
        require(_amount > 0, "Cannot stake 0");
        require(
            V1Token().balanceOf(msg.sender) >= _amount,
            "You don't have that much to stake!"
        );
        require(
            V2Token().balanceOf(address(this)) >= _amount,
            "All Tokens V2 Transfert!"
        );
        V1Token().safeTransferFrom(msg.sender, address(this), _amount);
        V2Token().safeTransfer(msg.sender, _amount);
    }

    function AlreadyClaim(address _address) public view returns (bool) {
        return stakers[_address];
    }

    // ADMIN
    function WithdrawV1() external onlyOwner {
        uint256 v1Balance = V1Token().balanceOf(address(this));
        V1Token().safeTransfer(msg.sender, v1Balance);
    }

    function WithdrawV2() external onlyOwner {
        uint256 v2Balance = V2Token().balanceOf(address(this));
        V2Token().safeTransfer(msg.sender, v2Balance);
    }
