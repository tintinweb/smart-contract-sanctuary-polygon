...
contract Lottery {
    ...
    bytes4 private constant transfer_function =
    ...
    event PotEvents(
        uint256 indexed lotteryId,
        uint256 indexed potTokenAmount,
        address indexed tokenAddress
    );
    event LotteryPayments(
        uint256 indexed lotteryId,
        bool isPaidAutomaticly,
        bool indexed paymentStatus
    );
    event Lotteries(
        uint256 indexed lotteryId,
    ...
    struct LotteryStructure {
        ...
        bool isLotteryFinished;
        uint256 winnerPrizeInToken;
        uint256 lastWinnerPaymentIndex;
    ...
    function createNewLottery(
        ...
        _lotteries[currentLotteryId].isPotfilled = false;
        _lotteries[currentLotteryId].lastWinnerPaymentIndex = 0;
        _lotteries[currentLotteryId]
        ...
        emit Lotteries(
            ...
            rewardPot,
            _lotteries[currentLotteryId].isPotfilled,
    ...
    {
        string memory isPotfilled = "false";
        if (_lotteries[_lotteryId].isPotfilled) {
            isPotfilled = "true";
        }
    ...
    function getWinnersWithSeed(
        ...
        require(toIndex < _lotteries[lotteryId].winners.length);
        ...
        for (uint256 i = fromIndex; i <= toIndex; i++) {
            if (isFirstItem) {
                string.concat(
                    "'",
                    Strings.toHexString(_lotteries[lotteryId].winners[i]),
                    "':",
                    Strings.toString(
    ...
    function checkPotHasMoney(uint256 lotteryId) public onlyOwner {
        require(
            !_lotteries[lotteryId].isLotteryFinished,
            "lottery is Finished"
        );
        uint256 usersCount = _lotteries[lotteryId].users.length;
        require(usersCount > 0, "Lottery does not exist");
        uint256 total_pot = 0;
        unchecked {
            total_pot =
                _lotteries[lotteryId].winners.length *
                _lotteries[lotteryId].winnerPrizeInToken;
        }
        IERC20 token = IERC20(_lotteries[lotteryId].rewardSmartContractAddress);
        uint256 storageBalance = token.balanceOf(address(this));
        require(storageBalance >= total_pot, "Insufficient funds.");
        _lotteries[lotteryId].isPotfilled = true;
        emit PotEvents(
            lotteryId,
            storageBalance,
            _lotteries[lotteryId].rewardSmartContractAddress
        );
    }

    function extractFoundsFromContract(address tokenAddress) public onlyOwner {
        IERC20 token = IERC20(tokenAddress);
        uint256 balance = token.balanceOf(address(this));
        require(balance > 0, "low balance");
        (bool success, bytes memory data) = address(tokenAddress).call(
            abi.encodeWithSelector(transfer_function, owner, balance)
        );
        require(
            success && (data.length == 0 || abi.decode(data, (bool))),
            "QPoker Lottery: TRANSFER_FAILED"
        );
    }

    function PayWinners(uint256 lotteryId, uint256 winnerCountToPay)
        public
        onlyOwner
    {
        require(
        ...
        require(winnerCountToPay > 0);
        uint256 remainingWinners = _lotteries[lotteryId].winners.length -
            _lotteries[lotteryId].lastWinnerPaymentIndex;
        require(remainingWinners > 0, "there is no winners left");

        require(
            winnerCountToPay <= remainingWinners,
            "WinnerCount is grater than the remainingWinnersToPay count"
        );
        uint256 total_pot = 0;
        unchecked {
            total_pot =
        ...
        uint256 storageBalance = token.balanceOf(address(this));
        require(storageBalance >= total_pot, "Insufficient funds.");
        uint256 startIndex = _lotteries[lotteryId].lastWinnerPaymentIndex;

        for (uint256 i = startIndex; i < startIndex + winnerCountToPay; ) {
            ...
            _lotteries[lotteryId].lastWinnerPaymentIndex = i;
            emit LotteryPayments(
                lotteryId,
                true,
                success && (data.length == 0 || abi.decode(data, (bool)))
            );
    ...
    function finishLottery(uint256 lotteryId) public onlyOwner {
        uint256 remainingWinners = _lotteries[lotteryId].winners.length -
            _lotteries[lotteryId].lastWinnerPaymentIndex -
            1;
        require(remainingWinners == 0, "Winners does not get paid");
    ...
    {
        require(!_lotteries[lotteryId].isLotteryFinished);
        require(_lotteries[lotteryId].isPotfilled);
        uint256 remainingWinners = _lotteries[lotteryId].winners.length -
