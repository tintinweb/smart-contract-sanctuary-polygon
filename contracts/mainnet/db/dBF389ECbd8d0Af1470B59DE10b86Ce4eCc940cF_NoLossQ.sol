...
contract NoLossQ is AbstractQ {
    ...
    function enter(
        ...
        _enter(_questId, _amount, _totalWeigth, _betOutcomes, _weightage, _user, _provider);
    }
}
...
abstract contract AbstractQ is AccessControl, BaseRelayRecipient {
    ...
    event QuestCreated(uint256 indexed questId, string indexed category, string title, uint256[] markets);
    event QuestUpdated(uint256 indexed questId, string newCategory);
    event QuestEntered(address indexed user, uint256 indexed questId, uint256 amount, uint256[] weightage);
    ...
    function createQuest(
        ...
        for (uint256 i = 0; i < _questions.length; i++) {
            marketIds[i] = market.createMarket(
                _questions[i],
                _predictionStartAt,
                _opensAt,
                _closesAt,
        ...
        quest.image = _image;
        quest.predictStartTimestamp = _predictionStartAt;
        quest.opensAtTimestamp = _opensAt;
        quest.closesAtTimestamp = _closesAt;
        quest.active = true;

        emit QuestCreated(questId, _category, _title, marketIds);
    }
    ...
    function _enter(
        ...
        if (address(msgSender()).isContract()) require(msgSender() == nativeWrapper, "invalid caller");
        else require(msgSender() == _user, "invalid caller");

        Quest storage quest = quests[_questId];
        require(quest.active == true, "quest inactive");
        require(
            quest.predictStartTimestamp <= block.timestamp && quest.opensAtTimestamp > block.timestamp,
            "Inactive quest"
        );
        ...
        for (uint256 i = 0; i < quest.markets.length; i++) {
            ...
            enterMarket(quest.markets[i], betOutcomes[i], _mAmount, _user, _provider);
            // validate total weightage
            totalWeightage -= weightage[i];
        }
        require(totalWeightage == 0, "invalid total weightage");

        emit QuestEntered(_user, _questId, _amount, weightage);
    }
    ...
    function claim(uint256 _questId, address _user) external {
        ...
        for (uint256 i = 0; i < quest.markets.length; i++) {
            ...
            uint256 bettedAmount = market.getUserBet(quest.markets[i], marketDetails.outcome, _user);
            if (
                marketDetails.state == IBhavishRealTimePrediction.MarketState.RESOLVED &&
                !market.isClaimed(quest.markets[i], _user) &&
                bettedAmount > 0
            ) rewards += market.claim(quest.markets[i], _user);
            if (
                marketDetails.state == IBhavishRealTimePrediction.MarketState.CLOSED &&
                !market.isRefunded(quest.markets[i], _user) &&
                bettedAmount > 0
            ) market.refundUser(quest.markets[i], _user);
        }
        emit Claimed(_user, _questId, rewards);
    }
    ...
    function resolveQuest(uint256 _questId) external onlyOperator(msg.sender) {
        Quest memory quest = quests[_questId];
        for (uint256 i = 0; i < quest.markets.length; i++) {
            market.resolveMarket(quest.markets[i]);
        }
    }
    ...
    function setNativeWrapper(address _address) external onlyAdmin(msg.sender) {
        require(_address.isContract(), "invalid address");
        nativeWrapper = _address;
    }

    function getMarkets(uint256 _questId) external view returns (uint256[] memory) {
        return quests[_questId].markets;
    }
}
