...
// File contracts/MTVRSMarketplace.sol
    ...
    NativeMetaTransaction
{
    ...
    event Purchase(
        ...
        address indexed _wearableCollection,
    ...
    constructor(address payable _vaultAddress, address _tokenAddress) {
        _initializeEIP712("MTVRSMarketplace", "1");

        updateVaultAddress(_vaultAddress);

        MTVRS = IERC20(_tokenAddress);
        erc721CollectionFactoryV2 = IERC721CollectionFactoryV2(
     ...
     */
    function updateVaultAddress(address payable _vaultAddress)
        public
        onlyOwner
    {
        require(
            _vaultAddress != address(0),
            "Cannot update the vault to the zero address"
        );
        require(
            _vaultAddress != address(this),
            "Cannot set the vault to the contract"
        );
        require(
            _vaultAddress != _msgSender(),
            "Intentional separation of vault and contract owner"
        );
     ...
     * @param _tokenAddress - Address of holding MTVRS tokens and Wearables
     */
    function updateTokenAddress(address payable _tokenAddress)
        public
        onlyOwner
    {
        require(
            _tokenAddress != address(0),
            "Cannot update the token to the zero address"
        );
        require(
            _tokenAddress != address(this),
            "Cannot set the token to the contract"
        );
        require(
            _tokenAddress != _msgSender(),
            "Intentional separation of token and contract owner"
        );
     ...
     */
    function addWearableCollection(address _wearableCollectionAddress)
        public
        onlyOwner
    {
        require(
            _wearableCollectionAddress != address(0),
            "Cannot add the zero address"
        );
        require(
            !wearableCollectionAdded[_wearableCollectionAddress],
            "Collection already added"
        );
        require(
            erc721CollectionFactoryV2.isCollectionFromFactory(
                _wearableCollectionAddress
            ),
            "Collection must be created by the DCL Factory contract"
        );

        IWearableCollection wearableCollection = IWearableCollection(
            _wearableCollectionAddress
        );
        require(
            wearableCollection.isApproved(),
            "Wearable collection must be approved by DCL wearable committe"
        );
        require(
            wearableCollection.isApprovedForAll(vaultAddress, address(this)),
            "Marketplace must be approvedForAll on the wearable collection"
        );
     ...
     */
    function removeWearableCollection(address _wearableCollectionAddress)
        public
        onlyOwner
    {
        require(
            wearableCollectionAdded[_wearableCollectionAddress],
            "Collection not in store"
        );
        ...
        for (uint256 i = 0; i < numCollections; i++) {
            if (approvedWearableCollections[i] == _wearableCollectionAddress) {
                approvedWearableCollections[i] = approvedWearableCollections[
                    numCollections - 1
                ];
    ...
    function updatePurchaseAmount(
        ...
        Rarity memory rarityCheck = rarityContract.getRarityByName(
            rarityLowerCase
        );
        require(
            keccak256(bytes(rarityCheck.name)) ==
                keccak256(bytes(rarityLowerCase)),
            "Rarity must be listed on rarity contract"
        );
        require(
            _purchaseAmount > itemMarketRates[rarityLowerCase].refundAmount,
            "Cannot set purchase amount lower than the refund amount"
        );
     ...
     */
    function updateRefundAmount(string memory _rarity, uint256 _refundAmount)
        public
        onlyOwner
    {
        string memory rarityLowerCase = String.toLowerCase(_rarity);
        ...
        require(
            _refundAmount < itemMarketRates[rarityLowerCase].purchaseAmount,
            "Cannot set refund amount higher than the purchase amount"
        );
     ...
     */
    function purchaseWearable(
        address _wearableCollectionAddress,
        uint256 _tokenId
    ) public whenNotPaused nonReentrant {
        address sender = _msgSender();
        require(
            sender != vaultAddress,
            "Vault cannot purchase wearables from the marketplace"
        );
        require(
            wearableCollectionAdded[_wearableCollectionAddress],
            "Wearable collection not approved by marketplace"
        );
        ...
        require(
            wearableCollection.ownerOf(_tokenId) == vaultAddress,
            "Wearable not owned by vault"
        );

        uint256 purchaseAmount = getPurchaseAmountForItem(
            wearableCollection,
            _tokenId
        );

        require(
            MTVRS.allowance(sender, address(this)) >= purchaseAmount,
            "Marketplace not approved to spend enough MTVRS tokens"
        );
        require(
            MTVRS.balanceOf(sender) >= purchaseAmount,
            "Not enough MTVRS tokens for purchase"
        );

        MTVRS.transferFrom(sender, vaultAddress, purchaseAmount);
        wearableCollection.safeTransferFrom(vaultAddress, sender, _tokenId);
    ...
    function returnWearable(
        ...
        require(
            sender != vaultAddress,
            "Vault cannot return wearables to the marketplace"
        );
        require(
            wearableCollectionAdded[_wearableCollectionAddress],
            "Wearable collection not approved by marketplace"
        ...
        require(
            wearableCollection.ownerOf(_tokenId) == sender,
            "Wearable not owned by customer"
        );
        require(
            wearableCollection.isApprovedForAll(sender, address(this)),
            "Marketplace must be approved by customer to pull wearables"
        );

        uint256 refundAmount = getRefundAmountForItem(
            wearableCollection,
            _tokenId
        );

        require(
            MTVRS.balanceOf(vaultAddress) > refundAmount,
            "Not enough MTVRS tokens in the vault"
        );
        require(
            MTVRS.allowance(vaultAddress, address(this)) > refundAmount,
            "Marketplace not approved to return enough MTVRS tokens"
        );

        MTVRS.transferFrom(vaultAddress, sender, refundAmount);
        wearableCollection.safeTransferFrom(sender, vaultAddress, _tokenId);

        emit Refund(sender, _wearableCollectionAddress, _tokenId, refundAmount);
    }
    ...
    function exchangeWearable(
        ...
        require(
            wearableCollection.ownerOf(_tokenIdCustomer) == sender,
            "Wearable not owned by customer"
        );
        require(
            wearableCollection.ownerOf(_tokenIdVault) == vaultAddress,
            "Wearable not owned by vault"
        );
        require(
            wearableCollection.isApprovedForAll(sender, address(this)),
        ...
        require(
            wearableCollection.isApprovedForAll(vaultAddress, address(this)),
            "Marketplace must be approved by vault to pull wearables"
        );

        uint256 exchangeValueCustomer = getPurchaseAmountForItem(
            wearableCollection,
            _tokenIdCustomer
        );
        uint256 exchangeValueVault = getPurchaseAmountForItem(
            wearableCollection,
            _tokenIdVault
        );

        require(
            exchangeValueCustomer >= exchangeValueVault,
            "Exchange value must equal or exceed value of item in marketplace"
        );

        wearableCollection.safeTransferFrom(
            sender,
            vaultAddress,
            _tokenIdCustomer
        );
     ...
     * @param _tokenId - Token ID on the specified wearable collection
     */
    function getItemMarketRateForItem(
        IWearableCollection _wearableCollection,
        uint256 _tokenId
    ) public view returns (ItemMarketRate memory) {
        (uint256 itemId, ) = _wearableCollection.decodeTokenId(_tokenId);
        (string memory itemRarity, , , , , , ) = _wearableCollection.items(
            itemId
        );
        return itemMarketRates[itemRarity];
    }

    function getPurchaseAmountForItem(
        IWearableCollection _wearableCollection,
        uint256 _tokenId
    ) public view returns (uint256) {
        return
            getItemMarketRateForItem(_wearableCollection, _tokenId)
                .purchaseAmount;
    }

    function getRefundAmountForItem(
        IWearableCollection _wearableCollection,
        uint256 _tokenId
    ) public view returns (uint256) {
        return
    ...
    function getItemMarketRateByRarity(string memory _rarity)
        public
        view
        returns (ItemMarketRate memory)
    {
        string memory rarityLowerCase = String.toLowerCase(_rarity);
     ...
     */
    function getTokensInWallet(address _address)
        public
        view
        returns (WearableNFT[] memory)
    {
        require(
            _address != address(0),
            "Cannot look up the zero address balance"
        );
        require(
            _address != address(this),
            "Marketplace does not own any tokens"
        );
        ...
        for (uint256 i = 0; i < approvedWearableCollections.length; i++) {
            wearableCollection = IWearableCollection(
                approvedWearableCollections[i]
            );
        ...
        for (uint256 i = 0; i < approvedWearableCollections.length; i++) {
            ...
            for (uint256 j = 0; j < balanceOnContract; j++) {
                uint256 tokenId = wearableCollection.tokenOfOwnerByIndex(
                    _address,
                    j
                );
                tokens[index] = WearableNFT({
                    ...
                    tokenId: tokenId,
                    marketplaceApproved: wearableCollection.isApprovedForAll(
                        _address,
                        address(this)
                    )
                });
