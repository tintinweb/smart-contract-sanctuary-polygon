...
contract LimelightBondingCalculator is IBondingCalculator {

    using FixedPoint for *;
    using LowGasSafeMath for uint;
    using LowGasSafeMath for uint112;

    IERC20 public immutable Clout;

    constructor( address _Clout ) {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(Clout) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(Clout), "not a Clout lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** Clout.decimals() ) ).div( getTotalValue( _pair ) );
    }
}