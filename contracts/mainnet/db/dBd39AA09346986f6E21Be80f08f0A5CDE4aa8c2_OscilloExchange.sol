...
contract OscilloExchange is OwnableUpgradeable {
    ...
    event Executed(uint indexed askId, uint indexed bidId, uint txCost);
    ...
    function clear(uint offset, uint length) external onlyOwner {
    ...
    function execute(LibTrade.Execution[] memory chunk) external onlyWhitelisted {
        ...
        for (uint i = 0; i < chunk.length; i++) {
            ...
            emit Executed(exec.askId, exec.bidId, txCost);
        }
    }
