...
interface RaceInterface{
    ...
    function returnRaceFull(uint256 _race) external view returns(uint[] memory);
...
contract SharkRacing is Ownable {
    ...
    function grabScores2(address con, uint start, uint end) public onlyOwner {
        RaceInterface OLD = RaceInterface(con);
        uint i = start;
        for( i; i <= end; i ++ ) {
            uint[] memory _R = OLD.returnRacers(i);
            uint[] memory _P = OLD.returnRaceFull(i);
            uint j;
            for( j; j < _R.length; j ++ ) {
                uint pos = _P[j];
                uint sl = _R[pos];
