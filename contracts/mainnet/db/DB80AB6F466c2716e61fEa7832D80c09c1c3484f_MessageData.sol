...
interface IMessageFormat {
    ...
    // 7 = reposts
    // 8 = comments
    function buildMsg(uint256[] memory msgData, string memory message, address poster, string[] memory hashtags, address[] memory taggedAccounts, string memory asGroup) external view returns (string memory);
}
...
contract MessageData {
    ...
    */
    ...
    event logNewMsg(uint256 indexed msgID, address indexed poster, MsgData newMsg, string indexed asGroup);
    event logRemoveMsg(uint256 indexed msgID, uint256 indexed isCommentOf);
    event logUpdateMsgStats(uint256 indexed msgID, uint256 indexed statType, int256 statValue);
    ...
    */
    ...
    function saveMsg(MsgData memory newMsg) public onlyAdmins {
        ...
        uint256 bucketKeyID = getBucketKey(true);
        ...
        Posts.addPost(newMsg.msgID, newMsg.poster);

        // Emit to the logs for external reference
        emit logNewMsg(newMsg.msgID, newMsg.poster, newMsg, newMsg.asGroup);
    ...
    function removeMsg(uint256 msgID, address requester, uint256 isCommentOf) public onlyAdmins {
        ...
        string memory thisBucketKey = getBucketKeyByID(msgID, false);
        ...
        require(thisMsgData.poster == requester, "Only the message owner can erase it");

        if (isCommentOf > 0){
            // Find the bucket containing the message commented on
            string memory thisBucketKeyTmp = getBucketKeyByID(isCommentOf, false);
            ...
            Posts.removePost(msgID, msgData[thisBucketKey][msgID].poster);
        }
    ...
    function getMsgsByIDs(uint256[] memory msgIDs) public view returns (string memory){
        ...
        for (uint i=0; i < msgIDs.length; i++) {
            ...
            string memory thisBucketKey = getBucketKeyByID(msgIDs[i], false);
            ...
            uint256[] memory thisData = new uint256[](9);
            ...
            string memory thisMsg = MessageFormat.buildMsg(thisData, thisMsgData.message, thisMsgData.poster, thisMsgData.hashtags, thisMsgData.taggedAccounts, thisMsgData.asGroup);
    ...
    function getBucketKeyByID(uint256 msgID, bool forInsert) internal view returns (string memory){
        // Initialize the bucket key
        string memory thisBucketKey;
        
        uint256 topBucketKey = getBucketKey(forInsert);
        ...
        for (uint b=topBucketKey; b >= 0; b--) {
            ...
            // Get the poster address from the message for a post
            address poster = msgData[thisBucketKey][msgID].poster;
    ...
    function getBucketKey(bool toInsert) internal view returns (uint256){
        ...
        while (mapID == 99999999999999999999){
            ...
            string memory bucketToCheck = append('posts-',KUtils.toString(b),'','','');
