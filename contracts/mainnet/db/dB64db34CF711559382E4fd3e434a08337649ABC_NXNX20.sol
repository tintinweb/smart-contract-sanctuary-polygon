...
contract NXNX20 is Ownable, ReentrancyGuard, IERC1155Receiver {
    ...
    function depositNft(
        ...
        require(_msgSender() == tx.origin, "Invalid Access");
        require(!suspended, "pool is suspended");
    ...
    function addUserInfo(address account, uint256 amount) public onlyOwner {
        require(block.timestamp < rewardStartTime, "cannot add user info after started");
    ...
    function addUserInfoBatch(address[] calldata accounts, uint256[] calldata amounts) public onlyOwner {
        ...
        for (uint i=0; i<accountLen; i++) {
            addUserInfo(accounts[i], amounts[i]);
        }
    }
    ...
    function updateExtraBoost(
        ...
        require(block.timestamp < rewardStartTime, "pool has started");
    ...
    function getUserTotalBonus(address account) public view returns (uint256 boost) {
        ...
        for (uint i=0; i<user.nftItems.length; i++) {
            boost = boost.add(getExtraBoost(user.nftItems[i].nftAddress, user.nftItems[i].nftTokenId));
        }
    }
