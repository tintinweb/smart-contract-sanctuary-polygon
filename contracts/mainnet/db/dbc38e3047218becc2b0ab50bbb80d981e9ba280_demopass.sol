...
contract demopass is ERC721Enumerable, Ownable , whitelistCheck{
    using Strings for uint;
    using ECDSA for bytes32;

    uint public demopass_PRESALE = 5; //presale
    uint public demopass_WALLET_PRESALE_LIMIT = 2; //demopassperwalletInWholeDuration
    uint public demopass_WALLET_PUBLICSALE_LIMIT = 10; //demopassperwalletInWholeDuration
    uint public demopass_MAX_COUNT = 30; //maxSupply
    uint public totalTokensSoldInPresale;
    uint public preSaleAmountMinted;

    uint public demopass_MAINSALE_PRICE = 1 ether; //priceInMainSale
    uint public demopass_PRESALE_PRICE = 0.5 ether; //priceInPreSale
    ...
    string private _tokenBaseURI = "https://gateway.pinata.cloud/ipfs/QmWtGhnAXEHSBAaRr3H4ddJmmfDeRX2GnAPKP3FyPRJ9vt/";
    string private constant Sig_WORD = "private";
    address private _signerAddress = 0x2141fc90F4d8114e8778447d7c19b5992F6A0611;
    ...
    constructor() ERC721("DDSMPrime AI", "DDSMAI") {}

    function setdemopass_MAX_COUNT(uint _maxCount) external onlyOwner{
        demopass_MAX_COUNT = _maxCount;
    }

    function setdemopass_PRESALE(uint _presale) external onlyOwner{
        demopass_PRESALE = _presale;
    }

    function setdemopass_WALLET_PRESALE_LIMIT(uint _presaleLimit) external onlyOwner{
        demopass_WALLET_PRESALE_LIMIT = _presaleLimit;
    }

    function setdemopass_WALLET_PUBLICSALE_LIMIT(uint _publicSaleLimit) external onlyOwner{
        demopass_WALLET_PUBLICSALE_LIMIT = _publicSaleLimit;
    }

    function setdemopass_MAINSALE_PRICE(uint _mainSalePrice) external onlyOwner{
        demopass_MAINSALE_PRICE = _mainSalePrice;
    }

    function setdemopass_PRESALE_PRICE(uint _presalePrice) external onlyOwner{
        demopass_PRESALE_PRICE = _presalePrice;
    }
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= demopass_MAX_COUNT, "EXCEED_MAX");
    ...
    function buy(uint tokenQuantity) external payable {
        ...
        require (tokenQuantity <= demopass_WALLET_PUBLICSALE_LIMIT);
        require (saleListPurchase[msg.sender] + tokenQuantity <= demopass_WALLET_PUBLICSALE_LIMIT, "MAX MINT COMPLETE");
        require (totalSupply() + tokenQuantity <= (demopass_MAX_COUNT - preSaleAmountMinted), "EXCEED_MAX");
        require (demopass_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
    ...
    function founderMint(uint tokenQuantity) external onlyOwner {
        require(totalSupply() + tokenQuantity <= demopass_MAX_COUNT, "EXCEED_MAX");
        for(uint i = 0; i < tokenQuantity; i++) {
        _safeMint(msg.sender, totalSupply() + 1);
        }
    }
    ...
    function presaleBuy( Whitelist memory whitelist, uint tokenQuantity) external payable{
        require(presaleLive, "PRESALE_CLOSED");//demopass_WALLET_PRESALE_LIMIT
        require(getWhitelist(whitelist), "DIRECT_MINT_DISALLOWED");
        require(whitelist.userAddress == msg.sender);
        require(preSaleAmountMinted + tokenQuantity <= demopass_PRESALE, "EXCEED_PRIVATE");
        require(presalerListPurchases[msg.sender] + tokenQuantity <= demopass_WALLET_PRESALE_LIMIT, "EXCEED_ALLOC");
        require(demopass_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
