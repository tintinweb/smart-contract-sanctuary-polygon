...
/**
 * @title Deploys marketplace instances.
 */
contract CustomVaultFactoryUpgradeable is StandardAccessControlEnumerable {
    ...
    address[] public customVaults;

    //----------------------------------------
    // Events
    //----------------------------------------

    event CustomVaultDeployed(address indexed newCustomVault);
     ...
     * @param _finder Synthereum finder contract
     */
    constructor(IFinder _finder) {
        finder = _finder;
        address mananger = address(finder.getManager());
        _setAdmin(mananger);
        _setMaintainer(mananger);
        _grantRole(FinderLib.getMaintainerRole(), msg.sender);
        implementationAddress = address(new CustomVault());
    }

    // function deployCustomVault(
    //     string memory _name,
    //     address _stakeToken,
    //     address _xToken,
    //     address _manager
    // ) external onlyMaintainer returns (address) {
    //     // TODO: add duplicate custom vaults checks
    //     CustomVault _newVault = new CustomVault();
    //     _newVault.initializeVault(
    //         _name,
    //         _stakeToken,
    //         _xToken,
    //         _manager,
    ...
    //     emit CustomVaultDeployed(address(_newVault));
    //     bytes32 defaultAdminRole = FinderLib.getDefaultAdminRole();
    //     _newVault.grantRole(defaultAdminRole, address(finder.getManager()));
    //     _newVault.renounceRole(defaultAdminRole, address(this));
    //     customVaults.push(address(_newVault));
    //     return address(_newVault);
    // }
    ...
    function deployCustomVault(
        ...
        ERC1967Proxy _newVault = new ERC1967Proxy(
       ...
       emit CustomVaultDeployed(address(_newVault));
       bytes32 defaultAdminRole = FinderLib.getDefaultAdminRole();
        IAccessControlEnumerable(address(_newVault)).grantRole(defaultAdminRole, address(finder.getManager()));
        IAccessControlEnumerable(address(_newVault)).renounceRole(defaultAdminRole, address(this));
        customVaults.push(address(_newVault));
        return address(_newVault);
    }

    function getAllCustomVaults() external view returns (address[] memory) {
        return customVaults;
    }
    ...
    function setImplementationAddress(
        ...
        require(_newImplementationAddress != address(0), "Zero Address Input");
    ...
    Multicall
{
    ...
    function _beforeTokenTransfer(
        ...
        require(!blacklisted[to], "Receiver is on blacklist");
        require(!blacklisted[from], "You are on blacklist");
        super._beforeTokenTransfer(from, to, amount);
    }

    function blacklistUpdate(address user, bool value) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "callet not admin");
...
// import {BaseERC4626Custom, MintableBurnableSyntheticTokenPermitUpgradeable, SafeERC20Upgradeable} from "./utils/BaseERC4626Custom.sol";
    ...
    StandardAccessControlEnumerableUpgradeable
{
    ...
    using SafeERC20 for MintableBurnableSyntheticTokenPermit;
    ...
    MintableBurnableSyntheticTokenPermit public xToken;

    address public depositManager;

    uint public stakeTokenDecimal;
    ...
    function initialize(
        ...
        xToken = MintableBurnableSyntheticTokenPermit(_xToken);
        _setMAdmin(_manager);
        _setMAdmin(msg.sender);
        stakeTokenDecimal = MintableBurnableSyntheticTokenPermit(stakeToken)
            .decimals();
    ...
    function deposit(
        ...
        MintableBurnableSyntheticTokenPermit(stakeToken).safeTransferFrom(
            msg.sender,
            address(this),
            assets
        );
        shares = assets * (10 ** (18 - stakeTokenDecimal));
        _mint(receiver, shares);
    }

    function _mint(address _to, uint256 _amount) internal virtual {
        xToken.mint(_to, _amount);
    }

    function _burn(address _from, uint256 _amount) internal virtual {
        (_from);
        xToken.safeIncreaseAllowance(address(xToken), _amount);
        xToken.burn(_amount);
    }
    ...
    function withdraw(
        ...
        uint xtokens = amountToWithdraw * (10 ** (18 - stakeTokenDecimal));
        xToken.safeTransferFrom(_msgSender(), address(this), xtokens);
        _burn(_msgSender(), xtokens);
        MintableBurnableSyntheticTokenPermit(stakeToken).safeTransfer(
            receiver,
            amountToWithdraw
        );
    ...
    function rescueToken(
        ...
        MintableBurnableSyntheticTokenPermit(_tokenAddress).safeTransfer(
            msg.sender,
            _amount
        );
    }
    ...
    function borrow(uint amount) external onlyMarketplaceBorrowerRole {
        // uint scaledXAmount = amount * (10 ** (18 - stakeTokenDecimal));
        xToken.safeTransferFrom(msg.sender, address(this), amount);
        amount /= 10 ** (18 - stakeTokenDecimal);
        MintableBurnableSyntheticTokenPermit(stakeToken).safeTransfer(
            msg.sender,
            amount
        );
    ...
    function repay(
        address recepient,
        uint amount
    ) external onlyMarketplaceBorrowerRole {
        MintableBurnableSyntheticTokenPermit(stakeToken).safeTransferFrom(
            msg.sender,
            address(this),
            amount
        ...
        amount *=
            10 **
                (18 -
                    MintableBurnableSyntheticTokenPermit(stakeToken)
                        .decimals());
        xToken.safeTransfer(recepient, amount);
    }
