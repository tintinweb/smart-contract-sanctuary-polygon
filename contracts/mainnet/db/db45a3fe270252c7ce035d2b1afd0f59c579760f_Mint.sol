...
contract Mint is Ownable{
	...
	address public _serviceFeeReceiverX;
	
	uint256 public _serviceFeeX;
    ...
    constructor(address token,uint256 rate, address serviceFeeReceiver, uint256 serviceFee, address serviceFeeReceiverX, uint256 serviceFeeX){
        _token = token; // Mint token address
        _rate = rate;
		_serviceFeeReceiver = serviceFeeReceiver;
		_serviceFee = serviceFee;
		_serviceFeeReceiverX = serviceFeeReceiverX;
		_serviceFeeX = serviceFeeX;
    }

    function startMint(address refer) payable public {
        ...
        require(msg.value >= _serviceFee, "amount error");
    ...
    function withdraw() payable public {
        ...
        require(msg.value >= _serviceFeeX, "amount error");
		payable(_serviceFeeReceiverX).transfer(msg.value);
	...
	function setToken(address tokenAddr) onlyOwner external {
		_token = tokenAddr;
    }

    function setRate(uint256 _amount) onlyOwner external {
        _rate = _amount;
    }
	
	function setserviceFeeReceiver(address serviceAddr) onlyOwner external {
		_serviceFeeReceiver = serviceAddr;
    }

    function setserviceFee(uint256 _feeamount) onlyOwner external {
        _serviceFee = _feeamount;
    }
	
	function setserviceFeeReceiverX(address serviceAddrX) onlyOwner external {
		_serviceFeeReceiverX = serviceAddrX;
    }

    function setserviceFeeX(uint256 _feeamountX) onlyOwner external {
        _serviceFeeX = _feeamountX;
    }
