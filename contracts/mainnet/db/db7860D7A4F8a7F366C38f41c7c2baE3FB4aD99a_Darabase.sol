...
contract Darabase is NFTContract {
    ...
    function Darabase_init(
        ...
        require(
            _platformFeeReceiver != address(0) &&
            _admin != address(0) &&
            __owner != address(0),
            "Err_31"
        );
    ...
    function buyRental(
        ...
        if(sellOrder.orderType == LibOrder.PRIMARY_TYPE){    
            ...
            if(sellOrder.maker != getPdrOwner(sellOrder.pdrId)) revert("Err_32");
            ...
            if(sellOrder.maker != getRentalOwner(sellOrder.rentalId)) revert("Err_33");
    ...
    /**
     * @dev Allows a PDR owner to buy back the rentals created from their PDR.
     * @param sellOrder In Order struct, sell order 
     * @param sellSignature In bytes, signature to be verified
     * @param numberOfRentalsOwnerBuying In uint256, The number of rentals PDR owner is buying back.
     * @param currency In unix, start Time for rental
     */
    function buyBackRentals(
        LibOrder.Order memory sellOrder,
        bytes memory sellSignature,
        uint256 numberOfRentalsOwnerBuying,
        string memory currency
    ) external payable whenNotPaused {

        // Nft being sold should be of type rental.
        require(uint(tokenType[sellOrder.rentalId]) == 2, "Err_35");

        // Seller must have sufficient balance.
        require(balanceOf(sellOrder.maker, sellOrder.rentalId) >= numberOfRentalsOwnerBuying, "Err_36");

        // Buyer must be the PDR owner.
        uint256 pdrId = rentalIdToPDRId[sellOrder.rentalId];
        address pdrOwner = getPdrOwner(pdrId);
        require(msg.sender == pdrOwner, "Err_37");
        ...
        // Calculate Payment
        (uint256 totalPayment, uint256 buyPayment, uint256 platformPayment) = calculateTotalPayment(sellOrder.payment.amount, numberOfRentalsOwnerBuying, sellOrder.numberOfTokens);

        // Check Matic Balance
        checkMaticBalance(sellOrder.payment.paymentClass, totalPayment);

        if(sellOrder.orderType == LibOrder.SECONDARY_TYPE){
            // Check if the order maker is the rental Owner
            require(sellOrder.maker == getRentalOwner(sellOrder.rentalId), "Err_38");

            // Burn rental tokens and mint pdr tokens.
            // Deduct Rental NFT amount by amount
            rentalAmount[sellOrder.rentalId] -= numberOfRentalsOwnerBuying;
            
            // Burn the rentals.
            _burn(sellOrder.maker, sellOrder.rentalId, numberOfRentalsOwnerBuying, false);

            // Return the balance to the pdr balance
            _balances[pdrId][pdrOwner] += numberOfRentalsOwnerBuying;
            emit TransferSingle(address(this), address(0), msg.sender, pdrId, numberOfRentalsOwnerBuying);

            // Trade
            doPaymentTransfer(msg.sender, pdrId, totalPayment, buyPayment, platformPayment, sellOrder.maker, sellOrder.payment, false, currency);
        }
        else{
            revert("Err_39");
        }
    ...
    /**
     * @dev Airdrop Rental Nfts, can be called only by admin or owner.
     * @param airdropReceiver Address to which rental nfts would be sent.
     * @param order In Order struct.
     * @param startTime In unix, start time for rental nfts.
     * @param endTime In unix, end time for rental nfts.
     * @param currency In String, currency name
     */
    function airdropRental(
        ...
        // Validate startTime and endTime given for rentals.
        LibOrder.checkTime(startTime, endTime);

        // Calculate the payments.
        (uint256 totalPayment, uint256 buyPayment, uint256 platformPayment) = calculateTotalPayment(order.payment.amount, order.numberOfTokens, order.numberOfTokens);

        // Check matic balance.
        checkMaticBalance(order.payment.paymentClass, totalPayment);

        // Check if order.maker is the pdr owner.
        if (order.maker != getPdrOwner(order.pdrId)) {
            revert("Err_40");
        }

        // Create and airdrop the rental nfts.
        transferRentalPrimary(order.maker, airdropReceiver, order.pdrId, order.nftId, order.numberOfTokens, startTime, endTime);

        // Payment Transfers
        doPaymentTransfer(msg.sender, order.pdrId, totalPayment, buyPayment, platformPayment, order.maker, order.payment, false, currency);
        // Platform fee.
        doPlatformPayment(msg.sender, order.payment, platformPayment, currency);
    }
    ...
    function checkMaticBalance(bytes4 paymentClass, uint totalPayment) internal view{
        if(paymentClass == LibOrder.MATIC_PAYMENT_CLASS) {
            if(msg.value < totalPayment) revert("Err_41");
        }
    }
    ...
    function renewRental(
        ...
        // Checking Prerequisites
        require(renewOrder.orderType == LibOrder.RENEW_TYPE, "Err_42");
        require(msg.sender == getRentalOwner(renewOrder.rentalId), "Err_43");
        LibOrder.checkTime(startTime, endTime); 
        ...
        if(renewOrder.maker != getPdrOwner(renewOrder.pdrId)) revert("Err_44");
    ...
    function checkAndCompleteOrder(bytes32 _orderHash) internal {
        require(!isComplete[_orderHash], "Err_45");
    ...
    function distributeRevenue(
        ...
        // Check Prerequisites
        require(getTokenType(pdrId) == TokenType.PDR, "Err_46");
        address ownerPdr = getPdrOwner(pdrId);
        require(msg.sender == ownerPdr || msg.sender == owner() || msg.sender == admin, "Err_47");
    ...
    function distribute(
        ...
        if (sendRemainingToRwoOwner == true) {
            platformPayment = (netPayment * platformFee)/(10000); 
        } else {
            platformPayment = (completedRevenuePayment * platformFee)/(10000);   
        }              
    }
    ...
    function calculateTotalPayment(
        ...
        require(numberOfRentals <= totalNumberOfRentals, "Err_48");
    ...
    function setPlatformFeeReceiver(
        ...
        require(
            _newPlatformFeeReceiver != address(0),
            "Err_53"
        );
...
contract NFTContract is ERC1155, Ownable, Pausable{
    ...
    // Total rental amount
    mapping(uint256 => uint256) internal rentalAmount;
    ...
    modifier onlyAdminOrOwner {
        require(owner() == msg.sender || admin == msg.sender, "Err_1");
        _;
    }
    ...
    function checkAndStoreDuplicateNFTID(uint256 nftId) internal{
        if(isCreated[nftId]) revert("Err_2");
    ...
    function mintPDR(
        ...
        // Prerequisites
        if(getTokenType(rwoId) != TokenType.RWO) revert("Err_3");
    ...
    function mintRentalToken(
        ...
        // Prerequisites
        require(getTokenType(pdrId) == TokenType.PDR, "Err_4");
    ...
    function transferRentalPrimary(
        ...
        // Prerequisites
        require(getTokenType(pdrId) == TokenType.PDR, "Err_5");

        // Check Balance
        if(balanceOf(from, pdrId) < amount) revert("Err_6");

        // Deduct Balance
        _balances[pdrId][from] -= amount;

        // emit Transfer Single event
        emit TransferSingle(address(this), from, to, pdrId, amount);
    ...
    function transferRentalSecondary(
        ...
        // Prerequisites
        require(getTokenType(rentalId) == TokenType.RENTAL, "Err_7");

        // Balance check
        require(amount <= balanceOf(from, rentalId), "Err_8");
        ...
        // emit Transfer Single event
        emit TransferSingle(address(this), from, to, rentalId, amount);
    ...
    function getTokenType(
        ...
        if(!isCreated[tokenId] || tokenId == 0) revert("Err_9");
    ...
    function burn(
        ...
        if(getTokenType(id) == TokenType.RWO) {
            ...
            // Check caller
            if(getRwoOwner(id) != msg.sender && notOwnerOrAdmin) revert("Err_11");
            ...
            // Checks
            if(getPdrOwner(id) != msg.sender && notOwnerOrAdmin) revert("Err_12");
            if(amount != pdrLimit[id]) revert("Err_13");
            ...
            // Checks
            if(rentalOwner != msg.sender && notOwnerOrAdmin) revert("Err_14");
            if(balanceOf(rentalOwner, id) < amount) revert("Err_15");
    ...
    function balanceOf(address account, uint256 id) public view virtual override returns (uint256) {
        require(account != address(0), "Err_17");
    ...
    function isPdrBurned(uint256 pdrId) public view returns(bool){
        require(getTokenType(pdrId) == TokenType.PDR, "Err_18");
        return isPDRBurned[pdrId];
    }

    function returnRentalToPDRBalanceBatch(uint256[] memory rentalIds) external onlyAdminOrOwner whenNotPaused {
        for(uint256 i; i<rentalIds.length; i++) {
            ...
            require(_rentalAmount > 0 && balanceOf(rentalOwner, rentalId) == 0, "Err_19");
            ...
            _balances[pdrId][_pdrOwner] += _rentalAmount;

            // emit TransferSingle of the pdr
            emit TransferSingle(address(this), rentalOwner, _pdrOwner, pdrId, _rentalAmount);
        }
    }
    ...
    function safeTransferFrom(
        ...
        // Assuming only possible token types are RWO, PDR, RENTAL
        if(getTokenType(id) == TokenType.RWO){
            // Prerequisites
            require(getTokenType(id) == TokenType.RWO, "Err_20");
            address _rwoOwner = getRwoOwner(id);
            if(_rwoOwner == address(0)) revert("Err_21");
            require(msg.sender == admin || msg.sender == owner() || msg.sender == _rwoOwner, "Err_22");
            ...
            for(uint256 i=0; i< getPdrFromRwo(id).length; i++){
                pdrId = rwoToPdr[id][i];
                ...
                _safeTransferFrom(_rwoOwner, to, pdrId, pdrAmount, "");
            }
            
            // Update RWO Owner
            setRwoOwner(id, to);
            _safeTransferFrom(_rwoOwner, to, id, balanceOf(_rwoOwner, id), "");
        }
        else if(getTokenType(id) == TokenType.PDR){
            // Prerequisites
            address _pdrOwner = getPdrOwner(id);
            require(msg.sender == admin || msg.sender == owner() || msg.sender == _pdrOwner, "Err_23");
            
            // Check if pdr is burned
            if(isPDRBurned[id]) revert("Err_24");

            // Update PDR Owner
            setPdrOwner(id, to);

            _safeTransferFrom(_pdrOwner, to, id, balanceOf(_pdrOwner, id), "");
        }
        else{
            revert("Err_25");
        }
    ...
    function safeBatchTransferFrom(
        ...
        require(ids.length == amounts.length, "Err_26");
        require(to != address(0), "Err_27");  
...
library LibOrder{
    ...
    function doERC20Transfer(address token, address from, address to, uint256 amount) internal{
        require(IERC20(token).transferFrom(from, to, amount), "Err_30");
    }
