...
contract Lensdrop {
    ...
    function escrowTokens(string memory rewardDetails, uint256 amount, uint256 noOfRecipients, uint256 deadline) 
        public payable returns (EscrowDetails memory) {    
            uint256 fee = (noOfRecipients * 2 ether)/100;
            if (fee > 1 ether) {
                fee = 1 ether;
            } 
            require(msg.value >= fee + (amount * noOfRecipients));

            Escrows[totalEscrows].user = msg.sender;
            Escrows[totalEscrows].rewardDetails = rewardDetails;
            Escrows[totalEscrows].amount = amount;
            Escrows[totalEscrows].noOfRecipients = noOfRecipients;
            Escrows[totalEscrows].deadline = deadline;

            Users[msg.sender].push(totalEscrows);
            totalEscrows++;

            return Escrows[totalEscrows];
    }

    function escrowErc20Tokens(string memory rewardDetails, address tokenAddress, uint256 amount, uint256 noOfRecipients, uint256 deadline) 
        public payable returns (EscrowDetails memory) {    
            uint256 fee = (noOfRecipients * 2 ether)/100;
            if (fee > 1 ether) {
                fee = 1 ether;
