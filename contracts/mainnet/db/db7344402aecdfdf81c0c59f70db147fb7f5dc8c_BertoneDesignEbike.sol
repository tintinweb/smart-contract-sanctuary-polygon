...
import "./interfaces/IBertoneDesignEbike.sol";
import "hardhat/console.sol";

contract BertoneDesignEbike is
    IBertoneDesignEbike,
    Ownable,
    ERC1155,
    MerkleProofVerify
{
    /**
     * @dev See {IBertoneDesignEbike-name}
     */
    string public constant override name = "Lib.ike";

    /**
     * @dev See {IBertoneDesignEbike-symbol}
     */
    string public constant override symbol = "LIBIKE";

    /**
     * @dev See {IBertoneDesignEbike-maxSupply}
     */
    uint32 public constant override maxSupply = 500;

    /**
     * @dev See {IBertoneDesignEbike-price}
     */
    ...
    /**
     * @dev See {IBertoneDesignEbike-wallet}
     */
    address public immutable override wallet;

    /**
     * @dev See {IBertoneDesignEbike-totalSupply}
     */
    ...
    /**
     * @dev See {IBertoneDesignEbike-claims}
    ...
    /**
     * @dev See {IBertoneDesignEbike-tokensPrice}.
     */
    function tokensPrice(uint32 _amount)
        public
        pure
        override
        returns (uint256)
    ...
    /**
     * @dev See {IBertoneDesignEbike-mint}.
     *
    ...
    function mint(uint256 _value, uint32 _amount) external override {
        require(_value == tokensPrice(_amount), "invalid price");
    ...
    /**
     * @dev See {IBertoneDesignEbike-claim}.
     *
     * Emits a {NFTMinted} event indicating the mint of amount of a new NFT.
     *
     * @param _proof - bytes32 array of proofs.
     *
     * Requirements:
     *
     * - `_proof` must be valid.
     */
    function claim(bytes32[] memory _proof) external override {
        bytes32 _leaf = keccak256(abi.encodePacked(_msgSender()));
        require(verify(_proof, _leaf), "invalid proof");
        require(!claims[_msgSender()], "already claimed");
    ...
    /**
     * @dev See {IBertoneDesignEbike-freeMint}.
     *
     * Emits a {NFTMinted} event indicating the mint of amount of a new NFT.
     *
    ...
    /**
     * @dev See {IBertoneDesignEbike-setTokenURI}.
     *
     * Emits a {TokenURISet} event indicating update of uri of a token id.
     *
     * @param uri_ - ipfs hash or metadata
     *
     * Requirements:
     *
     * - caller must be {owner}.
     * - uri must not be already set.
     * - ipfs format:`ipfs://{hash}`.
     */
    function setURI(string memory uri_) public override onlyOwner {
        require((bytes(_uri).length == 0), "uri already set");
        _uri = uri_;
        emit URISet(_uri);
    }

    /**
     * @dev See {IBertoneDesignEbike-setRoot}.
     *
     ...
     * - `_root` must be valid.
     */
    function setRoot(bytes32 _root, string memory _proofHash)
        public
        override
        onlyOwner
    {
     ...
     * @param _amount - amount to mint.
     */
    function _mint(address _to, uint32 _amount) internal {
        require(_amount <= 3, "max 3 mints per txn");
...
contract MerkleProofVerify is IMerkleProofVerify {
    ...
    function _setRoot(bytes32 _root, string memory _proofHash)
        ...
        virtual
    {
        require(_root != "" && bytes(_proofHash).length != 0, "invalid root");
        root = _root;
...
interface IBertoneDesignEbike {
    ...
    /**
     * @dev Emitted when a URI is updated.
     */
