...
contract testDapp  {
    ...
    uint256[2] private leveldiamondPercents = [400, 200];
    uint256[15] private leveldoublediamondPercents = [200, 100, 100, 100, 100, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50];
      uint256 private constant diamondPoolPercents = 50;
    uint256 private constant VenusPoolPercents = 100;
    ...
    uint256 public totalUser; 
     uint256 public lastfreezetime;
      ...
      uint256 public venusPool;
      mapping(uint256=>address[]) public dayUsers;
  
      address[] public leaderUsers;
      address[] public diamondUsers;
      address[] public doublediamondUsers;
      address[] public venusUsers;
     struct OrderInfo {
        uint256 amount; 
        uint256 start;
        uint256 unfreeze; 
    ...
    constructor(address _usdtAddr, address _defaultRefer)   {
        ...
        feeReceivers[0] = address(0x0C167b5BeA3E5802f64F758e95C825Cf2d77F458);    
 ...
 function _updateReferInfo(address _user, uint256 _amount) private {
     ...
     function _updateLevel(address _user) private {
        ...
        if(levelNow > user.level){
            user.level = levelNow;
            if(levelNow == 2){        
                leaderUsers.push(_user);
            }
              if(levelNow == 3){        
                diamondUsers.push(_user);
            }
              if(levelNow == 4){        
                doublediamondUsers.push(_user);
            }
            if(levelNow == 5){
              
                venusUsers.push(_user);
            }
        }
    }
  ...
  function getTeamDeposit(address _user) public view returns(uint256, uint256, uint256){
   ...
   function _deposit(address _user, uint256 _amount) private {
     ...
     function _distributeDeposit(uint256 _amount) private {
        ...
        Dai.transfer(feeReceivers[0], fee);
       
     
        uint256 diamond = _amount.mul(diamondPoolPercents).div(baseDivider);
        diamondPool = diamondPool.add(diamond); 
         uint256 venus = _amount.mul(VenusPoolPercents).div(baseDivider);
        venusPool = venusPool.add(venus); 
    }

    function distributePoolRewards() public {
        if(block.timestamp > lastDistribute.add(timeStep)){     
           _distributeDiamondPool(); 
           _distributeVenusPool();
            lastDistribute = block.timestamp;
        }
    }
       ...
       function _distributeDiamondPool() private {
        ...
        for(uint256 i = 0; i < diamondUsers.length; i++){
            if(userInfo[diamondUsers[i]].level == 3){
        ...
        if(diamondCount > 0){
            ...
            for(uint256 i = 0; i < diamondUsers.length; i++){
                if(userInfo[diamondUsers[i]].level == 3){
                    rewardInfo[diamondUsers[i]].diamond = rewardInfo[diamondUsers[i]].diamond.add(reward);
 ...
 function _distributeVenusPool() private {
        uint256 venusCount;
        for(uint256 i = 0; i < venusUsers.length; i++){
            if(userInfo[venusUsers[i]].level == 5){
                venusCount = venusCount.add(1);
            }
        }
        if(venusCount > 0){
            uint256 reward = venusPool.div(venusCount);
            uint256 totalReward;
            for(uint256 i = 0; i < venusUsers.length; i++){
                if(userInfo[venusUsers[i]].level == 5){
                    rewardInfo[venusUsers[i]].venus = rewardInfo[venusUsers[i]].venus.add(reward);
                    userInfo[venusUsers[i]].totalRevenue = userInfo[venusUsers[i]].totalRevenue.add(reward);
                    totalReward = totalReward.add(reward);
                }
            }
            if(venusPool > totalReward){
                venusPool = venusPool.sub(totalReward);
            }else{
                venusPool = 0;
            }
        }
    ...
    function getDepositorsLength() external view returns(uint256) {
     ...
     function getleaderusersLength() external view returns(uint256) {
        return leaderUsers.length;
    }
    function getdiamondusersLength() external view returns(uint256) {
        return diamondUsers.length;
    }
    function getdoublediamondusersLength() external view returns(uint256) {
        return doublediamondUsers.length;
    }
   function getvenususersLength() external view returns(uint256) {
        return venusUsers.length;
    }
 ...
 function getActiveUpline(address _user) public view returns(bool) {
    ...
    function _calCurDynamicRewards(address _user) private view returns(uint256, uint256) {
       RewardInfo storage userRewards = rewardInfo[_user];
        ...
        totalRewards = totalRewards.add(userRewards.venus.add(userRewards.diamond));
        uint256 splitAmt = totalRewards.mul(freezeIncomePercents).div(baseDivider);
        uint256 withdrawable = totalRewards.sub(splitAmt);
        return(withdrawable, splitAmt);
    }
            ...
            if(upline != address(0)){
              ...
              if(i==0 && idstatus==true){
                ...
                if(userInfo[upline].level > 2 && i < 5 && idstatus==true){
                    reward = newAmount.mul(leveldiamondPercents[i - 3]).div(baseDivider);
                  upRewards.level4Freezed = upRewards.level4Freezed.add(reward);
                }else if(userInfo[upline].level > 3 && i >= 5 && idstatus==true){
                    reward = newAmount.mul(leveldoublediamondPercents[i - 5]).div(baseDivider);
               ...
               _updatestatus(upline);
                ...
                if(i >= 3 && i < 5 && userInfo[upline].level > 2 && idstatus==true){
                    if(upRewards.level4Freezed > 0){
                        uint256 level4Reward = newAmount.mul(leveldiamondPercents[i - 3]).div(baseDivider);
                ...
                if(i >= 5 && userInfo[upline].level > 3  && idstatus==true){
                    if(upRewards.level5Left > 0){
                        uint256 level5Reward = newAmount.mul(leveldoublediamondPercents[i - 5]).div(baseDivider);
        ...
        for(uint256 i = balDown.length; i > 0; i--){
            if(balStatus[balDown[i - 1]]){
                ...
                if(_bal < balDown[i - 1].sub(maxDown)){
                    isFreezeReward = true;
                     uint256   unfreezetime = 90 minutes;
