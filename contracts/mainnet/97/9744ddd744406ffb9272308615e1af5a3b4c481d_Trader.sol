...
contract Trader is Ownable {
    ...
    function _settleBalances(
        ...
        if(o.exchange.paymentAmt > 0){
            WETH.transferFrom(address(this), o.exchange.paymentAddress, o.exchange.paymentAmt * amount);
        }
        // paying prepayment
        if(o.prePayment.paymentAmt > 0){
            WETH.transferFrom(address(this), o.prePayment.paymentAddress, o.prePayment.paymentAmt * amount);
        }
        if (o.refererrAmt > 0 && referrer != address(0)) {
            // payEther(o.refererrAmt * amount, referrer);
            // paying referral
            WETH.transferFrom(address(this), referrer, o.refererrAmt);
            // paying sender
            WETH.transferFrom(address(this), msg.sender, (o.totalAmt - o.exchange.paymentAmt - o.prePayment.paymentAmt - o.refererrAmt) * amount - p.paymentAmt);
        } else {
            // paying sender
            WETH.transferFrom(address(this), msg.sender, (o.totalAmt - o.exchange.paymentAmt - o.prePayment.paymentAmt) * amount - p.paymentAmt);
           
        }
        if (p.paymentAmt > 0){
            // paying post payments
            WETH.transferFrom(address(this), p.paymentAddress, p.paymentAmt);
        }
