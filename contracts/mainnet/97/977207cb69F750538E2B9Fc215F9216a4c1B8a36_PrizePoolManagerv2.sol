...
contract PrizePoolManagerv2 is OwnableUpgradeable {
    ...
    function registerUsers(
        ...
        uint256 custodialEntryFeePayable = 0;
        for(uint256 i = 0; i < userIds.length; i++){ 
            bool transferFee = true;
            if(walletAddresses[i] == custodialAddress){
                transferFee = false;
                custodialEntryFeePayable += amounts[i];
            }
            _registerUser( 
                entityId,
                walletAddresses[i], 
        ...
        if(custodialEntryFeePayable > 0){
            USDC.transferFrom(custodialAddress, address(this), custodialEntryFeePayable);
        }
    }
    ...
    function registerUser(
        ...
        _registerUser(entityId, walletAddress, userId, amount, true);
    }
    ...
    function _registerUser(
        ...
        if(!prizePool.onlyPayFees && transferFee){
