...
contract NineWorldsMulti is Ownable, VRFConsumerBase {
    ...
    uint256 constant public MAX_BOATS_SUPPLY = 9;
    ...
    function goToNextState(uint256 _boatId) external onlyOwner {
        ...
        require(block.timestamp >= boats[_boatId].joinDeadline, "NineWorldsMulti: The join deadline is not finished");
        if (boats[_boatId].status == Status.Join) {
            if(boats[_boatId].stakeData.stakeUSDT >= boats[_boatId].stakeData.minStakeAmount && boats[_boatId].currentParticipants >= boats[_boatId].minParticipants) {
    ...
    function addBoat(uint256 _minPrice, uint256 _minParticipants, uint256 _joinDeadline, uint256 _latestOpeningTime, uint256 _minStakeAmount, uint256 _exponent, uint256 _minimumPercentage) external onlyOwner {
        ...
        require(MAX_BOATS_SUPPLY > boatsNumber.current(), "NineWorldsMulti: The maximum boats supply is reached");
    ...
    function participate(uint256 _boatId, uint256[] memory vikingIds, uint256 _amount) external {
        ...
        require(userStakesUSDT[_msgSender()][_boatId] == 0, "NineWorldsMulti: The user is already on the boat");
        require(_amount > 0 && _amount >= boats[_boatId].stakeData.minPrice, "NineWorldsMulti: The amount to participate is wrong");
        require(usdToken.balanceOf(_msgSender()) >= boats[_boatId].stakeData.minPrice && usdToken.balanceOf(_msgSender()) >= _amount, "NineWorldsMulti: User balance is not enough");
    ...
    function claimBoat(uint256 _boatId) external {
        require(boats[_boatId].stakeData.stakeUSDT > 0, "NineWorldsMulti: Invalid claim, stake total insufficient");
        require(boats[_boatId].status == Status.Claim, "NineWorldsMulti: The participation is still active");
        require(userStakesUSDT[_msgSender()][_boatId] > 0, "NineWorldsMulti: The user has not right to claim");
