...
interface IFarmStable {
    function getReward() external;
    function getStableReward() external;
}

contract StratX2_Rewards_SINGLE_GOGO_3 is StratX2_PCS {
        ...
        returns (uint256)
    {
        ...
        require(sharesAdded > 1000, "deposit too small");
        ...
        returns (uint256)
    {
        ...
        _wantAmt = sharesRemoved.mul(wantLockedTotal).div(sharesTotal);
        ...
        require(wantAmt >= (_wantAmt.mul(withdrawSafetyFactor).div(10000)), "!withdrawSafetyFactor" );
    ...
    function earn() public override nonReentrant whenNotPaused {
        ...
        uint256 gogoAmt = IERC20(token1Address).balanceOf(address(this));
    ...
    function _farm() internal override {
        ...
        IFarmGOGO(farmContractAddress).stake(wantAmt);
        wantLockedTotal = IFarmGOGO(farmContractAddress).balanceOf(address(this));
        
    }

    function _unfarm(uint256 _wantAmt) internal override {
        if (_wantAmt == 0){
            IFarmGOGO(farmContractAddress).getReward(); // claim GOGO
            IFarmStable(0x7FCf0f2dcEc385FCCEd98240A8A4bEC8e91da7D1).getStableReward();  // claim USD
        } else {
            IFarmGOGO(farmContractAddress).withdraw(_wantAmt);
        }
    }

    function noTimeLockFunc1() external onlyAllowGov {
        // Only to be used to rescue delayed withdrawals
        IFarmGOGO(farmContractAddress).withdrawPending();
        uint256 wantAmt = IERC20(wantAddress).balanceOf(address(this));
        IERC20(wantAddress).safeTransfer(govAddress, wantAmt);
    }
    
    function _updateWantLockedTotal() internal {
        wantLockedTotal = IFarmGOGO(farmContractAddress).balanceOf(address(this));
    }
...
abstract contract StratX2 is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public constant controllerFeeUL = 1500; // 300; updated 2021-07-22
    ...
    uint256 public withdrawSafetyFactor = 9900;    // 100 = 1%
    ...
    function setWithdrawSafetyFactor(uint256 _withdrawSafetyFactor) public virtual onlyAllowGov {
        withdrawSafetyFactor = _withdrawSafetyFactor;
    }
