...
contract Test is ERC721, ERC721Enumerable, Ownable, ERC2981PerTokenRoyalties, ERC721Pausable {
    ...
    uint256 public MAX_NFT = 40;
    uint256 public MAX_BY_MINT = 20;
	uint256 public PRICE = 3 * 10**17;
    ...
    event CreateTestNFT(uint256 indexed id);
    ...
    function _mintAnElement(address _to) private {
        ...
        emit CreateTestNFT(id);
    }

    function burn(uint256 id) external {
        address _owner = ERC721.ownerOf(id);
        require(_owner == msg.sender, "Not authorized!");
        _burn(id);
    }

    function Initialize_Mint(address _to, uint256 _count) external onlyOwner saleIsOpen{
       for (uint256 i = 0; i < _count; i++) {
            _mintAnElement(_to);
        }
    }
