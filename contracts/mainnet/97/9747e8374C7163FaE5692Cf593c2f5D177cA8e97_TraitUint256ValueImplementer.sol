...
contract TraitUint256ValueImplementer {

    uint8           public immutable    implementerType = 4;    // uint256
    ...
    event updateTraitEvent(uint16 indexed _tokenId, uint256 _newData);
    ...
    function setData(uint16[] memory _tokenIds, uint256[] memory _values) public onlyAllowed {
        for (uint16 i = 0; i < _tokenIds.length; i++) {
            data[_tokenIds[i]] = _values[i];
            emit updateTraitEvent(_tokenIds[i], _values[i]);
        }
    }

    // update one
    function setValue(uint16 _tokenId, uint256 _value) public onlyAllowed {
        data[_tokenId] = _value;
        emit updateTraitEvent(_tokenId, _value);
    }

    function getValue(uint16 _tokenId) public view returns (uint256) {
         return data[_tokenId];
    }

    function getValues(uint16 _start, uint16 _len) public view returns (uint256[] memory) {
        uint256[] memory retval = new uint256[](_len);
