...
contract ERC7210xtowersm is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControlEnumerable, ERC721Burnable {
...
/**
 * @title ERC7210xtowers
 * ERC-721 Marketplace with tokens and royalties support
 */

contract ERC7210xtowers is ERC7210xtowersm, ReentrancyGuard {
    ...
    constructor(
        ...
        ERC7210xtowersm(_owner, name, symbol, _anyoneCanMint) 
    {
        admin = _admin;
        contract_owner = _owner;
        require(_commissionRate<=100, "ERC7210xtowers: Commission rate has to be between 0 and 100");
    ...
    function sell(uint256 tokenId, uint256 price, address payable wallet) public {
        ...
        // onlyOwner
        require(ownerOf(tokenId)==msg.sender, "ERC7210xtowers: Only owner can sell this item");

        // cannot set a price if auction is activated
        require(!auctions[tokenId].open, "ERC7210xtowers: Cannot sell an item which has an active auction");
    ...
    function buy(uint256 tokenId) public nonReentrant {
        ...
        // is on sale
        require(!auctions[tokenId].open && sellBidPrice[tokenId]>0, "ERC7210xtowers: The collectible is not for sale");

        // transfer ownership
        address owner = ownerOf(tokenId);

        require(msg.sender!=owner, "ERC7210xtowers: The seller cannot buy his own collectible");
    ...
    function createAuction(uint256 tokenId, uint _closingTime, address payable _beneficiary, uint256 _reservePrice) public {
        ...
        require(sellBidPrice[tokenId]==0, "ERC7210xtowers: The selected NFT is open for sale, cannot be auctioned");
        require(!auctions[tokenId].open, "ERC7210xtowers: The selected NFT already has an auction");
        require(ownerOf(tokenId)==msg.sender, "ERC7210xtowers: Only owner can auction this item");
    ...
    function bid(uint256 tokenId, uint256 bid_value) public nonReentrant {
        ...
        address owner = ownerOf(tokenId);
        require(msg.sender!=owner, "ERC7210xtowers: The owner cannot bid his own collectible");
