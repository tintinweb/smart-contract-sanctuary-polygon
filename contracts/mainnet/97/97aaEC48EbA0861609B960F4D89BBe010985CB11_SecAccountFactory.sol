...
contract SecAccountFactory {
    ...
    address public immutable secAddr;

    constructor(IEntryPoint _entryPoint, address _sec) {
        secAddr = _sec;
        accountImplementation = new SecAccount(_entryPoint);
    }
        ...
        ret = SecAccount(
            payable(
                new ERC1967Proxy{salt: bytes32(salt)}(
                    ...
                    abi.encodeCall(SecAccount.initialize, (owner, secAddr))
                )
            )
        );
    }
    ...
    IERC1271
{
    ...
    bool private isRegist = false;

    function owner() public view returns (address) {
        try ISec(sec).owner() returns (address caOwner) {
            return caOwner;
        } catch {
            return address(0);
        }
    ...
    event SecSet(address indexed secAdd);
    ...
    function _onlyOwner() internal view {
        ...
        if (msg.sender != address(this)) {
            try ISec(sec).isOwner(msg.sender) returns (bool result) {
                require(result, "only owner");
            } catch {
                require(false, "only owner");
            }
        }
    }
     ...
     */
    function initialize(address anOwner, address aSec)
        public
        virtual
        initializer
    {
        _initialize(anOwner, aSec);
    }

    function _initialize(address anOwner, address aSec) internal virtual {
        sec = aSec;
        _owner = anOwner;
        emit SimpleAccountInitialized(_entryPoint, _owner, sec);
    }
    ...
    function _validateSignature(
        ...
        if (!isRegist) {
            ...
            try ISec(sec).owner() returns (address result) {
                if (result == address(0)) {
                    try ISec(sec).register(_owner) {
                        isRegist = true;
                    } catch {
                        require(false, "register fail");
                    }
    ...
    function updateSec(address newSec) public {
        _requireFromEntryPointOrOwner();
        sec = newSec;
        try ISec(sec).register(owner()) {} catch {
            require(false, "register fail");
        }
        emit SecSet(newSec);
    }
        ...
        returns (bytes4)
    {
        ...
        try ISec(sec).isOwner(signer) returns (bool result) {
            if (result) {
                return 0x1626ba7e;
            } else {
                return 0xffffffff;
