...
contract Elysium is ERC721AEnumerable, Payable, DefaultOperatorFilterer {
    using Strings for uint256;

    uint256 private maxSalePlusOne = 12001;
    ...
    constructor() ERC721AEnumerable("Elysium", "ELS") {
        
        _currentBaseURI = 'ipfs://QmQHoQRaLqSwfq2XGqEcUrJVoSUHJUe8gXeHU7BXFDf6fV/';
     ...
     */
    function mintPublic(address to, uint256 quantity, bytes32[] calldata proof)
        external
        payable
        noBots
     ...
     * @param quantity Amount of tokens to mint.
     */
    function mintReserved(address[] calldata to, uint256[] calldata quantity) external onlyOwner {
        require(to.length == quantity.length, "Invalid Arrays!");

        for (uint i; i < to.length; i++) {
            require(_totalMinted() + quantity[i] < maxSalePlusOne, "Exceeds Available Tokens!");
