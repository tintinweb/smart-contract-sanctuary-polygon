    ...
    ERC721AUpgradeable
{
    ...
    // mapping to store the exemptions
    mapping(address => bool) public exemptions;

    // max limit per wallet
    uint256 public MAX_LIMIT = 100;

    /*///////////////////////////////////////////////////////////////
                    Constructor + initializer logic
    //////////////////////////////////////////////////////////////*/

    modifier onlyUnderLimit(address to, uint256 amount) {
        if (!exemptions[to] && balanceOf(to) + amount > MAX_LIMIT) {
    ...
    function setBaseURI(uint256 _index, string calldata newBaseUri) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 batchId = getBatchIdAtIndex(_index);
        _setBaseURI(batchId, newBaseUri);
    }
    ...
    function setExemption(
        ...
        exemptions[_address] = state;
    }

    function setMaxWalletLimit(
        uint256 _limit
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_limit > 0, "exceed limit");
        MAX_LIMIT = _limit;
    }
    ...
    function _beforeClaim(
        ...
        bytes memory
    ) internal view override onlyUnderLimit(_msgSender(), _quantity) {
