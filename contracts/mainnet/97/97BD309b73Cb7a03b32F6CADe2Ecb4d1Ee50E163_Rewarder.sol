...
/// @title Calculates and pays out rewards
/// @author bitbeckers
/// @notice MVP release
/// @custom:source This is an experimental contract.
    ...
    UUPSUpgradeable
{
    ...
    uint256 private _totalAllocationPoints;

    CountersUpgradeable.Counter public sagepathUsers;
    CountersUpgradeable.Counter public low;
    CountersUpgradeable.Counter public mid;
    CountersUpgradeable.Counter public high;
    CountersUpgradeable.Counter public stoic;
    ...
    event AddressUpdated(string config, address newAddress);
    ...
    function updateToken(address newRewardsToken) external onlyRole(CONFIG_ROLE) {
        token = newRewardsToken;
        emit AddressUpdated("RewardsToken", newRewardsToken);
    }

    function updateStoa(address newStoa) external onlyRole(CONFIG_ROLE) {
        stoa = newStoa;
        emit AddressUpdated("STOA", newStoa);
    }
...
interface IRewarder {
    ...
    // Accomplishment
    function getAccomplishmentReward(address user) external view returns (uint256 totalAvailable, uint256 rewardAmount);
