...
import {DailyReturnsWithNFTPack} from "./DailyReturnsWithNFTPack.sol";
import {ADMIN_ROLE} from "../../utils/constants.sol";

/**
 * @title DailyReturnsFactoryWithNFTPack
 * @author Garen Vartanian (@cryptokiddies)
 * @notice Create new {DailyReturnsWithNFTPack} for unique `gamePairHash` and `gameLength` with `gameType` of `2`.
 * @dev The factory is in charge of minting new game instances for a given pair and game length (block duration). Already existing sets will revert.
 ...
 */
contract DailyReturnsFactoryWithNFTPack is RegistrySatellite {
    uint256 constant GAME_TYPE = 2;
    // hash of gameId abi encoded
    ...
    function getGameId(bytes32 gamePairHash, uint256 gameLength)
        ...
        returns (bytes32)
    {
        return keccak256(abi.encode(gamePairHash, gameLength, GAME_TYPE));
    }

    /**
     * @notice Predict new {DailyReturnsWithNFTPack} address based on instantiation arguments.
    ...
    /**
     * @notice New {DailyReturnsWithNFTPack} creation.
    ...
    function createGame(
        ...
        address newGameAddress = address(
            new DailyReturnsWithNFTPack{salt: gameId}(
                gameAdmin,
                registryContractAddress,
                gamePairHash,
                gameLength,
