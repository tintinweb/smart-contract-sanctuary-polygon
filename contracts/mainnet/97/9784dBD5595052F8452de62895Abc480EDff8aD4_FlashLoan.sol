...
contract FlashLoan is Ownable {
    ...
    event ReceivedBorrowedToken(address borrowedToken, uint256 value, uint256 owned);
    event SwappedForToken1(address token1, uint256 value);
    event SwappedForToken2(address token2, uint256 value);
    event SwappedBorrowedToken(address borrowedToken, uint256 value);
    ...
    function executeOperation(
        ...
        emit ReceivedBorrowedToken(borrowedToken, borrowedAmount, borrowedAmount + premium);
        ...
        emit SwappedForToken1(token1, token1Amount);
        uint256 token2Amount = swapExactInputSingle(
            token1,
            token2,
            token1Amount
        );
        emit SwappedForToken2(token2, token2Amount);
        uint256 arbitragedAmount = swapExactInputSingle(
            token2,
            borrowedToken,
            token2Amount
        );
        emit SwappedBorrowedToken(borrowedToken, arbitragedAmount);
