...
contract NftMinter is ERC721, AccessControlEnumerable, ERC2981 {
        ...
        payable
    {
        require(
            ...
            "Caller has already minted a whitelisted token."
        );
        require(
            onWhitelist(_msgSender(), proof),
            "Caller has an invalid proof."
        );
        require(msg.value == whitelistPricePerNft, "Invalid payment amount.");
        require(
            whitelistMintStartTime <= block.timestamp,
            "Whitelist minting is not open yet."
        );
        uint256 startTokenId = _tokenIds.current();
        _mintCommon(1);
    ...
    function owner() public view virtual returns (address) {
        require(
            ...
            "owner() only works if there is one admin."
        );
