...
contract WombatDao is ERC20Upgradeable, OwnableUpgradeable {
    ...
    uint public fertilizationStartDayTime;
    ...
    event FertilizationStartDaySet(uint timestamp);
    ...
    function setFertilizationStartDay(uint timestamp) external onlyOwner {
        require(!fertilizationStartDayReached(), "WombatDao: Fertilization day already reached!");
        fertilizationStartDayTime = timestamp;
        emit FertilizationStartDaySet(timestamp);
    }

    function fertilizationStartDayReached() public view returns (bool) {
        return fertilizationStartDayTime != 0 && block.timestamp > fertilizationStartDayTime;
    }

    function collectFromFertilization() external {
        uint reward = _calculateTotalRewardForUser(userFertilizationInfo[msg.sender]);
        uint rewardToCollect = reward - userFertilizationInfo[msg.sender].collectedReward;
        require(rewardToCollect > 0, "WombatDao: Nothing to collect!");

        userFertilizationInfo[msg.sender].collectedReward += rewardToCollect;
        _mint(msg.sender, rewardToCollect);
     ...
     */
    function fertilize(uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        _beforeFertilizeChecks(value);
        _wombatPoo.permit(msg.sender, address(this), value, deadline, v, r, s);
        _fertilize(value);
    }
    ...
    function _beforeFertilizeChecks(uint value) private view {
    ...
    function _fertilize(uint value) private {
        ...
        if (userInfo.amount > 0) {
            uint currentFertilizationStartDay = _getDaysSinceFertilizationStarted(userInfo.startTime);
            ...
            pendingReward = _calculateTotalRewardForUser(userInfo);
            ...
            if (currentDay > currentFertilizationStartDay) {
                ...
                zeroDayReward = _calculateRewardForDaySinceFertilizationStarted(currentDay, usersRemainingWpooToday);
            }
    ...
    // Days passed since fertilizationStartDayTime
    function getDaysSinceFertilizationStarted() public view returns (uint) {
        return _getDaysSinceFertilizationStarted(block.timestamp);
    }
    ...
    function getRemainingWpooAt(address user, uint timestamp) external view returns (uint) {
        ...
        uint userFertilizationStartDay = _getDaysSinceFertilizationStarted(userInfo.startTime);
        uint currentDay = _getDaysSinceFertilizationStarted(timestamp);
    ...
    function calculateRewardToCollect(address owner) external view returns (uint) {
        UserInfo storage user = userFertilizationInfo[owner];
        return _calculateTotalRewardForUser(user) - user.collectedReward;
    }
    ...
    function _calculateTotalRewardForUser(UserInfo storage user) private view returns (uint) {
        if (user.startTime == 0) {
            return 0;
        }
        uint startDay = _getDaysSinceFertilizationStarted(user.startTime);
        uint todayDay = _getDaysSinceFertilizationStarted(block.timestamp);
        ...
        for (uint i = startDay + 1; i <= Math.min(todayDay - 1, startDay + numberOfFertilizationDays); i++) {
            ...
            reward += _calculateRewardForDaySinceFertilizationStarted(i, userWpooForDay);
        }
    ...
    // Days passed since fertilizationStartDayTime for timestamp
    function _getDaysSinceFertilizationStarted(uint timestamp) private view returns (uint) {
        require(fertilizationStartDayReached(), "WombatDao: Fertilization day not reached yet");
        return (timestamp - fertilizationStartDayTime) / SECONDS_IN_DAY;
    }

    function _calculateRewardForDaySinceFertilizationStarted(uint daySinceFertilizationStarted, uint userWpooForDay) private view returns (uint) {
