...
contract fPRXY is Ownable, ERC20Burnable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public treasury = 0xf76FD435Bab7392B52B4123f0A16D8310EfDF8E0;
    ...
    //index1: 1000-10000
    //index2: 100000+
    uint256[3] private apy = [300, 500, 700];
    uint256 public userCount;
    uint256 public unstakeLockingPeriod = 1 days;

    bool public stakingStatus;

    address public V2_ROUTER = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    address public USDC = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    address public WBTC = 0x1BFD67037B42Cf73acF2047067bd4F2C47D9BfD6;
    address public PRXY = 0xab3D689C22a2Bb821f50A4Ff0F21A7980dCB8591;
    address public BTCpx = 0x9C32185b81766a051E08dE671207b34466DD1021;
    ...
    modifier updateReward(address account) {
        ...
        if (block.timestamp >= stakingStartDate) {
            ...
            for (uint256 i = 0; i < _stakeId; i++) {
                ...
                uint256 _apyIndex = _stakeAmount <= 1000e18
                    ? 0
                    : (_stakeAmount > 10000e18 ? 2 : 1);
                if (account != address(0)) {
                    rewards[account] = earned(account, _apyIndex, _stakeAmount);
    ...
    constructor(uint256 _stakingStartDate) ERC20("Farm PRXY", "fPRXY") {
    ...
    function updateApyValue(uint256[3] memory _apy) external onlyOwner {
        apy = _apy;
    }
        ...
        _rewardEarned = _stakeAmount
            .mul(
                rewardPerToken(_apyIndex).sub(
                    userRewardPerTokenPaidById[account][_stakeAmount]
                )
            )
            .div(1e18)
            .add(rewards[account]);
        ...
        returns (uint256 _rewardEarned)
    {
        for (uint256 i = 0; i < _stakeId.length; i++) {
            uint256 _stakeAmount = userStakeById[account][_stakeId[i]];
            uint256 _apyIndex = _stakeAmount <= 1000e18
                ? 0
                : (_stakeAmount > 10000e18 ? 2 : 1);
            _rewardEarned += earned(account, _apyIndex, _stakeAmount);
        }
    }
    ...
    function stake(uint256 amount) external updateReward(_msgSender()) {
        ...
        if (!isUserExisting(_msgSender())) {
            userList[userCount] = _msgSender();
            userCount++;
            isExisting[_msgSender()] = true;
        }
        IERC20(PRXY).safeTransferFrom(_msgSender(), treasury, amount);
        _mint(_msgSender(), amount);
        userStake[_msgSender()] += amount;
        uint256 _stakeId = userTotalStakeId[_msgSender()];
        userStakeById[_msgSender()][_stakeId] = amount;
        userStakeTimeById[_msgSender()][_stakeId] = block.timestamp;
        userTotalStakeId[_msgSender()]++;
        emit Staked(
            _msgSender(),
    ...
    function unstake(uint256 _stakeId) public updateReward(_msgSender()) {
        require(
            block.timestamp >
        ...
        IERC20(PRXY).safeTransferFrom(treasury, _msgSender(), _stakeAmount);
        emit Unstaked(
            _msgSender(),
            userStake[_msgSender()],
            _stakeAmount,
    ...
    function getReward(uint256[] memory _stakeId)
        public
        updateReward(_msgSender())
    {
        uint256 reward;
        for (uint256 i = 0; i < _stakeId.length; i++) {
            uint256 _stakeAmount = userStakeById[_msgSender()][_stakeId[i]];
            uint256 _apyIndex = _stakeAmount < 1000e18
                ? 0
                : (_stakeAmount > 10000e18 ? 2 : 1);
            reward = earned(_msgSender(), _apyIndex, _stakeAmount);
        }
        if (reward > 0) {
            rewards[_msgSender()] = 0;
            //divide by 1e8 to normalize the multiplication in reward rate)
            IERC20(BTCpx).safeTransferFrom(btcpxTreasury, _msgSender(), reward.div(1e8));
    ...
    {
        return (apy[0].mul(10**4), apy[1].mul(10**4), apy[2].mul(10**4));
    }
    ...
    function rewardTokenDecimals() external view returns (uint8) {
        return IERC20Metadata(BTCpx).decimals();
    }

    function stakeTokenDecimals() external view returns (uint8) {
        return IERC20Metadata(PRXY).decimals();
    }
