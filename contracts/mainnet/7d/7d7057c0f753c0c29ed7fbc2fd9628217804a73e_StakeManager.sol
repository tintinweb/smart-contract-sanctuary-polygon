...
contract StakeManager is IStakeManager, StakeBase {
    ...
    */
    ...
    //** ==== AUXILIARY === */
    function stakeTax(uint256 id, uint256 percent) external override onlyExternalCaller {
        require(stakeData[id].status != StakeStatus.Withdrawn, "TAX: Can't tax withdrawn stake");

        removeFromGlobalTotals(
            uint256(stakeData[id].amount) * 1e12,
            uint256(stakeData[id].shares) * 1e12
        );

        stakeData[id].amount = (uint256(stakeData[id].amount) -
            (uint256(stakeData[id].amount) * percent) /
            100)
            .toUint64();
        stakeData[id].shares = (uint256(stakeData[id].shares) -
            (uint256(stakeData[id].shares) * percent) /
            100)
            .toUint64();
