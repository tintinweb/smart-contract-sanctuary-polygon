...
contract PoolInitializable is AccessControlEnumerable, ReentrancyGuard {
    ...
    event NewRewardHolder(address rewardHolder);
     ...
     */
    function emergencyRewardWithdraw(uint256 _amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function updateRewardPerBlock(uint256 _rewardPerBlock) external onlyRole(WHITELISTED_ROLE) {
        _updatePool();
        rewardPerBlock = _rewardPerBlock;
     ...
     * @param _rewardPerBlock: the reward per block
     */
    function updateRewardHolder(address _rewardHolder) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(rewardHolder != _rewardHolder, "Reward holder must be different");
        rewardHolder = _rewardHolder;
        emit NewRewardHolder(_rewardHolder);
    }
