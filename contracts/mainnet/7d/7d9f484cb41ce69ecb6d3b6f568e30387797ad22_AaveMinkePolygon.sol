...
abstract contract ZapBaseV2_1 is Ownable {
    ...
    function affilliateWithdraw(address[] calldata tokens) external {
        uint256 tokenBal;
        for (uint256 i = 0; i < tokens.length; i++) {
            tokenBal = affiliateBalance[_msgSender()][tokens[i]];
            affiliateBalance[_msgSender()][tokens[i]] = 0;
            totalAffiliateBalance[tokens[i]] =
                totalAffiliateBalance[tokens[i]] -
                tokenBal;

            if (tokens[i] == ETHAddress) {
                Address.sendValue(payable(_msgSender()), tokenBal);
            } else {
                IERC20(tokens[i]).safeTransfer(_msgSender(), tokenBal);
            }
        }
    }
    ...
    function _subtractGoodwill(
        ...
        bool whitelisted = feeWhitelist[_msgSender()];
    ...
    receive() external payable {
        require(_msgSender() != tx.origin, "Do not send ETH directly");
    }
}
...
contract AaveMinkePolygon is ERC2771Context, ZapInBaseV3_1, ZapOutBaseV3_1 {
    using SafeERC20 for IERC20;
    ...
    function ZapIn(
        ...
        emit zapIn(_msgSender(), aToken, aTokensRec);
    }
    ...
    function enterAave(
        ...
        uint256 initialBalance = IERC20(aToken).balanceOf(_msgSender());

        _approveToken(underlyingToken, address(lendingPool), underlyingAmount);

        lendingPool.deposit(underlyingToken, underlyingAmount, _msgSender(), 151);

        aTokensRec = IERC20(aToken).balanceOf(_msgSender()) - initialBalance;
    ...
    function ZapOut(
        ...
        if (toToken == address(0)) {
            ...
            payable(_msgSender()).transfer(tokensRec - totalGoodwillPortion);
        } else {
            totalGoodwillPortion = _subtractGoodwill(
                toToken,
                tokensRec,
        ...
        emit zapOut(_msgSender(), toToken, tokensRec);
    }
