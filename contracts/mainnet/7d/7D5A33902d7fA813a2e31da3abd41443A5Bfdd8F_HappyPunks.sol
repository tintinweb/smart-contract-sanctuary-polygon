    ...
    ReentrancyGuard
{
    ...
    uint8 public constant MAX_MINTS_PER_TX = 5;
    ...
    ) ERC721("Happy Punks", "HPUNK") {
        if (feeRecipient_ == address(0)) {
            revert("HP: Zero address");
        }
        _setDefaultRoyalty(feeRecipient_, 690);
        saleIsActive = true;
        baseURI = baseURI_;
    ...
    function airdropMint(address[] memory users) external onlyOwner {
    ...
    function updateUriBatch(
        uint256[] calldata tokenIds,
        string[] calldata uris
    ) external onlyOwner {
        if (tokenIds.length != uris.length) {
            revert("HP: Invalid array lengths");
        }
    ...
    function updateSalePrice(uint256 price) external onlyOwner{
        PRICE = price;
    }
    ...
    function userTokens(
        ...
        if (user == address(0)) revert("HP: Zero address");
