...
contract Mitico is ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function initialize() public initializer {
    ...
    setBaseURI('https://api.mitico.io/token?id=');
  ...
  function mintBatch(
    ...
    require(_name.length == _price.length, 'name and price must be the same length');
    require(
      _royalty >= 0 && _royalty <= maxAllowedRoyalties,
      'ERC721: Very high royalty, you have to set a lower royalty'
    );
    ...
    uint256[] memory itemsId = new uint256[](_price.length);
    for (uint256 i = 0; i < _price.length; i++) {
      require(_price[i] > 0, 'prices must be greater than 0');
      _tokenIds.increment();
