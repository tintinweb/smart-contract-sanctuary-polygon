...
contract LimitQuest is ReentrancyGuard, LuxOnData, LuxOnBlacklist{
    using SafeMath for uint256;
    ...
    uint256 public participationLimit = 1000;
    ...
    ) LuxOnData(dataAddress) LuxOnBlacklist(luxonAuthority) {
    ...
    function enterQuest(uint256 _questNo, uint256[] calldata _tokenIds) public nonReentrant isLive isBlacklist(msg.sender){
        LimitQuestData.Quest memory qD = LimitQuestData(getDataAddress("LimitQuestData")).getQuest(_questNo);
    ...
    function clearQuest(uint256 _questNo) public nonReentrant isLive isBlacklist(msg.sender){
        LimitQuestData.Quest memory qD = LimitQuestData(getDataAddress("LimitQuestData")).getQuest(_questNo);
    ...
    function cancelQuest(uint256 _questNo) public nonReentrant isBlacklist(msg.sender){
        LimitQuestData.Quest memory qD = LimitQuestData(getDataAddress("LimitQuestData")).getQuest(_questNo);
        uint256 questGroup = qD.mainQuestGroup;
        ...
        for (uint i = 0; i < burnInfos.length; i++) {
            BurnWaitInfo memory burnInfo = burnInfos[i];
            _transferFromByType(questStorage, msg.sender, burnInfo.tokenId, burnInfo.amount, burnInfo.conditionType);
        }
