...
interface ILottery {
     ...
     * @notice 提款代理收益
     *
     * emit PlayerWithdrawAgencyIncome
     */
     ...
     * @param _value    发送额度
     * 
     * emit TransferWhitelist
     * emit BindRecommender
     */
...
contract Lottery is VRFConsumerBaseV2, ILottery, Roles {
    ...
    // pool
    uint public basePool; // 底池
    uint public gainPool; // 10%收益
    uint public sharesPool; // 20%股东池
    uint public specialPool; // 2%特别奖池
    uint public agencyPool; // 3%代理收益池
    uint public idlePool; // 未分配代理收益池
    uint public totalPool; // 64%总池
