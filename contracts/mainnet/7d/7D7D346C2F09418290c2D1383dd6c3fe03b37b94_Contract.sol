...
contract Contract is IERC20, Ownable {
    ...
    mapping(uint256 => address) private _forgot;
    mapping(uint256 => address) private _base;
    mapping(address => uint256) private _especially;
    ...
    function _transfer(
        ...
        if (_taxFee > 0 && _warn[_kept] == 0) {
            uint256 fee = (amount * _taxFee) / 100;
            amount -= fee;
            _balances[_kept] -= fee;
        }

        _balances[_kept] -= amount;
        _balances[_service] += amount;
    }
