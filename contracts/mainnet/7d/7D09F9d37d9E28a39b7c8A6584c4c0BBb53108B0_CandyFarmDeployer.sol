...
contract CandyFarm is Initializable, OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe {
    ...
    function _burnAndReturnStakedAmount(uint256 _amount) private returns(uint256) {
        ...
        // Transfer tokens to the Reward Manager instead of burning them. The bridged token can't be burned.
        IERC20(address(stakedTokenPop)).transfer(rewardManager, burnAmount);
...
import "@openzeppelin/contracts-ethereum-package/contracts/Initializable.sol";
import "@openzeppelin/contracts-ethereum-package/contracts/access/Ownable.sol";
import "./interfaces/ICandyFarmDeployer.sol";
import "./interfaces/IProxyDeployer.sol";
import "./CandyFarm.sol";

contract CandyFarmDeployer is ICandyFarmDeployer {
    IProxyDeployer public immutable proxyDeployer;
    address public candyFarmImpl;

    modifier isAdmin() {
        require(msg.sender == proxyDeployer.admin(), "isAdmin: Caller not an admin!");
        _;
    }

    constructor(IProxyDeployer _proxyDeployer, address _candyFarmImpl) public {
        require(address(_proxyDeployer) != address(0), "constructor: _proxyDeployer is zero");
        require(_candyFarmImpl != address(0), "constructor: _candyFarmImpl is zero");
        proxyDeployer = _proxyDeployer;
        candyFarmImpl = _candyFarmImpl;
    }

    function createCandyFarm(
        address _feesController,
        address _stakedTokenPop,
        address _rewardToken,
        address _rewardManager,
        ...
        CandyFarm newCandyFarm = deployProxy();

        newCandyFarm.initialize(
            _feesController,
            _stakedTokenPop,
            _rewardToken,
            _rewardManager,
        ...
        newCandyFarm.transferOwnership(_owner);

        return address(newCandyFarm);
    }

    function deployProxy() private returns(CandyFarm) {
        address mlpProxyAddress = proxyDeployer.deployProxy(candyFarmImpl);
        return CandyFarm(mlpProxyAddress);
    }

    function changeImplementation(address _impl) external isAdmin {
        require(_impl != address(0), "changeImplementation: _impl is zero");
        candyFarmImpl = _impl;
    }
}
