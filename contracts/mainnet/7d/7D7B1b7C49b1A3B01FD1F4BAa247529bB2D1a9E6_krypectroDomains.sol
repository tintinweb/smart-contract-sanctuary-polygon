...
contract krypectroDomains is ERC721URIStorage, Pausable{
  //Use _tokenIds to keep track of unique identifiers of NFTS. This is a number private _tokenIds that is automatically initialized to 0 when we declare it.
  ...
  string svgPartOne = '<svg  xmlns="http://www.w3.org/2000/svg" width="270" height="270" fill="none"><path fill="url(#B)" d="M0 0h270v270H0z"/><defs><filter id="A" color-interpolation-filters="sRGB" filterUnits="userSpaceOnUse" height="270" width="270"><feDropShadow dx="0" dy="1" stdDeviation="2" flood-opacity=".225" width="200%" height="200%"/></filter></defs><circle cx="50" cy="130" r="30" fill="url(#bubble)"><animate attributeName="cx" attributeType="XML" from="0"  to="270" begin="0s" dur="6.5s" fill="remove" repeatCount="indefinite"/></circle><radialGradient id="bubble"><stop offset="0%" stop-color="rgba(255,255,255,0)" ></stop><stop offset="80%" stop-color="rgba(255,255,255,0.1)" ></stop><stop offset="100%" stop-color="rgba(255,255,255,0.42)"></stop></radialGradient><path d="M72.863 42.949c-.668-.387-1.426-.59-2.197-.59s-1.529.204-2.197.59l-10.081 6.032-6.85 3.934-10.081 6.032c-.668.387-1.426.59-2.197.59s-1.529-.204-2.197-.59l-8.013-4.721a4.52 4.52 0 0 1-1.589-1.616c-.384-.665-.594-1.418-.608-2.187v-9.31c-.013-.775.185-1.538.572-2.208a4.25 4.25 0 0 1 1.625-1.595l7.884-4.59c.668-.387 1.426-.59 2.197-.59s1.529.204 2.197.59l7.884 4.59a4.52 4.52 0 0 1 1.589 1.616c.384.665.594 1.418.608 2.187v6.032l6.85-4.065v-6.032c.013-.775-.185-1.538-.572-2.208a4.25 4.25 0 0 0-1.625-1.595L41.456 24.59c-.668-.387-1.426-.59-2.197-.59s-1.529.204-2.197.59l-14.864 8.655a4.25 4.25 0 0 0-1.625 1.595c-.387.67-.585 1.434-.572 2.208v17.441c-.013.775.185 1.538.572 2.208a4.25 4.25 0 0 0 1.625 1.595l14.864 8.655c.668.387 1.426.59 2.197.59s1.529-.204 2.197-.59l10.081-5.901 6.85-4.065 10.081-5.901c.668-.387 1.426-.59 2.197-.59s1.529.204 2.197.59l7.884 4.59a4.52 4.52 0 0 1 1.589 1.616c.384.665.594 1.418.608 2.187v9.311c.013.775-.185 1.538-.572 2.208a4.25 4.25 0 0 1-1.625 1.595l-7.884 4.721c-.668.387-1.426.59-2.197.59s-1.529-.204-2.197-.59l-7.884-4.59a4.52 4.52 0 0 1-1.589-1.616c-.385-.665-.594-1.418-.608-2.187v-6.032l-6.85 4.065v6.032c-.013.775.185 1.538.572 2.208a4.25 4.25 0 0 0 1.625 1.595l14.864 8.655c.668.387 1.426.59 2.197.59s1.529-.204 2.197-.59l14.864-8.655c.657-.394 1.204-.95 1.589-1.616s.594-1.418.609-2.187V55.538c.013-.775-.185-1.538-.572-2.208a4.25 4.25 0 0 0-1.625-1.595l-14.993-8.786z" fill="url(#g1)"/><defs><linearGradient id="B" x1="0" y1="0" x2="270" y2="270" gradientUnits="userSpaceOnUse"><stop stop-color="#ec77ab"/><stop offset="1" stop-color="#7873f5" stop-opacity=".99"/></linearGradient></defs><linearGradient id="g1" x1="0%" y1="0%" x2="100%" y2="0"><stop offset="50%" stop-color="pink"></stop><stop offset="100%" stop-color="aqua"></stop></linearGradient><text x="32.5" y="231" font-size="20" fill="url(#g1)" filter="url(#A)" font-family="Plus Jakarta Sans,DejaVu Sans,Noto Color Emoji,Apple Color Emoji,sans-serif" font-weight="bold">';
  ...
  address[200] private whiteUser;
  uint256 whiteUserNum;
  ...
  constructor(string memory _tld) payable ERC721("krypectro Name Service", "create") {
    ...
    tld = _tld;
    console.log("Hello and welcome to the world of Krypectro domain names!");
  }


  // A registration function that adds their names to the map
   ...
   //   require(domains[name] == address(0));
      ...
      uint256 thisWhitePrice=whitePrice(name);
      uint256 _price = price(name);
      //Check whether the IP address is whitelisted
      if(whitelist[msg.sender]==true){
        //It's important to limit the number of times 
        require(msg.value >= thisWhitePrice, "This whiteAddress not enough Matic paid");
        removeWhitelist(msg.sender);
      }else{
        //Check to see if enough Matic was paid in this transaction
      ...
      string memory json = Base64.encode(
        abi.encodePacked(
          '{"name": "',
          _name,
          '", "description": "Decentralized domain names on Ploygon created by Krypectro", "image": "data:image/svg+xml;base64,',
      ...
      _tokenId.increment(); //Id+1
      getDomainId[name]=newRecordId;
  }


  //The address in the whitelist is added
  function addUser(address _user) public onlyOwner{
    if(block.timestamp<1668406515){
    require(whiteUserNum < 200, "number of users is limited to 200");
    whiteUser[whiteUserNum] = _user;
    whiteUserNum++;
    addWhitelist(_user);
    }else{
  ...
  function addWhitelist(address _newEntry) internal onlyOwner {
    require(whitelist[_newEntry]==false, "Your address has been whitelisted");
    if(whitelist[_newEntry]!=true){
        whitelist[_newEntry] = true;
    }
    else{
      console.log("This is a whiteAddress,",_newEntry);
    }
  }

  //Example Remove addresses from the whitelist
  function removeWhitelist(address _newEntry) public onlyOwner {
  ...
  function checkWhiteAddress(address _newEntry)public view returns(bool){
    if(whitelist[_newEntry]==false)
    {
      console.log("Your address not a whitelist");
      return false;
    } else{
      console.log("Your address has been whitelisted");
      return true;
    }
  }
  ...
  function getDomainAddress(string memory name) public view returns (address) {
      //Check that the owner is the transaction sender
      return domains[name];
  }
  ...
  function getNameToTokenId(string calldata name) public view returns(uint){
  ...
  function price(string calldata name) public pure returns(uint) {
    ...
    if (len == 3) {
      return 20 * 10**17; //20matic. 
    } else if (len == 4) {
      return 10 * 10**17; //10matic
    } else {
      return 1 * 10**17; //1matic
    }
    }
  ...
  //This function casts domain name prices for whitelists
  function whitePrice(string calldata name) public pure returns(uint) {
    uint len = StringUtils.strlen(name);
    require(len > 0 && 3<=len && len<=10);
   if (len == 3) {
      return 0 * 10**17; //0matic. 
    } else if (len == 4) {
      return 0 * 10**17; 
    } else {
      return 0 * 10**17; 
  ...
  function approvedFor(uint256 _ownerTokenId) public view returns (address) {     
      return tokenApprovals[_ownerTokenId];  
  }
  ...
  function userTokenURI(uint256 userTokenId) public view returns (string memory) {
    return tokenURI(userTokenId);   
  }

  function nftTransfer(address _from,address _to,uint256 _userTokenId)external payable{
    require(ERC721.ownerOf(_userTokenId) == _from, "ERC721: transfer from incorrect owner");
    require(_to != address(0), "ERC721: transfer to the zero address");
    safeTransferFrom(_from,_to,_userTokenId);
  }

  //The following is the withdrawal function
  modifier onlyOwner() {
  require(isOwner());
  _;
  }
