...
library REXStakeDAOTokenHelper {
    ...
    function upgrade(REXStakeDAOTokenStorage.sdam3CRVrex storage self, uint256 amount) public  {
      // Pre-condition: at this point
      // 1. `amount` sdam3CRV has be transfered to this contract
      //     and is awaitng deposit to minichef.
      // 2. `amount` sdam3CRVrex has be transfered to msg.sender who made the
      //     upgrade call

      // Must figure out:
      // - What was their existing IDA share allocation?
      // - What is their new share allocation (== sdam3CRVrex.balanceOf(msg.sender))

      // Make the deposit to StakeDAO MasterChef
      // Havest and distribute SUSHI rewards if there's any pending
      if (self.masterChef.pendingSdt(self.pid, address(this)) > 0) {
        harvest(self);
      }
      self.masterChef.deposit(self.pid, amount);

      amount = amount / 1e9;
      uint128 userShares = uint128(amount) * 98 / 100;
      uint128 daoShares = uint128(amount) - userShares;
      console.log("usershares", uint(userShares));
      console.log("daoshares", uint(daoShares));
      console.log("amount", amount);

      // Get the users existing IDA share allocation, we'll add to it
      (bool exist,
       bool approved,
       uint128 units,
       uint256 pendingDistribution) = _getIDAShares(self, 0, self.sdtx, msg.sender);

       // Add units to their shares
       userShares = units + userShares;
       console.log("userShares", uint(userShares));
      _updateSubscription(self, 0, msg.sender, userShares, self.sdtx);

      // Repeat the same for the DAO shares
      (exist,
       approved,
       units,
       pendingDistribution) = _getIDAShares(self, 0, self.sdtx, self.owner);
       console.log("daoshares", uint(units + daoShares));
      _updateSubscription(self, 0, self.owner, units + daoShares, self.sdtx);
    ...
    function downgrade(REXStakeDAOTokenStorage.sdam3CRVrex storage self, uint256 amount) public  {
      // Pre-condition: at this point
      // 1. `amount` sdam3CRVrex has NOT been burned yet
      // 2. `amount` sdam3CRV has NOT been transfered to msg.sender who made the
      //     upgrade call

      require(amount <= IERC20(address(this)).balanceOf(msg.sender), "tooMuch");

      // Withdraw amount of sdam3CRV from masterChef
      self.masterChef.withdraw(self.pid, amount);
      ...
      (exist,
       ...
       units,
       pendingDistribution) = _getIDAShares(self, 0, self.sdtx, self.owner);
       console.log("owner units", units);
      _updateSubscription(self, 0, self.owner, units - daoShares, self.sdtx);

    }

    function distribute(REXStakeDAOTokenStorage.sdam3CRVrex storage self, uint32 index, ISuperToken token) public {
      console.log("ida distribute", uint128(token.balanceOf(address(this))));
      console.log("ida token", address(token));
    ...
    function harvest(REXStakeDAOTokenStorage.sdam3CRVrex storage self) public {
      ...
      // Try to harvest from minichef, catch and continue iff there's no sdt in masterChef
      try self.masterChef.withdraw(self.pid, 0) {
      } catch Error(string memory reason) {
        console.log("SDT Claim failed");
      ...
      if (sdts > 0) {
        console.log("upgrading sdt", sdts);
      ...
      // console.log("units", uint(units));
      // console.log("pendingDistribution", uint(pendingDistribution));
    ...
    function _idaDistribute(REXStakeDAOTokenStorage.sdam3CRVrex storage self, uint32 index, uint128 distAmount, ISuperToken distToken) internal {
      ...
      (bool exist, uint128 indexValue, uint128 totalUnitsApproved, uint128 totalUnitsPending) = self.ida.getIndex(
                                                                    ...
                                                                    address(this),
                                                                    index);
      console.log("distributed to uints total", uint(totalUnitsApproved + totalUnitsPending));
    ...
    function _getIDAShares(REXStakeDAOTokenStorage.sdam3CRVrex storage self, uint32 index, ISuperToken idaToken, address streamer) public view returns (
                  bool exist,
                  bool approved,
                  uint128 units,
