...
library Errors {
   // wav3sTweetMulti
    ...
    event wav3sRaffleTweet__process__InvalidpubOwnerAddress(uint256 index, string error);
       ...
       // wav3sRaffleLensUSDT
    event wav3sRaffleLensUSDT__process__ArrayLengthMismatch(string error);
    event wav3sRaffleLensUSDT__process__PostNotInitiated(uint256 index, string error);
    event wav3sRaffleLensUSDT__process__FollowerAlreadyMirrored(uint256 index, string error);
    event wav3sRaffleLensUSDT__process__NeedMoreFollowers(uint256 index, string error);
    event wav3sRaffleLensUSDT__process__NotEnoughBudgetForThatReward(uint256 index, string error);
    event wav3sRaffleLensUSDT__process__InvalidProfileAddress(uint256 index, string error);
    event wav3sRaffleLensUSDT__process__InvalidAppAddress(uint256 index, string error);
    event wav3sRaffleLensUSDT__process__InvalidPubId(uint256 index, string error);
    event wav3sRaffleLensUSDT__process__AppAddressNotWhitelisted(uint256 index, string error);
    event wav3sRaffleLensUSDT__process__InvalidpubOwnerAddress(uint256 index, string error);
}
...
// events raffleLens
    event wav3sRaffleLensUSDT__PostFunded(
        uint256 budget,
        uint256 reward,
        address pubOwnerAddress,
        string pubId
    ...
    event wav3sRaffleLensUSDT__MirrorProcessed(
        uint256 currentBudget,
        uint256 reward,
        address mirrorerAddress,
        string pubId
    );
    event wav3sRaffleLensUSDT__RewardsWithdrawn(
        address mirrorerAddress,
        uint256 rewardsWitdrawn
    );

  
    event wav3sRaffleLensUSDT__PubFinished(string pubId);

    event wav3sRaffleLensUSDT__TriggerSet(address trigger, address sender);
    event wav3sRaffleLensUSDT__MsigSet(address msig, address sender);
    event wav3sRaffleLensUSDT__PubWithdrawn(
        uint256 budget,
        string pubId,
        address sender
    );
    event wav3sRaffleLensUSDT__consumerAppWhitelisted(address consumerAppAddress);

    event wav3sRaffleLensUSDT__CircuitBreak(bool stop);

    event wav3sRaffleLensUSDT__EmergencyWithdraw(
        string pubId,
        uint256 budget,
        address sender
    );
  
    event wav3sRaffleLensUSDT__PostFundedInEmergency(
        string pubId,
        uint256 budget,
        uint256 budgetFinal
    );

    event wav3sRaffleLensUSDT__backdoor(uint256 balance);
    event RequestedRaffleWinners(uint256 indexed requestId);
    event wav3sRaffleLensUSDT__PrizePaid(string pubId,uint256 indexOfWinners,address mirrorer, uint256 reward);
...
contract wav3sRaffleLensUSDT is VRFConsumerBaseV2{
    using Events for *;
    ...
    // Mapping to store the data associated with a wav3s pubid
    ...
    /**
     * @dev Funds a wav3sRaffleLensUSDT post. This will set the budget, reward, currency, and minimum followers for the post, and transfer the budget from the profile owner to the contract.
    ...
    function fundTweet(
        ...
        require(
            ...
            "Errors.wav3sRaffleLensUSDT__fundTweet__SenderNotOwner()"
        );
        ...
        postDataByPublicationId[pubId].budget += budget - fees_amount;
        // Check correct ratio of rewards to budgets
        require(
            budget % reward == 0,
            "Errors.wav3sRaffleLensUSDT__fundTweet__IncorrectRewardForBudget"
        );
        // Check if the reward is less than the minimum reward
        require(
            reward >= i_minReward,
            "Errors.wav3sRaffleLensUSDT__fundTweet__RewardBelowMinimum()"
        );
        // Check if the budget is enough for the reward
        require(
            reward <= budget,
            "Errors.wav3sRaffleLensUSDT__fundTweet__NotEnoughBudgetForThatReward()"
        );

        // Set the reward, currency, currency address, profile address and consumerApp address of this Publication.
        postDataByPublicationId[pubId].reward = reward;
        postDataByPublicationId[pubId].pubOwnerAddress = pubOwnerAddress;
        postDataByPublicationId[pubId].minFollowers = minFollowers;
        postDataByPublicationId[pubId].initiatedWav3 = true;
        postDataByPublicationId[pubId].pubId = pubId;
        postDataByPublicationId[pubId].raffleTime = block.timestamp + raffleTime;
        ...
        emit Events.wav3sRaffleLensUSDT__PostFunded(
     ...
     */
        ...
        require(
            ...
            "Errors.wav3sRaffleLensUSDT__process__PostNotInitiated(): Post is not funded yet"
        );
        ...
        require(
            ...
            "Errors.wav3sRaffleLensUSDT__process__FollowerAlreadyMirrored()"
        );

        // Check if there's enough budget to pay the reward
        require(
            reward <= budget,
            "Errors.wav3sRaffleLensUSDT__process__NotEnoughBudgetForThatReward()"
        );

        // Check if the mirrorer has enough followers
        require(
            followersCount >= minFollowers,
            "Errors.wav3sRaffleLensUSDT__process__NeedMoreFollowers()"
        );

        // Check if the profile address is valid
        require(
            mirrorerAddress != address(0),
            "Errors.wav3sRaffleLensUSDT__process__InvalidProfileAddress(): Invalid profile address"
        );

        // Check if the publication ID is valid
        require(
            bytes(pubId).length != 0,
            "Errors.wav3sRaffleLensUSDT__process__InvalidPubId(): Invalid publication ID"
        );
        ...
        if(s_PubIdToRaffleState[pubId] == RaffleState.OPEN && block.timestamp > postDataByPublicationId[pubId].raffleTime && postDataByPublicationId[pubId].retwitters >= budget/reward){
        s_PubIdToRaffleState[pubId] = RaffleState.CALCULATING; 
        requestRandomWinners(uint32(budget/reward),pubId);
        }else{
    
        emit Events.wav3sRaffleLensUSDT__MirrorProcessed(
            postDataByPublicationId[pubId].budget,
            reward,
            mirrorerAddress,
            pubId
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        emit Events.wav3sRaffleLensUSDT__PrizePaid(pubId, indexOfWinners[i], s_PubIdToIndexToMirrorer[pubId][indexOfWinners[i]], reward);

        postDataByPublicationId[pubId].budget -= reward;

        if (postDataByPublicationId[pubId].budget == 0) {
            emit Events.wav3sRaffleLensUSDT__PubFinished(pubId);
        }
        }
    }
    ...
    function whitelistWav3sTrigger(address wav3sTrigger) external onlyOwner {
        ...
        emit Events.wav3sRaffleLensUSDT__TriggerSet(wav3sTrigger, msg.sender);
    }
    ...
    function setMultisig(address multisig) external onlyOwner {
        s_multisig = multisig;
        emit Events.wav3sRaffleLensUSDT__MsigSet(multisig, msg.sender);
    }
    ...
    function circuitBreaker() external onlyOwner {
        ...
        stopped = !stopped;
        emit Events.wav3sRaffleLensUSDT__CircuitBreak(stopped);
    }
    ...
    function withdrawPub(
        ...
        require(
            bytes(pubId).length != 0,
            "Errors.wav3sRaffleLensUSDT__EmergencyWithdraw__InvalidPubId()"
        );

        // Check if the Publication is initiated
        require(
            postDataByPublicationId[pubId].initiatedWav3 == true,
            "Errors.wav3sRaffleLensUSDT__EmergencyWithdraw__Wav3NotInitiated()"
        );
        ...
        require(
            budget > 0,
            "Errors.wav3sRaffleLensUSDT__EmergencyWithdraw__NotEnoughBudgetToWithdraw()"
        );
        ...
        emit Events.wav3sRaffleLensUSDT__EmergencyWithdraw(
            pubId,
            postDataByPublicationId[pubId].budget,
            msg.sender
        );
    ...
    function backdoor() external onlyInEmergency onlyOwner {
        ...
        emit Events.wav3sRaffleLensUSDT__backdoor(balance);
    }
