    ...
    PausableUpgradeable
{
    ...
    modifier onlyManagedVaults(address vault) {
        require(vaults[vault].initialized == true, "Vault not found");
        _;
    }
    ...
    function rebalance(
        address vault,
        Rebalance calldata rebalanceParams,
        uint256 feeAmount
    )
        external
        override
        whenNotPaused
        onlyManagedVaults(vault)
        onlyGelatoOrOwner(feeAmount)
    {
        uint256 balance = _preExec(vault, feeAmount);
        IVaultV2(vault).rebalance(rebalanceParams);
        emit RebalanceVault(vault, balance);
    }

    function addRangeAndRebalance(
        address vault,
        Range[] calldata ranges,
        Rebalance calldata rebalanceParams,
        uint256 feeAmount
    )
        external
    ...
    {
        uint256 balance = _preExec(vault, feeAmount);
        IVaultV2(vault).addRangeAndRebalance(ranges, rebalanceParams);
        emit RebalanceVault(vault, balance);
    }
    ...
    function addVault(address vault) external override onlyOwner {
        require(vaults[vault].initialized == false, "Vault already added");
    ...
    function removeVault(address vault) external override onlyOwner {
        require(vaults[vault].initialized == true, "Vault not found");
        require(vaults[vault].balance == 0, "Vault still has balance");
        delete vaults[vault];
    ...
    function withdrawVaultBalance(
        ...
        require(
            vaults[vault].balance >= amount,
            "ManagerProxyV1: amount exceeds available balance"
        );
        vaults[vault].balance -= amount;
        Address.sendValue(payable(to), amount);
        emit WithdrawVaultBalance(vault, amount, to, vaults[vault].balance);
    }

    // ====== EXTERNAL FUNCTIONS ========
    function fundVaultBalance(address vault)
        external
        payable
        override
        onlyManagedVaults(vault)
    {
        vaults[vault].balance += msg.value;
        emit UpdateVaultBalance(vault, vaults[vault].balance);
    }

    // ====== INTERNAL FUNCTIONS ========
    function _preExec(address vault, uint256 feeAmount)
        internal
        returns (uint256 balance)
    {
        VaultInfo memory vaultInfo = vaults[vault];
        require(vaultInfo.balance > feeAmount, "Not enough balance to pay fee");
        balance = vaultInfo.balance - feeAmount;

        // update lastRebalance time
        // solhint-disable-next-line not-rely-on-time
        vaults[vault].lastRebalance = block.timestamp;
        vaults[vault].balance = balance;
    }
}
