...
/// @dev factory
import "./factory/FactoryTokens.sol";
    ...
    Oracle
{
    ...
    /// @param _wallet                              Address of the wallet destination
    /// @param _quantity                             Id of the token to transfer
    function transferReserved(
        uint256 _pairId,
        address _wallet,
        uint256 _quantity
    ) 
        external onlyUser noReentrant  
    {
        /// @dev get the pair
        Pair storage pair = _PairstoSwap[_pairId];

        require(pair.active, "Buy With Custom Token: Pair is not active");

        /// @dev Transfer tokens
        IPropertyToken(pair.tokenAddress).mintReserved(
            _wallet,
            _quantity
        );
    }
    ...
    /// @param _amountTokens                        Amount of tokens to buy
    function BuyWithCustomToken(
        address _token,
        uint256 _pairId,
    ...
    {
        /// @dev Validate quantity is valid
        require(
            _amountTokens > 0,
            "Buy With Custom Token: Specify an amount of token greater than zero"
        );

        /// @dev get Collection pair
        Pair storage pair = _PairstoSwap[_pairId];
        require(pair.active, "Buy With Custom Token: Pair is not active");


        /// @dev check if a whitelist token
        require(isWhiteListToken(_token), "Buy With Custom Token: Invalid token");

        /// @dev check whitelist token is available
        ERC20List memory erc20Token = getWhiteListTokenInfo(_token);
        require(erc20Token.active, "Buy With Custom Token: Token is not available");

        /// @dev check whitelist token is not native
        require(!erc20Token.isNative, "Buy With Custom Token: Not custom token native sended");

        /// @dev Validate user approve amount
        require(
            IERC20(erc20Token.tokenAddress).allowance(_msgSender(), address(this)) >=
                _amountTokens,
            "Buy With Custom Token: You don't have enough tokens to buy"
        );

        /// @dev Validate user token balance
        require(
            IERC20(erc20Token.tokenAddress).balanceOf(_msgSender()) >= _amountTokens,
            "Buy With Custom Token: Your balance is lower than the amount of tokens you want to sell"
        );

        /// @dev transfer tokens to the contrsenderact
        require(
            IERC20(erc20Token.tokenAddress).transferFrom(
                _msgSender(),
                address(this),
                _amountTokens
            ),
            "Buy With Custom Token: Failed to transfer tokens from user to vendor"
        );


        /// @dev Now we have to transfer the token to buyer


        /// @dev  Get price of the custom token in USD
        ...
        uint256 amountTo18 = transformAmountTo18Decimal(
            _amountTokens,
            uint8(ERC20(erc20Token.tokenAddress).decimals())
        );

        /// @dev calculate the amount sended in USD
        uint256 valueInUsd = amountTo18.mul(latestPrice).div(1e18);

        // @dev check if the user has enought tokens to buy the amount of token he wants to buy
        uint256 _quantity = valueInUsd.div(pair.price);

        require(
            _quantity > 0,
            "Buy With Custom Token: Quantity most be great that 0"
        );
        ...
        noReentrant
    {
        ...
        require(
            msg.value > 0,
            "BuyWithNativeToken: No amount sended"
        );

        /// @dev get the pair
        Pair storage pair = _PairstoSwap[_pairId];

        require(pair.active, "Buy With Native Token: Pair is not active");
        ...
        /// @dev calculate the amount sended in USD
        uint256 valueInUsd = msg.value.mul(latestPrice).div(1e18);
...
contract FactoryTokens is Administered {

    /// @dev struct
    struct Pair {
        address tokenAddress;                         // NFT to sell
    ...
    /// @dev mapping for tokens address
    mapping(address => PairTokenAddress) _PairstoSwapAddress;


    /// @dev Total number of pairs
    ...
    /// @notice Store Pair
    /// @dev Store a new pair for exchange
    /// @param _tokenAddress                          NFT contract address
    /// @param _price                               Price in USD
        ...
        onlyUser
    {
        ...
        require(
            isPair(_tokenAddress) == false,
            "Token Collection already stored"
        );

        uint256 pointer = pairCount;

        /// @dev save the pair
        _PairstoSwap[pointer] = Pair(_tokenAddress, _price, _active);

        /// @dev save token Address
        _PairstoSwapAddress[_tokenAddress] = PairTokenAddress(_tokenAddress, pointer);

        /// @dev Increment count of pairs
        pairCount++;
    }


    /// @notice Get all pairs
    /// @dev Get all pairs registered
    function pairList() 
        external 
        view 
        returns (Pair[] memory) 
    {
        unchecked {
            uint256 pointer = pairCount;
            Pair[] memory p = new Pair[](pointer);
        ...
        onlyUser 
    {
        ...
        /// @dev Update the price of the pair
        if (_type == 1) {
            _PairstoSwap[_id].price = _price;

        /// @dev Update status of the pair
        } else if (_type == 2) {
    ...
    /// @notice Verify Token already Store
    /// @dev Verify if the token is in the whitelist
    /// @param _tokenAddress                            Address of the token contract
    function isPair(address _tokenAddress) 
        private
        view
        returns (bool) 
    {
        if(_PairstoSwapAddress[_tokenAddress].tokenAddress == address(0x0)){
            return false;
        } else {
            return true;
        }
    ...
    /// @dev Get pair token information
    function getPairInformation(address _tokenAddress)
        internal
        view
        returns (Pair memory)
    {
        require(
            isPair(_tokenAddress),
            "Invalid Token"
        );

        PairTokenAddress storage row = _PairstoSwapAddress[_tokenAddress];
        return _PairstoSwap[row.index];
    }
}
...
contract Withdraw is Ownable {
    ...
    function withdrawMaticOwner(uint256 amount, address to)
        external
        payable
        onlyOwner
        returns (bool)
