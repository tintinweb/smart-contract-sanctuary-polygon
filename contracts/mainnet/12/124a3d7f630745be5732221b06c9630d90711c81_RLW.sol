...
contract RLW is ERC721Enumerable, Ownable, ERC2981, ClampedRandomizer{
    
    uint public maxSupply = 169;
    uint public wlPrice = 0;
    uint public price = 15 ether;
    address public devAddress = 0xd8936E602E38dfEe5d6466865068b94b1943DEBF;
    address public artistAddress = 0xd8936E602E38dfEe5d6466865068b94b1943DEBF;
    uint public publicTimestamp = 1683914400; 
    uint public wlTimestamp = 1683828000;
    mapping(address => uint) public whitelist;
    string public baseURI ='https://metacardsclub.io/rlw/json/';

    constructor() ERC721("Red Light Waifu", "RLW") ClampedRandomizer(maxSupply) {
    ...
    function setWhitelist(address[] memory _addresses, uint _amount) public onlyOwner {
        for(uint i = 0; i < _addresses.length; i++) {
            whitelist[_addresses[i]] = _amount;
        }
    }

    function setPublicTimestamp(uint _publicTimestamp) public onlyOwner {
        publicTimestamp = _publicTimestamp;
    }

    function setWlTimestamp(uint _wlTimestamp) public onlyOwner {
        wlTimestamp = _wlTimestamp;
    }



    function mintByOwner(address _to, uint _amount) public onlyOwner {
    ...
    //mint func for both wl and public
    function mint(uint _amount) public payable {
        require(totalSupply() + _amount <= maxSupply, "Exceeds max supply");
        require(block.timestamp >= wlTimestamp, "Sale has not started");
        payable(devAddress).transfer(msg.value * 0 / 100);
        payable(artistAddress).transfer(msg.value * 100 / 100);
        if(block.timestamp < publicTimestamp) {
            require(whitelist[msg.sender] >= _amount, "Not whitelisted");
            require(msg.value >= wlPrice * _amount, "Insufficient funds");
