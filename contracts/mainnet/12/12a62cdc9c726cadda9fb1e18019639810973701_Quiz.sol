...
contract Quiz {
    ...
    event CreateQuiz(string _appId, uint256 _quizId, string[] questions, IERC20Metadata _rewardToken, uint256 _totalReward,
        uint256 _startTime, uint256 _activeTime, uint256[] _lotteries);
    event Awards(bytes32 _quizId);

    constructor(address payable _operator) {
        owner = msg.sender;
    ...
    function createQuiz(string memory _appId, uint256 _quizId, string[] memory _questions, IERC20Metadata _rewardToken,
        uint256 _totalReward, uint256 _startTime, uint256 _activeTime, uint256[] memory _lotteries, string memory _title, string memory _photo) payable public onlyAdmin {
        require(_quizId != 0, "invalid quizId 0");
        ...
        emit CreateQuiz(_appId, _quizId, _questions, _rewardToken, _totalReward, _startTime, _activeTime, _lotteries);
    }
