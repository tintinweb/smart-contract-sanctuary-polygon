...
contract InstaAutomationImplementation is InstaAutomationHelper {
    ...
    function executeAutomation(
        ...
        executeAutomation(user_, id_, nonce_, true, params, metadata_);
    }
    ...
    function executeAutomation(
        ...
        ExecutionParams memory params,
        bytes calldata metadata_
    ) public onlyDSA(user_) onlyExecutor {
        require(_userLatestId[user_] == id_, "not-valid-id");
        Automation storage _userAutomationConfig = _userAutomationConfigs[id_];
        ...
        bool success = cast(AccountInterface(user_), spells_);
        ...
        if (!success) {
            ...
            params.swap.callData = "0x"; // Making it 0x, so it will reduce gas cost for event emission
        ...
        if (isSameToken_) {
            ...
            (spells._targets[0], spells._datas[0]) = (
                ...
                abi.encodeWithSignature(
                    ...
                    params.collateralToken, // debt = collateral token
                    params.collateralAmount, // collateral atoken to be used as payback
                    params.rateMode, // rate mode
                    0,
                    0
                )
            );
