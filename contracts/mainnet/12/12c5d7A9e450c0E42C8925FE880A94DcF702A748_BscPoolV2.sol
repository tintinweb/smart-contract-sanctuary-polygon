...
contract BscPoolV2 is Ownable {
  ...
  function getGiftTokenPercent() public view onlyOwner returns (uint256, uint256){
    ...
    returns (bool)
  {
    ...
    function _sendGiftToken(uint256 amount, string memory orderId ) private {
        uint256 _gift_amount = amount.mul(_gift_amount_nominator).div(_gift_amount_denominator);

        uint _gift_balance = IERC20(gift_token).balanceOf(address(this));
