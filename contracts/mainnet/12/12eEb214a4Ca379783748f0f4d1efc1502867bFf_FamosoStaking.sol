...
contract FamosoStaking {
    ...
    uint256 public lockedTime = 600; // 90 * 24 * 3600; // 90days
    uint private unlocked = 1;

    uint256 public startStakingTimeStamp;
    uint256 public stakingPeriod = 48 * 3600; // 180 * 24 * 3600; // 180days
    ...
    function setStartStakingTimeStamp() external onlyAdmin {
        startStakingTimeStamp = block.timestamp;
    }
    ...
    function getReward(address _user) external view returns (uint256) {
        ...
        if (block.timestamp > lastRewardTimeStamp) {
            ...
            uint256 tokenReward = stakedPeriod.mul(rewardPerSec);
            _accCifiPerShare = _accCifiPerShare.add(tokenReward.mul(1e12).div(totalStakedAmount));
        }
        return user.amount.mul(_accCifiPerShare).div(1e12).sub(user.rewardDebt).add(user.pendingReward);
    }
    ...
    function updatePool() internal {
        ...
        uint256 stakedPeriod = getStakedPeriod(lastRewardTimeStamp, block.timestamp);
        uint256 tokenReward = stakedPeriod.mul(rewardPerSec);
        accCifiPerShare = accCifiPerShare.add(tokenReward.mul(1e12).div(totalStakedAmount));
        lastRewardTimeStamp = block.timestamp;
    }
    ...
    function reward() public lock {
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        uint256 pending = user.amount.mul(accCifiPerShare).div(1e12).sub(user.rewardDebt).add(user.pendingReward);
    ...
    function withdraw() public lock {
        ...
        require(user.lastDepositTimeStamp + lockedTime < block.timestamp, "you are in locktime.");
        updatePool();
        uint256 withdrawAmount = user.amount;
        user.amount = 0;
        uint256 pending = withdrawAmount.mul(accCifiPerShare).div(1e12).sub(user.rewardDebt).add(user.pendingReward);
        user.pendingReward = 0;
        totalStakedAmount = totalStakedAmount - withdrawAmount;
        user.rewardDebt = 0;
        user.rewarded = user.rewarded + pending;
        TransferHelper.safeTransfer(lpToken, msg.sender, withdrawAmount);
        TransferHelper.safeTransfer(rewardToken, msg.sender, pending);
    ...
    function emergencyWithdraw(uint amount) external lock onlyAdmin {
        require(IERC20(rewardToken).balanceOf(address(this)) >= amount, "Withdraw-Overflow");
