...
contract Automated is ConfirmedOwner, Pausable, KeeperCompatibleInterface {
  ...
  AggregatorV2V3Interface public eth_price_contract;
  uint256 public downCount;
  uint256 public upCount;
  int256 public prvPrice;
  int256 public spotPrice;
  uint8 public priceDecimals;

  constructor(address keeperRegistryAddress,AggregatorV2V3Interface _eth_price_contract) ConfirmedOwner(msg.sender) {
    setKeeperRegistryAddress(keeperRegistryAddress);
    setPriceFetcherAddress(_eth_price_contract);
    priceDecimals=8; 
    setSpotPrice(eth_price_contract.latestAnswer());
  }

  function setSpotPrice(int256 _spotPrice) public {
    spotPrice=_spotPrice;
    downCount=0;
    upCount=0;
    prvPrice=eth_price_contract.latestAnswer();
  }

  function getDirection() public view returns (int8 direction){
    int256 currentPrice=eth_price_contract.latestAnswer();
    if(currentPrice>spotPrice){
      if(prvPrice>spotPrice)
      return 0;
      else
      return 1;
    } else if(currentPrice<spotPrice){
      if(prvPrice<spotPrice)
      return 0;
      else
      return -1;
  ...
  function addCount(int8 _direction) private whenNotPaused {
    if(_direction>0){
      upCount+=1;
  ...
  {
    int8 direction=getDirection();
    upkeepNeeded = direction!=0;
    performData = abi.encode(direction);
    return (upkeepNeeded, performData);
  }

  function performUpkeep(bytes calldata performData) external override onlyKeeperRegistry whenNotPaused {
    int8 direction = abi.decode(performData, (int8));
    addCount(direction);
  }
   ...
   */
  function setPriceFetcherAddress(AggregatorV2V3Interface _eth_price_contract) public onlyOwner {
    // require(_eth_price_contract != address(0));
    // emit KeeperRegistryAddressUpdated(s_keeperRegistryAddress, keeperRegistryAddress);
    eth_price_contract = _eth_price_contract;
  }
