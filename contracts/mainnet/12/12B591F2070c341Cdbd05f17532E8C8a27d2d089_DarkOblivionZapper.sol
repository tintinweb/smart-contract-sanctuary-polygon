...
contract DarkOblivionZapper is Owned, TokensRecoverable, ReentrancyGuard
{
    ...
    address dMagicLPAddress; // LP address axMatic-dMagic Sushi
    ...
    constructor(IAxMatic _axMatic, IERC20 _dmagic, address _LPAddress, IERC20 _Drax, IaxMatic_Direct _axMaticDirect, IDragonZapper _dragonDirect, IERC31337 _LPNFTWrapper, IERC1155 _ERC1155Token, uint _erc1155TokenID, address _dMagicLPAddress){
        ...
        dMagicLPAddress = _dMagicLPAddress;
  ...
  function estimateBuyLPFromMatic(uint256 _matic_amount) external view returns(uint256){
        uint256 maticToBuy = _matic_amount.div(4);
        (uint256 reserve0DM, uint256 reserve1DM,) = IUniswapV2Pair(dMagicLPAddress).getReserves(); 
        uint256 dMagicAmt = axMaticDirect.estimateBuy(maticToBuy);

       // liquidity = Math.min(amount0.mul(_totalSupply) / _reserve0, amount1.mul(_totalSupply) / _reserve1);
        uint256 amt1Drax = maticToBuy.mul(IUniswapV2Pair(dMagicLPAddress).totalSupply()).div(reserve0DM.add(maticToBuy));
        uint256 amt2Drax = dMagicAmt.mul(IUniswapV2Pair(dMagicLPAddress).totalSupply()).div(reserve1DM.sub(dMagicAmt));

        uint draxAmt;
        if(amt1Drax>amt2Drax) draxAmt = amt2Drax;
        else draxAmt = amt1Drax;

        (uint256 reserve0, uint256 reserve1,) = IUniswapV2Pair(LPAddress).getReserves(); 
        uint256 dMagicAmtForLP;// = axMaticDirect.estimateBuy(maticToBuy*2);
        uint256 amountIn = maticToBuy*2;

        uint reserveIn = reserve0DM.sub(dMagicAmt);
        uint amountInWithFee = amountIn.mul(997);
        // uint numerator = amountInWithFee.mul(reserve1);
        // uint denominator = reserveIn.mul(1000).add(amountInWithFee);
        dMagicAmtForLP = amountInWithFee.mul(reserve1DM.sub(maticToBuy)) / reserveIn.mul(1000).add(amountInWithFee);

        uint256 amt2 = dMagicAmtForLP.mul(IUniswapV2Pair(LPAddress).totalSupply()).div(reserve1);
        uint256 amt1 = draxAmt.mul(IUniswapV2Pair(LPAddress).totalSupply()).div(reserve0);

        if(amt1>amt2) return amt2;
        else return amt1;
    }

    function estimateBuyLPFromDmagic(uint256 _dmagic_amount) external view returns(uint256){
        uint256 dMagicToBuy = _dmagic_amount.div(4);
        (uint256 reserve0DM, uint256 reserve1DM,) = IUniswapV2Pair(dMagicLPAddress).getReserves(); 
        uint256 maticAmount = axMaticDirect.estimateSell(dMagicToBuy);

        uint256 amt1Drax = maticAmount.mul(IUniswapV2Pair(dMagicLPAddress).totalSupply()).div(reserve0DM.sub(maticAmount));
        uint256 amt2Drax = dMagicToBuy.mul(IUniswapV2Pair(dMagicLPAddress).totalSupply()).div(reserve1DM.add(dMagicToBuy));
        ...
        (uint256 reserve0, uint256 reserve1,) = IUniswapV2Pair(LPAddress).getReserves(); 
        uint256 amt2 = dMagicToBuy.mul(2).mul(IUniswapV2Pair(LPAddress).totalSupply()).div(reserve1);
        uint256 amt1 = draxAmt.mul(IUniswapV2Pair(LPAddress).totalSupply()).div(reserve0);
    ...
    function estimateBuyLPFromDrax(uint256 _drax_amount) external view returns(uint256){
        uint256 draxToBuy = _drax_amount.div(2);
        (uint256 reserve0, uint256 reserve1,) = IUniswapV2Pair(LPAddress).getReserves(); 

        address[] memory path = new address[](2);
        path[0] = address(Drax);
        path[1] = address(dmagic);
        (uint256[] memory amounts) = uniswapV2Router.getAmountsOut(draxToBuy, path);
        uint256 dMagicAmt = amounts[1];

        uint256 amt2 = dMagicAmt.mul(IUniswapV2Pair(LPAddress).totalSupply()).div(reserve1.sub(dMagicAmt));
        uint256 amt1 = draxToBuy.mul(IUniswapV2Pair(LPAddress).totalSupply()).div(reserve0.add(draxToBuy));
