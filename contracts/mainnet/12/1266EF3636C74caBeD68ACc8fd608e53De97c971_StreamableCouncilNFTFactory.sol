...
import "./StreamableCouncilNFT.sol";

contract StreamableCouncilNFTFactory is Ownable {
    StreamableCouncilNFT[] public contracts;

    address public sfHost;
    address public superToken;
    string public registrationKey;

    constructor(address _sfHost, address _superToken, string memory _registrationKey) {
        sfHost = _sfHost;
        superToken = _superToken;
        registrationKey = _registrationKey;
    }

    function createNew(string calldata name, string calldata symbol) external onlyOwner {
        StreamableCouncilNFT nft = new StreamableCouncilNFT(name, symbol, ISuperfluid(sfHost), ISuperToken(superToken), registrationKey);
...
contract StreamableCouncilNFT is SuperAppBase, CouncilNFT {
    ...
    address public streamOrigin;

    event ReceiverChanged(address oldRec, address newRec);
    ...
    function _deleteFlowWithContext(bytes memory ctx, address recipient) private returns (bytes memory) {
        (ctx, ) = _host.callAgreementWithContext(
            cfaV1Lib.cfa,
            abi.encodeWithSelector(
                cfaV1Lib.cfa.deleteFlow.selector,
                _acceptedToken,
                address(this),
                recipient,
    ...
    function _deleteFlowNoContext(address recipient) private {
        _host.callAgreement(
            cfaV1Lib.cfa,
            abi.encodeWithSelector(
                cfaV1Lib.cfa.deleteFlow.selector,
    ...
    function _createFlowWithContext(bytes memory ctx, address recipient, int256 rate) private returns (bytes memory) {
        (ctx, ) = _host.callAgreementWithContext(
            cfaV1Lib.cfa,
            abi.encodeWithSelector(
                cfaV1Lib.cfa.createFlow.selector,
                _acceptedToken,
                recipient,
                rate,
                new bytes(0) // placeholder
    ...
    function _createFlowNoContext(address recipient, int256 rate) private {
        _host.callAgreement(
            cfaV1Lib.cfa,
            abi.encodeWithSelector(
                cfaV1Lib.cfa.createFlow.selector,
    ...
    function _updateFlowNoContext(address recipient, int256 rate) private {
        _host.callAgreement(
            cfaV1Lib.cfa,
            abi.encodeWithSelector(
                cfaV1Lib.cfa.updateFlow.selector,
                _acceptedToken,
                recipient,
                rate,
                new bytes(0) // placeholder
    ...
    function _updateFlowWithContext(bytes memory ctx, address recipient, int256 rate) private returns (bytes memory) {
        (ctx, ) = _host.callAgreementWithContext(
            cfaV1Lib.cfa,
            abi.encodeWithSelector(
                cfaV1Lib.cfa.updateFlow.selector,
    ...
    function _updateOutflow(bytes memory ctx, bool useContext) private returns (bytes memory newCtx) {
        newCtx = ctx;

        if (useContext && streamOrigin == address(0)) {
            streamOrigin = _host.decodeCtx(newCtx).msgSender;
        }

        if (streamOrigin == address(0) || totalSupply() == 0) {
            return newCtx;
        }

        (,int96 inFlowRate,,) = cfaV1Lib.cfa.getFlow(
            _acceptedToken,
            streamOrigin, // fix this
            address(this)
        );

        int96 newOutflowRate = inFlowRate / int96(int256(totalSupply()));

        if (newOutflowRate == 0) {
            for (uint256 i = 0; i < totalSupply(); i++) {
                (, int96 ofr, , ) = cfaV1Lib.cfa.getFlow(
                    _acceptedToken,
                    address(this),
                ...
                if (ofr != int96(0)) {
                    if (useContext) {
                        newCtx = _deleteFlowWithContext(newCtx, ownerOf(tokenByIndex(i)));
                    }
                    else {
                        _deleteFlowNoContext(ownerOf(tokenByIndex(i)));
                    }
                }
            }
        }
            ...
            for (uint256 i = 0; i < totalSupply(); i++) {
                ...
                if (ofr == int96(0)) {
                    if (useContext) {
                        newCtx = _createFlowWithContext(newCtx, ownerOf(tokenByIndex(i)), newOutflowRate);
                    }
                    else {
                        _createFlowNoContext(ownerOf(tokenByIndex(i)), newOutflowRate);
                    }
                }
                else if (ofr != newOutflowRate) {
                    if (useContext) {
                        newCtx = _updateFlowWithContext(newCtx, ownerOf(tokenByIndex(i)), newOutflowRate);
                    }
                    else {
                        _updateFlowNoContext(ownerOf(tokenByIndex(i)), newOutflowRate);
                    }
                }
            }
        }
    ...
    // is called before token transfer
    function _changeReceiver(address oldReceiver, address newReceiver)
        internal
    {
        ...
        // if this is a mint, do nothing, _updateOutflow will do the work
        ...
        (, int96 oldOutFlowRate, , ) = cfaV1Lib.cfa.getFlow(
            _acceptedToken,
            address(this),
            oldReceiver
        );
        ...
        if (oldOutFlowRate > 0) {
            _deleteFlowNoContext(oldReceiver);

            // if this is a burn we only delete the old stream
            if (newReceiver != address(0)) {
                (, int96 newOutFlowRate, , ) = cfaV1Lib.cfa.getFlow(
                    _acceptedToken,
                    address(this),
                    newReceiver
                );
                ...
                if (newOutFlowRate == 0) {
                    _createFlowNoContext(newReceiver, oldOutFlowRate);
                }
                else if (newOutFlowRate != oldOutFlowRate) {
                    _updateFlowNoContext(newReceiver, oldOutFlowRate);
                }
            }
        }
    ...
    function _mint(address to, uint256 tokenId) internal override {
        require(totalSupply() < maxSupply, "StreamableCouncilNFT: Cannot exceed maximum supply");
        super._mint(to, tokenId);
        _updateOutflow("0x", false);
    }
    ...
    function beforeAgreementCreated(
        ...
        returns (bytes memory cbdata) {
            require(streamOrigin == address(0), "StreamableCouncilNFT: There is already an inbound stream to this app");
        }
    ...
    {
        // uint256 g = gasleft();
        newCtx = _updateOutflow(_ctx, true);
        // emit Gas(g - gasleft());
    }
    ...
    modifier onlyHost() {
        require(
            ...
            "StreamableCouncilNFT: support only one host"
        );
        _;
    }

    modifier onlyExpected(ISuperToken superToken, address agreementClass) {
        require(_isSameToken(superToken), "StreamableCouncilNFT: not accepted token");
        require(_isCFAv1(agreementClass), "StreamableCouncilNFT: only CFAv1 supported");
        _;
    }
}
...
contract CouncilNFT is ERC721Enumerable, Ownable {
    mapping(uint256 => mapping(address => bool)) public isApprovedSpenderForTokenId;
    mapping(address => bool) public isApprovedSpenderForAllTokens;

    constructor(string memory name_, string memory symbol_) ERC721(name_, symbol_) {}

    function addApprovedSpenderForAllTokens(address spender) external onlyOwner {
        isApprovedSpenderForAllTokens[spender] = true;
    }

    function removeApprovedSpenderForAllTokens(address spender) external onlyOwner {
        isApprovedSpenderForAllTokens[spender] = false;
    }

    function addApprovedSpenderForTokenId(address spender, uint256 tokenId) external onlyOwner {
        isApprovedSpenderForTokenId[tokenId][spender] = true;
    }

    function removeApprovedSpenderForTokenId(address spender, uint256 tokenId) external onlyOwner {
        isApprovedSpenderForTokenId[tokenId][spender] = false;
    }

    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view override returns (bool) {
        return isApprovedSpenderForAllTokens[spender] || isApprovedSpenderForTokenId[tokenId][spender];
    }

    function mint(address to, uint256 tokenId) public {
        require(_isApprovedOrOwner(msg.sender, tokenId), "CouncilNFT: Account is not approved to mint tokenId");
        _mint(to, tokenId);
    }

    function burn(uint256 tokenId) public {
        require(_isApprovedOrOwner(msg.sender, tokenId), "CouncilNFT: Account is not approved to burn tokenId");
        _burn(tokenId);
    }
}
