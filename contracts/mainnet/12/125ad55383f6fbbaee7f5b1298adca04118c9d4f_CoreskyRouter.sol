 ...
 */
contract CoreskyRouter is SeaportRouterInterface, ReentrancyGuard, Ownable {
    /**
     * @dev store the router address for exchange
     */
    struct Market{
        address exchange;
        bool    isActive;
    ...
    function updateMarketStatus(uint256 marketId, bool newStatus) external onlyOwner {
    ...
    {
        // note when one market is false, the corresponding index will be zeroAddress.
        marketContracts = new address[](markets.length);
        for (uint256 i=0; i < markets.length; ++i) {
            if (markets[i].isActive) {
                marketContracts[i] = markets[i].exchange;
            }
        }
    }

    /**
     * @notice Returns the market contracts status.
     */
    function getMarketsStatus()
    external
    view
    returns (Market[] memory marketContracts)
    {
        return markets;
    }
