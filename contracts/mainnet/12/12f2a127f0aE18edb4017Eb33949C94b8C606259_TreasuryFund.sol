...
contract TreasuryFund is DisplayERC20, BoringOwnableAndOperator, BoringBatchable {
    ...
    // TODO: tracks only current network
    ...
    // TODO: should it track multi network?
    ...
    function getValue(uint256 shares) public view returns (uint256 assets) {
        assets = totalAsset.toElastic(shares, false);
    }
    ...
    function deposit(uint256 assets) public onlyWhitelisted returns (uint256 shares) {
        ...
        asset.safeTransferFrom(msg.sender, fundReceiver, assets);
    ...
    function changeMaxFundSize(uint256 newMaxFundSize) public onlyOperator {
        fundMaxSize = newMaxFundSize;
        emit LogChangeMaxFundSize(newMaxFundSize);
    }

    function changeFundReceiver(address receiver) public onlyOperator {
        fundReceiver = receiver;
    }

    function updateMerkleRoot(bytes32 root, string memory hash_) public onlyOperator {
        merkleRoot = root;
        ipfsHash = hash_;
    ...
    function withdrawAsset (uint256 amount) public onlyOperator {
        asset.safeTransfer(fundReceiver, amount);
    }

    function logProfitOrLoss (uint128 amount, bool profit, bool loss) public onlyOperator {
        Rebase memory total = totalAsset;
