    ...
    ERC20VotesUpgradeable
{
    ...
    uint256 private COOLDOWN_SECONDS;
    uint256 private UNSTAKE_WINDOW;
    ...
    mapping(address => bool) private userCleanMapping;
    ...
    function stake(address onBehalfOf, uint256 amount) public {
        ...
        if (balanceOf(onBehalfOf) > 0) {
            uint256 pending = uint256(
                int256(
                    (balanceOf(onBehalfOf) * farm.accRewardsPerShare) /
                        ACC_REWARD_PRECISION
                ) - user.rewardDebt
            );
            if (pending > 0) {
                pendingRewards[onBehalfOf] += pending;
            }
        }
        user.rewardDebt += int256(
            (lpTokensToMint * farm.accRewardsPerShare) / ACC_REWARD_PRECISION
        );
