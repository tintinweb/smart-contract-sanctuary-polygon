...
library Errors {
   // wav3sMirrorMulti
    ...
    event wav3sMirrorMulti__process__ArrayLengthMismatch(string error);
    event wav3sMirrorMulti__process__PostNotInitiated(uint256 index, string error);
    event wav3sMirrorMulti__process__FollowerAlreadyMirrored(uint256 index, string error);
    event wav3sMirrorMulti__process__NeedMoreFollowers(uint256 index, string error);
    event wav3sMirrorMulti__process__NotEnoughBudgetForThatReward(uint256 index, string error);
    event wav3sMirrorMulti__process__InvalidProfileAddress(uint256 index, string error);
    event wav3sMirrorMulti__process__InvalidAppAddress(uint256 index, string error);
    event wav3sMirrorMulti__process__InvalidPubId(uint256 index, string error);
    event wav3sMirrorMulti__process__AppAddressNotWhitelisted(uint256 index, string error);
    event wav3sMirrorMulti__process__InvalidpubOwnerAddress(uint256 index, string error);
    //
    // Errores
...
library Events {
    // wav3sMirrorMulti Currency
       event wav3sMirrorMulti__PostFunded(
        uint256 budget,
        uint256 reward,
    ...
    event wav3sMirrorMulti__MirrorProcessed(
        uint256 currentBudget,
        uint256 reward,
        address mirrorerAddress,
        string pubId
    );
    event wav3sMirrorMulti__RewardsWithdrawn(
        address mirrorerAddress,
        uint256 rewardsWitdrawn
    );

  
    event wav3sMirrorMulti__PubFinished(string pubId);

    event wav3sMirrorMulti__TriggerSet(address trigger, address sender);
    event wav3sMirrorMulti__MsigSet(address msig, address sender);
    event wav3sMirrorMulti__PubWithdrawn(
        uint256 budget,
        string pubId,
        address sender
    );
    event wav3sMirrorMulti__consumerAppWhitelisted(address consumerAppAddress);

    event wav3sMirrorMulti__CircuitBreak(bool stop);

    event wav3sMirrorMulti__EmergencyWithdraw(
        string pubId,
        uint256 budget,
        address sender
    );
  
    event wav3sMirrorMulti__PostFundedInEmergency(
        string pubId,
        uint256 budget,
        uint256 budgetFinal
    );

    event wav3sMirrorMulti__backdoor(address currency, uint256 balance);

    event wav3sMirrorMulti__CurrencyWhitelisted(address currency,bool isSuperCurrency);

    // Raffle multi currency events
    event wav3sRaffleMirror__PostFunded(
        uint256 budget,
        uint256 reward,
        address pubOwnerAddress,
        uint256 pubId
    ...
    event wav3sRaffleMirror__MirrorProcessed(
        uint256 currentBudget,
        uint256 reward,
        address mirrorerAddress,
        string pubId
    );
    event wav3sRaffleMirror__RewardsWithdrawn(
        address mirrorerAddress,
        uint256 rewardsWitdrawn
    );
    ...
    event wav3sRaffleMirror__PubWithdrawn(
        uint256 budget,
        string pubId,
        address sender
    );
    ...
    event wav3sRaffleMirror__EmergencyWithdraw(
        string pubId,
        uint256 budget,
        address sender
    );
  
    event wav3sRaffleMirror__PostFundedInEmergency(
        string pubId,
        uint256 budget,
        uint256 budgetFinal
    );
 ...
 * @param minFollowers The minimum number of followers required to be eligible for rewards.
 ...
 * @param initiatedWav3 A boolean indicating if the wav3 has been initiated.
 * @param currency The address of the currency used for rewards.
 * @param retwitters The number of retwitters.
 */
...
contract wav3sMirrorMulti {
    using Events for *;
    // Events
    event wav3sMirrorMulti__PostFunded(uint256 budget, uint256 reward, address pubOwnerAddress, address currency, uint256 pubId);
    event wav3sMirrorMulti__MirrorProcessed(uint256 currentBudget, uint256 reward, address mirrorerAddress, string pubId);
    event wav3sMirrorMulti__RewardsWithdrawn(address mirrorerAddress, uint256 rewardsWitdrawn);
    event wav3sMirrorMulti__PubFinished(string pubId);
    event wav3sMirrorMulti__TriggerSet(address trigger, address sender);
    event wav3sMirrorMulti__MsigSet(address msig, address sender);
    event wav3sMirrorMulti__PubWithdrawn(uint256 budget, string pubId, address sender);
    event wav3sMirrorMulti__ConsumerAppWhitelisted(address consumerAppAddress);
    event wav3sMirrorMulti__CircuitBreak(bool stop);
    event wav3sMirrorMulti__EmergencyWithdraw(string pubId, uint256 budget, address sender);
    event wav3sMirrorMulti__PostFundedInEmergency(string pubId, uint256 budget, uint256 budgetFinal);
    event wav3sMirrorMulti__Backdoor(address currency, uint256 balance);
    event wav3sMirrorMulti__CurrencyWhitelisted(address currency, bool isSuperCurrency);
    ...
    uint256 public nextId = 1;  // NextId indexer
    uint256 immutable i_basefee;  // Base fee
    ...
    uint256 private minFollowers;  // The minimum number of followers for the post pointed to
    ...
    /**
     * @dev Funds a wav3sMirrorMulti post. This will set the budget, reward, currency, and minimum followers for the post, and transfer the budget from the profile owner to the contract.
     ...
     * @param _minFollowers The minimum number of followers required for the post.
     * @param _currency The currency used for the budget and reward.
    ...
    function fundMirror(
        ...
        require(
            s_currencyWhitelisted[currency] || s_superCurrencyWhitelisted[currency],
            "Errors.wav3sRaffleMirror__fundMirror__TokenNotWhitelisted()"
        );
        ...
        IERC20(_currency).transferFrom(_pubOwnerAddress, address(this), _budget);
        ...
        emit Events.wav3sMirrorMulti__PostFunded(
     ...
     * @param id The ID of the funded post.
     * @param pubId The publication ID to set.
     */
     ...
     * @param followersCount The number of followers the mirrorer has.
     */
    function processMirror(
        ...
        require(
            postDataByPublicationId[pubId].initiatedWav3,
        ...
        require(
            ...
            "Errors.wav3sMirrorMulti__process__InvalidProfileAddress(): Invalid profile address"
        );

        // Check if the publication ID is valid
        require(
            bytes(pubId).length != 0,
            "Errors.wav3sMirrorMulti__process__InvalidPubId(): Invalid publication ID"
        );

        // Transfer funds from the budget owner to the mirrorer
        ...
        emit Events.wav3sMirrorMulti__MirrorProcessed(
     ...
     */
    function getMirrorBudget(string calldata pubId) external view returns (uint256) {
        // Get budget for this Publication
        return postDataByPublicationId[pubId].budget;
    }

    /**
     * @dev Gets the currency address for a Publication.
     * @param pubId The ID of the Publication.
     * @return The currency address for the Publication.
     */
    function getMirrorCurrency(string calldata pubId) external view returns (address) {
        // Get currency for this Publication
        return postDataByPublicationId[pubId].currency;
    }

    /**
     * @dev Gets the PostData for a Publication.
     * @param pubId The ID of the Publication.
     * @return The PostData for the Publication.
     */
    ...
    function whitelistCurrency(address _currency, bool isSuperCurrency) external onlyOwner {
        ...
        emit Events.wav3sMirrorMulti__CurrencyWhitelisted(_currency, isSuperCurrency);
    }

    /**
     * @dev Checks if a currency is whitelisted.
     * @param _currency The address of the currency to check.
     * @return True if the currency is whitelisted, false otherwise.
     */
    ...
    /**
     * @dev Checks if a supercurrency is whitelisted.
     * @param _currency The address of the supercurrency to check.
     * @return True if the supercurrency is whitelisted, false otherwise.
     */
    ...
    function whitelistWav3sTrigger(address wav3sTrigger) external onlyOwner {
        ...
        emit Events.wav3sMirrorMulti__TriggerSet(wav3sTrigger, msg.sender);
    }

    /**
     * @dev Checks if a wav3s trigger address is whitelisted.
     * @param wav3sTrigger The address of the wav3s trigger to check.
     * @return True if the wav3s trigger is whitelisted, false otherwise.
     */
    ...
    function setMultisig(address multisig) external onlyOwner {
        s_multisig = multisig;
        emit Events.wav3sMirrorMulti__MsigSet(multisig, msg.sender);
    }

    /**
     * @dev Gets the current multisig address.
     * @return The current multisig address.
     */
    ...
    function withdrawMirrorBudget(string calldata pubId) external stopInEmergency {
        ...
        require(bytes(pubId).length != 0, "Errors.wav3sMirrorMulti__withdraw__InvalidPubId()");

        // Check if the Publication is initiated
        require(postDataByPublicationId[pubId].initiatedWav3 == true, "Errors.wav3sMirrorMulti__withdraw__PostNotInitiated()");

        // Check that the sender is the owner of the given profile
        require(postDataByPublicationId[pubId].pubOwnerAddress == msg.sender, "Errors.wav3sMirrorMulti__withdraw__NotSenderProfileToWithdraw()");
        ...
        require(budget > 0, "Errors.wav3sMirrorMulti__withdraw__BudgetEmpty()");

        IERC20(currency).transfer(msg.sender, budget);
        postDataByPublicationId[pubId].budget = 0;
        s_PublicationToWithdrawalTime[pubId] = 0;

        emit Events.wav3sMirrorMulti__PubWithdrawn(budget, pubId, msg.sender);
    }

    /**
     * @dev Checks if a post is initiated by Wav3.
     * @param pubId The ID of the post.
     * @return True if the post is initiated by Wav3, false otherwise.
     */
    ...
    function circuitBreaker() external onlyOwner {
        stopped = !stopped;
        emit Events.wav3sMirrorMulti__CircuitBreak(stopped);
    }

    /**
     * @dev Emergency withdrawal of funds from a post.
     * @param pubId The ID of the post.
     */
    function withdrawPub(string calldata pubId) external onlyInEmergency onlyWav3sTrigger {
        // Check pubid validity
        require(bytes(pubId).length != 0, "Errors.wav3sMirrorMulti__EmergencyWithdraw__InvalidPubId()");

        // Check if the Publication is initiated
        require(postDataByPublicationId[pubId].initiatedWav3 == true, "Errors.wav3sMirrorMulti__EmergencyWithdraw__Wav3NotInitiated()");

        budget = postDataByPublicationId[pubId].budget;
        currency = postDataByPublicationId[pubId].currency;

        // Check that there is enough funds in the post budget to withdraw
        require(budget > 0, "Errors.wav3sMirrorMulti__EmergencyWithdraw__NotEnoughBudgetToWithdraw()");

        IERC20(currency).transfer(msg.sender, postDataByPublicationId[pubId].budget);
        emit Events.wav3sMirrorMulti__EmergencyWithdraw(pubId, postDataByPublicationId[pubId].budget, msg.sender);
        postDataByPublicationId[pubId].budget = 0;
    }
    ...
    function backdoor(address _currency) external onlyInEmergency onlyOwner {
        ...
        emit Events.wav3sMirrorMulti__backdoor(_currency, balance);
    }
