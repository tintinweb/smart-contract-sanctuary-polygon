...
contract StrategyDualRewardsStMatic is BaseStrategyStakingRewards {
    ...
    address private constant LDO = 0xC3C7d422809852031b44ab29EEC9F1EfF2A58756;
    address private constant stMATIC = 0x3A58a54C066FdC0f2D55FC9C89F0415C92eBf3C4;
    ...
    address[] private quick_matic_path;
        ...
        )
    {
        ...
        a_b_path = [WMATIC, stMATIC];
        reward_matic_path = [LDO, WMATIC];
        quick_matic_path = [QUICK, WMATIC];

        IERC20(WMATIC).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
        IERC20(stMATIC).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
        IERC20(QUICK).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
        IERC20(LDO).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        uint256 _rewardBal = IERC20(LDO).balanceOf(address(this));
        if(_rewardBal > 0) {
            IERC20(LDO).safeTransfer(strategist, _rewardBal);
        }
        ...
        //Convert LDO + QUICK to MATIC
        uint256 quickHarvested = IERC20(QUICK).balanceOf(address(this));
        if (quickHarvested > 0) {
            _swapUniswapWithPathPreapproved(quick_matic_path, quickHarvested);
        }
        uint256 ldoHarvested = IERC20(LDO).balanceOf(address(this));
        if (ldoHarvested > 0) {
            _swapUniswapWithPathPreapproved(reward_matic_path, ldoHarvested);
        }

        //Deduct performance fee from MATIC
