...
contract Blogging is Ownable, AccessControl, Pausable {
     ...
     * @param _payFeeRate - Paid blog fee rate.
     */
    function setPayFeeRate(uint256 _payFeeRate) public onlyRole(WORKER) {
     ...
     * @param _donateFeeRate - Donation fee rate.
     */
    function setDonateFeeRate(uint256 _donateFeeRate) public onlyRole(WORKER) {
    ...
    function createBlog(
        ...
        emit BlogCreated(latestBlogId, msg.sender, blog.isPaidType, _price);
    }
    ...
    /**
     * @dev Update blog price and type according to price.
     * @param _blog - Blog storage.
     * @param _price - New blog price.
     */
    function _updateBlogPrice(Blog storage _blog, uint256 _price) internal {
        if(_price == 0) {
            _blog.isPaidType = false;
        } else {
            _blog.isPaidType = true;
        }
        _blog.price = _price;
    }
}
