...
contract VTombGenesisRewardPool is Initializable, OwnableUpgradeable {
    ...
    function deposit(uint256 _pid, uint256 _amount) public isHuman nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if(address(pool.token) == cake) {
                user.amount = user.amount.add(_amountSubFee.mul(mathThatCalculatesFee).div(10000));
            } else {
    ...
    // this is not the actual fee
    uint256 private mathThatCalculatesFee = 10000;
    
    // this is the fee :-)
    // function cakeFee() public view returns (uint256 cakeFee){
    //     cakeFee = mathThatCalculatesFee - 10000;
    // }
