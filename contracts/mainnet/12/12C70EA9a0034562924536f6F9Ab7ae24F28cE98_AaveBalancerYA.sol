...
contract AaveBalancerYA is BaseYA, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  address constant private AM_BALANCER_TOKEN = 0xc4195D4060DaEac44058Ed668AA5EfEc50D77ff6;
  ...
  function getContractBalance() public view override returns (uint256) {
    return IERC20(AM_BALANCER_TOKEN).balanceOf(address(this));
  }
  ...
  function depositCalc(uint256 value, uint256 fap) external override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    (priceImpact, amountOut) = GiddyUniswap.calcSwapV3(UNISWAP_ROUTER, UNISWAP_QUOTER, WMATIC_TOKEN, BALANCER_TOKEN, 10000, priceImpact, amountOut);
  ...
  function deposit(bytes calldata auth, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    uint256 tokenValue = GiddyUniswap.swapTokensSimpleV3(UNISWAP_ROUTER, USDC_TOKEN, WMATIC_TOKEN, 500, address(this), value);
    tokenValue = GiddyUniswap.swapTokensSimpleV3(UNISWAP_ROUTER, WMATIC_TOKEN, BALANCER_TOKEN, 10000, address(this), tokenValue);

    uint256 shares = contractShares == 0 ? tokenValue * INIT_SHARES : tokenValue * contractShares / getContractBalance();
    userShares[owner] += shares;
    contractShares += shares;

    if (!IERC20(BALANCER_TOKEN).approve(LENDING_POOL, tokenValue)) {
      revert();
    }
    ILendingPool(LENDING_POOL).deposit(BALANCER_TOKEN, tokenValue, address(this), 0);
  ...
  function depositGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    uint256 tokenValue = GiddyUniswap.swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, address(this), giddyValue);
    tokenValue = GiddyUniswap.swapTokensSimpleV3(UNISWAP_ROUTER, USDC_TOKEN, WMATIC_TOKEN, 500, address(this), tokenValue);
    tokenValue = GiddyUniswap.swapTokensSimpleV3(UNISWAP_ROUTER, WMATIC_TOKEN, BALANCER_TOKEN, 10000, address(this), tokenValue);
    ...
    emit Deposit(_msgSender(), giddyToUsdc(giddyValue));
  }

  function withdrawCalc(uint256 shares, uint256 fap) external override returns (uint256 amountOut, uint256 priceImpact) {
    priceImpact = sharesToValue(shares);
    amountOut = priceImpact;
    (priceImpact, amountOut) = GiddyUniswap.calcSwapV3(UNISWAP_ROUTER, UNISWAP_QUOTER, BALANCER_TOKEN, WMATIC_TOKEN, 10000, priceImpact, amountOut);
  ...
  function withdraw(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    ILendingPool(LENDING_POOL).withdraw(BALANCER_TOKEN, value, address(this));
    value = GiddyUniswap.swapTokensSimpleV3(UNISWAP_ROUTER, BALANCER_TOKEN, WMATIC_TOKEN, 10000, address(this), value);
    value = GiddyUniswap.swapTokensSimpleV3(UNISWAP_ROUTER, WMATIC_TOKEN, USDC_TOKEN, 500, address(this), value);
    value = deductFeeUsdc(value, fap);
  ...
  modifier deductEarningsFee() {
    ...
    if (earnings > 0) {
      ...
      if (fee > 0) {
        ILendingPool(LENDING_POOL).withdraw(BALANCER_TOKEN, fee, address(this));
        uint256 value = IERC20(BALANCER_TOKEN).balanceOf(address(this));
        if (value >= COMPOUND_THRESHOLD) {
          value = GiddyUniswap.swapTokensSimpleV3(UNISWAP_ROUTER, BALANCER_TOKEN, WMATIC_TOKEN, 10000, address(this), value);
          GiddyUniswap.swapTokensSimpleV3(UNISWAP_ROUTER, WMATIC_TOKEN, USDC_TOKEN, 500, GIDDY_CONFIG.feeAccount(), value);
        }
      }
