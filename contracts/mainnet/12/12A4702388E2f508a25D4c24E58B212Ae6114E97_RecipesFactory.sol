...
contract RecipesFactory is VRFConsumerBaseV2, Ownable  {
  ...
  uint public tokenFeeDollars;
  uint public maticFeeDollars;
  uint public devPerc;
  uint public maticOverallPerc;
  uint public tokenRewardPoolPerc;
  uint public tokenOverallPerc;
  ...
  {   
    ...
    devPerc = 10;
    maticOverallPerc = 15;
    tokenFeeDollars = 2125;
    maticFeeDollars = 375;
    tokenRewardPoolPerc = 75;
    tokenOverallPerc = 85;  
  }
  ...
  function purchase(uint8 mintingPack, string memory referral) external payable {
    ...
    uint tokenPart = uint(tokenQuantity) * priceOracleInstance.getUsdtToToken(tokenFeeDollars); //2125 is 85% of 25 dollars;
    uint maticPart = maticFeeDollars * uint(tokenQuantity) * (10 ** 18) * (10 ** 6) / (uint(priceOracleInstance.getLatestPrice())); // 375 is 15% of 25 dollars;
    uint _DevMaticPart = devPerc * 375 * uint(tokenQuantity) * (10 ** 18) * (10 ** 6) / (uint(priceOracleInstance.getLatestPrice()) * maticOverallPerc);
    ...
    bool sent = rdkToken.transferFrom(msg.sender, rewardPoolAddress, tokenPart * tokenRewardPoolPerc / tokenOverallPerc);
    require(sent, "Collection Recipes: rdk transfer failed");
    sent = rdkToken.transferFrom(msg.sender, retirementPlanAddress, tokenPart - (tokenPart * tokenRewardPoolPerc / tokenOverallPerc));
    require(sent, "Collection Recipes: rdk transfer failed");
  ...
  function setMintFees(
      uint _tokenFeeDollars, 
      uint _maticFeeDollars, 
      uint _devPerc,
      uint _maticOverallPerc,
      uint _tokenRewardPoolPerc,
      uint _tokenOverallPerc
      ) external onlyOwner {
    tokenFeeDollars = _tokenFeeDollars;
    maticFeeDollars = _maticFeeDollars;
    devPerc = _devPerc;
    maticOverallPerc = _maticOverallPerc;
    tokenRewardPoolPerc = _tokenRewardPoolPerc;
    tokenOverallPerc = _tokenOverallPerc;
  }
