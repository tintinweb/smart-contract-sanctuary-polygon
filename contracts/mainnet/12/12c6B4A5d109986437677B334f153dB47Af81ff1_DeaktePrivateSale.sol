...
/**
 *Deakte Private Sale Deakte.com
*/
...
interface IPrivateIDOImmutables {
  function token() external view returns(IERC20Metadata);
}
interface IPrivateIDOState {
...
contract DeaktePrivateSale is IPrivateIDOImmutables, IPrivateIDOState, IPrivateIDOEvents, Ownable {
    ...
    constructor(address _token, string memory tEvents, uint256 _notBeforeBlock, uint256 _notAfterBlock) {
        ...
        require( _notAfterBlock > _notBeforeBlock, "invalid Private Sale schedule"); // IPS
    ...
    function collectFunds(address fundsAddress) external onlyOwner afterEventsalePeriod{
    ...
    modifier inEventsalePeriod() {
        require(block.timestamp > notBeforeBlock, "Private-sale has not been started"); 
        require(block.timestamp < notAfterBlock, "Private-sale has already ended"); 
        _;
    }
    modifier afterEventsalePeriod() {
        require(block.timestamp > notAfterBlock, "Private-sale is still ongoing"); 
        _;
    }
