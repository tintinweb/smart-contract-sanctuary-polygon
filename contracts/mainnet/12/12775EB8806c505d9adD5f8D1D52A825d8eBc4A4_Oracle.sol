...
contract Oracle is Epoch {
    ...
    mapping(string => address) private tokenAggregator;
    ...
    public Epoch(_period, _startTime, 0) 
    {
        ...
        {
            router = 0x26e1E10980f101b44C978D3279F7D321A7dDD54f;
            usdc = 0x7C78aB011Fb312c2DF48e8bc87745De9b41875c5;

            tokenAggregator["WMATIC"] = 0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada;
            tokenAggregator["WETH"] = 0x0715A7794a1dc8e42615F059dD6e406A6594651A;
            tokenAggregator["USDC"] = 0x572dDec9087154dC5dfBB1546Bb62713147e0Ab0;
        }
        else 
        {
            ...
            tokenAggregator["WMATIC"] = 0xAB594600376Ec9fD91F8e885dADF0CE036862dE0;
            tokenAggregator["WETH"] = 0xF9680D99D6C9589e2a93a78A04A279e509205945;
            tokenAggregator["USDC"] = 0xfE4A8cc5b5B2366C1B58Bea3858e81843581b2F7;
        }
    ...
    function getPriceWToken(address token) public view returns (uint)
    {
        address[] memory path = new address[](2);
        path[0] = token; //tpen
        path[1] = IRouter(router).WETH(); //wbnb
        
        return IRouter(router).getAmountsOut(1 ether, path)[1];
    }
    function getPriceToken(address token) public view returns (uint)
    {
        ...
        uint256 priceW = getPrice("WMATIC", 8, 18);
        uint256 price  = getPriceWToken(token);
        
        return priceW.mul(price).div(1e18);
    }

   function getPrice(string memory tokenName, uint8 baseDecimals, uint8 decimals) public view returns (uint256) 
    {
        int p = 0;

        int basePrice = getLatestPrice(tokenName);
    ...
    function getLatestPrice(string memory tokenName) public view returns (int) {
        ...
        (
            ...
            /*uint timeStamp*/,
            /*uint80 answeredInRound*/
        ) = AggregatorV3Interface(tokenAggregator[tokenName]).latestRoundData();
        return price;
    }
    /* ========== MUTABLE FUNCTIONS ========== */
