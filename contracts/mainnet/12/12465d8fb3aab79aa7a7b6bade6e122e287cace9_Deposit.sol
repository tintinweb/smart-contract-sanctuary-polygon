...
contract Deposit is AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  function deposit(uint256 amount) external {
    ...
    coinToken.transferFrom(to, address(this), amount);
    depositedAmounts[to] += amount;
    emit Deposited(to, amount);
  }

  function getDeposited(address to) external view returns (uint256) {
    uint256 amount = depositedAmounts[to];
    return amount;
  }
}
