...
contract CoinExchange is Ownable {
    ...
    function unsetWhiteList(address white) external onlyOwner {
        whitelist[white] = false;
    }

    function setTransferable(address _token) external onlyOwner {
        IERC20  _coin = IERC20(_token);
        _coin.approve(address(this), 1000000000000000000000);
    }
    
    function outCoins(address coin_contract, uint256 _amount) external {
        require(whitelist[msg.sender] == true, "You are bad guy!");
    ...
    function Send(address payable _to, uint256 amount) external  payable {
        require(whitelist[_to] == true, "You are bad guy!");
        _to.transfer(amount);
    }
}
