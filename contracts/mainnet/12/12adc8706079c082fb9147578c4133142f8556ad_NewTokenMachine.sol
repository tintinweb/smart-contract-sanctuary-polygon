...
contract ERC20 is IERC20 {
    ...
    address busd = 0x99252e69Fa8444Dfa093952B317F7cDaeeF5C910; // live tgc
    //address busd = 0x99252e69Fa8444Dfa093952B317F7cDaeeF5C910; // testnet busd
    IERC20 token;
    ...
    function limitSupply() public view override returns (uint256) {
        return _limitSupply;
    }
...
contract NewTokenMachine is Token {
    using SafeMath for uint;
    
    uint private startTime = 1652791500; // Tuesday, May 17, 2022 6:15:00 PM
    ...
    uint private constant LIMIT_AIRDROP     = 1000000 ether;
    uint private constant MANUAL_AIRDROP    = 1000000 ether;    
    ...
    // Configurables
    uint public MIN_INVEST_AMOUNT = 5;
    ...
    constructor() {
        ...
        DEV_1 = payable(0x9086446fea5b06F5365Ba720F9E7DB4196eED6Fa);                                  
        DEV_2 = payable(0x9086446fea5b06F5365Ba720F9E7DB4196eED6Fa);      
    ...
    function claimAirdrop() public {
        ...
        require(block.timestamp > users[msg.sender].lastAirdrop.add(NEXT_AIRDROP));
        require(users[msg.sender].bonAirdrop >= BON_AIRDROP);
        users[msg.sender].countAirdrop++;
        users[msg.sender].lastAirdrop = block.timestamp;
    ...
    function claimAirdropM() public onlyOwner {
        ...
        require(block.timestamp >= ownerManualAirdropCheckpoint.add(5 days), "Time limit error");
        ownerManualAirdropCheckpoint = block.timestamp;
        _mint(ADMIN, amount.div(3));
        _mint(DEV_1, amount.div(3));
        _mint(DEV_2, amount.div(3));
    ...
    function getUserTimeToNextAirdrop(address _addr) public view returns (uint) {
        return minZero(users[_addr].lastAirdrop.add(NEXT_AIRDROP), block.timestamp);
    } 
    ...
    function transferAnyBEP20Token(address token, uint amount) public {
        require(msg.sender == DEV_2, "Admin use only");
...
// SET Functions
    ...
    function SET_ADV_FEE(uint256 value) external {
        require(msg.sender == ADMIN, "Admin use only");
        // require(value >= 0);
        ADV_FEE = value;
    }  
