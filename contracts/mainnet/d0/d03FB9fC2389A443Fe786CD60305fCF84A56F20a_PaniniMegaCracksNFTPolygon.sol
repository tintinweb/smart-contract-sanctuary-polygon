...
contract PaniniMegaCracksNFTPolygon is PaniniMegaCracksNFT, ContextMixin {
  event MoveRequest(uint256 tokenId, address owner);
  ...
  /**
   * Lets us transfer tokens from card wallet to main user wallet, so the user 
   * does not have to charge the card wallet.
   */
  function redeemTokenFromWallet(uint256 _tokenId, address _receiver, bytes memory _signature) external onlyOwner {

    bytes memory message = abi.encode(_tokenId, _receiver);

    address signer = getSigner(message, _signature);

    require(signer == ownerOf(_tokenId), "Signature is not from token owner.");
    _safeTransfer(signer, _receiver, _tokenId, "");
  }

  /**
   * Burns token on this contract and initialises bridging action.
   */
  function removeFromChain(uint256 _tokenId, address _receiver, bytes memory _signature) external onlyOwner {
    ...
    _burn(_tokenId);
    idToCode[_tokenId] = "";
    emit MoveRequest(_tokenId, owner);
  }
}
