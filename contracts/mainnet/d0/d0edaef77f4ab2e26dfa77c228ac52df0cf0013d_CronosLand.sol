...
contract CronosLand is ERC721Enumerable, Ownable, ReentrancyGuard  {
  using Strings for uint256;
  ...
  function mintBronze(uint256 _mintAmount) public nonReentrant() {
    ...
    uint256 bronze = 1;
    require(limit[msg.sender] + _mintAmount <= 50, "LIMIT REACHED");
   ...
   if (msg.sender != owner()) {
        if(onlyWhitelisted == true) {
            ...
            IERC20(erc20Address).safeTransferFrom(msg.sender, treasuryAddress, bronzePrice * _mintAmount);
   }
   ...
   if (msg.sender != owner()) {
        if(onlyWhitelisted == true) {
            ...
            IERC20(erc20Address).safeTransferFrom(msg.sender, treasuryAddress, silverPrice * _mintAmount);
   }
    ...
    uint256 gold = 3;
    require(limit[msg.sender] + _mintAmount <= 50, "LIMIT REACHED");
    require(maxMintGold + _mintAmount <= 150, "SOLD OUT");
    uint256 supply = totalSupply();
    require(_mintAmount > 0, "need to mint at least 1 NFT");
    require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
    require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
   ...
   if (msg.sender != owner()) {
        if(onlyWhitelisted == true) {
            ...
            IERC20(erc20Address).safeTransferFrom(msg.sender, treasuryAddress, goldPrice * _mintAmount);
   }
  ...
  function mintDiamond(uint256 _mintAmount) public nonReentrant() {
    ...
    if (msg.sender != owner()) {
        if(onlyWhitelisted == true) {
            ...
            IERC20(erc20Address).safeTransferFrom(msg.sender, treasuryAddress, diamondPrice * _mintAmount);
   }
