    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public creatorRoyaltyFee;
    uint256 public clientRoyaltyFee;
    ...
    mapping(address => bool) public whitelisted; // maps user account to boolean (if true account is whitelisted)
    ...
    event SetCreatorAndClientRoyaltyFee(
        uint256 _newCreatorRoyaltyFee,
        uint256 _newClientRoyaltyFee,
    ...
    function initialize(
        ...
        uint96 _royaltyFee,
        uint256 _creatorRoyaltyFee,
        uint256 _clientRoyaltyFee
        ...
        creatorRoyaltyFee = _creatorRoyaltyFee;
        clientRoyaltyFee = _clientRoyaltyFee;
        admin = owner();
    }
    ...
    function createERC721Collection(
        ...
        require(
            (creatorRoyaltyFee + clientRoyaltyFee) == 100,
            "Invalid share partition."
        );
        payees = [msg.sender, _receiver];
        shares = [creatorRoyaltyFee, clientRoyaltyFee];
     ...
     */
    function setCollectionIsDestroyed(
        address collection
    ...
    /**
    * @dev Updates `creatorRoyaltyFee` to `_newCreatorRoyaltyFee` and
    * `clientRoyaltyFee` to `_newClientRoyaltyFee`
    * @param _newCreatorRoyaltyFee.
    * @param _newClientRoyaltyFee.
    *
    * Emits {SetCreatorAndClientRoyaltyFee} event
     */
    function setCreatorAndClientRoyaltyFee(
        uint256 _newCreatorRoyaltyFee,
        uint256 _newClientRoyaltyFee
    ) external onlyOwner whenNotPaused {
        creatorRoyaltyFee = _newCreatorRoyaltyFee;
        clientRoyaltyFee = _newClientRoyaltyFee;
        emit SetCreatorAndClientRoyaltyFee(
            _newCreatorRoyaltyFee,
            _newClientRoyaltyFee,
            msg.sender
        );
    }

    /**
     * @dev Returns whether `account` is whitelisted or not\
     * @param account.
     */
     ...
     *
     * Emits {AccountWhitelistUpdated} event
     */
     ...
     *
     * Emits {AccountsWhitelistUpdated} event
     */
    function includeAllInWhitelist(
     ...
     * Emits {AccountsWhitelistUpdated} event
     */
    function excludeAllFromWhitelist(
        address[] memory accounts
    ) external onlyOwner {
        for (uint256 account = 0; account < accounts.length; account++) {
            if (
