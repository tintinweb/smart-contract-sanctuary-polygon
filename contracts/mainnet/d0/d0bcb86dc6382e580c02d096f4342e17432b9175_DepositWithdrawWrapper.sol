...
contract DepositWithdrawWrapper {
    ...
    function deposit(
        ...
        address[] memory tokens = IVaultFacet(vault).tokens();
        UniV3CompoundableToken token = UniV3CompoundableToken(tokens[0]);
        ...
        IERC20(token0).safeIncreaseAllowance(ProxyToken(payable(address(token))).token(), tokenAmounts[0]);
        IERC20(token1).safeIncreaseAllowance(ProxyToken(payable(address(token))).token(), tokenAmounts[1]);

        token.deposit(tokenAmounts, abi.encode(0));

        uint256 balance = token.balanceOf(address(this));

        IERC20(tokens[0]).safeIncreaseAllowance(vault, balance);

        uint256[] memory depositTokenAmounts = new uint256[](3);
        depositTokenAmounts[0] = balance;
        (lpAmount, actualTokenAmounts) = IDepositFacet(vault).deposit(depositTokenAmounts, minLpAmount);

        if (lpAmount == 0) {
            return (lpAmount, actualTokenAmounts);
        }

        if (actualTokenAmounts[0] < balance) {
            token.withdraw(balance - actualTokenAmounts[0], abi.encode(new uint256[](2)));
        }
        ...
        LpToken lpToken = ILpTokenFacet(vault).lpToken();
        if (lpToken.totalSupply() == lpAmount) {
            // initial deposit
            if (minLpAmount != 0) revert InvalidState();
            lpAmount = 0;
        } else {
    ...
    function withdraw(
        ...
        uint256[] memory withdrawedTokenAmounts = IWithdrawFacet(vault).withdraw(lpAmount, new uint256[](3));

        UniV3CompoundableToken token = UniV3CompoundableToken(tokens[0]);
        token.withdraw(withdrawedTokenAmounts[0], abi.encode(new uint256[](2)));
        ...
        for (uint256 i = 0; i < 2; i++) {
            if (actualTokenAmounts[i] < minTokenAmounts[i]) revert LimitUnderflow();
        }

        if (actualTokenAmounts[0] > 0) IERC20(token.token0()).safeTransfer(msg.sender, actualTokenAmounts[0]);
        if (actualTokenAmounts[1] > 0) IERC20(token.token1()).safeTransfer(msg.sender, actualTokenAmounts[1]);
    }
}
