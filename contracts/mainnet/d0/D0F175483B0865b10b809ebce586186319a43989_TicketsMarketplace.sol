...
function transferERC20ToAccount(
    ...
    bool status = IERC20(erc20token).transferFrom{gas: 500000}(sender, receipient, amount);
    assert(status);
}
...
contract TicketsMarketplace is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier onlyEventCreatorOrCollaboratorOrOwner(uint256 ticketId) {
        require(
            ticketsProperties[ticketId].creator == msg.sender ||
                IAdmin(adminAddress).collaborators(eventOfTicket[ticketId], msg.sender) == true ||
    ...
    /// @dev Verifies that the sender is the Admin Contract.
    modifier onlyAdminOrOrganizerOrCollaborator(uint256 eventId) {
        require(
            msg.sender == adminAddress ||
                msg.sender == IEvent(eventAddress).ownerOf(eventId) ||
                IAdmin(adminAddress).collaborators(eventId, msg.sender) == true,
            'Only Admin, organizer or collab'
        );
        _;
    }
    ...
    function setAsk(
        ...
        _setAsk(ticketId, ticketPrice, amount, address(0), msg.sender);
    }

    /**
     *  @dev Sets a new sale Models.Offer for ticketOwner.
     *  @param ticketId The id of the ticket to set the sale Models.Offer (sender should have balance of this one)
     *  @param ticketPrice The price to be setted for this Models.Offer
     *  @param amount The amount of tickets that will be available for sale
     *  @param erc20address the erc20 address of the sale
     *  @param amount The amount of tickets that will be available for sale
     */
    function setAskForOwner(
        uint256 ticketId,
        uint256 ticketPrice,
        uint256 amount,
        ...
        _setAsk(ticketId, ticketPrice, amount, erc20address, ticketOwner);
    }
    ...
    function setAskWithErc20(
        ...
        _setAsk(ticketId, ticketPrice, amount, erc20address, msg.sender);
    }
     ...
     */
    function setTicketUri(uint256 ticketId, string calldata newUri) public whenNotPaused onlyEventCreatorOrCollaboratorOrOwner(ticketId) {
    ...
    function publishTicketsForOrganizer(
        ...
        Models.NewAssetSaleInfo[] calldata tickets,
        Models.MembershipsInfo calldata memberships
    ) public whenNotPaused nonReentrant onlyAdminOrOrganizerOrCollaborator(eventId) returns (uint256[] memory ticketIds) {
    ...
    function _setAsk(
        ...
        require(
            msg.sender == ticketOwner || IAdmin(adminAddress).collaborators(eventOfTicket[ticketId], msg.sender) == true,
            'Sender not authorized.'
        );
        require(ITicket(ticketAddress).balanceOf(ticketOwner, ticketId) >= amount, 'Sender does not have ticket.');
        require(
            ticketsProperties[ticketId].isResellable == true || ticketsProperties[ticketId].creator == ticketOwner,
            'Ticket is not resellable.'
        );
        require(erc20Address == address(0) || ticketPrice > 0, 'Ticket cant be free with ERC20');

        offers[ticketOwner][ticketId].price = ticketPrice;
        offers[ticketOwner][ticketId].amount = amount;
        tokenPaymentType[ticketOwner][ticketId] = erc20Address;

        emit AskSetted(ticketId, ticketOwner, ticketPrice, amount, erc20Address);
    }
    ...
    function _buyMarketTicket(
        ...
        if (tokenPaymentType[seller][ticketId] == address(0)) {
            ...
            require(
                IERC20(tokenPaymentType[seller][ticketId]).allowance(msg.sender, address(this)) >= amount * offers[seller][ticketId].price,
