...
/**
  ...
  ~ 0.005 MATIC Minimum Investment.
  ...
  ~ 50 tickets max per user. 0.25 MATIC = 50 tickets.
  ~ 0.005 MATIC = 1 per ticket. based on user deposit/compound.
  ~ 90% of the pot will be given to the winning address.

  Whale Control Feature:
  ~ 36 hours cut off time.
  ~ 6 hours withdraw cooldown.
  ~ 10 MATIC max withdrawal.
  ~ 20 MATIC max deposits per wallet.
...
contract PolygonCommunityMiner {
    ...
    uint256 public MIN_INVEST = 5 * 1e15; /** 0.005 MATIC  **/
	uint256 public WITHDRAW_COOLDOWN = 6 * 60 * 60; /** 6 hours  **/
    uint256 public WITHDRAW_LIMIT = 10 ether; /** 10 MATIC  **/
    uint256 public WALLET_DEPOSIT_LIMIT = 20 ether; /** 20 MATIC  **/
  ...
  require(msg.sender == owner , "not the owner");
   ...
   /** will auto execute, when condition is met. buy, hatch and sell, can be triggered manually by admin if theres no user action. **/
    ...
    /** How many miners and eggs per day user will recieve for 1 MATIC deposit **/
