...
contract MaxityRouter is Ownable,IMRouter {
    ...
    //mint721
    function mintMax721(address _token,address designer, uint256 count,uint _futureRoyalty,string memory tokenURIOrigin,bytes calldata ) external override returns (uint[] memory tokenids){
        require (IMaxity721Token(_token).inWhiteList(msg.sender),'not owner');
        return IMaxity721Token(_token).mint(designer, msg.sender, count,_futureRoyalty, tokenURIOrigin);
    }

    //mintAndSellMax721
    function mintAndSellMax721(address _token,address designer, uint256 count,uint _futureRoyalty,string memory tokenURIOrigin
        ,uint unit_price
        ,bytes calldata ) external returns (uint[] memory tokenids){
        require (IMaxity721Token(_token).inWhiteList(msg.sender),'not owner');
        uint256 [] memory new_tokenids =  IMaxity721Token(_token).mint(designer, market, count,_futureRoyalty, tokenURIOrigin);
        IMaxityMarketPlace(market).sellByFixPrice(_token,new_tokenids,unit_price);

        return new_tokenids;
    }

    //mintAndAuctionMax721
    function mintAndAuctionMax721(address _token,address designer, uint256 count,uint _futureRoyalty,string memory tokenURIOrigin
        ,uint base_price,uint incre_price,uint deadline
        ,bytes calldata _calldata) external returns (uint[] memory tokenids){
        uint256 [] memory new_tokenids =  IMaxity721Token(_token).mint(designer, market, count,_futureRoyalty, tokenURIOrigin);
        IMaxityMarketPlace(market).auctionMax721(_token,new_tokenids,base_price,incre_price,deadline);
        return new_tokenids;
    }

    function buyNative(address _token,uint256 _tokenid) public payable {
        IWMATIC(wrappedNative).deposit{value: msg.value}();
        assert(IWMATIC(wrappedNative).approve(market, msg.value));
        IMaxityMarketPlace(market).buy(_token,_tokenid);
        IERC721(_token).transferFrom(address(this), msg.sender, _tokenid);
    }

    function bidNative(address _token,uint256 _tokenid,uint256 amount) public payable {
        IWMATIC(wrappedNative).deposit{value: msg.value}();
        assert(IWMATIC(wrappedNative).approve(market, msg.value));
        IMaxityMarketPlace(market).bid(_token,_tokenid,amount);
        IERC721(_token).transferFrom(address(this), msg.sender, _tokenid);
    }

    function delist(address _token,uint256 _tokenid) public  {
        IMaxityMarketPlace(market).delist(_token,_tokenid);
    }
    
    function profitShare(uint256 amountU) external override {
        require(maxityProfitShare!=address(0x0),'profit share is zero addr');
    ...
    function userLend(uint256 _pid,uint256 _lendAmount,address _lender) external override {
        require(maxityPool!=address(0x0),'maxityPool is zero addr');
...
interface IMRouter  {
    ...
    //mint721
    function mintMax721(address _token,address designer, uint256 count,uint _futureRoyalty,string memory tokenURIOrigin,bytes calldata _data) external returns (uint[] memory tokenids);

    //mintAndSellMax721
    function mintAndSellMax721(address _token,address designer, uint256 count
        ,uint _futureRoyalty,string memory tokenURIOrigin
        ,uint unit_price
        ,bytes calldata _data) external   returns (uint[] memory tokenids);

    //mintAndAuctionMax721
    function mintAndAuctionMax721(address _token,address designer, uint256 count
        ,uint _futureRoyalty,string memory tokenURIOrigin
        ,uint base_price,uint incre_price,uint deadline
        ,bytes calldata _data) external  returns (uint[] memory tokenids);

    //通过主链币购买
    function buyNative(address _token,uint256 _tokenid) external payable ;

    //通过主链币竞价
    function bidNative(address _token,uint256 _tokenid,uint256 amount) external payable ;
