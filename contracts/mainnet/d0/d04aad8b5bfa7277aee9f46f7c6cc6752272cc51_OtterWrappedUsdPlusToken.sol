...
contract OtterWrappedUsdPlusToken is BaseContractV2, ERC20Upgradeable {
    address public usdc;
    address public usdPlus;
    address public wUsdPlus;
    IUSDPlusMarket public usdPlusMarket;
    address public pearlBank;

    function initialize(
        address usdc_,
        address usdPlus_,
        address wUsdPlus_,
        address usdPlusMarket_,
        address pearlBank_
    ) public initializer {
        __BaseContract_init();
        __ERC20_init('ocUSD+', 'ocUSD+');
        usdc = usdc_;
        usdPlus = usdPlus_;
        wUsdPlus = wUsdPlus_;
        usdPlusMarket = IUSDPlusMarket(usdPlusMarket_);
        pearlBank = pearlBank_;
    }
    ...
    function mint(uint256 amount_) public returns (uint256) {
        ...
        IERC20(usdc).approve(address(usdPlusMarket), amount_);
        uint256 wrappedAmount = usdPlusMarket.wrap(
            usdc,
            amount_,
            address(this)
        );
    ...
    function burn(uint256 amount_) public returns (uint256) {
        _burn(msg.sender, amount_);
        return usdPlusMarket.unwrap(address(usdc), amount_, msg.sender);
    }

    function usdPlusAmount(uint256 amount_) public view returns (uint256) {
        return IWrappedUsdPlusToken(wUsdPlus).convertToAssets(amount_);
    }
    ...
    function _afterTokenTransfer(
        ...
        // auto unwrapped to USD+ if payout from pearl bank
        if (from == pearlBank && to != address(0)) {
            // console.log('unwrapping from pearl bank: %s', amount);
            IERC20(wUsdPlus).approve(address(usdPlusMarket), amount);
            usdPlusMarket.unwrap(usdPlus, amount, to);
            _burn(to, amount);
        }
    }
}
