...
contract IDO is Ownable{
    ...
    function adjustMaxContribution(uint _amount) external{
    ...
    function adjustClaims(bool _state) external{
        require(block.timestamp >= (start+CONTRIBUTION_PERIOD), "Contribution period is not over");
    ...
    function rescueFunds(address _token, uint _amount) external{
    ...
    function saleInfo() external view returns(uint _start, uint _contributionEnd, uint _amountForSale, uint _price, bool _openForClaims, uint _target){
        return(start, start+CONTRIBUTION_PERIOD, amountForSale, price, openForClaims, saleTarget);
    }
