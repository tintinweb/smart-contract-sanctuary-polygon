...
contract Book is ERC721Drop {
    ...
    event UserUpdate(string indexed name, string indexed role, uint256 tokenId); 
    event NewEntry(string indexed name, string indexed role, string indexed note, uint256 tokenId, uint256 entryCount); 
    ...
    function updateProfile(uint256 _tokenId, string memory _name, string memory _role) public {
        require(msg.sender == ownerOf(_tokenId), "You are not the Owner of this token!");
        users[_tokenId].name = _name;
        users[_tokenId].role = _role;
        emit UserUpdate(_name, _role, _tokenId);
    }

    function writeEntry(uint256 _tokenId, string memory _note, string memory _uri) public {
        require(msg.sender == ownerOf(_tokenId), "You are not the Owner of this token!");
        Entry memory entry = Entry(users[_tokenId].name, users[_tokenId].role, _note, _uri, msg.sender, block.timestamp);
        entries.push(entry);
        emit NewEntry( users[_tokenId].name, users[_tokenId].role, _note, _tokenId, entryCount());
    }
    ...
    function getEntries(uint256 page, uint256 pageSize) external view returns (Entry[] memory, uint256) {
        require(pageSize > 0, "page size must be positive");
        require(page == 0 || page*pageSize <= entries.length, "out of bounds");
        uint256 actualSize = pageSize;
        if ((page+1)*pageSize > entries.length) {
            actualSize = entries.length - page*pageSize;
        }
        Entry[] memory res = new Entry[](actualSize);
        for (uint256 i = 0; i < actualSize; i++) {
            res[i] = entries[page*pageSize+i];
        }
