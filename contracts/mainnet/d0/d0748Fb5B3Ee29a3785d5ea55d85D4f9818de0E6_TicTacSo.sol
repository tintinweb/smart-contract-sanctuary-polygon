...
contract TicTacSo {
    ...
    function getGames(address to) public view returns (uint256[] memory) {
        return runningGames[to];
    }

    function getArchivedGames(address to) public view returns (uint256[] memory) {
        return archivedGames[to];
    }
    ...
    function checkWinIn3x(
        ...
        for (uint8 sy = 0; sy < game.size; sy++) {
            for (uint8 sx = 0; sx < game.size; sx++) {
                if (game.board[sy][sx] == 0) return 0;
            }
        }
