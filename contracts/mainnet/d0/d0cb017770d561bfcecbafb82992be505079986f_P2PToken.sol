...
abstract contract Blacklistable is OwnableUpgradeable {
   ...
   * @param _from, to, caller The addresses to check
   */
  modifier notBlacklisted(
    ...
    require(!_blacklisted[_from], "Blacklistable: account is blacklisted");
    require(!_blacklisted[_to], "Blacklistable: account is blacklisted");
    require(!_blacklisted[_caller], "Blacklistable: account is blacklisted");
    _;
  }
  ...
  Blacklistable
{
  ...
  uint256 constant maxSupply = 1000000 * 1e18;
  
  function initialize(
  
  ) public initializer {
    __ERC20_init("P2PBIT", "P2P");
    __ERC20Burnable_init();
    __Pausable_init();
    __Ownable_init();
    __ERC20Permit_init("P2PBIT");
  }
  ...
  function mint(address to, uint256 amount) public onlyOwner {
    require(amount + tokensMinted <= maxSupply, "Max supply minted");
    tokensMinted += amount;
    _mint(to, amount);
  }
  ...
  function _beforeTokenTransfer(
    ...
    address to,
    uint256 amount
  ) internal override whenNotPaused notBlacklisted(from, to, msg.sender) {
