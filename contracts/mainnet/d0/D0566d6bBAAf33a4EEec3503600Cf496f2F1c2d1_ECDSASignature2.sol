...
contract ECDSASignature2 is Assembly, IECDSASignature2 {
    ...
    function insertNonce(
        uint256 nonceToInsert,
        uint256 nonce,
        uint256 timestamp,
        bytes[] memory signatures
    ) external {
        bytes32 hash = keccak256(abi.encodePacked(nonceToInsert, msg.sender));
        _verifyMessage(hash, nonce, timestamp, signatures);
        _useNonce(nonceToInsert);
    }

    function isNonceUsed(uint256 nonce) external view returns (bool) {
        return _checkNonce(nonce);
    }
    ...
    function setSignatureLifetime(
        uint256 time,
        uint256 nonce,
        uint256 timestamp,
        bytes[] memory signatures
    ) external {
        bytes32 hash = keccak256(abi.encodePacked(time, msg.sender));
        _verifyMessage(hash, nonce, timestamp, signatures);
...
abstract contract Config {
    uint256 private _timePeriodInSeconds = 5 minutes;

    event ChangeLifeTime(uint256 time);
...
abstract contract Verifier is Signers, Encoder {
    ...
    function _verifyByTheSigners(
        ...
        for (uint j = 0; j < signatures.length; j++) {
            ...
            for (uint i = 0; i < sLength; i++) {
                ...
                // If the signatory has already been verified, skip to the next
                ...
                if (result) {
                    ...
                    verifiedSigners[verifiedCount++] = signer; // Mark the signer as verified
    ...
    /// @param signatures Signatures created by each of the signatories
    /// @return uint8: 1 = nonce was already used, 2 = lifetime has expired, 0 = Ok - Success, 3 = hash or signature is invalid,
    function _signatureStatus(
        ...
        if (nonceUsed[nonce]) return 1;
        if (block.timestamp > timestamp) return 2;
