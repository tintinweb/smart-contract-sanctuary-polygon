...
contract BundleExecutor is FlashLoanReceiverBase {
    ...
    function executeOperation(
        ...
        address tokenIn;
        address tokenOut;
        address dex1RouterAddress;
        address dex2RouterAddress;
        (amount, tokenIn, tokenOut, dex1RouterAddress, dex2RouterAddress) = abi
            .decode(params, (uint256, address, address, address, address));
        ...
        // console.log("tokenBalance", IERC20(assets[0]).balanceOf(address(this)));
    ...
    function deposit(uint256 amount, address tokenAddress) public onlyOwner {
        require(amount > 0, "Deposit amount must be greater than 0");
        IERC20(tokenAddress).transferFrom(msg.sender, address(this), amount);
    }

    function withdraw(uint256 amount, address tokenAddress) public onlyOwner {
        uint256 tokenBalance = getERC20Balance(tokenAddress);
        require(amount <= tokenBalance, "Not enough amount deposited");
    ...
    function makeArbitrage(
        ...
        uint256 amountIn = getERC20Balance(tokenIn);
        console.log("amountIn", amountIn);
        amountIn = amount;
        console.log("amountIn", amountIn);
        address firstDexSwapRouterAddress;
        address secondDexSwapRouterAddress;
        (firstDexSwapRouterAddress, secondDexSwapRouterAddress) = _comparePrice(
            amountIn,
            tokenIn,
            tokenOut,
            dex1RouterAddress,
        ...
        require(firstDexSwapRouterAddress != address(0), "No swap happens!");
        uint256 amountOut = _swap(
            amountIn,
            firstDexSwapRouterAddress,
            tokenIn,
            tokenOut
        );
        _swap(amountOut, secondDexSwapRouterAddress, tokenOut, tokenIn);
    }
    ...
    function _comparePrice(
        ...
        if (dex1Price > dex2Price) {
            require(
                _checkIfArbitrageIsProfitable(amount, dex1Price, dex2Price),
                "Arbitrage not profitable"
            );
            return (dex1RouterAddress, dex2RouterAddress);
        } else if (dex1Price < dex2Price) {
            require(
                _checkIfArbitrageIsProfitable(amount, dex2Price, dex1Price),
                "Arbitrage not profitable"
            );
            return (dex2RouterAddress, dex1RouterAddress);
        } else {
            // return (dex1RouterAddress, dex2RouterAddress);
