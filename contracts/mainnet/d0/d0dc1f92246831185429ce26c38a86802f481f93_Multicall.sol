...
contract Multicall {
    ...
    function uniswapV3FlashCallback(uint256 fee0,uint256 fee1,bytes calldata data) external {
        ...
        (fee0 > fee1) ? IERC20(IUniV3Pool(msg.sender).token0()).transfer(msg.sender,fee0) : IERC20(IUniV3Pool(msg.sender).token1()).transfer(msg.sender,fee1);
    }

    function flashCallback(uint256 fee0,uint256 fee1,bytes calldata data) external {
        fee0 += am; fee1 += am;
        multicall(abi.decode(data, (bytes[])));
        (fee0 > fee1) ? IERC20(IKyberV3Pool(msg.sender).token0()).transfer(msg.sender,fee0) : IERC20(IKyberV3Pool(msg.sender).token1()).transfer(msg.sender,fee1);
    }
    ...
    function flashRoute(FlashRouteParams calldata _route) public {
        am = _route.amIn;
    ...
    function route(RouteParams calldata _route) public {
        IERC20(_route.tIn).transferFrom(msg.sender, address(this), _route.amIn);
        am = _route.amIn;
        multicall(_route.callPath);
        require(am >= _route.amOut, "amOut");
        IERC20(_route.tOut).transfer(msg.sender, am);
        delete am;
    }
    ...
    function uniV3Immutables(address pool)public view returns (Immutables memory){
        return Immutables(
                    ...
                    IUniV3Pool(pool).fee(),
                    selector("uniV3(address,bool)")
                );
    }

    function kyberV3Immutables(address pool)public view returns (Immutables memory){
        return Immutables(
                    IKyberV3Pool(pool).token0(),
                    IKyberV3Pool(pool).token1(),
                    IKyberV3Pool(pool).swapFeeUnits() * 10,
                    selector("kyberV3(address,bool)")
                );
    }

    function uniV2Immutables(address pool)public view returns(Immutables memory){
        return Immutables(
                    IUniV2Pool(pool).token0(),
                    IUniV2Pool(pool).token1(),
                    3000,
                    selector("uniV2(address,bool)")
                );
    }

    function algebraV2Immutables(address pool)public view returns (Immutables memory){
        (,,uint16 fee,,,,)=IAlgebraV2Pool(pool).globalState();
        return Immutables(
                    IAlgebraV2Pool(pool).token0(),
                    IAlgebraV2Pool(pool).token1(),
                    fee,
                    selector("uniV3(address,bool)")
                );
    }

    function selector(string memory func)public pure returns (bytes4){
...
interface IAlgebraV2Pool is IV3Pool {
