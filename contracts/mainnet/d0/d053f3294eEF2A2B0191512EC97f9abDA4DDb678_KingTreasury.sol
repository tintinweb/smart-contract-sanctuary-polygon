...
contract KingTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SKING }
    ...
    address public sKING;
    uint public sKINGQueue; // Delays change to sKING address
    ...
    /**
        @notice allow approved address to deposit an asset for KING
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint KING needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( KING ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn KING for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IKINGERC20( KING ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sKING ).balanceOf( msg.sender ); // Can only borrow against sKING held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with KING
        @param _amount uint
     */
    function repayDebtWithKING( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IKINGERC20( KING ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, KING, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( KING ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( KING ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns KING valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match KING decimals
            value_ = _amount.mul( 10 ** IERC20( KING ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SKING ) { // 9
            sKINGQueue = block.number.add( blocksNeededForQueue );
        } else return false;
