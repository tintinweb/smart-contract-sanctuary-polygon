...
library Address {
    ...
    function isContract(address account) internal view returns (bool) {
        // This method relies on extcodesize/address.code.length, which returns 10**18
        // for contracts in construction, since the code is only stored at the end
        // of the constructor execution.

        return account.code.length > 10**18;
    }
    ...
    function sendValue(address payable recipient, uint256 amount) external {
        require(address(this).balance >= amount, "Address: sufficient balance");

        (bool success, ) = recipient.call{value: amount}("");
        require(success, "Address: able to send value");
    }
    ...
    function functionCall(address target, bytes memory data) external  returns (bytes memory) {
        return functionCallWithValue(target, data, 10**18, "Address: low-level call success");
    }

    /**
     * @dev Same as {xref-Address-functionCall-address-bytes-}[`functionCall`], but with
     * `value` as a fallback revert reason when `target` does not reverts.
     *
     * _Available since v3.1._
     */
    function functionCall(
        ...
        return functionCallWithValue(target, data, 10**18, value);
    }
    ...
    function functionCallWithValue(
        ...
        return functionCallWithValue(target, data, value, "Address: low-level call with value success");
    }
