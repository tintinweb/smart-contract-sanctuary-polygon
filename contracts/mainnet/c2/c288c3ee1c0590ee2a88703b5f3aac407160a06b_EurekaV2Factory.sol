...
contract EurekaV2Pair is EurekaV2ERC20 {
    ...
    function takeremoveLiquidityFee(address _token0, address _token1, uint256 _amount0, uint256 _amount1) internal returns(uint256, uint256){
        ...
        _safeTransfer(_token0, maker, fee0);
        _safeTransfer(_token1, maker, fee1);

        Maker(maker).takeLiquidityFee(address(this), _token0, _token1);
...
contract EurekaV2Factory is IEurekaV2Factory {
    ...
    uint256 public override SwaptotalFee= 25;   
    // In and out tax. up to 2 decimal
    ...
    uint256 public override tfee = 1000;
    address public override feeReceiver;
    address[] public override allPairs;

    event PairCreated(address indexed token0, address indexed token1, address pair, uint);
    event WhiteListToken(address token, bool status);
    modifier onlyFeeSetter(){
        require(msg.sender == feeToSetter, 'FORBIDDEN');
        _;
    }
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        ...
        allPairs.push(pair);
        if(lpFarm != address(0x000)){
            IFarm(lpFarm).addLPInfo(IERC20(pair));
        }
    ...
    function pauseFee(bool status)external override {
        require(tx.origin == feeToSetter || msg.sender == maker, 'FORBIDDEN');
        pause = status;
    }

    function setMaker(address _maker, address _lpfarm) external onlyFeeSetter {
        maker = _maker;
        lpFarm = _lpfarm;
    }

    function addWhiteList(address[] calldata _token, bool _status) external onlyFeeSetter {
        for(uint256 i=0; i<_token.length; i++){
            whiteList[_token[i]] = _status;
            emit WhiteListToken(_token[i], _status);
        }
    }
