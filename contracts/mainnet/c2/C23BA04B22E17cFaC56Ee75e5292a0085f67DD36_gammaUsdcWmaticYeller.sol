...
contract gammaUsdcWmaticYeller is Ownable {
    ...
    function getPendingRewards() internal view returns(uint256, uint256){
        ...
        address[] memory outputToNative = new address[](2); //["0x958d208cdf087843e9ad98d23823d32e17d723a1", "0x0d500b1d8e8ef31e21c99d1db9a6444d3adf1270"]
        outputToNative[0] = outputToNativeToYelRoute[0];
        outputToNative[1] = outputToNativeToYelRoute[1];

        address[] memory nativeToYel = new address[](2);//["0x0d500b1d8e8ef31e21c99d1db9a6444d3adf1270", "0xd3b71117e6c1558c1553305b44988cd944e97300"]
        nativeToYel[0] = outputToNativeToYelRoute[1];
        nativeToYel[1] = outputToNativeToYelRoute[2];

        address chef = strategy.chef(); //0x20ec0d06F447d550fC6edee42121bc8C1817b97D
        uint poolId = strategy.poolId();    //2

        address rewarderQuick = Chef(chef).getRewarder(poolId, 0);
        address rewarderWmatic = Chef(chef).getRewarder(poolId, 1);

        uint pendingQuick = Rewarder(rewarderQuick).pendingToken(poolId, address(strategy));
        uint pendingNative = Rewarder(rewarderWmatic).pendingToken(poolId, address(strategy));

        uint[] memory amountsOutNative = AggregatorV3Interface(unirouter).getAmountsOut(pendingQuick, outputToNative);
        uint wholeRewardsNative = pendingNative + amountsOutNative[1];
    ...
    function getFullApr() public view returns (uint, uint) {
        ...
        uint nativeUsdtPrice = (uint(price) * 1e18) / divisor;  //69692300 * 1e18 / 1e8 = 696923000000000000
    ...
    function getSimpleApr(uint _nativeUsdtPrice) private view returns (uint, uint) {
        ...
        uint usdtRewardPerYear = (rewardsPerYearForPool * _nativeUsdtPrice) / 1e18; //696923000000000000
    ...
    function getLpValue() public view returns (uint, uint) {
        ...
        (uint total0, uint total1) = LPToken(lpToken).getTotalAmounts();
        uint stableInLP = total0 * 1e18 / 1e6;
        uint wethEquivalentStable = total1 * wethUsdtPrice / 1e18;
        uint usdtInLps = stableInLP + wethEquivalentStable;
    ...
    function getBalancesInLp() public view returns(uint256[2] memory){
        address lpToken = strategy.want();
        (uint256 totalUsdc, uint256 totalWeth) = LPToken(lpToken).getTotalAmounts();
        uint256[2] memory balances;
        balances[0] = totalUsdc;
        balances[1] = totalWeth;
