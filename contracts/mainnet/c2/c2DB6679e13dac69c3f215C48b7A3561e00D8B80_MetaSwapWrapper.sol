...
contract MetaSwapWrapper is Ownable, ReentrancyGuard, Pausable {
    ...
    function maybeApproveERC20(
        ...
        if (token.allowance(address(this), target) < amount) {
            ...
            if (isPermit) {
                IAllowanceTransfer(target).approve(address(token), callTarget, uint160(amount), uint48(block.timestamp + 100));
            }
        }
    }
    ...
    function _transferTokensIfNeeded(
        ...
        if (!_isETH(token)) {
            maybeApproveERC20(token, amount, approveTarget, callTarget, isPermit);
