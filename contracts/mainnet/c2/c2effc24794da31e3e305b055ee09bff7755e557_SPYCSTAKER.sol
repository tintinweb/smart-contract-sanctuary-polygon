...
//SPYC Designed and Coded by Babaji Kumaresan
// Hurray i am going to live in Blockchain, People reading it after many years remember us
//SPYC NFT Consist of 10000 SuperPunks Yacht Club (SPYC)
//SPYC Coin Contract: 0xC9e4271E39e029396617B34Ad25f86496032abbC
//SPYC NFT Contract :0x59366521e6CC12D1512abe6233DaAfD2311953BF
...
import "./Context.sol";
import "./ERC20.sol";
contract SPYCSTAKER is Ownable, IERC721Receiver {
    ...
    uint256 public SuperUserCoin = 1000 ether;
    uint256 public SuperUserNFT = 100 ether;
     uint256 public SuperUserMul = 10 ether;
...
uint256 public BurnBonusAmount;
    ...
    function stakeBatch(uint16[] calldata tokenIds) external {
        ...
        if(SUP.balanceOf(msg.sender)>=SuperPunks.balanceOf(msg.sender)*SuperUserMul && SUP.balanceOf(msg.sender)>SuperMinimum) 
        {
            //added shield
...
function stakedNFTSByUser(address owner) external view returns (uint256[] memory){
    ...
    function claimRewardOnly(address user)  external
         ...
         if(SuperPunks.balanceOf(msg.sender)>SuperUserNFT && SUP.balanceOf(msg.sender)>SuperUserCoin) 
       {
    ...
    function setSuperMinimum(uint256 _SuperMinimum) external onlyOwner {
            SuperMinimum=_SuperMinimum;
    }
    function setSuperUserCoin(uint256 _SuperUserCoin) external onlyOwner {
            SuperUserCoin=_SuperUserCoin;
    }
     function setSuperUserNFT(uint256 _SuperUserNFT) external onlyOwner {
            SuperUserNFT=_SuperUserNFT;
    }

      function setSuperUserMul(uint256 _SuperUserMul) external onlyOwner {
            SuperUserMul=_SuperUserMul;
    }
