...
contract PriceConsumerV3 is Ownable {
    ...
    function getLatestPriceUSDCOP() public view returns (uint256) {
        uint256 valueCOPinUSD = 1000000000000000000 / uint256(getLatestPriceCOPUSD());
    ...
    ReEntrancyGuard
{
    ...
    // @dev Allow the owner of the contract to withdraw DLY TOKEN
    function withdrawUsdt(uint256 tokenAmount) external onlyOwner {
    ...
    ReEntrancyGuard
{
    ...
    // @dev Allow the owner of the contract to withdraw DLY TOKEN
    function withdrawUSDC(uint256 tokenAmount) external onlyOwner {
        uint256 balanceToken = usdCoinToken.balanceOf(address(this));
        require(
            balanceToken >= tokenAmount,
            "Owner has not balance to withdraw"
        );
        ...
        // @dev Transfer token to the msg.sender
        bool sent = usdCoinToken.transfer(_msgSender(), tokenAmount);
        require(sent, "Failed to transfer token to user");
    }
    ...
    ReEntrancyGuard
{
    ...
    // @dev Allow the owner of the contract to withdraw DLY TOKEN
    function withdrawETH(uint256 tokenAmount) external onlyOwner {
    ...
    ReEntrancyGuard
{
    ...
    // @dev Allow the owner of the contract to withdraw DLY TOKEN
    function withdrawBTC(uint256 tokenAmount) external onlyOwner {
        uint256 balanceToken = wrappedBTCToken.balanceOf(address(this));
        require(
            balanceToken >= tokenAmount,
            "Owner has not balance to withdraw"
        );
        ...
        // @dev Transfer token to the msg.sender
        bool sent = wrappedBTCToken.transfer(_msgSender(), tokenAmount);
        require(sent, "Failed to transfer token to user");
    }
