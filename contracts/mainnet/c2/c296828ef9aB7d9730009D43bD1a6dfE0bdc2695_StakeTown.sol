...
contract StakeTown is Staking721Base {

    uint256 public maxRewardsPerTimeUnit;
    uint256 public sideConditionId;

    string private _secretWordFixed;
    string private _secretWordMultiplier;
    uint256 public fixedReward;
    bool public fixedRewardToggle;
    uint256 public maxWalletsForFixedReward;
    uint256 public _fixedRewardClaimedCount;
    uint256 public maxNFTsPerWallet;

    uint256 public rewardMultiplier;
    bool public multiplierToggle;
    uint256 public multiplierDuration;
    uint256 public maxWalletsForMultiplier;
    uint256 public _multiplierRewardClaimedCount;
    mapping(address => uint256) public multiplierEndTime;
    mapping(address => uint256) internal _multiplierClaimed;
    mapping(address => uint256) internal _fixedClaimed;

    //array to keep track of non-zero claimed addresses
    address[] private claimedAddresses;
    address[] private fixedClaimedAddresses;

    //mapping to keep track of whether an address is already in the claimedAddresses array
    mapping(address => bool) private isInClaimedAddresses;
    mapping(address => bool) private isInFixedClaimedAddresses;

    constructor(
        uint256 _timeUnit,
        uint256 _maxRewardsPerTimeUnit,
        uint256 _rewardsPerUnitTime,
        address _stakingToken,
        address _rewardToken,
        address _nativeTokenWrapper,
        ...
        maxRewardsPerTimeUnit = _maxRewardsPerTimeUnit;
    ...
    modifier onlyTokenHolder() {
        ...
        uint256 walletBalance = IERC721(_stakingToken).balanceOf(msg.sender);
        uint256 stakedBalance = stakers[msg.sender].amountStaked;
        require(walletBalance + stakedBalance > 0, "User must hold or have staked the NFT to participate");
        _;
    }

    function getMultiplierClaimed(address userAddress) external view returns (uint256) {
        return _multiplierClaimed[userAddress];
    }

    function getFixedClaimed(address userAddress) external view returns (uint256) {
        return _fixedClaimed[userAddress];
    }

    function getUserMultiplier() external view returns (uint256) {
        uint256 rewardMult = rewardMultiplier;
        if (multiplierToggle && multiplierEndTime[msg.sender] > 0 && block.timestamp < multiplierEndTime[msg.sender]) {
            if (rewardMult < 100) {
                rewardMult += 100;
            }
        }
    ...
    function getMultiplierRewardSettings() external view returns (uint256, bool, uint256, uint256) {
        return (
            rewardMultiplier,
            multiplierToggle,
            maxWalletsForMultiplier,
            _multiplierRewardClaimedCount
        );
    }

    function getFixedRewardSettings() external view returns (uint256, bool, uint256, uint256) {
        return (
            fixedReward,
            fixedRewardToggle,
            maxWalletsForFixedReward,
            _fixedRewardClaimedCount
        );
    }

    function getMultiplierSecretWord() external view returns (string memory) {
        require(msg.sender == owner(), "Must be the owner of the contract to call this function.");
        return _secretWordMultiplier;
    }

    function getFixedSecretWord() external view returns (string memory) {
        require(msg.sender == owner(), "Must be the owner of the contract to call this function.");
        return _secretWordFixed;
    }

    function _stake(uint256[] calldata _tokenIds) internal virtual override {
        uint256 len = _tokenIds.length;
        uint256 userStakedTokens = stakers[msg.sender].amountStaked;
        require(userStakedTokens + _tokenIds.length <= maxNFTsPerWallet, "Maximum NFTs staked per wallet reached");
        require(len != 0, "Staking 0 tokens");
        ...
        if (stakers[_stakeMsgSender()].amountStaked > 0) {
            ...
            stakersArray.push(_stakeMsgSender());
            stakers[_stakeMsgSender()].timeOfLastUpdate = block.timestamp;
            stakers[_stakeMsgSender()].conditionIdOflastUpdate = sideConditionId - 1;
        }
        for (uint256 i = 0; i < len; ++i) {
            require(
                IERC721(_stakingToken).ownerOf(_tokenIds[i]) == _stakeMsgSender() &&
    ...
    function setMaxNFTsPerWallet(uint256 _maxNFTsPerWallet) external onlyOwner {
        maxNFTsPerWallet = _maxNFTsPerWallet;
    }

    function getMaxNFTsPerWallet() external view returns (uint256) {
        return maxNFTsPerWallet;
    }

    function _claimRewards() internal override {
        uint256 rewards = stakers[_stakeMsgSender()].unclaimedRewards + _calculateRewards(_stakeMsgSender());
        uint256 roundedRewards = (rewards + 5 * (10 ** 17)) / (10 ** 18) * (10 ** 18);

        require(roundedRewards != 0, "No rewards");

        stakers[_stakeMsgSender()].timeOfLastUpdate = block.timestamp;
        stakers[_stakeMsgSender()].unclaimedRewards = 0;
        stakers[_stakeMsgSender()].conditionIdOflastUpdate = sideConditionId - 1;

        uint256 localRewardMultiplier = rewardMultiplier;

        if (multiplierToggle && multiplierEndTime[_stakeMsgSender()] > 0 && block.timestamp < multiplierEndTime[_stakeMsgSender()]) {
            if (localRewardMultiplier < 100) {
                localRewardMultiplier += 100;
            }
            roundedRewards = roundedRewards * (localRewardMultiplier / 100);
        }

        _mintRewards(_stakeMsgSender(), roundedRewards);

        emit RewardsClaimed(_stakeMsgSender(), roundedRewards);
    }

    function setSideCondition() external virtual onlyOwner {
        if (!_canSetStakeConditions()) {
            revert("Not authorized");
        }
        ...
        sideConditionId = _stakerConditionId + 1;
    }

    function setFixedRewardSettings(uint256 rewardAmount, bool toggle, uint256 maxWallets, string memory newSecretWordFixed) external onlyOwner {
        fixedReward = rewardAmount;
        fixedRewardToggle = toggle;
        maxWalletsForFixedReward = maxWallets;
        _secretWordFixed = newSecretWordFixed;
        _fixedRewardClaimedCount = 0;

        // Reset _fixedClaimed for all addresses in the fixedClaimedAddresses array
        for (uint256 i = 0; i < fixedClaimedAddresses.length; i++) {
            _fixedClaimed[fixedClaimedAddresses[i]] = 0;
            isInFixedClaimedAddresses[fixedClaimedAddresses[i]] = false;
        }
        
        // Reset the fixedClaimedAddresses array
        delete fixedClaimedAddresses;
    }

    function updateFixedRewardSettings(uint256 rewardAmount, bool toggle, uint256 maxWallets, string memory newSecretWordFixed) external onlyOwner {
        require(bytes(_secretWordFixed).length > 0, "Reward not found");
    ...
    // Make sure to update fixedClaimedAddresses and isInFixedClaimedAddresses whenever you update _fixedClaimed
    function updateFixedClaimed(address userAddress, uint256 newValue) internal {
        if (!isInFixedClaimedAddresses[userAddress] && newValue > 0) {
            fixedClaimedAddresses.push(userAddress);
            isInFixedClaimedAddresses[userAddress] = true;
        }
        _fixedClaimed[userAddress] = newValue;
    }

    function setRewardMultiplierSettings(uint256 multiplier, bool toggle, uint256 duration, uint256 maxWallets, string memory newSecretWordMultiplier) external onlyOwner {
        rewardMultiplier = multiplier;
        multiplierToggle = toggle;
        multiplierDuration = duration;
        maxWalletsForMultiplier = maxWallets;
        _secretWordMultiplier = newSecretWordMultiplier;
        _multiplierRewardClaimedCount = 0;

        // Reset _multiplierClaimed for all addresses in the claimedAddresses array
        for (uint256 i = 0; i < claimedAddresses.length; i++) {
            _multiplierClaimed[claimedAddresses[i]] = 0;
            isInClaimedAddresses[claimedAddresses[i]] = false;
        }
        
        // Reset the claimedAddresses array
        delete claimedAddresses;

    }

    function updateMultiplierRewardSettings(uint256 multiplier, bool toggle, uint256 duration, uint256 maxWallets, string memory newSecretWordMultiplier) external onlyOwner {
        require(bytes(_secretWordMultiplier).length > 0, "Reward not found");
    ...
    // Make sure to update claimedAddresses and isInClaimedAddresses whenever you update _multiplierClaimed
    function updateMultiplierClaimed(address userAddress, uint256 newValue) internal {
        if (!isInClaimedAddresses[userAddress] && newValue > 0) {
            claimedAddresses.push(userAddress);
            isInClaimedAddresses[userAddress] = true;
        }
        _multiplierClaimed[userAddress] = newValue;
    }

    function getMultiplierRewardClaimedCount() public view returns (uint256) {
        return _multiplierRewardClaimedCount;
    }

    function getFixedRewardClaimedCount() public view returns (uint256) {
        return _fixedRewardClaimedCount;
    }

    function claimSecretWordRewardMultiplier(string memory inputSecretWordMultiplier) external onlyTokenHolder {
        require(keccak256(abi.encodePacked(inputSecretWordMultiplier)) == keccak256(abi.encodePacked(_secretWordMultiplier)), "Incorrect secret word");
        
        require(_multiplierClaimed[msg.sender] == 0, "Multiplier reward already claimed");

        if (multiplierToggle && _multiplierRewardClaimedCount < maxWalletsForMultiplier) {
            multiplierEndTime[msg.sender] = block.timestamp + multiplierDuration;
            updateMultiplierClaimed(msg.sender, _multiplierClaimed[msg.sender] + 1);
            _multiplierRewardClaimedCount++;
        } else {
            if (multiplierToggle) {
                require(_multiplierRewardClaimedCount < maxWalletsForMultiplier, "The maximum rewards for this secret have been issued!");
            }
        }
    }

    function claimSecretWordRewardFixed(string memory inputSecretWordFixed) external onlyTokenHolder {
        require(keccak256(abi.encodePacked(inputSecretWordFixed)) == keccak256(abi.encodePacked(_secretWordFixed)), "Incorrect secret word");

        require(_fixedClaimed[msg.sender] == 0, "Fixed reward already claimed");

        if (fixedRewardToggle && _fixedRewardClaimedCount < maxWalletsForFixedReward) {
            _mintRewards(_stakeMsgSender(), fixedReward);
            updateFixedClaimed(msg.sender, _fixedClaimed[msg.sender] + 1);
            _fixedRewardClaimedCount++;
        } else {
            if (fixedRewardToggle) {
                require(_fixedRewardClaimedCount < maxWalletsForFixedReward, "The maximum rewards for this secret have been issued!");
            }
        }
    }
