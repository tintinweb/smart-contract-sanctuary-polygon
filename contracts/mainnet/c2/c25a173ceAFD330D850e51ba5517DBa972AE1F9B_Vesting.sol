...
contract Vesting is Ownable, ReentrancyGuard, Pausable {
    ...
    function _claim(address benificery) internal whenNotPaused onlyOwner {
        uint256 claimableTokens = getClaimableTokens(benificery);
        require(
            block.timestamp > start.add(cliff),
        ...
        if (elapsedTime >= duration) {
            ...
            // vesting is on. cooldown check.
            require(
                block.timestamp >=
                    beneficiaries[benificery].lastClaimTime.add(cooldown),
                "Vesting: The claim cool down period has not passed yet"
            );
        }

        _claimedTokens[benificery] += claimableTokens;

        token.transfer(benificery, claimableTokens);
        beneficiaries[benificery].claimedAmount = beneficiaries[benificery]
            .claimedAmount
            .add(claimableTokens);
        beneficiaries[benificery].lastClaimTime = block.timestamp;
    ...
    function allocateTokens(
        ...
        _allocateTokens(beneficiaries_, amounts);
    }
    ...
    function _allocateTokens(
        ...
        for (uint256 i = 0; i < beneficiaries_.length; i++) {
            ...
            _allocatedTokens[beneficiaries_[i]] = amounts[i];

            emit TokensAllocated(beneficiaries_[i], amounts[i]);
        }
    }
    ...
    function updateVestingCliff(uint256 _vestingCliff) external onlyOwner {
        cliff = _vestingCliff;
    }
    ...
    function withdrawUnclaimedTokens() public onlyOwner {
        require(
            ...
            "Vesting: Cannot withdraw tokens before end"
        );
