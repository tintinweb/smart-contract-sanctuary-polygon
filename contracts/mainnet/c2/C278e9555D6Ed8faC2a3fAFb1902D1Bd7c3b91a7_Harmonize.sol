...
contract Harmonize is TuningEngine {    
    ...
    function randomSeed() public returns (Seed memory) {
        randNonce++; 
        uint256 rando = uint256(uint(keccak256(abi.encodePacked(randNonce,block.difficulty,block.timestamp))));
        // tier 1keccak256(block.difficulty, now)
        uint32 hue = uint32(rando % 360);
        
        uint32 pick = uint32(rando % 5)+1;

        uint32 sat = uint32(rando % 60)+40;

        uint32 light = uint32(rando % 50)+30;

        // tier 2
        // harmonic divisors 1, 6, 28, 140, 270, 496, 672, 1638, 2970, 6200, 8128, 8190
        uint32 negated = uint32(rando % 28) < 1 ? 1: 0; // uncommon modifier 1/28 chance; colors have decreased light levels

        uint32 muted = uint32(rando % 140) < 1 ? 1: 0; // rare modifier 1/140 chance; colors have decreased saturation levels

        uint32 amplified = uint32(rando % 270) < 1 ? 1: 0; // epic modifier 1/270 chance; animation is twice as fast

        uint32 divergent = uint32(rando % 496) < 1 ? 1: 0; // legendary modifier 1/496 chance; bars are vertical 
        return Seed(hue,sat,light,pick,negated,muted,amplified,divergent);
    }

    function harvestSeed() public returns (Seed memory) {
        Seed memory rand = randomSeed();
        bytes memory packedSeed = abi.encode(rand);
        
        while (sown[packedSeed]) {
            rand = randomSeed();
            packedSeed = abi.encode(rand);
        }
        sown[packedSeed] = true;
        return rand;
    }

    function forMe(address recipient, uint256 quantity, bool enumerate) public onlyOwner {        
        uint256 mintedPrior = minted(); 
        uint256 tokenId = super.airdrop(recipient,quantity,enumerate);
        for (uint256 i = mintedPrior; i <= tokenId; i++) {
    ...
    function findHarmonic(uint32 pick) internal pure returns (string memory) {
        if (pick == 1) {
            return "complement";
        } else if (pick == 2) {
            return "split";
    ...
    function encodedTokenAttributes(uint256 tokenId, Seed memory flower) public view returns (bytes memory) {        
        string memory harmonic = findHarmonic(flower.pick);
...
abstract contract Phaseable is Mintable {  
    ...
    function airdrop(address recipient, uint256 quantity, bool enumerate) public onlyOwner returns (uint256) {        
        if (minted()+quantity > phaseables.getMaxSupply()) {
            revert ExceedsMaxSupply();
        }
        return _mint(recipient,quantity, enumerate);
    }
