...
contract GenericPaymaster is BasePaymaster {

    mapping(address=>mapping(bytes4=>bool)) private allowedTargets; //Maps address to allowed function selectors

    constructor(){
    }

    function setTargetAllowance(address target, bytes4 selector, bool allow) external onlyOwner {
        allowedTargets[target][selector] = allow;
    }

    function isAllowed(address target, bytes4 selector) public view returns(bool) {
        return allowedTargets[target][selector];
    }
    ...
    returns (bytes memory context, bool rejectOnRecipientRevert) {
        ...
        (address target, bytes4 selector) = extractTargetAndSelector(relayRequest);
        require(allowedTargets[target][selector], "target or method not allowed");        
        return (bytes(""), true);
    }
    ...
    function extractTargetAndSelector(GsnTypes.RelayRequest memory relayRequest) internal pure returns(address target, bytes4 selector) {
        target = relayRequest.request.to;
        selector = GsnUtils.getMethodSig(relayRequest.request.data);
    }
