    ...
    AccessControlEnumerable
{
    ...
    event GameSet(uint256 indexed _gameId);

    event GameCompleted(uint256 indexed _gameId, Winner[] _winner);
    event Hit(uint256 indexed _gameId);
    event HitReturned(uint256 indexed _gameId);
    event DoubleDown(uint256 indexed _gameId);
    event DoubleDownReturned(uint256 indexed _gameId);
    event Split(uint256 indexed _gameId);
    event SplitReturned(uint256 indexed _gameId);
        ...
        TreasuryManager(_treasuryContract)
    {
        ...
        grantRole(GAME_MANAGER, 0x012e8326acd4CF9DBBCa78f082a1A1C01ed2192F);
    }
    ...
    function startGame(uint256 _ante, address _token) public {
        ...
        require(playerInGame[sender] == 0, "startGame: player already in game");

        require(
            minimumAnte[_token] != 0 && maximumAnte[_token] != 0,
    ...
    function fulfillRandomWords(
        ...
        require(
            gameNumber != 0,
            "fulfillRandomWords: request ID maps to an invalid game number"
        );

        if (gameData[gameNumber].state == GameState.GETTING_INITIAL_VRF) {
            uint256[] memory dealtCards = dealCards(
                3,
                gameNumber,
                _randomWords,
                0
            ...
            // check for blackjack
            (uint256 handValue, ) = getHandValue(
                gameData[gameNumber].playerDealtCards[0]
            );
            ...
            // check for blackjack
            (uint256 handValue, ) = getHandValue(
                gameData[gameNumber].playerDealtCards[handPointer]
            );
            ...
            if (handValue >= BLACK_JACK_VALUE) {
                if (handValue > BLACK_JACK_VALUE) {
                    ...
                    if (
                        ...
                        uint256 playerPayout = getBonusPayout(
                            gameNumber,
                            handPointer
                        );
                        ...
                        if (!goToDealer) {
                            uint256 housePayout = gameData[gameNumber]
                                .playerHold +
                                gameData[gameNumber].houseHold -
                                playerPayout;
                            endGame(gameNumber, playerPayout, housePayout);
                            return;
                        }
                ...
                if (
                    handPointer + 1 < gameData[gameNumber].numberOfPlayerHands
                ) {
                    uint256 i;
                    ...
                    for (
                        ...
                        (uint256 nextHandValue, ) = getHandValue(
                            gameData[gameNumber].playerDealtCards[
                                gameData[gameNumber].playerHandPointer
                            ]
                        ); //check this
            ...
            emit DoubleDownReturned(gameNumber);
            ...
            if (handPointer + 1 < gameData[gameNumber].numberOfPlayerHands) {
                ...
                for (
                    ;
                    i <
                    gameData[gameNumber].numberOfPlayerHands - handPointer - 1;
            ...
            uint256 splitHandPointer = gameData[gameNumber]
        ...
        if (gameData[gameNumber].state == GameState.GETTING_VRF_DEALER) {
            ...
            while (dealerHandValue < 17 || (dealerHandValue == 17 && soft)) {
                ...
                (dealerHandValue, soft) = getHandValue(
                    gameData[gameNumber].dealerCards
                );
            ...
            for (; i < gameData[gameNumber].numberOfPlayerHands; ) {
                (playerHandValue, ) = getHandValue(
                    gameData[gameNumber].playerDealtCards[i]
                );
                ...
                if (
                    dealerHandValue == BLACK_JACK_VALUE &&
                    gameData[gameNumber].dealerCards.length == 2
                ) {
                    ...
                    (playerHandValue > dealerHandValue ||
                        dealerHandValue > BLACK_JACK_VALUE) &&
                    playerHandValue <= BLACK_JACK_VALUE
                ) {
                    // player wins
                    ...
                    if (gameData[gameNumber].doubleDownHand[i]) {
                        ...
                        if (
                            gameData[gameNumber].playerDealtCards[i].length >=
                            5 &&
                            playerHandValue == BLACK_JACK_VALUE
                        ) {
            ...
            uint256 housePayout = gameData[gameNumber].playerHold +
                gameData[gameNumber].houseHold -
                playerPayout;
    ...
    function hit() public {
        ...
        if (
            ...
            require(
                gameData[gameNumber].playerDealtCards[handPointer][0].number !=
                    0,
        ...
        if (
            (gameData[gameNumber].playerHandPointer + 1 ==
                gameData[gameNumber].numberOfPlayerHands) && handValue > 9
        ) {
            ...
            for (
                ;
                i < gameData[gameNumber].numberOfPlayerHands - handPointer - 1;
                ...
                needsDealerCards =
                    needsDealerCards &&
                    (nextHandValue == BLACK_JACK_VALUE && handValue > 9);
    ...
    function doubleDown() public {
        ...
        if (gameData[gameNumber].playerDealtCards[handPointer].length == 3) {
            ...
            uint256 i;
            for (; i < 3; ) {
                handValue += cardValueMap[
                    gameData[gameNumber].playerDealtCards[handPointer][i].number
                ];
        ...
        holdPlayerFunds(
            sender,
            gameData[gameNumber].anteToken,
            gameData[gameNumber].ante
        );
        ...
        if (
            ...
            for (
                ...
                needsDealerCards =
                    needsDealerCards &&
                    (nextHandValue == BLACK_JACK_VALUE);
    ...
    function split() public {
        ...
        require(
            gameData[gameNumber].playerDealtCards[handPointer].length ==
                MAX_NUMBER_OF_CARDS_BEFORE_SPLIT,
        ...
        game
            .playerDealtCards[gameData[gameNumber].numberOfPlayerHands - 1]
            .push(gameData[gameNumber].playerDealtCards[handPointer][1]);
    ...
    function surrender() public {
        ...
        require(
            gameData[gameNumber].numberOfPlayerHands == 1,
            "surrender: cannot surrender if player has more than 1 hand"
        );
        require(
            gameData[gameNumber].playerDealtCards[0].length == 2,
            "surrender: cannot surrender if player has more than 2 cards"
        );
        require(
            gameData[gameNumber].dealerCards[0].number != 0,
            "surrender: cannot surrender if dealer is showing an ace"
        );
    ...
    function getHandValue(Card[] memory _cards)
        public
        view
        returns (uint256, bool)
    {
        ...
        for (; i < numberOfAces; ) {
            if (cardTotal + 11 > BLACK_JACK_VALUE) cardTotal += 1;
            else {
                if (i == numberOfAces - 1) {
                    soft = true;
                    cardTotal += 11;
    ...
    function assignNextPlayerCard(
        uint256 _gameNumber,
        uint256 _handPointer,
        uint256 _dealtCard
    ) private {
        ...
        game.playerDealtCards[_handPointer].push(
            Card({suit: suit, number: number})
        );
    }

    function assignNextDealerCard(uint256 _gameNumber, uint256 _dealtCard)
        private
    {
        uint256 suit = (_dealtCard % 52) / 13;
        uint256 number = _dealtCard % 13;
        ...
        game.dealerCards.push(Card({suit: suit, number: number}));
    }
    ...
    function getBonusPayout(uint256 _gameNumber, uint256 _handIndex)
        private
        view
        returns (uint256)
    {
        ...
        for (
            ;
            j < gameData[_gameNumber].playerDealtCards[_handIndex].length &&
                !bonusAwarded;

        ) {
            cardCounter[
                gameData[_gameNumber].playerDealtCards[_handIndex][j].number
            ] += 1;
    ...
    function dealCards(
        ...
        for (; g.drawIndex < startingIndex + _numberOfCards; ) {
            uint256 randomNum = _randomWords[
                g.drawIndex - startingIndex + _randomWordsStartingIndex
            ] % cardsRemaining;
    ...
    function getShuffledCards(uint256 _gameNumber, uint32 _numberOfCards)
        private
    {
        ...
        onlyRole(GAME_MANAGER)
    {
        ...
        refund(
            _player,
            gameData[gameId].anteToken,
            gameData[gameId].playerHold,
            gameData[gameId].houseHold
        );
