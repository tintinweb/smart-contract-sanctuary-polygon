...
contract XVMCfarms is Ownable {
   ...
   struct ProposalTax {
    ...
    ProposalMaticVault[] public maticVaultProposals;
	...
	uint256 public maxNftAllocation = 5000;
	uint256 public maxMemeAllocation = 1000;
    uint256 public maxMaticVaultAllocation = 5000;
    ...
    event ProposeMaticVault(uint256 proposalID, uint256 valueSacrificedForVote, uint256 proposedType, uint256 proposedAmount, address indexed enforcer, uint256 delay);
	...
	function changeMaxAllocations(uint256 _lp, uint256 _nft, uint256 _meme, uint256 _maticVault) external onlyOwner {
		maxLpAllocation = _lp;
		maxNftAllocation = _nft;
		maxMemeAllocation = _meme;
        maxMaticVaultAllocation = _maticVault;
	}
    ...
    */    
    function initiateFarmProposal(
    	...
    	require(poolid == 0 || poolid == 1 || poolid == 8 || poolid == 9 || poolid == 10 || poolid == 11, "only allowed for these pools"); 
    	...
    	if(poolid == 0 || poolid == 1) {
			...
			require(
    	        newAllocation <= (IMasterChef(masterchef).totalAllocPoint() * maxMaticVaultAllocation / 10000),
    	        "exceeds max allocation"
    	       );
			require(depositFee == 0, "deposit fee must be 0 for NFTs");
		} else {
  ...
  function proposeGovTax(uint256 depositingTokens, uint256 _amount, uint256 delay) external {
    ...
    function proposeMaticVault(uint256 depositingTokens, uint256 _type, uint256 _amount, uint256 delay) external {
        require(depositingTokens >= IXVMCgovernor(owner()).costToVote(), "Costs to vote");
        require(delay <= IXVMCgovernor(owner()).delayBeforeEnforce(), "must be shorter than Delay before enforce");
        // Matic Vault has requirement for maximum amount
        
    	IERC20(token).safeTransferFrom(msg.sender, owner(), depositingTokens);
    	maticVaultProposals.push(
    	    ProposalMaticVault(true, block.timestamp, _type, _amount, depositingTokens, 0, delay)
    	    );

    	emit ProposeMaticVault(
    	    maticVaultProposals.length - 1, depositingTokens, _type, _amount, msg.sender, delay
    	   );
    }
	function voteMaticVaultY(uint256 proposalID, uint256 withTokens) external {
		require(maticVaultProposals[proposalID].valid, "invalid");
		
		IERC20(token).safeTransferFrom(msg.sender, owner(), withTokens);

		maticVaultProposals[proposalID].valueSacrificedForVote+= withTokens;
			
		emit AddVotes(5, proposalID, msg.sender, withTokens, true);
	}
	function voteMaticVaultN(uint256 proposalID, uint256 withTokens, bool withAction) external {
		require(maticVaultProposals[proposalID].valid, "invalid");
		
		IERC20(token).safeTransferFrom(msg.sender, owner(), withTokens);
		
		maticVaultProposals[proposalID].valueSacrificedAgainst+= withTokens;
		if(withAction) { vetoGovernorTransfer(proposalID); }

		emit AddVotes(5, proposalID, msg.sender, withTokens, false);
	}
    function vetoMaticVault(uint256 proposalID) public {
    	require(maticVaultProposals[proposalID].valid == true, "Invalid proposal"); 
		require(maticVaultProposals[proposalID].firstCallTimestamp + maticVaultProposals[proposalID].delay <= block.timestamp, "pending delay");
		require(maticVaultProposals[proposalID].valueSacrificedForVote < maticVaultProposals[proposalID].valueSacrificedAgainst, "needs more votes");
		
    	maticVaultProposals[proposalID].valid = false;

		emit EnforceProposal(5, proposalID, msg.sender, false);
    }
    function executeMaticVault(uint256 proposalID) public {
    	require(
    	    maticVaultProposals[proposalID].valid == true &&
    	    maticVaultProposals[proposalID].firstCallTimestamp + IXVMCgovernor(owner()).delayBeforeEnforce() + maticVaultProposals[proposalID].delay  < block.timestamp,
    	    "conditions not met"
        );
    	
		if(maticVaultProposals[proposalID].valueSacrificedForVote >= maticVaultProposals[proposalID].valueSacrificedAgainst) {

            IXVMCgovernor(owner()).updateMaticVault(maticVaultProposals[proposalID].proposedType, maticVaultProposals[proposalID].proposedValue);

			maticVaultProposals[proposalID].valid = false; 
			
			emit EnforceProposal(5, proposalID, msg.sender, true);
		} else {
			vetoMaticVault(proposalID);
		}
    }
}