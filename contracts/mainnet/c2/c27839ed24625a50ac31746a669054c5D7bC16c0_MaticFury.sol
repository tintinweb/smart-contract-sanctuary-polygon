...
contract MaticFury is Ownable {
    using SafeMath for uint256;

    uint256 private constant PRIMARY_BENIFICIARY_INVESTMENT_PERC = 100; //10%
    uint256 private constant PRIMARY_BENIFICIARY_REINVESTMENT_PERC = 60; //6%

    uint256 private constant MIN_WITHDRAW = 0.002 ether; //0.002 Matic
    uint256 private constant MIN_INVESTMENT = 0.01 ether; //0.01 Matic
    uint256 private constant TIME_STEP = 1 days;
    // uint256 private constant TIME_STEP = 1 minutes; //fast mode
    uint256 private constant DAILY_INTEREST_RATE = 600; //60% Daily    for 2 days
    uint256 private constant DAILY_AUTO_REINTEREST_RATE = 10;   //1% Bonus
    uint256 private constant ON_WITHDRAW_AUTO_REINTEREST_RATE = 10; //1% Auto Reinvest
	uint256 private constant PERCENTS_DIVIDER = 1000;
	uint256 private constant TOTAL_RETURN = 1200; //120% ROI
	uint256 private constant TOTAL_REF = 105;
	uint256[] private REFERRAL_PERCENTS = [50, 30, 15, 5, 5]; //10.5%
    ...
    function _invest(address _addr, address _ref, uint256 _amount ) private returns (bool){
        require(msg.value >= MIN_INVESTMENT, "Minimum investment is 0.01 Matic");
    ...
    function withdraw() public {
        ...
        require(dividendAmount >= MIN_WITHDRAW, "min withdraw amount is 0.002 matic");
