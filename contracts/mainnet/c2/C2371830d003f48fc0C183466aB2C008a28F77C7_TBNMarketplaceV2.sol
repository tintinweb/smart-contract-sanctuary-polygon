...
contract TBNMarketplaceV2 is ERC721Holder, AccessControl, ReentrancyGuard {
  ...
  */
  ...
  /**
    ...
    */
  /*  
        add param - bool for priceBound
            if above true then priceBandBasisPoint should be basis point above or below 10,000
            ie. 9,000 = 90%, 15,000 = 150%, 500,000 = 500%
        add param - uint256 for fixedPrice
            min price seller is willing to sell for
    */
    ...
    nonReentrant
  {
    ...
    if (!listing.isPriceBound) {
      ...
      if (
        ...
        paymentTokensPerTBNToken = (tbnPrice * 1 ether) / paymentPrice;
      } else if (tbnNftAddress == address(0) || tbnNftAddress == wethAddress) {
        isNative = true;
        ...
        // both tokens are not native and do exist on chainlink
        // Get token price from oracle - is ratio
      ...
      if (paymentTokenDecimals > tbnTokenDecimals && !isNative) {
        // tbnAmount is smaller than what it should be
        fullCost =
        ...
        // tbnAmount is bigger than what it should be
        fullCost =
