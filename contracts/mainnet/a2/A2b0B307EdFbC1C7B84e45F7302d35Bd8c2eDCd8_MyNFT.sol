...
contract MyNFT is ERC721LazyMint, Permissions {
    ...
    {
        NftInterface _main = NftInterface(0xC5CcbE76ee4Fc757CecF0ED4873A561561Be9c0F);

        uint v;

        uint num = _main.totalSupply();

        for(uint i = 0; i < num; i++){
            address _owner = _main.ownerOf(i);
            if (_owner == _receiver){
                uint c = i;
        ...
        require((v >= _quantity), "You don't have enough claims remaining to mint that many.");
    ...
    ReentrancyGuard
{
     ...
     */
    function claim(address _receiver, uint256 _quantity) public payable virtual override nonReentrant {
     ...
     */
    function verifyClaim(address _claimer, uint256 _quantity) public view override virtual {}
...
abstract contract OperatorFilterToggle is IOperatorFilterToggle {
    bool public override operatorRestriction;

    function setOperatorRestriction(bool _restriction) external override {
