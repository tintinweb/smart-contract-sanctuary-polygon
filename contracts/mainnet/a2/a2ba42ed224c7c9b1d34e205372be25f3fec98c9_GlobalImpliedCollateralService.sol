...
contract GlobalImpliedCollateralService is Permissions {
    ...
    onlyRoleMalt(UPDATER_MANAGER_ROLE, "Must have updater manager role")
  {
    ...
    require(oldPool != _pool, "Cannot set same pool");
    poolUpdaters[_updater] = _pool;
    address oldUpdater = poolUpdatersLookup[_pool];

    require(_updater != oldUpdater, "GlobImpCol: No change");
  ...
  function sync(PoolCollateral calldata _pool) external {
    require(
    ...
    total = total + _pool.total;
    swingTraderMalt = swingTraderMalt + _pool.swingTraderMalt;
    swingTraderCollat = swingTraderCollat + _pool.swingTrader;
    arb = arb + _pool.arbTokens;
    overflow = overflow + _pool.rewardOverflow;
    liquidityExtension = liquidityExtension + _pool.liquidityExtension;
