...
contract BidVerifierSlim is OwnableUpgradeable, RouterMigratable {
    event AddedOperator(address _op);

    event RemovedOperator(address _op);

    event AddRouter(address _router);

    event RemoveRouter(address _router);

    /// @notice how much bids user have made already
    // pool => user => bidListId => bid count
    mapping(address => mapping(address => mapping(uint256 => uint256))) public userBidsCount;

    /// @notice Operators can create pools and conduct action on them
    mapping(address => bool) public isOperator;

    /// @notice Only routers verify bids
    ...
    function verifyBid(address _pool, address _user, uint256 _roundId, uint256 _newBids) external onlyRouter {
        uint256 _bidListId = IAuctionPool(_pool).roundIdToBidListId(_roundId);
        userBidsCount[_pool][_user][_bidListId] += _newBids;
    }
    ...
    function addRouter(address _router) external onlyOperator {
        isRouter[_router] = true;
        emit AddRouter(_router);
    }

    function removeRouter(address _router) external onlyOperator {
        isRouter[_router] = false;
        emit RemoveRouter(_router);
    }

    function migrateRouter(address _newRouter) external override onlyRouter {
        isRouter[msg.sender] = false;
        emit RemoveRouter(msg.sender);
        isRouter[_newRouter] = true;
        emit AddRouter(_newRouter);
    }
}
