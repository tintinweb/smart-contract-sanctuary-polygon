...
// File: contracts/transformedNFT (1).sol
    ...
    ERC721URIStorageUpgradeable
{
    ...
    address private serumNftContract;
     ...
     */
    function mintToken(address owner, string memory metadataURI) public onlyOwner returns (uint256) {
    ...
    function getOwnerTokens(address _owner) public view virtual returns (TokenMeta[] memory) {
        uint256 ownerTokensLength = ERC721Upgradeable.balanceOf(_owner);

        TokenMeta[] memory tokensOwnerMeta = new TokenMeta[](ownerTokensLength);
        
        uint256 i;

        for (i = 0; i <  ownerTokensLength; i++) {
            tokensOwnerMeta[i] = _tokenMeta[tokenOfOwnerByIndex(_owner, i)];
        }
        return (tokensOwnerMeta);
    }

    /** set serum contract address */
    function setSerumContractAddress(address _serumAddress) public onlyOwner {
        serumNftContract = _serumAddress;
    }
     ...
     * @param _originalNFTContract address of original nft contract
     * @param _originalNFTId If of original nft contract 
     * @param _serumId id of applied serum
     * @param _classify class of applied serum
     * Requirements:
    ...
    function mintCollectable(
        ...
        require(_msgSender() == serumNftContract || _msgSender() == owner(), 'you are not authorized to mint tokens');
        _tokenIds.increment();
