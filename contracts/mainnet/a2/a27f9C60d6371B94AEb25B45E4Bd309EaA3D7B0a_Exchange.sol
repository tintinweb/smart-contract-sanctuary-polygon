...
import "../common/LibFillResults.sol";
import "../common/LibBytes.sol";
import "../common/LibOrder.sol";
import "../common/LibRichErrors.sol";
import "../common/LibExchangeRichErrors.sol";
import "./ExchangeHelpler.sol";
...
contract Exchange  is Ownable, EIP712, ReentrancyGuard, ExchangeHelpler {
    ...
    // for storage data
    ExchangeHelpler public exchangeHelpler;
    ...
    mapping(bytes32 => bool) public override cancelled;
    ...
    event Fill(
        ...
        address takerAddress,                 // Address that filled the order.
        address senderAddress,                // Address that called the Exchange contract (msg.sender).
        uint makerAssetFilledAmount,       // Amount of makerAsset sold by maker and bought by taker.
        uint makerAssetPaid
    );
  ...
  /// @dev Match two complementary orders that have a profitable spread.
	...
	{
        _matchOrders(leftOrder, rightOrder, leftSignature, rightSignature);
    }

	function _matchOrders(
		LibOrder.Order memory leftOrder,  //seller
		LibOrder.Order memory rightOrder, //buyer
		bytes memory leftSignature,
		bytes memory rightSignature
	) internal
    {
		address signer = getSigner();
		verify(leftSignature,leftOrder.makerAddress,leftOrder);        
		verify(rightSignature,signer,rightOrder);
		...
		bytes32 orderHash = LibOrder.getStructHash(leftOrder);

        _dispatchTransferFrom(
            orderHash,
            rightOrder.takerAssetData, //bytes memory assetData,
            leftOrder.makerAddress, //address from,
            rightOrder.makerAddress, //address to,
            1
        );

        _dispatchTransferFrom(
            orderHash,
            rightOrder.makerAssetData,//bytes memory assetData,
            rightOrder.makerAddress,  //address from,
            leftOrder.makerAddress,//address to,
            1
        );

        payFee(
            orderHash,
            rightOrder.makerFeeAssetData,//bytes memory assetData,
            rightOrder.makerAddress,  //address from,
            leftOrder.feeRecipientAddress,//address to,
            1
        );
    }
    ...
    function _dispatchTransferFrom(
        ...
        if (proxyId == bytes4(0)) {
            (address assetAddress, uint256 _amount) = abi.decode(
                assetData.sliceDestructive(4, assetData.length),
                (address, uint256)
            );
            payable(to).transfer(_amount);
        } else {
            address assetProxy = _assetProxies[proxyId];
            ...
            (bool didSucceed, bytes memory returnData) = assetProxy.call(
                proxyCalldata
            );
    ...
    function payFee(
        ...
        bytes4 proxyId = assetData.readBytes4(0);
        if (proxyId == bytes4(0)) {
            (, address[] memory receivers, uint256[] memory _amounts) = abi.decode(
                assetData.sliceDestructive(4, assetData.length),
                (address, address[], uint256[])
            );
            for(uint i; i < receivers.length; i++) {
                payable(receivers[i]).transfer(_amounts[i]);
            }
        } else {
            address assetProxy = _assetProxies[proxyId];
    ...
    function setSigner( address operator_ ) external view  onlyOwner returns (address) {
        return operator_;
    }
    ...
    function verify(
        ...
        //evan: internal
        bytes32 META_TRANSACTION_TYPEHASH = 0xf80322eb8376aafb64eadf8f0d7623f22130fd9491a221e902b713cb984a7534;
        bytes32 digest = _hashTypedDataV4(LibOrder.getStructHash(order));
    ...
    */
		...
		returns (LibOrder.OrderInfo memory orderInfo)
	{
        ...
        // Validate order availability
        if (orderInfo.orderTakerAssetFilledAmount >= order.makerAssetAmount) {
            orderInfo.orderStatus = LibOrder.OrderStatus.FULLY_FILLED;
            return orderInfo;
        }
		...
		// Check if order has been cancelled
		if (isCanceled(orderInfo.orderHash)) {
			orderInfo.orderStatus = LibOrder.OrderStatus.CANCELLED;
			return orderInfo;
		}
		// if (orderEpoch[order.makerAddress][order.senderAddress] > order.salt) {
        ...
        pure
    {
        ...
        if (orderInfo.orderStatus != LibOrder.OrderStatus.FILLABLE) {
            revert("Order is not fillable");// readl: custom
        }
    }


    // bytes32 EIP712_EXCHANGE_DOMAIN_HASH=0xf80322eb8376aafb64eadf8f0d7623f22130fd9491a221e902b713cb984a7534;
	...
	{
		orderHash = LibOrder.getStructHash(order);
		orderTakerAssetFilledAmount = getFilled(orderHash);
		return (orderHash, orderTakerAssetFilledAmount);
	}

    function getOrderHashAndFilledAmount(LibOrder.Order memory order)
		external
		view
		returns (bytes32 orderHash, uint256 orderTakerAssetFilledAmount)
	{
	...
	function verify2(
		...
		verify(leftSignature,leftOrder.makerAddress,leftOrder);
		verify(rightSignature,getSigner(),rightOrder);
		return true;
	}
    ...
    function _updateFilledState(
        ...
        address takerAddress,
        bytes32 orderHash,
        uint orderTakerAssetFilledAmount,
        uint amount,
        uint maxAmount
    )
        internal
    {
        // Update state
        filled[orderHash] = orderTakerAssetFilledAmount.safeAdd(amount);
        require(filled[orderHash] <= maxAmount, "Order filled amount exceeds max amount");
        emit Fill(
            order.makerAddress,
            order.feeRecipientAddress,
            order.makerAssetData,
    ...
    function isCanceled(bytes32 orderHash)
        public
        view
        returns (bool)
    {
        if(address(exchangeHelpler) != address(0)){
            return cancelled[orderHash] || exchangeHelpler.cancelled(orderHash);
        }
        return cancelled[orderHash];
    }

    function getFilled(bytes32 orderHash)
        public
        view
        returns (uint)
    {
        if(address(exchangeHelpler) != address(0) && filled[orderHash] == 0) {
            return exchangeHelpler.filled(orderHash);
        }
...
library LibFillResults {
    ...
    /// @return totalFillResults sum of both fill results.
...
library LibBytes {
    ...
    /// @return result byte that was popped off.
    ...
    /// @return equal if arrays are the same. False otherwise.
    function equals(
        bytes memory lhs,
        bytes memory rhs
    )
...
library LibOrder {
    ...
    /// @param order The order structure.
    ///// / @returan EIP712 hash of the order struct.
...
library LibMath {
    ...
    /// @param denominator Denominator.
    /// @param target Value to calculate partial of.
    /// @return partialAmount value of target rounded up.
    ...
    /// @return isError error is present.
