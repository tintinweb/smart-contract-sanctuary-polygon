...
import {StandardAccessControlEnumerableUpgradeable} from "./StandardAccessControlEnumerableUpgradeable.sol";

/**
 * @dev Extension of {AccessControlEnumerable} that offer support for maintainer role.
 */
contract RentShareAccessControlEnumerable is StandardAccessControlEnumerableUpgradeable {
    bytes32 public constant RENT_CLAIMER_VAULT_ROLE =
        keccak256("RentClaimerVault");

    modifier onlyRentClaimerVault() {
        require(
            hasRole(RENT_CLAIMER_VAULT_ROLE, msg.sender),
            "Sender must be the vault"
        );
        _;
    }

    function _setRentClaimerVault(address _account) internal {
        _setRoleAdmin(RENT_CLAIMER_VAULT_ROLE, DEFAULT_ADMIN_ROLE);
        _setupRole(RENT_CLAIMER_VAULT_ROLE, _account);
    }
}
...
import {RentShareAccessControlEnumerable} from "./../common/roles/RentShareAccessControlEnumerable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";
import {AccessControlEnumerableUpgradeable} from "./../common/roles/StandardAccessControlEnumerableUpgradeable.sol";
...
contract RentShare is
    ...
    Initializable,
    UUPSUpgradeable,
    RentShareAccessControlEnumerable,
    ...
    SelfPermitUpgradeable
{
    ...
    function deposit(
        ...
        console.log("FROM deposit after harvest");
    ...
    function harvestRewards(
        string memory symbol
    ) public returns (uint rewardsHarvested) {
        ...
        rewardsHarvested = _harvestRewards(
            storageParams.symbolToPoolId[symbol],
            _msgSender()
        );
        storageParams.userToPropertyRentClaimTimestamp[_msgSender()][
            symbol
    ...
    /**
     * @notice only vault should be able to harvest rewards for given pool id
     * @dev not restriction for vault to claim rent only once a week
     * @param symbol of the wrapped Property
     */

    function harvestRewardsForVault(
        string memory symbol
    ) public onlyRentClaimerVault returns (uint rewardsHarvested) {
        console.log("From harvestRewardsForVault");
        rewardsHarvested = _harvestRewards(
            storageParams.symbolToPoolId[symbol],
            _msgSender()
        );
    ...
    function _harvestRewards(
        ...
        console.log("Before update reward in _harvest");
        updatePoolRewards(_poolId);
        console.log("After update reward in _harvest");
        return storageParams.harvestRewards(_poolId, sender);
    }
