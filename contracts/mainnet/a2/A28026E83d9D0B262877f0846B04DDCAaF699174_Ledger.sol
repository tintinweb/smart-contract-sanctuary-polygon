 ...
 */
contract Ledger is Ownable, AccessControl {
  ...
  function createWallet() external returns (address wallet) {
    require(getWallet[msg.sender] == address(0), "Ledger::createWallet: WALLET EXISTS");
  ...
  function setImplementation(address _implementation) external onlyOwner {
    require(Address.isContract(_implementation), "Ledger::setImplementation: INVALID IMPLEMENTATION");
...
contract Wallet {
  /**
   * @dev Adress of the ledger contract
   */
  ...
  function _implementation() internal view returns (address) {
    return ILedger(ledger).implementation();
  }
