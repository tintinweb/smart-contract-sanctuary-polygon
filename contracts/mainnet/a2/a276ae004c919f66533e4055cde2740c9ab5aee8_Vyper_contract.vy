# @version 0.3.6
# @title remove_trident_liquidity
...
# For interfacing with the liquidity token
  ...
  def allowance(spendee: address, spender: address) -> uint256: nonpayable
...
def remove_liquidity(
  ...
  assert Lib(_lp_token).balanceOf(msg.sender) >= _amount, "Please check your wallet balance is greater than or equal to 'amount'"
  assert Lib(_lp_token).allowance(msg.sender, self) >= _amount, "Please check you have approved this contract to move your token"
