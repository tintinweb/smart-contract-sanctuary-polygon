...
// import "../DeFi/kyber/interfaces/IDMMFactory.sol";
import "../DeFi/kyber/interfaces/IDMMPool.sol";
...
contract OnChainPricing is UUPSUpgradeable, OwnableUpgradeable {
    ...
    address public constant KYBER_DMM_ROUTER =
    ...
    mapping(address => mapping(address => address)) public dmmPools;
    ...
    function findRouter(
        address[] memory path
    ) public view returns (address router, address pool) {
        uint256 pathLength = path.length;

        if (pathLength < 2) {
            return (QUICK_ROUTER, address(0));
        }

        if (dmmPools[path[0]][path[1]] != address(0)) {
            return (KYBER_DMM_ROUTER, dmmPools[path[0]][path[1]]);
        } else if (
            dmmPools[path[pathLength - 1]][path[pathLength - 2]] != address(0)
        ) {
            return (
                KYBER_DMM_ROUTER,
                dmmPools[path[pathLength - 1]][path[pathLength - 2]]
            );
        } else {
            return (QUICK_ROUTER, address(0));
        }
    ...
    function getOutAmount(
        ...
        (address oracleRouter, address pool) = findRouter(path);

        if (pool == address(0)) {
            try
                IUniswapRouterV2(oracleRouter).getAmountsOut(amountIn, path)
            ...
            for (uint256 index; index < pathLength - 1; ) {
                ...
                (oracleRouter, pool) = findRouter(subPath);

                if (pool == address(0)) {
                    try
                        IUniswapRouterV2(oracleRouter).getAmountsOut(
                            nextAmountIn,
                            subPath
                        )
                    returns (uint256[] memory amounts) {
                        nextAmountIn = amounts[1]; // Last one is the outToken
                    } catch (bytes memory) {
                        return 0;
                    }
                } else if (oracleRouter == KYBER_DMM_ROUTER) {
                    ...
                    try
                        IDMMExchangeRouter(oracleRouter).getAmountsOut(
                            nextAmountIn,
                            pools,
                            tokenPath
                        )
