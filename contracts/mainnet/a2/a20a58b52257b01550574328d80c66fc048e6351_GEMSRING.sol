...
contract GEMSRING {
    ...
    function IncomeWithdrawFor(address userAddress,uint256 amount) external
    {
        require(msg.sender==createrWallet,"Only contract owner");
        uint256 balanceReward = users[userAddress].totalincome - users[userAddress].totalwithdraw;     
        require(balanceReward>=amount, "Insufficient reward to withdraw!");
        users[userAddress].totalwithdraw+=amount;
    ...
    function registration(address userAddress, address referrerAddress) private {
        ...
        _distributelevelIncome(userAddress, packagePrice*50/100,1);
    ...
    function updateAutoReferrer(address userAddress, address referrerAddress, uint8 level) private{
		...
		if(level<13 && referrerAddress!=id1){ 
            ...
            users[referrerAddress].activeLevels[level] = true;
