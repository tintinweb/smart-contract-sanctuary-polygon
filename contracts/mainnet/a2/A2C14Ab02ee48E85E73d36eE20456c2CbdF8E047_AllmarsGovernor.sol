...
// Events
    ...
    function getAmountIn(uint256 tokenId) public isInitiated view returns(uint256)  {
        ...
        if (poolTokens[0] == WETHContract) {
            rate = WeightedMath._calcInGivenOut(
                balances[1],
                weights[1],
                balances[0],
                weights[0],
        ...
        return rate * 1000 * wethRate(tokenId) / 1 ether;
    }

    function wethRate (uint256 tokenId) public view returns(uint256) {
        if (tokenId == 1) return 0.001 ether;
        else if (tokenId >= 2 && tokenId <= 144) return 0.2 ether;
        else if (tokenId >= 145 && tokenId <= 1440) return 0.4 ether;
        else if (tokenId >= 1441 && tokenId <= 2160) return 0.8 ether;
        else if (tokenId >= 2161 && tokenId <= 2232) return 4 ether;
        else if (tokenId >= 2233 && tokenId <= 2304) return 2 ether;
        else if (tokenId >= 2305 && tokenId <= 2376) return 4 ether;
        else if (tokenId >= 2377 && tokenId <= 2448) return 2 ether;
        else if (tokenId >= 2449 && tokenId <= 2450) return 8 ether;
        else return 0;
    }
    ...
    function depositNFT(uint256 _tokenId) public isInitiated {
        uint256 erc20Rate = getAmountIn(_tokenId);
        require(
    ...
    function claimNFT(uint256 tokenId, address token) public isInitiated {
        ...
        if (token == WETHContract) {
            IERC20(WETHContract).safeTransferFrom(_msgSender(), address(this), wethRate(tokenId));
            sendNFT(_msgSender(), tokenId);
            uint256 toBalancer = wethRate(tokenId) * BALANCER_SHARE / 100;
            ...
            _joinPool(tokens, amounts, 1);
            IERC20(WETHContract).safeTransfer(owner(), wethRate(tokenId) - toBalancer);
        } else if (_deposits[tokenId].timestamp + minTerm > block.timestamp) {
            ...
            IERC20(ERC20Contract).safeTransferFrom(
                _msgSender(), address(this), getAmountIn(tokenId)
            );
            sendNFT(_msgSender(), tokenId);
        }
    }
