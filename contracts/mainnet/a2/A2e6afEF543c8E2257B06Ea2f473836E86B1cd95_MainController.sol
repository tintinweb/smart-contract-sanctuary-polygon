...
contract MainController is MainControllerStorageV1, MainControllerInterface, Initializable, UUPSUpgradeable, OwnableUpgradeable {
	...
	MatchInterface(matchAddress).initialize(a, b, startTime, endTime, usdt, ticketNFT);
...
abstract contract MatchInterface {
    ...
    /*** write functions ***/
    function initialize(uint8 _teamA, uint8 _teamB, uint _startTime, uint _endTime, address _usdtAddress, address _nftAddress) virtual external; 
...
contract MatchContract is MatchInterface{ 
    ...
    address public ticketNFT; // tickets
    ...
    /*** write functions ***/
    function initialize(uint8 _teamA, uint8 _teamB, uint _startTime, uint _endTime, address _usdtAddress, address _nftAddress) override external {
	...
	// issue NFT ticket to account
	MainControllerInterface.VoteRecord memory record = MainControllerInterface.VoteRecord(address(this), flag, teamA, teamB, 0, 0, block.timestamp, amount, true);
	ITickets(ticketNFT).newTicket(account, address(this), record);
	...
	//  issue NFT ticket to account
	MainControllerInterface.VoteRecord memory record = MainControllerInterface.VoteRecord(address(this), 0, teamA, teamB, _teamAScore, _teamBScore, block.timestamp, amount, false);
	ITickets(ticketNFT).newTicket(account, address(this), record);
...
interface ITickets {
     ...
     */
    //function newTicket(address voter, address matchAddress, uint8 teamA, uint8 teamB, uint256 voteType, uint8 scoreA, uint8 scoreB, uint256 flag, uint256 amount) external returns (uint256); 
    function newTicket(address voter, address matchAddress, MainControllerInterface.VoteRecord memory record) external returns (uint256); 
