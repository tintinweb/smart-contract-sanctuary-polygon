...
contract azizprofilepictures is ERC721, IERC2981, ReentrancyGuard, Ownable {
  using Counters for Counters.Counter;

  constructor(string memory customBaseURI_, address proxyRegistryAddress_)
    ERC721("aziz profile pictures", "azpp")
  {
    customBaseURI = customBaseURI_;

    proxyRegistryAddress = proxyRegistryAddress_;

    allowedMintCountMap[owner()] = 50;
  }
  ...
  uint256 public constant PRICE = 1000000000000;
  ...
  function mint(uint256 count) public payable nonReentrant {
    ...
    require(
      msg.value >= PRICE * count, "Insufficient payment, 0.000001 ETH per item"
    );
