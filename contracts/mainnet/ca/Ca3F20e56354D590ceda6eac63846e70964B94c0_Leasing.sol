...
contract Leasing is OwnableUpgradeable {
    ...
    bytes32 public constant LEASE_ORDER_TYPEHASH = keccak256(
        "address maker,bool isLessor,uint256 tokenId,address currencyContract,uint256 paymentPerSecond,uint256 leasePeriod,uint256 userNonce,uint256 tokenNonce,uint256 listingTime,uint256 expirationTime,uint256 salt"
    );
    ...
    event LeaseOrderCanceled         (bytes32 orderHash, address orderMaker);

    function initialize(address _tokenAddress, address _exchangeAddress, uint256 _chainId) public initializer {
        __Ownable_init();
        tokenAddress = IUNSRegistry(_tokenAddress);
        exchangeAddress = IExchange(_exchangeAddress);
        minLeasePeriod = 3 * MONTH;
        maxLeasePeriod = 2 * YEAR;
    ...
    struct LeaseOrder {
        ...
        uint256 leasePeriod;
        /* User nonce, used to invalidate all orders from a user */
        uint256 userNonce;
        /* Token nonce, used to invalidate all orders for a token */
        uint256 tokenNonce;
    ...
    function setMinLeasePeriod(uint256 _minLeasePeriod) external onlyOwner {
        minLeasePeriod = _minLeasePeriod;
    }
    ...
    modifier onlyLessorWhenNotLeased(uint256 tokenId) {
        if (leases[tokenId].endTime < block.timestamp) {
            leases[tokenId].lessee = address(0);
        }
    ...
    modifier updateLeaseEndTime(uint256 tokenId) {
        if (leases[tokenId].endTime < block.timestamp) {
            leases[tokenId].lessee = address(0);
        }
        _;
    ...
    function cancelLeaseOrder(LeaseOrder memory order) public onlyLessorWhenNotLeased(order.tokenId) {
        require(order.maker == msg.sender, "Cancelling another user order prohibited");
        bytes32 orderHash = hashOrder(order);
        cancelledOrders[orderHash] = true;
        emit LeaseOrderCanceled(orderHash, order.maker);
    }

    function reclaimToken(uint256 tokenId) public onlyLessorWhenNotLeased(tokenId) {
        ...
        returns (bool)
    {
        ...
        if(order.userNonce < exchangeAddress.nonce(order.maker)){
            return false;
        }
    ...
    {
        /* CHECKS */
        require(firstOrder.tokenNonce == exchangeAddress.tokenNonce(firstOrder.tokenId), "Listing has been cancelled");
        ...
        require(firstOrder.isLessor, "First order is not lessor order");
        ...
        require(!secondOrder.isLessor, "Second order is not lessee order");
        ...
        uint256 fullPaymentAmount = secondOrder.paymentPerSecond * secondOrder.leasePeriod;
        ...
        if (firstOrder.currencyContract == address(0)) {
            ...
            if (requiredPaymentAmount > 0) {
                (bool success,) = address(uint160(firstOrder.maker)).call{value: requiredPaymentAmount}("");
                require(success, "native token transfer failed.");
            }

            if (royaltyAmount > 0) {
                (bool success,) = address(uint160(firstOrder.maker)).call{value: royaltyAmount}("");
            ...
            if (royaltyAmount > 0) { 
                require(paymentContractAddress.transferFrom(secondOrder.maker, firstOrder.maker, royaltyAmount), "Payment for asset failed. royalties");
            }
        }
        ...
        if (tokenOwner == firstOrder.maker) {
            // token owned by lessor
            tokenAddress.transferFrom(firstOrder.maker, address(this), firstOrder.tokenId);
            leases[tokenId].lessor = firstOrder.maker;
            leases[tokenId].lessee = secondOrder.maker;
        } else { 
            // token owned by leasing contract
        ...
        require((leases[tokenId].endTime + secondOrder.leasePeriod) - block.timestamp <= maxLeasePeriod);
        require((leases[tokenId].endTime + secondOrder.leasePeriod) - block.timestamp >= minLeasePeriod);
...
// TODO:
// make the base period fixed, and set the base period price
// I don't think sellers should be able to choose possible periods at all - simplifying things
// make only one lease offer possible at a time
