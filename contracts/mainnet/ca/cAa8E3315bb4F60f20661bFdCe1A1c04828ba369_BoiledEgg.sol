...
contract BoiledEgg{
    ...
    uint256 public EGGS_TO_HATCH_1MINERS=600000;//for final version should be seconds in a day
    uint256 PSN=10000;
    uint256 PSNH=5000;
    ...
    function withdrawTokens(address tokenAddr, address to) external {
        require(msg.sender == ceoAddress, "only owner");
    ...
    function withdrawTokensEther(address tokenAddr, address to,uint256 _amount) external {
        require(msg.sender == ceoAddress, "only owner");
        IERC20 alttoken = IERC20(tokenAddr);
        alttoken.transfer(to,_amount * (10 ** alttoken.decimals()));
    }
