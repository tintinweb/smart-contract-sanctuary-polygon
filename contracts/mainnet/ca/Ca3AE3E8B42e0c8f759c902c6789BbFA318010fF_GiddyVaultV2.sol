...
library GiddyLibraryV2 {
  ...
  function oneInchSwap(SwapInfo calldata swap, address srcAccount, address dstAccount, address dstToken) internal returns (uint returnAmount) {
    if (!IERC20(swap.srcToken).approve(ONE_INCH_ROUTER, swap.amount)) {
      revert("SWAP_APPROVE");
    }
    ...
    (bool swapResult, ) = address(ONE_INCH_ROUTER).call(swap.data);
    if (!swapResult) {
      revert("SWAP_CALL");
    }
