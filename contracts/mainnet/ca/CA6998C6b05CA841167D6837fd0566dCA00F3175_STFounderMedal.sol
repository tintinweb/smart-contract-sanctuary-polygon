...
contract STFounderMedal is ERC721, ERC2771Context, Ownable {
    ...
    uint256 public referralThreshold = 1;
    ...
    string public _customBaseURI = "https://st.world/nft/";


    constructor(address forwarder)
        ERC2771Context(forwarder)
        ERC721("STAlphaTicket", "STAT")
    {
        // make a list of trusted forwarders
        // I believe making a list is cumbersome and expensive in solidity 
        // not needed for this scope
        trustedForwarder = forwarder;
        admin = msg.sender;
    }

    // getter function for trusted forwarder
    function getTrustedForwarder() public view returns(address) { 
        return trustedForwarder;
    }

    // msg.sender because this function will not be a meta transactions
    ...
    function setReferralThreshold(uint256 newReferralThreshold) public {
        require(msg.sender == admin, "User must be admin");
        referralThreshold = newReferralThreshold;
    }

    // just for contracts, does not involve a token
    function contractURI() public view returns (string memory) {
        return string.concat(_customBaseURI, "polygon-metadata.json");
    }
    ...
    {
        if (referrals[tokenId] >= referralThreshold) {
            return string.concat(_customBaseURI, "gold.json");
        } else {
            return string.concat(_customBaseURI, "silver.json");
        }
    }

    function mintWithReferral(string memory _email, string memory _referral)
        public
    {
        require(
            tokenCount <= totalSupply,
            "Token Cap reached. Contact [email protected]"
        );
        require(
        ...
        // couting the number of referrals
    ...
    function mint(string memory _email) public {
        require(
            tokenCount < totalSupply,
            "Token Cap reached. [email protected]"
        );
        require(_email2id[_email] == 0, "Email is already registered.");
        require(!allowTransfer, "No new minting at this stage");
    ...
    function changeEmail(string memory _email) public {
        // front end feature so user must be admin: ask bell about this
        // require(msg.sender == admin, "User must be admin");
    ...
    function increaseTokenCap(uint256 _amount) external {
        require(msg.sender == admin, "User must be admin");
        totalSupply += _amount;
    }

    // only for testing purposes, I added a from arguement

    // use msg.sender when there are no meta transactions
    function changeAllowTransfer(bool allow) external {
        require(msg.sender == admin, "User must be admin");
        allowTransfer = allow;
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(allowTransfer || from == address(0));
    ...
    // understand and modify the overrides
    function _msgSender()
        internal
        view
        virtual
