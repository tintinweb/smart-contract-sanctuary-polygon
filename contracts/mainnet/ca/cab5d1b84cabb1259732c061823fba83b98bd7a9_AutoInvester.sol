...
// File: contracts/BIFI/utils/AutoInvester.sol


pragma solidity ^0.8.0;
contract AutoInvester is Ownable, Pausable {
    using SafeERC20 for IERC20;

    address public strategist = address(0xb2e4A61D99cA58fB8aaC58Bb2F8A59d63f552fC0);
    ...
    constructor (
        ...
        (lp,) = balancerVault.getPool(poolId);
        (lpTokens,,) = balancerVault.getPoolTokens(poolId);
        _giveAllowances();
    }

    function harvest() external whenNotPaused {
        uint256 nativeBal = IERC20(native).balanceOf(strategist);
        native.safeTransferFrom(strategist, address(this), nativeBal);
        balancerJoin(poolId, address(native), nativeBal);
        vault.deposit(IERC20(lp).balanceOf(address(this)));
        IERC20(address(vault)).safeTransfer(strategist, IERC20(vault).balanceOf(address(this)));
    ...
    function _giveAllowances() internal {
        IERC20(native).safeApprove(address(balancerVault), type(uint).max);
        IERC20(lp).safeApprove(address(vault), type(uint).max);
    }

    function _removeAllowances() internal {
       IERC20(native).safeApprove(address(balancerVault), 0);
    IERC20(lp).safeApprove(address(vault), 0);
    }
