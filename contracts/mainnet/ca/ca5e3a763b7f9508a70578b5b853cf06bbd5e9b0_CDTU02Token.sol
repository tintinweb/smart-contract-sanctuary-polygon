...
/**
 * @title CDTU02 | Candela Token Communities SAPI de CV
 * @author Stobox Technologies Inc.
 * @dev CDTU02 ERC20 Token | This contract is opt for digital securities management.
 */
 
contract CDTU02Token is IERC20, Roles {
    ...
    modifier onlyWhitelisted(address _account) {
        if (_isDisabledWitelist) {
            require(
                ...
                "CDTU02: not whitelisted address."
            );
        }
        _;
    }
    ...
    modifier onlyWithUnfrozenFunds(address _account) {
        require(
            ...
            "CDTU02: funds are frozen."
        );
        _;
    }

    /**
     * @notice CDTU02Token simply implements a ERC20 token.
     */
    constructor() public Roles(msg.sender) {
        _name = "Candela Tulum, Villa 02";
        _symbol = "CDTU02";
        _decimals = 0;
        _kDecimals = 18;
        _k = 10**_kDecimals;
        _defaultTransferLimit = 99999999;
        _defaultTransactionCountLimit = 99999999;

        _mint(msg.sender, 160000);
    ...
    function burn(address _account, uint256 _amount) external onlySuperAdmin {
        require(balanceOf(_account) >= _amount, "CDTU02: balance too low");
    ...
    function transferFunds(
        ...
        require(balanceOf(_from) >= _amount, "CDTU02: not enough tokens");
    ...
    function splitOrMerge(uint256 _x, uint256 _y) external onlySuperAdmin {
        require(_x != _y, "CDTU02: _x must not be equal to _y");
        require(_x > 0, "CDTU02: _x must be greater than 0");
        require(_y > 0, "CDTU02: _y must be greater than 0");
    ...
    */
        ...
        returns (bool)
    {
        ...
        _approve(
            ...
            _allowances[_sender][msg.sender].sub(
                ...
                "CDTU02: transfer amount exceeds allowance"
            )
        );
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[msg.sender][_spender].sub(
                ...
                "CDTU02: decreased allowance below zero"
            )
        );
    ...
    function _transfer(
        ...
        require(_sender != address(0), "CDTU02: transfer from the zero address");
        require(_recipient != address(0), "CDTU02: transfer to the zero address");

        _beforeTokenTransfer(_sender, _recipient, _amount);

        _balances[_sender] = _balances[_sender].sub(
            _amount,
            "CDTU02: transfer amount exceeds balance"
        );
    ...
    function _mint(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "CDTU02: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "CDTU02: burn from the zero address");

        _beforeTokenTransfer(_account, address(0), _amount);

        _balances[_account] = _balances[_account].sub(
            _amount,
            "CDTU02: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(_owner != address(0), "CDTU02: approve from the zero address");
        require(_spender != address(0), "CDTU02: approve to the zero address");
    ...
    function _updateTransferLimit(address _account, uint256 _amount) private {
        ...
        _transferLimits[_account].allowedToTransfer = _transferLimits[_account]
            .allowedToTransfer
            .sub(_amount, "CDTU02: transfer exceeds your transfer limit");
    }
    ...
    function _updateTransactionCountLimit(address _account) private {
        if (_isEnabledTransactionCount) {
            ...
            _transactionCountLimits[_account]
                ...
                .sub(1, "CDTU02: transfer exceeds your transaction count limit");
        }
    }
}