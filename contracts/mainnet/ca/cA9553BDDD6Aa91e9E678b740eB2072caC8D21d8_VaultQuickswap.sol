...
// File: contracts_POL/main/libraries/Math.sol
...
// File: contracts_POL/main/libraries/SafeERC20.sol
...
// File: contracts_POL/main/Token.sol
...
// File: contracts_POL/main/Admin.sol
...
// File: contracts_POL/main/VaultQuickSwap.sol
...
/** 
* @author Formation.Fi.
* @notice Implementation of the contract VaultQuickswap;
*/
...
contract VaultQuickswap is Ownable {
    using SafeERC20 for IERC20;
    ...
    IQuickSwapRouter public quickSwap;
    Admin public admin;
    constructor (address _admin, address _safeHouse, address _quick) payable {
        ...
        admin = Admin(_admin);
        quick = _quick;
        quickSwap = IQuickSwapRouter(_quick);
        factory = quickSwap.factory();
        WETH = quickSwap.WETH();
    ...
    function addLiquidity(
        ...
        uint amountBMin,
        uint duration
    ) external  onlyManager returns (uint amountA, uint amountB, uint liquidity) {
    ERC20(tokenA).approve(quick,  amountADesired);
    ERC20(tokenB).approve(quick,  amountBDesired);
   (amountA,amountB,liquidity) = quickSwap.addLiquidity(tokenA,tokenB,amountADesired,amountBDesired,amountAMin,amountBMin, safeHouse, block.timestamp + duration);
    }
    ...
    function addLiquidityETH(
        ...
        uint amountETHMin,
        uint duration
    ) external onlyManager returns (uint amountToken, uint amountETH, uint liquidity) {
    ERC20(token).approve(quick, amountTokenDesired);
    require (address(this).balance >= amountETHDesired, "Formation.Fi: balance limit");
    (amountToken,amountETH,liquidity)= quickSwap.addLiquidityETH{value: amountETHDesired}(token,amountTokenDesired,amountTokenMin, amountETHMin, safeHouse, block.timestamp + duration);
    }
    ...
    function removeLiquidity(
        ...
        uint amountBMin,
        uint duration
    ) public onlyManager returns (uint amountA, uint amountB) {
       address _pair = pairFor(factory,  tokenA,  tokenB);
       ERC20(_pair).approve(quick, liquidity);
      (amountA,amountB)= quickSwap.removeLiquidity(tokenA,tokenB,liquidity,amountAMin,amountBMin, safeHouse, block.timestamp + duration);
    ...
    function removeLiquidityETH(
        ...
        uint amountETHMin,
        uint duration
    ) external onlyManager returns (uint amountToken, uint amountETH) {
        address _pair = pairFor(factory,  token,  WETH);
        ERC20(_pair).approve(quick, liquidity);
        (amountToken,amountETH)= quickSwap.removeLiquidityETH(token,liquidity,amountTokenMin,amountETHMin, safeHouse, block.timestamp + duration);
    ...
    function sendToSafeHouse( address _asset,  uint256 _amount) external onlyManager {
        ...
        if ( _asset == address(0)){
           ...
           payable (safeHouse).transfer(_amount);
        }
        else{   
    ...
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, "Formation.Fi: Identical addresses");
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), "Formation.Fi: zero address");
    }
