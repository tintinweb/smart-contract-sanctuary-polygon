...
contract AngelForceofAfrica is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  modifier additionalCompliance(uint256 _mintAmount) {
    ...
    require(totalSupply() + _mintAmount <= currentSupply, "current supply exceeded");
    if (onlyWhitelist == true) {
        require(whitelistAddresses[msg.sender] == true, "not on whitelist");
    }
    _;
  }
  ...
  function singleAirdrop(uint256 _mintAmount, address _receiver) public mintCompliance(_mintAmount) onlyOwner {
    uint256 supply = totalSupply();
  ...
  function batchAirdrop(uint _mintAmount, address[] memory _receivers) public mintCompliance(_mintAmount) onlyOwner {
    uint256 batch = _receivers.length;
    for (uint256 i = 1; i <= batch; i++) {
        uint256 supply = totalSupply();
        uint256 k = i - 1;
            for (uint256 j = 1; j <= _mintAmount; j++) {
                _safeMint(_receivers[k], supply + j);
            }
    }
  }
  ...
  function delistUser(address _user) public onlyOwner {
    whitelistAddresses[_user] = false;
  }

  function whitelistUsers(address[] calldata _users) public onlyOwner {
    for (uint256 l; l < _users.length; l++) {
        whitelistAddresses[_users[l]] = true;
    }
  }
