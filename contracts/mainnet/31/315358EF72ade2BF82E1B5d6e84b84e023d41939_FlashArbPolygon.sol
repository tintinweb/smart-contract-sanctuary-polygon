...
interface CurveLike {
    ...
    function exchange_underlying(address pool, int128 i, int128 j, uint256 _dx, uint256 _min_dy) external returns(uint);    
...
contract FlashArbPolygon {
    ...
    function dumpOnCurveUnderlying(address curve, address pool, address underlying, int128 i, int128 j) internal returns(uint) {
        uint swapAmount = IERC20(underlying).balanceOf(address(this));
        IERC20(underlying).approve(curve, swapAmount);

        return CurveLike(curve).exchange_underlying(pool, i, j, swapAmount, 1);
    }    

    function dumpUSDCToUSDT() public returns(uint) {
        return dumpOnCurveUnderlying(0x5ab5C56B9db92Ba45a0B46a207286cD83C15C939, 0x5e5A23b52Cb48F5E70271Be83079cA5bC9c9e9ac, USDC, 2, 3);
    }

    function dumpUSDCToFrax() public returns(uint) {
        return dumpOnCurveUnderlying(0x5ab5C56B9db92Ba45a0B46a207286cD83C15C939, 0x5e5A23b52Cb48F5E70271Be83079cA5bC9c9e9ac, USDC, 2, 0);
    }

    function dumpUSDCToDai() public returns(uint) {
        return dumpOnCurveUnderlying(0x5ab5C56B9db92Ba45a0B46a207286cD83C15C939, 0x5e5A23b52Cb48F5E70271Be83079cA5bC9c9e9ac, USDC, 2, 1);
    }
