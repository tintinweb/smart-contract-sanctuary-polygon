...
// File: contracts/base/StandardERC20.sol
...
// File: contracts/base/BasicVault.sol
...
// File: contracts/storage/RouteStorage.sol
...
contract Vault is BasicVault, ReentrancyGuard {
    ...
    function exitPool3(uint256 amount) external isAllowExit notInLup notAllowContract {
        ...
        uint256 beforeBal = wethToken.balanceOf(address(this));
        _exitPool(address(this), weth, amount);
        uint256 afterBal = wethToken.balanceOf(address(this));
        uint256 diff = afterBal.sub(beforeBal);
        if (diff > 0) {
            IWETH(weth).withdraw(diff);
