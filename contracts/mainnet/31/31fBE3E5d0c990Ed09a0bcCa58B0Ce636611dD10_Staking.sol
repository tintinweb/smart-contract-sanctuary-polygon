...
contract Staking is Ownable, ReentrancyGuard {
	...
	mapping(uint256 => uint256) public lastClaim; //token_id - timestamp
	IERC20 public karma; 
    IERC721 public buddies;
	...
	function claim( uint256 _tokenId) external nonReentrant {
        require(buddies.ownerOf(_tokenId) == msg.sender, "Must Own NFT" );
		uint256 unclaimed = unclaimedRewards(_tokenId);		
		lastClaim[_tokenId] = block.timestamp;
		require(karma.transfer(msg.sender, unclaimed));
	}

	//calculate unclaimed rewards for a token
	function unclaimedRewards(uint256 _tokenId) public view returns (uint256) {
		uint256 lastClaimDate = lastClaim[_tokenId];
		uint256 emissionRate = 5000000000000000000;

		//initial issuance
		if (lastClaimDate == uint256(0)) {
			return 100000000000000000000; // 100 $KARMA initially
		}
	...
	function setKarmaToken(IERC20 _contract) external onlyOwner {
		karma = _contract;
	}

    function setBuddyToken(IERC721 _contract) external onlyOwner {
        buddies = _contract;
    }
