...
contract SiloFactoryUIHelper is Ownable{
    ...
    */
    ...
    /***************************************public view *************************************/
    function getManagerStats(address _user) public view returns(uint currentBalance, uint minimumBalance, uint riskAdjustedBalance, uint warningBalance,uint96 riskBuffer, uint96 rejoinBuffer){
    ...
    function getMiniumTopupAmount(address _user) public view returns(uint miniumTopup){
        ...
        for(uint i=0; i< length; i++){
            ...
            if(silo.highRiskAction()){
                return miniumTopup = minimumBalance*3/2 * Manager.getRejoinBuffer() / uint96(10000);
            }
        }
    ...
    function viewSiloStrategyMetaData(uint siloID) public view returns(SiloInfo memory info){
        ...
        if(ISiloManagerFactory(SiloFactory.managerFactory()).managerApproved(_user)){
    ...
    */
    ...
    function depositAllowed(uint siloId) external view returns(bool){
        ISilo silo = ISilo(SiloFactory.siloMap(siloId));
        address user = SiloFactory.ownerOf(siloId);
        (uint currentBalance, uint minimumBalance, uint riskAdjustedBalance,,,) = getManagerStats(user);
    ...
    function getState(uint siloId) public view returns(string memory) {
        ISilo silo = ISilo(SiloFactory.siloMap(siloId));
        address user = SiloFactory.ownerOf(siloId);

        if(ISiloManagerFactory(SiloFactory.managerFactory()).managerApproved(user)){
            (uint currentBalance, uint minimumBalance, uint riskAdjustedBalance,,,) = getManagerStats(user);
        ...
        address manager = ISiloManagerFactory(SiloFactory.managerFactory()).userToManager(user);
