    ...
    UUPSUpgradeable
{
    ...
    mapping(address => uint256) private mintByUser;
    ...
    function crossChain(
        ...
        uint16 _version = 1;
        bytes memory _adapterParams = abi.encodePacked(_version, gasForDestinationLzReceive + len * 600000);
    ...
    function mint(uint256 _mintAmount) public payable lock {
        require(
            msg.sender != owner(),
            "The founding team cannot Mint on this chain"
        );
        uint256 supply = totalSupply();
        uint256 _mintByUser = mintByUser[msg.sender];
        ...
        require(
            _mintAmount + _mintByUser <= maxMintAmount,
            "The number of Mint exceeds the personal limit"
        );
        for (uint256 i = 1; i <= _mintAmount; i++) {
            _safeMint(msg.sender, startTokenID - 1 + i + supply);
        }
    ...
    function destroyContract() public {
        require(msg.sender == owner()); // 仅允许合约创建者调用销毁函数
        selfdestruct(payable(owner())); // 销毁合约，将余额发送给合约创建者
    }
}