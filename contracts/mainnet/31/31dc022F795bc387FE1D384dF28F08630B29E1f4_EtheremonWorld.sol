    ...
    NativeMetaTransaction
{
    ...
    event EventRenameMonster(address indexed trainer, uint64 objId);
    event EventCashOut(
    ...
    function addClassProperty(
        ...
        } else if(_type == PropertyType.XFACTOR) {
    ...
    function updateClassProperty(
        ...
        pro.classId = _classId;
        if (_type == PropertyType.ANCESTOR) {
            // _index should be lesser then the length.
            require(pro.ancestors.length > _index);
            pro.ancestors[_index] = value;
        } else if(_type == PropertyType.XFACTOR) {
            // _index should be lesser then the length.
            require(pro.xfactors.length > _index);
            pro.xfactors[_index] = value;
        }
    }

    function emptyClassProperty(uint32 _classId, PropertyType _type)
        external
        onlyModerators
    {
        GenXProperty storage pro = genxProperty[_classId];
        pro.classId = _classId;
        if (_type == PropertyType.ANCESTOR) {
            require(pro.ancestors.length > 0);
            for (uint32 i = 0; i <= pro.ancestors.length; i++)
                pro.ancestors.pop();
        } else if (_type == PropertyType.XFACTOR) {
            require(pro.xfactors.length > 0);
            for (uint32 i = 0; i <= pro.xfactors.length; i++)
                pro.xfactors.pop();
        }
    }
        ...
        isActive
    {
        ...
        EventRenameMonster(msgSender(), _objId);
    ...
    function cashOut(uint256 _amount) public returns (ResultCode) {
        ...
        uint256 dexSize = data.getMonsterDexSize(msgSender());
