...
contract MagpieTest is ReentrancyGuard, Ownable, IMagpieTest {
    ...
    // selectorId => selector
    mapping(uint16 => bytes4) private selectors;
    // ammId => selectorId
    mapping(uint16 => uint16) private ammSelectors;
    // ammId => amm
    ...
    function swap(bytes[] calldata hops, bytes[] calldata params) external payable nonReentrant override returns (uint256 amountOut) {
        Swap memory s = Swap({
            assetsLength: params[5].toUint8(0),
            i: 0,
            hopsLength: hops.length,
            amountIn: params[0].toUint256(0),
            accAmountIn: 0,
            lastAmountOut: 0,
            fromAssetAddress: params[1].toAddress(0),
            toAssetAddress: params[2].toAddress(0)
        });
        Hop memory h = Hop({
            j: 0,
            paramIndex: 0,
        ...
        uint256[] memory startingBalances = new uint256[](s.assetsLength+6);

        if (s.assetsLength > 255) {
        ...
        for (s.i; s.i < s.hopsLength; s.i++) {
            ...
            h.l = (hops[s.i].length - 6) / 2;
            h.payload = '';
            h.amountIn = (h.fromAssetAddress == s.fromAssetAddress ? params[h.amountInIndex].toUint256(0) : s.lastAmountOut);
            h.toAssetAddress = params[h.toAssetIndex].toAddress(0);
            ...
            for (h.j = 0; h.j < h.l; h.j++) {
                h.paramIndex = hops[s.i].toUint16(6 + s.i * 2);
                h.payload = bytes.concat(h.payload, h.paramIndex == h.amountInIndex ? abi.encodePacked(h.amountIn) : params[h.paramIndex]);
            }

            s.fromAssetAddress.approve(amms[h.ammId].ammAddress, h.amountIn);
            bytes memory data = abi.encodeWithSelector(selectors[ammSelectors[h.ammId]], h.payload);
        ...
        if (amountOut < params[4].toUint256(0)) {
            revert InsufficientOutputAmount();
        }

        if (msg.sender != magpieSimulatorAddress) {
            s.toAssetAddress.transfer(payable(params[3].toAddress(0)), amountOut);
        }
    }
}
...
interface IMagpieTest {
    ...
    // amountIn, fromAsset, toAsset, to, amountOutMin, hopsLength
