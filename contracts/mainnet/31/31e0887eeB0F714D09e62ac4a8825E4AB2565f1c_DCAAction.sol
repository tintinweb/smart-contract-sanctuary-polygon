...
contract DCAAction is BaseSiloAction {
    ...
    address public gov = 0xEe5578a3Bab33F7A56575785bb4846B90Be37d50;
    ...
    constructor(string memory _name, address _siloFactory) {
        name = _name;
        metaData = "address[4],address[4],uint[4],uint,address[],address";
        factory = _siloFactory;
        feeName = "DCA Harvest";
    }

    function claimGFIReward() private {
    ...
    function directSwap(
        ...
        bestQuote = oracle.findOptimalRouter(path, inputAmount);

        uint256 amountOut = bestQuote.amountOut;

        SafeERC20.safeApprove(IERC20(input), bestQuote.router, inputAmount);
        ...
        if (amountOut > 0) {
            ...
            if (result) {
                ...
                if (result) {
                    outputAmounts[0] = amounts[amounts.length - 1];
                }
            }
        } 
    }
    ...
    function dacAction(
        ...
        (inputs, outputs, , buyAmount, path, _oracle) = abi.decode(
            configuration,
            (address[4], address[4], uint256[4], uint256, address[], address)
        );

        if (inputAmounts[0] < buyAmount) {
            return outputAmounts;
        }

        IExchangeRateOracle oracle = IExchangeRateOracle(_oracle);

        outputAmounts = directSwap(
            inputs[0],
            outputs[0],
            path,
            buyAmount,
    ...
    function enter(
        ...
        outputAmounts = dacAction(implementation, configuration, inputData);
    }
    ...
    function decodeConfig(bytes memory configData)
        ...
        returns (
            address[4] memory inputs,
            address[4] memory outputs,
            uint256[4] memory triggers,
            uint256 buyAmount,
            address[] memory _path,
            address oracle
        )
    {
        (inputs, outputs, triggers, buyAmount, _path, oracle) = abi.decode(
            configData,
            (address[4], address[4], uint256[4], uint256, address[], address)
        );
    }
        ...
        returns (bool)
    {
        ...
        (inputs, outputs, triggers, buyAmount, _path, _oracle) = abi.decode(
            configData,
            (address[4], address[4], uint256[4], uint256, address[], address)
        );
        ...
        if (inputs[0] == address(0) || outputs[0] == address(0) || _oracle == address(0)) {
            return false;
        }
        ...
        if (_path[0] != inputs[0] || _path[_path.length - 1] != outputs[0]) {
            return false;
        }

        IExchangeRateOracle oracle = IExchangeRateOracle(_oracle);
        Quote memory bestQuote = oracle.findOptimalRouter(_path, buyAmount);
    ...
    //         return false;
    //     }
    //     uint256[4] memory triggerAmounts;
    //     address[4] memory inputs;
    //     uint buyAmount;
    //     (inputs, , triggerAmounts,buyAmount) = abi.decode(
    //         configuration,
    //         (address[4], address[4], uint256[4],uint)
    //     );

    //     if (IERC20(inputs[0]).balanceOf(msg.sender) >= triggerAmounts[0]) {
    //         return true;
    //     }
