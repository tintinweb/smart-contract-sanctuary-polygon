...
contract KoalaToken is ERC20("KoalaToken", "KOA"), Ownable {
  ...
  constructor() public {
    ...
    // maunalMint for initialize project setup
    manualMint(msg.sender, MANUAL_MINT_LIMIT);
  }

  modifier afterSetup() {
    require(startReleaseBlock != 0 && endReleaseBlock != 0, "release parameters not set");
    _;
  }

  function setup(uint256 _startReleaseBlock, uint256 _endReleaseBlock) public onlyOwner {
    require(startReleaseBlock == 0 && endReleaseBlock == 0, "release parameters have set");
    require(_startReleaseBlock > 0, "invalid start release block");
  ...
  function internalMint(address _to, uint256 _amount) private {
  ...
  function mint(address _to, uint256 _amount) public afterSetup onlyOwner {
    internalMint(_to, _amount);
  }

  function burn(address _account, uint256 _amount) external afterSetup onlyOwner {
  ...
  function lock(address _account, uint256 _amount) external afterSetup onlyOwner {
  ...
  function canUnlockAmount(address _account) public view returns (uint256) {
    // When block number less than startReleaseBlock, no KOAs can be unlocked
    if (startReleaseBlock == 0) {
      return 0; // before release parameters setup
    }
    if (block.number < startReleaseBlock) {
      return 0;
    }
    // When block number more than endReleaseBlock, all locked KOAs can be unlocked
    else if (block.number >= endReleaseBlock) {
      return _locks[_account];
    }
    // When block number is more than startReleaseBlock but less than endReleaseBlock,
    // some KOAs can be released
    else
    {
  ...
  function unlock() external afterSetup {
    require(_locks[msg.sender] > 0, "no locked KOAs");
  ...
  // @dev move KOAs with its locked funds to another account
  ...
  function delegateBySig(
    ...
    require(signatory != address(0), "KOA::delegateBySig: invalid signature");
    require(nonce == nonces[signatory]++, "KOA::delegateBySig: invalid nonce");
    require(now <= expiry, "KOA::delegateBySig: signature expired");
  ...
  function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
    require(blockNumber < block.number, "KOA::getPriorVotes: not yet determined");
  ...
  function _delegate(address delegator, address delegatee) internal {
    ...
    uint256 delegatorBalance = balanceOf(delegator); // balance of underlying KOAs (not scaled);
  ...
  function _writeCheckpoint(
    ...
    uint32 blockNumber = safe32(block.number, "KOA::_writeCheckpoint: block number exceeds 32 bits");
