    ...
    IFeeManagerUpgradeable
{
    ...
    */
    ...
    /**
        @notice Used to set multiple fee token price in USD * 10**9 manually. 
        @notice Only callable by admin or Fee Setter.
        @param  feeTokens Addresses of the fee token.
        @param  pricesInUSDx10e9 Prices in USD * 10**9.
     */
    function setMultipleFeeTokenToPriceX10e9(address[] calldata feeTokens, uint256[] calldata pricesInUSDx10e9)
        external
        virtual
        override
        onlyRole(FEE_SETTER_ROLE)
    {
        for (uint256 i = 0; i < feeTokens.length; i++) {
            _feeTokenToPriceX10e9[feeTokens[i]] = pricesInUSDx10e9[i];
        }
    }
