...
// File: contracts/EthermonTradeDebug.sol
...
contract EthermonTradeDebug is EthermonEnum, BasicAccessControl, SafeMathEthermon {
    ...
    function buyItemEmon11(uint64 _objId) external   view returns (uint256) {
    ...
    return requestPrice;
     ...
     function buyItemEmon12(uint64 _objId) external   view  returns (address) {
        return msgSender();     
    ...
    function buyItemEmon1(uint64 _objId)    external isActive {
        // check item is valid to sell
        EthermonTradeData monTradeData = EthermonTradeData(
            tradingMonDataContract
        );
        ...
        price = emon.balanceOf(msgSender());
    ...
    function buyItemEmon(uint64 _objId) external isActive {
        // check item is valid to sell
        EthermonTradeData monTradeData = EthermonTradeData(
            tradingMonDataContract
        );
        ...
        uint256 emonPrice = safeSubtract(requestPrice, fee) * emonRatio;
