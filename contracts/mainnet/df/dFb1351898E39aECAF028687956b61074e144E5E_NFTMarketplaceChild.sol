...
contract NFTMarketplaceChild is IERC721Receiver, Ownable, ILayerZeroReceiver {
    ...
    bytes public mainChainAddress;
    uint16 public mainChainId;

    event Received(bytes payload);
    ...
    //     (uint _nativeFee, uint _zeroFee) = endpoint.estimateFees(
    //         _chainId, mainChainAddress, _payloadBytes, false, bytes("")
    //     );
    //     uint _relayerFee = _nativeFee + _zeroFee;
    ...
    //         _payloadBytes,
    //         payable(msg.sender),
    //         address(0xAA2762F63944e9a6185879507b414417D1c34724),
    //         bytes("")
    //     );
    ...
    function setMainChain(bytes memory _address, uint16 _chainId) public onlyOwner {
    ...
    function lzReceive(uint16 , bytes memory _srcCounterMockAddress, uint64 _nonce, bytes memory _payload) override external {
        ...
        emit Received(_payload);
    }

    receive () external payable {
        emit ReceivedPayable();
    }
