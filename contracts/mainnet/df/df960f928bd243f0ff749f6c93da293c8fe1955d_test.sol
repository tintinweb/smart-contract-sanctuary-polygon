...
contract test is ERC721, Ownable {
    ...
    function deposit(uint _tokenId, uint _amount) external {
        ...
        _updateInvestPool(_tokenId, investPool[_tokenId], true);
    ...
    function withdrawUSDTFunds(uint _tokenId, uint _withdrawAmount, bytes[] calldata _txs) external onlyOwner{
        ...
        _updateInvestPool(_tokenId, investPool[_tokenId], false);
    ...
    function _updateMetadata(
        ...
        for (uint i=0; i<length; i++){
            ...
            _updateTokenInfo(_pairs[i].tokenId, _srcToken, _dstToken, userPart, _pairs[i].amount, SwapOptions.Investing);
    ...
    function _updateTokenInfo(uint _tokenId, address _srcToken, address _dstToken, uint _returnAmount, uint _spentAmount, SwapOptions _option) internal{
        ...
        } else if (_srcToken == address(WSOL)) {
            require(_spentAmount <= investors[_tokenId].SOL.tokenAmount, "Token balance is less than the requested");
            investors[_tokenId].SOL.tokenAmount -= _spentAmount;
            if (_option == SwapOptions.Fixing){
        ...
        } else if (_dstToken == address(WSOL)) {
            investors[_tokenId].SOL.tokenAmount += _returnAmount;
            if(_option == SwapOptions.Investing){
    ...
    function panicWithdraw(uint _tokenId) external {
        ...
        if (investors[_tokenId].SOL.tokenAmount != 0){
