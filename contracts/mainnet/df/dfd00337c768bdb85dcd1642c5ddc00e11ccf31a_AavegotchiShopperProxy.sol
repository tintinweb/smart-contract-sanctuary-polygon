...
// File: interfaces/IAavegotchi.sol
...
// File: interfaces/StakingContract.sol
...
// File: AavegotchiShopperProxy.sol
...
contract AavegotchiShopperProxy {
    ...
    function setApprovals() public onlyOwner{
        ...
        IERC20Upgradeable(ghstAddress).approve(diamondAddress, MAX_UINT);
        
        //staking address needs to take GHST for staking GHST
        IERC20Upgradeable(ghstAddress).approve(stakingAddress, MAX_UINT);
    ...
    function findERC1155ListingId (uint256 listingId, uint256 priceInWei, uint256 maxAttempts) private view returns (uint256) {
        ...
        for (uint256 attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            listing = IAavegotchi(diamondAddress).getERC1155Listing(foundListingId);
            if (listing.listingId == foundListingId) {
                require(priceInWei == listing.priceInWei, "Proxy - ERC1155Marketplace: wrong price or price changed");
                require(listing.timeCreated != 0, "Proxy - ERC1155Marketplace: listing not found");
                require(listing.sold == false, "Proxy - ERC1155Marketplace: listing is sold out");
                require(listing.cancelled == false, "Proxy - ERC1155Marketplace: listing is cancelled");
    ...
    function executeERC1155Listing(uint256 listingId, uint256 quantity,
        ...
        require(IERC20Upgradeable(ghstAddress).balanceOf(address(this)) >= cost, "Proxy - ERC1155Marketplace: not enough GHST");
        ...
        IAavegotchi(diamondAddress).executeERC1155Listing(foundListingId, quantity, priceInWei);
    }
    ...
    function findERC721ListingId (uint256 listingId, uint256 maxAttempts) private view returns (uint256) {
        ...
        for (uint256 attempt = 0; attempt < maxAttempts; attempt++) {
            foundListingId = listingId + attempt;
            listing = IAavegotchi(diamondAddress).getERC721Listing(foundListingId);
    ...
    function executeERC721Listing(uint256 listingId, uint256 maxAttempts) public onlyOwner {
        ...
        require(foundListingId != 0, 'Proxy - Listing could not be found');
        IAavegotchi(diamondAddress).executeERC721Listing(foundListingId);
    }
    ...
    function withdrawERC20(address erc20Address, uint256 amount) public onlyOwner {
        require(IERC20Upgradeable(erc20Address).transfer(msg.sender, amount), "Transfer failed");
    }
}