...
contract EthermonStaking is EthermonStakingBasic {
    ...
    function updateTokens(
        ...
        if (currentTime >= data.endTime) {
            ...
            if (data.teamPower > newSumTeamPower) newSumTeamPower = 1;
            else newSumTeamPower -= data.teamPower;

            if (data.emons > totalStaked) totalStaked = 0;
            else totalStaked -= data.emons;
    ...
    function withdrawRewards(uint256 _lockId) external {
        ...
        if (timeElapsed > 0 && data.lastCalled < data.endTime) {
            ...
            if (data.teamPower > newSumTeamPower) newSumTeamPower = 0;
            else newSumTeamPower -= data.teamPower;
