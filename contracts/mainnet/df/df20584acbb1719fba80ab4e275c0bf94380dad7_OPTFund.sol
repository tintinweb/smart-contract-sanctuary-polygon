...
contract OPTFund is Ownable, ReentrancyGuard {
    ...
    function borrowUsingOPT3(uint256 _OPT3Amount) external {
        ...
        if(existsInLoanTakers(msg.sender)) {
            uint256 loanID = adrToLoanID[msg.sender];
    ...
    function existsInLoanTakers(address _address) public view returns (bool) {
        for(uint256 i = 0; i < loanTakers.length; i++) {
            if(loanTakers[i] == _address) {
                return true;
            }
        }
        return false;
