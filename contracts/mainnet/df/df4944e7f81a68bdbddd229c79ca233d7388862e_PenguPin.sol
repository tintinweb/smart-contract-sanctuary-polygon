...
import "./LinkForge/LinkForgeBase.sol";

library PenguPinStorage {
    bytes32 internal constant STORAGE_SLOT =
        keccak256("PudgyPenguin.contracts.storage.PenguPin");
...
/// @title PenguPin Soulbound Token Contract
/// @author 0xEstarriol (Pudgy Penguins Penguineering Team & Ctor Lab), Harry (Ctor Lab)
contract PenguPin is
    OwnableUpgradeable,
    ERC1155Upgradeable,
    LinkForgeBase,
    UUPSUpgradeable
{
    ...
    function initialize(
        ...
        __ERC1155_init("");
        PenguPinStorage.layout().name = name_;
        PenguPinStorage.layout().symbol = symbol_;

        PenguPinStorage.layout().baseURI = baseURI_;
        PenguPinStorage.layout().baseURISuffix = baseURISuffix_;
    ...
    function name() public view returns (string memory) {
        return PenguPinStorage.layout().name;
    }

    function symbol() public view returns (string memory) {
        return PenguPinStorage.layout().symbol;
    }
    ...
    function uri(uint256 id) public view override returns (string memory) {
        return
            string(
                abi.encodePacked(
                    PenguPinStorage.layout().baseURI,
                    id.toString(),
                    PenguPinStorage.layout().baseURISuffix
                )
            );
    }
    ...
    function setURI(
        ...
        PenguPinStorage.layout().baseURI = _base;
        PenguPinStorage.layout().baseURISuffix = _suffix;
    }
    ...
    function _processClaim(
        ...
        if (_isGelatoRelay(msg.sender)) {
            // By capping the gas usage for the mint, this prevents the griefing attack by using a smart contact as the claimant.
            if (beforeGas - afterGas > _max_mint_gasusage()) revert();
        }
    }

    function toggleGelatoRelayEnabled() external onlyOwner {
        LinkForgeBaseStorage.layout().gelatoRelayEnabled = !LinkForgeBaseStorage
            .layout()
            .gelatoRelayEnabled;
    }

    function setCertificateAuthority(
        address certificateAuthority_
    ) external onlyOwner {
        _setDefaultCertificateAuthority(certificateAuthority_);
    }
    ...
    function verifyCertificate(
        ...
        return _verifyCertificate(signer, deadline, data, certificate);
    }
