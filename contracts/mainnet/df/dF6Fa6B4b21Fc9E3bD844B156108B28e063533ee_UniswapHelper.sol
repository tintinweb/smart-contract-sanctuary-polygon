...
contract UniswapHelper is IUniswapV3MintCallback {
    ...
    int24 public tickWidth = 2000; // for 2200 r = 1.2461 for 4000 r = 1.4918
    ...
    function _calcLimitOrderRebalance(uint256 _eta) internal view returns (bool) {
        ...
        uint256 wethInUsdc = _wethToWant(limitOrder.token1.add(strategy.balanceOfWeth()), true);
        uint256 usdcValue = limitOrder.token0.add(wethInUsdc).add(strategy.balanceOfWant());
        // TODO: change this to make it more efficient and dynamic without he max Limit order size
        if (usdcValue > _eta.div(factor)) {
            if (
    ...
    function setLimitOrderMaxSize(uint16 _limitOrderMaxSize, uint8 _factor)
        external
        onlyAuthorized
    {
