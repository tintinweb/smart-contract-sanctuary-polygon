...
contract LendingHelper {
    ...
    uint256 internal minRebalanceRatio = 5250;
    uint256 internal maxRebalanceRatio = 5520;
    ...
    function _unwindPartialPosition(uint256 _amountInWeth) internal {
        // TODO include strategy.swapToWeth(_amountInWeth.sub(_amountInWeth)) to unwind in any situation;
        uint256 availableWeth = strategy.balanceOfWeth();
        if (strategy.balanceOfWethInWant() < _amountInWeth) {
        ...
        if (toRepayInWeth > wethDust) {
            ...
            // leaving some borrow available for the next round.
            _withdrawExcessCollateral(targetCollatRatio);
        }
    }
