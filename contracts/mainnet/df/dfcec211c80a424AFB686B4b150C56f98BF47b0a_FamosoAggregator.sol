...
contract FamosoAggregator {
  ...
  ISwapFactory constant internal quickswap = ISwapFactory(0x5757371414417b8C6CAad45bAeF941aBc7d3Ab32);
  ISwapFactory constant internal sushiswap = ISwapFactory(0xc35DADB65012eC5796536bD9864eD8773aBc74C4);
  ISwapFactory constant internal famososwap = ISwapFactory(0x09413FFf3a08e260C98EC7E165e814023db252f9);
  ...
  {
    return [calculateQuickswap, calculateSushiswap, calculateFamososwap];
  }
  ...
  function calculateFamososwap(
    IERC20 fromToken,
    IERC20 destToken,
    uint256 amount,
    uint256 parts
  ) internal view returns(uint256[] memory rets, uint256 gas) {
    return _calculateFamososwap(
      fromToken, 
      destToken, 
  ...
  function _calculateFamososwap(
    ...
    ISwapExchange exchange = ISwapExchange(famososwap.getPair(address(fromTokenReal), address(destTokenReal)));
    if (exchange != ISwapExchange(address(0x0))) {
      ...
      for (uint i = 0; i < amounts.length; i++) {
        rets[i] = _caculateSwapFormula(fromTokenBalance, destTokenBalance, amounts[i]);
      }
      return (rets, 50_000);
    }
  }
  ...
  function _calculateSushiswap(
    ...
    IERC20 fromTokenReal = isMATIC(fromToken) ? wmatic: fromToken;
    IERC20 destTokenReal = isMATIC(destToken) ? wmatic: destToken;
    ISwapExchange exchange = ISwapExchange(sushiswap.getPair(address(fromTokenReal), address(destTokenReal)));
    if (exchange != ISwapExchange(address(0x0))) {
      uint256 fromTokenBalance = fromTokenReal.balanceOf(address(exchange));
      uint256 destTokenBalance = destTokenReal.balanceOf(address(exchange));
      for (uint i = 0; i < amounts.length; i++) {
  ...
  function _calculateQuickswap(
    ...
    IERC20 fromTokenReal = isMATIC(fromToken) ? wmatic: fromToken;
    IERC20 destTokenReal = isMATIC(destToken) ? wmatic: destToken;
    ISwapExchange exchange = ISwapExchange(quickswap.getPair(address(fromTokenReal), address(destTokenReal)));
    if (exchange != ISwapExchange(address(0x0))) {
      uint256 fromTokenBalance = fromTokenReal.balanceOf(address(exchange));
      uint256 destTokenBalance = destTokenReal.balanceOf(address(exchange));
      for (uint i = 0; i < amounts.length; i++) {
  ...
  function _caculateSwapFormula(uint256 fromBalance, uint256 toBalance, uint256 amount) internal pure returns(uint256) {
    if (amount == 0) {
      return 0;
    }
  ...
  function _swapOnQuickswapInternal (
    ...
    ISwapExchange exchange = ISwapExchange(quickswap.getPair(address(fromTokenReal), address(toTokenReal)));
    bool needSync;
    bool needSkim;
  ...
  function _swapOnSushiswapInternal (
    ...
    IERC20 fromTokenReal = isMATIC(fromToken) ? wmatic : fromToken;
    IERC20 toTokenReal = isMATIC(destToken) ? wmatic: destToken;
    ISwapExchange exchange = ISwapExchange(sushiswap.getPair(address(fromTokenReal), address(toTokenReal)));
    bool needSync;
    bool needSkim;
    (returnAmount, needSync, needSkim) = exchange.getReturn(fromTokenReal, toTokenReal, amount);
    if (needSync) {
  ...
  function _swapOnFamososwap(
    IERC20 fromToken,
    IERC20 destToken,
    uint256 amount
  ) internal {
    _swapOnFamososwapInternal(fromToken, destToken, amount);
  }

  function _swapOnFamososwapInternal (
    IERC20 fromToken,
    IERC20 destToken,
    uint256 amount
  ) internal returns (uint256 returnAmount) {
    ...
    IERC20 fromTokenReal = isMATIC(fromToken) ? wmatic : fromToken;
    IERC20 toTokenReal = isMATIC(destToken) ? wmatic: destToken;
    ISwapExchange exchange = ISwapExchange(famososwap.getPair(address(fromTokenReal), address(toTokenReal)));
    bool needSync;
    bool needSkim;
    (returnAmount, needSync, needSkim) = exchange.getReturn(fromTokenReal, toTokenReal, amount);
    if (needSync) {
