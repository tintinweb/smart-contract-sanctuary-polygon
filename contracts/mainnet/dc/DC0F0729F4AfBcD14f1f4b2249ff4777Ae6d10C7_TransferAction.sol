...
/**
 * @dev Transfers tokens to their intended recipient address
 */
...
/*          
        ...
        Token B->[              ]->X2
        Token C->[              ]->X3
        Token D->[______________]->X4

Note: Token[B,C,D] can be 0x0000....0000
Note: there is no output this action should be at the end of an action stack
Note: Token[A,B,C,D] can be the same token
Note: X 1,2,3,4 are up to 4 recipient addresses

*/
contract TransferAction is BaseSiloAction{

    constructor(string memory _name, address _siloFactory, address[4] memory _inputs, address[4] memory _outputs, address[4] memory _recipients) {
        name = _name;
        factory = _siloFactory;
        //configurationData = abi.encode(_inputs, _outputs, _recipients);
    }
    ...
    function enter(address implementation, bytes memory configuration, bytes memory inputData) public override returns(uint[4] memory outputAmounts){
        ...
        if(storedConfig.length > 0){
            (input, output, recipients) = abi.decode(storedConfig, (address[4],address[4],address[4]));
        }
        else{
            (input, output, recipients) = abi.decode(configuration, (address[4],address[4],address[4]));
        }
        uint[4] memory amounts = abi.decode(inputData, (uint[4]));
        for(uint i=0; i<4; i++){
            if(recipients[i] != address(0) && amounts[i] > 0){
                SafeERC20.safeTransfer(IERC20(input[i]), recipients[i], amounts[i]);
            }
        }
    }
    ...
    function createConfig(address[4] memory _inputs, address[4] memory _outputs, address[4] memory _recipients) public pure returns(bytes memory configData){
        configData = abi.encode(_inputs, _outputs, _recipients);
    }
