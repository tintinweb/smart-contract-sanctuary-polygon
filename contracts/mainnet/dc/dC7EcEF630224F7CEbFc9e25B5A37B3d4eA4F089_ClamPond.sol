    ...
    ReentrancyGuardUpgradeable
{
    ...
    function rebase() external onlyRewardManager {
        ...
        uint256 clamAmount = _swapUsdPlusToClam(rewardAmount);
        // stake CLAM
        IERC20(CLAM).approve(address(pearlBank), clamAmount);
        pearlBank.stake(clamAmount);
    ...
    function _swapUsdPlusToClam(uint256 amountIn_) internal returns (uint256) {
        IERC20(usdPlus).approve(swapRouter, amountIn_);
        IDystopiaRouter dystRouter = IDystopiaRouter(swapRouter);
        IDystopiaRouter.Route[] memory dystRoutes = new IDystopiaRouter.Route[](
            2
        );
        dystRoutes[0] = IDystopiaRouter.Route({
            from: usdPlus,
            to: usdc,
            stable: true
        });
        dystRoutes[1] = IDystopiaRouter.Route({
            from: usdc,
            to: CLAM,
            stable: false
        });
        return
            dystRouter.swapExactTokensForTokens(
                amountIn_,
                dystRouter.getAmountsOut(amountIn_, dystRoutes)[2],
                dystRoutes,
                address(this),
                block.timestamp
            )[2];
    ...
    function withdrawRewards() external onlyAdmin {
        require(stopRebase, 'rebase is not stopped');
        pearlBank.claimRewards();
        uint256 rewardAmount = IERC20(usdPlus).balanceOf(address(this));
        IERC20(usdPlus).transfer(msg.sender, rewardAmount);
    }
