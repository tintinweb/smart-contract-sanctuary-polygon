...
contract MgNftDex is DEXConfig, ReentrancyGuard, OrderValidator, TransferHelper {
    ...
    function fulfillFixedPriceOrder(FixedPriceOrder calldata _makerOrder, bytes calldata _makerSig)
        ...
        nonReentrant
    {
        // 验证订单正确性，参数为订单信息和签名信息，订单信息从中心化数据查询，签名信息从前端将订单信息打包签名,  taker 吃单=购买者
    ...
    function cancelOrder(FixedPriceOrder[] calldata _orders) external {
        ...
        for (uint256 i = 0; i < _orders.length; i++) {
            ...
            require(maker == order.maker,"MakerNotMatch");

            (, , bytes32 orderHash, ) = deriveOrder(order);

            require(!ordersStatus[orderHash].matched, "OrderIsDealt");
        ...
        returns (address)
    {
        ...
        return ECDSA.recover(message, _signature);
    }
}
...
abstract contract OrderValidator is NonceManager, OrderDeriver {
        ...
        )
    {
        ...
        require(maker == _makerOrder.maker,"MakerNotMatch");

        // 校验买卖双方是不同地址
        require(_makerOrder.taker == address(0),"TakerNotMatch");

        //校验订单nonce值，前端默认传0，扩展批量取消订单的方法（预留）
        require(_makerOrder.makerNonce == _nonce(maker),"MakerNonceNotMatch");
        ...
        // 校验手续费
        require((_makerOrder.royaltyRate < _maxRoyaltyRate()), "TooHighRoyaltyRate");
...
abstract contract TransferHelper is DEXConstantsAndStructs {
    ...
    function _transferFrom(
        ...
        require(status, "AssertCallError");
        require(!((ret.length > 0 && !abi.decode(ret, (bool)))), "TransferERC20Failed");
...
abstract contract DEXConstantsAndStructs is DEXEventsAndErrors {
    ...
    struct Assets {
        ...
        uint256 nftAmount;
        // address(0) means ETH, 0x000000000000000000000000000000000 0地址硬编码代表eth原生币，非eth支付采用支付token代币地址
        address ft;
        // price
        uint256 ftAmount;
    }
    ...
    struct FixedPriceOrder {
        ...
        // 订单过期时间
        uint64 expireAt;
        // nonce， 默认为0， 扩展批量取消订单预留字段， 取消订单后，nonce加1， 当前后台取消订单，该字段预留链上取消订单操作
        uint64 makerNonce;
