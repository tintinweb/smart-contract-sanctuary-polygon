...
contract ChancePrediction is Ownable, Pausable, ReentrancyGuard {
    ...
    uint80 public testCurrentRoundId;
    ...
    function setTokenAddr(address w_addr, address v_addr) public onlyOwner {
        WTOKEN = w_addr;
        VAULT_ADDR = v_addr;
    }
    ...
    function _invite(uint256 amount, string memory _inviteCode) private {
        ...
        if (inviteAddr != BLACKHOLE_ADDR) {
            invite_amount = amount * 3 / 100;
            invite_amount = invite_amount * 10 / 100;
            inviteMap[inviteAddr] += invite_amount;
     ...
     * @param epoch: epoch
     */
    function betBear(uint256 epoch, string memory _inviteCode) external payable whenNotPaused nonReentrant notContract {
        ...
        IWrappedToken(WTOKEN).transfer(VAULT_ADDR, msg.value);

        _invite(amount, _inviteCode);
     ...
     * @param epoch: epoch
     */
    function betBull(uint256 epoch, string memory _inviteCode) external payable whenNotPaused nonReentrant notContract {
        require(epoch == currentEpoch, "Bet is too early/late");
        require(_bettable(epoch), "Round not bettable");
        require(msg.value >= minBetAmount, "Bet amount must be greater than minBetAmount");
        require(ledger[epoch][msg.sender].amount == 0, "Can only bet once per round");
    ...
    function claim(uint256[] calldata epochs) external nonReentrant notContract {
        ...
        if (reward > 0) {
            ...
            if (treasuryAmount > 0){
                ...
                treasuryAmount = 0;
                IChanceVault(VAULT_ADDR).withdrow(address(msg.sender), reward, currentTreasuryAmount);
            } else {
                IChanceVault(VAULT_ADDR).withdrow(address(msg.sender), reward, 0);
            }
        }
    }
    ...
    function executeRound() external whenNotPaused onlyOperator {
        ...
        (uint80 currentRoundId, int256 currentPrice) = getTestPriceFromOracle();
    ...
    function getTestPriceFromOracle() private returns (uint80 currentRoundId, int256 currentPrice) {
        uint256 _seed = uint256(keccak256(abi.encode(block.difficulty, block.timestamp)));
        currentPrice = int256(395 + _seed%(10));
        testCurrentRoundId += 1;
        return (testCurrentRoundId, currentPrice);
    }
