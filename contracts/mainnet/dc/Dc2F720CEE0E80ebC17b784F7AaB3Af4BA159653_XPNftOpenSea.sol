...
// File lib/opensea-filter/src/IOperatorFilterRegistry.sol
...
// File lib/opensea-filter/src/OperatorFilterer.sol
...
// File lib/opensea-filter/src/DefaultOperatorFilterer.sol
...
// File @openzeppelin/contracts/utils/math/Math[email protected]
...
// File contracts/XPNftOpenSea.sol

pragma solidity ^0.8.0;

contract XPNftOpenSea is
    Ownable,
    ERC721Enumerable,
    BridgeNFT,
    ERC2981,
    DefaultOperatorFilterer
{
    ...
    modifier onlyMinter {
        require(msg.sender == minter, "Not a Minter. Unauthorised call.");
        _;
    }

    modifier onlyAdmin {
        require(msg.sender == admin, "Not an Admin. Unauthorised call.");
        _;
    }
    ...
    */
    ...
    function mint(
        address to,
        uint256 id,
        bytes calldata rawInfo
    ) external override onlyMinter {
        if (rawInfo.length != 0) {
            SetRoyalty memory setRoyaltyInfo = abi.decode(
                rawInfo,
                (SetRoyalty)
            );
            if (setRoyaltyInfo.receiver != address(0)) {
