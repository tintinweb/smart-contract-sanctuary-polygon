...
abstract contract LibertiFactory {
    ...
    /// @notice Returns the predicted address of the instance, for a given salt value.
    function predictInstanceAddress(bytes32 salt) external view returns (address) {
        return implementation.predictDeterministicAddress(salt);
    }

    /// @notice Creates a new vault and simultaneously initialize the instance.
...
import "./LibertiFactory.sol";
import "./LibertiVaultETH.sol";

contract LibertiFactoryETH is LibertiFactory {
    constructor(address _delegatedPriceFeed, address _weth)
        LibertiFactory(address(new LibertiVaultETH(_delegatedPriceFeed, _weth)))
    {
    ...
    OwnableUpgradeable
{
    ...
    function rebalance(
        ...
        return swap(data, address(this), type(uint256).max);
    }
...
import "../LibertiFactory.sol";
import "./LibertiVaultETHMock.sol";

contract LibertiFactoryETHMock is LibertiFactory {
    constructor(address _feedDirectory, address _weth)
        LibertiFactory(address(new LibertiVaultETHMock(_feedDirectory, _weth)))
    {
        // solhint-disable-previous-line no-empty-blocks
    }
}
...
import "../LibertiFactory.sol";
import "./LibertiVaultMock.sol";

contract LibertiFactoryMock is LibertiFactory {
    constructor(address _feedDirectory)
        LibertiFactory(address(new LibertiVaultMock(_feedDirectory)))
    {
        // solhint-disable-previous-line no-empty-blocks
    }
}
...
contract LibertiVaultETHMock is LibertiVaultETH {
    address private constant AGGREGATION_ROUTER_V4 = 0x1111111254fb6c44bAC0beD2854e76F90643097d;

    Weth9 private immutable weth;

    constructor(address _feedDirectory, address _weth) LibertiVaultETH(_feedDirectory, _weth) {
        weth = Weth9(_weth);
    }

    function transferEthAndSwap(bytes calldata data)
        external
        payable
        returns (uint256 returnAmount)
    {
        (, SwapDescription memory desc, ) = abi.decode(data[4:], (address, SwapDescription, bytes));
        weth.deposit{value: desc.amount}();
        SafeERC20.safeIncreaseAllowance(IERC20(desc.srcToken), AGGREGATION_ROUTER_V4, desc.amount);
        // solhint-disable-next-line avoid-low-level-calls
        (bool success, bytes memory returndata) = AGGREGATION_ROUTER_V4.call(data);
        if (!success) {
    ...
    function justSwap(bytes calldata data) external returns (uint256 returnAmount) {
        (, SwapDescription memory desc, ) = abi.decode(data[4:], (address, SwapDescription, bytes));
        SafeERC20.safeIncreaseAllowance(IERC20(desc.srcToken), AGGREGATION_ROUTER_V4, desc.amount);
        // solhint-disable-next-line avoid-low-level-calls
        (bool success, bytes memory returndata) = AGGREGATION_ROUTER_V4.call(data);
...
contract LibertiVaultMock is LibertiVault {
    address private constant AGGREGATION_ROUTER_V4 = 0x1111111254fb6c44bAC0beD2854e76F90643097d;

    constructor(address _feedDirectory) LibertiVault(_feedDirectory) {
        // solhint-disable-previous-line no-empty-blocks
    }

    function transferFromAndSwap(bytes calldata data) external returns (uint256 returnAmount) {
        (, SwapDescription memory desc, ) = abi.decode(data[4:], (address, SwapDescription, bytes));
