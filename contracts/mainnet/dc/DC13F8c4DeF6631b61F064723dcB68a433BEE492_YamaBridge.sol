...
contract YamaBridge is NonblockingLzApp {
    ...
    function sendStargate(
      ...
      bytes memory payload = generateLzPayload(
        ...
        uint256(hashStargateMsg(stargateMsg)),
        0,
        0
      );
    ...
    function handleWithdraw(
      ...
      withdrawn = router.withdraw(endpoint, amount);
    ...
    function processLzParams(bytes memory payload) internal returns (
      ...
      (callType, receiptId, amount, endpoint) = abi.decode(
        payload,
    ...
    function _nonblockingLzReceive(
      ...
      if (callType == uint16(CrossChainCall.VERIFY_MSG)) {
        bytes32 kHash = bytes32(receiptId);
...
contract YamaRouter is Ownable {
  ...
  function withdraw(uint256 id, uint256 LPTokenAmount) public returns (
    ...
    depositorPay = (LPTokenAmount * depositorRatio(id)) / precision;
