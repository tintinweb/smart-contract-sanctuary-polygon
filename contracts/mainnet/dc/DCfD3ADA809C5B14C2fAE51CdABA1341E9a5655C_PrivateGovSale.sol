...
contract PrivateGovSale is
        ...
        if(isAbleToBuy(_user, _boosterRunning)){
            ...
            if(_boosterRunning == 1){
                if(userStakedToken > 0){
                    userAlloc = uint128(SaleLibrary.calcAllocFromKom(
                        userStakedToken,
    ...
    function _amountInCalc(
        ...
        if(purchaseThisRound + _tokenReceived > alloc) amountInFinal = uint128(SaleLibrary.calcAmountIn(alloc - purchaseThisRound, _boosterPrice));
    ...
    function buyToken(
        ...
        (uint128 amountInFinal, uint128 tokenReceivedFinal) = _amountInCalc(uint128(SaleLibrary.calcTokenReceived(_amountIn, boosterPrice)), _buyer, running, boosterPrice);
        
        uint128 feeCharged;
        if(whitelist[_buyer] == 0) feeCharged = uint128(SaleLibrary.calcPercent2Decimal(amountInFinal, booster[running].fee_d2));
