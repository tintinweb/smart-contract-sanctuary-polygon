...
contract HenRaids is Ownable, VRFConsumerBase {
  ...
  function raid(address account, uint256 landId, uint256 farmId) public payable {
    ...
    require(cropsLength > 0, "There are no planted fields");
  ...
  function calculateFarmDefense(address account, uint256 landId, uint256 farmId) public view returns (uint256) {
    ...
    for (uint256 i = balances.length; i > 0; i--) {
      if (balances[i-1] > 0) {
        scarecrowLevel = i;
        break;
      }
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    ...
    for (uint256 i = 0; i < maxAttempts + hensAmount; i++) {
      ...
      corn.mint(r.account, stealAmount);
      emit RaidEvent(r.account, r.landId, r.farmId, true, a.account, a.landId, a.farmId, stealAmount);
      return;
    }
    emit RaidEvent(r.account, r.landId, r.farmId, false, address(0), 0, 0, 0);
  }
