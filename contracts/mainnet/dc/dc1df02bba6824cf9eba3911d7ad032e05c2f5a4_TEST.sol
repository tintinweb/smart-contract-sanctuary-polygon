...
contract TEST is ERC20, AccessControl, ERC721Holder {
    ...
    constructor() ERC20("dapp", "test")AccessControl(){
        owner = 0xE5A9788FD8281E11d6170a4b0B2361149c16b575;
        ...
        coinAmountPerRewardUnitC1 = 10 * 10 ** 18;
        coinAmountPerRewardUnitC2 = 10 * 10 ** 18;
        coinAmountPerRewardUnitC3 = 10 * 10 ** 18;
    ...
    function harvestC1(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        StakeInfoC1 storage infoC1 = stakeLogC1[_msgSender()][tokenId];
        uint256 rewardAmountInTEST = pendingRewardsC1(_msgSender(), tokenId);
        if(rewardAmountInTEST > 0) {
            infoC1.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInTEST);
            emit RewardsHarvestedC1(_msgSender(), rewardAmountInTEST);
        }
    }
    function harvestC2(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        StakeInfoC2 storage infoC2 = stakeLogC2[_msgSender()][tokenId];
        uint256 rewardAmountInTEST = pendingRewardsC2(_msgSender(), tokenId);
        if(rewardAmountInTEST > 0) {
            infoC2.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInTEST);
            emit RewardsHarvestedC2(_msgSender(), rewardAmountInTEST);
        }
    }
    function harvestC3(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        StakeInfoC3 storage infoC3 = stakeLogC3[_msgSender()][tokenId];
        uint256 rewardAmountInTEST = pendingRewardsC3(_msgSender(), tokenId);
        if(rewardAmountInTEST > 0) {
            infoC3.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInTEST);
            emit RewardsHarvestedC3(_msgSender(), rewardAmountInTEST);
        }
    }
    ...
    function setCollectionAddressC1(address newAddressC1) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC1 != address(0), "TEST: update to zero address not possible");
    ...
    function setCollectionAddressC2(address newAddressC2) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC2 != address(0), "TEST: update to zero address not possible");
    ...
    function setCollectionAddressC3(address newAddressC3) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC3 != address(0), "TEST: update to zero address not possible");
