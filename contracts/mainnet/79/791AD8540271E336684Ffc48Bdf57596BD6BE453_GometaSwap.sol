    ...
    function swapTokens(
        ...
        require(
            checkSellTokenAddress(sellToken_) &&
                stableCoinAddresses[0] != sellToken_,
        ...
        uint256 amountTokensToBuy = amountTokensToSwap_ / gometaPrice;
        require(
            buyToken.balanceOf(tokenStorage) >= amountTokensToBuy * 1e18,
        ...
        buyToken.transferFrom(
            tokenStorage,
            msg.sender,
            amountTokensToBuy * 1e18
        );
        return true;
    }

    /**
     * @dev 'swapTokensUSDT' using only for swap 'USDT' on 'GOMETA WAYS'
     *
     * @param usdtAddress_ - this function accept only usdt address with 6 decimals format
     * @param buyToken_ - address of 'GOMETA WAYS'
     * @param amountTokensToSwap_ - how many 'sellToken_' user want to exchange
     *
     * NOTE: transfer will be success only if user approved tokens for managed by 'this' contract
     */
    function swapTokensUSDT(
        address usdtAddress_,
        address buyToken_,
        uint256 amountTokensToSwap_
    ) external nonReentrant returns (bool swapStatus_) {
        require(
            usdtAddress_ != address(0) && buyToken_ != address(0),
            "GometaSwap: contract address can't be 0"
        );
        require(
            amountTokensToSwap_ != 0,
        ...
        require(
            stableCoinAddresses[0] == usdtAddress_,
            "GometaSwap: 'usdtAddress_' , not matched !"
        );
        IERC20 sellToken = IERC20(usdtAddress_);
        IERC20 buyToken = IERC20(buyToken_);
        uint256 amountOfApprovedSellToken = sellToken.allowance(
            msg.sender,
            address(this)
        ...
        uint256 amountTokensToBuy = amountTokensToSwap_ / gometaPrice;
        require(
            buyToken.balanceOf(tokenStorage) >= amountTokensToBuy * 1e12,
            "GometaSwap : not enougth tokens to transfer"
        );
        sellToken.transferFrom(msg.sender, tokenStorage, amountTokensToSwap_);
        buyToken.transferFrom(
