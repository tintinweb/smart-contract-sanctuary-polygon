    ...
    LazyMint
{
    ...
    function mintTo(
        ...
        uint256 tokenIdToMint = 0; // Set the tokenIdToMint to 0 for all tokens

        // Only set tokenURI and nextTokenIdToMint_ when there are no tokens minted yet
        if (nextTokenIdToMint_ == 0) {
            _setTokenURI(tokenIdToMint, _tokenURI);
            nextTokenIdToMint_ = 1; // Set nextTokenIdToMint_ to 1 after first minting
        }
    ...
    function batchMintTo(
        ...
        for (uint256 i = 0; i < _amounts.length; i++) {
            _tokenIds[i] = tokenIdToMint;
        }

        // Only set baseURI and nextTokenIdToMint_ when there are no tokens minted yet
        if (nextTokenIdToMint_ == 0) {
            _batchMintMetadata(tokenIdToMint, _amounts.length, _baseURI);
            nextTokenIdToMint_ = 1; // Set nextTokenIdToMint_ to 1 after first minting
        }
