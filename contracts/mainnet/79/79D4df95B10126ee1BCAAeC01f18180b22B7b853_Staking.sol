...
contract Staking is ERC1155Holder {
    ...
    function stake(uint256 _tokenId,uint256 _amount) public {
        stakes[msg.sender][_tokenId] = Stake(_tokenId, _amount, block.timestamp); 
        nft.safeTransferFrom(msg.sender, address(this), _tokenId, _amount, "0x00");
        emit TokenStaked(msg.sender, _tokenId,_amount);
    }
    function unstake(uint256 _tokenId,uint256 _amount) public {
        nft.safeTransferFrom(address(this), msg.sender, _tokenId, _amount, "0x00");
        Stake memory token=stakes[msg.sender][_tokenId];
        if(token.amount - _amount <1){
            delete stakes[msg.sender][_tokenId];
        }else{
            token.amount-=_amount;
        }
        emit TokenUnstaked(msg.sender, _tokenId, _amount);
    }
}