...
interface RulesInterface{
    function tokenCheck (uint id, uint valToCheck, address _contract) external view returns (bool);
...
contract ElegantSharkWeek is Ownable, Concatenate {
    ...
    function signUp (uint[] calldata _racers, uint _event, address _contract) public payable{
        ...
        for(uint i=0; i<_count; i++){
            ...
            if (rulesByRace[_event].conCheck != address(0)){
                bool answer = RulesInterface(rulesByRace[_event].conCheck).tokenCheck(_racers[i],rulesByRace[_event].conValue,_contract);
