...
contract IPaySwap is Initializable,AccessControlEnumerableUpgradeable, UUPSUpgradeable, OwnableUpgradeable{
    ...
    function initialize(IERC20 USDT_ADDRESS,IERC20 MATIC_ADDRESS, address QUICKSWAP_ROUTER_ADDRESS) public initializer {
        __Ownable_init();
        __UUPSUpgradeable_init();
        __AccessControlEnumerable_init();
        IPaySwap_init_unchained(USDT_ADDRESS,MATIC_ADDRESS,QUICKSWAP_ROUTER_ADDRESS);
    }
    function IPaySwap_init_unchained(IERC20 USDT_ADDRESS,IERC20 MATIC_ADDRESS, address QUICKSWAP_ROUTER_ADDRESS) internal onlyInitializing() {
        usdtToken = USDT_ADDRESS;
        maticToken = MATIC_ADDRESS;
        router = IQuickSwapRouter(QUICKSWAP_ROUTER_ADDRESS);
    ...
    function withdraw(address token, uint256 amount) external onlyRole(DEFAULT_FINANCIAL_ROLE) {
        IERC20(token).safeTransfer(_msgSender(), amount);
    }
    function withdrawNative(uint256 amount) external onlyRole(DEFAULT_FINANCIAL_ROLE) {
        payable(_msgSender()).transfer(amount);
    }

    function buyMATICWithUSDT(address recipient,uint256 usdtAmount, uint256 minMaticAmount) external onlyRole(DEFAULT_OP_ROLE) {
        ...
        require(usdtToken.approve(address(router), usdtAmount), "USDT approve failed");
        router.swapExactTokensForTokens(usdtAmount, minMaticAmount, path, recipient, block.timestamp + 10 minutes);

        // Transfer any remaining USDT back to the user
        uint256 buyMatic = maticToken.balanceOf(recipient);
        if (buyMatic < minMaticAmount) {
            revert("buyMatic < minMaticAmount");
        }
    }

    function canExchangeAmount(uint256 usdtAmount) external view returns(uint256) {
