...
import "./interfaces/IPalletLockerFactory.sol";
...
import "./EconomicsMirror.sol";

import { IChamberOfCommerceDataTypes, PalletRegistryDataTypes } from "./interfaces/IDataTypes.sol";
import { IChamberOfCommerceEvents } from "./interfaces/IEvents.sol";

/**
 * @title The ChamberOfCommerce is the contract that governs and controls the access actorrs have to the EFM contracts. 
 * @author https://github.com/kasper-keunen
 */
contract ChamberOfCommerce is 
        IChamberOfCommerce,
        AccessControlEnumerable,
        IChamberOfCommerceDataTypes,
        IChamberOfCommerceEvents,
        Pausable
    {
    ...
    // EFM Contracts
    address public palletMinter;
    address public bondCouncil;
    address public tellerKeeper;
    address public clearingHouse;
    address public ticketSaleOracle; 
    address public palletLockerFactory;
    address public palletRegistry; 

    // External contracts
    address public tellerContract; 
    address public economics;  // Economics.sol contract of the GET core
    address public fuelToken; // GET
    address public depositToken;  // xGET or GET 1e18
    address public stableToken; // USDC contract address 1e6
    
    // Backup address / emergency address
    address public emergencyMultisig;
    // multisigAddress => bool
    mapping(address => bool) public daoControllers;
    // safeAddress (Gnosis multi-sig) -> ActorAccount Struct
    mapping(address => ActorAccount) public accounts;
    // integratorIndex => safeAddress
    mapping(uint32 => address) public integratorToSafe;
    // safeAddress => CreditScoreStruct
    mapping(address => CreditScore) public creditScores;
    uint256 public defaultDeposit = 10 * 1e18;
    bool private isConfigured;

    constructor(
        address _firstClerk,
        address _firstDirector,
        address _fuelToken,
        address _tellerContract,
        address _depositToken,
        address _daoController,
        address _emergencyMultisig,
        address _stableToken
    ) {
        _setupRole(CLERK_ROLE, _firstClerk);
        _setupRole(DIRECTOR_ROLE, _firstDirector);
        _grantRole(DEFAULT_ADMIN_ROLE, _firstDirector);
        _setRoleAdmin(CLERK_ROLE, DIRECTOR_ROLE);
        _setRoleAdmin(DIRECTOR_ROLE, DIRECTOR_ROLE);
        _revokeRole(DEFAULT_ADMIN_ROLE, _msgSender());
        depositToken = _depositToken;
        tellerContract = _tellerContract;
        fuelToken = _fuelToken;
        stableToken = _stableToken;
        daoControllers[_daoController] = true;
        emergencyMultisig = _emergencyMultisig;
    }

    modifier onlyDAOController() {
        require(daoControllers[msg.sender],
        "ChamberOfCommerce:Caller not DAO");
        _;
    }

    function configureContract(
        address _palletLockerFactory,
        address _bondCouncilAddress,
        address _ticketSalesOracle,
        address _economics,
        address _palletRegistry,
        address _palletMinter,
        address _clearingHouse,
        address _tellerKeeper
    ) external onlyRole(DIRECTOR_ROLE) {
        require(
            !isConfigured,
            "ChamberOfCommerce:Contract can only be configured once"
        );
        palletLockerFactory = _palletLockerFactory;
        bondCouncil = _bondCouncilAddress;
        ticketSaleOracle = _ticketSalesOracle;
        economics = _economics;
        palletRegistry = _palletRegistry;
        palletMinter = _palletMinter;
        clearingHouse = _clearingHouse;
        tellerKeeper = _tellerKeeper;
        isConfigured = true;
        emit ContractsConfigured(
            _palletLockerFactory,
            _bondCouncilAddress,
            _ticketSalesOracle,
            _economics,
            _palletRegistry,
    ...
    /** Actor registration by safeAddress (a gnosis safe on Polygon)
     * @param _nickName the name of the ticketeer / string reference
     * @param _uriGeneral URL pointing to the main website of the borrower/ticketeer
     * @param _uriTerms URL pointing to the borrower terms / legal registration
     * @param _relayerAddress the relayerAddress this borrower(ticketeer) is using to ticket their events
     * @param _type if the account is a borrower or a lender (in V2 version only BORRROWER actor availabe)
     * @dev any address can register an account! This is since we want to make the EFM as non-custodial as possible
     */ 
    function registerActorAccount(
        string memory _nickName,
        string memory _uriGeneral,
        string memory _uriTerms,
        address _relayerAddress,
        uint32 _integratorIndex, // of the entity in the Economics.sol contract
        AccountType _type
    ) external {
        require(
            isConfigured,
            "ChamberOfCommerce:COC needs to be configured first"
        );
        // check if the caller already has a actorAccount (not allowed)
        require(
            accounts[msg.sender].status == AccountStatus.NONE, 
            "ChamberOfCommerce:ActorAccount is already setup"
        );
        // check if the [assed integratorIndex already has an actorAccount
        require(
            integratorToSafe[_integratorIndex] == address(0x0),
            "ChamberOfCommerce:Only one actorAccount per integrator"
        );
        ActorAccount storage _account = accounts[msg.sender];
        _account.integratorIndex = _integratorIndex;
        _account.nickName = _nickName;
        _account.uriGeneral = _uriGeneral;
        _account.uriTerms = _uriTerms;
        _account.accountType = _type;
        _account.relayerAddress = _relayerAddress;
        _account.status = AccountStatus.REGISTERED;
        _account.palletLocker = _deployPalletLocker(msg.sender); // Deploy PalletLocker 
        integratorToSafe[_integratorIndex] = msg.sender;
        creditScores[msg.sender] = CreditScore({
            minimumDeposit: defaultDeposit, // amount of GET that 
            fuelRequirement: 1e6 // default is that integrator should have 100% of the required fuel to ticket the whole event
        });
    ...
    function whitelistAccount(
        ...
        require(
            accounts[_safeAddress].status == AccountStatus.REGISTERED, 
            "ChamberOfCommerce:ActorAccount must be in REGISTERED state"
        );
        accounts[_safeAddress].status = AccountStatus.WHITELISTED;
        emit AccountWhitelisted(_safeAddress);
    }

    function blacklistAccount(
        address _safeAddress
    ) external onlyRole(DIRECTOR_ROLE) {
        accounts[_safeAddress].status = AccountStatus.BLACKLIST;
        emit AccountBlacklisted(_safeAddress);
    }

    function deleteAccount(
        address  _safeAddress 
    ) external onlyRole(DIRECTOR_ROLE) {
        uint32 integratorIndex_ = accounts[msg.sender].integratorIndex;
        delete accounts[_safeAddress];
        delete integratorToSafe[integratorIndex_];
        emit AccountDeleted(_safeAddress);
    }

    function setControllerDAO(
        address _newDAOController,
        bool _setting
    ) external onlyDAOController {
        daoControllers[_newDAOController] = _setting;
        emit ControllerSet(
            _newDAOController,
            _setting
        );
    ...
    function pauseChamber() external onlyRole(CLERK_ROLE) {
        _pause();
        emit ChamberPaused();
    }

    function unpauseChamber() external onlyRole(DIRECTOR_ROLE) {
        _unpause();
        emit ChamberUnPaused();
    }

    function viewIntegratorUSDBalance(uint32 _integratorIndex) external view returns (uint256 balance_) {
        balance_ = IEconomics(economics).viewIntegratorUSDBalance(_integratorIndex);
    }

    /**
     * @dev function that checks if the bond issuer has sufficient USD value on their fuelbalance to ticket all the tickets of a event that they want to collateralize. Since without fuel the EFM contracts cannot operate, the requirement of having fuel in the Economics.sol contract needs to checked.
     * @param _palletIssuerAddress the address of the borrower that has issued the pallet (the potential bond issuer)
     * @param _maxAmountInventory the maximum amount of tickets that the event can sell (as reported by the issuer)
     * @param  _averagePriceInventory estimation of average price per ticket in USD (scaled 1e3, as reported by the issuer)
     * @param _amountDeposit amount of GET or xGET that the specific borrower/issuer has deposited in the pallet
     * @return judgement_ bool indicating if the pallet/issuer passes the CreditScore check. If true is returned this means that the issuer has enough GET to ticket the whole event and the depositAmont is adquate according to the bondIssuers CreditScore
     * @dev both _maxAmountInventory and _averagePriceInventory are self-reported, when the DAO governers judges the pallet/bond these need to be checked for validity
     */
    function isFuelAndCollateralSufficient(
        ...
        uint256 _amountDeposit) external view returns(bool judgement_) {
            // cache variable to save an SLOAD
            uint32 integratorIndex_ = accounts[_palletIssuerAddress].integratorIndex; 
            // check if the borrower has enough fuel to ticket the whole event
            bool fuelcheck_ = _checkFuelBalance(
                _palletIssuerAddress,
                integratorIndex_, 
                _maxAmountInventory, 
                _averagePriceInventory
            );
            // check if the borrower has deposited enough GET (according to their CreditScore)
            bool depositcheck_ = _palletCollateralCheck(
                _palletIssuerAddress, 
                _amountDeposit
            );
            // if both checks are true; return true, if either is false; return false
            judgement_ = (fuelcheck_ && depositcheck_);
    }

    /**
     * Internal function that checks if the USD value of the integrator is sufficient to create collateral
     * This function calculates how much fuel (in USD) will be needed to fuel the entire event. If not enough fuel is available, the collateral will be deemed insufficient.
     * @param _palletIssuerAddress the pallet index of the pallet that 
     * @param _integratorIndex the index of the backer in Economics.sol
     * @param _maxAmountInventory the self reported maximum amount of tickets a certain event will be able to sell
     * @param _averagePriceInventory the average 
     * @return fuelcheck_ bool that signals weither or not the bondissuer has a sufficiently high fuel balance
     */
    function _checkFuelBalance(
        address _palletIssuerAddress,
        uint32 _integratorIndex, 
        uint64 _maxAmountInventory,
        uint64 _averagePriceInventory
        ) internal view returns(bool fuelcheck_) {
            uint256 usdBalance_ = IEconomics(economics).viewIntegratorUSDBalance(_integratorIndex);
            uint24 primaryRate_ = IEconomics(economics).integratorRates(_integratorIndex).primaryRate;
            uint64 maxRevenue_ = _maxAmountInventory * _averagePriceInventory;
            if (maxRevenue_ == 0) return true; // NOTE consider this doesn't correctly handle free ticket cases (in an edge case)
            uint256 maxSpend_ = maxRevenue_ * primaryRate_ * creditScores[_palletIssuerAddress].fuelRequirement / 1e12; // needs to be scaled by 1e12, because fuelRequirment is scaled 1e6 and primaryRate also with 1e6 
            fuelcheck_ = usdBalance_ >= maxSpend_;
    }

    function _palletCollateralCheck(
        address _palletIssuerAddress, 
        uint256 _amountDeposit) internal view returns(bool depositCheck_) {
            depositCheck_ = _amountDeposit <= creditScores[_palletIssuerAddress].minimumDeposit;
    }

    function palletIndexToBid(
        uint256 _palletIndex
    ) external view returns(uint256 _bidId) {
        _bidId = IClearingHouse(clearingHouse).returnPalletIndexToBid(_palletIndex);
    }

    /**
     * @param _integratorIndex the integratorindex of the backer in the Economics.sol(V2) contract 
     * @return _price the price of GET of the balance/silo of the integrator, with 1e18 to mean $1
    */ 
    function getIntegratorFuelPrice(
        uint32 _integratorIndex
    ) external view returns(uint256 _price) {
        _price = IEconomics(economics).returnIntegratorData(_integratorIndex).price;
    }

    function returnIntegratorData(
        uint32 _integratorIndex
    )  external view returns(IntegratorData memory data_) {
        // data_ = IEconomics(economics).integratorData(_integratorIndex);
        data_ = IEconomics(economics).returnIntegratorData(_integratorIndex);
    }

    function returnIntegratorIndexByRelayer(
        address _relayerAddress
    ) external view returns(uint32 integratorIndex_) {
        integratorIndex_ = IEconomics(economics).returnIntegratorIndexByRelayer(_relayerAddress);
    }

    /**
     * @param _eventAddress the EventImplementation contract of the event
     * @return _ticketCount amount of tickets/nfts sold according to the oracle
     * @dev THIS IS A EXTERNAL CALL
    */ 
    function nftsIssuedForEvent(
        address _eventAddress
    ) external view returns(uint32 _ticketCount) {
        _ticketCount = ITicketSaleOracle(ticketSaleOracle).nftsIssuedForEvent(_eventAddress);
    }

    /**
     * @param _eventAddress the EventImplementation contract of the event
     * @return _isFinalized if the event oracle is finalized (so if the ticket sale is completed, according to the oracle)
     * @dev THIS IS A EXTERNAL CALL
    */ 
    function isCountFinalized(
        address _eventAddress
    ) external view returns(bool _isFinalized) {
        _isFinalized = ITicketSaleOracle(ticketSaleOracle).isCountFinalized(_eventAddress);
    }

    function isAccountWhitelisted(
        address _addressAccount
    ) external view returns(bool isWhitelisted_) {
        isWhitelisted_ = accounts[_addressAccount].status == AccountStatus.WHITELISTED;
    }

    function isAccountBlacklisted(
        address _addressAccount
    ) external view returns(bool isBlacklisted_) {
        isBlacklisted_ = accounts[_addressAccount].status == AccountStatus.BLACKLIST;
    }

    function isAddressBorrower(
        address _addressSafeBorrower
    ) external view returns(bool isBorrower_) {
        isBorrower_ = (
            accounts[_addressSafeBorrower].accountType == AccountType.BORROWER &&
            accounts[_addressSafeBorrower].status == AccountStatus.WHITELISTED
        );
    }

    // SECURITY NOTE: If an attacker is able to change this address, it can drain all the lockers!
    function setRegistryOfMinter(
        address _palletRegistry
    ) external onlyRole(DIRECTOR_ROLE) {
        palletRegistry = _palletRegistry;
        IPalletMinter(palletMinter).setRegistry(_palletRegistry);
        emit RegisterySet(_palletRegistry);
    }

    /**
     * Internal function that deploys a stakingLocker contract
     * A palletLocker is a contract that holds 'bond specific collateral' of a actors pallet. 
     * @param _safeAddressAccount the gnosis safe address of the actor
     * @return palletLocker_ address of the deployed pallet locker
     */
    function _deployPalletLocker(
        address _safeAddressAccount) 
        internal 
        returns(address palletLocker_) {
        palletLocker_ = IPalletLockerFactory(
            palletLockerFactory).newPalletLocker(
                _safeAddressAccount
            );
        emit PalletLockerDeployed(
            _safeAddressAccount,
            palletLocker_
        );
    }

    function _checkAddressNotZero(address _addressToCheck) internal pure {
        require(
            _addressToCheck != address(0x0),
            "ChamberOfCommerce:Address is zero"
        );
    }

    function setDefaulteDeposit(uint256 _newDefaultDeposit) external onlyDAOController {
        defaultDeposit = _newDefaultDeposit;
        emit DefaultDepositSet(
            _newDefaultDeposit
        );
    }
    ...
    function editCreditScore(
        ...
        uint24 _fuelRequirement) external onlyDAOController {
            require(
                accounts[_safeAddress].status == AccountStatus.WHITELISTED,
                "ChamberOfCommerce:Can only edit CC of whitelisted Actors"
            );
            creditScores[_safeAddress].minimumDeposit = _minimumDeposit;
            creditScores[_safeAddress].fuelRequirement = _fuelRequirement;
            emit CreditScoreEdit(
                _safeAddress,
                _minimumDeposit,
                _fuelRequirement
            );
    ...
    function changeDepositToken(address _newDepositToken) external onlyDAOController {
        depositToken = _newDepositToken;
        emit DepositTokenChange(_newDepositToken);
    }

    function changeEconomicsContract(address _newEconomicsContact) external onlyDAOController {
        economics = _newEconomicsContact;
        emit EconomicsContractChange(_newEconomicsContact);
    }

    function returnIntegratorIndex(address _addressAccount) external view returns(uint32 index_) {
        return accounts[_addressAccount].integratorIndex;
    }
    ...
    function returnPalletLocker(address _safeAddress) external view returns(address _palletLocker) {
        _palletLocker = accounts[_safeAddress].palletLocker;
        _checkAddressNotZero(_palletLocker); // ensure that the returned address is not 0x0
    }

    function returnPalletEvent(
        uint256 _palletIndex
    ) external view returns(address eventAddress_) {
        eventAddress_ = IPalletRegistry(palletRegistry).returnPalletEvent(_palletIndex);
        _checkAddressNotZero(eventAddress_); // ensure that the returned address is not 0x0
    }
    
    function isDAOController(
        address _challengedController
    ) external view returns(bool isController_) {
        isController_ = daoControllers[_challengedController];
    }
...
import { IEconomicsDataTypes } from "./IDataTypes.sol";

// Slimmed down and enriched version of IEconomics Interface for the Event Financing Usecase
interface IEconomics is IEconomicsDataTypes {
    
    function viewIntegratorUSDBalance(uint32 _integratorIndex) external view returns (uint256);

    // KK: Added this one so i can read integratorData without importing the whole contract ABI
    function integratorData(uint32 _integratorIndex) external view returns(IntegratorData memory data_);

    function integratorRates(uint32 _integratorIndex) external view returns(DynamicRates memory rates_);

    function relayerData(address _relayerAddress) external view returns(RelayerData memory relayerData_);
...
interface IPalletLockerFactory {

    event PalletLockerCreated(
        address safeAddressAccount, 
        address collateralLocker
    );

    function isPalletLocker(address) external view returns (bool);
    
    function newPalletLocker(address _safeAddressAccount) external returns (address);

    function accountToPalletLocker(address _safeAddressAccount) external returns(address);

    function chamberOfCommerce() external returns(address registry_);
}
