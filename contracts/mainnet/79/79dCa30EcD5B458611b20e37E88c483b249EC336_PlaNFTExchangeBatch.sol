...
contract ExchangeCoreBatch is ReentrancyGuarded, Ownable {
    ...
    function approveOrder(Order memory order, uint256[] memory tokens) internal {
        ...
        if (order.orderType == OrderType.RandomOrder) {
            require(tokens.length > 0, "PlaExchange: tokens size must bigger than zero");
            matchIndex[hash] = 0;
        } else if (order.orderType == OrderType.ScopeOrder) {
            require(tokens.length == 2, "PlaExchange: tokens scope illegal");
            require(tokens[1] > tokens[0], "PlaExchange: tokens scope must asc");
            matchIndex[hash] = 0;
        } else {
            require(tokens.length == 0, "PlaExchange: tokens size must be zero");
        }
    ...
    function atomicMatch2(
        ...
        require(sell.maker == msg.sender, "PlaExchange: caller is not buy maker");
    ...
    function atomicMatch(
        ...
        /* Ensure buy order validity and calculate hash if necessary. */
        require(buy.maker == msg.sender, "PlaExchange: caller is not buy maker");
        ...
        if (sell.orderType == OrderType.RandomOrder) {
            require(matchIndex[sellHash] < tokens.length, "PlaExchange: illegal match index");
            tokenId = tokens[matchIndex[sellHash]];
        } else {
            require(matchIndex[sellHash] <= tokens[1] - tokens[0], "PlaExchange: illegal match index");
