...
library GameHelperLibrary {
    ...
    function _verifyInputOwnership(
        ...
        if (input.tokenType == GameRegistryLibrary.TokenType.ERC20) {
            ...
            require(
                IGameNFT(input.tokenContract).ownerOf(input.tokenId) == account,
                "NOT_OWNER"
            );
...
import {MINTER_ROLE, CURRENT_HEALTH_TRAIT_ID, SPEED_TRAIT_ID, HEALTH_TRAIT_ID, DAMAGE_TRAIT_ID, EVASION_TRAIT_ID, ACCURACY_TRAIT_ID} from "../Constants.sol";
...
import {IShipSystem, ID} from "../interfaces/IShipSystem.sol";
import {ITokenTemplateSystem, ID as TOKEN_TEMPLATE_SYSTEM_ID} from "../tokens/ITokenTemplateSystem.sol";

import "../GameRegistryConsumerUpgradeable.sol";

contract ShipSystem is GameRegistryConsumerUpgradeable, ILootCallback {
    // For ship token id counter
    ...
    Counters.Counter public currentShipId;
    ...
    function _grantLoot(
        ...
        for (uint8 idx = 0; idx < amount; idx++) {
            ...
            currentShipId.increment();

            uint256 shipTokenId = currentShipId.current();
