...
contract MaiVaultFront is BaseSiloAction {
    ...
    function enter(
        ...
        if (inputAmounts[0] > 0) {
            collateral.approve(maiVault, inputAmounts[0]);
        ...
        if (debt > 0 && inputAmounts[4] > 0) {
            if (inputAmounts[4] > debt) {
                //just repay the debt
                miMatic.approve(maiVault, debt);
                vault.payBackToken(vaultId, debt);
                // outputAmounts[0] += inputAmounts[4] - vaultDebt;
            } else {
                miMatic.approve(maiVault, inputAmounts[4]);
                vault.payBackToken(vaultId, inputAmounts[4]);
            }
        }
        if (inputAmounts[4] == 0) {
            ...
            uint256 collateralValue = ((vault.vaultCollateral(vaultId)) *
                oracle.latestAnswer()) / (10**oracle.decimals()); //in terms of collateral decimals
                ...
                if (inputAmounts[0] > 0) {
                    ...
                    if (idealDebt > debt) {
                        ...
                        if (amountToBorrow <= debtCeil) {
                            ...
                            vault.borrowToken(vaultId, debtCeil);
        ...
        if (vault.balanceOf(address(this)) > 0) {
            ...
            if (inputAmounts[4] > 0) {
                ...
                if (vaultDebt > 0) {
                    if (inputAmounts[4] > vaultDebt) {
                        ...
                        vault.payBackToken(vaultId, vaultDebt);
                    } else {
                        miMatic.approve(maiVault, inputAmounts[4]);
                        vault.payBackToken(vaultId, inputAmounts[4]);
                    }
            ...
            if (
                ...
                uint256 collateralValue = ((vault.vaultCollateral(vaultId) -
                    ...
                    (10**oracle.decimals()); //in terms of collateral decimals
                if (collateralValue > 0) {
                    collateralValue =
                        ((10**miMatic.decimals()) * collateralValue) /
                        (10**collateral.decimals()); //convert to miMatic decimals
    ...
    function vaultInfo(address _silo, bytes memory _configurationData)
        public
        view
        override
        returns (
