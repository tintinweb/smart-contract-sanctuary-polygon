...
contract TokenCondensor is BaseSiloAction {
    ...
    function directTokenCondense(
        bytes memory configuration,
        bytes memory inputData
    ) internal returns (uint256[4] memory outputAmounts,uint256[4] memory afterInputs,bool original) {
        address exchangeRateOracle;
        address[] memory path = new address[](2);
        address[4] memory inputs;
        address[4] memory outputs;
        ...
        for (uint256 i; i < 4; ) {
            ...
            for (uint256 j; j < 4; ) {
                ...
                uint256 amountOut =  oracle.getUniPrice(QUICK_ROUTER,path, inputAmounts[i]);
