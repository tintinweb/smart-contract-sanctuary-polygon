...
contract VaultCore is VaultStorage, MiniCurveExchange {
    ...
    function _redeem(uint256 _amount, uint256 _minimumUnitAmount) internal {
        ...
        // Send output
        address[] memory _strategiesToWithdrawFrom = new address[](strategyWithWeights.length);
        uint256 _amountToPull = (output + redeemFee).subOrZero(_ps.balanceOf(address(this)));
        if (_amountToPull > 0) {
            for(uint8 i = 0; i < _strategiesToWithdrawFrom.length; i++) {
        ...
        for(uint8 i = 0; i < strategyWithWeights.length; i++) {
            ...
            for(uint8 j = 0; j < allAssets.length; j++) { 
                if(allAssets[j] == _thisStrategy.token0() && assets[allAssets[j]].isSupported) {
                    ...
                    _distAmounts[j] += _share;
                    break;
                }
            }
        }

        // for(uint8 i = 0; i < _distAmounts.length; i++) {
        //     if (_distAmounts[i] > 0 && assets[allAssets[i]].isSupported && allAssets[i] != _asset) {
        //         _distAmounts[i] = _swapAsset(_asset, allAssets[i], _distAmounts[i]);
        //     }
        // }
        console.log("A0", IERC20(allAssets[0]).balanceOf(address(this)), allAssets[0]);
        console.log("A1", IERC20(allAssets[1]).balanceOf(address(this)), allAssets[1]);
        console.log("A2", IERC20(allAssets[2]).balanceOf(address(this)), allAssets[2]);

        for(uint8 i = 0; i < strategyWithWeights.length; i++) {
            IStrategy _thisStrategy = IStrategy(strategyWithWeights[i].strategy);
            console.log("Transferring to ", address(_thisStrategy));
            console.log(_thisStrategy.token0(), _stratsAmounts[i]);
    ...
    ****************************************/
    ...
    // Internal functions
    function _handleDirectDeposit(
        address _asset,
        uint256 _amount,
        uint256 _directDepositAmount,
        address _quickDepositStrategy
    ...
    function _handleDirectDepositWithUnalignedToken(
        address _asset,
        uint256 _amount,
        uint256 _directDepositAmount,
        address _quickDepositStrategy
        ...
        uint256 _assetsToSwap = howMuchToSwap(
            swappingPool,
            _asset,
            IStrategy(_quickDepositStrategy).token0(),
            _directDepositAmount.scaleBy(Helpers.getDecimals(IStrategy(_quickDepositStrategy).token0()), Helpers.getDecimals(primaryStableAddress))
        );

        console.log("ASSETS_TO_SWAP", _assetsToSwap);
        uint256 _token0GeneratedAmount = swapTillSatisfied(
            swappingPool,
            _asset,
            IStrategy(_quickDepositStrategy).token0(),
            _assetsToSwap,
            _directDepositAmount,
            _amount,
            1
        );
    ...
    function _handleTrivialDeposit(
        address _asset,
        uint256 _amount,
        uint256 _psDecimals
    ) internal returns (uint256) {
