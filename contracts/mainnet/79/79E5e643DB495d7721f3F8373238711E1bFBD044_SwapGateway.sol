...
contract SwapGateway is ISwapGateway, UpgradeableBase {
    ...
    function swapExactTokensForTokens(
        ...
        if (path[0] == ZERO_ADDRESS) {
            ...
            amounts = IPancakeRouter01(swap).swapExactETHForTokens{
                value: amountIn
        ...
        _approveTokenForSwapRouter(path[0], swap, amountIn);

        // swapExactTokensForETH
        if (path[path.length - 1] == ZERO_ADDRESS) {
    ...
    function swapTokensForExactTokens(
        ...
        if (path[0] == ZERO_ADDRESS) {
            ...
            amounts = IPancakeRouter01(swap).swapETHForExactTokens{
                value: amountInMax
        ...
        _approveTokenForSwapRouter(path[0], swap, amountInMax);

        // swapTokensForExactETH
        if (path[path.length - 1] == ZERO_ADDRESS) {
            path[path.length - 1] = wETH;

            amounts = IPancakeRouter01(swap).swapTokensForExactETH(
                amountOut,
                amountInMax,
                path,
                msg.sender,
            ...
            amounts = IPancakeRouter01(swap).swapTokensForExactTokens(
                amountOut,
                amountInMax,
                path,
                msg.sender,
    ...
    function uniswapV3ExactTokensForTokens(
        ...
        if (length == 2) {
            // Check pool and fee
            (, uint24 fee) = findUniswapV3Pool(swap, path[0], path[1]);
            require(fee > 0, "SG2");
            ...
            if (path[0] == wETH) {
                amounts[0] = ISwapRouter(swap).exactInputSingle{
                    value: amountIn
                }(params);
                ...
                amounts[0] = ISwapRouter(swap).exactInputSingle(params);
            }
        } else {
            // Multihop
            ...
            for (uint256 i = 0; i < length - 1; ) {
                // Change wETH address;
                if (path[i + 1] == ZERO_ADDRESS) path[i + 1] = wETH;

                // Get fee
                (, fees[i]) = findUniswapV3Pool(swap, path[i], path[i + 1]);
            ...
            if (path[0] == wETH) {
                amounts[0] = ISwapRouter(swap).exactInput{value: amountIn}(
                    params
                );
            } else {
                amounts[0] = ISwapRouter(swap).exactInput(params);
            }
        }
    ...
    function uniswapV3TokensForExactTokens(
        ...
        if (length == 2) {
            ...
            if (path[0] == wETH) {
                amounts[0] = ISwapRouter(swap).exactOutputSingle{
                    value: amountInMax
                }(params);
            } else {
                amounts[0] = ISwapRouter(swap).exactOutputSingle(params);
            }
        } else {
            // Multihop
            uint24[] memory fees = new uint24[](length - 1);
            ...
            for (uint256 i = 0; i < length - 1; ) {
                // Change wETH address;
                if (reversePath[i + 1] == ZERO_ADDRESS)
                    reversePath[i + 1] = wETH;

                // Get fee
                (, fees[i]) = findUniswapV3Pool(
                    swap,
                    reversePath[i],
                    reversePath[i + 1]
                );
            ...
            if (path[0] == wETH) {
                amounts[0] = ISwapRouter(swap).exactOutput{value: amountInMax}(
                    params
                );
            } else {
                amounts[0] = ISwapRouter(swap).exactOutput(params);
            }
        }
    ...
    /**
     * @notice Get information for Farming Rewards - Pancakeswap
     ...
     * @return rewardsAmount token amount of reward
     */
    function getFarmingRewardsPancakeSwap(
        ...
        if (isBalance) {
            rewardsAmount = IERC20Upgradeable(rewardsToken).balanceOf(holder);
        } else {
    ...
    /**
     * @notice Get information for Farming Rewards - ApeSwap
     * @param holder holder address
     * @param swapMaster swap masterchef address
     * @param isBalance true : get rewards balance, false : get pending rewards
     * @return rewardsToken rewards Token address
     ...
     * @param token address of token in
     * @param amountIn amount of tokenIn : deciaml = token.decimals
     * @param uniswapV3Pool pool of swap
     * @return amountOut amount of tokenOut : decimal = tokenOut.decimals;
     * @return tokenOut address of out token
     */
    function uniswapV3QuoteExactInput(
        address token,
        uint256 amountIn,
        address uniswapV3Pool
    ) external view override returns (uint256 amountOut, address tokenOut) {
        if (amountIn > 0) {
            Slot0 memory slot0 = IUniswapV3Pool(uniswapV3Pool).slot0();

            if (token == IUniswapV3Pool(uniswapV3Pool).token0()) {
                tokenOut = IUniswapV3Pool(uniswapV3Pool).token1();
                if (slot0.sqrtPriceX96 > 10**29) {
                    amountOut = ((slot0.sqrtPriceX96 * 10**9) / 2**96)**2;
                } else {
                    amountOut =
                        (uint256(slot0.sqrtPriceX96)**2 * BASE) /
                        (2**192);
                }
            } else {
                tokenOut = IUniswapV3Pool(uniswapV3Pool).token0();
                if (slot0.sqrtPriceX96 > 10**35) {
                    amountOut = ((2**96 * 10**9) / (slot0.sqrtPriceX96))**2;
                } else {
                    amountOut =
                        (2**192 * BASE) /
                        (uint256(slot0.sqrtPriceX96)**2);
                }
            }

            amountOut = (amountOut * amountIn) / BASE;
        }
    }
    ...
    function findUniswapV3Pool(
        ...
        address tokenA,
        address tokenB
    ) public view override returns (address pool, uint24 fee) {
    ...
    function _approveTokenForSwapRouter(
        ...
        if (allowance == 0) {
            IERC20Upgradeable(token).safeApprove(swap, amount);
            return;
        }
