...
interface IGogoRewards {
    function earned(address account) external view returns (uint256); //amount of GOGO earned
    function earnedStable(address user) external view returns (uint256); //amount of USDC earned
...
contract UsdcOrionStrategy is BaseStrategy, Pausable {

    uint256 private constant keepMax = 10000;

    //Tokens
    address private constant WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address private constant GOGO = 0xdD2AF2E723547088D3846841fbDcC6A8093313d6;
    ...
    //Orion bridge = 0x465862772F9c2E9D71982627BcFDC721A47DEb22

    // Uniswap swap paths
    address[] private reward_other_path;
    ...
    constructor()
        ...
        BaseStrategy(
            ...
            GOGO,
            QUICKSWAP_ROUTER,
            0x7FCf0f2dcEc385FCCEd98240A8A4bEC8e91da7D1 //GOGO USDC single staking
        )
    {
        multiHarvest = 0x3355743Db830Ed30FF4089DB8b18DEeb683F8546;
        reward_other_path = [GOGO, USDC];

        IERC20(GOGO).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
    ...
    function balanceOfPool() public override view returns (uint256) {
        //pause the contract if the last withdrawal from the vault is an async withdrawal
        //calling getPendingAmount, getStakedBalance and getContractBalance = error if user has never deposited before
        if(IGogoRewards(rewards).balanceOf(address(this)) == 0) {
            return 0;
        }
        return IGogoRewards(rewards).getStakedBalance(address(this)).add(
            Math.max(
                IGogoRewards(rewards).getPendingAmount(address(this)),
                IGogoRewards(rewards).getContractBalance(address(this))
            )
        );
    }

    function getHarvestable() external override view returns (uint256) {
        return IGogoRewards(rewards).earned(address(this));
    }
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant notPaused {
        ...
        //Distribute vault fee and convert GOGO to USDC
        ...
        if (amountHarvested > 0) {
            ...
            uint256 remaining = amountHarvested.sub(feeAmount);
            IERC20(harvestedToken).safeTransfer(strategist, feeAmount);

            _swapUniswapWithPathPreapproved(reward_other_path, remaining);
        }
    ...
    function deposit() public override notPaused {
        uint256 _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {
            IGogoRewards(rewards).stake(_want);
        }
    }
        ...
        returns (uint256)
    {
        ...
        uint256 result = IGogoRewards(rewards).withdraw(_amount);
        //withdrawals will fail since `IERC20(want).safeTransfer` will revert because no tokens were withdrawn
        require(result == 0, "insufficient funds in Orion bridge, please wait for tokens to be bridged");
        return _amount;
    }

    function _getReward() internal {
        IGogoRewards(rewards).getReward();
    }
    ...
    //Withdrawals happen sync or async based on if there is enough usdc balance in orion.
    //If balance is not enough, the withdrawal starts async and user come back later (usually in minutes) to finalize the withdrawal and get his usdc
    //This contract must be paused before initiating an async withdrawal because vault balance will be out of sync while waiting for the withdrawal
    function initiateWithdraw(uint256 _amount) external onlyOwner {
        IGogoRewards(rewards).withdraw(_amount);
    }

    //Safe to unpause after pending funds are withdrawn
    function withdrawPending() external onlyOwner {
        IGogoRewards(rewards).withdrawPending();
    }
}