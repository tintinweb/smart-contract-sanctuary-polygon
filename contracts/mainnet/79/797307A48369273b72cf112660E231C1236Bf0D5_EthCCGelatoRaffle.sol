...
import {_transfer} from "./functions/FUtils.sol";
...
contract EthCCGelatoRaffle is RaffleRules {
    ...
    ) RaffleRules(_ops, _gel, _gelReward, _roundInterval) {}
    ...
    function joinRaffle(address _player)
        ...
        notWinner(_player)
        notPreviousRoundWinner(_player)
        notJoined(_player)
    {
     ...
     */
    function confirmWinner(address payable _player)
        external
        onlyAdmins
        isPreviousRoundWinner(_player)
    {
        winners[_player] = true;
        _transfer(_player, gel, gelReward);
    }
    ...
    {
        uint256 round = roundNow;
        uint256 gelatoState = _getGelatoState(_player);

        if (winners[_player]) return (_GOLD_ID, uint256(GelatoState.FROZEN));

        if (_isPreviousRoundWinner(_player)) return (_GOLD_ID, gelatoState);
     ...
     */
    function _getGelatoState(address _player) private view returns (uint256) {
        uint256 round = roundNow;
        if (!isRaffleLive) return uint256(GelatoState.MELTED);
        if (
            !_roundInfo.playersContain(round, _player) &&
            !_isPreviousRoundWinner(_player)
        ) return (uint256(GelatoState.FROZEN));

        uint256 startTime = _roundInfo.getStartTimeOfRound(round);
...
contract RaffleRules is Proxied, RaffleStorage {
    ...
    modifier notWinner(address _player) {
        require(!winners[_player], "Raffle: Already won");
        _;
    }
    ...
    modifier isPreviousRoundWinner(address _player) {
        require(
            _isPreviousRoundWinner(_player),
            "Raffle: Not previous round winner (Too late)"
        );

        _;
    }

    modifier notPreviousRoundWinner(address _player) {
        require(
            !_isPreviousRoundWinner(_player),
            "Raffle: Previous round winner (Please claim prize)"
        );
        _;
    }
    ...
    constructor(
        ...
        uint256 _gelReward,
        uint256 _roundInterval
    ) RaffleStorage(_ops, _gel, _gelReward, _roundInterval) {}

    function _isPreviousRoundWinner(address _player)
        internal
        view
        returns (bool)
    {
        ...
        if (round > 1 && _player == _roundInfo.getWinnerOfRound(round - 1))
            return true;
        return false;
    }
}
...
abstract contract RaffleStorage {
    ...
    address public immutable gel;
    uint256 public immutable roundInterval;
    uint256 public immutable gelReward;
    uint256 internal constant _GOLD_ID = 5;
    uint256 internal constant _NORMAL_VARIANTS = 5;
