...
import {ForjTierLogic} from "contracts/utils/ForjTierLogic.sol";

contract ERC1155SelfMinterTiered is ForjERC1155, ForjTierLogic {
    ...
    function initialize(
        ...
        _paymentInitializer(_mintFee);
        _tierLogicInitialize(
            _saleId,
            tiersNumbers,
            limitPerWalletPerTier,
            starts,
            ends,
            merkleRoots,
            isTierPublic
    ...
    function tierMint(
        ...
        _tierChecks(to, tierNo, amount, merkleProof);
        _mintFeeCheck(msg.sender, amount, token);

        if(supplyPerId[saleId].total + amount > supplyPerId[saleId].max) revert TotalSupplyGreaterThanMaxSupply();

        supplyPerId[saleId].total += amount;

        mint(to, saleId, amount, "");

        emit TierMint(to, tierNo, amount);
    }
}
...
import "./ERC1155SelfMinterTiered.sol";

contract ERC1155SelfMinterTieredFactory {
    event CreatedTemplateERC1155SelfMinterTiered(address addr);
    function createTemplate(
        ...
        ERC1155SelfMinterTiered clone = new ERC1155SelfMinterTiered();
        clone.initialize(
            _multisig,
            _treasuryWallet,
            _baseURI,
        ...
        emit CreatedTemplateERC1155SelfMinterTiered(address(clone));
    }
}
...
contract ForjModifiers is Ownable {
    ...
    function setAdminStatus(address _admin, bool _status) public onlyAdminOrOwner(msg.sender){
...
contract ForjPaymentLogic is ForjTreasuryLogic{

    uint256 public mintFee;
    bool public paymentInitialized;

    mapping(address => uint256) public acceptedTokenPrice;

    function _paymentInitializer(uint256 _mintFee) internal {
        if(paymentInitialized) revert AlreadyInitialized();
        mintFee = _mintFee;
        paymentInitialized = true;
    }

    function setPayments(address[] memory _tokens, uint256[] memory _prices) public onlyAdminOrOwner(msg.sender){
        if(_tokens.length != _prices.length) revert ArrayLengthsDiffer();

        uint256 length = _tokens.length;
        
        for(uint256 i; i < length; i++){
            acceptedTokenPrice[_tokens[i]] = _prices[i];
        }
    }

    function removePaymentToken(address _token) public onlyAdminOrOwner(msg.sender){
        acceptedTokenPrice[_token] = 0;
    }
    ...
    function _mintFeeCheck(address _user, uint256 _amount, address _token) internal {

        uint256 payment;

        if (treasuryWallet != address(0)) {
            if (address(_token) == address(0)) {
               payment = mintFee * _amount;
               ...
               (bool sent, bytes memory data) = treasuryWallet.call{value: msg.value}("");
                ...
                payment = acceptedTokenPrice[_token] * _amount;
                if(acceptedTokenPrice[_token] == 0) revert TokenNotAcceptedAsPayment();
                if(ERC20(_token).balanceOf(_user) < payment) revert InsufficientBalance();

                ERC20(_token).transferFrom(msg.sender, treasuryWallet, payment);
            }
        }
    }
}
...
import {ForjERC1155} from "contracts/utils/ForjERC1155.sol";
import {ForjPaymentLogic} from "contracts/utils/ForjPaymentLogic.sol";
import {MerkleProof} from "@openzeppelin/contracts/utils/cryptography/MerkleProof.sol";

contract ForjTierLogic is ForjPaymentLogic{

    uint256 public saleId;
    bool public tierLogicInitialized;
    ...
    function _tierLogicInitialize(
        ...
        if(tierLogicInitialized) revert AlreadyInitialized();
        ...
        saleId = _saleId;
        tierLogicInitialized = true;
    }

    function setSaleId(uint256 _saleId) public onlyAdminOrOwner(msg.sender){
        saleId = _saleId;
    }

    function _tierChecks(
        address mintAddress,
        uint256 tierNo,
        uint256 amount,
        ...
        if(!tiers[tierNo].isPublic && !MerkleProof.verify(
            _merkleProof, 
            tiers[tierNo].merkleRoot, 
            keccak256(abi.encodePacked(mintAddress, tierNo)))
        ) revert InvalidProof();
    }
}
...
contract ForjTreasuryLogic is ForjModifiers, ForjCustomErrors {
