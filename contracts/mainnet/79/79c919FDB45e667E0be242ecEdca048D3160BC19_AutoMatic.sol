...
interface QuickSwapRouter02 is QuickSwapRouter01 {
...
contract DividendDistributor is IDividendDistributor {
    ...
    constructor (QuickSwapRouter02 _router) {
        router = _router;
        _token = msg.sender;
    }
...
contract AutoMatic is IERC20, Auth {
    
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    string constant _name = "AutoMatic";
    string constant _symbol = "AIA";
    ...
    address public autoLiquidityReceiver = 0x664210E1144C4a2EF0f0aAff40591b7C95D25d8c;
    address public marketingWallet = 0x81723063eaD5c32C64ED419a95dc916D6B71AaDA;

    QuickSwapRouter02 public router;
    address public pair;
    ...
    uint256 public swapThreshold = 2 * (10 ** _decimals);
    ...
    constructor () Auth(msg.sender) {
        ...
        QuickSwapRouter02 _router = QuickSwapRouter02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
        router = _router;

        pair = QuickSwapFactory(router.factory()).createPair(router.WETH(), address(this));
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 amountAIA = address(this).balance;

        uint256 totalAIAFee = totalFee.sub(liquidityFee.div(2));

        uint256 amountAIALiquidity = amountAIA.mul(liquidityFee).div(totalAIAFee).div(2);
        uint256 amountAIAReflection = amountAIA.mul(rewardsFee).div(totalAIAFee);
        uint256 amountAIAMarketing = amountAIA.sub(amountAIALiquidity).sub(amountAIAReflection);

        try dividendDistributor.deposit{value: amountAIAReflection}() {} catch {}

        uint256 marketingShare = amountAIAMarketing;
        ...
        if(amountToLiquify > 0){
            router.addLiquidityETH{value: amountAIALiquidity}(
                address(this),
                amountToLiquify,
                0,
                0,
            ...
            emit AutoLiquify(amountAIALiquidity, amountToLiquify);
        }
    }

    function RescueTokens(uint256 amount, address payable receiver) external onlyOwner {
        uint256 contractBalance = address(this).balance;
        require(contractBalance > amount,"Insufficient AIA");
        receiver.transfer(amount);
    }

    event AutoLiquify(uint256 amountAIA, uint256 amountBOG);
