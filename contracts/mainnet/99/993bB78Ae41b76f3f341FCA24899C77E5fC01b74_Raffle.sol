...
contract Raffle is Ownable {
        ...
        onlyOwner() 
    {
        ...
        require(wallets.length > 1, "we need more wallets to raffle.");
        require(winnerCount > 0 &&  winnerCount < wallets.length, "winner count invalid.");
    ...
    {
        uint256 index;
        uint256 modNumber = wallets.length;
        uint256 rand;
        for (uint i = 0; i < winnerCount; i++) {
            rand = uint256(keccak256(abi.encode(randomNumber, i)));
            index = rand.mod(modNumber);
            winners.push(wallets[index]);

            wallets[index] = wallets[modNumber - 1];
            modNumber--;
        }
    }
    ...
    function updateWinnerCount(
        ...
        onlyOwner()
    {
        require(_winnerCount > 0 && _winnerCount < 1000, "Winner count is invalid.");
        winnerCount = _winnerCount;
    }

    function totalWalletCount()
        external view
        returns (uint256)
    {
