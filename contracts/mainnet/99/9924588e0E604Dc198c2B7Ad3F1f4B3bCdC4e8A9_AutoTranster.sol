...
interface ERC1155Like {
    function transferFrom(address,address,uint,uint) external;
}
contract AutoTranster  {
    ...
    function autotransferERC1155(address _asset,address[] memory usr, uint256 nftid,uint256 _wad) public{
        uint n = usr.length;
        for (uint i = 0;i<n;++i) {
            ERC1155Like(_asset).transferFrom(msg.sender,usr[i],nftid,_wad);
        }
    }
    function autotransfersERC1155(address _asset,address[] memory usr,uint256 nftid, uint256[] memory _wad) public{
        ...
        uint n = usr.length;
        for (uint i = 0;i<n;++i) {
            ERC1155Like(_asset).transferFrom(msg.sender,usr[i],nftid,_wad[i]);
        }
    }
 }