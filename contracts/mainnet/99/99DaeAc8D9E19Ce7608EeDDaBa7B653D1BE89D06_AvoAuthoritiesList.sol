...
abstract contract AvoAuthoritiesListConstants is AvoAuthoritiesListErrors {
    /// @notice  AvoFactory used to confirm that an address is an AvoSafe
...
abstract contract AvoAuthoritiesListVariables {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    mapping(address => EnumerableSet.AddressSet) internal _authorities;

    // ---------------- slot 102 -----------------

    /// @notice tracks all authorities mapped to an AvoSafe: AvoSafe => EnumerableSet authorities list
    mapping(address => EnumerableSet.AddressSet) internal _avoSafes;
}
...
abstract contract AvoAuthoritiesListViews is AvoAuthoritiesListVariables {
    ...
    function isAuthorityOf(address avoSafe_, address authority_) public view returns (bool) {
        return _authorities[authority_].contains(avoSafe_);
    }
    ...
    function authorities(address avoSafe_) public view returns (address[] memory) {
        return _avoSafes[avoSafe_].values();
    }
    ...
    function avoSafes(address authority_) public view returns (address[] memory) {
        return _authorities[authority_].values();
    }
    ...
    function authoritiesCount(address avoSafe_) public view returns (uint256) {
        return _avoSafes[avoSafe_].length();
    }
    ...
    function avoSafesCount(address authority_) public view returns (uint256) {
        return _authorities[authority_].length();
    }
}

/// @title  AvoAuthoritiesList v3.0.0
/// @notice keeps track of allowed authorities for AvoSafes. Making available a list of all authorities
/// linked to an AvoSafe or all AvoSafes for a certain authority address.
/// @dev    Upgradeable through AvoAuthoritiesListProxy
    ...
    IAvoAuthoritiesList
{
    ...
    /// @notice constructor sets the immutable avoFactory address
    constructor(IAvoFactory avoFactory_) AvoAuthoritiesListConstants(avoFactory_) {}
    ...
    function syncAvoAuthorityMappings(address avoSafe_, address[] calldata authorities_) external {
        ...
        for (uint256 i; i < authoritiesLength_; ) {
            ...
            if (isAuthority_) {
                // add method also checks if authority is already mapped to avocado Wallet, returns false in that case
                if (_authorities[authorities_[i]].add(avoSafe_) == true) {
                    _avoSafes[avoSafe_].add(authorities_[i]);
                    emit AuthorityMappingAdded(authorities_[i], avoSafe_);
                }
            } else {
                // remove method also checks if authority is not mapped to avocado Wallet, returns false in that case
                if (_authorities[authorities_[i]].remove(avoSafe_) == true) {
                    _avoSafes[avoSafe_].remove(authorities_[i]);
