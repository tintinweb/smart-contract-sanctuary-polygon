...
import { MINTER_ROLE, OPERATOR_ROLE, HUNDER_PERCENT_IN_BPS } from "./libraries/Constants.sol";
    ...
    ERC721WithPermitUpgradable
{
    ...
    uint48 public slippagePercentInBps;
    uint48 public primarySalePercentInBps;
    ...
    function configPayment(
        address affiliate_,
        uint48 slippagePercentInBps_,
        uint48 primarySalePercentInBps_
    ) external onlyRole(OPERATOR_ROLE) {
        if (slippagePercentInBps_ > HUNDER_PERCENT_IN_BPS || primarySalePercentInBps_ > HUNDER_PERCENT_IN_BPS)
            revert Payment__ExceedBPS();
        if (affiliate_ == address(0)) revert Payment__InvalidAccount();
        affiliate = affiliate_;
        slippagePercentInBps = slippagePercentInBps_;
        primarySalePercentInBps = primarySalePercentInBps_;
    }

    function _calculateAmounts(
        uint256 value_,
        uint256 total_
    ) internal view returns (uint256 primarySaleAmount, uint256 affiliateAmount) {
        if (value_ < total_) revert Payment__InsufficientBalance();
        primarySaleAmount = (total_ * primarySalePercentInBps) / HUNDER_PERCENT_IN_BPS;
        affiliateAmount = value_ - primarySaleAmount;
    }

    function _processPayment(address paymentToken_, uint256 total_, address payer_) internal {
        uint256 total = total_;
        uint256 value;
        uint256 primarySaleAmount;
        uint256 affiliateAmount;

        if (isBusiness(payer_)) total = (total_ * slippagePercentInBps) / HUNDER_PERCENT_IN_BPS;

        if (paymentToken_ == address(0)) {
            value = msg.value;
            (primarySaleAmount, affiliateAmount) = _calculateAmounts(value, total);
            SafeTransferLib.safeTransferETH(_recipient, primarySaleAmount);
            SafeTransferLib.safeTransferETH(affiliate, affiliateAmount);
        } else {
            value = IERC20Upgradeable(paymentToken_).allowance(payer_, address(this));
            (primarySaleAmount, affiliateAmount) = _calculateAmounts(value, total);
            SafeTransferLib.safeTransferFrom(paymentToken_, payer_, _recipient, primarySaleAmount);
            SafeTransferLib.safeTransferFrom(paymentToken_, payer_, affiliate, affiliateAmount);
        }
    }
