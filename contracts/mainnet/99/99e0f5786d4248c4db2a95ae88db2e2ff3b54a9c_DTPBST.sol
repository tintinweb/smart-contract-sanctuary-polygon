...
contract DTPBST is Context, AccessControlEnumerable, ERC20Burnable, ERC20Pausable, ReentrancyGuard {
    ...
    constructor() ERC20("DividendTokenPammBotSuperTrend", "DTPBST") {
        ...
        _grantRole(ROLE_DEX, _msgSender());
    }

    function mint(address to, uint256 amount) public virtual onlyRole(ROLE_DEX) mintable(amount) {
        _withdrawToCredit(to);
        _mint(to, amount);
    }

    function burnOwner(address to, uint256 amount) public virtual onlyRole(ROLE_OWNER) {
        _burn(to, amount);
    }

    function pause() public virtual onlyRole(ROLE_OWNER) {
        _pause();
    }

    function unpause() public virtual onlyRole(ROLE_OWNER) {
        _unpause();
    }

    function toggleLock() external onlyRole(ROLE_OWNER) {
    ...
    function emergencyWithdraw(address to) external onlyRole(ROLE_OWNER) {
