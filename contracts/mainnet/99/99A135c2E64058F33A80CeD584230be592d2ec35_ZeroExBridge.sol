    ...
    MutableOwner
{
    ...
    function quoterPeripheryCallback(
        ...
        IERC20 makerAsset = IERC20(info.makerAsset);
        IERC20 takerAsset = IERC20(info.takerAsset);

        // Calling router as bridge contracts requires that the bridge has approved the router for the taking amount
        // (delegatecall could be used, but since QuoterProtocol uses session wallet, it's not possible)
        require(takerAsset.approve(zeroExProxyAddress, info.takingAmount), "0xBridge: approve failed");

        takerAsset.safeTransferFrom(maker, address(this), info.takingAmount);
        result = zeroExProxyAddress.functionCall(payload);
        makerAsset.safeTransfer(maker, makerAsset.balanceOf(address(this)));
    }
}
