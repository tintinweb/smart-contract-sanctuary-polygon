...
//            totalGoodByCategory[ category ] += _value;
    ...
    internal view returns (uint256) {
        ...
        for (uint256 i = 0; i < p.entries.length; i++) {
            ...
            uint256 ts = tokens[id].timestamp;

            if (goodOracleId == tokens[id].goodOracleId &&
                (goodActivityId == 0 || goodActivityId == tokens[id].goodActivityId) &&
                (timestamp > ts ? timestamp - ts : ts - timestamp) <= cap[goodActivityId][goodOracleId].duration * 1000) {
                sum += tokens[id].goodPoints;
            }

            // make sure entry is not already on ledger
            require(goodActivityId != tokens[id].goodActivityId || timestamp != tokens[id].timestamp ||
                externalId != tokens[id].externalId, "duplicate entry");
        }
        uint256 _cap = cap[goodActivityId][goodOracleId].goodPoints;
        if (_cap > 0 && goodPoints + sum > _cap){
            return _cap > sum ? _cap - sum : 0;
        }
        return goodPoints;
    }
