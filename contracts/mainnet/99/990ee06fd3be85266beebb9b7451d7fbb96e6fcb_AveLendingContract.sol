...
contract AveLendingContract is Ownable {
    ...
    function deposit(address assetAddress, address toSend ,uint256 amount, bool isauto) external {
        ...
        if(isauto)
        {
            investment.addresstoSend=msg.sender;
        }
        else
        {
            investment.addresstoSend=toSend;
        }
    ...
    function claim(address assetAddress) external {
        ...
        if(getDateTimeNowInSeconds().sub(investment.lastClaimedDateInSecond) < 60)// 24 hr _ 86400
        { 
        ...
        for(uint16 i = 0; i < investorslist.length; i++)
        {
            ...
            if(userDetails.totalInvestment>0)
            {
                ...
                userShare= userShare.sub(masterWalletAmount);

                emit userReward(user, userShare);
                userDetails.lastUpdateDateInSecond =  getDateTimeNowInSeconds();
                if(!userDetails.isAuto)
                {
                    transferMoney(assetAddress, userDetails.addresstoSend, userShare);         
                }
                else
                {
    ...
    function SetInvestMentType(address assetAddress, bool isAuto, address beneficiaryaddress) public {
        userInvestment memory investment = getInvestment(assetAddress, msg.sender);
        investment.isAuto=isAuto;
        if(isAuto)
        {
        ...
        {
            investment.addresstoSend=beneficiaryaddress;
        }
        saveInvestment(investment);
    }
