                ...
                if (skimmableMaticPlusAmount > 0) {
                    ...
                    try uniswapPair.partnerAddress() returns (address partnerAddress) {
                        if (partnerAddress != address(0)) {
                            uint256 _pAmount = skimmableMaticPlusAmount.mul(partnerBps).div(10000);
                            maticPlus.transfer(partnerAddress, _pAmount);
                            emit PartnerRemitted(partnerAddress, _pAmount);
                            skimmableMaticPlusAmount -= _pAmount;
                    ...
                    maticPlus.transfer(multiRewardPool, skimmableMaticPlusAmount);
                    console.log("MRP Amount:", address(uniswapPair), skimmableMaticPlusAmount);
                    emit RewardPoolRemitted(multiRewardPool, skimmableMaticPlusAmount);
                } else {
...
interface IVault {
    ...
    function mint(
        address _asset,
        uint256 _amount,
        uint256 _minimumMATICPlusAmount
    ) external;
