...
contract Swap is BaseContract {
  ...
  function swapUSDForToken(uint _usdAmount, ISwap.PaymentCurrency _paymentCurrency, bool _autoStake) external returns (uint) {
    ...
    if (_autoStake) {
      ...
      uint tax = _calculateSharkTax(msg.sender, taxAmount);
  ...
  function _calculateSharkTax(address _to, uint _taxAmount) private returns (uint) {
    ...
    if (sharkCheckpoints.length > 0) {
      ...
      if (checkpointIndex > 0) {
        return _taxAmount * sharkTaxPercentages[checkpointIndex] / DECIMAL3;
      }
    }
