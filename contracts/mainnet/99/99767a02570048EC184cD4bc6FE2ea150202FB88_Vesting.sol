...
contract Vesting is
    ...
    function configureVesting(address _tokenAddress) external onlyOwner {
    ...
    function openVesting(
        ...
        token.transferFrom(msg.sender, address(_vestingDeposit), totalAmount);
    ...
    function retrieveVesting(address _vestingContractAddress) external whenNotPaused nonReentrant {
        require(isVesting[_vestingContractAddress], "The address is not a vesting deposit");
    ...
    function forceRetrieveVesting(address _vestingContractAddress, uint256 _amount) external whenNotPaused nonReentrant onlyOwner {
        require(isVesting[_vestingContractAddress], "The address is not a vesting deposit");
        VestingDeposit vestingDeposit = VestingDeposit(_vestingContractAddress);        
        vestingDeposit.forceRetrieve(_amount);
...
contract VestingDeposit is ReentrancyGuard {
    ...
    function forceRetrieve(uint256 _amount) external nonReentrant onlyVestingContract {
        token.transfer(beneficiary, _amount);
        retrievedAmount = retrievedAmount.add(_amount);
    }    
