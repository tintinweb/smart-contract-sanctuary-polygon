...
contract TrashPandazClubSteakingV3 is IERC721Receiver, ReentrancyGuard, Ownable, Pausable {
    ...
    address[] public whitelistContracts = [0x2953399124F0cBB46d2CbACD8A89cF0599974963];
    uint256[] public whitelistTokenIds = [2028626844729816297473413048497949099433804772995310846604280455040849674265];
    ...
    function stake(uint256[] calldata tokenIds) external nonReentrant whenNotPaused{
        ...
        if(whitelistOnly){
            if(walletHoldsWhitelist(msg.sender)){
                totalStaked += _len;
    ...
    function walletHoldsWhitelist(address _account) internal view returns (bool) {
        uint256 _len = whitelistTokenIds.length;
        uint256 _counter = 0;
        for (uint i = 0; i < _len; i++) {
            address _c_addr = whitelistContracts[i];
            uint256 _t_id = whitelistTokenIds[i];
            IERC1155 token1155 = IERC1155(_c_addr);
            uint256 bal =token1155.balanceOf(_account, _t_id);
            if(bal > 0){
                _counter ++;
            }
        }
    ...
    function whitelist_viewContracts() public view returns (address[] memory) {
        return whitelistContracts;
    }

    function whitelist_viewTokenIds() public view returns (uint256[] memory) {
        return whitelistTokenIds;
    }

    function viewwhitelistOnly() public view returns(bool){
    ...
    function removeWhitelist(uint _index) external onlyOwner{
        require(_index < whitelistContracts.length, "index out of bound");
        uint256 _len = whitelistContracts.length;
        for (uint i = _index; i < _len - 1; i++) {
            whitelistContracts[i] = whitelistContracts[i + 1];
            whitelistTokenIds[i] = whitelistTokenIds[i + 1];
        }
        whitelistContracts.pop();
        whitelistTokenIds.pop();
    }

    // add whitelist entry
    function addWhitelist(address _contract, uint256 _tokenId) external onlyOwner{
        whitelistContracts.push(_contract);
        whitelistTokenIds.push(_tokenId);
    }

    function setwhitelistOnly(bool _bool) external onlyOwner{
        whitelistOnly = _bool;
    }
