...
event ConfirmedStats(uint256 xrequestId, uint256 tokenId);

function requestRandomWords(uint32 numWords, uint256 tokenId) external onlyOwner {
...
function fulfillRandomWords(
  ...
  emit ConfirmedStats(requestNumber, s_requestIdToTokenId[requestId]);
}


function queryStatsById(uint256 xrequestId) external view returns (uint256 [] memory) 
  {
    return s_requestIndexToRandomWords[xrequestId];
