    ...
    UUPSUpgradeable
{
    ...
    uint256 private minStakingDuration;
    uint256 private maxStakingDuration;
    ...
    function initialize() public initializer {
        ...
        tokenName = IERC20_Extended(tokenAddress).name();
        tokenDecimals = IERC20_Extended(tokenAddress).decimals();
        tokenSupply = IERC20Upgradeable(tokenAddress).totalSupply();

        stakingDurations = [90 days, 180 days, 365 days, 545 days];
        rewardRates = [2, 3, 4, 5];
        minStakingValue = 100000000000000000000;
        maxStakingDuration = 545 days;
        minStakingDuration = 30 days;
    ...
    function _stake(
        ...
        require(
            _duration <= maxStakingDuration && _duration >= minStakingDuration,
        ...
        userStakingIndexes[_address].push(uint8(stakingID));
    ...
    function _getStakingReward(
        ...
        stakingReward =
            (baseReward *
                _min(_min(endTime, stakingTimePassed), maxStakingDuration)) -
    ...
    function _claimReward(
        ...
        emit RewardClaimed(_address, stakingReward, _stakingID);
        return stakingReward;
    }
    ...
    function claimStakingReward(uint256 _stakingID) external returns (bool) {
        ...
        IERC20Upgradeable(tokenAddress).transfer(msg.sender, stakingReward);
        return true;
    }
    ...
    function isStaked(
        ...
        uint256 userStakingAmount = userAccount.valueStaked;
    ...
    function getUserStakedValueByStakinID(
        address _userAddress,
        uint256 _stakingID
    ) external view returns (uint256) {
        Account storage userAccount = user[_userAddress][_stakingID];
        return userAccount.valueStaked;
    }
    ...
    function getUserStakingStartTime(
        address _userAddress,
        uint256 _stakingID
    ) external view returns (uint256 userStakingStartTime) {
        Account storage userAccount = user[_userAddress][_stakingID];
        userStakingStartTime = userAccount.startTime;
    }

    function getUserStakingEndTime(
        address _userAddress,
        uint256 _stakingID
    ) external view returns (uint256 userStakingEndTime) {
        Account storage userAccount = user[_userAddress][_stakingID];
        userStakingEndTime = userAccount.endTime;
    }

    function getUserStakingDuration(
        address _userAddress,
        uint256 _stakingID
    ) external view returns (uint256 userStakingTotalDuration) {
        Account storage userAccount = user[_userAddress][_stakingID];
        userStakingTotalDuration = userAccount.endTime - userAccount.startTime;
    }

    function getUserStakingTimeRemaining(
        address _userAddress,
        uint256 _stakingID
    ) external view returns (uint256 userStakingTimeRemaining) {
        Account storage userAccount = user[_userAddress][_stakingID];

        if (block.timestamp >= userAccount.endTime) {
            userStakingTimeRemaining = 0;
        } else {
            userStakingTimeRemaining = userAccount.endTime - block.timestamp;
        }
    ...
    function getUserTotalRewardClaimed(
        ...
        uint256[] memory rewardClaimed = userRewardClaimed[_userAddress];
        uint256 rewardClaimedlength = rewardClaimed.length;
        uint256 totalRewardClaimed;

        for (uint256 i; i < rewardClaimedlength; i++) {
            totalRewardClaimed += rewardClaimed[i];
        }

        return totalRewardClaimed;
    }

    function getUserTotalRewardClaimedCount(
        address _userAddress
    ) external view returns (uint256) {
        return userRewardClaimed[_userAddress].length;
    }

    function getUserRewardClaimedArray(
        address _userAddress
    ) external view returns (uint256[] memory) {
        return userRewardClaimed[_userAddress];
    }

    function getUserTotalRewardClaimedTimeStampArray(
        address _userAddress
    ) external view returns (uint256[] memory) {
        return userRewardClaimedTimestamp[_userAddress];
    }
    ...
    function getStakingRewardRates() external view returns (uint256[] memory) {
        return rewardRates;
    }
    ...
    function getMinStakingValue() external view returns (uint256) {
    ...
    function setMinStakingDuration(
        uint256 _value
    ) external onlyOwner returns (uint256) {
        minStakingDuration = _value;
        return minStakingDuration;
    }

    function getMaxStakingDuration() external view returns (uint256) {
        return maxStakingDuration;
    }

    function setMaxStakingDuration(
        uint256 _value
    ) external onlyOwner returns (uint256) {
        maxStakingDuration = _value;
        return maxStakingDuration;
    }
