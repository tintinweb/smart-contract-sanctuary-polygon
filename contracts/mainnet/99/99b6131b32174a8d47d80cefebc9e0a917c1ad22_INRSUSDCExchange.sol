...
contract INRSUSDCExchange {
    ...
    constructor() {
        ...
        MATICPayable = payable(0x0000000000000000000000000000000000001010);
        ...
        owner = 0x5DFe20DEA8FA70C7da4F1B81c47FfD8f7B8fc5Bc;
    ...
    if (msg.value > 0) {
        ...
        payable(StableRINRS).transfer(INRSAmount);
         ...
         } else if (msg.sender == address(MATIC)) {
            depositMATIC();
          

        } else {
            // If the msg.sender is not the INRS or USDC token contract, we revert the transaction
            revert("Invalid Token Address: Only INRS or USDC Tokens are allowed. Matic can be send for Gas");
        }
    } else {
