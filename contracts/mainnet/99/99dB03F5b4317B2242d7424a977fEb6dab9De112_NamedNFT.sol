    ...
    Version
{
    ...
    event SetNFCSAddress(
        address indexed admin,
        INFCS oldNfcsAddress,
        INFCS newNfcsAddress,
        uint256 timestamp
    );

    event SetImageGenerator(
        address indexed admin,
        IImageGenerator oldGeneratorAddress,
        IImageGenerator newGeneratorAddress,
        uint256 timestamp
    );
    ...
    event SetTokenDescription(
        address indexed admin,
        string oldTokenDescription,
    ...
    //ImageGenerator address, handles image generation
    ...
    function initialize(
        ...
        _grantRole(Roles.UPDATER, admin);
    ...
    */
    function setNFCSAddress(INFCS _nfcs) external onlyRole(Roles.ADMIN) {
        require(address(_nfcs) != address(0), Errors.ZERO_VALUE);

        emit SetNFCSAddress(msg.sender, nfcs, _nfcs, block.timestamp);
    ...
    /**
    @notice Sets image generator address
    @param generator address of ImageGenerator contract
     */
    function setImageGenerator(IImageGenerator generator) external onlyRole(Roles.ADMIN) {
        emit SetImageGenerator(msg.sender, _imageGenerator, generator, block.timestamp);
        _imageGenerator = generator;
    }

    /**
    @notice Sets token description
    @param description of token
     */
    function setTokenDescription(string memory description) external onlyRole(Roles.ADMIN) {
        emit SetTokenDescription(msg.sender, _description, description, block.timestamp);
    ...
    function tokenURI(string memory name) external view virtual returns (string memory) {
        return
            nameData[name].exists
                ? _imageGenerator.buildNameImage(name.concat(extension), _description)
                : "";
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        return
            _exists(tokenId)
                ? _imageGenerator.buildNameImage(
                    tokenIdName[tokenId].concat(extension),
                    _description
                )
                : "";
    }
...
interface IImageGenerator {
    function buildNameImage(string memory name, string memory description)
        external
        view
