...
import "./BetVerseTokenTimelockICO.sol";
import '@openzeppelin/contracts/access/Ownable.sol';
import '@openzeppelin/contracts/token/ERC20/ERC20.sol';
import '@openzeppelin/contracts/utils/math/SafeMath.sol';

contract BetVerseTokenICO is ERC20, ReentrancyGuard, Ownable {
    ...
    function buy(uint256 daiAmount)
        external
        icoActive
        nonReentrant
        hasTokenBalance
        ...
        )
    {
        ...
        require(tokenBalance() > 0 && tokenBalance() >= tokenAmount, 'The ICO has no tokens available to sell anymore.');

        dai.transferFrom(msg.sender, walletTo, daiAmount);

        transferTokenToLockedAddresses(msg.sender, tokenAmount);

        return (tokenBalance(), tokenAmount, price);
    }

    function getTokenAmountToSell(uint256 daiAmount) public view returns (uint256) {
    ...
    {
        return (tokenBalance() >= getTokenAmountToSell(daiAmount), tokenBalance(), getTokenAmountToSell(daiAmount));
    }
    ...
    function transferTokenToLockedAddresses(address to, uint256 amount) public {
        ...
        for (uint256 i = 1; i <= term; i++) {
            ...
            BetVerseTokenTimelockICO timeLockContract = new BetVerseTokenTimelockICO();
    ...
    modifier hasTokenBalance() {
        require(tokenBalance() > 0, 'The ICO has no tokens available to sell anymore.');
        _;
    }

    modifier notInvesting() {
        require(isAlreadyInvesting() == false, "You can't participate of the ICO more than one time.");
        _;
    }
}
...
contract BetVerseTokenTimelockICO is TokenTimelockUpgradeable {
    function initialize(
...
contract BetVerseToken is ERC20Upgradeable, OwnableUpgradeable, Interest, ReentrancyGuardUpgradeable {
    ...
    function stake(uint256 amount_) external isAllowedToStake isStakingActive nonReentrant {
