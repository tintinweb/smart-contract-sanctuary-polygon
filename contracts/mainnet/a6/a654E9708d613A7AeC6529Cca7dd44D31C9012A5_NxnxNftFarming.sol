...
contract NxnxNftFarming is Ownable, IERC1155Receiver, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function updateUserInfo(
        ...
        uint256 beforeEtmAmount = getUserEtmAmount(_addr);
        user.amount = _amount;
        user.lastDepositedAt = _lastDepositedAt;
        user.lastHarvestedAt = _lastHarvestedAt;
        totalStakings = totalStakings.add(_amount).sub(beforeAmount);
        etmTotalStakings = etmTotalStakings.add(getUserEtmAmount(_addr)).sub(beforeEtmAmount);
    ...
    function batchUpdateUserInfo(
        ...
        uint lenAddrs = _addrs.length;
        uint lenAmounts = _amounts.length;
        require(lenAddrs == lenAmounts, "_addrs length and _amounts length is not match");
        for (uint i=0; i<lenAddrs; i++) {
            updateUserInfo(_addrs[i], _amounts[i], _lastHarvestedAt, _lastDepositedAt);
        }
    }

    function recalcTotalStakings() external onlyOwner {
        uint256 _totalStakings = 0;
        uint256 _etmTotalStakings = 0;
        uint len = userList.length;
        for (uint i=0; i<len; i++) {
            address _addr = userList[i];
            UserInfo memory user = userInfo[_addr];
            _totalStakings = _totalStakings.add(user.amount);
            _etmTotalStakings = _etmTotalStakings.add(getUserEtmAmount(_addr));
        }
        totalStakings = _totalStakings;
        etmTotalStakings = _etmTotalStakings;
    }
}