...
contract CRMAFIANFT is ERC721 , DefaultOperatorFilterer {
    uint256 public totalSupply;
    uint256 public cost = 1 ether;
    uint256 public maxMints = 15;
    ...
    constructor()
        ERC721("CRYPTO MAFIA NFT", "CRMAFIA")
    {}
    ...
    function mint() external payable {
        ...
        if(cost < threshold ) {
            if (msg.value < (cost + min_fee)) revert InsufficientFunds();
        } else {
        ...
        if(cost < threshold ) {
            payable(lmnft).transfer(min_fee);
            payable(owner).transfer(msg.value - min_fee);
        } else {
    ...
    function whitelistedMint(bytes32[] calldata _merkleProof) external payable {
        ...
        if (totalSupply + 1 > 10000) revert SoldOut();
        if(whitelistCost < 3 ether) {
            if (msg.value < (whitelistCost + min_fee)) revert InsufficientFunds();
        } else {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeih6wlrppanvgho42vik4aqwmlla4tarpvlmrelwtgqdzp3z26rwfe/", Strings.toString(tokenId), ".json")
            );
    }
