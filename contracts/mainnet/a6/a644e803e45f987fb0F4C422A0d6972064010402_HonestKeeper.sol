...
contract HonestKeeper is Initializable, OwnableUpgradeable {
	...
	modifier onlyOperator() {
		require(
			...
			"HonestKeeper :: You're not an operator"
		);
		_;
	}

	function initialize(
		IRegistryFarmerV2 _registryFarmer,
		IRewardFarmer _rewardFarmer
	) public initializer {
		__Ownable_init();
	...
	function transferPotatoFromRewardFarmer() external onlyOperator {
		rewardFarmer.transferFundsToRaffleWeekly();
	}
	...
	function setRewardFarmer(IRewardFarmer _rewardFarmer) external onlyOwner {
		rewardFarmer = _rewardFarmer;
	}
}
