...
/// @title An interface for a contract that is capable of deploying SMBSwap V3 Pools
/// @notice A contract that constructs a pool must implement this to pass arguments to the pool
/// @dev This is used to avoid having constructor arguments in the pool contract, which results in the init code hash
/// of the pool being constant allowing the CREATE2 address of the pool to be cheaply computed on-chain
interface ISMBV3PoolDeployer {
...
interface ISMBV3PoolImmutables {
    /// @notice The contract that deployed the pool, which must adhere to the ISMBV3Factory interface
...
interface ISMBV3PoolDerivedState {
...
interface ISMBV3PoolActions {
    ...
    /// @dev The caller of this method receives a callback in the form of ISMBV3MintCallback#smbV3MintCallback
    ...
    /// @dev The caller of this method receives a callback in the form of ISMBV3SwapCallback#smbV3SwapCallback
    ...
    /// @dev The caller of this method receives a callback in the form of ISMBV3FlashCallback#smbV3FlashCallback
...
interface ISMBV3PoolOwnerActions {
...
/// @title The interface for a SMBSwap V3 Pool
/// @notice A SMBSwap pool facilitates swapping and automated market making between any two assets that strictly conform
/// to the ERC20 specification
/// @dev The pool interface is broken up into many smaller pieces
interface ISMBV3Pool is
    ISMBV3PoolImmutables,
    ISMBV3PoolState,
    ISMBV3PoolDerivedState,
    ISMBV3PoolActions,
    ISMBV3PoolOwnerActions,
    ISMBV3PoolEvents
{
...
/// @title Minimal ERC20 interface for SMBSwap
/// @notice Contains a subset of the full ERC20 interface that is used in SMBSwap V3
...
/// @title The interface for the SMBSwap V3 Factory
/// @notice The SMBSwap V3 Factory facilitates creation of SMBSwap V3 pools and control over the protocol fees
interface ISMBV3Factory {
...
/// @title Callback for ISMBV3PoolActions#mint
/// @notice Any contract that calls ISMBV3PoolActions#mint must implement this interface
interface ISMBV3MintCallback {
    /// @notice Called to `msg.sender` after minting liquidity to a position from ISMBV3Pool#mint.
    /// @dev In the implementation you must pay the pool tokens owed for the minted liquidity.
    /// The caller of this method must be checked to be a SMBV3Pool deployed by the canonical SMBV3Factory.
    /// @param amount0Owed The amount of token0 due to the pool for the minted liquidity
    /// @param amount1Owed The amount of token1 due to the pool for the minted liquidity
    /// @param data Any data passed through by the caller via the ISMBV3PoolActions#mint call
    function smbV3MintCallback(
        uint256 amount0Owed,
        uint256 amount1Owed,
        bytes calldata data
    ) external;
...
/// @title Callback for ISMBV3PoolActions#swap
/// @notice Any contract that calls ISMBV3PoolActions#swap must implement this interface
interface ISMBV3SwapCallback {
    /// @notice Called to `msg.sender` after executing a swap via ISMBV3Pool#swap.
    ...
    /// @param data Any data passed through by the caller via the ISMBV3PoolActions#swap call
    function smbV3SwapCallback(
        int256 amount0Delta,
        int256 amount1Delta,
        bytes calldata data
    ) external;
...
/// @title Callback for ISMBV3PoolActions#flash
/// @notice Any contract that calls ISMBV3PoolActions#flash must implement this interface
interface ISMBV3FlashCallback {
    /// @notice Called to `msg.sender` after transferring to the recipient from ISMBV3Pool#flash.
    /// @dev In the implementation you must repay the pool the tokens sent by flash plus the computed fee amounts.
    /// The caller of this method must be checked to be a SMBV3Pool deployed by the canonical SMBV3Factory.
    /// @param fee0 The fee amount in token0 due to the pool by the end of the flash
    /// @param fee1 The fee amount in token1 due to the pool by the end of the flash
    /// @param data Any data passed through by the caller via the ISMBV3PoolActions#flash call
    function smbV3FlashCallback(
        uint256 fee0,
        uint256 fee1,
        bytes calldata data
    ) external;
...
contract SMBV3Pool is ISMBV3Pool {
    ...
    /// @inheritdoc ISMBV3PoolImmutables
    ...
    constructor() {
        int24 _tickSpacing;
        (factory, token0, token1, fee, _tickSpacing) = ISMBV3PoolDeployer(msg.sender).parameters();
    ...
    /// @inheritdoc ISMBV3PoolDerivedState
    function snapshotCumulativesInside(int24 tickLower, int24 tickUpper)
        external
        view
        override
    ...
    function mint(
        ...
        ISMBV3MintCallback(msg.sender).smbV3MintCallback(amount0, amount1, data);
    ...
    function swap(
        ...
        if (zeroForOne) {
            ...
            ISMBV3SwapCallback(msg.sender).smbV3SwapCallback(amount0, amount1, data);
    ...
    function flash(
        ...
        ISMBV3FlashCallback(msg.sender).smbV3FlashCallback(fee0, fee1, data);
    ...
    /// @inheritdoc ISMBV3PoolOwnerActions
    ...
    function setLmPool(address _lmPool) external override onlyFactoryOrFactoryOwner {
      lmPool = ISMBV3LmPool(_lmPool);
      emit SetLmPoolEvent(address(_lmPool));
    }
}

contract SMBV3PoolDeployer is ISMBV3PoolDeployer {
    struct Parameters {
        address factory;
        address token0;
        address token1;
    ...
    /// @param factory The contract address of the SMBSwap V3 factory
    /// @param token0 The first token of the pool by address sort order
    /// @param token1 The second token of the pool by address sort order
    /// @param fee The fee collected upon every swap in the pool, denominated in hundredths of a bip
    /// @param tickSpacing The spacing between usable ticks
    function deploy(
        ...
        pool = address(new SMBV3Pool{salt: keccak256(abi.encode(token0, token1, fee))}());
        delete parameters;
    }
}