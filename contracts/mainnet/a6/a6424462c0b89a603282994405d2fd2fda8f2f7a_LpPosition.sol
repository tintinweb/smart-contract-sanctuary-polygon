...
interface IDittoPool {
     ...
     */
    function _bondingCurve() external pure returns (string memory curve);
 ...
 */
contract LpPosition is  ILpPosition, ERC721Solmate {
     ...
     */
    constructor(address metadataGenerator_) ERC721Solmate("Ditto LP Positions NFT", "ULP") {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory curve = pool._bondingCurve();
        ...
        string memory poolAddress = Strings.toHexString(uint256(uint160(address(pool))), 20);
        //string memory admin = Strings.toHexString(uint256(uint160(pool._admin())), 20);
        string memory admin = "0x00000000000000000000000000000000DeaDBeef";
        string memory symbolNft = IERC721Metadata(address(pool.nft())).symbol();
        string memory symbolToken;
        IERC20 poolToken = IERC20(pool.token());
        if(address(poolToken) == address(0)) {
            symbolToken = 'ETH';
        } else {
            symbolToken = IERC20Metadata(address(poolToken)).symbol();
        }
        MetadataInfo memory info = MetadataInfo(curve, delta, fee, symbolNft, getNumNftsHeld(tokenId), address(poolToken), symbolToken, getLpPositionValueToken(tokenId), admin, poolAddress);

        return _metadataGenerator.payloadTokenUri(info, tokenId, address(pool), pool.token());
    }
...
contract MetadataGenerator {
    ...
    function payloadTokenUri(MetadataInfo memory input, uint256 tokenId_, address pool, address token) view public returns (string memory) {
...

        {
            ...
            uint256 decimals;
            try IERC20Metadata(address(input.token)).decimals() returns (uint8 decimals_){
