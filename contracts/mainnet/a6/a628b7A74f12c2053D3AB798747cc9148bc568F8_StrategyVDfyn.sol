...
contract StrategyVDfyn is Ownable, RouterCrossTalk {
    ...
    function _routerCrossTalkBurn(uint256 amt, address user) internal {
        bytes4 _selector = bytes4(keccak256("burn(_amount)"));
        bytes memory _data = abi.encode(user, amt);
        bool success = routerSend(chainId, _selector , _data , crossChainGas);
        // return success;
    }
    ...
    function balanceOfvDfyn() public view returns (uint256) {
        ...
        return (IvDFYN(vdfyn).balanceOf(address(this))).mul(getExchangeRate()).div(1e18);
    }

    function getExchangeRate() public view returns (uint256) {
        uint256 dfynBal = IERC20(want).balanceOf(vdfyn);
        uint256 totalSupply = IvDFYN(vdfyn).totalSupply();
        return dfynBal.mul(1e18).div(totalSupply);
    }
