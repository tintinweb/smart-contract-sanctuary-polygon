...
contract SampleKitchenSinkNFT is
    Initializable,
    ...
    OwnableUpgradeable
{
    ...
    /// @notice Mints multiple NFTs to an address
    /// @param to Receiver address
    /// @param quantity Number of NFTs to mint
    function safeMultiMint(address to, uint256 quantity) public onlyRole(MINTER_ROLE) {
    ...
    function multiBurn(uint256[] calldata tokenIds) public {
    ...
    function subscribeToFilterRegistry(address subscriptionOrRegistrantToCopy, bool subscribe)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function mintFor(
        ...
        uint256 tokenId = abi.decode(_blob, (uint256));
        _mint(_to, tokenId);
    }
    ...
    {
        uint256 royaltyAmount = (_salePrice * royaltyNumerator) / ROYALTY_DENOMINATOR;
        return (royaltyReceiver, royaltyAmount);
    }

    /// @notice Set the royalty info for all tokens
    /// @param _royaltyReceiver Account to receive sale royalties
    /// @param _royaltyNumerator Fraction relative to ROYALTY_DENOMINATOR
    function setRoyaltyInfo(address _royaltyReceiver, uint256 _royaltyNumerator)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
