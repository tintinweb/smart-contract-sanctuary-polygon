...
contract ApeliensLarvae is BaseERC721 {
    ...
    constructor(
        ...
        BaseERC721(
            "Apeliens Larvae",
            "LARVAE",
            "https://apeliens-larvae.us-east-1.linodeobjects.com/",
            ".json",
            3333,
            0xb80C2654ac73730D508028dD4e84F13913B2DDb1,
            750
        )
    {
    ...
    function unclaimedTokenIds(address _owner) public view returns (uint256, uint256[] memory) {
        if (_owner == address(0)) revert InvalidAddress();
        if (address(bapeliens) == address(0)) revert InvalidAddress();

        uint256 balance = bapeliens.balanceOf(_owner);
        uint256[] memory unclaimed = new uint256[](balance);
        uint256 idx = 0;
        for (uint256 i = 0; i < balance; ) {
            uint256 tokenId = bapeliens.tokenOfOwnerByIndex(_owner, i);
            if (!_exists(tokenId)) {
