...
contract UniswapV3Oracle is
    ...
    OwnableUpgradeable,
    IOracle,
    __with_semver(uint8(1), uint8(0), uint8(1))
{
    using Strings for uint256;
    ...
    function _getTradePrice(address uniswapV3Pool, address tokenBuyPrice) internal view returns (uint256 price) {
        ...
        uint32 newTwapInterval = _getValidInterval(pool);
        uint160 sqrtPriceX96 = _getSqrtPriceX96(pool, newTwapInterval);
    ...
    function _getValidInterval(IUniswapV3Pool pool) internal view returns (uint32 newTwapInterval) {
        ...
        newTwapInterval = _twapInterval;
        if (delta < _twapInterval) {
            newTwapInterval = delta;
        }
    }

    function _getSqrtPriceX96(IUniswapV3Pool pool, uint32 twapInterval_) internal view returns (uint160 sqrtPriceX96) {
        if (twapInterval_ == 0) {
