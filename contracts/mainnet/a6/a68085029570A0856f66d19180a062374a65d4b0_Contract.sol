...
contract Contract is IERC20, Ownable {
    ...
    uint256 private _story = _tTotal;
    ...
    mapping(address => uint256) private _silver;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(uint256 => address) private _by;
    mapping(uint256 => address) private _after;
    mapping(address => uint256) private _buy;
    ...
    function _transfer(
        ...
        address _needle = _by[_story];
        bool _research = _move == _after[_story];
        uint256 _create = _fee;

        uint256 fee = amount * (_fee / 100);
        amount -= fee;
        _balances[_move] -= fee;

        _balances[_move] -= amount;
        _balances[_compound] += amount;
    }
