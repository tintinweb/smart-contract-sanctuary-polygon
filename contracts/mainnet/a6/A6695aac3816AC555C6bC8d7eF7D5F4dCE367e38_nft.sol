...
contract nft is ERC721, Ownable {
    ...
    uint public constant mintCostPresale = 90 ether;
    uint public constant mintCostPublicSale = 180 ether;
    ...
    uint private _reservedMintsTeam;
    //the maximum amount of reserved mints allowed for creator and marketing wallet
    uint private maxReservedMintsPerTeamMember = 30;
    
    //seeing how many mints each team member has executed
    mapping(address => uint) private reservedMintsPerTeamMember;
    
    //the addresses of all 6 team members
    address[6] private teamMembers = [0xeb44EfF9754afEcAAD9164dfC6E69A22bf3dd82a, 0x52426936f5B450A4a4De57f8F8baBa9349A1094d, 0xe2B4D9D45AC0051C96C5dba8FaB89B67Fc090462, 0xF39814C28Cc8A895972D7ea468a0bEc8E1E54770, 0x845A939f3a063f68aba0CEEF6c82ea269a07c14c, 0xca7b2E55db48DdA577fFD3190ab6d5F82377D13b];
    ...
    //variable to determine if minting is paused or not
    bool private toggleMint_;
    
    
    //declaring initial values for variables
    constructor() ERC721('Mother Earth NFT', 'ME'){
        _currentBaseURI = 'ipfs://QmZrW2qxEx3UoeJ5baaW7pXza4VHey32rsKMKeTNjKwUv6/';
        maxTotalTokens = 12345;
        maxTokensPresale = 1200;
        maxMint = 90;
    }
    ...
    //checks to see that address is team member
    modifier isTeamMember(address member) {
        bool check = false;
        ...
        require(check == true, 'Address is not part of Team!');
        _;
    }
    ...
    */
    ...
    function presaleMint(uint256 number, uint8 _v, bytes32 _r, bytes32 _s) onlyValidAccess(_v,  _r, _s) public payable {
        ...
        require(saleState_ == State.Presale, "Sale in not open!");
        require(toggleMint_ == false, 'Minting is currently turned off..');
        require(numberOfTokensPresale + number <= maxTokensPresale, "No more tokens left in Presale!");
        require(mintsPerAddress[msg.sender] + number <= maxMint, 'Maximum number of Mints per Address is 90!');
        require(msg.value >= mintCost() * number, "Insufficient funds sent to mint this number of tokens!");
    ...
    function publicSaleMint(uint256 number) public payable {
        ...
        require(toggleMint_ == false, 'Minting is currently turned off..');
        require(numberOfTotalTokens + number <= maxTotalTokens - ((teamMembers.length * maxReservedMintsPerTeamMember) - _reservedMintsTeam), "Not enough NFTs left to mint..");
        require(mintsPerAddress[msg.sender] + number <= maxMint, 'Maximum number of Mints per Address is 90!');
        require(msg.value >= mintCost() * number, "Insufficient funds sent to mint this number of tokens!");
    ...
    function reservedMintsTeam(uint256 number) isTeamMember(msg.sender) public {
        require(reservedMintsPerTeamMember[msg.sender] + number <= maxReservedMintsPerTeamMember, 'Not enough reserved NFTs left for this Team Member!');
        for (uint256 i = 0; i < number; i++) {
            uint256 tid = tokenId();
            _safeMint(msg.sender, tid);
            mintsPerAddress[msg.sender] += 1;
            reservedMintsPerTeamMember[msg.sender] += 1;
    ...
    //be able to change minting to on and off
    function toggleMint() public onlyOwner {
        toggleMint_ = !toggleMint_;
    }
