...
contract Maticfinance {
	...
	uint[5] public ref_bonuses = [48,7,6,5,4];
    ...
    uint[5] public requiredDirect = [1,1,1,1,1];
    
	address payable public primaryAdmin;

    uint public maxupline = 15;
    uint public maxdownline = 15;
    ...
    constructor() public {
		primaryAdmin = 0xa8a5ee7fE55EFFC4106B8DA294129677B14d785a;
  ...
  function _usersDownlineIncomeDistribution(address _user, uint256 _Amount) internal {
      ...
      uint256 DistributionPayment = _Amount.mul(15).div(100);
      ...
      for (uint i = 0; i < maxdownline; i++) {
            if (upline != address(0)) {
                if(upline_Business_eligible[upline]){
                    ...
                    uint256 ReceivingPayment = users[upline].amount.mul(15).div(100);
