...
interface IKyberLM {
  ...
  function harvestMultiplePools(uint256[] calldata nftIds, HarvestData memory data) external;
...
contract StrategyKyberCLA is StratFeeManagerInitializable, IERC721Receiver {
    ...
    function _init(uint256 _amount0, uint256 _amount1) private returns (uint256 actual0, uint256 actual1, uint256 liquidity) {
        ...
        (ticksPrevious[0],) = ITickReader(nftInfo.tickReader).getNearestInitializedTicks(address(pool), ticks.lowerTick);
        (ticksPrevious[1],) = ITickReader(nftInfo.tickReader).getNearestInitializedTicks(address(pool), ticks.upperTick);
    ...
    function _withdrawFromPool(uint128 _liquidity) private returns (uint256 amt0, uint256 amt1) {
        farm.harvestMultiplePools(nftInfo.tokenIds, IKyberLM.HarvestData({
            pIds: nftInfo.pids
        }));
