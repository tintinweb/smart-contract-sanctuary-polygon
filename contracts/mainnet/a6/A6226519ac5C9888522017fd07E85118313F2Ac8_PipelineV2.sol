...
contract PipelineV2 is Ownable {
    ...
    /// Predefiended actions 

    bytes32 constant APPROVE_ACTION = keccak256("APPROVE"); // 8cf74985ef9d149204edb8e66f26e5768254597f6db68dda508d7d79669a95d2

    // mapping(address => bool) public whitelisted;

    // event Whitelist(address indexed addr, bool value);
    ...
    // // Use whitelist as merkle tree
    // function whitelistMany(address[] calldata addrs, bool[] calldata values) external onlyOwner {
    //     require(addrs.length == values.length, "Wrong params. Unequal length");
    //     for (uint256 i = 0; i < addrs.length; i++) {
    //         _whitelist(addrs[i], values[i]);
    //     }
    // }
    ...
    function run(Action[] calldata actions, uint256 slotsAmount, address tokenOut, uint256 minOutAmount) payable external {
        uint256 balanceBefore;
        if (minOutAmount > 0) {
            balanceBefore = IERC20(tokenOut).balanceOf(msg.sender); // #TODO eth case 
        }
        bytes32[] memory slots = new bytes32[](slotsAmount); // May be bytes[] memory? Will be problems with injection
        uint256 lastSlot;
        ...
        if(minOutAmount > 0) {
            uint256 balanceDiff = IERC20(tokenOut).balanceOf(msg.sender) - balanceBefore;
    ...
    function _contractCall(address target, uint256 ethValue, bytes memory callData) internal returns(bytes32) {
        ...
        // require(whitelisted[target], "Target is not whitelisted");
    ...
    // function _whitelist(address addr, bool value) internal {
    //     whitelisted[addr] = value;
    //     emit Whitelist(addr, value);
    // }
}