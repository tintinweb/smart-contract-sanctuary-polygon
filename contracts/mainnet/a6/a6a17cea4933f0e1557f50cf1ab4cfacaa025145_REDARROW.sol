...
contract REDARROW is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "RED ARROW";
    string private _symbol = "$REDARROW";
    ...
    function setTaxes(uint _newLp,uint _newBurn) public onlyOwner{
        burnFee = _newBurn;
        liquidity = _newLp;
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[recipient]){
            feeAmount = amount.mul(burnFee).div(1000);
            lp = amount.mul(liquidity).div(1000);
            swapandliquifiy(lp);
        }
        ...
        return amount.sub(feeAmount).sub(lp);
    }
    
    function swapandliquifiy(uint _amount) internal lockTheSwap {
        
        if(_amount == 0){return;}
        
        uint half = _amount.div(2);    
        uint otherhalf = _amount.sub(half);

        uint balance = address(this).balance;
        swapTokensForEth(half);
        uint amountBNBLiquidity = address(this).balance.sub(balance);
