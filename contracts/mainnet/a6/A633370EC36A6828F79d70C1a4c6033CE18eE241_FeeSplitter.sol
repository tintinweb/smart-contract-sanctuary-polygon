...
contract FeeSplitter is TokensRecoverable
{
    ...
    {
        require(royaltyCollector[msg.sender] || msg.sender == devAddress || msg.sender == owner);
        _;
    }
    ...
    function setFees(IGatedERC20 token, uint256 burnRate, uint256 sellRate, uint256 keepRate) public onlyRoyaltyCollector() // 100% = 10000
    {
    ...
    //todo update below
    function setChainTokenFeeCollectors(IGatedERC20 token, address[] memory collectors, uint16[] memory rates) public onlyRoyaltyCollector() // 100% = 10000
    {
    ...
    function setRootedTokenFeeCollectors(IGatedERC20 token, address[] memory collectors, uint16[] memory rates) public onlyRoyaltyCollector() // 100% = 10000
    {
        require (collectors.length == rates.length, "Fee Collectors and Rates must be the same size");
