...
contract SandStakingImplementation is ReentrancyGuard {
    ...
    ) external
    {
        ...
        if (_sponsor != address(0)) {
            IYieldManager(factoryAddress.getYieldManager()).setAffiliate(owner, _sponsor);
        }
    ...
    function withdraw(uint256 amount) external onlyOwner nonReentrant {
        ...
        // get user stats
        (uint val1, , uint val3) = IYieldManager(factoryAddress.getYieldManager()).getUserFactors(
            msg.sender,
            0
        );
        ...
        // get sponsor
        address sponsor = IYieldManager(factoryAddress.getYieldManager()).getAffiliate(msg.sender);
        // get sponsor stats
        ...
        // get withdrawal fee token
        IERC20(ISandStakingFactory(factoryAddress).getFeeToken()).transferFrom(msg.sender, address(this), val1);
        IERC20(ISandStakingFactory(factoryAddress).getFeeToken()).transfer(address(factoryAddress), val1);
    ...
    function getReward() external onlyOwner {
        ...
        uint sPerfFee = (perfFee * sval1) / 100 / 100;

        ISandStaking(ISandStakingFactory(factoryAddress).getStakingContract()).getReward();
        IERC20(ISandStakingFactory(factoryAddress).getRewardToken()).transfer(msg.sender, amount - perfFee - sPerfFee);
        IERC20(ISandStakingFactory(factoryAddress).getRewardToken()).transfer(sponsor, sPerfFee);

        emit GetRewards(msg.sender, amount);
    }
}
