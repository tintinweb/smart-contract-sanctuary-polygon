...
contract ElegantRace is Ownable, Concatenate {
    ...
    address public leagueAddress = 0x906a00f51587605B9D142CF6056043032A08627F;
    ...
    function getCoaches(address[] memory _wallets) public {
        LeagueInterface _league = LeagueInterface(leagueAddress);
        address[] memory _wal = _wallets;
        uint len = _wal.length;
        uint i;
        for (i; i < len; i++){
            address w = _wal[i];
            walletToCoach[w] = _league.walletToCoach(w);
        }
    }

    function grabRacers(uint _week, uint _start, uint _end) external { 
        LeagueInterface _league = LeagueInterface(leagueAddress);
        uint i = _start;
        for (i; i <= _end; i++){
            (uint _id, 
            ...
            uint _f,
            uint _e,
            uint _r) = _league.perfByRacer(0,i);
            if (perfByRacer[_week][i].id == 0 && _e != 100){
                perfByRacer[_week][i].id = _id;
                perfByRacer[_week][i].eventWanted = _e;
                racersByEvent[1].push(i);
            }           
        }
