...
contract Drn is ERC20, Ownable {
    ...
    uint claimInterval = 86400; //second
    ...
    constructor() ERC20("Drn Token", "DRN") {
        _mint(msg.sender, 10000000 * 10**18); // 1000万 x 10^18
        minter = msg.sender;
    }

    function mint(uint amount) public {
        require(msg.sender == minter);    // only admin(minter) can mint
        require(amount < 1e60);
    ...
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
        emit DebugAddress(owner());
        emit DebugAddress(to);
        emit DebugUint(amount);
        return true;
    }

    function setClaimInterval(uint interval) public  {
    ...
    function setRandomMin(uint minValue) public  {
        require(msg.sender == minter);
        randomMin = minValue;
    }

    function getRandomMin() public view returns (uint) {
        require(msg.sender == minter);
        return randomMin;
    }


    function setRandomMax(uint maxValue) public  {
        require(msg.sender == minter);
        randomMax = maxValue;
    }

    function getRandomMax() public view returns (uint) {
        require(msg.sender == minter);
        return randomMax;
    }

    function getLastClamTimestamp(address adrs) public view returns (uint256) {     
        return _lastClaimTimestamp[adrs];
    }

    function claim() external returns (uint) {
        if(_lastClaimTimestamp[msg.sender] != 0) {
            require(_lastClaimTimestamp[msg.sender] + claimInterval < block.timestamp, "Invalid interval."); // claim needs interval
        }

        // update last claim timestamp
        _lastClaimTimestamp[msg.sender] = block.timestamp;

        uint getAmount = getRandomUint();
        _transfer(owner(), msg.sender, getAmount * 10**18);

        return getAmount;
    }

    function getRandomUint() internal returns (uint) {

        _randNonce = block.number + 1;


        uint randomUint;

        while(randomUint < randomMin || randomUint > randomMax) { //条件式aが満たされている場合、繰り返し処理が続く。
            uint random = uint(keccak256(abi.encodePacked(block.timestamp, blockhash(_randNonce))));        
            randomUint = random % (randomMax +1);
        }
