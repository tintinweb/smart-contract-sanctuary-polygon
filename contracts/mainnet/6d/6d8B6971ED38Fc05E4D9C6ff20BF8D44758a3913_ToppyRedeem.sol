...
contract ToppyRedeem is Ownable{

    ToppyMaster public toppyMaster = ToppyMaster(address(0x6A4BCd9d780a1855De71CcbbB71bCF2a09730164));
    address public toppyMint = address(0xe86B2461994F595e2b68617e925691e30C004a9B);
    
    event _redeemNFT(bytes32 key, address nftContract, uint256 tokenId, address owner, address to);

    constructor() {}
    
    function redeemNFT(address _nftContact, uint _tokenId) public payable{
        
        bytes32 hashedKey = _getId(_nftContact, _tokenId);
        address creatorOwnerAddress = IToppyMint(toppyMint).getCreator(hashedKey);
        IERC721(_nftContact).transferFrom(msg.sender, creatorOwnerAddress, _tokenId);
        emit _redeemNFT(hashedKey, _nftContact, _tokenId, msg.sender, creatorOwnerAddress);
    }
    ...
    function updateContract(address _toppyMaster, address _toppyMint) public onlyOwner {
