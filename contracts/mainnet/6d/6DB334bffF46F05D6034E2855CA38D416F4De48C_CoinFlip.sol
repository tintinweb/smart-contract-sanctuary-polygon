...
contract CoinFlip is Ownable, ReentrancyGuard{
  ...
  uint256 public minticket = 10 ether;
  ...
  function game(uint256 bet, uint256 seed, uint256 _tokenId) public payable nonReentrant returns (bool) {
    ...
    require( msg.value >= minticket && msg.value <= amount , "wrong ETH");
 ...
 function manageRef(uint256 amount) internal {
  ...
  function checkWinner(uint8 random) internal {
      ...
      emit Result(games[gameId].id, games[gameId].bet, games[gameId].seed, games[gameId].amount, games[gameId].player, winAmount, random, block.timestamp, LoseStreak[currentRun][games[gameId].player],WinStreak[currentRun][games[gameId].player]);
