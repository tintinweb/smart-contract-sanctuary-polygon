...
//SilverCard
abstract contract BundotMembershipDiamondCard is ERC721Burnable, Ownable, ReentrancyGuarded{
...
contract RamperBridgeforBundotDiamondCard721 is IRamperBridgeInterface721 {

BundotMembershipDiamondCard public diamondcardaddress;

constructor(BundotMembershipDiamondCard _existingcontractaddress){
    diamondcardaddress = _existingcontractaddress;
}
...
function price() external view returns (uint256)
{
    return diamondcardaddress.getPrice();
}

function mint(address _userWallet, uint256 _quantity) override external payable {
        require(_quantity == 1);
        diamondcardaddress.mintToken{value:msg.value}(_userWallet);
    }
    ...
    function safeBulkTransferFrom(address _from, address _to, uint256[] memory _tokenIds) external{
        for(uint256 i;i < _tokenIds.length; i++)
               diamondcardaddress.safeTransferFrom(_from,_to,_tokenIds[i]);
    }
