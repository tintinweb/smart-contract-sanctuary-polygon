...
interface IS {
    ...
    function addFightToWl (address _fc) external;
    function fightMint(address _fa, uint _users, address _ls, uint spotCount) external;
...
contract Fight {
    ...
    constructor(address _ff, address _dev, address _dev2, uint _dscs,uint _dscs2, uint _spotResPrice, address _fightAdmin, address _tc, address _influ2, address _charity, address _lsp) {
        ...
        require(extention <=3600 && duration<= 604800 && duration > 86400, '');
  ...
  //flip
   ...
   //make withdrawal 
    ...
    //make withdrawal of Last Spot Pot
        ...
        actTimer=0;
        payable(msg.sender).transfer(amount);
        //minting reward to influencer 1
        IS(tc).fightMint(fightAdmin, keyList.length, ls, spotCounter);
    }
   ...
   //Vote 2
    ...
    //Show withrawal balances and votes
    function showBalance() view external returns (uint, uint, uint, address, uint, uint, uint, uint, uint) {
    return (pw[msg.sender], pw[charity], pw[lsp],ls, created, actTimer, v1, v2, totalStaked);
    }
