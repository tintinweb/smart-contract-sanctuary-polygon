...
contract SynassetsERC20Token is ERC20Permit, VaultOwned {
    ...
    constructor(IUniswapV2Router02 _uniswapV2Router, IERC20 _assetToken) ERC20("Synassets Test Matic Token", "sTESTMATIC", 9) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 initialBalance = assetToken.balanceOf(address(this));

        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = assetToken.balanceOf(address(this)).sub(initialBalance);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        assetToken.approve(address(uniswapV2Router), ethAmount);
