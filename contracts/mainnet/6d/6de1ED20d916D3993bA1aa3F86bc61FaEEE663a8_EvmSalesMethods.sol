...
contract EvmSalesMethods is ERC721Holder, ERC1155Holder, Ownable, ReentrancyGuard, TokensHandler {
    uint public id;
    uint public fee = 20;
    ...
    function purchaseCost(address buyer, uint saleId) public view returns(uint256) {
        uint discount = IDiscounts(discounts).calculateDiscount(buyer);
        return sales[saleId].price / fee / discount;
    }
    ...
    function cancel(uint saleId) external {
        require(msg.sender == sales[saleId].seller, 'You are not the owner of this sale');
        require(sales[saleId].status == Status.LISTED, 'Sale already cancelled');
    ...
    function buy(uint saleId) external payable {
        require(saleId < id);
        require(sales[saleId].seller != msg.sender);
        require(sales[saleId].buyer == address(0));

        uint bmarketFee = purchaseCost(msg.sender,saleId);

        if ( sales[saleId].currency != address(0) ) {
            require(IERC20(sales[saleId].currency).transferFrom(msg.sender,address(this),sales[saleId].price));
        ...
        transferTokens(sales[saleId].seller,address(this),sales[saleId].nftAddresses,sales[saleId].nftIds,sales[saleId].nftTypes);
