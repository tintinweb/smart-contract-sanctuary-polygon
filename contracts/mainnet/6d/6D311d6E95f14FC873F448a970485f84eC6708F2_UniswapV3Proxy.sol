...
contract UniswapV3Proxy {
    ...
    function withdraw() private {
        wmatic.withdraw(wmatic.balanceOf(msg.sender));
        require(
            payable(msg.sender).send(wmatic.balanceOf(msg.sender)),
            "Failed to send unwrapped base token"
        );
        emit WithdrawBaseToken(msg.sender, wmatic.balanceOf(msg.sender));
    }
}