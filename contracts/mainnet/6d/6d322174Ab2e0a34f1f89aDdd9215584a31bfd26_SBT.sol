...
contract SBT is ERC721, ERC2771Context, ERC721URIStorage, Ownable, EIP712 {
    ...
    function _check(uint256 id, string memory payment, bytes memory signature) public view returns (address) {     
       bytes32 digest = _hashTypedDataV4(
            keccak256(abi.encode(
                keccak256("Web3Struct(uint256 id,string payment)"),
                id,
                keccak256(bytes(payment))
            ))
        );
    ...
    function safeMintMeta(
        ...
        require(tokenId == id, "Signature already used");
        if (mintPrice > 0) {
            require((bytes(payment).length > 0), "Valid payment required");
        }
        require(
            _check(id, payment, signature) == signerAddress,
