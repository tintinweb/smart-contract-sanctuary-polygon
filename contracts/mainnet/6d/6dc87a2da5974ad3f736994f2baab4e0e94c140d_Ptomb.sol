...
contract Ptomb is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 PTOMB-WETH LP -> PTOMB pool
    uint256 public constant INITIAL_PTOMB_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the PTOMB ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("PolyTomb.Finance", "PTOMB") {
        // Mints 1 PTOMB to contract creator for initial pool setup
    ...
    function _getPtombPrice() internal view returns (uint256 _ptombPrice) {
        try IOracle(ptombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Ptomb: failed to fetch PTOMB price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _ptombPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_ptombPrice >= taxTiersTwaps[tierId]) {
    ...
    function setPtombOracle(address _ptombOracle) public onlyOperatorOrTaxOffice {
        require(_ptombOracle != address(0), "oracle address cannot be 0 address");
        ptombOracle = _ptombOracle;
    }
    ...
    /**
     * @notice Operator mints PTOMB to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of PTOMB to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentPtombPrice = _getPtombPrice();
            currentTaxRate = _updateTaxRate(currentPtombPrice);
            if (currentPtombPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_ptombPool != address(0), "!_ptombPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_ptombPool, INITIAL_PTOMB_POOL_DISTRIBUTION);
