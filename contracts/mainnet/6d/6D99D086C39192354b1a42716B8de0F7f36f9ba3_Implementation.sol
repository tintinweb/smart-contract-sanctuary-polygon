...
contract NFT is ERC721Enumerable,ERC721URIStorage {
    ...
    mapping (address => _owned[]) ownerlist;
    mapping (address => mapping (uint256 => uint256)) countertracker;

    constructor(address accessControlAddress, string memory _NAME, string memory _SYMBOL ) ERC721(_NAME, _SYMBOL) {
    ...
    function mint(address mintAddress, string memory _tokenURI) public onlyNftRole returns(uint256) {
        ...
        _setData(mintAddress,idNft,_tokenURI);
...
//// function owner data
    ...
    function _setData (address owner, uint256 tokenid, string memory uri) internal{
        _owned memory asd = _owned({tokenID:tokenid,uriNFT:uri});
        ownerlist[owner].push(asd);
        countertracker[owner][tokenid] = (ownerlist[owner].length-1);
    }

    function getData(address owner) view public onlyNftRole returns(_owned[] memory){
        require (balanceOf(owner)>0,"this address doesn't own NFT");
        return ownerlist[owner];
    }

    function _burnData(address owner, uint256 tokenid)internal{
        uint256 arrayID = countertracker[owner][tokenid];
        delete ownerlist[owner][arrayID];
        delete countertracker[owner][tokenid];
    }

    function _transferData(address from,address to, uint256 tokenid)internal{
        uint256 arrayID = countertracker[from][tokenid];
        _setData(to,ownerlist[from][arrayID].tokenID,ownerlist[from][arrayID].uriNFT);
        delete ownerlist[from][arrayID];
        delete countertracker[from][tokenid];        
    }
}
...
contract Implementation {
    ...
    constructor(address accessControlAddress,address nftAddress) {
        accessControl = AccessControlContract(accessControlAddress);
        nft = NFT(nftAddress); 
    }
    ...
    function mint(address mintWallet, string memory uri) public onlyBackEndRole returns (uint256) {
    ...
    function getDataOwner (address owner) view public onlyBackEndRole returns(NFT._owned[] memory ){
        return nft.getData(owner);
    }
}