...
contract CMF is AccessControlEnumerable,Pausable,ICMF {
    ...
    * 获取充值记录
    ...
    function _upgrade(address owner,uint256 price) private{
        ...
        emit Print(owner,price,true);
        while(true){
            for(uint256 i=0;i<teamUpNum.length;i++){
                uint256 level = i+1;
                emit Print(curAddress,level,true);
                ...
                emit Print(curAddress,userRechargePrice[curAddress],true);
                emit Print(curAddress,userUpPrice[i],true);
                emit Print(curAddress,teamRechargePrice[curAddress],true);
                emit Print(curAddress,teamUpPrice[i],true);
                if(userRechargePrice[curAddress]<userUpPrice[i] || teamRechargePrice[curAddress]<teamUpPrice[i]){
                    break;
                }
                emit Print(curAddress,123,true);
                if(level>1){
                    emit Print(curAddress,1231,true);
                    getUserTeamLevelNum(curAddress,i);
                    emit Print(curAddress,1232,true);
                }
                emit Print(curAddress,userLevelNum[curAddress][i],true);
                emit Print(curAddress,userZhiValid[curAddress],true);
                emit Print(curAddress,teamUpNum[i],(level == 1 && userZhiValid[curAddress]>=teamUpNum[i]) || (level>1 && userLevelNum[curAddress][i]>=teamUpNum[i]));
                if((level == 1 && userZhiValid[curAddress]>=teamUpNum[i]) || (level>1 && userLevelNum[curAddress][i]>=teamUpNum[i])){
                    userLevel[curAddress] = level;
                     emit Print(curAddress,level,false);
                    //s6 最高分红
                    if(userLevel[curAddress] == 6){
                        levelMaxUser.push(curAddress);
                    }
                    emit Print(curAddress,1233,false);
                }
            }
            emit Print(curAddress,1111,true);
            parentAddress = userReferrer[curAddress];
            if (parentAddress == address(0)) {
                break;
            }
            emit Print(curAddress,1112,true);
            curAddress = parentAddress;
            teamRechargePrice[curAddress] += price;
            emit Print(curAddress,1113,true);
        }
    }
    /**
     * 获取直推有效人数
