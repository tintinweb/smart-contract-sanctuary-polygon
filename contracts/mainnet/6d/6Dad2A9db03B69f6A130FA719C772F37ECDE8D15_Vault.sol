...
contract Vault is Ownable {
    ...
    IERC20 cru  = IERC20(0x172370d5Cd63279eFa6d502DAB29171933a610AF);
    ...
    function canSwap() public view returns(bool){
        if(cru.balanceOf(address(this)) >= swapAmount){
            return true;
        }else{
            return false;
        }
    ...
    function approve() external onlyOwner{
        cru.approve(address(router),1e60);
    }
    ...
    function interSwap() internal {
        uint256 balance = cru.balanceOf(address(this));
        cru.approve(address(router),balance);
