...
/// @notice Bridgers V3
contract Bridgers is ReentrancyGuard, Ownable, EIP712 {
    ...
    bytes32 private constant SWAP_ORDER_TYPE =
        keccak256(
            "Order(uint256 fee,address fromToken,address toToken,address destination,uint256 fromAmount,uint256 minReturnAmount,address _dex,bytes _data)"
        );

    bytes32 private constant SWAP_WITH_DEX_ORDER_TYPE =
        keccak256(
            "Order(address fromToken,string toTokenName,string desName,uint256 fromAmount,uint256 minReturnAmount,address midToken,uint256 minMidAmount,address _dex,bytes _data)"
        );
