...
contract WTMChanEdition is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Royalty, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private gTokenIdCounter;
     
    uint256 private gSupplyMaximum;
    
    string public baseUri;
    string public baseExtension = ".json";

    address private gCreator;
 
    receive() external payable {}
    fallback() external payable {}

    constructor() ERC721("What the Meme Chan Edition", "WTM") {
        gSupplyMaximum = 10000;
        baseUri = "https://bafybeicjr73afv6n6d4hvwproddt4nocopqtiah5hk335bpcbzdbhjjt7u.ipfs.nftstorage.link/";
        gCreator = msg.sender;
        _setDefaultRoyalty(gCreator, 750);
        gTokenIdCounter.increment(); 
    }
 
    function batchMint(uint256 amount) external onlyOwner {
        require(gTokenIdCounter.current() + amount <= gSupplyMaximum, "Exceeded maximum supply");
        
        for (uint256 i = 0; i < amount; i++) {
            uint256 tokenId = gTokenIdCounter.current();
            gTokenIdCounter.increment();

            string memory mytokenURI = concatenate(tokenId);
    ...
    function getNumber()  public view returns(uint256) {
        return gTokenIdCounter.current();
    }
