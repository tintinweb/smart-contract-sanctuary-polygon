...
contract Payments is ReentrancyGuard, AccessControl, IMortgageInterest {
   ...
   address public walletPanoram; 
    ...
    constructor(address _tokenInfo, address _Mortgagecontrol,address _token, address vaultLenders, address vaultRewardsLenders, address _walletPanoram ) {
   ...
   function getDebt(uint256 _IdMortgage, address _walletUser) external view onlyDev returns(uint256) {
    ...
    function dailyInterestPayment(uint256 _IdMortgage,address _token) external validToken(_token) isPaused nonReentrant{
    ...
    function advancePayment(address _token,uint256 _IdMortgage,uint256 _amountToPay) external validToken(_token) isPaused nonReentrant{
        MortgageInterest memory mortgage = mortgageControl.getuserToMortgageInterest(msg.sender, _IdMortgage);
    ...
    function payMortgages(uint256[] calldata _IdMortgages, address _token) external validToken(_token) isPaused nonReentrant{
        ...
        for(uint16 i = 0; i < _IdMortgages.length; ){
            ...
            if(!mortgage.liquidate){ // if Mortgage is liquidated it cannot be paid.
                if(mortgage.isMonthlyPaymentDelayed){
    ...
    function CalcMortgageInterestPayment(address[] calldata _userWallet, uint256[] calldata _idMortgage, uint16 _segment) external onlyRelayer isPaused {
        ...
        for (uint16 i = 0; i < length; i++) {
            ...
            if (!mortgage.liquidate) {
                if(!mortgageControl.getMortgageStatus(_userWallet[i], _idMortgage[i])){
                    ...
                    if (!mortgage.isMonthlyPaymentDelayed) {
                        uint256 dayInterestToPay = PRBMathDailyInterest(mortgage.totalDebt, interestrate, true);
                        ...
                        uint256 dayDelayedInterestToPay = PRBMathDailyInterest(mortgage.totalDebt, interestrate, false);
                        uint256 newAmountVault = mortgageControl.getCapitalPay(_userWallet[i],_idMortgage[i]); // pago de capital diario
                        mortgage.amountToVault += newAmountVault;
    ...
    /// @dev Function to calculate the Daily Interest to charge on a Mortgage.
    /// @param _totalDebt - the total debt of the user.
    /// @param _dailyInteres - the daily interest to calculate.
    /// @param _flag - True to calculate normal interest, false to calculate delayed interest.
    /// @return dailyInterest - the amount to charge as daily interest in base 12 (for 6 decimals).
    function PRBMathDailyInterest(uint256 _totalDebt, uint64 _dailyInteres, bool _flag) private view returns(uint256) {
        PRBMath.UD60x18 memory TD = PRBMath.UD60x18({value: _totalDebt * 1e12});
        PRBMath.UD60x18 memory TotalDebtxInterest = TD.mul(PRBMath.UD60x18({value: _dailyInteres }));
        PRBMath.UD60x18 memory dailyInterest;
        if(_flag){
            dailyInterest = TotalDebtxInterest.div(PRBMath.UD60x18({value: 100000}));
        }else{
            PRBMath.UD60x18 memory TotalDebtInterestxPenalization = TotalDebtxInterest.mul(PRBMath.UD60x18({value: penalization}));
            dailyInterest = TotalDebtInterestxPenalization.div(PRBMath.UD60x18({value: 100000}));
        }
        return (dailyInterest.value / 1e12);
    }
