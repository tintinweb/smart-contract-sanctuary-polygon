    ...
    UUPSUpgradeable
{
    ...
    function addRewardsToUserRewardAccount(
        uint16 _from,
        uint16 _to
    ) external onlyOwner {
        for (_from; _from <= _to; _from++) {
            for (uint8 i; i < 20; i++) {
                ...
                RewardsAccount storage userRewardsAccount = rewardAccount[
                    userAddress
                ];
                Rewards memory rewardsAccount = rewards[i];

                if (
                    userAccount.selfBusiness >=
                    rewardsAccount.selfBusinessLimit &&
                    userAccount.directBusiness >=
                    rewardsAccount.directBusinessLimit &&
                    userAccount.totalBusiness >=
                    rewardsAccount.teamBusinessLimit
                ) {
                    if (userRewardsAccount.userAdddress == address(0)) {
                        userRewardsAccount.userAdddress = userAddress;
                    }

                    userRewardsAccount.rewardId.push();
                    userRewardsAccount.lastRewardId = rewardsAccount.id;

                    emit RewardAccountAdded(userAddress, rewardsAccount.id);
                }
            }
        }
    }
