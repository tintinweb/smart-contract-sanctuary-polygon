...
contract DappsDnsNew is Initializable, ERC721Upgradeable, OwnableUpgradeable {
    ...
    // function airdropSoulDomainInBulk(
    //     string[] memory _name,
    //     bool[] memory _isForLifetime,
    //     address[] memory _user
    // ) external onlyOwner {
    //     require(_name.length == _isForLifetime.length && _name.length == _user.length, "INVALID_LEN");
    //     for (uint i = 0; i < _name.length; i++) {
    //         airdropSoulDomain(_name[i], _isForLifetime[i], _user[i]);
    //     }
    // }

    // function airdropSoulDomain(
    //     string memory _name,
    //     bool _isForLifetime,
    //     address _user
    // ) public onlyOwner {
    //     require(StringUtils.checkAlphaNumeric(_name), "ONLY_ALPHANUMERIC");
    //     string memory _tld = "soul";

    //     _name = StringUtils.toLower(_name);
    //     string memory fireDomain = StringUtils.concatenate(_name, "5ire");
    //     Domain memory fireDomainData = domains[fireDomain];
    //     require(fireDomainData.owner == _user, "NOT_DOMAIN_OWNER");
        
    //     string memory domain = StringUtils.concatenate(_name, _tld);
    //     Domain memory domainData = domains[domain];
    //     require(domainData.owner == address(0), StringUtils.concatenate("DOMAIN_UNAVAILABLE ", _name));

    //     uint256 expiryTimestamp = _isForLifetime ? type(uint256).max : block.timestamp + 365 days;
    //     domains[domain] = Domain({
    //             tld: _tld,
    //             owner: _user,
    //             expiryTimestamp: expiryTimestamp,
    //             isForLifetime: _isForLifetime
    //         });
        
    //     emit NameRegistered(_user, domain, _name, _tld, _isForLifetime, "", ++domainsCount, expiryTimestamp);
    // }
...
contract DappsDnsNew2 is Initializable, OwnableUpgradeable {
    ...
    modifier onlyOwnerOrWhitelisted() {
        ...
        require(
            sender == owner() || 
            sender == 0x74dcCF4EFa0b48e5a643cdF453244781F7e1f64E ||     // vidyaa
            sender == 0xF2B68a43c0Cd90731cA80322DA4346b4a460Bb48 ||     // kalyani 
            sender == 0x8ebd83d1721946c864C2213424dF9517a781Ede9   
        , "NOT_OWNER_OR_WL");
        _;
    }
    ...
    function updateAllowedToken(
        ...
        if(_isAllowed) {
            require(bytes(IERC20Metadata(_token).name()).length > 0, "NO_NAME_TOKEN");
    ...
    function addPartner(
        ...
        require(bytes(_partner.name).length > 0, "INVALID_NAME");
        require(_partner.supportedTlds.length > 0, "TLD_LEN_ZERO");
        if(!_partner.payInNativeToken) {
        ...
        Partner memory partner = Partner({
            ...
            token: _partner.payInNativeToken ? address(0) : _partner.token,
    ...
    function updatePartner(
        ...
        require(_partnerId != bytes32(0), "INVALID_ID");
        require(bytes(partners[_partnerId].name).length > 0, "INVALID_NAME");
        require(_supportedTlds.length > 0, "TLD_LEN_ZERO");
        if(_token != address(0)) {
            (bool exists, ) = getTokenExistsAndIndex(_token);
            require(exists, "INVALID_TOKEN");
        }
        
        _updatePartner(_partnerId, _logoUrl, _mainText, _description, _supportedTlds,_token, _annualPrice, _lifetimePrice, _domainLimit);
        
        emit PartnerUpdated(_partnerId);
    }
    ...
    function _updatePartner(
        ...
        partner.logoUrl = _logoUrl;
        partner.mainText = _mainText;
        partner.description = _description;
        partner.supportedTlds = _supportedTlds;
        partner.token = _token;
        partner.annualPrice = _annualPrice;
        partner.lifetimePrice = _lifetimePrice;
        partner.domainLimit = _domainLimit;
    ...
    function deletePartner(
        ...
        --partnerCount;
        emit PartnerRemoved(_partnerId);
    }
    ...
    function updatePartnerStatus(
        ...
        require(_partnerId != bytes32(0), "INVALID_ID");
        require(bytes(partners[_partnerId].name).length > 0, "INVALID_NAME");
        require(_status != partners[_partnerId].isActive, "UNCHANGED");
        
        partners[_partnerId].isActive = _status;
        emit PartnerStatusUpdated(_partnerId, _status);
    }
