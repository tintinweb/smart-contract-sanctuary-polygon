...
contract NUTMARKET is ERC721URIStorage, Ownable, ReentrancyGuard {
  ...
  //events
  event MarketItemCreated(uint256 indexed tokenId, address seller, address indexed owner, uint256 indexed price, uint256 tokens, bool sold);
  ...
  function mintTokenNUTG(
    ...
    idToMarketItem[newTokenId] = MarketItem(newTokenId, payable(address(creator)), payable(msg.sender), payable(creator), price, tokens, true);

    //transfer token
    IERC20(tokenContract).transferFrom(msg.sender, creator, tokens);

    // emit asset creation event
    emit MarketItemCreated(newTokenId, creator, msg.sender, price, tokens, true);
  }
  ...
  /* allows someone to purchase a listed token */
  function buyTokenNUTG(uint256 tokenId, address tokenContract) public nonReentrant {
    uint256 tokens = idToMarketItem[tokenId].tokens;
    address seller = idToMarketItem[tokenId].seller;
    address previousOwner = idToMarketItem[tokenId].owner;

    require(tokens <= IERC20(tokenContract).balanceOf(msg.sender), 'not enough nutg tokens');
