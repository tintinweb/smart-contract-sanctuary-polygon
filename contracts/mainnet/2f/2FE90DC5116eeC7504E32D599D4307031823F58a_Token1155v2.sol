...
contract Token1155v2 is ERC1155Upgradeable, IERC2981Upgradeable, OwnableUpgradeable {
  ...
  /**
    ...
    @return recoupline リクープライン
   */
  function getShare(
    uint256 _tokenId
  ) external view returns (address[] memory, uint32[] memory, address payable, uint256){
    return(musics[_tokenId].stakeHolders, musics[_tokenId].share, musics[_tokenId].aggregator, calculateRecoupLine(_tokenId));
  }
