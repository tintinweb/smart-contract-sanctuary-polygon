...
contract PIXELATEDWEREFLOWERWIZA is ERC721 , DefaultOperatorFilterer {
    ...
    constructor()
        ERC721("Pixelated Wereflower Wiza", "PWW")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 101) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeiarba45fhig55cdzo4ny5jcrkkiwbk5sihkixdqrbczcp7xuaiq5a/", Strings.toString(tokenId), ".json")
            );
    }
