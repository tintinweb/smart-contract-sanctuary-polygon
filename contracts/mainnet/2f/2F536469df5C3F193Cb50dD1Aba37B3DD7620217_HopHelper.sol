...
interface IHopLpToken {
    function swap() external view returns (address);
}
//test
contract HopHelper is ITokenHelper {

    function getTokenData(address _strategy) external override view returns (TokenData[] memory data) {
        IHopLpToken token = IHopLpToken(IStrategy(_strategy).want());
        IHopPool pool = IHopPool(token.swap());

        data = new TokenData[](2);
        data[0].token = pool.getToken(0); //value the normal underlying token the same as the normal token, so I don't have to add special checks to the website
        data[0].decimals = ERC20(pool.getToken(0)).decimals();
        data[0].balance = pool.getTokenBalance(0);

        data[1].token = pool.getToken(0);
        data[1].decimals = ERC20(pool.getToken(1)).decimals();
        data[1].balance = pool.getTokenBalance(1);
    }
}
