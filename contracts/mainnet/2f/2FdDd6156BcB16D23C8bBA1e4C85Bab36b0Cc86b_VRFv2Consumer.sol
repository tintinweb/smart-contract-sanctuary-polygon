...
// File: contracts/chainlink/RandomWinnerChooserMatic.sol
...
/// @title A Random Prize Distributor Contract
contract VRFv2Consumer is VRFConsumerBaseV2, ConfirmedOwner {
    event RequestSent(uint256 requestId, uint32 requestedNums);

    address[] addresses;
    address[] uploadedSeriesAddresses;
    string[] rewards;
    ...
    modifier onlyIfRandomNumberGenerated {
      ...
      require(randomNum>0, "Random numbers not found");
      _;
    }
    ...
    {   
        require(addresses.length>0,"No addresses uploaded");
        require(rewards.length>0,"No rewards uploaded");
        require(addresses.length==rewards.length,"Reward and addresses length mismatch");
    ...
    function getWinningAllotment() external view onlyIfRandomNumberGenerated returns(address[] memory, string[] memory){
        require(addresses.length>0,"No addressess uploaded");
        require(processed,"Please call processLastRequest, before this");
        address[] memory _adds = new address[](addresses.length);
        string[] memory _rewards = new string[](rewards.length);
        for(uint i=0;i<addresses.length;i++){
            _adds[i] = addresses[i];
            _rewards[i] = rewards[i];
        }
        return(_adds, _rewards);
    }
    ...
    function processLastRequest() external onlyOwner onlyIfRandomNumberGenerated{
        require(addresses.length>0,"No addressess uploaded");
        require(addresses.length==rewards.length,"addresses, rewards length mismatch");
        for (uint256 i = 0; i < addresses.length; i++) {
            uint256 n = i + uint256(keccak256(abi.encodePacked(randomNum))) % (addresses.length - i);
    ...
    function getAddressedUploaded() external view returns(uint, address[] memory){
        require(uploadedSeriesAddresses.length>0,"No addressess uploaded");
        address[] memory _arr = new address[](uploadedSeriesAddresses.length);
        for(uint i=0;i<uploadedSeriesAddresses.length;i++){
           _arr[i]=uploadedSeriesAddresses[i];
        }
        return (uploadedSeriesAddresses.length,_arr);
    }


    ///@return the count of rewards uploaded 
    ///@return all the rewards uploaded
    function getRewardsUploaded() external view returns(uint, string[] memory){
        require(rewards.length>0,"No rewards uploaded");
        string[] memory _arr = new string[](rewards.length);
        for(uint i=0;i<rewards.length;i++){
           _arr[i]=rewards[i];
        }
        return (rewards.length,_arr);
    }
    ...
    function resetContract() external onlyOwner{
        ...
        uploadedSeriesAddresses = new address[](0);
        fulfilled=false;
        requested=false;
        processed=false;
        randomNum= 0;
    ...
    function uploadAddresses(address[] memory _arr) external onlyOwner{
        for(uint i=0;i<_arr.length;i++){
            ...
            uploadedSeriesAddresses.push(_arr[i]);
        }
    }
    ...
    ///@param _arr the array of addresses to upload eg. ["0x16992aC238aD27E97b95e54ae8367054b0ff7cCb","0x52fad22dE6b3A5cAC9Be0774c46113091a5C8252"]
    ///@notice can only be called by the owner
    function uploadRewards(string[] memory _arr) external onlyOwner{
        for(uint i=0;i<_arr.length;i++){
            rewards.push(_arr[i]);
        }
    }
