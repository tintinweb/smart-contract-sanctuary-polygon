...
contract MockUSDC is ERC20("Mock USDC", "USDC"), AccessProtected {
    ...
    function mint() public {
        ...
        if (!isAdmin(sender)) {
            require(!restriction, "MockUSDC: restriction mode");
            require(!hasMinted[sender], "MockUSDC: already minted");
    ...
    function setProtocolAddress(address _protocolAddress) public onlyAdmin {
        protocolAddresses[_protocolAddress] = true;
    }

    function setRestriction(bool _restriction) public onlyAdmin {
    ...
    function setMintAmount(uint256 _amount) external onlyAdmin {
        mintAmount = _amount;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (
            (from != address(0) && to != address(0)) && // neither mint nor burn
            (!protocolAddresses[from] && !protocolAddresses[to]) && // not from protocol or to protocol
            !isAdmin(from) // not from admin
        ) {
            // then don't allow the transfer unless whitelist
            require(!restriction, "MockUSDC: restriction mode");
            require(whitelist[from], "MockUSDC: non protocol transfers disabled");
        }
    }

    /// withdraw any token stuck in contract
