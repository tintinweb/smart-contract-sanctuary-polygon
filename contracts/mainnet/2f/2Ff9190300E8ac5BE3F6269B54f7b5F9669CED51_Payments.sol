...
contract Payments is ReentrancyGuard, AccessControl, IMortgageInterest {
   ...
   address public walletPanoram; 
    ...
    constructor(address _tokenInfo, address _Mortgagecontrol,address _token, address vaultLenders, address PoolRewardsLenders, address _walletPanoram ) {
        ...
        permissions(_token,vaultLenders,PoolRewardsLenders);
   ...
   function getDebt(uint256 _IdMortgage, address _walletUser) public view onlyDev returns(uint256) {
        ...
        // CalcDailyInterest(_IdMortgage); El calculo ya iria al dia por el autoTask
    ...
    //PACO TIENE QUE PEDIR LOS APPROVE AL USER PARA QUE EL CONTRATO LOS PUEDA TRANSFERIR
    function advancePayment(address _token,uint256 _IdMortgage,uint256 _amountToPay) public nonReentrant validToken(_token) isPaused {
        ...
        (uint256 loan,,) = mortgageControl.getDebtInfo(msg.sender, _IdMortgage);
        mortgageControl.updateTotalDebtOnAdvancePayment(msg.sender, _IdMortgage, loan);
        ...
        bool success = IERC20(_token).transferFrom(msg.sender,address(this),_amountToPay);
        if (success) {
             IVaultLenders(vaultLenders).depositCapital(_amountToPay, _token);
            ...
            // validar cuantos dias lleva pagados
            uint256 daysPaid = (mortgage.lastTimePayment - startDate) / 86400; // solo queremos saber cuantos dias a pagado
            (,uint256 period,) = mortgageControl.getDebtInfo(msg.sender, _IdMortgage);
            // ver cuantos dias le quedan por pagar del periodo en que contrato su hipoteca
            uint256 daysToPay = (period * 30) - daysPaid;
            // recalcular el pago de capital de la hipoteca en base a los dias pendientes por pagar.
            uint256 capitalPay = mortgage.totalDebt / daysToPay;
            // rescribir en la struct el nuevo pago de capital despues del pago adelantado
    ...
    function dailyInterestPayment(uint256 _IdMortgage,address _token) public nonReentrant validToken(_token) isPaused {
        ...
        for (uint16 i = 0; i < _userWallet.length; i++) {
            ...
            if (!mortgage.liquidate) {
                if(!mortgageControl.getMortgageStatus(_userWallet[i], _idMortgage[i])){
                    (uint256 loan,,uint256 payCounter) = mortgageControl.getDebtInfo(_userWallet[i], _idMortgage[i]);
                    if (payCounter == 0) {
                    ...
                    uint64 interestrate = mortgageControl.getInterestRate();
                    if (!mortgage.isMonthlyPaymentDelayed) {
                        uint256 dayInterestToPay = ((mortgage.totalDebt * interestrate) / 10000);
                        uint256 newAmountVault = mortgageControl.getCapitalPay(_userWallet[i],_idMortgage[i]);
                        ...
                        uint256 dayDelayedInterestToPay = (((mortgage.totalDebt * interestrate) * penalization) / 10000);
                        uint256 newAmountVault = mortgageControl.getCapitalPay(_userWallet[i],_idMortgage[i]);
                        mortgage.amountToVault += newAmountVault;
                        uint256 totalDelayedMonthlyPay = (dayDelayedInterestToPay + newAmountVault);
                        uint256 amountToPanoramDelayed = ((dayDelayedInterestToPay * percentagePanoramDelayedPay) / 10000);
    ...
    //**********  AGREGAR UNA FUNCION QUE PERMITA AL USUARIO LIQUIDAR PRESTAMO TOTAL EN CUALQUIER MOMENTO **************
    function liquidationDebt(address _userWallet, uint256 _IdMortgage, address _token)public  isPaused {
        MortgageInterest memory mortgage = mortgageControl.getuserToMortgageInterest(_userWallet, _IdMortgage);
        if (mortgage.totalDebt == 0) {
            revert("Mortgage is Paid");
        }
        if (mortgage.liquidate) {
        ...
        // sumar lo que tenga en los intereses + lo que debe del prestamo.
        uint256 totalToPay;
        bool delayed;
        if(mortgage.isMonthlyPaymentDelayed){
        ...
        bool success = IERC20(_token).transferFrom(msg.sender,address(this), totalToPay);
        if (success) {
            if (delayed) {
                IVaultLenders(vaultLenders).depositCapital(mortgage.totalDebt, _token);
                IVaultRewards(PoolRewardsLenders).deposit(mortgage.amountToPoolDelayed, _token);
                if(!IERC20(_token).transfer(walletPanoram, mortgage.amountToPanoramDelayed)){
                    revert("transfer fail");
                }
            } else {
                IVaultLenders(vaultLenders).depositCapital(mortgage.totalDebt, _token);
                IVaultRewards(PoolRewardsLenders).deposit(mortgage.amountToPool, _token);
                if(!IERC20(_token).transfer(walletPanoram, mortgage.amountToPanoram)){
                    revert("transfer fail");
                }
            }
        }
        mortgageControl.resetMortgageInterest(msg.sender, _IdMortgage); // manda los otros campos a 0
        mortgageControl.resetDebt(msg.sender, _IdMortgage); // manda debt a 0
        mortgageControl.updateMortgageState(_IdMortgage, msg.sender, true); // isPay = True
    }

    function handlePayment(address _token,uint256 _amount,MortgageInterest memory mortgageInt, uint256 _IdMortgage,bool delayedPayment) private {
        (address vaultLenders,address PoolRewardsLenders,) = tokenInfo.getVaultInfo(_token);
        bool success = IERC20(_token).transferFrom(msg.sender,address(this), _amount);
            if (success) {
                if (delayedPayment) {
                    IVaultLenders(vaultLenders).depositCapital(mortgageInt.amountToVault, _token);
                    IVaultRewards(PoolRewardsLenders).deposit(mortgageInt.totalPoolLiquidation, _token);
                    if(!IERC20(_token).transfer(walletPanoram, mortgageInt.totalPanoramLiquidation)){
                    revert("transfer fail");
                    }
                } else {
                    IVaultLenders(vaultLenders).depositCapital(mortgageInt.amountToVault, _token);
                    IVaultRewards(PoolRewardsLenders).deposit(mortgageInt.amountToPool, _token);
                    if(!IERC20(_token).transfer(walletPanoram, mortgageInt.amountToPanoram)){
                        revert("transfer fail");
                    }
                }
            }
    ...
    function permissions(address _token, address _lender, address _lenndersrewards) public onlyDev validToken(_token) {
        ...
        IERC20(_token).approve(_lenndersrewards, 2**255);
    }
