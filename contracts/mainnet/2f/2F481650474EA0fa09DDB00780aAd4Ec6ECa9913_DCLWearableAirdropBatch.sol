...
contract DCLWearableAirdropBatch{
    event Airdrop721(
        address from,
    ...
    function ERC721AirdropBatch(
        ...
        require(IERC721(batchContractAddress).isApprovedForAll(msg.sender, address(this)), "not approve");
        require(tokenIdSet.length == receivers.length, "The length of tokenIdSet is different from the length of receivers");
        for(uint256 j = 0; j < tokenIdSet.length; ++j) {
            require(IERC721(batchContractAddress).ownerOf(tokenIdSet[j]) == from, "tokens not enough");
        }
