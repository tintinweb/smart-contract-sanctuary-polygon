...
contract NomisScore is ERC721, ERC721Burnable, Ownable, Pausable, EIP712 {
    ...
    */
    constructor(uint256 initialFee) ERC721("NomisScore", "NMSS") EIP712("NMSS", "0.2") {
        _tokenIds.reset();
        _mintFee = initialFee;
    }
    ...
    function setScore(
        ...
        Score storage scoreStruct = scores[msg.sender];
        if (scores[msg.sender].value != score) {
        ...
        uint256 tokenId;
        if (scores[msg.sender].updated != 0) {
            if (scores[msg.sender].value != score) {
        ...
        emit ScoreChanged(tokenId, msg.sender, score, calculationModel);
    }
    ...
    */
    function getScore(address addr) external view returns(uint16 score, uint256 updated, uint256 tokenId, uint16 calculationModel) {
        require(scores[addr].updated != 0, "getScore: No score found for the specified address");
