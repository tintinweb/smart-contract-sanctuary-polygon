...
contract StakingContract is Ownable {
    ...
    uint256 public initialReward = 50 * (10 ** 9);
    uint256 public rewardRaiseStep = 5 * (10 ** 9);
    uint256 public maxReward = 70 * (10 ** 9);
    ...
    function pendingReward(address _user, uint256 _tokenId) public view returns (uint256) {
        ...
        for (day = 0; day < spendDay; day ++){
            if (day > 4) {
                rewardAmount = rewardAmount + initialReward.add(rewardRaiseStep.mul(4));
            } else {
                rewardAmount = rewardAmount + initialReward.add(rewardRaiseStep.mul(day));
            }
        }
        if (day > 4) {
            rewardAmount = rewardAmount + initialReward.add(rewardRaiseStep.mul(4)).mul(remainTime).div(TIME_STEP);
        } else {
            rewardAmount = rewardAmount + initialReward.add(rewardRaiseStep.mul(day)).mul(remainTime).div(TIME_STEP);
        }
