...
contract XFUCK is ERC20, ERC20Burnable, Operator, ReentrancyGuard, AccessControl, Taxable {
 ...
 bytes32 public constant GOVERNOR_ROLE = keccak256("GOVERNOR_ROLE");
     ...
     constructor( address _multisig, address _treasury, bool taxed_, uint default_tax_, uint maxtax_, uint mintax_)
        ERC20("XFUCK", "XXFUCK")
        Taxable(taxed_, default_tax_, maxtax_, mintax_, _treasury)
        payable
    {
        _grantRole(DEFAULT_ADMIN_ROLE, _multisig);
        _grantRole(GOVERNOR_ROLE, _treasury);
        _grantRole(PRESIDENT_ROLE, _multisig);
        _grantRole(EXCLUDER_ROLE, _treasury);
        _grantRole(EXCLUDED_ROLE, msg.sender);
        _grantRole(EXCLUDED_ROLE, _multisig);
        _grantRole(EXCLUDED_ROLE, _treasury);
        _mint(_treasury, 10000 * 10 ** decimals()); // Mint 10,000 $XFUCK to treasury hot wallet.
    }
...
address router = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; //Quickswap router
     ...
     function _transfer(address from, address to, uint256 amount) // Overrides the _transfer() function to use an optional transfer tax.
            ...
            } else {  super._transfer(from, to, amount); } // Transfers 100% of amount to recipient.
