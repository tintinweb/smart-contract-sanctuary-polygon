...
abstract contract GameRole is Context {
    ...
    modifier onlyCertified() {
        require(_isCertified[_msgSender()], "GameRole: caller is not certified");
        _;
    }
    ...
    function removeCertified(address certified) public onlyOwner {
        require(_isCertified[certified], "GameRole: this address is not Certified");
...
abstract contract GameBase is GameRole, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        uint256 getLevel = _gemLevel(randomness);
        if (gem.isLevelFixed) {
            gem.level = getLevel > gem.level ? uint(0) : getLevel;
        } else {
            gem.level = getLevel;
        }

        gem.color = randomness % 6;
...
contract Game is GameBase {
    constructor(address gem, address member, address rand) {
