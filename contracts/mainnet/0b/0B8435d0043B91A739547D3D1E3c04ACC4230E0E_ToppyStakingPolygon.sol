...
import "./BEP20.sol";
import "./IToppyPolygonPool.sol";

contract ToppyStakingPolygon is Ownable, ReentrancyGuard {
    ...
    IToppyPolygonPool toppyPolygonPool;
    ...
    ) public onlyOwner
    {
        ...
        toppyPolygonPool = IToppyPolygonPool(_toppyPool);
    ...
    function updateSmartContract(address _toppyMaster, address _toppyMint) public onlyOwner {
    ...
    function allPendingRewards(uint _pid) external view returns (uint) {
        
        uint totalRewards = 0;
        PoolInfo memory pool = poolInfo[_pid];
        uint ownerTokenCount = IERC721(pool.nftToken).balanceOf(msg.sender);
        for (uint i=0; i < ownerTokenCount; i++) {
            uint tokenId = IERC721Enumerable(pool.nftToken).tokenOfOwnerByIndex(msg.sender, i);
            totalRewards = totalRewards + pendingRewards(_pid, tokenId);
        }
        return totalRewards;
    }

    // View function to see pending CAKEs on frontend.
    function pendingRewards(uint _pid, uint _tokenId) public view returns (uint) {
        PoolInfo memory pool = poolInfo[_pid];
    ...
    function updatePool(uint _pid) public {
        ...
        // harvest from xwin farm in bsc, but polygon get from the toppypool
        toppyPolygonPool.getPendingRewards(cakeReward);
    ...
    function harvestAll(uint _pid) public nonReentrant payable{
        PoolInfo memory pool = poolInfo[_pid];
        uint ownerTokenCount = IERC721(pool.nftToken).balanceOf(msg.sender);
        for (uint i; i < ownerTokenCount; i++) {
            _harvest(_pid, IERC721Enumerable(pool.nftToken).tokenOfOwnerByIndex(msg.sender, i));
        }
    }

    function stakeAll(uint _pid) public nonReentrant payable{
        ...
        for (uint i=0; i < ownerTokenCount; i++) {
            uint tokenId = IERC721Enumerable(pool.nftToken).tokenOfOwnerByIndex(msg.sender, i);
            bytes32 hashedKey = _getId(pool.nftToken, tokenId);
            UserInfo memory user = userInfo[_pid][hashedKey];
            if(user.amount == 0){
                _stake(msg.sender, _pid, tokenId);
            }
        }
    ...
    function harvest(uint _pid, uint _tokenId) public nonReentrant payable{
        _harvest(_pid, _tokenId);
    }

    // harvest to get xwin as interest for nft owner
    function _harvest(uint _pid, uint _tokenId) internal {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                uint bal = BEP20(rewardsToken).balanceOf(address(this));
                require(bal >= pending, "Insufficient XWin Tokens to Withdraw");
...
interface IToppyPolygonPool {

    function getPendingRewards(uint _amount) external;
