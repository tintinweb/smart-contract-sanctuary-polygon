...
contract DecadeExchange is OwnableUpgradeable {
    ...
    event Executed(uint indexed askId, uint indexed bidId, uint askOut, uint bidOut);
    ...
    function execute(LibTrade.Execution[] memory chunk) external onlyWhitelisted {
        ...
        for (uint i = 0; i < chunk.length; i++) {
            ...
            emit Executed(exec.askId, exec.bidId, _amountQ(exec.base, exec.quote, exec.amount - reserveAsk - txCost, exec.price), exec.amount - reserveBid - txCost);
        }
    }
