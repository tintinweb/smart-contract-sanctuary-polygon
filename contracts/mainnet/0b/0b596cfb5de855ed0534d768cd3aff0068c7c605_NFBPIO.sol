...
// File contracts/NFBPIO.sol


pragma solidity 0.8.9;

contract NFBPIO is ERC721A, Ownable, ReentrancyGuard, ControlledAccess {
    using Strings for uint256; 
    ...
    string public hiddenMetadataUri = "https://nft.tudabirds.io/api/nfbpio/hidden";
    ...
    constructor() ERC721A( "tudaBirds NFB Pioneer", "NFBPIO", maxMintPerAddress, 9000) {
    }
    ...
    /** Modifier - ensures the function caller is the allowed packer contract */
    modifier isPackerContract() {
        require(packerContract == msg.sender, "Caller is not allowed packer contract");
        _;
    }
    ...
    function mintFromPacker(uint256 quantity, address to)
        external
        isPackerContract
        nonReentrant
    ...
    function setPackerContract(address _packerContract) external onlyOwner nonReentrant {
         packerContract = _packerContract;
    }

    function setTotalSize(uint256 _totalSize) external onlyOwner nonReentrant {
         totalSize = _totalSize;
    }

    function setMaxMintPerTx(uint256 _maxMintPerTx) external onlyOwner nonReentrant {
    ...
    function withdraw() external onlyOwner nonReentrant {
        ...
        address srv = payable(owner()); 

        bool success;
        (success, ) = srv.call{value: ((sendAmount * 100) / 100)}("");
