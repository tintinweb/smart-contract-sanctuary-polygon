...
contract StatController is Controllable, IStatController {
  ...
  mapping(address => mapping(uint => EnumerableMap.UintToIntMap)) internal _heroTotalAttributes;
  mapping(address => mapping(uint => CoreAttributes)) internal _heroCore;
  mapping(address => mapping(uint => EnumerableMap.UintToIntMap)) internal _heroBonusAttributes;
  mapping(address => mapping(uint => EnumerableMap.UintToIntMap)) internal _heroTemporallyAttributes;
  mapping(address => mapping(uint => ChangeableStats)) internal _heroStats;
  /// @dev hero => herod id => item slot => item
  mapping(address => mapping(uint => mapping(uint => NftItem))) internal _heroSlots;
  mapping(address => mapping(uint => EnumerableSet.UintSet)) internal _heroBusySlots;
  mapping(address => mapping(uint => EnumerableSet.AddressSet)) internal _usedConsumables;
  mapping(address => mapping(uint => mapping(bytes32 => uint))) internal _heroCustomData;
  mapping(bytes32 => uint) internal _globalCustomData;
  ...
  event NewHeroInited(address heroToken, uint heroTokenId, ChangeableStats stats);
  event LevelUp(
    address heroToken,
    uint heroTokenId,
    uint heroClass,
  ...
  function onlyRegisteredContract() internal view {
    IController _controller = IController(controller());
    require(
      _controller.validHeroes(msg.sender)
      || _controller.validItems(msg.sender)
      || _controller.validDungeons(msg.sender)
      || _controller.storyController() == msg.sender
      || IChamberController(_controller.chamberController()).validChambers(msg.sender)
      ,
      "SC: forbidden");
  }

  function onlyItem() internal view {
    require(IController(controller()).validItems(msg.sender), "Not item");
  }

  function onlyItemOrHero() internal view {
    require(IController(controller()).validItems(msg.sender)
    || IController(controller()).validHeroes(msg.sender), "Not item or hero");
  }

  function onlyHero() internal view {
    require(IController(controller()).validHeroes(msg.sender), "Not hero");
  }
  ...
  function heroAttributes(address token, uint tokenId) external view override returns (uint[] memory) {
    return _heroTotalAttributes[token][tokenId].flat();
  }

  function heroAttributesInt(address token, uint tokenId) external view returns (int[] memory) {
    return _heroTotalAttributes[token][tokenId].flatInt();
  }

  function heroBonusAttributes(address token, uint tokenId) external view returns (int[] memory) {
    return _heroBonusAttributes[token][tokenId].flatInt();
  }

  function heroTemporallyAttributes(address token, uint tokenId) external view returns (int[] memory) {
    return _heroTemporallyAttributes[token][tokenId].flatInt();
  }

  function heroAttributesLength(address token, uint tokenId) external view override returns (uint) {
    return _heroTotalAttributes[token][tokenId].length();
  }

  function heroAttribute(address token, uint tokenId, uint index) external view override returns (uint) {
    return _heroTotalAttributes[token][tokenId].getOrZero(index);
  }

  function heroBaseAttributes(address token, uint tokenId) external view override returns (CoreAttributes memory) {
    return _heroCore[token][tokenId];
  }

  function heroCustomData(address token, uint tokenId, bytes32 index) external view override returns (uint) {
    return _heroCustomData[token][tokenId][index];
  }

  function globalCustomData(bytes32 index) external view override returns (uint) {
    return _globalCustomData[index];
  }
  ...
  function heroItemSlots(address heroToken, uint heroTokenId) external view override returns (uint[] memory) {
    return _heroBusySlots[heroToken][heroTokenId].values();
  }
  ...
  function _isItemTypeEligibleToItemSlot(uint itemType, uint itemSlot) internal pure returns (bool) {
    ...
    if (itemType == uint(IItem.ItemType.OFF_HAND)) {
      return itemSlot == uint(ItemSlots.LEFT_HAND);
    }
  ...
  function isConsumableUsed(address heroToken, uint heroTokenId, address item) external view returns (bool) {
    return _usedConsumables[heroToken][heroTokenId].contains(item);
  }
  ...
  function buffHero(BuffInfo memory info) external view override returns (uint[] memory, uint) {
    uint length = info.buffTokens.length;
    if (length == 0) {
      return (_heroTotalAttributes[info.heroToken][info.heroTokenId].flat(), 0);
    }
    uint[] memory buffAttributes = new uint[](uint(ATTRIBUTES.END_SLOT));
    uint manaSum;
    for (uint i; i < length;) {
      // checked in fight calculator
//      require(IController(controller()).validItems(info.buffTokens[i]), "Item not registered");
//      require(IItem(info.buffTokens[i]).equippedOn(info.buffTokenIds[i], info.heroToken) == info.heroTokenId, "Not your buff item");

      (uint[] memory buff, uint[] memory debuff, uint manaConsume) = IBuffItem(info.buffTokens[i]).buff(info.buffTokenIds[i]);
      StatLib.attributesAdd(buffAttributes, buff);
      StatLib.attributesRemove(buffAttributes, debuff);
      manaSum += manaConsume;

      unchecked{++i;}
    }

    uint[] memory totalAttributes = StatLib.attributesAdd(_heroTotalAttributes[info.heroToken][info.heroTokenId].flat(), buffAttributes);
    StatLib.attributesAdd(buffAttributes, _heroBonusAttributes[info.heroToken][info.heroTokenId].flat());
    StatLib.attributesAdd(buffAttributes, _heroTemporallyAttributes[info.heroToken][info.heroTokenId].flat());
    return (
      StatLib.updateCoreDependAttributesInMemory(totalAttributes, buffAttributes, IHero(info.heroToken).heroClass(), info.heroLevel),
      manaSum
    );
  }
  ...
  function initNewHero(address heroToken, uint heroTokenId, uint heroClass) external override {
    ...
    CoreAttributes memory initialCore = heroClass.initialHero().core;

    _heroCore[heroToken][heroTokenId] = initialCore;

    EnumerableMap.UintToIntMap storage totalAttributes = _heroTotalAttributes[heroToken][heroTokenId];
    StatLib.initAttributes(totalAttributes, heroClass, 1, initialCore);

    ChangeableStats memory stats = ChangeableStats({
      level: 1,
      experience: 0,
      life: totalAttributes.getOrZeroA(ATTRIBUTES.LIFE),
      mana: totalAttributes.getOrZeroA(ATTRIBUTES.MANA),
      lifeChances: totalAttributes.getOrZeroA(ATTRIBUTES.LIFE_CHANCES)
    });
    _heroStats[heroToken][heroTokenId] = stats;
    emit NewHeroInited(heroToken, heroTokenId, stats);

    // set initial karma
    bytes32 karmaIndex = bytes32("KARMA");
    _heroCustomData[heroToken][heroTokenId][karmaIndex] = 1000;
    emit HeroCustomDataChanged(heroToken, heroTokenId, karmaIndex, 1000);

    // set hero class as parameter for stories
    bytes32 heroClassIndex = bytes32("HERO_CLASS");
    _heroCustomData[heroToken][heroTokenId][heroClassIndex] = heroClass;
    emit HeroCustomDataChanged(heroToken, heroTokenId, heroClassIndex, heroClass);
  }
  ...
  function changeHeroItemSlot(
    ...
    mapping(uint => NftItem) storage slots = _heroSlots[heroToken][heroTokenId];
    if (itemSlot == uint(ItemSlots.TWO_HAND)) {
      require(slots[uint(ItemSlots.RIGHT_HAND)].token == address(0)
      && slots[uint(ItemSlots.LEFT_HAND)].token == address(0), "busy");
    }

    NftItem memory currentItem = slots[itemSlot];
    if (equip) {
      require(currentItem.token == address(0), "Item slot busy");
      slots[itemSlot] = NftItem(itemToken, itemTokenId);
      require(_heroBusySlots[heroToken][heroTokenId].add(itemSlot), "Error slot");
    } else {
      require(currentItem.token == itemToken && currentItem.tokenId == itemTokenId, "Item not in slot");
      slots[itemSlot] = NftItem(address(0), 0);
      require(_heroBusySlots[heroToken][heroTokenId].remove(itemSlot), "Error slot");
    }
  ...
  function changeCurrentStats(
    ...
    EnumerableMap.UintToIntMap storage attrs = _heroTotalAttributes[heroToken][heroTokenId];
    uint maxLife = attrs.getUint(uint(ATTRIBUTES.LIFE));
    uint maxMana = attrs.getUint(uint(ATTRIBUTES.MANA));
    uint maxLC = attrs.getUint(uint(ATTRIBUTES.LIFE_CHANCES));
    ...
    uint lifeChances = currentStats.lifeChances;
    if (increase) {
      experience += change.experience;
      life = Math.min(maxLife, (life + change.life));
      mana = Math.min(maxMana, (mana + change.mana));
      lifeChances = Math.min(maxLC, (lifeChances + change.lifeChances));
    } else {
      ...
      require(lifeChances >= change.lifeChances, "Not enough chances");
      life = life - change.life;
      lifeChances = lifeChances - change.lifeChances;
  ...
  function registerConsumableUsage(address heroToken, uint heroTokenId, address item) external override {
    ...
    require(_usedConsumables[heroToken][heroTokenId].add(item), "used");
    emit ConsumableUsed(heroToken, heroTokenId, item);
  }

  function clearUsedConsumables(address heroToken, uint heroTokenId) external override {
    onlyRegisteredContract();

    EnumerableSet.AddressSet storage items = _usedConsumables[heroToken][heroTokenId];

    uint length = items.length();

    for (uint i; i < length; ++i) {
      address item = items.at(0);
      require(items.remove(item), "wrong");
      emit RemoveConsumableUsage(heroToken, heroTokenId, item);
    }
  }
  ...
  function changeBonusAttributes(ChangeAttributesInfo memory info) external override {
    ...
    ChangeableStats storage stats = _heroStats[info.heroToken][info.heroTokenId];
    EnumerableMap.UintToIntMap storage totalAttributes = _heroTotalAttributes[info.heroToken][info.heroTokenId];
    EnumerableMap.UintToIntMap storage bonusMain;
    EnumerableMap.UintToIntMap storage bonusExtra;
    if (info.temporally) {
      bonusMain = _heroTemporallyAttributes[info.heroToken][info.heroTokenId];
      bonusExtra = _heroBonusAttributes[info.heroToken][info.heroTokenId];
    } else {
      bonusMain = _heroBonusAttributes[info.heroToken][info.heroTokenId];
      bonusExtra = _heroTemporallyAttributes[info.heroToken][info.heroTokenId];
    }
    int[] memory baseValues = new int[](info.changeAttributes.length);
    for (uint i; i < info.changeAttributes.length; ++i) {
      if (info.changeAttributes[i] != 0) {
        int newTotalValue;
        if (info.increase) {
          bonusMain.increment(i, info.changeAttributes[i]);
          newTotalValue = totalAttributes.getOrZeroInt(i) + int(info.changeAttributes[i]);
        } else {
          bonusMain.decrement(i, int(info.changeAttributes[i]));
          newTotalValue = totalAttributes.getOrZeroInt(i) - int(info.changeAttributes[i]);
        }
        if (newTotalValue == 0) {
          totalAttributes.remove(i);
        } else {
          totalAttributes._set(i, newTotalValue);
        }
        baseValues[i] = newTotalValue;
      }
    }

    // handle core depend attributes in the second loop
    for (uint i; i < baseValues.length; ++i) {
      if (baseValues[i] != 0) {
        StatLib.updateCoreDependAttributes(totalAttributes, bonusMain, bonusExtra, stats, i, info.heroToken, baseValues[i].toUint());
      }
    }

    if (!info.increase) {
      _checkStats(info.heroToken, info.heroTokenId, stats);
    }
    emit BonusAttributesChanged(
      info.heroToken,
      info.heroTokenId,
      info.increase,
      info.temporally,
  ...
  function _checkStats(address heroToken, uint heroTokenId, ChangeableStats storage stats) internal {
    EnumerableMap.UintToIntMap storage totalAttributes = _heroTotalAttributes[heroToken][heroTokenId];
    uint life = totalAttributes.getOrZeroA(ATTRIBUTES.LIFE);
    uint mana = totalAttributes.getOrZeroA(ATTRIBUTES.MANA);
    if (life < stats.life) {
      stats.life = Math.min(life, stats.life);
    }
    if (mana < stats.mana) {
      stats.mana = Math.min(mana, stats.mana);
    }
  }
  ...
  function clearTemporallyAttributes(address heroToken, uint heroTokenId) external override {
    ...
    EnumerableMap.UintToIntMap storage tmpBonuses = _heroTemporallyAttributes[heroToken][heroTokenId];
    uint length = tmpBonuses.length();

    if (length != 0) {
      ChangeableStats storage stats = _heroStats[heroToken][heroTokenId];
      EnumerableMap.UintToIntMap storage bonus = _heroBonusAttributes[heroToken][heroTokenId];
      EnumerableMap.UintToIntMap storage totalAttributes = _heroTotalAttributes[heroToken][heroTokenId];

      uint[] memory indexes = new uint[](length);
      int[] memory baseValues = new int[](length);
      for (uint i; i < length; ++i) {
        (uint index, int value) = tmpBonuses._at(i);
        if (value != 0) {
          baseValues[i] = totalAttributes.decrement(index, value);
          indexes[i] = index;
        }
      }

      // need to clear struct separately for avoid data corruption
      for (uint i = 0; i < length; ++i) {
        tmpBonuses.remove(indexes[i]);
      }

      // remove bonuses in the third loop
      for (uint i; i < length; ++i) {
        if (baseValues[i] != 0) {
          StatLib.updateCoreDependAttributes(totalAttributes, bonus, tmpBonuses, stats, indexes[i], heroToken, baseValues[i].toUint());
        }
      }

      _checkStats(heroToken, heroTokenId, stats);
  ...
  function levelUp(
    ...
    ChangeableStats storage currentStats = _heroStats[heroToken][heroTokenId];

    require(currentStats.level < StatLib.MAX_LEVEL, "Max level");
    require(currentStats.level.levelExperience() <= currentStats.experience, "Not enough exp");
    currentStats.level++;

    CoreAttributes memory attributes = _heroCore[heroToken][heroTokenId];

    _heroCore[heroToken][heroTokenId] = CoreAttributes({
    ...
    // update
    _addCoreToTotal(change, heroToken, heroTokenId);

    // setup new level and restore life/mana
    EnumerableMap.UintToIntMap storage totalAttributes = _heroTotalAttributes[heroToken][heroTokenId];
    currentStats.life = totalAttributes.getOrZeroA(ATTRIBUTES.LIFE);
    currentStats.mana = totalAttributes.getOrZeroA(ATTRIBUTES.MANA);
  ...
  function _addCoreToTotal(CoreAttributes memory change, address heroToken, uint heroTokenId) internal {
    EnumerableMap.UintToIntMap storage totalAttributes = _heroTotalAttributes[heroToken][heroTokenId];
    EnumerableMap.UintToIntMap storage bonus = _heroBonusAttributes[heroToken][heroTokenId];
    EnumerableMap.UintToIntMap storage bonusTmp = _heroTemporallyAttributes[heroToken][heroTokenId];
    IStatController.ChangeableStats storage stats = _heroStats[heroToken][heroTokenId];

    if (change.strength != 0) {
      uint cur = totalAttributes.getOrZeroA(ATTRIBUTES.STRENGTH);
      totalAttributes.setUint(uint(ATTRIBUTES.STRENGTH), cur + change.strength);
      StatLib.updateCoreDependAttributes(totalAttributes, bonus, bonusTmp, stats, uint(ATTRIBUTES.STRENGTH), heroToken, cur + change.strength);
    }
    if (change.dexterity != 0) {
      uint cur = totalAttributes.getOrZeroA(ATTRIBUTES.DEXTERITY);
      totalAttributes.setUint(uint(ATTRIBUTES.DEXTERITY), cur + change.dexterity);
      StatLib.updateCoreDependAttributes(totalAttributes, bonus, bonusTmp, stats, uint(ATTRIBUTES.DEXTERITY), heroToken, cur + change.dexterity);
    }
    if (change.vitality != 0) {
      uint cur = totalAttributes.getOrZeroA(ATTRIBUTES.VITALITY);
      totalAttributes.setUint(uint(ATTRIBUTES.VITALITY), cur + change.vitality);
      StatLib.updateCoreDependAttributes(totalAttributes, bonus, bonusTmp, stats, uint(ATTRIBUTES.VITALITY), heroToken, cur + change.vitality);
    }
    if (change.energy != 0) {
      uint cur = totalAttributes.getOrZeroA(ATTRIBUTES.ENERGY);
      totalAttributes.setUint(uint(ATTRIBUTES.ENERGY), cur + change.energy);
      StatLib.updateCoreDependAttributes(totalAttributes, bonus, bonusTmp, stats, uint(ATTRIBUTES.ENERGY), heroToken, cur + change.energy);
    }
  }
  ...
  function generateMonsterAttributes(
    ...
    uint baseExperience
  ) external pure override returns (uint[] memory attributes, uint experience) {

    // reduce random
    amplifier = amplifier / 4;

    attributes = new uint[](uint(ATTRIBUTES.END_SLOT));
    for (uint i; i < ids.length;) {
      attributes[ids[i]] = StatLib.amplify(values[i], amplifier, dungeonMultiplier);
      unchecked{++i;}
    }
    experience = StatLib.amplify(baseExperience, amplifier, dungeonMultiplier);
  }

  function setHeroCustomData(address token, uint tokenId, bytes32 index, uint value) external override {
    onlyRegisteredContract();

    _heroCustomData[token][tokenId][index] = value;

    emit HeroCustomDataChanged(token, tokenId, index, value);
  }

  function setGlobalCustomData(bytes32 index, uint value) external override {
    onlyRegisteredContract();

    _globalCustomData[index] = value;

    emit GlobalCustomDataChanged(index, value);
  }
