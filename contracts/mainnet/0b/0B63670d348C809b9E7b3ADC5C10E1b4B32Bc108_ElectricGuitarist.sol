...
contract ElectricGuitarist is ERC721 {
    uint256 public totalSupply;
    uint256 public cost = 0.1 ether;
    uint256 public maxMints = 188;
    ...
    constructor()
        ERC721("Electric Guitarist", "EG")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 188) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeigiioprrh6uvwf3ga5obqud34drxpr4ezgtzt63ss4ozr4xnsaasq/", Strings.toString(tokenId), ".json")
            );
    }
