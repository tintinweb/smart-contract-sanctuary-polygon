...
contract PLGReferralReward is permission {
    ...
    function rewardDistribute(address account,address userContract,string memory dataSlot) public payable forRole("router") returns (bool) {
        ...
        for(uint256 i=0; i<30; i++){
            ...
            if(vipBlock[addrs[i]]>block.timestamp){ shouldreward = true; }else{
            ...
            if(shouldreward){
                ...
                if(vipBlock[addrs[i]]>block.timestamp || refmap.length>i){
    ...
    function updateRouter(address pool,address router) public forRole("owner") returns (bool) {
        PLGPool = pool;
        PLGv2 = router;
        return true;
    }
