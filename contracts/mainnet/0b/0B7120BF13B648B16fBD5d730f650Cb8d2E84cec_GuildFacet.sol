...
contract GuildFacet is Modifiers{
    ...
    event GuildAssigned(uint256 _tokenId,uint256 _guild);
    ...
    function assignGuild(uint256[] calldata _tokenIds,uint256 _guild) public onlyApproved{
        for(uint256 i = 0; i < _tokenIds.length; ){
            ...
            emit GuildAssigned(_tokenIds[i],_guild);
    ...
    function emitGuildEvents(uint256[] calldata _tokenIds) public onlyApproved{
        for(uint256 i = 0; i < _tokenIds.length; ){

            uint256 assignedGuild = s.gotchiMapping[_tokenIds[i]].assignedGuild;

            emit GuildAssigned(_tokenIds[i],assignedGuild);
