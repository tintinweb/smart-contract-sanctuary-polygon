...
contract WallchainWrapper is Ownable {
    ...
    address public multiPathImplementation;
    address public simpleSwapImplementation;

    constructor(IWChainMaster _wchainMaster) {
        wchainMaster = _wchainMaster;
        multiPathImplementation = 0x7719CC060A3348229DB764c9464B7738E4172e19;
        simpleSwapImplementation = 0x2674a5e9FDDCD775B466C312441942bD5278E513;
    }
    ...
    function megaSwap(
        ...
        (bool success, bytes memory resultData) = multiPathImplementation.delegatecall(abi.encodeWithSelector(0x46c67b6d, data));
        if (!success) {
    ...
    function buy(
        ...
        (bool success, bytes memory resultData) = multiPathImplementation.delegatecall(
    ...
    function multiSwap(
        ...
        (bool success, bytes memory resultData) = multiPathImplementation.delegatecall(abi.encodeWithSelector(0xa94e78ef, data));
        if (!success) {
            _revertWithData(resultData);
        }
        _returnWithData(resultData);
...
// 0x54e3f31b
    function simpleSwap(
        ...
        (bool success, bytes memory resultData) = simpleSwapImplementation.delegatecall(abi.encodeWithSelector(0x54e3f31b, data));
        if (!success) {
            _revertWithData(resultData);
        }
        _returnWithData(resultData);
    ...
    function simpleBuy(
        ...
        (bool success, bytes memory resultData) = simpleSwapImplementation.delegatecall(abi.encodeWithSelector(0x2298207a, data));
        if (!success) {
            _revertWithData(resultData);
        }
        _returnWithData(resultData);
