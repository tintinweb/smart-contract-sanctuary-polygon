...
contract MyChecker {
    ...
    // TODO
    // if decimal is not 18, need to convert the value to 18 decimal

    // getToken1USDValue  [ * (10 ** ( 18 + 8 )) ] - 단위임 이건
    // getToken2USDValue  [ * (10 ** ( 18 + 8 )) ]
    // getTotalUSDValue [ * (10 ** ( 18 + 8 ))]
    // checkIsTheValueNotSame - 1% 이내로 차이가 나는지 확인
    // checkIsTheValueOver100USD - 100 USD 이상인지 확인 [ 단위 통일해야함 ]
    ...
    function getToken1USDValue() public view returns (uint256 token1USDValue) {
        ...
        if (token1Decimals != 18) {
            token1Balance = token1Balance * (10**uint256(token1Decimals - 18));
        }
    ...
    function getToken2USDValue() public view returns (uint256 token2USDValue) {
        uint256 token2Balance = token2.balanceOf(msg.sender);
        uint8 token2Decimals = token2.decimals();

        if (token2Decimals != 18) {
            token2Balance = token2Balance * (10**uint256(token2Decimals - 18));
        }

        // get the latest price
        (, int256 price2, , , ) = priceFeed2.latestRoundData();

        // calculate the USD value of token2
        return (token2Balance * uint256(price2));
    }

    function getTotalUSDValue() public view returns (uint256 totalUSDValue) {
        uint256 token1USDValue = getToken1USDValue();
        uint256 token2USDValue = getToken2USDValue();

        return token1USDValue + token2USDValue;
    }
    ...
    function checkHavingMoreThan10USD()
        external
        view
        returns (bool canExec, bytes memory execPayload)
    {
