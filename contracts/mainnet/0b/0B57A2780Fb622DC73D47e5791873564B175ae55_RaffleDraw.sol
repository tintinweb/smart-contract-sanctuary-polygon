...
contract RaffleDraw is Base, IRaffleDraw {
     ...
     */
    mapping(uint256 => DrawWinner) private _raffleDrawWinners;
    /**
     * raffle id counter
     */
    ...
    function buyEntries(
        ...
        for (uint256 i = 0; i < newEntries; i++) {
            ...
            raffleDrawPlayerEntryMapping[draw.id][msg.sender].push(newEntry);
            // mapping entry => player
            raffleDrawEntryPlayerMapping[draw.id][newEntry] = msg.sender;
        ...
        for (uint256 i = 0; i < drawIds.length; i++) {
            ...
            if (
                ...
                uint256 walletAvailableEntries = draw.maxEntriesPerWallet -
                    raffleDrawPlayerEntryMapping[draw.id][player].length;
        ...
        onlyRole(MANAGER_ROLE)
    {
        ...
        if (draw.currentEntries <= draw.numberOfPrizes) {
            ...
            winners = new address[](draw.currentEntries);
            for (uint256 i = 0; i < draw.currentEntries; i++) {
                winnerEntries[i] = i;
                winners[i] = raffleDrawEntryPlayerMapping[drawId][
                    winnerEntries[i]
                ];
                _raffleDrawWinners[drawId].entryMapping[
                    winnerEntries[i]
                ] = true;
            }
        } else {
        ...
        _raffleDrawWinners[drawId].drawEntries = winnerEntries;
