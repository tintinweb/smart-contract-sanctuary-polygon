...
contract FlashArb {
    ...
    function arb(address bamm, uint cBorrowAmount, address cCollateral) public {
    ...
    function checkProfitableArb(uint cBorrowAmount, uint minProfit, BAMMSwapLike bamm, address cCollateral) external returns(bool){
        ...
        if(bamm.isCETH()) {
            ...
            balanceBefore = src.balanceOf(address(this));
        }

        this.arb(address(bamm), cBorrowAmount, cCollateral);
        uint balanceAfter;
        if(bamm.isCETH()) {
            ...
            IERC20 src = IERC20(ICToken(bamm.LUSD()).underlying());
            balanceAfter = src.balanceOf(address(this));
        }

        require((balanceAfter - balanceBefore) >= minProfit, "min profit was not reached");

        if(bamm.isCETH()) {
            msg.sender.transfer(balanceAfter - balanceBefore);
        }
        else {
            IERC20 src = IERC20(ICToken(bamm.LUSD()).underlying());
            src.transfer(msg.sender, balanceAfter - balanceBefore);            
        }
