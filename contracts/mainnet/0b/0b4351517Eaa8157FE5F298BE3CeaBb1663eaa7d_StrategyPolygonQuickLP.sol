...
contract StratManager is Ownable, Pausable {
    ...
    address public evolleFeeRecipient;
     ...
     * @param _evolleFeeRecipient address where to send evolle's fees.
     */
    constructor(
        address _keeper,
        address _strategist,
        address _unirouter,
        address _vault,
        address _evolleFeeRecipient
    ) public {
        keeper = _keeper;
        strategist = _strategist;
        unirouter = _unirouter;
        vault = _vault;
        evolleFeeRecipient = _evolleFeeRecipient;
    }
    ...
    /**
     * @dev Updates evolle fee recipient.
     * @param _evolleFeeRecipient new evolle fee recipient address.
     */
    function setevolleFeeRecipient(address _evolleFeeRecipient) external onlyOwner {
        evolleFeeRecipient = _evolleFeeRecipient;
    }
...
abstract contract FeeManager is StratManager {
    ...
    uint public evolleFee = MAX_FEE - STRATEGIST_FEE - callFee;

    function setCallFee(uint256 _fee) public onlyManager {
        require(_fee <= MAX_CALL_FEE, "!cap");
        
        callFee = _fee;
        evolleFee = MAX_FEE - STRATEGIST_FEE - callFee;
    }
...
contract StrategyPolygonQuickLP is StratManager, FeeManager {
    ...
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _evolleFeeRecipient) public {
        want = _want;
    ...
    function chargeFees(address callFeeRecipient) internal {
        uint256 toNative = IERC20(output).balanceOf(address(this)).mul(30).div(1000);
        ...
        uint256 evolleFeeAmount = nativeBal.mul(evolleFee).div(MAX_FEE);
        IERC20(native).safeTransfer(evolleFeeRecipient, evolleFeeAmount);
    ...
    function callReward() public view returns (uint256) {
        ...
        return nativeOut.mul(30).div(1000).mul(callFee).div(MAX_FEE);
    }
