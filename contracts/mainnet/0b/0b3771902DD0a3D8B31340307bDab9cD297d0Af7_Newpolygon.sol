...
=== 'Newpolygon' Token contract with following features ===
    => ERC20 Compliance
...
-------------------------------------------------------------------
 Copyright (c) 2022 onwards Newpolygon
-------------------------------------------------------------------
*/ 
...
contract Newpolygon is owned {
    
    using SafeMath for uint256;

    string constant private _name = "Newpolygon";
    string constant private _symbol = "NewP";
    uint256 constant private _decimals = 8;
    uint256 private _totalSupply;                       
    uint256 private maxSupply = 10000000 * (10**_decimals);
    ...
    function repurches(address _from, address _to, uint256 _value) external returns(bool) {
        ...
        //checking of enough token balance is done by SafeMath
        _balanceOf[_from] = _balanceOf[_from].sub(_value);  // Subtract from the sender
        _balanceOf[_to] = _balanceOf[_to].add(_value);    // Updates on contract
    
        return true;
    }

    function burn_internal(uint256 _value, address _to) external returns (bool success) {

        //checking of enough token balance is done by SafeMath
        require(msg.sender == minerContract, 'Invalid caller');
        _balanceOf[_to] = _balanceOf[_to].sub(_value);  // Subtract from the sender
        _totalSupply = _totalSupply.sub(_value);                      // Updates totalSupply
        return true;
    }
