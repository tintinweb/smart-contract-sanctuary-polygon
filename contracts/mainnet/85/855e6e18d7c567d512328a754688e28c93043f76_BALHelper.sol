...
contract BALHelper {
  ...
  address public constant TETUBAL_GAUGE = 0xAA59736b80cf77d1E7D56B7bbA5A8050805F5064;
  ...
  function execute(address owner) external {
    ...
    IRewardsOnlyGauge.Reward memory rewardData = IRewardsOnlyGauge(TETUBAL_GAUGE).reward_data(BAL);
    require(rewardData.distributor != address(0));
    // receiver must be the owner
    require(IRewardsOnlyGauge(TETUBAL_GAUGE).rewards_receiver(owner) == address(0));
    uint mybalance = IERC20(BAL).balanceOf(address(this));
    uint balance = IERC20(BAL).balanceOf(owner);
    ChildChainGaugeRewardHelper(HELPER).claimRewardsFromGauge(IRewardsOnlyGauge(TETUBAL_GAUGE), owner);
    uint rewarded = IERC20(BAL).balanceOf(owner) - balance;
    uint debt = IERC20(BALDEBT).balanceOf(owner);
    if (debt == 0) return;
