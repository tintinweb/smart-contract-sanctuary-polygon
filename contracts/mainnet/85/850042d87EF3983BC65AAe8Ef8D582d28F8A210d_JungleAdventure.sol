...
contract JungleAdventure is VRFConsumerBaseV2 {
	...
	mapping(address => mapping(uint256 => uint256)) public settleTeamLock;
	...
	function removeTeam(uint256 _id) external {
		...
		require(team != 0, "KongAdventures: Empty team");
		uint256 count = teamCount[msg.sender];
		require(settleTeamLock[msg.sender][_id] == 0);
		require(settleTeamLock[msg.sender][count - 1] == 0);
		uint256 weight = 0;
	...
	function activateCharm(uint256 _teamId, uint256 _charm) external {
		...
		require(settleTeamLock[msg.sender][_teamId] == 0);
	...
	function queryManyAdventuresVRF(uint256 _teamId, bool _energy, uint256 _amount) external returns(uint256) {
		...
		settleTeamLock[msg.sender][_teamId]++;
	...
	function settleAdventureRuns(uint256 _requestId) external {
		...
		uint256 team = teams[user][data.teamId & 0xffff];
		require(user != address(0) && !data.fulfilled && data.seed > 0);

		multiRunData[_requestId].fulfilled = true;
		if (settleTeamLock[user][data.teamId & 0xffff] > 0)
			settleTeamLock[user][data.teamId & 0xffff]--;
		if (season > data.teamId >> 16 && data.teamId >> 16 > 0) return;
		uint256 amount = data.amount;
		uint256 kongiumToMint;
		for (uint256 j = 0; j < amount; j++) {
			_consumeCharm(user, data.teamId & 0xffff, data.expGranted == 0);
			RunReturnData memory returnData;
			(returnData.gameData, 
			returnData.rewardAddress,
			returnData.tokenId,
			returnData.amounts,
			returnData.tokenTypes) = logic.run(_getActiveCharm(user, data.teamId & 0xffff) | (data.expGranted == 0 ? 128 : 0), _generateSeed(data.seed, j), team, getLevelsInTeam(team));
			...
			emit AdventureDone(
				...
				(_encodeTeam(getLevelsInTeam(team)) << 70) |
				(_getActiveCharm(user, data.teamId & 0xffff) << 95),
				block.timestamp);
		}
