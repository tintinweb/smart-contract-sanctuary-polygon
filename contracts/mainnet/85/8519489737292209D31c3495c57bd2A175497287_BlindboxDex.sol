...
contract DexStorage {
  ...
  // of-chain Offer state variables >>>>>>>
  struct Offer{
    address maker;
    address collection;
    uint256 tokenId;
  ...
  // mapping to track accepted offers to avoid duplication / reentercy
  mapping (bytes32 => bool ) public acceptedOffer;
...
function updateNonCryptoBalance(string memory ownerId, uint256 amount) public {
  require(admin == msg.sender, "not authorize");
    _nonCryptoWallet[ownerId].Alia = amount;
}   
