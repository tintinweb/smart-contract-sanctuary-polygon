...
abstract contract BaseAllowance4Mint is BaseAllowanceHandler
{
     ...
     */
    function _addOrUseAllowance(address address_, bytes4 bucketName, uint256 amount, bool add) internal virtual override
    returns (uint256 allowance, Bucket memory bucket)
    {
        (uint256 _allowance, Bucket memory _bucket) = _addOrUseAllowanceSuper(address_, bucketName, amount, add);
        // Every payable NFT have been minted, the user gain a free mint
        if(!add && bucketName == mintBucketName && amount != 0 && _allowance == 0)
        {
            _addOrUseAllowanceSuper(address_, freeMintBucketName, 1, true);
        }
        return (_allowance, _bucket);
    }
    function _addOrUseAllowanceSuper(address address_, bytes4 bucketName, uint256 amount, bool add) internal virtual
    returns (uint256 allowance, Bucket memory bucket);
}
/**
...
contract Allowance4MintProxy is BaseAllowance4Mint, AllowanceProxy
{
    ...
    function _addOrUseAllowance(address address_, bytes4 bucketName, uint256 amount, bool add) internal override(BaseAllowance4Mint, AllowanceProxy)
    returns (uint256 allowance, Bucket memory bucket)
    {
        return BaseAllowance4Mint._addOrUseAllowance(address_, bucketName, amount, add);
    }
    function _addOrUseAllowanceSuper(address address_, bytes4 bucketName, uint256 amount, bool add) internal override
    returns (uint256 allowance, Bucket memory bucket)
    {
        return AllowanceProxy._addOrUseAllowance(address_, bucketName, amount, add);
    }
...
contract Allowance4MintHandler is BaseAllowance4Mint, AllowanceHandler
{
    ...
    function _addOrUseAllowance(address address_, bytes4 bucketName, uint256 amount, bool add) internal override(BaseAllowance4Mint, AllowanceHandler)
    returns (uint256 allowance, Bucket memory bucket)
    {
        return BaseAllowance4Mint._addOrUseAllowance(address_, bucketName, amount, add);
    }
    ...
    returns (uint256 allowance, Bucket memory bucket)
    {
        return AllowanceHandler._addOrUseAllowance(address_, bucketName, amount, add);
    }
}
...
abstract contract PriceHandlerProxy is ProxyDiamond
{
    ...
    {
        return JarvixPriceHandler(getProxy(type(IJarvixPrice).interfaceId));
    }
 ...
 * and by checking that transfert is not allowed if initiated by platforms that don't apply creator's royalties on sells. That is
...
abstract contract RoyaltyImplementerProxy is ProxyDiamond, Ownable, IERC2981, RoyaltiesV2, DefaultOperatorFilterer
{
    ...
    {
        return RoyaltyHandler(getProxy(type(IRoyalty).interfaceId));
    }
    /**
 ...
 * see: https://cryptomarketpool.com/erc721-contract-that-supports-sales-royalties/
 */
abstract contract RoyaltyImplementerDirect is RoyaltyHandler, Ownable, IERC2981, RoyaltiesV2, DefaultOperatorFilterer
{
...
abstract contract TokenDataHandlerProxy is ProxyDiamond
{
    ...
    {
        return TokenDataHandler(getProxy(type(ITokenData).interfaceId));
    }
...
interface IJarvixFeedRegistry
{
    ...
    // function decimals(bytes4 baseCurrency, bytes4 quoteCurrency) external view returns (uint8);
    ...
    function getRoundAnswer(bytes4 baseCurrency, bytes4 quoteCurrency, uint80 _roundId, bool _resultDecimals) external view
    returns(int256 answer, uint8 entryDecimals, uint8 resultDecimals);
    // function latestRoundAnswerWithDecimals(bytes4 baseCurrency, bytes4 quoteCurrency) external view
    // returns(int256 answer, uint8 entryDecimals, uint8 resultDecimals);
    // function getRoundAnswerWithDecimals(bytes4 baseCurrency, bytes4 quoteCurrency, uint80 _roundId) external view
    // returns(int256 answer, uint8 entryDecimals, uint8 resultDecimals);

    // Proposed V3 AggregatorV3Interface implementation
    // function proposedDecimals(bytes4 baseCurrency, bytes4 quoteCurrency) external view returns (uint8);
    ...
    // function getFeed(bytes4 baseCurrency, bytes4 quoteCurrency) external view returns(AggregatorV3Interface feed);
    function getProposedFeedData(bytes4 baseCurrency, bytes4 quoteCurrency) external view returns(FeedData memory feedData);
    // function getProposedFeed(bytes4 baseCurrency, bytes4 quoteCurrency) external view returns(AggregatorV3Interface proposedFeed);
...
interface IJarvixFeedRegistryAdmin
{
    event FeedProposed(address indexed proposer, bytes4 indexed baseCurrency, bytes4 indexed quoteCurrency,
                       uint8 proposedEntryDecimals, uint8 previousEntryDecimals, uint8 currentEntryDecimals,
                       address proposedFeed, address previousFeed, address currentFeed);
...
contract JarvixFeedRegistry is IJarvixFeedRegistry, IJarvixFeedRegistryAdmin, CurrencyAndERC20Handler, AccessControlImpl
{
    ...
    constructor() CurrencyAndERC20Handler("USD") {}

    // function decimals(bytes4 baseCurrency, bytes4 quoteCurrency) external view override returns (uint8)
    // {
    //     return getFeed(baseCurrency, quoteCurrency).decimals();
    // }
    ...
    returns(int256 answer, uint8 entryDecimals, uint8 resultDecimals)
    {
        ...
        // (, answer, , ,) = feedData.feed.getRoundData(roundId);
        // entryDecimals = feedData.entryDecimals;
        // if(_resultDecimals)
        // {
        //     resultDecimals = feedData.feed.decimals();
    ...
    // function proposedDecimals(bytes4 baseCurrency, bytes4 quoteCurrency) external view returns (uint8)
    // {
    //     return getProposedFeed(baseCurrency, quoteCurrency).decimals();
    // }
    ...
    // function getFeed(bytes4 baseCurrency, bytes4 quoteCurrency) public view override returns(AggregatorV3Interface feed)
    // {
    //     feed = getFeedData(baseCurrency, quoteCurrency).feed;
    // }
    ...
    // function getProposedFeed(bytes4 baseCurrency, bytes4 quoteCurrency) public view override returns(AggregatorV3Interface proposedFeed)
    // {
    //     proposedFeed = getProposedFeedData(baseCurrency, quoteCurrency).feed;
    // }
    function hasFeed(bytes4 baseCurrency, bytes4 quoteCurrency) external view returns(bool)
    {
        // baseCurrency = baseCurrency == 0x0 ? DEFAULT : baseCurrency;
    ...
    existentCurrency(baseCurrency) existentCurrency(quoteCurrency) onlyRole(PROPOSER_ROLE) 
    {
        ...
        FeedData memory currentFeed = _usedFeeds[baseCurrency][quoteCurrency];
        ...
        emit FeedProposed(_msgSender(), baseCurrency, quoteCurrency,
                          entryDecimals, previousFeed.entryDecimals, currentFeed.entryDecimals,
                          feedAddress, address(previousFeed.feed), address(currentFeed.feed));
    }
...
abstract contract JarvixFeedRegistryProxy is ProxyDiamond
{
     ...
     * @dev Contract constructor. For final implementers, do not forget to call ProxyDiamond constructor first in order
     * to initialize address of the proxy hub used to reference proxies
     * @param feedRegistryAddress_ Address of the contract registring Chainlink feeds
     */
    ...
    /**
     * Getter of the contract registring Chainlink feeds
     */
    function getFeedRegistry() internal view virtual returns(IJarvixFeedRegistry)
    {
        return IJarvixFeedRegistry(getProxy(IJarvixFeedRegistryInterfaceId));
    }
...
{
    event CurrencyFiatAdded(address indexed admin, bytes4 indexed currency);
    event CurrencyFiatRemoved(address indexed admin, bytes4 indexed currency);
    event CurrencyTokenAdded(address indexed admin, bytes4 indexed currency, address indexed token);
    event CurrencyTokenRemoved(address indexed admin, bytes4 indexed currency, address indexed token);
    ...
    function encodeCurrencyFromERC20(ERC20 erc20) external view returns(bytes4 currency);
    function decodeCurrencyFromERC20(ERC20 erc20) external view returns(string memory currencyCode);
...
contract CurrencyAndERC20Handler is ICurrencyAndERC20Handler, CurrencyHandler
{
    ...
    {
        return encodeCurrencyFromERC20(ERC20(erc20Address));
    }
    function decodeCurrencyFromAddress(address erc20Address) public view override returns(string memory currencyCode)
    {
        return decodeCurrencyFromERC20(ERC20(erc20Address));
    }
    function encodeCurrencyFromERC20(ERC20 erc20) public view override returns(bytes4 currency)
    {
        return encodeCurrency(decodeCurrencyFromERC20(erc20));
    }
    function decodeCurrencyFromERC20(ERC20 erc20) public view override returns(string memory currencyCode)
    {
        return erc20.symbol();
    }

    function getERC20(bytes4 currency) public view override existentERC20(currency) returns(ERC20 erc20)
    {
        return _erc20Tokens[currency];
    }
    function getERC20Address(bytes4 currency) external view override existentERC20(currency) returns(address erc20Address)
    {
        return address(_erc20Tokens[currency]);
    }
    function getERC20Code(address erc20Address) public view override returns(bytes4 erc20Code)
    {
        return _erc20Codes[erc20Address];
    }

    function hasERC20(bytes4 currency) public view override returns(bool)
    {
        return address(_erc20Tokens[currency]) != address(0);
    }
    function hasERC20Address(address erc20Address) public view override returns(bool)
    {
    ...
    function _addCurrencyFiat(string memory symbol) internal virtual
    {
        // Store fiat currency
       bytes4 currency = _addCurrency(symbol);
       emit CurrencyFiatAdded(msg.sender/*TODO _msgSender()*/, currency);
    }
    function _addCurrencyToken(address erc20Address) internal virtual
    {
        // Get token currency
        ERC20 erc20 = ERC20(erc20Address);
        string memory currencyCode = decodeCurrencyFromERC20(erc20);
        if(_erc20Codes[erc20Address] != 0x00) revert CurrencyAndERC20Handler_ExistentERC20(erc20Address);
        // Store token currency
        bytes4 currency = _addCurrency(currencyCode);
        _erc20Tokens[currency] = erc20;
        _erc20Codes[erc20Address] = currency;
        emit CurrencyTokenAdded(msg.sender/*TODO _msgSender()*/, currency, erc20Address);
    }
    function _removeCurrency(bytes4 currency) internal virtual override
    {
        ...
        if(_erc20Tokens[currency] == ERC20(address(0)))
        {
            ...
            emit CurrencyFiatRemoved(msg.sender/*TODO _msgSender()*/, currency);
        }
        else
        {
            ...
            emit CurrencyTokenRemoved(msg.sender/*TODO _msgSender()*/, currency, erc20Address);
        }
    }
...
abstract contract CurrencyAndERC20HandlerProxy is ProxyDiamond
{
    ...
    {
        return ICurrencyAndERC20Handler(getProxy(ICurrencyAndERC20HandlerInterfaceId));
    }
    function _setCurrencyAndERC20HandlerProxy(address currencyAndERC20HandlerAddress_) internal virtual
    {
        _setProxy(ICurrencyAndERC20HandlerInterfaceId, currencyAndERC20HandlerAddress_, ICurrencyAndERC20HandlerInterfaceId, false, true, true);
    }
}
...
/**
 * @title Interface that lists base method needed by Currency Handling.
 * @author tazous
 */
...
contract CurrencyHandler is ICurrencyHandler
{
     ...
     * @param currency Currency code which existance among the one defined in this contract should be checked
     * @return True if given currency code is one of currently defined in this contract, false otherwise
     */
    ...
    /**
     * @dev This method adds given currency code as defined one in this contract (DEFAULT & COIN values are
     * not accepted as they are Permanent)
     * @param currency Currency code to be added among defined ones in this contract
     */
    ...
    /**
     * @dev This method removes given currency code from defined ones in this contract (DEFAULT & COIN values
     * are not accepted as they are Permanent)
     * @param currency Currency code to be removed from defined ones in this contract
     */
...
abstract contract CurrencyHandlerProxy is ProxyDiamond
{
    ...
    {
        return ICurrencyHandler(getProxy(ICurrencyHandlerInterfaceId));
    }
...
contract ProxyHub is PausableImpl
{
     ...
     * @return Found existing proxy address defined by given user on provided key
     */
    function getProxyAddressFor(address user, bytes4 key) external view returns (address)
    {
...
contract ProxyDiamond is IProxyDiamond
{
     ...
     */
    function getProxy(bytes4 key) public virtual view returns (address)
    {
...
interface IAllowance is ISecuredProcess
{
     ...
     */
    function getAllowance(address address_, bytes4 bucketName) external view returns (uint256 allowance, Bucket memory bucket);
    /**
     ...
     */
    function checkAllowance(address address_, bytes4 bucketName, uint256 amount) external view returns (uint256 allowance, Bucket memory bucket);
    /**
     ...
     */
    function addAllowances(address[] memory addresses, bytes4[] memory bucketNames, uint256[] memory amounts) external returns (uint256[] memory allowances, Bucket[] memory buckets);
    /**
     ...
     */
    function addAllowance(address address_, bytes4 bucketName, uint256 amount) external returns (uint256 allowance, Bucket memory bucket);
    /**
     ...
     */
    function useAllowance(address address_, bytes4 bucketName, uint256 amount) external returns (uint256 allowance, Bucket memory bucket);
}
...
abstract contract BaseAllowanceHandler is IAllowance, PausableImpl
{
     ...
     * @param bucketName Bucket name for which allowance should be retrieved
     */
    function getAllowance(address address_, bytes4 bucketName) public virtual view returns (uint256 allowance, Bucket memory bucket);
    /**
     ...
     * @param amount Minimal amount of allowance expected
     */
    function checkAllowance(address address_, bytes4 bucketName, uint256 amount) public view returns (uint256 allowance, Bucket memory bucket)
    {
     ...
     * @param amounts Amounts of allowance to be added
     */
    function addAllowances(address[] memory addresses, bytes4[] memory bucketNames, uint256[] memory amounts) external
    returns (uint256[] memory allowances, Bucket[] memory buckets)
    {
        ...
        {
            (allowances[i], buckets[i]) = _addOrUseAllowance(addresses[i], bucketNames[i], amounts[i], true);
        }
     ...
     * @param amount Amount of allowance to be added
     */
    function addAllowance(address address_, bytes4 bucketName, uint256 amount) external
    returns (uint256 allowance, Bucket memory bucket)
    {
        return _addOrUseAllowance(address_, bucketName, amount, true);
    }
    /**
     ...
     * @param amount Amount of allowance to be used
     */
    function useAllowance(address address_, bytes4 bucketName, uint256 amount) public
    returns (uint256 allowance, Bucket memory bucket)
    {
        return _addOrUseAllowance(address_, bucketName, amount, false);
    }
    /**
     ...
     */
    function _addOrUseAllowance(address address_, bytes4 bucketName, uint256 amount, bool add) internal virtual
    returns (uint256 allowance, Bucket memory bucket);

    /**
     * Check process for an allowance secured mechanism consists of checking allowance rights
     */
    function checkProcess(bytes4 bucketName, address address_, uint256 amount) public virtual view
    {
     ...
     */
    function doProcess(bytes4 bucketName, address address_, uint256 amount) public virtual
    {
...
contract AllowanceHandler is BaseAllowanceHandler {
    ...
    mapping(address => mapping(bytes4 => uint256)) private _allowances;
     ...
     */
    event AllowanceAdded(address indexed admin, address indexed beneficiary, bytes4 indexed bucket, uint256 amount, uint256 allowance);
    /**
     ...
     */
    event AllowanceUsed(address indexed consumer, address indexed beneficiary, bytes4 indexed bucket, uint256 amount, uint256 allowance);
     ...
     * @param bucketName Bucket name for which allowance should be retrieved
     */
    function getAllowance(address address_, bytes4 bucketName) public override view returns (uint256 allowance, Bucket memory bucket)
    {
     ...
     * @param add Should the amount of allowance added or user for defined user on a given bucket
     */
    function _addOrUseAllowance(address address_, bytes4 bucketName, uint256 amount, bool add) internal virtual override whenNotPaused()
    returns (uint256 allowance, Bucket memory bucket)
    {
...
abstract contract AllowanceProxy is ProxyDiamond, BaseAllowanceHandler
{
    ...
    {
        return IAllowance(getProxy(IAllowanceInterfaceId));
    }
...
{
    function checkProcess(bytes4 processName, address address2Process, uint256 amount2Process) external view;
    function doProcess(bytes4 processName, address address2Process,uint256 amount2Process) external;
}
...
abstract contract SecuredProcessProxy is ProxyDiamond
{
    ...
    function checkProcess(bytes4 processName, address address2Process, uint256 amount2Process) public view virtual
    {
        address securedProcessAddress = getProxy(ISecuredProcessInterfaceId);
    ...
    function _doProcess(bytes4 processName, address address2Process, uint256 amount2Process) internal virtual
    {
        address securedProcessAddress = getProxy(ISecuredProcessInterfaceId);
        if(securedProcessAddress != address(0))
        {
