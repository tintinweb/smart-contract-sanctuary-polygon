...
contract DTTDPOAPContract is ERC1155Supply, Ownable {
    ...
    modifier tidCheck(bytes32 tid) {
        require (tidUsed[tid] == false, "tid already used");
        _;
    }
     ...
     * Airdrop FUNCTIONS
     */
    function airdropSingle(bytes32 tid, address _to, uint256 _id, uint256 _quantity) external onlyOwner tidCheck(tid) {
        require(_id < numberOfTokens, "Invalid ID");
    ...
    function airdropBatch(bytes32 tid, address _to, uint256[] memory _ids, uint256[] memory _quantities) external onlyOwner tidCheck(tid) {
