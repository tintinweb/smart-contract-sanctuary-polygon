...
contract TITANIUM {
    ...
    constructor(address ownerAddress,address _devWallet,IBEP20 _DAI) {
        owner = ownerAddress; 
   ...
   function registration(uint256 _amount,string memory addr,address refadd,string memory _referrerId ) external {
        ...
        require(DAI.balanceOf(msg.sender) >= _amount,"Low DAI Balance");
        require(DAI.allowance(msg.sender,address(this)) >= _amount,"Invalid allowance");
        emit Registration(addr,refadd,_referrerId,_amount);
        DAI.transferFrom(msg.sender, owner, ownerAmt);
        DAI.transferFrom(msg.sender, devWallet, devAmt);
 	}
