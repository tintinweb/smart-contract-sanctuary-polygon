...
contract DegShipCommanderV2 is IDegShipCommanderV2, Ownable {
    ...
    IDegShipFactoryV2 public immutable shipFactory;
    IDegCrewExtend public immutable shipCrew;

    IDegSwap public degSwap;
    ...
    function _basicDistributeDeposit(address _ticket, address _ship, uint256 _amount) private returns (uint256 totalFee){
        ...
        if (_ticket == address(0)) {
            ...
            IERC20(_ticket).transfer(feeReceiver[2], coinFee);
    ...
    function register(address refer) external {
        ...
        require(refer != address(0) && !refer.isContract(), "Refer is illegel");
    ...
    function afterLaunch(string memory serial, address _launchShipAddr, uint256 _amount) private {
        // record invest order infos
        recordDeposits(serial, msg.sender, _amount);
        // ship stopped gameover
        bool _checkShipStatus = checkShipStop(_launchShipAddr);
        if ( _checkShipStatus ) {
            // stopped start another ship
            luckDividend[_launchShipAddr] = true;
            if(shipFactory.shipFactoryAutoMaker(serial)) {
                destroyShipOfSerial(serial);
            }
            emit ShipStopped(_launchShipAddr);
        }
    }
    ...
    function launchShip(string memory serial, uint256 _amount) external payable recordDayModifier prepareShipStock(serial) {
        ...
        afterLaunch(serial, _launchShipAddr, _amount);
    ...
    function dividendFomoBonus(address _ship) external {
        ...
        if (totalInvestor > 0) {
            uint startIndex = totalInvestor > 50 ? totalInvestor - 50 : 0;
            uint endIndex = totalInvestor - 1;
            for (uint i = startIndex; i <= endIndex; i++) {
                address accuqire = IDegShipV2(_ship).depositors(endIndex - i);
                uint bonus;
                if(i == 0){
                    //30%
                    bonus = totalPool.mul(30).div(100);
                } else if(i < 4 && i > 0){
                    //60%
                    bonus = totalPool.mul(20).div(100).div(3);
                } else {
                    bonus = totalPool.mul(50).div(100).div(46);
                }
                if( bonus > 0 ){
