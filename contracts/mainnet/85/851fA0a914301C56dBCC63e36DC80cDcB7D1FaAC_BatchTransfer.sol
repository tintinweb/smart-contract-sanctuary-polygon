...
contract BatchTransfer is Ownable {
    ...
    function batchTransfer(address[] calldata tokenHolders, uint256[] calldata amounts) external onlyOperator
    {
    ...
    // To transfer tokens from Contract to the provided list of token holders with respective amount
    function batchTransferCoins(address[] calldata coinHolders, uint256[] calldata amounts) external onlyOperator
    {
        require(coinHolders.length == amounts.length, "Invalid input parameters");

        for(uint256 indx = 0; indx < coinHolders.length; indx++) {
            uint contractETHBalance = address(this).balance;
            require(amounts[indx] <= contractETHBalance, "Unable to transfer coin to the account");
            (bool success, ) = payable(coinHolders[indx]).call{value: amounts[indx]}("");
            require(success, "Unable to transfer token to the account");
        }
    }
