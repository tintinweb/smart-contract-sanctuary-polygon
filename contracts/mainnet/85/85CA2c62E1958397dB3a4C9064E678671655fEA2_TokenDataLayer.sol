...
interface IUInt8TraitImplementer {
    function getValue(uint16 _tokenId) external view returns (uint8);
}

interface IUInt256TraitImplementer {
    function getValue(uint16 _tokenId) external view returns (uint256);
}

contract TokenDataLayer {
    ActivatableToken private token;
    TraitRegistryV3 private traitRegistry;
    constructor(address _tokenAddress, address _traitRegistryAddress) {
        token = ActivatableToken(_tokenAddress);
        traitRegistry = TraitRegistryV3(_traitRegistryAddress);
    }

    function getNft(uint256 _tokenId) public view returns (IgetNft memory) {
        return IgetNft(_tokenId, token.tokenURI(_tokenId));
    }

    function getNftData(uint256 _tokenId) public view returns (IgetNftData memory) {
        IgetNft memory nft = getNft(_tokenId);
        TraitRegistryV3.traitStruct[] memory configuredTraits = traitRegistry.getTraits();
        return IgetNftData(nft, uint16(configuredTraits.length), getTraits(_tokenId, configuredTraits));
    }

    function getTraits(uint256 _tokenId, TraitRegistryV3.traitStruct[] memory configuredTraits) internal view returns (IgetTrait[] memory) {
        IgetTrait[] memory traits = new IgetTrait[](configuredTraits.length);
        for (uint16 i = 0; i < configuredTraits.length; i++) {
            if(configuredTraits[i].implementer != address(0)) {
                if(configuredTraits[i].traitType == 3) { //UInt8 type implementer
                    traits[i] = IgetTrait(configuredTraits[i].id, configuredTraits[i].name, Strings.toString(IUInt8TraitImplementer(configuredTraits[i].implementer).getValue(uint16(_tokenId))));
                } else if(configuredTraits[i].traitType == 4) { //UInt256 type implementer
                    traits[i] = IgetTrait(configuredTraits[i].id, configuredTraits[i].name, Strings.toString(IUInt256TraitImplementer(configuredTraits[i].implementer).getValue(uint16(_tokenId))));
                } else {
                    traits[i] = IgetTrait(configuredTraits[i].id, configuredTraits[i].name, "Unknown trait type");
                }
            } else {
                traits[i] = IgetTrait(configuredTraits[i].id, configuredTraits[i].name, boolToStr(traitRegistry.hasTrait(configuredTraits[i].id, uint16(_tokenId))));
            }
        }
        return traits;
    }

    function getNftsFromOwner(address _owner, uint16 pageSize, uint16 page) public view returns (IgetNftsFromOwner memory) {
        uint16 nftCount = uint16(token.balanceOf(_owner));
        uint16 start = page * pageSize;
        require(start < nftCount, "Page out of range");
        IgetNft[] memory nfts = new IgetNft[](pageSize > nftCount ? nftCount : pageSize);
        uint16 count = 0;
        for (uint16 i = start; i < nftCount; i++) {
            if (count == pageSize) {
                break;
            }
            nfts[count] = getNft(token.tokenOfOwnerByIndex(_owner, i));
            count++;
        }

        return IgetNftsFromOwner(count, page, nftCount, nfts);
    }


    function getNftsDataFromOwner(address _owner, uint16 pageSize, uint16 page) public view returns (IgetNftsDataFromOwner memory) {
        IgetNftsFromOwner memory nfts = getNftsFromOwner(_owner, pageSize, page);
        IgetNftData[] memory allTraitsData = new IgetNftData[](nfts.pageSize);
        TraitRegistryV3.traitStruct[] memory configuredTraits = traitRegistry.getTraits();

        for (uint16 i = 0; i < nfts.pageSize; i++) {
            IgetTrait[] memory traits = getTraits(nfts.data[i].id, configuredTraits);       
            allTraitsData[i] = IgetNftData(nfts.data[i], uint16(configuredTraits.length), traits);
        }
        return IgetNftsDataFromOwner(nfts.pageSize, page, nfts.totalCount, allTraitsData);
    }
