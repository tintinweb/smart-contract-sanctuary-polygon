...
contract drop1 is Ownable, ERC721A, ReentrancyGuard, DefaultOperatorFilterer  {
    uint256 public MAX_SUPPLY = 100000;
    uint256 public PRICE = 10000; // 0.01 in USDC
    uint256 public PRESALE_PRICE = 10000; // 0.01 in USDC
    uint256 public maxPresale = 3000;
    uint256 public maxPublic = 3000;
    address public USDCproxy = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174; //  proxy USDC contract (use for approval)
    address public USDC = 0xDD9185DB084f5C4fFf3b4f70E7bA62123b812226; // actual USDC contract (use for balance check)
    ...
    function approveSpendOfUSDC(uint8 quantity)
        external
        nonReentrant
        callerIsUser
    {
        require(getERC20balance(USDC, msg.sender) >= PRICE * quantity, "Insufficient USDC");
        IERC20(USDC).approve(msg.sender, PRICE * quantity);
    }
        ...
        callerIsUser
    {
        ...
        require(getERC20balance(USDCproxy, msg.sender) >= PRICE * quantity, "Insufficient USDC");
        IERC20(USDCproxy).transferFrom(msg.sender, address(this), PRICE * quantity);
