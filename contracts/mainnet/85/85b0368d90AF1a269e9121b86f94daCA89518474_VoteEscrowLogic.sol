...
contract VoteEscrowResolver is DSMath {
	 ...
	 * @param ethaToken address of ETHA token.
	...
	function deposit(
		...
		IERC20 etha = IERC20(ethaToken);
		IVotingEscrow veEtha = IVotingEscrow(veEthaContract);
		etha.universalApprove(veEthaContract, realAmt);
	...
	/**
	 * @dev Withdraw tokens from the distribution contract.
	 * @param vault address of vault
	 * @param tokenAmt amount of vault tokens to withdraw
	 * @param getId read value of tokenAmt from memory contract
	 * @param setId set the value of tokenAmt into the memory contract
	 */
	function withdraw_distribution(
		IVault vault,
		uint256 tokenAmt,
		uint256 getId,
		uint256 setId
	 ...
	 * @param veEthaToken address of VoteEscrowETHA token.
	 * @param noOfDays amount of days to increase the lock.
	 */
	function increase_time(address veEthaToken, uint256 noOfDays)
		external
		payable
	{
		IVotingEscrow veEtha = IVotingEscrow(veEthaToken);

		veEtha.increase_unlock_time(noOfDays);
	}
}
