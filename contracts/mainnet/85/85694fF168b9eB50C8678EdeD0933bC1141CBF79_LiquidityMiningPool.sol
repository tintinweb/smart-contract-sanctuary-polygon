...
contract LiquidityMiningPool is Operator, ContractGuard, ReentrancyGuard 
{
  ...
  // Info of each user.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. lck to distribute per block.
        uint256 lastRewardTime; // Last time that lck distribution occurs.
        uint256 accTokenPerShare; // Accumulated lck per share, times 1e18. See below.
    ...
    address public marketingWallet = 0x569A0ff99F80644f2c35D949d6ab475d2e847cCb; //All Deposit Fee (if there is) will be sent to DaoFund
    ...
    uint256 public totalRewards = 15000000 ether;
    ...
    constructor(address _token) public 
    {
        ...
        tokenPerSecond = totalRewards.div(runningTime);  
    ...
    function updatePool(uint256 _pid) internal {
        ...
        if (!pool.isStarted) {
            pool.isStarted = poolStartTime <= block.timestamp;
            totalAllocPoint = totalAllocPoint.add(pool.allocPoint);
        }
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _lckReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accTokenPerShare = pool.accTokenPerShare.add(_lckReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }

    // Deposit LP tokens.
    function deposit(uint256 _pid, uint256 _amount) public onlyOneBlock nonReentrant {
        ...
        if (_amount > 0) {
            ...
            pool.token.safeTransferFrom(_sender, marketingWallet, _taxAmount);
    ...
    // Withdraw LP tokens.
    function withdraw(uint256 _pid, uint256 _amount) public onlyOneBlock nonReentrant {
        address _sender = msg.sender;
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_sender];
    ...
    function emergencyWithdraw(uint256 _pid) public onlyOneBlock nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        ...
        require(_tokenBal>0, "not balances");
