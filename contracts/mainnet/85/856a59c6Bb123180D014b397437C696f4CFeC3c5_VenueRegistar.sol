...
contract OwnedByReservUpgradable is OwnedByReservAbstract, Initializable, ContextUpgradeable {

    function __OwnedByReserv_init(address reservOwnerContract) internal onlyInitializing {
        ownerContract = reservOwnerContract;
    }
}
...
import "./OwnedByReservUpgradable.sol";
    ...
    ERC165
{
    ...
    function initialize(
        ...
        price = publicPrice;
        __OwnedByReserv_init(reservOwnerContract);
    ...
    function _authorizeUpgrade(address newImplementation) internal override onlyReserv {}

    /// @inheritdoc IVenueRegistar
    function whitelistPaymentToken(address token, bool whitelist) external virtual override onlyReserv {
        if(whitelist) _whitelistedPaymentTokens.add(token);
        else _whitelistedPaymentTokens.remove(token);
    }

    /// @inheritdoc IVenueRegistar
    function setVenueFactory(address venueFactory) external virtual override onlyReserv {
    ...
    function setTokenReviewParams(
        ...
        address tokenReviewPaymaster,
        uint256 minBalance
    ) external virtual override onlyReserv {
    ...
    /// @inheritdoc IVenueRegistar
    function setPrice(uint256 publicPrice) external virtual override onlyReserv {
        price = publicPrice;
    }
    ...
    /// @inheritdoc IVenueRegistar
    function removeVenue(address venue) external virtual override onlyReserv {
        if (!_venues.contains(venue)) revert RegistarVenueNotFound();
        _venues.remove(venue);
    }

    /// @inheritdoc IVenueRegistar
    function registerVenue(address venue) external virtual override onlyReserv {
    ...
    /// @inheritdoc IVenueRegistar
    function withdrawETH(address receiver) external virtual override onlyReserv {
