...
contract Bridge is Pausable, AccessControl {
    ...
    // destinationChainID => resourceID => deposit disabled
    mapping(uint8 => mapping(bytes32 => bool)) public _resourceIDToDepositDisabled;
    ...
    /**
        @notice Used to set availability of deposit
        @param destinationChainID ID of chain deposit will be bridged to.
        @param resourceID ResourceID to be used when making deposits.
        @param enable Is deposit enabled
     */
    function adminSetDepositAvailability(uint8 destinationChainID, bytes32 resourceID, bool enable) external onlyAdmin {
        _resourceIDToDepositDisabled[destinationChainID][resourceID] = !enable;
    }
    ...
    function deposit(uint8 destinationChainID, bytes32 resourceID, bytes calldata data) external whenNotPaused {
        require(!_resourceIDToDepositDisabled[destinationChainID][resourceID], "deposit disabled for this resource");
    ...
    /**
        ...
        @notice Emits {Deposit} event.
     */
    function depositBatch(uint8 destinationChainID, bytes32 resourceID, bytes[] calldata data) external whenNotPaused {
        require(!_resourceIDToDepositDisabled[destinationChainID][resourceID], "deposit disabled for this resource");
        ...
        for (uint i; i < data.length; i++) {
            uint64 depositNonce = ++_depositCounts[destinationChainID];
            _depositRecords[depositNonce][destinationChainID] = data[i];

            IDepositExecute depositHandler = IDepositExecute(handler);
            depositHandler.deposit(resourceID, destinationChainID, depositNonce, msg.sender, data[i]);
    ...
    function getClaimCounts(address depositer, bytes32 resourceID) external view returns (uint256 total, uint256 claimed) {
        address handler = _resourceIDToHandlerAddress[resourceID];
        require(handler != address(0), "resourceID not mapped to handler");

        IDepositExecute depositHandler = IDepositExecute(handler);
        total = depositHandler.claimOrderCount(depositer);
        claimed = depositHandler.claimOrderCurrent(depositer);
    }
...
import "../storage/BridgeStorageMeta.sol";
...
/**
    ...
    @notice This contract is intended to be used with the Bridge contract.
 */
contract ERC721HandlerMeta is IDepositExecute, HandlerHelpers {
    ...
    BridgeStorageMeta public bridgeStorage;
    ...
    /**
        ...
        @param feeTokenAddress_ Contract address ERC20 Token for fee.
     */
    constructor(
        address bridgeAddress_,
        address bridgeStorage_,
        address feeTokenAddress_
    ) public {
        bridgeAddress = bridgeAddress_;
        bridgeStorage = BridgeStorageMeta(bridgeStorage_);
    ...
    */
    ...
    function deposit(bytes32    resourceID,
        ...
        if (_burnList[tokenAddress]) {
            bridgeStorage.burnERC721(tokenAddress, tokenID);
        } else {
            bridgeStorage.lockERC721(tokenAddress, depositer, tokenID);
        }
    ...
    function executeProposal(bytes32 resourceID, bytes calldata data) external override onlyBridge {
        ...
        if (_tokenContractAddressToFee[tokenAddress] > 0) {
            bridgeStorage.addClaimOrder(recipientAddress, tokenAddress, tokenID, metaData);
            emit NewClaimOrder(recipientAddress, claimOrderCount(recipientAddress));
        }
        else {
            _executeProposal(tokenAddress, recipientAddress, tokenID, metaData);
        }

    }

    function claimOrderCount(address depositer) public override view returns (uint256) {
        return bridgeStorage.claimOrderCount(depositer);
    }

    function claimOrderCurrent(address depositer) public override view returns (uint256) {
        return bridgeStorage.claimOrderCurrent(depositer);
    }

    function claim(address depositer) external override onlyBridge {
        require(claimOrderCount(depositer) > claimOrderCurrent(depositer), "no records for claim");
    ...
    function claimAll(address depositer) external override onlyBridge returns (uint256) {
        uint256 tokens;
        while(claimOrderCount(depositer) > claimOrderCurrent(depositer)) {
            _claim(depositer);
            tokens = tokens.add(1);
        }
    ...
    function _claim(address depositer) internal {
        BridgeStorageMeta.ClaimOrderRecord memory orderRecord = bridgeStorage.getClaimOrder(depositer);

        bridgeStorage.lockERC20(feeTokenAddress, depositer, _tokenContractAddressToFee[orderRecord.tokenAddress]);
        _executeProposal(orderRecord.tokenAddress, depositer, orderRecord.tokenID, orderRecord.metaData);
    }
    ...
    function withdraw(address tokenAddress, address recipient, uint tokenID) external override onlyBridge {
        bridgeStorage.releaseERC721(tokenAddress, recipient, tokenID);
    }
    ...
    function withdrawFee(address recipient, uint amount) external override onlyBridge {
        bridgeStorage.releaseERC20(feeTokenAddress, recipient, amount);
    }
    ...
    function _executeProposal(
        ...
        if (_burnList[tokenAddress]) {
            bridgeStorage.mintERC721Meta(tokenAddress, recipientAddress, tokenID, metaData);
        }
        else {
            bridgeStorage.releaseERC721(tokenAddress, recipientAddress, tokenID);
        }

        emit ExecuteProposal(recipientAddress, tokenAddress, tokenID);
    }
}
...
import "./BridgeStorageERC721.sol";


contract BridgeStorageMeta is BridgeStorageERC721 {
    ...
    mapping (address => mapping (uint256 => ClaimOrderRecord)) public claimOrderRecords;


// Claim storage
    // TODO: maybe events

    function addClaimOrder(address depositer, address tokenAddress, uint256 tokenID, bytes memory metaData) public onlyHolder() {
        claimOrderRecords[depositer][_claimOrderCount[depositer]] = ClaimOrderRecord(
            tokenAddress,
            tokenID,
            metaData
            // false
        ...
        _claimOrderCount[depositer] = _claimOrderCount[depositer].add(1);
        // _claimOrderTotal = _claimOrderTotal.add(1);
    }

    function getClaimOrder(address depositer) public onlyHolder() returns (ClaimOrderRecord memory) {
        ClaimOrderRecord memory orderRecord = claimOrderRecords[depositer][_claimOrderCurrent[depositer]];
...
// ERC721
    ...
    function mintERC721Meta(address tokenAddress, address recipient, uint256 tokenID, bytes memory data) public onlyHolder() {
        _mintERC721Meta(tokenAddress, recipient, tokenID, data);
    }
}
...
import "./BridgeStorageBase.sol";
import "../ERC721Safe.sol";


contract BridgeStorageERC721 is ERC721Safe, BridgeStorageBase {
// ERC721

    function lockERC721(address tokenAddress, address owner, uint tokenID) public onlyHolder() {
        _lockERC721(tokenAddress, owner, tokenID);
    }

    function releaseERC721(address tokenAddress, address recipient, uint256 tokenID) public onlyHolder() {
        _releaseERC721(tokenAddress, recipient, tokenID);
    }

    function burnERC721(address tokenAddress, uint256 tokenID) public onlyHolder() {
        _burnERC721(tokenAddress, tokenID);
    }
}
...
contract BridgeStorageBase is AccessControl, ERC721Safe, ERC20Safe {
    using SafeMath for uint256;
    // uint256 private _claimOrderTotal = 0;

    // recipient => count
    mapping (address => uint256) internal _claimOrderCount;
    // recipient => current index
    mapping (address => uint256) internal _claimOrderCurrent;


    bytes32 public constant HOLDER_ROLE = keccak256("HOLDER_ROLE");
    modifier onlyHolder() {
        require(hasRole(HOLDER_ROLE, msg.sender), "sender doesn't have holder role");
        _;
    }

    constructor() public {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setRoleAdmin(HOLDER_ROLE, DEFAULT_ADMIN_ROLE);
    }


// Claim storage

    function claimOrderCount(address depositer) public virtual view returns(uint256) {
        return _claimOrderCount[depositer];
    }

    function claimOrderCurrent(address depositer) public virtual view returns(uint256) {
...
// ERC20
    ...
    function lockERC20(address tokenAddress, address owner, uint256 amount) public onlyHolder() {
        _lockERC20(tokenAddress, owner, amount);
    }

    function releaseERC20(address tokenAddress, address recipient, uint256 amount) public onlyHolder() {
        _releaseERC20(tokenAddress, recipient, amount);
    }

    function burnERC20(address tokenAddress, address owner, uint256 amount) public onlyHolder() {
        _burnERC20(tokenAddress, owner, amount);
    }

    function mintERC20(address tokenAddress, address recipient, uint256 amount) public onlyHolder() {
        _mintERC20(tokenAddress, recipient, amount);
    }
}

pragma solidity 0.6.4;

// import "@openzeppelin/contracts/token/ERC721/extension/ERC721Burnable.sol";
// import "@openzeppelin/contracts/presets/ERC721PresetMinterPauserAutoId.sol";
import "@openzeppelin/contracts/token/ERC721/IERC721.sol";
import "./interfaces/IERC721Burnable.sol";
import "./interfaces/IERC721Mintable.sol";
import "./interfaces/IERC721AutoId.sol";
...
contract ERC721Safe {
     ...
     */
    function _lockERC721(address tokenAddress, address owner, uint tokenID) internal {
    ...
    /**
        ...
        @param tokenID ID of token to transfer.
     */
    function _releaseERC721(address tokenAddress, address recipient, uint256 tokenID) internal {
        IERC721 erc721 = IERC721(tokenAddress);
     ...
     */
    function _mintERC721(address tokenAddress, address recipient, uint256 tokenID) internal {
        IERC721Mintable erc721 = IERC721Mintable(tokenAddress);
        erc721.mint(recipient, tokenID);
    }
    ...
    /**
        ...
        @param recipient Address to mint token to.
     */
    function _mintERC721AutoId(address tokenAddress, address recipient) internal {
        IERC721AutoId erc721 = IERC721AutoId(tokenAddress);
        erc721.mint(recipient);
    }

    /**
        @notice Used to create new ERC721 with Meta.
        @param tokenAddress Address of ERC721 to mint.
        @param recipient Address to mint token to.
        @param tokenID ID of token to mint.
        @param data Optional data to send along with mint call.
     */
    function _mintERC721Meta(address tokenAddress, address recipient, uint256 tokenID, bytes memory data) internal {
        IERC721Meta erc721 = IERC721Meta(tokenAddress);
    ...
    function _burnERC721(address tokenAddress, uint256 tokenID) internal {
        IERC721Burnable erc721 = IERC721Burnable(tokenAddress);
        erc721.burn(tokenID);
    }
...
contract ERC20Safe {
     ...
     */
    function _lockERC20(address tokenAddress, address owner, uint256 amount) internal {
    ...
    /**
        ...
        @param amount Amount of tokens to transfer.
     */
    function _releaseERC20(address tokenAddress, address recipient, uint256 amount) internal {
        IERC20 erc20 = IERC20(tokenAddress);
     ...
     */
    function _mintERC20(address tokenAddress, address recipient, uint256 amount) internal {
     ...
     */
    function _burnERC20(address tokenAddress, address owner, uint256 amount) internal {
...
interface IERC721Meta {
    function mint(address to, uint256 tokenId, string calldata _data) external;
}
    ...
    IERC721Enumerable
{
    ...
    function name() external view override returns (string memory) {
        return "Gravis Cards Collection TEST";
    }
...
interface IGravisCollectible is IERC721 {
    ...
    function isOwner(address who, uint256 tokenID) external view returns (bool);
}
...
contract CollectibleSafe {
    /**
        ...
        @param tokenID ID of token to transfer.
     */
    function _lockCollectible(address tokenAddress, address owner, uint tokenID) internal {
    ...
    /**
        ...
        @param tokenID ID of token to transfer.
     */
    function _releaseCollectible(address tokenAddress, address recipient, uint256 tokenID) internal {
        IGravisCollectible collectible = IGravisCollectible(tokenAddress);
        collectible.transferFrom(address(this), recipient, tokenID);
    }
     ...
     */
    function _mintCollectible(address tokenAddress, address recipient, uint256 tokenType) internal returns (uint256) {
        IGravisCollectible collectible = IGravisCollectible(tokenAddress);
    ...
    /**
        ...
        @param tokenAddress Address of ERC721.
        @param tokenID ID of token.
     */
    function getTokenType(address tokenAddress, uint256 tokenID) public view returns (uint256) {
        IGravisCollectible collectible = IGravisCollectible(tokenAddress);
    ...
    /**
        ...
        @param tokenID ID of token.
     */
    function isTokenOwner(address tokenAddress, address who, uint256 tokenID) public view returns (bool) {
        IGravisCollectible collectible = IGravisCollectible(tokenAddress);
...
import "./BridgeStorageBase.sol";
import "../CollectibleSafe.sol";


contract BridgeStorageCollectible is BridgeStorageBase, CollectibleSafe {
    struct ClaimOrderRecord {
        address tokenAddress;
        uint256 tokenID;
        uint256 tokenType;
    ...
    // TokenId => TokenId
    mapping (uint256 => uint256) public swapsDeposit;
    mapping (uint256 => uint256) public swapsProposal;


// Swaps storage

    function setSwapsDeposit(uint256 depositTokenId, uint256 proposalTokenId) public onlyHolder() {
       swapsDeposit[depositTokenId] = proposalTokenId;
    }

    function setSwapsProposal(uint256 proposalTokenId, uint256 depositTokenId) public onlyHolder() {
       swapsProposal[proposalTokenId] = depositTokenId;
    }
...
// Claim storage
    ...
    function addClaimOrder(address depositer, address tokenAddress, uint256 tokenID, uint256 tokenType, bool mintFlag) public onlyHolder() {
        claimOrderRecords[depositer][_claimOrderCount[depositer]] = ClaimOrderRecord(
            tokenAddress,
            tokenID,
            tokenType,
...
// Collectible
    ...
    function lockCollectible(address tokenAddress, address owner, uint tokenID) public onlyHolder() {
        _lockCollectible(tokenAddress, owner, tokenID);
    }

    function releaseCollectible(address tokenAddress, address recipient, uint256 tokenID) public onlyHolder() {
        _releaseCollectible(tokenAddress, recipient, tokenID);
    }

    function mintCollectible(address tokenAddress, address recipient, uint256 tokenType) public onlyHolder() returns (uint256) {
        _mintCollectible(tokenAddress, recipient, tokenType);
    }
}
...
import "../interfaces/IDepositExecute.sol";
import "./HandlerHelpers.sol";
import "../storage/BridgeStorageCollectible.sol";
...
/**
	...
	@notice This contract is intended to be used with the Bridge contract.
 */
contract CollectibleHandler is IDepositExecute, HandlerHelpers {
	using ERC165Checker for address;
	using SafeMath for uint256;
	...
	BridgeStorageCollectible public bridgeStorage;
	...
	constructor(
		...
		bridgeAddress = bridgeAddress_;
		bridgeStorage = BridgeStorageCollectible(bridgeStorage_);
		feeTokenAddress = feeTokenAddress_;
	}
	...
	*/
	...
	function deposit(bytes32    resourceID,
		...
		require(claimOrderCount(depositer) == claimOrderCurrent(depositer), "has records for claim"); // need claim first
		...
		tokenType = bridgeStorage.getTokenType(tokenAddress, tokenID);
		bridgeStorage.lockCollectible(tokenAddress, depositer, tokenID);


		uint256 swapTokenId = bridgeStorage.swapsProposal(tokenID.add(1));
		if (swapTokenId == 0) {
	...
	function executeProposal(bytes32 resourceID, bytes calldata data) external override onlyBridge {
		...
		address recipientAddress = address(recipient);
		if (_tokenContractAddressToFee[tokenAddress] > 0) {
			bridgeStorage.addClaimOrder(recipientAddress, tokenAddress, tokenID, tokenType, _mintFlag);
		}
		else {
	...
	function _claim(address depositer) internal {
		BridgeStorageCollectible.ClaimOrderRecord memory orderRecord = bridgeStorage.getClaimOrder(depositer);
	...
	/**
		@notice Used to manually release ERC721 tokens from CollectibleSafe.
		@param tokenAddress Address of token contract to release.
		@param recipient Address to release token to.
		@param tokenID The ERC721 token ID to release.
	 */
	function withdraw(address tokenAddress, address recipient, uint tokenID) external override onlyBridge {
		bridgeStorage.releaseCollectible(tokenAddress, recipient, tokenID);
	}
	...
	function _executeProposal(
		...
		uint256 swapTokenId = bridgeStorage.swapsDeposit(depositTokenId);
		if (_mintFlag) {
			_mintNewToken(tokenAddress, recipientAddress, tokenType, depositTokenId);
		}
		else if (swapTokenId > 0 && bridgeStorage.isTokenOwner(tokenAddress, address(this), swapTokenId.sub(1))) {
			bridgeStorage.releaseCollectible(tokenAddress, recipientAddress, swapTokenId.sub(1));
		}
		else if(bridgeStorage.swapsProposal(depositTokenId) == 0
			&& bridgeStorage.isTokenOwner(tokenAddress, address(this), tokenID)
			&& tokenType == bridgeStorage.getTokenType(tokenAddress, tokenID) // TODO: maybe other impossible
		) {
			bridgeStorage.releaseCollectible(tokenAddress, recipientAddress, tokenID);
		}
		else {
			_mintNewToken(tokenAddress, recipientAddress, tokenType, depositTokenId);
		}
	...
	function _mintNewToken(
		...
		uint256 proposalTokenId = bridgeStorage.mintCollectible(tokenAddress, recipientAddress, tokenType); // TODO: maybe add(1)
		bridgeStorage.setSwapsDeposit(depositTokenId, proposalTokenId);
		bridgeStorage.setSwapsProposal(proposalTokenId, depositTokenId);
	}
}
...
/**
	...
	@notice This contract is intended to be used with the Bridge contract.
 */
contract ERC721Handler is IDepositExecute, HandlerHelpers {
	using SafeMath for uint256;
	...
	constructor(
		...
		bridgeAddress = bridgeAddress_;
		bridgeStorage = BridgeStorage(bridgeStorage_);
		feeTokenAddress = feeTokenAddress_;
	}
	...
	*/
	...
	/**
		...
		@param depositNonce This value is generated as an ID by the Bridge contract.
		@param depositer Address of account making the deposit in the Bridge contract.
		@param data Consists of: {resourceID}, {tokenID}, {lenRecipientAddress}, and {recipientAddress}
	...
	function executeProposal(bytes32 resourceID, bytes calldata data) external override onlyBridge {
		...
		if (_tokenContractAddressToFee[tokenAddress] > 0) {
			bridgeStorage.addClaimOrder(recipientAddress, tokenAddress, tokenID);
			emit NewClaimOrder(recipientAddress, claimOrderCount(recipientAddress));
		}
		else {
			_executeProposal(tokenAddress, recipientAddress, tokenID);
		}
	...
	function _claim(address depositer) internal {
		BridgeStorage.ClaimOrderRecord memory orderRecord = bridgeStorage.getClaimOrder(depositer);

		bridgeStorage.lockERC20(feeTokenAddress, depositer, _tokenContractAddressToFee[orderRecord.tokenAddress]);
		_executeProposal(orderRecord.tokenAddress, depositer, orderRecord.tokenID);
	}
	...
	function _executeProposal(
		...
		if (_burnList[tokenAddress]) {
			// mintERC721AutoId(tokenAddress, recipientAddress);
			bridgeStorage.mintERC721(tokenAddress, recipientAddress, tokenID);
		}
		else {
			bridgeStorage.releaseERC721(tokenAddress, recipientAddress, tokenID);
		}
...
contract BridgeStorage is BridgeStorageERC721 {
    struct ClaimOrderRecord {
        address tokenAddress;
        uint256 tokenID;
        // bool claimed;
...
// Claim storage
    ...
    function addClaimOrder(address depositer, address tokenAddress, uint256 tokenID) public onlyHolder() {
        claimOrderRecords[depositer][_claimOrderCount[depositer]] = ClaimOrderRecord(
            tokenAddress,
            tokenID
            // false
...
// ERC721
    ...
    function mintERC721(address tokenAddress, address recipient, uint256 tokenID) public onlyHolder() {
        _mintERC721(tokenAddress, recipient, tokenID);
    }

    function mintERC721AutoId(address tokenAddress, address recipient) public onlyHolder() {
        _mintERC721AutoId(tokenAddress, recipient);
    }
}
