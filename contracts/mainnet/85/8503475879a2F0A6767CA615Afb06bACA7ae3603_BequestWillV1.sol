...
contract BequestWillV1 is Ownable, ReentrancyGuard {
   ...
   * @param owner: The owner of the Bequest
   */
  event AddedExecutor(address indexed executor, address owner);
  ...
  uint256 private bequestYearlyFee = 0.01 ether;
    ...
    if (bytes(_referral).length != 0) {
      ...
      if (referralDetails.profitShare > 0) {
        referralDetails.profit +=
          (creationFee * referralDetails.profitShare) /
          100;
      }
    ...
    onlyExecutor(_owner)
  {
    ...
    emit AddedExecutor(_executor, _owner);
  }
   ...
   */
  function extractFees(uint256 _amount) external onlyOwner {
