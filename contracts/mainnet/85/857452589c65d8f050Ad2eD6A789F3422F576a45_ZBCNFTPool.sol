...
contract ZBCNFTPool {
    ...
    constructor(address zbc_, address nft_) {
    ...
    function withdrawToken() external lock {
        ...
        userInfo.bonus = 0;
        IERC20(sys.usdtToken).transfer(msg.sender,amountWith);
        emit WithdrawToken(msg.sender,amountWith);
    }

    function updateUserNFT(address from, address to) external {
        _updatePool();
        users[from].nftNums = IERC721(sys.zbcNFT).balanceOf(from);
        users[to].nftNums = IERC721(sys.zbcNFT).balanceOf(to);
        _add(from);
        _add(to);
        _remove(from);
        _remove(to);
    ...
    function _updatePool() private {
        (uint256 avgAmount,uint256 lastTime,uint256 pendingAmount) = _pendingAvgAmount();
        if (sys.lastTime == lastTime) {
            return;
        }
        sys.lastTime = lastTime;
        if (avgAmount > 0) {
            depositTotal = depositTotal.add(pendingAmount);
            for (uint256 i = 0; i < userArr.length; i++) {
                users[userArr[i]].bonus = users[userArr[i]].bonus.add(avgAmount.mul(users[userArr[i]].nftNums));
            }
        }
    }

    function _pendingAvgAmount() private view returns(uint256 avgAmount,uint256 lastTime,uint256 pendingAmount) {
        lastTime = getLastTime(sys.startTime,block.timestamp);
        if (sys.lastTime == lastTime) {
            return (0,lastTime,0);
        }
        uint256 balance = IERC20(sys.usdtToken).balanceOf(address(this));
        pendingAmount = balance.add(debtTotal).sub(depositTotal);
        if (balance == 0 || sys.totalNFT == 0 || lastTime == sys.lastTime) {
            return(0,lastTime,0);
        }
        return (pendingAmount.div(sys.totalNFT), lastTime, pendingAmount);
    }

    function pendingReward(address addr) external view returns(uint256) {
        (uint256 avgAmount,,) = _pendingAvgAmount();
        return users[addr].bonus.add(avgAmount.mul(users[addr].nftNums));
    }
