...
contract MidasRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 depositFeeBP; //how many based point of depositFee charged, cannot be larger than 100
        uint256 withdrawFeeBP; //how many based point of withdrawFee charged, cannot be larger than 100
    }
    ...
    uint256 public tSharePerSecond = 0.00273277 ether; // 42500 tshare / (180 days * 24h * 60min * 60s)
    ...
    function add(
        ...
        require(_depositFeeBP >= 0 && _depositFeeBP <=100, "depositfee out of range");
        require(_withdrawFeeBP >= 0 && _withdrawFeeBP <=100, "withdrawfee out of range");
        bool _isStarted =
        (_lastRewardTime <= poolStartTime) ||
        (_lastRewardTime <= block.timestamp);
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint256 _depositFeeBP, uint256 _withdrawFeeBP) public onlyOperator {
        require(_depositFeeBP >= 0 && _depositFeeBP <=100, "depositfee out of range");
        require(_withdrawFeeBP >= 0 && _withdrawFeeBP <=100, "withdrawfee out of range");
        massUpdatePools();
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            uint256 _depositDebt = _amount.mul(pool.depositFeeBP).div(10000);
            pool.token.safeTransferFrom(_sender, address(this), _amount);
            user.amount = user.amount.add(_amount.sub(_depositDebt));
            pool.token.safeTransfer(daoFundAddress, _depositDebt);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            uint256 _withdrawDebt = _amount.mul(pool.withdrawFeeBP).div(10000);
            pool.token.safeTransfer(daoFundAddress, _withdrawDebt);
            user.amount = user.amount.sub(_amount);
            pool.token.safeTransfer(_sender, _amount.sub(_withdrawDebt));
        } 
        user.rewardDebt = user.amount.mul(pool.accTSharePerShare).div(1e18);
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        if (_pid > 1) {
            pool.token.safeTransfer(daoFundAddress, _withdrawDebt);
            pool.token.safeTransfer(msg.sender, _amount.sub(_withdrawDebt));
            emit EmergencyWithdraw(msg.sender, _pid, _amount.sub(_withdrawDebt));
        } else if (_pid <= 1) {
