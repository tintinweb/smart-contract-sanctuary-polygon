...
contract ApeYoungLeague is ERC721 {
    ...
    constructor()
        ERC721("Ape Young League", "AYL")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 5600) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeihxgtwyv2lzz52fktr6bnruyvsvnzfdfpx6r3rog3skpjt6zxbawu/", Strings.toString(tokenId), ".json")
            );
    }
