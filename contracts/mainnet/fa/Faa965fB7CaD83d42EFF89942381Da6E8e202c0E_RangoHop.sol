...
import "./IHop.sol";
import "../../rango/bridges/hop/IRangoHop.sol";
import "./RangoHopModels.sol";

/// @title The root contract that handles Rango's interaction with Hop bridge
/// @author Uchiha Sasuke
/// @dev This is deployed as a separate contract from RangoV1
contract RangoHop is IRangoHop, BaseContract {
    ...
    event HopBridgeSent(
        ...
        RangoHopModels.ActionType actionType,
        uint256 chainId,
        address recipient,
        uint256 amount,
        uint256 bonderFee,
    ...
    function addHopBridges(address[] calldata _addresses) external onlyOwner {
    ...
    function hopBridge(
        ...
        if (_fromToken == NULL_ADDRESS) {
            ...
            SafeERC20.safeTransferFrom(IERC20(_fromToken), msg.sender, address(this), _amount);
            approve(_fromToken, _request.bridgeAddress, _amount);
        }

        IHop hop = IHop(_request.bridgeAddress);
        if (_request.actionType == RangoHopModels.ActionType.SWAP_AND_SEND) {
        ...
        } else if (_request.actionType == RangoHopModels.ActionType.SEND_TO_L2) {
