...
contract DynastyExchangeVentures is Context, Ownable {
    ...
    uint256 private numDays = 30;
	
	uint16 constant PROFIT_MONTHLY = 4;
    ...
    constructor() {         
	    ceo = payable(0x4A424C70Ff7b9E53A775a822979D432580723cDC);
    ...
    function computePayout(address _addr) view external returns(uint256 value) {
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            if(from < to) {
                value += dep.amount * (to - from) * PROFIT_MONTHLY / PERCENT_DIVIDER / (30 days);
            }
        }
        return value;
    }
	...
	function withdrawInitial() public returns (bool success) {
        ...
        uint256 teamFee;
        uint256 penaltyFee;
        if (player.deposits[0].time + 1 days > block.timestamp) { // in 24 hours: 2%
            penaltyFee = refund * 2 / 100;
        } else if (player.deposits[0].time + 90 days > block.timestamp) { // 1 days ~ 90 days: 50%
            penaltyFee = refund * 50 / 100;
        } else { // 90 days~ : 0% penalty
            penaltyFee = 0;
        }
        teamFee = refund / 100;
