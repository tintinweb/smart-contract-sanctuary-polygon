...
contract FoodStake is Ownable {
    ...
    function _buyTickets(address userAddress, uint256 amount) private {
        ...
        uint256 numTickets = amount/(RAFFLE_TICKET_PRICE);
        ...
        uint256 RAFFLEAmount = amount*(RAFFLE_PERCENT)/(PERCENTS_DIVIDER);
        ...
        uint256 userTickets = ticketOwners[RAFFLERound][msg.sender];
        uint256 numTickets = amount/(RAFFLE_TICKET_PRICE);
        
        //if the user has no tickets before this point, but they just purchased a ticket
        if(userTickets == 0) {
            participantAdresses[RAFFLERound][participants] = msg.sender;
        ...
        ticketOwners[RAFFLERound][msg.sender] = userTickets+(numTickets);
        uint256 RAFFLEAmount = amount*MANLottoPrice;
        currentPot = currentPot+(RAFFLEAmount);
        totalTickets = totalTickets+(numTickets);
    ...
    function _payRAFFLEWinner(address userAddress) private {
		...
		currentPot = currentPot-(projectFee);
