...
/*! meta grow.sol | (c) 2022 Develop by BelovITLab LLC (smartcontract.ru), author @stupidlovejoy | SPDX-License-Identifier: MIT License */
...
contract metagrow is Ownable, Destructible, Pausable {
    struct User {
        uint256 cycle;
        address upline;
        uint256 referrals;
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        else require(_amount >= 100 ether && _amount <= cycles[0], "Bad amount");
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout = (users[_addr].deposit_amount * ((block.timestamp - users[_addr].deposit_time) / 1 days) / 33) - users[_addr].deposit_payouts;
