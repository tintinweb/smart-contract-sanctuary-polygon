...
contract LiquidityManager is BaseContract {
  struct Config {
  ...
  modifier onlySwapContract() {
    require(msg.sender == swapAddress, "LiquidityManager: only swap contract");
    _;
  }
  ...
  function swapUSDForToken(uint _usdAmount) external onlySwapContract returns (uint) {
    uint tokenAmount = _usdAmount * DECIMAL3 / _getTokenPrice();
    uint contractBalance = menToken.balanceOf(address(this));
    require(contractBalance >= config.minTokenReserved, "LiquidityManager: contract insufficient balance");
    if(contractBalance < tokenAmount) {
      menToken.releaseLMSAllocation(tokenAmount - contractBalance);
    }
    menToken.transfer(msg.sender, tokenAmount);
    return tokenAmount;
  }

  function swapTokenForUSDT(uint _amount) external onlySwapContract returns (uint) {
    uint usdtAmount = _amount * _getTokenPrice() / DECIMAL3;
    uint contractBalance = usdtToken.balanceOf(address(this));
    require(contractBalance > usdtAmount && contractBalance >= config.minUsdtReserved, "LiquidityManager: contract insufficient balance");
  ...
  function buyToken(uint _amount) external onlyContractCall {
    require(usdtToken.balanceOf(address(this)) >= _amount, "LiquidityManager: contract insufficient balance");
  ...
  function sellToken(uint _amount) external onlyContractCall {
    ...
    if (contractBalance < _amount) {
      menToken.releaseLMSAllocation(_amount - contractBalance);
    }
    address[] memory path = new address[](2);
  ...
  function addLiquidity(uint _usdtAmount, uint _lpTokenBurnPercentage) external onlyContractCall {
    require(_lpTokenBurnPercentage <= 100 * DECIMAL3, "LiquidityManager: burn percentage invalid");
    require(usdtToken.balanceOf(address(this)) >= _usdtAmount, "LiquidityManager: contract insufficient usdt balance");
    uint tokenAmount = _usdtAmount * DECIMAL3 / _getTokenPrice();
    uint contractBalance = menToken.balanceOf(address(this));
    if (contractBalance < tokenAmount) {
      menToken.releaseLMSAllocation(tokenAmount - contractBalance);
    }
  ...
  function removeLiquidity(uint _lpToken) external onlyMn {
    require(_lpToken <= lpToken.balanceOf(address(this)), "LiquidityManager: contract insufficient balance");
  ...
  function convertBusdToUsdt(uint _busdAmount) external onlyMn {
    require(busdToken.balanceOf(address(this)) >= _busdAmount, "LiquidityManager: contract insufficient balance");
    address[] memory path = new address[](2);
    path[0] = address(busdToken);
    path[1] = address(usdtToken);
    uniswapV2Router.swapExactTokensForTokens(_busdAmount, 0, path, address(this), block.timestamp);
  }

  function convertUsdcOrDaiToUsdt(uint _usdAmount, bool isUsdcToken) external onlyMn {
    IBEP20 usdToken = isUsdcToken ? usdcToken : daiToken;

    require(usdToken.balanceOf(address(this)) >= _usdAmount, "LiquidityManager: contract insufficient balance");
    address[] memory path = new address[](2);
    path[0] = address(usdToken);
    path[1] = address(usdtToken);
    uniswapV2Router.swapExactTokensForTokens(_usdAmount, 0, path, address(this), block.timestamp);
  }

  function mint(uint _amount) external onlyMn {
    menToken.releaseLMSAllocation(_amount);
  }
