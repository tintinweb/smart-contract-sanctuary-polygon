  ...
  RoyaltiesV2Impl
{
   ...
   */
  constructor(address _proxyRegistryAddress) ERC721("SoulablesTest", "SLBS") {
    proxyRegistryAddress = _proxyRegistryAddress;
    _initializeEIP712("SoulablesTest");
  }

  /**
   * @dev Contract owner mints the token
   */
...
abstract contract AbstractRoyalties {
  ...
  function _saveRoyalties(uint256 _id, LibPart.Part[] memory _royalties) internal {
    for (uint i = 0; i < _royalties.length; i++) {
      require(_royalties[i].account != address(0), "");
      require(_royalties[i].value != 0, "");
