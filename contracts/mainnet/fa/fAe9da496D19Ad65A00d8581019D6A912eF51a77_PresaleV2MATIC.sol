...
contract PresaleV2MATIC is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
  ...
  function buyWithUSDC(uint256 amount, uint256 lockup_months) external checkSaleState(amount, _msgSender()) whenNotPaused returns (bool) {
    uint256 usdPrice = usdtBuyHelper6(amount);
    ...
    (bool success, ) = address(USDC).call(abi.encodeWithSignature('transferFrom(address,address,uint256)', _msgSender(), owner(), usdPrice));
    require(success, 'Token payment failed');
    emit TokensBought(_msgSender(), amount, address(USDC), (newBonus), usdPrice, usdEq, block.timestamp);
    return true;
  }

  /**
   * @dev To buy into a presale using BUSD
   * @param amount No of tokens to buy
   */
  function buyWithBUSD(uint256 amount, uint256 lockup_months) external checkSaleState(amount, _msgSender()) whenNotPaused returns (bool) {
    uint256 usdPrice = usdtBuyHelper18(amount);
    uint256 usdEq = stagePrice * amount;
    require(usdEq <= 50001000000000000000000, 'Max limit for one sale');
    uint256 newBonus = update(amount, lockup_months, usdEq, _msgSender());
    uint256 ourAllowance = BUSD.allowance(_msgSender(), address(this));
    require(usdPrice <= ourAllowance, 'Make sure to add enough allowance');
    (bool success, ) = address(BUSD).call(abi.encodeWithSignature('transferFrom(address,address,uint256)', _msgSender(), owner(), usdPrice));
    require(success, 'Token payment failed');
    emit TokensBought(_msgSender(), amount, address(BUSD), (newBonus), usdPrice, usdEq, block.timestamp);
    return true;
  }
  ...
  function buyWithUSDT(uint256 amount, uint256 lockup_months) external checkSaleState(amount, _msgSender()) whenNotPaused returns (bool) {
    ...
    require(usdPrice <= ourAllowance, 'Make sure to add enough allowance');
    (bool success, ) = address(USDT).call(abi.encodeWithSignature('transferFrom(address,address,uint256)', _msgSender(), owner(), usdPrice));
    require(success, 'Token payment failed');
    emit TokensBought(_msgSender(), amount, address(USDT), (newBonus), usdPrice, usdEq, block.timestamp);
    return true;
  }

  /**
   * @dev To buy into a presale using MATIC
   * @param amount No of tokens to buy
   */
   ...
   * @param amount No of tokens to buy
   */
  function buyWithCoinDirectPay(uint256 amount, address buyer) external payable checkSaleState(amount, _msgSender()) whenNotPaused nonReentrant returns (bool) {
    uint256 ethAmount = ethBuyHelper(amount);
    uint256 usdEq = stagePrice * amount;
    require(usdEq <= 50001000000000000000000, 'Max limit for one sale');
    require(msg.value >= ethAmount, 'Less payment');
    uint256 excess = msg.value - ethAmount;
    uint256 newBonus = update(amount, 0, usdEq, buyer);
    sendValue(payable(owner()), ethAmount);
    if (excess > 0) sendValue(payable(buyer), excess);
    emit TokensBought(_msgSender(), amount, address(0), (newBonus), ethAmount, usdEq, block.timestamp);
    return true;
  }
   ...
   * @param amount No of tokens to buy
   */
  function usdtBuyHelper6(uint256 amount) public view returns (uint256 usdPrice) {
    usdPrice = (amount * stagePrice) / (10 ** 12);
  }

  function usdtBuyHelper18(uint256 amount) public view returns (uint256 usdPrice) {
    usdPrice = (amount * stagePrice);
  }
