...
contract StrategyVDfyn is Ownable, RouterCrossTalk {
    ...
    constructor (
        ...
        address _handlerReserve) RouterCrossTalk(_handler) public {
        ...
        want = _want;
        vdfyn = _farmAddress;
        handlerReserve = _handlerReserve;
        setLink(msg.sender);
    ...
    function _routerSyncHandler(bytes4 _interface, bytes memory _data)
        ...
        if(bytes4(keccak256("stake(_amount)")) == _interface) {
            ...
            balance(_v1, _v2, _v3);
            return (true, "");
        } else if (bytes4(keccak256("unstake(_amount)")) == _interface) {
            (
                uint256 _v1,
                uint256 _v2,
                uint256 _v3,
    ...
    function withdraw(
        ...
        // require(msg.sender == controller, "!controller");
        uint256 r = (_reserveBalance.add(balanceOf())).mul(_shares).div(_totalSupply);
        uint256 _balance = IERC20(vdfyn).balanceOf(address(this));

        require(_balance >= r, "!balance");

        IERC20(vdfyn).approve(vdfyn, r);
        IvDFYN(vdfyn).leave(r);
        IERC20(want).approve(handlerReserve, type(uint256).max);
        uint256[] memory x;
        address[] memory y;
        IBridge(bridge).deposit(chainId, _resourceId, _data, x, x, y, want);
    }
