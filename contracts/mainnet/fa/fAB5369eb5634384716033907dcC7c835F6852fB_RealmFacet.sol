...
import { Modifiers, im_diamondAddress, im_realmAddress } from "../libraries/LibAppStorage.sol";
...
contract RealmFacet is Modifiers{
    ...
    function setParcelsAccessRights(
        ...
        require(_realmIds.length == _actionRights.length && _actionRights.length == _accessRights.length, "setParcelsAccessRights: array mismatch");

        //if the sender is not an approved user, make sure they're the depositor of all realm parcels
        if(s.approvedUsers[msg.sender] == false){
            for(uint256 i = 0; i < _realmIds.length; ){
                require(s.deposits[im_realmAddress][_realmIds[i]] == msg.sender, "ownerSetParcelsAccessRights: not realm depositor");
                unchecked{
                    i++;
                }
            }
        ...
        IRealm(im_realmAddress).setParcelsAccessRights(_realmIds,_actionRights,_accessRights);
    }
