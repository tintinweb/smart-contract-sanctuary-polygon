...
function buySellOrder(bytes32 _orderId, uint256 _value) external onlyIfSenderNotContract sellOrderExists(_orderId) {
  ...
  function createBuyOrder(
    ...
    // Generate pseudorandom order ID
    bytes32 orderId = keccak256(abi.encodePacked(block.timestamp, msg.sender, _cardType, _bid));

    //save buyOrder money in contract
    bool success = eft.transferFrom(msg.sender,  address(this), _bid);
  ...
  * @notice Cancel buy order
  ...
  function cancelBuyOrder(bytes32 _orderId) external buyOrderExists(_orderId) senderOwnsBuyOrder(_orderId) {
    ...
    //Refund Buy order
    bool success = eft.transfer(buyOrder.owner, buyOrder.bid);
    require(success, "Refund previous offeror failed.");
