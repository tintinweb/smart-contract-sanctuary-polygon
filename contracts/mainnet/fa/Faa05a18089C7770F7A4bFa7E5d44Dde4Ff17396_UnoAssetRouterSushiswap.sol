...
contract UnoAssetRouterSushiswap is Initializable, PausableUpgradeable, UUPSUpgradeable {
    ...
    receive() external payable {
        require(msg.sender == WMATIC || msg.sender == address(SushiswapRouter), 'ONLY_ACCEPT_WMATIC_OR_ROUTER'); // only accept ETH via fallback from the WMATIC or router contract
    }
    ...
    function withdrawSingleETH(address lpPair,  uint256 amount, bytes[2] calldata swapData, address recipient) external returns(uint256 amountETH, uint256 amountA, uint256 amountB){
        ...
        if (tokenA != WMATIC) {
            IERC20Upgradeable(tokenA).approve(OneInchRouter, amountA);
            (uint256 returnAmount, uint256 spentAmount) = _swap(swapData[0], WMATIC);
        ...
        if (tokenB != WMATIC) {
            IERC20Upgradeable(tokenB).approve(OneInchRouter, amountB);
            (uint256 returnAmount, uint256 spentAmount) = _swap(swapData[1], WMATIC);
            amountETH += returnAmount;
    ...
    function _addLiquidity(
        ...
        IERC20Upgradeable(tokenA).approve(address(SushiswapRouter), amountA);
        IERC20Upgradeable(tokenB).approve(address(SushiswapRouter), amountB);

        (sentA, sentB, liquidity) = SushiswapRouter.addLiquidity(tokenA, tokenB, amountA, amountB, amountAMin, amountBMin, farm, block.timestamp);
        // Refund dust
    ...
    function _addLiquidityETH(
        ...
        IERC20Upgradeable(token).approve(address(SushiswapRouter), amount);

        (sentToken, sentETH, liquidity) = SushiswapRouter.addLiquidityETH{value: msg.value}(token, amount, amountTokenMin, amountETHMin, farm, block.timestamp);
        // Refund dust
    ...
    function _removeLiquidity(
        ...
        IERC20Upgradeable(lpPair).approve(address(SushiswapRouter), amount);
    ...
    function _removeLiquidityETH(
        ...
        IERC20Upgradeable(lpPair).approve(address(SushiswapRouter), amount);
        (amountToken, amountETH) = SushiswapRouter.removeLiquidityETH(token, amount, amountTokenMin, amountETHMin, recipient, block.timestamp);
    }
