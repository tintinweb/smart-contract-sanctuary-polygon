...
// Quickswap BNB ApeSwap only version
...
library ApeOnlyPriceGetter {
    ...
    bytes32 public constant INITCODEHASH =
        hex"96e8ac4277198ff8b6f785478aa9a39f403cb768dd02cbee326c3e7da348845f"; // for pairs created by ApeFactory
    ...
    //Ape LP addresses
    address constant USDC_WMATIC_PAIR = 0x6e7a5FAFcec6BB1e78bAE2A1F0B612012BF14827; // usdc is token1
    address constant DAI_WMATIC_PAIR = 0xEEf611894CeaE652979C9D0DaE1dEb597790C6eE; // dai is token1
    address constant USDT_WMATIC_PAIR = 0x604229c960e5CACF2aaEAc8Be68Ac07BA9dF81c3; // usdt is token1
    ...
    function getNativePrice() public view returns (uint256) {
        (uint256 wbnbReserve0, uint256 daiReserve, ) = IApePair(DAI_WMATIC_PAIR)
            .getReserves();

        (uint256 wbnbReserve1, uint256 usdcReserve, ) = IApePair(USDC_WMATIC_PAIR)
            .getReserves();

        (uint256 wbnbReserve2, uint256 usdtReserve, ) = IApePair(USDT_WMATIC_PAIR)
            .getReserves();

        uint256 wbnbTotal = wbnbReserve0 + wbnbReserve1 + wbnbReserve2;
        uint256 usdTotal = (daiReserve + usdtReserve) * PRECISION + usdcReserve * PRECISION / USDC_RAW_PRICE * PRECISION;

        return usdTotal / wbnbTotal;
    }
        ...
        returns (uint256)
    {
        ...
        if (token == WMATIC) return nativePrice;
        return 0;
    }
    ...
    function pegTokenPrice(address token) private view returns (uint256) {
        if (token == USDC) return PRECISION*1e12;
        if (token == USDT || token == DAI) return PRECISION;
        if (token == WMATIC) return getNativePrice();
        return 0;
    }
