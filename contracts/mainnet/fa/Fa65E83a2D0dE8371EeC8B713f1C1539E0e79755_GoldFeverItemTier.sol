    ...
    UUPSUpgradeable
{
    ...
    function initialize(
        ...
        __GoldFeverItemTier_init(admin, nftContract_, nglContract_, itemType_);
    }

    function __GoldFeverItemTier_init(
        address admin,
        address nftContract_,
        address nglContract_,
        address itemType_
        ...
        __GoldFeverItemTier_init_unchained(
            admin,
            nftContract_,
            nglContract_,
            itemType_
    ...
    function __GoldFeverItemTier_init_unchained(
        address admin,
        address nftContract_,
        address nglContract_,
        address itemType_
    ) internal onlyInitializing {
        _setupContractId("GoldFeverItemTier");
        _setupRole(DEFAULT_ADMIN_ROLE, admin);
        nftContract = IERC721Upgradeable(nftContract_);
        ngl = IERC20Upgradeable(nglContract_);
    ...
    mapping(uint256 => mapping(uint256 => bool)) public enabledTier;
    ...
    event ItemTypeContractUpdated(address itemType_);
    event ItemContractUpdated(address item_);
    ...
    function setItemTypeContract(
        address itemType_
    ) external only(DEFAULT_ADMIN_ROLE) {
        itemTypeContract = itemType_;
        emit ItemTypeContractUpdated(itemType_);
    }

    function setItemContract(
        address item_
    ) external only(DEFAULT_ADMIN_ROLE) {
        nftContract = IERC721Upgradeable(item_);
        emit ItemContractUpdated(item_);
    }
    ...
    function addTier(
        ...
        require(
            tierId >= 2,
            "GoldFeverItemTier: Tier must be greater than or equal to 2"
        );
        itemTypeIdToTierUpgradePrice[itemTypeId][tierId] = upgradePrice;
        enabledTier[itemTypeId][tierId] = true;

        emit TierAdded(itemTypeId, tierId, upgradePrice);
    }

    function setItemTier(
        uint256 itemId,
        uint256 tierId
    ) external only(DEFAULT_ADMIN_ROLE) {
        require(
            tierId >= 1,
            "GoldFeverItemTier: Tier must be greater than or equal to 1"
        );
        if (tierId > 1) {
            uint256 itemTypeId = IGoldFeverItemType(itemTypeContract)
                .getItemType(itemId);
            require(
                enabledTier[itemTypeId][tierId],
                "GoldFeverItemTier: Maximum tier"
            );
        }
        _tier[itemId] = tierId;
    ...
    function upgradeItem(uint256 itemId) external nonReentrant {
        address msgSender = _msgSender();
        require(
            IERC721Upgradeable(nftContract).ownerOf(itemId) == msgSender,
            "GoldFeverItemTier: Only item owner can upgrade"
        );
        uint256 nextTier = getItemTier(itemId) + 1;
        uint256 itemTypeId = IGoldFeverItemType(itemTypeContract).getItemType(
            itemId
        );
        require(
            enabledTier[itemTypeId][nextTier],
            "GoldFeverItemTier: Maximum tier"
        );
        ngl.transferFrom(
            msgSender,
            address(this),
            itemTypeIdToTierUpgradePrice[itemTypeId][nextTier]
        );
        _tier[itemId] = nextTier;
    ...
    function collectFee() external only(COMPANY_ROLE) {
        ngl.transfer(_msgSender(), ngl.balanceOf(address(this)));
    }
