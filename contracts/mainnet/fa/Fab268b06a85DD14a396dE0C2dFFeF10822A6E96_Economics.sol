...
contract Economics is IEconomics, AuthModifiers, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function reserveFuel(address _relayerAddress, IEventImplementation.TicketAction[] calldata _ticketActions)
        external
        onlyEvent
        ...
        returns (uint256)
    {
        ...
        if (!integrator.isBillingEnabled) return 0;
    ...
    */
     ...
     */
    function _collectSpentFuel() internal {
        if (spentFuel == 0) return;
    ...
    function safeCollectSpentFuel() public nonReentrant {
    ...
    */
    ...
    function setSpentFuelDestinations(SpentFuelDestination[] calldata _spentFuelDestinations) external onlyOwner {
        // This enforces that the current spentFuel must be flushed to the currently set destinations prior to changing
        // them. Prevents the desintations being changed to collect currently spent fuel to a particular address.
        _collectSpentFuel();
