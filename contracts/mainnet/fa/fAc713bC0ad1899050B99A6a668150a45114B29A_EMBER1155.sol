...
interface IERC1155 is IERC165 {
    ...
    event RoyaltyFeeChanged(uint256 indexed id, uint256 indexed _fee);
...
contract ERC1155 is Context, ERC165, IERC1155 {
    ...
    event FeeChanged(uint256 indexed id, uint256 indexed _fee);
    ...
    mapping(uint8 => uint256) public mintingFees;

    // Mapping from account to operator approvals
    mapping (address => mapping(address => bool)) private _operatorApprovals;
    string public tokenURIPrefix = "https://cryptocorn.mypinata.cloud/ipfs/QmUoaLQj2x2qCqMtAVoGkovNPVNf6EeApzcVWA5YZpcE7Y/";

    // Optional mapping for token URIs
    mapping(uint256 => string) private _tokenURIs;

    uint256 internal _totalSupply = 4;
    ...
    function changeCreator(address _creator) external onlyCreator returns(bool) {
        require(address(0) != _creator, "Creator address is zero address");
        emit CreatorChanged(creator, _creator);
        creator = _creator;
        return true;
    }

    function changeSigner(address _signer) external onlySigner returns(bool) {
        require(address(0) != _signer, "Operator address is zero address");
    ...
    */    
    ...
    function tokenURI(uint256 tokenId) external view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(base, tokenId.toString(),".mp4"));
    }
    ...
    */
    ...
    function mint(uint8 _tokenId, uint256 supply, Sign calldata sign) external payable {
        require(msg.value == mintingFees[_tokenId] * supply, "Mint: invalid Fees");
        require(_tokenId > 0 && _tokenId <= 4, "Mint: Invalid tokenId");
        require(supply <= 5, "Minting limit exceeds");
        require(payable(owner).send(mintingFees[_tokenId] * supply), "failed on transfer");
        require(!usedNonce[sign.nonce], "Nonce : Invalid Nonce");
        usedNonce[sign.nonce] = true;
        verifySign(_tokenId, supply, _msgSender(), sign);
        _mint(_msgSender(), _tokenId, supply);
    }

    function mint(address to, uint8 _tokenId, uint256 supply) external onlyOwner {
        require(_tokenId > 0 && _tokenId <= 4, "Mint: Invalid tokenId");
        _mint(to, _tokenId, supply);
    }

    function setMintingFees(uint8 _tokenId, uint256 _fee) external {
        mintingFees[_tokenId] = _fee;
        emit FeeChanged(_tokenId, _fee);
    }
    ...
    */
    ...
    function _mint(address to, uint256 tokenId, uint256 _supply) internal {
        require(_supply != 0, "Supply should be positive");
        _tokenOwners.set(tokenId, to);
        _balances[tokenId][to] = _balances[tokenId][to] + _supply;
        emit TransferSingle(to, address(0x0), to, tokenId, _supply);
    }

    function setRoyalty(uint256 _id, uint256 _fee) public onlyOwner {
        _royaltyFee[_id] = _fee;
        emit RoyaltyFeeChanged(_id, _fee);
    }
    ...
    */
    ...
    function verifySign(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(this, caller, _tokenId, supply, sign.nonce)
        );
        require(
            signer ==
                ecrecover(
