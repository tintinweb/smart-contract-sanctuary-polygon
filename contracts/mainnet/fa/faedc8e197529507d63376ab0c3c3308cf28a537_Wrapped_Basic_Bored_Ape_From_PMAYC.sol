...
contract Wrapped_Basic_Bored_Ape_From_PMAYC is
  ERC721,
  IERC721Receiver,
  Pausable,
  Ownable,
  ERC721Burnable
{
  ...
  constructor(address frostiesNFTContractAddress_)
    ERC721("Wrapped_Basic_Bored_Ape_From_PMAYC", "WBBACFM")
  {
    frostiesNFT = IERC721(frostiesNFTContractAddress_);
  }

  function _baseURI() internal pure override returns (string memory) {
    return "https://ipfs.io/ipfs/QmTZCpuEWQRv5kiGWxp9B6WinHiZshC97yADwLpLruD8zs/";
  }
  ...
  /// Wrap PMAYC NFT(s) to get Wrapped BBAC(s)
  ...
  /// Unwrap to get PMAYC NFT(s) back
  ...
  function _flip(
    ...
    if (isWrapping_) {
      // Mint Wrapped BBAC of same tokenID if not yet minted, otherwise swap for existing Wrapped BBAC
  ...
  // @notice Mints or transfers wrapped frosty nft to owner for users who incorrectly transfer an Ape directly to the contract without using safeTransferFrom.
  ...
  function emergencyMintWrapped(uint256 tokenId_) external onlyOwner {
    if (frostiesNFT.ownerOf(tokenId_) == address(this)) {
      ...
      if (_exists(tokenId_) && ownerOf(tokenId_) == address(this)) {
        ...
        revert("Wrapped BBAC minted and distributed already");
      }
    } else {
      revert("PMAYC is not locked in contract");
    }
  }
}