...
contract ActiveFarming is Initializable, ActiveFarmingContracts {
    ...
    onlyWorld
    {
        _startFarming(_params.tokenId, _params.itemBoostId, _owner);
    }
    ...
    onlyWorld
    {
        _claimFarming(_tokenId, _owner);

        delete tokenIdToInfo[_tokenId].startTime;
        delete tokenIdToInfo[_tokenId].hasClaimedItems;
    }
    ...
    private
    {
        uint32 _boost = _burnItemForBoost(_itemBoostId, _owner);
    ...
    returns(uint32 _boost)
    {
        ...
        dungeonCrawlingItem.burn(_owner, _itemId, _itemBoostInfo.amountNeeded);
    ...
    contractsAreSet
    {
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            _claimFarming(_tokenId, msg.sender);
    ...
    private
    {
        ...
        (EntityType _entityType, uint16 _classBoost) = _getEntityTypeAndBoost(_tokenId);

        ClassFarmingInfo storage _classInfo = typeToClassInfo[_entityType];

        uint256 _farmedAmount = _getFarmedAmount(_tokenId, _classBoost, _entityType, _classInfo);

        if(_farmedAmount > 0) {
            if(_classInfo.etherOrcItemId > 0) {
                etherOrcsItems.mint(_owner, _classInfo.etherOrcItemId, _farmedAmount);
            } else {
                zug.mint(_owner, _farmedAmount);
            }
        }
    ...
    returns(OutputOutcome[] memory _outcomes)
    {
        ...
        if(_startTime + minimumTimeForItems <= block.timestamp) {
            if(!tokenIdToInfo[_tokenId].hasClaimedItems) {
                uint256 _randomNumber = randomizer.getRandom(tokenIdToInfo[_tokenId].randomRequestKey);
