...
import "./interfaces/INFTicketDemoServiceContract.sol";
...
contract NFTicketDemoServiceContract is IDemoServiceContract, AccessControl {
    ...
    uint256 public ticketServiceDescriptor = 0x08000200;
    uint256 public cashVoucherServiceDescriptor = 0x0A000200;
    ...
    modifier onlyCashVoucher(uint256 _ticketId) {
        Ticket memory ticket = NFTicketContract.getTicketData(_ticketId);
        require(
            ticket.serviceDescriptor == CASH_VOUCHER,
            "Not a cash voucher"
        );
        _;
    }
    ...
    //===================================Public functions===================================//
    /**
     * @notice Mints an NFTicket with the metadata given in _URI
     *
     ...
     */
    function mintNfticket(string calldata _URI) 
        public override 
        returns(uint256) 
    {
        address user = _msgSender();

        uint256 companyCode = 0x200;
        Ticket memory ticket = getTicketParams(user, companyCode, _URI);
     ...
     * @dev Emits a TicketMinted(uint256 newTicketId, address userAddress) event, both parameters indexed
     */
    function mintNfticket(string calldata _URI, address user) 
        public override 
        returns(uint256) 
    {
    ...
    function redeemNfticket(uint256 _ticketId) public override {
        ...
        NFTicketContract.presentTicket(
            _ticketId,
            presenter,
            address(this),
            numCredits,
        ...
        onlyCashVoucher(_ticketId)
    {
        ...
        (creditsAffordable, chargedErc20) = NFTicketContract.topUpTicketNoPayload(
            _ticketId,
        ...
        returns (Ticket memory ticket)
    {
        ticket.tokenID = 0; // This Id will be assigned correctly in the NFTicket contract
        ticket.serviceProvider = address(this);
        ticket.serviceDescriptor = IS_TICKET.add(0x202) ; // hb230607 ticketServiceDescriptor;
        ticket.issuedTo = _recipient;
        ticket.certValue = 0;
        ticket.certValidFrom = 0;
        ticket.price = 0;
...
interface IDemoServiceContract {
    ...
    function mintNfticket(string calldata URI) external returns(uint256);
    function mintNfticket(string calldata URI, address sendTo) external returns(uint256);
