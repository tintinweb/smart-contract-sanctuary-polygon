...
contract StratManager is Ownable, Pausable {
    ...
    event ChargedFees(uint256 systemFees, uint256 treasuryFees, uint256 strategistFees);
    ...
    function chargeFees() internal returns (uint256) {
        ...
        uint256 systemFeeAmount = nativeBal.mul(systemFee).div(MAX_FEE);
        IERC20(wbnb).safeTransfer(systemFeeRecipient, systemFeeAmount);

        uint256 treasuryFeeAmount = nativeBal.mul(treasuryFee).div(MAX_FEE);
        IERC20(wbnb).safeTransfer(treasuryFeeRecipient, treasuryFeeAmount);

        uint256 strategistFeeAmount = nativeBal.sub(systemFeeAmount).sub(treasuryFeeAmount);
        IERC20(wbnb).safeTransfer(strategistFeeRecipient, strategistFeeAmount);

        emit ChargedFees(systemFeeAmount, treasuryFeeAmount, strategistFeeAmount);
...
contract StrategyRewardPool is StratManager {
    ...
    function withdraw(address account, uint256 share, uint256 totalShares) external {
        ...
        IERC20(want).safeTransfer(account, wantBal);
    }
