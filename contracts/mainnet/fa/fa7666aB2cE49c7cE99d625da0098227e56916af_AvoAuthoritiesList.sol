...
abstract contract AvoAuthoritiesListViews is AvoAuthoritiesListVariables {
    ...
    /// reverts with AvoAuthoritiesList__NotTracked() if `trackInStorage` is set to false (data is not available on-chain)
...
/// itself will not track authorities per signer / signers per authority!
...
interface IAvoAuthoritiesList {
    ...
    /// If `trackInStorage` flag is set to false, then only an event will be emitted for off-chain tracking. The contract
    /// itself will not track avoSafes per authority!
