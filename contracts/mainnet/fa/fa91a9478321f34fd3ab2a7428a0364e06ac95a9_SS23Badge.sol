...
import "../../extensions/ERC721LimitedExtension.sol";
import "../../extensions/ERC721FreeMintExtension.sol";

contract SS23Badge is ERC721Base, ERC721MaxSupplyExtension, ERC721SoulboundExtension, ERC721LimitedExtension, ERC721FreeMintExtension {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override (ERC721Base, ERC721MaxSupplyExtension, ERC721SoulboundExtension, ERC721LimitedExtension, ERC721FreeMintExtension) returns (bool) {
    ...
    function _beforeTokenMint(address to, uint256 firstTokenId, uint256 batchSize) internal virtual override (ERC721BetterHooksUpgradeable, ERC721MaxSupplyExtension, ERC721LimitedExtension, ERC721FreeMintExtension) {
        super._beforeTokenMint(to, firstTokenId, batchSize);
    }

    function _beforeTokenSend(address from, address to, uint256 firstTokenId, uint256 batchSize) internal virtual override (ERC721BetterHooksUpgradeable, ERC721SoulboundExtension, ERC721LimitedExtension) {
...
contract ERC721LimitedExtension is ExtensionUpgradeable, AccessControlEnumerableUpgradeable, ERC721BetterHooksUpgradeable {

    bytes32 public constant ERC721_LIMITED_EXTENSION = keccak256('ERC721LimitedExtension');
    bytes32 private constant DEPLOYER_ROLE = keccak256('DEPLOYER_ROLE');
    // Needed to support ERC721FreeMintExtension
    bytes32 private constant MINTER_ROLE = keccak256('MINTER_ROLE');

    error ERC721LimitedExtension_CannotSendToMinter();
    error ERC721LimitedExtension_ReceiverReachesTokenLimit();

    uint256 public tokenLimit;

    function initializeERC721LimitedExtension(uint256 _tokenLimit) public onlyRole(DEPLOYER_ROLE) {
        initializeExtension(ERC721_LIMITED_EXTENSION);
    ...
    function _beforeTokenMint(address to, uint256 firstTokenId, uint256 batchSize) internal virtual override {
        require(balanceOf(to) == 0, "ChuckUniqueExtension: Cannot owns more than one Chuck.");
        super._beforeTokenMint(to, firstTokenId, batchSize);
    }
    ...
    function _beforeTokenSend(address from, address to, uint256 firstTokenId, uint256 batchSize) internal virtual override {
        ...
        if (hasRole(MINTER_ROLE, to)) {
            revert ERC721LimitedExtension_CannotSendToMinter();
        }
        if (balanceOf(to) >= tokenLimit) {
            revert ERC721LimitedExtension_ReceiverReachesTokenLimit();
        }
        super._beforeTokenSend(from, to, firstTokenId, batchSize);
    }
