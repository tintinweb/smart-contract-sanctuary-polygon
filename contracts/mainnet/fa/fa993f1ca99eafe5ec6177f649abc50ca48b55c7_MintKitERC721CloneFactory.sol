...
// File: contracts/rarible/LibRoyaltiesV2.sol
...
// File: contracts/rarible/IRoyalties.sol
...
// File: contracts/opensea/ProxyRegistry.sol
...
// File: contracts/MintKitERC721.sol
...
contract MintKitERC721 is Initializable, OwnableUpgradeable, ERC721Upgradeable, ERC721URIStorageUpgradeable, ERC721RoyaltyUpgradeable, IRoyalties, ReentrancyGuardUpgradeable, BaseOpenSea {
  // TODO: add max mint per wallet (optional)
  // TODO: timestamp based reveal
  using StringsUpgradeable for uint256;
  ...
  bool public presale; // presale starts true so we go in presale state once paused is set to false
  ...
  // Vault address - where fee payments go.
  // Set to 0x0 to disable fee.
  ...
  function initialize(
    ...
    uint256 _maxMintPerAccount) initializer public {
      ...
      _setDefaultRoyalty(_owner, 5000);
      
      transferOwnership(_owner);      
  }

  function setDefaultRoyalty(address receiver, uint96 feeNumerator) public onlyOwner {
      // royaltyAmount = (_salePrice * royalty.royaltyFraction) / _feeDenominator() <-- defaults to 10.000;
      // 5% = 500
      require(feeNumerator <= 10000, "Royalty fee cannot be more than 10%");
  ...
  function getRaribleV2Royalties(uint256 id) override external view returns (LibPart.Part[] memory result) {
      ...
      result[0].account = payable(_royaltyAddress);
      ...
      returns (bool)
  {
      ...
      return super.isApprovedForAll(_owner, _operator) || isOwnersOpenSeaProxy(_owner, _operator);
  }

  function setOpenSeaProxyRegistryAddress(address addr) external onlyOwner {
      _setOpenSeaRegistry(addr);
  }
  ...
  modifier mintCompliance(uint256 _mintAmount) {
    ...
    if (maxMintPerAccount > 0) {
      require(balanceOf(msg.sender) <= maxMintPerAccount, 'Max mint per account exceeded!');
    }
    _;
  }

  modifier onlyVector37() {
    require(_msgSender() == vaultAddress, "Invalid Vector37 account!");
    _;
  }
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    if (presale) {
      require(isWhitelisted(msg.sender) || isUserInWhitelistedCollection(msg.sender), "Address is not whitelisted!");
    }
  ...
  function checkWhitelisted(address _address) public view returns (bool) {
    return (isWhitelisted(_address) || isUserInWhitelistedCollection(_address));
  }
  ...
  function isUserInWhitelistedCollection(address _user) public view returns (bool) {
    for (uint256 i = 0; i < whitelistedCollections.length; i++) {
      if (ERC165Checker.supportsInterface(whitelistedCollections[i], type(IERC721).interfaceId)) {
        IERC721 token = IERC721(whitelistedCollections[i]);
  ...
  function isWhitelistedCollection(address _collection) public view returns (bool) {
    for (uint256 i = 0; i < whitelistedCollections.length; i++) {
  ...
  function removeFromWhitelist(address _user) private {
    for (uint256 i = 0; i < whitelisted.length; i++) {
      if (whitelisted[i] == _user) {
  ...
  function setMaxMinPerAccount(uint256 _maxMintPerAccount) public onlyOwner {
    maxMintPerAccount = _maxMintPerAccount;
  }
  ...
  function withdraw() public onlyOwner nonReentrant {
    // This will pay Vector37 a percentage of the initial sale.
    // =============================================================================
    if (vaultAddress != address(0)) {
      (bool v37, ) = payable(vaultAddress).call{value: address(this).balance * feePercentage / 100}("");
      require(v37);
    }
    // =============================================================================
  ...
  function setFeePercentage(uint256 _feePercentage) public onlyVector37 {
    feePercentage = _feePercentage;
  }

  function setVaultAddress(address _vaultAddress) public onlyVector37 {
    vaultAddress = _vaultAddress;
  }
  ...
  function whitelistCollections(address[] calldata _collections) public onlyOwner {
    // _collections = ["", "", "", "", ...]
    delete whitelistedCollections;
    whitelistedCollections = _collections;
  }
...
// File: contracts/MintKitERC721CloneFactory.sol
...
contract MintKitERC721CloneFactory is Ownable {
    ...
    /*
        Ethereum: eth:0xf32d4B3A52F98E0793A40609025f450Bb98f40C5
        Polygon: matic:0xdCB281707B5Fedf0f3cD05e003F55C1E58c5cbd8
        Arbitrum: arb1:0x6D380f1949Ba2D272375F94ee689ce9BaB4F1892
        Gnosis: gno:0x61A2c56dD9F04Ed8B61C36f6e4bC6344AE081307
    */
    // Currently on Polygon
    address vaultAddress = 0xdCB281707B5Fedf0f3cD05e003F55C1E58c5cbd8;
    
    event contractCreated(string name, string symbol, address owner, address addr);
    
    constructor() {
        implementation = address(new MintKitERC721());
    }
    ...
    function newContract(
        ...
        MintKitERC721(clone).initialize(_name, _symbol, _owner, vaultAddress, _maxSupply, _costInWei, feePercentage, _maxMintAmountPerTx, _maxMintPerAccount);
        isValid[clone] = true;
        emit contractCreated(_name, _symbol, _owner, clone);
        return clone;
    }
