...
contract Marketplace is BaseContract {
  ...
  function setNFTListing(
    ...
    require(_checkPaymentAddressesNotHasZeroAddress(payment._paymentAddresses), "Marketplace: payment addresses invalid");
  ...
  function makeOrder(
    ...
    _validatePaymentAddresses(_nftContractAddress, _paymentAddresses);
    ...
    require(_duration > 0, "Marketplace: duration must be > 0");
    require(_startingPrice > 0 && _endingPrice > 0, "Marketplace: startingPrice and endingPrice must be > 0");
    ...
    orders[_nftContractAddress][_tokenId] = Order(
      ...
      _orderType == IMarketPlace.OrderType.installmentRental ? _weekNumber : 0,
      0
    );
  ...
  function takeOrder(address _nftContractAddress, uint _tokenId, address _paymentAddress, uint _weekNumber) external validOrder(_nftContractAddress, _tokenId) {
    ...
    emit OrderTaken(_nftContractAddress, _tokenId, msg.sender, currentPrice, order.orderType, _paymentAddress, _weekNumber, expiredTime, block.timestamp);
  }
  ...
  function getOrder(address _nftContractAddress, uint _tokenId) external view returns (
    address maker,
    address taker,
    uint startingPrice,
    uint endingPrice,
    ...
    return(order.maker, order.taker, order.startingPrice, order.endingPrice, order.duration, order.activatedAt, order.paymentAddresses, order.orderType, order.status, order.weekNumber, order.weekPrepaidNumber);
  }
  ...
  function _getCurrentPrice(address _nftContractAddress, uint _tokenId, uint _weekNumber, address _paymentAddress) private view returns (uint) {
    ...
    if (secondPassed >= order.duration) {
      currentPrice = order.endingPrice;
    } else {
      int changedPrice = int(order.endingPrice) - int(order.startingPrice);
      int currentPriceChange = changedPrice * int(secondPassed) / int(order.duration);
