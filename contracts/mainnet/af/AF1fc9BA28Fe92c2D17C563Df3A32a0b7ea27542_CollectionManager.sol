 ...
 * @notice It allows adding/removing NFT collections for trading on the ChickenDerby exchange.
 */
contract CollectionManager is ICollectionManager, Ownable {
     ...
     * @param _nftContract address of the collection to add
     */
    function addCollection(address _nftContract) external override onlyOwner {
        if (_whitelistedCollections.contains(_nftContract))
            revert Errors.CollectionExists();
        _whitelistedCollections.add(_nftContract);

        emit CollectionWhitelisted(_nftContract);
    }

    function removeCollection(address _nftContract)
        external
        override
        onlyOwner
    {
        ...
        _whitelistedCollections.remove(_nftContract);
    ...
    function isCollectionWhitelisted(address _nftContract)
        ...
        returns (bool)
    {
        return _whitelistedCollections.contains(_nftContract);
    }
}
