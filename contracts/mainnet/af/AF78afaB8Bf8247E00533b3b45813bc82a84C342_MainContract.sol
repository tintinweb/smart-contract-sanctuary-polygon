...
contract MainContract is ERC721Upgradeable {
  ...
  event SmsRequested(uint256 smsRequestId, address sender, address recipient, uint256 time, string text);
  ...
  function registerAuthorization(address sender, address recipient, uint256 qty, uint256 exp, uint8 _v, bytes32 _r, bytes32 _s) external {
  ...
  function sendSms(address recipient, string calldata text) external returns(bool) {
    ...
    emit SmsRequested(numberOfSmsRequests, msg.sender, recipient, block.timestamp, text);
    return true;
  }
