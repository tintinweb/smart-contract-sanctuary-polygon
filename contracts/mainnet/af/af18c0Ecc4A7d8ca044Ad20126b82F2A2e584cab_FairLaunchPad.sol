...
contract FairLaunchPad is Operator, ILaunchPad, ReentrancyGuard {
    ...
    function calcUserAllocation(address _user) public view returns (uint256 _allocation) {
        ...
        _allocation = committedAmt.mul(10 ** 18).div(totalCommitted).mul(lpFormedAfterSale);
    }
    ...
    function getInvestorReturnValue(address _investor) external view returns (uint256 _returnValue) {
        ...
        uint256 _totalReturnValue = _raisedAmount.mul(3).div(4);
    }
    ...
    function capturingRaiseAmount() external onlyOperator {
        ...
        raiseAmount = usdcHardCap.mul(10 ** 12).mul(10 ** 18).div(lpPrice);
    }
    ...
    function commitToken(uint256 _amount) public override checkLaunchPadRunning initialized nonReentrant {
        ...
        percentFilled = raiseAmount.mul(RATIO_PRECISION).div(totalCommitted);
        percentFilled = raiseAmount.mul(RATIO_PRECISION).div(totalCommitted);
    ...
    function updateUserInfo(address _user) internal {
        ...
        if (percentFilled >= RATIO_PRECISION) {
            uint256 spentAmount = percentFilled.mul(committedAmt).div(RATIO_PRECISION);
