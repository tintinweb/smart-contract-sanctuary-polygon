...
/**
 * @title JamonStakeLpVault
 * @notice Profit sharing pool on the JSTLP tokens deposited in stake. It distributes the tokens received based on the number of tokens staked in each wallet.
 */
contract JamonStakeLpVault is ReentrancyGuard, Pausable, Ownable {
    //---------- Libraries ----------//
    ...
    //---------- Contracts ----------//
    IERC20MintBurn private JSTLP; // JSTLP token contract.
    ...
    //---------- Events -----------//
    event Deposit(address indexed payee, address indexed token, uint256 amount, uint256 totalStaked);
    ...
    //---------- Constructor ----------//
    constructor(address jstlp_) {
        JSTLP = IERC20MintBurn(jstlp_);
        validTokens.add(jstlp_);
    }
    ...
    function _recalculateBalances() internal virtual {
        ...
        for (uint256 i = 0; i < tokensCount; i++) {
            ...
            uint256 balance = token == address(JSTLP)
        ...
        returns (bool)
    {
        ...
        bool success = JSTLP.transferFrom(wallet_, address(this), amount_);
        ...
        returns (uint256)
    {
        ...
        uint256 balance = token_ == address(JSTLP)
        ...
        if (balance > processed) {
            uint256 pending = balance.sub(processed);
            {
                if (pending > 1000000) {
                    ...
                    uint256 pendingBal = stakeHolders[wallet_].pendingTokenbal[token_];
    ...
    function stake(uint256 amount_) external whenNotPaused nonReentrant {
        ...
        require(
            JSTLP.allowance(_msgSender(), address(this)) >= amount_,
            "Amount not allowed"
        );
    ...
    function safeUnStake() external whenPaused nonReentrant {
        ...
        uint256 stakedBal = stakeHolders[_msgSender()].stakedBal;
        delete stakeHolders[_msgSender()];
        require(JSTLP.transfer(_msgSender(), stakedBal), "Transfer error");
        totalStaked = totalStaked.sub(stakedBal);
        totalHolders.decrement();
    }
    ...
    function setTokenList(address token_, bool add_) external onlyOwner {
        require(
            token_ != address(0) && token_ != address(JSTLP),
            "Invalid address"
        );
        if (add_) {
