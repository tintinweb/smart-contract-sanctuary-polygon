...
contract Token is IToken, AgentRoleUpgradeable, TokenStorage {
     ...
     *  @param _identityID the address of the IdentityID of the token
    ...
    function init(
        ...
        uint8 _decimals,
        // _identityID can be zero address if not set, can be set later by owner
        address _identityID
        ...
        _tokenIdentityID = _identityID;
        _tokenPaused = true;
        setIdentityRegistry(_identityRegistry);
        setCompliance(_compliance);
        emit UpdatedTokenInformation(_tokenName, _tokenSymbol, _tokenDecimals, _TOKEN_VERSION, _tokenIdentityID);
    }
    ...
    /**
     *  @dev See {IToken-setIdentityID}.
     *  if _identityID is set at zero address it means no IdentityID is bound to this token
     */
    function setIdentityID(address _identityID) external override onlyOwner {
        _tokenIdentityID = _identityID;
        emit UpdatedTokenInformation(_tokenName, _tokenSymbol, _tokenDecimals, _TOKEN_VERSION, _tokenIdentityID);
    }
    ...
    function recoveryAddress(
        ...
        IIdentity _identityID = IIdentity(_investorIdentityID);
        bytes32 _key = keccak256(abi.encode(_newWallet));
        if (_identityID.keyHasPurpose(_key, 1)) {
            uint256 investorTokens = balanceOf(_lostWallet);
            uint256 frozenTokens = _frozenTokens[_lostWallet];
            _tokenIdentityRegistry.registerIdentity(_newWallet, _identityID, _tokenIdentityRegistry.investorCountry(_lostWallet));
            ...
            emit RecoverySuccess(_lostWallet, _newWallet, _investorIdentityID);
            return true;
        }
    ...
    /**
     *  @dev See {IToken-identityID}.
     */
    function identityID() external view override returns (address) {
        return _tokenIdentityID;
    }
...
contract TokenStorage {
    ...
    address internal _tokenIdentityID;
