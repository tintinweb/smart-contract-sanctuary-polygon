...
// -------------------------EVENTS -------------------------------
   ...
   constructor() ERC721("NFT Lotto", "lot") {
      promo = "Crypto Lotto is in Beta.   Contact @pcashpeso on Twitter for support.  ";
     }
...
contract Lottery is VRFConsumerBaseV2, ConfirmedOwner, ReentrancyGuard {
    ...
    mapping(uint256 => LotteryResult) public lotteryResults;  // the lottery draw number is the key that points to the struct
    ...
    uint256 public s_rollOver = 90;  // percentage of prizepool that gets kept aside for next lottery 90% means 10% kept aside
...
function iterateRandomWords(uint256 requestId) public onlyOwner {
...
function uintToString(uint256 value) internal pure returns (string memory) {
  ...
  function claimPeriodLeft() public view returns (uint256) {
    ...
    // take away the 1% fee then calculate
    function getPrizepool() public view returns(uint256){
        return ((99 * address(this).balance / 100) * s_rollOver) / 100;
    }
...
library NFTSVG {
    ...
    function generateSVG( string memory id, string memory numbersChosen, string memory nftOwner, string memory _drawNumber, string memory _promo) internal pure returns (string memory svg) {
        SVGParams memory params;
        params.id = id;
        params.addr=nftOwner;
        params.numbers =numbersChosen;       
        params.drawNumber = _drawNumber;
        params.promo = _promo;
        return
            string(
                abi.encodePacked(
                    generateSVGDefs(params.id, params.numbers, params.drawNumber, params.promo),
                    '</svg>'
                )
            );
    }

    function generateSVGDefs(string memory id, string memory _numbers, string memory _draw, string memory _promo) internal pure returns (string memory svg) {
        svg = string(
            abi.encodePacked(
   ...
   '<g clip-path="url(#corners)"><g style="filter:url(#top-region-blur); transform:scale(1.5); transform-origin:center top;"><g clip-path="url(#corners)"><rect style="filter: url(#f1)" x="0px" y="0px" width="290px" height="500px" /><g style="filter:url(#top-region-blur); transform:scale(1.5); transform-origin:center top;"><rect fill="none" x="0px" y="0px" width="350px" height="500px" /><ellipse cx="50%" cy="0px" rx="180px" ry="120px" fill="#000" opacity="0.85" /></g><rect x="0" y="0" width="400" height="500" rx="42" ry="42" fill="rgba(0,0,0,0)" stroke="rgba(255,255,255,0.2)" /></g></g><g style="filter:url(#top-region-blur); transform:scale(1.5); transform-origin:center top;"><rect fill="none" x="0px" y="0px" width="290px" height="500px" /><ellipse cx="50%" cy="0px" rx="180px" ry="120px" fill="#000" opacity="0.85" /></g><text text-rendering="optimizeSpeed"></text><g mask="url(#fade-symbol)"><rect fill="none" x="0px" y="0px" width="290px" height="200px" /><text y="70px" x="145" text-anchor="middle" alignment-baseline="central" fill="white" font-weight="200" font-size="32px">NFT Lotto</text></g><text y="125px" x="80px"  fill="white" font-size="26px">Draw # ', _draw ,'</text>',
                '<g style="transform:translate(65px,284px)"><rect width="158px" height="26px" rx="8px" ry="8px" fill="rgba(0,0,0,0.6)" /><text x="12px" y="17px" font-size="12px" fill="white"><tspan fill="rgba(255,255,255,0.6)">NFT id : </tspan>', id, '</text></g>',
                '<g style="transform:translate(45px,254px)"><rect width="198px" height="26px" rx="8px" ry="8px" fill="rgba(0,0,0,0.6)" /><text x="12px" y="17px" font-size="12px" fill="white"><tspan fill="rgba(255,255,255,0.6)">Your Numbers : </tspan>',_numbers, '</text></g></g>',
                '<text text-rendering="optimizeSpeed"><textPath startOffset="0%" fill="white" font-size="12px" xlink:href="#text-path-a">', _promo , '<animate additive="sum" attributeName="startOffset" from="50%" to="-240%" begin="0s" dur="38s" repeatCount="indefinite" /></textPath></text>'
            )
        );
    }
