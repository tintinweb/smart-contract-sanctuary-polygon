...
contract CandleGenieDice is Ownable, ReentrancyGuard 
{
    ...
    struct Stat 
    {
        uint256 rollCount;
        uint256 rollAmount;
        uint256 paidRollCount;
        uint256 paidRollAmount;
    }
    ...
    function _safeRoll(address user, uint256 rollAmount, int position) internal
    {
        ...
        // User Stats
        UserStats[user].rollCount++;
        UserStats[user].rollAmount += rollAmount;
    ...
    function _safeDrop(uint256 rollId, uint256 result) internal
    {
        ...
        if (roll.guess == roll.result)
        {
            ...
            // User Stats
            UserStats[roll.user].paidRollCount++;
            UserStats[roll.user].paidRollAmount += reward;
