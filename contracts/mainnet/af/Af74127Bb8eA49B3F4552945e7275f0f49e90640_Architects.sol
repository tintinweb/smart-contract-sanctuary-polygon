...
contract Architects is ERC721, Ownable {
    using Strings for uint; 

       
    constructor( string memory baseURI_) ERC721("Architect", "ARC") {
        baseURI = baseURI_;
    }

    
    //constants
    uint constant maxPerAddress = 100;
    uint constant ethCost = 0.08 ether;
    uint constant maxId = 6000;
    ...
    modifier publicMintChecks(uint amount) {
        ...
        require(amountMinted[msg.sender] + amount <= maxPerAddress, "Mint would exceed maximum per address"); 
        require(totalSupply + amount <= maxId, "Mint would exceed supply");                                   
        _;
    }
    ...
    function adminSetSaleState(bool saleState_) external onlyOwner {
     ...
     address payable constant withdrawWallet = payable(0xAB5d300AA7Cf43cf64C14D4A5CC8E9ef050B9805);
    ...
    /// @notice allows admin to resvere the entire collection - use with caution   
    function AdminReserveNFT(uint amount) public onlyOwner {   
        require(totalSupply + amount <= maxId, "Mint would exceed supply");
        for(uint i = 0; i < amount; i++)
        _mint(msg.sender, ++totalSupply);
        amountMinted[msg.sender] += amount;
