...
contract LendingHelper {
    ...
    constructor(address _vault) public {
        ...
        maxCollatRatio = liquidationThreshold.sub(DEFAULT_COLLAT_MAX_MARGIN); // 8500 - 2000 = 65%
        maxBorrowCollatRatio = ltv.sub(DEFAULT_COLLAT_MAX_MARGIN); // 8250 - 2000 = 62.5%
    ...
    function getCurrentPositionInWant() public view returns (uint256 deposits, uint256 borrows) {
        ...
        // NOTE conversion needed here as borrows are returned in want
        borrows = wethToWant(balanceOfDebtToken());
    }
    ...
    function triggerLend() public view returns (bool) {
        ...
        if (currentCollatRatio >= maxCollatRatio) return true;
    }
    ...
    function initiateLending() external onlyAuthorized {
        ...
        if (strategyWantBalance() > wantDust && deposits < maxCollateralAmount) {
            uint256 collToDeposit = Math.min(strategyWantBalance(), balanceOfWantMaxForLend());
    ...
    function liquidateAllLend() public onlyAuthorized returns (uint256 _amountFree) {
        ...
        if (strategy.balanceOfWethInWant() < balanceOfDebtTokenInWant) {
            strategy.swapToWeth(balanceOfDebtTokenInWant);
        }
    ...
    /// @notice Borrows the maximum available from the total lend and the targetCollatRatio.
    function _borrowMax() internal {
        ...
        if (totalAmountToBorrow > wantDust) {
            _borrow(totalAmountToBorrow);
        }
    }

    /// @notice Borrows up a specific amount in want accounting some checks.
    ...
    /**
     * Adjust position down from currentBorrowed to newAmountBorrowed in want.
     */
    function _adjustDownTo(uint256 newBorrow, uint256 currBorrowed) internal {
