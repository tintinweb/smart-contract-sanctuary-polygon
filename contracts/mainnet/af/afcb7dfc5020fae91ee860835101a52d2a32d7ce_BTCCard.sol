...
contract BTCCard is ERC721, PaymentSplitter {
    ...
    constructor(address[] memory _payees, uint256[] memory _shares)
        ERC721("BTC Card Collection", "BC")
        PaymentSplitter(_payees, _shares)
    {}

    function mint() external payable {
        if (totalSupply + 1 > 840) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    "ipfs://QmZFLATDcAHMEm2BcXLJ9DSu9HVJZpprRyHdpgHk1peCG3/",
