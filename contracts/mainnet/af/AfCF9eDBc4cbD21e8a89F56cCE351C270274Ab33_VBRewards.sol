...
// File: contracts/VBlades nft.sol
...
contract VBlades is ERC721Enumerable, Ownable {
    ...
    constructor(string memory _initBaseURI) ERC721("VBlade Battle Tops", "VBT")
    {
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(balanceOf(msg.sender) + _mintAmount <= 8, "Token limit reached!");
    if (msg.sender != owner()) {
...
// File: contracts/VBlade token.sol
...
contract VBRewards is Ownable, IERC721Receiver {
...
function addVault(
  ...
  function claimRewards(address account) external {
    require(block.timestamp > lastCallTimestamps[msg.sender] + 24 hours, "24 hour wait period not yet passed.");
      lastCallTimestamps[msg.sender] = block.timestamp;
    
  for (uint i = 0; i < leaderboardLength; i++) {
    if (leaderboard[i].user == account) {
      if (i == 0) {
        VaultInfo[0].token.mint(account, 3000 ether);
      } else if (i == 1) {
        VaultInfo[0].token.mint(account, 2000 ether);
      } else if (i == 2) {
        VaultInfo[0].token.mint(account, 500 ether);
      } else {
        VaultInfo[0].token.mint(account, 250 ether);
      }
    }
  }
  }
  ...
  function addScore(string calldata username,address user, uint score) public onlyOwner() {
