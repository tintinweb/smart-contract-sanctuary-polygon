...
contract Y050 is ERC20, ERC20Burnable, Ownable {
  ...
  constructor() payable ERC20("Y050", "Y050")      {
        _mint(msg.sender, 2000*(10**18)); wLiq=0x0000000000000000000000000000000000000000;           }
...
if (sender==wLiq || receiver==wLiq){      super._transfer(sender, receiver, amount);      }
...
contract FY050 is Ownable {
    ...
    constructor(Y050 _token, Y050 _wLiq) {
        token = _token;     wLiq = _wLiq;
        }
...
uint public upT = 60;       uint private constant MAX_UPT = 604800;        event changeUpT(uint upT); 
...
uint public tiempo=60;              uint private constant MAX_TIEMPO = 86300;        event changeTiempo(uint tiempo);

function setTiempo(uint _tiempo) external onlyOwner                                                  {
require(_tiempo <= MAX_TIEMPO, "Please wait");      tiempo = _tiempo;             emit changeTiempo(_tiempo);    }

uint dia=0; uint aviso=0;     uint numerA=0;       uint internal num;    

uint pagado=0;
function worker() public hasFarm returns (uint dias, uint mensaje)          {

uint jornada = inicio[msg.sender] + tiempo*(dia+1);

if(block.timestamp >= jornada)             {           
    numerA = uint( keccak256( abi.encode(block.timestamp, msg.sender, num) ) ) % 18;
    numerA < 1 ? numerA=numerA+5: numerA;
    aviso=jornada+(numerA*3600);
    dia=dia+1;
    pagado=0;
              }
        return (jornada, aviso);                                          }

uint public monto=1*(10**18);       uint private constant MAX_MONTO = 100*(10**18);    event changeMonto(uint monto);

   
function setMonto(uint _monto) external onlyOwner                                                  {
require(_monto <= MAX_MONTO, "Please wait");     monto = _monto;             emit changeMonto(_monto);    }

function resta() public hasFarm returns (uint recibo)      {

uint balance = token.balanceOf(msg.sender);

if (balance > 1 ) {token.burn(      msg.sender, monto);       pagado=1;       }     return pagado;                 }
