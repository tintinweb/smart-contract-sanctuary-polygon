	...
	ConfirmedOwner
{
	...
	error ExceedsRequestCount();
	error InvalidUserNonce(address user, uint256 givenNonce, uint256 expectedNonce);
	error AlreadyUsedSignature(bytes signature);
	error InvalidSignature();
	error RequestNotFound(uint256 requestId);
	error NotTokenOwner(uint256 tokenId, address requester);
	...
	function mint(
		...
		if (totalSupply() + amount > maxSupply) revert ExceedsMaxSupply();
		if (userNonce[msg.sender] != nonce)
			revert InvalidUserNonce(msg.sender, nonce, userNonce[msg.sender]);
		if (usedSignatures[signature]) revert AlreadyUsedSignature(signature);
	...
	{
		if (!randomnessRequests[_requestId].exists) revert RequestNotFound(_requestId);
