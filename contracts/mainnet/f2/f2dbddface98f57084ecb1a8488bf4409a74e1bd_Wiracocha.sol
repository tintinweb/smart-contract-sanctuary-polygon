    ...
    UUPSUpgradeable
{
    ...
    mapping(address => uint256) private gapBetweenClaims;
    ...
    function registerNft(bytes memory _data) external {
        ...
        require(
            ...
            "Wiracocha: Already exists one for this pacha"
        );
    ...
    ) external whenNotPaused onlyRole(GAME_MANAGER) {
        require(!blackList[_exchanger], "Wiracocha: blacklisted");
        require(
            block.timestamp - gapBetweenClaims[_exchanger] > 1 days,
            "Wiracocha: once a day"
        );
        gapBetweenClaims[_exchanger] = block.timestamp;
    ...
    function includeInBlackList(address _account, bool _include)
        external
        onlyRole(GAME_MANAGER)
    {
        blackList[_account] = _include;
    }

    function isInBlacklList(address _account) external view returns (bool) {
