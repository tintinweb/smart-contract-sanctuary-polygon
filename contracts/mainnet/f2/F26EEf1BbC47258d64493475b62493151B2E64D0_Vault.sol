...
contract Vault {
    ...
    event Order(address tokenAddress, string orderId, uint256 amount);
    event Withdraw(address tokenAddress, address ownerAddress, uint256 amount);

    function pay(address tokenAddress, string memory orderId, uint256 amount) public payable {
        ...
        emit Order(tokenAddress, orderId, amount);
    }
    ...
    function withdraw(address tokenAddress, uint256 amount) public {
        require(IERC20(tokenAddress).balanceOf(address(this)) >= amount, "Token amount must be greater, then you are trying to withdraw");
