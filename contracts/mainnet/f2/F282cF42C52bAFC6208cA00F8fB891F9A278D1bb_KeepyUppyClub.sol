...
contract KeepyUppyClub is RiskyGame {
  ...
  // @notice number of attempts per token (assuming risky success)
  ...
  // @notice track number of rounds, active risky plays and whether the token is live
  struct TokenStatus {
    uint8 rounds;
    bool pending;
    bool disqualified;
  ...
  // @notice there are quite a few trait boosts :)
  function traitBoost(uint256 tokenId) public view override returns (uint32 boost) {
      ...
      boost += (metadata.skillLevel == 5 || metadata.quirks == 24 || metadata.quirks == 21 || metadata.quirks == 22) ? 40000 : 0;
  }

  // @notice boost for creativity
  ...
  // @notice override the played function to use tokenStatus rounds (which start at 1)
