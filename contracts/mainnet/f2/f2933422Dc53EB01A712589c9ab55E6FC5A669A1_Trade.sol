...
import "./RoyaltiesSupport.sol";
import "./Ownable.sol";
import "./TradeConfig.sol";
import "./handlers/IHandler.sol";
import "./utils/FractionMath.sol";
import "./verifiers/TradeSigVerifier.sol";
import "./model/TradeModel.sol";

/**
 * @notice Trading contract: buy and sell NFTs.
 * @dev Raw ETH payments are enabled for buying on the offer trade only.
 * Bids and offers are indexed by signatureHash, it cannot repeat globally to protect against double spend.
 * Nonce is user provided value and alters signatureHash.
 * It is recommended to change nonce on every bid or offer.
 */
contract Trade is RoyaltiesSupport, TradeStorage, Initializable, TradeSigVerifier,
    ...
    function __Trade_init_unchained(address feeRecipient) internal onlyInitializing {
    ...
    function buyOnOffer(address sellerAddress, bytes calldata signature, TradeOffer calldata offer) external payable {
        ...
        require(sellerAddress != address(0), "Trade: the seller is zero address");
        ...
        if (offer.tradeTokenAddress == address(0)) { // ETH
            ...
            IERC20Upgradeable(offer.tradeTokenAddress).safeTransferFrom(msg.sender, address(_feeRecipient), tradeFee);
            if (offer.to == address (0)) {
                IERC20Upgradeable(offer.tradeTokenAddress).safeTransferFrom(msg.sender, sellerAddress, netSalePrice);
            } else { // payment to additional address
                IERC20Upgradeable(offer.tradeTokenAddress).safeTransferFrom(msg.sender, offer.to, netSalePrice);
            }
        }
    ...
    function _sellOnBidTo(address buyerAddress, bytes calldata signature, TradeBid calldata bid, address to) internal {
        ...
        require(buyerAddress != address(0), "Trade: the buyer is zero address");
        ...
        IERC20Upgradeable(bid.tradeTokenAddress).safeTransferFrom(buyerAddress, address(_feeRecipient), tradeFee);
        IERC20Upgradeable(bid.tradeTokenAddress).safeTransferFrom(buyerAddress, to, netSalePrice);
    ...
    function emergencyNFTWithdrawal(address tokenAddress, address recipient, uint256 tokenId, uint256 amount) external onlyOwner {
        require(recipient != address(0), "Trade: emergency withdrawal to zero address");
    ...
    function emergencyTradeTokenWithdrawal(address tradeTokenAddress, address recipient, uint256 amount) external onlyOwner {
        ...
        if (tradeTokenAddress == address(0)) { // ETH
            ...
            IERC20Upgradeable(tradeTokenAddress).safeTransfer(recipient, amount);
        }
    }
}
...
abstract contract RoyaltiesSupport {
    ...
    function _payRoyalties(address royaltiesPayer, uint settlementPrice, address realityPropertiesContractAddress, uint256[] memory tokenIds, address tradeTokenAddress) internal returns (uint256) {
      ...
      if (tradeTokenAddress == address(0)) {
        ...
        IERC20Upgradeable(tradeTokenAddress).safeTransferFrom(royaltiesPayer, receiver, royalty);
      }
      return royalty;
    }
}
...
abstract contract TradeConfig is TradeStorage, Ownable {
    ...
    function supportItem(IHandler handler, address tokenAddress) external onlyOwner {
        ...
        // the return value is not checked, both false and true are valid
