...
import "../extensions/ERC721ByTierBurnable.sol";

contract FlyToArmyRoom is 
    ERC721ByTierBurnable,
    ERC2771Recipient,
    AccessControl,
    Ownable,
    Pausable {
    ...
    ) ERC721ByTier("Fly to Army Room", "ARMY22") {
        // Setup roles.
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721ByTierBurnable, AccessControl) returns (bool) {
        return 
            ERC721ByTierBurnable.supportsInterface(interfaceId) || 
            AccessControl.supportsInterface(interfaceId) || 
            super.supportsInterface(interfaceId);
    }

    /**
     * @dev See {IERC721ByTierBurnable-burnByContract}.
     */
    ...
    /**
     * @dev See {IERC721ByTierBurnable-burn}.
     */
    ...
    /**
     * @dev See {IERC721ByTierBurnable-setBaseBurnedURI}.
     */
    ...
    /**
     * @dev See {IERC721ByTierBurnable-setTokenBurnByContractState}.
     */
    ...
    /**
     * @dev See {IERC721ByTierBurnable-setTokenBurnState}.
     */
    ...
    /**
     * @dev See {IERC721ByTierBurnable-setFutureContractAddress}.
     */
    ...
    /**
     * @dev See {IERC721BaseURIStorage-setBaseURIPerTier}.
     */
    function setBaseURIPerTier(uint16[] calldata tierIds, string[] memory updateBaseURIPerTier, bool finalized) external onlyRole(ADMIN_ROLE) whenNotPaused {
        _setBaseURIPerTier(tierIds, updateBaseURIPerTier, finalized);
    }

    /**
     * @dev See {IERC721BaseURIStorage-setTokenURISuffix}.
     */
    ...
    /**
     * @dev See {IERC721ByTier-addTierIds}.
     */
    ...
    /**
     * @dev See {IERC721ByTier-removeTierIds}.
     */
    ...
    /**
     * @dev See {IERC721ByTier-setMaxSupply}.
     */
    ...
    /**
     * @dev See {IERC721ByTier-setTokenTransferState}.
     */
...
import "./ERC721BaseURIStorage.sol";
import "./IERC721ByTierBurnable.sol";

abstract contract ERC721ByTierBurnable is ERC721ByTier, ERC721BaseURIStorage, IERC721ByTierBurnable {
    ...
    // Mapping of tier ID is allowed to burn or burn by contract.
    // Returns `true` if is allowed. `false` otherwise.
    mapping (uint256 => bool) public isTierIdBurnable;

    // Tracker for burned token ID.
    mapping (uint256 => bool) private _isTokenIdBurned;

    // Recent state of token burn mechanism.
    // Returns `true` if is allowed. `false` otherwise.
    bool public isTokenBurnable;

    // Recent state of token burn by future contract mechanism.
    // Returns `true` if is allowed. `false` otherwise.
    bool public isTokenBurnableByContract;
     ...
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721ByTier, ERC721BaseURIStorage, IERC165) returns (bool) {
        return 
            interfaceId == type(IERC721ByTierBurnable).interfaceId ||
            super.supportsInterface(interfaceId);
    }
     ...
     */
    function totalSupply() public view virtual override(ERC721ByTier) returns (uint256) {
        unchecked {
     ...
     */
    function totalSupplyPerTier(uint256 tierId) public view virtual override(ERC721ByTier) returns (uint256) {
        unchecked {
     ...
     * @notice {tokenURI} is overriden to return base burned URI if the `tokenId` is burned
     * and return base URI based per tier ID if the `tokenId` does exist.
     */
    function tokenURI(uint256 tokenId) public view virtual override(ERC721, ERC721BaseURIStorage) returns (string memory) {
        // Check if token ID is burned.
        if (_isTokenIdBurned[tokenId]) {
            // Load base burned URI to memory.
            string memory baseBurnedURI = _baseBurnedURI();
            
            return bytes(baseBurnedURI).length > 0 ? string(abi.encodePacked(baseBurnedURI)): "";
        } 
        // Check if token ID does exist.
        if (!_exists(tokenId)) revert TokenIdDoesNotExist();
        /// Load state variables to memory.
        // Get tier ID of the `tokenId`.
        uint256 tierId = tierID[tokenId];
        // Get base URI per tier ID.
        string memory _baseURIPerTier = baseURIPerTier[tierId];
        // Get indexByTier of the `tokenId`.
        // uint256 indexByTierOfTokenId = indexByTier[tokenId];
        // Get _tokenURISuffix.
        // string memory tokenURISuffix = _tokenURISuffix;
        
        return bytes(_baseURIPerTier).length > 0 ?
            // string(abi.encodePacked(_baseURIPerTier, "/", indexByTierOfTokenId.toString(), tokenURISuffix)) : "";
            string(abi.encodePacked(_baseURIPerTier)) : "";
    }
    ...
    function _burnTokenByContract(address owner, uint256 tokenId) internal virtual {
        ...
        if (!isTokenBurnableByContract) revert TokenIdIsNotBurnable();
    ...
    function _burn(address owner, uint256 tokenId) internal virtual {
        ...
        if (!isTokenBurnable) revert TokenIdIsNotBurnable();
        ...
        // Call _portBurnByTierEvent
        _postBurnByTierEvent(tokenId);
    }

    /**
     * @dev Update mapping totalBurnedPerTier after token burn event.
     */
    function _postBurnByTierEvent(uint256 tokenId) internal virtual returns (uint256 tierId) {
...
abstract contract ERC721ByTier is ERC721, CollectibleBase, IERC721ByTier {
    using EnumerableSet for EnumerableSet.UintSet;
    using Counters for Counters.Counter;
    using Strings for uint256;

    // Tier ID tracker.
    EnumerableSet.UintSet internal _tierIds;
    ...
    // Mapping of recent state of maximum supply per tier ID state.
    // Returns `true` if is defined. `false` otherwise.
    mapping(uint256 => bool) public isMaxSupplyPerTierDefined;

    // Recent state of token transfer mechanism.
    // Returns `true` if is transferable. `false` otherwise.
    ...
    /**
     * @dev Returns the total quantity of tokens per `tierId`.
     */
    function totalSupplyPerTier(uint256 tierId) public view virtual returns (uint256) {
        unchecked {
            return totalMintedPerTier[tierId];
        }
    }
    ...
    function _mintByTierTo(address to, uint16 tierId) internal virtual {
        // Load next token ID to be minted to memory.
        uint256 _nextTokenId = nextTokenId();
        // Load total minted per tier ID to memory.
    ...
    function _addTierIds(uint256[] calldata tierIds) internal virtual {
        // Cache tierIds array length from input param.
        uint256 length = tierIds.length;
        // Iterate the statements below (length minus one) times.
        for (uint256 i = 0; i < length;) {
            // Load tierIds array element to memory.
            uint256 tierId = tierIds[i];
            // Check tier ID existance.
            if (_tierIds.contains(tierId)) revert TierIdExist();
    ...
    /**
     * @dev {IERC721ByTier-removeTierIds}.
     */
    function _removeTierIds(uint256[] calldata tierIds) internal virtual {
        ...
        for (uint256 i = 0; i < length;) {
            ...
            if (totalMintedPerTier[tierId] != 0) revert TotalMintedPerTierExist();
            // Remove tierIds from _tierIds.
            _tierIds.remove(tierId);
            // Unchecked iteration.
            unchecked {
                ++i;
            }
    ...
    /**
     * @dev See {IERC721ByTier-setMaxSupplyPerTier}.
     */
    function _setMaxSupplyPerTier(
        uint256[] calldata tierIds,
        uint256[] calldata updateMaxSupplyPerTier,
        bool defined
    ) internal virtual {
        // Check state.
        if (!defined) revert InvalidUpdateState();
        // Check input param array lengths.
        ...
        uint256 length = tierIds.length;
            ...
            for (uint256 i = 0; i < length;) {
                ...
                if (!_tierIds.contains(tierId)) revert TierIdDoesNotExist();
                // Check total minted per tier ID.
                if (updateMaxSupplyPerTier[i] < totalMintedPerTier[tierId]) revert InvalidUpdateValue();
                // Update max supply per tier ID.
                maxSupplyPerTier[tierId] = updateMaxSupplyPerTier[i];
                // Update state per tier ID.
                isMaxSupplyPerTierDefined[tierId] = true;
                // Unchecked iteration.
                unchecked {
                    ++i;
                }
...
import "./IERC721BaseURIStorage.sol";
import "../base/ERC721ByTier.sol";

abstract contract ERC721BaseURIStorage is ERC721ByTier, IERC721BaseURIStorage {
    using EnumerableSet for EnumerableSet.UintSet;
    using Strings for uint256;

    // Suffix of the token URI.
    string internal _tokenURISuffix;

    // Mapping of tier ID to base URI per tier ID.
    mapping(uint256 => string) public baseURIPerTier;

    // Mapping of tier ID to recent state of base URI per tier ID.
    // Returns `true` if is final. `false` otherwise.
    mapping(uint256 => bool) public isMetadataPerTierFinal;

    /**
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721ByTier, IERC165) returns (bool) {
        return
            interfaceId == type(IERC721BaseURIStorage).interfaceId || 
            super.supportsInterface(interfaceId);
    }

    /**
     * @dev See {IERC721Metadata-tokenURI}.
     * @notice {tokenURI} is overriden to return base URI based per tier ID if the `tokenId` does exist.
     */
     ...
     * @dev See IERC721BaseURIStorage-setBaseURIPerTier}.
     */
    function _setBaseURIPerTier(
        uint16[] calldata tierIds,
        string[] memory updateBaseURIPerTier,
        bool finalized
    ) internal virtual {
        // Check input param array lengths.
        if (tierIds.length != updateBaseURIPerTier.length) revert InvalidLength();
        // Cache tierIds array length from input param.
        uint256 length = tierIds.length;
        // Iterate the statements below (length minus one) times.
        for (uint256 i = 0; i < length;) {
            // Load tierIds array element to memory.
            uint16 tierId = tierIds[i];
            // Check tier ID existance.
            if (!_tierIds.contains(tierId)) revert TierIdDoesNotExist();
            // Check state.
            if (isMetadataPerTierFinal[tierId]) revert InvalidUpdateState();
            // Update base URI per tier mapping.
            baseURIPerTier[tierId] = updateBaseURIPerTier[i];
            // Update state.
            isMetadataPerTierFinal[tierId] = finalized;
            // Unchecked iteration.
            unchecked {
                ++i;
            }
        ...
        // Emits the event.
        emit BaseURIPerTierUpdated(_msgSender(), updateBaseURIPerTier, finalized);
    }

    /**
     * @dev See IERC721BaseURIStorage-setURISuffix}.
     */
...
interface IERC721ByTierBurnable is IERC165 {
    /**
...
abstract contract CollectibleBase is ERC721, ICollectibleBase {
    using Counters for Counters.Counter;
    using Strings for uint256;
    ...
    // Recent state of maximum number minted per wallet address state.
    // Returns `true` if is declared. `false` otherwise.
...
interface IERC721BaseURIStorage is IERC165 {
    /**
     * @notice Events to be emitted.
     */
     ...
     */
    event BaseURIPerTierUpdated(address indexed caller, string[] updatedBaseURIPerTier, bool isFinal);
    ...
    /**
     * @dev Set base URI per tier ID for computing {tokenURI} with no extension.
     *
     * Requirements:
     * 
     * @param tierIds tierIds must exist.
     * @param updateBaseURIPerTier must refer to where the base of collection metadata per tier ID is located (e.g "ipfs://..." or "ar://..." ).
     * @param finalized is a boolean value.
     * - Set to `true` if `updateBaseURIPerTier` is final.
     * - Set to `false` if `updateBaseURIPerTier` is not final.
     */
    function setBaseURIPerTier(uint16[] calldata tierIds, string[] memory updateBaseURIPerTier, bool finalized) external;
