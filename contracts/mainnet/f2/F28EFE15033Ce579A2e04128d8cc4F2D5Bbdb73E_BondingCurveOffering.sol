...
contract BondingCurveOffering is BondingCurve {
    ...
    modifier validBuy(uint256 _amount) {
        ...
        uint256 _tokens = calculateBuyAmount(_amount * rate);
    ...
    function Purchase(uint256 _amount) public validBuy(_amount) {
        ...
        uint256 _tokens = _purchase(_amount * rate);
    ...
    function Reimbursement(uint256 _tokens) public validSell(_tokens) {
        ...
        uint256 _amount = _reimbursement(_tokens);
...
contract BondingCurve is BancorFormula, Ownable, MaxGasPrice {
    ...
    function calculateReimbursement(uint256 _amount)
        public
        view
    ...
    function _reimbursement(uint256 _amount) internal returns (uint256) {
        uint256 reimbursement = calculateReimbursement(_amount);
