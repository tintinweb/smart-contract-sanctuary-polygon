...
// File: contracts/contracts/interfaces/common/IKyberElastic.sol
...
// File: contracts/contracts/utils/BytesLib.sol
...
// File: contracts/contracts/utils/Path.sol
...
// File: contracts/contracts/interfaces/common/IUniswapRouterV3WithDeadline.sol
...
// File: contracts/contracts/interfaces/common/IUniswapRouterV3.sol
...
// File: contracts/contracts/utils/UniV3Actions.sol
...
library UniV3Actions {
    ...
    function kyberSwap(
        ...
        IKyberElastic.ExactInputParams memory swapParams = IKyberElastic
            .ExactInputParams({
                path: _path,
...
// File: contracts/contracts/interfaces/curve-poly-usdr/IGaugeFactory.sol
...
// File: contracts/contracts/interfaces/curve-poly-usdr/ICurveSwap.sol
...
// File: contracts/contracts/interfaces/curve-poly-usdr/IConvexRewardPool.sol
...
// File: contracts/contracts/interfaces/curve-poly-usdr/IConvexBoosterL2.sol
...
// File: contracts/contracts/interfaces/curve-poly-usdr/IConvexBooster.sol
...
// File: contracts/contracts/interfaces/common/IWrappedNative.sol
...
// File: contracts/contracts/interfaces/common/IUniswapRouterV2.sol
...
// File: contracts/contracts/interfaces/common/IGFITierChecker.sol
...
// File: contracts/contracts/common/StratFeeManager.sol
...
// File: contracts/contracts/strategies/Curve-Poly-USDR.sol
...
contract StrategyConvexL2 is StratFeeManager {
    ...
    function withdraw(uint256 _amount, uint256 _feeAmount) external {
        ...
        uint256 tAmount = _amount + _feeAmount;
        uint256 wantBal = IERC20(want).balanceOf(address(this));
        uint256 fee = _feeAmount;

        if (wantBal < tAmount) {
            IConvexRewardPool(rewardPool).withdraw(tAmount - wantBal, false);
    ...
    function _harvest() internal whenNotPaused {
        uint256 initNativeBal = IERC20(native).balanceOf(address(this));
        IConvexRewardPool(rewardPool).getReward(address(this));
        swapRewardsToNative();
        uint256 nativeBalAfterHarvested = IERC20(native).balanceOf(
            address(this)
        );

        if (nativeBalAfterHarvested > 0) {
            uint256 nativeHarvested = nativeBalAfterHarvested - initNativeBal;
            if (nativeHarvested > 0) {
                chargeHarvestFees(nativeHarvested);
            }
            addLiquidity();
    ...
    function swapRewardsToNative() internal {
        ...
        for (uint i; i < rewards.length; ++i) {
            ...
            if (bal >= rewards[i].minAmount) {
                IUniswapV2Router02(rewards[i].router).swapExactTokensForTokens(
                    bal,
                    0,
    ...
    // performance fees
    function chargeHarvestFees(uint256 feeBal) internal {
    ...
    function setPool(address _pool) external onlyManager {
        pool = _pool;
    }

    function setZap(address _zap) external onlyManager {
        zap = _zap;
    }

    function setPid(uint _pid) external onlyManager {
        pid = _pid;
    }

    function setParams(uint[] memory _params) external onlyManager {
        poolSize = _params[0];
        depositIndex = _params[1];
        useUnderlying = _params[2] > 0;
        depositNative = _params[3] > 0;
