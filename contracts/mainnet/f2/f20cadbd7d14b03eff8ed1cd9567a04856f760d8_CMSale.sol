...
contract CMSale is Ownable {
  ...
  constructor () {
    // Add MATIC and TECH
    currencies.push(Currency(bytes10(bytes("MATIC")),address(0)));
    currencies.push(Currency(bytes10(bytes("TECH")),0x6286A9e6f7e745A6D884561D88F94542d6715698));
  }
  ...
  function addCurrencies (string[] calldata _names, address[] calldata _addresses) external onlyAdmin {
    ...
    for (uint256 i=0; i<count_; i++) {
      ...
      currencies.push(Currency(bytes10(bytes(_names[i])),_addresses[i]));
  ...
  function createSale(
    ...
    onlyAdmin
  {
    require(_currencyID < currencies.length, "CRE-00");
    require(_price > 0 && _cap > 0, "CRE-01");
    require(block.timestamp < _startTime && _startTime < _endTime, "CRE-02");
    require(!_hasDynamicPrice || (_priceDeltaPerMille > 0 && _priceDeltaPerMille <= 1000), "CRE-03");
    //
  ...
  function closeSale(uint256 _saleID) external onlyAdmin onlyActiveSale(_saleID) {
    ...
    if (amount_ > 0) {
      address currAddress_ = currencies[sale_.currencyID].currAddress;
      if (burnPct > 0) {
  ...
  function withdrawStuckTokens(uint256 amount_, address tokenAddress_) external onlyAdmin {
    require(_tokenTransfer(owner(), amount_, tokenAddress_), "Withdrawing stuck tokens failed");
  }
