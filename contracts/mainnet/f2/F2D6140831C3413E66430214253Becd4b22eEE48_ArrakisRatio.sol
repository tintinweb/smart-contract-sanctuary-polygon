...
contract ArrakisRatio {
    IUniV3PriceGetter public immutable priceGetter;

    constructor(IUniV3PriceGetter _priceGetter) {
    ...
    function getSwapRatio(
        ...
        external
        view
        returns (uint256 amount0, uint256 amount1, uint256 amount2)
    {
        (uint256 underlying0, uint256 underlying1) = IGUniPool(arrakisPool)
            .getUnderlyingBalances();
        uint256 token0decimals = ERC20(address(IGUniPool(arrakisPool).token0()))
            .decimals();
        uint256 token1decimals = ERC20(address(IGUniPool(arrakisPool).token1()))
            .decimals();
        ...
        uint256 percentage0 = (underlying0 *
            ...
            ((underlying0 * 10 ** (18 - token0decimals)) +
                (underlying1 * 10 ** (18 - token1decimals)) +
                price0 +
                price1);
        ...
        amount0 = (percentage0 * inputAmount) / 1e36;
        amount1 = (percentage1 * inputAmount) / 1e36;
        amount2 = inputAmount - amount0;
    }
}
