...
contract GotchiVault is IERC173, Initializable,PausableUpgradeable, IERC721ReceiverUpgradeable, IERC1155ReceiverUpgradeable {
    ...
    function _isApproved() internal view{
    ...
    function getDepositors(uint256 _skip, uint256 _end) public view returns(address[] memory){
        uint32[] memory myGotchis = IAavegotchi(im_diamondAddress).tokenIdsOfOwner(address(this));

        require(_skip >= 0 && _end < myGotchis.length,"getDepositors");

        //give the option to default to all
        if(_skip == 0 && _end == 0){
            _end = myGotchis.length;
        }

        address[] memory _depositors = new address[](myGotchis.length);

        for(uint256 i = _skip; i < _end; i++){
            _depositors[i] = s.deposits[im_diamondAddress][myGotchis[i]];
        }
    ...
    function setCustomSettingsFee(uint256 _newFee) public onlyOwner{
        s.customSettingsFee = _newFee;
    }

    function getCustomSettingsFee() public view returns(uint256){
        return s.customSettingsFee;
    }
   ...
   /* function setPetOperatorForAll(address _operator, bool _approved) public onlyOwner {
    ...
    function transferEscrow(uint256 _tokenId, address _erc20Contract, address _recipient, uint256 _transferAmount) public whenNotPaused{
        require(s.deposits[im_diamondAddress][_tokenId] == msg.sender, "transferEscrow");
    ...
    function transferAllEscrow(uint256 _tokenId, address _erc20Contract, address _recipient) public whenNotPaused{
        require(s.deposits[im_diamondAddress][_tokenId] == msg.sender, "transferAllEscrow");
   ...
   /* function spendSkillPoints(uint256 _tokenId, int16[4] calldata _values) public whenNotPaused{ 
        ...
        require(s.deposits[im_diamondAddress][_tokenId] == msg.sender, "spendSkillPoints");
    ...
    function depositERC721(address _tokenAddress, uint256[] calldata _tokenId) public whenNotPaused {
        ...
        //the number of gotchis that need to pay the fee
        uint8 numFee;

        for(uint256 i = 0; i<_tokenId.length; i++){

            require(IAavegotchi(_tokenAddress).ownerOf(_tokenId[i]) == msg.sender, "depositErc721");
        ...
        if(numFee > 0 && s.fee721 > 0){
            uint256 feeAmount = s.fee721 * numFee;
            IERC20(im_ghstAddress).transferFrom(msg.sender, address(this), feeAmount);
            s.totalFeesCollected += feeAmount;

            //50% of the fee goes to the vGHST pool, 50% goes to the contract owner and creator
            IERC20(im_ghstAddress).transfer(s.contractOwner, (feeAmount * 100) / 400);
            IERC20(im_ghstAddress).transfer(s.contractCreator, (feeAmount * 100) / 400);
            IERC20(im_ghstAddress).transfer(s.vGHSTAddress, (feeAmount * 200) / 400);
        }
    }

    function withdrawERC721(address _tokenAddress, uint256[] calldata _tokenId) public {

        require(_tokenId.length <= 10, "withdrawERC721" );

        for(uint256 i = 0; i<_tokenId.length; i++){
            //only the original depositor can withdraw an ERC721 token
            require(s.deposits[_tokenAddress][_tokenId[i]] == msg.sender, "withdrawERC721");
            require(!s.gotchiMapping[_tokenId[i]].locked,"withdrawERC721");
    ...
    function compoundEscrow(uint256 _tokenId) public whenNotPaused{
        ...
        require(s.deposits[im_diamondAddress][_tokenId] == msg.sender || msg.sender == s.contractOwner || 
            s.approvedUsers[msg.sender], "compoundEscrow");
        ...
        require(escrowBalance > 0, "compoundEscrow");
    ...
    ///////////////////////////////////////////////////////////////////////////////////////
    //Gotchi Lending functions

    event Lock(address _owner, uint256 _tokenId, uint256 _lockPeriod,bool _isLocked);
    event Pause(address _owner, uint256 _tokenId,bool _isPaused);

    //user calls this function to lock his gotchi for guild rentals
    function lockGotchis(uint256[] calldata _tokenIds,bool _lock, uint256 _lockPeriod) public{

        for(uint256 i = 0; i < _tokenIds.length; i++){

            require(s.deposits[im_diamondAddress][_tokenIds[i]] == msg.sender, "lockGotchis");

            Gotchi storage thisGotchi = s.gotchiMapping[_tokenIds[i]];

            //if we're not locking, then the lockup period must have expired
            require(_lock || (block.timestamp - thisGotchi.lastLocked) > thisGotchi.lockPeriod,"lockGotchis");
           
            //lock the gotchi and record when we locked it
            thisGotchi.locked = _lock;
            thisGotchi.lastLocked = block.timestamp;
            thisGotchi.lockPeriod = _lockPeriod;

            thisGotchi.pauseLending = false;

            emit Lock(msg.sender,_tokenIds[i],_lockPeriod,_lock);
        }
    }

    function isLocked(uint256 _tokenId) public view returns(bool,uint256){
        return (s.gotchiMapping[_tokenId].locked,s.gotchiMapping[_tokenId].lastLocked);
    }

    //returns how much time a gotchi has left to be locked up
    function lockTimeRemaining(uint256 _tokenId) public view returns(uint256){
        Gotchi storage thisGotchi = s.gotchiMapping[_tokenId];

        uint256 timeElapsed = (block.timestamp - s.gotchiMapping[_tokenId].lastLocked);

        if(timeElapsed > thisGotchi.lockPeriod){
            return 0;
        }
        
        uint256 _timeRemaining = thisGotchi.lockPeriod - timeElapsed;
        return _timeRemaining;
    }

    //callable by the owner of a gotchi
    //allows users to set custom settings for their gotchis
    function setRentalSettings(
        ...
        for(uint256 i = 0; i < _tokenIds.length; i++){
            require(s.deposits[im_diamondAddress][_tokenIds[i]] == msg.sender, "setRentalSettings");

            //we need to maintain our percentage cut for the vault
            require(_revenueSplit[2] == s.defaultSettings.revenueSplit[2],"setRentalSettings");

            s.gotchiMapping[_tokenIds[i]].preferredSettings = RentalSettings({
                initialized:true,   //initialize the settings
                initialCost:_initialCost,
                period:_period,
                revenueSplit:_revenueSplit,
                thirdParty:s.defaultSettings.thirdParty,   //the vault is always the third party
                whitelistId:_whitelistId,
                revenueTokens:_revenueTokens
            });
        }

        IERC20(im_ghstAddress).transferFrom(msg.sender, address(this), s.customSettingsFee * _tokenIds.length);
        s.totalFeesCollected += s.customSettingsFee * _tokenIds.length;
    }

    function resetToDefaultRentalSettings(uint256[] calldata _tokenIds) public {
        for(uint256 i = 0; i < _tokenIds.length; i++){
            require(s.deposits[im_diamondAddress][_tokenIds[i]] == msg.sender, "setRentalSettings");
            s.gotchiMapping[_tokenIds[i]].preferredSettings.initialized = false;
        }
    }
    ...
    function getUserSettings(uint256 _tokenId) public view returns(RentalSettings memory){
        return s.gotchiMapping[_tokenId].preferredSettings;
    }

    //callable by owner.  set the default rental setting that will be applied to gotchis by default
    function setDefaultRentalSettings(
        uint96 _initialCost,
        uint32 _period,
        uint8[3] calldata _revenueSplit,
        address _thirdParty,
        ...
        s.defaultSettings = RentalSettings({
            initialized:false,
            initialCost:_initialCost,
            period:_period,
            revenueSplit:_revenueSplit,
    ...
    function getDefaultRentalSettings() public view returns(RentalSettings memory){
        return s.defaultSettings;
    }

    //callable by gotchi owner -- allow owner to pause lending
    function pauseLending(uint256[] calldata _tokenIds,bool _pause) public{
        for(uint256 i = 0; i < _tokenIds.length; i++){
            require(s.deposits[im_diamondAddress][_tokenIds[i]] == msg.sender, "pauseLending");

            //we don't want locked gotchis to be paused by owners
            require(!s.gotchiMapping[_tokenIds[i]].locked);

            s.gotchiMapping[_tokenIds[i]].pauseLending = _pause;
            
            emit Pause(msg.sender, _tokenIds[i], _pause);
        }
    }
    ...
    function readyToRent(uint32 _tokenId) public view returns(bool _ready){

        //an aavegotchi is ready to rent if it's  both not lent AND not paused AND not listed
        _ready = 
            !(IAavegotchi(im_diamondAddress).isAavegotchiLent(_tokenId)) &&
            !(s.gotchiMapping[_tokenId].pauseLending) &&
            (IAavegotchi(im_diamondAddress).getGotchiLendingIdByToken(_tokenId) == 0);
       
    }

    //callable by approved users -- assign a gotchi to a guild
    function assignGuild(uint256[] calldata _tokenIds,uint256 _guild) public onlyApproved{
        for(uint256 i = 0; i < _tokenIds.length; i++){
            
            //gotchi must be locked to assign to a guild
            //require(s.gotchiMapping[_tokenIds[i]].locked, "assignGuild");
            s.gotchiMapping[_tokenIds[i]].assignedGuild = _guild;
        }
    }

   /* function setGuildName(uint256 _guild, string calldata _name) public onlyOwner{
        s.guilds[_guild].name = _name;
    }*/

    //set the whitelisted address for a guild
   /* function setGuildApproved(uint256 _guild, address _representative) public onlyOwner{
        s.guilds[_guild].approved = _representative;
    }

    function getGuildApproved(uint256 _guild) public view returns(address){
        return s.guilds[_guild].approved ;
    }*/

    //allow the approved guild rep or the contract owner to modify a guild's setting

    function modifyGuildSettings(
        uint256 _guild,
        uint96 _initialCost,
        uint32 _period,
        uint8[3] calldata _revenueSplit,
        address _thirdParty,    //this will be a profit splitting contract b/w vault and guild
        uint32 _whitelistId,
        address[] calldata _revenueTokens
        ) public onlyApproved{

        s.guilds[_guild].rentalSettings = RentalSettings({
            initialized:true,   //initialize the settings
            initialCost:_initialCost,
            period:_period,
            revenueSplit:_revenueSplit,
    ...
    function getGuildSettings(uint256 _guild) public view returns(RentalSettings memory){
        return s.guilds[_guild].rentalSettings;
    }
    ...
    //all rental profits go to the vault address.  we can call this function to split those profits among the managers,
    //creators, and the vGHST holders
    function withdrawProfits(address[] calldata _revenueTokens) public onlyApproved{
        ...
        for(uint256 i = 0; i < _revenueTokens.length; i++){
            ...
            token.transfer(s.contractOwner,(balance * 100) / 400);
            token.transfer(s.contractCreator,(balance * 100) / 400);
            token.transfer(s.vGHSTAddress,(balance * 200) / 400);

        }
    }

    //return our active listings for a certain whitelist
    /*function getListingsByWhitelistID(uint32 _whitelistId,uint256 _length) public view returns(uint32[] memory){

        GotchiLending[] memory allListings = IAavegotchi(im_diamondAddress).getOwnerGotchiLendings(address(this), "listed", _length);
        uint32[] memory listingIDs = new uint32[](_length);

        for(uint256 i = 0; i < allListings.length; i++){
            if(allListings[i].whitelistId == _whitelistId){
                listingIDs[i] = allListings[i].listingId;
            }
        }
    ...
    function withdrawProfits(address[] calldata _revenueTokens) public onlyApproved{
        ...
        for(uint256 i = 0; i < _revenueTokens.length; i++){
            ...
            token.transfer(s.contractOwner,balance.mul(100).div(400));
            token.transfer(s.contractCreator,balance.mul(100).div(400));
            token.transfer(s.vGHSTAddress,balance.mul(200).div(400));
