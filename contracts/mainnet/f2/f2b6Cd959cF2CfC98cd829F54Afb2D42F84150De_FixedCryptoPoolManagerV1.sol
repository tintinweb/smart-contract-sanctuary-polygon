...
contract FixedCryptoPoolManagerV1 is IFixedCryptoPoolManagerV1, BasePoolManagerV1 {
    ...
    function createFixedCryptoPool(
        ...
        FixedCryptoPoolV1(pool).addLiquidity(initReserve);
    }
...
contract FixedCryptoPoolV1 is IFixedCryptoPoolV1, ERC1155Supply {
    ...
    constructor(
        ...
        emit NewRoundStarted(0, getRoundData[0].tradeStartTime, getRoundData[0].tradeEndTime, getRoundData[0].roundEndTime);
    }
    ...
    function addLiquidity(uint256 reserve0) external override onlyNotStopped {
        require(reserves[0] == 0 && reserves[1] == 0, "already add liquidity");
        // require(reserve0 == initReserve, "reserve0 not equal to initReserve");
        ...
        reserves[0] = initReserve;
        reserves[1] = initReserve;
    ...
    function buy(
        address to,
        uint8 option
    ) external override onlyNotStopped whenNotPaused returns (uint256 shares) {
        ...
        if (isLong) {
            ...
            newReserveShort = amountWithoutFee * tradeFactor + reserves[1];
            newReserveLong = reserves[0].mulDiv(reserves[1], newReserveShort);
            ...
            newReserveLong = amountWithoutFee * tradeFactor + reserves[0];
            newReserveShort = reserves[0].mulDiv(reserves[1], newReserveLong);
            shares = amountWithoutFee.mulDiv(
                newReserveLong + newReserveShort,
                newReserveLong
            );
        ...
        reserves[0] = newReserveLong;
        reserves[1] = newReserveShort;
        collateralReserve = collateralBalance;
        emit Buy(msg.sender, to, option, amount, shares, currentRound, reserves);
    }

    function sell(
        address to,
        uint8 option
    ) external override onlyNotStopped whenNotPaused returns (uint256 amount) {
        require(to != address(0), "to is address zero");
        require(option <= 1, "wrong option");
        bool isLong = option == 0 ? true : false;
        RoundData memory roundData = getRoundData[currentRound];
        ...
        uint256 newReserveShort;
        if (isLong) {
            newReserveLong = reserves[0] + shares;
            newReserveShort = reserves[0].mulDiv(reserves[1], newReserveLong);
            ...
            newReserveShort = reserves[1] + shares;
            newReserveLong = reserves[0].mulDiv(reserves[1], newReserveShort);
            amount = shares.mulDiv(
                newReserveLong,
                newReserveLong + newReserveShort
        ...
        emit Sell(msg.sender, to, option, amount, shares, currentRound, reserves);
    }
    ...
    function setStartPrice() external override onlyNotStopped {
        ...
        (int256 targetPrice, uint80 targetOracleRoundID) = _getOraclePrice(roundData.tradeEndTime);
    ...
    function endCurrentRound() external override onlyNotStopped {
        // set endPrice
        RoundData memory roundData = getRoundData[currentRound];
        require(
        ...
        (int256 targetPrice, uint80 targetOracleRoundID) = _getOraclePrice(roundData.roundEndTime);
        ...
        returns (uint256[] memory tokenIds, uint256[] memory amounts)
    {
        ...
        for (uint256 i = 0; i < rounds.length; i++) {
            round = rounds[i];
            require(round < currentRound, "over current round");
            data = getRoundData[round];
            if (data.oracleRoundIdOfEndPrice > data.oracleRoundIdOfStartPrice) {
                if (data.startPrice < data.endPrice) {
                    tokenIds[i] = round * 2;
    ...
    function _getOraclePrice(uint256 endTime) internal view returns (int256 targetPrice, uint80 targetOracleRoundID) {
        ...
        if (endTime >= preUpdatedAt && endTime < nextUpdatedAt) {
            targetPrice = prePrice;
            targetOracleRoundID = preRoundID;
        } else {
            targetPrice = nextPrice;
            targetOracleRoundID = nextRoundID;
        }
    }
