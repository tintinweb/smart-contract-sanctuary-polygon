...
import "../interfaces/IDfynCallBack.sol";
import "../libraries/PathLib.sol";
import "../libraries/SafeCast.sol";
//import "../interfaces/IQuoterV2.sol";
import "../libraries/CallbackValidation.sol";
import "../libraries/TickMath.sol";
import "../interfaces/IDfynRouter.sol";
import "../interfaces/IPool.sol";
import "../libraries/TickCounter.sol";

/// @title Provides quotes for swaps
/// @notice Allows getting the expected amount out or amount in for a given swap without executing the swap
/// @dev These functions are not gas efficient and should _not_ be called on chain. Instead, optimistically execute
/// the swap and check the amounts in the callback.
contract QuoterV2 is IDfynCallBack, IDfynRouter, PeripheryImmutableState {
    using PathLib for bytes;
    using SafeCast for uint256;

    //   using PoolTicksCounter for IConcentratedLiquidityPool;
    ...
    function getPool(address tokenIn, address tokenOut) private view returns (IConcentratedLiquidityPool) {
        if (tokenIn > tokenOut) (tokenIn, tokenOut) = (tokenOut, tokenIn);
        return IConcentratedLiquidityPool(PoolHelper.computeAddress(factory, tokenIn, tokenOut));
    }

    ///@inheritdoc IDfynCallBack
    function swapCallBack(
        ...
        IConcentratedLiquidityPool pool = getPool(tokenIn, tokenOut);
        require(unWrapVault);
        (uint160 sqrtPriceX96After, ) = pool.getPriceAndNearestTicks();
        ...
        if (isExactInput) {
            ...
            if (amountOutCached != 0) require(amountOut == amountOutCached);
            assembly {
                let ptr := mload(0x40)
                mstore(ptr, amountIn)
                mstore(add(ptr, 0x20), sqrtPriceX96After)
    ...
    function handleRevert(
        ...
        uint256 gasEstimate
    ) private view returns (Response memory) {
        int24 tickAfter;
        uint256 amount;
        uint160 sqrtPriceX96After;
        uint32 initializedTicksCrossed;
        (, int24 tickBefore) = pool.getPriceAndNearestTicks();

        (amount, sqrtPriceX96After, tickAfter) = parseRevertReason(reason);

        initializedTicksCrossed = TickCounter.countTicks(pool, tickBefore, tickAfter);
    ...
    //0xeae012ba93f27748bce5187db57e7f1916b98f8a20736d547f67045e073fb81f
    function quoteExactInputSingle(
        address tokenIn,
        address tokenOut,
        bytes memory params,
        bytes memory path
    ) public returns (Response memory response) {
        IConcentratedLiquidityPool pool = getPool(tokenIn, tokenOut);
        // console.log("Pool address in Quoter: ", address(pool));
        // IConcentratedLiquidityPool pool = IConcentratedLiquidityPool(params.pool);
        // params.zeroForOne = tokenIn < tokenOut;
        uint256 gasBefore = gasleft();
        try IPool(address(pool)).swap(params, path) {} catch (bytes memory reason) {
            uint256 gasEstimate = gasBefore - gasleft();
            response = handleRevert(reason, pool, gasEstimate);
        }
    }
        ...
        )
    {
        ...
        // (bool zeroForOne, address recipient, bool unwrapVault, uint256 amountIn) = abi.decode(data, (bool, address, bool, uint256));

        uint256 i = 0;
        while (true) {
            {
                (address tokenIn, address tokenOut) = path.decodeFirstPool();
                // console.log("Token addresses in Quoter", tokenIn, tokenOut);
                // the outputs of prior swaps become the inputs to subsequent ones
                // bool zeroForOne = tokenIn < tokenOut;
                Response memory response = quoteExactInputSingle(
                    tokenIn,
                    tokenOut,
                    abi.encode(tokenIn < tokenOut, address(this), true, amountIn.toInt256()),
                    path
                );
                sqrtPriceX96AfterList[i] = response.sqrtPriceX96After;
                initializedTicksCrossedList[i] = response.initializedTicksCrossed;
                ...
                if (!path.hasMultiplePools()) return (amountIn, sqrtPriceX96AfterList, initializedTicksCrossedList, gasEstimate);
                path = path.skipToken();
            }
        }
    }
        ...
        )
    {
        ...
        while (true) {
            ...
            Response memory response = quoteExactOutputSingle(
                tokenIn,
                tokenOut,
                abi.encode(tokenIn < tokenOut, address(this), true, -amountOut.toInt256()),
                path
            );
...
library CallbackValidation {
    ...
    function verifyCallback(
        ...
        address tokenA,
        address tokenB
    ) internal view returns (IConcentratedLiquidityPool pool) {
        //return verifyCallback(factory, tokenA, tokenB);
        if (tokenA > tokenB) {
            (tokenA, tokenB) = (tokenB, tokenA);
        }
        pool = IConcentratedLiquidityPool(PoolHelper.computeAddress(factory, tokenA, tokenB));
...
library TickCounter {
    ...
    function countTicks(
        ...
        int24 ticksCrossed = 0;

        if (tickAfter > tickBefore) {
            ticksCrossed = tickBefore;
            while (tickAfter >= ticksCrossed) {
                ticksCrossed = pool.ticks(ticksCrossed).nextTick;
                unchecked {
                    count += 1;
                }
                if (ticksCrossed == MAX_TICK) break;
            }
        } else {
            ticksCrossed = tickBefore;
            while (ticksCrossed <= tickAfter) {
                ticksCrossed = pool.ticks(ticksCrossed).previousTick;
                unchecked {
                    count += 1;
                }
                if (ticksCrossed == MIN_TICK) break;
            }
        }
    }
}
