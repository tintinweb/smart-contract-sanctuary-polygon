...
contract Poption is IERC1155 {
    ...
    event ExercisePoption(address indexed owner, uint128 asset, uint128 tail);
    ...
    function swap(
        ...
        ISwap(marketMaker).onSwap(msg.sender, _out, _in);
    ...
    function exerciseTail(uint128 tail) public noReentrant {
        ...
        emit TransferBatch(msg.sender, msg.sender, address(0), allIds, value);
        emit ExercisePoption(msg.sender, asset, tail);
    }
...
contract BaseCFMMSwap is ISwap, IERC20Metadata {
    ...
    event Swap(
        ...
        uint128[STRIKE_NUM] in_,
        uint128[STRIKE_NUM] out_,
    ...
    function _onSwap(
        address sender_,
        uint128[STRIKE_NUM] calldata out_,
        uint128[STRIKE_NUM] calldata in_
    ) internal virtual {
        uint128[STRIKE_NUM] memory weight = getWeight();
        uint128[STRIKE_NUM] memory lpTo;
        for (uint256 i = 0; i < STRIKE_NUM; i++) {
            uint128 outi = out_[i].mul(feeRate);
            require(liqPool[i] > outi, "PLQ");
            lpTo[i] = liqPool[i] + in_[i].div(feeRate) - outi;
        }
        require(tradeDiff(liqPool, lpTo, weight) >= 0, "PMC");
        for (uint256 i = 0; i < STRIKE_NUM; i++) {
            liqPool[i] = liqPool[i] + in_[i] - out_[i];
        }
        emit Swap(sender_, in_, out_, weight, liqPool);
    }
