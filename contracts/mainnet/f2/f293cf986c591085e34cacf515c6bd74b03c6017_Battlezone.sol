...
contract Battlezone is IBattlezone, WarpBase, ReentrancyGuardUpgradeable {
    ...
    */
    function addStarship(uint256 _starshipId, uint112 _battleBid) external notContract {
        ...
        uint256 usdBattleBid = IBanker(banker).valueOf(bidToken, _battleBid); //calculate the value of warp in DAI
        require(usdBattleBid >= 10 * minBidFactor, 'Battle bid must over 10'); //must bid a minimum of 10 DAI worth of WARP

        defender.hull = shipInfo.hull - shipInfo.hullDmg;
        defender.bridge = shipInfo.bridge - shipInfo.bridgeDmg;
        defender.engine = shipInfo.engine - shipInfo.engineDmg;
        defender.strength = defender.hull + defender.bridge;
    ...
    */
        ...
        notContract
    {
        ...
        uint256 strength = ship.hull + ship.bridge - ship.hullDmg - ship.bridgeDmg; //calculate current battle strength for starship
        ...
        uint16 firePower = getFirePower(strength, false);

        //** Obtain defender power information */
        uint16 opponentFirePower = getFirePower(
    ...
    */
    function engageInBattleAdvanced(
        ...
        if (attacker.inFightWith == 0) {
            ...
            defender.inFightWith = uint112(_starshipId); //let the opponent know they are in fight with _starshipId
            defender.lastHitTime = uint64(block.timestamp);
            ...
            attacker.hull = asInfo.hull - asInfo.hullDmg;
            attacker.bridge = asInfo.bridge - asInfo.bridgeDmg;
            attacker.engine = asInfo.engine - asInfo.engineDmg;
            attacker.strength = attacker.hull + attacker.bridge;
            require(
                attacker.strength <= defender.strength,
                'You cannot fight a less powerful starship'
            );
            attacker.battleBid = defender.battleBid;
            attacker.inFightWith = uint112(_opponentStarshipId); //let the opponent know they are in fight with _starshipId
            attacker.lastHitTime = uint64(block.timestamp);
    ...
    function fireShot(
        ...
        uint16 firePower = getFirePower(battleStarships[_starshipId].strength, true); //set to true for advanced battle - different fire power percentage
        uint16 opponentFirePower = getFirePower(
            battleStarships[_opponentStarshipId].strength,
            true
        );
        ...
        if (battleStarships[_starshipId].engine > battleStarships[_opponentStarshipId].engine) {
    ...
    */
    function getFirePower(uint256 _strength, bool _advancedBattle) internal returns (uint16) {
        ...
        uint16 firePower = uint16((_strength * index) / 100);
        return firePower;
    }
    ...
    function isWinningShot(
        ...
        bool isWinner;
        if (_fireAtPart == 0) {
            if (battleStarships[_starshipId].hull <= _firePower) {
                isWinner = true;
                battleStarships[_starshipId].hull = 0;
                battleStarships[_starshipId].strength = battleStarships[_starshipId].bridge;
            } else {
                battleStarships[_starshipId].hull -= _firePower;
                battleStarships[_starshipId].strength -= _firePower;
            }
        }
        if (_fireAtPart == 1) {
            if (battleStarships[_starshipId].bridge <= _firePower) {
                isWinner = true;
                battleStarships[_starshipId].bridge = 0;
                battleStarships[_starshipId].strength = battleStarships[_starshipId].hull;
            } else {
                battleStarships[_starshipId].bridge -= _firePower;
                battleStarships[_starshipId].strength -= _firePower;
            }
        }
        if (_fireAtPart == 2) {
            if (battleStarships[_starshipId].engine <= _firePower) {
                isWinner = true;
                battleStarships[_starshipId].engine = 0;
            } else {
                battleStarships[_starshipId].engine -= _firePower;
            }
        }
    ...
    //change battle bid
    function changeBattleBid(uint256 _starshipId, uint112 _newBattleBid)
        external
        nonReentrant
        notContract
    {
        ...
        uint256 usdBattleBid = IBanker(banker).valueOf(bidToken, _newBattleBid); //calculate the value of warp in DAI
        require(usdBattleBid >= 10 * minBidFactor, 'Battle bid must be highen than 10');
    ...
    */
    ...
    function setBankerAddress(address _banker) external onlyOwner {
        banker = _banker;
    }
    ...
    function getWeakestPart(uint256 _starshipId) internal view returns (uint256) {
        uint256 weekestPart;
        if (battleStarships[_starshipId].hull > battleStarships[_starshipId].bridge) {
            weekestPart = 1;
        }
        if (battleStarships[_starshipId].bridge > battleStarships[_starshipId].engine) {
            weekestPart = 2;
        }
        return weekestPart;
    }
...
struct Ship {
    string name;
    uint80 experience;
    uint8 brand; // 21 brands
    uint8 archetype; // 11 archetypes
    uint16 archetypeIndex; // 1000 indexes
    uint16 origin; // max chainId number 43114 (avax)
    uint16 bonus;
    uint16 bridge;
    uint16 hull;
    uint16 engine;
...
interface IStarshipControl {
    ...
    function isPioneer(uint256 _shipId) external view returns (bool);

    function buidl(uint16[3] calldata _values, uint8 archetype) external;
...
abstract contract StarshipTranslations {
    function translateBrand(uint8 brand) public pure returns (string memory) {
        string[21] memory brands = [
            'Warp',
            'Axion',
            'Galaktic Gang',
            'Godjira',
        ...
        if (brand >= brands.length) return 'Unknown';

        return brands[brand];
    }

    function translateArchetype(uint8 archetype) public pure returns (string memory) {
        string[11] memory archetypes = [
            'Vanguard',
            'Hercules',
            'Javelin',
            'Shuttle',
        ...
        if (archetype >= archetypes.length) return 'Unknown';

        return archetypes[archetype];
    }

    function translateOrigin(uint16 origin) public pure returns (string memory) {
        if (origin == 1) return 'Ethereum';
        if (origin == 137) return 'Polygon';
        if (origin == 250) return 'Fantom';
...
struct BattleShip {
    // 256 bytes
    uint112 battleBid; //ex: 10 represents 10 DAI. No decimals.
    uint112 inFightWith; //starship id of the in fight with - advanced use only
    bool attacker; //false if starship is listed on the battlefield, true if starship is the attacker - advanced use only
    uint8 openForBattle; // 0 not open for battle, 1 open for battle, 2 engaged in battle
    uint64 lastHitTime; //time of last hit - advanced use only
    uint16 strength; //hull + bridge
    uint16 hull; //advanced use while in battle
    uint16 bridge; //advanced use while in battle
    uint16 engine; //advanced use while in battle
}
