...
contract MaxityRouterV2 is Ownable,IMRouter,ReentrancyGuard{
    ...
    address public marketnew;
    address public marketold;
    ...
    function setMarketNew (address _marketnew) public onlyOwner{
        marketnew = _marketnew;
    }

     function setMarketOld (address _marketold) public onlyOwner{
        marketold = _marketold;
    }
    ...
    //mintAndSellMax721
    function mintAndSellMax721(address _token,address designer
        ...
        uint256 [] memory new_tokenids =  IMaxity721Token(_token).mint(designer, marketnew,_futureRoyaltys, tokenURIOrigins);
        IMaxityMarketPlace(marketnew).sellByFixPrice(_token,new_tokenids,unit_prices,msg.sender);
        return new_tokenids;
    }
    ...
    function mintAndAuctionMax721(address _token,address designer
        ,uint [] memory _futureRoyaltys,string []memory tokenURIOrigins
        ,uint [] memory base_prices,uint [] memory incre_prices,uint [] memory starttimes,uint [] memory deadlines
        ,bytes calldata ) external override returns (uint[] memory tokenids){
        ...
        uint256 [] memory new_tokenids =  IMaxity721Token(_token).mint(designer, marketnew, _futureRoyaltys, tokenURIOrigins);
        IMaxityMarketPlace(marketnew).auctionMax721(_token,new_tokenids,base_prices,incre_prices,starttimes,deadlines,msg.sender);
        return new_tokenids;
    }
    ...
    function buyNative(address _token,uint256 _tokenid,address token_to,address nativeSweep) public nonReentrant override payable {
        ...
        address targetMarket = marketnew;
        if(IERC721(_token).ownerOf(_tokenid)==marketold){
            //by from old market
            targetMarket = marketold;
        }

        assert(IWMATIC(wrappedNative).approve(targetMarket, msg.value));
        // uint before_balance = IWMATIC(wrappedNative).balanceOf(address(this));
        IMaxityMarketPlace(targetMarket).buy(_token,_tokenid,token_to);
    ...
    function bidNative(address _token,uint256 _tokenid,address token_to,address nativeSweep) public nonReentrant override payable {
        ...
        assert(IWMATIC(wrappedNative).approve(targetMarket, msg.value));
        // uint before_balance = IWMATIC(wrappedNative).balanceOf(address(this));
        IMaxityMarketPlace(targetMarket).bid(_token,_tokenid,msg.value,token_to);
        uint after_balance = IWMATIC(wrappedNative).balanceOf(address(this));
        if(after_balance>0)
        {
            IWMATIC(wrappedNative).transfer(nativeSweep, after_balance);
    ...
    function auctionOnFixPriceNative(address _token,uint _tokenid,address token_to,address nativeSweep)public nonReentrant override payable{
        IWMATIC(wrappedNative).deposit{value: msg.value}();
        address targetMarket = marketnew;
        if(IERC721(_token).ownerOf(_tokenid)==marketold){
            //by from old market
        ...
        assert(IWMATIC(wrappedNative).approve(targetMarket, msg.value));
        // uint before_balance = IWMATIC(wrappedNative).balanceOf(address(this));
        IMaxityMarketPlace(targetMarket).auctionOnFixPrice(_token,_tokenid,msg.value,token_to);
        uint after_balance = IWMATIC(wrappedNative).balanceOf(address(this));
        if(after_balance>0)
        {
            IWMATIC(wrappedNative).transfer(nativeSweep, after_balance);
    ...
    function delist(address _token,uint256 _tokenid) public onlyOwner override{
        IMaxityMarketPlace(marketnew).delist(_token,_tokenid);
    }
...
interface IMRouter  {
    ...
    function mintAndAuctionMax721(address _token,address designer
        ,uint [] memory _futureRoyalty,string []memory tokenURIOrigin
        ,uint [] memory base_price,uint [] memory incre_price,uint [] memory starttime,uint [] memory deadline
        ,bytes calldata _data) external  returns (uint[] memory tokenids);

    //通过主链币购买
    function buyNative(address _token,uint256 _tokenid,address token_to,address nativeSweep) external payable ;

    //通过主链币竞价
    function bidNative(address _token,uint256 _tokenid,address token_to,address nativeSweep) external payable ;

    //下架
    function delist(address _token,uint256 _tokenid) external ;

    //喊价模式
    function auctionOnFixPriceNative(address _token,uint _tokenid,address token_to,address nativeSweep)external payable;
...
interface IMaxityMarketPlace{
    ...
    function auctionOnFixPrice(address _token,uint _tokenid,uint _price,address price_offer) external;
