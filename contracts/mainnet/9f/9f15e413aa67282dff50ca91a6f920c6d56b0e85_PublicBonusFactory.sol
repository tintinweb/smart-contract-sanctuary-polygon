...
contract PublicBonus{
    ...
    IPublicBonusFactory public factory;
    ...
    constructor(){
        factory = IPublicBonusFactory(msg.sender);
        owner = tx.origin;
    }
    ...
    function getUserStakedInfo(StakingChoice _choice, address _target) internal view returns(uint userStaked){
        if(_choice == StakingChoice.V1){
            ...
            userStaked = KommunitasStaking(factory.stakingV2()).getUserStakedTokens(_target);
        }else{
            revert("!good");
        }
    }
    ...
    function getUserAllocToken(address _user) internal view returns(uint userAllocToken){
        uint stakedV1 = getUserStakedInfo(StakingChoice.V1, _user);
        uint stakedV2 = getUserStakedInfo(StakingChoice.V2, _user);
        userAllocToken = stakedV1.add(stakedV2);
    }
     ...
     * @param _user User address
     * @param _tokenReceived Token received amount final
     * @param _amountIn Amount in to buy
     */
    ...
    function buyToken(uint _amountIn, address _tokenIn) isBoosterProgress isNotPaused public {
        ...
        require(getUserAllocToken(msg.sender) > 0, "!found");
...
contract PublicBonusFactory is IPublicBonusFactory{
    ...
    function createProject(
        ...
        project = address(new PublicBonus());

        allProjects.push(project);
        
        PublicBonus(project).initialize(
            _payment,
            _sale,
            _target,
            _start,
     ...
     */
    function config(address _devAddr, address _savior) public override onlyOwner{
