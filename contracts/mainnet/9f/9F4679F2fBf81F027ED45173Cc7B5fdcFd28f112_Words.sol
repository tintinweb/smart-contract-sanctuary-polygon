...
contract Words is ERC721BaseInternal, WordsInternal, VRFInternal {
    ...
    function powerCalculator(uint256 value, uint256 supply) public pure returns(uint256) {
        return _powerCalculator(value, supply);
    }
}
...
abstract contract WordsInternal {
    ...
    function __wordsInternal_init() internal {
        ...
        uint256 newPower = _powerCalculator(t.value, 10);
    ...
    function _newWord(
        ...
        uint256 power = _powerCalculator(value, id+1);
    ...
    function _dom(
        ...
        uint256 power = _powerCalculator(
            value * (10000 - l.withdrawableValueFraction) / 10000, 
            _nextTokenId()
        );
    ...
    function _powerCalculator(uint256 value, uint256 supply) internal pure returns(uint256 power) {
        uint8 divisor = 1;
        while(supply > 10) {
            supply /= 10;
            divisor *= 5;
