...
*/
contract TestStrategyPolygonBeefyQSwapUSDCTUSD is AccessControlUpgradeable, SafeAllowanceResetUpgradeable, EmergencyTimelock, IStrategy {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    function initialize(address _baseAsset, address _poolAddress,
                        address _beefyProxyAddr, address _beefyVaultAddr) public initializer {
    ...
    function setParams(address _baseAsset, address _beefyProxyAddr, address _beefyVaultAddr) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "admin only");
    ...
    function investInVault(uint256 _amount, uint256 _minimumAmount) public {
        ...
        resetAllowanceIfNeeded(IERC20Upgradeable(baseAsset), beefyProxyAddress, amountReceived);
        IBeefyProxy(beefyProxyAddress).beefIn(beefyVaultAddress, 0, baseAsset, amountReceived);
        ...
        uint256 lpFull = calcShareAmount(amountReceived);

        if (lpFull > lpReceived) {
            // realize small loss
            // loss = amount * (lpFull - lpReceived) / (lpFull)
            uint256 zapLoss = amountReceived.mul(lpFull.sub(lpReceived)).div(lpFull);
            amountInvested = amountInvested.add(amountReceived.sub(zapLoss));
            moverPool.realizeLoss(zapLoss);
        } else {
    ...
    function calcShareAmount(uint256 _amount) public view returns(uint256 sharesEstimate) {
        ...
        uint256 balanceLPUSDC = IERC20Upgradeable(USDC).balanceOf(USDCTUSDQuickSwapLP); // 6 decimals
        uint256 balanceLPTUSD = IERC20Upgradeable(TUSD).balanceOf(USDCTUSDQuickSwapLP); // 18 decimals
        uint256 totalUSD18Decimals = balanceLPUSDC.mul(1e12).add(balanceLPTUSD);
    ...
    function divestFromVault(uint256 _amount, bool _safeExecution) public {
        ...
        IBeefyProxy(beefyProxyAddress).beefOutAndSwap(beefyVaultAddress, sharesToWithdraw, baseAsset, 0);
        uint256 amountAfter = IERC20Upgradeable(baseAsset).balanceOf(address(this));
        ...
        moverPool.returnInvested(received);
    ...
    function reclaimFunds(uint256 _amount, bool _safeExecution) external override returns(uint256) {
        ...
        emit WithdrawReclaim(amountLP, _amount, received, lpTokensBalance);
    ...
    function harvestYield(uint256 minExpectedAmount, uint256 maxAmount) public {
        ...
        uint256 amountUSDC = IERC20Upgradeable(baseAsset).balanceOf(address(this));
        uint256 amountLP = IERC20Upgradeable(beefyVaultAddress).balanceOf(address(this));
        IBeefyProxy(beefyProxyAddress).beefOutAndSwap(beefyVaultAddress, lpTokensToWithdraw, baseAsset, 0);
        amountUSDC = IERC20Upgradeable(baseAsset).balanceOf(address(this)).sub(amountUSDC);
        amountLP = amountLP.sub(IERC20Upgradeable(beefyVaultAddress).balanceOf(address(this)));
        ...
        emit HarvestYield(lpTokensToWithdraw, lpTokensToWithdraw.mul(1e6).div(sharesForOneUSDC), amountUSDC, lpTokensBalance);
    }
}