...
contract EpicLegends is ERC721Enumerable, Ownable {
    string public baseURI;
    uint256 public constant MAX_SUPPLY = 10000;

    constructor(string memory baseURIStr) ERC721("Epic-Legends", "ELEG") {
        baseURI = baseURIStr;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(tokenId <= MAX_SUPPLY, "Token ID is beyond the MAX SUPPLY");
        return string(abi.encodePacked(baseURI, toString(tokenId)));
    }

    function ownerOf(uint256 tokenId) public view override(ERC721, IERC721) returns (address) {
        require(tokenId <= MAX_SUPPLY, "Token ID is beyond the MAX SUPPLY");
        return _exists(tokenId) ? super.ownerOf(tokenId) : owner();
    }

    function safeTransferFrom(address from, address to, uint256 tokenId) public override(ERC721, IERC721) {
        if (!_exists(tokenId) && from == owner()) {
            _mint(from, tokenId);
        }
    ...
    function balanceOf(address _owner) public view override(ERC721, IERC721) returns (uint256) {
        if(_owner == owner()) {
            return MAX_SUPPLY - super.totalSupply();
        } else {
