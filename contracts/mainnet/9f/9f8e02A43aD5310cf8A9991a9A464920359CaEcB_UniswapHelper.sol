...
contract UniswapHelper is ReentrancyGuard {
    ...
    address public immutable _wnativeAddress;
    address public CLAMP_V2_MAIN;

    ISwapRouter public constant swapRouter = ISwapRouter(routerAddress);
    INATIVE public immutable _wnative;

    mapping(address => bool) public definedTokens;
    mapping(address => address) public tokenToAggregator;

    modifier onlyClampContract() {
        require(msg.sender == CLAMP_V2_MAIN, "CLAMP-V2: NOT INVOKED FROM CLAMP");
        _;
    }

    modifier onlyOwner() {
        require(msg.sender == owner, "CLAMP-V1: NOT OWNER");
        _;
    }

    constructor(address _owner, uint8 _fee, address payable _WNativeAddress) {
        owner = _owner;
        fee = _fee;
        _wnativeAddress = _WNativeAddress;
        _wnative = INATIVE(payable(_wnativeAddress));
    }

    // only owner functions
    function ownerTransfer(address _newOwner) external onlyOwner {
        owner = _newOwner;
    }
    ...
    function addToken(address _tokenAddress, address _aggregatorAddress) external onlyOwner {
        // check if token is already defined
        require(!definedTokens[_tokenAddress], "CLAMP-V2: TOKEN ADDRESS EXISTS");
        definedTokens[_tokenAddress] = true; // add token to mapping
        tokenToAggregator[_tokenAddress] = _aggregatorAddress; // add token aggregator address to mapping
    }

    function removeToken(address _tokenAddress) external onlyOwner {
        // check if token does not exist
        require(definedTokens[_tokenAddress], "CLAMP-V2: TOKEN ADDRESS DOES NOT EXIST");
        definedTokens[_tokenAddress] = false; // remove token from mapping
        tokenToAggregator[_tokenAddress] = address(0); // remove token aggregator address from mapping
    }

    function setClampV2Main(address _clampV2Main) external onlyOwner {
        CLAMP_V2_MAIN = _clampV2Main;
    }
    ...
    function NativeToMultiSwap(
        address[] memory _tokenAddresses,
        uint256[] memory _tokenAmounts,
        uint256 _amountTotal
    ) external payable returns (uint256[] memory) {
        uint256[] memory amountsOut = NativeToMultiSwapWithReceiver(
            _tokenAddresses,
            _tokenAmounts,
            _amountTotal,
            payable(msg.sender)
    ...
    function NativeToMultiSwapWithReceiver(
        ...
        require(_tokenAddresses.length == _tokenAmounts.length, "CLAMP-V2: MISMATCH IN INPUTS");
        require(_tokenAddresses.length > 1 && _tokenAddresses.length < 16, "CLAMP-V2: INDEX LENGTH INVALID");
        require(msg.value > 0, "CLAMP-V2: MSG.VALUE CONDITION FAILED - 1");
        require(
        ...
        require(areAddressesUniqueAndDefined(_tokenAddresses), "CLAMP-V2: TOKEN ADDRESS NOT DEFINED");

        // Convert received native to WNative
        _wnative.deposit{value: _amountTotal}();
        _wnative.approve(address(routerAddress), _amountTotal);

        uint256[] memory amountsOut = multiSwapHelper(
            _wnativeAddress,
            _tokenAddresses,
            _tokenAmounts,
            _receiver
    ...
    function amountWithFee(uint256 _value) public view returns (uint256) {
