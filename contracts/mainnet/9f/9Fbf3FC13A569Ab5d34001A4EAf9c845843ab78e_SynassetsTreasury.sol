...
contract SynassetsTreasury is Ownable {
    ...
    uint public sSYNASSETSQueue; // Delays change to sSYNASSETS address
    ...
    /**
        @notice allow approved address to deposit an asset for SYNASSETS
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint SYNASSETS needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( SYNASSETS ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn SYNASSETS for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ISYNASSETSERC20( SYNASSETS ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sSYNASSETS ).balanceOf( msg.sender ); // Can only borrow against sSYNASSETS held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with SYNASSETS
        @param _amount uint
     */
    function repayDebtWithSYNASSETS( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISYNASSETSERC20( SYNASSETS ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, SYNASSETS, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( SYNASSETS ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( SYNASSETS ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns SYNASSETS valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match SYNASSETS decimals
            value_ = _amount.mul( 10 ** IERC20( SYNASSETS ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SSYNASSETS ) { // 9
            sSYNASSETSQueue = block.number.add( blocksNeededForQueue );
        } else return false;
