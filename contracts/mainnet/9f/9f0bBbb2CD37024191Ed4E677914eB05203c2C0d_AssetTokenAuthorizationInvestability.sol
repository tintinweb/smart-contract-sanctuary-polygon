...
contract AssetTokenAuthorizationInvestability is AccessControlUpgradeable {
    ...
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "AssetTokenAuthInvest: only DEFAULT_ADMIN");
        _;
    }

    /// @notice Check if the passed address is not zero
    modifier zeroAddressCheck(address account) {
        require(account != address(0), "AssetTokenAuthInvest: passed address = 0");
        _;
    }
    ...
    function initialize(
        ...
        external
        zeroAddressCheck(_permissionManagerAddress)
        zeroAddressCheck(_eurPriceFeedAddress)
        zeroAddressCheck(_assetTokenFactoryAddress)
        initializer
    {
    ...
    function regularAssetValidation(address _account) internal view returns (bool) {
        ...
        bool hasSoFToken = permissionManager.hasSoFToken(_account);

        return (hasTier2 && !isSuspended && !isRejected && hasSoFToken);
    }

    /// @notice Returns true if `account` is authorized in Swarm Ecosystem
    /// @param _account the address to be checked
    /// @return bool true if `account` is authorized
    function isAccountAuthorized(address _account) external view zeroAddressCheck(_account) returns (bool) {
    ...
    /// @param _tokenAddress the address of the current token
    /// @return bool true if `account` is authorized
    function securityAssetValidation(address _account, uint256 _amount, address _tokenAddress) internal returns (bool) {
        require(hasRole(AUTHORIZED_CALLER_ROLE, _msgSender()), "AssetTokenAuthInvest: only AUTHORIZED_CALLER");
        ...
        require(IERC20Metadata(_tokenAddress).decimals() == 18, "AssetTokenAuthInvest: decimals must be 18");
    ...
    function isTxAuthorized(
        ...
        require(assetTokenFactory.isTokenEnabled(_tokenAddress), "AssetTokenAuthInvest: tokenAddress is not enabled");
    ...
    function setPermissionManagerAddress(
        address _permissionManagerAddress
    ) external zeroAddressCheck(_permissionManagerAddress) onlyAdmin {
    ...
    function setEurPriceFeedAddress(
        address _eurPriceFeedAddress
    ) external zeroAddressCheck(_eurPriceFeedAddress) onlyAdmin {
    ...
    function setAssetTokenFactoryAddress(
        address _assetTokenFactoryAddress
    ) external zeroAddressCheck(_assetTokenFactoryAddress) onlyAdmin {
    ...
    function setValidationAsSecurityAndAmount(
        bool _validateAsSecurityToken,
        uint256 _fiatAmountValidation
    ) public onlyAdmin {
        if (_validateAsSecurityToken) {
            require(_fiatAmountValidation > 0, "AssetTokenAuthInvest: fiatAmountValidation must be > 0");
        } else {
            require(_fiatAmountValidation == 0, "AssetTokenAuthInvest: fiatAmountValidation must be 0");
        }
...
interface IEurPriceFeed {
     ...
     */
    function setAssetFeed(address _asset, address _feed, address _denomination) external;
}
