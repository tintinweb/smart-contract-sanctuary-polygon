...
// File contracts/interfaces/IMnAGame.sol
...
// File contracts/interfaces/IORES.sol
...
// File contracts/interfaces/IMnA.sol
...
// File contracts/interfaces/ISpidox.sol
...
// File contracts/interfaces/IRandomSeedGenerator.sol
...
contract MnAGameCR is UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, IMnAGame {
  ...
  // reference to randomSeedGenerator
  IRandomSeedGenerator public seedGenerator;
...
function _authorizeUpgrade(address) internal override onlyOwner {}
  ...
  function setRandomSeedGenerator(address _seedGenerator) external onlyOwner {
    seedGenerator = IRandomSeedGenerator(_seedGenerator);
  }
  ...
  function canMint(address addr) external view returns (bool) {
    return _pendingCommitId[addr] != 0 && (_commitRandoms[_pendingCommitId[addr]] > 0 || address(seedGenerator) != address(0));
  }
  ...
  function _addCommitRandom(uint256 seed) internal {
    _commitRandoms[_commitId] = seed;
    _commitId += 1;
  }  
  ...
  function reveal(address addr) internal {
    ...
    require(commitIdCur > 0, "No pending commit");
    if (_commitRandoms[commitIdCur] == 0) {
      require(address(seedGenerator) != address(0), "random seed not set");
      uint256 randomNum = seedGenerator.random();
      _addCommitRandom(randomNum);
    }
