...
contract CSPEarlyAccessPass is ERC721, Pausable, AccessControl, TransferWhitelistable {
    ...
    constructor(address[] memory initialWhitelist, address initAdminWallet, string memory initialBaseURI) ERC721("CSPEarlyAccessPass", "CEA") TransferWhitelistable(initialWhitelist) {
    ...
    function safeBatchMint(address[] calldata dest) public onlyRole(MINTER_ROLE) {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId)
        internal
        whenNotPaused
        onlyTransferWhitelisted(from, to)
        override
    {
...
abstract contract TransferWhitelistable {
    ...
    modifier onlyTransferWhitelisted(address from, address to) {
        ...
        require(from == address(0) || whitelisted(from) || whitelisted(to), "TransferWhitelistable: not whitelisted for transfer");
        _;
    }
