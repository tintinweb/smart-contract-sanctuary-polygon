...
contract Vault is BaseContract {
  ...
  uint private test; // TODO remove on next deployment
  ISwap public swap;
  ...
  function depositFor(address _userAddress, uint _amount, IVault.DepositType _depositType) public {
    require(internalCalling || msg.sender == addressBook.get("swap"), "Vault: only swap");
  ...
  function buyAutoCompound(uint _days) external payable {
    ...
    uint extraDay = config.secondsInADay * _days + config.secondsInADay - 1;
    User storage user = users[msg.sender];
  ...
  function updateQualifiedLevel(address _user1Address, address _user2Address) external {
    ...
    if(!(_user1Address == address(0) || _user1Address == shareAddress)) {
      _updateQualifiedLevel(_user1Address, rankingNft.balanceOf(_user1Address), stToken.balanceOf(_user1Address));
    }
    if(!(_user2Address == address(0) || _user2Address == shareAddress)) {
      _updateQualifiedLevel(_user2Address, rankingNft.balanceOf(_user2Address), stToken.balanceOf(_user2Address));
    }
  }
  ...
  function _increaseInviterDirectQualifiedF1(address _userAddress) private {
    ...
    _updateQualifiedLevel(inviterAddress, rankingNft.balanceOf(inviterAddress), stToken.balanceOf(inviterAddress));
  }
