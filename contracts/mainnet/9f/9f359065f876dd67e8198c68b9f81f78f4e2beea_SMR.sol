...
contract SMR is ERC721Upgradeable, UUPSUpgradeable, OwnableUpgradeable {

    // total number of SMRs minted
    uint256 public _smrCounter;
    // total supply which can be different from smrCounter in case some tokens are burned
    uint256 private _currentSupply;

    // map SMR's token id to its tokenURI
    ...
    function initialize() public initializer{
        ...
        __ERC721_init('SIGNIS Metaverse Rosary', "SMR");
        __Ownable_init();
    ...
    function mint(address to, string memory _tokenURI) external virtual onlyOwner  {
        ...
        require(_currentSupply < 10000, "Minting limit reached!");
        ...
        tokenURIs[_smrCounter] = _tokenURI;
        tokenURIExists[_tokenURI] = true;

        //minting the token
        _mint(to, _smrCounter);
    ...
    //get current tokenId
    function getSmrCount() external view returns(uint256 counter){
        return _smrCounter;
    }
