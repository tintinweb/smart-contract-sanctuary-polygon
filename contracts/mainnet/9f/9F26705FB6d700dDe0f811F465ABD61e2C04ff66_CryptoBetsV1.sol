...
contract CryptoBetsV1 is Ownable{
  ...
  function concludeBetting(bool startNew) external onlyOwner {
    ...
    require(playersFor.length!=0 && playersAgainst.length!=0,"Preventing burning of tokens: players=0 !");// Make sure the tokens will go to a player.
    bettingState = BETTING_STATE.CLOSED;
    (,int256 price,,,)= ethUsdPriceFeed.latestRoundData();
