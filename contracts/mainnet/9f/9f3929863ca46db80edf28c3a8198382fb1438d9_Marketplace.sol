    ...
    ReentrancyGuardUpgradeable
{
    ...
    function setPropertyVerificationAddress(
        address newPropertyVerificationAddress
    ) external onlyAdmin {
    ...
    function cancelBnftAuction(uint256 auctionId) external {
        // if the 'buy it now' price has been met, the auction should be removed already
        require(bnftAuctionIds.contains(auctionId), "Auction does not exist");
        BnftAuction memory auction = bnftAuctions[auctionId];
        require(
        ...
        // auction must be ongoing to cancel it
        require(
            block.timestamp < auction.endTime,
            "Cannot cancel auction after it has ended"
        );

        // remove the auction id
        bnftAuctionIds.remove(auctionId);

        // transfer the bnft back to the seller
        BnftContract.safeTransferFrom(
            address(this),
            auction.seller,
            auction.tokenId
        );
        // if the currency is native token
        if (address(auction.currencyAddress) == address(0)) {
            // return native token amount back to highest bidder
            (bool success, ) = auction.highestBid.bidder.call{
                value: auction.highestBid.amount
            }("");
            require(success, "Failed to transfer back to auction creator");
        } else {
            // return erc20 tokens back to the current highest bidder
            IERC20Upgradeable(auction.currencyAddress).safeTransfer(
                auction.highestBid.bidder,
                auction.highestBid.amount
            );
        }

        emit BnftAuctionCancelled(auctionId, auction.tokenId, auction.seller);
    }
}
