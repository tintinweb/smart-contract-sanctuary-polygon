...
contract MGP {
    ...
    uint256[6] private levelTeam = [0, 0, 0, 50, 200, 500];
    uint256[6] private levelInvite = [0, 0, 0, 10000e18, 20000e18, 50000e18];
    ...
    address[] private globalManagers;
    ...
    constructor(address _fee, address _insurance, address _users, uint256 _startTime) {
    ...
    function _deposit(address _userAddr, uint256 _amount, uint256 _types) private {
        ...
        payable(insuranceFunds).transfer(_amount.mul(insurancePercents).div(baseDividend));
        if(globalManagers.length > 0) globalManagerPool = globalManagerPool.add(_amount.mul(globalManagerPoolPercents).div(baseDividend));
    ...
    function withdraw() external {
        ...
        payable(feeReceiver).transfer(incomeFee);
    ...
    function predict(uint256 _amount) external payable {
        ...
        require(msg.value >= predictFee, "Insufficient Funds");
    ...
    function getGlobalManagers() external view returns(address[] memory) {
