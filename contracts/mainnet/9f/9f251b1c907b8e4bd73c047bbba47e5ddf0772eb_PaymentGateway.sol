...
contract PaymentGateway is EIP712MetaTransaction,Ownable,ERC721,ERC721URIStorage   {
using Counters for Counters.Counter;
    ...
    Counters.Counter private accountCounter;
  ...
  IERC20 public ICE_token  = IERC20(0xc6C855AD634dCDAd23e64DA71Ba85b8C51E5aD7c);
    ...
    function registerPaperKey(address _paperKey) external onlyOwner {
     ...
     constructor() ERC721("PaymentInvoice", "PaymentInvoice") EIP712Base("PaymentGateway", "v1.0") {
    ...
    function registerDeveloper(string calldata name, address beneficiary) public {
        ...
        Account storage account = accounts[accountCounter.current()];
        account.accountId = accountCounter.current();
        account.name = name;
    ...
    function processPayment(uint256 orderId, uint256 accountId, uint256 amount,bool sendInvoice) public {
        ...
        require(isActive(accountId), "DEVELOPER_NOT_FOUND");
         ...
         require(ICE_token.transferFrom(sender,beneficiary,amount - saleFee),"TRANSFER_PAYMENT_FAILED" );
         require(ICE_token.transferFrom(sender, feeOwner, saleFee),"TRANSFER_FEES_FAILED");
        
        emit PaymentProcess(orderId,account.accountId,amount,account.beneficiary,sender);

        if(sendInvoice) {
            sendNFTInvoice(sender,amount);
        }
    }

    function paperPayment(uint256 orderId,uint256 accountId, uint256 amount, address payer, bytes32 _nonce, bytes calldata _signature) external  payable
            onlyPaper(keccak256(abi.encode(orderId,accountId,amount,payer)), _nonce, _signature)
     {

            Account storage account = accounts[accountId];
            require(isActive(accountId), "DEVELOPER_NOT_FOUND");
            swapUSDCToIce(USDC_token.balanceOf(address(this)),address(this));
            ICE_token.transferFrom(address(this),account.beneficiary,amount);
         
        sendNFTInvoice(payer,amount);
        emit PaymentProcess(orderId,account.accountId,amount,account.beneficiary,payer);
      ...
      function swapUSDCToIce(uint amount,address to) private
  {
    ...
    USDC_token.approve(address(router), amount*100000);
    ...
    function sendNFTInvoice(address to,uint256  amount) private  {
            uint256 tokenId;
             tokenId = _tokenIdCounter.current();
             _tokenIdCounter.increment();
             _safeMint(to, tokenId);
             _setTokenURI(tokenId, string.concat("This is a receipt of your payment of: ",Strings.toString(amount)," at ",Strings.toString(block.timestamp)));
