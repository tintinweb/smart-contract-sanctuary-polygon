...
contract Otherdeed is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string baseURI;
  string public baseExtension = "";
  uint256 public cost = 25000000 gwei;
  uint256 public maxSupply = 100000;
  //CheckNext4
  uint256 public maxPublicMint = 99500;
  uint256 public PublicMinted;
  uint256 public maxDevMint = 500;
  ...
  function mint(uint256 _mintQuantity) public payable {
    ...
    if (msg.sender == owner()){
        require((DevMinted + _mintQuantity) <= maxDevMint);
        DevMinted = DevMinted + _mintQuantity;
    }
    else{
        require((PublicMinted + _mintQuantity) <= maxPublicMint);
        PublicMinted = PublicMinted + _mintQuantity;
    }
