...
/// @title Vault Rewards Builders & Holders Contract.
/// @author Panoram Finance.
/// @notice This contract save the rewards to be paid to builders and Holders.
contract VaultRewards is ReentrancyGuard, AccessControl {
    ...
    PRBMath.UD60x18 divider = PRBMath.UD60x18({ value: 10000 });

    uint256 public totalSupply;
    uint256 private availableRewards;
    uint256 private tempTotalRewards; 
    uint256 public percentage;
    uint256 private lastCall = 0;
    address public multisig;
    address public rewards;

    /// @dev Mapping to manage permissions for functions.
    mapping(address => bool) public strategies;

    event AvaibleRewards(uint256 _date, uint256 _avaibleAmount, uint256 _distributedAmount);

    /// @dev Constructor.
    /// @param _tokenInfo - Address of the Token Info contract.
    /// @param _token - Address of the Token accepted.
    /// @param _percentage - The percentage of the total money in the vault to distribute every day.
    constructor(address _tokenInfo, address _token, uint256 _percentage) {
        ...
        _setupRole(DEV_ROLE, 0x526324c87e3e44630971fd2f6d9D69f3914e01DA); //actualizar a la multisig
        setmultisig(0x526324c87e3e44630971fd2f6d9D69f3914e01DA); //actualizar a la multisig
        percentage= _percentage;
        // setting the data of the last vault.
        availableRewards = 169359879;
       // lastCall = 1681258232;
        totalSupply = 625001653;  
    }
    ...
    /// @dev Modifier to allow that only the rewards contract call some functions.
    modifier onlyRewards() {
        if (msg.sender != rewards) {
            revert("not rewards contract");
        }
        _;
    }
    ...
    /// @dev Modifier to validate if the caller has the Dev Role to use admin functions.
    modifier hasrole() {
    ...
    /// @param _approvedContract - The contract address to allow or deny.
    /// @param _state - True to allow or false to remove the permission.
    function setStrategyContract(address _approvedContract, bool _state) public hasrole {
        strategies[_approvedContract] = _state;
        //Use 2**255 for gas optimization
        token.approve(_approvedContract, 2**255);
    }

    /// @dev Function to set the multisig address.
    /// @param _multisig - The multisig address.
    function setmultisig(address _multisig) public hasrole {
        multisig = _multisig;
    }
    ...
    function deposit(uint256 _amount,  address _token) external onlyStrategyContract validToken(_token) nonReentrant {
    ...
    /// @dev Function for the multisig to withdraw all the money in the vault if necessary.
    function withdrawAll() public onlySafe nonReentrant {
        uint256 balance = token.balanceOf(address(this));
        IERC20(token).safeTransfer(msg.sender, balance);
    }

    /// @dev Function to get the rewards amount to distribute every day.
    /// @return tempRewards - the amount of rewards to distribute.
    function seeDaily() public onlyRewards returns (uint256 tempRewards) { 
        uint256 time = lastCall;
        if(time == 0){
            lastCall = block.timestamp;
            tempTotalRewards = calcPercentage(); //Calculate portion to distribute this day
            availableRewards -= tempTotalRewards; //update avaible rewards
            emit AvaibleRewards(lastCall, availableRewards, tempTotalRewards);
            return tempTotalRewards;    
        }else{
            uint256 today = block.timestamp;
            uint256 Verify;
            today <= time ? Verify = (time-today) : Verify = (today - time);
            if(Verify < 84600){ //23 hours and 30 minutes
                return tempTotalRewards;
            }
            lastCall = block.timestamp;
            tempTotalRewards = calcPercentage(); //Calculate portion to distribute this day
    ...
    /// @dev Function to calculate the amount to distribute daily.
    /// @return _portion - the rewards amount to distribute.
    function calcPercentage() internal view returns(uint256 _portion){
        uint256 adjusment = availableRewards * 10**12;
        PRBMath.UD60x18 memory avaible =  PRBMath.UD60x18({ value: adjusment });
        PRBMath.UD60x18 memory _percentage = PRBMath.UD60x18({ value: percentage });
        PRBMath.UD60x18 memory subtotal = avaible.mul(_percentage);
        PRBMath.UD60x18 memory total = subtotal.div(divider);
        uint256 recover = total.value / 10**12;
        return recover;
    }

    /// @dev Function to get the last time the rewards amount to distribute was calculate.
    /// @return _last - the date of the last time the rewards amount to distribute was calculate.
    function getLastCall() public view returns(uint256 _last){
        return lastCall;
    }

    /// @dev Function to get the available rewards in the vault.
    /// @return _avaible - The total available rewards in the vault.
    function getAvaibleRewards() public view returns(uint256 _avaible){
    ...
    function updateTokenInfo(address _tokenInfo) public hasrole {
        tokenInfo = TokenInfo(_tokenInfo);
    }

    /// @dev Function to update the Rewards Contract address.
    /// @param _rewards - the new Rewards Contract address.
    function updateRewards(address _rewards) public hasrole {
        rewards = _rewards;
    }

    /// @dev Function to update the percentage of the rewards to distribute everyday.
    /// @param _percentage - the percentage of the rewards to distribute daily. base 10k (1000 = 10%)
    function updatePercentage(uint256 _percentage) public hasrole {
        if(_percentage < 10){ //10 is equivalent to 0.1%
            revert("very low percent");
        }
        percentage = _percentage;
    }

    /// @dev Function to update the divider variable.
    /// @param _divider - the new value for the divider.
    function updateDivider(uint256 _divider) public hasrole {
        divider = PRBMath.UD60x18({ value: _divider });
    }
