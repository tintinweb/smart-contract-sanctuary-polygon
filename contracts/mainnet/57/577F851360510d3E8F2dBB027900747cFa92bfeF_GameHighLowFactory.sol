...
contract GameHighLow is GameBase {
    ...
    function playHandler(uint256 _randomnes) override internal {
        ...
        uint256 pLen = _participantsAddressArray.length;
        uint256 pAmount = getPredictionsAmount();
        address[] memory newParicipantsAddressesArr = new address[](pAmount);
        uint j = 0;
        for(uint i = 0; i < pLen; i++) {
            address currentParticipantAddress = _participantsAddressArray[i];
            Participant storage currentParticipant = addressToParticipants[currentParticipantAddress];
            if(currentParticipant.prediction != Prediction.Empty) {
                if(newParicipantsAddressesArr.length != 0) {
                    newParicipantsAddressesArr[j] = currentParticipantAddress;
                } else {
                    newParicipantsAddressesArr[0] = currentParticipantAddress;
                }
                
                currentParticipant.prediction = Prediction.Empty;
                j += 1;
            } else {
                delete addressToParticipants[currentParticipantAddress];
                gameEngine.changeInGameStatus(address(0), currentParticipantAddress);
            }
        }

        // Auto kick AFK participants with empty predictions

        _participantsAddressArray = newParicipantsAddressesArr;
        higherPredictionAddresses = new address[](0);
        lowerPredictionsAddresses = new address[](0);
        gamePot = 0;
    ...
    function getPredictionsAmount() public view returns(uint256) {
