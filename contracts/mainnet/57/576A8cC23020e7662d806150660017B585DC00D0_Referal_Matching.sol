...
contract Referal_Matching is RestrictedFunctions {
   ...
   function join (address _ref,uint256 _package, uint8 _flag) external isLock {
      internalProcess(_ref, _package, _flag, false,_msgSender());
   }

   function internalProcess (address _ref,uint256 _package, uint8 _flag, bool inject, address _user) private {
   ...
   function injectUsers (address _user,address _ref,uint256 _package, uint8 _flag) external onlyOwner {
      internalProcess(_ref, _package, _flag, true,_user);
   }
   ...
   function viewUserDetails (address _user) external view returns(ROI memory) {
    ...
    function viewLeftSideRef(
        address _user
    ) external view returns (address[] memory) {
        return ROI_DATA[_user].left_side_ref;
    }

    function viewRightSideRef(
        address _user
    ) external view returns (address[] memory) {
        return ROI_DATA[_user].right_side_ref;
    }

   function viewPackages () external view returns (uint256[] memory) {
      return package;
   }
