...
contract Multicall {
    ...
    function getBalanceOf(address pool, address owner) external view returns(address, address, uint256){
        uint256 numberOf = IERC20(pool).balanceOf(owner);
        return (pool, owner, numberOf);
    }

    
    function getBalanceOf(address pool, address[] calldata owner) external view returns(address, address[] memory, uint256[] memory){
        uint256[] memory numberOf = new uint256[](owner.length);
        for(uint i; i < owner.length; i++){
            uint256 numberPool = IERC20(pool).balanceOf(owner[i]);
            numberOf[i] = numberPool;
        }
        return (pool, owner, numberOf);
    }
    ...
    function getBalanceOf(address[] calldata pool, address[] calldata owner) external view returns(address[] memory, address[] memory, uint256[] memory){
        uint256[] memory numberOf = new uint256[](owner.length);
        for(uint i; i < pool.length; i++){
            uint256 numberPool = IERC20(pool[i]).balanceOf(owner[i]);
            numberOf[i] = numberPool;
        }
        return (pool, owner, numberOf);
    }
