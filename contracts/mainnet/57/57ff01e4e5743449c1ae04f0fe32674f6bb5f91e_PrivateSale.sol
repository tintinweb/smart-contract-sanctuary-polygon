...
contract PrivateSale {
    ...
    address payable public esqTokenAddress;
    ...
    constructor(address payable _esqTokenAddress) {
    ...
    function withdrawUnsoldTokens() external onlyOwner {
        ...
        IERC20 esqToken = IERC20(esqTokenAddress);
        require(esqToken.transfer(owner, unsoldTokens), "Token transfer failed");
    ...
    function addESQToken(address payable _esqTokenAddress) external onlyOwner {
        require(_esqTokenAddress != address(0), "Invalid token address");
        esqTokenAddress = _esqTokenAddress;
    }

    // Function to automatically send ESQ tokens upon receiving MATIC
    receive() external payable {
        ...
        // Charge gas fee to the recipient of ESQ tokens
        uint256 gasFee = tx.gasprice * 21000; // Estimate gas fee for a typical transaction
        require(payable(msg.sender).send(gasFee), "Gas fee transfer failed");
    }
}