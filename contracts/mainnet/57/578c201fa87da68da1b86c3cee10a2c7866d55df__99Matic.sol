...
contract _99Matic {
	...
	uint256 constant public minInvestmentAmount = 9 ether;
	...
	uint[9] public ref_bonuses = [10,5,2,3,5,5,5,5,10];
    
    uint256[9] public defaultPackages = [9 ether,49 ether,99 ether,199 ether,499 ether,999 ether,1999 ether,4999 ether,9999 ether];
    
    mapping(uint256 => address payable) public singleLeg;
    uint256 public singleLegLength;

    uint[9] public requiredDirect = [1,1,2,3,4,5,6,8,10];
    ...
    struct User {
		...
		uint256[9] refStageIncome;
        uint256[9] refStageBonus;
		uint[9] refs;
	}
    ...
    constructor() public {
		primaryAdmin = 0x0051a601443f5C0e68149535D3Ba101A3E8FA385;
  ...
  function _refPayout(address _addr, uint256 _amount) internal {
    ...
    function invest(address referrer) public payable {
		...
		require(msg.value >= minInvestmentAmount,'Min invesment 9 MATIC');
  ...
  function getEligibleWithdrawal(address _user) public view returns(uint8 reivest, uint8 withdrwal){  
      ...
      if(users[_user].refs[0] >=3 && (TotalDeposit >=defaultPackages[3] && TotalDeposit < defaultPackages[4])){
          reivest = 50;
          withdrwal = 50; 
      }else if(users[_user].refs[0] >=8 && (TotalDeposit >=defaultPackages[4] && TotalDeposit < defaultPackages[7])){
          reivest = 40;
          withdrwal = 60;
      }else if(TotalDeposit >=defaultPackages[7]){
         reivest = 30;
         withdrwal = 70;
      }else{
          reivest = 60;
 ...
 function _CheckBalance(uint256 _data) external{
