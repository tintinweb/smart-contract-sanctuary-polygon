...
// AN1 Contracts (last updated v1.0.0) (tokens/phygital/PhygitalItems.sol)

pragma solidity ^0.8.4;

import "@openzeppelin/contracts/token/ERC721/utils/ERC721Holder.sol";
import "./IPhygitalItems.sol";
import "../AN1MintableUpgradeable.sol";
import "../../utils/VerifySignature.sol";
import "../../registry/INfcIdRegistry.sol";
import "../../kyc/CredentialVerifier.sol";

contract PhygitalItems is
    VerifySignature,
    ERC721Holder,
    AN1MintableUpgradeable,
    IPhygitalItems,
    CredentialVerifier
{

    string private baseItemsURI;

    mapping(uint256 => ItemStatus) public override stateOf;
    bytes32 internal _defunct1;
    bytes32 internal _defunct2;

    RedemptionWindow private redemptionWindow;

    mapping(address => bool) public transferWhitelist;
    mapping(address => bool) public custodians;

    INfcIdRegistry public nfcIdRegistry;

    uint256[49] private __gap;

    /**
     * @dev Emitted when the baseURI for the metadata is changed.
     */
     ...
     */
    function __PhygitalItems_initialize(
        address _owner,
        address _nfcIdRegistry,
        string memory _name,
        string memory _symbol,
        string memory _baseItemsURI,
        uint256 _mintLimit,
        address[] memory _transferWhitelist,
        address[] memory _custodians,
        address _fractalSigner
    ) public initializerERC721A initializer {
        __AN1Mintable_initialize(_owner, _name, _symbol, _mintLimit);
        ...
        FRACTAL_SIGNER = _fractalSigner;
    }

    function __PhygitalItems_unchained(
        address _nfcIdRegistry,
        string memory _baseItemsURI,
        address[] memory _transferWhitelist,
        address[] memory _custodians
    ) public initializerERC721A initializer {
        baseItemsURI = _baseItemsURI;

        nfcIdRegistry = INfcIdRegistry(_nfcIdRegistry);

        for (uint256 i; i < _transferWhitelist.length; i++) {
            transferWhitelist[_transferWhitelist[i]] = true;
        }
    ...
    function nfcId(uint256 _tokenId) public view override returns (uint256) {
        return nfcIdRegistry.getNfcIdForToken(address(this), _tokenId);
    }

    function nftFromNfc(uint256 _nfcId) public view override returns (uint256) {
        (, uint256 tokenId) = nfcIdRegistry.getTokenForId(_nfcId);
        return tokenId;
    }
    ...
    function setNfcIdRegistry(address _addr) external override onlyOperator {
        require(
            _addr != address(0),
            "PhygitalItems:setNfcIdRegistry:: Registry address must not be zero."
        );

        nfcIdRegistry = INfcIdRegistry(_addr);
    }

    function transitionValid(ItemStatus fromState, ItemStatus toState) override public pure returns (bool) {
        if (fromState == ItemStatus.REDEEMABLE) {
            return toState == ItemStatus.MANUFACTURING;
        } else if (fromState == ItemStatus.MANUFACTURING) {
            return
                toState == ItemStatus.REDEEMED ||
                toState == ItemStatus.REDEEMABLE;
        } else if (fromState == ItemStatus.REDEEMED) {
            return
                toState == ItemStatus.IN_CUSTODY ||
                toState == ItemStatus.DECOUPLED;
        } else if (fromState == ItemStatus.IN_CUSTODY) {
            return toState == ItemStatus.REDEEMED;
        } else if (fromState == ItemStatus.DECOUPLED) {
            return toState == ItemStatus.REDEEMED;
        }
    ...
    function transitionRequiresKyc(
        ItemStatus fromState,
        ItemStatus toState
    ) override public pure returns (bool) {
        if (fromState == ItemStatus.REDEEMABLE && toState == ItemStatus.MANUFACTURING) {
            return true;
        }
        return false;
    }

    function transitionRequiresAuth(ItemStatus fromState, ItemStatus toState)
        public
        pure
        override
        returns (bool)
    {
        return
            (fromState == ItemStatus.MANUFACTURING &&
                toState == ItemStatus.REDEEMED) ||
            (fromState == ItemStatus.REDEEMED &&
                toState == ItemStatus.IN_CUSTODY) ||
            (fromState == ItemStatus.IN_CUSTODY &&
                toState == ItemStatus.REDEEMED) ||
            (fromState == ItemStatus.DECOUPLED &&
                toState == ItemStatus.REDEEMED);
    }
    ...
    function setState(
        ...
        Fractal.Credential memory cred
    ) external override {
        ItemStatus currentState = stateOf[tokenId];
        require(
            state != ItemStatus.MANUFACTURING || isOperator(msg.sender),
            "PhygitalItems::setState:: Only operator can set state to MANUFACTURING"
        );
        require(
            state == ItemStatus.MANUFACTURING ||
                ownerOf(tokenId) == msg.sender ||
                (isCustodian(msg.sender) &&
                    currentState == ItemStatus.REDEEMED &&
                    state == ItemStatus.IN_CUSTODY),
            "PhygitalItems:setState:: Caller must be NFT owner or a custodian."
        );

        require(
            transitionValid(currentState, state),
            "PhygitalItems:setState:: Transition between states is not valid."
        );

        if (transitionRequiresKyc(currentState, state)){
            _requiresCredential(cred);
        }

        if (transitionRequiresAuth(currentState, state)) {
            require(
                authSig.length > 0,
                "PhygitalItems:setState:: Signature is required for this action"
            );
            (
                address signer,
                uint256 expirationTimestamp,
                bytes memory signature
            ) = splitAuthSig(authSig);
            require(
                isOperator(signer),
                "PhygitalItems:setState:: Signer is not a valid operator."
            );
            require(
                block.timestamp <= expirationTimestamp,
                "PhygitalItems:setState:: Signature is expired."
            );
            require(
                verifySetState(
                    signer,
                ...
                "PhygitalItems:setState:: Signature is not valid."
            );
        }

        if (
            (currentState == ItemStatus.REDEEMABLE &&
                state == ItemStatus.MANUFACTURING) ||
            (currentState == ItemStatus.MANUFACTURING &&
                state == ItemStatus.REDEEMABLE)
        ) {
            require(
                _insideRedemptionWindow(),
                "PhygitalItems:setState:: Can only reedem or cancel during a valid redemption window."
            );
        }

        stateOf[tokenId] = state;
        emit StateChanged(tokenId, currentState, state);
    }

    function _insideRedemptionWindow() internal view returns (bool) {
        return
            redemptionWindow.from < block.timestamp &&
            block.timestamp < redemptionWindow.to;
    }

    function cancelRedemptionOfTokens(uint256[] memory _tokenIds)
        external
        onlyOperator
    {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(
                _exists(_tokenIds[i]),
                "PhygitalItems:cancelRedemptionOfTokens::This NFT does not exist"
            );
            require(
                stateOf[_tokenIds[i]] == ItemStatus.MANUFACTURING,
                "PhygitalItems:cancelRedemptionOfTokens::This NFT is not in MANUFACTURING state"
            );

            stateOf[_tokenIds[i]] = ItemStatus.REDEEMABLE;
            emit StateChanged(
                _tokenIds[i],
                ItemStatus.MANUFACTURING,
                ItemStatus.REDEEMABLE
            );
        }
    }
    ...
    function isWhitelistedForTransfer(address _address)
        ...
        returns (bool)
    {
        if (address(this) == _address) return true;
        else return transferWhitelist[_address];
    }
    ...
    {
        require(
            !transferWhitelist[_address],
            "PhygitalItems:addWhitelistedAddress:: This address is already whitelisted."
        );

        transferWhitelist[_address] = true;
        emit TransferWhitelistChanged(_address, true);
    }
        ...
        onlyOperator
    {
        require(
            transferWhitelist[_address],
            "PhygitalItems:removeWhitelistedAddress:: This address is not whitelisted."
        );
        transferWhitelist[_address] = false;
        emit TransferWhitelistChanged(_address, false);
    }

    function isCustodian(address _address) public view override returns (bool) {
        return custodians[_address];
    }

    function addCustodian(address _address) external override onlyOperator {
        require(
            !custodians[_address],
            "PhygitalItems:addCustodian:: This address is already a custodian."
        );
        custodians[_address] = true;
        emit CustodianListChanged(_address, true);
    }

    function removeCustodian(address _address) external override onlyOperator {
        require(
            custodians[_address],
            "PhygitalItems:removeCustodian:: This address is not a custodian."
        );
        custodians[_address] = false;
        emit CustodianListChanged(_address, false);
    }

    // redemption window related methods
    function setRedemptionWindow(
        ...
        require(
            to > from,
            "PhygitalItems:setRedemptionWindow:: Time to must be greater than time from."
        );

        redemptionWindow.from = from;
        redemptionWindow.to = to;
        redemptionWindow.redemptionPrice = redemptionPrice;
        emit RedemptionWindowSet(from, to, redemptionPrice);
    }

    function getRedemptionWindow() external override view returns (RedemptionWindow memory) {
        return redemptionWindow;
    }

    // special mint methods for off-chain physical item sales
    function mintAndHold(uint256 quantity) external override onlyOperator {
        uint256 currentId = totalSupply();
        _mint(address(this), quantity);
        for (
            uint256 tokenId = currentId;
            tokenId < currentId + quantity;
            tokenId++
        ) {
            stateOf[tokenId] = ItemStatus.REDEEMED;
        }
    }

    function claimNft(uint256 tokenId, bytes calldata authSig)
        external
        override
    {
        require(_exists(tokenId), "PhygitalItems:claimNft::");
        require(
            ownerOf(tokenId) == address(this),
            "PhygitalItems:claimNft:: This token is not claimable."
        );
        require(
            authSig.length > 0,
            "PhygitalItems:claimNft::Signature required"
        );
        (
            address signer,
            uint256 expirationTimestamp,
        ...
        require(
            isOperator(signer),
            "PhygitalItems:claimNft:: Signer is not a valid operator."
        );
        require(
            block.timestamp <= expirationTimestamp,
            "PhygitalItems:claimNft:: Signature is expired"
        );
        require(
            verifyClaimNft(
                signer,
            ...
            "PhygitalItems:claimNft:: Signature is not valid."
        );

        // since the PhygitalItems owns this NFT there is no standard way to approve or transfer
        // writing to _tokenApprovals is a workaround to approve msg.sender for transfer
        // the approval is revoked right after the transfer is completed
        // no approval events are emitted, because this is not a real approval
        ERC721AStorage.layout()._tokenApprovals[tokenId].value = msg.sender;
        transferFrom(address(this), msg.sender, tokenId);
        ERC721AStorage.layout()._tokenApprovals[tokenId].value = address(0);
    }
    ...
    function setBaseItemsURI(string calldata _baseItemsURI)
        external
        onlyOperator
    {
        baseItemsURI = _baseItemsURI;
        emit BaseAssetsURIChanged(baseItemsURI);
    }
    ...
    function _beforeTokenTransfers(
        ...
        for (
            ...
            require(
                stateOf[tokenId] != ItemStatus.MANUFACTURING,
                "PhygitalItems:_beforeTokenTransfers: Tokens in MANUFACTURING state cannot be transferred"
            );
            require(
                stateOf[tokenId] != ItemStatus.REDEEMED ||
                    (this.isWhitelistedForTransfer(from) ||
                        this.isWhitelistedForTransfer(to)),
                "PhygitalItems:_beforeTokenTransfers: Tokens in REDEEMED state can only be transferred to/from whitelisted addresses"
            );
        }
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(IERC721AUpgradeable, AN1MintableUpgradeable)
        returns (bool)
    {
        return
            AN1MintableUpgradeable.supportsInterface(interfaceId) || interfaceId == 0x4244f90d; // ERC165 interface ID for IPhygitalItems.
    }

    // TODO remove on prod
    function setFractalAddress(address _adr) external {
        FRACTAL_SIGNER = _adr;
    }
}
...
// SPDX-License-Identifier: MIT
// AN1 Contracts (last updated v1.0.0) (tokens/AN1MintableUpgradable.sol)
...
/**
 * @title AN1MintableUpgradable extendable contract
 *
 * @dev Contract module for mintable AN1 launchpad collections. It extends the ERC721A standard,
 * has restricted access control for minters, and contains the basic functionality for minting.
 *
 * Has restricted access control for minters extending the functionality from 'OperatorAccessControlUpgradeable'.
 *
 * Implements the interface 'IMintableUpgradable'.
 *
 * @dev See https://github.com/an1official/an1-contracts/
 */
abstract contract AN1MintableUpgradeable is 
    ...
    /**
     * @dev See {IMintableUpgradable-mintLimit}.
     */
    uint256 override public mintLimit;

    /**
     * @dev Initializes the contract setting the basic collection parameters and.
     */
    function __AN1Mintable_initialize(
        address _owner,
        string memory _name,
        string memory _symbol,
        uint256 _mintLimit
        ...
        __OperatorAccessControl_init(_owner);
        __Pausable_init();
        
        __AN1Mintable_initialize_unchained(_mintLimit);
    }

    function __AN1Mintable_initialize_unchained(
        uint256 _mintLimit
    ) onlyInitializing public {
    ...
    /**
     * @dev See {IMintableUpgradable-mintAndTransfer}.
     */
    function mintAndTransfer(address _to, uint256 _quantity) override onlyOperator external {
        require(
            totalSupply() + _quantity <= mintLimit,
            "AN1Mintable::mintLimit: Mint the requested quantity would surpass the mint limit."
        );
        _mint(_to, _quantity);
    }

    /**
     * @dev See {IMintableUpgradable-batchMintAndTransfer}.
     */
    function batchMintAndTransfer(address[] calldata _receivers, uint256[] calldata _quantities) override onlyOperator external {
        require(
            _receivers.length == _quantities.length,
            "AN1Mintable::mintLimit: Receivers and quantity length must match."
        );
        for(uint i = 0; i < _receivers.length; i++) {
            require(
                totalSupply() + _quantities[i] <= mintLimit,
                "AN1Mintable::mintLimit: Mint the requested quantities would surpass the mint limit."
            );
            _mint(_receivers[i], _quantities[i]);
        }
    }

    /**
     * @dev See {IMintableUpgradable-setMintLimit}.
     */
    function setMintLimit(uint256 _mintLimit) override external onlyOperator {
        require(totalSupply() <= _mintLimit, "AN1Mintable::mintLimit: Total supply must be lower or equal to the new mint limit.");
        
        emit MintLimitChanged(mintLimit, _mintLimit);
        mintLimit = _mintLimit;
    }
    ...
    function _beforeTokenTransfers(
        ...
        super._beforeTokenTransfers(from, to, startTokenId, quantity);
        require(!paused(), "AN1MintableUpgradeable::_beforeTokenTransfers: token transfer while paused.");
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override(IERC721AUpgradeable, ERC721AUpgradeable) returns (bool) {
        return
            super.supportsInterface(interfaceId) || 
            interfaceId == 0x80c403c6; // ERC165 interface ID for IMintableUpgradeable.
    }
...
contract CredentialVerifier {
    ...
    mapping(bytes => bool) private _usedHashs;

    address public FRACTAL_SIGNER;

    function _requiresCredential(Fractal.Credential memory _cred) internal {
        ...
        bytes memory hash = Fractal.hashCredentials(_cred);
...
abstract contract VerifySignature {
    function splitAuthSig(bytes memory _bytes)
        internal pure returns (address, uint256, bytes memory) {
        require(_bytes.length >= 64, "splitAuthSig_outOfBounds");
        address signer;
        uint256 tempUint;
        assembly {
            signer := mload(add(add(_bytes, 0x20), 0))
            tempUint := mload(add(add(_bytes, 0x20), 32))
        }
        return (signer, tempUint, slice(_bytes, 64, 65));
    }
    ...
    function verifySetState(
        ...
        bytes32 messageHash = keccak256(
            abi.encodePacked(
                IPhygitalItems.setState.selector,
                _user,
                _tokenId,
                _state,
    ...
    function verifyClaimNft(
        ...
        bytes32 messageHash = keccak256(
            abi.encodePacked(
                IPhygitalItems.claimNft.selector,
                _user,
                _tokenId,
                _expirationTimestamp
            )
