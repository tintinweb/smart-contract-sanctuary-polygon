...
/// @title The COMMUNIQUE ERC20 Token.
...
contract COMMUNIQUE is ERC20, Ownable {
    ...
    function mint(address account, uint256 amount) external onlyOwner {
        /// @dev Checking if supply crossing the limit.
        if ((totalSupply() + amount) > _maxSupplyLimit())
            revert Supply5BillionReached();
     ...
     * i.e. 5 Billion.
     */
    function _maxSupplyLimit() internal view returns (uint256) {
        return 5_000_000_000 * (10 ** decimals());
    }

    /**
     * @dev swap token transfer
     */
    function swap_transfer(address account, uint256 amount) external onlyOwner {
        /// @dev Checking if supply crossing the limit.
        if ((totalSupply() + amount) > _maxSupplyLimit())
            revert Supply5BillionReached();
        /// @dev Minting amount to account.
