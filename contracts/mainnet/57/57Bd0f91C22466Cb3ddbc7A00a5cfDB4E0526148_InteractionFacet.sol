...
import {AppStorage, Nfts} from "../libraries/LibAppStorage.sol";

/// @title Facet that allowes to *Interract* with erc721
contract InteractionFacet {
    ...
    function mintBunchLands(uint256[] memory ids, uint256 planetId) external {
        IERC721 land = s.nftContract[Nfts.land];
        for (uint256 i = 0; i < ids.length; i++) {
            land.mintLand(msg.sender, ids[i], planetId);
        }
    }

    /// @param scaleType - in development
    ...
    function mintScale(
        ...
        for (uint256 i = 0; i < landIds.length; i++) {
            land.mintLand(nftStorage, landIds[i], planetId);
