...
contract Ftech {
    ...
    IERC20 private token2 = IERC20(0xc2132D05D31c914a87C6611C10748AEb04B58e8F);
    ...
    uint256[6] private Newbies = [0, 4, 2, 2, 2, 5];
    ...
    uint256[2] private requiredBusiness = [5000e6, 15000e6];
    uint256[2] private requiredLeaders = [0, 2];
    uint256[2] private requiredLevel = [5, 6];
    uint256 private constant maxFromOneLeg = 40;
    mapping(address => address[]) public directTeam;
    address[] public founders;
    uint256 public totalFounders;
    uint256 public startTime;
    uint256 public FCLastDist;
    uint256 private FCDistTime = 7 days;
    address private regi;
    ...
    constructor(address _regi, address _manage, address _view, address _pool, address _teamManagement) {
        regi = _regi;
        management = _manage;
    ...
    function donate() external {
        ...
        usdt.transferFrom(msg.sender, address(this), Level[level-1]);
    ...
    function _distribute(address _user, uint256 _level) private {
        ...
        for(uint256 i=0; i<8; i++) {
            ...
            if(upline != address(0) && upline != address(this)) {
                if(i < 7) userInfo[upline].totalBusiness += Level[_level - 1];
    ...
    function withdrawPfi(uint256 _level) external {
        require(userInfo[msg.sender].level > _level || (userInfo[msg.sender].level == 6 && userInfo[msg.sender].newbies >= 5), "Upgrade Your level to Recevie reward");
        ...
        usdt.transfer(teamManagement, charge);
        if(isTokenChange && token2.balanceOf(address(this)) > (income * 80)/100) {
            token2.transfer(msg.sender, (income * 80)/100);        
        } else {
            usdt.transfer(msg.sender, (income * 80)/100);
        }
    }

    function withdraw(address _user) external {
        Reward storage curReward = rewardInfo[_user];
        ...
        userInfo[_user].revenue += totalReward;
        uint256 charge = (totalReward * 5)/100;
        distributeHelping(_user, (totalReward * 15)/100);
        ...
        if(msg.sender == regi) {
            if(left > upgrading[5]) {
                usdt.transfer(management, upgrading[5]);
                left -= upgrading[5];
            } else {
                usdt.transfer(management, left);
                left = 0;
            }
        }

        if(msg.sender == pool) usdt.transfer(pool, (upgrading[1] * 80)/100);

        if(isTokenChange && token2.balanceOf(address(this)) > (totalReward2 * 80)/100) {
            totalReward = totalReward - totalReward2;
            token2.transfer(_user, (totalReward2 * 80)/100);        
        }

        usdt.transfer(_user, (totalReward * 80)/100);
        usdt.transfer(teamManagement, charge); 
    }
        ...
        if(userInfo[msg.sender].leadership < 2) { 
            if(totalBusiness >= requiredBusiness[userInfo[msg.sender].leadership] && totalLeaders[0] >= requiredLeaders[userInfo[msg.sender].leadership] && userInfo[msg.sender].level >= requiredLevel[userInfo[msg.sender].leadership]) {
                ...
                for(uint256 i=0; i<7; i++) {
                    if(upline != address(0) && upline != address(this)) {
                        userInfo[upline].teamLeaders[userInfo[msg.sender].leadership - 1] += 1; 
                        upline = userInfo[upline].referrer;
                    } else {
                        break;
                    }             
    ...
    function reg(address _user, address _referrer ,uint256 _level) private {
        ...
        for(uint256 i=0; i<_level; i++) {
            ...
            if(users[i].length >= 2) {
                ...
                userInfo[_user].PFIUpline[i] = _AP;
    ...
    function changeCurrency(address _addr, uint256 _decimal) external onlyViewer {
        if(_decimal == 1) {
            token2 = usdt;
            usdt = IERC20(_addr);
            isTokenChange = true;
