...
//TODO:::Allowlist only once or just ocmbined amount?
//TODO:::Need contract uri? (i think no)

contract Doggerz is ERC721AUpgradeable, OwnableUpgradeable
{
    string private _baseTokenURI;

    struct SaleConfig {
        bool allowListMintStarted;
    ...
    {
        __ERC721A_init("Doggerz", "DGH");
        _transferOwnership(_msgSender());
        

        saleConfig.maxPerAddress = 3;
        saleConfig.collectionSize = 2000;
        saleConfig.allowListMintStarted = false;
        saleConfig.publicMintStarted = false;
        saleConfig.allowListMintPrice = 0.15 ether;
        saleConfig.publicMintPrice = 0.3 ether;
    }

    //=============================CONFIG=============================

    function setAllowListMintStarted(bool state) external onlyOwner
    {
        saleConfig.allowListMintStarted = state;
    }

    function setPublicMintStarted(bool state) external onlyOwner
    {
        saleConfig.publicMintStarted = state;
    }

    function setAllowListMintPrice(uint64 price) external onlyOwner
    {
        saleConfig.allowListMintPrice = price;
    }

    function setPublicMintPrice(uint64 price) external onlyOwner
    {
        saleConfig.publicMintPrice = price;
    }
    ...
    function devMint(uint256 quantity) external onlyOwner
    {
        ...
        require( totalSupply() + quantity <=  saleConfig.collectionSize,
    ...
    {
        require(saleConfig.allowListMintStarted, "allowlist sale has not started yet");
        require(allowlist[msg.sender] > 0, "not eligible for allowlist mint");

        require(
            numberMinted(msg.sender) + quantity <=  saleConfig.maxPerAddress,
            "can not mint this many"
        );

        allowlist[msg.sender] -= quantity;

        _mint(quantity, saleConfig.allowListMintPrice);
    }

    function publicMint(uint256 quantity) external payable 
    {
        require(saleConfig.publicMintStarted, "public sale has not started yet");
        require(
            numberMinted(msg.sender) + quantity <=  saleConfig.maxPerAddress,
            "can not mint this many"
        );

        _mint(quantity, saleConfig.publicMintPrice);
    }

    function _mint(uint256 quantity, uint64 price) internal
    {
