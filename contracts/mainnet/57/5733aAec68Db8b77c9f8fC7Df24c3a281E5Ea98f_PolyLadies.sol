...
contract PolyLadies is Ownable, ERC721Enumerable, ReentrancyGuard, ERC2981 {
    string private baseTokenURI;

    // Minting
    uint256 public immutable maxSupply = 3685;
    bool public isPaused;
    uint256 public maxMint = 20;

    uint256 public publicTimestamp = 1677704400;
    uint256 public claimTimestamp = 1677703500;

    uint256 public price = 15 ether;
    uint256 public whitelistPrice = 12 ether;

    mapping(address => uint256) private freelist;
    mapping(address => bool) private whitelist;

    mapping(uint256 => uint256) private tokensMatrix;

    // Royalties
    uint256 public mintRoyalties = 5;
    ...
    uint256 private immutable dooRoyalties = 10;
    address public immutable dooWallet;
    ...
    constructor(
        ...
        address dooWallet_
    ) ERC721("Poly Ladies", "POLYLADIES") {
        baseTokenURI = uri_;
        projectWallet = projectWallet_;
        dooWallet = dooWallet_;
    ...
    function addFreeClaim(
        ...
        for (uint256 i = 0; i < addresses_.length; ) {
            freelist[addresses_[i]] = amount_[i];
            unchecked {
                ++i;
            }
        }
    ...
    function airdrop(uint256 amount, address to) external onlyOwner {
        ...
        for (uint256 i = 1; i <= amount; ) {
            uint256 tokenId = randomTokenId(0);
            _safeMint(to, tokenId);
    ...
    function _payoutLaunchpadFee(uint256 _amount) private {
        (bool success, ) = payable(dooWallet).call{value: _amount, gas: 50000}(
            ""
        );
        require(success);
    }
    ...
    function freeMint(uint256 amount) external nonReentrant {
        ...
        if (block.timestamp < claimTimestamp) revert MintNotOpen();
    ...
    function mint(uint256 amount) external payable nonReentrant {
        ...
        if (isPaused) revert PausedMint();
        if (block.timestamp < publicTimestamp) revert MintNotOpen();
        ...
        uint256 fee = (amount * dooRoyalties) / 100;
        uint256 payout = totalPrice - royalties - fee;
        _payout(payout);
    ...
    function randomTokenId(uint256 startIndex) internal returns (uint256) {
        ...
        ) % (maxIndex - startIndex)) + startIndex;
    ...
    function _addRewards(uint256 amount) private {
        ...
        currentRoyalties[currentRound] += amount / totalSupply();
    }
    ...
    function claimAllRoyalties() external {
        ...
        totalClaimed[_msgSender()] += rewards;
