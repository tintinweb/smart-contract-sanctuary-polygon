 ...
 *         Eternity Deck game
 */
contract Dao is  Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  /**
   ...
   */
  uint64[] public shareholderOwnership;
  ...
  event ShareholdersChanged(
    ...
    uint64[] oldShareholderOwnership,
    address[] newShareholders,
    uint64[] newShareholderOwnership
  );
   ...
   */
  event Payout(address[] shareholders, uint64[] shareholderOwnership, uint256 value);
 ...
 /**
   ...
   */
  function initialize (address[] calldata _shareholders, uint64[] calldata _shareholderOwnership) public initializer{
...
/**
   ...
   */
  function changeShareholders(address[] memory _shareholders, uint64[] memory _shareholderOwnership) public onlyOwner {
    ...
    for(uint8 i = 0; i < _shareholderOwnership.length;) {
  ...
  function distributeToShareholders() public payable{
    ...
    for(uint8 i = 0; i < shareholders.length;) {
      shareholderBalance[shareholders[i]] += value * shareholderOwnership[i] / 100_000;
      unchecked {
          ++i;
      }
    }
