...
contract whitelistCheck is EIP712{
    ...
    string private constant SIGNING_DOMAIN = "DDSMAI";
...
contract ddsmproverbsv1 is ERC721Enumerable, Ownable , whitelistCheck{
    using Strings for uint;
    using ECDSA for bytes32;

    uint public ddsmproverbsv1_PRESALE = 1; //presale
    uint public ddsmproverbsv1_WALLET_PRESALE_LIMIT = 10; //ddsmproverbsv1perwalletInWholeDuration
    uint public ddsmproverbsv1_WALLET_PUBLICSALE_LIMIT = 20; //ddsmproverbsv1perwalletInWholeDuration
    uint public ddsmproverbsv1_MAX_COUNT = 144; //maxSupply
    uint public totalTokensSoldInPresale;
    uint public preSaleAmountMinted;

    uint public ddsmproverbsv1_MAINSALE_PRICE = 0.5 ether; //priceInMainSale
    uint public ddsmproverbsv1_PRESALE_PRICE = 0.3 ether; //priceInPreSale
    ...
    string private _tokenBaseURI = "https://gateway.pinata.cloud/ipfs/QmRar3fucuyAgwjLUL5ZYsicuRiTa4RpqH2DijSNWx2TGF/";
    string private constant Sig_WORD = "BlueSky561!";
    address private _signerAddress = 0x6Af1c32292752172D9c7F81e73a6DC56c1cf4C0f;
    ...
    constructor() ERC721("DDSM Proverbs", "ddsmproverbs") {}

    function setddsmproverbsv1_MAX_COUNT(uint _maxCount) external onlyOwner{
        ddsmproverbsv1_MAX_COUNT = _maxCount;
    }

    function setddsmproverbsv1_PRESALE(uint _presale) external onlyOwner{
        ddsmproverbsv1_PRESALE = _presale;
    }

    function setddsmproverbsv1_WALLET_PRESALE_LIMIT(uint _presaleLimit) external onlyOwner{
        ddsmproverbsv1_WALLET_PRESALE_LIMIT = _presaleLimit;
    }

    function setddsmproverbsv1_WALLET_PUBLICSALE_LIMIT(uint _publicSaleLimit) external onlyOwner{
        ddsmproverbsv1_WALLET_PUBLICSALE_LIMIT = _publicSaleLimit;
    }

    function setddsmproverbsv1_MAINSALE_PRICE(uint _mainSalePrice) external onlyOwner{
        ddsmproverbsv1_MAINSALE_PRICE = _mainSalePrice;
    }

    function setddsmproverbsv1_PRESALE_PRICE(uint _presalePrice) external onlyOwner{
        ddsmproverbsv1_PRESALE_PRICE = _presalePrice;
    }
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= ddsmproverbsv1_MAX_COUNT, "EXCEED_MAX");
    ...
    function buy(uint tokenQuantity) external payable {
        ...
        require (tokenQuantity <= ddsmproverbsv1_WALLET_PUBLICSALE_LIMIT);
        require (saleListPurchase[msg.sender] + tokenQuantity <= ddsmproverbsv1_WALLET_PUBLICSALE_LIMIT, "MAX MINT COMPLETE");
        require (totalSupply() + tokenQuantity <= (ddsmproverbsv1_MAX_COUNT - preSaleAmountMinted), "EXCEED_MAX");
        require (ddsmproverbsv1_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
    ...
    function founderMint(uint tokenQuantity) external onlyOwner {
        require(totalSupply() + tokenQuantity <= ddsmproverbsv1_MAX_COUNT, "EXCEED_MAX");
        for(uint i = 0; i < tokenQuantity; i++) {
        _safeMint(msg.sender, totalSupply() + 1);
        }
    }
    ...
    function presaleBuy( Whitelist memory whitelist, uint tokenQuantity) external payable{
        require(presaleLive, "PRESALE_CLOSED");//ddsmproverbsv1_WALLET_PRESALE_LIMIT
        require(getWhitelist(whitelist), "DIRECT_MINT_DISALLOWED");
        require(whitelist.userAddress == msg.sender);
        require(preSaleAmountMinted + tokenQuantity <= ddsmproverbsv1_PRESALE, "EXCEED_PRIVATE");
        require(presalerListPurchases[msg.sender] + tokenQuantity <= ddsmproverbsv1_WALLET_PRESALE_LIMIT, "EXCEED_ALLOC");
        require(ddsmproverbsv1_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
