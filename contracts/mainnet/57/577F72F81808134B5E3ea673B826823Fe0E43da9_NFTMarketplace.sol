...
contract NFTMarketplace is ERC721URIStorage {
    ...
    mapping(string => uint256) private tokenURIRarity;
    ...
    constructor() ERC721("UnikFame", "UFE") {
    ...
    function createToken(string memory tokenURI, uint256 price, uint flag, uint rarity, uint num, address royalty) public returns (uint) {
        require(msg.sender == owner, "Only Owner Can Mint");        
        if(tokenURINums[tokenURI] == 0)
        {
          tokenURIRarity[tokenURI] = rarity;          
        }
        require(tokenURIRarity[tokenURI] == rarity);
        uint256 actualcount = (uniknums[rarity] - tokenURINums[tokenURI]) <= num ? (uniknums[rarity] - tokenURINums[tokenURI]) : num;
        uint256 newTokenId;
        for(uint i = 0; i < actualcount; i++)
        {
          ...
          createMarketItem(newTokenId, price, flag, royalty);
        }      
    ...
    function createAuctionSale(
      uint256 [] calldata tokenIds
        ...
        for(uint i =0; i < tokenIds.length; i ++)
        {
          ...
          require(pricefrom > price, "Please bid the asking price in order to complete the Bid");
    ...
    function fetchMarketItems() public view returns (MarketItem[] memory) {
      ...
      MarketItem[] memory items = new MarketItem[](_itemsMarket.current());
      for (uint i = 0; i < itemCount; i++) {
        if (idToMarketItem[i + 1].owner == address(this) && idToMarketItem[i + 1].isMarket) {
          uint currentId = i + 1;
    ...
    function fetchAuctionItems() public view returns (MarketItem[] memory) {
      ...
      MarketItem[] memory items = new MarketItem[](_itemsAuction.current());
      for (uint i = 0; i < itemCount; i++) {
        if (idToMarketItem[i + 1].owner == address(this) && idToMarketItem[i + 1].isAuction) {
          uint currentId = i + 1;
          MarketItem storage currentItem = idToMarketItem[currentId];
          items[currentIndex] = currentItem;
          currentIndex += 1;
    ...
    /* Returns only items that a user has purchased */
    function fetchMyMarketNFTs() public view returns (MarketItem[] memory) {
      uint totalItemCount = _tokenIds.current();
      uint itemCount = 0;
      uint currentIndex = 0;

      for (uint i = 0; i < totalItemCount; i++) {
        if (idToMarketItem[i + 1].seller == msg.sender && idToMarketItem[i + 1].isMarket) {
          itemCount += 1;
        }
      }
    ...
    function fetchMyAuctionNFTs() public view returns (MarketItem[] memory) {
      uint totalItemCount = _tokenIds.current();
      uint itemCount = 0;
      uint currentIndex = 0;

      for (uint i = 0; i < totalItemCount; i++) {
        if (idToMarketItem[i + 1].seller == msg.sender && idToMarketItem[i + 1].isAuction) {
          itemCount += 1;
        }
      }
