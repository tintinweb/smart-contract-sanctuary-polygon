...
contract SignalSuscriptionModule is ModuleBase, Ownable, ReentrancyGuard {
    ...
    mapping(address => bool) public isFollowMethod;

    uint256 public warningLine;
    uint256 public unsubscribeLine;

    mapping(IJasperVault => address) public strategists;
    mapping(IJasperVault => uint256) public strategistPercentage;
    ...
    function setFollowMethod(address _jasperVault, bool _status) external {
        if (_status) {
            require(
                IJasperVault(_jasperVault).moduleStates(msg.sender) ==
                    IJasperVault.ModuleState.INITIALIZED,
                "Only the module can call"
            );
        }
        require(
            isFollowMethod[_jasperVault] == _status,
            "follow method status has been set"
        );
        isFollowMethod[_jasperVault] = _status;
    }
    ...
    function unsubscribe(
        ...
        isFollowing[address(_jasperVault)] = false;
    ...
    //settle
    function settle(IJasperVault _jasperVault) public {}
}