...
/// @title GET Protocol Event Financing - PalletLocker
/// @author KasperK

contract PalletLocker is IPalletLocker {
    using SafeERC20 for IERC20;
    address public immutable safeAddress;
    address public immutable chamberOfCommerce;
    address public immutable palletLockerFactory;
    ...
    constructor(
        ...
        palletLockerFactory = msg.sender;
    }

    modifier isPalletRegistry() {
        require(
            IChamberOfCommerce(chamberOfCommerce).palletRegistry() == msg.sender,
            "PalletLocker:Caller not PalletRegistry"
        );
        _;
    }
    ...
    modifier isDAOController() {
        require(
            ...
            "PalletLocker:Caller not a DAO controller"
        );
        _;
    }

    /**
     * function withdraws amounts to the palletRegistry.
     * @dev SECURITY NOTE: If an attacker is able to change palletRegistry in COC, it can drain all the lockers!!
     */
    function pullAmountToPalletRegistry(
        address _addressDepositTokens,
        uint256 _amountStakeAmount
    ) external isPalletRegistry {
        address palletRegistry_ = IChamberOfCommerce(chamberOfCommerce).palletRegistry();
        IERC20(_addressDepositTokens).safeTransfer(
            palletRegistry_,
            _amountStakeAmount
        );
        emit PullToFactory(_amountStakeAmount);
    }
     ...
     * @param _amountToWithdraw amount of tokens to withdraw to the registry contract
    ...
    fallback() external {
        revert("PalletLocker:Revert:No native assets allowed");
    }
}
