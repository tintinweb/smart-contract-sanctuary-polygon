...
interface IPLGReferralReward {
    function members(address account,uint256 index) external view returns (uint256);
}
...
contract PLGReader is permission {
    ...
    function viewData(address account) public view returns (uint256[] memory,uint256) {
        uint256[] memory result = new uint256[](31);
        uint256 sum;
        for(uint256 i=0; i<30; i++){
            result[i] = IPLGReferralReward(data).members(account,i);
            sum += result[i];
        }
        return (result,sum);
    }
