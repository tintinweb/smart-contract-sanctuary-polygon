...
contract EthermonTrade is EthermonEnum, BasicAccessControl, SafeMathEthermon {
   ...
   function withdrawWeth(address _sendTo, uint256 _amount) public onlyModerators {
    ...
    function buyItemMatic(uint64 _objId) external payable isActive {
        ...
        monsterNFT.triggerTransferEvent(trainerAdress, msg.sender, _objId);

        // transfer money
        trainerAdress.transfer(safeSubtract(requestPrice, fee));

        EventCompleteSellOrder(trainerAdress, msg.sender, _objId, requestPrice);
    }
        ...
        )
    {
        ...
        if (objId > 0) {
            (trainer, classId, exp, createIndex, bp) = getObjInfoWithBp(objId);
        }
    }
