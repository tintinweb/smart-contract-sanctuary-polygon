...
contract StrategySushiSwap is BaseStrategyLP {
    ...
    address[] public wmaticToMushPath;
    ...
    constructor(
        ...
        address[] memory _earnedToMushPath,
        address[] memory _wmaticToUsdcPath,
        address[] memory _wmaticToMushPath,
        ...
        earnedToMushPath = _earnedToMushPath;
        wmaticToUsdcPath = _wmaticToUsdcPath;
        wmaticToMushPath = _wmaticToMushPath;
        ...
        returns (uint256)
    {
        if (rewardRate > 0) {
            ...
            uint256 usdcAfter = IERC20(usdcAddress)
                ...
                .sub(usdcBefore);
            bool success = IStrategyMush(rewardAddress).depositReward(
                usdcAfter
            );
...
abstract contract BaseStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    address public constant rewardAddress =
        0x6D0019D985d16caDCb60b760B8Ad60207973725D;
    address public constant withdrawFeeAddress =
        0x85471BB38f5a244646763d76fFB597cbe706245f;
    ...
    address[] public earnedToMushPath;
    ...
    function distributeRewards(uint256 _earnedAmt) internal returns (uint256) {
        if (rewardRate > 0) {
            ...
            _safeSwap(fee, earnedToUsdcPath, address(this));
    ...
    function buyBack(uint256 _earnedAmt) internal virtual returns (uint256) {
        if (buyBackRate > 0) {
            ...
            _safeSwap(buyBackAmt, earnedToMushPath, buyBackAddress);
