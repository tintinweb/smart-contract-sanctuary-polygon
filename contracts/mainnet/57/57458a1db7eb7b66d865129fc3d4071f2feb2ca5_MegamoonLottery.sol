...
/** @title Megamoon Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract MegamoonLottery is ReentrancyGuard, IMegamoonLottery, Ownable {
    ...
    uint256 public maxPriceTicketInMoon = 50 ether;
    uint256 public minPriceTicketInMoon = 0.005 ether;

    uint256 public pendingInjectionNextLottery;

    uint256 public constant MIN_DISCOUNT_DIVISOR = 300;
    uint256 public constant MIN_LENGTH_LOTTERY = 1 hours - 55 minutes; // 4 hours
    uint256 public constant MAX_LENGTH_LOTTERY = 4 days + 5 minutes; // 4 days
    uint256 public constant MAX_TREASURY_FEE = 3000; // 30%

    IERC20 public moonToken;
    IRandomNumberGenerator public randomGenerator;
    IMegamoonDealer public dealer;
    ...
    struct Lottery {
        ...
        uint256[6] moonPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInMoon;
        uint256 playerPlaying;
        uint32 finalNumber;
    }
     ...
     */
    constructor(address _moonTokenAddress, address _randomGeneratorAddress, address _dealerAddress) {
        moonToken = IERC20(_moonTokenAddress);
        randomGenerator = IRandomNumberGenerator(_randomGeneratorAddress);
        dealerAddress = _dealerAddress;
        dealer = IMegamoonDealer(dealerAddress);
        ...
        nonReentrant
    {
        ...
        uint256 amountMoonToTransfer = _calculateTotalPriceForBulkTickets(
        ...
        moonToken.safeTransferFrom(address(msg.sender), address(this), amountMoonToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInMoon += amountMoonToTransfer;

        // Increment for player playing
        if ( _userTicketIdsPerLotteryId[msg.sender][_lotteryId].length == 0 ) {
            _lotteries[_lotteryId].playerPlaying += 1;
        } 
    ...
    function claimTickets(
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInMoonToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        moonToken.safeTransfer(msg.sender, rewardInMoonToTransfer);

        emit TicketsClaim(msg.sender, rewardInMoonToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInMoon) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in MOON for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].moonPerBracket[j] =
                ...
                // A. No MOON to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].moonPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInMoon - amountToShareToWinners);

        // Transfer MOON to treasury address
        moonToken.safeTransfer(dealerAddress, amountToWithdrawToTreasury);
        dealer.returnFunds(currentLotteryId, amountToWithdrawToTreasury);
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInMoon >= minPriceTicketInMoon) && (_priceTicketInMoon <= maxPriceTicketInMoon),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            moonPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInMoon: pendingInjectionNextLottery,
            playerPlaying: 0,
            finalNumber: 0
        });
        ...
        _lotteries[currentLotteryId].amountCollectedInMoon += dealer.injectToLottery(currentLotteryId);
    }
     ...
     */
    function setMinAndMaxTicketPriceInMoon(uint256 _minPriceTicketInMoon, uint256 _maxPriceTicketInMoon)
        external
        onlyOwner
    {
        require(_minPriceTicketInMoon <= _maxPriceTicketInMoon, "minPrice must be < maxPrice");
     ...
     * @param _priceTicket price of a ticket (in MOON)
    ...
    function viewNumbersAndStatusesForTicketIds(uint256[] calldata _ticketIds)
        external
        view
        returns (uint32[] memory, bool[] memory, address[] memory)
    {
        uint256 length = _ticketIds.length;
        uint32[] memory ticketNumbers = new uint32[](length);
        bool[] memory ticketStatuses = new bool[](length);
        address[] memory ticketOwners = new address[](length);

        for (uint256 i = 0; i < length; i++) {
            ticketNumbers[i] = _tickets[_ticketIds[i]].number;
            ticketOwners[i] = _tickets[_ticketIds[i]].owner;
        ...
        return (ticketNumbers, ticketStatuses, ticketOwners);
    }
