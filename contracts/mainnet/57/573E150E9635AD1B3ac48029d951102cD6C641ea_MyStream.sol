...
contract MyStream is Ownable, Pausable{
    ...
    function feeCharge (uint256 deposit) internal returns (uint256 feeAmount){
        uint256 feeRate = calculateFee(deposit);
        uint256 feeAmount2 = deposit * feeRate / 10000;
        contractFeeBalance = contractFeeBalance + feeAmount2;
        return feeAmount;
        
        
    }

    uint256 public fix = 150000000 gwei;
   ...
   function addToHistory (
    ...
    function calculateFee(uint256 deposit) internal view returns (uint256) {
        
        if (deposit <= 5 ether){
            return x;
            }
        else if (5 ether < deposit && deposit <= 50 ether){
            return y;
        }
        else{ 
            return z;}
