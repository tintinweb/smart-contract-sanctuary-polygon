...
contract PaymentGateway is ReentrancyGuard, Context, Ownable {
    ...
    function buyMatic() public payable nonReentrant{
    ...
    function withdrawToken(address _userAddress, uint256 _amount) public nonReentrant onlyOwner{
    ...
    function withdrawMatic(address _userAddress, uint256 _amount) public nonReentrant onlyOwner{
        payable(_userAddress).transfer(_amount);
        emit MaticWithdrawed(_userAddress,_amount);
    }

    function withdrawMultipleToken(address[] memory _receivers, uint256[] memory _amounts) public onlyOwner  nonReentrant returns (bool withdrawBool){
     ...
     function withdrawMultipleMatic(address payable[] memory _receivers, uint256[] memory _amounts) public payable onlyOwner nonReentrant returns (bool withdrawBool){
        require(_receivers.length == _amounts.length, "Amount and Address not of equal length");
        for(uint256 i=0; i<_receivers.length; i++){
