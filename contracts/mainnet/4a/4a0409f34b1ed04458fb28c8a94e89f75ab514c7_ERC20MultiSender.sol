...
contract ERC20MultiSender is OwnedUpgradeabilityStorage, Claimable {
    ...
    function multisendToken(address token, address[] _receivers, uint256[] _balances) public hasFee payable {
        if (token == 0x000000000000000000000000000000000000bEEF){
            multisendEther(_receivers, _balances);
        } else {
            uint256 total = 0;
            require(_receivers.length <= arrayLimit());
            ERC20 erc20token = ERC20(token);
            uint8 i = 0;
            for (i; i < _receivers.length; i++) {
                erc20token.transferFrom(msg.sender, _receivers[i], _balances[i]);
                total += _balances[i];
            }
    ...
    function multisendEther(address[] _receivers, uint256[] _balances) public payable {
        ...
        setTxCount(msg.sender, txCount(msg.sender).add(1));
        emit Multisent(msg.value, 0x000000000000000000000000000000000000bEEF);
    }
