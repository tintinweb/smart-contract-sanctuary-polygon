...
/*
 * ** author    : DRRRTTT CULT
 * ** package   : @contracts/ERC721/DRRRTTT.sol
 * ** Desc      : DRRRTTT is a social culture that provides access to give back averyone attentions.
 */
...
contract DrrrtttCult is ERC721AQueryable, DefaultOperatorFilterer, Ownable {
    ...
    constructor(INftFactory newFactory) ERC721A("DRRRTTT CULT", "DRTC") {
    ...
    *                       modifier and other operations
    ...
    modifier mintRequire(uint256 _mintAmount) {
        ...
        require(
            _mintAmount > 0,
            "Minimum 1 DRRRTTT has to be minted per transaction!"
        );
        require(
    ...
    *                                  MINT
    ...
    function mint(uint256 _mintAmount) public payable mintRequire(_mintAmount) {
        require(minting, "Sold Out!.");
        // require(msg.value >= mintPrice * _mintAmount, "Insufficient funds!");
        require(
            mintCount[msg.sender] + _mintAmount <= maxMintAmount,
            "Public mint limit exceeded"
        );
        if (msg.sender != owner()) {
            if (!(freeClaimed[msg.sender] >= maxFreeMint)) {
                if (_mintAmount <= maxFreeMint - freeClaimed[msg.sender]) {
                    ...
                    require(
                        msg.value >=
                            mintPrice *
                                (_mintAmount -
                                    (maxFreeMint - freeClaimed[msg.sender])),
...
/*
 * ** author    : DRRRTTT CULT
 * ** package   : @contracts/ERC721/interfaces/INftFactory.sol
 * ** Desc      : DRRRTTT is a social culture that provides access to give back averyone attentions.
 */
pragma solidity ^0.8.17;
