  ...
  ReentrancyGuard
{
  ...
  bytes32 public merkleRootMB = 0x8aa06fb4a8cd454dbaca53d875df328363b04aa367f639c938a491ba025b997b;
  ...
  uint256 public startTime = 1687849200;
  uint256 public endTime = 1689058800;
  ...
  nonReentrant
  {
    require(!isPaused, "FANSIHP9: currently paused");
    require(
      block.timestamp >= startTime,
      "FANSIHP9: public sale is not started yet"
    );
    require(
      block.timestamp < endTime,
      "FANSIHP9: public sale is ended"
    );
    require(
      _purchaseNum <= amountPerTrans,
      "FANSIHP9: each transaction can only purchase 20"
    );

    uint256 supply = totalSupply();
    require(
      (supply + _purchaseNum) <= maxSupply,
      "FANSIHP9: reached max supply"
    );
    ...
    require(sent, "FANSIHP9: Failed to send");
  }
  ...
  function batchTransferFrom(
    ...
    require(
            ...
            "FANSIHP9: incorrect array length"
    );
  ...
  modifier onlyUser() {
    require(_msgSender() == tx.origin, "FANSIHP9: no contract mint");
    _;
  }

  modifier onlyAdmin() {
    require(adminList[_msgSender()], "FANSIHP9: not admin");
    _;
  }
