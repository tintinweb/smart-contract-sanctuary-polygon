...
// File contracts/BIFI/strategies/Stargate/StrategyStargatePoly.sol
...
contract StrategyStargatePoly is StratManager, FeeManager, GasThrottler {
    ...
    struct Routes {
        ...
        address[] outputToInputRoute;
        address[] outputToNativePool;
        address[] outputToInputPool;
    }
    ...
    ITridentRouter.ExactInputParams public outputToNativeParams;
    ITridentRouter.ExactInputParams public outputToInputParams;
    ...
    constructor(
        ...
        output = _routes.outputToNativeRoute[0];
        native = _routes.outputToNativeRoute[_routes.outputToNativeRoute.length - 1];
        input = _routes.outputToInputRoute[_routes.outputToInputRoute.length - 1];

        require(_routes.outputToInputRoute[0] == output, '_outputToInputRoute[0] != output');
        outputToNativeRoute = _routes.outputToNativeRoute;
        outputToInputRoute = _routes.outputToInputRoute;

        uint256 routeLength = _routes.outputToNativePool.length;
        for (uint256 i; i < routeLength; i++) {
            outputToNativeParams.path.push(ITridentRouter.Path(
                _routes.outputToNativePool[i], abi.encode(outputToNativeRoute[i], address(this), false)
            ));
        }
        outputToNativeParams.tokenIn = output;
        outputToNativeParams.amountOutMinimum = 1;

        routeLength = _routes.outputToInputPool.length;
        for (uint256 i; i < routeLength; i++) {
            outputToInputParams.path.push(ITridentRouter.Path(
                _routes.outputToInputPool[i], abi.encode(outputToInputRoute[i], address(this), false)
            ));
        }
        outputToInputParams.tokenIn = output;
        outputToInputParams.amountOutMinimum = 1;
    ...
    function chargeFees(address callFeeRecipient) internal {
        outputToNativeParams.amountIn = IERC20(output).balanceOf(address(this)).mul(45).div(1000);
        ITridentRouter(unirouter).exactInput(outputToNativeParams);
    ...
    function addLiquidity() internal {
        outputToInputParams.amountIn = IERC20(output).balanceOf(address(this));
        ITridentRouter(unirouter).exactInput(outputToInputParams);
    ...
    function callReward() external view returns (uint256) {
        ...
        uint256 nativeOut;
        if (outputBal > 0) {
            for (uint256 i; i < outputToNativeRoute.length; i++) {
                bytes memory data = abi.encode(outputToNativeRoute[i], outputBal);
                outputBal = IBentoPool(outputToNativeParams.path[i].pool).getAmountOut(data);
            }
            nativeOut = outputBal;
        }
