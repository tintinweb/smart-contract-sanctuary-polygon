...
contract atest  {
    ...
    uint256[2] private leaderPercents = [200, 300];
    uint256[2] private executivePercents = [400, 200];
    uint256[15] private seniorexecutivePercents = [200, 100, 100, 100, 100, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50];
    uint256 private constant executivePoolPercents = 50;
    uint256 private constant seniorexecutivePoolPercents = 50;
    uint256 private constant chiefexecutivePoolPercents = 100;
    ...
    uint256 public totalUser; 
     ...
     uint256 public executivePool;
     uint256 public seniorexecutivePool;
     uint256 public chiefexecutivePool;
    mapping(uint256=>address[]) public dayUsers;
     
     address[] public leaderUsers;
     address[] public executiveUsers;
     address[] public seniorexecutiveUsers;
     address[] public chiefexecutiveUsers;
    ...
    struct RewardInfo{
       ...
       struct RewardInfoPool{
        uint256 executive;
        uint256 seniorexecutive;
        uint256 chiefexecutive;
    ...
    event SetfeeReceivers(address[2]  Address);
    ...
    constructor(address _usdtAddr)   {
        Dai = IERC20(_usdtAddr);
         ...
         feeReceivers[0] = address(0x305E5711206dfE593663A06f39fedb3189404bfF);
         feeReceivers[1] = address(0x50A76e2e24be132c7ae5B5175951E63dC0F9ee2d); 
        ...
        if(levelNow > user.level){
            user.level = levelNow;
             if(levelNow == 2){        
              ...
              if(levelNow == 3){        
                executiveUsers.push(_user);
            }
            if(levelNow == 4){        
                seniorexecutiveUsers.push(_user);
            }
            if(levelNow == 5){
              
                chiefexecutiveUsers.push(_user);
            }
  ...
  function getTeamDeposit(address _user) public view returns(uint256, uint256, uint256){
   ...
   function _deposit(address _user, uint256 _amount) private {
     ...
     function _distributeDeposit(uint256 _amount) private {
        ...
        uint256 executive = _amount.mul(executivePoolPercents).div(baseDivider);
        executivePool = executivePool.add(executive); 
          uint256 seniorexecutive = _amount.mul(seniorexecutivePoolPercents).div(baseDivider);
        seniorexecutivePool = seniorexecutivePool.add(seniorexecutive); 
         uint256 chiefexecutive = _amount.mul(chiefexecutivePoolPercents).div(baseDivider);
        chiefexecutivePool = chiefexecutivePool.add(chiefexecutive); 
    }

    function distributePoolRewards() public {
        if(block.timestamp > lastDistribute.add(timeStep)){     
           _distributeExecutivePool(); 
           _distributeSeniorExecutivePool();
           _distributeChiefExecutivePool();
            lastDistribute = block.timestamp;
        }
    }

       function _distributeExecutivePool() private {
        uint256 executiveCount;
        for(uint256 i = 0; i < executiveUsers.length; i++){
            if(userInfo[executiveUsers[i]].level == 3){
                executiveCount = executiveCount.add(1);
            }
        }
        if(executiveCount > 0){
            uint256 reward = executivePool.div(executiveCount);
            uint256 totalReward;
            for(uint256 i = 0; i < executiveUsers.length; i++){
                if(userInfo[executiveUsers[i]].level == 3){
                    rewardInfoPool[executiveUsers[i]].executive = rewardInfoPool[executiveUsers[i]].executive.add(reward);
                    userInfo[executiveUsers[i]].totalRevenue = userInfo[executiveUsers[i]].totalRevenue.add(reward);
                    totalReward = totalReward.add(reward);
                }
            }
            if(executivePool > totalReward){
                executivePool = executivePool.sub(totalReward);
            }else{
                executivePool = 0;
            }
        }
    }
 function _distributeSeniorExecutivePool() private {
        uint256 seniorexecutiveCount;
        for(uint256 i = 0; i < seniorexecutiveUsers.length; i++){
            if(userInfo[seniorexecutiveUsers[i]].level == 4){
                seniorexecutiveCount = seniorexecutiveCount.add(1);
            }
        }
        if(seniorexecutiveCount > 0){
            uint256 reward = seniorexecutivePool.div(seniorexecutiveCount);
            uint256 totalReward;
            for(uint256 i = 0; i < seniorexecutiveUsers.length; i++){
                if(userInfo[seniorexecutiveUsers[i]].level == 4){
                    rewardInfoPool[seniorexecutiveUsers[i]].seniorexecutive = rewardInfoPool[seniorexecutiveUsers[i]].seniorexecutive.add(reward);
                    userInfo[seniorexecutiveUsers[i]].totalRevenue = userInfo[seniorexecutiveUsers[i]].totalRevenue.add(reward);
                    totalReward = totalReward.add(reward);
                }
            }
            if(seniorexecutivePool > totalReward){
                seniorexecutivePool = seniorexecutivePool.sub(totalReward);
            }else{
                seniorexecutivePool = 0;
            }
        }
    }

 function _distributeChiefExecutivePool() private {
        uint256 chiefexecutiveCount;
        for(uint256 i = 0; i < chiefexecutiveUsers.length; i++){
            if(userInfo[chiefexecutiveUsers[i]].level == 5){
                chiefexecutiveCount = chiefexecutiveCount.add(1);
            }
        }
        if(chiefexecutiveCount > 0){
            uint256 reward = chiefexecutivePool.div(chiefexecutiveCount);
            uint256 totalReward;
            for(uint256 i = 0; i < chiefexecutiveUsers.length; i++){
                if(userInfo[chiefexecutiveUsers[i]].level == 5){
                    rewardInfoPool[chiefexecutiveUsers[i]].chiefexecutive = rewardInfoPool[chiefexecutiveUsers[i]].chiefexecutive.add(reward);
                    userInfo[chiefexecutiveUsers[i]].totalRevenue = userInfo[chiefexecutiveUsers[i]].totalRevenue.add(reward);
                    totalReward = totalReward.add(reward);
                }
            }
            if(chiefexecutivePool > totalReward){
                chiefexecutivePool = chiefexecutivePool.sub(totalReward);
            }else{
                chiefexecutivePool = 0;
            }
        }
    }
    ...
    function withdraw() external {
       ...
       userRewards.level3Released = 0;
        ...
        userRewardsf.executive = 0;
        userRewardsf.seniorexecutive = 0;
        userRewardsf.chiefexecutive = 0; 
    ...
    function _calCurDynamicRewards(address _user) private view returns(uint256, uint256) {
        ...
        totalRewards = totalRewards.add(userRewardsf.executive.add(userRewardsf.seniorexecutive).add(userRewardsf.chiefexecutive));
        uint256 splitAmt = totalRewards.mul(freezeIncomePercents).div(baseDivider);
        uint256 withdrawable = totalRewards.sub(splitAmt);
        return(withdrawable, splitAmt);
    }
    ...
    function setfeeReceivers(address[2] calldata _address) external {    
        require(_address.length==2, "invalid");
               ...
               _updatestatus(upline);
                ...
                RewardInfo storage upRewards = rewardInfo[upline];
                  ...
                  if(i > 0 && i < 3 && userInfo[upline].level > 1 && idstatus==true){
                    if(upRewards.level3Freezed > 0){
                        uint256 level3Reward = newAmount.mul(leaderPercents[i - 1]).div(baseDivider);
                ...
                if(i >= 3 && i < 5 && userInfo[upline].level > 2 && idstatus==true){
                    if(upRewards.level4Freezed > 0){
                        uint256 level4Reward = newAmount.mul(executivePercents[i - 3]).div(baseDivider);
                ...
                if(i >= 5 && userInfo[upline].level > 3  && idstatus==true){
                    if(upRewards.level5Left > 0){
                        uint256 level5Reward = newAmount.mul(seniorexecutivePercents[i - 5]).div(baseDivider);
