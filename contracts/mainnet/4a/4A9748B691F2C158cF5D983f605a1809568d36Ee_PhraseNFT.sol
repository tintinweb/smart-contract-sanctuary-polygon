 ...
 */
contract PhraseNFT is ERC721Enumerable , ContextMixin, NativeMetaTransaction, Ownable {
    using SafeMath for uint256;
    ...
    mapping (string => uint256) public phraseToID;
    mapping (uint256 => string) public idToPhrase;
    mapping (uint256 => uint256) public idToRarity;

    uint256 _initTokenID = uint256(0xBAAD3FDE86FAA3B42D6A047060308E49A24EC9E7000000000000000000000000);
    
    uint256 public baseFee = 150000000000;
    uint256 public perWordFee = 100000000000;
    uint128 public defaultRentalFee = 75000000000;

    IERC1155 OpenSeaContract = IERC1155(0x2953399124F0cBB46d2CbACD8A89cF0599974963);
    WordNFT WordContract = WordNFT(0x7bDfBCd72BeC49fDe3BF0f027aCAa3554d999C3d);
    ITheImpossibleGame StakingContract = ITheImpossibleGame(0x5A1b097f668c1Ea770dD571Ba0fd3d9167360e81);
    ZXVC ZXVCContract = ZXVC(0x416E244f3f46A26B6a9d5b81c325cce0Ba0114Df);

    string public contractURI = "https://ipfs.moralis.io:2053/ipfs/QmesqSHtWzto9erX8Dhu1fWEgphi831pXLKC5fszmdngPt";
    string public blankURI = "https://ipfs.moralis.io:2053/ipfs/QmRCKTcJERDH7A5YbncYpV6mt39L8h4g6iT3JTGoQpBynk";
    ...
    event WordUsagePaymentReceived(address renter, address owner, uint256 phraseID, uint256 payment);
    event PhraseMinted(address minter, uint256 phraseID, uint256 rarity, string phrase);

    constructor() ERC721("TIG: Phrases", "PHRASE") {
    ...
    function setStakingContract(ITheImpossibleGame newStaker) external onlyManager {
        StakingContract = newStaker;
    }

    function setBaseFee(uint256 fee) external onlyManager {
        baseFee = fee;
    }

    function setPerWordFee(uint256 fee) external onlyManager {
        perWordFee = fee;
    }

    function setDefaultRentalFee(uint128 fee) external onlyManager {
        defaultRentalFee = fee;
    }

    function overWritePhrase(uint256 phraseID, string memory newPhrase) external onlyManager {
        phraseToID[newPhrase] = phraseID;
        idToPhrase[phraseID] = newPhrase;
    }
    ...
    function priceEstimate(string memory phrase, uint256[] memory phraseComponents, address[] memory componentOwners, address _creator) external view returns (uint256) {
        require(phraseToID[phrase] == uint256(0), "Identical phrase already exists!");
        uint256 totalFee = baseFee;

        for(uint i = 0; i < phraseComponents.length; i++) {
            if(componentOwners[i] == address(0)) {
                //undefined words cost you extra
                totalFee += perWordFee + defaultRentalFee + defaultRentalFee;
            } else {
                totalFee += perWordFee;
                PhraseInfo memory phraseInfo = getParentInfo(phraseComponents[i], componentOwners[i], _creator);
                if(phraseInfo.rentalFee > 0) {
                    totalFee += phraseInfo.rentalFee;
                }
            }
        }
    ...
    function mintPhrase(string memory phrase, uint256[] memory phraseComponents, address[] memory componentOwners, uint256 rarityBonus, address _creator, uint256 expectedFee) external onlyManager {
        require(phraseToID[phrase] == uint256(0), "Identical phrase already exists!");
        require(phraseComponents.length > 1, "One words is not a phrase!");
        
        uint256 burnFee = baseFee;
        uint256 totalFee = 0;
        address managedAddress = ZXVCContract.depositAddress(_creator);
        uint256 phraseID = create(_creator);

        for(uint i = 0; i < phraseComponents.length; i++) {
            if(componentOwners[i] == address(0)) {
                //undefined words cost you extra
                burnFee += perWordFee + defaultRentalFee + defaultRentalFee;
            } else {
                burnFee += perWordFee;
                PhraseInfo memory phraseInfo = getParentInfo(phraseComponents[i], componentOwners[i], _creator);
                if(phraseInfo.rentalFee > 0) {
                    require(managedAddress != address(0) && ZXVCContract.balanceOf(managedAddress) > phraseInfo.rentalFee, "Not Enough ZXVC");
                    ZXVCContract.managedTransfer(_creator, ZXVCContract.depositAddress(phraseInfo.rentee), phraseInfo.rentalFee);
                    emit WordUsagePaymentReceived(_creator, phraseInfo.rentee, phraseID, phraseInfo.rentalFee);
                    totalFee += phraseInfo.rentalFee;
                }
            }
        }
        ...
        require(expectedFee >= totalFee, "fee exceeded");
        require(managedAddress != address(0) && ZXVCContract.balanceOf(managedAddress) > burnFee, "Not Enough ZXVC");

        ZXVCContract.managedTransfer(_creator, address(this), burnFee);
        ZXVCContract.burn(burnFee);

        phraseToID[phrase] = phraseID;
        idToPhrase[phraseID] = phrase;

        uint256 rarity = uint256(keccak256(abi.encodePacked(
                blockhash(block.number - 20), 
                blockhash(block.number - 15), 
                blockhash(block.number - 10)))) % 1000;

        if(rarityBonus > 900)
            rarityBonus = 900;

        //Trippy words add 9 (1%) to the rarityBonus
        //Animated words add 36 (4%)
        //Gilded words add 90 (10%)

        if (rarity + (rarityBonus / 9) >= 999) {
            idToRarity[phraseID] = 3;
        } else if (rarity + (rarityBonus / 3) >= 899 + (rarityBonus / 9)) {
            idToRarity[phraseID] = 2;
        } else if (rarity + rarityBonus >= 599 + (rarityBonus / 9) + (rarityBonus / 3)) {
            idToRarity[phraseID] = 1;
        }

        emit PhraseMinted(_creator, phraseID, idToRarity[phraseID], phrase);
    }

    function getParentInfo(uint256 _mother, address _motherOwner, address _creator) internal view returns (PhraseInfo memory) {
        PhraseInfo memory parentInfo;

        if (StakingContract._stakedToOwner(_mother) != address(0)) {
            //The mother is a Staked Word
            if(StakingContract._stakedToOwner(_mother) != _creator) {
                stakedNFT memory stakingData = StakingContract.getStakedNFTData(_mother);
                parentInfo.rentalFee = stakingData._usageFee;
                parentInfo.rentee = StakingContract._stakedToOwner(_mother);
            }                
        } else if(OpenSeaContract.balanceOf(_motherOwner, _mother) > 0) {
            //The mother is an unstaked Genesis Word
            if(_motherOwner != _creator) {
                parentInfo.rentee = StakingContract._stakedToOwner(_mother);
            }
        } else if(WordContract.ownerOf(_mother) != _creator) {
            //The mother is an unstaked v2 Word
            parentInfo.rentee = WordContract.ownerOf(_mother);
        }

        if(parentInfo.rentee != address(0) && parentInfo.rentalFee == 0) {
            parentInfo.rentalFee = defaultRentalFee;
        }
    ...
    */
    function create(
        ...
        _nextTokenId.increment();
        uint256 _id = _initTokenID.asIndex(currentTokenId);
    ...
    */
    ...
    function burn(uint256 tokenID) external {
        require(ownerOf(tokenID) == msgSender(), "Can't burn someone else's token.");
        _burn(tokenID);
    }
...
interface ITheImpossibleGame {
    function _stakedToOwner(uint256 a) external view returns (address);

    function _mintbotTokens(address a) external view returns (uint256);
...
contract TheImpossibleGame is Ownable, IERC1155Receiver, IERC721Receiver, ERC165, ITheImpossibleGame {
    ...
    mapping(uint256 => address) public override _stakedToOwner;
    mapping(address => uint256) public override _mintbotTokens;
