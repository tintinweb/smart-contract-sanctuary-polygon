...
contract Multicall {
    ...
    function uniV2(address pool, bool direc) public {
        address token=direc ? IUniV2Pool(pool).token0() : IUniV2Pool(pool).token1();
        uint am= IERC20(token).balanceOf(address(this));
        IERC20(token).transfer(pool, am);
        (uint112 reserve0, uint112 reserve1, ) = IUniV2Pool(pool).getReserves();
        am = (am * 997) / 1000;
        IUniV2Pool(pool).swap(direc ? 0 : (am * reserve0) / (reserve1 + am),direc ? (am * reserve1) / (reserve0 + am) : 0,address(this),"");
    }

    function uniV3(address pool, bool direc) public {
        IUniV3Pool(pool).swap(address(this),direc,int(IERC20(direc ? IUniV3Pool(pool).token0() : IUniV3Pool(pool).token1()).balanceOf(address(this))),direc ? 4295128740 : 1461446703485210103287273052203988822378723970341,"");
    }

    function kyberV3(address pool, bool direc) public {
        IKyberV3Pool(pool).swap(address(this),int(IERC20(direc ? IKyberV3Pool(pool).token0() : IKyberV3Pool(pool).token1()).balanceOf(address(this))),direc,direc ? 4295128740 : 1461446703485210103287273052203988822378723970341,"");
    }

    function route(Route calldata _route) public returns(uint amOut){
        IERC20(_route.tIn).transferFrom(msg.sender, address(this), _route.amIn);
        for (uint8 i = 0; i < _route.callPath.length; i++)
            address(this).call(_route.callPath[i]);
        amOut=IERC20(_route.tOut).balanceOf(address(this));
        require(amOut>=_route.amOut,"amOut");
        IERC20(_route.tOut).transfer(msg.sender, amOut);
    }
...
interface IUniV3Pool is IPool {
    function swap(address recipient,bool zeroForOne,int256 amountSpecified,uint160 sqrtPriceLimitX96,bytes calldata data) external returns (int256 amount0, int256 amount1);
    function fee() external view returns (uint24);
}

interface IKyberV3Pool is IPool {
