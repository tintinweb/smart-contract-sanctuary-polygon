...
contract Sharpe is IVault,IUniswapV3MintCallback,IUniswapV3SwapCallback,ERC20,ReentrancyGuard{
     ...
     * @param swapMin minimum amount of token1 or token2 output after a swap
     * @param to Recipient of shares
     * @return shares Number of shares minted
     * @return amount0 Amount of token0 deposited
     * @return amount1 Amount of token1 deposited
     */
    function deposit(uint256 amount0Desired,uint256 amount1Desired,uint256 amount0Min,uint256 amount1Min,uint256 swapMin, address to)
        ...
        returns (uint256 shares,uint256 amount0,uint256 amount1)
    {
        ...
        if (shares == 0){ //when only amount1 or amount2 available splits the amount in two and performs a swap
            ...
            if (amount0 > 0 && amount1 == 0){
                ...
                uint[] memory amount1ToStore = router.swapExactTokensForTokens(difference,swapMin,path,address(this), block.timestamp); //swaps the difference
                //calculate shares and this time shares should be more than zero
                (shares, amount0, amount1) = _calcSharesAndAmounts(amount0ToStore, amount1ToStore[1]);
                require(shares > 0, "swappedShares");
                uint256 dust0 = amount0ToStore.sub(amount0);
                uint256 dust1 = amount1ToStore[1].sub(amount1);
                ...
                uint[] memory amount0ToStore = router.swapExactTokensForTokens(difference,swapMin,path,address(this), block.timestamp);
                (shares, amount0, amount1) = _calcSharesAndAmounts(amount0ToStore[1], amount1ToStore);
                require(shares > 0, "swappedShares");
                uint256 dust0 = amount0ToStore[1].sub(amount0);
        ...
        returns (uint256 shares,uint256 amount0,uint256 amount1)
    {
        ...
        if (totalSupply == 0) {
            ...
            else { //at this point both tokens are available for deposit and the shares are calculated
