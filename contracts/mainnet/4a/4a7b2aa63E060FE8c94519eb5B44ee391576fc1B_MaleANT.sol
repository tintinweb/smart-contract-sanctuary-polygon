...
contract MaleANT is ERC721, Ownable {
    uint256 public counter = 1;
    uint256 public immutable MATE_DURATION = 1 minutes;

    constructor() ERC721("MaleANT","MaleANT") {
    }
    ...
    function getMatedMales(address _user) public view returns(uint256[] memory) {
        uint256[] memory matedMaleList;
        uint256[] memory _maleList = playerToMales[_user];
        uint256 matedMaCount;
        for (uint256 i; i < _maleList.length; i++){
            if (idToVirginity[_maleList[i]]==false) {
                matedMaleList[matedMaCount] = _maleList[i];
                matedMaCount++;
            }
        }
    ...
    function burn(address _user, uint256 _index) public {
        ...
        playerToMales[_user][_index] =  playerToMales[_user][ playerToMales[_user].length - 1];
