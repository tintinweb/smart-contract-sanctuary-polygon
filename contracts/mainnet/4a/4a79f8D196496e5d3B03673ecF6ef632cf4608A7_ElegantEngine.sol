...
contract ElegantEngine is Ownable, Concatenate {
    ...
    function fix1(address _from, uint _start, uint _end) public onlyOwner {
        EngineInterface _old = EngineInterface(_from);
        uint i = _start;
        uint len = _end + 1;
        for (i; i < len; i ++) {
            (uint _id, 
            ...
            uint _finish, 
            uint _event, 
            uint _race) = _old.perfByRacer(0,i);
            perfByRacer[0][i].id = _id;
            perfByRacer[0][i].nftContract = _con;
            perfByRacer[0][i].eventWanted = _event;

            racersByEvent[_event].push(i);
        }
