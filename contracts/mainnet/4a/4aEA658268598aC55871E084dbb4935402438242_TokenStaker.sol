...
contract TokenStaker is Ownable {
    ...
    */
    ...
    string public lockPeriod;  // locked period in text to be shown by the GUI
    uint256 private contractLockPeriod ; // in seconds
    ...
    */
    function getPoolTotalRewardSupply() public view returns(uint256) {
        return poolInfo.stakeToken.balanceOf(address(this));
    }
    ...
    */
    ...
    function releaseReward(uint256 _amount) public onlyOwner {
        ...
        uint256 totalRewardAmount = poolInfo.rewardToken.balanceOf(address(this));
        require(_amount <= totalRewardAmount, "releaseReward: insufficient reward token!");
        pool.rewardToken.safeTransfer(poolUnusedRewardAddress, _amount);
    }
...
*/
    ...
    */
    function claim() external {
        ...
        uint256 poolRewardTokenSupply = poolInfo.rewardToken.balanceOf(address(this));
        require(user.rewardEarned < poolRewardTokenSupply, 'claim: Insufficient reward token in the contract');

        // transfer the reward
        if (user.rewardEarned > 0) {
            poolInfo.rewardToken.safeTransfer(msg.sender, user.rewardEarned);
            pool.poolTotalRewardDebt = pool.poolTotalRewardDebt.sub(user.rewardEarned);
        }
        user.rewardEarned = 0;
    }
