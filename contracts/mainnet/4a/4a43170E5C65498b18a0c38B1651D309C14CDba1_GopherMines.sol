...
{
    modifier onlyDevs {
        //  require(msg.sender == primaryDevAddress , "Dev Only: caller is not the developer");
         _;
    }
    ...
    address  public royaltyAddress = 0x965E2595D2C6aDD00A42dD281eBf7902bC7D92Ff;
    address  public router = 0x965E2595D2C6aDD00A42dD281eBf7902bC7D92Ff;
    ...
    //traking NFT staking fees
    mapping(uint => uint) public stakingROI;

    //mapping tokenId to fee
    mapping(uint => uint) public idToFee;

    
    //mapping tokenId to NFTID
    mapping(uint => string) public tokenIdToNFTId;
    mapping(uint => string) public tokenIdToNFTName;
    ...
    ) ERC721("GopherMines", "GopherMines") {
        receiver = _receiver;
    ...
    function addNFT(uint _fee, string memory _id, string memory _uri, uint _percentage, string memory _name) public onlyOwner {
        NFT memory nft = NFT(_id, _fee , _uri, _percentage, _name);
        nfts[_id] = nft;
    }
...
function mintNFT(IERC20 _usdcContract, string memory _id) public payable {
    ...
    require(fee != 0,"NFT is not registered");

    require(_usdcContract.balanceOf(_msgSender()) >= fee, "Insufficient funds.");

   _usdcContract.transferFrom(_msgSender(), receiver, fee );
        internalMint(msg.sender, nft.uri, nft.percentage, fee, _id, nft.name);
         emit mintEvent(msg.sender, nft.id);

}

function multisender(address[] memory _receiver, uint256[] memory _amount, IERC20 _usdcContract) onlyOwner public {
    uint256 totalAmount = 0;
    for(uint256 i; i < _amount.length; i++){
        totalAmount = totalAmount + _amount[i];
    }
    //Dev checks of the allowance for the token to send is enough from the sender
    require(_usdcContract.allowance(msg.sender,address(this)) >= (totalAmount / 10 ** 6), "Spending not allowed by owner");

    //Dev checks if called has enough amount to make the transaction work
    require(_usdcContract.balanceOf(msg.sender) >= totalAmount, "Insufficient balance");

    for(uint256 i; i < _receiver.length; i++){
        _usdcContract.transferFrom(msg.sender, _receiver[i], _amount[i]);
    }
...
function setURI(uint256 tokenId, string memory uri) external onlyOwner {
    ...
    //Overrides
     ...
     function internalMint(address to, string memory _uri, uint _percentage, uint _fee, string memory _id, string memory _name) internal {
        _tokenIdCounter.increment();
        _safeMint(to, _tokenIdCounter.current());
        tokenURIs[_tokenIdCounter.current()] = _uri;
        stakingROI[_tokenIdCounter.current()] = _percentage;
        idToFee[_tokenIdCounter.current()] = _fee;
        tokenIdToNFTId[_tokenIdCounter.current()] = _id;
        tokenIdToNFTName[_tokenIdCounter.current()] = _name;
