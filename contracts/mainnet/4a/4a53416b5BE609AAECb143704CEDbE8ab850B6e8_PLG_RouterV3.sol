...
contract PLG_RouterV3 is permission {
    
    address public owner;

    address public plg_pool = 0x9aCCd8D9D0DDc42cB2DDad985B0Fdf0e0bFB81fC;
    address public plg_user = 0xB4c2bd52a184DD0796c26e136891Fdb10d60A016;
    address public plg_refreward = 0x5EEddE12d4F65af99a29c27Dcbb9389732ddAC4a;
    ...
    function depositWithMATIC_Migrate(address account,address migrate) public forRole("owner") returns (bool) {
        uint256[] memory load = IPLGData(migrate).invest(account);
        _updateWithPermit(account,load);
        return true;
    }
    ...
    function unlockVIP(address account) public returns (bool) {
        IERC20(plg_token).transferFrom(msg.sender,address(0xdead),vipBlockPrice);
