...
contract MaticxStrategy is BaseStrategy {
    ...
    bytes32 public wantPoolId = 0xc17636e36398602dd37bb5d1b3a9008c7629005f0002000000000000000004c4; //MATICX-WMATIC
    ...
    constructor()
        public
        BaseStrategy(
            0xC17636e36398602dd37Bb5d1B3a9008c7629005f, //MATICX-WMATIC LP
            0x01fE07ce760DA7a025e4Cf3f950aE236F8e62120, //fee dist fund
            BAL,
            0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff, //QS router
            0x48534d027f8962692122dB440714fFE88Ab1fA85 //reward gauge
        )
    {
        ...
        //reward_underlying_path = [BAL, WMATIC]; //not used, since we swap thru balancer
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        if (amountHarvested > 0) {
            uint256 _fee = amountHarvested.mul(IERCFund(strategist).getFee()).div(FEE_DENOMINATOR);
            IERC20(harvestedToken).safeTransfer(strategist, _fee);
        }
        uint256 amountHarvested2 = IERC20(harvestedToken2).balanceOf(address(this));
        if (amountHarvested2 > 0) {
            uint256 _fee = amountHarvested2.mul(IERCFund(strategist).getFee()).div(FEE_DENOMINATOR);
            IERC20(harvestedToken2).safeTransfer(strategist, _fee);
        }
        ...
        if (remainingSecondReward > 0) {
            _swapUniswapWithPath(reward2_underlying_path, remainingSecondReward);
        }
        ...
        if(underlyingBal > 0) {
            balancerJoin(wantPoolId, WMATIC, underlyingBal);
        }
    ...
    function balancerJoin(bytes32 _balancerPoolId, address _tokenIn, uint256 _amount) internal {
        uint256[] memory amounts = new uint256[](poolTokens.length);
        for (uint256 i = 0; i < amounts.length; i++) {
            if(poolTokens[i] == _tokenIn) {
                amounts[i] = _amount;
            }
        }
        bytes memory userData = abi.encode(1, amounts, 1);

        IBalancerVault.JoinPoolRequest memory request = IBalancerVault.JoinPoolRequest(poolTokens, amounts, userData, false);
        IBalancerVault(BALANCER_VAULT).joinPool(_balancerPoolId, address(this), address(this), request);
    }
