 ...
 * if (length of players) is equals to 1000 then (creates an internal variable winningAddress with initial value players with element (random number with seed (length of players)) modulo (length of players); then checks that (amount of native currency owned by the address of this contract) is greater than or equals to retainedForJackpotNative; then if retainedForJackpotNative is strictly greater than 0 then (transfers retainedForJackpotNative of the native currency to winningAddress); then updates retainedForJackpotNative as 0; then updates players as Empty List; then adds winningAddress to winners; and then emits event JackpotDone with inputs winningAddress)
*/
	function playJackpot() external payable {
		...
		if (((players).length == uint256(1000))){
