 ...
 */
contract VotingSnapshot is IVotingSnapshot, GovernableImplementation, ProxyImplementation {
  /*******************************************************
   *                     Configuration
   *******************************************************/
  ...
  mapping(address => mapping(address => Vote)) internal legacyAccountVoteByPool;
  mapping(address => mapping(uint256 => Vote)) internal legacyAccountVoteByIndex;
  mapping(address => mapping(address => uint256)) internal legacyAccountVoteIndexByPool;
  ...
  mapping(address => mapping(address => uint256)) public accountVoteIndexByPool;

  bool public isClearVoteRequired;

  // Modifiers
  modifier onlyVoteDelegateOrOwner(address accountAddress) {
    if (msg.sender != vlPenAddress) {
      bool voteDelegateSet = voteDelegateByAccount[accountAddress] != address(0);
      if (voteDelegateSet) {
        if (accountAddress == msg.sender) {
          revert("You have delegated your voting power (you cannot vote)");
        }
        require(voteDelegateByAccount[accountAddress] == msg.sender, "Only vote delegate can vote");
      } else {
        require(accountAddress == msg.sender, "Only users and delegates can vote");
      }
    }
    _;
  }
  ...
  function setIsClearVoteRequired(bool _isClearVoteRequired) external onlyGovernance {
    isClearVoteRequired = _isClearVoteRequired;
  }
  ...
  function registerVote(Vote memory vote) internal {
    ...
    uint256 newVotesLengthPerWeight = votesLengthByWeight[newPoolWeightAbsolute];
    uint256 currentVotesLengthPerWeight = votesLengthByWeight[currentPoolWeightAbsolute];
    ...
    if (poolHasNoVotes) {
      ...
      if (newWeightExists) {
        ...
        votesByWeight[newPoolWeightAbsolute][newVotesLengthPerWeight] = vote;
       ...
       */
      for (uint256 voteIndex; voteIndex < currentVotesLengthPerWeight; voteIndex++) {
        Vote memory currentVote = votesByWeight[currentPoolWeightAbsolute][voteIndex];
        ...
        if (currentVote.poolAddress == poolAddress) {
          ...
          if (newVoteIsNotZero) {
            Vote memory updatedVote = Vote({poolAddress: poolAddress, weight: newPoolWeight});
  ...
  function unregisterVote(Vote memory vote, uint256 voteIndex) internal {
    ...
    uint256 votesLengthPerWeight = votesLengthByWeight[currentPoolWeightAbsolute];
    ...
    Vote memory lastVote = votesByWeight[currentPoolWeightAbsolute][votesLengthPerWeight - 1];

    // Replace the vote to remove with the last vote
    votesByWeight[currentPoolWeightAbsolute][indexOfVoteInWeightList] = lastVote;
   ...
   */
  function voteWeightTotalByAccount(address accountAddress) public view returns (uint256) {
    return vlPen.lockedBalanceOf(accountAddress);
  }

  function voteWeightAvailableByAccount(address accountAddress) public view returns (uint256) {
    return voteWeightTotalByAccount(accountAddress) - voteWeightUsedByAccount[accountAddress];
  }
  ...
  function vote(
    ...
    registerVote(Vote({poolAddress: poolAddress, weight: accountVoteDelta}));
  }
  ...
  {
    // Find vote to remove
    Vote memory voteToRemove = accountVoteByPool[accountAddress][poolAddress];
    ...
    uint256 votesLength = votesLengthByAccount[accountAddress];
    Vote memory lastVote = accountVoteByIndex[accountAddress][votesLength - 1];
    ...
    accountVoteIndexByPool[accountAddress][lastVote.poolAddress] = voteIndex;

    // Reduce votes length
    votesLengthByAccount[accountAddress]--;

    // Remove vote weight used by account
    voteWeightUsedByAccount[accountAddress] -= Math.abs(voteToRemove.weight);

    // Remove account vote by pool
    delete accountVoteByPool[accountAddress][poolAddress];

    // Register a negating vote for the user
    registerVote(Vote({poolAddress: poolAddress, weight: -voteToRemove.weight}));
  }
   ...
   */
  function prepareVote() public view returns (address[] memory, int256[] memory) {
    ...
    _topVotesWeight = (_topVotesWeight * basis) / (basis - totalEcosystemPoolsMinWeight);
    ...
    for (uint256 voteIndex; voteIndex < _topVotes.length; voteIndex++) {
      ...
      if (isEcosystemPool[poolsAddresses[voteIndex]]) {
        poolWeight += int256(
          (ecosystemPoolMinWeight[poolsAddresses[voteIndex]] * _topVotesWeight) / basis
        );
      }

      int256 weightRatio = (int256(veBalanceOfNft) * poolWeight) / int256(_topVotesWeight);
  ...
  function submitVote() external {
    require(
      (block.timestamp >= nextVoteSubmission() && block.timestamp < nextEpoch()) ||
        msg.sender == governanceAddress(),
      "Votes can only be submitted within the allowed timeframe window"
    );
    if (isClearVoteRequired) {
      require(
        IVoter(controller.voter()).usedWeights(tokenId) == 0,
        "Clear votes before submitting"
      );
    }
    (address[] memory poolsAddresses, int256[] memory votes) = prepareVote();
    voterProxy.vote(poolsAddresses, votes);
    emit votesSubmitted(poolsAddresses, votes);
  }
  ...
  function clearVotesOnDystopia() external {
    ...
    require(dystopiaVotesLastCleared + window < nextEpoch(), "Epoch vote already cleared");
  ...
  function setEcosystemPools(
    ...
    require(_ecosystemPools.length == _ecosystemPoolWeights.length, "invalid input lengths");
    ...
    for (uint256 i; i < _ecosystemPools.length; i++) {
      ...
      ecosystemPoolMinWeight[_ecosystemPools[i]] = _ecosystemPoolWeights[i];
    }
   ...
   */
  function setUpOnlyPool(address poolAddress, bool status) external onlyGovernance {
  ...
  function votes(uint256 length) public view returns (Vote[] memory) {
    ...
    for (uint256 weightIndex; weightIndex < uniqueVotesLength; weightIndex++) {
      // For every vote in the weight node
      uint256 votesLengthForWeight = votesLengthByWeight[currentWeight];
      for (uint256 voteIndex; voteIndex < votesLengthForWeight; voteIndex++) {
   ...
   */
  function votesByAccount(address accountAddress) public view returns (Vote[] memory) {
    uint256 votesLength = votesLengthByAccount[accountAddress];
   ...
   */
  function setMaxPoolsLength(uint256 _maxPoolsLength) external onlyGovernance {
   ...
   */
  function setMaxVotesPerAccount(uint256 _maxVotesPerAccount) external onlyGovernance {
  ...
  event votesSubmitted(address[] poolAddresses, int256[] weights);
  event votesReset();
}
