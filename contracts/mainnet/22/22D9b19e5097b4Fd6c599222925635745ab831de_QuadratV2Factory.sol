...
import {IQuadratV2} from "./interfaces/IQuadratV2.sol";
import {QuadratV2FactoryStorage} from "./abstract/QuadratV2FactoryStorage.sol";
import {InitializePayload} from "./structs/SQuadratV2.sol";
import {_getTokenOrder, _append} from "./functions/FQuadratV2Factory.sol";

/// @title QuadratV2Factory factory for creating vault instances.
contract QuadratV2Factory is QuadratV2FactoryStorage {
    using EnumerableSet for EnumerableSet.AddressSet;

    // solhint-disable-next-line max-line-length
    constructor(IBeacon beacon_) QuadratV2FactoryStorage(beacon_) {} // solhint-disable-line no-empty-blocks

    /// @notice Deploys an instance of Vault using BeaconProxy or TransparentProxy.
    /// @param params_ contains all data needed to create an instance of QuadratV2 vault.
    /// @param isBeacon_ boolean, if true the instance will be BeaconProxy or TransparentProxy.
    /// @return vault the address of the Quadrat V2 vault instance created.
        ...
        returns (address vault)
    {
        ...
        _vaults.add(vault);
        _trustedVaults[vault] = true;
        emit VaultCreated(msg.sender, vault);
    }

    // #region public external view functions.

    /// @notice get Quadrat V2 standard token name for two corresponding tokens.
    /// @param token0_ address of the first token.
    /// @param token1_ address of the second token.
    /// @return name name of the Quadrat V2 vault.
        ...
        returns (string memory)
    {
        ...
        return _append("QuadratV2 ", symbol0, "/", symbol1);
    }
        ...
        returns (address[] memory)
    {
        ...
        require(endIndex_ <= numVaults(), "EI");
    ...
    function numVaults() public view override returns (uint256 result) {
        return _vaults.length();
    }

    /// @notice isVault checks for _vault
    /// @return returns true if _vault was created by the factory
    function isVault(address _vault) public view override returns (bool) {
        return _vaults.contains(_vault);
    }

    /// @notice verifies is vault is trsuted
    /// @param vault address to verify
    /// @return true if trusted otherwise false
    function isTrustedVault(address vault) external view override returns (bool) {
        return _trustedVaults[vault];
    }
        ...
        returns (address vault)
    {
        ...
        string memory name = "Quadrat V2";
        try this.getTokenName(token0, token1) returns (string memory result) {
            name = result;
        } catch {} // solhint-disable-line no-empty-blocks

        bytes memory data = abi.encodeWithSelector(
            IQuadratV2.initialize.selector,
            name,
            string(abi.encodePacked("QuadratV2-", _uint2str(numVaults() + 1))),
            params_
        );
...
import {IQuadratV2Factory} from "../interfaces/IQuadratV2Factory.sol";
import {
...
/// @title Quadrat Factory Storage Smart Contract
// solhint-disable-next-line max-states-count
abstract contract QuadratV2FactoryStorage is
    IQuadratV2Factory,
    OwnableUpgradeable
{
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    mapping(address => bool) internal _trustedVaults;

    event VaultTrustToggled(address pool, bool isTrusted);
    ...
    function upgradeVaults(address[] calldata vaults_) external onlyOwner {
    ...
    /// @param vaults_ the list of vault.
    /// @dev only callable by owner
    function makeVaultsImmutable(address[] calldata vaults_)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < vaults_.length; i++) {
    ...
    /// @param proxy instance of Quadrat V2.
    /// @return admin address of Quadrat V2 instance admin.
    ...
    /// @param proxy instance of Quadrat V2.
    /// @return implementation address of Quadrat V2 implementation.
    ...
    function toggleTrustedVaults(address[] calldata vaults) external onlyOwner {
        for (uint256 i = 0; i < vaults.length; ) {
            address vault = vaults[i];
            bool isTrusted = !_trustedVaults[vault];
            _trustedVaults[vault] = isTrusted;
            emit VaultTrustToggled(vault, isTrusted);
            unchecked {i++;}
        }
    }
}
...
import {IERC20, IQuadratV2Storage} from "../interfaces/IQuadratV2Storage.sol";
import {InitializePayload, Range, Rebalance} from "../structs/SQuadratV2.sol";

interface IQuadratV2 is IERC20, IQuadratV2Storage {
    function initialize(
...
interface IQuadratV2Factory {
    ...
    function upgradeVaults(address[] calldata vaults_) external;
    ...
    function makeVaultsImmutable(address[] calldata vaults_) external;

    function toggleTrustedVaults(address[] calldata pools) external;
    ...
    function isTrustedVault(address vault) external view returns (bool);
...
import {Range} from "../structs/SQuadratV2.sol";

interface IQuadratV2Storage {
