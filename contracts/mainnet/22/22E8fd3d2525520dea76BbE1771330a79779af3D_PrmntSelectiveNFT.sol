...
contract PrmntSelectiveNFT is ERC721Base{
    ...
    function setSourceAddress( address _address ) public {
        sourceAddress = _address;
    }
    
    function setParams(uint _index, uint _hue, bytes32  _projectId, string calldata _scoreType, address  _targetWallet) external  {
    ...
    function getParams(uint _index) external view returns (Params memory params ) {
        return paramsMap[_index];
    }
    
    function claim(uint256 tokenId, bytes32  _projectId, string calldata _scoreType, address  _targetWallet ) public {
        paramsMap[tokenId] = Params({
            hue: tokenId,
            projectId: _projectId,
            scoreType: _scoreType,
            targetWallet: _targetWallet
        });
        string memory tempURI = getURI(tokenId);
    ...
    function getURI(uint256 tokenId) public view  returns (string memory){
        string memory output = string.concat('data:application/json;base64,', 
            Base64.encode(
            abi.encodePacked(
                '{"name": "PRMNT XP NFT #', TWStrings.toString(tokenId), ', "description": "This is a dynamic NFT reflecting the current XP score of the owner of this NFT", "image": "data:image/svg+xml;base64,', 
                Base64.encode(
                    bytes(getSvg(tokenId))
                ),
                '"}'
            )
            )
