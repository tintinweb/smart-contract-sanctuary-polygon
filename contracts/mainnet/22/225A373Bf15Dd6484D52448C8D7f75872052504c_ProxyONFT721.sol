...
contract ProxyONFT721 is ONFT721Core, IERC721Receiver {
    ...
    // IERC721 does not work as tokenURI is not exposed
    ...
    constructor(address _lzEndpoint, address _proxyToken) ONFT721Core(_lzEndpoint) {
        ...
        token = IERC721Metadata(_proxyToken);
    }
    ...
    function _debitFrom(address _from, uint16, bytes memory, uint _tokenId) internal virtual override returns (string memory) {
        require(_from == _msgSender(), "ProxyONFT721: owner is not send caller");
        token.safeTransferFrom(_from, address(this), _tokenId);
        // override here to return the tokenURI
        return token.tokenURI(_tokenId);
    }

    function _creditTo(uint16, address _toAddress, uint _tokenId, string memory) internal virtual override {
