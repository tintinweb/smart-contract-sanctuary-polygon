...
contract PolySmurfsNFTStaking is Ownable {
    ...
    uint256 constant public UNSTAKE_COOLDOWN = 2 days;
    uint256 public initialReward = 50 * (10 ** 18);
    uint256 public rewardRaiseStep = 5 * (10 ** 18);
    uint256 public maxReward = 70 * (10 ** 18);
    ...
    uint256 public STOLEN_PERIOD = 28800;
    uint256 public STOLEN_PERCENT = 30;
    uint256 public STOLEN_CHECKPOINT;
    ...
    StolenInfo[] internal stolenInfo;
    ...
    constructor(address _nftAddress, address _rewardTokenAddress) {
        ...
        limitClaimValue = 56000000 * (10 ** 18);
        isFinished = false;
        STOLEN_CHECKPOINT = block.timestamp;
    }
    ...
    function changeInitialReward(uint256 _initialReward) public onlyOwner {
        initialReward = _initialReward;
    }

    function changeRewardRaiseStep(uint256 _rewardRaiseStep) public onlyOwner {
        rewardRaiseStep = _rewardRaiseStep;
    }
    ...
    function setStolenPeriod(uint256 _period) public onlyOwner {
        STOLEN_PERIOD = _period;
    }

    function setStolenPercent(uint256 _percent) public onlyOwner {
        STOLEN_PERCENT = _percent;
    }
    ...
    function pendingReward(address _user, uint256 _tokenId) public view returns (uint256) {
        ...
        uint256 spendDay = currentBlock.sub(_checkPoint).div(TIME_STEP);
        uint256 remainTime = currentBlock.sub(_checkPoint).mod(TIME_STEP);
        uint256 rewardAmount = 0;
        uint256 dailyReward = initialReward;
        uint256 day = 0;

        for (day = 0; day < spendDay; day ++){
            if (day > 4) {
                rewardAmount = rewardAmount + dailyReward;
            } else {
                dailyReward = getDailyReward(dailyReward);
                rewardAmount = rewardAmount + dailyReward;
            }
        }
        rewardAmount = rewardAmount + dailyReward.mul(remainTime).div(TIME_STEP);

        return rewardAmount;
    }

    function getDailyReward (uint256 _dailyReward) internal view returns(uint256) {
        uint256 random = _getRandom(2);
        if (random == 0) {
            return _dailyReward.add(rewardRaiseStep);
        } else {
            return _dailyReward.sub(rewardRaiseStep);
        }
    }
    ...
    function pendingTotalReward(address _user) public view returns(uint256) {
        ...
        for (uint256 i = 0; i < users[_user].stakedInfo.length; i++) {
            uint256 temp = pendingReward(_user, users[_user].stakedInfo[i].tokenId);
            pending = pending.add(temp);
        }
        return pending.sub(users[_user].stolenReward);
    }


    //For Stolen Berry
    function _getRandom(uint256 _value) private view returns(uint) {
        uint256 random = uint256(keccak256(abi.encode(block.timestamp,_value, block.difficulty)));
        return uint256 (random.mod (_value));
    }

    function stolenBerry(address _user) private returns (uint256) {
        uint256 totalReward = pendingTotalReward(_user);
        uint256 _stolenReward = totalReward.mul(STOLEN_PERCENT).div(100);
        users[_user].stolenReward = _stolenReward;

        return _stolenReward;
    }

    function runStolen() public {
        if (block.timestamp > STOLEN_CHECKPOINT.add(STOLEN_PERIOD)){
            uint256 random = _getRandom(userList.length);
            address randomUser = userList[random];
            if (randomUser != address(0)) {
                uint256 stolenAmount = stolenBerry(randomUser);
                STOLEN_CHECKPOINT = STOLEN_CHECKPOINT.add(STOLEN_PERIOD);
                stolenInfo.push(StolenInfo(STOLEN_CHECKPOINT,randomUser, stolenAmount));
            }
        }
    }

    function getStolenInfo (uint256 _index) public view returns(uint256 _checkPoint, address _user, uint256 _amount) {
        _checkPoint = stolenInfo[_index].checkPoint;
        _user = stolenInfo[_index].stolenUser;
        _amount = stolenInfo[_index].stolenAmount;
    }
    ...
    function stake(uint256[] memory tokenIds) public {
        ...
        if (block.timestamp.sub(STOLEN_CHECKPOINT) >= STOLEN_PERIOD) {
            runStolen();
        }
    ...
    function unstake(uint256[] memory tokenIds) public {
        ...
        uint256 pending = 0;
        for(uint256 i = 0; i < tokenIds.length; i++) {
            (bool _isStaked, uint256 _checkPoint) = getStakingItemInfo(msg.sender, tokenIds[i]);
            if(!_isStaked) continue;
            if(IERC721(nftAddress).ownerOf(tokenIds[i]) != address(this)) continue;

            require (_checkPoint.add(UNSTAKE_COOLDOWN) > block.timestamp,"Can't Unstake in 2 days after staking!");
        ...
        if(pending > 0) {
            if (pending < users[msg.sender].stolenReward) {
                users[msg.sender].stolenReward = users[msg.sender].stolenReward.sub(pending);
                pending = 0;
            } else {
                pending = pending.sub(users[msg.sender].stolenReward);
                users[msg.sender].stolenReward = 0;
            }

            if (totalClaimed.add(pending) >= limitClaimValue)
            {
                pending = limitClaimValue.sub(totalClaimed);
                isFinished = true;
            }
            IERC20(rtoken).transfer(msg.sender, pending);
            users[msg.sender].withdrawn = users[msg.sender].withdrawn.add(pending);
    ...
    function claim() public {
        ...
        if (totalClaimed.add(reward) >= limitClaimValue)
            {
                reward = limitClaimValue.sub(totalClaimed);
                isFinished = true;
            }     
        IERC20(rtoken).transfer(msg.sender, reward);
        users[msg.sender].withdrawn = users[msg.sender].withdrawn.add(reward);
