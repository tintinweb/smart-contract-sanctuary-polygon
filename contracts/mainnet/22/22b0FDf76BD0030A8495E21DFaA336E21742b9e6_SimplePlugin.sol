...
contract SimplePlugin is IPlugin, Ownable, Checkpointable {
    ...
    ************************************************/
    ...
    function claim(address account, address to, bytes calldata) external override onlyStaking returns (uint256) {
        if (_claimable[account] <= 0) {
            return 0;
        }

        uint256 amt = _claimable[account];
    ...
    function notifyStakeChange(address, uint256, uint256) external override pure {} // if this function did anything, it would have onlyStaking modifier
    ...
    ************************************************/
    ...
    function increaseClaimableBy(address account, uint256 amount) external onlyIncreaser returns (bool) {
        ...
        _totalOwed += amount;
        _writeCheckpoint(account, _claimable[account]);

        emit ClaimableIncreased(account, _claimable[account] - amount, _claimable[account]);
...
contract StakingModule is ReentrancyGuardUpgradeable, AccessControlEnumerableUpgradeable, PausableUpgradeable, Checkpointable {
    ...
    ************************************************/
    ...
    /// @notice Claims yield and withdraws all stake.
    /// @param auxData Auxiliary data for the plugins
    /// @return Amount claimed
    /// @return Amount withdrawn
    ...
    ************************************************/
    ...
    function _stake(address account, address from, uint256 amount) private {
        ...
        // notify plugins
        _notifyStakeChangeAllPlugins(account, _stakes[account], _stakes[account] + amount);
    ...
    /// @dev 2. If the user had some stake, call `notifyStakeChange` on all plugins
    /// @dev 3. Write COMP-like checkpoint
    /// @dev 4. Transfer `amount` of tokens to `to`
    /// @dev 5. Emit StakeChanged
    ...
    /// @dev Calls `notifyPartialExit` on all plugins that require it. This is done in case any given plugin needs to do some stuff when a user exits.
