    ...
    ISwapVault
{
    ...
    */
        ...
        )
    {
        ...
        if (isSuccess) {
            uint256 totalAllowedAmount = IERC20Metadata(tempPool.assetFrom)
            ...
            if (!isSuccess) {
                ...
                emit Withdrawn(
                    getPoolId(tempPool.assetTo, tempPool.assetFrom),
                    requestId,
                    allowanceAssetTo
                );
            } else {
    ...
    */
        ...
        onlyDuringPoolState(swapRequests[requestId].poolId, PoolStatus.UNLOCKED)
    {
        ...
        uint256 totalAllowendAmount = IERC20Metadata(assetAddress).allowance(
            address(this),
            msg.sender
        ) + _amount;
        require(
        ...
        emit Withdrawn(poolId, requestId, totalAllowendAmount);
    }
    ...
    */
        ...
        onlyDuringPoolState(poolId, PoolStatus.LOCKED)
    {
        ...
        pools[oppPoolId].midSwapRate =
            (SPOT_RATE_MULTIPLIER * SPOT_RATE_MULTIPLIER) /
            rate;
        emit SetMidSwapRate(poolId, rate, pools[oppPoolId].midSwapRate);
    }
    ...
    */
    ...
    /** 
        @notice function to distribute funds one by one for each request ID manually
        Should be called for all request Ids in case one of the TrufinVaults starts 
        throwing errors when receiveSwappedFunds is called and distributeFundsAutomatically 
        cannot delete the requestId.
