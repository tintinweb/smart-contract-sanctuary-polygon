...
contract API {
    ...
    event NewUpdate(address indexed token, string ipfsHash);

    event NewAssetListing(Token token);
    event NewAssetUpdate(Token token);
    ...
    function addStaticData(
        ...
        bool isNewListing = assetById[assetId].id == 0;
    ...
    function addAssetData(Token memory token) external {
        ...
        bool isNewListing = assetById[token.id].id == 0;
        assets.push(token);
