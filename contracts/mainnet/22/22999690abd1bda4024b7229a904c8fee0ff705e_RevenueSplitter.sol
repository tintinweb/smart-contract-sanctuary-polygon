...
interface IRevenueSplitter {
    ...
    // Token $$ Events
    // a2fc0bd51a5bc3589d1bbf031bceaabcd269febe30c17d6a0d84255c586004e1
    event BuyEvent(
        address indexed collection,
        uint256 tokenId,
        uint256 amount,
    ...
    AccessControlUpgradeable
{
    ...
    function computeFee(
        ...
        if (communityRegistry != address(0)) {
            IREMXCommunityRegistry registry = IREMXCommunityRegistry(
                communityRegistry
            );
            return registry.computeFee(collection, value);
        }
        return 0;
    }
    ...
    function buyNFT(
        ...
        // fetch fee (if any) for collection
        uint256 fee = computeFee(_collection, msg.value);

        _distributeFunds(_collection, msg.value, fee);
        emit BuyEvent(_collection, tokenId, msg.value, fee);
    ...
    function depositRoyalty(address _collection) external payable override {
        ...
        _distributeFunds(_collection, msg.value, 0);
    }
    ...
    function _distributeFunds(
        ...
        if (fee > 0) {
            require(fee < amount, "RMX: fee too high");
            _balances[feeReceiver] += fee;
            amount -= fee;
            emit DepositEvent(_collection, feeReceiver, fee);
        }
