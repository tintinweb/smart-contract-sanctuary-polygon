 ...
 */
contract ERC1155deshareInternal is Ownable, AccessControl, ERC1155Supply {
    ...
    modifier onlyMinter() {
        ...
        require(
            canIMint(),
            "ERC1155deshare: must have minter role to mint"
        );
        _;
    }
...
/**
 * @title ERC1155deshare
 * ERC-721 Marketplace with tokens and royalties support
 */
contract ERC1155Marketplace is ERC1155deshareInternal, ReentrancyGuard {
    ...
    constructor(
        ...
        ERC1155deshareInternal(_owner, uri, name, symbol, _anyoneCanMint)
    {
        require(_commissionRate + _royaltiesCommissionRate < 100, "deshare: total commissions should be lower than 100");
        admin = _admin;
    ...
    function sell(uint256 tokenId, uint256 qty, uint256 price, address wallet) external {
        ...
        // Limit the sell to owned items, the user cannot sell items do not have
        require(balanceOf(_msgSender(), tokenId) >= qty, "deshare: you do not have enough tokens to sell");
    ...
    function buy(uint256 tokenId, uint256 index, uint256 qty) external nonReentrant {
        ...
        // validate user
        require(openSales[tokenId][index].user != _msgSender() , "deshare: the user cannot buy his own offer");
    ...
    function cancelSale(uint256 tokenId, uint256 index) external {
        ...
        require(openSales[tokenId][index].user == _msgSender(), "deshare: only the original seller can cancel his sales");
    ...
    function bid(uint256 tokenId, uint256 qty, uint256 price) external nonReentrant {
        ...
        require(qty>0, "deshare: qty has to be positive");
        require(price>0, "deshare: price has to be positive");
        require(qty <= totalSupply(tokenId), "deshare: not enough items for sale");
    ...
    function cancelBid(uint256 tokenId, uint256 index) external nonReentrant {
        ...
        // Only the original bidder can cancel his bids
        require(openOffers[tokenId][index].user == _msgSender(), "deshare: only the original bidder can cancel his bids");
    ...
    function acceptBid(uint256 tokenId, uint256 index, uint256 qty) external nonReentrant {
        ...
        // validate user
        require(openOffers[tokenId][index].user != _msgSender() , "deshare: the user cannot accept his own bid");
    ...
    function callOptionalReturn(IERC1155 token, bytes memory data) private {
        ...
        require(address(token).isContract(), "deshare: call to non-contract");

        // solhint-disable-next-line avoid-low-level-calls
        (bool success, bytes memory returndata) = address(token).call(data);
        require(success, "deshare: low-level call failed");

        if (returndata.length > 0) { // Return data is optional
            // solhint-disable-next-line max-line-length
            require(abi.decode(returndata, (bool)), "deshare: ERC20 operation did not succeed");
        }
    }
 ...
 */
contract ERC1155deshare is ERC1155Marketplace, EIP712 {
    string private constant SIGNING_DOMAIN = "ERC1155deshare-Voucher";
    ...
    function redeem(
        ...
        require(chainId == getChainId(), "deshare: Wrong chain");
        require(vouchersMintedQty[id] + qtyToMint <= qty, "deshare: the qty exceeds the amount available for minting");
        require(nftId == 0 || exists(nftId), "deshare: Wrong NFT ID");
    ...
    function verify(
        ...
        require(recoveredSigner == signer, "deshare: wrong signature");
    }
