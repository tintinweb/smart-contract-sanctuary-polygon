...
contract CoolZoo is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        ERC721A("CoolZoo", "CoolZoo", _maxBatchSize, _collectionSize)
    {
    ...
    function reserve(address recipient, uint256 tokenQuantity) external onlyOwner {
        require(recipient != address(0), "CoolZoo: zero address");
        require(tokenQuantity > 0, "CoolZoo: invalid amount");
        require(
            totalSupply() + tokenQuantity <= collectionSize,
            "CoolZoo: Exceed max supply"
        );
        require(
            totaltokensReserved + tokenQuantity <= maxreserveAmount,
            "CoolZoo: Exceed max reserve"
        );
    ...
    function mint( uint256 tokenQuantity ) external payable {
        require(status == Status.PublicSale, "CoolZoo: Public mint is not active");
        require(
            tx.origin == msg.sender,
            "CoolZoo: contract is not allowed to mint."
        );
        require(
            numberMinted(msg.sender) + tokenQuantity <= MAX_MINT,
            "CoolZoo: Max public mint amount per wallet exceeded"
        );
        require(
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "CoolZoo: URI query for nonexistent token"
        );
    ...
    function refundIfOver(uint256 price) private {
        require(msg.value >= price, "CoolZoo: Not enough ether sent");
        if (msg.value > price) {
