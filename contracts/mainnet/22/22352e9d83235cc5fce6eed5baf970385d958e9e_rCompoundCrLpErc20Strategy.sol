...
// File: default_workspace/contracts/IrOracle.sol
...
// File: default_workspace/contracts/IrVault.sol
...
// File: default_workspace/contracts/rStrategy.sol
...
// File: default_workspace/contracts/IRoyaltyStaking.sol
...
// File: default_workspace/contracts/CompoundLPERC20Strategy.sol
...
/**
 * @author Oighty (h/t to Nat Eliason for authoring the RoyaltyStaking contract). Please contact me on Twitter (@oightytag) or Discord (Oighty #4287) if you have any questions about this contract.
 */
...
contract rCompoundCrLpErc20Strategy is rStrategy {
    ...
    //  -------------------- CONTRACT VARIABLES --------------------
    // Tokens and Contracts to Interface with
    IRoyaltyStaking internal _stakingContract; // The staking contract that the vault will compound into
    ...
    {
        // Assign initial variable values        
        _stakingContract = IRoyaltyStaking(_stakingContractAddr);
    ...
    function compound() external payable whenNotPaused nonReentrant onlyRole(MANAGER_ROLE) {
        ...
        if (address(_rewardToken) == address(_pairedToken)) { // Royalty/(ERC20) LP Case
            ...
            // Get the current price from the exchange
            (reserveA, reserveB, ) = _stakingToken.getReserves(); // Token A is the base token in all of the Royalty Pairs and Token B is the _pairedToken
