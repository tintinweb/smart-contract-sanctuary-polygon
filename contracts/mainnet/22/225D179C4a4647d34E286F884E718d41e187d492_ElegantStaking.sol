...
contract ElegantStaking is Ownable, Concatenate {
    ...
    function setBoost(uint _boost, uint _tokenID) public {
        ...
        require(walletToVC[msg.sender] != _tokenID, "Your anchor, captain, and vice captain, must have different IDs.");
        require(walletToAnchor[msg.sender] != _tokenID, "Your anchor, captain, and vice captain, must have different IDs.");
        uint nutso;
        if (_boost == 1){
            nutso = 125 ether;
            walletToAnchor[msg.sender] = _tokenID;
        } else if (_boost == 2){
            nutso = 50 ether;
            walletToVC[msg.sender] = _tokenID;
        }
    ...
    function addRacer(uint _tokenID, address _con, uint _event) internal { 
        bool ownership;
        if (_con == erc721Contract){
            address _own = stakeableNFT.ownerOf(_tokenID);
            ownership = (_own == msg.sender);
        } else if ( _con == customAddress) {
            uint v = customContract.balanceOf(msg.sender,_tokenID);
            ownership = (v > 0);
        }
        ...
        if (_event > 0 && _event < 18){
            tierFilled[_week][msg.sender][_event] = _tokenID;
        }
    ...
    function signUp (uint[] calldata _racers, address _con, uint _event) public payable {
        ...
        if (_event > 0 && _event < 18){
            require(tierFilled[_week][msg.sender][_event] == 0, "You already have a racer in that tier.");
        }
        ...
        for(uint i=0; i<_count; i++){
            addRacer(_racers[i], _con, _event);
            bool answer = RulesInterface(rulesByRace[_event].conCheck).tokenCheck(_racers[i],rulesByRace[_event].conValue,_con);
    ...
    function buildRace(uint _event) internal {
        ...
        for(i; i<racing; i++){ //go through every racer
            ...
            address _owner = stakeableNFT.ownerOf(_rID);

            uint boost = boostCheck(_owner, _rID);
    ...
    function boostCheck(address _owner, uint _ID) internal returns(uint){
        uint cpt = AccountInterface(accounts).getCaptain(_owner);
        uint boost;
        if (cpt == _ID){
            boost = 2;
        } else if (_ID == walletToVC[_owner]){
            boost = 1;
        } else if (_ID == walletToAnchor[_owner]){
            boost = 4;
        }
        return boost;
    }
