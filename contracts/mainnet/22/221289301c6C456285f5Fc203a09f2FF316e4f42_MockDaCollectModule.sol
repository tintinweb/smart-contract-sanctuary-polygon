...
contract MockDaCollectModule is ICollectModule, MetaTxReceiver {
    error OffchainLookup(address sender, string url, bytes callData);
    ...
    function initializePublicationCollectModule(uint256 _profileId, uint256 _pubId, bytes calldata data) external returns (bytes memory) {
        return data;
    }

    function processCollect(uint256 referrerProfileId, address collector, uint256 profileId, uint256 pubId, bytes calldata data) external view {
        if (data.length == 0) {
            revert OffchainLookup(
                address(this),
                 "https://phat-gateway.phala.network/0x00000000deadbeef/lensapi/post/{data}",
                abi.encode(profileId, pubId)
            );
        }

        // Validate attestation from the DA oracle
        ...
        // Verify the collect target exists
        (uint256 attestedProfileId, uint256 attestedPubId)  = abi.decode(metaTx.data, (uint256, uint256));
        require(profileId == attestedProfileId && pubId == attestedPubId, "Invalid DA publication proof of exist");
    }
}