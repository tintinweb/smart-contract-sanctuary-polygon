...
contract DaiVaultAction is BaseSiloAction {
    ...
    function exit(
        ...
        if (inputAmounts[0] == 0) {
            //remove entire stake
            uint256 amount = (user.maxDaiDeposited * DaiVault.maxWithdrawP()) / 100;

            DaiVault.withdrawDai(amount);
            ...
            if (
                inputAmounts[0] >
                (user.maxDaiDeposited * DaiVault.maxWithdrawP()) / 100
            ) {
                inputAmounts[0] =
                    (user.maxDaiDeposited * DaiVault.maxWithdrawP()) /
                    100; //withdraw max withdrawable
            }
            DaiVault.withdrawDai(inputAmounts[0]); ///@notice withdrawn amount is "dropped" from outputAmounts
            outputAmounts[0] = _takeFee(implementation, reward, output[0]);
        }
    }
    ...
    function decodeConfig(bytes memory configData) public pure returns(address[4] memory inputs, address[4] memory outputs,address farm, uint256 trigger){
        (inputs,  outputs, farm, trigger) = abi.decode(configData, (address[4], address[4], address, uint256));
    }
    ...
    function showDust(address _silo, bytes memory _configurationData) external view override returns(address[] memory, uint[] memory){
        ...
        tokens[0] = address(storageT.dai());
        uint[] memory balances = new uint[](1);
        balances[0] = storageT.dai().balanceOf(_silo);
...
abstract contract BaseSiloAction is Ownable {
    ...
    function canExit(address _silo, bytes memory _configurationData) public view virtual returns(bool){
        return true;
    }

    function withdrawAmount(address _silo, bytes memory _configurationData) public view virtual returns(uint){
        return 0;
    }
