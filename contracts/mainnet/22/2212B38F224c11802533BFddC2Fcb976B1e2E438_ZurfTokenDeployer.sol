...
contract ZurfTokenDeployer is Ownable {
    ...
    constructor(address _usdtAddress, uint256 _feeAmount, address payable _vaultAddress) {
        i_usdtAddress = _usdtAddress;
    ...
    function deployToken(string calldata _name, string calldata _ticker, uint256 _supply) external returns (address) {
        ...
        IERC20(i_usdtAddress).transferFrom(
            msg.sender,
            address(this),
            feeAmount
        );
    ...
    function approveTransfer() external {
        IERC20(i_usdtAddress).approve(address(this), feeAmount);
    }
    ...
    function withdrawUSDT() external onlyOwner {
       uint256 balance = IERC20(i_usdtAddress).balanceOf(address(this));
        IERC20(i_usdtAddress).transfer(msg.sender, balance);
    }
}