...
contract RelayPolygon is Ownable, ReentrancyGuard {
    ...
    /////////////////// takes matic dep from users balance and swaps ///////////////////////
    ...
    function swapTokensForToken(
        ...
        ApproveTokensMesh(tokenIn);
        ApproveTokensMesh(tokenOut);
        address[] memory path = new address[](2);
        path[0] = tokenIn;
        path[1] = tokenOut;
    ...
    function swapTokens(address tokenIn, address tokenOut) public nonReentrant {
        ...
        emit swapTokensComplete(tokenIn, tokenOut, msg.sender);
    }

    
    
    
    ////////////////////   swap tokens and send to address /////////////////////////


    function swapTokensTo(
        address tokenIn,
        address tokenOut,
        address to
    ) public nonReentrant {
    ...
    function ApproveTokensMesh(address _tokenAddress) public onlyOwner {
    ...
    function ApproveTokens(address _tokenAddress, address _spender)
        public
        onlyOwner
    {
        IERC20(_tokenAddress).approve(_spender, type(uint256).max);
    }
}