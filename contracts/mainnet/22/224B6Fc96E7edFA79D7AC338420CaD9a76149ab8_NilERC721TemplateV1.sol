...
import "./interfaces/INilERC721V1.sol";
    ...
    IERC2981Upgradeable
{
    ...
    function initialize(
        ...
        __NilRoles_init(operator_, contractAddresses_.dao, contractAddresses_.signer);
    ...
    function mintETH(bytes calldata data) external payable virtual nonReentrant nonContractCaller {
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            seeds[currentTokenId] = keccak256(
                abi.encodePacked(currentTokenId, block.number, blockhash(block.number - 1), msg.sender)
            );
            currentTokenId++;
        }
    ...
    function mintNIL(bytes calldata data) external virtual nonReentrant nonContractCaller {
        require(publicSaleOpen(), "NIL:PUBLIC_SALE_NOT_OPEN");
        require(nftParameters.isNilSale, "NIL:ONLY_POSSIBLE_TO_PAY_WITH_NIL");
        (uint256 amount, uint256 expiry, uint256 random, bytes memory signature) = abi.decode(
            data,
            (uint256, uint256, uint256, bytes)
        );
        ...
        contractAddresses.nil.transferFrom(msg.sender, address(this), amount * nftParameters.priceInWei);
        usedVouchers[signature] = true;
    }

    function mintWithNETH(uint256[] calldata tokenIds, bytes calldata data) external payable virtual nonReentrant nonContractCaller {
        require(nftParameters.nAllowance > 0 && nSaleOpen(), "NIL:N_SALE_NOT_OPEN");
        require(!nftParameters.isNilSale, "NIL:ONLY_POSSIBLE_TO_PAY_WITH_ETH");
        (uint256 amount, uint256 expiry, uint256 random, bytes memory signature) = abi.decode(
            data,
            (uint256, uint256, uint256, bytes)
        ...
        require(totalNMintsAvailable() >= amount, "NIL:MAX_ALLOCATION_REACHED");
        require(amount * nftParameters.priceInWei == msg.value, "NIL:INVALID_PRICE");
        for (uint256 i = 0; i < amount; i++) {
            require(contractAddresses.nOwnerResolver.ownerOf(tokenIds[i]) == msg.sender, "NIL:NOT_N_OWNER");
        }
        for (uint256 i = 0; i < amount; i++) {
            require(!usedN[tokenIds[i]], "NIL:N_ALREADY_USED");
            _safeMint(msg.sender, currentTokenId);
            seeds[currentTokenId] = keccak256(
                abi.encodePacked(currentTokenId, block.number, blockhash(block.number - 1), msg.sender)
            );
    ...
    function mintWithNNIL(uint256[] calldata tokenIds, bytes calldata data) external virtual nonReentrant nonContractCaller {
        require(nftParameters.nAllowance > 0 && nSaleOpen(), "NIL:N_SALE_NOT_OPEN");
        require(nftParameters.isNilSale, "NIL:ONLY_POSSIBLE_TO_PAY_WITH_NIL");
        (uint256 amount, uint256 expiry, uint256 random, bytes memory signature) = abi.decode(
            data,
    ...
    function burnRemainingSupply(bool setting) public onlyOperator {
    ...
    // Hack for OpenSea collection editing
    function transferOwner(address newOwner) public onlyOperator {
        owner_ = newOwner;
    }

    function setRoyalties(address payout, uint256 amount) public onlyOperator {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(nftParameters.metadataUri, "?nilSeed=", getSeed(tokenId)));
    }

    function setMetadataURI(string calldata metadataUri) public onlyAdmin {
        nftParameters.metadataUri = metadataUri;
    }

    function withdrawProceeds() public {
        if (nftParameters.isNilSale) {
            contractAddresses.nil.transfer(owner_, contractAddresses.nil.balanceOf(address(this)));
        } else {
            uint256 contractBalance = address(this).balance;
            payable(owner_).transfer((contractBalance * (1000 - protocolFeesInBPS)) / 1000);
            //sent 3.5% to DAO
    ...
    function canMint(address account) public view virtual override returns (bool) {
        ...
        uint256 nBalance = contractAddresses.nOwnerResolver.balanceOf(account);
        if (publicSaleOpen() && totalMintsAvailable() > 0 && balance < nftParameters.maxMintablePerAddress) {
            return true;
        }
        if (
            nSaleOpen() && nBalance > 0 && totalNMintsAvailable() > 0 && balance < nftParameters.maxMintablePerAddress
        ) {
            return true;
        }
        return false;
...
interface INilERC721V1 is IERC721EnumerableUpgradeable {
    ...
    struct ContractAddresses {
        INil nil;
        address dao;
        INOwnerResolver nOwnerResolver;
        address signer;
    }

    function mintETH(bytes calldata data) external payable;

    function mintNIL(bytes calldata data) external;

    function mintWithNETH(uint256[] calldata tokenIds, bytes calldata data) external payable;

    function mintWithNNIL(uint256[] calldata tokenIds, bytes calldata data) external;
...
contract NilRolesUpgradeable is Initializable, AccessControlUpgradeable {
    ...
    function __NilRoles_init(
        ...
        __NilRoles_init_unchained(operator_, admin_, signer_);
    }
