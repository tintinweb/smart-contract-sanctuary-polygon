...
contract Yeller is Ownable {
    ...
    function partInPool(address _gauge) private view returns (uint) {
        ...
        uint wholePool = gauge.totalSupply();
        uint stakerPartInPool = IERC20(rewardPool).balanceOf(address(strategy));
    ...
    function getLpValue() public view returns (uint, uint) {
        ...
        uint lpWorkFromGauge = IERC20(rewardPool).balanceOf(address(strategy));
    ...
    function getBalancesInLp() public view returns(uint256[2] memory){
        address lpToken = strategy.want();
        uint256[2] memory balances = LPTOKEN(lpToken).get_balances();
