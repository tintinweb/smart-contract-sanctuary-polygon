...
contract ICOContract is Owner, ReentrancyGuard {
    ...
    mapping(address => uint256) public totalOrdersOf;
    ...
    function getOrdersOf(address _account) external view returns(uint256[] memory){
        return ordersOf[_account];
    }

    function getMyOrders(address _account) external view returns(uint256[] memory, uint256[] memory, uint256[] memory, bool[] memory){
        uint256[] memory id = new uint256[](totalOrdersOf[_account]);
        uint256[] memory amount = new uint256[](totalOrdersOf[_account]);
        uint256[] memory paidAmount = new uint256[](totalOrdersOf[_account]);
        bool[] memory closed = new bool[](totalOrdersOf[_account]);

        for (uint256 i=0; i<totalOrdersOf[_account]; i++) {
            uint256 orderId = ordersOf[_account][i];
            id[i] = orderId;
            amount[i] = orders[orderId].amount;
            paidAmount[i] = orders[orderId].paidAmount;
            closed[i] = orders[orderId].closed;
        }
        return (id, amount, paidAmount, closed);
    }
    ...
    function buy(uint256 _amount) external nonReentrant{
        ...
        ordersOf[msg.sender].push(newOrderId);
        totalOrdersOf[msg.sender] = totalOrdersOf[msg.sender]+1;
