...
contract VaultV3 is Ownable, Pausable, DividendToken {
	using SafeMath for uint256;
	...
	function deposit(uint256 amount) external whenNotPaused {
		...
		IMasterChefDistribution(distribution).stake(msg.sender, amount);
	...
	function withdraw(uint256 amount) external {
		...
		// Update MasterChefDistribution contract state and distribute unclaimed rewards(if any)
		IMasterChefDistribution(distribution).withdraw(msg.sender, amount);
