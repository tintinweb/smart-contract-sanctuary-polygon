...
contract ZurfPreSale {
    ...
    address public vaultAddress; // Address of the zurf vault
    using SafeMath for uint256;
    using SafeERC20 for IERC20;  // SafeERC20 to transfer tokens.
     // The address of the Zurf multisig contract.
    address public s_multisig;
    
    uint256 public constant TOKEN_PRICE = 1300;  // Price per token in USDT (0.001300 USDT)
    uint256 public constant TOTAL_ZURF_SUPPLY = 20100000;  // Total supply of Zurf tokens for the presale
    ...
    constructor(
        ...
        cliffEnd = block.timestamp + (90 days);  // 90 days cliff period
    ...
    function buyAllocation(uint256 usdtAmount) external {
        ...
        uint256 zurfTokens = (usdtAmount * 1e12) / TOKEN_PRICE;  // Calculate Zurf tokens to allocate
        ...
        allocation.amountBought = allocation.amountBought.add(usdtAmount);
        allocation.allocatedTokens = allocation.allocatedTokens.add(zurfTokens);
        
        if (allocation.allocatedTokens >= 5000000) {
            allocation.vestedDuration = 42;  // Vesting duration of 42 months for large buyers
        } else {
            allocation.vestedDuration = 30;  // Vesting duration of 30 months for other buyers
        }
    ...
    function getLocked(address investor) external view returns (uint256) {
        ...
        uint256 lockedAmount = allocation.allocatedTokens.sub(vestedAmount);
    ...
    function dropTokens() external onlyZurfTrigger {
        for (uint256 i = 1; i <= investorsCount; i++) {
            ...
            uint256 vestedAmount = calculateVestedAmount(investor);
            uint256 claimableAmount = vestedAmount.sub(allocation.claimedTokens);
            
            if (claimableAmount > 0) {
                allocation.claimedTokens = allocation.claimedTokens.add(claimableAmount);
                
                // Transfer the claimable tokens to the investor
                IERC20(token).safeTransfer(investor, claimableAmount);
            }
        }
    }
    
    function getRemainingTokens() public view returns (uint256) {
        return TOTAL_ZURF_SUPPLY.sub(getTotalAllocatedTokens());
    }
    ...
    function getTotalAllocatedTokens() public view returns (uint256) {
        ...
        for (uint256 i = 1; i <= investorsCount; i++) {
            address investor = investorIndex[i];
            total = total.add(allocations[investor].allocatedTokens);
        }
    ...
    function calculateVestedAmount(address investor) public view returns (uint256) {
        ...
        if (block.timestamp < cliffEnd) {
            ...
            elapsedMonths = (block.timestamp.sub(cliffEnd)).div(30 days);
        }
        ...
        uint256 vestedAmount = (allocation.allocatedTokens.mul(elapsedMonths)).div(vestedDuration);
        return vestedAmount;
    }
    ...
    function setVault(address _vaultAddress) external onlyOwner {
        ...
        /**
     * @dev Sets the zurf trigger addresses. This can only be called by the contract owner.
     * @param _zurfTrigger The new zurf trigger address.
     */
