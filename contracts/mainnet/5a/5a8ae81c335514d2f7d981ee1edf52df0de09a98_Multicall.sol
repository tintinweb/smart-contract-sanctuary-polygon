...
contract Multicall {
    ...
    function _multicallRoute(Route calldata route) public returns (uint, address, uint) {
        amOut = route.amIn;
        tOut = route.tIn;
        multicall(route.callPath);
        return (amOut, tOut, gasleft());
    }

    function multicallRoute(Route calldata route) public {
        amOut = route.amIn;
        tOut = route.tIn;
        multicall(route.callPath);
