...
/// @title Assassin's Creed Pieces of Eden Pass
/// @author Terratecc.com
...
contract PiecesOfEdenPass is ERC721Enumerable, Payable, DefaultOperatorFilterer {
    using Strings for uint256;

    uint256 public maxSalePlusOne = 1501;
    uint256 public maxGuaranteed = 100;
    uint256 public maxAllowlist = 1150;

    uint256 public usdPrice = 20000000000 ether; //USD price has 8 decimals (5$ = 500000000)

    uint256 public txLimitPlusOne = 4;

    bytes32 public merkleRootGuaranteed;
    bytes32 public merkleRootAllowlist;

    mapping(address => bool) public guaranteedMint;
    ...
    constructor() ERC721("Assassins Creed Pieces of Eden Pass", "ACPE") {
        placeholderURI = 'ipfs://QmVGQ1JkCUVaBtVGajpxXb5iKHRFnkUfEVYVHYbagUyrAv/';

        priceFeed = AggregatorV3Interface(0xAB594600376Ec9fD91F8e885dADF0CE036862dE0); // 0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419
     ...
     */
    function mint(uint256 quantity, address to, bytes32[] calldata proofGuaranteed, bytes32[] calldata proofAllowlist, bytes32 leaf)
        external
        payable
        correctValue(getPriceEth() * quantity)
    {
        ...
        if (contractState == ContractState.PRESALE1) {
            require(totalSupply() + 1 < maxSalePlusOne, "Exceeds available amount of tokens!");
            if (verify(merkleRootGuaranteed, leaf, proofGuaranteed) && !guaranteedMint[to] && maxGuaranteed > 0) {
                maxGuaranteed--;
                guaranteedMint[to] = true;
                allowlistMint[to] = true;

                uint256 tid = tokenId();
                _safeMint(to, tid);
            }
            else if (verify(merkleRootAllowlist, leaf, proofAllowlist) && !allowlistMint[to] && maxAllowlist > 0) {
                maxAllowlist--;
                allowlistMint[to] = true;
                guaranteedMint[to] = true;
            ...
            require(totalSupply() + 1 < maxSalePlusOne, "Exceeds available amount of tokens!");
            if (verify(merkleRootGuaranteed, leaf, proofGuaranteed) && !guaranteedMint[to]) {
                guaranteedMint[to] = true;
                allowlistMint[to] = true;
            ...
            else if (verify(merkleRootAllowlist, leaf, proofAllowlist) && !allowlistMint[to]) {
                guaranteedMint[to] = true;
                allowlistMint[to] = true;
            ...
            require(totalSupply() + quantity < maxSalePlusOne, "Exceeds available amount!");
     ...
     */
    function setUSDPrice(uint256 newPrice) external onlyOwner {
        usdPrice = newPrice;
    }

    /**
     * Update maximum number of tokens for sale.
     * @param maxSale_ The maximum number of tokens available for sale.
     */
    function setMaxSale(uint256 maxSale_) external onlyOwner {
        uint256 maxSalePlusOne_ = maxSale_ + 1;
        require(maxSalePlusOne_ > totalSupply(), "Must be above current supply!");
        maxSalePlusOne = maxSalePlusOne_;
    }

    /**
     * Update maximum number 
     * @param newMax The maximum number of tokens available for sale.
     */
    function setMaxGuaranteed(uint256 newMax) external onlyOwner {
        maxGuaranteed = newMax;
    }
     ...
     * @param newMax The maximum number of tokens available for sale.
     */
    function setMaxAllowlist(uint256 newMax) external onlyOwner {
        maxAllowlist = newMax;
    }
    ...
    /**
     * Set the guaranteed Merkle root.
    ...
    function setMerkleRootGuaranteed(bytes32 merkleRoot_) external onlyOwner {
        merkleRootGuaranteed = merkleRoot_;
    }
     ...
     * @param merkleRoot_ The new merkle roo
     */
    function setMerkleRootAllowlist(bytes32 merkleRoot_) external onlyOwner {
        merkleRootAllowlist = merkleRoot_;
    }
    ...
    function tokenURI(uint256 tokenId_) public view virtual override returns (string memory) {
        require(_exists(uint16(tokenId_)), "URI query for nonexistent token");

        return
            bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId_.toString(), ".json")) : placeholderURI;
    }
     ...
     *      In this example the added modifier ensures that the operator is allowed by the OperatorFilterRegistry.
     */
    function approve(address operator, uint256 tokenId_) public override(ERC721, IERC721) onlyAllowedOperatorApproval(operator) {
        super.approve(operator, tokenId_);
    }       
     ...
     *      In this example the added modifier ensures that the operator is allowed by the OperatorFilterRegistry.
     */
    function transferFrom(address from, address to, uint256 tokenId_) public override(ERC721, IERC721) onlyAllowedOperator(from) {
        super.transferFrom(from, to, tokenId_);
    }
     ...
     *      In this example the added modifier ensures that the operator is allowed by the OperatorFilterRegistry.
     */
    function safeTransferFrom(address from, address to, uint256 tokenId_) public override(ERC721, IERC721) onlyAllowedOperator(from) {
        super.safeTransferFrom(from, to, tokenId_);
    }
     ...
     *      In this example the added modifier ensures that the operator is allowed by the OperatorFilterRegistry.
     */
    function safeTransferFrom(address from, address to, uint256 tokenId_, bytes memory data)
        public
        override(ERC721, IERC721)
        onlyAllowedOperator(from)
    {
        super.safeTransferFrom(from, to, tokenId_, data);
    }
}