...
contract TheStakingApp is Ownable {
    uint8 constant internal DEV_FEE = 50; // 5% of deposit and withdraw total fee 10%
    ...
    function addPlan(uint32 minimumDays, uint16 interestPercent, IERC20 token, uint256 startTime) public onlyOwner {
        plans.push(Plan({interestPercent: interestPercent,token: token,startTime: block.timestamp > startTime ? block.timestamp : startTime,minimumDays: minimumDays * DAY,amount: 0}));
    }
    ...
    function invest(uint8 index, uint256 amount, address referrer) public {
        ...
        user.deposits.push(Deposit({plan: index,amount: amount,start: start,finish: start + plan.minimumDays,interest: plan.interestPercent,lastWithdraw: 0}));
        plan.amount += amount;
    }
    ...
    function reinvest(uint16 index) public {
        ...
        deleteDeposit(index);
        user.deposits.push(Deposit({plan: deposit.plan,amount: amount,start: block.timestamp,finish: block.timestamp + plan.minimumDays,interest: plan.interestPercent,lastWithdraw: 0}));
    }
    /**
