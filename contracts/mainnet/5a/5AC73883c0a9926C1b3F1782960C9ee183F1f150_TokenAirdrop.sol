...
contract TokenAirdrop {
    ...
    event Airdropped(address recipient, uint256 weiSent, address tokenAddress);
    event DroppedMatic(address recipient, uint256 weiSent);
    ...
    function airdrop(
        ...
        for (uint256 _i = 0; _i < _amount.length; _i++)
        ...
        require(
            IERC20(_tokenAddress).allowance(msg.sender, address(this)) >=
                totalBalance,
            "the total allowed tokens are not enough to the perform transfer"
        );
        ...
        for (uint256 _i = 0; _i < _recipients.length; _i++) {
            emit Airdropped(_recipients[_i], _amount[_i], _tokenAddress);
    ...
    function airdropMatic(
        ...
        uint256 totalBalance = 0;
        // Check for the mismatiching arguments
        if (_recipients.length != _amount.length) {
        ...
        if (msg.value == totalBalance) {
            for (uint256 _i = 0; _i < _amount.length; _i++) {
                emit DroppedMatic(_recipients[_i], _amount[_i]);
                (bool sent, bytes memory data) = address(_recipients[_i]).call{
                    value: _amount[_i]
                }("");
                require(
                    sent && (data.length == 0 || abi.decode(data, (bool))),
