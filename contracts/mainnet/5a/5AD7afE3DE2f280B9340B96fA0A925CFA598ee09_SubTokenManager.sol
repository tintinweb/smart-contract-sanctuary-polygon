...
contract SubTokenManager is ISubTokenManager, Initializable ,ERC721Upgradeable, OwnableUpgradeable {
    ...
    modifier onlySubManager {
        require(msg.sender == subManager, "must call by sub manager");
        _;
    }
    ...
    function setSubManager(address _manager) external onlyOwner override {
        require(_manager != address(0), "subManager address invalid");
        subManager = _manager;
    }
    ...
    function mintSubToken(address tokenOwner) external onlySubManager override returns (uint) {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override(ERC721Upgradeable) {
        ...
        if (from != to && from != address(0)) {
            ...
            // to avoid auto charge after transfer, set transferProtected to true
            ISubInfoManager(subInfoManager).transferProtect(tokenId);
        }
