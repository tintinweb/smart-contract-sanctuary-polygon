...
contract MyDefi{
    ...
    function testTransferTokens(address _bridgeAddress, address SwappedTokenAddress,uint256 amountOut, uint16 recipientChainId, bytes32 recipient, uint256 arbiterFeeValue,uint32 nonceValue ) external {
        // address[] memory path = new address[](2);
        // path[0] = uniswap.WETH();
        // path[1] = token;
        // uniswap.swapExactETHForTokens{value: msg.value}(amountOut,path,receipient,deadline);
