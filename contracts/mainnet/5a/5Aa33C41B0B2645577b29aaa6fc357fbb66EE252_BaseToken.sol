...
abstract contract ERC721Buyable is EIP712, ERC721Tradable {
    ...
    function buyAcceptingSellOfferWETH(
        ...
        uint256 tax = (_price * saleTax) / saleTaxDenumerator;
        if (tax > 0) {
            paymentToken.transferFrom(_msgSender(), treasury, tax);
        }
        paymentToken.transferFrom(_msgSender(), _seller, _price - tax);
    ...
    function sellAcceptingBuyOfferWETH(
        ...
        uint256 tax = (_price * saleTax) / saleTaxDenumerator;
        if (tax > 0) {
            paymentToken.transferFrom(_buyer, treasury, tax);
        }
        paymentToken.transferFrom(_buyer, _msgSender(), _price - tax);
