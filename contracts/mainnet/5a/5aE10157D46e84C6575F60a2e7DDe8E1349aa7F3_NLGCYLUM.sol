...
contract NLGCYLUM is Context, IERC20, Ownable {
    ...
    string private _name = 'NLG-CYLUM';
    string private _symbol = 'NCYM';
    ...
    function excludeAccountRewards(address account) external onlyOwner() {
        require(!_isExcluded[account], "Account is already excluded from rewards");
	...
	function setFeeHolders(uint8 feeamt) external onlyOwner() {
        require(feeamt <= 20, "a resistribution fee of greater 20% cannot be set."); //prevent the dev, from set a higher fee. than 20%
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(amount > 0, "Transfer amount must be greater than zero");
            ...
            if (LOCK_CONTRACT != address(0)) {
                uint256 receivedAmount = ILockContract(LOCK_CONTRACT).lockedAmount(sender);
                require(balanceOf(sender).sub(amount) >= receivedAmount, "ERC20: You cannot transfer until unstake date");
            }
			
			if (LOCK_CONTRACT_TWO != address(0)) {
                uint256 receivedAmountsec = ILockContract(LOCK_CONTRACT_TWO).lockedAmount(sender);
                require(balanceOf(sender).sub(amount) >= receivedAmountsec, "ERC20: You cannot transfer until unstake date");
            }
