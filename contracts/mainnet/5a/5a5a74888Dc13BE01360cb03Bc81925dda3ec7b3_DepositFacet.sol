    ...
    function depositERC721(address _tokenAddress, uint256[] calldata _tokenId) public whenNotPaused {
        ...
        for(uint256 i = 0; i<_tokenId.length; ){
            ...
            if(_tokenAddress == im_diamondAddress){
                ...
                //check if this gotchi is currently in our unstaked lending management group; if so, remove it
                if(EnumerableSet.contains(s.unstakedLendingGotchis,_tokenId[i])){
                    EnumerableSet.remove(s.unstakedLendingGotchis,_tokenId[i]);
                }
            }
