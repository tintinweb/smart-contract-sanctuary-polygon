...
contract DominiumNFTDepository is Ownable, IERC721Receiver {
  ...
  function deposit(uint256 tokenID) public lock {
    ...
    IERC20(initialFeeCoin).safeTransferFrom(msg.sender, liquidityPair, getStakingCost(tokenID).div(100).mul(feeLiquidityPercent)); // Going to the liqudity
    IERC20(initialFeeCoin).safeTransferFrom(msg.sender, treasury, getStakingCost(tokenID).div(100).mul(100 - feeLiquidityPercent)); // Going to the treasury
    IUniswapV2Pair(liquidityPair).sync(); // Sync Balances in the liquidity pair 
  }
