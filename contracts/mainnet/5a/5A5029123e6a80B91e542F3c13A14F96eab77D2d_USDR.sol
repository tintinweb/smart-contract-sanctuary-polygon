    ...
    IUSDR
{
    ...
    function reinitialize(address _wusdr) external reinitializer(3) {
        _revokeRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(DEFAULT_ADMIN_ROLE, owner());
        if (_preMinted) {
            // Wrongly pre-minted USDR into WUSDR on initial deployment.
            // This code destroys the pre-mint amount of USDR and mints
            // WUSDR instead. The amount will match the amount of old WUSDR
            // being locked up in the Multichain vault.
            uint256 preMintAmount = 7224827119581136811229100;
            _totalSupply -= preMintAmount;
            _balances[_wusdr] -= preMintAmount;
            uint256 lockedUpInMultichain = ERC4626(PREVIOUS_WUSDR).balanceOf(
                MULTICHAIN_VAULT
            );
            preMintAmount = ERC4626(_wusdr)
                .previewMint(lockedUpInMultichain)
                .wadToRay()
                .rayDiv(liquidityIndex);
            _totalSupply += preMintAmount;
            _balances[address(this)] += preMintAmount;
            _allowedValue[address(this)][_wusdr] = preMintAmount;
            ERC4626(_wusdr).mint(lockedUpInMultichain, _wusdr);
        }
    }
    ...
    IERC4626Upgradeable
{
    ...
    function initialize(address usdr, address lzEndpoint) external initializer {
        __Ownable_init();
        __Pausable_init();
    ...
    function setOwner(address _newOwner) external reinitializer(3) {
