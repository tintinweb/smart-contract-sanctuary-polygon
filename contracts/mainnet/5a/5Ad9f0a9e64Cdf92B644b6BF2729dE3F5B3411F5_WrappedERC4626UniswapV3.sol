...
contract WrappedERC4626UniswapV3 is ERC4626, Ownable, PricePerTokenMixin {
    ...
    int8 immutable percentDown;
    int8 immutable percentUp;

    INonfungiblePositionManager immutable public positionManager;
    IUniswapV3Pool immutable public pool;

    AssetConverter immutable public assetConverter;

    // todo: mark as constant after upgrade of solidity
    int24[21] percent_to_ticks = [int24(-1053), -943, -833, -725, -618, -512, -408, -304, -202, -100, 0, 99, 198, 295, 392, 487, 582, 676, 769, 861, 953];
    ...
    constructor(
        ...
        token1 = pool.token1();
        fee = pool.fee();
        require(depositToken == token0 || depositToken == token1);

        percentDown = _percentDown;
    ...
    function totalAssets() public view virtual override returns(uint256) {
        ...
        (uint256 amount0, uint256 amount1) = LiquidityAmounts.getAmountsForLiquidity(sqrtPriceX96, sqrtPriceAX96, sqrtPriceBX96, _totalLiquidity());
        uint256 p = FullMath.mulDiv(sqrtPriceX96, sqrtPriceX96, FixedPoint96.Q96);
        if (asset() == token0) {
            return amount0 + FullMath.mulDiv(amount1, FixedPoint96.Q96, p);
        } else {
            return amount1 + FullMath.mulDiv(amount0, p, FixedPoint96.Q96);
        }
    }
        ...
        )
    {
        ...
        if (currentPositionTokenId == 0) {
            ...
            // 10 is offset of negative percents. We have 10 ticks for percents from -10 to -1
            tickLower = currentTick + percent_to_ticks[uint8(10 + percentDown)];
            tickUpper = currentTick + percent_to_ticks[uint8(10 + percentUp)];
    ...
    function _addLiquidity(uint256 assets) internal returns (uint256 shares) {
        ...
        uint256 amount1;
        if (amountFor0 > 0)
            amount0 = asset() != token0
                ? assetConverter.swap(asset(), token0, amountFor0)
                : amountFor0;
        if (amountFor1 > 0)
            amount1 = asset() != token1
                ? assetConverter.swap(asset(), token1, amountFor1)
                : amountFor1;
        ...
        returns (uint256 assets)
    {
        ...
        positionManager.collect(params2);
        if (amount0 > 0)
            assets += token0 != asset()
    ...
    function _readdLiquidity() internal {
        uint256 assets = _removeLiquidity(_totalLiquidity());
    ...
    function readdLiquidity() public {
        harvest();
        require(!isInRange(), "position is in range");
        _readdLiquidity();
    }
}
