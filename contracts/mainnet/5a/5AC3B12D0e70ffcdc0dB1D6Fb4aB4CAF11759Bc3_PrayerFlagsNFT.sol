...
contract PrayerFlagsNFT is ERC721A, Ownable, ReentrancyGuard, ERC2981ContractRoyalties {
    ...
    //Controlling the fading
    mapping(uint256 => uint256) public lastTransferTimestamp;
    uint256[21] public fadingArray = [100, 96, 92, 88, 84, 80, 76, 72, 68, 64, 60, 56, 52, 48, 44, 40, 36, 32, 28, 24, 20];

    event NFTMinted(uint256, uint256, address);

    constructor(string memory baseURI, address address1, address address2) ERC721A("PrayerFlags", "PRF") {
        baseTokenURI = baseURI;
        setRoyalties(address1, royalties_points);
        projectAddress1 = address1;
        projectAddress2 = address2;
    }
    ...
    function mintNFTs(address _to, uint256 amount) public payable {
        uint256 currentId = totalSupply();
        require(publicSaleIsOpen == true, "Public sale is not Open");
        require(currentId + amount < MAX_SUPPLY, "Not enough NFTs!");
        require(msg.value == price * amount , "Not enough/too much ether sent");

        _mint(_to, amount);

        for (uint256 i = 0; i < amount; i++){
            lastTransferTimestamp[currentId + i] = block.timestamp;
        }

        emit NFTMinted(amount, totalSupply(), _to);
    }
    ...
    function reservedMints(address _receiver, uint256 amount) public onlyOwner {
        uint256 currentId = totalSupply();
        ...
        emit NFTMinted(amount, totalSupply(), msg.sender);
    }
    ...
    function getTokenLastTransfer(uint256 tokenId) public view returns(uint256 lastBlockstamp){
        lastBlockstamp = lastTransferTimestamp[tokenId];
    }

    function getFadingArray() public view returns(uint256[21] memory){
        return fadingArray;
    }
