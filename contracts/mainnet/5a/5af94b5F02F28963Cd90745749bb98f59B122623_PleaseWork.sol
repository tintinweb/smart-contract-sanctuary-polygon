...
contract PleaseWork is Ownable, AccessControl {
    ...
    function buildRace(uint[] calldata _racers, address[] calldata _users) public {
        require(hasRole(TEAM_ROLE, msg.sender), "Caller is not a member of the team.");
        ...
        for(i; i<racing; i++){ //go through every racer
            ...
            perfByRacer[_racer].user = _users[i];
