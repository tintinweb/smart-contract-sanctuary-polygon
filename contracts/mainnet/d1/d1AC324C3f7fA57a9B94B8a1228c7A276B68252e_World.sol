...
contract World is Initializable, WorldContracts {
    ...
    requiresEitherRole(ADMIN_ROLE, OWNER_ROLE)
    {
        ...
        for(uint256 i = 0; i < _farmingParams.length; i++) {
            ...
            _requireValidEntityAndLocation(_farmingParam.tokenId, Location.ACTIVE_FARMING);

            // From
            _transferFromLocation(_farmingParam.tokenId);
    ...
    requiresEitherRole(ADMIN_ROLE, OWNER_ROLE)
    {
        ...
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            _requireValidEntityAndLocation(_tokenId, Location.NOT_STAKED);
    ...
    function _transferFromLocation(uint16 _tokenId) private {
        Location _oldLocation = tokenIdToInfo[_tokenId].location;

        if(_oldLocation == Location.ACTIVE_FARMING) {

            activeFarming.endFarming(_tokenId, msg.sender);
    ...
    function _requireValidEntityAndLocation(uint256 _tokenId, Location _newLocation) private view {
        Location _oldLocation = tokenIdToInfo[_tokenId].location;
        ...
        if(_oldLocation != Location.NOT_STAKED) {
            require(ownerToStakedTokens[msg.sender].contains(_tokenId), "World: User does not own entity");
        }
