...
contract ERC5516 is Context, ERC165, IERC1155, IERC1155MetadataURI, IERC5516 {
    ...
    string public constant name = "ZERTIFICATES";
    string public constant symbol = "ZERTS";
    string internal constant CONTRACT_URI =
        "Qmbpy53C1k9XLYhz7UR5YvACYPS3FQEaZb4ffnAGmD2fQL";
    ...
    function contractUri() external pure returns (string memory) {
        return string(abi.encodePacked(IPFSURI, CONTRACT_URI));
    }
    ...
    function tokensURIFrom(
        ...
        for (uint256 i = 0; i < _nTokens; ) {
            tokenURIS[i] = string(
                abi.encodePacked(
                    IPFSURI,
                    LibERC5516.getTokenURI(ownedTokens[i])
                )
            );
    ...
    function pendingURIFrom(
        ...
        for (uint256 i = 0; i < _nTokens; ) {
            tokenURIS[i] = string(
                abi.encodePacked(
                    IPFSURI,
                    LibERC5516.getTokenURI(pendingTokens[i])
                )
            );
    ...
    function _safeTransferFrom(
        ...
        require(
            LibERC5516.getPending(to, id) == false &&
                LibERC5516.getBalance(to, id) == false,
    ...
    function _batchTransfer(
        ...
        for (uint256 i = 0; i < to.length; ) {
            ...
            require(
                LibERC5516.getPending(_to, id) == false &&
                    LibERC5516.getBalance(_to, id) == false,
                "EIP5516: Already Assignee"
            );
     ...
     *
     */
    function _claimOrReject(address account, uint256 id, bool action) internal {
        require(
    ...
    function _burn(address account, uint256 id) internal {
        require(
            LibERC5516.getBalance(account, id) == true,
            "EIP5516: Unauthorized"
        );
