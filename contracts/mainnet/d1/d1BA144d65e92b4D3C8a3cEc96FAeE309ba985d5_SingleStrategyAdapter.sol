...
contract SingleStrategyPolygon is OwnableUpgradeable {
  ...
  */
  ...
  function getPendingShare(address user, uint256 perShare, uint256 debt) public view returns (uint256 share) {
  ...
  function withdrawReward(address user) public onlyVault {
    ...
    uint256 _pendingDQuick = getPendingShare(user, accDQuickPerShare, userDQuickDebt[user]); // 2908721557858648
    ...
    if (_dQuickBalance < _pendingDQuick) {
      ...
      uint256 dQuckBalanceInRewardsPool = IStakingRewards(dQuickExternalRewardsPool).balanceOf(address(this)); // 0

      remaining = MathUpgradeable.min(remaining, dQuckBalanceInRewardsPool); 
