...
contract StakingPoolDuration is Ownable, ReentrancyGuard, Metadata {
    ...
    function init(bytes memory extraData) external {
        ...
        require(
            farmInfo.endBlock >= block.number,
            "End block no must be greater than current"
        );
        require(
            farmInfo.endBlock > _localVars._startBlock,
            "Invalid start block"
        );
        require(farmInfo.endBlock >= _localVars._endBlock, "Invalid end block");
        require(
            _localVars._endBlock > _localVars._startBlock,
            "Invalid start and end block"
        );
    ...
    function addRewardToken(
        ...
        require(farmInfo.endBlock > _startBlock, "Invalid start block");
        require(farmInfo.endBlock >= _endBlock, "Invalid end block");
    ...
    function canHarvest(address _user) public view returns (bool) {
        ...
        return ((block.timestamp >= user.nextHarvestUntil) &&
            (block.number > farmInfo.endBlock));
    }
    ...
    function _withdraw(
        ...
        require(
            block.number >= farmInfo.endBlock,
            "Cannot withdraw before end block"
        );
        UserInfo storage user = userInfo[_user];
