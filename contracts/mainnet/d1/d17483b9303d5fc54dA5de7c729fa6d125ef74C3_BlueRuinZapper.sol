...
import "./libraries/UniswapV2LibraryQS.sol";
...
// zapper contract for NightBane-dMagic LP => Blue Ruin
contract BlueRuinZapper is Owned, TokensRecoverable, ReentrancyGuard
{
    using SafeMath for uint256;

    IAxMatic public immutable axMatic;
    IERC20 public immutable dmagic;
    IERC20 public immutable NightBane;
    ...
    address public LPAddress; // LP address of Nightbane-dMagic Quickswap
    uint256 erc1155TokenID;
    
    IUniswapV2Router02 private uniswapV2Router = IUniswapV2Router02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
    IUniswapV2Factory private uniswapV2Factory = IUniswapV2Factory(0x5757371414417b8C6CAad45bAeF941aBc7d3Ab32);
    
    event SlippageSet(uint slippage);

    constructor(IAxMatic _axMatic, IERC20 _dmagic, address _LPAddress, IERC20 _NightBane, IaxMatic_Direct _axMaticDirect, IDragonZapper _dragonDirect, IERC31337 _LPNFTWrapper, IERC1155 _ERC1155Token, uint _erc1155TokenID)
    {
        axMatic = _axMatic;
        NightBane = _NightBane;
        ...
        _NightBane.approve(address(_dragonDirect), uint256(-1));
        
        _axMatic.approve(address(_axMaticDirect), uint256(-1));
        _dmagic.approve(address(_axMaticDirect), uint256(-1));

        IERC20(_LPAddress).approve(address(_LPNFTWrapper),uint256(-1));

        _NightBane.approve(address(uniswapV2Router), uint256(-1));
    ...
    //  Matic => NFT
    function easyBuyNFT() external payable nonReentrant returns(uint) 
    {
        ...
        uint256 prevNightBane = NightBane.balanceOf(address(this));

        //swap half Matic to NightBane    
        uint256 tMatic=SafeMath.div(msg.value,2);
        dragonDirect.easyBuy{ value: tMatic }();

        uint256 nbaneAmt = NightBane.balanceOf(address(this)).sub(prevNightBane);
        ...
        uniswapV2Router.addLiquidity(address(NightBane), address(dmagic), nbaneAmt, dmagicAmt, 1, 1, address(this), block.timestamp+60);
        ...
        if(NightBane.balanceOf(address(this))>prevNightBane)
            NightBane.transfer(msg.sender, NightBane.balanceOf(address(this)).sub(prevNightBane));
    ...
    function easyBuyNFTFromAXMatic(uint256 axMaticAmt) public nonReentrant returns (uint256)
    {
        ...
        //swap half axMatic to NightBane    
        ...
        uint256 mints = LPNFTWrapper.depositTokens(LPAddress, IERC20(LPAddress).balanceOf(address(this)).sub(prevLPBalance));
        uint256 currLPNFTWrapperAmount = ERC1155Token.balanceOf(address(this), erc1155TokenID);
    ...
    //  dmagic => NFT
    function easyBuyNFTFromMagic(uint256 dmagicAmt) external nonReentrant returns(uint)
    {
        uint256 prevAxMaticAmount = axMatic.balanceOf(address(this));
        uint256 prevdmagicAmount = dmagic.balanceOf(address(this));
        uint256 prevNightBane = NightBane.balanceOf(address(this));
    ...
    //  axMatic => NFT
    function easyBuyNFTFromNightBane(uint256 nbaneAmt) public nonReentrant returns (uint256)
    {
        ...
        NightBane.transferFrom(msg.sender,address(this),nbaneAmt);

        //swap half NightBane to dmagic    
        uint256 NightBaneForBuy = nbaneAmt.div(2);
        dragonDirect.easySellTodMagic(NightBaneForBuy);
        uint256 dmagicAmt = dmagic.balanceOf(address(this)).sub(prevdmagicAmount);
