 ...
 * @notice Investment strategy for investing wMatic Balancer Strategy
 */
...
import "../../contracts/interfaces/balancer/interfaces/IVault.sol";
import "../../contracts/interfaces/balancer/interfaces/IAsset.sol";
import "../../contracts/interfaces/balancer/interfaces/IGauge.sol";
import "../../contracts/interfaces/balancer/interfaces/IBalancerPool.sol";
...
contract BalancerStrategy is InitializableAbstractStrategy, BalancerExchange, UniswapV2Exchange {
    ...
    IERC20 public maticXToken;
    IBalancerPool public lp;

    bytes32 pool;
    bytes32 poolIdSdMaticX;
    bytes32 poolIdMaticxMatic;
    IGauge public gauge;
     ...
     * InitializableAbstractStrategy initializer as wMatic strategy don't fit
     * well within that abstraction.
     * @param _platformAddress Address of the other token
     * @param _vaultAddress Address of the vault
     * @param _rewardTokenAddresses reward tokens
     * @param _assets Addresses of supported assets. MUST be passed in the same
     *                order as returned by coins on the pool contract, i.e.
     *                wMATIC
     * @param _pTokens Platform Token corresponding addresses
     * @param _pool Balancer vault address
     * @param _gauge Balancer gauge address
     * @param _rewardHelper BalancerRewardHelper
     */
    function initialize(
        address _platformAddress, // other token
        ...
        lp = IBalancerPool(_platformAddress);
        pool = _pool;
        gauge = IGauge(_gauge);
        ...
        // reward tokens
        balToken = IERC20(_rewardTokenAddresses[0]);
        staderToken = IERC20(_rewardTokenAddresses[1]);
        ldoToken = IERC20(_rewardTokenAddresses[2]);
        maticXToken = IERC20(_rewardTokenAddresses[3]); // for swaping

        super._initialize(_platformAddress, _vaultAddress, _rewardTokenAddresses, _assets, _pTokens);
    }

    function setBalancer(address _balancerVault, address _uniRouter, bytes32 _poolIdSdMaticX, bytes32 _poolIdMaticxMatic) external onlyGovernor {
        require(_balancerVault != address(0), "Zero address not allowed");
        require(_uniRouter != address(0), "Zero address not allowed");

        balancerVault = IVault(_balancerVault);
        setBalancerVault(_balancerVault);
        _setUniswapRouter(_uniRouter);
        poolIdSdMaticX = _poolIdSdMaticX;
        poolIdMaticxMatic = _poolIdMaticxMatic;
    }

    function _stake(uint256 _amount) internal {
        if (lp.balanceOf(address(this)) >= _amount) {
            lp.approve(address(gauge), _amount);
            gauge.deposit(_amount, address(this), true);
        }
    }
    ...
    function _arrangeLp(uint256 _lp) internal {
        ...
        gauge.withdraw(_lp - lp.balanceOf(address(this)), true);
    }

    function deposit(address _asset, uint256 _amount) external override onlyVaultOrGovernor nonReentrant {
        require(_asset == address(token0), "TK: NS");
    ...
    function _deposit(address _asset, uint256 _amount) internal {
        ...
        // uint256 _rate = lp.getRate();
        // console.log("@deposit:", _amount.mul(1 ether).div(_rate));

        (IERC20[] memory tokens, , ) = balancerVault.getPoolTokens(pool);
        ...
        token0.approve(address(balancerVault), _amount);
        uint256 joinKind = 1;
        ...
        balancerVault.joinPool(pool, address(this), address(this), request);
        // console.log("@deposit: lp got", lp.balanceOf(address(this)));
    }

    function depositAll() public override onlyVault nonReentrant {
        uint256 totalBalance = token0.balanceOf(address(this));
        _deposit(address(token0), totalBalance);
        _stake(lp.balanceOf(address(this)));
    ...
    function withdraw(address _beneficiary, address _asset, uint256 _amount) external override onlyVaultOrGovernor nonReentrant {
        _withdraw(_beneficiary, _asset, _amount);
    }

    function _withdraw(address _beneficiary, address _asset, uint256 _amount) internal {
        require(_asset == address(token0), "TK: NS");

        uint256 withdrawable = _amount.subOrZero(token0.balanceOf(address(this)));
        ...
        uint256 lpToWithdraw = _tokensToLP(_amount);
        uint256 closeAmount = ((lpBalance() * 97) / 100);
        // console.log("@lp to withdraw: %s; closeAmount: %s", lpToWithdraw, closeAmount);

        if (lpToWithdraw >= closeAmount) {
            // console.log("withdrawAll");
            _arrangeLp(lpBalance());
            _withdrawAll();

            uint256 _redepositAmount = _checkBalance();
            // console.log("want: %s; available: %s", _amount, token0.balanceOf(address(this)));

            if (_amount > 0 && _redepositAmount > _amount) {
                // console.log("subtracting remaining amount");
                _redepositAmount = _redepositAmount - _amount;
            }

            if (_redepositAmount > 0) {
                // console.log("Redeposit amount:", _redepositAmount);
                _deposit(_asset, _redepositAmount);
            }
        } else {
            _arrangeLp(lpToWithdraw);
            (IERC20[] memory tokens, , ) = balancerVault.getPoolTokens(pool);
            IAsset[] memory _poolAssets = new IAsset[](tokens.length);
            uint256[] memory _minAmountsOut = new uint256[](tokens.length);
            uint256[] memory bals = new uint256[](2);

            for (uint256 i; i < tokens.length; i++) {
                _poolAssets[i] = IAsset(address(tokens[i]));
                if (tokens[i] == token0) {
                    _minAmountsOut[i] = _amount;
                    bals[0] = _amount.addBasisPoints(10);
                } else {
                    _minAmountsOut[i] = 0;
                }
            }
            bytes memory _userData = abi.encode(uint256(1), bals, lpToWithdraw);
            IVault.ExitPoolRequest memory _request = IVault.ExitPoolRequest(_poolAssets, _minAmountsOut, _userData, false);
            // console.log("Exit pool calling");
            balancerVault.exitPool(pool, address(this), payable(address(this)), _request);
        }
        _sendBack(_beneficiary, _amount);
        _stake(lp.balanceOf(address(this)));
    }
    ...
    function _checkBalance() internal view returns (uint256) {
        ...
        return (token0Balance + lpBalance());
    }
    ...
    function _withdrawAll() internal {
        (IERC20[] memory tokens, , ) = balancerVault.getPoolTokens(pool);
        uint256[] memory _minAmountsOut = new uint256[](tokens.length);
        IAsset[] memory _assets = new IAsset[](tokens.length);
        ...
        // console.log("exitPool calling");
        balancerVault.exitPool(pool, address(this), payable(address(this)), _request);
    }

    function _tokensToLP(uint256 _amount) internal returns (uint256) {
        (IERC20[] memory tokens, , ) = balancerVault.getPoolTokens(pool);
        IAsset[] memory _assets = new IAsset[](tokens.length);
        uint256[] memory _minAmountsOut = new uint256[](tokens.length);
        uint256[] memory bals = new uint256[](2);
        ...
        for (uint256 i; i < tokens.length; i++) {
            ...
            if (tokens[i] == token0) {
                // if matic
                _minAmountsOut[i] = _amount;
                bals[i] = _amount.addBasisPoints(10);
            } else {
                _minAmountsOut[i] = 0;
            }
        }

        bytes memory _userData = abi.encode(uint256(1), bals, type(uint256).max);
        IVault.ExitPoolRequest memory _request = IVault.ExitPoolRequest(_assets, _minAmountsOut, _userData, false);

        (uint bptIn, ) = helper.queryExit(pool, address(this), payable(address(this)), _request);
        return bptIn;
    }

    function _sendBack(address _ben, uint256 _amount0) internal {
    ...
    function _collectRewards() internal {
        // // console.log("RewardCollection - BEFORE Token Balance: ", token0.balanceOf(address(this)));
        rewardHelper.claimRewardsFromGauge(address(gauge), address(this));
        // // console.log("before wmatic balance:", token0.balanceOf(address(this)));

        uint256 _balBalance = balToken.balanceOf(address(this));
        uint256 _ldoBalance = ldoToken.balanceOf(address(this));
        uint256 _staderBalance = staderToken.balanceOf(address(this));
        // // console.log("RewardCollection - Balancer balance:", _balBalance);
        // // console.log("RewardCollection - Lido balance:", _ldoBalance);
        // // console.log("RewardCollection - Stader balance:", _staderBalance);

        uint256 total = 0;
        if (_balBalance > 1e7) {
            // // console.log("swaping Balancer token");
            total += _swapExactTokensForTokens(address(balToken), address(token0), _balBalance, address(this));
        } else {
            // // console.log("not enough balancer token");
        }

        if (_ldoBalance > 1e7) {
            // console.log("swaping LDO token");
            total += _swapExactTokensForTokens(address(ldoToken), address(token0), _ldoBalance, address(this));
        } else {
            // // console.log("not enough ldo token");
        }

        if (_staderBalance > 1e7) {
            // // console.log("swaping stader token");
            staderToken.approve(address(balancerVault), _staderBalance);
            total += batchSwap(
                poolIdSdMaticX,
                poolIdMaticxMatic,
                IVault.SwapKind.GIVEN_IN,
                IAsset(address(staderToken)), // SD
                IAsset(address(maticXToken)), // MATICX
                IAsset(address(token0)), // wMATIC
                address(this),
                payable(address(this)),
                _staderBalance
            );
        } else {
            // // console.log("not enough stader token");
        }
        // // console.log("after wmatic balance:", token0.balanceOf(address(this)), token0.balanceOf(address(this)) / 1e18);

        uint256 balance = token0.balanceOf(address(this));
        // // console.log("RewardCollection - BAL, STADER, LDO -> wMATIC Balance: ", balance);
        if (balance > 0) {
            emit RewardTokenCollected(harvesterAddress, address(token0), total);
            // // console.log("@harvester addr:", harvesterAddress);
            // // console.log("@before harvestor bal:", token0.balanceOf(harvesterAddress));
            token0.transfer(harvesterAddress, balance);
            // // console.log("@after harvestor bal:", token0.balanceOf(harvesterAddress));
        }
    }
    ...
    function withdrawAll() external override onlyVaultOrGovernor nonReentrant {
        _arrangeLp(lpBalance());
        _withdrawAll();
        _sendBack(vaultAddress, token0.balanceOf(address(this)));
    }
...
abstract contract BalancerExchange {
    ...
    function swap(
        ...
        IERC20(address(tokenIn)).approve(
            ...
            IERC20(address(tokenIn)).balanceOf(address(this))
        );
    ...
    function onSwap(
        ...
        if (poolSpecialization == IVault.PoolSpecialization.GENERAL) {
            ...
            return
                IMinimalSwapInfoPool(pool).onSwap(
                    swapRequest,
                    balanceIn,
                    balanceOut
                );
    ...
    function _getAmountToSwap(
        ...
        uint256 amount0ToSwap = (amount0Total * reserve1) /
            ((reserve0 * denominator1) / denominator0 + reserve1);
        for (uint256 i = 0; i < precision; i++) {
            ...
            amount0ToSwap =
                (amount0Total * reserve1) /
                ((reserve0 * amount1) / amount0ToSwap + reserve1);
        }
    ...
    function _getAmountLpTokensToWithdraw(
        ...
        uint256 lpBalance = (totalLpBalance * amount0Total * denominator1) /
        ...
        for (uint256 i = 0; i < 1; i++) {
            ...
            lpBalance =
                (totalLpBalance * amount0Total * amount1) /
