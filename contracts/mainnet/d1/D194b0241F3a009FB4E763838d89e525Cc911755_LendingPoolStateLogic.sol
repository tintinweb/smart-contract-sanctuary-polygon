...
library LendingPoolStateLogic {
  ...
  function updateLPTokenValues(LendingPoolStateData storage _self, address _loanNFTAddress) public {
    uint256 poolValue = _getTotalValue(_self, _loanNFTAddress);
  ...
  function updateSeniorObligation(LendingPoolStateData storage _self, address _loanNFTAddress) public {
    ...
    for (uint256 i = 0; i < daysSinceLastUpdate; i++) {
      uint256 utilizationRatio = _getUtilization(_self, _loanNFTAddress);
  ...
  function _getTotalValue(LendingPoolStateData memory _self, address _loanNFTAddress) internal view returns (uint256) {
    uint256 poolLoanValue = (ILoanNFT(_loanNFTAddress)).getTotalLoanValue(address(this));
  ...
  function _getUtilization(LendingPoolStateData storage _self, address _loanNFTAddress)
    internal
    view
    returns (uint256)
  {
    uint256 totalValue = _getTotalValue(_self, _loanNFTAddress);
    if (totalValue == 0) {
      return 0;
    }
    uint256 loanValue = ILoanNFT(_loanNFTAddress).getTotalLoanValue(address(this));
    return loanValue.wdiv(totalValue);
  }

  function getReservePercentage(LendingPoolStateData memory _self, address _loanNFTAddress)
    public
    view
    returns (uint256)
  {
  ...
  function satisfiesMinUnderwritten(
    ...
    uint256 poolValue = _getTotalValue(_self, _loanNFTAddress);
    require(
      _self.juniorValue.wdiv(poolValue) >= _constraints.minPoolJuniorTranchePercentage,
  ...
  function satisfiesMaxPoolSize(LendingPoolConstraints memory _constraints, uint256 _poolValue) public pure {
    require(_poolValue < _constraints.maxPoolSize, "LendingPoolStateLogic: Pool size too large");
  }
  ...
  function satisfiesMaxReserve(
    ...
    if (_selfData.isPoolActive) {
      ...
      require(
        _getTotalValue(_selfData, _loanNFTAddress) < _constraints.maxReservePercentage.wmul(_constraints.maxPoolSize),
        "LendingPoolStateLogic: Cannot be above maxReserve"
      );
    }
  }
