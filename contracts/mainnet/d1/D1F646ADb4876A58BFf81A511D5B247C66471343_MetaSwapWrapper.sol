...
contract MetaSwapWrapper is Ownable, ReentrancyGuard, Pausable {
    ...
    ITokenTransferProxy public immutable tokenTransferProxy;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _whitelistedTargets.length; i++) {
            require(_whitelistedTargets[i] != address(_tokenTransferProxy));
    ...
    function addTarget(address _target) external onlyOwner {
        ...
        require(
            _target != address(tokenTransferProxy),
            "Target must not be tokenTransferProxy"
        );
    ...
    function _transferTokensIfNeeded(
        IERC20 token,
        uint256 amount,
        address approveTarget
    ) private {
        if (!_isETH(token)) {
            maybeApproveERC20(token, amount, approveTarget);
...
interface IWChainMaster {
    ...
    function execute(bytes calldata input, address sender, address[] calldata beneficiary, uint256 exchangeProfitShare) external;
}
