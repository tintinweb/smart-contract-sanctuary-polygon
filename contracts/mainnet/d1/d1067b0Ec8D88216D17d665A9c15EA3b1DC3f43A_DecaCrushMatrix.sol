...
contract DecaCrushMatrix {
    ...
    IERC20 decacrush = IERC20(0x79dbBB2E381d00323DbDcC6B6E5D7Fc6cE799beE);
    ...
    constructor(address _ownerAddress, address _devAddress) {
        ...
        LEVEL_PRICE[1] = 5 ether;
        LEVEL_PRICE[2] = 10 ether;
        LEVEL_PRICE[3] = 20 ether;
        LEVEL_PRICE[4] = 40 ether;
        LEVEL_PRICE[5] = 80 ether;
        LEVEL_PRICE[6] = 120 ether;
        LEVEL_PRICE[7] = 240 ether;
        LEVEL_PRICE[8] = 480 ether;
        LEVEL_PRICE[9] = 960 ether;
        LEVEL_PRICE[10] = 1920 ether;
    ...
    function setFlag(bool status) external {
        require(msg.sender==ownerWallet,"Invalid user");
        publicSaleFlag = status;
    }
    ...
    function regUser(address _referer) public payable {
        ...
        if(publicSaleFlag==false)
        {
            require(whitelisted[msg.sender],"Invalid userr");
        }else{
        ...
        if(publicSaleFlag){
            payable(ownerWallet).transfer(LEVEL_PRICE[1].mul(5).div(100));
            payable(devAddress).transfer(LEVEL_PRICE[1].mul(5).div(100));
        }
    ...
    function buyLevel(uint256 _slot) public payable {
        ...
        payable(userEarnings[msg.sender].referrer).transfer(
            LEVEL_PRICE[_slot].mul(40).div(100)
        );
        }

        emit EarningsMatrix(
            userEarnings[msg.sender].referrer,
            LEVEL_PRICE[_slot].mul(40).div(100),
            1,
            _slot
        );
    ...
    function payForLevel(
        ...
        if (!users[referer][_slot].isExist) {
            ...
            if((_level==1 && users[referer][_slot].noOfPayments[_level]==2) || (_level==2 && users[referer][_slot].noOfPayments[_level]==4)
            || (_level==3 && users[referer][_slot].noOfPayments[_level]==8) || (_level==4 && users[referer][_slot].noOfPayments[_level]==16))
            {

            }
            else if (
                _level == 5 && users[referer][_slot].noOfPayments[_level] == 32
            ) {
    ...
    function withdrawTokens(address recipient, uint256 amount) external {
        require(msg.sender == ownerWallet, "Invalid User");
        decacrush.transfer(recipient, amount);
    }
}