...
contract TreasureHuntShip is Initializable, ITreasureHuntTrait, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => uint256[]) public activeShipsByOwner;
    mapping(address => uint256[]) public inActiveShipsByOwner;
    ...
    function openBox(uint16 amt) public {
    ...
    function mint(address to, uint256 tokenID) internal {
        ...
        ITreasureHunt(treasureHuntPicker).randomShip(tokenID);
    ...
    function getShipInfo(uint256 tokenID) public view returns (uint8) {
        require(msg.sender == ownerOf(tokenID), "You are not the ship owner!");
        Ship memory ship = shipInfo[tokenID];
        return ship.Star;
    } 

    function setShip(uint256 tokenID, uint8 star) public {
        require(msg.sender == treasureHuntPicker, "You are not picker!");
        shipInfo[tokenID] = Ship(tokenID, star);
        string memory uri = string(abi.encodePacked(ShipBaseURI[star], StringsUpgradeable.toString(tokenID)));
    ...
    function disJoinShips(uint256[] memory ships) public _isFleet {
        for (uint256 i = 0; i < ships.length; i++) {
            ...
            safeTransferFrom(treasureHuntFleet, tx.origin, ships[i]);
