...
contract LXDAOBadge is ERC1155, ERC1155Supply, AccessControl {
    ...
    function addType(string memory badgeType) public onlyRole(OPERATION_ROLE) {
        require(_typeTokenIds[badgeType] == 0, "Type already exists");

        _tokenIds.increment();
        uint256 tokenId = _tokenIds.current();
        _tokenIdTypes[tokenId] = badgeType;
        _typeTokenIds[badgeType] = tokenId;
        _types.push(badgeType);

        emit TypeAdded(msg.sender, badgeType);
    }

    function removeType(
        string memory badgeType
    ) public onlyRole(OPERATION_ROLE) {
        uint256 tokenId = _typeTokenIds[badgeType];
        ...
        delete _typeTokenIds[badgeType];

        for (uint256 i = 0; i < _types.length; i++) {
            if (
                keccak256(abi.encodePacked(_types[i])) ==
                keccak256(abi.encodePacked(badgeType))
            ) {
        ...
        emit TypeRemoved(msg.sender, badgeType);
    }
    ...
    function getTokenId(
        ...
        return _typeTokenIds[badgeType];
    }
    ...
    function balanceOfType(
        ...
        for (uint256 i = 0; i < members.length; i++) {
            uint256 amount = balanceOf(members[i], _typeTokenIds[badgeType]);
            amounts[i] = amount;
        }
        return amounts;
    }
