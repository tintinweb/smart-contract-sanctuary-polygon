...
contract AutoMeshedV2 is Ownable, ReentrancyGuard {
    ...
    uint256 public constant MSDMaxSupply = 432_000e18; // 2 years
    uint256 public constant MSDPerBlock = 1e16; // 0.01 MSD per Block
    ...
    function initialize(address _MSD, uint256 _startBlock) public onlyOwner {
        ...
        require(_startBlock > block.number, "startBlock must be greater than current block");
...
contract MeshedToken is ERC20("memememem", "mmm"), Ownable {
    address public constant meshFactory = 0x9F3044f7F9FC8bC9eD615d54845b4577B833282d;
    address public meshRouter = 0x10f4A785F458Bc144e3706575924889954946639;

    bool public earlyBotDetect;
    uint256 public earlyBotDetectBlock = 3;
    uint256 public lpCreatedBlock;
    ...
    constructor() public {
        isWhitelist[meshFactory] = true;
        isWhitelist[meshRouter] = true;
    }
    ...
    function addBlackList(address _usr) public onlyOwner {
        isBlacklist[_usr] = true;

        emit Blacklisted(_usr);
    }

    function removeBlackList(address _usr) public onlyOwner {
        isBlacklist[_usr] = false;
    }

    function addWhitelist(address _usr) public onlyOwner {
        isWhitelist[_usr] = true;
    }

    function removeWhitelist(address _usr) public onlyOwner {
        isWhitelist[_usr] = false;
    }

    function setEarlyBotDetect(bool _enable, uint256 _block) public onlyOwner {
        earlyBotDetect = _enable;
        earlyBotDetectBlock = _block;
    }

    function _checkBlackList(address addr) internal view returns (bool) {
        if (isWhitelist[addr]) {
            return false;
        } else {
            return isBlacklist[addr];
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(!_checkBlackList(from), "Blacklisted address");

        if (earlyBotDetect) {
            if (to == meshFactory) {
                lpCreatedBlock = block.number;
            }
            if (block.timestamp - lpCreatedBlock <= earlyBotDetectBlock) {
                isBlacklist[to] = true;
