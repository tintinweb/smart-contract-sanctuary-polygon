...
/**
 * @title UnifriendChildTunnel
 */
contract UnifriendChildTunnel is Ownable, FxBaseChildTunnel, Create2 {
    bytes32 public constant DEPOSIT = keccak256("DEPOSIT");

    /// @notice event for token maping
    event TokenMapped(address indexed rootToken, address indexed childToken);

    /// @notice root to child token
    mapping(address => address) public rootToChildToken;

    /// @notice child token template
    address public childTokenTemplate;

    /// @notice root token template code hash
    ...
    modifier onlyModerator() {
        require(moderators[msg.sender], "UnifriendChildTunnel: NON_MODERATOR");
        _;
    }
    ...
    function deployChildToken(
        ...
        require(
            ...
            "UnifriendChildTunnel: ALREADY_MAPPED"
        );
        rootToChildToken[rootToken] = childToken;
        emit TokenMapped(rootToken, childToken);

        // initialize child token with all parameters
        FxERC20(childToken).initialize(
            address(this),
            rootToken,
            name,
            symbol,
    ...
    function mintToken(address childToken, uint256 amount)
        public
        onlyModerator
    {
    ...
    function withdraw(address childToken, uint256 amount) public {
        ...
        _sendMessageToRoot(
            abi.encode(rootToken, childToken, msg.sender, amount, metaData)
        );
    }
