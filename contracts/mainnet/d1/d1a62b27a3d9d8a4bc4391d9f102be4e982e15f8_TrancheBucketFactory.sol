...
/**
 * @title The TrancheBucketFactory deploys trancheBuckets for actors.
...
contract TrancheBucketFactory is ITrancheBucketFactory, ITrancheBucketFactoryEvents {
    ...
    function setNewTrancheBucketFactoryImplementation(
        ...
        emit NewImplementationSet(_newImplementation);
    }

    /**
     * Deploys a TrancheBucket contract for a collateral pallet
     * @param _palletIndex the index of the inventoryPallet that is backing the bucket
     * @param _stakedYield bool that when set to true, will pay out the performance yield in staked GET (so xGET). Default: false
     * @param _bondBacked bool that when set to true, will tell configure the bucket to be backed by a teller loan. Default: true
     * @dev the ability to deploy unbacked buckets, will be more trustlessly implemented in future versions of the EFM
     */
    function deployTrancheBucket(
        ...
        // Check if the pallet already has a bucket deployed for it (which is not allowed in this version of the EFM)
        require(
        ...
        uint32 integratorIndex = IChamberOfCommerce(chamberOfCommerce).returnIntegratorIndex(msg.sender);
        trancheBucket_ = Clones.clone(trancheBucketImplementation);
        _initializeBucket(trancheBucket_, integratorIndex, _palletIndex, _stakedYield, _bondBacked);
    ...
    /**
     * Returns if a pallet has a bucket associated with it
     */
    ...
    /**
     * Returns if a pallet has a BACKED bucket associated with it
     */
    ...
    /**
     * Deletes a buckets mapping in the factory manaully. Should only be called in exceptional cases
     */
    function deleteTrancheBucket(
        ...
        delete typeBucket[_palletIndex];
    }

    /**
     * If a bucket is manually cancelled by the owner in the bucket itself, the factory should be made aware.
     */
    ...
    /**
     * Manually sets a bucket to a certain state. Only should be used in exceptional cases.
     * @param _palletIndex the index of the pallet a bucket should be associated to
     * @param _state the BucketConfiguration that will be manually set
     * @param _toPause bool that when set true, will pause the movement of the trancheShares
     */
    function setTrancheState(
        ...
        // there must be a bucket associated with the provided palletIndex
        _doesBucketExistCheck(_palletIndex);
    ...
    /**
     * Process TellerLoan Cancellation
     * @param _palletIndex the palletIndex of the collateral/event/inventory
     * @dev function can only be called by the TellerKeeper 
     * @dev this function will set the BucketConfiguration to  INVALID_CANCELLED_VOID
     * @dev this function will pause the movement of any trancheShares!
     */
    ...
    /**
     * Process TellerLoan Liquidation
