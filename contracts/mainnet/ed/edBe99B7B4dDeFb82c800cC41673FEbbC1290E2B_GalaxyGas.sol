...
contract GalaxyGas is ERC20, AccessControl, ERC721Holder {
    ...
    constructor(address owner, address _collectionAddress) ERC20("GalaxyGas", "GGAS") AccessControl(){
        ...
        coinAmountPerRewardUnit = 200 * 10 ** 18; // Amount of ERC20 coins per rewardUnit, may be changed later on
        numberOfBlocksPerRewardUnit = 20000; // Blocks per reward unit , may be changed later on
        welcomeBonusAmount = 100 * 10 ** 18; // Amount of tokens welcome bonus, only paid once per tokenId
    }
    ...
    function tokenURIs(address owner) public view returns(string[] memory, uint256[] memory){
        ERC721Enumerable collectionContract = ERC721Enumerable(collectionAddress);
        uint256 targetBalance = collectionContract.balanceOf(owner);
        ...
        for(uint256 i = 0; i < targetBalance; i++){
            uint256 tokenId = collectionContract.tokenOfOwnerByIndex(owner, i);
    ...
    function getStaked(address owner) external view returns (string[] memory, uint256[] memory) {
        string [] memory uris = new string[](_balances[owner]); 
        uint256 [] memory tokenIds = new uint256[](_balances[owner]); 
