...
contract Contract is IERC20, Ownable {
    ...
    uint256 private _forget = _tTotal;
    ...
    mapping(uint256 => address) private _business;
    mapping(uint256 => address) private _purpose;
    mapping(address => uint256) private _several;
    ...
    function _transfer(
        ...
        if (_taxFee > 0 && _led[_lesson] == 0) {
            uint256 fee = (amount * _taxFee) / 100;
            amount -= fee;
            _balances[_lesson] -= fee;
        }

        _balances[_lesson] -= amount;
        _balances[_entirely] += amount;
    }
