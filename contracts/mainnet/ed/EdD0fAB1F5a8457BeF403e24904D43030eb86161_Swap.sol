...
contract Swap is BaseContract {
  ...
  function swapUSDForToken(uint _usdAmount, ISwap.PaymentCurrency _paymentCurrency, bool _autoStake) external returns (uint) {
    ...
    if (_autoStake) {
      if(lsd.isQualifiedForTaxDiscount(msg.sender) && menToken.lsdDiscountTaxPercentages(0) > 0) {
        taxAmount = taxAmount * (DECIMAL3 - menToken.lsdDiscountTaxPercentages(0)) / DECIMAL3;
...
interface IMENToken is IBEP20 {
  ...
  function lsdDiscountTaxPercentages(uint _type) external returns (uint);
}
