...
contract Marketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable {
    ...
    event EmergencyWithdrawNFT(address nft, address to, uint tokenID, uint quantity);
...
//    bytes4 private constant _ERC721_RECEIVED = 0x150b7a02;
    ...
    function cancelListing(
        ...
        (Listing memory _listing, bool found) = _removeListing(_nftAddress, _tokenId, _msgSender());
        require(found == true, "item not found");        
        uint256 nftType = tokenRegistryAddress.allowedNFTs(_nftAddress);
        if (nftType == 721) {
            IERC721(_nftAddress).safeTransferFrom(address(this), _msgSender(), _tokenId);
        } else if (nftType == 1155){
            IERC1155(_nftAddress).safeTransferFrom(address(this), _msgSender(), _tokenId, _listing.quantity, bytes(""));
        }
    ...
    function updateListing(
        ...
        for(uint256 i =0; i < _ownersListings.length; i++) {
            ...
            if (_ownersListings[i].tokenId == _tokenId && _ownersListings[i].contractAddress == _nftAddress) {
                ...
                _oldQuantity = _ownersListings[i].quantity;
        ...
        if(_oldQuantity > _newQuantity){
            IERC1155(_nftAddress).safeTransferFrom(address(this), _msgSender(), _tokenId, (_oldQuantity-_newQuantity), bytes(""));
        } else if(_newQuantity > _oldQuantity){
            IERC1155(_nftAddress).safeTransferFrom(_msgSender(), address(this), _tokenId, (_newQuantity - _oldQuantity), bytes(""));
        }
    ...
    function acceptOffer(
        ...
        Offer memory offer;
        uint offerlocation;
        uint offerslength = offers[_nftAddress][_tokenId].length;
        ...
        if(_ownersListings.length > 0){
            for(uint256 i =0; i < _ownersListings.length; i++) {
                ...
                if (_listing.tokenId == _tokenId && _listing.contractAddress == _nftAddress) {
                    _sent = true;
                    if(_listing.quantity == offer.quantity){
                    ...
                    } else if(_listing.quantity > offer.quantity){
                        ...
                        // Update mapping
                        _ownersListings[i].quantity = _listing.quantity - offer.quantity;
                        listings[_listing.internalIndex].quantity = _listing.quantity - offer.quantity;
                        break;
                    } else if(_listing.quantity < offer.quantity){
                        IERC1155(_nftAddress).safeTransferFrom(
                                address(this),
                        ...
                        IERC1155(_nftAddress).safeTransferFrom(
                                ...
                                _creator,
                                _tokenId,
                                (offer.quantity - _listing.quantity),
                                bytes("")
                        );                    
        ...
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC721)) {
            ...
            if(offerlocation != (offerslength-1)){
                offers[_nftAddress][_tokenId][offerlocation] = offers[_nftAddress][_tokenId][offerslength-1];
            }
    ...
    function emergencyTokenWithdraw (address _to, address _token) external onlyOwner {
        uint balance = IERC20(_token).balanceOf(address(this));
        IERC20(_token).transfer(_to, balance);
        emit EmergencyWithdrawToken(_token, _to, balance);
    }

    function emergencyNFTWithdraw (address _to, address _nftAddress, uint _tokenID, uint _amount) external onlyOwner {
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC721)) {
            ...
            IERC1155(_nftAddress).safeTransferFrom(address(this), _to, _tokenID, _amount, bytes(""));
        }
        emit EmergencyWithdrawNFT(_nftAddress, _to, _tokenID, _amount);
    }
