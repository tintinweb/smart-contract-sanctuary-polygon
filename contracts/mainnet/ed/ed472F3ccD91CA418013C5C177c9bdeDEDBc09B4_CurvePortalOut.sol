...
contract CurvePortalOut is PortalBaseV1 {
    using SafeTransferLib for address;
    using SafeTransferLib for ERC20;
    ...
    function portalOut(
        ...
        amount = _remove(sellToken, amount, intermediateToken, poolData);
    ...
    /// @notice Removes the intermediateToken from the pool using the correct interface based on the
    ...
    /// @param poolData Encoded pool data including the following:
    /// pool The address of the swap/deposit contract
    /// The index of the intermediateToken in the pool (i.e. the index of buyToken)
    /// isInt128 A boolean value specifying whether the index is int128 or uint256
    /// @return liquidity The quantity buyToken acquired
    function _remove(
        address sellToken,
        uint256 sellAmount,
        address buyToken,
        ...
        (address pool, uint256 coinIndex, bool isInt128) = abi.decode(
            poolData,
