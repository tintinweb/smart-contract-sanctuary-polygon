...
contract DepositConditionV2 is permission {
    ...
    mapping(address => uint256) public deleyed_block;
    ...
    function depositWithMATIC(address account,address referral) public payable returns (bool) {
        (uint256 balance,uint256 repeat,,,,,) = IPLGv4(PLGv4).invest(account);
        require(block.timestamp>=deleyed_block[account],"Revert By Minimum Deposit");
        ...
        if(balance>0){ require(msg.value<=balance*150/100,"Must Be Deposit Below or Equal 1.5x Balance"); }
        internalDeposit(account,referral,msg.value);
        updateAccountDeleyedBlock(account,deleyed_default + (86400 * repeat));
        return true;
    }
    ...
    function updateAccountDeleyedBlock(address account,uint256 deleyed_period) internal {
        if(deleyed_period>deleyed_maximumed){ deleyed_period = deleyed_maximumed; }
        deleyed_block[account] = block.timestamp + deleyed_period;
        (uint256 a,uint256 s,uint256 d,uint256 f,uint256 g,uint256 h,uint256 j) = IPLGv4(PLGv4).invest(account);
        if(deleyed_block[account]>g){
            uint256[] memory data = new uint256[](7);
            data[0] = a;
            data[1] = s;
            data[2] = d;
            data[3] = f;
            data[4] = deleyed_block[account] + (86400 * 3);
            data[5] = h;
            data[6] = j;
