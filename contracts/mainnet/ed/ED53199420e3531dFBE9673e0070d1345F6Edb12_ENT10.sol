...
contract ENT10 is IENT10, Auth {
  ...
  string constant _name = "Equipping NFT Token 1.0";
  string constant _symbol = "ENT10";
  ...
  constructor(string memory _folderpath,address _receiver,uint256 _mintPrice,uint256 _soldPrice) Auth(msg.sender) {
    receiver = _receiver;
  ...
  function changePath(string memory _folderpath) external authorized returns (bool) {
    path = _folderpath;
    return true;
  }
  ...
  function _burnFrom(address account, uint256 amount) internal {
    ...
    _approve(account, msg.sender, _allowances[account][msg.sender].sub(amount, "ENT10: burn amount exceeds allowance"));
  }
}