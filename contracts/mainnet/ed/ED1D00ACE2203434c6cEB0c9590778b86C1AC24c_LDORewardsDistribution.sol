...
contract LDORewardsDistribution is OwnableUpgradeable, PausableUpgradeable {
    ...
    // amount that will be distributed for this epoch
    struct EpochRewardsAmount {
    ...
    mapping(uint256 => mapping(address => bool)) public isEligible; // epoch -> addr -> eligible

    mapping(uint256 => EpochParameters) public epochParams;
    mapping(uint256 => EpochRewardsAmount) public epochRewards;
    ...
    function collectRewards() external {
        ...
        uint256 userRewards = calculateRewards(msg.sender);
        // check if this user has any rewards to collect
        require(userRewards > 0, "no rewards to collect.");
        // change all user position to collected
        updateUserPositionAfterCollectRewards(msg.sender);
    ...
    /**
     * @dev updates the amount of rewards that will be distributed to supply/borrow in an epoch.
     * @param epoch number for the rewards.
     * @param supply rewards amounts to distribute.
     * @param borrow rewards amounts to distribute.
     */
    function updateEpochRewards(
        uint256 epoch,
        uint256 supply,
        uint256 borrow
    ) external onlyOwner {
        EpochRewardsAmount memory epochRewardsAmount;
        epochRewardsAmount.supplyRewards = supply;
        epochRewardsAmount.borrowRewards = borrow;
        epochRewards[epoch] = epochRewardsAmount;

        emit UpdatedRewards(epoch, supply, borrow);
    }
     ...
     */
    function updateEpochParams(uint256 epoch) external onlyOwner {
        // maybe add a require to not try to update the same epoch more than once.
        require(
            epoch == epochNumber + 1,
        ...
        emit UpdatedEpochParameters(epoch, totalSupplied, totalBorrowed);
    }
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < length; ) {
            if (isEligible[epoch][user[i]] == false) {
    ...
    /**
     * @dev Called only when a user requests to redeem rewards via `collectRewards()`.
     * @dev Change the user `collected` variable to true.
     * @param user address to update the `collected` variable.
     */
    function updateUserPositionAfterCollectRewards(address user) internal {
        for (uint256 i = 1; i <= epochNumber; ) {
            epochUserPosition[i][user].collected = true;
            unchecked {
                ++i;
            }
        }
    ...
    /**
     * @dev calculates rewards for an epoch. Called only via `calculateRewards()`.
     * @param epoch number to load the params to calculate rewards.
     * @param user address to calculate rewards.
     * @return userEpochRewards amount for this epoch.
     */
    function calculateEpochRewards(uint256 epoch, address user)
        internal
        view
        returns (uint256)
    {
        // check if you are eligible for this epoch
        if (isEligible[epoch][user] == false) return 0;
        // checks if the user has already collected this epoch rewards
        if (epochUserPosition[epoch][user].collected == true) return 0;
        // checks whether the user's position has been modified after defining the parameters of this epoch
        if (
            epochUserPosition[epoch][user].timestamp >
            epochParams[epoch].timestamp
        ) return 0;

        uint256 userEpochRewards = 0;
        //rewards amount distributed
        uint256 supplyRewards = epochRewards[epoch].supplyRewards;
        uint256 borrowRewards = epochRewards[epoch].borrowRewards;

        uint256 totalEpochSupplied = epochParams[epoch].totalSupplied;
        uint256 totalEpochBorrowed = epochParams[epoch].totalBorrowed;

        uint256 userSupplyPosition = epochUserPosition[epoch][user].supplied;
        if (userSupplyPosition > 0) {
            uint256 userSupplyRewards = supplyRewards *
                ((userSupplyPosition * 1e18) / totalEpochSupplied);
            userEpochRewards += userSupplyRewards / 1e18;
        }

        uint256 userBorrowPosition = epochUserPosition[epoch][user].borrowed;
        if (userBorrowPosition > 0) {
            uint256 userBorrowRewards = borrowRewards *
                ((userBorrowPosition * 1e18) / totalEpochBorrowed);
            userEpochRewards += userBorrowRewards / 1e18;
        }
    ...
    /**
     * @dev helper to obtain the sum of the rewards of the epochs already defined in the contract.
     * @param user address to calculate rewards.
     * @return userRewardsAmount rewards amount.
     */
    function calculateRewards(address user) public view returns (uint256) {
        uint256 userRewardsAmount = 0;
        for (uint256 i = 1; i <= epochNumber; ) {
            userRewardsAmount += calculateEpochRewards(i, user);
            unchecked {
                ++i;
            }
        }
