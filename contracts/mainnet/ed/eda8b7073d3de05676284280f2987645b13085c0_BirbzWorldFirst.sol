...
contract Helpers is Ownable {
    ...
    uint public minRandomIntervalMinutes = 480;

    uint public maxRandomIntervalMinutes = 720;
}
...
pragma solidity ^0.8.0;
 ...
 // Game Logic -----------------------------------
  ...
  function resetAllStatsAndTimers() public {
    ...
    randomizedSickTimerEnd = getRandomizedSickTimerEnd();

    lastInteractionTimer = block.timestamp;

    ageTimerStart = block.timestamp;

    // Default Stats - start with less so we can have interaction right away
    defHunger = 2; // when hunger is low, the pet is hungry
    defEnergy = 4;
    defFun = 4;
    defIntellect = 3;
  ...
  function petStatDepletionTimerStart(uint8 _statIndex) public view returns(uint){
      if(_statIndex == 0){
          return foodDepletionTimerStart;
      }

      if(_statIndex == 1){
          return energyDepletionTimerStart;
      }

      if(_statIndex == 2){
          return funDepletionTimerStart;
      }

      if(_statIndex == 3){
          return intellectDepletionTimerStart;
      }
      
      if(_statIndex == 4){
          return cleanlinessDepletionTimerStart;
      }
  ...
  function petStatDepletionInterval(uint8 _statIndex) public view returns(uint){
    if(_statIndex == 0){
        return foodDepletionInterval;
    }

    if(_statIndex == 1){
          return energyDepletionInterval;
    }

    if(_statIndex == 2){
        return funDepletionInterval;
    }

    if(_statIndex == 3){
        return intellectDepletionInterval;
    }

    if(_statIndex == 4){
        return cleanlinessDepletionInterval;
    }

    return 0;
  }
  
  // the pet will get sick more often when younger
  function getRandomizedSickTimerEnd() public view returns(uint){
      return block.timestamp + getRandomInterval() * (age() + 2);
  }

  function getRandomInterval() private view returns(uint256){
    return (random(maxRandomIntervalMinutes) * 1 minutes) + minRandomIntervalMinutes;
    // (random(8) * 1 hours) + 4 hours;
  }
...
// for testing purposes only
  ...
  function ageToAdd() private view returns(uint256){
      return (block.timestamp - ageTimerStart) / 180 days; // every 180 days default (approx 6 months, the pet will age with 1)
  }
  ...
  // Default Stats
  uint256 defHunger = 10; // when hunger is low, the pet is hungry
...
// Death Count & Death - not in World First edition
  ...
  uint8 constant defStatToAddValue = 2;
    ...
    function perPetMultiStat(uint _hunger, uint _happiness, uint _energy, uint _intellect, uint _health, uint _cleanliness, uint8 _isSleeping, uint8 _isLearning, uint _sleepTimerEnd, uint _learnTimerEnd) public {
        // reset stats after setting them because we're setting them directly
        if(_hunger > 0){
            if(_hunger > maxStatValue && _hunger > hunger()){
                _hunger = maxStatValue;
            }
            // multiply the xp by the difference between old stat and new stat - could get negative values if using smaller than current hunger
            // removing for now
        ...
        if(_happiness > 0 && _happiness > fun()){
        ...
        if(_intellect > 0 && _intellect > intellect()){
        ...
        if(_cleanliness > 0 && _cleanliness > cleanliness()){
        ...
        // sleeping
        if(_isSleeping == 0){
            setPetIsSleeping(false);
        }else{
            setPetIsSleeping(true);
            if(_sleepTimerEnd > 0){
                sleepingTimerEnd = _sleepTimerEnd;
            }
        }
        ...
        // learning
        if(_isLearning == 0){
            setPetIsLearning(false);
        }else{
            setPetIsLearning(true);
            if(_learnTimerEnd > 0){
                learningTimerEnd = _learnTimerEnd;
            }
        }
    }

  // Sleep
  uint256 public sleepingTimerEnd;
  
  bool isSleeping;

  // only if the new value is different from current
  function setPetIsSleeping(bool _sleeping) private {
      if(isSleeping != _sleeping){
        isSleeping = _sleeping;
      }
  }

  function isPetSleeping() public view returns (bool){
      bool res = isSleeping;

      if(sleepingTimerEnd > 0){
          if(block.timestamp >= sleepingTimerEnd){
             res = false;
          }
      }
...
//    //require(isSleeping() == false, "Already sleeping");
//    //require(energy() <= 5, "Cannot sleep, not tired enough");
//    isSleeping = true;
//    addToXP(xpToAdd());
//    sleepingTimerStart = block.timestamp;
//    energyDepletionTimerStart = block.timestamp; // reset the timer, resulting in maxing energy out
//  }
  
  uint256 public learningTimerEnd;

  bool isLearning;

  // only if the new value is different from current
  function setPetIsLearning(bool _learning) private {
      if(isLearning != _learning){
        isLearning = _learning;
      }
  }

  function isPetLearning() public view returns (bool){
      bool res = isLearning;

      if(learningTimerEnd > 0){
          if(block.timestamp >= learningTimerEnd){
             res = false;
          }
      }
  ...
  uint256 randomizedSickTimerEnd = getRandomizedSickTimerEnd();
...
contract BirbzWorldFirst is ERC721Enumerable, Ownable, Helpers {
  using Strings for uint256;

  string public baseURI = "https://birbz.pet/world-first-meta/";
  string public baseExtension = ".json";
  uint256 public cost = 50 * 10**18;
  uint256 public costWL = 30 * 10**18;
  uint256 public nameChangeCost = 20 * 10**18;
  ...
  function mint(address _to, string memory _petName) public payable {
    ...
    // then freemint overrides wl
    if(freeminted[msg.sender] == true){
        wlmint = false;
        fmint = true;
    }

    //cost requirements - wl
    if(wlmint == true){
        require(msg.value >= costWL);
    }

    //not wl not fmint
    if(wlmint == false && fmint == false){
    ...
    //remove privileges only after mint success
    if(fmint == true){
  ...
  function getCostForAddress(address _addr) public view returns(uint){
      uint res = cost;
      
      if(whitelisted[_addr] == true){
          res = costWL;
      }

      if(freeminted[_addr] == true){
          res = 0;
      }
...
// Free mint users
  ...
  // Pet Interactions per ID -- Public
    ...
    function setMultiStat(uint petID, uint _hunger, uint _happiness, uint _energy, uint _intellect, uint _health, uint _cleanliness, uint8 _isSleeping, uint8 _isLearning, uint _sleepTimerEnd, uint _learnTimerEnd) public ownerOfTokenId(petID) returns (bool){
        petAtID[petID].perPetMultiStat(_hunger, _happiness, _energy, _intellect, _health, _cleanliness, _isSleeping, _isLearning, _sleepTimerEnd, _learnTimerEnd);
        return true;
    }
  ...
  function isSleeping(uint petID) public view returns (bool){
    return petAtID[petID].isPetSleeping();
  }

  function isLearning(uint petID) public view returns (bool){
    return petAtID[petID].isPetLearning();
  }
  ...
  function getStatDepletionTimerStart(uint petID, uint8 _statIndex) public view returns(uint){
      return petAtID[petID].petStatDepletionTimerStart(_statIndex);
  }

  function getStatDepletionInterval(uint petID, uint8 _statIndex) public view returns(uint){
      return petAtID[petID].petStatDepletionInterval(_statIndex);
  }
