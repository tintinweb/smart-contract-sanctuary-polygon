...
/**
 *   Purchase a royal LDA without a reservation but rather with proof that we generate and provide the purchaser offline.
...
contract NoReservationPurchaseLDA is RoyalPausableUpgradeable {
    ...
    function initialize(address paymentWallet) public initializer {
    ...
    function registerTierForPurchase(
        ...
        maxTokenIDToSellForTier[tierID] = Royal1155LDA(ldaContract).tierMaxSupply(
            tierID
        );
    }
    ...
    function updateTierPurchaseRegistration(
        ...
        require(
            maxTokenIDToSell <= Royal1155LDA(ldaContract).tierMaxSupply(
                tierID
            ),
    ...
    /**
     * @dev sets the next tokenID that will be listed for sale in this tier. This is to
     *      allow us to have control over which tokens of the tier are actually for sale.
     */
