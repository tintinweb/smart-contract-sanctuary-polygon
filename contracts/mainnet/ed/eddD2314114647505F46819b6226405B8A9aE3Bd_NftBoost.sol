...
contract NftBoost is Ownable, ReentrancyGuard, IERC1155Receiver {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public nexfi20Address = 0x6169dfd6fC64AB89B1e0328a03aaDa3A71701540;
    ...
    // Extra Boost
    mapping(address => mapping(uint256 => Nexfi20.BoostInfo)) private extraBoosts;
    ...
    struct UserInfo {
        ...
        uint256 nftTokenId;     // deposited main NFT item token ID
        NftInfo[] nftItems;     // deposited sub NFT item info
        Nexfi20.NftInfo[] nftItemsOrigin;   // deposited sub NFT item on Original Contract
    }
    ...
    function setNx20ContractAddress(address _address) external onlyOwner {
        nexfi20Address = _address;
    }
    ...
    function canDeposit() public view returns (bool available){
        available = !suspended 
                    && Nexfi20(nexfi20Address).canDeposit(); 
    }

    function cloneExtraBoost() external onlyOwner {
        Nexfi20.BoostInfo[] memory _boostInfoList = getExtraBoostList();
        for (uint i=0; i<_boostInfoList.length; i++) {
            Nexfi20.BoostInfo memory info = _boostInfoList[i];
            extraBoosts[info.nftContractAddress][info.nftTokenId] = info;
            // update set item category count
            if (numSetCategory <= info.setItemCategory) {
                numSetCategory = info.setItemCategory + 1;
            }
            setItemCategoryBonus[info.setItemCategory] = info.setItemBonus;
    ...
    function depositSubNft(
        ...
        UserInfo storage user = getUserInfo(_msgSender());
        ...
        uint256 currentStakedNftCount = 0;
        uint256 availableIndex = type(uint256).max;
        NftInfo memory _info;
        Nexfi20.NftInfo memory _info20;
        for (uint i=0; i<user.nftItemsOrigin.length; i++) {
            _info = user.nftItems[i];
            _info20 = user.nftItemsOrigin[i];

            if (_info20.nftAddress != address(0)) {
                currentStakedNftCount++;
            } else if (_info.nftAddress != address(0)) {
                currentStakedNftCount++;
            } else {
                availableIndex = i;
                break;
        ...
        require(
            availableIndex < maxSubNftLimit,
            "no space to deposit sub NFT"
        );
        ...
        _nftInfo.nftAddress = _nftAddress;
        _nftInfo.nftTokenId = _nftTokenId;
        _nftInfo.isStakedInThisContract = true;
        user.nftItems[availableIndex] = _nftInfo;
    ...
    function withdrawSubNftAll() public nonReentrant {
        ...
        UserInfo memory user = userInfo[_msgSender()];
        for (uint i=0; i<user.nftItems.length; i++) {
            if (user.nftItems[i].isStakedInThisContract) {
                _withdrawSubNft(i);
            }
        }
    }
    ...
    function _withdrawSubNft(uint _index) private {
        ...
        if (_index < user.nftItems.length) {
            NftInfo memory info = user.nftItems[_index];
            require(info.isStakedInThisContract, "Invalid request");
            if (info.nftAddress != address(0)) {
                IERC1155(info.nftAddress).safeTransferFrom(
                    address(this),
                    _msgSender(),
    ...
    {
        UserInfo memory user = viewUserInfo(account);
        // nftItems = user.nftItems;
        nftItems = new NftInfo[](maxSubNftLimit);
        
        // clone items in nexfi20
        for (uint i=0; i<user.nftItemsOrigin.length; i++) {
            NftInfo memory _info;
            _info.nftAddress = user.nftItemsOrigin[i].nftAddress;
            _info.nftTokenId = user.nftItemsOrigin[i].nftTokenId;
            nftItems[i] = _info;
        }
        // clone items in this contract
        for (uint i=0; i<user.nftItems.length; i++) {
            if (user.nftItems[i].nftAddress != address(0)) {
                NftInfo memory _info;
                _info.nftAddress = user.nftItems[i].nftAddress;
                _info.nftTokenId = user.nftItems[i].nftTokenId;
                _info.isStakedInThisContract = user.nftItems[i].isStakedInThisContract;
                nftItems[i] = _info;
            }
        }
    ...
    function getUserInfo(address account) internal returns (UserInfo storage user) {
        UserInfo memory _user = viewUserInfo(account);
        user = userInfo[_msgSender()];
        if (!user.registered) {
            user.registered = true;
            user.addr = _user.addr;
            userList.push(user.addr);

            // user.nftItems = new NftInfo[](maxSubNftLimit);
            // user.nftItemsOrigin = new Nexfi20.NftInfo[](maxSubNftLimit);
            for (uint i=0; i<maxSubNftLimit; i++) {
                NftInfo memory info;
                Nexfi20.NftInfo memory info20;
                user.nftItems.push(info);
                user.nftItemsOrigin.push(info20);
            }
        }

        // clone
        user.nftAddress = _user.nftAddress;
        user.nftTokenId = _user.nftTokenId;
        for (uint i=0; i<_user.nftItemsOrigin.length; i++) {
            user.nftItemsOrigin[i] = _user.nftItemsOrigin[i];
        }

    }

    function viewUserInfo(address account) public view 
        returns (
            UserInfo memory user
        )
    {
        (
            ...
            address nftAddress,
            uint256 nftTokenId
        ) = Nexfi20(nexfi20Address).userInfo(account);

        user = userInfo[account];
        if (!user.registered) {
            user.addr = addr;
        }
        user.nftAddress = nftAddress;
        user.nftTokenId = nftTokenId;
        user.nftItemsOrigin = Nexfi20(nexfi20Address).getUserSubNftItems(account);

    }


    function lockupPendingreward() internal {
        // INexfi20(nexfi20Address).deposit(0);
    }
    ...
    function getUserTotalBonus(address account) public view 
        returns (
            uint256 boost
        ) {
        UserInfo memory user = viewUserInfo(account);
        NftInfo[] memory nftInfo = new NftInfo[](maxSubNftLimit);
        Nexfi20.BoostInfo[] memory setBonuses = new Nexfi20.BoostInfo[](maxSubNftLimit + 1);

        boost = Nexfi20(nexfi20Address).getExtraBoost(user.nftAddress, user.nftTokenId);
        if (boost == 0) {
            return 0;
        }
        ...
        uint[] memory setCategories = new uint[](numSetCategory);
        setCategories[setCategory] += 1;
        setBonuses[0] = extraBoosts[user.nftAddress][user.nftTokenId];
        ...
        for (uint i=0; i<nftInfo.length; i++) {
            address _nftAddress = nftInfo[i].nftAddress;
            uint256 _nftTokenId = nftInfo[i].nftTokenId;

            boost = boost.add(
                extraBoosts[_nftAddress][_nftTokenId].subBoostPercent
            );
            setCategories[extraBoosts[_nftAddress][_nftTokenId].setItemCategory] += 1;
            setBonuses[i+1] = extraBoosts[_nftAddress][_nftTokenId];
        }
        ...
        for (uint i=0; i<setCategories.length; i++) {
            if (setCategories[i] > 1) {
                ...
                uint256 _nftTokenId;
                uint count = 0;
                for (uint j=0; j<setBonuses.length; j++) {
                    Nexfi20.BoostInfo memory b = setBonuses[j];
                    if (b.setItemCategory == i) {
                        if (_nftAddress == address(0)) {
                            _nftAddress = b.nftContractAddress;
                            _nftTokenId = b.nftTokenId;
                            count = count + 1;
                        } else {
                            if (_nftAddress != b.nftContractAddress || _nftTokenId != b.nftTokenId) {
                ...
                boost = boost.add(setItemCategoryBonus[i].mul(bonusCount));
            }
        }
    }
    ...
    function getExtraBoostList() public view 
        returns (
            Nexfi20.BoostInfo[] memory _boostInfoList
        )
    {
        _boostInfoList = Nexfi20(nexfi20Address).getExtraBoostList();
    }
