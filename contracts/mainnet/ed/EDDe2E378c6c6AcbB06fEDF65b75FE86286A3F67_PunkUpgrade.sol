...
contract PunkUpgrade is Ownable {
    ...
    function freighPunkStats(uint256 punkID)
        ...
        returns (TokenStats memory)
    {
        return getPunkStats[punkID];
    }

    function setPunkStats(
        uint256 punkID,
        uint256 racesJoined,
        uint256 firstPlaces,
        uint256 secondPlaces,
        ...
        getPunkStats[punkID].racesJoined = racesJoined;
        getPunkStats[punkID].firstPlaces = firstPlaces;
        getPunkStats[punkID].secondPlaces = secondPlaces;
        getPunkStats[punkID].thirdPlaces = thirdPlaces;
        getPunkStats[punkID].tokensWon = tokensWon;
    }

    mapping(uint256 => TokenStats) private getPunkStats;
    ...
    function skillPointsEarned(uint256 _punkID) public view returns (uint256) {
        uint256 totalJoined = freighPunkStats(_punkID).racesJoined *
            baseXPEarned;

        uint256 firstPlaceEarned = freighPunkStats(_punkID).firstPlaces *
            xpEarnedFormula[1];
        uint256 secondPlaceEarned = freighPunkStats(_punkID).secondPlaces *
            xpEarnedFormula[2];
        uint256 thirdPlaceEarned = freighPunkStats(_punkID).thirdPlaces *
            xpEarnedFormula[2];
