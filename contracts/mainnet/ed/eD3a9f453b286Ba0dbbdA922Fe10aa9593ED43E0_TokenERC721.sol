...
contract TokenERC721 is CommonERC721 {

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    mapping (uint256 => bool) public withdrawnTokens;

    constructor(string memory name, string memory symbol) CommonERC721(name, symbol) {
    ...
    function mint(address user, uint256 tokenId) public only(MINTER_ROLE) {
        require(!withdrawnTokens[tokenId], "PolygonERC721: TOKEN_EXISTS_ON_ROOT_CHAIN");
        _mint(user, tokenId);
    }

    function mintBatch(address user, uint256[] memory tokenIds) public only(MINTER_ROLE) {
        for (uint256 i = 0; i < tokenIds.length; i += 1) {
            require(!withdrawnTokens[tokenIds[i]], "PolygonERC721: TOKEN_EXISTS_ON_ROOT_CHAIN");
...
contract CommonERC721 is ERC721Enumerable, Pausable, AccessControl {
    ...
    function setBaseURI(string memory baseURI_) external only(DEFAULT_ADMIN_ROLE) {
        __baseURI = baseURI_;
    }

    function setTokenURI(uint256 tokenId, string memory _tokenURI) public only(DEFAULT_ADMIN_ROLE) {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return string(abi.encodePacked(__baseURI, uint2str(tokenId), '.json'));
    }
    ...
    function burn(uint256 tokenId) public only(BURNER_ROLE) {
        _burn(tokenId);
    }
