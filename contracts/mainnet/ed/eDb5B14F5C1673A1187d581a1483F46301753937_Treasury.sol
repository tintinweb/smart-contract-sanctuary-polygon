...
contract Treasury is ITreasury, Ownable {
    ...
    address public LIMITATION_TOKEN;
    uint256 public LIMITATION_FEE;
    ...
    constructor(
        ...
        address _limitation_token
    ) {
        setAdapters(_adapters);
        WNATIVE = _wrapped_native;
        LIMITATION_TOKEN = _limitation_token;
        FEE = 5;
        LIMITATION_FEE = 10;
    }
    ...
    function setLimitationToken(address _limitation_token) external onlyOwner {
        LIMITATION_TOKEN = _limitation_token;
    }

    function setLimitationFee(uint256 _limitation_fee) external onlyOwner {
        LIMITATION_FEE = _limitation_fee;
    }
    ...
    function getLowLimit(address _token) public view returns (uint256) {
        uint256 _decimals = IERC20(LIMITATION_TOKEN).decimals();
        Query memory bestQuery = queryNoSplit(
            uint256(10**_decimals).mul(LIMITATION_FEE),
            LIMITATION_TOKEN,
            _token
        );
        return bestQuery.amountOut;
    }
    ...
    function receiveFromUser(
        ...
        uint256 _limitation = getLowLimit(payTokenAddress);
        if (amount > _limitation) {
            convertTokenToToken(
                payTokenAddress,
                tokenAddress,
                user,
    ...
    function convertTokenToToken(
        ...
        IERC20(payTokenAddress).safeTransfer(_query.adapter, payAmount);
    ...
    function convertEthToCoin(address user, address _tokenAddress) internal {
        ...
        uint256 _limitation = getLowLimit(WNATIVE);
        if (amountIn > _limitation) {
            _wrap(amountIn);
