...
contract Strategy is BaseStrategy, IUniswapV3MintCallback, IUniswapV3SwapCallback {
    ...
    int24 public limitTickWidth = 20;
    int24 public maxLimitTickDeviation = 100;

    int24 public baseTickWidth = 300;
    int24 public maxBaseTickDeviation = 300;

    uint8 public basePositionRatio = 8;
    ...
    uint256 public slippageMax = 200;
    ...
    function name() external view override returns (string memory) {
        return 'Strategy Max Uniswap_V3 USDC-wETH';
    }
    ...
    function balanceOfWethInWant() public view returns (uint256) {
        return wethToWant(balanceOfWeth());
    }

    function wethToWant(uint256 _wethAmount) public view returns (uint256) {
        if (_wethAmount == 0) return 0;
        return wethPrice().mul(_wethAmount).div(1e18);
    }

    function wantToWeth(uint256 _wantAmount) public view returns (uint256) {
        if (_wantAmount == 0) return 0;
        return _wantAmount.mul(1e18).div(wethPrice());
    }
    ...
    function positionBalance() public view returns (uint256) {
        (uint256 basePosWant, uint256 basePosWeth, ) = _getPosition(baseTicks);
        (uint256 limitPosWant, uint256 limitPosWeth, ) = _getPosition(limitTicks);
        return (basePosWant.add(limitPosWant).add(wethToWant(basePosWeth.add(limitPosWeth))));
    }
    ...
    {
        return _getPosition(baseTicks);
    }

    function estimatedFees() public view returns (uint256, uint256) {
        (, int24 tick, , , , , ) = uniV3Pool.slot0();
        (uint256 wantFee, uint256 wethFee) = _estimateFees(baseTicks, tick);
        (uint256 wantFeeLO, uint256 wethFeeLO) = _estimateFees(limitTicks, tick);
        // Compute current fees earned
        return (wantFee.add(wethToWant(wethFee)), wantFeeLO.add(wethToWant(wethFeeLO)));
    }
    ...
    function baseRebalanceTrigger() public view returns (bool) {
        int24 lowerTick = limitTicks.lowerTick;
        int24 upperTick = limitTicks.upperTick;
        int24 tick = currentTick();
        if (
            (tick > lowerTick && tick > upperTick + maxBaseTickDeviation) ||
            (tick < upperTick && tick < lowerTick - maxBaseTickDeviation)
        ) {
            return true;
        }
    ...
    function limitRebalanceTrigger() public view returns (bool) {
        int24 lowerTick = baseTicks.lowerTick;
        int24 upperTick = baseTicks.upperTick;
        int24 tick = currentTick();

        if (
            (tick > lowerTick && tick > upperTick + maxLimitTickDeviation) ||
            (tick < upperTick && tick < lowerTick - maxLimitTickDeviation)
        ) {
            return true;
        }
    ...
    function summary()
        ...
        returns (
            uint256 basePosWant,
            uint256 basePosToken1,
            uint256 limitPosWant,
            uint256 limitPosToken1,
            uint256 baseFees,
            uint256 limitFees
        )
    {
        (basePosWant, basePosToken1, ) = _getPosition(baseTicks);
        (limitPosWant, limitPosToken1, ) = _getPosition(limitTicks);
        (baseFees, limitFees) = estimatedFees();
    }
    ...
    function setOrders() external onlyAuthorized {
        (uint256 fees0, uint256 fees1) = _unwindPosition(baseTicks);
        (uint256 fees0LO, uint256 fees1LO) = _unwindPosition(limitTicks);
        // Need to swap half want to ETH, but use the current exchange rate in the LP
        _setBasePosition(basePositionRatio);
        _setLimitOrder();

        totalFees = totalFees.add(fees0).add(fees0LO).add(wethToWant(fees1.add(fees1LO)));

        emit BasePosRebalance(
            currentTick(),
            baseTicks.lowerTick,
            baseTicks.upperTick,
            fees0,
        ...
        emit LimitPosRebalance(
            currentTick(),
            limitTicks.lowerTick,
            limitTicks.upperTick,
            fees0LO,
            fees1LO
        );
    }
    ...
    function renewLimitOrder() external onlyStupidRobot {
        (uint256 fees0, uint256 fees1) = _unwindPosition(limitTicks);
        _setLimitOrder();
        emit LimitPosRebalance(
            currentTick(),
            limitTicks.lowerTick,
        ...
        totalFees = totalFees.add(fees0).add(wethToWant(fees1));
    }

    function rebalanceBasePosition() external onlyStupidRobot {
        (uint256 fees0, uint256 fees1) = _unwindPosition(baseTicks);
        _setBasePosition(10);
        emit BasePosRebalance(
            currentTick(),
        ...
        )
    {
        ...
        if (wethFees > wethDust) _swapToWant(wethFees);
    ...
    function _setBasePosition(uint8 _ratio) internal {
        uint256 wantBal = balanceOfWant();
        // We take either 80% or 100% for the basePosition.
        uint256 posWant = wantBal.mul(_ratio).div(10);
        uint256 posWethInWant = wethToWant(balanceOfWeth());

        uint256 wantToSwap = 0;
        uint256 wethToSwap = 0;
        if (posWant > posWethInWant) {
            wantToSwap = posWant.sub(posWethInWant).div(2);
        } else {
            wethToSwap = wantToWeth(posWethInWant.sub(posWant)).div(2);
        }

        if (wethToSwap == 0) _swapToWeth(wantToSwap);
        else _swapToWant(wethToSwap);
    ...
    function _setBaseTicks() internal {
        int24 tickFloor = _floor(currentTick());
        int24 tickCeil = tickFloor + tickSpacing;
        baseTicks = Ticks({
            lowerTick: tickFloor - baseTickWidth,
            upperTick: tickCeil + baseTickWidth
        });
    }
    ...
    function liquidateAllPositions() internal override returns (uint256) {
        ...
        if (balanceOfWeth() > wethDust) _swapToWant(balanceOfWeth());
    ...
    function _withdrawSome(uint256 _initialWantNeeded) internal {
        ...
        if (amountNeeded > 0) {
            _withdrawExactFromPosition(amountNeeded, limitTicks);
        }
        ...
        if (amountNeeded > 0) {
            _withdrawExactFromPosition(amountNeeded, baseTicks);
        }
    }

    function _withdrawExactFromPosition(uint256 amountNeeded, Ticks memory _ticks) internal {
        (, , uint128 _liquidity) = _getPosition(_ticks);
        if (_liquidity > 0) {
            uint256 fraction = amountNeeded.mul(1e18).div(_getBalance(_ticks));
            ...
            _burnAndCollectLiquidity(liquidityToBurn, _ticks);
    ...
    function _swapNeededWeth(uint256 _wantNeeded) internal {
        if (_wantNeeded > 0) {
            uint256 wethToSell = Math.min(wantToWeth(_wantNeeded), balanceOfWeth());
    ...
    function _calculateLimitOrder()
        internal
        view
        returns (uint128 liquidity, Ticks memory _limitTicks)
    {
        (uint256 _wantBalance, uint256 _wethBalance) = _getLimitOrderAmounts();
        (Ticks memory bid, Ticks memory ask) = _getBidAsk();

        uint128 bidLiquidity = _liquidityForAmount0(bid.lowerTick, bid.upperTick, _wantBalance);
        uint128 askLiquidity = _liquidityForAmount1(ask.lowerTick, ask.upperTick, _wethBalance);
    ...
    function _swapToWant(uint256 _amountIn) internal returns (int256, int256) {
        uint256 wantBalBefore = balanceOfWant();
        (int256 amount0, int256 amount1) = _swap(int256(_amountIn), false);
        _enforceSlippage(wethToWant(_amountIn), balanceOfWant().sub(wantBalBefore), slippageMax);
        return (amount0 * -1, amount1);
    }

    function _swapToWeth(uint256 _amountIn) internal returns (int256, int256) {
        uint256 ethBalBeforeInWant = balanceOfWethInWant();
        uint256 toSwap = Math.min(_amountIn, balanceOfWant());
        if (toSwap > wantDust) {
            (int256 amount0, int256 amount1) = _swap(int256(toSwap), true);
            _enforceSlippage(toSwap, balanceOfWethInWant().sub(ethBalBeforeInWant), slippageMax);
            return (amount0, amount1 * -1);
        }
    }

    function _swap(int256 _amountIn, bool _in) internal returns (int256 amount0, int256 amount1) {
    ...
    function _unwindPosition(Ticks memory _tick) internal returns (uint256, uint256) {
        (uint128 _liquidity, , ) = _position(_tick);

        if (_liquidity > 0) {
            return _burnAndCollectLiquidity(_liquidity, _tick);
        }
    }

    function _setLimitOrder() internal {
        (uint128 liquidity, Ticks memory _limitTicks) = _calculateLimitOrder();
        _mintLiquidity(liquidity, _limitTicks);
        limitTicks = _limitTicks;
    }

    function _addLiquidity(uint256 amount0, uint256 amount1) internal returns (uint256, uint256) {
        Ticks memory _baseTicks = baseTicks;
        return _mintLiquidity(_liquidityForAmounts(_baseTicks, amount0, amount1), _baseTicks);
    }
        ...
        returns (uint256 wantBalance, uint256 wethBalance)
    {
        ...
        if (wantBalance < wantDust || wethBalance < wethDust) return (wantBalance, wethBalance);
    }
    ...
    function _getBalance(Ticks memory _ticks) internal view returns (uint256) {
        (uint256 totalUsdcBalance, uint256 totalWethBalance, ) = _getPosition(_ticks);
        return wethToWant(totalWethBalance).add(totalUsdcBalance);
    }

    // function _wethPrice() internal view returns (uint256) {
    //     return _oracle ? aaveOracle.getAssetPrice(address(wETH)).div(100) : wethPrice();
    // }
    ...
    function setBaseTickParams(int24 _tickWidth, int24 _maxTickDeviation)
        external
        onlyVaultManagers
    {
        require(_tickWidth % tickSpacing == 0);
        baseTickWidth = _tickWidth;
        maxBaseTickDeviation = _maxTickDeviation;
    }

    function setLimitTickParams(int24 _tickWidth, int24 _maxTickDeviation)
        external
        onlyVaultManagers
    {
        require(_tickWidth % tickSpacing == 0);
        limitTickWidth = _tickWidth;
        maxLimitTickDeviation = _maxTickDeviation;
    }

    function setBasePositionRatio(uint8 _ratio) external onlyVaultManagers {
        basePositionRatio = _ratio;
    }
