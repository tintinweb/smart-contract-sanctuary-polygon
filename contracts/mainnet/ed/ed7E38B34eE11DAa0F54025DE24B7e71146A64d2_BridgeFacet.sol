...
library LibStargate {
    ...
    function bridgeOut(BridgeOutArgs memory bridgeOutArgs) internal returns (uint256 amount) {
        ...
        amount = IMagpieStargateBridge(
            address(uint160(uint256(s.magpieStargateBridgeAddresses[bridgeOutArgs.transferKey.networkId])))
        ).withdraw(
...
library LibWormhole {
    ...
    function bridgeOut(BridgeOutArgs memory bridgeOutArgs) internal returns (uint256 amount) {
        ...
        LibTransferKey.validate(bridgeOutArgs.transferKey, LibTransferKey.decode(vm.payload.slice(165, 207)));
