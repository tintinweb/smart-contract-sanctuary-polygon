...
contract JWaveListenPlusCollection is BaseNFT {
    using Counters for Counters.Counter;

    uint256 private constant _minTokenType = 1;
    uint256 private _maxTokenType;
    bool private _isMaxTokenTypeFrozen;
    ...
    constructor() BaseNFT("J-WAVE LISTEN+ COLLECTION", "JWLC") {}

    function setMaxTokenType(uint256 maxTokenType_) external onlyOwner {
        _requireMaxTokenTypeNotFrozen();

        _maxTokenType = maxTokenType_;
    }

    function freezeMaxTokenType() external onlyOwner {
        _requireMaxTokenTypeNotFrozen();

        _isMaxTokenTypeFrozen = true;
    }
    ...
    function airdropByType(
        ...
        require(!_isAirdroppeds[tokenType_][to_], "JWLC: already airdropped");
    ...
    function _requireMaxTokenTypeNotFrozen() private view {
        require(!_isMaxTokenTypeFrozen, "JWLC: max token type frozen");
    }

    function _requireValidTokenType(uint256 tokenType_) private view {
        require(
            _minTokenType <= tokenType_ &&
                (_maxTokenType == 0 || tokenType_ <= _maxTokenType),
            "JWLC: invalid token type"
        );
    }
