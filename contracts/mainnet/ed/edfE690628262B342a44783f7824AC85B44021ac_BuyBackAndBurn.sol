...
contract BuyBackAndBurn is MyPausableUpgradeable, IBuyBackAndBurn {
  ...
  uint256 public collectedNativeTokenToBurn; // deprecated in v1.2.2

  /// Amount of ERC20 tokens collected per token
  mapping(address => uint256) public collectedERC20ToBurn; // deprecated in v1.2.2

  /// address of the ERC20 cross-chain bridge contract
  ICrossChainBridgeERC20 public override bridgeERC20;
  ...
  function initialize(IERC20 _burnToken, IRouter _router) external initializer {
    ...
    _setupRole(MANAGE_CONTRACTS_ROLE, _msgSender());
  ...
  /**
   * @notice Swaps all collected ERC20 tokens (of given type) into burn tokens using an external DEX and burns them
   *
   ...
   * @param amount amount of ERC20 tokens that should be transferred
  ...
  function setBridgeERC20(ICrossChainBridgeERC20 _bridgeERC20) external {
    if (!hasRole(MANAGE_CONTRACTS_ROLE, _msgSender())) revert Unauthorized();
  ...
  UUPSUpgradeable
{
  ...
  function _authorizeUpgrade(address) internal view override {
    if (!hasRole(MANAGE_UPGRADES_ROLE, _msgSender())) revert Unauthorized();
  }
  ...
  function pause() external whenNotPaused {
    if (!hasRole(PAUSABILITY_PAUSE_ROLE, _msgSender())) revert Unauthorized();
  ...
  function unpause() external whenPaused {
    if (!hasRole(PAUSABILITY_UNPAUSE_ROLE, _msgSender())) revert Unauthorized();
...
interface ICrossChainBridgeERC20 {
  ...
  function wrappedNative() external returns (INativeToken); // added in v1.1
}
...
interface IBuyBackAndBurn {
  ...
  function bridgeERC20() external returns (ICrossChainBridgeERC20); // added in v1.1
}
...
contract Bridge is ERC20Upgradeable, MyPausableUpgradeable {
  ...
  function mint(address to, uint256 amount) external whenNotPaused {
    if (!hasRole(MANAGE_TOKEN_ROLE, _msgSender())) revert Unauthorized();
    _mint(to, amount);
  }
  ...
  function burnFrom(address from, uint256 amount) external virtual whenNotPaused {
    ...
    if (allowance(from, _msgSender()) < amount) revert InvalidBalance();
...
/// Invalid message value transferred. Needed `required` but sent `amount`
/// @param sent sent amount.
/// @param required amount required.
error InvalidMessageValue(uint256 sent, uint256 required);
