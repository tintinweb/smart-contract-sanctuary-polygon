...
contract QuickDragonsLairNewYA is BaseYA, UniswapV2Giddy, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  uint256 constant private COMPOUND_THRESHOLD = 1000e18;
  ...
  function sharesToValue(uint256 shares) public view override returns (uint256) {
    ...
    uint256 pendingFees = (getContractBalance() - lastContractBalance) * GIDDY_CONFIG.earningsFee() / BASE_PERCENT;
  ...
  function depositCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    route[0] = USDC_TOKEN;
    route[1] = WMATIC_TOKEN;
    route[2] = QUICK_TOKEN;
    (amountOut, priceImpact) = calcPrice(QUICK_ROUTER, route, value - fap);
  }
  ...
  function deposit(bytes calldata auth, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    route[1] = WMATIC_TOKEN;
    route[2] = QUICK_TOKEN;
    uint256 valueQuick = swapTokens(QUICK_ROUTER, route, address(this), value);

    uint256 shares = contractShares == 0 ? valueQuick * INIT_SHARES : valueQuick * contractShares / getContractBalance();
  ...
  function depositGiddyCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    uint256 outBefore = value - fap;
    amountOut = outBefore;
    (outBefore, amountOut) = calcSwap(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, outBefore, amountOut);
    (outBefore, amountOut) = calcSwap(QUICK_ROUTER, USDC_TOKEN, WMATIC_TOKEN, outBefore, amountOut);
    (outBefore, amountOut) = calcSwap(QUICK_ROUTER, WMATIC_TOKEN, QUICK_TOKEN, outBefore, amountOut);
  ...
  function depositGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    value = swapTokensSimple(QUICK_ROUTER, USDC_TOKEN, WMATIC_TOKEN, address(this), value);
    value = swapTokensSimple(QUICK_ROUTER, WMATIC_TOKEN, QUICK_TOKEN, address(this), value);

    uint256 shares = contractShares == 0 ? value * INIT_SHARES : value * contractShares / getContractBalance();
    userShares[req.owner] += shares;
    contractShares += shares;

    if (!IERC20(QUICK_TOKEN).approve(DQUICK_TOKEN_AND_LAIR, value)) {
      revert();
    }
    IDragonsLair(DQUICK_TOKEN_AND_LAIR).enter(value);
  ...
  function withdrawCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    route[1] = WMATIC_TOKEN;
    route[2] = USDC_TOKEN;
    (amountOut, priceImpact) = calcPrice(QUICK_ROUTER, route, sharesToValue(shares));
    amountOut -= fap;
  }
  ...
  function withdraw(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    route[1] = WMATIC_TOKEN;
    route[2] = USDC_TOKEN;
    value = swapTokens(QUICK_ROUTER, route, address(this), value);
  ...
  function withdrawGiddyCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    uint256 outBefore = sharesToValue(shares);
    amountOut = outBefore;
    (outBefore, amountOut) = calcSwap(QUICK_ROUTER, QUICK_TOKEN, WMATIC_TOKEN, outBefore, amountOut);
    (outBefore, amountOut) = calcSwap(QUICK_ROUTER, WMATIC_TOKEN, USDC_TOKEN, outBefore, amountOut);
    (outBefore, amountOut) = calcSwap(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, outBefore, amountOut);
    priceImpact = outBefore * BASE_PERCENT / amountOut;
    if (priceImpact > BASE_PERCENT) {
      priceImpact -= BASE_PERCENT;
    }
  ...
  function withdrawGiddy(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    value = swapTokensSimple(QUICK_ROUTER, QUICK_TOKEN, WMATIC_TOKEN, address(this), value);
    value = swapTokensSimple(QUICK_ROUTER, WMATIC_TOKEN, USDC_TOKEN, address(this), value);
  ...
  modifier deductEarningsFee() {
    ...
    if (earnings > 0) {
      uint256 feeQUICK = earnings * GIDDY_CONFIG.earningsFee() / BASE_PERCENT;
      if (feeQUICK > 0) {
      ...
      if (IERC20(QUICK_TOKEN).balanceOf(address(this)) >= COMPOUND_THRESHOLD) {
        swapTokensSimple(QUICK_ROUTER, QUICK_TOKEN, USDC_TOKEN, GIDDY_CONFIG.feeAccount(), IERC20(QUICK_TOKEN).balanceOf(address(this)));
      }
