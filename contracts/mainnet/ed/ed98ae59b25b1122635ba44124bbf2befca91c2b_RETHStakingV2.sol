...
contract RETHStakingV2 is RETHStaking {
    ...
    constructor(IERC20 add_, RETHStaking staking) RETHStaking(add_){
        STAKINGV1 = staking;
    }

    function migrateStakes(address[] memory wallet) public onlyAuth {
        for(uint256 i=0;i<wallet.length;i++){
            if(!migrated[wallet[i]]){
                migrated[wallet[i]]=true;
            stakes[] memory staked = STAKINGV1.getStakes(wallet[i]);
            for(uint256 j=0;j<staked.length;j++){
                userstakes[msg.sender]++;
    ...
    function migrateStake(address wallet, uint256 stakeId) public onlyAuth {
        require(getStakes(wallet).length==stakeId, "Already Migrated");
        stakes[] memory staked = STAKINGV1.getStakes(wallet);
        userstakes[msg.sender]++;
        Stakes[wallet].push(staked[stakeId]);
        emit LogMigration(staked[stakeId]);
    }
