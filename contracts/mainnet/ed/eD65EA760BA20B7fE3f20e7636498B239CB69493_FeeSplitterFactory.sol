...
contract FeeSplitterFactory is Owned {
address factory;
//WETH MATIC MAINNET
...
function createFeeSplitter(address _factory) public onlyFactory() returns (FeeSplitter) {
    return new FeeSplitter(_factory, _router);
}
function createStaking(RootedToken root, string calldata xToken, string calldata xSymbol) public onlyFactory() returns (StakingToken) {
    return new StakingToken(root, xToken, xSymbol);
}

function createRoyaltyPump(address _factory, address staking, address root, RootedTransferGate gate) public onlyFactory() returns (RoyaltyPump) {
    return new RoyaltyPump(_factory, _router, signedLiquidityController, staking, root, gate);
}
}
