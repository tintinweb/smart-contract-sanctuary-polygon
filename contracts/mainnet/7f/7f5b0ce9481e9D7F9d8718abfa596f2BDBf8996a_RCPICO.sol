...
contract RCPICO is Ownable, ReentrancyGuard {
    ...
    // Buys tokens from the contract
    function buyTokens(
        uint256 _amount
    ) public payable nonReentrant returns (bool success) {
        require(
            IERC20(tokenAddress).balanceOf(address(this)) >= _amount,
            "ICO: Insufficient tokens at the contract."
        );
        require(
            msg.value >= (_amount * tokenPrice) / (10 ** 18),
            "ICO: Insufficient money"
        );
