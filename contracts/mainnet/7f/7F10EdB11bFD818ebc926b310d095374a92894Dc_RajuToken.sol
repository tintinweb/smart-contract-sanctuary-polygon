...
contract RajuToken is Initializable {
    ...
    function _specialTransfer(address sender, address recipient, uint256 amount) internal {
        ...
        if ((sender != pair) && (recipient != pair)) {
            if ((block.timestamp >= _phase1starttime) && (block.timestamp < _phase2starttime)) {
                require((sender == _owner) || (recipient == _owner), "Only the owner can transfer tokens to others and others can only transfer tokens to the owner.");
                _tokenTransfer(sender, recipient, amount);
                emit Transfer(sender, recipient, amount);
            } else if (block.timestamp >= _phase2starttime)
            {
