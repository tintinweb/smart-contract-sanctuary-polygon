...
contract HappyRedPacket is Initializable {
    ...
    public returns (uint claimed) {
        ...
        uint minium_value = IERC20(token_address).decimals() / 10**2;

        // check whether claimed_tokens reach minium value
        if ( claimed_tokens < minium_value){
            claimed_tokens = minium_value;
        }else{
            // Rounding
            claimed_tokens = claimed_tokens - (claimed_tokens % minium_value);
        }
