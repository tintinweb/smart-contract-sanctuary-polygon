...
contract Oracle is Controllable, IOracle {
  ...
  function onlyGovernance() internal view {
    require(isGovernance(msg.sender), "Oracle: Not gov");
  }
  ...
  function _getRandomNumber(uint maxValue, uint seed) internal returns (uint) {
    require(maxValue != 0, "Oracle: Wrong input");
    // pseudo random number
    uint r = (uint(keccak256(abi.encodePacked(blockhash(block.number), block.coinbase, block.difficulty, block.number, block.timestamp, msg.sender, tx.gasprice, gasleft(), seed))) % (maxValue + 1));
    emit Random(r, maxValue);
    return r;
  }
