...
contract MortgageControl is AccessControl {
    ...
    {   
        ...
        vars._quantity= usermortgage[vars._user][vars._mortgageId].quantity,
    ...
    function addRegistry(uint256 id, address wallet, address _collection, address _wrapContract,uint256 _nftId, uint256 _loan, uint256 _quantity,
    uint256 _downPay,uint256 _price,uint256 _startDate,uint256 _period ) public onlymortgage {
        ...
        usermortgage[wallet][id].quantity = _quantity;
    ...
    function updateRedeem(uint256 id, address wallet) public onlymortgage { 
        usermortgage[wallet][id].redeemed = true;
    }

    function getRedeemStatus(uint256 id, address wallet) public view returns(bool _state) { 
        return usermortgage[wallet][id].redeemed;
    }
    ...
    uint256 _startDate,uint256 _period, bool _mortageState) public onlymortgage {
        ...
        (vars._collection,vars._nftId, vars._wrapContract, vars._quantity, , , ,
        ...
        usermortgage[wallet][newId].quantity = vars._quantity;
    ...
    function addNormalMorgateInterestData( 
        address _wallet,
        uint256 _idMortgage,
        MortgageInterest calldata _mortgage
    ) public onlymortgage {
    ...
    function addDelayedMorgateInterestData(
      ...
      function updateOnPayMortgageInterest(
        address _wallet,
        uint256 _idMortgage,
        MortgageInterest calldata mort
    ) public onlymortgage {
        ...
        userToMortgageInterest[_wallet][_idMortgage].totalPanoramLiquidation = mort
            .totalPanoramLiquidation;
        ...
        userToMortgageInterest[_wallet][_idMortgage]
            .lastTimePayment = mort.lastTimePayment;
    }
    ...
    function getMortgagesForWallet(address _wallet, address _collection) public view returns (uint256[] memory _idMortgagesForCollection){
        // sacamos los mortgage que tiene
        uint24 counter = 0;
    ...
    function updatePaidStatusPruebas(address _wallet, uint256 _IdMortgage, MortgageInterest calldata mort) public onlymortgage{
        userToMortgageInterest[_wallet][_IdMortgage].lastTimePayment = mort.lastTimePayment;
        userToMortgageInterest[_wallet][_IdMortgage].lastTimeCalc = mort.lastTimeCalc;
        userToMortgageInterest[_wallet][_IdMortgage].strikes = mort.strikes;
        ...
        userToMortgageInterest[_wallet][_IdMortgage].liquidate = mort.liquidate;
    }

    function updatedliquidate(address _wallet, uint256 _IdMortgage, bool _state) public {
        if (!hasRole(DEV_ROLE, msg.sender)) {
            revert("have no admin role");
        }
        userToMortgageInterest[_wallet][_IdMortgage].liquidate = _state;
    }

    function updatedTotalDebt(address _wallet, uint256 _IdMortgage, uint256 _amount) public {
        if (!hasRole(DEV_ROLE, msg.sender)) {
            revert("have no admin role");
        }
        userToMortgageInterest[_wallet][_IdMortgage].totalDebt = _amount;
    }
    ...
    function updateFlag() public onlyMigrate onlymortgage{
        flag = true;
    }
    
    function migrateMortgateInterest(address _wallet, uint256 _IdMortgage, MortgageInterest calldata mort) public onlyMigrate onlymortgage{
        userToMortgageInterest[_wallet][_IdMortgage] = mort;
    }
    
    function migrateMortgateDebt(address _wallet, uint256 _IdMortgage, Information calldata _debt) public onlyMigrate onlymortgage {
             usermortgage[_wallet][_IdMortgage] = _debt;
    }

    ///@dev only for test erase in production 
     function getLoanInfo(address _user, uint256 _mortgageId)
        public
        view
        returns (
            uint256,
