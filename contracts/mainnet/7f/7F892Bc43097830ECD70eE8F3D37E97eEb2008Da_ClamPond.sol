    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// @notice rate of withdraw fee, 100 = (100 / 10000) = 1%
    uint256 public withdrawFeeRate;

    // ---  events
    event Deposit(address account, uint256 amount);
    event DepositPearl(address account, uint256 amount);
    event Withdraw(address account, uint256 amount);
    event WithdrawPearl(address account, uint256 amount);
    ...
    modifier onlyRewardManager() {
        require(msg.sender == rewardManager, 'Restricted to reward manager');
        _;
    }
    ...
    function setRewardManager(address rewardManager_) external onlyAdmin {
        rewardManager = rewardManager_;
        emit RewardManagerUpdated(rewardManager);
    }

    function setWithdrawFeeDuration(uint256 duration_) external onlyAdmin {
        withdrawFeeDuration = duration_;
    }

    function setWithdrawFeeRate(uint256 rate_) external onlyAdmin {
        withdrawFeeRate = rate_;
    }

    // --- logic

    function rebase() external onlyRewardManager {
        // reward paid in USD+
        pearlBank.claimRewards();
        // USD+ -> CLAM
        uint256 rewardAmount = IERC20(usdPlus).balanceOf(address(this));
        if (rewardAmount == 0) {
            return;
        }
        IERC20(usdPlus).approve(swapRouter, rewardAmount);
        uint256[] memory results = IDystopiaRouter(swapRouter)
        ...
        // stake CLAM
        IERC20(CLAM).approve(address(pearlBank), results[1]);
        pearlBank.stake(results[1]);

        uint256 totalClamPondRay = scaledTotalSupply();
        uint256 totalPearlBankRay = pearlBank
            .balanceOf(address(this))
            .wadToRay();
        // // in ray
        uint256 newLiquidityIndex = totalPearlBankRay.rayDiv(totalClamPondRay);
        uint256 delta = (newLiquidityIndex * 1e9) / liquidityIndex;
        // if (delta <= abroadMin) {
    ...
    /// @notice Deposit CLAM to auto-compound more CLAM from revenue
    function deposit(uint256 amount_) external nonReentrant {
        IERC20(CLAM).transferFrom(msg.sender, address(this), amount_);
        IERC20(CLAM).approve(address(pearlBank), amount_);
        pearlBank.stake(amount_);
    ...
    /// @notice Deposit PEARL to auto-compound more CLAM from revenue
    function depositPearl(uint256 amount_) external nonReentrant {
        pearlBank.transferFrom(msg.sender, address(this), amount_);
        _deposit(msg.sender, amount_);

        emit DepositPearl(msg.sender, amount_);
    }

    function _deposit(address account_, uint256 amount_) internal {
        // up to ray
        uint256 mintAmount = amount_.wadToRay();
        mintAmount = mintAmount.rayDiv(liquidityIndex);
        _mint(account_, mintAmount);
        _totalMint += mintAmount;

        depositInfo[account_] = DepositInfo({timestamp: block.timestamp});
    ...
    function withdraw(uint256 amount_) external nonReentrant {
        uint256 withdrawAmount = pearlBank.withdraw(amount_);
        uint256 fee = withdrawFee(msg.sender, withdrawAmount);

        uint256 burnAmount;
        if (amount_ == balanceOf(msg.sender)) {
            // burn all
            burnAmount = _balances[msg.sender];
        } else {
            // up to ray
            burnAmount = amount_.wadToRay();
        ...
        IERC20(CLAM).transfer(msg.sender, withdrawAmount - fee);

        emit Withdraw(msg.sender, amount_);
        emit Transfer(msg.sender, address(0), amount_);
    }

    function withdrawPearl(uint256 amount_) external nonReentrant {
        uint256 burnAmount;
        if (amount_ == balanceOf(msg.sender)) {
            // burn all
            burnAmount = _balances[msg.sender];
        ...
        pearlBank.transfer(msg.sender, amount_);
        emit WithdrawPearl(msg.sender, amount_);
        emit Transfer(msg.sender, address(0), amount_);
    }

    function withdrawFee(address account_, uint256 amount_)
        public
        view
        returns (uint256)
    {
        if (
            block.timestamp - depositInfo[account_].timestamp >
            withdrawFeeDuration
        ) {
            return 0;
        }
    ...
    function _afterTokenTransfer(
        ...
        if (from == address(0)) {
            ...
            depositInfo[to] = DepositInfo({
                timestamp: depositInfo[from].timestamp >
                    depositInfo[to].timestamp
                    ? depositInfo[from].timestamp
                    : depositInfo[to].timestamp
            });
        }
    }
