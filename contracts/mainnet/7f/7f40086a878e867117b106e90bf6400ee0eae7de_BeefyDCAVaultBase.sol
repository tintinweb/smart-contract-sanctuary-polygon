...
contract BeefyDCAVaultBase is ERC721, Ownable, ReentrancyGuard {
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        uint256 ltra = lastTimeRewardApplicable();
        uint256 lupt = lastUpdateTime;
        uint256 one = 1e18;
        uint256 rr = rewardRate;
        uint256 ubt = underlyingBalanceTotal();

        uint256 first = ltra - lupt;
        uint256 second = first * rr;
        uint256 third = second * one; 
        uint256 fourth = third / ubt;
        return rpts + fourth;
    }
