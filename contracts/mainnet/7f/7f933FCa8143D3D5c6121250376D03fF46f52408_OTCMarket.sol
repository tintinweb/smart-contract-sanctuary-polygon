...
contract OTCMarket is Operator, ContractGuard, ReentrancyGuard {
    ...
    //**
//    * @_buy_amount amount of token want to buy
//    * @_tid token Id
//    **//

    function buy(uint256 _tid, uint256 _buy_amount) external nonReentrant {
        SellToken storage sToken = tokenList[_tid];
        require(_buy_amount < sToken.amountForSale && _buy_amount > 0, "Exceed current sell amount");
        (uint256 cost_amount, uint256 amount_for_liquidity) = calcOnBuy(_tid, _buy_amount);
//        uint256 cost_amount = _buy_amount.mul(DENOMINATOR).div(sToken);
        // Calculate
//        uint256 amount_for_liquidity = ILiquidityHelper(liquidityHelper).getEstimateTokenAmountAddLp(sToken.token, sToken.purchaseToken, _buy_amount);
        IERC20(sToken.purchaseToken).safeTransferFrom(msg.sender, address(this), cost_amount.add(amount_for_liquidity));
        ...
        _depositYwAndLockUp(msg.sender, lp_added, _tid);
    ...
    function withdraw(uint256 _tid) external nonReentrant {
        UserLocker storage locker = userLocker[_tid][msg.sender];
        SellToken storage sToken = tokenList[_tid];
        require(block.timestamp >= locker.endTs, "Still in lock");
        ...
        IERC20(sToken.lpPair).safeTransfer(msg.sender, lp_owned);
    ...
    function calcOnBuy(uint256 _tid, uint256 _buy_amount) public view returns (uint256 _cost_amount, uint256 _amount_for_liquidity) {
        ...
        uint256 _discounted_value = _buy_amount.mul(sToken.discount).div(DENOMINATOR);
    ...
    function _depositYwAndLockUp(address _user, uint256 _lpAmount, uint256 _tid) internal {
        ...
        _approveTokenIfNeeded(sToken.lpPair);
        ...
        locker.endTs = block.timestamp.add(sToken.lockPeriod);
    }
