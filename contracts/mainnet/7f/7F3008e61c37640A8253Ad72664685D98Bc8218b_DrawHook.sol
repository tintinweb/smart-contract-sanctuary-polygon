...
contract DrawHook is IDrawHook, AccessControl {
    ...
    // NFT id => whether whitelist function is disabled
    mapping(uint256 => bool) internal _isDisabledNft;
    ...
    function isAllowedAccount(uint256 artId, address account) external view returns (bool) {
        return _isDisabledNft[artId] || _isAllowedAccount[artId][account];
    }
    ...
    function execute(
        ...
        require(_isDisabledNft[artId] || _isAllowedAccount[artId][tx.origin], "not allowed account");
    }

    function enableWhiteList(uint256 artId) external onlyAdmin {
        _isDisabledNft[artId] = false;
    }

    function disableWhiteList(uint256 artId) external onlyAdmin {
        _isDisabledNft[artId] = true;
    }
