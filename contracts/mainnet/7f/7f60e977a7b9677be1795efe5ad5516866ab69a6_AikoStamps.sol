...
// .......................***********************************,.....................
// 
// Made with love for Aiko Virtual by uwu Labs <3
...
contract AikoStamps is ERC1155URIStorage, Ownable {
	using Strings for string;

	string public name = "A:\\Stamp System";
	...
	function initializeStamp(address to, uint256 tokenId, string memory _tokenURI) public onlyOwner {
		require(!exists(tokenId), "AikoStamps: Stamp already exists");
		_mint(to, tokenId, 1, "");
		_setTokenURI(tokenId, _tokenURI);
	}
	...
	function mint(
        ...
        require(balanceOf(to, id) == 0, "already minted to");
		_mint(to, id, 1, "");
	}
	...
	// Mints 1 per address id combo.
	function mintMany(
		address[] memory receivers,
		uint256[] memory ids
	) public onlyManager {
		require(receivers.length == ids.length, "Wrong length");
		for (uint256 i; i < receivers.length; i++) {
            mint(receivers[i], ids[i]);
		}
	}
	...
	function adminTransferMany(
		...
		for (uint256 i; i < froms.length; i++) {
			_safeTransferFrom(froms[i], tos[i], ids[i], 1, "");
		}
	}
