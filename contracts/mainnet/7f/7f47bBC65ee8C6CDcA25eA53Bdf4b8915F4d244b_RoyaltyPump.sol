...
contract RoyaltyPump is TokensRecoverable
{
    ...
    mapping (address => bool) public royaltiesInitialized;
    ...
    function initializeRoyalties(address token, uint256 collected) public onlyRoyaltyCollector(){
        require(!royaltiesInitialized[token], "Royalties already initialized");
        royaltiesInitialized[token] = true;
        totalRoyaltiesCollected[token] += collected;
    }
    ...
    function depositRoyaltyAnyToken(address _artistToken, address _artistStaking, address _transGate) public payable onlyRoyaltyCollector(){
        ...
        totalRoyaltiesCollected[_artistToken] = totalRoyaltiesCollected[_artistToken] += msg.value;
