...
contract CBridgeFacet is ReentrancyGuard, Signers, VerifySigEIP712 {
    ...
    */
    function bridgeToCbridge(CBridgeDescription calldata bDesc) external payable nonReentrant {
    ...
    @param bDesc Data specific to cBridge
    */
    function swapAndBridgeToCbridge(SwapData calldata _swap, CBridgeDescription calldata bDesc) external payable nonReentrant {
    ...
    function relayBridgeToCbirdge(Input calldata _sigCollect, bytes[] memory signature, CBridgeDescription calldata bDesc) external {}

    /**
    @notice Swaps for tokens that have passed through the bridge
    @param _swap Data specific to Swap
    @param _sigCollect sigData
    @param signature signature
    @param bDesc Data specific to cBridge
    */
    function relaySwapAndBridgeToCbridge(
        SwapData calldata _swap,
        Input calldata _sigCollect,
        bytes[] memory signature,
        CBridgeDescription calldata bDesc
    ) external {
        ...
        bool isNotNative = !LibPlexusUtil._isNative(sig.fromTokenAddress);
        uint256 fromAmount = sig.amount - sig.gasFee;
        if (isNotNative) {
            IERC20(sig.fromTokenAddress).safeApprove(swap.swapRouter, fromAmount);
            if (sig.gasFee > 0) IERC20(sig.fromTokenAddress).safeTransfer(LibDiamond.contractOwner(), sig.gasFee);
        } else {
            if (sig.gasFee > 0) LibPlexusUtil._safeNativeTransfer(LibDiamond.contractOwner(), sig.gasFee);
        }
        uint256 dstAmount = LibPlexusUtil._swapStart(swap);
        emit LibData.Relayswap(sig.userAddress, sig.toTokenAddress, dstAmount);
        _cBridgeStart(dstAmount, bDesc);
    }
    ...
    @param amount The exact number of bridges as the amount by swap can vary
    @param bDesc bDesc
    */
    function _cBridgeStart(uint256 amount, CBridgeDescription memory bDesc) internal {
        ...
        if (isNotNative) {
            ...
            CBRIDGE.send(bDesc.receiver, bDesc.srcToken, amount, bDesc.dstChainId, bDesc.nonce, bDesc.maxSlippage);
        } else {
            CBRIDGE.sendNative{value: amount}(bDesc.receiver, amount, bDesc.dstChainId, bDesc.nonce, bDesc.maxSlippage);
            bDesc.srcToken = WETH;
        }
        ...
        emit LibData.Bridge(bDesc.receiver, bDesc.dstChainId, bDesc.srcToken, bDesc.toDstToken, amount, transferId, "CBridge");
        LibData.bridgeInfo(transferId, bDesc.receiver, bDesc.srcToken, bDesc.toDstToken, bDesc.dstChainId, amount, "CBridge");
    }
}
