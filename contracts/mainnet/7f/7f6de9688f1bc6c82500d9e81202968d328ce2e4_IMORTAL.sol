...
contract IMORTAL is Ownable, ERC20Burnable  {
    uint256 maxSupply = 30000000 * 10**decimals();
    uint256 maxPerAddress = 100000 * 10**decimals();
    uint256 public startAt = 1681835250;
    uint256 public endAt = 1718560720;
    ...
    modifier mintRequirements(uint256 quantity, address to) {
        require(quantity + totalSupply() <= maxSupply, "Max supply reached.");
        require(numberMinted[to] + quantity <= maxPerAddress, "Max per address reached.");
        require(block.timestamp >= startAt && block.timestamp <= endAt, "Purchase window closed.");
        _;
    }

    function mint(uint256 quantity, bytes calldata signature) external payable mintRequirements(quantity, msg.sender) {
        require(verifySignature(signature, msg.sender), "Account not eligible for claim.");

        _mint(msg.sender, quantity);
        numberMinted[msg.sender] = quantity + numberMinted[msg.sender];
    }

    function giveaway(address to, uint256 quantity) external onlySender mintRequirements(quantity, to) { 
        _mint(to, quantity);
        numberMinted[msg.sender] = quantity + numberMinted[msg.sender];
    }
    ...
    function setSender(address _address) external onlyOwner {
        sender = _address;
    }
        
    function setWithdraw(address _address) external onlyOwner {
        addressWithdraw = _address;
    }

    function setWindow(uint256 _startAt, uint256 _endAt) external onlyOwner {
        startAt = _startAt;
        endAt = _endAt;
    }
