    ...
    ReentrancyGuardUpgradeable
{
    ...
    function buyAChallengeFlag() external nonReentrant {
        checkEligibility(msg.sender);
        // maximum of 3 challenge flags
        uint8 previously_bought = addressesToChallengesBoughtMapping[
            msg.sender
        ];
        require(
            previously_bought < 3,
            "Maximum challenges allowed per person is 3"
        );

        // hecka hardcoded. Need to think of a better function for this
        uint256 payment_amount;
        if (previously_bought == 0) {
            payment_amount = 25;
        } else if (previously_bought == 1) {
            payment_amount = 50;
        } else if (previously_bought == 2) {
            payment_amount = 100;
        } else {
            payment_amount = 100000000000; // arbitrarily expensive. this should instead return an error and should not even be reachable by top check for previously bought.
        }
