    ...
    OwnableUpgradeable
{
    ...
    function claimReward() public nonReentrant {
        ...
        require(_iswithdrawingWithinMinimum(amountToTransfer), "Withdrawal amount is below minimum");
         
        if ( ! _isWithdrawalWithinMaximum(amountToTransfer)) {
            //we need to put the excess amount inside a leftwithdrawal
            uint256 maxTokenCanWithdraw = calculateTokenToDollarsNeeded(maximumWithdrawal);
            //get the difference and put inside excess token mappings
            uint256 excess = maxTokenCanWithdraw - amountToTransfer;
            _tokenLeftToWithdraw[msg.sender] = _tokenLeftToWithdraw[msg.sender] + excess;
            amountToTransfer = maxTokenCanWithdraw;
        } else{
            //we are within the withdrawal range. check if we have excess to add 
            uint256 excessTokenLeft = _tokenLeftToWithdraw[msg.sender];
            uint256 maxTokenCanWithdraw = calculateTokenToDollarsNeeded(maximumWithdrawal);
            //if the excess token left + amount withdrawing >= max 
            if ( excessTokenLeft + amountToTransfer >= maxTokenCanWithdraw ){
                _tokenLeftToWithdraw[msg.sender ] = ( excessTokenLeft + amountToTransfer) - maxTokenCanWithdraw;
                amountToTransfer = maxTokenCanWithdraw;
            }else{
    ...
    function _iswithdrawingWithinMinimum(uint256 _amount) private view returns (bool) {
        return _amount >= calculateTokenToDollarsNeeded(minimumWithdrawal);
    }

    function _isWithdrawalWithinMaximum(uint256 _amount) private view returns (bool) {
