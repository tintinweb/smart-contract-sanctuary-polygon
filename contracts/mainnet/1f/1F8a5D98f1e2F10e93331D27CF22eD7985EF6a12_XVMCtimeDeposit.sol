...
// File: xvmc-contracts/pools/6month.sol
...
contract XVMCtimeDeposit is ReentrancyGuard {
    ...
    uint256 public immutable withdrawFeePeriod = 183 days;
    uint256 public immutable gracePeriod = 7 days;
    ...
    function withdraw(uint256 _shares, uint256 _stakeID) public {
        ...
        if (block.timestamp < user.lastDepositedTime.add(withdrawFeePeriod)) {
           uint256 withdrawFee = uint256(4200).sub(((block.timestamp - user.lastDepositedTime).div(86400)).mul(231).div(10));
            ...
            uint256 withdrawFee = block.timestamp.sub(user.lastDepositedTime.add(withdrawFeePeriod)).div(86400).mul(231).div(10);
            if(withdrawFee > 4200) { withdrawFee = 4200; }
            currentWithdrawFee = currentAmount.mul(withdrawFee).div(10000);
            token.safeTransfer(treasury, currentWithdrawFee); 
            currentAmount = currentAmount.sub(currentWithdrawFee);
        }
