...
library IncentiveId {
    /// @notice 计算staking激励的键
    /// @param key 用于计算激励identifier的组件
    /// @return incentiveId 激励的identifier
...
/// @title Uniswap V3 Staker 接口
/// @notice 允许将不可变现的流动性代币抵押以换取奖励代币
interface IUniswapV3Staker is IERC721Receiver, IMulticall {
    /// @param rewardToken 作为奖励分发的令牌
    /// @param pool V3的pool
    /// @param startTime 开始时间
    /// @param endTime 结束时间
    /// @param refundee 奖励结束时接收任何剩余奖励令牌的地址
    struct IncentiveKey {
        IERC20Minimal rewardToken;
        IUniswapV3Pool pool;
        uint256 startTime;
    ...
    /// @notice The nonfungible position manager 与该抵押合同兼容
    function nonfungiblePositionManager() external view returns (INonfungiblePositionManager);

    /// @notice 激励的最长持续时间（秒）
    function maxIncentiveDuration() external view returns (uint256);

    /// @notice 可以设置激励开始时间的最长秒数
    function maxIncentiveStartLeadTime() external view returns (uint256);

    /// @notice 代表股权激励
    /// @param incentiveId 根据其参数计算的激励ID
    /// @return totalRewardUnclaimed 用户尚未领取的奖励令牌数量
    /// @return totalSecondsClaimedX128 已声明的总流动性秒数，表示为UQ32.128
    /// @return numberOfStakes 当前用于奖励的stake计数
    ...
    /// @notice 返回有关已存入NFT的信息
    /// @return owner 存放NFT的owner
    /// @return numberOfStakes 衡量流动性的激励措施数量
    /// @return tickLower 范围的下限
    /// @return tickUpper 范围的上限
    ...
    /// @notice 返回有关抵押流动性NFT的信息
    /// @param tokenId 标记的ID
    /// @param incentiveId 代币所用于的奖励的ID
    /// @return secondsPerLiquidityInsideInitialX128 secondsPerLiquidity 表示为UQ32.128
    /// @return liquidity 截至最后一次计算奖励时NFT的流动性金额
    function stakes(uint256 tokenId, bytes32 incentiveId)
        external
        view
        returns (uint160 secondsPerLiquidityInsideInitialX128, uint128 liquidity);

    /// @notice 根据上次更新所有stake的时间，返回欠给定地址的奖励令牌数量
    /// @param rewardToken 用于检查reward的令牌
    /// @param owner 检查所欠reward的所有者
    /// @return rewardsOwed 所有者可申请的reward令牌的金额
    function rewards(IERC20Minimal rewardToken, address owner) external view returns (uint256 rewardsOwed);

    /// @notice 创建新的流动性mining激励计划
    /// @param key 创建奖励的详细信息
    /// @param reward 要分发的reward令牌数量
    function createIncentive(IncentiveKey memory key, uint256 reward) external;

    /// @notice 在激励结束时间已过且所有stake已退出后结束激励
    /// @param key 创建奖励的详细信息
    /// @return refund 奖励结束时剩余的奖励代币
    function endIncentive(IncentiveKey memory key) external returns (uint256 refund);

    /// @notice 将存款ownership从发送方转移到给定接收方
    /// @param tokenId 要转移的令牌（和存款）的ID
    /// @param to 存款的新owner
    function transferDeposit(uint256 tokenId, address to) external;

    /// @notice 将Uniswap V3 LP令牌“tokenId”从此合同中撤回给收件人`to`
    /// @param tokenId Uniswap V3 LP令牌的唯一标识符
    /// @param to 将发送LP令牌的地址
    /// @param data 将通过NFT safeTransferFrom传递到`to`地址的可选数据数组
    function withdrawToken(
        uint256 tokenId,
        address to,
        bytes memory data
    ...
    /// @notice Stake 一个Uniswap V3 LP代币
    /// @param key NFT Stake激励的键
    /// @param tokenId NFT Stake的TokenId
    function stakeToken(IncentiveKey memory key, uint256 tokenId) external;

    /// @notice UnStake 一个Uniswap V3 LP代币
    /// @param key NFT Stake激励的键
    /// @param tokenId NFT Stake的TokenId
    function unstakeToken(IncentiveKey memory key, uint256 tokenId) external;

    /// @notice Transfers 将合同中累积的“rewardToken”奖励的“amountRequested”转移到收件人`to`
    /// @param rewardToken 作为reward分发的令牌
    /// @param to 领取reward的地址
    /// @param amountRequested 要申领的reward令牌的数量。如果设置为0，则申领全部reward金额。
    /// @return reward 领取的reward代币数量
    function claimReward(
        IERC20Minimal rewardToken,
        address to,
        uint256 amountRequested
    ...
    /// @notice 计算给定赌注将获得的reward金额
    /// @param key NFT Stake激励的键
    /// @param tokenId NFT Stake的TokenId
    /// @return reward 到目前为止，NFT对给定激励的累计reward
    function getRewardInfo(IncentiveKey memory key, uint256 tokenId)
        external
        returns (uint256 reward, uint160 secondsInsideX128);
...
/// @title 计算rewards的Math库
/// @notice 允许计算给定一些质押和激励参数的rewards的数量
library RewardMath {
    /// @notice 计算给定一些质押和激励参数的rewards的数量
    /// @param totalRewardUnclaimed 剩下的作为激励的未声明的rewards的总数量
    /// @param totalSecondsClaimedX128 已声明作为激励的liquidity-seconds的总数量
    /// @param startTime 用于激励的rewards的开始时间
    /// @param endTime 用于激励的rewards的结束时间
    /// @param liquidity liquidity的数量，假设在快照测量期间流动性保持不变
    /// @param secondsPerLiquidityInsideInitialX128 截至期初流动性刻度范围的每流动性秒数
    /// @param secondsPerLiquidityInsideX128 流动性刻度范围的每流动性秒数
    /// @param currentTime 当前块时间戳，必须大于或等于开始时间
    /// @return reward 获得的rewards的数量
    /// @return secondsInsideX128 持有stake期间头寸范围内的liquidity的总liquidity seconds
    function computeRewardAmount(
        ...
        // 这个操作是安全的, 正如差异不会超过 1/stake.liquidity
...
/// @notice 封装获取NFT令牌ID信息的逻辑
library NFTPositionInfo {
    /// @param factory 用于计算pool地址的Uniswap V3 factory的地址
    /// @param nonfungiblePositionManager 要查询的nonfungible position manager的地址
    /// @param tokenId Uniswap V3 LP token的唯一identifier
    /// @return pool Uniswap V3 pool的地址
    /// @return tickLower Tick of the Uniswap V3 position的下边界
    /// @return tickUpper Tick of the Uniswap V3 position的上边界
    /// @return liquidity The liquidity staked的数量
...
library TransferHelperExtended {
    ...
    /// @notice 将令牌从目标地址transfer到给定目标
    /// @notice 如果传输失败，则出现“STF”错误
    /// @param token 要transfer的令牌的合约地址
    /// @param from 将从中transfer令牌的from地址
    /// @param to 将从中transfer令牌的to地址
    /// @param value 需要transfer的数量
    ...
    /// @notice 将令牌从msg.sender transfer到recipient
    /// @dev 如果传输失败，则出现“ST”错误
    /// @param token 要transfer的令牌的合约地址
    /// @param to 将从中transfer令牌的to地址
    /// @param value 需要transfer的数量
