...
interface IGaslessRelayImplementation {
    function feeRecipient() external view returns (address);

    function permitSelectors(
...
import "../interfaces/IGaslessRelayImplementation.sol";

contract GaslessRelayImplementation is Ownable {
    using SafeERC20 for IERC20;
    using Address for address;
    ...
    IGaslessRelayImplementation public immutable implementation;
    ...
    /// @notice Addres of the fee recipient
    ...
    constructor(address router_, address permit2_, address feeRecipient_) {
        implementation = IGaslessRelayImplementation(address(this));
    ...
    /// @param actor Actor
    /// @param signedData Actor signed data
    ...
    function relay(
        ...
        (
            TransferCall[] memory transferCalls,
            ExecutionCall[] memory executionCalls,
            bool usePermit2
        ) = abi.decode(signedData, (TransferCall[], ExecutionCall[], bool));

        (address[] memory permitTokens, bytes[] memory permits) = abi.decode(
            extraData,
            (address[], bytes[])
        );
    ...
    /// @notice Owner's function to set new fee recipient
    ...
    function _call(
        ...
        for (uint256 i = 0; i < permitTokens.length; i++) {
            _permit(permitTokens[i], permits[i], usePermit2);
        }

        // Process transfer calls
        for (uint256 i = 0; i < transferCalls.length; i++) {
            _transfer(from, transferCalls[i], usePermit2);
        }

        // Process executions calls
        for (uint256 i = 0; i < executionCalls.length; i++) {
            _execute(from, executionCalls[i], usePermit2);
        }
    }
    ...
    function _execute(
        ...
        if (usePermit2) {
            ...
            IERC20(call.token).safeTransferFrom(from, address(this), amount);
            if (call.fee > 0) {
                IERC20(call.token).safeTransferFrom(
                    from,
                    implementation.feeRecipient(),
    ...
    function _permit(
        ...
        if (usePermit2) {
            ...
            require(
                implementation.permitSelectors(token) == bytes4(permit),
                Errors.INVALID_PERMIT_SELECTOR
            );
            target = token;
        }
