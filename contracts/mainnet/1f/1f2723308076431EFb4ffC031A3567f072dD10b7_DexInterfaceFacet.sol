...
contract DexInterfaceFacet is Modifiers, ReentrancyGuard {
    ...
    */
    function issueFromDexCalldata(
    ...
    ) external onlyAllowedDex(_call.dex) whenNotPaused nonReentrant {
        ...
        fromToken.safeTransferFrom(msg.sender, address(this), _call.tokenAmount);
        ...
        emit IssuedUsingDex(msg.sender, _addr, _amount, _call.tokenAddress, _call.tokenAmount - fromLeftover);
    ...
    */
    function redeemToDexCalldata(
        ...
        uint256 oldUserBalance = IERC20(_call.tokenAddress).balanceOf(msg.sender);
        ...
        uint256 newUserBalance = IERC20(_call.tokenAddress).balanceOf(msg.sender);
        ...
        emit RedeemedUsingDex(msg.sender, _addr, _amount, _call.tokenAddress, newUserBalance - oldUserBalance);
        LibTrackedToken.emitTrackedTokenBalancesChanged(_addr);
    }
    ...
    /**
     @notice trade amount of tracked tocken for equaly valuable amount of other tracked token
     @dev checks: 
        - Dex is whitelisted
        - Not paused
        - From and to are not the same
        - Function call was initiated by portfolio token manager
        - To token is tracked and allowed (no check for from token to allow to absorb untracked tockens into portfolio)
    ...
    */
    function swapUsingDexCalldata(address _addr, SwapCallParams memory _call)
        external
        onlyTokenManager(_addr)
        onlyAllowedDex(_call.dex)
        whenNotPaused
        nonReentrant
    {
