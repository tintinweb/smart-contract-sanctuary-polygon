...
contract MonthlyEqualVestingMultiWallet is  Ownable {
    ...
    /**
     * @dev Register purchase of tokens
     */
    function registerPurchase(address user, uint256 amount) public virtual onlyOwner{
    ...
    /**
     * @dev Batch register purchase of tokens
     */
    function registerPurchaseBatch(address[] calldata _addresses, uint256[] calldata _tokens) public onlyOwner {
        require(_addresses.length == _tokens.length);
        require(_addresses.length <= 100);

        for (uint256 i = 0; i < _tokens.length; i++) {
            registerPurchase(_addresses[i], _tokens[i]);
        }
    }
