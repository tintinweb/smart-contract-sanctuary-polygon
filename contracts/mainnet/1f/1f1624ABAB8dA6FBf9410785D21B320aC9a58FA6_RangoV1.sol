...
contract RangoV1 is BaseProxyContract {
    ...
    enum BridgeType { Across, CBridge, Hop, Hyphen, Multichain, Stargate, Synapse, Thorchain, Orbiter }
    ...
    function getProxyAddress(BridgeType bridgeType) public pure returns(address) {
        ...
        if (bridgeType == BridgeType.Orbiter) return s.orbiter;
    ...
    function setProxyAddress(BridgeType bridgeType, address _newAddress) internal {
        ...
        if (bridgeType == BridgeType.Orbiter) s.orbiter = _newAddress;
    }
    ...
    function orbiterBridge(
        SwapRequest memory request,
        Call[] calldata calls,
        IRangoOrbiter.OrbiterBridgeRequest memory bridgeRequest
    ) external payable whenNotPaused nonReentrant {
        address a = getProxyAddress(BridgeType.Orbiter);
        (uint out, uint value) = onChainSwapsPreBridge(a, request, calls, 0);

        IRangoOrbiter(a).orbiterBridge{value: value}(bridgeRequest);
    }
}
...
interface IRangoOrbiter {
    ...
    struct OrbiterBridgeRequest {
        ...
        address orbiterContractAddress;
        address orbiterWalletAddress;
        uint256 amount;
    ...
    enum DestinationChain { StarkNet }

    function orbiterBridge(OrbiterBridgeRequest memory _request) external payable;
}
