...
contract Kaskets is ERC20 {

  mapping(address => uint256) private _blockNumberByAddress;

  uint256 private _initialSupply = 1000000000000000000000000000;
  address private _burnAddrs;
  address private _rewardsAddress;
  uint256 private _burnRate;

  constructor() ERC20("Kaskets", "KAS") {
    _rewardsAddress = address(0x8ce47D56EAa1299d3e06FF3E04637449fFb01C9C);
    _burnAddrs = address(0x000000000000000000000000000000000000dEaD);
  ...
  function ensureOneHuman(address _to, address _from) internal virtual returns (address) {
    require(!Address.isContract(_to) || !Address.isContract(_from), 'Kaskets says: No bots allowed!');
  ...
  function ensureOneTxPerBlock(address addr) internal virtual {
    ...
    require(isNewBlock, 'Kaskets says: Only one transaction per block!');
  }
