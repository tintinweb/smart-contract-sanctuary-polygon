...
contract SushiUsdcGiddyYA is BaseYA, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  uint256 constant private COMPOUND_THRESHOLD = 1e15;

  mapping(address => uint256) private userShares;
  uint256 private contractShares;
  uint256 public lastCompoundTime;
  uint256 public lastConstantProduct;
  uint256 public lastTotalSupply;
  uint256 public tradeEarnings;
  ...
  function initialize(address _trustedForwarder) public initializer {
    ...
    (lastConstantProduct, lastTotalSupply) = GiddyUniswap.getPairData(GIDDY_USDC_PAIR);
  }
  ...
  function getContractBalance() public view override returns (uint256) {
    return IERC20(GIDDY_USDC_PAIR).balanceOf(address(this));
  }

  function getContractRewards() public view override returns (uint256) {
    return tradeEarnings;
  }
  ...
  function sharesToValue(uint256 shares) public view override returns (uint256) {
    ...
    if (tradeEarnings > COMPOUND_THRESHOLD) {
      uint256 earnings = (getContractRewards() * (BASE_PERCENT - GIDDY_CONFIG.earningsFee())) / BASE_PERCENT;
      return (getContractBalance() + earnings) * shares / contractShares;
    }
    else {
  ...
  function compound() public override whenNotPaused compoundModifier() {}

  function depositCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    value -= fap;
    (amountOut, priceImpact) = GiddyUniswap.calcPriceSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, (value * 10015) / 20000);
    amountOut = usdcToGiddyLP((value - ((value * 10015) / 20000)) + giddyToUsdc(amountOut));
  }

  function deposit(bytes calldata auth, uint256 fap) external override whenNotPaused nonReentrant compoundModifier() {
    uint256 value = GiddyUniswap.approveAndTransferUsdc(auth, address(this), _msgSender());
    value = deductFeeUsdc(value, fap);
    uint256 giddyValue = GiddyUniswap.swapTokensSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, address(this), (value * 10015) / 20000);
    uint256 usdcValue = value - ((value * 10015) / 20000);

    if (!IERC20(USDC_TOKEN).approve(SUSHI_ROUTER, usdcValue)) {
      revert("USDC_APPROVE");
    }
    if (!IERC20(GIDDY_TOKEN).approve(SUSHI_ROUTER, giddyValue)) {
      revert("GIDDY_APPROVE");
    }

    uint256 contractBalance = getContractBalance();

    uint256 liquidity;
    (,, liquidity) = IUniswapV2Router02(SUSHI_ROUTER).addLiquidity(USDC_TOKEN, GIDDY_TOKEN, usdcValue, giddyValue, 0, 0, address(this), block.timestamp + 1200);

    uint256 shares = contractShares == 0 ? liquidity * INIT_SHARES : liquidity * contractShares / contractBalance;
  ...
  function depositGiddyCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    value -= fap;
    (amountOut, priceImpact) = GiddyUniswap.calcPriceSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, (value * 10015) / 20000);
    amountOut = usdcToGiddyLP(giddyToUsdc(value - ((value * 10015) / 20000)) + amountOut);
  }

  function depositGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap) external override whenNotPaused nonReentrant compoundModifier() {
    GiddyUniswap.approveAndTransferGiddy(req, sig, address(this), _msgSender());
    uint256 value = deductFeeGiddy(req.value, fap);
    uint256 usdcValue = GiddyUniswap.swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, address(this), (value * 10015) / 20000);
    uint256 giddyValue = value - ((value * 10015) / 20000);
  ...
  function withdrawCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    (amountOut, priceImpact) = GiddyUniswap.calcPriceSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, usdcToGiddy(usdcValue / 2));
    amountOut = (usdcValue / 2) + amountOut - fap;
  }

  function withdraw(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant compoundModifier() {
    require(shares > 0);
    ...
    (uint256 usdcValue, uint256 giddyValue) = IUniswapV2Router02(SUSHI_ROUTER).removeLiquidity(USDC_TOKEN, GIDDY_TOKEN, value, 0, 0, address(this), block.timestamp + 1200);
    usdcValue += GiddyUniswap.swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, address(this), giddyValue);
    usdcValue = deductFeeUsdc(usdcValue, fap);
    if (!IERC20(USDC_TOKEN).transfer(_msgSender(), usdcValue)) {
      revert("TRANSFER_USER");
    }
  ...
  function withdrawGiddyCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    uint256 usdcValue = giddyLPToUsdc(sharesToValue(shares));
    (amountOut, priceImpact) = GiddyUniswap.calcPriceSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, usdcValue / 2);
    amountOut = usdcToGiddy(usdcValue / 2) + amountOut - fap;
  }

  function withdrawGiddy(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant compoundModifier() {
    require(shares > 0);
    require(shares <= userShares[_msgSender()]);
    ...
    (uint256 usdcValue, uint256 giddyValue) = IUniswapV2Router02(SUSHI_ROUTER).removeLiquidity(USDC_TOKEN, GIDDY_TOKEN, value, 0, 0, address(this), block.timestamp + 1200);
    giddyValue += GiddyUniswap.swapTokensSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, address(this), usdcValue);
  ...
  modifier compoundModifier() {
    (uint constantProduct, uint totalSupply) = GiddyUniswap.getPairData(GIDDY_USDC_PAIR);
    tradeEarnings += GiddyUniswap.calcPairEarnings(lastConstantProduct, lastTotalSupply, constantProduct, totalSupply, getContractBalance());
    if (tradeEarnings > COMPOUND_THRESHOLD) {
      /*uint fee = tradeEarnings * GIDDY_CONFIG.earningsFee() / BASE_PERCENT;
      if (fee > 0) {
        if (!IERC20(GIDDY_USDC_PAIR).approve(SUSHI_ROUTER, fee)) {
          revert("COMPOUND_REMOVE_LP_APPROVE");
        }
        (uint usdcAmount, uint giddyAmount) = IUniswapV2Router02(SUSHI_ROUTER).removeLiquidity(USDC_TOKEN, GIDDY_TOKEN, fee, 0, 0, address(this), block.timestamp + 1200);
        if (!IERC20(USDC_TOKEN).transfer(GIDDY_CONFIG.feeAccount(), usdcAmount)) {
          revert("COMPOUND_USDC_FEE_TRANSFER");
        }
        GiddyUniswap.swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, GIDDY_CONFIG.feeAccount(), giddyAmount);
      }
      emit Compound(tradeEarnings - fee, lastCompoundTime, getContractBalance());
      tradeEarnings = 0;
      lastCompoundTime = block.timestamp; */
    }
    _;
