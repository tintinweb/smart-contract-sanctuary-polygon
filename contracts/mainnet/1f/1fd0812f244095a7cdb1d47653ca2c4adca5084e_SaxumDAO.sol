...
contract SaxumDAO is ERC20 {
    ...
    modifier onlyOwner() {
        require(_msgSender() == _owner, "SaxumDAO: only owner can call this function");
        _;
    }
    ...
    // token airdropWithLockup.
    function airdropWithLockup(address[] memory recipients, uint256[] memory amounts, uint256 lockupPeriod) public returns (bool) {
...
// function to get locked balance of a recipient
    ...
    function burn(uint256 amount) public {
        require(_msgSender() == _owner, "SaxumDAO: only owner can burn");
        _burn(_msgSender(), amount);
    }

    function lockTreasury() external {
        require(_msgSender() == _owner, "SaxumDAO: only owner can lock treasury");
        _transfer(_owner, address(this), _treasuryLocked);
    }

    function lockDev() external {
        require(_msgSender() == _owner, "SaxumDAO: only owner can lock dev");
        _transfer(_owner, address(this), _devLocked);
    }

    function releaseTreasury() external {
        require(_msgSender() == _owner, "SaxumDAO: only owner can release treasury");
        require(block.timestamp >= _vestingStart + _treasuryvestingCliff, "SaxumDAO: vesting cliff not reached");
    ...
    function releaseDev() external {
        require(_msgSender() == _owner, "SaxumDAO: only owner can release dev");
        require(block.timestamp >= _vestingStart + _vestingCliff, "SaxumDAO: vesting cliff not reached");
    ...
    require(_msgSender() == _owner, "SaxumDAO: only owner can lock tokens");
    ...
    function unlockTokens(address account, uint256 amount) public {
        require(_msgSender() == _owner, "SaxumDAO: only owner can unlock tokens");
        ...
        require(releaseTime != 0, "SaxumDAO: tokens not locked");
        require(block.timestamp >= releaseTime, "SaxumDAO: lock duration not reached");
