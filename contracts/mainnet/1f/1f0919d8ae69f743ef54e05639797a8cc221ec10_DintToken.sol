 ...
 *      - Voting delegation and delegation on behalf via EIP-712 (like in Compound CMP token) - gives COT token
 ...
 */
contract DintToken is ERC1363, EIP2612, EIP3009, AccessControl {
	/**
	 * @dev Returns the name of the token.
	 */
	string public constant name = "Dint";

	/**
	 * @dev Returns the symbol of the token
	 */
	string public constant symbol = "DINT";
	 ...
	 *      it is implied version is concatenated to the name field, like "DintToken"
	 */
	...
	constructor() {
		...
		DOMAIN_SEPARATOR = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes(name)), keccak256(bytes("MMT_0.1")), block.chainid, address(this)));
