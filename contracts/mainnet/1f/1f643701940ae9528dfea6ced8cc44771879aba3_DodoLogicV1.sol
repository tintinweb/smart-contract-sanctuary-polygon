...
contract DodoLogicV1 {
  ...
  function getDataContract() internal pure returns (DodoStorageInterface) {
  ...
  function getTokenContract() public pure returns (address) {
    return address(0x25F07bA23c50e2DD1105F24c1518Ef38B7b84078);
  }
  ...
  function getIncome(address player) public view returns (uint256) {
    PlayerData memory playerData = getDataContract().getPlayerData(player);
  ...
  function getPropsIndex(address player) private view returns (uint) {
    ...
    return getDataContract().getPlayerExtraData(player, propExtraDataKey);
  }
  ...
  function getPropsCanBuy(address player) public view returns (Prop memory) {
    ...
    PlayerData memory playerData = DodoStorageInterface(getDataContract()).getPlayerData(player);
  ...
  function getInviteData(address player) public view returns (uint256, uint256) {
    ...
    DodoStorageInterface _dataContract = DodoStorageInterface(getDataContract());
    return (
  ...
  function makeMoneyBatch(address inviter, uint times) public payable {
    ...
    DodoStorageInterface _dataContract = getDataContract();
  ...
  function upgradeIncomeLevel() public {
    ...
    tokenContract().deductTokens(player, address(getDataContract()), upgradeCost);
  }
  ...
  function buyProp(uint256 propId) public {
    ...
    tokenContract().deductTokens(player, address(getDataContract()), prop.price);
    
    getDataContract().setPlayerExtraData(player, DodoConfig.getPropExtraDataKey(), propId);
  ...
  function casinoBet(uint256 amount) public {
    ...
    if(odds == 0) {
      ...
      tokenContract().deductTokens(player, address(getDataContract()), amount);
    } else if (odds > 1) {
