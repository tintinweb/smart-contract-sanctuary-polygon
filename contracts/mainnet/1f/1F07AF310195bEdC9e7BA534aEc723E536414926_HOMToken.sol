...
contract HOMToken is BEP20, MinterRole {
    ...
    uint256 public sellFee = 5;   // default 5%
    uint256 public minSwapAmount = 100000000;   //0.1 HOM tokens
    ...
    // In swap and liquify
    bool private _inSwapForUSDC;

    // Automatic swap and liquify enabled
    bool public swapForUSDCEnabled = true; // INMUTABLE
    ...
    event UpdateSellFee(uint16 sellFee);
     event UpdateLpcontract(address lpcontract);
    ...
    /**z
     * @notice Constructs the HOM token contract.
     */
    constructor(address _treasury, address _lpcontract) public BEP20("HOMToken", "HOM") {
        operator = _msgSender();
        ...
        require(_treasury != address(0), "wallet address is zero");
        feeWallet = _treasury;
    ...
    /// @dev overrides transfer function to meet tokenomics of HOM
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (sender == owner() || recipient == owner() || isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            ...
            // IUniswapV2Pair( lpcontract ).getReserves();
            // uint112 reserves=reserve0.mul(1000).div(reserve1);
            (uint112 reserve0, uint112 reserve1,) =  IUniswapV2Pair( lpcontract ).getReserves();
            uint112 reserves;
            if(reserve0<reserve1)
            {
                reserves = reserve0 * 1000 / reserve1;
            }else{
                reserves = reserve1 * 1000 / reserve0;
            }
            ...
            if(reserves >= 20)
            {
                ...
                if(automatedMarketMakerPairs[recipient] && sellFee > 0) {
                    ...
                    uint256 tokenamount = balanceOf(recipient);  // token amount in recipient
                    if(selltime[recipient] == 0)
                    {
                        require(amount < (tokenamount / 20), "You can sell the 5% of your tokens only"); 
                        selltime[recipient] = block.timestamp;
                    }else{
                        if(block.timestamp < selltime[recipient] + (86400 * 30))  //in 30 days
                        {
                            if(sellamount[recipient] == 0)
                            {
                                sellamount[recipient] = amount ;
                                require( amount < ( tokenamount / 20 ), "You can sell the 5% of your tokens in 30 days" );
                            }else{
                                require(( sellamount[recipient] + amount) < ( tokenamount / 20 ), "You can sell the 5% of your tokens in 30 days" );
                                sellamount[recipient] = sellamount[recipient] + amount ;
                            }
                  
                        }else{
                            sellamount[recipient] = 0 ;
                            selltime[recipient] = block.timestamp;
                            require(amount < ( tokenamount / 20 ), "You can sell the 5% of your tokens in 30 days");
                        }
    ...
    function swapTokensAndSend() private lockTheSwap {
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "HOM::transferOperator: new operator is the zero address");
    ...
    function updateBuyFee(uint16 _buyFee) public onlyOperator {
        require( _buyFee <= 100, "HOM:: Biu Fee can't exceed 100%" );
        emit UpdateBuyFee(_buyFee);
        buyFee = _buyFee;
    }

    function updateSellFee(uint16 _sellFee) public onlyOperator {
        require( _sellFee <= 100, "HOM:: Sell Fee can't exceed 100%");
    ...
    function enableSwapTokenForUSDC(bool _enabled) public onlyOperator {
        swapForUSDCEnabled = _enabled;
    }
     ...
     */
    function updateFeeWallet(address _treasury) public onlyOperator {
        require( _treasury != address(0), "HOM:: tax fee address is zero" );
        emit UpdateFeeWallet(_treasury);
        feeWallet = _treasury;
    }

    function getReserves1() public view returns (uint112 reserve3) {
       
        (uint112 reserve0, uint112 reserve1,) =  IUniswapV2Pair( lpcontract ).getReserves();
        if(reserve0<reserve1)
        {
            return reserve0 * 1000 / reserve1;
        }else{
            return reserve1 * 1000 / reserve0;
        }
    }

    function balance1(uint256 amount) public view returns (uint256 amount2) {
        uint256 tokenamount = balanceOf(msg.sender);  // token amount in msg.sender
            if(selltime[msg.sender] == 0)
            {
                require(amount < (tokenamount.div(20) ), "You can sell the 5% of your tokens only");
    ...
    function updateLpcontract(address _lpcontract) public onlyOperator {
        require( _lpcontract != address(0), "HOM:: lpcotontract address is not zero" );
        emit UpdateLpcontract(_lpcontract);
        lpcontract = _lpcontract;
    }
