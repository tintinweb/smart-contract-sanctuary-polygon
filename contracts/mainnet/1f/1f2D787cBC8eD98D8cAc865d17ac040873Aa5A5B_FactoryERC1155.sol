...
contract FactoryERC1155 {
    ...
    function deployERC1155(string memory _uri, uint[] memory _ids, string[] memory _names) public returns (address) {
        ERC1155Token t = new ERC1155Token(_uri, _names, _ids);
        tokens.push(t);
...
contract ERC1155Token is ERC1155, Ownable {
    ...
    event Mint(address _addy, address _owner, uint _amount);
    string[] public names;
    uint[] public ids;
    ...
    constructor(string memory _uri, string[] memory _names, uint[] memory _ids) ERC1155(_uri) {
        //require(_names.length == _ids.length, "must match in length");
        names = _names;
        ids = _ids;
        createMapping();
        setURI(_uri);
