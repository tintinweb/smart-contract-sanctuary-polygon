    ...
    ReentrancyGuardUpgradeable
{
    ...
    function sellPropertyWithPermit(
        ...
        require(
            ITokenWhitelist(tokenWhitelist).isOnWhitelist(tryB),
            "Token not supported"
        );
        require(
        ...
        uint tokensReceived = IMarketplace(_marketplace).swap(
            _swapArgs,
            isFeeInXeq
        );
    ...
    function approveAndSellProperty(
        ...
        // now buying property
        IMarketplace(_marketplace).swap(_swapArgs, isFeeInXeq);
    }

    // Approve and Swap on DFX
    function swapWithMetaTx(
        IMetaTx.MetaTxExecuteArgs memory _metaTxArgs,
        address _quoteCurrency,
        address _origin,
        address _target,
        uint256 _originAmount,
        ...
        uint256 _deadline,
        address receipient
    ) external nonReentrant returns (uint swappedAmount) {
        require(
            ITokenWhitelist(tokenWhitelist).isOnWhitelist(_origin),
            "Token not supported"
        );
        require(
            ITokenWhitelist(tokenWhitelist).isOnWhitelist(_target),
            "Token not supported"
        );
