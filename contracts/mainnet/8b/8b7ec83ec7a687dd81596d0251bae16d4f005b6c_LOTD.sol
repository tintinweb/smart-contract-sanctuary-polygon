...
contract LOTD is ERC20, Ownable {
    ...
    address public marketingWallet = 0xD3d15a19C421136488cBDEc7927246507Cd5b6CC;
    ...
    constructor() ERC20("Land Of The Dead", "LOTD") {
    ...
    function setBuyFee(uint _newmarketingfee, uint _newliqfee) external onlyOwner {
        require(_newmarketingfee + _newliqfee < 99, "setBuyFee: wrong fee");
        buyMarketingFee = _newmarketingfee;
        buyLiquidityFee = _newliqfee;
    }

    function setSellFee(uint _newmarketingfee, uint _newliqfee) external onlyOwner{
        require(_newmarketingfee + _newliqfee < 99, "setSellFee: wrong fee");
        sellMarketingFee = _newmarketingfee;
        sellLiquidityFee = _newliqfee;
    }
    ...
    function swapOnDemand() external onlyOwner {
        swapping = true;
        uint256 contractTokenBalanceforlp = balanceOf(address(this))-pendingMarketingFee;
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) {
                uint256 mfee = (amount*buyMarketingFee)/(100);
                uint256 lfee = (amount*buyLiquidityFee)/(100);
                uint totalfee = mfee+lfee;
                pendingMarketingFee = pendingMarketingFee+mfee;
                super._transfer(from, address(this), totalfee);
                amount = amount-totalfee;  
            }

            else if (automatedMarketMakerPairs[to]) {
                uint256 mfee = (amount*sellMarketingFee)/(100);
                uint256 lfee = (amount*sellLiquidityFee)/(100);
                uint totalfee = mfee+lfee;
                pendingMarketingFee = pendingMarketingFee+mfee;
                super._transfer(from, address(this), totalfee);
                amount = amount-totalfee;  
