...
//Bovine Battle Royale Finale Game #1

//premint game for barts farm frens -- take your final battle shot!

//visit bartsfarmfrens.io 
...
contract bartbattle1 is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  // dev function to mint token 0 and start the contract
  ...
  // gamelist mint function (allowlist required)
  function mint(uint256 quantity, bytes32[] calldata PresaleMerkleProof) external payable {
    uint256 supply = totalSupply();
    require(!paused);
    require(quantity > 0);
    require(quantity <= maxMintPerTransaction);
...
for(uint i = 0; i < quantity; i++) {
            uint mintIndex = uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender))) % 51;
            mintIndex = mintIndex + 0;
