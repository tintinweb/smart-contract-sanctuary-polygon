...
contract RLCoin is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 10_000_000_000 * 10**12;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "RL Coin";
    string private _symbol = "RLA";
    ...
    address payable public marketingWallet = payable(0x3925D81a7ff4E6e79E7dEF39dcDAC2f06d30A150);
    ...
    uint256 public maxWalletToken = 200000000 * 10**_decimals;           // 2%
    uint256 public maxSellTransactionAmount = 100000000 * 10**_decimals; // 1%
    uint256 public maxBuyTransactionAmount = 200000000 * 10**_decimals;  // 2%
    ...
    function _transfer(
        ...
        if (
            ...
            if (cooldownIsEnabled) {
                require(_lastSell[from] + antiBotSellCoolDown < block.timestamp, "You can not sell tokens right now");
        ...
        if (
            ...
            if (cooldownIsEnabled) {
                require(_lastBuy[to] + antiBotBuyCoolDown < block.timestamp, "You can not buy tokens right now");
    ...
    function setCooldownEnabled(bool status) external onlyOwner {
        cooldownIsEnabled = status;
    }

    function setBuyCoolDown(uint256 timeInSeconds) public onlyOwner {
        require(timeInSeconds <= 3600, "Time is too long");
        antiBotBuyCoolDown = timeInSeconds * 1 seconds;
    }

    function setSellCoolDown(uint256 timeInSeconds) public onlyOwner {
        require(timeInSeconds <= 3600, "Time is too long");
        antiBotSellCoolDown = timeInSeconds * 1 seconds;
    }
    ...
    */
    ...
    function setMarketingFeePercent(uint256 marketingFee) external onlyOwner {
        require(marketingFee <= 10, "Marketing is too high");
        _marketingFee = marketingFee;
    }

    function setTaxFeePercent(uint256 taxFee) external onlyOwner {
        require(taxFee <= 10, "Tax is too high");
        _taxFee = taxFee;
    }

    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner {
        require(liquidityFee <= 10, "Liquidity is too high");
        _liquidityFee = liquidityFee;
    }

    function setBurnFeePercent(uint256 burnFee) external onlyOwner {
        require(burnFee <= 10, "Burn is too high");
        _burnFee = burnFee;
    }
    ...
    function setMaxBuytx(uint256 _maxSellTxAmount) public onlyOwner {
        require(_maxSellTxAmount > 0, "Cannot set sell transaction amount as zero");
        maxSellTransactionAmount = _maxSellTxAmount * 10**_decimals;
    }

    function setMaxSelltx(uint256 _maxBuyTxAmount) public onlyOwner {
        require(_maxBuyTxAmount > 0, "Cannot set buy transaction amount as zero");
        maxBuyTransactionAmount = _maxBuyTxAmount * 10**_decimals;
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            automatedMarketMakerPairs[pair] != value,
            "NO: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function setMaxWalletToken(uint256 _maxToken) external onlyOwner {
        require(_maxToken > 0, "Must be greater than 0");
        maxWalletToken = _maxToken * 10**_decimals;
    }

    //New router address? No problem, just change it here!
