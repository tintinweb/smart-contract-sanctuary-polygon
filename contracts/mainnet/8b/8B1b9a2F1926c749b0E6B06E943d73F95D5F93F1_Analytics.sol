...
contract Analytics is Ownable, ReentrancyGuard {
    ...
    mapping(bytes32 => FileBase) public clusterFileBaseInfo;

    mapping(address => bool) public isWhitelisted;

    constructor() {}

    function setFileBaseData(
        bytes32 _clusterDns,
        bytes32 namespace,
        string memory cid,
        bytes32 nmtype
        ...
        bytes memory strKey = bytes.concat(namespace, _clusterDns, nmtype);
        clusterFileBaseInfo[bytes32(abi.encodePacked(strKey))].cid = cid;
    }
    ...
    function getCid(
        ...
        cidInfo memory getCidInfo = cidInfo({
            cid: clusterFileBaseInfo[bytes32(abi.encodePacked(strKey))].cid
        });
        // cid: clusterFileBaseInfo[namespace][_clusterDns][nmtype].cid

        return getCidInfo;
    }

    function setClusterAppCount(bytes32 _clusterDns, uint256 _appCount) public {
        if (this.owner() != msg.sender) {
            require(isWhitelisted[msg.sender], "Not allowed to update");
        }
    ...
    function setClusterPodCount(bytes32 _clusterDns, uint256 _podCount) public {
        if (this.owner() != msg.sender) {
            require(isWhitelisted[msg.sender], "Not allowed to update");
        }
