...
contract PoolOperations {
    ...
    address private s_poolsOwner;
    IDexwinCore private immutable Core;

    mapping(bytes32 => Pool) private pools;

    event PoolCreated(Pool _pool);
    event UserParticipated(Pool _pool, address _participant, address _token);
    event PoolUserSettled(Pool _pool, address _user, uint8 _result, address _token, uint _stake, uint _winAmount);
    ...
    modifier onlyPoolsOwner() {
        if (msg.sender != s_poolsOwner) {
            revert("Pools__OnlyOwnerMethod");
        }
        _;
    }

   function _transferOpsOwnership(address _newOwner) public onlyPoolsOwner {
        require(_newOwner != address(0), "Incorect address");
        s_poolsOwner = _newOwner;
    }
    function setCoreOwnershipInOps(address newOwner) public onlyPoolsOwner() {
        Core.setCoreOwnership(newOwner);
    }

    function disableCoreOwnershipInOps(address owner) public onlyPoolsOwner() {
        Core.disableCoreOwnership(owner);
    }

    function createPool(bytes32 _poolKey, uint _matchId, uint _totalPoolAmt, uint _totalSpots, uint _entryAmt, bytes32 _poolType, uint8 _maxParticipation) public onlyPoolsOwner {
    ...
    function participate(bytes32 _poolId, address _token, bytes32 _teamId) public {
        ...
        if (pools[_poolId].spotsLeft == 0) pools[_poolId].status = PoolStatus.Full;
    ...
    function settlePoolUser(bytes32 _poolId, address _user, uint8 _result, address _token, uint _stake, uint _winAmount) public onlyPoolsOwner {
        ...
        if (pools[_poolId].status != PoolStatus.Completed) pools[_poolId].status = PoolStatus.Completed;
