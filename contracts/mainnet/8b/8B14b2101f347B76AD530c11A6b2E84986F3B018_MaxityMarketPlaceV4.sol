...
contract MaxityMarketPlaceV4 is Ownable, Pausable, IMaxityMarketPlace, IERC721Receiver, ReentrancyGuard {
    ...
    event TokenIdOnAuctionByPrice(address indexed _token, uint256 indexed _tokenid, address indexed buyer, uint256 price, uint price_idx);
    ...
    mapping(address => mapping(uint => uint)) public override soldCount;
    address public ut;
    ...
    function sellByFixPrice(address _token, uint256[] memory _tokenids, uint [] memory _prices, address _seller) nonReentrant external virtual override returns (uint) {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            saleInfo.base_price = 0;
            saleInfo.incre_price = 0;
            saleInfo.starttime = 0;
            saleInfo.deadline = 0;
            saleInfo.cur_bid_price = 0;
            saleInfo.price_offer = address(0x0);
    ...
    virtual override nonReentrant returns (uint amountU){
        uint256 length = _tokenids.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            uint256 _tokenid = _tokenids[pid];
            require(marketUnits[_token][_tokenid].seller == address(0x0), "Token already on the marketplace");
    ...
    function buy(address _token, uint256 _tokenid, address to) public virtual override nonReentrant {
        SaleInfo storage saleInfo = marketUnits[_token][_tokenid];
...
//        }
    ...
    function auctionOnFixPrice(address _token, uint _tokenid, uint _price, address price_offer, uint deadline) public nonReentrant override returns (uint){
        ...
        emit TokenIdOnAuctionByPrice(_token, _tokenid, price_offer, _price, offer_idx);
        return offer_idx;
    }
    ...
    function cancelOrWithdrawAuctionOnFixPrice(address _token, uint _tokenid, uint _offer_idx, address _token_to) public nonReentrant {
        ...
        require(offerInfo.price_offer == msg.sender || owner() == _msgSender(), "Only seller or owner can cancel or withdraw");
    ...
    function delistV2(address _token, uint256 _tokenid, address _to) public nonReentrant {
        ...
        if (saleInfo.isAuction) {
            require(block.timestamp < saleInfo.starttime || block.timestamp > saleInfo.deadline, "Auction have been started");
            require(block.timestamp <= saleInfo.deadline || saleInfo.price_offer == address(0x0), "The auction has the bidder");
        }
...
//        }
    ...
    function bid(address _token, uint256 _tokenid, uint256 amount, address to) public nonReentrant virtual override returns (bool){
        SaleInfo storage saleInfo = marketUnits[_token][_tokenid];
        require(saleInfo.seller != address(0x0), "This token is not for sale");
        require(IERC721(_token).ownerOf(_tokenid) == address(this), "This token is not on the marketplace");
        ...
        require(priceOffer >= nextbidPrice, "Invalid bid price");
    ...
    function tradeDone(address _token, uint256 _tokenid, uint total_amount, address buyer, TradeMode mode) internal {
        ...
        if (mode == TradeMode.BID_PRICE) {
            amount = total_amount;
            // fee already count at price offer stage
        } else {
        ...
        if (_soldCount == 0) {//first sell
            if (IMaxityMetadata(_token).disigner(_tokenid) == address(0x0)) {
                ...
                TransferHelper.safeTransfer(ut, saleInfo.seller, amount_2_ngo);
                //to ngo
                TransferHelper.safeTransfer(ut, IMaxityMetadata(_token).disigner(_tokenid), amount.sub(amount_2_ngo));
                //to designer
            }
            ...
            if (IMaxityMetadata(_token).disigner(_tokenid) == address(0x0)) {//ngo has no ability to design nft
                ...
                TransferHelper.safeTransfer(ut, IMaxityMetadata(_token).ngowallet(), amount_2_ngo);
                //to ngo
                TransferHelper.safeTransfer(ut, IMaxityMetadata(_token).disigner(_tokenid), futureSaleAmount.sub(amount_2_ngo));
                //to designer
            }
