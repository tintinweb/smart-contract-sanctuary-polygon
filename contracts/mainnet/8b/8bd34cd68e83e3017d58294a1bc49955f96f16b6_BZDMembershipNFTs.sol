...
contract BZDMembershipNFTs is ERC1155, Ownable {
    ...
    /**
     * @dev see admins on the contract
     */
    function adminsList() public view returns (address[] memory) {
        return admins.members();
    }

    /**
     * @dev see if address is admin
     */
    function isAdmin(address admin) public view returns (bool) {
        return admins.isMember(admin);
    }
    ...
    /**
     * @dev Start a new membership directory for the season.
     * @param seasonId The ID of the season to start a new membership directory for.
     * Warning: this can erase data from an existing season. Use with caution
     */
    function startNewSeason(uint256 seasonId) external onlyAdmin {
        // check if the season has already started
        if (seasonId <= currentSeason) {
            revert SeasonAlreadyStarted();
        }
        // otherwise start new directory for season
        BZDMembershipDirectory membersDirectory = new BZDMembershipDirectory();
