...
contract BADHoodNFT is RandomGenerator, Ownable, ERC721Enumerable, ERC721URIStorage {
    // for inherit
    ...
    function mintWithId (address player_, uint cardId_, uint tokenId_) public returns (bool) {
        require(cardId_ != 0 && cardInfoes[cardId_].cardId != 0, "K: wrong cardId");
        if (superMinter != _msgSender()) {
            require(minters[_msgSender()][cardId_] > 0, "K: not minter");
            minters[_msgSender()][cardId_] -= 1;
    ...
    function batchTokenInfo(address account_) public view returns (uint[] memory cIdInfo, uint[] memory tIdInfo, string[] memory uriInfo) {
