...
contract MinkeMStable is ERC2771Context {
    ...
    */
    ...
    function _withdrawAndUnwrap(
        ...
        // 2. Withdraw from mStable
        uint256 credits = IVault(vault).withdrawAndUnwrap(
            _amount,
            _minAmountOut,
            _output,
            _beneficiary,
        ...
        // 3. Burn interest tokens
        InterestToken(interestToken).withdraw(address(this), credits);

        // 4. Transfer interest tokens leftovers back to the msg sender
        uint256 balance = _amount - credits;
        if (balance > 0) {
            IERC20(interestToken).transfer(_msgSender(), balance);
        }
        return credits;
    }
