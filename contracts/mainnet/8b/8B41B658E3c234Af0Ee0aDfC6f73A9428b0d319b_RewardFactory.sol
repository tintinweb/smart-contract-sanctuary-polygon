    ...
    TokenWithdrawer
{
    ...
    function depositRewards(
        ...
        require(positions.length == tokens.length, "array lengths mismatch");
        require(tokenTypes.length == tokens.length, "array lengths mismatch");
        require(tokenIDs.length == tokens.length, "array lengths mismatch");
        require(amounts.length == tokens.length, "array lengths mismatch");
        require(descriptions.length == tokens.length, "array lengths mismatch");
    ...
    function claimRewards(address winner)
        ...
        nonReentrant
    {
        require(rewardState == RewardState.AWARDED, "No Positions");

        uint position = positionResults[winner];
        uint[] memory posRewards = positionRewards[position];
        uint numPosRewards = posRewards.length;

        // For each Reward awarded to this position.
        for (uint256 i = 0; i < numPosRewards; i++) {
            // Get rewardID.
            uint256 rewardID = posRewards[i];
    ...
    function getRewardsForPosition(uint position_)
        ...
        returns (uint[] memory)
    {
        return positionRewards[position_];
    }

    function setPositionResults(address[] memory results)
        external
        override
        onlyOwner
    {
