...
contract AutomaticSprouter is Ownable {
    ...
    function plant(uint256[] memory seedIds) public onlyOwner {
        ...
        for (uint256 i = 0; i < seedIds.length; i++) {
            ...
            plantedSeedIds.push(seedId);
        }
    }

    function sprout(address recipient, uint256 count) public onlyOwner {
        MojoERC721 mojo = MojoERC721(mojoAddress);
        Sprouter sprouter = Sprouter(sprouterAddress);

        for (uint256 i = 0; i < count; i++) {
            uint256 seedId = plantedSeedIds[i];

            sprouter.sprout(seedId, recipient);
            assert(mojo.ownerOf(seedId) == recipient);
        }
    }

    function plantedSeedCount() public view returns (uint256) {
        return plantedSeedIds.length;
    }
    ...
    {
        uint256 length = plantedSeedIds.length;
