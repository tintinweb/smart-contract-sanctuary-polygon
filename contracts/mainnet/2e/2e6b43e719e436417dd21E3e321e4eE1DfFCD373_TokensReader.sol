...
contract TokensReader {
  ...
  function getFarmers(address account, bool includeMetadata) public view returns (Token[] memory) {
    uint256 balance = farmers.balanceOf(account);
    Token[] memory tokens = new Token[](balance);

    uint256 index = 0;
    for (uint256 i = 0; index < balance; i++) {
      if (farmers.ownerOf(i) == account) {
        string memory metadata = includeMetadata ? farmers.tokenURI(i) : "";
        tokens[index++] = Token(address(farmers), i, 1, metadata);
      }
    }
    return tokens;
  }
