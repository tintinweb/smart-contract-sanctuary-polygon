...
contract Luchador is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function mint(uint256 amount) external {
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            //No need to rearrange array if the selected URI is the last item in the array
            if (selectedTokenURIIndex != (unmintedTokenURIs.length - 1)) {
            ...
            _setTokenURI(tokenId, selectedTokenURI);
        }
    }

    function multisigMint() external onlyOwner {
        require(unmintedTokenURIs.length >= multisigMintCount, "Not enough unminted token URIs to fulfil multisig allocation");
    ...
    function removeTokenURI(uint256 index) external onlyOwner {
        //No need to rearrange array if the index is the last
        if (index != (unmintedTokenURIs.length - 1)) {
            string memory lastUnmintedTokenURI = unmintedTokenURIs[unmintedTokenURIs.length - 1];
    ...
    function removeFromSale(uint256 luchadorId) external {
        ...
        require(price > 0, "Sale not found");
