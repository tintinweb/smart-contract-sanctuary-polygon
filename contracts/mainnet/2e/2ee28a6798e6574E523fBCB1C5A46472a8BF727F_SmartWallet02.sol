...
import { MetaTx02 } from "./MetaTx02.sol";
	...
	ERC2771Context
{
	...
	// Rules for upgrading:
	// 1. Make a new contract and inherit the latest version of SmartWallet
	...
	modifier onlyValidTOTP(bytes32[] memory confirmMaterial) {
		...
		twoFAInfo.counter = counterProvided + 1;
	...
	function setupHOTP2FA(
		...
		twoFAInfo.counter = initialCounter;
		emit HOTP2FASetup(
			hashStorageID,
			rootHash,
			merkleHeight,
