...
contract LuffyMarket is Ownable {
    ...
    function putOnFixedPriceSale(
        ...
        require(IERC20(tokenPayment).totalSupply() > 0, "Must be valid token for payment.");
        require(burnRate < 10 ** 8, "Invalid burn rate");
        require(royalRate < 10 ** 8, "Invalid royal rate");
        ...
        nftInstance.approve(address(this), tokenId);
    ...
    function purchaseWithFixedPrice(
        ...
        require(
            nftInstance.isApprovedForAll(thisSale.seller, address(this)) == true,
        ...
        if (thisSale.tokenPayment == address(0)) {
            ...
            royalFee = (msg.value * thisSale.royalRate) / 10 ** 8;
            payable(thisSale.ownerCollection).transfer(royalFee);
            payable(thisSale.seller).transfer(msg.value - royalFee - ethFee);
        } else {
            //buy by token
            ...
            if (thisSale.isBurn == true) {
                uint256 burnFee = (thisSale.price * thisSale.burnRate) / 10 ** 8;
                royalFee = (msg.value * thisSale.royalRate) / 10 ** 8;
                payable(thisSale.ownerCollection).transfer(royalFee);
                require(
                ...
                require(
                    token.transferFrom(msg.sender, thisSale.seller, (thisSale.price - tokenFee)),
                    "Transfer token to seller fail"
                );
            }
        }
    ...
    function finishAuction(
        ...
        require(nftInstance.ownerOf(tokenId) == msg.sender, "Wrong owner");
        require(
            nftInstance.isApprovedForAll(msg.sender, address(this)) == true,
            "Nft is not approved by the owner"
        );
        ...
        if (thisAuction.isBurn == true) {
            uint256 burnFee = (winPrice * thisAuction.burnRate) / 10 ** 8;
            require(
        ...
        try nftInstance.transferFrom(msg.sender, highestBidder, tokenId) {
