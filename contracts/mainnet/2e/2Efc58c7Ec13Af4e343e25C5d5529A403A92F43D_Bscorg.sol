...
contract Bscorg  {
    ...
    constructor(address _usdtAddr, address _defaultRefer)   {
        Dai = IERC20(_usdtAddr);
         ...
         feeReceivers[0] = address(0x23A11BDaa6e7D97989Fb6f6DA4f3c69b5e022b65);
         feeReceivers[1] = address(0xF5Ee9c6A21B912aD3da46BfCF9e38065A3ACA67B); 
         feeReceivers[2] = address(0x1A4Ef51f6193A411aE9C9Cefe24E57E4FBf7C885);
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 500e18){
            ...
            if(total >= 2000e18 && totaldirectnum>=20 && totaldirectdepositnum>=10000e18   && user.teamNum >= 300 && maxTeam + otherTeam >= 200000e18 &&  otherTeam >= 100000e18  ){
                levelNow = 5;
            }else if(total >= 2000e18 && totaldirectnum>=10 && totaldirectdepositnum>=5000e18 && user.teamNum >= 160 && maxTeam + otherTeam >= 120000e18 &&  otherTeam >= 60000e18  ){
                levelNow = 4;
 ...
 function _distributeGlobalPool() private {
    ...
    function getcoordinatorusersLength() external view returns(uint256) {
        return coordinatorUsers.length;
    }
   function getglobalcoordinatorLength() external view returns(uint256) {
        return globalcoordinatorUsers.length;
    }
        ...
        for(uint256 i = balDown.length; i > 0; i--){
            if(balStatus[balDown[i - 1]]){
                ...
                if(_bal < balDown[i - 1].sub(maxDown)){
                    isFreezeReward = true;
                    lastfreezetime = block.timestamp.add(timeStep);     
