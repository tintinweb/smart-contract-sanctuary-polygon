...
contract EthermonRerollPriceData is BasicAccessControl {
    ...
    function getEmonPriceFromOracle1(uint32 _classId)
        external
        view
        returns (uint256 class_price_eth)
    {
          EtheremonOracleInterface oracleContract = EtheremonOracleInterface(oracleContract);
          ...
          return class_price_eth;
         
          
    }
    function getEmonPriceFromOracle2(uint32 _classId)
        external
        view
        returns (uint256 class_price_eth)
    {
        uint256 class_price_eth = reRollPriceEth[_classId];
        require(class_price_eth > 0);
        uint256 emonPrice =  getEmonPriceFromOracle22(class_price_eth);
        return emonPrice; 
          
    }
    function getEmonPriceFromOracle33(uint256 eth)
        external
        view
        returns (uint256 class_price_eth)
    {
        EtheremonOracleInterface oracleContract = EtheremonOracleInterface(oracleContract);
        uint256 emonPrice = oracleContract.getEmonRatesFromEth(eth);
        return emonPrice;
   ...
   function getEmonPriceFromOracle22(uint256 eth)
        internal
        view
        returns (uint256 class_price_eth)
    {
