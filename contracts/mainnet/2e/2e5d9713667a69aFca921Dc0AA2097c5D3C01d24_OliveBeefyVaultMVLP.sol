...
interface IMvlpRouter {
    function mintAndStakeMvlp(address _token, uint256 _amount, uint256 _minUsdm, uint256 _minMvlp) external returns (uint256);
    function unstakeAndRedeemMvlp(address _tokenOut, uint256 _mvlpAmount, uint256 _minOut, address _receiver) external  returns (uint256);
     function mvlpManager() external  returns (address);
}
    ...
    ERC20Upgradeable
{
    ...
    event DepositLP(address indexed account, uint256 amount,
        uint256 round, uint256 swapTokenAmount,uint256 bps);

    event WithdrawLP(address indexed account, uint256 amount, 
        uint256 shares, uint256 round, uint256 swapTokenAmount,uint256 bps);
   
   event InstantWithdrawLP(
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        // is not returned
        _depositFor(amount, newTokensReceived, msg.sender);
    ...
    function withdrawInstantly(uint256 amount) external nonReentrant {
        ...
        if(mooTokenAmount > 0){
            ...
            emit InstantWithdrawLP(msg.sender, assetReceived, vaultState.round, mooTokenAmount, bv.getPricePerFullShare());
        }
    }
     ...
     */
    function _depositFor(uint256 baseAmount, uint256 amount, address creditor) private {        
        ...
        IBeefy bv = IBeefy(BEEFY_VAULT);
        emit DepositLP(creditor, baseAmount, currentRound, amount, bv.getPricePerFullShare());
    }
    ...
    function _withdraw(uint256 numShares) internal returns (uint256) {
       require(numShares > 0, "!numShares");
        ...
        emit WithdrawLP(msg.sender,  afterBalance.sub(prevBalance), numShares, vaultState.round, withdrawAmount, bv.getPricePerFullShare());
...
import {BeefyVaultV1} from "./BeefyVaultV1.sol";
import {IMvlpRouter} from "../../interfaces/IMvlpRouter.sol";
import {IBeefy} from "../../interfaces/IBeefy.sol";
import "hardhat/console.sol";

contract OliveBeefyVaultMVLP is 
    BeefyVaultV1,
    PolysynthKikoVaultStorage
{
    ...
    function initialize(
        ...
        string memory _tokenSymbol,
        Beefy.VaultParams calldata _vaultParams,
        IMvlpRouter _mvlpRouter,
        address _token
    ) external initializer {
        mvlpRouter = _mvlpRouter;
        mvlpManager = mvlpRouter.mvlpManager();
        usdcToken = _token;
    ...
    function setGlpRewardRouter(address _glpRouter) external onlyOwner {
        ...
        mvlpRouter = IMvlpRouter(_glpRouter);
        mvlpManager = mvlpRouter.mvlpManager();
    }
    ...
    function settle(uint256 _amount) external nonReentrant {
        ...
        require(!optionState.isSettled && optionState.isBorrowed,"already settled");
        ...
        if(_amount == 0 ){
            ...
            IERC20(usdcToken).approve(mvlpManager, _amount);
    ...
    function _buyGlpToken(uint256 _usdcAmount, uint256 _minGlpAmount) internal {
        mvlpRouter.mintAndStakeMvlp(usdcToken, _usdcAmount, 0, _minGlpAmount);
    }

    function _sellGlpToken(uint256 _glpAmount) internal {
        mvlpRouter.unstakeAndRedeemMvlp(usdcToken, _glpAmount, 0, address(this));
    }
}
