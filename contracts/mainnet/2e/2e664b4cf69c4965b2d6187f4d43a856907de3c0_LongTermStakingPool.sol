...
contract LongTermStakingPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 macaronAtLastUserAction; // keep track of macaron deposited at the last user action.
    ...
    /**
     * @notice Unlock user macaron funds.
    ...
    /**
     * @notice Deposit funds into the Macaron Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in MCRN)
    ...
    function depositOperation(
        ...
        user.macaronAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Macaron Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (user.shares > 0) {
            user.macaronAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Macaron Pool
     */
