...
contract Coin is ERC20PresetMinterBurnableUpgradeable {
    ...
    mapping (address /*account*/ => bool) public isTaxWhitelisted;
    mapping (address /*account*/ => bool) public isTaxWhitelistedToReceive;
    ...
    /// @notice Adds an address to whitelist (only owner can call)
    ...
    /// @notice Removes an address to whitelist (only owner can call)
    /// @param addr some address
    ...
    /// @param totalSupplyValue total supply of the token
    ...
    function _transfer(address from, address recipient, uint256 amount) internal virtual override {
        uint256 initial_amount = amount;

        bool isUsualTransfer = true;  // we use it as a flag that we should write off the commission for a regular transfer

        if(isPool[recipient] && (!isPool[from])) {
            isUsualTransfer = false;
            if (!isTaxWhitelisted[from]) {
                if (saleDEXTaxNumerator > 0){
                    uint256 tax = initial_amount * saleDEXTaxNumerator / DENOMINATOR;
                ...
                if(saleDEXTaxForLPNumerator > 0) {
                    uint256 tax = initial_amount * saleDEXTaxForLPNumerator / DENOMINATOR;
        ...
        if(isPool[from] && (!isPool[recipient])) {
            isUsualTransfer = false;
            if (!isTaxWhitelisted[recipient]) {
                if (purchaseDEXTaxNumerator > 0) {
                    uint256 tax = initial_amount * purchaseDEXTaxNumerator / DENOMINATOR;
                    super._transfer(from, treasury, tax);
                    amount -= tax;
                    emit PurchaseDEXTaxPaid({purchaser: recipient, pool: from, treasury: treasury, taxAmount: tax});
                }
                if (purchaseDEXTaxForLPNumerator > 0) {
                    uint256 tax = initial_amount * purchaseDEXTaxForLPNumerator / DENOMINATOR;
                    super._transfer(from, treasuryLP, tax);
                    amount -= tax;
        ...
        if (
            ...
            uint256 tax = initial_amount * transferTaxNumerator / DENOMINATOR;
            super._transfer(from, treasury, tax);
            amount -= tax;
