...
contract BulkAirDrop is IBulkAirdrop, Ownable{
    ...
    function bulkAirdropNFT(
        ...
        address[] calldata _recipients
        ) override isWhitelisted(_msgSender()) external returns (bool){            
            ...
            for (uint256 i = 0; i < recipientCount; i++) {
                nftCollection.safeTransferFrom(_msgSender(), _recipients[i], _assetId, 1, "");
            }
...
interface IBulkAirdrop {
    ...
    function bulkAirdropNFT(address collection, uint nftId, address[] calldata _recipients) external returns (bool);
    event BulkAirdrop(address owner,address collection, uint nftId, address[] _recipients);
