...
contract BalanceTrigger is BaseSiloAction{
    ...
    function enter(address , bytes memory , bytes memory inputData) public override pure returns(uint[5] memory outputAmounts){
        uint[5] memory inputAmounts = abi.decode(inputData, (uint[5]));
    ...
    function exit(address , bytes memory , bytes memory outputData) public override pure returns(uint[5] memory outputAmounts){
        uint[5] memory inputAmounts = abi.decode(outputData, (uint[5]));
        outputAmounts[0] = inputAmounts[0];
        outputAmounts[1] = inputAmounts[1];
        outputAmounts[2] = inputAmounts[2];
        outputAmounts[3] = inputAmounts[3];
    ...
    function createConfig(address[5] memory _inputs, address[5] memory _outputs, uint[5] memory _triggers) public pure returns(bytes memory configData){
        configData = abi.encode(_inputs, _outputs, _triggers);
    }

    function decodeConfig(bytes memory configData) public pure returns(address[5] memory inputs, address[5] memory outputs, uint[5] memory triggers){
        (inputs, outputs, triggers) = abi.decode(configData, (address[5], address[5], uint[5]));
    }

    function checkMaintain(bytes memory configuration) public view override returns(bool){
        uint[5] memory triggerAmounts;
        address[5] memory inputs;
        (inputs,,triggerAmounts) = abi.decode(configuration, (address[5],address[5],uint[5]));
    ...
    function validateConfig(bytes memory configData) public pure override returns(bool){
        ...
        uint[5] memory triggers;
        (input,output,triggers) = abi.decode(configData, (address[5],address[5],uint[5]));
        for(uint i; i<5;){
            if(input[i] != output[i]){
                return false;
            }
