...
/**
 * @dev Random Mint Contract for WeirdBand.
 *
 * This contract features modules for:
 *
 * - Randomization using Chainlink
 * - Sub-collections within a contract
 * - Sub-collection modularization
 *
 */
contract WeirdBandNFT is VRFConsumerBaseV2, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    /*
     * @dev Constructor:
     * - Sets the initial configurations of the contract, such as the NFT price, maximum quantity per wallet, and maximum number of NFTs in the collection.
     * - Initializes variables related to Chainlink for random number generation.
     */
     ...
     *
     * - Checks if the contract is enabled for minting.
     * - Verifies if the requested NFT quantity is within the limit.
     * - Verifies if the sent value is sufficient to pay for the NFTs.
     * - Verifies if the sender's address is whitelisted (if whitelist is enabled).
     * - Generates a random number request using Chainlink.
     * - Increments relevant counts and emits events.
     *
     */
    ...
    function mintNFT(uint32 nftsQuantity, bytes memory _signature) public  payable nonReentrant returns (uint256[] memory) {
		...
		require(nftsQuantity <= 3 && nftsQuantity > 0, "You can only mint 3 NFTs at a time");
    ...
    /*
    * @dev Auxiliary functions for removing NFTs from arrays:
    *
    ...
    */
    ...
    /*
     * @dev mintNFTOwner function:
     *
     * - Allows the contract owner to mint NFTs for themselves.
     * - Verifies if the requested NFT quantity is within the limit for the owner.
     * - Generates a random number request using Chainlink.
     * - Increments relevant counts and emits events.
     */
    ...
    // @dev Signature verification functions:
    // Allow verifying if a message was signed by the contract owner
    ...
    // @dev Chainlink-related functions:
    // Used to handle requests and receipts of random numbers provided by Chainlink.
    ...
    *
    * - Called by Chainlink to provide the requested random numbers.
    * - Selects an NFT based on the random number and current collection.
    * - Increments relevant counts and emits events.
    */
    ...
    /*
     * @dev mintCollectionNFT function:
     * 
     * - Performs the minting of an NFT, associating it with an address and setting its URI.
     * - Calls the auxiliary functions to remove the NFT from the corresponding array.
     * - Emits a minted NFT event.
    */
    ...
    function mintCollectionNFT(uint256[2] memory numberR, address _address, uint256 tokenIdNFT) private {
        ...
        string memory nftURI = NFTCharacterToURI[h];
        ...
        emit NFTMinted(_tokenIds.current(), nftURI, _address);
      
    }

    //CONTRACT CONFIG:
    // @dev Contract configuration functions:
    // Allow the owner to update general contract settings such as maximum NFTs in the collection, NFT price, and current collection
    ...
    // @dev Array and URI manipulation functions:
    // Allow the owner to update the arrays of rare and common NFTs and associate URIs with NFT IDs.
    function setArrays( uint256[] memory _nftsRare, uint256[] memory _nftsComum ) public onlyOwner {
        nftsRare = _nftsRare;
        nftsComum = _nftsComum;
    }

    // @dev setURIs function:
    // Allows the owner to associate URIs with NFT IDs.

    function setURIs( uint256[] memory _nftsArray, string[] memory _nftsURIs) public onlyOwner {
        for (uint i = 0; i < _nftsArray.length; i++) { 
            NFTCharacterToURI[_nftsArray[i]] = _nftsURIs[i];
        }
    }
