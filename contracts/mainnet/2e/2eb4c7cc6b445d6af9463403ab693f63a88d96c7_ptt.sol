 ...
 */
 ...
 contract ptt is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable, ERC721Pausable, ERC721URIStorage, Ownable, ERC2981PerTokenRoyalties {
      ...
      * See {ERC721-tokenURI}.
      */
     constructor() ERC721("purothek", "ptt") {
          _setupRole(DEFAULT_ADMIN_ROLE, 0x388C9762944364710f1326EB3dEAB5E86Bd8fBb1);
          _setupRole(MINTER_ROLE, 0x388C9762944364710f1326EB3dEAB5E86Bd8fBb1);
          _setupRole(MINTER_ADMIN_ROLE, 0x388C9762944364710f1326EB3dEAB5E86Bd8fBb1);
          _setRoleAdmin(MINTER_ROLE, MINTER_ADMIN_ROLE);
          _baseTokenURI = "ipfs://";
          transferOwnership(0x388C9762944364710f1326EB3dEAB5E86Bd8fBb1);
     }
     ...
     function setBaseRoyalty (address recipient, uint256 percent, uint256 amount) public {
         require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ptt/minter-role-required");
         _recipient = recipient;
         _percent = percent;
         _amount = amount;
     }

     function _getRecipient() public view returns (address) {
         return _recipient;
     }

     function _getPercent() public view returns (uint256) {
         return _percent;
     }

     function _getAmount() public view returns (uint256) {
         return _amount;
     }
     ...
     function mint_by_id(address to, uint256 tokenId, string memory tokenIPFSURI) public virtual {
         require(hasRole(MINTER_ROLE, _msgSender()), "ptt/minter-role-required");
         _mint(to, tokenId);
         _setTokenURI(tokenId, tokenIPFSURI);
         _setTokenRoyalty(tokenId, _getRecipient(), _getPercent(), _getAmount()); 
     }
     ...
     function bulk_mint(address[] memory to, uint256[] memory tokenId, string[] memory tokenIPFSURI) public virtual {
         ...
         while (i < tokenId.length) {
             ...
             _setTokenRoyalty(tokenId[i], _getRecipient(), _getPercent(), _getAmount());
             i += 1;
         }
     }     
      ...
      * - the caller must have the `MINTER_ROLE`.
      */      
     function mint_by_id(address to, uint256 tokenId, string memory tokenIPFSURI, address creator, uint256 percent, uint256 amount) public virtual {
         require(hasRole(MINTER_ROLE, _msgSender()), "ptt/minter-role-required");
         _mint(to, tokenId);
         _setTokenURI(tokenId, tokenIPFSURI);
         if (percent == 0 && amount == 0){
            ...
            _setTokenRoyalty(tokenId, creator, percent, amount);     
         }
      ...
      * - the caller must have the `MINTER_ROLE`.
      */
     function bulk_mint(address[] memory to, uint256[] memory tokenId, string[] memory tokenIPFSURI, address[] memory creator, uint256[] memory percent, uint256[] memory amount) public virtual {
         require(hasRole(MINTER_ROLE, _msgSender()), "ptt/minter-role-required");
         ...
         while (i < tokenId.length) {
             _mint(to[i], tokenId[i]);
             _setTokenURI(tokenId[i], tokenIPFSURI[i]);
			   //_setTokenRoyalty(tokenId[i], creator[i], percent[i], amount[i]);
             if (percent[i] == 0 && amount[i] == 0){
                 _setTokenRoyalty(tokenId[i], _getRecipient(), _getPercent(), _getAmount()); 
             } else{
                 _setTokenRoyalty(tokenId[i], creator[i], percent[i], amount[i]);     
             }
