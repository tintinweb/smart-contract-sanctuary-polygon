...
contract SeedOrphanageV2 is Ownable, IERC721Receiver {
    // variables
    ...
    function clearRegister() public onlyOwner {
        ...
        for (uint i = 0; i < adoptionRegistry.length; i++) {
            registrationMap[adoptionRegistry[i]] = false;
        }
    ...
    function isRegistered(address _address) public view returns (bool) {
        if (!registrationMap[_address]) {
            return false;
        }
        return registrationMap[_address];
    }
