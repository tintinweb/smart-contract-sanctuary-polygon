...
// File: libraries/SafeMathCopy.sol
...
contract FlashSwapper is KeeperCompatibleInterface {
        ...
        )
    {
        ...
        uint256 maxProfit;
        uint256 tempProfit;
        int rewardTokenPriceInMATIC;

        if(WETH != rewardToken) {
            rewardTokenPriceInMATIC = getDerivedPrice();
        } else {
            rewardTokenPriceInMATIC = 1e18;
        }
        ...
        if(rewardTokenPriceInMATIC > 0) {
            if(maxProfit.div(1e10).mul(uint256(rewardTokenPriceInMATIC)) > minProfitInMATIC.mul(1e8)) {
                upkeepNeeded = true;
