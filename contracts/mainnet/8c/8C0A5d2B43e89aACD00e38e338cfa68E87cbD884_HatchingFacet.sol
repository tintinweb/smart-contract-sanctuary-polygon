...
contract HatchingFacet {
    ...
    function beginHatchingWithSignature(
        ...
        bytes32 hash = beginHatchingGenerateMessageHash(roundTripId, tokenId, inheritanceChance, bundleId, blockDeadline);
    ...
    function retryHatchingWithSignature(
        ...
        bytes32 hash = retryHatchingGenerateMessageHash(roundTripId, bundleId, blockDeadline);
        address gameServer = LibDiamond.gameServer();
        require(
            SignatureChecker.isValidSignatureNow(gameServer, hash, signature),
    ...
    function retryHatchingGenerateMessageHash(uint256 roundTripId, uint256 bundleId, uint256 blockDeadline) public view returns (bytes32) {
        bytes32 structHash = keccak256(
            abi.encode(
                keccak256(
    ...
    function finishHatchingGenerateMessageHash(uint256 roundTripId, string memory tokenURI, uint256 bundleId, uint256 blockDeadline) public view returns (bytes32) {
        bytes32 structHash = keccak256(
            abi.encode(
                keccak256(
                    "finishHatchingPayload(uint256 roundTripId, string memory tokenURI, uint256 bundleId, uint256 blockDeadline)"
                ),
                roundTripId,
                tokenURI,
                bundleId,
    ...
    function finishHatchingWithSignature(
        ...
        bytes32 hash = finishHatchingGenerateMessageHash(roundTripId, tokenURI, bundleId, blockDeadline);
        address gameServer = LibDiamond.gameServer();
        require(
            SignatureChecker.isValidSignatureNow(gameServer, hash, signature),
    ...
    function beginHatching(
        ...
        LibHatching.beginHatching(roundTripId, blockDeadline, tokenId, inheritanceChance);
    }
    ...
    function finishHatching(uint256 roundTripId, string memory tokenURI) internal {
        bytes32 vrfRequestId = LibHatching.getVRFRequestId(roundTripId);
        uint256 tokenId = LibHatching.getTokenId(vrfRequestId);
        LibHatching.finishHatching(roundTripId, tokenId, vrfRequestId, tokenURI);
    }
}
