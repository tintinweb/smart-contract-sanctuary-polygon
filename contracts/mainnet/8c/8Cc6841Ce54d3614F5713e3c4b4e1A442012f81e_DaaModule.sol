...
/// @title Safe Module DAA - A gnosis safe module allowing tokenization of deposits and shares redemption.
...
contract DaaModule is Ownable {
    ...
    uint public totalSharesOutstanding;
    ...
    constructor(GnosisSafe safe){
        _whitelisted = payable(address(this));
        _safe = safe;
        _AuthorizedCurrencyTickers["USDC"] = true;
        _addSupportedCurrency("USDC", 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174, true);//check address checksum
        _ETH = keccak256("ETH"); // To do: should get this value oc already to save gas
    }
        ...
        returns (uint)
    {
        ...
        // calculate fund nav
        uint pricePerShare = pricePerShare();
        ...
        uint tokenValue = getTokenValueUsd(currencyTicker, amount);
    ...
    function redeem(uint nOfShares) external returns (bool){
        ...
        _withdrawFromSafe(msg.sender, baseAmount);
    ...
    {
        // Transfer token
        _transfer(_safe, token, _whitelisted, amount);
    ...
    function checkFundsAvailability(address token, uint amount) public view returns (uint){
        uint liquidBalance = IERC20(token).balanceOf(address(_safe));
    ...
    function calculateNav() public view returns (uint) {
        ...
        for (uint256 i; i < length; i++) {
            internalBalances[i] = IERC20(allowedAssets.at(i)).balanceOf(address(_safe));
        }
    ...
    function _withdrawFromSafe(address _to, uint amount) internal {
        ...
        require(amountLeft == 0, "Not enough funds in Safe");
        for (uint i =0; i< len; i++) {
            if ( baseCurrencyAmount[i]> 0){
                _transfer(_safe, baseCurrencies[i], _whitelisted, baseCurrencyAmount[i]);
    ...
    function _transfer(GnosisSafe safe, address token, address payable to, uint amount) private {
        if (token == address(0)) {
    ...
    function _addBaseCurrency(address token) internal {
        // could add a decimals condition or decimal storage here
