...
contract ML_SwapPathFinder
{
    ...
    {        
        ...
        {
            //check direct path
            swapPath = makeSwapPath(_from, _to, IToken(address(0)));
            amountOut = getSwapEstimate(_amountIn, _routers[n], swapPath);
            pathInfo[n * _routers.length] = SwapPathInfo(
            {
                amountOut: amountOut,
                router: _routers[n],
                path: swapPath
            ...
            for (uint256 m = 0; m < _additionalTokens.length; m++)
            {
                ...
                swapPath = makeSwapPath(_from, _to, _additionalTokens[m]);
                amountOut = getSwapEstimate(_amountIn, _routers[n], swapPath);
                pathInfo[(n * _routers.length) + m + 1] = SwapPathInfo(
                {
                    amountOut: amountOut,
                    router: _routers[n],
                    path: swapPath
    ...
    function removeEmptyPathes(SwapPathInfo[] memory _pathes) public pure returns (SwapPathInfo[] memory)
    {
        //count used
        uint256 used = 0;
