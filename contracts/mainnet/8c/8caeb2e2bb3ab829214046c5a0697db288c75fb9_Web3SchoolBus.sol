...
contract Web3SchoolBus is ERC721, ReentrancyGuard, Ownable {
  using Counters for Counters.Counter;

  constructor(string memory customBaseURI_, address proxyRegistryAddress_)
    ERC721("Web 3.0 School Bus", "JIBAROBUS")
  {
  ...
  function mint(uint256 count, address client, string memory customBaseURI_) public nonReentrant onlyOwner {
    ...
    for (uint256 i = 0; i < count; i++) {
      _mint(client, totalSupply());

      supplyCounter.increment();
    }

   // safeTransferFrom(msg.sender,client,tokenId);
  }
