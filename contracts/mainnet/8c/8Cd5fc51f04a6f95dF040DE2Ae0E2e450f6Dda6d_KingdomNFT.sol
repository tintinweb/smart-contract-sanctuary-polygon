...
contract KingdomNFT is Ownable, ReentrancyGuard, Pausable, AccessControl, ERC721Holder {
    ...
    constructor(
        ...
        require(_harvestInterval <= MAXIMUM_HARVEST_INTERVAL, "ERR1");
    ...
    function canWithdraw() public view returns (bool) {
        return !withdrawWhenFinished || block.timestamp >= endTime;
    }

    // View function to see if user can harvest reward token.
    function canHarvest(address _userAddress) public view returns (bool) {
        return block.timestamp >= endTime || block.timestamp >= userInfo[_userAddress].nextHarvestUntil;
    }
