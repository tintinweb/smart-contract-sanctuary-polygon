...
contract Airdrop300 is Ownable, Pausable {
    ...
    function addWhitelist(address[] memory _user) public onlyOwner {
        for (uint256 index = 0; index < _user.length; index++) {
            checkClaim[_user[index]] = true;
        }
    }

    function claimToken() public {
        require(checkClaim[msg.sender] == true, "Error: Claimed");
        require(startTime < block.timestamp, "Error: Is not time claim");
        checkClaim[msg.sender] = false;
        usdtToken.transferFrom(msg.sender, fundAddress, 1 * (10**uint256(6)));
        FEN(fenToken).mint(msg.sender, totalClaim);
    }
}
