    ...
    OwnableUpgradeable
{
    ...
    mapping(address => uint256) private _tokenLeftToWithdraw;

    mapping(address => uint256) public tokenLeftToWithdraw;
    ...
    function claimReward() public nonReentrant {
        ...
        require(
            _iswithdrawingWithinMinimum(amountToTransfer),
            "Withdrawal amount is below minimum"
        );

        if (!_isWithdrawalWithinMaximum(amountToTransfer)) {
            //we need to put the excess amount inside a leftwithdrawal
            uint256 maxTokenCanWithdraw = calculateTokenToDollarsNeeded(
                maximumWithdrawal
            );
            //get the difference and put inside excess token mappings
            uint256 excess = amountToTransfer - maxTokenCanWithdraw;
            tokenLeftToWithdraw[msg.sender] =
                tokenLeftToWithdraw[msg.sender] +
                excess;
            amountToTransfer = maxTokenCanWithdraw;
        } else {
            //we are within the withdrawal range. check if we have excess to add
            uint256 excessTokenLeft = tokenLeftToWithdraw[msg.sender];
            uint256 maxTokenCanWithdraw = calculateTokenToDollarsNeeded(
                maximumWithdrawal
            );
            //if the excess token left + amount withdrawing >= max
            if ((excessTokenLeft + amountToTransfer) >= maxTokenCanWithdraw) {
                tokenLeftToWithdraw[msg.sender] =
                    (excessTokenLeft + amountToTransfer) -
                    maxTokenCanWithdraw;
                amountToTransfer = maxTokenCanWithdraw;
            } else {
                //we are good just transfer the excess left and the amountToTransfer
                amountToTransfer = excessTokenLeft + amountToTransfer;
            }
        }
    ...
    function _iswithdrawingWithinMinimum(uint256 _amount)
        private
        view
        returns (bool)
    {
    ...
    function _isWithdrawalWithinMaximum(uint256 _amount)
        private
        view
        returns (bool)
    {
        return _amount <= calculateTokenToDollarsNeeded(maximumWithdrawal);
    }
