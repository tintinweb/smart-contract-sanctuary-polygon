...
interface ILPPosition is IERC721Metadata {
    ...
    function pair(uint _tokenId) external view returns (address);
    function setApprovedUpshotPool(address upshotPool_) external;
    function mint(address to, address _pair) external returns(uint256);
    function burn(address sender_, address pair_) external;
}
...
import "../pool/CreateERC20PairParams.sol";
import {PoolType} from "../../sudoswap/PoolType.sol";

interface ILSSVMPairFactory {
    function createPairERC20(CreateERC20PairParams calldata params) external returns (address pair);
...
import "./CreateERC20PairParams.sol";
...
//
import {IOwnershipTransferCallback} from "../../sudoswap/lib/IOwnershipTransferCallback.sol";

import {AdminTwoStep} from "../util/AdminTwoStep.sol";
import {AssetRecovery} from "../util/AssetRecovery.sol";
import {Strings} from "../../../node_modules/@openzeppelin/contracts/utils/Strings.sol";
import {INFT} from "../interface/INFT.sol";
import {IUpshotPool} from "../interface/IUpshotPool.sol";
import {ILSSVMPair} from "../interface/ILSSVMPair.sol";
import {ILSSVMPairFactory} from "../interface/ILSSVMPairFactory.sol";
import {ILSSVMPairFactoryLike} from "../../sudoswap/ILSSVMPairFactoryLike.sol";
import {ICurve} from "../../sudoswap/bonding-curves/ICurve.sol";
import {LPPosition} from "../token/LPPosition.sol";
import {UpshotPoolFactory} from "./UpshotPoolFactory.sol";

contract UpshotPool is IUpshotPool, AdminTwoStep, AssetRecovery {
    ...
    address immutable _upshotPoolFactory;

    bool public _initialized;
    address public _token;

    PoolType public _poolType;
    address public _nft;
    ...
    mapping(address => address) private _ownerToPair;
    mapping(address => address) private _pairToOwner;
    mapping(address => bool) private _componentPair;
    ...
    /**
     * @notice modifier to restrict access to the admin
     */
    modifier onlyAdminPair() {
        bool authAdmin = msg.sender == admin;
        bool authPair = _componentPair[msg.sender] == true;
        require(authAdmin || authPair, "UpshotPool: xinvalid msg.sender");
        _;
    }
    ...
    function createPoolETH(
        ...
        _delta = delta_;
        _poolType = poolType_;
        _spotPrice = spotPrice_;
        _bondingCurve = bondingCurve_;
        
        _setAdmin(owner_);

        address pair_ = _createPairETH(tokenIdList_, owner_, msg.value);
        //_mintLPTokens(msg.sender, pair_);
        return pair_;    
    }
    ...
    function createPoolERC20(
        ...
        _fee = params.fee;
        _delta = params.delta;
        _poolType = params.poolType;
        _spotPrice = params.spotPrice;
        _bondingCurve = params.bondingCurve;
        
        _setAdmin(params.assetRecipient); //TODO: owner);

        IERC20(_token).approve(address(_sudoswapPairFactory), type(uint256).max);

        address pair_ = _createPairERC20(params.tokenIdList, params.assetRecipient, params.initialTokenBalance);
     ...
     * 
     * Tyler: Petition to rename this function to "create new liquidity position" or "deposit liquidity"
     */
    function _createPairETH(
            uint256[] calldata tokenIdList_,
            address payable owner_,
            uint256 valueInWei
        ) private returns (address pair) { 
        INFT(_nft).setApprovalForAll(_sudoswapPairFactory, true);
        address pair_;
        if (PoolType(_poolType) == PoolType.TRADE) {
            pair_ = ILSSVMPairFactory(_sudoswapPairFactory).createPairETH{value: valueInWei}(
                _nft,
                _bondingCurve,
                payable(address(0)),
                _poolType,
        ...
        _ownerToPair[owner_] = pair_;
        _pairToOwner[pair_] = owner_;
        _componentPair[pair_] = true;
        return pair_; 
    }
    ...
    function _createPairERC20(
        ...
        if (PoolType(_poolType) == PoolType.TRADE) {
            ...
            pair_ = ILSSVMPairFactory(_sudoswapPairFactory).createPairERC20(params);
        } else {
            CreateERC20PairParams memory params  = CreateERC20PairParams(
                _token,
                _nft,
     ...
     */
    function addLiquidityETH(uint256[] calldata tokenIdList_, uint256 valueInWei) external payable override {
        address owner_ = msg.sender;

        if(_ownerToPair[owner_] != address(0)){
            bool previouslyEmpty = _checkIfPairEmpty(_ownerToPair[owner_]);
            _transferNFT(msg.sender, _ownerToPair[owner_], tokenIdList_);
            _transferETH(_ownerToPair[owner_], valueInWei);

            emit LiquidityAdded(_ownerToPair[owner_]);
            if(previouslyEmpty)
                _mintLPTokens(msg.sender, _ownerToPair[owner_]);
            return;
        }
        _transferNFT(msg.sender, address(this), tokenIdList_);
        _createPairETH(tokenIdList_, payable(owner_), valueInWei);
    }
     ...
     * this function will DESTROY the existing LP position and mint a new one to the user
     */
    function addLiquidityERC20(uint256[] calldata tokenIdList, uint256 countERC20) external override {
        // if pair was empty prior to this call, create a new LP token to represent the existing liquidity
   ...
   /**
     *
     */
    function _withdrawERC721(uint256[] calldata tokenIdList) private {
        address owner_ = msg.sender;
        require(_ownerToPair[owner_] != address(0), "UpshotPool: msg.sender not pair owner");

        ILSSVMPair pair_ = ILSSVMPair(_ownerToPair[owner_]);
        pair_.withdrawERC721(_nft, tokenIdList);

        for (uint256 i = 0; i < tokenIdList.length; ) {
            INFT(_nft).transferFrom(address(this), owner_, tokenIdList[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function withdrawERC721(uint256[] calldata tokenIdList) public {
        _withdrawERC721(tokenIdList);
    }

    function withdrawERC20(uint256 amount) public {
        ILSSVMPair pair_ = ILSSVMPair(_ownerToPair[msg.sender]);
        pair_.withdrawERC20(_token, amount);
     ...
     * this function will DESTROY the existing LP position and mint a new one to the user
     */
    function pullLiquidityETH(uint256[] calldata tokenIdList, uint256 valueInWei) external override {
        _withdrawERC721(tokenIdList);
        
        ILSSVMPair pair_ = ILSSVMPair(_ownerToPair[msg.sender]);
        pair_.withdrawETH(valueInWei);
        bool pairNowEmpty = _checkIfPairEmpty(_ownerToPair[msg.sender]);
        if (pairNowEmpty) {
            _burnLPTokens(msg.sender, _ownerToPair[msg.sender]);
        }
    }
    ...
    /** 
     */
    function pullLiquidityERC20(uint256[] calldata tokenIdList, uint256 countERC20) external override {
        // if pair is empty, burn LP token from it
        //_burn(msg.sender, tokenIdList.length);


        /* at the end
        bool pairNowEmpty = _checkIfPairEmpty(_ownerToPair[owner_]);
        if (pairNowEmpty) {
            _burnLPTokens(owner_,_ownerToPair[owner_]);
        }
        */
    }
     ...
     */
    function _transferNFT(address sender, address receiver, uint256[] calldata tokenIdList) private {
        for (uint256 i = 0; i < tokenIdList.length; ) {
            INFT(_nft).transferFrom(sender, receiver, tokenIdList[i]);
            unchecked {
                ++i;
            }
        }      
    ...
    /**
     * @notice internal helper function to mint LP tokens
     */
    function _mintLPTokens(address recipient_, address sudoPair_) private {
        LPPosition lpNFT = LPPosition(UpshotPoolFactory(payable(_upshotPoolFactory)).nftCollectionToLPPosition(_nft));
        lpNFT.mint(recipient_, sudoPair_);
    }

    /**
     * @notice internal helper function to burn LP tokens
     */
    function _burnLPTokens(address sender_, address sudoPair_) private {
        LPPosition lpNFT = LPPosition(UpshotPoolFactory(payable(_upshotPoolFactory)).nftCollectionToLPPosition(_nft));
        lpNFT.mint(sender_, sudoPair_);
    }

    /**
     * @notice Internal helper function to transfer ETH to a recipient
     */
    function _transferETH(address recipient, uint256 valueInWei) private {
        (bool success,) = payable(recipient).call{value: valueInWei}("");
        require(success, "UpshotPool: value transfer unsuccessful");
    }

    /**
     * @notice Internal helper function to transfer ERC20 tokens to a recipient
     */
    function _transferERC20(address sender_, address recipient_, uint256 numTokens_) private {
        bool success = IERC20(_token).transferFrom(sender_, recipient_, numTokens_);
        require(success, "UpshotPool: ERC20 transfer unsuccessful");
    }


    /**
     * @dev checks if sudoswap Pair associated with this Pool has money or NFTs in it
     * @param pair_ sudoswap pair address
     */
    function _checkIfPairEmpty(address pair_) private view returns (bool) {
        LSSVMPair pair = LSSVMPair(pair_);
        ILSSVMPairFactoryLike.PairVariant pv = pair.pairVariant();
        if(pv == ILSSVMPairFactoryLike.PairVariant.ENUMERABLE_ETH 
          || pv == ILSSVMPairFactoryLike.PairVariant.MISSING_ENUMERABLE_ETH) {
            return pair_.balance == 0 && pair.getAllHeldIds().length == 0;
        } else {
            // apparently we're SSTORE-ing the token 
            // todo: stop storing state other people already store
            // ERC20 t = pair.token();
            return IERC20(_token).balanceOf(pair_) == 0 && pair.getAllHeldIds().length == 0;
        }
    }
     ...
     */
    function nft() external view returns (address nft_) {
        return _nft;
    }
    ...
    function assetRecipient(address _pair) external view returns (address owner) {
        return ILSSVMPair(_pair).getAssetRecipient();
    }

    event OwnershipTransferred(address indexed newOwner);

    //TODO: instead of two mappings we can maybe track this in a struct
    function transferOwnership(address _pair, address _newOwner) external {
        require(_ownerToPair[msg.sender] == _pair, "UpshotPool: invalid msg.sender");

        delete _ownerToPair[msg.sender];

        _ownerToPair[_newOwner] = _pair;
        _pairToOwner[_pair] = _newOwner;

        emit OwnershipTransferred(_newOwner);

        // Call the on ownership transfer callback if it exists
        // @dev try/catch is around 5k gas cheaper than doing ERC165 checking
        if (_newOwner.code.length > 0) {
            try
                IOwnershipTransferCallback(_newOwner).onOwnershipTransfer(msg.sender)
            {} catch (bytes memory) {}
        }
    }
    ...
    //TODO: instead of two mappings we can maybe track this in a struct
    function ownerOf(address _pair) external view returns (address owner) {
        return _pairToOwner[_pair];
    }

    /**
     * 
     */
    function pairOf(address _owner) public view virtual override returns (address pair) {
        address pair_ = _ownerToPair[_owner];
        require(pair_ != address(0), "UpshotPool: msg.sender not pair owner");
        return pair_;
    }
    ...
    /**
     * @notice This contract should not ever be sent ether
     */
    receive() external payable {}

    /**
     * @notice NFTs should not be sent to this contract directly
     * only this contract should take the NFTs itself from the user (pull not push)
     */
     ...
     */
    function changeSpotPrice(uint128 newSpotPrice) external onlyAdminPair {
        require(
     ...
     */
    function changeDelta(uint128 newDelta) external onlyAdminPair {
        require(
     ...
     */
    function changeFee(uint96 newFee) external onlyAdminPair {
    ...
    function changeAssetRecipient(address payable newRecipient, address pair) external {
        PoolType poolType_ = poolType();
        require(poolType_ != PoolType.TRADE, "UpshotPool: invalid Trade pool operation");

        address pair_ = _ownerToPair[msg.sender];
        require(pair_ == pair, "UpshotPool: invalid msg.sender");

        if (ILSSVMPair(pair_).assetRecipient() != newRecipient) {
            // AssetRecipientChange event gets fired in pair
            ILSSVMPair(pair_).changeAssetRecipient(newRecipient);
        }
    }
    ...
    function withdrawAllETH() external {
        ...
        //console.log("  666 msg.sender: %s", msg.sender);
        //console.log("  666      pair_: %s", pair_);

        ILSSVMPair(pair_).withdrawAllETH();
...
import {PoolType} from "../../sudoswap/PoolType.sol";
import "./UpshotPool.sol";
import "../token/LPPosition.sol";
import "./CreateERC20PairParams.sol";

import {IERC20} from "../interface/IERC20.sol";

contract UpshotPoolFactory {

    address public _tokenLP;
    address public _sudoswapPairFactory;
    
    address immutable public _upshotPool;
    address immutable public _LPPosition;

    mapping (address => address) public nftCollectionToLPPosition;

    //event UpshotPoolCreated(address indexed nft, address upshotPool, address owner);
    event UpshotPoolCreated(address upshotPool);
    event LPPositionCollectionCreated(address nftCollection, address lpPosition);

    constructor(address sudoswapPairFactory_) {
        _sudoswapPairFactory = sudoswapPairFactory_;
        _upshotPool = address(new UpshotPool());
        _LPPosition = address(new LPPosition());
    }

    /**
     */
    function setFactoryPair(address sudoswapPairFactory_) external {
        _sudoswapPairFactory = sudoswapPairFactory_;
    }

    /** 
     * @dev Create an Upshot Pool, transfer the NFT(s).
     * 
     * @notice User required to approve this smartcontract, so
    ...
    ) external payable returns (address upshotPool) {
        
        address payable poolClone = payable(_createClone(_upshotPool));
        emit UpshotPoolCreated(poolClone);
        initLPPositionNFT(_nft, poolClone);

        _transferNFT(_nft, poolClone, msg.sender, tokenIdList);
        address pair = UpshotPool(poolClone).createPoolETH{value: msg.value}(
            _sudoswapPairFactory,
            _nft,
            _bondingCurve,
            _assetRecipient,
            _poolType,
    ...
    function createUpshotPoolERC20(CreateERC20PairParams calldata params) external payable returns (address upshotPool) {
        
        address payable clone = payable(_createClone(_upshotPool));
        emit UpshotPoolCreated(clone);

        initLPPositionNFT(params.nft, clone);

        IERC20(params.token).transferFrom(msg.sender, clone, params.initialTokenBalance);

        _transferNFT(params.nft, clone, msg.sender, params.tokenIdList);
        address pair = UpshotPool(clone).createPoolERC20(params, _sudoswapPairFactory);
        return pair;
    }

    /**
     * @dev helper function to avoid stack too deep errors.
     * @notice Initializes, sets LPPosition NFT collection for the underlying NFT collection.
     */
    function initLPPositionNFT(address _nft, address pool) internal {
        address lpPositionCollection = nftCollectionToLPPosition[_nft];
        if (lpPositionCollection == address(0)) {
            lpPositionCollection = _createClone(_LPPosition);
            nftCollectionToLPPosition[_nft] = lpPositionCollection;
            LPPosition lpc = LPPosition(lpPositionCollection);
            lpc.init(_nft, pool);
            emit LPPositionCollectionCreated(_nft, lpPositionCollection);
        } else {
            LPPosition lpc = LPPosition(lpPositionCollection);
            lpc.setApprovedUpshotPool(pool);
        }
    }
    ...
    /**
     * @dev Transfer the NFTs to the clone, so that it can create the factory which will send them to the pair.
     */
    function _transferNFT(address _nft, address clone, address owner, uint256[] calldata tokenIdList) private {
        for (uint256 i = 0; i < tokenIdList.length; ) {
            uint256 tokenId = tokenIdList[i];
            IERC721Metadata(_nft).transferFrom(owner, clone, tokenId);
            unchecked {
                ++i;
            }
        }
...
import { ILPPosition, IERC721Metadata } from "../interface/ILPPosition.sol";
import { LSSVMPair } from "../../sudoswap/LSSVMPair.sol";
import { LSSVMPairERC20 } from "../../sudoswap/LSSVMPairERC20.sol";
import { ILSSVMPairFactoryLike } from "../../sudoswap/ILSSVMPairFactoryLike.sol";
import { ERC20 } from "../../../lib/solmate/src/tokens/ERC20.sol";

contract LPPosition is ILPPosition {

    address public immutable creator;
    address public nft;
    uint16 private lastMintedId;
    uint16 public totalSupply;

    mapping(uint256 => address) internal _pairs;
    mapping(address => bool) public upshotPools;
    ...
    function init(address nft_, address upshotPool_) external onlyCreator {
        nft = nft_;
        upshotPools[upshotPool_] = true;
    }
    ...
    modifier onlyApprovedUpshotPools() {
        require(upshotPools[msg.sender], "UPSHOT_POOL_ONLY");
        _;
    }

    function setApprovedUpshotPool(address upshotPool_) external onlyCreator {
        upshotPools[upshotPool_] = true;
    }

    function mint(address to, address _pair) external onlyApprovedUpshotPools returns (uint256 id) {
        require(to != address(0), "INVALID_RECIPIENT");

        id = uint256(uint160(_pair));
        //todo before launch delete following line, i don't think we need it because permissioned and can save gas
        require(_ownerOf[id] == address(0), "ALREADY_MINTED");

        require(address(LSSVMPair(_pair).nft()) == nft, "INVALID_PAIR");
        ...
        //todo security, think about re-entrancy attacks here
        require(
            to.code.length == 0 ||
    ...
    function burn(address sender_, address pair_) external onlyApprovedUpshotPools {
        uint256 id = uint256(uint160(pair_));
        //todo can we delete these two require statements since this is permissioned?
        require(sender_ == _ownerOf[id], "CANT_BURN_NOT_OWNER");
     ...
     *                        \_/ |_|\___| \_/\_/                            *
     ************************************************************************/
    function pair(uint256 _tokenId) external view returns (address) {
        return _pairs[_tokenId];
    }

    function getLPPositionValueETHToken(uint _tokenId) external view returns (uint) {
        LSSVMPair p = LSSVMPair(_pairs[_tokenId]);
        ...
        if(pv == ILSSVMPairFactoryLike.PairVariant.ENUMERABLE_ETH
            ...
            LSSVMPairERC20 pair20 = LSSVMPairERC20(_pairs[_tokenId]);
    ...
    function getNumNFTsHeld(uint _tokenId) external view returns (uint){
        LSSVMPair p = LSSVMPair(_pairs[_tokenId]);
        return p.getAllHeldIds().length; //todo gas optimize by calling balanceOf() nft collection directly 
    }
    ...
    function name() public view returns (string memory) {
        return string.concat("Upshot LP - ", IERC721Metadata(nft).name());
    }

    function symbol() public view virtual returns (string memory) {
        return string.concat("ULP_", IERC721Metadata(nft).symbol());
    }
...
abstract contract AssetRecovery is AdminTwoStep {
    ...
    function withdrawERC721(IERC721 a, uint256[] calldata nftIds)
        ...
        onlyAdmin
    {
        // dont allow owner to steal LPs NFTs
        // transfer nfts
    }
    ...
    function withdrawERC20(ERC20 a, uint256 amount)
        ...
        onlyAdmin
    {
        // dont allow owner to steal LPs ERC20s
    }
