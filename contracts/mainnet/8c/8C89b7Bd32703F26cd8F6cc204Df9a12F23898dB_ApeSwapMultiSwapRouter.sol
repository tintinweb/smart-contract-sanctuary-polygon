...
/// @dev Credit to Uniswap Labs under GPL-2.0-or-later license:
/// https://github.com/Uniswap/v3-periphery
abstract contract CallbackValidation is ContractWhitelist {
    // === Algebra validation ===
    /// @notice The identifying key of the pool
    struct PoolKeyAlgebra {
        address token0;
        address token1;
    }
    ...
    function getPoolKey(address tokenA, address tokenB) internal pure returns (PoolKeyAlgebra memory) {
        if (tokenA > tokenB) (tokenA, tokenB) = (tokenB, tokenA);
        return PoolKeyAlgebra({token0: tokenA, token1: tokenB});
    }
    ...
    function verifyCallback(
        ...
        return verifyCallback(poolDeployer, getPoolKey(tokenA, tokenB));
    }

    /// @notice Returns the address of a valid Algebra Pool
    /// @param factory The contract address of the Algebra pool deployer
    /// @param poolKey The identifying key of the V3 pool
    /// @return pool The V3 pool contract address
    function verifyCallback(address factory, PoolKeyAlgebra memory poolKey) internal view returns (IAlgebraPool pool) {
        pool = IAlgebraPool(computeAddress(IAlgebraFactory(factory).poolDeployer(), poolKey));
    ...
    function computeAddress(address factory, PoolKeyAlgebra memory key) internal view returns (address pool) {
        require(key.token0 < key.token1);
        pool = address(
            uint256(
                keccak256(
                    abi.encodePacked(hex'ff', factory, keccak256(abi.encode(key.token0, key.token1)), hashes[factory])
                )
            )
        );
    }
...
import './BytesLibAlgebra.sol';

/// @title Functions for manipulating path data for multihop swaps
/// @dev Credit to Uniswap Labs under GPL-2.0-or-later license:
/// https://github.com/Uniswap/v3-periphery
library PathAlgebra {
    using BytesLibAlgebra for bytes;
