...
contract HighSimpsonClub is ERC721Enumerable, Ownable {
    using Strings for uint256;

    uint256 public constant SIMPSON_GIFT = 88;
    uint256 public constant SIMPSON_PRIVATE = 800;
    uint256 public constant SIMPSON_PUBLIC = 8000;
    uint256 public constant SIMPSON_MAX = SIMPSON_GIFT + SIMPSON_PUBLIC + SIMPSON_PRIVATE;
    uint256 public constant SIMPSON_PRICE = 0.08 ether;
    uint256 public constant SIMPSON_PER_MINT = 3;
    uint256 public constant SIMPSON_PUBLIC_PER_SALER = 5;
    ...
    constructor(
        ...
        ERC721("The High Simpson Club", "THS")  
    {
    ...
    //     require(totalSupply() < SIMPSON_MAX, "All Avariks are minted");
    //     require(publicAmountMinted + tokenQuantity <= SIMPSON_PUBLIC, "Minting would exceed the max pubic supply");
    //     require(tokenQuantity <= SIMPSON_PER_MINT, "You can mint up to 3 Avariks per transaction");
    //     require(salerListPurchases[msg.sender] + tokenQuantity <= SIMPSON_PUBLIC_PER_SALER, "You can not mint exceeds maximum NFT");
    //     require(SIMPSON_PRICE * tokenQuantity <= msg.value, "Insufficient ETH sent");
    ...
    //     require(totalSupply() < SIMPSON_MAX, "All Avariks are minted");
    //     require(privateAmountMinted + tokenQuantity <= SIMPSON_PRIVATE, "Minting would exceed the presale allocation");
    ...
    function mintByContract(address receiver) external returns (uint256 tokenId) {
        require(whitelistSigner == msg.sender, "Contract is not whitelisted to call");
        require(totalSupply() + 1 <= SIMPSON_PUBLIC, "MAX_MINT");
        publicAmountMinted++;
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= SIMPSON_MAX, "MAX_MINT");
        require(giftedAmount + receivers.length <= SIMPSON_GIFT, "GIFTS_EMPTY");
