...
interface IVenueRegistar {
    ...
    function setTokenCreator(address newTokenCreator) external;
...
interface IVenueSBT {
    ...
    error VenueSenderIsNotRegistarOwner();
...
contract VenueSBT is IVenueSBT, ERC721A, IERC721Receiver, Ownable, AccessControl {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    /// @inheritdoc IVenueSBT
    function setRoyaltyInfo(address royaltyReceiver, uint96 royaltyFeeNumerator) external virtual override {
        if (_msgSender() != Ownable(_venueRegistar).owner()) revert VenueSenderIsNotRegistarOwner();
    ...
    function mint(
        ...
        address paymentToken,
        bytes memory extraData
    ) external virtual payable override onlyRole(MINTER_ROLE) {
    ...
    /// @inheritdoc IVenueSBT
    function burn(uint256 tokenId) external virtual override onlyRole(MINTER_ROLE) {
    ...
    /// @inheritdoc IVenueSBT
    function withdrawFromPaymaster(uint256 amount) external virtual override onlyOwner {
        IVenueRegistar registar = IVenueRegistar(_venueRegistar);
    ...
    /// @inheritdoc IVenueSBT
    function withdrawToken(address token) external virtual override onlyOwner {
    ...
    function withdrawETH() external virtual override onlyOwner {
        ...
        (bool sent, ) = _msgSender().call{value: total}('');
