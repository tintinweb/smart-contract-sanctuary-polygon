...
contract StratFeeManager is Ownable, Pausable {
    ...
    uint256[4] public withdrawalFeePerTier = [300, 200, 100, 0];
...
import "../../interfaces/gmx/IGravityVault.sol";
import "../../interfaces/gmx/IGMXStrategy.sol";
import "../Common/StratFeeManager.sol";

contract StrategyMVLP is StratFeeManager {
    ...
    modifier whenNotCooling() {
        if (cooldown) {
            require(
                block.timestamp >= withdrawOpen() + extraCooldownDuration,
                "cooldown"
            );
        }
        _;
    ...
    function rewardsAvailable() public view returns (uint256) {
        uint256 rewardMVLP = IGMXTracker(mvlpRewardStorage).claimable(
            address(this)
        );
        uint256 rewardMVX = IGMXTracker(mvxRewardStorage).claimable(
            address(this)
        );
    ...
    function retireStrat() external {
        ...
        IGravityVault.StratCandidate memory candidate = IGravityVault(vault)
            .stratCandidate();
    ...
    function withdrawOpen() public view returns (uint256) {
        return
            IGLPManager(mvlpManager).lastAddedAt(address(this)) +
    ...
    function setExtraCooldownDuration(
        uint256 _extraCooldownDuration
    ) external onlyManager {
        extraCooldownDuration = _extraCooldownDuration;
    }

    function acceptTransfer() external {
        address prevStrat = IGravityVault(vault).strategy();
