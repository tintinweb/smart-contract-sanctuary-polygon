...
// File: FarmBooster/libraries/IterateMapping.sol
...
// File: FarmBooster/interfaces/IMasterChefV2.sol
...
// File: FarmBooster/interfaces/IBarPool.sol
...
// File: FarmBooster/bsc-library/contracts/IBEP20.sol
...
// File: FarmBooster/contracts/utils/Context.sol
...
// File: FarmBooster/contracts/access/Ownable.sol
...
// File: FarmBooster/FarmBooster.sol
...
contract FarmBooster is Ownable {
    ...
    /// @notice Barpool operation(deposit/withdraw) automatically call this function.
    ...
    function onBarPoolUpdate(
        ...
        emit UpdateBarPool(_user, _lockedAmount, _lockedDuration, _totalLockedAmount, _maxLockDuration);
    }
    ...
    function avgLockDuration() public view returns (uint256) {
        uint256 totalStakedAmount = IBEP20(BAR).balanceOf(BAR_POOL);

        uint256 totalLockedAmount = IBarPool(BAR_POOL).totalLockedAmount();

        uint256 pricePerFullShare = IBarPool(BAR_POOL).getPricePerFullShare();
        ...
        uint256 boostedRatio = ((IBarPool(BAR_POOL).totalShares() - flexibleShares) * BOOST_RATIO_PRECISION) /
            originalShares;
        if (boostedRatio <= BOOST_RATIO_PRECISION) return 0;

        uint256 boostWeight = IBarPool(BAR_POOL).BOOST_WEIGHT();
        uint256 maxLockDuration = IBarPool(BAR_POOL).MAX_LOCK_DURATION() * BOOST_RATIO_PRECISION;
    ...
    function _boostCalculate(
        ...
        (, , , , uint256 lockStartTime, uint256 lockEndTime, , , uint256 userLockedAmount) = IBarPool(BAR_POOL)
            .userInfo(_user);
