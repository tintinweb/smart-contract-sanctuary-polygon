...
contract MagpieTest is ReentrancyGuard, Ownable, Multicall, IMagpieTest {
    ...
    function swap(bytes32[][] calldata hops, bytes32[] calldata params) external payable nonReentrant override returns (uint256 amountOut) {
        Swap memory s = Swap({
            assetsLength: params[0].toUint8(),
            i: 0,
            fromIsNative: false,
            toIsNative: false,
            hopsLength: hops.length,
            amountIn: params[1].toUint256(),
            accAmountIn: 0,
            lastAmountOut: 0,
            fromAssetAddress: params[4].toAddress(),
            toAssetAddress: params[5].toAddress()
        });
        ...
        for (s.i; s.i < s.hopsLength; s.i++) {
            bytes memory hop = hops[s.i].concat(hops[s.i][0].toUint8(0));
            h.ammId = hop.toUint16(1);
            h.amountInIndex = hop.toUint16(3);
            h.toAssetIndex = hop.toUint16(5);
            h.amountIn = (h.fromAssetAddress == s.fromAssetAddress ? params[h.amountInIndex].toUint256() : s.lastAmountOut);
            h.toAssetAddress = params[h.toAssetIndex].toAddress();
            ...
            (bool success,) = address(amms[h.ammId].ammAddress).staticcall(amms[h.ammId].selector.getPayload(hop, params));
            ...
            s.lastAmountOut =  startingBalances[h.toAssetIndex] > newBalance ? (startingBalances[h.toAssetIndex] - newBalance) : newBalance;
        ...
        if (amountOut < params[2].toUint256()) {
            revert InsufficientOutputAmount();
        }

        s.toAssetAddress.transfer(payable(params[3].toAddress()), amountOut);
    }
}
...
library LibBytes4 {
    ...
    function getPayload(bytes4 selector, bytes memory hop, bytes32[] memory params) internal pure returns (bytes memory) {
...
interface IMagpieTest {
    ...
    function swap(bytes32[][] calldata hops, bytes32[] calldata params) external payable returns (uint256 amountOut);
