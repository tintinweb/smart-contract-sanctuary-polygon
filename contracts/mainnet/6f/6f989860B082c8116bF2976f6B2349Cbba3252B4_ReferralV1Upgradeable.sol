    ...
    UUPSUpgradeable
{
    ...
    function _addReferrer(
        ...
        require(
            userAccount.referrerAddress != _referee,
            "Referrer cannot be referee"
        );
        require(
            _referee != _referrer,
    ...
    function _removeReferee(
        ...
        if (_referrerAccount.selfAddress != address(0)) {
            ...
            for (uint256 i; i < referrerRefereeCount; i++) {
                ...
                if (i == (_referrerAccount.refereeAddresses.length - 1)) {
                    break;
                }
            }
        }
    ...
    function removeRefereeAdmin(address _refereeAddress) external onlyOwner {
        ...
        for (uint256 i; i < _levelRates.length; i++) {
            ...
            if (prevReferrerAccount.selfAddress == address(0)) {
                break;
            }

            _removeReferee(prevReferrerAccount, _refereeAddress);

            refereeAccount = prevReferrerAccount;
        }
    }
