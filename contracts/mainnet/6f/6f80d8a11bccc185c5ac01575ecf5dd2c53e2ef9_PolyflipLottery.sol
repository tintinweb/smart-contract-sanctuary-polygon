...
contract PolyflipLottery is ReentrancyGuard, Random {
    ...
    event LotteryCreated(uint indexed lotteryId, string title, uint endSale, uint indexed maxTicketsSellable, Price[] prices, uint[] prizes, uint startSale);
    event TicketSold(uint indexed lotteryId, uint indexed ticketId, address indexed buyer, address token, uint price);
    event LotteryDrawn(uint indexed lotteryId, Prize[] prizes, uint nTicketsSold);

    // maxTicketsSellable = 0 means no limit
    function createLottery(string memory _title, Price[] memory _prices, uint _maxTicketsSellable, uint[] memory _prizes, uint _endsale, uint _startSale) external onlyOwner {
        require(_endsale > block.timestamp, "Endsale need to be greater than current time");
        require(_startSale < _endsale, "Startsale need to be less than current time");
        ...
        require(overallPrize <= balanceAvailable(), "Not enough funds in contract");
        lockedBalance += overallPrize;

        emit LotteryCreated(lotteryId, _title, _endsale, _maxTicketsSellable, _prices, _prizes, _startSale);
    }

    function buy(uint lotteryId, address _buyer, uint _nTickets, address _token) payable external nonReentrant {
        ...
        require(_maxTicketsSellable == 0 || _maxTicketsSellable >= lottery.nTicketsSold + _nTickets, "Max tickets sellable out of range");
    ...
    function drawWinners(uint lotteryId) external nonReentrant {
        Lottery storage lottery = lotteries[lotteryId];
        require(lottery.endSale < block.timestamp, "Lottery is still open");
        require(lottery.drawn == false, "Lottery already drawn");

        lotteryMap[requestRandomness(keyHash, chainlinkFee)] = lotteryId;
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint randomness) internal override {
        ...
        for (uint i = 0; i < lottery.prizes.length; i++) {
            ...
            uint randomNumber = (expand(randomness, i) % lottery.nTicketsSold) + 1;
    ...
    function balanceAvailable() view public returns (uint){
        return address(this).balance - lockedBalance;
    }

    function getPrice(uint lotteryId, address token) view external returns (uint){
        return lotteries[lotteryId].prices[token];
    }

    function getPrizeAmount(uint lotteryId, uint index) view external returns (uint){
        return lotteries[lotteryId].prizes[index].amount;
    }

    function getPrizeWinner(uint lotteryId, uint index) view external returns (address){
        return lotteries[lotteryId].prizes[index].winner;
    }

    function getTicketIdWinner(uint lotteryId, uint index) view external returns (uint){
        return lotteries[lotteryId].prizes[index].ticketId;
    }

    function getBuyer(uint lotteryId, uint ticketId) view external returns (address){
        return lotteries[lotteryId].ticketsSold[ticketId];
    }

    function getNPrizes(uint lotteryId) view external returns (uint){
