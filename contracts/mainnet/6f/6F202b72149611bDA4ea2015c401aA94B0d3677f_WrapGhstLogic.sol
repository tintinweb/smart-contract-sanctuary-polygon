...
contract WrapGhstResolver is DSMath {
    ...
    function wrap(
        ...
        Ghst.safeApprove(address(vGhst), 0);
        Ghst.safeApprove(address(vGhst), type(uint256).max);
    ...
    function unwrap(
        ...
        if (setId > 0) {
            setUint(setId, sub(Ghst.balanceOf(address(this)), balBefore));
        }
    }
}
