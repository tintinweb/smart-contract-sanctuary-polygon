...
/// @dev This contract should be cloned and initialized with a leveragefactory contract
contract Leverage is AccessControl, Initializable {
  ...
  ///@notice Initializes the leverage contract
  ...
  ///@param _owner The owner of this leverage contract
  function initialize(
    IAddressProvider _a,
  ...
  ///@notice Swaps a flashLoaned asset for PAR and deposits the assets and the PAR into a vault.
  ///@dev This Integrates with AAVE V2 flashLoans
  ///@dev This funciton is called by the lendingPool during execution of the leverage function
  ///@param assets An address array with one element corresponding to the address of the leveraged asset
  ///@param amounts A uint array with one element corresponding to the amount of the leveraged asset
  ///@param premiums A uint array with one element corresponding to the flashLoan fees
  ///@param params Bytes sent by the leverage function that contains information on the aggregator swap
  function executeOperation(
    ...
    require(assets.length == 1, "SV101");
    (Operation operation, bytes memory operationParams) = abi.decode(params, (Operation, bytes));

    if (operation == Operation.LEVERAGE) {
      return leverageOperation(assets, amounts, premiums, operationParams);
    }

    if (operation == Operation.REBALANCE) {
      return rebalanceOperation(assets, amounts, premiums, operationParams);
    }
  }
  ...
  function leverageOperation(
    ...
    require(token.balanceOf(address(this)) >= flashloanRepayAmount, "SV201");
  ...
  function rebalanceOperation(
    ...
    IERC20 collateral = IERC20(assets[0]);
    uint256 collateralAmount = amounts[0];

    (uint256 vaultId, address usdc, uint256 parAmount, bytes memory dexTxData, Aggregator aggregator) = abi.decode(params, (uint256, address, uint256, bytes, Aggregator));

    if (aggregator == Aggregator.ONEINCH) {
      collateral.approve(oneInch, collateralAmount);
      oneInch.call(dexTxData);
    } else if (aggregator == Aggregator.PARASWAP) {
      collateral.approve(paraswapProxy, collateralAmount);
      paraswap.call(dexTxData);
    }

    uint256 usdcAmount = IERC20(usdc).balanceOf(address(this));

    IERC20(usdc).approve(address(a.core()), usdcAmount);

    a.core().depositAndBorrow(usdc, usdcAmount, parAmount);
    a.core().repay(vaultId, parAmount);
    uint256 flashloanRepayAmount = amounts[0] + premiums[0];
    a.core().withdraw(vaultId, flashloanRepayAmount);

    require(collateral.balanceOf(address(this)) >= flashloanRepayAmount, "SV201");

    // we need to pay back AAVE
    collateral.approve(address(lendingPool), flashloanRepayAmount);
  ...
  function rebalance(
    ...
    address usdc,
    address collateralType, // save some gas by just passing in collateral type instead of querying VaultsDataProvider for it
    uint256 collateralAmount, // amount of collateral to reduce in main vault and borrow from Aave first
    ...
    address onBehalfOf = address(this);
    bytes memory rebalanceParams = abi.encode(vaultId, usdc, parAmount, dexTxData, aggregator);
  ...
  ///@dev This helper function is used to limit the number of local variables in the executeOperation function 
  ///@param params The params passed from the leverage function for the aggregator call 
  ...
  function leverageSwap(bytes memory params, IERC20 token) internal {
    ...
    if (aggregator == Aggregator.ONEINCH) {
      ...
      IERC20(a.stablex()).approve(paraswapProxy, parToSell);
      paraswap.call(dexTxData);
    }
  }
