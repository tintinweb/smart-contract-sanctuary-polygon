...
// File contracts/DigitalElonClub.sol
...
contract DigitalElonClubNFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    IERC20 public DECToken;
    using Strings for uint256;
    

    // Private
    uint16[1000] public nftIds;
    ...
    uint256 public price = 40 ether; // 10**18
    uint256 public maxSupply = 1000;
    uint256 public maxMintAmountPerTx = 50;
    uint256 public DECTPrice = 10000 ether;
    address public ownerAddress = 0xfd5221E7037B3bA7f540e3e095b0436C5958D263;
    ...
    event WhiteListOpened(bool _state);

    constructor() ERC721("Digital Elon Club", "DEC") {
    ...
    mintPriceCompliance(_mintAmount)
{
    ...
    address _sender = msg.sender;
        ...
        DECToken.transferFrom(ownerAddress, _sender, DECTPrice * _mintAmount);
    ...
    function claim() external {
        ...
        // Update the lifetime rewards
         lifetimeRewards[_sender] += _amount;

        // Transfer the claimable amount to the sender
        payable(msg.sender).transfer(_amount);

        // Reset the user's rewardDebt to the current accumulated amount
    ...
    function mintWhiteList(uint8 _mintAmount)
        ...
        mintPriceCompliance(_mintAmount)
    {
        require(isWhiteListActive, "White List is not ready");
        ...
        require(
            ...
            // Ensure that no more than 300 NFTs are minted during the whitelist period
         require(whiteListMinted + _mintAmount <= 300, "Whitelist mint limit exceeded!");

        whiteList[msg.sender] -= _mintAmount;

            // Update the number of NFTs minted during the whitelist period
        ...
        DECToken.transferFrom(ownerAddress, msg.sender, DECTPrice * _mintAmount);
    ...
    function setDECTAD(uint256 _price) public onlyOwner {
        DECTPrice = _price;
    }

    function setsDECTAddress(address addr) public onlyOwner {
        DECToken = IERC20(addr);
    }  
    ...
    function withdraw(bool asTest) public onlyOwner nonReentrant {
      ...
      if (asTest) {
       ...
       // Calculate the 15% rewards share
       uint256 rewardsShare = (contractBalance * 15) / 100;

        // Subtract the rewards share from the contract balance
        contractBalance -= rewardsShare;
