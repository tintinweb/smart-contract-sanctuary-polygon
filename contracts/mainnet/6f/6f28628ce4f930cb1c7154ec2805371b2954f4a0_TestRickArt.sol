...
contract TestRickArt is ERC721A, ReentrancyGuard, Ownable, DefaultOperatorFilterer {
    ...
    uint public maxPhunkyPurchase = 10;
    uint public maxWhitelistPurchase = 10;

    // Sale Status
    bool public saleIsActive = false;
    bool public phunkySaleIsActive = true;

    bool public whitelistSaleIsActive = false;
     
    struct PhunkyListaddr {
        uint256 presalemints;
        bool exists;
    }

    struct WhitelistListaddr {
        uint256 presalemints;
        bool exists;
    }
    ...
    mapping(address => PhunkyListaddr) private phunkylist;
    mapping(address => WhitelistListaddr) private whitelist;

    mapping(address => uint) public addressesThatMinted;
    
    // Metadata
    string _baseTokenURI = "https://gateway.pinata.cloud/ipfs/QmP61gXEXoepA7ygooQ3xFB1aa3TJdtBXfGrZ6p3gr4oiM/";
    bool public locked;

    // Events
    event SaleActivation(bool isActive);
    event PhunkySaleActivation(bool isActive);
    event WhitelistSaleActivation(bool isActive);
  
    constructor() ERC721A("Rick's Art", "RICK") {
      phunkylist[0x4e4CC29ab82cf8aa4EcD3578A26409E57793de4b].exists = true;
    }

    function onPhunkyList(address walletaddr)
    public
    view
    returns (bool)
    {
        if (phunkylist[walletaddr].exists){
            return true;
        }
        else{
            return false;
    ...
    function setMaxPhunkyPurchase(uint256 _maxPurchase) external onlyOwner {
        maxPhunkyPurchase = _maxPurchase;
    }
    
    function setMaxWhitelistPurchase(uint256 _maxPurchase) external onlyOwner {
        maxWhitelistPurchase = _maxPurchase;
    }

    function addToPhunkyList (address[] memory newWalletaddr) public onlyOwner{
        for (uint256 i = 0; i<newWalletaddr.length;i++){
            phunkylist[newWalletaddr[i]].exists = true;
        }        
    }

    function phunkyPresaleMint(uint256 numberOfTokens) public {
        require(phunkySaleIsActive, "PHUNKY_SALE_INACTIVE");
        require(phunkylist[msg.sender].exists == true, "This Wallet is not able mint for presale"); 
        require(phunkylist[msg.sender].presalemints + numberOfTokens <= maxPhunkyPurchase,"This Wallet has already minted its reserved NFTs");
        require(
            totalSupply() + numberOfTokens <= maxSupply,
            "SOLD_OUT"
        );
        addressesThatMinted[msg.sender] += numberOfTokens;
        phunkylist[msg.sender].presalemints += numberOfTokens;
    ...
    function whitelistMint(uint256 numberOfTokens) public payable {
        ...
        require(whitelist[msg.sender].presalemints + numberOfTokens <= maxWhitelistPurchase,"This Wallet has already minted its reserved NFTs");
        require(
    ...
    function togglePhunkySaleStatus() external onlyOwner {
        phunkySaleIsActive = !phunkySaleIsActive;
        emit PhunkySaleActivation(phunkySaleIsActive);
    }
