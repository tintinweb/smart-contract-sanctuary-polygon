...
contract PLG_RouterV2 is permission {
    ...
    address public plg_refreward = 0x9B3ece93b0313Cd8aBe4BAb0B0E8385b45Ab59F5;
    ...
    function depositWithPLG(uint256 amount,address referral) public noReentrant() returns (bool) {
        ...
        if(invest[msg.sender].balance>0){
    ...
    function tokenAddressSetting(address[] memory input) public forRole("owner") returns (bool) {
        plg_token = input[0];
        usdc_token = input[1];
        plg_pair = input[2];
        matic_pair = input[3];
        quick_router = input[4];
        updateImplement();
        return true;
    }
