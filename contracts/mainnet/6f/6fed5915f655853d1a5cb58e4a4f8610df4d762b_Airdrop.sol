...
contract Airdrop is Ownable {
    ...
    function dropTokens(address[] memory _recipients)
        public
        onlyOwner
        returns (bool)
    {
        for (uint256 i = 0; i < _recipients.length; i++) {
            ...
            require(IERC20(tokenAddr).transfer(_recipients[i], 50 ether));
        }
    ...
    function dropEther(address[] memory _recipients)
        public
        payable
        onlyOwner
        returns (bool)
    {
        for (uint256 i = 0; i < _recipients.length; i++) {
            ...
            payable(_recipients[i]).transfer(0.1 ether);
        }
