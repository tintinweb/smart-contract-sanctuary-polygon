...
import { IMIMOProxy } from "../proxy/interfaces/IMIMOProxy.sol";
import { IMIMOProxyFactory } from "../proxy/interfaces/IMIMOProxyFactory.sol";
import { IAAVELeverage } from "./interfaces/IAAVELeverage.sol";
import { IPool } from "@aave/core-v3/contracts/interfaces/IPool.sol";
import { MIMOSwap, IAddressProvider, IDexAddressProvider } from "./MIMOSwap.sol";
import { MIMOFlashLoan } from "./MIMOFlashLoan.sol";
import { MIMOPausable } from "./MIMOPausable.sol";
import { SafeERC20 } from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

contract AAVELeverage is MIMOPausable, MIMOFlashLoan, MIMOSwap, IAAVELeverage {
  ...
  /**
    @notice Leverages long or short an asset using a flashloan
    @notice Must be called through a MIMOProxy
    @dev Uses an AAVE V3 flashLoan that will call executeOperation
    @param _calldata Bytes containing struct FlashloanData, struct SwapData and struct ShortData
   */
  function executeAction(bytes calldata _calldata) external override whenNotPaused {
    (FlashLoanData memory flData, SwapData memory swapData, ShortData memory shortData) = abi.decode(
      _calldata,
      (FlashLoanData, SwapData, ShortData)
    );

    shortData.supplyAsset.safeTransferFrom(msg.sender, address(this), shortData.supplyAmount);
    shortData.supplyAsset.safeIncreaseAllowance(address(lendingPool), shortData.supplyAmount);
    lendingPool.supply(address(shortData.supplyAsset), shortData.supplyAmount, address(this), 0);

    bytes memory params = abi.encode(msg.sender, swapData, shortData.toAsset, shortData.interestRateMode);
  ...
  function executeOperation(
    ...
    (address owner, SwapData memory swapData, IERC20 toAsset, uint256 interestRateMode) = abi.decode(
      params,
      (address, SwapData, IERC20, uint256)
    );
    ...
    asset.safeTransfer(address(mimoProxy), amount);
    uint256 flashloanRepayAmount = amount + premiums[0];

    IMIMOProxy(mimoProxy).execute(
      address(this),
      abi.encodeWithSelector(
        IAAVELeverage.aaveLeverageOperation.selector,
        toAsset,
        asset,
        interestRateMode,
        amount,
  ...
  /**
    @notice Executes a leveraged operation on aave
    @dev Integrates with AAVE V3 flashLoans
    @param toAsset Address of the asset to short
    @param fromAsset Address of the asset to long (to use as collateral)
    @param interestRateMode Interest rate mode to be used for the borrow
    @param swapAmount Amount of fromAsset to be swapped for toAsset
    @param flashLoanRepayAmount Amount of fromAsset to be repaid to the flashLoan
    @param swapData Struct containing the swap data
   */
  function aaveLeverageOperation(
    IERC20 toAsset,
    IERC20 fromAsset,
    uint256 interestRateMode,
    uint256 swapAmount,
    ...
    uint256 toAssetBalanceBefore = toAsset.balanceOf(address(this));
    _aggregatorSwap(fromAsset, swapAmount, swapData);
    uint256 swapResult = toAsset.balanceOf(address(this)) - toAssetBalanceBefore;
    toAsset.safeIncreaseAllowance(address(lendingPool), swapResult);

    lendingPool.supply(address(toAsset), swapResult, address(this), 0);
    lendingPool.borrow(address(fromAsset), flashLoanRepayAmount, interestRateMode, 0, address(this));

    fromAsset.safeTransfer(msg.sender, flashLoanRepayAmount);
  }
}
...
import { IERC20 } from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import { IMIMOProxyAction } from "./IMIMOProxyAction.sol";
import { IMIMOProxyFactory } from "../../proxy/interfaces/IMIMOProxyFactory.sol";
import { IMIMOSwap } from "./IMIMOSwap.sol";

interface IAAVELeverage is IMIMOSwap, IMIMOProxyAction {
  /**
    @param supplyAsset The asset to supply to AAVE V3 in order to borrow the toAsset for FlashLoan repayment
    @param toAsset The asset to long e.g the asset to swap to and supply to AAVE V3
    @param interestRateMode The interest rate mode to borrow the toAsset at
    @param supplyAmount The amount of supplyAsset to supply to AAVE V3
   */
  struct ShortData {
    IERC20 supplyAsset;
    IERC20 toAsset;
    uint96 interestRateMode;
...
library Errors {
  ...
  error PAUSED();
  error SUPPLY_AMOUNT_CANNOT_BE_ZERO();
}
