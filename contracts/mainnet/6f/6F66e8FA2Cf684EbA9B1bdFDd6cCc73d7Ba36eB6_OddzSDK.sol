...
import "./../Integrations/Swap/OddzSwap.sol";
...
contract OddzSDK is IOddzSDK, BaseRelayRecipient, AccessControl {
    ...
    mapping(bytes32 => mapping(bytes32 => IOddzPrediction)) public predictionMap;
    mapping(IOddzPrediction => bool) public activePredictionMap;

    OddzSwap public oddzSwap;
    ...
    constructor(
        IOddzPrediction[] memory _oddzPrediction,
        bytes32[] memory _underlying,
        bytes32[] memory _strike,
        IERC20 _oddzToken
    ) {
        require(_oddzPrediction.length == _underlying.length, "Invalid array arguments passed");
        require(_strike.length == _underlying.length, "Invalid array arguments passed");
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        for (uint256 i = 0; i < _oddzPrediction.length; i++) {
            predictionMap[_underlying[i]][_strike[i]] = _oddzPrediction[i];
            activePredictionMap[_oddzPrediction[i]] = true;
        }
    ...
    function addPredictionMarket(
        ...
        for (uint256 i = 0; i < _oddzPrediction.length; i++) {
            predictionMap[_underlying[i]][_strike[i]] = _oddzPrediction[i];
            require(!activePredictionMap[_oddzPrediction[i]], "Prediction Market is already active");
            activePredictionMap[_oddzPrediction[i]] = true;
        }
    }

    function updatePredictionMarket(
        IOddzPrediction _oddzPrediction,
        bytes32 _underlying,
        bytes32 _strike
    ) external onlyAdmin(msg.sender) {
        require(address(predictionMap[_underlying][_strike]) != address(0), "Prediction market doesn't exist");
        predictionMap[_underlying][_strike] = _oddzPrediction;
    }

    function removePredictionMarket(IOddzPrediction _oddzPrediction) external onlyAdmin(msg.sender) {
        require(activePredictionMap[_oddzPrediction], "Prediction market is not in active state");
        activePredictionMap[_oddzPrediction] = false;
    }
    ...
    /**
     * @notice set the oddz swap
     * @param _oddzSwap oddz swap address
     */
    function setOddzSwap(OddzSwap _oddzSwap) external onlyAdmin(msg.sender) {
        require(address(_oddzSwap).isContract(), "Swapper is not a contract");
        oddzSwap = _oddzSwap;
    }
    ...
    function _populateProviderInfo(address _provider, uint256 _predAmt) internal {
        ...
        totalPremiumCollected[month] += _predAmt;
    }
    ...
    function predict(
        ...
        IOddzPrediction oddzPrediction = predictionMap[_predStruct.underlying][_predStruct.strike];

        require(address(oddzPrediction) != address(0), "Prediction Market for the asset is not present");
        require(activePredictionMap[oddzPrediction], "Prediction Market for the asset is not active");
        ...
        if (_predStruct.directionUp) oddzPrediction.predictUp{ value: msg.value }(_predStruct.roundId, userAddress_);
        else oddzPrediction.predictDown{ value: msg.value }(_predStruct.roundId, userAddress_);
    ...
    function predictWithGasless(PredictionStruct memory _predStruct, address _provider) external payable override {
        IOddzPrediction oddzPrediction = predictionMap[_predStruct.underlying][_predStruct.strike];
        require(address(oddzPrediction) != address(0), "Prediction Market for the asset is not active");
        require(activePredictionMap[oddzPrediction], "Prediction Market for the asset is not active");
        require(msg.value > minimumGaslessBetAmount, "Bet amount is not eligible for gasless");

        if (_predStruct.directionUp) oddzPrediction.predictUp{ value: msg.value }(_predStruct.roundId, msgSender());
        else oddzPrediction.predictDown{ value: msg.value }(_predStruct.roundId, msgSender());

        _populateProviderInfo(_provider, msg.value);
    }

    function _swapAndPredict(
        OddzSwap.SwapStruct memory _swapStruct,
        PredictionStruct memory _predStruct,
        uint256 _slippage,
        bool _isGasless,
        address _provider
        ...
        address[] memory path = oddzSwap.getPath(_swapStruct.fromAsset, _swapStruct.toAsset);
        IERC20(path[0]).safeTransferFrom(msgSender(), address(oddzSwap), _swapStruct.amountIn);

        uint256[] memory amounts = oddzSwap.swapExactTokensForETH(_swapStruct, address(this), _slippage);
        ...
        if (_predStruct.directionUp)
            oddzPrediction.predictUp{ value: amounts[amounts.length - 1] }(_predStruct.roundId, msgSender());
        else oddzPrediction.predictDown{ value: amounts[amounts.length - 1] }(_predStruct.roundId, msgSender());
    ...
    function allocateOddzReward(uint256 _amount) external override {
    ...
    function distributeReward(address[] memory _providers, uint256 _month) external override onlyAdmin(msg.sender) {
        ...
        for (uint256 i = 0; i < _providers.length; i++) {
            ...
            uint256 amount = (totalOddzAllocated[_month] * premiumCollected[_providers[i]][_month]) /
                totalPremiumCollected[_month];
    ...
    function setMinimumGaslessBetAmount(uint256 _amount) external onlyAdmin(msg.sender) {
        require(_amount >= 0.1 ether && _amount < 10 ether, "invalid minimum gasless premium");
...
contract OddzSwap is AccessControl {
    using SafeERC20 for IERC20;
