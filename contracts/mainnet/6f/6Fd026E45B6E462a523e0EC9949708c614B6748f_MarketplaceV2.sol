...
contract MarketplaceV2 is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    uint256 public constant USDC_TOKEN = 1;
    uint256 public constant USDT_TOKEN = 2;
    uint256 public constant DAI_TOKEN = 3;

    uint256 public usdFee = 200;
    uint256 public operationsFee = 100;
    ...
    mapping(address => uint256) public merchantAddressToId;
    ...
    constructor(
        address rxcCharactersAddress,
        address _usdcToken,
        address _usdtToken,
        address _daiToken,
        address _operationsAddress
    ) {
        allowedAssetList[rxcCharactersAddress] = true;
        usdcToken = IERC20(_usdcToken);
        usdtToken = IERC20(_usdtToken);
        daiToken = IERC20(_daiToken);
        operationsAddress = _operationsAddress;

        merchantIdToAddress[0] = msg.sender;
        merchantAddressToId[msg.sender] = 0;

        merchantIdToAddress[1] = 0x2729e66e1db2a75bD0d953DEa6D9e3081922dF4e;
        merchantAddressToId[0x2729e66e1db2a75bD0d953DEa6D9e3081922dF4e] = 1;

        merchantIdToAddress[2] = 0x8DED50f762A10471E42bbaf7c38C34774279e64b;
        merchantAddressToId[0x8DED50f762A10471E42bbaf7c38C34774279e64b] = 2;
    }

    function updateMerchantAddress(
        uint256 merchantId,
        address merchantAddress
    ) public onlyOwner {
        merchantIdToAddress[merchantId] = merchantAddress;
        merchantAddressToId[merchantAddress] = merchantId;
    }
    ...
    function setAllowAsset(address _assetAddress, bool allow) public onlyOwner {
    ...
    function createMarketItem(
        ...
        require(
            paymentMethod == USDC_TOKEN ||
                paymentMethod == USDT_TOKEN ||
                paymentMethod == DAI_TOKEN,
    ...
    function createMarketSale(
        ...
        require(!idToMarketItem[itemId].sold, "NFT already sold");
        require(
            merchantIdToAddress[merchantId] != address(0),
            "Invalid merchantId"
        );
        ...
        uint256 toMerchant = price.mul(usdFee).div(10000);
        uint256 toOperations = price.mul(operationsFee).div(10000);
        uint256 netAmount = price - (toMerchant + toOperations);

        if (paymentMethod == USDC_TOKEN) {
            usdcToken.safeTransferFrom(msg.sender, address(this), price);
            usdcToken.safeTransfer(idToMarketItem[itemId].seller, netAmount);
            usdcToken.safeTransfer(merchantIdToAddress[merchantId], toMerchant);
            usdcToken.safeTransfer(operationsAddress, toOperations);
        }

        if (paymentMethod == USDT_TOKEN) {
            usdtToken.safeTransferFrom(msg.sender, address(this), price);
            usdtToken.safeTransfer(idToMarketItem[itemId].seller, netAmount);
            usdtToken.safeTransfer(merchantIdToAddress[merchantId], toMerchant);
            usdtToken.safeTransfer(operationsAddress, toOperations);
        }

        if (paymentMethod == DAI_TOKEN) {
            daiToken.safeTransferFrom(msg.sender, address(this), price);
            daiToken.safeTransfer(idToMarketItem[itemId].seller, netAmount);
            daiToken.safeTransfer(merchantIdToAddress[merchantId], toMerchant);
            daiToken.safeTransfer(operationsAddress, toOperations);
        }
