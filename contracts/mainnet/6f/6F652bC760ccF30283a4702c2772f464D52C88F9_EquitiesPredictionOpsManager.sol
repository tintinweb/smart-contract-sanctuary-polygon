...
import ".././Impl/BhavishEquitiesPrediction.sol";

import "@openzeppelin/contracts/access/Ownable.sol";

contract EquitiesPredictionOpsManager is Ownable {
    BhavishEquitiesPrediction[] public predictionMarkets;

    constructor(BhavishEquitiesPrediction[] memory _bhavishPrediction) {
        for (uint256 i = 0; i < _bhavishPrediction.length; i++) {
            setPredicitionMarket(_bhavishPrediction[i]);
        }
    }

    function setPredicitionMarket(BhavishEquitiesPrediction _bhavishPredicition) public onlyOwner {
    ...
    function execute() public {
        for (uint256 i = 0; i < predictionMarkets.length; i++) {
            if (predictionMarkets[i].shouldExecuteRound()) {
    ...
    function canPerformTask() external view returns (bool canPerform) {
        for (uint256 i = 0; i < predictionMarkets.length; i++) {
            ...
            canPerform =
                ...
                round.roundState == IBhavishPrediction.RoundState.STARTED;
            if (canPerform) canPerform = predictionMarkets[i].shouldExecuteRound();
            if (canPerform) break;
        }
    }
}
 ...
 */
contract BhavishEquitiesPrediction is IBhavishPrediction, AccessControl, Pausable, ReentrancyGuard {
    ...
    /**
     * @notice set the bhavish market time
     * @param _marketTime Market Active time
     */
    function setMarketTime(IMarketTime _marketTime) external onlyAdmin(msg.sender) {
        require(address(_marketTime).isContract(), "Swapper is not a contract");
        ...
        onlyOperator(msg.sender)
    {
        ...
        (uint256 startTimestamp, uint256 endTimestamp) = marketTime.getMarketActiveTime(_roundzeroStartTimestamp);
        require(
            startTimestamp != 0 && startTimestamp == _roundzeroStartTimestamp,
            "Round zero start time stamp not equal"
        );

        if (_createAllRounds(startTimestamp, endTimestamp) > 0) {
    ...
    function shouldExecuteRound() external view returns (bool) {
        ...
        // if Current round is cancelled / Expired / Created return false
        bool isExec = curRound.roundState == RoundState.STARTED ||
            (curRound.roundState == RoundState.CREATED && curRound.roundId > 0);

        if (isExec && curRound.roundEndTimestamp <= block.timestamp) return true;

        Round memory roundPlusOne = bhavishPredictionStorage.getPredictionRound(currentRoundId + 1);
        (uint256 startTimestamp, ) = marketTime.getMarketActiveTime(block.timestamp);

        // Create and start the round next day before time for prediction ready
        if (
            roundPlusOne.roundStartTimestamp == 0 &&
            curRound.roundEndTimestamp < startTimestamp &&
            startTimestamp <= (block.timestamp + roundTime * 2) &&
            (curRound.roundState == RoundState.CANCELLED || curRound.roundState == RoundState.ENDED)
        ) return true;
        return false;
    }
    ...
    function executeRound() external override whenNotPaused {
        ...
        // Close the round if valid
        uint256 price = closeCurrentRound();

        (uint256 startTimestamp, uint256 endTimestamp) = marketTime.getMarketActiveTime(block.timestamp);
        Round memory roundPlusOne = bhavishPredictionStorage.getPredictionRound(currentRoundId + 1);

        // Check if roundPlusOne has value after create all round
        roundPlusOne = bhavishPredictionStorage.getPredictionRound(currentRoundId + 1);
        if (roundPlusOne.roundStartTimestamp > 0 && block.timestamp >= roundPlusOne.roundStartTimestamp) {
            _startRound(currentRoundId + 1, price);

            // round + 3 should always exist since it's created in advance otherwise we should't create new rounds
            Round memory roundPlusTwo = bhavishPredictionStorage.getPredictionRound(currentRoundId + 2);
            if (roundPlusTwo.roundEndTimestamp > 0 && (endTimestamp - roundPlusTwo.roundEndTimestamp) / roundTime > 0)
                _createRound(currentRoundId + 3, roundPlusTwo.roundEndTimestamp);

            // Point currentRoundId to next round
            currentRoundId = currentRoundId + 1;
        } else if (roundPlusOne.roundStartTimestamp == 0 && startTimestamp >= (block.timestamp + roundTime)) {
            _createAllRounds(startTimestamp, endTimestamp);
        }
    }

    function closeCurrentRound() private returns (uint256 closingPrice) {
        Round memory curRound = bhavishPredictionStorage.getPredictionRound(currentRoundId);

        // currentRoundId refers to current round n
        // fetch price to end current round and start new round
        (closingPrice, ) = bhavishPriceManager.getPrice(
            assetPair.underlying,
            assetPair.strike,
            curRound.roundEndTimestamp
        );

        if (curRound.roundState == RoundState.ENDED || curRound.roundState == RoundState.CANCELLED) return closingPrice;

        // End and Disperse current round
        // skip for non current rounds
        if (curRound.roundState != RoundState.CANCELLED && closingPrice != 0) {
            _endRound(currentRoundId, closingPrice);

            _calculateRewards(currentRoundId);
        } else if (curRound.roundState != RoundState.CANCELLED && closingPrice == 0) {
            _cancelRound(currentRoundId);
        }
    }

    function _createAllRounds(uint256 _start, uint256 _end) private returns (uint256 maxRounds) {
        // if end - start is 3 hours 30 minutes and if roundTime is 1 hour then only maximum 3 rounds will be created
        maxRounds = (_end - _start) / roundTime;
        if (maxRounds > 3) maxRounds = 3;
        for (uint256 pRound = 1; pRound <= maxRounds; pRound++) {
            _createRound(currentRoundId + pRound, _start + (pRound - 1) * roundTime);
        }
    }
