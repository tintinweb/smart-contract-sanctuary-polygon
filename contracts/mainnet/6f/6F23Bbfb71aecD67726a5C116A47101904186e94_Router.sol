...
contract Router is Ownable {
    ...
    */
    ...
    // @param _outputToken output token address
    // @param _swapCallData swap callData (intended for one specific protocol)
    // @notice allowance must be sufficient to cover fee and swap input amount
    */
    function swap(uint256 _adapterIndex, address _inputToken, uint256 _swapInputAmount, address _outputToken, bytes memory _swapCallData) public payable {
        ...
        if (_inputToken != NATIVE) {
            IERC20(_inputToken).safeTransferFrom(msg.sender, getAgent(), computeSwapFee(_swapInputAmount));
            IERC20(_inputToken).safeTransferFrom(msg.sender, address(adapters[_adapterIndex].deployedContract), _swapInputAmount);
            AdapterBase(adapter).callAction(msg.sender, _inputToken, _swapInputAmount, _outputToken, _swapCallData);
        } else {
            AdapterBase(adapter).callAction{value: _swapInputAmount}(msg.sender, _inputToken, _swapInputAmount, _outputToken, _swapCallData);
        }
    }
    ...
    function ftatSwapWithPermit(
        ...
        require(adapters[_adapterIndex].deployedContract != address(0), "ADAPTER_NOT_DEPLOYED");
        require(adapters[_adapterIndex].isActivated, "ADAPTER_NOT_ACTIVATED");
        require(_permitAmount >= _swapInputAmount, "PERMIT_AMOUNT_TOO_LOW");
    ...
    /**
    // @dev utility to compute swap fee for a given amount
    ...
    */
    ...
    /**
     * @dev Transfer agentship of the Router to _newAgent
    ...
    /**
     * @dev Set FTAT input token and associated fee
