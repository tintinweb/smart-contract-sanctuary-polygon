...
contract DoubleOrNothing is ERC721TokenReceiver, Ownable {
    ...
    function ApeNoLongerForSale(uint256 apeIndex) public reentrancyGuard {
      ...
      function AllApeNoLongerForSale() public reentrancyGuard {
        UserToken storage _userToken = userTokens[msg.sender];

        for(uint256 i =0 ; i <= _userToken.tokenIds.length ; i++){
            if(_userToken.tokenIds[i]>0){
                ApeNoLongerForSale(_userToken.tokenIds[i]);
            }
        }
    }
