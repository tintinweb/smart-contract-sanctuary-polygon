...
contract BrewlabsFactory is IBrewlabsFactory {
     ...
     * to see if the address is blacklisted.
     */

    function isAllowedToSwap(address _addr) external view returns(bool) {
        uint32 size;
        assembly {
    ...
    */
    ...
    function addToBlackList(address _addr) external onlyOwner {
        require(_addr != address(0), "Brewlabs: invalid address");
...
contract BrewlabsPair is IBrewlabsPair, BrewlabsERC20 {
    ...
    uint256 public constant MAX_FEE_PERCENT = 200000; // = 20%
     ...
     * Can only be called by the factory's feeManager
     */
    function setFeePercent(uint16 newfeePercent) external lock {
        require(msg.sender == IBrewlabsFactory(factory).feeManager(), "Brewlabs: only factory's feeManager");
