...
contract MarketplaceListing is Ownable,  MarketplaceListingHelper {
    ...
    mapping(address =>mapping(uint256 => uint256)) private tokenIdToListingId;
    mapping(uint256 => Listing) private listingIdToListing;

    // OFFER MAPPINGS
    mapping(uint256 => Offer) private offerIdToOffer;
    mapping(address =>mapping(uint256 => EnumerableSet.UintSet)) private tokenIdToActiveOffers;
    mapping(address => EnumerableSet.UintSet) private addrToActiveOffers;
    
    // AUTHORIZED TOKENS FOR MARKETPLACE
    mapping(address => bool) private activeMarketplaceTokens;

    // Addresses for USDC & Royalties
    address internal immutable usdcAddress;
    address internal immutable royaltyAddress;

    constructor(address _nftAddress, address _usdcAddress, uint256 _royalty, address _royaltyAddress) {
        activeMarketplaceTokens[_nftAddress] = true;
     ...
     */
    function createOffer(uint256 tokenId, uint256 price, uint256 duration, address _tokenAddress) public { 
        ...
        require(activeMarketplaceTokens[_tokenAddress] == true, "Token address is not authorized for marketplace.");
    ...
    function cancelOffer(uint256 offerId) public { 
        require(offerIdToOffer[offerId].buyer == msg.sender, "You did not create this offer!");
        ...
        emit OfferCancelled(
            offerId, 
            offerIdToOffer[offerId].offerAmount, 
            offerIdToOffer[offerId].tokenId,
            offerIdToOffer[offerId].buyer,
            offerIdToOffer[offerId].seller, 
            offerIdToOffer[offerId].tokenAddress
            );
    }
    ...
    function acceptOffer(uint256 offerId) public { 
        ...
        require(IERC721(offerIdToOffer[offerId].tokenAddress).ownerOf(offer.tokenId) == msg.sender, "You must own the token");
        require(IERC721(offerIdToOffer[offerId].tokenAddress).isApprovedForAll(msg.sender, address(this)),"Must approve this contract for transfers.");
        ...
        // Transfer the NFT
        IERC721(offerIdToOffer[offerId].tokenAddress).safeTransferFrom(
            msg.sender,
            offer.buyer,
            offer.tokenId
        );

        emit OfferAccepted(offerId, offer.offerAmount, offer.tokenId, offer.buyer, offer.seller, offer.tokenAddress);
    }
    ...
    function cancelAllUserOffers(address _addr) public { 
        ...
        for(uint  i = 0; i < userOfferIds.length; i++){
            ...
            emit OfferCancelled( offerIdToOffer[userOfferIds[i]].offerId,  offerIdToOffer[userOfferIds[i]].offerAmount,  offerIdToOffer[userOfferIds[i]].tokenId,  offerIdToOffer[userOfferIds[i]].buyer, offerIdToOffer[userOfferIds[i]].seller, offerIdToOffer[userOfferIds[i]].tokenAddress);
        }
    ...
    */
    function _removeOfferStorage(uint256 offerId) internal {     
        ...
        tokenIdToActiveOffers[offerIdToOffer[offerId].tokenAddress][offerIdToOffer[offerId].tokenId].remove(offerId);
    ...
    */
    function _addOfferStorage(Offer memory offer) internal {
        ...
        tokenIdToActiveOffers[offer.tokenAddress][offer.tokenId].add(offer.offerId);
    ...
    function createListing(uint256 tokenId, uint256 price, address _tokenAddress) public { 
        ...
        require(IERC721(_tokenAddress).isApprovedForAll(msg.sender, address(this)),"Must approve this contract for transfers.");
       

        if(listingIdToListing[tokenIdToListingId[_tokenAddress][tokenId]].state == State.OPEN){
            //Existing listing is previous owner
             // Make the removal
            listingIdToListing[tokenIdToListingId[_tokenAddress][tokenId]].state = State.CANCELLED;
            _removeListingStorage(listingIdToListing[tokenIdToListingId[_tokenAddress][tokenId]].listingId);

            // Emit the event
            emit ListingCancelled(listingIdToListing[tokenIdToListingId[_tokenAddress][tokenId]].listingId, listingIdToListing[tokenIdToListingId[_tokenAddress][tokenId]].tokenId, listingIdToListing[tokenIdToListingId[_tokenAddress][tokenId]].tokenAddress, listingIdToListing[tokenIdToListingId[_tokenAddress][tokenId]].seller);
        }

        require(
            listingIdToListing[tokenIdToListingId[_tokenAddress][tokenId]].state != State.OPEN,
    ...
    function buyFromListing(uint256 listingId) public {
        ...
        require(IERC721(listingIdToListing[listingId].tokenAddress).ownerOf(listing.tokenId) == listing.seller, "Seller no longer owns token.");
        require(getAllowanceForERC20(msg.sender) >= listing.price , "Please approve tokens before transferring");

        // Listing expired - should not be able to buy
        if(block.timestamp > listing.expiration){
            listingIdToListing[tokenIdToListingId[listing.tokenAddress][listing.tokenId]].state = State.CANCELLED;
            _removeListingStorage(listingIdToListing[tokenIdToListingId[listing.tokenAddress][listing.tokenId]].listingId);

            // Emit the event
            emit ListingCancelled(listingIdToListing[tokenIdToListingId[listing.tokenAddress][listing.tokenId]].listingId,listingIdToListing[tokenIdToListingId[listing.tokenAddress][listing.tokenId]].tokenId, listingIdToListing[tokenIdToListingId[listing.tokenAddress][listing.tokenId]].tokenAddress, listingIdToListing[tokenIdToListingId[listing.tokenAddress][listing.tokenId]].seller);
        }
        ...
        // Transfer the NFT
        IERC721(listingIdToListing[listingId].tokenAddress).safeTransferFrom(
            listing.seller,
            msg.sender,
            listing.tokenId
        );
        ...
        emit ListingSold(listing.listingId, listing.tokenId, listing.seller, listing.tokenAddress, msg.sender, listing.price);
    }
    ...
    */
    function cancelListing(uint256 listingId)
        external
        onlyNftOwner(
            listingIdToListing[listingId].tokenAddress,
        ...
        // Emit the event
        emit ListingCancelled(listingIdToListing[listingId].listingId, listingIdToListing[listingId].tokenId, listingIdToListing[listingId].tokenAddress, listingIdToListing[listingId].seller);
    }
    ...
    function _removeListingStorage(uint256 listingId) internal {
        ...
        delete tokenIdToListingId[listingIdToListing[listingId].tokenAddress][listingIdToListing[listingId].tokenId];
    ...
    */
    function _addListingStorage(Listing memory listing) internal {
        ...
        tokenIdToListingId[listing.tokenAddress][listing.tokenId] = listing.listingId;
    ...
    function getActiveListingByTokenId(uint256 _tokenId, address _address) external view returns (Listing memory){
        Listing memory listing = listingIdToListing[tokenIdToListingId[_address][_tokenId]];
        return listing;
    }

    // Use listing ID and return offers array
    function getOffersByTokenId(uint256 _tokenId, address _address) external view returns (Offer[] memory){
        uint256[] memory tokenIdToActiveOfferIds = tokenIdToActiveOffers[_address][_tokenId].values();
        Offer[] memory userOffers = new Offer[](tokenIdToActiveOffers[_address][_tokenId].values().length);
    ...
    function setRoyalties(uint256 _royalty) public onlyOwner {
        require(_royalty <= 10000000, "Royalty amount is too high!");
        royalty = _royalty;
    }

    function modifyTokenaddress(address _address, bool usable) public onlyOwner {
        activeMarketplaceTokens[_address] = usable;
    }
}
