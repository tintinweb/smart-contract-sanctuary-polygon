...
contract Treasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        // TODO: add all genesis pools
        address(0x244208F60593868B233571603F27394a3a69d683),
        address(0x4773346aF2c71082F18E8f139313F315eBa12BF4)
    ];
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of WLRS price
    ...
    event RedeemedBonds(address indexed from, uint256 wlrsAmount, uint256 bondAmount);
    event BoughtBonds(address indexed from, uint256 wlrsAmount, uint256 bondAmount);
    ...
    modifier checkEpoch() {
        ...
        epoch = epoch.add(1);
        epochSupplyContractionLeft = (getWlrsPrice() > wlrsPriceCeiling) ? 0 : getWlrsCirculatingSupply().mul(maxSupplyContractionPercent).div(10000);
    }
    ...
    function getBurnableWlrsLeft() public view returns (uint256 _burnableWlrsLeft) {
        uint256 _wlrsPrice = getWlrsPrice();
        if (_wlrsPrice <= wlrsPriceOne) {
            uint256 _wlrsSupply = getWlrsCirculatingSupply();
            uint256 _bondMaxSupply = _wlrsSupply.mul(maxDebtRatioPercent).div(10000);
            uint256 _bondSupply = IERC20(wBond).totalSupply();
            if (_bondMaxSupply > _bondSupply) {
                uint256 _maxMintableBond = _bondMaxSupply.sub(_bondSupply);
                uint256 _maxBurnableWlrs = _maxMintableBond.mul(_wlrsPrice).div(1e18);
                _burnableWlrsLeft = Math.min(epochSupplyContractionLeft, _maxBurnableWlrs);
            }
        }
    }
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (_wlrsPrice <= wlrsPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _discountAmount = _bondAmount.sub(wlrsPriceOne).mul(discountPercent).div(10000);
    ...
    function getBondPremiumRate() public view returns (uint256 _rate) {
        uint256 _wlrsPrice = getWlrsPrice();
        if (_wlrsPrice > wlrsPriceCeiling) {
            uint256 _wlrsPricePremiumThreshold = wlrsPriceOne.mul(premiumThreshold).div(100);
            if (_wlrsPrice >= _wlrsPricePremiumThreshold) {
                //Price > 1.10
                uint256 _premiumAmount = _wlrsPrice.sub(wlrsPriceOne).mul(premiumPercent).div(10000);
    ...
    function initialize(
        ...
        wlrsPriceOne = 10**18; // This is to allow a PEG of 1 WLRS per UST
        ...
        // First 10 epochs with 4% expansion
        bootstrapEpochs = 10;
    ...
    function setWlrsPriceCeiling(uint256 _wlrsPriceCeiling) external onlyOperator {
        require(_wlrsPriceCeiling >= wlrsPriceOne && _wlrsPriceCeiling <= wlrsPriceOne.mul(120).div(100), "out of range"); // [$1.0, $1.2]
    ...
    function setPremiumThreshold(uint256 _premiumThreshold) external onlyOperator {
        require(_premiumThreshold >= wlrsPriceCeiling, "_premiumThreshold exceeds wlrsPriceCeiling");
    ...
    function getWlrsCirculatingSupply() public view returns (uint256) {
        ...
        for (uint8 entryId = 0; entryId < excludedFromTotalSupply.length; ++entryId) {
            balanceExcluded = balanceExcluded.add(wlrsErc20.balanceOf(excludedFromTotalSupply[entryId]));
        }
        return totalSupply.sub(balanceExcluded);
    }

    function buyBonds(uint256 _wlrsAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        require(_wlrsAmount > 0, "Treasury: cannot purchase bonds with zero amount");
        ...
        require(_wlrsAmount <= epochSupplyContractionLeft, "Treasury: not enough bond left to purchase");
        ...
        require(newBondSupply <= wlrsSupply.mul(maxDebtRatioPercent).div(10000), "over max debt ratio");

        IBasisAsset(wlrs).burnFrom(msg.sender, _wlrsAmount);
        IBasisAsset(wBond).mint(msg.sender, _bondAmount);

        epochSupplyContractionLeft = epochSupplyContractionLeft.sub(_wlrsAmount);
        _updateWlrsPrice();
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        require(IERC20(wlrs).balanceOf(address(this)) >= _wlrsAmount, "Treasury: treasury has no more budget");

        seigniorageSaved = seigniorageSaved.sub(Math.min(seigniorageSaved, _wlrsAmount));
    ...
    function _calculateMaxSupplyExpansionPercent(uint256 _wlrsSupply) internal returns (uint256) {
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            _sendToBoardroom(wlrsSupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else {
            if (previousEpochWlrsPrice > wlrsPriceCeiling) {
                // Expansion ($WLRS Price > 1 $UST): there is some seigniorage to be allocated
                ...
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    ...
                    uint256 _seigniorage = wlrsSupply.mul(_percentage).div(1e18);
