...
__with_semver(
    ...
    function buy(address to, bytes32 tierValue) external payable {
        ...
        _claim();
        _transferToTreasury();
        address appAddress = _kernel.getAppAddress(AppsIds.ERC721);
...
__with_semver(
    ...
    function buy(bytes32[] calldata proof, bytes32 tierValue) external payable {
        ...
        _transferToTreasury();
        address appAddress = _kernel.getAppAddress(AppsIds.ERC721);
        ERC721Properties(appAddress).mint(msg.sender, tierValue);
        
        _afterBuy(msg.sender, appAddress, tierValue, cost, "Whitelist", __semver());
    }
