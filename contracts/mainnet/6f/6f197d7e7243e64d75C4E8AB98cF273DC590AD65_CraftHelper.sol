...
contract CraftHelper {
    ...
    function craft(
        ...
        for (uint256 i = 0; i < ingredients.length; i++) {
            if (ingredients[i].token != address(0)) {
                IERC20(ingredients[i].token).safeTransferFrom(msg.sender, address(crafter), ingredients[i].amount);
            } else {
                _addLiquidity(
    ...
    function _computeLpAmounts(IUniswapV2Pair pair, uint256 amount) private view returns (uint256, uint256) {
        ...
        if (factory.feeTo() != address(0)) {
            if (kLast != 0) {
                uint256 rootK = sqrt(uint256(reserve0).mul(reserve1));
                uint256 rootKLast = sqrt(kLast);
                if (rootK > rootKLast) {
                    uint256 numerator = totalSupply.mul(rootK.sub(rootKLast));
                    uint256 denominator = rootK.mul(9).add(rootKLast);
