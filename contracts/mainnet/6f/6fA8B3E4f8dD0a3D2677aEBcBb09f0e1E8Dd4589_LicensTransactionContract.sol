...
contract LicensTransactionContract is Ownable, ReentrancyGuard{
    ...
    constructor(address _accountContract, address _artListingNFT, address _licenseNFT, address _usdt, address _treasuryWallet, uint256 _taxPercent, uint256 _minimumFee){
        ...
        require(_usdt != address(0),"USDT contract cannot be zero address");
        require(_treasuryWallet != address(0),"Treasury NFT contract cannot be zero address");
    ...
    function createListing(string memory _listingCID, string memory _creativeCID, string memory _licenseCID, uint256 _price, bool _isExclusive, uint256 _quantity, uint256 _exclusivitySurcharge, bool _isUnlimitedSupply) external nonReentrant {
        ...
        totalListing++;
        uint256 tokenId = artListingNFT.safeMint(totalListing, msg.sender,_listingCID);
        ListingDetails memory _listingInfo = ListingDetails(totalListing, _listingCID, _creativeCID, _licenseCID, tokenId, msg.sender, _price,  _isExclusive, _quantity, _exclusivitySurcharge, _isUnlimitedSupply, 0, false);
    ...
    function license(uint256 _listingId, bool _exclusivePurchase) external nonReentrant{
        ...
        usdt.transferFrom(msg.sender, listing.owner, price);
        uint256 taxAmount = price  * taxPercent / 100;
        if(taxAmount < minimumFee){
            taxAmount = minimumFee;
        }
        usdt.transferFrom(msg.sender, treasuryWallet, taxAmount);
