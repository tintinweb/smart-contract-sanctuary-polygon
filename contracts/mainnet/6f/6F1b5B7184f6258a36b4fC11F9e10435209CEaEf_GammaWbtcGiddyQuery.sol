...
contract GammaWbtcGiddyQuery is GiddyQueryV2, Initializable, OwnableUpgradeable {
  ...
  function getDepositRatios() external view override returns (uint256[] memory ratios) {
    ...
    (ratios[0], ratios[1]) = IUniProxy(IHypervisor(POS).whitelistedAddress()).getDepositAmount(POS, GIDDY_TOKEN, 100000e18);
    ratios[0] = (ratios[0] + ratios[1]) / 2;
    ratios[1] = (100000e8 * RATIO_PERCENT) / (100000e8 + ratios[0]);
    ratios[0] = (ratios[0] * RATIO_PERCENT) / (100000e8 + ratios[0]);
  }
  ...
  function getWithdrawAmounts(uint256 staked) external view override returns (uint256[] memory amounts) {
    ...
    (amounts[0], amounts[1]) = calcTokens(POS, staked);
  }
