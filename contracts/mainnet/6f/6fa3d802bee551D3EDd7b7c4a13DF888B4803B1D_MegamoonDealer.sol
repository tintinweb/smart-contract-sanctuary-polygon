...
contract MegamoonDealer is Ownable, IMegamoonDealer {
    ...
    function returnFunds(uint256 _lotteryId, uint256 _amount, uint256 _totalPrize) external override {
        ...
        for ( uint256 i = 0; i < _users.length; i++ ) {
            ...
            if (userInjectAmount > 0) {
                ...
                user.stakingAmount -= userInjectAmount;
        ...
        emit ReturnFunds(_lotteryId, sumReward, _totalPrize);
    }
    ...
    function viewUserInfoByLotteryId(uint256 lotteryId, address user) external view returns (uint256, uint256, uint256) {
        uint256 injectAmount = _userInjectionHistory[user][lotteryId];
        uint256 rewardAmount = _userRewardHistory[user][lotteryId];
    ...
    function withdraw(uint _amount) public {
        ...
        if (user.stakingAmount == 0) { isActive[msg.sender] = false; }
