...
contract PaintParty is Ownable, NonDilutiveA, ReentrancyGuard {
    using Address for address;
    using Strings for uint256;
    uint256 public cost = 1 ether; //1 MATIC

    ///@notice mints reserve tokens upon deployment
    constructor(
         string memory _name
        ...
        ) 
    {
        ...
        //see Mimeticmetadata.sol
        return _tokenURIMimetic(_tokenId);
    }

    // Minting

    ///@notice public mint, requires mintOpen == true
    function mintPublic(uint16 amount) external payable nonReentrant callerIsUser
    {   
    ...
    ///@notice sets mint cost 
    function setCost(uint256 newCost) public onlyOwner {
        cost = newCost;
    }

    ///@notice sets base unrevealed token uri
    function setBaseUnrevealedUri(string memory newUri) public onlyOwner {    
        baseUnrevealedURI = newUri;
    }

    ///@notice withdraw all funds in contract and send to owner
    function withdraw() 
        external
        payable 
        onlyOwner 
