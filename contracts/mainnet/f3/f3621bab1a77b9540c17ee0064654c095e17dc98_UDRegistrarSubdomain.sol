...
contract UDRegistrarSubdomain is Ownable {
    ...
    mapping(address => uint) whitelistAddrPrice;
    mapping(address => bool) addrFreMint;
    ...
    function setFreeMint(address[] memory _addresses, bool _status) external onlyOwner {
        for (uint i=0; i<_addresses.length; i++) {
            addrFreMint[_addresses[i]] = _status;
        }
    }

    function setSpecialPrice(address[] memory _addresses, uint256 _price) external onlyOwner {
        for (uint i=0; i<_addresses.length; i++) {
            whitelistAddrPrice[_addresses[i]] = _price;
        }
    }

    function checkAddrPrice(address _address) external view returns (uint256) {
        return _checkAddrPrice(_address);
    }

    function _checkAddrPrice(address _address) internal view returns (uint256) {
        if (addrFreMint[_address]) {
            return 0;
        } else if (whitelistAddrPrice[_address] != 0x0) {
            return whitelistAddrPrice[_address];
        } else {
            return price;
        }
    }

    function buySubDomain(string calldata _name, address _address) payable external {
        uint _price = _checkAddrPrice(_address);
        require(msg.value == _price, "Buying Price not match");
        _buyDomain(_name, _address);
    }

    function buyBulkDomain(string[] memory _names, address _address) payable external {
        // check price
        uint _price = _checkAddrPrice(_address) * _names.length;
        require(msg.value == _price, "Buying Price not match");

        for (uint i=0; i<_names.length; i++) {
            _buyDomain(_names[i], _address);
        }
    }

    function _buyDomain(string memory _name, address _to) internal {
        string[] memory path;
        path = new string[](parents.length+1);
        path[0] = _name;
        for (uint i=0; i<parents.length; i++) {
        ...
        string[] memory value;
        value = new string[](1);
        value[0] = Strings.toHexString(_to);
        
        udManager.issueWithRecords(_to, path, keys, value);
    }
