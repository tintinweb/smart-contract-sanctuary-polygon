...
contract WLDYCoinTrade is AccessControl, NativeMetaTransaction{
    ...
    function metatrade(address _address1, uint256 quant1, bytes32 sigR1, bytes32 sigS1, uint8 sigV1,address _address2, uint256 quant2, bytes32 sigR2, bytes32 sigS2, uint8 sigV2) external {
    require(quant1 > 0 || quant2 > 0, "Atleast one quantity must be positive");
    ...
    if(quant1 > 0){
        ...
        bytes memory brunFunctionalSignature = abi.encodePacked(
        ...
        token.token_contract.executeMetaTransaction(_address1, brunFunctionalSignature, sigR1, sigS1, sigV1);
    }
    if(quant2 > 0){

        bytes memory brunFunctionalSignature2 = abi.encodePacked(
            bytes4(keccak256("transfer(address,uint256)")), // function signature
            abi.encode(
                _address1,
                quant2
        ...
        token.token_contract.executeMetaTransaction(_address2, brunFunctionalSignature2, sigR2, sigS2, sigV2);
    }
    }
