...
contract QuickDragonsLairYA is BaseYA, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  address constant private QUICK_USDC_PAIR = 0x1F1E4c845183EF6d50E9609F16f6f9cAE43BC9Cb;
  address constant private QUICK_TOKEN = 0x831753DD7087CaC61aB5644b308642cc1c33Dc13;
  address constant private DQUICK_TOKEN_AND_LAIR = 0xf28164A485B0B2C90639E47b0f377b4a438a16B1;
  uint256 constant private EARNINGS_FEE = 150000;
  ...
  function swapTokenForToken(address router, address tokenSrc, address tokenDst, address receiver, uint256 amount) private returns (uint256) {
    address[] memory route = new address[](2);
    route[0] = tokenSrc;
    route[1] = tokenDst;
    if (!IERC20(tokenSrc).approve(router, amount)) {
      revert("Failed token swap approval");
    }
    return IUniswapV2Router02(router).swapExactTokensForTokens(amount, 0, route, receiver, block.timestamp + 1200)[1];
  }
  ...
  function getContractBalance() public view override returns (uint256) {
    return IDragonsLair(DQUICK_TOKEN_AND_LAIR).QUICKBalance(address(this));
  }
  ...
  function sharesToValue(uint256 shares) public view override returns (uint256) {
    ...
    uint256 pendingFees = (getContractBalance() - lastContractBalance) * EARNINGS_FEE / BASE_PERCENT;
    return (getContractBalance() - pendingFees) * shares / contractShares;
  }
  ...
  function deposit(bytes calldata auth, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    uint256 valueQuick = swapTokenForToken(QUICK_ROUTER, USDC_TOKEN, QUICK_TOKEN, address(this), value);

    uint256 newShares = contractShares == 0 ? valueQuick * INIT_SHARES : valueQuick * contractShares / getContractBalance();
    userShares[owner] = userShares[owner] + newShares;
    contractShares = contractShares + newShares;

    if (!IERC20(QUICK_TOKEN).approve(DQUICK_TOKEN_AND_LAIR, valueQuick)) {
      revert("Failed QUICK approval");
    }
    IDragonsLair(DQUICK_TOKEN_AND_LAIR).enter(valueQuick);
  ...
  function depositGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    uint256 usdcValue = swapTokenForToken(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, address(this), giddyValue);
    uint256 valueQuick = swapTokenForToken(QUICK_ROUTER, USDC_TOKEN, QUICK_TOKEN, address(this), usdcValue);

    uint256 newShares = contractShares == 0 ? valueQuick * INIT_SHARES : valueQuick * contractShares / getContractBalance();
    userShares[req.owner] = userShares[req.owner] + newShares;
    contractShares = contractShares + newShares;
  ...
  function withdraw(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    uint256 valueDQuick = IERC20(DQUICK_TOKEN_AND_LAIR).balanceOf(address(this)) * shares / contractShares;
    userShares[_msgSender()] = userShares[_msgSender()] - shares;
    contractShares = contractShares - shares;

    uint256 valueQuick = IDragonsLair(DQUICK_TOKEN_AND_LAIR).dQUICKForQUICK(valueDQuick);
    IDragonsLair(DQUICK_TOKEN_AND_LAIR).leave(valueDQuick);
    uint256 valueUsdc = swapTokenForToken(QUICK_ROUTER, QUICK_TOKEN, USDC_TOKEN, address(this), valueQuick);
    valueUsdc = deductFeeUsdc(valueUsdc, fap);

    if (!IERC20(USDC_TOKEN).transfer(_msgSender(), valueUsdc)) {
      revert("User transfer failed");
    }

    emit Withdraw(_msgSender(), valueUsdc);
  }
  ...
  function withdrawGiddy(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    uint256 value = IDragonsLair(DQUICK_TOKEN_AND_LAIR).dQUICKForQUICK(valueDQuick);
    IDragonsLair(DQUICK_TOKEN_AND_LAIR).leave(valueDQuick);

    value = swapTokenForToken(QUICK_ROUTER, QUICK_TOKEN, USDC_TOKEN, address(this), value);
    value = swapTokenForToken(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, address(this), value);
  ...
  function unstickMoney() public override onlyOwner {
    ...
    uint256 dQuickBalance = IERC20(DQUICK_TOKEN_AND_LAIR).balanceOf(address(this));
    if (!IERC20(DQUICK_TOKEN_AND_LAIR).transfer(_msgSender(), dQuickBalance)) {
      revert("DQUICK transfer failed");
    }
    uint256 quickBalance = IERC20(QUICK_TOKEN).balanceOf(address(this));
    if (!IERC20(QUICK_TOKEN).transfer(_msgSender(), quickBalance)) {
      revert("QUICK transfer failed");
    }
  }
  ...
  function deductFeeGiddy(uint256 amount, uint256 fap) internal returns (uint256) {
    if (fap > 0) {
      swapTokenForToken(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, feeAccount, fap);
    }
    return amount - fap;
  }
  ...
  modifier deductEarningsFee() {
    ...
    if (earnings > 0) {
      uint256 feeQUICK = earnings * EARNINGS_FEE / BASE_PERCENT;
      if (feeQUICK > 0) {
        IDragonsLair(DQUICK_TOKEN_AND_LAIR).leave(IDragonsLair(DQUICK_TOKEN_AND_LAIR).QUICKForDQUICK(feeQUICK));
      }
      if (IERC20(QUICK_TOKEN).balanceOf(address(this)) >= COMPOUND_THRESHOLD) {
        swapTokenForToken(QUICK_ROUTER, QUICK_TOKEN, USDC_TOKEN, feeAccount, IERC20(QUICK_TOKEN).balanceOf(address(this)));
      }
      emit Compound(earnings - feeQUICK, lastCompoundTime, lastContractBalance);
      lastCompoundTime = block.timestamp;
    }
    _;
