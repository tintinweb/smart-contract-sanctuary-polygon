...
contract Treasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        address(0x92f80BF38ac59b7b8cB578d2EB3BCa2835Ca53A2) // UnicornGenesisRewardPool
    ];

    address public unicorn;
    address public wings;
    address public boardroom;
    address public oracle;

    // price
    uint256 public unicornPriceOne;
    uint256 public unicornPriceCeiling;
    uint256 public unicornPriceRebase;
    ...
    event SetUnicornPriceCeiling(uint256 newValue);
    ...
    modifier checkOperator() {
        require(
            IBasisAsset(unicorn).operator() == address(this) &&
            IBasisAsset(wings).operator() == address(this) &&
    ...
    // oracle
    function getUnicornPrice() public view returns (uint256) {
        try IOracle(oracle).consult(unicorn, 1e18) returns (uint144 price) {
            return uint256(price);
        } catch {
            revert("Treasury: failed to consult Unicorn price from the oracle");
        }
    }

    function getTwapPrice() public view returns (uint256) {
        try IOracle(oracle).twap(unicorn, 1e18) returns (uint144 price) {
            return uint256(price);
        } catch {
            revert("Treasury: failed to twap Unicorn price from the oracle");
        }
    }
    ...
    function initialize(
        ...
        require(_unicorn != address(0), "!_unicorn");
        require(_wings != address(0), "!_wings");
        ...
        unicornPriceOne = 10 ** 18;
        // This is to allow a PEG of 1 unicorn per USDC
        unicornPriceRebase = 102 * 10 ** 16;
        // 1.02 USDC
        unicornPriceCeiling = unicornPriceOne;
        ...
        IMainToken(unicorn).grantRebaseExclusion(address(this));
        IMainToken(unicorn).grantRebaseExclusion(daoFund);
        IMainToken(unicorn).grantRebaseExclusion(address(boardroom));
    ...
    function grantRebaseExclusion(address who) external onlyOperator {
        IMainToken(unicorn).grantRebaseExclusion(who);
    }

    function setUnicornPriceCeiling(uint256 _unicornPriceCeiling) external onlyOperator {
        require(_unicornPriceCeiling >= unicornPriceOne && _unicornPriceCeiling <= unicornPriceOne.mul(120).div(100), "out of range");
        // [$1.0, $1.2]
        unicornPriceCeiling = _unicornPriceCeiling;
        emit SetUnicornPriceCeiling(_unicornPriceCeiling);
          }
    ...
    function setTwapForMaxExpansion(uint256 _twapThresold) external onlyOperator {
        ...
        TWAP_DELTA_FOR_MAX_EXPANSION = _twapThresold.sub(unicornPriceOne);
    }
    ...
    function getUnicornCirculatingSupply() public view returns (uint256) {
        return IMainToken(unicorn).rebaseSupply();
    }

    function getEstimatedReward() external view returns (uint256) {
        uint256 unicornTotalSupply = IMainToken(unicorn).totalSupply();
        if (unicornTotalSupply < minMainSupplyToExpansion) {
            unicornTotalSupply = minMainSupplyToExpansion;
        }

        uint256 percentage = calcExpansionRate(unicornTotalSupply);
        uint256 estimatedReward = unicornTotalSupply.mul(percentage).div(10000);
    ...
    function _sendToBoardroom(uint256 _amount) internal {
        IMainToken unicornErc20 = IMainToken(unicorn);
        unicornErc20.mint(address(this), _amount);

        uint256 _daoFundSharedAmount = _amount.mul(daoFundSharedPercent).div(100);
        unicornErc20.transfer(daoFund, _daoFundSharedAmount);
        emit DaoFundFunded(block.timestamp, _daoFundSharedAmount);

        uint256 _devFundSharedAmount = _amount.mul(devFundSharedPercent).div(100);
        unicornErc20.transfer(devFund, _devFundSharedAmount);
        emit DevFundFunded(block.timestamp, _devFundSharedAmount);

         uint256 _pegRewardPoolSharedAmount = _amount.mul(pegRewardPoolPercent).div(100);
        unicornErc20.transfer(pegRewardPool, _pegRewardPoolSharedAmount);
        ...
        IERC20(unicorn).safeApprove(boardroom, 0);
        IERC20(unicorn).safeApprove(boardroom, _amount);
        IBoardroom(boardroom).allocateSeigniorage(_amount);
        emit BoardroomFunded(block.timestamp, _amount);
    }

    function calculateMaxSupplyExpansionPercent(uint256 _unicornSupply) public view returns (uint256) {
        ...
        for (uint256 tierId = supplyTierLength - 1; tierId >= 0; --tierId) {
            if (_unicornSupply >= supplyTiers[tierId]) {
    ...
    function calculateMinSupplyExpansionPercent(uint256 _unicornSupply) public view returns (uint256) {
    ...
    function calcDynamicExpansionPercent(uint256 _unicornSupply) public view returns (uint256 _percent) {
        uint256 max_expansion = calculateMaxSupplyExpansionPercent(_unicornSupply);
        uint256 min_expansion = calculateMinSupplyExpansionPercent(_unicornSupply);
        uint256 twap = getUnicornPrice();

        if (twap <= unicornPriceOne) {
            _percent = 0;
        } else {
            uint256 twap_delta = twap.sub(unicornPriceOne);
    ...
    function calcExpansionRate(uint256 _unicornSupply) public view returns(uint256 _expansion_rate) {
        if (useFixedTwap) {
            _expansion_rate = calculateMaxSupplyExpansionPercent(_unicornSupply);
        } else {
            _expansion_rate = calcDynamicExpansionPercent(_unicornSupply);
        }
    }

    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        _updatePrice();
        previousEpochMainPrice = getUnicornPrice();
        uint256 unicornTotalSupply = IMainToken(unicorn).totalSupply();
        if (epoch < bootstrapEpochs) {
            _sendToBoardroom(unicornTotalSupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else {
            if (previousEpochMainPrice > unicornPriceCeiling) {
                // Expansion
                if (unicornTotalSupply < minMainSupplyToExpansion) {
                    unicornTotalSupply = minMainSupplyToExpansion;
                }
                uint256 _percentage = calcExpansionRate(unicornTotalSupply);
                uint256 _savedForBoardroom = unicornTotalSupply.mul(_percentage).div(10000);
                if (_savedForBoardroom > 0) {
                    ...
                    if (boardRoomAmount > 0) {
                        ...
                        // mint to DAOFund
                        IMainToken(unicorn).mint(daoFund, _savedForBoardroom);
                    }
                }
            }

            // Rebase
            if (previousEpochMainPrice < unicornPriceOne) {
                consecutiveEpochHasPriceBelowOne = consecutiveEpochHasPriceBelowOne.add(1);
            } else {
                consecutiveEpochHasPriceBelowOne = 0;
            }

            if (rebaseStarted && previousEpochMainPrice < unicornPriceOne) {
                _rebase(unicornPriceOne);
                consecutiveEpochHasPriceBelowOne = 0;
            } else {
                rebaseStarted = false;
                // twap <= 0.85 MATIC => rebase
                // {consecutiveEpochHasPriceBelowOne} consecutive epoch has twap < 1 MATIC => rebase
                if (previousEpochMainPrice <= unicornPriceRebase || consecutiveEpochHasPriceBelowOne == numberOfEpochBelowOneCondition) {
                    _rebase(unicornPriceOne);
                    consecutiveEpochHasPriceBelowOne = 0;
                }
            }
    ...
    function computeSupplyDelta() public view returns (bool negative, uint256 supplyDelta, uint256 targetRate) {
        ...
        supplyDelta = mathRound(getUnicornCirculatingSupply().mul(rebasePercentage).div(ONE));
    }
    ...
    function _rebase(uint256 _oldPrice) internal onlyOperator {
        ...
        uint256 oldTotalSupply = IERC20(unicorn).totalSupply();
        ...
        if (supplyDelta > 0) {
            ...
            newTotalSupply = IMainToken(unicorn).rebase(epoch, supplyDelta, negative);
