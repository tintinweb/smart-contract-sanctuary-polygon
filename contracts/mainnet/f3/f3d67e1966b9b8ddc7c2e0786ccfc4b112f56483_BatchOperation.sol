...
contract BatchOperation is AccessControlEnumerableUpgradeable {
    ...
    function migrate(
        ...
        for (uint256 i = 0; i < _requests.length; i++) {
            ...
            IRarumNFT(request.nftAddress).permit(
                _from,
                address(this),
                true,
                request.signatureApproval.nonce,
                _deadline,
                request.signatureApproval.v,
                request.signatureApproval.r,
                request.signatureApproval.s
            );
            IRarumNFT(request.nftAddress).safeBatchTransferFrom(
                _from,
                _to,
                request.nftIds,
                request.balances,
            ...
            IRarumNFT(request.nftAddress).permit(
                ...
                address(this),
                false,
                request.signatureRenounce.nonce,
                _deadline,
                request.signatureRenounce.v,
                request.signatureRenounce.r,
                request.signatureRenounce.s
            );
        }
    }
}
