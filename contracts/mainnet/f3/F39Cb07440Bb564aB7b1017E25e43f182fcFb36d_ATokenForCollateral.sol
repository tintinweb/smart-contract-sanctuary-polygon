...
library SafeERC20 {
  ...
  function safeTransfer(
    ...
    callOptionalReturn1(token, abi.encodeWithSelector(token.transfer.selector, to, value));
  }
  ...
  function safeTransferFrom(
    ...
    callOptionalReturn2(token, abi.encodeWithSelector(token.transferFrom.selector, from, to, value));
  }
  ...
  function safeApprove(
    ...
    callOptionalReturn3(token, abi.encodeWithSelector(token.approve.selector, spender, value));
  }

  function callOptionalReturn1(IERC20 token, bytes memory data) private {
    require(address(token).isContract(), '1, SafeERC20: call to non-contract');

    // solhint-disable-next-line avoid-low-level-calls
    (bool success, bytes memory returndata) = address(token).call(data);
    require(success, '1, SafeERC20: low-level call failed');

    if (returndata.length > 0) {
      // Return data is optional
      // solhint-disable-next-line max-line-length
      require(abi.decode(returndata, (bool)), '1, SafeERC20: ERC20 operation did not succeed');
    }
  }

  function callOptionalReturn2(IERC20 token, bytes memory data) private {
    require(address(token).isContract(), '2, SafeERC20: call to non-contract');

    // solhint-disable-next-line avoid-low-level-calls
    (bool success, bytes memory returndata) = address(token).call(data);
    require(success, '2, SafeERC20: low-level call failed');

    if (returndata.length > 0) {
      // Return data is optional
      // solhint-disable-next-line max-line-length
      require(abi.decode(returndata, (bool)), '2, SafeERC20: ERC20 operation did not succeed');
    }
  }

  function callOptionalReturn3(IERC20 token, bytes memory data) private {
    require(address(token).isContract(), '3, SafeERC20: call to non-contract');

    // solhint-disable-next-line avoid-low-level-calls
    (bool success, bytes memory returndata) = address(token).call(data);
    require(success, '3, SafeERC20: low-level call failed');

    if (returndata.length > 0) {
      // Return data is optional
      // solhint-disable-next-line max-line-length
      require(abi.decode(returndata, (bool)), '3, SafeERC20: ERC20 operation did not succeed');
    }
  }
}
