...
contract KC is ERC721A, ReentrancyGuard, AccessControl, ContextMixin {
    ...
    uint256 public constant MAX_GUEST_SUPPLY = 850;
    uint256 public constant MAX_GUEST_SUPPLY_PER_WALLET = 1;
    uint256 public constant PRICE_PER_TOKEN = 0.1 ether;

    string private _baseURIextended;
    uint256 public reserveSupply;
    uint256 public guestSupply;

    address payable public immutable shareholderAddress;

    mapping(address => uint256) private _guestsNumMinted;

    constructor(address payable shareholderAddress_) ERC721A("Kent Christensen", "KC") {
     ...
     */
    function _setGuestListMinted(address to, uint256 numberOfTokens) internal virtual {
        _guestsNumMinted[to] += numberOfTokens;
    }

    /**
     * @dev gets the number of tokens from the address
     */
    function getGuestListMinted(address from) public view virtual returns (uint256) {
        return _guestsNumMinted[from];
    }
    ...
    function devMint(uint256 numberOfTokens) external onlyRole(SUPPORT_ROLE) ableToMint(numberOfTokens) nonReentrant {
        require(reserveSupply + numberOfTokens <= MAX_RESERVE_SUPPLY, "Exceeds max reserve supply");
    ...
    /**
     * Gets called when someone scans their wallet QR code IRL 
     */
    function guestMint(address recipient) external onlyRole(SUPPORT_ROLE) ableToMint(1) nonReentrant {
        require(guestSupply + 1 <= MAX_GUEST_SUPPLY, "Exceeds max guest supply");
        require(balanceOf(recipient) < MAX_GUEST_SUPPLY_PER_WALLET, "Guest already has one");
        guestSupply += 1;
        _setGuestListMinted(recipient, 1);
     ...
     * public
     */
    function mint(uint256 numberOfTokens) external payable ableToMint(numberOfTokens) nonReentrant {
        require(numberOfTokens <= MAX_PUBLIC_MINT, "Exceeds max token purchase");
