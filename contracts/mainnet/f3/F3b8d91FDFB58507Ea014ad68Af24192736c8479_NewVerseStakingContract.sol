...
contract NewVerseContract is ERC721, ERC2981, ERC721Enumerable, IMintableERC721, AccessControl, Ownable {
    ...
    */
    uint256 public constant UID = 0x5007e72d023f02da508c5cb2cde238b15f8cb07c509015d7c25372a4835eebb1;
    ...
    */
    ...
    event royaltySet(uint indexed tokenId, address indexed Royaltyreceiver, uint96 indexed royaltyValue);
    ...
    */
    ...
    /**
        * @inheritdoc ERC721
    */
    function supportsInterface(bytes4 interfaceId) public view override(ERC721, ERC2981, ERC721Enumerable, AccessControl) returns (bool) {
   ...
   /// @notice Mint one token to `_to`
   /// @param _to the recipient of the token
   /// @param _royaltyRecipient the recipient for royalties (if _royaltyValue > 0)
   /// @param _royaltyValue the royalties asked for (EIP2981)
   function mintRoyalty(address _to, address _royaltyRecipient, uint96 _royaltyValue) external onlyRole(MINTER_ROLE) {
        _tokenIds.increment();
        ...
        if (_royaltyValue > 0) {
            ...
            emit royaltySet(tokenId, _royaltyRecipient, _royaltyValue);
        }
    }
    ...
    function mintBatchRoyalty(address[] memory recipients, address[] memory royaltyRecipients, uint96[] memory royaltyValues) external onlyRole(MINTER_ROLE) {
        require(
        ...
        for (uint256 i; i < recipients.length; i++) {
            ...
            if (royaltyValues[i] > 0) {
                ...
                emit royaltySet(tokenId, royaltyRecipients[i], royaltyValues[i]);
            }
        }
    }
}
...
contract NewVerseStakingContract is IERC721Receiver, ReentrancyGuard, Ownable  {

    NewVerseContract newVerseContract;
    ...
    */
    ...
    uint256 public constant UID = 0xe51b9ad7b3b80916d1c8e5b9f4c165ca01a097165204ac742d9daa7f0245b599;
    ...
    // staker_address => (tokenId => timestamp)
    mapping (address => mapping(uint256 => uint256)) public stakes;

    // map staker total staking time
    // staker_address => (tokenId => stakingDuration_for_tokenId)
    mapping (address => mapping(uint256 => uint256)) public stakingTime;

    uint minStakingTime;

    event NFTStaked(address indexed owner, address indexed stakingContract, uint256 indexed tokenId);
    event NFTUnstaked(address indexed stakingContract, address indexed owner, uint256 indexed tokenId);
    event Claimed(address indexed owner, uint256 indexed amount);

    constructor(NewVerseContract _newVerseContract, uint _minStakingTime) {
        newVerseContract = _newVerseContract;
        minStakingTime = _minStakingTime;
    }

      function setMinStakingTime(uint _minStakingTime) public onlyOwner {
        minStakingTime = _minStakingTime;
    }

    function stake(uint256 _tokenId) external {
        // the start of staking time
        stakes[msg.sender][_tokenId] = block.timestamp;
        // (1) newVerseContract.approve(address(this), _tokenId);
        // (2) newVerseContract.safeTransferFrom(msg.sender, address(this), _tokenId)
        newVerseContract.safeTransferFrom(msg.sender, address(this), _tokenId);
        emit NFTStaked(msg.sender, address(this), _tokenId);
    }

    function unstake(uint256 _tokenId) external {
        uint stakeDuration = block.timestamp - stakes[msg.sender][_tokenId];
        require(stakes[msg.sender][_tokenId] > 0, "unstake: tokenId is not valid!");
        require(stakeDuration  >= minStakingTime, "StakingTime for this NFT is less than minStakingTime");
        // update staking time
        // StakingTime = now - StakingStartTime
        stakingTime[msg.sender][_tokenId] = stakeDuration;
        delete stakes[msg.sender][_tokenId];
        newVerseContract.safeTransferFrom(address(this), msg.sender, _tokenId);
        emit NFTUnstaked(address(this), msg.sender, _tokenId);
    }
