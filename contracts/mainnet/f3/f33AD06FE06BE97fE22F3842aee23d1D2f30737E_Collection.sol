...
contract Collection is Ownable {
  ...
  address public designAddr = 0xaf712802003ff33Fba186740C02b71B84C0f637D;
  address public CheckQuaAddr = 0x1c256eCD8149C0e20E29aB1db4A69dAA4fAE39b6;
  address public dataAddr = 0x2D0f300BE3F44fff5f1dE98Dd9C213b78f8FB124;
  address public AdAddr;

  // public
  function mintNewAnimat(string memory _Title,string[] memory _tagName, string[] memory _tagValue,string memory _imageLink, 
    ...
    dataContract.newData(totalToken, _Title, _tagName, _tagValue, _imageLink, _animaLink, BpaperMinterID, maxMint, getDate(block.timestamp));
  ...
  function mintNewImage(string memory _Title,string[] memory _tagName, string[] memory _tagValue,string memory _imageLink, 
    ...
    //store data in data contract
    Data dataContract = Data(dataAddr);
    dataContract.newData(totalToken, _Title, _tagName, _tagValue, _imageLink, "", BpaperMinterID, maxMint, getDate(block.timestamp));
  ...
  function updateAnimaContent(uint _PaperID, string memory _Title, string[] memory _tagName, string[] memory _tagValue,string memory _imageLink,
    string memory _animaLink) public {
    if (msg.sender != owner() && msg.sender != AdAddr) {
      CheckQua checkContract = CheckQua(CheckQuaAddr);
    ...
    string memory tempURI = designContract.buildMetadata(uint256(collectionID).toString(),
        _Title, _tagName, _tagValue, getDate(block.timestamp), _imageLink, _animaLink, BpaperMinterID, 
        getBcardEthName(BpaperMinterID));

    //update new data in database
    Data dataContract = Data(dataAddr);
    dataContract.editData(_PaperID, _Title, _tagName, _tagValue, _imageLink, _animaLink, getDate(block.timestamp));
    dataContract.updateUri(_PaperID, tempURI);
    BpaperContract.buildMetaData(_PaperID);
  }

  function updateImageContent(uint _PaperID, string memory _Title, string[] memory _tagName, string[] memory _tagValue,string memory _imageLink) public {
    if (msg.sender != owner() && msg.sender != AdAddr) {
      CheckQua checkContract = CheckQua(CheckQuaAddr);
      require(cStr(checkContract.checkAmend(_PaperID, msg.sender),"Match"),
        checkContract.checkAmend(_PaperID, msg.sender));
    ...
    uint BpaperMinterID = BcardContract.AddressToTokenID(msg.sender);
    string memory tempURI = designContract.buildMetadata(uint256(collectionID).toString(),
        _Title, _tagName, _tagValue, getDate(block.timestamp), _imageLink, "", BpaperMinterID, 
        getBcardEthName(BpaperMinterID));

    //update new data in database
    Data dataContract = Data(dataAddr);
    dataContract.editData(_PaperID, _Title, _tagName, _tagValue, _imageLink, "", getDate(block.timestamp));
    dataContract.updateUri(_PaperID, tempURI);
    BpaperContract.buildMetaData(_PaperID);
  }

  function PreviewTokenURI(uint _BcardID, string memory _title, string[] memory _tagName, string[] memory _tagValue,
    string memory _imageLink, string memory _animaLink) public view returns (string memory){
        Design designContract = Design (designAddr);
        return designContract.buildMetadata("3",_title, _tagName,
          _tagValue, getDate(block.timestamp), _imageLink, _animaLink, 
          _BcardID, "Creator Name");
  }
...
contract Data is Ownable {
  ...
  // public
  function newData(uint _BpaperID, string memory _Title,string[] memory _tagName, string[] memory _tagValue,string memory _imageLink, 
    string memory _animaLink, uint _BpaperMinterID, uint _maxMint, string memory _date) public payable {  
    ...
    string memory tempUri = DesignContract.buildMetadata(uint256(collectionID).toString(),
        _Title, _tagName, _tagValue, _date, _imageLink, _animaLink, _BpaperMinterID, 
  ...
  function editData(uint _BpaperID, string memory _Title,string[] memory _tagName, string[] memory _tagValue,
    string memory _imageLink, string memory _animaLink, string memory _date) public payable {  
    require (msg.sender == owner() || msg.sender == AdAddr || msg.sender == CollectionContractAddr);
...
contract Design is Ownable{
    ...
    function buildMetadata(string memory _collectionID, string memory _title, string[] memory _tagName, string[] memory _tagValue, 
        string memory _editTime, string memory _imageLink, string memory _animaLink, uint _tkId, string memory _ethName) 
        public pure returns (string memory) {
        string memory tokenID = uint256(_tkId).toString();
        string memory title=toUpper(_title);
        string memory ethName = toUpper(_ethName);
        ...
        return string(abi.encodePacked(
                'data:application/json;base64,', Base64.encode(bytes(abi.encodePacked(
                            '{"name":"',
                            string(abi.encodePacked('Bpaper ', title,' by Bcard #',tokenID,' ',ethName)),
                            '","description":"', 
                            _title,
                            '", "image": "', 
                            _imageLink,
