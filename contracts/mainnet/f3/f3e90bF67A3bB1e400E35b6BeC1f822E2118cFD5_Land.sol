    ...
    Ownable
{
    ...
    uint256 public constant NUMBER_OF_RESERVED_LANDS = 0;
    address public immutable adminSigner;
    address private constant _recipient =
        0x2b5DC835DB4356B966B557A2dBf2d10494C46D5A;

    string public baseURI;
    uint256 public maxMintAmount = 10;
    uint256 public salePhaseSupply = 0;
    uint256 public price = 0.01 ether;
    address public wethContract;
    SalePhase private _salePhase;

    constructor(
        address _trustedForwarder,
        address _wethContract,
        address _adminSigner
    ) ERC721("Chicken Derby Land NFT", "LND") RandomlyAssigned(MAX_SUPPLY, NUMBER_OF_RESERVED_LANDS) {
        trustedForwarder = _trustedForwarder;
        wethContract = _wethContract;
        adminSigner = _adminSigner;
        _salePhase = SalePhase.Locked;
    }
    ...
    function setActiveSalePhase(
        SalePhase _phase,
        uint256 _amount,
        uint256 _price
    ) public onlyOwner {
        if (_phase == SalePhase.Locked) revert Errors.IncorrectPhase();
        _salePhase = _phase;
        price = _price;
        salePhaseSupply = tokenCount() + _amount;
        if (salePhaseSupply > MAX_SUPPLY) revert Errors.ExceedMaxSupply();
    }

    function getActiveSalePhase() public view returns (SalePhase) {
        return _salePhase;
    }
    ...
    function _mintRandomId(address to) internal {
        ...
        assert(id > NUMBER_OF_RESERVED_LANDS && id <= MAX_SUPPLY);
        _safeMint(to, id);
    }
    ...
    {
        if (_salePhase != SalePhase.PreSale) revert Errors.PreSaleNotActive();
        if (_mintAmount == 0) revert Errors.MintNoNFT();
        if (_mintAmount > maxMintAmount) revert Errors.ExceedMaxMintAmount();
        if (tokenCount() + _mintAmount > salePhaseSupply)
            revert Errors.ExceedSalePhaseSupply();
    ...
    function mint(uint256 _mintAmount)
        ...
        ensureAvailabilityFor(_mintAmount)
    {
        if (_salePhase != SalePhase.PublicSale)
