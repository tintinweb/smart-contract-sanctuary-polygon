...
contract LFiFarmsV2 is AccessControlUpgradeable {
    ...
    // update Reward Token to a new token
    ///@notice function to set a reward Token
    ///@param _tokenAddress Address of the new token
    function setRewardToken(address _tokenAddress)
        external
        onlyRole(MANAGER_ROLE)
    {
        reward = IERC20Upgradeable(_tokenAddress);
    }
