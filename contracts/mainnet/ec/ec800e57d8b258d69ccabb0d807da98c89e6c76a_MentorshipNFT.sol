...
interface IImgURIDescriptor {
    function imgURI(bytes calldata data)
        external
        pure
...
contract MentorshipNFT is ERC721, AccessControl {
    ...
    mapping(uint256 => string) roles; // Stores Names of roles
    mapping(uint256 => address) public imgUriDescriptor; // Stores address of ImgURIDescriptors

    // Data stored for each tokenId:
    mapping(uint256 => uint256) public tokenImgDescriptor;
    ...
    function setImgUriDescriptor(
        uint256 _imgUriDescriptor,
        address _imgUriDescriptorAddr
    ) public onlyRole(MINTER_ROLE) {
        imgUriDescriptor[_imgUriDescriptor] = _imgUriDescriptorAddr;
    }
    ...
    function mint(
        ...
        require(
            imgUriDescriptor[_imgUriDescriptor] != address(0x0),
            "MENT: Invalid uriDescriptor"
        );
        require(
        ...
        tokenImgDescriptor[totalSupply] = _imgUriDescriptor;
        ...
        returns (string memory)
    {
        ...
        string memory _year = Strings.toString(year[tokenId]);
        string memory _season = Strings.toString(season[tokenId]);
        string memory _role = roles[role[tokenId]];
        string memory _handle = handle[tokenId];
        string memory _other = other[tokenId];

        address imgUriDescAddr = imgUriDescriptor[tokenImgDescriptor[tokenId]];

        IImgURIDescriptor d = IImgURIDescriptor(imgUriDescAddr);

        string memory img = d.imgURI(
        ...
        string memory json = string(
            abi.encodePacked(
                ...
                '", "description": "Mentorship Programme NFTs","attributes":[{"trait_type":"Year","value":"',
                _year,
