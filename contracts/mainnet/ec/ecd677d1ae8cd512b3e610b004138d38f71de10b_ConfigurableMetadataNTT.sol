...
import "../utils/ERC4671/ERC4671.sol";


contract ConfigurableMetadataNTT is ERC4671 {
	...
	function setTokenUri(uint256 tokenId,string memory tokenUri_) external {
		require(_isCreator(), "Not Authorised");
		_getTokenOrRevert(tokenId);

		_tokenUris[tokenId] = tokenUri_;
	} 

	function mint(address owner,string memory tokenUri_) external {
        require(_isCreator(), "Not Authorised");
...
abstract contract ERC4671 is IERC4671, IERC4671Metadata, IERC4671Enumerable, ERC165 {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        _getTokenOrRevert(tokenId);
        bytes memory baseURI = abi.encodePacked(_baseURI());
        if (baseURI.length > 0) {
