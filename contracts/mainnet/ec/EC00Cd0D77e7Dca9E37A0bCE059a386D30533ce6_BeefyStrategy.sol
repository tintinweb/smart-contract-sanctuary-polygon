    ...
    IAlluoStrategyV3
{
    ...
    function getExpectedEntryExchangeRequests(
        ...
        ExchangeRequest[] memory request = new ExchangeRequest[](0);
    ...
    function getExpectedExitExchangeRequests(
        ...
        address entryToken = IBeefyVaultV6(beefyVaultAddress).want();

        if (beefyBoostAddress == address(0) && !swapRewards) {
            ExchangeRequest[] memory requestSingle = new ExchangeRequest[](1);
            requestSingle[0] = ExchangeRequest(entryToken, outputCoin);
        ...
        request[0] = ExchangeRequest(entryToken, outputCoin);

        for (uint256 i = 1; i <= len; i++) {
            address reward = expectedRewards.at(i - 1);
