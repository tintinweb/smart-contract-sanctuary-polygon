...
contract DCAAdvancedTransfer is BaseSiloAction {
  ...
  function transferAction(
    ...
    (inputs, outputs, ratio, recipients, , ) = abi.decode(
      configuration,
...

    {
      for (uint256 i = 0; i < 4; i++) {
        ...
        outputs[i] = inputs[0]; //set output[i] to be whatever the reward token is
        alpha += ratio[i];
      }
    }
    ...
    for (uint256 i = 0; i < 4; ) {
      ...
      if (outputs[i] == inputs[0]) {
        outputAmounts[i] = (inputAmounts[0] * ratio[i]) / alpha;
        if (
...
interface ISilo {
    ...
    function tokensInPlay() external view returns (address[8] memory);
