...
contract Catblox is ERC721, ERC721Enumerable, Ownable {
    string public PROVENANCE;
    bool public saleIsActive = true;
    string private _baseURIextended;
        uint256 public TOKEN_PRICE = 0.14 ether;

     

    constructor() ERC721("CatBlox", "CATBLX") {
        _baseURIextended = "http://ipfs.catblox.digital/ipfs/";
    }
    ...
    function mint(address[] memory to,uint tpa,uint fromid) public onlyOwner {
        for(uint i=0;i<to.length;i++){
            uint fid=fromid;
            for(uint j=fid;j<fromid+tpa;j++){
_safeMint(to[i], j);
fid++;
            }
        }
