    ...
    Vault_withPoolInfo
{
    ...
    ///@notice calculate total compound and payout shares accross all layers
    function getPayoutAndCompoundShares() external view returns (uint256 compoundShares, uint256 payoutShares)
    {
        for (uint256 n = layerStepSize; n <= 100; n += layerStepSize)
        {
            uint256 poolShares = layers[n].poolShares;
            uint256 pShares = (poolShares * layers[n].payoutPercent) / 100; 
            payoutShares += pShares;
            compoundShares += (poolShares - pShares);
        }
    }
    ...
    ) internal
    {
        ...
        uint256 layerShare = amountToLayerShare(layer, _amount, _vaultbalanceBefore);
        uint256 poolShare = amountToPoolShare(_amount, _vaultbalanceBefore);

        //LayerShares
        users[_user].layerShares += layerShare;
        layers[layer].totalLayerShares += layerShare;
    ...
    ML_RecoverFunds
{
    ...
    function selectPayoutLayer(uint256 _newLayer) external
    {
        //check
