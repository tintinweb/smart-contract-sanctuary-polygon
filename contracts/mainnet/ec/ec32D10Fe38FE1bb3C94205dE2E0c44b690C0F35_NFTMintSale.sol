...
contract NFTMintSale is Ownable {
    ...
    function init(bytes calldata data) public payable {
        (string memory symbol, string memory name, string memory baseURI, address royaltyReceiver_, uint16 royaltyRate_, uint64 maxMint_, uint32 beginTime_, uint32 endTime_, uint128 price_, IERC20 paymentToken_) = abi.decode(data, (string, string, string, address, uint16, uint64, uint32, uint32, uint128, IERC20));
