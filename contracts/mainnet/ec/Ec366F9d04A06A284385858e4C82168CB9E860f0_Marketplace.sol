...
contract Marketplace is BaseContract {
  ...
  function getOrder(address _nftContractAddress, uint _tokenId) external view returns (
    ...
    return(order.maker, order.taker, order.startingPrice, order.endingPrice, order.rentingDuration, order.activatedAt, order.paymentAddresses, order.orderType, order.status, order.weekNumber, order.weekPrepaidNumber);
  }
