...
/// This contract has NOT been audited and its usage
/// will likely result in loss of funds.
///
    ...
    TroopMetadata
{
    function init(
        ...
        approvalFollowModule = ApprovalFollowModule(approvalFollowModule_);
    ...
    function _authorizeUpgrade(address caller) internal view override {
        require(caller == address(timelock), "Only timelock may upgrade proxy");
    }
    ...
    function isMember(
        ...
        view
        virtual
        override(ITroop, TroopCommon, TroopLensGoverned, TroopVaultManager)
        returns (bool)
    {
        return
            TroopCommon.isMember(account) &&
            TroopLensGoverned.isMember(account) &&
            TroopVaultManager.isMember(account);
    }
    ...
    function proposeJoin(
        ...
        ITroop.JoinProposal memory joinProposal = ITroop.JoinProposal({
            account: account,
            initialInvestmentAmount: initialInvestmentAmount,
            initialMinSharesQuantity: initialMinSharesQuantity
        });
        string memory dataURI = buildJoinProposalMetadataURI(joinProposal);
        proposalId = governor.propose(targets, values, calldatas, dataURI);
        storeJoinProposalMetadataURI(proposalId, joinProposal);
        return proposalId;
    }
    ...
    function join(
        ...
        followData.sig = sig;
        lensHub.followWithSig(followData);
    ...
    function claimLensProfile(
        ...
        lensProfileId = lensProfileId_;
        lensHub.setDefaultProfile(lensProfileId_);
    }
...
import {GovernorLens} from "./lens/GovernorLens.sol";
import {IGovernorUpgradeable} from "@openzeppelin/contracts-upgradeable/governance/IGovernorUpgradeable.sol";
import {GovernorLensSettings} from "./lens/GovernorLensSettings.sol";
import {GovernorLensCountingSimple} from "./lens/GovernorLensCountingSimple.sol";
import {GovernorLensTimelockControl} from "./lens/GovernorLensTimelockControl.sol";
import {TimelockControllerUpgradeable} from "@openzeppelin/contracts-upgradeable/governance/TimelockControllerUpgradeable.sol";
import {GovernorLensVotes} from "./lens/GovernorLensVotes.sol";
import {GovernorLensVotesQuorumFraction} from "./lens/GovernorLensVotesQuorumFraction.sol";
...
contract TroopGovernor is
    OwnableUpgradeable,
    GovernorLens,
    GovernorLensSettings,
    GovernorLensCountingSimple,
    GovernorLensVotes,
    GovernorLensVotesQuorumFraction,
    GovernorLensTimelockControl
{
    ITroopLensGoverned public troop;
    ...
    function init(
        ...
        __Ownable_init();
        __GovernorLens_init("TroopGovernor", lensHubAddress_, lensProfileId_);
        __GovernorLensSettings_init(votingDelay_, votingPeriod_, 1 /** TODO */);
        __GovernorLensVotesQuorumFraction_init(
            quorumNumerator_ == 0 ? 10 : quorumNumerator_
        );
        __GovernorLensTimelockControl_init(timelock_);

        // Assumption: the deployer (and therefore owner) is a {Troop} contract
        troop = ITroopLensGoverned(msg.sender);
    }

    /// @notice Transfer hook to point `troop` variable to current owner
    /// @inheritdoc OwnableUpgradeable
    function _transferOwnership(address newOwner) internal virtual override {
        troop = ITroopLensGoverned(newOwner);
    ...
    function propose(
        ...
        public
        virtual
        override(IGovernorUpgradeable, GovernorLens)
        returns (uint256)
    {
        ...
        troop.onAfterPropose(proposalId, bytes(contentURI));
        return proposalId;
    }
    ...
    ) internal override(GovernorLens, GovernorLensTimelockControl) {
    ...
    function votingDelay()
        public
        view
        override(GovernorLensSettings, IGovernorUpgradeable)
        returns (uint256)
    {
    ...
    function proposalThreshold()
        public
        view
        override(GovernorLens, GovernorLensSettings)
        returns (uint256)
    {
        // TODO: Should this be in GovernorLensSettings?
    ...
    function quorum(
        ...
        public
        view
        override(IGovernorUpgradeable, GovernorLensVotesQuorumFraction)
        returns (uint256)
    {
    ...
    function state(
        ...
        public
        view
        override(GovernorLens, GovernorLensTimelockControl)
...
import {ITroop} from "../interfaces/ITroop.sol";
import {ITroopFactory} from "../interfaces/ITroopFactory.sol";
import {TroopToken} from "../TroopToken.sol";
import {TroopTimelock} from "../TroopTimelock.sol";
import {TroopGovernor} from "../TroopGovernor.sol";
...
/// @title TroopCommon
abstract contract TroopCommon is TroopStorage {
    ...
    TroopCommon
{
    ...
    function initGov(
        ...
        timelock = TroopTimelock(payable(address(timelockProxy)));
        timelock.init(
        ...
        governor = TroopGovernor(payable(address(troopGovernorProxy)));
        governor.init(
            address(lensHub),
            lensProfileId,
            timelock,
        ...
        timelock.grantRole(timelock.TIMELOCK_ADMIN_ROLE(), address(governor));
    ...
    function getFollowNFT() public returns (FollowNFT) {
        address followNFT_ = address(followNFT);
        if (followNFT_ == address(0)) {
            // Try to initialise (might be 0 again)
            followNFT_ = lensHub.getFollowNFT(lensProfileId);
            // Cache in storage regardless
            followNFT = FollowNFT(followNFT_);
        }
    ...
    function isMember(
        ...
        bool isFollowing = followNFT.balanceOf(account) > 0;
        // TODO: Re-enable this once we have our custom follow module deployed on mainnet
        // --- {
    ...
    function kick(address account) external onlyWhenReady onlyTimelock {
        // TODO: Re-enable this once we have our custom follow module deployed on mainnet
        // --- {
        // address[] memory addresses = new address[](1);
        // addresses[0] = account;
    ...
    function propose(
        ...
        return governor.propose(targets, values, calldatas, contentURI);
    }
...
abstract contract TroopVaultManager is TroopCommon {
    ...
    function initVault(
        ...
        enzymeFundDeployer = IFundDeployer(enzymeFundDeployer_);
        (enzymeFundComptrollerProxy, enzymeVaultProxy) = enzymeFundDeployer
            .createNewFund(
                address(this),
                token.name(),
                token.symbol(),
    ...
    function isMember(
        ...
        // Rely on TroopLensGoverned for member check
        ...
        returns (uint256 sharesReceived)
    {
        ...
        token.mint(account, sharesReceived);
    }
    ...
    function redeemInKindInternal(
        ...
        token.burnFrom(account, sharesQuantity);
...
abstract contract TroopTokenised is TroopCommon {
    ...
    function initToken(
        ...
        token = TroopToken(address(tokenProxy));
        token.init(name, symbol);
    }
}
...
/// @title TroopMetadata
abstract contract TroopMetadata is TroopCommon {
    using Strings for uint256;
    using Strings for address;

    string private constant CURLY_OPEN = "%7B"; // {
    string private constant CURLY_CLOSE = "%7D"; // }
    string private constant QUOTES = "%22"; // "

    function kv(
        bytes memory key,
        bytes memory value,
        bool excludeTrailingComma
    ...
    function kv(
        ...
        return kv(key, value, false);
    }

    /// @notice Produce on-chain metadata (as a data URI) describing a join
    ///     proposal. This is intended to be consumed as a Lens publication.
    ///     NB: The metadata produced by this function is *hashed* when
    ///     creating a governance proposal and is included in the computation
    ///     of the proposal id. So try to only include constant information
    ///     in the output of this function. e.g. Don't try to fetch the
    ///     proposer's default Lens handle, because it might change.
    /// @param joinProposal See {ITroop.JoinProposal}
    /// @return Metadata (as a data URI)
    function buildJoinProposalMetadataURI(
        ITroop.JoinProposal memory joinProposal
    ) public view returns (string memory) {
        uint256 metadataId = uint256(
            keccak256(abi.encode(address(this), joinProposal.account))
        );
        string memory proposerLensHandle = lensHub.getHandle(
            lensHub.defaultProfile(joinProposal.account)
        );
        bytes memory title = abi.encodePacked(
            "Join Proposal by ",
            proposerLensHandle
        );
        ERC20 denominationAsset = ERC20(
            IFundComptroller(enzymeFundComptrollerProxy).getDenominationAsset()
        );
        uint256 normalisedInvestmentAmount = joinProposal
            .initialInvestmentAmount / (10 ** denominationAsset.decimals());
        bytes memory content = abi.encodePacked(
            proposerLensHandle,
            " wants to join ",
            token.name(),
            " with an initial investment of ",
            normalisedInvestmentAmount.toString(),
            " ",
            denominationAsset.symbol()
        );
        // TODO: Optimise this; hardcode all the things
        bytes memory json = abi.encodePacked(
            CURLY_OPEN,
            kv("version", "2.0.0"),
            kv("locale", "en"),
            kv("mainContentFocus", "ARTICLE"),
            kv("metadata_id", bytes(metadataId.toHexString())),
            kv("name", title),
            kv("content", content),
            '"attributes":[]',
            CURLY_CLOSE
    ...
    /// @notice Record a join proposal
    /// @param proposalId Proposal ID of the governance proposal
    function storeJoinProposalMetadataURI(
        uint256 proposalId,
        ITroop.JoinProposal memory joinProposal
    ) public {
        joinProposals[proposalId] = joinProposal;
    }

    /// @notice Get the metadata data URI for the join proposal
    /// @param proposalId Propose ID of the governance proposal
    function joinProposalMetadataURI(
        uint256 proposalId
    ) public view returns (string memory) {
        ITroop.JoinProposal memory joinProposal = joinProposals[proposalId];
        return buildJoinProposalMetadataURI(joinProposal);
    }
}
 ...
 */
abstract contract GovernorLens is
    Initializable,
    ContextUpgradeable,
    ERC165Upgradeable,
    EIP712Upgradeable,
    ...
    IERC1155ReceiverUpgradeable
{
     ...
     */
    function __GovernorLens_init(
        string memory name_,
        address lensHubAddress,
        uint256 lensProfileId_
    ) internal onlyInitializing {
        __EIP712_init_unchained(name_, version());
        __GovernorLens_init_unchained(name_, lensHubAddress, lensProfileId_);
    }

    function __GovernorLens_init_unchained(
        string memory name_,
        address lensHubAddress,
        uint256 lensProfileId_
    ) internal onlyInitializing {
 ...
 * _Available since v4.3._
 */
abstract contract GovernorLensCountingSimple is GovernorLens {
    /**
 ...
 */
abstract contract GovernorLensSettings is Initializable, GovernorLens {
     ...
     */
    function __GovernorLensSettings_init(
        uint256 initialVotingDelay,
        uint256 initialVotingPeriod,
        uint256 initialProposalThreshold
    ) internal onlyInitializing {
        __GovernorLensSettings_init_unchained(
            initialVotingDelay,
            initialVotingPeriod,
    ...
    function __GovernorLensSettings_init_unchained(
        uint256 initialVotingDelay,
        uint256 initialVotingPeriod,
        uint256 initialProposalThreshold
    ) internal onlyInitializing {
 ...
 * _Available since v4.3._
 */
abstract contract GovernorLensTimelockControl is
    ...
    GovernorLens
{
     ...
     */
    function __GovernorLensTimelockControl_init(
        TimelockControllerUpgradeable timelockAddress
    ) internal onlyInitializing {
        __GovernorLensTimelockControl_init_unchained(timelockAddress);
    }

    function __GovernorLensTimelockControl_init_unchained(
        TimelockControllerUpgradeable timelockAddress
    ) internal onlyInitializing {
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(IERC165Upgradeable, GovernorLens)
        returns (bool)
    {
        return
...
import {IERC721Enumerable} from "@openzeppelin/contracts/token/ERC721/extensions/IERC721Enumerable.sol";
import {GovernorLens} from "./GovernorLens.sol";
import {AllowlistedFollowModule} from "./AllowlistedFollowModule.sol";
import {FollowNFT} from "@aave/lens-protocol/contracts/core/FollowNFT.sol";

/// @dev Extension of {Governor} for voting weight extraction from a Lens profile's FollowNFT
abstract contract GovernorLensVotes is GovernorLens {
    /// @notice Read voting power from this Lens profile's built-in gov mechanism, also checks
    ///     an AllowlistedFollowModule if the account is approved, if one is attached to the profile.
    /// @inheritdoc GovernorLens
    function _getVotes(
        address account,
        uint256 blockNumber,
        bytes memory /*params*/
    ) internal view virtual override returns (uint256) {
        address followModule = lensHub.getFollowModule(lensProfileId);
        address followNFT_ = followNFT;
        require(
            followNFT_ != address(0),
            "GovernorLensVotes/FollowNFT not initialised"
        );
        if (followModule != address(0)) {
            bool hasFollowNFT = IERC721Enumerable(followNFT_).balanceOf(
                account
            ) > 0;
            // TODO: Re-enable this once we have our custom follow module deployed on mainnet
            // --- {
            // bool isAllowlistedMember = hasFollowNFT &&
            //     AllowlistedFollowModule(followModule).isFollowing(
            //         lensProfileId,
            //         account,
            //         IERC721Enumerable(followNFT_).tokenOfOwnerByIndex(
            //             account,
            //             0
            //         )
            //     );
            // if (!isAllowlistedMember) {
            //     return 0;
            // }
            // } ---
            // TODO: Remove this
        ...
        return
            FollowNFT(followNFT_).getPowerByBlockNumber(account, blockNumber);
    }
...
abstract contract GovernorLensVotesQuorumFraction is GovernorLensVotes {
     ...
     */
    function __GovernorLensVotesQuorumFraction_init(
        uint256 quorumNumeratorValue
    ) internal onlyInitializing {
        __GovernorLensVotesQuorumFraction_init_unchained(quorumNumeratorValue);
    }

    function __GovernorLensVotesQuorumFraction_init_unchained(
        uint256 quorumNumeratorValue
    ) internal onlyInitializing {
 ...
 * @author Lens Protocol
 *
 * @notice This follow module only is similar to ApprovalFollowModule, but also allows the owner of the Lens profile
 * to "block" certain addresses. Does NOT inherit FollowValidatorFollowModuleBase, instead implements its own isFollowing
 * logic.
 */
contract AllowlistedFollowModule is ModuleBase, IFollowModule {
    // We use a triple nested mapping so that, on profile transfer, the previous approved address list is invalid;
    mapping(address => mapping(uint256 => mapping(address => bool)))
        internal _allowedByProfileByOwner;

    constructor(address hub) ModuleBase(hub) {}

    /**
     * @notice Similar to the logic implemented in FollowValidatorFollowModuleBase, but returns `false`
    ...
    function isFollowing(
        ...
        address owner = IERC721(HUB).ownerOf(profileId);
        if (!_allowedByProfileByOwner[owner][profileId][follower]) {
            return false;
        }
    ...
    function approve(
        ...
        uint256 addressesLength = addresses.length;
        for (uint256 i = 0; i < addressesLength; ) {
            _allowedByProfileByOwner[owner][profileId][
                addresses[i]
            ] = toApprove[i];
            unchecked {
                ++i;
        ...
        returns (bytes memory)
    {
        ...
        if (data.length > 0) {
            ...
            uint256 addressesLength = addresses.length;
            for (uint256 i = 0; i < addressesLength; ) {
                _allowedByProfileByOwner[owner][profileId][addresses[i]] = true;
                unchecked {
                    ++i;
                }
            }
     ...
     * @dev Processes a follow by:
     *  1. Validating that the follower has been approved for that profile by the profile owner
     *  2. Validating that the follower is not already following profileId
     */
    function processFollow(
        address follower,
        uint256 profileId,
        ...
        if (followNFT != address(0)) {
            // 1. FollowNFT contract is only deployed just before the 1st follow.
            // 2. A FollowNFT is minted *before* this function is called.
            isAlreadyFollowing = IERC721(followNFT).balanceOf(follower) > 1;
        }
        if (
            isAlreadyFollowing ||
            !_allowedByProfileByOwner[owner][profileId][follower]
        ) revert Errors.FollowNotApproved();
    }
    ...
    function isApproved(
        ...
        return _allowedByProfileByOwner[profileOwner][profileId][toCheck];
    }
    ...
    function isApprovedArray(
        ...
        uint256 toCheckLength = toCheck.length;
        for (uint256 i = 0; i < toCheckLength; ) {
            approved[i] = _allowedByProfileByOwner[profileOwner][profileId][
                toCheck[i]
            ];
            unchecked {
                ++i;
...
contract TroopTimelock is TimelockControllerUpgradeable {
    // Reserved
    // slither-disable-next-line shadowing-state
...
/// @notice TimelockController from OZ, not actually upgradeable, but intended
///     to be deployed behind a proxy.
contract TroopToken is ERC20VotesUpgradeable {
