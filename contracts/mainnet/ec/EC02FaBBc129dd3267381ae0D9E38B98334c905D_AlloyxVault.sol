...
contract AlloyxVault is IAlloyxVault, AdminUpgradeable {
  ...
  function updateUsdcValuesAndGains(uint256 _increaseAmount, uint256 _decreaseAmount) internal {
    (uint256 totalInvestorUsdcValue, uint256 permanentStakerGain, uint256 regularStakerGain, uint256 protocolGain) = getTotalInvestorUsdcValueAndAdditionalGains();
    ...
    preProtocolFee = preProtocolFee.add(protocolGain);
   ...
   */
  function withdraw(uint256 _tokenAmount) external isWhitelisted notPaused nonReentrant {
   ...
   */
  function rebalance() external onlyAdmin nonReentrant {
    updateUsdcValuesAndGains(0, 0);
    uint256 usdcValue = config.getUSDC().balanceOf(address(this));
    require(usdcValue > preProtocolFee.add(preRegularStakerGain).add(prePermanentStakerGain), "not enough usdc");
    uint256 amountToInvest = usdcValue.sub(preProtocolFee).sub(preRegularStakerGain).sub(prePermanentStakerGain);
   ...
   * @return the additional gain from last checkpoint for protocol
   */
    ...
    )
  {
    ...
    if (totalValue > preTotalUsdcValue) {
      ...
      uint256 protocolGain = config.getProtocolFeeProportion().mul(interest).div(10000);
      ...
      uint256 investorGain = interest.sub(protocolGain).sub(permanentStakerGain).sub(regularStakerGain);
      uint256 totalInvestorUsdcValue = investorGain.add(preTotalInvestorUsdcValue);
      return (totalInvestorUsdcValue, permanentStakerGain, regularStakerGain, protocolGain);
    } else {
      uint256 loss = preTotalUsdcValue.sub(totalValue);
      uint256 totalInvestorUsdcValue = preTotalInvestorUsdcValue.sub(loss);
      return (totalInvestorUsdcValue, 0, 0, 0);
    }
  }
  ...
  /**
   * @notice Get total the protocol fee
   */
  function getProtocolFeeInVault() public view returns (uint256) {
    (, , , uint256 protocolFee) = getTotalInvestorUsdcValueAndAdditionalGains();
    return protocolFee.add(preProtocolFee);
  }
   ...
   */
  function collectProtocolFee() external override onlyManager {
    updateUsdcValuesAndGains(0, 0);
    _transferERC20(config.usdcAddress(), config.treasuryAddress(), preProtocolFee);
    preTotalUsdcValue = preTotalUsdcValue.sub(preProtocolFee);
    preProtocolFee = 0;
  }
   ...
   */
  function setComponents(Component[] memory _components) internal {
