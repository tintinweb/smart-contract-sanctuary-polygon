...
contract NFTMarketplace {
  ...
  function make_auction(uint duration, uint256 pric, uint nftid, address _TokenAddress, uint _At) public payable {
      uint256 price = pric;
      nftCollection.transferFrom(msg.sender, address(this), nftid);
      AuctionCount++;
      uint endAt = block.timestamp + (duration * 1 minutes);
  ...
  function make_bid(uint AuctionCoun) public payable {
     ...
     // require(_Auctiondata.auctionid == AuctionCoun, 'The Auction must exit' );
      ...
      _Auctiondata.duration = block.timestamp +  (_Auctiondata.init_duration * 1 minutes);
  ...
  function fillOffer(uint _offerId) public payable {
    ...
    userFunds[_offer.user] += msg.value - (msg.value * 100 / 10000);
    userFunds[creator] += msg.value * 100 / 10000;
  ...
  function End_auction(uint _AuctionCount) public {
    auctiondata storage _Auctiondata = Auctions[_AuctionCount];
    // require(_Auctiondata.auctionid == _AuctionCount, 'The Auction must exit' );
    require(_Auctiondata.nft_owner == msg.sender, 'The offer can only be canceled by the owner');
    ...
    require(_Auctiondata.completed == false, 'An Auction cannot be Ended twice');
     if(_Auctiondata.nft_owner == _Auctiondata.highest_address){
      ...
      nftCollection.transferFrom(address(this), _Auctiondata.highest_address, _Auctiondata.nftId);
      _Auctiondata.completed = true; 
      userFunds[_Auctiondata.nft_owner] += (_Auctiondata.current_price - ((_Auctiondata.current_price * 100) / 10000));
      userFunds[creator] += ((_Auctiondata.current_price * 100) / 10000);
      
      }
   
  }
  function End_dweb3auction(uint _AuctionCount) public {
    auctiondata storage _Auctiondata = Auctions[_AuctionCount];
    // require(_Auctiondata.auctionid == _AuctionCount, 'The Auction must exit' );
    require(_Auctiondata.nft_owner == msg.sender, 'The offer can only be canceled by the owner');
    require(_Auctiondata.duration < block.timestamp, 'The auctions is still on-going');
    ...
    _Auctiondata.completed = true;
     if(_Auctiondata.nft_owner == _Auctiondata.highest_address){
         ...
         nftCollection.transferFrom(address(this), _Auctiondata.highest_address, _Auctiondata.nftId);
         TokenTracker[_Auctiondata.TokenAddress][_Auctiondata.nft_owner] += (_Auctiondata.current_price - (_Auctiondata.current_price * 100 / 10000));
         TokenTracker[_Auctiondata.TokenAddress][creator] += (_Auctiondata.current_price * 100) / 10000;
      }
    
    if(_Auctiondata.TokenAddress != address(0)){
