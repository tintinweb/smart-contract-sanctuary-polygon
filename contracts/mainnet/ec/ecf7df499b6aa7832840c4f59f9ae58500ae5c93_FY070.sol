...
contract Y070 is ERC20, ERC20Burnable, Ownable {
  ...
  constructor() payable ERC20("Y070", "Y070")      {
...
uint public tiempoJ=180;              uint private constant MAX_TIEMPOJ = 86300;        event changeTiempoJ(uint tiempoJ);
...
uint internal num;    uint internal dia;

function myJornada() public hasFarm returns (uint d, uint verAviso)
{
    comienzo[msg.sender] = inicio[msg.sender] + tiempoJ*(dia+1);

if(block.timestamp >= comienzo[msg.sender])             {           
uint    numerA = uint( keccak256( abi.encode(block.timestamp, msg.sender, num) ) ) % 18;
    numerA < 1 ? numerA=numerA+5: numerA;
    horaAviso[msg.sender]=comienzo[msg.sender]+(numerA*3);
    dia=dia+1;      dinero[msg.sender]=0;
              }
if (dia>0){
    if(block.timestamp >= horaAviso[msg.sender] && dinero[msg.sender]==0)   {
    mostrarAviso[msg.sender]=1;  }     else { mostrarAviso[msg.sender]=0;  }
}
return (dia, mostrarAviso[msg.sender]);                                          
}
