...
contract MYSHARE is ERC20Burnable, Operator {
    ...
    address public communityFund; // treasuryFund
    ...
    ) public ERC20("xfactory", "xfactory") {
        require(
            _initialMint <= 877 ether,
            "xfactory: initial mint is over 10% allocated"
        );
        require(
            _vestingDuration <= 180 days,
            "Vested time: MAX, reassessment due at 180 days at most."
        );
        _mint(msg.sender, _initialMint);
        startTime = _startTime;
        vestingDuration = _vestingDuration; // 120 days for dev shares & treasuryFund mints 10% of their allocation initially during deployment.
        endTime = startTime + vestingDuration;

        communityFundLastClaimed = startTime;
        devFundLastClaimed = startTime;

        communityFundAllocation = _communityFundAllocation; //  remainder of treasuryFund shares allocation.
        devFundAllocation = _devFundAllocation; // devFund 777 shares
    ...
    function distributeReward(
        ...
        uint256 _farmingPoolAllocation
