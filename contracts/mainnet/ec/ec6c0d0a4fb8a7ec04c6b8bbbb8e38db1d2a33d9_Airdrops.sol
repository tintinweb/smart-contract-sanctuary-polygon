...
contract Airdrops is RegisterableUpgradeableBase {
    ...
    */
    function initialize(address registry_, bytes memory signature_) public initializer {
        __RegisterableUpgradeableBase_init(Constants.AIRDROPS, registry_, signature_);
    }
    /**
...
abstract contract RegisterableUpgradeableBase is UpgradeableBase {
    ...
    */
    function __RegisterableUpgradeableBase_init(bytes32 key_, address registry_, bytes memory signature_) internal {
        __UpgradeableBase_init();
        registry = IRegistry(registry_);
        registry.register(key_, signature_);
    }
    /**
    @notice Calls init
    @param key_ Key to register with
    ...
    @param signature_ Used to validate the whether the caller can register
    */
    function __RegisterableUpgradeableBase_init_unchained(bytes32 key_, address registry_, bytes memory signature_) internal {
        __RegisterableUpgradeableBase_init(key_, registry_, signature_);
    }
    /**
...
abstract contract UpgradeableBase is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    */
    function _isUpgrader(bytes memory signature_, bytes memory encodedArgs_) internal view returns (address) {
        bytes32 msgHash = keccak256(encodedArgs_);
        bytes32 signedHash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", msgHash));
