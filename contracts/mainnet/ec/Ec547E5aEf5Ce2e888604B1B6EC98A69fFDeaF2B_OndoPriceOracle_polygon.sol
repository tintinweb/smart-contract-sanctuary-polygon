...
contract OndoPriceOracle_polygon is Ownable {
  mapping(address => uint256) public tokenToPrice;
  mapping(address => address) public fTokenToUnderlying;

  Oracle public oracle = Oracle(0xd7cB92af677B9c9CA70229ea99bE741Eb60C6B0D);
  ...
  constructor(
    ...
    tokenToPrice[_fDaiAddress] = 1000000000000000000;
    // fTokenToUnderlying[_fDaiAddress] = _daiAddress;
  }
  ...
  function getUnderlyingPrice(address fToken) public view returns (uint256) {
    ...
    if (underlyingAddress != address(0)) {
      return oracle.getAssetPrice(underlyingAddress);
    } else {
