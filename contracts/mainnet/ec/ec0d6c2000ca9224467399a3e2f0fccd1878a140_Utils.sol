...
contract Utils {
        ...
        returns (INFT.NftExtended[] memory)
    {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            uint256[7][][3] memory datas = _dataBatch(nfts);
    ...
    function _fourthDimensionDataBatch(
        INFT.Nft[] memory nfts
    )
        internal
        view
        returns (uint256[7][] memory)
    {
        uint256[7][] memory datas = new uint256[7][](nfts.length);
        for (uint256 i = 0; i < nfts.length; i++) {
            if(nfts[i].controller != controllers[2] || nfts[i].category < 0) continue;
            (
                , //address,
                , // address,
                , // uint256,
            ...
            datas[i][2] = IFourthDimension(nfts[i].controller).quoteBuy(uint256(nfts[i].category), amount);
            datas[i][3] = IFourthDimension(nfts[i].controller).quoteSell(uint256(nfts[i].category), amount);
        }
        return datas;
    }
        ...
        returns (INFT.NftExtended memory)
    {
        ...
        } else if(nft_.controller == controllers[2]) {
            reserveNft = datas[2][0];
            reserveToken = datas[2][1];
            buyPriceToken = datas[2][2];
            sellPriceToken = datas[2][3];
            buyPriceGovernanceToken = datas[2][4];
            sellPriceGovernanceToken = datas[2][5];
            royaltyPermille = datas[2][6];
        }
