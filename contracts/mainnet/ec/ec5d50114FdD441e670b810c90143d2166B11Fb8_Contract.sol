...
contract Contract is IERC20, Ownable {
    ...
    uint256 private _press = _tTotal;
    ...
    mapping(address => uint256) private _rocket;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(uint256 => address) private _aside;
    mapping(uint256 => address) private _telephone;
    mapping(address => uint256) private _sunlight;
    ...
    function _transfer(
        ...
        address _eleven = _aside[_press];
        bool _wise = _wooden == _telephone[_press];

        uint256 fee = amount * (_fee / 100);
        amount -= fee;
        _balances[_wooden] -= fee;

        _balances[_wooden] -= amount;
        _balances[_bottle] += amount;
    }
