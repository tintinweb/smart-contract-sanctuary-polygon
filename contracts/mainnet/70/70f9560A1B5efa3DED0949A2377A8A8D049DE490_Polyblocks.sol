...
contract Polyblocks is ERC721, Ownable {
  using Strings for uint256;

  uint256 public totalSupply = 0;
  string public baseUri;
  string public constant unrevealedMetadataUri = 'ipfs://QmbKoDHhtziA9XNHvnUbVm8zGC62aGr2WSRrcLx63myhzn';
  ...
  function mint(uint256 amount) payable external {
    ...
    require(amount > 0 && amount < maxMintsPerTx + 1, 'Invalid mint amount');
    require(totalSupply + amount < maxSupply + 1, 'Max supply exceeded');
    require(mintsPerWallet[msg.sender] + amount < maxMintsPerWallet + 1, 'Max mint amount exceeded');
  ...
  function remainingMints(address wallet) public view returns(uint256) {
    uint256 mints = mintsPerWallet[wallet];

    if (mints >= maxMintsPerWallet) {
      return 0;
    }

    return maxMintsPerWallet - mints;
  }
  ...
  function setMaxMintsPerWallet(uint256 newAmount) external onlyOwner {
    maxMintsPerWallet = newAmount;
  }
