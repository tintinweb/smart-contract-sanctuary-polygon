...
contract CurvePortalOut is PortalBaseV1 {
    ...
    /// removeUnderlying A boolean value specifying whether to withdraw the unwrapped version of intermediateToken
    ...
    /// @notice Removes the intermediateToken from the pool using the correct interface
    /// @param sellToken The curve pool token address
    /// @param sellAmount The quantity of sellToken to Portal out
    /// @param buyToken The ERC20 token address to buy (address(0) if network token)
    /// @param poolData Encoded pool data including the following:
    ...
    function _remove(
        ...
        ) = abi.decode(poolData, (address, uint256, bool, bool));
