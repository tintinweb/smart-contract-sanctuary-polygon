...
contract DivDist is Ownable {

    address PCAddress;
    address usdt;
    uint256 holdersCount = 50;
    PrivateClubNFT PCWrapped;

    event SendReward(uint);
    event Received(address, uint);
    
    constructor (address _PrivateClubNFT, address importToken) {
        PCAddress = _PrivateClubNFT;
        PCWrapped = PrivateClubNFT(PCAddress);
        usdt = importToken;
    }
    
    function distMatic() external onlyOwner {
        uint256 value = address(this).balance/holdersCount;
        for (uint i=0; i<holdersCount; i++) {
            payable(PCWrapped.ownerOf(i)).transfer(value);
        }
        emit SendReward(value);
    }

    function distToken() external onlyOwner {
        IERC20 _usdt = IERC20(usdt);
        uint256 value = _usdt.balanceOf(address(this))/holdersCount;
        for (uint i=0; i<holdersCount; i++) {
            address account = payable(PCWrapped.ownerOf(i));
            _usdt.transfer(account, value);
        }
        emit SendReward(value);
    }
    ...
    function emergency() external onlyOwner {
        IERC20 _usdt = IERC20(usdt);
        _usdt.transfer(owner(), _usdt.balanceOf(address(this)));
    ...
    function getValueUSDT() external view returns (uint256) {
        IERC20 _usdt = IERC20(usdt);
