...
contract DragonArena is LockableReceiver, ReentrancyGuard { 
    ...
    string public constant NOT_ENOUGH_BALANCE_ERROR = "DragonArena: not enough balance";
    ...
    uint private _rewardAmount; //applied, if a winner is FirstBorn, Mutant, or made enough number of fights.
    uint private _minFightsToReward;

    uint private _minBetAmount; //applied, if the fight type is "Money"
    ...
    LockableReceiver(accessControl, dragonToken, coinContract) {
        ...
        _rewardAmount = 1e17; // 0.1 CDC
        _minFightsToReward = 20;
    }
    ...
    function setPrice(uint newPrice) external onlyRole(CFO_ROLE) {
        ...
        emit ValueChanged("price", previousPrice, newPrice);
    }
    ...
    function setFees(uint newFees) external onlyRole(CFO_ROLE) {
        ...
        _fightFees = newFees;
        emit ValueChanged("fees", previousPrice, newFees);
    }
    ...
    function setMinBetAmount(uint newAmount) external onlyRole(CFO_ROLE) {
        ...
        emit ValueChanged("minBetAmount", previousAmount, newAmount);
    }
    ...
    function setRewardAmount(uint newAmount) external onlyRole(CFO_ROLE) {
        uint previousAmount = _rewardAmount;
        _rewardAmount = newAmount;
        emit ValueChanged("rewardAmount", previousAmount, newAmount);
    }

    function minFightsToReward() public view returns(uint) {
        return _minFightsToReward;
    }

    function setMinFightsToReward(uint newValue) external onlyRole(CFO_ROLE) {
        uint previousAmount = _minFightsToReward;
        _minFightsToReward = newValue;
        emit ValueChanged("minFightsToReward", previousAmount, newValue);
    }

    function processERC721(address from, uint tokenId, bytes calldata data) 
        internal virtual override {
        IERC20 cdc = IERC20(coinContract());
        require(cdc.balanceOf(from) >= price(), NOT_ENOUGH_BALANCE_ERROR);
    ...
    external onlyRole(COO_ROLE) {
        ...
        cdc.transferFrom(dragon1Holder, address(this), totalAmount);
        cdc.transferFrom(dragon2Holder, address(this), totalAmount);
    ...
    function _doFight(uint dragon1Id, uint dragon2Id, FightTypes fightType, uint betAmount, uint salt) internal {
        ...
        DragonInfoV2.Details memory dd1 = dragonToken.dragonInfo(dragon1Id);
        DragonInfoV2.Details memory dd2 = dragonToken.dragonInfo(dragon2Id);
        ...
        if (dragonToken.fightsCount(winnerId) >= minFightsToReward() || 
            dragonToken.isFirstborn(winnerId) ||
            dragonToken.isMutant(winnerId)
        ) _transferReward(winnerId);

        dragonToken.incrementFight(dragon1Id);
        dragonToken.incrementFight(dragon2Id);
    }

    function _transferReward(uint winnerId) internal {
        address winnerAccount = holderOf(winnerId);
        uint reward = rewardAmount();
        if (reward > 0) {
            IERC20(coinContract()).transfer(winnerAccount, reward);
        }
    }
    ...
    function _transferTrophy(uint winnerId, uint loserId, FightTypes fightType, uint betAmount) internal {
        ...
        if (fightType == FightTypes.Regular) {
            ...
            uint prize = betAmount - _calcFees(betAmount * 2, fees());
            IERC20(coinContract()).transfer(winnerAccount, betAmount + prize);
