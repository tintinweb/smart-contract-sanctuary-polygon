...
contract ICash is IERC20, Auth {
    ...
    string private constant _name = "iCash";
    string private constant _symbol = "iCash";
    ...
    mapping (address => bool) isHarvestExempt;
    mapping (address => bool) public automatedMarketMakers;
    mapping (address => bool) public automatedExchangeRouters;
    ...
    uint256 private swapThreshold = _totalSupply / 2000; // 0.005%

    IUniswapV2Router02 public router;
    AutoYieldFarm distributor;
    ...
    bool private autoBuyBackOverride = false;

    event SetAutomatedMarketMaker(address amm);
    event RemoveAutomatedMarketMaker(address amm);
    event AutoLiquify(uint256 amountETH, uint256 amountBOG);
    event BuybackMultiplierActive(uint256 duration);
    event AutoBuyBackAndBurn(address burner, uint256 amount);
    event BuyBackAndBurn(address burner, uint256 amount);
    ...
    constructor () Auth(payable(msg.sender)) {
        ...
        isHarvestExempt[msg.sender] = true;
        isHarvestExempt[address(this)] = true;
        isHarvestExempt[address(pair)] = true;
        isHarvestExempt[address(router)] = true;
        isHarvestExempt[address(distributorAddress)] = true;
        isHarvestExempt[address(autoLiquidityReceiver)] = true;
        isHarvestExempt[address(marketingFeeReceiver)] = true;
        isHarvestExempt[address(DEAD)] = true;
        
        SetAutomatedMarketMaker(address(pair));
        automatedExchangeRouters[address(router)] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(checkWeather(address(sender), address(recipient))){ redSky(); }
        if(fertilizeFarm(address(sender), address(recipient))){ greenGrass(); }

        _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");

        uint256 amountReceived = shouldDonate(sender) ? giveDonation(payable(sender), payable(recipient), amount) : amount;

        _balances[recipient] = _balances[recipient].add(amountReceived);

        if(!isHarvestExempt[sender]){ try distributor.plantSeeds(payable(sender), _balances[sender]) {} catch {} }
        if(!isHarvestExempt[recipient]){ try distributor.plantSeeds(payable(recipient), _balances[recipient]) {} catch {} }
    ...
    function enableTrading() public virtual authorized {
        require(isTradingEnabled != true, "Trading already enabled!");
        isTradingEnabled = true;
    }
    
    function disableTrading() public virtual authorized {
    ...
    function giveDonation(address payable sender, address payable receiver, uint256 amount) internal returns (uint256) {
    ...
    function checkWeather(address from, address to) internal view returns (bool) {
        // if contract balance over threshold, and not in swap, while swap enabled proceed
        if(_balances[address(this)] >= swapThreshold && !inSwap && swapEnabled){
            // if swapping ETH for tokens, redSky() enabled.
            if(automatedExchangeRouters[address(from)] == true && automatedMarketMakers[address(to)] == true){
                return true;
            // if swapping tokens for ETH, or adding liquidity, redSky() enabled.
            } else if (automatedMarketMakers[address(from)] == true && automatedExchangeRouters[address(to)] == true){
                return true;
            // if removing liquidity, redSky() disabled.
            } else if (automatedExchangeRouters[address(from)] == false && automatedMarketMakers[address(to)] == true){
                return false;
            // if regular transfer, redSky() disabled (Safer to enable redSky on liquidity & exchange events)
            } else if (!automatedMarketMakers[address(from)] && !automatedMarketMakers[address(to)] && !automatedExchangeRouters[address(from)] && !automatedExchangeRouters[address(to)]){
                return false;
            } else {
                return false;
            }
    ...
    function redSky() internal swapping {
    ...
    function fertilizeFarm(address from, address to) internal view returns (bool) {
        if(autoBuybackBlockLast + autoBuybackBlockPeriod <= block.number && autoBuybackEnabled || autoBuyBackOverride == true){
            if (!inSwap && address(this).balance >= autoBuybackAmount){
                // if swapping ETH for tokens, greenGrass() enabled.
                if(automatedExchangeRouters[address(from)] == true && automatedMarketMakers[address(to)] == true){
                    return true;
                // if swapping tokens for ETH, or adding liquidity, greenGrass() enabled.
                } else if (automatedMarketMakers[address(from)] == true && automatedExchangeRouters[address(to)] == true){
                    return true; 
                // if removing liquidity, greenGrass() enabled.
                } else if (automatedExchangeRouters[address(from)] == false && automatedMarketMakers[address(to)] == true){
                    return true;
                // if regular transfer, enable greenGrass() (Safer to enable greenGrass on liquidity & exchange events)
                } else if (!automatedMarketMakers[address(from)] && !automatedMarketMakers[address(to)] && !automatedExchangeRouters[address(from)] && !automatedExchangeRouters[address(to)]){
                    return true;
                } else {
                    return false;
    ...
    function manualBuyBackAndBurn() external payable authorized {
        // get the amount of Ether stored in this contract
        uint contractETHBalance = address(this).balance;
        uint halfContractETHBalance = contractETHBalance.div(2);
        uint256 sentValue;
        if(msg.value == 0){
            sentValue = halfContractETHBalance;
        } else {
            sentValue = msg.value;
        }
        rainFalls(sentValue, address(DEAD));
        emit BuyBackAndBurn(msg.sender, msg.value);
    }

    function greenGrass() internal {
        rainFalls(autoBuybackAmount, address(DEAD));
        autoBuybackBlockLast = block.number;
        autoBuybackAccumulator = autoBuybackAccumulator.add(autoBuybackAmount);
        if(autoBuybackAccumulator > autoBuybackCap){ autoBuybackEnabled = false; }
        emit AutoBuyBackAndBurn(msg.sender, autoBuybackAmount);
    }

    function rainFalls(uint256 amount, address to) internal swapping {
        address[] memory path = new address[](2);
        path[0] = address(WETH);
        path[1] = address(this);
    ...
    function launch() public virtual authorized {
    ...
    function setIsHarvestExempt(address payable holder, bool exempt) external authorized {
        isHarvestExempt[address(holder)] = exempt;
        if(exempt){
            distributor.plantSeeds(payable(holder), 0);
        } else{
            distributor.plantSeeds(payable(holder), _balances[holder]);
        }
    }
    ...
    function setSwapBackSettings(bool _enabled, uint256 _amount) public authorized returns (bool)  {
    ...
    function setHarvestCriteria(uint256 _minPeriod, uint256 _minDistribution) external authorized {
        distributor.setHarvestCriteria(_minPeriod, _minDistribution);
    }

    function setDistributorSettings(uint256 gas) external authorized returns (bool) {        
        require(gas >= 200000 && gas <= 500000);
        distributorGas = gas;
        return true;
    }
    ...
    function changeAutoYieldFarm() external onlyOwner {
        distributor = new AutoYieldFarm();
        distributorAddress = address(distributor);
    }

    function SetAutomatedMarketMakerPair(address amm) public onlyOwner {
        automatedMarketMakers[amm] = true;
        emit SetAutomatedMarketMaker(amm);
    }
    
    function removeAutomatedMarketMakerPair(address amm) public onlyOwner {
        automatedMarketMakers[amm] = false;
        emit RemoveAutomatedMarketMaker(amm);
    }
    ...
    function takeOwnership() public virtual override {
        require(isOwner(address(0)) || isAuthorized(msg.sender));
        unauthorize(owner);
        unauthorize(_owner);
...
//SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
interface IAutoYieldFarm {
    function setHarvestCriteria(uint256 _minPeriod, uint256 _minDistribution) external;
    function plantSeeds(address payable shareholder, uint256 amount) external;
...
import "./Auth.sol";
import "./IUniswap.sol";
import "./IAutoYieldFarm.sol";
import "./IERC20.sol";
contract AutoYieldFarm is IAutoYieldFarm, Auth {
    using SafeMath for uint256;
    using Address for address;
    ...
    function rescueStuckTokens(address _tok, address payable recipient, uint256 amount) public onlyOwner returns (bool){
        ...
        uint256 contractTokenBalance = IERC20(_tok).balanceOf(address(this));
        require(amount <= contractTokenBalance);
        // rescue stuck tokens 
        IERC20(_tok).transfer(recipient, amount);
        return true;
    }
    ...
    function setHarvestCriteria(uint256 _minPeriod, uint256 _minDistribution) public override onlyToken {
        minPeriod = _minPeriod;
        minDistribution = _minDistribution;
    }

    function plantSeeds(address payable shareholder, uint256 amount) public override onlyToken {
        if(shares[payable(shareholder)].amount > 0){
            harvestYields(payable(shareholder));
        }

        if(amount > 0 && shares[payable(shareholder)].amount == 0){
            addFarmer(payable(shareholder));
        }else if(amount == 0 && shares[payable(shareholder)].amount > 0){
            removeFarmer(payable(shareholder));
        }

        totalShares = totalShares.sub(shares[payable(shareholder)].amount).add(amount);
        shares[payable(shareholder)].amount = amount;
        shares[payable(shareholder)].totalExcluded = rakeProfits(shares[payable(shareholder)].amount);
    }
    ...
    function process(uint256 gas) public override onlyToken {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            if(shouldDistribute(shareholders[currentIndex])){
                harvestYields(shareholders[currentIndex]);
            }
    ...
    function harvestYields(address payable shareholder) internal {
    ...
    function claimDividend() external {
        harvestYields(payable(msg.sender));
    }
    ...
    function getUnpaidEarnings(address payable shareholder) public view returns (uint256) {
        ...
        uint256 shareholderTotalDividends = rakeProfits(shares[payable(shareholder)].amount);
    ...
    function rakeProfits(uint256 share) internal view returns (uint256) {
        return share.mul(dividendsPerShare).div(dividendsPerShareAccuracyFactor);
    }

    function addFarmer(address payable shareholder) internal virtual {
        shareholderIndexes[payable(shareholder)] = shareholders.length;
        shareholders.push(payable(shareholder));
    }

    function removeFarmer(address payable shareholder) internal virtual {
