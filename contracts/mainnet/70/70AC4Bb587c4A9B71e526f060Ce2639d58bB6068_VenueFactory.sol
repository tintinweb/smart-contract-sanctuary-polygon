...
contract VenueFactory is IVenueFactory, OwnedByReserv, Context {

    address private _royaltyReceiver; // primary sales, FoB account
    address private _registar;
    uint96 private _royaltyFeeNumerator; // primary sales, 10000 is 100%, 360 for 3.6%
    uint96 private _royaltyFeeNumeratorSecondarySales; // 10000 is 100%, 1000 for 10%
    uint256 private _splitterFoBShares; // 36 -> 3.6%

    constructor(
        address registar,
        address reservOwnerContract,
        uint96 royaltyFeeNumerator,
        uint96 royaltyFeeNumeratorSecondarySales,
        uint256 splitterFoBShares
    ) OwnedByReserv(reservOwnerContract) {
        _registar = registar;
        _royaltyReceiver = reservTreasury();
        _royaltyFeeNumerator = royaltyFeeNumerator;
        _royaltyFeeNumeratorSecondarySales = royaltyFeeNumeratorSecondarySales;
        _splitterFoBShares = splitterFoBShares;
    }

     /// @inheritdoc IVenueFactory
    function setRoyaltyReceiver(address royaltyReceiver) external virtual override onlyReserv {
        _royaltyReceiver = royaltyReceiver;
    }

    /// @inheritdoc IVenueFactory
    function setRoyaltyFeeNumerator(uint96 royaltyFeeNumerator) external virtual override onlyReserv {
        _royaltyFeeNumerator = royaltyFeeNumerator;
    }
    ...
    /// @inheritdoc IVenueFactory
    function setSplitterFoBShares(uint96 splitterFoBShares) external virtual override onlyReserv {
        _splitterFoBShares = splitterFoBShares;
    }
    ...
    /// @inheritdoc IVenueFactory
    function getInfo() external virtual override view returns(address, uint256, uint96, uint96) {
        return (_royaltyReceiver, _splitterFoBShares, _royaltyFeeNumerator, _royaltyFeeNumeratorSecondarySales);
    }
    ...
    function deployVenue(
        ...
        VenueSBT newVenue = new VenueSBT(
            name,
            symbol,
            baseUri,
            _registar,
            ...
            _royaltyFeeNumeratorSecondarySales
        );

        address venue = address(newVenue);

         PaymentSplitter splitter = new PaymentSplitter(
            _getPayees(_royaltyReceiver, venue),
            _getShares(_splitterFoBShares, 100 - _splitterFoBShares)
        );

        address royaltyReceiverSecondarySales = address(splitter);

        newVenue.setRoyaltyReceiverSecondarySales(royaltyReceiverSecondarySales);
...
interface IVenueFactory {
    ...
    function setRoyaltyFeeNumerator(uint96 royaltyFeeNumerator) external;

    function setRoyaltyFeeNumeratorSecondarySales(uint96 royaltyFeeNumeratorSecondarySales) external;

    function setSplitterFoBShares(uint96 splitterFoBShares) external;

    function getInfo() external view returns(address, uint256, uint96, uint96);
...
contract VenueSBT is IVenueSBT, ERC721A, IERC721Receiver, OwnedByReserv {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    address private _royaltyReceiverSecondarySales;

    uint96 private _royaltyFeeNumerator; //from 0 to 10000 where 10000 is 100%
    uint96 private _royaltyFeeNumeratorSecondarySales; //from 0 to 10000 where 10000 is 100%
    ...
    function versionVenueSBT() external pure virtual override returns (string memory) {
        return '1.0.0-beta.0+fob.rsv.iVenueSBT';
    }
    ...
    function isVenueManager(address addr) public view returns (bool) {
        return addr == owner() || addr == _venueOwner || _eventManagers.contains(addr);
    }
    ...
    /// @inheritdoc IVenueSBT
    function setRoyaltyInfo(address royaltyReceiver, uint96 royaltyFeeNumerator) external virtual override onlyReserv {

        if (royaltyReceiver == address(0)) revert VenueInvalidRoyaltyReceiver();

        if (royaltyFeeNumerator > 10000) revert VenueInvalidRoyaltyFeeNumerator();
    ...
    /// @inheritdoc IVenueSBT
    function setRoyaltyInfoSecondarySales(address royaltyReceiver, uint96 royaltyFeeNumerator)
        external
        virtual
        override
        onlyReserv
    {
        ...
        _royaltyFeeNumeratorSecondarySales = royaltyFeeNumerator;
    }
    ...
    function mint(
        ...
        bytes memory mainData = abi.encode(
            ...
            paymentToken,
            _royaltyReceiver,
            _royaltyReceiverSecondarySales,
            _royaltyFeeNumerator,
            _royaltyFeeNumeratorSecondarySales,
            extraData
        );
