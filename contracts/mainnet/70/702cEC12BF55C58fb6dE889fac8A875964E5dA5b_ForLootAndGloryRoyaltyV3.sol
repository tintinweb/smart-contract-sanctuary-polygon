...
contract ForLootAndGloryRoyaltyV3 is AccessControl, ReentrancyGuard, Pausable {
    ...
    uint256 public totalStakedSupply = 1000000000000000000; // for tracking how much is staked in total
    uint256 public totalWeightedStakedSupply = 1000000000000000000; // adjusted total based on locking weights
    ...
    uint256 internal oneMonthMultiplier = 1;
    uint256 internal threeMonthMultiplier = 2;
    uint256 internal sixMonthMultiplier = 3;
    uint256 internal nineMonthMultiplier = 4;
    uint256 internal twelveMonthMultiplier = 5;
    ...
    uint256 public dailyEmissionsRate; // How much of the rewardToken gets distributed per day
    uint256 public bigMultiplier = 1000000000000; // Need this for getting around floating point issues
    ...
    //v2
    IRoyalty internal v2;
    mapping(address => bool) internal isV2Claim;
    ...
    constructor(
        ...
        dailyEmissionsRate = 1000000000000000000; // daily emissions for the initial
    ...
    modifier isStaking() {
        require(
            addressStakedBalance(msg.sender) > 0,
    ...
    function createStake(uint256 _amount, uint256 _duration)
        external
        whenNotPaused
    {
        ...
        require(
            _duration > 0 || userStakes[msg.sender] != 0,
            "You need a duration for your initial stake"
        );
        require(
            _duration.div(1) >= userRewardMultiplier[msg.sender] ||
                userStakes[msg.sender] == 0 ||
                _duration == 0,
            "You can't reduce your staking time!"
        );

        setLock(_duration);
        uint256 multiplier = userRewardMultiplier[msg.sender];
        ...
        totalWeightedStakedSupply -= userWeightedStakes[msg.sender];
        userWeightedStakes[msg.sender] = (userStakes[msg.sender]).mul(
            multiplier
        );
        totalWeightedStakedSupply += userWeightedStakes[msg.sender];
    ...
    function removeStake(uint256 _amount) external whenNotPaused isStaking {
        ...
        require(
            block.timestamp > userUnlockTime[msg.sender] || emergencyUnlock,
            "Your staking tokens are still locked!"
        );
        getRewards();
        uint256 multiplier = userRewardMultiplier[msg.sender];
        totalStakedSupply = totalStakedSupply.sub(_amount);
        totalWeightedStakedSupply = totalWeightedStakedSupply.sub(
            _amount.mul(multiplier)
        );
        userStakes[msg.sender] = userStakes[msg.sender].sub(_amount);
        userWeightedStakes[msg.sender] = userWeightedStakes[msg.sender].sub(
            _amount.mul(multiplier)
        );
    ...
    function removeStakeMax() external whenNotPaused isStaking {
        ...
        getRewards();
        totalStakedSupply = totalStakedSupply.sub(userStakes[msg.sender]);
        totalWeightedStakedSupply = totalWeightedStakedSupply.sub(
            userWeightedStakes[msg.sender]
        );
    ...
    function setLock(uint256 _duration) internal {
        if (_duration >= 0 && _duration < 1) {
            return;
        } else if (_duration >= 1 && _duration < 3) {
            userRewardMultiplier[msg.sender] = oneMonthMultiplier;
            userUnlockTime[msg.sender] = block.timestamp.add(31 days);
    ...
    function updateBigRewardsPerToken() public {
        if (timeSinceLastReward() > 0) {
            // so it doesn't run multiple times a second, possible during high activity
            uint256 rewardSeconds = timeSinceLastReward(); // get how many seconds its been since the last reward time
            lastRewardTime = block.timestamp; // update the last reward time
            uint256 emissionsPerSecond = dailyEmissionsRate.div(86400); // calc emissions per second
            uint256 newEmissionsToAdd = emissionsPerSecond.mul(rewardSeconds); // calculate how many new rewards to add
            uint256 newBigRewardsPerToken = (
                (newEmissionsToAdd.mul(bigMultiplier)).div(
                    totalWeightedStakedSupply
                )
            ); // calc how much to add
    ...
    function userPendingRewards(address _address)
        ...
        returns (uint256)
    {
        uint256 earnedBigRewardsPerToken = bigRewardsPerToken.sub(
            bigUserRewardsCollected[_address]
        );
        if (earnedBigRewardsPerToken > 0) {
            uint256 rewardsToSend = (
                earnedBigRewardsPerToken.mul(userStakes[_address]).mul(
                    userRewardMultiplier[_address]
                )
            ).div(bigMultiplier);
    ...
    function withdrawRewards(uint256 _amount) external onlyRole(MANAGER_ROLE) {
        rewardToken.safeTransfer(msg.sender, _amount);
    }

    function depositRewards(uint256 _amount) external onlyRole(DEPOSIT_ROLE) {
        rewardToken.safeTransferFrom(msg.sender, address(this), _amount);
    }

    function setDailyEmissions(uint256 _amount)
        external
    ...
    function emergencyUnlockToggle() external onlyRole(MANAGER_ROLE) {
    ...
    function showRewardsMultiplier() external view returns (uint256) {
    ...
    function getIsUserUpgrade(address user) public view returns (bool) {
        return isV2Claim[user];
    }

    // V2 Migration
    function RoyaltyUpgrade() public nonReentrant {
        require(
            v2.isStaker(msg.sender) == true && isV2Claim[msg.sender] == false,
            "Already Upgrade!"
        );
        userStakes[msg.sender] = v2.addressStakedBalance(msg.sender);
        totalStakedSupply += userStakes[msg.sender];
        uint256 pendingRewards = v2.userPendingRewards(msg.sender);
        uint256 v2UnlockTime = v2.showLockTimeRemaining(msg.sender);
        if (v2UnlockTime == 0) {
            userUnlockTime[msg.sender] = block.timestamp;
        } else {
            userUnlockTime[msg.sender] = v2UnlockTime.add(block.timestamp);
        }
        if (v2UnlockTime < 31 days) {
            userRewardMultiplier[msg.sender] = oneMonthMultiplier;
        } else if (v2UnlockTime >= 31 days && v2UnlockTime < 91 days) {
            userRewardMultiplier[msg.sender] = threeMonthMultiplier;
        } else if (v2UnlockTime >= 91 days && v2UnlockTime < 182 days) {
            userRewardMultiplier[msg.sender] = sixMonthMultiplier;
        } else if (v2UnlockTime >= 182 days && v2UnlockTime < 273 days) {
            userRewardMultiplier[msg.sender] = nineMonthMultiplier;
        } else if (v2UnlockTime >= 273 days) {
            userRewardMultiplier[msg.sender] = twelveMonthMultiplier;
        }
        userWeightedStakes[msg.sender] = userStakes[msg.sender].mul(
            userRewardMultiplier[msg.sender]
        );
        totalWeightedStakedSupply += userWeightedStakes[msg.sender];
        isV2Claim[msg.sender] = true;
