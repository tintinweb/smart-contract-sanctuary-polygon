...
import "../Rewards/RewardsPayer.sol";
import "../Rewards/IUserRewards.sol";
import "../Rewards/IRewardsControl.sol";

/// @title Interface Rewards.
/// @author Panoram.
/// @notice Use this interface to access the data register in the old rewards payer contract.
interface IRewardsOld {
    /// @dev Function to get the data register for Builder & Holder Rewards in the old rewards payer contract.
    /// @param _walletUser - the address of the user from whom we want to get the data.
    /// @return rewardPerBuilder - the builder rewards assigned to the user.
    /// @return lastTimePayBuilder - the last time the user claims his rewards for bein a builder.
    /// @return lastTimeCalcBuilder - the last time the builder's rewards were calculated.
    /// @return rewardPerHolder - the holder rewards assigned to the user.
    /// @return lastTimePayHolder - the last time the user claims his rewards for bein a holder.
    /// @return lastTimeCalcHolder - the last time the Holder's rewards were calculated.
    function getRewardsPerUserBH(address _walletUser) external view returns (uint256,uint256,uint256,uint256,uint256,uint256);
}

/// @title Migrations Rewards Contract.
/// @author Panoram.
/// @notice Use this contract to migrate the data register in the old rewards payer to the new rewards contracts.
contract MigrationsRewards is AccessControl, IUserRewards {
    bytes32 private constant DEV_ROLE = keccak256("DEV_ROLE");
    IRewardsOld private oldRewards;
    IRewardsControl private newRewards;

    /// @dev Contract's Constructor.
    /// @param _oldRewards - the address of the old Rewards Payer Contract.
    /// @param _newRewards - the address of the new Rewards Control Contract.
    constructor(address _oldRewards, address _newRewards){
        ...
        oldRewards = IRewardsOld(_oldRewards);
        newRewards = IRewardsControl(_newRewards);
    }

    // Modifiers
    modifier onlyDev() {
        if (!hasRole(DEV_ROLE, msg.sender)) {
            revert("Not enough Permissions in Migrations Contract");
        }
        _;
    }


    function migrate(address[] calldata _wallets) external onlyDev {
        uint256 length = _wallets.length;
        UserRewardsBuilder memory RB;
        UserRewardsHolder memory RH;

        for(uint256 i=0; i<length; ){
            // get the data from old contract.
            (RB.rewardPerBuilder, RB.lastTimePayBuilder, RB.lastTimeCalcBuilder, RH.rewardPerHolder, RH.lastTimePayHolder, RH.lastTimeCalcHolder) = oldRewards.getRewardsPerUserBH(_wallets[i]);
            RB.rewardPerBuilderNFT = 3346;
            RH.rewardPerHolderNFT = 10363;
            // write the data to new rewards control contract.
            newRewards.setMigrationData(RB, RH, _wallets[i]);
...
interface IRewardsControl is IUserRewards {
    ...
    function setMigrationData(UserRewardsBuilder calldata _bRewards, UserRewardsHolder calldata _hRewards, address _user) external;
...
contract RewardsPayer is AccessControl, HelperRewardsContract, IMortgageInterest, IUserRewards{
    ...
    address vaultLenders, address VaultRewardsContract, address PoolRewardsLenders, address _rewardsControl) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(DEV_ROLE, msg.sender);
    ...
    function CalcBuilderRewardsDaily(address[] calldata _walletsUsers,uint256[] calldata _numNFTsMinted, address _token) external onlyRelayer validToken(_token) isPaused{
    ...
    /// @param _token - The token address to get the vault associated with it.
    function CalcHolderRewardsDaily(address[] calldata _walletsUsers,uint256[] calldata _numNFTsHolder, address _token) external onlyRelayer validToken(_token) isPaused{
    ...
    function CalcRentRewardsForCollection(address _collection, address[] calldata _walletsUsers,uint256[] calldata _numNFTsHolder) external onlyRelayer isPaused{
        if (_walletsUsers.length != _numNFTsHolder.length) {
            revert("Array Mismatch");
        }
        ...
        uint256 payPerNFT = PRBMathForRents(monthlyRentIncome, _collection);
    ...
    function payRentReward(address _collection, address _token) external validToken(_token) isPaused nonReentrant{
        ...
        if (rents.rewardPerRent == 0) {
            revert("You have no rewards available");
        }
        checkDebt(rents, _collection, _token); 
        ...
        uint256 amountRentReward = rents.rewardPerRent;
       //lo que se modifica en la funcion de checkDebt de la struct se queda guardado en memoria de esta funcion por lo que los datos van actualizados con los calculos hechos en checkDebt
        RControl.setRentRewardsTimePay(block.timestamp, msg.sender, _collection);
        if(amountRentReward != 0){
            handleTransferUser(IERC20(_token), amountRentReward, msg.sender, rents.rewardPerRentNFT, _collection);
        }
    ...
    function payAllRewards(address[] calldata _collections, address _token) external validToken(_token) isPaused nonReentrant{
        ...
        for(uint16 i = 0; i < _collections.length; ) {
            ...
            checkDebt(rents, _collections[i], _token);
            amountRents = rents.rewardPerRent;
            if(amountRents > 0){
    ...
    /// @param _rent - the UserRewardsRent Struct eith the data of the user.
    /// @param _collection - the address of the NFT collection from which the user wants to claim his rents.
    /// @param _token - the token address used to pay the Rents.
    function checkDebt(UserRewardsRent memory _rent, address _collection, address _token) internal {
        uint256 rewardsRent = _rent.rewardPerRent;
        ...
        if (IdMortgagesxCollection.length > 0) {
            for(uint24 i = 0; i < IdMortgagesxCollection.length; ) {
                            ...
                            if (rewardsRent >= mortgage.totalToPayOnLiquidation) {
                               ...
                               if (rewardsRent >= mortgage.totalPoolLiquidation) {
                                ...
                                if(rewardsRent >= mortgage.amountToVault){
                                    ...
                                    mortgage.totalMonthlyPay -= mortgage.amountToVault;
                                    mortgage.totalDelayedMonthlyPay -= mortgage.amountToVault;
                ...
                // } // if !liquidated, si ya esta liquidado y quiere cobra la renta, se usara para pagar la deuda
                unchecked {
                    ++i;
                }
            }
            _rent.rewardPerRent = rewardsRent;
        }
    }
...
contract HelperRewardsContract is ReentrancyGuard, AccessControl{
   ...
   address public walletPanoram = 0x526324c87e3e44630971fd2f6d9D69f3914e01DA;  // Cambiar por el multisig
    ...
    constructor(){
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(DEFAULT_ADMIN_ROLE, 0x526324c87e3e44630971fd2f6d9D69f3914e01DA); // Cambiar por el MULTISIG.
        _setupRole(DEV_ROLE, 0x526324c87e3e44630971fd2f6d9D69f3914e01DA); // Cambiar por el MULTISIG.
