    ...
    UniversePairVaultStorage
{
    ...
    function initialize(
        ...
        token0.approve(_nftManager, uint256(-1));
        token1.approve(_nftManager, uint256(-1));
    ...
    function operate(
        ...
        if (otype == 0) {
            ...
            // 2 add whiteList; 3 del whiteList
    ...
    function claimStakeReward(IERC20Minimal rewardToken, address to) internal {
        ...
        for (uint256 i = 0; i < positionList.length; i++) {
            ...
            if (_pos.tokenId > 0) {
                ...
                PositionNFTForPair.nftStakeAll(
                    config,
                    address(nftManager),
                    _pos
                );
    ...
    function _quantityTransform(
        ...
        if (newAmt != 0) {
            newShare = (
                totalShare == 0
                    ? newAmt
                    : FullMath.mulDiv(newAmt, totalShare, totalAmt)
            );
        }
    }
        ...
        )
    {
        ...
        if (totalShare == 0) {
            ...
            if (total0 == 0) {
                ...
                amount0Desired.mul(total1) > amount1Desired.mul(total0)
            ) {
    ...
    function _calcBal(uint256 share)
        internal
        view
        ...
        )
    {
        ...
        for (uint256 i = 0; i < positionList.length; i++) {
            _pos = positionList[i];
            if (_pos.liquidity > 0) {
                (amount0, amount1) = nftManager.getTotalAmounts(
                    _pos.tokenId,
        ...
        )
    {
        ...
        if (total0 > 0) {
            utilizationRate0 = 1e5 - free0.mul(1e5).div(total0);
        }
        if (total1 > 0) {
            utilizationRate1 = 1e5 - free1.mul(1e5).div(total1);
        }
    }
    ...
    function getShares(uint256 amount0Desired, uint256 amount1Desired)
        external
        view
        returns (uint256 share0, uint256)
    {
    ...
    function getBals(uint256 share0, uint256)
        external
        view
    ...
    function _swapAndAddAll(
        ...
        if (_pos.tokenId > 0) {
            ...
            PositionNFTForPair.nftUnStakeAll(config, _pos.tokenId);
            // add liquidity
            (, , uint128 liquidity) = nftManager.addAll(
                _pos.tokenId,
                _balance0(),
                _balance1()
            );
    ...
    function _swap(
        ...
        if (bal0.mul(reserve1) >= bal1.mul(reserve0)) {
            ...
            divisor = FullMath.mulDiv(priceX96, reserve0, FixedPoint96.Q96).add(
                    reserve1
                );
            ...
            divisor = FullMath.mulDiv(reserve1, FixedPoint96.Q96, priceX96).add(
                    reserve0
                );
            amt = dividend.div(divisor);
        }
    ...
    function _mockReserve(PositionNFTForPair.Position memory _pos)
        internal
        view
        returns (uint256 reserve0, uint256 reserve1)
    {
        // calculate token0/token1
        (uint160 sqrtPrice, , , , , , ) = IUniswapV3Pool(_pos.poolAddress)
            .slot0();
    ...
    function _deposit(
        ...
        // Cal Share
        (share, amount0Desired, amount1Desired) = _calcShare(
            amount0Desired,
            amount1Desired
        );
        require(share > 0, "0");
    ...
    function _mintAll(
        ...
        (_lowerTick, _upperTick) = UniswapHelper.tickRegulate(
            _lowerTick,
            _upperTick,
            _pos.tickSpacing
        );
    ...
    function _multiStopMining(uint256 share, uint256 totalShare)
        internal
        returns (uint256 amt0, uint256 amt1)
    {
        ...
        for (uint256 i = 0; i < positionList.length; i++) {
            ...
            if (_pos.liquidity > 0) {
                PositionNFTForPair.nftUnStakeAll(config, _pos.tokenId);
                uint128 liq = _toUint128(
                    FullMath.mulDiv(uint256(_pos.liquidity), share, totalShare)
                );
                (
                    uint256 _amt0,
                    uint256 _amt1,
                    uint256 _fee0,
                    uint256 _fee1
                ) = nftManager.burn(_pos.tokenId, liq);
                amt0 = amt0.add(_amt0);
                amt1 = amt1.add(_amt1);
                fee0 = fee0.add(_fee0);
                fee1 = fee1.add(_fee1);
    ...
    function setInitPos(address _poolAddress, int24 _tickSpacing) internal {
    ...
    function burnAllAndCollectFee(PositionNFTForPair.Position memory _pos)
        internal
    ...
    function initPosition(int24 _lowerTick, int24 _upperTick)
        external
        override
        onlyManager
    {
    ...
    function reBalance(
        ...
        PositionNFTForPair.Position memory _pos = positionList[idx];
        require(
            band > 0 && reBalanceThreshold > 0 && _pos.tickSpacing > 0,
            "b"
        );
    ...
    function nftStake(IUniswapV3Staker.IncentiveKey memory key) external {
        ...
        for (uint256 i = 0; i < positionList.length; i++) {
            ...
            if (
                _pos.tokenId > 0 &&
                _pos.poolAddress == address(key.pool) &&
                block.timestamp >= key.startTime &&
                block.timestamp < key.endTime
            ) {
                PositionNFTForPair.nftStake(
                    config,
                    key,
                    address(nftManager),
                    _pos
    ...
    function onERC721Received(
        ...
        PositionNFTForPair.positionManager.approve(
            address(nftManager),
            tokenId
        );
...
library UniswapHelper {
    ...
    function floor(int24 tick, int24 _tickSpacing)
        internal
        pure
        returns (int24)
    {
