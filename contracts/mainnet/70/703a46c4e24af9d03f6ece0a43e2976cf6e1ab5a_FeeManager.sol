...
contract FeeManager is OwnableUpgradeable, UUPSUpgradeable {
    ...
    function convertAsset(address asset) external onlyWhitelist {
        uint256 balance = IERC20(asset).balanceOf(address(this));

        if (balance > 0) {
            uint256 wethAmount = (balance *
                (ethAllocation + buyBurnAllocation)) / PERCENT_DECIMALS;

            uint256 wbtcAmount = balance - wethAmount;
    ...
    function deposit() external onlyWhitelist {
        ...
        if (buyBurnAllocation > 0) {
            amountBuyBurn =
                ...
                (buyBurnAllocation + ethAllocation);
