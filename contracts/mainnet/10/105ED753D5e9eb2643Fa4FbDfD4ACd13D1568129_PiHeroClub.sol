...
contract PiHeroClub is ERC721, WithdrawAll {
    using Strings for uint256;
    ...
    uint256 public cost = 20000000000000000000;
    bool public paused = false;
    uint256 public openTime = 1690786800;
    ...
    event InviteRelations(address indexed inviter, address owner, uint256 tokenId);
    ...
    */
    ...
    ) ERC721("Pi Hero Club", "Pi Hero NFT") {
        setBaseURI(_initBaseURI);
        setNotRevealedURI(_initNotRevealedUri);
        inviter[address(1)] = address(2);
    }
    ...
    function mint(address _inviter, uint256 _mintAmount) public payable {
        require(!paused, "NFT: pause");
        require(block.timestamp >= openTime, "NFT: pause");

        require(_mintAmount <= maxMintAmount, "You can only mint 10 pieces at a time");

        require(totalSupply + _mintAmount < maxSupply + 1, "Exceeded the maximum number of mint");

        require(msg.value >= cost * _mintAmount, "Insufficient balance");

        require(msg.sender != _inviter, "Member: The superior cannot be himself");

        require(inviter[_inviter] != address(0), "The invitation address cannot be a black hole address");

        if (inviter[msg.sender] == address(0)) {
            inviter[msg.sender] = _inviter;
        } else {
            _inviter = inviter[msg.sender];
        }

        for (uint256 i = 1; i <= _mintAmount; i++) {
            uint256 newTokenId = totalSupply + i;
            _safeMint(msg.sender, newTokenId);
            emit InviteRelations(_inviter, msg.sender, newTokenId);
        }
