...
contract UniswapV3ResearchStrategy is IUniswapV3MintCallback, IUniswapV3SwapCallback {
  ...
  uint public earned;
  uint public rebalanceCost;
  uint public il;
  uint public lastAmount0;
  uint public lastAmount1;
  ...
  function getPureLiquidityBalances() public view returns (uint amount0Current, uint amount1Current) {
  ...
  function name() external view returns (string memory n) {
    n = string.concat("RESEARCH_", IERC20Metadata(pool.token0()).symbol(), "-", IERC20Metadata(pool.token1()).symbol(), "-", _feeString(pool.fee()), "_", _toString(int(tickRange)), "-", _toString(int(rebalanceTickRange)));
  }
  ...
  function deposit(address token, uint amount) external {
    ...
    (lastAmount0, lastAmount1) = getPureLiquidityBalances();
    lastAmount0 += _balance(pool.token0());
    lastAmount1 += _balance(pool.token1());
  }
  ...
  function rebalanceWithTracking() public {
    ...
    if (liquidity != 0) {
      ...
      (uint pureAmount0, uint pureAmount1) = getPureLiquidityBalances();
      ...
      if (trackingToken == pool.token0()) {
        earned += balance0 - pureAmount0 - balance0Before + (balance1 - pureAmount1 - balance1Before) * price1 / 10**IERC20Metadata(pool.token1()).decimals();
      } else {
        earned += balance1 - pureAmount1 - balance1Before + (balance0 - pureAmount0 - balance0Before) * price0 / 10**IERC20Metadata(pool.token0()).decimals();
      }
      ...
      (pureAmount0, pureAmount1) = getPureLiquidityBalances();
      uint totalAmount0 = pureAmount0 + _balance(pool.token0());
      uint totalAmount1 = pureAmount1 + _balance(pool.token1());

      if (trackingToken == pool.token0()) {
        rebalanceCost += totalAmount0Estimate - totalAmount0 - totalAmount1 * price1 / 10**IERC20Metadata(pool.token1()).decimals();
        il += lastAmount0 + lastAmount1 * price1 / 10**IERC20Metadata(pool.token1()).decimals();
        il -= totalAmount0 + totalAmount1 * price1 / 10**IERC20Metadata(pool.token1()).decimals();
      } else {
        rebalanceCost += totalAmount1Estimate - totalAmount1 - totalAmount0 * price0 / 10**IERC20Metadata(pool.token0()).decimals();
        il += lastAmount1 + lastAmount0 * price0 / 10**IERC20Metadata(pool.token0()).decimals();
        il -= totalAmount1 + totalAmount0 * price0 / 10**IERC20Metadata(pool.token0()).decimals();
      }

      lastAmount0 = totalAmount0;
      lastAmount1 = totalAmount1;
    } else {
      _setNewTickRange();
    }
