...
contract FlashloanV2 is FlashLoanReceiverBaseV2 {
    ...
    function executeOperation(
        ...
        // just borrows one matic and return it through the flashloan

        console.log(
            "Matic Balance of address(this) after flashLoan",
    ...
    function flashloan(address _asset, uint256 _amount) public onlyOwner {
        ...
        console.log(
            "Matic Balance of address(this) before flashLoan",
            IERC20(assets[0]).balanceOf(address(this))
        );
        ...
        console.log(
            "Matic Balance of address(this) after repaying flashLoan",
            IERC20(assets[0]).balanceOf(address(this))
        );
    }

    function getWethBal(address _asset) external view returns (uint) {
        return IERC20(_asset).balanceOf(address(this));
    }

    function withDraw(address _asset) external onlyOwner returns (bool) {
        uint amount = IERC20(_asset).balanceOf(address(this));

        IERC20(_asset).transfer(_owner, amount);

        console.log(
            "Matic Balance of address(this) after withdrawing flashLoan",
