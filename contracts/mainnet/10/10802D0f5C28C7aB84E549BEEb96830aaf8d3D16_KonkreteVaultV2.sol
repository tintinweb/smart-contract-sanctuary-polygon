...
interface IDatabase is IAccessControl {
    ...
    function addKycPermit(address toKyc, uint16 countryCode_, uint64 deadline, bytes32 r, bytes32 s, uint8 v)
        external;

    function addKycs(address[] memory toKyc, uint256[] memory countryCodes) external;

    function addWhitelist(address toWhitelist) external;

    function addWhitelistPermit(address toWhitelist, uint64 deadline, bytes32 r, bytes32 s, uint8 v) external;
...
interface IKonkreteVault is IERC4626Upgradeable, IAccessControlEnumerableUpgradeable {
    ...
    function refundCapital(uint256 capital_) external;
    ...
    function setDepositLimitsPerUser(uint256 minDeposit_, uint256 maxDeposit_) external;
    ...
    PausableUpgradeable
{
     ...
     */
    function _invest(uint256 amount, address sender, address receiver) internal returns (uint256) {
     ...
     * We did not use the totalAssets() method because it does not differentiate between assets committed through deposits and assets sent
     * directly to contract (security leak).
     */
    ...
    bool public isCapitalCollected;
