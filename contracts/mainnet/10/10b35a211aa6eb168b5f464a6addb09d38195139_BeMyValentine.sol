...
contract BeMyValentine is Ownable, ERC721{
    ...
    Counters.Counter private _tokenSupply;
        ...
        uint256 public PPrice = 1 ether;
        uint256 public MAX_PRIVATE = 0;
        uint256 public MAX_PURCHASE = 20;
        uint256 public MAX_TOKENS = 888-totalSupply();
        uint256 public MAX_RESERVE = 288;
...
constructor(
  ...
  ) ERC721("Be My Valentine Ai", "VALENTINEAI") {
      _baseTokenURI = "ipfs://QmSZoxMGBvMw53vUmNnwBzCyEmgZPABFYXVcZApAhaHZNf4a/";
...
// Reserve 288 Tokens
  function reserveTokens(address to,uint numberOfTokens) public onlyOwner { 
      uint256 ownerTokenCount = balanceOf(msg.sender);   
        ...
        require(numberOfTokens <= MAX_RESERVE, "Can only mint 288 tokens at a time");
        require(ownerTokenCount +numberOfTokens <=288, "Can only recieve a maximum of 288 tokens");
...
function internalPreMint(uint256 numberOfTokens) private{
        ...
        require(numberOfTokens <=20, "Can only mint 20 token");
...
function internalMint(uint256 numberOfTokens) private{
        uint256 ownerTokenCount = balanceOf(msg.sender);
        require(numberOfTokens > 0, "numberOfNfts cannot be 0");
        require(numberOfTokens <= MAX_PURCHASE, "Can only mint 40 tokens per wallet");
        require(ownerTokenCount + numberOfTokens<=40, "You can only mint 40 nfts per wallet");
        uint256 supply = _tokenSupply.current();
