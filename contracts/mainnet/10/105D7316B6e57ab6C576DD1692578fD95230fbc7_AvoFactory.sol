...
abstract contract AvoFactoryConstants is IAvoFactory {
...
abstract contract AvoForwarderCore is AvoFactoryConstants, AvoFactoryVariables, AvoFactoryErrors, AvoFactoryEvents {
    /// @notice constructor sets the immutable avoVersionsRegistry address
...
contract AvoFactory is AvoForwarderCore {
    ...
    function deploy(address owner_) external onlyEOA(owner_) returns (address deployedAvoSafe_) {
        // deploy AvoSafe deterministically using CREATE2 opcode (through specifying salt)
