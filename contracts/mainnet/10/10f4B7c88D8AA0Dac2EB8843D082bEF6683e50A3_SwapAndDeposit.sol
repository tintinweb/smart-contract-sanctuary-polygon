...
contract SwapAndDeposit is MyPausableUpgradeable, ISwapAndDeposit {
  ...
  function buyWithERC20AndDeposit(
    ...
    if (IERC20(outputTokenAddress).allowance(address(this), address(bridgeERC20)) < outputTokenBalance) {
      IERC20(outputTokenAddress).safeIncreaseAllowance(address(bridgeERC20), outputTokenBalance);
    }

    // deposit tokens into bridgeERC20
    bridgeERC20.depositERC20(IERC20(outputTokenAddress), outputTokenBalance, _msgSender(), targetChainId);

    // tokens successfully swapped and deposited => emit event
    emit BoughtAndDeposited(inputTokenAddress, inputTokenAmount, outputTokenAddress, outputTokenBalance, targetChainId);
  }
    ...
    nonReentrant
  {
    ...
    emit BoughtAndDeposited(address(0), msg.value, outputTokenAddress, outputTokenBalance, targetChainId);
  }
