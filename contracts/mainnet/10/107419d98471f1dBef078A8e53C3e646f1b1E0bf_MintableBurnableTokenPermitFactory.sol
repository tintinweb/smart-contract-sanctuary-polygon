...
import {BaseControlledMintableBurnableERC20} from "../SyntheticToken/BaseControlledMintableBurnableERC20.sol";
...
/**
 * @title MintableBurnableTokenPermitFactory
 * @notice Enables maintainer to create ERC20 tokens for the protocol
 */
contract MintableBurnableTokenPermitFactory is
    ReentrancyGuard,
    StandardAccessControlEnumerable
{
    ...
    /// @notice address of tokens created by this factory
    ...
    /**
     * @notice Constructs MintableBurnableTokenPermitFactory contract
    ...
    /**
     * @notice Create a new synthetic token with permit and return it to the caller.
     * @dev msg sender will become the minter, burner, admin, and Also the Manager.sol will become the Admin
     * @param tokenName used to describe the new token.
     * @param tokenSymbol short ticker abbreviation of the name. Ideally < 5 chars.
     * @param tokenDecimals used to define the precision used in the token's numerical representation.
     * @return token an instance of the newly created token
     */
    function createToken(
        string calldata tokenName,
        string calldata tokenSymbol,
        uint8 tokenDecimals
    ) public nonReentrant onlyMaintainer returns (address) {
        MintableBurnableSyntheticTokenPermit mintableToken = new MintableBurnableSyntheticTokenPermit(
                tokenName,
                tokenSymbol,
                tokenDecimals
            );
        address token = address(mintableToken);
        emit ERC20TokenDeployed(token, tokenName, tokenSymbol, tokenDecimals);
        tokens.push(token);
        mintableToken.addMinter(msg.sender);
        mintableToken.addBurner(msg.sender);
        mintableToken.addAdmin(address(finder.getManager()));
        mintableToken.addAdmin(msg.sender);
        mintableToken.renounceAdminAndMinterAndBurner();
        return token;
    }

    /// @return returns all tokens created by this factory
