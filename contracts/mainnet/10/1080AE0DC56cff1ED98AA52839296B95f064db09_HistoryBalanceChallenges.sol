...
contract ChallengeDetail is IERC721Receiver, IChallenge {
    ...
    */
    ...
    uint256[] public awardReceiversPercent;
    
    mapping(address => uint256[]) public awardTokenReceivers;

    uint256[] public listBalanceAllToken;

    uint256[] public amountTokenToReceiverList;
    ...
    */
    ...
    function transferToListReceiverSuccess() private {      
        ...
        if(allowGiveUp[2]) {
            indexNft = ExerciseSupplementNFT(erc721Address[0]).getIndexToken();
        ...
        for(uint256 j = 0; j < erc721AddressList.length; j++) {
            if(erc721AddressList[j] != address(0)) {
                for(uint256 i = 0; i < getIndexToken(erc721AddressList[j]); i++) {
                    if(getOwnerOfNft(erc721AddressList[j], i) == address(this)) {
                        if(!checkApprovedForAll(erc721AddressList[j], address(this), challenger)) {
                            TransferHelper.safeApproveForAll(
                                erc721AddressList[j],
                                challenger,
                                true
                            );
    ...
    /**
     * @dev Private function transfer NFT when challenge failed
     */
    function transferNFTWhenFailed() private {
        address[] memory erc721AddressList = ExerciseSupplementNFT(erc721Address[0]).getNftListAddress();
        ...
        for(uint256 j = 0; j < erc721AddressList.length; j++) {
            if(erc721AddressList[j] != address(0)) {
                for(uint256 i = 0; i < getIndexToken(erc721AddressList[j]); i++) {
                    ...
                    if(getOwnerOfNft(erc721AddressList[j], i) == address(this) && receiver != address(0)) {
                        if(!checkApprovedForAll(erc721AddressList[j], address(this), receiver)) {
                            TransferHelper.safeApproveForAll(
                                erc721AddressList[j],
                                receiver,
                                true
    ...
    function checkApprovedForAll(address _erc721Address, address _from, address _to) private view returns(bool){
       return ExerciseSupplementNFT(_erc721Address).isApprovedForAll(_from, _to);
    }

    function getIndexToken(address _erc721Address) private view returns(uint256) {
        return ExerciseSupplementNFT(_erc721Address).getIndexToken();
    }
...
contract HistoryBalanceChallenges{
    ...
    function getNftInfo(address payable _contractChallengeAddress) public view returns (
        address sponsor, address challenger, uint256 startTime, 
        uint256 endTime, uint256 dayRequired, uint256 targetStepsPerDay, 
        uint256 minimunAchivementDays, uint256 giveUpType
    ){
        bool isChallengeSuccess = ChallengeDetail(_contractChallengeAddress).isSuccess();
        if(isChallengeSuccess) {
            uint256 allowGiveUp = ChallengeDetail(_contractChallengeAddress).allowGiveUp(0) ? 0: 1;
            return (
                ChallengeDetail(_contractChallengeAddress).sponsor(),
                ChallengeDetail(_contractChallengeAddress).challenger(),
                ChallengeDetail(_contractChallengeAddress).startTime(),
                ChallengeDetail(_contractChallengeAddress).endTime(),
                ChallengeDetail(_contractChallengeAddress).dayRequired(),
                ChallengeDetail(_contractChallengeAddress).goal(),
                0,
                allowGiveUp
            );
        }
