...
contract TokenClaim {
    ...
    event JoinedContract(address indexed recipient);
    ...
    function isJoined(address recipient) public view returns (bool) {
    ...
    function joinContract() external {
        require(!isJoined(msg.sender), "Address has already joined the contract");

        // Additional checks if required before allowing an address to join
        
        joinedAddresses[msg.sender] = true;

        emit JoinedContract(msg.sender);
    }
}