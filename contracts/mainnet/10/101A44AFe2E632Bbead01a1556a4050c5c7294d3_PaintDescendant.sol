...
contract PaintDescendant is ERC721 , DefaultOperatorFilterer {
    ...
    constructor()
        ERC721("Paint descendant", "Paint")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 3500) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeifkjmthf6fzsqcug7lfmh7gbqxc5fgiwcrkncxketffv3bqmd3npe/", Strings.toString(tokenId), ".json")
            );
    }
