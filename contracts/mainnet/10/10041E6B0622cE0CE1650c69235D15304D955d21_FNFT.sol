 ...
 */
contract FNFT is IFNFT, Ownable, ERC165, IERC721Metadata {
    ...
    constructor(string memory name_, string memory symbol_,address setting_) {
        _name = name_;
        _symbol = symbol_;
        _currentIndex = _startTokenId();
        require(setting_ != address(0),"setting is zero");
        setting = setting_;
        IFSetting(setting).setInit(4,address(this));
    ...
    function approve(address to, uint256 tokenId) public override {
        address owner = FNFT.ownerOf(tokenId);
    ...
    function safeMint(address to, uint256 quantity) external {
        require(msg.sender == IFSetting(setting).mintOwner(),"mint: caller is not the mintOwner");
        if (quantity > 1000) {
            quantity = 1000;
        }
