...
// File: AULA07/LiftAMM_Leo.sol


pragma solidity ^0.8.9;


contract LiftAMMLeo is ERC20 { 
    constructor(address _tokenA, address _tokenB) ERC20("Lift AMM Leo", "LAMML") {
        tokenA = _tokenA;
        tokenB = _tokenB;
    }
    ...
    modifier checkBlock(uint deadlineSeconds) {
        uint deadline = block.timestamp + (deadlineSeconds * 360);
    ...
    function swap(
        address tokenIn, 
        uint256 amountIn, 
        uint256 minAmountOutPercent, 
        uint deadlineSeconds) 
        external checkBlock(deadlineSeconds) 
