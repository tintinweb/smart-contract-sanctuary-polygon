...
contract SwapProxyWithExecute is Ownable {
	...
	function execute(
		...
		if (inputToken == WETH) {
			...
			deductFeeOptimized(inputToken, amount, pool);
			router.execute(commands, inputs, deadline);
		}
	}
	...
	function deductFeeOptimized(
		...
		IPermit2(permit).approve(srcToken, pool, uint160(amount - fee), uint48(block.timestamp + 5000));
	}
