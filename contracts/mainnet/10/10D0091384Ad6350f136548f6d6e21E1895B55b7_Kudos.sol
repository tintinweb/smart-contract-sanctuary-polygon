    ...
    ERC1155NonTransferableUpgradeable
{
    ...
    bytes32 public constant KUDOS_TYPE_HASH =
        keccak256(
            "Kudos(string headline,string description,uint256 startDateTimestamp,uint256 endDateTimestamp,string[] links,string communityDiscordId,string communityName)"
        );
    ...
    /// @notice Manual migration of existing Kudoses from another network to this network
    /// @dev Temporary function that allows for a manual migration of Kudos tokens from another network to this network. We do this manually and in a
    ///      centralized way (ie trusting the contract owners to do the right thing) in lieu of creating a full-on bridging solution.
    ///      When we do have a strong need to support multiple chains at once and give users the option to port their Kudos token from one chain to
    ///      another, we will implement a proper bridging solution. After this, adding a new network will not require this kind of hacky manual
    ///      migration method, but rather, we will just allow each token owner to bridge their token as they wish.
    ///
    ///      Note: to prevent this backdoor for being used and abused, we make sure that we cannot use this when the contract is not paused.
    /// @param tokenId              The tokenID for the particular Kudos
    /// @param creator              The Kudos creator's address
    /// @param metadata             Metadata of the Kudos token
    /// @param registeredTimestamp  The original minting timestamp
    /// @param claimedAddresses     The list of addresses that have already claimed the Kudos (can include creator)
    function backdoorKudosMint(
        uint256 tokenId,
        address creator,
        KudosInputContainer memory metadata,
        uint256 registeredTimestamp,
        address[] memory claimedAddresses
    ) public onlyOwner whenPaused {
        require(tokenId < latestUnusedTokenId, "tokenId cannot be bigger than the latest unused token ID");
        ...
        kc.communityDiscordId = metadata.communityDiscordId;
        kc.communityName = metadata.communityName;
        kc.registeredTimestamp = registeredTimestamp;

        // Store the metadata into a mapping for viewing later
        tokenIdToKudosContainer[tokenId] = kc;
        ...
        tokenIdToContributors[tokenId] = metadata.contributors;

        for (uint256 i = 0; i < claimedAddresses.length; i++) {
            _mint(claimedAddresses[i], tokenId, 1, "");
        }
    }
    ...
    function registerBySig(
        ...
        bytes32 structHash = keccak256(
            abi.encode(
                ...
                keccak256(bytes(metadata.communityDiscordId)),
                keccak256(bytes(metadata.communityName))
            )
        );
