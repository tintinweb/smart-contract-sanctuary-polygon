...
contract WAGMIPro is Initializable, UUPSUpgradeable, AccessControlEnumerableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(string => uint) public referralCount;
    mapping(string => uint) public isWAGMIPro;
    CountersUpgradeable.Counter public wagmiProCounter;

    mapping(string => uint) public backendNonce;
    mapping(string => bool) public backendReceipt;

    event scComplete(string buyer, uint amount);
    event manualSCComplete(string buyer);
    event backendSCComplete(string buyer,uint nonce, string receipt);
  struct User {
        uint256 totalSparks;
        uint256 dailySparks;
        uint256 startDate;
 ...
 mapping(string => string) public referrers; 
...
//you do not own
function WAGMIProWallet(uint durationMonths, string memory userId, string memory referrer, address paymentMethod) public payable nonReentrant returns(bool){
    require(SCTxn[paymentMethod][durationMonths]!=0,"Invalid payment method");
            require(!blacklisted[userId], "Address is blacklisted");
...
require(keccak256(abi.encodePacked(referrer)) != keccak256(abi.encodePacked(userId)), "Invalid referrer");
   scpay.transferFrom(msg.sender, recipient, SCTxn[paymentMethod][durationMonths]); 
    
    
        referUser(referrer,durationMonths);
        referUser(userId,durationMonths);
       referralCount[referrer]++;
       referralCount[userId]++;
       referrers[userId] = referrer;

    
    if(isWAGMIPro[userId]==0 || isWAGMIPro[userId]<=block.timestamp){
    isWAGMIPro[userId]=block.timestamp+86400*((30*durationMonths>365)?365:30*durationMonths);
    }else{
      isWAGMIPro[userId]=isWAGMIPro[userId]+86400*((30*durationMonths>365)?365:30*durationMonths);
    }


     emit scComplete(userId, SCTxn[paymentMethod][durationMonths]);
     wagmiProCounter.increment();
 ...
 function manualWAGMIPro(uint durationMonths, string memory userId, string memory referrer) public onlyRole(AUTHORIZED_ROLE) nonReentrant returns(bool){
require(keccak256(abi.encodePacked(referrer)) != keccak256(abi.encodePacked(userId)), "Invalid referrer");
                  require(!blacklisted[userId], "Address is blacklisted");
...
return true;
 ...
 function backendWAGMIPro(uint durationMonths, string memory referrer, string memory userId, uint nonce, string memory receipt) onlyRole(AUTHORIZED_ROLE) nonReentrant public returns(bool){
require(keccak256(abi.encodePacked(referrer)) != keccak256(abi.encodePacked(userId)), "Invalid referrer");
       require(backendNonce[userId]<nonce && backendReceipt[receipt]==false,"Already claimed");
       require(!blacklisted[userId], "Address is blacklisted");

       backendNonce[userId]=nonce;
  ...
  emit backendSCComplete(userId,nonce,receipt);
  wagmiProCounter.increment();
...
//   }
  ...
  ///fix NFT staking to ensure we check if user is WAGMIPro
   function referUser(string memory _referrer, uint durationMonths) private {
        ...
        if (users[_referrer].totalSparks > 0 && block.timestamp <= users[_referrer].endDate) {
            ...
            uint256 alreadyEarnedSparks = ((block.timestamp - users[_referrer].startDate)/86400) * users[_referrer].dailySparks;
      ...
      function getSparksRate(string memory userId) public view returns(uint){
  
    return (block.timestamp<users[userId].endDate)?users[userId].dailySparks:0;
    }
function getIsWAGMIPro(string memory userId) public view virtual returns(uint){

    return isWAGMIPro[userId];
}
    function getReferralCount(string memory userId)public view virtual returns(uint){

    return referralCount[userId];
}
...
function getWagmiProCounter() public view returns (uint256) {
    return wagmiProCounter.current();
}
// Withdraw Ether
...
function blacklistUser(string memory userId) public onlyRole(AUTHORIZED_ROLE) {
        require(!blacklisted[userId], "Address already blacklisted");

        string memory referrer = referrers[userId]; // Get the referrer
        // Check if userId had contributed to referrer's sparks
        if (users[userId].totalSparks > 0) {
        ...
        // Reset userId details
        users[userId] = User(0, 0, 0, 0);
        isWAGMIPro[userId] = block.timestamp;

        // Add wallet to blacklist
        blacklisted[userId] = true;
    }
function unbanUser(string memory userId) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(blacklisted[userId], "Address not blacklisted");
