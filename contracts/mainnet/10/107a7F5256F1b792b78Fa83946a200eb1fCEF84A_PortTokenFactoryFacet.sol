...
contract PortTokenFactoryFacet is Modifiers {
    ...
    @param _name token name
    @param _symbol token symbol
    @param _feeSettings LibFee.TokenFeeSettings fee settings struct
    ...
    */
    function createAndInitPortToken(
        string calldata _name,
        string calldata _symbol,
        LibFee.TokenFeeSettings memory _feeSettings,
        ...
        LibPortTokenFactory.initPortToken(msg.sender, portTokenAddr, _trackedTokens, _amounts);
        LibFee.initFees(portTokenAddr, _feeSettings, LibPriceInterface.getTokensTotalPrice(_trackedTokens, _amounts));
    }
    ...
    */
    function createAndInitPortTokenFromRatios(
        ...
        LibPortTokenFactory.initPortToken(msg.sender, portTokenAddr, _trackedTokens, amounts);
        LibFee.initFees(portTokenAddr, _feeSettings, _targetPrice);
    }
    ...
    @param _name token name
    @param _symbol token symbol
    @param _feeDistributor address where to transfer referral fee
    @param _referralFee referral fee value
    @param _feeSettings LibFee.TokenFeeSettings fee settings struct
    @param _targetPrice target price of one portfolio token at the creation moment
    @param _trackedTokens list of tracked token addresses
    @param _ratios corresponding tracked token value ratios multiplied by SHARE_DENOMINATOR 
    */
    function createAndInitReferralPortTokenFromRatios(
        string calldata _name,
        string calldata _symbol,
        ...
        require(LibReferralToken.isAllowedManager(msg.sender), "Not allowed to create referral tokens");
        require(LibReferralToken.isAllowedFeeDistributor(_feeDistributor), "Fee distributor not allowed");

        portTokenAddr = LibPortTokenFactory.createPortToken(msg.sender, _name, _symbol);
        LibReferralToken.configureToken(portTokenAddr, true, _referralFee, _feeDistributor);
