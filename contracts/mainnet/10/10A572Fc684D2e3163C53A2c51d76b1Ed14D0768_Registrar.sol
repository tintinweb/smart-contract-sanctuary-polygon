...
contract Registrar is ERC721, Ownable, ReentrancyGuard {
    ...
    function setPrimaryDomain(uint256 _domainId) public {
        require(
            _isApprovedOrOwner(msg.sender, _domainId),
            "Caller is not owner nor approved"
        );
        addressToPrimaryDomain[msg.sender] = _domainId;
    }

    function getPrimaryDomain(address owner) external view returns (uint256) {
        return addressToPrimaryDomain[owner];
    }
    ...
    function register(string memory _domain) public payable nonReentrant {
        ...
        require(
            ...
            "Domain should only contain letters and numbers"
        );
