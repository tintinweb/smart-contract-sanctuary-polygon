...
import { IAutobribe } from "../interfaces/IAutobribe.sol";
...
contract VaultCore is VaultStorage {
    ...
    modifier onlyCashMetaStrategy() {
        require(
            msg.sender == cashMetaStrategy,
            "Caller is not the CASH meta strategy"
        );
        _;
    }
    ...
    function mint(
        ...
        address _depositFeeAddress = depositFeeAddress;
        IERC20 asset = IERC20(_asset);
        if (mintFeeBps > 0 && _depositFeeAddress != address(0) && !exemptFromFees[msg.sender]) {
            uint256 amount = _amount;
            _amount = _amount.sub(_amount.mul(mintFeeBps).div(10000));
            asset.safeTransferFrom(msg.sender, _depositFeeAddress, amount.sub(_amount));
        }
    ...
    /**
     * @dev Mint CASH for CASH Meta Strategy
     * @param _amount Amount of the asset being deposited
     *
     * Notice: can't use `nonReentrant` modifier since the `mint` function can
     * call `allocate`, and that can trigger `ConvexcASHMetaStrategy` to call this function
        ...
        onlyCashMetaStrategy
    {
        ...
        netCashMintedForStrategy += int256(_amount);

        require(
            abs(netCashMintedForStrategy) < netCashMintForStrategyThreshold,
            "Minted CASH surpassed netCashMintForStrategyThreshold."
        );

        // Mint matching CASH
        cASH.mint(msg.sender, _amount);
    }
    ...
    function _redeem(uint256 _amount, uint256 _minimumUnitAmount) internal {
        ...
        for (uint256 i = 0; i < allAssets.length; i++) {
            ...
            //send redeem fee to treasury
            address _withdrawFeeAddress = withdrawFeeAddress;
            if (redeemFeeBps > 0 && _withdrawFeeAddress != address(0) && !exemptFromFees[msg.sender]) {
                uint256 _redeemFee = outputs[i].mul(redeemFeeBps).div(10000);
                outputs[i] = outputs[i].sub(_redeemFee);
                asset.safeTransfer(_withdrawFeeAddress, _redeemFee);
            }
    ...
    /**
     * @dev Burn CASH for CASH Meta Strategy
     * @param _amount Amount of CASH to burn
     *
     * Notice: can't use `nonReentrant` modifier since the `redeem` function could
     * require withdrawal on `ConvexcASHMetaStrategy` and that one can call `burnForStrategy`
        ...
        onlyCashMetaStrategy
    {
        ...
        require(
            abs(netCashMintedForStrategy) < netCashMintForStrategyThreshold,
            "Attempting to burn too much CASH."
        );
    ...
    function _rebase() internal whenNotRebasePaused {
        ...
        address _performanceFeeAddress = performanceFeeAddress; // gas savings
        if (_performanceFeeAddress != address(0) && (vaultValue > cashSupply)) {
            uint256 yield = vaultValue.sub(cashSupply);
            uint256 fee = yield.mul(trusteeFeeBps).div(10000);
            require(yield > fee, "Fee must not be greater than yield");
            if (fee > 0) {
                cASH.mint(_performanceFeeAddress, fee);
            }
            emit YieldDistribution(_performanceFeeAddress, yield, fee);
        }

        cashSupply = cASH.totalSupply();

        // autobribe
        address _autobribeAddress = autobribeAddress; // gas savings
        if(_autobribeAddress != address(0) && vaultValue > cashSupply){
            cASH.mint(_autobribeAddress, IAutobribe(_autobribeAddress).getCashForBribes(vaultValue, cashSupply));
            IAutobribe(_autobribeAddress).distributeCashToBribes();
        }
        ...
        returns (uint256[] memory outputs, uint256 totalBalance)
    {
        ...
        // totalOutputRatio.
        //
        // For example, withdrawing: 30 CASH:
...
interface IVault {
    ...
    function depositFeeAddress() external view returns (address);
    function withdrawFeeAddress() external view returns (address);
    ...
    function cashMetaStrategy() external view returns (address);
    ...
    function netCashMintForStrategyThreshold() external view returns (uint256);

    function setCashMetaStrategy(address _cashMetaStrategy) external;

    function setNetCashMintForStrategyThreshold(uint256 _threshold) external;

    function netCashMintedForStrategy() external view returns (int256);
...
interface IAutobribe {
    function getCashForBribes(uint256, uint256) external view returns (uint256);
    function distributeCashToBribes() external;
}
...
contract VaultStorage is Initializable, Governable {
    ...
    event RebaseUnpaused();
    event VaultBufferUpdated(uint256 _vaultBuffer);
    event CashMetaStrategyUpdated(address _cashMetaStrategy);
    event RedeemFeeUpdated(uint256 _redeemFeeBps);
    event PriceProviderUpdated(address _priceProvider);
    event AllocateThresholdUpdated(uint256 _threshold);
    event RebaseThresholdUpdated(uint256 _threshold);
    ...
    event TrusteeFeeBpsChanged(uint256 _basis);
    event TrusteeAddressChanged(address _address);
    event NetCashMintForStrategyThresholdChanged(uint256 _threshold);
    ...
    address public autobribeAddress;
    ...
    // Meta strategy that is allowed to mint/burn CASH without changing collateral
    address public cashMetaStrategy = address(0);

    // How much CASH is currently minted by the strategy
    int256 public netCashMintedForStrategy = 0;

    // How much net total CASH is allowed to be minted by all strategies
    uint256 public netCashMintForStrategyThreshold = 0;
