...
contract MultichainFacet is PlexSwap, ReentrancyGuard {
    ...
    function multiChainBridge(MultiChainDescription calldata mDesc) public payable nonReentrant {
    ...
    function swapMultichain(SwapData calldata _swap, MultiChainDescription calldata mDesc) external payable nonReentrant {
        ...
        if (!s.allowedRouter[mDesc.router]) revert();
    ...
    function _multiChainBridgeStart(uint256 amount, MultiChainDescription calldata mDesc) internal {
        Storage storage s = getStorage();
        address anyToken = s.anyTokenAddress[mDesc.srcToken];
        ...
        if (mDesc.router == anyToken) {
            ...
            if (_isNative(IERC20(mDesc.srcToken))) {
                IBridge(mDesc.router).anySwapOutNative{value: amount}(anyToken, mDesc.receiver, mDesc.dstChainId);
            } else {
