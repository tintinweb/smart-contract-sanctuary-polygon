...
abstract contract FlightStandards is AccessControl, Ownable, ERC2981  {
  ...
  function updateRoyaltyInfo(address _receiver, uint96 _royaltyFeesInBips) public {
    require(block.timestamp > start + period * 1 days, "Period has not ended!");
  ...
  function transferOwnership(address newOwner) public override {
    if (block.timestamp > start + period * 1 days) {
      require(hasRole(CREATOR, msg.sender), "Caller is not the creator!");
      require(newOwner != address(0),"Ownable: new owner is the zero address");
      _transferOwnership(newOwner);
    } else {
...
import './FlightStandards.sol';

contract HesterERC721A is ERC721AQueryable, FlightStandards, ReentrancyGuard, DefaultOperatorFilterer {
  ...
  constructor(
    ...
    _grantRole(LICENSEE, msg.sender);
    _grantRole(CREATOR, _client);
    _grantRole(WITHDRAW_ROLE, msg.sender);
    _grantRole(WITHDRAW_ROLE, _client);
    ...
    setRoyaltyInfo(_payments, _royaltyFeesInBips);
  ...
  function supportsInterface(bytes4 interfaceId)
    public
    view
    override(ERC721A, FlightStandards, IERC165)
    returns (bool)
  {
    return super.supportsInterface(interfaceId);
  }
  ...
  function contractURI() public pure returns (string memory) {
    return "https://nsu-bo-hester.s3.us-east-2.amazonaws.com/contractURI.json";
  }
}
