...
contract uRevolt is Initializable, UUPSUpgradeable, ERC20Upgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    // mapping of users lock deposit
    mapping(address => uint) public userLockInfo;
    // governance contract address
    address public governanceAddress;
    // last cycle initiate time
    uint public lastCycleInitiated;
    // opt pool contract address
    address public optInContract;
    ...
    function updateGovernanceAddress(address _governanceAddress) external onlyOwner {
        require(_governanceAddress != address(0), "Invalid governance Address");
        governanceAddress = _governanceAddress;
    }

    // update user lock info
    function updateLockInfo(address _user, uint _lockTime) external {
        require(msg.sender == governanceAddress, "Invalid msg sender");
        userLockInfo[_user] = _lockTime;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external randomGenerationCompleted nonReentrant{
        require(_pid == 0,"Wrong pool Id");
        require(userLockInfo[msg.sender] <= block.timestamp, "Locked until proposal period");
        _withdraw(_pid,msg.sender,_amount);
        if(iCultMandator(msg.sender) && _amount > 0 && optInContract != address(0)){
            UserInfo storage user = userInfo[lastFulfilledId][msg.sender];
            _withdraw(lastFulfilledId, msg.sender, user.amount);
            IOptInCultMandator(optInContract).removeOptInUser(msg.sender);
            IOptInCultMandator(optInContract).updateCultmandatorStatus(lastFulfilledId, msg.sender, false);
    ...
    function updateCultMandatorsReward(uint256 _depositAmount) external onlyTreasury{
        ...
        if(lastCycleInitiated.add(uint(259200)) < block.timestamp) {
            ...
            totalRVLTRewardMandators[lastFulfilledId.sub(1)] = totalRVLTRewardMandators[lastFulfilledId.sub(1)].add(_depositAmount);
            emit CultMandatorsReward(lastFulfilledId.sub(1), _depositAmount);
        }
    }
    ...
    function iCultMandator(address _userAddress) public view returns(bool){
        if(optInContract == address(0)) {
            return is_mandator[lastFulfilledId][_userAddress];
        } else {
            return IOptInCultMandator(optInContract).checkCultmandatorStatus(lastFulfilledId, _userAddress);
        }
    }
    ...
    function initializeRandomAddressSelection(uint256 _lastFulfilledId, uint256[] memory _randomWords, address[] memory _nftOwners) external onlyRandomNumGenerator nonReentrant{
        ...
        IOptInCultMandator(optInContract).startRandomSelection(_randomWords, _lastFulfilledId);
        for(uint256 i =0; i < _nftOwners.length; i++ ){
            IOptInCultMandator(optInContract).updateCultmandatorStatus(lastFulfilledId, _nftOwners[i], true);
        }
        lastCycleInitiated = block.timestamp;
    }

    // cult manders deposit amount increase after voting
    function depositCultmanders(address _cultMander) external {
        require(msg.sender == governanceAddress, "Only governance contract can call");
        require(iCultMandator(_cultMander), "Only cultmanders can call");
        _deposit(lastFulfilledId, _cultMander, _cultMandatorStakeAmount);
    }
    ...
    function _randomSelectUsers(
        ...
        for(uint256 i=0; i< numberOfUsers; i++){
            ...
            if (
                ...
                _deposit(lastFulfilledId, stakerId[tmpValue], 0);
    ...
    // update OptIn CultManders contract address
    function updateOptInContractAddress(address _optInContract) external onlyOwner {
        optInContract = _optInContract;
    }

    // Enable all functionality called by optIn contract
    function enableStaking() external {
        require(msg.sender == optInContract, "Only Opt in cultmandator contract can call");
        isGenerationStarted = false;
    }
}

interface IOptInCultMandator {
    function startRandomSelection(uint256[] memory _randomWords, uint256 _lastFulfilledId) external;
    function checkCultmandatorStatus(uint256 _lastFulfilledId, address _userAddress) external view returns (bool);
    function removeOptInUser(address _userAddress) external;
    function updateCultmandatorStatus(uint256 _lastFulfilledId, address _userAddress, bool _status) external;
}
