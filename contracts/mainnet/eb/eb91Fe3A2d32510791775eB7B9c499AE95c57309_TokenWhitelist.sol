 ...
 */
contract TokenWhitelist is ITokenWhitelist, StandardAccessControlEnumerable {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    /**
     * @notice Constructs the TokenWhitelist contract
    ...
    function addToWhitelist(address newToken) external override onlyMaintainer {
        require(tokens.add(newToken), "Token already supported");
        emit AddedToWhitelist(newToken);
    }

    /**
     * @notice Adds bulk addresses to the whitelist.
     * @param newTokens the new address to add.
     */
    function addBulkToWhitelist(
        address[] calldata newTokens
    ) external onlyMaintainer {
        for (uint i; i < newTokens.length; i++) {
            require(tokens.add(newTokens[i]), "Token already supported");
            emit AddedToWhitelist(newTokens[i]);
        }
    }
    ...
    function removeFromWhitelist(
        ...
        require(tokens.remove(tokenToRemove), "Token not supported");
