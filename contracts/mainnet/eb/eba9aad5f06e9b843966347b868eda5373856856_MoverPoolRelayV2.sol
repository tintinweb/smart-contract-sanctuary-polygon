...
// Interface to represent asset pool interactions
interface IMoverPoolRelay {
    function depositOnBehalf(address _beneficiary, address _poolAddress, address _token, uint256 _amount) external;

    function swapAndDepositOnBehalf(address _beneficiary, address _poolAddress, address _tokenFrom, uint256 _amountFrom, address _tokenTo, bytes memory _swapData) external;
}
...
/*
   MoverPoolRelay is a contract that holds (briefly) assets to be deposited into pool or rebalanced
   - after bridge transactions output point, backend executor would make gasless deposit in this chain on user's behalf;
   - would be used for rebalancing assets between chains (when strategy is on another chain);
   - has means to convert funds using curve or 0x mesh to avoid stuck/misbridged funds;

   Pool address and token address are method parameters, so a single relay can serve multiple pools/tokens
   Contains no public write functions

   V2: added payable modifiers as bridge started senging tiny fractions of native token for gas
*/
contract MoverPoolRelayV2 is AccessControlUpgradeable, SafeAllowanceResetUpgradeable, EmergencyTimelock, IMoverPoolRelay {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    bytes32 public constant FINMGMT_ROLE = keccak256("FINMGMT_ROLE"); // role that grants financial settings (fees, etc.)
    bytes32 public constant TRUSTED_EXECUTION_ROLE = keccak256("TRUSTED_EXECUTION_ROLE");  // trusted execution wallets

    address public transferProxy;   // Mover transfer proxy
    address public exchangeProxy;   // Mover exchange proxy
    ...
    function setExchangeProxy(address _exchangeProxy) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "admin only");
        exchangeProxy = _exchangeProxy;
    }

    // deposit funds bridged to this contract
    // called by backend to arrange deposit
    function depositOnBehalf(address _beneficiary, address _poolAddress, address _token, uint256 _amount)
        public
        override
    {
        require(hasRole(TRUSTED_EXECUTION_ROLE, msg.sender), "trusted executor only");
        resetAllowanceIfNeeded(IERC20Upgradeable(_token), transferProxy, _amount);
        ITransferProxyV2(transferProxy).depositToPoolFromRelay(_beneficiary, _poolAddress, _token, _amount);
    }

    function swapAndDepositOnBehalf(address _beneficiary, address _poolAddress, address _tokenFrom, uint256 _amountFrom, address _tokenTo, bytes memory _swapData)
        public
        override
    {
        require(hasRole(TRUSTED_EXECUTION_ROLE, msg.sender), "trusted executor only");
        resetAllowanceIfNeeded(IERC20Upgradeable(_tokenFrom), exchangeProxy, _amountFrom);
        uint256 amountReceived = IExchangeProxyV2(exchangeProxy).executeSwap(_tokenFrom, _tokenTo, _amountFrom, _swapData);
        ITransferProxyV2(transferProxy).depositToPoolFromRelay(_beneficiary, _poolAddress, _tokenTo, amountReceived);
    }

    // swapUsingCurve function is to swap from stablecoin to stablecoin if bridged assets do not match
    // expected amount defines max slippage and must take decimals into consideration (could be zero to skip check)
    function swapUsingCurve(address _tokenFrom, uint256 _amountFrom, address _curvePool, uint256 _indexFrom, uint256 _indexTo, uint256 _expected) public {
        require(hasRole(TRUSTED_EXECUTION_ROLE, msg.sender), "trusted executor only");
        resetAllowanceIfNeeded(IERC20Upgradeable(_tokenFrom), _curvePool, _amountFrom);
        ICurvePool(_curvePool).exchange_underlying(_indexFrom, _indexTo, _amountFrom, _expected);
    }

    // payable transfer and fallback functions to allow this contract to receive ETH when swapping to raw ETH and protocol fee refunds.
