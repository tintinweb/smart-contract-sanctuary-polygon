...
contract AggregatorRTQuest is AccessControl {
    event NewOperator(address indexed operator);

    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");

    IBhavishRealTimePrediction public lossyRT;
    IBhavishRealTimePrediction public noLossRT;
    IBhavishQuest public lossyQ;
    IBhavishQuest public noLossQ;
    ...
    constructor(
        IBhavishRealTimePrediction _lossyRT,
        IBhavishRealTimePrediction _noLossRT,
        IBhavishQuest _lossyQ,
        IBhavishQuest _noLossQ
    ) {
        require(address(_lossyRT) != address(0) && address(_noLossRT) != address(0), "invalid markets");
        lossyRT = _lossyRT;
        noLossRT = _noLossRT;
        lossyQ = _lossyQ;
        noLossQ = _noLossQ;
    ...
    function createMarket(
        IBhavishRealTimePrediction.MarketQuestion memory _question,
        uint256 _opensAt,
        uint256 _closesAt,
        bool _singleAnswer,
        bool _arbitrator
    ) external onlyOperator(msg.sender) {
        lossyRT.createMarket(_question, _opensAt, _closesAt, _singleAnswer, false, _arbitrator);
        noLossRT.createMarket(_question, _opensAt, _closesAt, _singleAnswer, false, _arbitrator);
    }
    ...
    function createQuest(
        ...
        lossyQ.createQuest(_questions, _opensAt, _closesAt, _title, _category, _description, _image, _arbitrator);
        noLossQ.createQuest(_questions, _opensAt, _closesAt, _title, _category, _description, _image, _arbitrator);
    }
