...
import "../interfaces/algebra/IStrategyRebalanceStakerAlgebra.sol";

contract AlgebraV3Vault is Initializable, ERC20Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  using SafeERC20Upgradeable for IERC20Upgradeable;

  address public constant wmatic = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
  address payable public constant ALGEBRA_ROUTER = payable(0xf5b509bB0909a69B1c207E495f687a596C168E12);
  ...
  function liquidityOfThis() public view returns (uint256) {
    ...
    return
      uint256(
        algebraCalculator.liquidityForAmounts(sqrtRatioX96, _balance0, _balance1, getLowerTick(), getUpperTick())
      );
  }
  ...
  function deposit(uint256 token0Amount, uint256 token1Amount) external payable nonReentrant whenNotPaused {
    // IStrategyRebalanceStakerAlgebra(address(IControllerV7(controller).strategies(address(pool)))).harvest();
    bool _maticUsed;
  ...
  function getProportion() public view returns (uint256) {
    (uint160 sqrtRatioX96, , , , , , ) = pool.globalState();
    (uint256 a1, uint256 a2) = algebraCalculator.amountsForLiquidity(
      sqrtRatioX96,
      1e18,
      getLowerTick(),
      getUpperTick()
  ...
  function withdraw(uint256 _shares) public nonReentrant whenNotPaused {
    ...
    (uint160 sqrtRatioX96, , , , , , ) = pool.globalState();
    (uint256 _expectA0, uint256 _expectA1) = algebraCalculator.amountsForLiquidity(
      sqrtRatioX96,
      uint128(r),
      getLowerTick(),
      getUpperTick()
  ...
  /**
   * @notice  Wrap matic to wmatic and put it as token amount
   * @param   token0Amount  Amount of token0 if matic
   * @param   token1Amount  Amount of token1 if matic
   * @return  uint256  Amount of token0 (is > 0 if wmatic is token0)
   * @return  uint256  Amount of token1 (is > 0 if wmatic is token1)
    ...
    )
  {
    ...
    if (_matic > 0) {
      IWETH(wmatic).deposit{value: _matic}();
  ...
  /**
   * @notice  Refunds matic to user by unwrapping wmatic to matic
   * @param   _refund  amount to withdraw
   */
  function _refundMatic(uint256 _refund) internal {
    IWETH(wmatic).withdraw(_refund);
  ...
  function _refundUnused(bool _maticUsed) internal returns (uint256) {
    ...
    uint160 sqrtRatioAX96 = algebraCalculator.getSqrtRatioAtTick(getLowerTick());
    uint160 sqrtRatioBX96 = algebraCalculator.getSqrtRatioAtTick(getUpperTick());

    _cache.liquidity = uint128(
      algebraCalculator.getLiquidityForAmount0(sqrtRatioAX96, sqrtRatioBX96, _cache.amount0Desired) +
        (algebraCalculator.getLiquidityForAmount1(sqrtRatioAX96, sqrtRatioBX96, _cache.amount1Desired))
    );

    (_cache.amount0, _cache.amount1) = algebraCalculator.getAmountsForLiquidity(
      sqrtPriceX96,
      sqrtRatioAX96,
      sqrtRatioBX96,
      _cache.liquidity
    ...
    if (_cache.amount0Desired > _cache.amount0)
      if ((address(token0) == address(wmatic)) && _maticUsed) _refundMatic(_cache.amount0Desired - (_cache.amount0));
      else {
        token0.safeTransfer(msg.sender, _cache.amount0Desired - (_cache.amount0));
      }

    if (_cache.amount1Desired > _cache.amount1)
      if ((address(token1) == address(wmatic)) && _maticUsed) _refundMatic(_cache.amount1Desired - (_cache.amount1));
      else {
  ...
  function _balanceProportion(int24 _tickLower, int24 _tickUpper) internal {
    ...
    (uint160 sqrtPriceX96, , , , , , ) = pool.globalState();
    uint160 sqrtRatioAX96 = algebraCalculator.getSqrtRatioAtTick(_tickLower);
    uint160 sqrtRatioBX96 = algebraCalculator.getSqrtRatioAtTick(_tickUpper);
    ...
    if (_amountSpecified > 0) {
      ...
      IERC20Upgradeable(_inputToken).safeApprove(ALGEBRA_ROUTER, 0);
      IERC20Upgradeable(_inputToken).safeApprove(ALGEBRA_ROUTER, _amountSpecified);
