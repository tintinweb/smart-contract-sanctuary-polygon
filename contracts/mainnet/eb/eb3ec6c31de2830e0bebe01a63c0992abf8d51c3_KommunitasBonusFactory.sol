...
interface IKommunitasBonusFactory{
    ...
    function createProject(address, uint, uint, uint, uint, uint, uint[2] memory) external returns (address);
...
contract KommunitasBonusProject{
    using SafeMath for uint;
    ...
    IKommunitasBonusFactory public factory = IKommunitasBonusFactory(msg.sender);
    ...
    modifier isBoosterProgress{
        require(boosterProgress() == 1, "Not yet");
        _;
    }
     ...
     * @param _price Token project price (in 6 decimals)
     * @param _minMaxBuy Min and max token to buy (in 2 decimals)
     */
    function initialize(
        address _payment,
        uint _sale,
        ...
        uint _start,
        uint _price,
        uint[2] memory _minMaxBuy
    ) public {
        require(msg.sender == address(factory) && !initialized, "Not good");
        ...
        booster[1].start = _start;
        booster[1].price = _price.mul(10**payment.decimals()).div(1e6);

        minBuy = _minMaxBuy[0].mul(10**18).div(100);
        maxBuy = _minMaxBuy[1].mul(10**18).div(100);
    ...
    function boosterProgress() public view returns (uint running){
        running = 0;
        if(block.timestamp >= booster[1].start){
            running = 1;
        }
    }
    ...
    function amountInCalc(uint _tokenReceived, uint _amountIn, address _user) internal view returns(uint amountInFinal, uint tokenReceivedFinal){
        ...
        // }
        _amountIn = _tokenReceived.mul(booster[boosterProgress()].price).div(10 ** 18);
    ...
    function amountInCalcInner(address _user, uint _tokenReceived, uint _amountIn) internal view returns(uint amountInFinal, uint tokenReceivedFinal){
        ...
        // } else{
            require(_tokenReceived >= minBuy, "You buy less than minBuy");
            uint alloc = maxBuy;
        // }
        ...
        if(purchasePerRound[_user][boosterProgress()].add(_tokenReceived) > alloc){
            amountInFinal = (alloc.sub(purchasePerRound[_user][boosterProgress()])).mul(booster[boosterProgress()].price).div(10 ** 18);
        }

        require(amountInFinal > 0, "Your buy amount is too small");
        tokenReceivedFinal = amountInFinal.mul(10 ** 18).div(booster[boosterProgress()].price);
    }
    ...
    function buyTokenByETH(address[] calldata _path) payable isBoosterProgress isNotPaused public {
        require(_path[0] == factory.weth() && _path[1] == address(payment), "Not good");
        require(getUserAllocToken(msg.sender) > 0 && v2Staked > 0, "No staked");
        
        uint buyerId = setBuyer(msg.sender);
        
        uint amountOut = getAmountOut(_path[0], msg.value);
        
        uint tokenReceived = amountOut.mul(10 ** 18).div(booster[boosterProgress()].price);
    ...
    function buyTokenByToken(uint _amountIn, address[] calldata _path) isBoosterProgress isNotPaused public {
        require(_path[0] != address(payment) && _path[0] == factory.allPayments(factory.getPaymentIndex(_path[0])) && _path[1] == address(payment), "Not good");
        require(getUserAllocToken(msg.sender) > 0 && v2Staked > 0, "No staked");
    ...
    function buyToken(uint _amountIn, address _tokenIn) isBoosterProgress isNotPaused public {
        require(_tokenIn == address(payment), "Not good");
        require(getUserAllocToken(msg.sender) > 0 && v2Staked > 0, "No staked");
        
        uint buyerId = setBuyer(msg.sender);
        
        uint tokenReceived = _amountIn.mul(10 ** 18).div(booster[boosterProgress()].price);
     ...
     * @param _minMaxBuy Min and max token to buy (in 2 decimals)
     */
    function setMinMax_d2(uint[2] memory _minMaxBuy) public onlyOwner{
        require(boosterProgress() != 1, "Started");
...
contract KommunitasBonusFactory is IKommunitasBonusFactory{
    ...
    constructor(
        ...
        require(_swapFactory != address(0) && _weth != address(0) && _devAddr != address(0) && _savior != address(0) && _stakingV1 != address(0) && _stakingV2 != address(0));
    ...
    function createProject(
        ...
        project = address(new KommunitasBonusProject());

        allProjects.push(project);
        
        KommunitasBonusProject(allProjects[allProjects.length-1]).initialize(
            _payment,
            _sale,
            _target,
            _calculation,
     ...
     */
    function config(address _swapFactory, address _devAddr, address _savior) public override onlyOwner{
        require(_swapFactory != address(0) && _devAddr != address(0) && _savior != address(0), "Can't set to address(0)");
        swapFactory = _swapFactory;
        devAddr = _devAddr;
        savior = _savior;
    }
