...
contract NetlyFans is ERC721, Ownable, DefaultOperatorFilterer, ReferralStorage {
    ...
    uint public price = 372000000000000000000; // 350
    ...
    constructor() ERC721("Netlyfans", "NF") {
        baseUri = "ipfs://bafybeice5vfxevae3lhlmftyi2lhjuq742gh5fvu6c77v7f7jlcpclgat4/";
    }
    ...
    function withdrawAll() external payable onlyOwner {
        ...
        uint256 balanceThree = balance * 29 / 100;
        require(payable(feeW1).send(balanceOne), "Transfer failed.");
        require(payable(feeW2).send(balanceTwo), "Transfer failed.");
        require(payable(feeW3).send(balanceThree), "Transfer failed.");
}


    function withdrawFees(address _receiver, uint256 _amount) internal {
        require(payable(_receiver).send(_amount), "Transfer failed.");
}
