...
contract LikidaFounders is ERC1155, AccessControl, Pausable, ReentrancyGuard, ERC1155Burnable, ERC1155Supply {

  /// The public name of this contract.
    string public constant name = "Likida Founders";
    string public constant symbol = "LIKF";
    ...
    uint256 public constant ARMY  = 2;
    uint256 public constant LIKID = 3;
    ...
    mapping(address => uint256) private refeeredAddress;

    constructor() ERC1155("http://test-likida.io/token/{id}.json") {
        ...
        // Token limit
        tokenSupply[uint32(BASIC)] = 5000;
        tokenSupply[uint32(ARMY)]  = 3000;
        tokenSupply[uint32(LIKID)] = 500;
    }
    ...
    // Likida PS functions
        ...
        function giftBasics(uint256 _id) private whenNotPaused{
            if(_id == LIKID && !presaleEnded){
                _mint(msg.sender, BASIC, 2, "");
            }
        }

        function giftBasicsBatch(uint256[] memory ids, uint256[] memory amounts) private whenNotPaused{
            for(uint256 i = 0; i < ids.length; i++){
                if(uint32(ids[i]) == LIKID && !presaleEnded){
                    for(uint256 j = 0; j<uint32(amounts[i]); j++){
                        _mint(msg.sender, BASIC, 2, "");
                    }
                }
            }
        ...
        function tokenMintReferee(uint256 _id, address _referee) public payable nonReentrant whenNotPaused {
            require(msg.sender != _referee, "You cant refeer yourself");

            uint256 amoutReferee = balanceOf(_referee, BASIC);
            amoutReferee += balanceOf(_referee, ARMY);
            amoutReferee += balanceOf(_referee, LIKID);

            require(amoutReferee > 0, "The referee hasnt bought any NFT");
            require(tokenSupply[uint32(_id)] > 0, "Token supply is under 1");
            require(tokenPrice[uint32(_id)] > 0, "Token price is under 1");
            require(msg.value == tokenPrice[uint32(_id)], "Please submit the asking price in order to complete the purchase");
            ...
            giftBasics(_id);
            payable(_referee).transfer((5*msg.value)/100);
            refeeredAddress[_referee] += 1;
        ...
        function tokenMintBatchReferee(uint256[] memory ids, uint256[] memory amounts, address _referee) public payable nonReentrant whenNotPaused {
            require(msg.sender != _referee, "You cant refeer yourself");
            ...
            payable(_referee).transfer((5*msg.value)/100);
            refeeredAddress[_referee] += amounts.length;
    ...
    function getRefeeredNumber(address _address) public view returns(uint256){
        return refeeredAddress[_address];
    }
