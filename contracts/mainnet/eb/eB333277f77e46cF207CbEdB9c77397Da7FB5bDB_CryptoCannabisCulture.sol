 ...
 * This contract was inspired by BoredApeYachtClub contract.
 */
contract CryptoCannabisCulture is ERC721, Ownable, RoyaltiesV2Impl {
    using SafeMath for uint256;

    string public cBud_PROVENANCE = "";

    uint256 public startingIndexBlock;

    uint256 public startingIndex;

    uint256 public cBudPrice = 13 ether;
     ...
     * Set some CCCs aside
     */
    function reserveCryptoCannabisCulture() public onlyOwner {
    ...
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        cBud_PROVENANCE = provenanceHash;
    }
    ...
    /**
     * Mints CryptoCannabisCulture
     */
    function mintCBUD(uint256 numberOfTokens) public payable {
        ...
        require(
            totalSupply().add(numberOfTokens) <= Total,
            "Purchase would exceed max supply of Editions"
        );
        require(
            cBudPrice.mul(numberOfTokens) <= msg.value,
        ...
        if (
            ...
            (totalSupply() == Total || block.timestamp >= REVEAL_TIMESTAMP)
        ) {
    ...
    function setStartingIndex() public {
        ...
        startingIndex = uint256(blockhash(startingIndexBlock)) % Total;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint256(blockhash(block.number - 1)) % Total;
        }
