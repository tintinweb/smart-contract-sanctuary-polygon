...
contract HedgepieInvestor is ReentrancyGuard, HedgepieAccessControlled, IStargateReceiver {
    ...
    function deposit(
        ...
        require(adapterInfos.length == _stAmounts.length, "Error: Invalid stAmounts length");

        uint256 tStAmount;
        uint256 totalUsedBNB;
        uint256 i;
        for (i; i < _stAmounts.length; i++) tStAmount += _stAmounts[i];
        require(msg.value == _amountIn + tStAmount, "Error: Insufficient BNB");

        // 1. claim reward from adapters
        _calcReward(_tokenId);

        // 2. deposit to adapters
        for (i = 0; i < adapterInfos.length; i++) {
            IYBNFT.AdapterParam memory adapter = adapterInfos[i];
            uint256 amountIn = (_amountIn * adapter.allocation) / 1e4;

            if (adapter.isCross) {
                require(_stAmounts[i] != 0, "Error: Invalid stAmount");
                _crossDeposit(_tokenId, amountIn, _stAmounts[i], adapter);
            } else {
                totalUsedBNB += amountIn;
                if (amountIn != 0) IAdapter(adapter.addr).deposit{value: amountIn}(_tokenId);
            }
        }

        // 3. update user & token info saved in investor
        uint256 investedUSDT = (totalUsedBNB * HedgepieLibraryBsc.getBNBPrice()) / 1e18;
        ...
        // 5. emit events
        emit Deposited(msg.sender, authority.hYBNFT(), _tokenId, totalUsedBNB);
    }
    ...
    function _crossDeposit(
        uint256 _tokenId,
        uint256 _amountIn,
        uint256 _stAmount,
        IYBNFT.AdapterParam memory _adapter
    ) internal {
        IYBNFT.CAdapterInfo memory cAdapterInfo = IYBNFT(authority.hYBNFT()).cAdapterInfos(_tokenId, _adapter.addr);

        // Swap bnb to source token
        address[] memory path = IPathFinder(IHedgepieAuthority(authority).pathFinder()).getPaths(
            cAdapterInfo.srcSwapRouter,
            HedgepieLibraryBsc.WBNB,
            address(cAdapterInfo.srcToken)
        );
...

        {
            ...
            bytes memory bytesAddress = abi.encodePacked(_adapter.addr);
...
interface IYBNFT {
    ...
    function cAdapterInfos(uint256 tokenId, address adapter) external view returns (CAdapterInfo memory);
