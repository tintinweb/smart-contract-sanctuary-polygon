...
interface BEP20Base {
 ...
 contract CCNFT is ERC721 {
     ...
     uint256 counter;
     uint256 minimumPrice = 1000000000000000;
     mapping(address => uint) public ownerToken;
     
    bool activar = false;

     constructor()ERC721("NFT", "CCNFT") payable{
    ...
    function mint(address _address) public payable {
        require(activar == true, "isnt activate");
        require(msg.sender != addr, "Can be  0x");
        require(msg.value >= minimumPrice, "Can be less of the minimum");

        _mint(msg.sender, counter);

        ownerToken[_address] = counter;
    ...
    function SET_RECEIBER(address _address)public returns(bool){
    ...
    function SET_TOKEN(address _address)public onlyOwner{
        token = _address;
