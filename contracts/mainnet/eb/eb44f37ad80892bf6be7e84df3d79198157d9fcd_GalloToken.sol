...
contract GalloToken is Auth, Initializable, ERC20Upgradeable {
    /// @custom:oz-upgrades-unsafe-allow constructor
    string public US_SEC_EDGAR_CIK = "000123456";
    uint256 public max_supply = 200_000_000_000_000000000000000000;
    uint256 public supplied = 0;

    mapping(address => uint256) public mint_counter;
    ...
    function initialize() initializer public {
        __ERC20_init("TEST:Fowl Play Inc Security Token", "TGALLO");
    }

    function mint(address to, uint256 amount) public onlyRole("MINTER") {
        require((supplied + amount) <= max_supply, "Gallo:Exceed_Max_Supply");
        supplied += amount;
        _mint(to, amount);
        mint_counter[msg.sender] += amount;
    }

    function reset_counter() public {
        mint_counter[msg.sender] = 0;
    }
