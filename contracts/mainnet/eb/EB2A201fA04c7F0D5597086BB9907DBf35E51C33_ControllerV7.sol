...
contract ControllerV7 is IControllerV7 {
  ...
  function earn(address _token, uint256 _amount) public {
    ...
    address _want = IStrategyV2(_strategy).want();
    if (_want != _token) {
  ...
  /**
   * @dev Gets the balance of a strategy pool for a specific token.
   * @param _token Address of the token
   * @return Returns the balance of the pool for the token
   */
  function balanceOf(address _token) external view returns (uint256) {
    return IStrategyV2(strategies[_token]).balanceOf();
  }
