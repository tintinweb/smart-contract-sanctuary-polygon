...
contract QuickswapV3Converter is IAlgebraSwapCallback {
    ...
    function swap(
        ...
        IAlgebraPool pool = IAlgebraPool(
            factory.poolByPair(source, destination)
        );
    ...
    function algebraSwapCallback(
        ...
        (address source, address destination) = abi.decode(
            data,
            (address, address)
        );
        require(
            msg.sender == factory.poolByPair(source, destination),
            "Invalid caller"
        );
        IERC20(source).safeTransfer(
            msg.sender,
            uint256(source < destination ? amount0Delta : amount1Delta)
        );
    }
    ...
    function previewSwap(
        ...
        (amountOut, ) = quoter.quoteExactInputSingle(source, destination, value, 0);
    }
}
