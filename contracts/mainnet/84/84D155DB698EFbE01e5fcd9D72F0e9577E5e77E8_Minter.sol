...
contract Minter is IMinter {
  ...
  uint private constant _MIN_BASE = 1e18;
  ...
  mapping(address => bool) dungeonMinted;
  ...
  function onlyDungeon() internal view {
    require(controller.validDungeons(msg.sender), "Not dungeon");
  }
  ...
  function amountForDungeon(uint dungeonBiomeLevel, uint heroLvl) public view returns (uint) {
    require(dungeonBiomeLevel < 20, "Too high biome");
    uint totalSupply = Math.max(token.totalSupply(), 1);
    uint base = Math.min(Math.max(_TOTAL_SUPPLY_BASE * 1e18 / totalSupply, _MIN_BASE), 100e18);
    uint levelLog = ((dungeonBiomeLevel + 2) * 1e18).log2() - 1e18;
    base = base * levelLog / 1e18;
  ...
  function mintDungeonReward(uint dungeonBiomeLevel, uint heroLvl) external override returns (uint amount) {
    onlyDungeon();
    require(!dungeonMinted[msg.sender], "Already minted");

    amount = amountForDungeon(dungeonBiomeLevel, heroLvl);
    if (amount != 0) {
      token.mint(msg.sender, amount);
    }
    dungeonMinted[msg.sender] = true;
  }
