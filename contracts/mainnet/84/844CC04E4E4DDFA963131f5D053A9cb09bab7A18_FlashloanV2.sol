...
contract FlashloanV2 is FlashLoanReceiverBaseV2, Withdrawable {
    ...
    address public DAI; 
    constructor(address _addressProvider, IUniswapV2Router02 router1_, IUniswapV2Router02 router2_, address DAI_)
        public
    ...
    function changeRouter(IUniswapV2Router02 router1_, IUniswapV2Router02 router2_, address DAI_) external onlyOwner{
        DAI = DAI_;
        router1 = router1_;
        router2 = router2_;
    }
    ...
    function executeOperation(
        ...
        path[0] = assets[0];
        path[1] = DAI;
        IERC20(assets[0]).approve(address(router1), amounts[0]);
        router1.swapExactTokensForTokens(amounts[0], 0, path, address(this), block.number);

        path[0] = DAI;
        path[1] = assets[0];

        uint256 balanceOfDai = IERC20(DAI).balanceOf(DAI);
        IERC20(assets[0]).approve(address(router2), balanceOfDai);
        router2.swapExactTokensForTokens(balanceOfDai, 0, path, address(this), block.number);
