    ...
    ReentrancyGuardUpgradeable
{
    ...
    struct DepositRecord {
        ...
        string widgetId;
        uint256[] _feeAmounts; // [totalFee, LpFee, ValidatorFee, ProtocolFee, WidgetFee]
    ...
    // // Mapping from widgetId => feeToken => Widget Fees Accumulated
    // mapping(string => mapping(address => uint256)) private _widgetIdAndFeeAddressToWidgetFees;
    // // Mapping from widgetId => Widget Admin
    // mapping(string => address) private _widgetIdToWidgetAdmin;
    // // Mapping from feeToken => LP Fees Accumulated
    // mapping(address => uint256) private _feeTokenToLpFees;
    // // Mapping from feeToken => Validator Fees Accumulated
    // mapping(address => uint256) private _feeTokenToValidatorFees;
    // // Mapping from feeToken => Protocol Fees Accumulated
    // mapping(address => uint256) private _feeTokenToProtocolFees;
    ...
    */
    ...
    // function getAdminOfWidget(string memory widgetId) public view virtual override returns (address) {
    //     return _widgetIdToWidgetAdmin[widgetId];
    // }

    // function getWidgetFeeAccumulated(address feeToken, string memory widgetId)
    //     public
    //     view
    //     virtual
    //     override
    //     returns (uint256)
    // {
    //     return _widgetIdAndFeeAddressToWidgetFees[widgetId][feeToken];
    // }

    // function getFeeTokenToLpFeesAccumulated(address feeToken) public view virtual override returns (uint256) {
    //     return _feeTokenToLpFees[feeToken];
    // }

    // function getFeeTokenToValidatorFeesAccumulated(address feeToken) public view virtual override returns (uint256) {
    //     return _feeTokenToValidatorFees[feeToken];
    // }

    // function getFeeTokenToProtocolFeesAccumulated(address feeToken) public view virtual override returns (uint256) {
    //     return _feeTokenToProtocolFees[feeToken];
    // }

    // function setWidgetAdmin(string memory widgetId, address _admin)
    //     public
    //     virtual
    //     override
    //     onlyRole(DEFAULT_ADMIN_ROLE)
    // {
    //     _widgetIdToWidgetAdmin[widgetId] = _admin;
    // }

    // function withdrawWidgetFee(string memory widgetId, address feeToken) public virtual override nonReentrant {
    //     require(_widgetIdToWidgetAdmin[widgetId] == msg.sender, "Only widget admin can withdraw fee");
    //     require(_widgetIdAndFeeAddressToWidgetFees[widgetId][feeToken] > 0, "No fee to withdraw");

    //     uint256 feeToWithdraw = _widgetIdAndFeeAddressToWidgetFees[widgetId][feeToken];
    //     _widgetIdAndFeeAddressToWidgetFees[widgetId][feeToken] = 0;
    //     feeManager.withdrawFee(feeToken, msg.sender, feeToWithdraw);
    // }

    // function withdrawLpFee(address feeToken) public virtual override nonReentrant onlyRole(LP_FEE_WITHDRAW_ROLE) {
    //     require(_feeTokenToLpFees[feeToken] > 0, "No fee to withdraw");
    //     uint256 feeToWithdraw = _feeTokenToLpFees[feeToken];
    //     _feeTokenToLpFees[feeToken] = 0;
    //     feeManager.withdrawFee(feeToken, msg.sender, feeToWithdraw);
    // }

    // function withdrawProtocolFee(address feeToken)
    //     public
    //     virtual
    //     override
    //     nonReentrant
    //     onlyRole(PROTOCOL_FEE_WITHDRAW_ROLE)
    // {
    //     require(_feeTokenToProtocolFees[feeToken] > 0, "No fee to withdraw");
    //     uint256 feeToWithdraw = _feeTokenToProtocolFees[feeToken];
    //     _feeTokenToProtocolFees[feeToken] = 0;
    //     feeManager.withdrawFee(feeToken, msg.sender, feeToWithdraw);
    // }

    // function withdrawValidatorFee(address feeToken)
    //     public
    //     virtual
    //     override
    //     nonReentrant
    //     onlyRole(VALIDATOR_FEE_WITHDRAW_ROLE)
    // {
    //     require(_feeTokenToValidatorFees[feeToken] > 0, "No fee to withdraw");
    //     uint256 feeToWithdraw = _feeTokenToValidatorFees[feeToken];
    //     _feeTokenToValidatorFees[feeToken] = 0;
    //     feeManager.withdrawFee(feeToken, msg.sender, feeToWithdraw);
    // }
    ...
    /**
        ...
        @param sender Address of the sender of tx
    ...
    function deposit(
        ...
        require(
            _contractWhitelist[swapDetails.srcStableTokenAddress],
            "ERC20 Handler: provided tokenAddress is not whitelisted"
        );
        require(
            getTokenToUSDx1000(address(swapDetails.destStableTokenAddress)) != 0,
        ...
        uint256 priceInUSDx1000 = getTokenToUSDx1000(address(swapDetails.destStableTokenAddress));
        uint8 decimals = IERC20Upgradeable(address(swapDetails.destStableTokenAddress)).decimals();
        uint256 txVolInUSDx1000 = (destStableTokenAmount * priceInUSDx1000) / (10**decimals);
        ...
        if (address(swapDetails.srcTokenAddress) == swapDetails.srcStableTokenAddress) {
            ...
            uint256[] memory transferFee = getBridgeFee(
                destinationChainID,
                swapDetails.path[0],
                swapDetails.path[swapDetails.path.length - 1],
            ...
            require(swapDetails.srcStableTokenAmount == swapDetails.srcTokenAmount, "Invalid token amount");
            feeAmount = deductFeeAndHandleDepositForLPToken(
                swapDetails,
                destinationChainID,
                widgetId,
                txVolInUSDx1000,
    ...
    //     uint256 protocolFee;
    //     uint256 widgetFee;
    //     (lpFee, validatorFee, protocolFee, widgetFee) = abi.decode(data, (uint256, uint256, uint256, uint256));

    //     _feeTokenToLpFees[feeTokenAddress] += lpFee;
    //     _feeTokenToValidatorFees[feeTokenAddress] += validatorFee;
    //     _feeTokenToProtocolFees[feeTokenAddress] += protocolFee;
    //     _widgetIdAndFeeAddressToWidgetFees[widgetId][feeTokenAddress] += widgetFee;
    // }
...
contract HandlerHelpersUpgradeable is Initializable, ContextUpgradeable, AccessControlUpgradeable, IERCHandler {
    ...
    // bytes32 public constant LP_FEE_WITHDRAW_ROLE = keccak256("LP_FEE_WITHDRAW_ROLE");
    // bytes32 public constant VALIDATOR_FEE_WITHDRAW_ROLE = keccak256("VALIDATOR_FEE_WITHDRAW_ROLE");
    // bytes32 public constant PROTOCOL_FEE_WITHDRAW_ROLE = keccak256("PROTOCOL_FEE_WITHDRAW_ROLE");
    ...
    // token contract address => price in USD
    mapping(address => uint256) private _tokenToUSDx1000;
    ...
    function getTokenToUSDx1000(address token) public view override returns (uint256) {
        return _tokenToUSDx1000[token];
    }

    function setTokenToUSDx1000(address token, uint256 priceInUSDx1000)
        external
        virtual
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _tokenToUSDx1000[token] = priceInUSDx1000;
    }
