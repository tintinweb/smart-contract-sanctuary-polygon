...
contract CryptoBlob is ERC721 {
    uint256 public totalSupply;
    uint256 public cost = 0.5 ether;
    uint256 public maxMints = 6969;
    ...
    constructor()
        ERC721("CryptoBlob", "CBLOB")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 6969) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeifnk2ubjhtxvewd7bqipngl5mvrfshysch7wl3bvsdswrt6ff2baa/", Strings.toString(tokenId), ".json")
            );
    }
