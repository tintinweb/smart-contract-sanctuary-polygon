...
import "../interfaces/IAttackItem.sol";
...
contract FightCalculator is Controllable, IFightCalculator {
  ...
  struct Fighter {
    FighterInfo info;
    IStatController.MagicAttack magicAttack;
    uint health;
    uint manaConsumed;
    uint damage;
    uint damagePoison;
  ...
  uint constant internal _MAX_RESIST = 90;

  // ---- VARIABLES ----

  /// @dev dungeon => hero => heroId => slot => durability have to decreased
  mapping(address => mapping(address => mapping(uint => mapping(uint => bool)))) internal _skillSlotsForDurabilityReduction;

  event FightResultProcessed(address sender, FightInfoInternal result, FightCall callData, uint iteration);
  ...
  function skillSlotsForDurabilityReduction(address dungeon, address heroToken, uint heroTokenId) external override view returns (SkillSlots memory) {
    mapping(uint => bool) storage map = _skillSlotsForDurabilityReduction[dungeon][heroToken][heroTokenId];
    return SkillSlots(
      map[uint(IStatController.ItemSlots.SKILL_1)],
      map[uint(IStatController.ItemSlots.SKILL_2)],
      map[uint(IStatController.ItemSlots.SKILL_3)]
    );
  }

  function decodeAndCheckAttackInfo(bytes memory data, address _heroToken, uint _heroId) external override view returns (AttackInfo memory) {
    (AttackInfo memory attackInfo) = abi.decode(data, (AttackInfo));

    IController c = IController(controller());
    if (attackInfo.attackToken != address(0)) {
      require(c.validItems(attackInfo.attackToken), "Attack token not registered");
      require(IItem(attackInfo.attackToken).equippedOn(attackInfo.attackTokenId, _heroToken) == _heroId, "Not your attack item");
    }

    for (uint i; i < attackInfo.skillTokens.length; ++i) {
      require(c.validItems(attackInfo.skillTokens[i]), "Buff token not registered");
      require(IItem(attackInfo.skillTokens[i]).equippedOn(attackInfo.skillTokenIds[i], _heroToken) == _heroId, "Not your buff item");
    }
    return attackInfo;
  }

  /// @dev Items ownership must be checked before
  function fight(FightCall memory callData) external override returns (FightResult memory) {
    FightInfoInternal memory fResult = _prepareFightInternalInfo(callData.fighterA, callData.fighterB);

    _fightProcessing(fResult);

    // assume msg.sender is chamber
    emit FightResultProcessed(msg.sender, fResult, callData, IChamber(msg.sender).iterations(callData.heroAdr, callData.heroId));
  ...
  function releaseSkillSlotsForDurabilityReduction(address heroToken, uint heroTokenId) external override {
    mapping(uint => bool) storage map = _skillSlotsForDurabilityReduction[msg.sender][heroToken][heroTokenId];
    map[uint(IStatController.ItemSlots.SKILL_1)] = false;
    map[uint(IStatController.ItemSlots.SKILL_2)] = false;
    map[uint(IStatController.ItemSlots.SKILL_3)] = false;
  }

  function markSkillSlotsForDurabilityReduction(bytes memory data, address heroToken, uint heroTokenId) external override {
    (AttackInfo memory attackInfo) = abi.decode(data, (AttackInfo));
    ...
    if (length != 0 || attackInfo.attackToken != address(0)) {
      ...
      IStatController statCtr = IStatController(IController(controller()).statController());
      IStatController.NftItem memory skillSlot1 = statCtr.heroItemSlot(heroToken, heroTokenId, uint(IStatController.ItemSlots.SKILL_1));
      IStatController.NftItem memory skillSlot2 = statCtr.heroItemSlot(heroToken, heroTokenId, uint(IStatController.ItemSlots.SKILL_2));
      IStatController.NftItem memory skillSlot3 = statCtr.heroItemSlot(heroToken, heroTokenId, uint(IStatController.ItemSlots.SKILL_3));

      for (uint i; i < length; ++i) {
        address token = attackInfo.skillTokens[i];
        uint tokenId = attackInfo.skillTokenIds[i];

        if (token == skillSlot1.token && tokenId == skillSlot1.tokenId) {
          map[uint(IStatController.ItemSlots.SKILL_1)] = true;
        } else if (token == skillSlot2.token && tokenId == skillSlot2.tokenId) {
          map[uint(IStatController.ItemSlots.SKILL_2)] = true;
        } else if (token == skillSlot3.token && tokenId == skillSlot3.tokenId) {
          map[uint(IStatController.ItemSlots.SKILL_3)] = true;
        }
      }

      if (attackInfo.attackToken == skillSlot1.token && attackInfo.attackTokenId == skillSlot1.tokenId) {
        map[uint(IStatController.ItemSlots.SKILL_1)] = true;
      } else if (attackInfo.attackToken == skillSlot2.token && attackInfo.attackTokenId == skillSlot2.tokenId) {
        map[uint(IStatController.ItemSlots.SKILL_2)] = true;
      } else if (attackInfo.attackToken == skillSlot3.token && attackInfo.attackTokenId == skillSlot3.tokenId) {
        map[uint(IStatController.ItemSlots.SKILL_3)] = true;
      }
    }
  }
  ...
  function _prepareFightInternalInfo(
    FighterInfo memory fighterA,
    FighterInfo memory fighterB
  ) internal view returns (FightInfoInternal memory fInfo) {
    IStatController.MagicAttack memory fighterAMagicAttack;
    if (fighterA.attackToken != address(0)) {
      require(fighterA.attackType == AttackType.MAGIC, "!magic");
      fighterAMagicAttack = IAttackItem(fighterA.attackToken).attackAttributes(fighterA.attackTokenId);
    }
    IStatController.MagicAttack memory fighterBMagicAttack;
    if (fighterB.attackToken != address(0)) {
      require(fighterB.attackType == AttackType.MAGIC, "!magic");
      fighterBMagicAttack = IAttackItem(fighterB.attackToken).attackAttributes(fighterB.attackTokenId);
    }

    fInfo = FightInfoInternal({
      fighterA: Fighter({
      info: fighterA,
      magicAttack: fighterAMagicAttack,
      health: fighterA.fighterStats.life,
      manaConsumed: 0,
      damage: 0,
      damagePoison: 0,
      damageReflect: 0,
      statuses: Statuses(false, false, false, false, false, false)
    }),
      fighterB: Fighter({
      info: fighterB,
      magicAttack: fighterBMagicAttack,
      health: fighterB.fighterStats.life,
      manaConsumed: 0,
      damage: 0,
      damagePoison: 0,
      damageReflect: 0,
  ...
  function _fightProcessing(FightInfoInternal memory fResult) internal view {

    bool firstA = _calcFirstHit(fResult);

    _setStatuses(fResult, firstA);
    _setStatuses(fResult, !firstA);

    _reduceAttributesByStatuses(fResult.fighterA.info.fighterAttributes, fResult.fighterA.statuses, fResult.fighterB.info.fighterAttributes);
    _reduceAttributesByStatuses(fResult.fighterB.info.fighterAttributes, fResult.fighterB.statuses, fResult.fighterA.info.fighterAttributes);

    AttackResult memory resultA = _processAttack(fResult, true);
    AttackResult memory resultB = _processAttack(fResult, false);
    ...
    _stealLife(fResult.fighterA, resultA);
    _stealLife(fResult.fighterB, resultB);
  ...
  function _reduceHp(
    ...
    Fighter memory firstFighter,
    Fighter memory secondFighter
  ) internal pure {
    ...
    if (secondFighter.health != 0) {
      ...
      secondFighter.damageReflect = Math.min(firstAttack.reflectDamage, firstFighter.health);
      firstFighter.health -= secondFighter.damageReflect;

      // reflect damage from first to second
      firstFighter.damageReflect = Math.min(secondAttack.reflectDamage, secondFighter.health);
      secondFighter.health -= firstFighter.damageReflect;
    }

    // poison second firstly (he got damage and statuses early)
    firstFighter.damagePoison = _poisonDmg(secondFighter.health, secondFighter.statuses);
    secondFighter.health -= firstFighter.damagePoison;

    // poison first fighter
    secondFighter.damagePoison = _poisonDmg(firstFighter.health, firstFighter.statuses);
    firstFighter.health -= secondFighter.damagePoison;
  }

  function _stealLife(Fighter memory fighter, AttackResult memory attackResult) internal pure {
    if (fighter.health != 0) {
      uint newHealth = fighter.health + attackResult.lifeStolen;
      uint maxHealth = fighter.info.fighterAttributes[uint(IStatController.ATTRIBUTES.LIFE)];
      fighter.health = Math.min(newHealth, maxHealth);
    }
  }

  function _setStatuses(FightInfoInternal memory fResult, bool isA) internal view {
    if (!_skipTurn(fResult, isA)) {

      FighterInfo memory attackerInfo = isA ? fResult.fighterA.info : fResult.fighterB.info;
      FighterInfo memory defenderInfo = isA ? fResult.fighterB.info : fResult.fighterA.info;

      IStatController.MagicAttack memory attackerMA = isA ? fResult.fighterA.magicAttack : fResult.fighterB.magicAttack;

      Statuses memory statuses = isA ? fResult.fighterB.statuses : fResult.fighterA.statuses;

      uint resist = defenderInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.RESIST_TO_STATUSES)];

      statuses.stun = _statusChance(attackerInfo, attackerMA, IStatController.ATTRIBUTES.STUN, resist);
      statuses.burn = _statusChance(attackerInfo, attackerMA, IStatController.ATTRIBUTES.BURN, defenderInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.FIRE_RESISTANCE)]);
      statuses.freeze = _statusChance(attackerInfo, attackerMA, IStatController.ATTRIBUTES.FREEZE, defenderInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.COLD_RESISTANCE)]);
      statuses.confuse = _statusChance(attackerInfo, attackerMA, IStatController.ATTRIBUTES.CONFUSE, resist);
      statuses.curse = _statusChance(attackerInfo, attackerMA, IStatController.ATTRIBUTES.CURSE, resist);
      statuses.poison = _statusChance(attackerInfo, attackerMA, IStatController.ATTRIBUTES.POISON, resist);
    }
  }

  function _statusChance(
    FighterInfo memory attackerInfo,
    IStatController.MagicAttack memory attackerMA,
    IStatController.ATTRIBUTES index,
    uint resist
  ) internal view returns (bool) {
    uint chance = attackerInfo.fighterAttributes[uint(index)];

    if (attackerInfo.attackType == AttackType.MAGIC) {
      if (index == IStatController.ATTRIBUTES.BURN && attackerMA.aType == IStatController.MagicAttackType.FIRE) {
        chance += 20;
      }
      if (index == IStatController.ATTRIBUTES.FREEZE && attackerMA.aType == IStatController.MagicAttackType.COLD) {
        chance += 20;
      }
      if (index == IStatController.ATTRIBUTES.CONFUSE && attackerMA.aType == IStatController.MagicAttackType.LIGHTNING) {
        chance += 20;
      }
    }

    chance = chance - chance * Math.min(resist, _MAX_RESIST) / RESISTANCE_DENOMINATOR;
    if (chance == 0) {
      return false;
    }
    if (chance >= 100) {
    ...
    return CalcLib.pseudoRandom(RESISTANCE_DENOMINATOR) < chance;
  }

  function _skipTurn(FightInfoInternal memory fResult, bool isA) internal pure returns (bool) {
    return isA ? fResult.fighterA.statuses.stun : fResult.fighterB.statuses.stun;
  }

  function _processAttack(
    FightInfoInternal memory fResult,
    bool isA
  ) internal view returns (AttackResult memory attackResult) {

    uint defenderHealth = isA ? fResult.fighterB.health : fResult.fighterA.health;
    ...
    if (attackerInfo.attackType == AttackType.MELEE) {
      attackResult = _meleeDamageCalculation(
        attackerInfo,
        defenderInfo,
        defenderHealth
      );
    } else if (attackerInfo.attackType == AttackType.MAGIC) {
      attackResult = _magicDamageCalculation(
        attackerInfo,
        defenderInfo,
  ...
  function _calcFirstHit(FightInfoInternal memory fInfo) internal pure returns (bool aFirst){
    if (fInfo.fighterA.info.attackType == AttackType.MAGIC) {
      if (fInfo.fighterB.info.attackType == AttackType.MAGIC) {
        // if both fighters use magic we check attack rating
        aFirst = _isAttackerFaster(fInfo.fighterA.info, fInfo.fighterB.info);
      } else {
  ...
  function _magicDamageCalculation(
    ...
    // generate damage
    uint damage = _getMagicDamage(attackerInfo, magicAttack);
    damage = _increaseMagicDmgByFactor(damage, attackerInfo, magicAttack.aType);
    damage = _increaseRaceDmg(damage, attackerInfo, defenderInfo.race);
    damage = _criticalHit(damage, attackerInfo);

    // decrease damage
    damage = _decreaseRaceDmg(damage, defenderInfo, attackerInfo.race);
    damage = _decreaseDmgByDmgReduction(damage, defenderInfo);

    if (magicAttack.aType == IStatController.MagicAttackType.FIRE) {
      damage -= damage * Math.min(defenderInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.FIRE_RESISTANCE)], _MAX_RESIST) / RESISTANCE_DENOMINATOR;
    } else if (magicAttack.aType == IStatController.MagicAttackType.COLD) {
      damage -= damage * Math.min(defenderInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.COLD_RESISTANCE)], _MAX_RESIST) / RESISTANCE_DENOMINATOR;
    } else if (magicAttack.aType == IStatController.MagicAttackType.LIGHTNING) {
      damage -= damage * Math.min(defenderInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.LIGHTNING_RESISTANCE)], _MAX_RESIST) / RESISTANCE_DENOMINATOR;
    }

    uint defenderHealthResult = defenderHealth < damage ? 0 : defenderHealth - damage;
    damage = defenderHealth - defenderHealthResult;

    return AttackResult({
      defenderHealth: defenderHealthResult,
      damage: damage,
      lifeStolen: _lifeStolenPerHit(damage, attackerInfo),
      reflectDamage: _reflectMagicDmg(damage, defenderInfo) + _reflectChaos(magicAttack, attackerInfo)
    });
  }

  function _reflectChaos(IStatController.MagicAttack memory magicAttack, FighterInfo memory attackerInfo) internal view returns (uint) {
    if (magicAttack.aType == IStatController.MagicAttackType.CHAOS && CalcLib.pseudoRandom(1e18) > 5e17) {
      return attackerInfo.fighterStats.life / 2;
    } else {
      return 0;
    }
  }
  ...
  function _meleeDamageCalculation(
    ...
    uint damage = _getDamage(attackerInfo.fighterAttributes);
    damage = _increaseMeleeDmgByFactor(damage, attackerInfo);
    damage = _increaseRaceDmg(damage, attackerInfo, defenderInfo.race);
    damage = _criticalHit(damage, attackerInfo);
    ...
    // miss
    if (CalcLib.pseudoRandom(1e18) > StatLib.chanceToHit(
      attackerInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.ATTACK_RATING)],
      defenderInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.DEFENSE)],
      attackerInfo.fighterStats.level,
      defenderInfo.fighterStats.level,
      attackerInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.AR_FACTOR)]
    )
    ) {
      damage = 0;
    }
    // block
    if (CalcLib.pseudoRandom(100) < defenderInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.BLOCK_RATING)]) {
      damage = 0;
    }
    ...
    return AttackResult({
      ...
      damage: damage,
      lifeStolen: _lifeStolenPerHit(damage, attackerInfo),
      reflectDamage: _reflectMeleeDmg(damage, defenderInfo)
    });
  }

  function _getDamage(uint[] memory attributes) internal view returns (uint) {
    return CalcLib.pseudoRandomInRange(
      attributes[uint(IStatController.ATTRIBUTES.DAMAGE_MIN)],
      attributes[uint(IStatController.ATTRIBUTES.DAMAGE_MAX)]
    );
  }
  ...
  function _getMagicDamage(
    ...
    attributeFactorResult += (attackerInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.STRENGTH)] * mAttack.attributeFactors.strength / 1e18);
    attributeFactorResult += (attackerInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.DEXTERITY)] * mAttack.attributeFactors.dexterity / 1e18);
    attributeFactorResult += (attackerInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.VITALITY)] * mAttack.attributeFactors.vitality / 1e18);
    attributeFactorResult += (attackerInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.ENERGY)] * mAttack.attributeFactors.energy / 1e18);

    return CalcLib.pseudoRandomInRange(mAttack.min, mAttack.max) + attributeFactorResult;
  }

  function _increaseRaceDmg(uint dmg, FighterInfo memory attackerInfo, uint defenderRace) internal pure returns (uint){
    if (defenderRace == uint(IStatController.Race.HUMAN)) {
      return dmg + attackerInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.DMG_AGAINST_HUMAN)] * dmg / 100;
    } else if (defenderRace == uint(IStatController.Race.UNDEAD)) {
      return dmg + attackerInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.DMG_AGAINST_UNDEAD)] * dmg / 100;
    } else if (defenderRace == uint(IStatController.Race.DAEMON)) {
      return dmg + attackerInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.DMG_AGAINST_DAEMON)] * dmg / 100;
    } else if (defenderRace == uint(IStatController.Race.BEAST)) {
      return dmg + attackerInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.DMG_AGAINST_BEAST)] * dmg / 100;
    } else {
      return dmg;
    }
  }

  function _increaseMeleeDmgByFactor(uint dmg, FighterInfo memory attackerInfo) internal pure returns (uint){
    return dmg + attackerInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.MELEE_DMG_FACTOR)] * dmg / 100;
  }

  function _increaseMagicDmgByFactor(uint dmg, FighterInfo memory attackerInfo, IStatController.MagicAttackType aType) internal pure returns (uint) {
    if (aType == IStatController.MagicAttackType.FIRE) {
      return dmg + dmg * attackerInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.FIRE_DMG_FACTOR)] / 100;
    } else if (aType == IStatController.MagicAttackType.COLD) {
      return dmg + dmg * attackerInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.COLD_DMG_FACTOR)] / 100;
    } else if (aType == IStatController.MagicAttackType.LIGHTNING) {
      return dmg + dmg * attackerInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.LIGHTNING_DMG_FACTOR)] / 100;
    } else {
      return dmg;
    }
  }

  function _decreaseRaceDmg(uint dmg, FighterInfo memory defenderInfo, uint attackerRace) internal pure returns (uint) {
    if (attackerRace == uint(IStatController.Race.HUMAN)) {
      return dmg - Math.min(defenderInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.DEF_AGAINST_HUMAN)], _MAX_RESIST) * dmg / 100;
    } else if (attackerRace == uint(IStatController.Race.UNDEAD)) {
      return dmg - Math.min(defenderInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.DEF_AGAINST_UNDEAD)], _MAX_RESIST) * dmg / 100;
    } else if (attackerRace == uint(IStatController.Race.DAEMON)) {
      return dmg - Math.min(defenderInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.DEF_AGAINST_DAEMON)], _MAX_RESIST) * dmg / 100;
    } else if (attackerRace == uint(IStatController.Race.BEAST)) {
      return dmg - Math.min(defenderInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.DEF_AGAINST_BEAST)], _MAX_RESIST) * dmg / 100;
    } else {
      return dmg;
    }
  }

  function _decreaseDmgByDmgReduction(uint dmg, FighterInfo memory defenderInfo) internal pure returns (uint) {
    return dmg - Math.min(defenderInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.DAMAGE_REDUCTION)], _MAX_RESIST) * dmg / 100;
  }

  function _criticalHit(uint dmg, FighterInfo memory attackerInfo) internal view returns (uint) {
    if (CalcLib.pseudoRandom(100) < attackerInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.CRITICAL_HIT)]) {
      return dmg * 2;
    }
    return dmg;
  }

  function _reduceAttributesByStatuses(uint[] memory targetAttributes, Statuses memory statuses, uint[] memory casterAttributes) internal pure {
  ...
  function _poisonDmg(uint health, Statuses memory statuses) internal pure returns (uint){
    // poison should not kill
    if (statuses.poison && health > 1) {
      // at least 1 dmg
      return Math.max(health / 10, 1);
    }
    return 0;
  }

  function _isAttackerFaster(
    FighterInfo memory fighterAInfo,
    FighterInfo memory fighterBInfo
  ...
  function _lifeStolenPerHit(uint dmg, FighterInfo memory attackerInfo) internal pure returns (uint) {
    return dmg * attackerInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.LIFE_STOLEN_PER_HIT)] / 100;
  }

  function _reflectMeleeDmg(uint dmg, FighterInfo memory defenderInfo) internal pure returns (uint){
    return dmg * defenderInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.REFLECT_DAMAGE_MELEE)] / 100;
  }

  function _reflectMagicDmg(uint dmg, FighterInfo memory defenderInfo) internal pure returns (uint){
    return dmg * defenderInfo.fighterAttributes[uint(IStatController.ATTRIBUTES.REFLECT_DAMAGE_MAGIC)] / 100;
  }
