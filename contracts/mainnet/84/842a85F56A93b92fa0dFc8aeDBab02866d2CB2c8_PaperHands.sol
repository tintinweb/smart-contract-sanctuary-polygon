...
contract PaperHands is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  function swapUsdcForGiddyCalc(uint256 value, uint256 fap) external view returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = GiddyUniswap.calcPriceSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, value - fap);
  }
  ...
  function transferWalletToWalletUSDC(bytes calldata receiveAuthorization, address recipient, uint256 fap) external whenNotPaused nonReentrant {
    ...
    require((approvedSenders[address(msg.sender)] == true || isTrustedForwarder(address(msg.sender))) && owner == _msgSender(), "Invalid Sender");
    require(spender == address(this), "Recipient is not this contract");
  ...
  function transferWalletToWalletGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, address recipient, uint256 fap) external whenNotPaused nonReentrant {
    require((approvedSenders[address(msg.sender)] == true || isTrustedForwarder(address(msg.sender))) && req.owner == _msgSender(), "Invalid Sender");
    require(req.spender == address(this), "Recipient is not this contract");
