...
//0xF32c7F342c14fd8B895f22686D06Fe303B54D15d
...
contract Owned is Context {
    ...
    modifier onlyOwner {
        require(_msgSender() == _owner, "DappCensus: Only Owner can perform this task");
        _;
    }
    ...
    function acceptOwnership() public {
        require(_msgSender() == _newOwner, "DappCensus: Token Contract Ownership has not been set for the address");
 ...
 *
 */
contract DappCensus is IBEP20, Owned {
    ...
    function approve(address spender, uint256 amount) public override returns (bool) {
        ...
        require(spender != address(0), "DappCensus: approve to the zero address");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        require(amount <= _allowances[sender][_msgSender()],"DappCensus: Check for approved token count failed");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(recipient != address(0),"DappCensus: Cannot have recipient as zero address");
        require(sender != address(0),"DappCensus: Cannot have sender as zero address");
        require(_balances[sender] >= amount,"DappCensus: Insufficient Balance" );
        require(_balances[recipient] +amount >= _balances[recipient],"DappCensus: Balance check failed");
    ...
    function burn(address account, uint256 amount) public onlyOwner {
        ...
        require(account != address(0), "DappCensus: burn from the zero address");

        if( _balances[account] == _unlockedTokens[account]){
            _unlockedTokens[account] = _unlockedTokens[account].sub(amount, "DappCensus: burn amount exceeds balance");
        }

        _balances[account] = _balances[account].sub(amount, "DappCensus: burn amount exceeds balance");
        ...
        if(account != _msgSender()){
            ...
            require(amount <= _allowances[account][_msgSender()],"DappCensus: Check for approved token count failed");

            _allowances[account][_msgSender()] = _allowances[account][_msgSender()].sub(amount, "DappCensus: burn amount exceeds allowance");
    ...
    // ------------------------------------------------------------------------
    function disributeLockTokens(address to, uint256 tokens, uint256 lockingPeriod) onlyOwner public returns (bool success) {
    ...
    function _updateUnLockedTokens(address _from, uint256 tokens) private returns (bool success) {
        ...
        // if _unlockedTokens are less than "tokens" of "to", update _unlockedTokens by checking record with "now" time
            ...
            // otherwise revert
                ...
                revert("DappCensus: Insufficient unlocked tokens");
            }
        }
    }
