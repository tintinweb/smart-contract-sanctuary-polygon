...
contract ZoraMarketProxy is IMarketProxy, AccessControl {
    ...
    function setAuctionApproval(uint256 nftId, bool newApproved) external onlyAdmin {
        if (_contain(nftId)) {
    ...
    function setAuctionReservePrice(uint256 nftId, uint256 newReservePrice) external onlyAdmin {
    ...
    function cancelAuction(uint256 nftId) external onlyAdmin {
        if (_contain(nftId)) {
            AuctionInfo memory info = _auctions[nftId];
     ...
     */
    function endAuction(uint256 nftId) external onlyAdmin {
        if (_contain(nftId)) {
            AuctionInfo memory info = _auctions[nftId];
