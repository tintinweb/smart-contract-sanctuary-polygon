...
import "ERC20UtilityOperations.sol";

contract ERC20UtilityBackfill is ERC20UtilityOperations {
    function setupTimelocks(
        IViciAccess ams,
        uint256 release,
        address[] calldata addresses,
        uint256[] calldata amounts
    ) public virtual onlyOwnerOrRole(ams, msg.sender, AIRDROP_ROLE_NAME) {
        require(addresses.length == amounts.length, "invalid arrays");
        for (uint256 i = 0; i < addresses.length; i++) {
            require(
                unlockedBalanceOf(addresses[i]) >= amounts[i],
                "insufficient balance"
            );
            lockedAmount[addresses[i]] = amounts[i];
