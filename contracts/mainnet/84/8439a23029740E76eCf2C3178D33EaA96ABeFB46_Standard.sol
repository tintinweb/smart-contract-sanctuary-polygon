...
contract Standard is ERC721A, ReentrancyGuard, Delegated {
    ...
    // ======== CONSTRUCTOR ========
    constructor() ERC721A("I Love MacDonalds and Barney", "PUPMACDG") {}
        ...
        callerIsUser
    {
        ...
        require(msg.value >= allowListPrice, "Not enough ether sent");
        require(totalSupply() + 1 <= MAX_SUPPLY, "Max supply reached");
        require(!preSaleMinted[msg.sender], "Already minted");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        bool isValidProof = MerkleProof.verify(_proof, mintBatch1Root, leaf);
        ...
        callerIsUser
    {
        ...
        require(msg.value >= publicSalePrice, "Not enough ether sent");
        require(totalSupply() + 1 <= MAX_SUPPLY, "Max supply reached");
        require(!preSaleMinted[msg.sender], "Already minted");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        bool isValidProof = MerkleProof.verify(_proof, mintBatch2Root, leaf);
        require(isValidProof, "Invalid proof");
        preSaleMinted[msg.sender] = true;
        _safeMint(msg.sender, 1);
    }

    function mintBatch3(uint256 _quantity) external payable callerIsUser {
