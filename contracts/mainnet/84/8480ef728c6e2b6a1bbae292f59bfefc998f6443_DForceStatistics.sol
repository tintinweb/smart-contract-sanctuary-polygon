...
contract DForceStatistics is StatisticsBase {
    ...
    uint8 public dfSwapType; // 0 : pancakeswap, 1 : uniswapV3,
    address public pancakeSwapRouter;
    address[] public pathToSwapDFToStableCoin;
    address public uniswapV3DFPool;
    ...
    /**
     * @notice Set Uniswap information
     */
    function setUniswapV3DFPool(address _uniswapV3DFPool)
        external
        onlyOwnerAndAdmin
    {
        uniswapV3DFPool = _uniswapV3DFPool;
    }

    /**
     * @notice Set DF swap type
     * @param _dfSwapType 0 : pancakeswap, 1 : uniswapV3,
     */
    function setDFSwapType(uint8 _dfSwapType) external onlyOwnerAndAdmin {
        if (_dfSwapType == 0) {
            require(
                pancakeSwapRouter != ZERO_ADDRESS &&
                    pathToSwapDFToStableCoin.length >= 2,
                "DST1"
            );
        }

        if (_dfSwapType == 1) {
            require(uniswapV3DFPool != ZERO_ADDRESS, "DST1");
        }

        dfSwapType = _dfSwapType;
    }
        ...
        returns (uint256 priceUSD)
    {
        ...
        // UniswapV3
        if (dfSwapType == 1) {
            (amountOut, tokenOut) = ISwapGateway(swapGateway)
                .uniswapV3QuoteExactInput(
                    dfToken,
                    10**IERC20MetadataUpgradeable(dfToken).decimals(),
                    uniswapV3DFPool
                );
