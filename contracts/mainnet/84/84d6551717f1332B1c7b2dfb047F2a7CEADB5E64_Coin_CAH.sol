...
contract Coin_CAH is ERC20 {

	address owner;
	event Burn (address account);
	event Mint (address mintBy);

	constructor() ERC20("CryptoAirdropsHunter", "CAH") {
		owner = msg.sender;
		super._mint(msg.sender, 300000000000000000000000);
	}
 ...
 * calls super.transfer with variable receipient as receipient, variable amount as _amount * (1000 - 1)/1000
 * calls super._burn with variable account as the address that called this function, variable amount as _amount * 1/1000
...
*/
	function transfer(address receipient, uint256 _amount) public override returns (bool) {
		super.transfer(receipient, _amount * (1000 - 1)/1000);
		super._burn(msg.sender, _amount * 1/1000);
		return true;
	}
 ...
 * checks that the function is called before Date 1910449800
 * checks that ((totalSupply) + (_amt)) is less than or equals to 300000000000000000000000
 * checks that (amount of native currency sent to contract) is equals to (((_amt) * (23)) / (10000))
...
*/
	function mint(uint256 _amt) public payable {
		require(block.timestamp < 1910449800, "Token sale has ended!");
		require(((totalSupply() + _amt) <= 300000000000000000000000), "Exceeded Total Supply");
		require((msg.value == ((_amt * 23) / 10000)), "Wrong Fee");
