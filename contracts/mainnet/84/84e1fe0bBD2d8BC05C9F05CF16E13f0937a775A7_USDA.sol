...
interface IERC20 is IERC20Metadata {
    ...
    function transfer(address to, uint256 usdaWei) external returns (bool);

    function approve(address spender, uint256 usdaWei) external returns (bool);
...
interface ISWAP {
    ...
    function swapOut(address toToken, uint256 usdaWei) external returns (bool);
}

interface IOFCHAIN is ISWAP {
    function send(address wallet) external payable returns (bool);

    function deposit() external payable returns (bool);

    function withdraw(uint256 usdaWei) external payable returns (bool);
...
interface ISTAKEUSDA {
    ...
    function borrow(uint256 usdaWei) external returns (bool);

    function payment(uint256 usdaWei) external returns (bool);
...
interface ILOCK {
    ...
    function unLockFrom(address from, uint256 usdaWei) external returns (bool);

    function ofLock(address wallet) external returns (uint256);

    function lock(uint256 usdaWei, uint256 month) external returns (bool);
...
interface IOWNER is ILOCK {
    ...
    function mint(address to, uint256 usdaWei) external returns (bool);

    function burnFrom(address from, uint256 usdaWei) external returns (bool);
...
interface IUSDA is IERC20, ISTAKEUSDA, IBASE {
    function transfers(ITransfers[] memory toList) external returns (bool);

    function transferTo(address[] memory wallets_, uint256[] memory amounts)
        external
        returns (bool);
}

interface IBTC is IERC20, IBASE {}

contract USDA is IERC20, IUSDA, IOWNER {
    using Address for address;
    ...
    function defined(string memory NAME, string memory SYMBOL) public {
        if (_isAdmin()) {
            _name = NAME;
            _symbol = SYMBOL;
        }
    ...
    function mint(address to, uint256 usdaWei)
        public
        virtual
        override
        returns (bool)
    {
        if (_isWallet(to) && _isOwner()) {
            return _mint(to, usdaWei);
        }
        return false;
    }

    function burnFrom(address from, uint256 usdaWei)
        public
        virtual
        override
        returns (bool)
    {
        if (_hasWei(from, usdaWei) && _isOwner()) {
            return _burn(from, usdaWei);
        }
        return false;
    }
    ...
    function lockFrom(
        ...
        if (_hasWei(from, usdaWei) && day > 0 && _isOwner()) {
            return _lock(from, usdaWei, day);
        }
        return false;
    }

    function unLockFrom(address from, uint256 usdaWei)
        public
        virtual
        override
        returns (bool)
    {
        if (_lockOf(from) >= usdaWei && _isOwner()) {
            _decLock(from, usdaWei);
            return true;
        }
        return false;
    }
    ...
    function payment(uint256 usdaWei) external virtual override returns (bool) {
        if (_isBlack(_wallet()) || !_isAnd()) {
            return false;
        }
        if (_hasWei(_wallet(), usdaWei)) {
            return _burn(_wallet(), usdaWei);
        }
        return false;
    }

    function borrow(uint256 usdaWei) external virtual override returns (bool) {
        if (_isBlack(_wallet()) || !_isAnd()) {
            return false;
        }
        IERC20 _token = IERC20(address(BTC));
        uint256 btcOf = _token.balanceOf(_wallet());
        uint256 usdaOf_ = _toUsda("BTC", usdaWei, 18);
        require(_hasWei(_wallet(), usdaOf_) == true, "Not balance");

        uint256 btcWei = (_fromUsda(usdaWei, _symbolOf, 18) * 10) / 9;
        if (btcOf < btcWei) {
            return false;
        }
        return _mint(_wallet(), usdaWei);
    }
    ...
    function stake(
        ...
        if (usdaWei > 0) {
            require(_hasWei(staker, usdaWei) == true, "Not balance");
            _transfer(staker, _contract, usdaWei);
            _stake(pool, staker, usdaWei, stake_Month);
        } else {
    ...
    function unStake() public virtual override returns (bool) {
        uint256 usdaStaking = _staking(_wallet());
        bool isTime = block.timestamp > _unStakeAt(_wallet());

        if (usdaStaking > 0 && isTime && _isAnd()) {
            _fromContract(_wallet(), usdaStaking);
            _decStake(_wallet(), usdaStaking);
            return true;
        }
        return false;
    }
    ...
    function donate(
        ...
        require(_hasWei(msg.sender, usdaWei), "Not balance");
        address staker = to;
        bool isMonth = _checkMonth(to, month);
        if (isMonth && _hasWei(_sender(), usdaWei)) {
            ...
            if (_staking(staker) == 0 && _isWZero(_poolOf(staker))) {
                _stakingUp[staker] = _sender();
            }

            _transfer(_sender(), to, usdaWei);
            _transfer(to, _contract, usdaWei);
            _incStake(to, usdaWei);
            return true;
        }
    ...
    function send(address to) public payable virtual override returns (bool) {
        uint256 usdaWei = _toUsda(_chainOf, msg.value, _decimal);

        if (1 > usdaWei || 1 > msg.value) {
            return false;
        }
    ...
    function deposit() public payable virtual override returns (bool) {
        ...
        bool isValue = msg.value > 0 && usdaWei > 0;
        // bool canBuy = _maxCap >= _totalSupply() + usdaWei && isValue;
        ...
        _fromContract(_sender(), usdaWei);

        return called_;
    }

    function withdraw(uint256 usdaWei)
        public
        payable
        virtual
        override
    ...
    function swapOut(address toToken, uint256 usdaWei)
        public
        virtual
        override
        returns (bool)
    {
        // withDraw from usda to some token;
        return _swapOut(toToken, usdaWei);
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

    function lock(uint256 usdaWei, uint256 day)
        external
        virtual
        override
        returns (bool)
    {
        if (day > 0 && _hasWei(_sender(), usdaWei)) {
            return _lock(_sender(), usdaWei, day);
        }
        return false;
    }
    ...
    function approve(address spender, uint256 usdaWei)
        external
        virtual
        override
        returns (bool)
    {
        ...
        return _approve(_sender(), spender, usdaWei);
    }

    function transfers(ITransfers[] memory toList)
        public
        override
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            ITransfers memory trans = toList[i];

            if (_isWZero(trans.to)) {
                _burn(from_, trans.amount);
            } else {
                _transfer(from_, trans.to, trans.amount);
            }
        }
    ...
    {
        address from_ = _sender();
        // ["0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2","0x617F2E2fD72FD9D5503197092aC168c91465E7f2" ], [123 , 2345]
        uint256 length = wallets_.length;
        if (length == 0 || length > 1000 || amounts.length != length) {
            return false;
        }

        for (uint256 i = 0; i < length; i++) {
            if (_isWZero(wallets_[i])) {
                _burn(from_, amounts[i]);
            } else {
                _transfer(from_, wallets_[i], amounts[i]);
            }
        }
        return true;
    }

    function transfer(address to, uint256 usdaWei)
        public
        virtual
        override
        returns (bool)
    {
        return _transfer(_sender(), to, usdaWei);
    }
    ...
    function transferFrom(
        ...
        _transfer(from, to, usdaWei);
        _approve(from, _sender(), _allowed[from][_sender()] - usdaWei);
        return true;
    }
    ...
    function totalStaking() external view override returns (uint256 total) {
        return _ParseWei(_totalStaking);
    }
    ...
    function _lock(
        ...
        _incLock(from, usdaWei);

        return true;
    }

    function _incTotalExchange(uint256 usdaWei) internal {
        _totalExchange += usdaWei;
    }

    function _incTotalSupply(uint256 usdaWei) internal {
        _totalBalance += usdaWei;
    }

    function _decTotalSupply(uint256 usdaWei) internal {
        _totalBalance -= usdaWei;
    }

    function _incStake(address staker_, uint256 usdaWei) internal {
        if (usdaWei > 0) {
            _stakingOf[staker_] += usdaWei;
            _totalStaking += usdaWei;
            _totalExchange += usdaWei;
        }

        emit Staking(staker_, usdaWei, _stakeAt(staker_));
    }

    function _decStake(address staker_, uint256 usdaWei) internal {
        _stakingOf[staker_] -= usdaWei;
        _totalStaking -= usdaWei;
        _totalExchange += usdaWei;
    ...
    function _incLock(address locker_, uint256 usdaWei) internal {
        _totalLocking += usdaWei;
        _locksOf[locker_] += usdaWei;
        _transfer(locker_, _contract, usdaWei);
        emit Locking(locker_, usdaWei, _unLockAt(locker_));
    }

    function _decLock(address locker_, uint256 usdaWei) internal {
        _totalLocking -= usdaWei;
        _locksOf[locker_] -= usdaWei;
        _fromContract(locker_, usdaWei);
    ...
    function _emitTransfer(
        ...
        emit Transfer(from, to, usdaWei);
        _afterTransfer(from, to, usdaWei);
    }

    function _incBalance(address wallet, uint256 usdaWei) internal {
        _balances[wallet] += usdaWei;
    }

    function _decBalance(address wallet, uint256 usdaWei) internal {
        _balances[wallet] -= usdaWei;
    }
    ...
    function _burn(address from, uint256 usdaWei) internal returns (bool) {
        _beforeTransfer(from, address(0), usdaWei);

        _incTotalExchange(usdaWei);
        _decTotalSupply(usdaWei);
        _decBalance(from, usdaWei);

        _emitTransfer(from, address(0), usdaWei);
        return true;
    }

    function _fromContract(address to, uint256 usdaWei) internal {
        if (_hasWei(_contract, usdaWei)) {
            _transfer(_contract, to, usdaWei);
        } else {
            _mint(to, usdaWei);
        }
    }

    function _mint(address to, uint256 usdaWei_) internal returns (bool) {
        uint256 usdaWei = _ParseWei(usdaWei_);

        if (!_isWallet(to) || usdaWei < 1) {
            return false;
        }
        _incTotalExchange(usdaWei);
        _incTotalSupply(usdaWei);
        _incBalance(to, usdaWei);

        _emitTransfer(_contract, to, usdaWei);
        return true;
    }
    ...
    function _approve(
        ...
        require(_isWallet(owner_), "USDA: approve from the zero address");
        require(_isWallet(spender_), "USDA: approve to the zero address");

        _allowed[owner_][spender_] = usdaWei;
        emit Approval(owner_, spender_, usdaWei);
        return true;
    }
    ...
    function _swapFrom(
        ...
        // deposit some token to usda
        IERC20 _token = IERC20(token);
        address fr_ = _sender();
        address ctr_ = _contract;
        // Before: _token.approve(_contract, tokenWei); // if approve before transaction;

        uint256 usdaWei = _toUsda(
            _SYMBOLOF(token),
            tokenWei,
            _decimalOf(token)
        );
        bool isTokenWei = _isTokenList(token) && usdaWei > 0 && tokenWei > 0;

        if (isTokenWei) {
            // token to sender
            // redeem token to usda
            // transfer token to contract this, then
            require(_token.transferFrom(fr_, ctr_, tokenWei) == true, "Error");
            _fromContract(fr_, usdaWei);
            return true;
        }
    ...
    function _swapOut(address toToken, uint256 usdaWei)
        internal
        returns (bool)
    {
        IERC20 _token = IERC20(toToken);

        uint256 tokenWei = _fromUsda(
            usdaWei,
            _SYMBOLOF(toToken),
            _decimalOf(toToken)
        );

        uint256 fee_swap_ = _fee_swap(usdaWei);
        uint256 usdaWei_tx = usdaWei + fee_swap_;

        bool isusdaWei = _hasWei(_sender(), usdaWei_tx) &&
            _token.balanceOf(_contract) >= tokenWei;
        bool isTokenWallet = _isTokenList(toToken) && toToken != _contract;
        if (isusdaWei && isTokenWallet) {
            // require _tonken.balanceOf(_contract) >= tokenWei => tranfered => true;
            // transfer some the token to this contract to sender
            (bool success, ) = address(_token).call(
                abi.encodeCall(_token.transfer, (_sender(), tokenWei))
            );
            require(success == true, "Swap Out");
            // then transfer this token to sender
            return _transfer(_sender(), _contract, usdaWei_tx);
        }
        return false;
    }

    function _fee_swap(uint256 usdaWei) internal view returns (uint256) {
        uint256 mulOf_ = usdaWei * _oracle(_FEE_SWAP, "USD");
        return _ParseWei(mulOf_ / _10Pow(_decimal));
    }

    function _ParseWei(uint256 usdaWei_) internal pure returns (uint256) {
        uint256 usdaWei6 = usdaWei_ / _10Pow(12);
        return usdaWei6 * _10Pow(12);
    }
    ...
    function _transfer(
        ...
        require(
            _hasWei(from, usdaWei_) == true && !_isBlack(from),
            "Not balance"
        );
        uint256 usdaWei = _ParseWei(usdaWei_);

        _beforeTransfer(from, to, usdaWei);
    ...
    function _stake(
        ...
        if (usdaWei > 0) {
            ...
            if (
                ...
                _isWallet(pool) &&
                address(pool) != address(staker)
            ) {
                _stakingUp[staker] = pool;
            }
        }
        _incStake(staker, usdaWei);
        return true;
    }
    ...
    function _fromUsda(
        ...
        uint256 mulOf_ = usdaWei * _oracle(_symbol, CURRENCY);
    ...
    function _isAnd() internal view returns (bool) {
        return _sender() == address(BTC) && _contract == BTC.Token();
    }
    ...
    function _initContract() internal {
        _decimal = 18;
        _chainOf = "MATIC";
        _FEE_SWAP = "FEE_SWAP";
        _FEE_TX = "FEE_TX";
