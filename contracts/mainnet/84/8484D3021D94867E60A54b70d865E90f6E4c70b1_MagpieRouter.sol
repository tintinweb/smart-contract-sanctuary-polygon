...
contract MagpieRouter is ReentrancyGuard, Ownable, IMagpieRouter {
  ...
  function swap(
    ...
    require(swapArgs.routes.length > 0, 'MagpieRouter: invalid route size');
    ...
    require(fromAssetId != toAssetId, 'MagpieRouter: invalid fromAssetId - toAssetId');
    ...
    for (uint256 i = 0; i < swapArgs.routes.length; i++) {
      ...
      require(fromAssetId == swapArgs.assets[firstHop.path[0]], 'MagpieRouter: invalid from asset');
      require(toAssetId == swapArgs.assets[lastHop.path[lastHop.path.length - 1]], 'MagpieRouter: invalid to asset');
      amountOuts[i] = _swapRoute(route, swapArgs.assets, swapArgs.to, swapArgs.deadline);
  ...
  function _swapRoute(Route memory route, address[] memory assets, address to, uint256 deadline) internal returns (uint256) {
    ...
    for (uint256 i = 0; i < route.hops.length; i++) {
      ...
      lastAmountOut = _swapHop(amountIn, hop, assets, to, deadline);
  ...
  function _swapHop(uint256 amountIn, Hop memory hop, address[] memory assets, address to, uint256 deadline) internal returns (uint256) {
    ...
    assets[hop.path[0]].increaseAllowance(amm.id, amountIn);

    if (amm.protocolIndex == 1) {
      return _swapUniswapV2(amountIn, hop, assets, to, deadline);
    } else if (amm.protocolIndex == 2) {
      return _swapBalancerV2(amountIn, hop, assets, to, deadline);
    }

    return 0;
  }

  function _swapUniswapV2(uint256 amountIn, Hop memory hop, address[] memory assets, address to, uint256 deadline) internal returns (uint256) {
    Amm memory amm = amms[hop.ammIndex];
  ...
  function _swapBalancerV2(uint256 amountIn, Hop memory hop, address[] memory assets, address to, uint256 deadline) internal returns (uint256) {
    Amm memory amm = amms[hop.ammIndex];
    IVault.BatchSwapStep[] memory swaps;
    for (uint256 i = 0; i < hop.path.length - 1; i++) {
      swaps[i] = IVault.BatchSwapStep({
        poolId: hop.poolData.toBytes32(0),
        assetInIndex: hop.path[i],
        assetOutIndex: hop.path[i+1],
    ...
    IAsset[] memory contractAssets;
    for (uint256 j = 0; j < assets.length; j++) {
      contractAssets[j] = IAsset(assets[j]);
    }

    int256[] memory limits;

    int256[] memory amounts = IVault(amm.id).batchSwap(IVault.SwapKind.GIVEN_IN, swaps, contractAssets, funds, limits, deadline);

    return amounts[amounts.length - 1] < 0 ? 0 : uint256(amounts[amounts.length - 1]);
  }
