...
contract MultiPot is owned, EIP712MetaTransaction("MultiPot","1") {
    ...
    uint public minimumBetAmount = 0.001 ether;
    address wethBaseContract = 0xa4254439E51E196AC1f54c2ac958F928864AEa96;
    address nftContract = 0x7796A7306C286e050BcC18A3Ed650330e8885f2e;

    struct Pot {
        uint amount;
        address payable[] uniqueGamblers;
        mapping (address => uint) stakes;
        mapping (address => uint) nft_ids;
    }
    ...
    function setMaxNftGain(uint16 maxGain) external onlyOwner {
        require(current_state == State.seeding, "Not possible in current state");
        require(maxGain <= 100 && maxGain >= 50, "Only values between 50 and 100 are allowed");
        maxNftGain = maxGain;
    }
    ...
    function placeBet(uint amount, Color potColor, uint32 bet_round, uint32 nft_id, bytes memory approveAbiHex, bytes32 r, bytes32 s, uint8 v) external payable {
        ...
        require(gambler != owner, "Owner is not allowed to bet");
        ...
        if (nft_id != 0) {
            require(isNftAlreadySet(nft_id, gambler) == false, "This NFT is already set on a marble");
            pot.nft_ids[gambler] = nft_id;
        }
        pot.stakes[gambler] += amount;
        pot.amount += amount;
        emit BetPlaced(potColor, amount, nft_id);
    }
    ...
    function payoutWinners() external returns (uint pendingPayouts) {
        ...
        for(uint i = winningPot.uniqueGamblers.length; i >= 1; i--) {
            ...
            uint nftId = winningPot.nft_ids[gambler];
            if (nftId != 0 && IMiaMultiplier(nftContract).ownerOf(nftId) == gambler) {
                uint multiplier = IMiaMultiplier(nftContract).multiplierFor(nftId);
                uint multiplierPot = stake * (20 * maxNftGain / 100) / 100; // maxGain percentage of 20% house fee
                profit += multiplierPot * multiplier / 100;
            }
            ...
            emit PayoutSuccessful(gambler, profit, round, uint32(nftId));
            if(!(gasleft() > 26000)) {
                pendingPayouts = i - 1;
                break;
            }
    ...
    function getNftPots(address player) external view returns (uint[numPots] memory pairs) {
        for(uint8 j = 0; j < numPots; j++) {
            uint nftId = pots[j + numPots * round].nft_ids[player];
            if (nftId != 0) {
                pairs[j] = nftId;
            }
        }
    ...
    function getPlayerNfts(address player) external view returns (uint[] memory) {
        uint balance = IERC721Enumerable(nftContract).balanceOf(player);
        uint256[] memory tokenIds = new uint[](balance);
        for(uint8 j = 0; j < balance; j++) {
            uint nftId = IERC721Enumerable(nftContract).tokenOfOwnerByIndex(player, j);
            tokenIds[j] = nftId;
        }
        return tokenIds;
    }

    function isNftAlreadySet(uint nft_id, address player) internal view returns (bool) {
        for(uint8 j = 0; j < numPots; j++) {
            uint nftId = pots[j + numPots * round].nft_ids[player];
            if (nftId == nft_id) {
                return true;
    ...
    event PayoutSuccessful(address winner, uint amount, uint32 round, uint32 nft_id);
    ...
    event BetPlaced(Color pot, uint amount, uint32 nft_id);
...
// from https://github.com/bcnmy/metatx-standard
pragma solidity 0.7.3;
...
// code from: https://explorer-mainnet.maticvigil.com/address/0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619/contracts
...
contract NativeMetaTransaction is EIP712Base {
    ...
    function executeMetaTransaction(
        ...
        require(success, "Function call not successful erc20");
...
interface IMiaMultiplier {

	/**
	 * @dev Returns the multiplier of token with Id `tokenId`.
	 */
	function multiplierFor(uint256 tokenId) external view returns (uint256 multiplier);
