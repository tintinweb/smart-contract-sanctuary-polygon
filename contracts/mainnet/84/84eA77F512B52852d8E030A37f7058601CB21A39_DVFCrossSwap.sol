...
abstract contract Swap is Storage, UserWallet {
 ...
 bytes32 public constant _XCHAIN_TRANSACTION_TYPEHASH =
   keccak256("XChain(address user,bytes32 swapGateway,address[] tokensFrom,address[] tokensTo,uint256[] amountsFrom,uint256[] minAmountsTo,uint256[] maxFeesTo,uint256 nonce,uint256 deadline,uint256 chainId)");
  ...
  event SwapPerformedV2(bytes32 swapId, bytes32 swapGateway, address indexed user,
   address[] indexed tokensFrom, uint256[] amountsFrom, address[] indexed tokensTo,
  ...
  function verifyXChainTransactionSignature(
    ...
    bytes32 structHash = _hashTypedDataV4(keccak256(
      abi.encode(
        ...
        keccak256(abi.encodePacked(xChainTransaction.maxFeesTo)),
