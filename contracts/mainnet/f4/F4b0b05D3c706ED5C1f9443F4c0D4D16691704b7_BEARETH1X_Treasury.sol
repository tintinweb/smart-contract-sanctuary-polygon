...
contract BEARETH1X_Treasury is ContractGuard {
    ...
    uint256 public constant PERIOD = 180;
    ...
    /* =================== Added variables =================== */
    uint256 public previousEpochEthPrice;
    uint256 public currentEpochEthPrice;
    uint256 public previousEpochTokenIndexPrice;
    ...
    function getTokenPrice() public view returns (uint256 tokenPrice) {
        ...
        tokenPrice = IOracle(tokenOracle).consult(token, 1e18);
    }
    ...
    function initialize(
        ...
        previousEpochEthPrice = uint256(getLatestEthPrice());
        _updateTokenPrice();
        previousEpochTokenIndexPrice = getTokenPrice();
   ...
   //     try IOracle(tokenOracle).update() {} catch {}
    ...
    function getIndexPrice(uint _currentEthPrice, uint _previousEpochEthPrice, uint _previousEpochTombIndexPrice) public pure returns (uint) {
        uint256 temp1 = _currentEthPrice;
        uint256 temp2 = _previousEpochEthPrice.mul(2);
        if(temp2 < temp1){
            return _previousEpochTombIndexPrice.div(100);
        }else{
            uint256 IndexPrice = (temp2-temp1).mul(_previousEpochTombIndexPrice).div(_previousEpochEthPrice);
            return IndexPrice;
        }
    }

    function getRealtimeTokenIndexPrice() public view returns (uint) {
        uint256 RealtimeEthPrice = uint256(getLatestEthPrice());
        uint256 RealtimeTokenIndexPrice = getIndexPrice(RealtimeEthPrice, previousEpochEthPrice, previousEpochTokenIndexPrice);
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch /*checkOperator */{
        ...
        previousEpochEthPrice = currentEpochEthPrice;
        previousEpochTokenIndexPrice = currentEpochTokenIndexPrice;
    }
