...
abstract contract NFTFairVaultStorageBase is ConveyorBase {
    ...
    uint256 internal _categoriesCount;
...
abstract contract NFTFairVaultLogicBase is NFTFairVaultStorageBase, IERC721Receiver, IERC1155Receiver, ERC165 {
    ...
    function initialize(
        ...
        uint256[] memory _maxPurchases,
        uint256[] memory _prices,
        uint256[][] memory _catIds,
        uint256[][] memory _catAmounts
    ) public {
        require(!initialized);
        initialized = true;
        ...
        uint256 n = _nfts.length;
        if (n > 0) {
            require(n == _prices.length, "NFTFairVaultError: mismatch init length");
            require(n == _maxPurchases.length, "NFTFairVaultError: mismatch init length");
            if (_vaultType == VaultType.MINTER && _catIds.length > 0) {
                require(_catIds.length == n, "NFTFairVaultError: mismatch init length");
                if (_tokenType == TokenType.ERC1155) {
                    require(_catAmounts.length == n, "NFTFairVaultError: mismatch init length");
                }
            }
            for (uint256 i = 0; i < n; i++) {
                createCategory(_nfts[i], _maxPurchases[i], _prices[i], _tokenType);
                if (_vaultType == VaultType.MINTER) {
                    setCategoryIds(i, _catIds[i], _catAmounts[i]);
                }
            }
        }
    ...
    function recover(uint256[] memory _categoryArr, address _recipient) external virtual onlyOwner onlyInitialized {
        ...
        for (uint256 i = 0; i < _categoryArr.length; i++) {
            ...
            require(cat < _categoriesCount, "NFTFairVaultError: invalid category provided");
    ...
    function setCategoryIds(
        ...
        require(_category < _categoriesCount, "NFTFairVaultError: invalid category provided");
    ...
    function onERC721Received(
        ...
        require(category < _categoriesCount, "NFTFairVaultError: invalid category provided");
    ...
    function _updateCategoryPricing(uint256[] memory _prices) internal virtual onlyInitialized {
        ...
        if (n > 0) {
            require(n == _categoriesCount, "NFTFairVaultError: Pricing must be provided for all categories");
            for (uint256 i = 0; i < n; i++) {
...
// File contracts/templates/MysteryBoxVaultV2.sol
...
contract MysteryBoxVaultV2 is NFTFairVaultLogicBase {
    ...
    function claim(
        ...
        uint256[] memory seeds = parseData(data);

        uint256 idLen = seeds.length;
        require(idLen > 0, "must claim at least 1 nft id");
        uint256[] memory ids = new uint256[](idLen);
        ...
        if (category.tokenType == TokenType.ERC721) {
            ...
            if (idLen > availableToClaimCount) {
                idLen = availableToClaimCount;
            }
            ...
            if (idLen > (category.purchaseLimit - userClaimedCount)) {
                idLen = category.purchaseLimit - userClaimedCount;
            }

            for (uint256 i = 0; i < idLen; i++) {
                uint256 _id = _getIdFromSeed(seeds[i], category);
                ids[i] = _id;
                if (!category.claimable_ids.contains(_id)) {
                    continue;
                }
            ...
            for (uint256 i = 0; i < idLen; i++) {
                ...
                _userClaimedBalance[_msgSender()][_category][_id] =
                    _userClaimedBalance[_msgSender()][_category][_id] +
                    1;
    ...
    function parseData(bytes memory data) internal pure returns (uint256[] memory) {
        uint256 n = data.length / 32;
        uint256[] memory ret = new uint256[](n);

        for (uint256 i = 0; i < n; i++) {
            bytes memory dataToDecode = data.slice(0, 32);
            uint256 num = abi.decode(dataToDecode, (uint256));
            ret[i] = num;
            if (i < n - 1) {
