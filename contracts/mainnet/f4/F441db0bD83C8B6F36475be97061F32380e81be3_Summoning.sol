...
// BrewAxo is MasterChef's left hand and kinda a wizard. He can brew Axo from pretty much anything!
...
contract Summoning is Ownable, ReentrancyGuard {
    ...
    modifier onlyAuth() {
        require(isAuth[_msgSender()], "BrewAxo: FORBIDDEN");
        _;
    }
    ...
    modifier onlyEOA() {
        ...
        require(msg.sender == tx.origin, "BrewAxo: must use EOA");
        _;
    }
    ...
    //token bridges to try in order when swapping, first three are immutably matic, usdc, dai
    ...
    function setBridge(address token, address bridge) external onlyAuth {
        // Checks
        require(
            token != axo && token != matic && token != bridge,
            "BrewAxo: Invalid bridge"
        );
    ...
    function convertMultiple(
        ...
        for (i = 0; i < token0.length;) {
            ...
            require(address(pair) != address(0), "BrewAxo: Invalid pair");
        ...
        converted[matic] = block.number; // matic is done last
        for (i = 0; i < token0.length;) {
        ...
        // final step is to swap all matic to AXO and disperse it
        i = IERC20(matic).balanceOf(address(this)); //reuse i for amount to save gas
        bool success;
        if (devCut > 0) {
            uint cut = i.mul(devCut).div(10000);
            IERC20(matic).safeTransfer(devAddr, cut);
            i = i.sub(cut);
        }
        (, success) = _swap(matic, axo, i);
        if(!success)
