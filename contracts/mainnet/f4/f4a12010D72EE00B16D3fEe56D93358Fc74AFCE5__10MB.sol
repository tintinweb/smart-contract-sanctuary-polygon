...
contract _10MB is ERC20Burnable, Ownable {
    ...
    uint256 public burnThreshold = 0;//1.10e18;
...
// Tax Tiers
    uint256[] public taxTiersTwaps = [0, 5e16, 6e16, 7e16, 8e16, 9e16, 9.5e16, 1e17, 1.05e17, 1.10e17, 1.20e17, 1.30e17, 1.40e17, 1.50e17];
    uint256[] public taxTiersRates = [2000, 1500, 1500, 1500, 1500, 1000, 500, 100, 50, 50, 50, 50, 50, 50];
    ...
    /**
     * @notice Constructs the 10MB ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("10MB", "10MB") {
        ...
        _mint(0x6fBA0D1cc256226413f729ACbfe181d963Bb443D, 1000000 ether);
        taxRate = _taxRate;
    ...
    function _get10MBPrice() internal view returns (uint256 __10MBPrice) {
        try IOracle(oracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("10MB: failed to fetch 10MB price from Oracle");
        }
    }

    function _updateTaxRate(uint256 __10MBPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (__10MBPrice >= taxTiersTwaps[tierId]) {
    ...
    function setOperator(address operator, bool isOperator) public onlyOwner {
        require(operator != address(0), "oracle address cannot be 0 address");
        operators[operator] = isOperator;
    }

    function set10MBOracle(address _oracle) public onlyOperatorOrTaxOffice {
    ...
    /**
     * @notice Operator mints 10MB to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of 10MB to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 current10MBPrice = _get10MBPrice();
            currentTaxRate = _updateTaxRate(current10MBPrice);
            if (current10MBPrice < burnThreshold) {
                burnTax = true;
            }
        }
