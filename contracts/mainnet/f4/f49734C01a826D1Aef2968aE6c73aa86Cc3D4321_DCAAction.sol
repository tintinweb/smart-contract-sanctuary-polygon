...
contract DCAAction is BaseSiloAction {
  ...
  function swapAction(
    ...
    for (uint256 i = 0; i < subRoutesData.length; ) {
      ...
      SubRoute[] memory subRoutes = abi.decode(subRoutesData[i], (SubRoute[]));

      if (autoDCA) {
        outputAmounts[i] = swapWithUniRoutes(subRoutes, inputAmounts[i]);
      } else {
        ...
        outputAmounts[i] = swapWithUniRoutes(subRoutes, _investAmounts[i]);
      }
      unchecked {
        i++;
      }
