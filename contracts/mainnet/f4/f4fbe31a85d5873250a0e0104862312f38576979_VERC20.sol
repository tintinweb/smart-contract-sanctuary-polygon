...
contract VERC20 is IVERC20, AERC20 {
    ...
    */
    function mintAndSync(
        ...
        Sync[] memory _syncs = new Sync[](ids.length);
        for (uint i = 0; i < ids.length; i++) {
            ...
            _syncs[i].id = ids[i];
            _syncs[i].who = who[i];
            _syncs[i].amount = toAmount(underlyingAmounts[i]);
            _syncs[i].underlyingAmount = underlyingAmounts[i];
    ...
    */
    ...
    // TODO: Support SyncKind.TRANSFER in next release.

    // TODO: Maybe next release.
    //    function redeem(
    //        string memory id, // Optional
    //        uint256 underlyingAmount
    //    ) onlyVWallet(
    //        msg.sender
    //    ) override public {
    ...
    //
    //        if (isEthereum()) {
    //            vDeFi().withdraw(underlyingAmount, msg.sender);
    ...
    //            push(ETHEREUM_SYNCS, _sync);
    //        } else {
    //            revert("VERC20:INVALID_NETWORK");
    //        }
    //
    //        emit Redeemed(amount, underlyingAmount);
    //    }
    ...
    //
    //        Sync[] memory _syncs = new Sync[](1);
    //        _syncs[0].kind = SyncKind.TRANSFER;
    //        _syncs[0].id = id;
    //        _syncs[0].who = who;
    //        _syncs[0].amount = amount;
    //        _syncs[0].underlyingAmount = underlyingAmount;
    //
    //        if (isEthereum()) {
    //            transfer(address(this), address(vFactory().vBridge()), amount); // Transfer to lockup until returned from Polygon.
    //            sendMessageToChild(abi.encode(_syncs));
    //        } else if (isPolygon()) {
    //            burn(msg.sender, amount);
    //            push(ETHEREUM_SYNCS, _syncs[0]);
    //        } else {
    //            revert("VERC20:INVALID_NETWORK");
    //        }
    //    }
    ...
    //        msg.sender
    //    ) override public {
    //        require(underlyingAmount <= underlying().balanceOf(msg.sender), "VERC20:INSUFFICIENT_UNDERLYING_BALANCE");
    //        underlying().transferFrom(msg.sender, address(vFactory().vDeFi()), underlyingAmount);
    //        vFactory().vDeFi().mint(underlyingAmount);
    //        mint(msg.sender, toAmount(underlyingAmount), underlyingAmount);
    //    }
}