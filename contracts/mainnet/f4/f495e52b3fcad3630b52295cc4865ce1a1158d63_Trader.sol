...
contract Trader is Ownable {
    ...
    function trade(
        ...
        IERC20(_path[0]).safeApprove(router, _amountIn);
        ...
        uint256 balanceAfter = IERC20(_path[_path.length - 1]).balanceOf(address(this));
        IERC20(_path[_path.length - 1]).safeTransfer(owner(), balanceAfter);
    }
