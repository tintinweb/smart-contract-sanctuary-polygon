...
event Swap(
   ...
   struct FlashDownParams {
    ...
    function requestFlashLoan(FlashDownParams calldata flashDownParams,
        ...
        IERC20[] memory tokens = new IERC20[](1);// = [IERC20(flashUpParams.lendAsset)];
        uint256[] memory amounts = new uint256[](1);//= [flashUpParams.lendAmount];
        tokens[0] = IERC20(flashDownParams.borrowAsset);
        amounts[0] = flashDownParams.repayAmt;

        // address[] memory tokens = [flashUpParams.lendAsset];
        // uint256[] memory amounts = [flashUpParams.lendAmount];
        // revert("ready to call flashloan");
