...
contract BondingCalculatorV3 is IBondingCalculator {
    ...
    function getBondTokenValue( address _pair, uint amount_ ) external view override returns ( uint _value ) {
        if ( IUniswapV3Pool(_pair).token0() == SWAP ) {
            _value = getPriceX1(getPriceX96(_pair)).mul(amount_);
        } else {
            _value = amount_.div(getPriceX1(getPriceX96(_pair)));
        }
    }
    ...
    function getPriceX1(uint256 priceX96_) public pure returns(uint256 _priceX1) {
        return priceX96_.div(FixedPoint96.Q96);
    }
}
