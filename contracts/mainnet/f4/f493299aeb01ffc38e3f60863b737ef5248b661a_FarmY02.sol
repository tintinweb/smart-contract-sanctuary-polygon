...
uint nivel = 1;                 event changeUpT(uint upT); 
...
function myNivel() public returns(uint) {      
    if (nivel==1 && block.timestamp > inicio[msg.sender] + upT)    {nivel=2;}
    if (nivel==2 && block.timestamp > inicio[msg.sender] + upT*2)  {nivel=3;}
    if (nivel==3 && block.timestamp > inicio[msg.sender] + upT*2)  {nivel=4;}
    if (nivel==4 && block.timestamp > inicio[msg.sender] + upT*2)  {nivel=5;}
    return (nivel);                     }
