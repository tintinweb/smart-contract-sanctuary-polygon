...
library LibCoreStorage {
    ...
    function coreStorage() internal pure returns (CoreStorage storage cs) {
        bytes32 location = keccak256("swapper.proxy.core");
...
abstract contract ProxyOwnable {
    ...
    modifier onlyOwner {
        require(msg.sender == getOwner(), "Proxy: !OWNER");
        _;
    }
...
abstract contract ProxyReentrancyGuard {
    modifier nonReentrant {
        ...
        require(!cs.guardReentrancy, "Proxy: REENTRANCY_DISALLOWED");
...
contract SwapperProxy is ProxyOwnable, ProxyPausable, ProxyReentrancyGuard {
    fallback() external payable notPaused {
        address impl = getImplementation(msg.sig);
        require(impl != address(0), "Proxy: INVALID_SELECTOR");
    ...
    function setImplementation(bytes4 selector, address implementation, bool initialize) external onlyOwner {
        require(implementation == address(0) || _isContract(implementation), "Proxy: INVALID_IMPLEMENTATION");
        ...
        if(initialize){
            ...
            require(success, "Proxy: INITIALIZATION_FAILED");
        }
