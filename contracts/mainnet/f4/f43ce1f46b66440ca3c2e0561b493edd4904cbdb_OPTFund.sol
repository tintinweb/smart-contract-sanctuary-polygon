...
contract OPTFund is Ownable, ReentrancyGuard {
    ...
    address public constant USDT = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063;
    ...
    function depositOPT3(uint256 _amount) external {
        ...
        uint256 ownsDepositOrNot = getIDFromAddr(msg.sender);

        if(ownsDepositOrNot == 0) {
    ...
    function borrowUsingOPT3(uint256 _OPT3Amount) external {
        ...
        if(ownsLoanOrNot >= 1) {
            ...
            require(idToLoan[loanID].paidOff = true);
            ...
            IERC20(USDT).transfer(msg.sender, loanAmountToSend);
        ...
        if(ownsLoanOrNot == 0) {
            ...
            adrToLoanID[msg.sender] = newLoanID;
    ...
    function claimLoanRebases(uint256 _id) public {
    ...
    function repayLoan(uint256 _id) external {
        require(idToLoan[_id].taker == msg.sender);
        ...
        uint256 userNeedsToPay = repayingAmount.div(1000000);
        uint256 forDev = calculateRepayingToDev(_id);
        uint256 devPay = forDev.div(1000000);
        uint256 forLQ = calculateRepayingForOptimusLQ(_id);
        uint256 LQPay = forLQ.div(1000000);

        IERC20(USDT).transferFrom(msg.sender, address(this), userNeedsToPay);

        IERC20(USDT).transfer(devWallet, devPay);
        IERC20(USDT).transfer(OPT3Pair, LQPay);
