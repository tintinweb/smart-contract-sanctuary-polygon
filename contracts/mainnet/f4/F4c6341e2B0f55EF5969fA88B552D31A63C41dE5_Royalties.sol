...
contract Royalties is ReentrancyGuard {
    ...
    function deposit(uint256 amount) external payable allowed {
        ...
        if(msg.value == 0){
            ...
            IWETH(address(wmatic)).deposit{value: address(this).balance}();
            _amount = msg.value;
        }

        feesPerEpoch[epoch] = _amount;
        totalSupply[epoch] = nft.totalSupply();
        reservedAmounts[epoch] = nft.reservedAmount();
        epoch++;
    }
    ...
    function claimable(address user) public view returns(uint) {
        ...
        //Total fees * AeNFT.originalMinters[msg.sender] / (AeNFT.totalSupply - AeNFT.reservedAmount)
