    ...
    AccessControlFacet
{
    ...
    // The commissions as a multiple of DENOMINATOR.
    // Example: 10% = 0.1 * 100000 (aka 1e5) = 10000
    uint256 public referrerCommission;
    uint256 public referrerPremiumCommission;
    ...
    bytes32 public constant PREMIUM_REFERRER_ROLE =
        keccak256("PREMIUM_REFERRER_ROLE");
    ...
    function mint(string calldata identityHash) external payable {
        mint(identityHash, payable(address(0)));
    }

    function mint(
        string calldata identityHash,
        ...
        if (referrer == address(0)) {
            ...
            uint256 commissionAmount = (msg.value * _getCommission(referrer)) /
                DENOMINATOR;

            if (commissionAmount > 0) {
                referrer.sendValue(commissionAmount);
                emit ReferrerRewarded(referrer, commissionAmount, tokenId);
            }

            payable(_getOwner()).sendValue(msg.value - commissionAmount);
        }
    }
    ...
    function setCommissions(
        uint256 referrerCommission_,
        uint256 referrerPremiumCommission_
    ) external {
        LibAccessControl.enforceRole(LibRoles.DEFAULT_ADMIN_ROLE);

        referrerCommission = referrerCommission_;
        referrerPremiumCommission = referrerPremiumCommission_;
    }
    ...
    function burn(uint256[] calldata tokenIds) external {
        LibAccessControl.enforceRole(BURNER_ROLE);
    ...
    function _getCommission(
        address referrer
    ) internal view returns (uint256 commission) {
        if (LibAccessControl.hasRole(PREMIUM_REFERRER_ROLE, referrer)) {
            return referrerPremiumCommission;
        }
