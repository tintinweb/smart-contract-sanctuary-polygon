 ...
 */
 contract Presale is Owned{
    ...
    // address of usdt stablecoin
    ERC20 public usdt;
    // buy rate
    uint public rate;
    // amount of wei raised
    uint public usdtRaised;
    ...
    uint256 public minUsdtLimit = 300 * (10 ** 18);
    uint256 public maxUsdtLimit = 600 * (10 ** 18);
    ...
    constructor(uint256 _rate, address _wallet, ERC20 _usdt, ERC20 _pToken, uint _closingTime, uint _vestedTime) {
        require(_rate > 0);
    ...
    // allows buyers to put their usdt to get some pToken once the presale will closes
    function buy(uint _amount) public {
        require(isPresaleOpen, "Presale is not open yet");
        require(isList == false || whiteListed[msg.sender] == true);
        require(_amount >= minUsdtLimit, "You need to sell at least some min amount");
        require(paidTotal[msg.sender] + _amount <= maxUsdtLimit);
        ...
        // update state
        usdtRaised = usdtRaised.add(_amount);
        ...
        preBuys[msg.sender].usdtAmount = preBuys[msg.sender].usdtAmount.add(_amount);
    ...
    function getClaimableAmount(address _account) public view returns (uint) {
        ...
        uint totalpTokenToClaim = preBuys[_account].usdtAmount.mul(rate).div(RATE_DECIMALS).mul(percentReleased).div(VESTING_TIME_DECIMALS);
    ...
    function claim() public {
        ...
        uint totalpTokenToClaim = preBuys[msg.sender].usdtAmount.mul(rate).div(RATE_DECIMALS).mul(percentReleased).div(VESTING_TIME_DECIMALS);
    ...
    // allows operator wallet to get the usdt deposited in the contract
    function retreiveUSDT() public {
