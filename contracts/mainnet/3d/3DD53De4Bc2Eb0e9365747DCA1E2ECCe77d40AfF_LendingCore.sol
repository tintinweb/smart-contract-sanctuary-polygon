...
/* validators */
import { AccountValidator } from "../common/validator/AccountValidator.sol";
/* Logics */
import { CreditLogic } from "../common/logic/CreditLogic.sol";
import { SavingsLogic } from "../common/logic/SavingsLogic.sol";
import { FlashLoanLogic } from "../common/logic/FlashLoanLogic.sol";

contract LendingCore is ILendingCore, Initializable, UUPSUpgradeable {
	/* using deps */
	...
	mapping(address => mapping(address => uint256))
		private _lastBorrowedTimestamps;
	...
	function borrow(
		...
		CreditLogic.executeBorrow(
			_controller,
			_lastBorrowedTimestamps,
			SharedTypes.ExecuteBorrowParams({
				asset: asset,
				account: msg.sender,
				onBehalfOf: onBehalfOf,
				amount: amount,
				interestRateMode: interestRateMode,
				oracle: _controller.getPriceOracle()
			})
		);
	}
	...
	function repay(
		...
		return
			CreditLogic.executeRepay(
				_controller,
				_lastBorrowedTimestamps,
				SharedTypes.ExecuteRepayParams({
					asset: asset,
					onBehalfOf: onBehalfOf,
					amount: amount,
					rateMode: rateMode
	...
	function repayBySig(
		...
		IERC20PermitUpgradeable(asset).permit(
			msg.sender,
			address(_controller),
			amount,
			deadline,
	...
	///@inheritdoc ILendingCore
	function swapCreditRateMode(address asset, uint256 rateMode)
		external
		override
		whenNotPaused
	{
		CreditLogic.executeSwapCreditRateMode(_controller, asset, rateMode);
	}

	///@inheritdoc ILendingCore
	function rebalanceStableCreditRate(address asset, address account)
		external
		override
		whenNotPaused
	{
		CreditLogic.executeRebalanceStableCreditRate(
			_controller,
			asset,
			account
		);
	...
	function flashLoan(
		...
		FlashLoanLogic.executeSingleFlashLoan(
			_controller,
			SharedTypes.ExecuteFlashLoanParams({
				receiver: receiver,
				asset: asset,
				amount: amount,
				data: data
	...
	function flashLoan(
		...
		FlashLoanLogic.executeMultiFlashLoan(
			_controller,
			SharedTypes.ExecuteMultiFlashLoanParams({
				receiver: receiver,
				assets: assets,
				amounts: amounts,
				data: data
	...
	function validateTransfer(
		...
		SavingsLogic.executeValidateTransfer(
			_controller,
			SharedTypes.ValidateTransferParams({
				asset: asset,
				from: from,
				to: to,
				amount: amount,
	...
	function getVaultNormalizedVariableDebt(address asset)
		...
		returns (uint256)
	{
		return _controller.getVaultData(asset).getNormalizedDebt();
	}
	...
	{
		return
			CreditLogic.getAccountCurrentDebt(
				account,
				_controller.getVaultData(asset)
			);
	}
		...
		)
	{
		(
			...
			ltv,
			healthFactor
		) = _controller.getAccountLiquidityData(account);
	}
	...
	function flashFee(address asset, uint256 amount)
		...
		returns (uint256)
	{
		VaultModel.Data memory vault = _controller.getVaultData(asset);

		return FlashLoanLogic.flashFee(vault.vaultToken, amount);
	}
		...
		returns (uint256)
	{
		...
		return FlashLoanLogic.flashFeeInPercent(vault.vaultToken, amount);
	}

	function setController(ICoreController coreController) external {
...
import { IPriceOracle } from "../interface/IPriceOracle.sol";
/* validators */
import { GroupValidator } from "../validator/GroupValidator.sol";
/* types */
import { SharedTypes } from "../type/SharedTypes.sol";

/**
 * @title CreditLogic library
 * @author eazi
 * @notice Implements the base logic for all the actions related to lending/credits
 */
library CreditLogic {
	/* using deps */
	using VaultConfigController for VaultModel.Config;
	...
	/**
	 * @notice Implements the borrow logic of the LendingCore. Borrowing allows users that provided collateral(has Credit Limit > 0) to take loan from
	 * eazi Protocol available liquidity proportionally to their collateralization power.
	 * @dev Emits `Borrowed()` event
	 * @param coreController coreController instance
	 * @param params The additional parameters required by the borrow function
	 */
	function executeBorrow(
		ICoreController coreController,
		mapping(address => mapping(address => uint256))
			storage lastBorrowedTimestamps,
		SharedTypes.ExecuteBorrowParams memory params
	) external {
		VaultModel.Data memory vault = coreController.getVaultData(
			params.asset
		);
		AccountModel.Config memory accountConfig = coreController
			.getAccountConfig(params.onBehalfOf);

		{
			uint256 amountInBaseCurrency = (IPriceOracle(params.oracle)
				.getAssetPrice(params.asset) * params.amount) /
				(10**vault.config.getDecimals());

			GroupValidator.validateBorrow(
				vault,
				accountConfig,
				coreController.getVaultData,
				SharedTypes.ValidateBorrowParams({
					asset: params.asset,
					account: params.onBehalfOf,
					amount: params.amount,
					amountInBaseCurrency: amountInBaseCurrency,
					interestRateMode: params.interestRateMode,
					maxStaticLoanPercent: coreController.getStaticPercent(),
					oracle: params.oracle,
					vaults: coreController.getVaults()
				})
			);
			vault.updateState();
		}

		{
			...
			bool isReturningOrNew = false;

			if (
				SharedTypes.InterestRateMode(params.interestRateMode) ==
				SharedTypes.InterestRateMode.STABLE
			) {
				currentStableRate = vault.currentStableCreditRate;

				isReturningOrNew = IStableCreditToken(vault.stableCreditToken)
					.mint(
						params.account,
						params.onBehalfOf,
						params.amount,
				...
				isReturningOrNew = IVariableCreditToken(
					vault.variableCreditToken
				).mint(
						params.account,
						params.onBehalfOf,
						params.amount,
			...
			if (isReturningOrNew) {
				accountConfig.isBorrowingFromVault(vault.id, true);
				coreController.updateAccountConfig(
					params.onBehalfOf,
					accountConfig.data
				);
			}
			...
			vault.updateInterestRates(
				...
				0,
				params.amount,
				coreController.onInterestRateUpdate
			);

			// update Vault data
			coreController.updateVault(params.asset, vault);

			// create account ID if not exists
			coreController.tryCreateAccount(params.account);

			// create recipient account ID if not exists
			coreController.tryCreateAccount(params.onBehalfOf);

			//solhint-disable-next-line not-rely-on-time
			lastBorrowedTimestamps[params.asset][params.account] = block
				.timestamp;

			IVaultToken(vault.vaultToken).transferUnderlying(
				params.account,
				params.amount
			);
	...
	/**
	 * @notice Implements the repay logic of the LendingCore. Repaying transfers the underlying asset back to the vaultToken and clears the
	 * equivalent amount of debt for the account by burning the corresponding debt token.
	 * @dev Emits `Repaid()` event
	 * @param coreController coreController instance
	 * @param params The additional parameters required by the repay function
	...
	function executeRepay(
		...
		mapping(address => mapping(address => uint256))
			storage lastBorrowedTimestamps,
		SharedTypes.ExecuteRepayParams memory params
	) external returns (uint256) {
		VaultModel.Data memory vault = coreController.getVaultData(
			params.asset
		);

		(uint256 stableDebt, uint256 variableDebt) = getAccountCurrentDebt(
			params.onBehalfOf,
			vault
		);

		SharedTypes.InterestRateMode interestRateMode = SharedTypes
			.InterestRateMode(params.rateMode);

		GroupValidator.validateRepayment(
			vault,
			params.amount,
			interestRateMode,
			params.onBehalfOf,
			stableDebt,
			variableDebt,
			lastBorrowedTimestamps[params.asset][params.onBehalfOf]
		);
		...
		if (interestRateMode == SharedTypes.InterestRateMode.STABLE) {
			IStableCreditToken(vault.stableCreditToken).burn(
				params.onBehalfOf,
				paybackAmount
			);
		} else {
			IVariableCreditToken(vault.variableCreditToken).burn(
				params.onBehalfOf,
				paybackAmount,
				vault.variableCreditIndex
			);
		...
		address vaultToken = vault.vaultToken;
		...
		if ((stableDebt + variableDebt) - paybackAmount == 0) {
			AccountModel.Config memory accountConfig = coreController
				.getAccountConfig(params.onBehalfOf);
			accountConfig.isBorrowingFromVault(vault.id, false);
			coreController.updateAccountConfig(
				params.onBehalfOf,
				accountConfig.data
			);
		...
		IVaultToken(vaultToken).onRepayment(msg.sender, paybackAmount);

		emit Repaid(params.asset, params.onBehalfOf, msg.sender, paybackAmount);

		return paybackAmount;
	}

	/**
	 * @notice Implements the stable credit rate rebalance logic of the LendingCore. In case of liquidity crunches on the protocol, stable
	 * rate credits might need to be rebalanced to bring back equilibrium between the credit and savings interest rate(APY).
	 * @dev The rules that define if a position can be rebalanced are implemented in `GroupValidator.validateRebalanceStableCreditRate()`
	 * @dev Emits the `RebalanceStableCreditRate()` event
	 * @param coreController coreController instance
	 * @param asset The asset of the position being rebalanced
	 * @param account The account being rebalanced
	 */
	function executeRebalanceStableCreditRate(
		ICoreController coreController,
		address asset,
		address account
	) external {
		VaultModel.Data memory vault = coreController.getVaultData(asset);

		IERC20Upgradeable stableCreditToken = IERC20Upgradeable(
			vault.stableCreditToken
		);
		IERC20Upgradeable variableCreditToken = IERC20Upgradeable(
			vault.variableCreditToken
		);

		address vaultToken = vault.vaultToken;

		uint256 stableDebt = IERC20Upgradeable(stableCreditToken).balanceOf(
			account
		);

		GroupValidator.validateRebalanceStableCreditRate(
			vault,
			asset,
			stableCreditToken,
			variableCreditToken,
		...
		IStableCreditToken(address(stableCreditToken)).burn(
			account,
			stableDebt
		);
		IStableCreditToken(address(stableCreditToken)).mint(
			account,
			account,
			stableDebt,
		...
		coreController.updateVault(asset, vault);
		emit RebalancedStableCreditRate(asset, account);
	}

	/**
	 * @notice Implements the swap borrow rate feature. Borrowers can swap from variable to stable positions at any time.
	 * @dev Emits `SwappedCreditRateMode()` event
	 * @param coreController coreController instance
	 ...
	 */
	function executeSwapCreditRateMode(
		ICoreController coreController,
		address asset,
		uint256 rateMode
	) external {
		...
		GroupValidator.validateSwapRateMode(
			vault,
			coreController.getAccountConfig(msg.sender),
			stableDebt,
			variableDebt,
			interestRateMode
		);
		...
		// coreController.updateVault(asset, vault);
		/* 
			Would have updated the vaults here before making these external call, but its a trusted call
			and does not rely on any of the vault parameters which are to be updated 
		*/
		if (interestRateMode == SharedTypes.InterestRateMode.STABLE) {
			IStableCreditToken(vault.stableCreditToken).burn(
				msg.sender,
			...
			IVariableCreditToken(vault.variableCreditToken).mint(
				msg.sender,
				msg.sender,
				stableDebt,
				vault.variableCreditIndex
			...
			IStableCreditToken(vault.stableCreditToken).mint(
				msg.sender,
				msg.sender,
				variableDebt,
				vault.currentStableCreditRate
		...
		coreController.updateVault(asset, vault);
		emit SwappedCreditRateMode(msg.sender, asset, rateMode);
	}

	function getAccountCurrentDebt(
		address account,
		VaultModel.Data memory vault
	) public view returns (uint256, uint256) {
		return (
			IERC20Upgradeable(vault.stableCreditToken).balanceOf(account),
			IERC20Upgradeable(vault.variableCreditToken).balanceOf(account)
		);
	}
}
...
import { AccountController } from "../controller/AccountController.sol";
/* interfaces */
import { ICDVault } from "../interface/ICDVault.sol";
import { ICoreController } from "../interface/ICoreController.sol";
import { IVaultToken } from "../interface/IVaultToken.sol";
/* validators */
import { GroupValidator } from "../validator/GroupValidator.sol";
...
/**
 * @title SavingsLogic library
 * @author eazi
 * @notice Implements the base logic for all the actions related to Savings
 */
library SavingsLogic {
	/* using deps */
	using VaultController for VaultModel.Data;
	using AccountController for AccountModel.Config;
	...
	/**
	 * @notice Implements the deposit feature of SavingsCore. Through `deposit()`, accounts deposit assets to the protocol.
	 * @dev Emits the `Deposited()` event.
	 * @dev Emits `VaultEnabledAsCollateral()`, if the asset can be enabled as
	 * collateral.
	 */
	function executeDeposit(
		...
		GroupValidator.validateDeposit(vault, amount);
		...
		// create account ID if not exists
		coreController.tryCreateAccount(msg.sender);

		// create recipient account ID if not exists
		coreController.tryCreateAccount(to);

		/* let controller(only one who can make pull request) pull Asset */
		coreController.pullAsset(asset, msg.sender, vaultToken, amount);

		// mint the vault Token(Savings Token)
		bool isReturningOrNew = IVaultToken(vaultToken).mint(
			to,
			amount,
		...
		if (isReturningOrNew) {
			AccountModel.Config memory accountConfig = coreController
				.getAccountConfig(to);
			accountConfig.vaultIsCollateral(vault.id, true);
			coreController.updateAccountConfig(to, accountConfig.data);
			emit VaultEnabledAsCollateral(asset, to);
		}
	...
	/**
	 * @notice Implements the withdraw feature of SavingsCore. Through `withdraw()`, accounts unlock their vaults
	 * for the underlying asset previously deposited to the protocol.
	 * @dev Emits the `Withdrawn()` event.
	 * @dev If the account withdraws everything, `VaultDisabledAsCollateral()` is emitted.
	 * @return The actual amount withdrawn
	 */
	function executeWithdraw(
		ICoreController coreController,
		SharedTypes.ExecuteWithdrawParams memory params
	) external returns (uint256) {
		VaultModel.Data memory vault = coreController.getVaultData(
			params.asset
		);
		...
		uint256 accountBalance = IVaultToken(vaultToken).balanceOf(msg.sender);

		uint256 amountToWithdraw = params.amount;

		// if amount is equal to type(uint).max, the accountholder wants to redeem everything
		if (params.amount == type(uint256).max) {
			amountToWithdraw = accountBalance;
		}

		AccountModel.Config memory accountConfig = coreController
			.getAccountConfig(msg.sender);

		{
			GroupValidator.validateWithdraw(
				accountConfig,
				coreController.getVaultData,
				SharedTypes.ValidateWithdrawParams({
					amount: amountToWithdraw,
					accountBalance: accountBalance,
					asset: params.asset,
					oracle: coreController.getPriceOracle(),
					vaults: coreController.getVaults()
				})
			);
		...
		if (amountToWithdraw == accountBalance) {
			accountConfig.vaultIsCollateral(vault.id, false);
			coreController.updateAccountConfig(msg.sender, accountConfig.data);
			emit VaultDisabledAsCollateral(params.asset, msg.sender);
		}

		coreController.updateVault(params.asset, vault);

		IVaultToken(vaultToken).burn(
			msg.sender,
			params.to,
			amountToWithdraw,
			vault.liquidityIndex
		...
		emit Withdrawn(params.asset, msg.sender, params.to, amountToWithdraw);

		return amountToWithdraw;
	}

	/**
	 * @notice awaiting audit
	 */
	function executeCDWithdraw(
		ICoreController coreController,
		ICDVault cdVault,
		uint256 tokenId,
		address to
	) external returns (uint256) {
		ICDVault.AssetData memory assetData = cdVault.getData(tokenId);

		address asset = assetData.underlying;

		VaultModel.Data memory vault = coreController.getVaultData(asset);

		address vaultToken = vault.vaultToken;

		uint256 amountToWithdraw = cdVault.getBalance(tokenId);

		GroupValidator.validateCDWithdraw(
			asset,
			amountToWithdraw,
			coreController.getVaultData
		);
		...
		cdVault.burn(tokenId, msg.sender, vault.liquidityIndex);
		// withdraw all balance
		IVaultToken(vaultToken).transferUnderlying(to, amountToWithdraw);

		emit WithdrawnCD(asset, msg.sender, to, amountToWithdraw, tokenId);
	...
	function executeCDDeposit(
		...
		// pull Asset into Vault token, but
		coreController.pullAsset(asset, msg.sender, vaultToken, amount);

		// not mint the vault Token(Savings Token) as usual, instead minting vaultNFT
		// which won't count as collateral(for liquidations, borrowing), or other utilizations
		// within the group than being certificate of deposit
		uint256 id = cdVault.mint(
			to,
			vaultToken,
			asset,
			amount,
		...
		emit DepositedCD(asset, msg.sender, to, amount, id);
		return id;
	}

	/**
	 * @notice Validates a Vault Token transfer. The sender is subjected to health factor validation to avoid
	 * collateralization constraints violation.
	 * @dev Emits the `VaultDisabledAsCollateral()` and `VaultEnabledAsCollateral()` event.
	 * @param coreController The controller
	 * @param params The additional parameters needed to execute the validateTransfer function
	 */
	function executeValidateTransfer(
		ICoreController coreController,
		SharedTypes.ValidateTransferParams memory params
	) external {
		VaultModel.Data memory vault = coreController.getVaultData(
			params.asset
		);

		//accessible only by the vaultToken
		if (msg.sender != vault.vaultToken) {
			revert NotVaultToken();
		}

		GroupValidator.validateTransfer(
			params.from,
			coreController.getVaultData,
			coreController.getAccountConfig(params.from),
			coreController.getVaults(),
			coreController.getPriceOracle()
		);
		...
		if (params.from != params.to) {
			if (params.balanceFromBefore - params.amount == 0) {
				AccountModel.Config memory accountConfig = coreController
					.getAccountConfig(params.from);

				accountConfig.vaultIsCollateral(vaultId, false);
				coreController.updateAccountConfig(
					params.from,
					accountConfig.data
				);
				emit VaultDisabledAsCollateral(params.asset, params.from);
			}

			if (params.balanceToBefore == 0 && params.amount != 0) {
				AccountModel.Config memory toConfig = coreController
					.getAccountConfig(params.to);

				toConfig.vaultIsCollateral(vaultId, true);
				coreController.updateAccountConfig(params.to, toConfig.data);
				emit VaultEnabledAsCollateral(params.asset, params.to);
			}
		}
	}

	/**
	 * @notice Executes the 'set as collateral' feature. An account can choose to activate or deactivate an asset as
	 * collateral at any point in time. Deactivating an asset as collateral is subjected to the usual health factor
	 * checks to ensure collateralization.
	 * @dev Emits the `VaultDisabledAsCollateral()` and `VaultEnabledAsCollateral()`.
	 * @dev In case the asset is being deactivated as collateral, `ReserveUsedAsCollateralDisabled()` is emitted.
	 * @param coreController The controller
	 * @param asset The address of the asset being configured as collateral
	 * @param useAsCollateral True if the user wants to set the asset as collateral, false otherwise
	 */
	function executeUseVaultAsCollateral(
		ICoreController coreController,
		address asset,
		...
		GroupValidator.validateVaultAsCollateral(
			vault,
			asset,
			useAsCollateral,
			coreController.getVaultData,
		...
		AccountModel.Config memory accountConfig = coreController
			.getAccountConfig(msg.sender);
		accountConfig.vaultIsCollateral(vault.id, useAsCollateral);

		coreController.updateAccountConfig(msg.sender, accountConfig.data);

		if (useAsCollateral) {
			emit VaultEnabledAsCollateral(asset, msg.sender);
		} else {
			emit VaultDisabledAsCollateral(asset, msg.sender);
		}
	}
}
...
import { IMultiFlashLoanReceiver } from "../interface/IMultiFlashLoanReceiver.sol";
/* validators */
import { GroupValidator } from "../validator/GroupValidator.sol";
/* types */
import { SharedTypes } from "../type/SharedTypes.sol";
 ...
 * @author eazi
 * @notice Implements the base logic for all the actions related to Flash Loans
 */
library FlashLoanLogic {
	...
	/**
	 * @dev This feature requires the receiver to be compatible with ERC3156->IFlashLoanReceiver interface
	 * @notice Implements the flashloan feature of LendingCore. Allows smart contracts to access and utilize the Group's liquidity within one transaction,
	 * as long as the amount taken plus a fee(which can be calculated in advance with calculateFlashFee) is returned.
	 * @dev At the end of the transaction the protocol will pull amount borrowed + fee from the receiver,
	 * if the receiver have not approved `coreController` the transaction will revert.
	...
	function executeSingleFlashLoan(
		ICoreController coreController,
		SharedTypes.ExecuteFlashLoanParams memory params
	) external {
		address vaultToken = coreController
			.getVaultData(params.asset)
			.vaultToken;

		uint256 fee = flashFee(vaultToken, params.amount);

		IVaultToken(vaultToken).transferUnderlying(
			params.receiver,
			params.amount
		);

		if (
			IFlashLoanReceiver(params.receiver).onFlashloan(
				msg.sender,
				params.asset,
				params.amount,
				fee,
		...
		uint256 amountPlusPremium = (params.amount + fee);

		_onFlashLoanSuccess(
			coreController,
			coreController.getVaultData(params.asset),
			SharedTypes.FlashLoanRepaymentParams({
				premium: fee,
				amount: params.amount,
				totalPremium: amountPlusPremium,
				asset: params.asset,
	...
	/**
	 * @dev This feature requires the receiver to be compatible with IMultiFlashLoanReceiver interface
	 * @notice Implements the flashloan feature of LendingCore. Allows smart contracts to access and utilize the Group's liquidity within one transaction,
	 * as long as the amount taken plus a fee(which can be calculated in advance with calculateFlashFee) is returned.
	 * @dev At the end of the transaction the protocol will pull amount borrowed + fee from the receiver,
	 * if the receiver have not approved `coreController` the transaction will revert.
	...
	function executeMultiFlashLoan(
		ICoreController coreController,
		SharedTypes.ExecuteMultiFlashLoanParams memory params
	) external {
		FlashLoanLocalVars memory vars;

		GroupValidator.validateMultiFlashloan(params.assets, params.amounts);

		address[] memory vaultTokens = new address[](params.assets.length);
		uint256[] memory fees = new uint256[](params.assets.length);

		for (vars.i = 0; vars.i < params.assets.length; vars.i++) {
			vaultTokens[vars.i] = coreController
				.getVaultData(params.assets[vars.i])
				.vaultToken;

			fees[vars.i] = flashFee(
				vaultTokens[vars.i],
				params.amounts[vars.i]
			);

			IVaultToken(vaultTokens[vars.i]).transferUnderlying(
				params.receiver,
				params.amounts[vars.i]
			);
		}
		...
		if (
			IMultiFlashLoanReceiver(params.receiver).onFlashloan(
				msg.sender,
				params.assets,
				params.amounts,
				fees,
		...
		for (vars.i = 0; vars.i < params.assets.length; vars.i++) {
			...
			vars.currentPremium = fees[vars.i];
			vars.currentVaultToken = vaultTokens[vars.i];
			vars.currentAmountPlusPremium = (vars.currentAmount +
				vars.currentPremium);

			_onFlashLoanSuccess(
				coreController,
				coreController.getVaultData(vars.currentAsset),
				SharedTypes.FlashLoanRepaymentParams({
					premium: vars.currentPremium,
					amount: vars.currentAmount,
					totalPremium: vars.currentAmountPlusPremium,
					asset: vars.currentAsset,
					receiver: params.receiver,
					vaultToken: vars.currentVaultToken
				})
			);
		}
	}
	...
	/**
	 * @notice Handles repayment of flashloan-ed assets + premium
	 * @dev Will pull the amount + premium from the receiver, so must have approved coreController
	 * @param coreController The Controller
	 * @param vault The flashloan-ed vault Data
	 * @param params The additional parameters needed to execute the repayment function
	 */
	function _onFlashLoanSuccess(
		ICoreController coreController,
		VaultModel.Data memory vault,
		SharedTypes.FlashLoanRepaymentParams memory params
	) internal {
		vault.updateState();
		vault.cumulateToLiquidityIndex(
			IERC20Upgradeable(params.vaultToken).totalSupply(),
			params.premium
		);
		vault.updateInterestRates(
			params.asset,
	...
	function flashFeeInPercent(address asset, uint256 amount)
		...
		returns (uint256)
	{
		uint256 supply = IERC20Upgradeable(asset).totalSupply();
		uint256 supplyPercent = (amount * 100) / supply;

		if (supplyPercent >= 50) return 9;
		else if (supplyPercent >= 30) return 6;
		else if (supplyPercent >= 10) return 3;
	...
	function flashFee(address asset, uint256 amount)
		...
		returns (uint256)
	{
		return (amount * flashFeeInPercent(asset, amount)) / 10000;
	}
}
...
/**
 * @title GroupValidator library
 * @author eaZI
 * @notice validate actions between various protocols(e.g SavingsCore, CreditCore, ...) in the group
 */
library GroupValidator {
	/* errors */
	/// Invalid amount, must be greater than 0
	error InvalidAmount();

	/// Inactive Vault, action requires an active vault
	error InactiveVault();

	/// Frozen Vault, action cannot be performed because the vault is frozen
	error FrozenVault();

	/// Only EOA is allowed to make this call
	error OnlyEOA();

	/// Account Balance not enough for action(usually withdrawal)
	error NotEnoughAccountBalance();

	/// Unallowed Transfer from Vault
	error UnallowedTransfer();

	/// The requested amount is greater than the max loan size in stable rate mode
	error AmountBiggerThanMaxStableCreditLimit();

	/// Borrowing and repaying same asset in the same block is not allowed, to prevent certain kind of abuses
	error BorrowingAndRepayingInSameBlock();

	/// Current Available Liquidity Not Enough, please try again later
	error CurrentAvailableLiquidityNotEnough();

	/// Collateral Balance is Zero
	error CollateralBalanceIsZero();

	/// There is not enough collateral to cover a new credit(debt)
	error CollateralCannotCoverNewDebt();

	/// Collateral is (or mostly) the same asset that is being borrowed
	error CollateralSameAsBorrowingAsset();

	/// Credit is not enabled on this vault
	error CreditNotEnabled();

	/// Credit is not enabled on this vault
	error StableCreditNotEnabled();

	/// Health factor is below the liquidation threshold
	error HealthFactorBelowLiquidationThreshold();

	/// Health factor is not below the liquidation threshold
	error HealtFactorNotBelowLiquidationThreshold();

	/// Invalid interest rate mode selected
	error InvalidInterestRateModeSelected();

	/// Account have no debt in selected mode
	/// @notice for repayment of stable debt, the account needs to have stable debt, otherwise, needs to have variable debt
	error NoDebtOfSelectedMode();

	/// Account does not have a stable rate loan currently on this vault
	error NoStableRateLoanInVault();

	/// Account does not have a variable rate loan currently on this vault
	error NoVariableRateLoanInVault();

	/// Repaying on behalf of another account needs explicit amount to repay
	error RepayOnBehalfOfNeedsExplicitAmount();

	/// Collateral cannot be liquidated
	error CollateralCannotBeLiquidated();

	/// Specified asset not borrowed by account
	error SpecifiedAssetNotBorrowedByAccount();

	/// Valid conditions to rebalance Interest rate not met
	error InvalidConditionToRebalanceInterestRate();

	/// Underlying asset balance is Zero
	error UnderlyingBalanceIsZero();

	/// Vault deposit already in use
	error VaultDepositAlreadyInUse();
	...
	// using SavingsController for VaultModel.Config;
	using VaultController for VaultModel.Data;
	using VaultConfigController for VaultModel.Config;
	using AccountController for AccountModel.Config;

	uint256 public constant REBALANCE_UP_LIQUIDITY_RATE_THRESHOLD = 4000;
	uint256 public constant REBALANCE_UP_USAGE_RATIO_THRESHOLD = 0.80 * 1e27; //usage ratio of 80%

	/* Savings Validator - formerly in SavingsValidator.sol */
	/**
	 * @dev Validates a deposit action
	 * @param vaultData The data object of vault on which deposit is being made
	 * @param amount The amount to be deposited
	 */
	function validateDeposit(VaultModel.Data memory vaultData, uint256 amount)
		external
		pure
	{
		(bool isActive, bool isFrozen, , ) = vaultData.config.getFlags();
	...
	/**
	 * @dev Validates a withdrawal
	 * @param vaultsData The vaults state
	 * @param accountConfig The account configuration
	...
	function validateWithdraw(
		...
		function(address)
			...
			view
			returns (VaultModel.Data memory) vaultsData,
		SharedTypes.ValidateWithdrawParams memory params
	) external view {
		if (params.amount == 0) {
			revert InvalidAmount();
		}

		if (params.amount > params.accountBalance) {
			revert NotEnoughAccountBalance();
		}

		(bool isActive, , , ) = vaultsData(params.asset).config.getFlags();
		if (!isActive) {
			revert InactiveVault();
		}

		if (
			!AccountValidator.balanceDeductionAllowed(
				params.asset,
				msg.sender,
				params.amount,
				vaultsData,
		...
		) revert UnallowedTransfer();
	}

	/**
	 * @dev Validates CD withdrawal
	 * @param vault The vault
	 * @param balance The balance to be withdrawn
	 * @param vaultsData The vaults state
	 */
	function validateCDWithdraw(
		address vault,
		uint256 balance,
		function(address)
			external
		...
		(bool isActive, , , ) = vaultsData(vault).config.getFlags();
		if (!isActive) revert InactiveVault();
	}

	struct ValidateBorrowLocalVars {
		uint256 currentLtv;
		uint256 currentLiquidationThreshold;
		uint256 amountOfCollateralNeededInBaseCurrency;
		uint256 accountCollateralBalanceInBaseCurrency;
		uint256 accountDebtBalanceInBaseCurrency;
		uint256 availableLiquidity;
		uint256 healthFactor;
		bool isActive;
		bool isFrozen;
	 ...
	 * @param vault The vault state from which the account is borrowing
	 * @param vaultsData The state of all the vaults
	 * @param accountConfig The state of the account for the specific vault
	 * @param params struct of required data
	 * 	asset The asset to borrow
	 *  account The account
	 *  amount The amount to be borrowed
	 *  amountInBaseCurrency The amount to be borrowed, in Base currency
	 *  interestRateMode The interest rate mode at which the account is borrowing
	 *  maxStaticLoanPercent The max amount of the liquidity that can be borrowed at stable/fixed rate, in percentage
	 *  vaults The list of active vaults
	 *  oracle The price oracle
	 */
	function validateBorrow(
		...
		function(address)
			...
			view
			returns (VaultModel.Data memory) vaultsData,
		SharedTypes.ValidateBorrowParams memory params
	) external view {
		...
		(
			...
			vars.isFrozen,
			vars.creditEnabled,
			vars.stableCreditEnabled
		) = vault.config.getFlags();

		if (!vars.isActive) revert InactiveVault();
		if (vars.isFrozen) revert FrozenVault();
		if (params.amount == 0) revert InvalidAmount();

		if (!vars.creditEnabled) revert CreditNotEnabled();

		//validate interest rate mode
		if (
			uint256(SharedTypes.InterestRateMode.VARIABLE) !=
			params.interestRateMode &&
			uint256(SharedTypes.InterestRateMode.STABLE) !=
			params.interestRateMode
		) revert InvalidInterestRateModeSelected();

		(
			vars.accountCollateralBalanceInBaseCurrency,
			vars.accountDebtBalanceInBaseCurrency,
			vars.currentLtv,
		...
		if (vars.accountCollateralBalanceInBaseCurrency == 0)
			revert CollateralBalanceIsZero();

		if (
			vars.healthFactor <=
			AccountValidator.HEALTH_FACTOR_LIQUIDATION_THRESHOLD
		) revert HealthFactorBelowLiquidationThreshold();

		//add the current already borrowed amount to the amount requested to calculate the total collateral needed.
		vars.amountOfCollateralNeededInBaseCurrency = (vars
			.accountDebtBalanceInBaseCurrency + params.amountInBaseCurrency)
			.percentDiv(vars.currentLtv);
		//LTV is calculated in percentage

		if (
			vars.amountOfCollateralNeededInBaseCurrency >
			vars.accountCollateralBalanceInBaseCurrency
		) revert CollateralCannotCoverNewDebt();

		vars.availableLiquidity = IERC20Upgradeable(params.asset).balanceOf(
			vault.vaultToken
		);

		/**
		 * Following conditions need to be met if the account is borrowing at a stable rate:
		 * 1. Vault must be enabled for stable rate borrowing
		 * 2. Accounts cannot borrow from the vault if their collateral is (mostly) the same currency
		 *    they are borrowing, to prevent abuses.
		 * 3. Accounts will be able to borrow only a portion of the total available liquidity
		 **/
		...
		if (
			...
			//check if the borrow mode is stable and if stable rate borrowing is enabled on this vault

			if (!vars.stableCreditEnabled) revert StableCreditNotEnabled();

			if (
				accountConfig.vaultIsCollateral(vault.id) &&
				vault.config.getLtv() > 0 &&
				params.amount <
				IERC20Upgradeable(vault.vaultToken).balanceOf(params.account)
			) revert CollateralSameAsBorrowingAsset();
			...
			uint256 maxLoanSizeStable = vars.availableLiquidity.percentMul(
				params.maxStaticLoanPercent
			);

			if (params.amount > maxLoanSizeStable)
				revert AmountBiggerThanMaxStableCreditLimit();
		}

		if (params.amount > vars.availableLiquidity)
			revert CurrentAvailableLiquidityNotEnough();
	}

	/**
	 * @dev Validates a repayment action
	 * @param vault The vault state from which the account is repaying
	 * @param amountSent The amount sent for the repayment. Can be an actual value or type(uint).max
	 * @param onBehalfOf The account msg.sender is repaying for
	 * @param stableDebt The (stable) debt balance of the account
	 * @param variableDebt The (variable) debt balance of the account
	 */
	function validateRepayment(
		VaultModel.Data memory vault,
		uint256 amountSent,
		SharedTypes.InterestRateMode rateMode,
		address onBehalfOf,
		uint256 stableDebt,
		uint256 variableDebt,
		uint256 lastBorrowedTime
	) external view {
		bool isActive = vault.config.getActive();

		if (!isActive) revert InactiveVault();

		if (amountSent == 0) revert InvalidAmount();

		//solhint-disable-next-line not-rely-on-time
		if (lastBorrowedTime == block.timestamp)
			revert BorrowingAndRepayingInSameBlock();

		if (
			variableDebt == 0 &&
			SharedTypes.InterestRateMode(rateMode) ==
			SharedTypes.InterestRateMode.VARIABLE
		) revert NoDebtOfSelectedMode();
		...
		if (amountSent == type(uint256).max && msg.sender != onBehalfOf)
			revert RepayOnBehalfOfNeedsExplicitAmount();
	}

	/**
	 * @dev Validates a swap of credit rate mode.
	 * @param vault The vault state on which the account is swapping the rate
	 * @param accountConfig The account vaults config
	 * @param stableDebt The stable debt of the account
	 * @param variableDebt The variable debt of the account
	...
	function validateSwapRateMode(
		...
		uint256 stableDebt,
		uint256 variableDebt,
		SharedTypes.InterestRateMode currentRateMode
	) external view {
		(bool isActive, bool isFrozen, , bool stableRateEnabled) = vault
			.config
			.getFlags();

		if (!isActive) revert InactiveVault();
		if (isFrozen) revert FrozenVault();

		if (currentRateMode == SharedTypes.InterestRateMode.STABLE) {
			if (stableDebt == 0) revert NoStableRateLoanInVault();
		} else if (currentRateMode == SharedTypes.InterestRateMode.VARIABLE) {
			if (variableDebt == 0) revert NoVariableRateLoanInVault();
			/**
			 * account wants to swap to stable, before swapping we need to ensure that
			 * 1. stable credit rate is enabled on the vault
			 * 2. account is not trying to abuse the vault by depositing
			 * more collateral than he is borrowing, artificially lowering
			 * the interest rate, borrowing at variable, and switching to stable
			 **/
			if (!stableRateEnabled) revert StableCreditNotEnabled();

			if (
				!(!accountConfig.vaultIsCollateral(vault.id) ||
					vault.config.getLtv() == 0 ||
					stableDebt + variableDebt >
					IERC20Upgradeable(vault.vaultToken).balanceOf(msg.sender))
			) revert CollateralSameAsBorrowingAsset();
		} else {
			revert InvalidInterestRateModeSelected();
		}
	...
	/**
	 * @dev Validates a stable credit rate rebalance action
	 * @param vault The vault state on which the account is getting rebalanced
	 * @param vaultAddress The vault
	 * @param stableCreditToken The stable credit token instance
	 * @param variableCreditToken The variable credit token instance
	 * @param vaultToken The vaultToken contract
	 */
	function validateRebalanceStableCreditRate(
		VaultModel.Data memory vault,
		address vaultAddress,
		IERC20Upgradeable stableCreditToken,
		IERC20Upgradeable variableCreditToken,
		address vaultToken
	) external view {
		// to avoid potential abuses, the rebalance stable rate must happen through an EOA
		if (AddressUpgradeable.isContract(address(msg.sender))) {
			revert OnlyEOA();
		}

		(bool isActive, , , ) = vault.config.getFlags();

		if (!isActive) revert InactiveVault();

		//if the usage ratio is below 95%, no rebalances are needed
		uint256 totalDebt = (stableCreditToken.totalSupply() +
			variableCreditToken.totalSupply()).wadToRay();
		uint256 availableLiquidity = IERC20Upgradeable(vaultAddress)
			.balanceOf(vaultToken)
			.wadToRay();
		uint256 usageRatio = totalDebt == 0
			? 0
			: totalDebt.rDiv(availableLiquidity + totalDebt);

		//if the liquidity rate is below REBALANCE_UP_THRESHOLD of the max variable APR at 95% usage,
		//then we allow rebalancing of the stable rate positions.

		uint256 currentLiquidityRate = vault.currentLiquidityRate;
		uint256 maxVariableCreditRate = IInterestRateModel(
			vault.interestRateModel
		).getMaxVariableCreditRate();

		if (
			usageRatio < REBALANCE_UP_USAGE_RATIO_THRESHOLD ||
			currentLiquidityRate >
			maxVariableCreditRate.percentMul(
				REBALANCE_UP_LIQUIDITY_RATE_THRESHOLD
			)
		) {
			revert InvalidConditionToRebalanceInterestRate();
		}
	}

	/**
	 * @dev Validates the action of setting an asset as collateral
	 * @param vault The state of the vault that the account is enabling or disabling as collateral
	 * @param vaultAddress The vault
	 * @param vaultsData The data of all the vaults
	 * @param accountConfig The state of the account for the specific vault
	 * @param vaults The list of active vaults
	 * @param oracle The price oracle
	 */
	function validateVaultAsCollateral(
		VaultModel.Data memory vault,
		address vaultAddress,
		bool useAsCollateral,
		function(address)
		...
		uint256 underlyingBalance = IERC20Upgradeable(vault.vaultToken)
			.balanceOf(msg.sender);

		if (underlyingBalance == 0) {
			revert UnderlyingBalanceIsZero();
		}
		...
		if (
			...
			revert VaultDepositAlreadyInUse();
		}
	}
	 ...
	 **/
	function validateMultiFlashloan(
	 ...
	 * @param principalVault The vault data of the principal
	 * @param accountConfig The account config
	 * @param accountHealthFactor The account's health factor
	 * @param accountStableDebt Total stable debt balance of the account
	 * @param accountVariableDebt Total variable debt balance of the account
	 **/
	function validateLiquidation(
		VaultModel.Data memory collateralVault,
		VaultModel.Data memory principalVault,
		AccountModel.Config memory accountConfig,
		uint256 accountHealthFactor,
		uint256 accountStableDebt,
		uint256 accountVariableDebt
	) internal pure {
		if (
			!collateralVault.config.getActive() ||
			!principalVault.config.getActive()
		) revert InactiveVault();

		if (
			accountHealthFactor >=
			AccountValidator.HEALTH_FACTOR_LIQUIDATION_THRESHOLD
		) revert HealtFactorNotBelowLiquidationThreshold();

		bool isCollateralEnabled = collateralVault
			.config
			.getLiquidationThreshold() >
			0 &&
			accountConfig.vaultIsCollateral(collateralVault.id);

		//if collateral asset isn't enabled as collateral by account, it cannot be liquidated
		if (!isCollateralEnabled) {
			revert CollateralCannotBeLiquidated();
		}

		if (accountStableDebt == 0 && accountVariableDebt == 0)
			revert SpecifiedAssetNotBorrowedByAccount();
	}

	/**
	 * @dev Validates vaultToken transfer
	 * @param from The account from which the vaultTokens are being transferred
	 * @param vaultsData The state of all the vaults
	 * @param accountConfig The state of the account for the specific vault
	 * @param vaults The list of active vaults
	 * @param oracle The price oracle
	...
	function validateTransfer(
		...
		(, , , , uint256 healthFactor) = AccountValidator.calculateAccountData(
			from,
			vaultsData,
			accountConfig,
			vaults,
		...
		if (healthFactor < AccountValidator.HEALTH_FACTOR_LIQUIDATION_THRESHOLD)
			revert UnallowedTransfer();
	}
}
...
interface ICDVault is IERC721Upgradeable, IERC721EnumerableUpgradeable {
	/* Asset Data structure */
	struct AssetData {
		address underlying; /* underlying asset */
		address vault; /* overlying vault */
		address owner; /* asset depositor */
		uint256 amount; /* saved amount */
		uint256 scaledAmount; /* scaled amount */
		uint256 index; /* liquidity index */
		uint256 term; /* term length(in seconds) */
		uint256 timestamp; /* deposit timestamp */
		uint256 id; /* asset/NFT ID */
	}
	...
	/// Action requires owner or approved account(usually to transfer or withdraw a CD)
	error NotOwnerOrApproved();

	/// Can't transfer CD while term is valid(within the lock-in period)
	error LockedIn();

	/// Invalid CD Term(How long to lock Vault NFT for) selected
	error InvalidCDTerm();
	 ...
	 * @param index The vault's new liquidity index
	 **/
	event Mint(address indexed to, uint256 tokenId, uint256 index);

	/**
	 * @dev Emitted after CDvault is burned
	 * @param from The owner of the CDVault getting burned
	 * @param to The account that will receive the underlying token
	 * @param tokenId The token ID burned
	 * @param index The vault's new liquidity index
	 **/
	event Burn(
		address indexed from,
	...
	function term(uint256 key) external view returns (uint32);

	function termList() external view returns (uint32[12] memory);
	...
	function getData(uint256 tokenId) external view returns (AssetData memory);

	function getUnderlyingAsset(uint256 tokenId)
		external
		view
		returns (address);

	function getPrincipalBalance(uint256 tokenId)
		external
		view
		returns (uint256);
	...
	function getScaledBalance(uint256 tokenId) external view returns (uint256);

	function getTerm(uint256 tokenId) external view returns (uint256);

	function getDepositTimeStamp(uint256 tokenId)
		external
		view
		returns (uint256);

	/**
	 * @dev Get Deposit(CD)'s Validity(expiry)
	 * @notice This is the timestamp when transfers or burn could ever be possible
	 **/
	function getDepositValidity(uint256 tokenId)
		external
		view
		returns (uint256);

	function getDepositIndex(uint256 tokenId) external view returns (uint256);
 ...
 * @dev Meant to be compatible with ERC3156
 * @dev Used the same bytes32 as ERC3156 since some limited tests showed returning bool was more expensive
 **/
...
/**
 * @title IMultiFlashLoanReceiver interface
 * @notice Interface for the IFlashLoanReceiver.
 * @author eaZI
 * @dev implement this interface to develop a flashloan-compatible flashLoanReceiver contract
 * @dev Meant to be compatible with ERC3156 were it meant for multi-assets
 * @dev Used the same bytes32 as ERC3156 since some limited tests showed returning bool was more expensive
 **/
