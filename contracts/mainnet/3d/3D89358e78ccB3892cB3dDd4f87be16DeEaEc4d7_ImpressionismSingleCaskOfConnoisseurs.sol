...
import "./interfaces/IImpressionismSingleCaskOfConnoisseurs.sol";
...
contract ImpressionismSingleCaskOfConnoisseursStorage {
    ...
    uint256 public maxTokensTierAmount;
    uint256 public maxTokensTierPerTx;
    uint256 public tokensPrice;

    uint256 public tokensAuctionSaleTimeStep;
    uint256 public tokensAuctionSaleStartPrice;
    uint256 public tokensAuctionSaleEndPrice;
    uint256 public tokensAuctionSalePriceStep;
    uint256 public tokensAuctionSaleMaxStepAmount;
...
contract ImpressionismSingleCaskOfConnoisseurs is 
    IImpressionismSingleCaskOfConnoisseurs, 
    ImpressionismSingleCaskOfConnoisseursStorage,
    Ownable,
    ReentrancyGuard,
    ERC721A,
    ERC721AQueryable,
        ...
        ERC721A(_tokenName, _tokenSymbol)
    {
        ...
        maxTokensAmount = _maxSupply;
        tokensPrice = _tokensPrice;
        
        treasury = _treaduryAddress;
        _setDefaultRoyalty(_treaduryAddress, _royaltyFee);
    }
    ...
    function mintGiveawayTokens(
        ...
        onlyAuthorized
    {
        if (totalSupply().add(quantities) > maxTokensAmount) {
            revert ExceedMaxTokens();
        }

        _safeMint(to, quantities);
        emit MintTokens(to, quantities, totalSupply());
    }

    /** @dev Mint designated amount of tokens to a list of address as owner
     * @param toList Address list to transfer the tokens
     * @param quantitiesList List of designated amount of tokens
     */
    function mintGiveawayTokensInBatch(
        address[] calldata toList, 
        uint256[] calldata quantitiesList
    )
        external
        override
        onlyAuthorized
    {
        if(toList.length != quantitiesList.length) {
            revert InvalidInput();
        }

        for(uint256 index = 0; index < toList.length; index++) {
            mintGiveawayTokens(toList[index], quantitiesList[index]);
        }
    }

    /** @dev Mint tokens as Whitelisted Address
     * @param quantities Amount of tokens the address wants to mint
     * @param maxQuantites Maximum amount of tokens the address can mint
        ...
        nonReentrant
    {
        ...
        if (totalSupply().add(quantities) > maxTokensTierAmount) {
            revert ExceedMaxTierTokens();
        }

        if (msg.value != quantities.mul(tokensPrice)) {
        ...
        emit MintTokens(msg.sender, quantities, totalSupply());
    }

    /** @dev Mint tokens during public sale
     * @param quantities Amount of tokens the address wants to mint
     */
    function mintPublicTokens(uint256 quantities)
        external
        payable
        override
        publicMintActive
        nonReentrant
    {
        ...
        if (quantities > maxTokensTierPerTx) {
    ...
    /** @dev Mint tokens during dutch auction sale
     * @param quantities Amount of tokens the address wants to mint
     */
    function mintDutchAuctionTokens(uint256 quantities)
        external 
        payable
        override
        ...
        returns (uint256 price)
    {
        if (block.timestamp < dutchAuctionMintStartTimestamp ||
            ...
            return tokensAuctionSaleStartPrice;
        } else {
            uint256 curStep = (block.timestamp.sub(dutchAuctionMintStartTimestamp)).div(tokensAuctionSaleTimeStep);

            return curStep < tokensAuctionSaleMaxStepAmount ?
                tokensAuctionSaleStartPrice.sub(curStep.mul(tokensAuctionSalePriceStep)) :
                tokensAuctionSaleEndPrice;
        }
    }
    ...
    {
        if (maxTokensTierAmount == 0 ||
            tokensAuctionSaleTimeStep == 0 ||
            tokensAuctionSaleStartPrice == 0 ||
            tokensAuctionSalePriceStep == 0 ||
            tokensAuctionSaleMaxStepAmount == 0)
        {
            revert SalesInfoUnset();
        }
        ...
        onlyOwner
    {
        ...
        tokensAuctionSaleTimeStep = newDutchAuctionTimestep;
        tokensAuctionSaleStartPrice = newDutchAuctionStartPrice;
        tokensAuctionSaleEndPrice = newDutchAuctionEndPrice;
        tokensAuctionSalePriceStep = newDutchAuctionPriceStep;
        tokensAuctionSaleMaxStepAmount = newDutchAuctionMaxStepAmount;
        ...
        onlyOwner
    {
        ...
        for(uint256 index = 0; index < listLength; index++) {
            ...
            emit AuthorizeAddressChange(authorizedAddressArray[index], newAuthorizedStatusArray[index]);
        }
    }
        ...
        onlyOwner
    {
        ...
        for(uint256 index = 0; index < listLength; index++) {
            ...
            emit SignerAddressChange(signerAddressArray[index], newSignerStatusArray[index]);
        }
    }
        ...
        onlyOwner
    {
        ...
        emit TokenStatusChange(tokenId, true);
    }
        ...
        onlyOwner
    {
        ...
        for(uint256 index = 0; index < listLength; index++) {
            ...
            emit TokenStatusChange(tokenIds[index], true);
        }
    }
        ...
        onlyOwner
    {
        isTokenInvalid.unset(tokenId);
         ...
         emit TokenStatusChange(tokenId, false);
    }
        ...
        onlyOwner
    {
        ...
        for(uint256 index = 0; index < listLength; index++) {
            isTokenInvalid.unset(tokenIds[index]);
             ...
             emit TokenStatusChange(tokenIds[index], false);
        }
    }
     ...
     * @param newBaseURI New URI that caller wants to set as the tokenURI
     */
    ...
    /** @dev Set the price of the tokens
     * @param newPrice New price of the tokens
     */ 
    function setTokensPrice(uint256 newPrice)
        external
        override
        onlyOwner
    {
        ...
        tokensPrice = newPrice;

        emit TokensPriceSet(newPrice);
    }

    /** @dev Set the maximum total amount of tokens
     * @param newMaxTokensAmount Maximum total amount of the tokens
     */
    function setMaxTokensAmount(uint256 newMaxTokensAmount)
        external
        override
        onlyOwner
    {
        if (newMaxTokensAmount == 0) { 
            revert InvalidInput();
        }

        if (newMaxTokensAmount < maxTokensTierAmount) {
            revert InvalidInput();
        }

        if (newMaxTokensAmount < totalSupply()) {
            revert InvalidInput();
        }

        maxTokensAmount = newMaxTokensAmount;

        emit TokensMaxAmountSet(newMaxTokensAmount);
    }

    /** @dev Set the maximum limit amount of each sales phase
     * @param newMaxTokensTierAmount Maximum limit amount of this sales phase
     */
    function setMaxTokensTierAmount(uint256 newMaxTokensTierAmount)
        external
        override
        onlyOwner
        notDuringSales
    {
        if (newMaxTokensTierAmount > maxTokensAmount) { 
            revert InvalidInput();
        }

        if (newMaxTokensTierAmount < totalSupply()) {
            revert InvalidInput();
        }

        maxTokensTierAmount = newMaxTokensTierAmount;

        emit TokensMaxTierAmountSet(newMaxTokensTierAmount);
    }

    /** @dev Set the maximum limit amount of a single transaction
     * @param newMaxTokensPerTx Maximum limit amount of a single transaction
     */
    function setMaxTokensPerTx(uint256 newMaxTokensPerTx)
        external
        override
        onlyOwner
    {
        ...
        if (newMaxTokensPerTx > maxTokensTierAmount) { 
            revert InvalidInput();
        }

        maxTokensTierPerTx = newMaxTokensPerTx;

        emit TokensMaxPerTxAmountSet(newMaxTokensPerTx);
    }
        ...
        onlyOwner
    {
        ...
        emit TreasurySet(newTreasuryAddress);
    }
        ...
        onlyOwner
    {
        ...
        emit FundWithdraw(amount, treasury);
    }
...
interface IImpressionismSingleCaskOfConnoisseurs {
  ...
  function mintGiveawayTokens(address to, uint256 quantities) external;
  // Mint Giveaway Tokens to a list of address by owner.
  function mintGiveawayTokensInBatch(address[] calldata toList, uint256[] calldata quantitiesList) external;
  // Whitelisted addresses mint specific amount of tokens with signature & maximum mintable amount to verify.
  function mintWhitelistTokens(uint256 quantities, uint256 maxQuantites, bytes calldata signature) external payable;
  // Public addresses mint specific amount of tokens in public sale.
  function mintPublicTokens(uint256 quantities) external payable;
  // Public addresses mint specific amount of tokens in dutch aution sale.
  function mintDutchAuctionTokens(uint256 quantities) external payable;
  ...
  // Get all the holders of the tokens.
  function getAllHolders() external view returns(address[] memory holdersList);
  // Get all the holders and their balance of the tokens.
  ...
  function setAuthorizedInBatch(address[] calldata authorizedAddressArray, bool[] calldata newAuthorizedStatusArray) external;
  // Set the address to generate and validate the signature in batch.
  function setSignerInBatch(address[] calldata signerAddressArray, bool[] calldata newSignerStatusArray) external;
  ...
  function setTokenInvalidInBatch(uint256[] calldata tokenIds) external;
  ...
  function setTokenValidInBatch(uint256[] calldata tokenIds) external;
  ...
  function setTokensPrice(uint256 newPrice) external;
  // Set the maximum total amount of token.
  function setMaxTokensAmount(uint256 newMaxTokensAmount) external;
  // Set the maximum limit amount of each sales phase.
  function setMaxTokensTierAmount(uint256 newMaxTokensTierAmount) external;
  ...
  // This event is triggered whenever a call to #mintGiveawayTokens, #mintWhitelistTokens, #mintPublicTokens, and #mintDutchAuctionTokens succeeds.
  event MintTokens(address owner, uint256 quantity, uint256 totalSupply);
  ...
  event TokenStatusChange(uint256 tokenId, bool tokenStatus);
  // This event is triggered whenever a call to #setBaseURI succeeds.
  event BaseURISet(string newBaseURI);
  // This event is triggered whenever a call to #setTokensPrice succeeds.
  event TokensPriceSet(uint256 newPrice);
  // This event is triggered whenever a call to #setMaxTokensAmount succeeds.
  event TokensMaxAmountSet(uint256 newMaxTokensAmount);
  // This event is triggered whenever a call to #setMaxTokensTierAmount succeeds.
  event TokensMaxTierAmountSet(uint256 newMaxTokensTierAmount);
  // This event is triggered whenever a call to #setMaxTokensPerTx succeeds.
  event TokensMaxPerTxAmountSet(uint256 newMaxTokensPerTx);
  // This event is triggered whenever a call to #setTreasury succeeds.
  event TreasurySet(address newTreasuryAddress);
  // This event is triggered whenever a call to #withdraw
  event FundWithdraw(uint256 amount, address treasuryAddress);
}
