...
abstract contract RewardsDistributionRecipient {
 ...
 // SPDX-License-Identifier: <SPDX-License>
contract SPYRITStakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    ...
    function stakeWithPermit(uint256 amount, uint deadline, uint8 v, bytes32 r, bytes32 s) external nonReentrant updateReward(msg.sender) {
        ...
        _totalSupply = _totalSupply.add(amount.mul(99).div(100));
    ...
    function stake(uint256 amount) external override nonReentrant updateReward(msg.sender) {
        require(amount > 0, "Cannot stake 0");
        uint256 contractBalance = stakingToken.balanceOf(address(this));
        stakingToken.transferFrom(msg.sender, address(this), amount.mul(99).div(100));
        stakingToken.transferFrom(msg.sender, devwallet, amount.mul(1).div(100));
        uint256 addedAmount = stakingToken.balanceOf(address(this)) - contractBalance;
    ...
    function getReward() public override nonReentrant updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            rewardsToken.safeTransfer(address(this), reward.mul(10).div(100));
            emit RewardPaid(msg.sender, reward);
        }
    }
    
    function setrewardsDistribution(address _rewardsDistributionaddress) public onlyOwner returns (address) {
        rewardsDistribution = _rewardsDistributionaddress;
        return rewardsDistribution;
    }

       function setdevwallet(address newdev) public virtual onlyOwner {
           devwallet=newdev;
