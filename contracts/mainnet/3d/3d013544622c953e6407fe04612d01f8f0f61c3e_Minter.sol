...
contract Minter is ERC721AQueryable {
    ...
    constructor(string memory _URI) ERC721A("Verified Devs","VD") {
        admin = msg.sender;
        URI = _URI;
    }
    ...
    function changeURI(string memory _URI) external onlyAdmin {
        URI = _URI;
    }

    function changeAdmin(address _new) external onlyAdmin {
        require(_new != address(0),"ERR:NA");//NA => Null Address
        admin = _new;
    }
    ...
    function _beforeTokenTransfers(
        ...
        require(from == address(0),"ERR:SB");//SB => Soul Bound
    }
