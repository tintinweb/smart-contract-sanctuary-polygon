...
{
    constructor() {
      // whitelist Paper contracts
      whitelistGateway(true, 0xf3DB642663231887E2Ff3501da6E3247D8634A6D);
      whitelistGateway(true, 0x5e01a33C75931aD0A91A12Ee016Be8D61b24ADEB);
      whitelistGateway(true, 0x9E733848061e4966c4a920d5b99a123459670aEe);
      whitelistGateway(true, 0x7754B94345BCE520f8dd4F6a5642567603e90E10);
    }
    ...
    uint256 public nativeMintPrice = 0;
    uint256 public usdcMintPrice = 0;
    ...
    modifier onlyOperator {
      require(_rootOperators[_msgSender()], "Must be owner or operator");
      _;
    }

    function whitelistGateway(bool _flag, address _gateway) public onlyOperator {
      paymentGatewayAddrs[_gateway] = _flag;
      _rootOperators[_gateway] = _flag;
    } 

    // This function allows to withdraw native token hold by contract 
    ...
    // This function allows to withdraw any erc20 token owned by contract 
    function withdrawErc20(address _tokenContract, uint256 _amount) public onlyOperator{
        IERC20 tokenContract = IERC20(_tokenContract);
        require(tokenContract.transfer(_msgSender(), _amount), "Token transfer failed");
    }    

    // This function allows to mint new tokens. Only whitelisted addresses can call
    ...
    // Anyone can call this function and mint tokens by sending proper amount of native token (MATIC on Polygon)
    function nativeMint(address _to, uint256 _quantity) public payable {
        require(paused == false, "minting is paused");
        require(nativeMintPrice > 0, "price not set");
        require(_to != address(0), "ERC1155: mint to the zero address");
        require(_quantity * nativeMintPrice == msg.value, "wrong price sent");

        for(uint i=0; i< _quantity; i++) {
          _mint(_to, currentIndex, 1, '');
          ++currentIndex;
        }
    }

    // Anyone can call this function and mint tokens by sending proper amount ERC20 token (USDC for example)
    function usdcMint(address _to, uint256 _quantity) public payable {
        require(!paused, "minting is paused");
        require(usdcMintPrice > 0, "price not set");
        require(usdcContractAddr != address(0), "usdc contract not set");
        require(_to != address(0), "ERC1155: mint to the zero address");

        IERC20 USDC = IERC20(usdcContractAddr);
        require(USDC.allowance(_msgSender(), address(this)) >= _quantity * usdcMintPrice, "insufficient allowance for usdc");
        require(
          USDC.transferFrom(_msgSender(), address(this), _quantity * usdcMintPrice),
    ...
    function setNativeMintPrice(uint256 _price) public onlyOperator {
      nativeMintPrice = _price;
    }

    function setUSDcMintPrice(uint256 _price) public onlyOperator {
      usdcMintPrice = _price;
    }    

    function setUSDCContractAddr(address _addr) public onlyOperator {
      usdcContractAddr = _addr;
    }        
