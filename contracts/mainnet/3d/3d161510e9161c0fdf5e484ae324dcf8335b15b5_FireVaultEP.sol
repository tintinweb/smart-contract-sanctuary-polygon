...
// File: contracts/FireVaultEP.sol
...
interface IEP is IERC20 {
    ...
    function claimableRewards(address account) external view returns (uint256 totalRewards, uint256 teamRewards, uint256 userRewards);
}

contract FireVaultEP is ERC20 {
    ...
    uint256 public constant MINT_RATE = 300;

    constructor() ERC20("FireVault EP", "fireEP") {
    }
    
    // Returns FBX reserve, from claimable rewards and balance
    function getReserveFBX() public view returns (uint256) {
        (,,uint256 rewards) = EP.claimableRewards(address(this));
        uint256 fee = rewards.mul(3).div(100);
        return rewards.sub(fee).add(FBX.balanceOf(address(this)));
    }

    // Returns EP reserve, from staking and balance
    function getReserveEP() public view returns (uint256) {
        (uint256 stakedEP,,) = EP.userInfos(address(this));
        return stakedEP.add(EP.balanceOf(address(this)));
    }

    // Returns how many EPs can be withdrawn per fireEP
    function withdrawalRate() public view returns (uint256) {
        if (totalSupply() == 0) {
            return 1e18;
        }
    ...
    function claimMintStake() public {
        ...
        if (rewards > 0) {
            // Fee: permanently locks 3% of the rewards as fireEP in the contract
            uint256 fee = rewards.mul(3).div(100);
            _mint(address(this), fee.div(MINT_RATE).mul(1e18).div(withdrawalRate()));
            // Mint new EP
            uint256 toSpend = FBX.balanceOf(address(this));
            if (toSpend >= MINT_RATE) {
