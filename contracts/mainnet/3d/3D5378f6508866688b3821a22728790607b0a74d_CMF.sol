...
contract CMF is AccessControlEnumerable,Pausable,ICMF {
    ...
    function register(address referrer) public {
        ...
        while(true){
            ...
            userInfoTeams[referrer].push(_msgSender());
        }
    ...
    function recharge(uint256 types,uint256 price,uint256 orderId) public{
        ...
        require(userReferrer[_msgSender()] == address(0) || _msgSender() == DEFAULT_REFERRER, "Referrer invalid");
        //验证充值金额
        rechargeDay(price);
        //充值记录 
    ...
    //验证充值金额
   function rechargeDay(uint256 price) private{
        ...
        if(day<Setting.startDay){
            ...
            uint256 price1 = rechargeCount[day - 1] / 1e8;
            uint256 price2 = Setting.dayPrice / 1e8;
            if(rechargeCount[day] == 0 && price1 >= price2){
    ...
    function payLevel() public{
        ...
        payUserInfo[_msgSender()].profit = 0;
    ...
    * 获取充值记录
    ...
    function getLevelInfo1(address owner) public view returns(uint256,uint256,uint256,uint256,uint256,uint256){
        ...
        uint256 zhiEnd = level == 6 ? 0: teamUpNum[level];
    ...
    function getLevelInfo2(address owner) public view returns(uint256,uint256,uint256,uint256){
        ...
        uint256 userPriceEnd = level == 6 ? 0 : userUpPrice[level];
        uint256 teamPrice = teamRechargePrice[owner];
        uint256 teamPriceEnd = level == 6 ? 0 : teamUpPrice[level];
    ...
    function _upgrade(address owner,uint256 price) private{
        ...
        while(true){
            for(uint256 i=0;i<teamUpNum.length;i++){
                ...
                if(level>1){
                    getUserTeamLevelNum(curAddress,i);
                }
                if((level == 1 && userZhiValid[curAddress]>=teamUpNum[i]) || (level>1 && userLevelNum[curAddress][i]>=teamUpNum[i])){
     ...
     * 获取直推有效人数
     */
    function getUserTeamLevelNum(address owner,uint256 level) public{
        uint256 count = 0;
        for(uint256 i=0;i<userTeams[owner].length;i++){
            address userAddr = userTeams[owner][i];
            if(userLevel[userAddr] >= level){
                count++;
            }else{
                for(uint256 j=0;j<userInfoTeams[owner].length;j++){
                    uint256 levelUser = userLevel[userInfoTeams[owner][j]];
                    if(levelUser>=level){
                        count++;
                        break;
                    }
        ...
        userLevelNum[owner][level] = count;
    }
    /**
     * 开始返佣
     */
    function _profit(address owner,uint256 price) private{
        ...
        while(true){
            ...
            if (userStatusValid[parentAddress] == 1) {
                ...
                if(userLevel[parentAddress]>0 && userLevel[parentAddress]<6){
...
abstract contract ICMF is CMFDATA {
    ...
    //三代以内
    mapping(address => address[]) public userInfoTeams;
    //推荐关系
    ...
    //团队5级极差返佣
    uint256[6] public teamRebate = [1000, 2000,3000,4000,5000,0];
    //团队升级人数
    uint256[6] public teamUpNum = [5,2,2,2,2,2];
     //个人升级金额
