...
contract House is ReentrancyGuard, Ownable {
     ...
     */
    function setLockedInRewards(uint256 _amount) external onlyOwner {
        lockedInRewards = _amount;
    }

    function setProjectRewards(uint256 _amount) external onlyOwner {
        projectRewards = _amount;
    }

    function setJackPot(uint256 _amount) external onlyOwner {
        jackpot = _amount;
    }

    function setNftHoldersRewardsToDistribute(uint256 _amount)
        external
        onlyOwner
    {
    ...
    address[] public lv1Addr;
    address[] public lv2Addr;
    address[] public lv3Addr;
    address[] public lv4Addr;
    ...
    function distributeNftHoldersRewards() external onlyOwner {
        ...
        delete lv3Addr;
        delete lv4Addr;
        address[] memory lv1SourceAddr = dicewarNft.getLvList(1);
        for (uint256 i = 0; i < lv1SourceAddr.length; i++) {
            if(lv1SourceAddr[i] != address(0)){
                lv1Addr.push(lv1SourceAddr[i]);
            }
        }
        address[] memory lv2SourceAddr = dicewarNft.getLvList(2);
        for (uint256 i = 0; i < lv2SourceAddr.length; i++) {
            if(lv2SourceAddr[i] != address(0)){
                lv2Addr.push(lv2SourceAddr[i]);
            }
        }
        address[] memory lv3SourceAddr = dicewarNft.getLvList(3);
        for (uint256 i = 0; i < lv3SourceAddr.length; i++) {
            if(lv3SourceAddr[i] != address(0)){
                lv3Addr.push(lv3SourceAddr[i]);
            }
        }
        lv4Addr = dicewarNft.getLvList(4);
