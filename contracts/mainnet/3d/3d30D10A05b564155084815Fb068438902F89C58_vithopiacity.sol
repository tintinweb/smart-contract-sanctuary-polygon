...
contract vithopiacity is ERC721  {
    ...
    constructor()
        ERC721("vi-thopiacity", "vithop")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 1150) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeigseucv24ff6kvqdeo2e2cyc5mekywr2ztz7aymjta3kfaobeqyla/", Strings.toString(tokenId), ".json")
            );
    }
