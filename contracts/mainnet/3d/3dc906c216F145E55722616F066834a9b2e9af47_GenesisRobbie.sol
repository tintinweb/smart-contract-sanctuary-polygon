...
contract GenesisRobbie is ERC721, ERC721Enumerable, Ownable, RoyaltiesV2Impl {
    ...
    uint256 public constant PRICE_PER_TOKEN = 1250000000; // USDC has 6 decimals
    ERC20 PAYMENT_TOKEN = ERC20(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174);

    mapping(address => uint8) private _allowList;

    address public artist;

    constructor(address artist_) ERC721("Robbie Genesis Collection", "ROBBIE") {
        artist = artist_;
    }
    ...
    function mintAllowList(uint8 numberOfTokens, string calldata email) external {
        ...
        require(PRICE_PER_TOKEN * numberOfTokens <= PAYMENT_TOKEN.balanceOf(msg.sender), "Caller has insufficient usdc token balance.");
        require(PRICE_PER_TOKEN * numberOfTokens <= PAYMENT_TOKEN.allowance(msg.sender, address(this)), "Contract has insufficient allowance to spend payment");

        PAYMENT_TOKEN.transferFrom(msg.sender, address(this), PRICE_PER_TOKEN * numberOfTokens);
    ...
    function mint(uint numberOfTokens, string calldata email) external {
        uint256 ts = totalSupply();
