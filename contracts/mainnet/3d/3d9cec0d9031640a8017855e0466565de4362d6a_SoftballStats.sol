...
contract SoftballStats is Ownable {
    ...
    function leaderByCategory() external view returns (string memory runsLeader, string memory doublesLeader, string memory triplesLeader, string memory homeRunsLeader, string memory rbisLeader) {
        ...
        for (uint i; i < totalPlayers;) {
            runsLeader = stats[player[i]].runs > runsLead ? player[i] : runsLeader;
            unchecked { 
                ++i; 
            }
        }
        ...
        uint256 doublesLead;
        for (uint j; j < totalPlayers;) {
            doublesLeader = stats[player[j]].doubles > doublesLead ? player[j] : doublesLeader;
            unchecked {
                ++j;
            }
        }
        ...
        uint256 triplesLead;
        for (uint k; k < totalPlayers;) {
            triplesLeader = stats[player[k]].triples > triplesLead ? player[k] : triplesLeader;
            unchecked {
                ++k;
            }
        }
        ...
        for (uint l; l < totalPlayers;) {
           homeRunsLeader = stats[player[l]].homeRuns > homeRunsLead ? player[l] : homeRunsLeader;
            unchecked {
                ++l;
            }
        }
        ...
        uint256 rbisLead;
        for (uint m; m < totalPlayers;) {
            rbisLeader = stats[player[m]].rbis > rbisLead ? player[m] : rbisLeader;
            unchecked {
                ++m;
            }
        }
