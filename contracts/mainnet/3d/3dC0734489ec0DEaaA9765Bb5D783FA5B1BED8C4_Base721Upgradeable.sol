    ...
    ERC721WithPermitUpgradable
{
    ...
    function initialize(
        ...
        __PoseType_init_unchained(typeInfos_);
        _idCounter = 1;
    }
    ...
    function buy(
        ...
        _processPayment(typeInfo.paymentToken, total, _msgSender(), referral_);
        _setSold(typeNFT_, quantity_);
        uint256 tokenId = _idCounter;
        emit Registered(recipient_, typeNFT_, tokenId, quantity_, _sold[typeNFT_], referral_);
    ...
    function batchMint(uint256 typeNFT_, uint256 quantity_, address recipient_) external onlyRole(MINTER_ROLE) {
        _setSold(typeNFT_, quantity_);
        uint256 tokenId = _idCounter;
        emit Registered(recipient_, typeNFT_, tokenId, quantity_, _sold[typeNFT_], address(0));
        _batchProcess(
            recipient_,
            tokenId,
            quantity_,
...
import { Helper } from "../libraries/Helper.sol";
import { FACTORY_ROLE } from "../libraries/Constants.sol";
...
abstract contract BusinessAddressesUpgradeable is BaseUpgradeable {
    ...
    function setBusinessAddress(address address_, bool status_) external onlyRole(FACTORY_ROLE) {
    ...
    FeeDistributorUpgradeable
{
    ...
    function _calculateAmounts(
        ...
        primarySaleAmount = (total_ * _clientInfo.percentageInBps) / HUNDER_PERCENT_IN_BPS;
        affiliateAmount = (total_ * affiliatePercentageInBps) / HUNDER_PERCENT_IN_BPS;
        systemAmount = value_ - primarySaleAmount - affiliateAmount;

        if (affiliate_ == address(0)) primarySaleAmount += affiliateAmount;
    }

    function _processPayment(address paymentToken_, uint256 total_, address payer_, address affiliate_) internal {
        uint256 discountTotal = total_;
        uint256 value;
        uint256 primarySaleAmount;
...
abstract contract PoseTypeUpgradeable is IPoseTypeUpgradeable, BaseUpgradeable {
    ...
    function __PoseType_init(TypeInfo[] calldata typeInfos_) internal onlyInitializing {
        __PoseType_init_unchained(typeInfos_);
    }

    function __PoseType_init_unchained(TypeInfo[] calldata typeInfos_) internal onlyInitializing {
        uint256 length = typeInfos_.length;
        for (uint256 i = 0; i < length; ) {
            _setType(i, typeInfos_[i]);
            unchecked {
                ++i;
            }
        }
