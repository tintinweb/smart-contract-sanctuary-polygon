...
contract BitchPolygon is ERC721  {
    ...
    constructor()
        ERC721("Bitch Polygon", "BTP")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 8777) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeigw3ysk6p5eudz4jdvhftrbd3e44lpndxssfmqgqmciwwvggdmdpe/", Strings.toString(tokenId), ".json")
            );
    }
