...
contract TransferAuthorizer is BaseACL {
    ...
    // External functions.
    function addTokenReceiver(TokenReceiver[] calldata tokenReceiver) external onlyOwner {
        for (uint i = 0; i < tokenReceiver.length; i++) {
            tokenSet.add(tokenReceiver[i].token);
            tokenToReceivers[tokenReceiver[i].token].add(tokenReceiver[i].receiver);

            emit tokenReceiverAdded(tokenReceiver[i].token, tokenReceiver[i].receiver);
        }
    }

    function removeTokenReceiver(TokenReceiver[] calldata tokenReceiver) external onlyOwner {
        for (uint i = 0; i < tokenReceiver.length; i++) {
            tokenToReceivers[tokenReceiver[i].token].remove(tokenReceiver[i].receiver);

            emit tokenReceiverRemoved(tokenReceiver[i].token, tokenReceiver[i].receiver);
        }
    }
    ...
    function transfer(address recipient, uint256 amount) external view {
        require(_txn.value == 0, "ETH transfer not allowed");
        _checkTransfer(_txn.to, recipient);
    }

    fallback() external override {
        require(_txn.data.length == 0, "TransferAuthorizer Method Not allowed");
...
/// @title BaseACL which uses the call-self trick to perform function and parameters check.
/// @author Cobo Safe Dev Team ([email protected])
/// @notice Can not be used directly by end user.
...
///        3. BaseACL may serve for multiple target contracts. Use `onlyContract` modifier or
///           check `_txn.to` with your own code in checking functions.
...
abstract contract BaseACL is BaseAuthorizer {
    ...
    using TransactionLib for TransactionData;
