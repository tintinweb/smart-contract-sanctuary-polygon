    ...
    Ownable
{
    ...
    constructor(
        ...
        collector1 = _salesInfo.collector1;
        collector2 = _salesInfo.collector2;
        collector1Ratio = _salesInfo.collector1Ratio;
    }
    ...
    function withdraw(uint256 _amount) external onlyOperator nonReentrant {
        ...
        uint256 amount1 = (amount * collector1Ratio) / ratioDenominator;
        uint256 amount2 = amount - amount1;
        if (address(payment) == address(0)) {
            (bool success, ) = payable(collector1).call{value: amount1}("");
            require(success);
            (success, ) = payable(collector2).call{value: amount2}("");
            require(success);
        } else {
            IERC20(payment).transfer(collector1, amount1);
            IERC20(payment).transfer(collector2, amount2);
        }
    }
