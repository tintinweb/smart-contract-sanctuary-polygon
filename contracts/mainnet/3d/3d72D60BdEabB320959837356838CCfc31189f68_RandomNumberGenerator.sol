...
// File contracts/staking/IRNG.sol
...
// File contracts/staking/RNG.sol
...
contract RandomNumberGenerator is VRFV2WrapperConsumerBase, ConfirmedOwner, IRNG {
    //mapping(uint256 => RequestStatus) public s_requests;
    mapping(uint => RequestStatus) public requestHistory;
    ...
    constructor(address linkAddress_, address wrapperAddress_) ConfirmedOwner(msg.sender) VRFV2WrapperConsumerBase(linkAddress_, wrapperAddress_) {
    ...
    function getRandomNumber(uint day) external view returns (uint256 randNumber) {
        require(requestHistory[day].fulfilled, "Not generated the number yet");

        return  requestHistory[day].randomWords[0];
    }

    // Will request random number, where 0 <= randomNumber < maxNumber
    function requestRandomWords(uint day, uint256 maxNumber) external returns (uint256 requestId) {
        require(requestHistory[day].paid == 0, "Today's request has been already paid.");
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        uint randomNumber = _randomWords[0] % requests[_requestId].max;

        requestHistory[requests[_requestId].day].fulfilled = true;
        requestHistory[requests[_requestId].day].randomWords[0] = randomNumber;
        requests[_requestId].fulfilled = true;
        requests[_requestId].randomWords[0] = randomNumber;
