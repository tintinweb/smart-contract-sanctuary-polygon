...
library MarketplaceLib {
    ...
    //     console.log("after decimals fetchhing");
    //     if (_inputCurrencyDecimals == _baseCurrencyDecimals)
    //         return _amountInBaseCurrency;
    //     console.log("after same check");

    //     if (_inputCurrencyDecimals > _baseCurrencyDecimals) {
    //         console.log("_inputCurrencyDecimals", _inputCurrencyDecimals);
    //         console.log("_baseCurrencyDecimals", _baseCurrencyDecimals);
    //         console.log(
    //             _inputCurrencyDecimals - _baseCurrencyDecimals,
    //             "__inputCurrencyDecimals - _baseCurrencyDecimals"
    //         );
    //              console.log(
    //             _amountInBaseCurrency ,
    //             "__inputCurrencyDecimals "
    //         );
    //         return
    //             _amountInBaseCurrency *
    //             (10 ** (_inputCurrencyDecimals - _baseCurrencyDecimals));
    //     }
    //     console.log("after the last else");
    //     return
    //         _amountInBaseCurrency /
    ...
    function _handleBuyProperty(
        ...
        //     console.log("inside the if after proeprty");
        //     _transferParams.quotePrice = _scaleAmount(
        //         _transferParams.from,
        //         _property.currency,
        //         _transferParams.quotePrice
        //     );
        //     console.log("after scale amount");
        // }
        ...
        if (isFeeInXEQ) {
            ...
            console.log("inside the else of STF", _transferParams.quotePrice);
            console.log(
                "inside the else of STF",
                _transferParams.quotePrice + buyFeeAmount
            );
            console.log("inside the else of STF", buyFeeAmount);
            IERC20(_transferParams.from).safeTransferFrom(
                sender,
                address(this),
                _transferParams.quotePrice + buyFeeAmount
    ...
    ReentrancyGuardUpgradeable
{
    ...
    ) external onlyMaintainer nonReentrant returns (address WLegalShares) {
    ...
    function _swap(
        ...
        // uint8 baseCurrencyDecimals = IERC20Metadata(_property.currency)
        //     .decimals();
        // uint8 askCurrencyDecimals = IERC20Metadata(_from).decimals();
        if (
            _property.priceFeed == _currencyToFeed &&
            IERC20Metadata(_from).decimals() ==
            IERC20Metadata(_property.currency).decimals()
        ) {
            ...
            uint256 quotePrice = propertyQuotePrice(
                QuotePriceParams(
                    _amountOfShares,
                    _property.currency,
                    _from,
    ...
    function propertyQuotePrice(
        IMarketplace.QuotePriceParams memory _quoteParams
    ) public view returns (uint256 quotePrice) {
        return MarketplaceLib._propertyQuotePrice(_quoteParams);
    }
    ...
    {
        return storageParams._getPropertyPrice(_from, _to);
    }
