...
import "../includes/interfaces/IOblivionNft.sol";
...
contract OblivionMintingService is Ownable, ReentrancyGuard {
    ...
    function setUsesReviveRug(uint256 _id, bool _usesReviveRug) public onlyOwner() {
        listings[_id].usesReviveRug = _usesReviveRug;
    }
    ...
    function recoverNftOwnership(uint256 _id) public {
        ...
        IOblivionNft nft = IOblivionNft(listing.nft);
    ...
    function mintBnb(uint256 _id) public payable nonReentrant() {
        ...
        uint256 tokenId;
        if (!listing.usesReviveRug) tokenId = IOblivionNft(listing.nft).mint(msg.sender);
        else tokenId = IRugZombieNft(listing.nft).reviveRug(msg.sender);
    ...
    function mintMultiBnb(uint256 _id, uint256 _quantity) public payable nonReentrant() {
        ...
        for (uint256 i = 0; i < _quantity; i++){
            if (!listing.usesReviveRug) finalTokenId = IOblivionNft(listing.nft).mint(msg.sender);
            else finalTokenId = IRugZombieNft(listing.nft).reviveRug(msg.sender);
        }
