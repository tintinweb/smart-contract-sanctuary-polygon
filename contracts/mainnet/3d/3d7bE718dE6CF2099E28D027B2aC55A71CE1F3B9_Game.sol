...
contract Game is IGame, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable{
    ...
    modifier eventValidateById(uint256 eventId_) {
        require(eventId_ < freeEventInfos.length, "not exist");
        _;
    }

    modifier whitelistValidateById(uint256 eventFreeId_) {
        require(eventFreeId_ < countEventFree, "not exist");
        _;
    }
    ...
    function canShoot(address account) public override view returns (bool, uint256) {
        ...
        bool condition = !requestIdUsed[request.requestId] && request.requestId != 0;
        if (!condition) {
            return (false, 0);
        } else {
    ...
    function deleteTypeMissile(uint256 typeMissile) public onlyOwner {
        require(typeMissile <= countTypeMissile && typeMissile != 0, "invalid type");
    ...
    function arrangeMissile(uint256[12] memory positions) external payable override nonZero(positions) nonReentrant whenNotPaused {
        (uint256 price, uint256[] memory events, UseCache[] memory useCache, uint256 typeAff, uint256 amountAff, uint256 amountToMinimum) = calPrice(positions, msg.sender);
        ...
        require(amountToMinimum == 0, "less than minimum");
    ...
    function shoot() external override nonReentrant whenNotPaused {
        ...
        playNumbers += 1;
        SafeToken.safeTransferETH(msg.sender, finalReward);
    ...
    function calPrice(
        ...
        address account
    ) public override view returns (uint256 price, uint256[] memory, UseCache[] memory, uint256 typeAff, uint256 amountAff, uint256) {
        uint256 value;
        ...
        if (price >= minimumAmount) {
            return (price, events, useCache, typeAff, amountAff, 0);
        }    
        return (price, events, useCache, typeAff, amountAff, minimumAmount - price);
    }
    ...
    function setAddress(address treasury_, address operator_) external onlyOwner {
        treasury = treasury_;
        operator = operator_;
    }
    ...
    function setRate(uint256 newGoldRate, uint256 newGhostRate) external onlyOwner {
        require(newGoldRate < 1000000 && newGoldRate != 0 && newGhostRate < 1000000 && newGhostRate != 0, "invalid");
    ...
    function addToBlacklist(address account_, uint256 limitSpend_, uint256 limitNumber_) external onlyOwner {
        require(blacklistIds[account_] == 0 && account_ != address(0), "existed");
    ...
    function removeFromBlacklist(address account) external onlyOwner {
        ...
        require(id != 0 && account != address(0), "not blacklist");
    ...
    function removeFromWhitelist(address account) external onlyOwner {
        ...
        require(id != 0 && account != address(0), "not whitelist");
    ...
    function addToWhitelistFree(address[] memory accounts_, uint256 freeEventId_) external whitelistValidateById(freeEventId_) onlyOwner {
        for(uint256 i = 0; i < accounts_.length; i++) {
            address account_ = accounts_[i];
            if (whitelistFreeIds[account_][freeEventId_] == 0 && account_ != address(0)) {
    ...
    function removeFromWhitelistFree(address account_, uint256 freeEventId_) external whitelistValidateById(freeEventId_) onlyOwner {
        ...
        require(id != 0 && account_ != address(0), "not whitelist");
    ...
    function startEventFree(
        ...
        require(object_ < 5 && startTime_ < endTime_ && typeMissile_ != 0 && typeMissile_ <= countTypeMissile, "invalid");
    ...
    function startEventReward(uint256 startTime_, uint256 endTime_, uint256 object_, uint256 multiplier_) external onlyOwner {        
        require(startTime_ < endTime_ && object_ < 5, "invalid");
...
pragma solidity ^0.8.0; 
 ...
 interface IGame {
    ...
    function calPrice(uint256[12] memory positions, address account) external view returns (uint256, uint256[] memory, UseCache[] memory, uint256 typeAff, uint256 amountAff, uint256);
