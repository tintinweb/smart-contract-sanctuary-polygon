...
contract TransferResolver is Helpers {
    ...
    function deposit(
        ...
        if (erc20 != getAddressETH()) {
            IERC20(erc20).safeTransferFrom(msg.sender, address(this), realAmt);
        }

        emit LogDeposit(erc20, realAmt);
    }
    ...
    function withdraw(address erc20, uint256 amount) external {
        require(!IRegistry(getRegistryAddr()).notAllowed(erc20), "Token withdraw not allowed");
    ...
    function withdrawTo(
        ...
        emit LogWithdraw(erc20, realAmt);
    }
