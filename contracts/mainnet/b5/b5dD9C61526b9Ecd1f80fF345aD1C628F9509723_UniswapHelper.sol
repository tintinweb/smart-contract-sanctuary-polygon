...
contract UniswapHelper is IUniswapV3MintCallback {
    ...
    uint256 internal factor = 100; // eta / 500
    ...
    function _calcLimitOrderRebalance(uint256 _eta) internal view returns (bool) {
        ...
        uint256 token_0_in_1 = token0To1(limitOrder.token0.add(strategy.balanceOfWeth()), true);
        uint256 token_0 = limitOrder.token1.add(token_0_in_1).add(strategy.balanceOfWant());
    ...
    function estimateLimitOrderFees() public view returns (uint256) {
        (, int24 tick, , , , , ) = uniV3Pool.slot0();
        (uint256 token0, uint256 token1) = _estimateFees(limitTicks, tick);
        ...
        returns (uint256 token0Balance, uint256 token1Balance)
    {
        ...
        uint256 limitOrderSize = token1Balance.add(token0To1(token0Balance, false));
        uint16 divideBy = 1;
