...
contract AggregatorSwapBaseImpl is SwapBase, ReentrancyGuard {
    address payable public aggregatorRouter;
    address private constant NATIVE_TOKEN_ADDRESS =
        address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE);

    constructor(address _aggregatorRouter, address _router) SwapBase(_router) {
        aggregatorRouter = payable(_aggregatorRouter);
    }

    event UpdateAggregatorRouter(address indexed aggregatorRouter);
    event Swap(uint256 amount, address fromToken, address toAddress, uint256 toAmount, address toToken, string channel, uint256 channelFee);

    function setAggregatorRouter(address _aggregatorRouter) external onlyOwner {
        aggregatorRouter = payable(_aggregatorRouter);
        emit UpdateAggregatorRouter(_aggregatorRouter);
    }
    ...
    function swap(
        ...
        address _feeAddress
    ) external payable override onlyRouter nonReentrant returns (uint256) {
        require(_fromToken != address(0), Errors.ADDRESS_0_PROVIDED);
        require(_amount != 0, Errors.INVALID_AMT);

        (address toTokenAddress, bytes memory swapCallData, string memory _channel, uint256 _channelFee) = abi
            .decode(_extraData, (address, bytes, string, uint256));
        ...
        if (_fromToken != NATIVE_TOKEN_ADDRESS) {
            ...
            TransferHelper.safeApprove(_fromToken, aggregatorRouter, _amount);
            (bool success, ) = aggregatorRouter.call(swapCallData);
            ...
            (bool success, ) = aggregatorRouter.call{value: _amount}(swapCallData);
            require(success, Errors.SWAP_FAILED);
        }
        ...
        if (returnAmount > 0) {
            if (toTokenAddress == NATIVE_TOKEN_ADDRESS) {
                payable(_receiverAddress).transfer(returnAmount);
            } else {
