...
interface IGauge {
    ...
    function depositFor(uint256 amount, uint256 tokenId, address _for) external;
...
import {TokenAccessControlEnumerableUpgradeable} from "./TokenAccessControlEnumerableUpgradeable.sol";

contract MintableBurnableSyntheticTokenPermitUpgradeable is
    Initializable,
    ERC20Upgradeable,
    ERC20BurnableUpgradeable,
    TokenAccessControlEnumerableUpgradeable,
...
/**
 * @dev Extension of {AccessControlEnumerable} that offer support for synth token roles.
 */
contract TokenAccessControlEnumerableUpgradeable is
    AccessControlEnumerableUpgradeable
{
    ...
    uint256[50] private __gap_tokenAccessControl;

    modifier onlyMinter() {
        _require(hasRole(MINTER_ROLE, msg.sender), Errors.CALLER_NOT_MINTER);
        _;
    }

    modifier onlyBurner() {
        _require(hasRole(BURNER_ROLE, msg.sender), Errors.CALLER_NOT_BURNER);
        _;
    }
    ...
    function _setTokenAdmin(address _account) internal {
        _setRoleAdmin(DEFAULT_ADMIN_ROLE, DEFAULT_ADMIN_ROLE);
        _setupRole(DEFAULT_ADMIN_ROLE, _account);
    }

    function _setMinter(address _account) internal {
        _setRoleAdmin(MINTER_ROLE, DEFAULT_ADMIN_ROLE);
        _setupRole(MINTER_ROLE, _account);
    }

    function _setBurner(address _account) internal {
...
import {BaseERC4626, MintableBurnableSyntheticTokenPermitUpgradeable, SafeERC20Upgradeable, FinderLib, IFinder} from "./utils/BaseERC4626.sol";
import {IGauge} from "./../Interfaces/IGauge.sol";
import {IRentShare} from "./../Interfaces/IRentShare.sol";
import {IMarketplaceMeta} from "./../Interfaces/IMarketplaceMeta.sol";
import {CTokenManager} from "./utils/CTokenManager.sol";
...
/// Takes erc20 token (xJTRY) as underlying token and mints bb-tokens(bb-xJTRY) . xJTRY and bb-xJTRY varies according to supply in this contract
contract ERC4626Vault is BaseERC4626 {
    /// -----------------------------------------------------------------------
    /// Library usage
    /// -----------------------------------------------------------------------
    using SafeERC20Upgradeable for MintableBurnableSyntheticTokenPermitUpgradeable;
    using FinderLib for IFinder;

    /// @notice Representation of 100%. Equals t0 10000 that means 100 = 1%
    uint256 public PERCENTAGE_BASED_POINT; // 100 %

    /// -----------------------------------------------------------------------
    /// Events
    /// -----------------------------------------------------------------------

    event RentAsRewardNotified(address propertyAddress, uint rentAmount);
    event GaugeUpdated(address gaugeAddr);
    event SupplyIncreased(uint amount);
    event SupplyDecreased(uint amount);
    event RewardNotified(uint rewardToTransfer);
    event TokensLentToMP(address sender, uint amount);
    event PropertySoldToMP(
        address propertyToken,
        uint amountOfTokens,
        uint repayAmount
    );

    event MarketplaceBorrowerAllowed(address addr);
    event MarketplaceBorrowerRemoved(address addr);
    event FeesUpdated(uint newFees);

    /// -----------------------------------------------------------------------
    /// Immutable parameters
    /// -----------------------------------------------------------------------

    /// @notice erc20 token that will be staked as underlying token, (x-Jtry in our case)
    address public stakeToken;

    /// @notice allowed MarketplaceBorrowers to borrow tokens from this pool
    mapping(address => bool) public allowedMarketPlaceBorrowers;

    /// @notice Gauge from where the rewards should be claimed after depositing "this(bb-xJTRY)" token in Gauge
    address public gaugeAddress;

    /// @notice supply of stakeToken in "this"
    uint256 public assetTotalSupply;

    /// @notice percentage of fees to be deducted when MarketplaceBorrower is borrowing
    uint256 public fees; // 375 = 3.75%

    /// @notice to ensure the contract is only initialized once
     ...
     * @param _name name of "this" contract, BB-JTRY in our case
     * @param _initialOwner maintainer of created token (BB-JTRY);
     * @param _stakeToken erc20token(x-JTRY) to be used as underlying token
     * @param _finder address of finder.sol
     */
    function initialize(
        string memory _name,
        ...
        _require(!isInitialized, Errors.ALREADY_INITIALIZED);
        _initializeBase4626(
            MintableBurnableSyntheticTokenPermitUpgradeable(_stakeToken),
            _name,
            _name,
            _initialOwner,
            _finder
        );
        _require(_initialOwner != address(0x00), Errors.ZERO_ADDRESS);
        stakeToken = _stakeToken;
        PERCENTAGE_BASED_POINT = 10000;
        fees = 375;
        address manager = address(IFinder(_finder).getManager());
        _setTokenAdmin(manager);
        isInitialized = true;
    }

    // modifier

    /// @notice to ensure caller is allowed MarketplaceBorrower
    modifier onlyAllowedMarketplaceBorrower() {
        if (!allowedMarketPlaceBorrowers[msg.sender]) {
            _revert(Errors.CALLER_NOT_MARKETPLACE_BORROWER);
        }
        _;
    }
     ...
     * @param sender reciever of the BB-JTRY
     * @return shares amount of BB-JTRY received
     */
    function stake(
        uint256 assets,
        address sender
    ...
    /**
     * @notice to deposit BB-JTRY and withdraw x-JTRY
     * @param assets amount of x-JTRY to withdraw
     * @return shares amount of BB-JTRY transferFrom user
     */
    ...
    /**
     * @notice to deposit all BB-JTRY and get X-JTRY against it
     * @return assets amount of BB-JTRY transferFrom user
     */
    function exit() external returns (uint256 assets) {
        assets = withdraw(balanceOf(msg.sender), msg.sender, msg.sender);
    }

    /**
     * @param assets number of underlying token to deposit
     * @param receiver reciever of the BB-JTRY
     * @return amount of BB-JTRY received
     */
    function deposit(
        uint256 assets,
        address receiver
    ...
    /**
     * @param shares amount of BB-JTRY wants to mint
     * @param receiver reciever of the BB-JTRY
     * @return amount X-JTRY deposited
     */
    function mint(
        uint256 shares,
        address receiver
     ...
     * @notice to deposit BB-JTRY and withdraw x-JTRY
     * @param assets amount of x-JTRY to withdraw
     * @param receiver receiver of x-JTRY
     * @return amount of BB-JTRY transferFrom user
     */
    function withdraw(
        uint256 assets,
        address receiver,
        ...
        uint shares = super.withdraw(assets, receiver, owner_);
        assetTotalSupply -= assets;
        return shares;
    }

    /**
     * @param shares number of BB-JTRY token to deposit
     * @param receiver reciever of the X-JTRY
     * @param owner_ owner of the BB-JTRY tokens
     * @return amount of X-JTRY transferred to receiver
     */
    function redeem(
        uint256 shares,
        address receiver,
        ...
        uint assets = super.redeem(shares, receiver, owner_);
        assetTotalSupply -= assets;
        return assets;
    }
    ...
    /**
     * @return amount of X-JTRY in this contract (not includes tokens direclty transferred to this contract)
     */
     ...
     * @return actual amount transferred to Marketplace after deducting fees
     */
    function borrow(
        uint256 _amount
    ) external onlyAllowedMarketplaceBorrower returns (uint256) {
        // deducting fees
        uint256 _fee = (_amount * fees) / PERCENTAGE_BASED_POINT;
        uint actualBorrowAmount = _amount - _fee;

        asset.safeTransfer(msg.sender, actualBorrowAmount);

        emit TokensLentToMP(msg.sender, _amount);
        return actualBorrowAmount;
    }

    /**
     * @notice Transfers property token to Marketplace and receives amount lent to MarketplaceBorrower
     * @param _propertyToken address of WLegal Token
     * @param _amountOfTokens amount ot tokens MarketplaceBorrower wants to buy for Marketplace.sol
     * @param _marketPlace address of Marketplace
     * @param _repayAmount amount repaid by Marketplace to buy back the property tokens
     * @param _propertyBasecurrency base Currency of the property
     */
    function buyPropertyTokens(
        address _propertyToken,
        uint256 _amountOfTokens,
        ...
        _notifyRentAsRewardToGauge(_propertyToken);
        MintableBurnableSyntheticTokenPermitUpgradeable(_propertyToken)
            .safeTransfer(_marketPlace, _amountOfTokens);
        MintableBurnableSyntheticTokenPermitUpgradeable(_propertyBasecurrency)
            .safeIncreaseAllowance(msg.sender, _repayAmount * 2);

        emit PropertySoldToMP(_propertyToken, _amountOfTokens, _repayAmount);
    }

    /**
     * @notice sets an address to be an allowed Marketplace Borrower
     * @param _addr address of the MarketplaceBorrower to allow to
     */
    function setAllowedMarketPlaceBorrower(
        address _addr
    ) external onlyMaintainer {
        _require(_addr != address(0x00), Errors.ZERO_ADDRESS);
        allowedMarketPlaceBorrowers[_addr] = true;
        emit MarketplaceBorrowerAllowed(_addr);
    }

    /**
     * @notice removes an address from allowed Marketplace Borrowers
     * @param _addr address of the MarketplaceBorrower to remove from allow list
     */
    function removeMarketplaceBorrower(address _addr) external onlyMaintainer {
        allowedMarketPlaceBorrowers[_addr] = false;
        emit MarketplaceBorrowerRemoved(_addr);
    }

    /**
     * @notice Updates fees
     * @param _newFees percentage of fee ; 100 is 1%
     */
    function updateFees(uint256 _newFees) external onlyMaintainer {
        // fees can't be more than 10%
        if (_newFees > 1000) {
            _revert(Errors.INVALID_FEE_PERCENTAGE);
        }
        fees = _newFees;
    ...
    /**
     * @notice for allowed Marketplace Borrower that notifies the reward to the Gauge
     * @notice _rewardAmount amount to sent as reward
     * @notice actual amount sent as reward after settling loss if any
     */
    ...
    function notiftyRewardToGauge(
        ...
        if (_rewardToTransfer > 0) {
            asset.safeIncreaseAllowance(gaugeAddress, _rewardToTransfer);
            IGauge(gaugeAddress).notifyRewardAmount(
                address(asset),
                _rewardToTransfer
            );
        }
        emit RewardNotified(_rewardToTransfer);
        return _rewardToTransfer;
    }

    /**
     * @notice claims rent before transfering property to Marketplace and sends claimed rent to gauge
     * @param _propertyAddress address of  WLegal token
     */
    function _notifyRentAsRewardToGauge(address _propertyAddress) internal {
        address propertyRentToken = finder.getPropertyRentToken();
        address rentShare = finder.getRentShareAddress();
        string
            memory propertySymbol = MintableBurnableSyntheticTokenPermitUpgradeable(
                _propertyAddress
            ).symbol();
        uint rentAmount = IRentShare(rentShare).harvestRewardsForVault(
            propertySymbol
        );
        if (rentAmount > 0) {
            MintableBurnableSyntheticTokenPermitUpgradeable(propertyRentToken)
                .safeApprove(gaugeAddress, rentAmount);
            ...
            emit RentAsRewardNotified(_propertyAddress, rentAmount);
        }
    }

    /**
     * @notice settles loss if pool is in loss before notifying reward
     * @param _rewardAmount amount of reward
     */
    function _settleLossIfAny(
        ...
        if (assetSupply >= totalSupply()) {
            ...
            uint amount = _rewardAmount >= difference
                ? _rewardAmount - difference
                : 0;
            return amount;
        }
    }
    ...
    /**
     * @notice updated address of gauge
     * @param _gaugeAddr address of the gauge
     */
    function setGauge(address _gaugeAddr) external onlyMaintainer {
        gaugeAddress = _gaugeAddr;

        emit GaugeUpdated(_gaugeAddr);
    }
    ...
    /**
     * @notice for MarketplaceBorrower to sent remaining amount after filling all positions to fees receiver
     * @param amount number of tokens recieved by MarketplaceBorrower
    ...
    function rescueToken(
        ...
        MintableBurnableSyntheticTokenPermitUpgradeable(_tokenAddress)
...
// import {MintableBurnableSyntheticTokenPermitUpgradeable} from "./../../SyntheticToken/MintableBurnableSyntheticTokenPermit.sol";
import {MintableBurnableSyntheticTokenPermitUpgradeable} from "./../../SyntheticTokenUpgradeable/MintableBurnableSyntheticTokenPermitUpgradeable.sol";
...
/// forked from (https://github.com/transmissions11/solmate/blob/main/src/mixins/ERC4626.sol)
abstract contract BaseERC4626 is
    MintableBurnableSyntheticTokenPermitUpgradeable
{
    using SafeERC20Upgradeable for MintableBurnableSyntheticTokenPermitUpgradeable;
    ...
    /// @notice address of underlying erc20 token
    MintableBurnableSyntheticTokenPermitUpgradeable public asset;

    /**
     * @notice Initializes the contract.
     * @param _asset address of underlying ERC20 token
     * @param _name name of "this" contract
     * @param _symbol symbol of "this" contract
     * @param _initialOwner maintainer of created token / x-token
     * @param _finder address of finder.sol
     */
    function _initializeBase4626(
        MintableBurnableSyntheticTokenPermitUpgradeable _asset,
        string memory _name,
        string memory _symbol,
        address _initialOwner,
        address _finder
    ) internal {
        initializeB(_name, _symbol, _asset.decimals());
        asset = _asset;
        finder = IFinder(_finder);
        _setRoleAdmin(MAINTAINER_ROLE, DEFAULT_ADMIN_ROLE);
        _setupRole(MAINTAINER_ROLE, _initialOwner);
        _setRoleAdmin(MARKETPLACEBORROWER_ROLE, DEFAULT_ADMIN_ROLE);
    }
    ...
    /// @notice Role required by MarketplaceBorrower to borrow and repay Tokens against underlying
    bytes32 public constant MARKETPLACEBORROWER_ROLE =
        keccak256("MarketplaceBorrower");
    ...
    /// @notice to ensure caller is allowed MarketplaceBorrower
    modifier onlyMarketplaceBorrowerRole() {
        _require(
            hasRole(MARKETPLACEBORROWER_ROLE, msg.sender),
            Errors.CALLER_NOT_MARKETPLACE_BORROWER
        );
        _;
    }
    ...
    /**
     * @notice transferFrom underlying tokens from caller and mints share token to the receiver
     * @param assets number of underlying user wants to deposit
     * @param receiver reciever of share token
     * @return amount of shares minted to the reciever
     */
    function deposit(
        uint256 assets,
        address receiver
        ...
        _require((shares = previewDeposit(assets)) != 0, Errors.ZERO_SHARES);
    ...
    /**
     * @notice helper to mint shares to the receiver
     * @param shares amount of shares to be minted to _to
     * @param receiver address of shares receiver
     */
    function mint(
        uint256 shares,
        address receiver
    ) public virtual returns (uint256) {
        uint assets = previewMint(shares); // No need to check for rounding error, previewMint rounds up.
    ...
    /**
     * @param assets amount of underlying token to be withdrawed
     * @param receiver address to receive underlying erc20 tokens
     * @param owner owner of shares
     * @return amount of underlying erc20 token withdrew
     */
    function withdraw(
        uint256 assets,
        address receiver,
        ...
        uint shares = previewWithdraw(assets); // No need to check for rounding error, previewWithdraw rounds up.
    ...
    /**
     * @param shares number of shares to deposit
     * @param receiver reciever of the underlying erc20
     * @param owner owner of the shares tokens
     * @return amount of underlying erc20 tokens transferred to receiver
     */
    function redeem(
        uint256 shares,
        address receiver,
        ...
        _require((assets = previewRedeem(shares)) != 0, Errors.ZERO_ASSETS);
    ...
    function _handleCTokensMinting(
        ...
        if (cTokensReceiver == address(0x00)) {
            ...
            if (addressToCTokenPercentage[cTokensReceiver] != 0) {
                ...
                _revert(Errors.ADDRESS_NOT_REGISTERED);
            }
        }
    }
...
contract DepositManager {
    ...
    event UserDepositRegistered(
        address sender,
        address cTokensReceiver,
        uint assets
    );

    event ControllerBorrowedFund(address controller, uint amount);
    event ControllerRepayedFund(address controller, uint amount);
    ...
    modifier onlyVaultCaller() {
        _require(msg.sender == vaultCaller, Errors.CALLER_NOT_VAULT);
        _;
    }
    ...
    function registerUserDeposits(
        ...
        emit UserDepositRegistered(sender, cTokensReceiver, assets);
    }
    ...
    function borrowFund(address _controller, uint256 _amount) external {
        ...
        emit ControllerBorrowedFund(_controller, _amount);
    }

    function repayFund(address _controller, uint256 _amount) external {
        controllerUtilization[_controller] -= _amount;
        emit ControllerRepayedFund(_controller, _amount);
    }

    function toWithdrawFromController(
        address controller,
        address user
    ) public view returns (uint256) {
    ...
    function withdraw(
        ...
        for (uint256 i; i < controllers.length; i++) {
            ...
            (, bool flag) = toWithdrawFromReserves(
                controllers[i],
                sender,
                amountToWithdraw
            );
