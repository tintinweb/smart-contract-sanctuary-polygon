...
contract GnosisGuard is BaseGuard, Ownable {
    uint256[] public nonces;
    mapping(bytes32 => bool) public transactionHashes;

    function vetoTransaction(bytes32 transactionHash, uint256 nonce) public onlyOwner() {
    ...
    function checkTransaction(
      ...
      for (uint256 i = 0; i < nonces.length; i++) {
        bytes32 transactionHash = IZodiacReality(_msgSender()).getTransactionHash(to, value, data, operation, nonces[i]);
        require(!transactionHashes[transactionHash], "GnosisGuard: transaction has been vetoed");
      }
    }
