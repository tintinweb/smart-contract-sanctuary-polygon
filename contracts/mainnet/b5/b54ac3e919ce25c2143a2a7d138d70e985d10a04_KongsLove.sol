...
contract KongsLove is ERC721 {
    uint256 public totalSupply;
    uint256 public cost = 0.12 ether;
    uint256 public maxMints = 117;
    ...
    constructor()
        ERC721("Kong's Love", "KL")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 117) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeiap4wpbps3cor53ikjumbjsiwvjgwwlirdte6ozfvz33bjkxdb6hq/", Strings.toString(tokenId), ".json")
            );
    }
