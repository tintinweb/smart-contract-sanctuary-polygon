...
contract HILO is ERC1155Supply, Ownable, Pausable, ReentrancyGuard {
    ...
    function getHiBalance(address player) public view returns (uint256) {
        return balanceOf(player, HI);
    }

    function getLoBalance(address player) public view returns (uint256) {
        return balanceOf(player, LO);
    }

    function getApprovedSpend(address player) public view returns (uint256) {
        return usdc.allowance(player, address(this));
    }

    function getHiTotal() public view returns (uint256) {
        return totalSupply(HI);
    }

    function getLoTotal() public view returns (uint256) {
        return totalSupply(LO);
    }
