    ...
    CredentialVerifier
{
    ...
    // a fixer function to change the start token id for the collection
    // requires that there are no tokens minted
    // TODO: remove in next release as _startTokenId() returning 1 fixes this
    function overwriteStartTokenId() external onlyOperator {
        require(
            !tokenExists(0),
            "PhygitalItems:overwriteStartTokenId:: Cannot overwrite start token id when there are tokens minted."
        );
    ...
    function mintAndHold(uint256 quantity) external override onlyOperator {
        ...
        for (
            ...
            emit StateChanged(tokenId, ItemStatus.REDEEMABLE, ItemStatus.REDEEMED);
        }
    }
