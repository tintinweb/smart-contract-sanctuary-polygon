...
contract StrategyRebalanceStakerAlgebra is Initializable, OwnableUpgradeable {
  ...
  function claimFromNftManager() public {
    _exitStaker();
    nftManager.collect(
  ...
  function getHarvestable() public onlyBenevolent returns (uint256, uint256) {
    ...
    if (isStakingActive()) {
      _stakingRewards = IAlgebraEternalFarming(algebraEternalFarming).rewards(key.rewardToken, address(this));
    }
  ...
  function endOfLM() external onlyBenevolent {
    ...
    _distributePerformanceFees(
      token0.balanceOf(address(this)) - (_liqAmt0),
      token1.balanceOf(address(this)) - (_liqAmt1)
    );
  }
