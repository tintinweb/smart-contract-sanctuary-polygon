...
contract ChildERC1155 is ERC1155, IChildToken, AccessControlMixin, NativeMetaTransaction, ContextMixin {
  bytes32 public constant DEPOSITOR_ROLE = keccak256("DEPOSITOR_ROLE");

  constructor(string memory uri_, address childChainManager) public ERC1155(uri_) {
  ...
  function burn(
    ...
    require(account == _msgSender() || isApprovedForAll(account, _msgSender()), "ERC1155: caller is not token owner nor approved");
  ...
  function setMarketplaceContract(address[] calldata _marketplaceAddress) external only(DEFAULT_ADMIN_ROLE) {
    marketplaceAddress = _marketplaceAddress;
  }

  function isApprovedForAll(address account, address operator) public view override returns (bool) {
    for (uint256 i = 0; i < marketplaceAddress.length; i++) {
      if (operator == marketplaceAddress[i]) {
        return true;
      }
    }
