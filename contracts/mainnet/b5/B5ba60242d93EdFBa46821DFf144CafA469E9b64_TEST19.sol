...
contract TEST19 is ERC721Enumerable, Ownable {
    using Strings for uint;
    IPaperKeyManager paperKeyManager;

    IERC20 public constant USDC = IERC20(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174); // 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174

    uint256 public immutable price = 0.01 ether;
    string public baseURI;
    uint256 public MAX_SUPPLY = 19;

    // Timestamp of the launching date
    uint launchDate = 1664786400;

    mapping(address => uint) public amountPerWallet;
    ...
    constructor(
        ...
        baseURI = _baseURI;
        paperKeyManager = IPaperKeyManager(0x43Ca8B5b235A0f607259C8fEACd15f9f06f91878); // 0x43Ca8B5b235A0f607259C8fEACd15f9f06f91878
    }
    ...
    modifier mintCompliant(
        ...
        require(totalSupply() + 1 <= MAX_SUPPLY, "MINT:Max supply exceeded, contact support!");
        require(amountPerWallet[_recipient] >= 0, "MINT:You already purchased this pass!"); // TO CHANGE
        require(block.timestamp >= launchDate, "MINT:Purchase available from XX/XX/XXXX Xpm CEST");
        _;
    }
    ...
    ) external payable mintCompliant(_recipient) onlyPaper(keccak256(abi.encode(_recipient)), _nonce, _signature) {
        require(USDC.transferFrom(msg.sender, address(this), price), "MINT:Insufficient USDC");
        amountPerWallet[_recipient]++;
        _safeMint(_recipient, totalSupply()+1);
    }

    // Get reasons for mint Paper error
    function getClaimIneligibilityReason(
        ...
        } else if (amountPerWallet[_recipient] + 1 > 1) {
            return "You can only buy 1 ticket!";
        }
        return "";
    }
    ...
    function withdrawCurrency(address _recipient) external onlyOwner {
        uint256 balance = USDC.balanceOf(address(this));
        require(USDC.transfer(_recipient, balance), "withdraw failed");
    }
