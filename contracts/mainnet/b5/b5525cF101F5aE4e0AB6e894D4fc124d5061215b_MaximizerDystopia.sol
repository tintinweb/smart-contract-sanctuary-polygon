...
contract MaximizerDystopia is StratManagerUpgradeable, DynamicFeeManager {
  ...
  uint256 public lastPrimaryPoolBalance;
  // It keeps track of primary want balance received by vault, ignoring any other balance received by any other acoount
  uint256 public primaryWantBalance;
  ...
  function __MaximizerDystopia_init(
    ...
    __StratManager_init_unchained(_strategist, address(0), _maximizerVault, _feeRecipient1, _feeRecipient2);
    __DynamicFeeManager_init_unchained();
    __MaximizerDystopia_init_unchained(
      _primaryVault,
      _secondaryWant,
      _rewards,
      _penroseAddresses,
      _native,
      _maximizerRoutesAddress
    );
  }
  ...
  /*
   * @param lastPoolBalance Previous value of `balanceOfPrimaryPool()`
   * @dev += doesn't work when balanceOfPrimaryPool() - lastPoolBalance is negative
   *      updates `lastPrimaryPoolBalance`
   */
  function _updateLastPrimaryPoolBalance(uint256 lastPoolBalance) internal {
    lastPrimaryPoolBalance = lastPrimaryPoolBalance + balanceOfPrimaryPool() - lastPoolBalance;
  }

  /**
   * @param lastBalance previous value of `balanceOfPrimaryWant()`
   * @dev used to update `primaryWantBalance`
   *      meant to be called each time when the primary want token enters or leaves this contract
   */
  function _updatePrimaryWantBalance(uint256 lastBalance) internal {
    primaryWantBalance = primaryWantBalance + balanceOfPrimaryWant() - lastBalance;
  }
   ...
   */
  function beforeDepositAndWithdraw(address sender) external {
  ...
  /**
   * @dev can only be called by the vault
   * @dev update various reward debts of the user
   * @param sender address of the user
   */
  function afterDepositAndWithdraw(address sender) external {
    require(msg.sender == address(maximizerVault), "msg.sender != maximizer vault");
    uint256 userLatestShare = maximizerVault.balanceBelongTo(sender);
    UserInfo storage user = userInfo[sender];
    user.reward1Debt = (userLatestShare * accReward1PerShare) / 1e18;
    user.reward2Debt = (userLatestShare * accReward2PerShare) / 1e18;
    user.secondaryWantDebt = (userLatestShare * accSecondaryWantPerShare) / 1e18;
  }

  /// @dev updates `primaryWantBalance` along with the transfer
  function _safeTransferWantFromVault(uint256 amount) internal {
    require(amount > 0, "amount == 0");
    primaryWantBalance += amount;
    primaryVault.want().safeTransferFrom(address(maximizerVault), address(this), amount);
  }

  /// @dev updates `primaryWantBalance` along with the transfer
  function _safeTransferToVault(uint256 amount) internal {
    require(amount > 0, "amount == 0");
    primaryWantBalance -= amount;
    primaryVault.want().safeTransfer(address(maximizerVault), amount);
  }
  ...
  function deposit(uint256 amount) public whenNotPaused {
    ...
    _safeTransferWantFromVault(amount);
    uint256 lastPoolBal = balanceOfPrimaryPool();
    uint256 lastPrimaryWantBal = balanceOfPrimaryWant();
    primaryVault.deposit(amount);
    _updatePrimaryWantBalance(lastPrimaryWantBal);
    _updateLastPrimaryPoolBalance(lastPoolBal);
  }

  function withdraw(uint256 amount) external {
    require(msg.sender == vault, "!vault");
    if (amount > primaryWantBalance) {
      uint256 sharesToBurn = ((amount - primaryWantBalance) * primaryVault.totalSupply()) / primaryVault.balance();
      uint256 lastPoolBal = balanceOfPrimaryPool();
      uint256 lastPrimaryWantBal = balanceOfPrimaryWant();
  ...
  function _updateLP(address callFeeRecipient) internal whenNotPaused {
    if (block.timestamp > lastHarvest + 4 hours) {
      if (maximizerVault.totalSupply() == 0) return;

      primaryVault.strategy().harvest(callFeeRecipient);

      uint256 primaryWantEarnedSinceLastHarvest = balanceOfPrimaryPool() - lastPrimaryPoolBalance;
      ...
      uint256 primaryWantBefore = primaryWantBalance;
      uint256 lastPrimaryWantBal = balanceOfPrimaryWant();
      primaryVault.withdraw(sharesToBurn);
      _updatePrimaryWantBalance(lastPrimaryWantBal);
      primaryWantAmountForSecondary = primaryWantBalance - primaryWantBefore;
      if (primaryWantAmountForSecondary > 0) {
        lastPrimaryPoolBalance = balanceOfPrimaryPool();
      ...
      uint256 secondaryWantBalance = secondaryWant.balanceOf(address(this));
      if (secondaryWantBalance > 0) {
        IPenroseMasterChef(penroseChef).depositLpAndStake(dystPoolAddress, secondaryWantBalance);
      }
      accSecondaryWantPerShare += ((secondaryWantBalance) * 1e18) / maximizerVault.totalSupply();
  ...
  function _harvestRewardAndSecondaryLP(address _sender) internal {
    ...
    if (userBalance > 0) {
      ...
      if (pendingReward > 0) {
        reward1.safeTransfer(_sender, pendingReward);
      }
      ...
      if (pendingReward > masterBalance) pendingReward = masterBalance;
      if (pendingReward > 0) {
        reward2.safeTransfer(_sender, pendingReward);
      }

      // give secondaryWant to user
      uint256 pendingSecondaryWant = (userBalance * accSecondaryWantPerShare) / 1e18 - user.secondaryWantDebt;
      if ((pendingSecondaryWant > 0) && (balanceOfSecondaryPool() > 0)) {
        if (pendingSecondaryWant > balanceOfSecondaryPool()) pendingSecondaryWant = balanceOfSecondaryPool();
      ...
      if (pendingSecondaryWant > 0) {
        secondaryWant.safeTransfer(_sender, pendingSecondaryWant);
      }
  ...
  function _zapPrimaryWantToNative(uint256 primaryWantAmount) internal {
    require(primaryWantBalance >= primaryWantAmount, "zap::doesn't have enough primaryWant balance");
    ...
    if (primaryLpToken0 != native) {
      ...
      IDystopiaRouter.Route[] memory routeArray = getRoute(
        maximizerRoutes.getPrimaryLp0ToNativeRoute(),
        maximizerRoutes.getIsStablePrimaryLp0ToNative()
      );
    ...
    if (primaryLpToken1 != native) {
      ...
      IDystopiaRouter.Route[] memory routeArray = getRoute(
        maximizerRoutes.getPrimaryLp1ToNativeRoute(),
        maximizerRoutes.getIsStablePrimaryLp1ToNative()
      );
  ...
  function _zapNativeToSecondaryWant(uint256 nativeBalance) internal {
    require(
      IERC20Upgradeable(native).balanceOf(address(this)) >= nativeBalance,
    ...
    if (native != secondaryLpToken0) {
      IDystopiaRouter.Route[] memory routeArray = getRoute(
        maximizerRoutes.getNativeToSecondaryLpToken0Route(),
        maximizerRoutes.getIsStableNativeToSecondaryLpToken0()
      );
      IDystopiaRouter(router).swapExactTokensForTokens(
    ...
    if (native != secondaryLpToken1) {
      IDystopiaRouter.Route[] memory routeArray = getRoute(
        maximizerRoutes.getNativeToSecondaryLpToken1Route(),
        maximizerRoutes.getIsStableNativeToSecondaryLpToken1()
      );
  ...
  function _chargeFees(address callFeeRecipient) internal {
    ...
    if (generalFeeAmount > 0) {
      ...
      if (callFeeAmount + feeAmount1 + feeAmount2 + strategistFeeAmount != generalFeeAmount) {
        if (fee1 > 0) {
          feeAmount1 = (generalFeeAmount - callFeeAmount - feeAmount2 - strategistFeeAmount);
        } else if (fee2 > 0) {
          feeAmount2 = (generalFeeAmount - callFeeAmount - feeAmount1 - strategistFeeAmount);
        } else {
          strategistFeeAmount = (generalFeeAmount - callFeeAmount - feeAmount1 - feeAmount2);
        }
      }
  ...
  function getRoute(address[] memory addressRoute, bool[] memory isStable)
    internal
    pure
    returns (IDystopiaRouter.Route[] memory)
  {
  ...
  // calculate the total underlaying 'primary want' held by the strat.
  function balanceOf() public view returns (uint256) {
    return primaryWantBalance + balanceOfPrimaryPool();
  }

  // it calculates how much 'primary want' this contract holds.
  function balanceOfPrimaryWant() public view returns (uint256) {
  ...
  function inCaseTokensGetStuck(address _token) external onlyOwner {
    require(
      (_token != address(primaryVault.want())) &&
        (_token != address(secondaryWant)) &&
        (_token != native) &&
        (_token != address(reward1)) &&
        (_token != address(reward2)) &&
        (_token != primaryLpToken0) &&
        (_token != primaryLpToken1) &&
        (_token != secondaryLpToken0) &&
        (_token != secondaryLpToken1),
      "!token"
    );
  ...
  function unpause() external onlyManager {
    ...
    uint256 lastPoolBal = balanceOfPrimaryPool();
    uint256 lastPrimaryWantBal = balanceOfPrimaryWant();
    primaryVault.deposit(primaryWantBalance);
    _updatePrimaryWantBalance(lastPrimaryWantBal);
    _updateLastPrimaryPoolBalance(lastPoolBal);
...
contract StratManagerUpgradeable is Initializable, OwnableUpgradeable, PausableUpgradeable {
  ...
  function __StratManager_init(
    ...
    __Ownable_init_unchained();
    __Pausable_init_unchained();
    __StratManager_init_unchained(_strategist, _dystRouter, _vault, _feeRecipient1, _feeRecipient2);
  }
...
interface IMaximizerDystopia {
  ...
  function balanceOfPrimaryWant (  ) external view returns ( uint256 );
  function callFee (  ) external view returns ( uint256 );
  function claimReward (  ) external;
