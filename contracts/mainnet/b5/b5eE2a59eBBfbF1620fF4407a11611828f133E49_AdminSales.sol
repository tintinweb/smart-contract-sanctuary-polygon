...
import "./IAdminSales.sol";

contract AdminSales is IAdminSales, App, __with_semver(uint8(1), uint8(0), uint8(0)){

    uint8 public immutable CONTROLLER = _initNextRole();
    /// @custom:oz-upgrades-unsafe-allow state-variable-immutable
    UpgradeableBeacon internal immutable beaconWhiteListSale;
    /// @custom:oz-upgrades-unsafe-allow state-variable-immutable
    UpgradeableBeacon internal immutable beaconOpenSale;
    ...
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor(address trustedForwarder, address whiteListSaleImpl, address publicSaleImpl) ERC2771Context(trustedForwarder) {
        beaconWhiteListSale = new UpgradeableBeacon(whiteListSaleImpl);
        beaconOpenSale = new UpgradeableBeacon(publicSaleImpl);
    }
    ...
    function addWhiteListSale(bytes calldata options)
    external
    requirePermission(CONTROLLER)
    {
        address whiteList = address(new BeaconProxy(address(beaconWhiteListSale), ""));
        (bool success, ) = whiteList.call(
            abi.encodeWithSignature("initialize(IKernel,address,bytes)", kernel, address(this), options)
        );

        require(success, Exceptions.INVALID_INITIALIZATION_ERROR);
        sales.push(Sale(SaleType.whiteList, address(whiteList)));

        emit AddSale(sales.length-1, address(whiteList), SaleType.whiteList);
    }

    function addOpenSale(bytes calldata options)
    external
    requirePermission(CONTROLLER)
    {
        address openSale = address(new BeaconProxy(address(beaconOpenSale), ""));
        (bool success, ) = openSale.call(
            abi.encodeWithSignature("initialize(IKernel,address,bytes)", kernel, address(this), options)
        );

        require(success, Exceptions.INVALID_INITIALIZATION_ERROR);
        sales.push(Sale(SaleType.open, address(openSale)));
        emit AddSale(sales.length-1, address(openSale), SaleType.open);
    }
    ...
    function executeSingle(address app, bytes memory data) public requirePermission(CONTROLLER) returns (bytes memory){
        return Address.functionCall(
            ...
            Exceptions.ADMIN_SALES_ERROR
        );
    }

    function executeBatch(address [] calldata apps, bytes [] calldata data) external requirePermission(CONTROLLER) returns (bytes [] memory returnData) {
...
contract ERC721OpenSale is ERC721BaseSale, __with_semver(uint8(1), uint8(1), uint8(2)) {
    ...
    //TODO нужно ли зашивать метод setTokenSaleAddress в initialize ? нужно!
    function initialize(IKernel _kernel, address _adminSales, bytes calldata options) external initializer {
        ...
        setTokenSaleAddress(_tokenSaleAddress);
...
//            ERC721Properties(appAddress).mint(to, tierValue);
            IAdminSales(adminSales).executeSingle(appAddress, abi.encodeCall(ERC721Properties.mint, (to, tierValue)));
...
//            ERC721Properties(appAddress).mint(to, tierValue);
    ...
    function setTokenSaleAddress(address tokenSaleAddress_) public onlyAdminSales() {
