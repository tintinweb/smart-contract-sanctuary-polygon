...
contract VaultResolver is IResolver {
    VaultManager public vaultManager;

    uint256 public constant GAS_PER_HARVEST = 300000;
    ...
    {
        for (uint256 i = 0; i < vaultManager.getVaultLength(); i++) {
            IVault vault = IVault(vaultManager.getVault(i));

            canExec = shouldExecuteHarvest(vault);

            if (canExec) {
                execPayload = abi.encodeWithSelector(
                    VaultManager.harvestVault.selector,
                    address(vault)
                );
                break;
            }
            ...
            execPayload = bytes(
                "VaultResolver: No Vaults harvestable right now"
            );
        }
    }

    function shouldExecuteHarvest(IVault _vault) public view returns (bool) {
        return (_vault.accruedFees() * 100) / 5 > GAS_PER_HARVEST * tx.gasprice;
    }
}
