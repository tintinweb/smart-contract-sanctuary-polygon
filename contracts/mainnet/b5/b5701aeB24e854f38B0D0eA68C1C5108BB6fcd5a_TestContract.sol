...
contract TestContract is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  struct SwapInfo {
    address srcToken;
    address dstToken;
    uint256 amount;
  ...
  event TestDeposit(uint256 amount, SwapInfo[] swaps);
  ...
  function depositTest(bytes calldata auth, SwapInfo[] calldata swaps) external whenNotPaused nonReentrant {
    ...
    emit TestDeposit(value, swaps);
    //doSwaps(swaps);
  }

  function doSwaps(SwapInfo[] calldata swaps) private {
    for (uint8 i; i < swaps.length; i++) {
      IAggregationRouterV5.SwapDescription memory desc;
      desc.srcToken = swaps[i].srcToken;
      desc.dstToken = swaps[i].dstToken;
      desc.srcReceiver = payable(address(this));
      desc.dstReceiver = payable(address(this));
      desc.amount = swaps[i].amount;
      desc.minReturnAmount = swaps[i].minReturnAmount;
      desc.flags = swaps[i].flags;
      if (!IERC20Upgradeable(desc.srcToken).approve(ONE_INCH_ROUTER, desc.amount)) {
        revert();
      }
      IAggregationRouterV5(ONE_INCH_ROUTER).swap(swaps[i].executor, desc, swaps[i].permit, swaps[i].data);
    }
  }
}
...
interface IAggregationRouterV5 {
  ...
  function swap(address executor, SwapDescription calldata desc, bytes calldata permit, bytes calldata data) external payable returns (uint256 returnAmount, uint256 spentAmount);
}
