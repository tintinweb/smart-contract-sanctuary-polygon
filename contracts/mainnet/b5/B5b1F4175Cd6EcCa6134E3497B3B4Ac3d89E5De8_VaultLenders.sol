...
/// @title Vault Lenders Contract.
/// @author Panoram Finance.
/// @notice This contract save the money deposit in lending, this money is used to give mortgages for the purchase of Panoram NFTs.
contract VaultLenders is ReentrancyGuard, AccessControl {
    ...
    /// @dev mapping for permissions: Contract address => true/false.
    mapping(address => bool) public contracts;
    /// @dev mapping to save the amount of money borrow: address (token address) => uint256 (borrowed money).
    mapping (address => uint256) private borrows;
    /// @dev mapping to save the amount of money available to lend: address (token address) => uint256 (available money).
    ...
    /// @param _minDeposit - the minimum deposit accepted.
    /// @param _maxdeposit - the maximum deposit accepted.
    constructor(address _tokenInfo, address _token, uint256 _minDeposit, uint256 _maxdeposit) {
        ...
        // Panoram's Money, already lend it.
        totalSupply = 20954037808;
        borrows[address(token)] = 20954037808;
    ...
    /// @param _lendingContract - The contract address to allow or deny.
    ...
    /// @dev Function to withdraw money when a loan is created or when a lender wants to withdraw his money.
    ...
    /// @return _usage - the usage rate of the vault.
    ...
    /// @return _borrows - the amount of money borrowed.
    ...
    /// @return _avaible - the amount of money available for loans.
    ...
    /// @return maxDeposit - The maximum deposit allowed.
    ...
    /// @return minDeposit - The minimum deposit allowed.
    function getMinDeposit() public view returns(uint256){
       return minDeposit; 
    }

    /// @dev Function to update the minimum deposit allowed in the vault.
    /// @param _minDeposit - the new value for minDeposit.
    function updateMinDeposit(uint256 _minDeposit) public onlyStrategyContract {
        minDeposit = _minDeposit;
    }

    /// @dev Function to update the maximum deposit allowed in the vault.
    /// @param _maxDeposit - the new maximum deposit value.
