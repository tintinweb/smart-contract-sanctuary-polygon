...
// File: contracts/CSUtility.sol


pragma solidity ^0.8.2;


contract CSUtility is ERC20, AccessControl, ERC721Holder {
    bytes32 public constant CONTRACT_ADMIN_ROLE = keccak256("CONTRACT_ADMIN_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    address public collectionAddressCastleSagaNFT;
    address public collectionAddressNFTv2;

    uint256 public numberOfBlocksPerRewardUnit;

    uint256 public coinAmountPerRewardUnitCastleSagaNFT;
    uint256 public coinAmountPerRewardUnitNFTv2;

    uint256 public welcomeBonusAmountCastleSagaNFT;
    uint256 public welcomeBonusAmountNFTv2;

    uint256 public amountOfStakersCastleSagaNFT;
    uint256 public amountOfStakersNFTv2;

    uint256 public tokensStakedCastleSagaNFT;
    ...
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogCastleSagaNFT;

        /// owner => tokenId => StakeInfo
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogNFTv2;

    /// owner => #NFTsStaked
    mapping (address => uint256) public tokensStakedByUserCastleSagaNFT;
    ...
    mapping (uint256 => bool) public welcomeBonusCollectedCastleSagaNFT;

        /// tokenId => true/false (true if welcome bonus has been collected for a specific tokenId)
    mapping (uint256 => bool) public welcomeBonusCollectedNFTv2;

    /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserCastleSagaNFT;

        /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserNFTv2;

    /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioCastleSagaNFT;

        /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioNFTv2;

    event RewardsHarvestedCastleSagaNFT (address owner, uint256 amount);
    event NFTStakedCastleSagaNFT (address owner, uint256 tokenId);
    event NFTUnstakedCastleSagaNFT (address owner, uint256 tokenId);
    ...
    constructor(address owner, address _collectionAddressCastleSagaNFT, address _collectionAddressNFTv2) ERC20("CastleSagaUtility", "CSU") AccessControl(){
        _mint(owner, 100000 * 10 ** 18);
        _setupRole(DEFAULT_ADMIN_ROLE, owner);
        _setupRole(CONTRACT_ADMIN_ROLE, owner);
        _setupRole(BURNER_ROLE, owner);
        _setupRole(MINTER_ROLE, owner);
        collectionAddressCastleSagaNFT = _collectionAddressCastleSagaNFT;
        collectionAddressNFTv2 = _collectionAddressNFTv2;
        contractCreationBlock = block.number;
        coinAmountPerRewardUnitCastleSagaNFT = 15 * 10 ** 18; // 5 ERC20 coins per rewardUnit, may be changed later on
        ...
        welcomeBonusAmountCastleSagaNFT = 300 * 10 ** 18; // 300 tokens welcome bonus, only paid once per tokenId
        welcomeBonusAmountNFTv2 = 600 * 10 ** 18; // 600 tokens welcome bonus, only paid once per tokenId
    }

    function stakedNFTSByUserCastleSagaNFT(address owner) external view returns (uint256[] memory){
        return stakePortfolioByUserCastleSagaNFT[owner];
    }
    ...
    function pendingRewardsCastleSagaNFT(address owner, uint256 tokenId) public view returns (uint256){
        StakeInfo memory info = stakeLogCastleSagaNFT[owner][tokenId];
        ...
        return rewardAmount * coinAmountPerRewardUnitCastleSagaNFT;
    ...
    function stakeCastleSagaNFT(uint256 tokenId) public {
        IERC721(collectionAddressCastleSagaNFT).safeTransferFrom(_msgSender(), address(this), tokenId);
        require(IERC721(collectionAddressCastleSagaNFT).ownerOf(tokenId) == address(this),
            "CSUtility: Error while transferring token");
        StakeInfo storage info = stakeLogCastleSagaNFT[_msgSender()][tokenId];
        info.stakedAtBlock = block.number;
        info.lastHarvestBlock = block.number;
        info.currentlyStaked = true;
        if(tokensStakedByUserCastleSagaNFT[_msgSender()] == 0){
            amountOfStakersCastleSagaNFT += 1;
        }
        tokensStakedByUserCastleSagaNFT[_msgSender()] += 1;
        tokensStakedCastleSagaNFT += 1;
        stakePortfolioByUserCastleSagaNFT[_msgSender()].push(tokenId);
        uint256 indexOfNewElement = stakePortfolioByUserCastleSagaNFT[_msgSender()].length - 1;
        indexOfTokenIdInStakePortfolioCastleSagaNFT[tokenId] = indexOfNewElement;
        if(!welcomeBonusCollectedCastleSagaNFT[tokenId]) {
            _mint(_msgSender(), welcomeBonusAmountCastleSagaNFT);
            welcomeBonusCollectedCastleSagaNFT[tokenId] = true;
        }

        emit NFTStakedCastleSagaNFT(_msgSender(), tokenId);
    }
    ...
    function stakeBatchCastleSagaNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            stakeCastleSagaNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakeBatchCastleSagaNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            unstakeCastleSagaNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakeCastleSagaNFT(uint256 tokenId) public {
        if(pendingRewardsCastleSagaNFT(_msgSender(), tokenId) > 0){
            harvestCastleSagaNFT(tokenId);
        }
        StakeInfo storage info = stakeLogCastleSagaNFT[_msgSender()][tokenId];
        info.currentlyStaked = true;
        IERC721(collectionAddressCastleSagaNFT).safeTransferFrom(address(this), _msgSender(), tokenId);
        require(IERC721(collectionAddressCastleSagaNFT).ownerOf(tokenId) == _msgSender(),
            "CSUtility: Error while transferring token");
        if(tokensStakedByUserCastleSagaNFT[_msgSender()] == 1){
            amountOfStakersCastleSagaNFT -= 1;
        }
        tokensStakedByUserCastleSagaNFT[_msgSender()] -= 1;
        tokensStakedCastleSagaNFT -= 1;
        stakePortfolioByUserCastleSagaNFT[_msgSender()][indexOfTokenIdInStakePortfolioCastleSagaNFT[tokenId]] = 0;
        emit NFTUnstakedCastleSagaNFT(_msgSender(), tokenId);
    }
    ...
    function harvestCastleSagaNFT(uint256 tokenId) public {
        StakeInfo storage info = stakeLogCastleSagaNFT[_msgSender()][tokenId];
        uint256 rewardAmountInERC20Tokens = pendingRewardsCastleSagaNFT(_msgSender(), tokenId);
        if(rewardAmountInERC20Tokens > 0) {
            info.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInERC20Tokens);
            emit RewardsHarvestedCastleSagaNFT(_msgSender(), rewardAmountInERC20Tokens);
        }
    }
    ...
    function harvestBatchCastleSagaNFT(address user) external {
        uint256[] memory tokenIds = stakePortfolioByUserCastleSagaNFT[user];

        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            harvestCastleSagaNFT(tokenIds[currentId]);
        }
    }
    ...
    function setCoinAmountPerRewardUnitCastleSagaNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        coinAmountPerRewardUnitCastleSagaNFT = coinAmount;
    }
    ...
    function setWelcomeBonusAmountCastleSagaNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        welcomeBonusAmountCastleSagaNFT = coinAmount;
    }
    ...
    function setCollectionAddressCastleSagaNFT(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "VIBE: update to zero address not possible");
        collectionAddressCastleSagaNFT = newAddress;
    }
