...
contract BotShipPool is Managable, Pausable, ReentrancyGuard, ERC721Holder {
    ...
    function buyItem(
        ...
        _buyItem(_listing, _nftAddress, _tokenId);
    }
    ...
    function buyTeam(
        ...
        _buyTheTeam(_nftAddress, _payToken, _price, _tokenId);
    ...
    function _buyItem(Listing memory _listing, address _nftAddress, uint _tokenId) private {
       ...
       if(_listing.payToken == BITS_ADDRESS){
            ...
            ITokenPaymentSplitter(treasuryAddress).split(BITS_ADDRESS, msg.sender, _listing.price);
       } else {
            require(msg.value == _listing.price, "not enough native token");
            (bool success,) = treasuryAddress.call{value: msg.value}(
                abi.encodeWithSignature("split(address,address,uint256)", ZERO_ADDRESS, msg.sender, _listing.price)
            );
    ...
    function _buyTheTeam(address _nftAddress, address[3] memory _payToken, uint256[3] memory _price, uint[3] memory _tokenId) private {
       uint _bitsPrice;
       uint _maticPrice;
       ...
       if(_maticPrice > 0){
        ...
        emit TeamSold(_nftAddress, _msgSender(), _tokenId);
    }
