...
// File: nft_market/metaTx/eip720.sol
...
// File: nft_market/nft_receiver.sol
...
// File: nft_market/nft_meta_data.sol
...
// File: nft_market/nft_contract.sol
...
contract ERC1155Basic is IERC1155, ERC165, Context, Ownable, NftMetaDataSupport, Pausable, ReentrancyGuard, EIP712Base {
    /**
        Using libraries on different types for different operation
    **/
    // using SafeMath for uint256;
    using Address for address;

    /**
        Define token owners balances
    **/
    mapping(uint256 => mapping(address => uint256)) private _balances;

    /**
        Operator address approvals
    **/
    mapping(address => mapping(address => bool)) private _approvals;

    /**
        Determines of total supply for a token in system
    **/
    mapping(uint256 => uint256) private _totalSupply;
    ...
    /**
    Standard impelementation of an eth foundation interface to determine
    this contract is supporting and implementing declared interfaces or not !
    **/
    ...
    **/
    function renounceOwnership() public view override onlyOwner {
        require(false , "Owner connact rennounce ownership");
    }
    ...
    //     require(newOwner != owner() , "Cannot transfer owner to owner");
    //     _transferOwnership(newOwner);
    // }
    ...
    **/
    modifier onlySenderOrApproved(address requester, address target) {
        require(requester == _msgSender() || isApprovedForAll(requester, target) || isApprovedForAll(requester,_msgSender()) == true, "ERC1155: transfer caller is not owner nor approved");
        _;
    }

    /**
        Gets and returns balance of a nft for an account
    **/
    ...
    /**
        returns batch balance of owned of tokens owned by a group
    **/
    function balanceOfBatch(address[] calldata accounts, uint256[] calldata ids) public override view returns (uint256[] memory)
    {
        require(accounts.length == ids.length, "ERC1155: ids and addresses should have same balance");
    ...
    **/
    function setApprovalForAll(address operator, bool approved) public override
    {
        ...
        require(operator != sender, "ERC1155: Cannot set approval for your self");
        _approvals[sender][operator] = approved;
        emit ApprovalForAll(sender, operator, approved);
    }

    /**
        Checks whether an operator is apporved for operations on address or not
    **/
    function isApprovedForAll(address account, address operator) override view public returns (bool)
    {
        return _approvals[account][operator];
    }

    /**
        Safe transfer an nft to another user
    **/
    function safeTransferFrom(
        address from,
        address to,
        ...
        uint256 amount,
        bytes calldata data
    ) override onlySenderOrApproved(from, to) public {
        require(to != address(0), "ERC1155: transfer to the zero address");

        address operator = _msgSender();

        require(_balances[id][from] >= amount, "Owner doesnt have sufficient Tokens");
    ...
    /**
        Transfer batch Tokens in safe mode
    **/
    function safeBatchTransferFrom(
        address from,
        address to,
        uint256[] calldata ids,
        uint256[] calldata amounts,
        bytes calldata data) override onlySenderOrApproved(from, to) public
    {
        require(to != address(0), "To should not be zero address of ethereum");
        require(ids.length == amounts.length, "Ids length and amounts are not the same");
    ...
    **/
    ...
    function batchMint(
        ...
        require(ids.length == values.length, "Id's and values should be the same length");
        require(to != address(0), "To should not be zero address");
    ...
    function burn(
        ...
        require(from != address(0), "ERC1155: burn from the zero address");
        require(amount > 0, "ERC115: Amounts to burn should be more than 0");

        address operator = _msgSender();

        _beforeTokenTransfer(operator, from, address(0), asSingletonArray(id), asSingletonArray(amount), "");
    ...
    /**
        Returns the available total supply for given token
    **/
    function totalSupply(uint256 id) public view returns(uint)
    {
        return _totalSupply[id];
    }

    /**
        Determines if a token with a given id exists or not
    **/
    ...
    /**
        Safe Transfer Check after tranfser transaction has been made
    **/
    function _doSafeTransferAcceptanceCheck(
        address operator,
        address from,
    ...
    /**
        Do safe transaction check after batch transaction has been accured
    **/
    function _doSafeBatchTransferAcceptanceCheck(
        address operator,
        address from,
