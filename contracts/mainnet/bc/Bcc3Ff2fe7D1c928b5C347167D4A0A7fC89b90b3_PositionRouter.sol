...
contract PositionRouter is BasePositionManager, IPositionRouter {
    ...
    function setIsActive(bool _isActive) external onlyAdmin {
        isActive = _isActive;
    ...
    function marketCreateIncreasePosition(
        ...
        require(isActive == true, "PositionRouter: not active");
        ...
        if (_amountIn > 0) {
           ...
           if (_path.length > 1) {
               IERC20(_path[0]).safeTransfer(vault, amountIn);
               amountIn = _swap(_path, _minOut, address(this));
           }

           uint256 afterFeeAmount = _collectFees(msg.sender, _path, amountIn, _indexToken, _isLong, _sizeDelta);
           IERC20(_path[_path.length - 1]).safeTransfer(vault, afterFeeAmount);
       }
       _increasePosition(msg.sender, _path[_path.length - 1], _indexToken, _sizeDelta, _isLong, _acceptablePrice);
    ...
    function marketCreateIncreasePositionETH(
        address[] memory _path,
        address _indexToken,
        uint256 _minOut,
        uint256 _sizeDelta,
    ...
    function marketCreateDecreasePosition(
        address[] memory _path,
        address _indexToken,
        ...
        // _transferInETH();
        // Execute Order
         uint256 amountOut = _decreasePosition(msg.sender, _path[0], _indexToken, _collateralDelta, _sizeDelta, _isLong, address(this), _acceptablePrice);
        ...
        if (_withdrawETH) {
           _transferOutETHWithGasLimit(amountOut, payable(_receiver));
        } else {
    ...
    function executeIncreasePosition(bytes32 _key, address payable _executionFeeReceiver) public nonReentrant returns (bool) {
       ...
       if (request.amountIn > 0) {
           ...
           if (request.path.length > 1) {
               IERC20(request.path[0]).safeTransfer(vault, request.amountIn); // caller is msg.sender = account
               amountIn = _swap(request.path, request.minOut, address(this)); // this is PositionRouter
           }

           uint256 afterFeeAmount = _collectFees(msg.sender, request.path, amountIn, request.indexToken, request.isLong, request.sizeDelta); // msg.sende is account
...
contract BasePositionManager is IBasePositionManager, ReentrancyGuard, Governable {
    ...
    function _decreasePosition(address _account, address _collateralToken, address _indexToken, uint256 _collateralDelta, uint256 _sizeDelta, bool _isLong, address _receiver, uint256 _price) internal returns (uint256) {
        ...
        // ITimelock(timelock).enableLeverage(_vault); // Now not use timelock
