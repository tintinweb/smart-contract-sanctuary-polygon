...
// File: contracts/interfaces/IERC20WLDY.sol
...
// File: contracts/interfaces/IERC721Verifiable.sol
...
contract MarketplaceV2 is Initializable, Pausable, AccessControl, NativeMetaTransaction {
  ...
  event FeesCollectorSet(address indexed oldFeesCollector, address indexed newFeesCollector);
    ...
    constructor()  {
    // EIP712 init
    _initializeEIP712('Wilderness Marketplace', '0.1');


    _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
    _grantRole(OWNER_ROLE, address(0x61Af372B685e91Db552aefd23220533f0843fd4A));
    // Address init
    setFeesCollector(address(0xdc59523c97ebb107E002934442A45a1132945b4B));

    // Fee init
    setFeesCollectorCutPerMillion(25000);

    require(address(0xdc59523c97ebb107E002934442A45a1132945b4B) != address(0), "MarketplaceV2#constructor: INVALID_OWNER");

    require(address(0x258D8423d9d6Fae7BFF3E5B7D1aA13348492aA0b).isContract(), "MarketplaceV2#constructor: INVALID_ACCEPTED_TOKEN");
    acceptedToken = IERC20(address(0x258D8423d9d6Fae7BFF3E5B7D1aA13348492aA0b));
  ...
  /**
    ...
    */
  function setPublicationFee(uint256 _publicationFee) external onlyRole(OWNER_ROLE) {
  ...
  /**
    ...
    */
  function setFeesCollectorCutPerMillion(uint256 _feesCollectorCutPerMillion) public onlyRole(OWNER_ROLE) {
  ...
  */
  function setFeesCollector(address _newFeesCollector) onlyRole(OWNER_ROLE) public {
   ...
   whenNotPaused
  {
    ...
    IERC721CollectionV2 nftContract = IERC721CollectionV2(nftAddress);
    bytes memory approveFunctionalSignature = abi.encodePacked(
    bytes4(keccak256("approve(address,uint256)")), // function signature
      abi.encode(
          address(this),
          assetId
      )
    );
    nftContract.executeMetaTransaction(sender, approveFunctionalSignature, sigR, sigS, sigV);
    _metaCreateOrder(
      userAddress,
      nftAddress,
      assetId,
   ...
   whenNotPaused
  {
    ...
    IERC20WLDY coinContract = IERC20WLDY(address(0x258D8423d9d6Fae7BFF3E5B7D1aA13348492aA0b));
    bytes memory approveFunctionalSignature = abi.encodePacked(
    bytes4(keccak256("approve(address,uint256)")), // function signature
      abi.encode(
          address(this),
    ...
    coinContract.executeMetaTransaction(userAddress, approveFunctionalSignature, sigR, sigS, sigV);
    _metaExecuteOrder(
      userAddress,
      nftAddress,
      assetId,
...
function _metaCreateOrder(
  ...
  function _cancelOrder(address nftAddress, uint256 assetId) internal returns (Order memory) {
    ...
    require(order.seller == sender, "MarketplaceV2#_cancelOrder: UNAUTHORIZED_USER");
   ...
   internal returns (Order memory)
  {
    ...
    require(
      acceptedToken.transferFrom(sender, order.seller, price - feesCollectorShareAmount),
