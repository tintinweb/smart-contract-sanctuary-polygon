...
// File: contracts/url_builder.sol
...
contract URIBuilder {
    ...
    URIRule[] private uri_rules;
    bool private reveal_locked;

    error RevealLocked();

    function _addURIRule(URIRule memory rule) internal {
        uri_rules.push(rule);
    }
    ...
    function _lockReveal() internal {
        reveal_locked = true;
    }

    function _applyURIRule(URIRule storage rule, uint id) private view returns(string memory) {
        if (!rule.add_id) {
            return rule.prefix;
        }
        return string(abi.encodePacked(rule.prefix, id.toString(), rule.postfix));
    }

    function _buildURI(uint id) internal view returns(string memory) {
        for (uint i = 0; i < uri_rules.length; i++) {
            if (id <= uri_rules[i].max_id) {
                return _applyURIRule(uri_rules[i], id);
            }
        }
        return "";
    }
...
contract VKNFTCollection is ERC721WithPermit, Ownable, URIBuilder {

  using Strings for uint256;
  using Counters for Counters.Counter;
  Counters.Counter private _tokenIds;

  bool private revealLock;
  address private _minter;
  bool private _minting_disabled;
  uint private different_tokens_count;

  error Forbidden();

  constructor(string memory name_, string memory symbol_) ERC721WithPermit(name_, symbol_) {
    _minter = msg.sender;
    different_tokens_count = 1;
  }


  function addURIRule(uint max_id, bool add_id, string calldata prefix, string calldata postfix) external onlyOwner {
    _addURIRule(URIRule(max_id, add_id, prefix, postfix));
  }

  function setDifferentTokensCount(uint _different_tokens_count) external onlyOwner {
    different_tokens_count = _different_tokens_count;
  }
  ...
  */
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
      _requireMinted(tokenId);
      return _buildURI(tokenId % different_tokens_count);
  }
  ...
  function disableMinting() external onlyOwner {
    _minting_disabled = true;
  }

  function mint(address to, uint id) external {
    if (_minting_disabled) {
      revert Forbidden();
    }
  ...
  function mintMany(uint from_id, address[] calldata to) external {
    if (_minting_disabled) {
      revert Forbidden();
    }
    if (msg.sender != _minter) {
