...
contract VisionDaoTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SVISIONDAO }
    ...
    address public sVISIONDAO;
    uint public sVISIONDAOQueue; // Delays change to sVISIONDAO address
    ...
    /**
        @notice allow approved address to deposit an asset for VISIONDAO
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint VISIONDAO needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( VISIONDAO ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn VISIONDAO for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IVISIONDAOERC20( VISIONDAO ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sVISIONDAO ).balanceOf( msg.sender ); // Can only borrow against sVISIONDAO held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with VISIONDAO
        @param _amount uint
     */
    function repayDebtWithVISIONDAO( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IVISIONDAOERC20( VISIONDAO ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, VISIONDAO, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( VISIONDAO ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( VISIONDAO ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns VISIONDAO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match VISIONDAO decimals
            value_ = _amount.mul( 10 ** IERC20( VISIONDAO ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SVISIONDAO ) { // 9
            sVISIONDAOQueue = block.number.add( blocksNeededForQueue );
        } else return false;
