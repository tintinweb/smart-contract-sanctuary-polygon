...
contract CoinFlip is AccessControl, VRFConsumerBase {
    struct Flip {
        address player;
        uint betAmount;
        uint side;
    ...
    mapping (bytes32 => Flip) public flipRequests;
    mapping (bytes32 => uint256) _randomnesses;

    // polygon testnet VRF values
    // bytes32 internal _keyHash = 0x6e75b569a01ef56d18cab6a8e71e6600d6ce853834d4a5748b720d06f878b3a4; 
    // uint256 internal _fee = 100000000000000; // 0.0001
    ...
    // polygon mainnet VRF values
    bytes32 internal _keyHash = 0xf86195cf7690c55907b2b611ebb7343a6f649bff128701cc542f0569e2c549da; 
    uint256 internal _fee = 100000000000000; // 0.0001
    address internal _vrfCoordinator = 0x3d2341ADb2D31f1c5530cDC622016af293177AE0;
    address internal _linkToken = 0xb0897686c545045aFc77CF20eC7A532E3120E0F1;    

    uint _minBet = 15000000000000000000; // default minimum bet is 15 MATIC in wei units
    uint _maxBet = 125000000000000000000; // default maximum bet is 125 MATIC ETH in wei units
    uint _randNonce = 0;
    ...
    event vrfRandomnessReceived(bytes32 requestId);
    event playerFlipped(address indexed playerAddress, uint betAmount, uint side, uint amountWon, uint amountCommission, uint amountSent, uint numberGenerated);

    constructor() VRFConsumerBase(_vrfCoordinator, _linkToken) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender); // set deployer as admin
        _setupRole(BACKEND_ROLE, msg.sender); // set deployer as backend
    }
    ...
    modifier onlyAdminOrBackend() {
        // this modifier restrictions function calls to only succeed when caller either has admin or backend role
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || hasRole(BACKEND_ROLE, msg.sender), "Caller is neither admin nor backend");
        _;
    }
    ...
    function flip(uint side) public payable {
        ...
        require(getLinkBalance() >= _fee, "Contract does not have enough LINK");
        ...
        require(side <= 1, "Pass 0 for heads, 1 for tails");

        totalBets += msg.value;

        bytes32 requestId = requestRandomness(_keyHash, _fee);
        flipRequests[requestId] = Flip(msg.sender, msg.value, side);
    }    

    // get the LINK balance held in this contract
    function getLinkBalance() public view returns (uint256) {
        return LINK.balanceOf(address(this));
    }

    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        _randomnesses[requestId] = randomness;
        emit vrfRandomnessReceived(requestId);        
    }

    function finalizeFlip(bytes32 requestId) public onlyAdminOrBackend {
        Flip memory f = flipRequests[requestId];
        uint256 randomness = _randomnesses[requestId];
        uint randomResult = (randomness % 100) + 1;
        bool playerWon = false;
        if (randomResult <= 50 && f.side == 0) {
            playerWon = true;
        }
        if (randomResult > 50 && f.side == 1) {
            playerWon = true;
        }
        ...
        if (playerWon) {
            ...
            payable(f.player).transfer(amountToSend);                        
            emit playerFlipped(f.player, f.betAmount, f.side, winnings, commission, amountToSend, randomResult);
        } else {
            playerLosses++;
            emit playerFlipped(f.player, f.betAmount, f.side, 0, 0, 0, randomResult);
        }

        // automation: check if backend balance is less than 2 matic and send 2 matic 
        if (msg.sender.balance < 2000000000000000000) {
