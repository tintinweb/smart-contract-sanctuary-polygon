...
contract USDL is IUSDL, ERC20PermitUpgradeable {
    // STORAGE
    ...
    error SenderIsNotVault(address sender, address vault);

    // MODIFIERS

    modifier onlyVault() {
        if (_msgSender() != vault) revert SenderIsNotVault(_msgSender(), vault);
        _;
    }
