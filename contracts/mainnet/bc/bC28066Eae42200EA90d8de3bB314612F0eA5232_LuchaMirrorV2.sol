...
// Credit: https://github.com/geeogi/loot-mirror
...
contract LuchaMirrorV2 is AccessControl {
  ILuchadores luchadores;
  ...
  constructor(address _luchadores, address _polygonBridge) {
    luchadores = ILuchadores(_luchadores);
  ...
  function balanceOf(address owner) public view returns (uint256) {
    ...
    uint256 balanceL1 = _balances[owner];
    uint256 balanceL2 = luchadores.balanceOf(owner);

    // check L2 token is owned by 0 on L1 to prevent double balance
    for(uint256 i; i < balanceL2; i++) {
      require(
        _owners[luchadores.tokenOfOwnerByIndex(owner, i)] == address(0),
        "LuchaMirror: pending ownership update"
      );
    }

    return balanceL1 + balanceL2;
  }

  function ownerOf(uint256 tokenId) external view returns (address) {
    // check if token has been bridged to L2
    try luchadores.ownerOf(tokenId) returns(address) {
  ...
  function tokenOfOwnerByIndex(address owner, uint256 index) external view returns (uint256) {
    ...
    // lookup index on L1 then L2
    if (index >= balanceL1) {
      return luchadores.tokenOfOwnerByIndex(owner, index - balanceL1);
    } else {
   ...
   */
  function setOwners(OwnerUpdate[] calldata _ownerUpdates) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    for (uint256 i = 0; i < _ownerUpdates.length; i++) {
      ...
      for (uint256 k = 0; k < tokenIds.length; k++) {
        ...
        // Reset the owner of the token id
    ...
    for (uint256 k = 0; k < _ownerUpdates.length; k++) {
      ...
      // save gas by not tracking tokens locked in polygonBridge 
      if (owner != polygonBridge) {
