...
contract ExchangePriceOracle is AccessControl {
    ...
    mapping(address => mapping(address => PriceRequest)) private _priceRequests;
    ...
    function priceRequests(
        address from,
        address to
    ) external view returns (PriceRequest memory) {
        if (uint160(from) > uint160(to)) {
            return _priceRequests[from][to];
        } else {
            PriceRequest memory request = _priceRequests[to][from];
            uint216 reversePrice = uint216(
                (10 ** (request.decimals * 2)) / uint256(request.result)
            );
            request.result = reversePrice;
            return request;
        }
    }
    ...
    function submitPrice(
        ...
        _priceRequests[fromToken][toToken] = PriceRequest(
            result,
            decimals,
