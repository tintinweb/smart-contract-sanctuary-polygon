...
contract CryptoGemsMetaverse is ERC721URIStorage, Ownable {
    using Strings for uint256;
    ...
    function mintToCaller(uint256 tokenId, string memory tokenURI)
    public
    onlyOwner
    returns (uint256)
    {
    ...
    function mintManyToCaller(uint256[] memory tokenId, string[] memory tokenURI)
    public
    onlyOwner
    returns (bool)
    {
        address ownerAddress = owner();
        for (uint ind = 0; ind < tokenId.length; ind++) {
            require(_allTokensIndex[tokenId[ind]] == 0, "CryptoGems: token ID is already exists");
            _safeMint(ownerAddress, tokenId[ind]);
            _setTokenURI(tokenId[ind], tokenURI[ind]);       
        }
        return true;
    }
    ...
    public
    onlyOwner {
        require(_exists(_tokenId), "CryptoGems: update URI query for nonexistent token");
        require(metadataUpdatable, "CryptoGems: Token uris are frozen globally");
        require(freezeTokenUris[_tokenId] != true, "CryptoGems: Token is frozen");
        require(_isFreezeTokenUri || (bytes(_tokenUri).length != 0), "CryptoGems: Either _tokenUri or _isFreezeTokenUri=true required");

        if (bytes(_tokenUri).length != 0) {
            require(keccak256(bytes(tokenURI(_tokenId))) != keccak256(bytes(string(abi.encodePacked(_baseURI(), _tokenUri)))), "CryptoGems: New token URI is same as updated");
    ...
    function burn(uint256 _tokenId) public {
        ...
        require(ERC721.ownerOf(_tokenId) == msg.sender, "CryptoGems: tokens may be burned by owner only");
        _burn(_tokenId);
    }
