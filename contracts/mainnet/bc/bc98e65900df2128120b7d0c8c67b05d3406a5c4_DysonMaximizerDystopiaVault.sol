...
import "../interfaces/Dystopia/maximizer/IMaximizerDystopia.sol";
import "../interfaces/Dystopia/maximizer/IBoostPool.sol";
 ...
 */
contract DysonMaximizerDystopiaVault is Initializable, OwnableUpgradeable, ERC20Upgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // The strategy currently in use by the vault.
    IMaximizerDystopia public strategy;
     ...
     */
    function __DysonMaximizerDystopiaVault_init(
        string memory _name,
        string memory _symbol) public initializer {
        __Ownable_init_unchained();
        __ERC20_init_unchained(_name, _symbol);
        __ReentrancyGuard_init_unchained();
        __DysonMaximizerDystopiaVault_init_unchained();
    }

    function __DysonMaximizerDystopiaVault_init_unchained() internal initializer {}
    ...
    function balanceBelongTo(address _address) public view returns (uint256){
        uint256 userBalance = balanceOf(_address);
        if (address(boostPool) != address(0)) {
            uint256 pid1 = boostPool.poolId1(address(this));
            if (pid1 != 0) {
                uint256 boostAmount;
                (boostAmount,) = boostPool.userInfo(pid1 - 1, _address);
    ...
    function withdraw(uint256 _shares) public nonReentrant {
        require(_shares <= balanceOf(msg.sender), "DysonMaximizerDystopiaVault::amount exceeds balance!");
        uint256 amount = strategy.withdraw(msg.sender, _shares);
        _burn(msg.sender, _shares);
        want().safeTransfer(msg.sender, amount);
    }

    function setStrategy(IMaximizerDystopia _strategy) public onlyOwner {
    ...
    function setBoostPool(address _address) public onlyOwner {
        boostPool = IBoostPool(_address);
    }
}
