...
contract SOMSOCGallery is ERC721, ERC721Enumerable, Ownable, AccessControl, ReentrancyGuard {

  using Strings
  for uint256;

  string public uriPrefix = "ipfs://QmeXFxvr8gbyZpkByxp5ydLwnxCxQi6qZRKtCHGFNQCbfE/";
  string public uriSuffix = ".json";
  uint256 public costMint = 1 ether;
  string internal royaltyURI = "https://harti.mypinata.cloud/ipfs/QmeXFxvr8gbyZpkByxp5ydLwnxCxQi6qZRKtCHGFNQCbfE/royalty.json";
  string internal tokenName;
  string internal tokenSymbol;
  uint256 public maxSupply;
  bool public paused;
  bool public locked = true;
  bytes32 internal constant ADMIN = keccak256("SOMSOC-ADMIN");
  bytes32 internal constant MINTER = keccak256("SOMSOC-MINTER");
  bytes32 internal constant PAUSE = keccak256("PAUSE");
  uint96 internal royaltyFee = 1000;
  ...
  function publicMint(address _to, uint256 tokenId, uint256 _mintAmount) public payable {
    require(!locked, "Uppss, we locked this function");
    require(_mintAmount > 0);
    require(tokenId <= maxSupply, "cannot mint more than maxSupply");
    if (msg.sender != owner()) {
      require(msg.value >= costMint * _mintAmount);
    }
  ...
  function minter(address _to, uint256 tokenId) public {
    require(hasRole(MINTER, msg.sender), "Sorry, you are not a Minter");
    require(!paused, "Uppss, we paused the function");
    require(tokenId <= maxSupply, "cannot mint more than maxSupply");
    _safeMint(_to, tokenId);
    mintedTokens.push(tokenId);
    _setTokenRoyalty(tokenId, royaltyAddress, royaltyFee);
  ...
  function bulkMint(address[] memory _to, uint256[] memory _id) public {
    require(hasRole(ADMIN, msg.sender), "Sorry, you are not an Admin");
    require(!paused, "Uppss, we paused the function");
    require(_to.length == _id.length, "Receivers and IDs are different length");
    for (uint256 i = 0; i < _to.length; i++) {
      _safeMint(_to[i], _id[i]);
      mintedTokens.push(_id[i]);
      _setTokenRoyalty(_id[i], royaltyAddress, royaltyFee);
    }
  }
  ...
  function setLocked(bool _state) public onlyRole(ADMIN) {
    locked = _state;
  }
  ...
  function setContractURI(string memory _setContractURI) public onlyRole(ADMIN) {
    royaltyURI = _setContractURI;
  }
  ...
  function royaltyInfo(
    ...
    require(checkMintedTokens(_tokenId), "Token not minted");
  ...
  function _setTokenRoyalty(
    ...
    fee = royaltyFee;
    require(fee <= 10000, "royalty is too high");
    require(receiver != address(0), "invalid address");
    _royalties[tokenId] = RoyaltyInfo(receiver, fee);
    emit Royalty(tokenId, receiver, uint256(fee));
  }

  function checkMintedTokens(uint256 tokenId) internal view returns(bool) {
