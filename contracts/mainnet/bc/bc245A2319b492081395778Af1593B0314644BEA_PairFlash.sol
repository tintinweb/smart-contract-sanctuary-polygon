...
contract PairFlash is Ownable,IUniswapV3FlashCallback, PeripheryImmutableState, PeripheryPayments {
    ...
    function uniswapV3FlashCallback(
        ...
        address token1 = decoded.poolKey.token0;
        address token2 = decoded.poolKey.token1;
        uint256 amount2 = decoded.amount2;
        uint256 amount3 = decoded.amount3;
        ...
        IERC20(token1).approve(lendingCore, amount);

        // Deposit 1000 DAI
        lendingPool.deposit(token1, amount,address(this), referral);        
        ...
        // Deposit 1000 DAI
        lendingPool.borrow(token2, amount,2,0, address(this));   

        }


        TransferHelper.safeApprove(token1, address(swapRouter), decoded.amount1);
        TransferHelper.safeApprove(token2, address(swapRouter), decoded.amount2);
        ...
        uint256 amount1Owed = LowGasSafeMath.add(decoded.amount1, fee0);
    ...
    //token0 is minimum
    //token1 is to deposit at AAVE and exchange from uniswap
    //token2 is to borrow from AAVE and exchange into uniswap
    //amount3 is to deposit AAVE
    ...
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token1, token1: params.token2, fee: params.fee2});
        IUniswapV3Pool pool = IUniswapV3Pool(address(params.pool));
