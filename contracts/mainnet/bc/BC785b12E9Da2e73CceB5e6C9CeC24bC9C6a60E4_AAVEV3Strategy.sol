...
contract AAVEV3Strategy is StrategyBase {
    ...
    mapping(address => bool) public amTokens;
    ...
    */
    function rebalanceTokenAAVE(address _token) external onlyRebalancer {
        ...
        // Account for small rounding errors that could occur 
        uint256 interest = ((IERC20(tokenToAm[_token]).balanceOf(address(this)) - totalDeposits[_token]) * 99) / 100;
    ...
    */
    function rebalanceToken(address _token) external onlyRebalancer {
        // amTokens are rebalanced using 'rebalanceTokenAAVE()'
        // Failure to check would allow the rebalancer to withdraw all amTokens held in
        // this contract since amTokens are not tracked under totalDeposits.
        require(!amTokens[_token], "CornFi AAVE Strategy: Call 'rebalanceTokenAAVE()'");
    ...
    */
    function _addToken(address _token, address _amToken) internal {
        ...
        // Set AAVE market token as active
        amTokens[_amToken] = true;
