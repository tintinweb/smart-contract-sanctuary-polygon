...
contract Ecoland is ERC721Enumerable, Ownable {
    ...
    uint256 constant TWENTY_ONE_MILLION = 21_000_000;
    uint256 public nextCollectionNumber = 1;
    string public customURI;

    constructor(string memory _customURI) ERC721("Ecoland", "ECO"){
    ...
    {   
        ...
        require(_maxTokensAvailable < TWENTY_ONE_MILLION, "The maximum number of tokens available per collection must be less than 1 million");
    ...
    function _mintNft(address _to, uint256 _collectionNumber) internal{
        uint256 tokenIdToMint = (_collectionNumber * TWENTY_ONE_MILLION) + (++collections[_collectionNumber].tokensMinted);
        _safeMint(_to, tokenIdToMint);
    }

    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        require(_exists(_tokenId), "ERC721Metadata: URI query for nonexistent token");
        string memory baseURI = collections[_tokenId/TWENTY_ONE_MILLION].baseURI;
        return bytes(baseURI).length > 0
    ...
    function getTokenIdsInCollection(uint256 _collectionNumber) public view returns(uint256[] memory tokenIds) {
        ...
        for(uint256 index=0; index < totalMinted; index++){
            tokenIds[index] = (_collectionNumber * TWENTY_ONE_MILLION) + index + 1;
        }
    }
}