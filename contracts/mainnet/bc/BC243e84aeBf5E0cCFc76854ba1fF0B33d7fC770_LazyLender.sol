...
contract LazyLender is PokeMeReady {
  ...
  mapping (uint32 => LendingParameters) gotchiUnchanneledLendingParameters;
  mapping (uint32 => LendingParameters) gotchiChanneledLendingParameters;
  ...
  constructor(address payable _pokeMe, address gotchiDiamond, address realmDiamond, address _gotchiOwner) PokeMeReady(_pokeMe) {
    ...
    gotchiUnchanneledLendingParameters[24428] = unchanneledRates;
    gotchiUnchanneledLendingParameters[11963] = unchanneledRates;
    gotchiUnchanneledLendingParameters[15296] = unchanneledRates;

    gotchiChanneledLendingParameters[24428] = channeledRates;
    gotchiChanneledLendingParameters[11963] = channeledRates;
    gotchiChanneledLendingParameters[15296] = channeledRates;
  }
  ...
  function setLendingParameters(uint32 _gotchiId, bool _unchanneled, uint96 _initialCost, uint32 _period, address[] calldata _revenueTokens, uint32 _whitelistId, uint8[3] calldata _revenueSplit, address _thirdParty) external {
    require(msg.sender == gotchiOwner);
    if (_unchanneled) {
      gotchiUnchanneledLendingParameters[_gotchiId] = LendingParameters(
        _initialCost,
        _period,
        _revenueTokens,
        _whitelistId,
      ...
      gotchiChanneledLendingParameters[_gotchiId] = LendingParameters(
        _initialCost,
        _period,
        _revenueTokens,
        _whitelistId,
  ...
  function lendGotchis() external onlyPokeMe {
    ...
    for (uint i = 0; i < gotchiIds.length; i++) {
      ...
      LendingParameters memory lendingParamaters = gotchiChanneledLendingParameters[gotchiId];
      uint gotchiLastChanneledDay = alchemicaFacet.getLastChanneled(gotchiId) / (60 * 60 * 24);
      uint currentDay = (block.timestamp + lendingParamaters.period) / (60 * 60 * 24);
      if (currentDay > gotchiLastChanneledDay) {
        lendingParamaters = gotchiUnchanneledLendingParameters[gotchiId];
      }

      if (!isListed) {
        AddGotchiListing memory addGotchiListing = AddGotchiListing(
          gotchiId,
          lendingParamaters.initialCost,
          lendingParamaters.period,
          lendingParamaters.revenueSplit,
          gotchiOwner,
          lendingParamaters.thirdParty,
          lendingParamaters.whitelistId,
          lendingParamaters.revenueTokens
        );
