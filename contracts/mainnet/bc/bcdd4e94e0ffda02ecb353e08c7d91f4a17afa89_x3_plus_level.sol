...
contract x3_plus_level is Ownable {
    ...
    event payForLevelOneEv(address ref, uint amt, address from, uint timeNow);
    function payForLevelOne(address _user) internal returns (bool){
        address payable ref = userAddressByID[userInfos[_user].referrerID];
        uint factor = 10 ** 18;
        ...
        for(uint i=0;i<10;i++)
        {
            ...
            emit payForLevelOneEv(ref, pay[i] * factor, _user, now);
    ...
    function updateNPayAutoPool(uint _level,address _user) internal returns (bool)
    {
        ...
        uint[6] memory dCount = [uint(1),uint(2),uint(4),uint(6),uint(8),uint(10)];
        for(uint i=0;i<6;i++)
        {
