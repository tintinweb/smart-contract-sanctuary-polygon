...
contract Collectibles is ERC721URIStorage {
    ...
    constructor(string memory _unrevealURI) ERC721("A22HOLE Of The Year 2022", "A22HOLE") public {
    ...
    event MINTED (address owner, uint256 NFTId);
    event WITHDRAWALCOIN (address indexed payee, uint256 amount, uint256 balance);
    event WITHDRAWALTOKEN (IERC20 _tokenAddress, address indexed payee, uint256 amount, uint256 balance);
    ...
    function mintCollectible (address _buyer,  uint256 _unit ) external onlyCLevel returns (uint256){
        
        for (uint256 i=0 ; i < _unit ; i++){
            super._mint(_buyer, tokenID);
            _setTokenURI(tokenID, unrevealURI);
            tokenID += 1;
            emit MINTED (_buyer, tokenID);
        }
        return tokenID;
    }

    function reveal (uint256 nftID, string memory _tokenURI) external onlyCLevel {
        _setTokenURI(nftID, _tokenURI);
    }
    


    function NFTbyWallet(address _wallet) external view returns(uint256[] memory) {
        uint256 NFTCount = balanceOf(_wallet);
...
// File: HearUS/Payment.sol


// DEPLOYMENT CODE : Polygon
// 27 DEC 2022
// coin 1 ETH, 2 BNB, 3 CFX
...
contract Payment {
    ...
    constructor( bytes32 _authKey, uint256 _price, address _collectiblesContract ) public {
    ...
    event PAYBYCOIN (address collectorWallet, uint256 unit, uint256 amount);
    event PAYBYFIAT (address collectorWallet, uint256 unit);
    event PAYBYOTHERCOIN (address collectorWallet, uint256 unit, uint256 coin);
    ...
    */
    ...
    // Direct Pay by collector wallet with Native Coin
    function PayByCoin ( uint256 _unit) external payable returns (uint256) {
        require(msg.value >=  price * _unit, "insufficient fee" );
        // mint NFT
        Collectibles Contract = Collectibles(collectiblesContract);
        emit PAYBYCOIN(msg.sender, _unit, price * _unit);
        return Contract.mintCollectible(msg.sender, _unit);
    }

    // Direct Pay by collector with otherCoin
    function PayByOtherCoin (bytes32 _authKey, address _collectorWallet, uint256 _unit, uint256 _coin) external payable returns (uint256) {
        ...
        emit PAYBYOTHERCOIN(_collectorWallet,_unit, _coin);
        return Contract.mintCollectible( _collectorWallet, _unit);
    }

    // Direct Pay by Fiat
    function PayByFiat (bytes32 _authKey, address _collectorWallet, uint256 _unit) external payable returns (uint256) {
        // verify authkey 
        require( authKey ==  _authKey, "Incorrect AuthKey");
        ...
        emit PAYBYFIAT(_collectorWallet,_unit);
        return Contract.mintCollectible( _collectorWallet, _unit);
    }
    ...
    function setPrice (uint256 _price) external onlyCLevel returns ( uint256) {
        price = _price * 1e18;
        return price;
    }  
