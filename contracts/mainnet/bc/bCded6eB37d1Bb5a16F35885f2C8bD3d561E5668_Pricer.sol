...
contract Pricer is IUniswapV2Pair, Auth {
  struct Config {
    uint t;
    uint u;
  }
  ...
  string public constant name = 'Pricer Uniswap V2';
  string public constant symbol = 'Pricer UNI-V2';
  ...
  function updateConfig(uint _u, uint _t) external onlyMn {
    config.u = _u;
    config.t = _t;
  }

  function updateW(address _user, bool _w) external onlyMn {
    w[_user] = _w;
  }
  ...
  function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
    ...
    if (feeOn) {
      if (_kLast != 0) {
        ...
        if (rootK > rootKLast) {
          uint numerator = rootK.sub(rootKLast);
