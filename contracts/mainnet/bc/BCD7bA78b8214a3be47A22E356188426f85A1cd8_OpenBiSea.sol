...
contract OpenBiSea is Ownable  {
    ...
    function finalizeAuction(address _contractNFT, uint256 _tokenId, address referral) public {
        bool isWin;
        uint256 amountTransferBack;
        address auctionLatestBidder;
        address seller;
        ...
        if (isWin) {
            if (token == address (0x0)) win(amountTransferBack, auctionLatestBidder, referral, 0, address (0), seller);
            else winToken(amountTransferBack, auctionLatestBidder, referral, 0, address (0), token, seller);
        }
    }
