...
contract vvGHST is Initializable, ERC20Upgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function initDystopia() public onlyApproved{

        //set the address for the balancer gauge
        address _dystopiaRouter = 0xbE75Dd16D029c6B32B7aD57A0FD9C1c20Dd2862e;
        s.dystopiaRouter = _dystopiaRouter;

        //approve the gauge to take our tokens
        IERC20Upgradeable(im_vQi).approve(_dystopiaRouter,MAX_INT);
        IERC20Upgradeable(im_Qi).approve(_dystopiaRouter,MAX_INT);
    }

    function dystopiaSwap(address _tokenA, address _tokenB) public onlyApproved{
        _dystopiaSwap(_tokenA,_tokenB);
    }

    function _dystopiaSwap(address _tokenA, address _tokenB) internal{
        ...
        uint256 balance = IERC20Upgradeable(_tokenA).balanceOf(address(this));

        Dystopia(s.dystopiaRouter).swapExactTokensForTokens(
            balance,   //    uint amountIn
            balance,   //    uint amountOutMin, -- we're only swapping when its favorable to do so, so we expect to get at least the same amount
            routes,    //    Route[] calldata routes,
            address(this), //    address to,
            block.timestamp+600 //    uint deadline -- 10 minutes
        );
