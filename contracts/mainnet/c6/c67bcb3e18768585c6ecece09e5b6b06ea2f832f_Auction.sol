...
contract Auction is ERC721Holder, Ownable {
    ...
    function bid(uint256 tokenId, uint256 amount) public {
        require(block.timestamp > auctionTimestampStarted[tokenId], "Auction not yet started");
        require(block.timestamp < auctionTimestampFinished[tokenId], "Auction already finished");
        require(currentPrice(tokenId) < amount, "Bid underpriced");
        require(amount >= currentPrice(tokenId) + 1*10^18);

        if(block.timestamp + finalCooldown > auctionTimestampFinished[tokenId]) {
            auctionTimestampFinished[tokenId] = block.timestamp + finalCooldown;
        }
    ...
    function finalize(uint256 tokenId) public {
        require(block.timestamp >= auctionTimestampFinished[tokenId], "Auction not yet finished");
