...
interface IPriceOracle {
    ...
    struct oracle {
        uint256[2] price0Cumulative;
        uint256[2] price1Cumulative;
        uint32[2] timeStamp;
        uint8 index; // 0 or 1
    }

    function getPrice(address pairAddress)
        external
        returns (
    ...
    function calculateMinAmount(
        ...
        uint256 amount,
        address pairAddress
    ) external returns (uint256 minAmount, uint256 timeTillValid);

    function getOracleTime(address pairAddress)
        external
        view
        returns (uint256 currentTimestamp, uint256 otherTimestamp);

    function priceValidStart() external view returns (uint256);

    function priceValidEnd() external view returns (uint256);
}
...
contract FeeManager is OwnableUpgradeable, UUPSUpgradeable {
    ...
    uint256 public buyBurnAllocation;

    uint256 public constant PERCENT_DECIMALS = 10000;

    address public exchangeOracle;

    address public incinerator; /// 0x4F6cCd1242323c23fEcf95b9C073C839db18649F
    ...
    function initialize(address _factory, address _incinerator)
        public
        initializer
    {
        __Ownable_init();
     ...
     * @param token1 the second token address of the swap pair
     **/
    function catalougeTokens(address token0, address token1)
        external
        onlyFactory
    {
     ...
     **/
    function validTimeWindow(address asset)
        external
        returns (uint256 valid, uint256 expiration)
    {
        ...
        (uint256 pairCurrentTime, ) = PriceOracle.getOracleTime(pairAddress);
    ...
    function oracleStepSwap(address asset, bool half) external onlyWhitelist {
        uint256 tokenBal = IERC20(asset).balanceOf(address(this));
        if (half) {
            tokenBal = tokenBal / 2;
        }
    ...
    function convertAsset(address asset, address[] memory _path)
        external
        onlyWhitelist
    {
        ...
        if (balance > 0) {
            ...
            if (asset == GFI) {
                IERC20(asset).approve(GRAVITY_ROUTER, balance);
                ...
                IERC20(asset).approve(QUICK_ROUTER, balance);

                (amounts, result) = swapTokens(QUICK_ROUTER, _path, balance, 0);
            }
        }
    }
    ...
    function deposit() external onlyWhitelist {
        ...
        IERC20 weth = IERC20(Factory.weth());
        IERC20 wbtc = IERC20(Factory.wbtc());
        ...
        if (amountWETH > 0) {
            ...
            if (buyBurnAllocation > 0) {
                amountBuyBurn =
                    (amountWETH * buyBurnAllocation) /
                    PERCENT_DECIMALS;

                weth.transfer(incinerator, amountBuyBurn);
            ...
            if (btcAllocation > 0) {
                ...
                uint256 amountBTCAllocation = (amountWETH * btcAllocation) /
                    PERCENT_DECIMALS;

                amountWETH -= amountBTCAllocation;

                IERC20(WETH).approve(QUICK_ROUTER, amountBTCAllocation);
            ...
            emit FeeDeposited(amountWETH, amountWBTC, amountBuyBurn);
        }
    }
    ...
    function updateIncinerator(address _incinerator) external onlyOwner {
        require(_incinerator != address(0), "wrong incinerator");
        incinerator = _incinerator;
    }

    function updateExchangeOracle(address _exchangeOracle) external onlyOwner {
        require(_exchangeOracle != address(0), "wrong oracle");
    ...
    function updateAllocation(
        ...
        uint256 _buyBurnAllocation
    ) external onlyOwner {
        require(
            _btcAllocation + _ethAllocation + _buyBurnAllocation ==
                PERCENT_DECIMALS,
            "worng allocation"
        );
        ...
        buyBurnAllocation = _buyBurnAllocation;
    }
}
