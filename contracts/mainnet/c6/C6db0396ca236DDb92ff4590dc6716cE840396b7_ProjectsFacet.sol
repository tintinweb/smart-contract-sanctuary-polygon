...
contract ProjectsFacet is Modifiers {
    ...
    struct CreateProjectArgsStruct {
        string displayName;
        string name;
        string imageCID;
        string description;
    ...
    struct UpdateProjectArgsStruct {
        uint256 projectId;
        string displayName;
        string name;
        string imageCID;
    ...
    struct UserProjects {
        ...
        string discord;
        bool isAdmin;
        uint256 keyChainTokenId;
    }

    //  ==========  project logic    ==========
    function createNewProject(CreateProjectArgsStruct calldata _args) external payable onlyTrustedForwarder {
        require(getProjectIdByName(_args.name) == 0, "ProjectsFacet: That name has been taken");
        ...
        s.projectMapping[_args.name] = projectId;
    ...
    function updateProject(UpdateProjectArgsStruct calldata _args) external payable onlyTrustedForwarder {
        uint256 projectId = _args.projectId;
        address msgSender = LibERC2771Context.msgSender();
        require(LibProjects.getAdminByProjectId(projectId, msgSender) == 1, "ProjectsFacet: Not authorized");

        ReturnProjectStruct memory currentProject = LibProjects.getProjectById(projectId);
        string memory currentName = currentProject.name;

        require(
            LibStrings.equalStrings(currentName, _args.name) || getProjectIdByName(_args.name) == 0,
            "ProjectsFacet: That name has been taken"
        );

        ++s.userExecutionCount[msgSender]["updateProject"];

        if (!LibStrings.equalStrings(currentProject.displayName, _args.displayName)) {
            s.projects[projectId].displayName = _args.displayName;
        }

        if (!LibStrings.equalStrings(currentName, _args.name)) {
            s.projects[projectId].name = _args.name;
            s.projectMapping[_args.name] = projectId;
            delete s.projectMapping[currentName];
        }

        if (bytes(_args.imageCID).length != 0 && !LibStrings.equalStrings(currentProject.imageCID, _args.imageCID)) {
            s.projects[projectId].imageCID = _args.imageCID;
        }

        if (!LibStrings.equalStrings(currentProject.description, _args.description)) {
            s.projects[projectId].description = _args.description;
        }

        if (!LibStrings.equalStrings(currentProject.website, _args.website)) {
            s.projects[projectId].website = _args.website;
        }

        if (!LibStrings.equalStrings(currentProject.twitter, _args.twitter)) {
            s.projects[projectId].twitter = _args.twitter;
        }

        if (!LibStrings.equalStrings(currentProject.discord, _args.discord)) {
            s.projects[projectId].discord = _args.discord;
        }
    }
    ...
    //  ==========  userProjects logic    ==========
    function getUserProjects(address _walletAddress) external view returns (UserProjects[] memory) {
        uint256[] memory projectIds = s.userProjects[_walletAddress];
        uint256 count = projectIds.length;
        UserProjects[] memory userProjects = new UserProjects[](count);

        for (uint256 i; i < count; ) {
            uint256 projectId = projectIds[i];
            ProjectStruct memory project = s.projects[projectId];
            UserProjects memory userProject = UserProjects({
                id: project.id,
                name: project.name,
                ...
                isAdmin: LibProjects.getAdminByProjectId(projectId, _walletAddress) == 1,
                keyChainTokenId: project.keyChainTokenId
            });
    ...
    function updateAdminByProjectId(
        ...
        bool _isAdmin
    ) external payable onlyTrustedForwarder {
