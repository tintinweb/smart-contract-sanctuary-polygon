...
contract KingOfDefiV0Perp {
    ...
    uint256 public constant initialVUSD = 100_000 * 1e18;
    ...
    function play() external {
        uint256 gw = gameWeek();
        require(!subscribed[gw][msg.sender], "already subscribed");
        subscribed[gw][msg.sender] = true;
        balances[gw][msg.sender][0] = initialVUSD; // 0 is VUSD
        unchecked{++numberOfPlayers[gw];}
        emit Subscribed(msg.sender, gw);
    }
    ...
    function swap(uint256 _fromIndex, uint256 _toIndex, uint256 _amount) external {
        uint256 gw = gameWeek();
        require(subscribed[gw][msg.sender], "player not subscribed");
        require(block.timestamp < (gw * 1 weeks) + 1 weeks - disputeDuration, "crown dispute period");
        ...
        require(balances[gw][msg.sender][_fromIndex] >= _amount, "amount not enough");
        ...
        // swap
        unchecked{balances[gw][msg.sender][_fromIndex] -= _amount;}
        balances[gw][msg.sender][_toIndex] += amountToBuy;

        // store the actual ts to manage the swap delay
        lastSwap[msg.sender] = block.timestamp;

        emit Swapped(gw, _fromIndex, _toIndex, _amount, amountToBuy);
    }
    ...
    function redeemPrize(uint256 _gameWeek, address _token, uint256 _amount) external {
        uint256 gw = gameWeek();
        require(_gameWeek < gw, "Week not elapsed");
        ...
        emit PrizeRedeemed(msg.sender, gw, _token, _amount);
    }
    ...
    function stealCrown() external {
        uint256 gw = gameWeek();
        require(block.timestamp > (gw * 1 weeks) + 1 weeks - disputeDuration, "only during dispute time");
        if (kings[gw] == address(0)) {
            kings[gw] = msg.sender;
            return;
        }
        uint256 actualKingUSD = calculateTotalUSD(kings[gw]);
        uint256 rivalUSD = calculateTotalUSD(msg.sender);
        if (rivalUSD > actualKingUSD) {
            emit NewKing(gw, kings[gw], msg.sender, actualKingUSD, rivalUSD);
            kings[gw] = msg.sender;
        }
    }
    ...
    function topUpPrize(address _token, uint256 _amount) external {
        ...
        prizes[gw][_token] += _amount;
        emit RewardAdded(gw, _token, _amount);
    }
    ...
    function balanceOfInUSD(address _player, uint256 _assetIndex) external view returns(uint256) {
        uint256 gw = gameWeek();
        uint256 amount = balances[gw][_player][_assetIndex];
    ...
    function calculateTotalUSD(address _player) public view returns(uint256) {
        ...
        usdTotalAmount += balances[gw][_player][0]; // v-usd
        for(uint256 index = 1; index < nextIndex;) {
            uint256 amount = balances[gw][_player][index];
            if (amount > 0) {
    ...
    /// @notice function to get the current game week
    function gameWeek() public view returns(uint256) {
