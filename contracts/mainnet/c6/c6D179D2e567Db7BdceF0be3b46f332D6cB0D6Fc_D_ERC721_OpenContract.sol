...
/**
 * @title D_ERC721_OpenContract
 * @dev This contract encapsulates all the logic for creation, modification, and destruction of ERC721 tokens. It's
 * designed to be used as a contract for managing unique tokens with variable metadata. Admin of this contract has the ability
 * to mint new tokens, update their metadata, and burn them.
 */
contract D_ERC721_OpenContract is Context, ERC165, IERC721, IERC721Metadata {
    ...
    /**
     * @dev Emitted when a new ERC721 token is minted.
     *
     * @param receiver The address that will receive the minted token.
     * @param uri The metadata URI associated with the minted token.
     * @param backendTokenKey A unique key provided by the backend to identify the token.
     * @param tokenId The unique identifier of the token as per the ERC721 standard.
     */
     ...
     *
     * @param tokenId The unique identifier of the token as per the ERC721 standard.
     * @param newUri The new metadata URI associated with the token.
    ...
    /**
     * @dev Mints a new unique ERC721 token with a specified URI.
     * Can only be called by the contract owner.
     *
     * Requirements:
     * - `account_` cannot be the zero address.
     * - `uri_` cannot be empty.
     * - `backendTokenKey_` must be unique and not associated with an existing token.
     *
     * @param account_ The address that will be assigned ownership of the newly minted token.
     * @param uri_ The metadata URI that will be associated with the newly minted token.
     * @param backendTokenKey_ A unique key provided by the backend to identify the token.
     *
    ...
    function mint(
        ...
        if (
            ...
            keccak256(abi.encodePacked((ds.contractUri)))
        ) {
            require(
                ds.existingUris[uri_] != true,
                "D_ERC721_OpenContract: NFT with such metadata already exist!"
            );
        }
        require(
        ...
        ds.tokenIdToBackendTokenKey[newTokenId] = backendTokenKey_;
    ...
    /**
     * @dev Updates the metadata URI of a specified token.
     ...
     * - `uri_` cannot be empty.
     *
     * @param tokenId_ The unique identifier of the token as per the ERC721 standard.
     * @param uri_ The new metadata URI that will be associated with the token.
     *
     * Emits a {NFTmetadataChanged} event.
     */
    function changeNFTMetadata(
        uint256 tokenId_,
        string memory uri_
    ) external onlyOwner {
        LibDiamond.ERC721Storage storage ds = _getERC721Storage();
        ...
        ds.tokens[tokenId_].uri = uri_;
        emit NFTmetadataChanged(tokenId_, uri_, ownerOf(tokenId_));
    }
    ...
    /**
     * @dev Changes the URI for the ERC721 contract.
     * This function can only be called by the owner of the contract.
     *
     * @param newURI_ the new URI to be set for the contract.
     *
     * @return The updated URI of the contract.
     */
    function changeMainURI(
        string memory newURI_
    ) external onlyOwner returns (string memory) {
        _getERC721Storage().contractUri = newURI_;
    ...
    /**
     * @dev Returns backend key of provided token.
     */
    function getBackendKeyOfToken(
        uint256 tokenId_
    ) external view returns (string memory) {
        LibDiamond.ERC721Storage storage ds = _getERC721Storage();
        return ds.tokenIdToBackendTokenKey[tokenId_];
    }
