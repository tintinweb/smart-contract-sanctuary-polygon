    ...
    ReentrancyGuardUpgradeable
{
    ...
    event BitflixSet(address newBitflix);
    ...
    function initialize(address bitflix_, uint256 gracePeriod_)
        public
        initializer
    {
        bitflix = bitflix_;
    ...
    ) external onlyCreator returns (address collection) {
        collection = address(new Collection(msg.sender, isStar, name, symbol));
    ...
    function createListingFromCollection(
        ...
        require(isCollection[msg.sender], "Can only be called by collection");
    ...
    function buyETH(uint256 listingId) external payable nonReentrant {
        require(
            address(listings[listingId].currency) == address(0),
            "Listing isn't for ETH"
        );
        require(
            msg.value >= listings[listingId].minimalBid,
            "Not enough ETH passed"
        );
    ...
    function bid(uint256 listingId, uint256 amount) external nonReentrant {
        uint256 totalBid = amount +
            listings[listingId].pendingAmounts[msg.sender];
    ...
    function bidETH(uint256 listingId) external payable nonReentrant {
        ...
        uint256 totalBid = msg.value +
            listings[listingId].pendingAmounts[msg.sender];
        listings[listingId].pendingAmounts[msg.sender] = 0;
        _bid(listingId, totalBid);
    }

    function claimCollectible(uint256 listingId) external nonReentrant {
        require(
            block.timestamp >= listings[listingId].timeEnd,
            "Sale hasn't finished yet"
        );
        ...
        address target;
        if (
            listings[listingId].listingType == ListingType.Auction &&
            listings[listingId].lastBidder != address(0)
        ) {
        ...
        listings[listingId].collection.transferFrom(
            address(this),
            target,
    ...
    function setCollectionCreator(address account, bool creator)
        external
        onlyOwner
    {
        require(
    ...
    function setCurrency(
        ...
        currencies[currency] = Currency({enabled: enabled, bidStep: bidStep});
    ...
    function setBitflix(address bitflix_) external onlyOwner {
        require(bitflix_ != address(0), "Bitflix can't be zero address");
        bitflix = bitflix_;

        emit BitflixSet(bitflix);
    }
    ...
    function setCollection(address _collection, bool _status)
        external
        onlyOwner
    {
    ...
    function _createListing(
        ...
        require(
            currencies[address(currency)].enabled,
        ...
        listings[_lastListingId].isFirstListing = (tokenToLastListing[
            address(collection)
        ][tokenId] == 0);
    ...
    function _buy(uint256 listingId) private {
        require(
            listings[listingId].listingType == ListingType.FixedPrice,
            "Buys are only for fixed price"
        );
        require(
            block.timestamp >= listings[listingId].timeStart,
        ...
        require(
            amount >=
                listings[listingId].lastBid +
                    currencies[address(listings[listingId].currency)].bidStep,
            "Bid should be not less than previous plus bid step"
        );
        require(
            amount >= listings[listingId].minimalBid,
            "Bid should not be less than minimal bid"
        );
    ...
    function _distributeValue(uint256 listingId) private {
        uint256 artistValue;
        uint256 celebValue;
        uint256 bitflixValue;
        if (listings[listingId].isFirstListing) {
            if (listings[listingId].collection.isStar()) {
                artistValue = (listings[listingId].lastBid * 40) / 100;
                celebValue = (listings[listingId].lastBid * 50) / 100;
                bitflixValue = (listings[listingId].lastBid * 10) / 100;
            } else {
                artistValue = (listings[listingId].lastBid * 90) / 100;
                bitflixValue = (listings[listingId].lastBid * 10) / 100;
            }
        } else {
            if (listings[listingId].collection.isStar()) {
                artistValue = (listings[listingId].lastBid * 5) / 100;
                celebValue = (listings[listingId].lastBid * 25) / 1000;
                bitflixValue = (listings[listingId].lastBid * 25) / 1000;
            } else {
                artistValue = (listings[listingId].lastBid * 5) / 100;
                bitflixValue = (listings[listingId].lastBid * 5) / 100;
            }
        }
        _transferCurrency(
            listingId,
            listings[listingId].collection.artists(listings[listingId].tokenId),
            artistValue
        );
        _transferCurrency(
            listingId,
            listings[listingId].collection.celebs(listings[listingId].tokenId),
            celebValue
        );
        _transferCurrency(listingId, bitflix, bitflixValue);
        _transferCurrency(
            listingId,
    ...
    modifier onlyCreator() {
        require(
            collectionCreators[msg.sender],
            "Caller isn't a collection creator"
        );
        _;
    }
}
...
interface ICollection is IERC721 {
    function isStar() external view returns (bool);

    function artists(uint256 tokenId) external view returns (address);

    function celebs(uint256 tokenId) external view returns (address);
}
...
contract Collection is ERC721URIStorage, Ownable {
    ...
    mapping(uint256 => address) public celebs;
    ...
    function setGalleryBridge(address _bridge) external onlyOwner {
        galleryBridge = _bridge;
    }
    ...
    function _mintToken(
        ...
        artists[_lastTokenId] = artist;
        celebs[_lastTokenId] = celeb;
        bool claimed = mintTo != address(gallery);
        gallery.createListingFromCollection(
            listingType,
            mintTo,
            _lastTokenId,
            timeStart,
        ...
        emit TokenMinted(_lastTokenId, artist, celeb, tokenURI);
    ...
    {
        require(
            !gallery.isBlacklisted(operator),
            "Impossible to approve blacklisted address"
        );
    ...
    function _transfer(
        ...
        require(
            !gallery.isBlacklisted(to),
            "Impossible to transfer to blacklisted address"
        );
        require(
            isGalleryTransfer(from, to, address(gallery)) ||
                isGalleryTransfer(from, to, galleryBridge),
            "Transfer is allowed only over gallery"
        );
        super._transfer(from, to, tokenId);
    }

    function isGalleryTransfer(
        address _from,
        address _to,
        address _gallery
    ) private pure returns (bool) {
        if (_from != _gallery && _to == address(_gallery)) {
            return true;
        }
        if (_from == _gallery) {
            return true;
