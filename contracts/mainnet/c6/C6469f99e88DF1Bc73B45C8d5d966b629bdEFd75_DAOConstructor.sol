...
contract AdminController is App,
__with_semver(
    uint8(1),
    uint8(0),
    uint8(0)
    ...
    modifier onlyAdmin() {
        require(_isAdmin(msg.sender), Exceptions.INVALID_AUTHORIZATION_ERROR);
        _;
    }

    modifier onlyAdminOrSudo() {
        require(_isAdmin(msg.sender) || _hasPermission(SUDO), Exceptions.INVALID_AUTHORIZATION_ERROR);
        _;
    }

    function initialize(IKernel kernel, address[] calldata admins) external initializer {
        __App_init(kernel);
    ...
    function mint(address to, bytes32 tierValue) external onlyAdminOrSudo {
        ERC721Properties(_kernel.getAppAddress(AppsIds.ERC721)).mint(to, tierValue);
    }

    function batchMint(address[] calldata to, bytes32[] calldata tierValues) external onlyAdminOrSudo {
        for (uint256 i = 0; i < to.length; i++) {
            ERC721Properties(_kernel.getAppAddress(AppsIds.ERC721)).mint(to[i], tierValues[i]);
        }
    }

    function burn(uint32 tokenId) external onlyAdminOrSudo {
        ERC721Properties(_kernel.getAppAddress(AppsIds.ERC721)).burn(tokenId);
    }

    function batchBurn(uint32[] calldata tokenIds) external onlyAdminOrSudo {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ERC721Properties(_kernel.getAppAddress(AppsIds.ERC721)).burn(tokenIds[i]);
        }
    }

    function setBaseURI(string calldata uri) external onlyAdminOrSudo {
        ERC721Properties(_kernel.getAppAddress(AppsIds.ERC721)).setBaseURI(uri);
    }
    ...
    function openSale__setPaymentPolicy(bytes32[] calldata tierValues, uint256[] calldata tierPrices_)
        ...
        onlyAdminOrSudo
    {
        ERC721OpenSale(_kernel.getAppAddress(AppsIds.ERC721_OPEN_SALE)).setPaymentPolicy(tierValues, tierPrices_);
    }
    ...
    function whitelistSale__setPaymentPolicy(bytes32[] calldata tierValues, uint256[] calldata tierPrices_)
        ...
        onlyAdminOrSudo
    {
        ERC721WhitelistSale(_kernel.getAppAddress(AppsIds.ERC721_WHITELIST_SALE)).setPaymentPolicy(
            tierValues,
            tierPrices_
        );
    }
    ...
    function whitelistSale__setMerkleTree(bytes32 merkleRoot, bytes calldata merkleProofIpfsHash)
        ...
        onlyAdminOrSudo
    {
        ERC721WhitelistSale(_kernel.getAppAddress(AppsIds.ERC721_WHITELIST_SALE)).setMerkleTree(
            merkleRoot,
            merkleProofIpfsHash
        );
    }
...
__with_semver(
    ...
    uint8 immutable public CONTROLLER = _initNextRole();
    ...
    function setRandomMint(bytes32 tierValue, uint256 tokenCount, uint256 artworksCount) external requirePermission(CONTROLLER) {
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && to != address(0)) {
            ...
            uint256 isTransferable = uint256(_getAttribute(
                PROPERTY_TIER,
                propValue,
                ATTRIBUTE_IS_TRANSFERABLE
            ));
            uint256 unlockTransferTimestamp = uint256(_getAttribute(
                PROPERTY_TIER,
                propValue,
                ATTRIBUTE_UNLOCKS_AT_HOURS
            ));
    ...
    function _setAttribute(
        ...
        internal 
    {
        _attributesByPropertyKey[_calculateKeyHash(propKey)][propValue]
            .attributes[_calculateKeyHash(attrKey)] = attrValue;
...
contract ERC721BaseSale is App {
    ...
    function _claim() internal {
        if (claimLimit != 0) {
            require(claims[msg.sender] > claimLimit, "Invalid amount of NFTs for this address");
        }
        claims[msg.sender]++;
    }

    function _transferToTreasury() internal {
        address wallet = _kernel.getAppAddress(AppsIds.TREASURY);
...
contract ERC721OpenSale is ERC721BaseSale,
__with_semver(
    uint8(1),
    uint8(0),
    uint8(0)
) {
    function initialize(IKernel kernel, uint64 claimLimit_) external initializer {
        __App_init(kernel);
        claimLimit = claimLimit_;
    }
...
contract ERC721WhitelistSale is ERC721BaseSale,
__with_semver(
    uint8(1),
    uint8(0),
    uint8(0)
    ...
    function buy(bytes32[] calldata proof, bytes32 tierValue) external payable {
        ...
        bool anyTier = MerkleProof.verify(proof, _merkleRoot, keccak256(abi.encodePacked(msg.sender)));
        if (!anyTier) {
            require(
                MerkleProof.verify(proof, _merkleRoot, keccak256(abi.encodePacked(msg.sender, tierValue))),
        ...
        _claim();
        _transferToTreasury();
        ERC721Properties(_kernel.getAppAddress(AppsIds.ERC721)).mint(msg.sender, tierValue);
    }
...
abstract contract __with_semver {
    uint8 immutable private __semver_major;
    uint8 immutable private __semver_minor;
    uint8 immutable private __semver_patch;
    ...
    function __semver() external view returns (string memory) {
        return string(
            abi.encodePacked(
                "v", __semver_major, ".", __semver_minor, ".", __semver_patch
            )
        );
    }
}
...
contract DAOConstructor is Ownable, __with_semver(
    uint8(1),
    uint8(0),
    uint8(0)
) {
    ...
    constructor(IUpdateManager updateManagerProxy_) {
    ...
    function deploy(
        ...
        kernel.initialize(updateManager, address(this));
