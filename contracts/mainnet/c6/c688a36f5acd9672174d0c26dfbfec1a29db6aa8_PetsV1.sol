...
import "./shared/CosTokenUpgradeable.sol";

contract PetsV1 is CosTokenUpgradeable {
    ...
    function initialize() public initializer {
        __CosToken_init("Clash of Streamers - Pets", "PET");
    }

    function _baseURI() internal pure override returns (string memory) {
        return "https://api.cos-nft.com/api/opensea/v2/pets/";
    }
    ...
    function _packCollectibleSpecific(
        ...
        assert(unpacked.shinyHue <= type(uint8).max);
...
abstract contract CosTokenUpgradeable is ERC721Upgradeable, UUPSUpgradeable {
    /// @custom:oz-upgrades-unsafe-allow state-variable-immutable
    IDubiV1Minimal private immutable _dubi;
    ...
    {
        require(dubi != address(0), "COS_TOKEN: bad initialize");

        _dubi = IDubiV1Minimal(dubi);
    }

    address private _secondaryMinter;

    mapping(uint96 => PackedCollectible) internal _packedCollectibles;
    mapping(uint96 => mapping(bytes32 => bytes32)) internal _extraAttributes;

    modifier onlyOwnerOrSecondaryMinter() {
        address caller = _msgSender();
        require(
            _secondaryMinter == caller || owner() == caller,
            "COS: bad caller"
        );
        _;
    }
    ...
    event Killed(uint96 tokenId, address owner);
    ...
    event SetExtraAttributes(uint96 tokenId, bytes32[] keys, bytes32[] values);
    event Fraudulent(uint96 tokenId, bool active);

    function __CosToken_init(string memory name_, string memory symbol_)
        internal
        onlyInitializing
    {
        __ERC721_init_unchained(name_, symbol_);
        _secondaryMinter = _msgSender();
    }
    ...
    {
        require(_exists(tokenId), "COS: bad token");

        UnpackedCollectible
            memory unpackedCollectible = _getUnpackedCollectible(tokenId);

        unpackedCollectible.isFraud = active;
        _packedCollectibles[tokenId].packedData = _packCollectible(
            unpackedCollectible
        );

        emit Fraudulent(tokenId, active);
    }
    ...
    function setExtraAttributes(
        ...
        require(_exists(tokenId), "COS: bad token");
        require(keys.length == values.length, "COS: bad input");

        for (uint256 i = 0; i < keys.length; i++) {
            _extraAttributes[tokenId][keys[i]] = values[i];
        }

        emit SetExtraAttributes(tokenId, keys, values);
    }
    ...
    {
        require(_exists(tokenId), "COS: bad token");
        return _extraAttributes[tokenId][key];
    }
    ...
    function batchMint(
        ...
        // Unused - only part of the function call to store arbitrary messages on-chain
    ...
    function ownerMint(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _mint(
                ...
                empty,
                payloads[i],
                true // NOTE: The owner can mint collectibles with DUBI!
            );
        }
    }
    ...
    function _mint(
        ...
        address tokenOwner,
        uint256 packedData,
        bytes32[] memory extraKeys,
        bytes32[] memory extraValues,
        // Unused - only part of the function signature to have it on-chain
        bytes calldata payload,
        // true when migrating; can only be used by the contract owner!
        bool isMigrating
    ) private {
        require(tokenOwner != address(0), "COS: bad tokenOwner");
        require(tokenId > 0, "COS: bad tokenId");
        require(packedData > 0, "COS: empty data");
        require(extraKeys.length == extraValues.length, "COS: bad extra");
        require(_neverExisted(tokenId), "COS: duplicate");
        require(payload.length > 0, "COS: empty payload");

        if (!isMigrating) {
            require(uint96(packedData >> 24) == 0, "COS: mint contains DUBI");
        }
        ...
        if (extraKeys.length > 0) {
            mapping(bytes32 => bytes32) storage _extra = _extraAttributes[
                tokenId
            ];
    ...
    function empower(uint96 tokenId, uint96 amount) external {
        require(amount > 0, "COS: bad amount");

        address owner = ownerOf(tokenId);

        address funder = _msgSender();

        bool success = _dubi.transferFrom(funder, address(this), amount);
        require(success, "COS: bad dubi transfer");
    ...
    function merge(uint96 tokenIdSource, uint96 tokenIdTarget) external {
        ...
        address caller = _msgSender();
        require(tokenIdSource != tokenIdTarget, "COS: same token");
        require(
            _isApprovedOrOwner(caller, tokenIdSource) &&
                _isApprovedOrOwner(caller, tokenIdTarget),
            "COS: neither owner nor approved"
        );
        require(
        ...
        // Stay fraud if already a fraud, else inherit from source
        if (!unpackedTarget.isFraud) {
            unpackedTarget.isFraud = unpackedSource.isFraud;
        }

        // The actual collectible may have additional merge specific logic
        _applyMerge(unpackedSource, unpackedTarget);

        // Pick source abilities if compatible
        if (
            unpackedSource.class == unpackedTarget.class &&
            unpackedSource.faction == unpackedTarget.faction
        ) {
            unpackedTarget.abilities = unpackedSource.abilities;
        }

        // Burn the source token without refunding empoweredDUBI, since it moved to the `target`.
        _burn(tokenIdSource);
        ...
        // We can infer that a token got killed when the tokenId no longer exists, but the packedData is > 0
        // _packedCollectibles[tokenIdSource].packedData = _packCollectible(unpackedSource)
        _packedCollectibles[tokenIdTarget].packedData = _packCollectible(
            unpackedTarget
        );

        // Emit
        emit Merged(tokenIdTarget, tokenIdSource, caller);
    }

    function kill(uint96 tokenId) external {
        address tokenOwner = ownerOf(tokenId);
        require(tokenOwner == _msgSender(), "COS: caller is not owner");

        UnpackedCollectible
            memory unpackedCollectible = _getUnpackedCollectible(tokenId);

        _burn(tokenId);
        if (unpackedCollectible.empoweredDUBI > 0) {
            require(
                _dubi.transfer(tokenOwner, unpackedCollectible.empoweredDUBI),
                "COS: bad dubi transfer"
            );
        }
        ...
        emit Killed(tokenId, tokenOwner);
    }

    // MISC

    // The packed collectible makes heavy use of bit packing to minimize storage costs.
        ...
        returns (uint256)
    {
        ...
        assert(_unpackedCollectible.headIdAlias <= type(uint24).max);
        assert(_unpackedCollectible.empoweredDUBI <= type(uint96).max);
        assert(_unpackedCollectible.season <= type(uint32).max);
        assert(_unpackedCollectible.abilities <= type(uint32).max);
        assert(_unpackedCollectible.stars <= type(uint8).max);
        assert(_unpackedCollectible.level <= type(uint8).max);
        assert(_unpackedCollectible.faction <= type(uint8).max / 2);
        ...
        // Ensure that the faction only uses 4 bits by masking the faction with a bitmask where the first 4 bits are 1 (00001111)
    ...
    IERC721MetadataUpgradeable
{
    ...
    {
        require(_exists(tokenId), "ERC721: transfer to zero address");
    ...
    {
        return
            _skipAllowance(msg.sender) || _operatorApprovals[owner][operator];
    }
    ...
    function _transfer(
        ...
        require(
            ...
            "ERC721: transfer from bad owner"
        );
    ...
    function _checkOnERC721Received(
        ...
        if (to.isContract()) {
            ...
            returns (bytes4 retval) {
                ...
                if (reason.length == 0) {
                    revert("ERC721: bad ERC721Receiver");
                } else {
                    assembly {
