...
contract SwitchCelerSenderPolygon is SwitchPolygon {
    ...
    function swapByCeler(
        ...
        bytes calldata srcParaswapData
    )
        external
        payable
        returns (bytes32 transferId)
    {
        ...
        if (transferArgs.srcSwap.srcToken == transferArgs.srcSwap.dstToken) {
            ...
            if (transferArgs.paraswapUsageStatus == uint(DataTypes.ParaswapUsageStatus.OnSrcChain)
                ...
                returnAmount = _swapFromParaswap(
                    transferArgs,
                    srcParaswapData,
                    amountAfterFee);
        ...
        returns (uint256 returnAmount)
    {
        ...
        returnAmount = _swapInternalWithParaSwap(IERC20(swapArgs.srcSwap.srcToken), IERC20(swapArgs.srcSwap.dstToken), amount, callData);
    }
