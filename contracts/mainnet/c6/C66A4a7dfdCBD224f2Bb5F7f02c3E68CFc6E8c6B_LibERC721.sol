...
library LibERC721 {
    ...
    bytes32 public constant ERC721_STORAGE_POSITION = keccak256("diamond.standard.erc721.storage");
    ...
    function erc721Storage() internal pure returns (ERC721Storage storage ds) {
        bytes32 slot = ERC721_STORAGE_POSITION;
        assembly {
            ds.slot := slot
        }
    }
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ERC721Storage storage s = erc721Storage();
        require(s.owners[tokenId] != address(0), "ERC721Metadata: URI query for nonexistent token");
    ...
    function getApproved(uint256 tokenId) internal view returns (address) {
        ERC721Storage storage s = erc721Storage();
        require(s.owners[tokenId] != address(0), "ERC721: approved query for nonexistent token");
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view returns (bool) {
        ERC721Storage storage s = erc721Storage();
        require(s.owners[tokenId] != address(0), "ERC721: operator query for nonexistent token");
        address owner = ownerOf(tokenId);
