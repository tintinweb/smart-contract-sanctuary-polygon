...
contract NxnxNftFarming is Ownable, IERC721Receiver, IERC1155Receiver {
    ...
    // for debug
    event DebugMessage(address indexed account, uint256 amount, string message);
    ...
    function getUserEtmAmount(address _account) public  returns (uint256) {
    ...
    function getEtmBalance(
        ...
        emit DebugMessage(_msgSender(), boostMultiplier, "Main NFT Boost");
        ...
        if (boostMultiplier > 10000) {
            ...
            for (uint i=0; i<_itemLength; i++) {
                if (extraBoosts[_nftItems[i].nftAddress][_nftItems[i].nftTokenId].boostPercent > 0) {
                    ...
                    emit DebugMessage(_msgSender(), boostMultiplier, "Sub NFT Boost");
                }
            }
        }
    ...
    function depositNft(
        ...
        // debug
        emit DebugMessage(_msgSender(), _beforeAmount, "beforeAmount");
        ...
        // debug
        emit DebugMessage(_msgSender(), etmAmount, "etmAmount");
    ...
    function withdrawSubNft() private {
        UserInfo storage user = userInfo[_msgSender()];
        for (uint i=0; i<user.nftItems.length; i++) {
            if (user.nftItems[i].isERC721) {
                IERC721(user.nftItems[i].nftAddress).safeTransferFrom(
                    address(this),
                    _msgSender(),
                    user.nftItems[i].nftTokenId
                );
            } else {
                IERC1155(user.nftItems[i].nftAddress).safeTransferFrom(
                    address(this),
                    _msgSender(),
                    user.nftItems[i].nftTokenId,
                    1,
            ...
            emit NftWithdrawn(
                _msgSender(),
                user.nftItems[i].isERC721,
                user.nftItems[i].nftAddress,
                user.nftItems[i].nftTokenId
            );
