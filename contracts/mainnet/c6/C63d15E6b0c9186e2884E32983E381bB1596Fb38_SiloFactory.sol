...
// File contracts/DeFi/Vaults/Silo.sol
...
contract Silo is Initializable, KeeperCompatibleInterface{
    ...
    function Deposit() external onlyFactory{
        ...
        if(!inStrategy){//first time running deposit
            require(gas - gasleft() <= 2000000, "Silo uses too much gas for keepers");
        }
        inStrategy = true;
    }
    ...
    function Withdraw() external onlyFactory{
        if(inStrategy){//only withdraw if silo is in a strategy
            ...
            for(uint i=0; i<strategy.length; i++){
                ...
                if(!success){
                    uint[4] memory tmp;
                    bytes memory data = abi.encode(tmp); //things like amounts
                    exitData = data;
                    emit StrategyFailed(i);
                }
                exitData = result;
            }
            inStrategy = false;
    ...
    function setStrategy(address[4] memory _inputs, bytes[] memory _configurationData, address[] memory _implementations) external onlyFactory{
        ...
        for(uint i=0; i< _implementations.length; i++){
            ...
            if(storedConfig.length > 0){
                ...
                require(IAction(_implementations[i]).validateConfig(_configurationData[i]), "User defined configuration not valid");
            }
...
// File contracts/DeFi/Vaults/SiloFactory.sol
...
contract SiloFactory is Ownable, ERC721Enumerable{
    ...
    mapping(uint => address[4]) strategyInputs;
    mapping(uint => bytes[]) strategyConfigurationData;
    mapping(uint => address[]) strategyImplementations;
    mapping(string => uint) strategyName;
    ...
    function Deposit(uint[4] memory amount, uint siloID) external isOwner(siloID){
    ...
    function Withdraw(uint siloID) external isOwner(siloID){
        ISilo silo = ISilo(siloMap[siloID]);
        silo.Withdraw();
    }

    function RemoveFunds(uint siloID) external isOwner(siloID){
        ISilo silo = ISilo(siloMap[siloID]);
    ...
    function WithdrawToken( uint siloID, address token) external isOwner(siloID){
        ISilo silo = ISilo(siloMap[siloID]);
    ...
    function setActionStack(uint siloID, address[4] memory input, address[] memory _implementations, bytes[] memory _configurationData) external isOwner(siloID){
        ISilo silo = ISilo(siloMap[siloID]);
        silo.setStrategy(input, _configurationData, _implementations);
    }


    function createConfiguration() external view returns(bytes memory){
        //helper function useful for front end to make configuration data
    }
    ...
    function SiloDelay(uint siloID) external view returns(uint){
        return ISilo(siloMap[siloID]).siloDelay();
    }

    function LastTimeMaintained(uint siloID) external view returns(uint){
        return ISilo(siloMap[siloID]).lastTimeMaintained();
    }

    function timeToNextMaintain(uint siloID) external view returns(uint time){
    ...
    //TODO funciton that lets users change _ratio, _recipients, _siloName, _siloDelay
    //TODO maybe I can make special contracts the interact with the factory and are designed to deploy specific silo  strategies? People could send their Silo Token to it, it would change their strategy, then it would send it back in 1 TX?
}