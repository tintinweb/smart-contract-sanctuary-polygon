...
contract ElegantLeague is Ownable, Concatenate {
    ...
    function addRacers(address _sender, address _con, uint[] calldata _id, uint[] calldata _ev) external { 
        ...
        _temp.count = getOwnerStakedCount(_sender);
        require(_temp.count + _count <= 5, "Too many racers! You can only use up to 5.");
        ...
        for(uint i=0; i<_count; i++){
            ...
            bool answer = RulesInterface(rulesByRace[_temp.tempEvent].conCheck).tokenCheck(_temp.tempID,rulesByRace[_temp.tempEvent].conValue,_temp.tempCon);
        }
    }
    ...
    function returnTokens(uint256[] memory _tokenID) public {
        ...
        for( uint i = 0; i < _racers.length; i ++ ) {
            ...
            require(_owner == msg.sender || isTeamMember[msg.sender] == true , "One of those NFTs does not belong to you.");
    ...
    function getBoost(uint _id, bool _ext) public view returns(uint) {
        StakingInterface _staking = StakingInterface(stakingAddress);
        uint _week = _staking.getWeek();
    ...
    function buildRace(uint _event) internal {
        ...
        for(i; i<racing; i++){ //go through every racer
            ...
            //boosts.boost = _staking.getBoost(_temp.tempOwner,_temp.tempRID);
            boosts.boost = boosts.boost + getBoost(_temp.tempID,false);
            if (boosts.coach != 0){
            ...
            _temp.newN = randomNumber(_nonce, 7, 10);
            
            perfByRacer[_temp.week][_temp.tempID].firstLeg = _temp.newN + boosts.boost;
            _nonce = _nonce + 1;
            _temp.newN = randomNumber(_nonce, 8, 16);
                        
            perfByRacer[_temp.week][_temp.tempID].secondLeg = _temp.newN + boosts.boost;
            perfByRacer[_temp.week][_temp.tempID].steps = returnSteps(perfByRacer[_temp.week][_temp.tempID].firstLeg,perfByRacer[_temp.week][_temp.tempID].secondLeg);
