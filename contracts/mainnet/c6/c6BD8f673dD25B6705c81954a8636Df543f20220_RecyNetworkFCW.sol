...
interface FCW {
    ...
    function revokeStatus(uint256 tokenId) external;
    ...
    function getReport(
        uint256 statusID,
        bytes32 yearMonth
    ) external view returns (FCWLibrary.Report memory);
    ...
    function getReportSignatures(
        uint256 statusID,
        bytes32 yearMonth
    ) external view returns (FCWLibrary.Signature[] memory);
...
contract NFTMetadata is ERC721, ERC721Burnable, MetadataStorage {
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public virtual view override(ERC721) returns (bool) {
...
contract RecyNetworkFCW is NFTMetadata, FCW {
    using ECDSA for bytes32;

    // id status => year month => report
    mapping(uint256 => mapping(bytes32 => FCWLibrary.Report)) reports;
    // id status => year month => signatures
    mapping(uint256 => mapping(bytes32 => FCWLibrary.Signature[])) reportSignatures;
    ...
    function createStatus(
        ...
        MetadataLibrary.Property[] memory properties,
        MetadataLibrary.Attribute[] memory attributes
    ) public virtual override(FCW) onlyOwner {
        safeMintMetadata(to, tokenId, properties, attributes);
    }

    function revokeStatus(uint256 tokenId) external override {
        revoke(tokenId);
    }
    ...
    function addReport(
        ...
        FCWLibrary.Report memory report = FCWLibrary.Report(
            yearMonth,
            url,
            block.timestamp
        );
        FCWLibrary.Report memory reportExist = reports[statusID][yearMonth];
    ...
    function getReport(
        uint256 statusID,
        bytes32 yearMonth
    ) public view override returns (FCWLibrary.Report memory) {
        require(_exists(statusID), "RecyNetworkFCW: status not exist");
        FCWLibrary.Report memory reportExist = reports[statusID][yearMonth];
    ...
    function signReport(
        ...
        FCWLibrary.Signature memory sign = FCWLibrary.Signature(
            yearMonth,
            signature,
            nonce,
            block.timestamp,
    ...
    function getReportSignature(
        ...
        bytes32 yearMonth,
        address auditorAddress
    ) public view override returns (FCWLibrary.Signature memory) {
        require(_exists(statusID), "RecyNetworkFCW: status not exist");
        FCWLibrary.Report memory reportExist = reports[statusID][yearMonth];
        require(!isEmpty(reportExist.url), "RecyNetworkFCW: Report not exist");
        uint256 idx = indexSignatures[statusID][yearMonth][auditorAddress];
        // id status => year month => signatures
        FCWLibrary.Signature memory signature = reportSignatures[statusID][
            yearMonth
        ][idx];
        require(
    ...
    function getReportSignatures(
        ...
        require(!isEmpty(reportExist.url), "RecyNetworkFCW: Report not exist");
        // id status => year month => signatures
        FCWLibrary.Signature[] memory signatures = reportSignatures[statusID][
            yearMonth
        ];
        return signatures;
    }
    ...
    function supportsInterface(
        ...
        return
            interfaceId == type(FCW).interfaceId ||
            super.supportsInterface(interfaceId);
    }
}