...
contract InventoryMinter {
    ...
    function getOrder0() external view returns (OrderLib.Order memory) {
        return order0;
    }

    function getOrder1() external view returns (OrderLib.Order memory) {
        return order1;
    }
    ...
    constructor(address _token0, address _token1, uint256 _amount0, uint256 _amount1, int24 _tickLower, int24 _tickUpper, uint24 _fee, address _owner, int256 _price) {
        ...
        IERC20(token0).approve(address(positions), type(uint256).max);
        IERC20(token1).approve(address(positions), type(uint256).max);
    ...
    function _openLimitOrder(address _token0, address _token1, uint256 _tokenId, uint256 _amount0, uint256 _amount1) internal returns (OrderLib.Order memory order) {
        ...
        bytes32 DOMAIN_SEPARATOR = _routerDomainSeparator(router);
        bytes32 hash = order.hash(DOMAIN_SEPARATOR);
