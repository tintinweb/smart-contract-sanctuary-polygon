...
contract Greeter is AccessControl, RouterCrossTalk {
    ...
    constructor(address _genericHandler, IBridge _bridge) RouterCrossTalk(_genericHandler) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        bridge = _bridge;
    }

    // Admin Functions for Cross Talk Start
    function setBridge (IBridge _bridge) onlyRole(DEFAULT_ADMIN_ROLE) external {
        bridge = _bridge;
    }

    function setLinker ( address _addr ) onlyRole(DEFAULT_ADMIN_ROLE) external {
        setLink(_addr);
    }

    function setFeeTokens ( address _feeToken ) onlyRole(DEFAULT_ADMIN_ROLE) external {
        setFeeToken(_feeToken);
    }

    function setCrossChainGas ( uint256 _gas ) onlyRole(DEFAULT_ADMIN_ROLE) external {
        crossChainGas = _gas;
    }

    function fetchCrossChainGas ( ) view external returns (uint256) {
    ...
    function SendGreeter(uint8 _chainID , string memory _greeting, 
        bytes32 resourceID,
        bytes calldata data,
        ...
        bytes4 _selector = bytes4(keccak256("SetGreeterCrossChain(string)"));
        bytes memory _data = abi.encode( _greeting );
    ...
    function SetGreeterCrossChain( string memory _greeting ) external isSelf {
        greeting = _greeting;
    }

    function SetGreeter( string memory _greeting ) external {
        greeting = _greeting;
    }
