...
contract UserProfiles {
    ...
    */
    ...
    event logProfileUpdated(address indexed requester, string uri, string avatar, string handle, string location, string bio);
    ...
    */
    function updateContracts(address _followers, address _kutils) public onlyAdmins {
    ...
    */
    function updatePostCount(address posterAddress, uint16 postCount, bool add) public onlyAdmins {
        ...
        * @param contractAddress : the contract address they made a post into
    */
    function logPost(address posterAddress, address contractAddress, uint256 tipPerTag, address[] calldata tipReceivers) public onlyAdmins {

        // Require a valid address
        require(posterAddress != address(0), "Bad Poster Address");
        require(contractAddress != address(0), "Bad Contract Address");
        ...
        // Loop through each contract they have posted into
        for (uint i=0; i < usrProfileMap[posterAddress].contractsPostedIn.length; i++) {

            // If the token matches the one we want
            if (KUtils.stringsEqual(KUtils._toLower(KUtils.addressToString(usrProfileMap[posterAddress].contractsPostedIn[i])),KUtils._toLower(KUtils.addressToString(contractAddress)))){
                // This contract already added
                _added = true;
            }
        }

        // if this contract isn't added yet, add it to the array
        if (!_added){
            // Push the contract address into the array
            usrProfileMap[posterAddress].contractsPostedIn.push(contractAddress);
        }
    ...
    */
    function updateProfile(string calldata handle, string calldata location, string calldata avatar, string calldata _uri, string calldata _bio) public {
        // If it's a new user, setup their initial profile
        if (usrProfileMap[msg.sender].joinBlock == 0){
            joinUser(msg.sender);
        }
        ...
        // Update the URI
        usrProfileMap[msg.sender].uri = _uri;

        // Update the Avatar
        usrProfileMap[msg.sender].avatar = avatar;

        // Update the Handle
        usrProfileMap[msg.sender].handle = handle;

        // Update the Location
        usrProfileMap[msg.sender].location = location;

        // Update the Bio
        usrProfileMap[msg.sender].bio = _bio;

        // Emit to the logs for external reference
        emit logProfileUpdated(msg.sender, _uri, avatar, handle, location, _bio);
    }
    ...
    */
    function getFollowings(address posterAddress, uint256 startFrom) public view returns(address[] memory){
        ...
        // See how many records we need to return
        if (usrProfileMap[posterAddress].userStats.followingCount - startFrom <= maxFollowReturn){
            counter = usrProfileMap[posterAddress].contractsPostedIn.length - startFrom;
        } else {
            counter = maxFollowReturn;
        }

        // Initialize the return array of users they're following
        address[] memory usersFollowing = new address[](counter);

        for (uint i=startFrom; i < (startFrom + counter); i++) {
            usersFollowing[i] = (usrProfileMap[posterAddress].contractsPostedIn[startFrom + i]);
        }
    ...
    function followUser(address addressRequester, address addressToFollow) public onlyAdmins {
        ...
        // Add a follower to the users profile
    ...
    function unfollowUser(address addressRequester, address addressToUnfollow) public onlyAdmins {
        ...
        // Subtract a follower from the dropped user
