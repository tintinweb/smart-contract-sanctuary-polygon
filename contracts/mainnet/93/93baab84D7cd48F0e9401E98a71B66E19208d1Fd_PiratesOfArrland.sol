...
contract WithSupervisedTransfers is AccessControl {
    ...
    bytes32 public constant SUPERVISED_TRANSFERS_ADMIN_ROLE = keccak256("SUPERVISED_TRANSFERS_ADMIN_ROLE");
    bytes32 public constant SUPERVISED_TRANSFERS_MANAGER_ROLE = keccak256("SUPERVISED_TRANSFERS_MANAGER_ROLE");

    bool public isTransferSupervised = true;
    bool public isTransferFromSupervised = true;

    event SupervisedTransferDisabled();
    event SupervisedTransferFromDisabled();
    ...
    constructor() {
        ...
        _setRoleAdmin(SUPERVISED_TRANSFERS_MANAGER_ROLE, SUPERVISED_TRANSFERS_ADMIN_ROLE);

        _setRoleAdmin(ALLOWED_TRANSFER_ROLE, SUPERVISED_TRANSFERS_MANAGER_ROLE);
        _setRoleAdmin(ALLOWED_TRANSFER_FROM_ROLE, SUPERVISED_TRANSFERS_MANAGER_ROLE);
    }

    function isTransferAllowed(address account) public view returns (bool) {
        return !isTransferSupervised || hasRole(DEFAULT_ADMIN_ROLE, account) || hasRole(ALLOWED_TRANSFER_ROLE, account);
    }

    function isTransferFromAllowed(address account) public view returns (bool) {
        return !isTransferFromSupervised || hasRole(DEFAULT_ADMIN_ROLE, account) || hasRole(ALLOWED_TRANSFER_FROM_ROLE, account);
    }

    /* Configuration
     ****************************************************************/

    function disableSupervisedTransfer() external onlyRole(DEFAULT_ADMIN_ROLE) {
        isTransferSupervised = false;

        emit SupervisedTransferDisabled();
    }

    function disableSupervisedTransferFrom() external onlyRole(DEFAULT_ADMIN_ROLE) {
        isTransferFromSupervised = false;

        emit SupervisedTransferFromDisabled();
    }
}
...
abstract contract Withdrawable {
    ...
    function withdrawToken(address to, address token_) public virtual protectedWithdrawal {
...
interface IVestable {
    /// @notice Side effect that is called after valut is created.
    /// @param vault Address of the vault.
    function onVaultCreated(address vault) external;

    /// @notice Side effect that is called after vesting is created.
    /// @param vault Address of the vault.
...
contract PiratesOfArrland is ERC20, ERC165, ERC20Burnable, ERC20Permit, Withdrawable, IVestable, WithSupervisedTransfers, AntiBot {
    ...
    function supportsInterface(bytes4 interfaceId) public view override(AccessControl, ERC165) returns (bool) {
        return interfaceId == type(IVestable).interfaceId || super.supportsInterface(interfaceId);
    }
    ...
    function onVaultCreated(address vault) external override {
        if (hasRole(SUPERVISED_TRANSFERS_MANAGER_ROLE, _msgSender())) grantRole(ALLOWED_TRANSFER_ROLE, vault);
    }
    ...
    function onVestingCreated(address vesting) external override {
        ...
        if (hasRole(SUPERVISED_TRANSFERS_ADMIN_ROLE, sender)) grantRole(SUPERVISED_TRANSFERS_MANAGER_ROLE, vesting);
        if (hasRole(SUPERVISED_TRANSFERS_MANAGER_ROLE, sender)) grantRole(ALLOWED_TRANSFER_ROLE, vesting);
    }
}