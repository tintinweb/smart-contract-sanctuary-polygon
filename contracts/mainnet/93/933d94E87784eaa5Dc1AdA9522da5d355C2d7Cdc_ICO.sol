...
contract ICO is Ownable, ReentrancyGuard {
    ...
    uint256 private constant MAX_ROUND_ICO = 200;
    uint256 private constant START_PRICE_USD = 5; // since this value is 0.0005 . Multiplying it by 10000
    uint256 private constant MAX_COMMISSION = 2 * 10 ** 18;
    ...
    event ReferralBonus(uint256 referralBonus);
    event Commission(uint256 commission);
    event AddAddressToAllowed(address indexed investor);
    event RemoveAddressFromAllowed(address indexed investor);
    ...
    modifier whenIcoStart {
        require(icoEndTime == 0, 'ICO completed');
   ...
   * @dev Returns the address of the current admin.
    ...
    */
    ...
    constructor(address _tokenAddress, address owner, address priceFeedAddress){
        require(_tokenAddress != address(0), "_tokenAddress is zero address");
        require(owner != address(0), "owner is zero address");
        require(priceFeedAddress != address(0), "price feed is zero address");
    ...
    /// can be accessed only by admin
    function setMinLimit(uint256 newLimit) external onlyAdmin {
        require(newLimit < maxLimit, "min limit should be less than max limit");
        minLimit = newLimit;
        emit MinLimitUpdated(minLimit);
    }

    /// can be accessed only by admin
    function setMaxLimit(uint256 newLimit) external onlyAdmin {
        require(newLimit > minLimit, "max limit should be more than min limit");
        maxLimit = newLimit;
        emit MaxLimitUpdated(maxLimit);
    }

    /// can be accessed only by admin
    function setReferralBonus(uint256 newReferralBonus) external onlyAdmin {
        require(newReferralBonus <= 100, 'Referral bonus should not be more than 100%');
        referralBonus = newReferralBonus;
        emit ReferralBonus(newReferralBonus);
    }
    ...
    /// can be accessed only by admin
    function setCommission(uint256 newCommission) external onlyAdmin {
        require(newCommission < MAX_COMMISSION, "new commission should be less than max commission");
        commission = newCommission;
        emit Commission(newCommission);
    }
    ...
    /// can be accessed only by admin
    function addAddressToAllowed(address client) external onlyAdmin {
        require(client != address(0), 'client address should not be empty');
        allowedInvestors[client] = true;
        emit AddAddressToAllowed(client);
    }

    /// can be accessed only by admin
    function removeAddressFromAllowed(address client) external onlyAdmin {
        require(client != address(0), 'client address should not be empty');
        allowedInvestors[client] = false;
        emit RemoveAddressFromAllowed(client);
    }

    function _allocate() private {
        require(currentIterationOfICO <= MAX_ROUND_ICO, 'Funding cycle ended');
        if (icoStartTime == 0) {
            usdPrice = START_PRICE_USD;
        } else if (currentIterationOfICO == 0) {
            oneIterationTokenAmount = 19 * 10 ** 5 * 10 ** 18;
            usdPrice = 10;
            // to have correct order over START_PRICE_USD -> Multiplying 0.001 by 10000
            stepPriceInUSD = 1;
            ...
            usdPrice = 110;
            // to have correct order over START_PRICE_USD -> Multiplying 0.011 by 10000
            stepPriceInUSD = 10;
            currentIterationOfICO++;
        } else if (currentIterationOfICO > 100 && currentIterationOfICO <= MAX_ROUND_ICO) {
            usdPrice = usdPrice.add(stepPriceInUSD);
            currentIterationOfICO++;
        }
        ...
        if (amountTokens > current_allocatedTokens) {
            ...
            while (usdAmount > 0) {
                ...
                if (amount > current_allocated) {
                    ...
                    if (currentIterationOfICO < MAX_ROUND_ICO) {
                        icoRound++;
                    }
    ...
    function getCost(uint amount) public view returns (uint256){
        ...
        if (amount <= current_allocatedTokens) {
            ...
            uint256 current_allocated = current_allocatedTokens;
            uint256 icoRound = currentIterationOfICO;
            uint256 iterationAmount = 0;
            while (amount > 0) {
                if (current_allocated > 0) {
                    amount = amount.sub(current_allocated);
                    usdCost = current_allocated.mul(price).div(10 ** 4).add(usdCost);
                    current_allocated = 0;
                    icoRound++;
                ...
                if (amount > iterationAmount) {
                    ...
                    usdCost = iterationAmount.mul(price).div(10 ** 4).add(usdCost);
                    icoRound++;
                    (price, stepPrice) = getTokenPrice(icoRound);
                    price = price.add(stepPrice);
                } else {
                    usdCost = amount.mul(price).div(10 ** 4).add(usdCost);
                    amount = 0;
                }
            }
            ethCost = getPriceInETH(usdCost, exchangeRate);
    ...
    function _completeICO() private {
        if (current_allocatedTokens < minLimit && currentIterationOfICO >= MAX_ROUND_ICO) {
    ...
    function withdrawReward() external nonReentrant onlyAdmin {
    ...
    function getCurrentICOState() external view returns (ICOStateSchema memory currentState) {
    ...
    /// payable
    /// accessible only by allowed investors
    function buyWithReferral(address payable referral) external payable allowedInvestor whenIcoStart {
    ...
    /// payable
    /// can be accessed only by admin
    function buyFor(uint256 value, address buyer) external payable onlyAdmin whenIcoStart nonReentrant {
    ...
    /// read only
    /// returns uint256
    function getTokenPrice(uint256 icoIteration) public pure returns (uint256, uint256) {
        require(icoIteration <= MAX_ROUND_ICO, 'ICO finished');
        uint256 price = 0;
        uint256 stepPrice = 0;
        if (icoIteration == 0) {
            price = START_PRICE_USD;
    ...
    /// read only
    /// returns uint256
    function getTokensPerIteration(uint256 icoIteration) public pure returns (uint256) {
        require(icoIteration <= MAX_ROUND_ICO, 'Incorrect ICO round');
        uint256 amount;
        if (icoIteration == 0) {
...
contract Vesting is Ownable, ReentrancyGuard {
    ...
    event Released(address indexed userAddress, uint256 amount, uint256 iteration);
     ...
     */
    function startVesting(address memberAddress, uint256 amount) external onlyOwner {
        require(memberAddress != address(0), 'member address should not be empty');
        VestingInfo memory vestingInfo = VestingInfo(
            block.timestamp,
            amount,
            0,
            amount / COUNT_OF_MONTH,
            block.timestamp
        );
    ...
    */
    function getReleaseReadyAmount() external view returns (uint256) {
        uint256 releaseReadyAmount = 0;
        for (uint i = 0; i < usersIndexes[msg.sender]; i++) {
            VestingInfo memory vesting = usersVestings[msg.sender][i];
            uint256 diff = getDiffMonth(vesting.startDate, vesting.lastReleaseDate);

            if (diff > COUNT_OF_MONTH) {
                continue;
            }

            if (vesting.amount - vesting.releasedAmount >= vesting.amountPerMonth) {
                uint256 month = getDiffMonth(vesting.lastReleaseDate, block.timestamp);
                if (month > COUNT_OF_MONTH) {
                    month = COUNT_OF_MONTH - diff;
                }
                uint256 availableReleaseAmount = vesting.amountPerMonth * month;
                if (vesting.amount - (vesting.releasedAmount + availableReleaseAmount) < vesting.amountPerMonth) {
                    availableReleaseAmount += vesting.amount - (vesting.releasedAmount + availableReleaseAmount);
                }
                releaseReadyAmount += availableReleaseAmount;
            } else {
                releaseReadyAmount += vesting.amount - vesting.releasedAmount;
            }
        }
        return releaseReadyAmount;
    }
    ...
    */
    function release() external nonReentrant {
        ...
        for (uint i = 0; i < usersIndexes[msg.sender]; i++) {
            ...
            if (releaseReadyAmount > 0) {
                VestingInfo storage updateVesting = usersVestings[msg.sender][i];
                updateVesting.releasedAmount += releaseReadyAmount;
                updateVesting.lastReleaseDate += SECONDS_IN_MONTH * month;
    ...
    function getDiffMonth(uint256 startTimestamp, uint256 endTimestamp) internal pure returns (uint256) {
        return (endTimestamp - startTimestamp) / SECONDS_IN_MONTH;
    }
}
