...
contract DAM is ERC721Enumerable, Ownable {
  ...
  function airdrop(address[] memory _to, uint256[] memory _amounts) public onlyOwner {
    require(_to.length == _amounts.length, "Array lengths must match");

    for (uint i = 0; i < _to.length; i++) {
        uint256 tokenId = totalSupply() + 1;
        for (uint j = 0; j < _amounts[i]; j++) {
            _safeMint(_to[i], tokenId + j);
            emit Transfer(address(0), _to[i], tokenId + j);
        }
    }
  }
