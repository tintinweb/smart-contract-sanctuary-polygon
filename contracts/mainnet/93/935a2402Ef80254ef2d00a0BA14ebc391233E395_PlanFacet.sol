...
contract PlanFacet is S10NSubManagerBaseFacet {
    event PlanCreated(
    ...
    function createPlan(
        ...
        // avoid stack depth limit
        {
            ...
            require(bytes(strings[0]).length <= 200, "invalid name length");
            require(
                bytes(strings[1]).length <= 1000,
                "invalid description length"
            );
            require(
                IPlatformFeeManager(_getManagerAddress("PlatformFeeManager"))
                    .validPrice(addresses[0], uints[2]),
                "price invalid"
            );
        }
        ...
        uint256 planIndex = IPlanManager(_getManagerAddress("PlanManager"))
            .createPlan(plan);
        emit PlanCreated(
    ...
    function updatePlan(
        ...
        // avoid stack depth limit
        {
            address merchantOwner = IERC721(
                _getManagerAddress("MerchantTokenManager")
            ).ownerOf(merchantTokenId);
            require(msg.sender == merchantOwner, "must call by merchant owner");
            require(payeeAddress != address(0), "invalid payeeAddress");
            require(bytes(name).length <= 200, "invalid name length");
            require(
        ...
        IPlanManager(_getManagerAddress("PlanManager")).updatePlan(
            merchantTokenId,
            planIndex,
            plan
        );
    ...
    function disablePlan(
        ...
        require(plan.enabled == true, "plan already disabled");
    ...
    function enablePlan(
        ...
        require(plan.enabled == false, "plan already enabled");
        address merchantOwner = IMerchantTokenManager(
            _getManagerAddress("MerchantTokenManager")
        ).ownerOf(merchantTokenId);
        require(msg.sender == merchantOwner, "must call by merchant owner");
