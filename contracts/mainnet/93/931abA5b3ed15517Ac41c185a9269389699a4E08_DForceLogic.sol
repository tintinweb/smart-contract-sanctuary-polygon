...
contract DForceLogic is LogicUpgradeable {
    ...
    address private iBNB;
    mapping(address => bool) private usediTokens;
    mapping(address => address) private iTokens;
    ...
    modifier isUsediToken(address iToken) {
        require(usediTokens[iToken], "E2");
        _;
    }

    modifier isUsedSwap(address swap) {
        require(swap == pancakeRouter || swap == uniswapRouter, "E3");
        _;
    }
    ...
    /**
     * @notice Add iToken in Contract and approve token  for storage, venus,
     ...
     * @param iToken Address of iToken
     */
    function addiTokens(address token, address iToken)
        external
        onlyOwnerAndAdmin
    {
        require(IComptrollerDForce(controller).hasiToken(iToken), "E5");

        if ((token) != address(0)) {
            IERC20Upgradeable(token).approve(iToken, type(uint256).max);
            ...
            iTokens[token] = iToken;
        } else {
            iBNB = iToken;
        }

        usediTokens[iToken] = true;
    }
    ...
    function setBLID(address blid_) external onlyOwner {
        ...
        blid = blid_;
        IERC20Upgradeable(blid).safeApprove(pancakeRouter, type(uint256).max);
        IERC20Upgradeable(blid).safeApprove(uniswapRouter, type(uint256).max);
    ...
    function approveTokenForSwap(address token) public onlyOwnerAndAdmin {
        ...
        (IERC20Upgradeable(token).approve(uniswapRouter, type(uint256).max));
    }
    ...
    /**
     * @notice Enter into a list of markets(address of iTokens) - it is not an
     * error to enter the same market more than once.
     * @param _iTokens The addresses of the iToken markets to enter.
    ...
    function enterMarkets(address[] calldata _iTokens)
        ...
        returns (bool[] memory)
    {
        return IComptrollerDForce(controller).enterMarkets(_iTokens);

        revert("E13");
    }

    /**
     * @notice Every Venus user accrues DF for each block
     * they are supplying to or borrowing from the protocol.
     * @param _iTokens The addresses of the iToken markets to enter.
     */
    function claim(address[] calldata _iTokens) external onlyOwnerAndAdmin {
        address[] memory holders = new address[](1);
        holders[0] = address(this);

        IDistributionDForce(rewardsDistributor).claimReward(holders, _iTokens);
    }

    /**
     * @notice Stake token and mint iToken
     * @param iToken: that mint iTokens to this contract
     * @param mintAmount: The amount of the asset to be supplied, in units of the underlying asset.
     * @return 0 on success, otherwise an Error code
     */
    function mint(address iToken, uint256 mintAmount)
        external
        isUsediToken(iToken)
        onlyOwnerAndAdmin
        returns (uint256)
    {
        ...
        if (iToken == iBNB) {
            IiTokenETH(iToken).mint{value: mintAmount}(address(this));
            return 0;
        }

        IiToken(iToken).mint(address(this), mintAmount);
        return 0;
    }
     ...
     * @return 0 on success, otherwise an Error code
     */
    function borrow(address iToken, uint256 borrowAmount)
        external
        payable
        isUsediToken(iToken)
        onlyOwnerAndAdmin
        returns (uint256)
    {
        ...
        (liquidity, shortfall, , ) = IComptrollerDForce(controller)
            .calcAccountEquity(address(this));

        require(shortfall == 0, "E11");
        require(liquidity > 0, "E12");

        IiToken(iToken).borrow(borrowAmount);
        return 0;
    }
     ...
     * @return 0 on success, otherwise an Error code
     */
    function repayBorrow(address iToken, uint256 repayAmount)
        external
        isUsediToken(iToken)
        onlyOwnerAndAdmin
        returns (uint256)
    {
        if (iToken == iBNB) {
            IiTokenETH(iToken).repayBorrow{value: repayAmount}();
            return 0;
        }

        IiToken(iToken).repayBorrow(repayAmount);
        return 0;
    }

    /**
     * @notice The redeem underlying function converts iTokens into a specified quantity of the
     * underlying asset, and returns them to the user.
     * The amount of iTokens redeemed is equal to the quantity of underlying tokens received,
     ...
     * @return 0 on success, otherwise an Error code
     */
    function redeemUnderlying(address iToken, uint256 redeemAmount)
        external
        isUsediToken(iToken)
        onlyOwnerAndAdmin
        returns (uint256)
    {
        IiToken(iToken).redeemUnderlying(address(this), redeemAmount);
        return 0;
    }
        ...
        returns (uint256[] memory amounts)
    {
        ...
        if (swap == uniswapRouter) {
            require(path.length == 2, "E18");
            uint24 fee = _uniswapGetPoolFee(path[0], path[1]);
            require(fee > 0, "E19");
            ...
            amountOuts[0] = ISwapRouter(swap).exactInputSingle(params);
        } else if (swap == pancakeRouter) {
            amountOuts = IPancakeRouter01(swap).swapExactTokensForTokens(
                amountIn,
                amountOutMin,
                path,
                address(this),
        ...
        returns (uint256[] memory amounts)
    {
        ...
        if (swap == uniswapRouter) {
            ...
            amountIns[0] = ISwapRouter(swap).exactOutputSingle(params);
        } else if (swap == pancakeRouter) {
            amountIns = IPancakeRouter01(swap).swapTokensForExactTokens(
                amountOut,
                amountInMax,
                path,
                address(this),
        ...
        returns (uint256[] memory amounts)
    {
        ...
        if (swap == uniswapRouter) {
            ...
            amountOuts[0] = ISwapRouter(swap).exactInputSingle{
                value: amountETH
            }(params);
        } else if (swap == pancakeRouter) {
            amountOuts = IPancakeRouter01(swap).swapExactETHForTokens{
                value: amountETH
        ...
        returns (uint256[] memory amounts)
    {
        ...
        if (swap == uniswapRouter) {
            ...
            amountIns = IPancakeRouter01(swap).swapTokensForExactETH(
                amountOut,
                amountInMax,
                path,
                address(this),
        ...
        returns (uint256[] memory amounts)
    {
        ...
        if (swap == uniswapRouter) {
            ...
            amountOuts = IPancakeRouter01(swap).swapExactTokensForETH(
                amountIn,
                amountOutMin,
                path,
                address(this),
        ...
        returns (uint256[] memory amounts)
    {
        ...
        if (swap == uniswapRouter) {
            ...
            amountIns[0] = ISwapRouter(swap).exactOutputSingle{
                value: amountETH
            }(params);
        } else if (swap == pancakeRouter) {
            amountIns = IPancakeRouter01(swap).swapETHForExactTokens{
                value: amountETH
     ...
     * @return Pool fee, 3000, 5000, 1000, if 0, pool isn't exist
     */
    function _uniswapGetPoolFee(address tokenA, address tokenB)
        private
        returns (uint24)
    {
        ...
        for (uint8 i = 0; i < 3; ) {
            ...
            if (pool != address(0)) return fees[i];
            unchecked {
                ++i;
            }
        }
...
contract DForceLogicOld is LogicUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
...
interface IDistributionDForce {
    ...
    function getAccountRewardAmount(address holder) external returns (uint256);
}
...
interface IDForceLogic {
    function addiTokens(address token, address xToken) external;

    function approveTokenForSwap(address token) external;

    function claim(address[] calldata iTokens) external;
    ...
    function repayBorrow(address iToken, uint256 repayAmount) external;
