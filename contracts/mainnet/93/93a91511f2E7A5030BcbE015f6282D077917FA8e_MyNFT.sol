...
contract MyNFT is AccessControlUpgradeable, UUPSUpgradeable{
    ...
    function initialize (address _nftContract) public initializer{
        ...
        Ierc721(nftContract).initialize(msg.sender);
    }
     ...
     *
     *En caso de que no contenga los permisos se cortar├í la ejecuci├│n y se notificar├í al usuario
     */
     ...
     *El address no deber├¡a estar registrado
    ...
    function transferNft(address _to, uint _nftId, string memory _name) public hasFinancieraRole(msg.sender) {
        Ierc721(nftContract).transferFrom(msg.sender, _to, _nftId);
    ...
    function mintNFT(address _recipient, string memory _tokenURI, Metadata memory _metadata) public hasFinancieraRole(_recipient) 
     returns (uint){
        ...
        fintech[_recipient].nfts.push(tokenId);
         ...
         Ierc721(nftContract).mintNFT(_recipient);
         Ierc721(nftContract).setTokenURI(tokenId, _tokenURI);
    ...
    function _authorizeUpgrade(address newImplementation) internal virtual override hasAdminRole(msg.sender) {
    }

    function pause() public hasAdminRole(msg.sender){
        Ierc721(nftContract).pause();
    }

    function unpause() public hasAdminRole(msg.sender){
        Ierc721(nftContract).unpause();
    }
