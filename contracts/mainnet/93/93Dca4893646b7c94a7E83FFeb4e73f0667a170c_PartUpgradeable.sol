...
interface IPartTypeUpgradeable {
    event SetTypeNFT(uint256 typeNFT, uint256 limit);
...
interface IPartUpgradeable {
    ...
    function batchMint(address recipient_, uint256 typeNFT_, uint256 quantity_) external;
}
...
import { IPartTypeUpgradeable } from "../interfaces/IPartTypeUpgradeable.sol";
import { BaseUpgradeable } from "./BaseUpgradeable.sol";
import { OPERATOR_ROLE } from "../libraries/Constants.sol";

abstract contract PartTypeUpgradeable is IPartTypeUpgradeable, BaseUpgradeable {
    mapping(uint256 => uint256) internal _limited;
    mapping(uint256 => uint256) internal _sold;

    function __PartType_init(uint256[] calldata types_, uint256[] calldata limits_) internal onlyInitializing {
        __PartType_init_unchained(types_, limits_);
    }

    function __PartType_init_unchained(
        uint256[] calldata types_,
        uint256[] calldata limits_
    ) internal onlyInitializing {
        _setTypes(types_, limits_);
    }
    ...
    function setTypes(uint256[] calldata types_, uint256[] calldata limits_) external onlyRole(OPERATOR_ROLE) {
        _setTypes(types_, limits_);
    }

    function _setTypes(uint256[] calldata types_, uint256[] calldata limits_) internal {
        uint256 length = types_.length;
        for (uint256 i = 0; i < length; ) {
            _setType(types_[i], limits_[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function _setType(uint256 type_, uint256 limit_) internal {
        if (_sold[type_] > limit_) revert NonFungibleType__ExceedLimit(type_);
        _limited[type_] = limit_;
        emit SetTypeNFT(type_, limit_);
    }

    function _setSold(uint256 typeNFT_, uint256 quantity_) internal {
        _sold[typeNFT_] = _sold[typeNFT_] + quantity_;
        // if (_sold[typeNFT_] > _limited[typeNFT_]) revert NonFungibleType__ExceedLimit(typeNFT_);
    }
...
import { PartTypeUpgradeable } from "./internal-upgradeable/PartTypeUpgradeable.sol";
...
import { IPartUpgradeable } from "./interfaces/IPartUpgradeable.sol";

contract PartUpgradeable is
    IPartUpgradeable,
    BaseUpgradeable,
    PartTypeUpgradeable,
    ...
    ERC721URIStorageUpgradeable
{
    ...
    function getTypeNFT(uint256 typeNFT) external view returns (uint256, uint256) {
        return (_limited[typeNFT], _sold[typeNFT]);
    }
