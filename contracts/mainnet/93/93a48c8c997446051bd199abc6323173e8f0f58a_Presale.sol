...
contract Presale is ReentrancyGuard, Ownable {
    ...
    function buy(uint256 busdAmount) external {
        ...
        require(totalSold + busdAmount <= hardCap, "HardCap reached");
        uint256 tokens = busdAmount * tokenPrice;

        busd.transferFrom(msg.sender, address(this), busdAmount);
        totalSold += busdAmount;
    ...
    function claim() external nonReentrant {
        ...
        uint256 monthsPassed = timePassed / 5 minutes;
        uint256 tokens = (balances[msg.sender] * vestingPercentage * monthsPassed) / 100;
    ...
    function nextClaimAvailable(address user) public view returns (uint256) {
        ...
        else if(lastClaim[user] + 5 minutes > block.timestamp) {
            return lastClaim[user] + 5 minutes;
        } 
        else {
