...
contract ANIVLandSale is Ownable {
	...
	event LandSale(address indexed sellAddress, address indexed buyAdress, uint256 indexed tokenId, uint256 amount);
	...
	function landSale(address from, address to, uint256 tokenId, uint256 amount) public onlyOwner {
		require(erc721.ownerOf(tokenId) == from);
		require(erc20.balanceOf(to) >= amount);
		erc20.transferFrom(to, from, amount);
		erc721.transferFrom(from, to, tokenId);
		emit LandSale(from, to, tokenId, amount);
	}
