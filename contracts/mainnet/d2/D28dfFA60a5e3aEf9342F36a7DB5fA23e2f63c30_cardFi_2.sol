...
contract cardFi_2 is Initializable, OwnableUpgradeable{
    ...
    function initialize() public initializer{
        ...
        addCurrency(IERC20Upgradeable(0x01Af8c15A54Ff442b868C3587d3a2c3ceeebcee6)); 
        setRoyalty_ERC20(IERC20Upgradeable(0x01Af8c15A54Ff442b868C3587d3a2c3ceeebcee6), 0, 0);      
        ...
        struct Card {
            ...
            uint256 lockTime;
            //this shows if the NFT has a currency attached 
            bool ERC20Added;
            //this shows if the NFT has a native currency attached
            bool nativeCurrency;
        }

        event newRoyalty(uint8 deposit, uint8 withdraw); //setRoyalty
        event currencyAdded(IERC20Upgradeable token_added); //addCurrency
        event tokenToNftEvent(IERC721Upgradeable nftAddress, uint256 tokenId, IERC20Upgradeable currency,  bool ERC20Added, uint256 lockTime); //tokenToNft
        event deposit_nativeEvent(IERC721Upgradeable nftAddress, uint256 tokenId, uint256 depositAmount, uint256 balance, uint256 lockTime, uint256 royalty); //deposit_native
        event deposit_ERC20Event(IERC721Upgradeable nftAddress, uint256 tokenId, uint256 depositAmount, uint256 balance, IERC20Upgradeable currency, uint256 lockTime, uint256 royalty); //deposit_ERC20
        event redeemEvent(IERC721Upgradeable nftAddress, uint256 tokenId, uint256 redeemAmount, uint256 royalty, uint256 balance, IERC20Upgradeable currency); //redeem


        
            function setRoyaltyAddress(address payable _newRoyaltyAddress) public onlyOwner{
            ...
            function setRoyalty_native(uint8 depositFee, uint8 withdrawFee) public onlyOwner {        
                require(depositFee<=100 && withdrawFee<=100, "royalties must be lower than 100%");
                deposit=depositFee;
            ...
            function setRoyalty_ERC20(IERC20Upgradeable tokenAddress, uint8 depositFee, uint8 withdrawFee) public onlyOwner {
                 require(depositFee<=100 && withdrawFee<=100, "royalties must be lower than 100%");
            ...
            function seeRoyalty_native() public onlyOwner view returns(uint256, uint256){
                 
                return(deposit, withdraw);
                
            }
            
             function seeRoyalty_ERC20(IERC20Upgradeable tokenAddress) public onlyOwner view returns(uint256, uint256){
        ...
        function tokenToNft(IERC721Upgradeable _contractAddress, uint256 _tokenId, IERC20Upgradeable _currency) private 
        {   
            ...
            //emit tokenToNftEvent(_contractAddress, _tokenId, _currency,  _Card.ERC20Added, _Card.lockTime);  
        }       

       
        // this adds the amount of ERC20 to the NFT
        function deposit_ERC20(IERC721Upgradeable _contractAddress, uint256 _tokenId, IERC20Upgradeable _currency,  uint256 _depositAmount, uint256 newTime) public  
          {
            require(_depositAmount>=100,"100 wei is minimum");                    
             ...
             if(!_Card.ERC20Added){
                ...
                bool succeededRoyalty=_currency.transferFrom(msg.sender, royaltyAddress, royalty);
                require(succeeded, "transfer failed");
                require(succeededRoyalty, "transfer failed");
                address cardFi_NFT=(0x505c7BF089197EB7c2cC62804b155cc094FEB793);
                
                if(_contractAddress.ownerOf(_tokenId)==msg.sender && block.timestamp>=_Card.lockTime){
                    _Card.lockTime=block.timestamp+newTime;
                } else if(msg.sender==cardFi_NFT){
                    _Card.lockTime=block.timestamp+newTime;
                }    
            emit deposit_ERC20Event(_contractAddress, _tokenId, amount, _Card.balance, _currency, _Card.lockTime, royalty);
            
        }


        // this adds the amount of ERC20 to the NFT
        function deposit_native(IERC721Upgradeable _contractAddress, uint256 _tokenId, uint256 _depositAmount, uint256 newTime) public payable 
          {
            require(_depositAmount>=100,"100 wei is minimum");
            Card storage _Card =vaultBox[_contractAddress][_tokenId];
            ...
            emit deposit_nativeEvent(_contractAddress, _tokenId, amount, _Card.balance, _Card.lockTime, royalty);     
        }

            // this shows the owner of this contract how much of the ERC20 is stored in the smart contract
        function contractBalance_ERC20(IERC20Upgradeable _currency) public onlyOwner view returns(uint256){
            require(tokenExist(_currency));  
            return _currency.balanceOf(address(this));                     
        }
        

              // this shows the owner of this contract how much of native currency is stored in smart contract
        function contractBalance_native() public onlyOwner view returns(uint256){
        ...
        function cardInfo(IERC721Upgradeable _contractAddress, uint256 _tokenId) public view
            returns (uint256 balance, IERC20Upgradeable currencyAddress, bool ERC20Added, bool owner, uint256 time, bool nativeCurrency){
        ...
        function redeem(IERC721Upgradeable _contractAddress, uint256 _tokenId, uint256 _redeemAmount) public {
            
            require(_redeemAmount>=100,"100 wei is minimum");
            require(_contractAddress.ownerOf(_tokenId)==msg.sender, "You are not the owner of this NFT");
            Card storage _Card =vaultBox[_contractAddress][_tokenId]; 
