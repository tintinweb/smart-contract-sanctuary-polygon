...
contract SyncGTLPModule is OwnableUpgradeable, AccessControlUpgradeable{
    ...
    function executePermitted(
     ...
     function mintGT(address holder, uint256 amount) external onlyRole(RELEASER_ROLE){
        Dao(token).mint(holder, amount);
    }
    // function mintGTPermitted(address holder, uint256 amount) external onlyRole(RELEASER_ROLE){
    //     Dao(token).executePermitted(token, holder ,0);
    // }
    function setGT(Dao _token) public onlyRole(ADMIN_ROLE) returns (bool) {
        token = _token;
        return true;
    }
    ...
    function sync(address[] memory holders, int256[] memory amounts) external onlyRole(RELEASER_ROLE){
        for (uint256 i = 0; i < holders.length; i++) {
            if (amounts[i]>0){
                uint256 amount = uint256(amounts[i]);
                Dao(token).mint(holders[i], amount);
            }
            else if (amounts[i]<0){
                uint256 amount = uint256(-amounts[i]);
                Dao(token).burn(holders[i], amount);
            } 
        }
    }
}