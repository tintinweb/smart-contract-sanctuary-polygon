...
// File: contracts/NClipMarketplace.sol
...
contract NClipMarketPlace is ERC721, Pausable, Ownable {
    ...
    event itemAdded(uint256 id, uint256 tokenId, address tokenAddress, uint256 askingPrice, address owner_of);
    event itemSold (uint256 id, address buyer, uint256 askingPrice);
	event itemRemoved(uint256 id, uint256 tokenId, address tokenAddress, address owner_of);
    ...
    function addItemToMarket(uint256 tokenId, address tokenAddress, uint256 askingPrice) OnlyItemOwner(tokenAddress, tokenId) HasTransferApproval(tokenAddress, tokenId) external returns(uint256) {
        ...
        emit itemAdded(newItemId, tokenId, tokenAddress, askingPrice, owner_of);
        return newItemId;
    }
	...
	function removeItemFromMarket(uint256 id, address tokenAddress, uint256 tokenId) external ItemExists(id) IsForSale(id)
    {			
        IERC721 tokenContract = IERC721(tokenAddress);
        address owner_of = tokenContract.ownerOf(tokenId);
