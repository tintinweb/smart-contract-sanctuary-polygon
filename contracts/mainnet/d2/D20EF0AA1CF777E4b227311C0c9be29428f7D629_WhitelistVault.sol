...
contract WhitelistVault is IWhitelistVault, Ownable, ReentrancyGuard {
    ...
    // For vaults that are farming pools with a deposit fee i.e. Mai Finance pools
    ...
    function deposit(uint256 _amount) public override nonReentrant {
        require(IVaultFactory(vaultFactory).isVault(msg.sender), "not a vault");
    ...
    function withdraw(uint256 _shares) public override nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        require(_shares <= user.shares, "cannot withdraw more than balance");
