...
contract LazyLender is PokeMeReady {
  ...
  function lendGotchis() external onlyPokeMe {
    ...
    uint gotchiLastChanneled = alchemicaFacet.getLastChanneled(gotchiIds[0]);
    uint gotchiChannelable = block.timestamp / (60 * 60 * 24);
    if (gotchiChannelable > gotchiChannelable) {
