...
contract LimitOrders is EIP712("FBS Token", "1"){
    ...
    function shouldFulfilOrder(Order memory ord, bytes memory _signature) public view returns (bool) {
        ...
        if(IERC20(ord.tokenIn).balanceOf(ord.owner) >= ord.amountIn) isEnoughToken = true;
        ...
        if(ord.status == 0) isPendingOrder = true;

        bool isSigner = false;
        address _signer = recoverSigner(ord, _signature);
        if(_signer == ord.owner) isSigner = true;

        bool isFulfiledOrder = false;
        if(getAmountOut(ord.amountIn,ord.pair, ord.tokenIn, ord.tokenOut) >= ord.priceExecute) isFulfiledOrder = true;
