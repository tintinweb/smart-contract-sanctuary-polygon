...
contract EquityPolygonRouter {
    ...
    modifier ensureFee(uint256 fee) {
        require(fee >= IFeeable(feeable).getBaseFee(), "EquityPolygonRouter: min fee subceeded");
        require(fee <= IFeeable(feeable).getMaxFee(), "EquityPolygonRouter: max fee exceeded");
