...
contract MaticMikeData is Initializable, UUPSUpgradeable, OwnableUpgradeable{
    // Trait structure
    struct Trait{
        string traitName;
        string traitType;
    ...
    */
        ...
        returns (string memory)
    {
        ...
        // external contract call for boss loot and graphics, and any background gfx for future
        if(xxlAddress != nullAddress){
            IMikeExtended.Traits[] memory XxlTraits = IMikeExtended(xxlAddress).getAllPlayerLoot(_tokenId);
        ...
        svgString = string(
            abi.encodePacked(
                svgString,
                bodyMap[MaticMikeLibrary.parseInt(MaticMikeLibrary.substring(_hash, 8, 9))]
            )
        );
        ...
        svgString = string(
            abi.encodePacked(
                '<svg id="mm-svg" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMinYMin meet" viewBox="0 0 24 24"> ',
                svgString,
    ...
    **********************************/
    ...
    function populateSVG(uint8 thisTrait, uint256 thisTraitIndex, string calldata svg) public onlyOwner
    {
        if(thisTrait == 8){
