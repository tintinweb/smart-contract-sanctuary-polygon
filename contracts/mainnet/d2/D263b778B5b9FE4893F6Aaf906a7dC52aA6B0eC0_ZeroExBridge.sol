    ...
    MutableOwner
{
    ...
    function quoterPeripheryCallback(
        ...
        require(takerAsset.approve(zeroExProxyAddress, type(uint256).max), "0xBridge: approve failed");

        takerAsset.safeTransferFrom(maker, address(this), info.takingAmount);
        (bool success, bytes memory returndata) = zeroExProxyAddress.call(payload);
        require(success, "0xBridge: call failed");
