    ...
    UUPSUpgradeable
{
    ...
    function _getExchangeRate() private view returns (uint256) {
        IExchangeRateOracle.ExchangeRate memory data = IExchangeRateOracle(oracle).getExchangeRate(
            evmChainId,
            address(tokenA),
            evmChainId,
            address(tokenB)
    ...
    function withdrawFee(uint256 amountA, uint256 amountB, address to) external onlyOwner {
        tokenA.safeTransfer(to, amountA);
        tokenB.safeTransfer(to, amountB);
    }
    ...
    function _swap(
        ...
        uint256 exchangeRate = _getExchangeRate();
        require(exchangeRate == currentExchangeRate, "RATE_CHANGED");
        IERC20Upgradeable erc20TokenIn = IERC20Upgradeable(tokenIn);
    ...
    function _swapExactOutput(
        ...
        address to,
        uint256 currentExchangeRate
    ) private returns (uint256 amountInWithFee, address tokenIn, uint256 totalFee) {
        uint256 exchangeRate = _getExchangeRate();
        require(exchangeRate == currentExchangeRate, "StableTokensPool: EXCHANGE_RATE_CHANGED");
