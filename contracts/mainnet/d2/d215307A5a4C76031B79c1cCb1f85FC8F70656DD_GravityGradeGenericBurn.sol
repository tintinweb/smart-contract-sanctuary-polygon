...
contract GravityGradeGenericBurn is IGravityGradeGenericBurn, OwnableUpgradeable {
    ...
    function createContentCategory(
        uint256 _tokenId
    ) external onlyGov returns (uint256 _categoryId) {
        unchecked {
     ...
     */
    function _arrayToCumulative(
        uint256[] memory _arr
    ) private pure returns (uint256[] memory _cumulativeArr) {
    ...
    function _binarySearch(
        uint256[] memory _arr,
        uint256 _target
    ) private pure returns (uint256 _location) {
        uint256 left;
        uint256 mid;
