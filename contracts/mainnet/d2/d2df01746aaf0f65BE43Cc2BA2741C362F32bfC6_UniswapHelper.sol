...
contract UniswapHelper {
    ...
    constructor(address _uniV3Pool) public {
    ...
    function rawPosition(Ticks memory _tick, address _strategy)
        internal
        view
        returns (
            uint128 liquidity,
        ...
        )
    {
        ...
        bytes32 key = keccak256(abi.encodePacked(_strategy, _tick.lowerTick, _tick.upperTick));
    ...
    {
        (liquidity, , , owed0, owed1) = rawPosition(_tick, msg.sender);
    }
        ...
        returns (uint256, uint256)
    {
        (
            ...
            uint128 owed0,
            uint128 owed1
        ) = rawPosition(_ticks, msg.sender);
        // Compute current fees earned
        uint256 wantFee =
            computeFeesEarned(_currentTick, true, feeGrowthInside0Last, _ticks, liquidity) + owed0;
        uint256 wethFee =
            computeFeesEarned(_currentTick, false, feeGrowthInside1Last, _ticks, liquidity) + owed1;
    ...
    {
        (uint128 _liquidity, uint128 owed0, uint128 owed1) = position(_ticks);

        (uint256 amount0, uint256 amount1) = amountsForLiquidity(_liquidity, _ticks);
