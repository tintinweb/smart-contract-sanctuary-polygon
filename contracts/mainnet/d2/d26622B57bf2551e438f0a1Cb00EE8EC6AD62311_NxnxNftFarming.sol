...
contract NxnxNftFarming is Ownable, IERC721Receiver, IERC1155Receiver {
    ...
    function withdrawSubNft(uint256 _index) public {
        UserInfo storage user = userInfo[_msgSender()];
        if (user.nftItems[_index].isERC721) {
            IERC721(user.nftItems[_index].nftAddress).safeTransferFrom(
                address(this),
                _msgSender(),
                user.nftItems[_index].nftTokenId
            );
        } else {
            IERC1155(user.nftItems[_index].nftAddress).safeTransferFrom(
                address(this),
                _msgSender(),
                user.nftItems[_index].nftTokenId,
                1,
        ...
        emit NftWithdrawn(
            _msgSender(),
            user.nftItems[_index].isERC721,
            user.nftItems[_index].nftAddress,
            user.nftItems[_index].nftTokenId
        );
