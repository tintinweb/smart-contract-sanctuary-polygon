...
// File: NFTChildMintableToken.sol
...
contract NFTChildMintableToken is NFTToken {
    ...
    event NFTPriceUpdated(uint256 updatedPrice);
    ...
    function updateNftPrice(uint256 updatedPrice) public onlyRole(DEFAULT_ADMIN_ROLE){
        ...
        emit NFTPriceUpdated(updatedPrice);
    }
    ...
    function rotateTransactor(address payable newTransactor) external payable onlyRole(TRANSACTOR_ROLE) {
        ...
        require(newTransactor != _msgSender(),"The new transactor must be different from old transactor");
        require(msg.value > 0 ,"The amount should be greater than 0");
        rotate(newTransactor,_msgSender());
        newTransactor.transfer(msg.value);
    }

    function rotateTransactorByOwner(address payable newTransactor, address oldTransactor) external onlyRole(DEFAULT_ADMIN_ROLE){
        require(hasRole(TRANSACTOR_ROLE,oldTransactor),"The old transactor must have transactor role");
        require(newTransactor != address(0x0), 'Address cannot be empty');
        require(newTransactor != oldTransactor,"The new transactor must be different from old transactor");
