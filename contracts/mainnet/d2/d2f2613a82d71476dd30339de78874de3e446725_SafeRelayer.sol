...
contract MultiSend2 {
    ...
    /// @dev Sends multiple transactions and reverts all if one fails.
    ///      Modified to require a specific call target and method
    /// @param transactions Encoded transactions. Each transaction is encoded as a packed bytes of
    ///                     operation as a uint8 with 0 for a call or 1 for a delegatecall (=> 1 byte),
    ///                     to as a address (=> 20 bytes),
    ///                     value as a uint256 (=> 32 bytes),
    ...
    /// @notice This method is payable as delegatecalls keep the msg.value from the previous call
    ///         If the calling method (e.g. execTransaction) received ETH this would revert otherwise
    function multiSend(address safe, bytes4 sig, bytes memory transactions) public payable {
        require(address(this) != multisendSingleton, "MultiSend should only be called via delegatecall");
        // solhint-disable-next-line no-inline-assembly
        assembly {
            let length := mload(transactions)
            let i := 0x20
            /*if not(lt(i, length)) {
                // at least one transaction needed
                revert(0,0)
            }*/
            for {
                // Pre block is not used in "while mode"
                ...
                let to := shr(0x60, mload(add(transactions, add(i, 0x01))))
                // expect target call
                let safe_ := and(safe, 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF)
                if not(eq(to,safe_)) {
                    revert(0,0)
                }
                // We offset the load address by 21 byte (operation byte + 20 address bytes)
                ...
                // We offset the load address by 85 byte (operation byte + 20 address bytes + 32 value bytes + 32 data length bytes)
                let data := add(transactions, add(i, 0x55))
                // grab the signature from the call data
                let sig2 := mload(shr(0xe0, data))
                /*if not(eq(sig,sig2)) {
                    revert(0,0)
                }*/
                let success := 0
...
contract SafeRelayer is GelatoRelayContext {
    ...
    constructor() {
        MULTISEND = address(new MultiSend2());
    }

    function multiSend(address safe, bytes memory transactions) onlyGelatoRelay public payable {
        pullFeesFrom(safe);
        // MultiSend2(MULTISEND).multiSend(safe, GnosisSafe.execTransaction.selector, transactions);
        // notably this multisend requires the following:
        // * at least one transaction is executed
        // * all calls are to the 'safe' contract
        // * all calls are execTransaction
        // these are used to ensure that it is not possible to abuse this interface
        // to waste fees with junk calls
        (bool success,) = MULTISEND.delegatecall(abi.encodeWithSelector(MultiSend2.multiSend.selector, safe, GnosisSafe.execTransaction.selector, transactions));
    ...
    function pullFeesFrom(address safe) internal {
