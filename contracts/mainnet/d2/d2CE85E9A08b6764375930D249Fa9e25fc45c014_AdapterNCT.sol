...
interface IOpenRegistry is IERC20Upgradeable {
    function setPrice(uint256 _price) external;

    function stablecoin() external returns (IERC20Upgradeable);

    function DrainTokens(IERC20Upgradeable[] calldata _tokens, address _receiver) external;

    function buyTokens(uint256 _amount, address _to) external;

    function getPrice() external view returns (uint256);

    function retireTokens(uint256 _amount) external;
...
import "../router/IAdapter.sol";
import "../IOpenRegistry.sol";
import "./AdapterNCTRolesMixin.sol";
import "../interfaces/IUniswapV2Router02.sol";
import "../interfaces/Toucan/INatureCarbonTonne.sol";
import "../interfaces/Toucan/ITCO2.sol";

/**
 * @title AdapterNCT
 * @author Aleh Natseuski
 * @dev This adapter is used to integrate Toucan's Nature 2.0 Carbon Tonne (NCT) token into the Senken ecosystem.
 */
contract AdapterNCT is IAdapter, Initializable, AdapterNCTRolesMixin {
    using SafeERC20 for IERC20;

    uint constant public FEE_PRECISION = 10000;

    IERC20 public stablecoin;
    address public feeCollector;
    uint public defaultFee;
    address public nctToken;
    IUniswapV2Router02 public dexRouter;

    // events
    event StablecoinUpdated(address indexed stablecoin);
    event DefaultFeeUpdated(uint fee);
    event FeeCollectorUpdated(address feeCollector);
    event DexRouterUpdated(address dexRouter);
    event NCTTokenUpdated(address nctToken);
    event FeeCollected(address indexed feeCollector, address indexed token, uint amount);
    event TokenBought(address indexed token, address indexed buyer, uint amount, uint price);
    event TokenRetired(address indexed token, address indexed retiree, uint amount);

    // errors
    error ZERO_ADDRESS();
    error ZERO_AMOUNT();
    error AMOUNT_EXCEED_MAX_TOKEN_AVAILABLE();
    error PRICE_TOO_HIGH();

    /// @param _stablecoin Stablecoin address
    /// @param _feeCollector Fee collector address. Collected fees will be send to this address
    /// @param _nctToken NCT pool token address
    /// @param _dexRouter DEX router address. Used to swap stablecoin to NCT
    /// @param _defaultFee Default fee in percents with FEE_PRECISION precision
    function initialize(address _stablecoin, address _feeCollector, address _nctToken, address _dexRouter, uint _defaultFee) initializer public {
        if (_stablecoin == address(0)) revert ZERO_ADDRESS();
        if (_feeCollector == address(0)) revert ZERO_ADDRESS();
        if (_nctToken == address(0)) revert ZERO_ADDRESS();
        if (_dexRouter == address(0)) revert ZERO_ADDRESS();
        ...
        emit NCTTokenUpdated(_nctToken);
        emit DexRouterUpdated(_dexRouter);
    }

    /// @inheritdoc IAdapter
    function buyToken(address buyer, address token, uint amount, uint maxStablecoin) external {
        if(amount == 0) revert ZERO_AMOUNT();
        if(amount > getAvailableAmount(token)) revert AMOUNT_EXCEED_MAX_TOKEN_AVAILABLE();

        uint nctRedeemFee = INatureCarbonTonne(nctToken).feeRedeemPercentageInBase();
        uint amountWithFee = amount * FEE_PRECISION / (FEE_PRECISION - nctRedeemFee);

        address[] memory path = new address[](2);
        path[0] = address(stablecoin);
        path[1] = address(nctToken);

        uint[] memory amounts = dexRouter.getAmountsIn(amountWithFee, path);
        uint amountIn = amounts[0];
        uint amountOut = amounts[1];

        uint amountInWithSenkenFee = amountIn * FEE_PRECISION / (FEE_PRECISION - defaultFee);
        if(amountInWithSenkenFee > maxStablecoin) revert PRICE_TOO_HIGH();

        stablecoin.safeTransferFrom(msg.sender, address(this), amountInWithSenkenFee);
        stablecoin.safeApprove(address(dexRouter), amountIn);
        dexRouter.swapExactTokensForTokens(amountIn, amountOut, path, address(this), block.timestamp);
        ...
        IERC20(token).safeTransfer(buyer, tokenBalance);

        emit TokenBought(token, buyer, tokenBalance, amountInWithSenkenFee);
        //transfer fee
        uint feeAmount = stablecoin.balanceOf(address(this));
        stablecoin.safeTransfer(feeCollector, feeAmount);
        emit FeeCollected(feeCollector, address(stablecoin), feeAmount);
    }

    /// @inheritdoc IAdapter
    function retireToken(address retiree, address token, uint amount) external {
        if(amount == 0) revert ZERO_AMOUNT();
        IERC20(token).safeTransferFrom(msg.sender, address(this), amount);
        ITCO2(token).retire(amount);
        emit TokenRetired(token, retiree, amount);
    }
    ...
    /// @notice Used by the manager to update default fee
    function updateDefaultFee(uint newDefaultFee) external {
        _onlyManager();
        defaultFee = newDefaultFee;
        emit DefaultFeeUpdated(newDefaultFee);
    }

    /// @notice Used by the manager to update feeCollector address
    ...
    /// @inheritdoc IAdapter
    function quote(address token, uint amount) external view returns (uint){
        if(amount == 0) revert ZERO_AMOUNT();
        if(amount > getAvailableAmount(token)) revert AMOUNT_EXCEED_MAX_TOKEN_AVAILABLE();
        ...
        uint[] memory amounts = dexRouter.getAmountsIn(amountWithFee, path);
        uint amountWithSenkenFee = amounts[0] * FEE_PRECISION / (FEE_PRECISION - defaultFee);
        return amountWithSenkenFee;
    }

    /// @inheritdoc IAdapter
    function getAvailableAmount(address token) public view returns (uint){
        return INatureCarbonTonne(nctToken).tokenBalances(token);
    }
...
contract AdapterNCTRolesMixin is AccessControlUpgradeable {
    // roles
    bytes32 internal constant MANAGER = keccak256("MANAGER");

    // events
    event DefaultAdminAdded(address indexed account);
    event DefaultAdminRemoved(address indexed account);
    ...
    function isDefaultAdmin(address account) public view returns (bool) {
        return hasRole(DEFAULT_ADMIN_ROLE, account);
    }

    function addDefaultAdmin(address account) public {
        grantRole(DEFAULT_ADMIN_ROLE, account);
        emit DefaultAdminAdded(account);
    }

    function renounceDefaultAdmin() public {
        renounceRole(DEFAULT_ADMIN_ROLE, msg.sender);
        emit DefaultAdminRemoved(msg.sender);
    }
    ...
    function addManager(address account) public {
        grantRole(MANAGER, account);
        emit ManagerAdded(account);
    }

    function revokeManager(address account) public {
        revokeRole(MANAGER, account);
        emit ManagerRemoved(account);
    }

    uint256[256] private __safe_upgrade_gap;
}
 ...
 * @author Aleh Natseuski
 * @dev This interface is used to integrate liquidity providers into the Senken ecosystem.
 */
interface IAdapter {
    /// @notice Buy specified amount of token for the specified buyer
    function buyToken(address buyer, address token, uint amount, uint maxStablecoin) external;

    /// @notice Retire specified amount of token for the specified retiree
    function retireToken(address retiree, address token, uint amount) external;

    /// @notice Returns the amount of stablecoin needed to buy the specified amount of token. Should include all fees.
    function quote(address token, uint amount) external view returns(uint);

    /// @notice Returns the amount of token available for buying
    function getAvailableAmount(address token) external view returns(uint);
}