...
contract MATICALPHA {
	...
	uint256 constant public INVEST_MIN_AMOUNT = 10 ether; // 0.01 MATIC
	...
	function getPlanBalanceByDeposit(address userAddress, uint256 depositIndex) public view returns(uint256) {
		...
		uint256 numberOfHoursPassed = (currentTime - planStart).mul(24).div(TIME_STEP);
		planBalance = plans[userDeposit.plan].stakePercent.mul(userDeposit.amount).mul(numberOfHoursPassed).div(24).div(10000).sub(userDeposit.planWithdrawn);
		return planBalance;
	}
	...
	function getHoldBalanceByDeposit(address userAddress, uint256 depositIndex) public view returns(uint256) {
		...
		if(user.deposits[depositIndex].eligible) {
			...
			uint256 numberOfHoursPassed = (currentTime - planStart).mul(24).div(TIME_STEP);
			holdBalance = plans[userDeposit.plan].holderPercent.mul(userDeposit.amount).mul(numberOfHoursPassed).div(24).div(10000).sub(userDeposit.holdwithdrawn);
			return holdBalance;
		}
		else return 0;
	}
