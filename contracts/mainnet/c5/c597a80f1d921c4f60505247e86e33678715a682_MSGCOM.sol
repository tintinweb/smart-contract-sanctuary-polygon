...
contract MSGCOM {
	...
	uint256 constant public INVEST_MIN_AMOUNT = 5e6;
	...
	constructor(address _usdtAddr,address payable wallet, address payable dWallet, uint256 startDate) {
        ...
        plans.push(Plan(45, 33));  // 3.3% per day for 45 days
        plans.push(Plan(60, 26));  // 2.6% per day for 60 days
        plans.push(Plan(30, 61)); // 6.1% per day for 30 days
        plans.push(Plan(45, 46)); // 4.6% per day for 45 days
        plans.push(Plan(60, 47)); // 4.7% per day for 60 days
	}
	...
	function invest(address referrer, uint256 _amount,uint8 plan) external  {
		...
		require(_amount >= INVEST_MIN_AMOUNT, "Minimum investment is 5 USDT");
        require(_amount <= INVEST_MAX_AMOUNT, "You can not invest more than 50000 at once");
