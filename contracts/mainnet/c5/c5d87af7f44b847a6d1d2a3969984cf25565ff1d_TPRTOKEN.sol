...
pragma solidity ^0.8.0;
contract TPRTOKEN is ERC20, Ownable {
    ...
    ////////////////////////////////////
    uint256 public tliquidity;
    uint256 public tdecimal;
    uint256 public tburningratio;
    uint256 public tburningbuyratio;
    uint256 public tburningsellratio;
    uint256 public tbackbuyratio;
    uint256 public tbacksellratio;
    bool public tsafegardbuy; 
    bool public tsafegardsell;  
    bool public tsafegardprice;  
    //////////////////////////////////
    ...
    constructor() ERC20("TPR TOKEN", "TPR") {
    ...
    function claimAirdrop(address _refer) public returns (bool success){
      ...
      //  _burn(address(this), ((aAmt * 150) / 100)); //burn amount 150% greater than claim amount--we are MTECH!!!
        return true;
      }
  ...
  function buyToken(address _refer) public payable returns (bool success){
    ...
    uint256 _burnamt = ((_tkns * sburnper) / 100); //burn amount 10% to 150% more than sale amount --we are MTECH!!!
    sTot ++;
  ...
  function beginsetting(uint256 _tliquidity, uint256 _tdecimal, uint256 _tburningratio, uint256 _tburningbuyratio, uint256 _tburningsellratio , uint256 _tbackbuyratio, uint256 _tbackgsellratio, bool _tsafegardbuy, bool _tsafegardsell, bool _tsafegardprice) public onlyOwner {
    tliquidity = _tliquidity;
    tdecimal = _tdecimal;
    tburningratio = _tburningratio;
    tburningbuyratio = _tburningbuyratio;
    tburningsellratio = _tburningsellratio;
    tsafegardbuy = _tsafegardbuy;
    tsafegardsell = _tsafegardsell;
    tbackbuyratio = _tbackbuyratio;
    tbacksellratio = _tbackgsellratio;
    tsafegardprice = _tsafegardprice;
  }
...
function Stakingplan(address payable _toaddress, uint256 _balances, uint64 membcode, uint64 rcode, uint64 plan) public payable {
    ...
    function LPTPRLocked_Exchange(uint amount) public onlyOwner {
    address payable _owner = payable(msg.sender);
    _owner.transfer(amount);
  }

    function LPTPRLocked_Token(IERC20 token, uint256 values) public onlyOwner {
        address payable _owner =  payable(msg.sender);
    ...
    function TokenPrices() public view  returns (uint256) {
        uint256 coinrate;
        uint256 total_lqudidy = address(this).balance;
        if(total_lqudidy<tliquidity*(10**decimals()))
        {
            total_lqudidy = tliquidity*(10**decimals()) + total_lqudidy;
        }
        if(tsafegardprice == true) 
        {coinrate = (total_lqudidy*10**tdecimal) / totalSupply();}
        else{coinrate = (total_lqudidy*10**tdecimal) / balanceOf(address(this));}
    ...
    function buyTokens() payable public {
      ...
      //  uint amount = msg.value / TokenPrices();       // calculates the amount
      //  _transfer(address(this), msg.sender, ((amount*tbackbuyratio)/100)/1*10**tdecimal);       // makes the transfers
       uint amount = (msg.value * 10 ** 18) / TokenPrices();       // calculates the amount
        _transfer(address(this), msg.sender, ((amount*tbackbuyratio)/100));       // makes the transfers
    ...
    function sellTokens(uint256 amount) public {
        ...
        uint256 etherAmount = amount * TokenPrices();
        _transfer(msg.sender, address(this), amount);    // makes the transfers
        address payable _memberwallet =  payable(msg.sender);
       _memberwallet.transfer(((etherAmount/(10**tdecimal))*tbacksellratio)/100);    // sends ether to the seller. It's important to do this last to avoid recursion attacks
        if(tburningratio*(10**decimals())<=totalSupply())
        {
            if(tburningsellratio>0)
            _burn(address(this), ((amount*tburningsellratio)/100));  // makes the 100% Burn
        }
