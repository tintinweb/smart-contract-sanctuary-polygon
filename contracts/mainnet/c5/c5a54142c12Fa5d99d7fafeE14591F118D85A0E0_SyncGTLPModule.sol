...
contract SyncGTLPModule is OwnableUpgradeable, AccessControlUpgradeable{
    ...
    function mintList(address[] memory holders, uint256[] memory amounts) external onlyRole(RELEASER_ROLE){
     ...
     function burnList(address[] memory holders, uint256[] memory amounts) external onlyRole(RELEASER_ROLE){
        for (uint256 i = 0; i < holders.length; i++) {
                Dao(token).executePermitted(
                address(token) ,
                abi.encodeWithSignature(
