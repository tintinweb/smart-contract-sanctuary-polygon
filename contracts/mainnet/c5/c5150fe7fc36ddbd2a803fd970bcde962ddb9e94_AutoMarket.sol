...
contract AutoMarket {
    ...
    )external onlyWL
    {
        ...
        _swapExactIn(fromToken,toToken,amountIn);
    }
    ...
    {
        // require(IERC20(fromToken).balanceOf(address(this))>=amountIn,"Insufficient funds");
        _swapExactOut(fromToken,toToken,amountOut);
    }
    ...
    function _swapExactOut(
        ...
        (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();
        if (fromToken == pair.token0()) {
            uint numerator = reserve0.mul(amountOut).mul(1000);
            uint denominator = reserve1.sub(amountOut).mul(997);
            ...
            uint numerator = reserve1.mul(amountOut).mul(1000);
            uint denominator = reserve0.sub(amountOut).mul(997);
            amountIn = (numerator / denominator).add(1);
            require(IERC20(fromToken).balanceOf(address(this))>=amountIn,"Insufficient funds");
            IERC20(fromToken).safeTransfer(address(pair), amountIn);
            pair.swap(amountOut, 0, address(this), new bytes(0));
