...
import "./UnifarmAccounts.sol";

contract MessagingUpgradeable is Initializable, OwnableUpgradeable, PausableUpgradeable {

    uint256 dappId;     // dappID for this decentralized messaging application (should be fixed)
    ...
    struct Attachment {
        string location;
        address receiver;
        string encRsaKeyReceiver;       // encrypted using sender's private key and receiver's public key
        string encRsaKeySender;         // encrypted using sender's public key
        bool isPrimaryEnc;
    }
    ...
    struct Message {
        string encTextMessageReceiver;      // encrypted using sender's private key and receivevr's public key
        string encTextMessageSender;        // encrypted using sender's public key
        uint256[] attachmentIds;
        bool isPrimaryEnc;                  // to check if the message has been encrypted by the primary wallet
        uint256 timestamp;
    }
    ...
    // sender => messages[]
    mapping(address => Message[]) public userSentMessages;

    // receiever => messages[]
    mapping(address => Message[]) public userReceivedMessages;
    ...
    function writeData(
        ...
        address _receiver,
        string memory _encRsaKeyReceiver,
        string memory _encRsaKeySender,
        bool _isPrimaryEnc
        ...
        Attachment memory attachment = Attachment({
            location: _location,
            receiver: _receiver,
            encRsaKeyReceiver: _encRsaKeyReceiver,
            encRsaKeySender: _encRsaKeySender,
            isPrimaryEnc: _isPrimaryEnc
        });
    ...
    function newMessage(
        address _to,
        string calldata _encTextMessageReceiver,
        string calldata _encTextMessageSender,
        Attachment[] calldata _attachments,
        bool _isPrimaryEnc,
        ERC20 _token
    ) public {
        bool isSendWhitelisted = isWhitelisted[_msgSender()][_to];
        ...
        if(isSpamProtecting[_to] && !isSendWhitelisted) {
            uint256 amount = getTokenAmountToSend(address(_token));
            require(amount > 0, "INVALID_TOKEN");
            uint256 adminAmount;
            if(address(_token) != ufarmToken) {
                adminAmount = amount / 5;   // 20% goes to admin
                amount -= adminAmount;
                _token.transferFrom(_msgSender(), spamTokensAdmin, adminAmount);
            }
            _token.transferFrom(_msgSender(), _to, amount);
        }

        if(!isSendWhitelisted || !isReceiveWhitelisted) {
            isWhitelisted[_msgSender()][_to] = true;
            isWhitelisted[_to][_msgSender()] = true;
        }

        uint[] memory attachmentIds;
        for (uint i = 0; i < _attachments.length; i++) {
            uint256 dataId = writeData(
                _attachments[i].location, 
                _attachments[i].receiver, 
                _attachments[i].encRsaKeyReceiver, 
                _attachments[i].encRsaKeySender, 
                _attachments[i].isPrimaryEnc
            );
            attachmentIds[i] = dataId;
        }

        Message memory message = Message({
            encTextMessageReceiver: _encTextMessageReceiver,
            encTextMessageSender: _encTextMessageSender,
            isPrimaryEnc: _isPrimaryEnc,
            attachmentIds: attachmentIds,
            timestamp: block.timestamp
        });
        messages[_msgSender()][_to].push(message);
        userSentMessages[_msgSender()].push(message);
        userReceivedMessages[_to].push(message);
    }

    function getTokenAmountToSend(
        address _token
    ) public view returns (uint256) {
        for (uint256 i = 0; i < userSpamTokens[_msgSender()].length; i++) {
            if(userSpamTokens[_msgSender()][i].token == _token)
                return userSpamTokens[_msgSender()][i].amount;
        }
        return 0;
    }
    ...
    function getMessageForReceiver(
        ...
        Message[] memory receivedMessages;
    ...
    function adminAddPaymentToken(
        ...
        require(_msgSender() == unifarmAccounts.getDappAdmin(_dappId), "ONLY_DAPP_ADMIN");
    ...
    function addSpamProtectionToken(
        ...
        uint len = userSpamTokens[_msgSender()].length;
        for(uint256 i = 0; i < len; i++) {
            // if token already exists then update its price and return
            if(userSpamTokens[_msgSender()][i].token == _token) {
                userSpamTokens[_msgSender()][i].amount = _amount;
                return;
            }
        }
        ...
        userSpamTokens[_msgSender()].push(token);
    }
    ...
    function removeSpamProtectionToken(
        ...
        for(uint256 i = 0; i < len; i++) {
            if(userSpamTokens[_msgSender()][i].token == _token) {
                if(i < len-1) {
                    userSpamTokens[_msgSender()][i] = userSpamTokens[_msgSender()][len-1];
                }
                userSpamTokens[_msgSender()].pop();
                return;
            }
        }
        revert("NO_TOKEN");
    ...
    function addWhitelist(
        ...
        isWhitelisted[_account][_msgSender()] = true;
    }

    function removeWhitelist(
        address _account
    ) public {
        isWhitelisted[_account][_msgSender()] = false;
    }
...
contract UnifarmAccountsUpgradeable is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    mapping(address => mapping(uint256 => SecondaryWallet)) public userWallets;

    function __UnifarmAccounts_init() public initializer {
        __Ownable_init();
        __Pausable_init();
    }
    ...
    function addNewDapp(
        ...
        // need a condition to check that the dapp doesn't already exists******************************
        Dapp memory dapp = Dapp({
            name: _name,
            imageUrl: _imageUrl,
            admin: _admin
        });
        dapps[dappsCount++] = dapp;
    }

    function getDappAdmin(
        uint256 _dappId
    ) public view returns (address) {
        return dapps[_dappId].admin;
    }
    ...
    function createWallet(
        ...
        require(userWallets[_msgSender()][_dappId].account == address(0), "ACCOUNT_ALREADY_EXISTS");
        SecondaryWallet memory wallet = SecondaryWallet({
            account: _account,
            encPvtKey: _encPvtKey,
            publicKey: _publicKey
        });
        userWallets[_msgSender()][_dappId] = wallet;
    }
