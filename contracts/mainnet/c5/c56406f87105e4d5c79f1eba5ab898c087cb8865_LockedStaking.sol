...
contract LockedStaking is IERC20Staking {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public periodicTime = 365 days;
    uint256 planLimit = 3;
    uint256 public refRate = 3; // 3%
    ...
    function referralStake(uint256 _stakingId, uint256 _amount, address _referrer) public {   
        if ((_referrer != msg.sender) && (_referrer != address(0))) {
            if(userRef[msg.sender].status == false){
                ...
                address up1 = userRef[_referrer].referredBy; 
                address up2 = userRef[up1].referredBy; 
                userRef[up1].totalRef += 1;
                userRef[up2].totalRef += 1;                                             
            }                      
        }
