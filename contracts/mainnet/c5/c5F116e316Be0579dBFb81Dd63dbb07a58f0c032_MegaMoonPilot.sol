...
contract MegaMoonPilot is Ownable {
    ...
    // Total rate. Must be the sum of all rate in all pools.
    ...
    function add(IERC20 _lpToken, bool _withUpdate, uint256 _moonPerBlock) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        totalRate = totalRate.add(_moonPerBlock);
    }
    ...
    function set(uint256 _pid, bool _withUpdate, uint256 _moonPerBlock) public onlyOwner {
        ...
        totalRate = totalRate.add(_moonPerBlock).sub(poolInfo[_pid].moonPerBlock);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            require (moonBal >= pending, 'Token not enough to transfer');
            if(pending > 0) {
    ...
    // Withdraw all moon in pools. OWNER EMERGENCY ONLY.
