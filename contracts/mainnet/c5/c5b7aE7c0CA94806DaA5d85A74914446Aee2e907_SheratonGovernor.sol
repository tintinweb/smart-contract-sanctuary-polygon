...
contract SheratonGovernor is Governor, GovernorSettings, GovernorCompatibilityBravo, GovernorVotesComp, GovernorTimelockCompound, Ownable {

    mapping(address => bool) public secretory;
    mapping(address => bool) public bot;
    
    event UpdateSecretory(address account, bool value);
    event UpdateBOT(address account, bool value);
    constructor(
        ERC20VotesComp _token, 
        ICompoundTimelock _timelock,
        uint256 _votingDelay,            // 1 // 1 block
        uint256 _votingPeriod,            // 604800 // 1 week    //1800 30 min   //600 - 10 min //300 - 5 min
        uint256 _votingThreshold
        )
        Governor("SheratonGovernor")
        GovernorSettings(_votingDelay /* 1 block */, _votingPeriod /* 1 week */, _votingThreshold)
    ...
    function updateBOT(address _account) public onlyOwner
    {
        bot[_account] = !bot[_account];
        emit UpdateBOT(_account, bot[_account]);
    }
...
abstract contract Governor is Context, ERC165, EIP712, IGovernor {
    ...
    function _castVote(
        ...
        uint256 weight = getVotes(account, block.number-1);
