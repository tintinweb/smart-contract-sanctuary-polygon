...
contract UniswapV3SwapRouterACLUUPS is OwnableUpgradeable, UUPSUpgradeable {
    ...
    address private constant MSG_SENDER_MAP = address(1);
    ...
    //ACL functions 
    function multicall(uint256,bytes[] calldata data) external onlySelf {
    ...
    function wrapETH(uint256) external onlySelf{} // just use for eth case
    ...
    function exactInputSingle(ExactInputSingleParams calldata params) external onlySelf {
        ...
        if(isCheckRoleSlippage){
            uint256 amountIn = params.amountIn == 0 ? _checkedValue : params.amountIn;
            slippageCheck(params.tokenIn, params.tokenOut, amountIn, params.amountOutMinimum);
        }
    ...
    function exactInput(ExactInputParams memory params) external onlySelf {
        ...
        if(isCheckRoleSlippage){
            uint256 amountIn = params.amountIn == 0 ? _checkedValue : params.amountIn;
            slippageCheck(tokenIn, tokenOut, amountIn, params.amountOutMinimum);  
        }
        check_recipient(params.recipient); 
    ...
    function swapExactTokensForTokens(uint256 amountIn,uint256 amountOutMin,address[] calldata path,address to) external onlySelf{
        ...
        address recipient = to == MSG_SENDER_MAP ? safeAddress : to;
        check_recipient(recipient);
        if(isCheckRoleSlippage){
            uint256 amountInReal = amountIn == 0 ? _checkedValue : amountIn;
            slippageCheck(tokenIn, tokenOut, amountInReal, amountOutMin);
        }
    }  
