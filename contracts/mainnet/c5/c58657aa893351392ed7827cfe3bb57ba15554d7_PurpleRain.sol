...
contract PurpleRain is Context, Ownable, IBEP20 {
    ...
    uint16[5] private ref_bonuses = [100, 30, 20, 10, 10]; 
    ...
    constructor() {
        ...
        tarifs[1] = Tarif(30, 120); //4% daily for 30 days => 120% ROI    
        tarifs[2] = Tarif(30, 135); //4.5% daily for 30 days => 135% ROI    
        tarifs[3] = Tarif(35, 175); //5% daily for 35 days => 175% ROI    
        tarifs[4] = Tarif(40, 240); //6% daily for 40 days => 240% ROI    
        tarifs[5] = Tarif(20, 150); //10+% daily for 20 days => 150% ROI    
        
    }

       
    function OrderMuse(address referral, uint256 tarip) public payable {
        
        require(msg.value >= 5 ether, "Minimum Deposit is 5 MATIC!");
        if(tarip == 5){
            require(msg.value >= 300 ether, "Minimum Deposit is 300 MATIC!");
            require(msg.value <= 500 ether, "Maximum Deposit is 500 MATIC!");
        }
    ...
    function UpgradeROI() public payable {        
        require(msg.value >= 100 ether, "Minimum is 100 MATIC!");
        ...
        }else if(msg.value == 200 ether){
            player.roi_upper = player.roi_upper + 2;
        }
    }
    ...
    function computePayout(address _addr) view external returns(uint256 value) {
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            if(from < to) {
                value += dep.amount * (to - from) * ((tarif.percent / tarif.life_days)+player.roi_upper) / 8640000;
            }
        }
        return value;
    }
    ...
    function HappyEnding() external {
        ...
        require(player.dividends > 0, "No earnings yet!");
