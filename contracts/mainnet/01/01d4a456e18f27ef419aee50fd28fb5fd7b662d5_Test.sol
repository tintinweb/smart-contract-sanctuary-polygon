...
contract Test {
    ...
    function Deposit(address ref,uint amount) public payable {
    ...
    function Joinin(uint index) public payable{
        require(bonustype[index].isActive, "index false");
        require(!user[msg.sender].isActive, "user false");
        require((user[msg.sender].amount+msg.value)>=bonustype[index].Amount, "no amount");
        Investor storage  userer = user[msg.sender];
