...
contract OnlySwapV8 {
    ...
    function autoLiquidityFee(uint fee) internal{
        uint minBalanceFeeToken=(fee*liquidity.getTokenPrice())/(10**IERC20(USDC).decimals());
        if(IERC20(HBLOCK).balanceOf(address(liquidity))>=minBalanceFeeToken){
            ...
            if(tAmount>200*10**IERC20(HBLOCK).decimals()){
    ...
    function payfee(address token) internal {
        ...
        if(token!=USDC){
            (uint price,IRouter.Swap_router[] memory path) = route.getGasEfficientSwapsByTokens(token,USDC,feeOnTokenPair);
            if(price>0){
    ...
    function bestSwapTokensForTokensByPath(IRouter.Swap_router[] calldata path,uint amountA,uint minAmountTokenB) external payable {
        ...
        (hasTokenA,tokenB)=receiveTokenA(tokenA,tokenB,amountA,isNative);
        if(hasTokenA){
            for(uint32 i;i<path.length;i=unsafe32_inc(i)){
                swapTokensForTokens(path[i].dex.router,path[i].tokenA,path[i].tokenB,path[i].dex.version);
            }
    ...
    function receiveTokenA(address tokenA,address tokenB,uint amountA,bool isNative) internal returns(bool,address){
        if(isNative && WMATIC==tokenB){
