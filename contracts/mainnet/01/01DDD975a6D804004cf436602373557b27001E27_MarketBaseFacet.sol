...
/// @dev Returns an asset with asset ID `assetId`.
    function getAsset(
        address owner,
        address token,
        uint256 id
        ...
        return s.market.assets[owner][token][id]; 
    }

/// @dev Returns a bundle with bundle ID `bundleId`.
    function getBundle(
        uint256 bundleId
    ) public view returns (Bundle memory) {  
        
        return s.market.bundles[bundleId];
    }
...
/// @dev Sets signer account.
    function setSigner(
        ...
        s.marketConfig.signers[signer] = enabled;
    }

/// @dev Sets resolver account.
    function setResolver(
        address resolver,
        bool enabled
    ) external onlyDiamondOwner {

        s.marketConfig.resolvers[resolver] = enabled;
    }

/// @dev Enables / disables ERC20 `token` for use as a payment token.
    function setPaymentToken(
        ...
        // s.marketConfig.paymentTokens[token] = enabled;
        s.marketConfig.minPrices[token] = minPrice;
    }

/// @dev Enables / disables ERC20 `token` for use as a target token.
    function setTargetToken(
        address token,
        bool enabled
    ) external onlyDiamondOwner {

        s.marketConfig.targetTokens[token] = enabled;
    }

/// @dev Sets platform ERC20 token
    function setPlatformToken(
        address token
    ) external onlyDiamondOwner {

        s.marketConfig.platformToken = token;
    }

/// @dev Setup an asset token.
    function setAssetToken(
        ...
        s.marketConfig.assetTokenStandards[token] = standard;
        s.marketConfig.assetTokenVersions[token] = version;
        s.marketConfig.assetTokens[standard][version] = token;
    }

/// @dev Sets default `version` for asset token `standard`.
    function setAssetTokenVersion(
        AssetStandard standard,
        AssetVersion version
    ) external onlyDiamondOwner {

        s.marketConfig.defaultTokenVersions[standard] = version;
    }

/// @dev Sets {skipPlatformToken} value
    function setSkipPlatformToken(
        bool value
    ) external onlyDiamondOwner {

        s.marketConfig.skipPlatformToken = value;
    }

/// @dev Sets stepped comission for payment token `token`.
/// @param steps Lower bounds of token amount intervals starting from 0.
/// @param percentages Comission percentages corresponding to commision steps.
/// @dev `steps` and `percentages` must have the same length.
    function setComission(
        address token,
        uint256[] memory steps,
        uint24[] memory percentages
     ) external onlyDiamondOwner {

        require(
            steps.length == percentages.length,
            "BaseFacet: ARRAY_SIZE_MISMATCH"
        );

        s.marketConfig.comissionSteps[token] = steps;
        s.marketConfig.comissionPercentages[token] = percentages;
    }

/// @dev Sets comission receiver address
    function setComissionReceiver(
        address receiver
    ) external onlyDiamondOwner {

        s.marketConfig.comissionReceiver = receiver;
    }

/// @dev Sets maximum royalty value
    function setMaxRoyalty(
        uint24 value
    ) external onlyDiamondOwner {

        s.marketConfig.maxRoyalty = value;
    }

/// @dev Sets maximum amount of assets in a bundle
    function setMaxBundleSize(
        uint256 value
    ) external onlyDiamondOwner {

        s.marketConfig.maxBundleSize = value;
    }
...
/// @dev Sets swap router
    function setExchangeRouter(
        address router
    ) external onlyDiamondOwner {

        s.exchangeConfig.router = router;
    }

/// @dev Sets swap max delay
    function setSwapMaxDelay(
        uint256 delay
    ) external onlyDiamondOwner {

        s.exchangeConfig.maxSwapDelay = delay; 
    }
...
//  --------------------------------------------------------------------------
    ...
    function auctionSetMinReservePrice(
        ...
        s.marketConfig.minReservePriceTotal[paymentToken] = price;
    }

    function auctionSetStep(
        address paymentToken,
        uint256 step
    ) external onlyDiamondOwner {

        s.marketConfig.auctionSteps[paymentToken] = step;
    }
    ...
    function auctionSetDurationLimits(
        ...
        s.marketConfig.minAuctionDuration = minDuration;
        s.marketConfig.maxAuctionDuration = maxDuration;
    }

    function auctionSetProlongation(
        uint64 prolongation
    ) external onlyDiamondOwner {

        s.marketConfig.auctionProlongation = prolongation;
    }

    function auctionSetRelaxation(
        uint64 relaxation
    ) external onlyDiamondOwner {

        s.marketConfig.auctionRelaxationTime = relaxation;
    }
