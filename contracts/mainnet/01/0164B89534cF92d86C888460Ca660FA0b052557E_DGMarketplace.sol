    ...
    EIP712MetaTransactionUpgradeable
{
    ...
    function sell(
        ...
        require(
            ...
            "DGMarketplace#sell: Seller is backlisted"
        );
        require(
    ...
    function buy(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            require(
                order.price > 0,
                "DGMarketplace#buy: Collection is claimable for free"
            );
            ...
            if (implements2981) {
                (royaltyBeneficiary, royaltyFee) = getRoyalty(
                    _nftAddress,
                    tokenId,
                    saleAmount - saleFee
                );
    ...
    function paperCallback(
        ...
        nonReentrant
        onlyPaper(
            keccak256(abi.encode(paymentId, _transferTo, _nftAddress, tokenId)),
            _nonce,
            _signature
        )
    {
        ...
        require(
            acceptedToken.balanceOf(address(this)) >= order.price,
            "DGMarketplace#PaperCallBack: ICE amount is lower than price to pay"
        );
        require(
            acceptedToken.transfer(
                order.beneficiary,
                order.price - (order.price * fee) / BASE_FEE
            ),
            "DGMarketplace#PaperCallBack: TRANSFER_PRICE_FAILED"
        );
        require(
            acceptedToken.transfer(feeOwner, (order.price * fee) / BASE_FEE),
            "DGMarketplace#PaperCallBack: TRANSFER_FEES_FAILED"
        );
    ...
    function setBackWallet(address _backWallet) public {
        require(
            msgSender() == backWallet,
            "DGMarketplace#cancel: FAILED_UNAUTHORIZED"
        );
    ...
    function setFreeClaimCost(uint256 _freeClaimCost) public {
        require(
            msgSender() == backWallet,
            "DGMarketplace#cancel: FAILED_UNAUTHORIZED"
        );
    ...
    function withdrawERC721(address _nftAddress, uint256 tokenId) external {
        address sender = msgSender();
        require(
            sender == backWallet,
            "DGMarketplace#cancel: FAILED_UNAUTHORIZED"
