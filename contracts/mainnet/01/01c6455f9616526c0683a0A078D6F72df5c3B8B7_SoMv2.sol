...
contract SoMv2 is SoM  {
    ...
    function setWhiteList(address[] memory whitelists, bool[] memory enableds) external onlyOwner {
        uint whitelistsLegth = whitelists.length;
        require(whitelistsLegth == enableds.length,"Not Same");
        for (uint i=0; i<whitelistsLegth; i++) {
            _whiteList[whitelists[i]] = enableds[i];
        }
    }
...
contract SoM is Initializable, ERC20Upgradeable, ERC20BasicApproveExtension, WithOwnerUpgradeable, WithSuperOperators,ERC2771HandlerUpgradeable {
