...
// File contracts/distribution/HelioMasterNode.sol
...
contract HelioMasterNode {
    ...
    IERC20 public constant HELIO = IERC20(0xB38B969f151E8217C917fE0532382172BB6Fb83E);
    uint256[] public tierAllocPoints = [1 ether, 2.1 ether, 4.41 ether, 9.261 ether, 19.4481 ether];
    uint256[] public tierAmounts = [500 ether, 1000 ether, 2000 ether, 4000 ether, 8000 ether];
    struct User {
    ...
    constructor(uint256 _startTime) public {
        maxReturnPercent = 400; // 400% roi
        dripRate = 2880000; // 3% daily
        treasuryFeePercent = 2; // 2% fee
    ...
    function claimTreasuryRewards() external {
        if (treasury_rewards > 0) {
            HELIO.safeTransfer(dev, treasury_rewards);
            treasury_rewards = 0;
        }   
    }
    ...
    function create(uint256 nodeTier, uint256 numNodes) external {
        ...
        require(HELIO.balanceOf(_sender) >= tierPrice, "Insufficient balance");
        require(HELIO.allowance(_sender, address(this)) >= tierPrice, "Insufficient allowance");
        HELIO.safeTransferFrom(_sender, address(this), tierPrice);
        ...
        treasury_rewards = treasury_rewards.add(
            tierPrice.mul(SafeMath.sub(100, treasuryFeePercent)).div(100)
        );
    ...
    function claim() public {
        ...
        if (_rewards > 0) {
            ...
            IERC20(HELIO).safeTransfer(_sender, _rewards);
    ...
    function getRewardDrip() public view returns (uint256) {
        if (lastDripTime < block.timestamp) {
            uint256 poolBalance = getHelioBalancePool();
    ...
    function getHelioBalance() public view returns (uint256) {
        return IERC20(HELIO).balanceOf(address(this));
    }

     function getHelioBalancePool() public view returns (uint256) {
        return getHelioBalance().sub(total_rewards).sub(treasury_rewards);
    }
