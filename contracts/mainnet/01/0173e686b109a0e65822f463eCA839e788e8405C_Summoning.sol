...
contract Summoning is Ownable, VRFConsumerBase {
    ...
    function summon(uint256 _orbID, address _beneficiary, uint256 _nonce) external onlyWhitelisted {
        require(!_usedNonces[_beneficiary][_nonce], "Nonce already used");
        require(orb.balanceOf(_beneficiary, _orbID) > 0, "not owning the orb");
        require(!isSummoning[_beneficiary][_orbID], "already requested to summon");
        isSummoning[_beneficiary][_orbID] = true;

        (, uint8 rarity, ) = orb.orbs(_orbID);

        bytes32 requestID = getRandomNumber();
        SummoningRequest storage request = summoningRequests[requestID];
        request.requester = _beneficiary;
        request.orbID = _orbID;

        _usedNonces[_beneficiary][_nonce] = true;

        emit SummoningRequested(_beneficiary, _orbID, requestID);
    }
    ...
    function upgradeProphet(uint256[5] memory _ids, address _beneficiary, uint256 _nonce)
        external
        isUnique(_ids)
        owningItems(prophet, _ids, _beneficiary)
        onlyWhitelisted
    {
        require(!_usedNonces[_beneficiary][_nonce], "Nonce already used");
        for (uint256 i = 0; i < 4; i++) {
            require(
                _hasSameGenerationRarityRace(_ids[i], _ids[i + 1]),
                "prophets should have same generation, rarity and race"
            );
        }

        (, uint16 rarity, , , , ) = prophet.prophets(_ids[0]);
        ...
        for (uint256 i = 0; i < 5; i++) {
            request.ids[i] = _ids[i];
        }

        _usedNonces[_beneficiary][_nonce] = true;

        emit UpgradeRequested(_beneficiary, true, _ids, requestID);
    }

    function upgradeItem(uint256[5] memory _ids, address _beneficiary, uint256 _nonce)
        external
        isUnique(_ids)
        owningItems(item, _ids, _beneficiary)
        onlyWhitelisted
    {
        require(!_usedNonces[_beneficiary][_nonce], "Nonce already used");
        for (uint256 i = 0; i < 4; i++) {
            require(
                _hasSameGenerationRarityClassMagicSource(_ids[i], _ids[i + 1]),
                "items should have same generation, rarity, class and magic source"
            );
        }

        (, uint16 rarity, , , , , ) = item.items(_ids[0]);
        ...
        emit UpgradeRequested(_beneficiary, false, _ids, requestID);
    }
