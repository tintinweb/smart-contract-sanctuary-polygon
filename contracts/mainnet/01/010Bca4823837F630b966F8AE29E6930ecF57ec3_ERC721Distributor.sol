...
import {PlanetMojoERC721Base} from "../token/erc721/base/PlanetMojoERC721Base.sol";
import {IERC165} from "@openzeppelin/contracts/utils/introspection/IERC165.sol";
import {IERC721Receiver} from "@openzeppelin/contracts/token/ERC721/IERC721Receiver.sol";

import "@openzeppelin/contracts/access/Ownable.sol";

contract ERC721Distributor is Ownable, IERC165 {
    ...
    function safeTransferToSingleRecipient(
        address erc721ContractAddress,
        address recipient,
        uint256[] memory ids
    ) public onlyOwner {
        PlanetMojoERC721Base erc721Contract = PlanetMojoERC721Base(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            erc721Contract.safeTransferFrom(address(this), recipient, id);
            assert(erc721Contract.ownerOf(id) == recipient);
        }
    }

    function transferToSingleRecipient(
        address erc721ContractAddress,
        address recipient,
        uint256[] memory ids
    ) public onlyOwner {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            erc721Contract.transferFrom(address(this), recipient, id);
            assert(erc721Contract.ownerOf(id) == recipient);
        }
    }

    function safeTransferToManyRecipients(
        address erc721ContractAddress,
        ...
        require(recipients.length == ids.length, "recipients and ids must be the same length");
    ...
    function transferToManyRecipients(
        address erc721ContractAddress,
        address[] memory recipients,
        uint256[] memory ids
    ) public onlyOwner {
    ...
    function safeTransferManyTokensToManyRecipients(
        address erc721ContractAddress,
        address[] memory recipients,
        uint256[][] memory ids
    ) public onlyOwner {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            safeTransferToSingleRecipient(erc721ContractAddress, recipient, ids[i]);
        }
    }

    function transferManyTokensToManyRecipients(
        address erc721ContractAddress,
        address[] memory recipients,
        uint256[][] memory ids
    ) public onlyOwner {
       ...
       require(recipients.length == ids.length, "recipients and ids must be the same length");
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            transferToSingleRecipient(erc721ContractAddress, recipient, ids[i]);
        }
    }
