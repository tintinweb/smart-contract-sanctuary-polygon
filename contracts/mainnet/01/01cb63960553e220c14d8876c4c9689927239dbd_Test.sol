...
// File: @quadrata/contracts/storage/QuadConstant.sol
...
// File: @quadrata/contracts/interfaces/IQuadSoulbound.sol
...
// File: @quadrata/contracts/interfaces/IQuadPassportStore.sol
...
// File: @quadrata/contracts/interfaces/IQuadPassport.sol
...
// File: @quadrata/contracts/storage/QuadGovernanceStore.sol
...
// File: @quadrata/contracts/interfaces/IQuadGovernance.sol
...
// File: @quadrata/contracts/storage/QuadPassportStore.sol
...
// File: @quadrata/contracts/interfaces/IQuadReader.sol
...
contract Test {
    ...
    IQuadReader constant reader =
        IQuadReader(0xFEB98861425C6d2819c0d0Ee70E45AbcF71b43Da);
    mapping(address => mapping(bytes32 => IQuadPassportStore.Attribute[]))
        internal attributes;
    ...
    function encodeAttr(string calldata _attr) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(_attr));
    }

    function saveAttributes(address _account, string calldata _attr) external {
        bytes32 attr = encodeAttr(_attr);
        uint256 queryFee = reader.queryFee(_account, attr);

        IQuadPassportStore.Attribute[] memory attrs = reader.getAttributes{
            value: queryFee
        }(_account, attr);
        for (uint256 i; i < attrs.length; i++) {
            attributes[_account][attr].push(attrs[i]);
        }
    }

    function getAttributesCount(address _account, string calldata _attr)
        public
        view
        returns (uint256)
    {
        bytes32 attr = encodeAttr(_attr);
        return attributes[_account][attr].length;
    }

    function getAttributes(
        address _account,
        uint256 _attrIndex,
        string calldata _attr
    ) public view returns (IQuadPassportStore.Attribute memory) {
        bytes32 attr = encodeAttr(_attr);
        return attributes[_account][attr][_attrIndex];
    }
