...
interface IFactory {
    ...
    function enterPriceMATIC() external view returns(uint256);
...
contract PickPicture is PriceFeed, Ownable {
    ...
    function newGame(
        ...
        rePointGames.register{
            value : rePointGames.enterPriceMATIC()
        }(
            _username, "", msg.sender, _avatarId
        );
        newGame();
    }
    ...
    function newGame() public payable {
        ...
        uint256 enterPriceMATIC = msg.value;
        require(
            rePointGames.userAddrRegistered(userAddr),
            "PickPicture : First you have to register in rePoint Games"
        );
        uint256 enterPriceUSD = enterPriceMATIC * MATIC_USD() /10 ** 18;

        heroPool.transfer(enterPriceMATIC * 5/100);
        systemPool.transfer(enterPriceMATIC * 5/100);
        address referral = rePointGames.userReferral(userAddr);
        if(referral != address(0)) {
            payable(referral).transfer(enterPriceMATIC * 5/100);
            rePointPool.transfer(enterPriceMATIC * 15/100);
        } else {
            rePointPool.transfer(enterPriceMATIC * 20/100);
        }
    ...
    function score(address userAddr, uint256 points) public onlyOwner {
        ...
        require(
            len > 0,
            "PickPicture : You have no games in this round"
        );
        if(round.userRound[userAddr].points == 0) {
