    ...
    ERC721Upgradeable 
{
    ...
    uint256 public mintPrice = 630000000000000000; // 0.63 ether
    ...
    function initialize(Initializer.Initialize calldata _init)
        external
        initializer
        returns (bool)
    {
        ...
        __Ownable_init();
        __ERC721_init(_init._name, _init._symbol);
        __AccessControl_init();

        transferOwnership(_init._newOwner);

        _setupRole(DEFAULT_ADMIN_ROLE, _init._newOwner);
        _setupRole(MINTER_ROLE, _init._newOwner);
        _setupRole(MINTER_ROLE, _init._minter);
        
        _setRoyalties(_init._royaltyRecipient, _init.royaltyAmount);

        _setBaseURI(_init._baseURI);
        _setId();

        totalSupply = _init._totalSupply > 0 ? _init._totalSupply : type(uint256).max;

        isPublicMint = _init._isPublicMint;
        mintPrice = _init._mintPrice;
        return true;
    }
    ...
    //     _setRoyalties(_royaltyRecipient, royaltyAmount);

    //     _setBaseURI(_baseURI);
    //     _setId();

    //     totalSupply = _totalSupply > 0 ? _totalSupply : type(uint256).max;
    ...
    function mint() external payable returns (uint256 LastId) {
        
        require(isPublicMint, "Public mint is not enabled for this smart contract!");
        require(msg.value >= mintPrice, "Not enough ether sent to mint!");
    ...
    function createToken(
        ...
        returns (uint256 id) 
    {
        require(ids.current() <= totalSupply, "NFT token limit reached");
    ...
    function setRoyaltyInfo(address recipient, uint256 _royaltyAmount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _setRoyalties(recipient, _royaltyAmount);
    }

    function setContractURI(string memory _storeFrontURI) external onlyRole(MINTER_ROLE) {
        storeFrontURI = _storeFrontURI;
    }
    ...
    function supportsInterface(
        ...
        public view override(ERC2981Base, AccessControlUpgradeable, ERC721Upgradeable) 
        returns (bool) 
    {
