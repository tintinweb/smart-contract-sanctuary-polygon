...
contract ISBDapp  {
    ...
    uint256[2] private diamondPercents = [200, 100];
    uint256[2] private bluediamondPercents = [300, 200];
    uint256[15] private crowndiamondPercents = [100, 100, 100, 100, 100, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50];
    uint256 private constant bluediamondPoolPercents = 25;
    uint256 private constant crowndiamondPoolPercents = 50;
    uint256 private constant crownAmbdiamondPoolPercents = 100;
    ...
    uint256 public totalUser; 
     ...
     uint256 public crownAmbDiamondPool;
     ...
     address[] public crownAmbdiamondUsers;
    ...
    constructor(address _usdtAddr)   {
          ...
          feeReceivers[0] = address(0x4202b0Eb3c07A72B25829CD6a997dc2712627c0F);
          feeReceivers[1] = address(0x6d8650A8012217EAC893Df115cbD434D9998b781);    
          feeReceivers[2] = address(0x312fffE8CadfE95Bf9f7385B4987640e33e3b5d5); 
          feeReceivers[3] = address(0x0d43A3d959979BaACa58bA12F1A731682700721F);    
     ...
     function _updateLevel(address _user) private {
        ...
        if(levelNow > user.level){
            ...
            if(levelNow == 5){
                ...
                crownAmbdiamondUsers.push(_user);
            }
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 500e18){
            ...
            if(total >= 2000e18 && totaldirectnum>=20 && totaldirectdepositnum>=10000e18   && user.teamNum >= 300 && maxTeam + otherTeam >= 200000e18 &&  otherTeam >= 100000e18  ){
                levelNow = 5;
            }else if(total >= 2000e18 && totaldirectnum>=10 && totaldirectdepositnum>=5000e18 && user.teamNum >= 125 && maxTeam + otherTeam >= 100000e18 &&  otherTeam >= 50000e18  ){
                levelNow = 4;
            }else if(total >= 500e18  && totaldirectnum>=5 && totaldirectdepositnum>=1000e18 && user.teamNum >= 40 && maxTeam + otherTeam >= 20000e18 && otherTeam>=10000e18 ){
   ...
   function _deposit(address _user, uint256 _amount) private {
     ...
     function _distributeDeposit(uint256 _amount) private {
        ...
        crownDiamondPool = crownDiamondPool.add(crowndiamond); 
         uint256 amb = _amount.mul(crownAmbdiamondPoolPercents).div(baseDivider);
        crownAmbDiamondPool = crownAmbDiamondPool.add(amb); 
    }
    ...
    function distributePoolRewards() public {
        if(block.timestamp > lastDistribute.add(timeStep)){     
           ...
           _distributeCrownAmbDiamondPool();
            lastDistribute = block.timestamp;
        }
    }
       ...
       function _distributeBlueDiamondPool() private {
        ...
        for(uint256 i = 0; i < bluediamondUsers.length; i++){
            if(userInfo[bluediamondUsers[i]].level == 3 && userInfo[bluediamondUsers[i]].isactive == true){
 ...
 function _distributeCrownDiamondPool() private {
        ...
        for(uint256 i = 0; i < crowndiamondUsers.length; i++){
            if(userInfo[crowndiamondUsers[i]].level == 4 && userInfo[crowndiamondUsers[i]].isactive == true){
    ...
    function _distributeCrownAmbDiamondPool() private {
        uint256 ambCount;
        for(uint256 i = 0; i < crownAmbdiamondUsers.length; i++){
            if(userInfo[crownAmbdiamondUsers[i]].level == 5 && userInfo[crownAmbdiamondUsers[i]].isactive == true){
                ambCount = ambCount.add(1);
            }
        }
        if(ambCount > 0){
            uint256 reward = crownAmbDiamondPool.div(ambCount);
            uint256 totalReward;
            for(uint256 i = 0; i < crownAmbdiamondUsers.length; i++){
                if(userInfo[crownAmbdiamondUsers[i]].level == 5 && userInfo[crownAmbdiamondUsers[i]].isactive == true){
                    rewardInfoPool[crownAmbdiamondUsers[i]].CrownAmb = rewardInfoPool[crownAmbdiamondUsers[i]].CrownAmb.add(reward);
                    userInfo[crownAmbdiamondUsers[i]].totalRevenue = userInfo[crownAmbdiamondUsers[i]].totalRevenue.add(reward);
                    totalReward = totalReward.add(reward);
                }
            }
            if(crownAmbDiamondPool > totalReward){
                crownAmbDiamondPool = crownAmbDiamondPool.sub(totalReward);
            }else{
                crownAmbDiamondPool = 0;
            }
        }
    }
    ...
    function withdraw() external {
       ...
       userRewards.level3Released = 0;
        ...
        userRewardsf.bluediamond = 0;
        userRewardsf.Crowndiamond = 0;
        userRewardsf.CrownAmb = 0;       
    ...
    function getcrownAmbdiamondusersLength() external view returns(uint256) {
        return crownAmbdiamondUsers.length;
    }
    ...
    function _calCurDynamicRewards(address _user) private view returns(uint256, uint256) {
        ...
        totalRewards = totalRewards.add(userRewardsf.bluediamond.add(userRewardsf.Crowndiamond).add(userRewardsf.CrownAmb));
        uint256 splitAmt = totalRewards.mul(freezeIncomePercents).div(baseDivider);
        uint256 withdrawable = totalRewards.sub(splitAmt);
        return(withdrawable, splitAmt);
    }
    ...
    function setfeeReceivers(address[4] calldata _address) external {    
        require(_address.length==4, "invalid");
         if(msg.sender==defaultRefer) 
         {
            feeReceivers = _address;
         }   
               ...
               _updatestatus(upline);
                ...
                RewardInfo storage upRewards = rewardInfo[upline];
                  ...
                  if(i > 0 && i < 3 && userInfo[upline].level > 1 && idstatus==true){
                    if(upRewards.level3Freezed > 0){
                        uint256 level3Reward = newAmount.mul(diamondPercents[i - 1]).div(baseDivider);
                ...
                if(i >= 3 && i < 5 && userInfo[upline].level > 2 && idstatus==true){
                    if(upRewards.level4Freezed > 0){
                        uint256 level4Reward = newAmount.mul(diamondPercents[i - 3]).div(baseDivider);
                ...
                if(i >= 5 && userInfo[upline].level > 3  && idstatus==true){
                    if(upRewards.level5Left > 0){
                        uint256 level5Reward = newAmount.mul(crowndiamondPercents[i - 5]).div(baseDivider);
