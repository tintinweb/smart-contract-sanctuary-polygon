...
contract ChanceVault is Ownable, Pausable {
    ...
    function syncAmount(uint256 gAmount, uint256 cAmount, uint256 iAmount) external {
        ...
        gameAmount += gAmount;
        communityAmount += cAmount;
        inviteAmount += iAmount;
    }

    function syncInvite(address _addr, uint256 amount) external {
        require(allowMap[msg.sender] == true, "Permission Denied");
        inviteMap[_addr] += amount;
    }

    function deposit() public payable {
    }

    function inviteWithdrow(address _to) public {
        require(inviteMap[msg.sender] <= inviteAmount, "NotEnough");
        require(inviteMap[msg.sender] >= 0.025 ether, "NotEnough");
        uint256 amount = inviteMap[msg.sender];
        inviteAmount -= amount;
    ...
    function withdrow(address _to, uint256 amount) external {
        require(allowMap[msg.sender] == true, "Permission Denied");
        require(amount <= gameAmount, "NotEnough");
        bool success;
        if (communityAmount > 0){
            (success, ) = COMMUNITY_ADDR.call{value: communityAmount}("");
            require(success, "FailedToSend");
            communityAmount = 0;
        }
