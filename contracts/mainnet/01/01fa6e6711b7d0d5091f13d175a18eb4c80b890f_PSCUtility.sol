...
contract PSCUtility is ERC20, AccessControl, ERC721Holder {
    bytes32 public constant CONTRACT_ADMIN_ROLE = keccak256("CONTRACT_ADMIN_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    address public collectionAddressPandaSpaceClubNFT;
    address public collectionAddressNFTv2;

    uint256 public numberOfBlocksPerRewardUnit;

    uint256 public coinAmountPerRewardUnitPandaSpaceClubNFT;
    uint256 public coinAmountPerRewardUnitNFTv2;

    uint256 public welcomeBonusAmountPandaSpaceClubNFT;
    uint256 public welcomeBonusAmountNFTv2;

    uint256 public amountOfStakersPandaSpaceClubNFT;
    uint256 public amountOfStakersNFTv2;

    uint256 public tokensStakedPandaSpaceClubNFT;
    ...
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogPandaSpaceClubNFT;

        /// owner => tokenId => StakeInfo
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogNFTv2;

    /// owner => #NFTsStaked
    mapping (address => uint256) public tokensStakedByUserPandaSpaceClubNFT;
    ...
    mapping (uint256 => bool) public welcomeBonusCollectedPandaSpaceClubNFT;

        /// tokenId => true/false (true if welcome bonus has been collected for a specific tokenId)
    mapping (uint256 => bool) public welcomeBonusCollectedNFTv2;

    /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserPandaSpaceClubNFT;

        /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserNFTv2;

    /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioPandaSpaceClubNFT;

        /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioNFTv2;

    event RewardsHarvestedPandaSpaceClubNFT (address owner, uint256 amount);
    event NFTStakedPandaSpaceClubNFT (address owner, uint256 tokenId);
    event NFTUnstakedPandaSpaceClubNFT (address owner, uint256 tokenId);
    ...
    constructor(address owner, address _collectionAddressPandaSpaceClubNFT, address _collectionAddressNFTv2) ERC20("PSCUtility", "PSCU") AccessControl(){
        _mint(owner, 100000 * 10 ** 18);
        _setupRole(DEFAULT_ADMIN_ROLE, owner);
        _setupRole(CONTRACT_ADMIN_ROLE, owner);
        _setupRole(BURNER_ROLE, owner);
        _setupRole(MINTER_ROLE, owner);
        collectionAddressPandaSpaceClubNFT = _collectionAddressPandaSpaceClubNFT;
        collectionAddressNFTv2 = _collectionAddressNFTv2;
        contractCreationBlock = block.number;
        coinAmountPerRewardUnitPandaSpaceClubNFT = 5 * 10 ** 18; // 5 ERC20 coins per rewardUnit, may be changed later on
        coinAmountPerRewardUnitNFTv2 = 15 * 10 ** 18; // 15 ERC20 coins per rewardUnit, may be changed later on
        numberOfBlocksPerRewardUnit = 34000; // 24 hours per reward unit , may be changed later on
        welcomeBonusAmountPandaSpaceClubNFT = 300 * 10 ** 18; // 300 tokens welcome bonus, only paid once per tokenId
        welcomeBonusAmountNFTv2 = 600 * 10 ** 18; // 600 tokens welcome bonus, only paid once per tokenId
    }

    function stakedNFTSByUserPandaSpaceClubNFT(address owner) external view returns (uint256[] memory){
        return stakePortfolioByUserPandaSpaceClubNFT[owner];
    }
    ...
    function pendingRewardsPandaSpaceClubNFT(address owner, uint256 tokenId) public view returns (uint256){
        StakeInfo memory info = stakeLogPandaSpaceClubNFT[owner][tokenId];
        ...
        return rewardAmount * coinAmountPerRewardUnitPandaSpaceClubNFT;
    ...
    function stakePandaSpaceClubNFT(uint256 tokenId) public {
        IERC721(collectionAddressPandaSpaceClubNFT).safeTransferFrom(_msgSender(), address(this), tokenId);
        require(IERC721(collectionAddressPandaSpaceClubNFT).ownerOf(tokenId) == address(this),
            "PSCUtility: Error while transferring token");
        StakeInfo storage info = stakeLogPandaSpaceClubNFT[_msgSender()][tokenId];
        info.stakedAtBlock = block.number;
        info.lastHarvestBlock = block.number;
        info.currentlyStaked = true;
        if(tokensStakedByUserPandaSpaceClubNFT[_msgSender()] == 0){
            amountOfStakersPandaSpaceClubNFT += 1;
        }
        tokensStakedByUserPandaSpaceClubNFT[_msgSender()] += 1;
        tokensStakedPandaSpaceClubNFT += 1;
        stakePortfolioByUserPandaSpaceClubNFT[_msgSender()].push(tokenId);
        uint256 indexOfNewElement = stakePortfolioByUserPandaSpaceClubNFT[_msgSender()].length - 1;
        indexOfTokenIdInStakePortfolioPandaSpaceClubNFT[tokenId] = indexOfNewElement;
        if(!welcomeBonusCollectedPandaSpaceClubNFT[tokenId]) {
            _mint(_msgSender(), welcomeBonusAmountPandaSpaceClubNFT);
            welcomeBonusCollectedPandaSpaceClubNFT[tokenId] = true;
        }

        emit NFTStakedPandaSpaceClubNFT(_msgSender(), tokenId);
    }
    ...
    function stakeBatchPandaSpaceClubNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            stakePandaSpaceClubNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakeBatchPandaSpaceClubNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            unstakePandaSpaceClubNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakePandaSpaceClubNFT(uint256 tokenId) public {
        if(pendingRewardsPandaSpaceClubNFT(_msgSender(), tokenId) > 0){
            harvestPandaSpaceClubNFT(tokenId);
        }
        StakeInfo storage info = stakeLogPandaSpaceClubNFT[_msgSender()][tokenId];
        info.currentlyStaked = true;
        IERC721(collectionAddressPandaSpaceClubNFT).safeTransferFrom(address(this), _msgSender(), tokenId);
        require(IERC721(collectionAddressPandaSpaceClubNFT).ownerOf(tokenId) == _msgSender(),
            "PSCUtility: Error while transferring token");
        if(tokensStakedByUserPandaSpaceClubNFT[_msgSender()] == 1){
            amountOfStakersPandaSpaceClubNFT -= 1;
        }
        tokensStakedByUserPandaSpaceClubNFT[_msgSender()] -= 1;
        tokensStakedPandaSpaceClubNFT -= 1;
        stakePortfolioByUserPandaSpaceClubNFT[_msgSender()][indexOfTokenIdInStakePortfolioPandaSpaceClubNFT[tokenId]] = 0;
        emit NFTUnstakedPandaSpaceClubNFT(_msgSender(), tokenId);
    }
    ...
    function harvestPandaSpaceClubNFT(uint256 tokenId) public {
        StakeInfo storage info = stakeLogPandaSpaceClubNFT[_msgSender()][tokenId];
        uint256 rewardAmountInERC20Tokens = pendingRewardsPandaSpaceClubNFT(_msgSender(), tokenId);
        if(rewardAmountInERC20Tokens > 0) {
            info.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInERC20Tokens);
            emit RewardsHarvestedPandaSpaceClubNFT(_msgSender(), rewardAmountInERC20Tokens);
        }
    }
    ...
    function harvestBatchPandaSpaceClubNFT(address user) external {
        uint256[] memory tokenIds = stakePortfolioByUserPandaSpaceClubNFT[user];

        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            harvestPandaSpaceClubNFT(tokenIds[currentId]);
        }
    }
    ...
    function setCoinAmountPerRewardUnitPandaSpaceClubNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        coinAmountPerRewardUnitPandaSpaceClubNFT = coinAmount;
    }
    ...
    function setWelcomeBonusAmountPandaSpaceClubNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        welcomeBonusAmountPandaSpaceClubNFT = coinAmount;
    }
    ...
    function setCollectionAddressPandaSpaceClubNFT(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "VIBE: update to zero address not possible");
        collectionAddressPandaSpaceClubNFT = newAddress;
    }
