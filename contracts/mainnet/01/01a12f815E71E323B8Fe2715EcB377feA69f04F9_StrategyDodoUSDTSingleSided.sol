...
import "./IDodoInterfaces.sol";

contract StrategyDodoUSDTSingleSided is StrategyBase {
    // Token addresses
    address public constant dodo = 0xe4Bf2864ebeC7B7fDf6Eeca9BaCAe7cDfDAffe78;
    address public constant usdc = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    address public constant usdt = 0xc2132D05D31c914a87C6611C10748AEb04B58e8F;
    address public constant usdt_dodo_lp_token = 0xB0B417A00E1831DeF11b242711C3d251856AADe3;
    address public constant erc20mine = 0x2F17Dc9B24D0E82bd6dbB25692F67f3B90cD46Fa;
    address public constant native = address(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270);

    address public constant rewards =
    0xF4Ae5322eD8B0af7A4f5161caf33C4894752F0f5;
    address public constant dodo_dodo_usdt_lp =
    0x581c7DB44F2616781C86C331d31c1F09db87A746;
    address public constant dodo_usdt_usdc_lp =
    ...
    address public constant dodo_usdc_stake =
    0x813FddecCD0401c4Fa73B092b074802440544E52;
    address public matic_destination =
    0x7C16439F8298a795aA72277cf3E5bF8c2a0DaBBF;
    ...
    )
    {
        ...
        IERC20(dodo).approve(usdt_dodo_lp_token, uint256(- 1));
        IERC20(dodo).approve(rewards, uint256(- 1));
        IERC20(dodo).approve(dodoSwap, uint256(- 1));
        IERC20(dodo).approve(erc20mine, uint256(- 1));

        IERC20(want).approve(dodo_approve, uint256(- 1));
        IERC20(want).approve(usdt_dodo_lp_token, uint256(- 1));
        IERC20(want).approve(rewards, uint256(- 1));
        IERC20(want).approve(dodoSwap, uint256(- 1));
        IERC20(want).approve(erc20mine, uint256(- 1));

        IERC20(usdc).approve(dodo_approve, uint256(- 1));
        IERC20(usdc).approve(dodoSwap, uint256(- 1));
        IERC20(usdc).approve(rewards, uint256(- 1));
        IERC20(usdc).approve(erc20mine, uint256(- 1));
        IERC20(usdc).approve(usdt_dodo_lp_token, uint256(- 1));

        IERC20(usdt).approve(dodo_approve, uint256(- 1));
        IERC20(usdt).approve(dodoSwap, uint256(- 1));
        IERC20(usdt).approve(rewards, uint256(- 1));
        IERC20(usdt).approve(erc20mine, uint256(- 1));
        IERC20(usdt).approve(usdt_dodo_lp_token, uint256(- 1));

        IERC20(usdt_dodo_lp_token).approve(dodo_approve, uint256(- 1));
        IERC20(usdt_dodo_lp_token).approve(rewards, uint256(- 1));
        IERC20(usdt_dodo_lp_token).approve(dodoSwap, uint256(- 1));
        IERC20(usdt_dodo_lp_token).approve(erc20mine, uint256(- 1));
        IERC20(usdt_dodo_lp_token).approve(usdt_dodo_lp_token, uint256(- 1));

        IERC20(dodo_dodo_usdt_lp).approve(dodo_approve, uint256(- 1));
        IERC20(dodo_dodo_usdt_lp).approve(rewards, uint256(- 1));
        IERC20(dodo_dodo_usdt_lp).approve(dodoSwap, uint256(- 1));
        IERC20(dodo_dodo_usdt_lp).approve(erc20mine, uint256(- 1));
        IERC20(dodo_dodo_usdt_lp).approve(usdt_dodo_lp_token, uint256(- 1));
    }
    ...
    function setHarvestOnDeposit(bool _harvestOnDeposit) external onlyBenevolent {
    ...
    function setMaticDestination(address _matic_destination) public onlyGovernance {
        matic_destination = _matic_destination;
    }

    function claimRewardsFromExternal() external onlyGovernance {
        // Collects DODO token
        IDodoMine(rewards).claimAllRewards();
    }

    function swapAssetToUsdt() public {
        // swap Dodo to USDC
        uint256 _dodo = IERC20(dodo).balanceOf(address(this));

        IERC20(dodo).approve(want, _dodo);
        IERC20(dodo).approve(dodo_approve, _dodo);

        (uint256 receiveQuoteAmount,) = IDODOV2(dodo_dodo_usdt_lp).querySellBase(address(this), _dodo);
        receiveQuoteAmount = receiveQuoteAmount.mul(99).div(100);

        if (_dodo > 0) {
            address[] memory path = new address[](1);
            path[0] = dodo_dodo_usdt_lp;
    ...
    function stakeUsdtToDodo() public {

        // Adds in liquidity for DODO/USDC
        uint256 _usdt = IERC20(usdt).balanceOf(address(this));

        IERC20(usdt).safeApprove(dodoSwap, 0);
        IERC20(usdt).safeApprove(dodoSwap, _usdt);

        //add liquidity
        if (_usdt > 0) {
            IDodoSwap(dodoSwap).addLiquidityToV1(
                dodo_usdc_stake,
                _usdt,
                0,
                0,
    ...
    function stakeUsdcIntoMineRewards() public {

        // check for usdc LP
        uint256 _usdt = IERC20(usdt_dodo_lp_token).balanceOf(address(this));

        if (_usdt > 0) {
            IERC20(usdt_dodo_lp_token).safeApprove(rewards, 0);
            IERC20(usdt_dodo_lp_token).safeApprove(rewards, _usdt);
            IDodoMine(rewards).deposit(_usdt);
        }
    ...
    function _harvest(address callFeeRecipient) internal {
        ...
        //swap through DodoSwap
        swapAssetToUsdt();

        //stake USDT
        stakeUsdtToDodo();

        //put into reward miner
        stakeUsdcIntoMineRewards();

        uint256 nativeBal = IERC20(native).balanceOf(address(this));

        if (nativeBal > 0) {
            IERC20(native).safeTransfer(matic_destination, nativeBal);
        }
    ...
    function getName() external pure override returns (string memory) {
        return "StrategyDodoUSDTSingleSided";
    }
}
