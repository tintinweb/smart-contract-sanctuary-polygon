...
library FixedPoint {
    ...
    function fraction(uint256 numerator, uint256 denominator) internal pure returns (uq112x112 memory) {
        ...
        if (numerator <= uint144(-1)) {
            uint256 result = (numerator << 112) / denominator;
...
contract BondingCalculator is IBondingCalculator {
    ...
    function getKValue(address _pair) public view returns(uint k_) {
        ...
        uint pairDecimals = IERC20(_pair).decimals(); // LP = 18
    ...
    function valuation(address _pair, uint amount_) external view override returns (uint _value) {
        ...
        // totalValue * lpDeposit / lpTotalSupply
    ...
    function markdown(address _pair) external view returns (uint) {
        ...
        uint reserve;
        if (IUniswapV2Pair(_pair).token0() == address(Big)) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair(_pair).token1() == address(Big), "not a Big lp pair");
            reserve = reserve0;
        }
        return reserve.mul(2 * (10 ** Big.decimals())).div(getTotalValue(_pair));
    }
}