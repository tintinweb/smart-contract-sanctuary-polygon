...
error SponsoredERC1155__NotMinter();
error SponsoredERC1155__InvalidArg_TokenAndAddressLengthMismatch();
error SponsoredERC1155__InvalidArg_AddressZero();
error SponsoredERC1155__InvalidArg_SellerFeeBasisPoints(uint16 basisPoints);
error SponsoredERC1155__AlreadyMinted(uint256 tokenId);
error SponsoredERC1155__NotImplemented();
contract SponsoredERC1155 is ERC1155Upgradeable, OwnableUpgradeable, ERC2771Recipient, IERC2981 {
  using AddressUpgradeable for address;
  ...
  modifier onlyMinter() {
    ...
    if (sender != minter && sender != prevMinter) {
      revert SponsoredERC1155__NotMinter();
    }
    _;
  }

  function initialize(RoyaltyInfo calldata _royalties, address minterAddress, address owner, address forwarder, string calldata contracturi) public initializer {
    __ERC1155_init("_");
  ...
  function _mintBatch(
    ...
    revert SponsoredERC1155__NotImplemented();
  }
...
//   }
  ...
  function setMinter(address account) public onlyOwner {
    if (account == address(0)){
      revert SponsoredERC1155__InvalidArg_AddressZero();
    }
  ...
  function batchMint(uint256[] calldata tokenIds, address[] calldata ownerIds) public onlyMinter {
    if (tokenIds.length != ownerIds.length) {
      revert SponsoredERC1155__InvalidArg_TokenAndAddressLengthMismatch();
    }
  ...
  function _updateRoyalties(RoyaltyInfo calldata _royalties) internal {
    if(_royalties.basisPoints > 10000){
      revert SponsoredERC1155__InvalidArg_SellerFeeBasisPoints(_royalties.basisPoints);
    }
  ...
  function _updateTrustedForwarder(address forwarder) internal {
    emit TrustedForwarderUpdated(forwarder, getTrustedForwarder());
