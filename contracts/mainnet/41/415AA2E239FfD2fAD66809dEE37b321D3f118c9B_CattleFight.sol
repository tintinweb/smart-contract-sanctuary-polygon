...
contract CattleFight is OwnableUpgradeable {
    ...
    uint public mostWins;
    address public mostWinsUser;
    ...
    function userFight(uint times, uint txId, bytes32 r, bytes32 s, uint8 v) external freshTime {
        ...
        if(userInfo[msg.sender].totalWins > mostWins){
            mostWins = userInfo[msg.sender].totalWins;
            mostWinsUser = msg.sender;
        }
    ...
    function calWithDay(address addr, uint timestamp) public view returns (uint){
        if (userInfo[addr].dailyWin[timestamp] == 0 || isUserClaimedRound[addr][timestamp] || dailyWins[timestamp] == 0) {
            return 0;
        }
    ...
    function deleteUserRound(address addr) external onlyOwner{
        userInfo[addr].userRound.pop();
    }

    function getCurrentRoundInfo(address addr) public view returns (uint userWins,uint rew,uint totalWin, uint dailyOut_, string memory symbol, uint decimals){
        uint _lastTime = block.timestamp - (block.timestamp % 86400);
        totalWin = dailyWins[_lastTime];
        if (roundInfo[_lastTime].dailyOut == 0) {
            dailyOut_ = dailyOut;
            symbol = metadata(address(BVT)).symbol();
            decimals = 18;
        } else {
            dailyOut_ = roundInfo[_lastTime].dailyOut;
            symbol = metadata(roundInfo[_lastTime].payToken).symbol();
            decimals = metadata(roundInfo[_lastTime].payToken).decimals();
        }
        userWins = userInfo[addr].dailyWin[_lastTime];
        rew = calWithDay(addr,_lastTime);
    }


    function addUserRoundInfo(address addr, uint[] memory timestamp, uint[] memory times, uint[] memory totalWins, address[] memory tokens, uint[] memory dailyOuts) external onlyOwner {
        for (uint i = 0; i < timestamp.length; i ++) {
            roundInfo[timestamp[i]].dailyOut = dailyOuts[i];
            roundInfo[timestamp[i]].payToken = tokens[i];
            userInfo[addr].userRound.push(timestamp[i]);
            userInfo[addr].dailyWin[timestamp[i]] = times[i];
            dailyWins[timestamp[i]] = totalWins[i];
        }
    }
