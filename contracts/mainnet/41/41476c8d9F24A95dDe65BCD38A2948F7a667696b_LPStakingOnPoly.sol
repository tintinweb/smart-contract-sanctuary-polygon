...
contract LPStakingOnPoly is LPStakingOnPolyWrapper, OwnableUpgradeable {
    ...
    */
    function claimReward() public updateReward(msg.sender) { 
        ...
        uint boost = vebalance / boostRate;
        boost = boost > 50 * FULL_SCALE ? 50 * FULL_SCALE : boost;
        if (reward > 0) {
            rewards[msg.sender] = 0;
            if (vebalance > 0)
                reward += reward * boost / FULL_SCALE / 100;
    ...
    */
    function notifyRewardAmount(int256 _rewardDelta) internal onlyOwner updateReward(address(0)) { 
        require((_rewardDelta < 1e25) && (_rewardDelta > -1e25), "Cannot notify with more than 10 millions units");
