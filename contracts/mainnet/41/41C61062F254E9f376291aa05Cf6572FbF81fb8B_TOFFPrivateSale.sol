...
contract TOFFPrivateSale is VerifySignature{
    ...
    function startBuying(uint256 tokenAmount, uint256 price) public onlyOwner{
        ...
        require(saleStatus != true, "Sale has already started.");
        minLimit = 60000000;
        maxLimit = 1550000000;
        saleStatus = true;
    ...
    function buyToken(bytes memory _signature, uint256 amountOfToken, address signer) public noReentrant payable{
        require(verify(signer, _signature), "Your wallet is not verified.");
        require(saleStatus == true, "TokenSale has ended.");
        require(amountOfToken >= minLimit,"You are exceeding min value.");
        require(amountOfToken <= maxLimit,"You are exceeding max value.");
        require(userBalance[msg.sender] + amountOfToken <= maxLimit,"You are exceeding max value.");
    ...
    function endSale() public onlyOwner{
        require(saleStatus != false, "Sale is not active.");
        saleStatus = false;
    }
}