...
import "../rights/RightsDeclaration.sol";
import "../rights/RightsProtocol.sol";
import "../@rarible/royalties/contracts/impl/RoyaltiesV2Impl.sol";
import "../@rarible/royalties/contracts/LibPart.sol";
import "../@rarible/royalties/contracts/LibRoyaltiesV2.sol";

contract MintangibleERC721v1 is ERC721, RoyaltiesV2Impl, Ownable, ReentrancyGuard {

    // Mapping for token metadata CIDs (tokenID => tokenCID)
    mapping(uint256 => string) private _tokenCIDs;
    
    uint256 private _mintingPrice;
    address private _rightsProtocol;
    address private _rightsDeclaration;
    
    bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a;

    constructor(string memory name, string memory symbol, address rightsProtocol_, address rightsDeclaration_) ERC721(name, symbol) {       
        _rightsProtocol = rightsProtocol_;
        _rightsDeclaration = rightsDeclaration_;
    }
    ...
    function rightsProtocol() public view returns (address) {
        return _rightsProtocol;
    }

    function setRightsProtocol(address rightsProtocol_) public onlyOwner {
        _rightsProtocol = rightsProtocol_;
    }

    function rightsDeclaration() public view returns (address) {
        return _rightsDeclaration;
    }

    function setRightsDeclaration(address rightsDeclaration_) public onlyOwner {
        _rightsDeclaration = rightsDeclaration_;
    }

    function mintToken(uint256 tokenID, string calldata tokenCID, string calldata rightsURI, uint96 feeNumerator, string calldata isccCode, string calldata isccURL, string memory isccMessage) public payable {
        ...
        RightsProtocol(_rightsProtocol).transferTokenAuthority(address(this), tokenID, tx.origin);

        RightsDeclaration(_rightsDeclaration).declare(address(this), tokenID, rightsURI, isccCode, isccURL, isccMessage);
        ...
        if (feeNumerator > 0) {
            ...
            royalties[0].value = feeNumerator;
            royalties[0].account = payable(msg.sender);
...
interface IERC2981 {
    /* 
     * ERC165 bytes to be registered in the parent contract implementing this standard
