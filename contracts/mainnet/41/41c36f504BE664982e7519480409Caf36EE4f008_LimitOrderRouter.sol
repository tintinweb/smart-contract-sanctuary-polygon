...
interface ILimitOrderBook {
    ...
    function placeOrder(LimitOrderBook.LimitOrder[] calldata orderGroup)
        external
        payable
...
contract LimitOrderBook {
    ...
    function placeOrder(LimitOrder[] calldata orderGroup)
        public
        payable
        returns (bytes32[] memory)
    {
...
contract LimitOrderRouter is ILimitOrderRouter, LimitOrderBook {
    ...
    function refreshOrder(bytes32[] calldata orderIds) external nonReentrant {
        ...
        for (uint256 i = 0; i < orderIds.length; ) {
            ...
            LimitOrder memory order = getOrderById(orderId);
...
contract SandboxLimitOrderBook is ISandboxLimitOrderBook {
    ...
    uint256 private constant MIN_ORDER_VALUE_IN_WETH = 10e14;
    ...
    event UnknownCaller(address caller);
        ...
        nonReentrant
    {
        ...
        if (order.orderId == bytes32(0)) {
            emit UnknownCaller(msg.sender);
        }
        require(order.orderId == bytes32(0), "order does not exist");
        if (order.owner != msg.sender) {
            revert MsgSenderIsNotOrderOwner();
        }
    ...
    ///@param orderGroup - List of newly created orders to be placed.
    /// @return orderIds - Returns a list of orderIds corresponding to the newly placed orders.
    function placeOrder(SandboxLimitOrder[] calldata orderGroup)
        public
        payable
        returns (bytes32[] memory)
    {
    ...
    function validateAndCancelOrder(bytes32 orderId)
        ...
        returns (bool success)
    {
        SandboxLimitOrder memory order = getOrderById(orderId);
