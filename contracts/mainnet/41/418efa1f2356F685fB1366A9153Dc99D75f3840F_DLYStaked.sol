...
contract DLYStaked is Context, Ownable, ReEntrancyGuard, Stakeable {
    ...
    constructor(address _dlyTokenAddress) {
        _dlyToken = IERC20(_dlyTokenAddress);
        _stakeCount = 0;
    }
    ...
    {
        // Transfer token to the msg.sender
        bool sent = _dlyToken.transfer(_msgSender(), amount);
        require(sent, "Failed to transfer token to user");
        return sent;
    }
...
contract Stakeable is Context, Ownable {
        ...
        returns (uint256)
    {
        ...
        // We will reward the if user 0.1% per Hour So thats 0.1% per 3600 seconds
        // the alghoritm is  seconds = block.timestamp - stake seconds (block.timestap - _stake.since)
        // hours = Seconds / 3600 (seconds /3600) 3600 is an variable in Solidity names hours
        // we then multiply each token by the hours staked , then divide by the rewardPerHour rate

        return
            (((getTime() - _current_stake.sinceBlock) / 1 hours) *
