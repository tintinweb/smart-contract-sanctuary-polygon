...
contract RaffleStore is IERC721Receiver, IERC1155Receiver, VRFConsumerBase {
    ...
    function getCurrentStuckBalance(address currencyAddress) public view returns (uint256) {
        ...
        if (currencyAddress != address(0x7D1AfA7B718fb893dB30A3aBc0Cfc608AaCfeBB0)) {
            uint256 totalBalance = IERC20(currencyAddress).balanceOf(address(this));
            stuckBalance = totalBalance - totalTicketBalance[currencyAddress];
        } else {
    ...
    function distributionFunds(uint256 _raffleId) internal {
        ...
        if (raffles[_raffleId].tokenAddress == address(0x7D1AfA7B718fb893dB30A3aBc0Cfc608AaCfeBB0)) {
            ...
            IERC20(raffles[_raffleId].tokenAddress).transfer(raffles[_raffleId].creator, incomeAmount * rewardCreator / 1000);
    ...
    function withdrawExtraTokens(address currency) public onlyOwner {
        ...
        if (currency == address(0x7D1AfA7B718fb893dB30A3aBc0Cfc608AaCfeBB0)){
            payable(walletA).transfer(stuckBalance);
        }
        else{
    ...
    function withdrawAllLinkTokens() public onlyOwner {
        address linkAddress = address(0xb0897686c545045aFc77CF20eC7A532E3120E0F1);
        uint256 linkBalance = LINK.balanceOf(address(this));
        require(linkBalance > 0, "No LINK tokens to withdraw");

        require(IERC20(linkAddress).transfer(msg.sender, linkBalance), "LINK token transfer failed");
    }
