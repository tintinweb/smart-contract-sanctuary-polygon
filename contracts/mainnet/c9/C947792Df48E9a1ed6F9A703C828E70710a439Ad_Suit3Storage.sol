...
contract Suit3Storage is ERC2771Recipient {
    ...
    event SendMessage(address indexed owner, address indexed to, string cid, uint256 expire);

    // Event when a file is stored
    event StoreFile(address indexed owner, string cid, string name, string extension, uint256 size, uint256 expire);
    ...
    */
    function sendMessage(string memory _cid, File[] memory _files, uint _expire, address[] memory _recipients) public {

        // Send message to every recipient
        for (uint i = 0; i < _recipients.length; i++) {
            emit SendMessage( _msgSender(), _recipients[i], _cid, _expire);
        }

        // Send file
        for (uint i = 0; i < _files.length; i++) {
            emit StoreFile(_msgSender(), _files[i].cid, _files[i].name, _files[i].extension, _files[i].size, _files[i].expire);
    ...
    * @param _walletRecipient
    */
    function storeFile(File[] memory _files) public {
