...
contract Swap is ISwap, Ownable, Pausable {
    ...
    bytes32 public constant ORDER_TYPEHASH =
        keccak256(
            "Order(bytes32 hashedNote,uint256 nonce,uint256 expiry,address signerWallet,address signerToken,uint256 signerAmount,uint256 protocolFee,address senderWallet,address senderToken,uint256 senderAmount)"
        );

    bytes32 public constant DOMAIN_NAME = keccak256("TROF");
    bytes32 public constant DOMAIN_VERSION = keccak256("5");
    ...
    // instead of taking v,r,s as parameters take into account entire signature to avoid
    ...
    /**
     * @notice ERC20 Swap for Any Sender
    ...
    /**
     * @notice Swap for Whitelisted Sender
    * @param hashedNote bytes32 hash of note contents
     * @param nonce uint256 Unique and should be sequential
     * @param expiry uint256 Expiry in seconds since 1 January 1970
     * @param signerWallet address Wallet of the signer
     * @param signerToken address ERC20 token transferred from the signer
    ...
    function swapRecipient(
        ...
        Signature memory signatureStruct;
        (
            signatureStruct.r,
            signatureStruct.s,
            signatureStruct.v
                ...
                abi.encodePacked(
                    ...
                    keccak256(
                        bytes.concat(
                            ...
                            abi.encode(msg.sender, senderToken, senderAmount)
                        )
                    )
                )
            ),
    ...
    /**
     * @notice Validates Swap Order for a potential error
     * @param hashedNote bytes32 hash of note contents
    ...
    function check(
        ...
        address signatory = _getSignatory(hashed, signature);
        ...
        if (order.senderWallet != address(0)) {
            ...
            if (senderAllowance < order.senderAmount) {
                return "SENDER_ALLOWANCE_LOW";
            }

            if (senderBalance < order.senderAmount) {
                return "SENDER_BALANCE_LOW";
            }
        }
        ...
        uint256 signerFeeAmount = (order.signerAmount * protocolFee) /
            FEE_DIVISOR;

        if (signerAllowance < order.signerAmount + signerFeeAmount) {
            return "SIGNER_ALLOWANCE_LOW";
        }

        if (signerBalance < order.signerAmount + signerFeeAmount) {
            return "SIGNER_BALANCE_LOW";
        }
