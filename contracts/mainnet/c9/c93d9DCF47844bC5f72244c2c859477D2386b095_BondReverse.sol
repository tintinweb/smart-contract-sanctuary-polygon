 ...
 * For a generic mechanism see {ERC20PresetManagerPauser}.
 *
...
contract BondReverse is ManagerRole, Ownable, Pausable {
    ...
    /**
        ...
        @param _price uint256 new price of dYel token in USDC
     */
    function setPriceInUSDC(uint256 _price) external {
        if(isManager(msg.sender)) {
            require(
                returnPercentPrice(_price) < percent,
                "BondReverse: The price difference is more than allowed"
            );
        } else {
            require(owner() == msg.sender, "BondReverse: caller is not the owner or manager");
        }
        currentPrice = _price;
    }
