 ...
 * @notice All Bridge Implementation will follow this interface.
 */
abstract contract BridgeImplV2Base {
    using SafeERC20 for IERC20;
...
contract HopImplL1 is BridgeImplBase {
        ...
        returns (address token, uint256 bridgedAmount)
    {
        ...
        ERC20(_token).safeTransferFrom(msg.sender, socketGateway, _amount);
...
import {ERC20} from "lib/solmate/src/tokens/ERC20.sol";
import {ISocketGateway} from "../../../interfaces/ISocketGateway.sol";
import {BridgeImplV2Base} from "../../BridgeImplV2Base.sol";
 ...
 * // @author Movr Network.
 */
contract HopImplL2V2 is BridgeImplV2Base {
    using SafeERC20 for IERC20;
    using SafeTransferLib for ERC20;

    constructor(address _socketGateway) BridgeImplV2Base(_socketGateway) {}

    struct HopBridgeRequest {
        address _receiverAddress;
        address _token;
        address _hopAMM;
        uint256 _amount;
    ...
    function bridgeExternalTo(
        HopBridgeRequest calldata _hopBridgeRequest
    ) external payable
    {
        // token address might not be indication thats why passed through extraData
        if (_hopBridgeRequest._token == NATIVE_TOKEN_ADDRESS) {
            // perform bridging
            HopAMM(_hopBridgeRequest._hopAMM).swapAndSend{value: _hopBridgeRequest._amount}(
                _hopBridgeRequest._toChainId,
                _hopBridgeRequest._receiverAddress,
                _hopBridgeRequest._amount,
                _hopBridgeRequest._bonderFee,
                _hopBridgeRequest._amountOutMin,
                _hopBridgeRequest._deadline,
                _hopBridgeRequest._amountOutMinDestination,
                _hopBridgeRequest._deadlineDestination
            );
        }

        ERC20(_hopBridgeRequest._token).safeTransferFrom(msg.sender, socketGateway, _hopBridgeRequest._amount);
        ERC20(_hopBridgeRequest._token).safeApprove(_hopBridgeRequest._hopAMM, _hopBridgeRequest._amount);

        HopAMM(_hopBridgeRequest._hopAMM).swapAndSend(
            _hopBridgeRequest._toChainId,
            _hopBridgeRequest._receiverAddress,
            _hopBridgeRequest._amount,
            _hopBridgeRequest._bonderFee,
    ...
    {
        // decode data
        IERC20(_hopBridgeRequest._token).safeIncreaseAllowance(_hopBridgeRequest._hopAMM, _hopBridgeRequest._amount);
...
contract SocketGateway is Ownable, ReentrancyGuard {
    ...
    //Route storage
    mapping(uint256 => ISocketRoute.RouteData) public routes;

    uint256 public routesCount;

    // Controller Storage
    mapping(uint256 => ISocketController.Controller) public controllers;
    ...
    function addController(address _controllerAddress) external onlyOwner returns (uint256) {
        ...
        uint256 controllerId = controllerCount;
    ...
    function bridgeNew(uint256 routeId, bytes calldata data) external payable {
        // load bridge info and validate
        ISocketRoute.RouteData memory routeInfo = routes[routeId];
    ...
    function bridge(uint256 routeId, bytes calldata data) external payable returns (bytes memory) {
        // load bridge info and validate
        ISocketRoute.RouteData memory routeInfo = routes[routeId];
