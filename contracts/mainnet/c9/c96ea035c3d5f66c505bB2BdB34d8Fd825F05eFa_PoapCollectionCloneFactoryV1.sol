...
import "./PoapCollectionInitializableV1.sol";

/**
 * @title A Factory contract that can create new clones of `PoapCollectionInitializableV1`
 ...
 * that allows to create new clones of the `PoapCollectionInitializableV1` contract.
 *
 * On calling the {createPoapCollection} new clones of `PoapCollectionInitializableV1`
 * are created on behalve of a creator. All parameters are forwared to the initializer
 * of `PoapCollectionInitializableV1`
 * The clones are not upgradeable because the implementation contracts address is hardcoded
 * in the bytecode of the clone.
 */
contract PoapCollectionCloneFactoryV1 is AccessControl {
    PoapCollectionInitializableV1[] private poapCollections;

    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");

    address public immutable poapCollectionImplementation;

    /**
     * @dev Emitted when a `manager` created a new `PoapCollectionInitializableV1` through
     * a call to {createPoapCollection}.
     * `poapCollection` is the address of the new `PoapCollectionInitializableV1`.
     * been created.
     */
    event PoapCollectionCreated (
        address indexed poapCollection
    );

    /**
     * @notice Constructs the `PoapCollectionInitializableV1`.
     * @dev This contract should only be deployed once.
     ...
     * Can create new `PoapCollectionInitializableV1` through {createPoapCollection}
     */
    constructor(
        address _roleAdmin,
        address _manager
        ...
        poapCollectionImplementation = address(new PoapCollectionInitializableV1());
    }

    // External functions

    /**
     * @notice Create a new clone of `PoapCollectionInitializableV1`
     * @dev clones of `PoapCollectionInitializableV1` are created by the `manager`
    ...
    function createPoapCollection(
        PoapCollectionInitializableV1.InitializeData calldata _init
    ) external onlyRole(MANAGER_ROLE) returns (address) {
        address clone = Clones.clone(poapCollectionImplementation);
        PoapCollectionInitializableV1(clone).initialize(_init);
        poapCollections.push(PoapCollectionInitializableV1(clone));
        emit PoapCollectionCreated(clone);
        return clone;
    }
    ...
    /**
     * @notice Get address of previously created `PoapCollectionInitializableV1` clones
     * @dev this function is used for the fuel-blockchain-listner
     * event handler get all created `PoapCollectionInitializableV1` in order to
     * register event handlers.
     * @param _i index in the collection
     * @return the address of PoapCollectionInitializableV1
     */
    function getCollection(uint256 _i) external view returns(address) {
        return address(poapCollections[_i]);
    }

    /**
     * @notice Get how many `PoapCollectionInitializableV1` clones have previously been created.
     * @dev this function is used for the fuel-blockchain-listner
     * event handler to get all created `PoapCollectionInitializableV1` in order to
     * register event handlers.
     * @return length of the `poapCollections` array
     */
    function collectionsLength() external view returns(uint256) {
        return poapCollections.length;
    }
}
