...
/**
 * @title Simple Flowers For Society paymaster
 * @author deeptechdev
 * @notice Facilitate gas less transactions on supported smart contracts
 */
contract SimpleFFSPaymaster is BasePaymaster {
    ...
    function addTarget(address _address) external onlyOwner {
        require(!supportedTargets[_address], "TARGET_ALREADY_SUPPORTED");
        supportedTargets[_address] = true;
    }

    function removeTarget(address _address) external onlyOwner {
        require(supportedTargets[_address], "UNKNOWN_TARGET");
        delete supportedTargets[_address];
    }

    /**
     * @notice Accepts to pay for all requests as long the target contract is the currentTarget
     */
        ...
        returns (bytes memory context, bool revertOnRecipientRevert)
    {
        ...
        require(supportedTargets[relayRequest.request.to], "WRONG_TARGET");
        return ("", true);
    }
