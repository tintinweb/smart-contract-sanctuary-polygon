...
contract CryptoAstrum is Initializable, ERC721Upgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, 
ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable {
    ...
    uint256 private commPerc;
    string private baseURI;
    mapping (uint256 => TokensMinted) private toksMinted;
    event ForSale(uint256 indexed _token, bool _val, uint256 _price);

    function initialize() initializer public {
        __ERC721_init("Astrum", "AST");
        ...
        commPerc = 500;
        baseURI = "ipfs://QmWnfjjFKcYC58m26YXt5TQgXua5D9ZXKVE5ztYrcJFSc1/";
    }
    ...
    function setCommPerc(uint256 _commPerc) external onlyOwner() {
        commPerc = _commPerc;
    }
    ...
    function getMintPrice() external view onlyOwner() returns (uint256) {
        return mintPrice;
    }

    function getCommPerc() external view onlyOwner() returns (uint256) {
        return commPerc;
    }

    function mint(address to, uint256 _tokenId) whenNotPaused() nonReentrant() public payable {
        require(msg.value >= mintPrice);
        toksMinted[_tokenId].minted = true;
    ...
    function tokenMinted(uint256 _tokenId) external view returns (bool) {
        if (toksMinted[_tokenId].minted) {
            return true;
        }
        else {
            return false;
    ...
    function withdrawfunds(address payable contractOwner) external onlyOwner() {
        AddressUpgradeable.sendValue(contractOwner, address(this).balance);
    }
