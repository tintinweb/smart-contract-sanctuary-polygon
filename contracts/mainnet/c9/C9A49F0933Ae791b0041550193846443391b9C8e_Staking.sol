...
contract Staking is Ownable, Pausable, ReentrancyGuard {
    ...
    event Withdrawed(address indexed user, uint256 cellId, uint256 bonusCount);
    event AddedToWhiteList(address indexed tokenAddress);
    event CellSet(uint256 cellId);
    event SetDeadline(uint256 newDeadline);

    constructor(uint256 _stakingDeadline) {
    ...
    function stake(uint256 _cellId, Character[] calldata _characters) external whenNotPaused nonReentrant {
        ...
        require(checkInWhiteList(_characters), "Only tokens from white list");
        require(usersStaking[_msgSender()].active == false, "Limit exceeded");
        require(cells[_cellId].active, "This cell is not active");
        require(cells[_cellId].charactersPower <= getPower(_characters), "Incorrect characters");
    ...
    function withdraw() external whenNotPaused nonReentrant {
        ...
        for (uint256 i = 0; i < _currentCell.charactersPower; i++) {
            ...
            // get bonuses
            salt = (salt * block.timestamp) % 77773;
            if (salt == 0) salt = random(block.number);
            uint256 _randomNumber = random(100);
            if (_randomNumber < _currentCell.bonusChance) {
                // transfer bonuses
                if (_currentCell.bonuses.length > 0) {
                    uint256 _bonusIdx = random(_currentCell.bonuses.length);
        ...
        emit Withdrawed(_msgSender(), usersStaking[_msgSender()].cellId, _bonusCount);
    ...
    function getPurityBonus(address _tokenAddress, uint256 _tokenId) external view returns (uint256) {
        return purityBonus[_tokenAddress][_tokenId];
    }


    // internal functions

    function random(uint256 _index) internal view returns (uint256) {
        uint256 randomHash = uint256(
            keccak256(
                abi.encodePacked(blockhash(block.number), block.timestamp % block.number, _msgSender(), salt, _index)
            )
        );
        return randomHash % _index;
    }

    function checkInWhiteList(Character[] calldata _characters) internal view returns (bool) {
        for (uint256 i = 0; i < _characters.length; i++) {
            if (!whiteList[_characters[i].tokenAddress]) return false;
        }
        return true;
    }

    function getPower(Character[] calldata _characters) internal view returns (uint256) {
        uint256 allPower;
        for (uint256 i = 0; i < _characters.length; i++) {
    ...
    function setPurityBonus(address[] calldata _tokenAddresses, uint256[] calldata _tokenIds, uint256[] calldata _bonuses) external onlyOwner {
    ...
    function setStakingDeadline(uint256 _stakingDeadline) external onlyOwner {
        stakingDeadline = _stakingDeadline;
        emit SetDeadline(_stakingDeadline);
    }
}