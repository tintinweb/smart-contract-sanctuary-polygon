...
import "./ItemBase.sol";
import "../../interfaces/IBuffItem.sol";

abstract contract BuffItemBase is ItemBase, IBuffItem {
  ...
  string public constant BUFF_ITEM_BASE_VERSION = "1.0.0";

  // ---- VARIABLES ----

  bool internal _buffItemInited;
  mapping(uint => EnumerableMap.UintToIntMap) internal _buffCasterAttributes;
  mapping(uint => EnumerableMap.UintToIntMap) internal _debuffCasterAttributes;
  mapping(uint => EnumerableMap.UintToIntMap) internal _debuffTargetAttributes;
  mapping(uint => EnumerableMap.UintToIntMap) internal _buffTargetAttributes;
  mapping(uint => uint) internal _buffItemMana;
  GenerateInfo internal _casterInfo;
  GenerateInfo internal _targetInfo;
  uint internal _manaConsumptionBase;

  // ---- EVENTS ----

  event BuffInfoChanged(GenerateInfo buffInfo, GenerateInfo debuffInfo, uint manaConsumption);

  // ---- INITIALIZER ----

  constructor() {_buffItemInited = true;}

  // ---- VIEWS ----

  function isBuffItem() external pure override returns (bool) {
    return true;
  }

  function buff(uint tokenId)
  external view override returns (uint[] memory casterBuff, uint[] memory casterDebuff, uint mana) {
    return (_buffCasterAttributes[tokenId].flat(), _debuffCasterAttributes[tokenId].flat(), _buffItemMana[tokenId]);
  }

  function debuff(uint tokenId)
  external view override returns (uint[] memory targetDebuff, uint[] memory targetBuff) {
    return (_debuffTargetAttributes[tokenId].flat(), _buffTargetAttributes[tokenId].flat());
  }

  // ---- GOV ACTIONS ----

  function setBuffInfo(GenerateInfo memory casterInfo_, GenerateInfo memory targetInfo_, uint manaConsumption_) external {
    require(isGovernance(msg.sender) || !_buffItemInited, "!gov");

    _casterInfo = casterInfo_;
    _targetInfo = targetInfo_;
    _manaConsumptionBase = manaConsumption_;

    _buffItemInited = true;

    emit BuffInfoChanged(casterInfo_, targetInfo_, manaConsumption_);
  }
  ...
  function _afterMint(uint tokenId) internal override {
    ...
    GenerateInfo memory __buffInfo = _casterInfo;
    _generateAttributes(_buffCasterAttributes[tokenId], __buffInfo, tokenId, true);
    _generateAttributes(_debuffCasterAttributes[tokenId], __buffInfo, tokenId, false);

    GenerateInfo memory __debuffInfo = _targetInfo;
    _generateAttributes(_buffTargetAttributes[tokenId], __debuffInfo, tokenId, true);
    _generateAttributes(_debuffTargetAttributes[tokenId], __debuffInfo, tokenId, false);

    _buffItemMana[tokenId] = _manaConsumptionBase;
  }

  function _additionalAugment(uint tokenId) internal override virtual {
    IItemCalculator itemCalculator = IItemCalculator(IController(controller()).itemCalculator());
    _augmentAttributes(itemCalculator, _buffCasterAttributes[tokenId]);
    _augmentAttributes(itemCalculator, _debuffTargetAttributes[tokenId]);

    _buffItemMana[tokenId] = itemCalculator.augmentAttribute(_buffItemMana[tokenId]);
  }

  function _augmentAttributes(IItemCalculator _itemCalculator, EnumerableMap.UintToIntMap storage attributes) internal {
    uint length = attributes.length();
    for (uint i; i < length;) {
      (uint index, int valueI) = attributes._at(i);
      if (valueI > 0) {
        uint value = uint(valueI);
        attributes.setUint(index, _itemCalculator.augmentAttribute(value));
      }
    unchecked{++i;}
    }
  }
  ...
  function _generateAttributes(
    ...
    GenerateInfo memory info,
    uint tokenId,
    bool onlyPositiveChances
  ) internal virtual {
    // consumable items always normal rarity
    itemRarity[tokenId] = uint(IItem.ItemRarity.NORMAL);

    IItemCalculator itemCalculator = IItemCalculator(IController(controller()).itemCalculator());

    (uint[] memory ids, uint[] memory values) = itemCalculator.generateSkillAttributes(info, onlyPositiveChances);
...
import "../base/item/BuffItemBase.sol";

contract ItemCommonBuff is BuffItemBase {
