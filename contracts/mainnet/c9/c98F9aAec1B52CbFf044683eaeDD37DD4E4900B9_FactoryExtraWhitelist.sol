...
contract FactoryExtraWhitelist is OwnableUpgradeable {
  address public immutable TIMELOCK_IMPL;

  address private fixedPriceExtraWhitelistDeployer; // pay erc20 tokens to mint PASS with fixed price
  ...
  constructor(
    address _timelock,
    address _fixedPriceExtraWhitelistDeployer,
    address payable _platform,
    uint256 _platformRate
  ) {
    __Ownable_init(_timelock);
    timelock = _timelock;
    fixedPriceExtraWhitelistDeployer = _fixedPriceExtraWhitelistDeployer;
    _setPlatformParms(_platform, _platformRate);

    TIMELOCK_IMPL = address(new Timelock());
  }

  event FixedPriceExtraWhitelistDeploy(
  ...
  function fixedPriceExtraWhitelistDeploy(
    string memory _name,
    ...
    IFixedPriceExtraWhitelistDeployer factory = IFixedPriceExtraWhitelistDeployer(fixedPriceExtraWhitelistDeployer);
    address addr = factory.deployFixedPriceExtraWhitelist(
      _name,
      _symbol,
      _bURI,
    ...
    emit FixedPriceExtraWhitelistDeploy(
      addr,
      _name,
      _symbol,
      _bURI,
