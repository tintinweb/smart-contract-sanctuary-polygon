...
// File: contracts/721_Base/Contract Base721 V45.sol
/*
...
contract ContractLive721V45new is ERC721, Ownable, ERC2981, DefaultOperatorFilterer, ContextMixin, RoyaltiesV2Impl { 
    ...
    function mintAllAtOnce() public onlyOwner  { 
        require(!paused, "mintAllAtOnce - the contract is paused.");
    ...
    function mintArea(uint256 areaStart, uint256 areaEnd) public onlyOwner  { 
        require(!paused, "mintArea - the contract is paused.");

        for(uint256 i= areaStart; i<= areaEnd; i++) {
            uint256 startTokenId = 0;
            require(areaEnd <= maxTokenInCollection, "mintArea - the areaEnd value can not be bigger then the max. number of token in collection. Decrease the value areaEnd and try again");
    ...
    function mintSingle(uint256 tokenId) public onlyOwner {
        require(!paused, "mintSingle - the contract is paused.");

        require(tokenId <= maxTokenInCollection, "mintSingle - the tokenId can not be bigger then the max. number of token in collection. Try again");
        require(exist[tokenId]  == false, "mintSingle - you cannot mint a token that already exists OR the token was previously burned.");
    ...
    function transferFrom(address from, address to, uint256 tokenId) public override onlyAllowedOperator(from) onlyOwner {                         
        super.transferFrom(from, to, tokenId);
    }

    function safeTransferFrom(address from, address to, uint256 tokenId) public override onlyAllowedOperator(from) onlyOwner {                     
        super.safeTransferFrom(from, to, tokenId);
    }

    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory data) public override onlyAllowedOperator(from) onlyOwner {  
