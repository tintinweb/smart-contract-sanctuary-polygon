...
interface IERC721Fund {
    function initializeDonation(address _receiver, uint256 _fee) external;
}
...
interface IManagement {
    ...
    function tokenERC20PerShare() external view returns (uint256);
    ...
    function newFundCollection(
        ...
        string memory _symbol,
        uint256 _maxShares,
        uint256 _pricePerShareInETH,
        uint256 _pricePerShareInUSD,
        uint256 _pricePerShareInCreatorsCoin,
        string memory _baseURI,
        uint256 _tokenERC721PerShare,
...
import "./interfaces/IERC721Fund.sol";
...
contract Management is IManagement, Initializable, UUPSUpgradeable {
    ...
    uint256 public tokenERC20PerShare;

    mapping(address => bool) public allowedCreators;
    mapping(address => bool) public managers;
    mapping(address => mapping(uint256 => HashObject)) public hashObjects;
    ...
    /** @dev event for when a new ERC721 crowdfund collection is instantiated
        @param collection: new ERC721 crowdfund collection address
        @param creator: collection creator address */
    event FundCollection(address indexed collection, address indexed creator);
    ...
    /** @dev event for when a new amount of ERC20 per share is set
        @param manager: manager address that has done the setting
        @param newERC20PerShare: new amount of ERC20 per share */
    event TokenERC20PerShareSet(
        address indexed manager,
        uint256 newERC20PerShare
    );
    ...
    /** @notice instantiates/deploys new NFT fund collection smart contract.
        @param _name: name of the NFT collection
        @param _symbol: symbol of the NFT collection
        @param _maxShares: maximum shares for the crowdfund
        @param _pricePerShareInETH: price per share in ETH/MATIC
        @param _pricePerShareInUSD: price per share in USD (USDC/DAI)
        @param _pricePerShareInCreatorsCoin: price per share in CreatorsCoin
        @param _baseURI: base URI for the collection's metadata 
        @param _tokenERC721PerShare: mintable ERC721 token per share
        @param _donationReceiver: address from donation receiver 
        @param _donationFee: donation fee */
    function newFundCollection(
        string memory _name,
        string memory _symbol,
        uint256 _maxShares,
        ...
        validateAddress(beaconAdminFund)
    {
        ...
        bytes memory ERC721initialize = abi.encodeWithSignature(
            ...
            _pricePerShareInETH,
            _pricePerShareInUSD,
            _pricePerShareInCreatorsCoin,
            _baseURI,
            _tokenERC721PerShare
        );
        ...
        IERC721Fund(address(newCollectionProxy)).initializeDonation(
            _donationReceiver,
            _donationFee
        );
    ...
    {
        require(
            _price.length == _priceUSDC.length &&
                _price.length == _priceCreatorsCoin.length &&
                _price.length == _maxSupply.length,
            "Management: prices/max supply array don't have same length"
        );

        for (uint256 ii; ii < _maxSupply.length; ++ii) {
            require(
                _maxSupply[ii] > 0,
                "Management: batch max supply cannot be 0"
            );
        }
    ...
    function setHashObject(
        ...
        require(
            ...
            "Management: arrays haven't same length"
        );
    ...
    /** @notice sets a new amount of ERC20 token per share
        @param  _tokenERC20PerShare: new amount of ERC20 token per share */
    function setTokenERC20PerShare(uint256 _tokenERC20PerShare)
        external
        onlyManagers
    {
        tokenERC20PerShare = _tokenERC20PerShare;

        emit TokenERC20PerShareSet(msg.sender, _tokenERC20PerShare);
    }
    ...
    function getImplementation() external view onlyManagers returns (address) {
