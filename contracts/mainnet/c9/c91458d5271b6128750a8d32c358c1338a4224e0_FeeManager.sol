...
contract FeeManager is OwnableUpgradeable, UUPSUpgradeable {
    ...
    uint256 public poolAllocation;

    uint256 public poolWETHAllocation;
    uint256 public poolWBTCAllocation;
    uint256 public poolWMATICAllocation;
    uint256 public poolUSDCAllocation;
    ...
    function deposit() external onlyWhitelist {
        ...
        uint256 balance = IERC20(GFI).balanceOf(address(this));

        if (balance > 0) {
            TransferHelper.safeTransfer(GFI, incinerator, balance);
        }
        ...
        if (amountWETH > 0) {
            ...
            if (poolAllocation > 0) {
                uint256 amountPool = (amountWETH * poolAllocation) /
                    PERCENT_DECIMALS;
                poolAct(amountPool);
            }
    ...
    function addLiquidity(
        ...
        uint256 swapAmount = _getOptimal(pair, token0, amount0);
        path[0] = token0;
        path[1] = token1;
        ...
        if (swapAmount > 0) {
            ...
            (amounts, result) = swapTokens(router, path, swapAmount, 0);
            ...
            if (result) {
                ...
                TransferHelper.safeTransfer(pair, governor, liquidity);
            }
        }
    }

    function addPool() external onlyWhitelist {
        uint256 amountWETH = IERC20(WETH).balanceOf(address(this));
        if (amountWETH > 0 && poolAllocation > 0) {
            uint256 amountPool = (amountWETH * poolAllocation) /
                PERCENT_DECIMALS;
            poolAct(amountPool);
        }
    ...
    function poolAct(uint256 amountPool) internal {
        address[] memory path = new address[](2);
        uint256[] memory amounts = new uint256[](2);
        bool result;

        IERC20(WETH).approve(QUICK_ROUTER, amountPool);

        if (poolWETHAllocation > 0) {
            uint256 amountPoolWETH = (amountPool * poolWETHAllocation) /
                PERCENT_DECIMALS;
            addLiquidity(WETH, GFI, amountPoolWETH);
        }

        if (poolWBTCAllocation > 0) {
            uint256 amountPoolWBTC = (amountPool * poolWBTCAllocation) /
                PERCENT_DECIMALS;
            ...
            if (result) {
                addLiquidity(WBTC, GFI, amounts[1]);
            }
        }

        if (poolWMATICAllocation > 0) {
            uint256 amountPoolWMATIC = (amountPool * poolWMATICAllocation) /
                PERCENT_DECIMALS;
            ...
            if (result) {
                addLiquidity(WMATIC, GFI, amounts[1]);
            }
        }

        if (poolUSDCAllocation > 0) {
            uint256 amountPoolUSDC = (amountPool * poolUSDCAllocation) /
                PERCENT_DECIMALS;
            ...
            if (result) {
                addLiquidity(USDC, GFI, amounts[1]);
            }
        }
    }
    ...
    function updateGovernor(address _governor) external onlyOwner {
        require(_governor != address(0), "wrong governor");
        governor = _governor;
    }
    ...
    function updatePoolAllocation(
        uint256 _poolWETHAllocation,
        uint256 _poolWBTCAllocation,
        uint256 _poolWMATICAllocation,
        uint256 _poolUSDCAllocation
    ) external onlyOwner {
        require(
            _poolWETHAllocation +
                _poolWBTCAllocation +
                _poolWMATICAllocation +
                _poolUSDCAllocation ==
                PERCENT_DECIMALS,
            "wrong pool allocation"
        );
        poolWETHAllocation = _poolWETHAllocation;
        poolWBTCAllocation = _poolWBTCAllocation;
        poolWMATICAllocation = _poolWMATICAllocation;
        poolUSDCAllocation = _poolUSDCAllocation;
    }
}
