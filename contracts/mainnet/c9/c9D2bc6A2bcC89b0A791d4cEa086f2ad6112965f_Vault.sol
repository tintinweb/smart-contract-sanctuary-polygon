...
interface IStrategy {
    ...
    function claimFromStaking() external returns (uint256);
    ...
    function takeFee(uint256) external;
    function setPercentCheckDifference(uint256) external;
...
contract Vault is ERC20Simplified, Ownable {
    ...
    uint256 public depositLimitMIN = 1 * 10 ** 16;
    uint256 public defaultStrategyID = 0;
    uint256 public depositLimit = 1000 * 10 ** 18;
    uint256 public maxLimitPerDeposit = 1000 * 10 ** 18;
    address public governance = 0x4e5b3043FEB9f939448e2F791a66C4EA65A315a8;
    bool public depositForOne = true;
    // string messages
    ...
    constructor() ERC20Simplified("Ponzi Shares", "PS") {}
    ...
    function deposit() public payable notBlocked {
        _checkDeposit(msg.value);
        getFee();
        require(getTotalCapital() + msg.value <= depositLimit, "Deposit is limited by contract");
        require(names.length != 0, NO_STRATEGY_ERROR);
        uint256 shares;
        uint256 totalCapitalInternal = getTotalCapitalInternal();
        uint256 depositorCapitalValue;

        if(depositForOne) {
            depositorCapitalValue = _depositForOne(msg.value);
        } else {
            depositorCapitalValue = _depositForAll(msg.value);
        }
    ...
    function _depositForOne(uint256 _value) internal returns (uint256) {
        return IStrategy(currentStrategy()).deposit{value: _value}();
    }

    function _depositForAll(uint256 _totalValue) internal returns (uint256 dcv) {
        // vps = Value Per Strategy
        uint256 vps = _totalValue / names.length;
        for (uint256 i=1; i < names.length; i++) {
            // dcv = Depositor Capital Value
            dcv += IStrategy(strategies[names[i]]).deposit{value: vps}();
        }
    }
    ...
    function setDefaultStrategy(uint256 _nameID) public onlyOwner {
        _checkParameters(_nameID);
        defaultStrategyID = _nameID;
    }

    function setDepositRule(bool _isDepositForOne) public onlyOwner {
        depositForOne = _isDepositForOne;
    }
    ...
    function getFee() public executableAfterUpdate {
        require(names.length != 0, NO_STRATEGY_ERROR);
        address mainToken;
        uint256 oldResult;
        uint256 newResult;
        for (uint256 i = 0; i < names.length; i++) {
            mainToken = IStrategy(strategies[names[i]]).mainToken();
            oldResult = IERC20(mainToken).balanceOf(strategies[names[i]]);
            newResult = IStrategy(strategies[names[i]]).claimFromStaking();
            if(newResult - oldResult > 0) {
                IStrategy(strategies[names[i]]).takeFee(newResult - oldResult);
            }
        }
    }
    ...
    function setNumberOfBlocksBtwTX(uint256 _amount) public onlyOwner {
        NUMBER_OF_BLOCKS_BTW_TX = _amount;
    }

    function updateTWAP() public onlyOwnerOrGovernance {
        for(uint256 i = 0; i < names.length; i++) {
    ...
    function setMaxDeposit(uint256 _amount) external onlyOwner {
        maxLimitPerDeposit = _amount;
    }

    function setPercentCheckDifference(uint256 _strategyID, uint256 _percent) external onlyOwner {
        require(nameExist[_strategyID], STRATEGY_DOES_NOT_EXIST);
        IStrategy(strategies[_strategyID]).setPercentCheckDifference(_percent);
    }

    function addStrategy(address _newStrategy, uint256 _nameID) public onlyOwner {
    ...
    function removeStrategy(uint256 _nameID) public onlyOwner {
        _checkParameters(_nameID);
        require(
    ...
    function setRouterForStrategy(address _newRouter, uint256 _nameID) public onlyOwner {
        _checkParameters(_nameID);
    ...
    function migrate(
        ...
        uint256 _nameIdTo) public onlyOwner {
    ...
    function _migrate(uint256 _nameIdFrom, uint256 _amountInPercent, uint256 _nameIdTo) internal {
        ...
        address wrappedToken = IStrategy(strategies[_nameIdFrom]).wrappedToken();
        // take wrapped Tokens from old strategy
        IStrategy(strategies[_nameIdFrom]).migrate(_amountInPercent);
        uint256 _balance = IERC20(wrappedToken).balanceOf(address(this));
        if(_balance > 0){
            // put wrapped Tokens to new strategy
            IERC20(wrappedToken).safeTransfer(strategies[_nameIdTo], _balance);
    ...
    function _checkDeposit(uint256 _value) internal view {
        require(
        ...
        require(_value >= depositLimitMIN, "Funds should be >= than depositLimitMIN");
        require(_value <= maxLimitPerDeposit, "Deposit is more then maxLimitPerDeposit");
    }
}