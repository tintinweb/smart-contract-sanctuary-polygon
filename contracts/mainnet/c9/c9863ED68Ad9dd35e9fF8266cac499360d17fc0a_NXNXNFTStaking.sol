...
contract NXNXNFTStaking is Ownable, IERC1155Receiver, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    enum NftCategory {
        MAIN_ITEM,  // Main NFT item
        SUB_ITEM    // Sub NFT item
    }
    ...
    // max NFT sub item limit
    uint256 public maxSubNftLimit = 3;

    // Harvest lock configuration
    uint256 public harvestInterval = type(uint256).max;
    bool public harvestLockOn = true;
    uint256 public totalLockedUpRewards;

    // Withdraw lock configuration
    WithdrawLockProperty public withdrawLockMethod =
        WithdrawLockProperty.NEVER_UNLOCKABLE;
    uint256 public withdrawLockPeriod = 0; // in seconds
    uint16 public withdrawFee = 0; // 100x multied, 80% default
    ...
    uint256 public constant DEFAULT_BOOST_PERCENT = 0; // Default amount boost percent for staking nft 0%
    ...
    struct BoostInfo {
        ...
        NftCategory nftCategory;    // NFT Category
        address nftContractAddress;
        uint256 nftTokenId;
        string nftName;
        uint256 value;  // NFT value worth Staking Token
    }
    ...
    function getUserEtmAmount(address _account) public view returns (uint256) {
        UserInfo memory _user = userInfo[_account];
        return getEtmBalance(_user.amount, true, _user.nftAddress, _user.nftTokenId, _user.nftItems);
    }   
    ...
    function getEtmBalance(
        ...
        // no count if main nft is not eligible
        if (boostMultiplier > 10000) {
            // calculate boosted amount from sub nft items
            uint _itemLength = _nftItems.length;
            for (uint i=0; i<_itemLength; i++) {
                if (extraBoosts[_nftItems[i].nftAddress][_nftItems[i].nftTokenId].boostPercent > 0) {
                    boostMultiplier = boostMultiplier.add(
                        extraBoosts[_nftItems[i].nftAddress][_nftItems[i].nftTokenId].boostPercent
                    ); 
                }
            }
        }
    ...
    function depositNft(
        ...
        require(
            NftCategory.MAIN_ITEM == extraBoosts[_nftAddress][_nftTokenId].nftCategory
        ...
        uint256 _beforeAmount = getUserEtmAmount(_msgSender());
        ...
        totalStakings = totalStakings.add(extraBoosts[_nftAddress][_nftTokenId].value);

        // increase stakedAmount for nft address + token id
        adjustStakingAmountForNFT(user.amount, user.nftAddress, user.nftTokenId, user.nftItems, true);
        ...
        etmTotalStakings = etmTotalStakings.add(etmAmount).sub(_beforeAmount);
    ...
    function depositSubNft(
        ...
        require(
            NftCategory.SUB_ITEM == extraBoosts[_nftAddress][_nftTokenId].nftCategory
            && extraBoosts[_nftAddress][_nftTokenId].boostPercent > 0,
            "This NFT is not eligible"
        );
        ...
        // increase stakedAmount for nft address + token id
        // increaseStakingAmountForNFT(user.amount, user.nftAddress, user.nftTokenId, user.nftItems);
        extraBoosts[_nftAddress][_nftTokenId
            ].stakedAmount = extraBoosts[_nftAddress][_nftTokenId].stakedAmount.add(user.amount);
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        UserInfo storage user = userInfo[_msgSender()];
        require(
            _amount == 0, "depositing token is not allowed"
        );
        require(
        ...
        if (_amount > 0) {
            ...
            if (user.nftStakeStatus != NftStakeProperty.NO_STAKED) {
                adjustStakingAmountForNFT(_amount, user.nftAddress, user.nftTokenId, user.nftItems, true);
    ...
    function withdrawNft() external nonReentrant {
        ...
        if (user.nftStakeStatus == NftStakeProperty.ERC721_STAKED) {
            ...
            // address withdrawFrom = address(this);
            // if (isNFTorigin[_msgSender()]) {
            //     withdrawFrom = oldContract;
            // }
        ...
        extraBoosts[user.nftAddress][user.nftTokenId
            ].stakedAmount = extraBoosts[user.nftAddress][user.nftTokenId
                ].stakedAmount.sub(user.amount);
    ...
    function withdrawSubNft(uint256 _index) public {
        ...
        if (_index < user.nftItems.length) {
            ...
            if (info.nftAddress != address(0)) {
                ...
                extraBoosts[info.nftAddress][info.nftTokenId
                    ].stakedAmount = extraBoosts[info.nftAddress][info.nftTokenId
                        ].stakedAmount.sub(user.amount);
                delete user.nftItems[_index];

                uint256 etmAmount = getUserEtmAmount(_msgSender());
                etmTotalStakings = etmTotalStakings.add(etmAmount).sub(beforeAmount);
            }
        }

    }

    function withdrawSubNftAll() public {
    ...
    function adjustStakingAmountForNFT(
        uint256 _amount, address _nftAddress, 
        uint256 _nftTokenId, NftInfo[] storage _nftItems,
        bool isIncrease
    ) internal {
        // update the amount of staking for each NFT
        if (isIncrease) {
            extraBoosts[_nftAddress][_nftTokenId]
                        .stakedAmount = extraBoosts[_nftAddress][
                        _nftTokenId
                    ].stakedAmount.add(_amount);
        } else {
            extraBoosts[_nftAddress][_nftTokenId]
                        .stakedAmount = extraBoosts[_nftAddress][
                        _nftTokenId
        ...
        uint length = _nftItems.length;
        if (length > 0) {
            NftInfo memory info;
            for (uint i=0; i<_nftItems.length; i++) {
                info = _nftItems[i];
                if (info.nftAddress == address(0)) {
                    continue;
                }
                if (isIncrease) {
                    extraBoosts[info.nftAddress][info.nftTokenId]
                                .stakedAmount = extraBoosts[info.nftAddress][
                                info.nftTokenId
                            ].stakedAmount.add(_amount);
                } else {
                    extraBoosts[info.nftAddress][info.nftTokenId]
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (withdrawAvailable) {
            ...
            if (user.nftStakeStatus != NftStakeProperty.NO_STAKED) {
                adjustStakingAmountForNFT(_amount, user.nftAddress, user.nftTokenId, user.nftItems, false);
                _amount = getEtmBalance(
                    _amount,
                    true,
                    user.nftAddress,
        ...
        returns (bool _available, uint256 _feeAmount)
    {
        ...
        if (
            withdrawLockMethod == WithdrawLockProperty.UNLOCKABLE_WITH_FEE &&
            withdrawLockPeriod == 0 && block.number < rewardEndBlock
        ) {
            _feeAmount = _amount.mul(withdrawFee).div(10000);
        }
    }
    ...
    function payOrLockupPendingReward() internal {
        ...
        // emit PendingReward(_msgSender(), pending);
        if (user.amount == 0) {
        ...
        if (canHarvest(_msgSender())) {
            if (pending > 0) {
                ...
                if (rewardTransferred > 0) {
                    ...
                    user.lastHarvestedAt = block.timestamp;
                    user.rewardReceived = user.rewardReceived.add(rewardTransferred);
                }
            ...
            totalLockedUpRewards = totalLockedUpRewards.add(pending).sub(user.rewardLocked);
            user.rewardLocked = pending;
            emit RewardLocked(_msgSender(), pending);
        }
    }
    // event PendingReward(address indexed account, uint256 amount);
    ...
    function updatePool() internal {
        ...
        // uint256 oldAccRewardPerShare = accRewardPerShare;
    ...
    function updateExtraBoost(
        ...
        extraBoosts[_nftAddress][_nftTokenId].nftCategory = _category;
        ...
        extraBoosts[_nftAddress][_nftTokenId].value = _value;
    ...
    function getUserSubNftItems(address _account) public view 
        returns (
            NftInfo[] memory _nftItems
        )
    {
        UserInfo memory user = userInfo[_account];
        _nftItems = user.nftItems;
    }

    /**
     * @notice update max stakings .
     * @param _maxStakings: new maxStakings must be grater than old one
     */
    function updateMaxStakings(uint256 _maxStakings) external onlyOwner {
        require(_maxStakings > maxStakings, 'max stakings must be grater than old one');
        maxStakings = _maxStakings;
    }
     ...
     * @param _rewardStartBlock: the new start block, not change if it's 0
    ...
    function updateRewardBlocks(
        ...
        require(
            block.number < _rewardStartBlock || _rewardStartBlock == 0,
