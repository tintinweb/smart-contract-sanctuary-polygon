...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
        ...
        )
    {
        ...
        rewardToFarmTokenPath0[1] = usdcAddr;
    ...
    function harvest() public override nonReentrant {
        ...
        if (_rewardBalance > harvestCutoffBps) {
            ...
            uint256 _amountPostSwap = IERC20(feeTokenAddr).balanceOf(address(this));
            if (kogefarmFeeAmountBps > 0) {
              uint256 _feeAmount = _amountPostSwap.mul(kogefarmFeeAmountBps).div(
                  10000
              );
              // Send fee to strategist
              IERC20(feeTokenAddr).safeTransfer(strategist, _feeAmount);
            ...
            // Stake farm0
            uint256 _amountPostFee = IERC20(farmToken0Addr).balanceOf(address(this));
            IERC20(farmToken0Addr).safeApprove(stargateRouter, 0);
            IERC20(farmToken0Addr).safeApprove(stargateRouter, _amountPostFee);
            IStargateRouter(stargateRouter).addLiquidity(1, _amountPostFee, address(this));
            // Stake the LP tokens
            uint256 _want = IERC20(want).balanceOf(address(this));
            deposit(_want);
        }
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    ...
    address public LP_TOKEN = 0x29e38769f23701A2e4A8Ef0492e19dA4604Be62c;
