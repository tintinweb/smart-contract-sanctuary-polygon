...
contract PositionNFTManager is UniSwapV3Pending, Ownable {
    ...
    modifier onlyWhiteList() {
        require(whiteList[msg.sender] || msg.sender == address(this), "1");
        _;
    }
    ...
    /// @dev Low Level
    function _burnLiquidity(uint256 tokenId, uint128 _liquidity)
        internal
        returns (uint256 amount0, uint256 amount1)
    {
        INonfungiblePositionManager.DecreaseLiquidityParams
            memory param = INonfungiblePositionManager.DecreaseLiquidityParams({
                tokenId: tokenId,
...
contract UniSwapV3Pending {
    ...
    function getPendingAmounts(uint256 tokenId)
        public
        view
    ...
    function _getFeeGrowthInside(
        ...
        (uint256 feeGrowthBelow0X128, uint256 feeGrowthBelow1X128) = _tickInfo(
            pool,
            lowerTick
        );
        ...
        (uint256 feeGrowthAbove0X128, uint256 feeGrowthAbove1X128) = _tickInfo(
            pool,
            upperTick
        );
    ...
    function _poolInfo(IUniswapV3Pool pool)
        internal
        view
        returns (
            int24,
    ...
    function _positionInfo(uint256 tokenId)
        internal
        view
        returns (address poolAddress, NftInfo memory nftInfo)
    {
        nftInfo = getNftInfo(tokenId);
        poolAddress = PoolAddress.computeAddress(
            factory,
            PoolAddress.getPoolKey(nftInfo.token0, nftInfo.token1, nftInfo.fee)
        );
    }

    function _tickInfo(IUniswapV3Pool pool, int24 tick)
        internal
        view
        returns (uint256 feeGrowthOutside0X128, uint256 feeGrowthOutside1X128)
    {
        ...
        (, , feeGrowthOutside0X128, feeGrowthOutside1X128, , , , ) = pool.ticks(
            tick
        );
    }
...
library PositionNFTUtil {
    ...
    function nftStakeAll(
        ...
        IUniswapV3Staker.IncentiveKey[] memory activeList = config
            .findActiveIncentive(address(this), poolAddress);
        if (activeList.length > 0) {
            (uint128 liquidity, , ) = _positionInfo(tokenId);
            if (liquidity > 0) {
    ...
    // @dev stake to Incentive
    function nftUnStakeAll(IUniverseVaultConfig config, uint256 tokenId)
        internal
    {
        (address owner, , , ) = v3Staker.deposits(tokenId);
        if (owner == address(this)) {
        ...
        )
    {
        ...
        if (!status) {
            ...
            if (
                position.positionTick - tickNow >= reBalanceThreshold ||
                tickNow - position.positionTick >= reBalanceThreshold
            ) {
                status = true;
            }
        }
    ...
    function checkDiffTick(
        ...
        require(
            tick - _spotTick < int24(_diffTick) &&
                _spotTick - tick < int24(_diffTick),
            "DF"
        );
    }
    ...
    function currentTick(address poolAddress)
        internal
        view
        returns (int24 tick)
    {
...
interface IUniverseVaultConfig {
    ...
    /// @param vaultAddress Address of Universe V3 Vault
    function maxShares(address vaultAddress)
        external
        returns (MaxShares memory);

    /// @notice The Safety Param for deposit
    /// @param vaultAddress Address of Universe V3 Vault
    function safeAddLiq(address vaultAddress)
        external
        returns (SafeAddLiq memory);
