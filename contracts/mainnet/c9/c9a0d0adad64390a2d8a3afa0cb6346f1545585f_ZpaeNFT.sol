...
contract ContractRoles is AccessControlUpgradeable {
    ...
    function grantAdminRole(address user) external onlyRole(ADMIN_ROLE) {
        _grantRole(ADMIN_ROLE, user);
    }

    function revokeAdminRole(address user) external onlyRole(ADMIN_ROLE) {
    ...
    function revokeMinterRole(address user) external onlyRole(ADMIN_ROLE) {
        _revokeRole(MINTER_ROLE, user);
    }

    function userHasRole(bytes32 role, address user) external view returns(bool) {
...
contract ZpaeNFT is ERC20Upgradeable, ContractRoles {
