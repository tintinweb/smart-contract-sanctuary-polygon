    ...
    ERC1155HolderUpgradeable
{
    ...
    IBalanceVaultV2 public balanceVaultV2;
    ...
    uint256 public gachaponFees;
    uint256 public priceFeesPercentage;
    uint256 public latestGachaId;
    uint256 public itemCapPerTx;

    event GachaponCreated(uint256 indexed gachaId, address indexed gachaOwner, uint256 price, bool status);
    event GachaponItemsAdded(uint256 indexed gachaId);
    event GachaponItemsRemoved(uint256 indexed gachaId, uint256 indexed itemsId);
    ...
    event GachaponBought(uint256 indexed gachaId, address indexed userAddress, uint256 requestId, uint256 price, uint256 gachaponFees, uint256 priceFees);
    ...
    event GachaStatusUpdated(uint256 indexed gachaId, bool status);
     ...
     * @param  _balanceVaultV2Address - address of balance vault.
     ...
     * @param  _gachaponFees - fixed fees for each gachapon sold.
     * @param  _priceFeesPercentage - price fees percentage in two precision decimal.
     */
    function initialize(
        address _balanceVaultV2Address,
        ...
        uint32 _numWords,
        uint256 _gachaponFees,
        uint256 _priceFeesPercentage,
        uint256 _itemCapPerTx
    ) public initializer {
        ...
        balanceVaultV2 = IBalanceVaultV2(_balanceVaultV2Address);
        coordinator = VRFCoordinatorV2Interface(_vrfCoordinator);

        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        grantRole(WORKER, msg.sender);
        grantRole(DEPLOYER, msg.sender);
        ...
        numWords = _numWords;
        gachaponFees = _gachaponFees;
        priceFeesPercentage = _priceFeesPercentage;
        itemCapPerTx = _itemCapPerTx;
    }
    ...
    // VRF setter
    function setVRFCoordinator(address _vrfCoordinator) external whenPaused onlyOwner {
        coordinator = VRFCoordinatorV2Interface(_vrfCoordinator);
    }
    ...
    function setGachaponFees(uint256 _gachaponFees) external onlyRole(WORKER) {
        gachaponFees = _gachaponFees;
    }
    function setPriceFeesPercentage(uint256 _priceFeesPercentage) external onlyRole(WORKER) {
        require(
            _priceFeesPercentage <= 10000,
            "[GachaponService.setPriceFeesPercentage] Invalid price fees percentage"
        );
        priceFeesPercentage = _priceFeesPercentage;
    }
    function setItemCapPerTx(uint256 _itemCapPerTx) external onlyRole(WORKER) {
       require(
            _itemCapPerTx  > 0,
            "[GachaponService.setItemCapPerTx] Item cap per transaction must be more than 0"
        );
        itemCapPerTx = _itemCapPerTx;
    }
    function setGachaPrice(uint256 _gachaId, uint256 _price) external onlyGachaOwner(_gachaId) {
        ...
        emit GachaPriceUpdated(_gachaId, _price);
    }
    function setGachaStatus(uint256 _gachaId, bool _isActive) external onlyGachaOwner(_gachaId) {
        gachaByGachaId[_gachaId].isActive = _isActive;
        emit GachaStatusUpdated(_gachaId, _isActive);
    }
    ...
    function setBalanceVaultV2Address(address _balanceVaultV2Address) external onlyOwner {
        balanceVaultV2 = IBalanceVaultV2(_balanceVaultV2Address);
    }
    ...
    /**
     * @dev Create gachapon, gachapon index, transfer and store item.
     * @param _itemsList - array of set of gachapon item.
     * @param _price - betting period ending time.
     * @param _isActive - status whether gachapon available for purchase.
     */
    function createGachapon(
        Item[][] memory _itemsList,
        uint256 _price,
        bool _isActive
    ) external whenNotPaused notEmptyItems(_itemsList) {
        latestGachaId++;
        ...
        gacha.isActive = _isActive;
        gacha.gachaIdxActualSize = _itemsList.length;
        _verifyAndAddItems(gacha, _itemsList);

        emit GachaponCreated(latestGachaId, msg.sender, _price, _isActive);
    }

    /**
     * @dev Add item to exist gachapon, update gachapon index, transfer and store added item.
     * @param _gachaId - existing gachapon id.
     * @param _itemsList - array of set of gachapon item.
     */
    function addGachaponItems(uint256 _gachaId, Item[][] memory _itemsList)
        external
        whenNotPaused
        onlyGachaOwner(_gachaId)
        notEmptyItems(_itemsList)
    {
        ...
        emit GachaponItemsAdded(_gachaId);
    }

    /**
     * @dev Buy gachapon, request random words from chainlink vrf, store random info, transfer balance from buyer.
     * @param _gachaId - existing gachapon id.
     */
    function buyGachapon(uint256 _gachaId) external whenNotPaused {
        ...
        require(
            gacha.isActive,
            "[GachaponService.buyGachapon] Gachapon is not active"
        );
        require(
        ...
        uint256 priceFees;
        if(priceFeesPercentage > 0) {
            priceFees = gacha.gachaPrice * priceFeesPercentage / 10000;
        }
        
        balanceVaultV2.payWithUpo(msg.sender, gacha.gachaPrice + gachaponFees);
        fees += priceFees + gachaponFees;
        balanceVaultV2.transferUpoToAddress(gacha.gachaOwner, gacha.gachaPrice - priceFees);
        gacha.soldCount++;
        ...
        emit GachaponBought(_gachaId, msg.sender, requestId, gacha.gachaPrice, gachaponFees, priceFees);
    }

    /**
     * @dev Transfer gachapon items back to gacha owner if valid, update gachapon index.
     * @param _gachaId - existing gachapon id.
     * @param _itemsId - to be removed items id.
     */
    function removeGachaponItems(
        uint256 _gachaId,
        uint256 _itemsId
    ) external whenNotPaused onlyGachaOwner(_gachaId) {
        Gacha storage gacha = gachaByGachaId[_gachaId];
        require(
            gacha.gachaIdxActualSize > 0,
        ...
        // Verify item in gachapon index
        uint256[] memory gachaponIndexes = getGachaponIndexes(_gachaId, 0, gacha.gachaIdxCurrentSize - 1);
        uint256 gachaIdx;
        bool isMatch;
        for (uint256 i = 0; i < gachaponIndexes.length; i++) {
            if(gachaponIndexes[i] == _itemsId) {
                gachaIdx = i;
                isMatch = true;
                break;
            }
        ...
        require(
            isMatch,
            "[GachaponService.removeGachaponItems] Gachapon items not exist in gachapon index"
        );

        gacha.gachaIdxActualSize--;
        _removeGachaponIndex(gacha, gachaIdx);
        _transferItems(_gachaId, gacha.gachaOwner, _itemsId);

        emit GachaponItemsRemoved(_gachaId, _itemsId);
    }
    ...
    function claimGachaponItems(uint256 _gachaId, uint256 _requestId) external {
        ...
        _transferItems(_gachaId, msg.sender, randomInfo.itemsId);

        emit GachaponItemsClaimed(_gachaId, _requestId, randomInfo.itemsId);
    }

    /**
     * @dev Transfer remaining fees to admin address.
     */
    function adminFeesWithdraw() external whenNotPaused onlyRole(WORKER) {
        uint256 withdrawAmount = fees;
        require(
               fees != 0,
            "[GachaponService.adminFeesWithdraw] Fees is zero"
        );
        fees = 0;
        balanceVaultV2.transferUpoToAddress(msg.sender, withdrawAmount);
    }

    /**
     * @dev VRFConsumerBaseV2 rawFulfillRandomWords.
     */
    ...
    function getGachaponIndexItemsList(uint256 _gachaId)
        external
        view
        returns (Items[] memory gachaItemsList)
    {
        Gacha storage gacha = gachaByGachaId[_gachaId];
        Items[] memory blank;
        if (gacha.gachaIdxCurrentSize == 0) {
            gachaItemsList = blank;
        } else {
     ...
     */
    function getGachaponItemsListFromRequestIdList(
        uint256 _gachaId,
        uint256[] memory _requestIdList
    ) external view returns (Items[] memory gachaItemsList) {
        Gacha storage gacha = gachaByGachaId[_gachaId];
     ...
     */
    function getGachaponItemsList(
        uint256 _gachaId,
        uint256[] memory _itemsIdList
    ) public view returns (Items[] memory gachaItemsList) {
        Gacha storage gacha = gachaByGachaId[_gachaId];
     ...
     */
    function getGachaponIndexes(
        uint256 _gachaId,
        uint256 _start,
        uint256 _end
    ) public view returns (uint256[] memory gachaIdx) {
        Gacha storage gacha = gachaByGachaId[_gachaId];
     ...
     * @param _itemsList - array of set of gachapon item.
     */
    function _verifyAndAddItems(Gacha storage _gacha, Item[][] memory _itemsList) internal {
        uint256 itemCount;
        // Loop create items index
        for (uint256 i = 0; i < _itemsList.length; i++) {
            itemCount += _itemsList[i].length;
            ...
            for (uint256 j = 0; j < _itemsList[i].length; j++) {
                ...
                if (item.nftType == NftType.UNKNOWN) {
                    ...
                    IERC721Upgradeable erc721 = IERC721Upgradeable(item.itemAddress);
                    ...
                    if (erc721.getApproved(item.itemTokenId) != address(this))
                        revert InvalidItem(i, j, "Invalid item approval");
        ...
        require(
            itemCount <= itemCapPerTx,
            "[GachaponService._verifyAndAddItems] Item need to be less than or equal to item cap per transaction"
        );
    ...
    /**
     * @dev Transfer specify items id to receiver.
     * @param _gachaId - existing gachapon id.
     * @param _receiver - items receiver.
     * @param _itemsId - gachapon items id.
     */
    function _transferItems(
        ...
        require(
            items.receiver == address(0),
            "[GachaponService.transferItems] Items already transferred"
        );
        require(
            _receiver != address(0),
            "[GachaponService.transferItems] Items receiver invalid"
        );
        items.receiver = _receiver;
        for (uint256 i = 0; i < items.itemList.length; i++) {
            Item memory item = items.itemList[i];

            if (item.nftType == NftType.NFT721) {
                IERC721Upgradeable erc721 = IERC721Upgradeable(item.itemAddress);
                erc721.transferFrom(address(this), _receiver, item.itemTokenId);
            } else {
                IERC1155Upgradeable erc1155 = IERC1155Upgradeable(item.itemAddress);
                erc1155.safeTransferFrom(
                    address(this),
            ...
            emit GachaponItemTransfered(_gachaId, items.receiver, item);
        }
    }
     ...
     */
    function _removeGachaponIndex(Gacha storage _gacha, uint256 _gachaIdx) internal {
    ...
    /**
     * @dev UUPSUpgradeable _authorizeUpgrade implementation.
     */
    ...
    function _fulfillRandomWords(
        ...
        _removeGachaponIndex(gacha, gachaIdx);
        // Update items
        gacha.itemsById[itemsId].isFulfilled = true;
