...
contract LimitOrderMTC is EIP712("forbitspace Limit Order Protocol", "1.0.0") {
        ...
        returns (bool result, string memory message)
    {
        ...
        if (execute(ord)) {
            ...
            emit FailedExecOrder(ord);
            return (result = false, message = "F");
        }
    }

    function ExecuteSignedOrders(SignedOrder[] memory signOrd) external
        {
        require(signOrd.length > 0, "NON-ORDER");
        // require(isNotCancelledOrder(ord) == true, "ICO");
        
        for (uint i = 0; i < signOrd.length; i++) {
            if(shouldFulfilOrder(signOrd[i].ord, signOrd[i].sign) && execute(signOrd[i].ord)){
                // this.ExecuteCustomOrder(signOrd[i].ord, signOrd[i].sign);
                bytes32 _msgHash = getMessageHash(signOrd[i].ord);
                msgHashOrderCancel[signOrd[i].ord.owner].push(_msgHash);
                emit ExecuteOrder(signOrd[i].ord);
            } else{
                if(isNotCancelledOrder(signOrd[i].ord) == false){
                    emit FailedExecOrder(signOrd[i].ord);
                }
            }
        }
    }
    ...
    function execute(Order memory ord) internal returns (bool) {
        ...
        if(spaceRouter.makeTokenTokenSwap(
                ord.owner,
                ord.tokenIn,
                ord.tokenOut,
                pair,
...
contract SpaceOrderRouter {
    ...
    function makeTokenTokenSwap(address _owner, address tokenIn, address tokenOut, address pair, uint256 amountIn, uint256 minAmountOut, uint256 deadline) external onlyAuthorized returns (bool) {
        ...
        uint amountOutwFee = amountOut - amountOut.div(1000);
        
        IERC20(tokenOut).transfer(_owner, amountOutwFee);
        IERC20(tokenOut).transfer(address(feeAddress), amountOut.div(1000));
        
        // if(IERC20(tokenOut).balanceOf(_owner).sub(balanceOwnerBefore) >= minAmountOut && deadline >= block.timestamp){
        //     return true;
        // } else {
        //     return false;
        // }
