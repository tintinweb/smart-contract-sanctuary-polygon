...
import "../FixedPriceExtraWhitelist.sol";
import "../util/OwnableUpgradeable.sol";
import "../interfaces/IFixedPriceExtraWhitelistDeployer.sol";

contract FixedPriceExtraWhitelistDeployer is IFixedPriceExtraWhitelistDeployer, OwnableUpgradeable {
  address public immutable FIXEDPRICEEXTRAWHITELIST_IMPL;

  constructor() {
    __Ownable_init(msg.sender);
    FIXEDPRICEEXTRAWHITELIST_IMPL = address(new FixedPriceExtraWhitelist());
  }

  function deployFixedPriceExtraWhitelist(
    string memory _name,
    ...
    address clone = Clones.clone(FIXEDPRICEEXTRAWHITELIST_IMPL);

    FixedPriceExtraWhitelist(clone).initialize(
      _name,
      _symbol,
      _bURI,
      _timelock,
...
contract FixedPriceExtraWhitelist is
  Initializable,
  ContextUpgradeable,
  OwnableUpgradeable,
  ERC721Upgradeable,
  ReentrancyGuardUpgradeable
{
  ...
  event BaseURIFrozen();
  event SetWhitelistAddress(address newWhitelistAddress);

  address public TIMELOCK;

  bool public baseURIFrozen;
  uint256 public isRepeat;  //Number of single repeats
  ...
  address public whitelistAddress; // add whitelist address
  ...
  // only contract owner can set add whitelist address
  function setWhitelistAddress(address _whiteAddress) public onlyOwner{
    require(_whiteAddress != address(0), "It cannot be a zero address");
    whitelistAddress = _whiteAddress;

    emit SetWhitelistAddress(_whiteAddress);
  }
  ...
  function mint() public nonReentrant returns (uint256 tokenId) {
    ...
    require(isWhite(_msgSender()), "Not white user");
    require(mintNumber[_msgSender()] <= isRepeat, "Exceeding the limit of mint number");
  ...
  function addWhite(address[] memory _addWhite) public onlyWhitelistAddress returns (bool) {
  ...
  function delWhite(address[] memory _delWhites) public onlyWhitelistAddress returns (bool) {
  ...
  modifier onlyWhitelistAddress() {
    require(whitelistAddress == msg.sender, "WhitelistAddress: caller is not the whitelist address");
    _;
  }
...
interface IFixedPriceExtraWhitelistDeployer {
  function deployFixedPriceExtraWhitelist(
    string memory _name,
    string memory _symbol,
    string memory _bURI,
