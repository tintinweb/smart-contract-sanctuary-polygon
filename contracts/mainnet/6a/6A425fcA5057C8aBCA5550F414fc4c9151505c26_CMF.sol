    ...
    function _upgrade(address owner,uint256 price) private{
        ...
        while(true){
            for(uint256 i=0;i<teamUpNum.length;i++){
                ...
                if(userLevel[curAddress]<level && userRechargePrice[curAddress]>=userUpPrice[i] && teamRechargePrice[curAddress]>=teamUpPrice[i]){
                    if(level>1){
                        getUserTeamLevelNum(curAddress,i);
                    }
    ...
    function getUserTeamLevelNum(address owner,uint256 level) public{
        ...
        for(uint256 i=0;i<userTeams[owner].length;i++){
            ...
            if(userLevel[userAddr] >= level){
                ...
                for(uint256 j=0;j<userInfoTeams[userAddr].length;j++){
                    uint256 levelUser = userLevel[userInfoTeams[userAddr][j]];
                    if(levelUser>=level){
                        count++;
                        break;
                    }
...
abstract contract ICMF is CMFDATA {
    ...
    uint256[6] public userUpPrice = [1000000000,2000000000,3000000000,4000000000,5000000000,6000000000];
    
    uint256[6] public teamUpPrice = [10000000000,50000000000,200000000000,1000000000000,3000000000000,6000000000000];
