...
contract ScaPass is ERC1155, Ownable {
    ...
    uint256[3] public maxSupply = [2000,2000,2000];
    uint256[3] public maxMintPerWallet = [1,1,1];
    uint256[3] public totalMinted = [0,0,0];
    uint256[3] public mintPrice = [0,0,100 ether];
    ...
    function setMerkleRoot(bytes32 _merkleRoot1, bytes32 _merkleRoot2, bytes32 _merkleRoot3) external onlyOwner {
        merkleRoot[0] = _merkleRoot1;
        merkleRoot[1] = _merkleRoot2;
        merkleRoot[2] = _merkleRoot3;
    }
    ...
    function mint(uint256 _amount, uint256 _tokenId, bytes32[] calldata _proof) public onlyNotPaused {
        ...
        if(!isPublisSale)
            require(MerkleProof.verify(_proof, merkleRoot[_tokenIndex], keccak256(abi.encodePacked(msg.sender))), "Address does not exist in whitelist.");

        if(_tokenId == 3) {
            require(IERC20(testTokenAddress).balanceOf(msg.sender) >= mintPrice[2] * _amount, 'Ether value is too low');
