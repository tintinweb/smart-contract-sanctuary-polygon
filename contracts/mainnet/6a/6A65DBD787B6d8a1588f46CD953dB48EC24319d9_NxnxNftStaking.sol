...
// File: contracts\NxnxNftStaking.sol
...
contract NxnxNftStaking is Ownable, IERC721Receiver, IERC1155Receiver {
    ...
    uint16 public withdrawFee = 500; // 100x multied, 5% default
    uint16 public constant MAX_WITHDRAW_FEE = 3000; // max 30%
    ...
    // fixed apr value, multiplied by 100. Default value 100%
    uint256 public mainApr = 10000;
    // Additional apr with staking NFT
    // mapping(contract_address => mapping (token_id => apr))
    mapping(address => mapping(uint256 => uint256)) private additionalAprs;
    uint256 public defaultAdditionalApr = 1000; // Default additional apr 10%

    uint256 public constant MIN_APR_VALUE = 100; // Min apr value is 1%
    uint256 public constant MAX_APR_VALUE = 10**8; // Max apr value is 1000000%
     ...
     * @param _mainApr: apr value
     * @param _rewardStartTime: start block time
     * @param _rewardEndTime: end block time
     * @param _maxStakingPerUser: pool limit per user in stakedToken (if any, else 0)
    ...
    function initialize(
        ...
        require(
            _mainApr >= MIN_APR_VALUE && _mainApr <= MAX_APR_VALUE,
            "Out of apr range"
        );
        require(
    ...
    function payOrLockupPendingReward() internal {
        ...
        user.lastCalculatedAt = block.timestamp;
    }

    function updateIsFarming(bool _is_farming) external onlyOwner {
        ...
        returns (uint256 rewardAmount)
    {
        ...
        if (
            ...
            user.lastCalculatedAt < rewardEndTime
        ) {
            uint256 calcTo = block.timestamp > rewardEndTime
                ? rewardEndTime
                : block.timestamp;
            uint256 calcFrom = user.lastCalculatedAt > rewardStartTime
                ? user.lastCalculatedAt
                : rewardStartTime;
            if (calcTo < calcFrom) {
                calcFrom = calcTo;
            }
            uint256 multiplier = calcTo.sub(calcFrom);

            uint256 stakedAmount = user.amount;
            if (address(stakedToken) != address(rewardToken)) {
                if (is_farming) {
                    address token0 = IPancakePair(address(stakedToken)).token0();
                    (uint112 r0, uint112 r1, uint32 t) = IPancakePair(address(stakedToken)).getReserves();
                    uint total = IPancakePair(address(stakedToken)).totalSupply();
                    if (token0 == address(rewardToken)) {
                        stakedAmount = stakedAmount.mul(r0).mul(2).div(total);
                    } else {
                        stakedAmount = stakedAmount.mul(r1).mul(2).div(total);
                    }
                    ...
                    try swapRouter.getAmountsOut(stakedAmount, path) returns (
                        uint256[] memory amounts
                    ) {
                        stakedAmount = amounts[1];
                    } catch (
            ...
            if (user.nftStakeStatus != NftStakeProperty.NO_STAKED) {
                if (additionalAprs[user.nftAddress][user.nftTokenId] > 0) {
                    apr = apr.add(
                        additionalAprs[user.nftAddress][user.nftTokenId]
                    );
                } else {
                    apr = apr.add(defaultAdditionalApr);
                }
            }
    ...
    /**
     * @dev Update the apr value
     * Can only be called by the owner.
     */
    function updateMainApr(uint256 _mainApr) external onlyOwner {
        require(
            _mainApr <= MAX_APR_VALUE && _mainApr >= MIN_APR_VALUE,
            "Invalid apr value range"
        );
        mainApr = _mainApr;
    }

    /**
     * @notice Update default apr value for the case when nft is staked
     * @dev Can only be called by the owner
     */
    function updateDefaultAdditionalApr(uint256 _defaultAdditionalApr)
        external
        onlyOwner
    {
        require(
            _defaultAdditionalApr <= MAX_APR_VALUE &&
                _defaultAdditionalApr >= MIN_APR_VALUE,
            "Invalid apr value range"
        );
        defaultAdditionalApr = _defaultAdditionalApr;
    }

    /**
     * @notice Update additional apr value for the specific nft and id
     * @dev Can only be called by the owner
     */
    function updateAdditionalApr(
        address _nftAddress,
        uint256 _nftTokenId,
        uint256 _additionalApr
    ) external onlyOwner {
        require(
            _additionalApr <= MAX_APR_VALUE && _additionalApr >= MIN_APR_VALUE,
            "Invalid apr value range"
        );
        additionalAprs[_nftAddress][_nftTokenId] = _additionalApr;
    }
    ...
    function isFrozen() public view returns (bool) {
        return
            block.timestamp >= freezeStartTime &&
            block.timestamp <= freezeEndTime;
    }
     ...
     * @param _rewardStartTime: the new start block
     * @param _rewardEndTime: the new end block
     */
    function updateRewardTimes(uint256 _rewardStartTime, uint256 _rewardEndTime)
        external
        onlyOwner
    {
        require(block.timestamp < rewardStartTime, "Pool started already");
        require(
            _rewardStartTime < _rewardEndTime,
     ...
     * @dev This function is only callable by owner.
     * @param _freezeStartTime: the new freeze start block
     * @param _freezeEndTime: the new freeze end block
     */
    function updateFreezeTimes(uint256 _freezeStartTime, uint256 _freezeEndTime)
        external
        onlyOwner
    {
        require(
            _freezeStartTime < _freezeEndTime,
            "Freeze start time must be before end time"
        );
        require(
            block.timestamp < _freezeStartTime,
            "Free start time must be after now"
        );
