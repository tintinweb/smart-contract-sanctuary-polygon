...
// File: contracts\interfaces\external\IStakingRewards.sol
...
// File: contracts\interfaces\external\IUniswapV2Router01.sol
...
// File: contracts\interfaces\external\IdQuick.sol
...
// File: contracts\yield\impulse-multi\ImpulseQuickSwap3pool.sol
...
contract ImpulseQuickSwap3pool is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event DepositedTokens(uint256 amount0, uint256 amount1, uint256 amount2, uint256 amount3);
    ...
    function depositInUnderlying(uint256[] calldata _amounts) external nonReentrant onlyRole(STRATEGIST_ROLE) returns (uint256) {
        ...
        emit DepositedTokens(_amounts[0], _amounts[1], _amounts[2], _amounts[3]);
    ...
    function wantPriceInUsd(uint256[] memory _wantAmounts) external view returns (uint256) {
        ...
        //USDT-MAI price
        wantPrices[0] =
            ((underlyings[1].balanceOf(address(wantTokens[0]))) * 10**12 + underlyings[3].balanceOf(address(wantTokens[0]))) /
            wantTokens[0].totalSupply();

        //USDC-DAI price
        wantPrices[1] =
            ((underlyings[0].balanceOf(address(wantTokens[1]))) * 10**12 + underlyings[2].balanceOf(address(wantTokens[1]))) /
            wantTokens[1].totalSupply();

        //USDC-USDT price
        wantPrices[2] =
            ((underlyings[0].balanceOf(address(wantTokens[2]))) * 10**12 + underlyings[1].balanceOf(address(wantTokens[2])) * 10**12) /
    ...
    function _swapAllWantToOneUnderlying(address _underlying, address _receiver) internal {
        ...
        for (uint256 i = 0; i < underlyings.length; i++) {
            ...
            if ((i < 2) && (_underlying == address(underlyings[2]) || _underlying == address(underlyings[3])))
                // Add 12 decimals if source has 6 and destination has 18
                slippageTolerance *= 10**12;
            else if ((i >= 2) && (_underlying == address(underlyings[0]) || _underlying == address(underlyings[1])))
                // Sub 12 decimals if source has 18 and destination has 6
                slippageTolerance /= 10**12;
    ...
    function _swapUnderlyingsToUSDToken(uint256[] memory _excessAmounts, address usdToken) internal {
        ...
        for (uint256 u = 0; u < underlyings.length; u++) {
            ...
            if ((u < 2) && (usdToken == address(underlyings[2]) || usdToken == address(underlyings[3])))
                // Add 12 decimals if source has 6 and destination has 18
                slippageTolerance *= 10**12;
            else if ((u >= 2) && (usdToken == address(underlyings[0]) || usdToken == address(underlyings[1])))
                // Sub 12 decimals if source has 18 and destination has 6
                slippageTolerance /= 10**12;
    ...
    function _swapUSDTokenToUnderlyings(uint256[] memory _missingAmounts, address usdToken) internal {
        ...
        for (uint256 u = 0; u < underlyings.length; u++) {
            ...
            if ((usdToken == address(underlyings[0]) || usdToken == address(underlyings[1])) && (u >= 2))
                // Sub 12 decimals if source has 6 and destination has 18
                slippageTolerance /= 10**12;
            else if ((usdToken == address(underlyings[2]) || usdToken == address(underlyings[3])) && (u < 2))
                // Add 12 decimals if source has 18 and destination has 6
                slippageTolerance *= 10**12;
