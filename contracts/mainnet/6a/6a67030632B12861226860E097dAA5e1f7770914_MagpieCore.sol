    ...
    IMagpieCore
{
    ...
    event DepositReceived(uint8 networkId, uint64 coreSequence, uint256 amount);

    mapping(address => uint256) public gasFeeAccumulatedByToken;
    mapping(address => mapping(address => uint256)) public gasFeeAccumulated;
    mapping(uint8 => mapping(uint64 => bool)) public sequences;
    mapping(uint8 => mapping(uint64 => uint256)) public deposits;
        ...
        returns (uint256[] memory amountOuts)
    {
        ...
        uint256 amountIn = deposits[payload.senderNetworkId][coreSequence] > 0
            ? deposits[payload.senderNetworkId][coreSequence]
    ...
    function sgReceive(
        ...
        (uint8 networkId, uint64 coreSequence) = payload.parseSgPayload();
        deposits[networkId][coreSequence] = amount;
        emit DepositReceived(networkId, coreSequence, amount);
    }
}
