...
abstract contract LibertiAggregationRouterV4 {
    ...
    function swap(
        ...
        if (address(desc.dstToken) != vault.asset() && address(desc.dstToken) != vault.other()) {
            revert TokenError();
        }
    ...
    OwnableUpgradeable
{
    ...
    error MaxFeeError();
    error SanctionedError(address sanctionedAddr);
    ...
    IWeth9 internal immutable weth; // the wrapped token of native currency
    ILibertiPriceFeed public immutable priceFeed; //feedDirectory -- directory
    ISanctionsList internal constant SANCTIONS_LIST =
        ISanctionsList(0x40C57923924B5c5c5455c48D93317139ADDaC8fb);

    IERC20Upgradeable public asset;
    IERC20Upgradeable public other;

    uint256 public invariant; // Vault target allocation coefficient, in basis point
    ...
    function rebalance(
        ...
        if (10_000 < newInvariant) {
            revert InvariantError(); //FIXME: Replace with off-chain check (oz defender)
        }
        invariant = newInvariant; // For subsequent deposits
        (, SwapDescription memory desc, ) = abi.decode(data[4:], (address, SwapDescription, bytes));
        returnAmount = swap(data, address(this), desc.amount);
    ...
    function _deposit(
        ...
        if (SANCTIONS_LIST.isSanctioned(_msgSender())) {
            revert SanctionedError(_msgSender());
        }
        ...
        uint256 feeAmount = shares.mulDiv(entryFee, 10_000, MathUpgradeable.Rounding.Down);
    ...
    function sharesToToken1(uint256 shares) external view returns (uint256) {
        ...
        if (0 < totalToken1) {
            shares -= shares.mulDiv(exitFee, 10_000, MathUpgradeable.Rounding.Down);
    ...
    function redeem(
        ...
        if (SANCTIONS_LIST.isSanctioned(receiver)) {
            revert SanctionedError(receiver);
        }
    ...
    function _redeem(
        ...
        uint256 feeAmount = shares.mulDiv(exitFee, 10_000, MathUpgradeable.Rounding.Down);
        shares -= feeAmount;
    ...
    function convertToAssets(uint256 shares) public view returns (uint256 assets) {
        ...
        return
            (supply == 0)
                ? shares
                : shares.mulDiv(totalAssets(), supply, MathUpgradeable.Rounding.Down);
    }
