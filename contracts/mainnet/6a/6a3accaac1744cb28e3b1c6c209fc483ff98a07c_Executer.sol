...
contract Executer {
    ...
    function changeExecuter(address _executor) external onlyOwner{
        executor = _executor;
    }
    ...
    function uniswapWeth(uint256 _wethAmountToFirstMarket, address[] memory _targets, bytes[] memory _payloads) external onlyExecutor payable {
        require (_targets.length == _payloads.length, "Targets' lenght does not match with payloads' lenght!");
        ...
        require(WETH.balanceOf(address(this)) > _wethBalanceBefore, "Loss trade");
    }

    function sendWeth(address payable _to, uint256 _amount) external onlyOwner{
