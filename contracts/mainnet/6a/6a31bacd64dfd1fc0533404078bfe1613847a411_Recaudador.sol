    ...
    UUPSUpgradeable
{
    ...
    UniswapV2Router02 private router; //variable a la cual se le envian transacciones del router
    IRoles private roles; //variable a la cual se le envian transacciones de roles
    ...
    function buy() external payable nonReentrant whenNotPaused {
        ...
        nftSold++;
        uint256 amountUsdcTransfered = _swapTokens(nftPrice); //se transfieren usdc a la wallet recaudadora
    ...
    function setWalletRecadadora(address _newWalletRecaudadora) external onlyOwner {
    ...
    function canRedeem(address beneficiary)
        external
        nonReentrant
        whenNotPaused
        returns (bool)
    {
        ...
        if (
            clientInfo[beneficiary].nftsPresalePurchased ==
    ...
    function getClientInfo(address beneficiary)
        external
        view
