...
contract BullManager {
    ...
    IERC20 private USDC = IERC20(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174); // rinkeby
    ...
    // CHANGE THIS 
    address private TREASURY = 0x87a68551721b9Bf6045Fa6A57b730E574A1051a4;
    address private REWARDS = 0xBa9003f5567F6566624893EA6c3bfeFD605EfAE8; 
    address private LIQUIDITY = 0x637AdE8fFdD65D7D82F98D1cC253c067D9CD40Ee;
    address private OPERATING_COST = 0xE9EEeBf3227ab436956D41653FBc6443A51D8447;
    // address private constant UNISWAP_V2_ROUTER = 0x8954AfA98594b838bda56FE4C12a09D7739D179b; // Mumbai
    address private constant UNISWAP_V2_ROUTER = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // Rinkeby
    address owner;

    // PER DAY 
    uint rewardPerTree = 0.2256*10**18; 

    // Cost to buy a tree 
    uint costPerTree = 10*10**18;
    // Cost to refresh
    uint refreshCost = 30*10**6;
    uint claimFee = 5*10**6;
    ...
    function createTreeWithTokens(string memory _treeName) external NotBlacklist checkNameConflict(msg.sender, _treeName) {
        ...
        splitToken();
        Tree memory newTree = Tree(_treeName, block.timestamp, block.timestamp + 90 days, block.timestamp, rewardPerTree);
    ...
    function splitToken() internal {
        ...
        uint _reward = costPerTree - _treasury - _liquidity;
    ...
    function calculateRewards(address _user) public view returns(uint) {
        ...
        for (uint i=0; i<treesOwned[_user].length; i++) {
            if (treesOwned[_user][i].expiration >= block.timestamp) {
                ...
                _totalRewards += epochsPassed * treesOwned[_user][i].rewardRate;
            }
        }
    ...
    function calculateRewardsWithTreeIndex(address _user, uint _treeIndex) public view returns (uint) {
        ...
        if (treesOwned[_user][_treeIndex].expiration >= block.timestamp) {
            uint epochsPassed = (block.timestamp - treesOwned[_user][_treeIndex].lastClaimTime) / 1 days;
            _treeRewards += epochsPassed * treesOwned[_user][_treeIndex].rewardRate;
        }
        else {
            uint epochsPassed = (treesOwned[_user][_treeIndex].expiration - treesOwned[_user][_treeIndex].lastClaimTime) / 1 days;
            _treeRewards += epochsPassed * treesOwned[_user][_treeIndex].rewardRate;
        }
        return _treeRewards;
    }
    function calculateRewardsForEachTree(address _user) external view returns(uint[] memory) {
        uint[] memory rewardsArray;
        rewardsArray = new uint[](treesOwned[_user].length);
        for( uint i = 0; i < treesOwned[_user].length; i ++) {
            rewardsArray[i] = calculateRewardsWithTreeIndex(_user, i);
        }
        return rewardsArray;
    }
    ...
    function refreshTrees() external NotBlacklist {
        ...
        for (uint i=0; i<treesOwned[msg.sender].length; i++) {
            if (treesOwned[msg.sender][i].expiration < block.timestamp) {
                ...
                treesOwned[msg.sender][i].expiration = block.timestamp + 90 days;
    ...
    function refreshTreeWithIndex(uint _treeIndex) external NotBlacklist {
        ...
        USDC.transferFrom(msg.sender, OPERATING_COST, _cost);
        treesOwned[msg.sender][_treeIndex].expiration = block.timestamp + 90 days;
    ...
    function setRewardsForOneTree(address _to, uint _index, uint _reward) external onlyAdmin {
        treesOwned[_to][_index].rewardRate = _reward;
    }
