...
contract PIXAssetStakeableAdapter is IMissionControlStakeable, OwnableUpgradeable, ERC1155HolderUpgradeable {
    ...
    function setTokenInfo(
        ...
        tokenInfo[_tokenId].isStakeable = _isStakeable;
        tokenInfo[_tokenId].isBase = _isBase;
        tokenInfo[_tokenId].isRaidable = _isRaidable;
        tokenInfo[_tokenId].class = _class;
        tokenInfo[_tokenId].resourceEV = _resourceEv;
        tokenInfo[_tokenId].resourceId = _resourceId;
        tokenInfo[_tokenId].resourceCap = _resourceCap;
    ...
    /// @inheritdoc IMissionControlStakeable
    function checkTile(address _userAddress, uint256 _nonce)
        external
        view
        override
        returns (uint256 _amount, uint256 _retTokenId)
    {
        return __check_tile(_userAddress, _nonce);
    }

    /// @inheritdoc IMissionControlStakeable
    function onCollect(address _userAddress, uint256 _nonce)
        external
        override
        onlyMissionControl
        returns (uint256, uint256)
    {
        (uint256 available, uint256 id) = __check_tile(_userAddress, _nonce);
        seeds[_userAddress][_nonce] = block.timestamp;
        return (available, id);
    }

    /// @inheritdoc IMissionControlStakeable
    function isBase(uint256 _tokenId) external view override returns (bool _isBase) {
        _isBase = tokenInfo[_tokenId].isBase;
    }

    /// @inheritdoc IMissionControlStakeable
    function isRaidable(uint256 _tokenId) external view override returns (bool _isRaidable) {
        _isRaidable = tokenInfo[_tokenId].isRaidable;
    }
    ...
    function onStaked(
        ...
        require(tokenInfo[_tokenId].isStakeable, "MCS1155_ADAPTER: Token not stakeable");
        require(seeds[_currentOwner][_nonce] == 0, "MCS1155_ADAPTER: Nonce already used");
        if (tokenInfo[_tokenId].class == Distribution.Facility) {
            require(
    ...
    function timeLeft(
        ...
        _timeLeft = __timeLeft(_userAddress, _nonce);
    }

    function __timeLeft(address _userAddress, uint256 _nonce) private view returns (int256 _timeLeft) {
        uint256 tokenId = tokenIds[_userAddress][_nonce];
        Distribution class = tokenInfo[tokenId].class;
        if (class == Distribution.Poisson) {
            return -1;
        }
        if (class == Distribution.LinearDeterministic) {
            return __timeLeft_deterministic_linear(_userAddress, tokenId, _nonce);
        }
        if (class == Distribution.Facility) {
            return __timeLeft_facility(_userAddress, tokenId, _nonce);
        }
    ...
    function __timeLeft_deterministic_linear(
        ...
        uint256 completionTime = (DAY_IN_SEC / tokenInfo[tokenId].resourceEV) *
            tokenInfo[tokenId].resourceCap +
            localSeed;
        int256 timeLeft = int256(completionTime) - int256(block.timestamp);
        return timeLeft > 0 ? timeLeft : int256(0);
    }
    ...
    function __timeLeft_facility(
        ...
        uint256 localSeed = seeds[_userAddress][_nonce];
        require(localSeed != 0, "MCS1155_ADAPTER: No such token staked");
        uint256 completionTime = (tokenInfo[tokenId].resourceCap * facilityLeadTime[_userAddress][_nonce]) /
    ...
    function __check_tile(address _userAddress, uint256 _nonce) private view returns (uint256, uint256) {
        uint256 tokenId = tokenIds[_userAddress][_nonce];
        Distribution class = tokenInfo[tokenId].class;
        if (class == Distribution.Poisson) {
            return __checkTile_Poisson(_userAddress, tokenId, _nonce);
        }
        if (class == Distribution.LinearDeterministic) {
            return __checkTile_deterministic_linear(_userAddress, tokenId, _nonce);
        }
        if (class == Distribution.Facility) {
            return __checkTile_facility(_userAddress, tokenId, _nonce);
        }
        revert("Critical: Distribution class not supported, blame whoever implemented it");
    }
    ...
    function __checkTile_facility(
        ...
        uint256 ret = (facilityOutput[_userAddress][_nonce] *
            ((block.timestamp - localSeed) / facilityLeadTime[_userAddress][_nonce]));
        if (ret > tokenInfo[tokenId].resourceCap) {
            ret = tokenInfo[tokenId].resourceCap;
        }
        return (ret, tokenInfo[tokenId].resourceId);
    }
    ...
    function __checkTile_deterministic_linear(
        ...
        uint256 localSeed = seeds[_userAddress][_nonce];
        require(localSeed != 0, "MCS1155_ADAPTER: No such token staked");
        uint256 ret = (tokenInfo[tokenId].resourceEV * (block.timestamp - localSeed)) / DAY_IN_SEC;
        if (ret > tokenInfo[tokenId].resourceCap) {
            ret = tokenInfo[tokenId].resourceCap;
        }
        return (ret, tokenInfo[tokenId].resourceId);
    ...
    function __checkTile_Poisson(
        ...
        require(localSeed != 0, "MCS1155_ADAPTER: No such token staked");
        uint256 ret = 0;
        if (tokenInfo[tokenId].resourceEV == 0) {
            return (ret, tokenInfo[tokenId].resourceId);
        }
        int256 blockTime = int256(block.timestamp);
        if (uint256(blockTime - localSeed) > cutoffs[tokenId]) {
            return (tokenInfo[tokenId].resourceCap, tokenInfo[tokenId].resourceId);
        }
        uint256 localEV = tokenInfo[tokenId].resourceEV;
        uint256 localCap = tokenInfo[tokenId].resourceCap;
        int256 T = _getT(_player_address, tokenId, localSeed, localEV);
        while (localSeed + T < blockTime && ret < localCap) {
            ret++;
    ...
    */
    ...
    function _getT(
        ...
        int256 seed,
        uint256 ev
    ) internal view returns (int256 T) {
