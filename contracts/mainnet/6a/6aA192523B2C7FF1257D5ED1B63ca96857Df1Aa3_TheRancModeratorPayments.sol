...
contract TheRancModeratorPayments is 
    ...
    IERC20 public usdt;
    mapping(address => uint256) public allowedWithdrawalAmount;
    mapping(address => bool) public approvedModerators;
    mapping(address => uint256) public lastWithdrawalTimestamp;
    mapping(address => uint256) public weeklyPayAmount;
    ...
    modifier onlyApprovedModerator() {
        require(approvedModerators[msg.sender], "Not approved moderator");
        _;
    }

    function addModerator(address _moderatorAddress, uint256 _weeklyPayAmount) external onlyOwner {
        approvedModerators[_moderatorAddress] = true;
        weeklyPayAmount[_moderatorAddress] = _weeklyPayAmount;
    }

    function setWeeklyPayAmount(address _moderatorAddress, uint256 _amount) external onlyOwner {
        weeklyPayAmount[_moderatorAddress] = _amount;
    }

    function withdrawWeeklyAmount() external onlyApprovedModerator {
        require(block.timestamp >= lastWithdrawalTimestamp[msg.sender] + 6 days, "Can only withdraw once per week");
     
        uint256 withdrawalAmount = weeklyPayAmount[msg.sender];
    
        require(withdrawalAmount <= usdt.balanceOf(address(this)), "Not enough USDT available");

        lastWithdrawalTimestamp[msg.sender] = block.timestamp;

        usdt.safeTransfer(msg.sender, withdrawalAmount * 10 ** 6);
    }

    function deposit(uint256 _amountInDollars) external payable {
        usdt.safeTransferFrom(msg.sender, address(this), _amountInDollars * 10 ** 6);
    }
    ...
    function hoursUntilNextWithdrawal(address _moderator) external view returns (uint256) {
        uint256 currentTimestamp = block.timestamp;
        uint256 timeSinceLastWithdrawal = currentTimestamp - lastWithdrawalTimestamp[_moderator];
        uint256 timeRemaining = 0;
        if (timeSinceLastWithdrawal < 6 days) {
            timeRemaining = (6 days - timeSinceLastWithdrawal) / 3600;
        }
        return timeRemaining;
    }


    function setAddressForUSDT(address _address) external onlyOwner{
