...
contract YBNFT is ERC721, HedgepieAccessControlled {
    ...
    // tokenId => adapter address => CAdapterInfo
    mapping(uint256 => mapping(address => CAdapterInfo)) public cAdapterInfos;
    // tokenId => AdapterDate
    ...
    /**
     * @notice Update cross adapter information
     * @param _tokenId  tokenId of NFT
     * @param _adapterAddr  the address of adapter
     * @param _cAdapterInfo  the information of cross adapter
     */
    function updateCAdapterInfo(
        uint256 _tokenId,
        address _adapterAddr,
        CAdapterInfo memory _cAdapterInfo
    ) external onlyNftOwner(_tokenId) {
        require(isExist[_tokenId][_adapterAddr], "Adapter not added");

        cAdapterInfos[_tokenId][_adapterAddr] = _cAdapterInfo;
    }
