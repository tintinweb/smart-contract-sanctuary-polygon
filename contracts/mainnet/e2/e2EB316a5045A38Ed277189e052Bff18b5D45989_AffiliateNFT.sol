...
contract AffiliateNFT is ERC721,Ownable {
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory)
    {
        ...
        uint256 id = _tokenId + 1;
        return
            bytes(baseTokenURI).length != 0
                ? string(
                    abi.encodePacked(baseTokenURI, id.toString(), ".json")
                )
                : "";
    }
    ...
    function addInitialUser(string memory _name, address _user, string memory _userLink) public onlyOwner{
        require(isRegistered(_user) == false,"Already Signed Up");
        require(isUserNameRegistered(_name) == false,"Already name used");
        user memory newUser= user(_name, _user, _user, _userLink);
        users.push(newUser);
    }
    function signUp(string memory _name, string memory _affiliateLink, string memory _userLink) external{
        ...
        require(isUserNameRegistered(_name) == false,"Already name used");
        uint256 i;
        user memory newUser = user(_name, msg.sender, msg.sender, _userLink);
        for(i = 0; i < users.length; i++)
        {
    ...
    function mintNFT() public payable {
        ...
        require(isRegistered(msg.sender),"You have to registered first");
        require(totalSupply < MAX_SUPPLY, "You can't mint more than max mint limit");               
        PRICE = uint256(getTokenPrice());
        uint256 price = 0;
        
    
        address sponsor1 = getSponsor(msg.sender);
        address sponsor2 = getSponsor(sponsor1);
        if(sponsor1 != msg.sender)
        {
            price = price + PRICE / 10;
            if(sponsor2 != sponsor1)
                price = price + PRICE / 20;
        }           

        require(msg.value >= PRICE + price, "Not Enough Ether"); // check if buyer has enough eth.        
        
        if(sponsor1 != msg.sender)
        {
            bool sent = payable(sponsor1).send(PRICE / 10);
            require(sent, "Failed to send Ether");
            if(sponsor2 != sponsor1)
            {
                sent = payable(sponsor2).send(PRICE / 20);
                require(sent, "Failed to send Ether");
            }
    ...
    function getTokenPrice() public view returns(int256){
        FEED A = FEED(0xAB594600376Ec9fD91F8e885dADF0CE036862dE0);

        int256 price = 1* 10 **26 / A.latestAnswer();
