...
contract Marketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address payable _platformFeeRecipient, uint16 _platformFee)
    public
    initializer
    {
    ...
    function listItem(
        ...
        require(_nListQty(_nftAddress, _tokenId, msg.sender) == 0, "not listed item");
    ...
    nonReentrant
    {
        ...
        require(block.timestamp >= listedItem.startingTime && _nListQty(_nftAddress, _tokenId, _owner) > 0, "item not buyable");
        ...
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC2981)) {
            ...
            (
                erc2981Receiver,
                erc2981feeAmount
            ) = IERC2981(_nftAddress).royaltyInfo(_tokenId, price);

            if (erc2981feeAmount > 0) {
                feeAmount = feeAmount + erc2981feeAmount;
                if (feeAmount > price) {
                    feeAmount = price;
                }
     ...
     */
    function updatePlatformFee(uint16 _platformFee, address payable _platformFeeRecipient) external onlyOwner {
        platformFee = _platformFee;
        feeReceipient = _platformFeeRecipient;
