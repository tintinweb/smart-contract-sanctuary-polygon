...
contract MembershipPurchase is MjolnirRBAC {
    ...
    address public memDB = 0xF6D02fF3f6b35259ddd04647bA52EF91Aeab7BA5;
    address public stableCoin = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    ...
    function buyPro(uint256 timeframe) external {
        ...
        uint256 fPrice;
        if(timeframe == 1){fPrice = t2PriceMo;}
        if(timeframe == 6){fPrice = t2Price180;}
        if(timeframe == 12){fPrice = t2Price360;}
        require(
            sb.transferFrom(
                msg.sender,
                address(this),
    ...
    function buyPremium(uint256 timeframe) external {
        require(tx.origin == msg.sender);
        uint256 fPrice;
        if(timeframe == 1){fPrice = t3PriceMo;}
        if(timeframe == 6){fPrice = t3Price180;}
        if(timeframe == 12){fPrice = t3Price360;}
        require(
            sb.transferFrom(
                msg.sender,
                address(this),
    ...
    function buyEnterprise(uint256 timeframe) external {
        require(tx.origin == msg.sender);
        uint256 fPrice;
        if(timeframe == 1){fPrice = t4PriceMo;}
        if(timeframe == 6){fPrice = t4Price180;}
        if(timeframe == 12){fPrice = t4Price360;}
        require(
            sb.transferFrom(
                msg.sender,
                address(this),
