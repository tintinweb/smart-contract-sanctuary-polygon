...
/**
 * @title TokenVestingWithCliff
 * @dev A token holder contract that can release its token balance gradually like a
 * typical vesting scheme, with a cliff and vesting period. Optionally revocable by the
 * owner.
 */
contract TokenVestingWithCliff is Ownable {
   ...
   * beneficiary, monthly release until start + total months. By then all
  ...
  function release(address token) public {
    ...
    require(unreleased > 0,"TokenVesting: No releasable amount!");

    _released[token] = _released[token].add(unreleased);

    IERC20(token).safeTransfer(_beneficiary, unreleased);
