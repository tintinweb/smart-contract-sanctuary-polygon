...
import {FixedPriceListOrder, FixedPriceUnlistOrder} from "../../shared/libraries/LibOrders.sol";
import {
    AppStorage,
    Asset,
    Modifiers
...
contract FixedPriceListFacet is Modifiers {
    ...
    function fixedPriceListBundle(
        FixedPriceListOrder calldata order
    ) public isSender(order.owner) {

        LibFixedPrice.checkFixedPriceListOrder(order);

        LibFixedPrice.fixedPriceList(order);
    }

    function fixedPriceUnlistBundle(
        FixedPriceUnlistOrder calldata order
    ) public isSender(order.owner) {

        LibFixedPrice.checkFixedPriceUnlistOrder(order);

        LibFixedPrice.fixedPriceUnlist(order);
    }
}
