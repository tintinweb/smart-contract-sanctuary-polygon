    ...
    ReentrancyGuardUpgradeable
{
    ...
    function getPicksResultsByAddress(address addressToCheck)
        public
        view
        returns (WeeklyResult[] memory)
    {
        WeeklyResult[] memory gameOutcomes = new WeeklyResult[](
            contestantWinnersPicked[addressToCheck].length
        );
        for (uint8 i; i < contestantWinnersPicked[addressToCheck].length; i++) {
            NFLTeam teamPickedToWin = contestantWinnersPicked[addressToCheck][
                i
            ];
            gameOutcomes[i] = teamResultsByWeek[teamPickedToWin][i];
        }
        return gameOutcomes;
    }
        ...
        external
    {
        checkEligibility(msg.sender);
        require(
            !isEliminated(msg.sender),
            "Must not be eliminated to make a weekly pick. See if you are eligible to buy a challenge flag."
        );
    ...
    function advanceActiveWeekIndex() external onlyOwner {
        // when advancing the active week forward, the None pickers get an elimination strike.
        teamResultsByWeek[NFLTeam.NONE][activeWeekIndex] = WeeklyResult.LOSS;
        activeWeekIndex++;
    }
