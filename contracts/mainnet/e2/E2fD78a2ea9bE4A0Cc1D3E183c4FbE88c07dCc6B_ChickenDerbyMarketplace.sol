    ...
    OwnableUpgradeable
{
    ...
    EnumerableSet.UintSet private _openItems;
    ...
    function setFloorPrice(uint256 floorPrice_) public override onlyOwner {
        floorPrice = floorPrice_;
    }
    ...
    function createMarketItem(
        ...
        if (price_ < floorPrice) revert Errors.LowFloorPrice();
    ...
    function editMarketItem(
        ...
        if (item.itemId != itemId_) revert Errors.NotOnMarket();
        ...
        if (tradeType_ == TradeType.None && item.tradeType != tradeType_)
    ...
    function cancelMarketItem(uint256 itemId_) external override nonReentrant {
        MarketItem memory item = idToMarketItems[itemId_];
        // To check if the item is on market
        if (item.itemId != itemId_) revert Errors.NotOnMarket();
        // Confirm the market item is not sold
        ...
        idToMarketItems[itemId_] = item;
        if (_openItems.contains(itemId_)) _openItems.remove(itemId_);
        _cancelMultiOffersByItemId(itemId_);
    ...
    function makeOffer(
        ...
        for (uint256 i = 0; i < tokensAttached_.length; i++) {
            if (
                IERC721(nftContract_).ownerOf(tokensAttached_[i]) !=
                _msgSender()
            ) revert Errors.NotTokenOwner();
        }
    ...
    function cancelOrDeclineOffer(uint256 offerId_)
        external
        override
        nonReentrant
    {
        TradeOffer memory offer = idToOffers[offerId_];
        if (offer.offerId != offerId_) revert Errors.NotExistOffer();
        ...
        if (_openOffers.contains(offerId_)) _openOffers.remove(offerId_);
        if (_msgSender() == offer.offerMaker) {
            offer.canceledAt = block.timestamp;
            emit TradeOfferCanceled(offerId_);
        } else if (_msgSender() == item.seller) {
            offer.declinedAt = block.timestamp;
            emit TradeOfferDeclined(offerId_);
        }
        idToOffers[offerId_] = offer;
    }
