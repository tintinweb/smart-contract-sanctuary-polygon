...
contract NFT1155 is ERC1155Upgradeable, AccessControlUpgradeable {
    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR");
    bytes32 public constant MERKLE_OPERATOR_ROLE = keccak256("MERKLE_OPERATOR");
    ...
    function __NFT1155_init_unchained(string memory _name, string memory _symbol)
        public
        initializer
    {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(OPERATOR_ROLE, msg.sender);
        _setupRole(MERKLE_OPERATOR_ROLE, msg.sender);
    ...
    function GrantMerkleOperator(address account) external onlyAdmin {
        grantRole(MERKLE_OPERATOR_ROLE, account);
    }

    function RevokeMerkleOperator(address account) external onlyAdmin {
        revokeRole(MERKLE_OPERATOR_ROLE, account);
    }
    ...
    modifier onlyMerkleOperator() {
        require(
            hasRole(MERKLE_OPERATOR_ROLE, msg.sender),
            "not merkle operator"
        );
        _;
    }

    function add(uint256 ids) external onlyOperator {
