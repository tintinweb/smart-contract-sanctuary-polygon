...
abstract contract DEXBase is ACLBase {
    ...
    function setSwapInToken(bytes32 _role, address _token, bool _tokenStatus) external onlySafe {
        // sell
        if (_tokenStatus) {
    ...
    function setSwapOutToken(bytes32 _role, address _token, bool _tokenStatus) external onlySafe {
        // buy
        if (_tokenStatus) {
    ...
    function slippageCheck(address token0, address token1, uint256 amountIn, uint256 amountOut) internal view {
        ...
        require(
            valueOutput >= (valueInput * (SLIPPAGE_BASE - role_maxSlippagePercent[_checkedRole])) / SLIPPAGE_BASE,
...
abstract contract ACLBase is OwnableUpgradeable, UUPSUpgradeable {
    ...
    // Initializer
    function initialize(address _safeAddress, address _safeModule) public initializer {
    ...
    function check(bytes32 _role, uint256 _value, bytes calldata data) external virtual onlyModule returns (bool) {
        _checkedRole = _role;
