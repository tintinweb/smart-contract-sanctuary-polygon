...
contract MobileSuit is ERC721Enumerable, Ownable {
	...
	mapping (uint256 => bool) internal _whitelistRAT;

  
   MobileSuit internal RAT = MobileSuit(0x39239b6040207A4D3d657aD00554669A992cA451);  
    ...
    constructor(string memory baseURI) ERC721("Mobile Suit", "MbS")  {
        setBaseURI(baseURI);
    }
    ...
    function withdrawAll() public payable onlyOwner {
     ...
     function RATview(address _owner_) external view  returns (uint256[] memory) {
         return RAT.tokensOfOwner(_owner_); 
    }
    
    function CompletedMintForRAT(uint256 _mespid) public view virtual returns (bool) {
         return _whitelistRAT[_mespid];
    }
   
    function TokenForMintAvailable(address _holder) public view virtual returns (uint256) {
        uint128  tokenAvailable = 0;
        uint256[] memory tokensId = new uint256[](RAT.balanceOf(_holder));
        for(uint i = 0; i < RAT.balanceOf(_holder); i++){
            tokensId[i] = RAT.tokenOfOwnerByIndex(_holder, i);
            
            if (_whitelistRAT[tokensId[i]])
              {} else {
                   tokenAvailable++;
              }
        ...
        function mintForRAT(uint _count) public {
        require(msg.sender == _owner || saleEnable, "Sale not enable");
        require(totalSupply() + MAX_RESERV +_count <= MAX_TOKEN, "Exceeds limit");
        ...
        uint256[] memory tokensId = new uint256[](RAT.balanceOf(msg.sender));
        for(uint i = 0; i < RAT.balanceOf(msg.sender); i++){
            tokensId[i] = RAT.tokenOfOwnerByIndex(msg.sender, i);
