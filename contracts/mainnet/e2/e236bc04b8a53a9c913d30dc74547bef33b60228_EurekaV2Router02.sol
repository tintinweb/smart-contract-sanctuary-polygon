...
library EurekaV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'95554683b1cc54830ccc58bfb3d224dbb3a0173457201118c4ca537173a47ddb' // init code hash
...
// File: contracts/uniswapv2/IMaker.sol
...
contract EurekaV2Router02 is IEurekaV2Router02 {
        ...
        returns (uint[] memory amounts)
    {
        amounts = EurekaV2Library.getAmountsOut(factory, amountIn, path);
         if(!IEurekaV2Factory(factory).pause()){
            uint256 fee = isWhitelist(path[0]);
            uint256 len  = amounts.length.sub(1);
            amounts[len] = amounts[len].sub(amounts[len].mul(fee).div(IEurekaV2Factory(factory).PERCENT100()));
        }
        return amounts;
        ...
        returns (uint[] memory amounts)
    {
        ...
        if(!IEurekaV2Factory(factory).pause()){
            uint256 fee = isWhitelist(path[0]);
            amounts[0] = amounts[0].add(amounts[0].mul(fee).div(IEurekaV2Factory(factory).PERCENT100()));
        }
        return amounts;
    }

    function isWhitelist(address _token)public view returns(uint256){
        uint256 fee;
            if(IEurekaV2Factory(factory).whiteList(_token)){
                fee = (IEurekaV2Factory(factory).SwaptotalFee())+(IEurekaV2Factory(factory).tfee());
            }else{
                fee = (IEurekaV2Factory(factory).SwaptotalFee());
            }
        return fee;
    }
   ...
   function takeAddLiquidityFee(address _lp,address _token0, address _token1, uint256 _amount0, uint256 _amount1, bool isEth) internal returns(uint256, uint256){
        ...
        IMaker(maker).takeLiquidityFee(_lp, _token0, _token1);
    ...
    function takeSwapFee(address lp, address token, uint256 amount, bool isEth) internal returns(uint256){
        ...
        if(IEurekaV2Factory(factory).whiteList(token)){
            uint tax = amount.mul(IEurekaV2Factory(factory).tfee()).div(IEurekaV2Factory(factory).PERCENT100()+(IEurekaV2Factory(factory).tfee()));
            if(!isEth){
               TransferHelper.safeTransferFrom(token, msg.sender, IEurekaV2Factory(factory).feeReceiver(), tax);
            }else{
              TransferHelper.safeTransfer(token, IEurekaV2Factory(factory).feeReceiver(), tax);
            }
        ...
        IMaker(maker).swapFeeConvert(lp, token, swapTotalFee);
