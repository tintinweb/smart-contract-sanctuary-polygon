    ...
    */
        ...
        returns(uint256 _destAmount)
    {
        ...
        if (ETHER_ERC20 == _src) {
            ...
            IERC20(address(weth)).transfer(_deposits, newSrcAmount);
        } else {
            // Transfer user tokens to target
            _src.transferFrom(msg.sender, _deposits, newSrcAmount);
        }
    ...
    {
        {
            ...
            for (uint i = 0; i < _deposits.length; i++) {
                ...
                if (ETHER_ERC20 == _src) {
                    IERC20(address(weth)).transfer(_deposits[i], amountForThisRound);
                } else {
                    _src.transferFrom(msg.sender, _deposits[i], amountForThisRound);
                }
            }
        }
    }
