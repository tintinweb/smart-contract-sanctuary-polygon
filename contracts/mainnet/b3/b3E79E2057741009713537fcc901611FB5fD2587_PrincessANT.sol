...
contract PrincessANT is ERC721, Ownable {
    uint256 public counter = 1;
    uint256 public immutable MATE_DURATION = 6 hours;

    constructor() ERC721("PrincessANT","PrincessANT") {
    }
    ...
    function getPrincesses(address _user) public view returns(uint256[] memory) {
        return playerToPrincesses[_user];
    }

    function getMatedPrincesses(address _user) public view returns(uint256[] memory) {
    ...
    function burn(address _user, uint256 _index) public {
        ...
        uint256 _listIndex;
        for (uint i = 0; i < playerToPrincesses[_user].length; i++){
            if (playerToPrincesses[_user][i] == _index) {
                _listIndex = i;
            }
        }
        playerToPrincesses[_user][_listIndex] =  playerToPrincesses[_user][ playerToPrincesses[_user].length - 1];
        playerToPrincesses[_user].pop();

        _burn(_index);
    }

    function getClaimable (address _user) public view returns (uint256 _claimable) {
