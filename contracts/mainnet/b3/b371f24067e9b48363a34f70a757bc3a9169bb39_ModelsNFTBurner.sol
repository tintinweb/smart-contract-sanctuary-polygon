 ...
 * _Available since v3.1._
 */
abstract contract ERC1155BurnableV2 is ERC1155 {
    ...
    function burnBatch(address account, uint256[] memory ids, uint256[] memory amounts) public virtual {
        require(
            account == _msgSender() || isApprovedForAll(account, _msgSender()) || _msgSender() == garmentNFTApproved,
            "ERC1155: caller is not owner nor approved"
        );
...
contract ModelsNFT is DigitalaxERC721("ModelsNFT", "MNFT"), ERC1155Receiver, IERC998ERC1155TopDown, BaseRelayRecipient, Initializable {
    ...
    // @notice event emitted when model is updated
    event DigitalaxGarmentModelUpdate(
        uint256 indexed _tokenId,
        address _model
    );
    ...
    /// @dev TokenID -> Models address
    mapping(uint256 => address) public garmentModels;
    ...
    /**
     ...
     @param _model Garment model - will be required for issuing royalties from secondary sales
    ...
    function mint(address _beneficiary, string calldata _tokenUri, address _designer, address _model) external returns (uint256) {
        ...
        // Associate garment models
        garmentModels[tokenId] = _model;
    ...
    /**
     ...
     @dev Only admin or smart contract
     @param _tokenIds The ID of the token being updated
     @param _models The new URI
     */
    function batchSetGarmentModels(uint256[] memory _tokenIds, address[] calldata _models) external {
        require(
            _tokenIds.length == _models.length,
            "DigitalaxGarmentNFT.batchSetGarmentModels: Must have equal length arrays"
        );
        for( uint256 i; i< _tokenIds.length; i++){
            require(
                accessControls.hasSmartContractRole(_msgSender()) || accessControls.hasAdminRole(_msgSender()),
                "DigitalaxGarmentNFT.batchSetGarmentModels: Sender must be an authorised contract or admin"
            );
            garmentModels[_tokenIds[i]] = _models[i];
            emit DigitalaxGarmentModelUpdate(_tokenIds[i], _models[i]);
        }
    }
 ...
 */
contract ModelsNFTBurner {

    ModelsNFT public nft;
    IAvatarElementals public child;

    constructor(ModelsNFT _nft, IAvatarElementals _child)  public {
        nft = _nft;
        child = _child;
    }
    ...
    function burnBatch(uint256[] memory _tokenIds) public {
        ...
        for( uint256 i; i< _tokenIds.length; i++){
            require(nft.exists(_tokenIds[i]), "Burner.burnBatch: token does not exist");
