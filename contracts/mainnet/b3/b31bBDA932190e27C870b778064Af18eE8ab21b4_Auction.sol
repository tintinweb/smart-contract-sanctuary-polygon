...
contract Auction is IAuction, Ownable, NFTAssist, ReentrancyGuard {
    ...
    function _bid(
        ...
        _addUserActivity(lot.lastBidder, lotId, lot.lastBid, Direction.SELL);
        _addUserActivity(msg.sender, lotId, bid_, Direction.BUY);
    ...
    function _finishLot(uint256 lotId) internal {
        ...
        if (lot.parameters.tokenAddress != WETH) {
            IERC20(lot.parameters.tokenAddress).transfer(lot.owner, profit);
        } else {
    ...
    function lotsStatus(
        ...
        while (offset + inIndex < _lots.length && outIndex < limit) {
            LotOutput memory lotOutput = getLot(offset + inIndex);
...
contract NFTAssist is INFTAssist, ERC1155Holder, ERC721Holder {
    ...
    {
        NFTType nftType = _getNFTType(nft);
        if (nftType == NFTType.ERC721) ERC721(nft).tokenURI(tokenId);
        if (nftType == NFTType.ERC1155) ERC1155(nft).uri(tokenId);
        return '';
    }
