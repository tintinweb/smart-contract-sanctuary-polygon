...
contract NFTLiquifierFactory is BoringOwnable {
    event LogLiquifierCreated(NFTLiquifier indexed liquifier, IERC721 indexed tokenContract, bytes32 merkleRoot, string ipfsURL, string symbol, bool isPublic);
    ...
    ) external onlyOwner returns (NFTLiquifier liquifier){
        liquifier = NFTLiquifier(liquifierMaster.deploy(address(liquifierMaster), abi.encode(_tokenContract, _merkleRoot, _ipfsURL, symbol, address(this)), true));
        ownerLiquifier.push(liquifier);
        emit LogLiquifierCreated(liquifier, _tokenContract, _merkleRoot, _ipfsURL, symbol, false);
    }
    ...
    function makeLiquifierPublic (
        ...
        string memory _ipfsURL,
        string memory symbol
    ) external returns (NFTLiquifier liquifier) {
        liquifier = NFTLiquifier(liquifierMaster.deploy(address(liquifierMaster), abi.encode(_tokenContract, _merkleRoot, _ipfsURL, symbol, address(this)), true));
        publicLiquifier.push(liquifier);
        emit LogLiquifierCreated(liquifier, _tokenContract, _merkleRoot, _ipfsURL, symbol, false);
    }

    function transferLiquifierOwnership(NFTLiquifier[] calldata liquifiers, address _newOwner) external onlyOwner {
        uint i;
