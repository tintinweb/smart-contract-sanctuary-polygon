...
import {ITokensWhitelist} from "./interfaces/ITokensWhitelist.sol";
...
/**
 * @title TokensWhitelist
 * @notice Contract to keep track of whitelisted addresses.
 */
contract TokensWhitelist is ITokensWhitelist, StandardAccessControlEnumerable {
    using EnumerableSet for EnumerableSet.AddressSet;

    /// @notice To keep track of all the whitelisted tokens
    ...
    function addToWhitelist(address newToken) external override onlyMaintainer {
        _require(tokens.add(newToken), Errors.TOKEN_ALREADY_WHITELISTED);
        emit AddedToWhitelist(newToken);
    }

    /**
     * @notice Adds bulk addresses to the whitelist.
     * @param newTokens the new addresses to add.
     */
    ...
    function removeFromWhitelist(
        ...
        _require(tokens.remove(tokenToRemove), Errors.TOKEN_NOT_WHITELISTED);
