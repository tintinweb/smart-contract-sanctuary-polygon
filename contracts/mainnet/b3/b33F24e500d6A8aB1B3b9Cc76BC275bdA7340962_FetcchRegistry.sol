...
contract FetcchRegistry is OwnableUpgradeable, helper {
    ...
    function acquireNamespace(
        ...
        bytes memory data = abi.encodeWithSignature(
            "initialize(string,address,address,address,string,uint8)",
            _name,
            _owner,
            _fetcchAdmin,
            commLayer,
    ...
    function setCommLayerAggregator(address _commlayer) external onlyOwner {
        if (_commlayer == address(0)) revert InvalidAddress();
