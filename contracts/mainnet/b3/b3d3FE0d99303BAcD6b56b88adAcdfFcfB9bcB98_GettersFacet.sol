...
contract GettersFacet is Modifiers {
    ...
    function getAaveV2Tokens(address token) external view returns (address aToken, address vToken) {
        aToken = s.aTokensV2[token];
        vToken = s.vTokensV2[token];
    }

    function getAaveV3Tokens(address token) external view returns (address aToken, address vToken) {
        aToken = s.aTokensV3[token];
        vToken = s.vTokensV3[token];
    }
