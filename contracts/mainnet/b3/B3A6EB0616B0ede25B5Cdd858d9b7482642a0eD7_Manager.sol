...
/**
 * @title Manager
 * @notice Contract that can assign/revoke roles to adddresses on multiple contracts
 */
contract Manager is IManager, ReentrancyGuard, StandardAccessControlEnumerable {
    //----------------------------------------
    // Events
    //----------------------------------------
    ...
    event RolesRenounced(address[] contracts, bytes32[] roles);
    ...
    /**
     * @notice Constructs the Manager contract
    ...
    function grantRoles(
        ...
        _require(rolesCount > 0, Errors.ZERO_LENGTH_ARRAY);
        _require(rolesCount == accounts.length, Errors.ARRAY_LENGTH_MISMATCH);
        _require(rolesCount == contracts.length, Errors.ARRAY_LENGTH_MISMATCH);
        for (uint256 i; i < rolesCount; i++) {
            IAccessControlEnumerable(contracts[i]).grantRole(
                roles[i],
                accounts[i]
            );
        }
        emit RolesGranted(contracts, roles, accounts);
    }
    ...
    function revokeRoles(
        ...
        _require(rolesCount == contracts.length, Errors.ARRAY_LENGTH_MISMATCH);
        for (uint256 i; i < rolesCount; i++) {
            IAccessControlEnumerable(contracts[i]).revokeRole(
                roles[i],
                accounts[i]
            );
        }

        emit RolesRevoked(contracts, roles, accounts);
    }

    /**
     * @notice Allow Manager to renounce roles in contracts
     * @param contracts contracts
     * @param roles Roles id
     */
    function renounceRoles(
        ...
        emit RolesRenounced(contracts, roles);
    }
}