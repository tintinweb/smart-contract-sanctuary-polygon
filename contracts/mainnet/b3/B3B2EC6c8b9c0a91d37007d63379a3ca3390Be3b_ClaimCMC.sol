...
contract ClaimCMC is Ownable {
     ...
     */
    function distributeRandomNFTs(address _user, uint256 _random)
        public
        onlyAdmin
        returns (uint256[] memory, uint256[] memory)
    {
        require(!_user.isContract(), "Claim: Minter should be an address");
        require(claimers[_user] == false, "Already claimed");

        uint256[] memory nftId = new uint256[](_random);
        uint256[] memory tokenIdsWonQty = new uint256[](_random);
        uint256 i = 0;
        uint256 j = 78765;
        while (i < _random) {
            uint256 rand = uint256(
                keccak256(abi.encodePacked(j, block.number, block.timestamp))
            ) % (nftsToDistribute.length);

            uint256 NFTID = nftsToDistribute[rand];

            if (orareToken1155.balanceOf(address(this), NFTID) > 0) {
                nftId[i] = NFTID;
                tokenIdsWonQty[i] = 1;
                i++;
        ...
        emit NFTClaimed(_user, nftId, tokenIdsWonQty);
        return (nftId, tokenIdsWonQty);
    }
