...
contract Internals is Variables, Events {
    ...
    */
    function withdrawInternal(address token_, uint amount_, uint itokenAmount_) internal returns (uint supplyExchangePrice_) {
        ...
        require(itokenAmount_ <= userBal_, "P1:M2: not-enough-user-balance");
    ...
    modifier nonReentrant() {
        require(_status != 2, "P1:M2:ReentrancyGuard: reentrant call");
        _status = 2;
        _;
        _status = 1;
    }

    modifier tokenEnabled(address token_) {
        require(_tokenEnabled[token_], "P1:M2: token-not-enabled");
        _;
    }
...
contract UserModule is Internals {
    ...
    */
    function updateRewards(address token_) public tokenEnabled(token_) returns (uint[] memory newRewardPrices_) {
        ...
        for (uint i = 0; i < rewardTokens_.length; i++) {
            RewardRate memory rewardRate_ = _rewardRate[token_][rewardTokens_[i]];
            RewardPrice memory rewardPrice_ = _rewardPrice[token_][rewardTokens_[i]];
            if (rewardPrice_.lastUpdateTime != block.timestamp && rewardRate_.startTime <= block.timestamp) {
                uint fromTime_ = (rewardPrice_.lastUpdateTime > rewardRate_.startTime) ? rewardPrice_.lastUpdateTime : rewardRate_.startTime;
                uint toTime_ = (block.timestamp > rewardRate_.endTime) ? rewardRate_.endTime : block.timestamp;
                ...
                newRewardPrices_[i] = itokenTotalSupply_ == 0 ? rewardPrice_.rewardPrice :
                    rewardPrice_.rewardPrice.add(rewardRate_.rewardRate.mul(toTime_.sub(fromTime_)).mul(1e27).div(itokenTotalSupply_));
    ...
    */
    function updateUserReward(address user_, address token_) public tokenEnabled(token_) returns (uint[] memory updatedRewards_) {
        address[] memory rewardTokens_ = _rewardTokens[token_];
    ...
    */
    function supply(address token_, uint amount_) external tokenEnabled(token_) nonReentrant returns (uint itokenAmount_) {
        ...
        approve(TokenInterface(token_), address(liquidity), amount_);
    ...
    */
    function withdraw(address token_, uint amount_) external tokenEnabled(token_) nonReentrant returns (uint itokenAmount_) {
        require(amount_ != 0, "P1:M2: amount-cannot-be-0");
        (uint supplyExchangePrice_,) = liquidity.updateInterest(token_);
        updateUserReward(msg.sender, token_);
        // multiplying with 18 decimal as exchange price is in 18 decimals
        itokenAmount_ = amount_.mul(1e18).div(supplyExchangePrice_);
    ...
    */
    function withdrawItoken(address token_, uint itokenAmount_) external tokenEnabled(token_) nonReentrant returns (uint amount_) {
        require(itokenAmount_ != 0, "P1:M2: itoken-amount-cannot-be-0");
        (uint supplyExchangePrice_,) = liquidity.updateInterest(token_);
        updateUserReward(msg.sender, token_);
    ...
    */
    function claim(address user_, address token_) external tokenEnabled(token_) nonReentrant returns (uint[] memory updatedRewards_) {
        address[] memory rewardTokens_ = _rewardTokens[token_];
        updatedRewards_ = updateUserReward(user_, token_);
        for (uint i = 0; i < rewardTokens_.length; i++) {
            _userRewards[user_][token_][rewardTokens_[i]].reward = 0;
            IERC20(rewardTokens_[i]).safeTransferFrom(address(rewardPool), user_, updatedRewards_[i]);
        }
    ...
    */
    function updateRewardsOnTransfer(address from_, address to_) external {
        ...
        require(token_ != address(0), "P1:M2: not-itoken-contract"); // TODO: Need to update the address here
...
contract Variables {
    ...
    IRewardPool constant internal rewardPool = IRewardPool(0x50568FE685e982B15bB8ce155257C651404f19b1); // TODO: add the reward pool address
