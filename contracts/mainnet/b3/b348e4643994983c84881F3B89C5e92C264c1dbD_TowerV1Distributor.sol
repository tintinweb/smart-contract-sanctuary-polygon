...
contract TowerV1Distributor is TowerProtocol {
    ...
    uint256 public constant INIT_VESTING_DURATION = 1095 days;
    ...
    function pendingClaim(address _claimer) public view returns (uint256) {
        ...
        if (_now <= claimer.lastClaimed || _now <= vestingStartTime) {
            return 0;
        }
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < _claimers.length; i++) {
            ...
            claims[_claimers[i]].emissionRate =
                _amounts[i] /
                INIT_VESTING_DURATION;
        }

        emit ClaimersSet();
    }

    function togglePaused() external onlyOwnerOrOperator {
        isPaused = !isPaused;
        emit ClaimPaused();
    }
