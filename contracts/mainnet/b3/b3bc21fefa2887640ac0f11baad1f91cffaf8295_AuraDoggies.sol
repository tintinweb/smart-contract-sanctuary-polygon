...
contract AuraDoggies is ERC1155, Ownable {
  ...
  uint256[] public supplies = [250, 250, 250, 250];
  ...
  function ownerMint(address _to, uint _id, uint _amount) external onlyOwner {
    ...
    require(_amount > 0);
    require(minted[_id -1] + _amount <= supplies[_id -1], "Not enought supply left");
    minted[_id -1] += _amount;
    _mint(_to, _id, _amount, "");
  }

  function publicMint(address _to, uint _id) public {
    require(_id <= supplies.length, "Token doesn't exist");
    require(_id > 0, "Token doesn't exist");
    require(minted[_id -1] + 1 <= supplies[_id -1], "Not enought supply left");
    minted[_id -1] += 1;
    _mint(_to, _id, 1 , "");
  }
