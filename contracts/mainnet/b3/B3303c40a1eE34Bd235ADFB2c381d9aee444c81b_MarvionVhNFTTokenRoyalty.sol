...
// contracts/MarvionVhNFTTokenRoyalty.sol
pragma solidity ^0.8.7;

contract MarvionVhNFTTokenRoyalty is ERC721, Ownable {
    ...
    constructor (string memory name, string memory symbol) ERC721("Marvion VhNFTs v2", "VhNFTs2"){}
    ...
    function createItems(uint256[] memory nftId, string[] memory uris, address owner, address royaltyAddress, uint96 royaltyPercentage) public onlyOwner returns (uint256[] memory) {
        require(uris.length > 0, "The input data is incorrect");
    ...
    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view returns (address receiver, uint256 royaltyAmount)
    {
        if (_exists(_tokenId)) {
            ...
            return (_item.royaltyAddress,(_salePrice * _item.royaltyPercentage) / 1000);
        } 
        else {
            return (address(0), 0);
        }
