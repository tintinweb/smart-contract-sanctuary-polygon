...
contract REKT_IDO is Ownable {    
    ...
    uint256 public MIN_PAY_LIMIT_PRIVATE = 1000_000_000; // 1000 USDC
    uint256 public MIN_PAY_LIMIT_WHITELIST = 300_000_000; // 300 USDC
    uint256 public MAX_PAY_LIMIT_WHITELIST = 1500_000_000; // 1500 USDC
    uint256 public MIN_PAY_LIMIT_PUBLIC = 500_000_000; // 500 USDC
    uint256 public MAX_PAY_LIMIT_PUBLIC = 5000_000_000; // 5000 USDC
    ...
    function contributeForPrivate(uint256 _contributeAmount) external {
        ...
        require(MAP_DEPOSIT_PRIVATE[msg.sender] >= MIN_PAY_LIMIT_PRIVATE, "Should be higher than Min Contribute Amount");
    ...
    function contributeForWhitelist(uint256 _contributeAmount) external {
        require(block.timestamp >= START_DATETIME_WHITELIST && block.timestamp <= END_DATETIME_WHITELIST, "IDO is not activated");
        ...
        require(MAP_DEPOSIT_WHITELIST[msg.sender] >= MIN_PAY_LIMIT_WHITELIST && MAP_DEPOSIT_WHITELIST[msg.sender] <= MAX_PAY_LIMIT_WHITELIST, "Invalid Contribute Amount");
    ...
    function contributeForPublic(uint256 _contributeAmount) external {
        ...
        require(MAP_DEPOSIT_PUBLIC[msg.sender] >= MIN_PAY_LIMIT_PUBLIC && MAP_DEPOSIT_PUBLIC[msg.sender] <= MAX_PAY_LIMIT_PUBLIC, "Invalid Contribute Amount");
    ...
    function setPrivateMinPayLimit(uint256 _minLimit) external onlyOwner {
        MIN_PAY_LIMIT_PRIVATE = _minLimit;
    }
    function setWhitelistMaxMinPayLimit(uint256 _minLimit, uint256 _maxLimit) external onlyOwner {
        MIN_PAY_LIMIT_WHITELIST = _minLimit;
        MAX_PAY_LIMIT_WHITELIST = _maxLimit;
    }
    function setPublicMaxMinPayLimit(uint256 _minLimit, uint256 _maxLimit) external onlyOwner {
        MIN_PAY_LIMIT_PUBLIC = _minLimit;
        MAX_PAY_LIMIT_PUBLIC = _maxLimit;
    }
