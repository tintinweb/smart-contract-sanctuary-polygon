...
interface INFTMarket{
    function mintingMortgage(address _collection, uint256 tokenId,address _owner, address _user,uint256 quantity,uint256 _value) external;

    function mintingPresaleMortgage(address _collection,  uint256 tokenId,address _owner, address _user, uint256 quantity,uint256 _value) external;

    function minting(address _collection, uint256 tokenId, address _owner, uint256 _value, uint256 _quantity, address _token) external;

    function presaleMint(address _collection, uint256 tokenId, address _owner, uint256 _value, uint256 _quantity, address _token) external;
}
...
interface IMortgageControl is IMortgageInterest {
    ...
    uint256 _downPay,uint256 _price,uint256 _startDate,uint256 _period )  external; 
...
import "../NFTContracts/IWrapMembership.sol";
...
contract Mortgages is AccessControl, ReentrancyGuard {
    ...
    address public guardian = 0x858e4C659f2A23D944304365249215eAc5b60433; // relayer mumbai: 0x1921a154365A82b8d54a3Cb6e2Fd7488cD0FFd23; //relayer address 
    ...
    uint256 private feeMembership = 400; //400 is equal to 4%
    ...
    event mortgageEvent(uint256 idMortgage,address wallet,address collection, address wrapContract, uint256 nftId, uint256 loan, uint256 quantity);
    event releaseEvent(uint256 idMortgage,address wallet, address collection,uint256 nftId, string status, uint256 quantity);
    event seized(uint256 idMortgage,address wallet, address collection,uint256 nftId, string status, uint256 quantity);

    constructor(address _tokenInfo,address _mortgageControl, address _referalsControl,address _market,address _token, address foundersRewards, 
    address RewardsLenders, address _membershipVault, address _referalVault, address _membershipValuation, address _walletPanoram){
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(DEV_ROLE, msg.sender);
        _setupRole(DEFAULT_ADMIN_ROLE, 0xb818dd90d667D6D269FA7a690114f98473aF7b95); // cambiar por multisig
        _setupRole(DEV_ROLE, 0xb818dd90d667D6D269FA7a690114f98473aF7b95); // cambiar por multisig
        ...
        permissions(_token, foundersRewards, RewardsLenders, _membershipVault, _referalVault);
        approveToken(_token);
    }
    ...
    function openMortgage(address collection, uint256 tokenId, uint256 loan, uint256 _quantity, uint256 period,address token) public validToken(token) nonReentrant NotPaused returns(uint256 id){
        ...
        locals.price = IMembershipValuation(membershipValuation).getPrice(locals2.tokenId);
        locals.avaibleLoan = _getPortion(locals.price, maxLoan);

        if(locals3._loan > (locals.avaibleLoan * _quantity)){
            revert ("exceed your maximum loan");
        }
        locals2.minLoan = _getPortion(locals.price, minLoan);
        if(locals3._loan < (locals2.minLoan * _quantity)){
            revert ("Less than allowed");
        }
        locals.value = (locals.price * _quantity);
        locals.downPay = locals.value - locals3._loan;
        
        id = ++mortgageId;
        locals.wrapContract = wrapContract;

        IERC20(token).safeTransferFrom(msg.sender, address(this), locals.downPay);
      
        locals2.debt = submitFees(token, locals.value, locals3._loan); 
        //Create and wrap NFT
        INFTMarket(market).mintingMortgage(locals._collection, locals2.tokenId, address(this), msg.sender, _quantity, locals.price);
        IWrapMembership(locals.wrapContract).wrapNFT(msg.sender, locals2.tokenId, _quantity);
        IMembership(locals._collection).safeTransferFrom(address(this),locals.wrapContract,locals2.tokenId,_quantity,"");
        ...
        _quantity, locals.downPay,locals.price, block.timestamp,locals3._period); 
        ...
        emit mortgageEvent(id,msg.sender,locals._collection, locals.wrapContract,locals2.tokenId, locals2.debt, _quantity);
    }
    

    function openManualMortgage(address collection, uint256 tokenId, uint256 loan, uint256 _quantity, uint256 period,address token, 
    address _sponsor, address _owner) public validToken(token) onlyManual nonReentrant NotPaused returns(uint256 id){
        LocalMortgage memory locals;
        LocalMortgage2 memory locals2;
        LocalMortgage3 memory locals3;
        ...
        if(locals3._loan > (locals.avaibleLoan * locals3._quantity)){
            revert ("exceed your maximum loan");
        }
        locals2.minLoan = _getPortion(locals.price, minLoan);
        if(locals3._loan < (locals2.minLoan * locals3._quantity)){
            revert ("Less than allowed");
        }
        locals.value = (locals.price * locals3._quantity);
        locals.downPay = locals.value - locals3._loan;
        
        manualDownpayment(token, locals.downPay);
        ...
        locals2.debt = submitFees(token, locals.value, locals3._loan); 
        //Create and wrap NFT
        INFTMarket(market).mintingMortgage(locals._collection, locals2.tokenId, address(this), _owner, locals3._quantity, locals.price);
        IWrapMembership(locals.wrapContract).wrapNFT(_owner, locals2.tokenId, locals3._quantity);
        IMembership(locals._collection).safeTransferFrom(address(this),locals.wrapContract,locals2.tokenId,locals3._quantity,"");
        //Register information
        IMortgageControl(mortgageControl).addRegistry(id,_owner, locals._collection, locals.wrapContract,locals2.tokenId, locals2.debt,
        locals3._quantity, locals.downPay,locals.price, block.timestamp,locals3._period); 
    
        IMortgageControl(mortgageControl).addIdInfo(id,_owner);
        IMortgageControl(mortgageControl).updateLastTimeCalc(_owner, id, block.timestamp);

        emit mortgageEvent(id,_owner,locals._collection, locals.wrapContract,locals2.tokenId, locals2.debt, locals3._quantity);
    }

    function openPresaleMortgage (address collection, uint256 tokenId, uint256 loan, uint256 _quantity, uint256 period,address token) public validToken(token) nonReentrant NotPaused returns(uint256 id) {
        LocalMortgage memory locals;
        LocalMortgage2 memory locals2;
        LocalMortgage3 memory locals3;
        ...
        locals.price = IMembershipValuation(membershipValuation).getPresalePrice(locals2.tokenId);
        locals.avaibleLoan = _getPortion(locals.price, maxLoan);
        ...
        locals2.debt = submitFees(token, locals.value, locals3._loan); 
        //Create and wrap NFT
        INFTMarket(market).mintingPresaleMortgage(locals._collection, locals2.tokenId, address(this), msg.sender, _quantity, locals.price);
        IWrapMembership(locals.wrapContract).wrapNFT(msg.sender, locals2.tokenId, _quantity);
        IMembership(locals._collection).safeTransferFrom(address(this),locals.wrapContract,locals2.tokenId,_quantity,"");
        //Register information
        IMortgageControl(mortgageControl).addRegistry(id,msg.sender, locals._collection, locals.wrapContract,locals2.tokenId, locals2.debt,
    ...
    function openManualPresaleMortgage (address collection, uint256 tokenId, uint256 loan, uint256 _quantity, uint256 period,address token,
    address _sponsor, address _owner) public validToken(token) onlyManual nonReentrant NotPaused returns(uint256 id) {
        LocalMortgage memory locals;
        LocalMortgage2 memory locals2;
        LocalMortgage3 memory locals3;
        ...
        locals2.debt = submitFees(token, locals.value, locals3._loan); 
        //Create and wrap NFT
        INFTMarket(market).mintingPresaleMortgage(locals._collection, locals2.tokenId, address(this), _owner, locals3._quantity, locals.price);
        IWrapMembership(locals.wrapContract).wrapNFT(_owner, locals2.tokenId, locals3._quantity);
        IMembership(locals._collection).safeTransferFrom(address(this),locals.wrapContract,locals2.tokenId,locals3._quantity,"");
        //Register information
        IMortgageControl(mortgageControl).addRegistry(id,_owner, locals._collection, locals.wrapContract,locals2.tokenId, locals2.debt,
    ...
    function submitFees(address _token, uint256 _value, uint256 _loan) private returns (uint256){
        Fee memory fee;
        LocalMortgage2 memory locals2;
        (fee.lendersVault,fee.vlenRewards,fee.memberVault, fee.referalVault) = tokenInfo.getVaultInfo(_token);
        (, fee.vFounderRewards,) = tokenInfo.getFoundersVaults(_token);
        (fee.membership, fee.founders, fee.referals) = calcFees(_value);
        fee.lenRewards = _getPortion(_loan,creditFee);
        fee.amount1 = _value - fee.membership;
        fee.amount2 = fee.amount1 - fee.founders;
        fee.amount3 = fee.amount2  - fee.referals; //remaining after subtraction of rewards, panoram and referrals feess
        locals2.debt = _loan + fee.lenRewards; //loan + credit fee
        IVaultLenders(fee.lendersVault).withdraw(locals2.debt, _token);
        ...
        IVaultRewards(fee.vlenRewards).deposit(fee.lenRewards, _token);
        IERC20(_token).safeTransfer(walletPanoram, fee.membership);
        return locals2.debt;
    }

    function manualDownpayment(address _token, uint256 _downpayment) private {
        (,,address memberVault,) = tokenInfo.getVaultInfo(_token);
        IVaultRewards(memberVault).withdraw(_downpayment, _token);
    }

    function mortgagePaid(uint256 idMortgage,address collection, address user, uint256 tokenId, uint256 _quantity) public nonReentrant NotPaused {
       LocalMortgage memory locals;
       LocalMortgage2 memory locals2;
       locals._collection = collection;
       locals2.tokenId = tokenId;
        ...
        locals.wrapContract = wrapContract;
        locals2.number = IWrapMembership(locals.wrapContract).balanceOf(user, locals2.tokenId);
        if(locals2.number == 0){
            revert("not have this token");
        }
        locals2.isPay = IMortgageControl(mortgageControl).mortgageStatuts(user, idMortgage);
        if(!locals2.isPay){
            revert("active mortgage");
        }
        if(IMortgageControl(mortgageControl).getRedeemStatus(idMortgage, user)){
            revert("already redeemed");
        }

        IMortgageControl(mortgageControl).updateRedeem(idMortgage, user);
    
        //burn wrapNFT and transfer the original NFT
        IWrapMembership(locals.wrapContract).unWrap(user, locals2.tokenId, _quantity);
        
        emit releaseEvent(idMortgage,user,locals._collection,locals2.tokenId, "released", _quantity); 
    }

    function seizeNFT(uint256 idMortgage,address collection, address user, uint256 tokenId, uint256 _quantity) public NotPaused onlyRelayer{
        ...
        IWrapMembership(wrapContract).unWrap(guardian, tokenId, _quantity);

        emit seized(idMortgage,user,collection,tokenId, "seized", _quantity); 
    }
    ...
    function calcFees(uint256 _fee) internal view returns(uint256 membership, uint256 founders, uint256 referals){
        membership =   _getPortion(_fee, feeMembership); //4% fee
        founders =   _getPortion(_fee, feeFounders);
        referals =   _getPortion(_fee, feeReferals);
        return (membership, founders, referals);
    }

    function updateWrapContract(address _newWrapContract) public onlydev {
        wrapContract = _newWrapContract;
    }
    ...
    function updateFeeRewards(uint256 _newFeeFounders) public onlydev {
            feeFounders = _newFeeFounders;
    }

     function updateFeePanoram(uint256 _newFeeMembership) public onlydev {
            feeMembership = _newFeeMembership;
    }
    function updateFeeLenders(uint64 _newCreditFee) public onlydev {
            creditFee = _newCreditFee;
    }
    ...
    function permissions(address _token, address _foundersRewards, address _RewardsLenders, address _membershipVault, address _referalVault) public onlydev validToken(_token) {
        IERC20(_token).approve(_foundersRewards, MAX_UINT);
        IERC20(_token).approve(_RewardsLenders, MAX_UINT);
        IERC20(_token).approve(_membershipVault, MAX_UINT);
        IERC20(_token).approve(_referalVault, MAX_UINT);
    }
...
import "@openzeppelin/contracts/token/ERC1155/IERC1155.sol";
interface IWrapMembership is IERC1155{

    function wrapNFT(address _wallet, uint256 id, uint256 quantity) external;

    function unWrap(address _wallet, uint256 _Id,  uint256 quantity) external;
