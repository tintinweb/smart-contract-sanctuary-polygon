...
import { LibStargateTokenBridge } from "../libraries/LibStargateTokenBridge.sol";

contract StargateTokenBridgeFacet {
     ...
     * @param sgArgs - Stargate arguments.
     */
    function initStargateSwap(
        LibSrcCrossSwap.CrossSwapArgs calldata crossSwapArgs,
        LibSrcCrossSwap.SrcSwapArgs calldata srcSwapArgs,
        LibStargateTokenBridge.StargateArgs calldata sgArgs
    ) public payable {
        // TODO: split Direct USDC transfer and Paraswap swap into two different methods
        require(s.whitelistedConnectorTokens[crossSwapArgs.connectorToken], "Connector token is not whitelisted");
        require(s.whitelistedStargateRouter[sgArgs.router], "Stargate Router is not whitelisted");
        ...
        uint256 connectorTokenIncome = IERC20(crossSwapArgs.connectorToken).balanceOf(address(this)) - connectorTokenBalanceBeforeSwap;

        require(connectorTokenIncome > 0, "Connector token income should be positive");

        LibStargateTokenBridge._bridgeConnectorToken(
...
library LibStargateTokenBridge {
    using SafeERC20 for IERC20;

    struct StargateArgs {
        uint256 minConnectorTokenAmount;
        uint16 destChainId;
        uint256 srcPoolId;
        uint256 destPoolId;
        uint256 value;
    ...
    /**
     * @notice method to bridge connector token from source chain to destination chain via Stargate
     *
     * @param connectorToken - address of the connector token to be bridged
     * @param connectorTokenHolder - address where to bridge the connector token on the destination chain
     * @param amount - amount of the connector token to be bridged during cross chain swap
     * @param sgArgs - sgArgs amount of the destination token to receive during cross chain swap
     */
    function _bridgeConnectorToken(
        address connectorToken,
        address connectorTokenHolder,
        uint256 amount,
        StargateArgs memory sgArgs,
        uint256 nonce
    ) internal {
        IERC20(connectorToken).approve(sgArgs.router, amount);
        IStargateRouter(sgArgs.router).swap{ value: sgArgs.value }(
            sgArgs.destChainId,
            sgArgs.srcPoolId,
            sgArgs.destPoolId,
            // TODO: who is reposnsible for refund address? (msg.sender or connectorTokenHolder)
            payable(msg.sender),
            amount,
            sgArgs.minConnectorTokenAmount,
            IStargateRouter.lzTxObj(0, 0, ""),
            abi.encodePacked(connectorTokenHolder),
            // TODO: consider using payload to transfer connector token on dest chain
            // rigth to the destUser via Stargate transaction
            // pass encoded nonce to the dest chain in payload
