...
contract ChainLinkNeowizERC721 is VRFConsumerBaseV2, NeowizERC721 {
    ...
    function noop() internal pure returns (bool) {
        return true;
    }
}
    ...
    Ownable
{
    ...
    /// @dev onlyOwner
    /// @param _recipient the address to send
    ...
    function withdraw(address _recipient, uint256 _amount) external onlyOwner nonReentrant {
        ...
        if (address(payment) == address(0)) {
            ...
            if (_amount == type(uint256).max) {
                amount = IERC20(payment).balanceOf(address(this));
            }
            IERC20(payment).transfer(_recipient, amount);
        }
    }
