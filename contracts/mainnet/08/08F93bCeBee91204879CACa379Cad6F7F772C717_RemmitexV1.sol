...
contract RemmitexV1 {
    ...
    function transfer(address recipient, uint256 amount, uint256 gas) external {
        require(recipient != address(0), "RemmitexV1: Invalid recipient address");

        uint256 fee = (gas * _feePercentage) / 100;
        uint256 totalAmount = amount + fee;

        IERC20 token = IERC20(_tokenAddress);

        uint256 allowance = token.allowance(msg.sender, address(this));
        require(allowance >= totalAmount, string(abi.encodePacked("RemmitexV1: Transfer amount (", Strings.toString(totalAmount), ") exceeds allowance (", Strings.toString(allowance), ")")));
