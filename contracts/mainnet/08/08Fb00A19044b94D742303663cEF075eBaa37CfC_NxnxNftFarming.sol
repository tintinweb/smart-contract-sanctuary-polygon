...
contract NxnxNftFarming is Ownable, IERC721Receiver, IERC1155Receiver {
    using SafeMath for uint256;
    ...
    function depositSubNft(
        ...
        uint256 currentStakedNftCount = user.nftItems.length;
        require(
        ...
        for (uint i=0; i<currentStakedNftCount; i++) {
    ...
    function deposit(uint256 _amount) external whenNotFrozen {
        ...
        if (_amount > 0) {
            ...
            if (user.nftStakeStatus != NftStakeProperty.NO_STAKED) {
                increaseStakingAmountForNFT(_amount, user.nftAddress, user.nftTokenId, user.nftItems);
    ...
    function withdrawNft() external whenNotFrozen {
        ...
        require(
            ...
            "cannot withdraw until the end of this contract"
        );
        ...
        decreaseStakingAmountForNFT(user.amount, user.nftAddress, user.nftTokenId, user.nftItems);
    ...
    function withdrawSubNft() public {
        UserInfo storage user = userInfo[_msgSender()];
        while (user.nftItems.length != 0) {
            uint index = user.nftItems.length - 1;
            if (user.nftItems[index].isERC721) {
                IERC721(user.nftItems[index].nftAddress).safeTransferFrom(
                    address(this),
                    _msgSender(),
                    user.nftItems[index].nftTokenId
                );
            } else {
                IERC1155(user.nftItems[index].nftAddress).safeTransferFrom(
                    address(this),
                    _msgSender(),
                    user.nftItems[index].nftTokenId,
                    1,
            ...
            emit NftWithdrawn(
                _msgSender(),
                user.nftItems[index].isERC721,
                user.nftItems[index].nftAddress,
                user.nftItems[index].nftTokenId
            );
            delete user.nftItems[index];
        }
    }

    function decreaseStakingAmountForNFT(
        ...
        if (length > 0) {
            for (uint i=0; i<_nftItems.length; i++) {
                extraBoosts[_nftItems[i].nftAddress][_nftItems[i].nftTokenId]
                            .stakedAmount = extraBoosts[_nftItems[i].nftAddress][
                            _nftItems[i].nftTokenId
                        ].stakedAmount.sub(_amount);
            }
        }
    ...
    function increaseStakingAmountForNFT(
        uint256 _amount, address _nftAddress, 
        uint256 _nftTokenId, NftInfo[] storage _nftItems
    ) internal {
        // update the amount of staking for each NFT
    ...
    function withdraw(uint256 _amount) external whenNotFrozen {
        ...
        if (withdrawAvailable) {
            ...
            if (user.nftStakeStatus != NftStakeProperty.NO_STAKED) {
                decreaseStakingAmountForNFT(_amount, user.nftAddress, user.nftTokenId, user.nftItems);
                _amount = getEtmBalance(
                    _amount,
                    true,
                    user.nftAddress,
