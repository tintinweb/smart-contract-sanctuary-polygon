...
// File: contracts/Admin/GachaMachineV3.sol
        ...
        for (uint256 i = 0; i < _gachas.length; i++) {
            ...
            if (uint256(GachaType.Character) == gachaType) {
                ...
                for (uint256 j = 0; j < _gacha.randomSeed.length; j++) {
                    ...
                    _character[j] = ILuxOnCharacter.Character(_gacha.randomSeed[j].actorTokenId, name);
                    emit GachaActor(_gacha.randomSeed[j].userAddress, _gacha.gachaTokenId, _gacha.randomSeed[j].actorTokenId, _character[j].name);
                }
                ...
                for (uint256 j = 0; j < _gacha.randomSeed.length; j++) {
                    ...
                    string memory name = DspFateCoreData(fateCoreDataAddress).getFateCoreName(actorId);
                    _character[j] = ILuxOnCharacter.Character(_gacha.randomSeed[j].actorTokenId, name);
                    emit GachaActor(_gacha.randomSeed[j].userAddress, _gacha.gachaTokenId, _gacha.randomSeed[j].actorTokenId, _character[j].name);
                }
            }
    ...
    function gachaActorSimulator(GachaSimulator memory _gacha) public view returns (string[] memory) {
        ...
        if (uint256(GachaType.Character) == gachaType) {
            ...
            for (uint256 j = 0; j < _gacha.seed.length; j++) {
                ...
                uint256 actorId = DspGachaData(gachaDataAddress).getFateCoreByIndex(_gacha.gachaTokenId, index);
                _characterName[j] = DspFateCoreData(fateCoreDataAddress).getFateCoreName(actorId);
            }
        }
