...
// File: contracts/boki1155.sol

pragma solidity ^0.8.4;

contract BokiEvent is ERC1155, Ownable, ERC1155Supply {
    // token URI
    string baseURI;
    string URISuffix;
    uint256 public eventID = 1;
    // test name
    string public name = "Boki Events";
    ...
    // batch mint
    function airdropByID(address[] calldata receivers, uint256 tokenId)
        public
        onlyOwner
    {
    ...
    function airdrop(address[] calldata receivers) public onlyOwner {
        require(receivers.length > 0, "Need at least one address");
        for (uint256 i; i < receivers.length; ++i) {
            _mint(receivers[i], eventID, 1, "");
        }
    }

    function setEventID(uint256 _newID) external onlyOwner {
        eventID = _newID;
    }

    function incrementEventID() external onlyOwner {
        eventID += 1;
    }

    function decrementEventID() external onlyOwner {
        eventID -= 1;
    }
