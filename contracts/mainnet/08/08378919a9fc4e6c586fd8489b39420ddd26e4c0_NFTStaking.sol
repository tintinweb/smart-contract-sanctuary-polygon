...
// File: https://github.com/abc-media-net/nft-staking/Collection.sol
...
contract Collection is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("ABC Media NFT Collection", "ABC") {}


    function _baseURI() internal view virtual override returns (string memory) {
    return "https://abc-media.net/";
...
// File: https://github.com/abc-media-net/nft-staking/Token.sol
...
contract ABCToken is ERC20, ERC20Burnable, Ownable {
...
contract NFTStaking is Ownable, IERC721Receiver {
  ...
  function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      earned += 0.1 ether * (block.timestamp - stakedAt) / 1 days;
      vault[tokenId] = Stake({
        owner: account,
        tokenId: uint24(tokenId),
        timestamp: uint48(block.timestamp)
  ...
  function earningInfo(uint256[] calldata tokenIds) external view returns (uint256[2] memory info) {
     ...
     uint256 earned = 0;
      ...
      uint256 stakedAt = staked.timestamp;
      earned += 0.1 ether * (block.timestamp - stakedAt) / 1 days;
    uint256 earnRatePerSecond = totalScore * 0.1 ether / 1 days;
    earnRatePerSecond = earnRatePerSecond / 1;
