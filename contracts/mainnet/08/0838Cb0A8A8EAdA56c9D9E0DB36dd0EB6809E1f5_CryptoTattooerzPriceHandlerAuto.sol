...
contract CryptoTattooerzPriceHandlerAuto is CryptoTattooerzPriceHandler{
     ...
     * 'priceUSD' TOKEN USD price
     * 'decimalsUSD' Number of decimals of the TOKEN USD price
     */
    event TokenPriceDataChanged(address indexed admin, uint8 decimals, uint256 priceUSD, uint8 decimalsUSD);
    ...
    function getPriceData(bytes32 currency) public view override returns (CurrencyPriceData memory) {
        ...
        (uint256 priceUSD, uint8 decimalsUSD) = Decimals.cleanFromTrailingZeros(uint256(priceUSD_), decimalsUSD_);
    ...
    function _getCurrencyData(bytes32 currency) internal view returns (CurrencyData storage) {
        ...
        require(currencyData.usdAggregatorV3Address != address(0), "PriceHandler: Unknown requested currency");
        return currencyData;
    }
    /**
    ...
    function _setCurrencyData(bytes32 currency, uint8 decimals, address usdAggregatorV3Address) internal {
        require(currency != getTOKEN(), "PriceHandler: Price data cannot be set for TOKEN");
        CurrencyData storage currencyData = _currenciesData[currency];
    ...
    function _setTokenPriceData(uint8 decimals, uint256 priceUSD, uint8 decimalsUSD) internal {
        ...
        (_priceData.priceUSD, _priceData.decimalsUSD) = Decimals.cleanFromTrailingZeros(priceUSD, decimalsUSD);
        emit TokenPriceDataChanged(msg.sender, _priceData.decimals, _priceData.priceUSD, _priceData.decimalsUSD);
    }
}
...
abstract contract CryptoTattooerzPriceHandler is ICryptoTattooerzPriceHandler, CryptoTattooerzCurrencyHandler, AccessControlEnumerable{
     ...
     */
    event TokenPriceDiscountChanged(address indexed admin, uint256 endingAmountUSD, uint32 maxDiscountRate, uint8 decimals);

    /**
     * @dev Default constructor
     */
    constructor(bytes32 TOKEN_) CryptoTattooerzCurrencyHandler(TOKEN_) {
     ...
     * @return The amount of 'toCurrency' corresponding to given amount of 'fromCurrency' associate with applyed bonus
     * rate and pivot USD amount (and their applicable decimals)
     */
    function transform(bytes32 fromCurrency, bytes32 toCurrency, uint256 amount) public view override returns (uint256, uint256, uint8, uint256, uint8) {
        // No calculation needed
        uint256 bonusRate = 0;
        uint8 bonusDecimals = 0;
        if(amount == 0 || fromCurrency == toCurrency) {
            return (amount, bonusRate, bonusDecimals, 0, 0);
        }
        ...
        (uint256 pivotPriceUSD, uint8 pivotDecimalsUSD) = (fromPriceUSD, fromDecimalsUSD);
        // When converting to TOKEN, a bonus may apply
        if(toCurrency == getTOKEN()) {
            // Get the applicable discount
            (bonusRate, bonusDecimals) = calculateTokenPriceDiscountRate(fromPriceUSD, fromDecimalsUSD);
            // Apply the potential bonus, ie increase usable amount of USD
            fromPriceUSD = fromPriceUSD * (10**bonusDecimals + bonusRate);
            fromDecimalsUSD += bonusDecimals;
        }
        ...
        require(result > 0, "PriceHandler: Requested quantity too low to calculate a price");
        return (result, bonusRate, bonusDecimals, pivotPriceUSD, pivotDecimalsUSD);
    }
    /**
     ...
     * @return The amount of 'fromCurrency' corresponding to given amount of 'toCurrency' associate with applyed discount
     * rate and pivot USD amount (and their applicable decimals)
     */
    function transformBack(bytes32 fromCurrency, bytes32 toCurrency, uint256 amount) public view override returns (uint256, uint256, uint8, uint256, uint8) {
        // No calculation needed
        uint256 discountRate = 0;
        uint8 discountDecimals = 0;
        if(amount == 0 || fromCurrency == toCurrency) {
            return (amount, discountRate, discountDecimals, 0, 0);
        }
        ...
        // Keep it as pivot USD amount
        (uint256 pivotPriceUSD, uint8 pivotDecimalsUSD) = (toPriceUSD, toDecimalsUSD);
        // When converting back from TOKEN, a discount may apply
        if(toCurrency == getTOKEN()) {
            // Get the applicable discount
            (discountRate, discountDecimals) = calculateTokenPriceDiscountRate(toPriceUSD, toDecimalsUSD);
            // Apply the potential discount, ie decrease needed amount of USD
            toPriceUSD = toPriceUSD * (10**discountDecimals - discountRate);
            toDecimalsUSD += discountDecimals;
            pivotPriceUSD = toPriceUSD;
            pivotDecimalsUSD = toDecimalsUSD;
        }
        // Align USD decimals if needed
        if(fromDecimalsUSD < toDecimalsUSD) {
        ...
        uint256 result = toPriceUSD / fromRateUSD;
        require(result > 0, "PriceHandler: Requested quantity too low to calculate a price");
        return (result, discountRate, discountDecimals, pivotPriceUSD, pivotDecimalsUSD);
    }
    /**
     ...
     */
    function setTokenPriceDiscount(uint256 endingAmountUSD, uint32 maxDiscountRate, uint8 decimals) external onlyRole(PRICES_ADMIN_ROLE){
        (maxDiscountRate, decimals) = Decimals.cleanFromTrailingZeros_uint32(maxDiscountRate, decimals);
        _tokenPriceDiscount = TokenPriceDiscount(endingAmountUSD, maxDiscountRate, decimals);
        emit TokenPriceDiscountChanged(msg.sender, endingAmountUSD, maxDiscountRate, decimals);
    }
    /**
     ...
     */
    function calculateTokenPriceDiscountRate(uint256 amountUSD, uint8 decimalsUSD) public view returns(uint256, uint8){
        ...
        return Decimals.cleanFromTrailingZeros(discountRate, decimals);
    }
}
...
interface ICryptoTattooerzPriceHandler {
     ...
     * @param toCurrency Currency into which amount should be transformed from 'toCurrency'
     * @return The amount of 'toCurrency' corresponding to given amount of 'fromCurrency' associate with applyed bonus
     * rate and pivot currency amount (and their applicable decimals)
     */
    function transform(bytes32 fromCurrency, bytes32 toCurrency, uint256 amount) external view returns (uint256, uint256, uint8, uint256, uint8);
    /**
     ...
     * rate and pivot currency amount (and their applicable decimals)
     */
    function transformBack(bytes32 fromCurrency, bytes32 toCurrency, uint256 amount) external view returns (uint256, uint256, uint8, uint256, uint8);
    /**
     * @dev Getter of the USD price for given currency amount (could be default chain coin "COIN", proprietary token
      "TOKEN", or any other of the handled tokens such as "USDC"...)
