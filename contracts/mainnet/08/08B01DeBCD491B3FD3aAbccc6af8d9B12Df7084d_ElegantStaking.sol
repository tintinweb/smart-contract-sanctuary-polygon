...
contract ElegantStaking is Ownable, AccessControl, Concatenate {
    ...
    function getOwnerStaked(address _owner) public view returns ( uint [] memory){
        ...
        uint[] memory ownersStaked;
        uint index;
        ownersTokens = RacerInterface(erc721Contract).tokensOfOwner(_owner);
    ...
    function returnTokens(uint256[] calldata _tokenID) public {
        ...
        for( uint i = 0; i < _racers.length; i ++ ) {
            ...
            //require(perfByRacer[_tok].user == msg.sender, "That NFT does not belong to you.");
            uint index;
            for (index = 0; index < stakedNFTs.length; index ++){
                if (stakedNFTs[index] == _tok) {
                    break;
                }
            }

            uint _tt = perfByRacer[_tok].id;
