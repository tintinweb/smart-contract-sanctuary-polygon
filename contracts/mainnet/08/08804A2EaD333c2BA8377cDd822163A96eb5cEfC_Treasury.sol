...
contract Treasury is ContractGuard {
    ...
    {
        return (get10MBUpdatedPrice(), get10SHAREPrice(), IERC20(_10MB).totalSupply(), target_collateral_ratio, effective_collateral_ratio, globalCollateralValue(), minting_fee, redemption_fee);
    }
    ...
    // Iterate through all pools and calculate all value of collateral in all pools globally
    function globalIronSupply() public view returns (uint256) {
        uint256 total_iron_10MB_minted_ = 0;
        for (uint256 i = 0; i < pools_array.length; i++) {
            // Exclude null addresses
            if (pools_array[i] != address(0)) {
                total_iron_10MB_minted_ = total_iron_10MB_minted_.add(IPool(pools_array[i]).netSupplyMinted());
            }
        }
        return total_iron_10MB_minted_;
    }
    ...
    function getBurnable10MBLeft() public view returns (uint256 _burnable10MBLeft) {
        ...
        if (__10MBPrice <= _10MBPriceOne) {
            ...
            if (_bondMaxSupply > _bondSupply) {
                ...
                if (_rate > 0) {
                    uint256 _maxBurnable10MB = _maxMintableBond.mul(1e5).div(_rate);
    ...
    function get10MBCirculatingSupply() public view returns (uint256) {
        ...
        uint256 totalCircSupply =  totalSupply.sub(balanceExcluded);
        uint256 totalIronSupply = globalIronSupply();
    ...
    function buyBonds(uint256 __10MBAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        //_update10MBPrice();
        try this.treasuryUpdates() {} catch {}
