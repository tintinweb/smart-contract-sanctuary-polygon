...
interface IERC721EnumerableUpgradeable is IERC721Upgradeable {
    ...
    function allTokenOfOwner(address owner) external view returns (uint256[] memory);

    function allTokenFromLatest(uint256 quantity) external view returns (uint256[] memory);
...
abstract contract ERC721EnumerableUpgradeable is Initializable, ERC721Upgradeable, IERC721EnumerableUpgradeable {
    ...
    /**
     * @dev See {IERC721Enumerable-allTokenOfOwner}.
     */
    function allTokenOfOwner(address owner) public view virtual override returns (uint256[] memory) {
        uint256 totalOwned = ERC721Upgradeable.balanceOf(owner);
    ...
    /**
     * @dev See {IERC721Enumerable-allTokenFromLatest}.
     */
    function allTokenFromLatest(uint256 quantity) public view virtual override returns (uint256[] memory) {
        require(quantity > 0, "ERC721Enumerable: quantity must be big more than 0");
        uint256 totalToken = totalSupply();
        if (totalToken < quantity) quantity = totalToken;
        uint256[] memory ret = new uint256[](quantity);
        for (uint256 i = totalToken - quantity; i < totalToken; i++) {
            ret[i + quantity - totalToken] = _allTokens[i];
        }
        return ret;
    }
...
contract StepwatchAccessory is
    Initializable,
    ERC721Upgradeable,
    ...
    ERC721BurnableUpgradeable
{
    ...
    function initialize() public initializer {
        __ERC721_init("Stepwatch Accessory", "SWA");
    ...
    uint256 public latestTokenId = allTokenFromLatest(1).length > 0 ? allTokenFromLatest(1)[0] : 0;

    function _safeMint(address to, uint256 tokenId) internal override(ERC721Upgradeable) {
        super._safeMint(to, tokenId);
        latestTokenId = tokenId > latestTokenId ? tokenId : latestTokenId;
    }
    ...
    function safeMint(
        ...
        require(
            ...
            "The number of tokenIds must be equal to the number of tokenUris"
        );
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            _safeMint(_to, _tokenIds[i]);
            _setTokenURI(_tokenIds[i], _tokenUris[i]);
            emit Mint(_tokenIds[i], _msgSender(), _to);
        }
    }

    function multipleTransferFrom(
        address _from,
        address[] memory _to,
        uint256[] memory _tokenIds
    ) external {
        require(
            ...
            "The number of recipients must be equal to the number of tokens"
        );
        for (uint256 i = 0; i < _tokenIds.length; i++)
    ...
    function safeMintV2(
        ...
        require(
            (_to.length == _tokenIds.length) &&
                (_tokenIds.length == _tokenUris.length),
            "MultiMint: The length of arrays is different"
        );
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            _safeMint(_to[i], _tokenIds[i]);
            _setTokenURI(_tokenIds[i], _tokenUris[i]);
            emit Mint(_tokenIds[i], _msgSender(), _to[i]);
        }
    }

    function multipleTransferFromV2(
        ...
        require(
            ...
            "MultiTransfer: The length of arrays is different"
        );
        for (uint256 i = 0; i < _tokenIds.length; i++)
            safeTransferFrom(_from[i], _to[i], _tokenIds[i], "");
    }

    function multipleBurn(uint256[] memory _tokenIds) external {
        for (uint256 i = 0; i < _tokenIds.length; i++) burn(_tokenIds[i]);
    }
        ...
        returns (bool)
    {
        ...
        return super.isApprovedForAll(from, operator);
    }
}