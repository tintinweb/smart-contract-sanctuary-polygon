...
contract Web3Dao {
    ...
    IdaoContract Web3DaoContract;

    constructor(){
        owner = msg.sender;
        nextProposal = 1;
        Web3DaoContract = IdaoContract (0x2953399124F0cBB46d2CbACD8A89cF0599974963); // contract address
        validTokens = [830545988597858266588487762776785078850967657980002945226505712555467472897]; // this helps determine owner of the contract listing the first minted nft in the collection
    }
    ...
    function checkProposalEligibility(address _proposalist) private view returns (
        ...
        for(uint i = 0; i < validTokens.length; i++){
            if(Web3DaoContract.balanceOf(_proposalist, validTokens[i]) >= 1){
                return true;
            }
        }
        return false;
