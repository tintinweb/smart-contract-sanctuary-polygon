...
contract Valireum is ERC20, ERC20Burnable {
    
    uint256 public preSalePrice = 0.01 ether; // PreSale Price --
    uint256 public pubSalePrice = 0.015 ether; // Public Sale Price --

    uint256 public preSaleSupply = 25000000 ether; // PreSales available supply
    uint256 public pubSaleSupply = 100000000 ether; // Sales available supply
    uint256 public cap = 500000000 ether; // VLM Maximum supply
        
    bool public preSale = true; // Is presale is active?
    bool public pubSale = false; // Is Public sale is active?
    ...
    uint256 oldSeedCount = 199; // number of old VLM holders
    ...
    constructor(address[] memory _holders, uint256[] memory amounts) ERC20 ("Valireum", "VLM") {
    ...
    function buyToken(uint256 _amount) public payable {
        ...
        require(preSale || pubSale, "Sorry the token sales has ended");
        uint256 minAmount = preSale? 10000 ether: 1000 ether;
        require(_amount >= minAmount, "Minimum amount is low");
        require(_amount % 2 == 0, "Amount is not valid");
        // Check if the amount requested is available
        uint SaleSupply = preSale? preSaleSupply: pubSaleSupply;
        ...
        // Add the 1/2 amount to the seeder balance && mint 1/2 amount
        seederBalance[msg.sender].totalAmount += _amount / 2;
        _mint(msg.sender, _amount / 2);
        // Remove the amount from SaleSupply
        SaleSupply -= _amount;
        preSale? preSaleSupply = SaleSupply: pubSaleSupply = SaleSupply;
        ...
        if (pubSale) {
            ...
            // If preSale
            // transfer marketing funds 100%
            marketingFunds.transfer(msg.value);
        }
    ...
    function getSaleInfo () public view returns (uint256 price, bool saleLive, string memory activeStage, uint256 available){
        price = preSale? preSalePrice : pubSalePrice;
        saleLive = preSale || pubSale;
        activeStage = preSale? "Presale" : "Public Sale";
        available = preSale? preSaleSupply: pubSaleSupply;
        
        
        return (price, saleLive, activeStage, available);
    }

    
    //  Check the seeder balance of the sender

    function getVestedBalance(address _address) public view returns (uint256 balance) {
        balance = seederBalance[_address].totalAmount - seederBalance[_address].paidAmount;
        return balance;
    }
    ...
    function seederPayment() internal {
        for (uint i = 0; i < _seederIds.current(); i++) {
            ...
            if (!seederBalance[_to].paid) { // Check if the seeder is not paid
                ...
                if (currentBalance < 1000 ether) { // Check if the balance is less than 1000 VLM
    ...
    function setActiveSale(bool _preSale, bool _pubSale) external {
        require(msg.sender == factory, "Transaction not authorized");
        preSale = _preSale;
        pubSale = _pubSale;
    }
