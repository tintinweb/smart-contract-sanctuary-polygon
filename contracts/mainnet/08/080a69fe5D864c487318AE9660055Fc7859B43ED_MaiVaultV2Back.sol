...
*/
contract MaiVaultV2Back is BaseSiloAction {
    ...
    function extraWithdrawCollateral(
        ...
        if (vaultDebt > 0) {
            uint256 maiPrice = getPrice(path, collateralUnit);

            if (maiPrice > 0) {
                uint256 collateralBalance = collateral.balanceOf(address(this));
                uint256 collateralAmount = (((vaultDebt * collateralUnit) /
                    maiPrice) * 105) / 100;
    ...
    function getPrice(
        ...
        IExchangeRateOracle oracle = IExchangeRateOracle(
            0xDF34c95f9Fb8Ea0e6Ce2669f4f4170b276bce801
        );

        quote = oracle.getOutAmount(path, amountIn); //0
    }
