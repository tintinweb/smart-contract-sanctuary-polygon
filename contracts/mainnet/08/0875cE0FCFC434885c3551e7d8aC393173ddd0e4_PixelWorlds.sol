...
library colorGenerator {
    ...
    function getColors(uint256 tokenId) internal pure returns (string memory color){
...
contract PixelWorlds is ERC721,PixelWorldsRight,PixelWorldsMiddle,PixelWorldsLeft  {
    ...
    function generateHTMLandSVG(uint256 tokenId) internal view returns (string memory, string memory) {
        return(
            string(abi.encodePacked('<svg xmlns="http://www.w3.org/2000/svg" width="350" height="350" style="background-color: grey;text-align: center;"><rect x="0" y="0" width="350" height="350" style="fill:#',colorGenerator.getColors(data[tokenId]),';"></rect><rect x="105" y="105" width="140" height="140" style="fill:#AAAAAA;"></rect><rect x="111" y="111" width="128" height="128" style="fill:#9b7653;"></rect><rect x="111" y="111" width="128" height="48" style="fill:#63b521;"></rect><text x="50%" y="60%" style="font-size:27px; font-family:Arial; text-anchor:middle">Open to inspect</text></svg>')),
            string(abi.encodePacked(PixelWorldsLeft.dataL,Strings.toString(data[tokenId]),PixelWorldsMiddle.dataM,Strings.toString(getSize(tokenId)),PixelWorldsRight.dataR))
            );
    }
                ...
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"',
                                "Pixel World | ", _toString(tokenId),"",
                                '", "description":"Be a god of your pixel world!", "image":"', string(abi.encodePacked("data:image/svg+xml;base64,",Base64.encode(bytes(string(abi.encodePacked(svg)))))),
                                '", "animation_url":"', string(abi.encodePacked("data:text/html;base64,", Base64.encode(bytes(string(abi.encodePacked(html)))))),'","attributes": [{"trait_type": "Grid size", "value": "',_toString(getSize(tokenId)),'"}]}'
                            )
                        )
                    )
                )
    ...
    function mint() public {
        ...
        uint256 randomness = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, nonce)));
