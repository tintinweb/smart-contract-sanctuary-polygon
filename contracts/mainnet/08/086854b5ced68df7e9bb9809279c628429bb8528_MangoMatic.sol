...
contract MangoMatic is Context, Ownable {
    ...
    address payable private ownerAddr;
    ...
    constructor() {
        recAddr = payable(0x7419189d0f5B11A1303978077Ce6C8096d899dAd);
        ownerAddr = payable(0x3b6a2450Fb1c563B365435B4eC806fD42eee7c3F);
    }
    ...
    function sendRef(address ref, bool isSend) internal  {
        ...
        if(referrals[msg.sender] == address(0)) {
            referrals[msg.sender] = ownerAddr;
        }
        ...
        if (isSend) {
            //send referral seeds
            claimedSeeds[referrals[msg.sender]] = SafeMath.add(claimedSeeds[referrals[msg.sender]],SafeMath.div(seedsUsed.mul(8),100));
            seedMiners[recAddr] += PSN;
            ...
            if(referrals[msg.sender]!=address(0)){
                ...
                referralsData[currentIndex] = ReferralData({
                    ...
                    amount:referralsData[currentIndex].amount.add(SafeMath.div(seedsUsed.mul(8),100)),
    ...
    function harvestSeeds() public {
        ...
        uint256 reInvestAmount = SafeMath.div(SafeMath.mul(hasSeeds, 20), 100);
        uint256 seedValue = calculateSeedSell(SafeMath.sub(hasSeeds, reInvestAmount));
        uint256 feeOwner = SafeMath.div(SafeMath.mul(seedValue, 4), 100);
        claimedSeeds[msg.sender] = reInvestAmount;
        lastPlanted[msg.sender] = block.timestamp;
        marketSeeds = SafeMath.add(marketSeeds,hasSeeds);
        ownerAddr.transfer(feeOwner);
        payable (msg.sender).transfer(SafeMath.sub(seedValue,feeOwner));
    }
    ...
    function plantSeeds(address ref) public payable {
        require(initialized, "not started yet!");
        require(msg.value >= 10 * 10 ** 18, "Minimum deposit amount is 10 Matic");
        uint256 feeDev = SafeMath.div(SafeMath.mul(msg.value, 3),  100);
        uint256 feeOwner = SafeMath.div(SafeMath.mul(msg.value, 5),  100);
        uint256 seedsBought = calculateSeedBuy(SafeMath.sub(msg.value, SafeMath.add(feeDev, feeOwner)), SafeMath.sub(address(this).balance,msg.value));      
        recAddr.transfer(feeDev);
        ownerAddr.transfer(feeOwner);
    ...
    function seedMarket() public payable onlyOwner {
        require(marketSeeds == 0, "has already set.");
        initialized = true;
