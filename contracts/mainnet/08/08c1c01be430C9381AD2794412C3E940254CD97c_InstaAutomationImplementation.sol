...
contract Helpers is Events {
    ...
    function _buildSpell(
        bool isIsolated, // remove this params: unused
        uint256 debtAmt_,
        uint256 loanAmt_,
        uint256 loanAmtWithFee_,
        uint256 rateMode_,
        ...
        if (loanAmt_ > 0) {
            ...
            bool isSameToken = swap.buyToken == swap.sellToken;
    
            (flashloanSpell._targets, flashloanSpell._datas) = (
                new string[]((isSameToken) ? 3 : 4),
                new bytes[]((isSameToken) ? 3 : 4)
            );
            ...
            (spell._targets, spell._datas) = (
                new string[]((swap.buyToken == swap.sellToken) ? 2 : 3),
                new bytes[]((swap.buyToken == swap.sellToken) ? 2 : 3)
            );

            /**
             * This case if the user have enough collateral to payback the debt
             */
            (spell._targets[ctr], spell._datas[ctr++]) = (
                "AAVE-V3-A",
                abi.encodeWithSignature(
                    "withdraw(address,uint256,uint256,uint256)",
