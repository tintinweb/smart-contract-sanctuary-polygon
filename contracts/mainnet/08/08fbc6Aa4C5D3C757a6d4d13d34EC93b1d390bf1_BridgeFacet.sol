...
contract BridgeFacet is IBridge {
    ...
    function bridgeIn(BridgeInArgs calldata bridgeInArgs) external payable override returns (uint64 tokenSequence) {
        tokenSequence = LibBridge.bridgeIn(bridgeInArgs);
    }
...
interface IBridge {
    ...
    function bridgeIn(BridgeInArgs calldata bridgeInArgs) external payable returns (uint64 tokenSequence);
...
library LibBridge {
    ...
    function bridgeIn(BridgeInArgs memory bridgeInArgs) internal returns (uint64 tokenSequence) {
        ...
        if (bridgeInArgs.bridgeArgs.bridgeType == BridgeType.Wormhole) {
            LibWormhole.bridgeIn(
                bridgeInArgs.recipientNetworkId,
                tokenSequence,
                bridgeInArgs.bridgeArgs,
                bridgeInArgs.amount,
                bridgeInArgs.toAssetAddress
            );
        } else if (bridgeInArgs.bridgeArgs.bridgeType == BridgeType.Stargate) {
            LibStargate.bridgeIn(
                bridgeInArgs.recipientNetworkId,
                tokenSequence,
                bridgeInArgs.bridgeArgs,
                bridgeInArgs.amount,
    ...
    function bridgeOut(BridgeOutArgs memory bridgeOutArgs) internal returns (uint256 amount) {
        if (bridgeOutArgs.bridgeArgs.bridgeType == BridgeType.Wormhole) {
            amount = LibWormhole.bridgeOut(bridgeOutArgs.bridgeArgs.payload, bridgeOutArgs.transaction);
        } else if (bridgeOutArgs.bridgeArgs.bridgeType == BridgeType.Stargate) {
            amount = LibStargate.bridgeOut(
                bridgeOutArgs.bridgeArgs.payload,
                bridgeOutArgs.transaction,
                bridgeOutArgs.transferKey
            );
        } else {
            revert InvalidBridgeType();
        }
...
import {TransferKey} from "../data-transfer/LibCommon.sol";
...
import {BridgeArgs} from "./LibCommon.sol";
...
import {DataTransferType} from "../libraries/LibMagpieAggregator.sol";
