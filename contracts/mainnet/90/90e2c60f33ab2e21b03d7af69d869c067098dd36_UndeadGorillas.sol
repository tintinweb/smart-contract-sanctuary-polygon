...
contract UndeadGorillas is ERC721Enumerable, Ownable {
    uint public  MAX_TOKEN = 3000;
    uint public  basePrice = 50*10**18; // MATIC
    uint public  basePriceMGG = 30*10**18; // MATIC
    uint public  basePriceGiB = 30*10**18; // MATIC
    mapping (uint256 => bool) public _whitelistMGG;
    mapping (uint256 => bool) public _whitelistGiB;
    mapping (address => uint256) public _WL_balances;
    uint public  WhitelistIdCount = 0;
	string _baseTokenURI;
	bool public saleEnable = false;

    UndeadGorillas internal MGG = UndeadGorillas(0x5f2338c3748C58C57bc52747ee1f292e7960b369);  
	UndeadGorillas internal GiB = UndeadGorillas(0x5f2338c3748C58C57bc52747ee1f292e7960b369);  
    ...
    function setBasePriceGiB(uint  _basePriceGiB) public onlyOwner {
         basePriceGiB = _basePriceGiB;
    }

    constructor(string memory baseURI) ERC721("Undead Gorillas", "UG")  {
        setBaseURI(baseURI);
    }
    ...
    function CompletedMintForGiB(uint256 _GiBid) public view virtual returns (bool) {
         return _whitelistGiB[_GiBid];
    }
   
    function TokenForMintAvailable(address _holder) public view virtual returns (uint256) {
        uint128  tokenAvailable = 0;
        uint256[] memory tokensId = new uint256[](GiB.balanceOf(_holder));
        for(uint i = 0; i < GiB.balanceOf(_holder); i++){
            tokensId[i] = GiB.tokenOfOwnerByIndex(_holder, i);
            
            if (_whitelistGiB[tokensId[i]])
              {} else {
                   tokenAvailable++;
              }
        ...
        function mintForGiB(uint _count) public payable{
        require(msg.sender == _owner || saleEnable, "Sale not enable");
        require(totalSupply() +_count <= MAX_TOKEN, "Exceeds limit");
        require(TokenForMintAvailable(msg.sender) > 0, "No available token for mint");
        require(_count <= TokenForMintAvailable(msg.sender), "Exceeds limit");
        require(msg.value >= basePriceGiB * _count , "Value below price");
       
        // mint counter per transaction
        uint  tokenMintLimit = 0;
        
        //check ever token whitelist
        uint256[] memory tokensId = new uint256[](GiB.balanceOf(msg.sender));
        for(uint i = 0; i < GiB.balanceOf(msg.sender); i++){
            tokensId[i] = GiB.tokenOfOwnerByIndex(msg.sender, i);
            //if not true in whitelist, to mint token
            if (_whitelistGiB[tokensId[i]])
            {} else {
