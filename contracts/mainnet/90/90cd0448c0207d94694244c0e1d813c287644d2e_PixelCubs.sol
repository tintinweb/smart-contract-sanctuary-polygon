...
// Contract paused on Deployment and Holder Only Mint
  bool public paused = true;
  bool public hodlrMint = true;
// Contract that is referenced for Discounted Cost
  address public genesisContract;
  address public colorContract;
...
// These values are set on deployment 
  constructor(
    ...
    colorContract = _colorContract;
    setBaseURI(_initBaseURI);
  }
// Checks if contract is paused, max mint amount not exceeded, if holder only mint, and determines cost for mint
  modifier mintCompliance(uint256 _mintAmount) {
    require(!paused, "CONTRACT IS PAUSED");
    require(totalMints[msg.sender] + _mintAmount <= maxMint, "MAX MINT EXCEEDED");
    uint256 _genesisBalance = IERC721(genesisContract).balanceOf(msg.sender);
    uint256 _colorBalance = IERC721(colorContract).balanceOf(msg.sender);
      if (hodlrMint == true) { require(_genesisBalance > 0 || _colorBalance > 0 , "HOLDER MINT ONLY");
      }
      ...
      } else if (_genesisBalance > 0 || _colorBalance > 0) {
...
// Updates the Contract that is referenced for Discounted Cost
  function setColorContract(address _newColorContract) public onlyOwner {
    colorContract = _newColorContract;
  }
...
// Set to False to Unpause and True to Pause
  function hodlrOnly(bool _state) public onlyOwner {
    hodlrMint = _state;
  }
// Set to False to Unpause and True to Pause
