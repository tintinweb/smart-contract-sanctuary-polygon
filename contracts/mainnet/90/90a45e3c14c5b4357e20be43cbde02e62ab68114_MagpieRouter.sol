...
contract MagpieRouter is ReentrancyGuard, Ownable, IMagpieRouter {
  ...
  function _swapBalancerV2(uint256 amountIn, Hop memory hop, address[] memory assets, address to, uint256 deadline) internal returns (uint256) {
    ...
    IAsset[] memory contractAssets = new IAsset[](assets.length);
    int256[] memory limits = new int256[](assets.length);
    for (uint256 j = 0; j < assets.length; j++) {
      contractAssets[j] = IAsset(assets[j]);
      limits[j] = j == 0 ? int256(amountIn) : int256(0);
    }

    int256[] memory deltas = IVault(amm.id).batchSwap(IVault.SwapKind.GIVEN_IN, swaps, contractAssets, funds, limits, deadline);
    int256 lastDelta = deltas[hop.path[hop.path.length - 1]];
