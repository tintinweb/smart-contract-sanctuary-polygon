...
contract Bee is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 BEE-WETH LP -> BEE pool
    uint256 public constant INITIAL_BEE_POOL_DISTRIBUTION = 672000 ether;
    ...
    /**
     * @notice Constructs the BEE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("bee.money", "BEE") {
        // Mints 1 BEE to contract creator for initial pool setup
    ...
    function _updateTaxRate(uint256 _beePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_beePrice >= taxTiersTwaps[tierId]) {
    ...
    function setBeeOracle(address _beeOracle) public onlyOperatorOrTaxOffice {
    ...
    /**
     * @notice Operator mints BEE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BEE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBeePrice = _getBeePrice();
            currentTaxRate = _updateTaxRate(currentBeePrice);
            if (currentBeePrice < burnThreshold) {
                burnTax = true;
            }
        }
