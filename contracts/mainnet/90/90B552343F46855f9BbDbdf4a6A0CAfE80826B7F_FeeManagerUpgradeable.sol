    ...
    IFeeManagerUpgradeable
{
    ...
    // default feeTokens list on destination chains
    ...
    // WidgetID => fees
    mapping(uint256 => uint256) private _widgetFeeInBps;

    // all feeTokens list on destination chains
    mapping(uint8 => address[]) private _allFeeTokens;
    ...
    event WidgetFeeUpdated(uint256 widgetID, uint256 widgetFeeInBps);
    ...
    */
     ...
     */
    function getWidgetFeeInBps(uint256 widgetID) public view virtual override returns (uint256) {
        ...
        )
    {
        ...
        uint256 widgetFeeInBps = getWidgetFeeInBps(widgetID);
        uint256 totalBpsFee = fees.lpValidatorAndProtocolFeeInBps[0] +
            fees.lpValidatorAndProtocolFeeInBps[1] +
            fees.lpValidatorAndProtocolFeeInBps[2] +
            widgetFeeInBps;
        return (fees.baseFeeInUSD, fees.swapFeeInUSD, fees.maxFeeInUSD, totalBpsFee);
    }
    ...
    */
    ...
    function setWidgetFeeInBps(uint256 widgetID, uint256 widgetFeeInBpsX1000)
        ...
        onlyRole(FEE_SETTER_ROLE)
    {
        require(widgetID != 0, "Fee Manager: Widget ID cannot be 0");
     ...
     */
    function setFeeTokenToPriceX10e9(address[] calldata feeToken, uint256[] calldata priceInUSDx10e9)
        external
        virtual
        override
        onlyRole(FEE_SETTER_ROLE)
    {
        require(feeToken.length == priceInUSDx10e9.length, "FeeManager: Array length mismatch");
        for (uint256 i = 0; i < feeToken.length; i++) {
            _feeTokenToPriceX10e9[feeToken[i]] = priceInUSDx10e9[i];
        }
    }
    ...
    */
    function setFee(
        ...
        if (!_feeTokenWhitelisted[destChainId][feeToken]) {
            ...
            _allFeeTokens[destChainId].push(feeToken);
            if (isDefault) {
    ...
    */
    function getFee(
        ...
        uint256 feeTokenToPrice = getFeeTokenToPrice(feeToken);

        require(feeTokenToPrice != 0, "Fee Manager: Fee not set!");

        uint256 widgetFeeInBps = getWidgetFeeInBps(widgetID);
        bool whitelisted = isWhitelisted(sender);
        ...
        additionalFeesInUSD += (widgetFeeInBps * transactionVolumeInUSDx1000 * 1000000000) / _feeDenominator;
        totalBps += widgetFeeInBps;
        ...
        if (totalBps != 0) {
            lpValidatorProtocolWidgetFee[3] +=
                ((feeInUSD - fees.baseFeeInUSD * 1000000000) * widgetFeeInBps) /
                totalBps;
        }

        uint256 feeInToken;

        feeInToken = ((feeInUSD * (10**9) * (10**fees.feeTokenDecimals)) / feeTokenToPrice);

        lpValidatorProtocolWidgetFee[0] = ((lpValidatorProtocolWidgetFee[0] * (10**9) * (10**fees.feeTokenDecimals)) /
            feeTokenToPrice);

        lpValidatorProtocolWidgetFee[1] = ((lpValidatorProtocolWidgetFee[1] * (10**9) * (10**fees.feeTokenDecimals)) /
            feeTokenToPrice);

        lpValidatorProtocolWidgetFee[2] = ((lpValidatorProtocolWidgetFee[2] * (10**9) * (10**fees.feeTokenDecimals)) /
            feeTokenToPrice);

        lpValidatorProtocolWidgetFee[3] = ((lpValidatorProtocolWidgetFee[3] * (10**9) * (10**fees.feeTokenDecimals)) /
            feeTokenToPrice);
    ...
    */
    function getFeeWithSwap(
        ...
        lpValidatorProtocolWidgetFee[3] +=
            ((feeInUSD - (fees.baseFeeInUSD + fees.swapFeeInUSD) * 1000000000) * widgetFeeInBps) /
            totalBps;

        uint256 feeInToken = ((feeInUSD * (10**9) * (10**fees.feeTokenDecimals)) / feeTokenToPrice);
    ...
    */
     ...
     */
    function withdrawNativeTokens(address recipient, uint256 amount)
        external
        virtual
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        require(bal >= amount, "Fee Manager: not enough tokens to withdraw");

        (bool success, ) = recipient.call{ value: bal }("");
        require(success, "Fee Manager: withdrawal failed");
    }
}
