...
contract InfiniteApeSpaceshipClub is ERC721A, Ownable {
  ...
  function addpublicPrice() internal {
    publicPrice = publicPrice * 1001e15 / 1e18;
  }
  
  function addpublicPriceBatch(uint256 _quantity) internal {
  ...
  function addPrice(uint256 inputPrice) internal pure returns (uint256) {
    return inputPrice * 1001e15 / 1e18;
  }
    ...
    callerIsUser 
  {
    ...
    require(
      totalSupply() + quantity <= amountForWhitelist,
    ...
    addpublicPriceBatch(quantity);
	addRandEquity(quantity, _tokenIDStart, IASCRandType);
  }
  ...
  function allowlistMint() external payable callerIsUser {
	...
	addpublicPrice();
	addRandEquity(1, _tokenIDStart, IASCRandType);
  }
  ...
  function addRandEquity(uint256 _quantity, uint256 _tokenIDStart, uint8 _RandType) internal {
    for (uint i = 0; i < _quantity; i++) {
      uint _Equity = getRandEquity(_tokenIDStart + i, _RandType);
	  IASCEquity[_tokenIDStart + i] = _Equity;
	  totalIASCEquity += _Equity;
    }
  }

  function getRandEquity(uint256 _tokenID, uint8 _RandType) public view returns (uint256) {
    uint D_NO = _tokenID;
    uint G_NO = _tokenID * 5 + 1000;
    uint S_NO = _tokenID * 10 + 5000;
  ...
  function getIASCEquity(uint256 _tokenID) public view returns (uint256) {
    return IASCEquity[_tokenID];
  }
  ...
  function setIASCEquity(uint8 sRandType) external onlySCOwner {
    require(sRandType == 1 || sRandType == 2, "ERC721A: RandType error");
    IASCRandType = sRandType;
  }
