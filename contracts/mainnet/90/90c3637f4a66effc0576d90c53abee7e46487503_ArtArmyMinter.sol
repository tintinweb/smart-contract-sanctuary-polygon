...
// File contracts/ArtArmyArtwork.sol
...
// File contracts/ArtArmyMinter.sol
...

{
    ...
    ArtArmyArtwork public _nftContract;

    constructor (
        string memory contractName,
        ArtArmyArtwork nftContract
    ) {
    ...
    */
    function mintCollection(uint256 amount, address to, string memory URI, address _artistWallet, uint256 _royaltyInBasisPoints) external returns (bool){
        require(address(_msgSender()) == address(owner()), "Only the owner can call this function");

        for(uint i=0; i < amount; i++){
            _nftContract.mint(to, URI, _artistWallet, _royaltyInBasisPoints);
        }
