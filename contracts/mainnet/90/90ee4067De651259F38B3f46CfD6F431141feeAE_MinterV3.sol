...
// File contracts/libraries/minterLib.sol
...
// File contracts/structs/stats.sol
...
contract MinterV3 is ERC721Enumerable, VRFConsumerBase{
    ...
    string private notRevealed = "QmNxm3ovpF43QKHwno2VFfo2hTpG9PcCaVKWbUETk5pjUa";
    ...
    function updateSplit(address[] memory _paymentsTo, uint16[] memory _shares) external onlyAdmin {
    ...
    function tokenURI(uint16 _tokenId) external view virtual returns(string memory uri){
        require(_exists(_tokenId));

        if(totalMintSupply <2000) {uri = string(abi.encodePacked(baseURI, notRevealed));}
    ...
    function splitFunds(uint256 fundsToSplit) internal {
        ...
        for(uint i=0; i<shares.length; i++){
            payable(paymentsTo[i]).transfer(fundsToSplit * shares[i]/totalShares);
        }
    }
    ...
    function resetCooldown(uint16 _raptor) external onlyGameAddress returns (bool) {
    ...
    function reward(address[] memory rewardedAddresses, uint8[] memory rewardedAmounts) external onlyAdmin {
