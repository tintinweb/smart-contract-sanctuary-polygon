 ...
 */
interface IETH1155Upgradeable is IETH165Upgradeable {
    /**
...
interface BeArtCollection721 {
    ...
    function setRoyaltyPETHentage(uint96 _royPer) external;
...
contract BeArtTrade is Initializable, OwnableUpgradeable {
    ...
    function orderPlace(
        ...
        if (
            ...
            if (_type == 721) {
            require(
                IETH721Upgradeable(_conAddress).ownerOf(tokenId) == msg.sender,
                "Not a Owner"
            );
        } else {
            require(
                IETH1155Upgradeable(_conAddress).balanceOf(
                    msg.sender,
                    tokenId
                ) > 0 ,
                "Not a Owner"
    ...
    function saleToken(
        ...
        if (ids[3] == 721) {
            IETH721Upgradeable(_conAddr).safeTransferFrom(
                from,
                msg.sender,
                ids[0]
            );
            ...
            if (IETH1155Upgradeable(_conAddr).balanceOf(from, ids[0]).sub(ids[2]) == 0) {
                if (order_place[from][ids[0]].price > 0) {
                    delete order_place[from][ids[0]];
                }
            }
            IETH1155Upgradeable(_conAddr).safeTransferFrom(
                from,
                msg.sender,
                ids[0],
                ids[2],
    ...
    function _saleToken(
        ...
        ldatas[6] = pETHent(ids[1], serviceValue).add(ids[1]);
        address create;
        ...
        if (
            ...
            IETH20Upgradeable t = IETH20Upgradeable(tokentype[bidtoken]);
    ...
    function acceptBId(
        ...
        if (ids[3] == 721) {
            ...
            if (order_place[msg.sender][ids[0]].price > 0) {
             ...
             if (
                IETH1155Upgradeable(_conAddr).balanceOf(msg.sender, ids[0]).sub(ids[2]) == 0
            ) {
                if (order_place[msg.sender][ids[0]].price > 0) {
                    delete order_place[msg.sender][ids[0]];
                }
    ...
    function SetRoyalty(uint96 _royPer) public onlyOwner{
        BeArtCollection721(BeArt721).setRoyaltyPETHentage(_royPer);
        BeArtCollection1155(BeArt1155).setRoyaltyPETHentage(_royPer);
    }
