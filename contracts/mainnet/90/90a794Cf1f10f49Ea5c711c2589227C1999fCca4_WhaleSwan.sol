...
contract WhaleSwan is ERC721 {
    uint256 public totalSupply;
    uint256 public cost = 0.07 ether;
    uint256 public maxMints = 233;
    ...
    constructor()
        ERC721("Whale Swan", "WS")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 233) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://QmNoVtBsvJLWjjizG3DLQbBoKz1EG14SXjUdXMsbenzuGi/", Strings.toString(tokenId), ".json")
            );
    }
