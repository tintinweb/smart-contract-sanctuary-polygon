...
contract Bank is Pausable , Ownable  , AccessControl {
    ...
    function addPurchaseRecord (  address _buyer ,  uint256 _indexId , uint256 _purchaseValue , uint256 _purchaseDate , uint256 _amount , uint256[] memory _assetIdList , uint256[] memory _amountAsset , uint256[] memory _weightList  ) public onlyAdmin returns ( uint256 )
    {
        ...
        emit  PurchasedAssets ( _buyer , purchasedIndexInfo.purchaseId , purchasedIndexInfo.indexId  , _assetIdList  , _amountAsset  , _weightList );
...
contract MainController is Pausable , Ownable  , AccessControl {
    ...
    function buyIndex ( uint256 _indexId , uint256 amount ) public whenNotPaused  payable 
    {
        ...
        if ( automatic == true )
        {
            ...
            bankManager.addPurchaseRecord ( msg.sender , _indexId , amountAfterFee ,  block.timestamp , indexTokenAmount , assetConstituent.assetIdList , assetBoughtAmount , assetConstituent.weightList );
    ...
    function sellIndex ( uint256 _indexId , uint256 amount ) public whenNotPaused  payable 
    {
        ...
        require ( amount <= indexTokenV1.balanceOf(msg.sender) , "Insufficient Balance" );
        ...
        for ( uint256 i = 0 ; i < indexManager.getIndexAssetCount(  _indexId ) ; i++ )
        {
            ...
            uint256 amountIn = ( amount  ).mul( assetAmount ).div(  indexTokenV1.balanceOf(msg.sender)   )  ;
            ...
            ISwapRouter.ExactInputSingleParams memory params = ISwapRouter
                .ExactInputSingleParams({
                    tokenIn: assetsOfIndex[i].tokenAddress ,
                    tokenOut: USDC.tokenAddress ,
                    fee: _poolFee,
                    recipient: address(this),
                    deadline: block.timestamp,
                    amountIn: amountIn,
            ...
            uint256 remainder = sum.sub ( sum.mul( feePercent ).div(10000)    ) ;
            emit SellLog ( msg.sender  , amount , _indexId , assetConstituent.assetIdList  , assetSoldAmount  ,  sum , sum.mul( feePercent ).div(10000) );
