...
contract AlloyxOperator is IAlloyxOperator, AdminUpgradeable {
  ...
  /**
   * @notice If called from vault
   */
  modifier fromVault() {
    require(config.getManager().isVault(msg.sender), "only vault address can call");
    _;
  }
   ...
   */
  function getTotalBalanceInUsdc(address _vaultAddress) public view override returns (uint256) {
    uint256 result = 0;
    // Using block scoping to avoid error `Stack too deep, try removing local variables.`
    {
      result = config
        .getUSDC()
        .balanceOf(_vaultAddress)
        .add(config.getGoldfinchDesk().getFiduBalanceInUsdc(_vaultAddress))
        .add(config.getGoldfinchDesk().getGoldFinchPoolTokenBalanceInUsdc(_vaultAddress))
        .add(config.getTruefiDesk().getTruefiWalletUsdcValue(_vaultAddress))
        .add(config.getClearPoolDesk().getClearPoolWalletUsdcValue(_vaultAddress))
        .add(config.getRibbonDesk().getRibbonWalletUsdcValue(_vaultAddress));
    }

    {
      result = result
        .add(config.getRibbonLendDesk().getRibbonLendWalletUsdcValue(_vaultAddress))
        .add(config.getMapleDesk().getMapleWalletUsdcValue(_vaultAddress))
        .add(config.getCredixDesk().getCredixWalletUsdcValue(_vaultAddress))
        .add(config.getFluxDesk().getFluxBalanceInUsdc(_vaultAddress))
        .add(config.getBackedDesk().getBackedTokenValueInUsdc(_vaultAddress));
    }
    return result;
  }
  ...
  function getAdditionalDepositAmount(
    ...
    if (_source == Source.GOLDFINCH) {
      investedAmount = config.getGoldfinchDesk().getGoldFinchPoolTokenBalanceInUsdcForPool(vaultAddress, _poolAddress, _tranche);
    } else if (_source == Source.FIDU) {
      investedAmount = config.getGoldfinchDesk().getFiduBalanceInUsdc(vaultAddress);
    } else if (_source == Source.TRUEFI) {
      investedAmount = config.getTruefiDesk().getTruefiWalletUsdcValueOfPortfolio(vaultAddress, _poolAddress);
    } else if (_source == Source.RIBBON) {
      investedAmount = config.getRibbonDesk().getRibbonUsdcValueOfVault(vaultAddress, _poolAddress);
    } else if (_source == Source.RIBBON_LEND) {
      investedAmount = config.getRibbonLendDesk().getRibbonLendUsdcValueOfPoolMaster(vaultAddress, _poolAddress);
    } else if (_source == Source.CLEAR_POOL) {
      investedAmount = config.getClearPoolDesk().getClearPoolUsdcValueOfPoolMaster(vaultAddress, _poolAddress);
    } else if (_source == Source.MAPLE) {
      investedAmount = config.getMapleDesk().getMapleWalletUsdcValueOfPool(vaultAddress, _poolAddress);
    } else if (_source == Source.CREDIX) {
      investedAmount = config.getCredixDesk().getCredixWalletUsdcValue(vaultAddress);
    } else if (_source == Source.FLUX) {
      investedAmount = config.getFluxDesk().getFluxBalanceInUsdc(vaultAddress);
    } else if (_source == Source.BACKED) {
      investedAmount = config.getBackedDesk().getBackedTokenValueInUsdc(vaultAddress);
    } else if (_source == Source.USDC) {}
    uint256 targetAmount = _proportion.mul(_investableUsdc).div(10000);
    if (targetAmount > investedAmount) {
      return targetAmount.sub(investedAmount);
    }
    return 0;
  }
  ...
  function performDeposit(
    ...
    address vaultAddress = msg.sender;
    if (_source == Source.GOLDFINCH) {
      _transferERC20(config.usdcAddress(), config.goldfinchDeskAddress(), _amount);
      config.getGoldfinchDesk().purchasePoolToken(vaultAddress, _amount, _poolAddress, _tranche);
    } else if (_source == Source.FIDU) {
      _transferERC20(config.usdcAddress(), config.goldfinchDeskAddress(), _amount);
      config.getGoldfinchDesk().purchaseFIDU(vaultAddress, _amount);
    } else if (_source == Source.TRUEFI) {
      _transferERC20(config.usdcAddress(), config.truefiDeskAddress(), _amount);
      config.getTruefiDesk().depositToTruefi(vaultAddress, _poolAddress, _amount);
    } else if (_source == Source.MAPLE) {
      _transferERC20(config.usdcAddress(), config.mapleDeskAddress(), _amount);
      config.getMapleDesk().depositToMaple(vaultAddress, _poolAddress, _amount);
    } else if (_source == Source.RIBBON) {
      _transferERC20(config.usdcAddress(), config.ribbonDeskAddress(), _amount);
      config.getRibbonDesk().deposit(vaultAddress, _poolAddress, _amount);
    } else if (_source == Source.RIBBON_LEND) {
      _transferERC20(config.usdcAddress(), config.ribbonLendDeskAddress(), _amount);
      config.getRibbonLendDesk().provide(vaultAddress, _poolAddress, _amount);
    } else if (_source == Source.CLEAR_POOL) {
      _transferERC20(config.usdcAddress(), config.clearPoolDeskAddress(), _amount);
      config.getClearPoolDesk().provide(vaultAddress, _poolAddress, _amount);
    } else if (_source == Source.CREDIX) {
      _transferERC20(config.usdcAddress(), config.credixDeskAddress(), _amount);
      config.getCredixDesk().increaseUsdcValueForPool(vaultAddress, _amount);
    } else if (_source == Source.FLUX) {
      _transferERC20(config.usdcAddress(), config.fluxDeskAddress(), _amount);
      config.getFluxDesk().mint(vaultAddress, _amount);
    } else if (_source == Source.BACKED) {
      _transferERC20(config.usdcAddress(), config.backedDeskAddress(), _amount);
      config.getBackedDesk().deposit(vaultAddress, _amount);
    } else if (_source == Source.USDC) {
      _transferERC20(config.usdcAddress(), vaultAddress, _amount);
    }
  }
  ...
  function performWithdraw(
    ...
    address vaultAddress = msg.sender;
    if (_source == Source.GOLDFINCH) {
      config.getGoldfinchDesk().withdrawFromJuniorToken(vaultAddress, _tokenId, _amount, _poolAddress);
    } else if (_source == Source.FIDU) {
      config.getGoldfinchDesk().sellFIDU(vaultAddress, _amount);
    } else if (_source == Source.TRUEFI) {
      config.getTruefiDesk().withdrawFromTruefi(vaultAddress, _poolAddress, _amount);
    } else if (_source == Source.MAPLE) {
      if (_step == WithdrawalStep.INIT) {
        config.getMapleDesk().requestWithdraw(vaultAddress, _poolAddress, _amount);
      } else if (_step == WithdrawalStep.COMPLETE) {
        config.getMapleDesk().withdrawFromMaple(vaultAddress, _poolAddress, _amount);
      }
    } else if (_source == Source.RIBBON) {
      if (_step == WithdrawalStep.DEFAULT) {
        config.getRibbonDesk().withdrawInstantly(vaultAddress, _poolAddress, _amount);
      } else if (_step == WithdrawalStep.INIT) {
        config.getRibbonDesk().initiateWithdraw(vaultAddress, _poolAddress, _amount);
      } else if (_step == WithdrawalStep.COMPLETE) {
        config.getRibbonDesk().completeWithdraw(vaultAddress, _poolAddress);
      }
    } else if (_source == Source.RIBBON_LEND) {
      config.getRibbonLendDesk().redeem(vaultAddress, _poolAddress, _amount);
    } else if (_source == Source.CLEAR_POOL) {
      config.getClearPoolDesk().redeem(vaultAddress, _poolAddress, _amount);
    } else if (_source == Source.FLUX) {
      config.getFluxDesk().redeem(vaultAddress, _amount);
    } else if (_source == Source.USDC) {
      return;
    }
  }
