    ...
    AccessControlFacet
{
    ...
    event IdentityMinted(address account, uint256 tokenId, string identityHash);

    constructor() {
        _disableInitializers();
    }

    function initialize(uint256 mintPrice_, address owner) public initializer {
        __ERC721_init("Block'd Indentity NFTS", "BLKD");
    ...
    function mint(string calldata identityHash) external payable nonReentrant {
        require(mintPrice > 0 && msg.value == mintPrice, "Value != mint price");
        require(bytes(identityHash).length > 0, "Empty identity hash");
        ...
        payable(_getOwner()).sendValue(msg.value);

        emit IdentityMinted(msg.sender, tokenId, identityHash);
    }

    function setMintPrice(uint256 mintPrice_) external {
        LibAccessControl.enforceRole(PRICE_UPDATER_ROLE);
