...
/// @dev overloaded from above
function _prepareVault(Deployer _d, uint256 _userDepositLimits, uint256 _vaultDepositLimits) {
    require(_userDepositLimits < _vaultDepositLimits, "PrepareVault::USER DEPOSIT LIMITS TOO HIGH");
    // unpause the vault
    _d.vaultProxy().triggerPause();
...
contract XChainHubSingle is XChainHub {
    ...
    function setStrategyForChain(address _strategy, uint16 _chain) external onlyOwner {
    ...
    function _setStrategyForChain(address _strategy, uint16 _remoteChainId) internal {
        
        // require the strategy is fully withdrawn before calling
        address currentStrategyForChain = strategyForChain[_remoteChainId];
        require(
            exitingSharesPerStrategy[_remoteChainId][currentStrategyForChain] == 0 && sharesPerStrategy[_remoteChainId][currentStrategyForChain] == 0,
    ...
    /// @notice external setter callable by the owner
    function setVaultForChain(address _vault, uint16 _remoteChainId) external onlyOwner {
    ...
    function _setVaultForChain(address _vault, uint16 _remoteChainId) internal {
        ...
        IVault.BatchBurnReceipt memory receipt = vault.userBatchBurnReceipts(strategy);

        require(vault.balanceOf(strategy) == 0 && receipt.shares == 0, "XChainHub::setVaultForChain:NOT EMPTY");
    ...
    function _sg_depositAction(uint16 _srcChainId, bytes memory, uint256 _amountReceived) internal override {
        _makeDeposit(_srcChainId, _amountReceived, strategyForChain[_srcChainId], vaultForChain[_srcChainId]);
    }
}
...
contract Vault is ERC20, Pausable {
    ...
    /// @param harvestFeeReceiver_ The harvesting fee receiver address.
    /// @param burnFeeReceiver_ The batched burns fee receiver address.
    function initialize(ERC20 underlying_, Authority auth_, address harvestFeeReceiver_, address burnFeeReceiver_)
        external
        initializer
    {
    ...
    function harvest(IStrategy[] calldata strategies) external requiresAuth(msg.sender) {
        ...
        estimatedReturn =
            computeEstimatedReturns(strategyHoldings, uint256(maxLockedProfit_), lastHarvestIntervalInBlocks);
    ...
    function computeEstimatedReturns(uint256 invested, uint256 profit, uint256 interval)
        internal
        view
        returns (uint256)
    {
...
function getDeployers() pure returns (Deployers memory) {
    return Deployers({
        avax: address(0),
        polygon: 0x40903cf2D1Bf34d41d8E315185f29329E49acfa7,
        optimism: address(0),
...
function getDeployers_test() pure returns (Deployers memory) {
    return Deployers({
        avax: 0xE6489A6a6D85e5BCC2CE0f64BF76cA073892E344,
        polygon: address(0),
        optimism: address(0),
        arbitrum: address(0),
...
function getChains() pure returns (Chains memory) {
    ChainConfig memory optimism = ChainConfig({
        ...
        usdc: StargateToken({addr: 0x7F5c764cBc14f9669B88837ca1490cCa17c31607, poolId: 1})
    });
    ...
    ChainConfig memory arbitrum = ChainConfig({
        ...
        usdc: StargateToken({addr: 0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8, poolId: 1})
    });
    ...
    ChainConfig memory polygon = ChainConfig({
        ...
        usdc: StargateToken({addr: 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174, poolId: 1})
    });
    ...
    ChainConfig memory avax = ChainConfig({
        ...
        lz: 0x3c2269811836af69497E5F486A85D7316753cf62,
        sg: 0x45A01E4e04F14f7A4a6702c74187c5F6222033cd,
        usdc: StargateToken({addr: 0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E, poolId: 1})
    });
    ...
    ChainConfig memory fantom = ChainConfig({
        ...
        usdc: StargateToken({addr: 0x04068DA6C83AFCFA0e13ba15A6696662335D5B75, poolId: 1})
    });

    return Chains({optimism: optimism, arbitrum: arbitrum, polygon: polygon, avax: avax, fantom: fantom});
}
...
function getChains_test() pure returns (Chains memory) {
    ChainConfig memory optimismKovan = ChainConfig({
        ...
        usdc: StargateToken({addr: 0x567f39d9e6d02078F357658f498F80eF087059aa, poolId: 1})
    });
    ...
    ChainConfig memory arbitrumRinkeby = ChainConfig({
        ...
        usdc: StargateToken({addr: 0x1EA8Fb2F671620767f41559b663b86B1365BBc3d, poolId: 1})
    });
    ...
    ChainConfig memory polygonMumbai = ChainConfig({
        ...
        usdc: StargateToken({addr: 0x742DfA5Aa70a8212857966D491D67B09Ce7D6ec7, poolId: 1})
    });
    ...
    ChainConfig memory avaxFuji = ChainConfig({
        ...
        usdc: StargateToken({addr: 0x4A0D1092E9df255cf95D72834Ea9255132782318, poolId: 1})
    });
    ...
    ChainConfig memory ftmTest = ChainConfig({
        ...
        usdc: StargateToken({addr: 0x076488D244A73DA4Fa843f5A8Cd91F655CA81a1e, poolId: 1})
    });
