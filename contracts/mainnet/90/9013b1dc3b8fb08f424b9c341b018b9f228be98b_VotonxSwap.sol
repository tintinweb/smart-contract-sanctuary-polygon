...
/**
 *Submitted for verification at cronoscan.com on 2023-07-01
*/

/**
 *Submitted for verification at cronoscan.com on 2023-06-16
*/
...
contract VotonxSwap is Ownable {
    ...
    function customSwapTokens(address _tokenA, address _tokenB, uint256 _amountIn, address[]memory path) external payable{
        if (_tokenA == address(0)) {
            require(weth == path[0] && _tokenB == path[path.length - 1], "ShinuSwap: Path must be specified");
        } else if (_tokenB == address(0)) {
            require(_tokenA == path[0] && weth == path[path.length - 1], "ShinuSwap: Path must be specified");
        } else {
            require(_tokenA == path[0] && _tokenB == path[path.length - 1], "ShinuSwap: Path must be specified");
        }
        _proceedToUniswap(_tokenA, _tokenB, _amountIn, path);
    }

    function _proceedToUniswap(address _tokenA, address _tokenB, uint256 _amountIn, address[]memory path) internal {
        require(_amountIn > 0, "ShinuSwap: amountIn must be greater than 0");
        
        if (_tokenA == address(0)) {
            require (_amountIn >= msg.value, "ShinuSwap: Insufficient ETH");
