...
contract SmartShopper is Pausable, Ownable, ReentrancyGuard {
    ...
    event AllOrdersClaimed(string[] claimedOrders, uint256 amountToClaim);
    ...
    function claimOrder(
        ...
        USDC.transfer(_msgSender(), claimedAmount);
    ...
    function claimAll() external nonReentrant onlyOwner {
        ...
        string[] memory claimedOrders = new string[](ordersIdsValue.length);
        ...
        for (uint256 i = 0; i < ordersIdsValue.length; i++) {
            ...
            if (!order.returned) {
                if (order.lockEndTime > block.timestamp) {
                    continue;
                }
            }
            if (!order.claimed) {
                if (order.lockEndTime <= block.timestamp) {
                    ...
                    amountToClaim = amountToClaim + currentAmountToClaim;
                    order.claimed = true;
                    claimedOrders[i] = currentOrderId;
                }
            }
        }

        USDC.transfer(_msgSender(), amountToClaim);

        emit AllOrdersClaimed(claimedOrders, amountToClaim);
    }
    ...
    function processRefund(
        ...
        USDC.transfer(order.buyer, returnedAmount);
