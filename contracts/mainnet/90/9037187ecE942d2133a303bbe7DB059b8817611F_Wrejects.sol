...
contract Wrejects is Ownable, ERC1155Supply, ERC2981 {
    using SafeMath for uint256;
    mapping(address => bool) private adminAccess;
    event SetAdminAccess(address indexed user, bool access);

    constructor( string memory uri, address feeReceiver, uint96 feeNumerator) ERC1155(uri) {
        _setDefaultRoyalty(feeReceiver, feeNumerator);
    }
    function setRoyalty(address feeReceiver, uint96 feeNumerator) external onlyAdminAccess {
        _setDefaultRoyalty(feeReceiver, feeNumerator);
    }
    function setURI(string memory newuri) external onlyAdminAccess {
        _setURI(newuri);
    }
    function mintItems(
        uint256 itemId,
    ...
    function setRoyalty(string memory newuri) external onlyAdminAccess {
        _setURI(newuri);
    }
