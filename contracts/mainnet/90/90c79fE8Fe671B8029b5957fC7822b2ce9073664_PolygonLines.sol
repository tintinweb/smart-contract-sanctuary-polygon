...
contract PolygonLines is DynamicNftUriGetter {
    ...
    function _shape(bytes32 seed) private pure returns(string memory) {
        ...
        return string(
            abi.encodePacked(
                "<rect fill='hsl(",(uint256(seed) % 360).toString(),",60%,55%)' width='",(uint128(xy[1]) % 256).toString(),"' height='100%' x='",(uint128(xy[0]) % 2048).toString(),"'/>"
            )
        );
    }
    ...
    function _svg() private view returns(string memory) {
        ...
        return string(
            abi.encodePacked(
                "<svg width='2048' height='2048' viewPort='0 0 2048 2048' style='background:#fff' xmlns='http://www.w3.org/2000/svg'>",
                "<style>rect{transform:translate(-256px,0)}</style>",
                shapes,
                "</svg>"
            )
        );
        ...
        return string(
                abi.encodePacked(
                    ...
                    Base64.encode(string(abi.encodePacked(
                        '{"name":"Polygon Lines",', 
                        '"description":"A dynamic NFT changing based on 16 most recent blocks on the chain.\\n',
                            'Every line is representing a block, positioned and colored based on the block hash.\\n\\n',
