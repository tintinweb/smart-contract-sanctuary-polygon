...
contract FeeSplitter is TokensRecoverable
{
    ...
    //todo update below
    function setChainTokenFeeCollectors(IGatedERC20 token, address[5] memory collectors, uint16[5] memory rates) public onlyRoyaltyCollector() // 100% = 10000
    {
    ...
    function setRootedTokenFeeCollectors(IGatedERC20 token, address[5] memory collectors, uint16[5] memory rates) public onlyRoyaltyCollector() // 100% = 10000
    {
        require (collectors.length == rates.length, "Fee Collectors and Rates must be the same size");
