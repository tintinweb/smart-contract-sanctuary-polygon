...
contract NFTProxy is Ownable, Pausable {
    IERC20 public USDC;
    ...
    event Deposit(address indexed sender, uint256 amount, uint256 orderId);
    ...
    constructor(address _usdc, string memory _name) {
        USDC = IERC20(_usdc);
        name = _name;
        receiver = msg.sender;
	...
	function deposit(address account, uint256 amount, uint256 orderId) public whenNotPaused onlyWhitelisted(msg.sender) {

		USDC.transferFrom(msg.sender, receiver, amount);

        emit Deposit(account, amount, orderId);
	}
}