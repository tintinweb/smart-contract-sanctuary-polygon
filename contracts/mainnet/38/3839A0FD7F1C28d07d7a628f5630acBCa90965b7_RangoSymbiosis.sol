...
contract RangoSymbiosis is IRangoSymbiosis, BaseInterchainContract {
    ...
    function symbiosisBridge(
        ...
        if (_request.bridgeType == SymbiosisBridgeType.META_BURN) {
            otherSideCalldata = abi.encodeWithSelector(metaBurnSyntheticTokenSelector, metaBurnData(_request));
        } else {
            otherSideCalldata = abi.encodeWithSelector(metaSynthesizeSelector, metaSynthesizeData(_request));
        }
