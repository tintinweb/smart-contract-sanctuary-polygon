...
import {ResultValidator} from "../helpers/ResultValidator.sol";
import {EnumOracleCode} from "../utils/BetEnums.sol";
import {Converter} from "../utils/Converter.sol";
import "../utils/BetErrors.sol";
import "../utils/BetConstants.sol";

/**
 * @title The Chainlink Oracle Comsumer Contract
 * @notice Fetch match result from Oracle
 */
contract OracleConsumer is
    IOracleConsumer,
    ChainlinkClient,
    AccessControl,
    ResultValidator
{
    ...
    string private matchResultJobId;

    mapping(uint256 => int256[]) private matchKeyToResult;
    mapping(bytes32 => uint256) private requestIdToMatchKey;

    constructor(address chainlinkTokenContract, address oracleContract) {
        setChainlinkToken(chainlinkTokenContract);
        setChainlinkOracle(oracleContract);
    ...
    {
        return matchKeyToResult[matchKey];
    }
    ...
    function requestMatchResult(uint256 matchKey, bool enforce)
        external
        override
        onlyRole(CONTRACTOR_ROLE)
        returns (bytes32 requestId)
    {
        if (matchKeyToResult[matchKey].length > 0 && !enforce) {
            revert MatchResultAlreadyExisted(matchKey);
        } else {
            Chainlink.Request memory request = buildChainlinkRequest(
                matchResultJobId.toBytes32(),
                address(this),
                this.fulfillMatchResult.selector
            );
            request.add("matchKey", Strings.toString(matchKey));
            request.add("path", "");
            requestId = sendChainlinkRequest(request, ORACLE_PAYMENT);
            requestIdToMatchKey[requestId] = matchKey;
            emit LogOracleMatchResultRequested(requestId, matchKey);
        }
    }

    /**
     * @notice Chainlink callback function to deal with match result
     * @param requestId chainlink requestId
     * @param oracleResult oracle match result
     */
    function fulfillMatchResult(bytes32 requestId, uint256 oracleResult)
        public
        recordChainlinkFulfillment(requestId)
    {
        uint256 matchKey = requestIdToMatchKey[requestId];
        (
            EnumOracleCode oracleCode,
            int256[] memory matchResult
        ) = validateOracleResult(matchKey, oracleResult);
        matchKeyToResult[matchKey] = matchResult;
        delete requestIdToMatchKey[requestId];
        emit LogOracleMatchResultFulfilled(
            requestId,
            matchKey,
            oracleCode,
            matchResult
    ...
    /**
     * @notice Set chainlink jobId
     * @param externalJobId  The Job Specification ID that the request will be created for
     */
    function setMatchResultJobId(string memory externalJobId)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        matchResultJobId = externalJobId;
    }
...
contract ResultValidator {
    ...
    function validateOracleResult(uint256 matchKey, uint256 oracleResult)
        internal
        pure
        returns (EnumOracleCode oracleCode, int256[] memory matchResult)
    {
        (uint256 resultString, uint256 code) = oracleResult.decode(
            DIGITS_ORACLE_CODE
        );
        oracleCode = EnumOracleCode(code);

        (uint256 assertIdx, ) = matchKey.decode(DIGITS_ASSERT_IDX_MASK);

        if (oracleCode == EnumOracleCode.SUCCESS) {
            (uint256 unitCount, ) = resultString.countUnits(
                MATCH_RESULT_UNIT_LENGTH
            );
            uint256 item;

            if (assertIdx == 10) {
                matchResult = new int256[](unitCount);
            } else {
                matchResult = new int256[](2);
            }
            for (uint256 i = 0; i < unitCount; i++) {
                (resultString, item) = resultString.decode(DIGITS_MATCH_RESULT);
                matchResult[i] = int256(item);
            }
        } else if (oracleCode == EnumOracleCode.MATCH_CANCELED) {
            matchResult = new int256[](1);
            matchResult[0] = -1;
        }
    }
}
 ...
 */
library Converter {
    function toBytes32(string memory value)
        internal
        pure
