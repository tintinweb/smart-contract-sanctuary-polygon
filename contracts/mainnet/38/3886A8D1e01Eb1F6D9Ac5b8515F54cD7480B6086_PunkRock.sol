...
contract PunkRock is ERC721 {
    uint256 public totalSupply;
    uint256 public cost = 0 ether;
    uint256 public maxMints = 32;
    ...
    constructor()
        ERC721("Punk Rock NFT", "PRN")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 2266) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeignuix2ju374lovaouekgrg3l4bgaqsrz2f77bw6pqpmew6pdzseu/", Strings.toString(tokenId), ".json")
            );
    }
