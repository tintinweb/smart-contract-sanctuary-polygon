...
contract BaseTournamentService is BaseService, ITournamentService {
  ...
  // reviewed
  function forceWithdrawFee(address _currency, address _account, uint256 _amount) external onlyRoler("forceWithdrawFee") {
    IERC20(_currency).transfer(_account, _amount);
  }

  // reviewed
  function updateCurrentTournamentID(uint64 _id) external onlyRoler("updateCurrentTournamentID") {
    currentTournamentID = _id;
  }
}
