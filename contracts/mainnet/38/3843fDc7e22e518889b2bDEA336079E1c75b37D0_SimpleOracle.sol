...
contract SimpleOracle {
    ...
    returns (uint256)
  {
    ...
    */
    ...
    uint256 paymentTokenPrice = paymentWETHReserves / paymentTokenReserves;
    uint256 tbnTokenPrice = tbnWETHReserves / tbnTokenReserves;
    // if decimals are the same
    //   return contained / payment
    // else
    if (tbnTokenDecimals == paymentTokenDecimals) {
      return tbnTokenPrice / paymentTokenPrice;
      //   if tbn decimals is bigger than payment decimals
      //     get decimal difference (tbn decimals - paymentDecimals)
      //     return (10 ^ decimalDiff * (wmatic / tbn )) / (wmatic / payment)
    } else if (tbnTokenDecimals > paymentTokenDecimals) {
      uint8 decimalDiff = tbnTokenDecimals - paymentTokenDecimals;
      return (((10**decimalDiff) * tbnTokenPrice) / paymentTokenPrice);
      //   else if (aka payment decimals are bigger than tbn decimals)
      //     get decimal difference (paymentDecimals - tbn decimals)
      //     return (contained) / ((payment) * 10 ^ decimal diff)
    } else {
      uint8 decimalDiff = paymentTokenDecimals - tbnTokenDecimals;
      return (tbnTokenPrice / ((10**decimalDiff) * paymentTokenPrice));
    }
  }
}
