...
interface ILayerZeroEndpoint is ILayerZeroUserApplicationConfig {
    ...
    function outboundNonce(uint16 _chainId, address _userApplication) external view returns (uint64);
}
...
abstract contract LzApp is OwnableUpgradeable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    function _lzSend(uint16 _dstChainId, bytes memory _payload, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams, uint _nativeFee) internal virtual returns (uint64) {
        bytes memory trustedRemote = trustedRemoteLookup[_dstChainId];
        require(trustedRemote.length != 0, "LzApp: destination chain is not a trusted source");
        lzEndpoint.send{value : _nativeFee}(_dstChainId, trustedRemote, _payload, _refundAddress, _zroPaymentAddress, _adapterParams);
        return lzEndpoint.outboundNonce(_dstChainId, address(this));
    }
