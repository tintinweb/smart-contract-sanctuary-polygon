...
contract AdvancedTransfer is BaseSiloAction {
    ...
    function transferAction(
        ...
        if (info.beta > 0 && info.path.length > 1) {
            ...
            for (uint256 i = 0; i < (info.path.length - 1); i++) {
                path[0] = info.path[i];
                path[1] = info.path[i + 1];
                best = info.Oracle.findOptimalSwap(
                    info.path[i],
                    info.path[i + 1],
                    amount
                );
                if (best.amountOut > 0) {
                    ...
                    amounts = IUniswapV2Router02(best.router)
            ...
            amounts[0] =
                inputAmounts[0] -
                ((inputAmounts[0] * info.beta) / baseDecimals); //remaining input[0]
        ...
        returns (bool)
    {
        ...
        address[4] memory output;
        address[4] memory recipients;
        (input, output, ratio, recipients) = abi.decode(
            configData,
            (address[4], address[4], uint256[4], address[4])
        );
        uint256 sum;
        for (uint256 i = 0; i < 4; i++) {
            sum += ratio[i];
