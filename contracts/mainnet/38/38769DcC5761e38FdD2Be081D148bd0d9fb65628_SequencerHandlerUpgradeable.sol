    ...
    ISequencerHandler
{
    ...
    function unsupportChain(uint8 _destChainId, bool _shouldUnsupport) public virtual {
        _unsupportedChains[_destChainId] = _shouldUnsupport;
    }

    /// @notice Pauses certain functions of the contract.
    /// @dev Only the wallets with PAUSER_ROLE can pause the contract
    function pause() public virtual onlyRole(PAUSER_ROLE) whenNotPaused {
        _pause();
    }

    /// @notice UnPauses the functions of the contract.
    /// @dev Only the wallets with PAUSER_ROLE can unpause the contract
    ...
    function genericDepositWithERC(
        ...
        address _feeToken,
        bool _isTransferFirst
    ) external virtual override notUnsupportedChain(_destChainID) nonReentrant whenNotPaused returns (uint64) {
    ...
    function genericDepositWithETH(
        ...
        address _feeToken,
        bool _isTransferFirst
    ) external payable virtual override notUnsupportedChain(_destChainID) nonReentrant whenNotPaused returns (uint64) {
        require(defaultGas[_destChainID] != 0, "Router Sequencer: Destination gas not set");
        require(defaultGasPrice[_destChainID] != 0, "Router Sequencer: Destination gas price not set");
        ...
        returns (bool)
    {
        ...
        require(executeRecords.isExecuted == false, "Router Generichandler : Deposit record already handled");
