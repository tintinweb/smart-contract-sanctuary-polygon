...
contract WhoopyFactory {
	...
	function createClone() onlyOwner external returns(address instance) { 
...
/**
 * @title WhoopyP2A Contract Implementation
 * @author Whoopy
 *
 */
...
contract Whoopy is Initializable, VRFConsumerBaseV2, KeeperCompatibleInterface, ReentrancyGuard {
    ...
    function performUpkeep(
        ...
        if (s_players.length < 1000 || maxWinners < 3 || maxWinners > 5) { 
            if (preloadAmount > 0) {
...
//we will enable emergency mode and allow players to withdraw their balances.
    ...
    function withdrawPreload() emergency public {
        (bool success, ) = payable(contractOwner).call{value: preloadAmount * 1000000000000000000}("");
