...
contract Battlezone is IBattlezone, WarpBase, ReentrancyGuardUpgradeable {
    ...
    */
    function addStarship(uint256 _starshipId, uint256 _battleBid) external notContract {
        ...
        require(_battleBid >= 10 && _battleBid % 10 == 0, 'Battle bid must be divisible by 10');
        ...
        defender.battleBid = _battleBid; //multiply by 10 as only multiplier of 10 are allowed;
        defender.openForBattle = 1;

        // Ensure battle bid is sufficient and then add to list and transfer dai from defender.
        require(defender.battleBid <= defender.strength / 10, 'Battle bid too high'); //starship max bid is 10% of their current strength
        starshipsList.add(_starshipId); //add to Starships List
        IERC20(dai).safeTransferFrom(msg.sender, address(this), defender.battleBid * minBidFactor); //pay the battle bid
    ...
    */
    function removeStarship(uint256 _starshipId) external nonReentrant notContract {
        ...
        IERC20(dai).safeTransfer(msg.sender, defender.battleBid * minBidFactor); //pay back the battle bid
    ...
    */
        ...
        notContract
    {
        ...
        IERC20(dai).safeTransferFrom(msg.sender, address(this), battleBid * minBidFactor); //pay the battle bid
        ...
        opponentFirePower = (opponentFirePower * 102) / 100;
    ...
    */
    function engageInBattleAdvanced(
        ...
        if (attacker.inFightWith == 0) {
            ...
            IERC20(dai).safeTransferFrom(
                msg.sender,
                address(this),
                defender.battleBid * minBidFactor
            ); //pay the battle bid
    ...
    function fireShot(
        ...
        uint256 opponentShieldOnPart = get1of(5); //60% of the time shield will be on weakest part
        if ((opponentShieldOnPart == 3) || (opponentShieldOnPart == 4)) {
    ...
    */
        ...
        notContract
    {
        ...
        require(IStarship(starships).ownerOf(_starshipId) == msg.sender, 'Not owner of starship!');
        require(
            _newBattleBid >= 10 && _newBattleBid % 10 == 0,
            'Battle bid must be divisible by 10'
        );
        require(_newBattleBid <= battleStarships[_starshipId].strength / 10, 'Battle bid too high'); //starship max bid is 10% of their current strength
        ...
        if (_newBattleBid > battleStarships[_starshipId].battleBid) {
            ...
            IERC20(dai).safeTransferFrom(msg.sender, address(this), bidDifference * minBidFactor);
        } else {
            //new bid is lower, need to refund the difference
            bidDifference = battleStarships[_starshipId].battleBid - _newBattleBid;
            battleStarships[_starshipId].battleBid = _newBattleBid;
            IERC20(dai).safeTransfer(msg.sender, bidDifference * minBidFactor);
        }
        //TO DO emit event
    }
    ...
    */
    function callWinner(
        ...
        uint256 partDamage = _battleBid / 10 / 3;
        if (partDamage < 1) partDamage = 1; // always ensure atleast one dmg
        ...
        if (_winner) {
            ...
            attacker.exp += ((_battleBid / 10) * 8); //80% battle bid exp for winner
            ...
            defender.exp += ((_battleBid / 10) * 2); //20% battle bid exp for loser
            ...
            defender.exp += ((_battleBid / 10) * 8); //80% battle bid exp
            ...
            attacker.exp += ((_battleBid / 10) * 2); //20% battle bid exp for loser
            //set winner
        ...
        // payment
        IERC20(dai).safeTransfer(winnerAddress, battlePayment * minBidFactor);
        IERC20(dai).safeTransfer(growth, growthFee * minBidFactor);
    }
    ...
    function clearBattle(uint256 _starshipId) internal {
        battleStarships[_starshipId].fights += 1;
