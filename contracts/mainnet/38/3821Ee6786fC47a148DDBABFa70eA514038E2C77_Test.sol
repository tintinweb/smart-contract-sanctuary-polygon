...
contract Test {
    ...
    function _redeemAndSendPXLT(uint256 setAmount) internal returns(uint256) {
        (address[] memory components, uint256[] memory amounts) = issuanceModule
            .getRequiredComponentUnitsForIssue(address(PXLT), setAmount);
        issuanceModule.redeem(address(PXLT), setAmount, msg.sender);
        uint256 totalUSDC;
        for (uint256 i = 0; i < components.length; i++) {
            IERC20(components[i]).approve(address(router), amounts[i]);
            uint256 amountOut;
            if(components[i] == WETH) {
                amountOut = _directSwap(amounts[i]);
            } else {
                amountOut = _hopSwap(amounts[i], components[i]);
            }
            totalUSDC += amountOut;
        }
