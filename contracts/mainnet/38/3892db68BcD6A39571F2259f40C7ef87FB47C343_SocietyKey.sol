...
contract SocietyKey is ERC20Rewards {
    constructor(
        string memory name,
    ...
    ) ERC20Rewards(name, symbol, decimals, rate) {
...
contract ERC20Rewards is ERC20Permit {
    ...
    address public operator; // Exclude from reward
    uint256 public rewardRate; // Reward rate for each token per second
    uint128 private start; // Start point of Reward
    ...
    function setOperator(address user) external {
    ...
    function latest(uint128 x, uint128 y) internal pure returns (uint128 z) {
        z = (x > y) ? x : y;
    }
    ...
    function calculate(address user) public view returns (uint256) {
        ...
        return
            ...
            (_balanceOf[user] *
                rewardRate *
                (block.timestamp - latest(start, userRewards_.checkpoint))) /
            1 ether; // We must scale down the rewards by the precision factor
    }
    ...
    function _updateUserRewards(address user) internal {
        ...
        userRewards_.accumulated = uint128(calculate(user));
        userRewards_.checkpoint = uint128(block.timestamp);
