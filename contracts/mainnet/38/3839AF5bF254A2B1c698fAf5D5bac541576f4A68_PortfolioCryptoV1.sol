...
contract PortfolioCryptoV1 {
    ...
    function getBalancesTokensInUSDC() public view returns(uint totalAssetsInUSDC,uint[] memory balancesTokensInUSDC){
        ...
        while(i<len){
            ...
            if(token.token==USDC){
                ...
                balancesTokensInUSDC[i]=token.currentBalance*getPriceOnChainWithDecimals(token.token,token.decimals)/10**token.decimals;
            }
    ...
    function swapAndSetInitTokens(uint amount,uint totalEquivalentUSDC,uint[] memory distribution) internal {
        ...
        TokenERC20[] memory initBalance;
        if(totalEquivalentUSDC==0){
            initBalance=new TokenERC20[](1);
            initBalance[0]=TokenERC20(USDC,6,amount);
            ERC20Data[USDC].currentBalance=amount;
        ...
        uint calc;
        uint i;
        initBalance=new TokenERC20[](distribution.length); 
        while(i<distribution.length){
            _token=tokens[i];
    ...
    function getTokensBalanceOf(address ad) public view returns(uint _totalInvestedUSDC,TokenERC20[] memory balancesTokens){
        ...
        _totalInvestedUSDC+=inv[0].investedUSDC;
    ...
    function getTokensBalanceInUSDCOf(address ad) public view returns(uint totalAssetsInUSDC,TokenERC20[] memory balancesTokens){
        ...
        while(i<balancesTokens.length){
            if(balancesTokens[i].token!=USDC){
                balancesTokens[i].currentBalance=balancesTokens[i].currentBalance*getPriceOnChainWithDecimals(balancesTokens[i].token,balancesTokens[i].decimals)/10**balancesTokens[i].decimals;
            }
