...
contract LayerZeroApes is Ownable, ERC721A, NonblockingReceiver {
    using Strings for uint256;

    address public _owner;
    uint256 nextTokenId = 0;
    uint256 MAX_MINT_SUPPLY = 750;
    uint256 maxPerTX = 1;
    uint256 maxPerAddress = 3;
    bool public revealed = false;
    uint public cost = 300000000000000000;
    uint256 gasForDestinationLzReceive = 350000;

    string public uriPrefix = '';
    string public uriSuffix = '.json';
    string public hiddenMetadataUri = 'ipfs://QmeaTTTCjd4mT4zEmRTnePN7oT8yRn6vYAyUZhiwjGN9BW';

    bool public paused = true;

    constructor(address _layerZeroEndpoint)
        ERC721A("Layer Zero Apes", "LZA")
    {
        _owner = msg.sender;
    ...
    function mint(uint256 numTokens) external payable {
        require(numTokens <= maxPerTX, "Max NFTs per transaction limit");
        require(
            nextTokenId + numTokens <= MAX_MINT_SUPPLY,
            "Mint exceeds supply"
        );
        require(
            balanceOf(_msgSender()) + numTokens <= maxPerAddress,
            "Max NFTs per wallet limit"
        );
        uint256 price = getPrice();
        require(msg.value >= price * numTokens, "Insufficient ETH sent!");
    ...
    function airdropWinner(uint256 _mintAmount, address _receiver) public onlyOwner {
