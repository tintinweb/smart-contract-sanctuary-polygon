...
import './staking/interfaces/IERC20Mintable.sol';
/*
...
contract PolygonPlasma is ERC20, IChildToken, AccessControlMixin, NativeMetaTransaction, ContextMixin, IERC20Mintable {
    ...
    constructor(
        ...
        _setupContractId('PolygonPlasma');
     ...
     */
    function mint(address user, uint256 amount) external override only(MINTER_ROLE) {
        _mint(user, amount);
    }

    function addMinterRole(address minter) external only(DEFAULT_ADMIN_ROLE) {
        grantRole(MINTER_ROLE, minter);
    }

    function removeMinterRole(address minter) external only(DEFAULT_ADMIN_ROLE) {
        revokeRole(MINTER_ROLE, minter);
    }

    function addMultipleMinters(address[] calldata newMinters) external only(DEFAULT_ADMIN_ROLE) {
    ...
    function removeMultipleMinters(address[] calldata mintersToRemove) external only(DEFAULT_ADMIN_ROLE) {
