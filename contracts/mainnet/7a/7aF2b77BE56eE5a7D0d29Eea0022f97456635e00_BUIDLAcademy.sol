...
contract BUIDLAcademy is Context, Ownable {
    ...
    function addCurrency(IERC20 _paytoken, uint256 _decimals) public onlyOwner {
        AllowedCrypto.push(TokenInfo({paytoken: _paytoken, decimals: _decimals}));
    ...
    function buyCourse(uint256 _id, uint256 _pid) public payable {
        ...
        if (_pid == 0 || _pid == 1) {
            ...
            uint256 decimals = tokens.decimals;


            //course price is 18 decimals make it so it is = to the decimals of the token
            uint256 price = (courses[_id].price * 10**decimals) / 10**18;
            
            
            require(
                paytoken.balanceOf(_msgSender()) >= price,
