...
contract OmniMfersPOLY is ERC721A, Ownable, LayerZeroable {
    bool public saleEnabled;
    string public metadataBaseURL;
    string public PROVENANCE;

    uint256 public constant START_NUM = 3892;
    ...
    function reserve(uint256 num) external onlyOwner {
        require((totalSupply() + num) <= (MAX_SUPPLY+START_NUM), "Exceed max supply");
        _safeMint(msg.sender, num);
    }

    function freeMint(uint256 numOfTokens) external payable {
        require(saleEnabled, "Sale must be active.");
        require(totalSupply() + numOfTokens <= (MAX_SUPPLY+START_NUM), "Exceed max supply");
