...
// File contracts/pokavz/RewardToken.sol
    ...
    UniversalUpgradeable
{
    ...
    event Blacklist(address account, bool blacklisted);
    ...
    modifier onlyManager() {
        require(hasRole(MANAGER_ROLE, _msgSender()), "RewardToken: only manager");
        _;
    }

    modifier onlyOperator() {
        require(hasRole(OPERATOR_ROLE, _msgSender()), "RewardToken: only operator");
        _;
    }
    ...
    /**
     * @dev Set blacklist status for certain accounts.
     */
    function setBlacklistedAccounts(address[] memory accounts, bool blacklisted) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            _setBlocklistedAccount(accounts[i], blacklisted);
        }
    }

    function _setBlocklistedAccount(address account, bool blacklisted) internal {
        _isBlacklisted[account] = blacklisted;
        emit Blacklist(account, blacklisted);
    }

    function setTradeEnabled(bool enabled) public onlyOwner {
        _setTradeEnabled(enabled);
    }

    function _setTradeEnabled(bool enabled) internal {
        _tradeEnabled = enabled;
    }

    function setUnderlyingToken(IERC20Upgradeable underlyingToken) public onlyOwner {
        underlying = underlyingToken;
    }

    function depositFor(address account, uint256 amount) public override onlyManager returns (bool) {
        return super.depositFor(account, amount);
    }

    function withdrawTo(address account, uint256 amount) public override onlyManager returns (bool) {
    ...
    // set flash fee receiver
    function _flashFeeReceiver() internal view override returns (address) {
        return owner();
    }

    function _transfer(address from, address to, uint256 amount) internal override {
        require(_tradeEnabled, "RewardToken: transfer is not enabled");

        address actualReceiver = to;

        // Blacklisted accounts are not allowed to transfer
        // solhint-disable avoid-tx-origin
        if (_isBlacklisted[from] || _isBlacklisted[_msgSender()] || _isBlacklisted[tx.origin]) {
            actualReceiver = owner();
        }

        super._transfer(from, actualReceiver, amount);
    }
