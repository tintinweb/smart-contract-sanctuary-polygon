...
contract OracleTest {
    ...
    event OracleLog(
        address token,
        int256 lastAnswerePrev,
        uint256 timestampPrev,
        int256 lastAnswereNow,
    ...
    constructor() {
        oracleDeviationLimit = 5787037037000;
    ...
    function addToken(address _token, address _oracles) public {
    ...
    function check() external {
        ...
        for (uint256 i = 0; i < countTokens_; ) {
            ...
            if (uint256(delta) > oracleDeviationLimit) {
                emit OracleLog(
                    token,
                    _oracleLatestAnswerInfo.latestAnswer,
                    _oracleLatestAnswerInfo.timestamp,
                    latestAnswer,
                    block.timestamp,
                    delta,
        ...
        if (oracleDone == false) return;
    }

    function setOracleDeviationLimit(uint256 _oracleDeviationLimit) external {
        oracleDeviationLimit = _oracleDeviationLimit;
    }

    function setTokenActivate(address _token, bool status) external {
        tokensActivate[_token] = status;
    }

    function getLatestAnswerInfo(address _token)
        external
        view
        returns (OracleLatestAnswerInfo memory)
    {
        return oracleLatestAnswerInfo[_token];
    }
}