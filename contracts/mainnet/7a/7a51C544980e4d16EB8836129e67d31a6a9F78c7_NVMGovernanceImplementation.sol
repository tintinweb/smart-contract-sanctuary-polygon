...
import "./interfaces/NVMGovernanceStorageV1.sol";

import "hardhat/console.sol";

contract NVMGovernanceImplementation is
    Initializable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable,
    NVMGovernanceStorageV1
{
    using SafeERC20 for IERC20;

    /// @notice The name of this contract
    string public constant NAME = "NVMGovernance";

    /// @notice The maximum number of actions that can be included in a proposal
    uint256 public constant PROPOSAL_MAX_OPERATIONS = 10; // 10 actions

    /// @notice The precision for the percentage computations
     ...
     * @param _proposalThresholdPercentage The initial proposal threshold
     * @param _quorumVotesPercentage The initial quorum votes
     */
    function initialize(
        address _timelock,
        address _token,
        ...
        uint256 _quorumVotesPercentage
    ) public override initializer {
        require(INVMTimelock(_timelock).admin() == address(this), "Invalid timelock admin");
        require(_quorumVotesPercentage >= _proposalThresholdPercentage, "Invalid quorum votes");
        timelock = INVMTimelock(_timelock);

        __Ownable_init();
        __ReentrancyGuard_init();

        token = IVotingToken(_token);
        releaseToken = IVotingToken(_releaseToken);
    ...
    function proposalThreshold() public view override returns (uint256) {
        return (proposalThresholdPercentage * token.totalSupply()) / PERCENTAGE_PRECISION;
    }

    function quorumVotes() public view override returns (uint256) {
        return (quorumVotesPercentage * token.totalSupply()) / PERCENTAGE_PRECISION;
    }
    ...
    function propose(
        ...
        require(_targets.length != 0, "Too few actions");
        ...
        if (_latestProposalId != 0) {
            ...
            require(
                ...
                "User has another active proposal"
            );
            require(
                proposersLatestProposalState != ProposalState.Pending,
                "User has other pending proposal"
            );
        }
    ...
    function queue(uint256 _proposalId) external override {
        require(state(_proposalId) == ProposalState.Succeeded, "Proposal must be succeeded");
    ...
    function execute(uint256 _proposalId) external payable override {
        require(state(_proposalId) == ProposalState.Queued, "Proposal must be queued");
        Proposal storage _proposal = proposals[_proposalId];
    ...
    function cancel(uint256 _proposalId) external override {
        require(state(_proposalId) != ProposalState.Executed, "Proposal is already executed");
    ...
    function castVoteBySig(
        ...
        require(_v == 27 || _v == 28, "Invalid v value");
        require(
        ...
        require(_signatory != address(0), "Invalid signature");
        emit VoteCast(
            _signatory,
            _proposalId,
            _support,
    ...
    function castVoteInternal(
        ...
        require(_receipt.hasVoted == false, "Voter already voted");
     ...
     */
    function setVotingDelay(uint256 _newVotingDelay) external virtual override onlyOwner {
     ...
     * @param _newQuorumVotesPercentage new quorum votes
     */
    function setQuorumVotesPercentage(uint256 _newQuorumVotesPercentage)
        external
        override
        onlyOwner
    {
        require(_newQuorumVotesPercentage >= proposalThresholdPercentage, "Invalid quorum votes");

        uint256 _oldQuorumVotesPercentage = quorumVotesPercentage;
        quorumVotesPercentage = _newQuorumVotesPercentage;

        emit QuorumVotesSet(_oldQuorumVotesPercentage, _newQuorumVotesPercentage);
    }
     ...
     */
    function setVotingPeriod(uint256 _newVotingPeriod) external virtual override onlyOwner {
     ...
     * @param _newProposalThresholdPercentage new proposal threshold
     */
    function setProposalThresholdPercentage(uint256 _newProposalThresholdPercentage)
        external
        override
        onlyOwner
    {
        uint256 _oldProposalThresholdPercentage = proposalThresholdPercentage;
        proposalThresholdPercentage = _newProposalThresholdPercentage;

        emit ProposalThresholdSet(_oldProposalThresholdPercentage, _newProposalThresholdPercentage);
    }
...
import "./INVMGovernance.sol";

/**
 * @title Storage for Governance Proxy
 * @notice For future upgrades, do not change NVMGovernanceStorageV1. Create a new
 * contract which implements NVMGovernanceStorageV1 and following the naming convention
 * NVMGovernanceStorageVX.
 */
abstract contract NVMGovernanceStorageV1 is INVMGovernance {
    ...
    /// @notice The number of votes required in order for a voter to become a proposer, expressed as percentage of the total supply (1 to 100)
    uint256 public override proposalThresholdPercentage;

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed, expressed as percentage of the total supply (1 to 100)
    uint256 public override quorumVotesPercentage;

    /// @notice The total number of proposals
    uint256 public override proposalCount;

    /// @notice The address of the Governance Timelock
    INVMTimelock public override timelock;

    /// @notice The address of the governance token
    IVotingToken public override token;

    /// @notice The address of the "Release" governance token
    IVotingToken public override releaseToken;
...
import "@ubeswap/governance/contracts/interfaces/INonTransferrableToken.sol";
import "@ubeswap/governance/contracts/interfaces/IHasVotes.sol";

interface IVotingToken is INonTransferrableToken, IHasVotes {}

//SPDX-License-Identifier: Apache-2.0
pragma solidity 0.8.4;

import "./NVMGovernanceEvents.sol";
import "./INVMTimelock.sol";
import "./IVotingToken.sol";

interface INVMGovernance is INVMGovernanceEvents {
    struct Proposal {
    ...
    function proposalThresholdPercentage() external returns(uint256);
    function quorumVotesPercentage() external returns(uint256);
    function proposalThreshold() external returns(uint256);
    function quorumVotes() external returns(uint256);
    function proposalCount() external returns(uint256);
    function timelock() external returns(INVMTimelock);
    function token() external returns(IVotingToken);
    function releaseToken() external returns(IVotingToken);
    function proposals(uint256 _proposalId) external returns(
        uint256 id,
        address proposer,
        uint256 eta,
        uint256 startBlock,
    ...
    function proposalTargets(uint256 _proposalId, uint256 _index) external returns(address);
    function proposalValues(uint256 _proposalId, uint256 _index) external returns(uint256);
    function proposalSignatures(uint256 _proposalId, uint256 _index) external returns(string memory);
    function proposalCalldatas(uint256 _proposalId, uint256 _index) external returns(bytes memory);
    function latestProposalIds(address _proposarAddress) external returns(uint256);
    function initialize(
        address _timelock,
        address _token,
        address _releaseToken,
    ...
    function setVotingDelay(uint256 _newVotingDelayPercentage) external;
    function setQuorumVotesPercentage(uint256 _newQuorumVotesPercentage) external;
    function setVotingPeriod(uint256 _newVotingPeriod) external;
    function setProposalThresholdPercentage(uint256 _newProposalThreshold) external;
}
...
interface INVMGovernanceEvents {
    ...
    event ProposalThresholdSet(uint256 oldProposalThresholdPercentage, uint256 newProposalThresholdPercentage);

    /// @notice An event emitted when the quorum votes is set
    event QuorumVotesSet(uint256 oldQuorumVotesPercentage, uint256 newQuorumVotesPercentage);
