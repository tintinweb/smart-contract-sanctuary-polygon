...
contract ConstantVariables {
    ...
    IInstaIndex internal constant INSTA_INDEX_CONTRACT =
        IInstaIndex(0xA9B99766E6C676Cf1975c0D3166F96C0848fF5ad);
...
contract UserModule is Helpers, Events {
    ...
    function collectRevenue(uint256 amount_) external {
        ...
        if (amount_ == 0) revert UserModule__NotValidCollectReveuneAmount();
        if (amount_ > revenue) revert UserModule__CollectReveuneAmountIsHigh();
        ...
        IERC20Upgradeable(STETH_ADDRESS).safeTransferFrom(
            address(this),
            treasury,
            amount_
        );
        ...
        returns (uint256 newExchangePrice_, uint256 newRevenue_)
    {
        ...
        if (newExchangePrice_ > revenueExchangePrice) {
            uint256 newProfit_ = currentNetAssets_ -
                ((exchangePrice * iTokenSupply_) / 1e18); // currentNetAssets_(excluding reveune) - oldNetAssets_(excluding reveune)
