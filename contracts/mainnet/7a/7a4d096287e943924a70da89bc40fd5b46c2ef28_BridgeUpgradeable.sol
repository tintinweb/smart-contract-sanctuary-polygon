 ...
 */
    ...
    */
    function setBridgeFee(
        ...
        require(address(_feeManager) != address(0), "Bridge: Fee manager not set");
        _feeManager.setFee(
            destinationChainID,
            srcToken,
            destToken,
    ...
    function setTokenToUSDx1000(
        bytes32 resourceID,
        address token,
        uint256 priceInUSDx1000
    ) external virtual onlyRole(FEE_SETTER_ROLE) {
        address handlerAddress = _resourceIDToHandlerAddress[resourceID];
        IERCHandler handler = IERCHandler(handlerAddress);
        require(handler.getFeeStatus(), "fee is not enabled");
        handler.setTokenToUSDx1000(token, priceInUSDx1000);
    }
