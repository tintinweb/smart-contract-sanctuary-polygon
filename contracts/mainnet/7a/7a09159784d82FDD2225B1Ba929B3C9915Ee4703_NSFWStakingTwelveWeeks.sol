...
contract NSFWStakingTwelveWeeks is OwnableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, NSFWFreezableUpgradable {    
    ...
    uint256 constant private MIN_DEPOSIT = 1000000;
    uint256 constant private MAX_DEPOSIT = 1000000000;
    uint256 constant public  PERIOD_COUNT = 12;
    ...
    ISwapRouter public _uniswapV3Router;
    ...
    function claimReward() external hardUpdate nonReentrant whenInitialized whenFreezeIs(CLAIM_FREEZE, false) {
        ...
        payable(msg.sender).transfer(uint256(userDatas[msg.sender].totalRewards));
        delete userDatas[msg.sender].totalRewards;
    }

    function sendReward() payable external nonReentrant whenInitialized onlyRole(DEFAULT_ADMIN_ROLE) whenFreezeIs(REWARD_FREEZE, false) {
        require(contractFirstMonday + (currentPeriod + 1) * ONE_WEEK <= block.timestamp, "You can't send any rewards until next monday");

        require((totalStaked > 0 && msg.value > 0) || (totalStaked == 0 && msg.value == 0), "Token must be staked to send more than 0 BNB");
        if (totalStaked > 0)
            sharePrice[currentPeriod] = (int256(msg.value) * _PRECISION_FACTOR) / totalStaked; //Warning, critical failure if multiple call during one period
        currentPeriod++;
        totalStaked += globalHistory[currentPeriod];
        //delete globalHistory[currentPeriod];
    }
    ...
    function claimRewardAsNsfwV3() external hardUpdate nonReentrant whenInitialized whenFreezeIs(CLAIM_FREEZE, false) {
        require(userDatas[msg.sender].totalRewards > 0, "You have no rewards to claim");

        address wethAddress = _uniswapV3Router.WETH9();
        IWETH9 weth = IWETH9(wethAddress);
        weth.deposit{value: uint256(userDatas[msg.sender].totalRewards)}();

        ERC20Upgradeable(wethAddress).safeApprove(address(_uniswapV3Router), uint256(userDatas[msg.sender].totalRewards));

        ISwapRouter.ExactInputSingleParams memory params =
            ISwapRouter.ExactInputSingleParams({
                tokenIn: wethAddress,
                tokenOut: address(_token),
                fee: 3000, //TODO: add to contract attributes
                recipient: address(this),
                deadline: block.timestamp + 15,
                amountIn: uint256(userDatas[msg.sender].totalRewards),
                amountOutMinimum: 0,
                sqrtPriceLimitX96: 0
            });

        uint256 amountToken = _uniswapV3Router.exactInputSingle(params);
        _token.safeTransfer(msg.sender, amountToken);

        delete userDatas[msg.sender].totalRewards;
    }

    function getUserRewardAsNsfwV3(address account) external view whenInitialized returns(uint256) {
        int256 staked = userDatas[account].totalStaked;
        int256 rewards = userDatas[account].totalRewards;
        for (uint256 i = userDatas[account].lastUpdate; i < currentPeriod; i++) {
            staked += userDatas[account].history[i];
        ...
        address wethAddress = _uniswapV3Router.WETH9();
        (int24 arithmeticMeanTick, ) = OracleUtils.consult(_uniswapV3Pool, 10);
        return OracleUtils.getQuoteAtTick(arithmeticMeanTick, uint128(int128(rewards)), wethAddress, address(_token));
    }
    ...
    function forceUnlock(address account, uint256 effectivePeriod, int256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (userDatas[account].unlock.amount > 0)
            return;
        UserData storage userData = userDatas[account];
        userData.unlock.amount = amount;
        userData.unlock.effectivePeriod = effectivePeriod;
        userData.history[userData.unlock.effectivePeriod] -= userData.unlock.amount;
        globalHistory[userData.unlock.effectivePeriod] -= userData.unlock.amount;
    }

    function setUnlockedBalance(address account, int256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        userDatas[account].totalUnlocked = amount;
    }

    function setlastDepositperiod(address account, uint256 period) external onlyRole(DEFAULT_ADMIN_ROLE) {
        userDatas[account].lastDepositPeriod = period;
    }

    function batchInsert(address[] memory accounts, int256[] memory entranceBalances, int256[] memory stakedBalances, int256[] memory
            unlockedBalances,bool[] memory unlocksRequested, uint256[] memory unlocksEffectivePeriods, uint256[] memory lastDeposits) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function setUniswapV3Router(address uniswapV3RouterAddress) external whenInitialized onlyRole(DEFAULT_ADMIN_ROLE) {
        _uniswapV3Router = ISwapRouter(uniswapV3RouterAddress);
    }

    function setUniswapV3Pool(address uniswapV3PoolAddress) external whenInitialized onlyRole(DEFAULT_ADMIN_ROLE) {
        _uniswapV3Pool = uniswapV3PoolAddress;
    }
...
// From OracleLibrary.sol, Uniswap V3
pragma solidity ^0.8.10;
