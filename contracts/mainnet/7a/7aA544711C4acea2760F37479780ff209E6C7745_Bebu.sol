...
contract Bebu is ERC20Burnable, Operator {
    ...
    // TOTAL MAX SUPPLY = 1,000,000 bebu
    uint256 public constant FARMING_POOL_REWARD_ALLOCATION = 500000 ether;
    uint256 public constant STABLE_FARMING_POOL_REWARD_ALLOCATION = 150000 ether;
    uint256 public constant COMMUNITY_FUND_POOL_ALLOCATION = 200000 ether;
    ...
    event AddAddressBlacklist(address indexed _address);
    event RemoveAddressBlacklist(address indexed _address);
    event SetTreasuryFund(address indexed _communityFund);
    event SetDevFund(address indexed _devFund);
    event DistributeReward(address indexed _farmingIncentiveFund, address _stableFarmingIncentiveFund);

    constructor(uint256 _startTime, address _communityFund, address _devFund) ERC20("BEBU", "BEBU") {
        _mint(msg.sender, 1 ether); // initial liquidity
        _mint(_communityFund, 50000 ether); // mint 50000 bebu for initial pools deployment and launchpad
    ...
    function addAddressBlacklist(address _address) public onlyOwner returns (bool) {
        ...
        emit AddAddressBlacklist(_address);
        return true;
    }
    ...
    function removeAddressBlacklist(address _address) public onlyOwner returns (bool) {
        ...
        emit RemoveAddressBlacklist(_address);
        return true;
    }
    ...
    function setTreasuryFund(address _communityFund) external {
        require(msg.sender == communityFund, "!communityFund");
        require(_communityFund != address(0), "zero");
        communityFund = _communityFund;
        emit SetTreasuryFund(_communityFund);
    }
    ...
    function distributeReward(address _farmingIncentiveFund, address _stableFarmingIncentiveFund) external onlyOperator {
        ...
        emit DistributeReward(_farmingIncentiveFund, _stableFarmingIncentiveFund);
    }
