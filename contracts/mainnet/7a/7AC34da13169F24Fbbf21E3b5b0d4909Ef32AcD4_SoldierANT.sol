...
contract SoldierANT is ERC721, Ownable {

    uint256 public counter = 1;
    uint256 public immutable STAKE_DURATION = 1 minutes;
    uint256 public immutable RAID_DURATION = 3 minutes;
    uint256 public immutable HEAL_WINDOW = 1 hours;
    uint256 public immutable HEALING_FEE = 80;
    constructor() ERC721("SoldierANT","SoldierANT") {
    }
    ...
    function getAvailableSoldiers(address _user) public view returns(uint256[] memory) {
        ...
        for (uint i=0;i<_soldierList.length;i++){
            if (idToOnRaidMission[_soldierList[i]]){
                _soldierOnRaidCount ++;
            } else if (idToStaked[_soldierList[i]]){
                _stakedSoldierCount ++;
            } else if (idToPassive[_soldierList[i]]){
                _zombieSoldierCount ++;
            }     
        }
        uint256[] memory availableSoldierList = new uint256[](_soldierList.length-_soldierOnRaidCount-_stakedSoldierCount-_zombieSoldierCount);
        for (uint256 i; i < _soldierList.length; i++){
            if (!idToStaked[_soldierList[i]] && !idToOnRaidMission[_soldierList[i]] && !idToPassive[_soldierList[i]]) {
    ...
    function getZombieSoldiers(address _user) public view returns(uint256[] memory) {
    ...
    function getInfectedSoldiers(address _user) public view returns(uint256[] memory) {
        uint256[] memory _soldierList = playerToSoldiers[_user];
        uint256 soldierWithDamageCount;
        for (uint i=0;i<_soldierList.length;i++){
            if (idToDamageCount[_soldierList[i]]>0 && !idToPassive[_soldierList[i]]){
                soldierWithDamageCount ++;
            }
        }
        uint256[] memory infectedSoldierList = new uint256[](soldierWithDamageCount);

        uint256 infectedSoldierCount;
        for (uint256 i; i < _soldierList.length; i++){
            if (idToDamageCount[_soldierList[i]]>0) {
    ...
    function increaseDamage (uint256 _index)  public {
        ...
        if(idToDamageCount[_index]>=3 && _timeElapsed > HEAL_WINDOW){
            idToPassive[_index] = true;
        }
    }

    function getClaimableLarvaCount(address _user) public view returns(uint256 _count){
        uint256[] memory _soldierList = playerToSoldiers[_user];
        uint256 _claimableLarvaCount;
        for (uint i = 0; i < _soldierList.length; i++){
        if(idToOnRaidMission[_soldierList[i]] &&
            RAID_DURATION <= block.timestamp - idToRaidDate[_soldierList[i]]){
    ...
    function infectionSpread(address _user) public {
        uint256[] memory _soldierList = playerToSoldiers[_user];
        uint256[] memory _zombieSoldierList = getZombieSoldiers(_user);
        for (uint256 i=0; i<_soldierList.length; i++){
            if (_zombieSoldierList.length>0){
                uint chance = uint(
                keccak256(abi.encodePacked(blockhash(block.number - 1), block.timestamp))
            ) % 100;  
                if(chance<1 && idToPassive[_soldierList[i]]==false &&
                idToDamageCount[_soldierList[i]]<3){
    ...
    function reduceDamage (uint256 _index, uint256 _damageReduced)  public {
        ...
        for (uint i=0;i<_damageReduced;i++){
            _damageCount -= 1;
        }
    ...
    function burn(address _user, uint256 _index) external {
        require(playerToSoldiers[_user].length > 0 , "index out of bound");   
            playerToSoldiers[_user][_index-1] =  playerToSoldiers[_user][ playerToSoldiers[_user].length - 1];
            playerToSoldiers[_user].pop();
            _burn(_index);
            console.log("soldier burnt", _index);
    }
