...
contract BeatBattle_11 {
    ...
    function SetRegistrationTrue(string memory ueDate, string memory ueLink) public {
        require(_counter < 5, "BeatBattle_11::Exceeds maximum contestants");
        for (uint256 i=0; i<5; i++){
...
// طب لو حد إدي أصوات وبعدين أشترك، هل ندور علي الأصوات دي ونخليها باطلة
    ...
    function Vote (address faddress) public {     
        ...
        for (uint256 i = 0; i < 5; i++) {
            if (contestants[i]._address == msg.sender && contestants[i].votingTickets <2) {              
                    for (uint256 j = 0; j < 5; j++) {
                        if (contestants[j]._address == faddress) {
                            contestants[j].votes++;
                            break;
                        }
                    }
                contestants[i].votingTickets++;
                break;
            }
        }
    }
