...
contract MetaAttendanceNFT is ERC721EnumerableAutoIdPropertyManager, BaseRelayRecipient, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function changeSession(uint256 eid, uint8 sid, uint256 start, uint256 length) external onlyRole(EVENT_MANAGER_ROLE) {
        require(eid < events.length, "wrong eid");
        Event storage evt = events[eid];
    ...
    function sessionInfo(uint256 eid, uint8 sid) public view returns(uint256 start, uint256 length, bool canAttend) {
        require(eid < events.length, "wrong eid");
        Event storage evt = events[eid];
        require(sid < evt.sessions.length, "wrong sid");
        Session storage ss = evt.sessions[sid];
        start = ss.start;
        length = ss.length;
        canAttend = (block.timestamp >= ss.start) && (block.timestamp <= ss.start+ss.length);
    }
    ...
    function attended(uint256 eid, uint8 sid, address account) public view returns(bool) {
        ...
        uint248 tid = _getTidForEvent(eid, pid);
    ...
    function _attend(uint256 eid, uint8 sid, address account) internal returns(uint248 tid){
        ...
        uint256 pid = _getOrMintMetaNFTId(account, property());
        tid = _getOrMintTidForEvent(eid, pid, account);
        require(!data[tid].bitmap.get(sid), "already attended");
        data[tid].bitmap.set(sid);
    }

    function _getTidForEvent(uint256 eid, uint256 pid) internal view returns(uint248) {
        if(pid == 0) return 0;
        return eventToTid[pid][eid];
    }

    function _getOrMintTidForEvent(uint256 eid, uint256 pid, address account) internal returns(uint248) {
        uint248 tid = _getTidForEvent(eid, pid);
        if (tid > 0) {
            return tid;
        }
