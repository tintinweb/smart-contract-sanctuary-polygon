...
///
////////////////////////////////////////////////////////////////////////////////////////////
contract BribePool is Initializable, BribePoolConf, IFeeDistributor {
    ...
    modifier onlyUnblockedIncentive(bytes32 proposalId, uint256 optionIndex) {
        require(!incentives[proposalId][optionIndex].cancelled, Errors.PROPOSAL_BLOCKED);
        _;
    }
    ...
    /// @param _minimumDepositForPotRefund minimum deposit that should be available in incentive for refund
    function initialize(
    ...
    /// @notice change bid asset for all further created incentives
    /// @param _bidAsset new bid asset to use
    function changeBidAsset(address _bidAsset) external onlyOwner {
        // withdraw all fees from previous bidAsset
        withdrawFees();
        bidAsset = _bidAsset;
    }
        ...
        returns (IncentiveState)
    {
        ...
        } else if (incentive.cancelled) {
            return IncentiveState.CANCELLED;
        } else if (incentive.executedAt == 0) {
            return IncentiveState.ACTIVE;
        } else if (block.timestamp < incentive.executedAt + 30 days) {
    ...
    function deposit(
        ...
        address to,
        uint128 amount
    ) external override onlyUnblockedIncentive(proposalId, optionIndex) {
        ...
        require(
            ...
            Errors.ONLY_WHEN_ACTIVE_OR_NOT_CREATED
        );
        require(amount > 0, Errors.ZERO_AMOUNT);

        Incentive storage incentive = incentives[proposalId][optionIndex];
        incentive.bidAsset = bidAsset;

        /// interactions
        IERC20(incentive.bidAsset).safeTransferFrom(msg.sender, address(this), amount);
        ...
        emit DepositedBribe(proposalId, optionIndex, to, amount);
    }
    ...
    function submitMerkleRoot(
        ...
        bytes32 _ipfsHash,
        uint128 _totalVotes
    ) external onlyUnblockedIncentive(proposalId, optionIndex) onlyOwner {
        require(
            getPotState(proposalId, optionIndex) == IncentiveState.ACTIVE,
            Errors.POT_NOT_ACTIVE
        );
    ...
    /// @dev cancel incentive for invalid params
    /// @param proposalId propoasl id
    /// @param optionIndex index/number of the bribe incentive for that proposal
    function cancelInvalidIncentive(bytes32 proposalId, uint256 optionIndex) external onlyOwner {
        Incentive storage incentive = incentives[proposalId][optionIndex];
        require(!incentive.cancelled, Errors.POT_ALREADY_CANCELLED);
        require(incentive.executedAt == 0, "POT_ALREADY_EXECUTED");
        incentive.cancelled = true;
        emit CancelledIncentive(msg.sender, proposalId, optionIndex);
    }
    ...
    function withdrawLeftovers(
        ...
        Incentive storage incentive = incentives[proposalId][optionIndex];
        (uint256 leftovers, uint256 fees) = leftoversToWithdraw(proposalId, optionIndex, to);

        // effects
        feesCollected += fees;
        incentive.depositReclaimed += leftovers;
        depositors[proposalId][optionIndex][to] = 0;

        // interactions
        IERC20(incentive.bidAsset).safeTransfer(to, leftovers - fees);

        emit WithdrewDeposit(proposalId, optionIndex, msg.sender, to, leftovers - fees);
    }
    ...
    function rewardsToClaim(
        ...
        address to,
        uint256 vp
    ) external view returns (uint256 rewards, uint256 fees) {
        Incentive memory incentive = incentives[proposalId][optionIndex];
        // If a merkleroot is attached to the incentive, we can claim
        require(verify(proof, incentive.merkleRoot, to, vp), Errors.INVALID_MERKLE_PROOF);
        require(!claimedReward[proposalId][optionIndex][to], Errors.REWARD_ALREADY_CLAIMED);
        ...
        fees = (rewards * rewardFee) / FEE_MAX_SCALE;
    }
    ...
    function claimRewards(
        ...
        require(proposalId.length == optionIndex.length, Errors.ARITY_MISMATCH);
        require(optionIndex.length == proof.length, Errors.ARITY_MISMATCH);
        require(optionIndex.length == vp.length, Errors.ARITY_MISMATCH);
    ...
    function _claimRewards(
        ...
        // interactions
        IERC20(incentive.bidAsset).safeTransfer(to, reward - fee);

        emit ClaimedReward(proposalId, optionIndex, to, reward - fee);
    ...
    function withdrawFees() public override returns (uint256 feeAmount) {
...
library Errors {
    ...
    string public constant INVALID_MERKLE_PROOF = "9";
    ...
    string public constant POOL_ALREADY_EXISTS = "20";
    string public constant POT_ALREADY_CANCELLED = "21";
    string public constant ONLY_WHEN_ACTIVE_OR_NOT_CREATED = "22";
    string public constant ARITY_MISMATCH = "23";
}
...
interface IBribePool {
    ...
    enum IncentiveState {
        ...
        CANCELLED // incentive is cancelled
    }
