...
contract VibeFactory is BoringBatchable {
    ...
    event ERC1155Created(address indexed sender, address indexed proxy);
    event LogRoyaltyReceiver(address indexed sender, address indexed proxy);
    event LogNFTMintSale(address indexed sender, address indexed proxy);
    event LogNFTMintSaleMultiple(address indexed sender, address indexed proxy);
    event ClaimantDropCreated(address indexed sender, address indexed proxy);
    ...
    function createRoyaltyReceiver(
        ...
        emit LogRoyaltyReceiver(msg.sender, proxy);
    }
    ...
    function createNFTMintSale(
        ...
        emit LogNFTMintSale(msg.sender, proxy);
    }
    ...
    function createNFTMintSaleMultiple(
        ...
        emit LogNFTMintSaleMultiple(msg.sender, proxy);
    }
    ...
    function createClaimantDrop(
        ...
        emit ClaimantDropCreated(msg.sender, proxy);
    }
    ...
    function createClaimantDropLimited(
        ...
        emit ClaimantDropLimitedCreated(msg.sender, proxy);
    }
    ...
    function createVibeWhitelistSale(
        ...
        emit VibeWhitelistSaleCreated(msg.sender, proxy);
    }
...
contract NFTMintSale is Ownable {
    ...
    function removeTokensAndReclaimOwnership(address proceedRecipient) external onlyOwner {
        require(block.timestamp > endTime || nft.totalSupply() == maxMint);
