...
import {IgDIAT} from "./interfaces/IgDIAT.sol";
import {IsDIAT} from "./interfaces/IsDIAT.sol";
import {IStaking} from "../staking/interfaces/IStaking.sol";

contract sDiat is IsDIAT, ERC20Permit, Ownable {
    /* ========== DEPENDENCIES ========== */
    ...
    constructor() ERC20("Staked DIAT", "sDIAT", 9) ERC20Permit("Staked DIAT") {
    ...
    function setgDIAT(address _gDIAT) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gDIAT) == address(0), "gDIAT:  gDIAT already set");
        require(_gDIAT != address(0), "gDIAT:  gDIAT is not a valid contract");
        gDIAT = IgDIAT(_gDIAT);
    }
    ...
    /**
        @notice increases sDIAT supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sDIAT of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sDIAT
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sDIAT: insufficient balance");
    }
    ...
    // toG converts an sDIAT balance to gDIAT terms. gDIAT is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gDIAT.balanceTo(amount);
    }

    // fromG converts a gDIAT balance to sDIAT terms. sDIAT is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gDIAT.balanceFrom(amount);
    }

    // Staking contract holds excess sDIAT
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gDIAT.balanceFrom(IERC20(address(gDIAT)).totalSupply())).add(
