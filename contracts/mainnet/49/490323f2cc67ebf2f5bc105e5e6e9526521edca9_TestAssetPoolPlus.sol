...
/*
   TestAssetPoolPlus is a contract that holds user assets
*/
contract TestAssetPoolPlus is AccessControlUpgradeable, IAssetPoolV3 {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    function initialize(address _baseAsset) public initializer {
        ...
        byteDiffer = "Another set of bytes for implementation contract Another set of bytes for implementation contract ";
    }

    function getBaseAsset() public view override returns (address) {
        string memory byteDifferLocal = "Bytes to make test bytecode differ Bytes to make test bytecode differ Bytes to make test bytecode differ ";
        return baseAsset;
    }
    ...
    function realizeLoss(uint256 _amount) public override {
        // check permissions
        require(investProxiesStatuses[msg.sender] > 0, "invest strategy only"); // statuses 1 (active) or 2 (withdraw only) are ok
