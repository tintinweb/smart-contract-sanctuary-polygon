...
library MakiswapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'46ee21082550c535ac75013850b3106f9f5a2c08a3c6d4a687ffcfc2d26e0f1e' // init code hash
            ))));
    }
...
// File: maki-swap-periphery/contracts/interfaces/IWMATIC.sol
...
contract MakiswapRouter is IMakiswapRouter02 {
        ...
        )
    {
        ...
        address pair = MakiswapLibrary.pairFor(factory, token, WMATIC);
        ...
        if (msg.value > amountMATIC)
            TransferHelper.safeTransferMATIC(msg.sender, msg.value - amountMATIC);
    }
    ...
    function removeLiquidityMATIC(
        ...
        returns (uint256 amountToken, uint256 amountMATIC)
    {
        ...
        returns (uint256 amountToken, uint256 amountMATIC)
    {
        ...
        (amountToken, amountMATIC) = removeLiquidityMATIC(
            token,
            liquidity,
            amountTokenMin,
            amountMATICMin,
    ...
    function swapExactMATICForTokens(
        ...
        returns (uint256[] memory amounts)
    {
        require(path[0] == WMATIC, "MakiswapRouter: INVALID_PATH");
    ...
    function swapTokensForExactMATIC(
        ...
        returns (uint256[] memory amounts)
    {
        require(path[path.length - 1] == WMATIC, "MakiswapRouter: INVALID_PATH");
        amounts = MakiswapLibrary.getAmountsIn(factory, amountOut, path);
        require(
            amounts[0] <= amountInMax,
            "MakiswapRouter: EXCESSIVE_INPUT_AMOUNT"
        ...
        returns (uint256[] memory amounts)
    {
        ...
        if (msg.value > amounts[0])
            TransferHelper.safeTransferMATIC(msg.sender, msg.value - amounts[0]);
    }
    ...
    function swapExactTokensForMATICSupportingFeeOnTransferTokens(
        ...
        uint256 amountOut = IERC20(WMATIC).balanceOf(address(this));
        require(
