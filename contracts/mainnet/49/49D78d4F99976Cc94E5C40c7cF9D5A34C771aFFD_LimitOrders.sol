...
contract LimitOrders is EIP712("FBS Token", "1"){
    ...
    bytes32 LIMIT_ORDER_TYPEHASH = keccak256(
        "Order(address owner,uint256 status,address tokenIn,address tokenOut,uint256 amountIn,uint256 priceExecute, uint256 minAmountOut,uint256 deadline,uint256 salt)"
        );
    // orders
        ...
        returns (address)
    {
        ...
        bytes32 _hash = getEthSignedMessageHash(_getMsgHash);
    ...
    function shouldFulfilOrder(Order memory ord, bytes memory _signature) public view returns (bool) {
        ...
        if(IERC20(address(ord.tokenIn)).balanceOf(address(ord.owner)) < ord.amountIn) _isEnoughToken = false;

        // bool _allowance = false;
        // if(IERC20(ord.tokenIn).allowance(ord.owner,address(uniswapRouter)) >= ord.amountIn) _allowance = true;
        ...
        if(getAmountOut(ord.amountIn, ord.tokenIn, ord.tokenOut) >= ord.priceExecute) _isFulfiledOrder = true;
    ...
    function ExecuteCustomOrder(Order memory ord, bytes memory _signature) public reentrancyGuard returns (bool result, string memory message){
        ...
        require(IERC20(ord.tokenIn).balanceOf(ord.owner) >= ord.amountIn, "NEB");
        require(ord.status == 0, "NPD");

        if(execute(ord)) {
            msgHashOrderCancel[ord.owner].push(_msgHash);
            uint256 takeFee = IERC20(ord.tokenOut).balanceOf(address(ord.owner)).div(1000);
            IERC20(ord.tokenOut).transfer(address(feeAddress), takeFee);
    ...
    function execute(Order memory ord) internal returns (bool filled) {
        uint256 deadline = ord.deadline; 
        address tokenIn = ord.tokenIn;
        address tokenOut = ord.tokenOut;
        uint24 fee = 3000;
        address recipient = ord.owner;
        uint256 amountIn = ord.amountIn;
        uint256 amountOutMinimum = ord.minAmountOut;
    ...
    //     uniswapRouter.exactInputSingle{ value: msg.value }(params)
    //     { 
    //     uint takefee = IERC20(tokenOut).balanceOf(address(recipient)).div(1000);
    //     IERC20(tokenOut).transfer(address(feeAddress), takefee);
    //     return true; 
    //     }
    // catch { return false; }
    }
