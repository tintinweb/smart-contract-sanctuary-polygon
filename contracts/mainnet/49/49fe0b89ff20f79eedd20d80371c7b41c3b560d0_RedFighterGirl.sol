...
contract RedFighterGirl is ERC721 , DefaultOperatorFilterer {
    ...
    constructor()
        ERC721("Red Fighter Girl", "RFG")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 13) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeihcbabky5twcyodz55iee2im4qtgvclgl3uhwsaeqjyct5t2kaqzm/", Strings.toString(tokenId), ".json")
            );
    }
