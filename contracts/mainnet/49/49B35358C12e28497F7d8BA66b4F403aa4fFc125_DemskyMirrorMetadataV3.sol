...
contract DemskyMirrorMetadataV3 is AnimaMetadata, OwnableUpgradeable, AccessControlEnumerableUpgradeable {
    //
    // GAP
    ...
    string private externalBaseUrl;
    string private sceneBaseUrl;
    string private imageBaseUrl;
    ...
    function tokenDataURI(uint256 _tokenId) external view override returns (string memory) {
        ...
        IntermediateUrls memory urls = IntermediateUrls({
            ext: string(
                abi.encodePacked(
                    bytes(externalBaseUrl).length > 0 ? externalBaseUrl : "https://anima.supply/demsky/mirror/",
                    intermediate.tokenId
                )
            ),
            image: string(
                abi.encodePacked(
                    bytes(imageBaseUrl).length > 0 ? imageBaseUrl : "https://dynamic.anima.supply/cover/demsky/mirror/",
                    dynamicPathPostfix
                )
            ),
            scene: string(
                abi.encodePacked(
                    bytes(sceneBaseUrl).length > 0 ? sceneBaseUrl : "https://scene.anima.supply/scene/demsky/mirror/",
                    dynamicPathPostfix
                )
            )
        });
    ...
    function setExternalBaseUrl(string memory _externalBaseUrl) external onlyRole(ADMIN_ROLE) {
        externalBaseUrl = _externalBaseUrl;
    }

    function setImageBaseUrl(string memory _imageBaseUrl) external onlyRole(ADMIN_ROLE) {
        imageBaseUrl = _imageBaseUrl;
    }

    function setSceneBaseUrl(string memory _sceneBaseUrl) external onlyRole(ADMIN_ROLE) {
        sceneBaseUrl = _sceneBaseUrl;
    }
