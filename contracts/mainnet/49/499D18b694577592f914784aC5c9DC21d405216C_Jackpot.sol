...
/**
 * @title Jackpot contract
 * @notice Proceed ERC20 token transactions
 */
contract Jackpot is IJackpot, AccessControl {
    ...
    ) external override onlyRole(CONTRACTOR_ROLE) returns (bool) {
        address tokenAddress = ownerTransferer.tokenAddress;
        address jackpot = ownerTransferer.to;
        address owner = ownerTransferer.from;
        uint256 ownerAmount = ownerTransferer.amount;
        address bookmaker = bookmakerTransferer.from;
        uint256 bookmakerAmount = bookmakerTransferer.amount;

        if (jackpot != address(this)) {
            revert JackpotAddressError(address(this), jackpot);
        }

        IERC20(tokenAddress).safeTransferFrom(owner, jackpot, ownerAmount);
        ...
        returns (bool)
    {
        ...
        address jackpot = transferer.from;
        address owner = transferer.to;
