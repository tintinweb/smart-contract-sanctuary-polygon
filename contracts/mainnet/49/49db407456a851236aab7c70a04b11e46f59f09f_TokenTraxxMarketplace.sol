...
interface ICollection {
   ...
   */
  function getCollectionData(uint256 _id) external returns (Collection memory);
    ...
    ErrorCodes
{
        ...
        override
    {
        ...
        require(_params.primarySplitAddress != address(0), throwError(ZERO_ADDRESS));
        ...
        Listing memory newListing = Listing({
            ...
            primarySplitAddress: _params.primarySplitAddress,
        ...
        external
    {
        ...
        require(
            listings[_listingId].assetContract == _params.assetContract,
            throwError(CANT_MODIFY_LISTING_ASSET)
        );
        require(
            listings[_listingId].tokenId == _params.tokenId,
        ...
        require(listings[_listingId].quantity > 0, throwError(CANNOT_MODIFY_INACTIVE_SALE));

        listings[_listingId].buyoutPrice = _params.buyoutPrice;
        listings[_listingId].quantity = _params.quantityToList;
        listings[_listingId].primarySplitAddress = _params.primarySplitAddress;
    ...
    function executeSale(
        ...
        if (_currencyAmountToTransfer > 0) {
            address payee = _targetListing.primarySplitAddress != address(0) ? _targetListing.primarySplitAddress : _targetListing.tokenOwner;
            payout(
                msg.sender,
                payee,
                _currency,
    ...
    function removeListing(uint256 listingId) external {
        ...
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender) ||
            hasRole(MAINTAINER_ROLE, msg.sender) ||
                listings[listingId].tokenOwner == msg.sender ||
                validateApproval(
                    listings[listingId].tokenOwner,
                    listings[listingId].assetContract,
                    listings[listingId].tokenId,
                    listings[listingId].tokenType,
                    msg.sender
                ),
            throwError(UNAUTHORISED_ACCESS)
        );
        address assetContract = listings[listingId].assetContract;
        ...
        require(
            listings[listingId].assetContract != address(0),
            throwError(LISTING_ID_NOT_FOUND)
        );
        ...
        returns (uint256 listingCount, uint256 activeCount)
    {
        ...
        for (uint256 i; i < listingCount; ++i) {
            ...
            if (listings[_tokenListings[_assetContract][_tokenId][i]].quantity > 0) {
                activeCount++;
            }
        }
    }
    ...
    /**
     * @dev Stores the maintaner role
     * allows the  of collections
     */
