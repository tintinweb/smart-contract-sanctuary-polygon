...
contract SpecMatch is IFight, Ownable {
    ...
    function registerFighter(uint256 eventIndex, uint256 luchadorId, uint256 pricePaid, address payee) public {
        require(isEventManager(msg.sender), "Sender not the event manager");

        uint256 fighterIndex = fights[eventIndex].fighterCount;

        fights[eventIndex].fighters[fighterIndex].luchadorId = luchadorId;
        fights[eventIndex].fighters[fighterIndex].paid = pricePaid;
        fights[eventIndex].fighters[fighterIndex].payee = payee;
        fights[eventIndex].fighters[fighterIndex].weightClass = calculateWeightClass(calculateWeight(luchadorId));
    ...
    function formRoster() public {
        ...
        for (uint256 i = 0; i <= weightClasses.length; i++) {
            ...
            if (luchadorCount == 2) {
                fights[eventIndex].fightCards[i].luchador1 = weightClassLuchadores[0];
                fights[eventIndex].fightCards[i].luchador2 = weightClassLuchadores[1];
    ...
    function placeWager(uint256 eventIndex, uint256 amount, uint256 luchadorId) public {
        require(eventIndex == eventManager.getCurrentEventIndex(eventId), "Desired event index too old");

        nacho.transferFrom(msg.sender, address(this), amount);

        uint256 wagerIndex = fights[eventIndex].wagerCount;

        fights[eventIndex].wagers[wagerIndex].owner = msg.sender;
        fights[eventIndex].wagers[wagerIndex].amount = amount;
        fights[eventIndex].wagers[wagerIndex].luchadorId = luchadorId;
