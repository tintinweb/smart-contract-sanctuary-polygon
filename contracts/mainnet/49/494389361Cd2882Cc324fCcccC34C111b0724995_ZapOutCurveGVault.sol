...
///@author Zapper, modified and adapted for Grizzly.fi by Gonzalo & Elias.
...
contract ZapOutCurveGVault is ZapOutBase {
    ...
    ICurveRegistry public curveReg; // V3 now 0x0c32FCDC542d7A00ddcb0fF1AC6e266a04eEBDB1
    ...
    */
    function ZapOut(
        ...
        address poolTokenAddress = 0xf8a57c1d3b9629b77b6726a042ca48990A84Fb49; // ERC20 Curve LP Token
        ...
        uint256 toTokensBought = _zapOut(
            swapAddress, // Curve pool address
            underlyingTokenReceived,
            intermediateToken, // renBTC for example
            toToken, // USDC as well
            _swapTarget, // 0x0000000000000000000000000000000000000000 in case of USDC
            _swapCallData // 0x00
        );
    ...
    function _vaultWithdraw(
        ...
        IVault(fromVault).withdraw(amount);
    ...
    function _zapOut(
        ...
        (bool isUnderlying, uint256 underlyingIndex) = (true, 1); // renBTC

        // Not Metapool
        if (isUnderlying) {
            uint256 intermediateBought = _exitCurve(
                swapAddress, // Address of the pool
                incomingCrv,
                underlyingIndex, // 1 for USDC
                intermediateToken
            );
    ...
    */
    function _exitCurve(
        ...
        address depositAddress = 0xC2d95EEF97Ec6C17551d45e77B590dc1F9117C67;

        address tokenAddress = 0xf8a57c1d3b9629b77b6726a042ca48990A84Fb49;
    ...
    */
    function _fillQuote(
        ...
        if (_fromTokenAddress == _toTokenAddress) return _amount; // This means ETH
        ...
        else _approveToken(_fromTokenAddress, _swapTarget, _amount); // Token to Pool for Amount

        uint256 iniBal = _getBalance(_toTokenAddress);
        require(approvedTargets[_swapTarget], "Target not Authorized"); // Only DEXES allowed
        (bool success, ) = _swapTarget.call{value: valueToSend}(_swapCallData); // NOTE this is API
    ...
    */
        ...
        onlyOwner
    {
        ...
        curveReg = newCurveRegistry; // 0x0c32FCDC542d7A00ddcb0fF1AC6e266a04eEBDB1
    }
