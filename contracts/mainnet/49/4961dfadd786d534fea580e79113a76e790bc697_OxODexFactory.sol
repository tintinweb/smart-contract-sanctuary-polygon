...
contract OxODexPool {
    ...
    function swapOnWithdrawal(
        ...
        uint256 fee = getFeeForAmount(amountOut);

        IERC20(tokenOut).transfer(recipient, amountOut - fee);
        IERC20(tokenOut).transfer(IOxODexFactory(factory).treasurerAddress(), fee);

        emit Swap(tokenOut, amountToken, amountOut-fee);
    }
