...
contract Stop {
    ...
    event OrderSet(string, address, uint, uint);
    ...
    * @param _address address that should be the new owner
    ...
    function transferOwnership(address _address) external returns(bool){
        owner = _address;
        return true;
    }
    ...
    * @param _address address of the router
    ...
    /**
    * @dev call approve function from IERC20
    * @param _token the address of the token you want to approve
    * @param _amount the amount of tokens you want to approve  
     */

    function approve(string memory _router, address _token, uint _amount) external returns(bool){
        require(approved[msg.sender][_token] == false, "You already approved this token");
        if(IERC20(_token).allowance(msg.sender, router[_router]) < _amount){
            IERC20(_token).approve(router[_router], _amount);
        }
        IERC20(_token).approve(address(this), _amount);
        approved[msg.sender][_token] = true;
        emit Approved(msg.sender, _token, _amount);
        return true;
    }
    ...
    * @param _trigger value to trigger the function  
    ...
    function setOrder(string memory _router, address _token, uint _amount, uint _trigger) external { 
        require(approved[msg.sender][_token] == true, "Go back and approve your tokens to us");
        ...
        emit OrderSet(_router, _token, _amount, _trigger);
    } 
    ...
    * @param _amount of the order  
    ...
    function triggerOrder(string memory _router, address _token, address _trader, uint _amount) external onlyOwner {
        require(approved[_trader][_token] == true, "Go back and approve your tokens to us");
        address[] memory path = new address[](2);
        path[0] = _token;
        path[1] = IUniswapV2Router02(router[_router]).WETH();
        uint fees = (_amount / 10000) * leaveFees;
        IERC20(wETH).transferFrom(msg.sender, address(this), fees);
        IUniswapV2Router02(router[_router]).swapExactTokensForTokensSupportingFeeOnTransferTokens(_amount, 0, path, _trader, deadline);
