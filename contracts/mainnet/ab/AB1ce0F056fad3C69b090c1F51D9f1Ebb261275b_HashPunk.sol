...
contract HashPunk is ERC721rUpgradeable, HashPunkStorage {
    ...
    {
        return string(abi.encodePacked(baseMetadataURI, _uint2str(tokenId), ".json"));
    }
        ...
        if (from != address(0)) {
            ...
            for (uint256 i = 0; i < index; i++) {
                if (userToRareIds[from][i] == tokenId) {
                    userToRareIds[to].push(userToRareIds[from][i]);
                    userToRareIds[from][i] = userToRareIds[from][index - 1];
                    userToRareIds[from].pop();
                    return;
                }
            }
        }
