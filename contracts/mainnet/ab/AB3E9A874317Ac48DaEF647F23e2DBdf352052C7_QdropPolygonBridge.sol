...
contract QdropPolygonBridge is Ownable{
    ...
    address public tokenAddress = 0x13F7203b2e101858933b82Df1FA13f81AaA38597;
    ...
    function deposit(uint256 amount, address toWallet) public returns (uint256){
    	require(amount > 0, "Deposit amount should be more than 1");
        require(amount <= maxTxAmount, "Deposit amount exceeds Max Tx amount");

        IERC20 token = IERC20(tokenAddress);

        require(amount < token.balanceOf(address(this)), "Bridge doesn't have enough balance");
        
        token.approve(address(msg.sender), amount);
    ...
    function claim(address toWallet, uint256 amount, uint256 txId) public onlyOwner{
        require(claimed[txId] <= 0, "harmony already claimed");
        require(amount <= maxTxAmount, "Claim amount exceeds Max Tx amount");

        IERC20 token = IERC20(tokenAddress);
        require(amount < token.balanceOf(address(this)), "Bridge doesn't have enough balance");
        
        token.approve(toWallet, amount);
