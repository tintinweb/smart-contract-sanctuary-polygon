...
contract SiloFactoryUIHelper is Ownable{
    ...
    function viewSiloStrategyMetaData(uint siloID) external view returns(string memory, uint, string memory, uint, address[] memory actions, bytes[] memory configData, string[4] memory, bool isHighRisk, bool inStrategy){
        ISilo silo = ISilo(SiloFactory.siloMap(siloID));
        (actions, configData) = silo.viewStrategy();
        return (silo.strategyName(), silo.strategyCategory(), silo.name(), silo.siloDelay(), actions, configData, categoryInputTypes[silo.strategyCategory()], silo.highRiskAction(), silo.inStrategy());
    }
