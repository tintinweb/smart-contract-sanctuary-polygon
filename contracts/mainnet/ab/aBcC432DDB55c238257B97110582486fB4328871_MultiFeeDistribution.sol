...
contract MultiFeeDistribution is IMultiFeeDistribution, Ownable {
    ...
    address public stakingTokenVault;

    // team fee
    address public teamVault;
    uint public teamFee = 2000; // 1% = 100
    ...
    constructor(address _stakingToken, address _stakingTokenVault, address _teamVault) Ownable() {
        ...
        stakingTokenVault = _stakingTokenVault;
    }

    function setTeamVault(address vault) external onlyOwner {
      teamVault = vault;
    }

    function setTeamFee(uint fee) external onlyOwner {
    ...
    function mint(address user, uint256 amount, bool withPenalty) external override {
        ...
        if (amount == 0) return;
        _updateReward(user);
        stakingToken.safeTransferFrom(stakingTokenVault, address(this), amount);
    ...
    function _adjustReward(address _rewardsToken, uint reward) internal returns (uint adjustedAmount) {
      if (reward > 0 && teamVault != address(0) && _rewardsToken != address(stakingToken)) {
        uint feeAmount = reward.mul(teamFee).div(10000);
        adjustedAmount = reward.sub(feeAmount);
        if (feeAmount > 0) {
          IERC20(_rewardsToken).safeTransfer(teamVault, feeAmount);
        }
      } else {
