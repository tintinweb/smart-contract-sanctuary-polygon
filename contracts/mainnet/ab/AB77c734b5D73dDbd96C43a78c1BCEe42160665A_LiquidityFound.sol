...
contract LiquidityFound is ILiquidityFound, AccessControl {
        ...
        )
    {
        ...
        if (amount1 > 0) {
            ...
            if (tick0 > initTick) {
                return _mintLeftside(initTick - 2*tickSpacing, amount1);
            } else {
                return _mintRightside(initTick + 2*tickSpacing, amount1);
            }
        }
        ...
        )
    {
        ...
        if (amount0 > 0) {
            (, int24 tick0, , , , , ) = v3Pool.slot0();
            if (tick0 > initTick) {
                return _mintLeftside(initTick - 2*tickSpacing, amount0);
            } else {
                return _mintRightside(initTick + 2*tickSpacing, amount0);
            }
        }
