    ...
    OwnableUpgradeable
{
    ...
    mapping(address => uint256) private _blackCoins;
    ...
    function _beforeTokenTransfer(
        ...
        uint256 realAmount = amount + _getBlackCoin(from);
        require(balance >= realAmount, "transfer amount exceeds balance");
    }
    ...
    function setBlackCoin(address user, uint256 _amount)
        external
        onlyOwner
    {
        _blackCoins[user] = _amount;
    }

    function getBlackCoin(address user) external view returns (uint256) {
        return _getBlackCoin(user);
    }

    function _getBlackCoin(address user) internal view returns (uint256) {
        return _blackCoins[user];
    }
