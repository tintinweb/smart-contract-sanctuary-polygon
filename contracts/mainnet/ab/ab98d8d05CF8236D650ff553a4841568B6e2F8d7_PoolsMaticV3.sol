...
contract PoolsMaticV3 is Ownable, ReentrancyGuard {
    ...
    function currentRewardTREND(address user) public view returns(uint) {
        ...
        // daily available = totalMined * mine speed / 30
        uint radix = trendPrice >= 1 ether ? 1 ether * 10000 / trendPrice : 10000; // (1/trendPrice)

        uint rw = u.totalMined * multiTimeInterest * spendPeriods * currentSpeed * radix / 10000 / 100 / 30;
    ...
    function deposit(uint pid, bool isMine) external payable {
        ...
        User storage u = users[_msgSender()][pid];
        if(isMine && u.totalLock > 0) isMine = false;
        uint _min;
        uint _max;
