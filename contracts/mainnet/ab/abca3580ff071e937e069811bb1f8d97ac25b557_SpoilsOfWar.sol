...
contract SpoilsOfWar is Context, Ownable {
	...
	event Invest(address indexed addr, uint256 amount, uint40 time, uint8 fleet, uint8 category);
	...
	function supportFleet(address referral, uint8 fleet, uint8 category) public payable {
		...
		if(category == 2){
			fleet_aircrafts[player.fleet-1] += SafeMath.div(msg.value, 5 ether);
		}else{
		...
		devSupport(msg.value,6);     
		mkgSupport(msg.value,4);     

    	commissionPayouts(msg.sender, msg.value);
		
		emit Invest(msg.sender, msg.value, uint40(block.timestamp), fleet, category);		
    }
    ...
    function switchAllegiance(uint8 fleet) public payable {
        ...
        devSupport(msg.value,8);     
		mkgSupport(msg.value,2);     
	...
	function endGame(uint256 report, uint256 score) external {
		...
		if(player.battles.length >= 1) {
			...
			{			
				...
				if(player.battles[len].result > 50 && report>0){
    ...
    function computePayout(address _addr) view external returns(uint256 value) {
		...
		uint256 levelno = SafeMath.div(player.downlines1.length,100) + SafeMath.div(player.total_battle_score, 100000);			
    ...
    function playerLevel(address _addr) view external returns(uint256 level) {
		Player storage player = players[_addr];
        return( (SafeMath.div(player.downlines1.length,100) + SafeMath.div(player.total_battle_score, 100000)) + 1);
	}
