...
contract Game is Ownable {  
  ...
  uint256 internal _totalGameCredits;
  ...
  function addCredits(address player_, uint256 amount_) external onlyOwner {
    ...
    if ( _totalGameCredits + amount_ > pokerDaoChips.balanceOf(address(this)) )  { revert NotEnoughCredits(); }

    // increase the internal credits for a player in the game
    gameCredits[player_] += amount_;
  }
}
