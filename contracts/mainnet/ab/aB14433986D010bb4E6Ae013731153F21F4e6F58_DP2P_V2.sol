...
contract DP2P_V2 is ReentrancyGuard {
    ...
    function setTokenAddress(string memory tokenName, address _token)
        external
        onlyAdmin
    {
    ...
    function postAd(uint256 amount, string memory tokenName) external payable {
        ...
        if (_token != address(0) && isTokenContract(_token)) {
            ...
            require(msg.value == totalAmount, "Insufficient Ether Balance");
        }
        ...
        emit AdPosted(msg.sender, adId, totalAmount);
    }
    ...
    function updatePostAd(
        ...
        emit AdUpdated(adId, oldAmount, totalAmount);
    }
    ...
    function getAssignedAmount(address recipient)
        external
        view
        returns (uint256)
    {
    ...
    function getAssignedAdID(address recipient)
        external
        view
        returns (uint256)
    {
