...
/* 
feecollection: removed fee collection from contract because it was bunch of SSTORE and SLOAD operations which were consuming a lot of gas along with a storage slot.
there was no way to matic come in to contract except maticToMultiSwap function so address(this).balance was the fee collected.
setTokenAggregators: added onlyOwner modifier to setTokenAggregators function because giving access to anyone to set aggregators can be dangerous.
need to add index to the event based on how you need graph api to work.
need to fix one license here it was GPL and in Wallet it was MIT.

@important - always try to use ERC20ToMultiSwapWithReceiver and MaticToMultiSwapWithReceiver function because it is cheaper and more gas efficient.
*/
...
contract SwapToken {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "CLAMPV1: NOT OWNER");
        _;
    }

    constructor(address _owner, uint8 _fee, address payable _WMaticAddress) {
        owner = _owner;
        fee = _fee;
    ...
    function addInDefinedTokenWithAggregator(
        address _tokenAddress,
        address _tokenAggregatorAddress
    ) external onlyOwner {
        require(!definedTokens[_tokenAddress], "CLAMPV1: TOKEN ADDRESS EXISTS"); // Check that the address is not already in the mapping
        definedTokens[_tokenAddress] = true; // Add the address to the mapping
        tokenToAggregator[_tokenAddress] = _tokenAggregatorAddress;
    }

    function removeFromDefinedTokenWithAggregator(
        address _tokenAddress
    ) external onlyOwner {
        require(
            definedTokens[_tokenAddress],
            "CLAMPV!: TOKEN ADDRESS DOES NOT EXISTS"
        ); // Check that the address is in the mapping
        definedTokens[_tokenAddress] = false; // Remove the address from 'definedTokens' mapping
        tokenToAggregator[_tokenAddress] = address(0); //Remove the tokenAggregator address from 'tokenToAggregator' mapping
    }

    function isDefinedToken(
        address _tokenAddress
    ...
    function MaticToMultiSwapWithReceiver(
        ...
        require(
            ...
            "CLAMPV1: INDEX-LENGTH-INVALID"
        );
        require(
            _tokenAddresses.length == _tokenAmounts.length,
            "Mismatched token addresses and amounts"
        );
    ...
    function multiSwapHelper(
        ...
        if (wallets[_receiver] == address(0)) {
            createWallet(_receiver);
        }

        //Fetch Wallet Contract Explicitly from address
        Wallet walletAddress = Wallet(payable(wallets[_receiver]));
    ...
    // internal functions
    function createWallet(address _user) internal {
        require(wallets[_user] == address(0), "wallet already exists");
        // address _wallet = Clones.clone(walletImplementation);
        address payable clone = payable(Clones.clone(walletImplementation));
        Wallet _wallet = Wallet(clone);
        _wallet.initialize(_user);

        wallets[_user] = address(_wallet);
        emit WalletCreated(address(_wallet), _user);
    }
...
contract Wallet is Initializable {
   ...
   */
    function update(
        ...
        uint _indexIdentifier = uint(
            keccak256(abi.encodePacked(_tokenAddresses))
        );
