...
contract CaiLookup is Ownable {
    ...
    function add( string memory _code ,  string memory _value  ) public  onlyOwner returns(bool) {
        valueByCode[_code] = _value;
        return EnumerableSetString.add(set,_code);
    }

    function remove( string memory _code ) public  onlyOwner returns(bool) {
        delete valueByCode[_code] ;
        return EnumerableSetString.remove(set, _code);
    }
    ...
    function length() public view returns (uint256){
        return EnumerableSetString.length(set);
    }
