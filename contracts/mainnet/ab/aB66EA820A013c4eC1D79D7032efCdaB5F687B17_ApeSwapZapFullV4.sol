...
contract ApeSwapZapV2 is IApeSwapZapV2, ReentrancyGuard {
    ...
    function _zapPrivate(
        ...
        if (address(routers[1]) == address(ARRAKIS_ROUTER)) {
            vars.uniV3Pool = IUniswapV3Factory(ARRAKIS_ROUTER.factory())
                .getPool(lpTokens[0], lpTokens[1], uniV3PoolFees[2][0]);
        ...
        if (address(routers[1]) == address(ARRAKIS_ROUTER)) {
            (vars.amountA, vars.amountB, ) = ARRAKIS_ROUTER.addLiquidity(
                IGUniPool(getArrakisPool(vars.uniV3Pool)),
                vars.amount0,
                vars.amount1,
                minAmounts[1][0],
                minAmounts[1][1],
    ...
    function getSwapRatio(
        ...
        vars.arrakisPool = getArrakisPool(uniV3Pool);
        (vars.underlying0, vars.underlying1) = IGUniPool(vars.arrakisPool)
            .getUnderlyingBalances();
        vars.token0decimals = IApePair(address(token0)).decimals();
        vars.token1decimals = IApePair(address(token1)).decimals();

        vars.weightedPrice0 = inputToken == token0
            ? 1e18
            : getWeightedPrice(paths[0], uniV3PoolFees[0]);
        vars.weightedPrice1 = inputToken == token1
            ? 1e18
            : getWeightedPrice(paths[1], uniV3PoolFees[0]);

        vars.percentage0 =
            (((vars.underlying0 * 10 ** (18 + 18 - vars.token0decimals)) /
                ((vars.underlying0 * 10 ** (18 - vars.token0decimals)) +
                    (vars.underlying1 * 10 ** (18 - vars.token1decimals)))) *
                vars.weightedPrice0) /
            (vars.weightedPrice0 + vars.weightedPrice1);

        vars.percentage1 =
            (((vars.underlying1 * 10 ** (18 + 18 - vars.token1decimals)) /
                ((vars.underlying0 * 10 ** (18 - vars.token0decimals)) +
                    (vars.underlying1 * 10 ** (18 - vars.token1decimals)))) *
                vars.weightedPrice1) /
            (vars.weightedPrice0 + vars.weightedPrice1);
    ...
    function getWeightedPrice(
        address[] calldata path,
        uint24[] memory uniV3PoolFees
    ) public view returns (uint256 weightedPrice) {
        weightedPrice = 1e18;
        for (uint256 index = 0; index < path.length - 1; index++) {
            weightedPrice =
                (weightedPrice *
    ...
    function pairTokensAndValue(
        ...
        address tokenPegPair = IUniswapV3Factory(ARRAKIS_ROUTER.factory())
        ...
        require(size != 0, "ApeSwapZap: UniV3 pair not found");
