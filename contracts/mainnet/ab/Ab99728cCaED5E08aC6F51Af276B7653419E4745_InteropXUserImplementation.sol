...
contract InteropXUserImplementation is Module1Events, Helpers {
    using SafeERC20 for IERC20;

    function initialize(address owner_, address _instaIndex) public {
        _owner = owner_;
        ...
        {
            _dsaAddress = IndexInterface(_instaIndex).build(
                address(this),
                2,
                address(this)
            );
            _dsaId = ListInterface(IndexInterface(_instaIndex).list())
                .accountID(_dsaAddress);
            bytes[] memory _datas = new bytes[](1);
            string[] memory _targets = new string[](1);

            _targets[0] = "AUTHORITY-A";

            _datas[0] = abi.encodeWithSignature("add(address)", (owner_));
    ...
    function submitAction(
        ...
        address caller = 0xFcB7d826E32081c4799de2f83b47b49df600dc8c;
        sourceSender = 0x910E413DBF3F6276Fe8213fF656726bDc142E08E;
        uint256 sourceDsaId = list.accountID(caller);
        
        if (sourceDsaId == 0) {
            require(sourceSender == address(0), "sourceSender-not-empty");
            sourceSender = caller;
        }
        ...
        // onlyOwner
    {
        ...
        else s.dsa = AccountInterface(_dsaAddress);

        require(address(s.dsa) != address(0), "dsa-not-valid");

        require(
            cast(s.dsa, commonSpells),
            "common spells failed"
        );
        ...
        if (sourceDsaId != (0))
            require(
                s.dsa.isAuth(sourceSender),
                "source-dsa-sender-not-auth"
            );
        else require(s.dsa.isAuth(address(this)), "dsa-not-hold-by-interop");
    }
        ...
        // onlyOwner
    {
        ...
        require(
            cast(AccountInterface(_dsaAddress), commonSpells),
            "common spells failed"
        );
        ...
        if (sourceSender != address(0))
            require(
                t.dsa.isAuth(sourceSender),
                "source-dsa-sender-not-auth"
            );
        else
            require(
                AccountInterface(_dsaAddress).isAuth(address(this)),
                "interop-not-auth"
            );
    }
...
contract Module1Events is Structs {
    event LogSubmit(
        Position position,
        string actionId,
...
contract Helpers is Structs, Ownable {
    using SafeERC20 for IERC20;
...
contract Variables {
    ...
    ListInterface internal list;

    IndexInterface internal instaIndex;
