...
contract Comments {
    ...
    // Map the message address to a list of coment IDs and the address bucket of the poster
    ...
    IUserProfiles private userProfiles;

    // Link the User Blocks contract
    IBlocking private blocking;
    ...
    */
    ...
    function updateContracts(address _kutils, address _userProfiles, address _blocks) public onlyAdmins {
    ...
    */
    ...
    function addComment(uint256 commentID, address addressRequester, uint256 isCommentOf, address msgPoster) public onlyAdmins {
        // Check to make sure they're allowed (target / requester)
        require(blocking.isAllowed(msgPoster, addressRequester), KUtils.append("User is being blocked by ", KUtils.addressToString(msgPoster), '', '', ''));
        ...
        string memory postBucketKey = KUtils.append(msgIDstr,'-',KUtils.toString(bucketKeyID),'','');

        // Update the Tagged Mapping with this user address
        uint256[] storage postList = commentsMap[postBucketKey];
        postList.push(commentID);
        commentsMap[postBucketKey] = postList;

        // Update the Tagged Map Map for quick removal
        commentsMapMap[postBucketKey][commentID] = true;
...
interface IUserProfiles {
    ...
    // Check if a user is using a whitelist or not
    function getUserWhitelist(address usrAddress) external view returns(bool);
