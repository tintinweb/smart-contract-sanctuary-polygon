...
contract HRVSTToken is ERC20('HRVST Token', 'HRVST', 8), Ownable {
    ...
    modifier onlyAuthorized(){
        require(isAuthorized[msg.sender] || owner() == msg.sender, "Address not Authorized");
        _;
    }

    function setAuthorized(address account, bool _authorized) external onlyOwner {
        isAuthorized[account] = _authorized;
    }

    // other ecosystem contracts need the ability to mint tokens as well as the owner.
    ...
    function burnTokens(uint256 amount) external onlyAuthorized {
        require(balanceOf(msg.sender) >= amount, "burnTokens: Not enough tokens to burn");
        _burn(msg.sender, amount);
        tokensBurnedByWallet[msg.sender] += amount;
