...
contract VoyagerDepositHandler is AccessControl, ReentrancyGuard, Pausable {
    ...
    // address public immutable routeToken;
 ...
 */
    ...
    function iRelay(
        ...
        address _recipient = bytesToAddress(recipient);
        if (destToken == address(wrappedNativeToken)) {
            IERC20(destToken).safeTransferFrom(
                msg.sender,
                address(this),
                amount
            );
            _unwrapwrappedNativeTokenTo(payable(_recipient), amount);
        } else
            IERC20(destToken).safeTransferFrom(msg.sender, _recipient, amount);
        ...
        uint256 newNonce = depositNonce++;
        emit FundsPaid(messageHash, msg.sender, chainId(), newNonce);
    }
