...
interface IOndoCashManager {
    function requestMint(uint256 collateralAmountIn) external;
    function requestRedemption(uint256 amountCashToRedeem) external;

    // Function used by users to claim an airdrop for a given epoch
    function claimMint(address user, uint256 epochToClaim) external;
}

/**
 * @title Strategy for Ondo Finance
 * @dev At the time of this writing, Ondo is available on the Ethereum MainNet only, and it accepts USDC only.
 */
contract OndoStrategy is FractBaseTokenizedStrategy, ReentrancyGuard {
    using SafeERC20 for IERC20;

    address public constant USDC_ADDRESS = address(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174);
    address public constant ONDO_CASH_MANAGER_ADDRESS = address(0xd3978611616aa6712aeD96E58AD243993f108C75);
    address public constant ONDO_OUSG_ADDRESS = address(0xe25D3fd9dcd38450D8A155441801801046b662a1);
    ...
    constructor(string memory receiptTokenName, string memory receiptTokenSymbol, uint8 receiptTokenDecimals) FractBaseTokenizedStrategy(receiptTokenName, receiptTokenSymbol, receiptTokenDecimals) {
    }

    function addLiquidity(uint256 collateralAmountIn) external nonReentrant onlyOwnerOrOperator {
        IERC20(USDC_ADDRESS).safeApprove(ONDO_CASH_MANAGER_ADDRESS, collateralAmountIn); 
        IOndoCashManager(ONDO_CASH_MANAGER_ADDRESS).requestMint(collateralAmountIn);
        IERC20(USDC_ADDRESS).safeApprove(ONDO_CASH_MANAGER_ADDRESS, 0); 
        emit OnAddLiquidity(collateralAmountIn, block.timestamp, USDC_ADDRESS);
    }

    function removeLiquidity(uint256 numberOfOUSGTokens) external nonReentrant onlyOwnerOrOperator {
        IERC20(ONDO_OUSG_ADDRESS).safeApprove(ONDO_CASH_MANAGER_ADDRESS, numberOfOUSGTokens); 
        IOndoCashManager(ONDO_CASH_MANAGER_ADDRESS).requestRedemption(numberOfOUSGTokens);
        IERC20(ONDO_OUSG_ADDRESS).safeApprove(ONDO_CASH_MANAGER_ADDRESS, 0); 
        emit OnRemoveLiquidity(numberOfOUSGTokens, block.timestamp, ONDO_OUSG_ADDRESS);
    }

    function claimRewards(uint256 epochToClaim) external nonReentrant onlyOwnerOrOperator {
        IOndoCashManager(ONDO_CASH_MANAGER_ADDRESS).claimMint(address(this), epochToClaim);
        emit OnRewardsClaimed(epochToClaim);
    }
