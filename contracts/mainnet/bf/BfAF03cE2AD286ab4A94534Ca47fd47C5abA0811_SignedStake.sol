...
contract SignedStake is ISignedStake, TokensRecoverable
{
    ...
    function calculateExcess() internal {
        uint256 excess;
        //check for 0 stakers
        if (stakerTracker.levelOneStakers == 0)
        {
            excess += stakerRates[0] * maticPaidOut / 10000;
        }
        if (stakerTracker.levelTwoStakers == 0)
        {
            excess += stakerRates[1] * maticPaidOut / 10000;
        }
        if (stakerTracker.levelThreeStakers == 0)
        {
            excess += stakerRates[2] * maticPaidOut / 10000;
        }
        if (stakerTracker.levelFourStakers == 0)
        {
            excess += stakerRates[3] * maticPaidOut / 10000;
        }
        if (stakerTracker.levelFiveStakers == 0)
        {
            excess += stakerRates[4] * maticPaidOut / 10000;
        }
