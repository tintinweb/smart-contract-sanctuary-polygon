...
// TODO:
// Reduce Modifiers (gas inefficient)
// Add Refund/Return functions for Item Offers, Collection Offers, and Listings
// Reduce functionality where possible

contract PhenixNFTMarketplace is
    ...
    OwnableUpgradeable
{
    ...
    function createMarketplaceSaleWithERC20(uint256[] calldata itemIds) public {
        uint256 totalPrice = 0;
    ...
    function createMarketplaceOfferWithETH(
        ...
        require(
            idToMarketplaceItem[itemId].status == Status.AVAILABLE,
            "Item not available."
        );
    ...
    function getMarketplaceItemOffers(
        ...
        uint256 startIndex,
        uint256 endIndex
    ) public view returns (MarketplaceItemOffer[] memory) {
    ...
    function getMarketplaceCollectionOffers(
        ...
        uint256 startIndex,
        uint256 endIndex
    ) public view returns (MarketplaceCollectionOffer[] memory) {
        require(startIndex <= endIndex, "Invalid range");
        if (startIndex == 0 && endIndex == 0) {
    ...
    function getMarketplaceItemsByIndexes(
        uint256[] memory indexes
    ) public view returns (MarketplaceItem[] memory) {
        MarketplaceItem[] memory items = new MarketplaceItem[](indexes.length);
        for (uint256 i = 0; i < indexes.length; i++) {
            items[i] = idToMarketplaceItem[indexes[i]];
        }
        return items;
    }
