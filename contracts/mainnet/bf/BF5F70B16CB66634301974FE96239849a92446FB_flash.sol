...
interface IWBNB {
    function balanceOf(address dst) external returns (uint);
...
contract flash is FlashLoanReceiverBase, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 cheapRouter;
    IUniswapV2Router02 expensiveRouter;
    uint deadline;
    IERC20 token;
    IWBNB wbnb;
    ...
    returns (bool)
    {
        emit OperationStarted(amounts[0], assets[0]);
        emit CheckingBalance(address(this).balance, wbnb.balanceOf(address(this)));
        amountToTrade = amounts[0]; // how much wei you want to trade

        // change borrowed weth to eth
        wbnb.withdraw(amounts[0]);
        //emit WBNBWithdraw(amounts[0]);

        // execute arbitrage strategy
        executeArbitrage();

        // change eth back to weth to pay back the loan
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            IWBNB(assets[i]).approve(address(LENDING_POOL), amountOwing);
            emit RePaying(amountOwing, assets[i]);
        }
    ...
    function executeArbitrage() public {
        ...
        try cheapRouter.swapExactETHForTokens{
            value: amountToTrade
        }(
            0,
        ...
        emit FirstSwap(amountToTrade, tokenAmount);
        // grant uniswap / sushiswap access to your token, DAI used since we're swapping DAI back into ETH
        // token.approve(address(quickswapV2Router), tokenAmount);
        token.approve(address(expensiveRouter), tokenAmount);

        // Trade 2: Execute swap of the ERC20 token back into ETH on Sushiswap to complete the arb
        try expensiveRouter.swapExactTokensForETH (
            tokenAmount,
            0,
        ...
        emit SecondSwap(tokenAmount, address(this).balance);
    }
    ...
    function myFlashLoanCall(
        ...
        IUniswapV2Router02 _cheapRouter,
        IUniswapV2Router02 _expensiveRouter
    ) public {
        cheapRouter = IUniswapV2Router02(address(_cheapRouter));
        expensiveRouter = IUniswapV2Router02(address(_expensiveRouter));
        ...
        deadline = block.timestamp + 3000; // 50 minutes

        address receiverAddress = address(this);

        address[] memory assets = new address[](1);
        assets[0] = address(_flashAsset); // WETH
    ...
    function WithdrawBalance() public payable onlyOwner {
        ...
        // withdraw all wbnb
        uint256 wethbalance = wbnb.balanceOf(address(this));
        if (wethbalance > 0) {
