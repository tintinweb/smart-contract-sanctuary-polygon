...
contract SpecMatch is IFight, Ownable {
    ...
    // Declare a new state variable to store the random numbers used in each round
    mapping(uint256 => mapping(uint256 => mapping(uint256 => uint256[2]))) public fightRoundRandomNumbers;
    ...
    function formRoster() public {
        ...
        uint256[] memory _randomWords = new uint256[](randomNumberCount);

        for (uint256 i = 0; i < randomNumberCount; i++) {
            _randomWords[i] = uint256(keccak256(abi.encodePacked(block.timestamp + i)));
        }
    ...
    function execute() public {
        ...
        uint32 randomNumberCount = uint32(fights[eventIndex].fightCards.length) * 14;
    ...
    function playMatch(uint256[] memory randomNumbers) internal {
        ...
        for (uint256 i = 0; i < fights[eventIndex].fightCards.length; i++) {
            require((currentRandomNumber + 14) <= randomNumbers.length, "Not enough random numbers");

            uint256[] memory luchador1Numbers = new uint256[](7);
            uint256[] memory luchador2Numbers = new uint256[](7);

            // Round calculations for each Luchador
            for (uint256 round = 0; round < 7; round++) {
                uint256 attr1;
                uint256 attr2;
                if (round < 3) {
                    attr1 = luchador.luchadoresAttributes(fights[eventIndex].fightCards[i].luchador1, 3); // style
                    attr2 = luchador.luchadoresAttributes(fights[eventIndex].fightCards[i].luchador2, 3); // style
                } else if (round < 7) {
                    attr1 = luchador.luchadoresAttributes(fights[eventIndex].fightCards[i].luchador1, 4); // health
                    attr2 = luchador.luchadoresAttributes(fights[eventIndex].fightCards[i].luchador2, 4); // health
                }

                uint256 attr3 = luchador.luchadoresAttributes(fights[eventIndex].fightCards[i].luchador1, round % 3); // speed, strength, stamina
                uint256 attr4 = luchador.luchadoresAttributes(fights[eventIndex].fightCards[i].luchador2, round % 3); // speed, strength, stamina

                luchador1Numbers[round] = attr1 * attr3 * ((randomNumbers[currentRandomNumber] % 20) + 1);
                luchador2Numbers[round] = attr2 * attr4 * ((randomNumbers[currentRandomNumber + 1] % 20) + 1);

                fightRoundRandomNumbers[eventIndex][i][round][0] = (randomNumbers[currentRandomNumber] % 20) + 1;
                fightRoundRandomNumbers[eventIndex][i][round][1] = (randomNumbers[currentRandomNumber + 1] % 20) + 1;

                currentRandomNumber += 2;
            }

            // Sum up the scores for each Luchador
            uint256 luchador1Score = 0;
            uint256 luchador2Score = 0;
            for (uint256 round = 0; round < 7; round++) {
                luchador1Score += luchador1Numbers[round];
                luchador2Score += luchador2Numbers[round];
            }

            // Determine the winner
            if (luchador1Score > luchador2Score) {
            ...
            } else if (luchador2Score > luchador1Score) {
    ...
    function deleteFight(uint256 eventIndex) public onlyOwner {
        delete fights[eventIndex];
    }
    ...
    function getRandomNumbersForFightCard(uint256 eventIndex, uint256 fightCard) public view returns (uint256[2][7] memory) {
        uint256[2][7] memory roundNumbers;
        for (uint256 round = 0; round < 7; round++) {
            roundNumbers[round] = fightRoundRandomNumbers[eventIndex][fightCard][round];
        }
        return roundNumbers;
    }
    ...
    function fulfillRandomWords(
        ...
        if (fights[eventIndex].fightCardsGenerated)
        {
