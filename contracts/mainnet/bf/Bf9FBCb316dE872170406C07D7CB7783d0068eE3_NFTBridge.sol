...
contract NFTBridge is
    ...
    Ownable
{
    // @notice Linking signature verifying library with {bytes32} type.
    using ECDSA for bytes32;

    uint256 public nextDepositId = 1;

    /**
     * @dev {Deposit} represents data about the deposited NFT.
     * owner Address representing owner of NFT.
     * collection Address of NFT collection to which the token belongs.
    ...
    // @notice List of active deposits.
    mapping(uint256 => Deposit) public depositById;

    // @notice Mapping of (user address => list of deposit ids)
    mapping(address => uint256[]) public depositIdsByUser;

    // @notice Mapping of (nft address => token id => depositId)
    mapping(address => mapping(uint256 => uint256)) public depositIdByToken;

    // @notice Mapping of (nft address => approve status)
    mapping(address => bool) public approvedNFT;

    // @notice Mapping of (requestId => execution status)
    mapping(uint256 => bool) public executed;

    // @notice List of addresses of {oracles}.
    address[] public oracles;

    // @notice Minimum number of valid signatures required to reach quorum.
    uint256 public threshold;

    // @notice {bool} indicating if consensus has been initialized.
    bool public consensusInitialized;

    /**
     * @notice Returns a boolean value indicating if give {interfaceId} is
     * supported by the bridge or not.
     * @dev {supportsInterface} function allows successful receiving of
     * NFTs from smart contracts implementing ERC721 or ERC1155 standard.
     *
     * @param interfaceId The interface id to check for implementation.
     * @return bool Bool value to show if the provided {interfaceId} is implemented.
     */
    ...
    /**
     * @notice Allows initial set up of consesus.
     ...
     * @return success Bool value to show success status of the transaction.
     */
        ...
        returns (bool success)
    {
        require(
            ...
            "NFTBridge:initConsensus:: consensus has been already initialized"
        );
        consensusInitialized = true;

        return
            _setConsensus(initialOracles, initialThreshold);
    }

    /**
     * @notice Sets a collection's approval status.
    ...
    /**
     * @notice Updates the bridge's consensus.
     ...
     * @param newThreshold The new {threshold} required to reach quorum.
        ...
        returns (bool success)
    {
        ...
        return
            _setConsensus(newOracles, newThreshold);
    }
     ...
     * owner retrieved from the mapping {depositById}.
     *
     * The function is publicly callable but the recipient of the NFT
     * is always the owner retrieved from {depositById} mapping.
     *
     * It updates the mapping {depositIdsByUser} to delete the deposit entry
     * corresponding to each token in {tokenIds}. Also the {Deposit} itself is
     * removed from the storage by being deleted from mappings {depositIdByToken}
     * and {depositById}.
     *
     * Requirements:
     * - The token/s being claimed must not be locked.
     *
     * @param collection The address of token contract of which the NFTs
     * are being claimed.
     * @param tokenIds The Ids of the tokens to be claimed from contract {collection}.
     * @return success Bool value to show success status of the transaction.
     */
    function claim(
        address collection,
        ...
        returns (bool success)
    {
        bool is721 = ERC165(collection)
            .supportsInterface(type(IERC721).interfaceId);

        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 depositId = depositIdByToken[collection][tokenIds[i]];
            Deposit memory deposit = depositById[depositId];
            require(
                !deposit.locked,
            ...
            _removeDepositEntryFromUser(depositId, deposit.owner);
            delete depositIdByToken[deposit.collection][deposit.tokenId];
            delete depositById[depositId];
        }

        emit NFTClaimed(collection, tokenIds);
        return true;
    }

    /**
     * @notice Called by token owner to teleport their transferred NFT/s.
     ...
     *
     * The NFT contract address {collection} must be approved for successfully
     ...
     *
     * Requirements:
     * - The {msg.sender} must be the owner of NFT/s in {deposits} mapping.
     * - A token must not be locked.
     *
     * @param collection The address of NFT contract.
     * @param tokenIds The list of Ids of tokens from {collection}.
        ...
        returns (bool success)
    {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 depositId = depositIdByToken[collection][tokenIds[i]];
            Deposit storage deposit = depositById[depositId];
            require(
    ...
    /**
     * @notice Is called by the consensus operator to unlock the locked NFT.
     * @dev {unlock} function allows unlocking of NFT by the quorum of signatures
     * from {oracles}.
     *
     * Upon unlocking it unlocks the token and sets {newOwner} as the owner of NFT
     ...
     * @param collection The address of NFT contract.
     * @param tokenIds The id/s of NFT tokens.
        ...
        returns (bool success)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (deposit.owner != newOwner) {
                _removeDepositEntryFromUser(
                    depositId,
                    deposit.owner
                );

                deposit.owner = newOwner;
                depositIdsByUser[newOwner].push(depositId);
            }
            deposit.locked = false;
        }

        emit NFTUnlocked(collection, tokenIds, newOwner);
        return true;
    }

    /**
     * @notice Allows recovery of accidently send tokens to the bridge.
     * @dev {recover} allows recovering of NFTs that are sent using
     * by the depositIdsByUser using {transferFrom} function instead of {safeTransferFrom}
     * function. Since transfer of NFTs with {transferFrom} does not call into
     * bridge contract upon transfer, there is no record of token data against
     * the transferred NFT rendering user not being able to {teleport} the NFT
     * and the bridge is not able to unlock it either.
     *
     * The {recover} function allows oracles to sign data with identifier of {0x0011}
     * to transfer out the unrecorded NFTs out of the bridge. The signed data contains
     * the token's address, NFT ids and owners' addresses to send the NFTs to.
     *
     * Requirements:
     * - The provided list of {signatures} reach quorum.
     * - The token must not have record in {deposits} mapping.
     *
     * @param requestId unique request identifier, must never repeat.
     * @param collection The address of NFT contract.
     * @param tokenIds The array of ids of NFTs.
     * @param tokenOwners The array of owners of NFTs.
     * @param signatures The list of signatures from oracles that are checked to decide consensus.
     * @return success Bool value to show success status of the transaction.
     */
    function recover(
        ...
        returns (bool success)
    {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(
                depositId == 0,
                "NFTBridge::recover: token is not recoverable"
            );
        }
        ...
        require(
            _validateSignaturesAndTx(signatures, hash, requestId),
            "NFTBridge::recover: consensus failed"
        );
    ...
    /**
     * @notice A hook called when ERC721 token is transferred to the bridge.
     ...
     *
     * It populates the {deposits} mapping with the data of NFT transfer.
     *
     * @param from The sender of NFT token.
     * @param tokenId The id of the NFT token being transferred.
    ...
    function onERC721Received(
        ...
        returns (bytes4)
    {
        registerDeposit(from, tokenId);
        return
            IERC721Receiver.onERC721Received.selector;
    }

    /**
     * @notice A hook called when ERC1155 token is transferred to the bridge.
    ...
    function onERC1155Received(
        ...
        returns (bytes4)
    {
        registerDeposit(from, id);
        return
            IERC1155Receiver.onERC1155Received.selector;
    }

    /*
     * @notice A hook called when ERC1155 token batch is transferred to the bridge.
     ...
     *
     * @param from The sender of NFT token.
     * @param ids The list of ids of NFT tokens.
    ...
    /**
     * @notice Returns the Deposit data associated with token.
     * @dev It fetched {depositId} from the mapping {depositIdByToken} and
     * retrieves the deposit data from the mapping {depositById}.
     *
     * requirements:
     * - {depositId} must not be zero.
     * @param collection The address of collection.
     * @param tokenId The id of the token.
     * @return Deposit The Deposit data against the provided token.
     */
    function depositDataByCollectionAndId(
        address collection,
        uint256 tokenId
    )
        external
    ...
    {
        uint256 depositId = depositIdByToken[collection][tokenId];
        require(
            depositId != 0,
            "NFTBridge:depositDataByCollectionAndId:: token data not found"
        );

        return depositById[depositId];
    }

    /**
     * @notice Returns the list of NFT owned by the {user}.
     * @dev It gets the deposit ids for user deposited NFTs and
     * populates {depositListByUser} with NFT deposit data.
     *
     * @param user The address of user to return the owned list of Deposits.
     * @return depositListByUser The list deposits owned by user.
     */
    function tokensByUser(address user)
        external
        view
        returns (Deposit[] memory depositListByUser)
    {
        uint256[] memory depositIds = depositIdsByUser[user];
        depositListByUser = new Deposit[](depositIds.length);

        for (uint256 i = 0; i < depositIds.length; i++) {
            depositListByUser[i] = depositById[depositIds[i]];
        }
    }
     ...
     * @param _hash The list of message hashes corresponding to {_signatures}.
    ...
    /**
     * @notice Removes {Deposit} entry from the list of user owned deposits.
     * @dev It removes deposit represented by {depositId} in mapping {depositIdsByUser}.
     *
     * @param depositId The id of the deposit to remove from user's owned deposit list.
     * @param owner The address of the owner.
     */
    function _removeDepositEntryFromUser(
        uint256 depositId,
        address owner
    ) private {
        uint256[] storage depositIds = depositIdsByUser[owner];
        uint256 length = depositIds.length;
        for (uint256 i = 0; i < length; i++) {
            if (depositIds[i] == depositId) {
                depositIds[i] = depositIds[length - 1];
                break;
            }
        }
        depositIds.pop();
    ...
    /**
     * @notice Record the NFT transfer as {Deposit} in bridge.
     * @dev It constructs {Deposit} struct against the transferred NFT's data.
     * and populates mappings to keep track of it through collection and token id,
     * and also through the list maintained against each user's owned list of
     * deposits.
     *
     * @param from The sender/depositor of NFT token being deposited.
    ...
    function registerDeposit(
        ...
        depositById[currentDepositId] = deposit;
        depositIdByToken[collection][tokenId] = currentDepositId;

        depositIdsByUser[from].push(currentDepositId);
    }
}
...
interface INFTBridge {
    ...
    event NFTClaimed(address collection, uint256[] tokenIds);

    event NFTLocked(address collection, uint256[] tokenIds);
