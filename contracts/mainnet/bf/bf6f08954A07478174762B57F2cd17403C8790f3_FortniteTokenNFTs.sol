...
---------------------- [ fntoken.tech ] ----------------------
    ...
    |  Flavors
    |
    |  >  Fortnite Token NFTs v.2: Standard ERC-721 Token with Marketplace
...
/**
 * @title Fortnite Token NFTs
 * ERC-721 Marketplace with tokens and royalties support
 */

contract FortniteTokenNFTs is ERC721Mochi2, ReentrancyGuard {
        ...
        ERC721Mochi2(_owner, name, symbol, _anyoneCanMint) 
    {
        ...
        require(_commissionRate<=100, "FortniteTokenNFTs: Commission rate has to be between 0 and 100");
    ...
    function sell(uint256 tokenId, uint256 price, address payable wallet) public {
        ...
        // onlyOwner
        require(ownerOf(tokenId)==msg.sender, "FortniteTokenNFTs: Only owner can sell this item");

        // cannot set a price if auction is activated
        require(!auctions[tokenId].open, "FortniteTokenNFTs: Cannot sell an item which has an active auction");
    ...
    function buy(uint256 tokenId) public nonReentrant {
        ...
        // is on sale
        require(!auctions[tokenId].open && sellBidPrice[tokenId]>0, "FortniteTokenNFTs: The collectible is not for sale");

        // transfer ownership
        address owner = ownerOf(tokenId);

        require(msg.sender!=owner, "FortniteTokenNFTs: The seller cannot buy his own collectible");
    ...
    function createAuction(uint256 tokenId, uint _closingTime, address payable _beneficiary, uint256 _reservePrice) public {
        ...
        require(sellBidPrice[tokenId]==0, "FortniteTokenNFTs: The selected NFT is open for sale, cannot be auctioned");
        require(!auctions[tokenId].open, "FortniteTokenNFTs: The selected NFT already has an auction");
        require(ownerOf(tokenId)==msg.sender, "FortniteTokenNFTs: Only owner can auction this item");
    ...
    function bid(uint256 tokenId, uint256 bid_value) public nonReentrant {
        ...
        address owner = ownerOf(tokenId);
        require(msg.sender!=owner, "FortniteTokenNFTs: The owner cannot bid his own collectible");
