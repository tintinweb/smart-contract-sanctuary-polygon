...
import "../Common/FeeConfigManager.sol";
import "../../utils/StringUtils.sol";
import "../../utils/GasThrottler.sol";

contract StrategyCommonChefLP is FeeConfigManager, GasThrottler {
    ...
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _beefyFeeRecipient) FeeConfigManager(_beefyFeeConfig) public {
        want = _want;
        poolId = _poolId;
        chef = _chef;
    ...
    function chargeFees(address callFeeRecipient) internal {
        (uint256 totalFee, uint256 beefyFee, uint256 callFee, uint256 strategistFee,,) = beefyFeeConfig.getFees();

        uint256 toNative = IERC20(output).balanceOf(address(this)).mul(totalFee).div(DIVISOR);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toNative, 0, outputToNativeRoute, address(this), now);

        uint256 nativeBal = IERC20(native).balanceOf(address(this));

        uint256 callFeeAmount = nativeBal.mul(callFee).div(DIVISOR);
        IERC20(native).safeTransfer(callFeeRecipient, callFeeAmount);

        uint256 beefyFeeAmount = nativeBal.mul(beefyFee).div(DIVISOR);
        IERC20(native).safeTransfer(beefyFeeRecipient, beefyFeeAmount);

        uint256 strategistFeeAmount = nativeBal.mul(strategistFee).div(DIVISOR);
    ...
    function callReward() public view returns (uint256) {
        (uint256 totalFee,, uint256 callFee,,,) = beefyFeeConfig.getFees();
        ...
        return nativeOut.mul(totalFee).div(DIVISOR).mul(callFee).div(DIVISOR);
    }
...
abstract contract FeeConfigManager is StratManager {
    ...
    constructor(address _beefyFeeConfig) public {
    ...
    function setBeefyFeeConfig(address _beefyFeeConfig) public onlyOwner {
        beefyFeeConfig = IFeeConfig(_beefyFeeConfig);
    }
