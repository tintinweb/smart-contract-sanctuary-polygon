...
contract CBridgeFacet is ReentrancyGuard, Signers {
    using SafeERC20 for IERC20;

    IBridge private immutable CBRIDGE;
    address private immutable WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;

    constructor(IBridge cBridge) {
        CBRIDGE = cBridge;
    }
    ...
    function swapCBridge(SwapData calldata _swap, CBridgeDescription calldata bDesc) external payable nonReentrant {
        ...
        uint256 dstAmount = LibPlexusUtil._tokenDepositAndSwap(swapData);
    ...
    function sigWithdraw(
        ...
        LibData.DiamondData storage ds = LibData.dataStorage();
    ...
    function _cBridgeStart(uint256 amount, CBridgeDescription calldata bdesc) internal {
        ...
        amount = LibPlexusUtil._fee(bDesc.srcToken, amount);
        bool isNotNative = !LibPlexusUtil._isNative(bDesc.srcToken);
        if (isNotNative) {
            IERC20(bDesc.srcToken).safeApprove(address(CBRIDGE), amount);
        ...
        emit LibData.Bridge(tif.user, tif.chainId, tif.dstToken, tif.amount, tif.nonce, transferId, tif.bridge);
    }
}
...
interface ISigsVerifier {
     ...
     * @param _powers powers of current signers
     */
    function verifySigs(bytes memory _msg, bytes[] calldata _sigs, address[] calldata _signers, uint256[] calldata _powers) external view;
}
...
library LibPlexusUtil {
    ...
    function _swapStart(SwapData calldata swapData) internal returns (uint256 dstAmount) {
        ...
        bool isNative = _isNative(swap.srcToken);
        IERC20(swap.srcToken).approve(swap.swapRouter, swap.amount);
        uint256 initDstTokenBalance = getBalance(swap.dstToken);
        (bool succ, ) = swap.swapRouter.call{value: isNative ? swap.amount : 0}(swap.callData);
        if (succ) {
