...
// File: ContractEnkardia/ContractEnkardia/Pass.sol
...
contract PassContract is ERC2981, ERC721URIStorage, ERC721Enumerable, DefaultOperatorFilterer, Ownable {    

    mapping (uint => address) public originalMinter;
    mapping (address => bool) public whitelist;
    uint public open;
    uint public fcfsOpen;

    constructor() ERC721("Enkardia - Pass", "PASS") {
        _setDefaultRoyalty(msg.sender, 500);
        open = 1686232800;
        fcfsOpen = open + 8 hours;
    }
    
    function mintPass() external returns (uint) {
        require((block.timestamp >= open && whitelist[msg.sender]) || (block.timestamp >= fcfsOpen), "Unable to mint now.");
        require(totalSupply() < 1000, "All minted.");
        require(balanceOf(msg.sender) < 1, "Max 1 per address.");
        uint id = totalSupply();
        _safeMint(msg.sender, id);
        originalMinter[id] = msg.sender;
        return id;   
    }
    ...
    function mintState() view external returns(uint state) {
        if (block.timestamp < open) state = 0;
        else if (block.timestamp >= open && block.timestamp < fcfsOpen) state = 1;
        else if (block.timestamp >= fcfsOpen) state = 2;
        return state;
    }
    ...
    function tokenURI(uint256 tokenId) public pure override(ERC721, ERC721URIStorage) returns (string memory) {
        tokenId = 0;
        return string(abi.encodePacked("ipfs://", "QmWagohRDtPjQJCcmw9wEBza8iL1VJhs6q4wbzdmBXQM1T"));
    }
