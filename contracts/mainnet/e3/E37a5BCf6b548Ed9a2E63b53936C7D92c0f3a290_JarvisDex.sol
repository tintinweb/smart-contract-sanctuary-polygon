...
/**
 * @title JarvisDex
 * @notice Integrates Jarvis' SynthereumMultiLpLiquidityPool for any pair(JTRY/USDC in this case) to mint and redeem
 */
contract JarvisDex is ReentrancyGuard {
    ...
    /**
     * @notice Allows user to deposit USDC and mint JTRY (valid for any available pair).
     * @param _mintParams see IJarvisDex
     * @param _poolAddress Address of Jarvis' SynthereumMultiLpLiquidityPool for the pair
     * @return syntheticTokensMinted amount synthetic tokens(JTRY in this case) minted
     * @return feePaid feesPaid to mint
     */
    function mint(
        IJarvisDex.MintParams calldata _mintParams,
    ...
    {
        // fetching collateral token
        address collateralToken = IJarvisDex(_poolAddress).collateralToken();
        // transferring collateral tokens from sender
        IERC20(collateralToken).safeTransferFrom(
            msg.sender,
            address(this),
            _mintParams.collateralAmount
        );
        // giving allowance to pool
        IERC20(collateralToken).safeIncreaseAllowance(
            _poolAddress,
            _mintParams.collateralAmount
        );
        // calling pool to mint, minted synthetic tokens will be transfered to recipient in _mintParams
        (syntheticTokensMinted, feePaid) = IJarvisDex(_poolAddress).mint(
            _mintParams
        );
    }

    /**
     * @notice Allows user to deposit deposit JTRY and redeem USDC (valid for any available pair).
     * @param _redeemParams see IJarvisDex
     * @param _poolAddress Address of Jarvis' SynthereumMultiLpLiquidityPool for the pair
     * @return collateralRedeemed amount of Underlying token(USDC in this case) redeemed
     * @return feePaid feesPaid to redeem
     */
    function redeem(
        IJarvisDex.RedeemParams calldata _redeemParams,
        address _poolAddress
    )
        external
        nonReentrant
    ...
    {
        // fetching synthetic token
        address syntheticToken = IJarvisDex(_poolAddress).syntheticToken();

        // transferring syntheticToken tokens from sender
        IERC20(syntheticToken).safeTransferFrom(
            msg.sender,
            address(this),
            _redeemParams.numTokens
        );
        // giving allowance to pool
        IERC20(syntheticToken).safeIncreaseAllowance(
            _poolAddress,
            _redeemParams.numTokens
        );

        // calling pool to redeem, redeemed tokens will be transfered to recipient in _redeemParams
        (collateralRedeemed, feePaid) = IJarvisDex(_poolAddress).redeem(
            _redeemParams
        );
    }
    ...
    function getMintTradeInfo(
        ...
        (synthTokensReceived, feePaid) = IJarvisDex(_poolAddress)
            .getMintTradeInfo(_collateralAmount);
    }
    ...
    {
        (collateralAmountReceived, feePaid) = IJarvisDex(_poolAddress)
            .getRedeemTradeInfo(_syntTokensAmount);
    }
}