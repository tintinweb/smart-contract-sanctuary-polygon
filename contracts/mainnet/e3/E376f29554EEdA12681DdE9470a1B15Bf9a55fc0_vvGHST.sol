...
contract vvGHST is Initializable, ERC20Upgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function depositCollateral(uint256 vaultID, uint256 amount) public onlyApproved{
        QiVault(s.vaultAddress).depositCollateral(vaultID, amount);
    }

    function withdrawCollateral(uint256 vaultID, uint256 amount) public onlyApproved{
        QiVault(s.vaultAddress).withdrawCollateral(vaultID, amount);
    }

    function borrowToken(uint256 vaultID, uint256 amount) public onlyApproved{
        QiVault(s.vaultAddress).borrowToken(vaultID, amount);
    }

    function payBackToken(uint256 vaultID, uint256 amount) public onlyApproved{
        QiVault(s.vaultAddress).payBackToken(vaultID, amount);
    }
    ...
    function enter(uint256 _amount) public whenNotPaused returns(uint256){
        ...
        if(debtRemaining == 0){
            ...
            require(s.supplyCap - totalShares > vvGHSTAmount,"insufficient supply cap");
        }
   ...
   /* function mint(address _receiver, uint256 _amount) public onlyOwner{
    ...
    function burnBalance(address _receiver) public onlyOwner{
        _burn(_receiver,balanceOf(_receiver));
    }*/
    ...
    function _rebalanceVault() internal {
        ...
        if(debtValue == 0 || CDR > s.targetCDRHigh){
            ...
            /*if(debtAvailable == 0 && CDR > 405){

                //400 == collateralValue*100 / debtValue
                //400 * debtValue == collateralValue*100
                //(vaultBalance*QiVault(s.vaultAddress).getEthPriceSource())*100 == 400 * debtValue
                //vaultBalance == 4*debtValue / QiVault(s.vaultAddress).getEthPriceSource()

                uint256 targetCollateral = (350*debtValue) / (QiVault(s.vaultAddress).getEthPriceSource()*100);
                QiVault(s.vaultAddress).withdrawCollateral(s.vaultID, vaultBalance - targetCollateral);
            }
            else*/ if(debtAvailable < newDebt){
    ...
    function totalvGHST() public view returns(uint256 _totalvGHST){
        ...
        uint256 vaultvGHST = QiVault(s.vaultAddress).vaultCollateral(s.vaultID);
        uint256 walletvGHST = IERC20Upgradeable(im_vGHST).balanceOf(address(this));
        _totalvGHST = vaultvGHST + walletvGHST;
    }
    ...
    function vQiClaimable(address _user) public view returns(uint256 totalQi) {
        ...
        for(uint256 i = 1; i < s.qiAirdrops.length; i++){
