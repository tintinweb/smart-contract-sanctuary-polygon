...
import "./libs/app/StringUtil.sol";
...
contract Citizen is Auth {
  ...
  function initialize(address _mn) public initializer {
    Auth.init(_mn);
    defaultInviter = _mn;
  }
  ...
  function register(string calldata _userName, address _inviter) external returns (uint) {
    ...
    soulBoundNft.mint(msg.sender, 1);
  ...
  function isSameLine(address from, address to) external view returns (bool) {
    address inviter;
    while (from != address(0)) {
      inviter = residents[from].inviter;
      if (inviter == to) {
        return true;
      }
      from = inviter;
  ...
  function setWhiteList(address _address, bool _status) external onlyMn {
    require(_address != address(0), "Citizen: invalid address");
    whiteList[_address] = _status;
    emit SetWhiteListed(_address, _status);
  }

  function updateInviter(address[] calldata _addresses, address _inviter) external onlyMn {
  ...
  function updateDefaultInviter(address _inviter) external onlyMn {
  ...
  function _initDependentContracts() override internal {
    address soulBoundNftAddress = addressBook.get("soulBoundNft");
    require(soulBoundNftAddress != address(0), "Citizen: please setup soulBoundNft");
    soulBoundNft = INFTPass(soulBoundNftAddress);
  }
}
