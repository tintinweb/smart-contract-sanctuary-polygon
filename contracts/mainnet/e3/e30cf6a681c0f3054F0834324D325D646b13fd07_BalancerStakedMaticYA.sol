...
import "./interfaces/balancer/IRewards.sol";
import "./BaseYA.sol";
import "./libraries/GiddyUniswap.sol";
import "./GiddyConfigYA.sol";

contract BalancerStakedMaticYA is BaseYA, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  uint256 constant private COMPOUND_THRESHOLD = 1e16;
  address constant private WMATIC_TOKEN = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
  address constant private STMATIC_TOKEN = 0x3A58a54C066FdC0f2D55FC9C89F0415C92eBf3C4;
  address constant private BST_TOKEN = 0x8159462d255C1D24915CB51ec361F700174cD994;
  address constant private BSTG_TOKEN = 0x2Aa6fB79EfE19A3fcE71c46AE48EFc16372ED6dD;
  address constant private BALANCER_TOKEN = 0x9a71012B13CA4d3D0Cdc72A177DF3ef03b0E76A3;
  address constant private BALANCER_POOL = 0xBA12222222228d8Ba445958a75a0704d566BF2C8;
  address constant private BALANCER_HELPER = 0x239e55F427D44C3cc793f49bFB507ebe76638a2b;
  address constant private BALANCER_REWARDS = 0x2Aa6fB79EfE19A3fcE71c46AE48EFc16372ED6dD;
  bytes32 constant private POOL_ID = 0x8159462d255c1d24915cb51ec361f700174cd99400000000000000000000075d;
  address constant private UNISWAP_QUOTER = 0xb27308f9F90D607463bb33eA1BeBb41C27CE5AB6;
  address constant private UNISWAP_ROUTER = 0xE592427A0AEce92De3Edee1F18E0157C05861564;
  ...
  function getContractBalance() public view override returns (uint256) {
    return IERC20(BSTG_TOKEN).balanceOf(address(this));
  }

  function getContractRewards() public view override returns (uint256) {
    return IRewards(BALANCER_REWARDS).claimable_reward(address(this), BALANCER_TOKEN) + IERC20(BALANCER_TOKEN).balanceOf(address(this));
  }
  ...
  function compound() public override whenNotPaused {
    IRewards(BALANCER_REWARDS).claim_rewards();
    uint256 earnings = IERC20(BALANCER_TOKEN).balanceOf(address(this));
    if (earnings > COMPOUND_THRESHOLD) {
      earnings = GiddyUniswap.swapTokensSimpleV3(UNISWAP_ROUTER, BALANCER_TOKEN, WMATIC_TOKEN, 10000, address(this), earnings);
      earnings = GiddyUniswap.swapTokensSimpleV3(UNISWAP_ROUTER, WMATIC_TOKEN, USDC_TOKEN, 500, address(this), earnings);
      earnings = deductFeeUsdc(earnings, earnings * GIDDY_CONFIG.earningsFee() / BASE_PERCENT);

      uint256 preBalance = getContractBalance();
      join(earnings);
      emit Compound(getContractBalance() - preBalance, lastCompoundTime, preBalance);
      lastCompoundTime = block.timestamp;
    }
  }

  function depositCalc(uint256 value, uint256 fap) external override returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = GiddyUniswap.calcPriceSimpleV3(UNISWAP_ROUTER, UNISWAP_QUOTER, USDC_TOKEN, WMATIC_TOKEN, 500, value - fap);
    amountOut = joinPool(amountOut, true);
  }
  ...
  function depositGiddyCalc(uint256 value, uint256 fap) external override returns (uint256 amountOut, uint256 priceImpact) {
    priceImpact = value - fap;
    amountOut = priceImpact;
    (priceImpact, amountOut) = GiddyUniswap.calcSwap(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, priceImpact, amountOut);
    (priceImpact, amountOut) = GiddyUniswap.calcSwapV3(UNISWAP_ROUTER, UNISWAP_QUOTER, USDC_TOKEN, WMATIC_TOKEN, 500, priceImpact, amountOut);
  ...
  function withdrawCalc(uint256 shares, uint256 fap) external override returns (uint256 amountOut, uint256 priceImpact) {
    amountOut = exitPool(sharesToValue(shares), true);
    (amountOut, priceImpact) = GiddyUniswap.calcPriceSimpleV3(UNISWAP_ROUTER, UNISWAP_QUOTER, WMATIC_TOKEN, USDC_TOKEN, 500, amountOut);
    amountOut -= fap;
  }
  ...
  function withdrawGiddyCalc(uint256 shares, uint256 fap) external override returns (uint256 amountOut, uint256 priceImpact) {
    amountOut = exitPool(sharesToValue(shares), true);
    priceImpact = amountOut;
    (priceImpact, amountOut) = GiddyUniswap.calcSwapV3(UNISWAP_ROUTER, UNISWAP_QUOTER, WMATIC_TOKEN, USDC_TOKEN, 500, priceImpact, amountOut);
    (priceImpact, amountOut) = GiddyUniswap.calcSwap(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, priceImpact, amountOut);
    priceImpact = GiddyUniswap.calcPriceImpact(priceImpact, amountOut);
    amountOut -= fap;
  }
  ...
  function deductFeeGiddy(uint256 amount, uint256 fap) internal returns (uint256) {
    if (fap > 0) {
      ...
      route[0] = GIDDY_TOKEN;
      route[1] = USDC_TOKEN;
      GiddyUniswap.swapTokens(SUSHI_ROUTER, route, GIDDY_CONFIG.feeAccount(), fap);
    }
    return amount - fap;
  }

  function joinPool(uint256 wmaticValue, bool query) private returns(uint256 amountOut) {
    IBalancer.JoinPoolRequest memory joinRequest;

    address[] memory assets = new address[](3);
    assets[0] = WMATIC_TOKEN;
    assets[1] = STMATIC_TOKEN;
    assets[2] = BST_TOKEN;
    joinRequest.assets = assets;

    uint256[] memory amounts = new uint256[](3);
    amounts[0] = wmaticValue;
    amounts[1] = 0;
    amounts[2] = 0;
    joinRequest.maxAmountsIn = amounts;
    joinRequest.fromInternalBalance = false;

    uint256[] memory userAmounts = new uint256[](2);
    userAmounts[0] = wmaticValue;
    userAmounts[1] = 0;

    joinRequest.userData = abi.encode(IBalancer.JoinKind.EXACT_TOKENS_IN_FOR_BPT_OUT, userAmounts, 0);
    if (query) {
      (amountOut,) = IBalancer(BALANCER_HELPER).queryJoin(POOL_ID, address(this), address(this), joinRequest);
    }
    else {
      if (!IERC20(WMATIC_TOKEN).approve(BALANCER_POOL, wmaticValue)) {
        revert();
      }
      IBalancer(BALANCER_POOL).joinPool(POOL_ID, address(this), address(this), joinRequest);
      amountOut = IERC20(BST_TOKEN).balanceOf(address(this));
    }
  }

  function join(uint256 value) private returns(uint256 shares) {
    value = GiddyUniswap.swapTokensSimpleV3(UNISWAP_ROUTER, USDC_TOKEN, WMATIC_TOKEN, 500, address(this), value);

    value = joinPool(value, false);
    shares = contractShares == 0 ? value * INIT_SHARES : value * contractShares / getContractBalance();

    if (!IERC20(BST_TOKEN).approve(BALANCER_REWARDS, value)) {
      revert();
    }
    IRewards(BALANCER_REWARDS).deposit(value);
  }

  function exitPool(uint256 bstValue, bool query) private returns (uint256 wmaticValue) {
    IBalancer.ExitPoolRequest memory exitRequest;
    ...
    amounts[1] = 0;
    amounts[2] = 0;
    exitRequest.minAmountsOut = amounts;
    exitRequest.toInternalBalance = false;

    exitRequest.userData = abi.encode(IBalancer.ExitKind.EXACT_BPT_IN_FOR_ONE_TOKEN_OUT, bstValue, 0);

    if (query) {
      (,amounts) = IBalancer(BALANCER_HELPER).queryExit(POOL_ID, address(this), address(this), exitRequest);
      return amounts[0]; 
    }
    else {
      IBalancer(BALANCER_POOL).exitPool(POOL_ID, address(this), address(this), exitRequest);
      return IERC20(WMATIC_TOKEN).balanceOf(address(this));
    }
  }

  function exit(uint256 shares) private returns (uint256 usdcValue) {
    ...
    IRewards(BALANCER_REWARDS).withdraw(value);
    value = exitPool(value, false);
    return GiddyUniswap.swapTokensSimpleV3(UNISWAP_ROUTER, WMATIC_TOKEN, USDC_TOKEN, 500, address(this), value);
  }
}
...
interface IBalancer {
  ...
  function queryJoin(bytes32 poolId, address sender, address recipient, JoinPoolRequest memory request) external returns (uint256 bptOut, uint256[] memory amountsIn);
  function queryExit(bytes32 poolId, address sender, address recipient, ExitPoolRequest memory request) external returns (uint256 bptIn, uint256[] memory amountsOut);
}
