...
library HashTransaction {
    ...
    bytes32 private constant UPDATE_ALL_TYPEHASH = keccak256("UpdateAll(uint256 nonce,address user,string bio,string website,string ipfsHash)");
    ...
    function hashUpdateAllTransaction(address verifyingContract, uint256 _nonce, address _user, string memory _bio,
        ...
        return keccak256(abi.encodePacked("\x19\x01", getDomainSeperator(verifyingContract), keccak256(abi.encode(UPDATE_ALL_TYPEHASH,
            _nonce, _user, keccak256(bytes(_bio)), keccak256(bytes(_website)), keccak256(bytes(_ipfsHash))))));
    }
