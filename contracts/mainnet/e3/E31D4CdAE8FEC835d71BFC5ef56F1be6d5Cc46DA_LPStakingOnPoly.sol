...
contract LPStakingOnPolyWrapper is ReentrancyGuardUpgradeable {
    ...
    function initialized(address _stakingToken, string memory _nameArg, string memory _symbolArg) public { 
...
contract LPStakingOnPoly is LPStakingOnPolyWrapper, OwnableUpgradeable {
    ...
    function initialize(
        ...
        __Ownable_init();
        LPStakingOnPolyWrapper.initialized(
            _stakingToken,
            _name,
            _symbol
        );
    ...
    */
    function claimReward() public updateReward(msg.sender) { 
        ...
        uint256 vebalance = veToken.balanceOf(msg.sender);
        if (reward > 0) {
            rewards[msg.sender] = 0;
            if (vebalance > 0)
                reward += reward * FULL_SCALE / vebalance / 1000 / 100;
    ...
    */
    ...
    function adjustBoostReward(int256 rewardDelta) external onlyOwner { 
        if (rewardDelta > 0) { rewardToken.transferFrom(this.owner(), address(this), uint256(rewardDelta)); }
        else { rewardToken.transfer(this.owner(), uint256(-rewardDelta)); }
    }
    ...
    function updateVeToken(address _veToken) external onlyOwner {
        veToken = IERC20(_veToken);
    }

    function updateBoostRate(uint256 _boostRate) external onlyOwner {
