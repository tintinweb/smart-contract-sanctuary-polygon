...
contract MATVENTURE {
    ...
    uint constant public WITHDRAW_COOLDOWN	= 24 hours;
	...
	constructor(address payable wallet)  {
		...
		plans.push(Plan(30, 41)); // 4.1% per day for 14 days
        plans.push(Plan(40, 34)); // 3.4% per day for 21 days
        plans.push(Plan(60, 25)); // 2.5% per day for 28 days
		plans.push(Plan(30, 53)); // 5.3% per day for 14 days (at the end)
        plans.push(Plan(40, 50)); // 5% per day for 21 days (at the end)
        plans.push(Plan(60, 50)); // 5% per day for 28 days (at the end)
	}
	...
	function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			if (user.checkpoint < user.deposits[i].finish) {
				if (user.deposits[i].plan < 3 || block.timestamp > user.deposits[i].finish) {
					uint256 share = user.deposits[i].amount.mul(user.deposits[i].percent.add(getContractBalanceRate())).div(PERCENTS_DIVIDER);
   ...
   function getContractBalanceRate() public view returns (uint256) {
	 ...
	 function getContractInfo() public view returns(uint256, uint256, uint256,uint256) {
        return(totalStaked, totalRefBonus, totalUsers,totalWithdrawn);
    }
