...
contract StrategyQuickSyrupV1 is StrategyBase {

    address public constant quick = 0x831753DD7087CaC61aB5644b308642cc1c33Dc13;
    address public syrupPool = 0xcC6Cc4e642c889742278CF302d5276E7eF498d66;//stake QUICK to earn
    address public rewardToken = 0xdF7837DE1F2Fa4631D716CF2502f8b230F1dcc32;
    address[] public rewardSwapPath;
    ...
    constructor(
        ...
        address _controller,
        address _timelock
    ) public StrategyBase(quick, _governance, _strategist, _controller, _timelock) {
        IERC20(quick).safeApprove(syrupPool, uint256(-1));
        //reward -> WETH -> WMATIC
        rewardSwapPath = new address[](3);
        rewardSwapPath[0] = rewardToken;
        rewardSwapPath[1] = weth;
        rewardSwapPath[2] = WMATIC;
    }
	
    // **** migrator ****

    function switchSyrup(address _newSyrup, address _newReward, address[] calldata _swapPath) external onlyBenevolent{
        require(_swapPath.length >= 2 && _swapPath[0] == _newReward && _swapPath[_swapPath.length - 1] == WMATIC, '!rewardSwapPath');
        
        uint256 _totalVal = balanceOfPool();
        _withdrawAll();

        IERC20(want).safeApprove(syrupPool, 0);
        IERC20(want).safeApprove(_newSyrup, uint256(-1));

        syrupPool = _newSyrup;
        rewardToken = _newReward;
        rewardSwapPath = _swapPath;
        
        deposit();
        require(_totalVal == balanceOfPool(), '!invalidSwitch');
    }
    
    // **** State View functions ****
	
    function balanceOfPool() public override view returns (uint256) {
        return IQuickSwapSyrup(syrupPool).balanceOf(address(this));
    }

    function getHarvestable() external view returns (uint256) {
        return IQuickSwapSyrup(syrupPool).earned(address(this));
    }
    ...
    function _convertWantToBuyback(uint256 _lpAmount) internal virtual override returns (address, uint256){
        if (_lpAmount > 0){
            ...
            _swapUniswap(want, WMATIC, _lpAmount);
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            IQuickSwapSyrup(syrupPool).stake(_want);
        }
    }
    ...
    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        ...
        uint256 _max = balanceOfPool();
        IQuickSwapSyrup(syrupPool).withdraw(_amount > _max? _max : _amount);		
    ...
    function _claimReward() internal {
        IQuickSwapSyrup(syrupPool).getReward();
    }
	
    function _convertReward() internal {
        uint256 _rewardAmt = IERC20(rewardToken).balanceOf(address(this));
        if (_rewardAmt <= 0){
            return;
        }		
        _swapUniswapWithDetailConfig(rewardToken, WMATIC, _rewardAmt, 1, rewardSwapPath, univ2Router2);
    }
    
    function getName() override external pure returns(string memory){
        return "StrategyQuickSyrupV1";
    }
}