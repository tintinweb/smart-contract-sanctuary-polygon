...
contract Dao {
    ...
    ABC ABCs;
    uint256 voting_time = 300;
    uint256 fees_As_token = 5 *10 **18;
    constructor() {
        owner = msg.sender;
        nextProposal = 1;
    ...
    function createProposal(string memory _description) public {
        require(ABCs.balanceOf(msg.sender) >= 30, "Proposal fees is 5 ABCs ");
        proposal storage newProposal = Proposals[nextProposal];
        newProposal.proposalar = msg.sender;
        newProposal.exists = true;
        newProposal.description = _description;
        newProposal.deadline = block.number + voting_time;
        ABCs.transferFrom(msg.sender,owner, fees_As_token);
    ...
    function voteOnProposal(uint256 _id, bool _vote) public {
        ...
        require(
            voteStatus[_id][msg.sender] == false,
    ...
    function countVotes(uint256 _id) public {
        ...
        require(Proposals[_id].exists, "This Proposal does not exist");
        require(block.number >= Proposals[_id].deadline,"Voting time is not ended");
    ...
    function result(uint256 _id) public view returns (bool){
        require(Proposals[_id].exists, "This Proposal does not exist");
        return Proposals[_id].passed;
    }
    function getproposal_detail(uint256 _id) public view returns(proposal memory){
        require(Proposals[_id].exists, "This Proposal does not exist");
    ...
    function change_voting_time(uint256 _voting_time) public {
        require(msg.sender == owner,"owner can call ");
        voting_time = _voting_time;
    }
    function change_voting_fess(uint256 _fees_As_token) public {
        require(msg.sender == owner,"owner can call ");
        fees_As_token = _fees_As_token;
    }
}