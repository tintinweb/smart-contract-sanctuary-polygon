...
contract CloudKey is
  ...
  ICloudKey
{
  // Create the roles on deployment to save computation
  bytes32 public MINT_TOKENS;
  bytes32 public CLAIM_TOKENS;
  bytes32 public UPDATE_TOKEN_URI;
  ...
  function initialize(
    ...
    require(_admin != address(0), "CloudKey : Invalid admin address");
    require(
      _backendWallet != address(0),
      "CloudKey : Invalid backend wallet address"
    );
    require(
      _trustedForwarder != address(0),
      "CloudKey : Invalid trusted forwarder address"
    );
    // Save these values in the initializer function, this is because it is an upgradeable contract
    MINT_TOKENS = keccak256("MINT_TOKENS");
    CLAIM_TOKENS = keccak256("CLAIM_TOKENS");
    UPDATE_TOKEN_URI = keccak256("UPDATE_TOKEN_URI");
    ...
    // Assign the relevant roles to the provided admin address
    _setupRole(MINT_TOKENS, _admin);
    _setupRole(DEFAULT_ADMIN_ROLE, _admin);

    _setupRole(CLAIM_TOKENS, _backendWallet);

    _setupRole(UPDATE_TOKEN_URI, _backendWallet);
    _setupRole(UPDATE_TOKEN_URI, _admin);

    contractURI = _contractURI;
  }

  /**
   * Helper function for checking if the supplied tokenId is held by the CloudKey contract. This is useful when users are claiming their NFT
   */
  function _isThisOwner(uint256 tokenId) internal view virtual returns (bool) {
  ...
  function validateClaimTokenSignature(
    ...
    address recoveredAddress = SigRecovery.recreateSigAndRecoverAddress(
      _to,
      _tokenId,
      _signature
    );

    return hasRole(CLAIM_TOKENS, recoveredAddress);
  }
  ...
  {
    uint256 mintStartIndex = totalSupply() + 1;

    for (uint256 i = 0; i < mintArgs.length; i++) {
      // Not doing a 0x address check here because we are relying on the MINT_TOKENS role wallet to pass the correct addresses for gas optimisation.
      uint256 mintIndex = mintStartIndex + i;
  ...
  function claimToken(
    ...
    // 0x address check
    require(_to != address(0), "CloudKey: Invalid address");
    // Custom revert message if the recovered signing address doesn't contain the required role
    require(
      validateClaimTokenSignature(_to, _tokenId, _signature),
      "CloudKey: Invalid Signature"
    );

    require(
      _isThisOwner(_tokenId),
      "CloudKey: TokenId not held by CloudKey contract"
    );

    _safeTransfer(address(this), _to, _tokenId, "");

    emit TokenClaimed(_tokenId, _to);
  }

  // This function will be used when updating the tokenURI's from clouds to CloudKeys
  function batchSetTokenURI(SetTokenURIArgs[] calldata setTokenURIArgs)
    external
    override
    onlyRole(UPDATE_TOKEN_URI)
  {
    for (uint256 i = 0; i < setTokenURIArgs.length; i++) {
      setTokenURI(setTokenURIArgs[i]._tokenId, setTokenURIArgs[i]._tokenURI);
    }
  }
    ...
    onlyRole(UPDATE_TOKEN_URI)
  {
    ...
    emit TokenURIUpdated(_tokenId, _tokenURI, _msgSender());
  }
    ...
    onlyRole(UPDATE_TOKEN_URI)
  {
    ...
    emit TokenInvalidated(_tokenId, _msgSender(), block.timestamp);
  }
  ...
  /**
   * Allow this contract to receive NFT's
   */
  ...
  // **********************************************************************
  // Polygon Opensea Overrides
  // (https://docs.opensea.io/docs/polygon-basic-integration)
  // **********************************************************************

  /**
   * Allows the usage of meta transactions within the ERC721 contract
   */
  function _msgSender()
    internal
    view
    ...
    returns (address sender)
  {
    ...
    if (msg.data.length >= 20 && isTrustedForwarder(msg.sender)) {
      ...
      // Otherwise use the ContextMixin logic
      sender = ContextMixin.msgSender();
    }
  }

  /**
   * Gas Station Network requirements
   */
    ...
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    require(
      _trustedForwarder != address(0),
      "Cannot set forwarder to 0 address"
    );
    _setTrustedForwarder(_trustedForwarder);
  }
  ...
  // **********************************************************************
  // Required parent contract function overrides
  // **********************************************************************
  ...
  /**
   * Stub this internal overriden _burn function to disable burning
   */
...
// VENDOR: <Vendor name>, URL: https://docs.opensea.io/docs/polygon-basic-integration
pragma solidity 0.8.10;
...
/**
 * Helper library for signature verification
 */
library SigRecovery {
  /// @notice Helper function to recreate the message hash and calculate the signing address
  /// @dev The signature was created by signing the _to and _tokenId parameters
  /// @param _to  The receiving address of the token
  /// @param _tokenId  The token ID being distributed
  /// @param _signature  The signature that was created
  /// @return recoveredAddress Returns the recovered signing address
  function recreateSigAndRecoverAddress(
    address _to,
    uint256 _tokenId,
    bytes memory _signature
  ) internal pure returns (address recoveredAddress) {
    bytes32 messageHash = recreateMessageHash(_to, _tokenId);
  ...
  /// @notice Helper function for recreating the message hash, the message hash is created with the _to and _tokenId params. This required when verifying the signer of a signature.
  /// @param _to  The receiving address of the token
  /// @param _tokenId  The token ID being distributed
  /// @return messageHash Returns the message hash
  function recreateMessageHash(address _to, uint256 _tokenId)
    internal
    pure
    returns (bytes32 messageHash)
  {
    messageHash = keccak256(
      abi.encodePacked(
        ...
        keccak256(abi.encode(_to, _tokenId))
      )
    );
  }
