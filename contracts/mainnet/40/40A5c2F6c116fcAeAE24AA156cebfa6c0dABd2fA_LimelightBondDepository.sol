...
contract LimelightBondDepository is Ownable {
    ...
    IERC20 public immutable Clout; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints Clout when receives principle
    ...
    function deposit( 
        ...
        uint balanceBefore = Clout.balanceOf(address(this));
        /**
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            Clout.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == Clout.balanceOf(address(this)), "Not enough Clout to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Clout.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Clout.approve( address(stakingHelper), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Clout.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
