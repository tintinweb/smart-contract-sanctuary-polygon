...
contract Totee {
    using IterableUserStorage for IterableUserStorage.UserStorage;
    IterableUserStorage.UserStorage private users;

    // mapping(address => User) public users;

    function submit(bytes32 _username) public {
        users.set(msg.sender, User({username: _username}));
    }

    function getUsername() public view returns (bytes32) {
        return users.get(msg.sender).username;
    }
}