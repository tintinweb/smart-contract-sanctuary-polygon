 ...
 * @member feeAbsolute Agreed fee amount for order in output token.
 * @member feePercent Agreed fee amount in percents, where 1_00 equals one percent. Is not used when `feeAbsolute` is set.
...
contract AnbotoExecV2 is Ownable, EIP712 {
    ...
    bytes32 private constant ORDER_TYPEHASH =
        keccak256(
            "Order(address inputToken,uint256 totalAmount,address outputToken,uint256 outMin,uint256 feeAbsolute,uint256 feePercent,uint256 deadline,uint256 salt)"
        );
    ...
    function executeOrder(
        ...
        outputToken.safeTransfer(
            _maker,
            swapOutputBalance - getFee(_order, sliceInputAmount, swapOutputBalance)
        );
    ...
    function settleCow(
        ...
        outputToken.safeTransfer(
            _maker,
            buyAmount - getFee(_order, sellAmount, buyAmount)
        );
    ...
    function getFee(
        ...
        if (_order.feeAbsolute > 0) {
            // Take absolute fee when specified.
            ...
            // slice as compared to the total order.
            feeAmount = (_order.feeAbsolute * _soldAmount) / _order.totalAmount;
        } else {
