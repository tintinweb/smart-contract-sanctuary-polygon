...
contract CampaignContract is ICampaignContract, Ownable, Initializable, ERC1155Holder {
    ...
    address private fractionContract;
    ...
    function createCampaign(address creator,
    ...
    /**
        Claim
    **/
    function claimFractionSuperfan(string memory campaignId, string memory fanId, address fanAddress) override external onlyOwner {
        SuperFan storage _sjfan = fanToCamp[campaignId][fanId];
        ...
        Campaign memory _camp = idToCampaign[campaignId];
        require(_camp.completed, "Campaign is not completed");

        // TODO: Transfer fragments token to user
        SJFractions _sjfractions = SJFractions(fractionContract);
        uint256 itemId = _sjfractions.getItemId(campaignId);
        _sjfractions.safeTransferFrom(address(this), fanAddress, itemId, _sjfan.numberOfFractions, "0x00");

        // Update sjfan info
        _sjfan.claimed = true;
    }

    function tranferToSJAssetWallet(string memory _campaignId) private onlyOwner{
        ...
        _sjfractions.safeTransferFrom(address(this), collectableCampaignsAssetWallet, itemId, _camp.fractionCount * 10 / 100, "0x00");
    }


    function claimFlexGoalReached(string memory _campaignId) override external onlyOwner{
        tranferToSJAssetWallet(_campaignId);
    }

    function claimFlexPartiallyReached(string memory _campaignId) override external onlyOwner{
        tranferToSJAssetWallet(_campaignId);
    }

    function claimFixGoalReached(string memory _campaignId) override external onlyOwner{
        tranferToSJAssetWallet(_campaignId);

    }

    function claimFixPartiallyReached(string memory _campaignId) override external onlyOwner{
        tranferToSJAssetWallet(_campaignId);
...
abstract contract ICampaignContract {
    ...
    function createCampaign(address creator, string memory _campaignId,string memory _uri, uint256 _raise, CampaignType _campaignType, uint256 _startDate,uint256 _endDate,
    ...
    function claimFlexGoalReached(string memory _campaignId) external virtual;
    function claimFlexPartiallyReached(string memory _campaignId) external virtual;
    function claimFixGoalReached(string memory _campaignId) external virtual;
    function claimFixPartiallyReached(string memory _campaignId) external virtual;
...
contract SJFractions is ERC1155, AbstractERC1155 {
    ...
    function getItemId(string memory _campaignId) public view returns(uint256){
        ...
        require(bytes(_camp.campaignId).length != 0, "SJFractions::getItemIdByCampaignId: Campaign is not exist");
    ...
    function setManager(address userAdress) public onlyOwner {
        _isManager[userAdress] = true;
    }
