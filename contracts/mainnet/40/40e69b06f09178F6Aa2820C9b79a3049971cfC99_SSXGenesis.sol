...
contract SSXGenesis is Operator, ReentrancyGuard {
    ...
    address public constant XSHARE = address(0x9f64D1aAcb129B844500119B50938c57aa9aD6E5);

    uint256 public xSharePerUsdc = 3000 ether;
    uint256 public totalUsdcCompensated;
    ...
    event CompensateXShareHolders(address indexed user, uint256 usdcAmount, uint256 xshareAmount);
    event Initialized();
    event Finalized();
    ...
    function finalizing(
        uint256 _xSharePerUsdc,
        uint256 _totalRewards
    ) external onlyOperator {
        require(!finalized, "Already finalized");
        xSharePerUsdc = _xSharePerUsdc;
        totalRewards = _totalRewards;
        tokenPerSecond = _totalRewards.div(runningTime);
    ...
    function calcUsdcCompensate(uint256 _xshare_amount) public view returns (uint256 _usdc_amount) {
        _usdc_amount = _xshare_amount.mul(10 ** 18).div(xSharePerUsdc).div(10 ** 12);
    }

    function compensateXshareHolders(address _user, uint256 _xshare_amount) internal {
        uint256 _usdc_for_comepensate = calcUsdcCompensate(_xshare_amount);
        totalUsdcCompensated = totalUsdcCompensated.add(_usdc_for_comepensate);
        IERC20(USDC).safeTransfer(_user, _usdc_for_comepensate);
        emit CompensateXShareHolders(_user, _usdc_for_comepensate, _xshare_amount);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        if (address(pool.token) == XSHARE) {
            compensateXshareHolders(msg.sender, _amount);
        }
        if (user.amount > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            //Not allow XSHARE early withdraw
            if (address(pool.token) == XSHARE) require(block.timestamp >= poolEndTime, "Genesis still running");
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        require(block.timestamp >= poolEndTime, "Genesis still running");
        PoolInfo storage pool = poolInfo[_pid];
