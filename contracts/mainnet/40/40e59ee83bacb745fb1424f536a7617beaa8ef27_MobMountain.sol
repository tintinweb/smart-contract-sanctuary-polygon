...
contract MobMountain is Pausable, VRFConsumerBaseV2, ConfirmedOwner  {
  ...
  uint256 public gemReward;
  uint256 public deploymentTime;
  uint256 public stakedMobCount; // purely for informational purposes;


  uint256[] public stakedTokenIds;
  uint8[100] public randomResultPool;

  mapping(uint16 => uint16) public mobRarityToMultipler;
  mapping(uint256 => bool) public mobIdToStakedState;
  mapping(uint256 => uint256) public mobTokenIdToRarity;
  ...
  enum RunResults { SAFE_WITH_GEMS, SAFE_WITHOUT_GEMS, DEATH }
  ...
  event RunComplete(uint256 indexed weekIndex, uint256 indexed raiderId, uint256 indexed mobId, uint8 runResult);
    ...
    ConfirmedOwner(msg.sender)
{
  ...
  function stakeMob(uint256 tokenId) external whenNotPaused {
    ...
    uint16 thisMultiplier = mobRarityToMultipler[uint16(mobTokenIdToRarity[tokenId])];
  ...
  function unstakeMob(uint256 tokenId) external whenNotPaused {
    require(IERC721(mobsAddress).ownerOf(tokenId) == msg.sender,"Not your mob");
  ...
  function beginRun(uint256 tokenId) external whenNotPaused  {
    uint256 thisWeek = getCurrentWeek();
    require(IERC721(raidersAddress).isApprovedForAll(msg.sender, address(this)),"Must have permission");
    require(stakedTokenIds.length > 0,"No Mobs staked");
    require(IERC721(raidersAddress).ownerOf(tokenId) == msg.sender,"Not your raider");
    require(weeksToRaidersToRunState[thisWeek][tokenId] == RunStates.NOT_RUN, "Already began run for this week");
  ...
  function finishRun(RequestStatus memory request) internal {
    // TODO Should this revert if ownership has changed since begin run?
    uint tokenId = request.tokenId;
    address raiderOwner = IERC721(raidersAddress).ownerOf(tokenId);

    if (IERC721(raidersAddress).isApprovedForAll(msg.sender, address(this))) {
      require(stakedTokenIds.length > 0,"No Mobs staked");
      ...
      require(request.fulfilled,"VRF not fufilled yet");

      weeksToRaidersToRunState[request.weekIndex][tokenId] = RunStates.RETRIEVED;

      uint8 finalRunResult = uint8(request.randomWords[0] % randomResultPool.length) + 1;

      uint pickedMobTokenId = (request.randomWords[0] % stakedTokenIds.length) + 1;

      if (finalRunResult == uint8(RunResults.SAFE_WITH_GEMS)) {
        ERC20PresetMinterPauser(gemsAddress).mint(raiderOwner, gemReward);
      } else {
        address mobOwner = IERC721(mobsAddress).ownerOf(pickedMobTokenId);
        ERC20PresetMinterPauser(gemsAddress).mint(mobOwner, gemReward);
        if (finalRunResult == uint8(RunResults.DEATH)) {
          IERC721(raidersAddress).safeTransferFrom(raiderOwner, address(0), tokenId);
        }
      }
      emit RunComplete(request.weekIndex, tokenId, pickedMobTokenId, finalRunResult);
    } else {
    ...
    function fulfillRandomWords(
        ...
        require(requestIdToStatus[_requestId].exists, "request not found");
  ...
  function changeGemReward(uint256 newReward) external onlyOwner {
    gemReward = newReward;
  }

  function updateMobRarity(uint256 tokenId, uint8 rarity) external onlyOwner {
    mobTokenIdToRarity[tokenId] = rarity;
  }

  function updateMobMultiplier(uint16 rarityInt, uint16 rarityMultipler) external onlyOwner {
    mobRarityToMultipler[rarityInt] = rarityMultipler;
  }

  function updateResultThresholds(uint8[3] memory thresholds) external onlyOwner {
    require(thresholds[0] + thresholds[1] + thresholds[2] == 100,"Invalid percentages");
    delete randomResultPool;
    uint8 firstCode = uint8(RunResults.SAFE_WITH_GEMS);
    uint8 secondCode = uint8(RunResults.SAFE_WITHOUT_GEMS);
    uint8 thirdCode = uint8(RunResults.DEATH);
