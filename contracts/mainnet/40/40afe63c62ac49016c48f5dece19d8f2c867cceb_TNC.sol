...
contract TNC is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    function mint(address _to, uint256 _mintAmount) public payable {
        ...
        require(isTNCListed[msg.sender], "You are not qualified for this certificate");
        require(!paused);
    ...
    function TNCListedMint() external {
        uint256 supply = totalSupply();

        require(!TNCpaused, "TNC Certificate minting is over!");
        require(isTNCListed[msg.sender],  "You are not TNCListed");
    ...
    function setTNCPaused() external authorized {
    TNCpaused = !TNCpaused;
    }
    ...
    function addToTNCListed(address[] memory _users) public authorized {
		require(_users.length > 0,"No Users");
        for (uint256 i = 0; i < _users.length; i++) {
            isTNCListed[_users[i]] = true;
        }
    }

	function removeFromTNCListed(address[] memory _user) public authorized {
		require(_user.length > 0,"No User");
		for(uint256 i = 0;i < _user.length;i++)
        isTNCListed[_user[i]] = false;
    }
