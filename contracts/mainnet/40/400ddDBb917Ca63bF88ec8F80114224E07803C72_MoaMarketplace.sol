...
contract MoaMarketplace is Context, IMoaMarketplace, ReentrancyGuard, Ownable {
    ...
    mapping(address => mapping(address => mapping(uint256 => uint256))) private getItemIdFromAddress;
    ...
    /// @dev Prevent market item being duplicated
    modifier isUnqiueMarketItem(address _nftAddress, uint256 _tokenId) {
        uint256 _itemId = getItemIdFromAddress[_nftAddress][_msgSender()][_tokenId];
        MarketItem storage _marketItem = idToMarketItem[_itemId];
        address seller = _marketItem.seller;
        require(
            (_marketItem.itemId == 0) ||
            (_marketItem.itemId != 0 && _marketItem.sold != false)  ||
            (_marketItem.itemId != 0 && _marketItem.revoke != false) ||
            (_marketItem.salesFinishTime > block.timestamp), "already exists");
        _;
    }
    ...
    function _getSaleEndTime(uint256 _salesDays) private view returns (uint64) {
        return uint64(block.timestamp + (_salesDays * 1 days));
    }
    ...
    function listForSale(
        ...
        uint256 _price,
        uint256 _salesDays
    ) public override isUnqiueMarketItem(_nftContract, _tokenId) nonReentrant returns (uint256){
        ...
        require(nftContractWhitelist[_nftContract] == true, "Not whitelisted NFT contract");

        _itemIds.increment();
        uint256 _itemId = _itemIds.current();
        uint64 _salesFinishTime = _getSaleEndTime(_salesDays);
        ...
        getItemIdFromAddress[_nftContract][address(_msgSender())][_tokenId] = _itemId;
      ...
      nonReentrant
    {
        ...
        // Event
        emit MarketItemUpdate(_itemId, nftContract, tokenId, _msgSender(), _price, _marketItem.salesDays, _marketItem.salesFinishTime);
    }
    ...
    function getItemId(
        ...
        return getItemIdFromAddress[_nftContract][IERC721(_nftContract).ownerOf(_tokenId)][_tokenId];
    }
