...
contract RefractlyStableStrategy is StratFeeManager {
  ...
  address private constant penLensAddress = 0x1432c3553FDf7FBD593a84B3A4d380c643cbf7a2;
  address private constant userProxyAddress = 0xc9Ae7Dac956f82074437C6D40f67D6a5ABf3E34b;

  IPenLens private constant PEN_LENS = IPenLens(penLensAddress);
  IUserProxy private constant USER_PROXY = IUserProxy(userProxyAddress);

  uint256 private constant NATIVE_THRESHOLD = 1 ether / 200; // 0.005 wmatic
  ...
  function deposit() public whenNotPaused {
    ...
    if (wantBal > 0) {
      USER_PROXY.depositLpAndStake(want, wantBal);
  ...
  function withdraw(uint256 _amount) external {
    ...
    if (wantBal < _amount) {
      USER_PROXY.unstakeLpWithdrawAndClaim(want, _amount - wantBal);
  ...
  function _harvest(address callFeeRecipient) internal whenNotPaused {
    ...
    if (lpBalance > 0) {
      USER_PROXY.claimStakingRewards();
    }
  ...
  function addLiquidity() internal {
    ...
    uint256 lpToken0Balance = IERC20(lpToken0).balanceOf(address(this));
    uint256 lpToken1Balance = IERC20(lpToken1).balanceOf(address(this));

    // get price of lp tokens and force volatile route
    IRouter.Route[] memory routes0 = new IRouter.Route[](1);
    routes0[0] = IRouter.Route({ from: native, to: lpToken0, stable: false });
    uint256 price0 = IRouter(router).getAmountsOut(1 ether, routes0)[1];

    IRouter.Route[] memory routes1 = new IRouter.Route[](1);
    routes1[0] = IRouter.Route({ from: native, to: lpToken1, stable: false });
    uint256 price1 = IRouter(router).getAmountsOut(1 ether, routes1)[1];

    // get reserves
    (uint256 reserve0, uint256 reserve1) = IRouter(router).getReserves(lpToken0, lpToken1, true);

    // R0/R1 = ((1-c)*X*P0+C0)/(c*X*P1 + C1)
    // R0: Reserve of token0
    // R1: Reserve of token1
    // P0: Price of token0
    // P1: Price of token1
    // C0: Current balance of token 0
    // C1: Current balance of token 1
    // X: Balance of native
    uint256 c = (10000 *
      (reserve1 * (lpToken0Balance * 1 ether + price0 * nativeBalance) - (lpToken1Balance * 1 ether * reserve0))) /
      (nativeBalance * (price0 * reserve1 + price1 * reserve0));

    uint256 nativeForSwap0 = ((10000 - c) * nativeBalance) / 10000;
    uint256 nativeForSwap1 = (c * nativeBalance) / 10000;
    ...
    IRouter(router).addLiquidity(lpToken0, lpToken1, true, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
  }

  // returns reward unharvested
  function rewardAvailable(address rewardToken) public returns (uint256) {
    address stakingPool = PEN_LENS.stakingRewardsByDystPool(address(want));
    address proxyAddress = PEN_LENS.userProxyByAccount(address(this));
  ...
  function retireStrat() external {
    ...
    USER_PROXY.unstakeLpWithdrawAndClaim(want, balanceOfPool());
  ...
  function panic() public onlyOwner {
    pause();
    USER_PROXY.unstakeLpAndWithdraw(want, balanceOfPool());
  }
  ...
  function _giveAllowances() internal {
    IERC20(want).safeApprove(userProxyAddress, 0);
    IERC20(want).safeApprove(userProxyAddress, type(uint256).max);
...
interface IPenLens {
  ...
  function stakingRewardsByDystPool(address dystPoolAddress) external returns (address);
}
