...
contract Fetcher {
    ...
    function test(address vaultAddress, uint256 end)
        external
        view
        returns (Vault[] memory vaults)
    {
        for (uint256 i = 0; i < end; i++) {
            if (IVault(vaultAddress).exists(i)) {
                uint256 collateral = IVault(vaultAddress).vaultCollateral(i);
                uint256 debt = IVault(vaultAddress).vaultDebt(i);
                uint256 collateralRatio = IVault(vaultAddress)
                    .checkCollateralPercentage(i);
                bool isLiquidatable = IVault(vaultAddress).checkLiquidation(i);
                vaults[i] = Vault(
                    true,
                    collateral,
                    debt,
                ...
                vaults[i] = Vault(false, 0, 0, 0, false);
            }
        }
        return vaults;
    }
