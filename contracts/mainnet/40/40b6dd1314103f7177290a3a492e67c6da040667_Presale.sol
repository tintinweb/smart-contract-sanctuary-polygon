...
contract Presale is Ownable {
    ...
    // maximum contribution for public
    uint256 public max_contribution_public = 2000 * 10 ** 18;

    // maximum contribution for alpha
    uint256 public max_contribution_alpha = 25000 * 10 ** 18;

    // maximum contribution for beta
    uint256 public max_contribution_beta = 10000 * 10 ** 18;
    ...
    constructor(address presaleReceiver_, address raiseToken_) {
    ...
    function setMaxContributionPublic(uint max) external onlyOwner {
        max_contribution_public = max;
    }

    function setMaxContributionAlpha(uint max) external onlyOwner {
        max_contribution_alpha = max;
    }

    function setMaxContributionBeta(uint max) external onlyOwner {
        max_contribution_beta = max;
    }
    ...
    function donate(uint256 amount, bytes32[] calldata proof) external {
        uint256 identifier = 2;
        bool isAlpha = MerkleProof.verify(
            proof,
            root0,
            keccak256(abi.encodePacked(msg.sender))
        );
        bool isBeta = MerkleProof.verify(
            proof,
            root1,
            keccak256(abi.encodePacked(msg.sender))
        );
        ...
        require(identifier <= phase, "Invalid Whitelist Phase");

        uint received = _transferIn(amount);
        _process(msg.sender, received, identifier);
    }
    ...
    function _process(address user, uint amount, uint256 identifier) internal {
        require(amount > 0, "Zero Amount");
        if (amount < 25_000 * 10 ** 18) {
            require(hasStarted, "Sale Has Not Started");
        }
        ...
        require(donors[user] >= min_contribution, "Contribution too low");
        require(_totalDonated <= hardCap, "Hard Cap Reached");

        // in alpha
        if (identifier == 0) {
            require(
                donors[user] <= max_contribution_alpha,
        ...
        // in beta
            ...
            require(
                donors[user] <= max_contribution_beta,
                "Max Contribution Reached"
            );
        }
        // in public
            ...
            require(
                donors[user] <= max_contribution_public,
                "Max Contribution Reached"
            );
        }
    ...
    function _transferIn(uint amount) internal returns (uint256) {
        ...
        require(
            raiseToken.transferFrom(msg.sender, presaleReceiver, amount),
