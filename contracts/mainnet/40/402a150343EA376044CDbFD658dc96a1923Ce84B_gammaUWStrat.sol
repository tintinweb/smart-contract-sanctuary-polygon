...
contract gammaUWStrat is StratFeeManager {
    ...
    function withdraw(uint256 _amount) external {
        ...
        uint256 wantBal = IERC20(want).balanceOf(address(this));
        if (wantBal < _amount) {
            IMasterChef(chef).withdraw(poolId, _amount - wantBal, address(this));
    ...
    function _harvest() internal whenNotPaused {
        IMasterChef(chef).deposit(poolId, 0, address(this));
        uint256 rewardsTokenBal = IERC20(output).balanceOf(address(this));
        uint256 rewardsNativeBal = IERC20(native).balanceOf(address(this));
        if (rewardsTokenBal > 0 || rewardsNativeBal > 0) {
            chargeFees(rewardsTokenBal, rewardsNativeBal);
            transferToYeller();
        }
    }

    // performance fees
    function chargeFees(uint _rewardsTokenBal, uint _rewardsNativeBal) internal {
        IFeeConfig.FeeCategory memory fees = getFees();
        
        swapRewardsToYel(_rewardsTokenBal, _rewardsNativeBal);
    ...
    function swapRewardsToYel(uint _rewardsTokenBal, uint _rewardsNativeBal) internal {
        if(_rewardsTokenBal > 0) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(
                _rewardsTokenBal, 0, outputToNativeToYelRoute, address(this), block.timestamp
            );
        }

        if(_rewardsNativeBal > 0) {
            address[] memory nativeToYel = new address[](2);
            nativeToYel[0] = native;
            nativeToYel[1] = yel;

            IUniswapRouterETH(unirouter).swapExactTokensForTokens(
                _rewardsNativeBal, 0, nativeToYel, address(this), block.timestamp
            );
        }
    }
