...
contract Treasury {
    ...
    address private immutable treasurer1;
    address private immutable treasurer2;
    address private immutable treasurer3;
    address private immutable treasurer4;
    ...
    function endVote() public {
        require(voteObject.nay > 1 || voteObject.yay > 1, "The vote has not yet completed");

        isVoteActive = false;
        voteObject = Vote(address(0), payable(address(0)), 0, 0, 0, 0, "");

        hasVoted[treasurer1] = false;
        hasVoted[treasurer2] = false;
        hasVoted[treasurer3] = false;
        hasVoted[treasurer4] = false;
    }

    function executeNativeTransfer() public {
        require(isVoteActive, "There is no active vote");
        require(voteObject.yay > 1, "Not enough YAY votes");
        require(compareStrings(voteObject.currency, "NATIVE"), "Currency is not native");
    ...
    function executeUSDCTransfer() public {
        require(isVoteActive, "There is no active vote");
        require(voteObject.yay > 1, "Not enough YAY votes");
        require(compareStrings(voteObject.currency, "USDC"), "Currency is not USDC");
