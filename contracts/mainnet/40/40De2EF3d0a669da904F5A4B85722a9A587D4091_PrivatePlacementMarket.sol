...
contract PrivatePlacementMarket is OwnablePausable, ReentrancyGuardUpgradeable{
    ...
    function erc721Address() external view returns(address[] memory addresses) {
        addresses = new address[](bagSize+2);
        for (uint256 index; index < bagSize; index++) {
            addresses[index] = address(_stockNftFactory.getStockERC721(index));
        }
        addresses[bagSize] = address(_identityNftFactory.getIdentityERC721(998));
        addresses[bagSize+1] = address(_identityNftFactory.getIdentityERC721(999));
        return addresses;
    }
