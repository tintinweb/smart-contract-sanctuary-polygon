  ...
  IKommunitasStakingV3
{
  ...
  function init(
    ...
    minPrivatePartner = 500000 * 1e8; // 500 komToken
    minGetKomV = 3000 * 1e8; // 3000 komToken
  ...
  function unstake(
    ...
    // set staker
    if(!isWorker[_msgSender()]) _staker = _msgSender();
  ...
  function changeCompoundType(
    ...
    // owner validation
    if(_msgSender() != owner()) _staker = _msgSender();
  ...
  function _compound(
    ...
    if(_compoundType == CompoundTypes.RewardOnly){
      _stake(_sender, reward, _lockIndex, _compoundType);
  ...
  function _unstake(
    ...
    if(_isPremature){
      ...
      if(stakeDetail.compoundType == CompoundTypes.RewardOnly){
        withdrawableAmount = stakeDetail.amount;
