...
contract ARTHRedeemer is Ownable, ReentrancyGuard {
    ...
    uint256 public immutable arthPRECISION;
    uint256 public immutable usdcPRECISION;

    event Refunded(address indexed token, uint256 amount);
    event Redeemed(address indexed user, uint256 arthIn, uint256 usdcOut);

    constructor(ERC20Burnable arth_, ERC20 usdc_) {
        arth = arth_;
        usdc = usdc_;
        arthPRECISION = 10**arth_.decimals();
        usdcPRECISION = 10**usdc_.decimals();
    }
    ...
    function refundToken(address token, uint256 amount)
        external
        nonReentrant
        onlyOwner
    {
    ...
    function getUSDCOut(uint256 arthIn) public view returns (uint256) {
        return (arthIn.mul(2).mul(usdcPRECISION).div(arthPRECISION));
    }
}
