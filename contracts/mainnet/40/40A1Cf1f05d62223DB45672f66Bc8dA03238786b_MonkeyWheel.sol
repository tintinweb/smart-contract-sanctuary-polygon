...
contract MonkeyWheel is VRFConsumerBaseV2, ConfirmedOwner, ReentrancyGuard {
    uint8[] payouts = [2, 5];
    ...
    function spin(BetType betType, bool isRand)
        external
        payable
        nonReentrant
        returns (uint256)
    ...
    function fulfillRandomWords(
        ...
        uint256 result = _randomWords[0] % 100;
        statuses[_requestId].fulfilled = true;

        if (statuses[_requestId].betType == BetType.X2) {
            if (result > 60) {
        ...
        if (statuses[_requestId].betType == BetType.X5) {
            if (result > 70) {
                statuses[_requestId].didWin = true;
            } else {
                statuses[_requestId].didWin = false;
