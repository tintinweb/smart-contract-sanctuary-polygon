...
contract MGP {
    ...
    function register(address _referral) external {
        ...
        require(userInfo[msg.sender].referrer == address(0));
    ...
    function _deposit(address _userAddr, uint256 _amount, uint256 _types) private {
        ...
        if(_types == 0){
            ...
            require(user.level == 0);
            require(userRewards.split >= _amount, "insufficient Split balance");
    ...
    function predict(uint256 _amount) external payable {
        require(userInfo[msg.sender].referrer != address(0));
        require(_amount.mod(levelDeposit[0]) == 0);
        uint256 curDay = getCurDay();
        require(userPredicts[curDay][msg.sender].length < dayPredictLimit);
        uint256 predictEnd = startTime.add(curDay.mul(timeStep)).add(predictDuration);
        require(block.timestamp < predictEnd);
        require(msg.value >= predictFee);
    ...
    function transferBySplit(address _receiver, uint256 _amount) external {
        if(msg.sender == users) {
            require(growth >= _amount);
            rewardInfo[_receiver].split += _amount;
            growth -= _amount;
        } else {
            uint256 minTransfer = levelDeposit[0].mul(2);
            require(_amount >= minTransfer && _amount.mod(minTransfer) == 0);
    ...
    function distributeGlobalManagerPool() external {
        require(block.timestamp >= poolLastDistribute.add(timeStep));
