...
contract Harvester is Initializable, Governable {
     ...
     * @param _primaryTokenAddress Address of primary token
     */
    function initialize(address _vaultAddress, address _primaryTokenAddress) external onlyGovernor initializer {
    ...
    ****************************************/
    ...
    modifier onlyVaultOrGovernor() {
        require(msg.sender == vaultAddress || isGovernor(), "Caller is not the Vault or Governor");
        _;
    }
    modifier onlyVault() {
     ...
     */
    function setRewardsProceedsAddress(address _rewardProceedsAddress) external onlyGovernor {
        require(_rewardProceedsAddress != address(0), "Rewards proceeds address should be a non zero address");

        rewardProceedsAddress = _rewardProceedsAddress;
    }

    function setLabs(address _labs, uint256 _feeBps) external onlyVaultOrGovernor {
        require(_labs != address(0), "Labs address should be a non zero address");
        require(_feeBps > 0, "Labs fee should be greater than zero");
        labsAddress = _labs;
        labsFeeBps = _feeBps;
    }
     ...
     */
    function setSupportedStrategy(address _strategyAddress, bool _isSupported) external onlyVaultOrGovernor {
    ...
    ****************************************/
    /*
     * @dev Collect reward tokens from all strategies and distrubte primary token
    ...
    function _distribute() internal {
        if (IERC20(primaryTokenAddress).balanceOf(address(this)) > 1e5) {
            _distributeFees(IERC20(primaryTokenAddress).balanceOf(address(this)));
            _distributeProceeds(IERC20(primaryTokenAddress).balanceOf(address(this)));
        }
    }
    ...
    function _distributeFees(uint256 _amount) internal {
        ...
        IERC20(primaryTokenAddress).transfer(labsAddress, labsfees);
        IERC20(primaryTokenAddress).transfer(teamAddress, teamfees);
    }
    ...
    function _distributeProceeds(uint256 _amount) internal {
        ...
        IERC20(primaryTokenAddress).transfer(rewardProceedsAddress, _amount);
    }
    ...
    function _harvest(address _strategyAddr) internal {
        require(supportedStrategies[_strategyAddr], "Not a valid strategy address");
