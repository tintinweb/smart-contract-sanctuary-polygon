...
contract PublicGovSale is Initializable, IPublicGovSale, PausableUpgradeable, OwnableUpgradeable {
   ...
   */
  function chainUserInvoicesLength(
    uint240 _chainID,
    address _user
  ) external view virtual returns (uint256 chainUserInvoicesLengthNumber) {
   ...
   */
  function purchasedPerRound(
    address _user,
    uint128 _boosterRunning
  ) external view virtual returns (uint256 purchasedPerRoundNumber) {
    uint256 chainStakedLength = factory.allChainsStakedLength();
    for (uint256 i = 0; i < chainStakedLength; ++i) {
   ...
   */
  function buyToken(uint240 _chainID, uint256 _amountIn, address _user) external virtual {
  ...
  function _setUser(uint240 _chainID, address _user) internal virtual returns (uint256) {
    if (!isChainUser[_chainID][_user]) {
      ...
      chainUsersBought[_chainID].push(0);
      chainUsersReceived[_chainID].push(0);
      chainUsersFee[_chainID].push(0);
    }
