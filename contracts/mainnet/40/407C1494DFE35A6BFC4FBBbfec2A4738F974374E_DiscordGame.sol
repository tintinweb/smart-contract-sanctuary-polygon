...
contract DiscordGame is ERC721, Pausable, Ownable {

    struct role {
        uint256 totalTokensToMint;
        uint256 totalTokensAwarded;
        mapping (uint256 => bool) tokensAwardedMap;
    }
    ...
    string private baseURI = "https://s3.amazonaws.com/nfts.nftarca.de/surfsup/";

    constructor() ERC721("NFTArcadeRoles", "NFTAR"){}
    ...
    function mint(address sendTo, string memory roleName, uint256 tokenID, string memory tokenURIString) external onlyOwner whenNotPaused {
        ...
        // Make sure it's not already awarded (if minted / it's already awarded)
        require(roles[roleName].tokensAwardedMap[tokenID], "That token has already been minted");
        ...
        roles[roleName].tokensAwardedMap[tokenID] = true;

        // Then send the mint to the wallet
        _safeMint(sendTo, tokenID);
        tokenIdToURI[tokenID] = tokenURIString;
    }
    ...
    function tokenLimitInRole(string memory roleName) public view returns (uint256){
    ...
    function getRoleFromTokenID(uint256 tokenID) public view returns(string memory){
        ...
        for (uint256 i = 0; i < roleNames.length; i++) {
            if (roles[roleNames[i]].tokensAwardedMap[tokenID]){
