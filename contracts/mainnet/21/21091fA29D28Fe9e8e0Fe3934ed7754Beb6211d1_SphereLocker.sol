...
interface IAutocompounder {
  function autocompound(address _account, address _token, bytes calldata _data, address _oneInchRouter) external;
}

interface IXSPHERE {
  function buyBackTo(address _address, uint256 _amount) external;
}
...
contract SphereLocker is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  uint256 public constant REWARDS_DURATION = 10 minutes;
  //     Duration of lock/earned penalty period
  uint256 public constant LOCK_DURATION = REWARDS_DURATION * 2;
  ...
  struct VestTokenBalance {
    uint idx;
    uint amount;
  }

  struct NewLockedBalance {
    uint idx;
    uint112 amount;
    uint32 unlockTime;
  }

  mapping(address => mapping(uint256 => uint256)) public vSphereBalance;
  IERC20Upgradeable public vStakingToken;
  mapping(address => uint256) public vestBalances;
  ...
  /**
   * @notice Adds a vStaking token.
   * @param _vStakingToken Address of the vStaking token to add.
   * @dev Only the contract owner can call this function. _vStakingToken cannot be the staking token.
   **/
  function addVStakingToken(address _vStakingToken) external onlyOwner {
    require((_vStakingToken) != address(0), "Invalid address");
    require((_vStakingToken) != address(stakingToken), "WT");
    require(address(vStakingToken) == address(0), "Already set");
    vStakingToken = IERC20Upgradeable(_vStakingToken);
  }
   ...
   * @dev Calls the `updateReward` modifier for the given account.
   */
  function lockVest(address _account, uint256 _amount) external nonReentrant updateReward(_account) {
    // Transfer the specified amount of tokens from the caller to the contract
    stakingToken.safeTransferFrom(msg.sender, address(this), _amount);
    // Lock the tokens for the given account
    _lock(_account, _amount, true);
  }

  /**
   * @notice Locks tokens during autocompound.
   * @param _account Address to lock tokens for.
   * @param _amount Amount of tokens to lock.
   */
  ...
  function lockAll() external nonReentrant updateReward(msg.sender) {
    ...
    _lock(msg.sender, balance, false);
  }
  ...
  function lockAllFor(address _account) external nonReentrant updateReward(_account) {
    ...
    _lock(_account, balance, false);
  }

  function __checkDelay1(uint256 _checkDelay, uint256 _amount, address _account) internal returns (uint256 _reward) {
    ...
    _reward = (uint256(_amount) * (rRate)) / (DENOMINATOR);
  }

  function __checkDelay2(uint256 _checkDelay, uint256 _amount, uint256 _unlockTime) internal returns (uint256 _reward) {
    //check for kick reward
    //this wont have the exact reward rate that you would get if looped through
    //but this section is supposed to be for quick and easy low gas processing of all locks
    //we'll assume that if the reward was good enough someone would have processed at an earlier epoch
    uint256 currentEpoch = block.timestamp.sub(_checkDelay).div(REWARDS_DURATION).mul(REWARDS_DURATION);
    uint256 epochsover = currentEpoch.sub(uint256(_unlockTime)).div(REWARDS_DURATION);
    uint256 rRate = SphereMath.min(kickRewardPerEpoch.mul(epochsover + 1), DENOMINATOR);
    _reward = (uint256(_amount) * (rRate)) / (DENOMINATOR);
  }
   ...
   */
  function _lock(address _account, uint256 _amount, bool _isVest) internal notBlacklisted(msg.sender, _account) {
    ...
    if (idx == 0 || userLocks[_account][idx - 1].unlockTime < unlockTime) {
      ...
      if(_isVest) {
        uint256 idx2 = userLocks[_account].length;
        console.log("idx2:", idx2);
        vSphereBalance[_account][idx2 - 1] += _amount;
        vestBalances[_account] += _amount;
      }
    } else {
      ...
      if(_isVest) {
        vSphereBalance[_account][idx - 1] += _amount;
        vestBalances[_account] += _amount;
      }
    }
  ...
  function emergencyWithdraw() external nonReentrant {
    ...
    uint vBalance = vestBalances[msg.sender];
    vestBalances[msg.sender] = 0;
    _resetvSphereBalance(msg.sender, length);

    lockedSupply -= amt;

    emit Withdrawn(msg.sender, amt, false);

    stakingToken.safeTransfer(address(vStakingToken), vBalance);
    IXSPHERE(address(vStakingToken)).buyBackTo(msg.sender, vBalance);
    stakingToken.safeTransfer(msg.sender, amt - vBalance);
  }
  ...
  function _resetvSphereBalance(address _account, uint256 _length) internal returns (uint256 _balance) {
    for (uint256 i = 0; i < _length + 1; i++) {
      _balance += vSphereBalance[_account][i];
      vSphereBalance[_account][i] = 0;
    }
  }
  ...
  function _processExpiredLocks(
    ...
    if (isShutdown || locks[length - 1].unlockTime <= expiryTime) {
      ...
      vBalance = _resetvSphereBalance(_account, length);
      require(vestBalances[_account] == vBalance, "vestBalances[_account] != vBalance");
      vestBalances[_account] = 0;

      //dont delete, just set next index
      userBalance.nextUnlockIndex = length.to32();
      if (_checkDelay > 0) {
        reward = __checkDelay1(_checkDelay, locked, _account);
      }
    } else {
      //use a processed index(nextUnlockIndex) to not loop as much
      //deleting does not change array length

      for (uint256 i = userBalance.nextUnlockIndex; i < length; i++) {
        //unlock time must be less or equal to time
        if (locks[i].unlockTime > expiryTime) break;
        //add to cumulative amounts
        locked += (locks[i].amount);
        vBalance += vSphereBalance[_account][i];
        vestBalances[_account] -= vSphereBalance[_account][i];
        vSphereBalance[_account][i] = 0;

        //check for kick reward
        //each epoch over due increases reward
        if (_checkDelay > 0) {
          reward += __checkDelay2(_checkDelay, uint256(locks[i].amount), uint256(locks[i].unlockTime));
        }
        //set next unlock index
        userBalance.nextUnlockIndex = userBalance.nextUnlockIndex++;
      }
    }
    ...
    if (_relock) {
      _lock(_account, locked - vBalance, false);
      _lock(_account, vBalance, true);
    } else {
      stakingToken.safeTransfer(address(vStakingToken), vBalance);
      IXSPHERE(address(vStakingToken)).buyBackTo(_account, vBalance);
      stakingToken.safeTransfer(_account, locked - vBalance);
    }
  }
  ...
  function lockedBalances(
    ...
    returns (
      ...
      uint256 unlockable,
      uint256 locked,
      NewLockedBalance[] memory lockData,
      VestTokenBalance[] memory vestBalance
    )
  {
    LockedBalance[] storage locks = userLocks[_user];
    ...
    for (uint256 i = nextUnlockIndex; i < locks.length; i++) {
      if (locks[i].unlockTime > block.timestamp) {
        if (idx == 0) {
          lockData = new NewLockedBalance[](locks.length - i);
          vestBalance = new VestTokenBalance[](locks.length - i);
        }
        lockData[idx].amount = locks[i].amount;
        lockData[idx].unlockTime = locks[i].unlockTime;
        lockData[idx].idx = i;
        vestBalance[idx].idx = i;
        vestBalance[idx].amount = vSphereBalance[_user][i];
        idx++;
    ...
    return (userBalance.locked, unlockable, locked, lockData, vestBalance);
  }
