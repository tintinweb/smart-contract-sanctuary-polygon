...
contract IceStoneToken is ERC20 {
    ...
    constructor(IStoneFactory stoneFactory, uint256 premintAmount) ERC20("IceStone Cash", "ISC", 18, premintAmount) {
        operator = msg.sender;
    ...
    function claimableAmount(IStone stone, uint256 tokenId) public view returns (uint256 amountToClaim) {
        ...
        uint256 last = lastClaim[address(stone)][tokenId];
        if(last == 0) {
            last = stoneData.createdTime;
        }

        uint256 blockTimeToClaim = block.timestamp - last;
