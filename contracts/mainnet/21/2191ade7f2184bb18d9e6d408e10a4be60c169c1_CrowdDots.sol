...
contract CrowdDots is DefaultOperatorFilterer, ERC721A, Ownable {
  using Address for address;
  using Strings for uint256;
  ...
  uint256 private maxMintAmount = 50;
  uint256 private FreeMintPerAddressLimit = 0;
  ...
  uint256 private _promoterLinkPercentage = 7;
  uint256 private _royaltyCrowd = 4;  
  uint256 private _royaltyMobiup = 3;  
  uint256 private _royaltyFund = 85;  
  uint256 private _royaltyArtist = 0;  
  uint256 private _royaltyCreator = 1;  

  address private wMobiup = 0x2919771f951fd264445d4D4c682EE5D8326B93ec;
  address private wCrowd = 0x008f29615C49ca32207ecb4BBc57fA02DF67C58f;
  address private wFund = 0xD53dA08799d5c201287BC59B1E4Be6a2091f57FB;
  address private wArtist = 0x4F7B4Bfc763418162dF4921c5e82163f6AE08a2c;
  address private wCreator = 0x4F7B4Bfc763418162dF4921c5e82163f6AE08a2c;
  uint256 royaltiesSpender;

  uint256 private _royaltyFundMarket = 1;  
  uint256 private _royaltyArtistSecondMarket = 0;
  uint256 private _royaltyCreatorSecondMarket = 1;

  mapping(address => uint256) private _addressLastMintedTokenId;

  bool private _isFreeMint = false;
  uint256 private _nftEtherValue = 10 ether;
  uint256 private _nftEtherValueWaitList = 10 ether;  

  mapping(uint256 => string) private cids;

  uint256 private initSaleWaitList = 1689630231;
  uint256 private endSaleWaitList = 1689630231;
  uint256 private startSalePublic = 1689630231;

  event _transferSend(address _from, address _to, uint _amount);

  constructor(    
    bytes32 _root    
  ) ERC721A("CrowdDots", "DOTS") {    
    root = _root;
    _contractUri = "";
    ...
    function phase() public view returns (uint256) {
        if (block.timestamp >= initSaleWaitList && block.timestamp <= endSaleWaitList) {
            return 0; //For WaitList
        }

        if (block.timestamp >= startSalePublic) {
            return 1; //For Public
        }
    ...
    function conditionPrice() public view returns (uint256) {
        ...
        if (phaseTemp == 0) {
            return _nftEtherValueWaitList;
        }

        if (phaseTemp == 1 || phaseTemp == 2) {
            return _nftEtherValue;
        }
    }

    function setNewTimesSales(uint256 _initSaleWaitList, uint256 _endSaleWaitList, uint256 _startSalePublic) public onlyOwner {
        require(_endSaleWaitList > _initSaleWaitList, "Data Inicial de venda waitlist deve ser menor que da data de venda da WaitList");        
        initSaleWaitList = _initSaleWaitList;
        endSaleWaitList = _endSaleWaitList;
        startSalePublic = _startSalePublic;
    }
    ...
    function getFreeMintPerAddressLimit() public view returns (uint) {
        return FreeMintPerAddressLimit;
    }
    ...
    function mint(
      ...
      require(phase() == 1, "Mint Public desativado");  
      ...
      if(!_isFreeMint ) {
        ...
        if(_freeMintAmountPerWallet[msg.sender] >= FreeMintPerAddressLimit ) {
            split(_mintAmount, _recommendedBy, _indicationType);
        } else {            
    ...
    function mintWhitelist(uint256 _mintAmount,
      ...
      address payable endUser,
      bytes32[] memory proof,
      string memory _cid) public payable {
        require(phase() == 0, "Mint WaitList desativado");
        require(isWhitelisted(msg.sender, proof), "A carteira deve pertencer a whitelist");
        require(!paused, "O contrato esta pausado.");
        
        require(_mintAmount > 0, "Precisa mintar pelo menos 1 NFT.");
        require(totalSupply() + _mintAmount <= maxSupply, "Quantidade limite de NFT excedida.");
    ...
    function split(uint256 _mintAmount, address payable _recommendedBy, uint256 _indicationType ) public payable{
      ...
      uint256 amountRoyaltyArtist = _royaltyArtist;

      if(_indicationType > 1){

        uint256 amountLink = msg.value * _promoterLinkPercentage / 100;
        
        payable(_recommendedBy).transfer(amountLink);
      } else {
         amountRoyaltyArtist = amountRoyaltyArtist + 7;        
      }

      uint256 splitMobiup =  (msg.value * _royaltyMobiup) / 100; 
      uint256 splitCrowd =  (msg.value * _royaltyCrowd) / 100;           

      payable(wMobiup).transfer(splitMobiup);
      payable(wCrowd).transfer(splitCrowd);
      

      uint256 amountFund = msg.value * _royaltyFund / 100;        
      payable(wFund).transfer(amountFund);
      
      uint256 amountArtist = msg.value * amountRoyaltyArtist / 100;        
      payable(wArtist).transfer(amountArtist);

      uint256 amountCreator = msg.value * _royaltyCreator / 100;        
      payable(wCreator).transfer(amountCreator);
    ...
    receive() external payable {
        ...
        uint256 amountRoyaltiesOne = msg.value * _royaltyFundMarket / 100;
        payable(wFund).transfer(amountRoyaltiesOne);

        uint256 amountRoyaltiesTwo = msg.value * _royaltyArtistSecondMarket / 100;
        payable(wArtist).transfer(amountRoyaltiesTwo);

        uint256 amountRoyaltiesThree = msg.value * _royaltyCreatorSecondMarket / 100;
        payable(wCreator).transfer(amountRoyaltiesThree);
