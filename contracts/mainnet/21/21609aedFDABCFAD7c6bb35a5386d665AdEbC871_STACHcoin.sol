...
contract STACHcoin is ERC20, AccessControl, ERC721Holder {
    ...
    constructor() ERC20("STACHcoin", "STACH")AccessControl(){
        owner = 0x18058F7Ba124ab0EDBbe11CaeFf813AaBc723f66;
        collectionAddressC1 = 0xF8207A7A9cc5e20955c7eADa1DABF504590cB783;
        ...
        welcomeBonusAmountC1 = 50 * 10 ** 18; 
        welcomeBonusAmountC2 = 50 * 10 ** 18; 
        welcomeBonusAmountC3 = 50 * 10 ** 18; 
    }
    ...
    function harvestC1(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        StakeInfoC1 storage infoC1 = stakeLogC1[_msgSender()][tokenId];
        uint256 rewardAmountInSTACH = pendingRewardsC1(_msgSender(), tokenId);
        if(rewardAmountInSTACH > 0) {
            infoC1.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInSTACH);
            emit RewardsHarvestedC1(_msgSender(), rewardAmountInSTACH);
        }
    }
    function harvestC2(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        StakeInfoC2 storage infoC2 = stakeLogC2[_msgSender()][tokenId];
        uint256 rewardAmountInSTACH = pendingRewardsC2(_msgSender(), tokenId);
        if(rewardAmountInSTACH > 0) {
            infoC2.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInSTACH);
            emit RewardsHarvestedC2(_msgSender(), rewardAmountInSTACH);
        }
    }
    function harvestC3(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        StakeInfoC3 storage infoC3 = stakeLogC3[_msgSender()][tokenId];
        uint256 rewardAmountInSTACH = pendingRewardsC3(_msgSender(), tokenId);
        if(rewardAmountInSTACH > 0) {
            infoC3.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInSTACH);
            emit RewardsHarvestedC3(_msgSender(), rewardAmountInSTACH);
        }
    }
    ...
    function setCollectionAddressC1(address newAddressC1) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC1 != address(0), "STACH: update to zero address not possible");
    ...
    function setCollectionAddressC2(address newAddressC2) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC2 != address(0), "STACH: update to zero address not possible");
    ...
    function setCollectionAddressC3(address newAddressC3) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC3 != address(0), "STACH: update to zero address not possible");
