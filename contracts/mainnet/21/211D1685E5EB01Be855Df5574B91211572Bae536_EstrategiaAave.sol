...
contract EstrategiaAave is Ownable {
    ...
    constructor(
        ...
        (aToken, , ) = IDataProvider(dataProvider).getReserveTokensAddresses(token);
    ...
    function retirar(uint256 _cantidad) external {
        ...
        if (tokenBal < _cantidad) {
            ...
            ILendingPool(lendingPool).withdraw(token, _cantidad - tokenBal, address(this));
    ...
    function reservasUsuario() public view returns (uint256, uint256) {
        (uint256 balDepositado, , uint256 balPrestado, , , , , , ) = IDataProvider(dataProvider)
