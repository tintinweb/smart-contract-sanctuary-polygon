...
contract ApotropaicAmulet is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    constructor() ERC721("Apotropaic Amulets by Philippe Cramer", "AMULET") {}
    uint256 public constant maxSupply = 168;
    ...
    function safeMint14(address to) public onlyOwner {
        uint totalSupply = totalSupply();
        uint currentId = totalSupply + 1;
        for (currentId = currentId; currentId <= (totalSupply + 14); currentId++) {
            safeMint(to, currentId);
        }
    }

    function _baseURI() internal override(ERC721) view virtual returns (string memory) {
        return "ipfs://QmS35CNW4tzpDWtTq4FKrieyNRTAXTkFrM3exhGiC9fF6G/";
    }
