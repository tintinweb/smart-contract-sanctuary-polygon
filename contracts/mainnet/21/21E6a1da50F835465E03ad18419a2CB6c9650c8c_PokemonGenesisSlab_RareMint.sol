...
/**
 * @title NFT Drop "Pokemon Genesis Slab Series", by RareMint, on Polygon Mainnet.
 */
contract PokemonGenesisSlab_RareMint is TokenDropBase {
    ...
    constructor()
        TokenDropBase(
            "Pokemon Genesis Slab Series | by RareMint",
            10, // Max 10 tokens per transaction
            100, // Max 100 tokens per address
            230000000000000000, // 0.23 ETH on Polygon (subject to be updated before the drop)
            391.5 * 10**18, // 391 MATIC on Polygon (subject to be updated before the drop)
            0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619, // ETH on Polygon Mainnet
            0x7637cE4C37A796d8cECd92C0c2441ab62c16EF79, // RareMint Engineering Multisig on Polygon Mainnet
            0x43eE2B7d68a2d7d7CDf9Fe3EABC2fc27FD772CE1, // RareMint Registry deployed on Polygon Mainnet
            ...
            0xf86195cf7690c55907b2b611ebb7343a6f649bff128701cc542f0569e2c549da, // Chainlink keyHash on Polygon Testnet
...
abstract contract TokenDropBase is Context, ReentrancyGuard, Ownable, ERC20PaymentProcessor, PresaleList, TokenMintingPool {
    ...
    uint256 public mintPriceMatic; // mint price per token in MATIC for CC checkout.
    ...
    address public paymentReceiver = 0x7637cE4C37A796d8cECd92C0c2441ab62c16EF79;
        ...
        TokenMintingPool(tokenRegistryAddress, vrfCoordinator, linkToken, vrfKeyHash, vrfLinkFee)
    {
        ...
        mintPriceMatic = mintPriceNativeToken;
    }
    ...
    /**
     * @dev Updates the mint price of MATIC.
     * Requirement: the drop hasn't started (i.e. the contract is not in presale or in public sale).
     */
    function updateMintPriceMatic(uint256 _mintPriceMatic) public onlyOwner {
        require(!(_presaleIsOpen || _publicSaleIsOpen), "TokenDropBase: The drop already started - cannot update the mint price of MATIC.");
     ...
     *  - Mints the tokens to the minter.
     */
    function _processMintRequestCC(address minter, uint256 requestedAmount) private {
        uint256 remainingSupply = remainingSupplyCount();
        require(requestedAmount > 0, "TokenDropBase: Invalid request for zero tokens.");
        require(remainingSupply > 0, "TokenDropBase: No more tokens left.");
        require(_mintsPerAddress[minter] < maxPerAddress, "TokenDropBase: Token limit per wallet already reached.");
        ...
        uint256 mintableAmount = _mintableAmountForAddress(minter, requestedAmount, remainingSupply);
        _mintsPerAddress[minter] += mintableAmount;
        require(msg.value == (mintableAmount * mintPriceMatic), "Value sent does not match price");
     ...
     *  - see {_processMintRequest}
     */
    function claimTokensToAddress(address to, uint256 amount) external payable nonReentrant {
        address caller = _msgSender();
        if (!_publicSaleIsOpen) {
            require(_presaleIsOpen, "TokenDropBase: The presale is not open yet.");
            require(_hasPresaleAccess(caller), "TokenDropBase: The public sale is not open yet and caller does not have presale address.");
        ...
        _processMintRequestCC(to, amount);
    }
