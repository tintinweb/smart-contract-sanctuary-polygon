...
{
    function findEligibleRef(address userAddress, uint8 level) external view returns(bool);
...
contract META_GFORCE_s30 is owned
{
   ...
   // mapping(address => mapping(uint => uint8)) public autoLevelBuy; 
    ...
    {
        require(levelCheck(levelAddress).findEligibleRef(msg.sender, 1), "level mis-match");
    ...
    {
        require(levelCheck(levelAddress).findEligibleRef(usermsg, 1), "level mis-match");
