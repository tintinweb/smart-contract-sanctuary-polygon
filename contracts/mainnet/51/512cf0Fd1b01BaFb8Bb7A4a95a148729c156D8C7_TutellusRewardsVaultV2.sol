...
contract TutellusRewardsVaultV2 is
    ITutellusRewardsVaultV2,
    UUPSUpgradeableByRole
{
    bytes32 internal constant _REWARDS_MANAGER_ROLE =
    ...
    /// @inheritdoc ITutellusRewardsVaultV2
    function add(address account, uint256[] memory allocations)
        public
    ...
    /// @inheritdoc ITutellusRewardsVaultV2
    function setRewardPerBlock(uint256 value)
        public
        onlyRole(_REWARDS_MANAGER_ROLE)
    {
    ...
    /// @inheritdoc ITutellusRewardsVaultV2
    function setAllocations(uint256[] memory allocations)
        public
        onlyRole(_REWARDS_MANAGER_ROLE)
    {
        require(
        ...
        for (uint256 i = 0; i < totalAccounts; i++) {
            ...
            _releasedOffset[account] +=
                (releasedAfterOffset * allocation[account]) /
                100 ether;
        ...
        require(
            sum == 100 ether,
            "TutellusRewardsVaultV2: total allocation must be 100 ether"
        );
    }
    ...
    function distribute(address account, uint256 amount) public {
        require(
            amount <= available(msg.sender),
            "TutellusRewardsVaultV2: amount exceeds available"
        );
        distributed[msg.sender] += amount;
        ITutellusERC20 tokenInterface = ITutellusERC20(
            ITutellusManager(config).get(keccak256("ERC20"))
        );
