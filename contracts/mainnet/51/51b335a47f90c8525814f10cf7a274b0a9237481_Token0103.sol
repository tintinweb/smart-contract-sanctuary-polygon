...
contract Token0103 is ERC20, Ownable {
  ...
  function burn(address user, uint256 amount) external {
         require(allowedAddresses[msg.sender] || msg.sender == address(Parent), "Address does not have permission to burn");
        _burn(user, amount);
    }
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        return Parent.amountOfNFT(user) * BASE_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 1800;
    }
}