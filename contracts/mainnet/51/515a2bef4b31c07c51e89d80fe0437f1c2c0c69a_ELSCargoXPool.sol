...
contract ELSCargoXPool is ReentrancyGuard, Ownable, DSMath {
    ...
    function changeDistributionFeeAddress(address _newDistributionFeeAddress) external onlyOwner {
        distributionFeeAddress = _newDistributionFeeAddress;

        emit DistributionFeeAddressChanged(_newDistributionFeeAddress);
    }

    // owner of the pool can change fee, but not more than 70%
    function changeFee(uint256 _newFee) external onlyOwner {
        ...
        require(_newFee <= 70, "Fee cannot be more than 70%.");
    ...
    event DistributionFeeAddressChanged(address newDistributionFeeAddress);
}
