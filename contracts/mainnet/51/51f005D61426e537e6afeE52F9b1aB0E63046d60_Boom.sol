...
/// @notice Contract for Hunters OnChain BOOM token
/// @author zetsub0ii.eth & Tansel Kaya
contract Boom is ERC20, ERC20Burnable, Blacklist, AccessControl {
    // Base precision of 10000 allows 2 decimals.
    // e.g. 0.2 is represented as 20 in the contract.
    uint256 constant BASE_PRECISION = 10_000;

    uint256 constant INITIAL_SUPPLY = 1_000_000 * 10 ** NUM_DECIMALS;
    uint256 constant REST_SUPPLY = 4_999_000_000 * 10 ** NUM_DECIMALS;

    uint8 constant NUM_DECIMALS = 2;

    // Can control the blacklist
    bytes32 constant BLACKLIST_ADMIN = keccak256("BLACKLIST_ADMIN");
    // Can control the contribution address and rate
    bytes32 constant CONTRIBUTION_ADMIN = keccak256("CONTRIBUTION_ADMIN");
    ...
    address[] private _excludedReceiver;

    uint256 private _contributionRate = 500; // out of BASE_PRECISION
    address private _contributionAddress;

    address private _defaultAdmin;
    address private _assignedDefaultAdmin = address(0x0);

    bool private _isRestMinted = false;
    ...
    constructor(address admin, address reserveOwner, address contributionAdmin, address blacklistAdmin)
        ERC20("BOOM", "BOOM")
    {
        _setupRole(DEFAULT_ADMIN_ROLE, admin);

        _setupRole(CONTRIBUTION_ADMIN, contributionAdmin);
        _setupRole(BLACKLIST_ADMIN, blacklistAdmin);

        _mint(reserveOwner, INITIAL_SUPPLY);
        _contributionAddress = owner();
    }
    ...
    function transfer(address to, uint256 amount) public override returns (bool success) {
        if (isExcluded(msg.sender, true) || isExcluded(to, false)) {
            super.transfer(to, amount);
        } else {
            // Calculate and spend the tax
            uint256 contributionAmt = (amount * _contributionRate) / BASE_PRECISION;
            super.transfer(_contributionAddress, contributionAmt);

            // Send rest to the recipient
            super.transfer(to, amount - contributionAmt);
        }
        return true;
    }

    function transferFrom(address from, address to, uint256 amount) public override returns (bool success) {
        if (isExcluded(from, true) || isExcluded(to, false)) {
            return super.transferFrom(from, to, amount);
        } else {
            // Calculate and spend the tax
            uint256 contributionAmount = (amount * _contributionRate) / BASE_PRECISION;
            super.transferFrom(from, _contributionAddress, contributionAmount);

            // Send rest to the recipient
            super.transferFrom(from, to, amount - contributionAmount);
        }
        return true;
    }
    ...
    /// @notice Gets contribution rate
    function getContributionRate() public view returns (uint256) {
        return _contributionRate;
    }

    /// @notice Sets contribution rate
    /// @dev Requires CONTRIBUTION_ADMIN role
    function setContributionRate(uint256 rate) external onlyRole(CONTRIBUTION_ADMIN) {
        require(rate <= BASE_PRECISION, "Rate should be be between 0 and 100");
        _contributionRate = rate;
    }

    /// @notice Returns contribution address
    function getContributionAddress() public view returns (address) {
        return _contributionAddress;
    }

    /// @notice Sets contribution address
    /// @dev Requires CONTRIBUTION_ADMIN role
    function setContributionAddress(address _target) external onlyRole(CONTRIBUTION_ADMIN) {
        _contributionAddress = _target;
    }

    /// @notice Returns if address is excluded
    function isExcluded(address account, bool isSender) public view returns (bool) {
        if (isSender) {
            return _isExcludedSender[account];
        } else {
            return _isExcludedReceiver[account];
        }
    }

    /// @notice Excludes account for tax
    /// @param account Address to exclude
    /// @param isSender True if sender, false if receiver
    function excludeAccount(address account, bool isSender) external onlyRole(BLACKLIST_ADMIN) {
        if (isSender) {
            require(!_isExcludedSender[account], "Sender account is already excluded");
            _isExcludedSender[account] = true;
            _excludedSender.push(account);
        } else {
            require(!_isExcludedReceiver[account], "Receiver account is already excluded");
            _isExcludedReceiver[account] = true;
            _excludedReceiver.push(account);
        }
    }

    /// @notice Includes account for tax
    /// @param account Address to include
    /// @param isSender True if sender, false if receiver
    function includeAccount(address account, bool isSender) external onlyRole(BLACKLIST_ADMIN) {
        if (isSender) {
            require(_isExcludedSender[account], "Sender account is already included");
            ...
            require(_isExcludedReceiver[account], "Account is already included");
            for (uint256 i = 0; i < _excludedReceiver.length; i++) {
                if (_excludedReceiver[i] == account) {
                    _excludedReceiver[i] = _excludedReceiver[_excludedReceiver.length - 1];
                    _isExcludedReceiver[account] = false;
                    _excludedReceiver.pop();
                    break;
                }
            }
        }
    ...
    /// @notice Mints the rest of the tokens
    /// @param reserveOwner Address to mint the rest to
    /// @dev Requires DEFAULT_ADMIN_ROLE
    /// @dev Can be called only once
    function mintRest(address reserveOwner) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(!_isRestMinted, "Rest is already minted");
        _isRestMinted = true;

        _mint(reserveOwner, REST_SUPPLY);
    }
