...
// File: contracts/Admin/AdminGachaTicketAirdropMachineV2.sol
...
contract AdminGachaTicketAirdropMachine is LuxOnSuperOperators, LuxOnLive, LuxOnData {
    ...
    constructor(
        ...
        string memory operator,
        address luxOnAdmin
    ) LuxOnData(dataAddress) LuxOnLive(luxOnService) LuxOnSuperOperators(operator, luxOnAdmin) {
    ...
    function airdrop(AirdopInfo memory airdopInfo) external onlySuperOperator {
        ...
        uint256 gachaType = DspGachaData(gachaDataAddress).getGachaType(airdopInfo.tokenId);
        if (uint256(GachaType.Character) == gachaType) {
            GachaInfo memory _gachaInfo = DspGachaData(gachaDataAddress).getGachaInfo(airdopInfo.tokenId);
            require(_gachaInfo.isValid, "not valid token id");
        } else if (uint256(GachaType.FateCore) == gachaType) {
            FateCoreGachaInfo memory fateCoreGachaInfo = DspGachaData(gachaDataAddress).getFateCoreGachaInfo(airdopInfo.tokenId);
    ...
    function airdropMany(AirdopInfo[] memory airdopInfo) external onlySuperOperator {
        ...
        for (uint256 i = 0; i < airdopInfo.length; i++) {
            uint256 gachaType = DspGachaData(gachaDataAddress).getGachaType(airdopInfo[i].tokenId);
            if (uint256(GachaType.Character) == gachaType) {
                GachaInfo memory _gachaInfo = DspGachaData(gachaDataAddress).getGachaInfo(airdopInfo[i].tokenId);
                require(_gachaInfo.isValid, "not valid token id");
            } else if (uint256(GachaType.FateCore) == gachaType) {
                FateCoreGachaInfo memory fateCoreGachaInfo = DspGachaData(gachaDataAddress).getFateCoreGachaInfo(airdopInfo[i].tokenId);
                require(fateCoreGachaInfo.isValid, "not valid token id");
            }
