    ...
    Version
{
     ...
     * @param timeFrame time between last and next interest accuring in seconds
     * @param amount principal of loan
     * @return interest interest to accrue on next interest accuring
     */
     ...
     * @param loanId id of the loan
     * @param toTimestamp timestamp of the next interest accuring
     * @return interest interest to accrue on next interest accuring to loan
     */
    ...
    function borrow(
        ...
        uint16 score = scoreDB.getCreditScoreAndValidate(nfcs.getToken(msg.sender));

        limitManager.onBorrow(msg.sender, pool, score, amount);
        ...
        if (collateralToAdd > 0) {
            if (msg.value > 0) {
                ...
                if (msg.value > collateralToAdd) {
                    (bool success, ) = payable(msg.sender).call{value: msg.value - collateralToAdd}(
                        ""
                    );
                    require(success, Errors.LOAN_MANAGER_NATIVE_RETURN);
                }
            } else {
        ...
        LoanLib.Loan memory loan = LoanLib.Loan(
            msg.sender,
            amount,
            s.interest,
            ltv,
        ...
        _loans[nextLoanId.current()] = loan;

        userLoanIds[msg.sender].push(nextLoanId.current());

        pool.underlyingToken().safeTransferFrom(address(pool), msg.sender, amount);

        emit LoanCreated(msg.sender, address(pool), nextLoanId.current(), loan.amount);
        ...
        )
    {
        ...
        loan.pool.updatePoolValue(info.poolValueAdjust - int256(treasuryShare));
        ...
        returns (LoanLib.DelinquencyInfo memory)
    {
        ...
        require(isDelinquent(loanId), Errors.LOAN_MANAGER_LOAN_IS_LINQUENT);
        ...
        //This case will occurif frozen collateral amount of the loan covers exact loan principal
...
interface ILoanManager {
    ...
    event LoanLiquidated(
        ...
        uint256 notCoveredBy,
        uint256 unfrozenCollateral,
        int256 poolValueDecreasedBy,
        uint256 timestamp
    );
...
library Errors {
    ...
    string public constant LOAN_MANAGER_LOAN_IS_LINQUENT = "202"; //Loan is liquient
    ...
    string public constant SCORE_DB_VERIFICATION = "402"; // Unverified score
    string public constant SCORE_DB_UNKNOW_FETCHING_SCORE = "405"; //  Unknown error fetching score.
...
library LoanLib {
    ...
    /**
     * @dev Status matrix is a 3 dimensional transitionstable
...
abstract contract SelectivePausable is PausableUpgradeable {
    ...
    /**
     * @dev whenNotPaused modifier can't be overriden, thus we added out modifier that implements both functionalities
     */
...
string constant COLLATERAL_MANAGER_VERSION = "1.0.4";
string constant LIMIT_MANAGER_VERSION = "1.0.2";
string constant LOAN_MANAGER_VERSION = "1.0.4";
string constant NFCS_VERSION = "1.0.4";
string constant POOL_VERSION = "1.0.1";
string constant PRICE_FEED_VERSION = "1.0.1";
string constant SCORE_DB_VERSION = "1.0.2";
string constant SETTINGS_PROVIDER_VERSION = "1.0.2";
