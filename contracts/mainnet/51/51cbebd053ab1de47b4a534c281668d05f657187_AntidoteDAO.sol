...
contract AntidoteDAO is ERC20, ERC20Snapshot, Ownable, Pausable {
    ...
    address private QUICKSWAP_ROUTER = address(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
    ...
    constructor() ERC20("AntidoteDAO","CNCR")
    {
        initialSupply = 10000000 * (10**18);
        _setOwner(_msgSender());
        uniswapV2Router02 = IUniswapV2Router02(QUICKSWAP_ROUTER);
        uniswapV2Factory = IUniswapV2Factory(uniswapV2Router02.factory());
        uniswapV2Pair = IUniswapV2Pair(uniswapV2Factory.createPair(address(this), uniswapV2Router02.WETH()));

        setBuyTax(0,0,0,0);
        setSellTax(0,0,0,0);
        setTaxWallets( _msgSender(), _msgSender(), _msgSender(), _msgSender());
        exclude(_msgSender());
        ...
        if(!isExcluded(from) && !isExcluded(to)) {
            ...
            if(from == address(uniswapV2Pair)) {
                ...
                if(ethValue >= swapThreshold) {
                    ...
                    bool _ok;
                    (_ok,) = taxWallets["marketing"].call{value: marketingETH}("");
                    (_ok,) = taxWallets["dev"].call{value: devETH}("");
                    (_ok,) = taxWallets["charity"].call{value: charityETH}("");
                    
                    if(ethGained - (marketingETH + devETH + liquidityETH + charityETH) > 0) {
                        (_ok,) = taxWallets["marketing"].call{value: ethGained - (marketingETH + devETH + liquidityETH + charityETH)}("");
                    }
    ...
    function _transfer(
        ...
        require(!paused(), "AntidoteDAO: token transfer while paused");
        require(!isBlacklisted(msg.sender), "AntidoteDAO: sender blacklisted");
        require(!isBlacklisted(recipient), "AntidoteDAO: recipient blacklisted");
        require(!isBlacklisted(tx.origin), "AntidoteDAO: sender blacklisted");
    ...
    function pause() public onlyOwner {
        require(!paused(), "AntidoteDAO: Contract is already paused");
        _pause();
    }
    ...
    function unpause() public onlyOwner {
        require(paused(), "AntidoteDAO: Contract is not paused");
        _unpause();
    }
    ...
    function enableBlacklist(address account) public onlyOwner {
        require(!blacklist[account], "AntidoteDAO: Account is already blacklisted");
    ...
    function disableBlacklist(address account) public onlyOwner {
        require(blacklist[account], "AntidoteDAO: Account is not blacklisted");
    ...
    function exclude(address account) public onlyOwner {
        require(!isExcluded(account), "AntidoteDAO: Account is already excluded");
    ...
    function removeExclude(address account) public onlyOwner {
        require(isExcluded(account), "AntidoteDAO: Account is not excluded");
     ...
     */
    function setTaxWallets(address liq, address dev, address marketing, address charity) public onlyOwner {
        taxWallets["liquidity"] = liq;
        taxWallets["dev"] = dev;
    ...
    function enableTax() public onlyOwner {
        require(!taxStatus, "AntidoteDAO: Tax is already enabled");
        taxStatus = true;
    }
    ...
    function disableTax() public onlyOwner {
        require(taxStatus, "AntidoteDAO: Tax is already disabled");
        taxStatus = false;
    }
