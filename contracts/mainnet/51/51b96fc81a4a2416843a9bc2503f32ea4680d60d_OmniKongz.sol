...
/// @dev
// File: contracts/OmniKongz-eth.sol
...
contract OmniKongz is Ownable, ERC721, NonblockingReceiver {
    using Strings for uint256;
    address public _owner;
    string private baseURI = "ipfs://QmWWj1D6P8snGqDXDvmWgmpKhFwGrFP3M5hyVhHtMNgfsH/";
    string baseExtension = ".json";
    uint256 nextTokenId = 0;
    uint256 MAX_MINT_POLYGON = 500;

    uint256 gasForDestinationLzReceive = 350000;

    constructor(address _layerZeroEndpoint) ERC721("OmniKongz", "OK") {
        _owner = msg.sender;
    ...
    function mint(uint8 numTokens) external payable {
        require(numTokens < 2, "Max 1 Kongz per transaction :(");
        require(
            nextTokenId + numTokens <= MAX_MINT_POLYGON,
            "There are no kongz left to be minted :("
        );
