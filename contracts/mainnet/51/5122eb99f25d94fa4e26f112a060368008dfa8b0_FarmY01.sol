...
uint public upT = 60;            uint private constant MAX_UPT = 604800; 
uint nivel = 11;                 event changeUpT(uint upT); 
   
function setUpT(uint _upT) external onlyOwner                                                  {
require(_upT <= MAX_UPT, "Wait please");      upT = _upT;             emit changeUpT(_upT);    }

function myNivel() public returns(uint) {      
    if (nivel==11 && block.timestamp > inicio[msg.sender] + upT)    {nivel=22;}
    if (nivel==22 && block.timestamp > inicio[msg.sender] + upT*2)  {nivel=33;}
    if (nivel==33 && block.timestamp > inicio[msg.sender] + upT*2)  {nivel=44;}
    if (nivel==44 && block.timestamp > inicio[msg.sender] + upT*2)  {nivel=55;}
    return (nivel);                     }
