...
contract HOMGOLDToken is BEP20, MinterRole {
    ...
    uint256 public sellFee = 5;   // default 10%
    uint256 public minSwapAmount = 100000000;   //0.1 HOMGOLD tokens
    ...
    /**z
     * @notice Constructs the HOMGOLD token contract.
     */
    constructor(address _treasury, address _lpcontract) public BEP20("HOM-GOLDToken", "HOM-GOLD") {
        operator = _msgSender();
    ...
    /// @dev overrides transfer function to meet tokenomics of HOMGOLD
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "HOMGOLD::transferOperator: new operator is the zero address");
    ...
    function updateBuyFee(uint16 _buyFee) public onlyOperator {
        require( _buyFee <= 100, "HOMGOLD:: Biu Fee can't exceed 100%" );
        emit UpdateBuyFee(_buyFee);
        buyFee = _buyFee;
    }

    function updateSellFee(uint16 _sellFee) public onlyOperator {
        require( _sellFee <= 100, "HOMGOLD:: Sell Fee can't exceed 100%");
    ...
    function updateFeeWallet(address _treasury) public onlyOperator {
        require( _treasury != address(0), "HOMGOLD:: tax fee address is zero" );
    ...
    function updateLpcontract(address _lpcontract) public onlyOperator {
        require( _lpcontract != address(0), "HOMGOLD:: lpcotontract address is not zero" );
