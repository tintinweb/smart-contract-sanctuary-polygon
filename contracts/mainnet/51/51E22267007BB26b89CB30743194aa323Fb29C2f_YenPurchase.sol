...
contract YenPurchase is Ownable {
  ...
  constructor(
    address _yenToken,
    address jpy_usd_feed,
    address uth_usd_feed
  ) {
    supplier = msg.sender;
    yenToken = _yenToken;
  ...
  function getPriceFeedContract(address _tokenAddress)
    external
    view
    returns (AggregatorInterface contractAddress)
  {
   ...
   */
  function getETHAmountFromYen(uint256 _yenAmount)
    public
    view
   ...
   */
  function getYenAmountFromETH(uint256 _ethAmount)
    public
    view
    returns (uint256 yenAmount)
  {
   ...
   */
  function purchaseExactYenWithETH(uint256 _yenAmount, uint256 _amountOutMax)
    external
    payable
  {
    require(
      minimumPurchaseAmount <= _yenAmount &&
        _yenAmount <= maximumPurchaseAmount,
      'purchase amount must be within purchase range'
    );
    require(
      _yenAmount <= IERC20(yenToken).allowance(supplier, address(this)),
      'insufficient allowance of Yen'
    );
    ...
    require(
      msg.value >= ethAmount,
      'msg.value must greater than calculated ether amount'
    );
   ...
   */
  function purchaseYenWithExactETH(uint256 _amountInMin) external payable {
    uint256 yenAmountFromEth = getYenAmountFromETH(msg.value);
    require(
      minimumPurchaseAmount <= yenAmountFromEth &&
        yenAmountFromEth <= maximumPurchaseAmount,
      'purchase amount must be within purchase range'
    );
    require(
      yenAmountFromEth <= IERC20(yenToken).allowance(supplier, address(this)),
      'insufficient allowance of YEN'
    );
   ...
   */
  function getERC20AmountFromYen(uint256 _yenAmount, address _tokenAddress)
    public
    view
    returns (uint256 erc20Amount)
  {
    uint256 usdAmount = uint256(getLatestJpyUsdPrice()).mul(_yenAmount).div(
      10**(decimals.sub(IERC20(_tokenAddress).decimals()))
    );
    return
      erc20Amount = usdAmount.div(
        uint256(getLatestERC20UsdPrice(_tokenAddress))
      );
  }
   ...
   */
  function getYenAmountFromERC20(uint256 _erc20Amount, address _tokenAddress)
    public
    view
    returns (uint256 yenAmount)
  {
    uint256 usdAmount = uint256(getLatestERC20UsdPrice(_tokenAddress))
      .mul(_erc20Amount)
      .mul(10**(decimals.sub(IERC20(_tokenAddress).decimals())));
    return yenAmount = usdAmount.div(uint256(getLatestJpyUsdPrice()));
  }
   ...
   */
  function purchaseExactYenWithERC20(
    uint256 _yenAmount,
    ...
    require(
      IERC20(_tokenAddress).balanceOf(msg.sender) >= erc20Amount,
   ...
   */
  function purchaseYenWithExactERC20(
    uint256 _erc20Amount,
    uint256 _amountInMin,
    address _tokenAddress
  ) external {
    uint256 yenAmountFromERC20 = getYenAmountFromERC20(
      _erc20Amount,
      _tokenAddress
    );
    require(
      minimumPurchaseAmount <= yenAmountFromERC20 &&
        yenAmountFromERC20 <= maximumPurchaseAmount,
      'purchase amount must be within purchase range'
    );
    require(
      yenAmountFromERC20 <= IERC20(yenToken).allowance(supplier, address(this)),
      'insufficient allowance of YEN'
    );
    require(yenAmountFromERC20 >= _amountInMin, 'excessive slippage amount');

    require(
      IERC20(_tokenAddress).balanceOf(msg.sender) >= _erc20Amount,
      'insufficient balance of ERC20 token'
    );
