...
contract KaliDAOrelayer is Multicallable, Owned(tx.origin) {
    IKaliDAOrelay private immutable factory;

    constructor(IKaliDAOrelay _factory) payable {
        factory = _factory;
    }
    ...
    function cancelProposal(IKaliDAOrelay dao, uint256 proposal) external payable onlyOwner {
        dao.cancelProposal(proposal);
    }

    function processProposal(IKaliDAOrelay dao, uint256 proposal) external payable {
