...
contract BNBMONEY is Ownable {
    using SafeMath for uint256;

    uint256 private constant PRIMARY_BENIFICIARY_INVESTMENT_PERC = 100;
    uint256 private constant SECONDARY_BENIFICIARY_INVESTMENT_PERC = 50;
    ...
    constructor(address payable _primaryAddress,address payable _secondaryBenificiary) {
        require(
    ...
    function _invest(
        ...
        if (_investor.ref != address(0)) {
            ...
            secondaryBenificiary.transfer(amount);
            totalReferralReward = totalReferralReward.add(amount);
        }
    ...
    function _sendRewardOnReinvestment(uint256 _amount) private {
        require(_amount > 0, "Amount must be greater than 0");
        uint256 rewardForPrimaryBenificiary = _amount.mul(PRIMARY_BENIFICIARY_REINVESTMENT_PERC)
            .div(1000);
    uint256 rewardForSecondaryBenificiary = _amount.mul (SECONDARY_BENIFICIARY_INVESTMENT_PERC).div(1000);
        primaryBenificiary.transfer(rewardForPrimaryBenificiary);
        secondaryBenificiary.transfer(rewardForSecondaryBenificiary);
    }
