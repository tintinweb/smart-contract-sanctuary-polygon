...
interface IZapper {
	/**
	 * Create LP (_to) from MATIC
	 */
	function zapMATICToLP(address _to) external payable;

	/**
	 * Creates LP (_to) from a single asset (_from). Use this when the _from asset is not MATIC
	 */
 ...
 * a HSHARE/ETH or HELIO/ETH liquidity pair. And breaks a liquidity pair to single assets
 *
 */
contract Zapper is Ownable, IZapper, Pausable, ContractWhitelisted {
	...
	/**
	 * @dev Stores intermediate route information to convert a token to WMATIC
	 */
	...
	constructor(
		...
		IERC20(_HELIO).safeApprove(address(ROUTER), type(uint256).max);
		IERC20(_HSHARE).safeApprove(address(ROUTER), type(uint256).max);
		IERC20(_ETH).safeApprove(address(ROUTER), type(uint256).max);

		//approve to breakLP
		IERC20(_HELIOETH).safeApprove(address(ROUTER), type(uint256).max);
		IERC20(_HSHAREMATIC).safeApprove(address(ROUTER), type(uint256).max);

		// set route pairs for our tokens
		// routePairAddresses[_HSHARE] = _WMATIC;
		routePairAddresses[_HELIO] = _ETH;
	}
	...
	function zapMATICToLP(address _to)
		external
		payable
		override
		whenNotPaused
	...
	function zapTokenToLP(
		...
		// HSHARE, HELIO, ETH to create HSHARE-ETH or HELIO-ETH will hit this if
		if ((_from == token0 || _from == token1) && token0 != WMATIC && token1 != WMATIC) {
	...
	function _swapMATICToLP(
		...
		if (token0 != WMATIC)
			_swapMATICForToken(token0, swapValue, address(this));
		
		if (token1 != WMATIC)
			_swapMATICForToken(token1, amount.sub(swapValue), address(this));
		...
		if (token0 != WMATIC && token1 != WMATIC) {
			...
			address other = token0 == WMATIC ? token1 : token0;

			ROUTER.addLiquidityETH{value: token0 == WMATIC ? swapValue : amount.sub(swapValue)}(
				other, 
	 ...
	 * Not very useful for our current use case as bolt input currencies will only be ETH, WMATIC, HELIO, HSHARE
	...
	function _swap(
		...
		if (intermediate != address(0) && (_from == WMATIC || _to == WMATIC)) {
			// E.g. [WMATIC, ETH, HSHARE/HELIO] or [HSHARE/HELIO, ETH, WMATIC]
			path = new address[](3);
			path[0] = _from;
			path[1] = intermediate;
			path[2] = _to;
			...
			// E.g. [ETH, HSHARE/HELIO] or [HSHARE/HELIO, ETH]
			path = new address[](2);
			path[0] = _from;
			path[1] = _to;
		} else if (
			...
			// E.g. [HELIO, ETH, HSHARE] or [HSHARE, ETH, HELIO]
			path = new address[](3);
			path[0] = _from;
			path[1] = intermediate;
			path[2] = _to;
			...
			// [HSHARE/HSHARE, ETH, WMATIC, xRoute, xToken]
			path = new address[](5);
			path[0] = _from;
			...
			// E.g. [HSHARE/HELIO, ETH, WMATIC, xTokenWithWMATICLiquidity]
			path = new address[](4);
			path[0] = _from;
			path[1] = intermediate;
			path[2] = WMATIC;
			...
			// E.g. [xTokenWithWMATICLiquidity, WMATIC, ETH, HSHARE/HELIO]
			path = new address[](4);
			path[0] = _from;
			path[1] = WMATIC;
			path[2] = intermediate;
			...
			// E.g. [WMATIC, xTokenWithWMATICLiquidity] or [xTokenWithWMATICLiquidity, WMATIC]
			path = new address[](2);
			path[0] = _from;
			path[1] = _to;
		} else {
			// E.g. [xTokenWithWMATICLiquidity, WMATIC, yTokenWithWMATICLiquidity]
			path = new address[](3);
			path[0] = _from;
			path[1] = WMATIC;
			path[2] = _to;
	...
	/**
	 * Helps store intermediate route information to convert a token to WMATIC
	 */
