...
contract TokenPaymentSplitter is Managable, ReentrancyGuard {
    ...
    event SetReferrerStorage(address _address);
    ...
    constructor(
        ...
        _setReferrerStorage(_referrerStorage);
    ...
    function split(address _token, address _sender, uint256 _amount) external payable nonReentrant {
        ...
        // Rewarding referrers
        (address _referrer, uint256 _royaltyPercMulByHundreed) = IReferrerStorage(referrerStorage).getReferreeComission(_sender);
        uint256 _payment = 0;
        if (_referrer != address(0) && _royaltyPercMulByHundreed > 0) {
            _payment = _amount * _royaltyPercMulByHundreed / 10000;
            require(_payment <= _amount, "incorrect referrer royalty");
            emit PaymentSplit(_token, _amount, _payment, _royaltyPercMulByHundreed);
             
            if(_token == ZERO_ADDRESS){
                (bool success,) = referrerStorage.call{value: _payment, gas: 150000}(
                    abi.encodeWithSignature("rewardReferrer(address,address,uint256)", _referrer, _token, _payment)
                );
                require(success, "referrerStorage native call fail");
            } else {
                (bool success,) = referrerStorage.call{value: 0, gas: 150000}(
                    abi.encodeWithSignature("rewardReferrer(address,address,uint256)", _referrer, _token, _payment)
                );
                require(success, "referrerStorage call fail");
            }
        }
        ...
        if (externalShare != address(0)) {
            ...
            if (_receiver != address(0) && _share > 0) {
                _payment2 = _amountLeft * _share / 10000;
                emit PaymentSplit(_token, _amount, _payment2, _share);
                 
                if(_token == ZERO_ADDRESS){
                    payable(_receiver).transfer(_payment2);
                } else {
                    IERC20(_token).safeTransfer(_receiver, _payment2);  
                }
            }
            _amountLeft -= _payment2; 
        }
    ...
    function setReferrerStorage(address _address) external onlyManager {
        _setReferrerStorage(_address);
    }
    ...
    function _setReferrerStorage(address _address) internal {
        referrerStorage = _address;
        emit SetReferrerStorage(_address);
    }
