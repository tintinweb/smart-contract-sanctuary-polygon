...
contract Summoning is Ownable, VRFConsumerBase {
    ...
    event SummoningSuccessful(
        address indexed requester,
        uint256 indexed orbID,
        uint8 indexed orbType,
        uint256 newNftId
    );
    ...
    event UpgradeRequested(
        address indexed requester,
        bool indexed isProphet,
        uint256[5] orbIDs,
        bytes32 requestID
    );
    event UpgradeSuccessful(
        address indexed requester,
    ...
    event NewWhitelisted(address whitelisted);
    event RemoveWhitelisted(address whitelisted);
    ...
    modifier isUnique(uint256[5] memory ids) {
        for (uint256 i = 0; i < 4; i++) {
            for (uint256 j = i + 1; j < 5; j++) {
                require(ids[i] != ids[j], "all ids must be unique");
            }
        }
        _;
    }

    modifier owningItems(INFT nft, uint256[5] memory ids, address beneficiary) {
        for (uint256 i = 0; i < 5; i++) {
            require(nft.ownerOf(ids[i]) == beneficiary, "not owning the items");
        }
        _;
    }

    modifier onlyWhitelisted() {
        require(_whitelisted[_msgSender()], "msg.sender should be whitelisted");
        _;
    }
    ...
    function summon(uint256 orbID, address beneficiary) external onlyWhitelisted {
        require(orb.balanceOf(beneficiary, orbID) > 0, "not owning the orb");
        require(!isSummoning[beneficiary][orbID], "already requested to summon");
        isSummoning[beneficiary][orbID] = true;

        (, uint8 rarity, ) = orb.orbs(orbID);

        bytes32 requestID = getRandomNumber();
        SummoningRequest storage request = summoningRequests[requestID];
        request.requester = beneficiary;
        request.orbID = orbID;

        emit SummoningRequested(beneficiary, orbID, requestID);
    }
                    ...
                    if (orbType == 0) {
                        if (
                            ...
                            try prophet.mint(
                                    ...
                                    rarity,
                                    uint16(
                                        _getLocalRandomness(randomness) %
                                            numRacePerGen[generation]
                                    ),
                                    uint16(
                                        _getReverseLocalRandomness(randomness) %
                                            numCharPerGen[generation]
                                    )
                                )
                            returns (uint256 nftId) {
                                emit SummoningSuccessful(
                                    request.requester,
                                    request.orbID,
                                    orbType,
                                    nftId
    ...
    function upgradeProphet(uint256[5] memory ids, address beneficiary)
        external
        isUnique(ids)
        owningItems(prophet, ids, beneficiary)
        onlyWhitelisted
    {
        for (uint256 i = 0; i < 4; i++) {
            require(
        ...
        emit UpgradeRequested(beneficiary, true, ids, requestID);
    }

    function upgradeItem(uint256[5] memory ids, address beneficiary)
        external
        isUnique(ids)
        owningItems(item, ids, beneficiary)
        onlyWhitelisted
    {
        for (uint256 i = 0; i < 4; i++) {
            require(
        ...
        emit UpgradeRequested(beneficiary, false, ids, requestID);
    }
    ...
    /**
    * Get Rnandom based on link + salt
    */
    function _getLocalRandomness(uint256 randomness)
        internal returns (uint256)
    {   
        return uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, randomness)));
    }

    function _getReverseLocalRandomness(uint256 randomness)
        internal returns (uint256)
    {   
        return uint256(keccak256(abi.encodePacked(randomness, block.difficulty, block.timestamp)));
    }
    ...
    function updateVrfCoordinator(address _vrfCoordinator)
        external
        onlyOwner
    {
        require(vrfCoordinator != _vrfCoordinator, "Must be different than existing");
    ...
    /**
    * Whitelisting addresses that could interact
    */

    function addWhitelisted(address _addr) public onlyOwner {
        require(!_whitelisted[_addr], "user is already whitelisted");
        _whitelisted[_addr] = true;

        emit NewWhitelisted(_addr);
    }

    function removeWhitelisted(address _addr) public onlyOwner {
        require(_whitelisted[_addr], "user is not whitelisted");
        _whitelisted[_addr] = false;

        emit RemoveWhitelisted(_addr);
    }
}
