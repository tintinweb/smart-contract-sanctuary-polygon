...
interface IRoyaltyPool {
    function getInfoFromMarket(
        uint256 _royaltyAmount,
        address payable _businessOwner,
        address payable _buyer,
        address payable _marketOwner
    ) external payable;
}

contract AdValoremNFTMarketplace is ReentrancyGuard, OwnableUpgradeable {
    ...
    function initialize(
        address payable _royaltyContract,
        address _vlrTokenContract
    ...
    function depositRewardToken(uint256 _amount) external onlyOwner {
        uint256 originalBalance = IBEP20(vlrTokenContract).balanceOf(
            address(this)
        );
        ...
        uint256 currentBalance = IBEP20(vlrTokenContract).balanceOf(
            address(this)
        );
        totalRewardSupply = totalRewardSupply.add(
    ...
    /* createMultiMarketItem */
    function createMultiMarketItem(PMarketItem[] memory pmarketItems)
        public
        nonReentrant
    {
        for (uint256 i = 0; i < pmarketItems.length; i++) {
            createMarketItem(
                pmarketItems[i].nftContract,
                pmarketItems[i].tokenId,
                pmarketItems[i].businessOwnerPercent,
                pmarketItems[i].sellerPercent,
                pmarketItems[i].royaltyPercent
            );
        }
    }
    ...
    function createMarketItem(
        ...
        require(
            businessOwnerPercent + sellerPercent + royaltyPercent == 10000,
            "NFTMarketplace: The sum of all percentages should be 100%"
        );
        require(
            businessOwners[tokenId] == address(0) ||
                businessOwners[tokenId] == msg.sender,
            "NFTMarketplace: Not authorized"
        );
    ...
    /* sell market item */
    function sellMarketItem(uint256 itemId, uint256 price)
        public
        payable
        nonReentrant
    {
        MarketItem storage item = idToMarketItem[itemId];
        address nftOwnerAddress = IERC721(item.nftContract).ownerOf(
            item.tokenId
        );
        require(
            nftOwnerAddress == msg.sender,
            "NFTMarketplace : You are incorrect owner"
        );
        require(
            item.status != ListingStatus.Listed,
            "NFTMarketplace : NFT token is not active"
        );
    ...
    function buyMarketItem(uint256 itemId) public payable nonReentrant {
        ...
        require(
            item.status == ListingStatus.Listed,
            "NFTMarketplace : NFT token is not listed"
        );
        require(
            nftOwnerAddress == address(this),
            "NFTMarketplace : Marketplace doesn't have NFT token"
        );
        require(
            item.seller != msg.sender,
            "NFTMarketplace : You have already listed this NFT token"
        );

        uint256 feeAmount = (item.price * marketFeePercent) / 10000;
        uint256 businessOwnerAmount = ((item.price - feeAmount) *
            item.businessOwnerPercent) / 10000;
        uint256 sellerAmount = ((item.price - feeAmount) * item.sellerPercent) /
            10000;
        uint256 royaltyAmount = ((item.price - feeAmount) *
            item.royaltyPercent) / 10000;
        ...
        uint256 realRoyaltyAmount = royaltyAmount -
            (vlrTokenTaxFee * royaltyAmount) /
            10000;
        oldRoyaltyContract.getInfoFromMarket(
            realRoyaltyAmount,
            payable(item.businessOwner),
            payable(msg.sender),
    ...
    function cancelSale(uint256 itemId) public {
        ...
        require(
            ...
            "NFTMarketplace : Only seller can cancel NFT token"
        );
        require(
            item.status == ListingStatus.Listed,
            "NFTMarketplace : This NFT token is not listed"
        );
    ...
    //only owner
    function seMarketFeePercent(uint256 _marketFeePercent) public onlyOwner {
    ...
    function setRoyaltyContract(address payable _royaltyContract)
        public
        onlyOwner
    {
        royaltyContract = _royaltyContract;
    ...
    function setRoyaltyPool(
        ...
        MarketItem storage item = idToMarketItem[itemId];
        require(
            msg.sender == item.businessOwner,
            "NFTMarketplace : Not business Owner"
        );
