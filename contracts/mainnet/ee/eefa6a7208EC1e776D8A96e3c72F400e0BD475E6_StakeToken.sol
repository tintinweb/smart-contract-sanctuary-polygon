...
contract StakeToken is Ownable, Stakeable{
  ...
  */
  ...
  IERC20 public _usdt;
  bool private _ICO;
  uint256 private _unitsOneUSDTCanBuy;
  ...
  */
  constructor(string memory token_name, string memory short_symbol, uint8 token_decimals, uint256 token_totalSupply, address _usdt_address, uint256 unitsOneUSDTCanBuy){
      _name = token_name;
      _symbol = short_symbol;
      _decimals = token_decimals;
      _totalSupply = token_totalSupply;
      _usdt = IERC20(_usdt_address);
      _unitsOneUSDTCanBuy = unitsOneUSDTCanBuy;
      _ICO = true;
   ...
   */
    ...
    * @notice withdrawStake is used to withdraw stakes from the account holder
    ...
    function buyTokens(uint256 amount) external {
      uint cost = amount * _unitsOneUSDTCanBuy;
      require(_usdt.allowance(msg.sender, address(this)) >= cost, "Not enough allowance");

      _transfer(owner(), msg.sender, amount);
      // emit an event to inform of the transfer        
      emit Transfer(owner(), msg.sender, amount);

      _usdt.transferFrom(msg.sender, address(this), cost);
    }  
}