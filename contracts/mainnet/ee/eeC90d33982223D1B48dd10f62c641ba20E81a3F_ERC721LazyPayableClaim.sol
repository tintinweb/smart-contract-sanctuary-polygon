...
contract ERC721LazyPayableClaim is IERC165, IERC721LazyPayableClaim, ICreatorExtensionTokenURI, LazyPayableClaim {
    ...
    function initializeClaim(
        ...
        uint256[] memory claim_token_cost;
        ...
        _claims[creatorContractAddress][instanceId] = Claim({
            ...
            membership: claimParameters.membership,
            token_cost: claim_token_cost
        });

        _claims[creatorContractAddress][instanceId].token_cost.push(0);
    ...
    function mint(address creatorContractAddress, uint256 instanceId, uint32 mintIndex, bytes32[] calldata merkleProof, address mintFor) external payable override {
        ...
        uint256 platformFee = getMintFeeEvent(creatorContractAddress);
        bool isMembership = _validateMembership(mintFor, claim.membership);
        if (isMembership) {
            // Transfer funds
            _transferFunds(platformFee, claim.erc20, 0, claim.paymentReceiver, 1, claim.merkleRoot != "");
        }
        else {
            uint256 mintCostByValueSent = msg.value - platformFee;
            bool validPrice = _validatePrice(mintCostByValueSent, claim.cost);
            
            require(validPrice, "Insufficient funds");
 
            // Transfer funds
            _transferFunds(platformFee, claim.erc20, mintCostByValueSent, claim.paymentReceiver, 1, claim.merkleRoot != "");
        }
        ...
        if (claim.contractVersion >= 3) {
            ...
            newTokenId = IERC721CreatorCore(creatorContractAddress).mintExtension(msg.sender, tokenData);
        } else {
            newTokenId = IERC721CreatorCore(creatorContractAddress).mintExtension(msg.sender);
        ...
        claim.token_cost.push(msg.value - platformFee);
    ...
    function mintBatch(address creatorContractAddress, uint256 instanceId, uint16 mintCount, uint32[] calldata mintIndices, bytes32[][] calldata merkleProofs, address mintFor) external payable override {
        ...
        if (isMembership) {
            // Transfer funds
            _transferFunds(platformFee, claim.erc20, 0, claim.paymentReceiver, mintCount, claim.merkleRoot != "");
        }
        else {
            uint256 mintCostByValueSent = msg.value / mintCount - platformFee;
            bool validPrice = _validatePrice(mintCostByValueSent, claim.cost);
            
            require(validPrice, "Insufficient funds");
            // Transfer funds
            _transferFunds(platformFee, claim.erc20, mintCostByValueSent, claim.paymentReceiver, mintCount, claim.merkleRoot != "");
        }
        ...
        if (claim.contractVersion >= 3) {
            ...
            newTokenIds = IERC721CreatorCore(creatorContractAddress).mintExtensionBatch(msg.sender, tokenData);
        } else {
            newTokenIds = IERC721CreatorCore(creatorContractAddress).mintExtensionBatch(msg.sender, mintCount);
            for (uint256 i; i < mintCount;) {
        ...
        for (uint256 i; i < mintCount;) {
            claim.token_cost.push(msg.value / mintCount - platformFee);
            unchecked { ++i; }
        }
    ...
    //     uint256 platformFee = getMintFeeEvent(creatorContractAddress);
    //     // Transfer funds
    //     _transferFundsProxy(platformFee, claim.erc20, claim.cost, claim.paymentReceiver, mintCount, claim.merkleRoot != "");
...
interface IERC721LazyPayableClaim is ILazyPayableClaim {
    struct ClaimParameters {
        uint32 totalMax;
        uint32 walletMax; // maximum token a wallet can own
        uint48 startDate;
        uint48 endDate;
        StorageProtocol storageProtocol;
        bool identical;
...
interface ILazyPayableClaim {
    ...
    /**
     * @notice Set the platform mint fee
     */
    function setMintFee(uint256 newMintFee) external;

    /**
     * @notice Get the corporate platform mint fee
     */
    function getMintFeeEvent(address eventAddress) external returns(uint256); 

    /**
     * @notice Set the corporate platform mint fee
     */
    function setMintFeeEvent(address eventAddress, uint256 mintFee) external; 
...
abstract contract LazyPayableClaim is ILazyPayableClaim, AdminControl {
    ...
    mapping(address => uint256) public MINT_FEE_EVENT;
    mapping(address => bool) public HAS_MINT_FEE_RESET;
    mapping(address => uint256[]) public PRICE_EVENT;
     ...
     */
    function setMintFee(uint256 newMintFee) external override onlyOwner {
        MINT_FEE = newMintFee;
    }
    ...
    /**
     * See {ILazyPayableClaim-getMintFeeEvent}.
     */
    function getMintFeeEvent(address eventAddress) public view returns (uint256) {
        return HAS_MINT_FEE_RESET[eventAddress] ? MINT_FEE_EVENT[eventAddress] : MINT_FEE;
    }

    /**
     * See {ILazyPayableClaim-setMintFeeCorporate}.
     */
    function setMintFeeEvent(address eventAddress, uint256 mintFee) external override adminRequired {
        require(mintFee >= 0, "Invalid platform fee");
        if (!HAS_MINT_FEE_RESET[eventAddress]) {
            HAS_MINT_FEE_RESET[eventAddress] = true;
        }

        MINT_FEE_EVENT[eventAddress] = mintFee;
    }
    ...
    function _transferFunds(uint256 platformFee, address erc20, uint256 cost, address payable recipient, uint16 mintCount, bool merkle) internal {
        uint256 payableCost;
        ...
        if (MEMBERSHIP_ADDRESS != ADDRESS_ZERO) {
            if (!IManifoldMembership(MEMBERSHIP_ADDRESS).isActiveMember(msg.sender)) {
                payableCost += merkle ? MINT_FEE_MERKLE : platformFee; 
            }
        } else {
            payableCost += merkle ? MINT_FEE_MERKLE : platformFee; 
        }
        ...
        // Check price
        require(msg.value >= payableCost, "Invalid amount"); // make sure the called value more than payableCost
    ...
    function _transferFundsProxy(uint256 platformFee, address erc20, uint256 cost, address payable recipient, uint16 mintCount, bool merkle) internal {
        require(erc20 == address(0) && _proxyAddresses.contains(msg.sender), "Not approved");
        uint256 payableCost = cost + (merkle ? MINT_FEE_MERKLE : platformFee);
        if (mintCount > 1) {
            payableCost *= mintCount;
            cost *= mintCount;
        }
    ...
    function _validateMembership(address mintFor, address[] storage claimMembership) internal view returns (bool) {
        require(claimMembership.length < 256, "Invalid claim membership");
        for (uint256 i = 0; i < claimMembership.length;) {
            if (IERC721(claimMembership[i]).balanceOf(mintFor) > 0) {
                return true;
            }
            unchecked {
                ++i;
    ...
    function _validatePrice(uint256 price, uint256[] storage cost) internal view returns (bool) {
        for (uint i = 0; i < cost.length;) {
            if (cost[i] == price) {
                return true;
            }
            unchecked { ++i; }
