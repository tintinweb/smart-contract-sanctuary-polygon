...
contract MainVault is Ownable {
    ...
    //Tokenomi
    uint256 public crowdfundingShare = 613_820_200 ether;
    ...
    uint256 public treasuryShare = 226_179_800 ether;
    uint256 public stakingShare = 300_000_000 ether;
    uint256 public advisorShare = 150_000_000 ether;
    uint256 public airdropShare = 150_000_000 ether;
    ...
    constructor(IManagers _managers, address _botPreventionAddress) {
        managers = _managers;
        // soulsToken = new SoulsToken("SOULS Token", "SOULS", address(managers), _botPreventionAddress); 
        soulsToken = new SoulsToken("SLS Token", "SLS", address(managers), _botPreventionAddress); //TODO: change with line above
	...
	// //Write Functions
    // function initStakingVault(address _stakingVaultAddress, uint256 _initialReleaseDate) external onlyOwner {
    //     if (stakingVaultAddress != address(0)) {
    //         revert AlreadyInitialized();
    //     }
    //     if (_stakingVaultAddress == address(0)) {
    //         revert ZeroAddress();
    //     }
    ...
    //     string memory _title = "Init Staking Vault";
    //     bytes memory _encodedValues = abi.encode(_stakingVaultAddress, _initialReleaseDate);
    ...
    //         IVault _stakingVault = IVault(stakingVaultAddress);
    //         soulsToken.approve(stakingVaultAddress, stakingShare);

    //         _stakingVault.createVestings(stakingShare, 0, 0, 6, _initialReleaseDate + 90 days, 90);

    //         managers.addAddressToTrustedSources(stakingVaultAddress, "Staking Vault");
    ...
    function initLiquidityVault(
        ...
        if (
            !stableToken.transferFrom(msg.sender, liquidityVaultAddress, _liquidityVault.stableAmountForInitialLiquidity())
        ) {
    ...
    //Managers Function
    /** TEST INFO
	 **** Managers can transfer LP tokens from Main Vault contract when reach unlock time
	 * Kilit açılma zamanından önce fonksiyon çağırıldığında 'StillLocked()' hatasının döndüğü gözlemlenmiştir.
	 * Blok zamanı kilit açılma zamanına (Vault init edilirken 365 gün sonrasına set edilen değişken) simüle edildikten 
	 sonra fonksiyon çağırılıdğında LP tokenların başarılı bir şekilde alıcı adrese transfer edilebildiği gözlemlenmiştir.
	 */
    function withdrawLPTokens(address _to) external onlyManager {
        ...
        if (managers.isApproved(_title, _encodedValues)) {
            if (!IERC20(dexPairAddress).transfer(_to, _tokenBalance)) {
                revert TokenTransferError();
            }
            managers.deleteTopic(_title);
        }
    ...
    //FIXME: Below lines for test purpose and will be deleted for production.

    address public approveTopicTestVariable;
    ...
    function testApproveTopicFunction(address _testAddress) external onlyManager {
        ...
        bytes memory _encodedValues = abi.encode(_testAddress);
        managers.approveTopic(_title, _encodedValues);

        if (managers.isApproved(_title, _encodedValues)) {
            approveTopicTestVariable = _testAddress;
            managers.deleteTopic(_title);
        }
    }
...
contract SoulsToken is ERC20Burnable, Ownable {
    ...
    /** TEST INFO
     **** Managers can disable/enable bot prevention by voting
     * 3 manager tarafından contract üstünde bot preventionın disable edilmiştir.
     * Arkasından arka arkaya iki defa 1000 token satılmasının başarılı olduğu gözlemlenmiştir.
     * 3 manager tarafından contract üstünde bot preventionın enable edilmiştir.
     * Token satışı denendiğinde 'TransferHelper: TRANSFER_FROM_FAILED' hatası döndüğü gözlemlenmiştir.
     * Bir trade işlemini yapabileceği zamana simüle edilmiş ve trade işleminin başarılı şekilde yapılabildiği gözlemlenmiştir.
     */
    ...
    /** TEST INFO
     * Tested with disableBotPrevention function
     */
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal view override {
        if (botPreventionEnabled && from != address(0)) {
            if (!managers.isManager(tx.origin)) {
