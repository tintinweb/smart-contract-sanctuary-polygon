...
contract OPTFund is Ownable, ReentrancyGuard {
    ...
    address private devWallet = 0x17400Ca22b75870bea1b3Ff9F2276c13d516FF32;

    address public forLQWallet = 0x7D2468fbb9Bedb2B46705E6A04d05d0f0Ef695eE; // i will convert DAI to USDT and send it to pair
    ...
    function existsInDepositors(address _address) public view returns (bool) {
    ...
    function depositOPT3(uint256 _amount) external {
        ...
        if(existsInDepositors(msg.sender)) {
        ...
        if(!existsInDepositors(msg.sender)) {
            IBEP20(OPT3).transferFrom(msg.sender, address(this), _amount);
            uint256 dID = adrToID[msg.sender];
    ...
    mapping(address => LoanStruct) public adrToLoanStruct;
    ...
    function borrowUsingOPT3(uint256 _OPT3Amount) external {
        require(borrowingOpen = true, 'borrowing closed');

        IBEP20(OPT3).transferFrom(msg.sender, address(this), _OPT3Amount);

        LoanStruct memory loan = adrToLoanStruct[msg.sender];

        require(!loan.locked, "Already borrowed");
        ...
        adrToLoanStruct[msg.sender].locked = true;
        adrToLoanStruct[msg.sender].taker = msg.sender;
        adrToLoanStruct[msg.sender].CollateralOPT3Amount = _OPT3Amount;
        adrToLoanStruct[msg.sender].lastRebaseClaim = block.timestamp;
        adrToLoanStruct[msg.sender].USDTBorrowed = loanAmount;
        adrToLoanStruct[msg.sender].OPT3PriceAtBorrowing = currentPrice;
        adrToLoanStruct[msg.sender].liquidationPrice = liquidPrice;

        IERC20(USDT).transfer(msg.sender, loanAmountToSend);
    }

    function calculateAmountToRepay(address _address) public view returns (uint256) {
        uint256 amountToRepay = (adrToLoanStruct[_address].USDTBorrowed).add((adrToLoanStruct[_address].USDTBorrowed).div(100).mul(8));
        return amountToRepay;
    }

    function calculateRepayingToDev(address _address) public view returns (uint256) {
        uint256 amountToDev = (adrToLoanStruct[_address].USDTBorrowed).div(100).mul(4);
        return amountToDev;
    }

    function calculateRepayingForOptimusLQ(address _address) public view returns (uint256) {
        uint256 amountToLQ = (adrToLoanStruct[_address].USDTBorrowed).div(100).mul(4);
        return amountToLQ;
    }

    function calculateLoanRebases(address _address) public view returns (uint256) {

        uint256 lastRoiTime = block.timestamp - adrToLoanStruct[_address].lastRebaseClaim;

        uint256 allClaimableAmount = (lastRoiTime * adrToLoanStruct[_address].CollateralOPT3Amount * dailyRebase).div(percentRate * rewardPeriod);

        return allClaimableAmount;
    }

    function claimLoanRebases(address _address) public {
        require(adrToLoanStruct[_address].taker == msg.sender);

        uint256 toClaim = calculateLoanRebases(_address);

        adrToLoanStruct[_address].lastRebaseClaim = block.timestamp;

        IBEP20(OPT3).transfer(msg.sender, toClaim);
    }

    function repayLoan(address _address) external {
        require(adrToLoanStruct[_address].taker == msg.sender);

        uint256 currentPrice = getOPT3Price();

        require(currentPrice > adrToLoanStruct[_address].liquidationPrice, 'Loan is currently locked (price too low)');

        uint256 OPT3ToReturn = adrToLoanStruct[_address].CollateralOPT3Amount;

        uint256 repayingAmount = calculateAmountToRepay(_address);
        uint256 userNeedsToPay = repayingAmount.div(1000000);
        uint256 forDev = calculateRepayingToDev(_address);
        uint256 devPay = forDev.div(1000000);
        uint256 forLQ = calculateRepayingForOptimusLQ(_address);
        uint256 LQPay = forLQ.div(1000000);

        adrToLoanStruct[_address].CollateralOPT3Amount = 0;
        adrToLoanStruct[_address].USDTBorrowed = 0;
        adrToLoanStruct[_address].OPT3PriceAtBorrowing = 0;
        adrToLoanStruct[_address].liquidationPrice = 0;
        adrToLoanStruct[_address].locked = false;

        IERC20(USDT).transferFrom(msg.sender, address(this), userNeedsToPay);

        IERC20(USDT).transfer(devWallet, devPay);
        IERC20(USDT).transfer(forLQWallet, LQPay);

        if(calculateLoanRebases(_address) > 1) {
            claimLoanRebases(_address);
        }

        IBEP20(OPT3).transfer(msg.sender, OPT3ToReturn);
    }

    function liquifyLoan(address _address) external {
        require(adrToLoanStruct[_address].taker == msg.sender);
