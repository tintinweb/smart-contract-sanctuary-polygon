...
/** 
 *  SourceUnit: /home/evanyx/Axoswap-MasterchefV2/contracts/MasterChef.sol
*/
...
contract MasterChef is Ownable {
    ...
    uint private constant ACC_AXO_PRECISION = 2e12;

    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event Harvest(address indexed user, uint indexed pid, uint amount);
    event LogPoolAddition(uint indexed pid, uint allocPoint, IERC20 indexed lpToken, bool update);
    event LogSetPool(uint indexed pid, uint allocPoint, bool overwrite, bool update);
 ...
 function checkForDuplicate(IERC20 _lpToken) internal view {
    ...
    function getFarmData(uint pid) external view returns (PoolInfo memory, uint) {
        return (poolInfo[pid], totalAllocPoint);
    }
   ...
   function add(uint64 allocPoint, IERC20 _lpToken, bool update) external onlyOwner {
        ...
        emit LogPoolAddition(poolInfoAmount - 1, allocPoint, _lpToken, update);
    }

    // Update the given pool's Axo allocation point. Can only be called by the owner.
    
function set(uint _pid, uint64 _allocPoint, bool overwrite, bool update) external onlyOwner {
        _set(_pid, _allocPoint, overwrite, update);
    }
function _set(uint _pid, uint64 _allocPoint, bool overwrite, bool update) internal validatePid(_pid) {
        if (update) {
            massUpdatePools();
        }
        ...
        emit LogSetPool(_pid, _allocPoint, overwrite, update);
    }
    function setBatch(uint[] memory _pid, uint64[] memory _allocPoint, bool[] memory overwrite, bool update) external onlyOwner {
        require(_pid.length == _allocPoint.length && _allocPoint.length == overwrite.length, "MCV2: all arrays need to be the same length");
        ...
        for(uint i = 0; i < len; i++)
            _set(_pid[i], _allocPoint[i], overwrite[i], false);
    }
