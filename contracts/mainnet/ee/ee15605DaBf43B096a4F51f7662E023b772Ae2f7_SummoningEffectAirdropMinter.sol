...
contract SummoningEffectAirdropMinter is Initializable, OwnableUpgradeable {
    IPolygear public polygear;

    function initialize(IPolygear _polygear) public initializer {
        __Ownable_init();
        polygear = _polygear;
    }
    ...
    function mint(
        ...
        // store the element type and summoning level in extraData
        uint16 extraData = SummoningEffectTraits.packSummoningEffect(
            elementType,
            summoningLevel
        );

        polygear.mint(to, 1, amount, PolygearLib.Origin.Giveaway, extraData);
    }
}