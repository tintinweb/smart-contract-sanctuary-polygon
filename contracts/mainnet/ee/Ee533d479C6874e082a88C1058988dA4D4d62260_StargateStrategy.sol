...
contract StargateStrategy is InitializableAbstractStrategy, UniswapV2Exchange, CurveExchange {
    ...
    function _stake(uint256 _amount) internal {
        ...
        console.log("STAKE: ", lpBal);
        lpToken.approve(address(chef), lpBal);
        chef.deposit(poolId, lpBal);
        console.log("LP TOKEN", lpToken.balanceOf(address(this)));
    ...
    function lpBalance() public view returns (uint256) {
        ...
        return amount + lpToken.balanceOf(address(this));
    }
    ...
    function _getAssetAmount(address _asset) internal view returns (uint256, uint256) {
        if (address(token0) == _asset) {
            return (token0.balanceOf(address(this)), lpBalance().scaleBy(Helpers.getDecimals(address(token0)), Helpers.getDecimals(address(lpToken)))); 
        }
    ...
    function _withdrawAsset(address _asset, uint256 _amountInUsd) internal returns (uint256) {
        ...
        require(IERC20(_asset).balanceOf(address(this)) >= _inTokenAmount, "Stargate - LOW_BAL_IN_TOKEN");
        IERC20(_asset).safeTransfer(vaultAddress, _inTokenAmount);
        return _inTokenAmount;
    }

    function _arrangeLP(uint256 _lp) internal {
        if (lpToken.balanceOf(address(this)) > _lp) {
            return;
        }
        chef.withdraw(poolId, _lp - lpToken.balanceOf(address(this))); // _amount, claim_rewards
    }

    // @dev STRATEGY DEPENDENT FUNCTION
    // @dev This function will withdraw the token0 amount from the strategy's pool
    function _directWithdraw(uint256 _amountOfToken0) internal {
        _amountOfToken0 = _amountOfToken0.scaleBy(Helpers.getDecimals(address(lpToken)), Helpers.getDecimals(address(token0))); // LP token of LP is 6 decimals
        _arrangeLP(_amountOfToken0);
        uint256 _amountSD = stargateRouter.instantRedeemLocal(uint16(routerPoolId), _amountOfToken0, address(this));
        require(_amountSD >= _amountOfToken0.subBasisPoints(30), "INCON_WITHD");

    }
    function _withdrawAll() internal {
        console.log("LP Balance: %s", lpBalance());
        console.log("LP: ", lpToken.balanceOf(address(this)));
        _arrangeLP(lpBalance());
        console.log("LP Balance: %s", lpBalance());
        console.log("LP: ", lpToken.balanceOf(address(this)));
        uint256 _amountSD = stargateRouter.instantRedeemLocal(uint16(routerPoolId), lpBalance(), address(this));
        require(_amountSD >= lpBalance().subBasisPoints(30), "INCON_WITHD_ALL");
    }
