...
contract EVILGHOST is ERC721  {
    ...
    constructor()
        ERC721("Evil Ghost", "EGST")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 888) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeicvajfki6lbwvtwfs6skgun4qhtkmdehe3o7zf76immdshf26ejqq/", Strings.toString(tokenId), ".json")
            );
    }
