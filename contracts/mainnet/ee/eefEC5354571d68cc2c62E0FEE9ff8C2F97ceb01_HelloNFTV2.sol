  ...
  UUPSUpgradeable
{
  ...
  function __HelloNFT_init(string memory baseURI) public onlyInitializing {
    __HelloNFT_init_unchained(baseURI);
  }

  function __HelloNFT_init_unchained(string memory baseURI) public onlyInitializing {
    __baseURI = baseURI;
  }
  ...
  function batchMint(address receiver, uint256 count) public onlyRole(MINTER_ROLE) {
  ...
  function batchTransfer(
    ...
    for (uint256 i = 0; i < count; i++) {
      tokenIds[i] = this.tokenOfOwnerByIndex(msg.sender, i);
    }
...
contract HelloNFTV2 is HelloNFT, OwnableUpgradeable
{
  function setOwner() onlyRole(DEFAULT_ADMIN_ROLE) external {
