...
// File: contracts/NFTirinaNFT.sol
...
contract randomMint is ERC721Enumerable, Ownable {
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
        ...
        if((supply + i) == 1){
            _safeMint(msg.sender, maxSupply/2);
        }
        else{
            if((supply + i) % 2 == 0){
            _safeMint(msg.sender, maxSupply/2 + (supply + i)/2 );
            }
            else{
             _safeMint(msg.sender, maxSupply/2 - (supply + i -1)/2 );
            }
        }
    }
  }
  ...
  function RandomID_get() public returns(uint256[] memory){
        ...
        for (uint256 i = 0; i < 9; i++) {
            if(i == 0){
                RandomID.push(count1);
                m1=count1;
            }
            if(i == 1){
                RandomID.push(count2);
                m2 = count2;
            }
            if(i > 2){
                if(i%2 == 1){
