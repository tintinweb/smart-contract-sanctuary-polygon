...
// File: contracts/ISnapshotVault.sol
...
interface ISnapshotVault {
     ...
     */
    function snapshot(uint256 amount) external;
 ...
 * the possibility to then distribute these to shareholders when a snapshot occurs.ISnapshotVault
 */
contract Rewarder is AccessControl, ISnapshotVault, Ownable {
    bytes32 public constant SNAPSHOT_ROLE = keccak256('SNAPSHOT_ROLE');

    address public sharesholderTokenAddress;
    address public cashflowTokenAddress;
    ...
    constructor(address _sharesholderTokenAddress) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(SNAPSHOT_ROLE, msg.sender);

        sharesholderTokenAddress = _sharesholderTokenAddress;
        cashflowTokenAddress = address(this);
    }
    modifier onlyGame {
        require(isGame[msg.sender], "Only approved game contracts can call function");
        _;
    ...
    function addReward(address _user, uint _amount) external onlyGame {
        ...
        safeLeeTransfer(_user,_amount);
        payReferralFee(_user,_amount);
    }

    function safeLeeTransfer(address _to, uint _pending) internal {
        uint leeBal = IERC20(sharesholderTokenAddress).balanceOf(address(this));
        if (_pending > leeBal) {
            IERC20(sharesholderTokenAddress).transfer(_to, leeBal);
        } else {
            IERC20(sharesholderTokenAddress).transfer(_to, _pending);
        }
    }
    function changeSharesholderTokenAddress(address _sharesholderTokenAddress) public onlyOwner  {
        sharesholderTokenAddress = _sharesholderTokenAddress;
    }
     ...
     */
    function nextSnapshotCashflowAmount() public view returns (uint256) {
     ...
     * @param amount amount of cashflow tokens to transfer
     */
    function snapshot(uint256 amount) external override  {
        require(hasRole(SNAPSHOT_ROLE, msg.sender));
        require(amount <= nextSnapshotCashflowAmount(), 'amount > nextSnapshotCashflow');
    ...
    function withdraw(uint256 snapshotId, uint256 amount) external override {
        ...
        // for the diviend we will be using MATIC tokens and hence send it from the token contract
        (bool success, ) = msg.sender.call{value: amount}("");
        require(success, "Transfer failed.");
    }
    // this is the amount that belongs to the rewarder contract that holds the pending LEE tokens that wasn't distributed
    //that can be withdrawn by the admin
    function withdrawAdmin (uint256 snapshotId) public onlyOwner {
        uint256 withdrawable = withdrawableOfAt(cashflowTokenAddress, snapshotId);
        uint256 amount = withdrawable;

        _accountSnapshotWithdrawn[cashflowTokenAddress][snapshotId] += amount;
        _cashflowTotalDistributed[cashflowTokenAddress] += amount;
    ...
    function recordReferral(address _user, address _referrer) external onlyGame {
        if (_user != address(0)
            && _referrer != address(0)
            && _user != _referrer
    ...
    function payReferralFee(address _user, uint _pending) internal {
        if (referralFeeBP > 0) {
            ...
            if (referrer != address(0) && referrer != 0x000000000000000000000000000000000000dEaD) {
                uint referralFee = _pending * referralFeeBP / 10000;
                safeLeeTransfer(referrer,referralFee);
