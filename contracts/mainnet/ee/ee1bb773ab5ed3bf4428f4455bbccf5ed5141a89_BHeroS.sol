...
contract BHeroS is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
  ...
  function getTokenDetailByID(uint256 id) public view returns (uint256) {
...
//   }
  ...
  /** Claims tokens. */
  function claim(uint256 count, address _to) external onlyRole(CLAIMER_ROLE) {
    uint256 _rarity = 0;
    claimableTokens[_to][_rarity] += count;
    ...
    mapping(uint256 => uint256) storage tokens = claimableTokens[_to];
    for (uint256 rarity = 0; rarity <= 6; ++rarity) {
      ...
      requestCreateToken(_to, mintCount, rarity, 0, isHeroS, dropRateOption);
