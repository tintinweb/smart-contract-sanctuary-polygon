...
contract  TheMaticFarming{
	...
	uint256[] public PACKAGES=[1 ether,5 ether,100 ether,500 ether,1000 ether];
	uint256[] public PACKAGESMatch=[4 ether,99 ether,499 ether,999 ether,10000000000000 ether];
	...
	function invest(address referrer,uint256 Package_Index) public payable {
		...
		require(msg.value <= Package_Amount_Max,'Maximum Investment Condition');
   ...
   function top5refclsoing(uint _flag) private {	   		 
	...
	function updateMaxPackage(uint256 _newValue,uint256 _updateIndex) onlyOwner public returns(bool) {
        PACKAGESMatch[_updateIndex-1]=_newValue;
        return true;
