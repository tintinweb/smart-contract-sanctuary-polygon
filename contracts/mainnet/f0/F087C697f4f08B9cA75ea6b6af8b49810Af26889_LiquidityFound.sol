...
contract LiquidityFound is ILiquidityFound, AccessControl {
    ...
    function batchMint(int256 startTick, int256 tickSapce, uint256 num, uint256 amount0Desired, uint256 amount1Desired) external onlyRole(MANAGE_ROLE) {
        if (tickSapce < 0) {
            startTick += tickSapce * int256(num);
            tickSapce = -tickSapce;
        }
    ...
    function _collect(CollectParams calldata params)
        private
        returns (uint256 amount0, uint256 amount1)
    {
    ...
    function collect0Mint1(CollectParams calldata params) external
        onlyRole(MANAGE_ROLE)
        returns (
            uint256 tokenId,
            uint128 liquidity,
    ...
    {
        (, uint256 amount1Get) = _collect(params);
        if (amount1 > 0) {
            (, int24 tick0, , , , , ) = v3Pool.slot0();
            if (tick0 > initTick) {
                return _mintLeftside(initTick, amount1Get);
            } else {
                return _mintRightside(initTick, amount1Get);
            }
        }
    }

    function collect1Mint0(CollectParams calldata params) external
        onlyRole(MANAGE_ROLE)
        returns (
            uint256 tokenId,
            uint128 liquidity,
    ...
    {
        (uint256 amount0Get, ) = _collect(params);
        if (amount0 > 0) {
            (, int24 tick0, , , , , ) = v3Pool.slot0();
            if (tick0 > initTick) {
                return _mintLeftside(initTick, amount0Get);
            } else {
                return _mintRightside(initTick, amount0Get);
            }
        }
    }
