...
contract OddzPrediction is IOddzPrediction, AccessControl, Pausable, ReentrancyGuard {
    ...
    uint256 public roundTime = 3600; //10 mintues of round
    ...
    /**
     * @notice Set the oddz administrator
     * @param _oddzAdministrator Address of the oddz admin contract
     */
    function setOddzAdministrator(IOddzAdministrator _oddzAdministrator) external onlyAdmin(msg.sender) {
        require(address(_oddzAdministrator).isContract(), "Not Oddz Administrator");
        oddzAdmin = _oddzAdministrator;
    }
    ...
    function startPredictionMarket() external override whenNotPaused onlyOperator(msg.sender) {
        ...
        (uint256 price, ) = oddzPriceManager.getPrice(assetPair.underlying, assetPair.strike, roundzeroStartTimestamp);
    ...
    function claim(
        ...
        if (convert && reward > 0) {
            OddzSwap.SwapStruct memory swapStruct = OddzSwap.SwapStruct({
                deadline: block.timestamp + (roundTime * 2),
                fromAsset: bytes32("MATIC"),
                toAsset: toAsset,
                amountIn: 0
            });
            oddzSwap.swapExactETHForTokens{ value: reward }(swapStruct, msg.sender, slippage);
        } else if (reward > 0) {
    ...
    function getCurrentRoundDetails() external view returns (IOddzPrediction.Round memory round) {
        round = oddzPredictionStorage.getPredictionRound(currentRoundId);
    }
}
