...
import "./TuningEngine.sol";
import { Color,Seed } from "./render/SetRenderable.sol";
...
interface Renderable {
    function svg(Seed memory flower) external view returns (bytes memory);
}

contract Harmonize is TuningEngine {    
    // using SetBars for SvgData;
    using Strings for uint256;
    ...
    mapping(uint256 => Seed) garden;

    mapping(bytes => bool) sown;

    Rendering[] renderers;

    mapping(uint256 => Rendering) assignments;

    mapping(uint256 => Rendering[]) collected;

    mapping(uint256 => mapping(uint256 => uint256)) collectedIndicies;

    uint256 constant BASE_RENDER_KEY = 0;
    string constant BASE_RENDER_NAME = "bars";
    
    constructor(string memory name, string memory symbol, address _renderer) TuningEngine(name,symbol) {
        renderers.push(Rendering(BASE_RENDER_KEY,BASE_RENDER_NAME,_renderer,true));        
    }


    function assignRenderer(string memory _name, address _renderer) external onlyOwner {
        renderers.push(Rendering(renderers.length,_name,_renderer,true));
    }      

    function assignConductor(address pact) external onlyOwner {
        CONDUCTOR = payable(pact);
    }  

    function peekSeed(uint256 tokenId) public view returns (Seed memory) {
        return pickFlower(tokenId);
    }

    function pickFlower(uint256 tokenId) private view returns (Seed memory) {
        return garden[tokenId];
    }

    function randomSeed() public returns (uint32 hue, uint32 pick, uint32 sat, uint32 light, uint32 negated, uint32 amplified, uint32 muted, uint32 divergent) {
        randNonce++; 
        uint256 rando = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender,randNonce)));
        // tier 1
        hue = uint32(rando % 360);
        
        pick = uint32(rando % 5);

        sat = uint32(rando % 60)+40;

        light = uint32(rando % 50)+30;

        // tier 2
        // harmonic divisors 1, 6, 28, 140, 270, 496, 672, 1638, 2970, 6200, 8128, 8190
        negated = uint32(rando % 28) < 1 ? 1: 0; // uncommon modifier 1/28 chance; colors have decreased light levels

        muted = uint32(rando % 140) < 1 ? 1: 0; // rare modifier 1/140 chance; colors have decreased saturation levels

        amplified = uint32(rando % 270) < 1 ? 1: 0; // epic modifier 1/270 chance; animation is twice as fast

        divergent = uint32(rando % 496) < 1 ? 1: 0; // legendary modifier 1/496 chance; bars are vertical 
    }

    function harvestSeed() public returns (Seed memory seed) {
        (uint32 hue, uint32 pick, uint32 sat, uint32 light, uint32 negated, uint32 amplified, uint32 muted, uint32 divergent) = randomSeed();
        bytes memory packedSeed = abi.encode(Seed(hue,sat,light,pick,negated,amplified,muted,divergent));
        
        while (sown[packedSeed]) {
            (hue, pick, sat, light, negated, amplified, muted, divergent) = randomSeed();
            packedSeed = abi.encode(Seed(hue,sat,light,pick,negated,amplified,muted,divergent));
        }
        sown[packedSeed] = true;
        seed = Seed(hue,sat,light,pick,negated,amplified,muted,divergent);
    }

    function privateMint(uint64 quantity) external payable {
        Phase memory phased = findPhase(PRIVATE);

        payAdmission(phased.cost, quantity);

        uint256 mintedPrior = minted();      

        uint256 tokenId = phasedMint(PRIVATE, quantity, false);

        for (uint256 i = mintedPrior+1; i <= tokenId; i++) {
            (Seed memory plant) = harvestSeed();
            garden[i] = plant;
        }
        
        tallyMint(PRIVATE,quantity);
    }  

    function allowMint(uint256 quantity) external payable  {
        Phase memory phased = findPhase(ALLOWED);

        uint256 mintedPrior = minted(); 
        
        payAdmission(phased.cost, quantity);
        
        uint256 tokenId = phasedMint(ALLOWED, quantity, false);
    ...
    function openMint(uint256 quantity) external payable {
        Phase memory phased = findPhase(OPEN);

        uint256 mintedPrior = minted(); 
        
        payAdmission(phased.cost, quantity);
        
        uint256 tokenId = phasedMint(OPEN, quantity, false);
    ...
    function addOnPhase(uint256 quantity, uint64 phase) external payable {
        if (phase <= OPEN) {
            revert Unphased();
        }
    ...
    error AlreadyCollected(uint256 renderKey);
    uint256 rendererFee = .01 ether;
    function harmonize(uint256 tokenId, uint256 renderKey) external payable {
        if (renderKey > renderers.length-1) revert  InvalidRenderer();
        
        validateApprovedOrOwner(msg.sender, tokenId);  
        isValidPayment(rendererFee,1);

        if (collected[tokenId].length == 0) {
            collected[tokenId].push(renderers[BASE_RENDER_KEY]);
        }
        if (collectedIndicies[tokenId][renderKey] > 0) {
            revert AlreadyCollected(renderKey);
        }
        
        collectedIndicies[tokenId][renderKey] = collected[tokenId].length;

        collected[tokenId].push(renderers[renderKey]);
    ...
    function payAdmission(uint256 cost, uint256 quantity) internal {   
        isValidPayment(cost,quantity);   
        CONDUCTOR.transfer(cost*quantity);
    }      
    
    
    function encodedTokenImage(uint256 tokenId, Seed memory flower) public view returns (bytes memory) {  
        bytes memory svg;
        if (assignments[tokenId].exists) {
            svg = Renderable(assignments[tokenId].renderer).svg(Seed(flower.hue,flower.sat,flower.light,flower.pick,0,0,0,0));
        } else {
            svg = Renderable(renderers[BASE_RENDER_KEY].renderer).svg(Seed(flower.hue,flower.sat,flower.light,flower.pick,0,0,0,0));
        }
        return abi.encodePacked("data:image/svg+xml;base64,",Base64.encode(svg));        
    }

    function encodedContractImage(bytes memory svg) public pure returns (bytes memory) {  
        return abi.encodePacked("data:image/svg+xml;base64,",Base64.encode(svg));        
    }    


    function packTraits(uint256 tokenId, Seed memory flower, string memory harmonic, uint256 bars) public view returns (bytes memory) {
        bytes memory traits = abi.encodePacked('{',
                '"trait_type": "hue",', 
                '"value":',  flower.hue.toString(),
            '},',
            '{',
                '"trait_type": "saturation",', 
                '"value":',  flower.sat.toString(),
            '},',
            '{',
                '"trait_type": "light",', 
                '"value":',  flower.light.toString(),
            '},',
            '{',
                '"trait_type": "harmonic",', 
                '"value":"',  harmonic,
            ...
            '{',
                ...
                '"value":',  bars.toString(),
            '}');
        bytes memory rareTraits;

        if (flower.negated > 0) {
            rareTraits = abi.encodePacked(string(rareTraits),',{',
                '"value": "negated"'
            '}');
        }
        if (flower.amplified > 0) {
            rareTraits = abi.encodePacked(string(rareTraits),',{',
                '"value": "amplified",'
            '}');
        }     
        ...
        if (flower.divergent > 0) {
            rareTraits = abi.encodePacked(string(rareTraits),',{',
                '"value": "divergent",'
            '}');
        }
        for (uint i = 0; i < collected[tokenId].length; i++) {
            rareTraits = abi.encodePacked(string(rareTraits),',{',
                '"trait_type": "renderers",', 
                '"value": ',collected[tokenId][i].name,','
            '}');            
        }
        return abi.encodePacked(string(traits),string(rareTraits));
    }
    function encodedTokenAttributes(uint256 tokenId, Seed memory flower) public view returns (bytes memory) {        
        string memory harmonic = flower.pick > 0 ? "complement" : flower.pick == 2 ? "split" : flower.pick == 3 ? "triadic" : flower.pick == 4 ? "tetradic" : flower.pick == 4 ? "analogous": "monochrome";
        uint256 bars = flower.pick > 0 ? 2 : flower.pick == 2 ? 4 : flower.pick == 3 ? 4 : 5;  
        return abi.encodePacked(
            '[',string(packTraits(tokenId,flower,harmonic,bars)),']'
        );
    }
    function encodedTokenURI(uint256 tokenId) public view returns (string memory) {        
        Seed memory flower = pickFlower(tokenId);
        bytes memory dataURI = abi.encodePacked(
            '{',
                '"name": "Harmonix #', tokenId.toString(),
                '","description": "Chromic blocks for the vibes"', 
                ',"image":"', encodedTokenImage(tokenId, flower),'"',
                ',"attributes":', encodedTokenAttributes(tokenId, flower),
            '}'
        );
    ...
    uint256 royalty_basis = 1000;
    /**
     * @notice ERC721 contractURI 
     */
    function contractURI() public view virtual override returns (string memory) { 
        // Renderable(renderers[BASE_RENDER_KEY].renderer).svg(Seed(226,90,52,4,0,0,0,0))        
        bytes memory dataURI = abi.encodePacked(
            '{',
                '"name": "This is only a test"',
                ',"description": "Something goes here"', 
                ',"image":"',encodedContractImage(Renderable(renderers[BASE_RENDER_KEY].renderer).svg(Seed(226,90,52,4,0,0,0,0))),'"',
                // ',"external_link":"https://harmonix.is"',
                ',"seller_fee_basis_points":', royalty_basis.toString(), 
                ',"fee_recipient":"', Strings.toHexString(uint160(defaultPayable), 20),'"'
            '}'
        );
...
uint16 constant COMPLEMENT=1;
uint16 constant SPLIT=2;
uint16 constant TRIAD=3;
uint16 constant TETRAD=4;
uint16 constant ANALOG=5; 
uint16 constant MONO=6; 
library SetRenderable {  
    using Strings for uint256;
    using Strings for uint64;
    using Strings for uint32;
    using Strings for uint16;
    ...
    string constant baseGradient = 'x1="0" y1="0" x2="100%" y2="0"';
    string constant aniBase = ' repeatCount="indefinite" fill="freeze" values="';
    
    function findColors3(Renderable storage self, Seed memory flower) public pure returns (Color[3] memory) {
        return flower.pick == SPLIT ? split(self,Color(flower.hue, flower.muted > 0 ? 
            flower.sat-30:
            flower.sat, flower.light, 2)) : triad(self,Color(flower.hue, 
            flower.muted > 0 ? 
                flower.sat-30:
                flower.sat,
            flower.negated > 0 ? 
    ...
    function findColors4(Renderable storage self, Seed memory flower) public pure returns (Color[4] memory) {
        return flower.pick == TETRAD ? 
            tetrad(self,Color(flower.hue, 
                flower.muted > 0 ? 
                    flower.sat-30:
                    flower.sat,
                flower.negated > 0 ? 
                    flower.light-15:
                    flower.light, 4)) : 
                analogous(self,Color(flower.hue, 
                flower.muted > 0 ? 
                    flower.sat-30:
                    flower.sat,
                flower.negated > 0 ? 
    ...
    function harmonizeOne(Renderable storage, Color memory flower) public pure returns(Color[4] memory) {        
        Color[4] memory colors;
        colors[0] = Color(flower.hue,flower.sat,flower.light,flower.pick);
        uint256 index = 1;
        for(uint64 i = 0; i < 40; i += 10) {
            uint64 nextLight = flower.light - i;
            colors[index] = Color(flower.hue,flower.sat,nextLight,flower.pick);
            index++;
        }
    ...
    function harmonizeTwo(Renderable storage, Color memory flower, uint64 start, uint64 end, uint64 interval) public pure returns(Color[2] memory) {        
        Color[2] memory colors;
        colors[0] = flower;
        uint256 index = 1;
        for(uint64 i = start; i <= end; i += interval) {
            uint64 nexthue = (flower.hue + i) % 360;
            colors[index] = Color(nexthue,flower.sat,flower.light,flower.pick);
            index++;
        }
    ...
    function harmonizeThree(Renderable storage, Color memory flower, uint64 start, uint64 end, uint64 interval) public pure returns(Color[3] memory) {        
    ...
    function harmonizeFour(Renderable storage, Color memory flower, uint64 start, uint64 end, uint64 interval) public pure returns(Color[4] memory) {        
        Color[4] memory colors;
        colors[0] = Color(flower.hue,flower.sat,flower.light,flower.pick);
        uint256 index = 1;
        for(uint64 i = start; i <= end; i += interval) {
    ...
    function monochrome(Renderable storage self, Color memory flower) public pure returns(Color[4] memory) {
        return harmonizeOne(self,flower);
    }

    function complement(Renderable storage self, Color memory flower) public pure returns(Color[2] memory) {
        return harmonizeTwo(self, flower, 180, 180, 1);
    }

    function split(Renderable storage self, Color memory flower) public pure returns(Color[3] memory) {
        return harmonizeThree(self,flower, 150, 210, 60);
    }

    function triad(Renderable storage self, Color memory flower) public pure returns(Color[3] memory) {
        return harmonizeThree(self,flower, 120, 240, 120);
    }

    function tetrad(Renderable storage self, Color memory flower) public pure returns(Color[4] memory) {
        return harmonizeFour(self,flower, 90, 270, 90);
    }

    function analogous(Renderable storage self, Color memory flower) public pure returns(Color[4] memory) {
        return harmonizeFour(self,flower, 30, 90, 30);
    }    
...
contract TuningEngine is EIP712Allowlisting {  
    address constant defaultPayable = 0x5aE09f46967A92f3cF976e98f82B6FDd00784815;
    address payable internal CONDUCTOR = payable(defaultPayable);
    uint64 PRIVATE = 0;
    uint64 ALLOWED = 1;
    uint64 OPEN = 2; 
    mapping(uint256 => mapping(address => uint256)) phasedMints;
    constructor(string memory name, string memory symbol) FlexibleMetadata(name,symbol) {
        ...
        phases.push(Phase(PRIVATE,1,140,(1 ether)*.01736)); // 2.4304
        phases.push(Phase(ALLOWED,6,672,(1 ether)*.024206)); // 12.877592
        phases.push(Phase(OPEN,28,1638,(1 ether)*.033264)); // 32.133024
        initialize(phases,1638);
    }
    ...
    function tallyMint(uint256 phase, uint256 quantity) internal {
    ...
    function canMint(uint64 phase, uint256 quantity) internal override virtual returns(bool) {
        ...
        uint256 requestedMints = quantity+phasedMints[phase][msg.sender];
