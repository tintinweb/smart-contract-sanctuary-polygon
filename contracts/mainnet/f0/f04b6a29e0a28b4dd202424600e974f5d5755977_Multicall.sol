...
contract Multicall {
    ...
    function uniV3Immutables(address pool)public returns (Immutables memory){
        uint gasLeft=gasleft();
        am=IERC20(IPool(pool).token0()).balanceOf(pool)/2;
        address(this).call(abi.encodeWithSignature("uniV3(address,bool)",pool,true));
        return Immutables(IPool(pool).token0(),IPool(pool).token1(),IUniV3Pool(pool).fee(),factory(pool),bytes4(keccak256("uniV3(address,bool)")),gasLeft-gasleft());
    }

    function kyberV3Immutables(address pool)public returns (Immutables memory){
        uint gasLeft=gasleft();
        am=IERC20(IPool(pool).token0()).balanceOf(pool)/2;
        address(this).call(abi.encodeWithSignature("kyberV3(address,bool)",pool,true));
        return Immutables(IPool(pool).token0(),IPool(pool).token1(),IKyberV3Pool(pool).swapFeeUnits() * 10,factory(pool),bytes4(keccak256("kyberV3(address,bool)")),gasLeft-gasleft());
    }

    function uniV2Immutables(address pool)public returns(Immutables memory){
        uint gasLeft=gasleft();
        am=IERC20(IPool(pool).token0()).balanceOf(pool)/2;
        address(this).call(abi.encodeWithSignature("uniV2(address,bool)",pool,true));
        return Immutables(IPool(pool).token0(),IPool(pool).token1(),3000,factory(pool),bytes4(keccak256("uniV2(address,bool)")),gasLeft-gasleft());
    }

    function algebraV3Immutables(address pool)public returns (Immutables memory){
        uint gasLeft=gasleft();
        am=IERC20(IPool(pool).token0()).balanceOf(pool)/2;
        address(this).call(abi.encodeWithSignature("algebraV3(address,bool)",pool,true));
        (,,uint16 fee,,,,)=IAlgebraV3Pool(pool).globalState();
        return Immutables(IPool(pool).token0(),IPool(pool).token1(),fee,factory(pool),bytes4(keccak256("algebraV3(address,bool)")),gasLeft-gasleft());
    }
