...
interface IIGO {
    ...
    function tokenToCard(uint tokenID) external view returns (uint);

    function balanceOf(address addr) external view returns (uint);

    function batchTokenURI(address account_) external view returns (uint[] memory, string[]memory);
}
...
contract ExchangeCard is OwnableUpgradeable {
    IHalo public halo;
    ...
    function initAddress(address halo_, address item_, address cattle_, address planet_, address box_, address voucher_, address igo_, address cattleVoucher_) external onlyOwner {
        halo = IHalo(halo_);
        item = ICattle1155(item_);
        cattle = ICOW(cattle_);
        planet = IPlanet721(planet_);
        box = IBOX(box_);
        voucher = CattleVoucher1155(voucher_);
        igo = IIGO(igo_);
        cattleVoucher = IHalo(cattleVoucher_);
    }

    function setGala(address gala_) external onlyOwner{
        gala = IHalo(gala_);
    }
    ...
    function exchangeHalo(uint256 tokenId) internal {
        halo.burn(tokenId);

        uint cardId = halo.cardIdMap(tokenId);
        require(cardId != 1, 'wrong halo');
        if (cardId == 2) {
    ...
    function exchangeHaloBatch(uint[] memory tokenId) external onlyEOA {
        bvt.transferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, tokenId.length * price);
        for (uint i = 0; i < tokenId.length; i++) {
            exchangeHalo(tokenId[i]);
        }
    }

    function exchangeVoucher(uint tokenId, uint amount) internal {
        voucher.burn(msg.sender, tokenId, amount);
        item.mint(msg.sender, tokenId, amount);
    }

    function exchangeVoucherBatch(uint[] memory tokenId, uint[] memory amount) external onlyEOA {
        require(tokenId.length == amount.length, 'wrong length');
        for (uint i = 0; i < tokenId.length; i++) {
            if (amount[i] == 0) {
                continue;
            }
            exchangeVoucher(tokenId[i], amount[i]);
        }
    }

    function exchangeCattle(uint tokenId) internal onlyEOA {
        cattleVoucher.burn(tokenId);
        uint id = cattle.currentId();
        cattle.mintNormalAdultWithoutParents(msg.sender);
        checkCattleLife(id);
    }

    function exchangeCattleBatch(uint[] memory tokenId) external onlyEOA {
        bvt.transferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, tokenId.length * price);
        for (uint i = 0; i < tokenId.length; i++) {
            exchangeCattle(tokenId[i]);
        }
    }

    function checkIGOToken(address addr, uint cardId) public view returns (uint[] memory){
        uint amount;
        uint token;
        (uint[] memory tokenList,) = igo.batchTokenURI(addr);
        for (uint i = 0; i < tokenList.length; i++) {
            token = tokenList[i];
            if (igo.tokenToCard(token) == cardId) {
                amount ++;
            }
        }
    ...
    function checkGalaxiesToken(address addr) public view returns(uint[] memory){
        uint amount;
        uint token;
        uint tempBalance = gala.balanceOf(addr);
        for (uint i = 0; i < tempBalance; i++) {
            token = gala.tokenOfOwnerByIndex(addr,i);
            if (gala.cid(token) == 7062) {
                amount ++;
            }
        }
        uint[] memory list = new uint[](amount);
    ...
    function exchangeIGO(uint tokenId) internal {
        igo.burn(tokenId);
        uint cardId = igo.tokenToCard(tokenId);
        require(cardId != 0, 'wrong igo');
        if (cardId == 10001) {
            item.mint(msg.sender, 20003, 2);
        } else if (cardId == 10002) {
            item.mintBatch(msg.sender, itemPackId, itemPack);
        } else if (cardId == 10003) {
            uint id = cattle.currentId();
            cattle.mintNormallWithParents(msg.sender);
            checkCattleLife(id);
        } else if (cardId == 10004) {
            cattle.mint(msg.sender);
        }
    }
    ...
    function exchangeGalaxies(uint tokenId) internal {
        require(gala.cid(tokenId) == 7062,'wrong tokenId');
        gala.safeTransferFrom(msg.sender,address(0x000000000000000000000000000000000000dEaD),tokenId);
        uint id = cattle.currentId();
        cattle.mintNormallWithParents(msg.sender);
        checkCattleLife(id);
    }

    function exchangeGalaxiesBatch(uint[] memory tokenId) external onlyEOA {
        bvt.transferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, tokenId.length * price);
        for (uint i = 0; i < tokenId.length; i++) {
            exchangeGalaxies(tokenId[i]);
        }
    }

    function exchangeIgoBatch(uint[] memory tokenId) external onlyEOA {
        bvt.transferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, tokenId.length * price);
        for (uint i = 0; i < tokenId.length; i++) {
            exchangeIGO(tokenId[i]);
        }
    }
