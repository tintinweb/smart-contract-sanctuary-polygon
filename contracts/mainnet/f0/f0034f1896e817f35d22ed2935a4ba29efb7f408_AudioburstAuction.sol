...
contract AudioburstAuction is Ownable, ERC721Holder {
    ...
    function createAuction(
        ...
        uint256 _startTime, 
        uint256 _endTime
    ) onlyTokenOwner(_collection, _token_id) public {
        ...
        newAuction.auction_id = auctionId;
        ...
        newAuction.creator = nft.creatorOf(_token_id);
        newAuction.royalty = nft.royalties(_token_id);
     ...
     * @param _auction_id uint256 ID of the created auction
     */
    function finalizeAuction(uint256 _auction_id) public {
        ...
        if (bidsLength == 0) {
            IAudioburstNFT(myAuction.collection).safeTransferFrom(address(this), myAuction.owner, myAuction.token_id);
            ...
            uint256 creatorAmount = lastBid.bidPrice.mul(myAuction.royalty).div(PERCENTS_DIVIDER);
            uint256 sellerAmount = lastBid.bidPrice.sub(feeAmount).sub(creatorAmount);
            payable(swapFeeAddress).transfer(feeAmount);
            payable(myAuction.creator).transfer(creatorAmount);
            payable(myAuction.owner).transfer(sellerAmount);
            // approve and transfer from this contract to the bid winner 
            IAudioburstNFT(myAuction.collection).safeTransferFrom(address(this), lastBid.from, myAuction.token_id);
            auctions[_auction_id].active = false;
     ...
     * @param _auction_id uint256 ID of the created auction
     */
    function bidOnAuction(uint256 _auction_id) payable external AuctionExists(_auction_id) {
        // owner can't bid on their auctions
        Auction memory myAuction = auctions[_auction_id];
        require(myAuction.owner != msg.sender, "owner can not bid");
        require(myAuction.active, "not exist");
        // if auction is expired or not started
        ...
        require(msg.value >= tempAmount, "bid amount is not valid");
    ...
    modifier AuctionExists(uint256 _auction_id){
        require(_auction_id <= auctions.length && auctions[_auction_id].auction_id == _auction_id, "Could not find item");
        _;
    }
     ...
     * @param _auction_id uint256 ID of the auction
     */
    ...
    /**
     * @dev Gets an array of owned auctions
     * @param _auction_id uint256 of the auction owner
    ...
    modifier onlyAuctionOwner(uint256 _auction_id) {
        require(auctions[_auction_id].owner == msg.sender);
        _;
    }

    modifier onlyTokenOwner(address _collection, uint256 _token_id) {
        address tokenOwner = IERC721(_collection).ownerOf(_token_id);
