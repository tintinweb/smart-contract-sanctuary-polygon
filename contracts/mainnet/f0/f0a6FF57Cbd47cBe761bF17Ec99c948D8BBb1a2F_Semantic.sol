...
contract Semantic is ISemantic, AccessControl, Semantic721 {
     ...
     * @param  operateDatas 操作数据
     */
    function testSign(ClaimOperateData[] memory operateDatas) external view returns(string memory signOriginalData,address singer) {
        for (uint256 i = 0; i < operateDatas.length; i++) {
            ClaimOperateData memory operateData = operateDatas[i];
            ...
            uint256 subjectId = operateData.subjectId;
            signOriginalData = string.concat(operateData.expireTime.toString(),subjectId.toString());
            for (
                uint256 j = 0;
                j < operateData.predicateAndObjects.length;
                j++
