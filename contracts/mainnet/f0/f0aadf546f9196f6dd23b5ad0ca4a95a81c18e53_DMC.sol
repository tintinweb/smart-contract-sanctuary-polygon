...
contract DMC is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(
    ...
    address _initdmAddress
  ) ERC721(_name, _symbol) {
    setBaseURI(_initBaseURI);
    setNotRevealedURI(_initNotRevealedUri);
    setdmAddress(_initdmAddress);
  }
  ...
  function claim() public {
    ...
    IERC721 token = IERC721(dmAddress);
    uint256 ownedAmount = token.balanceOf(msg.sender);
    require(ownedAmount >= requiredAmount, "You don't own enough DM NFTs");
  ...
  //only owner
  function setdmAddress(address _newAddress) public onlyOwner {
    dmAddress = _newAddress;
  }
