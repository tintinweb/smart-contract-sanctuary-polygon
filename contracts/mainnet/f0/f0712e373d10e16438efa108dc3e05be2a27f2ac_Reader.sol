...
contract Reader is WarpBase {
    ...
    /** @notice set warp utils */
    function setWarpUtils(address _warpUtils) external onlyOwner {
        warpUtils = IWarpUtilities(_warpUtils);
    }

    /** @notice set warp utils */
    function setBattlezone(address _battlezone) external onlyOwner {
        bzone = IBattlezone(_battlezone);
    }
    ...
    function getStarships(address owner) public view returns (SS[] memory) {
        ...
        for (uint256 i = 0; i < balance; i++) {
            ...
            ships[i] = SS({
                ...
                battleInfo: bzone.getBattleship(shipId)
            });
        }
