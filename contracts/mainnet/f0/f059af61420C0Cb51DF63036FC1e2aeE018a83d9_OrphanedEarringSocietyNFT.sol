...
// File: OrphanedEarringSocietyNFT.sol

/**
 * @title Orphaned Earring Society
 * 
 * Almost everyone has a story to tell about a lost earring, and Michelle
 * Phillips created the Orphaned Earring Society to tell her story and
 * encourage others to share theirs. Join Michelle and friends to help
 * raise money to support Ukrainian refugees.
 *
 * Starting with Michelle’s favorite orphaned earring story, the community
 * will gather submissions, and share them to the private community. When
 * the voting begins, the community votes on the best story. The winner
 * gets to meet Michelle and receive the earring she still has from the
 * orphaned set!
 *
 * After the community selects a winner, another celebrity will share their
 * story. Once the first winner is selected, everyone’s story will become
 * an NFT to live forever on the blockchain.
 *
 * Orphaned Earring Society is a unique opportunity to tell a good story
 * and perhaps even meet one of your favorite celebrities and receive a
 * truly unique gift. Maybe you’ll even be able to find its match :)
 */
contract OrphanedEarringSocietyNFT is ReliefDAONFT {
    string public namePrefix = "Orphaned Earring ";

    constructor() ViciERC721v4("Orphaned Earring Society", "OES", 1000000) {
        setContractURI(
            "https://storage.googleapis.com/arrival-shadow-motion-newman/vici/lost-earring/OrphanedEarringSociety.json"
        );
    }

    function setNamePrefix(string memory _namePrefix)
        public
        onlyOwnerOrRole(CREATOR_ROLE_NAME)
    {
    ...
    function getNFTName(uint256 tokenId)
        ...
        returns (string memory)
    {
        return string.concat(namePrefix, super.getNFTName(tokenId));
    }
}