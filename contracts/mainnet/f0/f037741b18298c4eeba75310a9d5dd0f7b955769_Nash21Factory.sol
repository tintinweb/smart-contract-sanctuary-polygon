...
import {UUPSUpgradeableByRole} from "./utils/UUPSUpgradeableByRole.sol";
import {INash21Manager} from "./interfaces/INash21Manager.sol";
import {INash21Guarantee} from "./interfaces/INash21Guarantee.sol";
import {INash21Factory} from "./interfaces/INash21Factory.sol";

/// @title The contract of Nash21Factory
/// @notice Handles non-fungible tokens balances and actions
/// @dev Any data regarding non-fungible tokens is placed here
contract Nash21Factory is
    INash21Factory,
    ...
    EIP712Upgradeable
{
    ...
    bytes32 internal constant _N21_FACTORY_TYPEHASH = keccak256(
        "Create(bytes32 hashId,address account,uint256 value,bytes32 currency,uint256 startDate,uint256 endDate,bytes32 uri,uint256 deadline)"
    );

    bytes32 internal constant _AUTH_ROLE = keccak256("AUTH_ROLE");
    bytes32 internal constant _GUARANTEE = keccak256("GUARANTEE");
    bytes32 internal constant _FACTORY_ADMIN_ROLE = keccak256("FACTORY_ADMIN_ROLE");
    ...
    /// @inheritdoc INash21Factory
    mapping(bytes32 => bool) public unauthorized;

    /// @inheritdoc INash21Factory
    mapping(bytes32 => uint256) public tokenCreationFee;

    /// @inheritdoc INash21Factory
    mapping(uint256 => uint256) public tokenTransferFee;

    /// @inheritdoc INash21Factory
    mapping(bytes32 => bool) public authorizedHash;

    /// @inheritdoc INash21Factory
    mapping(address => uint256) public ownerTransferFee;
    ...
    function initialize() public initializer {
        __ERC721_init("N21", "Nash21 ERC721");
        __ERC721URIStorage_init();
        __ERC721Enumerable_init();
        __EIP712_init("N21_FACTORY", "1");
        __AccessControlProxyPausable_init(msg.sender);

        _setBaseURI("https://sandbox-nash.jointheredpill.com/nft/");
        _setCreationFee(1 ether);
        _setTransferFee(1 ether);
        _id = 1;
    }

    /// @inheritdoc INash21Factory
    function reinitialize() public reinitializer(uint8(2)) {
        __ERC721_init("Nash21 ERC721", "N21");
    }
    ...
    function authAndCreate(
        ...
        uint256 endDate,
        string memory uri
    ) external payable onlyRole(_AUTH_ROLE) whenNotPaused returns (uint256) {
        uint256 id = _authorize(hashId, account, value, currency, startDate, endDate, uri);
        _create(id);
        return id;
    }
    ...
    function authorize(
        ...
        return _authorize(hashId, account, value, currency, startDate, endDate, uri);
    }

    /// @inheritdoc INash21Factory
    function unauthorize(uint256 id) external payable onlyRole(_AUTH_ROLE) {
        address account = authorized[id];
        require(account != address(0), "N21F001");
        authorized[id] = address(0);
        emit Unauthorize(id);
    }

    /// @inheritdoc INash21Factory
    function create(uint256 id) external payable whenNotPaused {
        address account = msg.sender;
        address vault = INash21Manager(manager).get(keccak256("VAULT"));
        require(vault != address(0), "N21F002");
        uint256 feeRequired = getCreationFee(id);
        if (feeRequired != 0) {
            IERC20 tokenInterface = IERC20(INash21Manager(manager).get(keccak256("USDT")));
            tokenInterface.safeTransferFrom(account, vault, feeRequired);
        }
        _create(id);
    }

    /// @inheritdoc INash21Factory
    function setCreationFee(uint256 fee) external payable onlyRole(_FACTORY_ADMIN_ROLE) {
        _setCreationFee(fee);
    }

    /// @inheritdoc INash21Factory
    function setTokenCreationFee(uint256 fee, bytes32 hashId) external payable onlyRole(_FACTORY_ADMIN_ROLE) {
        _setTokenCreationFee(fee, hashId);
    }

    /// @inheritdoc INash21Factory
    function setTransferFee(uint256 fee) external payable onlyRole(_FACTORY_ADMIN_ROLE) {
        _setTransferFee(fee);
    }

    /// @inheritdoc INash21Factory
    function setTokenTransferFee(uint256 fee, uint256 id) external payable onlyRole(_FACTORY_ADMIN_ROLE) {
        _setTokenTransferFee(fee, id);
    }

    /// @inheritdoc INash21Factory
    function setOwnerTransferFee(uint256 fee, address owner) external payable onlyRole(_FACTORY_ADMIN_ROLE) {
        _setOwnerTransferFee(fee, owner);
    }
    ...
    /// @inheritdoc INash21Factory
    function setBaseURI(string memory uri) external payable onlyRole(_FACTORY_ADMIN_ROLE) {
        _setBaseURI(uri);
    }

    /// @inheritdoc INash21Factory
    function setTokenURI(uint256 id, string memory _tokenURI) external payable onlyRole(_FACTORY_ADMIN_ROLE) {
        _setTokenURI(id, _tokenURI);
    }

    /// @inheritdoc INash21Factory
    function setContractURI(string memory contractURI_) external payable onlyRole(_FACTORY_ADMIN_ROLE) {
        _setContractURI(contractURI_);
    }

    /// @inheritdoc INash21Factory
    function getCreationFee(uint256 id) public view returns (uint256) {
        uint256 _creationFee = tokenCreationFee[data[id].hashId] == 0 ? creationFee : tokenCreationFee[data[id].hashId];
        if (_creationFee > MAX_FEE) _creationFee = 0;
        INash21Guarantee guaranteeInterface = INash21Guarantee(INash21Manager(manager).get(_GUARANTEE));
        return guaranteeInterface.transformCurrency(data[id].currency, (data[id].value * _creationFee) / 100 ether);
    }

    /// @inheritdoc INash21Factory
    function getTransferFee(uint256 id) public view returns (uint256) {
        require(_exists(id), "N21F018");
        uint256 _transferFee = transferFee;
        if (ownerTransferFee[ownerOf(id)] != 0) {
            _transferFee = ownerTransferFee[ownerOf(id)];
        } else if (tokenTransferFee[id] != 0) {
            _transferFee = tokenTransferFee[id];
        }

        if (_transferFee > MAX_FEE) _transferFee = 0;
        INash21Guarantee guaranteeInterface = INash21Guarantee(INash21Manager(manager).get(_GUARANTEE));
        return guaranteeInterface.transformCurrency(data[id].currency, (data[id].value * _transferFee) / 100 ether);
    }
    ...
    function tokenURI(uint256 id)
        public
        view
        override(INash21Factory, ERC721Upgradeable, ERC721URIStorageUpgradeable)
    ...
    /// @inheritdoc INash21Factory
    function split(address account, uint256 id, uint256 timestamp)
        external
        payable
        whenNotPaused
        returns (uint256, uint256)
    {
        require(INash21Manager(manager).get(_GUARANTEE) == msg.sender, "N21F007");
        require(ownerOf(id) == account, "N21F008");
        _checkTimestamp(id, timestamp);
        ...
        uint256 value1 = (data[id].value * (timestamp - data[id].startDate)) / (data[id].endDate - data[id].startDate);
        uint256 value2 = data[id].value - value1;
        ...
        _setTokenURI(id1, data[id1].uri);
        _setTokenURI(id2, data[id2].uri);

        emit Burn(account, id);
        emit Mint(account, id1);
        emit Mint(account, id2);
    ...
    function setRecipient(uint256 id, address recipient) external payable {
        require(msg.sender == ownerOf(id), "N21F009");
        data[id].recipient = recipient;
    }

    /**
     * PRIVATE/INTERNAL/OVERRIDE FUNCTIONS
     */
    ...
    function _authorize(
        ...
        bytes32 _authHash = keccak256(abi.encodePacked(hashId, account, value, currency, startDate, endDate, uri));
        require(!authorizedHash[_authHash], "N21F010");
        authorizedHash[_authHash] = true;
        require(account != address(0), "N21F011");
        require(startDate < endDate, "N21F012");
        require(value > 0, "N21F013");
        uint256 id = _id;
        _id++;
        data[id] = Data(id, value, currency, startDate, endDate, uri, account, hashId);
        authorized[id] = account;
        emit Authorize(hashId, account, id, value, currency, startDate, endDate, uri);
        return id;
    }

    function _create(uint256 id) internal whenNotPaused {
        address account = authorized[id];
        require(account != address(0), "N21F014");
        authorized[id] = address(0);
        _mint(account, id);
        _setTokenURI(id, data[id].uri);
        emit Mint(account, id);
    }

    function _checkTimestamp(uint256 id, uint256 timestamp) internal view {
        require(timestamp > data[id].startDate, "N21F015");
        require(timestamp < data[id].endDate, "N21F016");
    }
    ...
    function _setTokenURI(uint256 id, string memory _tokenURI) internal override {
    ...
    function _setCreationFee(uint256 fee) internal {
        creationFee = fee;
        emit SetCreationFee(fee);
    }

    function _setTokenCreationFee(uint256 fee, bytes32 hashId) internal {
        tokenCreationFee[hashId] = fee;
        emit SetTokenCreationFee(hashId, fee);
    }

    function _setTransferFee(uint256 fee) internal {
        transferFee = fee;
        emit SetTransferFee(fee);
    }

    function _setTokenTransferFee(uint256 fee, uint256 id) internal {
        tokenTransferFee[id] = fee;
        emit SetTokenTransferFee(id, fee);
    }

    function _setOwnerTransferFee(uint256 fee, address owner) internal {
        ownerTransferFee[owner] = fee;
        emit SetOwnerTransferFee(owner, fee);
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 id, uint256 batchSize)
        ...
        whenNotPaused
    {
        data[id].recipient = to;

        if ((from != address(0)) && (to != address(0))) {
            INash21Guarantee guaranteeInterface = INash21Guarantee(INash21Manager(manager).get(_GUARANTEE));
            require((guaranteeInterface.distributed(id) == 0 || to == address(0)), "N21F017");

            uint256 feeRequired = getTransferFee(id);
            if (feeRequired != 0) {
                IERC20 tokenInterface = IERC20(INash21Manager(manager).get(keccak256("USDT")));
                address vault = INash21Manager(manager).get(keccak256("VAULT"));
                require(vault != address(0), "N21F002");
                tokenInterface.safeTransferFrom(from, vault, feeRequired);
            }
        }
...
import {AccessControlProxyPausable} from "../utils/AccessControlProxyPausable.sol";

contract UUPSUpgradeableByRole is AccessControlProxyPausable, UUPSUpgradeable {
    function _authorizeUpgrade(address newImplementation) internal view override onlyRole(keccak256("UPGRADER_ROLE")) {} //solhint-disable-line no-empty-blocks
}
...
interface INash21Manager {
    ...
    function deploy(bytes32 id, bytes calldata bytecode, bytes calldata initializeCalldata)
        external
    ...
    /// @param initializeCalldata Encoded initialize calldata
    function deployProxyWithImplementation(bytes32 id, address implementation, bytes calldata initializeCalldata)
        external;
    ...
    function implementationByProxy(address proxy) external view returns (address);
    ...
    /// @param implementation Address of the implementation
    /// @param initializeCalldata Encoded initialize calldata
    function upgrade(bytes32 id, address implementation, bytes calldata initializeCalldata) external;
}
...
interface INash21Guarantee {
    ...
    event Claim(uint256 indexed id, address indexed account, uint256 amount, bytes32 currency);
    ...
    /// @param currency Hashed currency symbol
    event Pay(uint256 indexed id, address indexed account, uint256 amount, bytes32 currency);
    ...
    event Split(uint256 indexed id, address indexed account, uint256 timestamp, uint256 beforeId, uint256 afterId);
    ...
    event Fund(address indexed token, address indexed from, uint256 amount);
    ...
    function claimBatch(uint256[] memory ids) external;
    ...
    function extractFunds(address token, address to, uint256 amount) external;
    ...
    function payBatch(uint256[] calldata ids, uint256[] calldata amounts) external;
    ...
    function setFeeds(bytes32[] memory currencies, address[] memory feeds) external;
    ...
    function split(uint256 id, uint256 timestamp) external returns (uint256, uint256);

    /// @notice Returns the amount in USDT of a selected amount of currency
    /// @param currency Hashed currency symbol
    /// @param amount Amount to be transformed
    function transformCurrency(bytes32 currency, uint256 amount) external view returns (uint256);
}
...
interface INash21Factory is IERC721EnumerableUpgradeable {
    ...
    /// @notice Emitted when the creationFee is set/updated
    ...
    /// @notice Emitted when the transferFee is set/updated
    /// @param fee Creation fee
    event SetTransferFee(uint256 fee);

    /// @notice Emitted when the ownerTransferFee is set
    /// @param owner Origin account
    /// @param fee Transfer fee
    event SetOwnerTransferFee(address indexed owner, uint256 fee);
    ...
    /// @notice Returns whether or not a hashId has been authorized or not
    /// @param hashId Offchain identificator
    /// @return Authorized True/false if already authorized or not
    function authorizedHash(bytes32 hashId) external view returns (bool);
    ...
    /// @notice Reainitializes the contract
    /// @dev Fixes name and symbol change
    ...
    /// @param fee Creation fee with 18 decimals
    /// @param owner Origin account
    function setOwnerTransferFee(uint256 fee, address owner) external payable;
    ...
    function split(address account, uint256 id, uint256 timestamp)
        external
        payable
        returns (uint256 id1, uint256 id2);

    /// @notice Returns next minted token ID
    /// @dev Counter starting in 1 always incremented in mints
    ...
    /// @return Token transfer fee
    function ownerTransferFee(address owner) external view returns (uint256);
...
abstract contract AccessControlProxyPausable is PausableUpgradeable {
    ...
    function hasRole(bytes32 role, address account) public view returns (bool) {
        IAccessControlUpgradeable managerInterface = IAccessControlUpgradeable(manager);
        return managerInterface.hasRole(role, account);
    }

    // solhint-disable-next-line
    function __AccessControlProxyPausable_init(address manager_) internal initializer {
        __Pausable_init();
        __AccessControlProxyPausable_init_unchained(manager_);
    }

    // solhint-disable-next-line
    function __AccessControlProxyPausable_init_unchained(address manager_) internal initializer {
        manager = manager_;
    }
    ...
    function updateManager(address manager_) public onlyRole(DEFAULT_ADMIN_ROLE) {
        manager = manager_;
    }
}
