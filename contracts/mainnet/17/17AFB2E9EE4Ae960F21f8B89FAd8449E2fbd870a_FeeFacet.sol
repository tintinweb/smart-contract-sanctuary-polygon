...
contract FeeFacet is Modifiers, ITransferHookReceiver {
    ...
    function setStreamingFee(address _addr, uint16 _value) external onlyTokenManager(_addr) {
        LibFee.setStreamingFee(_addr, _value);
    }

    function setSuccessFee(address _addr, uint16 _value) external onlyTokenManager(_addr) {
        LibFee.setSuccessFee(_addr, _value);
    }

    function getStreamingFee(address _addr) external view returns (uint32) {
        LibFee.StreamingFeeData storage d = LibFee.streamingFeeDataForToken(_addr);
        return d.value;
    }

    function getAccumulatedStreamingFee(address _addr) external view returns (uint256) {
        LibFee.StreamingFeeData storage d = LibFee.streamingFeeDataForToken(_addr);
        return d.accumulatedAmount;
    }

    function accumulateStreamingFee(address _addr) public {
        LibFee.StreamingFeeData storage d = LibFee.streamingFeeDataForToken(_addr);
        if (d.value != 0) {
            ...
            uint256 unclaimedAmount = IERC20(_addr).balanceOf(address(this));
            d.accumulatedAmount +=
                ((IERC20(_addr).totalSupply() - unclaimedAmount) * d.value * (block.timestamp - d.lastAccumulatedAt)) /
                31536000 /
                1000;
        }

        d.lastAccumulatedAt = block.timestamp;
        emit StreamingFeeAccumulated(_addr, d.accumulatedAmount);
    }

    function mintAccumulatedStreamingFee(address _addr) external {
        LibFee.StreamingFeeData storage d = LibFee.streamingFeeDataForToken(_addr);
        uint256 amt = d.accumulatedAmount;
        if (amt == 0) {
            return;
        }

        d.accumulatedAmount = 0;
        PortToken(_addr).controllerMint(address(this), amt);

        emit StreamingFeeAccumulated(_addr, 0);
        emit StreamingFeeMinted(_addr, amt);
    }

    function claimStreamingFee(address _addr, uint256 _amount) external onlyTokenManager(_addr) {
        PortToken port = PortToken(_addr);
        port.transfer(port.manager(), _amount);
    }

    function burnStreamingFee(address _addr, uint256 _amount) external onlyTokenManager(_addr) {
        PortToken(_addr).burn(_amount);
    }
