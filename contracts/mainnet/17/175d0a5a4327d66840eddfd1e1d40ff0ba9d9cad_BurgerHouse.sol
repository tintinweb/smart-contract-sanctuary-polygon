...
contract BurgerHouse {
    ...
    uint256 public constant COIN_PRICE = 5 * 10**(18 - 3); // 1 coin = 0.005 USDC
    uint256 public constant CASH_PRICE = 5 * 10**(18 - 5); // 100 cash = 0.005 USDC
    ...
    function addCoins(address _ref, uint256 _amount) external {
        require(_amount > 0, "ZERO_USDC_AMOUNT");
        require(
        ...
        require(
            USDC.transfer(DEV_WALLET, (_amount * DEV_FEE) / DENOMINATOR),
            "TRANSFER_FAIL"
        );
    }
    ...
    function withdrawMoney() external whenLaunched {
        ...
        amount = USDC.balanceOf(address(this)) < amount
            ? USDC.balanceOf(address(this))
            : amount;
        require(USDC.transfer(user, amount), "TRANSFER_FAIL");
