...
contract PRCUtility is ERC20, AccessControl, ERC721Holder {
    bytes32 public constant CONTRACT_ADMIN_ROLE = keccak256("CONTRACT_ADMIN_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    address public collectionAddressPandaRaiseClubNFT;
    address public collectionAddressNFTv2;

    uint256 public numberOfBlocksPerRewardUnit;

    uint256 public coinAmountPerRewardUnitPandaRaiseClubNFT;
    uint256 public coinAmountPerRewardUnitNFTv2;

    uint256 public welcomeBonusAmountPandaRaiseClubNFT;
    uint256 public welcomeBonusAmountNFTv2;

    uint256 public amountOfStakersPandaRaiseClubNFT;
    uint256 public amountOfStakersNFTv2;

    uint256 public tokensStakedPandaRaiseClubNFT;
    ...
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogPandaRaiseClubNFT;

        /// owner => tokenId => StakeInfo
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogNFTv2;

    /// owner => #NFTsStaked
    mapping (address => uint256) public tokensStakedByUserPandaRaiseClubNFT;
    ...
    mapping (uint256 => bool) public welcomeBonusCollectedPandaRaiseClubNFT;

        /// tokenId => true/false (true if welcome bonus has been collected for a specific tokenId)
    mapping (uint256 => bool) public welcomeBonusCollectedNFTv2;

    /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserPandaRaiseClubNFT;

        /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserNFTv2;

    /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioPandaRaiseClubNFT;

        /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioNFTv2;

    event RewardsHarvestedPandaRaiseClubNFT (address owner, uint256 amount);
    event NFTStakedPandaRaiseClubNFT (address owner, uint256 tokenId);
    event NFTUnstakedPandaRaiseClubNFT (address owner, uint256 tokenId);
    ...
    constructor(address owner, address _collectionAddressPandaRaiseClubNFT, address _collectionAddressNFTv2) ERC20("PRCUtility", "PRCU") AccessControl(){
        _mint(owner, 100000 * 10 ** 18);
        _setupRole(DEFAULT_ADMIN_ROLE, owner);
        _setupRole(CONTRACT_ADMIN_ROLE, owner);
        _setupRole(BURNER_ROLE, owner);
        _setupRole(MINTER_ROLE, owner);
        collectionAddressPandaRaiseClubNFT = _collectionAddressPandaRaiseClubNFT;
        collectionAddressNFTv2 = _collectionAddressNFTv2;
        contractCreationBlock = block.number;
        coinAmountPerRewardUnitPandaRaiseClubNFT = 5 * 10 ** 18; // 5 ERC20 coins per rewardUnit, may be changed later on
        ...
        welcomeBonusAmountPandaRaiseClubNFT = 300 * 10 ** 18; // 300 tokens welcome bonus, only paid once per tokenId
        welcomeBonusAmountNFTv2 = 600 * 10 ** 18; // 600 tokens welcome bonus, only paid once per tokenId
    }

    function stakedNFTSByUserPandaRaiseClubNFT(address owner) external view returns (uint256[] memory){
        return stakePortfolioByUserPandaRaiseClubNFT[owner];
    }
    ...
    function pendingRewardsPandaRaiseClubNFT(address owner, uint256 tokenId) public view returns (uint256){
        StakeInfo memory info = stakeLogPandaRaiseClubNFT[owner][tokenId];
        ...
        return rewardAmount * coinAmountPerRewardUnitPandaRaiseClubNFT;
    ...
    function stakePandaRaiseClubNFT(uint256 tokenId) public {
        IERC721(collectionAddressPandaRaiseClubNFT).safeTransferFrom(_msgSender(), address(this), tokenId);
        require(IERC721(collectionAddressPandaRaiseClubNFT).ownerOf(tokenId) == address(this),
            "PSCUtility: Error while transferring token");
        StakeInfo storage info = stakeLogPandaRaiseClubNFT[_msgSender()][tokenId];
        info.stakedAtBlock = block.number;
        info.lastHarvestBlock = block.number;
        info.currentlyStaked = true;
        if(tokensStakedByUserPandaRaiseClubNFT[_msgSender()] == 0){
            amountOfStakersPandaRaiseClubNFT += 1;
        }
        tokensStakedByUserPandaRaiseClubNFT[_msgSender()] += 1;
        tokensStakedPandaRaiseClubNFT += 1;
        stakePortfolioByUserPandaRaiseClubNFT[_msgSender()].push(tokenId);
        uint256 indexOfNewElement = stakePortfolioByUserPandaRaiseClubNFT[_msgSender()].length - 1;
        indexOfTokenIdInStakePortfolioPandaRaiseClubNFT[tokenId] = indexOfNewElement;
        if(!welcomeBonusCollectedPandaRaiseClubNFT[tokenId]) {
            _mint(_msgSender(), welcomeBonusAmountPandaRaiseClubNFT);
            welcomeBonusCollectedPandaRaiseClubNFT[tokenId] = true;
        }

        emit NFTStakedPandaRaiseClubNFT(_msgSender(), tokenId);
    }
    ...
    function stakeBatchPandaRaiseClubNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            stakePandaRaiseClubNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakeBatchPandaRaiseClubNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            unstakePandaRaiseClubNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakePandaRaiseClubNFT(uint256 tokenId) public {
        if(pendingRewardsPandaRaiseClubNFT(_msgSender(), tokenId) > 0){
            harvestPandaRaiseClubNFT(tokenId);
        }
        StakeInfo storage info = stakeLogPandaRaiseClubNFT[_msgSender()][tokenId];
        info.currentlyStaked = true;
        IERC721(collectionAddressPandaRaiseClubNFT).safeTransferFrom(address(this), _msgSender(), tokenId);
        require(IERC721(collectionAddressPandaRaiseClubNFT).ownerOf(tokenId) == _msgSender(),
            "PSCUtility: Error while transferring token");
        if(tokensStakedByUserPandaRaiseClubNFT[_msgSender()] == 1){
            amountOfStakersPandaRaiseClubNFT -= 1;
        }
        tokensStakedByUserPandaRaiseClubNFT[_msgSender()] -= 1;
        tokensStakedPandaRaiseClubNFT -= 1;
        stakePortfolioByUserPandaRaiseClubNFT[_msgSender()][indexOfTokenIdInStakePortfolioPandaRaiseClubNFT[tokenId]] = 0;
        emit NFTUnstakedPandaRaiseClubNFT(_msgSender(), tokenId);
    }
    ...
    function harvestPandaRaiseClubNFT(uint256 tokenId) public {
        StakeInfo storage info = stakeLogPandaRaiseClubNFT[_msgSender()][tokenId];
        uint256 rewardAmountInERC20Tokens = pendingRewardsPandaRaiseClubNFT(_msgSender(), tokenId);
        if(rewardAmountInERC20Tokens > 0) {
            info.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInERC20Tokens);
            emit RewardsHarvestedPandaRaiseClubNFT(_msgSender(), rewardAmountInERC20Tokens);
        }
    }
    ...
    function harvestBatchPandaRaiseClubNFT(address user) external {
        uint256[] memory tokenIds = stakePortfolioByUserPandaRaiseClubNFT[user];

        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            harvestPandaRaiseClubNFT(tokenIds[currentId]);
        }
    }
    ...
    function setCoinAmountPerRewardUnitPandaRaiseClubNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        coinAmountPerRewardUnitPandaRaiseClubNFT = coinAmount;
    }
    ...
    function setWelcomeBonusAmountPandaRaiseClubNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        welcomeBonusAmountPandaRaiseClubNFT = coinAmount;
    }
    ...
    function setCollectionAddressPandaRaiseClubNFT(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "VIBE: update to zero address not possible");
        collectionAddressPandaRaiseClubNFT = newAddress;
    }
