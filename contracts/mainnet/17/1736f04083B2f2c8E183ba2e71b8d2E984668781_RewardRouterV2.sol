...
contract RewardRouterV2 is ReentrancyGuard, Governable {
    ...
    address public glp; // CBH Liquidity Provider token
    ...
    function mintAndStakeClp(address _token, uint256 _amount, uint256 _minUsdg, uint256 _minClp) external nonReentrant returns (uint256) {
        ...
        uint256 glpAmount = IClpManager(glpManager).addLiquidityForAccount(account, account, _token, _amount, _minUsdg, _minClp);
        IRewardTracker(feeClpTracker).stakeForAccount(account, account, glp, glpAmount);
        IRewardTracker(stakedClpTracker).stakeForAccount(account, account, feeClpTracker, glpAmount);

        emit StakeClp(account, glpAmount);
    ...
    function mintAndStakeClpETH(uint256 _minUsdg, uint256 _minClp) external payable nonReentrant returns (uint256) {
        ...
        address account = msg.sender;
        uint256 glpAmount = IClpManager(glpManager).addLiquidityForAccount(address(this), account, weth, msg.value, _minUsdg, _minClp);
    ...
    function unstakeAndRedeemClp(address _tokenOut, uint256 _glpAmount, uint256 _minOut, address _receiver) external nonReentrant returns (uint256) {
        require(_glpAmount > 0, "RewardRouter: invalid _glpAmount");

        address account = msg.sender;
        IRewardTracker(stakedClpTracker).unstakeForAccount(account, feeClpTracker, _glpAmount, account);
        IRewardTracker(feeClpTracker).unstakeForAccount(account, glp, _glpAmount, account);
        uint256 amountOut = IClpManager(glpManager).removeLiquidityForAccount(account, _tokenOut, _glpAmount, _minOut, _receiver);

        emit UnstakeClp(account, _glpAmount);

        return amountOut;
    }

    function unstakeAndRedeemClpETH(uint256 _glpAmount, uint256 _minOut, address payable _receiver) external nonReentrant returns (uint256) {
        require(_glpAmount > 0, "RewardRouter: invalid _glpAmount");

        address account = msg.sender;
        IRewardTracker(stakedClpTracker).unstakeForAccount(account, feeClpTracker, _glpAmount, account);
        IRewardTracker(feeClpTracker).unstakeForAccount(account, glp, _glpAmount, account);
        uint256 amountOut = IClpManager(glpManager).removeLiquidityForAccount(account, weth, _glpAmount, _minOut, address(this));
    ...
    function handleRewards(
        ...
        if (_shouldClaimEsGmx) {
            ...
            uint256 esGmxAmount1 = IRewardTracker(stakedClpTracker).claimForAccount(account, account);
    ...
    function acceptTransfer(address _sender) external nonReentrant {
        ...
        uint256 glpAmount = IRewardTracker(feeClpTracker).depositBalances(_sender, glp);
        if (glpAmount > 0) {
            IRewardTracker(stakedClpTracker).unstakeForAccount(_sender, feeClpTracker, glpAmount, _sender);
            IRewardTracker(feeClpTracker).unstakeForAccount(_sender, glp, glpAmount, _sender);

            IRewardTracker(feeClpTracker).stakeForAccount(_sender, receiver, glp, glpAmount);
            IRewardTracker(stakedClpTracker).stakeForAccount(receiver, receiver, feeClpTracker, glpAmount);
        }
    ...
    function _compoundClp(address _account) private {
        uint256 esGmxAmount = IRewardTracker(stakedClpTracker).claimForAccount(_account, _account);
        if (esGmxAmount > 0) {
            _stakeGmx(_account, _account, esGmx, esGmxAmount);
        }
    }
