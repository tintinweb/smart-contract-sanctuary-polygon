...
contract MarketplaceListing is Ownable,  MarketplaceListingHelper {
    ...
    function cancelOffer(uint256 offerId) public { 
        ...
        // Emit the event
        emit OfferCancelled(offerId, offerIdToOffer[offerId].offerAmount, offerIdToOffer[offerId].tokenId, offerIdToOffer[offerId].tokenAddress);
    }
    ...
    function acceptOffer(uint256 offerId) public { 
        ...
        emit OfferAccepted(offerId, offer.offerAmount, offer.tokenId, offer.tokenAddress);
    }
    ...
    function cancelAllUserOffers(address _addr) public { 
        ...
        for(uint  i = 0; i < userOfferIds.length; i++){
            ...
            emit OfferCancelled( offerIdToOffer[userOfferIds[i]].offerId,  offerIdToOffer[userOfferIds[i]].offerAmount,  offerIdToOffer[userOfferIds[i]].tokenId,  offerIdToOffer[userOfferIds[i]].tokenAddress);
        }
    ...
    */
    ...
    function createListing(uint256 tokenId, uint256 price, address _tokenAddress) public { 
        ...
        if(listingIdToListing[tokenIdToListingId[tokenId]].state == State.OPEN){
            ...
            // Emit the event
            emit ListingCancelled(listingIdToListing[tokenIdToListingId[tokenId]].listingId, listingIdToListing[tokenIdToListingId[tokenId]].tokenId, listingIdToListing[tokenIdToListingId[tokenId]].tokenAddress, listingIdToListing[tokenIdToListingId[tokenId]].seller);
        }
    ...
    function buyFromListing(uint256 listingId) public {
        ...
        if(block.timestamp > listing.expiration){
            listingIdToListing[tokenIdToListingId[listing.tokenId]].state = State.CANCELLED;
            _removeListingStorage(listingIdToListing[tokenIdToListingId[listing.tokenId]].listingId);

            // Emit the event
            emit ListingCancelled(listingIdToListing[tokenIdToListingId[listing.tokenId]].listingId,listingIdToListing[tokenIdToListingId[listing.tokenId]].tokenId, listingIdToListing[tokenIdToListingId[listing.tokenId]].tokenAddress, listingIdToListing[tokenIdToListingId[listing.tokenId]].seller);
        }
        ...
        emit ListingSold(listing.listingId, listing.tokenId, listing.seller, listing.tokenAddress, msg.sender);
    }
