...
contract MATICBANK {
	...
	uint constant public			PROJECT_FEE						= 200;					// project fee 20% of deposit
	uint constant public			WITHDRAW_COOLDOWN				= 1 days;			// claim 1 time per day
	uint constant public			BASE_PERCENT  					= 10;	
	uint constant public			Total_PROFIT					= 250;	
	uint constant public 			Amount_Slab						= 30000 ether;
	...
	function invest(address _referrer) public payable {
		...
		_createDeposit( msg.sender, 0, msg.value, false );

		_insuranceTrigger();

		USERS[USERS[msg.sender].referrer].directBusiness +=msg.value;
	...
	function claim() public {
		...
		uint claimAmount = _getUserDividendsSettle(msg.sender) + user.refDividends + user.debtBuffer;
	...
	function _createDeposit( address _user, uint _planIdx, uint _amount, bool _isReinvest ) internal returns(uint o_depIdx) {
		...
		TDeposit memory newDep = TDeposit( _planIdx, _amount, block.timestamp, block.timestamp + PLANS[_planIdx].durationDays * TIME_STEP, _isReinvest,0 );
	...
	function getPercent(address _user) public view returns(uint256 percent){
		if(USERS[_user].directBusiness>=Amount_Slab){
	...
	function _calculateDepositDividends(address _user, uint _depIdx) internal view returns (uint o_amount) {
		...
		uint totalReward = deposit.amount * getPercent(_user) / PERCENTS_DIVIDER;
		...
		if (timeA < timeB) {
			...
			if(deposit.withdrawn+(o_amount) > deposit.amount*(Total_PROFIT)/(100))
			{
				o_amount = (deposit.amount*(Total_PROFIT)/(100)) - deposit.withdrawn;
			}
		}
	}

	function _settleDepositDividends(address _user, uint _depIdx) internal returns (uint o_amount) {
		...
		if (timeA < timeB) {
			...
			deposit.withdrawn += o_amount;
		}
	}

	function _getUserDividendsSettle(address _user) internal returns (uint o_amount) {

		for(uint i=0;i<USERS[_user].deposits.length;i++) {
			if(_isDepositDeceased(_user,i)) continue;
			o_amount += _settleDepositDividends(_user,i);
		}
	...
	function getUserDepositHistory(address _user, uint _numBack) public view returns(TDeposit[5] memory o_deposits, uint o_total, uint o_idxFrom, uint o_idxTo, uint o_timestamp) {
		...
		o_timestamp = block.timestamp;
		o_total = USERS[_user].deposits.length;
		...
		for(uint i=o_idxFrom; i<o_idxTo; i++) {
			o_deposits[i-o_idxFrom] = USERS[_user].deposits[i];
		}
