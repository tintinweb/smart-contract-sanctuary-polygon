...
contract Security is ERC20, ERC20Permit, ERC20Burnable, Pausable {
    ...
    string[] _docNames;
    uint256 private _totalDividendToDistribute;
    ...
    event UpdateDividendValue(address[] holders, uint256[] amount);
    ...
    function distributeDividends(uint256 _startIndex, uint256 _endIndex)
        external
        onlyRole(MANAGER_ROLE)
    {
        uint256 dividendTotal = _totalDividendToDistribute;
        uint256 contractBalance = getTetherContractBalance();
        require(dividendTotal != 0, "Dividend value is zero.");
        require(contractBalance > 0, "There are no tokens in contract.");
        require(
            contractBalance == dividendTotal,
            "balance in contract is not equal to dividendTotal."
        );
        ...
        uint256[] memory dividendValue = new uint256[](size);
        ...
        for (uint256 i = _startIndex; i <= _endIndex; i++) {
            ...
            if (balance != 0) {
                uint256 value = (balance * dividendTotal) / totalSupply;
                USDT.safeTransfer(key, value);
                investorsAddress[i] = key;
                dividendValue[i] = value;
            }
        }
        _totalDividendToDistribute = 0;
        emit DistributeDividends(investorsAddress, dividendValue);
    }
    ...
    function setDividendValue() public onlyRole(MANAGER_ROLE) {
        require(getTetherContractBalance() > 0, "No token in contract.");
        _totalDividendToDistribute = getTetherContractBalance();
    }
    ...
    function getTotalDividendToDistribute()
        public
        view
        onlyRole(MANAGER_ROLE)
        returns (uint256)
    {
        return _totalDividendToDistribute;
    }
    ...
    function _beforeTokenTransfer(
        ...
        bool fromIsBlocklisted = accessControl.hasRole(
            keccak256(abi.encodePacked("BLOCKLISTED_ROLE")),
            from
        );
        bool toIsAdmin = accessControl.hasRole(
            keccak256(abi.encodePacked(MANAGER_ROLE)),
            to
        );
        require(!fromIsBlocklisted || toIsAdmin, "Blocklisted");
