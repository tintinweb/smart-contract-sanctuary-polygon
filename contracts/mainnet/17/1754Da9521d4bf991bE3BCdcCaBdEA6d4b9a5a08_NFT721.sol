 ...
 * method only.
 */
abstract contract Adminarable is AccessControl {
    ...
    modifier onlyAdmin() {
        require(isAdmin(_msgSender()), "Adminarable: restricted to admins");
        _;
    }
 ...
 */
contract NFT721 is ERC721URIStorage, Adminarable {
    ...
    function castTransfer(address from, address to, uint256 tokenId) public onlyAdmin {
        require(ownerOf(tokenId) == from, "from is not owner or tokenId is not existing");
        _transfer(from, to, tokenId);
	}
     ...
     */
	function updateTokenURI(uint256 tokenId, string memory tokenURI) public onlyAdmin {
