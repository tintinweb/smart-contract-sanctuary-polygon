...
contract Emitter is Initializable, AccessControl {
    ...
    function changeImplementationAddress(address _newImp)
        external
        payable
        onlyOwner
    {
    ...
    ReentrancyGuard
{
    ...
    function updateMinMaxDeposit(
        ...
        if (minDepositPerUser == 0)
            revert AmountInvalid("minDepositPerUser", minDepositPerUser);
    ...
    function deposit(
        ...
        uint256 daoBalance = ERC20(usdcTokenAddress).balanceOf(address(this));
        ...
        uint256 customTokenDecimals = ERC20(usdcTokenAddress).decimals();
        ...
        if (feeUSDC) {
            USDCDeposited[usdcTokenAddress] += userShareUsdc;
            ERC20(usdcTokenAddress).transferFrom(
                msg.sender,
                address(this),
                userShareUsdc
            );
            USDCTransferedToAdmin[usdcTokenAddress] += adminShareUsdc;
            ERC20(usdcTokenAddress).transferFrom(
                msg.sender,
                deployerAddress,
                adminShareUsdc
            ...
            USDCDeposited[usdcTokenAddress] += _totalAmount;
            ERC20(usdcTokenAddress).transferFrom(
                msg.sender,
                address(this),
                _totalAmount
    ...
    function updateRaiseAmount(uint256 _totalRaiseAmount) internal {
        ...
        if (totalRaiseAmount == 0)
            revert AmountInvalid("totalRaiseAmount", totalRaiseAmount);
