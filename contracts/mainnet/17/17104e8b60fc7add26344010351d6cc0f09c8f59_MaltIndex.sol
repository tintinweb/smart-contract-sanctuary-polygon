...
import "../interfaces/IRewardReinvestor.sol";
import "../interfaces/IRewardMine.sol";


/// @title Malt Index
/// @author 0xMojo <[email protected]>
/// @notice Provide an index of return on $100 of malt that is never unbonding and constantly reinvested
contract MaltIndex is Permissions {
  using SafeERC20 for ERC20;

  IBonding public bonding;
  IRewardReinvestor public rewardReinvestor;
  IRewardMine public rewardMine;
  ERC20 public lpToken;
  uint256 poolId;

  mapping(address => uint256) public reinvestTriggers;
  uint256 public totalEarned;

  event Deposit(address depositor, uint256 amount, IERC20 lpToken);
  event Bond(address bonder, uint256 amount);
  event Unbond(address bonder, uint256 amount);
  event Reinvest(address executor, uint256 rewards);
  event Withdraw(address executor, uint256 amount);

  constructor (
    address _bonding,
    address _rewardReinvestor,
    address _rewardMine,
    address _lpToken,
    address initialAdmin,
    uint256 _poolId
  ) {
    require(_bonding != address(0), "Bonding addr(0)");
    require(_rewardReinvestor != address(0), "Reward Reinvestor addr(0)");
    require(_rewardMine != address(0), "Reward mine addr(0)");
    require(_lpToken != address(0), "lpToken addr(0)");
    require(initialAdmin != address(0), "Admin addr(0)");
    
    bonding = IBonding(_bonding);
    rewardReinvestor = IRewardReinvestor(_rewardReinvestor);
  ...
  /// @dev Allows Admin to deposit initial LP tokens
  /// @notice Will need to approve outside of contract
  function deposit(uint256 amount)
    onlyRoleMalt(ADMIN_ROLE, "Must be admin")
    external
  {
    SafeERC20.safeTransferFrom(lpToken, msg.sender, address(this), amount);
    lpToken.safeApprove(address(bonding), amount);
    bonding.bond(poolId, amount);
    lpToken.safeApprove(address(bonding), 0);
    emit Bond(msg.sender, amount);
  }

  ///@dev Allows Admin to unbond their LP tokens, only will be called for a migration.
  function unbond(uint256 amount)
    onlyRoleMalt(ADMIN_ROLE, "Must be admin")
    external
  {
    bonding.unbond(poolId, amount);
    emit Unbond(msg.sender, amount);
  }

  ///@dev allows admin to withdraw ERC20 tokens from contract
  ...
  ///@dev Allows Admin to change poolId, only will be called for a migration.
  ...
  /// @dev Allows anyone to reinvest the MaltIndex funds.
  /// @param rewardReserves passed from dapp for security purposes.
  function reinvest(uint256 rewardReserves)
    public
  {   
    uint256 earnings = rewardMine.earned(address(this));
    rewardReinvestor.splitReinvest(poolId, earnings, rewardReserves, 500);
    
    reinvestTriggers[msg.sender] += 1;
    totalEarned += earnings;

    emit Reinvest(msg.sender, earnings);
  }

  /// @dev Returns the current reward balance earned by the index.
  function currentRewardBalance()
    public
    view
    returns (uint256)
  {
    return rewardMine.earned(address(this));
  }

  /// @dev Returns the balance of the LP used in this contract.
  function liquidityPoolBalance()
    public
    view
    returns (uint256)
  {
    return bonding.balanceOfBonded(poolId, address(this));
  }
...
interface IRewardReinvestor {
    function provideReinvest(uint256 poolId, uint256 rewardLiquidity, uint256 maltLiquidity, uint256 slippageBps) external;
    function splitReinvest(uint256 poolId, uint256 rewardLiquidity, uint256 rewardReserves, uint256 slippageBps) external;
}
