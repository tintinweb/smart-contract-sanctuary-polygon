 ...
 *
 */
contract AdminController is App, __with_semver(uint8(1), uint8(1), uint8(0)) {
    using EnumerableSet for EnumerableSet.AddressSet;
...
contract ERC721BaseSale is App {
    ...
    event OnBuy(
        address buyer, address appAddress, 
        bytes32 tierValue, uint256 cost, 
        address buyToken, string saleType, string semver);
    ...
    function _afterBuy(
        ...
        _afterBuy(buyer, appAddress, tierValue, cost, 0x0000000000000000000000000000000000001010, saleType, semver);
    }
}
...
contract ERC721OpenSale is ERC721BaseSale, __with_semver(uint8(1), uint8(1), uint8(0)) {
...
contract UniswapV3Oracle is
    ...
    OwnableUpgradeable,
    IOracle,
    __with_semver(uint8(1), uint8(0), uint8(2))
{
    ...
    address public constant nativeTokenAddress = 0x0000000000000000000000000000000000001010;
    ...
    function getTradePrice(address tokenSalePrice, address tokenBuyPrice) public view returns (uint256 price) {
...
contract DAOConstructor is
    ...
    OwnableUpgradeable,
    IDAOConstructor,
    __with_semver(uint8(1), uint8(2), uint8(1))
{
    ...
    function _deployOpenSale(
        ...
        address openSaleCode = updateManager.getLastAppCode(AppsIds.ERC721_OPEN_SALE);
        ERC721OpenSale openSale = ERC721OpenSale(
            kernel.deployApp(
                AppsIds.ERC721_OPEN_SALE,
                openSaleCode,
                abi.encodeCall(ERC721OpenSale(openSaleCode).initialize, (kernel))
            )
        );
