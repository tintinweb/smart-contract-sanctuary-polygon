...
/// @title ERC-721 Token for FitMint NFT
contract FitMintPassNFT is ERC721, ERC721Enumerable, ERC721Burnable, Pausable, Ownable {
    ...
    constructor(string memory uri) public ERC721("FitMint 2.0 NFT", "FITNFT") {
        _defaultUri = uri;
    }
    ...
    /// @dev Withdraw Functions for recovering ERC20, ERC721 & native coin
    function withdrawMatic(uint256 _amount, address payable _to) external onlyOwner {
        require(address(this).balance >= _amount, "Insufficient Matic in the NFT Contract");
    ...
    function withdrawERC20(address _tokenAddress, address _to, uint256 _amount) external onlyOwner {
        require(_amount <= IERC20(_tokenAddress).balanceOf(address(this)), "NFT Contract does not have requested amount of given ERC20");
    ...
    function withdrawERC721(address _tokenAddress, address _to, uint256 _tokenId) external onlyOwner {
        require(address(this) <= IERC721(_tokenAddress).ownerOf(_tokenId), "NFT Contract does not have ERC721 of given tokenId");
    ...
    function mint(address _address) external{
        require(msg.sender == gameContractAddress, "Only Game Contract can call the function & mint the NFT");
        _mint(_address, supplyCount);
        supplyCount += 1;
    }
}
