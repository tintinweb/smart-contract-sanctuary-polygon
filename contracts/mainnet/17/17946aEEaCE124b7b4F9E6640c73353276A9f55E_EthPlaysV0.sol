...
contract EthPlaysV0 is Ownable {
  ...
  function submitControlBid(uint256 amount) external onlyActive onlyRegistered {
    ...
    if (block.timestamp > controlAuctionTimestamp + controlAuctionDuration) {
...
contract Poke is ERC20, Ownable {
  ...
  function gameMint(address account, uint256 amount) external onlyGameContract {
  ...
  function gameBurn(address account, uint256 amount) external onlyGameContract {
