...
import "../signVerifierRegistry/SignVerifierRegistryClient.sol";

contract Autograph__v1_0 is
  Initializable,
  ERC721Upgradeable,
  OwnableUpgradeable,
  UUPSUpgradeable,
  SignVeriferRegistryClient
{
  ...
  constructor() {
    initialize(0x0000000000000000000000000000000000000000, 0, "");
  }
  ...
  function initialize(
    ...
    __ERC721_init("Autograph__v1_0", "AUTO");
    __Ownable_init();
  ...
  function getMintSigningHash(
    ...
    return
      keccak256(abi.encodePacked(blockExpiry, recipient, tokenId, autographer, address(this), mintNonces[recipient]));
  }

  function mintWithSigSelf(bytes memory sig, uint256 blockExpiry, address recipient, uint256 tokenId) public virtual {
    // Make sure the autograph is valid
    bytes32 message = getMintSigningHash(blockExpiry, recipient, tokenId, msg.sender).toEthSignedMessageHash();
  ...
  function mintWithSigRelayed(
    bytes memory sig,
    uint256 blockExpiry,
    address recipient,
    uint256 tokenId,
    address autographer,
    bytes memory autographerSig
  ) public virtual {
    // Make sure the autograph is valid
    bytes32 message = getMintSigningHash(blockExpiry, recipient, tokenId, autographer).toEthSignedMessageHash();
    require(ECDSA.recover(message, sig) == getSignVerifier(), "Permission to call this function failed");
    require(block.number < blockExpiry, "Sig expired");

    // Verify that the autographer signed the tokenId and recipient
    require(
      ECDSA.recover(keccak256(abi.encodePacked(tokenId, recipient)), autographerSig) == autographer,
      "Permission to call this function failed"
    );

    mintNonces[recipient]++;
    mintedBy[tokenId] = autographer;
...
abstract contract SignVeriferRegistryClient {
  // Address of registry that resolves the signVerifier
  ISignVerifierRegistry public signVerifierRegistry;

  // ID of the desired signVerifier to lookup in the registry
   ...
   */
  function setSignVerifierRegistry(address _signVerifierRegistry) internal {
   ...
   */
  function setSignVerifierId(bytes32 _signVerifierId) internal {
