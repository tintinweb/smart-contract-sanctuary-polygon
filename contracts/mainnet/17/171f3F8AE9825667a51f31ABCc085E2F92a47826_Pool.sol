...
contract RewardManager is Ownable {
    ...
    // Reward token
    ERC20 public token;
    // Info of each user that stakes virtual LP tokens
    mapping(address => UserInfo) public userInfo;

    // Armin fee share (in BP)
    ...
    function setAdminFeeShare(uint256 _adminFeeShareBP) external onlyOwner {
        require(_adminFeeShareBP <= BP, "RewardManager: too high");
        adminFeeShareBP = _adminFeeShareBP;
    }

    // Add revard to the pool, split admin fee share and update accumulated rewards per share
    ...
    // Returns pending rewards for the address
    function pendingReward(address user_) external view returns (uint256)
    {
        UserInfo memory user = userInfo[user_];
        return ((user.lpAmount * accRewardPerShareP) >> P) - user.rewardDebt;
    }

    // Deposit LP amount for the user, updates user reward debt and pays pending rewards
    ...
    // Claims pending rewards without updating stake balance
    function claimRewards() external {
        UserInfo storage user = userInfo[msg.sender];
        if (user.lpAmount > 0) {
            uint256 rewards = (user.lpAmount * accRewardPerShareP) >> P;
...
contract Pool is RewardManager {
    using SafeERC20 for ERC20;
    uint256 public a;
    int256 constant public PP = 1e4; // Price Precision
    uint256 private constant BP = 10000; // Basis Points
    uint256 private constant MAX_TOKEN_BALANCE = 2 ** 40; // Max possible token balance
    uint256 public feeShareBP;

    address public router;
    uint256 public tokenBalance;
    uint256 public vUsdBalance;
    uint256 public d;

    uint256 private tokenAmountReduce;
    uint256 private tokenAmountIncrease;
    ...
    constructor(address router_,
        uint256 a_,
        ERC20 token_,
        uint256 feeShareBP_) RewardManager(token_) {
        a = a_;
        router = router_;
        ...
        if (decimals > 3) {
            tokenAmountReduce = 10 ** (decimals - 3);
        }
        if (decimals < 3) {
            tokenAmountIncrease = 10 ** (3 - decimals);
        }
    }

    modifier onlyRouter() {
        require(router == msg.sender, "Pool: caller is not router");
        _;
    }

    function toSystemPrecision(uint256 amount) internal view returns (uint256) {
    ...
    function fromSystemPrecision(uint256 amount) internal view returns (uint256) {
        if (tokenAmountReduce > 0) {
            return amount * tokenAmountReduce;
        } else if (tokenAmountIncrease > 0) {
            return amount / tokenAmountIncrease;
    ...
    // Calculate price and deposit token according to the amount and price, then add the same amount to the X and to the Y
    ...
    function _preWithdrawSwap(uint256 amountToken, uint256 amountVUsd) internal view returns (uint256) { unchecked {
        if (amountToken > amountVUsd) {
            uint256 extraToken = (amountToken - amountVUsd) >> 1; // Underflow is impossible here
            uint256 extraVUsd = vUsdBalance - this.getY(tokenBalance + extraToken); // Underflow is ok
            return Math.min(amountToken - extraToken, amountVUsd + extraVUsd);
        } else {
            uint256 extraVUsd = (amountVUsd - amountToken) >> 1; // Underflow is impossible here
            uint256 extraToken = tokenBalance - this.getY(vUsdBalance + extraVUsd); // Underflow is ok
            return Math.min(amountVUsd - extraVUsd, amountToken + extraToken);
        }
    }}

    // Transfer amount from the user to the bridge, calculate new Y according to new X, and return the difference between old and new value
    ...
    // Calculate new X according to new Y, Transfer calculated amount from the bridge to the user, and return the amount
    ...
    function getY(uint256 x) external view returns (uint256) {
        uint256 d_ = d; // Gas optimization
        uint256 a4_ = a << 2;
        uint256 a8_ = a4_ << 1;
        // 4A(D - x) - D
        int256 part1 = int256(a4_) * (int256(d_) - int256(x)) - int256(d_);
        // x * (4AD³ + x(part1²))
        uint256 part2 = x * (a4_ * d_ * d_ * d_ + x * uint256(part1 * part1));
        // (sqrt(part2) + x(part1)) / 8Ax)
        return SafeCast.toUint256(int256(sqrt(part2)) + int256(x) * part1) / (a8_ * x) + 1;// +1 to offset rounding errors
    }

    //     price = (1/2) * ((D³ + 8ADx² - 8Ax³ - 2Dx²) / (4x * sqrt(x(4AD³ + x (4A(D - x) - D )²))))
    function getPrice() external view returns (uint256) {
        uint256 x = tokenBalance;
        uint256 a8_ = a << 3;
        uint256 d_cube_ = d * d * d;

        // 4A(D - x) - D
        int256 p1 = int256(a << 2) * (int256(d) - int256(x)) - int256(d);
        // x * 4AD³ + x(p1²)
        uint256 p2 = x * ((a << 2) * d_cube_ + x * uint256(p1 * p1));
        // D³ + 8ADx² - 8Ax³ - 2Dx²
        int256 p3 = int256(d_cube_) + int256((a << 3) * d * x * x)
            - int256(a8_ * x * x * x) - int256((d << 1) * x * x);
        // 1/2 * p3 / (4x * sqrt(p2))
        return SafeCast.toUint256((PP >> 1) + (PP * p3 / int256((x << 2) * sqrt(p2))));
    }

    function _updateD() private {
        uint256 x = tokenBalance;
        uint256 y = vUsdBalance;
        // a = 8 * Axy(x+y)
        // b = 4 * xy(4A - 1) / 3
        // c = sqrt(a² + b³)
        // D = cbrt(a + c) + cbrt(a - c)
        ...
        // p1² + p2³
        uint256 p3 = sqrt((p1 * p1) + (p2 * p2 * p2));
        unchecked {
            uint256 d_ = cbrt(p1 + p3);
            if (p3 > p1) {
                d_ -= cbrt(p3 - p1);
            } else {
                d_ += cbrt(p1 - p3);
            }
            d = (d_ << 1) + 1; // +1 to offset rounding errors
        }
    }
    ...
    function setFeeShare(uint256 _feeShareBP) external onlyOwner {
