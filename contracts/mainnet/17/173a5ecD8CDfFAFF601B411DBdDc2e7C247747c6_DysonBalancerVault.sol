...
import "../../interfaces/balancer/IStrategyBalancerAC.sol";

/**
 * @dev Implementation of a vault to deposit funds for yield optimizing.
 * This is the contract that receives funds and that users interface with.
 * The yield optimizing strategy itself is implemented in a separate 'StrategyBalancer.sol' contract.
 */
contract DysonBalancerVault is Initializable, OwnableUpgradeable, ERC20Upgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // The strategy currently in use by the vault.
    IStrategyBalancerAC public strategy;
    ...
    function __DysonBalancerVault_init(string memory _name, string memory _symbol) public initializer {
        __Ownable_init_unchained();
        __ERC20_init_unchained(_name, _symbol);
        __ReentrancyGuard_init_unchained();
        __DysonBalancerVault_init_unchained();
    }

    function __DysonBalancerVault_init_unchained() internal initializer {
    }
    ...
    function balance() public view returns (uint) {
        return want().balanceOf(address(this)) + IStrategyBalancerAC(strategy).balanceOf();
    }
    ...
    /**
     * Function to update the strategy this vault is connected to
     * @param strategy_ Address of the new strategy
     */
    function setStrategy(IStrategyBalancerAC strategy_) public onlyOwner {
        require(!_isStrategyInitialized, 'strategy already initialized');
        strategy = strategy_;
    }

    function isStrategyInitialized() public view returns (bool) {
        return _isStrategyInitialized;
    }
}
