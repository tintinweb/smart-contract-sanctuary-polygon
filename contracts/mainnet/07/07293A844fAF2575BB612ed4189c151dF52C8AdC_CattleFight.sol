...
contract CattleFight is OwnableUpgradeable {
    ...
    mapping(address => mapping(uint => bool)) public isUserFought;
    mapping(address => mapping(uint => uint)) public userWinsWithoutRate;
    NewUserRefer public newRefer;
    mapping(address => mapping(uint => bool)) public userClaimedTx;
    ...
    function userFight(uint times, uint timeWithoutRate, uint txId, bytes32 r, bytes32 s, uint8 v) external freshTime {

        bytes32 hash = keccak256(abi.encodePacked(msg.sender, times, timeWithoutRate, txId));
        address a = ecrecover(hash, v, r, s);
        require(a == banker, "not banker");
        require(stable.checkUserCows(msg.sender).length > 0, 'no cows user');
        require(timeWithoutRate == 10 || timeWithoutRate == 15 || timeWithoutRate == 20, 'not right time');
        require(userWinsWithoutRate[msg.sender][lastTime] < 20, 'today is full');
        ...
        userWinsWithoutRate[msg.sender][lastTime] = timeWithoutRate;
        if (!isUserFought[msg.sender][lastTime]) {
            userInfo[msg.sender].userRound.push(lastTime);
            isUserFought[msg.sender][lastTime] = true;
        }
    ...
    function upLoadFight(uint timestamp, uint times, uint timeWithoutRate, uint txID, bytes32 r, bytes32 s, uint8 v) external freshTime {
        bytes32 hash = keccak256(abi.encodePacked(msg.sender, timestamp, times, timeWithoutRate, txID));
        address a = ecrecover(hash, v, r, s);
        require(a == banker, "not banker");
        require(stable.checkUserCows(msg.sender).length > 0, 'no cows user');
        require(timeWithoutRate == 10 || timeWithoutRate == 15 || timeWithoutRate == 20, 'not right time');
        ...
        require(!userClaimedTx[msg.sender][txID], 'already add');
        userClaimedTx[msg.sender][txID] = true;
        uint tempUserTime = userInfo[msg.sender].dailyWin[lastTime];
        dailyWins[lastTime] += times - tempUserTime;
        userInfo[msg.sender].totalWins += times - tempUserTime;
        userInfo[msg.sender].dailyWin[lastTime] += times - tempUserTime;
        userWinsWithoutRate[msg.sender][lastTime] = timeWithoutRate;
        uint tempRate = tec.checkUserTecEffet(msg.sender, 4004);
        uint costPower = times * 300;
        stable.useCattlePower(msg.sender, costPower - (costPower * tempRate / 100));
        if (!isUserFought[msg.sender][lastTime]) {
            userInfo[msg.sender].userRound.push(lastTime);
            isUserFought[msg.sender][lastTime] = true;
        }
    ...
    function getCurrentRoundInfo(address addr) public view returns (uint userWins, uint userWinsWithoutRates, uint rew, uint totalWin, uint dailyOut_, string memory symbol, uint decimals){
        uint _lastTime = block.timestamp - (block.timestamp % 86400);
        ...
        userWinsWithoutRates = userWinsWithoutRate[addr][_lastTime];
    ...
    function addUserRoundInfo(address addr, uint[] memory timestamp, uint[] memory times, uint[] memory totalWins, address[] memory tokens, uint[] memory dailyOuts) external onlyOwner {
        for (uint i = 0; i < timestamp.length; i ++) {
            ...
            if (!isUserFought[addr][timestamp[i]]) {
                userInfo[addr].userRound.push(timestamp[i]);
                isUserFought[addr][timestamp[i]] = true;
            }
    ...
    function claimReward(uint[] memory rounds) public freshTime {
        ...
        if (userInfo[msg.sender].totalClaimed > 1000 ether) {
