...
contract Swap is BaseContract {
  ...
  function initialize() public initializer {
    ...
    config.nftStakingRate = 1_000 ether;
  ...
  function swapUSDForToken(uint _usdAmount, ISwap.PaymentCurrency _paymentCurrency, bool _autoStake) external returns (uint) {
    ...
    uint tokenAmount;
    if(config.toPancake) {
      _swapUSDForTokenViaPancake(_usdAmount, _paymentCurrency);
    } else {
      tokenAmount = liquidityManager.swapUSDForToken(_usdAmount);
    }
  ...
  function swapTokenForUSDT(uint _amount, bool _lms) external {
    ...
    uint usdAmount;
    if (_lms) {
      usdAmount = liquidityManager.swapTokenForUSDT(swappedToken);
    } else if(config.toPancake) {
      usdAmount = _swapTokenForUSDTViaPancake(swappedToken);
    } else {
      usdAmount = liquidityManager.swapTokenForUSDT(swappedToken);
    }
  ...
  function updateConfig(
    ...
    config.nftStakingRate = _nftStakingRate;
    config.toPancake = _toPancake;
    config.treasury = _treasury;
    emit ConfigUpdated(_secondsInADay, _dnoPrice, _dnoUserCap, _dnoSystemCap, _nftStakingRate, _toPancake, _treasury, block.timestamp);
  }
  ...
  function _buyDNO(uint _quantity, uint _orderValue, ISwap.PaymentCurrency _paymentCurrency) private {
    ...
    IBEP20 usdToken = _loadUsdToken(_paymentCurrency);
    _takeFund(usdToken, _orderValue, config.treasury);
    nftPass.mint(msg.sender, _quantity);
    vault.depositFor(msg.sender, _quantity * config.nftStakingRate, IVault.DepositType.swapBuyDNO);
    config.dnoSold += _quantity;
  }

  function _swapTokenForUSDTViaPancake(uint _amount) private returns (uint) {
  ...
  function _swapBusdToUsdt(uint _amount) private returns (uint) {
    address[] memory path = new address[](2);
  ...
  function _swapUSDForTokenViaPancake(uint _amount, ISwap.PaymentCurrency _paymentCurrency) private returns (uint) {
    address[] memory path = new address[](2);
    if (_paymentCurrency == ISwap.PaymentCurrency.busd) {
      _amount = _swapBusdToUsdt(_amount);
    }
    path[0] = address(usdtToken);
    path[1] = address(menToken);
    uint currentMehBalance = menToken.balanceOf(address(this));
    uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(_amount, 0, path, address(this), block.timestamp);
    return menToken.balanceOf(address(this)) - currentMehBalance;
  }
  ...
  function _loadUsdToken(ISwap.PaymentCurrency _paymentCurrency) private view returns (IBEP20) {
    return _paymentCurrency == ISwap.PaymentCurrency.busd ? busdToken : usdtToken;
  }

  function _initDependentContracts() override internal {
    busdToken = IBEP20(addressBook.get("busdToken"));
    ...
    liquidityManager = ILiquidityManager(addressBook.get("lms"));
