...
contract AOWAYNFT is ERC721Enumerable, Ownable {
	...
	uint256 public whitelistPrice = 80 ether;
	uint256 public privateSalePrice = 140 ether;
	uint256 public publicSalePrice = 215 ether;
	...
	constructor(string memory _baseURI, uint256 _maxPerMint) ERC721("Sinapsi NFT", "SIN") {
	...
	function tokenURI(uint256 _tokenId) public view override returns (string memory) {
		require(_tokenId > 0 && _tokenId <= NFTId.current(), "Invalid tokenID");
	...
	function removeFromWhiteList(address[] calldata entries) external onlyOwner {
		for (uint256 i = 0; i < entries.length; i++) {
			whitelist[entries[i]] = false;
		}
	}

	function toggleWhitelistSaleLive() external onlyOwner {
	...
	function mintTo(address _to, uint256 _quantity) external onlyOwner {
		...
		require((NFTId.current() + _quantity) <= maxSupply, "Not enough NFTs left!");
	...
	function mint(uint256 quantity) external payable {
		...
		if (whitelistSaleLive && whitelist[msg.sender]) {
			require(msg.value >= (whitelistPrice * quantity), "Not enough ether to purchase NFTs");
		} else if (publicSaleLive) {
			require(msg.value >= (publicSalePrice * quantity), "Not enough ether to purchase NFTs");
		} else if (privateSaleLive) {
			require(msg.value >= (privateSalePrice * quantity), "Not enough ether to purchase NFTs");
		} else {
