 ...
 * @title MajrIDPolygon
 * @author MAJR, Inc.
 * @notice MajrCanon is the core governance contract of the MAJR DAO ecosystem
 */
contract MajrCanonPolygon is Ownable, Pausable {
  ...
  constructor(
    ...
    require(
      _weth != address(0),
      "MajrCanonPolygon:: WETH address cannot be address(0)."
    );
    require(
      _stakingContract != address(0),
      "MajrCanonPolygon: Staking contract address cannot be address(0)."
    );
    require(
      _daoTreasury != address(0),
      "MajrCanonPolygon: DAO treasury address cannot be address(0)."
    );
    require(
      _quorumVotes >= 1 && _quorumVotes <= 51,
      "MajrCanonPolygon: Quorum votes must be greater than or equal to 1 and lesser than or equal to 51."
    );
  ...
  modifier validProposalId(uint256 _proposalId) {
    require(
      proposalCount.current() > _proposalId,
      "MajrCanonPolygon: Proposal ID is out of bounds."
    );
    _;
  }
  ...
  modifier validPreSetProposalId(uint256 _proposalId) {
    require(
      preSetProposalCount.current() > _proposalId,
      "MajrCanonPolygon: Pre set proposal ID is out of bounds."
    );
    _;
  }
  ...
  function vote(
    uint256 _proposalId,
    bool _support
  ) external whenNotPaused validProposalId(_proposalId) {
    Proposal storage proposal = proposals[_proposalId];
    require(
      proposal.state == ProposalState.Active,
      "MajrCanonPolygon: Voting is not active for this proposal."
    );

    Receipt storage receipt = receipts[_proposalId][msg.sender];
    require(
      receipt.hasVoted == false,
      "MajrCanonPolygon: Voter already voted on this proposal."
    );
   ...
   */
  function votePreSet(
    uint256 _preSetProposalId,
    uint256 _option
  ) external whenNotPaused validPreSetProposalId(_preSetProposalId) {
    PreSetProposal storage preSetProposal = preSetProposals[_preSetProposalId];
    require(
      preSetProposal.state == PreSetProposalState.Active,
      "MajrCanonPolygon: Voting is not active for this pre set proposal."
    );
    ...
    require(
      _option == 1 ||
        ...
        _option == 4 ||
        _option == 5,
      "MajrCanonPolygon: Invalid option."
    );

    PreSetReceipt storage preSetReceipt = preSetReceipts[_preSetProposalId][
      msg.sender
    ];
    require(
      preSetReceipt.hasVoted == false,
      "MajrCanonPolygon: Voter already voted on this pre set proposal."
    );
  ...
  function getReceipt(
    uint256 _proposalId,
    address _voter
  ) public view validProposalId(_proposalId) returns (Receipt memory) {
   ...
   */
  function getPreSetReceipt(
    uint256 _preSetProposalId,
    address _voter
  )
    public
    view
    validPreSetProposalId(_preSetProposalId)
    returns (PreSetReceipt memory)
  {
  ...
  function propose(
    ...
    string calldata _longDescriptionURI
  ) external whenNotPaused {
    if (msg.sender != owner()) {
      require(
        stakingContract.balanceOf(msg.sender) >= proposalThreshold,
        "MajrCanonPolygon: Insufficient number of staked MAJR IDs to add a proposal."
      );
      require(
        weth.balanceOf(msg.sender) >= proposalCost,
        "MajrCanonPolygon:: Insufficient WETH balance to add a proposal."
      );

      bool approved = weth.approve(address(this), proposalCost);
      require(
        approved,
        "Splitter: Couldn't approve tokens for spending by the contract."
      );

      bool success = weth.transferFrom(msg.sender, daoTreasury, proposalCost);
      require(success, "MajrCanonPolygon:: WETH transfer failed.");
    }

    uint256 currentProposalCount = proposalCount.current();
    ProposalState _proposalState = msg.sender == owner()
      ? ProposalState.Active
      : ProposalState.Pending;
    uint256 _proposalQuorum = uint256(
      (stakingContract.totalSupply() / 100) * quorumVotes
    );
  ...
  function proposePreSet(
    ...
    uint256 currentPreSetProposalCount = preSetProposalCount.current();
    ...
    emit PreSetProposalCreated(
      currentPreSetProposalCount,
      _title,
      block.timestamp,
      msg.sender,
      PreSetProposalState.Active
    );
  }
  ...
  function approveProposal(
    uint256 _proposalId
  ) external onlyOwner validProposalId(_proposalId) {
    Proposal storage proposal = proposals[_proposalId];

    require(
      proposal.state == ProposalState.Pending,
      "MajrCanonPolygon: Proposal must be in Pending state to be approved."
    );
    proposal.state = ProposalState.Active;
  ...
  function queueProposal(
    ...
    require(
      proposal.state == ProposalState.Active,
      "MajrCanonPolygon: Proposal must be in Active state to be queued."
    );
    require(
      !queuedProposals[_proposalId],
      "MajrCanonPolygon: Proposal must not be queued already."
    );

    uint256 totalVotes = proposal.forVotes + proposal.againstVotes;
    require(
      totalVotes >= proposal.proposalQuorum,
      "MajrCanonPolygon: Proposal must have reached the quorum to be queued for execution."
    );
   ...
   * @dev Only guardian council can call it
   */
  function queuePreSetProposal(
    uint256 _preSetProposalId
  ) external onlyOwner validPreSetProposalId(_preSetProposalId) {
    PreSetProposal storage preSetProposal = preSetProposals[_preSetProposalId];

    require(
      preSetProposal.state == PreSetProposalState.Active,
      "MajrCanonPolygon: PreSetProposal must be in Active state to be queued."
    );
    require(
      !queuedPreSetProposals[_preSetProposalId],
      "MajrCanonPolygon: PreSetProposal must not be queued already."
    );
  ...
  function executeProposal(
    ...
    require(
      proposal.state == ProposalState.Queued,
      "MajrCanonPolygon: Proposal must be in Queued state to be executed."
    );
    require(
      queuedProposals[_proposalId],
      "MajrCanonPolygon: Proposal must be queued to be executed."
    );
   ...
   * @dev Only guardian council can call it
   */
  function executePreSetProposal(
    uint256 _preSetProposalId
  ) external onlyOwner validPreSetProposalId(_preSetProposalId) {
    PreSetProposal storage preSetProposal = preSetProposals[_preSetProposalId];
    ...
    require(
      preSetProposal.state == PreSetProposalState.Queued,
      "MajrCanonPolygon: PreSetProposal must be in Queued state to be executed."
    );
    require(
      queuedPreSetProposals[_preSetProposalId],
      "MajrCanonPolygon: PreSetProposal must be queued to be executed."
    );
  ...
  function cancelProposal(
    ...
    require(
      proposal.state == ProposalState.Pending ||
        proposal.state == ProposalState.Active,
      "MajrCanonPolygon: Proposal must be in Pending or Active state to be canceled."
    );
  ...
  function cancelPreSetProposal(
    ...
    require(
      preSetProposal.state == PreSetProposalState.Active,
      "MajrCanonPolygon: PreSetProposal must be in Active state to be canceled."
    );
  ...
  /**
   * @notice Returns the voting power of the specified voter (i.e. their balance in the staking contract)
   * @param _account address
   * @return uint256
   */
  function getVotingPower(address _account) public view returns (uint256) {
    return stakingContract.balanceOf(_account);
  }
  ...
  function setStakingContract(address _stakingContract) external onlyOwner {
    require(
      _stakingContract != address(0),
      "MajrCanonPolygon: Staking contract address cannot be address zero."
    );
  ...
  function setDaoTreasury(address _daoTreasury) external onlyOwner {
    require(
      _daoTreasury != address(0),
      "MajrCanonPolygon: DAO treasury address cannot be address zero."
    );
   ...
   * @param _quorumVotes uint256
   * @dev Only guardian council can call it
   * @dev Must be greater than or equal to 1 and lesser than or equal to 51
   */
