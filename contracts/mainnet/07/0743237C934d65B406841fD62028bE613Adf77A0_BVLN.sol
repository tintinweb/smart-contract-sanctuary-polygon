...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function transferFrom(
        ...
        require(currentAllowance >= amount, "BVLN: transfer amount exceeds allowance");
        unchecked {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        ...
        require(currentAllowance >= subtractedValue, "BVLN: decreased allowance below zero");
        unchecked {
    ...
    function _transfer(
        ...
        require(sender != address(0), "BVLN: transfer from the zero address");
        require(recipient != address(0), "BVLN: transfer to the zero address");

        _beforeTokenTransfer(sender, recipient, amount);

        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "BVLN: transfer amount exceeds balance");
        unchecked {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        require(account != address(0), "BVLN: mint to the zero address");
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "BVLN: burn from the zero address");

        _beforeTokenTransfer(account, address(0), amount);

        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "BVLN: burn amount exceeds balance");
        unchecked {
    ...
    function _approve(
        ...
        require(owner != address(0), "BVLN: approve from the zero address");
        require(spender != address(0), "BVLN: approve to the zero address");
...
contract BVLN is ERC20, AccessControl, ERC721Holder {
    ...
    constructor(address owner, address _collectionAddress) ERC20("BevalinDAO", "BVLN") AccessControl(){
        _mint(owner, 20 * 10 ** 18);
        ...
        coinAmountPerRewardUnit = 5 * 10 ** 18; // 5 ERC20 coins per rewardUnit, may be changed later on
        numberOfBlocksPerRewardUnit = 20571; // 12 hours per reward unit , may be changed later on
        welcomeBonusAmount = 10 * 10 ** 18; // 10 tokens welcome bonus, only paid once per tokenId
    }
    ...
    function stake(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddress).ownerOf(tokenId) == address(this),
            "BVLN: Error while transferring token");
    ...
    function setCollectionAddress(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "BVLN: update to zero address not possible");
