...
contract ElegantEngine is Ownable, Concatenate {
    ...
    function setHalves(uint _first, uint _second) public {
        require(isTeamMember[msg.sender] == true, "Caller is not a member of the team.");
        firstHalf = _first;
        secondHalf = _second;
    }
    ...
    function returnSteps(uint first, uint second) public view returns(uint){
        ...
        _rem = firstHalf.mod(first);
        if (_rem > 0){_rem = 1;}
                
        _steps = firstHalf.div(first) + _rem;
        _rem = secondHalf.mod(second);
        if (_rem > 0){_rem = 1;}
                
        _steps = _steps + secondHalf.div(second) + _rem;
    ...
    function getSpeedBoost(uint _id, bool _ext) public view returns(uint) {
        ...
        uint j = 31;
        for (j; j <= 36; j++ ){
            bool isBoosted = _rules.tokenCheck(_tokenID,j,_con);
            if (isBoosted == true){
                if (j == 31 || j == 33 || j == 34){
                    _boost += 1;
                } else if (j == 32){
