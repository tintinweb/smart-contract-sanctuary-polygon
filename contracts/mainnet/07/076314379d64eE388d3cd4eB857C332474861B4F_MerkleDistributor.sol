...
contract MerkleDistributor is AccessControl,ReentrancyGuard {
    ...
    event Claimed(uint256 indexed roundID, address indexed sender, uint256 indexed index);

    event freeClaimed(uint256 indexed roundID, address indexed sender, uint256 indexed index, uint256 amount);
    ...
    // Setting
    function launchpad(uint256 _roundID, address _target, bytes32 _merkleRoot, bytes32 _vipMerkleRoot, address payable _receipt, address _payment, uint256 _price, uint256 _startTime, uint256 _endTime) public onlyRole(CREATE_ROLE) {
    ...
    // anyone can pay
    function claim(uint256 roundID, uint256 index, uint256 num, bytes calldata calldataABI, bytes32[] calldata merkleProof) public payable nonReentrant{
        ...
        bytes32 node = keccak256(abi.encodePacked(roundID, index, num, calldataABI, msg.sender));
     ...
     * batchClaim
     */
    function batchClaim(uint256 roundID, uint256 amount, ClaimData[] calldata claimdata) public payable nonReentrant{
        Project storage project = round[roundID];
        ...
        // Receipt token && Refund token
        if (project.payment == address(0)) {
            require(msg.value >= amount, "You have to pay enough eth.");
            uint256 refund = msg.value - totalAmount;
            if (refund > 0) payable(msg.sender).transfer(refund);
            project.receipt.transfer(totalAmount);
        } else {
            require(msg.value == 0, "You don't need to pay eth");
            IERC20(project.payment).safeTransferFrom(msg.sender, project.receipt, totalAmount);
        }
        // batch mint
        for (uint256 j = 0; j < claimdata.length; j++) {
            ...
            // Verify the merkle proof.
            bytes32 node = keccak256(abi.encodePacked(roundID, claimdata[j].index, claimdata[j].num, claimdata[j].calldataABI, msg.sender));
            if (!MerkleProof.verify(claimdata[j].merkleProof, project.merkleRoot, node)) revert InvalidProof();
            project.bitmap.set(claimdata[j].index);

            // execute
            // TODO unsafe : Non-Detected contract call return value
            project.target.functionCall(claimdata[j].calldataABI, "MerkleDistributor freeMint: Call ABI failed.");

            emit Claimed(roundID, msg.sender, claimdata[j].index);
        }
    }
     ...
     * freeBatchMint
     */
    function freeBatchMint(uint256 roundID, ClaimData[] calldata claimdata) public nonReentrant{
        Project storage project = round[roundID];
        // Verify time
        require(project.startTime <= block.timestamp, "The Launchpad activity not started");
        require(project.endTime >= block.timestamp, "The Launchpad activity has ended");
        ...
        for (uint256 j = 0; j < claimdata.length; j++) {
            if (project.vipBitmap.get(claimdata[j].index)) revert AlreadyClaimed();
            // Verify the merkle proof.
            bytes32 node = keccak256(abi.encodePacked(roundID, claimdata[j].index, claimdata[j].num, claimdata[j].calldataABI, msg.sender));
            if (!MerkleProof.verify(claimdata[j].merkleProof, project.vipMerkleRoot, node)) revert InvalidProof();

            project.vipBitmap.set(claimdata[j].index);
            
            // execute
            // TODO unsafe : Non-Detected contract call return value
            project.target.functionCall(claimdata[j].calldataABI, "MerkleDistributor batchMint: Call ABI failed.");

            emit freeClaimed(roundID, msg.sender, claimdata[j].index, claimdata[j].num);
        }
    }
    ...
    // Returns project details by this round.
    function getProjectV1(uint256 roundID) external view returns (address, address, bytes32, bytes32, address, uint256, uint256, uint256){
        Project storage project = round[roundID];
        return (project.target, project.receipt, project.merkleRoot, project.vipMerkleRoot, project.payment, project.price, project.startTime, project.endTime);
    }
    ...
    // Returns true if the index has been marked claimed by this round.
    function isClaimedVip(uint256 roundID, uint256 index) external view returns (bool) {
        Project storage project = round[roundID];
