...
contract Camelit is ICamelit, ERC721Enumerable, Ownable, Pausable, VRFConsumerBase {
  mapping (uint256 => uint256) private mintedAt;
  //VRF setup NOTE Polygon : khash 0xf86195cf7690c55907b2b611ebb7343a6f649bff128701cc542f0569e2c549da fee : 0.0001, coordinator 0x3d2341ADb2D31f1c5530cDC622016af293177AE0, link 0xb0897686c545045aFc77CF20eC7A532E3120E0F1
  bytes32 public immutable keyHash = 0xf86195cf7690c55907b2b611ebb7343a6f649bff128701cc542f0569e2c549da;
...
mapping(address => uint16[]) tokensOfOwner;  
// Addresses for withdraw function (dev = me)
  address public immutable devWallet = 0xb9110CE83EaF19ce6722e5970c2f2A25860b1d61;
  address public immutable ownerWallet = 0x1a64A5E2FC17094d033012E646D58ec407C04AbF;
  ...
  // To keep track of pending mints because of VRF delay
  struct Minting {
        address minter;
        uint256 tokenId;
        bool fulfilled;
  ...
  constructor(address _gold, address _traits, uint256 _maxTokens) ERC721("Desert Clash", 'Desert Clash') VRFConsumerBase(0x3d2341ADb2D31f1c5530cDC622016af293177AE0, 0xb0897686c545045aFc77CF20eC7A532E3120E0F1) { 
    gold = GOLD(_gold);
  ...
  function mint(uint256 amount, bool stake) external payable whenNotPaused {
    ...
    require(LINK.balanceOf(address(this)) >= fee, "Not enough LINK in the contract, ask the team to refill");
    ...
    mintings[requestId] = Minting(msg.sender, minted, false, stake);
    }

  }

  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    Minting storage current = mintings[requestId];
    uint256 seed = randomness;
    address origin = current.minter; 
    generate(current.tokenId, seed);   
    address recipient = selectRecipient(origin, seed);
    if (current.stake == false || recipient != origin) {
      _safeMint(recipient, current.tokenId);
    } else {
      _safeMint(address(pool), current.tokenId);
    }
    _minted[origin]++;
    tokenMinted(current.tokenId);  
    if (current.stake) {
      uint16[] memory tokenIds = new uint16[](1); 
      tokenIds[0] = minted;
      pool.addManyToPool(origin, tokenIds);
    }
  }
  function _safeMint(address to, uint256 tokenId) internal virtual override {
    addToMapping(to, tokenId);
  ...
  function _transfer(address from, address to, uint256 tokenId) internal virtual override {
    deleteFromMapping(from, tokenId);
    addToMapping(to, tokenId);
    super._transfer(from, to, tokenId);
  }
  function deleteFromMapping(address account, uint256 tokenId) internal {
    uint len = tokensOfOwner[account].length;
    uint pos;
    for (uint i; i < len; i++) {
      if(tokensOfOwner[account][i] == tokenId) {
        pos = i; 
        break;
      }
      else { continue; }
    }
    tokensOfOwner[account][pos] = tokensOfOwner[account][len - 1];
    tokensOfOwner[account].pop();
  }
  
  function addToMapping(address account, uint256 tokenId) internal {
    tokensOfOwner[account].push(uint16(tokenId));
  }
  ...
  * @dev a 'for' loop is okay since we're in memory, and it won't loop more than 5 times anyways
  ...
  function getPrice(uint256 amount, uint256 firstTokenID) internal pure returns(uint256) {
    uint256 totalPrice;
    for(uint i; i < amount; i++) {
      totalPrice += ((((firstTokenID + i - 5000) / 200) * 20) + 150) * 10 ** 18;
    }
    return totalPrice;
  }
  /**
  ...
  function selectTrait(uint16 seed, uint8 traitType) internal view returns (uint8) {
    ...
    uint8 trait = (uint8(seed) % (traitProbabilities[traitType].numTraits)) + 1;
    return trait;
  }
   ...
   */
  function selectRecipient(address origin, uint256 seed) internal view returns (address) {
    if (minted <= paidTokens || ((seed >> 245) % 10) != 0) return origin; // top 10 bits haven't been used
    address thief = pool.randomBanditOwner(seed >> 160); // 160 bits reserved for trait selection
    if (thief == address(0x0)) return origin;
    return thief;
  }
  ...
  function camelsOfOwner(address account) external view returns(uint16[] memory tokenIds) {
    uint len;
    uint16[] memory temp = new uint16[](tokensOfOwner[account].length);
    for (uint i; i < tokensOfOwner[account].length; i++) {
      if(tokenTraits[tokensOfOwner[account][i]].isCamel == true) {
        temp[len] = tokensOfOwner[account][i];
        len++;
      }
    }
    uint16[] memory cleanOutput = new uint16[](len);
    for(uint x; x < cleanOutput.length; x++) {
      cleanOutput[x] = temp[x];
    }
    return cleanOutput;
  }
  
  function banditsOfOwner(address account) external view returns(uint16[] memory) {
    uint len;
    uint16[] memory temp = new uint16[](tokensOfOwner[account].length);
    for (uint i; i < tokensOfOwner[account].length; i++) {
      if(tokenTraits[tokensOfOwner[account][i]].isCamel == false) {
        temp[len] = tokensOfOwner[account][i];
        len++;
      }
    }
