...
contract TGXen is ERC20, IERC165, IBurnRedeemable, Ownable { 
    ...
    function burnXEN(uint256 amount) external {
       require(balanceOf(address(this)) >= totalTGXenUnclaimed.add(amount.mul(2)), "TGXen: Supply not available to mint");
    ...
    function onTokenBurned(address user, uint256 amount) public {
	   ...
	   else
	   {
		   ...
		   totalTGXenUnclaimed += amount.mul(2); 
	   }
	   totalXENBurned += amount;
	   totalTGXenMinted += amount.mul(2); 
       emit Redeemed(address(user), address(XEN_ADDRESS), address(this), amount, amount.mul(2));
    }
	...
	function claim(uint256 targetDay) external {
	    ...
	    _transfer(address(this), address(_msgSender()), mapTGXenClaim[_msgSender()][targetDay].amount, false);
