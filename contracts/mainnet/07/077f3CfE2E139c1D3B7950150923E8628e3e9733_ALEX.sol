...
contract ALEX is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, OwnableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable {
...
///////////////////////////////////////////////////////////////////////////////
    function initialize(
        ...
        uniswapV2Caller=IUniswapV2Caller(_accounts[3]);
        baseTokenForPair=_accounts[2];
        require(_accounts[0]!=address(0), "Tax Wallet address can not be 0");
        require(_fees[0]+_fees[2]+_fees[4]<=300, "sell fee <= 30%");
        require(_fees[1]+_fees[3]+_fees[5]<=300, "buy fee <= 30%");

        taxWallet=_accounts[0];
    ...
    function updateLiquidityFee(
        ...
        require(
            _sellLiquidityFee + sellTaxFee + sellBurnFee <= 300,
            "sell fee <= 30%"
        );
        require(_buyLiquidityFee + buyTaxFee + buyBurnFee <= 300, "buy fee <= 30%");
    ...
    function updateTaxFee(
        ...
        require(
            _sellTaxFee + sellLiquidityFee + sellBurnFee <= 300,
            "sell fee <= 30%"
        );
        require(_buyTaxFee + buyLiquidityFee + buyBurnFee <= 300, "buy fee <= 30%");
        emit UpdateTaxFee(
            _sellTaxFee,
            _buyTaxFee,
            sellTaxFee,
    ...
    function updateBurnFee(
        ...
        require(
            sellTaxFee + sellLiquidityFee + _sellBurnFee <= 300,
            "sell fee <= 30%"
        );
        require(buyTaxFee + buyLiquidityFee + _buyBurnFee <= 300, "buy fee <= 30%");
        emit UpdateTaxFee(
            _sellBurnFee,
            _buyBurnFee,
            sellBurnFee,
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 _feeTotal = _liquidityFee + _taxFee;
            if (_feeTotal > 0) super._transfer(from, address(this), _feeTotal);
            amount = amount - _feeTotal - _burnFee;
    ...
    function takeFee() private lockTheSwap {
        ...
        uint256 totalTokensTaken = _liquidityFeeTokens + _taxFeeTokens;
