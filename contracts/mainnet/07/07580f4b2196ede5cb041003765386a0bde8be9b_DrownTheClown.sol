...
'---'      `---'            `--`   `---`                                                   `--`----'     `---`    '---'                   '---`"
            ...
            v1.2.6
            @author Krypto Sucks!
            @title TheDrunkenWizard
            @url https://drunkenwizard.nftarca.de
...
// File: contracts/TheDrunkenWizard.sol
...
contract DrownTheClown {
    ...
    function pickWinner(uint newCostPerPlayer,uint newTotalPlayerLimit, uint256[3] calldata purses, uint newSideBetCost) public onlyOwner {
    ...
    function adminUpdateGame(uint newCostPerPlayer,uint newTotalPlayerLimit, uint256[3] calldata purses, uint newSideBetCost) public onlyOwner {
        ...
        require(newTotalPlayerLimit >= players.length, "Player count must be greater than or equal to current amount of players registered");
    ...
    function nukeItFromOrbit(bool failSafe) public onlyOwner {
        require((failSafe == true), "ah ah ah, you didn't say the magic word!");
        // Transfer any GP, GBar and MATIC out of contract in case we need to close and relaunch a new contract
