...
contract Unitest is ERC20, AccessControl, ERC721Holder {
	...
	address public collectionAddressC5;
	...
	uint256 public numberOfBlocksPerRewardUnitC5;
	...
	uint256 public coinAmountPerRewardUnitC5;
	...
	uint256 public stakingStartBlockC5;
	...
	mapping (address => mapping(uint256 => StakeInfoC5)) public stakeLogC5;
	...
	mapping (address => uint256) public tokensStakedByUserC5;
	...
	mapping (address => uint256[]) public stakePortfolioByUserC5;
	...
	mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioC5;
	...
	event RewardsHarvestedC5 (address owner, uint256 amount);
	...
	event NFTStakedC5 (address owner, uint256 tokenId);
	...
	event NFTUnstakedC5 (address owner, uint256 tokenId);

    constructor() ERC20("Unitest", "Unitest")AccessControl(){
		...
		collectionAddressC5 = 0x0000000000000000000000000000000000000000;
		...
		numberOfBlocksPerRewardUnitC5 = 3600;
    }
    ...
    function stakeC1(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddressC1).ownerOf(tokenId) == address(this),
            "Unitest: Error while transferring token");
	...
	function stakeC5(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        IERC721(collectionAddressC5).safeTransferFrom(_msgSender(), address(this), tokenId);
        require(IERC721(collectionAddressC5).ownerOf(tokenId) == address(this),
            "Unitest: Error while transferring token");
        StakeInfoC5 storage infoC5 = stakeLogC5[_msgSender()][tokenId];
        infoC5.stakedAtBlock = block.number;
        infoC5.lastHarvestBlock = block.number;
        infoC5.currentlyStaked = true;
        if(tokensStakedByUserC5[_msgSender()] == 0){
            amountOfStakersC5 += 1;
        }
        tokensStakedByUserC5[_msgSender()] += 1;
        tokensStakedC5 += 1;
        stakePortfolioByUserC5[_msgSender()].push(tokenId);
        uint256 indexOfNewElementC5 = stakePortfolioByUserC5[_msgSender()].length - 1;
        indexOfTokenIdInStakePortfolioC5[tokenId] = indexOfNewElementC5;
    ...
    function stakeBatchC5(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
	...
	function setNumberOfBlocksPerRewardUnitC5(uint256 numberOfBlocksC5) external onlyRole(CONTRACT_ADMIN_ROLE){
        numberOfBlocksPerRewardUnitC5 = numberOfBlocksC5;
    }
	...
	function setCoinAmountPerRewardUnitC5(uint256 coinAmountC5) external onlyRole(CONTRACT_ADMIN_ROLE){
        coinAmountPerRewardUnitC5 = coinAmountC5;
    }

    function setCollectionAddressC1(address newAddressC1) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC1 != address(0), "Unitest: update to zero address not possible");
    ...
    function setCollectionAddressC2(address newAddressC2) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC2 != address(0), "Unitest: update to zero address not possible");
    ...
    function setCollectionAddressC3(address newAddressC3) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC3 != address(0), "Unitest: update to zero address not possible");
	...
	function setCollectionAddressC4(address newAddressC4) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC4 != address(0), "Unitest: update to zero address not possible");
        collectionAddressC4 = newAddressC4;
    }
	
	function setCollectionAddressC5(address newAddressC5) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC5 != address(0), "Unitest: update to zero address not possible");
        collectionAddressC5 = newAddressC5;
    }
	...
	function setStakingStartBlockC5(uint256 newStakingStartBlockC5) external onlyRole(CONTRACT_ADMIN_ROLE){
        stakingStartBlockC5 = newStakingStartBlockC5;
    }
}