...
contract STUDIOMARKET is ReentrancyGuard, ERC721URIStorage, Ownable {
  ...
  function takeTokenCommission(
    ...
    //validate listing fee value
    require(commissionPercentage <= 500, 'Value Overflow: Steted Value Is Above 50 percent');

    // divide by 1000 because commission percentage is expressed as a uint * 10
    uint256 royaltyFee = (tokens * royaltyPercent) / 1000;
    tokens -= royaltyFee;

    //distribute tokens if takefess is true
    if (_takeFees == true) {
  ...
  function mintTokenTXS(
    ...
    takeTokenCommission(seller, owner(), address(this), tokens, listingPercentage, royalty, tokenContract);

    emit MarketItemCreated(newTokenId, seller, address(this), msg.sender, price, royalty, false);
  ...
  function mintToken(
    ...
    takeCommission(seller, msg.sender, owner(), price, listingPercentage, royalty);

    emit MarketItemCreated(newTokenId, seller, msg.sender, msg.sender, price, royalty, false);
  ...
  function buyTokenTXS(uint256 tokenId, address tokenContract) public nonReentrant {
    ...
    //finish transaction and transfer token
    takeTokenCommission(seller, owner(), address(this), tokens, listingPercentage, royaltyFee, tokenContract);
  }
