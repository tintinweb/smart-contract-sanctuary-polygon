...
contract Flashloan {  
  ...
  bytes arbdata;
  enum Direction { QuicktoSushi,SushitoQuick  } 
  struct ArbInfo {
        Direction direction;
        uint repayAmount;
          }
  ...
  constructor(address _sushiFactory, address _quickRouter, address _sushiRouter02 ) public {
    sushiFactory =IUniswapV2Factory(_sushiFactory);  
    quickRouter = IUniswapV2Router02(_quickRouter);
    sushiRouter = IUniswapV2Router02(_sushiRouter02);
  ...
  function uniswapV2Call(
    ...
    require(
      msg.sender == UniswapV2Library.pairFor(SushiFactory, token0, token1), 
      'Unauthorized'
    ); 
    ...
    if(arbInfo.direction == Direction.QuicktoSushi) {
          //Buy WBNB on Bakery
          token.approve(address(quickRouter), amountToken); 
          address[] memory path = new address[](2);
          path[0] = address(token0);
          path[1] = address(token1);
          uint[] memory minOuts = quickRouter.getAmountsOut(amountToken, path); 
          quickRouter.swapExactTokensForETH(
          ...
          uint[] memory minOuts2 = sushiRouter.getAmountsOut(address(this).balance, path2); 
        ...
        //Buy WBNB on Bakery
          ...
          path[0] = address(token0);
          path[1] = address(token1);
          uint[] memory minOuts = sushiRouter.getAmountsOut(amountToken, path); 
          ...
          path2[0] = address(token1);
          path2[1] = address(token0);
          uint[] memory minOuts2 = quickRouter.getAmountsOut(address(this).balance, path2); 
          quickRouter.swapExactETHForTokens.value(address(this).balance)(
            minOuts2[1], 
            path2, 
            address(this), 
            now
