...
contract OscilloExchange is OwnableUpgradeable {
    ...
    event Executed(uint indexed askId, uint indexed bidId, uint askTx, uint bidTx);
    ...
    function acceptance(LibTrade.Execution[] memory chunk, uint gasprice) public view returns (LibTrade.Acceptance[] memory) {
        ...
        for (uint i = 0; i < chunk.length; i++) {
            ...
            require(exec.ask.amount >= filledAsk + exec.amount, "!ask.amount");
            require(exec.bid.amount >= filledBid + exec.amount, "!bid.amount");

            (uint reserveAsk, uint reserveBid) = _reservesOf(exec.ask.account, exec.bid.account, exec.amount, exec.reserve);
            uint txCost = _txCostOf(exec.base, GAS_EXPECTATION, gasprice);
            (uint askTx, uint bidTx) = (filledAsk == 0 ? txCost : 0, filledBid == 0 ? txCost : 0);
            require(reserveAsk + askTx <= exec.amount && reserveBid + bidTx <= exec.amount, "!txCost");

            uint[2] memory askTransfers = [exec.amount, _amountQ(exec.base, exec.quote, exec.amount - reserveAsk - askTx, exec.price)];
            uint[2] memory bidTransfers = [_amountQ(exec.base, exec.quote, exec.amount, exec.price), exec.amount - reserveBid - bidTx];
    ...
    function execute(LibTrade.Execution[] memory chunk) external onlyWhitelisted {
        ...
        for (uint i = 0; i < chunk.length; i++) {
            ...
            (uint reserveAsk, uint reserveBid) = _reservesOf(exec.ask.account, exec.bid.account, exec.amount, exec.reserve);
            uint txCost = _txCostOf(exec.base, gasUsed - gasleft(), tx.gasprice);
            (uint askTx, uint bidTx) = (filledAsk == 0 ? txCost : 0, filledBid == 0 ? txCost : 0);

            IERC20(exec.quote).safeTransfer(exec.ask.account, _amountQ(exec.base, exec.quote, exec.amount - reserveAsk - askTx, exec.price));
            IERC20(exec.base).safeTransfer(exec.bid.account, exec.amount - reserveBid - bidTx);
            emit Executed(exec.askId, exec.bidId, askTx, bidTx);
        }
    }
    ...
    function _txCostOf(address base, uint gasUsed, uint gasprice) private view returns (uint) {
        (,int _priceBase,,,) = AggregatorV3Interface(_oracles[base]).latestRoundData();
        (,int _priceMatic,,,) = AggregatorV3Interface(MATIC_ETH_FEED).latestRoundData();
        return gasprice * gasUsed * uint(_priceMatic) / uint(_priceBase) / (10 ** (18 - IERC20Metadata(base).decimals()));
    }
