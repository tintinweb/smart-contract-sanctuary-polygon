...
contract PriceOracle {
    ...
    function priceOfMATIC() public view returns (uint256) {
        address token = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174; // 6 decimals
        address LP = IUniswapV2Factory(router.factory()).getPair(token, router.WETH());
        uint256 amt0 = IERC20(token).balanceOf(LP); //6
        uint256 amt1 = IERC20(router.WETH()).balanceOf(LP); //18
        return ( (amt0 * 10**12) * 10**18 / amt1); // leftover 18 decimals
    }
