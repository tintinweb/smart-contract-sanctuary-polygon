...
interface IVeSwapper {

  function lockHarvestAmount(uint256 _amount) external;
}
...
contract StrategyCommonSatinWrapperStakerLP is StrategyCommonSolidlyStakerLP {
  using SafeERC20Upgradeable for IERC20Upgradeable;
  address public requiredToken;
  address public cashToken;
  address public spSatinToken;
  ...
  function addLiquidity() internal override {
    swapToRequiredToken();
    uint256 outputBal = IERC20Upgradeable(requiredToken).balanceOf(address(this));
    uint256 lp0Amt = outputBal / 2;
    uint256 lp1Amt = outputBal - lp0Amt;

    (uint256 peg, ) = ISolidlyRouter(dystRouter).getAmountOut(lp0Amt, requiredToken, gaugeStaker);

    if (lp0Amt > peg) {
      IGaugeStaker(gaugeStaker).deposit(lp0Amt);
    } else {
      IERC20Upgradeable(requiredToken).safeApprove(dystRouter, 0);
      IERC20Upgradeable(requiredToken).safeApprove(dystRouter, lp0Amt);
      ISolidlyRouter(dystRouter).swapExactTokensForTokens(
        lp0Amt,
        0,
        outputToSpSatinRoute,
        address(this),
        block.timestamp
      );
    }

    uint256 lp0Bal = IERC20Upgradeable(gaugeStaker).balanceOf(address(this));
    uint256 lp1Bal = IERC20Upgradeable(requiredToken).balanceOf(address(this));

    IERC20Upgradeable(gaugeStaker).safeApprove(dystRouter, 0);
    IERC20Upgradeable(gaugeStaker).safeApprove(dystRouter, lp0Bal);
    IERC20Upgradeable(requiredToken).safeApprove(dystRouter, 0);
    IERC20Upgradeable(requiredToken).safeApprove(dystRouter, lp1Bal);
  ...
  function setRequiredToken(
    ...
    require(requiredToken == address(0), "requiredToken already set");
    require(cashToken == address(0), "_cashToken already set");
  ...
  function setSpSatinRoute(ISolidlyRouter.Routes[] memory _outputToSpSatinRoute) external onlyOwner {
    delete outputToSpSatinRoute;
    // delete existing values
    for (uint256 i = 0; i < _outputToSpSatinRoute.length; ++i) {
      outputToSpSatinRoute.push(_outputToSpSatinRoute[i]);
      // add new values
    }
  }
  ...
  function swapToRequiredToken() internal {
    ...
    uint256 out0 = lp0Amt;
    uint256 out1 = (ISolidlyRouter(dystRouter).getAmountsOut(lp1Amt, outputToCashRoute)[outputToCashRoute.length] *
      1e18) / lp1Decimals;
    ...
    IERC20Upgradeable(cashToken).safeApprove(dystRouter, 0);
    IERC20Upgradeable(cashToken).safeApprove(dystRouter, lp1Bal);
    ISolidlyRouter(dystRouter).addLiquidity(
      cashToken,
      output,
      false,
...
import "../../../interfaces/common/IVeSwapper.sol";
...
contract StrategyCommonSolidlyStakerLP is StratManagerUpgradeable, DynamicFeeManager {
  ...
  address public veSwapper;
  address public satinLPToken;
  ...
  function initialize(
    ...
    veSwapper = _veSwapper;
    satinLPToken = _satinLPToken;
  ...
  function chargeFees(address callFeeRecipient) internal {
    ...
    if (strategistFeeAmount > 0) {
     ...
     IVeSwapper(veSwapper).lockHarvestAmount(lockAmount);
     lockAmount = IERC20Upgradeable(satinLPToken).balanceOf(address(this));
  ...
  function _giveAllowances() internal {
    ...
    IERC20Upgradeable(satinLPToken).safeApprove(gaugeStaker, type(uint256).max);
    IERC20Upgradeable(output).safeApprove(veSwapper, type(uint256).max);
  }
