...
// File: IDGLightBridge.sol

interface IDGLightBridge {
    function goLight(uint256 _classicAmountToDeposit) external;
}
...
// File: OptimizerDG.sol

contract OptimizerDG is OwnableUpgradeable {
    using SafeERC20 for IERC20;

    address public constant GHST = address(0x385Eeac5cB85A38A9a07A70c73e0a3271CfB54A7);
    address public constant DG_OLD = address(0x2a93172c8DCCbfBC60a39d56183B7279a2F647b4); // classic dg token
    address public constant DG = address(0xef938b6da8576a896f6E0321ef80996F4890f9c4); // light dg token

    /// @notice Route for swapping DG token to GHST token.
    ...
    function optimize(
        ...
        shares[2] = _newDgShare;

        // Withdraw from strategy
        uint256 dgOnLockWithdraw = IERC20(DG_OLD).balanceOf(_dgStrat);
        IStrategyPlugin(_dgStrat).withdraw(dgOnLockWithdraw);

        //Return old DG from accumulation to cluster
        address[] memory underlyings = IClusterTokenV1(_asset).getUnderlyings();
        uint256[] memory returnDGfromAccumulation = new uint256[](underlyings.length);
        returnDGfromAccumulation[2] = IERC20(DG_OLD).balanceOf(address(this));
        IClusterTokenV1(_asset).returnDebtFromAccumulation(returnDGfromAccumulation, 0);
        
        // Optimize DG proportion
        IClusterTokenV1(_asset).optimizeProportion(shares);
        
        // Now we have old (classic) DG
        IERC20(DG_OLD).safeApprove(_dgLightBridge, IERC20(DG_OLD).balanceOf(address(this)));

        // Swap classic DG to light DG
        uint256 swapToDG = IERC20(DG_OLD).balanceOf(address(this));
        IDGLightBridge(_dgLightBridge).goLight(swapToDG);

        // Swap light DG to GHST
        uint256 swapToGHST = IERC20(DG).balanceOf(address(this));
        IERC20(DG).safeApprove(router, 0);
        IERC20(DG).safeApprove(router, swapToGHST);
        swapRouter.swapExactTokensForTokens(swapToGHST, 0, swapRoute, address(this), block.timestamp + 100);

        // Prepare shares for adding GHST
        shares = IClusterTokenV1(_asset).getUnderlyingInCluster();
        uint256 ghstTotal = (IERC20(GHST).balanceOf(address(this))) + IClusterTokenV1(_asset).getUnderlyingBalance(GHST);
        shares[0] = (ghstTotal * 10**6) / IERC20(_asset).totalSupply();

        // Optimize
        IClusterTokenV1(_asset).optimizeProportion(shares);

        // Send tokens to strategies
        ghstTotal = IERC20(GHST).balanceOf(address(this));
        IERC20(GHST).safeTransfer(_ghstStrat, ghstTotal);
        IStrategyPlugin(_ghstStrat).deposit(ghstTotal);
    }
}