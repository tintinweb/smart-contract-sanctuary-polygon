...
contract SiloManager is Initializable, KeeperCompatibleInterface {
    ...
    function setUpkeepDuration(uint256 _duration) external onlyOwner {
        require(_duration > 0, "wrong duration");
        upkeepDuration = _duration;
    }
            ...
            if (!upkeepNeeded) {
                if (autoTopup) {
                    if (
                        ...
                        // not healthy status
                        uint256 ERC20LinkBal = ERC20Link.balanceOf(
                            address(this)
                        );
                        ...
                        if (linkToAdd >= addFundsThreshold) {
                            upkeepNeeded = true;
                            uint256 addAmount = ManagerFactory
                                .getMinimumUpkeepBalance(owner) -
                                ManagerFactory.getUpkeepBalance(owner);
                ...
                if (!upkeepNeeded) {
                    ...
                    for (uint256 i; i < SiloFactory.balanceOf(owner); ) {
                        ...
                        if (Silo.highRiskAction()) {
                            ...
                            uint96 balance = ManagerFactory.getBalance(
                                upkeepId
                            );
                        ...
                        if (!upkeepNeeded) {
                            if (!detected[address(Silo)]) {
                                (upkeepNeeded, siloPerformData) = Silo
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        if (silo != address(this)) {
            if (detected[silo]) {
                revert("stop fast burn");
            }
            if (lastSilo != address(0) && lastUpkeep != 0) {
                if (
                    lastSilo == silo &&
                    (lastUpkeep + upkeepDuration) > block.number
                ) {
                    detected[silo] = true;
                }
            }
            ...
            if (task == 2) {
                ...
                if (balance >= addFundsThreshold) {
                    ...
                    uint256 addAmount = ManagerFactory.getMinimumUpkeepBalance(
                        owner
                    ) - ManagerFactory.getUpkeepBalance(owner);
                    addAmount = (addAmount * 110) / 100;
                    ...
                    uint256 topupAmount = addAmount > balance
                        ...
                        : addAmount;
                    ERC677Link.approve(
                        ManagerFactory.getKeeperRegistry(),
                        topupAmount
                    );

                    IKeepersRegistry(ManagerFactory.getKeeperRegistry())
                        .addFunds(upkeepId, uint96(topupAmount)); //add funds to CURRENT registry
