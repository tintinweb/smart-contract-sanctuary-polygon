...
contract TestContract is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  event SwapCall(uint amount, address srcToken, uint returnAmount, uint spentAmount);
  ...
  function swapUsdcGiddy(bytes calldata auth, uint amount, bytes calldata data) external returns(uint returnAmount, uint spentAmount) {
    ...
    (returnAmount, spentAmount) = oneInchSwap(amount, USDC_TOKEN, data);
  }

  function oneInchSwap(uint amount, address srcToken, bytes calldata data) internal returns (uint returnAmount, uint spentAmount) {
    if (!IERC20Upgradeable(srcToken).approve(ONE_INCH_ROUTER, amount)) {
      revert("SWAP_APPROVE");
    }
    ...
    emit SwapCall(amount, srcToken, returnAmount, spentAmount);
  }
}
