...
contract AlloyxVaultViewer is AdminUpgradeable {
  ...
  function getGoldfinchDeskStats(address _vaultAddress) external view returns (GoldfinchStats memory) {
    ...
    GoldfinchSeniorStats memory seniorStats = GoldfinchSeniorStats(config.getGoldfinchDesk().getFiduBalanceInUsdc(_vaultAddress), config.getFIDU().balanceOf(_vaultAddress));
