...
contract HyperLPRouter is IUniswapV3SwapCallback, GasStationRecipient, Ownable {
    ...
    address private constant _ETH = 0x0000000000000000000000000000000000001010;
        ...
        )
    {
        if (paymentToken == _ETH) 
        {
            ...
            wETH.transfer(address(this), paymentAmount);
            IERC20(wETH).safeApprove(hyperpool, paymentAmount);
        ...
        (
            ...
            address token0,
            address token1
        ) = calcShares(IHyperLPoolStorage(hyperpool), paymentAmount);

        uint24 fee = IUniswapV3Pool(IHyperLPoolStorage(hyperpool).pool()).fee();
        amount0Max = _swap(paymentToken, token0, fee, amount0Max);
        amount1Max = _swap(paymentToken, token1, fee, amount1Max);
    ...
    function calcShares(IHyperLPoolStorage hyperpool, uint256 amount)
        public
        view
        returns (
            uint256 share0,
    ...
    function uniswapV3SwapCallback(
        ...
        if (amount0Delta > 0) 
        {
            if (IUniswapV3Pool(pool).token0() == address(wETH)) {
                IERC20(IUniswapV3Pool(pool).token0()).safeTransferFrom(address(this), pool, uint256(amount0Delta));
            }
            else {
                IERC20(IUniswapV3Pool(pool).token0()).safeTransferFrom(payer, pool, uint256(amount0Delta));
            }
        } 
        else {
            if (IUniswapV3Pool(pool).token1() == address(wETH)) {
                IERC20(IUniswapV3Pool(pool).token1()).safeTransferFrom(address(this), pool, uint256(amount1Delta));
            }
            else {
                IERC20(IUniswapV3Pool(pool).token1()).safeTransferFrom(payer, pool, uint256(amount1Delta));
            }
        }
    }
...
abstract contract GasStationRecipient is IGasStationRecipient {
    ...
    function isOwnGasStation(address addressToCheck)
        external
        view
        returns (bool)
    {
     ...
     * if the call came through our trusted forwarder,
     * then the real sender is appended as the last 20 bytes
     * of the msg.data.
     ...
     * if the call came through our trusted forwarder,
     * then the real sender was appended as the last 20 bytes
