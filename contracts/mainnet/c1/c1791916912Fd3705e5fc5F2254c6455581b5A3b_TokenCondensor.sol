...
contract TokenCondensor is BaseSiloAction {
    ...
    event GFIDeposit(address input,address router,uint256 amount,uint8 aType);
    ...
    function directTokenCondense(
        ...
        for (uint256 i; i < 4; ) {
            ...
            for (uint256 j; j < 4; ) {
                ...
                if (address(inputs[i]) == address(GFI)) {
                    path[0] = inputs[i];
                    path[1] = outputs[j];
                    ...
                    emit GFIDeposit(inputs[i],GRAVITY_ROUTER,amountOut,1);
                ...
                emit GFIDeposit(inputs[i],bestQuote.router,bestQuote.amountOut,2);
    ...
    function swapTokenWithBridge(
        ...
        if (amountOut > 0) {
            (amounts, result) = swapTokens(router, path2, amountIn, amountOut);
        } else {
            path2[0] = tokenIn;
            path2[1] = USDC;
            path2[2] = tokenOut;
            amountOut = oracle.getUniPrice(router, path2, amountIn);
            if (amountOut > 0) {
                (amounts, result) = swapTokens(
                    router,
                    path2,
