...
contract EthermonSeasonExpClaim is BasicAccessControl, SafeMathEthermon {
    ...
    mapping(uint256 => uint256) public requestStatus; // request_id => status
    ...
    function claimExp(
        ...
        if (eToken.rId == 0 || requestStatus[eToken.rId] > 0) revert(); //rId is reqId coming from python backend should be diff everytime
