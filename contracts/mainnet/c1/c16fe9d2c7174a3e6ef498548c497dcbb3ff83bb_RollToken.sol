...
contract RollToken is ERC20("MATIC CASINO", "SILVER"), Ownable {

    uint256 public constant MAX_SUPPLY = 200000000 ether;
    
    function mint(address _to, uint256 _amount) public onlyOwner {
        uint _totalSupply = totalSupply();
        require(_totalSupply < MAX_SUPPLY, "Total supply must not exceed max supply.");

        // Impose maximum total Supply on minting of token.
        if (_totalSupply + _amount <= MAX_SUPPLY) {
            _mint(_to, _amount);
            _moveDelegates(address(0), _delegates[_to], _amount);
        } else if (_totalSupply + _amount > MAX_SUPPLY && _totalSupply <= MAX_SUPPLY) {
            _mint(_to, MAX_SUPPLY - _totalSupply);
            _moveDelegates(address(0), _delegates[_to], MAX_SUPPLY - _totalSupply);
        }
