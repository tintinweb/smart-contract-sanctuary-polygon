...
contract PerformanceTokens is ERC20, ReentrancyGuard {
    ...
    constructor(address _token, string memory name_, string memory symbol_, address _owner) ERC20(name_, symbol_)  public {
        token = _token;
        owner=_owner;
        _name=name_;
        _symbol=symbol_;
    ...
    function withdrawFee() public onlyOwner() {
        uint256 what = calculateUnderlying(perfEarned);
        _burn(msg.sender, perfEarned);
        ERC20(token).transfer(msg.sender, what);
        perfEarned=0;
    }
