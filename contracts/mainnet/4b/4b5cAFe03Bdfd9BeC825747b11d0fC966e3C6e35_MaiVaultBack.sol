...
contract MaiVaultBack is BaseSiloAction {
    ...
    function exit(
        ...
        (input, , maiVault, ratioRange, path) = abi.decode(
            configuration,
            (address[5], address[5], address, uint256[3], address[])
        );
        ...
        if (vaultDebt <= miBalance) {
            ...
            if (
                vault.checkCollateralPercentage(vaultId) >
                ratioRange[1] / 10**18
            ) {
                //withdraw collateral
                ...
                idealCollateralValue =
                    (10**oracle.decimals() * idealCollateralValue) /
                ...
                if (idealCollateral < vault.vaultCollateral(vaultId)) {
                    uint256 amountToWithdraw = vault.vaultCollateral(vaultId) -
                        idealCollateral;
                    vault.withdrawCollateral(vaultId, amountToWithdraw);
                }
            } else if (vault.checkCollateralPercentage(vaultId) == 0) {
        ...
        extraWithdrawCollateral(vault, vaultId, path);
    }
    ...
    function extraWithdrawCollateral(
        ...
        IERC20Decimals collateral = IERC20Decimals(vault.collateral());
        uint256 collateralUnit = 10**collateral.decimals() ;
        uint256 vaultDebt = vault.vaultDebt(vaultId);

        if (vaultDebt > 0) {
            uint256 maiPrice = getUniPrice(path, collateralUnit);
            if (maiPrice > 0) {
                uint256 collateralBalance = collateral.balanceOf(address(this));
                uint256 collateralAmount = (((vaultDebt * collateralUnit) / maiPrice) *
                    105) / 100;
                if (collateralBalance > collateralAmount) {
                    ...
                    if (result && amounts[amounts.length - 1] > 0) {
                        IERC20(vault.mai()).approve(address(vault), vaultDebt);
                        vault.payBackToken(vaultId, vaultDebt);
                        vault.withdrawCollateral(
                            vaultId,
                            vault.vaultCollateral(vaultId)
    ...
    function createConfig(
        ...
        configData = abi.encode(_inputs, _outputs, _qiVault, _ratioRange, path);
    }

    function getUniPrice(address[] memory path, uint256 amountIn)
        public
        view
        returns (uint256)
    {
        uint256 quote; //0
        try
            IUniswapV2Router02(QUICK_V2_ROUTER).getAmountsOut(amountIn, path)
    ...
    function validateConfig(bytes memory configuration)
        public
        view
        override
        returns (bool)
    {
        ...
        if (
            path[0] != vault.collateral() ||
            path[path.length - 1] != vault.mai()
        ) {
            return false;
        }

        if (getUniPrice(path, 1 ether) == 0) {
            return false;
        }
