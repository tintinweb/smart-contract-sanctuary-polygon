...
contract DAOFactory {
    ...
    Kernel public kernelCode;
    Sudo public sudoCode;
    Admin public adminCode;
    ERC721Properties public erc721Code;

    address public kernelProxy;
    address public sudoProxy;
    address public adminProxy;
    address public erc721Proxy;
    ...
    constructor(
        ...
        kernelCode = new Kernel();
        sudoCode = new Sudo();
        adminCode = new Admin();
        erc721Code = new ERC721Properties();

        kernelProxy = address(new AppProxy(address(kernelCode)));
        sudoProxy = address(new AppProxy(address(sudoCode)));
        adminProxy = address(new AppProxy(address(adminCode)));
        erc721Proxy = address(new AppProxy(address(erc721Code)));

        Kernel kernel = Kernel(kernelProxy);
        Sudo sudo = Sudo(sudoProxy);
        ERC721Properties erc721 = ERC721Properties(erc721Proxy);
        Admin admin = Admin(adminProxy);

        kernel.initialize(sudoProxy);

        address[] memory sudoAdmins = new address[](2);
        sudoAdmins[0] = sudoAdmin;
        sudoAdmins[1] = address(this);

        Sudo(sudoProxy).initialize(IKernel(kernelProxy), sudoAdmins);
        Admin(adminProxy).initialize(IKernel(kernelProxy), admins);
        ERC721Properties(erc721Proxy).initialize(IKernel(kernelProxy), true, 0, admins[0], ipfsHash, "TOKEN", "TKN");

        sudo.connectApp(ADMIN_APP, adminProxy);
        sudo.connectApp(ERC721_APP, erc721Proxy);

        sudo.addPermission(SUDO_APP, ERC721_APP, erc721.APP_UPDATER());
        sudo.addPermission(SUDO_APP, ADMIN_APP, admin.APP_UPDATER());
        sudo.addPermission(SUDO_APP, ADMIN_APP, admin.SUDO());
        sudo.addPermission(ADMIN_APP, ERC721_APP, erc721.ADMIN());

        sudo.setAttribute("TIER", bytes32("CLASSIC"), "MAX_AMOUNT", bytes32(uint256(100)));
        sudo.setAttribute("TIER", bytes32("EPIC"), "MAX_AMOUNT", bytes32(uint256(100)));
        sudo.setAttribute("TIER", bytes32("LEGENDARY"), "MAX_AMOUNT", bytes32(uint256(100)));

        sudo.removeSudoAdmin(address(this));
    }
}
