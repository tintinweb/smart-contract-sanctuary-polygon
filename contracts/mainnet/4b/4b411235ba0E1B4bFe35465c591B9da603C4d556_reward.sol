...
contract reward {
    ...
    function getReward() public {
        ...
        (reward, tmpAmount) = pendingReward(msg.sender);
    ...
    function pendingReward(address _address) public view returns(uint, uint){
        User storage user = stackers[_address];
        if(user.amount <= 0){
            return (0, 0);
        }
