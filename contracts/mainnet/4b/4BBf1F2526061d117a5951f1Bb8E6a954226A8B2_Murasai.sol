...
contract Murasai is ERC721, IERC2981, Ownable {
  ...
  uint8 public MAX_MINT = 2;
  uint public idBendo = 1;

  uint256 public publicPrice = 0.092 ether;
  ...
  function mintWhitelist(uint8 amount) public payable {
    ...
    require(MAX_SUPPLY >= _currentId + amount);
  ...
  function mint(uint8 amount) public payable {
    ...
    require(amount > 0 && balanceOf(_msgSender()) + amount <= MAX_MINT , "you can't mint this number");
  ...
  function getWhitelistAmountOf(address addr) public view returns(uint) {
