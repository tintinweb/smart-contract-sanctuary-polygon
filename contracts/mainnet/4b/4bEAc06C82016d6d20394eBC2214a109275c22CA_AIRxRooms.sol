...
contract AIRxRooms is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdTracker;

    uint256 public MAX_ELEMENTS = 765;
    uint256 public PRICE = 1 * 10 ** 17; // 500 * 10**18; // 500 MATIC
    uint256 public MAX_BY_MINT = 25;
    
    string public baseTokenURI; // IPFS
    bool private risk_protect_mode = true;
    ...
    constructor(string memory baseURI) ERC721("AIRxRooms", "AIRxRooms") {
        baseTokenURI = baseURI;
        pause(true);
    }

    modifier saleIsOpen(uint256 _count) {
        require(_totalSupply() + _count <= MAX_ELEMENTS, "MINT: exceed limitation");
        require(isPresale == false, "Presale: processing");
    ...
    function _mintAnElement(address _to) private {
        ...
        if (risk_protect_mode && _exists(id)) {
            for (uint256 i = id; i <= MAX_ELEMENTS; i++) {
                if (!_exists(i)) break;
    ...
    // Admin mint
    function _adminMint(uint256 _tId, address _to) private {
    ...
    // Admin batch mint
    function adminBatchMint(Presale[] calldata presales) external onlyOwner {
        for(uint256 i = 0; i < presales.length; i++) {
            _adminMint(presales[i].id, presales[i].holder);
        }
    }

    // Admin mint
    function adminMint(address holder, uint256 id) external onlyOwner {
        _adminMint(id, holder);
    }
    ...
    // Set presale as true or not
    function setPresale(bool val) public onlyOwner {
        require(isPresale != val, "Already set");
        isPresale = val;
    }
