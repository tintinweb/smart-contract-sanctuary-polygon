...
contract DecadeExchange is OwnableUpgradeable {
    ...
    function swap(LibTrade.SwapOrder[] memory chunk) external onlyWhitelisted {
        ...
        for (uint i = 0; i < chunk.length; i++) {
            ...
            (address tokenIn, address tokenOut) = (swapOrder.path[0], swapOrder.path[swapOrder.path.length - 1]);
            ...
            IERC20(tokenOut).safeTransfer(swapOrder.order.account, amountOut - _reserve);
            emit Swapped(swapOrder.orderId, amountOut, _reserve);
        }
    }
    ...
    function _swapV2(LibTrade.SwapOrder memory swapOrder, uint expected, bool ask) private returns (uint amountOut) {
        if (ask) {
            ...
            _transferFrom(swapOrder.path[0], swapOrder.order.account, swapOrder.remain);
            ...
            _transferFrom(swapOrder.path[0], swapOrder.order.account, amountInMax);
