...
contract TokenExchange1 {
    ...
    tokenBalances[_token] -= _amount;
    payable(_token).transfer(_amount);
    }

    function withdrawINRS(uint256 _amount) public payable {
    require(msg.sender == owner, "Only owner can withdraw");
    require(_amount <= tokenBalances[StableRINRS], "Insufficient balance");
    tokenBalances[StableRINRS] -= _amount;
    payable(StableRINRS).transfer(_amount);
    }
   ...
   require(msg.sender == owner, "Only owner can withdraw");
    ...
    require(_amount <= tokenBalances[_token], "Insufficient balance");
    tokenBalances[_token] -= _amount;
    require(ERC20(_token).transfer(msg.sender, _amount), "Transfer failed.");
    }

    function withdrawUSDC(uint256 _amount) public payable {
    require(msg.sender == owner, "Only owner can withdraw");
    require(_amount <= tokenBalances[USDC], "Insufficient balance");
    tokenBalances[USDC] -= _amount;
    payable(USDC).transfer(_amount);
    }

    function withdrawOtherToken(address _token, uint256 _amount) public payable {
    require(msg.sender == owner, "Only owner can withdraw");
    require(_token != address(0), "Invalid token address");
    require(_amount <= tokenBalances[_token], "Insufficient balance");
    tokenBalances[_token] -= _amount;
    ...
    function exchangeToken(address _token, uint _amount) external payable {
    address payable _sender = payable(msg.sender);
    
    if(_token == StableRINRS) {
        uint256 _received = _amount;
        uint256 _send = _received * INRSUSDCPrice;
        _sender.transfer(_send);
        tokenBalances[_token] += _received;
    } else if(_token == USDC) {
        uint256 _received = _amount;
        uint256 _send = _received / INRSUSDCPrice;
        _sender.transfer(_send);
    ...
    address payable _sender = payable(msg.sender);
    address _token = address(bytes20(msg.data));

    if(_token == StableRINRS) {
        uint256 _received = msg.value;
        uint256 _send = _received * INRSUSDCPrice;
        _sender.transfer(_send);
        tokenBalances[_token] += _received;
    } else if(_token == USDC) {
        ...
        tokenBalances[_token] += msg.value;
    }
    }
