...
// File: ElementalParticles_flat.sol
...
contract ElementalParticles is ERC20, ERC20Burnable {
    ...
    IERC20 public constant SAFE_FBX = IERC20(0xD125443F38A69d776177c2B9c041f462936F8218);
    ...
    uint256 public constant DAILY_EMISSION_DENOMINATOR = 2015; // <<TESTING to calculate before deployment>>
    ...
    constructor() ERC20("ElementalParticles", "EP") {
        _mint(msg.sender, 12700 * 1e18); // <<TESTING to calculate before deployment>> Initial airdrop distributed to NFT owners
    }
    ...
    function FBXMaxSupply() public view returns (uint256) {
        return SAFE_FBX.totalSupply();
    }

    function FBXBurntSupply() public view returns (uint256) {
        return 21000000 * 1e18 - FBXMaxSupply();
    }
    ...
    function dailyFBXEmissionPerEP() public view returns (uint256) {
        return dailyFBXEmission() / totalStakedEP;
    }

    /// EP functions

    function claimableFBXRewards(address account) public view returns (uint256) {
        if (stakedEP[account] == 0) {
	        return 0;
        }
        uint256 elapsedTime = block.timestamp - lastClaim[account];
        if (elapsedTime > 30 * 24 * 3600) {
            elapsedTime = 30 * 24 * 3600; // Max claimable FBXRewards period is 30 days
        }
        return dailyFBXEmissionPerEP() * 9 / 10 * stakedEP[account] * elapsedTime / 24 * 3600 / 1e18;
    }

    function claimFBXRewards() public returns (uint256) {
        uint256 FBXRewards = claimableFBXRewards(msg.sender);
        lastClaim[msg.sender] = block.timestamp;
        if (FBXRewards > 0) {
            amountClaimed[msg.sender] += FBXRewards;
            SAFE_FBX.safeTransferFrom(address(this), msg.sender, FBXRewards);
            SAFE_FBX.safeTransferFrom(address(this), TEAM_MULTISIG, FBXRewards / 9); // Distribute the corresponding part of the vested FBX to the team
        }
        return FBXRewards;
    }
    ...
    function unstakeEP(uint256 amountEP) public {
        ...
        require(stakedEP[msg.sender] >= amountEP, "Not enough staked EP.");
        claimFBXRewards();
        stakedEP[msg.sender] -= amountEP;
        totalStakedEP -= amountEP;
        transferFrom(address(this), msg.sender, amountEP);
    }

    function burnFBXToMintEP(uint256 amountEP) public {
        require(amountEP > 0, "Amount to mint should be greater than 0.");
        FBX.burnFrom(msg.sender, 300 * amountEP);
        _mint(msg.sender, amountEP);
    }

    function mintAndStake(uint256 amountEP) public {
        burnFBXToMintEP(amountEP);
        stakeEP(amountEP);
    }
}