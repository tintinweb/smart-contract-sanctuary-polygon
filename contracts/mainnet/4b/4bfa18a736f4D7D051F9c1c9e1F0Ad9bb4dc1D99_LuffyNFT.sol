...
contract LuffyMarket is Ownable {
    ...
    function purchaseWithFixedPrice(
        ...
        address from = thisSale.seller;
        address tokenAddr = thisSale.tokenPayment;
        uint256 salePrice = thisSale.price;
        bool isBurn = thisSale.isBurn;
        uint256 burnRate = thisSale.burnRate;
        ...
        require(
            nftInstance.isApprovedForAll(from, address(this)) == true,
            "Nft is not approved by the owner"
        );

        if (tokenAddr == address(0)) {
            //buy by ETH
            require(msg.value >= salePrice, "Insufficient ETH amount");
            ethFee = (msg.value * getFee) / (100 * feeDecimal);
            payable(from).transfer(msg.value - ethFee);
        } else {
            //buy by token
            require(token.balanceOf(to) >= salePrice, "Insufficient token balance");
            require(
                token.allowance(to, address(this)) >= salePrice,
                "Insufficient token allowance"
            );

            if (isBurn == true) {
                uint256 burnPrice = (salePrice * burnRate) / 10 ** 8;
                require(token.transferFrom(to, burnAddr, burnPrice), "Transfer burn amount fail");
                require(
                    token.transferFrom(to, from, (salePrice - burnPrice)),
                    "Transfer token to seller fail"
                );
            } else {
                getFee = setFee[tokenAddr];
                tokenFee = (salePrice * getFee) / (100 * feeDecimal);
                require(
                    token.transferFrom(to, address(this), tokenFee),
                    "Transfer fee amount fail"
                );
                require(
                    token.transferFrom(to, from, (salePrice - tokenFee)),
                    "Transfer token to seller fail"
                );
            }
        }
        try nftInstance.transferFrom(from, to, tokenId) {
            sales[nftAddress][tokenId].isCompleted = true;
            emit NftSold(from, nftAddress, tokenId, tokenAddr, salePrice, to);
    ...
    function finishAuction(
        ...
        bool isBurn = thisAuction.isBurn;
        uint256 burnRate = thisAuction.burnRate;
        ...
        if (isBurn == true) {
            uint256 burnPrice = (winPrice * burnRate) / 10 ** 8;
            require(
                token.transferFrom(highestBidder, burnAddr, burnPrice),
                "Transfer burn amount fail"
            );
            require(
                token.transferFrom(highestBidder, from, (winPrice - burnPrice)),
                "Transfer token to seller fail"
            );
        } else {
            ...
            require(
                token.transferFrom(highestBidder, from, (winPrice - tokenFee)),
                "Transfer token to seller fail"
            );
        }
        try nftInstance.transferFrom(from, highestBidder, tokenId) {
