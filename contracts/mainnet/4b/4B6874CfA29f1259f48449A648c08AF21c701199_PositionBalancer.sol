...
contract PositionBalancer is IERC721Receiver, IUniswapPositionBalancerResolver, OpsReady {
    ...
    returns (bool canExec, bytes memory execPayload)
  {
    ...
    if (positionsToRebalanceIndex == 0) {
      return (false, '');
    }
    ...
    execPayload = abi.encodeWithSelector(
      ...
      _removeEmptyPositionsFromRebalanceRequestArray(positionsToRebalance)
    );
  }

  function _removeEmptyPositionsFromRebalanceRequestArray(
    RebalanceRequest[] memory positionsToRebalance
  ) internal pure returns (RebalanceRequest[] memory clearedPositionsToRebalance) {
    uint256[] memory nonEmptyIds = new uint256[](positionsToRebalance.length);
    uint256 nonEmptyIdsIndex = 0;
    // find non empty positions
    for (uint256 i = 0; i < positionsToRebalance.length; i++) {
      // if all the fields are not equal 0 then it's not an empty position
      if (
        positionsToRebalance[i].tokenId != 0 &&
        positionsToRebalance[i].newLowerTick != 0 &&
        positionsToRebalance[i].newUpperTick != 0
      ) {
        nonEmptyIds[nonEmptyIdsIndex] = i;
        nonEmptyIdsIndex++;
      }
    }
    // create a new array containing only non empty positions
    clearedPositionsToRebalance = new RebalanceRequest[](nonEmptyIdsIndex);
    for (uint256 i = 0; i < nonEmptyIdsIndex; i++) {
      clearedPositionsToRebalance[i] = positionsToRebalance[nonEmptyIds[i]];
    }
  }
