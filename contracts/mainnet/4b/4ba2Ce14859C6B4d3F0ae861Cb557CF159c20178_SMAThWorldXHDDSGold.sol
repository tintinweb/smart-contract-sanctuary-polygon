 ...
 * @notice This contract handles minting SMATh World x HDDS Gold tokens.
 */
contract SMAThWorldXHDDSGold is
    ERC1155Supply,
    Ownable,
    Pausable,
    ReentrancyGuard,
    ...
    RevokableOperatorFilterer
{
    ...
    address private presaleSignerAddress =
        0x16DA71Cd37E27ff1bb75882bEC61Ff0dF7089C38;
    address public royaltyAddress = 0x53829D74DA8092cA8B76A3632A41b5b0B07ea305;
    ...
    address[] public payoutAddresses = [
        0xE7E01E536865A965f1335A5615419f847fc0B7AA
    ];
    ...
    mapping(uint256 => BurnToken[]) public tokenBurnTokens;
    mapping(uint256 => bool) public tokenBurnClaimActive;
    ...
    mapping(uint256 => uint256) public tokenBurnPayment;
    ...
    mapping(uint256 => uint256) public tokenMintsPerBurn;
    ...
    string public name = "SMATh World x HDDS Gold";
    string public symbol = "SHGOLD";
    ...
    constructor()
        ERC1155(
            "ipfs://bafybeichkultz4zmjisfss7xxmtal6dcb65ikvg6vrp3lkjybqzrttt5zm/{id}"
        )
        RevokableOperatorFilterer(
        ...
        )
    {
        ...
        tokenMaxSupply[1] = 100;
        tokenMintsPerBurn[1] = 1;
        tokenMaxSupply[2] = 100;
        tokenMintsPerBurn[2] = 1;
        tokenMaxSupply[3] = 100;
        tokenMintsPerBurn[3] = 1;
        require(
    ...
    function setBurnClaimState(
        uint256 _tokenId,
        bool _tokenBurnClaimActive
    ) external onlyOwner {
        require(
            tokenBurnClaimActive[_tokenId] != _tokenBurnClaimActive,
            "NEW_STATE_IDENTICAL_TO_OLD_STATE"
        );
        tokenBurnClaimActive[_tokenId] = _tokenBurnClaimActive;
    }
     ...
     * @param _tokenId The tokenId of the token to be minted on this contract
     * @param _tokenMintsPerBurn The number of tokens to be minted per burn transaction
     */
    function updateTokenMintsPerBurn(
        uint256 _tokenId,
        uint256 _tokenMintsPerBurn
    ) external onlyOwner {
        tokenMintsPerBurn[_tokenId] = _tokenMintsPerBurn;
    }
     ...
     * @param _tokenId The tokenId of the token to be minted on this contract
     * @param _tokenBurnPayment The payment required alongside a burn tx to mint _tokenId tokens
     */
    function updateTokenBurnPayment(
        uint256 _tokenId,
        uint256 _tokenBurnPayment
    ) external onlyOwner {
        tokenBurnPayment[_tokenId] = _tokenBurnPayment;
    }
     ...
     */
    function updateTokenBurnTokens(
        uint256 _tokenId,
        BurnToken[] calldata _burnTokens
    ) external onlyOwner {
        BurnToken[] storage burnTokens = tokenBurnTokens[_tokenId];
    ...
    function burnToMint(
        ...
        require(tokenBurnClaimActive[_tokenId], "NOT_ACTIVE");
        require(
            !tokenMintingPermanentlyDisabled[_tokenId],
            "MINTING_PERMANENTLY_DISABLED"
        );
        BurnToken[] storage burnTokens = tokenBurnTokens[_tokenId];
        uint256 mintsPerBurn = tokenMintsPerBurn[_tokenId];
        uint256 burnTokenLength = burnTokens.length;
        uint256 maxSupply = tokenMaxSupply[_tokenId];
        require(
            _contracts.length == _tokenIds.length,
            "ARRAY_LENGTHS_MUST_MATCH"
        );
        require(
            _contracts.length == burnTokenLength,
            "ARRAY_LENGTHS_MUST_MATCH"
        );
        require(burnTokenLength > 0, "NO_BURN_TOKENS_SET");
        require(
            maxSupply == 0 ||
                totalSupply(_tokenId) + _tokensToMint <= maxSupply,
            "MAX_SUPPLY_EXCEEDED"
        );
        uint256 burnPayment = tokenBurnPayment[_tokenId] *
            (_tokensToMint / mintsPerBurn);
        require(msg.value == burnPayment, "PAYMENT_INCORRECT");
        ...
        tokensMintedByAddress[msg.sender][_tokenId] += _tokensToMint;
