...
interface UsersCheck
{
    //function userInfos(address userAddress, uint8 level) external view returns(bool);
    function userInfos(address _user) external view returns(bool, uint, uint, uint);
}
...
contract META_GFORCE_s30 is owned
{
   ...
   // mapping(address => mapping(uint => uint8)) public autoLevelBuy; 
    ...
    function setLeveladdress(address newLeveladdress) onlyOwner public returns(bool)
    {
     ...
     function getjoing(address _user) public view returns(bool UE)
    {
        (UE,,,) = UsersCheck(levelAddress).userInfos(_user);
        return UE;
    }
    ...
    function regUser() public returns(bool)
    {
        ...
        require(getjoing(msg.sender), "This users is not exists!!!");
    ...
    function regUser_own(address usermsg) onlyOwner public returns(bool)
    {
        ...
        require(getjoing(usermsg), "This users is not exists!!!");
    ...
    function buyLevel_own(address usermsg, uint _level) onlyOwner public returns(bool)
    {
       ...
       require(_level < 13 && _level > 1, "invalid level");
        ...
        uint prc = levelPrice[_level];
        tokenInterface(tokenAddress).transferFrom(msg.sender, address(usermsg), prc);
