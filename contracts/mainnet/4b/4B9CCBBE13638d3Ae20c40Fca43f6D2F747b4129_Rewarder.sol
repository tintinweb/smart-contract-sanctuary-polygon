...
// File contracts/main/Rewarder.sol
...
contract Rewarder is Ownable {
    ...
    IPolyInfFarm public immutable polyInfFarm;
    IPolyInfFarm public immutable polyInfFarmV2;

    /// @dev pending represent how much rewardToken user received
    event OnPITReward(uint256 indexed pid, address indexed user, uint256 amount, uint256 pending);
    ...
    modifier onlyPolyInfFarmV2() {
        require(msg.sender == address(polyInfFarmV2), "Only PolyInfFarmV2 can call this function.");
        _;
    }
    ...
    constructor(
        ...
        IPolyInfFarm _polyInfFarm,
        IPolyInfFarm _polyInfFarmV2
    ) public {
        rewardToken = _rewardToken;
        rewardPerSecond = _rewardPerSecond;
        rewardStartTimestamp = _rewardStartTimestamp;
        rewardEndTimestamp = _rewardEndTimestamp;
        polyInfFarm = _polyInfFarm;
        polyInfFarmV2 = _polyInfFarmV2;
    ...
    /// @dev MUST call this function before update user.amount at polyInfFarmV2. If contract has not enough
    ...
    function onPITReward(
    ...
    ) external onlyPolyInfFarmV2 {
        updatePool(_pid);

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        (uint256 userAmount, ) = polyInfFarmV2.userInfo(_pid, _user);
        ...
        emit OnPITReward(_pid, _user, _currentAmount, pending);
    }
    ...
    /// @param _pid Pid on PolyInfFarmV2
    function add(
        uint256 _allocPoint,
        uint256 _pid,
        bool _withUpdate
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (address, uint256) {
        ...
        (uint256 lpSupply, ) = polyInfFarm.userInfo(_pid, address(polyInfFarmV2));
    ...
    // Release remain reward Token
    function releaseRewardToken() external onlyOwner{
        uint256 remainTokenAmount = rewardToken.balanceOf(address(this));
        if (remainTokenAmount > 0) {
            rewardToken.safeTransfer(msg.sender, remainTokenAmount);
        }
    }
}