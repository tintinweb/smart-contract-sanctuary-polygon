...
contract MintXAndBorrowY {
   ...
   address constant nativeToken = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;
    ...
    function uniswapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external {
        ...
        address _tokenY = tokenY==nativeToken ? router.WETH() : tokenY;
        (uint reserve0, uint reserve1,) = pair.getReserves();
        (uint reserveIn, uint reserveOut) = pair.token0() == _tokenY ? (reserve1, reserve0) : (reserve0, reserve1);
        ...
        router.swapExactTokensForTokens(IERC20(path[0]).balanceOf(address(this)), amountRequired, path, address(this), block.timestamp);
