 ...
 */
contract FalconPeg is ERC20, ERC20Burnable, Ownable {
    //---------- Contracts ----------//
    ...
    address public immutable underlying; // Multichain required.
    uint256 public immutable maxSupply; // Multichain required.
    uint256 public constant DELAY = 2 days; // Delay for timelock functions
    ...
    mapping(address => bool) public _isMinter; // set of minters for bridge.
    ...
    event NewMinterRequest(address newMinter, uint256 applyDate);
    event ModifiedMinter(address minter, bool enabled);

    //---------- Constructor ----------//
    constructor(IRouterV2 _dexRouter, address _bridgeRouter)
        ERC20("Falcon", "FLN")
    {
        maxSupply = 1_500_000_000 * 10**decimals();
        ...
        _isMinter[_bridgeRouter] = true;
        treasury = payable(msg.sender);
        hasLiquidity = false;
        startTrading = 1670180406;
    ...
    modifier onlyAuth() {
        require(_isMinter[msg.sender], "FalconPeg: FORBIDDEN");
        _;
    }
    ...
    function mint(address to, uint256 amount) external onlyAuth returns (bool) {
        require(amount + totalSupply() <= maxSupply, "FalconPeg: Oversupply");
        _mint(to, amount);
        return true;
    }
    ...
    function burn(address from, uint256 amount)
        ...
        returns (bool)
    {
        require(from != address(0), "FalconPeg: address(0)");
        _burn(from, amount);
        return true;
    }
    ...
    function setMinter(address _auth) external onlyOwner {
        require(_auth != address(0), "ShibaWorldCupPeg: address(0)");
        pendingMinter = _auth;
        delayMinter = block.timestamp + DELAY;
        emit NewMinterRequest(pendingMinter, delayMinter);
    }

    function applyMinter() external onlyOwner {
        require(pendingMinter != address(0) && block.timestamp >= delayMinter);
        _isMinter[pendingMinter] = true;
        emit ModifiedMinter(pendingMinter, true);
    ...
    function revokeMinter(address _auth) external onlyOwner {
        _isMinter[_auth] = false;
        emit ModifiedMinter(_auth, false);
    }
}
