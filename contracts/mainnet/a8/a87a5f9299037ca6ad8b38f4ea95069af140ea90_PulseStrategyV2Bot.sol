...
contract PulseStrategyV2Bot {
    ...
    constructor(PulseStrategyV2 strategy_, ISwapRouter swapRouter_, UniV3TokenRegistry registry_) {
        strategy = strategy_;
    ...
    function _createNewToken(
        ...
        int24 tickUpper,
        bytes memory params
    ) private returns (address newToken) {
        UniV3Token.InitParams memory initParams;
        initParams.name = token.name();
        initParams.symbol = token.symbol();
        initParams.admin = token.admin();
        initParams.oracle = token.oracle();
        initParams.securityParams = token.securityParams();

        address token0 = token.token0();
        address token1 = token.token1();

        (uint256 desired0, uint256 desired1) = abi.decode(params, (uint256, uint256));

        INonfungiblePositionManager positionManager = token.positionManager();
        IERC20(token0).safeIncreaseAllowance(address(positionManager), desired0);
        IERC20(token1).safeIncreaseAllowance(address(positionManager), desired1);

        (initParams.uniV3Nft, , , ) = positionManager.mint(
        ...
        positionManager.approve(address(registry), initParams.uniV3Nft);

        IERC20(token0).safeApprove(address(positionManager), 0);
        IERC20(token1).safeApprove(address(positionManager), 0);

        (, newToken) = UniV3Token(token).registry().createCompoundableToken(initParams);
    }

    function rebalance(bytes memory params) external {
        (
            PulseStrategyV2.ImmutableParams memory immutableParams,
            PulseStrategyV2.MutableParams memory mutableParams,
            PulseStrategyV2.State memory state
        ) = strategy.parameters();

        address token = address(immutableParams.token);
...

        {
            ...
            UniV3CompoundableToken(ProxyToken(payable(token)).token()).compound();

            UniV3Token(token).withdraw(IERC20(token).balanceOf(address(this)), abi.encode(new uint256[](2)));

            (int24 tickLower, int24 tickUpper) = strategy.calculateNewPosition(state, pool, mutableParams);
            address newToken = _createNewToken(UniV3Token(token), tickLower, tickUpper, params);
            ProxyToken(payable(token)).upgradeTo(newToken);
        }
...

        {
            ...
            UniswapCalculations.PositionParams memory position = UniswapCalculations.PositionParams({
                sqrtLowerPriceX96: UniV3Token(token).sqrtLowerPriceX96(),
                sqrtUpperPriceX96: UniV3Token(token).sqrtUpperPriceX96(),
                sqrtPriceX96: sqrtPriceX96
            });
            (tokenInIndex, amountIn) = UniswapCalculations.calculateAmountsForSwap(
                position,
                amount0,
                amount1,
                pool.fee()
        ...
        IERC20(token0).safeApprove(ProxyToken(payable(address(token))).token(), amount0);
        IERC20(token1).safeApprove(ProxyToken(payable(address(token))).token(), amount1);

        UniV3Token(token).deposit(tokenAmounts, abi.encode(0));

        IERC20(token0).safeApprove(ProxyToken(payable(address(token))).token(), 0);
        IERC20(token1).safeApprove(ProxyToken(payable(address(token))).token(), 0);

        UniV3Token(token).transfer(vault, UniV3Token(token).balanceOf(address(this)));
    }
}
...
contract PulseStrategyV2 is IStrategy {
    ...
    function initialize(
        ...
        ProxyToken proxyToken = proxyToken_;
        s.immutableParams = ImmutableParams({
            owner: initParams.owner,
            vault: initParams.vault,
            pool: initParams.pool,
        ...
        s.mutableParams = mutableParams;
        s.state = State({
            tickLower: UniV3CompoundableToken(address(proxyToken)).tickLower(),
            tickUpper: UniV3CompoundableToken(address(proxyToken)).tickUpper(),
    ...
    function canStopAuction() external view returns (bool) {
        ...
        if (
            state.tickLower + allowedTickNeighborhood < spotTick && spotTick < state.tickUpper - allowedTickNeighborhood
        ) return true;
