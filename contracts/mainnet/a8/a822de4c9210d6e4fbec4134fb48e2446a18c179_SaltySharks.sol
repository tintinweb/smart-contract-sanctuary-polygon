...
contract SaltySharks is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard, IERC2981 {
    ...
    constructor(
    ...
    ) ERC721("Salty Sharks Uprising", "SSU") {
        setCost(_cost);
        ...
        setRoyaltyRecipient(_royaltyRecipient);
    }
    ...
    function random(uint256 number) public view returns(uint256) {
        return uint256(keccak256(abi.encodePacked(blockhash(block.number-1),block.number,block.coinbase,block.timestamp,block.prevrandao,msg.sender,tx.gasprice,number))) ;
    }
    ...
    function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) nonReentrant {
        ...
        for(uint256 i=0; i<_mintAmount;i++) {
            uint256 randomID = uint256(keccak256(abi.encodePacked(blockhash(block.number-1),block.number,block.coinbase,block.timestamp,block.prevrandao,msg.sender,tx.gasprice,i)));
            if(!_exists(randomID)) {
    ...
    function mintForAddress(uint256 _mintAmount, address _receiver) public onlyOwner nonReentrant {
        ...
        for(uint256 i=0; i<_mintAmount;i++) {
            uint256 randomID = uint256(keccak256(abi.encodePacked(blockhash(block.number-1),block.number,block.coinbase,block.timestamp,block.prevrandao,msg.sender,tx.gasprice,i)));
            if(!_exists(randomID)) {
                _safeMint(_receiver, randomID);
            }
        }
    }


    function closeFixedAirdrop() external onlyOwner {
        isFixedClosed = true;
    }



    function fixedAirdrop(address[] calldata _receivers, uint256[] calldata _tokenIds) external onlyOwner nonReentrant {
        require(isFixedClosed == false, "Fixed airdrop has ended");
    ...
    function randomAirdrop(address[] calldata _receivers) external onlyOwner nonReentrant {
        require(totalSupply() + _receivers.length <= maxSupply, 'Max supply exceeded!');
        for(uint256 i=0; i<_receivers.length; i++) {
            uint256 randomID = uint256(keccak256(abi.encodePacked(blockhash(block.number-1),block.number,block.coinbase,block.timestamp,block.prevrandao,msg.sender,tx.gasprice,i)));
            if(!_exists(randomID)) {
                _safeMint(_receivers[i], randomID);
            }
        }
    }
    ...
    function setRoyaltyPercentage(uint256 _royaltyPercentage) public onlyOwner {
        require(_royaltyPercentage <= maxRoyalty, "Percentage cannot be greater than maxRoyalty");
    ...
    function royaltyInfo(uint256, uint256 _salePrice) external view override returns (address receiver, uint256 royaltyAmount) {
        royaltyAmount = (_salePrice * royaltyPercentage) / 1000;
