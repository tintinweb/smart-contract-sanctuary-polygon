...
// File: contracts/BCnewyear2023.sol
...
contract BCnewyear2023 is ERC1155Supply, Ownable {
...
//    mapping (uint256 => string) nfttag;
    ...
    string constant rl2='{"name": "BlueChip NewYear2023 Soulbound NFT';
    string constant rl3='", "description": "';
    string constant rl4='", "image": "';
    string constant tr1='", "attributes": [{"trait_type": "Happy","value": "MAX"},{"trait_type": "Eto","value": "rabbit';
    string constant tr2='"},{"trait_type": "Attack","value": "';
    string constant tr3='"},{"trait_type": "Defence","value": "';
    string constant tr4='"},{"trait_type": "Speed","value": "';
    string constant tr5='"},{"trait_type": "Minted Time(JST)","value": "';
    string constant tr6='"}]}';
    string constant date="2023/1/";

    string private ra1='DD';
    string private ra2='EE';
    string private ra3='FF';
...
//    }
    ...
    function getCurrentTime() public view returns (string memory){
        ...
        uint256 day = ((block.timestamp - 1671807600) / 86400 ) % 365 -7;
    ...
    function ownermint(address _to, uint256 _id, uint256 _amount) public onlyOwner {
        require(tx.origin == msg.sender, "Cannot mint from contracts");
        require(_to != address(0), "ERC1155: mint to the zero address");
        _mint(_to, _id, _amount,"");
    }

    // 1st multi mint by owner or operator
    function multiMint(address[] memory _tos , uint256 _id, uint256[] memory _amounts) public onlyOwner{
        require(tx.origin == msg.sender, "Cannot mint from contracts");
        ...
        for (uint256 i = 0; i < _amounts.length; i++) {
            nftowner[_tos[i]] = string(abi.encodePacked(" holder address is ",Strings.toHexString(uint256(uint160(_tos[i])), 20),"  and minted time(JST) is ",minttime));
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        require(tx.origin == msg.sender, "Cannot approve from contracts");
        require(msg.sender == address(0), "This SBT cannnot list on any market");
        _setApprovalForAll(_msgSender(), operator, approved);
    }
    ...
    // generate randomize number from string
    function random(string memory input) internal pure returns (uint256) {
        return uint256(keccak256(abi.encodePacked(input)));
    }

    // generate randomized value for each trait
    function randomOne(uint256 id) internal view returns (Traits memory) {
        Traits memory traits;
        traits.att = uint8(random(string(abi.encodePacked(ra1,Strings.toString(id)))) % 999);
        traits.def = uint8(random(string(abi.encodePacked(ra2,Strings.toString(id)))) % 999);
        traits.spe = uint8(random(string(abi.encodePacked(ra3,Strings.toString(id)))) % 999);
        return traits;
    }

    // get generated traits
    function getTraits(Traits memory traits) public virtual view returns(string memory) {
        string memory o=string(abi.encodePacked(tr1,tr2,Strings.toString(traits.att),tr3,Strings.toString(traits.def),tr4,Strings.toString(traits.spe),tr5));
        return o;
    }
    ...
    function getAttributes(uint256 id) public virtual view returns(string memory) {
        ...
        return string(abi.encodePacked(rl2,rl3,nftcomment[id],holder,rl4,nftimage[id]));
    }


    // refer uri by token id with the generated metadata json file
    function uri(uint256 id) public view override returns (string memory) {
        Traits memory traits = randomOne(id);
        return string(abi.encodePacked(rl1,Base64.encode(bytes(string(abi.encodePacked(getAttributes(id),getTraits(traits),mintedtime[id],tr6))))));
    }
