...
contract HFunds is HandlerBase {
    ...
    function sendTokens(
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (amount > 0) {
                // ETH case
                if (tokens[i] == address(0) || tokens[i] == NATIVE_TOKEN_ADDRESS) {
            ...
            if (tokens[i] == address(0)) {
                if (address(this).balance < amounts[i]) {
                    string memory errMsg = string(
                        abi.encodePacked("error: ", _uint2String(i), "_", _uint2String(address(this).balance))
                    );
                    _revertMsg("checkSlippage", errMsg);
                }
            } else if (IERC20(tokens[i]).balanceOf(address(this)) < amounts[i]) {
                string memory errMsg = string(
                    abi.encodePacked(
                        "error: ",
                        _uint2String(i),
