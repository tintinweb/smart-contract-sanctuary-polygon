...
contract RunbitWallet is AccessControl {
    ...
    function claimBonus(address tech, address community) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function updateTokens(uint256 newIndex) external onlyRole(OPERATE_ROLE) {
        ...
        for (uint i = currentIndex; i < newIndex; ++i) {
            ...
            if (tokenId !=0 && _gets(tokenId) == 0) {
                ...
                _set(tokenId, dayEarn * (meta.durability / 100) / 100000000000000);
                sumIncrease += dayEarn;
    ...
    function _getMinEarnPrice(IRunbitCollection.ShoesCollection memory colection) private view returns (uint256) {
