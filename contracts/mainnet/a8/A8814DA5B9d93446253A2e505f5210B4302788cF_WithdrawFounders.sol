...
interface IVaultCapital {
    ...
    function getPaid() external view returns(uint256 _borrows);
...
import "./Calcs.sol";
import "./IFounders.sol";
import "./IVaultCapital.sol";
import "../NFTContracts/ICollectionNFT.sol";

/// @title Contract to Withdraw Money in Founders section.
/// @author Panoram Finance.
contract WithdrawFounders is AccessControl, ReentrancyGuard, IFounders, Calcs {

    bytes32 public constant DAO_ROLE = keccak256("DAO_ROLE");
    
    // Events emitted
    event Request(address wallet, uint256 _foundersId, uint256 idRequest, uint256 amount);
    event Withdraw(uint256 _foundersId, uint256 idSuccess,address wallet, uint256 amount, address token);
    ...
    uint128 private lockedDays = 90 days;
    uint128 private exitPrice;
    uint256 private constant MAX_UINT = 2**255;
    
     struct localWithdraw {
        uint32 _quantity;  
        uint256 foundersId; 
        address _foundersCollection; 
        address _token;
        uint256 unlock;
        uint256 withdraw;
        uint256 realAmount;
    ...
    ///@param _id is the request withdraw ID
    modifier cancel(uint256 _id) {
        if(!FoundersControl.validateId(msg.sender, _id) && !hasRole(DAO_ROLE, msg.sender)){
            revert("without permission");
        }
        _;
    }

    /// @dev Modifier to validate that only the authorized entity can complete a withdrawal.
    modifier authorized() {
    ...
    constructor(address _tokenInfo, address _vaultFounders,address _foundersControl, address _token, uint128 _exitPrice)
     Calcs(_foundersControl) {
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(DEV_ROLE, msg.sender);
        _setupRole(DEV_ROLE, 0xb818dd90d667D6D269FA7a690114f98473aF7b95); //upgrade to multi-sig on mainnet
        _setupRole(DAO_ROLE, 0xb818dd90d667D6D269FA7a690114f98473aF7b95); //upgrade to multi-sig on mainnet
        permissions(_vaultFounders);
        exitPrice = _exitPrice;
     }

     /// @dev Function to create a founders withdrawal request, the user must first create this request 
    /// @param _quantity - amount of NFT to be withdrawn.
    /// @param foundersId - the user's Founders ID.
    /// @param _foundersCollection - Address of the Founders' collection corresponding to the stage
    function setWithdraw(uint256 _quantity, uint256 foundersId, address _foundersCollection, address _token) public NotPaused nonReentrant {
       localWithdraw memory locals;
       locals._quantity = safeCastToUint32(_quantity);
       locals.foundersId = foundersId;
       locals._foundersCollection = _foundersCollection;
       locals._token = _token;
       if(locals._quantity == 0){
            revert ("greater than zero");
        }
        locals.unlock = FoundersControl.getDepositTime(msg.sender, foundersId) +  lockedDays;
        if(locals.unlock > block.timestamp){
            revert("too soon");
        }
        if(FoundersControl.getHaveRequest(msg.sender)){
            revert("open request");
        }
        (uint256 amount,,,,,, bool isFullyPaid) = FoundersControl.getFoundersInfo(msg.sender, foundersId);
        if(amount == 0){
            revert("not your id");
        }
        if(isFullyPaid){
        ...
        locals.withdraw = _quantity * exitPrice;
        
       (,,address vCapital) = tokenInfo.getFoundersVaults(_token);

       if(locals.withdraw > IVaultCapital(vCapital).getAvaible()){
            revert("not avaible");
       }    
        
        updateDaysWithdraw(foundersId,msg.sender, locals.withdraw, amount, locals._foundersCollection);
        IVaultCapital(vCapital).addRequest(locals.withdraw);
        locals.realAmount = realamount(locals._quantity,locals._foundersCollection);
        FoundersControl.claimMoney(foundersId, msg.sender, locals.withdraw, locals.realAmount);
        FoundersControl.addHaveRequest(msg.sender);
        uint256 _id = FoundersControl.createRequest(msg.sender,locals.withdraw,0,1, locals._quantity);

        emit Request(msg.sender, foundersId,_id,locals.withdraw);
    }
    ...
    /// @param idFounders - The user's Lending ID.
    function completeWithdraw(uint256 idrequest, uint256 idFounders, address _user, address _token,address _foundersCollection) public NotPaused authorized nonReentrant {
        (uint256 _quantity,,Status _state,,uint96 _numNFT)  = FoundersControl.getRequest(_user, idrequest);
        ...
        (,,address vCapital) = tokenInfo.getFoundersVaults(_token);
        FoundersControl.closeRequest(_user, idrequest, Status.complete, idFounders);
        IVaultCapital(vCapital).claimRequest(_quantity);
        FoundersControl.resetClaimed(idFounders, _user);
        updateNFT(_user,idFounders, _foundersCollection, _numNFT);
        IERC20(token).safeTransfer(_user, _quantity);
        emit Withdraw(idFounders, idrequest,_user,_quantity, address(token));
    }

    function completeWithdrawInternal(uint256 idrequest, uint256 idFounders, address _user, address _token,address _foundersCollection) private {
        (uint256 _quantity,,Status _state,,uint96 _numNFT)  = FoundersControl.getRequest(_user, idrequest);
    ...
    function batchCompleteWithdraw(uint256[] memory idrequest, uint256[] memory idFounders, address[] memory _user, address _token,address _foundersCollection) public NotPaused authorized nonReentrant {
        uint256 idLength = idrequest.length;
        if(idLength > 100 ){
            revert ("many records");
        }
        if(idLength != idFounders.length || idLength != _user.length){
            revert("array mismatch");
        }
        for (uint256 i = 0; i < idLength; ){
            completeWithdrawInternal(idrequest[i], idFounders[i], _user[i], _token, _foundersCollection);
            unchecked {
                ++i;
            }
        }
    ...
    /// @param idFounders - The user's Lending ID.
    /// @param _foundersCollection - Address of the Founders' collection corresponding to the stage
    function cancelWithdraw(uint256 idRequest, address _user, uint256 idFounders, address _foundersCollection, address _token) public cancel(idRequest) {
        (uint256 _quantity,,Status _state,,uint96 _numNFT)  = FoundersControl.getRequest(_user, idRequest);
        if(_state != Status.pending){
            revert("Request not pending");
        }
        (uint256 amount,,,,,,) = FoundersControl.getFoundersInfo(msg.sender, idFounders);
        (,,address vCapital) = tokenInfo.getFoundersVaults(_token);
        uint256 _realAmount = realamount(_numNFT, _foundersCollection);
        updateDaysCancel(idFounders,msg.sender, amount, _foundersCollection, _realAmount); //update rewards before cancel request
        IVaultCapital(vCapital).deleteRequest(_quantity);
        FoundersControl.updateMoney(idFounders, _user, _realAmount);
        FoundersControl.closeRequest(_user,idRequest,Status.cancelled, idFounders);

        emit Cancel(_user, idRequest, msg.sender); // emit event Cancel
    }

    function realamount(uint256 nftAmount, address _collection) private view returns(uint256 realAmount){
        uint256 _price = ICollectionNFT(_collection).getPrice();
        return nftAmount * _price;
    }

    function updateDaysWithdraw(uint256 id,address _wallet, uint256 _amount, uint256 lastAmount, address _collection ) private {
        uint256 totalAmount;
        uint256 payPersecond;
        lastAmount > _amount ? totalAmount = lastAmount - _amount: totalAmount = _amount - lastAmount;
    
        uint256 endFoundersDate = uint256(FoundersControl.getEndFoundersDay(_wallet, id));
        uint256 timeOfCalc = FoundersControl.UpdateInteresAccumulated(_wallet, id);//update rewards before update deposit
        if(_amount > lastAmount){
            payPersecond = 0; 
        } else {
            payPersecond = calcInterestForSecond(totalAmount, FoundersControl.getCollectionToInterest(_collection));
        }
        uint256 _totalToPaid;
        if(timeOfCalc <= endFoundersDate){
            _totalToPaid = calcTotalPayInterest(endFoundersDate, timeOfCalc,  payPersecond);
        }
        FoundersControl.updatePayPerSecond(_wallet, id , payPersecond, _totalToPaid);
    }

    function updateDaysCancel(uint256 id,address _wallet, uint256 lastAmount, address _collection, uint256 _realAmount) private {
        uint256 totalAmount = lastAmount + _realAmount;
        uint256 endFoundersDate = uint256(FoundersControl.getEndFoundersDay(_wallet, id));
        uint256 timeOfCalc =  FoundersControl.UpdateInteresAccumulated(_wallet, id);//update rewards before update deposit
        uint256 payPersecond = calcInterestForSecond(totalAmount, FoundersControl.getCollectionToInterest(_collection)); 
        uint256 _totalToPaid;
        if(timeOfCalc <= endFoundersDate){
            _totalToPaid = calcTotalPayInterest(endFoundersDate, timeOfCalc,  payPersecond);
        }
    ...
    function updateNFT(address _wallet, uint256 _idFounders, address _foundersCollection, uint256 _nftAmount) private {
        (uint256 _startId,uint256 _finalId, uint256 _number) = FoundersControl.getNFTInformation(_wallet,_idFounders);
        if(_nftAmount < _number){
        uint256 newQuantity;
        _nftAmount < _number ? newQuantity = (_number - _nftAmount) : newQuantity = (_nftAmount -_number);
    
        for(uint i = 0; i < _nftAmount;){
            ICollectionNFT(_foundersCollection).burn(_startId);
            ++_startId;
            unchecked{
                ++i;
            }
        } 
            FoundersControl.updateNFTInformation(_wallet, _idFounders,newQuantity,_startId,_finalId);
        }else{
        for(uint i = 0; i < _number;){
            ICollectionNFT(_foundersCollection).burn(_startId);
            ++_startId;
            unchecked{
                ++i;
            ...
            FoundersControl.updateNFTInformation(_wallet, _idFounders,0,0,0);
        }
    }

    function getLockedDays() public view returns(uint256) {
        return lockedDays;
    }

    function updateLockedDays(uint128 _newLockedDays) public onlyDev {
        lockedDays = _newLockedDays;
    }

    function updateExitPrice(uint128 _newExitPrice) public onlyDev {
        exitPrice = _newExitPrice;
    }
    

    function permissions(address _founders) public onlyDev validToken(address(token)) {
        IERC20(token).safeApprove(_founders, MAX_UINT);
    }
    ...
    function safeCastToUint32(uint256 value) private pure returns(uint32){
...
contract TokenInfo is AccessControl {
    ...
    struct Vaults{
        ...
        address membershipVault;
        address referalVault;
    }

    struct FoundersVaults{
        address vFounders;
    ...
    //token address => vaults for Founders Phase.
    mapping(address => FoundersVaults) internal vaultsFounders;
    ...
    function addVaultRegistry(address _token, address _lender,address _lenderRewards, address _membershipVault, address _referalVault) public onlydev  {
        vaultsInfo[_token].lender = _lender;
        vaultsInfo[_token].lenderRewards = _lenderRewards;
        vaultsInfo[_token].membershipVault = _membershipVault;
        vaultsInfo[_token].referalVault = _referalVault;
    }

    /// @dev Function to add the Vaults used for Founders.
    function addFoundersVaults(address _token, address _founders,address _FoundersRewards, address _vCapital) public onlydev  {
        vaultsFounders[_token].vFounders = _founders;
        vaultsFounders[_token].vFounderRewards = _FoundersRewards;
        vaultsFounders[_token].vCapital = _vCapital;
    }
    ...
    function removeVaultRegistry(address _token) public onlydev  {
        ...
        vaultsInfo[_token].membershipVault = address(0);
        vaultsInfo[_token].referalVault = address(0);
    }

    function getVaultInfo(address _token) public view returns(address _lender, address _lenderRewards, address _membershipVault, address _referalVault ){
        return ( vaultsInfo[_token].lender,
        vaultsInfo[_token].lenderRewards, vaultsInfo[_token].membershipVault, vaultsInfo[_token].referalVault);
    }

   function getFoundersVaults(address _token) public view returns(address , address, address){
        return (
            vaultsFounders[_token].vFounders,
            vaultsFounders[_token].vFounderRewards,
            vaultsFounders[_token].vCapital
        );
    }
