...
@title 2112 Registry
@author 2112.run
@notice This contract resolves addresses of the ecosystem
*/
contract Registry is UpgradeableBase {
    /**
    @notice Mapping of address by key
    */
    mapping(bytes32 => address) internal registry;
    /**
    @notice Mapping of key by address
    */
    mapping(address => bytes32) internal keys;
    /**
    @notice Emitted when an entry changes
    @param sender The msg.sender
    @param key The key to resolve an address
    @param previousAddress The previous address of the key
    @param newAddress The new address of the key
    */
    ...
    @param sender The msg.sender
    @param key The key used to resolve
    @param previousAddress The address corresponding to the key
    */
    ...
    @param sender The msg.sender
    @param key The key used to resolve the address
    @param newAddress The address resolved by the key
    */
    ...
    */
    ...
    /**
    @notice Changes the value for an entry
    @param key_ The key of the address to be resolved
    @param address_ The address of the key
    */
    function setEntry(bytes32 key_, address address_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if(address_ == address(0))
            revert CannotSetZeroAddress();
    ...
    /**
    @notice Removing an entry from the registry
    @param key_ The key of the entry to be removed
    */
    function removeEntry(bytes32 key_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        address previousValue = registry[key_];
    ...
    /**
    @notice Resolve an address by the key
    @param key_ The key used for resolution
    */
    function getEntry(bytes32 key_) external view returns (address) {
        return registry[key_];
    }
    /**
    @notice Get a key for an address in the registry
    @param address_ The address used for resolution
    */
    ...
    @dev Uses a signature to validate it's from the ecosystem
    @param key_ The key used for resolution
    @param nonce_ Used to prevent signature re-use
    @param signature_ The signature to validate
    */
    function register(bytes32 key_, uint256 nonce_, bytes memory signature_) external {
        address signer = _isUpgrader(signature_, nonce_, abi.encodePacked(nonce_, key_));
        registry[key_] = msg.sender;
        keys[msg.sender] = key_;
        emit Registered(signer, key_, msg.sender);
    }
}
