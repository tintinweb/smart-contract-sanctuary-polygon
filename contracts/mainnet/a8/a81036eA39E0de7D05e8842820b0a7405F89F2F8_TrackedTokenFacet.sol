...
contract TrackedTokenFacet is Modifiers {
    ...
    function removeTrackedToken(address _addr, address _trackedAddr) external onlyTokenManager(_addr) whenNotPaused {
        ...
        require(
            !LibPriceInterface.amountIsSignificant(_trackedAddr, IERC20(_trackedAddr).balanceOf(_addr)),
            "Can't remove tracked token with significant balance"
        );
    ...
    function redeemToTrackedTokens(address _addr, uint256 _amount) external whenNotPaused {
    ...
    function issueFromTrackedTokens(address _addr, uint256 _amount) external whenNotPaused {
