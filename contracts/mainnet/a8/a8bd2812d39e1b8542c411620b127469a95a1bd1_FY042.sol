...
uint dia=0; uint aviso=0;     uint numero=0;       uint internal num;    

function worker() public hasFarm returns (uint dias, uint mensaje)          {

uint inTrabajo = inicio[msg.sender] + tiempo*(dia+1);
if(block.timestamp >= inTrabajo)             {           
    numero = uint( keccak256( abi.encode(block.timestamp, msg.sender, num) ) ) % 18;
    numero < 1 ? numero=numero+5: numero;
    aviso=inTrabajo+(numero*3600);
    dia=dia+1;          }
        return (inTrabajo, aviso);                                          }


uint paga=1*(10**18);
function resta() public hasFarm returns (uint bal)      {

uint balance = token.balanceOf(msg.sender);

if (balance > 1 ) {token.burn(msg.sender, paga);}
        return balance;                                 }
