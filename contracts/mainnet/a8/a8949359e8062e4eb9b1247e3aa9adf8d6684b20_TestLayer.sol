...
contract TestLayer {
    ...
    uint256 private constant lotteryEntryFee = 10e6;
    ...
    event LotteryEntry(uint256 time, address user, uint256 number);
    ...
    function lotteryEntry(uint256 _number) external {
        ...
        require(userRewards.lottery >= lotteryEntryFee, "insufficient lottery");
        userRewards.lottery = userRewards.lottery.sub(lotteryEntryFee);
        allLotteryRecord[dayNow][_number].push(msg.sender);
        userLotteryRecord[msg.sender].push(LotteryRecord(block.timestamp, _number));
        emit LotteryEntry(block.timestamp, msg.sender, _number);
    }
