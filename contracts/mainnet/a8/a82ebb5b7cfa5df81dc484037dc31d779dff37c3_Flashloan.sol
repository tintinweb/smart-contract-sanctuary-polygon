...
contract Flashloan {
    ...
    function FlashLoan(
        ...
        address flashLoanBase = IPair(flashLoanPool)._BASE_TOKEN_();
        if(flashLoanBase == loanToken) {
            IPair(flashLoanPool).flashLoan(loanAmount, 0, address(this), data);
        } else {
            IPair(flashLoanPool).flashLoan(0, loanAmount, address(this), data);
        }
    }
