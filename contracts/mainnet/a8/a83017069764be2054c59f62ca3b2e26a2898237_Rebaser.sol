...
abstract contract Token is Context, IBEP20 {
    ...
    string private constant _name   = "Botcoin";
...
contract Rebaser is Token {
    ...
    //rebases
    uint256 public rewardYield           = 100; //<------------------- 10 percent (should be 1% in production build)
    ...
    //exceptions
    mapping(address => bool) private noRebase; //<-------------------- for liquidity pool contracts (set to true)
    ...
    function getNoRebase(address atAddress) public view returns (bool) {
        return noRebase[atAddress];
    }

    function setNoRebase(address atAddress, bool toValue) external onlyAdmin {
        noRebase[atAddress] = toValue;
    }

    function balanceOf(address account) public view virtual override returns (uint256) {
        if(getNoRebase(account)) {
            return _balances[account].div(BIG_INT);
        }
        else {
