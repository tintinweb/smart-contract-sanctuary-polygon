...
// File: contracts/Collection2.sol
...
contract Collection is Ownable {
  ...
  string public name = "Moon River";
  uint public collectionID = 2;
  uint256 public cost=0 ether;
  address public BpaperContractAddr = 0x4E943D8155018816D070D00715894A3496F75922;
  Bpaper BpaperContract= Bpaper(BpaperContractAddr);
  address public BcardContractAddr = BpaperContract.BcardContractAddr();
  address public designAddr = 0xD848dDDF852D75c734b1447Cac5749cb043Cc252;
  address public CheckQuaAddr = 0xF782BC2632Fb92Ec6048a2aA67552869FF357547;
  address public dataAddr = 0x19853E5f10DD32EC0139191fC6f9973988f71C50;
  address public AdAddr;

  // public
  function mintNew(string memory _Title,string[] memory _tagName, string[] memory _tagValue,string memory _Content, 
    uint _Amount, address cardholder) public payable {  
    CheckQua checkContract = CheckQua(CheckQuaAddr);
    uint maxMint = checkContract.checkConNewMint(msg.sender);
    ...
    dataContract.newData(totalToken, _Title, _tagName, _tagValue, _Content, BpaperMinterID, maxMint, getDate(block.timestamp));
  ...
  function updateContent(uint _PaperID, string memory _Title, string[] memory _tagName, string[] memory _tagValue,string memory _Content) public {
    if (msg.sender != owner() && msg.sender != AdAddr) {
      CheckQua checkContract = CheckQua(CheckQuaAddr);
    ...
    Bcard BcardContract = Bcard(BcardContractAddr);
    uint BpaperMinterID = BcardContract.AddressToTokenID(msg.sender);
    string memory tempURI = designContract.buildMetadata(
        _Title, _tagName, _tagValue, getDate(block.timestamp), _Content, BpaperMinterID, 
        getBcardEthName(BpaperMinterID));

    //update new data in database
    Data dataContract = Data(dataAddr);
    dataContract.editData(_PaperID, _Title, _tagName, _tagValue, _Content, getDate(block.timestamp));
    dataContract.updateUri(_PaperID, tempURI);
    BpaperContract.buildMetaData(_PaperID);
  }

  function PreviewTokenURI(uint _BcardID, string memory _title, string[] memory _tagName, string[] memory _tagValue,
    string memory _content) public payable returns (string memory){
        string memory minterBcardEthName=getBcardEthName(_BcardID);
        Design designContract = Design (designAddr);
        return designContract.buildMetadata(_title, _tagName, _tagValue, getDate(block.timestamp), _content, 
          _BcardID, minterBcardEthName);
  }
...
abstract contract Data is Ownable {
  struct Paper { 
    string title;
    string[] tagName;
    string[] tagValue;
  ...
  function newData(uint _BpaperID, string memory _Title,string[] memory _tagName, string[] memory _tagValue,string memory _Content, 
    uint _BpaperMinterID, uint _maxMint, string memory _date) public payable {  
  }

  function editData(uint _BpaperID, string memory _Title,string[] memory _tagName, string[] memory _tagValue,string memory _Content, 
    string memory _date) public payable {  
  }
...
abstract contract Design is Ownable{
    function buildMetadata(string memory _title, string[] memory _tagName, string[] memory _tagValue, string memory _editTime, string memory _content, uint _tkId, string memory _ethName) 
...
abstract contract CheckQua is Ownable{
    ...
    function checkConNewMint(address _msgSender) public view returns(uint) {  
    }
