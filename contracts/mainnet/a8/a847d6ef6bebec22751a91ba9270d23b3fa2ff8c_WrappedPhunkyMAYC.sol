...
/// @title Main contract for WrappedPhunkyMAYC. This contract converts PMAYC between the ERC721 standard and the
///  ERC20 standard by locking mutants into the contract and minting 1:1 backed ERC20 tokens, that
///  can then be redeemed for mutants when desired.
/// @notice When wrapping a mutant, you get a generic WPMAYC token. Since the WPMAYC token is generic, it has no
///  no information about what mutant you submitted, so you will most likely not receive the same mutant
///  back when redeeming the token unless you specify that mutant's ID. The token only entitles you to receive 
///  *a* mutant in return, not necessarily the *same* mutant in return. A different user can submit
///  their own WPMAYC tokens to the contract and withdraw the mutant that you originally deposited. WPMAYC tokens have
///  no information about which mutant was originally deposited to mint WPMAYC - this is due to the very nature of 
///  the ERC20 standard being fungible, and the ERC721 standard being nonfungible.
contract WrappedPhunkyMAYC is ERC20, ReentrancyGuard {
    ...
    /// @dev This event is fired when a user deposits mutants into the contract in exchange
    ///  for an equal number of WPMAYC ERC20 tokens.
    /// @param mutantId  The cryptomutant id of the mutant that was deposited into the contract.
    event DepositMutantAndMintToken(
        uint256 mutantId
    );

    /// @dev This event is fired when a user deposits WPMAYC ERC20 tokens into the contract in exchange
    ///  for an equal number of locked cryptomutants.
    /// @param mutantId  The mutant id of the mutant that was withdrawn from the contract.
    ...
    /// @dev An Array containing all of the mutants that are locked in the contract, backing
    ///  WPMAYC ERC20 tokens 1:1
    ...
    /// @dev The metadata details about the "Wrapped Mutants" WPMAYC ERC20 token.
    uint8 constant public decimals = 18;
    string constant public name = "Wrapped Punky Mutants";
    string constant public symbol = "WPMAYC";

    /// @dev The address of official Phunky Mutants contract that stores the metadata about each cat.
    /// @notice The owner is not capable of changing the address of the Phunky Mutants Core contract
    ///  once the contract has been deployed.
    address public mutantCoreAddress = 0xdFF760Ec52a498464c670B680DC81e6861f10BAd;
    ...
    /// @notice Allows a user to lock mutants in the contract in exchange for an equal number
    ///  of WPMAYC ERC20 tokens.
    /// @param _mutantIds  The ids of the mutants that will be locked into the contract.
    /// @notice The user must first call approve() in the mutants Core contract on each mutant
    ///  that thye wish to deposit before calling depositMutantsAndMintTokens(). There is no danger 
    ///  of this contract overreaching its approval, since the Mutants Core contract's approve() 
    ///  function only approves this contract for a single mutant. Calling approve() allows this 
    ...
    function depositMutantsAndMintTokens(uint256[] calldata _mutantIds) external nonReentrant {
        ...
        for(uint i = 0; i < _mutantIds.length; i++){
            ...
            require(msg.sender == mutantCore.ownerOf(mutantToDeposit), 'you do not own this mutant');
            require(mutantCore.getApproved(mutantToDeposit) == address(this), 'you must approve() this contract to give it permission to withdraw this mutant before you can deposit a mutant');
    ...
    /// @notice Allows a user to burn WPMAYC ERC20 tokens in exchange for an equal number of locked 
    ///  mutants.
    ...
    function burnTokensAndWithdrawMutants(uint256[] calldata _mutantIds, address[] calldata _destinationAddresses) external nonReentrant {
        require(_mutantIds.length == _destinationAddresses.length, 'you did not provide a destination address for each of the mutants you wish to withdraw');
        require(_mutantIds.length > 0, 'you must submit an array with at least one element');

        uint256 numTokensToBurn = _mutantIds.length;
        require(balanceOf(msg.sender) >= numTokensToBurn.mul(10**18), 'you do not own enough tokens to withdraw this many ERC721 mutants');
        ...
        for(uint i = 0; i < numTokensToBurn; i++){
            ...
            if(mutantToWithdraw == 0){
                ...
                require(address(this) == mutantCore.ownerOf(mutantToWithdraw), 'the contract does not own this mutant');
                mutantIsDepositedInContract[mutantToWithdraw] = false;
            }
            mutantCore.transferFrom(address(this),_destinationAddresses[i], mutantToWithdraw);
    ...
    function _popMutant() internal returns(uint256){
        require(depositedMutantsArray.length > 0, 'there are no mutants in the array');
...
/// @title Interface for interacting with the Mutants Core contract.
contract MutantCore {
