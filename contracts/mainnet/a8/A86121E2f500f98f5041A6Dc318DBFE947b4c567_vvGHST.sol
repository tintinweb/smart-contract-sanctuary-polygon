...
contract vvGHST is Initializable, ERC20Upgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    // claim vGhst by burning vvHST -- this function is largely from QiDao code
    ...
    function leave(uint256 _share) public {
        if(_share>0){
            ...
            QiVault(s.vaultAddress).withdrawCollateral(s.vaultID, vghstAmount.sub(vghst_feeAmount.mul(200).div(400)));
            ...
            if(vQiToClaim > 0){
                uint256 vQi_feeAmount = vQiToClaim.mul(s.withdrawalFeeBP).div(10000); 
                IERC20Upgradeable(im_vQi).transfer(s.owner, vQi_feeAmount.mul(200).div(400));
                IERC20Upgradeable(im_vQi).transfer(s.contractCreator, vQi_feeAmount.mul(200).div(400));
