...
contract MissionControl is IMissionControl, OwnableUpgradeable, EIP712Upgradeable {
    ...
    function _handle_raid_collect(HandleRaidCollectInputs memory args)
        private
        validCoordinates(args.x, args.y, -(args.x + args.y))
        returns (uint256, uint256)
    {
        HandleRaidCollectVars memory vars;

        vars.tileElement = tiles[args.defender][args.x][args.y][__zeroOrOne(args.x, args.y, args.defender)];
        vars.tileRentalInfo = tileRentalInfo[args.defender][args.x][args.y];
        vars.iStakeable = IMissionControlStakeable(vars.tileElement.stakeable);

        if (!vars.iStakeable.isRaidable(vars.tileElement.tokenId, args.x, args.y)) {
            return (0, WASTE_ID);
        }

        return
            vars.iStakeable.onCollect(
                args.defender,
                vars.tileElement.nonce,
                vars.tileRentalInfo.pausedAt,
                args.x,
