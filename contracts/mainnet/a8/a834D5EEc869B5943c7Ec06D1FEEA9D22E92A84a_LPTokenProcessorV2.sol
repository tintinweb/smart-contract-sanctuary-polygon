...
contract LPTokenProcessorV2 is ILPTokenProcessorV2, KeeperCompatible, AccessControlEnumerable {
    ...
    uint24 public constant wethToUsdV3PoolFee = 3000; // 0.3%
    ...
    constructor(
        ...
        require(routerAddressForFloki != address(0), "LPTokenProcessorV2::constructor::ZERO: Router cannot be zero address.");
        require(usdtAddress != address(0), "LPTokenProcessorV2::constructor::ZERO: USDT cannot be zero address.");
    ...
    function _processTokenSwapping(uint256 index) private returns (bool) {
        ...
        if (_isV2LPToken[info.tokenAddress]) {
            // V2 LP Tokens
            console.log("V2 Tokens");
            _swapV2TokenByUSDT(info.tokenAddress, info.amount, routerAddress);
        } else {
            // Regular ERC20 tokens
            console.log("Regular ERC20 Tokens");
            bool success = false;
            if (V2Routers[routerAddress]) {
                success = _swapTokensV2(info.tokenAddress, info.amount, USDT, address(this), routerAddress);
            } else {
                uint24[] memory poolFees = new uint24[](2);
                poolFees[0] = info.v3PoolFee;
                poolFees[1] = wethToUsdV3PoolFee;
                success = _swapTokensV3(info.tokenAddress, info.amount, USDT, poolFees, address(this), routerAddress);
            }
            require(success, "LPTokenProcessorV2::performUpkeep: Failed to swap ERC20 token by USDT.");
        }
    ...
    function _swapV2TokenByUSDT(
        ...
        require(routerAddress != address(0), "LPTokenProcessorV2::_swapV2TokenByUSDT: Unsupported router.");
        ...
        bool success = _swapTokensV2(token0, IERC20Metadata(token0).balanceOf(address(this)), USDT, address(this), routerAddress);
        require(success, "LPTokenProcessorV2::_swapV2TokenByUSDT: Failed to swap token0 to USDT.");
        success = _swapTokensV2(token1, IERC20Metadata(token1).balanceOf(address(this)), USDT, address(this), routerAddress);
        require(success, "LPTokenProcessorV2::_swapV2TokenByUSDT: Failed to swap token1 to USDT.");
    ...
    function _burnFloki(uint256 usdtAmount, address vault) private returns (uint256) {
        ...
        if (FLOKI != address(0)) {
            ...
            _swapTokensV2(USDT, burnShare, FLOKI, BURN_ADDRESS, routerForFloki);
    ...
    function swapTokens(
        ...
        require(V2Routers[routerAddress], "LPTokenProcessorV2::swapTokens: Unsupported router.");
        return _swapTokensV2(sourceToken, sourceAmount, destinationToken, receiver, routerAddress);
    }

    function _swapTokensV2(
        address sourceToken,
        uint256 sourceAmount,
        address destinationToken,
        address receiver,
    ...
    function _swapTokensV3(
        ...
        bytes memory path;
        if (sourceToken == WETH || destinationToken == WETH) {
            console.log("swapping V3 directly");
            path = abi.encodePacked(sourceToken, poolFees[0], destinationToken);
        } else {
            console.log("swapping V3 via WETH9");
