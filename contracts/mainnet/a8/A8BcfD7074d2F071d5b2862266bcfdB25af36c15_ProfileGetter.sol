...
contract ProfileGetter {
  ...
  IStakeBalance public stakeBalance;
  ...
  function _getSyntheticMarketBalance(
    ...
    uint256 _userMarketUpdateIndex;

    if (_useStakeBalance) {
      _userMarketUpdateIndex = staker.userNextPrice_stakedActionIndex(_marketIndex, _user);
    } else {
      _userMarketUpdateIndex = longShort.userNextPrice_currentUpdateIndex(_marketIndex, _user);
    }
  ...
  function _getMintShiftMarketPendingActions(
    ...
    if (
      ((longShort.userNextPrice_currentUpdateIndex(_marketIndex, _user) <=
        longShort.marketUpdateIndex(_marketIndex)) && !_useStakerValues) ||
  ...
  */
  ...
  ) private view returns (RedeemMarketPendingActions memory) {
