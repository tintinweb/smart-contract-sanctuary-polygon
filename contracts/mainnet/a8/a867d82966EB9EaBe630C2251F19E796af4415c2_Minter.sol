...
interface ITree{
    ...
    function mint(uint8 _numberToMint, uint16 _cause, uint256 _treeValue, address _user)external returns(uint256[] memory);
    function fertilise(uint256 _amount, uint256 _tokenId)external;
    function addValue(uint256 _valueToAdd, uint256 _tokenId, uint16 _causeId)external;
    function dataReturn(uint256 _tokenId)external view returns(uint16, uint256, uint8);

}
// File: contracts/ICauses.sol
...
interface ICauses {
    ...
    function contractSentToCause(uint16 _index, address _token, uint256 _amount)external;
    function causeDataCall(uint16 _index) external view returns(bool, address);
...
contract Minter is ReentrancyGuard, AccessControl{
    ...
    constructor(){
        tree = ITree(0xA919fBa8a22282057E93b4D380956C00914015d2); 
        cause = ICauses(0xf695F0e903B958D3720847E12d06Bb169beF217E);
        reserve = msg.sender;
        tokens.push();
        _setupRole(ADMIN_ROLE, msg.sender);
		_setupRole(GATEWAY_ROLE, msg.sender);

		// set the admin of GATEWAY_ROLE as the ADMIN_ROLE
    ...
    */
    function addToken(address _token, uint256 _treeCost)external onlyRole(ADMIN_ROLE){
     ...
     */
    function setAddress(address _reserve) external onlyRole(ADMIN_ROLE){
        reserve = _reserve;
    ...
    */
    function mintTree(uint8 _numberToMint, uint16 _cause, uint256 _valueToSpend, address _token) external nonReentrant returns(uint256[] memory){
        ...
        return tree.mint(_numberToMint, _cause, treeValue, user);
    ...
    */
    ...
    function internalTokenTransfer(address _token, uint256 amount, address _cause, address user, uint16 index)internal{
        ...
        uint256 causeA;
        uint256 fee;
        if(hasRole(GATEWAY_ROLE, msg.sender)){
            fee = num2 * base;
            causeA = (1000-num2) * base;
        }else{
            fee = num * base;
            causeA = (1000-num) * base;
        }
        token.safeTransferFrom(user, _cause, causeA);
        token.safeTransferFrom(user, reserve, fee);
    ...
    function changePercentage(uint256 _num, uint256 _num2)external onlyRole(ADMIN_ROLE){
        require(_num <= 200); // no more then 20%
        require(_num2 <= 100); // no more then 10%
        num = _num;
        num2 = _num2;
    }
