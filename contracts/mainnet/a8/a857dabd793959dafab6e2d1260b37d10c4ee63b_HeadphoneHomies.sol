    ...
    ReentrancyGuard
{
    ...
    event BatchRevealed(uint256 indexed seed, uint256 indexed tokenIdUntil);

    // Store a batch's end token + random value used as a seed
    struct ShuffleData {
        uint256 seed;
        uint256 endId;
    }
    ShuffleData[] _shuffleData;

    bool public isSaleActive;

    string public defaultTokenUri =
        "ipfs://bafybeiekmdmznainan2axwdqlwdaal6rj3f4di2osl73z457popr3ypqm4";
    string private _tokenBaseURI =
        "https://arweave.net/OVAmf1xgB6atP0uZg1U0fMd0Lw6DlsVqdvab-WTXZ1Q";
    string public contractURI =
        "ipfs://bafkreihuwhnie5helcz2u4eqpqp7boyx7yhrr4wfqjgexnh3kfml2hweu4";

    bytes32 public guestlistMerkleRoot =
        0x98397b2942ad2eca174c41a1cd8f59c8ea7242f99e89d6e047229f63ca964d66;
    bytes32 public vipMerkleRoot =
        0x410b38d85e5f955a07caeead64a9252030453c1caeb70862f1ec1435fb53eef0;

    uint256 public maxSupply = 5000;
    uint256 private _amountAirdropped = 0;
    uint256 public constant mintPrice = 3000000000000000; // 0.003 ETH
    uint256 public constant guestlistMintPrice = 2500000000000000; // 0.0025 ETH
    uint256 public constant vipMintPrice = 2000000000000000; // 0.002 ETH
    ...
    modifier mintCompliance(uint256 _quantity, uint256 price) {
        if (totalSupply() + _quantity > maxSupply)
            revert ExceedsMaxSupply(maxSupply);
        if (msg.value != (_quantity * price) || _quantity == 0)
            revert WrongAmount(msg.value, _quantity * price);
        _;
    }

    modifier saleActive() {
        if (!isSaleActive) revert SaleClosed();
        _;
    }

    constructor(address initialOwner) payable ERC721A("Testers", "TESTS") {}

    function setDefaultURI(string memory defaultURI_) external onlyOwner {
        defaultTokenUri = defaultURI_;
        emit SetDefaultURI(defaultURI_);
    }
    ...
    function airdrop(
        ...
        require(_len == _amts.length, "!length");
        for (uint256 _index = 0; _index < _len; ) {
            _mint(_receivers[_index], _amts[_index]);
            _amountAirdropped += _amts[_index];
            unchecked {
                ++_index;
            }
        }
    ...
    function guestlistMint(
        ...
        payable
        nonReentrant
        mintCompliance(_quantity, guestlistMintPrice)
        saleActive
    {
        bytes32 leaf = keccak256(abi.encodePacked(to));
        if (!MerkleProof.verify(_merkleProof, guestlistMerkleRoot, leaf))
            revert BadProof();
        address signer = ecrecover(hash, v, r, s);
        require(signer == to, "Signer and to dont match");
    ...
    function vipMint(
        ...
        payable
        nonReentrant
        mintCompliance(_quantity, vipMintPrice)
        saleActive
    {
        bytes32 leaf = keccak256(abi.encodePacked(to));
        if (!MerkleProof.verify(_merkleProof, vipMerkleRoot, leaf))
            revert BadProof();
        address signer = ecrecover(hash, v, r, s);
        require(signer == to, "Signer and to dont match");
        _mint(to, _quantity);
    ...
    function publicMint(
        uint256 _quantity,
        address to
    ) external payable mintCompliance(_quantity, mintPrice) saleActive {
        _mint(to, _quantity);
    }

    /// Set the new Merkle Root
    function setGuestlistMerkleRoot(
        bytes32 _guestlistMerkleRoot
    ) external onlyOwner {
        guestlistMerkleRoot = _guestlistMerkleRoot;
    }
    ...
    function getLastRevealed() public view returns (uint256) {
        return
            _shuffleData.length > 0
                ? _shuffleData[_shuffleData.length - 1].endId
                : 0;
    }

    /// @notice Called by owner to reveal all unrevealed tokens
    function revealBatch() external onlyOwner {
        require(getLastRevealed() < _totalMinted() - 1, "None to reveal");
        _shuffleData.push(
            ShuffleData(block.number + getLastRevealed(), _totalMinted() - 1)
        );

        emit BatchRevealed(
            block.number + getLastRevealed(),
            _totalMinted() - 1
        );
    }
    ...
    function _getMetadata(
        ...
        if (_tokenId > getLastRevealed() || (_shuffleData.length == 0)) {
        ...
        for (uint256 i; i < _shuffleData.length; i++) {
            ShuffleData memory batch = _shuffleData[i];
            ...
            lowestToken = i > 0 ? _shuffleData[i - 1].endId + 1 : 0;

            uint256 batchSize = batch.endId - lowestToken + 1;

            uint256[] memory metadata = new uint256[](batchSize);

            // Initializes the metadata array with the base values
            for (i = lowestToken; i <= batch.endId; i++) {
    ...
    // Override parent implementation to set up operator filters
