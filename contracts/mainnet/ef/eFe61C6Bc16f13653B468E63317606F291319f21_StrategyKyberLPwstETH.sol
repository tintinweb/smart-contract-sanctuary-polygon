...
contract StrategyKyberLPwstETH is StratFeeManagerInitializable {
   ...
   // Use elastic router
    address[] public outputToNativePath;
    ...
    function initialize(
        ...
        outputToNativeRoute = _routes[0];
        nativeToLp0Route = _routes[1];
        nativeToLp1Route = _routes[2];
    ...
    function swapRewardsToNative() internal {
       ...
       if (nativeBal > 0) {
        ...
        if (outputBal > 0) {
            dmmSwap(outputToNativeRoute, outputBal, outputToNativePath);
        }
        // extras
    ...
    function addLiquidity() internal {
        ...
        IERC20[] memory lp0Path = buildPath(nativeToLp0Route);
        IERC20[] memory lp1Path = buildPath(nativeToLp1Route);
        uint256[] memory outs0 = IDMMRouter(unirouter).getAmountsOut(lp0Amt, nativeToLp0Path, lp0Path);
        uint256[] memory outs1 = IDMMRouter(unirouter).getAmountsOut(lp1Amt, nativeToLp1Path, lp1Path);
        uint256 out0 = outs0[outs0.length - 1];
        uint256 out1 = outs1[outs1.length - 1];
        ...
        if (lpToken0 != native) {
            dmmSwap(nativeToLp0Route, lp0Amt, nativeToLp0Path);
        }

        if (lpToken1 != native) {
            dmmSwap(nativeToLp1Route, lp1Amt, nativeToLp1Path);
        }
