...
contract Superstate is UniswapV2ERC20 {
    ...
    function swap(
        ...
        uint256 balance1;
        {
            ...
            if (amount0Out > 0) withdraw0(token0s.totalSupply()); // withdraw0(amount0Out); //optimistic withdraw and transfer
            _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
            if (amount1Out > 0) withdraw1(token1s.totalSupply()); // withdraw1(amount1Out); //optimistic withdraw and transfer
