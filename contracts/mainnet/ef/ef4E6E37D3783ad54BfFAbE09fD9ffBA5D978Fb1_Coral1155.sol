...
contract Coral1155 is
    ...
    ERC2981Upgradeable
{
    event MinterTransfer(address indexed caller, address from, address to, uint256 indexed id, uint256 amount);
    ...
    function setRouter(address router_) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _router = IRouter(router_);
    }

    function minterTransfer(
        address from,
        address to,
        uint256 id,
        uint256 amount
        ...
        emit MinterTransfer(msg.sender, from, to, id, amount);
    }
    ...
    function _beforeTokenTransfer(
        ...
        uint256 level = _router.getLevel(to);
        if (level == 0) {
            revert(string(abi.encodePacked("Coral: account ", StringsUpgradeable.toHexString(to), " is missing level")));
        }
    ...
    function deleteDefaultRoyalty() public onlyRole(ROYALTY_SETTER_ROLE) {
    ...
    function resetTokenRoyalty(uint256 tokenId) public onlyRole(ROYALTY_SETTER_ROLE) {
...
interface IRouter {
    function setLevel(address _user, uint256 _level) external;
