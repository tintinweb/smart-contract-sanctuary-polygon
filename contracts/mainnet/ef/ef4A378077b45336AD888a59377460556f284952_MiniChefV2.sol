...
/// It is the only address with minting rights for the reward token.
/// The idea for this MasterChef V2 (MCV2) contract is therefore to be the owner of a dummy token
/// that is deposited into the MasterChef V1 (MCV1) contract.
/// The allocation point for this pool on MCV1 is the total allocation point for all pools that receive double incentives.
// contract MiniChefV2 is BoringOwnable, BoringBatchable {
contract MiniChefV2 is Ownable, ReentrancyGuard {
    ...
    /// @param _rewardPerSec The amount of reward token to be distributed per second.
        ...
        returns (uint256 pending)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 rewardForPool = (time *
                (rewardPerSec) *
                (pool.allocPoint)) / totalAllocPoint;
            accRewardPerShare =
                accRewardPerShare +
                ((rewardForPool * (ACC_REWARDS_PRECISION)) / lpSupply);
        }
        // pending = uint256(
        //     uint256((user.amount * accRewardPerShare) / ACC_REWARDS_PRECISION) -
        //         (user.rewardDebt)
        // );
    ...
    function deposit(
        ...
        user.rewardDebt =
            ...
            (
                int256(
                    (amount * (pool.accRewardPerShare)) / ACC_REWARDS_PRECISION
                )
            );
    ...
    function harvest(uint256 pid, address to) public {
        ...
        int256 accumulatedReward = int256(
            (user.amount * (pool.accRewardPerShare)) / ACC_REWARDS_PRECISION
        );
        uint256 _pendingRewards = uint256(
