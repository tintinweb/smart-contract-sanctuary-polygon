...
contract WorldCupActivityV2 is Ownable, Initializable, AccessControl, SemanticBaseStruct {
    ...
    string private _defaultPO;

    // Mapping of address of ( Mapping of subject index of (Mapping of poValue to true/false ))
    mapping(address => mapping(uint256 => mapping(string => bool)))  _mintedSPO;

    // Default not allow duplicate mint
    bool private _duplicatable;

    // Default not allow free mint
    ...
    function initialize(
        ...
        _defaultPO = string.concat(CONCATENATION_CHARACTER, CONCATENATION_CHARACTER, CONCATENATION_CHARACTER,
            _pIndex.toString(), _oIndex.toString(), CONCATENATION_CHARACTER);
    }
    ...
    function mint() external {
        require(_freeMintable || whiteList[msg.sender], "permission denied");
        require(_duplicatable || !_mintedSPO[msg.sender][0][_defaultPO], "already minted");
        _mintedSPO[msg.sender][0][_defaultPO] = true;
     ...
     * @dev claim sbt
     */
    function claim(Claim[] memory claimList) external {
        for (uint256 i = 0; i < claimList.length; i++) {
            Claim memory c = claimList[i];

            require(
                c.expireTime > block.timestamp,
                "Signature data expired"
            );
            string memory signData = string.concat(
                address(this).toHexString(),
                Strings.toHexString(msg.sender),
                c.expireTime.toString());
            string memory intPO = _buildIntPOSignData(c.intPO);
            string memory stringPO = _buildStringPOSignData(c.stringPO);
            string memory addressPO = _buildAddressPOSignData(c.addressPO);
            string memory subjectPO = _buildSubjectPOSignData(c.subjectPO);
            string memory blankNodePO = _buildBlankNodePOSignData(c.blankNodePO);

            string memory _POValue = string.concat(intPO, CONCATENATION_CHARACTER, stringPO, CONCATENATION_CHARACTER,
                addressPO, CONCATENATION_CHARACTER, subjectPO, CONCATENATION_CHARACTER, blankNodePO);
            require(_duplicatable || !_mintedSPO[msg.sender][c.sIndex][_POValue], "already minted");
            _mintedSPO[msg.sender][c.sIndex][_POValue] = true;


            signData = string.concat(signData, intPO, stringPO, addressPO, subjectPO, blankNodePO);
            ...
            _semanticSBT.mint(msg.sender, c.sIndex, c.intPO, c.stringPO, c.addressPO, c.subjectPO, c.blankNodePO);
        }

    }

    function _buildIntPOSignData(IntPO[] memory intPOList) internal pure returns (string memory signData){
        for (uint256 i = 0; i < intPOList.length; i++) {
            IntPO memory intPO = intPOList[i];
            signData = string.concat(signData, intPO.pIndex.toString(), intPO.o.toString());
        }
    }

    function _buildStringPOSignData(StringPO[] memory stringPOList) internal pure returns (string memory signData){
        for (uint256 i = 0; i < stringPOList.length; i++) {
            StringPO memory stringPO = stringPOList[i];
            signData = string.concat(signData, stringPO.pIndex.toString(), stringPO.o);
        }
    }


    function _buildAddressPOSignData(AddressPO[] memory addressOList) internal pure returns (string memory signData){
        for (uint256 i = 0; i < addressOList.length; i++) {
            AddressPO memory addressPO = addressOList[i];
            signData = string.concat(signData, addressPO.pIndex.toString(), addressPO.o.toHexString());
        }
    }


    function _buildSubjectPOSignData(SubjectPO[] memory subjectPOList) internal pure returns (string memory signData){
    ...
    function _buildBlankNodePOSignData(BlankNodePO[] memory blankNodePOList) internal pure returns (string memory signData){
        for (uint256 i = 0; i < blankNodePOList.length; i++) {
            BlankNodePO memory blankNodePO = blankNodePOList[i];

            string memory intSignData = _buildIntPOSignData(blankNodePO.intO);
            string memory stringSignData = _buildStringPOSignData(blankNodePO.stringO);
            string memory addressSignData = _buildAddressPOSignData(blankNodePO.addressO);
            string memory subjectSignData = _buildSubjectPOSignData(blankNodePO.subjectO);

            signData = string.concat(signData, intSignData, stringSignData, addressSignData, subjectSignData);
        }
    }
