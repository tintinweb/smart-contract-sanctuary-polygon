...
contract ForLootAndGloryLiquidityStaking is Ownable, ReentrancyGuard, Pausable { 
  ...
  uint public totalStakedSupply = 1; 
  uint public lastRewardTime; 

  IERC20 internal stakingToken; 
  IERC20 internal rewardToken; 

  uint public dailyEmissionsRate; 
  uint public bigMultiplier = 1000000000000000000; 
  ...
  constructor(address _stakingToken, address _rewardToken) {
        require(address(_stakingToken) != address(0), "It should be token addressNot Zero address");
         require(address(_rewardToken) != address(0), "It should be token addressNot Zero address");
    stakingToken = IERC20(_stakingToken);
    rewardToken = IERC20(_rewardToken);
    lastRewardTime = block.timestamp; 
    dailyEmissionsRate = 250000000000000000000; 
  ...
  function createStake(uint _amount) external nonReentrant whenNotPaused {
    require(_amount > 0, "Cannot stake 0");
    require(stakingToken.balanceOf(msg.sender) >= _amount, "You don't have that much to stake!");
  ...
  function removeStake(uint _amount) external nonReentrant whenNotPaused {
    require(_amount > 0, "Cannot remove 0");
    require(_amount <= userStakes[msg.sender], "You haven't staked that much!");
    getRewardsInternal();
  ...
  // emergency
  function emergencyUnstake(uint _amount) external nonReentrant {
    require(_amount > 0, "Cannot remove 0");
    require(_amount <= userStakes[msg.sender], "You haven't staked that much!");
    totalStakedSupply = totalStakedSupply.sub(_amount);
    userStakes[msg.sender] = userStakes[msg.sender].sub(_amount);
  ...
  function updateBigRewardsPerToken() public {
    if (timeSinceLastReward() > 0) { 
      ...
      uint newEmissionsToAdd = emissionsPerSecond.mul(rewardSeconds); 
      uint newBigRewardsPerToken = ((newEmissionsToAdd.mul(bigMultiplier)).div(totalStakedSupply));
      bigRewardsPerToken = bigRewardsPerToken.add(newBigRewardsPerToken); 
    }
  }
