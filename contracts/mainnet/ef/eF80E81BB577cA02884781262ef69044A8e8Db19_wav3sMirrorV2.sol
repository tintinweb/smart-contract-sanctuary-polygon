...
contract wav3sMirrorV2 {
    ...
    // Mapping to track coin budgets
    mapping(address => mapping(address => uint256)) s_profileToCurrencyToBudget;
    ...
    function fundMirrorGlobalBudget(
        address profileAddress,
        address currency,
        uint256 amount
    ) external {
        ...
        if (msg.sender != profileAddress) {
            revert('wav3sMirror__BudgetSenderNotOwner()');
        }
        // Check: Budget for the publication must be higher than zero.
        if (amount <= 0) {
            revert('wav3sMirror__BudgetInvalidBudget()');
        }
        // Check: Only accept whitelisted currencies.
        if (!currencyWhitelisted(currency)) {
            revert('wav3sMirror__BudgetCurrencyNotWhitelisted()');
        }

        // Separate budget from fees
        uint256 fees = (amount / (100 + i_fee)) * i_fee;

        // Transfer funds from the budget owner to wav3s contract
        IERC20(currency).safeTransferFrom(profileAddress, address(this), amount);

        // Transfer the fees to the wav3s multisig, keep the apps fees here
        IERC20(currency).safeTransferFrom(address(this), s_multisig, (fees * 2) / 3);

        s_profileToCurrencyToBudget[profileAddress][currency] += amount;
    }

    function withdrawMirrorGlobalBudget(
        address profileAddress,
        address currency,
        uint256 amount
    ) external {
        ...
        if (amount > s_profileToCurrencyToBudget[profileAddress][currency]) {
     ...
     * //frontendAddress The address of the frontend contract.
    ...
    function fundMySuperReach(
        ...
        address profileAddress,
        //  address frontendAddress,
        uint256 _minFollowers // address _tokenFilter
    ) public {
        // Check: Only the owner of the profile can fund the super reach.
        if (msg.sender != profileAddress) {
        ...
        // Check: Reward for mirroring must be equal or higher than the minimum permitted.
        ...
        // Pre calculate new budget
        uint256 newBudget = dataByPublication[pubId].budget + _budget;

        // Check: Reward can't be higher than the budget for the publication.
        if (_reward > newBudget) {
        ...
        dataByPublication[pubId].budget += _budget;
        ...
        dataByPublication[pubId].feePerMirror = ((_budget / (100 + (i_fee / 3))) * (i_fee / 3));
        // Update: Global budget for this profile and currency
        s_profileToCurrencyToBudget[profileAddress][currencyAddress] -= _budget;
     ...
     * @param appAddress The number of followers that the mirrorer has.
    ...
    function withdrawMirrorBudget(string memory pubId, uint256 amount) public {
        ...
        budget = dataByPublication[pubId].budget;
        currency = dataByPublication[pubId].currencyAddress;
        profileAddress = dataByPublication[pubId].profileAddress;
    ...
    function payApps(address appAddress) external {
        ...
        IERC20(currency).safeTransferFrom(
            address(this),
            appAddress,
            s_appToCurrencyToFees[appAddress][i_USDCoin]
        );
        IERC20(currency).safeTransferFrom(
            address(this),
            appAddress,
            s_appToCurrencyToFees[appAddress][i_DAI]
        );

        s_appToCurrencyToFees[appAddress][i_wMatic] = 0;
        s_appToCurrencyToFees[appAddress][i_USDCoin] = 0;
        s_appToCurrencyToFees[appAddress][i_DAI] = 0;
    }
