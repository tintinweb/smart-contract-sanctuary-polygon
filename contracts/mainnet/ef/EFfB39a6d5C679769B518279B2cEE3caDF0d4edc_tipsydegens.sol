...
contract tipsydegens is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 public costPB = 0.1 ether;
  uint256 public costWL = 0.1 ether;
  ...
  constructor( address _signerAddress ) ERC721A ( "Tipsy Degens", "TDT" ) {
    setNotRevealedURI( "ipfs://NOT-REVEALED-IPFS-CID/hidden.json" );
...
// ~~~~~~~~~~~~~~~~~~~~ Mint Functions ~~~~~~~~~~~~~~~~~~~~
  ...
  function mint(uint256 _mintAmount, bytes memory sig) public payable mintCompliance(_mintAmount) {
  ...
  // FREE MINT
  function mintfree(uint256 _mintAmount, bytes memory sig) public mintCompliance(_mintAmount) {
    require(freeSupply - _mintAmount >= 0, "Not enough free supply!");
    require(freeSupply > 0, "Free supply sold out.");

    if (isValidData(msg.sender, sig) == true) {
        require(freeClaimed[msg.sender] + _mintAmount <= MaxFreeWL, "Whitelist Mint: Max free mints exceeded!");
        }
    else
        {
        require(freeClaimed[msg.sender] + _mintAmount <= MaxFreePB, "Public Mint: Max free mints exceeded!");
        }
