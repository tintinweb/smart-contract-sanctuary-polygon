...
contract MaxityRouter is Ownable,IMRouter {
    ...
    function mintAndSellMax721(address _token,address designer
        ...
        IMaxityMarketPlace(market).sellByFixPrice(_token,new_tokenids,unit_prices,msg.sender);
        return new_tokenids;
    }
    ...
    function mintAndAuctionMax721(address _token,address designer
        ...
        uint256 [] memory new_tokenids =  IMaxity721Token(_token).mint(designer, market, _futureRoyaltys, tokenURIOrigins);
        IMaxityMarketPlace(market).auctionMax721(_token,new_tokenids,base_prices,incre_prices,starttimes,deadlines,msg.sender);
        return new_tokenids;
    }

    function buyNative(address _token,uint256 _tokenid,address token_to,address nativeSweep) public override payable {
        ...
        IMaxityMarketPlace(market).buy(_token,_tokenid,token_to);
    ...
    function bidNative(address _token,uint256 _tokenid,address token_to,address nativeSweep) public override payable {
        IWMATIC(wrappedNative).deposit{value: msg.value}();
        assert(IWMATIC(wrappedNative).approve(market, msg.value));
        // uint before_balance = IWMATIC(wrappedNative).balanceOf(address(this));
        IMaxityMarketPlace(market).bid(_token,_tokenid,msg.value,token_to);
        uint after_balance = IWMATIC(wrappedNative).balanceOf(address(this));
        if(after_balance>0)
        {
            IWMATIC(wrappedNative).transfer(nativeSweep, after_balance);
    ...
    function auctionOnFixPriceNative(address _token,uint _tokenid,address token_to,address nativeSweep)public override payable{
        IWMATIC(wrappedNative).deposit{value: msg.value}();
        assert(IWMATIC(wrappedNative).approve(market, msg.value));
        // uint before_balance = IWMATIC(wrappedNative).balanceOf(address(this));
        IMaxityMarketPlace(market).auctionOnFixPrice(_token,_tokenid,msg.value,token_to);
        uint after_balance = IWMATIC(wrappedNative).balanceOf(address(this));
        if(after_balance>0)
        {
            IWMATIC(wrappedNative).transfer(nativeSweep, after_balance);
