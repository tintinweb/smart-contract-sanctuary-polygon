...
contract Community {
  ...
  function depositTokens(uint256 amount) external  {
    ...
    balance += amount;
    IERC20(tokenAddress).transferFrom(msg.sender, address(this), amount * (10 ** ERC20(tokenAddress).decimals()));
  ...
  function withdrawNFTS(Airdrop[] memory nfts) external {
    ...
    for (uint i = 0; i < nfts.length; i++) {
      IERC721(nfts[i].nft).transferFrom(address(this), msg.sender, nfts[i].id);
    }
  }
}
