...
contract NFTContract is ERC1155, Ownable, ERC2981Base  {
    ...
    uint256 private totalSupply = 1111;
    uint256 private price = 40000000000000000000;
    uint256 private salePrice = 10000000000000000000;
    uint256 private whitelistPrice = 0;
    ...
    constructor()
        ERC1155(
            "ipfs://QmbFreQiicpMj5d6YtzBBPiuW7GkFVzofszyWpYpp5W4Ds/metadata/{id}.json" // You can get this saved in dasboard of your Moralis server instance.
        )
    {
    ...
    function contractURI() public pure returns (string memory) {
        return "https://ipfs.io/ipfs/QmTAJpH4qZZneRdGsUHynfvGY7DzA1op7ZpZr957XXjKT3";
    }
    ...
    function mint(address account) public payable {
        if(verifyUser(account)) {
            require(msg.value >= whitelistPrice, "Amount of ether sent not correct.");
            if(account != contractCreator)
            whitelistedAddresses[account] = false; 
        }
        else if(saleActive) require(msg.value >= salePrice, "Amount of ether sent not correct.");
        else require(msg.value >= price, "Amount of ether sent not correct.");
        
        require(_tokenIdCounter.current() <= totalSupply, "Sold out");

        _mint(account, _tokenIdCounter.current(), 1, "");
        _tokenIdCounter.increment();
    }

    function getMintPrice(address account) public view returns (uint256) {
        if(verifyUser(account)) return whitelistPrice;
        else if(saleActive) return salePrice;
        else return price;
    }
    ...
    function addUsers(address[] memory _addressToWhitelist) public onlyOwner {
        for (uint i = 0; i < _addressToWhitelist.length; i++) {
            whitelistedAddresses[_addressToWhitelist[i]] = true;
        }
    }

    function verifyUser(address account) public view returns(bool)  {
        bool userIsWhitelisted = whitelistedAddresses[account];
    ...
    function startSale(uint256 newPrice) public onlyOwner {
        salePrice = newPrice;
        saleActive = true;
    }
