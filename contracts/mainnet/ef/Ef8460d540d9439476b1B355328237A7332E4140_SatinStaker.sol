...
contract DySATINManager is OwnableUpgradeable, PausableUpgradeable {
  /**
   * @dev Dyson Contracts:
  ...
  function __DySATINManager_init(address _keeper, address _voter) public initializer {
    __DySATINManager_init_unchained(_keeper, _voter);
  }

  function __DySATINManager_init_unchained(address _keeper, address _voter) internal initializer {
    keeper = _keeper;
    voter = _voter;
  }
...
import "./IVeSATIN.sol";
import "./DySATINManager.sol";

interface ISatinBoostedStrategy {
...
contract SatinStaker is DySATINManager, ReentrancyGuardUpgradeable, ERC20Upgradeable {
  using SafeERC20Upgradeable for IERC20Upgradeable;

  uint256 public constant MAX_TIME = 365 * 86400;
  uint256 public constant MAX_LOCK = 365 * 86400;

  // Addresses used
  ISolidLizardVoter public solidVoter;
  IVeSATIN public veToken;
  ...
  function __SatinStaker_init(
    ...
    __DySATINManager_init(_keeper, _voter);
    __Ownable_init();
    __ReentrancyGuard_init();
    solidVoter = ISolidLizardVoter(_solidVoter);
    veToken = IVeSATIN(solidVoter.ve());
  ...
  /**
   * @notice  Deposits $SATIN for $spTETU and sends depositor minted token
   * @dev     Simsala
   * @param   _amount  amount to be deposited
   */
  function _deposit(uint256 _amount) internal nonReentrant whenNotPaused {
   //  require(canMint(), "!canMint");
    uint256 _pool = balanceOfSatin();
    want.safeTransferFrom(msg.sender, address(this), _amount);
    uint256 _after = balanceOfSatin();
    ...
    if (_amount > 0) {
      ...
      emit DepositWant(totalSatin());
    }
  }

  function depositViaChecker(
  ...
  function deposit(address _gauge, uint256 _amount) external onlyWhitelist(_gauge) {
    // Grab needed info
    IERC20Upgradeable _underlying = ISatinBoostedStrategy(msg.sender).want();
  ...
  /**
   * @notice  Calculates how much of the $SATIN is locked
   * @dev     Simsala
   * @return  satin  returns the quantity of $SATIN in VE
   */
  function balanceOfSatinInVe() public view returns (uint256 satin) {
    return veToken.balanceOfNFT(veTokenId);
  }

  /**
   * @notice  dictates balance of want in contract
   * @return  uint256  quantity of $SATIN in Contract
   */
  function balanceOfSatin() public view returns (uint256) {
  ...
  /**
   * @notice  calculates all $SATIN in all contracts
   * @dev     Simsala
   * @return  uint256  returns the $SATIN in all contracts
   */
  function totalSatin() public view returns (uint256) {
    return balanceOfSatin() + (balanceOfSatinInVe());
  }
  ...
  function whitelistStrategy(address _strategy) external onlyManager {
    IERC20Upgradeable _want = ISatinBoostedStrategy(_strategy).want();
    address _gauge = ISatinBoostedStrategy(_strategy).gauge();
