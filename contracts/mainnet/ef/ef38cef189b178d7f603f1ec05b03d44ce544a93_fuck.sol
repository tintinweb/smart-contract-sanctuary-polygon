...
contract fuck is  ERC721, ReentrancyGuard, OwnableRoles, DefaultOperatorFilterer, ERC2981{
  ...
  function setFundingRecipient(address fundingRecipient) external onlyRolesOrOwner(ADMIN_ROLE | DEV_ROLE) {
    _fundingRecipient = fundingRecipient;
  }
  ...
  function adminMint(uint256 planetNumber, address recipient, uint256 count) public onlyRolesOrOwner(ADMIN_ROLE | DEV_ROLE)  {
    if (planetNumber == 1) {
  ...
  // ============================================================= //
   ...
   function RainbowListMint(uint256 planetNumber, address recipient, uint256 count, bytes32[] calldata _proof) public payable nonReentrant  {
    //     if (!_verifyProof(msg.sender, _proof)) { //may gave to change to "recipient"
    //   revert InvalidProof();
    // }
            if (!_verifyProof(recipient, _proof)) { //may gave to change to "recipient"
      revert InvalidProof();
    }
