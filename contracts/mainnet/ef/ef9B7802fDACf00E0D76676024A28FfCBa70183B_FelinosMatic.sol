...
contract FelinosMatic is Ownable {
    ...
    function getRefUser(address _user) public view returns(uint[9][9]  memory) {
        uint[9][9] memory result;
        for(uint i = 0; i < 9; i++) {
            for(uint j = 0; j < 9; j++) {
                result[i][j] = poolUserReferrals[_user][i][j];
            }
        }
    ...
    function getAllPoolUsers() public view returns(uint[9] memory) {
        uint[9] memory result;
        for(uint i; i < 9; i++) {
            result[i] = poolUsersCounts[i];
        }
