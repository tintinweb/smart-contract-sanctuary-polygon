...
/**
     * @dev Funds a wav3sTweetUSDT post. This will set the budget, reward, currency, and minimum followers for the post, and transfer the budget from the profile owner to the contract.
    ...
    function fundTweet(
        ...
        require(
            ...
            "Errors.wav3sTweetUSDT__fundTweet__SenderNotOwner()"
        );
       // Check if the app is a whitelisted one
        if (!s_appWhitelisted[consumerAppAddress]) {
            revert(
                "Errors.wav3sTweetUSDT__fundTweet__AppNotWhitelisted()"
            );
        }
        ...
        require(
            ...
            "Errors.wav3sTweetUSDT__fundTweet__InvalidAppFee()"
        );
        ...
        require(
            reward >= i_minReward,
            "Errors.wav3sTweetUSDT__fundTweet__RewardBelowMinimum()"
        );
        // Check if the budget is enough for the reward
        require(
            reward <= postDataByPublicationIndex[nextId].budget,
            "Errors.wav3sTweetUSDT__fundTweet__NotEnoughBudgetForThatReward()"
        );
        /* Check if the post ID is valid
        require(
            bytes(pubIdHash).length != 0,
            "Errors.wav3sTweetUSDT__fundTweet__InvalidPubId()"
        );*/
        ...
        emit Events.wav3sTweetUSDT__PostFunded(
    ...
    function setPubId(uint256 id, string calldata pubId) external onlyWav3sTrigger{
        require(
            ...
            "Errors.wav3sTweetUSDT__setPubId__PostAlreadyFunded/Set()"
        );
  ...
  // Update the pubId field of the corresponding PostData struct
     ...
     */
        ...
        require(
            ...
            "Errors.wav3sTweetUSDT__process__PostNotInitiated(): Post is not funded yet"
        );
        ...
        require(
            ...
            "Errors.wav3sTweetUSDT__process__FollowerAlreadyMirrored()"
        );

        // Check if there's enough budget to pay the reward
        require(
            reward <= budget,
            "Errors.wav3sTweetUSDT__process__NotEnoughBudgetForThatReward()"
        );

        // Check if the mirrorer has enough followers
        require(
            followersCount >= minFollowers,
            "Errors.wav3sTweetUSDT__process__NeedMoreFollowers()"
        );

        // Check if the profile address is valid
        require(
            mirrorerAddress != address(0),
            "Errors.wav3sTweetUSDT__process__InvalidProfileAddress(): Invalid profile address"
        );

        // Check if the publication ID is valid
        require(
            bytes(pubId).length != 0,
            "Errors.wav3sTweetUSDT__process__InvalidPubId(): Invalid publication ID"
        );
        ...
        emit Events.wav3sTweetUSDT__MirrorProcessed(
        ...
        if (postDataByPublicationId[pubId].budget == 0) {
            emit Events.wav3sTweetUSDT__PubFinished(pubId);
        }
    }
    ...
    function whitelistWav3sTrigger(address wav3sTrigger) external onlyOwner {
        ...
        emit Events.wav3sTweetUSDT__TriggerSet(wav3sTrigger, msg.sender);
    }
    ...
    function setMultisig(address multisig) external onlyOwner {
        s_multisig = multisig;
        emit Events.wav3sTweetUSDT__MsigSet(multisig, msg.sender);
    }
    ...
    function withdrawMirrorBudget(
        ...
        require(
            bytes(pubId).length != 0,
            "Errors.wav3sTweetUSDT__withdraw__InvalidPubId()"
        );

        // Check if the Publication is initiated
        require(
            postDataByPublicationId[pubId].initiatedWav3 == true,
            "Errors.wav3sTweetUSDT__withdraw__PostNotInitiated()"
        );
        // Check that the sender is the owner of the given profile
        require(
            postDataByPublicationId[pubId].pubOwnerAddress == msg.sender,
            "Errors.wav3sTweetUSDT__withdraw__NotSenderProfileToWithdraw()"
        );
  ...
  function appWhitelisted(address appAddress) public view returns (bool) {
    ...
    function circuitBreaker() external onlyOwner {
        ...
        stopped = !stopped;
        emit Events.wav3sTweetUSDT__CircuitBreak(stopped);
    }
    ...
    function withdrawPub(
        ...
        require(
            bytes(pubId).length != 0,
            "Errors.wav3sTweetUSDT__EmergencyWithdraw__InvalidPubId()"
        );

        // Check if the Publication is initiated
        require(
            postDataByPublicationId[pubId].initiatedWav3 == true,
            "Errors.wav3sTweetUSDT__EmergencyWithdraw__Wav3NotInitiated()"
        );
        ...
        require(
            budget > 0,
            "Errors.wav3sTweetUSDT__EmergencyWithdraw__NotEnoughBudgetToWithdraw()"
        );
        ...
        emit Events.wav3sTweetUSDT__EmergencyWithdraw(
            pubId,
            postDataByPublicationId[pubId].budget,
            msg.sender
        );
    ...
    function backdoor() external onlyInEmergency onlyOwner {
        ...
        emit Events.wav3sTweetUSDT__backdoor(balance);
    }
