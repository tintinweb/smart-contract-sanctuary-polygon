    ...
    UUPSUpgradeable
{
    ...
    function revenueCalculate(
        ...
        for (uint8 i = 0; i < 6; i++) {
            uint256[2] memory _activeOrderIndex = activeOrderIndex[
                _revenueOwner
            ][i];
            uint256 _length = _activeOrderIndex[1];
            uint256 _start = _activeOrderIndex[0];
            if (_length == 0) {
                continue;
            }
    ...
    function baseRevenueCalculate(
        ...
        uint256[2] memory _activeOrderIndex
    ) public view returns (uint256) {
        if (_userSpeedUpCard.endTime <= orderToTakeRevenue.refreshTime) {
            _userSpeedUpCard.activeState = false;
        }
