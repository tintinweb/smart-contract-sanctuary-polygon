...
ERC1155SupplyUpgradeable {
    ...
    function initialize(
        ...
        /*IQuoterV2 _swapQuoter,*/
        address _weth
    ) public initializer {
        name             = "Square Supply: Corners";
        symbol           = "TEST.C";
        cost             = 1000000000000 wei;
        maxSupply        = 10*1000;
        maxMintAmount    = 4;
        ...
        /*uniswap          = new EffuPami(_swapRouter, _weth, cost);*/
    ...
    function uri(uint256 _tokenId) public view override virtual returns (string memory) {
        // require(totalSupply(_tokenId) == 1, "ERC1155Metadata: URI query for nonexistent token");
        //return baseURI.concat(_tokenId.toString()).concat(baseExtension);
    ...
    function mintBatch(address _to, uint[] memory _ids) public onlyOwner {
    ...
    function mintCornerWithETH(uint256 _howMany) public payable {
    ...
    function mintCornerWithToken(uint256 _howMany, address _paymentToken, uint256 _paymentMax, uint256 _poolFee) public {
        /*uint256 balancePreSwap = address(this).balance;*/
        /*uniswap.swapExactOutputSingle(_paymentToken, _paymentMax);*/
        uint256 payment = swapExactOutputSingle(_paymentToken, _paymentMax, _howMany, _poolFee);
        require(cost * _howMany == payment, "insufficient payment");
        /*uint256 balanceIncrease = address(this).balance - balancePreSwap;*/
        /*require(cost * _howMany == balanceIncrease, "insufficient payment");*/
        /*require(cost * _howMany <= balanceIncrease, "insufficient payment");*/
        _mintCornerSafe(_howMany);
    }

    /*// INFO: this one does a 1hop-pool-swap quote estimate for you magically.*/
    /*function mintCornerWithToken(uint256 _howMany, address _paymentToken, uint _poolFee) public {*/
        /*var (paymentMaxQuote, sqrtPriceX96After, initializedTicksCrossed, gasEstimate) = swapQuoter.quoteExactOutputSingle({*/
          /*tokenIn: _paymentToken,*/
          /*tokenOut: wETH,*/
          /*fee: poolFees[_poolFee],*/
          /*amountIn: cost * _howMany,*/
          /*// -2%*/
          /*sqrtPriceLimitX96: 0*/
        /*});*/
        /*mintCornerWithToken(_howMany, _paymentToken, paymentMaxQuote);*/
    /*}*/
    ...
    // Private internal mint interface.
    ...
    function _mintCorner() private {
        ...
        require(totalSupply(newCornerID) == 0, "that id is already minted!");
    ...
    function _mintCorner(uint256 _howMany) private {
        ...
        for (uint256 i = 0; i < _howMany; i++) {
            ...
            _amounts[i] = 1;
            require(totalSupply(_ids[i]) == 0, "id already minted");
        }
        _mintBatch(msg.sender, _ids, _amounts, "0x00");
    }

    // The number of corners that have been minted so far.
    function mintedCorners() public view returns (uint256) {
    ...
    /// @notice swapExactOutputSingle swaps a *minimum possible amount of _paymentToken* for a _fixed amount of wETH_.
    ...
    function swapExactOutputSingle(address _paymentToken, uint256 _paymentMax, uint256 _numToMint, uint _poolFee)
        private
