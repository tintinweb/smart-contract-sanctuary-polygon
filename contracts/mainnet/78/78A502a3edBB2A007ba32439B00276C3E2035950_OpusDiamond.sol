...
struct ConstructorArgs {
    OpusForwarder trustedForwarder;
    address owner;
    address init;
    bytes initCalldata;
    string cardName;
...
contract OpusDiamond {
    AppStorage s;

    constructor(IDiamondCut.FacetCut[] memory _diamondCut, ConstructorArgs memory _args) {
        LibDiamond.setContractOwner(_args.owner);
        LibERC2771Context.updateTrustedForwarder(address(_args.trustedForwarder), true);
        LibDiamond.diamondCut(_diamondCut, _args.init, _args.initCalldata);

        s.cardName = _args.cardName;
        s.cardSymbol = _args.cardSymbol;
        s.cardImageCID = _args.cardImageCID;

        LibCards.incrementCardId();
        LibCards.setCardLimit(_args.cardLimit);
...
contract OpusForwarder is EIP712 {
    ...
    constructor() EIP712("OpusForwarder", "1.0.0") {}
    ...
    function verify(ForwardRequest calldata _req, bytes calldata _signature) public view returns (bool) {
        address signer = _hashTypedDataV4(
            keccak256(abi.encode(_TYPEHASH, _req.from, _req.to, _req.value, _req.gas, _req.nonce, keccak256(_req.data)))
        ).recover(_signature);
    ...
    function execute(
        ...
        require(verify(_req, _signature), "OpusForwarder: signature does not match request");
        _nonces[_req.from] = _req.nonce + 1;

        (bool success, bytes memory returndata) = _req.to.call{gas: _req.gas, value: _req.value}(
