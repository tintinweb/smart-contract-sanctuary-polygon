...
contract BapesClan is ERC721, Ownable {
  ...
  constructor(string memory _hiddenMetadataURI, address _paymentsContract) ERC721("Bapes Clan", "BAPE") {
  ...
  function mintBape(uint256 wethAmount) external {
    uint256 totalMinted = totalSupply();

    // checks
    require(whitelistedWallets[msg.sender], "This wallet is not authorized to mint.");
    require(!paused, "Minting is paused.");
    require(totalMinted.add(maxPerMint) <= maxSupplyPrivate, "Not enough Bapes.");
    require(wethAmount >= price, "Insufficient balance to mint.");
    require(
    ...
    // transfer weth
    weth.transferFrom(msg.sender, paymentsContract, wethAmount);
  ...
  function reserveBapes(uint256 _count) public onlyOwner {
    ...
    require(totalMinted.add(_count) <= maxSupplyTotal, "Not enough Bapes.");
