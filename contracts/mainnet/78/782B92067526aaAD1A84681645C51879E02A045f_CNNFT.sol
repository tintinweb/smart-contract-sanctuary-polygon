...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    // Base URI
    string private _baseURI = "https://lsjkgo.github.io/imgJson/nftJson/";
...
interface RandomInterface{
    ...
    function Randomafunction() external returns(uint256);
}
...
contract CNNFT is OwnableContract, ERC721("CNNFT", "CNNFT"){
    ...
    uint public oneBla = 0.1 ether;
    
    mapping(uint256 => ChickAttribute) public tokenIdChickAttribute;
    mapping(uint256 => bool) public tokenIdChickAttributeBool;
    ...
    function setMaxSupply(uint256 amount) public onlyOwner{
      ...
      function getTokenIdChickAttribute(uint256 tokenId) public view returns(ChickAttribute memory attribute){
     ...
     */
    function createNFT(uint amount) public payable returns(uint256){
    require(totalSupply() < maxSupply, 'The limit has been reached!');
    require(msg.value == oneBla.mul(amount),"Insufficient transfer quantity");
       uint _rarity = 0;
       address owner = msg.sender;
        uint256 tokenId = _mintChick(owner);
        if(tokenIdChickAttributeBool[tokenId]){
            return tokenId;
        }
         ChickAttribute memory chickAttribute;
        chickAttribute.types = _rarity;
        tokenIdChickAttribute[tokenId] = chickAttribute;
        tokenIdChickAttributeBool[tokenId] = true;
        return tokenId;
    }

  function setOneBal(uint amount) public onlyOwner {
       oneBla = amount;
    } 
