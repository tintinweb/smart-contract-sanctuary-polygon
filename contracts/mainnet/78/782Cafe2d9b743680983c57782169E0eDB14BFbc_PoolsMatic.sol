...
contract PoolsMatic is Ownable, ReentrancyGuard {
    ...
    mapping(address => mapping(uint => bool)) userRankRewardClaimed; // user => month => is claimed
    ...
    uint[] public rankR = [0,2,10,30,50,100,200];
    ...
    function logVolume(uint amount) internal {
        ...
        for(uint i = 0; i < 7; i++) {
            ...
            from = _refferBy;
            if(volumeOntree[_refferBy] >= 50_000 ether && volumeOntree[_refferBy] < 200_000 ether && userTotalLock[_refferBy] >= 1000 ether && childs[_refferBy].direct >= 2 && childs[_refferBy].downLine >= 10) {
                userRank[_refferBy] = 1;
                rankRewards[1].totalMember += 1;
            }
            else if(volumeOntree[_refferBy] >= 200_000 ether && volumeOntree[_refferBy] < 500_000 ether && userTotalLock[_refferBy] >= 5000 ether && childs[_refferBy].direct >= 5 && childs[_refferBy].downLine >= 50) {
                userRank[_refferBy] = 2;
                rankRewards[1].totalMember -= 1;
                rankRewards[2].totalMember += 1;
            }
            else if(volumeOntree[_refferBy] >= 500_000 ether && volumeOntree[_refferBy] < 1000_000 ether && userTotalLock[_refferBy] >= 10000 ether && childs[_refferBy].direct >= 10 && childs[_refferBy].downLine >= 100) {
                userRank[_refferBy] = 3;
                rankRewards[2].totalMember -= 1;
                rankRewards[3].totalMember += 1;
            }
            else if(volumeOntree[_refferBy] >= 1000_000 ether && volumeOntree[_refferBy] < 3000_000 ether && userTotalLock[_refferBy] >= 20000 ether && childs[_refferBy].direct >= 10 && childs[_refferBy].downLine >= 200) {
    ...
    function giveRankRewardMonthly() external onlyGnosisSafe {
        for(uint i = 1; i < 6; i++) {
            if(rankRewards[i].total >= rankRewards[i].minStart) {
