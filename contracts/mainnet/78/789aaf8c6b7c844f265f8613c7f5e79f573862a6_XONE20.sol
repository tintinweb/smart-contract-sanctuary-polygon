...
contract XONE20 is ERC20Burnable, Operator {
    ...
    address public taxCollectorAddress; // treasuryFund or team wallet.. can be set to null to burn it all
    ...
    // the only time the peg oracle gets called is by Treasury, where I locally definedit that way
    function _getXONEPrice() internal view returns (uint256 _xonePrice) {
        try IOracle(taxOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("XONE: failed to fetch XONE price from taxOracle");
        }
    }
    ...
    ) public ERC20("XONE", "XONE") {
        require(_taxRate < 10000, "tax less than 100%");
        require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");
        require(_initialMint > 0, "XONE: initial mint CAN'T zero");
    ...
    // the only time the peg oracle gets called is by Treasury, where I locally definedit that way
    function setXONEOracle(address _oracle) external onlyOperatorOrTaxOffice {
        require(_oracle != address(0), "taxOracle address cannot be 0 address");
        taxOracle = _oracle;
    }

    function setTaxCollectorAddress(address _taxCollectorAddress) external onlyTaxOffice {
        require(_taxCollectorAddress != address(0), "Collector needs non-zero, or burn");
        taxCollectorAddress = _taxCollectorAddress;
    }
    ...
    function setTaxRate(uint256 _taxRate) external onlyTaxOffice {
        ...
        require(_taxRate < 10000, "tax rate under 100%?");
        taxRate = _taxRate;
    }
    ...
    function transferFrom(
        ...
        bool burnTax = false;
        if (autoCalculateTax) {
            uint256 currentXONEPrice = _getXONEPrice();
            currentTaxRate = _updateTaxRate(currentXONEPrice);
            if (currentXONEPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function _updateTaxRate(uint256 _xonePrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_xonePrice >= taxTiersTwaps[tierId]) {
