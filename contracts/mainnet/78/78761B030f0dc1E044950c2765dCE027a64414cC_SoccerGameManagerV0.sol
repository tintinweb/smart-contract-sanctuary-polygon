...
contract SoccerGameManagerV0 is Ownable {
    ...
    address soccerTransfersAddress = 0x56b5F0FddA4E7071c1B37933B4088e53F61E090D;
    ...
    GameEngineInterface gameEngineInstance;
    ...
    uint256 public playPrice = 0 wei; //1000000000000000000 wei
    ...
    function playGame() public payable {
        ...
        address enemy = choosePlayer(msg.sender);
        uint256 isWinner = gameEngineInstance.linkStart(msg.sender, enemy); // 1 win 2 lose 0 draw
    ...
    function multipleAdds (addPlayer [] memory _newPlayers) public {
        require(_newPlayers.length > 0);
        require(_newPlayers.length + activeTeam[msg.sender].length <= 5);
        
        for(uint256 i = 0; i < _newPlayers.length; i++) {
            addPlayer memory p = _newPlayers[i];
            addPlayerToTeam(p.collectionAddress, p.id, p.role);
        }
    ...
    //MATCHMAKING
    function choosePlayer(address _player) internal view returns(address) {
        ...
        while(true) {
            ...
            enemy = users[_random];
            if(userCanPlay[enemy] && enemy != _player) {
                break;
            } else{
                _randomizer += 1;
            }
    ...
    function getSquadInfo(address _coach) public view returns(ChoosenPlayer [] memory) {
        return activeTeam[_coach];
    }
    // #############################################
    ...
    function setGameEngineAddress (address _newAddress) public onlyOwner {
        gameEngineAddress = _newAddress;
        gameEngineInstance = GameEngineInterface(_newAddress);
    }
    // ###############################################
