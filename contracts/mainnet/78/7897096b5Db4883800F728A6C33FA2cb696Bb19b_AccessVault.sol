...
contract AccessVault is Initializable, Storage {
    ...
    function setMarginSwap (
        ...
        require(
            amount <= _baaRegistry[baaAddress].loanAmount, '13.3'
        );
    ...
    function withdrawLiquidationRemains (
    // todo discuss this approach find out a workaround for an outstanding balance
        address stablecoinAddress
    ) external returns (bool) {
        require(
            !_hasOutstandingBalance[msg.sender], '14.1'
        );
        uint256 amount = _liquidationRemains[msg.sender][stablecoinAddress];
        require(amount > 0, '14.2');
        _liquidationRemains[msg.sender][stablecoinAddress] = 0;
        TransferHelper.safeTransfer(stablecoinAddress, msg.sender, amount);
        return true;
    }
    ...
    function liquidate (
        ...
        _functionCall(
            ...
            abi.encodeWithSignature(
                'withdraw(address,uint256)',
                    _baaRegistry[baaAddress].stablecoinAddress, balance
            ), '22.4'
        );
        balance += _baaRegistry[baaAddress].depositAmount;
        uint256 liquidationAmount = owedAmount + owedAmount * _liquidationFeeFactor / DECIMALS;
        if (balance > liquidationAmount) {
            _liquidationRemains[baaOwner][_baaRegistry[baaAddress].stablecoinAddress] +=
                balance - liquidationAmount;
        } else if (balance < liquidationAmount) {
            _hasOutstandingBalance[baaOwner] = true;
            _outstandingBalance[baaOwner][_baaRegistry[baaAddress].stablecoinAddress] +=
                liquidationAmount - balance;
        }
    ...
    function getLiquidationRemains (
        address userAddress,
        address stablecoinAddress
    ) external view returns (uint256) {
        return _liquidationRemains[userAddress][stablecoinAddress];
    }
...
//        console.log('atLiquidation owedAmount', owedAmount);
        return baaBalance < owedAmount - negativeVolatilityThreshold;
    }
