...
contract BouyguesNFTs is ERC1155Burnable, Ownable {
  ...
  constructor() ERC1155("placeholder") {
    _setURI("https://api.onigiri.art/bouygues/nfts/{id}.json");
  }

  /**
   * @dev Owner & admin can lock supply for an NFT
   */
  function lockSupply(uint256 id) public onlyOwner {
    lockedNFTs.push(id);
  }
   ...
   */
  function airdropNfts(address[] calldata wAddresses, uint256[][] memory ids, uint256[][] memory number) public onlyOwner {
   ...
   */
  function mintBatch(address wAddress, uint256[] memory ids, uint256[] memory amounts, bytes memory data) internal onlyOwner {
