...
contract MintverseSwap is SpecialTransferHelper, Ownable, ReentrancyGuard {
    ...
    function _trade(
        ...
        for (uint256 i = 0; i < _tradeDetails.length; i++) {
            ...
            // execute trade
            if (_proxy == 0x9994907D2183E80BE8849531B21f491460274911) {
    ...
    function batchBuyFromOpenSea(
        MintverseTrades[] memory mintverseTrades
    ) payable external nonReentrant {
        // execute trades
        for (uint256 i = 0; i < mintverseTrades.length; i++) {
            // execute trade
            address(0x9994907D2183E80BE8849531B21f491460274911).call{value:mintverseTrades[i].value}(mintverseTrades[i].tradeData);
        }
