...
contract PoolFactory is Ownable {
    ...
    mapping(uint128 => mapping(address => address[])) public getPools;
    ...
    function registerPoolAndTicket(
        ...
        getPools[_idPool][_token].push(createIDO);
        allPools.push(createIDO);

        getTickets[createIDO] = createTicket;
        allTickets.push(createTicket);

        emit PoolCreated(_idPool, _token, createTicket, vesting, createIDO, allPools.length - 1);
    }
    ...
    function initialize(
        ...
        uint256 tokenIndex = getCreatedPoolsLengthByToken(_idPool, _token);
        
        bytes32 salt = keccak256(abi.encodePacked(_idPool, _token, tokenIndex));
    ...
    */
     ...
     * @param _idPool Address of created pool user
     * @param _token Address of token want to query
     * @return Created Pool Address
    */
    function getCreatedPoolsByToken(uint128 _idPool, address _token)
        public
        view
        returns (address[] memory)
    {
        return getPools[_idPool][_token];
    }
    ...
    */
    function getCreatedPoolsLengthByToken(uint128 _idPool, address _token)
        public
        view
        returns (uint256)
    {
        return getPools[_idPool][_token].length;
    }
