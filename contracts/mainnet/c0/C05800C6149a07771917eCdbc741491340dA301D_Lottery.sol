...
contract Lottery is Ownable {
    ...
    function selectWinners() external onlyOwner {
        ...
        uint16 _count_winners = 0;
        if (countTicket <= drawHistory[currentDraw].winningTicketCount) {
            _count_winners = countTicket;
        } else {
            _count_winners = drawHistory[currentDraw].winningTicketCount;
        }

        uint16[] memory winners = new uint16[](_count_winners);
