...
interface ITidePool {
    ...
    event Deposit(address owner, uint256 amount0, uint256 amount1);
...
library TidePoolMath {
    ...
    function getWindowSize(int24 _previousWindow, uint256 _lastRebalance, bool _outOfRange) public view returns (int24 window) {
        uint256 diff = (block.timestamp - _lastRebalance) / 1 days;
        window = _outOfRange ? _previousWindow + 4 : _previousWindow - int24(diff);
        if(window < 2) window = 2;
    }
}
