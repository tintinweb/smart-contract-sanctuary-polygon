...
contract CoinFlip is Ownable, ReentrancyGuard{
  ...
  bytes32 public constant GAME_TYPEHASH = keccak256("PlayGame(address buyer,uint256 seed)");
  INFT public NFT;
  ...
  mapping(address => uint256 ) public LoseStreak;
  ...
  event Result(uint256 id, uint256 bet, uint256 randomSeed, uint256 amount, address player, uint256 winAmount, uint256 randomResult, uint256 time, uint256 loseStreak, uint256 winStreak);
  event Referral(uint256 indexed _tokenId, address indexed _user);
  event SendWLStreak(uint256 ts, address win,uint256 winAmount, address lose, uint256 loseAmount, uint256 maxLoseStreak, uint256 maxWinStreak);
  ...
  function claimWLStreak() internal {
        ...
        emit SendWLStreak(block.timestamp, WinStreakLeaderBoard, winFees, LoseStreakLeaderBoard, loseFees, maxLoseStreak, maxWinstreak);
  ...
  function game(uint256 bet, uint256 seed, uint256 _tokenId) public payable nonReentrant returns (bool) {
    ...
    require( msg.value > 0 && msg.value <= amount && amount > 0, "wrong ETH");
    require( poolBal >= msg.value.mul(2) , "Error, insufficent vault balance");
  ...
  function getRandomNumber(uint256 userProvidedSeed) internal {
    ...
    _randomNumber = uint8(_structHash  % 10);

    checkWinner(_randomNumber);
  }
 ...
 function manageRef(uint256 amount) internal {
 
    uint256 tID = referrers[msg.sender];
    ...
    if ( tID != 0 && Level >0 ){
         ...
         manageAmount = amount.mul(Level).div(maxLevel);
         sendRefFunds(payable(referal),manageAmount,tID,slipFees);
      }

      uint256 tID2 = selectNFTRef();
      address referaltID2 = NFT.ownerOf(tID2);
      sendRefFunds(payable(referaltID2),amount.sub(manageAmount),tID2,referalFees.sub(slipFees));
    }

  function checkWinner(uint8 random) internal {
      uint256 winAmount = 0;
      ...
      if(
            (random < 5 && games[gameId].bet == 0) || (random >= 5 && games[gameId].bet == 1) 
        )
      {
        ...
        WinStreak[games[gameId].player] += 1;
        LoseStreak[games[gameId].player] = 0;

        if ( maxWinstreak < WinStreak[games[gameId].player] )
        {
            maxWinstreak = WinStreak[games[gameId].player];
      ...
      {
        WinStreak[games[gameId].player] = 0;
        LoseStreak[games[gameId].player] += 1;

        if ( maxLoseStreak < LoseStreak[games[gameId].player] )
        {
            maxLoseStreak = LoseStreak[games[gameId].player];
            LoseStreakLeaderBoard = games[gameId].player;
        }

      }

      manageRef(games[gameId].amount);
      claimWLStreak();
      sendFund(payable(address(devAddr)), devFeesAmount);

      outcomes[games[gameId].player] = Outcome(games[gameId].id, games[gameId].bet, games[gameId].seed,random,winAmount,block.timestamp,  games[gameId].amount, fees, totalfees);
      emit Result(games[gameId].id, games[gameId].bet, games[gameId].seed, games[gameId].amount, games[gameId].player, winAmount, random, block.timestamp, LoseStreak[games[gameId].player],WinStreak[games[gameId].player]);
