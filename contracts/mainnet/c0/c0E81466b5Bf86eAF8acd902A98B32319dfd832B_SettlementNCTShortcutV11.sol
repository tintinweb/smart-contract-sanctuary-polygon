...
contract SettlementNCTShortcutV11 is ERC2771Context, SettlementV11{
    ...
    constructor(
        ...
        address _dex
    ) ERC2771Context(address(_forwarder)) SettlementV11(orderBookChainId, orderBookAddress){
    ...
    *   and if you are submitting an odd it must be an array of 1 that is the shortcut order
    ...
    function batchFill(Orders.Order[] memory argsArray) external override {
        _batchFill(argsArray);
    }

   /**
   * @notice batchFill with transfer approval of asset to be supplied done via permit function
   ...
   */
    function batchFillWithPermit(Orders.Order[] memory argsArray, uint256 deadline, uint8 permitV, bytes32 permitR, bytes32 permitS) external {
        require(argsArray.length == 1, "At the moment we only support odd shortcut array of size 1");
        Orders.Order memory firstOrder = argsArray[0];
        IERC20WithPermit(firstOrder.fromToken).permit(
            _msgSender(),
            address(this),
            firstOrder.amountIn,
            deadline,
    ...
    function _batchFill(Orders.Order[] memory argsArray) internal {
...
abstract contract SettlementV11 is Ownable{
    using SafeERC20 for IERC20;
    using Orders for Orders.Order;
    ...
    * a = (I'_s * I_b / O_b)
    ...
    function _settle(Orders.Order memory big, Orders.Order memory small) internal virtual{
        ...
        IERC20(small.fromToken).safeTransferFrom(big.maker, _msgSender(), a - O_s - senkenSpreadFee);
    ...
    function cancelOrder(bytes32 hash) external {
        canceledOfHash[_msgSender()][hash] = true;
