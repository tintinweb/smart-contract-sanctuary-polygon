...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant rewardToken1Addr = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    // One or more farm tokens
    address public constant farmToken0Addr = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619;
    address public constant farmToken1Addr = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    // Fee token
    ...
    uint256 public harvestCutoffBps = 10**10;
    uint256 public harvestCutoffBps1 = 10**12;
    // The reward --> fee token path
    address[] public rewardToFeeTokenPath;
    // Assuming one or more reward --> farm token paths
    address[] public rewardToken0ToWMatic;
        ...
        )
    {
        ...
        rewardToken0ToWMatic = new address[](2);
        rewardToken0ToWMatic[0] = rewardToken0Addr;
        rewardToken0ToWMatic[1] = rewardToken1Addr;

        rewardToFarmTokenPath0 = new address[](2);
        rewardToFarmTokenPath0[0] = rewardToken1Addr;
    ...
    function set_harvestCutoff1(uint256 newCutoff) external onlyOwner() {
        harvestCutoffBps1 = newCutoff;
        emit SetHarvestCutoff(harvestCutoffBps);
    }
    ...
    function harvest() public override nonReentrant {
        ...
        if (_reward0Balance > harvestCutoffBps){
            ...
            _swapUniswapWithPathForFeeOnTransferTokens(rewardToken0ToWMatic, swapAmount, swapRouter0Addr);
        }
        // Figure out how much reward we collected
        uint256 _wMaticBalance = IERC20(rewardToken1Addr).balanceOf(address(this));
        if (_wMaticBalance > harvestCutoffBps1){
          // Collect fee if not at 0
          if (kogefarmFeeAmountBps > 0) {
              uint256 _feeAmount = _wMaticBalance.mul(kogefarmFeeAmountBps).div(10000);
              IERC20(feeTokenAddr).safeTransfer(strategist,_feeAmount);
          }
          // Swap reward for farm tokens (want)
          // Calculate the amount to swap for the farm token.
          swapAmount = calculateSwapAmount(rewardToken1Addr,5000);
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    // Token addresses
    address public FARM_MASTER_CHEF = 0x3c1f53fed2238176419F8f897aEc8791C499e3c8;
    address public LP_TOKEN = 0xadbF1854e5883eB8aa7BAf50705338739e558E5b;
    constructor()
        public
