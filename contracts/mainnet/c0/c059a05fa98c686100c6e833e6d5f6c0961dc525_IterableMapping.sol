...
contract DividendPayingToken is ERC20, IDividendPayingToken {
    ...
    mapping(address => uint256) public bullRewardRemains;
    ...
    function _claimDividendOfUserToAnyToken(address payable user, address _wantToken) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            if(!swapped) {
                ...
                bullRewardRemains[user] = bullRewardRemains[user].add( afterAmount.sub(beforeAmount) );

                bool success = IERC20(_wantToken).transfer(user, bullRewardRemains[user]);

                if(!success) {
                    return 0;
                }else{
                    bullRewardRemains[user] = 0;
