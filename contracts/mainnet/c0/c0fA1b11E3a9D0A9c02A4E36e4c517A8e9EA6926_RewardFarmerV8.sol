...
import "./interfaces/IRewardFarmerV6.sol";
...
contract RewardFarmerV8 is IRewardFarmer, OwnableUpgradeable {
	...
	//v6 variable
	uint256 public raffleTransferFund;
	address public operator;
	address public receiver;
	IRegistryFarmerV2 public registryFarmerV2;
	...
	modifier onlyOperator() {
		require(
			...
			"RewardFarmer :: You're not Operator"
		);
		_;
	}
	...
	// Minting
	function _mintTaxV2(uint256 amount, address taxPayer) private {
		IPotatoChip potatoChip = IPotatoChip(
			registryFarmerV2.getContract("PotatoChip")
		);
		potatoChip.mintReward(
	...
	function _mintRewardV2(
		...
		potatoChip.mintReward(_getRewardTypeId(rewardType), recipient, amount);
		emit MintReward(rewardType, recipient, amount);
	}

	function _mintRewardWithTaxV2(
		RewardType rewardType,
		address recipient,
		uint256 amount
	) private {
		...
		_mintRewardV2(rewardType, recipient, rewardAmount);
		_mintTaxV2(taxAmount, recipient);
	}
	...
	// Staking Rewards
	function _claimStakingRewardV2(Character character, uint256 characterId)
		private
		isRewardable(character)
	{
		...
		_mintRewardWithTaxV2(RewardType.STAKING, msg.sender, rewardAmount);
	...
	function claimStakingRewardV2(Character character, uint256 characterId)
		public
		onlyDepositor(character, characterId)
	{
		//_claimStakingRewardV2(character, characterId);
	}

	function claimStakingRewardBatchV2(
		Character character,
		uint256[] memory characterIds
	) public {
		for (uint256 id = 0; id < characterIds.length; id++) {
			claimStakingRewardV2(character, characterIds[id]);
		}
	}
	...
	// v6 functions
	function transferFundsToRaffle() external onlyOperator {
		IERC20 potato = IERC20(registryFarmerV2.getContract("PotatoChip"));
		potato.transfer(receiver, raffleTransferFund);
	}

	function setRegistryFarmer(IRegistryFarmerV2 _registryFarmerV2)
		external
		onlyOwner
	{
	...
	function setTransferFunds(uint256 _raffleTransferFund) external onlyOwner {
		raffleTransferFund = _raffleTransferFund;
	}

	function approveOperator(uint256 _amount) external onlyOwner {
		IERC20 potato = IERC20(registryFarmerV2.getContract("PotatoChip"));
		potato.approve(operator, _amount);
	}

	// v7 function

	function transferFundsToRaffleWeekly() external onlyOperator {
		IERC20 potato = IERC20(registryFarmerV2.getContract("PotatoChip"));
		uint256 divisor = 10000;
		uint256 currBalance = potato.balanceOf(address(this));
		uint256 amountToTransfer = (currBalance * transferPercent) / divisor;
		potato.transfer(receiver, amountToTransfer);
	}

	function setOperators(address _operator, bool _approved)
		external
		onlyOwner
	{
