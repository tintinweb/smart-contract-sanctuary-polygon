...
contract UniswapV3TwapOracle is IRelativePriceOracle, Ownable {
    ...
    event TimeWindowLengthSecondsChanged(uint256 oldTime, uint256 newTime);
    ...
    constructor(address _governor, address[] memory pools_) Ownable() {
        timeWindowLengthSeconds = DEFAULT_TIME_WINDOW_LENGTH_SECONDS_AGO;
        transferOwnership(_governor);
        for (uint256 i = 0; i < pools_.length; i++) {
            _registerPool(pools_[i]);
        }
        emit TimeWindowLengthSecondsChanged(0, timeWindowLengthSeconds);
    }
    ...
    function setTimeWindowLengthSeconds(uint32 _timeWindowLengthSeconds) external onlyOwner {
        emit TimeWindowLengthSecondsChanged(timeWindowLengthSeconds, _timeWindowLengthSeconds);
