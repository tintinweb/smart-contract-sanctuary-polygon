...
contract Penalties is Game, VRF{
  ...
  // @notice we are using a dedicated queue library to track the live games
  using PenaltyQueue for PenaltyQueue.Queue;

  // @notice determines the number of rounds, and the payout for multiple goals
  uint256[] public goalPayouts = [30 ether, 75 ether, 143 ether, 244 ether, 396 ether];

  // @notice simple view function instead of a separate state variable
  function rounds() public view returns(uint256) {
    return goalPayouts.length;
  }

  // @notice length of a round
  uint256 public constant period = 1 days;
  
  // @notice we start in round one
  uint8 public currentRound = 1;

  // @notice variable to enforce only one random word request at a time
  bool public pending = false;

  // @notice base likelihood of hitting the target
  ...
  // @notice tracking the keeper's choices
  mapping(uint8 => PenaltyQueue.Choice) public gafferChoices;

  // @notice we mostly instantiate with chainlink config & other contract pointers
  ...
  // @notice view function to calculate the reveal time for each round
  function revealTime(uint256 _round) public view returns(uint256) {
    return gameTime.start + (_round * period);
  }

  // @notice once a round is complete, we can initiate the Gaffer's dive
  function requestRandomWords() external whenNotPaused {
  ...
  // @notice dedicated Chainlink function
  function fulfillRandomWords(uint256, uint256[] memory randomWords) internal override {
    pending = false;
    uint256 choice = randomWords[0] % 3;
    gafferDives(PenaltyQueue.Choice(choice));
  }

  // @notice owner only function in case chainlink fails
  function ownerFufillRandomWords() external onlyOwner {
    pending = false;
    uint256 choice = uint256(keccak256(abi.encodePacked(blockhash(block.number - 1)))) % 3;
    gafferDives(PenaltyQueue.Choice(choice));
  }

  function updateVRFConfiguration(uint64 _subscriptionId, address _vrfCoordinator, bytes32 _keyHash, uint32 _callbackGasLimit, uint16 _requestConfirmations) external onlyOwner {
  ...
  // @notice gaffer takes a dive
  function gafferDives(PenaltyQueue.Choice choice) internal {
    require(block.timestamp > revealTime(currentRound), "ROUND NOT OVER");
    require(currentRound <= rounds(), "SHOOTOUT OVER");
  ...
  // @notice indicates whether a player has played or not; overrides Game.sol function to use the queue instead
  ...
  // @notice tracks the number of remaining payouts
  function payouts() public view returns(uint256) {
    return queue.length();
  }

  // @notice reveal as many plays as possible up to a specified amount
  function payout(uint256 amount) public whenNotPaused {
  ...
  // @notice boost for good finishers
  function attributeBoost(uint256 tokenId) public view returns (uint32) {
    ...
    return uint32(uint256(stats.finishing) * 1966); // will exceed maxUint16
  }

  // @notice mysterious attribute based boosts
  function traitBoost(uint256 tokenId) public view returns (uint32) {
    ...
    return penaltyBoost ? 10000 : 0; // exceeds maxUint16
  }
...
// @notice Queue library for the Penalties risky game
