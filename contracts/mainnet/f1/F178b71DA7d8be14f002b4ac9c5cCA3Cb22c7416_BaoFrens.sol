...
contract BaoFrens is ERC721, Ownable{
    ...
    constructor() ERC721 ("BaoFrens", "BAOFREN"){}

    function addToken(string memory metadata) external onlyOwner {
        _metadata[maxTokenId++] = metadata;
        _mint(msg.sender, maxTokenId );
    }
    
    function setTokenMetadata(uint256 tokenId, string memory metadata) external onlyOwner {
        if (tokenId < 1) {revert NoToken();}
        if (tokenId > maxTokenId) {revert NoToken();}
