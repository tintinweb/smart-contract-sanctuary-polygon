...
contract CattleFight is OwnableUpgradeable {
    ...
    function userFight(uint times, uint txId, bytes32 r, bytes32 s, uint8 v) external freshTime {
        ...
        bytes32 hash = keccak256(abi.encodePacked(times, txId));
        ...
        txClaimed[txId] = true;
        dailyWins[lastTime] += times;
        userInfo[msg.sender].totalWins += times;
        userInfo[msg.sender].dailyWin[lastTime] += times;
