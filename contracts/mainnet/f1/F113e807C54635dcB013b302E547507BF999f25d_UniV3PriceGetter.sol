...
contract UniV3PriceGetter {
    ...
    function getArrakisPrice(IArrakisVaultV1 _token, uint32 _secondsAgo) external view returns (uint256) {
        address token0 = _token.token0();
        address token1 = _token.token1();
        uint256 price0 = getPrice(token0, _secondsAgo);
        uint256 price1 = getPrice(token1, _secondsAgo);
    ...
    function _getPrice(
        ...
        for (uint24 feeIndex = 0; feeIndex < 3; feeIndex++) {
            ...
            if (price > 0) {
                ...
                totalPrice += ((price * ethPrice) / 1e18) * balance;
                totalBalance += balance;
            }
