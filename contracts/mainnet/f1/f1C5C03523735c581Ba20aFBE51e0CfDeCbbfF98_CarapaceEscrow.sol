...
/// @title Carapace Escrow Contract
/// @notice The Carapace Escrow receives and keeps the ERC20 tokens after a successful vault execution
///         and permits to withdraw tokens by the beneficiaries
contract CarapaceEscrow is ICarapaceEscrow {
    using SafeERC20 for IERC20;
    
    ICarapaceAccess carapaceAccess;

    constructor(
        ICarapaceAccess _carapaceAccessAddress) { 
    ...
    function _onlyAuthorized() private view {
        require(carapaceAccess.getAccess(msg.sender), "NA_ESC");
    }

    /// @inheritdoc ICarapaceEscrow
    function withdraw(address _token, address _to, uint256 _amount) override external onlyAuthorized() {     
