...
contract MaticHunt is Context, Ownable, IBEP20 {
    ...
    uint256 public numWallets = 5;
    uint16 constant PERCENT_DIVIDER = 10000; 
    uint16[5] private ref_bonuses = [1500, 500, 300, 200, 100]; 
    ...
    uint256 sellRate = 1000;
    uint256 constant BUYRATE = 300;
    uint256 constant POOLRATE = 100;
     
    constructor() {
        ceo = payable(msg.sender);		
        dev = payable(msg.sender);		
	
	    _name = "MaticHunt Relics";
        _symbol = "RELICS";
        _decimals = 18;
        _totalSupply =  100_000_000_000 * 10**uint(_decimals); // 100B
        _balances[address(this)] = 60_000_000_000 * 10**uint(_decimals); 
		emit Transfer(address(0), address(this), _balances[address(this)]);    
		_balances[msg.sender] = 40_000_000_000 * 10**uint(_decimals); 
		emit Transfer(address(0), msg.sender, _balances[msg.sender]);    
		
        tarifs[45] = Tarif(45, 360); //8% daily for 45 days => 360% ROI
        tarifs[120] = Tarif(120, 360); //3% daily for 120 days => 360% ROI  
    }
    ...
    function BuyItem(uint8 itemno) public payable {
        ...
        require(msg.value >= 20 ether, "Minimum Buy is 20 MATICS!");
        ...
        uint256 shares1 = SafeMath.div(msg.value,10);
        uint256 shares2 = SafeMath.div(msg.value,5) + shares1;
        ...
        player.huntingLuck = player.huntingLuck + msg.value.div(20 ether);
        
        //distribute 30% Token Purchase Commission
        if(nextMemberNo > numWallets){
            uint256 startno = this.rand(nextMemberNo-numWallets);

            uint256 tokenSum;
                    
            for(uint256 i = startno; i < startno+numWallets; i++) {
    ...
    function HuntRelics(address referral) public payable {
        ...
        require(msg.value >= 0.5 ether, "Minimum Buy is 0.5 MATICS!");
        ...
        if(player.deposits.length > 0 && msg.value >= 20 ether)
        {
            player.huntingLuck = player.huntingLuck + msg.value.div(20 ether);        
        }
    ...
    function userInfo2(address _addr) view external returns(address up, uint40 lastPayout, uint256 luck, string memory profile, uint8[] memory items)
    {
        Player storage player = players[_addr];
        Member storage member = players2[_addr];
        return ( player.upline, player.lastWithdrawn, player.huntingLuck, member.profileData, player.boughtItems);
    }
    ...
    function CollectLoots() external {
        ...
        uint256 deduction = SafeMath.div(SafeMath.mul(amount, 10), 100);
        uint256 reinvest = SafeMath.div(SafeMath.mul(amount, 30), 100);
        uint256 toreceive = SafeMath.div(SafeMath.mul(amount, 60), 100);
        ...
        if(nextMemberNo > numWallets){
            uint256 startno = this.rand(SafeMath.sub(nextMemberNo,numWallets));
    ...
    function ShareAJoy(uint startno, uint256 count) public payable {   
    ...
    function setNumWallets(uint newval) public onlyOwner returns (bool success) {
        numWallets = newval;
        return true;
    }
