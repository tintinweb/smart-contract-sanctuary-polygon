...
/**
 * @title MRL Mintpass Minter
 * @dev Contract for minting the exclusive mintpasses for the https://monsterracingleague.com project
 * @author Phat Loot DeFi Developers
 * @custom:version v1.0
 * @custom:date 24 May 2022
 */
contract MintpassMinter is AccessControl, Withdrawable {
    bytes32 public constant STAFF_ROLE = keccak256("STAFF_ROLE");

    Mintpass private immutable _mintpass = Mintpass(address(0xEDA315c8557015e8d4C57f89A9902DAB1caEF540)); // Polygon
    IERC20 private immutable _usdc = IERC20(address(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174)); // Polygon
    ...
    function flipAllowList(address _address) external onlyRole(STAFF_ROLE) {
        _allowList[_address] = !_allowList[_address];
    }

    function flipAllowListBatch(address[] memory _addresses) external onlyRole(STAFF_ROLE) {
        for (uint256 i = 0; i < _addresses.length; i++) {
            _allowList[_addresses[i]] = !_allowList[_addresses[i]];
        }
    }
    ...
    function safeMint() public returns (uint256 tokenId) {
        ...
        if(!isAllowedToMint(msg.sender)) revert NotOnTheAllowlist();
    ...
    function _safeMint() internal returns (uint256 tokenId) {
        return _mintpass.safeMint(msg.sender);
    }
}
...
/**
 * @title Mintpass for the MRL Container mint
 * @dev ERC721 for the https://monsterracingleague.com project, this is the mintpass that is tradeable for a container
 * @author Phat Loot DeFi Developers
 * @custom:version v1.0
 * @custom:date 24 May 2022
 */
contract Mintpass is ERC721Preset {
    bytes32 public constant MINT_ROLE = keccak256("MINT_ROLE");
    bytes32 public constant APPROVE_ROLE = keccak256("APPROVE_ROLE");

    constructor() ERC721Preset("MRL Mintpass", "MINTPASS") {
