...
contract Membership is Ownable {
    ...
    function userBets(address _user) public view returns (uint256) {
        return _users[_user].bets;
    }

    function userScore(address _user) public view returns (uint256) {
        return _users[_user].score;
    }

    function userNameIndex(address _user) public view returns (uint256) {
        return _users[_user].name;
    }
    ...
    {
        User storage user = _users[account];
        require(score <= user.score, "Not Enough Score");
        user.score -= score;
        return user.score;
    }
