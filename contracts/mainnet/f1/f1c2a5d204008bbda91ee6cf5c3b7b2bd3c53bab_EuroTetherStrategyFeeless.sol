...
contract EuroTetherStrategyFeeless is BaseStrategy {
    ...
    constructor() public BaseStrategy(crvEURTUSD, FUND, CRV, sushiRouter, crvEURTUSD_deposit) {
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        if(crv_harvested > 0) {
            _swapUniswapWithPathPreapproved(crv_usdc_path, crv_harvested);
        }
    ...
    function salvage(address recipient, address token, uint256 amount) public onlyOwner {
        //Note: No CRV should be in this contract after harvesting, if there is, something went wrong or someone manually sent it here
