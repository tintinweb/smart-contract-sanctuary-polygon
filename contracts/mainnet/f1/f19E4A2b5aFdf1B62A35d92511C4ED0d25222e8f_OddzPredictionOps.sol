...
import "hardhat/console.sol";
 import "./OddzPrediction.sol";

contract OddzPredictionOps is KeeperCompatibleInterface, IResolver, Ownable {
    IOddzPrediction[] public predictionMarkets;


    constructor(OddzPrediction[] memory _oddzPrediction) {
        for(uint i = 0 ; i < _oddzPrediction.length; i++) {
            setPredicitionMarket(_oddzPrediction[i]);
        }
        
    }

    /*
     * Implement chainlink Keeper compatible methods
     * to check/execute rounds
     */
        ...
        )
    {
        for(uint i = 0 ; i< predictionMarkets.length; i++ ){
          IOddzPrediction.Round memory round = predictionMarkets[i].getCurrentRoundDetails();

          upkeepNeeded = block.timestamp > round.betEndTimestamp;
    ...
    function performUpkeep(bytes calldata) external override {
        for(uint i = 0 ; i< predictionMarkets.length; i++ ){
            ...
            if (block.timestamp > round.betEndTimestamp && round.roundState != IOddzPrediction.RoundState.ENDED) {
    ...
    /*
     * implements resolver for gelato ops
     */
    function checker() external view override returns (bool canExec, bytes memory execPayload) {
        ...
        for(uint i = 0 ; i< predictionMarkets.length; i++ ){
           ...
           canExec = block.timestamp > round.betEndTimestamp;

           execPayload = abi.encodeWithSignature("executeRound()",  predictionMarkets[i]);
    ...
    function setPredicitionMarket(OddzPrediction _oddzPredicition) public onlyOwner {
       
        require(address(_oddzPredicition) != address(0), "Invalid predicitions");

        predictionMarkets.push(_oddzPredicition);
