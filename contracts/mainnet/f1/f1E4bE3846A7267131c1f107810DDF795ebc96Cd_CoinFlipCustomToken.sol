...
contract CoinFlipCustomToken is VRFV2WrapperConsumerBase {
    ...
    function getCustomTokenBalance() external view returns (uint256) {
        ...
        return ICE.balanceOf(msg.sender);
    }

    function flip(CoinFlipSelection choice, uint256 amount) external returns (uint256, bytes32) {
        IERC20 ICE = IERC20(ICETokenAddress);

        uint256 allowance = ICE.allowance(msg.sender, address(this));
        require(allowance >= amount, "Not enough allowance");
        require(allowance >= entryBet, "Not enough allowance entry bet");
