...
contract BoredApePolyChainClub is ERC721 {
    uint256 public totalSupply;
    uint256 public cost = 0.2 ether;
    uint256 public maxMints = 444;
    ...
    constructor()
        ERC721("Bored Ape Poly Chain Club", "BAPCC")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 444) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://QmaFUHYcEB7sJrMUwGEk44L6TuYK7pYdnBfbVubdB9soN4/", Strings.toString(tokenId), ".json")
            );
    }
