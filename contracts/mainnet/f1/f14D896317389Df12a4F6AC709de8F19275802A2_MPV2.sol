...
import "../interface/IBonusNFT.sol";
contract MPV2 is
    Initializable,
	IMarketplace,
	OwnableUpgradeable,
...

{
	...
	// bonuse nft contract address. will use it to caculate the reducing amount of royalty fee.
	address private _bonusNFTAddress;
...
// create batch transaction to create sale items
	function createDirectSaleItemByBatch(uint256[] memory _tokenIds, uint256[] memory _prices) external onlyOwnerWhenRestricted{
		uint256 _length = _tokenIds.length;
		require(_tokenIds.length == _prices.length,"invalid arguments length");
		for(uint i = 0 ; i < _length; i++) {
			createDirectSaleItem(_tokenIds[i], _prices[i]);
		}
	}
...
// create batch transaction to update sale items
	function updateDirectSaleItemByBatch(uint256[] memory _tokenIds, uint256[] memory _prices) external whenNotPaused{
		uint256 _length  = _tokenIds.length;
		require(_prices.length == _length, "invalid arguments length");
		for(uint i = 0 ; i < _length; i++) {
			updateDirectSaleItem(_tokenIds[i], _prices[i]);
		}
	}
	/// @inheritdoc IMarketplace
	function removeDirectSaleItem(uint256 _tokenId) public override whenNotPaused {
		uint256 index = _allTokensIndex[_tokenId];
		require(index > 0, "Marketplace: non exist marketplace item");
...
// create batch transaction to delete sale items
	function removeDirectSaleItemByBatch(uint256[] memory _tokenIds) external whenNotPaused{
		uint256 _length = _tokenIds.length;
		for(uint i = 0 ; i < _length ; i++) {
			removeDirectSaleItem(_tokenIds[i]);
		}
	}
...
// create batch transaction to buy multi-items
	function batchBuy(uint256[] memory _tokenIds) external payable nonReentrant whenNotPaused {
		address buyer = _msgSender();
		uint256 _totalPrice = 0;
		for(uint j = 0 ; j < _tokenIds.length; j ++ ) {
			MarketItem memory _targetItem = getItemByTokenId(_tokenIds[j]);
			_totalPrice += _targetItem.buyoutTokenPrice;
		}
		// Check whether the settled total price
		require(
			msg.value >= _totalPrice && _totalPrice != 0,
			"Marketplace: invalid price"
		);

		for(uint i = 0 ; i < _tokenIds.length; i++) {
			MarketItem memory targetItem = getItemByTokenId(_tokenIds[i]);
			require(
				buyer != targetItem.tokenOwner,
				"Marketplace: you cannot buy yourself"
			);
	...
	/// @dev Sets `_bonusNFTAddress` address for Bonuse NFT contract
	function setBonuseNFTAddress(address _bonusAddress) external onlyOwner  {
		_bonusNFTAddress = _bonusAddress;
	}
	...
	function payout(uint256 _payoutAmount, MarketItem memory _item) internal  {
		...
		returns (address royaltyFeeRecipient, uint256 royaltyFeeAmount) {
			if (royaltyFeeAmount > 0) {
				...
				(,uint256 reduceMPPercent,,,,) = IBonusNFT(_bonusNFTAddress).getBonusInfoByUser(payee);
				uint256 reduceAmount = royaltyFeeAmount * reduceMPPercent / 10000; // reduceMPPercent's decimals is 2.
				royaltyFeeAmount -= reduceAmount;
