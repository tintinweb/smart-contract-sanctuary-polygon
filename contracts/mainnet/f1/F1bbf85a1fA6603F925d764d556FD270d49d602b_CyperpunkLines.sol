...
contract CyperpunkLines is ERC721 {
    uint256 public totalSupply;
    uint256 public cost = 0.11 ether;
    uint256 public maxMints = 14;
    ...
    constructor()
        ERC721("Cyperpunk Lines", "CPL")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 15) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeiadfoi4fh3rlhviv3jltn5stg6khlqbi75b525wfibng4hq3migme/", Strings.toString(tokenId), ".json")
            );
    }
