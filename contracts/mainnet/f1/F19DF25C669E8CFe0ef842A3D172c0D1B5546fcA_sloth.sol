...
contract sloth is ERC721Enumerable, Ownable {
  ...
  function airDrop (address[] memory _addressList) public onlyOwner {
      uint256 contractOwnerTokenCount = balanceOf(msg.sender);
      uint256 numberOfAddresses = _addressList.length;
      require(contractOwnerTokenCount>=numberOfAddresses);
      
      uint256[] memory tokenIDs = walletOfOwner( msg.sender);
      for (uint256 i; i<numberOfAddresses;i++){
          safeTransferFrom(msg.sender,_addressList[i],tokenIDs[i]);
      }
