...
contract MockToken {
    ...
    function setCompliance(address comply) external {
        _compliance = comply;
    }

    function batchMockMint(address[] calldata users, uint256[] calldata values) external {
        for (uint256 i = 0; i < users.length; i++) {
            ICompliance(_compliance).created(users[i], values[i]);
            emit Transfer(address(0), users[i], values[i]);
        }
    }

    function batchMockBurn(address[] calldata users, uint256[] calldata values) external {
        for (uint256 i = 0; i < users.length; i++) {
            ICompliance(_compliance).destroyed(users[i], values[i]);
            emit Transfer(users[i], address(0), values[i]);
        }
    }

    function batchMockTransfer(address[] calldata from, address[] calldata to, uint256[] calldata values) external {
        for (uint256 i = 0; i < from.length; i++) {
            ICompliance(_compliance).transferred(from[i], to[i], values[i]);
