...
/**
* @dev
* Interface used to get the winners of the lottery and also their token allocation based on the outcome.
**/
interface IPublicLotteryFantomStarter {
    ...
    **/
    function getWinnerTokenAllocation(address _winner) external view returns (uint256);

    /**
    * @dev
    * Returns the maximum allowed participants in the lottery
    **/
    function getMaximumLotteryWinners() external view returns (uint256);
    ...
    **/
    function getLotteryWinnersLength() external view returns (uint256);
    ...
    **/
    function isAlreadyParticipating(address _participant) external view returns (bool);

    /**
    * @dev
    * Check to see if the address already exists in the winners list
    **/
    function isAlreadyWinner(address _participant) external view returns (bool);

    /**
    * @dev
    * Check if lottery has already been played
    **/
    function hasLotteryBeenPlayed() external view returns (bool);

    /**
    * @dev
    * Check if token allocation calculation has been run
    **/
    function hasTokenAllocationCalculationBeenRun() external view returns (bool);
}

/**
* The LaunchpadFantomStarter for the IDO FantomStarter
**/
contract default_FSMultiChainSale is AccessControl {
    ...
    // Lottery contract
    IPublicLotteryFantomStarter public lotteryContract;

    // The Initial sale token price
    // Price of idoToken in payToken value based on ratio
    uint256 public itemPriceRatio;  // If the ido is 0.5 paytoken worth, then this would be 0,5 * 10_000 = 5_000
    ...
    // Max items per user on INO flow
    uint256 public maxItemsPerUser = 1;
    ...
    bool public isINO = false;
    bool public isWhitelistSaleWithoutLottery = false;
    bool public isWhitelistSaleWithLottery = false;
    ...
    // Mapping to keep track of how many buys per wallet
    mapping(address => uint256) public buysPerUserMapping;
    ...
    event ChangedIdoTokenAddress(address admin, IERC20Metadata oldItoToken, IERC20Metadata newIdoToken);

    constructor(
        IERC20Metadata _payToken,
        IPublicLotteryFantomStarter _lotteryContract,
        uint256 _itemPriceRatio,
        ...
        itemPriceRatio = _itemPriceRatio;
    ...
    /**
    * @dev
    * Modifier used to check if the msg.sender is a winner from the lottery
    **/
    modifier onlyWinners() {
        require(isEligibleToBuy() == true, "You are not eligible to buy");
        _;
    }
    ...
    **/
    modifier isNotSoldOut() {
        if (isFCFS == true || isWhitelistSaleWithoutLottery == true) {
            require(isSoldOut() == false, "All items are sold out");
        }
        _;
    }
    ...
    /**
    * @dev
    * Modifier used to check if the lottery has been run already and the allocations have been calculated
    **/
    modifier isLotteryPlayedAndAllocationCalculated() {
        if (isFCFS == false) {
            require(lotteryContract.hasLotteryBeenPlayed(), "The lottery has not been played yet");
            require(lotteryContract.hasTokenAllocationCalculationBeenRun(), "The calculation on IDO tokens has not been run yet");
        }
        _;
    }
    ...
    **/
    modifier isFCFSSale() {
        require(isFCFS == true, "Is not a FCFS Sale");
        _;
    }
    ...
    * @dev Modifier to keep track if the Funding has been canceled
    **/
    modifier isWhitelistWithoutLotterySale() {
        require(isWhitelistSaleWithoutLottery == true, "Is not a Whitelist sale");
        _;
    }
    ...
    * @dev Modifier to keep track if the Funding has been canceled
    **/
    modifier isWhitelistWithLotterySale() {
        require(isWhitelistSaleWithLottery == true, "Is not a Lottery sale");
        _;
    }

    // Getters and Setters
    /**
    * @dev Set if this is a FCFS
    **/
    function setIsFCFS(bool _isFCFS) public isAdmin {
        isFCFS = _isFCFS;
    }

    /**
    * @dev Set If Whitelist Sale Without Lottery
    **/
    function setIsWhitelistSaleWithoutLottery(bool _isWhitelistSaleWithoutLottery) public isAdmin {
        isWhitelistSaleWithoutLottery = _isWhitelistSaleWithoutLottery;
    }

    /**
    * @dev Set If Whitelist Sale with Lottery
    **/
    function setIsWhitelistSaleWithLottery(bool _isWhitelistSaleWithLottery) public isAdmin {
        isWhitelistSaleWithLottery = _isWhitelistSaleWithLottery;
    }

    /**
    * @dev
    * Change the IDO Token address
    **/
    function setIdoTokenAddress(IERC20Metadata _idoToken) external isAdmin {
        IERC20Metadata _oldIdoToken = idoToken;

        idoToken = _idoToken;

        emit ChangedIdoTokenAddress(msg.sender, _oldIdoToken, _idoToken);
    }
    ...
    **/
    function enableClaiming(bool _isClaimingOpen, uint256 _startDateOfClaimingTokens) public isAdmin {
    ...
    **/
    function setMaxItemAllocation(uint256 _maxItemAllocation) external isAdmin {
        require(isClaimingOpen == false, "Claiming is already enabled");
        require(_maxItemAllocation > 0, "The item allocation should be greater than zero");

        maxItems = _maxItemAllocation;
    }
    ...
    **/
    function setEndDateOfClaimingTokens(uint256 _endDateOfClaimingTokens) public isAdmin {
    ...
    **/
    function payTokensSpend() public view returns (uint256) {
    ...
    **/
    function getTotalIdoTokensBoughtAndPayTokensSpend(address _buyer) public view returns (uint256 idoTokens, uint256 payTokens) {
    ...
    **/
    function getTotalIdoTokensBoughtAndPayTokensSpend() public view returns (uint256 idoTokens, uint256 payTokens) {
    ...
    **/
    function getPaymentForEntireAllocation(address _buyer) public view returns (uint256 payTokensAmount) {
        if (isFCFS == false) {
            require(lotteryContract.isAlreadyWinner(_buyer) == true, "The address is not in the winners list");
        }

        if (isINO == false) {
            return calculateMaxPaymentToken(lotteryContract.getWinnerTokenAllocation(_buyer));
        } else {
            if (isFCFS == false) {
                return calculateMaxPaymentINOToken(lotteryContract.getWinnerTokenAllocation(_buyer));
            } else {
                return calculateMaxPaymentINOToken(1);
            }
        }
    }
    ...
    **/
    function getClaimableTokens(address _buyer) public view returns (uint256 claimableTokens) {
        if (isINO == true) {
            return lotteryContract.getWinnerTokenAllocation(_buyer);
        }
    ...
    /**
    * @dev
    * Check to see if the contract owner is eligible to buy
    **/
    function isEligibleToBuy() public view returns (bool canBuy) {
        bool result = false;

        if (isFCFS == false) {
            result = lotteryContract.isAlreadyWinner(msg.sender);
        } else {
            result = true;
        }
    ...
    **/
    function isSoldOut() public view returns (bool soldOut) {
        bool result = false;
    ...
    **/
    function isPreStartFunding() public view returns (bool isPreStart){
    ...
    **/
    function calculateMaxPaymentToken(uint256 _idoTokensToGet) public view returns (uint256 maxPaymentTokens) {
        ...
        uint256 _divideByRatio = itemPriceRatio.mul(10 ** uint256(payToken_decimals()).sub(2)); 38 * 100_000 = 3.800.000
        ...
        **/
        ...
        uint256 _divideByRatio = itemPriceRatio.mul(10 ** uint256(payToken.decimals()).sub(2)); // 38 * 100_000 = 3.800.000
    ...
    /**
    * @dev
    * Calculates how much Payment tokens you need to spend to acquire the exact INO allocation
    **/
    function calculateMaxPaymentINOToken(uint256 _inoToGet) public view returns (uint256 maxPaymentTokens) {

        // price per item is 100.5 * 10_000 = 1_005_000

        // Calculate for 2 items
        uint256 _totalPaytokenPrice = _inoToGet.mul(itemPriceRatio);  // 2 * 1_005_000 = 2_010_000

        _totalPaytokenPrice = _totalPaytokenPrice.mul(10 ** uint256(payToken.decimals())); // 2_010_000 * 10^6 = 2.010.000.000.000

        uint256 _amountInPayToken = _totalPaytokenPrice.div(idoTokenPriceMultiplier);  // 2.010.000.000.000 / 10_000 = 201_000_000
    ...
    **/
    function calculateIdoTokensBought(uint256 _amountInPayToken) public view returns (uint256 idoTokensBought) {
        ...
        **/
        ...
        uint256 _divideByRatio = itemPriceRatio.mul(10 ** uint256(payToken.decimals()).sub(2));
    ...
    **/
    function fundToContract(uint256 _amountInIdoTokens) public isAdmin isFundingClosed isFundingNotCanceledByAdmin {
        require(itemsSold <= _amountInIdoTokens, "You should at least match the itemsSold");
    ...
    **/
    function buy(uint256 _amountInPayToken) external isFundingOpenAndRunning isFundingNotCanceledByAdmin isLotteryPlayedAndAllocationCalculated onlyWinners isNotSoldOut {
        ...
        if (isINO == false) {
            require(_amountInPayToken <= calculateMaxPaymentToken(lotteryContract.getWinnerTokenAllocation(msg.sender)), "You cannot buy more tokens than is allowed according to your lottery allocation calculation");
            require(spendPayTokensMapping[msg.sender].add(_amountInPayToken) <= calculateMaxPaymentToken(lotteryContract.getWinnerTokenAllocation(msg.sender)), "You cannot buy more tokens than is allowed according to your lottery allocation calculation");

            if (inOneTransaction) {
                require(calculateMaxPaymentToken(lotteryContract.getWinnerTokenAllocation(msg.sender)) == _amountInPayToken, "You need to buy the entire allocation in one transaction");
            }
        } else {
            if (isFCFS == false) {
                require(calculateMaxPaymentINOToken(lotteryContract.getWinnerTokenAllocation(msg.sender)) == _amountInPayToken, "You need to buy the entire allocation in one transaction");
            } else {
                require(calculateMaxPaymentINOToken(1) == _amountInPayToken, "You need to buy the entire allocation in one transaction");
            }
        }

        uint256 _idoTokensToBuy = 0;
        if (isINO == false) {
            _idoTokensToBuy = calculateIdoTokensBought(_amountInPayToken);
        } else {
            if (isFCFS == false) {
                _idoTokensToBuy = lotteryContract.getWinnerTokenAllocation(msg.sender);
            } else {
                _idoTokensToBuy = 1;
            }

            buysPerUserMapping[msg.sender] = _idoTokensToBuy;
        }
        ...
        itemsSold = itemsSold.add(_idoTokensToBuy);
    ...
    **/
    function claimTokens() external isFundingClosed isFundingNotCanceledByAdmin isLotteryPlayedAndAllocationCalculated {
    ...
    **/
    function cancelIdoSale(bool _cancel) external isAdmin {
        isFundingCanceled = _cancel;
    }
