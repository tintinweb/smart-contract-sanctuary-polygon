...
contract CryptoTattooerzTokenDirectSale is BaseCryptoTattooerzTokenDirectSale{
    ...
    BaseCryptoTattooerzTokenDirectSale(priceHandlerAddress, tokenAddress, TOKEN) {
    }
}
...
abstract contract BaseCryptoTattooerzTokenDirectSale is CryptoTattooerzCurrencyHandler, Pausable, AccessControlEnumerable, IERC777Recipient{
     ...
     */
    constructor(address priceHandlerAddress, address tokenAddress, bytes32 TOKEN_) CryptoTattooerzCurrencyHandler(TOKEN_) {
    ...
    function setPriceHandlerAddress(address priceHandlerAddress) public onlyRole(DEFAULT_ADMIN_ROLE){
        ...
        if(priceHandlerAddress != address(0)) {
            ...
            require(result == 1, "DirectSale: New PriceHandler contract address does not seem to be valid");
            require(priceHandler.getTOKEN() == getTOKEN(), "DirectSale: New PriceHandler contract is not based on same TOKEN");
        }
    ...
    function _buyWithTokens(bytes32 currency, uint256 erc20TokenAmountRequired, uint256 erc20TokenAmountTransfered, address sender) internal returns (uint256) {
        ...
        _buyFromTokens(currency, tokenAmountRequested, erc20TokenAmountRequired, erc20TokenAmountTransfered,
    ...
    function _buyAmountWithTokens(bytes32 currency, uint256 tokenAmountRequested, uint256 erc20TokenAmountTransfered, address sender) internal returns (uint256) {
        // Calculate the amount of coins corresponding to requested amount of TOKENs
    ...
    function _buyFromTokens(bytes32 currency, uint256 tokenAmountRequested, uint256 erc20TokenAmountRequired, uint256 erc20TokenAmountTransfered,
        ...
        if(sender == address(0)) {
            require(erc20TokenAmountTransfered == 0 , "DirectSale: No amount could already been transfered in pure ERC20 case");
            sender = msg.sender;
        }
        ...
        require(getBalance(getTOKEN()) >= tokenAmountRequested, "DirectSale: Buying more that available TOKENs");
        ...
        if(erc20TokenAmountTransfered < erc20TokenAmountRequired) {
            SafeERC20.safeTransferFrom(getERC20(currency), sender, address(this), erc20TokenAmountRequired - erc20TokenAmountTransfered);
        }
        // Reimburse oversent ERC20 tokens amount if applicable (
        else if(erc20TokenAmountTransfered > erc20TokenAmountRequired) {
            SafeERC20.safeTransfer(getERC20(currency), sender, erc20TokenAmountTransfered - erc20TokenAmountRequired);
        }
