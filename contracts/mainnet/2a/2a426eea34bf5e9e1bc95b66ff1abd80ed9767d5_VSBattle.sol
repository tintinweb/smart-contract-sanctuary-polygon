...
contract AccessControl {
    ...
    modifier onlyCREATOR() {
        require(msg.sender == creatorAddress, 'E0');
        _;
    }
...
contract VSBattle is AccessControl {
    ...
    function beforeRound(uint64 battleId) internal returns (bool) {
        // E2 = battle already over
        require(Battles[battleId].round < 100, 'E2');
    ...
    function battleRound(uint8 _action) public {
        ...
        if (getOrder(Battles[battleId].attacker.speed, Battles[battleId].defender.speed) == true) {
    ...
    function getOrder(uint16 speed1, uint16 speed2) internal view returns (bool) {
        uint8 chance = getRandomNumber(100, 0, msg.sender);

        if (speed1 >= speed2) {
         if (chance < (50 + speed1 - speed2)) {
                return true;
            }
            return false;
        }
        else {
            if (chance < (50 + speed2 - speed1)) {
                return false;
            }
            return true;
        }
    ...
    function endBattle(uint64 battleId) internal {
        // take no further action if the battle is already over
        if (Battles[battleId].round > 100) {
            return;
        }
        IBattleMtnData BattleMtnData = IBattleMtnData(BattleMtnDataContract);
