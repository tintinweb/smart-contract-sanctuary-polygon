...
interface MintRouter {
    function mint(uint256 _cid, uint256 _amount, bool _useNative) external;
}
 ...
 * @author YieldWolf
 */
contract ACMasterChefERC20IndexToken is AutoCompoundVault {
    constructor(
        string memory _name,
    ...
    function _earnToStake(uint256 _earnAmount) internal override {
        ...
        if (stakeToken != earnToken) {
            _safeSwap(_earnAmount, address(earnToken), address(usdc));
        }

        uint256 amountToMint = IERC20(usdc).balanceOf(address(this));

        if (IERC20(address(usdc)).allowance(address(this), address(0xc19B47E9f746F9635e76b65258B02c3472CAaC3c)) < amountToMint) {
            IERC20(address(usdc)).approve(address(0xc19B47E9f746F9635e76b65258B02c3472CAaC3c), type(uint256).max);
        }

        MintRouter(0xc19B47E9f746F9635e76b65258B02c3472CAaC3c).mint(0, amountToMint, true);
    }
