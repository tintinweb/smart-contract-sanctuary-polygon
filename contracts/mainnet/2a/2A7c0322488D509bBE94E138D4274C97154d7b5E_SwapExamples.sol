...
contract SwapExamples is Ownable {
    ...
    // These are all the Polygon versions of the tokens
    address public constant DAI = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063;
    address public constant WMATIC_ADDRESS =
        0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; //Verified
    address public constant USDC = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    address public constant stMATIC =
        0x3A58a54C066FdC0f2D55FC9C89F0415C92eBf3C4; // Staked Lido Matic on Polygon
    address public KYBER_ROUTER_ADDRESS =
        0xC1e7dFE73E1598E3910EF4C7845B68A9Ab6F4c83;

    //      WETH9 (Wrapped Ether) on Ethereum: 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2
    //      WMATIC (Wrapped Matic) on Polygon: 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270

    // For this example, we will set the pool fee to 0.3%. (3000) or 0.008% (80)
    uint24 public constant poolFee = 80;

    //swap router of Kyberswap
    constructor() {
        swapRouter = IRouter(KYBER_ROUTER_ADDRESS);
    }

    // FUNCTION TAKES NATIVE MATIC TOKEN, WRAPS TO WMATIC / 2 AND THEN SWAPS IT ON KYBERSWAP 2 TIMES

    function swapNativeAssetForTokens(uint256 minAmountOut) external payable {
        require(minAmountOut > 1, "Error: Min Out must be more than 0");
        require(msg.value > 0, "Error: No native asset sent");

        WMATIC(WMATIC_ADDRESS).deposit{value: msg.value}();
        emit LogDeposit(msg.value);

        uint256 halfMaticAmount = msg.value / 2;
        ...
        emit LogApproval(WMATIC_ADDRESS, address(swapRouter), msg.value);

        IRouter.ExactInputSingleParams memory paramsToken1 = IRouter
            .ExactInputSingleParams({
                ...
                amountIn: halfMaticAmount,
                minAmountOut: minAmountOut,
                limitSqrtP: 0
            });

        IRouter.ExactInputSingleParams memory paramsToken2 = IRouter
            .ExactInputSingleParams({
                tokenIn: WMATIC_ADDRESS,
                tokenOut: DAI,
                fee: poolFee,
        ...
        uint256 token1Out = swapRouter.swapExactInputSingle(paramsToken1);
        emit LogSwapExecution(
            halfMaticAmount,
            token1Out,
            WMATIC_ADDRESS,
            stMATIC
        ...
        uint256 token2Out = swapRouter.swapExactInputSingle(paramsToken2);
        emit LogSwapExecution(halfMaticAmount, token2Out, WMATIC_ADDRESS, DAI);
    }
}
