...
contract ERC721PaymentOpenSale is ERC721BaseSale, __with_semver(uint8(1), uint8(0), uint8(0)) {
        ...
        returns (uint256 stablePrice, uint256 tokenPrice)
    {
        ...
        require(stableCoinAddress != address(0), Exceptions.BASIC_TOKEN_ADDRESS_NOT_SET);
        uint256 price = UniswapV3Oracle(_oracleAddress).getPrice(stableCoinAddress, tokenAddress);
    ...
    function buy(
        ...
        (, uint256 costInTokens) = getPrice(tokenAddress, tierValue);
        require(costInTokens != 0, Exceptions.VALIDATION_ERROR);
        
        _claim();
        address nativeTokenWrapperAddress = UniswapV3Oracle(_oracleAddress).nativeTokenWrapperAddress();
        if (nativeTokenWrapperAddress != tokenAddress) {
            // transfer ERC20 token to Treasury 
            _tokenTransferFromToTreasury(to, tokenAddress, costInTokens);
        } else {
            require(msg.value >= costInTokens, Exceptions.VALIDATION_ERROR);
            _transferToTreasury();
        }
        // transfer NFT to user
        address appAddress = kernel.getAppAddress(AppsIds.ERC721);
        ERC721Properties(appAddress).mint(to, tierValue);
        _afterBuy(to, appAddress, tierValue, costInTokens, "ERC20_SALE", __semver());
    }

    function setStableCoinAddress(address stableCoinAddress_) external requirePermission(CONTROLLER) {
        require(
            stableCoinAddress_ != address(0x00) && stableCoinAddress_.code.length > 0, 
            Exceptions.NULL_STABLECOIN_ADDRESS
        );
        stableCoinAddress = stableCoinAddress_;
        emit SetStableCoinAddress(stableCoinAddress_);
    }

    function _tokenTransferFromToTreasury(address from, address tokenAddress_, uint256 costInTokens_) internal {
        address wallet = kernel.getAppAddress(AppsIds.TREASURY);
        require(wallet != address(0x00), Exceptions.NO_TREASURY_ADDRESS);
        ...
        require(currentAllowance >= costInTokens_, Exceptions.INSUFFICIENT_ALLOWANCE);

        token.safeTransferFrom(from, wallet, costInTokens_);
    }
}
    ...
    __with_semver(uint8(1), uint8(0), uint8(0))
{
    ...
    function getPrice(address tokenAddressA, address tokenAddressB) external view returns (uint256 price) {
        ...
        if (tokenAddressA == tokenAddressB) {
            return 10 ** _getDecimals(tokenAddressA);
        }
        ...
        return _getPrice(poolAddress, tokenAddressA, _twapInterval);
    }
    ...
    function _getPrice(address uniswapV3Pool, address tokenAddress, uint32 twapInterval) 
