...
contract FlashLoanAAVE is Ownable {
    ...
    ISwapRouter public uniswapRouterSell;
    ISwapRouter public uniswapRouterBuy;
    uint256 sellAmount;
    bool public sellV3;
    bool public buyV3;
    ...
    function _sellAsset(address asset_) internal {
        ...
        if(sellV3){
            sellAsset.approve(address(uniswapRouterSell), sellAmount);
            uniswapRouterSell.exactInputSingle{value: 0}(
    ...
    function _buyAsset(address asset_) internal {
        if(buyV3){
            IERC20(buyToken).approve(address(uniswapRouterBuy), IERC20(buyToken).balanceOf(address(this)));
            uniswapRouterBuy.exactInputSingle{value: 0}(
                ISwapRouter.ExactInputSingleParams({
                    tokenIn: buyToken,
                    tokenOut: asset_,
                    fee: buyFee,
    ...
    function flashloan (
        ...
        if(sellV3){
            uniswapRouterSell = ISwapRouter(_sellDexRouter);
        }
        if(buyV3){
            uniswapRouterBuy = ISwapRouter(_buyDexRouter);
        }
