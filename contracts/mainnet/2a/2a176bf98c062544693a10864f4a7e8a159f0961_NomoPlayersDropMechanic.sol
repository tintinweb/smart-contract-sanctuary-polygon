    ...
    RandomNumberConsumer
{
    ...
    uint256 public privilegedLength;
    ...
    function setPrivileged(address[] memory members) public onlyOwner {
        ...
        privilegedLength += members.length;
    ...
    function executeAirdrop() public onlyOwner isValidRandomNumber {
        ...
        require(privilegedLength > totalAirdropped, "Invalid Airdrop length");

        uint256 max = 150; // max 150 tokens to be airdropped in a single tx

        uint256 airdropLength = (totalAirdropped + max) < privilegedLength ? max : privilegedLength - totalAirdropped;

        if(privilegedLength < max) {
            airdropLength = privilegedLength;
        }

        require(
            (tokens.length >= airdropLength) && (privilegedLength > 0),
        ...
        uint256[] memory airdroppedTokens = new uint256[](airdropLength);
        uint256 zeroBasedCounter = 0;

        uint256 start = totalAirdropped;
        uint256 end = totalAirdropped + airdropLength;

        for (uint256 i = start; i < end; i++) {
            uint256 randomNumber = randomNumbers[zeroBasedCounter] % tokens.length;
            uint256 tokenId = tokens[randomNumber];

            airdroppedTokens[zeroBasedCounter] = tokenId;
        ...
        if(totalAirdropped == privilegedLength) {
