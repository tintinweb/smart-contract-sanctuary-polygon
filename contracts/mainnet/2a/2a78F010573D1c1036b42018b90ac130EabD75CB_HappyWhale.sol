...
contract HappyWhale is ERC721 {
    ...
    constructor()
        ERC721("Happy Whale NFT", "HWN")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 1082) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeibcwpxmnq5fsxviyl366xqo3qpds2czeckmdivasds5ltkcqnsjfu/", Strings.toString(tokenId), ".json")
            );
    }
