  ...
  Ownable
{
  ...
  uint256 m_randNonce = 0;
  mapping(uint256 => uint256) private m_tokenIdToSeed;
  ...
  bool private m_isWhitelisteSaleOpen = false;
  bool private m_isPublicSaleOpen = false;
  ...
  bytes32 public constant ROLE_MUTATOR = keccak256("ROLE_MUTATOR");
  ...
  constructor() ERC721("Creature", "CRNFT") {
    ...
    _setRoleAdmin(ROLE_MUTATOR, ROLE_ADMIN);
  }
  ...
  function setBaseURI(string memory __baseURI) public onlyRole(ROLE_ADMIN) {
    m_baseURI = __baseURI;
  }
  ...
  function adminMint(address to) public payable onlyRole(ROLE_ADMIN) returns (uint256) {
    m_tokenIds.increment();
    uint256 newItemId = m_tokenIds.current();
    require(
      !m_withdrawnTokens[newItemId],
  ...
  function _mintSeed(uint256 _tokenId) internal {
    uint256 l_seed = uint256(
      keccak256(
        abi.encodePacked(block.timestamp, msg.sender, m_randNonce, _tokenId)
      )
    );
    m_tokenIdToSeed[_tokenId] = l_seed;
  }

  function updateSeed(uint256 _tokenId, uint256 _seed)
    external
    onlyRole(ROLE_MUTATOR)
  {
    m_tokenIdToSeed[_tokenId] = _seed;
  }
  ...
  function seed(uint256 _tokenId) public view returns (uint256) {
    return m_tokenIdToSeed[_tokenId];
  }

  function payout(address payable _to) public onlyRole(ROLE_ADMIN) {
    // Payout all value to _to
    return _to.transfer(address(this).balance);
  }
}
