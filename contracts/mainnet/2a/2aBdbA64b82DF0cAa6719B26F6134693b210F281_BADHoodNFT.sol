...
contract BADHoodNFT is RandomGenerator, Ownable, ERC721Enumerable, ERC721URIStorage {
    ...
    function editCard(string calldata name_, uint cardId_, uint curAmount_, uint burnAmount_, uint maxAmount_, string calldata tokenURI_) public onlyOwner {
    ...
    function mintWithId (address player_, uint cardId_, uint tokenId_) public returns (bool) {
        ...
        _safeMint(player_, tokenId_);
        _setTokenURI(tokenId_, tokenId_.toHexString(32));

        return true;
    }

    function mintMulti(address player_, uint cardId_, uint amount_) public returns (uint256[] memory tokenIds) {
