...
contract TransferHelper {
    ...
    /// @notice transfer tokens in if neccesary
    ...
    function _transferIn(IERC20 token, uint256 inputAmount) internal returns (uint256) {
        if (inputAmount == Constants.CONTRACT_BALANCE) {
            inputAmount = IERC20(token).balanceOf(address(this));
        } else {
    ...
    /// @notice transfer tokens out if neccesary
    /// @param token input token
    ...
    function _transferOut(IERC20 token, uint256 outputAmount, address recipient) internal returns (uint256) {
        if (outputAmount == Constants.CONTRACT_BALANCE) {
            outputAmount = IERC20(token).balanceOf(address(this));
        }
        if (outputAmount > 0) {
...
contract WrapNative {
    ...
    function wrapNative(uint256 value) external payable {
        IWETH(WNative).deposit{value: value}();
    }
}
...
contract ZapLiquidity is IZapLiquidity, TransferHelper {
    ...
    function removeLiquidityV2(
        ...
        IERC20(address(params.lp)).safeTransferFrom(msg.sender, address(this), params.amount);
