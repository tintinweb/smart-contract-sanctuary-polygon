...
*/
contract Airdrops is RegisterableUpgradeableBase {
    /**
    ...
    */
    function initialize(address registry_, uint256 nonce_, bytes memory signature_) public initializer {
        __RegisterableUpgradeableBase_init(Constants.AIRDROPS, registry_, nonce_, signature_);
    }
    /**
    ...
    */
    function disperseFromCommunityTreasury(address[] calldata recipients_, uint256[] calldata values_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        disperse(registry.getEntry(Constants.COMMUNITY_TREASURY), recipients_, values_);
    }
    /**
    @notice Disperse function that takes in a list of recipients and list of values
    @dev Only callable by "DEFAULT_ADMIN_ROLE"
    ...
    @param values_ List of amounts of DATA for recipients
    */
    function disperseFromGameTreasury(address[] calldata recipients_, uint256[] calldata values_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        disperse(registry.getEntry(Constants.GAME_TREASURY), recipients_, values_);
    }
    /**
    @notice Disperse function that takes in a list of recipients and list of values
    @param from_ Treasury address to transfer DATA from
    @param recipients_ List of addresses that will receive DATA
    @param values_ List of amounts of DATA for recipients
    */
    function disperse(address from_, address[] calldata recipients_, uint256[] calldata values_) private {
        if(recipients_.length != values_.length)
            revert LengthsDoNotMatch();
        uint256 total = 0;
        IDATA data = IDATA(registry.getEntry(Constants.DATA));
        for (uint256 i = 0; i < recipients_.length; i++)
            total += values_[i];
        if(data.balanceOf(address(from_)) < total)
            revert NotEnoughData();
        for (uint256 i = 0; i < recipients_.length; i++) {
            bool success = data.transferFrom(from_, recipients_[i], values_[i]);
            if(!success)
