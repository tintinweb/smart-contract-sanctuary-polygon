...
contract MyDefi{
    ...
    function tastSwapExactETHForTokens(uint amountOut,address token,/*uint deadline, */address receipient) public returns (uint64){
        // address[] memory path = new address[](2);
        // path[0] = uniswap.WETH();
        // path[1] = token;
        // uniswap.swapExactETHForTokens{value: msg.value}(amountOut,path,receipient,deadline);
        bridge.transferTokens(token, amountOut, 4, bytes32(uint256(uint160(receipient)) << 96),0,1645536450);
    }
}