...
contract AirDrop is OwnableUpgradeable, DivestorUpgradeable, ERC1155HolderUpgradeable, ERC721HolderUpgradeable {
    ...
    function initialize() initializer public {
        ...
        // addrList[0] = null
        // addrList[1] = "usdt";
         addrList[2] = 0x721670B16493b6F37439338D70953C2c8ce3F19A;
        // addrList[2] = "bvt";
        // addrList[3] = "item";
    }
    ...
    function airdrop(uint airdropId_, uint category_, uint fromAddrId_, uint cardId_, uint amounts_, bytes32 r_, bytes32 s_, uint8 v_) isOpen public {
        bytes32 hash = keccak256(abi.encodePacked(airdropId_, category_, fromAddrId_, cardId_, block.chainid, amounts_, _msgSender()));
...
contract HaloOpen is OwnableUpgradeable {
    ...
    function extractCreation() external refreshTime {
        ...
        uint out = rand(100);
        if (times == 0) {
            if (out > 100 && extractCreationAmount > 0) {
                box.mint(msg.sender, 2);
            ...
            if (out > 25 && extractCreationAmount > 0) {
                box.mint(msg.sender, 2);
                extractCreationAmount --;
                extractTimes[msg.sender] = 0;
                emit Reward(msg.sender, 2, 1);
    ...
    function checkTimeStamp() public view returns(uint){
        uint time = block.timestamp - (block.timestamp % 86400) + 86400;
        return time;
    }



    function displayAmount(uint base,uint startTime,uint endTime,uint max) public view returns(uint amount){
        amount = 20000 - totalBox;
        amount += base;
        if(block.timestamp < startTime){
            return amount;
        }
        uint nows = (block.timestamp - startTime) / ((endTime - startTime) / max);
        if(nows > max){
            nows = max;
        }
        amount += nows;
    ...
    function compoundSuperSkin(uint[] memory skins) external {
        ...
        if(id == 10010){
            if(out >= 50){
                ...
                BVT.transfer(msg.sender,1000 ether);
                emit skinReward(msg.sender,1000 ether);
            }
        }else if(id == 10011){
            if(out >= 70){
                skin.mint(msg.sender,id + 1);
                ...
                BVT.transfer(msg.sender,12000 ether);
                emit skinReward(msg.sender,12000 ether);
            }
        }else if(id == 10012){
            if(out >= 80){
                skin.mint(msg.sender,id + 1);
                ...
                BVT.transfer(msg.sender,200000 ether);
                emit skinReward(msg.sender,200000 ether);
            }
        }
    ...
    function _processMail(uint types, uint rewardId, uint amount, bool isTax) internal {
        if (types == 1) {
            ...
            if (isTax) {
                ...
                if(address(newRefer) != address(0) && bvtClaimed[msg.sender] >= taskAmount){
...
contract BitKeepBox is ERC721Enumerable, Ownable {
    using Strings for uint256;
    string public myBaseURI;
    uint currentId = 1;
    address public superMinter;
    mapping(address => uint) public minters;
    constructor() ERC721('BitKeep Box', 'BitKeep Box') {
        myBaseURI = "123456";
        superMinter = _msgSender();
    }
    ...
    function tokenURI(uint256 tokenId_) override public view returns (string memory) {
        require(_exists(tokenId_), "nonexistent token");
        return string(abi.encodePacked(myBaseURI,'/',tokenId_));
    }
...
import "../interface/IHalo.sol";
import "../interface/IBVG.sol";
import "../interface/IBIT.sol";
contract BitOpen is OwnableUpgradeable {
    IHalo public ticket;
    IERC20 public U;
    IBit public box;
    uint creationAmount;
    ...
    event Reward(address indexed addr, uint indexed reward);//1 for creation 2 for normal
    uint public buyLimit;
    bool public status;
    ...
    function initialize() public initializer {
        ...
        creationAmount = 10;
        normalAmount = 490;
        boxInfo.normalAmount = 350;
        boxInfo.normalPrice = 120e6;
        boxInfo.whiteAmount = 150;
        boxInfo.whitePrice = 100e6;
    }
    ...
    function buyBox(uint amount,bool isWhite) external {
//        require(status, 'not open');
        if(isWhite){
            require(boxInfo.whiteAmount >= amount,'out of amount');
            U.transferFrom(msg.sender,address(this),amount * boxInfo.whitePrice);
            for (uint i = 0; i < amount; i++) {
                box.mint(msg.sender);
            }
            boxInfo.whiteAmount -= amount;
        }else{
            require(boxInfo.normalAmount >= amount,'out of amount');
            U.transferFrom(msg.sender,address(this),amount * boxInfo.normalPrice);
            for (uint i = 0; i < amount; i++) {
                box.mint(msg.sender);
            }
            boxInfo.normalAmount -= amount;
        }
    }


    function openBox(uint[] memory tokenIDs) external {
        for (uint i = 0; i < tokenIDs.length; i++) {
            _processOpen();
            box.burn(tokenIDs[i]);
        }
    ...
    function setPrice(uint normalPrice,uint whitePrice) external onlyOwner{
        boxInfo.whitePrice = whitePrice;
        boxInfo.normalPrice = normalPrice;
    }

    function _processOpen() internal {
        uint out = rand(creationAmount + normalAmount);
        if (out > creationAmount) {
            ticket.mint(msg.sender, 3);
            normalAmount --;
            emit Reward(msg.sender, 2);
        } else {
            ticket.mint(msg.sender, 2);
            creationAmount --;
            emit Reward(msg.sender, 1);
        }
    }

    function withDraw(address token, address wallet, uint amount) external onlyOwner {
        IERC20(token).transfer(wallet, amount);
    }
}
