...
/**
Buy/Sell/transfer Tax – 1%

• 0.5% Zanon Grant Treasury - 0.5% of each transaction are auto converted to ETH
and deposited.
• 0.5% to DAO - 0.5% of each transaction are auto converted to ETH and
deposited to the DAO address
 */
...
contract zanonTokenFinal is Initializable, ERC20Upgradeable {
    ...
    address public DAOAddress; // fee wallet address
    // VALUES -----------------------------------------------------------------------------------------------
    ...
    function initialize() public initializer {
       ...
       __ERC20_init("TestERC20ForZanon", "ZCT");
        _mint(address(0x69d3d4499b500A96CB4f8399E6d60dD92FAc829C), 100000000000000000000000000);
        ...
        owner = address(0x69d3d4499b500A96CB4f8399E6d60dD92FAc829C);
        // default fees
        // 1% on BUY
        // 1% on SELL
        // 1% on Transfer
        ...
        DAOAddress = 0x77A9C02C764067d046C767532c5346Ec6C1Ef11a; // fee wallet address
        ...
        // contract do swap when have 3 tokens balance
        swapThreshold = 5000000000000000000000000;
        zPercent = 50; // 0.5%
        DAOPercent = 50; // 0.5%
        
    
        // Set Router Address (uniswap by default)
        address currentRouter =  0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; //mainnet router quickswap
    ...
    function swapBackInETH() internal swapping {
        ...
        //Zanon Grant Tressury    
        uint256 zanonGrantShare = (currentBalanceETH * zPercent) / _feesRates;
        if (zanonGrantShare > 0) {
        (bool zanonGrantShareSuccess, ) = payable(zAddress).call{
                value: zanonGrantShare,
                gas: 30000
            }("");
            zanonGrantShareSuccess = false;
        }

        //Zanon Grant Tressury    
        uint256 DAOShare = (currentBalanceETH * DAOPercent) / _feesRates;
        if (DAOShare > 0) {
        (bool DAOSuccess, ) = payable(DAOAddress).call{
                value: DAOShare,
                gas: 30000
            }("");
            DAOSuccess = false;
