...
contract IFOByProxy is ReentrancyGuard, Initializable {
  ...
  uint256 public maxCap = 200000000000000000000000;
  ...
  function setTimes(uint256 _time, uint256[] memory _weeks) public onlyAdmin {
      require(_weeks.length == 15, 'wrong weeks');
      start = _time;
      for(uint i=0; i<_weeks.length; i++){
          weeksd.push(_weeks[i]);
      }
  }
  ...
  function setActivateRefund(bool _status) public onlyAdmin {
      refund = _status;
  }
  ...
  function deposit(uint256 _amount) public {
    require (block.number > startBlock && block.number < endBlock, 'not presale time');
    require (_amount > 0, 'need _amount > 0');
    require( totalAmount.add(_amount) <= maxCap, 'amount beyond max cap');
  ...
  function harvest() public nonReentrant {
    ...
    uint256 offeringTokenAmount = contribution * 625 * 1000000000000; // 1000000000000 comes from dpex decimals being 18, usdc 6
    // uint256 refundingTokenAmount = getRefundingAmount(msg.sender);

    if(refund == true){
        usdc.safeTransfer(address(msg.sender), contribution);
        userInfo[msg.sender].claimed = true;
        emit Harvest(msg.sender, contribution, 0);
    } else {
        // add logic for 25% at IDO, 5% weekly
        require(block.timestamp > start, 'not starting time');
        uint256 hasClaimed = timesClaimed[msg.sender];
        if(hasClaimed == 0){
            // hasn't claimed, gets 25% unlock 
            uint256 pc25 = offeringTokenAmount.mul(25).div(100);
            timesClaimed[msg.sender] = timesClaimed[msg.sender]+1;
            usdc.safeTransfer(address(msg.sender), pc25);
            emit Harvest(msg.sender, pc25, 1);
        } else {
            // user has claimed already
            // check which cliff we are at
            uint256 indexOfClaim = timesClaimed[msg.sender];
            require(indexOfClaim <= 16, 'invalid claim, you have claimed everything');
            if(block.timestamp >= weeksd[indexOfClaim-1]){
                uint256 pc5 = offeringTokenAmount.mul(5).div(100);
                timesClaimed[msg.sender] = timesClaimed[msg.sender]+1;
                usdc.safeTransfer(address(msg.sender), pc5);
                emit Harvest(msg.sender, pc5, 1);
            }
...
//   }
  ...
  function finalWithdraw(uint256 _lpAmount, uint256 _offerAmount) public onlyAdmin {
    require (_lpAmount <= usdc.balanceOf(address(this)), 'not enough token 0');
    require (_offerAmount <= offeringToken.balanceOf(address(this)), 'not enough token 1');
    usdc.safeTransfer(address(msg.sender), _lpAmount);
