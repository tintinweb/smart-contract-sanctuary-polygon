...
contract Traits is Ownable {
    ...
    mapping(uint256 => uint256) tvvVer;
    ...
    function _checkCompat(string storage trait, string storage value, string storage variation, uint256 cid, Trait[] memory ts, uint256 tsn) internal view virtual returns (bool) {
        if (tvvVer[uint256(keccak256(abi.encodePacked(trait, value, variation)))] > cid + 1) {
            return false;
        }
    ...
    function _ensureVariation(
        ...
        uint256 key = uint256(keccak256(abi.encodePacked(trait, value, variation)));
        if (tvvVer[key] == 0) {
            tvvVer[key] = allCompatibilities.length;
        }
    }
