...
contract Receiver {
    ...
    event Received(uint256 chainIdFrom, bytes nativeSender, bytes message);
    ...
    function receive(bytes memory message) external {
        (uint256 chainIdFrom, bytes memory nativeSender) = _onlyCallProxy();
        emit Received(chainIdFrom, nativeSender, message);
    }

    /// @dev Check that method was called by correct dlnDestinationAddresses from the take chain
    function _onlyCallProxy()
        internal
        view
        returns (uint256 chainIdFrom, bytes memory nativeSender)
    {
        ICallProxy callProxy = ICallProxy(deBridgeGate.callProxy());
        if (address(callProxy) != msg.sender) revert CallProxyBadRole();

        nativeSender = callProxy.submissionNativeSender();
        chainIdFrom = callProxy.submissionChainIdFrom();
    }
}
