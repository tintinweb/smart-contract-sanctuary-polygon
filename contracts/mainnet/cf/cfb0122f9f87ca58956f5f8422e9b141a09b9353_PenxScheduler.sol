...
// File: contracts/mixins/RoleControl.sol
...
contract PenxScheduler is RoleControl {
    IERC20 USDC;
    ISwapRouter router;
    IERC20 set;
    ...
    modifier isValidDepositDate() {
        require(
            _getDayOfWeek(block.timestamp) < 4,
            "Can only deposit before thursdat of each week"
        );
        // require(
    ...
    function _getDayOfWeek(uint256 timestamp) internal pure returns (uint256 dayOfWeek) {
        uint256 _days = timestamp / (24 * 60 * 60);
