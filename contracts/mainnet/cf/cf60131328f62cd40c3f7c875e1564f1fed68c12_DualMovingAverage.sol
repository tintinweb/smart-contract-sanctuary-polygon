...
contract DualMovingAverage is Permissions {
  ...
  event Update(uint256 value, uint256 cumulativeValue, uint256 valueTwo, uint256 cumulativeValueTwo);
  ...
  constructor(
    ...
    require(_timelock != address(0), "MA: Timelock addr(0)");
    ...
    for (uint i = 0; i < _sampleMemory; i++) {
      samples.push();
    }
  }
  ...
  function getValue() public view returns (uint256, uint256) {
    ...
    uint256 sampleDiff = __currentSample.cumulativeValue - firstSample.cumulativeValue;
    uint256 sampleDiffTwo = __currentSample.cumulativeValueTwo - firstSample.cumulativeValueTwo;

    FixedPoint.uq112x112 memory sampleAverage = FixedPoint.fraction(sampleDiff, timeElapsed);
    FixedPoint.uq112x112 memory sampleAverageTwo = FixedPoint.fraction(sampleDiffTwo, timeElapsed);

    return (sampleAverage.decode(), sampleAverageTwo.decode());
  }

  function getValueWithLookback(uint256 _lookbackTime) public view returns (uint256, uint256) {
    ...
    uint256 sampleDiff = __currentSample.cumulativeValue - nthSample.cumulativeValue;
    uint256 sampleDiffTwo = __currentSample.cumulativeValueTwo - nthSample.cumulativeValueTwo;
  ...
  function getLiveSample() external view returns (
    uint64 timestamp,
    uint256 value,
    uint256 valueTwo,
    uint256 cumulativeValue,
  ...
  function getSample(uint256 index) public view returns (
    uint64 timestamp,
    uint256 value,
    uint256 valueTwo,
    uint256 cumulativeValue,
    ...
    onlyRoleMalt(UPDATER_ROLE, "Must have updater privs")
  {
    ...
    liveSample = _createNewSample(nextSampleTime, cumulativeValue, cumulativeValueTwo);
    ...
    for (uint256 i = 1; i <= elapsedSamples; i = i + 1) {
      ...
      liveSample = _createNewSample(sampleTime, cumulativeValue, cumulativeValueTwo);
    }
   ...
   * INTERNAL METHODS
   */
  function _createNewSample(uint64 sampleTime, uint256 cumulativeValue, uint256 cumulativeValueTwo)
    internal
    returns(Sample storage liveSample)
  {
    ...
    if (oldSample.timestamp > 0 && activeSamples > 1) {
      previousSample.value = (previousSample.cumulativeValue - oldSample.cumulativeValue) / sampleLength;
      previousSample.valueTwo = (previousSample.cumulativeValueTwo - oldSample.cumulativeValueTwo) / sampleLength;
    }
...
library FixedPoint {
    ...
    function muli(uq112x112 memory self, int256 y) internal pure returns (int256) {
        uint256 z = FullMath.mulDivRoundingUp(self._x, uint256(y < 0 ? -y : y), Q112);
