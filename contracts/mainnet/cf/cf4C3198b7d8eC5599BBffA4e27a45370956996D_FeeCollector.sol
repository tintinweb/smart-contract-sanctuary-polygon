...
uint256 constant BNB_STARGATE = 102;
uint256 constant BNB_TESTNET_STARGATE = 10102;
...
import {
    ...
    BNB_TESTNET,
    BNB_STARGATE,
    BNB_TESTNET_STARGATE
} from "./constants/Networks.sol";
...
contract FeeCollector is FeeCollectorBase {
    ...
    EnumerableSetUpgradeable.AddressSet private _maxApprovedTokensLifi;
    ...
    function toogleLifiMaxApproval(
        bool _shouldApprove,
        address[] calldata _tokens
    ) external onlyProxyAdmin {
        for (uint256 i; i < _tokens.length; i++) {
            if (_shouldApprove && isTokenMaxApprovedLifi(_tokens[i])) continue;
            else if (!_shouldApprove && !isTokenMaxApprovedLifi(_tokens[i]))
                continue;

            IERC20(_tokens[i]).approve(
                address(LIFI),
                _shouldApprove ? type(uint256).max : 0
            );

            _shouldApprove
                ? _maxApprovedTokensLifi.add(_tokens[i])
                : _maxApprovedTokensLifi.remove(_tokens[i]);
        }
    }
    ...
    function bridgeViaLiFi(
        ...
        if (!_isNative) {
            if (!isTokenMaxApprovedLifi(_srcToken)) {
                IERC20(_srcToken).safeIncreaseAllowance(address(LIFI), _amount);
            }
        }

        _isNative ? LIFI.call{value: _value}(_data) : LIFI.call(_data);
    }

    function maxApprovedTokensLifi() external view returns (address[] memory) {
        return _maxApprovedTokensLifi.values();
    }

    function numberOfMaxApprovedTokensLifi() external view returns (uint256) {
        return _maxApprovedTokensLifi.length();
    }

    function isTokenMaxApprovedLifi(address _token) public view returns (bool) {
        return _maxApprovedTokensLifi.contains(_token);
    }
}
