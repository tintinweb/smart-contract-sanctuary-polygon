...
contract Banker is OwnableUpgradeable, PausableUpgradeable {
    ...
    modifier OwnerOrMember() {
        require(
            owner() == msg.sender || wallets.contains(msg.sender),
    ...
    function releaseToken(address token) external {
        ...
        for (uint256 i = 0; i < wallets.length(); i++) {
            address wallet = wallets.at(i);
            uint256 amountToPay = payPerWeight * team[wallet];
            IERC20(token).safeTransfer(wallet, amountToPay);
        }
    }
    ...
    function sendTokenEth(address to, uint256 amount) external onlyOwner {
