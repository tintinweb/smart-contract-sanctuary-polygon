    ...
    UUPSUpgradeable
{
    ...
    mapping(uint256 => uint256) private _relicSupplyLimits;
        ...
        initializer
    {
        ...
        _relicIds = 6;
        _mint(msg.sender, WEREWOLF_IDENTITY_FRAGMENT, treasury, "");
        _mint(msg.sender, VAMPIRE_IDENTITY_FRAGMENT, treasury, "");
        _mint(msg.sender, UNDEAD_IDENTITY_FRAGMENT, treasury, "");
        emit Mint(msg.sender, WEREWOLF_IDENTITY_FRAGMENT, treasury);
        emit Mint(msg.sender, VAMPIRE_IDENTITY_FRAGMENT, treasury);
        emit Mint(msg.sender, UNDEAD_IDENTITY_FRAGMENT, treasury);
    }
    ...
    function _safeMint(uint256 id) private onlyExistingRelic(id) {
        require(
            totalSupply(id) < _relicSupplyLimits[id],
            "E02: Supply limit exceeded"
        );
        emit Mint(msg.sender, id, 1);
    ...
    function setRelicSupplyLimit(uint256 id, uint256 amount)
        external
        onlyOwner
    {
        _relicSupplyLimits[id] = amount;
    }
