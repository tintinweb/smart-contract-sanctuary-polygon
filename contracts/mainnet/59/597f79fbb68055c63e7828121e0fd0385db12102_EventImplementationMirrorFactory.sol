...
import { IEventImplementationDataTypes } from "./interfaces/IDataTypes.sol";
import "./interfaces/IChamberOfCommerce.sol";
import "./interfaces/IEventImplementation.sol";

/**
 * @title The EventImplementationMirror contract mirrors the behaviour of the EventImplementation.sol contract of GET Protocol core.
 * @author https://github.com/kasper-keunen
 */
contract EventImplementationMirror is IEventImplementationDataTypes{
    ...
    event EventDataSync(EventData eventData);
    event UpdateFinancing(EventFinancing financingData);
    event EventDataSet(EventData eventData);

    constructor(
        address _eventImplementationAddress, 
        address _chamberOfCommerce) {
            eventImplementation = _eventImplementationAddress;
            chamberOfCommerce = _chamberOfCommerce;
            bondCouncil = IChamberOfCommerce(_chamberOfCommerce).bondCouncil();
    }

    modifier councilOrDAO() {
        require(
            ((IChamberOfCommerce(chamberOfCommerce).isDAOController(msg.sender)) || (bondCouncil == msg.sender)),
            "EventImplementationMirror:Caller not DAO Controller or bondCouncil"
        );
        _;
    }

    function syncEventData() external {
        eventData = IEventImplementation(eventImplementation).eventData();
        emit EventDataSync(eventData);
    }

    function setEventData(EventData calldata _data) external councilOrDAO {
        eventData = _data;
        emit EventDataSet(_data);
    }

    // AlERT ONLY DAO CONTROLLER IS NOT ACTIVATE
    function setFinancing(EventFinancing calldata _financing) external councilOrDAO {
...
import "./interfaces/IChamberOfCommerce.sol";
import "./EventImplementationMirror.sol";

/**
 * @title The EventImplementationMirrorFactory deploys EventImplementationMirror contracts. 
 * @author https://github.com/kasper-keunen
 */
contract EventImplementationMirrorFactory is Ownable {
    address public chamberOfCommerce;
    // eventAddress => AddressMirror
    mapping(address => address) public events;
    // addressMirror => bool (is a certain address a mirror deployed by this factory)
    mapping(address => bool) public isCreated;
    // eventAddress => bool  (has a mirror been deployed for a certain eventAddress)
    mapping(address => bool) public isMirror;
    ...
    modifier onlyDAOController() {
        require(
            ...
            "EventImplementationMirrorFactory:Caller not DAO Controller"
        );
        _;
    }
    ...
    function newEventMirror(
        address _eventImplementationAddress
    ) external onlyDAOController returns(address event_) {
        event_ = address(
            new EventImplementationMirror(
                _eventImplementationAddress,
                chamberOfCommerce
            )
        );
        events[_eventImplementationAddress] = event_;
        isMirror[event_] = true;
        isCreated[_eventImplementationAddress] = true;
        emit MirrorDeployed(
            _eventImplementationAddress,
            event_
        );
