...
contract DecadeExchange is OwnableUpgradeable {
    ...
    event Executed(uint indexed askId, uint indexed bidId);
    ...
    function initialize() external initializer {
        ...
        // TODO
        //        _oracles[0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599] = 0xdeb288F737066589598e9214E782fa5A8eD689e8;
        //        // wbtc-eth
        //        _oracles[0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48] = 0x986b5E1e1755e3C2440e960477f25201B0a8bbD4;
        //        // usdc-eth

        // Matic
        _oracles[0x1BFD67037B42Cf73acF2047067bd4F2C47D9BfD6] = 0xA338e0492B2F944E9F8C0653D3AD1484f2657a37;
        // wbtc-eth
        _oracles[0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174] = 0xefb7e6be8356cCc6827799B6A7348eE674A80EaE;
        // usdc-eth
    }
    ...
    function acceptance(LibTrade.Execution[] memory chunk, uint gasprice) public view returns (LibTrade.Acceptance[] memory) {
        ...
        for (uint i = 0; i < chunk.length; i++) {
            ...
            if (_max(exec.ask.deadline, exec.bid.deadline) < block.timestamp) continue;
            if (exec.price < exec.ask.lprice || exec.price > exec.bid.lprice) continue;
            if (!exec.recover(_domainSeparator)) continue;

            if (exec.ask.amount < _fills[exec.askId] + exec.amount) continue;
            if (exec.bid.amount < _fills[exec.bidId] + exec.amount) continue;
                ...
                if (reserveAsk + txCost > exec.amount || reserveBid + txCost > exec.amount) continue;
                ...
                if (IERC20(exec.base).available(exec.ask.account, address(this)) < askTransfers[0]) continue;
                if (IERC20(exec.quote).available(exec.bid.account, address(this)) < bidTransfers[0]) continue;
    ...
    function execute(LibTrade.Execution[] memory chunk) external onlyWhitelisted {
        ...
        for (uint i = 0; i < chunk.length; i++) {
            ...
            emit Executed(exec.askId, exec.bidId);
        }
    }
    ...
    function _reservesOf(address accountAsk, address accountBid, uint amount, uint reserve) private view returns (uint reserveAsk, uint reserveBid) {
        uint _reserve = amount * _min(reserve, RESERVE_MAX) / RESERVE_DENOM;
