...
contract PriceOracle {
    ...
    event PriceUpdated(address indexed token, uint256 old, uint256 newPrice);
    event PriceInitialized(address indexed token, uint256 price);
    event PriceIncrease(address indexed token, uint256 oldPrice, uint256 newPrice, uint256 changedByAmt, uint256 currencyID);
    event PriceDecrease(address indexed token, uint256 oldPrice, uint256 newPrice, uint256 changedByAmt, uint256 currencyID);

    UD60x18 internal ticks;
    UD60x18 internal tickValue;
    UD60x18 internal initial;

    constructor() {
        ticks = ud(1);
        UD60x18 tv = ud(0.000000256e18);
        tickValue = tv;
        initial = ud(0.10e18);
    }

    function calcPrice(uint256 supply) public view returns(UD60x18) {
        UD60x18 total = ud(supply);
        ticks.add(ud(1));
        return total.mul(tickValue.mul(total)).mul(initial).div(total);
    }
