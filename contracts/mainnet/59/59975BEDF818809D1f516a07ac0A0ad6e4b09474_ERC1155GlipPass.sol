...
contract ERC1155GlipPass is ERC1155Lazy {
    /// @dev true if collection is private, false if public
    bool isPrivate;

    event CreateERC1155GlipPass(address owner, string name, string symbol);
    
    function __ERC1155GlipPass_init(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address exchangeProxy, address defaultMinter) external initializer {
        __ERC1155GlipPass_init_unchained(_name, _symbol, baseURI, contractURI, exchangeProxy, defaultMinter);
        emit CreateERC1155GlipPass(_msgSender(), _name, _symbol);
    }
    
    function __ERC1155GlipPass_init_unchained(string memory _name, string memory _symbol,  string memory baseURI, string memory contractURI, address exchangeProxy, address defaultMinter) internal {
    ...
    IERC2981
{
    ...
    mapping(uint256 => uint256) private tokenIdCounter;
    ...
    function convertPass( address account, uint256 id, uint256 amount ) external virtual {
        ...
        require(balanceOf(account, id) >= amount, "Amount too high");
        ...
        uint256 passId = id << 216 ;
        _burn(account, id, amount);

        for (uint256 i = 0; i < amount; i++) {
            tokenIdCounter[id]++;
            uint256 newTokenId = composeTokenId(address(id >> 96), tokenIdCounter[id], passId);
