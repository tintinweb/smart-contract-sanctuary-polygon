...
contract IlluminateTrade is Initializable, OwnableUpgradeable {
    ...
    event BatchCreate(string status);
...
// ids[0] - tokenId, ids[1] - amount, ids[2] -  nooftoken, ids[3] - nftType, isd[4] - collectionId
    ...
    function _saleToken(
        address[] memory fromContract,
        uint256[] memory ids,
        string memory bidtoken // address _conAddr
    ) internal {
        ...
        if (
            ...
            if (ldatas[4] != 0) {
                if (cRoyaltyDetails[ids[0]].colletionRoyalty == ldatas[0]) {
                    for (
                        uint256 i = 0;
    ...
    function saleWithToken(
        ...
        uint256[] memory ids,
        address[] memory fromContract // address _conAddr
    ) public {
        require(
            ids[1] == order_place[fromContract[0]][ids[0]].price.mul(ids[2]),
            "Order is Mismatch"
    ...
    function acceptBId(
        ...
        commonTransfer(msg.sender, bidaddr, _conAddr, ids[0], ids[2], ids[3]);
    }
...
// users[0] - from, users[1] - to
    ...
    function bulkMinting(
        ...
        if (datas[1] == 721) {
            IlluminateOriginals(Illuminate721).mintBatch(
                ipfsmetadata,
                from,
                to,
                count,
            ...
            IlluminateEditions(Illuminate1155).mintBatch(
                ipfsmetadata,
                from,
                to,
                datas[0],
    ...
    function bulkBuy(
        ...
        for (uint i = 0; i < _ids.length; i++) {
            ...
            address[] memory adatas = new address[](2);
            tdatas[0] = _ids[i];
            tdatas[1] = amount[i];
            tdatas[2] = count[i];
            tdatas[3] = _type[i];
            adatas[0] = from[i];
            adatas[1] = fromContract[i];
            if (
                keccak256(abi.encodePacked((bidtoken[i]))) ==
                keccak256(abi.encodePacked(("coin")))
            ) {
                _saleToken(adatas, tdatas, "ETH");
            } else {
                _saleToken(adatas, tdatas, bidtoken[i]);
            }
            commonTransfer(
                from[i],
                msg.sender,
    ...
    function commonTransfer(
        ...
        if (_type == 721) {
            IERC721Upgradeable(_conAddress).safeTransferFrom(from, to, id);
            if (order_place[from][id].price > 0) {
                delete order_place[from][id];
            }
        } else {
            if (
                IERC1155Upgradeable(_conAddress).balanceOf(from, id).sub(count) ==
                0
            ) {
                if (order_place[from][id].price > 0) {
                    delete order_place[from][id];
            ...
            IERC1155Upgradeable(_conAddress).safeTransferFrom(
                from,
                to,
                id,
                count,
    ...
    function bulkorder(
        ...
        for (uint i = 0; i < tokenId.length; i++) {
            orderPlace(tokenId[i], _price, _conAddress, _type, "order");
        }
    }
}