...
contract RewardInternalBackerDistribution is ERC1155Holder, ERC721Holder, ReentrancyGuard, AccessController {
    using ECDSA for bytes32;
    using Address for address;

    /** Event */
    event DepositReward(address indexed campaignOwner, string campaignId, uint256 reward, uint256 timestamp);
    event ClaimedReward(address indexed user, string campaignId, uint256 reward, uint256 amount, uint256 rewardType, uint256 timestamp);
    event OwnerWithdrawReward(address indexed owner, string campaignId, uint256 reward, uint256 rewardType, uint256 remainReward, uint256 timestamp);
    ...
    function depositReward(
        ...
        uint256 _collectionId, // For SemiNFT Type
        ...
        _validateSignatureVerifyReward(_rewardSignature, _rewardOwner, _orderId);
        ...
        if(_rewardOwner.rewardType == RewardStorage.RewardType.Token){
            TransferHelper.safeTransferFrom(_rewardOwner.contractAddress, _msgSender(), address(this), totalAmount);
        } else if(_rewardOwner.rewardType == RewardStorage.RewardType.SemiNft){
            require(_collectionId != 0, "CollectionId must be greater than 0");
            rewardStorage.setCollectionIdByCampaign(_campaignType.campaignId, _campaignType.reward, _collectionId);
        }
        emit DepositReward(_msgSender(), _rewardOwner.campaignId, uint256(_campaignType.reward), block.timestamp);
    }

    function claimReward(
        CampaignType calldata _campaignType,
        uint256 _amount, // claim referral included
        string calldata _orderId,
        ...
        require(_totalUserClaimInCampaign + 1 <= existReward.totalWinner, "Claim: Reward in campaign is over");
        ...
        require(!rewardStorage.isOrderExcuted(_orderId), "OrderId is excuted");
        _validateSignatureClaim(_signature, _campaignType, _orderId, _timestamp, _amount, _msgSender());
        rewardStorage.setUserClaim(_orderId, _campaignType.campaignId, _msgSender(), _campaignType.reward, _amount);
        if(existReward.rewardType == RewardStorage.RewardType.Token){
            TransferHelper.safeTransfer(existReward.contractAddress, _msgSender(), existReward.amountPerWinner * _amount);
        } else if(existReward.rewardType == RewardStorage.RewardType.SemiNft){
            uint256 _collectionId = rewardStorage.getCollectionIdByCampaign(_campaignType.campaignId, _campaignType.reward);
            IBacker1155(existReward.contractAddress).mintItem(_collectionId, existReward.amountPerWinner * _amount, _msgSender());
        }
        emit ClaimedReward(_msgSender(), _campaignType.campaignId, uint256(_campaignType.reward), _amount, uint256(existReward.rewardType), block.timestamp);
    }

    function withdrawRewardFT(
        CampaignType calldata _campaignType, 
        uint256 _timestamp, 
        string calldata _orderId,
        bytes calldata _signature
        ...
        require(!rewardStorage.isOrderExcuted(_orderId), "OrderId is excuted");
        _validateSignatureToWithdraw(_signature, _orderId, _campaignType, _timestamp);
        require(existReward.rewardType == RewardStorage.RewardType.Token, "Reward type must be FT");
        ...
        rewardStorage.setOrderExcuted(_orderId, true);
        TransferHelper.safeTransfer(existReward.contractAddress, _msgSender(), remainReward);
        emit OwnerWithdrawReward(_msgSender(), _campaignType.campaignId, uint256(_campaignType.reward), uint256(existReward.rewardType), remainReward, block.timestamp);
    }
    ...
    function _validateSignatureClaim(
        ...
        bytes32 hashValue = keccak256(
            abi.encodePacked(_orderId, _campaignType.campaignId, _address, uint256(_campaignType.reward), _timestamp, _amount)
        );
        address recover = hashValue.toEthSignedMessageHash().recover(_signature);
        require(recover == _adminVerifier, "Invalid input claim");
    }
    ...
    function _validateSignatureVerifyReward(
        ...
        bytes32 verifyReward = keccak256(
            abi.encodePacked(_orderId, _rewardOwner.amountPerWinner, _rewardOwner.totalWinner, uint256(_rewardOwner.rewardType), _rewardOwner.contractAddress)
        );
    ...
    function _validateSignatureToWithdraw(
        ...
        address _adminVerifier = rewardStorage.getAdminVerifier();
        bytes32 verifyReward = keccak256(
            abi.encodePacked(_orderId, _campaignType.campaignId, uint256(_campaignType.reward), _timestamp)
        );
