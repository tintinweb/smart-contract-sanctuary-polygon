...
contract TestNFTCase is Context, ERC165, IERC721, IERC721Metadata, Ownable {

    using Address for address;

    // Token name
    string private constant _name = "TestNFTCase";
    ...
    // Mapping amount of NFTs in a wallet
    ...
    uint256 public cost = 0.1 * 10**6;

    // Max Wallet Mints per wallet
    uint256 public userLimit = 1;
    ...
    // Mint Token
    IERC20 public constant mintToken = IERC20(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174);

    // Team Receive Wallet
    address public teamWallet = 0xb6322e1B2Dd2accEAABc4a68611457B6B1DD5AAb;

    // Team Receive Wallet
    address public insuranceWallet = 0x99f06de65Ba8Ef57dD264B395be18AA67034B764;

    // Swap Path
    address[] private path;

    // Breakpoints for minting in phases
    uint256 private constant breakpoint0 = 100;
    uint256 private constant breakpoint1 = 400;
    uint256 private constant breakpoint2 = 700;
    ...
    function setMAX_SUPPLY(uint256 newMAX_SUPPLY) external onlyOwner {
        MAX_SUPPLY = newMAX_SUPPLY;
    }
    ...
    function setinsuranceWallet(address insuranceWallet_) external onlyOwner {
        require(
            insuranceWallet_ != address(0),
            'Zero Address'
        );
        insuranceWallet = insuranceWallet_;
    }
    ...
    function mint(uint256 numberOfMints) external {
        ...
        require(
            walletMints[msg.sender] <= userLimit, "I'm sorry you reached the Limit per user"
        );
    ...
    */
    ...
    function _distribute() internal {
        ...
        uint forInsurance = mintToken.balanceOf(address(this));
        if (forInsurance > 0) {
            mintToken.transfer(
                insuranceWallet,
                forInsurance
