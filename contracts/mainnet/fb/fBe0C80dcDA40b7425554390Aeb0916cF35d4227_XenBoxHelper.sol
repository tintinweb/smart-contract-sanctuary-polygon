...
interface IXenBoxUpgradeable {
    ...
    function tokenMap(uint256 tokenId) external view returns (Token memory);
}
...
contract XenBoxHelper {
    ...
    function getReferTokenIdList(
        ...
        require(start < end, "XenBoxHelper: end must over start");
        IXenBoxUpgradeable xenbox = IXenBoxUpgradeable(target);
        tokenIdList = new uint256[](end - start);
        uint256 index;
        for (uint256 tokenId = start; tokenId < end; tokenId++) {
            if (xenbox.tokenMap(tokenId).refer == refer) {
                tokenIdList[index] = tokenId;
                index++;
            }
        }
...
contract XenBoxUpgradeable is ERC721Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize() external initializer {
        ...
        referFeePercent = 20;
        forceDay = 30;
        baseURI = "https://xenbox.store/api/token/1/";
