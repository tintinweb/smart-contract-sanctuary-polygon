    ...
    ERC2771Context
{
    ...
    constructor(
        ...
        GameManager(_minimumAnte, _maximumAnte)
        VRFManager(_oracleContract)
    {
    ...
    function startGame(Outcome _guess) external payable {
        ...
        require(
            ...
            "startGame: House does not have enough funds to start game"
        );
        require(
        ...
        initializeGameData(sender, msg.value, _guess);
    ...
    function _endGame(uint256 _gameId, uint256 _randomNumber) private {
        ...
        removePlayerFromGame(gameData[_gameId].player);
    ...
    function voidGame(uint256 _gameId)
        public
        override
        onlyRole(GAME_MANAGER_ROLE)
    {
        super.voidGame(_gameId);
        houseHold -= gameData[_gameId].ante * RESERVE_RATIO;
    ...
    function toggleOff() public override onlyRole(GAME_MANAGER_ROLE) {
        super.toggleOff();
    }

    /// @notice enables the contract to be used
    function setMinimumAnte(uint256 _minimumAnte)
        public
        override
        onlyRole(GAME_MANAGER_ROLE)
    {
        super.setMinimumAnte(_minimumAnte);
    }

    /// @notice enables the contract to be used
    function setMaximumAnte(uint256 _maximumAnte)
        public
        override
        onlyRole(GAME_MANAGER_ROLE)
    {
        super.setMaximumAnte(_maximumAnte);
    }

    function removePlayerFromGame(address _player) internal override {
...
contract GameManager {
    ...
    event MinimumAnteUpdated(uint256 ante);
    event MaximumAnteUpdated(uint256 ante);
    event GameEnabled();
    event GameDisabled();
    ...
    function initializeGameData(
        ...
        require(
            _guess != Outcome.NOT_SET,
            "initializeGameData: guess must be heads, tails, or side"
        );
    ...
    function removePlayerFromGame(address _player) internal virtual {
        delete playerInGame[_player];
    }

    function voidGame(uint256 _gameId) public virtual {
        require(
    ...
    /// @notice enables the contract to be used
    function setMinimumAnte(uint256 _minimumAnte) public virtual {
        require(
            _minimumAnte != 0,
            "setMinimumAnte: cannot set minimum ante to 0"
        );
        require(
            _minimumAnte < maximumAnte,
            "setMinimumAnte: cannot set minimum ante above maximum ante"
        );
    ...
    /// @notice enables the contract to be used
    function setMaximumAnte(uint256 _maximumAnte) public virtual {
        require(
            _maximumAnte > minimumAnte,
            "setMaximumAnte: cannot set maximum ante below minimum ante"
        );
    ...
    /// @notice enables the contract to be used
    function toggleOn() public virtual {
    ...
    /// @notice pauses the contract from use
    function toggleOff() public virtual {
