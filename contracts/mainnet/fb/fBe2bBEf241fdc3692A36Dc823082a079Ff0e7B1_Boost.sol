...
contract Boost is Initializable, Governable  {
    ...
    function initialize(address _stakedToken, address _rewardToken,  uint256 _duration, address _treasury)
    ...
    modifier onlyNotifier() {
        require(msg.sender == _governor() || notifiers[msg.sender], "!notifier");
        _;
    }
    ...
    function setRewardDuration(uint256 _duration) external onlyGovernor {
        require(block.timestamp >= periodFinish);
        duration = _duration;
    }

    function setTreasuryFee(uint256 _fee) external onlyGovernor {
        require(_fee <= 500);
        treasuryFee = _fee;
    }
    ...
    function openPreStake() external onlyGovernor {
        isPreStake = true;
    }

    function setNotifier(address _notifier, bool _enable) external onlyGovernor {
        notifiers[_notifier] = _enable;
    }

    function _notify(uint256 _reward) internal updateReward(address(0)) {
    ...
    function inCaseTokensGetStuck(address _token) external onlyGovernor {
        uint256 amount = IERC20(_token).balanceOf(address(this));
        inCaseTokensGetStuck(_token, msg.sender, amount);
    }

    function inCaseTokensGetStuck(address _token, address _to, uint _amount) public onlyGovernor {
        if (totalSupply() != 0) {
