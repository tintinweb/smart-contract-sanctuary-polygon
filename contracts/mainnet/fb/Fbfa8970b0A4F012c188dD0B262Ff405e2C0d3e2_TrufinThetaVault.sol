...
contract TrufinThetaVault is TrufinVault, TrufinThetaVaultStorage {
        ...
        returns (uint256 withdrawAmount)
    {
        ...
        if (optionState.nextOption == address(0))
            lastQueuedWithdrawAmount = uint128(
                uint256(lastQueuedWithdrawAmount) - withdrawAmount
            );
            //
        else {
            // Update the vault lockedAmount when we complete the withdrawal between commitAndClose and rollToNextOption
            vaultState.lockedAmount = uint104(
                vaultState.lockedAmount - uint104(withdrawAmount)
            );
        }
    ...
    function commitAndClose() external nonReentrant {
        require(optionState.nextOption == address(0), "t20");
    ...
    function rollToNextOption() external onlyKeeper nonReentrant {
        require(optionState.nextOption != address(0x0), "t19");
        //slither-disable-next-line reentrancy-benign
        (
            address newOption,
        ...
        uint104 lockedBalance = uint104(
            vaultState.lockedAmount - queuedWithdrawAmount
        );
        vaultState.lockedAmount = lockedBalance;
...
.
    ...
    function _completeWithdraw() internal returns (uint256) {
        ...
        require(
            withdrawalRound < round || optionState.nextOption != address(0),
        ...
        ///@dev whitelisted vaults must withdraw each round in order to be able to claim the rebate
        if (IMasterWhitelist(WHITELIST).isVaultWhitelisted(msg.sender)) {
