...
contract Purrcoin is ERC20, AccessControl, ERC721Holder {
    ...
    constructor(address owner, address _collectionAddress) ERC20("Purrcoin", "PURR") AccessControl(){
        ...
        coinAmountPerRewardUnit = 1 * 10 ** 18; // 10 ERC20 coins per rewardUnit, may be changed later on
    ...
    function stake(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddress).ownerOf(tokenId) == address(this),
            "PURR: Error while transferring token");
    ...
    function setCollectionAddress(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "PURR: update to zero address not possible");
