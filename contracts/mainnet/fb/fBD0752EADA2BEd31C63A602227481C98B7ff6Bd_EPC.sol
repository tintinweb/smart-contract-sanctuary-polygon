...
contract EPC{
    ...
    constructor(address ownerAddress, address payable _devAddress, ERC20 EarthPlanet_) public 
    {
        owner = ownerAddress;
        devAddress=_devAddress; 
        EarthPlanet=EarthPlanet_;
    ...
    function adminRegistration(address userAddress, address referrerAddress, uint256 _jamt) public 
    {
        ...
        for(uint8 j=1; j<=10; j++)
        {
            uint256 reward=(_jamt.mul(communityPercent)).div(1000);
    ...
    function buyGameSlot(address userAddress, uint256 amount) payable public 
    {
        ...
        EarthPlanet.transferFrom(msg.sender,address(this),game);
    ...
    function adminBuyLevel(address _user, uint8 level, uint256 _amt) public
    {
        require((msg.sender == owner || msg.sender == devAddress), "only Owner");
        require(isUserExists(_user), "user not exists!");
        require(level<11, "Max 10 Level!");  
        require(users[_user].levelActive[level-1],"Buy Previous Level First!");
        require(!users[_user].levelActive[level],"Level Already Activated!");
        users[_user].levelActive[level]=true;
        total_liquidity+=(levelPrice[level].mul(liquidityFee)).div(100);


        uint256 mintedToken=getMintedToken(_user);

        users[_user].genReward+=mintedToken;
        users[_user].currentLevel=level;
        users[_user].lastTokenWithdraw=block.timestamp;

        communityGlobalCount[level]+=1;
        communityGlobalIndex[level][communityGlobalCount[level]]=_user;

        if(users[_user].partnersCount>=2){
            globalCount[level]=globalCount[level]+1;
            globalIndex[level][globalCount[level]]=_user;
        }

        address boardRef=globalIndex[level][currentGlobalCount[level]];
        if(users[boardRef].boardMatrix[level].firstReferrals.length<1) {
           users[boardRef].boardMatrix[level].firstReferrals.push(_user); 
        }           
        else {
            users[boardRef].boardMatrix[level].reinvestCount+=1;
            uint256 reward=(boardReward[level].mul(1e18)).div(maticRate);
            ...
            if(users[boardRef].boardMatrix[level].reinvestCount<3 || boardRef==owner)
            {
                ...
                users[boardRef].boardMatrix[level].firstReferrals=new address[](0);
                currentGlobalCount[level]=currentGlobalCount[level]+1; 
                adminUpdateBoard(boardRef, level+1);
            }  
            else{
                users[boardRef].boardMatrix[level].firstReferrals=new address[](0);
                currentGlobalCount[level]=currentGlobalCount[level]+1; 
        ...
        for(uint8 i=0; i<2;)
        {
            if(users[upline].levelActive[level])
            {
                uint256 reward=(_amt.mul(REFERRAL_PERCENTS[i])).div(1000);
                emit ReferralReward(upline, _user, reward, level, i+1, maticRate);            
                leftPer=leftPer-REFERRAL_PERCENTS[i];
                i++;
            }
            upline=users[upline].referrer;
        ...
        while(j<11)
        {
            if(users[communityGlobalIndex[level][globalId]].levelActive[level])
            {
                uint256 reward=(_amt.mul(communityPercent)).div(1000);              
                emit CommunityReward(communityGlobalIndex[level][globalId], _user, reward, level, j, maticRate);
                leftCom=leftCom-communityPercent;
                j++;         
            }
            globalId--;
        ...
        address refer= communityGlobalIndex[level][communityGlobalCount[level]-2];
        emit BuyNewLevel(_user, users[_user].id, level, refer, users[refer].id);
    }  
    ...
    function withdrawMintedToken() public {
        ...
        users[msg.sender].lastTokenWithdraw=block.timestamp;
        EarthPlanet.transfer(msg.sender,totalReward);
