...
contract CattleFight is OwnableUpgradeable {
    ...
    function testFight(address addr, uint times) external freshTime {
        dailyWins[lastTime] += times;
        userInfo[addr].totalWins += times;
        userInfo[addr].dailyWin[lastTime] += times;
        uint tempLength = userInfo[addr].userRound.length;
        if (tempLength > 0) {
            if (userInfo[addr].userRound[tempLength - 1] != lastTime) {
                userInfo[addr].userRound.push(lastTime);
            }
        } else {
            userInfo[addr].userRound.push(lastTime);
        }
