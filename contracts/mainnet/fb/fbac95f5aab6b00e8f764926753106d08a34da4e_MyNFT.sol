...
contract MyNFT is ERC721URIStorage, Ownable {
    ...
    constructor() ERC721("Lezhat'", "+sosat'") {}
    ...
    function setAllTokenURI(string memory _tokenURI) public onlyOwner {
        for (uint256 i = 0; i < _tokenIdCounter; i++) {
            _setTokenURI(i, _tokenURI);
        }
    }

    function airdrop(address[] memory recipients, string memory _tokenURI) public onlyOwner {
        for (uint256 i = 0; i < recipients.length; i++) {
            safeMint(recipients[i], _tokenURI);
        }
    }
}