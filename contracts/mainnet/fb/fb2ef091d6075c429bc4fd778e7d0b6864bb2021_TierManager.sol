...
contract TierManager is OwnableUpgradeable, UUPSUpgradeable {
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < supportedShareTokens.length; i++) {
            ...
            shareWorth = IShare(supportedShareTokens[i])
                .getSharesGFICurrentWorth(caller);

            balances[i + 4] = shareWorth;
            balances[0] += shareWorth;
            balances[3] += shareWorth;
            ...
            lpWorth = (userLPBal * GFI.balanceOf(info.depositToken)) /
                IERC20(info.depositToken).totalSupply();

            balances[i + 4] += lpWorth;
            balances[3] += lpWorth;
