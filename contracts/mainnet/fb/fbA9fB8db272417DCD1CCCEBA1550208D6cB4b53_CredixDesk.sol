 ...
 * @author AlloyX
 */
contract CredixDesk is ICredixDesk, AdminUpgradeable {
   ...
   */
  function getCredixWalletUsdcValue(address _poolAddress) external view override returns (uint256) {
    return config.getCredixOracle().getNetAssetValueInUsdc(_poolAddress);
  }
   ...
   */
  function increaseUsdcValueForPool(address _vaultAddress, uint256 _amount) external override onlyOperator {
    config.getCredixOracle().increaseUsdcValue(_vaultAddress, _amount);
  }
