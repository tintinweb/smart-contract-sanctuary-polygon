        ...
        for(uint256 i = 0; i < tokens_.length; i++) {
            ...
            for(uint256 j = 0; j < protocols_.length; j++) {
                ...
                if (tokenInfos_[i].protocolTokenInfos[j].isProtocol) {
                    ...
                    tokenInfos_[i].protocolTokenInfos[j].protocolTokenSupplyLimit = liquidity.protocolSupplyLimit(protocols_[j], tokens_[i]);        
                    tokenInfos_[i].protocolTokenInfos[j].protocolTokenBorrowLimit = liquidity.protocolBorrowLimit(protocols_[j], tokens_[i]);
                }
            }
        }
    }
