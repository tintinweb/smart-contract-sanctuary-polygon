...
contract ElementalParticles is ERC20, ERC20Burnable {
    ...
    uint256 public constant DAILY_EMISSION_DENOMINATOR = 1341; // <<TESTING to calculate before deployment>>
    ...
    constructor() ERC20("Elemental Particles", "EP") {
        _mint(msg.sender, 1e18 * 18133); // <<TESTING to calculate before deployment>> Initial airdrop distributed to NFT owners
    }
    ...
    function claimableRewards(address account) public view returns (uint256, uint256, uint256) {
        ...
        uint256 totalRewards = dailyFBXEmissionPerEP() * stakedEP[account] * elapsedTime / 24 / 3600 / 1e18;
    ...
    function claimRewards() public returns (uint256) {
        ...
        if (totalRewards > 0) {
            amountClaimed[msg.sender] += userRewards;
            // SAFE_FBX.approve(address(this), totalRewards); TODO
            SAFE_FBX.safeTransfer(TEAM_MULTISIG, teamRewards);
            SAFE_FBX.safeTransfer(msg.sender, userRewards);
        }
        return userRewards;
    }
    ...
    function mintEP(uint256 amountEP) public {
