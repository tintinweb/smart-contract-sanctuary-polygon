...
contract MimibrosMarketplace is ReentrancyGuard {
    ...
    function _validatePurchase (address _beneficiary, uint256 weiAmount ) internal view returns (uint256){
        uint256 requiredAmount=0;
        requiredAmount = weiAmount.mul(10 ** 18).div(tokenPrice);
        require(IERC20(_tokenAddress).balanceOf(address(this))>=requiredAmount,'Not enough tokens to buy');
        require(requiredAmount>0,"Not enough bnb");
    ...
    function buyToken() external payable nonReentrant{
        ...
        tokensAmount = _validatePurchase(msg.sender, weiAmount);   
    ...
    function setTokenPrice(uint256 _newprice) public onlyModerator{
        require(_newprice!=tokenPrice);
        tokenPrice = _newprice;
        emit TokenPriceChanged(
            _newprice
        );
    }
    function tokenTransfer(uint256 amount)public onlyOwner{
