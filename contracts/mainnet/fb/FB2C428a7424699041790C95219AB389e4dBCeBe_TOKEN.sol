...
contract TOKEN is IERC20, OwnableUpgradeSafe, LGEWhitelisted {
    ...
    bool public _allowedCheck;
	
	mapping(address => bool) public _allowedSender;
	mapping(address => bool) public _allowedRecipient;
	
	mapping(address => bool) public _allowedExcluded;
        ...
        initializer
    {
		...
		setAllowedCheck(true);
		setAllowedExcluded(_msgSender(), true);
    }

    function setAllowedCheck(bool check) public onlyOwner {
        _allowedCheck=check;
    }

    function setAllowedExcluded(address a, bool excluded) public onlyOwner {
        _allowedExcluded[a]=excluded;
    }
    
    function setAllowedSender(address a, bool allowed) public onlyOwner {
        _allowedSender[a]=allowed;
    }

	function setAllowedRecipient(address a, bool allowed) public onlyOwner {
        _allowedRecipient[a]=allowed;
    }
	...
	function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(_allowedCheck && !_allowedExcluded[sender])
            require(_allowedSender[sender] || _allowedRecipient[recipient], "Invalid transfer");
