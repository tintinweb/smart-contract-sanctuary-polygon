...
contract ERC20 is Context, IERC20, IERC20Metadata, ReentrancyGuard {
    ...
    uint public CAP = 990 * 1000 * 1000 * 1000 * ( 10 ** decimals()); 
...
contract SuiDoge is ERC20("SuiDogeCoin","SUIDG"), GetPriceFeed {

    uint public TIME_LAPSE = 14400; 

    uint public BASE = 10 ** 8;
    ...
    function airdrop(uint _amount) public payable nonReentrant() {
        ...
        if(lockedUntil[msg.sender] == 0){ users = users + 1; }

        uint payment = price(_amount);

        require(msg.value * 10_030 >= payment * 10_000, "SuiDoge: Insufficient payment.");
    ...
    function updateTier(uint8 _tier) public onlyOwner(){
        require(tier >= 0 && tier < 5,"SuiDoge: Invalid Tier.");
        tier = _tier;
    }   
    ...
    function updatePurchaseLock(bool _locked) public onlyOwner(){
     ...
     function price(uint _amount) public view returns(uint){
            ...
            if(_amount <= AIRDROP_AMOUNT){
                ...
                uint dollars = priceInDollar(_amount - AIRDROP_AMOUNT); 

                uint coinPrice = uint(getPrice());  

                return dollars * (10 ** decimals()) / (coinPrice); 

            }
    }


    function priceInDollar(uint _amount) public view returns (uint){
        if(tier == 1){
            return _amount * BASE / (800 * 1000 * (10 ** 18));
        } else if (tier == 2){
            return _amount * BASE / (400 * 1000 * (10 ** 18));
        } else if( tier == 3){
            return _amount * BASE / (200 * 1000 * (10 ** 18));
        } else {
            return _amount * BASE / (100 * 1000 * (10 ** 18));
        }
    }
