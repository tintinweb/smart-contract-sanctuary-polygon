...
/*
 * @Title An ERC-1155 Smart Contract For NUDIE COMMUNITY
 * @Web https://www.nudiecommunity.io/
...
abstract contract ERC2981 is IERC2981, ERC165 {
     ...
     */
    function _setDefaultRoyalty(address receiver, uint96 fraction)
        internal
        virtual
    {
        require(
...
// NudieCommunityAirdrop.sol

pragma solidity ^0.8.2;

contract NudieCommunityAirdrop is
    ERC1155,
    ERC2981,
    AccessControl,
    Pausable,
    ...
    ERC1155Supply
{
    ...
    constructor()
        ERC1155(
            "https://gateway.pinata.cloud/ipfs/QmdkmQ8bK2xyS3S4x95h9JgoAgJD2PmScqXGuQKEKE6HXa/{id}.json"
        )
    {
    ...
    function airdrop_cycle(
        address[] calldata recipients,
        uint256 startTokenId,
        uint256 endTokenId
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < recipients.length; i++) {
            if (holders[recipients[i]] != true) {
                for (uint256 j = startTokenId; j <= endTokenId; j++) {
                    _mint(recipients[i], j, 1, "");
                }
                holders[recipients[i]] = true;
            }
        }
    }
    ...
    function airdrop_cycle(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            for (uint256 ii = 0; ii < amounts[i]; ii++) {
