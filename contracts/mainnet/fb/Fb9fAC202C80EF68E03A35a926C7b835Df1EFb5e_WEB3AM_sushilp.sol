...
contract WEB3AM_sushilp {
    ...
    // Add Liquidity Function ,if user has both the tokens 
    function addLiquidity(address _tokenA, address _tokenB, uint _amountA, uint _amountB) public {
        ...
        uniswapV2Router.addLiquidity(
            ...
            0,//Bounds the extent to which the B/A price can go up before the transaction reverts. Must be <= amountADesired.
            0,//Bounds the extent to which the A/B price can go up before the transaction reverts. Must be <= amountBDesired.
    ...
    // Add Liquidity when user has ETH 
    ...
    function addLiquidityETH(address _token, uint _amountToken) public payable {
        ...
        uniswapV2Router.addLiquidityETH{value : msg.value}(
            _token,//A pool token.
            _amountToken,//The amount of token to add as liquidity if the WETH/token price is <= msg.value/amountTokenDesired (token depreciates).
