...
// File: contracts/StableR_INRS_USDC_MATIC_Exchange.sol
...
contract StableR_INRS_USDC_MATIC_Exchange {
 ...
 function getINRSUSDCPrice() public view returns (int) {
    ...
    int inrs_Usdc_Price = inrUsdPrice / usdcUsdPrice;

    return inrs_Usdc_Price;
}
 ...
 function getINRSMATICPrice() public view returns (int) {
    ...
    int inrs_Matic_Price = inrUsdPrice / maticUsdPrice;

    return inrs_Matic_Price;
}
...
function setINRSMATICPrice(uint _INRSMATICPrice) public {
        ...
        int latestPrice = inrUsdPrice / maticUsdPrice;
    ...
    function depositINRS(uint256 _amount) public  {
    require(ERC20(StableRINRS).transferFrom(msg.sender, address(this), _amount), "Transfer failed.");
    tokenBalances[StableRINRS] += _amount;
    }

    function depositUSDC(uint256 _amount) public  {
    require(ERC20(USDC).transferFrom(msg.sender, address(this), _amount), "Transfer failed.");
    ...
    function withdrawINRS(uint256 _amount) public {
    require(msg.sender == owner, "Only owner can withdraw");
    require(_amount <= tokenBalances[StableRINRS], "Insufficient balance");
    tokenBalances[StableRINRS] -= _amount;
    require(ERC20(StableRINRS).transfer(msg.sender, _amount), "Transfer failed.");
    }
   ...
   require(msg.sender == owner, "Only owner can withdraw");
    ...
    tokenBalances[USDC] -= _amount;
    require(ERC20(USDC).transfer(msg.sender, _amount), "Transfer failed.");
    }

    function withdrawOtherToken(address _token, uint256 _amount) public {
    require(msg.sender == owner, "Only owner can withdraw");
    require(_token != address(0), "Invalid token address");
    require(_amount <= tokenBalances[_token], "Insufficient balance");
    tokenBalances[_token] -= _amount;
    ...
    function exchangeToken(address _token, uint _amount) external  {
    ...
    if(_token == StableRINRS) {
        ...
        ERC20(USDC).transfer(_sender, _send);
        ...
        ERC20(StableRINRS).transfer(_sender, _send);
...
// for MATIC TO INRS & INRS to MATIC

function exchangeMATICFORToken(address _token, uint _amount) external payable  {
    require(!paused, "Contract is paused");
    address  _sender = address(msg.sender);
    
    if(_token == StableRINRS) {
        uint256 _received = _amount;
        uint256 _send = _received * INRSMATICPrice;
        tokenBalances[_token] += _received;
        payable(msg.sender).transfer(_send);
