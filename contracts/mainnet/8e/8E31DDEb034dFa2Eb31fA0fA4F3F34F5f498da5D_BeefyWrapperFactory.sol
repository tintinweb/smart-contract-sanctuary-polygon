...
contract BeefyWrapperFactory {
  ...
  constructor(address _instance) {
    if (_instance == address(0)) {
      instance = new BeefyWrapper();
    } else {
      instance = BeefyWrapper(_instance);
    }
  }

  // Creates a new Beefy Vault wrapper as a proxy of the template instance
  // A reference to the new proxied Beefy Vault wrapper
  function cloneVault() external returns (BeefyWrapper) {
    return BeefyWrapper(cloneContract(address(instance)));
  }
