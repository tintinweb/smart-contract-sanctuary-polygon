...
contract Token is ERC20, AccessControl {
	...
	address private constant VALUE_ADDRESS = 0x000000000000000000000000000000000000dEaD;
	...
	constructor(
		...
		_transfer(admin, VALUE_ADDRESS, 10**21);

		emit Transfer(address(0), _msgSender(), _tTotal - 10**21);
	}

	/**
	 * @notice Initialize function
	 * @dev Also request price and save as lastRebasePrice
	...
	/**
	 * @notice Change fee params
	 * @dev Holders fee calculates as 1000 - operation - burn
	 * @param _operationFee operation fee in range [0,500]
	 * @param _burnFee burn fee in range [0,500]
	 * @param _feeFree percent that user can sold in month
	 * @param _sellFee fee that takes from sell above fee free limit
	 */
	function setFee(
	...
	/**
	 * @notice Change exclude from fee
	 * @param who address
	 */
	function changeExcludedFromFee(address who) public onlyRole(DEFAULT_ADMIN_ROLE) {
		_isExcludedFromFee[who] = !_isExcludedFromFee[who];
	}

	/**
	 * @notice Exclude from reward
	...
	/**
	 * @notice Include in reward
	 * @param account address account
	 */
	function includeInReward(address account) public onlyRole(DEFAULT_ADMIN_ROLE) {
		require(_isExcluded[account], "Account is already excluded");
	...
	function rebase() public initialized {
		...
		if (currentPrice > lastRebasePrice && currentPrice > (lastRebasePrice * 1005) / 1000) {
			...
			uint256 previousSupply = _tTotal;
			lastRebasePrice = currentPrice;
			_rebase(supplyDelta);
			emit Rebase(_tTotal, previousSupply);
		}
	}

	/**
	 * @notice Request how much user can sell in current month
	...
	/**
	 * @notice Request current price of token
	 */
	function getCurrentPrice() public view initialized returns (uint256) {
		uint256 price;
	...
	/**
	 * @notice Request current value of dead wallet
	 */
	function getWalletValue() public view returns (uint256) {
		return getCurrentPrice() * balanceOf(VALUE_ADDRESS);
	}
...
library BokkyPooBahsDateTimeLibrary {
	...
	uint256 constant TEST_OFFSET = 0;

	// uint256 constant SECONDS_PER_DAY = 20;
	// uint256 constant TEST_OFFSET = 1646002147;
	...
	{
		(year, month, day) = _daysToDate((timestamp - TEST_OFFSET) / SECONDS_PER_DAY);
	}
