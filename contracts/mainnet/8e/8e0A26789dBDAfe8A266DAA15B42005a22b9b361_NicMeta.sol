...
contract NicMeta is ERC721Enumerable, Delegated, ReentrancyGuard{
    using Strings for uint256;
     IERC20 public tokenAddress;
     uint256 public mintPrice = 100 * 10 ** 18;//单价

    bool public _isSaleActive = false;//售卖激活
    bool public _revealed = false;//开关盲盒

    // Constants
    uint256 public constant MAX_SUPPLY = 1000;//总量

    uint256 public maxBalance = 10;//最大数量
    uint256 public maxMint = 1;//一次只能挖一个

    string baseURI;//盲盒的url
    string public notRevealedUri;//默认未开启的盲盒url
    ...
    // uint[] public tokenIDPledgeArray
  ...
  /**
    ...
    constructor(address _tokenAddress)//设置合约
        ERC721("Nic Meta", "NM")
    {
         tokenAddress = IERC20(_tokenAddress);
    }

    function mintNicMeta(uint256 tokenQuantity) public  {//开始挖盲盒
        require(
    ...
    function marketingMint(uint256 tokenQuantity) external onlyOwner {//团队预留
        ...
        //_safeMint(owner(), quantity);//发给自己
        _mintNicMeta(tokenQuantity);
  }

//     function pledge(uint256  tokenId) public { //质押
//        uint256[] memory array;
//        array = tokenIDPledge[msg.sender];
//        tokenIDPledge[msg.sender].push(tokenId);
//        tokenIDPledge[msg.sender] = array;
//        safeTransferFrom(msg.sender, address(this), tokenId);
//   }

//     function redemption(uint256 tokenId) public { //赎回

//         uint256 len = tokenIDPledge[msg.sender].length;
//         for (uint256 i=0; i<len-1; i++) {
//             if (tokenIDPledge[msg.sender][i] == tokenId) {
//                 delete tokenIDPledge[msg.sender][i];
...
/**
  ...
  */
    function unStake( uint[] calldata tokenIds ) external {
        require( isStakeActive, "Staking is not active" );
  ...
  /**
    ...
    *   @dev usable only if _array contains unique elements only
    ...
    function tokenURI(uint256 tokenId)//h获取对应的token的url
        public
        view
        virtual
        override
    ...
    //only owner
    function flipSaleActive() public onlyOwner {//允许盲盒的销售
        _isSaleActive = !_isSaleActive;
    }

    function flipReveal() public onlyOwner {//开启盲盒
        _revealed = !_revealed;
    }

    function setMintPrice(uint256 _mintPrice) public onlyOwner {//设置的盲盒的价钱
        mintPrice = _mintPrice;
    }

    function setNotRevealedURI(string memory _notRevealedURI) public onlyOwner {//设置未开盲盒的url
        notRevealedUri = _notRevealedURI;
    }

    function setBaseURI(string memory _newBaseURI) public onlyOwner {//设置盲盒的url
        baseURI = _newBaseURI;
    }

    function setBaseExtension(string memory _newBaseExtension)//设置基本扩展
        public
        onlyOwner
    {
    ...
    function setMaxBalance(uint256 _maxBalance) public onlyOwner {//setMaxBalance
        maxBalance = _maxBalance;
    }

    function setMaxMint(uint256 _maxMint) public onlyOwner {//设置最大余额
        maxMint = _maxMint;
    }

    function withdraw(address to) public onlyOwner {//提取
        uint256 balance = address(this).balance;
        payable(to).transfer(balance);
    }

    function batchsafeTransferFrom(address from, address to, uint256[] calldata tokenIds) public {//批量转账
