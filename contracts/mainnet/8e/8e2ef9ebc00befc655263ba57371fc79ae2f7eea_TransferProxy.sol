...
contract TransferProxy {
    ...
    */    
    ...
    function erc721Mint(IERC721 token, address from, string memory tokenURI) external onlyOperator {
        token.createCollectible(from, tokenURI);
    }

    function erc1155Mint(IERC1155 token, address from, string memory tokenURI, uint256 supply) external onlyOperator {
        token.mint(from, tokenURI, supply);
    }

    function mintEnable(IERC721 token) external onlyOperator {
        token.enableMint();
    }
}