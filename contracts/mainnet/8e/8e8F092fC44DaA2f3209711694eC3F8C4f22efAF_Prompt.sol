...
contract Prompt is ERC1155, ERC1155URIStorage, ReentrancyGuard {
    ...
    function authoriseBatch(
        ...
        require(_initialAmount > 0, "Initial amount must be greater than zero");
        require( _tokenIds.length == _tokenURI.length , "Invalid length");

        for (uint256 i = 0; i < _tokenURI.length; i++) {
            require(tokenOwners[_tokenIds[i]] == address(0) ,"the slot is occupied");
            tokenOwners[_tokenIds[i]] = msg.sender;

            // first mint
            _mint(msg.sender, _tokenIds[i], _initialAmount, "");
            _setURI(_tokenIds[i], _tokenURI[i]);
            roots[_tokenIds[i]] = _root[i];

            emit Authorised(_tokenIds[i], msg.sender);
        }
    }
