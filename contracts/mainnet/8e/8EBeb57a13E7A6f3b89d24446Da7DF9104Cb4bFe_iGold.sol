...
contract iGold is ERC20, Ownable {
    ...
    function returnVersion() public pure returns (string memory) {
        return ("Version: 1.0");
    }
    ...
    function buy(uint256 usdtAmount) public {
        ...
        require(_totalSupply <= goldReserve.div(10), "gold reserve reached");
        int256 goldPrice = getLatestGoldPriceGram();
        require(goldPrice > 0, "Invalid gold price");

        uint256 goldAmount = usdtAmount.mul(1e2).div(uint256(goldPrice)).mul(1e1).mul(1e8); // 0.1g per token
    ...
    function sell(uint256 goldAmount) public {
        int256 goldPrice = getLatestGoldPriceGram();
        require(goldPrice > 0, "Invalid gold price");

        uint256 usdtAmount = goldAmount.mul(uint256(goldPrice)).div(1e4).div(1e1).div(1e8); // 0.1g per token
        uint256 islamiFee = getIslamiPrice(usdtAmount.div(100)); // 1% fee
        ...
        returns (uint256 goldValue, uint256 usdtInVault)
    {
        ...
        goldValue = totalMintedGold.mul(uint256(goldPrice)).div(1e4).div(1e1).div(1e8); // Calculate the value of minted iGold tokens in USDT
