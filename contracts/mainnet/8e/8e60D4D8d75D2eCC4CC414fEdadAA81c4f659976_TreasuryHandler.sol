 ...
 * @dev Accumulate tokens from taxes, sell part of the tokens for other token
 */
contract TreasuryHandler is
    ITreasuryHandler,
    ...
    ExchangePoolProcessor
{
    ...
    /// @notice The treasury address. Can withdraw from contract
    address payable public treasury;

    /// @notice The farm address.
    address payable public farm;

    /// @notice The token that accumulates through taxes. This will be sold for feesSwapToToken.
    IERC20 public token;

    /// @notice Token fees to swap to
    IERC20 public feesSwapToToken;

    /// @notice The basis points of tokens to sell and add as liquidity to the pool.
    uint256 public marketingBasisPoints;
    ...
    event MarketingBasisPointsUpdated(
    ...
    /// @notice Emitted when the farm address is updated.
    event FarmAddressUpdated(
        address oldFarmAddress,
     ...
     * @param initialMarketingBasisPoints Initial basis points value of swap to add to liquidity.
    ...
    constructor(
        ...
        address feesSwapToTokenAddress,
        address routerAddress,
        uint256 initialMarketingBasisPoints,
        uint256 initialPriceImpactBasisPoints
    ) {
        treasury = payable(treasuryAddress);
        farm = payable(farmAddress);
        token = IERC20(tokenAddress);
        feesSwapToToken = IERC20(feesSwapToTokenAddress);
        router = IUniswapV2Router02(routerAddress);
        marketingBasisPoints = initialMarketingBasisPoints;
    ...
    function beforeTransferHandler(
        ...
        if (contractTokenBalance > 0) {
            uint256 tokensForMarketing = (contractTokenBalance *
                marketingBasisPoints) / 10000;
            uint256 tokensForCommunity = contractTokenBalance - tokensForMarketing;

            _swapTokens(tokensForMarketing);

            token.approve(address(farm), tokensForCommunity);
            token.transfer(
                address(farm),
                tokensForCommunity
            );
     ...
     * calculation.
     */
    function setMarketingBasisPoints(uint256 newBasisPoints)
        external
        onlyOwner
    {
        require(
        ...
        uint256 oldBasisPoints = marketingBasisPoints;
        marketingBasisPoints = newBasisPoints;

        emit MarketingBasisPointsUpdated(oldBasisPoints, newBasisPoints);
    }
    ...
    /**
     * @notice Set new farm address.
     * @param newFarmAddress New treasury address.
     */
    function setFarm(address newFarmAddress) external onlyOwner {
        require(
            newFarmAddress != address(0),
            "TreasuryHandlerAlpha:setTreasury:ZERO_TREASURY: Cannot set zero address as farm."
        );

        address oldFarmAddress = address(farm);
        farm = payable(newFarmAddress);

        emit FarmAddressUpdated(oldFarmAddress, newFarmAddress);
    }
    ...
    /**
     * @dev Swap accumulated tokens for given token.
     * @param tokenAmount Number of tokens to swap for given token.
     */
    function _swapTokens(uint256 tokenAmount) private {
        address[] memory path = new address[](2);
        path[0] = address(token);
        path[1] = address(feesSwapToToken);
