...
contract RmwNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, Pausable, ERC721Burnable {
    bytes4 internal constant MAGIC_ON_ERC721_RECEIVED = 0x150b7a02;
    bytes4 internal constant _INTERFACE_ID_ROYALTIES_EIP2981 = 0x2a55205a;
    using Strings for uint256;

    uint256 currentId = 1;
    uint256 mintingLimit = 10000000;
    uint256 maxMint = 200;
    uint256 price = 128 * 10**8;

    uint256 gas = 5000000;

    string baseURI = "ipfs://QmfX5fmJt8nxuhChSSFRPFG1mdyDcfTbUC6Hoi45Fydxxu/";
    ...
    constructor() ERC721("RMW Money NFT", "RMW NFT1") {
        _royaltyBps = 500;
   ...
   function mint(uint256 amount, address _to) external onlyOwner {
    ...
    function withdraw(uint256 percent, address to) external onlyOwner {
        payable(to).transfer((address(this).balance * percent) / 100);
    }
    ...
    function updateSearchGas(uint256 _gas) external onlyOwner {
        gas = _gas;
    }
    ...
    function getMyNFTs(address me, uint256 start, uint256 limit) external view returns (uint256[] memory myNFTIDs, string[] memory myNFTuris, uint256 lastId) {
        ...
        if (balance > 0) {
            ...
            for (uint256 i=0; gasUsed < gas && i < limit; i++) {
                lastId = i+start;
