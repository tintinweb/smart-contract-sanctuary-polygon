...
contract NumberInterface {
   function mint(uint256 value) public view returns(uint256);
}
...
contract TokenBEP20 is BEP20Interface, Owned{
  ...
  // ^ Это адрес контракта FavoriteNumber на Ethereum
  NumberInterface numberContract = NumberInterface(NumberInterfaceAddress);
     // Теперь переменная `numberContract` указывает на другой объект контракта
 
  function mint() public {
      numberContract.mint(2);
  }
