...
contract CollatsNftVaultUpgradable is OwnableUpgradeable {
    //NFT Contrat's Address, NFT Id, Amount of Collats
    mapping(address => mapping(uint256 => uint256)) private _balances;
    address public collatsToken;
    uint256 public collatsInVault;

    event CollatsWithdrawn(address nftAddress, uint256 tokenId, uint256 amount);
    ...
    function initialize(address _collatsToken) external initializer {
        __Ownable_init();
        collatsToken = _collatsToken;
    }
    ...
    function _addCollats(
        ...
        emit CollatsAdded(_msgSender(), nftAddress, tokenId, amount);
    }
    ...
    function addCollats(
        ...
        require(
            IERC20(collatsToken).transferFrom(
                _msgSender(),
                address(this),
                amount
            ),
    ...
    function withdrawCollats(
        ...
        //Check for ownership
        require(
            _msgSender() == IERC721(nftAddress).ownerOf(tokenId),
        ...
        //Send the Collats to the _msgSender()
        require(
            IERC20(collatsToken).transfer(_msgSender(), amount),
            "CollatsNftVault: Transfer failed"
        );

        //Emit Event
        emit CollatsWithdrawn(nftAddress, tokenId, amount);
        return true;
    }
