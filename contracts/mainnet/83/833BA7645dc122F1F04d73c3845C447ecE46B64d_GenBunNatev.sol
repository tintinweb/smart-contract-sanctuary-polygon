...
contract GenBunNatev is Ownable, ERC721Burnable, ERC721Pausable, ERC721Enumerable{
    ...
    constructor(string memory baseUri, string memory _placeholder, IERC20 _token) ERC721("GenBun: Natev", "GENBUN-N"){
        setBaseUri(baseUri);
    ...
    function setWhitelistSaleEnd(uint256 _whitelistSaleEnd) external onlyOwner{
    ...
    function mintTeam() external onlyOwner{
        require(!teamMinted, "GenBun: Team is already minted");
        teamMinted = true;
        for(uint256 index = 0; index < MAX_TEAM_MINT; index++){
            _safeMint(msg.sender, totalSupply() + 1);
        }
    }

    //function for minting an NFT
    function mint(uint256 payment) external{
        require(block.timestamp > whitelistSaleEnd, "GenBun: Public Sale hasn't started yet");
        require((totalSupply() + 1) <= MAX_SUPPLY, "GenBun: Quantity of minting exceeds public sale!"); // ensures that minting will not exceed max number of NFT.
        require(payment >= (PUBLIC_MINT_PRICE), "GenBun: Payment is below the Price!"); // ensures that the price given is not less than the set minting price.
    ...
    function whitelistMint(uint256 payment) external{
        require(block.timestamp <= whitelistSaleEnd, "GenBun: Whitelist sale has ended!");
        require(isWhitelisted[msg.sender], "GenBun: Address not whitelisted!");
        require((totalSupply() + 1) <= MAX_SUPPLY, "GenBun: Quantity of minting exceeds public sale!"); // ensures that minting will not exceed max number of NFT.
        require(payment >= (WHITELIST_MINT_PRICE), "GenBun: Payment is below the Price!"); // ensures that the price given is not less than the set minting price.
