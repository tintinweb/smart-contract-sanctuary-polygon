...
// File: contracts/LMUtility.sol


pragma solidity ^0.8.2;


contract LMUtility is ERC20, AccessControl, ERC721Holder {
    bytes32 public constant CONTRACT_ADMIN_ROLE = keccak256("CONTRACT_ADMIN_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    address public collectionAddressLionMastersNFT;
    address public collectionAddressNFTv2;

    uint256 public numberOfBlocksPerRewardUnit;

    uint256 public coinAmountPerRewardUnitLionMastersNFT;
    uint256 public coinAmountPerRewardUnitNFTv2;

    uint256 public welcomeBonusAmountLionMastersNFT;
    uint256 public welcomeBonusAmountNFTv2;

    uint256 public amountOfStakersLionMastersNFT;
    uint256 public amountOfStakersNFTv2;

    uint256 public tokensStakedLionMastersNFT;
    ...
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogLionMastersNFT;

        /// owner => tokenId => StakeInfo
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogNFTv2;

    /// owner => #NFTsStaked
    mapping (address => uint256) public tokensStakedByUserLionMastersNFT;
    ...
    mapping (uint256 => bool) public welcomeBonusCollectedLionMastersNFT;

        /// tokenId => true/false (true if welcome bonus has been collected for a specific tokenId)
    mapping (uint256 => bool) public welcomeBonusCollectedNFTv2;

    /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserLionMastersNFT;

        /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserNFTv2;

    /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioLionMastersNFT;

        /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioNFTv2;

    event RewardsHarvestedLionMastersNFT (address owner, uint256 amount);
    event NFTStakedLionMastersNFT (address owner, uint256 tokenId);
    event NFTUnstakedLionMastersNFT (address owner, uint256 tokenId);
    ...
    constructor(address owner, address _collectionAddressLionMastersNFT, address _collectionAddressNFTv2) ERC20("LionMastersUtility", "LMU") AccessControl(){
        _mint(owner, 100000 * 10 ** 18);
        _setupRole(DEFAULT_ADMIN_ROLE, owner);
        _setupRole(CONTRACT_ADMIN_ROLE, owner);
        _setupRole(BURNER_ROLE, owner);
        _setupRole(MINTER_ROLE, owner);
        collectionAddressLionMastersNFT = _collectionAddressLionMastersNFT;
        collectionAddressNFTv2 = _collectionAddressNFTv2;
        contractCreationBlock = block.number;
        coinAmountPerRewardUnitLionMastersNFT = 15 * 10 ** 18; // 5 ERC20 coins per rewardUnit, may be changed later on
        ...
        welcomeBonusAmountLionMastersNFT = 300 * 10 ** 18; // 300 tokens welcome bonus, only paid once per tokenId
        welcomeBonusAmountNFTv2 = 600 * 10 ** 18; // 600 tokens welcome bonus, only paid once per tokenId
    }

    function stakedNFTSByUserLionMastersNFT(address owner) external view returns (uint256[] memory){
        return stakePortfolioByUserLionMastersNFT[owner];
    }
    ...
    function pendingRewardsLionMastersNFT(address owner, uint256 tokenId) public view returns (uint256){
        StakeInfo memory info = stakeLogLionMastersNFT[owner][tokenId];
        ...
        return rewardAmount * coinAmountPerRewardUnitLionMastersNFT;
    ...
    function stakeLionMastersNFT(uint256 tokenId) public {
        IERC721(collectionAddressLionMastersNFT).safeTransferFrom(_msgSender(), address(this), tokenId);
        require(IERC721(collectionAddressLionMastersNFT).ownerOf(tokenId) == address(this),
            "LMUtility: Error while transferring token");
        StakeInfo storage info = stakeLogLionMastersNFT[_msgSender()][tokenId];
        info.stakedAtBlock = block.number;
        info.lastHarvestBlock = block.number;
        info.currentlyStaked = true;
        if(tokensStakedByUserLionMastersNFT[_msgSender()] == 0){
            amountOfStakersLionMastersNFT += 1;
        }
        tokensStakedByUserLionMastersNFT[_msgSender()] += 1;
        tokensStakedLionMastersNFT += 1;
        stakePortfolioByUserLionMastersNFT[_msgSender()].push(tokenId);
        uint256 indexOfNewElement = stakePortfolioByUserLionMastersNFT[_msgSender()].length - 1;
        indexOfTokenIdInStakePortfolioLionMastersNFT[tokenId] = indexOfNewElement;
        if(!welcomeBonusCollectedLionMastersNFT[tokenId]) {
            _mint(_msgSender(), welcomeBonusAmountLionMastersNFT);
            welcomeBonusCollectedLionMastersNFT[tokenId] = true;
        }

        emit NFTStakedLionMastersNFT(_msgSender(), tokenId);
    }
    ...
    function stakeBatchLionMastersNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            stakeLionMastersNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakeBatchLionMastersNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            unstakeLionMastersNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakeLionMastersNFT(uint256 tokenId) public {
        if(pendingRewardsLionMastersNFT(_msgSender(), tokenId) > 0){
            harvestLionMastersNFT(tokenId);
        }
        StakeInfo storage info = stakeLogLionMastersNFT[_msgSender()][tokenId];
        info.currentlyStaked = true;
        IERC721(collectionAddressLionMastersNFT).safeTransferFrom(address(this), _msgSender(), tokenId);
        require(IERC721(collectionAddressLionMastersNFT).ownerOf(tokenId) == _msgSender(),
            "LMUtility: Error while transferring token");
        if(tokensStakedByUserLionMastersNFT[_msgSender()] == 1){
            amountOfStakersLionMastersNFT -= 1;
        }
        tokensStakedByUserLionMastersNFT[_msgSender()] -= 1;
        tokensStakedLionMastersNFT -= 1;
        stakePortfolioByUserLionMastersNFT[_msgSender()][indexOfTokenIdInStakePortfolioLionMastersNFT[tokenId]] = 0;
        emit NFTUnstakedLionMastersNFT(_msgSender(), tokenId);
    }
    ...
    function harvestLionMastersNFT(uint256 tokenId) public {
        StakeInfo storage info = stakeLogLionMastersNFT[_msgSender()][tokenId];
        ...
        if(rewardAmountInERC20Tokens > 0) {
            ...
            emit RewardsHarvestedLionMastersNFT(_msgSender(), rewardAmountInERC20Tokens);
        }
    }
    ...
    function harvestBatchLionMastersNFT(address user) external {
        uint256[] memory tokenIds = stakePortfolioByUserLionMastersNFT[user];

        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            harvestLionMastersNFT(tokenIds[currentId]);
        }
    }
    ...
    function setCoinAmountPerRewardUnitLionMastersNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        coinAmountPerRewardUnitLionMastersNFT = coinAmount;
    }
    ...
    function setWelcomeBonusAmountLionMastersNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        welcomeBonusAmountLionMastersNFT = coinAmount;
    }
    ...
    function setCollectionAddressLionMastersNFT(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "VIBE: update to zero address not possible");
        collectionAddressLionMastersNFT = newAddress;
    }
