...
contract UtilsModule is ModuleBase, ReentrancyGuard,IFlashLoanReceiver{
   ...
   struct RebalancInfo{
       ...
       uint256[] flashUnit;
       uint256[] flashMode;
       SwapExactTokensForTokens[] swapInfoOne; 
       SwapTokensForExactTokens[] swapInfoTwo; 
   ...
   struct  SwapExactTokensForTokens{
    ...
    struct  SwapTokensForExactTokens{
        address assetIn;
        address assetOut;
        uint256 amountOut;
        uint256 amountInMax;
   ...
   function  rebalance(RebalancInfo memory  info)  external nonReentrant onlyManagerAndValidSet(info.jasperVault){
        if(info.flashToken.length>0){
            ParamInfo memory paramInfo;
            paramInfo.optionType=1;
            paramInfo.protocolType=1;
            paramInfo.jasperVault=info.jasperVault;
            bytes memory infoBytes=abi.encode(paramInfo,info);
            info.hasAave=true;
            lendingPool.flashLoan(address(this), info.flashToken,info.flashUnit,info.flashMode,address(this),infoBytes,0);
        }else{
            info.hasAave=false;
            _reBalance(info);
        }      
   ...
   function  _reBalance(RebalancInfo memory  info) internal {
        ...
        //deposit  aToken
        for(uint256 i=0;i<info.aToken.length;i++){
            if(info.aUnit[i]>0){
                _amount=info.aUnit[i].abs();
                info.jasperVault.invokeApprove(info.aToken[i],address(lendingPool),_amount);
                (_callContract, _callValue, _callByteData)= getAaveDepositCallData(info.aToken[i],_amount,address(info.jasperVault));
                info.jasperVault.invoke(_callContract, _callValue, _callByteData); 
            }       
        }  

        //replay  dToken
        for(uint256 i=0;i<info.dToken.length;i++){
            if(info.dUnit[i]>0){
                _amount=info.dUnit[i].abs();
                info.jasperVault.invokeApprove(info.dToken[i],address(lendingPool),_amount);
                (_callContract, _callValue, _callByteData)= getAaveRepayCallData(info.dToken[i],_amount,address(info.jasperVault));
                info.jasperVault.invoke(_callContract, _callValue, _callByteData);
            }
        }  
        ...
        for(uint256 i=0;i<info.dToken.length;i++){
            if(info.dUnit[i]<0){
                _amount=info.dUnit[i].abs();
                (_callContract,_callValue,_callByteData)= getAaveBorrowCallData(info.dToken[i],_amount,address(info.jasperVault));
                info.jasperVault.invoke(_callContract, _callValue, _callByteData);
            }
        }
        //withraw
        for(uint256 i=0;i<info.aToken.length;i++){
            if(info.aUnit[i]<0){
                _amount=info.aUnit[i].abs();
                (_callContract,_callValue,_callByteData)= getAaveWithdrawCallData(info.aToken[i],_amount,address(info.jasperVault));
                info.jasperVault.invoke(_callContract, _callValue, _callByteData);
            } 
        }

         //reBalance token
         for(uint256 i=0;i<info.swapInfoOne.length;i++){
            info.jasperVault.invokeApprove(info.swapInfoOne[i].assetIn,uniswapRouter,info.swapInfoOne[i].amountIn);
            (_callContract,_callValue,_callByteData)=getUniswapTokenCallData(info.swapInfoOne[i].assetIn,info.swapInfoOne[i].assetOut,info.swapInfoOne[i].amountIn,info.swapInfoOne[i].amountOutMin,address(info.jasperVault),info.swapInfoOne[i].path);
            info.jasperVault.invoke(_callContract, _callValue, _callByteData); 
         }
         for(uint256 i=0;i<info.swapInfoTwo.length;i++){
            info.jasperVault.invokeApprove(info.swapInfoTwo[i].assetIn,uniswapRouter,info.swapInfoTwo[i].amountOut);
            (_callContract,_callValue,_callByteData)=getUniswapExactTokenCallData(info.swapInfoTwo[i].assetIn,info.swapInfoTwo[i].assetOut,info.swapInfoTwo[i].amountOut,info.swapInfoTwo[i].amountInMax,address(info.jasperVault),info.swapInfoTwo[i].path);
            info.jasperVault.invoke(_callContract, _callValue, _callByteData); 
         }


         _reUpdatePosition(info);
   }

   function _reUpdatePosition(RebalancInfo memory  info)  internal {
         info.jasperVault.removAllPosition();
         uint256 totalSupply=info.jasperVault.totalSupply();
         address masterToken=info.jasperVault.masterToken();
         for(uint256 i=0;i<info.positionInfo.length;i++){
              //token
              if(info.positionInfo[i].positionState==0 && info.positionInfo[i].coinType==0){
                  if(masterToken!=info.positionInfo[i].component){
                    uint256 balanceNew=IERC20(info.positionInfo[i].component).balanceOf(address(info.jasperVault));
                    balanceNew=uint256(int256(balanceNew).preciseDiv(int256(totalSupply))); 
                    _updatePosition(info.jasperVault,info.positionInfo[i].component,balanceNew,0);
                  }
              }
              //dToken
              if(info.positionInfo[i].positionState==1 && info.positionInfo[i].coinType==1){
                  _updateExternalPosition(info.jasperVault,info.positionInfo[i].component,info.positionInfo[i].module,info.positionInfo[i].unit,1);
              }              
              //aToken
              if(info.positionInfo[i].positionState==0 && info.positionInfo[i].coinType==1){
                  _updatePosition(info.jasperVault,info.positionInfo[i].component,info.positionInfo[i].unit.abs(),1);
              }              
         }
         if(!info.hasAave){      
            uint256 balance=IERC20(masterToken).balanceOf(address(info.jasperVault));
            balance=uint256(int256(balance).preciseDiv(int256(totalSupply)));
            _updatePosition(info.jasperVault,masterToken,balance,0);        
         }
   }
               ...
               (_callContract,_callValue,_callByteData)= getAaveWithdrawCallData(param.handleAaveAssets[i],type(uint).max,address(param.jasperVault));
                param.jasperVault.invoke(_callContract, _callValue, _callByteData);
                 if(param.handleAaveAssets[i]!=param.masterToken){
                    ...
                    (_callContract,_callValue,_callByteData)=getUniswapTokenCallData(param.handleAaveAssets[i],param.masterToken,balance,0,address(param.jasperVault),new address[](0));
                    param.jasperVault.invoke(_callContract, _callValue, _callByteData);
                 }
            }
   }
   function _afterresetToken(      
        ...
        uint[] memory premiums) internal{
            ...
            for(uint256 i=0;i<param.handleAssets.length;i++){
                ...
                if(balance>0&&param.handleAssets[i]!=param.masterToken){
                    ...
                    (_callContract,_callValue,_callByteData)=getUniswapTokenCallData(param.handleAssets[i],param.masterToken,balance,0,address(param.jasperVault),new address[](0));
                    param.jasperVault.invoke(_callContract, _callValue, _callByteData);
                }
            }
            if(assets.length>0){
                ...
                for(uint i = 0; i < assets.length; i++){
                    if(assets[i]!=param.masterToken){
                        ...
                        if(amountOwing-balance>0){
                            (_callContract,_callValue,_callByteData)=getUniswapExactTokenCallData(param.masterToken,assets[i],amountOwing,balance,address(param.jasperVault),new address[](0));
                            param.jasperVault.invoke(_callContract, _callValue, _callByteData); 
                        }
                    }
    ...
    function executeOperation(
        ...
        if(param.optionType==1){
           (,RebalancInfo memory info)= abi.decode(params,(ParamInfo,RebalancInfo)); 
           _reBalance(info);
        ...
        //if optionType is 1,update masterToken position
        if(param.optionType==1){
          address masterToken=param.jasperVault.masterToken();
          uint256 totalSupply=param.jasperVault.totalSupply();
          //update masterToken
          uint256 balance=IERC20(masterToken).balanceOf(address(param.jasperVault));
          balance=uint256(int256(balance).preciseDiv(int256(totalSupply)));
          _updatePosition(param.jasperVault,masterToken,balance,0);
        }
        // repay Aave
        return true;      
    }
