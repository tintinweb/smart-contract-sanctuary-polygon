...
contract WarpTreasury is WarpBase {
    ...
    function deposit(
        ...
        send_ = value - _profit;
        if (send_ != 0) IERC20Mintable(WARP).mint(msg.sender, send_);
    ...
    function valueOf(address _token, uint256 _amount) public view returns (uint256 value_) {
        ...
        if (reserveTokens.contains(_token)) {
            ...
            value_ = (_amount * IBondCalculator(bondCalculator[WARP]).getUniswapWarpPrice()) / 1e2;
        }
    }
    ...
    function bondingCalculatorFor(address _calculator, address _liquidityToken) external onlyOwner {
        require(
            liquidityTokens.contains(_liquidityToken) || _liquidityToken == WARP,
            'Not a liquidity token'
        );
