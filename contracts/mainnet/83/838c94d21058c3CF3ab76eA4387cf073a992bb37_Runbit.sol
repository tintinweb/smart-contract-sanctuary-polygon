...
contract Runbit is AccessControl {
    ...
    struct EquipInfo {
        address owner; //所有者
        uint64 emptyId; //暂时未用
        uint32 latestDay; // 最新更新日期
    }

    struct CardInfo {
        uint192 equipId; //装备于哪个装备
        uint64 idx; // 装备于装备的哪个槽
    }

    struct UserState {
        uint8 status; // 0 未收获，1 已收获
        uint8 lottery; // 0 未抽奖，1 已抽奖
        uint16 trackId; // 选用的哪个跑道
        uint32 cardCount; // 当天共使用了多少属性卡
        uint64 lastSteps; // 最新的步数
        uint128 RBReward; // 该天的收益
    }

    struct DailyInfo {
        uint64 track0; // 跑道0 拥挤指数
        uint64 track1; // 跑道1 拥挤指数
        uint64 track2; // 跑道2 拥挤指数
        uint64 userCount; // 当日用户数
        uint64 totalSpecialty; // 当日功能值总和
        uint64 totalComfort; // 当日舒适度总和
        uint64 totalAesthetic; // 当日美观度总和
    }
    ...
    function bindCard(uint256 equipId, uint256 cardId, uint256 index) external onlyReferral {
        ...
        if(info.equipId > 0) {
            equipCards[info.equipId][info.idx] = cardId;
        }
    ...
    // 解绑指定属性卡的指定卡槽
    // 如果要解绑指定的属性卡，则先通过cardId查询出其所在的装备id及卡槽
    ...
    function claimBonus(uint256 day, address to) external onlyRole(MANAGE_ROLE) {
        ...
        require(dailyBonus[day] == 0, "claimed!");
        uint256 amount = _calcBonus(day);
    ...
    function _calcBonus(uint256 day) private view returns (uint256) {
        ...
        return totalReward * bonusRate / 100000000;
    }
    ...
    function setRate(uint256 _ref, uint256 _bonus) external onlyRole(DEFAULT_ADMIN_ROLE) {
        refRate = _ref;
        bonusRate = _bonus;
    }
    ...
    // 获取每天的社区奖励
    // function getPendingBonus(uint256 day) external view returns (uint256) {
    //     return _calcBonus(day);
    // }
