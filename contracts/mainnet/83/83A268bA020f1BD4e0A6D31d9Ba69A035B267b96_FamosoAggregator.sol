...
contract FamosoAggregator {
  ...
  IWMATIC constant internal wmatic = IWMATIC(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270);
  IERC20 private constant ZERO_ADDRESS = IERC20(0x0000000000000000000000000000000000000000);
  IERC20 private constant MATIC_ADDRESS = IERC20(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE);

  IUniswapV2Factory constant internal quickswap = IUniswapV2Factory(0x5757371414417b8C6CAad45bAeF941aBc7d3Ab32);
  ...
  {
    return [calculateQuickswap, calculateSushiswap];
  }
  ...
  function _calculateSushiswap(
    ...
    IERC20 fromTokenReal = isMATIC(fromToken) ? wmatic: fromToken;
    IERC20 destTokenReal = isMATIC(destToken) ? wmatic: destToken;
  ...
  function _calculateQuickswap(
    ...
    IERC20 fromTokenReal = isMATIC(fromToken) ? wmatic: fromToken;
    IERC20 destTokenReal = isMATIC(destToken) ? wmatic: destToken;
    IUniswapV2Exchange exchange = IUniswapV2Exchange(quickswap.getPair(address(fromTokenReal), address(destTokenReal)));
    if (exchange != IUniswapV2Exchange(address(0x0))) {
      uint256 fromTokenBalance = fromTokenReal.balanceOf(address(exchange));
      uint256 destTokenBalance = destTokenReal.balanceOf(address(exchange));
      for (uint i = 0; i < amounts.length; i++) {
  ...
  function swap(
    ...
    uint256 remainingAmount = isMATIC(fromToken) ? address(this).balance : fromToken.balanceOf(address(this));
    ...
    returnAmount = isMATIC(destToken) ? address(this).balance : destToken.balanceOf(address(this));
    require(returnAmount >= minReturn, "FamosoAggregator: Return amount was not engoth");
    if (isMATIC(destToken)) {
  ...
  function swapMulti(
    ...
    returnAmount = isMATIC(tokens[0]) ? address(this).balance : tokens[0].balanceOf(address(this));
    for (uint i = 1; i < tokens.length; i++) {
      if (tokens[i - 1] == tokens[i]) {
        continue;
      }
      ...
      for (uint k = 0; k < dist.length; k++) {
        ...
        uint256 swapAmount = amount.mul(distribution[k]).div(parts);
      ...
      returnAmount = isMATIC(tokens[i]) ? address(this).balance : tokens[i].balanceOf(address(this));
      // fund remainingAmount
      if (isMATIC(tokens[i - 1])) {
    ...
    if (isMATIC(tokens[tokens.length - 1])) {
      payable(msg.sender).transfer(returnAmount);
    } else {
  ...
  function _swapOnQuickswap(
    ...
    _swapOnQuickswapInternal(fromToken, destToken, amount);
  }
  ...
  function _swapOnQuickswapInternal (
    ...
    IERC20 fromTokenReal = isMATIC(fromToken) ? wmatic : fromToken;
    IERC20 toTokenReal = isMATIC(destToken) ? wmatic: destToken;
    IUniswapV2Exchange exchange = IUniswapV2Exchange(quickswap.getPair(address(fromTokenReal), address(toTokenReal)));
    bool needSync;
    bool needSkim;
