...
// Had to put this in afer customising our own ownership script, because opensea requires it to allow profile editing. (Fuck knows why)
// Oh I know why because that is how they scripted it and are too busy partying on an island to fix it

pragma solidity ^0.8.0;



abstract contract Ownable is Context, metaCasperzAdmin {
...
contract metaCasperzMain is metaCasperz, metaCasperzAdmin, metaCasperzSecurity, metaCasperzVault, metaCasperzSpooks, Ownable {
    ...
    constructor () metaCasperz("https://metacasperz.com/casperzmatic/{id}.json"){    
    }
...
function returnCasperzCurrentPrice(uint256 _casperzMintedTotal) pure returns (uint256 _casperzCurrentPrice){
    _casperzCurrentPrice=305000000000000000000; // maximum by default
    if (_casperzMintedTotal < 11 ){
        _casperzCurrentPrice = 16000000000000000000; // 0.01 ETH 16 MATIC  
    }else if (_casperzMintedTotal > 10 && _casperzMintedTotal < 201){
        _casperzCurrentPrice = 25000000000000000000; // 0.02 ETH 25 MATIC       
    }else if (_casperzMintedTotal > 200 && _casperzMintedTotal < 501){
        _casperzCurrentPrice = 52000000000000000000; // 0.03 ETH 52 MATIC
    }else if (_casperzMintedTotal > 500 && _casperzMintedTotal < 1001){
        _casperzCurrentPrice = 143000000000000000000; // 0.08 ETH 143 MATIC
    }else if (_casperzMintedTotal > 1000 && _casperzMintedTotal < 2001){
        _casperzCurrentPrice = 200000000000000000000; // 0.11 ETH 200 MATIC
    }else if (_casperzMintedTotal > 2000 && _casperzMintedTotal < 3001){
        _casperzCurrentPrice = 250000000000000000000; // 0.14 ETH 250 MATIC
    }else{
        if (_casperzMintedTotal > 3000){
            _casperzCurrentPrice = 305000000000000000000; // 0.17 ETH 305 MATIC
        }
    }
