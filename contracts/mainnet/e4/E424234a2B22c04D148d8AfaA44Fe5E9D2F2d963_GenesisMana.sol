...
import "./interface/IDavaV2.sol";
import "./interface/IAvatar.sol";
import "./interface/IPartCollection.sol";
...
contract GenesisMana is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    IDavaV2 private _davaContract =
        IDavaV2(0xf81CB9BFea10d94801F3e445D3D818e72E8d1dA4);
    // 0xf81CB9BFea10d94801F3e445D3D818e72E8d1dA4 // Mainnet
    ...
    {
        require(
            !(detailsByToken[tokenId].lootTokenId == 0 &&
                detailsByToken[tokenId].suffixId == 0 &&
                detailsByToken[tokenId].inventoryId == 0 &&
                _compareStrings(detailsByToken[tokenId].itemName, "")),
            "TOKEN_ID_NOT_MINTED"
        );

        string[11] memory parts;
        string memory name = string(
            abi.encodePacked("Genesis Mana #", Strings.toString(tokenId))
        );

        parts[
            0
        ] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; widht: 350px} .italic {font-style: italic}</style><rect width="100%" height="100%" fill="';
        ...
        parts[
            6
        ] = '</text><text x="10" y="320" class="base italic">Distilled from ';
        if (detailsByToken[tokenId].lootTokenId == 0) {
            ...
            parts[7] = string(
                abi.encodePacked(
                    "Dava Loot Bag #",
                    Strings.toString(detailsByToken[tokenId].lootTokenId)
                )
            );
        }
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        name,
                        '", "description": "This item is Genesis Mana used in Loot (for Adventurers)", "image": "data:image/svg+xml;base64,',
        ...
        nonReentrant
    {
        ...
        require(
            _msgSender() == _davaContract.ownerOf(lootTokenId),
            "MUST_OWN_TOKEN_ID"
        );
    ...
    function _claim(
        ...
        // Check that the loot token ID is in range
        require(
            lootTokenId >= 0 && lootTokenId <= 9999,
            "LOOT_TOKEN_ID_OUT_OF_RANGE"
        );

        // checks
        string memory itemName;
        IAvatar avatar = IAvatar(_davaContract.getAvatar(lootTokenId));
        Part[] memory parts = avatar.allParts();

        itemName = IPartCollection(parts[inventoryId].collection).partTitle(
            parts[inventoryId].id
        );
        ...
        suffixId = _findSuffixId(
            itemSlice.split(separatorStr.toSlice()).toString()
        );
...
interface IDavaV2 is IERC721Metadata {
    ...
    function getAvatar(uint256 tokenId) external returns (address);
