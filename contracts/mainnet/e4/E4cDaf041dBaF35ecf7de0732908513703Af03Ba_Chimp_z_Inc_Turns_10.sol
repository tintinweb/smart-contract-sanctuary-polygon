...
contract Chimp_z_Inc_Turns_10 is ERC721A, DefaultOperatorFilterer, Ownable, ReentrancyGuard {
    uint256 public MAX_SUPPLY = 12;

    
    uint256 public CHIMPTEAM_SUPPLY = 11;
    
    bool public IS_CHIMPTEAM_ACTIVE = false;

    
    uint256 public CHIMPTEAM_TX_LIMIT = 1;

    
    uint256 public CHIMPTEAM_WALLET_LIMIT = 1;

    
    uint256 public CHIMPTEAM_PRICE = 0 ether;

    
    uint256 public CHIMPTEAM_COUNT = 0;

    

    
    mapping(address => uint) public CHIMPTEAM_Minted;

    bool public _revealed = false;

    string private baseURI = "";
    string private preRevealURI = "https://ipfs.w3bmint.xyz/ipfs/QmNpc2HuhEcE5q27vEPQUdoqD3yKQGeND2ciG8xBfs7Rdv";
    ...
    constructor() ERC721A("Chimp_z_Inc_Turns_10", "CANDZ") {
        
        
        _safeMint(0x8764e167D2626553238D86A99161D37BDaFB6718, 1);
    }
    ...
    function CHIMPTEAMMint(address to, uint256 numberOfTokens) external isSecured(CHIMPTEAM_PRICE * numberOfTokens) payable {
        validateMint(IS_CHIMPTEAM_ACTIVE, CHIMPTEAM_COUNT, CHIMPTEAM_SUPPLY, CHIMPTEAM_Minted[msg.sender], CHIMPTEAM_WALLET_LIMIT, CHIMPTEAM_TX_LIMIT, numberOfTokens, true);
        addressBlockBought[to] = block.timestamp;
        CHIMPTEAM_Minted[to] += numberOfTokens;
        CHIMPTEAM_COUNT += numberOfTokens;
    ...
    function toggleCHIMPTEAMMintStatus() external onlyOwner {
        IS_CHIMPTEAM_ACTIVE = !IS_CHIMPTEAM_ACTIVE;
    }

    // SUPPLY CHANGERS
    
    function setCHIMPTEAMSupply(uint256 _supply) external onlyOwner {
        CHIMPTEAM_SUPPLY = _supply;
    }


    // PRICE CHANGERS
    
    function setCHIMPTEAMPrice(uint256 _price) external onlyOwner {
        CHIMPTEAM_PRICE = _price;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        (bool successFinal, ) = payable(0x8764e167D2626553238D86A99161D37BDaFB6718).call{ value: address(this).balance }("");
