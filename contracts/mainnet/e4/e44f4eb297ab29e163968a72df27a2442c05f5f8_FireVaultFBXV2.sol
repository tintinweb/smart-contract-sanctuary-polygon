...
contract FireVaultFBXV2 is ERC20, ERC20Permit {
    ...
    // TODO test to delete
    function claimRewardsForVault() public {
        EP.claimRewards();
    }

    function stakeVaultEP() public {
        uint256 unstakedEP = EP.balanceOf(address(this));
        if (unstakedEP > 0) {
            EP.approve(address(EP), unstakedEP);
            EP.stakeEP(unstakedEP);
        }
    }

    function stakeVaultEP2() public {
        uint256 unstakedEP = EP.balanceOf(address(this));
        if (unstakedEP > 0) {
            EP.approve(address(this), unstakedEP);
            EP.stakeEP(unstakedEP);
        }
    }

    function getCurrentValuation() public view returns (uint256) {
        return 1e18 * totalAssetsValuation() / totalSupply();
    }

    function refreshWithdrawalRate() public {
        uint256 valuation = getCurrentValuation();
        if (valuation > withdrawalRate) {
            withdrawalRate = valuation;
        }
    }
    // end test
    ...
    function deposit(uint256 amountFBX) external {
        ...
        _mint(msg.sender, uint256(1e18 * amountFBX / withdrawalRate));
        emit Deposit(msg.sender, amountFBX);
    }

    function withdraw(uint256 amountFireFBX) external {
        require(amountFireFBX > 0, "Amount to withdraw should be greater than 0.");
        claimRewardsAndBalance();
        _burn(msg.sender, amountFireFBX);
        uint256 amountFBX = amountFireFBX * withdrawalRate / 1e18;
