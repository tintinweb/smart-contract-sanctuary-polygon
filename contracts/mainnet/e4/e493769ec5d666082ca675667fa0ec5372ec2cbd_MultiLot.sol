...
contract MultiLot is Initializable, OwnableUpgradeable {
    ...
    function createLot(
        ...
        for (uint256 i = 0; i < _tokenBChoices.length; i++) {
            require(
                keccak256(bytes(_tokenA)) != keccak256(bytes(_tokenBChoices[i])),
                "tokens can not be identitcal"
            );
    ...
    function joinLot(
        ...
        if (compareStrings(_token, lot.tokenA)) {
            ...
            require(!lot.isChallenge || lot.totalDepositPoolA == 0, "multiple users not allowed in challenge");
            lot.userDepositPoolA[msg.sender] += _size;
            lot.totalDepositPoolA += _size;
        } else {
            if (bytes(lot.tokenB).length == 0) {
                require(lot.tokenBChoices[_token], "invalid token id for lot");
                lot.tokenB = _token;
            }
            else {
                require(compareStrings(_token, lot.tokenB));
            }
            require(
                lot.userDepositPoolA[msg.sender] == 0,
                "cannot join on both sides"
            );
            require(!lot.isChallenge || lot.totalDepositPoolB == 0, "multiple users not allowed in challenge");
            require(!lot.isChallenge || lot.totalDepositPoolA == _size, "lot size must be same in challenge");
