...
contract BettingGame {
    ...
    function getGameFromStage(uint _stage) public view returns (string memory) {
        ...
        for (uint i = 0; i < nextGameId; i++) {
            uint256 endTime = games[i].endTime;
            bool isClaimed = games[i].isClaimed;

            if (_stage == 1 && block.timestamp < endTime) {
                (gameIds, gameFound) = _concatString(gameIds, Strings.toString(i), gameFound);
            }

            if (_stage == 2 && block.timestamp >= endTime && !isClaimed) {
                (gameIds, gameFound) = _concatString(gameIds, Strings.toString(i), gameFound);
            } 
            
            if (_stage == 3 && block.timestamp >= endTime && isClaimed) {
                (gameIds, gameFound) = _concatString(gameIds, Strings.toString(i), gameFound);
            }
        }
        ...
        for (uint i = 0; i < game.playerAddresses.length; i++) {
            ...
            for(uint j = 0; j < getMaxHandAmounts(_gameId, playerAddress) + 1; j++) {
                if (currentPlayer.hands[j].product == _highestProduct) {
                    ...
                    if(_winner == playerAddress) isWinner = true;
    ...
    function claimAllGame() external onlyOwner {
        for(uint i=0; i<nextGameId; i++) {
            ...
            if(block.timestamp >= endTime && !games[i].isClaimed) {
                claimReward(i);
            }
        }
    }
