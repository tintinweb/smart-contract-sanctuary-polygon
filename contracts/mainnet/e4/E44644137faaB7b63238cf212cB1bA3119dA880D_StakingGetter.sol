...
contract StakingGetter {
  ...
  function getTokensOfType(address account, address addr, bool includeMetadata, uint256 landId, uint256 farmId) public view returns (Asset[] memory) {
    IERC165 c165 = IERC165(addr);
    bool is721 = c165.supportsInterface(type(IERC721).interfaceId);

    FarmStaker.Staking[] memory stakings = staker.allStakingsOfTypeOnFarm(addr, account, landId, farmId);
    Asset[] memory assets = new Asset[](stakings.length);
    MetadataGetter m = MetadataGetter(addr);

    for (uint256 i = 0; i < assets.length; i++) {
      FarmStaker.Staking memory staking = stakings[i];
      bytes32 key = staker.getKey(addr, staking.owner, staking.landId, staking.farmId, staking.tokenId);
      string memory metadata = includeMetadata ? (is721 ? m.tokenURI(staking.tokenId) : m.uri(staking.tokenId)) : "";
      assets[i] = Asset(staking, key, metadata);
    }
