 ...
 */
contract NomisScore is ERC721, ERC721Burnable, Ownable, Pausable, EIP712 {
    ...
    * @param _fee The fee to be checked.
    * @param _wallet The wallet address should be whitelisted for free mint.
    * @param _calcModel The scoring calculation model.
    * Requirements:
    * The fee passed must be equal to the current mint fee set.
    */
    modifier equalsFee(uint _fee, address _wallet, uint16 _calcModel) {
        require(whitelist[_wallet][_calcModel] || _fee == _mintFee || _tokenIds.current() < CalculationModelsFreeMintCount[_calcModel], "Mint fee: wrong mint fee value");
        _;
    }

    /**
     * @dev A mapping of addresses and calculation methods to scores.
     */
    mapping (address => mapping (uint16 => Score)) scores;

    /**
     * @dev A mapping of addresses with scoring calculation model to whitelist.
     */
    mapping(address => mapping (uint16 => bool)) whitelist;
    ...
    uint private _mintFee = 0.0001 ether;

    /**
     * @dev A mapping of calculation model to free mint count.
     */
    mapping (uint16 => uint16) CalculationModelsFreeMintCount;

    /**
     * @dev Emitted when an existed score is changed.
     ...
     */
    event ScoreChanged(uint256 indexed tokenId, address indexed owner, uint16 score, uint16 calculationModel);
    ...
    */
    event Withdrawal(address indexed owner, uint indexed balance);

    /**
    * @dev Constructor for the NomisScore ERC721 contract.
    ...
    * @param initialfreeMintCount The initial free mint count.
    * Initializes the token ID counter to zero and sets the initial minting fee.
    */
    constructor(uint256 initialFee, uint16 initialCalcModelsCount, uint16 initialfreeMintCount) ERC721("NomisScore", "NMSS") EIP712("NMSS", "0.4") {
        _tokenIds.reset();
        _mintFee = initialFee;
        require(initialCalcModelsCount > 0, "constructor: initialCalcModelsCount should be greater than 0");
        _calcModelsCount = initialCalcModelsCount;
        if (initialfreeMintCount > 0) {
            for (uint16 i = 0; i < initialCalcModelsCount; i++) {
                CalculationModelsFreeMintCount[i] = initialfreeMintCount;
            }
        }
    }
    ...
    function setScore(
        ...
        uint16 calculationModel,
        uint256 deadline
    ) external payable whenNotPaused equalsFee(msg.value, msg.sender, calculationModel) {
        ...
        require(calculationModel < _calcModelsCount, "setScore: calculationModel should be less than _calcModelsCount");

        // Verify the signer of the message
        bytes32 messageHash = _hashTypedDataV4(keccak256(abi.encode(
            keccak256("SetScoreMessage(uint16 score,uint16 calculationModel,address to,uint256 nonce,uint256 deadline)"),
            score,
            calculationModel,
            msg.sender,
            nonces[msg.sender],
        ...
        require(signer == owner() && signer != address(0), "setScore: Invalid signature");
        nonces[msg.sender]++;

        bool isNewScore = false;
        Score storage scoreStruct = scores[msg.sender][calculationModel];
        ...
        if (scoreStruct.value != score || scoreStruct.calculationModel != calculationModel) {
            scoreStruct.value = score;
            scoreStruct.calculationModel = calculationModel;
        }

        if (isNewScore) {
            _safeMint(msg.sender, scoreStruct.tokenId);
            _tokenIds.increment();
        }

        emit ScoreChanged(scoreStruct.tokenId, msg.sender, score, calculationModel);
    }
    ...
    */
    function getScore(address addr, uint16 calcModel) external view returns(uint16 score, uint256 updated, uint256 tokenId, uint16 calculationModel) {
        require(scores[addr][calculationModel].updated != 0, "getScore: No score found for the specified address and calculation model");

        Score storage _score = scores[addr][calcModel];

        score = _score.value;
        updated = _score.updated;
        tokenId = _score.tokenId;
        calculationModel = _score.calculationModel;
    }
    ...
    * @param calcModel The scoring calculation model.
    */
    function WhitelistAddresses(address[] calldata actors, uint16 calcModel) public onlyOwner {
    ...
    * @param calcModel The scoring calculation model.
    */
    function UnWhitelistAddresses(address[] calldata actors, uint16 calcModel) public onlyOwner {
        for (uint256 i = 0; i < actors.length; i++) {
    ...
    */
    function setMintFee(uint mintFee) external onlyOwner {
        _mintFee = mintFee;
    }
    ...
    */
    function getFreeMints(uint16 calcModel) external view returns (uint16) {
        return CalculationModelsFreeMintCount[calcModel];
    }
    ...
    * @notice Only the contract owner can call this function.
    */
    function setFeeMints(uint16 freeMintCount, uint16 calcModel) external onlyOwner {
        CalculationModelsFreeMintCount[calcModel] = freeMintCount;
    }
    ...
    */
    function setCalcModelsCount(uint16 calcModelsCount) external onlyOwner {
        require(calcModelsCount > 0, "setCalcModelsCount: calcModelsCount should be greater than 0");
    ...
    */
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override (ERC721) {
        require(from == address(0) || to == address(0), "NonTransferrableERC721Token: Nomis score can't be transferred.");
    ...
    */
    function _afterTokenTransfer(address from, address to, uint256 tokenId) internal override (ERC721) {
        if (to == address(0))
        {
            for (uint16 i = 0; i < _calcModelsCount; i++) {
                if (scores[from][i].tokenId == tokenId) {
                    delete scores[from][i];
                }
            }
        }
    ...
    /**
    * @dev Burns the token with the given ID and deletes any corresponding scores associated with it.
    * @param tokenId The ID of the token being burned.
    */
    function _burn(uint256 tokenId) internal pure override (ERC721) {
        require(false, "NonTransferrableERC721Token: Nomis score can't be burned.");
    }
}