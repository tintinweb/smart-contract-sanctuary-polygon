...
// AN1 Contracts (last updated v1.0.0) (tokens/phygital/DefaultAN1SaleFactory.sol)
...
import "../beacons/DefaultAN1SaleBeacon.sol";
import "./DefaultAN1Sale.sol";

/**
 * @title DefaultAN1SaleFactory
 * @dev This contract is a factory to create DefaultAN1Sales.
 */
contract DefaultAN1SaleFactory is OperatorAccessControlUpgradeable {

    mapping(uint => address) private sales;
    mapping(address => bool) private saleLookup;
    uint public salesCount;
    DefaultAN1SaleBeacon beacon;
    address private nfcIdRegistry;

    /**
     * @dev Emitted when a new DefaultAN1Sale contract is created.
     */
    event DefaultAN1SaleCreated(
    ...
    /**
     * @dev Initializes the contract. Creates the beacon proxy and adds all existing sales.
     */
    function __DefaultAN1Factory_initialize(
        address _initBlueprint
    ) public initializer onlyInitializing {
        __OperatorAccessControl_init(_msgSender());

        __DefaultAN1Factory_initialize_unchained(_initBlueprint);
    }

    function __DefaultAN1Factory_initialize_unchained(
        address _initBlueprint
    ) public onlyInitializing {
        beacon = new DefaultAN1SaleBeacon(_initBlueprint);
    }
    ...
    function createSale(
        ...
        uint256 _usdRedemptionPrice
    ) external onlyOperator returns (address) {

        address payable payableAddress = payable(address(uint160(address(0))));

        BeaconProxy sale = new BeaconProxy(
            address(beacon),
            abi.encodeWithSelector(
                DefaultAN1Sale(payableAddress).__DefaultAN1Sale_initialize.selector,
                msg.sender,
                _an1SaleAddress,
                _salePeriods,
                _eRC20address,
        ...
        address newSaleAddress = address(sale);
        sales[salesCount] = newSaleAddress;
        saleLookup[newSaleAddress] = true;
        salesCount++;

        emit DefaultAN1SaleCreated(
            msg.sender,
            newSaleAddress,
            _an1SaleAddress
        );
    ...
    function getSaleAddress(uint _index) public view returns (address) {
        return sales[_index];
    }

    function saleExists(address _addr) public view returns (bool) {
        return saleLookup[_addr];
    }
...
// SPDX-License-Identifier: MIT
// AN1 Contracts (last updated v1.0.0) (tokens/phygital/DefaultAN1SaleBeacon.sol)
...
contract DefaultAN1SaleBeacon is UpgradeableBeacon {
