...
contract RaffleFacet is RaffleModifiers {
    ...
    function pickWinners(uint256 _raffleId, uint128 seed) 
        external 
        isRaffled( _raffleId)
    returns (address[] memory winners)
    {
        ...
        winners = _generateWinners( _raffleId,  seed);
    ...
    function _generateWinners(uint256 _raffleId, uint256 seed) 
        private 
        returns (address[] memory winners) 
    {
        address[] memory winnersTemp = new address[](s.raffles[_raffleId].maxWinners);
        address[] memory participants = s.raffles[_raffleId].participants;
        uint256 projectId = s.raffles[_raffleId].projectId;
        s.raffles[_raffleId].winnersCount = 0;

        uint256 indexRandon = 0;
        uint256 total = 0;
        for (uint256 counter = 0; s.raffles[_raffleId].winnersCount < s.raffles[_raffleId].maxWinners && total < participants.length; counter++) {
            indexRandon = randomIndex(participants,seed+indexRandon+counter);
            address winner = participants[indexRandon];
            //console2.log("Winner %s at %s", winner, indexRandon);
            if (winner != address(0)) {
                participants[indexRandon] = address(0);
                total++;
    ...
    function randomIndex(address[] memory participants, uint256 seed) private view returns (uint) {
        // solhint-disable-next-line not-rely-on-time
        return uint(keccak256(abi.encodePacked(block.timestamp, participants, seed))) % participants.length;
    }
