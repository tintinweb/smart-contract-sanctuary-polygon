...
contract Vault is ERC4626, ReentrancyGuard, Ownable {
    ...
    ) ERC20(_name, _symbol) ERC4626(IERC20(__asset)) {
    ...
    function totalAssets() public view virtual override returns (uint256) {
        uint256 assets = IStrategy(_strategy).totalAssets();
        return assets;
    }

    function earn() public {
        IStrategy(_strategy).updatePool();
    }
        ...
        returns (uint256)
    {
        ...
        TransferHelper.safeApprove(asset(), _strategy, assets);

        IStrategy(_strategy).deposit(asset(), assets);
    ...
    function withdraw(
        ...
        uint256 withdraw_amt = IStrategy(_strategy).withdraw(asset(), assets);
        ...
        _withdraw(_msgSender(), receiver, _owner, assets, shares); // address(this) or msg.sender
