...
// File: contracts/DigitalElonNFTStaking.sol

pragma solidity ^0.8.0;

contract DigitalElonNFTStaking is Ownable {
    ...
    uint256 multiplier = getMultiplier(balance);
    rewards = (rewardPerSecond * timeElapsed * balance * multiplier) / (10**18);
...
function getPendingRewards(address user) public view returns (uint256) {
    ...
    uint256 multiplier = getMultiplier(balance);
    return (rewardPerSecond * timeElapsed * balance * multiplier) / (10**18);
}
    ...
    function getMultiplier(uint256 nftBalance) public pure returns (uint256) {
        if (nftBalance >= 100) {
            return 5;
        } else if (nftBalance >= 50) {
            return 3;
        } else if (nftBalance >= 20) {
            return 2;
