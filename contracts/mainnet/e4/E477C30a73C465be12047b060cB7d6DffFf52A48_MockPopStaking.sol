...
contract MockPopStaking is IPopStaking, Ownable, ReentrancyGuard {
    ...
    function updatePool() public {
        ...
        uint256 curBlock = _currentBlock;
    ...
    function deposit(uint256 lockAmount, uint256 lockType) public override {
        require(_isEmergency != true, "NOW YOU CAN NOT DEPOSIT");
        require(lockType < 8, "INVALID LOCK TYPE");
    ...
    function reedem(uint256 userIndex) public override nonReentrant {
        ...
        require(_userInfo[msg.sender][userIndex].endTime < _currentBlock, "NOT UNLOCKED YET.");
        updatePool();
    ...
    function getDailyRewards() public view override returns (uint256 dailyRewards) {
        uint256 rewardPerBlock = getRewardPerBlock(_currentBlock);
    ...
    function getRewardPerBlock(uint256 blockNumber) public view override returns (uint256 perBlockReward) {
        if (blockNumber >= _startBlockNumber) {
            ...
            console.log("throughBlock", throughBlock);
    ...
    function getPeriodReward(uint256 startBlock, uint256 endBlock) public view returns (uint256 popRelease) {
        if (startBlock < endBlock) {
            ...
            console.log("wholeEpiccccccccccccccccccc", wholeEpic);

            uint256 popPerInStart = getRewardPerBlock(startBlock);
            uint256 popPerInEnd = getRewardPerBlock(endBlock);
            console.log("popPerInStart", popPerInStart);
            console.log("popPerInEnd", popPerInEnd);
            if (popPerInStart == popPerInEnd) {
                ...
                for (uint i; i < wholeEpic; i++) {
                    wholeBlockPopRelease += oneDayBlocks * popPerInStart + oneDayBlocks * releasePerBlock * 8 * (i + 1) / 10000;
                }
                
                console.log("wholeBlockPopRelease", wholeBlockPopRelease);
            ...
            console.log("popReleaseeeeeeeeeeeeeeeee", popRelease);
        }
    }

    function getPopPrice() public view override returns (uint256) {
        // decimal  6
        return _popPrice;
    }

    function getStakeTVL() public view override returns (uint256 tvl) {
        uint256 popPrice = getPopPrice();  //6
    ...
    function annualPerRate() public view override returns (uint256 apr) {
        ...
        apr = _oneDayBlocks * popPerBlock * 365 * 1e12 / _totalStakingAmount;    //decimal   6
    }
    ...
    function getUserRewardList() public view override returns (uint256[] memory userReward) {
        ...
        if(userLength != 0) {
            ...
            if(_isEmergency == false) {
                ...
                console.log('accPerShare', accPerShare);
            }
            ...
            for (uint i; i < userLength; i++) {
                ...
                console.log("i", userReward[i]);
                console.log("_userInfo[msg.sender][i].amount", _userInfo[msg.sender][i].amount);
                console.log("_userInfo[msg.sender][i].rewardDebt", _userInfo[msg.sender][i].rewardDebt);
                console.log("=========================================================");
            }
        }
    }
    ...
    function setLpAddress(address newLpPool) public onlyOwner {
        _lpPool = newLpPool;
    }
    ...
    function setBlockNumber(uint256 newBlockNumber) public {
        _currentBlock = newBlockNumber;
    }
    ...
    function setPopPrice(uint256 newPrice) public {
        _popPrice = newPrice; //6
    }

    function setStakeOut(address newStakeOut) public {
        _stakeOut = newStakeOut;
    }
