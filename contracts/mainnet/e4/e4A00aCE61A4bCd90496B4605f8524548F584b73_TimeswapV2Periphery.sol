...
contract TimeswapV2Periphery is ITimeswapV2Periphery {
  ...
  ///@return poolPair address of poolPair
  function deployPool(address token0, address token1) external returns (address poolPair) {
    address optionPair = OptionFactoryLibrary.getWithCheck(optionFactory, token0, token1);

    poolPair = ITimeswapV2PoolFactory(poolFactory).create(optionPair);
  }
}
