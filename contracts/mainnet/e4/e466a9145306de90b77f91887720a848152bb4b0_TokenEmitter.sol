...
contract TokenEmitter is LinearVRGDA, ITokenEmitter, AccessControlEnumerable, ReentrancyGuard {
    ...
    function buyToken(
        ...
        uint totalTokens = getTokenAmountForMultiPurchase(msg.value, numChunks);
    ...
    function getTokenAmountForMultiPurchase(uint256 payment, uint256 numChunks) public view returns (uint256) {
        // payment is split up into chunks of numTokens
        // each chunk is estimated and the total is returned
        uint256 total = 0;
        for (uint256 i = 0; i < numChunks; i++) {
            total += _getTokenAmountForSinglePurchase(payment/numChunks, totalSupply() + total);
        }
        return total;
    }
