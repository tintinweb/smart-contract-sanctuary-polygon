...
//参考erc721
contract Semantic is ISemantic, AccessControl,ERC721 {
    ...
    //token信息
    // mapping(Trida => uint256) public tokenDataForIdMap;
    mapping(uint256 => mapping(uint256=>mapping(string=>uint256))) public tokenDataForIdMap;  // s p:owner caller ==exist==>  tokenId 

    mapping(uint256 => Triple) public tokenIdForDataMap;
    ...
    string public constant NO_PERMISSION =  "0x0000000000000000000000000000000000000000";
    string public constant ALL_PERMISSION = "0x0000000000000000000000000000000000000001";
    ...
    function initialize(
        ...
        string memory defaultClassValue,
        string memory defaultPredicateValue
    ) public {
        require(deployer == msg.sender, "not deployer");
        _grantRole(DEFAULT_ADMIN_ROLE, owner);
        baseUri = _baseUri;

        _setClass(Class(defaultClassValue,FieldType.ADDRESS));
        _setPredicate(Predicate(defaultPredicateValue,FieldType.ADDRESS));
    ...
    function addClasss(Class[] memory classes) external {
        for (uint256 i = 0; i < classes.length; i++) {
            ...
            require(
                keccak256(abi.encode(class.value)) !=
                    keccak256(abi.encode("")),
                "class can not empty"
            );
            require(classSize == 0 || classDataForIdMap[class.value] == 0,
                "class already exist"
            );
            _setClass(class);
        }
    ...
    function adminMint(OperateData[] memory operateDatas) external {
        for (uint256 i = 0; i < operateDatas.length; i++) {
            ...
            require(operateData.predicateId < predicateSize,"predicate undefined");
            uint256 subjectId = subjectDataForIdMap[operateData.subject.classId][operateData.subject.value];
            if(subjectId == 0){
                subjectSize++;
                subjectDataForIdMap[operateData.subject.classId][operateData.subject.value] = subjectSize;
                subjectIdForDataMap[subjectSize] = operateData.subject;
            }
            
            Triple memory triple = Triple(subjectId,operateData.predicateId,operateData.object);
            _mint(msg.sender,operateData.subject.value,operateData.subject.classId,triple);
        }
    }
    ...
    function soulMint(SoulOperateData[] memory operateDatas) external {
        for (uint256 i = 0; i < operateDatas.length; i++) {
            ...
            require(operateData.predicateId > 0 && operateData.predicateId <= predicateSize,"predicate undefined");
            uint256 subjectId = generateSubjectId(1,Strings.toHexString(msg.sender));
            Triple memory triple = Triple(subjectId,operateData.predicateId,operateData.object);
            require(tokenDataForIdMap[triple.subjectId][triple.predicateId][triple.object] == 0,"already exist" );
    ...
    function claim(ClaimOperateData[] memory claimOperateDatas) external {
        for (uint256 i = 0; i < claimOperateDatas.length; i++) {
            ...
            string memory rdf = generateRdf(1,Strings.toHexString(msg.sender),operateData.predicateId,operateData.object);
            string memory signOriginalData = string.concat(
                operateData.expireTime.toString(),
                rdf
            );
            address singer = verifyMessage(
     ...
     function testSign(ClaimOperateData memory operateData) external  view returns(address caller,string memory callerStr,string memory rdf,
            string memory signOriginalData,address singer,uint8 v,bytes32 r,bytes32 s){
            require(
                operateData.expireTime > block.timestamp,
                "signData has expired"
            );
            ...
            rdf = generateRdf(1,Strings.toHexString(caller),operateData.predicateId,operateData.object);
 ...
 /**
     ...
     * @param operateDatas 操作数据
     */
    function burn(OperateData[] memory operateDatas) external {
        // for (uint256 i = 0; i < operateDatas.length; i++) {
        //     OperateData memory operateData = operateDatas[i];
     ...
     */
    function semanticMetaDataUri()
        external
        view
        returns (string memory)
    {
        return string.concat(baseUri,"sematic/metadata");
    }

    /**
     * @dev 查询tokenUrl
     * @param tokenId tokenId,e.g: 0
     * @return tokenUri
     */
    ...
    function _mint(address owner, string memory subjectValue,uint256 classId,Triple memory triple) private {
        uint256 tokenId = totalSupply;
        tokenDataForIdMap[triple.subjectId][triple.predicateId][triple.object] = tokenId;
        tokenIdForDataMap[tokenId] = triple;

        _balanceOf[owner] = _balanceOf[owner] + 1;
        tokenHolderMap[tokenId] = owner;

        totalSupply++;
        string memory rdf = generateRdf(classId,subjectValue,triple.predicateId,triple.object);
    ...
    function generateSubjectId(uint256 classId,string memory subjectValue) public returns(uint256){
    ...
    function generateRdf(uint256 classId,string memory subjectValue,uint256 predicateId,string memory object) public view returns(string memory) {
        string memory subject = string.concat(classIdForDataMap[classId].value,subjectValue);
        string memory predicate = predicateIdForDataMap[predicateId].value;
        if (predicateIdForDataMap[predicateId].predicateType == FieldType.STRING){
    ...
    function _checkPermission(uint256 subjectId,string memory object) internal virtual {
        require(
            tokenDataForIdMap[subjectId][1][object] > 0 || tokenDataForIdMap[subjectId][1][ALL_PERMISSION] > 0,
            "not have permission"
        );
    }
    ...
    function verifyMessage(bytes32 _hashedMessage, uint8 _v, bytes32 _r, bytes32 _s) public  view returns (address) {
