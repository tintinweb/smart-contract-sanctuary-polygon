...
contract Marketplace is BaseContract {
  ...
  address public receiveBonusAddress;
  bytes4 constant ERC4907InterfaceId = 0xad092b5c;

  event NftListed(address indexed nftContractAddress, address owner, uint startingBlock, uint timestamp);
  ...
  event OrderExtended(address indexed nftContractAddress, uint indexed tokenId, address user, uint price, uint weekNumber, address paymentAddress, uint expiredTime, uint takeOrderPrice, uint discountPrice,uint timestamp);
  ...
  event PaymentUpdated(address indexed paymentAddress, address poolAddress, bool isUniSwapV3, bool isR0DivR1, bool isStableCoin, uint timestamp);
  ...
  modifier isCitizen() {
    require(citizen.isCitizen(msg.sender), "Marketplace: you are not a citizen");
    _;
  }
  ...
  function initialize() public initializer {
    ...
    receiveBonusAddress = msg.sender;
  }
  ...
  function updatePayment(
    ...
    require(_paymentAddress != address(0), "Marketplace: payment address invalid");

    if (_isStableCoin) {
      require(_poolAddress == address(0), "Marketplace: pool address invalid");
    }

    payments[_paymentAddress] = Payment(_poolAddress, _isUniSwapV3, _isR0DivR1, _isStableCoin);

    emit PaymentUpdated(_paymentAddress, _poolAddress, _isUniSwapV3, _isR0DivR1, _isStableCoin, block.timestamp);
  }
  ...
  function setNFTListing(
    ...
    require(_checkPaymentAddressesNotHasZeroAddress(_paymentAddresses), "Marketplace: payment addresses invalid");
    require(_startingBlock > 0, "Marketplace: starting block invalid");
    ...
    nftListings[_nftContractAddress] = NftListing(
      ...
      true,
      _startingBlock,
      IERC165(_nftContractAddress).supportsInterface(ERC4907InterfaceId)
    );

    emit NftListed(_nftContractAddress, _owner, _startingBlock, block.timestamp);
  }
  ...
  function makeOrder(
    address _nftContractAddress,
    uint _tokenId,
    MakeOrderData calldata _orderData
  ) external isCitizen isOwnerNftListing(_nftContractAddress, _tokenId) {
    if (_orderData._orderType != IMarketPlace.OrderType.trading) {
      require(nftListings[_nftContractAddress].canRental, "Marketplace: This NFT does not support rental");
    }
    require(_orderData._discounts.length == _orderData._paymentAddresses.length, "Marketplace: data invalid");
    _validateDiscounts(_orderData._discounts);
    require(nftListings[_nftContractAddress].enable, "Marketplace: nft contract invalid");
    _validatePaymentAddresses(_nftContractAddress, _orderData._paymentAddresses);
    IERC4907 nftContract = IERC4907(_nftContractAddress);

    if (_orderData._startingPrice != _orderData._endingPrice) {
      require(_orderData._actionDuration > 0, "Marketplace: actionDuration must be > 0");
    }

    if (nftContract.userExpires(_tokenId) <= block.timestamp) {
      _deleteOrder(_nftContractAddress,_tokenId);
    }

    if (_orderData._orderType == IMarketPlace.OrderType.renting) {
      require(_orderData._rentingDuration > 0, "Marketplace: rentingDuration must be > 0");
    } else if (_orderData._orderType == IMarketPlace.OrderType.installmentRental) {
      require(_orderData._weekNumber > 0, "Marketplace: weekNumber must be > 0");
    }

    require(orders[_nftContractAddress][_tokenId].maker == address(0), "Marketplace: NFT is being sold");
    require(_orderData._startingPrice > 0 && _orderData._endingPrice > 0, "Marketplace: startingPrice and endingPrice must be > 0");
    ...
    if (_orderData._orderType == IMarketPlace.OrderType.trading) {
      ...
      rentingDuration = _orderData._orderType == IMarketPlace.OrderType.renting
      ? _orderData._rentingDuration
      : _orderData._weekNumber * config.secondsInWeek;
    }

    orders[_nftContractAddress][_tokenId] = Order(
      msg.sender,
      address(0),
      _orderData._startingPrice,
      _orderData._endingPrice,
      _orderData._startingPrice == _orderData._endingPrice ? 0 : _orderData._actionDuration,
      rentingDuration,
      block.timestamp,
      _orderData._paymentAddresses,
      _orderData._orderType,
      IMarketPlace.OrderStatus.created,
      _orderData._orderType == IMarketPlace.OrderType.installmentRental ? _orderData._weekNumber : 0,
      0,
      0,
      _orderData._discounts
    );
  ...
  function takeOrder(
    ...
    address _paymentAddress,
    uint _weekNumber
  ) external isCitizen validOrder(_nftContractAddress, _tokenId) {
    ...
    (uint currentPrice, uint takeUSDPrice, uint discountPrice) = _getCurrentPrice(_nftContractAddress, _tokenId, _weekNumber, _paymentAddress);
    require(IBEP20(_paymentAddress).balanceOf(msg.sender) >= currentPrice, "Marketplace: insufficient balance");
    uint tokenAmount = _takeToken(currentPrice, _paymentAddress);
    _handleFundToken(order.maker, tokenAmount, _paymentAddress);
    ...
    if (order.orderType == IMarketPlace.OrderType.trading) {
      ...
      expiredTime = order.orderType == IMarketPlace.OrderType.installmentRental
      ...
      order.weekPrepaidNumber = order.orderType == IMarketPlace.OrderType.installmentRental ? _weekNumber : 0;
      nftContract.setUser(_tokenId, msg.sender, uint64(expiredTime));
    }

    order.takeOrderPrice = takeUSDPrice;

    TakeOrder memory takeOrderInfo = TakeOrder(
      msg.sender,
      order.maker,
      tokenAmount,
      order.orderType,
    ...
    emit OrderTaken(_nftContractAddress, _tokenId, takeOrderInfo, block.timestamp);
  }

  function cancelOrder(address _nftContractAddress, uint _tokenId) external isCitizen validOrder(_nftContractAddress, _tokenId) {
    Order storage order = orders[_nftContractAddress][_tokenId];
  ...
  function extendOrder(address _nftContractAddress, uint _tokenId, address _paymentAddress, uint _weekNumber) external isCitizen {
    Order storage order = orders[_nftContractAddress][_tokenId];
    ...
    (uint currentPriceAfterDiscount, uint discountPrice) = _discountPrice(order, order.takeOrderPrice * _weekNumber, _paymentAddress);
    uint currentPrice = _calculateCurrenPriceByTokenAddress(currentPriceAfterDiscount, _paymentAddress);
    ...
    emit OrderExtended(_nftContractAddress, _tokenId, msg.sender, tokenAmount, _weekNumber, _paymentAddress, expiredTime, order.takeOrderPrice, discountPrice, block.timestamp);
  }

  function updateReceiveBonus(address _treasuryAddress, address _receiveBonusAddress) external onlyMn {
    treasuryAddress = _treasuryAddress;
    receiveBonusAddress = _receiveBonusAddress;
  }
  ...
  function getNftListed(address _nftContractAddress) external view returns(
    ...
    uint paymentAddressLength = nftListing.paymentAddresses.length;
    address[] memory tmpPoolAddresses= new address[](paymentAddressLength);
    bool[] memory tmpIsUniSwapV3= new bool[](paymentAddressLength);
    bool[] memory tmpIsR0DivR1= new bool[](paymentAddressLength);
    bool[] memory tmpIsStableCoin= new bool[](paymentAddressLength);

    for (uint i = 0; i < paymentAddressLength; i += 1) {
      tmpPoolAddresses[i] = payments[nftListing.paymentAddresses[i]].poolAddresses;
      tmpIsUniSwapV3[i] = payments[nftListing.paymentAddresses[i]].isUniSwapV3;
      tmpIsR0DivR1[i] = payments[nftListing.paymentAddresses[i]].isR0DivR1;
      tmpIsStableCoin[i] = payments[nftListing.paymentAddresses[i]].isStableCoin;
    }

    return (nftListing.user, nftListing.uri, nftListing.paymentAddresses, tmpPoolAddresses, tmpIsUniSwapV3, tmpIsR0DivR1, tmpIsStableCoin, nftListing.enable);
  }

  function getOrderPayment(address _nftContractAddress, uint _tokenId) external view returns (address[] memory paymentAddresses, uint32[] memory discounts) {
    Order memory order = orders[_nftContractAddress][_tokenId];
    return (order.paymentAddresses, order.discounts);
  }

  function gerReferralConfig() external view returns(uint[] memory refBonusPercentages, uint depositMenCondition) {
    return (referralConfig.refBonusPercentages, referralConfig.depositMenCondition);
  }

  // ============ PRIVATE FUNCTIONS ============

  function _deleteOrder(address _nftContractAddress, uint _tokenId) private {
    delete orders[_nftContractAddress][_tokenId];
  }

  function _discountPrice(Order storage _order, uint _currentPrice, address _paymentAddress) private view returns(uint, uint) {
    uint indexOrderPayment = _order.paymentAddresses.getAddressElementIndexInArray(_paymentAddress);
    uint discountPrice;

    if (_order.discounts[indexOrderPayment] > 0) {
      discountPrice = _currentPrice * _order.discounts[indexOrderPayment] / oneHundredPercentageDecimal3;
      _currentPrice -= discountPrice;
    }

    return (_currentPrice, discountPrice);
  }

  function _validateDiscounts(uint32[] calldata _discounts) private pure {
    for (uint i = 0; i < _discounts.length; i += 1) {
      require(_discounts[i] <= 99e3, "Marketplace: discount invalid");
    }
  }
  ...
  function _checkPaymentAddressesNotHasZeroAddress(address[] calldata _paymentAddresses) private pure returns(bool) {
    bool valid = true;

    for (uint i = 0; i < _paymentAddresses.length; i += 1) {
      if (_paymentAddresses[i] == address(0)) {
        valid = false;
        break;
      }
    }
  ...
  function _convertUsdToTokenByDecimals(address _paymentAddress, uint _currentPrice) private view returns(uint) {
    uint decimalTokenIn = ERC20(usdtTokenAddress).decimals();
    uint decimalTokenOut = ERC20(_paymentAddress).decimals();

    return _currentPrice * (10 ** (decimalTokenOut - decimalTokenIn));
  }

  function _calculateCurrenPriceByTokenAddress(uint _currentPrice, address _paymentAddress) private view returns(uint) {
    if (payments[_paymentAddress].isStableCoin) {
      return (_convertUsdToTokenByDecimals(_paymentAddress, _currentPrice));
    }

    if (payments[_paymentAddress].isUniSwapV3) {
      uint decimalTokenIn = ERC20(usdtTokenAddress).decimals();
      uint decimalTokenOut = ERC20(_paymentAddress).decimals();
      uint tokenPrice = getTokenPrice(payments[_paymentAddress].poolAddresses, decimalTokenIn);
      _currentPrice = _currentPrice * (10 ** decimalTokenOut) / tokenPrice;
    } else {
      (uint r0, uint r1) = ILPToken(payments[_paymentAddress].poolAddresses).getReserves();
      uint tokenPrice = payments[_paymentAddress].isR0DivR1 ? r0 * DECIMAL9 / r1 : r1 * DECIMAL9 / r0;
      _currentPrice = _convertUsdToTokenByDecimals(_paymentAddress, _currentPrice) * DECIMAL9 / tokenPrice;
    }

    return _currentPrice;
  }

  function _getCurrentPrice(address _nftContractAddress, uint _tokenId, uint _weekNumber, address _paymentAddress) private view returns (uint, uint, uint) {
    Order storage order = orders[_nftContractAddress][_tokenId];
    ...
    takeUSDPrice = currentPrice;
    (uint currentPriceAfterDiscount, uint discountPrice) = _discountPrice(order, currentPrice, _paymentAddress);
    currentPrice = _calculateCurrenPriceByTokenAddress(currentPriceAfterDiscount, _paymentAddress);
    currentPrice = order.orderType == IMarketPlace.OrderType.installmentRental ? currentPrice * _weekNumber : currentPrice;
    return (currentPrice, takeUSDPrice, discountPrice);
  }
  ...
  function _takeToken(uint _amount, address _tokenAddress) private returns(uint) {
    IBEP20 usdToken = IBEP20(_tokenAddress);
    uint mkpBalanceBefore = usdToken.balanceOf(address(this));
    ...
    require(usdToken.transferFrom(msg.sender, address(this), _amount), "MarketPlace: transfer error");
    uint mkpBalanceAfter = usdToken.balanceOf(address(this));

    return mkpBalanceAfter - mkpBalanceBefore;
  }

  function _transferToken(uint _amount, address _tokenAddress, address _receiver) private {
    IBEP20 usdToken = IBEP20(_tokenAddress);

    require(usdToken.balanceOf(address(this)) >= _amount, "MarketPlace: insufficient contract balance");
    require(usdToken.transfer(_receiver, _amount), "MarketPlace: transfer error");
  }
  ...
  function _bonusReferral(address _userAddress, uint _amount, address _tokenAddress) private {
    ...
    address receiver = receiveBonusAddress;

    if (!citizen.isCitizen(_userAddress) || referralConfig.refBonusPercentages.length == 0) {
      _transferToken(_amount, _tokenAddress, receiveBonusAddress);
      emit RefBonusSent(senderAddress, receiveBonusAddress, _amount, block.timestamp);
      return;
    }
    ...
    for (uint i = 0; i < referralConfig.refBonusPercentages.length; i++) {
      ...
      if (depositedAndCompounded >= referralConfig.depositMenCondition) {
        _transferToken(refBonusAmount, _tokenAddress, inviterAddress);
        receiver = inviterAddress;
      } else {
        _transferToken(refBonusAmount, _tokenAddress, receiveBonusAddress);
        receiver = receiveBonusAddress;
      }
    ...
    if (remainingRefBonusAmount > 0) {
      _transferToken(remainingRefBonusAmount, _tokenAddress, receiveBonusAddress);
    }
  }
