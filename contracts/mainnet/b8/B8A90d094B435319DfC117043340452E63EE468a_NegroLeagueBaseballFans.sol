...
// File: NegroLeagueBaseballFans.sol

/**
 * The Negro League Baseball Museum (NBLM) celebrates the historical 
 * significance of the negro leagues, including the great Jackie Robinson 
 * and each unsung hero of those teams. Joining this exclusive community 
 * will solidify your own special place in history and take you back in 
 * time to commemorate some of the greatest Negro League Baseball 
 * players. The NLBM is the world’s only museum dedicated solely to 
 * preserving this incredible history and as a privately funded, not-for-
 * profit organization, it needs your support to continue to educate 
 * future generations. This exclusive collection is sure to be a homerun 
 * to all baseball fans, both young and old alike!
 */
contract NegroLeagueBaseballFans is ViciManagedERC721WithRecall {
    event StateChange(uint256 indexed tokenId, string oldState, string newState);
    event URI(string value, uint256 indexed id);
    
    StateMachineDropManager internal stateMachine;
    
    bytes32 UMPIRE_ROLE = "umpire";
    ...
    constructor()
        ViciManagedERC721WithRecall(
            ...
            "NLBF",
            37476,
            IERC2981(address(0xf3f6DC7b33DF68a40A9B3425A4Fa694EfeBdB526))
        )
    {
        setContractURI("https://storage.googleapis.com/arrival-shadow-motion-newman/nlbm/CollectionMetadata.json");
    }
    ...
    function changeState(uint256 _tokenId, string memory _stateName) 
	    public 
	    onlyOwnerOrRole(UMPIRE_ROLE) 
	{
		require(exists(_tokenId));
		stateMachine.changeState(_tokenId, _stateName);
    }

    function _initializeTokenManager() internal virtual override {
        StateMachineDropManager dropManager = new StateMachineDropManager(maxSupply);
