...
contract BrewlabsPair is IBrewlabsPair, BrewlabsERC20 {
    ...
    // this low-level function should be called from a contract which performs important safety checks
    function swap(uint256 amount0Out, uint256 amount1Out, address to, uint256 discount, bytes calldata data) external lock {
    ...
    function setFeePercent(uint16 newfeePercent) external lock {
        ...
        if (feeMgr == address(0)) {
            if (msg.sender != IBrewlabsFactory(factory).feePercentOwner()) {
                revert("Brewlabs: only factory's feePercentOwner");
            }
        } else {
            if (msg.sender != feeMgr) {
                revert("Brewlabs: only factory's feeManager");
            }
        }
