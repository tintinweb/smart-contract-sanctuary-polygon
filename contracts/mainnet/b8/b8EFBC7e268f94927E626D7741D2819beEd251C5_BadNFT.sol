...
contract BadNFT is Ownable, ERC721Enumerable, ERC721URIStorage {
	...
	function mint(address player_, uint256 cardId_) public returns (uint256) {
		require(cardId_ != 0 && cardInfoes[cardId_].cardId != 0, "K: wrong cardId");
	...
	function tokenURI(uint256 tokenId_) public view override(ERC721URIStorage, ERC721) returns (string memory) {
		require(_exists(tokenId_), "ERC721Metadata: URI query for nonexistent token");
		// string memory cURI = cardInfoes[cardIdMap[tokenId_]].tokenURI;
		// string memory baseURI = myBaseURI;
	...
	function batchTokenInfo(address account_)
		...
		view
		returns (
			uint256[] memory cIdInfo,
			uint256[] memory tIdInfo,
			string[] memory uriInfo
		)
	{
		uint256 amount = balanceOf(account_);
		uint256 tokenId;
		cIdInfo = new uint256[](amount);
