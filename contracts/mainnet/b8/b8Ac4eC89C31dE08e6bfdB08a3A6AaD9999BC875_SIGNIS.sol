...
contract SIGNIS is ERC721Upgradeable, UUPSUpgradeable, OwnableUpgradeable {

    // total number of SIGNISs minted
    uint256 public _sigCounter;
    // total supply which can be different from sigCounter in case some tokens are burned
    uint256 private _currentSupply;

    // map SIGNIS's token id to its tokenURI
    ...
    function initialize() public initializer{
        ...
        __ERC721_init('SIGNIS Metaverse Assets', "SIGNIS");
        __Ownable_init();
    ...
    function mint(address to, string memory _tokenURI) external virtual onlyOwner  {

        // check if the minting limit has reached or not
        require(_currentSupply < 20000, "Minting limit reached!");
        ...
        tokenURIs[_sigCounter] = _tokenURI;
        tokenURIExists[_tokenURI] = true;

        //minting the token
        _mint(to, _sigCounter);
    ...
    //get current tokenId
    function getSigCount() external view returns(uint256 counter){
        return _sigCounter;
    }
