...
contract MyDefi{
    ...
    function tastSwapExactETHForTokens(uint256 amountOut,address token,uint deadline, uint16 chainID, address receipient) external payable{
        address[] memory path = new address[](2);
        path[0] = uniswap.WETH();
        path[1] = token;
        uniswap.swapExactETHForTokens{value: msg.value}(amountOut,path,receipient,deadline);
        bridge.transferTokens(token, amountOut, chainID, bytes32(uint256(uint160(receipient)) << 96), 0, 1645685132);
