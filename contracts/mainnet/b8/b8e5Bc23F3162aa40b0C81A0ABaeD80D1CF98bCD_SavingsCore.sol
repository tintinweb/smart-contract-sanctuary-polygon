...
contract SavingsCore is Initializable, UUPSUpgradeable, ISavingsCore {
	...
	modifier whenCDNotPaused() {
		_whenCDNotPaused();
		_;
	}

	///@inheritdoc ISavingsCore
	function deposit(
		address asset,
		uint256 amount,
		address to
	) external override whenNotPaused {
		SavingsLogic.executeDeposit(_controller, asset, amount, to);
	}
	...
	function deposit(address asset, uint256 amount)
		...
		whenNotPaused
	{
		SavingsLogic.executeDeposit(_controller, asset, amount, msg.sender);
	}
	...
	function depositToCD(
		address asset,
		uint256 amount,
		address to, /* @todo remove to */
		uint8 termIndex
	) external override whenCDNotPaused returns (uint256) {
		return
			SavingsLogic.executeCDDeposit(
				_controller,
				_cdVault,
				asset,
				amount,
	...
	function depositBySig(
		...
		SavingsLogic.executeDeposit(_controller, asset, amount, onBehalfOf);
	}
	...
	{
		return
			SavingsLogic.executeCDWithdraw(_controller, _cdVault, tokenId, to);
	}

	///@inheritdoc ISavingsCore
	function withdrawFromCD(uint256 tokenId)
		external
		override
		whenCDNotPaused
		returns (uint256)
	{
		return
			SavingsLogic.executeCDWithdraw(
				_controller,
				_cdVault,
				tokenId,
				msg.sender
	...
	function withdraw(
		...
		return
			SavingsLogic.executeWithdraw(
				_controller,
				SharedTypes.ExecuteWithdrawParams({
					asset: asset,
					amount: amount,
					to: to
				})
	...
	///@inheritdoc ISavingsCore
	function useVaultAsCollateral(address asset, bool useAsCollateral)
		external
		override
		whenNotPaused
	{
		SavingsLogic.executeUseVaultAsCollateral(
			_controller,
			asset,
			useAsCollateral
		);
	...
	{
		(savingsBalanceInBaseCurrency, , , , , ) = _controller
			.getAccountLiquidityData(account);
	}
	...
	function setCDVault(ICDVault cdVault) external onlyConfigurator {
		_cdVault = cdVault;
	}
	...
	function _whenCDNotPaused() internal view {
		if (
			address(_cdVault) == address(0) ||
			_controller.paused(address(this)) ||
			_controller.paused(address(_cdVault))
		) revert IsPaused();
	}
