...
interface IGasProvider {
    function repayGas(bytes32 borrowHash_) external payable;
}

contract GasRepayProtocol is IUseProtocol, NativeReturnMods {
    error InvalidParam();

    address private immutable _gasProvider;

    constructor(address gasProvider_) { _gasProvider = gasProvider_; }

    function use(UseParams calldata params_) external payable {
        if (params_.chain != block.chainid
         || params_.account != msg.sender
         || params_.args.length != 32
         || params_.inAmounts.length != 1
         || params_.ins.length != 1
         || !TokenHelper.isNative(params_.ins[0].token)
         || params_.ins[0].minAmount != params_.inAmounts[0]
         || params_.ins[0].maxAmount != params_.inAmounts[0]
         || params_.outs.length != 1
         || params_.outs[0].token != address(0)
         || params_.outs[0].minAmount != 0
         || params_.outs[0].maxAmount != 0
        ) revert InvalidParam();

        NativeClaimer.State memory nativeClaimer;
        _repayGas(params_.inAmounts[0], abi.decode(params_.args, (bytes32)), nativeClaimer);
    }

    function _repayGas(uint256 amount_, bytes32 borrowHash_, NativeClaimer.State memory nativeClaimer_) private returnUnclaimedNative(nativeClaimer_) {
        TokenHelper.transferToThis(TokenHelper.NATIVE_TOKEN, msg.sender, amount_, nativeClaimer_);
        IGasProvider(_gasProvider).repayGas{value: amount_}(borrowHash_);
    }
}