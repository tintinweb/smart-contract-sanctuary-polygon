...
contract CuteZebras is ERC721 {
    uint256 public totalSupply;
    uint256 public cost = 0.2 ether;
    uint256 public maxMints = 118;
    ...
    constructor()
        ERC721("Cute Zebras", "CZ")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 118) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeig6iagwuc2q5k5qxx6qqwhikxg4czbfurog2lhsz6ljpjv7pg7hfa/", Strings.toString(tokenId), ".json")
            );
    }
