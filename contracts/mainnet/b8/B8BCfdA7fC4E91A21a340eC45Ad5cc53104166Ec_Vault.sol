...
contract Vault is Ownable, Signers, VerifySigEIP712 {
    ...
    function _portalBridgeStart(uint256 dstAmount, PortalBridgeDescription calldata _Pdesc) internal {
        ...
        bytes32 transferId = keccak256(
            abi.encodePacked(address(this), pDesc.recipient, pDesc.token, pDesc.amount, pDesc.recipientChain, pDesc.nonce, uint64(block.chainid))
        );
    ...
    function portalCompletePayload(bytes memory encodeVm, bool isNotNative) public onlyOwner {
        if (isNotNative) {
            IBridge(PORTAL).completeTransferWithPayload(encodeVm);
        } else {
            IBridge(PORTAL).completeTransferAndUnwrapETHWithPayload(encodeVm);
    ...
    function portalComplete(bytes memory encodeVm, bool isNotNative) public onlyOwner {
        if (isNotNative) {
