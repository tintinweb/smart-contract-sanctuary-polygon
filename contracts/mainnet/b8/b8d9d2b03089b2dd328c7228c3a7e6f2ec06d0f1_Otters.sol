...
// Contract: Project Dignity - Otters
// Token ID: "ProjectDignityOtters"
// Token Symbol: "PDO"
// Author: tiffanydys
// Coder: tiffanydys
...
contract Otters is ERC721, ERC721Enumerable, DefaultOperatorFilterer, Ownable {
    // paytoken: PD-WL
    // costvalue1: PD-WL Cost
    // costvalue2: MATIC Cost
    struct TokenInfo {
        IERC20 paytoken;
        uint256 costvalue1;
        uint256 costvalue2;
    ...
    TokenInfo[] public AllowedCryptoOtters;
    
    address public fundAddress;
    using Strings for uint256;
    string public baseURI;
    string public baseExtension = ".json";
    uint256 public otterSupply = 0;
    ...
    constructor(
      ...
      setBaseURIOtters(_initBaseURI);
      setFundAddress(_fundAddress);
      setMintWave(0, 3000, 0);
    }

    function addCurrencyOtters(
        IERC20 _paytoken,
        uint256 _costvalue1,
        uint256 _costvalue2
    ) public onlyOwner {
        AllowedCryptoOtters.push(
            TokenInfo({
                paytoken: _paytoken,
    ...
    function mint() public payable {
        ...
        require(otterSupply + 1 <= allowedMintSupply, "Max supply reached!");
        require(blacklisted[msg.sender] == false, "Your account is blacklisted.");

        TokenInfo storage tokens = AllowedCryptoOtters[activePID];
        IERC20 paytokenPDWL;

        paytokenPDWL = tokens.paytoken;

        uint256 costPDWL = tokens.costvalue1;
        uint256 costMATIC = tokens.costvalue2;
        ...
        // do not burn PD-WL tokens on public mint
        if (mintWave > 0) {
            costPDWL = 0;
        }
        ...
        _safeMint(msg.sender, otterSupply + 1);
        otterSupply = otterSupply + 1;
    }

    function walletOfOwnerOtters(address _owner)
    public
    view
    ...
    // Check Otter Rarity
    function checkOtterRarity(uint256 _otterId) public view returns (string memory) {
        return rarity[_otterId];
    }

    function retrieveMapSize() public view returns (uint256) {
        return blacklistSize;
    } 

    // --------------- OWNLY OWNER FUNCTIONS ---------------
    // update otter rarity manually on reveal
    function updateOtterRarity(uint256[] memory _otterId, string[] memory _rarity) public onlyOwner() {
        uint256 x = 0;
        for (x = 0; x < _rarity.length; x++) {
            rarity[_otterId[x]] = _rarity[x];
        }
    }
    ...
    function setBaseURIOtters(string memory _newBaseURI) public onlyOwner() {
        baseURI = _newBaseURI;
    }
    
    function setBaseExtensionOtters(string memory _newBaseExtension) public onlyOwner() {
