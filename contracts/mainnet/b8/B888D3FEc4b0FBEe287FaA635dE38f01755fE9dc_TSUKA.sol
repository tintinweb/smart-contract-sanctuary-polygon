...
library Address{
 ...
 contract TSUKA is Context, IERC20, Ownable { 
    ...
    address public marketingWallet = 0xb34E8d159872Ae54613f2ac14171cc40DCf50812;
    address public devWallet = 0xb34E8d159872Ae54613f2ac14171cc40DCf50812;
    ...
    string private constant _name = "TSUKALYGON";
    ...
    constructor (address routerAddress) {
        ...
        IERC20(WETH).approve(address(router), ~uint256(0));
    ...
    function setBuyTaxes(uint256 _reward, uint256 _marketing, uint256 _liquidity, uint256 _project) public onlyOwner {
       buytaxes = Taxes(_reward,_marketing,_liquidity,_project);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _reward, uint256 _marketing, uint256 _liquidity, uint256 _project) public onlyOwner {
       sellTaxes = Taxes(_reward,_marketing,_liquidity,_project);
        emit FeesChanged();
    }


   function enableTransferTax() external onlyOwner {
       require (!transferTaxEnabled, "Transfer tax already enabled");
       transferTaxEnabled = true;
   }

   function disableTransferTax() external onlyOwner {
       require (transferTaxEnabled, "Transfer tax already disabled");
       transferTaxEnabled = false;
   }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        //Ensures that there is no fees on wallet to wallet transfers.
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        swapBNBForWETH(address(this).balance);

        uint256 deltaBalance = IERC20(WETH).balanceOf(address(this)) - initialBalance;
        uint256 unitBalance = deltaBalance / (denominator - temp.liquidity);
        uint256 wethToAddLiquidityWith = unitBalance * temp.liquidity;

        if(wethToAddLiquidityWith > 0){
            // Add liquidity to pancake
            addLiquidity(tokensToAddLiquidityWith, wethToAddLiquidityWith);
        }

        uint256 marketingAmt = unitBalance * 2 * temp.marketing;
        if(marketingAmt > 0){
           IERC20(WETH).transfer(marketingWallet,marketingAmt);
        }
        uint256 projectAmt = unitBalance * 2 * temp.project;
        if(projectAmt > 0){
            IERC20(WETH).transfer(devWallet, projectAmt);
        }
    }
   ...
   function swapTokensForBNB(uint256 tokenAmount) private {
     ...
     function swapBNBForWETH(uint256 amount) private {
    ...
    //Use this in case WETH dust remains in the contract
    function rescueWETH(uint256 wethAmount) external onlyOwner{
        require(IERC20(WETH).balanceOf(address(this)) >= wethAmount , "insufficient WETH balance");
        IERC20(WETH).transfer(owner(),wethAmount);
    }
