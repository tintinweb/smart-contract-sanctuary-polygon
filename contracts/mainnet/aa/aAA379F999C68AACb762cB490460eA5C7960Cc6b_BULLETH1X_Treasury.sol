...
contract BULLETH1X_Treasury is ContractGuard {
   ...
   //     try IOracle(tokenOracle).update() {} catch {}
    ...
    function getIndexPrice(uint _currentEthPrice, uint _previousEpochEthPrice, uint _previousEpochTokenIndexPrice) public pure returns (uint) {
        uint IndexPrice = _currentEthPrice.mul(_previousEpochTokenIndexPrice).div(_previousEpochEthPrice);
        return IndexPrice;
    }
