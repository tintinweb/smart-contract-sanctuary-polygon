...
import "./interfaces/curve/ICryptoPoolETH.sol";
...
contract MagpieCurveRouter is ReentrancyGuard, IMagpieCurveRouter {
    ...
    function mainExchange(ExchangeArgs memory exchangeArgs, address registry) private {
        ...
        if (exchangeArgs.from.isNative()) {
            ethAmount = exchangeArgs.amount;
        }

        if (isUnderlying) {
            ICurvePool(exchangeArgs.pool).exchange_underlying{value: ethAmount}(
                i,
                j,
                exchangeArgs.amount,
                exchangeArgs.expected
            );
        } else {
            ICurvePool(exchangeArgs.pool).exchange{value: ethAmount}(i, j, exchangeArgs.amount, exchangeArgs.expected);
        }
    }
    ...
    function cryptoExchange(ExchangeArgs memory exchangeArgs, address registry) private {
        ...
        if (exchangeArgs.from.isNative() || exchangeArgs.to.isNative()) {
            ICryptoPoolETH(exchangeArgs.pool).exchange{value: ethAmount}(
                i,
                j,
                exchangeArgs.amount,
                exchangeArgs.expected,
    ...
    function exchange(ExchangeArgs calldata exchangeArgs) external payable nonReentrant returns (uint256) {
        if (exchangeArgs.from.isNative()) {
            require(exchangeArgs.amount == msg.value, "Incorrect ETH amount");
        } else {
        ...
        if (!exchangeArgs.from.isNative() && !isApproved[exchangeArgs.from][exchangeArgs.pool]) {
            exchangeArgs.from.approve(exchangeArgs.pool, type(uint256).max);
            isApproved[exchangeArgs.from][exchangeArgs.pool] = true;
        }

        if (IRegistry(cryptoRegistry).get_decimals(exchangeArgs.pool)[0] > 0) {
            cryptoExchange(exchangeArgs, cryptoRegistry);
            // Some networks dont have cryptoFactory
        } else if (!cryptoFactory.isNative() && IRegistry(cryptoFactory).get_decimals(exchangeArgs.pool)[0] > 0) {
...
interface IMagpieCurveRouter {
    ...
    function exchange(ExchangeArgs calldata exchangeArgs) external payable returns (uint256 amountOut);
}
