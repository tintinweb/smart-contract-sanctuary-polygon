...
contract TokenUKR is ERC20Std, Governance {
    ...
    uint256 public ratioOfExchange = 10;//1 USDT = 10 UKR
    uint256 private _balance4DAO = 0;
    uint256 private _balance4Redeem = 0;
    uint256 private _destUsdtOfDonation;//Destination amount of USDT of donation : 50 millions
    ...
    event StartDonation(uint256 startTime, uint256 destUsdtOfDonation);
    ...
    constructor() public ERC20Std("Save Ukraine DAO", "UKR", 18){
        //ETH
        //        swapRouter = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        //        USDT = IERC20(0xdAC17F958D2ee523a2206206994597C13D831ec7);
        //        //USDT
        //        _whiteList[0xdAC17F958D2ee523a2206206994597C13D831ec7] = 1;
        //        //WBTC
        //        _whiteList[0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599] = 1;
        //        //SHIB
        //        _whiteList[0x95aD61b0a150d79219dCF64E1E6Cc01f0B64C4cE] = 1;

        //Polygon
        swapRouter = IUniswapV2Router02(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);
        USDT = IERC20(0xc2132D05D31c914a87C6611C10748AEb04B58e8F);
        //USDT
        _whiteList[0xc2132D05D31c914a87C6611C10748AEb04B58e8F] = 1;
        //WBTC
        _whiteList[0x1BFD67037B42Cf73acF2047067bd4F2C47D9BfD6] = 1;


        WETH = swapRouter.WETH();

        _destUsdtOfDonation = 5 * 10 ** 7 * 10 ** uint256(USDT.decimals());
    ...
    modifier claimable(){
        ...
        require(packages[msg.sender].locked > 0, 'INSUFFICIENT');
        _;
    }
    ...
    function donateETH() lock public payable returns (uint256 halfAmountOfUKR) {
        IWETH(WETH).deposit{value : msg.value}();
        halfAmountOfUKR = _donate(WETH, msg.value);
    }

    //Donate tokens（include USDT）
    function donate(address token, uint256 amount) lock external returns (uint256 halfAmountOfUKR) {
        require(inWhiteList(token), "UNSUPPORTED_TOKEN");
        IERC20(token).safeTransferFrom(msg.sender, address(this), amount);
        halfAmountOfUKR = _donate(token, amount);
    }

    function _donate(address token, uint256 amount) internal returns (uint256 halfAmountOfUKR) {
        ...
        uint256 usdt2Redeem = usdtAmount.mul(25).div(10 ** 2);
        uint256 usdt2DAO = usdtAmount.sub(usdt2Redeem);
        _balance4Redeem = _balance4Redeem.add(usdt2Redeem);
        _balance4DAO = _balance4DAO.add(usdt2DAO);

        uint256 reserveUKR = usdtAmount.mul(ratioOfExchange).mul(10 ** uint256(decimals())).div(10 ** uint256(USDT.decimals()));
        halfAmountOfUKR = reserveUKR.mul(50).div(10 ** 2);
        _mint(msg.sender, halfAmountOfUKR);

        Package storage package = packages[msg.sender];
        package.owner = msg.sender;
        package.claimed = package.claimed.add(halfAmountOfUKR);
        package.locked = package.locked.add(halfAmountOfUKR);
    }
    ...
    function isOver() public view returns (bool) {
        return _totalUsdtOfDonation >= _destUsdtOfDonation;
    }
    ...
    function locked(address account) public view returns (uint256) {
        return packages[account].locked;
    }

    function claim() claimable lock external returns (bool){
        require(isOver(), "NOT_ALLOWED");
        Package storage package = packages[msg.sender];
        require(package.locked > 0, "INSUFFICIENT");
        package.claimed = package.claimed.add(package.locked);
        _mint(msg.sender, package.locked);
        emit Claimed(msg.sender, package.locked);
        package.locked = 0;
        return true;
    }

    //赎回
    function redeem(uint256 amountOfUKR) external returns (bool){
        require(amountOfUKR > 0, "ZERO_AMOUNT");
        require(_balance4Redeem > 0, "INSUFFICIENT_USDT_FOR_REDEEM");

        uint256 usdtCanReceived = _balance4Redeem.mul(amountOfUKR).div(_totalSupply);
        _balance4Redeem = _balance4Redeem.sub(usdtCanReceived);

        _burn(msg.sender, amountOfUKR);
        USDT.safeTransfer(msg.sender, usdtCanReceived);

        emit Redeem(msg.sender, amountOfUKR, usdtCanReceived);
        return true;
    }
    ...
    function startWith(uint256 destUsdtOfDonation_) onlyGovernance external {
        canTest = false;
        _startDonation(destUsdtOfDonation_);
    }

    function start() onlyGovernance external {
        canTest = false;
        _startDonation(5 * 10 ** 7 * 10 ** uint256(USDT.decimals()));
    }

    function startForTest() onlyGovernance external {
        require(canTest, "FORBIDDEN");
        _startDonation(500 * 10 ** uint256(USDT.decimals()));
    }

    function _startDonation(uint256 destUsdtOfDonation_) internal {
        if (destUsdtOfDonation_ > 0) {
            _destUsdtOfDonation = destUsdtOfDonation_;
        }

        emit StartDonation(block.timestamp, _destUsdtOfDonation);
    }

    //TODO:Remove this when deploying on main chain.
    function safeWithdraw(address recipient, uint256 amount) onlyGovernance external {
