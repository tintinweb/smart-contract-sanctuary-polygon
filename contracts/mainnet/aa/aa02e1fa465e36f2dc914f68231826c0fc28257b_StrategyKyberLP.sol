...
interface IDMMPool {
    ...
    function getTradeInfo()
        external
        view
        returns (uint112 _reserve0, uint112 _reserve1, uint112 _vReserve0, uint112 _vReserve1, uint256 feeInPrecision);
}
...
interface IKyberFairLaunch {
    ...
    function getUserInfo(
        uint256 _pid,
        address _account
    ) external view returns (uint256 amount, uint256[] memory unclaimedRewards, uint256[] memory lastRewardPerShares);
...
contract StrategyKyberLP is CompoundStrat {
    using Path for bytes;
    using SafeERC20 for IERC20;
    ...
    constructor(
        ...
        outputToNativeRoute = UniswapV3Utils.pathToRoute(_paths[0]);
        nativeToLp0Route = UniswapV3Utils.pathToRoute(_paths[1]);
        nativeToLp1Route = UniswapV3Utils.pathToRoute(_paths[2]);
    ...
    function swapRewardsToNative() internal {
        ...
        if (nativeBal > 0) {
            IWETH(native).deposit{value: nativeBal}();
        }

        uint256 outputBal = IERC20(output).balanceOf(address(this));
        if (outputBal > 0) {
            if (outputToNativePath.length > 0) {
                kyberSwap(elasticRouter, outputToNativePath, outputBal);
            }
        }
        // extras
        for (uint i; i < rewardTokens.length; ) {
            ...
            if (bal >= rewards[rewardTokens[i]].minAmount) {
                if (rewards[rewardTokens[i]].useElastic) {
                    kyberSwap(elasticRouter, rewards[rewardTokens[i]].rewardToNativePath, bal);
                } else {
    ...
    function addLiquidity() internal override {
        ...
        lp1Amt = (nativeBal * 1 ether) / (ratio + 1 ether);
        lp0Amt = nativeBal - lp1Amt;

        if (lpToken0 != native) {
            kyberSwap(elasticRouter, nativeToLp0Path, lp0Amt);
        }

        if (lpToken1 != native) {
            kyberSwap(elasticRouter, nativeToLp1Path, lp1Amt);
        }
