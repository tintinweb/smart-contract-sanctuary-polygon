...
contract VaultManager is IVaultManager {
  ...
  function claimRewards() external override {
    require(vaults[msg.sender].addr != address(0), "No vault");
    IVault(vaults[msg.sender].addr).claimRewards(msg.sender);
  }

  function unstakeAndClaim() external override {
    require(vaults[msg.sender].addr != address(0), "No vault");
    IVault(vaults[msg.sender].addr).unstakeAndClaim(msg.sender);
  }
  ...
  function balanceOfSphere(address _vaultOwner) external view returns (uint256) {
    ...
    uint256 lpAmount = liquidityBalanceOf(_vaultOwner);
    (uint256 token0Balance,) =
      IDystRouter(router).quoteRemoveLiquidity(token0, token1, false, lpAmount);

    return token0Balance;
  }

  function liquidityBalanceOf(address _vaultOwner) public view returns (uint256) {
    if(vaults[_vaultOwner].addr == address(0)) {
      return 0;
    }
...
contract Vault is IVault {
  ...
  function claimRewards(address _to) external override onlyOwnerOrManager {
    require(_to != address(0), "Zero address");
    IPenroseProxy(penroseProxy).claimAllStakingRewards();
    IPenroseProxy(penroseProxy).convertDystToPenDystAndStake();
    IERC20(pen).transfer(_to, IERC20(pen).balanceOf(address(this)));
  }

  function unstakeAndClaim(address _to) external override onlyOwnerOrManager {
    require(_to != address(0), "Zero address");
    IPenroseProxy(penroseProxy).unstakePenDyst();
    IERC20(PENDYST).transfer(_to, IERC20(PENDYST).balanceOf(msg.sender));
  }

  function withdraw(address _token) external override onlyOwnerOrManager {
...
interface IVault {
  ...
  function unstakeAndClaim(address _to) external;
