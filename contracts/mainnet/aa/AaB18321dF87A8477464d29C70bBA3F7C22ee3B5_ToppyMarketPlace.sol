...
contract ToppyMarketPlace is Ownable{
    ...
    uint public interBlockTime = 2300; // average time between blocks (~2.3s for polygon)
    ...
    function offer(bytes32 _key, uint256 _amount) public payable {
        ...
        uint secondsPassed = (block.number - listing_.startedAt) * interBlockTime / 1000;
    ...
    function _getDutchCurrentPrice(Listing memory listing_) internal view returns (uint) {
        ...
        secondsPassed = (block.number - listing_.startedAt) * interBlockTime / 1000;
    ...
    function _isAuctionExpired(uint _startedAt, uint _duration) internal view returns (bool) {     
        uint secondsPassed = (block.number - _startedAt) * interBlockTime / 1000;
