    ...
    ERC20Upgradeable
{
    ...
    function _completeWithdraw() internal returns (uint256) {
        //TODO: 1. Proceed normally if the withdrawal exists
        // 2. Else, withdraw based on [currentRound-1]
        ...
        if (withdrawalShares != 0) {
            withdrawAmount = ShareMath.sharesToAsset(
                withdrawalShares,
        ...
        //check total shares held by users and transfer
        uint256 userShares = balanceOf(msg.sender);
        if (userShares != 0) {
            _transfer(msg.sender, address(this), userShares);
            withdrawalShares = withdrawalShares.add(userShares);
        ...
        require(withdrawalShares > 0, "!shares");
    ...
    function totalPending() external view returns (uint256) {
     ...
     function correctPPS(uint256 _round, uint256 _pps) external onlyOwner {
        roundPricePerShare[_round] = _pps;
    }
