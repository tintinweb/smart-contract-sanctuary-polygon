...
contract LimelightStaking is Ownable {
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        Clout.safeTransferFrom( msg.sender, address(this), _amount );
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( sClout.gonsForBalance( _amount ) ),
            expiry: epoch.number.add( warmupPeriod ),
            lock: false
        });
        
        sClout.safeTransfer( address(warmupContract), _amount );
    ...
    function claim ( address _recipient ) external {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint256 amount = sClout.balanceForGons( info.gons );
    ...
    function forfeit() external {
        ...
        uint memoBalance = sClout.balanceForGons( info.gons );
        warmupContract.retrieve( address(this),  memoBalance);
        Clout.safeTransfer( msg.sender, info.deposit);
    ...
    function unstake( uint _amount, bool _trigger ) external {
        ...
        Clout.safeTransfer( msg.sender, _amount );
    ...
    function rebase() public {
        if( epoch.endTime <= uint32(block.timestamp) ) {
            ...
            sClout.rebase( epoch.distribute, epoch.number );
            ...
            uint staked = sClout.circulatingSupply();
    ...
    function contractBalance() public view returns ( uint ) {
        return Clout.balanceOf( address(this) ).add( totalBonus );
    }
