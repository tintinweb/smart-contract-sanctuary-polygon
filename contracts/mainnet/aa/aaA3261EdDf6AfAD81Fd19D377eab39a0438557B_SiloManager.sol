...
contract SiloManager is Initializable, KeeperCompatibleInterface {
    ...
    function setPreventFast(bool _flag) external onlyOwner {
        preventFast = _flag;
        if(_flag){
            lastSilo = address(0);
            lastUpkeep = 0 ;
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        if (silo != address(this)) {
            if(preventFast && lastSilo != address(0) && lastUpkeep != 0){
                if(lastSilo == silo && (lastUpkeep + upkeepDuration) > block.number){
