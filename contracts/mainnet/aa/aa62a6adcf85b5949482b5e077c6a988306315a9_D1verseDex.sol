...
contract D1verseDex {
    ...
    function setNameVersion(string memory _name, string memory _version) public onlyDAO {
        name = _name;
        version = _version;
        HashEIP712Name = keccak256(bytes(name));
        HashEIP712Version = keccak256(bytes(version));
    ...
    function setFeeRate(uint256 _feeRate) public onlyDAO {
        require(_feeRate <= feeDenominator, "Fee rate is too high");
        feeRate = _feeRate;
    }

    function setFeeRecipient(address _feeRecipient) public onlyDAO {
        require(_feeRecipient != address(0), "Fee recipient rate is address(0)");
        feeRecipient = _feeRecipient;
    }

    function setMaxRoyaltyRate(uint256 _maxRoyaltyRate) public onlyDAO {
    ...
    function orderState(FixedPriceOrder memory order) public view returns(OrderQuery memory) {
    ...
    function orderStateBatch(FixedPriceOrder[] memory orders) public view returns(OrderQuery[] memory) {
