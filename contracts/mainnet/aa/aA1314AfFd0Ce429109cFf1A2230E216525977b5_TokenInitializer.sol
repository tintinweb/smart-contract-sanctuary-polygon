...
/// @title Token Initializer
/// @author Shiva Shanmuganathan
/// @notice This contract registers the RBW Token in the Diamond token registry
/// @dev TokenInitializer contract is not attached to the Diamond as a Facet
contract TokenInitializer {
    ...
    /// @notice Registers the RBW Token in the Diamond token registry
    /// @dev The function can only be accessed by Diamond owner
    /// @param params The addresses of the tokens to be registered
    /// @custom:emits AddedTokenRegistry
    function registerTokens(InitParams memory params) external {
        LibDiamond.enforceIsContractOwner();
        LibSatBank.setRBWAddress(params.RBWAddress);
        LibSatBank.setUNIMAddress(params.UNIMAddress);
        LibSatBank.setWETHAddress(params.WETHAddress);
    }
}
