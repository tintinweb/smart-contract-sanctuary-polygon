...
// File: tatum/Tatum721General.sol
...
contract Tatum721General is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, AccessControl, ERC721Burnable {
    ...
    /**
      * @dev modifier for transfer function, only execute when the the collection is set transferable
     */
    modifier transferable() {
        require(isTransferable(),"Tatum721General: not transferable");
        _;
    }
    /**
      * @dev only execute when the the collection is set not transferable
     */
    modifier nonTransferable() {
        require(!isTransferable(),"Tatum721General: transferable");
        _;
    }

    /**
      *@dev function to set token transferable status 
     */
    function setTransferable(bool transferable_) external  {
        require(
            hasRole(TRANSFER_ROLE, _msgSender()),
            "TatumGeneral721: must have transfer setter role to set"
        );
    ...
    */
    ...
    /**
      * @dev set transferable modifier
    */
    function safeTransfer(address to, uint256 tokenId, bytes calldata data) public virtual transferable {
        super._safeTransfer(_msgSender(), to, tokenId, data);
    }

    function safeTransfer(address to, uint256 tokenId) public virtual transferable {
        super._safeTransfer(_msgSender(), to, tokenId, "");
    }

    /**
      * @dev override transferFrom public function in ERC721.sol, in order to keep all transfer function use transferable() modifier
    */
    function transferFrom(address from, address to, uint256 tokenId) public override transferable {
