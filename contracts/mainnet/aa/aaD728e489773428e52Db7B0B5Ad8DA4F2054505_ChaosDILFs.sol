...
contract Chaos is Ownable {
  ...
  function addChaos(address h) public onlyOwner {
    ogs[h] = true;
  }

  function removeChaos(address h) public onlyOwner {
    ogs[h] = false;
  }

  modifier isChaos() {
    address sender = _msgSender();
    require(owner() == sender || ogs[sender], "Chaos: Unauthorized");
    _;
  }
}
// File: ChaosDILFs.sol
...
contract ChaosDILFs is Ownable, ERC1155, Chaos {
  ...
  function mint(address account, uint256 tokenId, uint256 _mintAmount) public payable {
	uint256 supply = 0;
    require(!paused);
    ...
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount);
		...
		_mint(account, tokenId, _mintAmount, "");
  }
