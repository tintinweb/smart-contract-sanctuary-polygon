...
contract vvGHST is Initializable, ERC20Upgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function joinPool(uint256 _mai) internal{
        ...
        //once we join the pool, must stake BPSP tokens to get rewards
        uint256 bpspBalance = IERC20Upgradeable(s.BPSP).balanceOf(address(this));
        BalancerGauge(s.balancerGauge).deposit(bpspBalance);
    }

    function leavePoolExactAmount(uint256 _mai) internal {

        BalancerGauge(s.balancerGauge).withdraw(_mai);
    ...
    function leave(uint256 _share) public whenNotPaused {
        if(_share>0){
            ...
            if(debtValue == 0 || newCDR < s.targetCDRLow){
    ...
    function swapQi() public onlyApproved{
        //get the balance of Qi in this contract
        uint256 balanceQi = IERC20Upgradeable(im_Qi).balanceOf(address(this));

        IERC20Upgradeable(im_Qi).approve(im_balancer,balanceQi);

        balancerSwap(im_Qi,im_vQi,0xb4670d1389c758e4380c4211bcbc85342688b9c50002000000000000000003d8);
    }


    //this function is called by our approved bot when the contract receives Qi airdrop
    function receivedAirdrop(bool _swap) public onlyApproved{
        ...
        //compound remaining Qi into vQi either by swapping or minting
        if(_swap){
            swapQi();
        }else{
            compoundQi();
    ...
    function withdrawERC20(address _token, uint256 _amount) public onlyApproved{
        IERC20Upgradeable(_token).transfer(s.owner,_amount);
    }
    ...
    function compoundBalancerRewards(bytes memory _msg, bool _swap) public onlyApproved{
        ...
        receivedAirdrop(_swap);

        //sell Bal for USDC
        uint256 balBalance = IERC20Upgradeable(im_bal).balanceOf(address(this));
        if(balBalance > 0){
    ...
    function initBalancerGauge(address _balancerGauge) public onlyApproved{

        //set the address for the balancer gauge
        s.balancerGauge = _balancerGauge;

        //approve the gauge to take our tokens
        IERC20Upgradeable(s.BPSP).approve(_balancerGauge,MAX_INT);

        //stake all our tokens
        uint256 bpspBalance = IERC20Upgradeable(s.BPSP).balanceOf(address(this));
        BalancerGauge(_balancerGauge).deposit(bpspBalance);
    }

    function depositBPSP(uint256 _amount) public onlyApproved{
        BalancerGauge(s.balancerGauge).deposit(_amount);
    }

    function withdrawBPSP(uint256 _amount) public onlyApproved{
        BalancerGauge(s.balancerGauge).withdraw(_amount);
    }

    function claimGaugeRewards() public onlyApproved{
        BalancerGauge(s.balancerGauge).claim_rewards();
    }
