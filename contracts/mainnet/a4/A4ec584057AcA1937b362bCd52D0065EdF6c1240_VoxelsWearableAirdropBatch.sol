...
contract VoxelsWearableAirdropBatch {
    event Airdrop1155(
        address from,
        address NftContract,
        uint256 totalNumber
    ...
    function ERC1155AirdropBatch(
        ...
        require(IERC1155(batchContractAddress).isApprovedForAll(msg.sender, address(this)), "not approve");

        uint256 totalNFT = perDrop * receivers.length;
        for(uint256 i = 0; i < tokenIdSet.length; ++i) {
            require(IERC1155(batchContractAddress).balanceOf(from, tokenIdSet[i]) >= totalNFT, "tokens not enough:");

            for(uint256 j = 0; j < receivers.length; ++j) {
                IERC1155(batchContractAddress).safeTransferFrom(from, receivers[j], tokenIdSet[i], perDrop, "");
            }
        }
