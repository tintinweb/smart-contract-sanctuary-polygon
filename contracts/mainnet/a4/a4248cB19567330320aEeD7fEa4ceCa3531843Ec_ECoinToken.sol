...
contract ECoinToken is GasHelper, ERC20 {
  ...
  string constant _name = "Etest Finance";
  string constant _symbol = "ETTv1";
  ...
  uint256 public _minAmountToAutoSwap =  10 * (10 ** decimal); // 1000

  // Fees
  uint256 public feePool = 200; // 1%
  uint256 public feeStake = 200;  // 2%
  uint256 public feeBurnRate = 100; // 1%
  uint256 public feeAdministrationWallet = 200; // 2%
  uint256 public feeMarketingWallet = 100; // 1%
  ...
  bool public pausedStake = false;
  bool public pausedSwapPool = false;
  bool public pausedSwapAdmin = false;
  bool public disabledStake = false;
  ...
  uint public restrictedDailySell = maxSupply / 1000; // 0.1%
  ...
  bool public emitStakeEvent = false;
  ...
  constructor()ERC20(_name, _symbol) {
    ...
    PancakeRouter router = PancakeRouter(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506); // POLYGON
  ...
  function getFeeTotal() public view returns(uint256) { return feePool + feeStake + feeBurnRate + feeAdministrationWallet + feeMarketingWallet; }
  function getSpecialWalletFee(address target) public view returns(uint stake, uint pool, uint burnRate, uint adminFee, uint marketFee ) {
  ...
  function setMarketingWallet(address account) public isAuthorized(0) { marketingWallet = account; }
  function setDevWalet(address account, uint pos) public isAuthorized(0) {
    uint dailySeconds = 86400;
    require(devLimitLastSell[devsWallet[pos]] + dailySeconds < block.timestamp, "Cant change dev wallet within 24h after execute a sell");
    devsWallet[pos] = account;
  }
  function setRestrictedDailySell(uint amount) public isAuthorized(0) { restrictedDailySell = amount; }
  ...
  function setFeesOperational(uint256 administration, uint256 feeMarketing) external isAuthorized(1) {
    feeAdministrationWallet = administration;
    feeMarketingWallet = feeMarketing;
    require(getFeeTotal() <= maxTotalFee, "All rates and fee together must be lower than 16%");
  }

  function setSpecialWalletFee(address target, uint stake, uint pool, uint burnRate, uint adminFee, uint marketFee)  external isAuthorized(1) {
    uint total = stake + pool + burnRate + adminFee + marketFee;
    require(total <= maxTotalFee, "All rates and fee together must be lower than 16%");
    uint composedValue = stake + (pool * 1e4) + (burnRate * 1e8) + (adminFee * 1e12) + (marketFee * 1e16);
    specialFeesByWallet[target] = composedValue;
  }

  // ----------------- Token Flow Configurations -----------------
    ...
    if(!isExemptFee(senderAttributes) && !isExemptFeeReceiver(receiverAttributes)) {
      if(!isExemptInnerTrade(senderAttributes) || !isExemptInnerTrade(receiverAttributes)) { // Special trade between defined wallets
        ...
        if(isSpecialFeeWallet(senderAttributes)) { // Check special wallet fee on sender
          (stakeFee, poolFee, burnFee, adminFee, marketingFee) = getSpecialWalletFee(sender);
        } else if(isSpecialFeeWalletReceiver(receiverAttributes)) { // Check special wallet fee on receiver
          (stakeFee, poolFee, burnFee, adminFee, marketingFee) = getSpecialWalletFee(receiver);
        }
        feeAmount = ((stakeFee + poolFee + burnFee + adminFee + marketingFee) * amount) / 10000;
      }
    }

    if (feeAmount != 0) splitFee(feeAmount, sender, adminFee, poolFee, burnFee, marketingFee, stakeFee);
  ...
  function splitFee(uint256 incomingFeeTokenAmount, address sender, uint adminFee, uint poolFee, uint burnFee, uint marketingFee, uint stakeFee) private {
    uint256 totalFee = adminFee + poolFee + burnFee + marketingFee + stakeFee;
    ...
    if (marketingFee > 0) {
      uint feeAmount = (incomingFeeTokenAmount * marketingFee) / totalFee;
  ...
  function checkLimitOfResitrictedWallets(address sender, address receiver, uint amount) private {
    ...
    amounSell += amount;
    require(amounSell <= restrictedDailySell, "Dev wallet has reached its daily sell limit");
  ...
  function executeStakeOperations(address sender, address receiver, uint senderAmount, uint receiverAmount, uint senderAttributes, uint receiverAttributes) private {
    ...
    if(!pausedStake && !isExemptDistributionMaker(senderAttributes)) distributeStake(false);
  }
  
  function distributeStake(bool forced) public {
    if(_holders.length == 0) return;
    if (totalTokensStaked - totalTokensDistributed < minTokenToDistribute) {
      if (forced) revert("Minimum token to distribute not reached");
      return;
    }
    ...
    uint stakePerShareVal = stakePerShare;
    uint stakePrecisionVal = stakePrecision;
    uint minTokenToReceiveVal = minTokenToReceive;
    uint amountDistributed = 0;

    bool emitStakeEventVal = emitStakeEvent;
    ...
    while(gasUsed < maxGasBeUsed && iterations < holdersLength) {
      ...
      amountDistributed += claimDistribution(_holders[index], false, stakePerShareVal, stakePrecisionVal, minTokenToReceiveVal, emitStakeEventVal);
    ...
    _holdersIndex = index;
    totalTokensDistributed += amountDistributed;
  }

  function claimDistribution(address receiver, bool forced, uint stakePerShareVal, uint stakePrecisionVal, uint minTokenToReceiveVal, bool emitStakeEventVal) private returns(uint) {
    uint256 pendingToReceive = calculateDistribution(receiver, holderMap[receiver].amountToken, stakePerShareVal, stakePrecisionVal);
    if (pendingToReceive < minTokenToReceiveVal && !forced) { return 0; }
    _balances[receiver] += pendingToReceive;

    holderMap[receiver].totalReceived += pendingToReceive;
    holderMap[receiver].pendingReceive = 0;
    holderMap[receiver].receivedAt = block.timestamp;

    if(emitStakeEventVal == true) emit Transfer(address(this), receiver, pendingToReceive);
    return pendingToReceive;
  }

  function claimStake(address target) public returns (uint amount) {
    return claimDistribution(target, true, stakePerShare, stakePrecision, minTokenToReceive, emitStakeEvent);
  }
  ...
  function calculateDistribution(address holder, uint amountToken, uint stakePerShareValue, uint stakePrecisionValue) private returns (uint) {
    ...
    if (totalToBePaid <= entryPointMarkup) return 0;
  ...
  function buyBackWithDecimals(uint256 decimalAmount, address destAddress) private {
    ...
    if (destAddress == address(0)) {
      ...
      if(!isExemptStaker(_attributeMap[destAddress])) _updateHolder(destAddress, tokenAmount, minTokenHoldToStake, stakePerShare, stakePrecision);
    }
