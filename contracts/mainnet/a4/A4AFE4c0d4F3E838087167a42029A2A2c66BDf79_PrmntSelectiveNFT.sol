...
interface IDescriptor {
    function getPath(uint tokenId) external view returns (string memory path);
    function getArt(Params memory _params) external view returns (string memory artwork);
    function getSvg(Params memory _params) external view returns (string memory svg);
    // function getArt(address _counter, uint hue, string memory seed, uint minId, uint maxId, uint itemSize, uint gridCount) external view returns (string memory artwork);
}

contract PrmntSelectiveNFT is ERC721Base{ // , Ownable 
    mapping (uint => Params) internal paramsMap;   
    // mapping (uint256 => uint256) internal tokenHue;   
    //  ArtworkDescriptor public descriptor;
    
    // IDescriptor public w;
    ...
    constructor(
        ...
        address _royaltyRecipient,
        uint128 _royaltyBps,
        address _intitalDescriptorAddress
        ) ERC721Base (
            _name,
            _symbol,
            _royaltyRecipient,
        ...
        descriptorAddress = _intitalDescriptorAddress;
        // IDescriptor w = IDescriptor(_intitalDescriptorAddress);
    }

    function setDescriptorAddress( address _address ) public {
        descriptorAddress = _address;
    }
    
    function setParams(uint _index, address _counter, uint _hue, string calldata _seed, uint[] calldata _ids, uint _itemSize, uint _gridCount, uint _offset) external  {
        paramsMap[_index] = Params({
            seed: _seed,
            ids: _ids,
            hue: _hue,
            itemSize: _itemSize,
            ...
            offset: _offset,
            artContract: _counter
        });
    }
    function getParams(uint _index) external view returns (Params memory prams ) {
        return paramsMap[_index];
    }
    
    function claim(uint256 tokenId,  uint hue, uint _paramsIndex ) public {
        string memory tempURI = getURI(tokenId, hue, _paramsIndex);
        mintTo(msg.sender, tempURI);
    }
    
    
    function getArt(uint _index) public view  returns (string memory path){
        return IDescriptor(descriptorAddress).getArt(paramsMap[_index]);
    }
    
    function getSvg(uint _index) public view  returns (string memory path){
        return IDescriptor(descriptorAddress).getSvg(paramsMap[_index]);
    }

    function getURI(uint256 tokenId, uint hue, uint _paramsIndex) public view  returns (string memory){
        string memory output = string.concat('data:application/json;base64,', 
            Base64.encode(
            abi.encodePacked(
                '{"name": "Generative Art", "description": "PRMNT generative", "image": "data:image/svg+xml;base64,', 
                Base64.encode(
                    'test'
                    // abi.encodePacked(IDescriptor(descriptorAddress).getArt(paramsMap[_paramsIndex]))
                    // abi.encodePacked(IDescriptor(descriptorAddress).getArt(_counter,  hue, 'seedphase', 0, 8, 320, 2))
                    // abi.encodePacked(w.getArt(_counter,  hue, 'seedphase', 0, 8, 320, 2))
                    // abi.encodePacked(w.getPath(1))
                ), 
                '"}'
            )
            )
...
// function getArtworkFromObject(address _counter, uint tokenId) public view returns (string memory artwork) {
    ...
    // function getPathFromDescriptor(uint tokenId) public view returns (string memory artwork) {
    //     return IOnChain(_counter).getArtwork(tokenId).image;    
    // }

// function getDescriptorAddress() public returns (address descriptorAddress ) {
    //     return descriptorAddress;
    // }
    
    // function setInterface( address _address ) public {
    //     IDescriptor w = IDescriptor(_address);
    // }
    // function getInterface( ) public returns (IDescriptor descriptor){
    //     return w;
    // }

    // function getPath( uint tokenId ) public returns (string memory path){
    //     return descriptor.getPath(tokenId);
    // }