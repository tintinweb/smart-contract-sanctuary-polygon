...
contract KAITEI is ERC721A, AccessControl {

  using Strings for uint256;
  string public uriSuffix = ".json";
  string public uriPrefix = "ipfs://Qmc6renwRs5Ut51nyz1ViuZHJJfHKWwkpGmTANfn7bfzxP/";
  ...
  function mint(uint256 _mintAmount) public onlyRole(ADMIN) {
    require(totalSupply() + _mintAmount <= maxSupply, "Max supply exceeded!");
    _safeMint(_msgSender(), _mintAmount);
  }

  function minter(address to, uint256 qty) external onlyRole(MINTER) {
        require(totalSupply() + qty <= maxSupply, "Max supply exceeded!");
        _safeMint(to, qty);
  }

  function bulkMint(address[] calldata _to, uint256[] calldata _id) public onlyRole(MINTER) {
