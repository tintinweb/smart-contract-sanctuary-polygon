...
/**
 * @dev used to split up 1 input amount into 5 output amounts, and send amounts to recipients
 */
...
contract DCAAdvancedTransfer is BaseSiloAction {
    ...
    function transferAction(
        ...
        (inputs, ratio, recipients) = abi.decode(
            configuration,
            (address[5], uint256[4], address[4])
        );
    ...
    function createConfig(
        ...
        configData = abi.encode(_inputs, _ratio, _recipients);
    }
...
abstract contract BaseSiloAction is Ownable {
    ...
    uint constant public MAX_TRANSIENT_VARIABLES = 5;
     ...
     */
    function enter(address implementation, bytes memory configuration, bytes memory inputData) public virtual returns(uint[5] memory){}

    /**
     * @dev what a silo should do when exiting a strategy and running this action
     */
    function exit(address implementation, bytes memory configuration, bytes memory outputData) public virtual returns(uint[5] memory){}
    ...
    function actionValid(bytes memory ) external view virtual returns(bool, bool){
    ...
    function extraInfo(bytes memory ) public view virtual returns(uint,uint,uint,uint){
        return (0,0,0,0);
    }
...
interface IAction{
    ...
    function extraInfo(bytes memory configuration) external view returns(uint,uint, uint, uint);
