...
contract GammaLPFarmV2 is BaseSiloAction {
    address private constant FarmV3_Address =
    ...
    address private constant QUICK_V3_QUOTER =
        0xa15F0D7377B2A0C0c10db057f641beD21028FC89;
    ...
    constructor(string memory _name, address _siloFactory) {
        name = _name;
        metaData = "address[5],address[5],uint256,uint256";
    ...
    //     if (inputAmounts[1] > 0 && inputAmounts[2] == 0) {
    //         address[] memory path = new address[](2);

    //         path[0] = inputs[1];
    //         path[1] = inputs[2];

    //         uint256 price = getUniPrice(QUICK_V2_ROUTER, path, inputAmounts[1]);

    //         uint256 swapAmount = (amount1 * inputAmounts[1]) /
    //             (amount1 + (amount0 * price) / inputAmounts[1]);
    ...
    //         amounts[0] = inputAmounts[1] - swapAmount;
    //     } else if (inputAmounts[1] == 0 && inputAmounts[2] > 0) {
    //         address[] memory path = new address[](2);

    //         path[0] = inputs[2];
    //         path[1] = inputs[1];

    //         uint256 price = getUniPrice(QUICK_V2_ROUTER, path, inputAmounts[2]);

    //         uint256 swapAmount = (amount0 * inputAmounts[2]) /
    //             (amount0 + (amount1 * price) / inputAmounts[2]);
    ...
    //         amounts[1] = inputAmounts[2] - swapAmount;
    //     }
    // }
    ...
    function getGamamaLp(
        ...
        uint256[] memory amounts = prepareShareLp(inputs, inputAmounts, pos);

        if (amounts[0] == 0 || amounts[1] == 0) {
            return 0;
        }

        SafeERC20.safeIncreaseAllowance(IERC20(inputs[1]), pos, amounts[0]);

        SafeERC20.safeIncreaseAllowance(IERC20(inputs[2]), pos, amounts[1]);
        ...
        //     console.log("deposit error :%s", string(err));
        // }
    ...
    function enter(
        ...
        (address[5] memory inputs, , , uint256 pid) = abi.decode(
            configuration,
            (address[5], address[5], uint256, uint256)
        );

        IFarmV3 farm = IFarmV3(FarmV3_Address);
        address pos = address(farm.poolInfo(pid).lpToken);

        if (inputAmounts[0] + inputAmounts[1] + inputAmounts[2] == 0) {
            outputAmounts[0] = deposit(implementation, 0, farm, pos, pid);
            return outputAmounts;
        }

        uint256 share = getGamamaLp(inputs, inputAmounts, pos);
        outputAmounts[0] = deposit(
            implementation,
    ...
    function deposit(
        ...
        IERC20 rewardToken = farm.erc20();

        reward = rewardToken.balanceOf(address(this));

        farm.deposit(pid, shareLp);

        reward = rewardToken.balanceOf(address(this)) - reward;

        if (reward > 0) {
            reward = _takeFee(implementation, reward, address(rewardToken));
        }
    }
    ...
    function exit(
        ...
        uint256 deposited = farm.deposited(pid, address(this));

        if (deposited > 0) {
            farm.withdraw(pid, deposited);

            reward = rewardToken.balanceOf(address(this)) - reward;

            (uint256 amount0, uint256 amount1) = IHypervisor(pos).withdraw(
                deposited,
                address(this),
                address(this),
                [uint256(0), 0, 0, 0]
            ...
            outputAmounts[1] = amount0;

            outputAmounts[2] = amount1;
    ...
    function createConfig(
        ...
        configData = abi.encode(_inputs, _outputs, _trigger, _pid);
    }
    ...
    function showBalances(
        ...
        IFarmV3 farm = IFarmV3(FarmV3_Address);
        address pos = address(farm.poolInfo(pid).lpToken);
        uint256 deposited = IFarmV3(FarmV3_Address).deposited(pid, _silo);
    ...
    function checkMaintain(
        ...
        (, , uint256 trigger, uint256 pid) = abi.decode(
            configuration,
            (address[5], address[5], uint256, uint256)
        );

        uint256 reward = IFarmV3(FarmV3_Address).pending(pid, msg.sender);
    ...
    function extraInfo(
        ...
        uint256 pendingReward = IFarmV3(FarmV3_Address).pending(
            pid,
            msg.sender
        );

        info[0] = pendingReward;
        info[3] = MAX_UINT256;
    }
