...
contract LckLiquidityMiningPool is Operator, ReentrancyGuard {
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "lckRewardPool: existing pool?");
        }
    }
    ...
    function migrate(address _newRewardPool) public onlyOperator {
        ...
        migrated = true;
        tokenPerSecond = 0;
        safeLckTransfer(_newRewardPool, TOTAL_REWARDS);

        emit MigrateRewardPool(_newRewardPool);
    }

    // Safe lck transfer function, just in case if rounding error causes pool to not have enough lck.
    function safeLckTransfer(address _to, uint256 _amount) internal {
        uint256 _lckBal = lck.balanceOf(address(this));
        if (_lckBal > 0) {
            if (_amount > _lckBal) {
                lck.safeTransfer(_to, _lckBal);
            } else {
    ...
    function updateLckPerSec(uint256 _new_lckPerSec) public onlyOperator {
        require(_new_lckPerSec >= 0, "Invalid amount");
        tokenPerSecond = _new_lckPerSec;
    }
