...
contract SphereLocker is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function _forceLockExpire(address _account) internal updateReward(_account) {
    LockedBalance[] storage locks = userLocks[_account];
    Balances storage userBalance = balances[_account];
    Balances storage rageQuitUserBalance = balances[_account];
    uint112 locked;
    uint256 length = locks.length;
    uint32 currentUnlockTime;
    require(length > 0, "no locks");
    ...
    for (uint256 i = 0; i < locks.length; i++) {
      if (locks[i].unlockTime > block.timestamp && locks[i].unlockTime > currentUnlockTime) {
        currentUnlockTime = locks[i].unlockTime;
        break;
      }
    }

    require(currentUnlockTime > block.timestamp, "Nothing locked");

    rageQuitUserBalance.locked += userBalance.locked;
    userBalance.locked = 0;
    userBalance.nextUnlockIndex = locks.length.to32();
    lockedSupply -= locked;
