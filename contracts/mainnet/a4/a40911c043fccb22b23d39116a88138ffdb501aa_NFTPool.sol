...
// File: contracts/interfaces/IGameMaster.sol
...
// SPDX-License-Identifier: MIT
contract NFTPool is IERC721Receiver, Initializable, AccessControlUpgradeable, PausableUpgradeable {
    using SafeMath for uint256;
    IGameMaster public gameMaster;
    IERC20 public rewardToken;
    IERC721 public weapons;
    IERC721 public operators;
    ...
    mapping(IERC721 => uint256) public inventoryLimit;
    ...
    function initialize(IGameMaster _gameMaster, address _poolAdmin) public initializer {
        ...
        rewardToken = IERC20(address(gameMaster.tagToken()));
        weapons = IERC721(address(gameMaster.weapons()));
        operators = IERC721(address(gameMaster.characters()));
    }
    ...
    function deposit(IERC721 token, uint256 tokenId) public virtual whenNotPaused tokenAllowed(token) {
    ...
    function fight(
        ...
        if (balanceAfter > balanceBefore) {
            ...
            tokenRewards[operatorOwner] = tokenRewards[operatorOwner].add(operatorReward);
            ...
            tokenRewards[weaponOwner] = tokenRewards[weaponOwner].add(weaponReward);
            emit WeaponOwnerDistributed(weaponOwner, weaponReward);
            totalReward = totalReward.sub(operatorReward).sub(weaponReward);
            tokenRewards[msg.sender] = tokenRewards[msg.sender].add(totalReward);
    ...
    function setInventoryLimit(IERC721 _token, uint32 _limit) public onlyGameController {
    ...
    modifier deposited(IERC721 token, uint256 tokenId) {
        require(token.ownerOf(tokenId) == address(this), 'The asset is not deposited in the pool');
        _;
    }
    modifier tokenAllowed(IERC721 token) {
