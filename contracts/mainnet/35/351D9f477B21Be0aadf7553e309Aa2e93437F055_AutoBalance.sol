...
import { IBhavishNativeSDK } from "../Interface/IBhavishNativeSDK.sol";
...
interface IBhavishPredictionStorage {
    function getPredictionRound(uint256 roundId) external view returns (IBhavishPrediction.Round memory);
}

contract AutoBalance is AccessControl, Ownable {
    IBhavishNativeSDK public nativeSDK;
    IBhavishPredictionStorage public bhavishPredictionStorage;
    bytes32 underlying;
    bytes32 strike;
    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");

    IBhavishPredictionNative.SwapParams swapParams;

    constructor(
        IBhavishNativeSDK _nativeSDK,
        bytes32 _underlying,
        bytes32 _strike,
        IBhavishPredictionStorage _bhavishPredictionStorage
    ) {
        nativeSDK = _nativeSDK;
    ...
    function performPredict(uint256 _roundId) external onlyOperator(msg.sender) {
        uint256 _predictAmount = address(this).balance / 2;
        nativeSDK.predict{ value: _predictAmount }(
            IBhavishSDK.PredictionStruct(underlying, strike, _roundId, true),
            address(this),
            address(this)
        );
        nativeSDK.predict{ value: _predictAmount }(
            IBhavishSDK.PredictionStruct(underlying, strike, _roundId, false),
            address(this),
            address(this)
        );
    }

    function performClaimAndRefund(uint256 _roundId) external {
        IBhavishPrediction.RoundState rStatus = bhavishPredictionStorage.getPredictionRound(_roundId).roundState;

        if (rStatus == IBhavishPrediction.RoundState.ENDED) {
            uint256[] memory roundIds = new uint256[](1);
            roundIds[0] = _roundId;

            nativeSDK.claim(IBhavishSDK.PredictionStruct(underlying, strike, 0, false), roundIds, swapParams);
        } else if (rStatus == IBhavishPrediction.RoundState.CANCELLED) {
            nativeSDK.refundUsers(IBhavishSDK.PredictionStruct(underlying, strike, 0, false), _roundId);
        }
    }
