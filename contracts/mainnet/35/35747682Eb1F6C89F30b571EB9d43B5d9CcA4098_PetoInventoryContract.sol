    ...
    UUPSUpgradeable
{
    ...
    function mint(address to, uint32 count) public onlyOwner {
    ...
    function createItem(uint32 tokenId, address owner_) private {
    ...
    function addToWhiteList(address to) public onlyOwner {
        require(!_whiteList[to].active, "This address has already been added");
        _whiteListCounter.increment();
        _whiteList[to] = WhiteListItem(true, 0, false);
    }

    function addToWhiteListBatch(address[] calldata toAddAddresses) external onlyOwner {
        for (uint i = 0; i < toAddAddresses.length; i++) {
            addToWhiteList(toAddAddresses[i]);
        }
    }

    // function removeFromWhiteList(address[] calldata toRemoveAddresses) external onlyOwner {
    //     for (uint i = 0; i < toRemoveAddresses.length; i++) {
    //         delete _whiteList[toRemoveAddresses[i]];
    //     }
    // }

    function fetchWhiteAccount(address whiteAccount) external view returns (WhiteListItem memory) {
        return _whiteList[whiteAccount];
    }

    function claim() external {
        address sender = _msgSender();
        WhiteListItem storage whiteAccount = _whiteList[sender];
        require(whiteAccount.active, "Not whitelisted");
        require(!whiteAccount.transfered, "User claimed earlier");
        uint32 tokenId = _mint(sender);
        whiteAccount.tokenId = tokenId;
        whiteAccount.transfered = true;
    }
}
