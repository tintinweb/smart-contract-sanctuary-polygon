...
contract Swap {
    ...
    function SwapInUni(address tokenTo, address tokenOut, uint256 amount) public OnlyOwner returns (uint256 amountOut) {
     ...
     function SwapOutUni(address tokenTo, address tokenOut) public OnlyOwner returns (uint256 amountOut) {
    ...
    function SwapInSushi(address tokenIn, address tokenOut, uint amount) public OnlyOwner returns(uint[] memory){
    ...
    function SwapOutSushi(address tokenIn, address tokenOut) public OnlyOwner returns(uint[] memory){
        IERC20 token = IERC20(tokenIn);
        uint ballanceToken = token.balanceOf(address(this));
    ...
    function SwapInQuick(address tokenIn, address tokenOut, uint amount) public OnlyOwner returns(uint[] memory){
        IERC20 token = IERC20(tokenIn);
        token.transferFrom(msg.sender, address(this), amount);
        token.approve(address(sushiRouter), amount);
        address[] memory path = new address[](2);
    ...
    function SwapOutQuick(address tokenIn, address tokenOut) public OnlyOwner returns(uint[] memory){
        IERC20 token = IERC20(tokenIn);
        uint ballanceToken = token.balanceOf(address(this));
        token.approve(address(sushiRouter), ballanceToken);
        address[] memory path = new address[](2);
