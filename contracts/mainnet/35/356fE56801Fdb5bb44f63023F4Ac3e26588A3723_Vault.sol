...
/** 
 *  SourceUnit: /home/shashvat/router-protocol/WORK/vault/contracts/Vault.sol
*/
...
contract Vault is IVault, ReentrancyGuard, NativeMetaTransaction {
    ...
    function earned(address account) public view override returns (uint256) {
        ...
        for (uint256 i = 0; i < userVaultData.length; i++) {
            ...
            uint256 currentTime = block.timestamp > userVaultData[i].vestingPeriodEnds
    ...
    function stake(uint256 amount) external override nonReentrant {
        ...
        UserVault memory userVaultNewEntry = UserVault(
            amount,
            block.timestamp,
            block.timestamp.add(vestingPeriod),
            block.timestamp,
            0,
            0
        );
