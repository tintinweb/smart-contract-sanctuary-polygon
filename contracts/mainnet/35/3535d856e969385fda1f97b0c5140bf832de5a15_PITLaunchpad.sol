...
// File contracts/main/interfaces/IPITLaunchpad.sol
...
// Interface for IPITLaunchpad
interface IPITLaunchpad {
    // /**
    //  * @notice It initializes the contract (for proxy patterns)
    //  * @dev It can only be called once.
...
// File contracts/main/PITLaunchpad.sol
...
contract PITLaunchpad is IPITLaunchpad, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    IPITLaunchpadVesting public pitLaunchpadVesting;

    // Number of pools
    uint8 public constant numberPools = 2;

    // The timestamp when PITLAUNCHPAD starts
    uint256 public startTimestamp;

    // The timestamp when PITLAUNCHPAD ends
    ...
    event SetVestingContract(address pitLaunchpadVesting);
     ...
     * @dev It can only be called once.
     * @param _lpToken: the LP token used
     * @param _offeringToken: the token that is offered for the PITLAUNCHPAD
     * @param _startTimestamp: the start timestamp for the PITLAUNCHPAD
     * @param _endTimestamp: the end timestamp for the PITLAUNCHPAD
     * @param _adminAddress: the admin address for handling tokens
     */
    constructor(
        IERC20 _lpToken,
    ...
    function depositPool(
        ...
        if (harvestLockRatio > 0) {
            require(address(pitLaunchpadVesting) != address(0), "Must set vesting contract");
        }
    ...
    function _safeTransfer(
        ...
        if (harvestLockRatio == 0) {
            ...
            IPITLaunchpadVesting(pitLaunchpadVesting).addVesting(_pid, lockedAmount, _user);

            // Send token over to the vesting schedule contract
            offeringToken.safeTransfer(address(pitLaunchpadVesting), lockedAmount);
    ...
    function setWhitelist(uint8 _pid, bytes32 _merkleRoot) external override onlyOwner {
        require(block.timestamp < startTimestamp, "PITLAUNCHPAD has started");
    ...
    /**
     * @notice Set the vesting contract
     * @param _pitLaunchpadVesting: the ratio
     */
    function setVestingContract(address _pitLaunchpadVesting) external override onlyOwner {
        require(block.timestamp < startTimestamp, "PITLAUNCHPAD has started");

        require(address(pitLaunchpadVesting) == address(0), "pitLaunchpadVesting has been set");
        require(_pitLaunchpadVesting != address(0), "_pitLaunchpadVesting should not be address(0)");

        pitLaunchpadVesting = IPITLaunchpadVesting(_pitLaunchpadVesting);

        emit SetVestingContract(address(_pitLaunchpadVesting));
    }
