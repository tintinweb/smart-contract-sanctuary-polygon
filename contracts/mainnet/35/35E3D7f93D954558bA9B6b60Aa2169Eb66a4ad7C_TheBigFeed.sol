...
contract TheBigFeed {
  ...
  function listFeeders() public view returns (Feeder[] memory) {
    Feeder[] memory list = new Feeder[](feeders.length);
    for (uint256 i = 0; i < feeders.length; i++) {
      list[i] = Feeder(feeders[i], fed[feeders[i]]);
    }
    return list;
  }

  function feed(uint256 amount) public {
    require(owner != address(0), "Must be active");
    require(amount > 0, "Amount must be greater than 0");
    totaldFed += amount;
    if (fed[msg.sender] == 0) {
      fed[msg.sender] = amount;
      feeders.push(msg.sender);
    } else {
      fed[msg.sender] += amount;
    }
