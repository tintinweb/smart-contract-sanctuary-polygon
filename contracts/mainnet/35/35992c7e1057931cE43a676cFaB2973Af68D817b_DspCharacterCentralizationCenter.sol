...
contract DspCharacterCentralizationCenter is LuxOnSuperOperators, LuxOnLive, LuxOnData {
    ...
    ) LuxOnData(dataAddress) LuxOnSuperOperators(operator, luxOnAdmin) LuxOnLive(luxOnService) {}

    function deposit(address from, uint256 _tokenId) external isLive {
    ...
    function depositMany(address from, uint256[] memory _tokenIds) external isLive {
        address dspCharacterAddress = getDataAddress(dspCharacter);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(
    ...
    function withdraw(address to, uint256 _tokenId) external isLive {
        address dspCharacterAddress = getDataAddress(dspCharacter);
        address previousOwner = Erc721RealOwnerData(getDataAddress(erc721RealOwnerData)).getRealOwner(dspCharacterAddress, _tokenId);
        require(
    ...
    function withdrawMany(address to, uint256[] memory _tokenIds) external isLive {
        address dspCharacterAddress = getDataAddress(dspCharacter);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            address previousOwner = Erc721RealOwnerData(getDataAddress(erc721RealOwnerData)).getRealOwner(dspCharacterAddress, _tokenIds[i]);
            require(
    ...
    function mintDspCharacter(address to, CharacterDecentralizationInfo memory _characterDecentralizationInfo) external isLive {
    ...
    function mintDspCharacters(address to, CharacterDecentralizationInfo[] memory _characterDecentralizationInfo) external isLive {
        require(to == msg.sender || isSuperOperator(msg.sender), "Not real owner this token id");
        address dspCharacterAddress = getDataAddress(dspCharacter);
