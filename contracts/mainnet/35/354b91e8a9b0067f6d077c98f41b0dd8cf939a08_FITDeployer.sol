...
import {FITToken} from "../token/FITToken.sol";
import {FITDistributionVaultVesting} from "../distribution/FITDistributionVaultVesting.sol";
...
// import {FITDistributionVaultMerkle} from "../distribution/FITDistributionVaultMerkle.sol";
// import {FITDistributionVaultVesting} from "../distribution/FITDistributionVaultVesting.sol";
// import {FITStaking} from "../defi/FITStaking.sol";
// import {FITFarming} from "../defi/FITFarming.sol";
// import {FITMove} from "../defi/FITMove.sol";
...
contract FITDeployer {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeERC20Upgradeable for FITToken;

    FITToken public immutable token;
    IERC20Upgradeable public immutable pair;

    address public immutable pool;
    uint256 public immutable tokenAmount;
    uint256 public immutable pairAmount;
    address public immutable airdropVault;
    address public immutable clientsVault;
    address public immutable incentivesVault;
    address public immutable staking;
    address public immutable farming;
    address public immutable move;
    address public immutable moveVault;

    constructor(
        address[] memory addresses,
        uint256[] memory amounts,
        bytes32 airdropMerkleRoot,
        string memory airdropUri
    ) {
        // TOKEN
        {
            bytes memory initializeCalldata = abi.encodeWithSignature("initialize(address)", addresses[0]);
            address tokenAddress = _createProxy(addresses[4], initializeCalldata);
            token = FITToken(tokenAddress);
            pair = IERC20Upgradeable(addresses[1]);
        }

        // POOL PREP
        {
            AggregatorV3Interface aggregatorInterface = AggregatorV3Interface(addresses[3]);

            uint256 pairToUsdPrice18 =
                uint256(aggregatorInterface.latestAnswer()) * (10 ** (18 - aggregatorInterface.decimals()));
            pairAmount = pair.balanceOf(addresses[0]);
            uint256 pairAmount18 = pairAmount * (10 ** (18 - IERC20MetadataUpgradeable(addresses[1]).decimals()));
            tokenAmount = (pairAmount18 * pairToUsdPrice18) / amounts[4];
        }

        // POOL CREATION
        {
            pair.safeTransferFrom(addresses[0], address(this), pairAmount);
            pair.approve(addresses[2], pairAmount);
            token.approve(addresses[2], tokenAmount);
            IUniswapV2Router01 _router = IUniswapV2Router01(addresses[2]);
            ...
            pool = factory.getPair(address(token), addresses[1]);
            IERC20Upgradeable lpToken = IERC20Upgradeable(pool);
            uint256 lpBalance = lpToken.balanceOf(addresses[0]);
            require(lpBalance == liquidity, "Liquidity mismatch");
        }

        // AIRDROP
        {
            bytes memory initializeData = abi.encodeWithSignature(
                "initialize(address,address,bytes32,string)", addresses[0], token, airdropMerkleRoot, airdropUri
            );
            airdropVault = _createProxy(addresses[5], initializeData);
        }

        // CLIENTS
        {
            bytes memory initializeData = abi.encodeWithSignature(
                "initialize(address,address,bytes32,string)", addresses[0], token, bytes32(0), ""
            );
            clientsVault = _createProxy(addresses[5], initializeData);
        }
        ...
        // MOVEVAULT
        {
            ...
            moveVault = _createProxy(addresses[5], initializeData);
        }
        ...
        // INCENTIVES
        {
            ...
            incentivesVault = _createProxy(addresses[6], initializeData);
        }
        ...
        // STAKING
        {
            ...
            staking = _createProxy(addresses[7], initializeData);
        }

        // FARMING
        {
            bytes memory initializeData =
                abi.encodeWithSignature("initialize(address,address,address)", addresses[0], incentivesVault, pool);
            farming = _createProxy(addresses[8], initializeData);
        }
        ...
        // MOVE
        {
            ...
            move = _createProxy(addresses[9], initializeData);
        }

        // DISTRIBUTION
        {
            FITDistributionVaultVesting incentivesVaultInterface = FITDistributionVaultVesting(incentivesVault);

            uint256[] memory allocations0 = new uint256[](1);
            allocations0[0] = 100 ether;

            incentivesVaultInterface.addAccount(staking, allocations0);

            uint256[] memory allocations1 = new uint256[](2);
            allocations1[0] = 20 ether;
            allocations1[1] = 80 ether;

            incentivesVaultInterface.addAccount(farming, allocations1);
            incentivesVaultInterface.transferOwnership(addresses[0]);
        }

        // TRANSFERS
        {
            token.safeTransfer(airdropVault, amounts[0]);
            token.safeTransfer(clientsVault, amounts[1]);
            token.safeTransfer(incentivesVault, amounts[2]);
            token.safeTransfer(moveVault, amounts[7]);
            token.burn(amounts[3]);

            uint256 left = token.balanceOf(address(this));

            token.safeTransfer(addresses[0], left);
        }
    }
...
import {DistributionVaultAllocationUpgradeable} from
    "dappcomposer-contracts-upgradeable/distribute/DistributionVaultAllocationUpgradeable.sol";
import {VestingUpgradeable} from "dappcomposer-contracts-upgradeable/distribute/utils/VestingUpgradeable.sol";
import {Ownable2StepUpgradeable} from "openzeppelin-contracts-upgradeable/access/Ownable2StepUpgradeable.sol";
import {UUPSUpgradeable} from "openzeppelin-contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";

contract FITDistributionVaultVesting is
    ...
    UUPSUpgradeable
{
    ...
    function initialize(address owner_, address token_, uint256 totalAmount_, uint256 startDate_, uint256 endDate_)
        public
        initializer
    {
        __DistributionVaultAllocationUpgradeable_init(token_);
        __VestingUpgradeable_init(1, totalAmount_, startDate_, endDate_);
        __Ownable2Step_init();
    ...
    // solhint-disable-next-line
    function _authorizeUpdateAllocation() internal view override onlyOwner {}

    function _releasedRange(uint256 from, uint256 to)
        internal
        view
        override(DistributionVaultAllocationUpgradeable, VestingUpgradeable)
        returns (uint256)
    {
