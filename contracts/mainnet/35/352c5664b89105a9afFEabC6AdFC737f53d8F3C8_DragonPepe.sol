...
contract DragonPepe is Ownable, ReentrancyGuard, ERC20 {
    using SafeERC20 for IERC20;

    uint256 public immutable devTaxBuy;
    uint256 public immutable devTaxSell;
    uint256 public immutable devTaxTransfer;

    uint256 public immutable marketingTaxBuy;
    uint256 public immutable marketingTaxSell;
    uint256 public immutable marketingTaxTransfer;

    uint256 public immutable reflectionTaxBuy;
    uint256 public immutable reflectionTaxSell;
    uint256 public immutable reflectionTaxTransfer;

    uint256 public immutable denominator;

    bool public isDevTaxBuyEnabled;
    bool public isDevTaxSellEnabled;
    bool public isDevTaxTransferEnabled;

    bool public isMarketingTaxBuyEnabled;
    bool public isMarketingTaxSellEnabled;
    bool public isMarketingTaxTransferEnabled;

    bool public isReflectionTaxBuyEnabled;
    bool public isReflectionTaxSellEnabled;
    bool public isReflectionTaxTransferEnabled;
    ...
    constructor() ERC20("DragonPepe", "DGNPEPE") {
        //_transferOwnership(0x8B2044DB6196b7FCB6d144Ac7B29B4c1829E3575);
        ...
        isDevTaxBuyEnabled = true;
        isDevTaxSellEnabled = true;
        isDevTaxTransferEnabled = true;

        isMarketingTaxBuyEnabled = true;
        isMarketingTaxSellEnabled = true;
        isMarketingTaxTransferEnabled = true;

        isReflectionTaxBuyEnabled = true;
        isReflectionTaxSellEnabled = true;
        isReflectionTaxTransferEnabled = true;

        devWallet = 0x5Ae31C5Af631C297A1C2CE2E23f9c4Cda6354DD0;
        marketingWallet = 0xCB390d1cfbDcB9C29F16729412C92Dc7349F4405;
    ...
    function toggleBuyTax(
        bool _devTaxBuyStatus,
        bool _marketingTaxBuyStatus,
        bool _reflectionTaxBuyStatus
    ) external onlyOwner {
        require(
            isDevTaxBuyEnabled != _devTaxBuyStatus ||
                isMarketingTaxBuyEnabled != _marketingTaxBuyStatus ||
                isReflectionTaxBuyEnabled != _reflectionTaxBuyStatus,
            "Buy Tax already on that state"
        );
        ...
        emit UpdateBuyTax(
            _devTaxBuyStatus,
            _marketingTaxBuyStatus,
            _reflectionTaxBuyStatus
        );
    }
    ...
    function _transfer(
        ...
        if (
            ...
            isSwapBackEnabled &&
            !reflecting &&
            devTokenAmount + marketingTokenAmount + reflectionTokenAmount > 0
        ) {
            swapBack();
        }
        ...
        if (takeFee) {
            ...
            if (_isAutomatedMarketMakerPair[from]) {
                if (isDevTaxBuyEnabled) {
                    tempDevAmount = (amount * devTaxBuy) / denominator;
                }
                if (isMarketingTaxBuyEnabled) {
                    tempMarketingAmount =
                        (amount * marketingTaxBuy) /
                        denominator;
                }
                if (isReflectionTaxBuyEnabled) {
                ...
                if (isReflectionTaxTransferEnabled) {
                    tempReflectionAmount =
                        (amount * reflectionTaxTransfer) /
                        denominator;
                }
            }
    ...
    function swapBack() internal inSwap {
        ...
        uint256 newBalance = address(this).balance - contractTokenBalance;

        uint256 devBNB = (newBalance * devTokenAmount) / totalTax;
        uint256 marketingBNB = (newBalance * marketingTokenAmount) / totalTax;
        uint256 reflectionBNB = newBalance - devBNB - marketingBNB;
