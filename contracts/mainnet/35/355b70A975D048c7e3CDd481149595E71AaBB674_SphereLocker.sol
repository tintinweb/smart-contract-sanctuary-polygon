...
contract SphereLocker is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function modifyWhitelist(address _account, bool _whitelisted) external onlyOwner {
    whitelist[_account] = _whitelisted;
    emit WhitelistModified(_account, _whitelisted);
  }

  function updatePenaltyReceiver(address _penaltyReceiver) external onlyOwner {
    penaltyReceiver = _penaltyReceiver;
    emit PenaltyReceiverUpdated(_penaltyReceiver);
  }

  /**
   * @notice  Updates the capped supply.
   * @dev     Updates the capped supply and emits the CappedSupplyUpdated event.
   * @param   _cappedSupply  amount of capped supply increase
   */
  function updateCappedSupply(uint256 _cappedSupply) external onlyOwner {
    cappedSupply = _cappedSupply;
    emit CappedSupplyUpdated(_cappedSupply);
  }
  ...
  function _lock(address _account, uint256 _amount) internal notBlacklisted(msg.sender, _account) {
    ...
    require(lockedSupply.add(_amount) < cappedSupply, "capped supply reached");
  ...
  function _checkpointEpoch() internal {
    ...
    if (nextEpochDate < currentEpoch) {
      ...
      while (nextEpochDate != currentEpoch) {
        ...
        nextEpochDate = nextEpochDate.add(REWARDS_DURATION);
  ...
  function _processExpiredLocks(
    ...
    uint256 expiryTime = _checkDelay == 0 && _relock
      ? block.timestamp.add(REWARDS_DURATION)
  ...
  /**
   * @notice Withdraws all currently locked tokens at a penalty.
   */
  function rageQuit() external {
    getReward(msg.sender, false);
    _forceLockExpire(msg.sender);
  }

  function _forceLockExpire(address _account) internal updateReward(_account) {
    LockedBalance[] storage locks = userLocks[_account];
    Balances storage userBalance = balances[_account];
    uint112 locked;
    uint256 length = locks.length;
    uint256 expiryTime = block.timestamp.add(REWARDS_DURATION);
    require(length > 0, "no locks");
    require(!isShutdown, "is shutdown");
    ...
    require(locked > 0, "no early locks");
    ...
    emit WithdrawnWithPenalty(_account, locked);

    //send staking token after reward slashing back
    uint256 _penaltyAmount = locked / (2);
    stakingToken.safeTransfer(_account, _penaltyAmount);
    stakingToken.safeTransfer(penaltyReceiver, locked - (_penaltyAmount));
  }
  ...
  //    uint256 upcomingEpoch = block.timestamp.add(REWARDS_DURATION).div(REWARDS_DURATION).mul(REWARDS_DURATION);
  //    uint256 i = len - 1;
  ...
  //        // e.g. week 1 + 17 <= 18
  //        else if (prevCkpt.epochStart + LOCK_DURATION <= upcomingEpoch) {
  //          ckpts.push(
  //            DelegateeCheckpoint({
  ...
  function balanceAtEpochOf(uint256 _epoch, address _user) public view returns (uint256 amount) {
    ...
    for (uint256 i = locksLength; i > 0; i--) {
      uint256 lockEpoch = uint256(locks[i - 1].unlockTime).sub(LOCK_DURATION);
