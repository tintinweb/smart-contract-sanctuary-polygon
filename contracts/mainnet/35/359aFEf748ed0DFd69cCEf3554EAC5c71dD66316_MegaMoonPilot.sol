...
contract MoonToken is BEP20('Megamoon Token', 'Moon') {
...
interface IMigratorPilot {
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to MegaMoon LP tokens.
    // MegaMoon must mint EXACTLY the same amount of MegaMoon LP tokens or
    // else something bad will happen. Traditional MegaMoon does not
...
contract MegaMoonPilot is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 moonPerBlock; // MOON tokens created per block.
    }
    ...
    IMigratorPilot public migrator;
    ...
    function add(IBEP20 _lpToken, bool _withUpdate, uint256 _moonPerBlock) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update the given pool's MOON allocation point. Can only be called by the owner.
    function set(uint256 _pid, bool _withUpdate, uint256 _moonPerBlock) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
        poolInfo[_pid].moonPerBlock = _moonPerBlock;
    }

    // Set the migrator contract. Can only be called by the owner.
    function setMigrator(IMigratorPilot _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function pendingMoon(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 moonReward = multiplier.mul(pool.moonPerBlock);
    ...
    // Deposit LP tokens to MegamoonPilot for MOON allocation.
    function deposit(uint256 _pid, uint256 _amount) public {

        require (_pid != 0, 'deposit MOON by staking');
    ...
    // Withdraw LP tokens from MegamoonPilot.
    function withdraw(uint256 _pid, uint256 _amount) public {

        require (_pid != 0, 'withdraw MOON by unstaking');
    ...
    // Stake MOON tokens to MegamoonPilot
    ...
    // Withdraw MOON tokens from STAKING.
    ...
    function emergencyWithdrawMoon(uint256 _amount) public onlyOwner {
        safeMoonTransfer(msg.sender, _amount);
    }
    ...
    function safeMoonTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > moonBal) {
            moon.transfer(_to, moonBal);
        } else {
            moon.transfer(_to, _amount);
        }
    }
