...
contract RandomGenerator is VRFConsumerBaseV2, Ownable {
  ...
  uint constant VRF_CALL_DELAY = 600; // min delay between vrfCalls for same lotteryId in case of failed fulfill response
  ...
  function setDrawEndTime(uint lotteryId) external {
    require(lotteryData[lotteryId].endTime == 0, "Already set.");
    lotteryData[lotteryId].endTime = block.timestamp;
  }
