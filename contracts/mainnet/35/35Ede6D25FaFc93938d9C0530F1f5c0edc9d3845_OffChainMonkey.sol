...
// @title: OffChainMonkey
// @author: 5tudio9 adapted from huuep 
//
// On chain PFP collection of 10k unique OnChain Monkey Sister profile images with the following properties:
//   - a single Ethereum-Polygon transaction created everything
...
//   - all 10,000 OffChain Monkeys are unique
...
//   - "OffChain Monkey Generation Sister" is in homage of, not affiliated with, OnChain Monkey or MetaGood
//   - 30% of all OCMSIS secondary royalties will be donated to MetaGood
// And, yes we did say Monkeys!
...
// Bring on the OffChain Monkeys! 143 to DY, AT, BT and OCM/MG Team for all they do! 
contract OffChainMonkey is ERC721Enumerable, ReentrancyGuard, Ownable {
  using Strings for uint256;

  uint256 public constant maxSupply = 10000;
  uint256 public numClaimed = 0;
  string[] private background = ["62a","dbc","ea4","1cb","684","9ff","38a","c9f"]; // only trait that is uniform, no need for rarity weights
  ...
  uint8[] private fur_w =[247, 245, 224, 140, 117, 113, 94, 91, 88, 85, 75, 73, 56, 47, 40, 31, 29, 13, 9];
  string[] private eyes = ["abe","0a0","653","888","be7","abe","0a0","653","888","be7","cef","abe","0a0","653","888","be7","cef","abe","0a0","653","888","be7","cef"];
  uint8[] private eyes_w = [246, 122, 108, 100, 80, 77, 71, 67, 62, 58, 56, 51, 50, 48, 44, 38, 35, 33, 30, 22, 15, 11, 8];
  ...
  uint8[] private mouth_w = [249, 170, 82, 78, 55, 50, 37, 33, 31, 30, 28, 27, 26, 23, 22, 18, 17, 15, 13, 12, 11, 10, 10, 9, 8, 8, 7, 7, 6, 5, 5, 4, 3];
  string[] private earring = ["999","fe7","999","999","fe7","bdd"];
  uint8[] private earring_w = [250, 34, 28, 17, 16, 8, 6];
  ...
  uint8[] private clothes_w = [252, 54, 44, 43, 39, 38, 35, 34, 33, 31, 31, 31, 31, 30, 30, 29, 29, 29, 28, 27, 27, 27, 26, 25, 24, 22, 21, 20, 19, 19, 19, 19, 19, 19, 18, 17, 16, 15, 13, 12, 11, 10, 8, 7];
  ...
  uint8[] private hat_w = [250, 68, 45, 43, 41, 39, 36, 35, 34, 34, 31, 29, 27, 26, 26, 25, 25, 25, 24, 23, 21, 19, 19, 18, 16, 16, 15, 13, 12, 12, 11, 11, 11, 10, 9, 9, 8];
  string[] private z = ['<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 500 500"><rect x="0" y="0" width="500" height="500" style="fill:#',
     '"/><rect width="230" height="140" x="135" y="385" style="fill:#', '"/><circle cx="190" cy="440" r="5" style="fill:#', '"/><circle cx="310" cy="440" r="5" style="fill:#',
    '"/><circle cx="115" cy="250" r="38" style="fill:#', '"/><circle cx="115" cy="250" r="18" style="fill:#', '"/><circle cx="385" cy="250" r="38" style="fill:#',
    '"/><circle cx="385" cy="250" r="18" style="fill:#', '"/><circle cx="250" cy="250" r="135" style="fill:#', '"/><circle cx="250" cy="250" r="115" style="fill:#',
    '"/><circle cx="202" cy="222" r="31" style="fill:#fff"/><circle cx="302" cy="228" r="29" style="fill:#fff"/><circle cx="202" cy="227" r="18" style="fill:#',
    '"/><circle cx="298" cy="227" r="18" style="fill:#', '"/><circle cx="202" cy="227" r="6.3" style="fill:#000"/><circle cx="298" cy="227" r="6.3" style="fill:#000"/>',
    ...
    '</svg>'];
  string private cross='<rect x="105" y="268" width="10" height="40" style="fill:#872"/><rect x="95" y="278" width="30" height="10" style="fill:#872"/><rect x="385" y="268" width="10" height="40" style="fill:#872"/><rect x="375" y="278" width="30" height="10" style="fill:#872"/>';
  string private clo1='<rect width="230" height="115" x="135" y="385" style="fill:#';
  string private clo2='"/><rect width="38" height="42" x="185" y="420" style="fill:#';
  string private hh1='<rect width="144.5" height="85" x="177.75" y="51" style="fill:#';
  string private hh2='"/><rect width="144.5" height="32" x="177.75" y="104" style="fill:#';
  string private sl1='<rect x="158" y="197" width="183.5" height="30" style="fill:#';
  string private sl2='"/><rect x="167.5" y="179.5" width="165" height="45" style="fill:#';
  string private mou='<line x1="289" y1="321" x2="311" y2="355" style="stroke:#000;stroke-width:5"/>';
  string private ey1='<rect x="168" y="200" width="65" height="12" style="fill:#';
  string private ey2='"/><rect x="275" y="200" width="60" height="12" style="fill:#';
  string private ey3='<rect x="160" y="240" width="180" height="24" style="fill:#';
  string private zz='"/>';
  string private ea1='<circle cx="110" cy="283" r="13" style="fill:#';
  string private ea2='<circle cx="390" cy="283" r="13" style="fill:#';
  string private ea3='fe7';
  string private ea4='999';
  string private ea5='"/><circle cx="110" cy="283" r="4" style="fill:#000"/>';
  string private ea6='"/><circle cx="390" cy="283" r="4" style="fill:#000"/>';
  string private ea7='<circle cx="110" cy="283" r="11" style="fill:#';
  string private ea8='<circle cx="390" cy="283" r="11" style="fill:#';
  string private ea9='bdd';
  string private mo1='<line x1="';
  string private mo2='" y1="288" x2="';
  string private mo3='" y2="289" style="stroke:#000;stroke-width:1"/>';
  string private mo4='" y1="285" x2="';
  string private mo5='" y2="286" style="stroke:#000;stroke-width:1"/>';
  ...
  string private co1=', ';
  string private rl1='{"name": "OffChain Monkey #';
  string private rl3='"}';
  ...
  // this was used to create the distribution of 10,000 and tested for uniqueness for the given parameters of this collection
  ...
  function randomOne(uint256 tokenId) internal view returns (Ape memory) {
    ...
    ape.eyes = usew(eyes_w,random(string(abi.encodePacked(ra2,tokenId.toString())))%1432);
    ape.mouth = usew(mouth_w,random(string(abi.encodePacked(ra3,tokenId.toString())))%1109);
    ape.earring = usew(earring_w,random(string(abi.encodePacked(ra4,tokenId.toString())))%359);
    ape.clothes = usew(clothes_w,random(string(abi.encodePacked(ra5,tokenId.toString())))%1331);
    ape.hat = usew(hat_w,random(string(abi.encodePacked(ra6,tokenId.toString())))%1116);
    if (tokenId==7403) {
  ...
  function genMouth(uint8 h) internal view returns (string memory) {
    ...
    if ((h>24) || ((h>8) && (h<16))) {
      for (i=0;i<7;i++) {
        out = string(abi.encodePacked(out,mo1,(214+i*12).toString(),mo2,(214+i*12).toString(),mo3));
      }
      for (i=0;i<6;i++) {
        out = string(abi.encodePacked(out,mo1,(220+i*12).toString(),mo4,(220+i*12).toString(),mo5));
      }
    }
    if (h>15) {
  ...
  function genEarring(uint8 h) internal view returns (string memory) {
    ...
    if (h<3) {
      if (h>1) {
        return string(abi.encodePacked(ea1,ea3,ea5,ea2,ea3,ea6));
      } 
      return string(abi.encodePacked(ea1,ea4,ea5,ea2,ea4,ea6));
    }
    if (h>3) {
      if (h>5) {
        return string(abi.encodePacked(ea7,ea9,zz,ea8,ea9,zz));
      } 
      if (h>4) {
        return string(abi.encodePacked(ea7,ea3,zz,ea8,ea3,zz));
      } 
      return string(abi.encodePacked(ea7,ea4,zz,ea8,ea4,zz));
    }
    return cross;
  }
  ...
  constructor() ERC721("OffChainMonkey", "OCMSIS") Ownable() {}
}