    ...
    OwnableUpgradeable
{
    ...
    mapping(bytes32 => Trade) public releasedTrades;
        ...
        nonReentrant
    {
        ...
        bytes32 tradeIndex = keccak256(
            abi.encodePacked(nftContract, tokenId, item.seller, item.buyer)
        );
    ...
    {
        TradeOffer memory offer = idToOffers[_offerId];
        if (offer.offerId != _offerId) revert Errors.NotExistOffer();
        if (offer.fulfilledAt > 0) revert Errors.NotActiveOffer();
        MarketItem memory item = idToMarketItems[offer.marketItemId];
        if (offer.offerMaker != _msgSender() && item.seller != _msgSender())
            revert Errors.NotOfferMakerOrReceiver();
        delete offer;
        idToOffers[_offerId] = offer;
        if (_openOffers.contains(_offerId)) _openOffers.remove(_offerId);
    ...
    function _cancelMultiOffersByItemId(bytes32 _itemId) internal {
        ...
        while (index < _openOffers.length()) {
            bytes32 offerId = _openOffers.at(index);
    ...
    function fetchReleasedTradeByIndex(bytes32 index)
        external
        view
        override
