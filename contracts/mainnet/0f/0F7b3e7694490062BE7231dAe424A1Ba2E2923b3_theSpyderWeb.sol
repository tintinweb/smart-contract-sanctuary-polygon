...
contract theSpyderWeb {
    using SafeMath for uint256;
    IERC20 public dai = IERC20(0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063);
    address public defaultRefer = 0x5D7a23A1D7d6e1346e114f01C070c39a4F836B89;
    ...
    uint256 private constant guessingFundPercents = 100; 
    ...
    address private guessingReward; 
    address private registerationFee; 
    ...
    constructor(address _feeReceiver, address _guessing, address _jackpot, address _addr, address _register) {
        startTime = block.timestamp;
        poolsLastDistributed = block.timestamp;
        lotteryLastDistributed = block.timestamp;
        FeeReceiver = _feeReceiver;
        guessingReward = _guessing;
    ...
    function register(address _ref) public {  
        ...
        require(userInfo[_ref].referrer != address(0) || _ref == defaultRefer, "Invalid Referrer");
        require(userInfo[_ref].isActive == true || _ref == defaultRefer, "referrer inactive");
    ...
    function _upgrade(address _user, uint256 amount, uint256 _type) private {
        ...
        dai.transfer(guessingReward, amount.mul(guessingFundPercents).div(baseDivider));
        dai.transfer(FeeReceiver, amount.mul(feePercents).div(baseDivider));
        lotteryPool = lotteryPool.add(amount.mul(lotteryPoolPercents).div(baseDivider));
        rewardInfo[growth].growth = rewardInfo[growth].growth.add(amount.mul(200).div(baseDivider));
    ...
    function transferByGrowth(address _to, uint256 _amount) public {
        ...
        if(msg.sender != growth) {
            require(userInfo[msg.sender].isActive == true, "inactive account");
            require(_amount >= 50e18 && _amount.mod(50e18) == 0, "invalid amount");
        }
        ...
        if(msg.sender != growth) {
            ...
            charge = isChargeable ? _amount.mul(1000).div(baseDivider) : 0;
        }
    ...
    function getWeeklyBusiness(address _user, uint256 _amount) public view returns(uint256) {
        ...
        for(uint256 i = 0; i < teamUsers[_user][0].length; i++){
            ...
            for(uint256 k=user.weeklyBusiness.length; k>0; k--) {
                if(block.timestamp.sub(user.weeklyBusiness[k-1].time) <= weekTimeStep) {
