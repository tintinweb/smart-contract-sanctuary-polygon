  ...
  IStaking
{
  ...
  function totalTokenStaked() external view returns(uint256 total){
    for(uint16 i=0; i<lockNumber; ++i){
  ...
  function locked(uint16 _lockIndex) external view returns(
    uint128 lockPeriodInDays,
    uint64 apy_d2,
    uint64 feeInPercent_d2,
    uint256 tokenStaked,
  ...
  function setMin(uint64 _minStaking) external onlyOwner {
