...
contract A4AMarket is Ownable, IERC721Receiver {
    ...
    event ItemSold(uint256 tokenId, address buyer, address seller, uint256 price, address author);
    ...
    function listItem(uint256 itemID, uint256 price) external {
        require( a4A_items.ownerOf(itemID) == _msgSender(), "Sender is not the owner." );
    ...
    function delistItem(uint256 itemID) external {
        require(_listedITemsOwner[itemID] == _msgSender(), "Sender is not the owner.");
    ...
    function buyItem(uint256 itemID) external payable {
        require(_listedITemsPrice[itemID] > 0, "Item is not for sale (price = 0).");
        require(_listedITemsOwner[itemID] != address(0), "Item is not for sale (address = 0).");
        ...
        emit ItemSold(itemID, a4A_items.ownerOf(itemID), seller, _itemPrice, a4A_items.author(itemID));
        _removeSaleData(itemID);

    }

    function updateItemPrice(uint256 itemID, uint256 newPrice) external {
        require(_listedITemsOwner[itemID] == _msgSender() || owner() == _msgSender(), "The authorship can only be changed by the owner.");
