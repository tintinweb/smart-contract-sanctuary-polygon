...
contract TidePoolFactory {
    ...
    function deploy(address _pool, int24 _tickWindow) public returns (address validatedPool) {
        validatedPool = FactoryValidator.validate(uniswapFactory, _pool, _tickWindow);
        getTidePool[validatedPool] = address(new TidePool(IUniswapV3Pool(validatedPool), _tickWindow, treasury));
    }
}
...
library FactoryValidator {
    function validate(address _uniswapFactory, address _pool, int24 _tickWindow) public view returns (address validatedPool) {
        require(_tickWindow >= 10 && _tickWindow <= 50,"TW");
