...
contract Fraxferry {
   ...
   event Embark(address, uint, uint, uint);
   event Disembark(uint, uint, bytes32);
   event Depart(uint,uint,uint,bytes32);
   event RemoveBatch(uint);
   event DisputeBatch(uint);
   event Cancelled(uint, bool);
   event Pause(bool);
   event SetOwner(address);
   event SetCaptain(address);   
   event SetFirstOfficer(address);
   event SetCrewmember(address,bool);
   event SetFee(uint);
   event SetMinWaitPeriods(uint,uint);
   ...
   function embark(uint amount) public notPaused {
      ...
      emit Embark(msg.sender,amount,amountAfterFee,block.timestamp);
   ...
   function depart(uint start, uint end, bytes32 hash) external notPaused isCaptain {
      require ((batches.length==0 && start==0) || (batches.length>0 && start==batches[batches.length-1].start+1),"Wrong start");
   ...
   function setFirstOfficer(address newFirstOfficer) external isOwner {
      ...
      emit SetCaptain(newFirstOfficer);
   }    
