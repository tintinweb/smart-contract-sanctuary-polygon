...
contract Staking is Ownable, Lockable {
    ...
    event ActiveSeted(uint256 indexed stakeId, bool indexed value, address operator);
    ...
    event RewardPerBlockSeted(uint256 indexed stakeId, uint256 indexed newRewardPerBlock, address indexed operator);
    event StakeCreated(
        uint256 indexed stakeId,
    ...
    event Withdrawed(uint256 indexed stakeId, uint256 amount, address indexed operator);
    event TokensStaked(uint256 indexed stakeId, uint256 amount, address indexed operator);
    event Harvested(uint256 indexed stakeId, uint256 amount, address indexed operator);
    event RewardWithdrawed(uint256 indexed stakeId, uint256 amount, address indexed operator);
    ...
    function depositRewards(uint256 stakeId, uint256 amount) external onlyOwner {
        ...
        emit RewardsDeposited(stakeId, stake.rewardToken, amount, _msgSender());
    }
    ...
    function emergencyWithdrawal(
        ...
        emit EmergencyWithdrawals(stakerAddress, stakeId, payHarvest, _msgSender());
    }

    /*
    withdraw only reward
  */
    function harvest(uint256 stakeId) external withLock updateReward(stakeId, _msgSender()) {
        _withdrawHarvest(_stakes[stakeId], _stakers[stakeId][_msgSender()]);
    }

    function stakeTokens(uint256 stakeId, uint256 amount) external withLock updateReward(stakeId, _msgSender()) {
    ...
    function setActive(uint256 stakeId, bool value) external onlyOwner {
        ...
        emit ActiveSeted(stakeId, value, _msgSender());
    }
    ...
    function setRewardPerBlock(uint256 stakeId, uint256 _newRewardPerBlock) public onlyOwner {
        ...
        emit RewardPerBlockSeted(stakeId, _newRewardPerBlock, _msgSender());
    }
     ...
     **/
    function withdraw(uint256 stakeId) external withLock updateReward(stakeId, _msgSender()) {
        Stake storage stake = _stakes[stakeId];
        Staker storage staker = _stakers[stakeId][_msgSender()];
        _withdrawHarvest(stake, staker);
        _withdrawLP(stake, staker);
    ...
    function withdrawalRewards(uint256 stakeId, uint256 amount) external onlyOwner {
        ...
        emit RewardWithdrawed(stakeId, amount, _msgSender());
    }
    ...
    function _stake(
        ...
        emit TokensStaked(stake.id, amount, staker.stakerAddress);
    }
    ...
    function _withdrawLP(Stake storage stake, Staker storage staker) internal {
        ...
        if (_msgSender() != owner()) {
            require(
                block.timestamp >= staker.startTime + stake.lpLockTime || block.number > stake.lastRewardedBlock,
        ...
        emit Withdrawed(stake.id, amount, staker.stakerAddress);
    }
    ...
    function _withdrawHarvest(Stake storage stake, Staker storage staker) internal {
        ...
        emit Harvested(stake.id, reward, staker.stakerAddress);
    }
    ...
    modifier updateReward(uint256 stakeId, address stakerAddress) {
        ...
        stake.lastUpdateBlock = block.number < stake.lastRewardedBlock ? block.number : stake.lastRewardedBlock;
