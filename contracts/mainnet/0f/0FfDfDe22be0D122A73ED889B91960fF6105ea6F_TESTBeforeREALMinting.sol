...
abstract contract NFTStandard is ERC721Enumerable, ERC721Burnable, ERC721Pausable, IERC2981, Ownable, ReentrancyGuard {
    ...
    // @dev +1 when minting period ends or when tokens' sold out
    Counters.Counter private saleCount;

    // @dev manage token ID
    Counters.Counter private tokenIdTracker;

    // @dev check if tokens are sold out
    ...
    */
    function clientIdBatch(uint256[] calldata _tokenIds) external view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            id = _tokenIds[i];
            require(_exists(id), string(abi.encodePacked("NFTStandard: token id #", id.toString(), " is not minted")));
            clientIds[i] = tokenIdClientId[id];
        }
        return clientIds;
    }
    ...
    */
    ...
    function setMintingCondition(MintingConditionStruct calldata _mintingCondition) external onlyOwner {
        ...
        bool saleIsSet = (mintingCond._maximumAmountPerAddress != 0);
        // when in sale
        if (saleIsSet && inRange(block.number, mintingCond._blockRange) && !tokensSoldOut) {
            mintingCond._blockRange._endId = _mintingCondition._blockRange._endId;
            mintingCond._whitelistMerkleRoot = _mintingCondition._whitelistMerkleRoot;
        } else {
        ...
        emit MintingConditionSet(mintingCond);
    }
    ...
    /**
     * @notice The minted tokens belong to msg.sender. The remaining coin after minting is refunded.
    ...
    function mint(uint256 _amountPerAddress, uint256[] calldata _clientIds, bytes32[] calldata _merkleProof) external payable nonReentrant {
        ...
        uint256 tokenAccumulation = whoMintedHowMany[saleCount.current()][_msgSender()] + _amountPerAddress;
        ...
        require(tokenAccumulation <= mintingCond._maximumAmountPerAddress, "NFTStandard: it exceeds the maximum token amount per address");
        if (mintingCond._whitelistMerkleRoot != reservedMerkleRoot) {
        ...
        for (uint256 i = 0; i < _clientIds.length; i++) {
            require(inRange(_clientIds[i], mintingCond._clientIdRange), string(abi.encodePacked("NFTStandard: invalid client id #", _clientIds[i].toString())));
            require(clientIdSet.add(_clientIds[i]), string(abi.encodePacked("NFTStandard: client id #", _clientIds[i].toString(), " is already minted")));
        }
        ...
        for(uint256 counter = 0; counter < _amountPerAddress; counter ++) {
            ...
            tokenIdClientId[tokenId] = _clientIds[counter];
        }

        whoMintedHowMany[saleCount.current()][_msgSender()] = tokenAccumulation;
...
contract TESTBeforeREALMinting is NFTStandard {
