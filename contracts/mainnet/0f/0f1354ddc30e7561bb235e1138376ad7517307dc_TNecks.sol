...
// File: contracts/TNecks.sol

pragma solidity ^0.8.2;

contract TNecks is ERC20, AccessControl, ERC721Holder {
    ...
    constructor(address owner, address _collectionAddress) ERC20("TallNecks Coin", "TNecks") AccessControl(){
        ...
        coinAmountPerRewardUnit = 25 * 10 ** 18; // 10 ERC20 coins per rewardUnit, may be changed later on
        numberOfBlocksPerRewardUnit = 20571; // 12 hours per reward unit , may be changed later on
        welcomeBonusAmount = 250 * 10 ** 18; // 200 tokens welcome bonus, only paid once per tokenId
    }
    ...
    function setCollectionAddress(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "SPCC: update to zero address not possible");
