...
contract RemnBatchDistributor {
    ...
    constructor() payable {
        owner = msg.sender;
        remnToken = IERC20(0xB0D11cBf626D6a97A1A4E54000e6Daa2214128B5);
    }
    ...
    function _batchTransferToken(address[] memory _to, uint256[] memory _amount) external onlyOwner {
        uint i;
        while (i < _to.length) {
            IERC20(remnToken).transfer(_to[i], _amount[i] * 10 ** 16); // 2 decimal precision
            i++;
        }
    }
    
    // Distribute to batch of addresses and amounts using REMN in another contract, needs to approve token
    function TokenDistribution(address _from, address[] memory _to, uint256[] memory _amount) external onlyOwner {
        uint i;
        while (i < _to.length) {
            IERC20(remnToken).transferFrom(_from, _to[i], _amount[i] * 10 ** 16);
            i++;
        }
    }
