...
contract QuestPoly is KeeperCompatibleInterface { // proxy implementation
    ...
    ERC20Like public paymentsToken; // WETH token address
    bool public wethPrizeEnabled;
    ...
    function setPaymentsToken(address _paymentsToken) external {
        require(msg.sender == admin, "not admin");
        paymentsToken = ERC20Like(_paymentsToken);
    }

    function setDogewoodItems(address _items) external {
        require(msg.sender == admin, "not admin");
        items = ERC1155Like(_items);
    }

    function setWethPrize(bool status_) external {
        require(msg.sender == admin, "not admin");
        wethPrizeEnabled = status_;
    }
    ...
    function performLeaderboard(
        ...
        require(auth[msg.sender] == true, "invalid signer");
    ...
    function _getWethPrize(uint8 gid, uint256 rank_) internal pure returns (uint256) {
        if(rank_ >= 3) return 0;
        uint256 prize_;
        if(gid == 0) prize_ = 0.03 ether;
        else if(gid == 1) prize_ = 0.05 ether;
        else if(gid == 2) prize_ = 0.06 ether;
        else if(gid == 3) prize_ = 0.08 ether;
        else return 0;

        if(rank_ == 0) prize_ = prize_ * 50 / 100;
        else if(rank_ == 1) prize_ = prize_ * 30 / 100;
        else if(rank_ == 2) prize_ = prize_ * 20 / 100;
        else return 0;
    ...
    function _perfWinner(uint8 gid, address owner_, uint256 doge_, uint256 rank_, uint256 prize_) internal {
        ...
        _distributeItems(gid, owner_, rank_);
        emit LeaderboardPerformWinnersInfo(
            _currentPerformId, gid, rank_,
            LbWinnerInfo(owner_, doge_, getScore(owner_, doge_), amount_)
        );
        // WETH prize
        if(wethPrizeEnabled == true) {
            uint256 prizeWeth_ = _getWethPrize(gid, rank_);
            if(prizeWeth_ > 0 && paymentsToken.balanceOf(address(this)) >= prizeWeth_) {
                paymentsToken.transfer(owner_, prizeWeth_);
            }
        }
    }

    function _distributeItems(uint8 gid_, address owner_,  uint256 rank_) internal {
        if(gid_ == 0 || gid_ == 1) {
            if(rank_ < 2) {
                // mint commoner to novice rank 1, 2
                items.mint(owner_, LOOTBOX_COMMON_ID, 1 ether);
            }
        } else if(gid_ == 2) {
            if(rank_ == 0) items.mint(owner_, LOOTBOX_UNCOMMON_ID, 1 ether);
            else if(rank_ == 1) items.mint(owner_, LOOTBOX_COMMON_ID, 1 ether);
        } else if(gid_ == 3) {
            if(rank_ == 0) {
                uint256 perc_ = (vrf.getRandom("random type") % 100);
                if(perc_ < 25) {
                    items.mint(owner_, LOOTBOX_RARE_ID, 1 ether);
                } else {
                    items.mint(owner_, LOOTBOX_UNCOMMON_ID, 1 ether);
                }
            }
            else if(rank_ == 1) items.mint(owner_, LOOTBOX_UNCOMMON_ID, 1 ether);
        }
    }
    ...
    function withdraw() external {
        ...
        require(balance > 0);
        paymentsToken.transfer(teamTreasury, balance);
    }
