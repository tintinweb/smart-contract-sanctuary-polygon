...
// 5/30號  時間：15:00 空投一次(空投人工操作)
// 隨機
// 空投地址一:0x6a7841B79958FE0758B26734920d3b1E0e50caAb
// 數量：150

// 空投地址二:0x8A2023223B776C299Ec1abEa85851Ce832e9091B
// 數量：150

// 空投地址三：0x5809426f4700e104976BDA3a3547f85C292f091A
// 數量：30

// 6/5號18:00～6/6號21:00第一次白名單：價格:130顆matic
// 數量:100個白名單
...
// 公售。6/10號 時間：18:00～不截止  價格：195顆matic

// 一個地址只能mint一次，數量最多6隻

// [V]  原先的手動提款改mint直接分好比例直接進錢包

// [V] 收款歸集錢包地址80%的：0xf9D8760A6E38961137fF328A6a77eb11B165dC4A

// [V] 收款歸集錢包地址20%的：0x86ba69336848e15a5808D873E3888f54Ad0c78fb

// OpenSea版稅地址：0x8A482e203a827038F10cD85D1676c95A6d001169
...
contract mfContract is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Jhchg", "Jinhe") {
        setBaseURI("https://jhchg.net/metadata/");
        setBlindBaseURI("https://jhchg.net/metadata/blind.json");
    }

    string baseURI;
    string public blindBaseURI;
    string public baseExtension = ".json";
    // 解盲。6/20號  時間：18:30
    uint256 public unBlindTime = 1655721000;

    //首次白名单价，二次白名单价，开售价
    uint256 public firstWlCost = 130000000000000000000; //130 matic
    uint256 public secondWlCost = 160000000000000000000;
    uint256 public finalCost = 195000000000000000000;

    //时间设定
    // 5/30號15:00  1653894000
    // 6/5 號18:00  1654423200
    // 6/6 號21:00  1654520400
    // 6/7 號21:30  1654608600
    // 6/8 號21:30  1654695000
    // 6/10號18:00  1654855200
    ...
    uint256 public adminKeep = 330;

    //不重复随机指数
    uint256[] public indices = new uint256[](maxSupply);
    ...
    //收款钱包，不可修改
    address public mainPayoutAddress =
        0xf9D8760A6E38961137fF328A6a77eb11B165dC4A;
    address public secPayoutAddress =
        0x86ba69336848e15a5808D873E3888f54Ad0c78fb;
    ...
    function randomIndex() private returns (uint256) {
        uint256 totalSize = maxSupply - indicesNonce;
        ...
        isAllowed(msg.sender)
    {
        ...
        uint256 mainPay = (balance * 8) / 10;
        uint256 secPay = balance - mainPay;

        payable(mainPayoutAddress).transfer(mainPay);
        payable(secPayoutAddress).transfer(secPay);
    }
        ...
        returns (string memory)
    {
        ...
        if (block.timestamp < unBlindTime) {
            return blindBaseURI;
        } else {
    ...
    //官方预铸，可指定给某个地址，数量受到adminKeep限制，全随机
    ...
    //设定盲盒URI
    function setBlindBaseURI(string memory _blindBaseURI) public onlyOwner {
        blindBaseURI = _blindBaseURI;
    }
    ...
    //修改收款人
    // function setMainPayoutAddress(address _payoutAddress) public onlyOwner {
    //     mainPayoutAddress = _payoutAddress;
    // }
    // function setSecPayoutAddress(address _payoutAddress) public onlyOwner {
    //     secPayoutAddress = _payoutAddress;
    // }
    //收款提出
