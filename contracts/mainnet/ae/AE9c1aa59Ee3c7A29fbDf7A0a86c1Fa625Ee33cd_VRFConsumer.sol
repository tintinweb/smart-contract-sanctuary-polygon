...
contract VRFConsumer is VRFConsumerBaseV2 {
  ...
  // BSC LINK token contract. For other networks,
  // see https://docs.chain.link/docs/vrf-contracts/#configurations
  ...
  function getSeed(uint256 _s) external returns(uint256){
    ...
    req_times += 1;
    uint256 salt = uint256(keccak256(abi.encode(block.difficulty, block.timestamp, req_times)));
    uint256 s;
