...
library SolarswapV2OracleLibrary {
        ...
        )
    {
        ...
        price0Cumulative = ISolarswapPair(pair).price0CumulativeLast();
        price1Cumulative = ISolarswapPair(pair).price1CumulativeLast();
        ...
        ) = ISolarswapPair(pair).getReserves();
...
library SolarswapV2LiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = SolarswapV2Library.getReserves(
            factory,
            tokenA,
            tokenB
        );
        ...
        require(
            ...
            "SolarswapV2ArbitrageLibrary: ZERO_PAIR_RESERVES"
        );
        ...
        if (aToB) {
            uint256 amountOut = SolarswapV2Library.getAmountOut(
                amountIn,
                reserveA,
                reserveB
            );
    ...
    function getLiquidityValue(
        ...
        (uint256 reservesA, uint256 reservesB) = SolarswapV2Library.getReserves(
            factory,
            tokenA,
            tokenB
        );
        ISolarswapPair pair = ISolarswapPair(
            SolarswapV2Library.pairFor(factory, tokenA, tokenB)
        );
        bool feeOn = ISolarswapFactory(factory).feeTo() != address(0);
...
library SolarswapV2Library {
    ...
    {
        require(tokenA != tokenB, "SolarswapV2Library: IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB
            ? (tokenA, tokenB)
            : (tokenB, tokenA);
        require(token0 != address(0), "SolarswapV2Library: ZERO_ADDRESS");
    }
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            hex"ec57b935131361c30e3d934ae25e79a3f0e696005090e3dcf3ee6c13c5f67eab" // init code hash
                        )
                    )
                )
            )
    ...
    function quote(
        ...
        require(amountA > 0, "SolarswapV2Library: INSUFFICIENT_AMOUNT");
        require(
            reserveA > 0 && reserveB > 0,
            "SolarswapV2Library: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, "SolarswapV2Library: INSUFFICIENT_INPUT_AMOUNT");
        require(
    ...
    function getAmountIn(
        ...
        require(
            amountOut > 0,
            "SolarswapV2Library: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        require(
            reserveIn > 0 && reserveOut > 0,
            "SolarswapV2Library: INSUFFICIENT_LIQUIDITY"
    ...
    function getAmountsOut(
        ...
        require(path.length >= 2, "SolarswapV2Library: INVALID_PATH");
...
// library with helper methods for oracles that are concerned with computing average prices
library SolarswapOracleLibrary {
    using FixedPoint for *;
...
library SolarswapLiquidityMathLibrary {
    using SafeMath for uint256;
    ...
    function getReservesAfterArbitrage(
        ...
        // first get reserves before the swap
        (reserveA, reserveB) = SolarswapLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        ...
        require(
            reserveA > 0 && reserveB > 0,
            "SolarswapArbitrageLibrary: ZERO_PAIR_RESERVES"
        );
        ...
        // now affect the trade to the reserves
        if (aToB) {
            uint256 amountOut = SolarswapLibrary.getAmountOut(
                amountIn,
                reserveA,
                reserveB
            );
    ...
    function getLiquidityValue(
        ...
        (uint256 reservesA, uint256 reservesB) = SolarswapLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        ISolarswapPair pair = ISolarswapPair(
            SolarswapLibrary.pairFor(factory, tokenA, tokenB)
        );
        bool feeOn = ISolarswapFactory(factory).feeTo() != address(0);
        uint256 kLast = feeOn ? pair.kLast() : 0;
        uint256 totalSupply = pair.totalSupply();
...
contract SolarswapRouter is ISolarswapRouter {
    using SafeMath for uint256;
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "Solarswap: INSUFFICIENT_B_AMOUNT"
                );
                ...
                require(
                    ...
                    "Solarswap: INSUFFICIENT_A_AMOUNT"
                );
        ...
        returns (uint256 amountA, uint256 amountB)
    {
        ...
        (amountA, amountB) = tokenA == token0
            ? (amount0, amount1)
            : (amount1, amount0);
        require(amountA >= amountAMin, "Solarswap: INSUFFICIENT_A_AMOUNT");
        require(amountB >= amountBMin, "Solarswap: INSUFFICIENT_B_AMOUNT");
    }
        ...
        returns (uint256[] memory amounts)
    {
        ...
        require(amounts[0] <= amountInMax, "Solarswap: EXCESSIVE_INPUT_AMOUNT");
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            SolarswapLibrary.pairFor(factory, path[0], path[1]),
    ...
    function swapExactETHForTokens(
        ...
        returns (uint256[] memory amounts)
    {
        require(path[0] == WETH, "Solarswap: INVALID_PATH");
    ...
    function swapTokensForExactETH(
        ...
        returns (uint256[] memory amounts)
    {
        require(path[path.length - 1] == WETH, "Solarswap: INVALID_PATH");
        amounts = SolarswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, "Solarswap: EXCESSIVE_INPUT_AMOUNT");
        TransferHelper.safeTransferFrom(
            path[0],
        ...
        returns (uint256[] memory amounts)
    {
        require(path[0] == WETH, "Solarswap: INVALID_PATH");
        amounts = SolarswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, "Solarswap: EXCESSIVE_INPUT_AMOUNT");
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(
            IWETH(WETH).transfer(
                SolarswapLibrary.pairFor(factory, path[0], path[1]),
