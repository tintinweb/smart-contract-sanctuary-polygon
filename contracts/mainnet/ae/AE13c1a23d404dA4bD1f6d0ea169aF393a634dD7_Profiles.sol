...
library Helpers {
    ...
    function _validateUsername(string memory _username) internal pure {
        ...
        if (!_validateUsernameBySymbol(byteUsername))
            revert Errors.InvalidSymbols();
    }
    ...
    ERC721EnumerableUpgradeable
{
    ...
    function createProfileWithApprove(
        DataTypes.CreateProfileData memory args,
        bytes memory sig
    ) external payable {
        if (msg.value != profilePrice)
            revert Errors.InsufficientFunds(profilePrice);
        if (
            !_signerVerification.isMessageVerified(
                ...
                string(
                    abi.encodePacked(
                        Helpers._addressToString(args.to),
                        args.username
                    )
                )
            )
    ...
    function _createProfile(DataTypes.CreateProfileData memory args) private {
