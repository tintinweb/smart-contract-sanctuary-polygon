...
interface IAdminSales {
    ...
    function addOpenSale(bytes calldata options) external;
    function addWhiteListSale(bytes calldata options) external;
 ...
 *
 */
abstract contract App is UpgradeableApp, ERC1967Upgrade, Initializable, ERC2771Context {
...
contract ERC721BaseSale is App {
    ...
    event SetAdminSales(address admin);
    event SetTimeOfSale(uint256 start, uint256 end);


    modifier onlyAdminSales() {
        require(_msgSender()==adminSales, Exceptions.INVALID_AUTHORIZATION_ERROR);
        _;
    }

    modifier saleTime(){
        require(block.timestamp >= start && block.timestamp <= end, Exceptions.SALE_TIME_ERROR);
        _;
    }

    constructor(address trustedForwarder) ERC2771Context(trustedForwarder) {}

    function setAdminSales(address newAdminSales) public onlyAdminSales {
        _setAdminSales(newAdminSales);
    }

    function _setAdminSales(address newAdminSales) internal {
        adminSales = newAdminSales;
        emit SetAdminSales(adminSales);
    }

    function setTimeOfSale(uint256 start_, uint256 end_) public onlyAdminSales {
        start = start_;
        end = end_;
        emit SetTimeOfSale(start, end);
    }

    function setPaymentPolicy(bytes32[] memory tierValues, uint256[] memory tierPrices_)
        public
        onlyAdminSales()
    {
    ...
    function setClaimLimit(uint64 claimLimit_) public onlyAdminSales() {
        claimLimit = claimLimit_;
        emit SetClaimLimit(claimLimit);
    }

    function setTierPerWalletLimits(bytes32[] memory tierValues, uint256[] memory tierLimits)
        public
        onlyAdminSales()
    {
    ...
    function setTotalClaimsLimits(uint64 totalClaims) public onlyAdminSales() {
    ...
    function setActive(bool isActive_) public onlyAdminSales() {
        isActive = isActive_;
...
import "../AdminSales/IAdminSales.sol";
...
contract ERC721WhitelistSale is ERC721BaseSale, __with_semver(uint8(1), uint8(0), uint8(1)) {
    ...
    function initialize(IKernel _kernel, address _adminSales, bytes calldata options) external initializer {
        __App_init(_kernel);
        (uint64 claimLimit_, uint64 totalClaimsLimit,
        uint256 start_, uint256 end_, bool isActive,
        bytes32 merkleRoot, bytes memory merkleProofIpfsHash_,
        bytes32[] memory tierValues, uint256[] memory tierPrices_, uint256[] memory tierLimits) = abi.decode(options,
            (uint64, uint64,
        ...
        _setAdminSales(_adminSales);
        setActive(isActive);
        setClaimLimit(claimLimit_);
        setTotalClaimsLimits(totalClaimsLimit);
        setPaymentPolicy(tierValues, tierPrices_);
        setTimeOfSale(start_, end_);
        setTierPerWalletLimits(tierValues, tierLimits);
        _setMerkleTree(merkleRoot, merkleProofIpfsHash_);

    }

    function setMerkleTree(bytes32 merkleRoot, bytes memory merkleProofIpfsHash_)
        external
        onlyAdminSales()
    {
        _setMerkleTree(merkleRoot, merkleProofIpfsHash_);
    ...
    function buy(bytes32[] calldata proof, bytes32 tierValue) external saleTime payable {
        require(_merkleRoot != bytes32(0), Exceptions.INVALID_INITIALIZATION_ERROR);

        uint256 cost = tierPrices[tierValue];
        require(msg.value >= cost, Exceptions.VALIDATION_ERROR);
...
//        ERC721Properties(appAddress).mint(_msgSender(), tierValue);
        IAdminSales(adminSales).executeSingle(appAddress, abi.encodeCall(ERC721Properties.mint, (_msgSender(), tierValue)));
...
//        ERC721Properties(appAddress).mint(_msgSender(), tierValue);
    ...
    function _setMerkleTree(bytes32 merkleRoot, bytes memory merkleProofIpfsHash_) internal {
...
library AppsIds {
    ...
    bytes32 constant ADMIN_SALES = keccak256(abi.encodePacked("ADMIN_SALES"));
}
