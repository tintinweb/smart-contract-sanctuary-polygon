...
contract ViriumStake is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    uint private constant REWARD_RATE = 79 * 10 ** 18; //Inflation is about 5%
    uint private constant ONE_YEAR = 365 days;
    uint private constant REWARD_RATE_ANNUAL = REWARD_RATE * ONE_YEAR;
    uint private constant FOUR_YEAR = 4 * ONE_YEAR;

    mapping(address => uint) private userLastUpdateTime;
    mapping(address => uint) public rewards;
    uint public totalSupply;
    mapping(address => uint) private _userShare;
    ...
    function initialize() initializer public {
        ...
        _grantRole(UPGRADER_ROLE, 0x1540602fA43D9b4237aa67c640DC8Bb8C4693dCD);
        _grantRole(MAINTAIN_ROLE, 0x1540602fA43D9b4237aa67c640DC8Bb8C4693dCD);
    ...
    function earned() public view returns (uint) {
        ...
        uint lastUpdateTime = userLastUpdateTime[msg.sender];
        if (currentStatus == Status.Paused) {
            lastUpdateTime = pausedTime;
        }
        if (lastUpdateTime == 0 || totalSupply == 0) {
            return 0;
        }
        return _userShare[msg.sender] * (block.timestamp - lastUpdateTime) * getRewardRate() / totalSupply + rewards[msg.sender];
    }

    function start() external onlyRole(MAINTAIN_ROLE) {
        currentStatus = Status.Running;
    }

    function pause() external onlyRole(MAINTAIN_ROLE) {
        currentStatus = Status.Paused;
        pausedTime = block.timestamp;
    }

    function earnedPerDay() external view returns (uint){
        return _userShare[msg.sender] * 1 days * getRewardRate() / totalSupply;
    }
    ...
    function getApr() external view returns (uint){
        ...
        return REWARD_RATE_ANNUAL * 10000 / totalSupply;
    }
    ...
    function stake(uint256[][] calldata tokenIds) external updateReward {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            stake(tokenIds[i], pledges[i].token, pledges[i].rate);
        }
    }
    ...
    function unstake(uint256[][] calldata tokenIds) external updateReward {
        require(currentStatus == Status.Running, "ViriumStake: Status error");
        for (uint i = 0; i < tokenIds.length; i++) {
            unstake(tokenIds[i], pledges[i].token, pledges[i].rate);
        }
    }
...
interface IViriumId is IERC721Lockable {
    ...
    function softMint() external returns (uint256);
}
