...
contract SVaultCore is SVaultStorage  {
    ...
    function _deposit(
        ...
        // check if user balance >= _amount
        require(_asset().balanceOf(msg.sender) >= _amount, "Not Enough Balance");
    ...
    function claimReward(address _strategy) public returns (uint256) {
        ISingleStrategy strategy = ISingleStrategy(_strategy);
        return strategy.collectRewardTokens();
    }
    ...
    function claimRewards() public returns (uint256) {
        ...
        for (uint256 i = 0; i < allStrategies.length; i++) {
            uint256 reward0 = claimReward(allStrategies[i]);
            totRew0 += reward0;
        }
        return totRew0;
    }
