...
contract BhavishEquitiesPrediction is IBhavishPrediction, AccessControl, Pausable, ReentrancyGuard {
    ...
    function shouldExecuteRound() external view returns (bool) {
        ...
        if (currentRoundId > 1) {
            Round memory roundMinusOne = bhavishPredictionStorage.getPredictionRound(currentRoundId - 1);
            int256 val = (int256(startTimestamp) - int256(roundMinusOne.roundEndTimestamp)) / int256(roundTime);
            if (
                val > 5 &&
                curRound.roundStartTimestamp < block.timestamp &&
                curRound.roundStartTimestamp == startTimestamp &&
                curRound.roundEndTimestamp > block.timestamp
            ) return true;
        }
        return false;
    }
    ...
    function isStartPredictionMarket() private view returns (bool) {
        (uint256 startTimestamp, ) = marketTime.getMarketActiveTime(block.timestamp);
        ...
        int256 val = (int256(startTimestamp) - int256(roundMinusOne.roundEndTimestamp)) / int256(roundTime);
        if (
            // val > 5 signifies the market was active atleast before 5 times the roundTime
            val > 5 &&
            curRound.roundStartTimestamp < block.timestamp &&
            curRound.roundStartTimestamp == startTimestamp &&
            curRound.roundEndTimestamp > block.timestamp &&
    ...
    function closeCurrentRound() private returns (uint256 closingPrice) {
        ...
        if (
            curRound.roundState == RoundState.CREATED ||
            curRound.roundState == RoundState.ENDED ||
            curRound.roundState == RoundState.CANCELLED
        ) return closingPrice;
