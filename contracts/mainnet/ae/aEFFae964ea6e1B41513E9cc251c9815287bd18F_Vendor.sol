...
contract Vendor{
  FlashareGasToken gastoken;
  uint256 public tokensPerMatic = 1000 * 1e18;
  address MainFlashare;
  address owner;
  ...
  constructor(address gastokenadd,address MainFlashareAdd,address newOwner) {
    owner = newOwner;
    MainFlashare=MainFlashareAdd;
    gastoken = FlashareGasToken(gastokenadd);
  }

  function changeTokensPerMatic(uint256 newValue)public {
      require(msg.sender==owner);
      tokensPerMatic = newValue;
  }

  function changeFlashareGasToken(address newGasTokenAdd)public {
      require(msg.sender==owner,"error");
      gastoken = FlashareGasToken(newGasTokenAdd);
  }

  function changeCanBuy(bool value) public {
      require(msg.sender==owner);
      canBuy=value;
  }
    function changeCanSell(bool value) public {
      require(msg.sender==owner);
      canSell=value;
  }

  function buyTokens(uint amount) public payable returns (uint256 tokenAmount) {
    require(canBuy==true);
    require(msg.value > 0, "You need to send some MATIC to proceed");
    uint256 amountMATIC = (amount* 1e18) / tokensPerMatic;
    uint256 vendorBalance = gastoken.balanceOf(address(this));
    require(msg.value>=amountMATIC,"raise the msgvalue");
    require(vendorBalance >= amount, "Vendor has insufficient tokens");
  ...
  function sellTokens(uint256 tokenAmountToSell) public {
    ...
    gastoken.increaseAllowance(msg.sender,address(this),tokenAmountToSell);
    uint256 userBalance = gastoken.balanceOf(msg.sender);
    require(userBalance >= tokenAmountToSell, "You have insufficient tokens");
    uint256 amountOfMATICToTransfer = (tokenAmountToSell* 1e18) / tokensPerMatic;
    uint256 ownerMATICBalance = address(this).balance;
    console.log(ownerMATICBalance);
    console.log(amountOfMATICToTransfer);
    require(ownerMATICBalance >= amountOfMATICToTransfer, "Vendor has insufficient funds");
    (bool sent) = gastoken.transferFrom(msg.sender, address(this), tokenAmountToSell);
