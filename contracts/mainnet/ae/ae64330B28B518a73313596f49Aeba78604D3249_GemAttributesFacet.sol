...
contract GemAttributesFacet is Modifiers, ITokenAttributeActor {

    using MetadataLib for MetadataContract;

    /// @notice get a random value to set an attribute with
    function _getRandomAttributeValue(uint256 salt)
        internal
        view
        returns (uint256)
    {
    ...
    function encodeField(string memory key, string memory value)
        internal
        pure
    ...
    function _getAttributeKeys() internal pure returns (string[] memory attributeKeys) {
        attributeKeys = new string[](10);
        attributeKeys[0] = "cut";
        attributeKeys[1] = "color";
        attributeKeys[2] = "carat";
        attributeKeys[3] = "clarity";
        attributeKeys[4] = "claim_price";
        attributeKeys[5] = "created_on";
        attributeKeys[6] = "maturity";
        attributeKeys[7] = "type";
        attributeKeys[8] = "symbol";
        attributeKeys[9] = "claim_display";
    }

    function _getAttributes() internal view returns (string[] memory taNames, string[] memory taValues) {
        MetadataContract storage metadata = LibAppStorage.diamondStorage().metadata;
        string[] memory attributeKeys = _getAttributeKeys();
        taNames = new string[](attributeKeys.length);
        taValues = new string[](attributeKeys.length);
        for(uint256 i = 0; i < attributeKeys.length; i++) {
            if(keccak256(abi.encodePacked(attributeKeys[i])) == keccak256(abi.encodePacked("type"))) {
                taNames[i] = attributeKeys[i];
                taValues[i] = "gem";
                i++;
                taNames[i] = "claim_display";
                taValues[i] = "none";
            }
            else if (keccak256(abi.encodePacked(attributeKeys[i])) == keccak256(abi.encodePacked("symbol"))) {
                taNames[i] = attributeKeys[i];
                taValues[i] = metadata._symbol;
            }
            taNames[i] = attributeKeys[i];
        }
    }

    function _getTokenAttributes(uint256 tokenId) internal view returns (string[] memory taNames, string[] memory taValues) {
        // get the token image and attributes
        AttributeContract storage attrs = s().attributes;
        string[] memory attributeKeys = _getAttributeKeys();
        taNames = new string[](attributeKeys.length);
        taValues = new string[](attributeKeys.length);
        
        for (uint256 i = 0; i < attributeKeys.length; i++) {
            Attribute memory attrib = AttributeLib._getAttribute(
                attrs,
                tokenId,
                attributeKeys[i]
            );
            taNames[i] = attributeKeys[i];
            taValues[i] = attrib.value;
            if (
                keccak256(bytes("type")) == keccak256(bytes(attrib.key)) &&
                keccak256(bytes("claim")) == keccak256(bytes(attrib.value))
            ) {
                i++;
                taNames[i] = "claim_display";
                taValues[i] = "block";
    ...
    function getTokenAttributes(
        uint256 tokenId
    ) external view override returns (string[] memory taNames, string[] memory taValues) {
        (taNames, taValues) = _getTokenAttributes(tokenId);
    }

    function getContractAttributes() external view override returns (string[] memory taNames, string[] memory taValues) {
        return _getAttributes();
    }

    function _getReplacements(
        string[] memory colorPalette,
        string[] memory taNames,
        string[] memory taValues
    ) internal pure returns (Replacement[] memory replacements) {
        ...
        for(uint256 i = 0; i < taNames.length; i++) {
            replacements[l+i] = Replacement(taNames[i], taValues[i]);
        }
        return replacements;

    }

    function _getColorPalette() internal view returns (string[] memory colorPalette) {
        MetadataContract storage metadata = LibAppStorage.diamondStorage().metadata;
        colorPalette = metadata._imageColors;
        if(colorPalette.length == 0) {
            // create a 16 hue color starting at #444 and ending at #eef
            colorPalette = new string[](16);
            for(uint256 i = 0; i < 16; i++) {
                colorPalette[i] = string(abi.encodePacked("#", Strings.toHexString(i * 0x111111, 6)));
            }
        }
    }

    function getContractReplacements() external view override returns (Replacement[] memory) {
        (string[] memory taNames, string[] memory taVals) = _getAttributes();
        return _getReplacements(_getColorPalette(), taNames, taVals);
    }

    function getColorPalette() external view returns (string[] memory) {
        return _getColorPalette();
    }

    function getColorPaletteReplacements() external view returns (Replacement[] memory) {
        (string[] memory taNames, string[] memory taVals) = _getAttributes();
        return _getReplacements(_getColorPalette(), taNames, taVals);
    }

    function getTokenReplacements(uint256 tokenId) external view override returns (Replacement[] memory) {
        string[] memory palette = _getColorPalette();
        console.log('got color palette');
        (string[] memory taNames, string[] memory taVals) = _getTokenAttributes(tokenId);
        console.log('got token attributes');

        return _getReplacements(palette, taNames, taVals);
    }
    ...
    function initTokenAttributes(
        ...
        AttributeContract storage attrs = s().attributes;
        BitGemContract storage bg = s().bitgems;
        SaltStorage storage salt = s().salts;
        ...
        AttributeLib._setAttribute(
            ...
            Attribute(
                "symbol",
                AttributeType.String,
                bg.settings.tokenDefinition.symbol
            )
        );
        AttributeLib._setAttribute(
            attrs,
...
library MetadataLib {
    ...
    function getContractImage(
        ...
        require(imageAddress != address(0), "no image has been set");   
    ...
    returns (string memory) {
        ...
        string memory externalUrl = string(
            abi.encodePacked(self._externalUri, 'pools/', Strings.toHexString(bitgemAddress))
        );
        string memory json = Base64.encode(
            bytes(
                getTokenMetadata(self, dum, svg, externalUrl)
            )
        );
        ...
        returns (string memory _traitsString)
    {
        ...
        for (uint256 i = 0; i < _traits.length; i++) {
            ...
            console.log("creating trait", traitObj.key, traitObj.value);
    ...
    function getTokenMetadata(
        ...
        string memory externalUrl = bytes(_externalUrl).length > 0
            ? string(
                abi.encodePacked(
                    '"external_url": "',
                    _externalUrl,
    ...
    function contractURI(MetadataContract storage self, string memory svg)
        internal
        view
        returns (string memory)
    {
        Trait[] memory dum;
        string memory externalUrl = string(
            abi.encodePacked(self._externalUri, 'pools/', address(this))
        );
        string memory json = Base64.encode(
            bytes(
                getTokenMetadata(self, dum, svg, externalUrl)
...
abstract contract MultipartData  {
    ...
    {
        MultiPartContract storage ds = LibAppStorage.diamondStorage().multiParts;
