...
contract City is ERC20Burnable, Ownable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 72h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 60000 ether;  // 60000 CITY / (3*(24h * 60min * 60s)) 0.231481
    ...
    /**
     * @notice Constructs the CITY ERC-20 contract.
     */
    constructor() ERC20("CITY", "CITY") {
        // Mints 1 CITY to contract creator for initial pool setup
        _mint(msg.sender, 1 ether);
    }

    function _getCityPrice() internal view returns (uint256 _CityPrice) {
        try IOracle(CityOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("City: failed to fetch CITY price from Oracle");
        }
    }
    function setCityOracle(address _CityOracle) public onlyOwner {
        require(_CityOracle != address(0), "oracle address cannot be 0 address");
        CityOracle = _CityOracle;
    }

    /**
     * @notice Operator mints CITY to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of CITY to mint to
