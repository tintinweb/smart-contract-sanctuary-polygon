...
import "./interfaces/gains/IGNSStakingV6_2.sol";
import "./BaseYA.sol";
import "./libraries/GiddyUniswap.sol";
import "./GiddyConfigYA.sol";

contract GainsGnsYA is BaseYA, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  uint256 constant private COMPOUND_THRESHOLD = 1e18;
  ...
  address constant private DAI_TOKEN = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063;
  address constant private GAINS_POOL = 0xFb06a737f549Eb2512Eb6082A808fc7F16C0819D;
  ...
  function initialize(
    address _trustedForwarder) public initializer {
    __Ownable_init();
    __Pausable_init();
  ...
  function getContractBalance() public view override returns (uint256) {
    (uint stakedTokens,,,,) = IGNSStakingV6_2(GAINS_POOL).users(address(this));
    return stakedTokens;
  }

  function getContractRewards() public view override returns (uint256) {
    (,uint debtDai,,,) = IGNSStakingV6_2(GAINS_POOL).users(address(this));
    return debtDai;
  }
  ...
  function compound() public override whenNotPaused {
    IGNSStakingV6_2(GAINS_POOL).harvest();
    uint256 earnings = IERC20(DAI_TOKEN).balanceOf(address(this));
    if (earnings > COMPOUND_THRESHOLD) {
      earnings = deductFeeDai(earnings, earnings * GIDDY_CONFIG.earningsFee() / BASE_PERCENT);
      earnings = GiddyUniswap.swapTokensSimple(QUICK_ROUTER, DAI_TOKEN, GNS_TOKEN, address(this), earnings);

      emit Compound(earnings, lastCompoundTime, getContractBalance());
      if (!IERC20(GNS_TOKEN).approve(GAINS_POOL, earnings)) {
        revert();
      }
      IGNSStakingV6_2(GAINS_POOL).stakeTokens(earnings);
      lastCompoundTime = block.timestamp;
    }
  }
  ...
  function depositCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    route[1] = DAI_TOKEN;
    route[2] = GNS_TOKEN;
    (amountOut, priceImpact) = GiddyUniswap.calcPrice(QUICK_ROUTER, route, value - fap);
  }
  ...
  function deposit(bytes calldata auth, uint256 fap) external override whenNotPaused nonReentrant {
    ...
    route[1] = DAI_TOKEN;
    route[2] = GNS_TOKEN;
    uint256 valueGns = GiddyUniswap.swapTokens(QUICK_ROUTER, route, address(this), value);

    uint256 shares = contractShares == 0 ? valueGns * INIT_SHARES : valueGns * contractShares / getContractBalance();
    userShares[_msgSender()] += shares;
    contractShares += shares;

    if (!IERC20(GNS_TOKEN).approve(GAINS_POOL, valueGns)) {
      revert("Failed GNS approve");
    }
    IGNSStakingV6_2(GAINS_POOL).stakeTokens(valueGns);
  ...
  function depositGiddyCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    amountOut = outBefore;
    (outBefore, amountOut) = GiddyUniswap.calcSwap(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, outBefore, amountOut);
    (outBefore, amountOut) = GiddyUniswap.calcSwap(QUICK_ROUTER, USDC_TOKEN, DAI_TOKEN, outBefore, amountOut);
    (outBefore, amountOut) = GiddyUniswap.calcSwap(QUICK_ROUTER, DAI_TOKEN, GNS_TOKEN, outBefore, amountOut);
  ...
  function withdrawCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    route[1] = DAI_TOKEN;
    route[2] = USDC_TOKEN;
    (amountOut, priceImpact) = GiddyUniswap.calcPrice(QUICK_ROUTER, route, sharesToValue(shares));
    amountOut -= fap;
  }
  ...
  function withdraw(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant {
    ...
    IGNSStakingV6_2(GAINS_POOL).unstakeTokens(value);

    address[] memory route = new address[](3);
    route[0] = GNS_TOKEN;
    route[1] = DAI_TOKEN;
    route[2] = USDC_TOKEN;
    value = GiddyUniswap.swapTokens(QUICK_ROUTER, route, address(this), value);

    value = deductFeeUsdc(value, fap);
    if (!IERC20(USDC_TOKEN).transfer(_msgSender(), value)) {
      revert("Failed USDC transfer");
    }
  ...
  function withdrawGiddyCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    amountOut = outBefore;
    (outBefore, amountOut) = GiddyUniswap.calcSwap(QUICK_ROUTER, GNS_TOKEN, DAI_TOKEN, outBefore, amountOut);
    (outBefore, amountOut) = GiddyUniswap.calcSwap(QUICK_ROUTER, DAI_TOKEN, USDC_TOKEN, outBefore, amountOut);
    (outBefore, amountOut) = GiddyUniswap.calcSwap(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, outBefore, amountOut);
    priceImpact = outBefore * BASE_PERCENT / amountOut;
    if (priceImpact > BASE_PERCENT) {
      priceImpact -= BASE_PERCENT;
    }
  ...
  function deductFeeUsdc(uint256 amount, uint256 fap) internal returns (uint256) {
    if (fap > 0) {
      if (!IERC20(USDC_TOKEN).transfer(GIDDY_CONFIG.feeAccount(), fap)) {
        revert("Failed USDC fee transfer");
      }
    }
    return amount - fap;
  }
  ...
  function deductFeeDai(uint256 amount, uint256 fap) internal returns (uint256) {
    if (fap > 0) {
      GiddyUniswap.swapTokensSimple(SUSHI_ROUTER, DAI_TOKEN, USDC_TOKEN, GIDDY_CONFIG.feeAccount(), fap);
    }
    return amount - fap;
  }
}
...
interface IGNSStakingV6_2 {
  ...
  function users(address account) external view returns (uint256 stakedTokens, uint256 debtDai, uint256 stakedNftsCount, uint256 totalBoostTokens, uint256 harvestedRewardsDai);
}
