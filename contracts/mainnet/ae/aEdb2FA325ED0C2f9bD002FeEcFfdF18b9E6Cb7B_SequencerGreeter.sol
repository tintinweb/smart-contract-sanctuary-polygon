  ...
  ERC165
{
  ...
  function fetchSequencerHandler() external override view returns (address) {
    return address(sequencerHandler);
  }
  
  function fetchErcHandler() external override view returns (address) {
    return address(erc20Handler);
  }
  
  function fetchReserveHandler() external override view returns (address) {
  ...
  function routerSend(Params memory _params)
    internal
    isLinkUnSet(_params._destChainID)
    returns (bool, bytes32)
  {
    _manageTransfers(_params);

    uint64 nonce = sequencerHandler.genericDepositWithERC(
  ...
  function _manageTransfers(Params memory _params) internal{
    (uint8 destinationChainID, , , , , address feeTokenAddress) = 
              abi.decode(_params._erc20, (uint8, bytes32, uint256[], address[], bytes[], address));
    (, uint256 exchangeFee) = erc20Handler.getBridgeFee(destinationChainID, feeTokenAddress);
    
    IERC20(feeTokenAddress).transferFrom(msg.sender, address(this), exchangeFee);
    IERC20(feeTokenAddress).approve(reserveHandler, exchangeFee);

    this.tranferAmt(_params._swapData);
  }

  function tranferAmt(bytes calldata data) external {
      (uint256 srcTokenAmount, , , , , uint256 lenRecipientAddress, uint256 lenSrcTokenAddress, ) = abi.decode(data, (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256));

      uint256 index = 256; // 32 * 6 -> 8
      ...
      IERC20(srcTokenAddress).transferFrom(msg.sender, address(this), srcTokenAmount);
      IERC20(srcTokenAddress).approve(reserveHandler, srcTokenAmount);
  }
  ...
  function routerReplay(
    ...
    sequencerHandler.replayGenericDepositWithERC(
  ...
  function approveTokens(address _toBeApproved, address _token, uint256 _value) internal {
    IERC20 token = IERC20(_token);
    token.approve(_toBeApproved, _value);
  }
...
interface iSequencerHandler {
  ...
  function replayGenericDepositWithERC(
    uint8 _destChainID,
...
interface iRouterSequencerCrossTalk is IERC165 {
  ...
  /// @notice fetchSequencerHandler This is a public function and fetches the sequencer handler address.
  function fetchSequencerHandler() external view returns (address);

  /// @notice fetchReserveHandler This is a public function and fetches the reserve handler address.
  function fetchReserveHandler() external view returns (address);

  /// @notice fetchErcHandler This is a public function and fetches the erc handler address.
  function fetchErcHandler() external view returns (address);
