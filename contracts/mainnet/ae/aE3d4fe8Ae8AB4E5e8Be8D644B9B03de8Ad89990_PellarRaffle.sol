...
contract PellarRaffle is VRFConsumerBase, Ownable {
  struct Raffle {
    bool seeded;
    uint256 seedNumber;
    uint256 mark;
  ...
  /* VRF */
  function createRandomSeed(uint256 _raffleId) external onlyRoler("createRandomSeed") returns (bytes32 _requestId) {
  ...
  function getRaffleEntries(
    ...
    uint256 _start,
    uint256 _end
  ) public view returns (string[] memory _entries) {
  ...
  function getRaffleWinners(
    ...
    uint256 _start,
    uint256 _end
  ) public view returns (string[] memory _winners) {
    Raffle memory raffle = raffles[_raffleId];
    _end = _end > raffle.winners.length ? raffle.winners.length : _end;
    uint256 size = _end - _start;
    _winners = new string[](size);
    for (uint256 i = 0; i < size; i++) {
  ...
  /* Admin controls */
  function createRaffle(uint256 _raffleId, uint256 _totalWinners) external onlyRoler("createRaffle") {
    require(getWinnersLength(_raffleId) == 0, "Winners drawn");
    raffles[_raffleId].totalWinners = _totalWinners;
  }

  function addEntries(uint256 _raffleId, string[] calldata _addresses) external onlyRoler("addEntries") {
  ...
  function setEntries(uint256 _raffleId, string[] memory _addresses) external onlyRoler("setEntries") {
    Raffle storage raffle = raffles[_raffleId];
    require(getWinnersLength(_raffleId) == 0, "Winners drawn");
  ...
  function drawWinners(uint256 _raffleId, uint256 _amount) external onlyRoler("drawWinners") {
    Raffle storage raffle = raffles[_raffleId];
    ...
    for (uint256 i = 0; i < _amount; i++) {
      ...
      string memory winner = raffle.entries[index];
  ...
  function withdraw() external onlyRoler("withdraw") {
    // withdraw matic
    uint256 balance = address(this).balance;
    payable(msg.sender).transfer(balance);
  }

  function withdrawLink() external onlyRoler("withdrawLink") {
