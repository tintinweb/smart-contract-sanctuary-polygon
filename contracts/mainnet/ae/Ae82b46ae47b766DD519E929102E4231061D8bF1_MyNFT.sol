...
contract MyNFT is ERC1155LazyMint {
    ...
    function verifyClaim(address _claimer, uint256 _tokenId, uint256 _quantity) public view virtual override {
      ...
      require(currentSupply+_quantity<= getMaxSupply(_tokenId), "Not enough supply exists please try a lower number" );
    }

    function getMaxSupply(uint256 _tokenId) public pure returns(uint256){
        if(_tokenId == 0)
            return TYPE_1000_SUPPLY_MAX;
        if(_tokenId == 1)
            return TYPE_2500_SUPPLY_MAX;
        if (_tokenId == 2)
            return TYPE_5000_SUPPLY_MAX;
        if(_tokenId == 3)
            return TYPE_10000_SUPPLY_MAX;
        return 0;
    }
        ...
        require(caller != address(0), "FROM_ZERO_ADDR");

        address operator = msg.sender;

        _beforeTokenTransfer(operator, caller, address(this), asSingletonArray(_tokenId), asSingletonArray(_amount), "");

        uint256 fromBalance = balanceOf[caller][_tokenId];
        require(fromBalance >= _amount, "INSUFFICIENT_BAL");
        unchecked {
            balanceOf[caller][_tokenId] = fromBalance - _amount;
        }

        emit TransferSingle(operator, caller, address(0), _tokenId, _amount);
    }
