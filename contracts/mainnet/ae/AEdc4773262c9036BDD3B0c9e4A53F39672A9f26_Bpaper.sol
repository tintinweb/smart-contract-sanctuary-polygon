...
abstract contract MAKEPaper is Ownable{
    address public designContractAddr = 0x01134e78B410a96ECdcA7be595156C87D203bd6b;

    function master(string memory _title, string memory _category, uint _editTime, string memory _content, uint _tkId, string memory _ethName) 
...
abstract contract DesignCard {
  ...
  function buildMetadata(string memory _title, string memory _category, uint _editTime, string memory _content, uint _tkId, string memory _ethName) 
...
abstract contract CheckQua is Ownable{
    address BcardContractAddr = 0xc6Dd0F44910eC78DAEa928C4d855A1a854752964;
    address BpaperContractAddr = 0xc6Dd0F44910eC78DAEa928C4d855A1a854752964;
...
contract Bpaper is ERC1155, Ownable {
  ...
  address BcardContractAddr=0xc6Dd0F44910eC78DAEa928C4d855A1a854752964;
  address CheckQuaAddr=0xd94d65bB29f994b2a7166af8b7F69F873ab6c8e0;
  address MAKEPaperAddr=0xdf01bA3833ecaB5816faEe6f4a3b23DE46A5e7c5;
  address AdAddr = 0x175284aF8E36CC01bf54943Ef06E5488352Abd45;
  ...
  function mintNew(string memory _Title,string memory _Category,string memory _Content, uint _Amount, address cardholder) public payable {  
    ...
    require(_Amount<=NMaxPaper,">Max paper amount");
  ...
  function mintAdd(address _to, uint256 _ID, uint256 _Amount) public payable {    
    if (msg.sender != owner() && msg.sender != AdAddr) {
      require(msg.value >= cost*_Amount,"sent less than price");
        CheckQua checkCon = CheckQua(CheckQuaAddr);
      require(cStr(checkCon.checkConAddPaper(_ID,msg.sender, _Amount),"Match"),
        checkCon.checkConAddPaper(_ID,msg.sender, _Amount));
    }
    totalPaper[_ID]+=_Amount;  
  ...
  function buildMetaData(uint256 _BpaperID) public payable {
    if (_BpaperID !=0){  
      ...
      address BpaperMinter=cPaper.minter; 
      MAKEPaper MAKEPaperContract = MAKEPaper(MAKEPaperAddr);
      tokenURI[_BpaperID]= MAKEPaperContract.master(cPaper.title,cPaper.category,cPaper.editTime,cPaper.content,
  ...
  function updateContent(uint _PaperID, string memory _title, string memory _category,string memory _content) public {
    ...
    if (msg.sender != owner() && msg.sender != AdAddr) {
      require (msg.sender == cPaper.minter);
    }
    cPaper.title = _title;
    cPaper.category = _category;
    cPaper.editTime = block.timestamp;
  ...
  function checkNewPaperforMint(uint _ID) view public returns (uint){
    CheckQua checkCon = CheckQua(CheckQuaAddr);
    return checkCon.checkNewPaperforMint(_ID);
  }

  function setCostNStartCard(uint256 _newCost, uint256 _newNMaxPaper) public onlyOwner {
    cost = _newCost * 10000000000000000 wei; //0.01eth
    NMaxPaper = _newNMaxPaper;
  }
  ...
  function updateContracts(address _CheckQua,address _MAKEPaper,address _BCard, address _Admin) public onlyOwner {
