...
contract AHoney is Ownable {
    ...
    uint public constant HNYPrice = 100;

    bool public isRedeemLocked = true;
    uint public aHNYReserve = 50000 ether;
    uint public HNYReserve;

    uint constant maxaHNY = 50000 ether;
    uint public constant maxCap = 50 ether;
    ...
    */
    constructor(address _honey, address _dai, address[] memory _recipients, uint[] memory _amount) {
        HONEY = IERC20(_honey);
        DAI = IERC20(_dai);
        require(_recipients.length == _amount.length, "lists not the same size");

        for(uint i = 0; i < _recipients.length; i++) {

            address user = _recipients[i];
            uint quantity = (_amount[i] * 1 ether) / HNYPrice;

            aHNYBalance[user] += quantity;
            totalaHNYPurchased[user] += quantity;
            aHNYReserve -= quantity;
    ...
    */
    function buy(uint quantity, bytes32[] calldata proof) external onlyIfLocked onlyWhitelisted(proof) {
    ...
    */
    function redeem(uint amount) external onlyIfUnlocked {
    ...
    function aHNYBalanceOf(address _addr) external view returns(uint) {
        return aHNYBalance[_addr];
    }

    function totalHNYRedeemed(address _addr) external view returns(uint) {
    ...
    */
    function SetHoneyAddress(address _honey) external onlyOwner {

        require(_honey != address(0), "Zero Address");
    ...
    */
    function HNYDeposit(uint amount) external onlyOwner {
    ...
    * Requires the sender to be the owner of this contract
    */
    function setRedeemLock(bool _value) external onlyOwner {
