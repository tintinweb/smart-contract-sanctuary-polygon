...
contract Suit3Storage is ERC2771Recipient {
    ...
    event SendMessage(address indexed owner, address[] to, string cid, string cidKeys, uint256 expire);

    // Event when a file is stored
    event StoreFile(address indexed owner, address[] to, string cid, string cidKeys, string name, string extension, uint256 size, uint256 expire);
