...
contract RandomnessFarmer is IRandomnessFarmer, VRFConsumerBase, Ownable {
	bytes32 internal keyHash;
	uint256 internal fee;

	mapping(bytes32 => uint256) public requestIdToRandomNumber;
	mapping(address => bool) private isWhitelistedByAccount;
	...
	modifier onlyWhitelist() {
		require(
			isWhitelistedByAccount[msg.sender],
			"Only whitelisted accounts can request randomness"
		);
		_;
	}
		...
		returns (bytes32 requestId)
	{
		require(
			...
			"Insufficient LINK balance for VRF call"
		);
	...
	// Administrative
	function setWhitelist(address account, bool isWhitelisted)
		public
		onlyOwner
	{
		isWhitelistedByAccount[account] = isWhitelisted;
	}
