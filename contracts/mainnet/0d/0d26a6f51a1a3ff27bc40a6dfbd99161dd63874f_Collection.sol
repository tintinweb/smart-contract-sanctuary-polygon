    ...
    ICollection
{
    ...
    function overwriteWhitelists(uint256 _collectionId, uint256[] memory _whitelistIds, WhitelistParams[] memory _whitelistMods) 
    external
    returns (bool)
    {
        require(
        ...
        uint256 paramsLength = _whitelistIds.length;
        WhitelistParams[] memory curWhitelists = _collections[_collectionId].whitelistParams;
        require(
            (paramsLength == _whitelistMods.length) && (paramsLength <= curWhitelists.length),
            throwError(INVALID_FUNCTION_PARAMS)
        );

        for(uint i; i < paramsLength; i++) {
            if(curWhitelists[_whitelistIds[i]].merkleRoot != _whitelistMods[i].merkleRoot)
                _collections[_collectionId].whitelistParams[_whitelistIds[i]].merkleRoot = _whitelistMods[i].merkleRoot;
        
            if(curWhitelists[_whitelistIds[i]].endTime != _whitelistMods[i].endTime)
                _collections[_collectionId].whitelistParams[_whitelistIds[i]].endTime = _whitelistMods[i].endTime;
            
            if(curWhitelists[_whitelistIds[i]].startTime != _whitelistMods[i].startTime)
                _collections[_collectionId].whitelistParams[_whitelistIds[i]].startTime = _whitelistMods[i].startTime;        
