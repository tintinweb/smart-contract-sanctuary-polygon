...
contract Polytank is Ownable, ReentrancyGuard, IERC721, IERC165, IERC721Metadata, IERC721Enumerable {
    ...
    uint256 public MINTING_LIMIT = 10000;
    uint256 public starter = 0;

    uint256 public Minimal_Mint = 0;
...
mapping (uint256 => address) public poolUser;
    ...
    uint256 public mintingPrice = 700 ether;
    uint256 public whitelistOff = 50 ether;
    ...
    // devs
    address private dev1 = 0x879512342c35f5966028251d26c951C815237271;
    ...
    function changeMintlimilt(uint256 amount,uint256 _starter)onlyOwner public{
         indices = new uint256[](amount);
         MINTING_LIMIT = MINTING_LIMIT + amount;
         starter = _starter;
    ...
    function changeWhiteListOff(uint256 _amount)onlyOwner public{
        whitelistOff = _amount;
    }

    function changeMinimalMint(uint256 amount)onlyOwner public{
        Minimal_Mint = amount;
    }
  ...
  function addToWhiteList(address _walletForWhitelist) onlyOwner external  {
    ...
    function getNFTPrice(uint256 amount , address psua,address sender) public view returns (uint256) {
        require(totalSupply() < MINTING_LIMIT, "Sale has already ended.");
        uint256 nftPrice = amount.mul(mintingPrice);
        uint256 calPrice = 0;
        
        if(users[sender].isExist )
        {
             calPrice = nftPrice.sub(whitelistOff.mul(2).mul(amount));

        }
        else if(users[psua].isExist && !users[sender].isExist)
        {
             calPrice = nftPrice.sub(whitelistOff.mul(amount));

        }
        
        else if(!users[sender].isExist && !users[psua].isExist)
        {
           calPrice = nftPrice;
    ...
    function mint(uint256 amount,address psua) external payable nonReentrant mintingEnabled {
        ...
        require(balanceOf(msg.sender) >= Minimal_Mint,"Your total Mint should be bigger than Minimal Mint");

        uint256 nftPrice = getNFTPrice(amount,psua,msg.sender);
        if (msg.value > nftPrice) {
            payable(msg.sender).transfer(msg.value.sub(nftPrice));
        }
        ...
        else if(users[psua].isExist && !users[msg.sender].isExist)
        {
    ...
    function _generateRandomId() private returns (uint256) {
        ...
        if (indices[totalSize - 1] == 0) {
            indices[index- starter] = (totalSize + starter) - 1;    // Array position not initialized, so use position
        } else { 
            indices[index- starter] = indices[(totalSize ) - 1];   // Array position holds a value so use that
        }
        nonce++;
