...
import "./FairTravel.sol";

contract FairBooth is Ownable, VRFConsumerBase {
    Tickets tickets;
    PolyArtifacts artifacts;
    FairTravel fairTravel;
    ...
    mapping(address => mapping(uint8 => uint256)) public runnerScores;
    ...
    constructor(
        ...
        fairTravel = FairTravel(_fairTravel);
        keyHash = _keyHash;
        fee = _fee;
        IERC20(_link).approve(msg.sender, type(uint256).max);
    }

    function getGames(address account) public view returns (Game[] memory) {
        bytes32[] memory ids = gamesByAddress[account];
        Game[] memory list = new Game[](ids.length);
    ...
    function runnerGame(uint256 score) public {
        require(fairTravel.isActive(msg.sender), "You must travel to the fair");
        if (score > 300) {
            require(runnerScores[msg.sender][2] == 0, "Already claimed Gold");
            runnerScores[msg.sender][2] = score;
            artifacts.mint(msg.sender, 52, 1);
        } else if (score > 200) {
            require(runnerScores[msg.sender][1] == 0, "Already claimed Silver");
            runnerScores[msg.sender][1] = score;
            artifacts.mint(msg.sender, 51, 1);
        } else if (score > 100) {
            require(runnerScores[msg.sender][0] == 0, "Already claimed Bronze");
            runnerScores[msg.sender][0] = score;
            artifacts.mint(msg.sender, 50, 1);
        }
    }

    function randomGame(uint256 t) public {
        require(fairTravel.isActive(msg.sender), "You must travel to the fair");
        require(t > 0, "Must have at least one ticket");
        require(
            t <= tickets.balanceOf(msg.sender, 1),
            "Must have enough tickets"
        );
        tickets.burn(msg.sender, 1, t);
        bytes32 requestId = requestRandomness(keyHash, fee);
        games[requestId] = Game(t, msg.sender, false, 0);
    ...
    function claim(bytes32 requestId) public {
        ...
        game.claimed = true;
        tickets.mint(game.player, 1, uint16(game.reward));
    }
    ...
    {
        Game storage game = games[requestId];
        require(!game.claimed, "Game already claimed");
        uint256 seed = randomness % 190;
        game.reward = (game.tickets * seed) / 100;
        emit Play(game.player, game.tickets, seed, game.reward, requestId);
        claim(requestId);
    }
}
...
contract LandCoordinates is Ownable {
  mapping(uint256 => Coordinate) public coordinates;

  function setLandCoordinate(uint256 tokenId, int128 x, int128 y) public onlyOwner {
    coordinates[tokenId] = Coordinate(x, y);
  }

  function setMultiLandCoordinates(uint256[] calldata tokenId, int128[] calldata x, int128[] calldata y) public onlyOwner {
    require(tokenId.length == x.length && tokenId.length == y.length, "Incorrect params");

    for (uint256 i = 0; i < tokenId.length; i++) {
      coordinates[tokenId[i]] = Coordinate(x[i], y[i]);
    }
  }
}
...
contract FoxDenS2 is Ownable, VRFConsumerBase {
  ...
  uint256[] public heistPrices = [150000000, 300000000, 450000000, 600000000, 750000000];
  bool public paused;
...
contract FarmCrops is Handlers {
  ...
  uint256 public timeoutPeriod = 11.75 hours;
  mapping(address=>mapping(address=>bool)) public isApprovedForAll;
...
contract FairTravel {
  ...
  constructor(address _coordinates, address _realtor, address _corn) {
    coordinates = LandCoordinates(_coordinates);
    realtor = Realtor(_realtor);
    corn = Corn(_corn);
  }
  ...
  function abs(int128 v) public pure returns (uint128) {
    return uint128(v > 0 ? v : v * -1);
  }

  function getDistance(uint256 tokenId) public view returns (uint128) {
    (int128 x, int128 y) = coordinates.coordinates(tokenId);
    uint128 distance = 0;
    uint128 ax = abs(x);
    uint128 ay = abs(y);
    if (ax > 2) {
  ...
  function getCost(uint256 tokenId) public view returns (uint256) {
    uint128 distance = getDistance(tokenId);

    return 30 ether + 20 ether * distance;
  }

  function isActive(address account, uint256 offset) public view returns (bool) {
    return block.timestamp <= (timestamp[account] + 24 hours - offset);
  }

  function isActive(address account) public view returns (bool) {
    return isActive(account, 0);
  }

  function travel(uint256 landId, uint256 farmId) public {
    require(!isActive(msg.sender, 15 minutes), "Cooldown");
    require(realtor.tenants(landId, farmId) == msg.sender, "Wrong farm");

    uint256 cost = getCost(landId);
    corn.burn(msg.sender, cost);
