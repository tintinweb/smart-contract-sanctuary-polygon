...
interface IMinterAmm {
    ...
    function addPermissionedUsers(address _userAddress, bool _permissioned) external;
    ...
    MinterAmmStorageV4
{
    ...
    //Emitted when AddressesProvider is updated
    event AddPermissionedUser(address userAddress, bool permissinoed);

    //Emitted when AddressesProvider is updated
    event SetPermissionedAmm(bool permissioned);
    ...
    modifier whiteList() {
        if (isPermissioned == true) {
            require(
                permissionedUsers[msg.sender] == true,
    ...
    function addPermissionedUsers(address _userAddress, bool _permissioned)
        ...
        onlyOwner
    {
        permissionedUsers[_userAddress] = _permissioned;

        emit AddPermissionedUser(_userAddress, _permissioned);
    }

    /**
     * @notice allows the OtokenFactory module to whitelist a new option
     * @dev can only be called from the OtokenFactory address
     * @param _permissioned set to if amm is whitelisted or not
     */
    function setPermissionedAmm(bool _permissioned) external override onlyOwner {
        isPermissioned = _permissioned;

        emit SetPermissionedAmm(isPermissioned);
    }
}
...
abstract contract MinterAmmStorageV4 is MinterAmmStorageV3 {
    ...
    mapping(address => bool) internal permissionedUsers;
}
