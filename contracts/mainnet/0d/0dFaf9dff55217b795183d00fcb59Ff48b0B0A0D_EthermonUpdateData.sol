    ...
    SafeMathEthermon
{
    ...
    */
    ...
    function withdrawAmount(uint256 amount) public {
      ...
      function setRerollPrice(string calldata rarity, uint256 _price_in_emon)
        external onlyModerators
    {
        reRollPriceEmon[rarity] = _price_in_emon;
    }
    ...
    {
        string memory rarity = getClassRarityMatic(_classId);
        if (bytes(rarity).length == 0) revert();
        return reRollPriceEmon[rarity];
    }



 function getMaticRarityString(uint8 index) public view returns(string memory) {
        return maticRarity[index];
    }

     function getEthRarityString(uint8 index) public view returns(string memory) {
        return ethRarity[index];
    }

      function getKey (string[] memory rarityArray, string memory rarity) internal pure returns (int8){
          int8 key = -1;
      for (uint8 i; i< rarityArray.length;i++){
          if (keccak256(abi.encodePacked(rarityArray[i]))==keccak256(abi.encodePacked(rarity))) {
                key = int8(i);
          }
      }
          return key;
    ...
    function getClassRarityMatic(uint32 class_id) public view returns(string memory) {
        return classRarityMatic[class_id];
    }

  function getClassRarityEth(uint32 class_id) public view returns(string memory) {
        return classRarityEth[class_id];
    }

  function assignClassRarityMatic(uint32 class_id, string calldata rarity) external onlyModerators{
  ...
  function assignClassRarityEth(uint32 class_id, string calldata rarity) external onlyModerators{
