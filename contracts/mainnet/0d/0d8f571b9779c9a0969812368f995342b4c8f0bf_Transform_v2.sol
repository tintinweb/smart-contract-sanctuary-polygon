...
contract Transform_v2 is Ownable {
    using SafeMath for uint256;

    address private HODLAddress = 0xF35DE05F2712899cfA60BcB630230a5ca0429656;
    ...
    struct PoolInfo {
        ...
        uint256 totalHODLamountForBonus;
        uint256 startTime;
        uint256 duration;
    }
    ...
    function updatePool(
        ...
        uint256 _totalHODLamountForBonus
        ) public onlyOwner {
            ...
            pools[_poolindex].totalHODLamountForBonus = _totalHODLamountForBonus * (10 ** HODL_DECIMALS);
    }
    ...
    function distributionHODLsForBonus(uint256 _poolindex) public onlyOwner {
        require(enableDistribution);

        uint256 totalAmount = pools[_poolindex].totalAmount_Affiliate;

        for(uint256 i = 0; i < pools[_poolindex].addresses_Affiliate.length; i++) {
            address add = pools[_poolindex].addresses_Affiliate[i];
            uint256 amount = pools[_poolindex].affiliate_info[add].amount;
            address affiliate = pools[_poolindex].affiliate_info[add].affiliate;
            uint256 hodlamount = pools[_poolindex].totalHODLamountForBonus.mul(amount).div(totalAmount);               
            if(hodlamount <= 0) continue;
            uint256 hodlamountforuser = hodlamount.mul(60).div(100);
            uint256 hodlamountforaffiliate = hodlamount.sub(hodlamountforuser);
            IERC20(HODLAddress).transfer(add, hodlamountforuser);
            IERC20(HODLAddress).transfer(affiliate, hodlamountforaffiliate);
        }       
    }
