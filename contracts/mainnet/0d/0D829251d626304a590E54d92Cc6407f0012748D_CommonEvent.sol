...
abstract contract EventBase is ChamberBase, IEvent {
  using EnumerableMap for EnumerableMap.UintToUintMap;
  using StructLib for EnumerableMap.UintToUintMap;
  ...
  /// @notice Version of the contract
  /// @dev Should be incremented when contract changed
  string public constant EVENT_BASE_VERSION = "1.0.0";

  // ---- VARIABLES ----

  EventGenerateInfo internal _info;

  // ---- EVENTS ----

  event EventInfoChanged(EventGenerateInfo info);
  event EventResult(address sender, address heroToken, uint heroTokenId, uint stageId, ActionInternalInfo gen, uint iteration);

  // ---- INITIALIZER ----

  function __EventBase_init(
    address controller_,
    ChamberMeta memory meta,
    EventGenerateInfo memory info_
  ) internal {
    _onlyInitializing();
    __ChamberBase_init(controller_, meta);
    _checkMintItems(info_.mintItems, info_.mintItemsChances);
    _info = info_;
  }

  // ---- GOV ACTIONS ----

  function setEventInfo(EventGenerateInfo memory info_) external {
    require(isGovernance(msg.sender), "!gov");
    _info = info_;
    emit EventInfoChanged(info_);
  }
  ...
  function _action(
    ...
    (bool accept) = abi.decode(data, (bool));

    if (accept) {

      ActionInternalInfo memory gen = _generate(heroToken, heroTokenId);
      ...
      return _noActionResult(heroToken, heroTokenId);
    }
  }

  function _acceptResult(
    address sender,
    address heroToken,
    uint heroTokenId,
    uint stageId,
    ActionInternalInfo memory gen
  ) internal returns (ChamberResult memory) {
    ChamberResult[] memory emptyResult = new ChamberResult[](1);
    ChamberResult memory result = emptyResult[0];

    IStatController.ChangeableStats memory stats = IHero(heroToken).stats(heroTokenId);

    result.chamber = address(this);
    result.heroToken = heroToken;
    result.heroTokenId = heroTokenId;
    ...
    result.manaConsumed = Math.min(gen.manaConsumed, stats.mana);
    result.mintItems = _mintRandomItem(stats.level, stats.experience);

    if(stats.life <= gen.damage) {
      result.kill = true;
    }

    emit EventResult(sender, heroToken, heroTokenId, stageId, gen, iterations[heroToken][heroTokenId]);
    return result;
  }

  function _noActionResult(address heroToken, uint heroTokenId) internal view returns (ChamberResult memory) {
    ChamberResult[] memory emptyResult = new ChamberResult[](1);
    ChamberResult memory result = emptyResult[0];
  ...
  function _generate(address heroToken, uint heroTokenId) internal returns (ActionInternalInfo memory) {
    ...
    ActionInternalInfo[] memory tmp = new ActionInternalInfo[](1);
    ActionInternalInfo memory result = tmp[0];

    result.posAttributes = _generateAttributes(_controller, _info.positiveAttributes);
    result.negAttributes = _generateAttributes(_controller, _info.negativeAttributes);
    ...
    if (random <= _info.positiveStatsChance) {
      PositiveStats memory stats = _info.positiveStats;
      ...
      NegativeStats memory stats = _info.negativeStats;
  ...
  function _mintRandomItem(uint heroLevel, uint heroExp) internal returns (address[] memory) {
    return ChamberLib.mintRandomItems(ChamberLib.MintItemInfo({
    mintItems : _info.mintItems,
    mintItemsChances : _info.mintItemsChances,
    biome : biome,
    amplifier : 0,
    seed : 0,
    oracle : _oracle(),
    heroExp : heroExp,
    heroCurrentLvl : heroLevel,
    magicFind : 0,
    destroyItems : 0,
  ...
  function _generateAttributes(IController _controller, IItem.GenerateInfo memory info) internal returns (uint[] memory attributes) {
    bool hasValues;
  ...
  function _checkMintItems(
    ...
    for (uint i; i < length;) {
      ...
      require(mintItemsChances_[i] != 0, "Zero chance");
      require(mintItemsChances_[i] <= _ITEM_MAX_CHANCE, "Too high chance");
      require(maxChance <= mintItemsChances_[i], "Wrong ordering");
      maxChance = mintItemsChances_[i];
    unchecked{++i;}
    }
  }
...
import "../base/chamber/EventBase.sol";


contract CommonEvent is EventBase {
