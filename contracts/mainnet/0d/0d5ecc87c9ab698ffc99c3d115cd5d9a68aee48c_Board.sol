...
contract Board is IBoard, ERC721, AccessControl, ERC2771Context {
  ...
  mapping(address => uint256[]) private tokenIdsFromMember;
  ...
  /**
   * @dev Resolve DID from the board which tokenId is `tokenId`.
   */
  ...
  /**
   * @dev Returns DID spec's namespace corresponding to the boards.
   */
  ...
  function mint(address _to) public {
    ...
    _mint(_to, tokenId);
    tokenIdsFromMember[_to].push(tokenId);
    tokenIdTracker.increment();
  ...
  function burn(uint256 _tokenId) public {
    ...
    _removeTokenIdFromMember(owner, _tokenId);
  ...
  /**
   * @dev Returns the tokenIds of the board which member is `_member`.
   */
  function getTokenIdsFromMember(address _member) external view returns (uint256[] memory) {
    return tokenIdsFromMember[_member];
  }
  ...
  function _removeTokenIdFromMember(address _member, uint256 _tokenId) internal {
    uint32 newIdx = 0;
    for (uint256 i = 0; i < tokenIdsFromMember[_member].length; i++) {
      if (_tokenId != tokenIdsFromMember[_member][i]) {
        tokenIdsFromMember[_member][newIdx++] = tokenIdsFromMember[_member][i];
      }
    }
    require(newIdx < tokenIdsFromMember[_member].length, "Board: cannot remove tokenId");
    tokenIdsFromMember[_member].pop();
  }
