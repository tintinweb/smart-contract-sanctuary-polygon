...
import "./GeneralMetaCalculator.sol";
import "./GeneralMeta.sol";

import "../ERC721/General.sol";
...
/**
 * @title General Minter
 * @dev Contract to mint Guppy Generals from Cracked Eggs
 * @author Phat Loot DeFi Developers
 * @custom:version v1.0
 * @custom:date 30 April 2022
 */
contract GeneralMinter is AccessControl {
    bytes32 public constant STAFF_ROLE = keccak256("STAFF_ROLE");

    ERC721Preset public eggCrackedContract;
    General public generalContract;

    GeneralMetaDiceRoller public metaDiceRoller;
    GeneralMetaCalculator public metaCalculator;
    ...
    constructor() {
        ...
        generalContract = General(address(0x38a390E40F7Cf43D13C9b42D89794CecDaDD46C3)); // Polygon

        metaDiceRoller = GeneralMetaDiceRoller(payable(address(0x984C526072e2d4e245Dee789353000104d465B1F))); // Polygon
        metaCalculator = GeneralMetaCalculator(address(0x6bB5AD3b16727561a44ec4e493f6082bfa33Bebe)); // Polygon
    }

    function safeMint(uint256 eggId) external returns (uint256 tokenId) {
        if (!mintActive) revert MintInactive();
        return _safeMintSingle(eggId);
    }

    function safeMintBatch(uint256[] memory eggIds) external {
        if (!mintActive) revert MintInactive();
        for (uint256 i = 0; i < eggIds.length; i++) {
            _safeMintSingle(eggIds[i]);
        }
    }

    function _safeMintSingle(uint256 eggId) internal returns (uint256 tokenId) {
        uint256 randomness = metaDiceRoller.getRandomResult(eggId);
        if (randomness == 0) revert DiceNotRolled();

        eggCrackedContract.burn(eggId);
        tokenId = generalContract.safeMint(eggId, msg.sender);

        metaCalculator.calculateMetadata(tokenId, randomness);
        return tokenId;
    }
...
/**
 * @title Guppy General
 * @dev ERC721 for the Guppy Generals project, this is the Guppy General.
 * @author Phat Loot DeFi Developers
 * @custom:version v1.0
 * @custom:date 30 April 2022
 */
contract General is ERC721Preset {
    bytes32 public constant MINT_ROLE = keccak256("MINT_ROLE");

    // Mapping to the cracked Egg the General came out of
    mapping(uint256 => uint256) private _eggs;

    // Officially approved contract addresses that contain VRF generated Metadata or User Upgradeable Metadata
    mapping(address => bool) private _metadata;

    constructor() ERC721Preset("Guppy General", "GENERAL") {
        _grantRole(MINT_ROLE, msg.sender);
    }

    function safeMint(uint256 eggId, address to) external onlyRole(MINT_ROLE) returns (uint256 tokenId) {
    ...
    function flipApprovalMetadataContract(address contractAddress) external onlyRole(STAFF_ROLE) {
        _metadata[contractAddress] = !_metadata[contractAddress];
    }

    function isApprovedMetadataContract(address contractAddress) external view returns (bool) {
