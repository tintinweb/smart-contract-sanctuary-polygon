...
contract PrivateMintNFT is Context, Ownable {
    ...
    function newOffer(address forAddress,uint256 offerPrice,uint256[] memory period,uint256 multipler) public onlyPermission returns (bool) {
        require(multipler>0,"Please Insert Amount For Mint");
        totalOffer += 1;
        ...
        offer[totalOffer].multipler = multipler;
        if(period.length==1){
    ...
    function offerMint(uint256 offerid) public payable returns (bool) {
       ...
       for(uint256 i=0; i<offer[offerid].multipler; i++){
