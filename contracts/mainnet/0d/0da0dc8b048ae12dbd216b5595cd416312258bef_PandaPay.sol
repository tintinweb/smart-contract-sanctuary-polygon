...
contract PandaPay is Ownable, ERC223ContractInterface {
    using Address for address;
    ...
    constructor(uint256 feeRate, address dexRouter, address[] memory tokens, address[] memory stableTokens) {
        FEE_RATE = feeRate;
        routerAddress = dexRouter;
        router = IUniswapV2Router01(dexRouter);
        factory = IUniswapV2Factory(router.factory());
        for (uint i = 0; i < tokens.length; ++i) {
            supportedTokens.add(tokens[i]);
        }
        for (uint i = 0; i < stableTokens.length; ++i) {
            routerTokens.add(stableTokens[i]);
        }
    }
    ...
    function payWithEth(Payment calldata p) payable external {
        ...
        uint256 fee = SafeMath.div(SafeMath.mul(total, FEE_RATE),FEE_DIVISOR);
        ...
        address[] memory path = this.routerPath(router.WETH(), p.merchantTokenAddress);
        uint256[] memory amounts = router.swapETHForExactTokens{value: msg.value}(total, path, address(this), block.timestamp + 600);
    ...
    function payWithToken(address tokenAddress, Payment calldata p) external {
        ...
        address[] memory path = this.routerPath(tokenAddress, p.merchantTokenAddress == NATIVE_CURRENCY ? router.WETH() : p.merchantTokenAddress);
        ...
        token.safeIncreaseAllowance(routerAddress, SafeMath.sub(MAX_ALLOWANCE, token.allowance(address(this), routerAddress)));
    ...
    function getBalancesAndAmounts(address user, Payment calldata p) external view returns(address[] memory tokens, uint256[] memory balances, uint256[] memory amounts) {
        address merchantTokenAddress = p.merchantTokenAddress == NATIVE_CURRENCY
            ? router.WETH()
            : p.merchantTokenAddress;
        uint256 total = SafeMath.mul(p.quantity, p.unitPrice);
        ...
        for (uint i = 0; i < tokens.length; ++i) {
            ...
            if (tokens[i] == p.merchantTokenAddress) {
                ...
                try this.routerPath(tokens[i] == NATIVE_CURRENCY ? router.WETH() : tokens[i], merchantTokenAddress) returns (address[] memory path) {
