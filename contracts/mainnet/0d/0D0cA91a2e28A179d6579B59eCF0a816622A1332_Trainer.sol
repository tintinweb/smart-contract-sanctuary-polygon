...
contract Trainer is ITrainer, Ownable {
    uint256 public constant MAX_SPOTS = 20;
...
contract Luchador is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function getLuchadoresData() public view returns (LuchadorData[] memory){
        ...
        for (uint i = 1; i < _totalSupply + 1; i++) {
            luchadores[i].tokenURI = tokenURI(i);
            luchadores[i].attributes = luchadoresAttributes[i];
            luchadores[i].location = luchadoresLocation[i];
            luchadores[i].currentTrainerOwner = luchadoresCurrentTrainerOwner[i];
            luchadores[i].locationStartTime = luchadoresLocationStartTime[i];
            luchadores[i].weight = luchadoresWeight[i];
            luchadores[i].wins = luchadoresWins[i];
            luchadores[i].losses = luchadoresLosses[i];
            luchadores[i].name = luchadoresName[i];
            luchadores[i].bio = luchadoresBio[i];
            luchadores[i].price = luchadoresPrice[i];
            luchadores[i].index = luchadoresIndex[i];
            luchadores[i].lastSalePrice = luchadoresLastSalePrice[i];
            luchadores[i].saleStatus = luchadoresSaleStatus[i];
        }
