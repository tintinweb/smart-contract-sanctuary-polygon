...
/**
 *  Staking pool with deposit limit and withdraw lock.
 */
contract StakingPoolDLWL is Ownable, ReentrancyGuard, Metadata {
    ...
    event BlockRewardPerSecUpdated(
        uint256 _blockRewardPerSec,
        uint256 _rewardPoolIndex
    );
    ...
    event WithdrawTimestampUpdated(uint256 _newWithdrawTimestamp);
    ...
    function init(bytes memory extraData) external {
        ...
        // Decoding is done in three parts due to stack too deep issue.
        (
        ...
        (
            ...
            ,
            ,
            _localVars._blockRewardPerSec,
            farmInfo.harvestInterval,
            feeAddress,
            farmInfo.withdrawalFeeBP,
            owner // StakingPool owner address
        ) = abi.decode(
            extraData,
            (
                IERC20,
        ...
        require(
            _localVars._endTimestamp >= block.timestamp,
            "Reward End block timestamp must be greater than current timestamp"
        );

        require(
            farmInfo.endTimestamp >= farmInfo.withdrawTimestamp,
            "End block timestamp must be greater than or equal to withdraw timestamp"
        );
        ...
        rewardPool.push(
            RewardInfo({
                ...
                lastRewardBlockTimestamp: block.timestamp >
                    ...
                    ? block.timestamp
                    : _localVars._startTimestamp,
                blockRewardPerSec: _localVars._blockRewardPerSec,
                accRewardPerShare: 0
            })
        );
    ...
    function updateWithdrawTimestamp(uint256 _newWithdrawTimestamp)
        external
        onlyOwner
    {
        require(
            _newWithdrawTimestamp <= farmInfo.endTimestamp,
            "Invalid withdraw timestamp"
        );

        farmInfo.withdrawTimestamp = _newWithdrawTimestamp;

        emit WithdrawTimestampUpdated(_newWithdrawTimestamp);
    }
    ...
    function addRewardToken(
        ...
        rewardPool.push(
            RewardInfo({
                ...
                blockRewardPerSec: _blockRewardPerSec,
                accRewardPerShare: 0
            })
        );
    ...
    function updateBlockRewardPerSec(
        uint256 _blockRewardPerSec,
        ...
        rewardPool[_rewardTokenIndex].blockRewardPerSec = _blockRewardPerSec;
        emit BlockRewardPerSecUpdated(_blockRewardPerSec, _rewardTokenIndex);
    }
    ...
    function canHarvest(address _user) public view returns (bool) {
        ...
        return ((block.timestamp >= user.nextHarvestUntil) &&
            (block.timestamp >= farmInfo.withdrawTimestamp));
    }
    ...
    function _withdraw(
        ...
        require(
            block.timestamp >= farmInfo.withdrawTimestamp,
            "Cannot withdraw before end timestamp"
        );
        UserInfo storage user = userInfo[_user];
