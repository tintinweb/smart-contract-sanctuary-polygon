...
contract Generator is Ownable {
  ...
  base64mod public b;
   ...
   constructor(address _base64contract) { 
     b = base64mod(_base64contract);
    }
  ...
  function attributeForTypeAndValueNumber(string memory _componentType, string memory _value) public pure returns (string memory) {
    return string(abi.encodePacked(
  ...
  function compileAttributes(uint256 _tokenId, ICharacter.Character memory s, uint256 _getpaindTokens) public view returns (string memory) {
    string memory components;
      ...
      components = string(abi.encodePacked(
        ...
        attributeForTypeAndValueNumber("charisma", s.charisma.toString()),','
      ));
   ...
   */
  function selectTraits(uint256 seed, uint256 _tokenId) public view returns (ICharacter.Character memory t) {  
    ...
    else  // %1 Militia
    {
     ...
     // Adding +100 charisma points for gen.0 NFTs
     if(_tokenId <= 10000)
     {
       t.charisma += 100;
     }
