...
contract ClearPoolDesk is IClearPoolDesk, AdminUpgradeable {
   ...
   * @param _alloyxPoolAddress the pool address of which we calculate the balance
   */
  function getClearPoolWalletUsdcValue(address _alloyxPoolAddress) external view override returns (uint256) {
    uint256 length = poolToClearPoolTokenAddresses[_alloyxPoolAddress].length();
    uint256 allBalance = 0;
    for (uint256 i = 0; i < length; i++) {
      uint256 balance = getClearPoolUsdcValueOfPoolMaster(_alloyxPoolAddress, poolToClearPoolTokenAddresses[_alloyxPoolAddress].at(i));
      allBalance += balance;
    }
    return allBalance;
  }
   ...
   */
  function getClearPoolUsdcValueOfPoolMaster(address _alloyxPoolAddress, address _address) public view override returns (uint256) {
    IPoolMaster poolMaster = IPoolMaster(_address);
    uint256 exchangeRate = poolMaster.getCurrentExchangeRate();
    uint256 balanceOfWallet = poolToClearPoolTokenAmountMapping[_alloyxPoolAddress][_address];
