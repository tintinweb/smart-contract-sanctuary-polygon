...
contract GenericNFTFactory is Ownable {
    ...
    mapping(address => GenericNFT) public userAddressToNFTContract;
    ...
    */
    ...
    function applyToWhitelist(address nftContractAddress, uint256 _admissionFee) public payable {
        ...
        require(_admissionFee >= admissionFee, "Admission fee is higher, check the fee value");
        deposit(_admissionFee);

        userAddressToNFTContract[msg.sender] = GenericNFT(nftContractAddress);
    ...
    function claimOwnership() public onlyWhitelist {
        GenericNFT targetNFTContract = userAddressToNFTContract[msg.sender];
        ...
        delete userAddressToNFTContract[msg.sender];
    ...
    */
    ...
    // Transfers the contract balance of the depost token
    ...
    */
    ...
    // Allows users in whitelist to create a new token for their contract
    // recipient -> The address that to which the token will be assigned
    // tokenURI -> The token URI to be provided by the user
    // _fee -> the fee to be payed for the token creation 
    function mintNFT(address recipient, string memory tokenURI, uint256 _fee) public onlyWhitelist returns (uint256) {
        require(_fee >= fee, "Minting fee is higher, check the fee value");
        deposit(_fee);        
        
        GenericNFT nftContract = userAddressToNFTContract[msg.sender];
        nftContract.unlock();
    ...
    // userAddress -> Address of the user, needed to retrieve the NFT contract address
    // tokenId -> ID of the token to be updated
    // newTokenURI -> new URI to be applied
    function changeTokenURI(uint256 tokenId, string memory newTokenURI, uint256 _editingFee) public onlyWhitelist {
    ...
    function deleteNFT(uint256 tokenId, uint256 _editingFee) public onlyWhitelist {
