...
contract FlashLoan {
    ...
    event WithdrawBNBFromContract(uint256 amount);
    event TokenFromContractTransfered(
        address indexed tokenAddress,
        address indexed toAddress,
        uint256 amount
    );
    event UpdateEQZFee(uint256 _fee);
    ...
    constructor() {
        ...
        uint256 totalInvestmentWithFee = _amountTokenPay + repayFee;

        if(totalInvestmentWithFee > amountRepay) {
            return (0, amountOut);
        }

        return (
            uint256(amountRepay - totalInvestmentWithFee), // our profit or loss; example output: BNB amount
    ...
    function onFlashLoan(
        ...
        ) = getEncodedDatas(data);
        (uint256 profit, ) = check(
            outputToken,
            amount,
            token,
            sourceRouter,
        ...
        executeTrade(token, amount, data);
    ...
    function executeTrade(
        ...
        uint256 amountReceivedFromSourceDex = IUniswapV2Router(sourceRouter)
    ...
    function getEncodedDatas(bytes calldata data)
        public
        pure
        returns (
            address,
    ...
    {
        (address outputToken, address sourceRouter, address targetRouter) = abi
    ...
    function withdrawBNBFromContract() external {
        ...
        emit WithdrawBNBFromContract(address(this).balance);
    }

    function withdrawTokenFromContract(address _tokenContract, uint256 _amount)
        external
    {
        require(msg.sender == owner, "Owner Only");
    ...
    function updateEQZFee(uint256 _fee) external {
        require(msg.sender == owner, "Owner Only");
        repayFee = _fee;
        emit UpdateEQZFee(_fee);
    }
