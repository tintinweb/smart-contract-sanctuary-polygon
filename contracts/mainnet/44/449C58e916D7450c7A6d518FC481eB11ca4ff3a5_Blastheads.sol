...
contract Blastheads is ERC721A, Ownable, ReentrancyGuard {
    IPaperKeyManager private paperKeyManager;
    ...
    modifier mintCompliance(address _userAddress, uint256 _mintAmount) {
        require(
        ...
        require(
            balanceOf(_userAddress) + _mintAmount < maxMintAmountPerWallet + 1,
    ...
    */
    ...
    function _mint(address _to, uint256 _mintAmount) 
      internal 
      onlyOwner
      mintCompliance(_to, _mintAmount) {
    ...
    function ownerAirdrop(address[] memory _receivers, uint256 _mintAmount) external onlyOwner {
        for (uint256 i = 0; i < _receivers.length; i++) {
          _mint(_receivers[i], _mintAmount);
        }
    }
    ...
    */
    ...
    function paperMint(address _to, uint256 _mintAmount, bytes32 _nonce, bytes calldata _signature)
        public
        payable
        onlyPaper(keccak256(abi.encode(_to, _mintAmount)), _nonce, _signature)
        mintCompliance(_to, _mintAmount)
    {
        _safeMint(_to, _mintAmount);
    }

    function paperCheck(address _wallet, uint256 _mintAmount)
        public
        view
        returns (string memory)
    {
        if (!saleEnabled) {
            return "La campagne est actuellement close";
        } else if (balanceOf(_wallet) + _mintAmount > maxMintAmountPerWallet) {
            return "Vous avez deja obtenu une edition !";
        } else {
            return "";
        }
    }
