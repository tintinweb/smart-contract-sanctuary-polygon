...
contract UniswapSwapPlace is ISwapPlace {
    ...
    function swap(SwapRoute calldata route) external override returns (uint256) {
        ...
        IERC20(route.tokenIn).approve(route.pool, route.amountIn);
        ...
        uint[] memory amounts = IUniswapV2Router02(route.pool).swapExactTokensForTokens(
            route.amountIn,
    ...
    function _getAmountOut(
        ...
        path[0] = tokenIn;
        path[1] = tokenOut;
        uint[] memory amounts = IUniswapV2Router02(pool).getAmountsOut(amountIn, path);
        return amounts[1];
    }
