...
contract ConcentratedLiquiditySystem is BaseContract {
  ...
  function swapTokenForUSDT(uint _amount, bool _cls) external onlySwapContract returns (uint) {
    // TODO cls case
  ...
  function convertBusdToUsdt(uint _busdAmount) external onlyMn {
    require(busdToken.balanceOf(address(this)) >= _busdAmount, "ConcentratedLiquiditySystem: contract insufficient balance");
    address[] memory path = new address[](2);
