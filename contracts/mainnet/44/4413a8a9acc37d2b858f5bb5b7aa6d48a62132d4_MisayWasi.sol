    ...
    UUPSUpgradeable
{
    ...
    function updateDateMisayWasi(uint256 _misayWasiUuid, uint256 _newDate)
        external
        onlyRole(GAME_MANAGER)
    {
        ...
        require(
            misayWasiInfo.isCampaignActive,
            "Misay Wasi: Campaign is not active"
        );

        misayWasiInfo.campaignEndDate = _newDate;
    }

    function increaseFundsInRaffle(uint256 _misayWasiUuid, uint256 _funds)
        external
    {
        require(_funds > 0, "Misay Wasi: Zero value");
        ...
        _transferFromUSDC(_msgSender(), _bizWalletAddress, _funds);

        misayWasiInfo.rafflePrize += _funds;
    }
