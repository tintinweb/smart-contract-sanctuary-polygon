...
contract Lottery {
    ...
    uint256 public constant maxTickets = 25; // maximum tickets per lottery
    uint256 public constant ticketCommission = 3; // commition per prize pool in percents
    uint256 public constant duration = 5 days; // The duration set for the lottery
    ...
    function DrawWinnerTicket() public isOperator {
        require(tickets.length > 2, "less than 2 tickets were purchased");
        ...
        lastWinner = winner;
        uint256 totalTicketPrice = tickets.length * ticketPrice;
        uint256 operatorPercentage =  (totalTicketPrice * ticketCommission) / 100;
        winnings[winner] += (tickets.length * (ticketPrice - operatorPercentage));
        lastWinnerAmount = winnings[winner];
        operatorTotalCommission += operatorPercentage;
        delete tickets;
        expiration = block.timestamp + duration;
    }
