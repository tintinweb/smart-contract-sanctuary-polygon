...
contract C250Gold is AccessControl, ERC20, Ownable, ReentrancyGuard {
    ...
    function referralCount(uint256 userID) external view returns (uint256) {
        return users[userID].referrals.length;
    }

    function getReferrals(uint256 userID, uint256 startIndex)
        external
        view
    ...
    function getLinkAccounts(uint256 userID, uint256 startIndex)
        external
        view
        returns (uint256[10] memory)
    {
        uint256[10] memory ids;
        if (userAccounts[userAddresses[userID]].length > startIndex) {
            for (uint256 i = 0; i < 10; i++) {
                if (
                    i + startIndex >= userAccounts[userAddresses[userID]].length
                ) {
                    break;
                }
                ids[i] = userAccounts[userAddresses[userID]][i + startIndex];
            }
        }
        return ids;
    }

    function activeDownlinesByUser(uint256 userID, uint256 day)
        external
        view
        returns (uint256)
    {
        return users[userID].activeDownlines[getTheDayBefore(day)];
    }

    function classicEarningByUser(uint256 userID, uint256 day)
        external
        view
        returns (uint256)
    {
        return users[userID].classicEarningCount[getTheDayBefore(day)];
    }
        ...
        )
    {
        ...
        require(matrices[userID][part].registered, "IVL");
    ...
    function setTimeProvider(address _timePovider) external onlyOwner {
        timeProvider = TimeProvider(_timePovider);
    }
    ...
    function getAccounts(address addr, uint256 startIndex)
        external
        view
        returns (uint256[20] memory)
    {
        uint256[20] memory result;
        for(uint256 i = startIndex; i < userAccounts[addr].length; i++) {
            result[i] = userAccounts[addr][startIndex+i];
        }
        return result;
    }
    ...
    function creatChangeWalletRequest(uint256 userID, address newWallet)
        external
    {
        require(userAddresses[users[userID].referralID] == msg.sender, "NA");
        require(changeWalletRequests[userID].newWallet == address(0), "RE");
    ...
    function deleteChangeWalletRequest(uint256 userID) external {
        ...
        require(changeWalletRequests[userID].newWallet != address(0), "RNF");
    ...
    function addAndActivateMultipleAccounts(
        ...
        require(balanceOf(msg.sender) >= amountFromDollar(ACTIVATION_FEE).mul(no), "ISB");
    ...
    function withdraw(uint256 userID) external nonReentrant {
        require(userAddresses[userID] == msg.sender, "NA");
        (uint256 dollarAmount, uint256 earningCounter) = withdrawable(userID);
        require(dollarAmount > 0, "NF");
    ...
    function upgradeToPremium(uint256 userID, uint256 random) external {
        require(live, "NA");
        require(userID > 0 && userID <= lastID, "IVID");
        require(
            balanceOf(msg.sender) >= amountFromDollar(UPGRADE_FEE),
            "INS_BAL"
        );

        require(users[userID].classicIndex > 0, "CLNA");
    ...
    function getWeekday(uint256 timestamp) public pure returns (uint8) {
        return uint8((timestamp / (ADAY) + 4) % 7);
    }
    ...
    function _register(
        ...
        require(referralID <= lastID, "NF");
        lastID++;
        userAddresses[lastID] = addr;
        users[lastID].registered = true;
    ...
    function register(
        ...
        require(live, "NS");
        require(userAccounts[addr].length == 0, "DUP");
        _register(referralID, uplineID, addr);
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(presaleBalance[from].add(amount) <= balanceOf(from), "NA");
    }

    function activate(uint256 id) public nonReentrant {
        require(live, "NS");
        require(userAddresses[id] != address(0), "NF");
        require(users[id].classicIndex == 0, "DUP");
        uint256 feeAmount = amountFromDollar(ACTIVATION_FEE);
        require(balanceOf(msg.sender) >= feeAmount, "INS_BAL");
    ...
    function withdrawable(uint256 userID)
        public
        view
        returns (uint256, uint256)
    {
        ...
        // @dev keep track of the number of days that've been considered in the current loop so
        //as to stay within the limit for the level
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 minRec = 0;
            if (classicActivationDays.length > 30) {
                minRec = classicActivationDays.length - 30;
            }
            for (
                uint256 i = classicActivationDays.length - 1;
                i >= minRec;
                i--
            ) {
    ...
    uint256[] public premiumAccounts;
    ...
    function importV1Account(ImportOption[] calldata options) external {
        require(isUserAdmin(msg.sender) && !live, "NA");

        for (uint256 i = 0; i < options.length; i++) {
            ImportOption calldata option = options[i];

            if (option.classicCheckpoint == 0) {
                continue;
            }
            if (option.userClassicIndex == 0) {
                continue;
            }
            if (option.referralID > 0 && !users[option.userID].registered) {
                users[option.referralID].referrals.push(option.userID);
            }
            users[option.userID].registered = true;
            users[option.userID].imported = option.imported;
            users[option.userID].referralID = option.referralID;
            users[option.userID].importClassicLevel = option.importClassicLevel;
            users[option.userID].premiumLevel = option.premiumLevel;
            users[option.userID].importedReferralCount = option.importedReferralCount;
            users[option.userID].outstandingBalance = option.outstandingBalance;
            users[option.userID].classicCheckpoint = option.classicCheckpoint;
            users[option.userID].classicIndex = option.userClassicIndex;

            if (option.premiumLevel > 18) {
                users[option.userID].premiumLevel = 18;
            }

            userAddresses[option.userID] = option.addr;
            if (classicIndex < option.userClassicIndex) {
                classicIndex = option.userClassicIndex;
            }

            if (lastID < option.userID) {
                lastID = option.userID;
            }

            if (option.addr != address(0)) {
                userAccounts[option.addr].push(option.userID);
            }

            if (
                !users[option.referralID].imported ||
                users[option.referralID].referrals.length -
                    users[option.referralID].importedReferralCount >=
                classicConfigurations[users[option.referralID].importClassicLevel - 1].exDirectReferral
            ) {
                users[option.referralID].activeDownlines[getTheDayBefore(block.timestamp)] = users[
                    option.referralID
                ].referrals.length;
            }

            if (users[option.userID].premiumLevel == 0) {
                return;
            }

            if (!matrices[option.userID][1].registered) {
                premiumCounter = premiumCounter.add(1);
                premiumAccounts.push(option.userID);
            }

        }
    }

    struct MatrixImportOption {
        uint256 userID;
        uint256 part;
        uint256 left;
        uint256 right;
    ...
    function processMatrix(MatrixImportOption[] calldata options) external {
        require(isUserAdmin(msg.sender) && !live, "NA");
        for (uint256 i = 0; i < options.length; i++) {
            MatrixImportOption calldata option = options[i];

            matrices[option.userID][option.part].registered = true;
            matrices[option.userID][option.part].uplineID = option.uplineID;
            matrices[option.userID][option.part].left = option.left;
            matrices[option.userID][option.part].right = option.right;
        }
    }
    ...
    function getAvailableUplineInMatrix(
        ...
        require(matrices[uplineID][part].registered, "UNIP");
    ...
    function getMatrixPayoutCount(uint256 userID, uint256 level)
        public
        view
        returns (uint256)
    {
        ...
        input[0] = userID;
        uint256[16] memory ids = blockDownlines(
            input,
            level,
        ...
        for (uint256 i = 0; i < 16; i++) {
            ...
            if (users[ids[i]].premiumLevel >= level) {
                count += 1;
            }
        }
        return count;
    ...
    function blockDownlines(
        ...
        uint256 level,
        uint256 gen
    ) private view returns (uint256[16] memory) {
        if (gen == 0) {
            return ids;
        }
        ...
        returns (uint256)
    {
        ...
        if (
            getMatrixPayoutCount(beneficiary, level) >=
    ...
    function moveToNextLevel(uint256 userID, uint256 random) private {
        ...
        uint256 pendingPayoutCount = getMatrixPayoutCount(userID, newLevel);
        if (pendingPayoutCount > 0) {
            uint256 pendingAmount = pendingPayoutCount.mul(
    ...
    function levelCompleted(uint256 userID) private view returns (bool) {
        ...
        uint256 lineCount = getMatrixPayoutCount(
            userID,
    ...
    function levelFromPart(uint256 part) private pure returns (uint256) {
        if (part == 1) {
            return 1;
        }
        if (part == 2) {
...
contract C250PriceOracle {
    ...
    function getQuote(
        ...
        address pool = IUniswapV3Factory(factory).getPool(usdc, tokenOut, 3000);
        if (pool == address(0)) {
            return 0;
        }
