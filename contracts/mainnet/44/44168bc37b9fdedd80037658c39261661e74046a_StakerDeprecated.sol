...
contract StakerDeprecated is IStakerDeprecated, AccessControlledAndUpgradeable {
  ...
  ILongShortOriginal contract whenever there is a state change for a market.
  ...
  */
  function pushUpdatedMarketPricesToUpdateFloatIssuanceCalculations(
    ...
    // This function does nothing now! Staker deprecated
  }
    ...
    returns (uint256 floatReward)
  {
    ...
    if (usersNextPriceActionIndex > 0 && usersNextPriceActionIndex <= currentRewardIndex) {
      ...
      if (amountForAction_userShiftOrMint > 0) {
        amountStakedShort += ILongShortOriginal(longShort)
      ...
      if (amountForAction_userShiftOrMint > 0) {
        amountStakedLong += ILongShortOriginal(longShort).getAmountSyntheticTokenToMintOnTargetSide(
            marketIndex,
            amountForAction_userShiftOrMint,
            false,
            usersNextPriceActionIndex
      ...
      if (amountForAction_userShiftOrMint > 0) {
        uint256 syntheticTokenValue = ILongShortOriginal(longShort)
          .get_syntheticToken_priceSnapshot_side(marketIndex, true, usersNextPriceActionIndex);
      ...
      if (amountForAction_userShiftOrMint > 0) {
        uint256 syntheticTokenValue = ILongShortOriginal(longShort)
          .get_syntheticToken_priceSnapshot_side(marketIndex, false, usersNextPriceActionIndex);
  ...
  */
  function _withdraw(
    ...
    ISyntheticTokenOriginal(token).transfer(user, amount);

    emit StakeWithdrawn(user, token, amount);
  }
  ...
  @param user Anyone can UNSTAKE other users! (we don't want to allow anyone to withdraw other users from the market though)
  */
  function withdrawAllUser(
    ...
    uint256 userAmountStakedBeforeWithdrawal = userAmountStaked[token][user];

    if (userAmountStakedBeforeWithdrawal == 0) return;

    uint256 amountToShiftForThisToken = userNextPrice_amountStakedSyntheticToken_toShiftAwayFrom[
      marketIndex
    ][isWithdrawFromLong][user];
    userAmountStaked[token][user] = amountToShiftForThisToken;

    ILongShortOriginal(longShort).executeOutstandingNextPriceSettlementsUser(
      address(this),
      marketIndex
    );
    // _withdraw reverts if you try withdraw 0.
    if (userAmountStakedBeforeWithdrawal - amountToShiftForThisToken > 0) {
      _withdraw(
        marketIndex,
        token,
        userAmountStakedBeforeWithdrawal - amountToShiftForThisToken,
        user
      );
    }
  }
  ...
  //   withdrawAllUser(marketIndex, isWithdrawFromLong, msg.sender);
  // }

  function withdrawAllUser(uint32 marketIndex, address user) external {
    // LONG:
    withdrawAllUser(marketIndex, true, user);
    // SHORT:
    withdrawAllUser(marketIndex, false, user);

    address tokenLong = syntheticTokens[marketIndex][true];
    address tokenShort = syntheticTokens[marketIndex][false];

    require(userAmountStaked[tokenLong][user] == 0);
    require(userAmountStaked[tokenShort][user] == 0);
  }

  function userAmountStakedWithPending(address token, address user)
    public
    view
    returns (uint256 amountStaked)
  {
    amountStaked = userAmountStaked[token][user];

    uint32 marketIndex = marketIndexOfToken[token];
    uint256 stakerIndex = latestRewardIndex[marketIndex];
    uint256 userIndex = userNextPrice_stakedActionIndex[marketIndex][user];

    bool isLong = syntheticTokens[marketIndex][true] == token;

    if (userIndex > 0 && userIndex <= stakerIndex) {
      uint256 amountToken = userNextPrice_amountStakedSyntheticToken_toShiftAwayFrom[marketIndex][
        isLong
      ][user];
      if (amountToken > 0) {
        amountStaked -= amountToken;
      }
      amountToken = userNextPrice_amountStakedSyntheticToken_toShiftAwayFrom[marketIndex][!isLong][
        user
      ];
      if (amountToken > 0) {
        amountStaked += ILongShortOriginal(longShort).getAmountSyntheticTokenToMintOnTargetSide(
          marketIndex,
          amountToken,
          !isLong,
          userIndex
      ...
      amountToken = userNextPrice_paymentToken_depositAmount[marketIndex][isLong][user];
      if (amountToken > 0) {
        amountStaked +=
          (amountToken * 1e18) /
          ILongShortOriginal(longShort).get_syntheticToken_priceSnapshot_side(
            marketIndex,
            isLong,
            userIndex
          );
