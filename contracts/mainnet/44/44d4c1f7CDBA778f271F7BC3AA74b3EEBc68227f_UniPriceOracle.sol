...
import "../apis/uniswap/UniswapOracleLibrary.sol";
...
/**
 * One Token pair should only be add once.
 */

// fixed window oracle that recomputes the average price for the entire period once every period
// note that the price average is only guaranteed to be over at least 1 period, but may be over a longer period
contract UniPriceOracle is OwnableUpgradeSafe, PriceOracle {
    ...
    // pair => price oracle, lp to oracle map
    mapping(address => Oracle) public oracles;
    address[] public pairs;

    // keccak256(token0, token1) => pair
    mapping(bytes32 => address) public pairsMap;
    ...
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'PriceOracle: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'PriceOracle: ZERO_ADDRESS');
    }

    function addOracle(address pair) external onlyOwner {
        IUniswapPair _pair = IUniswapPair(pair);
        ...
        (address tokenA, address tokenB) = sortTokens(oracle.token0, oracle.token1);
    ...
    function update(uint256 offset, uint256 size) external {
        for (uint256 idx = offset; idx < offset + size; ++idx)
        {
            ...
            {
                require(timeElapsed >= PERIOD, 'PriceOracle: PERIOD_NOT_ELAPSED');
            }
    ...
    function getPrice(address token0, address token1) external view override returns (uint256 price, uint256 lastUpdate) {
        ...
        bytes32 key = keccak256(abi.encodePacked(tokenA, tokenB));
        address _pair = pairsMap[key];
        require(_pair != address(0), "PriceOracle: Price Oracle pair not added");

        Oracle memory oracle = oracles[_pair];
        require(oracle.blockTimestampLast != 0, "PriceOracle: oracle not exists");

        if (oracle.token0 == token0) {
            price = oracle.price0Average.mul(1e18).decode144();
        } else {
            require(oracle.token0 == token1, 'PriceOracle: INVALID_TOKEN');
            price = oracle.price1Average.mul(1e18).decode144();
        }

        require(price != 0, "PriceOracle::getPrice:: bad price data");
