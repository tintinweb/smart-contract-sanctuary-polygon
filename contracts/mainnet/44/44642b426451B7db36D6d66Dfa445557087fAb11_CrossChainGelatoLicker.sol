...
contract CrossChainGelatoLicker is NonblockingLzApp {
    mapping(uint16 => uint256) public lastLicked;
    ...
    function initiateCCLick(
        uint16 _dstChainId,
        address _iceCreamNFT,
        uint256 _tokenId
    ) external payable onlyWhitelistedCaller {
        bytes memory lickPayload = abi.encode(_iceCreamNFT, _tokenId);
        ...
        _lzSend(_dstChainId, lzFee, lickPayload);
    }
    ...
    function checker(
        ...
        if (block.timestamp < lastLicked[_dstChainId] + 600) {
            execPayload = bytes(
    ...
    function _lick(address _iceCreamNFT, uint256 _tokenId) internal {
        IIceCreamNFT(_iceCreamNFT).lick(_tokenId);
    }
    ...
    function _nonblockingLzReceive(
        ...
        (address iceCreamNFT, uint256 tokenId) = abi.decode(
            lickPayload,
            (address, uint256)
        );

        _lick(iceCreamNFT, tokenId);
    }
}
