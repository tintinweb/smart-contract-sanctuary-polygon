...
contract HouseBets is ERC1155, Ownable, PaymentSplitter, ReentrancyGuard, DefaultOperatorFilterer, IERC2981 {
    ...
    function setType(
        ...
        require(_maxSupply >= tokens[_id].totalSupply, "Bad supply");
    ...
    function airdrop(address[] memory _addresses, uint256 _id, uint24[] memory _quantities) external onlyOwner {
        for (uint256 i; i < _addresses.length; i++) {
            require(tokens[_id].totalSupply + _quantities[i] <= tokens[_id].maxSupply, "No supply");
            _mint(_addresses[i], _id, _quantities[i], "");
            tokens[_id].totalSupply += _quantities[i];
        }
    }
}
