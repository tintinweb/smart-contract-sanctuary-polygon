...
contract StakeWithOption is Ownable, ReentrancyGuard {
    ...
    IERC20 public usdt; // USDT in OKC
    ...
    event SafeModeSet(bool safeMode);
    ...
    function createPool(
        ...
        if (startTime < block.timestamp || startTime >= expireTime || expireTime >= unlockTime) {
        ...
        if (maxStake * 1e6 / exchangeRate < minPoolUSDTCap) {
    ...
    function _calcFee(uint256 usdAmount) view private returns (uint256) {
        if (usdAmount < 1e7) {
            return 0;
        }
    ...
    */
    function withdrawDueToStop(uint256 poolId) nonReentrant public {
        ...
        // keep pool stopped
        pools[poolId].status = PoolStatus.Stopped;
    ...
    */
     ...
     * this function can only be call when pool is active
     */
    ...
    uint256 public usdFee = 1000000; // 1 USDT

    function setParams(bool _safeMode, uint256 _minPoolUSDTCap, uint256 _usdFee) onlyOwner public {
        safeMode = _safeMode;
        minPoolUSDTCap = _minPoolUSDTCap;
        usdFee = _usdFee;
    }
}