  ...
  ReentrancyGuard
{
  ...
  /**
   * @dev Returns the item info in one go
   */
  function item(uint256 itemId) external view returns(
   ...
   */
  function maxSupply(uint256 itemId) public view returns(uint256) {
   ...
   */
  function totalSupply(uint256 itemId) public view returns(uint256) {
  ...
  /**
   * @dev Allows anyone to mint with proof
   */
  function mint(
    address to, 
    uint256 itemId,
   ...
   */
  function updateURI(string memory uri_) external onlyRole(_CURATOR_ROLE) {
    _setURI(uri_);
  }
