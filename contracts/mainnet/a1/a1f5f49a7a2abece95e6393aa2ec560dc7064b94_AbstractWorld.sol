...
contract AbstractWorld is ERC721  {
    uint256 public totalSupply;
    uint256 public cost = 11 ether;
    ...
    constructor()
        ERC721("Abstract World - Ai vision ", "awav")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 110) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeig7rl6nhs5ummjsmeipiyksbzzesexch4bg3hilaqytit7dt2agwm/", Strings.toString(tokenId), ".json")
            );
    }
