...
contract Auction is
    ...
    bool public isAuctioneerLocked;
    ...
    function __Auction_init(uint96 _platformFee, address _auctioneer, address _minter)
        external
        initializer
    {
        __Ownable_init();
        ...
        setAuctioneerContract(_auctioneer);
        setMinterContract(_minter);
    }

    function setAuctioneerContract(address _auctioneer) public onlyOwner {
        require(!isAuctioneerLocked, "Auctioneer contract is locked");
        auctioneer = _auctioneer;
    }

    function lockAuctioneerContract() public onlyOwner {
        isAuctioneerLocked = true;
    }

    function setMinterContract(address _minter) public onlyOwner {
        require(!isMinterLocked, "Minter contract is locked");
        minter = _minter;
    }

    function lockMinterContract() public onlyOwner {
        isMinterLocked = true;
    }

    function setPlatformFee(uint96 _platformFee) public onlyOwner {
    ...
    */
    ...
    function recieveRoyalty() external payable override {
        LibPart.Part[] memory withdrawSplit = IMinterUpgradeable(minter).getWithdrawSplits(_msgSender());
        for (uint256 i = 0; i < withdrawSplit.length; i++) {
            stake[withdrawSplit[i].account] += msg.value * withdrawSplit[i].value / 10000;
        }
...
interface IMinterUpgradeable {
    ...
    function getWithdrawSplits(address) external view returns (LibPart.Part[] memory);
}
