...
interface ISaleGateway {    
	function finalizeRemote(address, bytes calldata, bytes calldata, uint256) external;
...
contract PrivateGovSale is
    ...
    mapping(address => bool) public isRemoteUser;
    ...
    function init(
        ...
        gov = _gov;
        minStake = StakingLibrary.minPrivateSale();
     ...
     */
    function getRemoteBuyersLength() public view returns(uint) {
    ...
    /**
     * @dev Check whether buyer/participant is eligible to buy
     * @param _user User address
     * @param _boosterRunning Booster progress
     */
    function isAbleToBuy(address _user, uint32 _boosterRunning) public view returns (bool){
        if(_boosterRunning == 1) {
            if(whitelist[_user] > 0) return true;
            return (userStaked[_user] >= minStake);
        } else if(_boosterRunning == 2) {
            return (getUserTotalStaked(_user) >= minStake);
    ...
    function getTotalPurchase(address _user) public view returns(uint128 total){
        total = purchasePerRound[_user][1] + purchasePerRound[_user][2] + purchasePerRound[_user][3];
    }
    ...
    function getRemoteData() external view returns(
        uint256 rSold,
        uint256 rRaised,
        uint256 rRevenue
    ){
    ...
    function getRemoteUserData(address _user) external view returns(
        uint256 index,
        uint128 bought,
        uint128 received,
        uint128 fee
    ...
    function getUserAllocation(
        ...
        if(isAbleToBuy(_user, _boosterRunning)){
    ...
    function _getPayload() internal view returns(bytes memory payload){
        payload = abi.encode(
            targetSale,
            remoteRaised,
            remoteRevenue,
    ...
    function _getAdapterParams() internal view returns(bytes memory adapterParams){
        uint16 version = 1;
    ...
    function estimateFees() public view returns(uint256 fees){
        (fees,) = endpoint.estimateFees(dstChainId, factory.saleGateway(), _getPayload(), false, _getAdapterParams());
    }
    ...
    function _isSufficient(uint256 _amount) internal view {
        require(payment.balanceOf(address(this)) >= _amount, "less");
    }
    
    function _isNotStarted() internal view {
        require(uint128(block.timestamp) < booster[1].start, "started");
    }

    function _isCalculated() internal view {
    ...
    function moveFund(
        ...
        uint amount = (raised - remoteRaised).calcPercent2Decimal(_percent_d2);
    ...
    function moveFee() external {
        ...
        _releaseToken(factory.operational(), left.calcPercent2Decimal(factory.operationalPercentage_d2()));
        _releaseToken(factory.marketing(), left.calcPercent2Decimal(factory.marketingPercentage_d2()));
        _releaseToken(factory.treasury(), left.calcPercent2Decimal(factory.treasuryPercentage_d2()));
    }
    ...
    function buyToken(
        ...
        if(running == 1){
            require(_setUserAllocation(_buyer), "bad");
            require(candidateTotalStaked > 0 && (userStaked[_buyer] > 0 || whitelist[_buyer] > 0), "!eligible");
        } else if(running == 2){
            require(_setUserTotalStaked(_buyer), "bad");
            require(userStaked[_buyer] > 0, "!eligible");
        }

        uint128 boosterPrice = price;
        
        (uint128 amountInFinal, uint128 tokenReceivedFinal) = _amountInCalc(_amountIn.calcTokenReceived(boosterPrice), _buyer, running, boosterPrice);
        
        uint128 feeCharged = amountInFinal.calcPercent2Decimal(booster[running].fee_d2);
        
        invoices[_buyer].push(Invoice(_setBuyer(_buyer), running, uint64(block.timestamp), tokenReceivedFinal, amountInFinal, feeCharged));
    ...
    function finalizeRemote() external whenPaused {
        ...
        ISaleGateway(factory.saleGateway()).finalizeRemote(address(factory), _getPayload(), _getAdapterParams(), estimateFees());
    }
    ...
    function _setUserTotalStaked(address _user) internal returns(bool) {
        ...
        if(userStaked[_user] == 0 && staked >= minStake) userStaked[_user] = staked;
     ...
     * @param _user User address
     */
    function _setUserAllocation(address _user) internal returns(bool) {
     ...
     * @param _user User address
     */
    function _setBuyer(address _user) internal returns(uint32 buyerId){
...
library StakingLibrary {
	IKommunitasStaking public constant stakingV1 = IKommunitasStaking(0x453D0a593D0af91e77e590a7935894f7AB1b87ec);
    ...
    function minPrivateSale() internal view returns(uint128){
        return uint128(stakingV2.minPrivateSale());
    }
}
