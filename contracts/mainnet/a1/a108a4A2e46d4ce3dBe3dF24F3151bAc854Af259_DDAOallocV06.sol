...
//	uint256 public SaleNum = 0;
	...
	function SaleModify(uint256 id,string memory name,string memory url,string memory img,bool enabled,uint256 amount1,uint256 amount2,uint256 amount3,uint256 cap,uint256 test_mode,address[] memory tokens)public onlyAdmin returns(uint256)
	{
	        //SaleNum++;
...
/*
	function SaleModify(uint256 id,string memory name,bool enabled,uint256 amount1,uint256 amount2,uint256 amount3)public onlyAdmin
	{
		require(Sale[id].exists,"Sale with this ID not exists");
		Sale[id].id = id;
		Sale[id].name = name;
		//Sale[id].vault = AddrVault;
		Sale[id].exists = true;
		Sale[id].enabled = enabled;
		Sale[id].amount[1] = amount1;
		Sale[id].amount[2] = amount2;
...
*/
	...
	function Allocate(uint256 sale, uint8 level, address addr, uint256 amount, uint8 token)public
	{
		...
		require(amount <= Sale[sale].amount[level] * 10**IToken(Sale[sale].tokens[token]).decimals(),"Amount must be equal Amount for this level");
		...
		uint256 amount_human = amount.div(10**IToken(Sale[sale].tokens[token]).decimals());
                IERC20(Sale[sale].tokens[token]).safeTransferFrom(_msgSender(),AddrVault, amount);
