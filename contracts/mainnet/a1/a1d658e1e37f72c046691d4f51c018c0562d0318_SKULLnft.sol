...
contract SKULLnft is ERC721, ERC721Enumerable, Ownable, ERC721Pausable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdTracker;

    uint256 public MAX_NFT = 33333;
    uint256 public MAX_BY_MINT = 1000;
	uint256 public PRICE = 4 * 10**17;
	
    string public baseTokenURI;
	
    event CreateChickenNFT(uint256 indexed id);
	
    constructor(string memory baseURI) ERC721("SkullNFT", "SkullNFT") {
        setBaseURI(baseURI);
        pause(true);
    }
