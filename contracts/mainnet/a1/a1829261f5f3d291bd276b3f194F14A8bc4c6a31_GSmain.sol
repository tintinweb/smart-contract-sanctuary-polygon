...
contract GSmain is Ownable{

    using ECDSA for bytes32;

    IGSstorage public gsStorage;
    bool public mainPaused;
    ...
    function buyItem(uint256 ref, uint256 amount, uint256 rand, bytes memory sig) public {
        if (ref == 0) revert WrongItemRef();
        if (amount == 0) revert InvalidAmount();
        if (gsStorage.gsPaused() || mainPaused) revert Paused();
        if (msg.sender != tx.origin) revert EoaOnly();
        if (gsStorage.sigRequired() && (_getSigner(_hashTx(msg.sender, ref, amount,
                    gsStorage.getUserBoughtItemsCount(msg.sender) + rand), sig) != gsStorage.cbSigner())
        ) revert InvalidSignature();
        _isItemValid(ref, amount);
        gsStorage._distributeItem(msg.sender, ref, amount);
        emit ItemPurchased(msg.sender, ref, amount, gsStorage.itemsTitles(ref));
    }

    // ITEM VALIDITY

    function _isItemValid(uint256 _ref, uint256 _amount) public view {
        if (bytes(gsStorage.itemsTitles(_ref)).length == 0) revert ItemDoesntExist();
        IGSstorage.Item memory _item = gsStorage.itemsRegistry(_ref);
        if (!_item.active) revert ItemNotActive();
        if (_item.supply == 0 || _amount > _item.supply) revert InsufficientSupply();
        if(_amount > _item.maxAmount) revert ExceedsMaxAmount();
        if (_item.startTime > block.timestamp) revert SaleDidNotStart();
        if (_item.endTime > 0 && _item.endTime < block.timestamp) revert SaleEnded();
        if (_item.interval > 0 &&
            block.timestamp - gsStorage.lastBoughtItem(_ref, msg.sender) < _item.interval
        ) revert TooSoon();
        if (gsStorage.nitConditActivated() && gsStorage.getItemConditionsCount(_ref) > 0) {
            uint256 _len = gsStorage.getItemConditionsCount(_ref);
            uint32[] memory _lis = new uint32[](_len);
            for(uint256 j = 0; j< _len; j ++) {
                _lis[j] = gsStorage.itemsConditions(_ref,j);
            }
            if (!INitConditions(gsStorage.nitConditionsContract()).simpleCheck(msg.sender,_lis)) 
    ...
    function _hashTx( address user, uint256 _ref, uint256 _amount, uint256 _nonce) internal pure returns (bytes32 _hash) {
        _hash = keccak256(abi.encodePacked(user, _ref, _amount, _nonce));
    }
    ...
    function setMainPaused(bool _newMainPausedState) public onlyOwner {
        mainPaused = _newMainPausedState;
    }
}
