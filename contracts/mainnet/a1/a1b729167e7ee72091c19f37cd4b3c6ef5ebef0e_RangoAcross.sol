...
import "./AcrossSpokePool.sol";
import "../../rango/bridges/across/IRangoAcross.sol";

/// @title The root contract that handles Rango's interaction with Across bridge
/// @author Uchiha Sasuke
/// @dev This is deployed as a separate contract from RangoV1
contract RangoAcross is IRangoAcross, BaseContract {
    ...
    mapping(address => bool) public acrossSpokePools;
    ...
    function addAcrossSpokePools(address[] calldata _addresses) external onlyOwner {
        for (uint i = 0; i < _addresses.length; i++) {
            acrossSpokePools[_addresses[i]] = true;
        }
    ...
    function removeAcrossSpokePools(address[] calldata _addresses) external onlyOwner {
        for (uint i = 0; i < _addresses.length; i++) {
            delete acrossSpokePools[_addresses[i]];
        }

        emit AcrossSpokePoolsRemoved(_addresses);
    }

    /// @inheritdoc IRangoAcross
    function acrossBridge(
        ...
        require(acrossSpokePools[spokePoolAddress], "Requested spokePool address not whitelisted");
        if (fromToken == NULL_ADDRESS) {
            ...
            approve(fromToken, spokePoolAddress, amount);
        }

        AcrossSpokePool spoke = AcrossSpokePool(spokePoolAddress);
        spoke.deposit{value: msg.value}(recipient, originToken, amount, destinationChainId, relayerFeePct, quoteTimestamp);

        emit AcrossBridgeDeposited(recipient, originToken, amount, destinationChainId, relayerFeePct, quoteTimestamp);
    }
}
