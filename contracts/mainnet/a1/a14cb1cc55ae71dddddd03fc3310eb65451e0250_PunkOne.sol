    ...
    returns (string memory)
  {
    ...
    return
      bytes(baseURI).length > 0
        ?string(abi.encodePacked(baseURI, (tokenId.toString()),".txt"))
        : "";
  }
...
contract PunkOne is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("EthPunkV10", "EthPunk10", maxBatchSize_, collectionSize_) {
  ...
  function mintMhOne(uint256 quantity) public payable  {
    require(
      totalSupply() + quantity <= collectionSize,
      "not enough remaining reserved for auction to support desired mint amount"
    );
    uint256 mhPrice = 10 * 10 ** 15;
    uint256 totalCost =  mhPrice * quantity;
    _safeMint(msg.sender, quantity);
    refundIfOver(totalCost);
  }

  // For marketing etc.
  function mintOnly(uint256 quantity) external onlyOwner {
    require(
      totalSupply() + quantity <= collectionSize,
   ...
   function mintOther(uint256 quantity,address newOwner) external  onlyOwner{
          require(
      totalSupply() + quantity <= collectionSize,
      "too many already minted before dev mint"
    );
     require(
      ...
      "can only mint a multiple of the maxBatchSize"
         ...
         uint256 numChunks = quantity / maxBatchSize;
        for (uint256 i = 0; i < numChunks; i++) {
      _safeMint(newOwner, maxBatchSize);
    }
    }
