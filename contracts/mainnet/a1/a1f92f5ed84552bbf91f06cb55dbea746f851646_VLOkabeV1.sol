    ...
    OwnableUpgradeable
{
    ...
    function init(
        uint256 price_,
        uint256 initialStock_,
        address addressCoin_,
        address recipient_
    ) public virtual {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "Stock is set by only ADMIN"
        );
        price = price_;
        stock = initialStock_;
        _recipient = recipient_;
        _erc20 = IERC20(addressCoin_);
    }

    function buy() public virtual {
        require(
            _erc20.balanceOf(_msgSender()) > price,
            "Miner must have enough coin."
        );
        require(stock > 0, "Out of Stock");
        _erc20.transferFrom(_msgSender(), _recipient, price);
        ...
        returns (string memory)
    {
        ...
        string[3] memory titles = [
            "Very Long Egg",
            "Very Long Okabe",
            "Very Very Long Okabe"
        ];

        if (_erc20.balanceOf(owner) < 100000 ether) {
            imageIndex = 0;
        } else if (_erc20.balanceOf(owner) < 200000 ether) {
            imageIndex = 1;
        } else {
            imageIndex = 2;
        }

        bytes memory u = abi.encodePacked(
            "ipfs://",
            _uri,
            "/",
            imageIndex.toString(),
        ...
        bytes memory json = abi.encodePacked(
            '{"name": "',
            titles[imageIndex],
            '", "description": "\u30a6\u30a9\u30ec\u30c3\u30c8\u306eY-TORA\u304c\u5897\u3048\u308b\u307b\u3069Very Long\u306b\u306a\u308b\u5ca1\u90e8\u3055\u3093", "image": "',
            u,
            '"}'
        );
        return
    ...
    function setPrice(uint256 price_) public virtual {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "Price is set by only ADMIN"
        );
        price = price_;
    }

    function setStock(uint256 stock_) public virtual {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "Stock is set by only ADMIN"
        );
