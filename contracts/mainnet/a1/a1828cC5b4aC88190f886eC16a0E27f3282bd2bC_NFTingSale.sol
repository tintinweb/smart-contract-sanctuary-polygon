...
contract NFTingSale is NFTingBase {
    ...
    {
        traceIntLogs('_saleId', _saleId);
        Listing memory listedItem = listings[_saleId];
        uint256 buyPrice = _addBuyFee(listedItem.price);
        traceIntLogs('listedItem.price', listedItem.price);
        traceIntLogs('msg.value', msg.value);
        traceIntLogs('buyPrice', buyPrice);
        if (msg.value < buyPrice) {
            traceIntLogs('not enough eth provided', buyPrice);
            revert NotEnoughEthProvided(msg.value, buyPrice);
        }

        traceStringLogs('before transfer', '721 and 1155');
        console.log('nft address', listedItem.nftAddress);
    ...
    function traceStringLogs(string memory key, string memory value)
        public
        view
    {
        console.log('-> Log string message', key, value);
    }

    function traceIntLogs(string memory key, uint256 value)
        public
        view
    {
        console.log('-> Log uint256', key, value);
    }
}
