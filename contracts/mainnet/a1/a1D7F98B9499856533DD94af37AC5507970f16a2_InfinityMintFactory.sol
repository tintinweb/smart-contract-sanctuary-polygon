	...
	IInfinityMintRoyalties
{
	...
	function getPreview() public {
		_mintCheck(0, false); //does not check the price
	...
	*/
	function mintPreview(uint64 previewId) public payable onlyOnce {
		...
		_mintCheck(value, !approved[sender()]); //will not check the price for approved members
	...
	function _mintCheck(uint256 msgValue, bool checkPrice) public view {
	...
	function selectiveMint(uint256 pathId) public payable onlyOnce {
		...
		_mintCheck(value, !approved[sender()]);
	...
	//emergency withdraw in case things go wrong
	function emergency() public onlyOnce onlyDeployer {

		if(valuesController.isTrue("disableEmergency"))
			revert();
