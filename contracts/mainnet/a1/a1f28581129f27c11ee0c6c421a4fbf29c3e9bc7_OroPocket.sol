...
abstract contract Ownable is Pausable {
     ...
     */
    constructor(address adminAddress) Pausable() {
        _owner = _msgSender();
        _admin = adminAddress;
...
// File contracts/OroPocket.sol

pragma solidity =0.8.10;


contract OroPocket is Ownable, IERC20 {
    ...
    function lockToken(address _user, uint256 _amount)
        external
        onlyOwner
        returns (bool)
    {
        require(_balances[_user] >= _amount, "insufficient balance");
        _balances[_user] = _balances[_user].sub(_amount);
        lockedTokens[_user] = lockedTokens[_user].add(_amount);
        return true;
    }

    function unlockToken(address _user, uint256 _amount)
        external
        onlyOwner
        returns (bool)
    {
        require(lockedTokens[_user] >= _amount, "overflow locked balance");
        lockedTokens[_user] = lockedTokens[_user].sub(_amount);
