...
contract OpenMarketUpgradeable is
    Initializable,
    ...
    AuctionSaleUpgradeable
{
    ...
    //to keep track of NFT contract addresses that are allowed to operate on the marketplace
    mapping(address => bool) public whiteListedNFTAddresses;
     ...
     */
    function initialize(
        address[] memory nftAddressesToWhiteList,
        address _usdcAddress,
        address _isekaiCoinAddress,
        ...
        __Pausable_init();
        Token = IERC20Upgradeable(_usdcAddress);
        Token2 = IERC20(_isekaiCoinAddress);
        numTokensSellToAddToLiquidity = _liquidityTokenThreshold;

        // add contracts to marketplace whitelist
        for (uint256 i = 0; i < nftAddressesToWhiteList.length; i++) {
            whiteListedNFTAddresses[nftAddressesToWhiteList[i]] = true;
        }

        treasuryWallet = 0x56AEc0B83c2642132e4cD510Ea5db537B60aa52c;
        playToEarnWallet = 0x56AEc0B83c2642132e4cD510Ea5db537B60aa52c;
        foundersWallet = 0x56AEc0B83c2642132e4cD510Ea5db537B60aa52c;
    ...
    function whiteListNFTAddress(address _nftAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        whiteListedNFTAddresses[_nftAddress] = true;
    }
    ...
    function preSaleChecks(
        ...
        require(
            whiteListedNFTAddresses[_nftAddress],
            "NFT address not allowed"
        );
        ...
        return (saleReference.isERC721, saleReference.seller);
    }
    ...
    function createSaleAuction(
        ...
        uint64 _duration
    ) public whenNotPaused onlyValidStartingTime(_startingTime) {
        SaleReference memory saleReference;

        (saleReference.isERC721, saleReference.seller) = preSaleChecks(
            _nftAddress,
            _tokenId,
            _amount
        );

        uint16[] memory emptyCuts = new uint16[](0);
        address[] memory emptyCutReceivers = new address[](0);
    ...
    function createSaleFixedPrice(
        ...
        (_isERC721, _seller) = preSaleChecks(_nftAddress, _tokenId, _amount);
    ...
    /**
     * @dev overriden function to buy a token on fixed price sale
     * @param _saleId Id of the sale to buy
     */
    function _buy(uint256 _saleId) internal override {
    ...
    /**
     * @dev overriden function to finish the auction after the auction time has ended
     * @param _saleId ID of the sale to finish
     */
    function _finishAuction(uint256 _saleId) internal override {
    ...
    function changeFees(
        ...
        require(
            _treasuryCut + _foundersCut + _playToEarnCut + _liquidityCut <= 250,
            "cuts should be less than 25%"
        );
...
contract FixedPriceBaseUpgradeable is SaleBaseUpgradeable {
     ...
     * @param _saleId Id of the sale to buy
     */
    function _buy(uint256 _saleId) internal virtual {}
...
contract FixedPriceSaleUpgradeable is FixedPriceBaseUpgradeable {
    /**
     * @dev Overloaded internal function to create fixed sale.
     ...
     * @param _cuts cut %ages of all the cut receivers
     * @param _cutReceivers addresses of all the cut receivers
     */
    function createFixedSale(
        ...
        address _seller,
        uint16[] memory _cuts,
        address[] memory _cutReceivers
    ) internal {
        // create and add the sale
        FixedPrice memory fixedSale;
        fixedSale.amount = _amount;
        fixedSale.seller = _seller;
        fixedSale.cuts = _cuts;
        fixedSale.cutReceivers = _cutReceivers;
        fixedSale.tokenId = _tokenId;
        fixedSale.nftContract = _nftContract;
        fixedSale.isERC721 = _isERC721;
        fixedSale.fixedPrice = _fixedPrice;
        fixedSale.startedAt = _startingTime;
     ...
     * @param _saleId Id of the sale to buy
     */
    function buy(uint256 _saleId) external nonReentrant whenNotPaused {
     ...
     * @param _saleId Id of the sale to finish
     */
    function finishFixedSale(uint256 _saleId)
        external
        whenNotPaused
        nonReentrant
    {
...
contract AuctionBaseUpgradeable is SaleBaseUpgradeable {
    ...
    function _bid(uint256 _saleId, uint256 _bidAmount) internal {
        ...
        if (auction.basePriceMet == false) {
            require(_bidAmount >= price, "Bid is too low");
            auction.basePriceMet = true;
        } else {
            require(
                _bidAmount >=
        ...
        if (previousBuyer != address(0)) {
            ...
            Token.transfer(previousBuyer, price);
        }
     ...
     * @param _saleId ID of the sale to finish
     */
    function _finishAuction(uint256 _saleId) internal virtual {}
...
contract AuctionSaleUpgradeable is AuctionBaseUpgradeable {
    /**
     * @dev Overloaded function to create auction.
    ...
    function createAuctionSale(
        ...
        auction.basePriceMet = false;
        auction.isERC721 = _isERC721;
        auction.startPrice = uint128(_startingPrice);
        auction.currentPrice = uint128(_startingPrice);
        auction.duration = uint64(_duration);
        auction.startedAt = _startingTime;
        auction.nftContract = _nftAddress;
        auction.seller = _seller;
        auction.cuts = _cuts;
        auction.cutReceivers = _cutReceivers;
        _addAuction(auction);
    }
     ...
     */
    function finishAuction(uint256 _saleId) external whenNotPaused {
...
import "../tokensUpgradeables/IERC2981.sol";
    ...
    PausableUpgradeable
{
    ...
    bytes4 constant InterfaceSignature_ERC2981 = bytes4(0x2a55205a);
    ...
    address internal playToEarnWallet;
    address internal foundersWallet;
    // address internal marketPlaceWallet;
    ...
    // USDT for official market and black market, IsekaiCoin for game market
    IERC20Upgradeable internal Token;
    // IsekaiCoin for official market and black market, USDT for game market
    IERC20 internal Token2;

    // accumulated cuts of artists
    mapping(address => uint256) public artistCuts;

    IPancakeRouter02 public constant pancakeRouter =
        IPancakeRouter02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff); //bsc: 0x10ED43C718714eb63d5aA57B78B54704E256024E eth: 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D polygon: 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff
    ...
    /**
     * @dev official marketplace payout that calculates the cuts of all wallets and distributes the payment amongst them.
     * @param _cuts array of artist cuts
     * @param _cutReceievers array of artist wallet addresses
     * @param _amount the sale amount
     */
    function _payout(
        uint16[] memory _cuts,
        address[] memory _cutReceievers,
        uint256 _amount
    ) internal {
        ...
        // distribute artist cuts
        for (uint256 i = 0; i < _cuts.length; i++) {
            cutAmount = (_cuts[i] * _amount) / 1000;
            remainingAmount -= cutAmount;
            artistCuts[_cutReceievers[i]] += cutAmount;
        }

        uint256 liquidityAmount = (liquidityCut * remainingAmount) / 1000;

        liquidityTokens =
            (liquidityTokens - swappedLiquidity) +
            liquidityAmount;

        // transfer treasury cut from primary cut
        uint256 treasuryAmount = (treasuryCut * remainingAmount) / 1000;
        Token.transfer(treasuryWallet, treasuryAmount);

        // transfer play to earn cut from primary cut
        uint256 playToEarnAmount = (playToEarnCut * remainingAmount) / 1000;
        Token.transfer(playToEarnWallet, playToEarnAmount);

        // transfer founders cut from remaining cut
        uint256 foundersAmount = remainingAmount -
            (liquidityAmount + treasuryAmount + playToEarnAmount);
        Token.transfer(foundersWallet, foundersAmount);
    }

    /**
     * @dev open marketplace payout that calculates the cuts of all wallets and distributes the payment amongst them.
     * @param _isERC721 tells if NFT is an ERC721. If so, we will check for royalties
     * @param _amount the sale amount
     */
    function _payout(
        uint256 _tokenId,
        ...
        // distribute artist cuts (if EIP-2981 is implemented)
        address royaltyReceiver;
        uint256 royaltyAmount;
        if (
            _isERC721 &&
            IERC165Upgradeable(_nftContract).supportsInterface(
                InterfaceSignature_ERC2981
            )
        ) {
            (royaltyReceiver, royaltyAmount) = IERC2981(_nftContract)
        ...
        // transfer treasury cut from remaining cut
        uint256 treasuryAmount = (treasuryCut * remainingAmount) / 1000;
        Token.transfer(treasuryWallet, treasuryAmount);

        // transfer play to earn cut from remaining cut
        uint256 playToEarnAmount = (playToEarnCut * remainingAmount) / 1000;
        Token.transfer(playToEarnWallet, playToEarnAmount);

        // transfer founders cut from remaining cut
        uint256 foundersAmount = (foundersCut * remainingAmount) / 1000;
        Token.transfer(foundersWallet, foundersAmount);

        // transfer remaining to seller
        uint256 sellerAmount = remainingAmount -
            (liquidityAmount +
                treasuryAmount +
                playToEarnAmount +
                foundersAmount);
    ...
    /**
     * @dev this method is responsible for creating swap for USDT and adding liquidity
    ...
    function swapTokens(uint256 tokenAmount) private {
        // generate the uniswap pair path of USDT -> IsekaiCoin
    ...
    function changeTokenAddress(IERC20Upgradeable _tokenAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        Token = _tokenAddress;
    ...
    function changeToken2Address(IERC20 _tokenAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        Token2 = _tokenAddress;
    }
