...
contract PlaSubscriptionNFT is ERC721, Ownable {
    ...
    mapping(string => mapping(uint256 => uint256))
        public _updateCountForCollection;
    ...
    event DoUpdateGroupIdCount(
        uint256 oldUpdateGroupIdCount,
        uint256 newUpdateGroupIdCount
    );
    ...
    modifier socialTypeExists(uint256 socialType) {
        require(
            bytes(_socialTypes[socialType]).length != 0,
            "PlaNFT: socialType nonexistent"
        );
        _;
    }
    ...
    modifier collectionExists(string memory collection) {
        require(
            _collectionDetails[collection].minter != address(0),
            "PlaNFT: collection nonexistent"
        );
        _;
    }
    ...
    modifier groupIdExists(uint256 groupId, uint256 socialType) {
        require(
            bytes(_collectionForGroupId[groupId][socialType]).length != 0,
            "PlaNFT: groupId nonexistent"
        );
        _;
    }
    ...
    modifier collectionVaild(string memory collection) {
        require(
            !equals(name(), collection),
            "PlaNFT: collection can't equal contract name"
        );
        _;
    }
    ...
    function _doUpdateGroupIdCount(uint256 newUpdateGroupIdCount)
        public
        onlyOwner
    {
    ...
    function _setSocialTypes(uint256 socialType, string memory socialName)
        public
        onlyOwner
    {
        require(
    ...
    function _renewal(uint256 tokenId, uint256 renewalCycle)
        public
        exists(tokenId)
    {
        require(
            ERC721.ownerOf(tokenId) == msg.sender,
            "PlaNFT: caller must be owner of tokenId"
        );
        Collection memory collection = _collectionDetails[
            _collectionForTokenId[tokenId]
        ];
    ...
    function _updateSubscriptPay(
        ...
        require(
            _collectionDetails[collection].minter == msg.sender,
            "PlaNFT: caller must be minter of collection"
        );
    ...
    function _updateCollectionGroupId(
        ...
        uint256 groupId,
        uint256 socialType
    ) public socialTypeExists(socialType) {
        require(
            _collectionDetails[collection].minter == msg.sender,
            "PlaNFT: collection minter can't match"
        );
        require(
            bytes(_collectionForGroupId[groupId][socialType]).length == 0,
            "PlaNFT: groutId has exists"
        );
        require(
            _updateCountForCollection[collection][socialType] <
                _updateGroupIdCount,
    ...
    function withdraw(address to, uint256 value) public onlyOwner {
        require(
            ...
            "PlaNFT: value must less than balances"
        );
        require(_relayWithdrawTime != 0, "PlaNFT: relayWithdraw not call");
        require(
            _relayWithdrawTime + _delayWithdrawTime <= block.timestamp,
            "PlaNFT: delayTime not arrive"
        );
    ...
    function minterForTokenId(uint256 tokenId)
        public
        view
        exists(tokenId)
        returns (address)
    ...
    function subscriptCycleForTokenId(uint256 tokenId)
        public
        view
        exists(tokenId)
        returns (uint256)
    {
        return
            _collectionDetails[_collectionForTokenId[tokenId]].subscriptCycle;
    }

    function subscriptPriceForTokenId(uint256 tokenId)
        public
        view
        exists(tokenId)
        returns (uint256)
    {
        return
            _collectionDetails[_collectionForTokenId[tokenId]].subscriptPrice;
    }

    function paymentTokenForTokenId(uint256 tokenId)
        public
        view
        exists(tokenId)
        returns (address)
    ...
    function minterForCollection(string memory collection)
        public
        view
    ...
    function groupIdForTokenId(uint256 tokenId, uint256 socialType)
        public
        view
        exists(tokenId)
        returns (uint256)
    {
        return
            _groupIdForCollection[_collectionForTokenId[tokenId]][socialType];
    }
    ...
    {
        return
            _collectionDetails[_collectionForGroupId[groupId][socialType]]
                .minter;
    }
        ...
        exists(tokenId)
    {
        require(
            ERC721.ownerOf(tokenId) == msg.sender,
            "PlaNFT: not token owner"
        );
        require(
            bytes(_tokenURIs[tokenId]).length == 0,
            "PlaNFT: tokenURI exists"
        );
    ...
    function equals(string memory _first, string memory _second)
        internal
        pure
        returns (bool)
    {
        return (keccak256(abi.encodePacked(_first)) ==
            keccak256(abi.encodePacked(_second)));
    }
    ...
    function mint(
        ...
        public
        payable
        socialTypeExists(socialType)
        moderate(amount)
        collectionVaild(collection)
    {
    ...
    function batchMint(
        ...
        if (_collectionDetails[collection].minter != address(0)) {
            require(
                _collectionDetails[collection].minter == owner,
                "PlaNFT: collection minter can't match"
            );
            require(
                _groupIdForCollection[collection][socialType] == groupId,
                "PlaNFT: collection for groutId can't match"
            );
        } else {
            require(
                bytes(_collectionForGroupId[groupId][socialType]).length == 0,
            ...
            _collectionDetails[collection] = Collection(
                owner,
                subscriptCycle,
                paymentToken,
                subscriptPrice
