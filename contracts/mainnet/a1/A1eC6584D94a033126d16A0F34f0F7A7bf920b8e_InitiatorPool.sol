...
// File contracts/pools/lottery/InitiatorPool.sol

// 
pragma solidity ^0.8.0;
contract InitiatorPool is Pool, Roles {
    address public FT;
    address public agentPool;

    event Withdraw(address FT, address owner, uint256 amount);

    constructor(address _platform, address ft) Roles(_msgSender()) {
        platform = _platform;
        FT = ft;
    }

    function initialize(address _agentPool) external onlyOwner {
        require(agentPool == address(0), "Initialized before");
        agentPool = _agentPool;
        IERC20(FT).approve(_agentPool, type(uint256).max);
    }

    function approve(uint256 amount) external onlyOwner {
        require(agentPool != address(0), "Not initialized");
        IERC20(FT).approve(agentPool, amount);
    }
    ...
    function withdraw(address recipient, uint256 amount) external onlyManager {
        ...
        IERC20(FT).transfer(recipient, amount);
        emit Withdraw(FT, recipient, amount);
    }
}