...
abstract contract LogicBase is ILogic, UpgradeableBase {
     ...
     * @param exactInput true : swapExactTokensForTokens, false : swapTokensForExactTokens
     * @param swapType 0 : Pancakeswap/biswap/apeswap, 1: uniswapV3
     */
    function swap(
        address swapRouter,
        uint256 amountIn,
        ...
        if (swapType == 0) {
            if (exactInput) {
                return
                    ISwapGateway(swapGateway).swapExactTokensForTokens(
                        swapRouter,
                        amountIn,
                        amountOut,
                        path,
                ...
                return
                    ISwapGateway(swapGateway).swapTokensForExactTokens(
                        swapRouter,
                        amountOut,
                        amountIn,
                        path,
        ...
        if (swapType == 1) {
            ...
            if (exactInput) {
                amountOuts = ISwapGateway(swapGateway)
                    .uniswapV3ExactTokensForTokens(
                        swapRouter,
                        amountIn,
                        amountOut,
                        path,
