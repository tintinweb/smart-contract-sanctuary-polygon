...
interface IStakingRewards {
    ...
    function pendingRewardToken() external view returns(uint);

    function users(address _user) external view returns (uint256, uint256, uint256, uint256, uint256, address, uint256);
}
...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function deposit(uint256 _amount) internal override {
        ...
        if (_amount > 0) {
            ...
            IStakingRewards(rewards).stake(_amount, strategist);
        }
    }
        ...
        returns (uint256)
    {
        ...
        IStakingRewards(rewards).unstake(_amount);
    ...
    function _getReward() internal {
        IStakingRewards(rewards).harvest();
    ...
    function emergencyWithdraw() external onlyOwner nonReentrant {
        ...
        (uint256 amount, , , , , , ) = IStakingRewards(rewards).users(address(this));
        IStakingRewards(rewards).unstake(amount);
...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant rewardToken1Addr = 0xE5417Af564e4bFDA1c483642db72007871397896;
    // One or more farm tokens
    address public constant farmToken0Addr = 0xE5417Af564e4bFDA1c483642db72007871397896;
    address public constant farmToken1Addr = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063;
    address public constant usdcAddr = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    address public constant daiAddr = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063;
    // Fee token
    address public constant feeTokenAddr = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063;
    // Routers
        ...
        )
    {
        ...
        rewardToken0ToDai = new address[](3);
        rewardToken0ToDai[0] = rewardToken0Addr;
        rewardToken0ToDai[1] = usdcAddr;
        rewardToken0ToDai[2] = daiAddr;

        rewardToken1ToDai = new address[](2);
        rewardToken1ToDai[0] = rewardToken1Addr;
        rewardToken1ToDai[1] = daiAddr;

        rewardToFarmTokenPath0 = new address[](2);
        rewardToFarmTokenPath0[0] = daiAddr;
    ...
    function harvest() public override nonReentrant {
        ...
        if (_reward0Balance > harvestCutoffBps){
            ...
            _swapUniswapWithPathForFeeOnTransferTokens(rewardToken0ToDai, swapAmount, swapRouter0Addr);
        }
        // Figure out how much LIDO reward we collected
        uint256 _reward1Balance = IERC20(rewardToken1Addr).balanceOf(address(this));
        if (_reward1Balance > harvestCutoffBps1){
            swapAmount = calculateSwapAmount(rewardToken1Addr,10000);
            _swapUniswapWithPathForFeeOnTransferTokens(rewardToken1ToDai, swapAmount, swapRouter0Addr);
        }

        // Figure out how much reward we collected
        uint256 _DaiBalance = IERC20(daiAddr).balanceOf(address(this));
        if (_DaiBalance > harvestCutoffBps2){
          // Collect fee if not at 0
          if (kogefarmFeeAmountBps > 0) {
              uint256 _feeAmount = _DaiBalance.mul(kogefarmFeeAmountBps).div(10000);
              IERC20(feeTokenAddr).safeTransfer(strategist,_feeAmount);
          }
          // Swap reward for farm tokens (want)
          // Calculate the amount to swap for the farm token.
          swapAmount = calculateSwapAmount(daiAddr,5000);
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    // Token addresses
    address public FARM_MASTER_CHEF = 0x151757c2E830C467B28Fe6C09c3174b6c76aA0c5;
    address public LP_TOKEN = 0x6E53cB6942e518376E9e763554dB1A45DDCd25c4;
    constructor()
        public
