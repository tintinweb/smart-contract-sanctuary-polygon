...
contract Lunabets is owned, ReentrancyGuard
{
    ...
    // Pre-determined LFI coinIndex
    uint32 constant unstakedLunaCoinIndex = 2;
    ...
    /**
     * Withdraws the contract LunaFi balance.
     */
    function withdrawTotalContractLuna() public onlyTreasurer {
        require(lunaContract != IERC20(address(0)), "Luna contract not yet defined");

        IERC20 token = IERC20(lunaContract);

        require(token.transfer(treasurer, lunaContractBalance), "Transfer failed");

        lunaContractBalance = 0;
    }
    ...
    function balanceSync(address payable[] memory users, uint32[] memory coinIndices, uint256[] memory changed, uint256 syncId) public onlyAgent {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            if (coinIndex == stakedLunaCoinIndex) {
                ...
                if (coinIndex == unstakedLunaCoinIndex)
                    lunaContractBalance = lunaContractBalance.add(spent);
                else
    ...
    /**
     * Complete several LFI claims at once.
     */
    function withdrawLFIMulti(uint256[] calldata claimIds, bytes[] calldata signatures) public nonReentrant {
        require(claimIds.length == signatures.length, "Array size mismatch");

        for (uint256 i = 0; i < claimIds.length; i++) {
            withdrawLFI(claimIds[i], signatures[i]);
        }
    }
