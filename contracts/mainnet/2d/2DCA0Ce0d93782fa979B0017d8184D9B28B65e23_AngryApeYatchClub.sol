...
contract AngryApeYatchClub is ERC721 {
    uint256 public totalSupply;
    uint256 public cost = 0.09 ether;
    uint256 public maxMints = 5000;
    ...
    constructor()
        ERC721("AngryApeYatchClub", "AAYC")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 9991) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeihck4ehzolz2fvx5sudwej77rmcsxyjquigzluvhdguhigbld5bwe/", Strings.toString(tokenId), ".json")
            );
    }
