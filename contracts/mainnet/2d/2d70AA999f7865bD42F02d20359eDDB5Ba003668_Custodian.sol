...
contract Custodian is AccessControl, IERC721Receiver {
    ...
    event UpdateWallet(address indexed wallet, uint256 indexed key, bool indexed add);

    bytes32 public constant SERVICE_ADMIN = keccak256("SERVICE_ADMIN");
    ...
    modifier canModify {
        if( !hasRole(DEFAULT_ADMIN_ROLE, _msgSender()) &&
            !hasRole(SERVICE_ADMIN, _msgSender())) revert NFTWalletCannotTransfer();
        _;
    }

    constructor(address superOwner, address admin) {
        _setupRole(DEFAULT_ADMIN_ROLE, superOwner);
        _setupRole(SERVICE_ADMIN, admin);
        serviceAdmins.push(admin);
    }

    function updateAdmins(address admin, bool add) public {
        require( hasRole(SERVICE_ADMIN, _msgSender()) ||
                 hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "NOT_ADMIN");
        if(add){
            _setupRole(SERVICE_ADMIN, admin);
            serviceAdmins.push(admin);
        } else {
            _revokeRole(SERVICE_ADMIN, admin);
            serviceAdmins = removeElement(serviceAdmins, admin);
        }
    }

    function revokeAllAdmins() external onlyRole(DEFAULT_ADMIN_ROLE){
        for(uint i = 0; i < serviceAdmins.length; i++){
            _revokeRole(SERVICE_ADMIN, serviceAdmins[i]);
        }
        serviceAdmins = new address[](0);
    }
    ...
    function updateKeyToWallet(uint key, address wallet, bool add) public canModify {
        if(add){
            require(walletToKey[wallet] == 0, "WALLET_ALREADY_ASSOCIATED");
            walletToKey[wallet] = key;
        }
        else if(keccak256(abi.encodePacked(walletToKey[wallet])) == keccak256(abi.encodePacked(key))) {
            delete walletToKey[wallet];
        }
    }

    function updateTokenKey(uint key, address nft, uint tokenId) public canModify {
        nftTokenToKey[nft][tokenId] = key;
    }

    function updateCoinKeyBalance(uint key, address coin, uint balance) public canModify {
    ...
    function withdrawToken(address nft, uint tokenId, uint key, address receiver) public canModify {
    ...
    function withdrawCoin(address coin, uint amount, uint key, address receiver) public canModify {
    ...
    function claimToken(uint key, address nft, uint256 tokenId) external {
        ...
        require(walletToKey[_msgSender()] == key, "NOT_USER_WALLET");
        require(nftTokenToKey[nft][tokenId] == key, "NOT_USER_TOKEN");
        withdrawToken(nft, tokenId, key, _msgSender());
    }
    ...
    function claimCoin(uint key, address coin) external {
        ...
        withdrawCoin(coin, balance, key, _msgSender());
    }
