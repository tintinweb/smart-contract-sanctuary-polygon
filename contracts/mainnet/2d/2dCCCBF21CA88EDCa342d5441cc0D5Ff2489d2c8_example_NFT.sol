...
interface burnToken{
  function burnTokens(uint _qty) external;
  function balanceOf(address owner) external view returns (uint256 balance);
}

// File: contracts/example_NFT.sol

pragma solidity >= 0.8.0 < 0.9.0;

contract example_NFT is ERC721A, Ownable, ReentrancyGuard {
  ...
  address public parentAddress = 0x062E6604fFA8D4AE459dF58AeC848a2D3171D881;
  burnToken burnContract = burnToken(parentAddress);

  constructor (
    address _signerAddress
  ) ERC721A ( "example_NFT", "ENFT" ) {
    setUriPrefix( "ipfs://example-CID/" );
...
// ~~~~~~~~~~~~~~~~~~~~ MINT Functions ~~~~~~~~~~~~~~~~~~~~
  // MINT
  function mint(uint256 _mintAmount, address _address, bytes memory sig) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    
    require(msg.sender == _address, "_address and msg.sender do not match.");
    
    require(isValidData(_address, sig) == true, "User is not whitelisted.");
