...
contract ItemCalculator is Controllable, IItemCalculator {

  struct GenerationInternalInfo {
  ...
  uint public constant DURABILITY_REDUCTION = 3;
  uint public constant AUGMENT_FACTOR = 20;
  uint internal constant _RANDOM_MAX = 1e18;

  // ---- VARIABLES ----

  /// @dev 0-100 range
  uint public override augmentGovFee;
  /// @dev 0-100 range
  uint public override repairGovFee;

  // ---- EVENTS ----

  event AugmentGovFeeChanged(uint value);
  event RepairGovFeeChanged(uint value);
  ...
  function init(address controller_) external initializer {
    ...
    augmentGovFee = 50;
    repairGovFee = 50;
    emit AugmentGovFeeChanged(50);
    emit RepairGovFeeChanged(50);
  }

  // ---- GOV ACTIONS ----

  function setAugmentGovFee(uint value) external {
    require(isGovernance(msg.sender), "Not gov");
    require(value <= 100, "Max");

    augmentGovFee = value;
    emit AugmentGovFeeChanged(value);
  }

  function setRepairGovFee(uint value) external {
    require(isGovernance(msg.sender), "Not gov");
    require(value <= 100, "Max");

    repairGovFee = value;
    emit RepairGovFeeChanged(value);
  }

  // ---- STATIC LOGIC ----

  function score(uint[] memory attributes, uint baseDurability) external pure override returns (uint) {
    return ScoreLib.itemScore(attributes, baseDurability);
  }

  function calculateRarity(uint randomSum, uint chancesSum, uint attrCounter, uint maxAttr) public pure returns (IItem.ItemRarity) {
    if (attrCounter == 0) {
      return IItem.ItemRarity.NORMAL;
    }

    uint random = randomSum / attrCounter;
    uint averageChance = chancesSum / attrCounter;

    require(random <= _RANDOM_MAX, "Too high random");

    if (random < averageChance / 4 && attrCounter == maxAttr) {
      return IItem.ItemRarity.RARE;
    } else if (random < averageChance * 3 / 4) {
      if (attrCounter > 2) {
        return IItem.ItemRarity.RARE;
      }
      return IItem.ItemRarity.MAGIC;
    } else {
      if (attrCounter > 1) {
        return IItem.ItemRarity.MAGIC;
      }
  ...
  /// @dev The bigger difference between biome level and item level the greater durability reduction (exponentially).
  function calcReduceDurability(
    uint dungeonBiomeLevel,
    uint currentDurability,
    uint _itemLevel
  ) external pure override returns (uint) {
    uint itemBiomeLevel = _itemLevel / StatLib.BIOME_LEVEL_STEP + 1;
    uint value = DURABILITY_REDUCTION;
    if (itemBiomeLevel < dungeonBiomeLevel) {
      value = DURABILITY_REDUCTION ** (dungeonBiomeLevel - itemBiomeLevel + 1);
    }
  ...
  function augmentSuccess(uint random) external pure override returns (bool) {
    // 50% chance to not augment
    return random < 5e17;
  }


  function augmentAttribute(uint value) public pure override returns (uint) {
    if (value == 0) {
      return 0;
    }
    // bonus must be not lower than 1
    return value + Math.max(Math.ceilDiv(value, AUGMENT_FACTOR), 1);
  }

  function augmentMagicAttack(IStatController.MagicAttack memory current) external pure override returns (IStatController.MagicAttack memory) {
    return IStatController.MagicAttack({
    aType : current.aType,
    min : augmentAttribute(uint(current.min)),
    max : augmentAttribute(uint(current.max)),
    attributeFactors : current.attributeFactors,
    manaConsume : current.manaConsume
    });
  }

  function generateAttributes(IItem.GenerateInfo memory _info, uint rarity)
  external override returns (uint[] memory ids, uint[] memory values, IItem.ItemRarity itemRarity) {
    IOracle _oracle = IOracle(IController(controller()).oracle());
    ids = new uint[](_info.ids.length);
    values = new uint[](_info.ids.length);
    if (_info.ids.length != 0) {

      // Fisher–Yates shuffle
      for (uint i; i < _info.ids.length; i++) {
        uint randomIndex = CalcLib.pseudoRandomInRange(i, _info.ids.length - 1);
        uint temp = _info.ids[randomIndex];
        uint temp2 = _info.mins[randomIndex];
        uint temp3 = _info.maxs[randomIndex];
        uint temp4 = _info.chances[randomIndex];

        _info.ids[randomIndex] = _info.ids[i];
        _info.ids[i] = temp;

        _info.mins[randomIndex] = _info.mins[i];
        _info.mins[i] = temp2;

        _info.maxs[randomIndex] = _info.maxs[i];
        _info.maxs[i] = temp3;

        _info.chances[randomIndex] = _info.chances[i];
        _info.chances[i] = temp4;


      }

      GenerationInternalInfo memory internalInfo = GenerationInternalInfo(0, 0, false, 0);
      for (uint i; i < _info.ids.length; ++i) {
        if (_info.chances[i] >= _RANDOM_MAX || !internalInfo.stopGenerateRandom) {
          (uint attr, uint random) = generateAttribute(_oracle, _info.mins[i], _info.maxs[i], _info.chances[i]);
          // count only random attributes for calc rarity
          if (attr != 0 && random != 0 && _info.chances[i] < _RANDOM_MAX) {
            internalInfo.randomAttrCounter++;
            internalInfo.randomSum += random;
            internalInfo.chancesSum += _info.chances[i];
          }
          if (attr != 0) {
            ids[i] = _info.ids[i];
            values[i] = attr;
          ...
          if (internalInfo.randomAttrCounter == _info.maxRandomAttributes) {
            internalInfo.stopGenerateRandom = true;
          }
        }
      }

      if (internalInfo.randomAttrCounter < _info.minRandomAttributes) {
        // currently we support only 1 mandatory attribute for reduce gas usage
        require(_info.minRandomAttributes == 1, "Too many min");
        uint i = _info.ids.length == 1 ? 0 : _oracle.getRandomNumber(_info.ids.length - 1, values[0]);
        (uint attr,) = generateAttribute(_oracle, _info.mins[i], _info.maxs[i], _RANDOM_MAX);
        ids[i] = _info.ids[i];
        values[i] = attr;
      }

      itemRarity = rarity != 0 ?
      IItem.ItemRarity(rarity) :
      calculateRarity(internalInfo.randomSum, internalInfo.chancesSum, internalInfo.randomAttrCounter, _info.maxRandomAttributes);
    } else {
      itemRarity = IItem.ItemRarity.UNKNOWN;
    }
  }

  function generateSkillAttributes(IItem.GenerateInfo memory _info, bool onlyPositiveChances)
  external override returns (uint[] memory ids, uint[] memory values) {
    IOracle _oracle = IOracle(IController(controller()).oracle());
    ids = new uint[](_info.ids.length);
    values = new uint[](_info.ids.length);
    ...
    for (uint i; i < _info.ids.length; ++i) {
      if (
        (onlyPositiveChances && _info.chances[i] > 0)
        || (!onlyPositiveChances && _info.chances[i] == 0)
      ) {
        (uint attr,) = generateAttribute(_oracle, _info.mins[i], _info.maxs[i], _RANDOM_MAX);
  ...
  function generateAttack(IAttackItem.GenerateAttackInfo memory _info) external override returns (IStatController.MagicAttack memory) {
    IOracle _oracle = IOracle(IController(controller()).oracle());

    return IStatController.MagicAttack({
    aType : _info.attackType,
    min : _info.damageMin,
    max : generateAttributeSimple(_oracle, _info.damageMin, _info.damageMax, _RANDOM_MAX),
    attributeFactors : _info.attributeFactors,
    manaConsume : generateAttributeSimple(_oracle, _info.manaConsumeMin, _info.manaConsumeMax, _RANDOM_MAX)
    });
  }

  function generateAttributeSimple(IOracle _oracle, uint min, uint max, uint chance) public returns (uint) {
    (uint value,) = generateAttribute(_oracle, min, max, chance);
    return value;
  }

  function generateAttribute(IOracle _oracle, uint min, uint max, uint chance) public returns (uint attr, uint rnd) {
    if (max != 0) {
      if (chance >= _RANDOM_MAX) {
        if (max == min) {
          return (min, 0);
        }
        // return zero random - no need to calc rarity for mandatory elements
        return (min + (_oracle.getRandomNumber(_RANDOM_MAX, max) % (max - min + 1)), 0);
      } else {
        uint random = _oracle.getRandomNumber(_RANDOM_MAX, max);
        if (random < chance) {
          uint r = _RANDOM_MAX - random * (_RANDOM_MAX / chance);
          return (min + ((r * (max - min + 1)) / _RANDOM_MAX), random);
        } else {
          return (0, 0);
        }
      }
  ...
  function consumableInfoToGenerateInfo(IConsumableItem.GenerateConsumableInfo memory consumableGenInfo) external pure override returns (IItem.GenerateInfo memory pos, IItem.GenerateInfo memory neg) {
  unchecked{
    uint posLength;
    uint negLength;
    for (uint i; i < consumableGenInfo.ids.length; ++i) {
      if (consumableGenInfo.increase[i]) {
        ++posLength;
      } else {
        ++negLength;
      }
    ...
    uint[] memory idsPos = new uint[](posLength);
    uint[] memory minsPos = new uint[](posLength);
    uint[] memory maxsPos = new uint[](posLength);
    uint[] memory chancesPos = new uint[](posLength);

    uint[] memory idsNeg = new uint[](negLength);
    uint[] memory minsNeg = new uint[](negLength);
    uint[] memory maxsNeg = new uint[](negLength);
    uint[] memory chancesNeg = new uint[](negLength);

    uint posCounter;
    uint negCounter;
    for (uint i; i < consumableGenInfo.ids.length; ++i) {
      if (consumableGenInfo.increase[i]) {
        idsPos[posCounter] = consumableGenInfo.ids[i];
        minsPos[posCounter] = consumableGenInfo.values[i];
        maxsPos[posCounter] = consumableGenInfo.values[i];
        chancesPos[posCounter] = _RANDOM_MAX;
        ++posCounter;
      } else {
        idsNeg[posCounter] = consumableGenInfo.ids[i];
        minsNeg[posCounter] = consumableGenInfo.values[i];
        maxsNeg[posCounter] = consumableGenInfo.values[i];
        chancesNeg[posCounter] = _RANDOM_MAX;
        ++negCounter;
      }
    }
    return (
    ...
    maxs : maxsPos,
    chances : chancesPos,
    minRandomAttributes : 0,
    maxRandomAttributes : 0
    }),
    IItem.GenerateInfo({
    ids : idsNeg,
    mins : minsNeg,
