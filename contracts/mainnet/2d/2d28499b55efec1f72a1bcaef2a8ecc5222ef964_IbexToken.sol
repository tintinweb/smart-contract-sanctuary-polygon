...
contract IbexToken is ERC20 {
	...
	function lastAPR() public view returns (uint) {
		...
		for (uint i; i < usedBorrowables.length; ++i) {
			...
			tmpBal += borrowable.balanceOf(address(this)) * tmpExchRate;

			tmpAPR += tmpBal * borrowable.borrowRate() * 365 * 24 * 60 * 60 * (1e18 - borrowable.reserveFactor()) * borrowable.totalBorrows() / (borrowable.totalSupply() * tmpExchRate);

		}

		return tmpAPR / lastUnderlyingValue();
	...
	function incrBorrowable(address _borrowable, uint _weight) public onlyOwner() {
		...
		require(uint(borrowables[_borrowable].weight) < _weight, "weight too small");
