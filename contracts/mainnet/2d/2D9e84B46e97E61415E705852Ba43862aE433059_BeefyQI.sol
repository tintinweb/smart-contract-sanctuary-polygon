...
contract BeefyQI is ERC20, ReentrancyGuard, QiManager {
    ...
    ) QiManager(_keeper, _qiBatch, _rewardPool) ERC20(_name, _symbol) {
        eQI = IeQI(_eQI);
        want = IERC20(eQI.Qi());
    ...
    function withdraw(uint256 _amount) external {
        require(_amount <= balanceOfWant() - outstandingReward(), "Not enough QI");
...
contract QiManager is Ownable, Pausable {
     ...
     * {qiBatch} - Address for distributing locked want rewards.
     * {rewardPool} - Address for QI rewards distribution.
     */
    address public keeper;
    address public qiBatch;
    ...
    event NewQiBatch(address oldBatch, address newBatch);
   ...
   constructor(
     ...
     * @param _qiBatch new qiBatch address.
     */
    function setQiBatch(address _qiBatch) external onlyOwner {
        emit NewQiBatch(qiBatch, _qiBatch);
        qiBatch = _qiBatch;
    }
