...
contract AssetTokenization is Initializable, ERC1155Upgradeable, OwnableUpgradeable {
    ...
    function initialize(address _addressUSDC) public initializer {
    USDC = IERC20Upgradeable(_addressUSDC);
    __Ownable_init();
    }
    ...
    function setMaintenancePer(uint256 _percentValue) public onlyOwner {
        require(_percentValue <= 50, "Maintenance Fees of 50% or more");
        maintenanceFees = _percentValue;
    }
    function setVaultAddress(address _vaultAddress) public onlyOwner {
        require(_vaultAddress != address(0), "Zero address is not allowed");
    ...
    function updateRent(string memory _propertyId, uint256 _updatedAmount) public onlyOwner {
        require(_updatedAmount > 0, "Not valid amount");
    ...
    function addProperty(string memory _userId, address _user, string memory _propertyId, uint256 _propertyPrice,
        ...
        require(_rentAmount > 0, "Not valid amount");
        ...
        propertyDetails[_propertyId].totalInvestmentPortion = _investmentPortion;
        //investmentPortion is the part of the property the owner wants investment for
        propertyDetails[_propertyId].leftInvestmentPortion = _investmentPortion;
        _mint(vaultAddress, propertyDetails[_propertyId].propertyTokenId, _investmentPortion, "");
        //Create Property specific fungible tokens based on ERC1155 standard
        tokenDetails[propertyDetails[_propertyId].propertyTokenId].totalTokens = _investmentPortion;
        //The amount of property specific tokens created is equals to investment portion
    ...
    function invest(string memory _investorUserId, address _user, string memory _propertyId, uint256 _amount) public onlyOwner {
        require(propertyDetails[_propertyId].isPropertyAdded, "You are not have a Property");
        require(propertyDetails[_propertyId].leftInvestmentPortion >= _amount, "Insufficient investment portion");
        require(_amount > 0, "Not valid token amount");
        ...
        if (!userDetails[_investorUserId].isUserAdded) {
            ...
            isWhiteListed[vaultAddress][_user] = true;
            isWhiteListed[_user][vaultAddress] = true;

            emit UserAdded(_investorUserId, userDetails[_investorUserId].user);
        }

        safeTransferFrom(vaultAddress, userDetails[_investorUserId].user, propertyDetails[_propertyId].propertyTokenId, _amount, "");
    ...
    function transferDividend(string memory _propertyId) public onlyOwner returns (userDividend[] memory) {
        require(USDC.balanceOf(vaultAddress) >= propertyDetails[_propertyId].rentAmount, "Insufficient vaultAddress balance");
        ...
        for (uint256 i = 0; i < investors[_propertyId].length; i++) {
            ...
            if (isInvestor[_propertyId][_investorUserId]) {
                ...
                USDC.transferFrom(vaultAddress, _investor, _dividendAmount * 1e6);
    ...
    function withdrawInvestment(string memory _investorUserId, string memory _propertyId, uint256 _amount) public {
        ...
        safeTransferFrom(userDetails[_investorUserId].user, vaultAddress, propertyDetails[_propertyId].propertyTokenId, _amount, "");
        propertyWiseInvestment[_investorUserId][_propertyId] -= _amount;
        propertyDetails[_propertyId].leftInvestmentPortion += _amount;
