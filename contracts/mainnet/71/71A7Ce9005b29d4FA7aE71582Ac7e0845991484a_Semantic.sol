...
contract Semantic is ISemantic, AccessControl,ERC721 {
    ...
    function claim(ClaimOperateData[] memory claimOperateDatas) external {
     ...
     function testSign(ClaimOperateData memory operateData) external  view returns(address caller,string memory callerStr,string memory rdf,string memory signOriginalData,address singer){
            require(
                operateData.expireTime > block.timestamp,
                "signData has expired"
            );
