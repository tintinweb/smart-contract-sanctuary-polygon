...
contract GoldenEyeCoin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GECTokenDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Golden Eye Coin", "GEC") {
        dividendTracker = new GECTokenDividendTracker();

        teamWallet = 0xADb56b91253fbB1a546daB4ddc5F54225E1EE5dc;
        devWallet = 0xA3f74E130546D43D1C052d860016f22ff263c238;
        marketingWallet = 0xA768Bd56518baa2A312D6e117D70355eC0cb89e7;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "GEC: The dividend tracker already has that address"
        );

        GECTokenDividendTracker newDividendTracker = GECTokenDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "GEC: The new dividend tracker must be owned by the GEC token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "GEC: The router already has that address"
        );
    ...
    function updateDevAddress(address newAddress) private onlyOwner {
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "GEC: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "GEC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "GEC: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "GEC: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "GEC: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[to]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 maticSwap = buyDevFee.add(buyTeamFee).add(
                    buyMarketingFee
                );
                uint256 swapTokens = contractTokenBalance.mul(maticSwap).div(
                    buyTotalFees
                );
                swapAndSendMatic(swapTokens, maticSwap);
                ...
                if (rewardEnable) {
                    if (_dividendToken == uniswapV2Router.WETH()) {
                        ...
                        swapAndSendDividendsInMATIC(sellTokens);
                    } else {
                        uint256 sellTokens = balanceOf(address(this));
    ...
    function swapAndSendMatic(uint256 tokenAmount, uint256 totalFee) private {
    ...
    function swapAndSendDividendsInMATIC(uint256 tokens) private {
        uint256 currentMATICBalance = address(this).balance;
        swapTokensForMATIC(tokens);
        uint256 newMATICBalance = address(this).balance;

        uint256 dividends = newMATICBalance.sub(currentMATICBalance);
...
contract GECTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("GEC_Dividend_Tracker", "GEC_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "GEC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "GEC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GEC contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "GEC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "GEC_Dividend_Tracker: Cannot update claimWait to same value"
        );
