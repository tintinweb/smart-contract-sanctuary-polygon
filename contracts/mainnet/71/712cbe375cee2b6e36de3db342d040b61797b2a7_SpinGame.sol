...
contract SpinGame is Ownable
{
    ...
    //We also emit an event (SpinSucceeded) which can be viewed and includes the random number.
    //0 = Purple
    //1 = Yellow
    //2 = Blue
    //3 = Orange
    function fulfillRandomWords(
        ...
        vrfRequests[requestId].randomWord = randomWords[0] % 4;

        emit SpinSucceeded(requestId, (randomWords[0] % 4));
    }
    //
    ...
    //is the random number (Based on the RequestStatus struct. This can also viewed from the event system as we fire off SpinSucceded with the random number included.
    ...
    //Admin
    function spin() public onlyOwner returns (uint256 requestId)
    {
