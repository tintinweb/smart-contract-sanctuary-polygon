...
contract NFTChildToken is NFTToken {
    ...
    function safeMint(address to) public onlyTransactor {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
    }

    function safeMintBatch(address[] memory addresses) external onlyTransactor {
        require(addresses.length > 0, "The addresses length should be greater than 0");
        for (uint256 i = 0; i < addresses.length; i++) {
            safeMint(addresses[i]);
        }
    }

    function safeDevMint(address to) public onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
    }

    function devMintBatch(address[] memory addresses) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(addresses.length > 0, "The addresses length should be greater than 0");
        for (uint256 i = 0; i < addresses.length; i++) {
            safeDevMint(addresses[i]);
        }
    }
}