...
contract Marketplace is BaseContract {
  struct NftListing {
    ...
    address poolAddress;
    bool isUniSwapV3;
    bool isR0DivR1; // use calculate price token / usd -> use for uniswapV2
    bool enable;
  }
  ...
  event NftListed(address indexed nftContractAddress, address owner, NftListing nftListing, uint timestamp);
  ...
  function setNFTListing(
    ...
    emit NftListed(_nftContractAddress, _owner, nftListings[_nftContractAddress], block.timestamp);
  }
  ...
  function takeOrder(address _nftContractAddress, uint _tokenId, address _paymentAddress, uint _weekNumber) external validOrder(_nftContractAddress, _tokenId) {
    ...
    uint currentPrice = _getCurrentPrice(_nftContractAddress, _tokenId, _weekNumber);
  ...
  function getNftListed(address _nftContractAddress) external view returns(
    ...
    return (nftListing.user, nftListing.paymentAddresses, nftListing.uri, nftListing.poolAddress, nftListing.isUniSwapV3, nftListing.isR0DivR1, nftListing.enable);
  }
  ...
  function _getCurrentPrice(address _nftContractAddress, uint _tokenId, uint _weekNumber) private view returns (uint) {
    Order storage order = orders[_nftContractAddress][_tokenId];
    NftListing storage nftListing = nftListings[_nftContractAddress];
    ...
    if (nftListing.isUniSwapV3) {
      uint decimalTokenIn = ERC20(usdtTokenAddress).decimals();
      uint tokenPrice = getTokenPrice(nftListing.poolAddress, decimalTokenIn);
      return currentPrice * (10 ** decimalTokenIn) / tokenPrice;
    } else {
      (uint r0, uint r1) = ILPToken(nftListing.poolAddress).getReserves();
      uint tokenPrice = nftListing.isR0DivR1 ? r0 * DECIMAL9 / r1 : r1 * DECIMAL9 / r0;
