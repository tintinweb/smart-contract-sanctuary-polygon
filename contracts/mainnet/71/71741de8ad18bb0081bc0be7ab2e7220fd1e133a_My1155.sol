...
//import "hardhat/console.sol";
contract My1155 is ERC1155PresetMinterPauser, Ownable, RoyaltiesV2Impl{
    using Address for address;
    ...
    mapping(uint256=>bool) private _born;
    ...
    function uri(uint256 id_) public view  override returns (string memory) {
        require(_born[id_], "URI query for nonexistent token");

        token memory tkn = _tokens[id_];

        bytes memory json = abi.encodePacked(
            '{"name": "', tkn.name, '", "description": "', tkn.description , '", "image": "ipfs://', tkn.image, 
            '", "animation_url": "', tkn.animation_url, '", "atttributes": "', tkn.attributes, '", "external_url": "',
            tkn.external_url, '"}'
        );
        return string(abi.encodePacked("data:application/json;base64,", Base64.encode(json)));
    }

    function setMetadata(uint256 id_, string memory name_, string memory description_, string memory image_, string memory animation_url_, string memory attributes_, string memory external_url_, bool freeze_) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Must be ADMIN");
        require(_tokens[id_].freezed == false, "alreadly frozen");
        if(!_born[id_]){
            _born[id_] = true;
            _tokenList.push(id_);
        }
        token memory tkn;
        tkn.name = name_;
        tkn.description = description_;
        tkn.image = image_;
        tkn.animation_url = animation_url_;
        tkn.attributes = attributes_;
        tkn.external_url = external_url_;
        tkn.freezed = freeze_;
        _tokens[id_] = tkn;
    }

    function setAttributes(uint256 id_, string memory attributes_) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Must be ADMIN");
        require(_born[id_], "query for nonexistent token");
        _tokens[id_].attributes = attributes_;
    }

    function freeze(uint256 id_) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Must be ADMIN");
        require(_born[id_], "query for nonexistent token");
        require(_tokens[id_].freezed == false, "alreadly frozen");
        _tokens[id_].freezed = true;
    }

    function isFrozen(uint id_) public view returns(bool){
        require(_born[id_], "query for nonexistent token");
        return _tokens[id_].freezed;
    }

    function tokenList() public view returns(uint256[] memory){
        return tokenListWithPager(0, 100);
    }

    function tokenListWithPager(uint256 page_, uint256 n_) public view returns(uint256[] memory){
        uint256 m;
        if((page_ + 1)*n_ <=_tokenList.length){
            m = n_;
        }else{
            m = _tokenList.length - page_*n_;
        }
        uint256[] memory array = new uint256[](m);
        uint256 j;
        for(uint256 i = page_ * n_; (j < n_)&&(i<_tokenList.length); i++ ){
            array[j++] = _tokenList[i]; 
        }
        return array;
    }

    function tokenListNumPage(uint256 n_) public view returns(uint256){
        uint256 num;
        num = (_tokenList.length / n_);
        if((_tokenList.length % n_)!=0){
            num++;
        }
        return num;
    }

    function tokenNum() public view returns(uint256){
...
// File contracts/rarible/IRoyaltiesProvider.sol
...
// File contracts/rarible/RoyaltiesV2TestImpl.sol
