...
contract moderntst2haiku is ERC721Enumerable, Ownable , whitelistCheck{
    using Strings for uint;
    using ECDSA for bytes32;

    uint public moderntst2haiku_PRESALE = 1; //presale
    uint public moderntst2haiku_WALLET_PRESALE_LIMIT = 10; //moderntst2haikuperwalletInWholeDuration
    uint public moderntst2haiku_WALLET_PUBLICSALE_LIMIT = 20; //moderntst2haikuperwalletInWholeDuration
    uint public moderntst2haiku_MAX_COUNT = 1400; //maxSupply
    uint public totalTokensSoldInPresale;
    uint public preSaleAmountMinted;

    uint public moderntst2haiku_MAINSALE_PRICE = 0.5 ether; //priceInMainSale
    uint public moderntst2haiku_PRESALE_PRICE = 0.3 ether; //priceInPreSale
    ...
    string private _tokenBaseURI = "https://gateway.pinata.cloud/ipfs/QmSkBczHPpQKavkw7qBjfnY2MRvw18h8mkEGQkrHNYaKV7/";
    ...
    constructor() ERC721("moderntst2 Haiku", "moderntst2haiku") {}

    function setmoderntst2haiku_MAX_COUNT(uint _maxCount) external onlyOwner{
        moderntst2haiku_MAX_COUNT = _maxCount;
    }

    function setmoderntst2haiku_PRESALE(uint _presale) external onlyOwner{
        moderntst2haiku_PRESALE = _presale;
    }

    function setmoderntst2haiku_WALLET_PRESALE_LIMIT(uint _presaleLimit) external onlyOwner{
        moderntst2haiku_WALLET_PRESALE_LIMIT = _presaleLimit;
    }

    function setmoderntst2haiku_WALLET_PUBLICSALE_LIMIT(uint _publicSaleLimit) external onlyOwner{
        moderntst2haiku_WALLET_PUBLICSALE_LIMIT = _publicSaleLimit;
    }

    function setmoderntst2haiku_MAINSALE_PRICE(uint _mainSalePrice) external onlyOwner{
        moderntst2haiku_MAINSALE_PRICE = _mainSalePrice;
    }

    function setmoderntst2haiku_PRESALE_PRICE(uint _presalePrice) external onlyOwner{
        moderntst2haiku_PRESALE_PRICE = _presalePrice;
    }
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= moderntst2haiku_MAX_COUNT, "EXCEED_MAX");
    ...
    function buy(uint tokenQuantity) external payable {
        ...
        require (tokenQuantity <= moderntst2haiku_WALLET_PUBLICSALE_LIMIT);
        require (saleListPurchase[msg.sender] + tokenQuantity <= moderntst2haiku_WALLET_PUBLICSALE_LIMIT, "MAX MINT COMPLETE");
        require (totalSupply() + tokenQuantity <= (moderntst2haiku_MAX_COUNT - preSaleAmountMinted), "EXCEED_MAX");
        require (moderntst2haiku_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
    ...
    function founderMint(uint tokenQuantity) external onlyOwner {
        require(totalSupply() + tokenQuantity <= moderntst2haiku_MAX_COUNT, "EXCEED_MAX");
        for(uint i = 0; i < tokenQuantity; i++) {
        _safeMint(msg.sender, totalSupply() + 1);
        }
    }
    ...
    function presaleBuy( Whitelist memory whitelist, uint tokenQuantity) external payable{
        require(presaleLive, "PRESALE_CLOSED");//moderntst2haiku_WALLET_PRESALE_LIMIT
        require(getWhitelist(whitelist), "DIRECT_MINT_DISALLOWED");
        require(whitelist.userAddress == msg.sender);
        require(preSaleAmountMinted + tokenQuantity <= moderntst2haiku_PRESALE, "EXCEED_PRIVATE");
        require(presalerListPurchases[msg.sender] + tokenQuantity <= moderntst2haiku_WALLET_PRESALE_LIMIT, "EXCEED_ALLOC");
        require(moderntst2haiku_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
