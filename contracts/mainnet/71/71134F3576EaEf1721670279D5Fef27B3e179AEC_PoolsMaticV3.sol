...
contract PoolsMaticV3 is Ownable, ReentrancyGuard {
    ...
    constructor(IRefferal _refer, address gnosisSafeAddress, ILiquidity _liquidity, address mm, address trend) {
        require(gnosisSafeAddress != address(0), "Pools::setGnosisSafe: invalid input");
        require(mm != address(0), "Pools::mm: invalid input");
        refer = _refer;
    ...
    function currentRewardTREND1(address user) public view returns(
        uint _totalMined,
        uint _spendPeriods,
        uint _currentSpeed,
uint _rw
...
//        uint radix = trendPrice >= 1 ether ? 1 ether * 10000 / trendPrice : 10000; // (1/trendPrice)
    ...
    function claimRewardTREND() external nonReentrant {
        ...
        if(reward > 0) {
            ...
            userClaimMined[_msgSender()].push(Claim(getPeriodMined(), reward, u.remain, u.mineSpeed + u.mineSpeedLevel));
            u.claimed += reward;
            u.remain -= reward;
            totalClaimed += reward;
        }
