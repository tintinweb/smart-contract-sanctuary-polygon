...
contract FBET is IERC20, protected
{
    ...
    uint free_qty = 100000000000000000000; // 100 MATIC
    ...
    function BETTING_place_bet_FREE(uint256 id, uint256 side) public safe {
        ...
        bets[id].side_balance["MATIC"]["MATIC"][side] += free_qty;
        betters[msg.sender].in_bet[id] = true;
        betters[msg.sender].bet_side[id] = side;
        betters[msg.sender].bet_chain[id] = "MATIC";
        betters[msg.sender].bet_coin[id] = "FBET";
        betters[msg.sender].bet_value[id] = free_qty;
        emit place_bet(msg.sender, id, side, "MATIC", "MATIC", free_qty);
    }

    /// @dev native betting with MATIC
    function BETTING_place_bet_MATIC(uint256 id, uint256 side) public safe payable {
        require(!betters[msg.sender].in_bet[id], "Already in bet");
        require(!betters[msg.sender].banned, "Banned");
        require(bets[id].active, "Bet not active");
        require(block.timestamp < (bets[id].end_time - 5 minutes), "Expired bet");
        bets[id].side_balance["MATIC"]["MATIC"][side] += msg.value;
        betters[msg.sender].in_bet[id] = true;
        betters[msg.sender].bet_side[id] = side;
        betters[msg.sender].bet_chain[id] = "MATIC";
        betters[msg.sender].bet_coin[id] = "MATIC";
        betters[msg.sender].bet_value[id] = msg.value;
        emit place_bet(msg.sender, id, side, "MATIC", "MATIC", msg.value);
    }
    ...
    function BETTING_place_bet_FBET(uint256 id, uint256 side, uint256 qty) public safe {
        ...
        bets[id].side_balance["MATIC"]["FBET"][side] += qty;
        betters[msg.sender].in_bet[id] = true;
        betters[msg.sender].bet_side[id] = side;
        betters[msg.sender].bet_chain[id] = "MATIC";
        betters[msg.sender].bet_coin[id] = "FBET";
        betters[msg.sender].bet_value[id] = qty;
        emit place_bet(msg.sender, id, side, "MATIC", "FBET", qty);
    }

    /// @dev registering a bet outside MATIC and FBET is done by using the middleware and relative wallets
    ...
    function BETTING_cancel_single_bet(address actor, uint256 id) public safe returns(uint256, string memory, string memory, bool){
        ...
        if(compareStrings(is_chain, "MATIC") && compareStrings(is_currency, "MATIC")) {
            // Payout in MATIC
            _MATIC_payout(refund, actor);
            emit cancel_bet(actor, id, refund, is_chain, is_currency, true);
            return (refund, is_chain, is_currency, true);
        } else if (compareStrings(is_chain, "MATIC") && compareStrings(is_currency, "FBET")) {
            // Payout in FBET
    ...
    function _MATIC_payout(uint refund_MATIC, address actor) private {
            require(address(this).balance >= refund_MATIC, "Not enough MATIC");
            payable(actor).transfer(refund_MATIC);
    }
    ...
    function BETTING_issue_bet(uint256 sides, uint256 starting, bytes32 _name_, uint256 ending, string[] calldata chains, string[] calldata wallet) public onlyAuth {
    ...
    function BETTING_payout_bet(uint256 id) public safe returns(uint, string memory, string memory, bool) {
        ...
        if(compareStrings(payout_chain, "MATIC") && compareStrings(payout_coin, "MATIC")) {
            _MATIC_payout(payout_value, msg.sender);
            emit pay_bet(msg.sender, id, payout_value, payout_chain, payout_coin, true);
            return (payout_value, payout_chain, payout_coin, true);
        } else if(compareStrings(payout_chain, "MATIC") && compareStrings(payout_coin, "FBET")) {
