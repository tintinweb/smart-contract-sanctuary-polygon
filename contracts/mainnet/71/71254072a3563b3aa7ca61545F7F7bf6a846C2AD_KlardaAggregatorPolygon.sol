...
contract KlardaAggregatorPolygon is KlardaAuth {
    ...
    function getETHPrice() public view returns (uint256 reserveETH, uint256 reserveUSDT) {
        // get reserves 
        (reserveETH, reserveUSDT) = getReserves(0, wmatic, usdt);
        ...
        for(uint256 i=0; i<dexCount; i++){
            ...
            for(uint256 n=0; n<baseTokens.length; n++) {
                ...
                if(nextPair != address(0)) {
                    if (n == 0) {
                        uint256 bnbAmount = IBEP20(wmatic).balanceOf(nextPair);
                        (uint256 rA, uint256 rB) = getETHPrice();
                        // Use this for not divide for zero problem
                        lpValue += (bnbAmount*2*rB*10**18)/(rA*10**6);
                    } else {
