...
contract PoolsMaticDebug is Ownable, ReentrancyGuard {
...
//            from = _refferBy;
//            uint volume = getVolumeOnTre(_refferBy);
//            uint lock = getUserTotalLock(_refferBy);
//            uint direct;
//            uint downLine;
//            (direct, downLine) = getChildren(_refferBy);
//            if(volume >= 50_000 ether && volume < 200_000 ether && lock >= 500 ether && direct >= 2 && downLine >= 10) {
//                if(userRank[_refferBy] == 0) {
    ...
    function giveComm(uint amount, uint pid) internal {
        ...
        for(uint i = 0; i <= 7; i++) {
            ...
            if((i == 7 || from == _refferBy)) {
                ...
                uint comm = totalComm / (2 ** (i+1));
                uint totalLock = getUserTotalLock(_refferBy);
                if(totalLock > 0) {
                    remainComm[_refferBy] += comm;
                    currentComm -= comm;
                }
