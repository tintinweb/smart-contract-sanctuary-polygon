    ...
    ERC20VotesUpgradeable
{
    ...
    function getRewards(
        ...
        if (balanceOf(_benefiter) > 0) {
            ...
            int256 accumulatedReward = int256(
                (balanceOf(_benefiter) * accRewardPerShare) /
            ...
            pending = pendingRewards[_benefiter];
        }
    }
    ...
    function unstakeMax() public {
        //added by etherauthority
        require(
            (tx.origin == msg.sender && !FrozenAccount[msg.sender]) ||
                whitelistedAccount[msg.sender],
            "Invalid caller"
        );
    ...
    ///param _state the value whether true/false to set the whitelist state to active or now
    ...
    ///param _state the value whether true/false to set the frozen state to active or now
    ...
    function claimRewards(address to) public {
        ...
        if (balanceOf(msg.sender) > 0) {
            ...
            _pendingReward =
                uint256(accumulatedReward - user.rewardDebt) +
                pendingRewards[msg.sender];
            user.rewardDebt = accumulatedReward;
        } else {
            _pendingReward = pendingRewards[msg.sender];
        }
    ...
    function storeBets(
        ...
        for (uint256 i = 0; i < betinformation.length; i++) {
            //added by etherauthority
            if (
                betinformation[i].Id > 0 &&
                betInfoWithId[betinformation[i].Id].Id == 0
            ) {
                ...
                betInfoWithId[betinformation[i].Id].parlay = betinformation[i]
                    .parlay;
                betInfoWithId[betinformation[i].Id].user = betinformation[i]
                    .user;
                ...
                betInfoWithId[betinformation[i].Id].payout = betinformation[i]
                    .payout;
                betInfoWithId[betinformation[i].Id].commission = betinformation[
                    i
                ].commission;
    ...
    function updateBets(
        ...
        for (uint256 i = 0; i < _Id.length; i++) {
            ...
            if (
                betInfoWithId[_Id[i]].parlay &&
                betInfoWithId[_Id[i]].result == 0
            ) {
