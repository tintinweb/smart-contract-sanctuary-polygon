...
contract YespXStakeVerifier is Ownable, AxelarExecutable {
    ...
    string constant SOURCE_CHAIN = "Ethereum";
    string public sourceAddress = ""; //TODO

    constructor()
    AxelarExecutable(0x6f015F16De9fC8791b234eF68D486d2bF203FBA8) // Polygon
    {
    ...
    function setSourceAddress(string calldata sourceAddress_) external onlyOwner {
    ...
    function _execute(
        ...
        if (packHash(sourceChain_) != packHash("Ethereum"))
            revert AxelarBadSourceChain();
        if (packHash(sourceAddress_) != packHash(sourceAddress)) 
            revert AxelarBadSourceAddress();
        (address user, uint256 stakedAmount) = abi.decode(payload_, (address, uint256));
        amountStaked[user] = stakedAmount;
        stakeModifiedTimestamp[user] = block.timestamp;
        if (firstStakedTimestamp[user] == 0) firstStakedTimestamp[user] = block.timestamp;
    }

    function packHash(string memory data) internal pure returns (bytes32 dataHash) {
