...
contract Silo is Initializable, KeeperCompatibleInterface {
    ...
    address public constant GOV  = 0xEe5578a3Bab33F7A56575785bb4846B90Be37d50;
    ...
    event SiloUpkeepCall(uint256 siloID, uint256 actionType);
    ...
    function setMinGravityReward(uint256 _reward)
        external
    {
        require(msg.sender == Factory.ownerOf(SILO_ID) || msg.sender == factory,"wrong caller");
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        if(pendingReward >= minGravityGovernance){
            ...
            upkeepNeeded = true;
        }else  if (status == Statuses.UNWIND) {
            performData = abi.encode(true, 0);
            if (
                withdrawAmount > 0 &&
            ...
            if (siloDelay != 0 && (deposited || possibleReinvestSilo())&& status == Statuses.MANAGED) {
    ...
    function performUpkeep(bytes calldata performData)
        ...
        onlySiloManager
    {
        (bool keeperExit, uint task) = abi.decode(performData, (bool,uint));
        if(task == 2){
            ...
            if (keeperExit) {
                ...
                if (deposited) {
                    ...
                    _exitStrategy(0);
                    emit SiloUpkeepCall(SILO_ID, 0);
                }
            } else {
                _runStrategy();
                emit SiloUpkeepCall(SILO_ID, 1);
            }
        }
    ...
    function setStrategy(
        ...
        for (uint256 i; i < _implementations.length; ) {
    ...
    function _exitStrategy(uint256 _requestedOut) internal {
        ...
        if (withdrawLimitAction) {
            ...
            if (status == Statuses.UNWIND) {
                withdrawTokens(Factory.ownerOf(SILO_ID));
            }

            if (currentBalance <= Factory.minBalance()) {
                deposited = false;
                status = Statuses.DORMANT;
            } else {
        ...
        )
    {
        for (uint256 i; i < strategy.length; ) {
            (
                ...
                availableBlock,
                pendingReward
            ) = IAction(strategy[i].implementation).withdrawLimit(
                strategy[i].configurationData
            );
    ...
    function _setTriggerConfigData(bytes memory configData) internal {
        (address[4] memory _inputs, , uint256[4] memory _triggers) = abi.decode(
            configData,
            (address[4], address[4], uint256[4])
        );
        unchecked {
            for (uint256 i; i < 4; i++) {
