...
contract LoanNFT is ERC721URIStorage, ILoanNFT {
  ...
  event LoanCreated(address indexed _lendingPoolAddress, uint256 indexed _loanID, string _purpose, string _description);
  ...
  constructor(address _addressRegistry) ERC721("Loan NFT", "COPNFT") {
    ...
    )
  {
    ...
    if (loan.status == LoanStatus.DISBURSED) {
      uint256 feeRepayment = _loanRepaymentAmount.wmul(loan.balances.fee).wdiv(loan.balances.balance);
      ...
      loan.balances.balance = loan.balances.balance.sub(_loanRepaymentAmount);
      ...
      address copraGlobalAddress = addressRegistry.get("CopraGlobal");
      (, uint256 originatorFee, uint256 governanceFee) = (ICopraGlobal(copraGlobalAddress)).getFees();
  ...
  function disburse(uint256 _loanID) external override {
    ...
    loans[_loanID].balances.outstanding = loans[_loanID].principal;
  ...
  function _updateLoanBalances(Loan memory _loan) internal view returns (Loan memory) {
    ...
    uint256 lastUpdatedAt = _loan.balances.lastUpdatedAt;
    uint256 daysSinceLastUpdate = 1;
    while (block.timestamp - lastUpdatedAt >= 1 days) {
      ...
      if (_loan.balances.lastUpdatedAt + (daysSinceLastUpdate * 1 days) > _loan.repaymentDate) {
      ...
      if (lastUpdatedAt == 0) {
        ...
        lastUpdatedAt = lastUpdatedAt + 1 days;
      }
      daysSinceLastUpdate += 1;
    }
    _loan.balances.lastUpdatedAt = lastUpdatedAt;
    return _loan;
  }
  ...
  function mintNewLoan(LoanRegistrationParams calldata _loanParams) external override returns (uint256) {
    address lendingPoolRegistryAddress = addressRegistry.get("LendingPoolRegistry");
    ILendingPoolRegistry lpRegistry = ILendingPoolRegistry(lendingPoolRegistryAddress);
    require(
    ...
    Loan memory loan = Loan({
      repaymentDate: _loanParams.repaymentDate,
      principal: _loanParams.principal,
      amountRepaid: 0,
      lateFee: _loanParams.lateFee.toPercent(),
      borrower: _loanParams.borrowerAddress,
      disbursementDate: _loanParams.timeDisbursed,
      actualTimeDisbursed: 0,
      dailyRate: _loanParams.dailyRate.toPercent(),
    ...
    _setTokenURI(newloanID, _loanParams.purpose);
    loanIDs.increment();
    loans.push(loan);
    ...
    emit LoanCreated(_msgSender(), newloanID, _loanParams.purpose, _loanParams.description);
    return newloanID;
  }
  ...
  function _calculateLoanValue(Loan memory _loan) internal view returns (uint256) {
    ...
    address copraGlobalAddress = addressRegistry.get("CopraGlobal");
    uint256 maxLateDays = (ICopraGlobal(copraGlobalAddress)).getMaxLateDays().toWAD();
