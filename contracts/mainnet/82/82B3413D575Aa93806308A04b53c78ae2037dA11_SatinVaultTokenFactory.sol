...
interface ISatinVaultTokenFactory {
...
import "@openzeppelin/contracts/ownership/Ownable.sol";
import "./interfaces/IBaseV1Pair.sol";
import "./SatinVaultToken.sol";
import "./interfaces/ISatinVaultTokenFactory.sol";

contract SatinVaultTokenFactory is Ownable, ISatinVaultTokenFactory {
    address public optiSwap;
    address public router;
    address public voter;
    address public pairFactory;
        ...
        returns (address vaultToken)
    {
        ...
        bytes memory bytecode = type(SatinVaultToken).creationCode;
        assembly {
            vaultToken := create2(0, add(bytecode, 32), mload(bytecode), _underlying)
        }
        SatinVaultToken(vaultToken)._initialize(
            _underlying,
            optiSwap,
            router,
            voter,
