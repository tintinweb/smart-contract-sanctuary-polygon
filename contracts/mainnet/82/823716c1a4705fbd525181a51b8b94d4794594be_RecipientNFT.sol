...
contract RecipientNFT is ERC1155, Ownable {
    ...
    constructor(string memory baseTokenURI) ERC1155(baseTokenURI) {
        ...
        _mint(address(this), ARTWORK, 10000, "");
    }

    function airdrop(address[] memory recipients) public onlyOwner {
        uint64 recipientsLength = uint64(recipients.length);
        for (uint64 i = 0; i < recipientsLength; i++) {
            _safeTransferFrom(address(this), recipients[i], ARTWORK, 1, "");
            addressInfo[recipients[i]] = 1;
        }
    }
