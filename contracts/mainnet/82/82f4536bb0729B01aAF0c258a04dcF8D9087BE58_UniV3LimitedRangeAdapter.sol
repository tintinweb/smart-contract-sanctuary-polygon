...
contract UniV3LimitedRangeAdapter is AdapterBase, IUniV3Adapter {
  ...
  address constant public pmAddress = 0xC36442b4a4522E871399CD717aBDD847Ab11FE88;

  INonfungiblePositionManager constant positionManager = INonfungiblePositionManager(pmAddress);

  uint256 id;
  /// @notice Address of Uniswap V3 pool
  IUniswapV3Pool public basePool; // uniswap pool

  /// @notice Liquidity position range lower bound tick
  int24 public pool_min_tick;

  /// @notice Liquidity position range upper bound tick
  int24 public pool_max_tick;

  /// @notice Struct containing token0, token1, fee
  PoolAddress.PoolKey public poolKey;

  /// @notice Token ID of Uniswap V3 position NFT
  uint256 public tokenId;

  /// @notice "L" value of position
  /// @dev This value remains the same iff there's only one senior tranche depositor
  ...
  // @notice Tolerance value for senior deposits
  ...
  function initialize(
    ...
    require(_pool != address(0), "NZI");
    IUniswapV3Pool pool = IUniswapV3Pool(_pool);
    depositTolerance = _depositTolerance;

    poolKey = PoolAddress.getPoolKey(pool.token0(), pool.token1(), pool.fee());
    basePool = pool;
    id = _id;

    UniV3InitData memory params = abi.decode(data, (UniV3InitData));

    int24 ts = pool.tickSpacing();
    require(params.minTick % ts == 0 && params.maxTick % ts == 0, "BTS");
    pool_min_tick = params.minTick;
    pool_max_tick = params.maxTick;
  }
  ...
  function deployCapital(
    address user,
    bytes calldata data // passing in abi encoded DeployCapitalData struct
  ) external override onlyWithVaultAttached onlyVault returns (uint256, uint256) {
    DeployCapitalData memory decoded = abi.decode(data, (DeployCapitalData));
    PoolAddress.PoolKey memory pk = poolKey;

    uint128 srCapacity = uint128(IUniV3Vault(vaultAddress).srCapacity());

    // Transfer tokens from vault to adapter
    (uint160 sqrtRatioX96, , , , , , ) = basePool.slot0();
    (uint256 amount0, uint256 amount1) = LiquidityAmounts.getAmountsForLiquidity(
      sqrtRatioX96,
      pool_min_tick.getSqrtRatioAtTick(),
      pool_max_tick.getSqrtRatioAtTick(),
      srCapacity
    );
    ...
    INonfungiblePositionManager.MintParams memory params = INonfungiblePositionManager.MintParams({
      ...
      token1: pk.token1,
      fee: pk.fee,
      tickLower: pool_min_tick,
      tickUpper: pool_max_tick,
    ...
    (uint256 mintedAmount0, uint256 mintedAmount1) = mintToken(user, pk, params, srCapacity);
  ...
  function mintToken(
    ...
    (uint256 _tokenId, uint128 _liquidity, uint256 minted0, uint256 minted1) = positionManager.mint(params);
    require(l - uint128(((uint256(l) * depositTolerance)) / 10000) < _liquidity, "L");
    tokenId = _tokenId;
    liquidity = _liquidity;

    // Transfer change back to vault
    if (minted0 < params.amount0Desired) {
      TransferHelper.safeApprove(pk.token0, address(positionManager), 0); // Remove unused approval amount
      TransferHelper.safeTransfer(pk.token0, user, params.amount0Desired - minted0);
    }
    if (minted1 < params.amount1Desired) {
      TransferHelper.safeApprove(pk.token1, address(positionManager), 0); // Remove unused approval amount
      TransferHelper.safeTransfer(pk.token1, user, params.amount1Desired - minted1);
    }

    return (minted0, minted1);
  }
  ...
  function returnCapital(
    ...
    uint256 amount1,
    uint256 tranche
  ) external override onlyWithVaultAttached onlyVault {
  ...
  function earlyReturnCapital(
    ...
    PoolAddress.PoolKey memory pk = poolKey;
    (uint256 amount0, uint256 amount1) = _removeLiquidity(data);
    // transfer fees
    TransferHelper.safeTransfer(pk.token0, to, amount0);
    TransferHelper.safeTransfer(pk.token1, to, amount1);
  ...
  function setVault(address _vaultAddress) public override(AdapterBase, IAdapter) onlyWithoutVaultAttached onlyFactory {
    require(IVault(_vaultAddress).srCapacity() < type(uint128).max, "CTL");
  ...
  /// @inheritdoc IUniV3Adapter
  function removeLiquidity(address to, bytes calldata data) external override onlyVault returns (uint256, uint256) {
    (uint256 amount0, uint256 amount1) = _removeLiquidity(data);
    return (amount0, amount1);
  }

  function _removeLiquidity(bytes calldata data) internal returns (uint256 amount0, uint256 amount1) {
    RemoveLiquidityData memory decoded = abi.decode(data, (RemoveLiquidityData));

    uint256 _tokenId = tokenId;

    INonfungiblePositionManager.DecreaseLiquidityParams memory dlp = INonfungiblePositionManager
    ...
    positionManager.decreaseLiquidity(dlp);
  ...
  function collect(uint256 _tokenId) internal returns (uint256 fee0, uint256 fee1) {
