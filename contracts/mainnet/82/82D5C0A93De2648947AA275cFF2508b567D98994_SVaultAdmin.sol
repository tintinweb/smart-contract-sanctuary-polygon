...
contract SVaultAdmin is SVaultStorage {
    ...
    modifier onlyVaultOrGovernor() {
        require(msg.sender == address(this) || isGovernor(), "Caller is not the Vault or Governor");
        _;
    }
    ...
    ****************************************/
     ...
     */
    function withdrawAllFromStrategy(address _strategyAddr) external onlyGovernor {
    ...
    *************************************/
    ...
    function setStrategyWithWeights(StrategyWithWeight[] calldata _strategyWithWeights) external onlyGovernor {
        ...
        for (uint8 i = 0; i < _strategyWithWeights.length; i++) {
            ...
            require(strategyWithWeight.minWeight <= strategyWithWeight.targetWeight, "minWeight shouldn't higher than targetWeight");
            require(strategyWithWeight.targetWeight <= strategyWithWeight.maxWeight, "targetWeight shouldn't higher than maxWeight");
    ...
    **********************************/
    ...
    function setCXVaults(ChildXVault[] calldata _childXVaults) external onlyGovernor {
        ...
        for (uint8 i = 0; i < _childXVaults.length; i++) {
            ...
            childChainIds.push(childXVault.chainId);
        }
    }
    ...
    ************************************/
     ...
     */
    function getQuickDepositStrategies() external view onlyGovernor returns (address[] memory) {
    ...
    ****************************/
    ...
    function _intraRebalance() internal {
        ...
        for (uint8 i; i < stratsWithWeights.length; i++) {
            if (!stratsWithWeights[i].enabled) {
        ...
        for (uint8 i; i < stratsWithWeights.length; i++) {
            ...
            if (targetLiquidity < currentLiquidity) {
                // unstake now
                ISingleStrategy(stratsWithWeights[i].strategy).withdraw(address(this), currentLiquidity - targetLiquidity);
            } else {
                // save to stake later
                stakeOrders[stakeOrdersCount] = Order(true, stratsWithWeights[i].strategy, targetLiquidity - currentLiquidity);
                stakeOrdersCount++;
        ...
        for (uint8 i; i < stakeOrdersCount; i++) {
            ...
            ISingleStrategy(strategy).deposit(amount);
        }
    }

    function _balance() internal view virtual returns (uint256 fbalance) {
    ...
    function setDataStore(address _dataStore) external onlyGovernor {
    ...
    function rebalance(uint256[] memory _chainIds, uint256[] memory _amounts, uint256[] memory _minAmounts) public onlyGovernor {
        ...
        require (_total <= _balance(),"NEF");
        uint256 _withdrawable = _total - IERC20(asset).balanceOf(address(this));
        if (_withdrawable > 0 ) {
            _strategyWithdraw(_withdrawable);
        }
    ...
    function _strategyWithdraw(uint256 _withdrawable) internal {
        ...
        if ((totalAmount < _withdrawable) && (strategyWithWeights.length == 0)) {
            revert("NO_SOURCE");
        }
        uint8 strategyIndex = 0;
        uint8 index = 0;
        while ((totalAmount < _withdrawable) && (strategyIndex < strategyWithWeights.length)) {
            uint256 currentStratBal = ISingleStrategy(strategyWithWeights[strategyIndex].strategy).balance();
            if (currentStratBal > 0) {
                if ((currentStratBal + totalAmount) > _withdrawable) {
                    strategiesToWithdrawFrom[index] = strategyWithWeights[strategyIndex].strategy;
                    amountsToWithdraw[index] = currentStratBal - ((currentStratBal + totalAmount) - _withdrawable);
                    totalAmount += currentStratBal - ((currentStratBal + totalAmount) - _withdrawable);
                } else {
                    strategiesToWithdrawFrom[index] = strategyWithWeights[strategyIndex].strategy;
        ...
        require(totalAmount >= _withdrawable, "Not enough funds anywhere to redeem.");
        ...
        for (uint8 i = 0; i < strategyWithWeights.length; i++) {
            ...
            if (amountsToWithdraw[i] > 0) {
                ISingleStrategy(strategiesToWithdrawFrom[i]).withdraw(address(this), amountsToWithdraw[i]);
            } else {
                ISingleStrategy(strategiesToWithdrawFrom[i]).withdrawAll();
            }
        }
        require(IERC20(asset).balanceOf(address(this)) >= _withdrawable, "Not enough funds after withdrawl.");
    }
