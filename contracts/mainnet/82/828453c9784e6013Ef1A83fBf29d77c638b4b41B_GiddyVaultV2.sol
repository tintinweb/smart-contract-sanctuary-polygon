...
contract GiddyVaultV2 is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  event DepositExact(address indexed from, uint256 fap, uint8 fapIndex, uint256[] amounts, uint256 shares);
  ...
  function compound(SwapInfo[] calldata swaps) public whenNotPaused {
  ...
  function compoundCheck(SwapInfo[] calldata swaps, uint startIndex) private {
    if (rewardsEnabled) {
      strategy.claimRewards();
      if (swaps.length > startIndex) {
        compound(swaps);
      }
  ...
  function depositExact(uint256[] memory amounts, uint256 fap, uint8 fapIndex, SwapInfo[] calldata swaps) external whenNotPaused nonReentrant {
    compoundCheck(swaps, 0);
    ...
    for (uint8 i; i < depositTokens.length; i++) {
      if (amounts[i] > 0) {
        ...
        if (fap > 0 && fapIndex == i) {
          amounts[i] = deductFee(depositTokens[i], amounts[i], fap);
        }
      }
    }
    ...
    emit DepositExact(_msgSender(), fap, fapIndex, amounts, shares);
  }

  function depositNative(uint256 amount, uint256 fap, SwapInfo[] calldata swaps) external whenNotPaused nonReentrant {
    compoundCheck(swaps, 0);
  ...
  function depositSingle(address token, uint256 amount, uint256 fap, SwapInfo[] calldata swaps) external whenNotPaused nonReentrant {
    compoundCheck(swaps, 0);
    require(config.verifiedContracts(token), "VERIFY_TOKEN");
  ...
  function depositUsdc(bytes calldata auth, uint256 fap, SwapInfo[] calldata swaps) external whenNotPaused nonReentrant {
  ...
  function depositGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap, SwapInfo[] calldata swaps) external whenNotPaused nonReentrant {
  ...
  function joinStrategy(address user, SwapInfo[] calldata swaps) private returns (uint256 shares) {
    address[] memory depositTokens = query.getDepositTokens();
    compoundCheck(swaps, depositTokens.length);
  ...
  function withdraw(uint256 shares, uint256 fap, uint8 fapIndex, SwapInfo[] calldata swaps) external whenNotPaused nonReentrant {
    compoundCheck(swaps, 0);
  ...
  function oneInchSwap(SwapInfo calldata swap, address dstToken) private returns (uint returnAmount) {
    IAggregationRouterV5.SwapDescription memory desc;
    desc.srcToken = swap.srcToken;
    desc.dstToken = dstToken;
    desc.srcReceiver = swap.srcReceiver;
    desc.dstReceiver = payable(address(strategy));
    desc.amount = swap.amount;
    desc.minReturnAmount = swap.minReturnAmount;
    desc.flags = swap.flags;

    if (!IERC20Upgradeable(swap.srcToken).approve(ONE_INCH_ROUTER, swap.amount)) {
      revert("SWAP_APPROVE");
    }
    uint spentAmount;
    (returnAmount, spentAmount) = IAggregationRouterV5(ONE_INCH_ROUTER).swap(swap.executor, desc, swap.permit, swap.data);
