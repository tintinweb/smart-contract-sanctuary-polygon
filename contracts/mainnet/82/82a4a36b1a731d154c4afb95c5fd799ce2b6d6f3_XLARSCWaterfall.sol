...
abstract contract BaseRSCWaterfall is OwnableUpgradeable {
    ...
    struct RecipientData {
        uint256 received;  // Either USD for RSCWaterfallUsd or ETH for RSCWaterfall
        uint256 maxCap;
        uint256 priority;
    }
    ...
    event SetRecipients(address payable [] recipients, uint256[] maxCaps, uint256[] priorities, string[] names);
    ...
    fallback() external payable {
        ...
        // and that msg.value is high enough to automatic distribution
        if (autoEthDistribution && msg.value >= minAutoDistributionAmount) {
            _redistributeEth(msg.value, false);
        }
    }
    ...
    /**
     * @notice Internal function to redistribute ETH based on waterfall rules
     ...
     */
    function _redistributeEth(uint256 _valueToDistribute, bool _recursive) internal virtual {}


    /**
     * @notice External function to redistribute ETH based on waterfall rules
     */
    function redistributeEth() external onlyDistributor {
        ...
        _redistributeEth(balance, false);
    }
    ...
    function _setRecipients(
        ...
        emit SetRecipients(_newRecipients, _maxCaps, _priorities, _names);
    }
    ...
    function setRecipients(
        ...
        _setRecipients(_newRecipients, _maxCaps, _priorities, _names);
    }
...
contract XLARSCWaterfall is BaseRSCWaterfall {
    ...
    function initialize(
        ...
        // Recipients settings
        _setRecipients(_initialRecipients, _maxCaps, _priorities, _names);
    }

    /**
     * @notice Internal function to redistribute Eth
     * Sends msg.value and contract balance to currentRecipient. When remainCap (in USD) of currentRecipient
     * is reached rest of the msg.value is locked in contract and currentRecipient is
     * removed and new currentRecipient is set.
     * @param _valueToDistribute amount in ETH to be distributed
     * @param _recursive When recursive is True we don't charge additional fee
     */
    function _redistributeEth(uint256 _valueToDistribute, bool _recursive) internal override {
        if (currentRecipient == address(0)) {
        ...
        uint256 ethValueToSent = _valueToDistribute + (currentBalance - _valueToDistribute);

        // Check if current recipient was fulfilled
        bool setNewCurrentRecipient = false;
        if (ethValueToSent >= remainCap) {
            ethValueToSent = remainCap;
            setNewCurrentRecipient = true;
        }

        // Send ETH to current currentRecipient
        recipientData.received += ethValueToSent;
        (bool success,) = payable(currentRecipient).call{value: ethValueToSent}("");
        if (success == false) {
            revert TransferFailedError();
        }
        ...
        if (setNewCurrentRecipient) {
            ...
            if (remainingBalance > 0) {
                _redistributeEth(remainingBalance, true);
            }
        }
    }
    ...
    function _redistributeToken(address _token, bool _recursive) internal override {
        ...
        // if any subtract platform Fee and send it to platformWallet
        if (platformFee > 0 && !_recursive) {
            uint256 fee = tokenValueToSent / 10000 * platformFee;
        ...
        uint256 remainCap = recipientData.maxCap - recipientData.received;
        uint256 ethValueToSent = _convertTokenToEth(_token, tokenValueToSent);

        // Check if current recipient was fulfilled
        bool setNewCurrentRecipient = false;
        if (ethValueToSent >= remainCap) {
            ethValueToSent = remainCap;
            tokenValueToSent = _convertEthToToken(_token, ethValueToSent);
            setNewCurrentRecipient = true;
        }
        recipientData.received += ethValueToSent;
