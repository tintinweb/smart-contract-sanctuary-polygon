...
contract Multicall {
    ...
    function uniV2Swap(address pool,address _tOut) public{
        ...
        amOut = direc ? ((amInWithFee*reserve1) / (reserve0*1000+amInWithFee)) : ((amInWithFee*reserve0) / (reserve1*1000+amInWithFee));
    ...
    function multicall(address _tIn, address _tOut,uint _amIn,uint _amOut,uint lastBlock,bytes[] calldata callPath)public returns(uint,uint){
        amOut=_amIn;
