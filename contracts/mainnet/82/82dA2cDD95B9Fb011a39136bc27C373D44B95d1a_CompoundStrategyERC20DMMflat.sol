...
interface IrOracle {
    function observationIndexOf(uint timestamp)
        external
        view
        returns (uint8 index);
...
contract CompoundStrategyERC20DMMflat is rStrategy {
    ...
    address public _wmaticAddr = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; // The WMATIC token address
        ...
        onlyRole(MANAGER_ROLE)
    {
        ...
        if (address(_rewardToken) == address(_pairedToken)) {
            ...
            swap.oracleAmount = _oracle.consult(
                ...
                swap.desiredAmountPairedToken, // uint amountIn
                address(_stakingLpBaseToken) // address tokenOut
            );
            require(
                (swap.minAmountBaseToken >=
...

        {
            ...
            swap.vReserveA = _vReservePath[0];
            swap.vReserveB = _vReservePath[1];
            ...
            (, , uint newStakingTokens) = _router.addLiquidity(
                ...
                swap.minAmountPairedToken,
                _vReservePath, // minimum amount of token to pair
