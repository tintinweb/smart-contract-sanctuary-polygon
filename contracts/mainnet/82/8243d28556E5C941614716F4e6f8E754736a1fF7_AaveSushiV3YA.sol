...
contract AaveSushiV3YA is BaseYA, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  address constant private AM_SUSHI_TOKEN = 0xc45A479877e1e9Dfe9FcD4056c699575a1045dAA;
  ...
  function getContractBalance() public view override returns (uint256) {
    return IERC20(AM_SUSHI_TOKEN).balanceOf(address(this));
  }
  ...
  function depositCalc(uint256 value, uint256 fap) external override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    (priceImpact, amountOut) = GiddyUniswap.calcSwapV3(UNISWAP_ROUTER, UNISWAP_QUOTER, WMATIC_TOKEN, SUSHI_TOKEN, 3000, priceImpact, amountOut);
  ...
  function deposit(bytes calldata auth, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    tokenValue = GiddyUniswap.swapTokensSimpleV3(UNISWAP_ROUTER, WMATIC_TOKEN, SUSHI_TOKEN, 3000, address(this), tokenValue);
    ...
    if (!IERC20(SUSHI_TOKEN).approve(LENDING_POOL, tokenValue)) {
      revert();
    }
    ILendingPoolV3(LENDING_POOL).supply(SUSHI_TOKEN, tokenValue, address(this), 0);
  ...
  function withdrawCalc(uint256 shares, uint256 fap) external override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    amountOut = priceImpact;
    (priceImpact, amountOut) = GiddyUniswap.calcSwapV3(UNISWAP_ROUTER, UNISWAP_QUOTER, SUSHI_TOKEN, WMATIC_TOKEN, 3000, priceImpact, amountOut);
  ...
  function withdraw(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    ILendingPoolV3(LENDING_POOL).withdraw(SUSHI_TOKEN, value, address(this));
    value = GiddyUniswap.swapTokensSimpleV3(UNISWAP_ROUTER, SUSHI_TOKEN, WMATIC_TOKEN, 3000, address(this), value);
  ...
  modifier deductEarningsFee() {
    ...
    if (earnings > 0) {
      ...
      if (fee > 0) {
        ILendingPoolV3(LENDING_POOL).withdraw(SUSHI_TOKEN, fee, address(this));
