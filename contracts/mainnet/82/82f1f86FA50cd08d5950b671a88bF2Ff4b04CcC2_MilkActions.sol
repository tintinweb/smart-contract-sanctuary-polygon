...
contract MilkActions is HSystemChecker, Multicall {
    ...
    mapping(bytes32 => Action) public _action;
    mapping(bytes32 => mapping(address => uint256)) public _userActionsBought;
    mapping(bytes32 => uint256) public _totalActionsBought;
    ...
    /// @param maxlimit - Limit of action population
    ...
    modifier actionExists(bytes32 key) {
        require(_action[key].price > 0, 'MA 400: Action doesnt exist');
        _;
    }
    ...
    function buy(
        ...
        Action memory action = _action[actionKey];

        require(
            _totalActionsBought[actionKey] + 1 < action.maxlimit,
            'MA 100 - Exceed token population'
        );
        ...
        // burn MILK
        treasury.burn(buyer, uint256(action.price) * 1 gwei);

        for (uint256 i; i < action.itemIds.length; ) {
            itemFactory.burnItem(buyer, action.itemIds[i], action.itemAmounts[i]);
            unchecked {
                ++i;
            }
        }
        ...
        unchecked {
            ++_userActionsBought[actionKey][buyer];
            ++_totalActionsBought[actionKey];
        }
    ...
    function createAction(
        ...
        _createAction(actionKey, price, maxlimit, itemIds, itemAmounts);
        emit LogCreateAction(actionKey, price, maxlimit, itemIds, itemAmounts);
    }
    ...
    function editAction(
        ...
        _createAction(actionKey, price, maxlimit, itemIds, itemAmounts);
        emit LogEditAction(actionKey, price, maxlimit, itemIds, itemAmounts);
    }

    /// @notice Delete action
    ...
    function _createAction(
        ...
        require(price > 0, 'MA 101 - Price cant be zero');
        require(itemIds.length == itemAmounts.length, 'MA 102 - Array mismatch');
        _action[actionKey] = Action(price, maxlimit, itemIds, itemAmounts);
    }
}
