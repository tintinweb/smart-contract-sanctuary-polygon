...
contract GiddyLiquidityYA is BaseYA, UniswapV2Giddy, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  uint256 constant private PID = 8;
  uint256 constant private EARNINGS_FEE = 48500;
  uint256 constant private COMPOUND_THRESHOLD = 50e18;
  ...
  function sharesToValue(uint256 shares) public view override returns (uint256) {
    ...
    uint256 earnings = (getContractRewards() * (BASE_PERCENT - EARNINGS_FEE)) / BASE_PERCENT;
    uint256 value = usdcToGiddy(IERC20(USDC_TOKEN).balanceOf(address(this)));
    if (earnings > value && (earnings - value) > COMPOUND_THRESHOLD) {
      return (getContractBalance() + usdcToGiddyLP(giddyToUsdc(earnings + value))) * shares / contractShares;
    }
    else {
  ...
  function compound() public override whenNotPaused {
    ...
    uint256 fee = earnings * EARNINGS_FEE / BASE_PERCENT;
    uint256 value = usdcToGiddy(IERC20(USDC_TOKEN).balanceOf(address(this)));
    if (earnings > (value + fee) && (earnings - (value + fee)) > COMPOUND_THRESHOLD) {
      earnings = deductFeeGiddy(earnings, fee);

      swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, address(this), ((earnings - value) * 10015) / 20000);
      uint256 usdcValue = IERC20(USDC_TOKEN).balanceOf(address(this));
      uint256 giddyValue = IERC20(GIDDY_TOKEN).balanceOf(address(this));
      ...
      emit Compound(liquidity, lastCompoundTime, getContractBalance());

      if (!IERC20(GIDDY_USDC_PAIR).approve(GIDDY_CHEF, liquidity)) {
        revert();
      }
      IGiddyChef(GIDDY_CHEF).deposit(PID, liquidity);
  ...
  function depositCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    value -= fap;
    (amountOut, priceImpact) = calcPriceSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, (value * 10015) / 20000);
  ...
  function deposit(bytes calldata auth, uint256 fap) external override whenNotPaused nonReentrant {
    ...
    uint256 giddyValue = swapTokensSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, address(this), (value * 10015) / 20000);
    ...
    uint256 shares = contractShares == 0 ? liquidity * INIT_SHARES : liquidity * contractShares / getContractBalance();
  ...
  function depositGiddyCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    value -= fap;
    (amountOut, priceImpact) = calcPriceSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, (value * 10015) / 20000);
  ...
  function depositGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap) external override whenNotPaused nonReentrant {
    ...
    uint256 usdcValue = swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, address(this), (value * 10015) / 20000);
  ...
  function withdrawCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    (amountOut, priceImpact) = calcPriceSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, usdcValue / 2);
  ...
  function withdraw(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant{
    ...
    usdcValue += swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, address(this), giddyValue);
  ...
  function withdrawGiddyCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    uint256 giddyValue = usdcToGiddy(giddyLPToUsdc(sharesToValue(shares)));
    (amountOut, priceImpact) = calcPriceSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, giddyValue / 2);
    amountOut = (giddyValue / 2) + amountOut - fap;
  }
  ...
  function withdrawGiddy(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant {
    ...
    uint256 giddyValue;
    (usdcValue, giddyValue) = IUniswapV2Router02(SUSHI_ROUTER).removeLiquidity(USDC_TOKEN, GIDDY_TOKEN, value, 0, 0, address(this), block.timestamp + 1200);
    giddyValue += swapTokensSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, address(this), usdcValue);
  ...
  function usdcToGiddyLP(uint256 value) public view returns (uint256) {
  ...
  function giddyLPToUsdc(uint256 value) public view returns (uint256) {
    (uint256 resUsdc,,) = IUniswapV2Pair(GIDDY_USDC_PAIR).getReserves();
  ...
  function deductFeeGiddy(uint256 amount, uint256 fap) private returns (uint256) {
    if (fap > 0) {
      swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, feeAccount, fap);
    }
    return amount - fap;
  }
}
