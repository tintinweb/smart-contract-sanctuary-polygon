...
contract testSwap is Modifiers {
    ...
    function preMulticall(address token, uint256 value, bytes[] memory data) public {
    ...
    function multicall2(address token, uint256 value, bytes[] memory data) public {
        transferErc20In(token, value, 1);
        validateAllowance(token, getValueWithoutFees(value));
        uniswapRouter.multicall{value:0}(data);
    }
