...
contract Magicneers is ERC721, ERC721URIStorage, ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    uint public version = 1;

    constructor() ERC721("Magicneers", "MAGIC") {}

    function mintItem(address owner, string memory withTokenURI)
    public
    onlyOwner
    returns (uint256)
    {
        uint256 newItemId = _tokenIds.current();
        safeMint(owner, newItemId);
        _setTokenURI(newItemId, withTokenURI);
        _tokenIds.increment();
        return newItemId;
    }
