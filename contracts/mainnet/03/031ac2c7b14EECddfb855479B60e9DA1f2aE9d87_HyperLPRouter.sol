...
contract HyperLPRouter is IUniswapV3SwapCallback, GasStationRecipient, Ownable {
        ...
        )
    {
        ...
        if (paymentToken == _ETH) {
            ...
            wETH.transfer(_msgSender(), paymentAmount); // change for gasStation usage
        ...
        if (paymentToken != token0) {
            amount0 = _swap(paymentToken, token0, fee, amount0, sqrt1PriceLimitX96);
        }

        if (paymentToken != token1) {
            amount1 = _swap(paymentToken, token1, fee, amount1, sqrt2PriceLimitX96);
        }
