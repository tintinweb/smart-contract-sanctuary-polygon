...
/**
 * @title CDTU01 | Candela Token Communities SAPI de CV
 * @author Stobox Technologies Inc.
 * @dev CDTU01 ERC20 Token | This contract is opt for digital securities management.
 */
 
contract CDTU01Token is IERC20, Roles {
    ...
    modifier onlyWhitelisted(address _account) {
        if (_isDisabledWitelist) {
            require(
                ...
                "CDTU01: not whitelisted address."
            );
        }
        _;
    }
    ...
    modifier onlyWithUnfrozenFunds(address _account) {
        require(
            ...
            "CDTU01: funds are frozen."
        );
        _;
    }

    /**
     * @notice CDTU01Token simply implements a ERC20 token.
     */
    constructor() public Roles(msg.sender) {
        _name = "Candela Tulum, Villa 01";
        _symbol = "CDTU01";
        _decimals = 0;
        _kDecimals = 18;
        _k = 10**_kDecimals;
        _defaultTransferLimit = 9999999999;
        _defaultTransactionCountLimit = 9999999999;
    ...
    function burn(address _account, uint256 _amount) external onlySuperAdmin {
        require(balanceOf(_account) >= _amount, "CDTU01: balance too low");
    ...
    function transferFunds(
        ...
        require(balanceOf(_from) >= _amount, "CDTU01: not enough tokens");
    ...
    function splitOrMerge(uint256 _x, uint256 _y) external onlySuperAdmin {
        require(_x != _y, "CDTU01: _x must not be equal to _y");
        require(_x > 0, "CDTU01: _x must be greater than 0");
        require(_y > 0, "CDTU01: _y must be greater than 0");
    ...
    */
        ...
        returns (bool)
    {
        ...
        _approve(
            ...
            _allowances[_sender][msg.sender].sub(
                ...
                "CDTU01: transfer amount exceeds allowance"
            )
        );
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[msg.sender][_spender].sub(
                ...
                "CDTU01: decreased allowance below zero"
            )
        );
    ...
    function _transfer(
        ...
        require(_sender != address(0), "CDTU01: transfer from the zero address");
        require(_recipient != address(0), "CDTU01: transfer to the zero address");

        _beforeTokenTransfer(_sender, _recipient, _amount);

        _balances[_sender] = _balances[_sender].sub(
            _amount,
            "CDTU01: transfer amount exceeds balance"
        );
    ...
    function _mint(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "CDTU01: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "CDTU01: burn from the zero address");

        _beforeTokenTransfer(_account, address(0), _amount);

        _balances[_account] = _balances[_account].sub(
            _amount,
            "CDTU01: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(_owner != address(0), "CDTU01: approve from the zero address");
        require(_spender != address(0), "CDTU01: approve to the zero address");
    ...
    function _updateTransferLimit(address _account, uint256 _amount) private {
        ...
        _transferLimits[_account].allowedToTransfer = _transferLimits[_account]
            .allowedToTransfer
            .sub(_amount, "CDTU01: transfer exceeds your transfer limit");
    }
    ...
    function _updateTransactionCountLimit(address _account) private {
        if (_isEnabledTransactionCount) {
            ...
            _transactionCountLimits[_account]
                ...
                .sub(1, "CDTU01: transfer exceeds your transaction count limit");
        }
    }
}