...
contract LimitOrders is EIP712("FBS Token", "1"){
    ...
    function shouldFulfilOrder(Order memory ord, bytes memory _signature) public view returns (bool) {
        ...
        if(IERC20(address(ord.tokenIn)).balanceOf(address(ord.owner)) < ord.amountIn) return _isEnoughToken = false;
        ...
        for(uint i=0; i<=msgHashOrderCancel[ord.owner].length; i++){
            if(_hashOrder == msgHashOrderCancel[ord.owner][i]) {
                return (_isNotCancelledOrder = false);
            }
        }

        bool _isPendingOrder = true ;
        if(ord.status != 0) return _isPendingOrder = false;

        bool _isSigner = true;
        address _signer = recoverSigner(ord, _signature);
        if(address(_signer) != address(ord.owner)) return _isSigner = false;

        bool _isFulfiledOrder = true;
        if(getAmountOut(ord.amountIn,ord.pair, ord.tokenIn, ord.tokenOut) < ord.priceExecute) return _isFulfiledOrder = false;
    ...
    function ExecuteCustomOrder(Order memory ord, bytes memory _signature) public reentrancyGuard returns (bool result, string memory message){
        ...
        address _signer = recoverSigner(ord, _signature);
        require(_signer == ord.owner, "SPACE: invalid signature");
        require(isNotCancelledOrder(ord) == false, "SPACE: Can't executed cancelled order!!!");
