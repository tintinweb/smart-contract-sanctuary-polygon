...
contract DefiWrapSandboxStaking is StakeLandSand, Ownable, Pausable, ReentrancyGuard {
    ...
    uint8[3] public _share_origin = [45, 45, 10];
    uint8[3] public _share = [45, 45, 10];
    // defi balance;
    ...
    // rewards for all;
    UintTwoPool public totalRewards = UintTwoPool(0, 0);
    // rewards from this contract;
    UintTwoPool public defiRewards = UintTwoPool(0, 0);
    // total rewards for users;
    // uint256 public userTotalRewards;
    UintTwoPool public userTotalRewards = UintTwoPool(0, 0);
    // this value muliplied by user contribution
    UintTwoPool public rewardPerTokenStored = UintTwoPool(0, 0);

    SandboxLandPool internal _landPool;
    SandboxSandPool internal _sandPool;
    ...
    // get share
    function getShare() public view returns (uint8[3] memory) {
        return _share;
    }
    ...
    function withdrawDefiRewards(address to) external onlyOwner {
        uint256 defiTxLandRewards = defiRewards.land - defiRewards.land * _perLockedRewards();
        uint256 defiTxSandRewards = defiRewards.sand - defiRewards.sand * _perLockedRewards();
        if (defiTxLandRewards > 0 || defiTxSandRewards > 0) {
            _withdrawSandsIfNeed(defiTxLandRewards + defiTxSandRewards);
            _sand.safeTransfer(to, defiTxLandRewards + defiTxSandRewards);
            defiRewards.land = defiRewards.land - defiTxLandRewards;
            defiRewards.sand = defiRewards.sand - defiTxSandRewards;
            _updateTotalRewards();
        }
    }

    function defiEarned() external view returns (uint256, uint256) {
        (, , uint256 per) = _rewardPerToken();
        (uint256 land, uint256 sand) = _compulteDefi(per);
        return (defiRewards.land + land, defiRewards.sand + sand);
    }

    function defiEarnedLocked() external view returns (uint256, uint256) {
        (, , uint256 per) = _rewardPerToken();
        (uint256 land, uint256 sand) = _compulteDefi(per);
        uint256 perLocked = _perLockedRewards();
        return ((defiRewards.land + land) * perLocked, (defiRewards.sand + sand) * perLocked);
    }
    ...
    function earnedTx(address account) external view returns (uint256, uint256) {
        ...
        uint256 perLocked = _perLockedRewards();
        return (mRewardsLand - mRewardsLand * perLocked, mRewardsSand - mRewardsSand * perLocked);
    }
    ...
    function earnedLocked(address account) external view returns (uint256, uint256) {
        ...
        uint256 mRewardsSand = rewardsSand[account] + sandEarned;
        uint256 perLocked = _perLockedRewards();
        return (mRewardsLand * perLocked, mRewardsSand * perLocked);
    }

    function perLockedRewards() external view returns (uint256) {
        return _perLockedRewards();
    }
    ...
    function _withdarwLandRewards(address account) internal {
        uint256 reward = rewardsLand[account] - rewardsLand[account] * _perLockedRewards();

        if (reward > 0) {
            _withdrawSandsIfNeed(reward);
            rewardsLand[account] = rewardsLand[account] - reward;
            userTotalRewards.land = userTotalRewards.land - reward;
            _updateTotalRewards();
    ...
    function _withdarwSandRewards(address account) internal {
        uint256 reward = rewardsSand[account] - rewardsSand[account] * _perLockedRewards();
        if (reward > 0) {
            _withdrawSandsIfNeed(reward);
            rewardsSand[account] = rewardsSand[account] - reward;
            userTotalRewards.sand = userTotalRewards.sand - reward;
            _updateTotalRewards();
            _sand.safeTransfer(account, reward);
        }
        emit RewardPaid(account, reward);
    ...
    function _updateTotalRewards() internal {
        totalRewards.land = userTotalRewards.land + defiRewards.land;
        totalRewards.sand = userTotalRewards.sand + defiRewards.sand;
    }
    ...
    function _perLockedRewards() internal view returns (uint256) {
        uint256 total = totalRewards.land + totalRewards.sand;
        if (total == 0) return 0;
        return _totalLocked() / total;
    }
    ...
    function _compulteDefi(uint256 per) internal view returns (uint256, uint256) {
        uint256 land = (((per * _share[2]) / 100) * _share[0]) / (_share[0] + _share[1]);
        uint256 sand = (((per * _share[2]) / 100) * _share[1]) / (_share[0] + _share[1]);
        return (land, sand);
    }
    ...
    function _updateRewards(address account) internal {
        ...
        _doTryGetRewards();
        (uint256 land, uint256 sand) = _compulteDefi(per);
        defiRewards.land = defiRewards.land + land;
        defiRewards.sand = defiRewards.sand + sand;
        ...
        userTotalRewards.land = userTotalRewards.land + earnedLand;
        userTotalRewards.sand = userTotalRewards.sand + earnedSand;
        _updateTotalRewards();
    ...
    function _rewardPerToken() internal view returns (uint256, uint256, uint256) {
        ...
        uint256 landPer = 0;
        if (_landsTotalContributions != 0) landPer = (perRewards * 1e24 * _share[0]) / 100 / _landsTotalContributions;
        uint256 sandPer = 0;
        if (_sandsTotalContributions != 0) sandPer = (perRewards * 1e24 * _share[1]) / 100 / _sandsTotalContributions;
    ...
    function _withdrawSandsIfNeed(uint256 withdrawAmount) internal {
        ...
        if (balance < withdrawAmount) {
            ...
            if (sandPoolBalance + balance >= withdrawAmount) {
                ...
                _sandPool.withdraw(sandPoolBalance);
    ...
    function _updatePools() internal {
        ...
        if (maxLandPool > landPoolBalance) {
            ...
            if (sands >= maxLandPool - landPoolBalance) {
                ...
                _sand.approve(address(_landPool), sands);
                _landPool.stake(sands);
                sands_changed = true;
            }
