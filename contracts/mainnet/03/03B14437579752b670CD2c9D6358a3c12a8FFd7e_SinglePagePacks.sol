...
/**
    @title RandomBits Single Page Packs
    @notice Assemble a single image from puzzle pieces delivered to players in RandomPacks
...
contract SinglePagePacks is ISinglePagePacks, ERC721Holder, AccessControl {
    ...
    function grantPack(address player) external onlyRole(DEFAULT_ADMIN_ROLE) returns (TokenId tokenId) {
        ...
        tokenId = randomPack.mint(player, PackSize.Regular, pieces);
...
interface ISinglePagePacks {
    ...
    function grantPack(address player) external returns (TokenId tokenId);
...
/**
    @title RandomBits Single Sticker Pack
    @notice Assemble a single image from puzzle pieces delivered to players in RandomPacks
    @author Gene A. Tsvigun
  */
