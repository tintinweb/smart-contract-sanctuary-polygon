    ...
    ERC2981Upgradeable
{
    ...
    function mint(uint256 _amount) external payable whenNotPaused nonReentrant {
        ...
        if (maxSupply > 0) {
            require(
                ...
                "ERC721Art: max supply reached"
            );
        }
        require(
            !(msg.value < _amount * price),
            "ERC721Art: not enough value sent"
        );
    ...
    correct token price. 
    Parameters are the same of safeTransferFrom function. */
    function creatorProSafeTransferFrom(
        address from,
        address to,
        uint256 tokenId
    ) public payable {
        uint256 _tokenPrice = tokenPrice[tokenId];
        if (_tokenPrice == 0) {
            _tokenPrice = price;
        }

        require(!(msg.value < _tokenPrice), "ERC721Art: not enough ETH sent");
    ...
    /**@dev transfers CreatorPRO royalties to the Reserve contract.
       @param _value: value to be transfered */
    function _creatorProRoyalty(uint256 _value) internal {
        payable(management.reserve()).transfer(_value);
    }
    ...
    function approve(address operator, uint256 tokenId)
        ...
        onlyAllowedOperatorApproval(operator)
    {
        ERC721AUpgradeable.approve(operator, tokenId);
    }
