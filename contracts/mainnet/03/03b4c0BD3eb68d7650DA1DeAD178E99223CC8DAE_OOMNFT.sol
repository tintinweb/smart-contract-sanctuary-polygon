    ...
    Ownable
{
    ...
    mapping(address => bool) private isAmountTransferred; // tranfer amount to cover the gass fee to list nft first time
    ...
    constructor(address _owner) ERC721("ONE_OF_A_MINT", "OOMNFT") {
        ...
        maxSupply = 10000;
        approvalAmount = 50000000000000000; // 0.05
    }
    ...
    /**************************
     ***** Gasless MINT FUNCTIONS *****
     *************************/
    function mintGL(address _to, string memory uri) external payable onlyOwner {
        require(totalSupply() + 1 <= maxSupply, "NFT: out of stock");
        require(_to != address(0), "NFT: invalid address");
        ...
        _safeMint(_to, tokenId);
        _setTokenURI(tokenId, uri);
        if (isAmountTransferred[_to] != true && approvalAmount != 0) {
            require(
                msg.value >= approvalAmount,
                "Amount should be greater or equal to the approval amount"
            );
            payable(_to).transfer(approvalAmount);
            isAmountTransferred[_to] = true;
        }
    ...
    function setCreatorApprovalAmount(uint256 _amount) public onlyOwner {
        approvalAmount = _amount;
    }

    function getCreatorApprovalAmount() public view returns (uint256) {
        return approvalAmount;
    }

    function getIsAmountTransferred(address to_) public view returns (bool) {
        return isAmountTransferred[to_];
    }
    ...
    function setRoyalty(
        ...
        require(
            creators[_tokenId] == ownerOf(_tokenId),
    ...
    function setMaxSupply(uint256 _maxSupply) external onlyOwner {
        require(
            ...
            "The max supply should larger than current value"
        );
