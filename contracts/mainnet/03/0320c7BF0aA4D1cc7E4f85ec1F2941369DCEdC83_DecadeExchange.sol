...
contract DecadeExchange is OwnableUpgradeable {
    ...
    event Swapped(uint indexed orderId);
    ...
    function initialize() external initializer {
        ...
        _oracles[0x9b47AaD59A49d702708E570D804a1d25F9C867CC] = 0xA338e0492B2F944E9F8C0653D3AD1484f2657a37;
        _oracles[0xa2b762986783Dba0fe4464cc1d86A4975Fb1DeB5] = 0xefb7e6be8356cCc6827799B6A7348eE674A80EaE;
    }
    ...
    function swapV2(LibTrade.SwapOrder[] memory chunk) external onlyWhitelisted {
        ...
        for (uint i = 0; i < chunk.length; i++) {
            ...
            IERC20(tokenIn).safeTransferFrom(swapOrder.order.account, address(this), amountIn);

            uint amountOutMin = askSide ? IUniswapV2Router01(swapOrder.router).getAmountsOut(swapOrder.remain, swapOrder.path)[swapOrder.path.length - 1] : IUniswapV2Router01(swapOrder.router).getAmountsIn(swapOrder.remain, swapOrder.path)[0];

            uint amountOut;
            if (askSide) {
                if (amountOutMin < expected) continue;
                ...
                if (amountOutMin > expected) continue;
                amountOut = IUniswapV2Router01(swapOrder.router).swapExactTokensForTokens(expected, swapOrder.remain, swapOrder.path, address(this), block.timestamp + 100)[swapOrder.path.length - 1];
            }
            ...
            emit Swapped(swapOrder.orderId);
        }
    }
