...
interface IVCPool {
    error PoolNotStarterError();
    error PoolNotAdminError();
    error PoolUnexpectedAddressError();
    error PoolERC20TransferError();
    error PoolAmountTooHighError();
    error PoolInvalidCurrencyError();

    event PoolFunding(address indexed user, uint256 amount);
    event PoolWithdrawal(IERC20 indexed currency, address indexed to, uint256 amount);
...
contract VCPool is IVCPool {
    ...
    function setPoCNft(address pocNft) external {
        _onlyAdmin();
        _checkAddress(pocNft);
        _pocNft = IVCPoCNft(pocNft);
    }

    function setCurrency(IERC20 currency) external {
        _onlyAdmin();
        _checkAddress(address(currency));
        _currency = currency;
    }

    function setStarter(address starter) external {
        _onlyAdmin();
        _checkAddress(starter);
        _starter = starter;
    }
    ...
    function withdraw(IERC20 currency, address _to, uint256 _amount) external {
        ...
        emit PoolWithdrawal(currency, _to, _amount);
    }
    ...
    function fund(uint256 _amount) external {
        ...
        emit PoolFunding(msg.sender, _amount);
    }
