 ...
 *
 * @dev Deprecated: Use TokenEnumeratorV2 for new projects.
 */
 ...
 * behavior and stay under the bytecode limit.
 */
abstract contract TokenEnumeratorV2 is ITokenManager {
    using Address for address;
    using SafeMath for uint256;
    using TokenManagement for TokenManagement.TokenManager;

    TokenManagement.TokenManager private tokenManager;
    ...
    // @inheritdoc ITokenManager
    function tokenByIndex(uint256 index) public view returns (uint256) {
        return tokenManager.tokenByIndex(index);
    }
...
// File: TokenDropManagerV2.sol

contract TokenDropManagerV2 is
    ITokenDropManager,
    TokenEnumeratorV2,
    ViciAccess
{
    ...
    DropManagement.DropManager internal dropManager;
    ...
    constructor(uint256 _maxSupply) TokenEnumeratorV2(_maxSupply) {}
    ...
    function startNewDrop(
        ...
        Drop memory newDrop = dropManager.startNewDrop(
            _dropName,
            _dropStartTime,
            _dropSize,
            INITIAL_STATE,
    ...
    function startSubDrop(
        ...
        Drop memory originalDrop = dropManager.dropForName(_parentDropName);

        (Drop memory parentDrop, Drop memory subDrop) = dropManager
            .startSubDrop(_parentDropName, _dropName, _dropSize, _baseURI);

        emit DropEnded(originalDrop);
        if (dropManager.isDropActive(_parentDropName)) {
            emit DropAnnounced(parentDrop);
        }
        emit DropAnnounced(subDrop);
    }
    ...
    {
        (Drop memory deactivated, uint32 remaining) = dropManager
            .deactivateDrop(_dropName);
        tokensReserved -= remaining;
        emit DropEnded(deactivated);
    }
    ...
    function setBaseURIForState(
        ...
        string memory _stateName,
        string memory _baseURI
    ) public onlyOwnerOrRole(URI_MANAGER) {
        ...
        returns (string memory)
    {
        ...
        string memory result = dropManager.getTokenURI(_tokenId);
        if (bytes(result).length == 0 && bytes(baseURI).length > 0) {
            return
                string(abi.encodePacked(baseURI, Strings.toString(_tokenId)));
        }
    ...
    function beforeTokenTransfer(
        ...
        address _toAddress,
        uint256 _tokenId
    ) public virtual override(ITokenManager, TokenEnumeratorV2) onlyOwner {
        TokenEnumeratorV2.beforeTokenTransfer(
            _fromAddress,
            _toAddress,
            _tokenId
        );
    ...
    function beforeMint(
        ...
        if (bytes(_category).length == 0) {
            require(1 <= totalAvailable() - tokensReserved, "sold out");
        } else {
    ...
    function beforeBatchMint(
        ...
        if (bytes(_category).length == 0) {
            require(
                _tokenIds.length <= totalAvailable() - tokensReserved,
                "sold out"
            );
        } else {
            dropManager.onBatchMint(_category, _tokenIds);
            tokensReserved -= _tokenIds.length;
        }
    }
    ...
    function getState(uint256 _tokenId) public view returns (string memory) {
        string memory state = dropManager.getState(_tokenId);
    ...
    function _post_burn_hook(uint256 _tokenId) internal virtual override {
        ...
        dropManager.postBurnUpdate(_tokenId);
    }
}
...
// File: ViciERC721_V2.sol

/**
 * @dev Extention of ViciManagedERC721 that includes a 
 * Drop Manager, state machine, and recall feature.
 * 
 * @dev subclasses might want to expose functions for manipulating state.
 */
contract ViciERC721_V2 is ViciManagedERC721WithRecall {
    TokenDropManagerV2 internal stateMachine;
    ...
    constructor(
        ...
        uint256 _maxSupply,
        IERC2981 _royaltiesContractIn
    ) ViciManagedERC721WithRecall(_name, _symbol, _maxSupply, _royaltiesContractIn) {}

    function maxRecallPeriod() public pure virtual override returns (uint32) {
        return 1209600; // 14 days
    }

    function _initializeTokenManager() internal virtual override {
        TokenDropManagerV2 dropManager = new TokenDropManagerV2(maxSupply);
...
// File: StrategicEntrepreneurism.sol

/**
 * Jon Fisher’s Strategic Entrepreneurism: Shattering the Start-Up Entrepreneurial 
 * Myths shows entrepreneurs how to design their companies towards the path of 
 * least resistance, maximum payoff, and lowest risk.
 * 
 * When the book was released, Jon had lectured at nearly 50 business schools 
 * and universities across the US, including Columbia Business School, where his 
 * presentation received an exciting reception.
 * 
 * Jon is now extending his book to the blockchain by offering an NFT with 
 * exclusive holder benefits. NFT owners will get Jon’s book, access to Zoom 
 * webinars, and course materials to see how strategic entrepreneurism is being 
 * taught across top universities.
 * 
 * All proceeds go to charity.
 */
contract StrategicEntrepreneurism is ViciERC721_V2 {
    constructor()
        ViciERC721_V2(
            "StrategicEntrepreneurism", // collection name
            "JBF-SE", // collection symbol
            1000, // max supply (collection can be no larger than this)
            IERC2981(address(0xf3f6DC7b33DF68a40A9B3425A4Fa694EfeBdB526))
        )
    {
        setContractURI("https://storage.googleapis.com/arrival-shadow-motion-newman/jbf-se/metadata/StrategicEntrepreneurism.json");
        setBaseURI("https://storage.googleapis.com/arrival-shadow-motion-newman/jbf-se/metadata/");
    }
}