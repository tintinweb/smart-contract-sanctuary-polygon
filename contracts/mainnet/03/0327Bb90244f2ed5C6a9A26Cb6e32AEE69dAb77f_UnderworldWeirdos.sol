 ...
 *
 * NOTE: ` ` is generally not needed starting with Solidity 0.8, since the compiler
...
pragma solidity ^0.8.0;
contract UnderworldWeirdos is ERC721, ERC721Enumerable, Ownable, Pausable {
  ...
  uint256 public MAX_MINTABLE = 6666;
  uint256 public mintPrice = 24 ether;
  uint256 public maxPurchaseCount = 1000;
  uint256 public totalWeirdos = 0;
  ...
  function _mintTokens(uint256 numberOfTokens) internal {
    for (uint256 i = 0; i < numberOfTokens; i++) {
      _mint(msg.sender, totalWeirdos + 1);
      totalWeirdos += 1;
    }
  }
  ...
  */
   ...
   */
  function addBucket(uint256 min, uint256 max, string memory prefix) public onlyOwner {
    require(min < max, "Min must be less than Max");
    require(bytes(prefix).length > 0, "Prefix can not be blank");
    for (uint256 i = 0; i < buckets.length; i++) {
      require(min > buckets[i].max, "Bucket min must start after previous bucket max");
    }
  ...
  * we surpass MAX_MINTABLE Weirdos the mintTokens is
  * no longer available. From then on the only way to
  * mint a Visitor you must recycle and burn existing
  * tokens
  */
  modifier mintCountMeetsSupply(uint256 numberOfTokens) {
    require(
      totalWeirdos + numberOfTokens <= MAX_MINTABLE,
  ...
  modifier meetsRecyclingSupply() {
    require(
      recycleSupply > 0,
      "There are no Weirdos available via recycling"
    );
    _;
  }
  ...
  modifier meetsRecyclingCost(uint256[] memory tokenIds) {
    require(
      ...
      "Incorrect number of Weirdos passed for recycling"
    );
    _;
  }
  ...
  modifier meetsOwnership(uint256[] memory tokenIds) {
    for (uint256 i = 0; i < tokenIds.length; i++) {
      require(this.ownerOf(tokenIds[i]) == msg.sender, "You may only recycle Weirdos you own");
    }
    _;
  }
