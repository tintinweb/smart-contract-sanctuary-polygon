...
contract MegamoonDealer is Ownable, IMegamoonDealer {
    ...
    function withdraw(uint _amount) public {
        ...
        uint256 availableAmount = getAvailableBalance(msg.sender);

        require (_amount <= availableAmount, 'withdraw not good');
        ...
        if (user.stakingAmount == 0) { _isActive[msg.sender] = false; }
    ...
    function withdrawAll() public {
        withdraw(_userInfo[msg.sender].stakingAmount - getLockedBalance(msg.sender));
    }

    function totalBalance() external view returns (uint256) {
        uint256 balance = 0;
        for ( uint256 i = 0; i < _users.length; i++ ) {
            UserInfo storage user = _userInfo[_users[i]];
    ...
    function getAvailableBalance(address user) private view returns (uint256) {
        // lottery is open - the injected amount can't be withdraw yet
        if (currentLotteryId != nextLotteryId) {
            return _userInfo[user].stakingAmount - _userInjectionHistory[user][currentLotteryId];
        } else {
            return _userInfo[user].stakingAmount;  // lottery is close
        }
    }

    function getLockedBalance(address user) private view returns (uint256) {
        // lottery is open - the injected amount can't be withdraw yet
        if (currentLotteryId != nextLotteryId) {
            return _userInjectionHistory[user][currentLotteryId];
        } else {
            return 0; // lottery is close
        }
    }
    
    function viewUserInfo(address user) external view returns (uint256, uint256, uint256) {
        uint256 stakingBalance = _userInfo[user].stakingAmount;
    ...
    function getInjectAmount(uint256 _lotteryId) private returns (uint256) {
        uint256 totalInjectAmount = 0;

        for ( uint256 i = 0; i < _users.length; i++ ) {
            if (_isActive[_users[i]]) {
                ...
                uint256 userInjectAmount = _userInfo[_users[i]].stakingAmount * injectPercentage / 10000;

                _userInjectionHistory[_users[i]][_lotteryId] = userInjectAmount; // save history in mapping
                totalInjectAmount += userInjectAmount;
            }          
        }
        nextLotteryId = _lotteryId + 1;
        return totalInjectAmount;
    }
    ...
    function returnFunds(uint256 _lotteryId, uint256 _amount) external override {
        ...
        for ( uint256 i = 0; i < _users.length; i++ ) {
            // 
            uint256 userInjectAmount = _userInjectionHistory[_users[i]][currentLotteryId];
            if (userInjectAmount > 0) {
                uint256 reward = calculateReward(userInjectAmount, currentLotteryId);

                UserInfo storage user = _userInfo[_users[i]];
                user.stakingAmount += ( reward - userInjectAmount );
                // user.lockedAmount = 0;
                sumReward += reward;

                _userRewardHistory[_users[i]][_lotteryId] = reward; // save history in mapping
            }
        }

        uint256 fee = _amount - sumReward;
        ...
        emit ReturnFunds(_lotteryId, sumReward);
    }

    function setActiveOrInactive(bool _active) external {
        _isActive[msg.sender] = _active;
    }
