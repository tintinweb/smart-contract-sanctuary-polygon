...
// MasterChef is the master of Bel. He can make Bel and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Bel is sufficiently
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Bels to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Bels distribution occurs.
        uint256 accBelPerShare;   // Accumulated Bels per share, times 1e12. See below.
    ...
    function pendingBel(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = address(pool.lpToken) == address(belAddress) ? totalStakedAmount : pool.lpToken.balanceOf(address(this));
    ...
    function safeBelTransfer(address _to, uint256 _amount) internal {
        uint256 belBal = IBEP20(belAddress).balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > belBal) {
            transferSuccess = IBEP20(belAddress).transfer(_to, belBal);
        } else {
            transferSuccess = IBEP20(belAddress).transfer(_to, _amount);
        }
    ...
    function updateEmissionRate(uint256 _belPerBlock) public onlyOwner {
        massUpdatePools();
        belPerBlock = _belPerBlock;
        emit UpdateEmissionRate(msg.sender, _belPerBlock);
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && referralCommissionRate > 0) {
            ...
            safeBelTransfer(referrer, commissionAmount);
