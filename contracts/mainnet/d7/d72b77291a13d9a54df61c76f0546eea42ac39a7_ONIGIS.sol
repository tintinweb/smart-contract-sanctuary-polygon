...
// File: https://github.com/erc721r/ERC721R/blob/main/contracts/ERC721R.sol
...
contract ONIGIS is ERC721r {
    ...
    constructor(string memory _baseURI_) ERC721r("ONIGIS", "ONIGIS", 1111){
        owner = msg.sender;
        teamVault = msg.sender;
        baseURI = _baseURI_;
    }
    ...
    function purchase(uint256 numOfNft) external{
        if(!mintEnabled) revert MintNotEnabled();
        if(numOfNft + balanceOf(msg.sender) > MAX_MINT_PER_WALLET) revert MaxMintReached();

        (bool success, ) = WETH.call(abi.encodeWithSelector(0x23b872dd, msg.sender, teamVault, numOfNft * price));
    ...
    // Mint non-random tokens in decreasing order of id. 
    // E.g., if you want to mint id 200 to one person and id 100 to another person, 
    // you should mint id 200 first (because 200 > 100).
    function staffMint(uint256[5] memory ids) external onlyOwner{
        for(uint256 i; i < 5;) {
            if(i != 4){
                if(ids[i] < ids[i+1]) revert WrongOrder();
            }
            _mintAtIndex(teamVault, ids[i]);
            unchecked { ++i; }
        }
    }
    ...
    function changeTeamVault(address newTeamVault) external onlyOwner{
        teamVault = newTeamVault;
    }

    function setPrice(uint256 weiAmount) external onlyOwner{
        price = weiAmount;
    }
    ...
    function rescueTokens(address token, uint256 value) external onlyOwner{
        (bool success, ) = token.call(abi.encodeWithSelector(0xa9059cbb, teamVault, value));
        if(!success) revert FailedForward();
    }
}