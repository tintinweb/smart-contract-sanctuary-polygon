...
contract CoreInternals is Helpers {
    ...
    struct OverallPosition {
        ...
        address token1;
        uint128 liquidity;
        uint totalSupplyInUsd;
        uint totalNormalSupplyInUsd;
        uint totalBorrowInUsd;
        uint totalNormalBorrowInUsd;
    }
...
contract UserModule2 is CoreInternals {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    modifier nonReentrant() {
        require(_status != 2, "P3:M3::ReentrancyGuard: reentrant call");
        _status = 2;
        _;
        _status = 1;
    }
    ...
    */
    modifier isPositionOwner(uint96 NFTID_) {
        require(_position[msg.sender][NFTID_], "P3:M3: not-an-owner");
        _;
    }
    ...
    */
    modifier isNotStaked(uint96 NFTID_) {
        require(!_isStaked[NFTID_], "P3:M3: NFT-should-be-unstaked");
        _;
    }
    ...
    */
    function addLiquidity(
        ...
        uint256 minAmount1_,
        uint256 deadline_
    ) external isNotStaked(NFTID_) nonReentrant  returns (uint256 exactAmount0_, uint256 exactAmount1_) {
        ...
        approve(TokenInterface(token0Addr_), nftManagerAddr, amount0_);
        approve(TokenInterface(token1Addr_), nftManagerAddr, amount1_);
    ...
    */
        ...
        returns (uint256 exactAmount0_, uint256 exactAmount1_)
    {
        ...
        (exactAmount0_, exactAmount1_) = _collect(NFTID_, uint128(exactAmount0_), uint128(exactAmount1_));
        OverallPosition memory overallPosition_;
        (
            overallPosition_.pool,
            overallPosition_.token0,
            overallPosition_.token1,
            ,
            overallPosition_.totalSupplyInUsd,
            overallPosition_.totalNormalSupplyInUsd,
            overallPosition_.totalBorrowInUsd,
            overallPosition_.totalNormalBorrowInUsd
        ) = getOverallPosition(
            NFTID_
        );
        verifyOracles(overallPosition_.pool, overallPosition_.token0, overallPosition_.token1);
        ...
        require(isOk_, "P3:M3: position-will-liquidate");

        emit removeLiquidityLog(NFTID_, exactAmount0_, exactAmount1_);
    }

    /**
    * @dev borrow against an NFT debt position. Throws error if borrow more than borrow limit.
    * @param NFTID_ NFT ID
    ...
    */
    function borrow(
        ...
        updateNftReward(NFTID_, token_);
        uint256 amountRaw_ = _borrow(NFTID_, token_, amount_);
        OverallPosition memory overallPosition_;
       (
            overallPosition_.pool,
            overallPosition_.token0,
        ...
        require(_borrowAllowed[overallPosition_.pool][token_], "P3:M3: token-not-allowed-for-borrow");
        verifyOracles(overallPosition_.pool, overallPosition_.token0, overallPosition_.token1);
        bool isOk_ = liquidationCheck(
            overallPosition_.pool,
            overallPosition_.totalSupplyInUsd,
        ...
        require(isOk_, "P3:M3: position-will-liquidate");
        (,,,uint borrowExchangePrice_) = liquidity.borrow(token_, amount_);
        _updatePoolBorrow(overallPosition_.pool, token_, amountRaw_, borrowExchangePrice_);
    ...
    */
    function payback(
        ...
        if (exactAmount_ > 0) {
            ...
            approve(TokenInterface(token_), address(liquidity), exactAmount_);
    ...
    /**
    * @dev collect fee accrued of an NFT. Throws error if on withdrawing fees position is going to Liquidate.
    * @param NFTID_ NFT ID.
    ...
    */
    ...
    function stake(
        ...
        isPositionOwner(NFTID_) nonReentrant
    {
        require(_stakeCount[NFTID_] < maxStakeCount, "cannot-stake-more");
    ...
    /**
    * @dev claim rewards for all borrowings on an NFT.
    * @param NFTID_ NFTID of user.
    * @return reward infos.
    */
    function claimBorrowingRewards(uint96 NFTID_) public isPositionOwner(NFTID_) nonReentrant returns (BorrowingReward[] memory) {
        (address token0_, address token1_, uint24 fee_) = getNftTokenPairAddresses(NFTID_);
        address pool_ = getPoolAddress(token0_, token1_, fee_);
        address[] memory markets_ = _poolMarkets[pool_];
        BorrowingReward[] memory rewards_ = new BorrowingReward[](markets_.length);
        for(uint i = 0; i < markets_.length; i++) {
            rewards_[i].token = markets_[i];
            (rewards_[i].rewardTokens, rewards_[i].rewardAmounts) = claimBorrowingRewards(NFTID_, markets_[i]);
        }
        return rewards_;
    }

    /**
    * @dev claim rewards for a particular borrow token on an NFT.
    * @param NFTID_ NFTID of user.
    * @param token_ address of token.
    * @return rewardTokens_ list of reward token addresses.
    * @return rewardAmounts_ amount of reward token sent to user.
    */
    function claimBorrowingRewards(uint96 NFTID_, address token_) public isPositionOwner(NFTID_) nonReentrant returns (address[] memory rewardTokens_, uint[] memory rewardAmounts_) {
        rewardTokens_ = _rewardTokens[token_];
        rewardAmounts_ = updateNftReward(NFTID_, token_);
        for (uint i = 0; i < rewardTokens_.length; i++) {
            _nftRewards[NFTID_][token_][rewardTokens_[i]].reward = 0;
            IERC20(rewardTokens_[i]).safeTransferFrom(address(rewardPool), msg.sender, rewardAmounts_[i]);
        }

        emit claimLog(msg.sender, NFTID_, token_, rewardTokens_, rewardAmounts_);
    }
...
contract OraclesHelpers is UniswapHelpers {
        ...
        returns (uint256)
    {
        ...
        return uint256(oracle_.latestAnswer()) * 10**(18 - oracle_.decimals());
    }
        ...
        returns (uint256)
    {
        ...
        return price0_.mul(1e18).div(price1_);
    }
    ...
    function getUniswapOracleTicksWithSecondsAgos(
        ...
        for (uint256 i = 1; i < secondsAgos_.length; i++) {
            ticks_[i - 1] = (tickCumulatives_[0] - tickCumulatives_[i]) / int56(uint56(secondsAgos_[i] - secondsAgos_[0]));
        }
    }
        ...
        returns (int24 currentTick_)
    {
        ...
        require(dif_[0] < tickCheck_.tickSlippage1, "P3:M3: excess-tick-slippage1");
        require(dif_[1] < tickCheck_.tickSlippage2, "P3:M3: excess-tick-slippage2");
        require(dif_[2] < tickCheck_.tickSlippage3, "P3:M3: excess-tick-slippage3");
        require(dif_[3] < tickCheck_.tickSlippage4, "P3:M3: excess-tick-slippage4");
        require(dif_[4] < tickCheck_.tickSlippage5, "P3:M3: excess-tick-slippage5");
    }

    /**
     * Returns the product of 2 uint256 numbers (which can cross uint256) in the form of r0 and r1. a * b = r1 * (2 ** 256) + r0
     * @param a_ First uint256 no.
     * @param b_ Second uint256 no.
     */
    function mul512(uint a_, uint b_) internal pure returns (uint r0_, uint r1_) {
        assembly {
            let mm_:= mulmod(a_, b_, not(0))
            r0_:= mul(a_, b_)
            r1_:= sub(sub(mm_, r0_), lt(mm_, r0_))
        }
    }
     ...
     */
    function tickToPrice(int24 tick_, uint token0Decimals_, uint token1Decimals_) internal pure returns (uint256 price_) {
        uint sqrtPriceX96_ = TickMath.getSqrtRatioAtTick(int24(tick_));
        // price_ = (((sqrtPriceX96_ * sqrtPriceX96_) * 1e18) >> (96 * 2)) * (10 ** token0Decimals_) / (10 ** token1Decimals_);
        uint a_ = sqrtPriceX96_ * 1e18;
        uint b_ = sqrtPriceX96_ * (10 ** token0Decimals_);
        (uint r0_, uint r1_) = mul512(a_, b_);
        price_ = ((r1_ << 64) + (r0_ >> 192)) / (10 ** token1Decimals_);
    }
     ...
     */
    function verifyOracles(address poolAddr_, address token0_, address token1_) internal view {
        IUniswapV3Pool pool_ = IUniswapV3Pool(poolAddr_);
        int24 currentTick_ = verifyTicksSlippage(pool_);
        uint256 token0Decimals_ = TokenInterface(token0_).decimals();
        uint256 token1Decimals_ = TokenInterface(token1_).decimals();
        uint256 uniswapOraclePrice_ = tickToPrice(currentTick_, token0Decimals_, token1Decimals_);
...
contract CoreReadHelpers is OraclesHelpers {
     ...
     * @param tickUpper_ Upper tick of the NFT.
     * @param liquidity_ Liquidity of the NFT.
     * @return amount0_ total amount 0.
     * @return amount1_ total amount 1.
     */
    function getNetNFTLiquidity(
        ...
        (amount0_, amount1_) = LiquidityAmounts
...
contract Helpers is CoreReadHelpers {
    ...
    function _borrow(
        ...
        address token_,
        uint256 amount_
    ) internal returns (uint amountRaw_) {
        (, uint updateExchangePrice_) = liquidity.updateInterest(token_);
        amountRaw_ = amount_.mul(1e18).div(updateExchangePrice_);
        uint256 borrowBalRaw_ = _borrowBalRaw[NFTID_][token_];
        _borrowBalRaw[NFTID_][token_] = borrowBalRaw_.add(amountRaw_);
    }

    /**
     * Updates the storage, checks if borrow limit exceeded
     * @param pool_ pool address.
     * @param token_ token to borrow.
     * @param amountRaw_ raw amount to borrow.
     */
    function _updatePoolBorrow(
        address pool_,
        address token_,
        uint256 amountRaw_,
        uint256 borrowExchangePrice_
    ) internal {
        uint256 poolRawBorrow_ = _poolRawBorrow[pool_][token_];
        _poolRawBorrow[pool_][token_] = poolRawBorrow_.add(amountRaw_);
        require(_poolBorrowLimit[pool_][token_] > _poolRawBorrow[pool_][token_].mul(borrowExchangePrice_).div(1e18), "P3:M3: pool-borrow-limit-exceeded");
    }
    ...
    function _payback(
        ...
        if (amount_ != type(uint256).max) {
            ...
            // throws error if amountRaw_ is greater than borrowBalRaw_
    ...
    */
    function updateRewards(address token_) public returns (uint[] memory newRewardPrices_) {
        ...
        uint totalBorrow_ = liquidity.protocolRawBorrow(address(this),token_);
        for (uint i = 0; i < rewardTokens_.length; i++) {
            ...
            if (rewardPrice_.lastUpdateTime != block.timestamp && rewardRate_.startTime <= block.timestamp) {
                ...
                newRewardPrices_[i] = totalBorrow_ == 0 ? rewardPrice_.rewardPrice :
                    rewardPrice_.rewardPrice.add(rewardRate_.rewardRate.mul(toTime_.sub(fromTime_)).mul(1e27).div(totalBorrow_));
    ...
    */
    function updateNftReward(uint96 NFTID_, address token_) public returns (uint[] memory updatedRewards_) {
        ...
        uint borrowOnNFT_ = _borrowBalRaw[NFTID_][token_];
        for (uint i = 0; i < rewardTokens_.length; i++) {
            NftReward memory nftRewards_ = _nftRewards[NFTID_][token_][rewardTokens_[i]];
            if (newRewardPrices_[i] > nftRewards_.lastRewardPrice) {
                // dividing by 1e27 to get proper reward as we multiplied it with 1e27 at the time of setting price
                updatedRewards_[i] = nftRewards_.reward.add(newRewardPrices_[i].sub(nftRewards_.lastRewardPrice).mul(borrowOnNFT_).div(1e27));
                _nftRewards[NFTID_][token_][rewardTokens_[i]] = NftReward(newRewardPrices_[i], updatedRewards_[i]);
            }
        }
        emit updateNftRewardLog(NFTID_, token_, rewardTokens_, updatedRewards_);
    }

    /**
    * @dev updates rewards on NFT for all markets.
    * @param NFTID_ NFT ID.
    * @param pool_ address of pool.
    */
    function updateNftRewardsForAllMarkets(uint96 NFTID_, address pool_) public {
        address[] memory markets_ = _poolMarkets[pool_];
        for(uint i = 0; i < markets_.length; i++) {
            updateNftReward(NFTID_, markets_[i]);
        }
    }
...
contract Events {
    ...
    event updateNftRewardLog(uint96 indexed NFTID_, address indexed token_, address[] rewardTokens_, uint[] updatedRewards_);

    event claimLog(address indexed user_, uint NFTID_, address indexed token_, address[] rewardTokens_, uint[] rewards_);
