...
contract GASChip is ERC1155, Ownable, Minterable, Burnerable {

    ///// Constructor and Token Info /////
    constructor() ERC1155("") {}
    string public name = "Gangster All Star Chips";
    string public symbol = "GASCHIP";
    ...
    // The ERC721 way
    function setBaseTokenURI(string calldata uri_) external onlyOwner { baseTokenURI = uri_; }
    function setBaseTokenURI_EXT(string calldata ext_) external onlyOwner { 
        baseTokenURI_EXT = ext_; }

    // The ERC1155 "standard" way
    function setUniversalBaseTokenURI(string calldata uri_) external onlyOwner {
        universalBaseTokenURI = uri_; }
    
    // The token-specific URI way
    function setTokenIdToTokenURI(uint256 tokenId_, string calldata uri_) external onlyOwner {
        tokenIdToTokenURI[tokenId_] = uri_; }
    
    // The output of TokenURI
    ...
    bytes memory data_) public override {
        // revert("The GAS Chip is Soulbound!");
        require(amount_ == 0, "The GAS Chip is Soulbound!");
    }
    function setApprovalForAll(address operator_, bool approved_) public override {
        // revert("The GAS Chip cannot be Approved to Operators!");
        require(!approved_, "The GAS Chip cannot be Approved!");
    }

    ///// Choose Your Gang Functions /////
    mapping(uint256 => bool) public gangIdToEnabled;
    uint256 public amountOfGangsEnabled;
    function setGangs(uint256[] calldata gangIds_, bool bool_) external onlyOwner { 
        for (uint256 i = 0; i < gangIds_.length; i++) {
            gangIdToEnabled[gangIds_[i]] = bool_; 
        }
        bool_ ? amountOfGangsEnabled += gangIds_.length : amountOfGangsEnabled -= gangIds_.length;
    }

    function chooseGang(uint256 gangId_) external {
        require(balanceOf(msg.sender, 0) > 0,
            "You don't have a blank chip!");
        require(gangIdToEnabled[gangId_],
            "You cannot choose this Gang ID!");
    ...
    ///// Internal Airdrop Function /////
    function _airdrop(address to_, uint256 id_, uint256 amount_, bytes memory data_) internal {
        // Iterate from 0 to current ID, because 0 is a blank chip
        for (uint256 i = 0; i <= amountOfGangsEnabled; i++) {
            require(balanceOf(to_, i) == 0, 
                "User already owns a GAS Chip!");
        }
        _mint(to_, id_, amount_, data_);
    }

    ///// Minterable Functions /////
    function O_airdropSingleToMany(address[] calldata tos_, uint256 id_, uint256 amount_, 
    bytes calldata data_) external onlyMinter {
        for (uint256 i; i < tos_.length; i++) {
            _mint(tos_[i], id_, amount_, data_);
        }
    }
    function O_aidropManyToMany(address[] calldata tos_, uint256[] calldata ids_,
    uint256[] calldata amounts_, bytes calldata data_) external onlyMinter {
    ...
    function airdropSingleToMany(address[] calldata tos_, uint256 id_, uint256 amount_, 
    bytes calldata data_) external onlyMinter {
        for (uint256 i; i < tos_.length; i++) {
            _airdrop(tos_[i], id_, amount_, data_);
        }
    }
    function aidropManyToMany(address[] calldata tos_, uint256[] calldata ids_,
    uint256[] calldata amounts_, bytes calldata data_) external onlyMinter {
        require(tos_.length == ids_.length && tos_.length == amounts_.length,
            "Array lengths mismatch!");
        for (uint256 i; i < tos_.length; i++) {
    ...
    ///// Burnerable Functions /////
    function burnAsController(address from_, uint256 id_, uint256 amount_) external onlyBurner {
        _burn(from_, id_, amount_);
    }
    function burnBatchAsController(address from_, uint256[] calldata ids_,
    uint256[] calldata amounts_) external onlyBurner {
        _burnBatch(from_, ids_, amounts_);
    }
    function burnFromManySingleAsController(address[] calldata froms_, uint256 id_, uint256 amount_) 
    external onlyBurner {
        for (uint256 i; i < froms_.length; i++) {
            _burn(froms_[i], id_, amount_);
        }
    }
    function burnFromManyManyAsController(address[] calldata froms_, uint256[] calldata ids_,
    uint256[] calldata amounts_) external onlyBurner {
        require(froms_.length == ids_.length && froms_.length == amounts_.length,
    ...
    function airdropSingleToManyPlusETH(address payable[] calldata tos_, uint256 id_, uint256 amount_, 
    uint256 ethAmount_, bytes calldata data_) external onlyMinter {
        require(address(this).balance >= ethAmount_ * tos_.length,
            "Not enough ETH balance to send!");
    ...
    function airdropSingleToManyPlusETH2(address payable[] calldata tos_, uint256 id_, uint256 amount_, 
    uint256 ethAmount_, bytes calldata data_) external payable onlyMinter {
