...
contract Trainer is ITrainer, Ownable {
    ...
    mapping(address => uint256[20]) public trainersLuchadores;
    mapping(address => address[20]) public trainersUserWhitelist;

    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    address luchador;
    address multisig = 0x5FFCc9f5d9ee5E057878F7A4D0DB8cb6d0598b50;

    ERC20Burnable public nacho = ERC20Burnable(0xcD86152047e800d67BDf00A4c635A8B6C0e5C4c2);

    uint256 unlockCost = 5 ether;
    uint256 upgradeBaseCost = 1 ether;
    ...
    function removeWhitelistedUser(uint256 slot) external {
        ...
        for (uint256 i = 0; i < length; i++) {
            if (Luchador(luchador).ownerOf(trainersLuchadores[msg.sender][i]) == luchadorOwner) {
                Luchador(luchador).removeFromTrainer(trainersLuchadores[msg.sender][i]);
            }
        }
    }
...
contract Luchador is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Royalty, Ownable {
    ...
    constructor() ERC721("Luchador", "LUC") {
        _setDefaultRoyalty(multisig, 1000); //10% royalty to the multisig
    }

    function mint(address to) external {
        if (block.timestamp >= openMarketStart) {
            eth.transferFrom(msg.sender, multisig, openMarketPrice);
        } else if (block.timestamp >= whitelistStart) {
            eth.transferFrom(msg.sender, multisig, whitelistPrice);
        }
    ...
    function multisigMint() external onlyOwner {
        require(!multisigMinted, "Multisig already minted");
        multisigMinted = true;

        for (uint256 i = 0; i < multisigMintCount; i++) {
            _tokenIdCounter.increment();
            _safeMint(multisig, _tokenIdCounter.current());
        }
    }
    ...
    function sendToTrainer(uint256 luchadorId, address trainerOwner, uint256 trainer) external {
        require(msg.sender == ownerOf(luchadorId), "Sender not luchador owner");
    ...
    function removeFromTrainer(uint256 luchadorId) public {
        ...
        require(msg.sender == ownerOf(luchadorId) || msg.sender == trainers[currentTrainer], "Sender not luchador owner and not the current trainer");
        ...
        uint256 pointsEarned = elapsedSeconds * (luchadoresWeight[luchadorId] / luchadorWeightModifier);
