  ...
  EIP712Upgradeable
{
  ...
  // Struct to represent target contract and it's calldata
  struct Call {
    address target;
    bytes callData;
    uint256 value;
  ...
  bytes32 constant CALL_INFO_TYPEHASH =
    keccak256(
      "CallInfo(Call[] callList)Call(address target,bytes callData,uint256 value)"
    );
   ...
   */
  function __SBINFTMultiCall_init() external initializer {
    AdminUpgradeable.__Admin_init();
    __EIP712_init("SBINFT MultiCall", "1.0");
  ...
  /**
   * @dev Prepares keccak256 hash for Call
   *
   * @param callInfo CallInfo calldata
   */
  function _hashCallInfo(
    CallInfo calldata callInfo
  ) internal pure returns (bytes32) {
    return
      keccak256(
        abi.encode(CALL_INFO_TYPEHASH, _hashCallList(callInfo.callList))
      );
  }

  /**
   * @dev Prepares keccak256 hash for Call List
   *
   * @param callList Call[] calldata
   */
  function _hashCallList(
    Call[] calldata callList
  ) internal pure returns (bytes32) {
    bytes32[] memory keccakData = new bytes32[](callList.length);

    for (uint256 idx = 0; idx < callList.length; idx++) {
      keccakData[idx] = _hashCall(callList[idx]);
    }
   ...
   * @dev Prepares keccak256 hash for Call
   *
   * @param call Call calldata
   */
  function _hashCall(Call calldata call) internal pure returns (bytes32) {
    return
      keccak256(
        abi.encode(
          CALL_TYPEHASH,
          call.target,
          keccak256(call.callData),
          call.value
        )
      );
  }
   ...
   * @param callList Call[]
   */
  function _hashCall(Call[] calldata callList) internal pure returns (bytes32) {
    bytes32[] memory keccakData = new bytes32[](callList.length);
  ...
  /**
   * @dev Calls many functions at once
   *
   * @param callInfo CallInfo calldata
  ...
  function multiCall(
    ...
    uint256 callLength = callInfo.callList.length;
    require(callLength > 0, "MultiCall:multiCall empty call list");

    address recoverdAddress = _domainSeparatorV4()
      .toTypedDataHash(_hashCallInfo(callInfo))
      .recover(sign);
    require(isAdmin(recoverdAddress), "MultiCall:multiCall invalid signature");

    blockNumber = block.number;
    returnData = new bytes[](callLength);

    for (uint256 idx = 0; idx < callLength; idx++) {
      Call memory call = callInfo.callList[idx];

      returnData[idx] = call.target.functionCallWithValue(
        call.callData,
        call.value
      );
    }
