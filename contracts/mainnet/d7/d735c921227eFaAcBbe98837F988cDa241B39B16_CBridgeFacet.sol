...
contract CBridgeFacet is PlexSwap, ReentrancyGuard {
    ...
    /// @param cbridge The contract address of the cbridge on the source chain.
    constructor(address cbridge, address router) {
        CBRIDGE = cbridge;
        ROUTER = router;
    }
    ...
    function _cBridgeStart(uint256 amount, CBridgeDescription calldata bdesc) internal {
        ...
        bytes32 transferId = keccak256(
            abi.encodePacked(address(this), bDesc.receiver, bDesc.srcToken, amount, bDesc.dstChainId, bDesc.nonce, uint64(block.chainid))
        );
