...
contract DirectInvestorFacet is Modifiers, ReentrancyGuard {
    ...
    @dev    if investing in refferal token emit IssuedFromTrackedTokens as usual for full amount, 
    ...
    */
    function issueFromTrackedTokens(address _addr, uint256 _amount) external whenNotPaused nonReentrant {
        ...
        require(!LibReferralToken.isReferralToken(_addr), "Direct issue forbidden for refferal tokens");

        //mint pending fees before issue so fees wont affect amount calculations
        LibFee.accumulateAndMintFees(_addr);
    ...
    function redeemToTrackedTokens(address _addr, uint256 _amount) external whenNotPaused nonReentrant {
        ...
        require(!LibReferralToken.isReferralToken(_addr), "Direct redeem forbidden for refferal tokens");
