...
contract UrBox is OwnableUpgradeable {
    ...
    event Reward(address indexed addr, uint indexed types, uint indexed id, uint amount);
    ...
    function _processOpen(address addr, uint boxID) internal {
        ...
        if (boxID == 30005) {
            if (out > 30) {
                skin.mint(addr,10011);
                skin.mint(addr,10011);
                skin.mint(addr,10011);
                emit Reward(msg.sender,1,10011,3);
            }else if(out > 60){
                badge.mint(addr,40004);
                emit Reward(msg.sender,2,40004,1);
            }else{
                bvt.transfer(msg.sender,50000 ether);
                emit Reward(msg.sender,4,0,50000 ether);
            }
        } else if (boxID == 30004) {
            if (out <= 30) {
                skin.mint(addr,10010);
                skin.mint(addr,10010);
                skin.mint(addr,10010);
                emit Reward(msg.sender,1,10010,3);
            }else if(out <= 60){
                badge.mint(addr,40003);
                emit Reward(msg.sender,2,40003,1);
            }else{
                bvt.transfer(msg.sender,30000 ether);
                emit Reward(msg.sender,4,0,30000 ether);
            }
        } else if (boxID == 30003) {
            if (out <= 30) {
                badge.mint(addr,40002);
                emit Reward(msg.sender,2,40002,1);
            }else if(out <= 60){
                item.mint(addr,20004,3);
                emit Reward(msg.sender,3,20004,3);
            }else{
                bvt.transfer(msg.sender,15000 ether);
                emit Reward(msg.sender,4,0,15000 ether);
            }
        } else if (boxID == 30002) {
            if (out <= 30) {
                badge.mint(addr,40001);
                emit Reward(msg.sender,2,40001,1);
            }else if(out <= 60){
                item.mint(addr,20004,2);
                emit Reward(msg.sender,3,20004,2);
            }else{
                bvt.transfer(msg.sender,8000 ether);
                emit Reward(msg.sender,4,0,8000 ether);
            }
        } else {
            if(out <= 40){
                item.mint(addr,20004,2);
                ...
                bvt.transfer(msg.sender,2000 ether);
                emit Reward(msg.sender,4,0,2000 ether);
            }
        }
