...
contract MultichainProtocol is IUseProtocol, Ownable, Multicall, WhitelistWithdrawable, NativeReturnMods {
    ...
    function use(UseParams calldata params_) external payable {
        ...
        _hop(params_.ins[0], params_.inAmounts[0], params_.chain, params_.account, params_.args, nativeClaimer);
    }

    function _hop(TokenCheck calldata in_, uint256 inAmount_, uint256 chain_, address account_, bytes calldata args_, NativeClaimer.State memory nativeClaimer_) private returnUnclaimedNative(nativeClaimer_) {
        TokenHelper.transferToThis(in_.token, msg.sender, inAmount_, nativeClaimer_);

        address multichain = address(bytes20(args_[:20]));
        MultichainMethod method = MultichainMethod(uint8(bytes1(args_[20:21])));

        if (method == MultichainMethod.Swapout) {
            require(multichain == in_.token, "MC: swapout token mismatch");
            IMultichain(multichain).Swapout(inAmount_, account_);
        } else {
            require(_multichainWhitelist.isAccountWhitelisted(multichain), "MC: contract not allowed");
            if (method == MultichainMethod.Transfer) {
                TokenHelper.transferFromThis(in_.token, multichain, inAmount_);
            } else {
                address fromTokenOverride = address(bytes20(args_[21:41]));
                address fromToken = fromTokenOverride == _NO_TOKEN_OVERRIDE ? in_.token : fromTokenOverride;

                uint256 sendValue = TokenHelper.approveOfThis(in_.token, multichain, inAmount_);
                if (method == MultichainMethod.AnySwapOutUnderlying) {
                    IMultichain(multichain).anySwapOutUnderlying(fromToken, account_, inAmount_, chain_);
                } else if (method == MultichainMethod.AnySwapOutNative) {
                    IMultichain(multichain).anySwapOutNative{value: sendValue}(fromToken, account_, chain_);
                } else if (method == MultichainMethod.AnySwapOut) {
                    IMultichain(multichain).anySwapOut(fromToken, account_, inAmount_, chain_);
                } else {
                    revert("MC: unexpected method");
                }
                TokenHelper.revokeOfThis(in_.token, multichain);
            }
        }
    }
}