...
contract Registry is Initializable, OwnableUpgradeable, UUPSUpgradeable, EIP712Upgradeable, IERC2771Recipient {
    ...
    /// @notice Emitted when a new term is added to an agreement
    /// @param agreementHash The hash of the agreement to which the term was added
    /// @param term The address of the new term that was added
    event TermAdded(bytes32 indexed agreementHash, uint32 indexed term);

    /// @notice Emitted when a term is removed from an agreement
    /// @param agreementHash The hash of the agreement from which the term was removed
    /// @param removedTerm The address of the term that was removed
    event TermRemoved(bytes32 indexed agreementHash, uint32 indexed removedTerm);
    ...
    /*                                  constants                                 */
    /* -------------------------------------------------------------------------- */
    uint256 private constant PROMISOR_ALLOWED_TO_VOID_AGREEMENT_MASK = 0x1;
    uint256 private constant OFFERER_ALLOWED_TO_VOID_AGREEMENT_MASK = 0x2;
    uint256 private constant OFFERER_ALLOWED_TO_UPDATE_OFFERER_MASK = 0x4;
    uint256 private constant PROMISOR_ALLOWED_TO_UPDATE_PROMISOR_MASK = 0x8;
    uint256 private constant OFFERER_ALLOWED_TO_UPDATE_PROMISOR_MASK = 0x10;
    uint256 private constant PROMISOR_ALLOWED_TO_UPDATE_OFFERER_MASK = 0x20;
    uint256 private constant BOTH_SIGNATURES_REQUIRED_TO_VOID_AGREEMENT_MASK = 0x40;
    uint256 private constant BOTH_SIGNATURES_REQUIRED_TO_UPDATE_OFFERER_MASK = 0x80;
    uint256 private constant BOTH_SIGNATURES_REQUIRED_TO_UPDATE_PROMISOR_MASK = 0x100;

    bytes32 constant NOTARY_ACTION_TYPEHASH =
        keccak256("NotaryAction(bytes32 agreementHash,uint256 nonce,string action)");
    bytes32 constant UPDATE_ACTION_TYPEHASH =
        keccak256("UpdateAction(bytes32 agreementHash,uint256 nonce,address replacingAgent,string action)");
    ...
    bytes32 constant ADD_SINGLE_TERM_TYPEHASH =
        keccak256("AddSingleTerm(bytes32 agreementHash,uint256 nonce,uint32 termToAdd,string action)");
    bytes32 constant REMOVE_SINGLE_TERM_TYPEHASH =
        keccak256("RemoveSingleTerm(bytes32 agreementHash,uint256 nonce,uint32 termToRemove,string action)");
        ...
        external
    {
        ...
        if (signerNoncesUsed[nonce]) revert NonceUsed();
        if (userNoncesUsed[user][nonce]) revert NonceUsed();
    ...
    function selfBurnNonce(uint256 nonce) external {
        if (msg.sender == _trustedForwarder) revert NotGaslessTransaction();
        if (signerNoncesUsed[nonce]) revert NonceUsed();
    ...
    function updateOfferer(
        ...
        bytes32 message = _createUpdateActionDigest(agreementHash, nonce, newOfferer, "updateOfferer");
        ...
        if (_areBothPartiesRequiredToUpdateOfferer(agreement.flags)) {
            if (!verifySigner(agreement.offerer, message, offererSignature)) {
                revert InvalidOffererSignature();
            }
            if (!verifySigner(agreement.promisor, message, promisorSignature)) {
                revert InvalidPromisorSignature();
            }
        } else if (_isOffererAllowedToUpdateOfferer(agreement.flags)) {
            if (!verifySigner(agreement.offerer, message, offererSignature)) {
                revert InvalidOffererSignature();
            }
        } else if (_isPromisorAllowedToUpdateOfferer(agreement.flags)) {
            if (!verifySigner(agreement.promisor, message, promisorSignature)) {
                revert InvalidPromisorSignature();
            }
        } else {
            revert FieldNotUpdateable();
        }
    ...
    function updatePromisor(
        ...
        bytes32 message = _createUpdateActionDigest(agreementHash, nonce, newPromisor, "updatePromisor");
        ...
        if (_areBothPartiesRequiredToUpdatePromisor(agreement.flags)) {
            ...
            if (userNoncesUsed[oldPromisor][nonce]) revert NonceUsed();
            if (userNoncesUsed[offerer][nonce]) revert NonceUsed();
            userNoncesUsed[oldPromisor][nonce] = true;
            userNoncesUsed[offerer][nonce] = true;
        } else if (_isPromisorAllowedToUpdatePromisor(agreement.flags)) {
            if (!verifySigner(oldPromisor, message, promisorSignature)) {
                revert InvalidPromisorSignature();
            }
            if (userNoncesUsed[oldPromisor][nonce]) revert NonceUsed();
            userNoncesUsed[oldPromisor][nonce] = true;
        } else if (_isOffererAlloweedToUpdatePromisor(agreement.flags)) {
            if (!verifySigner(offerer, message, offererSignature)) {
                revert InvalidOffererSignature();
            }
            if (userNoncesUsed[offerer][nonce]) revert NonceUsed();
    ...
    function nullAgreement(
        ...
        if (_areBothPartiesRequiredToNullAgreement(agreementToRemove.flags)) {
            ...
            if (userNoncesUsed[agreementToRemove.offerer][nonce]) revert NonceUsed();
            if (userNoncesUsed[agreementToRemove.promisor][nonce]) revert NonceUsed();
            userNoncesUsed[agreementToRemove.offerer][nonce] = true;
            userNoncesUsed[agreementToRemove.promisor][nonce] = true;
        } else if (_isPromisorAllowedToNullAgreement(agreementToRemove.flags)) {
            if (!verifySigner(agreementToRemove.promisor, message, promisorSignature)) {
                revert InvalidPromisorSignature();
            }
            if (userNoncesUsed[agreementToRemove.promisor][nonce]) revert NonceUsed();
            userNoncesUsed[agreementToRemove.promisor][nonce] = true;
        } else if (_isOffererAllowedToNullAgreement(agreementToRemove.flags)) {
            if (!verifySigner(agreementToRemove.offerer, message, offererSignature)) {
                revert InvalidOffererSignature();
            }
            if (userNoncesUsed[agreementToRemove.offerer][nonce]) revert NonceUsed();
    ...
    function createAgreement(
        ...
        if (agreement.promisor == address(0)) revert CannotSetToAddressZero();
        if (agreement.offerer == address(0)) revert CannotSetToAddressZero();
        if (block.timestamp > agreement.expiration) {
            revert AgreementExpirationMustBeInTheFuture();
        }
    ...
    function singleCreateAgreementsBulk(
        ...
        {
            if (signerNoncesUsed[singleNonce]) revert NonceUsed();
            if (userNoncesUsed[promisor][singleNonce]) revert NonceUsed();
        ...
        unchecked {
            for (uint256 i; i < _agreements.length; ++i) {
                ...
                if (agreement.promisor != promisor) revert NotCorrectPromisor();
    ...
    function amendAgreementEfficient(
...

        {
            for (uint256 i; i < termsToAdd.length;) {
                if (_containsElement(agreements[agreementHash].terms, termsToAdd[i])) revert TermAlreadyIncluded();
                if (!_agreementTermExists(termsToAdd[i])) {
                    revert TermDoesNotExist();
                }
    ...
    function amendAgreement(
        ...
        unchecked {
            for (uint256 i; i < termsToAdd.length; ++i) {
                ...
                if (_containsElement(terms, term)) revert TermAlreadyIncluded();
                if (!_agreementTermExists(term)) revert TermDoesNotExist();
                terms.push(term);
            }
        }
        {
    ...
    function delegateSigners(address[] calldata delegates, bool status) external {
        ...
        unchecked {
            for (uint256 i; i < delegates.length; ++i) {
                ...
                if (delegate == address(0)) revert CannotSetToAddressZero();
                if (delegate == msg.sender) revert CannotSetSelfToDelegate();
    ...
    {
        Agreement storage agreement = agreements[agreementHash];
        if (agreement.offerer == address(0)) revert AgreementDoesNotExist();
        ...
        returns (bytes32)
    {
        return keccak256(
            abi.encodePacked(
                "\x19\x01",
                _domainSeparatorV4(),
                keccak256(abi.encode(BULK_CREATE_AGREEMENTS_TYPEHASH, agreementHashes, nonce))
            )
        );
    }
    ...
    function _createUpdateActionDigest(
        ...
        return keccak256(
            abi.encodePacked(
                ...
                _domainSeparatorV4(),
                keccak256(
                    abi.encode(UPDATE_ACTION_TYPEHASH, _agreementHash, nonce, replacingAgent, keccak256(bytes(action)))
                )
            )
        );
    }
    ...
    function _checkIsGroupNonceAvailable(uint256 nonce, address offerer, address promisor) internal view {
        if (signerNoncesUsed[nonce]) revert NonceUsed();
        if (userNoncesUsed[offerer][nonce]) revert NonceUsed();
        if (userNoncesUsed[promisor][nonce]) revert NonceUsed();
    }
    ...
    function _isOffererAllowedToNullAgreement(uint256 flag) internal pure returns (bool) {
        return flag & OFFERER_ALLOWED_TO_VOID_AGREEMENT_MASK != 0;
    }

    function _isPromisorAllowedToNullAgreement(uint256 flag) internal pure returns (bool) {
        return flag & PROMISOR_ALLOWED_TO_VOID_AGREEMENT_MASK != 0;
    }

    function _areBothPartiesRequiredToNullAgreement(uint256 flag) internal pure returns (bool) {
        return flag & BOTH_SIGNATURES_REQUIRED_TO_VOID_AGREEMENT_MASK != 0;
    }

    //------------------Update Offerer Bitmask Functions------------------//
    function _isOffererAllowedToUpdateOfferer(uint256 flag) internal pure returns (bool) {
        return flag & OFFERER_ALLOWED_TO_UPDATE_OFFERER_MASK != 0;
    }

    function _isPromisorAllowedToUpdateOfferer(uint256 flag) internal pure returns (bool) {
        return flag & PROMISOR_ALLOWED_TO_UPDATE_OFFERER_MASK != 0;
    }

    function _areBothPartiesRequiredToUpdateOfferer(uint256 flag) internal pure returns (bool) {
        return flag & BOTH_SIGNATURES_REQUIRED_TO_UPDATE_PROMISOR_MASK != 0;
    }

    //------------------Update Promisor Bitmask Functions------------------//
    function _isPromisorAllowedToUpdatePromisor(uint256 flag) internal pure returns (bool) {
        return flag & PROMISOR_ALLOWED_TO_UPDATE_PROMISOR_MASK != 0;
    }

    function _isOffererAlloweedToUpdatePromisor(uint256 flag) internal pure returns (bool) {
        return flag & OFFERER_ALLOWED_TO_UPDATE_OFFERER_MASK != 0;
    }

    function _areBothPartiesRequiredToUpdatePromisor(uint256 flag) internal pure returns (bool) {
        return flag & BOTH_SIGNATURES_REQUIRED_TO_UPDATE_PROMISOR_MASK != 0;
    }
