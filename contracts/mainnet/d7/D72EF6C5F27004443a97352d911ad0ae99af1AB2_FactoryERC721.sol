...
contract ERC721Token is ERC721Enumerable, Pausable, Ownable {
    ...
    */
    function setOnlyInvestor(address _investor) external onlyOwner {
     ...
     /**
    * @notice set the artist address
    */
    function setOnlyArtist(address _artist) external onlyOwner {
        artist = _artist;
    }
    ...
    */
    function Burn() public onlyInvestor{

        for(uint i=0; i < categories.length; i++){
            
            categories[i].maxSupply=categories[i].counterSupply;
        }
...
contract FactoryERC721 is Ownable {
    ...
    function crossMintFactory(address payable _contract, address _to, uint _category, uint _amount) public payable  { 
        require(msg.sender == 0x12618F46FF6E841470Bf71F428AAe41Ee5BC3c39,
        "This function is for Crossmint only."
        );
        uint totalPrice = ERC721Token(_contract).getPriceByCategory(_category) * _amount;
        uint ratioMaticUsdc = getPrice();
        uint256 priceMaticInUSDC = ((10 ** 12) * (ratioMaticUsdc ** 2 )) / (2 ** 192);
        require(
            (totalPrice/10**18) <= ((msg.value * priceMaticInUSDC)/10**18),
            "Matic value sent is not correct"
        ); 
        ERC721Token(_contract).crossMint(_to,_category, _amount);
    }

    function mintUSDCFactory(address payable _contract, uint _quantity, uint categoryId) public payable  {
        uint price = ERC721Token(_contract).getPriceByCategory(categoryId);
        require(contractERC20(usdc).balanceOf(msg.sender) >= _quantity * price, "Not enought USDC");
        contractERC20(usdc).transferFrom(msg.sender, _contract, _quantity * price);
