...
contract VaultAdmin is VaultStorage {
    ...
    ****************************************/
    ...
    /**
     * @dev Set maximum amount of CASH that can at any point be minted and deployed
     * to strategy (used only by ConvexcashMetaStrategy for now).
     * @param _threshold CASH amount with 18 fixed decimals.
     */
    function setNetCashMintForStrategyThreshold(uint256 _threshold)
        external
        onlyGovernor
    {
        /**
         * Because `netCashMintedForStrategy` check in vault core works both ways
         * (positive and negative) the actual impact of the amount of CASH minted
         * could be double the threshold. E.g.:
         *  - contract has threshold set to 100
         *  - state of netCashMinted is -90
         *  - in effect it can mint 190 CASH and still be within limits
         *
         * We are somewhat mitigating this behaviour by resetting the netCashMinted
         ...
         * amount and not have problems with current netCashMinted being near
         * limits on either side.
         */
        netCashMintedForStrategy = 0;
        netCashMintForStrategyThreshold = _threshold;
        emit NetCashMintForStrategyThresholdChanged(_threshold);
    }
     ...
     */
    function setDepositFeeAddress(address _address) external onlyGovernor {
        depositFeeAddress = _address;
    }
     ...
     *      Setting to the zero address disables this feature.
     */
    function setWithdrawFeeAddress(address _address) external onlyGovernor {
        withdrawFeeAddress = _address;
    }
     ...
     *      Setting to the zero address disables this feature.
     */
    function setPerformanceFeeAddress(address _address) external onlyGovernor {
        performanceFeeAddress = _address;
    }

    function setAutobribeAddress(address _address) external onlyGovernor {
        autobribeAddress = _address;
    }
    ...
    /**
     * @dev Set CASH Meta strategy
     * @param _cashMetaStrategy Address of CASH meta strategy
     */
    function setCashMetaStrategy(address _cashMetaStrategy)
        external
        onlyGovernor
    {
        cashMetaStrategy = _cashMetaStrategy;
        emit CashMetaStrategyUpdated(_cashMetaStrategy);
    }
    ...
    */
    ...
    function setFeeExempt(address _addr, bool _value) external onlyGovernor {
        exemptFromFees[_addr] = _value;
    }
}
