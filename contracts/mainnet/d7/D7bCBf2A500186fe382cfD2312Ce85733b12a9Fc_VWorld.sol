...
/// @notice could potentially same gas here if I would know max limit variables
struct LandData {
    /// x-axis
    uint256 horizontal;
    /// y-axis
...
contract VWorld {
    ...
    /// Set's base avaiableLands
    constructor() {
        uint256[3][3] memory baseLandData = [
            [uint256(1), uint256(1), uint256(1)],
            [uint256(2), uint256(2), uint256(2)],
            [uint256(3), uint256(3), uint256(3)]
        ];
        for (uint256 i = 1; i < baseLandData.length + 1; i++) {
            uint256[3] memory selectedArr = baseLandData[i - 1];
