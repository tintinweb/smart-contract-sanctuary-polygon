...
import "../libraries/PolygearLib.sol";

interface ITokenURIResolver {
    function tokenURI(
        uint256 tokenId,
        PolygearLib.Origin origin,
        uint16 extraData
...
library PolygearLib {
    ...
    function getOriginLabel(Origin origin) public pure returns (string memory) {
        if (origin == Origin.Other) {
            return "Other";
        } else if (origin == Origin.Giveaway) {
            return "Giveaway";
        } else if (origin == Origin.NFB) {
            return "NFB";
        } else if (origin == Origin.AlphaTesting) {
            return "AlphaTesting";
        } else if (origin == Origin.TournamentWin) {
...
interface ISummoningEffectTraitsMap {
    function hashToTraits(
        bytes32 hash
    ) external view returns (ISummoningEffectTypes.SummoningEffect memory);

    function registerTraits(
        bytes32 hash,
        ISummoningEffectTypes.SummoningEffect memory traits
    ) external;
}
...
import "./interfaces/ISummoningEffectTraitsMap.sol";
import "./interfaces/ISummoningEffectTypes.sol";
import "./libraries/SummoningEffectTraits.sol";
import "../libraries/PolygearLib.sol";

contract SummoningEffectTokenURIResolver is ITokenURIResolver, Initializable, OwnableUpgradeable {
    ...
    function setBaseURI(string memory _baseURI) external onlyOwner {
        ...
        baseURI = _baseURI;
        emit BaseURIChanged(oldBaseURI, _baseURI);
    }
    ...
    function tokenURI(
        ...
        (
            ISummoningEffectTypes.ElementType elementType,
            ISummoningEffectTypes.SummoningLevel summoningLevel
        ) = SummoningEffectTraits.unpackSummoningEffect(extraData);
                        ...
                        bytes(
                            string(
                                abi.encodePacked(
                                    ...
                                    '","attributes": [',
                                    '{"trait_type":"Type","value":"Summoning Effect"},',
                                    '{"trait_type":"Origin","value":"',
                                    PolygearLib.getOriginLabel(origin),
                                    '"},',
                                    '{"trait_type":"Summoning Type","value": "',
                                    SummoningEffectTraits
                                        .getSummoningEffectTypeLabel(
                                            elementType
                                        ),
                                    '"},',
                                    '{"trait_type":"Summoning Level","value":"',
                                    SummoningEffectTraits
                                        .getSummoningLevelLabel(summoningLevel),
                                    '"}',
                                    "]}"
                                )
                            )
        ...
        return
            string(
                abi.encodePacked(
                    baseURI,
                    "T00",
                    uint256(elementType).toString(),
                    "L00",
                    uint256(summoningLevel).toString(),
                    suffix
                )
            );
    }
