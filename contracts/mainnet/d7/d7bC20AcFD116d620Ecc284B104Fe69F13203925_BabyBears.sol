...
contract BabyBears is ERC721, Ownable, VRFConsumerBase {
    ...
    */
    ...
    function payMultipleMaintenanceFees(uint[] calldata _ids, uint[] calldata _numMonths) external payable {
        ...
        for(uint i = 0; i < _ids.length; i++) {
            ...
            require(nextTimePeriod > 0, "Next time to pay should be Greator than zero");
    ...
    //Pay maintanence fees for a single nft, for a set number of months
    function payMaintenanceFee(uint _ids, uint _numMonths) external payable {

        uint feePerMonth = monthlyTributeFee;
        uint totalFee;

        uint nextTimePeriod = nextTimePeriodToPayFee[_ids];

        require(nextTimePeriod > 0, "Next time to pay should be Greator than zero");

        totalFee += _numMonths * feePerMonth;

        nextTimePeriod += (_numMonths * subscriptionInterval);

        //make sure the amount of time being purchased doesn't cover maintanence for over 90 days
        require((block.timestamp + 90 days) >= nextTimePeriod, "Can only maintain up to 90 days in advance");

        nextTimePeriodToPayFee[_ids] = nextTimePeriod;
    ...
    * Requires the sender to be the owner of this address
    */
    function AirdropMint(address[] calldata _reciepient) external onlyOwner {

        require(isPublicMint == false, "Cannot call when minting is public");

        uint total;

        for(uint i = 0; i < _reciepient.length; i++) {
