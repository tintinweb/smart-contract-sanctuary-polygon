...
contract Auction is ERC721Holder, Ownable {
    ...
    address public overrideFinalizeAddress;
    uint256 public finalCooldown = 0;

    constructor(address _auctionStarter, address _EWP, address _WeirdToken, address _overrideFinalizeAddress) {
        auctionStarter = _auctionStarter;
        EWP = ERC721(_EWP);
        WeirdToken = ERC20(_WeirdToken);
        overrideFinalizeAddress = _overrideFinalizeAddress;
    }
    ...
    function bid(uint256 tokenId, uint256 amount) public {
        ...
        if(block.timestamp + finalCooldown > timestampFinished[tokenId]) {
            timestampFinished[tokenId] = block.timestamp + finalCooldown;
        }
    ...
    function upcomingAuctions() public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < totalAuctions; i++) {
            if(timestampStarted[auctionId[i]] >= block.timestamp) {
    ...
    function previousAuctions() public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < totalAuctions; i++) {
            if(timestampFinished[auctionId[i]] <= block.timestamp) {
