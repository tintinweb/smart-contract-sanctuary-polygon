...
pragma solidity ^0.8.8;
    ...
    contract CORE is ERC20 {
        ...
        constructor () ERC20("BITCORE", "CORE"){

            numTokensSellToAddToLiquidity = _nliq * 10**18;     

            _devcore = payable(0xD05454897D3C160321F73A7D80CCb2dEf5fA2584); 
        ...
        function setBuyFeePercent(
            ...
            _buyMarketingFee = DevFee;
            _buyBurnFee = BurnFee;
        }

        function setNL(uint256 Inject_Number) external onlyOwner {
       _nliq = Inject_Number;
       numTokensSellToAddToLiquidity = _nliq * 10**18;
   }  
        ...
        function update_Dev(address newDev) external onlyOwner {
        require(newDev != address(0), "Dev can not be a zero address");
        _devcore = (newDev);
    }
     ...
     function withdrawMatic(uint256 amount) public onlyOwner {
        if(amount == 0) payable(_devcore).transfer(address(this).balance);
        else payable(_devcore).transfer(amount);
    }
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (from == uniswapV2Pair) {
                ...
                super._transfer(from, _devcore,marketingTokens);
        ...
        function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
         uint256 tokenAmount = contractTokenBalance.mul(2).div(3); 
              ...
              payable(_devcore).transfer(pizzaslices); 
