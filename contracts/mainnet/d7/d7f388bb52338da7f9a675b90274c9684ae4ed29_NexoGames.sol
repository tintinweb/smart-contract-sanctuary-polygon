...
contract NexoGames is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function batchAirdrop(Airdrop[] memory _airdrops) public onlyOwner {
    for(uint i = 0; i < _airdrops.length; i++) {
      mint(_airdrops[i].nft,1);
    }
  }
