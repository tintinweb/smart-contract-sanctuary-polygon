    ...
    ContextMixin
{
    ...
    using Strings for string;
    using SafeMath for uint256;
    using ERC721Operations for ERC721Operations.ERC721Data;
    ...
    ERC721Operations.ERC721Data tokenData;
     ...
     *     contract that implements the DynamicURI interface (0xc87b56dd).
     */
    function setDynamicURI(DynamicURI dynamicURI)
        public
        noBannedAccounts
        onlyOwnerOrRole(CREATOR_ROLE_NAME)
    {
        tokenData.setDynamicURI(dynamicURI);
    }
...
// File: RestaurantReliefNFT.sol

/**
 * @title Restaurant Relief NFT
 * Nominate a restaurant in need: https://restaurantrelief.io/nominate
 * 
 * The mission of the Restaurant Relief DAO (decentralized autonomous 
 * organization) is to directly support local restaurants and provide financial 
 * relief to those determined by the NFT holding community.
 * 
 * Many restaurants are still struggling post-pandemic with spiking energy 
 * costs, staffing shortages, and compounding debt, so a DAO was created 
 * (https://restaurantrelief.io) to raise funds and distribute according to 
 * community votes.  
 * 
 * 80% of net proceeds go into a Restaurant Relief Fund (RRF) controlled by the 
 * DAO. When the RRF reaches $500,000, the community (NFT owners) will vote on 
 * which two restaurants will each receive $250,000. Restaurants will also 
 * receive $5,000 out of the fund for every 100 nominations.
 * 
 * Thank you for your support!
 */
contract RestaurantReliefNFT is ViciERC721, INominator {
    mapping(uint256 => string) nftNames;
    mapping(uint256 => bool) needsNomination;

    constructor() ViciERC721("Restaurant Relief DAO", "RRD", 100000) {
        setContractURI(
            "https://storage.googleapis.com/arrival-shadow-motion-newman/vici/restaurant-relief/RestaurantReliefMetadata.json"
        );
    }
    ...
    /**
     * @dev sets the NFT name metadata element.
     * 
     * Requirements:
     * - caller MUST be the contract owner or have the CREATOR role
     * - `tokenId` MAY refer to an invalid token. This can be called before 
     *      minting.
     */
    function setNFTName(uint256 tokenId, string memory name)
        public
        onlyOwnerOrRole(CREATOR_ROLE_NAME)
    {
    ...
    /**
     * @dev sets the NFT name metadata element for a batch of tokens.
     * 
     * Requirements:
     * - caller MUST be the contract owner or have the CREATOR role
     * - `tokenIds` and `names` MUST be the same length.
     * - elements in `tokenIds` MAY refer to an invalid token. This can be 
     *     called before minting.
     */
    function setNFTNames(uint256[] memory tokenIds, string[] memory names)
        public
        onlyOwnerOrRole(CREATOR_ROLE_NAME)
    {
        require(tokenIds.length == names.length, "array length mismatch");
        for (uint256 i = 0; i < names.length; i++) {
            nftNames[tokenIds[i]] = names[i];
        }
    }

    /**
     * @dev returns whether this token has nominated a restaurant.
     * @dev this is used to tell the difference between a token that has not
     *     submitted a nomination and one who has but their nomination has not
     *     been recorded yet.
     */
    function hasNominated(uint256 tokenId) public view returns (bool) {
        return needsNomination[tokenId];
    }

    /**
     * @dev sets whether this token has nominated a restaurant.
     */
    function setNominated(uint256 tokenId, bool nominated)
        public
        onlyOwnerOrRole(CUSTOMER_SERVICE)
    {
        needsNomination[tokenId] = nominated;
    }
}