...
contract MNFT is ERC721Upgradeable {
  ...
  // Mapping from token ID to token's metadata
  mapping(uint256 => string) public tokenURIs;
  string public baseTokenURI;

  event UpdateUser(uint tokenId, address user, uint64 expires);

//  constructor() ERC721("MyNFT", "MyNFT") {}
  function initialize() public initializer {
    ERC721Upgradeable.__ERC721_init('Mock NFT 2', 'MNFT 2');
  }
  ...
  function mint(address to, string memory _tokenURI) external returns (uint256) {
    ...
    // Mint the token and transfer it to the specified address
    _mint(to, tokenId);
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    return bytes(baseTokenURI).length > 0 ? string(abi.encodePacked(baseTokenURI, tokenURIs[_tokenId])) : "";
  }
