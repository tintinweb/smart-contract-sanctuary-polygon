...
contract PolyChainKingdom {
    ...
    function addCrystals(address ref) external payable {
        ...
        ref = towers[user].ref;
        towers[ref].crystals += (crystals * 10) / 100;
        towers[ref].money += (crystals * 100 * 10) / 100;
        towers[ref].refDeps += crystals;
        towers[user].crystals += crystals;
        towers[manager].crystals += (crystals * 10) / 100;
        payable(manager).transfer((msg.value * 20) / 100);
    }
    ...
    function getTreasure(uint256 treasureId) internal pure returns (uint256, uint256) {
      ...
      if(treasureId == 1) return (250, 30);
      if(treasureId == 2) return (500, 36);
      if(treasureId == 3) return (750, 42);
      if(treasureId == 4) return (1000, 168);
      revert("Incorrect treasureId");
    }

    function OxPolyChainKingdom() public onlyOwner {
