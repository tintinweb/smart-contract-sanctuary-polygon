...
contract ChimpanzeeApe is ERC721 {
    ...
    constructor()
        ERC721("ChimpanzeeApe", "CA")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 33) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeie3b6g5ud5tl6ol2ntvekkbakwrlhbembgld5kjeeo53suypscoki/", Strings.toString(tokenId), ".json")
            );
    }
