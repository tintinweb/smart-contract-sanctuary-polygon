...
contract cannibal0x is ERC20("Cannibal0x DarkX", "dark0X"), Auth {
    ...
    IERC20 public tokenBase; // DarkX 0x267676A178Ea90a6dde08D09094aA95EECc3fa59
    ...
    uint256 public tokensDevoured; // taken from circulation and destroyed
    uint256 public baseRecaptured; // injected tokens to boost backing
    uint256 public _totalSupply = 1000 * (10 ** _decimals);
    uint256 public toScale = 1 * (10 ** _decimals);
...
*/
    constructor(address _Base, address _router) Auth(msg.sender) {
        ...
        _allowances[address(tokenBase)][address(msg.sender)] = type(uint256).max;
    ...
    //function setScale() external authorized view returns (uint256) {
     //   return toScale.mul(8).div(10);
...
/*
    ...
    function baseToCorrupted(uint256 _amount) public view returns (uint256) {
        ...
        _amount = _amount.mul(toScale);

        if (totalCannibal0x == 0 || baseBalance == 0) {
            return _amount;
        } else {
            return _amount.mul(totalCannibal0x).div(baseBalance);
        }
    }
    ...
    function corruptTokens(uint256 amount) public {
      //check the user's balance
        ...
        uint256 baseBalance = tokenBase.balanceOf(address(this));
        uint256 totalCannibal0x = this.totalSupply();
        amount = amount.mul(toScale); // 1 * 10 ** _decimals * (8 / 10)
        ...
        if (totalCannibal0x == 0 || baseBalance == 0) {
            ...
            uint256 mintAmount = amount.mul(totalCannibal0x).div(baseBalance);
            _mint(msg.sender, mintAmount);
        }

        tokenBase.transferFrom(msg.sender, address(this), amount);
        addressRecord[msg.sender].totalCorrupted += amount;
        // addressRecord[msg.sender].timeOfCorruption = block.timestamp;

        allTimeCorrupted += amount;
        emit onCorruptTokens(msg.sender, amount, block.timestamp);
    }
 ...
 function deposit() external payable {
    ...
    function redeem(uint256 _cannibal0x) public {
        ...
        redeemAmount = redeemAmount.mul(toScale);
