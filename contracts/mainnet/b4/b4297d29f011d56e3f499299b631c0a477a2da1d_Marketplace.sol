...
contract Marketplace is BaseContract {
  ...
  function extendOrder(address _nftContractAddress, uint _tokenId, address _paymentAddress, uint _weekNumber) external {
    ...
    uint currentPrice = _calculateCurrenPriceByTokenAddress(order, currentPriceAfterDiscount, _paymentAddress, _weekNumber);
  ...
  function _calculateCurrenPriceByTokenAddress(Order storage _order, uint _currentPrice, address _paymentAddress, uint _weekNumber) private view returns(uint) {
    if (payments[_paymentAddress].isStableCoin) {
      _currentPrice = _order.orderType == IMarketPlace.OrderType.installmentRental
      ? _weekNumber * _currentPrice
      : _currentPrice;
    ...
    return (_order.orderType == IMarketPlace.OrderType.installmentRental ? _currentPrice * _weekNumber : _currentPrice);
  }
  ...
  function _getCurrentPrice(address _nftContractAddress, uint _tokenId, uint _weekNumber, address _paymentAddress) private view returns (uint, uint, uint) {
    ...
    currentPrice = _calculateCurrenPriceByTokenAddress(order, currentPriceAfterDiscount, _paymentAddress, _weekNumber);
