...
contract Gallery is Ownable, IGallery, Multicall {
	...
	///@param artistSplit split rate to be transferred to artist while selling nft
	...
	function sellNft(
		...
		Token.onSell = true;
		fee.artistFee = artistFee;
		Token.hasPhysicalTwin = physicalTwin;
		Token.USD = USD;
		fee.gallerySplit = gallerySplit;
		listOfTokenIdsForSale.add(tokenId);
		nft.setApprovalForAll(address(market), true);
		if (Token.totalSell == 0) nft.setArtistRoyalty(tokenId, Token.artist, uint96(artistFee));
		market.sell(
			tokenId,
			amount,
			artistSplit,
	...
	function mintAndSellNft(
		...
		uint256 tokenId = mintNFT(_uri, artist, thirdParty, feeExpiryTime);
		sellNft(tokenId, amount, artistSplit, gallerySplit, artistFee, thirdPartyFee, physicalTwin, true);
