...
contract DexInterfaceFacet is Modifiers, ReentrancyGuard {
    ...
    modifier onlyAllowedDex(address _dexAddress) {
        require(isDexAllowed(_dexAddress), "Unknown DEX address");
        _;
    }

    function allowDex(address _dexAddr) external onlyOwner {
        LibDexInterface.DexInterfaceStorage storage ds = LibDexInterface.diamondStorage();
    ...
    function issueFromDexCalldata(
        ...
        for (uint256 i = 0; i < tl.length; i++) {
            ...
            if (tl.tokens[i] == _call.tokenAddress) {
                require(_call.tokenAmount >= tl.amounts[i], "From amount is not enough for direct token transfer");
    ...
    function redeemToDexCalldata(
        ...
        require(isDexAllowed(_call.dex), "Unknown DEX address");
        ...
        for (uint256 i = 0; i < tl.length; i++) {
            require(IERC20(tl.tokens[i]).balanceOf(_addr) >= tl.amounts[i], "Tracked token reserve is not enought");
            ...
            // transfer or allow tracked tokens
            {
                ...
                if (tl.tokens[i] == _call.tokenAddress) {
                    ...
                    res = portToken.externalCall(tl.tokens[i], abi.encodeWithSelector(IERC20.approve.selector, _call.dex, tl.amounts[i]), 0);
                    if (res.length > 0) {
        ...
        require(oldUserBalance + _call.tokenAmount <= newUserBalance, "Not enought tokens received by user");
        ...
        nonReentrant
    {
        ...
        require(_call.fromToken != _call.toToken, "Source and target tokens are the same");
        ...
        PortToken(_addr).externalCall(_call.dex, _call.dexCalldata, 0, "DEX call failed");
        ...
        require(newToBalance >= oldToBalance + _call.toAmount, "Not enough to tokens received from DEX");
