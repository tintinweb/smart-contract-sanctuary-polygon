...
contract D1verseNFT is IERC721, IERC721Metadata, IERC2981, ILazyMint {
    ...
    function setNftName(string memory nft_name) public onlyDAO {
        _name = nft_name;
    }

    function setNftSymbol(string memory nft_symbol) public onlyDAO {
        _symbol = nft_symbol;
    }

    function setContractURI(address contract_uri) public onlyDAO {
    ...
    function setD1verseDex(address addr, bool flag) public onlyOperator {
        if(!flag) {
    ...
    function migrate(uint256[] memory tokenIds) public onlyDAO {
    ...
    function lazyMint(address to, uint256 tokenId, address receiver, uint96 royaltyRate) public override onlyD1verseDEX {
        // _mint(to, tokenId);
        require(to != address(0), "ERC721: mint to the zero address");
        require(!_exists(tokenId), "ERC721: token already minted");
    ...
    function setRoyaltyInfo(uint256 tokenId, address receiver, uint96 royaltyRate) public override onlyD1verseDEX {
        _royaltyInfo[tokenId].royaltyRate = royaltyRate;
        _royaltyInfo[tokenId].receiver = receiver;
    }
