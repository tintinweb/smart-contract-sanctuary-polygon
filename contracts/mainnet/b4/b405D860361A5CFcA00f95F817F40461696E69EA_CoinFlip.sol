...
contract CoinFlip is Ownable, ReentrancyGuard{
  ...
  uint256 public overFlow = 29000 ether;
  ...
  event Result(uint256 id, uint256 bet, uint256 randomSeed, uint256 amount, address player, uint256 winAmount, uint256 randomResult, uint256 time, uint256 loseStreak, uint256 winStreak, uint256 currentRun);
 ...
 function manageRef(uint256 amount) internal {
  ...
  function checkWinner(uint8 random) internal {
      ...
      emit Result(games[gameId].id, games[gameId].bet, games[gameId].seed, games[gameId].amount, games[gameId].player, winAmount, random, block.timestamp, LoseStreak[currentRun][games[gameId].player],WinStreak[currentRun][games[gameId].player],currentRun);
