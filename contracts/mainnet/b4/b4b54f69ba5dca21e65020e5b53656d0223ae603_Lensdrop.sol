...
contract Lensdrop {
   ...
   function escrowErc1155Tokens(address payable referrer, string memory rewardDetails, address tokenAddress, uint256[] memory tokenIds, uint256 noOfRecipients, uint256 deadline) 
    ...
    function reward(address[] memory recipients) public {
        require(msg.sender == controller, "only controller can reward users");
        require(Escrows[totalEscrows].paid = false, "reward already paid");
        require(recipients.length <= Escrows[totalEscrows].noOfRecipients);
        require(Escrows[totalEscrows].deadline >= block.timestamp, "Contest still ongoing");
        for (uint256 i=0; i<recipients.length; i++) {
            payable(recipients[i]).transfer(Escrows[totalEscrows].amount);
        }
        if (recipients.length < Escrows[totalEscrows].noOfRecipients) {
            uint256 balance = Escrows[totalEscrows].amount * (Escrows[totalEscrows].noOfRecipients - recipients.length);
            payable(Escrows[totalEscrows].user).transfer(balance);
        }
        Escrows[totalEscrows].paid = true;
    }

    function rewardTokens(address[] memory recipients) public {
        require(msg.sender == controller, "only controller can reward users");
        require(Escrows[totalEscrows].paid = false, "reward already paid");
        require(recipients.length <= Escrows[totalEscrows].noOfRecipients);
        require(Escrows[totalEscrows].deadline >= block.timestamp, "Contest still ongoing");

        IERC20Upgradeable token = IERC20Upgradeable(Escrows[totalEscrows].token);
        for (uint256 i=0; i<recipients.length; i++) {
            token.transferFrom(address(this), recipients[i], Escrows[totalEscrows].amount);
        }
        if (recipients.length < Escrows[totalEscrows].noOfRecipients) {
            uint256 balance = Escrows[totalEscrows].amount * (Escrows[totalEscrows].noOfRecipients - recipients.length);
            token.transferFrom(address(this), Escrows[totalEscrows].user, balance);
        }
        Escrows[totalEscrows].paid = true;
    }

    function rewardErc721Tokens(address[] memory recipients) public {
        require(msg.sender == controller, "only controller can reward users");
        require(Escrows[totalEscrows].paid = false, "reward already paid");
        require(recipients.length <= Escrows[totalEscrows].noOfRecipients);
        require(Escrows[totalEscrows].deadline >= block.timestamp, "Contest still ongoing");

        IERC721Upgradeable token = IERC721Upgradeable(Escrows[totalEscrows].token);
        for (uint256 i=0; i<recipients.length; i++) {
            token.transferFrom(address(this), recipients[i], Escrows[totalEscrows].tokenIds[i]);
        }
        if (recipients.length < Escrows[totalEscrows].noOfRecipients) {
            uint256 balance = (Escrows[totalEscrows].noOfRecipients - recipients.length);
            for (uint256 i=balance; i<Escrows[totalEscrows].noOfRecipients; i++) {
                token.transferFrom(address(this), Escrows[totalEscrows].user, Escrows[totalEscrows].tokenIds[i]);
            }
        }
        Escrows[totalEscrows].paid = true;
    }

     function rewardErc1155Tokens(address[] memory recipients) public {
        require(msg.sender == controller, "only controller can reward users");
        require(Escrows[totalEscrows].paid = false, "reward already paid");
        require(recipients.length <= Escrows[totalEscrows].noOfRecipients);
        require(Escrows[totalEscrows].deadline >= block.timestamp, "Contest still ongoing");

        IERC1155Upgradeable token = IERC1155Upgradeable(Escrows[totalEscrows].token);
        for (uint256 i=0; i<recipients.length; i++) {
            token.safeTransferFrom(address(this), recipients[i], Escrows[totalEscrows].tokenIds[i], 1, "");
        }
        if (recipients.length < Escrows[totalEscrows].noOfRecipients) {
            uint256 balance = (Escrows[totalEscrows].noOfRecipients - recipients.length);
            for (uint256 i=balance; i<Escrows[totalEscrows].noOfRecipients; i++) {
                token.safeTransferFrom(address(this), Escrows[totalEscrows].user, Escrows[totalEscrows].tokenIds[i], 1, "");
            }
        }
        Escrows[totalEscrows].paid = true;
    }
