...
contract Csgfod {
    ...
    function deposit(uint256 _dai) external {
        _deposit(msg.sender, _dai,0,false);
        emit Deposit(msg.sender, _dai);
    }
    function _deposit(address _user, uint256 _amount, uint8 _isReDept, bool isSplitDept) private {
    ...
    function depositBySplit(uint256 _amount) external {
        ...
        _deposit(msg.sender, _amount,1,true);
