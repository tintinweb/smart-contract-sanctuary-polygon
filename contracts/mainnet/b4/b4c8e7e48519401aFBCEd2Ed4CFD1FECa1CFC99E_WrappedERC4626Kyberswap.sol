...
abstract contract BaseConcentratedLiquidityStrategy is HarvestableApyFlowVault {
    ...
    int24 private immutable ticksDown;
    int24 private immutable ticksUp;
    ...
    function _tickSpacing() internal virtual view returns(int24);
...
interface IKyberSwapElasticLM {
    ...
    function pools(uint256 pId) external view returns (LMPoolInfo memory);
}
...
library KyberswapLibrary {
    ...
    function getPoolData(Data storage self) public view returns(int24 currentTick, uint160 sqrtPriceX96) {
        (sqrtPriceX96, currentTick, , ) = self.pool.getPoolState();
    }
    ...
    function collectFarmingRewards(Data storage self) public {
        ...
        (, uint256 staked) = self.farm.positions(self.positionTokenId);
        if (staked == 0) {
            return;
        }
    ...
    function enterFarming(Data storage self) public {
        ...
        (address currentOwner, uint256 currentLiquidity) = self.farm.positions(self.positionTokenId);
        ...
        liqs[0] = getPositionData(self).liquidity - currentLiquidity;
        if (liqs[0] > 0) {
    ...
    function exitFarming(Data storage self) public {
        ...
        uint256[] memory liqs = new uint256[](1);
        ids[0] = self.positionTokenId;
        (, liqs[0]) = self.farm.positions(self.positionTokenId);
        if (liqs[0] > 0) {
...
// File: BaseKyberswapStrategy.sol

abstract contract BaseKyberswapStrategy is
    BaseConcentratedLiquidityStrategy
{
    ...
    constructor(
        ...
        kyberswap = KyberswapLibrary.Data({
            ...
            endTime: farm.pools(farmPid).endTime,
    ...
    function _getPositionData()
        ...
        returns (ConcentratedLiquidityLibrary.PositionData memory)
    {
        return kyberswap.getPositionData();
    }
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        (amount0, amount1) = kyberswap.decreaseLiquidity(liquidity);
        kyberswap.enterFarming();
    }

    function _mint(int24 tickLower, int24 tickUpper, uint256 amount0, uint256 amount1)
        internal
        override
    {
    ...
    function _harvest() internal virtual override {
        ...
        kyberswap.collect(0, 0);
        kyberswap.enterFarming();
        assetConverter.safeSwap(rewardToken, asset(), IERC20(rewardToken).balanceOf(address(this)));
        super._harvest();
    }
}
...
contract WrappedERC4626Kyberswap is BaseKyberswapStrategy {
    ...
    constructor(
        ...
        BaseKyberswapStrategy(params.pool, params.positionManager, params.farmPid, params.farm, params.rewardToken)
