...
contract UserInventoryFacet is Base {
    ...
    function getDirectBuys(address user_)
        external
        view
        returns (uint256[] memory directBuyIds, uint256[] memory balances)
    {
        if (_usersFacet().isUserFrozen(user_)) {
            revert UserIsFrozen();
        }
        UserInventoryStorage storage userInventory = s.userInventory[user_];
        directBuyIds = userInventory.directBuysIds;
        balances = new uint256[](directBuyIds.length);
        for (uint256 i = 0; i < directBuyIds.length; i++) {
            balances[i] = userInventory.directBuysBalance[directBuyIds[i]];
        }
    }
    ...
    function getUserProjectBalance(address user_, uint256 projectId_) 
        external 
        view 
        returns (uint256 projectBalance) 
    {
        ...
        projectBalance = s.userInventory[user_].projectBalance[projectId_];
    }
