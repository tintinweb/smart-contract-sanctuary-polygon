...
# Checker to whitelist wallets that are allowed to create locks for others
future_lock_creator_checker: public(address)
lock_creator_checker: public(address)
...
@external
def commit_lock_creator(addr: address):
    """
    @notice Set an external contract to check for approved lock creator wallets
    @param addr Address of Smart contract checker
    """
    assert msg.sender == self.admin
    self.future_lock_creator_checker = addr


@external
def apply_lock_creator():
    """
    @notice Apply setting external contract to check approved lock contract wallets
    """
    assert msg.sender == self.admin
    self.lock_creator_checker = self.future_lock_creator_checker


@internal
def assert_lock_creator(addr: address):
    """
    @notice Check if the call is from a whitelisted smart contract, revert if not
    @param addr Address to be checked
    """
    checker: address = self.lock_creator_checker
    if checker != ZERO_ADDRESS:
        if SmartWalletChecker(checker).check(addr):
            return
    raise "Lock creator not allowed"
...
def _deposit_for(_sender: address, _addr: address, _value: uint256, unlock_time: uint256, locked_balance: LockedBalance, type: int128):
    ...
    @param _sender Wallet from which to take to=he locked funds
...
def create_lock_for(_addr: address, _value: uint256, _unlock_time: uint256):
    ...
    """
    self.assert_lock_creator(msg.sender)
