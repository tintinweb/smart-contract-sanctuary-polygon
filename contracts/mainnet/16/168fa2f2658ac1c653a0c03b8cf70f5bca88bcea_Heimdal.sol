 ...
 * functions have heimdaln added to mitigate the well-known issues around setting
...
// File contracts/Heimdal.sol


pragma solidity 0.6.12;
contract Heimdal is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 48h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 150000 ether;
  
    // Distribution for airdrops wallet
    uint256 public constant INITIAL_AIRDROP_WALLET_DISTRIBUTION = 200000 ether;

    // Have the rewards heimdaln distributed to the pools
    bool public rewardPoolDistributed = false;

    /* ================= Taxation =============== */
    // Address of the Oracle
    address public heimdalOracle;
    ...
    /**
     * @notice Constructs the HEIMDAL ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("heimdal.money", "HEIMDAL") {
        // Mints 1 HEIMDAL to contract creator for initial pool setup
    ...
    function _getHeimdalPrice() internal view returns (uint256 _heimdalPrice) {
        try IOracle(heimdalOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Heimdal: failed to fetch HEIMDAL price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _heimdalPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_heimdalPrice >= taxTiersTwaps[tierId]) {
    ...
    function setHeimdalOracle(address _heimdalOracle) public onlyOperatorOrTaxOffice {
        require(_heimdalOracle != address(0), "oracle address cannot be 0 address");
        heimdalOracle = _heimdalOracle;
    }
    ...
    /**
     * @notice Operator mints HEIMDAL to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of HEIMDAL to mint to
     * @return whether the process has heimdaln done
     */
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentHeimdalPrice = _getHeimdalPrice();
            currentTaxRate = _updateTaxRate(currentHeimdalPrice);
            if (currentHeimdalPrice < burnThreshold) {
                burnTax = true;
            }
        }
