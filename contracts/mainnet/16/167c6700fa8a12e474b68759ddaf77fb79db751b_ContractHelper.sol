...
contract ContractHelper is Multicall{


    function checkIfContracts(address[]memory _addresses) external view returns (bytes memory contractArray) {
        require(_addresses.length < 32, "Cannot Process more than 32 addresses at a time");
    ...
    function returnTokenNames(address[] memory _addresses) external view returns (string[] memory) {
        bytes memory isERC20Result = _getIsERC20Bytes(_addresses);
        string[] memory NameArray = new string[](isERC20Result.length);
        for(uint i = 0; i < isERC20Result.length; i++){
            address addr = _addresses[i];
            if(isERC20Result[i] == 0x01){
                ...
                if(success){ //is lp
                    ...
                    (,bytes memory token1name) = abi.decode(token1addr,(address)).staticcall(abi.encodeWithSignature("symbol()"));
                        NameArray[i] = string.concat(string(token0name),"/",string(token1name),"-",string(lpName));
                    }
                if(!success){
                    bytes memory name;
    ...
    function _getIsERC20Bytes(address[] memory _addresses) internal view returns (bytes memory){
        uint len = _addresses.length;
