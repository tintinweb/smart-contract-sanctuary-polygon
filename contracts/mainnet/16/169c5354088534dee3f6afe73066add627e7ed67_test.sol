...
contract test is ERC721, Ownable {
    ...
    uint constant WITHDRAW_FEE = 1000000; // 1 USDT
    ...
    function _balanceOfNFT(address _user) private view returns (uint256) {
            uint256 counter = 0;
            for (uint256 i = 0; i < tokens.length; i++) {
                if (tokens[i].owner == _user) {
                    counter++;
    ...
    function withdrawUSDTFunds(uint _tokenId, uint _amount) external {
        ...
        _updateInvestPool(_tokenId, _amount, false);

        uint withdrawAmount = _amount - WITHDRAW_FEE;
    ...
    function _updateTokenInfo(uint _tokenId, address srcToken, address dstToken, uint returnAmount, uint spentAmount, bool takeProfit) internal{
        ...
        if (srcToken == address(USDT)){
            require(spentAmount <= investors[_tokenId].USDT.tokenAmount, "Token balance is less than the requested");
            investors[_tokenId].USDT.tokenAmount -= spentAmount;

        } else if (srcToken == address(WMATIC)) {
            require(spentAmount <= investors[_tokenId].MATIC.tokenAmount, "Token balance is less than the requested");
            ...
            require(spentAmount <= investors[_tokenId].BTC.tokenAmount, "Token balance is less than the requested");
            ...
            require(spentAmount <= investors[_tokenId].ETH.tokenAmount, "Token balance is less than the requested");
            ...
            require(spentAmount <= investors[_tokenId].LINK.tokenAmount, "Token balance is less than the requested");
            ...
            require(spentAmount <= investors[_tokenId].SOL.tokenAmount, "Token balance is less than the requested");
