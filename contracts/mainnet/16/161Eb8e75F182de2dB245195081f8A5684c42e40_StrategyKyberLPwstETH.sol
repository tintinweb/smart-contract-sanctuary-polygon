...
import "../../interfaces/kyber/Quoter.sol";
...
contract StrategyKyberLPwstETH is StratFeeManagerInitializable {
    ...
    address[] public ethToLp0Route;
    address[] public ethToLp1Route;
   
   // Use elastic router
    bytes public outputToNativePath;
    bytes public nativeToEthPath;
    address[] public ethToLp0Path;
    address[] public ethToLp1Path;
    ...
    function initialize(
        ...
        ethToLp0Route = _routes[0];
        ethToLp1Route = _routes[1];

        outputToNativePath = _elasticPaths[0];
        nativeToEthPath = _elasticPaths[1];
        ethToLp0Path = _paths[0];
        ethToLp1Path = _paths[1];

        address[] memory route = pathToRoute(nativeToEthPath);
        native = route[0];
        eth = route[route.length - 1];
        address[] memory outputRoute = pathToRoute(outputToNativePath);
        output = outputRoute[0];
        lpToken0 = ethToLp0Route[ethToLp0Route.length - 1];
        lpToken1 = ethToLp1Route[ethToLp1Route.length - 1];
        ...
        for (uint i; i < rewardTokens.length;) {
            ...
            if (bal >= rewards[rewardTokens[i]].minAmount) {
                if (rewards[rewardTokens[i]].useElastic) {
                    ...
                    dmmSwap(rewards[rewardTokens[i]].rewardToNativeRoute, bal, rewards[rewardTokens[i]].rewardPath);
                }
                unchecked {
                    ++i;
                }
    ...
    function dmmSwap(address[] memory _route, uint256 _amount, address[] memory _path) internal {
        IERC20[] memory route = buildPath(_route);
    ...
    function buildPath(address[] memory _route) internal pure returns (IERC20[] memory route) {
        uint256 len = _route.length;
        route = new IERC20[](len);
        for (uint j; j < len; j++) {
            route[j] = IERC20(_route[j]);
        }
    }
    ...
    function addLiquidity() internal {
        uint256 nativeBal = IERC20(native).balanceOf(address(this));
        if (nativeBal > 0)  UniV3Actions.kyberSwap(elasticRouter, nativeToEthPath, nativeBal);
        uint256 ethBal = IERC20(eth).balanceOf(address(this));
        uint256 lp0Amt = ethBal / 2;
        uint256 lp1Amt = ethBal - lp0Amt;
        ...
        uint256 out0;
        if (lpToken0 != eth) {
            IERC20[] memory lp0Path = buildPath(ethToLp0Route);
            uint256[] memory outs0 = IDMMRouter(unirouter).getAmountsOut(lp0Amt, ethToLp0Path, lp0Path);
            out0 = outs0[outs0.length - 1];
        } else {
            out0 = lp0Amt;
        }
        ...
        uint256 out1;
        if (lpToken1 != eth) {
            IERC20[] memory lp1Path = buildPath(ethToLp1Route);
            uint256[] memory outs1 = IDMMRouter(unirouter).getAmountsOut(lp0Amt, ethToLp1Path, lp1Path);
            out1 = outs1[outs1.length - 1];
        } else {
            out1 = lp1Amt;
        }
        ...
        lp1Amt = ethBal * 1e18 / (ratio + 1e18);
        lp0Amt = ethBal - lp1Amt;

        if (lpToken0 != eth) {
            dmmSwap(ethToLp0Route, lp0Amt, ethToLp0Path);
        }

        if (lpToken1 != eth) {
            dmmSwap(ethToLp1Route, lp1Amt, ethToLp1Path);
        }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(eth).safeApprove(unirouter, type(uint).max);
