...
contract AdanAnniversary is ERC721, ERC721Enumerable, Pausable, Ownable, ERC721Burnable {
    using Strings for uint256;
    ...
    // The collection URI for the metadata of the collection
    string private collectionURI;
    // The address allowed to mint new tokens
    address public minter;

    constructor() ERC721("Adan NFT birthday cake 2023", "ADAN3") {}
    ...
    function mint(address to, uint256 tokenId) external {
        require(msg.sender == minter || msg.sender == owner(), "Only minter or owner can mint");
        require(totalSupply() < MAX_SUPPLY, "Max supply reached");
        require(balanceOf(to) == 0, "Only 1 token per address");
        require(tokenId >= 1 && tokenId <= MAX_SUPPLY, "Token ID not in range");
        _safeMint(to, tokenId);
    }
