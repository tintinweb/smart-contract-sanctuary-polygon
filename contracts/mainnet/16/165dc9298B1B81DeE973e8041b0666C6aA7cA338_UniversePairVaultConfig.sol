...
contract UniversePairVaultConfig is IUniversePairVaultConfig, Ownable {
    ...
    mapping(address => IUniswapV3Staker.IncentiveKey[])
        private incentiveListMap;

    PairVaultParam defaultParam =
        PairVaultParam({performanceFee: 10, diffTick: 300});
    ...
    function removeIncentive(
        ...
        IUniswapV3Staker.IncentiveKey[]
            storage incentiveList = incentiveListMap[vaultAddress];
        ...
        returns (IUniswapV3Staker.IncentiveKey[] memory activeList)
    {
        ...
        IUniswapV3Staker.IncentiveKey[] memory incentiveList = incentiveListMap[
            vaultAddress
        ];
        // Temp
        IUniswapV3Staker.IncentiveKey[]
            memory tempList = new IUniswapV3Staker.IncentiveKey[](
                incentiveList.length
            );
        uint256 number;
        IUniswapV3Staker.IncentiveKey memory key;
    ...
    function _pairVaultParam(address vaultAddress)
        internal
        view
        returns (PairVaultParam memory param)
    {
...
interface IUniversePairVaultV2 is
    IERC20,
    IVaultOwnerActions,
    ...
    IVaultEvents
{
    ...
    function positionList(uint256 index)
        external
        view
        returns (
            uint256 tokenId,
    ...
    function getBals(uint256 share, uint256)
        external
        view
        returns (uint256, uint256);
    ...
    function getBalancedAmount(uint256 amount0Desired, uint256 amount1Desired)
        external
        view
        returns (
            uint256,
