...
contract FarmAction is BaseSiloAction{
    ...
    constructor(string memory _name, address _siloFactory){
        ...
        usesTakeFee = true;
        feeName = "Gravity Farm Harvest Fee";
    }
    ...
    function enter(address implementation, bytes memory configuration, bytes memory inputData) public override returns(uint[4] memory outputAmounts){
        ...
        outputAmounts[0] = _takeFee(implementation, reward, address(info.rewardToken));
    }
    ...
    function exit(address implementation, bytes memory configuration, bytes memory outputData) public override returns(uint[4] memory outputAmounts){
        ...
        if(inputAmounts[0] == 0){//remove entire stake
            uint reward = Farm.pendingReward(address(this));
            Farm.withdraw(info.amount);
            outputAmounts[0] = _takeFee(implementation, reward, output[0]);
        }
        else{
            if(inputAmounts[0] > info.amount){
                inputAmounts[0] = info.amount;//withdraw max withdrawable
            }
            uint reward = Farm.pendingReward(address(this));
            Farm.withdraw(inputAmounts[0]);///@notice withdrawn amount is "dropped" from outputAmounts
            outputAmounts[0] = _takeFee(implementation, reward, output[0]);
        }
    }

    function createConfig(address[4] memory _inputs, address[4] memory _outputs, address _farm, uint _trigger) public pure returns(bytes memory configData){
    ...
    function validateConfig(bytes memory configData) public view override returns(bool){
        ...
        uint trigger;
        (input,output,farm, trigger) = abi.decode(configData, (address[4],address[4],address,uint));
        if(farm == address(0)){
            return false;
        }
        IFarmV2 Farm = IFarmV2(farm);
        FarmInfo memory info = Farm.farmInfo();
        if(address(info.rewardToken) != output[0]){
            return false;
        }
        if(address(info.lpToken) != input[0]){
            return false;
        }
        return true;
    }
    ...
    function showBalances(address _silo, bytes memory _configurationData) external view override returns(ActionBalance memory){
        (,,address farm) = abi.decode(_configurationData, (address[4],address[4],address));
        IFarmV2 Farm = IFarmV2(farm);
        UserInfo memory ui = Farm.userInfo(_silo);
        FarmInfo memory fi = Farm.farmInfo();
        if(address(fi.lpToken) != address(fi.rewardToken)){//assume collateral is an LP
            IUniswapV2Pair pair = IUniswapV2Pair(address(fi.lpToken));
            uint token0Bal = ui.amount * IERC20(pair.token0()).balanceOf(address(pair))/ pair.totalSupply();
            uint token1Bal = ui.amount * IERC20(pair.token1()).balanceOf(address(pair))/ pair.totalSupply();
            return ActionBalance({
                collateral: ui.amount,
                debt: 0,
                collateralToken: address(fi.lpToken),
                debtToken: address(0),
                collateralConverted: 2*token0Bal,
                collateralConvertedToken: pair.token0(),
                lpUnderlyingBalances: string(abi.encodePacked(Strings.toString(token0Bal), ",", Strings.toString(token1Bal))),
                lpUnderlyingTokens: string(abi.encodePacked(Strings.toHexString(uint160(pair.token0()), 20), ",", Strings.toHexString(uint160(pair.token1()), 20)))
            });
        }
        else{
            return ActionBalance({
                ...
                collateralToken: address(fi.lpToken),
                debtToken: address(0),
                collateralConverted: ui.amount,
                collateralConvertedToken: address(0),
                lpUnderlyingBalances: string(abi.encodePacked((Strings.toString(ui.amount)))),
                lpUnderlyingTokens: string(abi.encodePacked(Strings.toHexString(uint160(address(fi.lpToken)), 20)))
            });
        }
    }
    ...
    function actionValid(bytes memory _configurationData) external view override returns(bool, bool){
        ...
        IFarmV2 Farm = IFarmV2(farm);
        FarmInfo memory fi = Farm.farmInfo();
        return (ISiloFactory(getFactory()).actionValid(address(this)), block.number < fi.endBlock);//second bool overwritten to logically account for the end block
    }

    function checkMaintain(bytes memory configuration) public view override returns(bool){
        address farm;
        uint trigger;
        (,,farm,trigger) = abi.decode(configuration, (address[4],address[4],address,uint));
        IFarmV2 Farm = IFarmV2(farm);
        FarmInfo memory fi = Farm.farmInfo();
        if(block.number < fi.endBlock && Farm.pendingReward(msg.sender) >= trigger){
            return true;
        }
        return false;
    }
