...
contract RelationChristmas is Ownable, Initializable,SemanticSBCore {
    ...
    address[] _whiteLists;
    uint256  constant _pIndex = 1;
    uint256  constant _oIndex = 1;

    mapping(address =>  bool) public _ownedSBT;
    ...
    function isClaimed(address account)public view returns(bool){
        return _ownedSBT[account];
    }

    function addWhiteList(address[] memory addressList) external onlyMinter {
    ...
    function mint() external {
        require(whiteList[msg.sender], "Relation : permission denied");
        require(!_ownedSBT[msg.sender], "Relation : already minted");
        ...
        _ownedSBT[msg.sender] = true;

    }


    function claim(Signature memory signature, uint256 expireTime) external{
        require(!_ownedSBT[msg.sender], "You have successfully claimed it");
