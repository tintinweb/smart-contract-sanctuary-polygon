...
contract WhaleWars {
    ...
    // minimum deposit is 1 MATIC
    uint private minimumDeposit = 1 ether;

    // Every epoch is about 1.5 minutes
    uint private constant EPOCH_DURATION_IN_BLOCKS = 50;

    // Default epoch game time is 7 epochs which is about 11 minutes
    uint private settlementDuration = EPOCH_DURATION_IN_BLOCKS * 7;

    // Default owner tax on winnings is 4.9%
    uint private ownerTaxBasisPoints = 490;
    ...
    function getBullsWon(uint256 _epoch) public returns (bool) {
        ...
        do
        {
            epoch = epochs[epoch].nextEpochWithDeposit;
            if (epoch == 0 || epoch >= currentEpoch) {
                if (epoch == 0 && delta != 0 && currentEpoch > settlementEpoch) {
                    break;
                }
                revert("Not yet settled");
            }
            delta += epochs[epoch].depositsDelta;
        } while(epoch <= settlementEpoch || delta == 0);
    ...
    // Apply new settings only if 5 days have passed since the settings were announced.
    function ownerApplySettingsAnnouncement() external {
        require(msg.sender == owner);
        require(block.number > announcementBlock + 36000);
