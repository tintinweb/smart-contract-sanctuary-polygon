...
contract HolaMatic {
  ...
  function initialize(address payable _admin, address payable _admin2) public{
    require(isIntinalized == false,"Already done");
        ...
        actualData = [200,1000,5000,20000,50000,100000,500000,1000000,5000000];
        requriedData =[200,1200,6200,26200,76200,176200,676200,1676200,6676200];
        eligibleFor =[100,300,1000,2000,5000,10000,50000,100000,500000];
        maxupline = 50;
        maxdownline = 50;
		singleLegLength++;
        isIntinalized =true;
