...
contract testRouter is Modifiers {
    ...
    function swapTokensToEth0(Swap memory swap, bytes memory sig) public checkActive checkDeadline(swap.deadline) checkSign(swap,sig) {
        transferErc20In(swap.token0, swap.valueWithFees, 1);
        validateAllowance(swap.token0, getValueWithoutFees(swap.valueWithFees));
        //bytes[] memory results = uniswapRouter.multicall(swap.data);
        //return results;
    ...
    function swapTokensToEth1(Swap memory swap, bytes memory sig) public checkActive checkDeadline(swap.deadline) checkSign(swap,sig) returns (bytes[] memory) {
        transferErc20In(swap.token0, swap.valueWithFees, 1);
        validateAllowance(swap.token0, getValueWithoutFees(swap.valueWithFees));
        bytes[] memory results = uniswapRouter.multicall(swap.data);
        return results;
    ...
    function swapTokensToEth2(Swap memory swap, bytes memory sig) public checkActive checkDeadline(swap.deadline) checkSign(swap,sig) returns (uint256) {
        transferErc20In(swap.token0, swap.valueWithFees, 1);
        validateAllowance(swap.token0, getValueWithoutFees(swap.valueWithFees));
        bytes[] memory results = uniswapRouter.multicall(swap.data);
        return uint256(bytes32(results[results.length - 1]));
    }

    function swapTokensToEth3(Swap memory swap, bytes memory sig) public checkActive checkDeadline(swap.deadline) checkSign(swap,sig) {
        transferErc20In(swap.token0, swap.valueWithFees, 1);
        validateAllowance(swap.token0, getValueWithoutFees(swap.valueWithFees));
        bytes[] memory results = uniswapRouter.multicall(swap.data);
