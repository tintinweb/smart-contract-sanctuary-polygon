...
contract Bond is Ownable {
    ...
    function buyWithMatic(uint256 id) external payable {
        ...
        uint256 vsqAmount = (msg.value * 1e5) / vsqRate_;
        ...
        IERC20(vsqAddress_).bondMint(vsqAmount * 1e4);
        IERC20(vsqAddress_).approve(
            lockerAddress_,
            ((vsqAmount / 1e5) * (100000 - fee_)) * 1e4
        );
        IERC20(vsqAddress_).approve(
            revenueAddress_,
            ((vsqAmount / 1e5) * fee_) * 1e4
        );
        ...
        IERC20(revenueAddress_).addRevenue(((vsqAmount / 1e5) * fee_) * 1e4);
    }
    ...
    function buyWithCrypto(
        ...
        uint256 tokenAmount = amount.mul(rate_[crypto]);
