...
contract StakingFactory is Ownable {
    ...
    function deployPool(
        ...
        assembly {
            StakingAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
...
contract StakingInitializable is Ownable, ReentrancyGuard {
    ...
    // The block number when luffy mining ends.
    uint256 public bonusEndBlock;

    // The block number when luffy mining starts.
    ...
    // luffy tokens created per block.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 luffyReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare = accTokenPerShare.add(
                luffyReward.mul(PRECISION_FACTOR).div(stakedTokenSupply)
            );
            return
