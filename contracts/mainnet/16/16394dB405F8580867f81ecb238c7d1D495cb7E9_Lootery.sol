...
contract Lootery is ERC721Tradable, VRFConsumerBase {
    ...
    LooteryState public state = LooteryState.IDLE;
    ...
    IERC20 constant WBNB = IERC20(0xA649325Aa7C5093d12D6F98EB4378deAe68CE23F);
    ...
    function drawWinners() external onlyOwner {
        require(state == LooteryState.IDLE, "Already closed");
        _drawWinners();
    }
    ...
    function withdrawPrize() external {
        ...
        for (uint256 index = 0; index < winningTickets.length; index++) {
            if (msg.sender == ownerOf(winningTickets[index])) {
                IERC20 prizeToken = _allPrizes[index];
                prizeToken.transfer(msg.sender, 1 ether);
                emit PrizeCollected(
                    msg.sender,
