...
contract ERC721 is ERC165, IERC721, TokenAccessControl {
...
contract CC_Vehicles is ERC721, ERC2981{
    ...
    function mintTo(address to, address royaltyReceiver, uint32 vehicleType) public onlyOwnerOrAuthorizedUser{
        tokenIdCounter.increment();
        _mint(to, tokenIdCounter.current());

        _vehicles[tokenIdCounter.current()] = vehicleType;
    ...
    function initialMintBatch(uint256 count, address royaltyReceiver, uint32 vehicleType) public onlyOwner{
        for(uint i=0;i<count;i++){
            mintTo(msg.sender, royaltyReceiver, vehicleType);
        }
    }

    function getVehicle(uint256 tokenId) public view returns (uint32) {
