...
interface IFCNProduct {
    ...
    function vaults(address vaultAddress) external view returns (FCNVaultMetadata memory);
...
interface IFCNVault is IERC4626 {
...
contract TestERC20 is ERC20 {
    constructor(uint256 _totalSupply) ERC20("Test", "TST") {
...
import { OptionBarrier, VaultStatus, FCNVaultMetadata } from "../Structs.sol";
...
contract FCNProductViewer {
    ...
    function getFCNVaultInfo(
        address fcnProductAddress,
        address fcnVaultAddress
    ) external view returns (FCNVaultInfo memory) {
        IFCNProduct fcnProduct = IFCNProduct(fcnProductAddress);
        IFCNVault fcnVault = IFCNVault(fcnVaultAddress);

        FCNVaultMetadata memory vaultMetadata = fcnProduct.vaults(fcnVaultAddress);

        OptionBarrier[] memory optionBarriers = fcnProduct.getOptionBarriers(fcnVaultAddress);

        return
            FCNVaultInfo({
                totalAssets: fcnVault.totalAssets(),
                totalSupply: fcnVault.totalSupply(),
                vaultStart: vaultMetadata.vaultStart,
                tradeDate: vaultMetadata.tradeDate,
                tradeExpiry: vaultMetadata.tradeExpiry,
                aprBps: vaultMetadata.aprBps,
                tenorInDays: vaultMetadata.tenorInDays,
                underlyingAmount: vaultMetadata.underlyingAmount,
                currentAssetAmount: vaultMetadata.currentAssetAmount,
                totalCouponPayoff: vaultMetadata.totalCouponPayoff,
                vaultFinalPayoff: vaultMetadata.vaultFinalPayoff,
                queuedWithdrawalsSharesAmount: vaultMetadata.queuedWithdrawalsSharesAmount,
                queuedWithdrawalsCount: vaultMetadata.queuedWithdrawalsCount,
                optionBarriersCount: vaultMetadata.optionBarriersCount,
                leverage: vaultMetadata.leverage,
                vaultAddress: vaultMetadata.vaultAddress,
                vaultStatus: vaultMetadata.vaultStatus,
                isKnockedIn: vaultMetadata.isKnockedIn,
                optionBarriers: optionBarriers
            });
    }
}