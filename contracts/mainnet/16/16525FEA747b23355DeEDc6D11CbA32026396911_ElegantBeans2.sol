...
contract ElegantBeans2 is Ownable, AccessControl {
    ...
    function setCoach(uint _token, address _owner) public {
    ...
    function returnRaceFull(uint _num) public view returns(uint[] memory){
        ...
        for (uint i=0; i<_size; i++){
            ...
            uint third = perfByRacer[_rc].thirdLeg;
    ...
    function buildRace(uint[] calldata _racers,uint[] calldata _legs) public onlyOwner{
        ...
        uint i = 0;
        uint racing = _racers.length;

        for(i; i < racing; i++ ){ //go through every racer

            _temp.tempID =  _racers[i];
            
            perfByRacer[rHack].id = _temp.tempID;

            racersByRace[raceNum].push(rHack);  
            ...
            perfByRacer[rHack].firstLeg = _temp.newN + boosts.boost + boosts.accel;
            _nonce = _nonce + 1;
            _temp.newN = randomNumber(_nonce, 5, 12);
                        
            perfByRacer[rHack].secondLeg = _temp.newN + boosts.boost;
            _nonce = _nonce + 1;

            _temp.newN = randomNumber(_nonce, 8, 14);
            perfByRacer[rHack].thirdLeg = _temp.newN + boosts.boost;
            perfByRacer[rHack].steps = returnSteps(perfByRacer[rHack].firstLeg,perfByRacer[rHack].secondLeg,perfByRacer[rHack].thirdLeg);
            
            perfByRacer[rHack].raceIn = raceNum;
