    ...
    */
    function getPrice(address _token, address standardToken) public view returns(uint price){
        ...
        if(standardToken != polygonUSDT && standardToken != polygonUSDC){
            ...
            return price/10**uint256(decimalDiff);
        }
        price = price/1e12;
    }
}

contract HquickSwap is Hcommon{
    ...
    function swapTokensForOtherToken(address tokenIn,address tokenOut,uint256 amountIn) external {
        address router = getRouter();
        address[] memory path = new address[](2);
        path[0] = address(tokenIn);
        ...
        uint inbalance = _tokenBalance(tokenIn);
        // make the swap
        ...
        require(inbalance > _tokenBalance(tokenIn));
    }
    ...
    function addLiquidity(address token1, address token2 ,uint256 amount1,uint256 amount2) external {
        ...
        require(token1Balance > _tokenBalance(token1));
    }

    function removeLiquidity(address token0,address token1,uint removeAmount) external {
        address router = getRouter();
        address lpToken = getPair(token0,token1);
        IERC20(lpToken).approve(router,removeAmount);

        uint lpTokenBalance = _lpTokenBalance(lpToken);
        ...
        require(lpTokenBalance > _lpTokenBalance(lpToken));
    }

    function removeLiquidity(address LpToken,uint removeAmount) external {
    ...
    function getPair(address token0,address token1) public view returns(address){
        return IUniswapV2Factory(getFactory()).getPair(token0,token1);
    }

    // input token0 amount, output token1 amount
    function getAmountsOut(uint amountIn ,address[] calldata path) external view returns (uint[] memory amounts){
      amounts = UniswapV2Library.getAmountsOut(getFactory(), amountIn,path) ;
    }

    // input lpTokenAmount, output tokenA,tokenB amount
    ...
    function _lpTokenBalance(address token) private view returns(uint){
       return IUniswapV2Pair(token).balanceOf(address(this));
    }
