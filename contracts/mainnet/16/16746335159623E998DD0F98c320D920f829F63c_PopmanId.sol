...
contract PopmanId is Ownable {
    address _popmanNFT;

    constructor (address popmanAdd) {
        _popmanNFT = popmanAdd;
    }

    function getUserNFTList(address user) public view returns (uint256[] memory popmanList) {
        uint256 popmanBal = IERC721(_popmanNFT).balanceOf(user);

        popmanList = new uint256[](popmanBal);

        for (uint i; i < popmanBal; i++) {
            popmanList[i] = IERC721Enumerable(_popmanNFT).tokenOfOwnerByIndex(user, i);
        }
    }

    function setNFTAddress(address newNFTAdd) public onlyOwner {
        _popmanNFT = newNFTAdd;
    }
}
