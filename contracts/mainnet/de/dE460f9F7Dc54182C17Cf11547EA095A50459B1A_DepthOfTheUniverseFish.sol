...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _baseURI() internal view virtual returns (string memory) {
        return 'ipfs://QmWK9kxtnaQiKnGdmGBvcbouiH3G5kfaxBKmLuXhJoD7nS/';
    }
...
contract newerRandom {
    ...
    function getNextRandom() internal returns (uint256 _randomNumber) {
        require(nextIndex <= _inclusiveUpperBorder, 'Used every index');
        if (nextIndex < _inclusiveUpperBorder) {
            uint256 rand = (uint256(keccak256(abi.encodePacked(block.timestamp, blockhash(block.number - 1), msg.sender, nextIndex))) %
                (_inclusiveUpperBorder - nextIndex + 1)) + nextIndex;
            _randomNumber = recorded[rand] > 0 ? recorded[rand] : rand;
            recorded[rand] = recorded[nextIndex] > 0 ? recorded[nextIndex] : nextIndex;
            nextIndex++;
        } else {
...
contract DepthOfTheUniverseFish is newerRandom(1, 1500), ERC721('DepthOfTheUniverseFish', 'DOTUF') {
    uint256 public price = 6900000000000000000;
    bool _paused = false;

    address public liquidityAddress = 0x600b76e0F98e9284d8Ac3d4AAfF9ce7E002B02BF;
    address public projectAddress = 0x65AF853b4Fe298B43E4b77f522f7C6eF83cB5C9B;
    ...
    function setFundedAddresses(address _liquidityAddress, address _projectAddress) public onlyOwner {
    ...
    function distributeFunds() private {
        liquidityAddress.call{value: (address(this).balance * 70) / 100}('');
