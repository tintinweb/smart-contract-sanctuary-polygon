...
contract SubManager is ISubManager, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event TransferAccepted(uint256 indexed subscriptionTokenId, address indexed owner);
    ...
    function charge(uint256 subscriptionTokenId) external override nonReentrant whenNotPaused {
        ...
        require(subInfo.transferProtected == false, "subscription transfer protected");
    ...
    function  _charge(uint merchantTokenId, uint planIndex, uint subscriptionTokenId, address subOwner, address payeeAddress, address paymentToken, uint pricePerBillingPeriod, uint billingTime) internal {
        ...
        if (adjustedPrice > 0) {
            IERC20(paymentToken).transferFrom(subOwner, payeeAddress, adjustedPrice);
        }

        if (platformFee > 0) {
            IERC20(paymentToken).transferFrom(subOwner, platformFeeAddress, platformFee);
        }
    ...
    function acceptTransfer(uint256 subscriptionTokenId) external override whenNotPaused {
        ISubInfoManager.SubInfo memory subInfo = ISubInfoManager(subInfoManager).getSubInfo(subscriptionTokenId);
        address subOwner = IERC721(subTokenManager).ownerOf(subscriptionTokenId);
