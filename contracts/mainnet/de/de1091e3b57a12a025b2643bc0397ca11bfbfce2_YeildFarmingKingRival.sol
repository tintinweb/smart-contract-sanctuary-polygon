...
contract YeildFarmingKingRival is ReentrancyGuard, AccessControl, Ownable, Pausable {
    ...
    uint256 public totalTokenInYear; // reward prize in 1 year
    ...
    event ludenaRewardEvent(address user, uint256 prizeId, uint256 prize, uint256 timeUnlock);
    ...
    address tokenPrizeAddress;
    constructor(address _tokenPrizeAddress, address _lpTokenAddress, uint256 _lockTimePrize) {
        _setRoleAdmin(ADMIN_ROLE, ADMIN_ROLE);
        _setRoleAdmin(MANAGERMENT_ROLE, MANAGERMENT_ROLE);
        _setupRole(ADMIN_ROLE, _msgSender());
        _setupRole(MANAGERMENT_ROLE, _msgSender());
        tokenPrizeAddress = _tokenPrizeAddress;
     ...
     */
    function changeLockTimePrize(uint256 _lockTimePrize) external onlyRole(ADMIN_ROLE) {
        lockTimePrize = _lockTimePrize;
     ...
     * @param _totalTokenInYear: total prize in a year
     */
    function changeRewardPrizeIn1Second(uint256 _totalTokenInYear) external onlyRole(ADMIN_ROLE) {
        require(isEnable, "pool is closed");
        if (_totalTokenInYear > 0){
            isEnable = true;
        } else {
            isEnable = false;
        ...
        rewardPrizeIn1Second = _totalTokenInYear.div(31536000); //31536000 = 86400 * 365 day 
        totalTokenInYear = _totalTokenInYear;
    ...
    function DepositStake(uint256 amountLPLiquidity) external whenNotPaused {
        ...
        if (amountStake > 0){
           _ludenaReward(amountStake, timeStake, timeNow);
        } else {
     ...
     * @param amountLPLiquidity: lp token stake
     */
    function WithdrawStake(uint256 amountLPLiquidity) external nonReentrant {
        uint256 amountStake = userInfor[msg.sender].amountStake;
        uint256 timeStake = userInfor[msg.sender].timeStake;
        uint256 timeNow;
        require(amountLPLiquidity > 0, "amount LP need bigger than zero");
     ...
     */
    function ludenaReward() external nonReentrant {
        uint256 amountStake = userInfor[msg.sender].amountStake;
        require(amountStake > 0, "user don't have token LP in pool");
        uint256 timeStake = userInfor[msg.sender].timeStake;
        uint256 timeNow;
     ...
     */
    function _ludenaReward(uint256 amountStake, uint256 timeStake, uint256 timeNow) internal {
        ...
        emit ludenaRewardEvent(msg.sender, count, userPrize.amount, userPrize.timeUnlock);
    }

    /**
     * gather prize when token prize unlock
     */
    function gatherReward() external nonReentrant{
        ...
        IERC20(tokenPrizeAddress).transfer(msg.sender, prizeClaim);
     ...
     */
    function emergencyRewardWithdraw() external onlyRole(ADMIN_ROLE) {
        uint256 balance =  IERC20(tokenPrizeAddress).balanceOf(address(this));
        IERC20(tokenPrizeAddress).transfer(msg.sender, balance);
    }
