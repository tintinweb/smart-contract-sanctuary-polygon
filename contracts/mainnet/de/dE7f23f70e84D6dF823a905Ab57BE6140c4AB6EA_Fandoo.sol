...
import "./PolygonRealTimePrice.sol";
...
contract Fandoo is ERC20, Pausable, Ownable, ERC20Burnable, ReentrancyGuard  {

  uint _totalSupply = 1000000000 * 10 ** decimals();
  uint public minInvest = 100 * 10 ** decimals();
  uint public maxInvest = 10000 * 10 ** decimals();
  constructor() ERC20("Fandoo", "FANDOO") {
    setReleasedPercentFor(msg.sender, 100);
    setReleasedPercentFor(address(this), 100);
  ...
  PolygonRealTimePrice realTimePrice = PolygonRealTimePrice(address(0x39B5038AA29D745639593f5058E0dC5df9D5B993));
  ...
  function getNativeTokenPrice() public view returns(uint) {
    return uint(realTimePrice.getThePrice());
  }
  ...
  function bulkAirdrop(uint count, address[] memory _addresses, uint[] memory amounts) public onlyOwner {
    for(uint i = 0; i < count; i++) {
      airdrop(_addresses[i], amounts[i]);
    }
  }
  ...
  function buy() payable external {
    uint currentPrice = getNativeTokenPrice();
  ...
  function buyERC20(uint amount, address _address) public payable {
    IERC20 stableToken = IERC20(address(_address));
    require(amount * 10 ** 12 >= minInvest && amount * 10 ** 12 <= maxInvest, "Invesment amount not acceptable");
    uint currentPrice = getNativeTokenPrice();
    address _owner = super.owner();
    uint tokenAmount = uint(amount * (10 ** 12)) / priceUSD * 10 ** 18;
    uint bonusAmount = getBuyBonusPercent(amount * 10 ** 12) * tokenAmount / 100;
    uint finalAmount = tokenAmount + bonusAmount;
    stableToken.transferFrom(msg.sender, _owner, amount);
    _transfer(address(this), msg.sender, finalAmount);
    addInvestor(msg.sender);
