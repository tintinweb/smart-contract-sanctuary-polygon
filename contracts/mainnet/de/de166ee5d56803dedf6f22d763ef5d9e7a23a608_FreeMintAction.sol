...
contract FreeMintAction is OwnableUpgradeable, Verify {
    ...
    function setMintInterval(uint64 _start, uint64 _end) external onlyOwner {
        require(_end > _start, "end must be greater than start");
        require(_end < cfg.start || _start > cfg.end, "invalid interval");
        cfg.start = _start;
        cfg.end = _end;
        cfg.count = 0;
    ...
    function claim(
        ...
        nftContract.adminMintTo(sender, cfg.start + cfg.count);
