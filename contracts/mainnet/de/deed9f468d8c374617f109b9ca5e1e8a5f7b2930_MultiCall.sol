...
interface IRegistrar {
    ...
    function consumedAmount() external view returns (uint256 amount);
}
...
contract MultiCall is KeyEnumBase {
    ...
    {
        consumed = IRegistrar(registrarList[index]).consumedAmount();
        consumable = IRegistrar(registrarList[index]).consumable();
        ...
        )
    {
        ...
        if (length > 0) {
            ...
            owners = new address[](length); // TODO
        ...
        )
    {
        ...
        if (start < length) {
            ...
            owners = new address[](capacity); // TODO
