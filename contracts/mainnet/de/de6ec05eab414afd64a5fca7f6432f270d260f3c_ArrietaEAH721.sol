...
import {QurableERC721V4} from "../v4/QurableERC721V4.sol";

contract ArrietaEAH721 is QurableERC721V4 {
    mapping(address => bool) public allowedToBurnOperators;

    function addAllowedToBurnOperator(address operator) external onlyOwner {
        require(operator != address(0), "InvalidBurnOperator");

        allowedToBurnOperators[operator] = true;
    }

    function removeAllowedToBurnOperator(address operator) external onlyOwner {
        allowedToBurnOperators[operator] = false;
    }

    modifier onlyOperatorOrOwnerOrAllowedToBurn() {
        require(
            owner() == _msgSender() ||
                allowedToBurnOperators[_msgSender()] ||
                _qurableRegistry.transferOperators(_msgSender()),
            "NotOperatorOrOwnerOrAllowedToBurn"
        );
        _;
    }

    function burn(uint256 tokenId) external onlyOperatorOrOwnerOrAllowedToBurn {
        super._burn(tokenId);
    }
}
...
contract QurableERC721V4 is
    BaseERC721,
    BaseERC721Enumerable,
    BaseERC721URIStorage,
    ...
    IHasVersion
{
    ...
    mapping(uint256 => uint256) private _baseTokenURIsIndex;
    mapping(uint256 => uint256) private _hasSingleTokenUri;
    mapping(uint256 => uint256) private isFreezedTokenURI;
    string[] private _baseTokenURIs;
    ...
    function freezeAllTokenURIs() external onlyOwner {
        require(collectionId > 0, "InvalidCollectionId");
        uint256 maxTokenId = collectionId + _tokenIdCounter;

        for (uint256 index = 0; index < maxTokenId; index++) {
            uint256 tokenId = collectionId + index;
            isFreezedTokenURI[tokenId] = 1;
    ...
    function setTokenURIs(
        ...
        require(
            tokenIds_.length == tokenURIs_.length,
            "IdsAndURIsLengthAreDifferent"
        );

        for (uint256 index = 0; index < tokenIds_.length; index++) {
            uint256 tokenId = tokenIds_[index];
            string memory thisTokenURI = tokenURIs_[index];

            require(_exists(tokenId), "InvalidTokenId");
            require(bytes(thisTokenURI).length > 0, "InvalidTokenURI");
            require(isFreezedTokenURI[tokenId] == 0, "TokenURIFreezed");

            _setTokenURI(tokenId, thisTokenURI);
            _hasSingleTokenUri[tokenId] = 1;

            if (freeze_) {
                isFreezedTokenURI[tokenId] = 1;
                emit PermanentURI(thisTokenURI, tokenId);
            }
        }
    }
    ...
    function mint(
        ...
        return _mintInternal(from, to, newTokenURI, freezeTokenURI);
    }
    ...
    function mintMultiple(
        ...
        require(
            amount > 0 && amount <= MarketLibV2.MAX_ALLOWED_MINTING_INTX,
            "0 < Amount <= 1000"
        );
        require(bytes(baseTokenURI).length > 0, "InvalidTokenURI");

        uint256 startTokenIdCounter = _tokenIdCounter;
        _tokenIdCounter += amount;

        _baseTokenURIs.push(baseTokenURI);
        uint256 baseTokenURIIndex = _baseTokenURIs.length - 1;
        for (uint256 index = 0; index < amount; index++) {
            uint256 newTokenId = collectionId + startTokenIdCounter + index;

            _baseTokenURIsIndex[newTokenId] = baseTokenURIIndex;
    ...
    function tokenURI(uint256 tokenId)
        public
        view
        override(BaseERC721, BaseERC721URIStorage)
        returns (string memory)
    {
        if (_hasSingleTokenUri[tokenId] > 0) {
        ...
        return
            string(
                abi.encodePacked(
                    _baseTokenURIs[_baseTokenURIsIndex[tokenId]],
                    "/",
    ...
    function _mintInternal(
        ...
        _mintSingle(from, to, newTokenId);
        _hasSingleTokenUri[newTokenId] = 1;
        _setTokenURI(newTokenId, newTokenURI);

        if (freezeTokenURI) {
            isFreezedTokenURI[newTokenId] = 1;
            emit PermanentURI(newTokenURI, newTokenId);
        }
    ...
    IERC721MetadataUpgradeable
{
     ...
     */
    function __BaseERC721_init(string memory name_, string memory symbol_)
        internal
        onlyInitializing
    {
    ...
    function _transfer(
        ...
        require(
            BaseERC721.ownerOf(tokenId) == from,
 ...
 * account.
 */
abstract contract BaseERC721Enumerable is
    Initializable,
    BaseERC721,
    IERC721EnumerableUpgradeable
{
    function __BaseERC721Enumerable_init() internal onlyInitializing {}

    function __BaseERC721Enumerable_init_unchained()
        internal
        onlyInitializing
    {}
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(IERC165Upgradeable, BaseERC721)
        returns (bool)
    {
        return
    ...
    {
        require(
            index < BaseERC721.balanceOf(owner),
    ...
    {
        require(
            index < BaseERC721Enumerable.totalSupply(),
    ...
    function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
        uint256 length = BaseERC721.balanceOf(to);
        ...
        private
    {
        ...
        uint256 lastTokenIndex = BaseERC721.balanceOf(from) - 1;
 ...
 */
abstract contract BaseERC721URIStorage is Initializable, BaseERC721 {
    function __BaseERC721URIStorage_init() internal onlyInitializing {}

    function __BaseERC721URIStorage_init_unchained()
        internal
        onlyInitializing
    {}
...
library DecimalLib {
    ...
    uint256 internal constant BASE_POW = 18;
    uint256 internal constant BASE = 10**BASE_POW;
    ...
    function mul(uint256 target, D256 memory d)
        ...
        returns (uint256)
    {
        return MathLib.getPartial(target, d.value, BASE);
    }
    ...
    function div(uint256 target, D256 memory d)
        ...
        returns (uint256)
    {
        return MathLib.getPartial(target, BASE, d.value);
    }
}
