...
contract SpaceShipVault is Ownable, IERC721Receiver {
    ...
    constructor(
        ...
        setVaultManager(msg.sender);
        initializeScarcities();
        setRentalParameters(1, 0.1 ether, 500, 100, 604_800);
        setRentalParameters(2, 0.1 ether, 600, 100, 604_800);
        setRentalParameters(4, 0.1 ether, 700, 100, 604_800);
        setRentalParameters(8, 0.1 ether, 800, 100, 604_800);
    }
    ...
    function deposit(uint256[] memory tokenIDs) public payable {
        ...
        if (tokenIDs.length > 0) {
            ...
            /*
            (bool sent, bytes memory data) = vaultManager.call{value: msg.value}("");
    ...
    function initiateRental(uint256[] memory tokenIDs) internal {
        for (uint256 i = 0; i < tokenIDs.length; i++) {
            ...
            IRentalProtocol.RentalOffer memory rentalParam = IRentalProtocol.RentalOffer({
                ...
                deadline: block.timestamp + scarcityParam.listingDuration
            });
    ...
    function terminateRental(uint256[] memory tokenIds) public {
        require(msg.sender == vaultManager, "Only Vault Manager");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            rentalProtocol.endRental(address(spaceship), tokenIds[i]);
        }
    }

    function restartRental(uint256[] memory tokenIds) public {
        terminateRental(tokenIds);
        initiateRental(tokenIds);
    }
    ...
    function tempWithdrawERC20(address ERC20Address, uint256 amount) public onlyOwner {
        IERC20(ERC20Address).transferFrom(address(this), msg.sender, amount);
    }
