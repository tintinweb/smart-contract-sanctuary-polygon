...
contract CegaState is AccessControl {
    ...
    // Initialize contract with immutable variables
    constructor(address _operatorAdmin, address _traderAdmin) {
...
import { Deposit, FCNVaultMetadata, OptionBarrierType, OptionBarrier, VaultStatus } from "./Structs.sol";
...
contract FCNProduct {
    ...
    event VaultCreated(address vaultAddress, uint index);
    event DepositQueued(uint256 amount, address receiver);
    event DepositQueueProcessed(address vaultAddress, uint256 totalUnderlyingAmount);
    event DepositsSentToTrade(address receiverAddress, uint256 totalUnderlyingAmount);

    CegaState public cegaState;

    address public asset;
    string public name;
    uint public managementFeeBps; // basis points
    uint public yieldFeeBps; // basis points
    bool public isAcceptingDeposits;
    ...
    constructor(
        ...
        address _asset,
        string memory _name,
        uint _managementFeeBps,
        uint _yieldFeeBps,
        uint256 _maxDepositAmountLimit
    ) {
        // console.log("Deploying an FCNProduct with asset: '%s', name: '%s'", _asset, _name);
        // console.log(
        //     "FCNProduct Fee Structure: managementFeeBps: '%d', yieldFeeBps: '%d'",
        //     _managementFeeBps,
        //     _yieldFeeBps
        // );
    ...
    modifier onlyDefaultAdmin() {
        require(cegaState.isDefaultAdmin(msg.sender), "Invalid sender. Expected default admin");
        _;
    }

    modifier onlyTraderAdmin() {
        require(cegaState.isTraderAdmin(msg.sender), "Invalid sender. Expected trader admin");
        _;
    }

    modifier onlyOperatorAdmin() {
        require(cegaState.isOperatorAdmin(msg.sender), "Invalid sender. Expected operator admin");
        _;
    }

    function getDepositQueueLength() public view returns (uint) {
    ...
    function setManagementFeeBps(uint _managementFeeBps) public onlyOperatorAdmin {
        managementFeeBps = _managementFeeBps;
    }

    function setYieldFeeBps(uint _yieldFeeBps) public onlyOperatorAdmin {
        yieldFeeBps = _yieldFeeBps;
    }

    function setIsAcceptingDeposits(bool _isAcceptingDeposits) public onlyOperatorAdmin {
        isAcceptingDeposits = _isAcceptingDeposits;
    }
    ...
    function createVault(
        ...
        vaultMetadata.isKnockedOut = false;
        vaultMetadata.underlyingAmount = 0;
        vaultMetadata.totalCouponPayoff = 0;
        vaultMetadata.vaultFinalPayoff = 0;
        vaultMetadata.optionBarriersCount = 0;

        // console.log("Created new vault: Address: %s, VaultIndex: %d", newVaultAddress, vaultAddresses.length - 1);
        emit VaultCreated(newVaultAddress, vaultAddresses.length - 1);
        return newVaultAddress;
    }

    function setVaultMetadata(address vaultAddress, FCNVaultMetadata calldata metadata) public onlyDefaultAdmin {
    ...
    function getOptionBarrier(address vaultAddress, uint index) public view returns (OptionBarrier memory) {
        FCNVaultMetadata storage metadata = vaults[vaultAddress];
        if (index >= metadata.optionBarriersCount) {
            revert("Index out of bounds");
        }
        return metadata.optionBarriers[index];
    }

    function setVaultStatus(address vaultAddress, VaultStatus _vaultStatus) public onlyOperatorAdmin {
        FCNVaultMetadata storage metadata = vaults[vaultAddress];
        metadata.vaultStatus = _vaultStatus;
    }

    function getVaultUnderlyingAmount(address vaultAddress) public view returns (uint256) {
        FCNVaultMetadata storage vaultMetadata = vaults[vaultAddress];
        return vaultMetadata.underlyingAmount;
    }

    function addToDepositQueue(uint256 amount, address receiver) public {
        require(isAcceptingDeposits, "Product must be active and able to receive deposits");
        totalDepositAmount = totalDepositAmount.add(amount);
        require(totalDepositAmount <= maxDepositAmountLimit, "Deposit has exceeded max deposit limit");

        IERC20(asset).safeTransferFrom(msg.sender, address(this), amount);
        depositQueue.push(Deposit({ amount: amount, receiver: receiver }));
        emit DepositQueued(amount, receiver);
    }

    function processDepositQueue(address vaultAddress) public onlyTraderAdmin {
        FCNVaultMetadata storage vaultMetadata = vaults[vaultAddress];
        require(vaultMetadata.vaultStatus == VaultStatus.DepositsOpen, "Vault is not accepting deposits");

        FCNVault vault = FCNVault(vaultAddress);
        for (uint i = 0; i < depositQueue.length; i++) {
            Deposit storage deposit = depositQueue[i];

            vaultMetadata.underlyingAmount += deposit.amount; // TODO: safe math?
            vault.deposit(deposit.amount, deposit.receiver);
        }
        delete depositQueue;

        emit DepositQueueProcessed(vaultAddress, vaultMetadata.underlyingAmount);
        vaultMetadata.vaultStatus = VaultStatus.NotTraded;
    }

    function sendAssetsToTrade(address vaultAddress, address receiver) public onlyTraderAdmin {
        FCNVaultMetadata storage vaultMetadata = vaults[vaultAddress];
        require(
            vaultMetadata.vaultStatus == VaultStatus.NotTraded,
            "Deposit queue must be processed before sending assets to trade"
        );
        uint depositBalance = vaultMetadata.underlyingAmount;
        IERC20(asset).safeTransfer(receiver, depositBalance);
        emit DepositsSentToTrade(receiver, depositBalance);
...
contract FCNVault is IERC4626, ERC20, Ownable {
    ...
    function totalAssets() public view returns (uint256) {
        FCNProduct fcnProduct = FCNProduct(owner());
        return fcnProduct.getVaultUnderlyingAmount(address(this));
    }
    ...
    // TODO decorate with onlyOwner once I figure out how to set fcnProduct owner as the vault owner
...
struct OptionBarrier {
    ...
    uint barrierBps;
    uint barrierAbsoluteValue;
    uint strikeBps;
    uint strikeAbsoluteValue;
    uint observationTime;
    uint observationTimeBuffer;
}
