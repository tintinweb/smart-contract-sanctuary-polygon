...
contract ERC721r is Context, ERC165, IERC721, IERC721Metadata {
    ...
    uint256 public _numAvailableTokensSubCollection;
    uint256 private _subCollectionSupply;
    uint256 public _currentSubCollection = 0;
    
    uint256 private _subCollectionOffset;
    bool public _randomMintPause = false;
     ...
     */
    constructor(string memory name_, string memory symbol_, uint maxSupply_, uint subcollectionSupply_) {
        _name = name_;
        _symbol = symbol_;
        _maxSupply = maxSupply_;
        _numAvailableTokens = maxSupply_;
        _subCollectionSupply = subcollectionSupply_;
        _numAvailableTokensSubCollection = _subCollectionSupply;
        _subCollectionOffset = _currentSubCollection*_subCollectionSupply + 1;
    }
    ...
    function _mintRandom(address to, uint _numToMint) internal virtual {
        require(_currentSubCollection > 0, "Can't random mint at subcollection 0.");
        require(_msgSender() == tx.origin, "Contracts cannot mint");
        require(to != address(0), "ERC721: mint to the zero address");
        require(_numToMint > 0, "ERC721r: need to mint at least one token");
        require(_randomMintPause == false, "Random minting is paused");
        
        // TODO: Probably don't need this as it will underflow and revert automatically in this case
        //require(_numAvailableTokensSubCollection >= _numToMint, "ERC721r: minting more tokens than available");
        
        //uint updatedNumAvailableTokens = _numAvailableTokensSubCollection;
        for (uint256 i; i < _numToMint; ++i) { // Do this ++ unchecked?
            //uint256 tokenId = getRandomAvailableTokenId(to, updatedNumAvailableTokens);
            uint256 tokenId = getRandomAvailableTokenId(to, _numAvailableTokensSubCollection);
            tokenId += _subCollectionOffset;
            
            _mintIdWithoutBalanceUpdate(to, tokenId);
            
            //--updatedNumAvailableTokens;
            _numAvailableTokensSubCollection--;
        }
        
        //_numAvailableTokensSubCollection = updatedNumAvailableTokens;
        _balances[to] += 1;// _numToMint;
        if(_numAvailableTokensSubCollection == 0) {
            _randomMintPause = true;
        }
    }

    function _unpauseRandomMint() internal virtual {
        require(_randomMintPause == true, "Mint is already unpaused");
        _randomMintPause = false;
        _numAvailableTokensSubCollection = _subCollectionSupply;
        _currentSubCollection++;
        for(uint256 i = 0; i <= _numAvailableTokensSubCollection; i++) {
            delete _availableTokens[i];
        }
        _subCollectionOffset = _currentSubCollection*_subCollectionSupply + 1;
    }
    ...
    function _mintAtIndex(address to, uint index) internal virtual {
        require(_randomMintPause == false, "Minting is paused");
        require(_currentSubCollection == 0, "Can only mint at specific index at subcollection 0.");
        require(_msgSender() == tx.origin, "Contracts cannot mint");
        require(to != address(0), "ERC721: mint to the zero address");
...
// File: WUBI/ERC721REnumerable.sol
