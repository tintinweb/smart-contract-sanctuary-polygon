...
contract ExchangeCore is ReentrancyGuarded, EIP712, Ownable {
    ...
    function atomicMatch(
        ...
        /* Must be matchable. */
        require(ordersCanMatch(buy, sell), "000");
        ...
        require(proxy.proxy(sell.target, sell.howToCall, sell.data), "55");

        /* Static calls are intentionally done after the effectful call so they can check resulting state. */

        /* Handle buy-side static call if specified. */
        if (buy.staticTarget != address(0)) {
            require(StaticCall.staticCall(buy.staticTarget, sell.data, buy.staticExtradata), "66");
        }

        /* Handle sell-side static call if specified. */
        if (sell.staticTarget != address(0)) {
            require(StaticCall.staticCall(sell.staticTarget, sell.data, sell.staticExtradata), "77");
        }
