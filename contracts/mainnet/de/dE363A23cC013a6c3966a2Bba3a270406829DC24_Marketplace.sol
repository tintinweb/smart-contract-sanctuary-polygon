...
contract Marketplace {
    ...
    function listForSale(address nftAddress, uint256 itemId, uint256 price) external {
        require(!isItemApproved[nftAddress][itemId], "Item already listed");
        require(IERC721(nftAddress).ownerOf(itemId) == msg.sender, "You don't own this item");

        // Transfer the NFT to the marketplace contract
        IERC721(nftAddress).transferFrom(msg.sender, address(this), itemId);
        isItemApproved[nftAddress][itemId] = true;

        items[itemId] = Item(msg.sender, price, true, false, 0, 0, 0, address(0));
        emit ItemListed(itemId, msg.sender, price, false);
    }

    function buy(address nftAddress, uint256 itemId) external payable {
        Item storage item = items[itemId];
        require(item.isForSale, "Item is not for sale");
        require(item.seller != address(0), "Item does not exist");
        require(item.price > 0, "Price not set");
        ...
        IERC721(nftAddress).transferFrom(address(this), msg.sender, itemId);
    ...
    function startAuction(address nftAddress, uint256 itemId, uint256 startTime, uint256 endTime) external {
        require(!isItemApproved[nftAddress][itemId], "Item already listed");
        require(IERC721(nftAddress).ownerOf(itemId) == msg.sender, "You don't own this item");
    ...
    function placeBid(uint256 itemId) external payable {
        ...
        require(item.seller != address(0), "Item does not exist");
        require(item.startTime > 0 && item.endTime > 0, "Auction period not set");
    ...
    function endAuction(uint256 itemId) external onlyMarketplaceOwner {
        ...
        require(item.isAuction, "Auction is not active");
        require(item.seller != address(0), "Item does not exist");
        require(item.endTime > 0, "Auction period not set");
        ...
        IERC721(msg.sender).transferFrom(address(this), winner, itemId);
    ...
    function cancelAuction(address nftAddress, uint256 itemId) external {
        Item storage item = items[itemId];
        require(item.isAuction, "Auction is not active");
        ...
        IERC721(nftAddress).transferFrom(address(this), msg.sender, itemId);
        isItemApproved[nftAddress][itemId] = false;
    }

    function cancelSale(address nftAddress, uint256 itemId) external {
        Item storage item = items[itemId];
        require(item.seller == msg.sender, "You are not the seller");
