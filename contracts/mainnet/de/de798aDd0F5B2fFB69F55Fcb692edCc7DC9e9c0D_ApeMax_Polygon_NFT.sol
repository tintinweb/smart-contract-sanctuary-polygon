...
contract ApeMax_Polygon_NFT is ERC721, Ownable {
    ...
    event _mint_auth_apemax(address indexed recipient_address, uint128 amount_minted);

    // ------- Global Vars -------
    uint32 public token_count;
    string public nft_image;
    
    address internal constant pricing_authority = 0x83258645a1E202ED1EAA70cAA015DCfaD8557b3b;

    mapping(uint32 => uint128) public token_id_to_balance;
    mapping(address => uint128) public address_to_balance;

    // ------- Init -------
    constructor(string memory image_url) ERC721("ApeMax", "APEMAX") {
        nft_image = image_url;
    }
        ...
        public payable
    {
        ...
        )), v, r, s) == pricing_authority, "Invalid signature");

        uint128 one_percent_less = amount_payable - amount_payable / 100;
        require(msg.value >= one_percent_less, "Incorrect Polygon / Matic amount sent");
    ...
    function transfer(address to, uint32 token_id) public {
        ...
        address_to_balance[to] += apemax_balance;
        emit _mint_auth_apemax(to, apemax_balance);
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory json = string(abi.encodePacked(
            ...
            " $APEMAX",
            '", "description":"This NFT represents ApeMax tokens and has been designed to facilitate purchases using a credit card. To convert this NFT into actual ApeMax tokens, please transfer or export it to a custodial Ethereum wallet. Upon completing this process, you will be able to mint your ApeMax tokens from within the ApeMax dashboard.", "image":"',
            nft_image,
            '"}'
        ));
    ...
    function get_address_balance(
        ...
        returns (uint128)
    {
        return address_to_balance[recipient];
    }
