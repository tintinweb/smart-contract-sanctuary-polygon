...
contract Connoisseurs is ERC721 , DefaultOperatorFilterer {
    uint256 public totalSupply;
    uint256 public cost = 0.05 ether;
    uint256 public maxMints = 60;
    ...
    constructor()
        ERC721("Connoisseurs", "CNSS")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 60) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeic6xjxnk5ns6jui46ddwt7vnaozar7mfida73src3m2ryccd6nixq/", Strings.toString(tokenId), ".json")
            );
    }
