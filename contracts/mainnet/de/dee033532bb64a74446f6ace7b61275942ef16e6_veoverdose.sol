...
contract veoverdose is ERC721  {
    uint256 public totalSupply;
    uint256 public cost = 0 ether;
    uint256 public maxMints = 323;
    ...
    constructor()
        ERC721("ve-overdose", "ovose")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 323) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeif6fb4jajl665xj4v7muzhqhgzmubavhe2ccnl3gkzra4vtpxiyem/", Strings.toString(tokenId), ".json")
            );
    }
