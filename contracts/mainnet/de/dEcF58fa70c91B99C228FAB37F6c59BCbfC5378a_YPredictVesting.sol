...
contract YPredictVesting is VestingHub {
    uint256 internal constant _DURATION = 184 days;
    uint256 internal constant _INITIAL_RELEASE_PERCENTAGE = 50;
    uint256 internal constant _LOCK_PERIOD = 6 weeks;

    mapping(address => bool) public s_manager;

    modifier onlyManager() {
        require(s_manager[msg.sender] == true, "Only Manager Function");
        _;
    }
...
contract VestingHub is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        lockEndDate = start_ + lockPeriod_;
    ...
    function claimTokens(address[] memory beneficiaries) public nonReentrant {
        for (uint256 i = 0; i < beneficiaries.length; i++) {
            ...
            require(
                s_blackList[beneficiaries[i]] == false,
                "User is balcklisted for claim"
            );
            require(claimableTokens > 0, "Vesting: no claimable tokens");
            require(block.timestamp >= lockEndDate, "Lock Period is going on");
    ...
    function blackList(address user, bool isBlackList) external onlyOwner {
        s_blackList[user] = isBlackList;
    }
}
