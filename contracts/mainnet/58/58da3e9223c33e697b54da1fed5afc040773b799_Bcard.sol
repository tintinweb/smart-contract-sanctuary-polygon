...
abstract contract MAKECard is Ownable{
    ...
    function AddressToTokenID (address _address, uint256 _totalSupply) public view returns (uint){}
...
abstract contract CheckQua is Ownable{    
    function checkConNewMint(address _msgSender, uint256 _totalSupply) public view returns(string memory){}
    function checkConAddCard(uint _ID, address _msgSender, uint _NumOfCards, uint256 _totalSupply) public view returns(string memory) {  }
...
abstract contract BcardRecSto is Ownable  {  
...
contract Bcard is ERC1155, Ownable {
  ...
  address CheckQuaAddr=0xF442B0AC0a248ca680A701Be8681Df4c8f5759A5;
  address MAKECardAddr=0x01F7d694F6BEC42A9dD62eDEbD5c02E9E075FA0B;
  address RecStorageAddr=0xC7D5d5ab92ddAc53e88b067DF7F099ae0e8065C1;
  address AdAddr = 0xFd7244253e4901f98846a3Dc7034425eF7A0AaDC;
  ...
  function mintNew(string memory _ethName,string memory _Nick, address cardholder) public payable {  
    ...
    if (msg.sender != owner() && msg.sender != AdAddr) {
      require(msg.value >= cost*NstartCard,"sent less than price");
        CheckQua checkCon = CheckQua(CheckQuaAddr);
      require(cStr(checkCon.checkConNewMint(msg.sender,totalToken),"Match"),
        checkCon.checkConNewMint(msg.sender,totalToken));
    }
  ...
  function mintAdd(address _to, uint256 _ID, uint256 _NumOfCards) public payable {    
    if (msg.sender != owner() && msg.sender != AdAddr) {
      require(msg.value >= cost*_NumOfCards,"sent less than price");
        CheckQua checkCon = CheckQua(CheckQuaAddr);
      require(cStr(checkCon.checkConAddCard(_ID,msg.sender, _NumOfCards, totalToken),"Match"),
        checkCon.checkConAddCard(_ID,msg.sender, _NumOfCards, totalToken));
    }
  ...
  function buildMetaData(uint256 _ID) public payable {
    if (_ID !=0){
      ...
      tokenURI[_ID]= makeCardContract.master(_ID,cCard.ethName,cCard.cNoENS,cCard.ethNick);
  ...
  function safeTransferFrom(address from,address to,uint256 id,uint256 amount,bytes memory data
        ...
        _safeTransferFrom(from, to, id, amount, data);
        buildMetaData(AddressToTokenID(to));
    }

  function AddressToTokenID(address _address) public view returns (uint){
    MAKECard makeCardContract = MAKECard(MAKECardAddr);
    return makeCardContract.AddressToTokenID(_address,totalToken);
  }
