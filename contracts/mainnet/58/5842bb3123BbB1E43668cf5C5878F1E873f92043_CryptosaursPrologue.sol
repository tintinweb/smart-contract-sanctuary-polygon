...
contract CryptosaursPrologue is ERC1155, ReentrancyGuard, Ownable {
    using Strings for uint256;

    string public name = "CryptosaursPrologue";
    ...
    modifier onlyStarted() {
        require(block.timestamp >= blockStart && block.timestamp <= blockEnd, "Modifier: Not started yet");
    ...
    constructor(
        ...
        emit SetStartTimestamp(startDate);
    ...
    // mint
    function mint(uint256 tokenId, address mintAddress) external onlyStarted onlyAdminOrOwner(msg.sender) {
        require(mintedAmount[tokenId] < maxSupply[tokenId], "Exception: All was minted.");
        require(mintedPerAddress[mintAddress] < maxMintAmount, "Exception: Reached the limit for each user. You can't mint no more");
        _mint(mintAddress, tokenId, 1, "");
        mintedPerAddress[mintAddress] = mintedPerAddress[mintAddress] + 1;
        mintedAmount[tokenId] = mintedAmount[tokenId] + 1;
    }
    ...
    function uri(uint256 typeId) public view override returns (string memory) {
        ...
        return string(abi.encodePacked(baseURI, typeId.toString(), metaDataExt));
    }
}