    ...
    IFungibleLda
{
    ...
    /// @dev Mapping (tokenId) => RightType for this token.
    mapping(uint256 => RightsInfo) internal _RIGHTS_INFO_;
    ...
    function registerToken(
        RightsInfo calldata rightsInfo
    )
        external
        onlyOwner
        whenNotPaused
    {
        ...
        _RIGHTS_INFO_[tokenId] = rightsInfo;

        emit RegisteredToken(tokenId);
        emit UpdatedRightsInfo(rightsInfo);
    }
    ...
    function getRightsInfo(
        ...
        external
        view
        returns (RightsInfo memory)
    {
        return _RIGHTS_INFO_[tokenId];
    }
