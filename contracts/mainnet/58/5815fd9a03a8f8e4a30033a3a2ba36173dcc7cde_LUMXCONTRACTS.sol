...
contract LUMXCONTRACTS is Ownable, ReentrancyGuard, ERC721AQueryable, OperatorFilterer {
    using Strings for uint256;
    using ECDSA for bytes32;

    // Addresses
    address immutable public sender = 0xA65aae78EdEF916d4102BA7b5672068C0D35fbff;
    ...
    function mintInternal(address to, uint256 quantity, uint256[] calldata id) private {
      ...
      for (uint256 index = 0; index < quantity; index++) {
        require(currentSupply[id[index]] + 1 <= maxSupplyEach[id[index]], "This amount of tokens would surpass the max supply for this token.");
        tokenIdsUris[_nextTokenId() + index] = id[index];
        currentSupply[id[index]]++;
      }

      _mint(to, quantity);  
    }

    function mint(uint256 quantity, uint256[] calldata id, bytes calldata signature) external payable {
        require(verifySignature(signature, msg.sender), "Account not eligible for claim.");

        mintInternal(msg.sender, quantity, id);
    }
    ...
    function setMaxSupply(uint256[] calldata _maxSupplyEach, uint256 _maxSupply) external onlySender {  
        maxSupplyEach = _maxSupplyEach;
        maxSupply = _maxSupply;
    }
    ...
    function withdrawMoney() external onlyOwner {
        uint256 amount1 = address(this).balance / 100 * primaryRoyalties;
