...
contract FactoryManager is Owned, TokensRecoverable {
    ...
    function setChainTokenFeeCollectors(uint deploymentId, address[] memory collectors, uint16[]  memory collectRates) public onlySignedAdmin(){
        FeeSplitter(deployData[deploymentId].feeSplitter).setChainTokenFeeCollectors(RootedToken(tokenDeployments[deploymentId].rootToken), collectors, collectRates);
    }
    ...
    function setRootedTokenFeeCollectors(uint deploymentId, address[] memory collectors, uint16[]  memory collectRates) public onlySignedAdmin(){
        FeeSplitter(deployData[deploymentId].feeSplitter).setRootedTokenFeeCollectors(RootedToken(tokenDeployments[deploymentId].rootToken), collectors, collectRates);
    }
    ...
    function liquidityRecovery(uint deploymentId) private onlySignedAdmin() {
        address _elitePool = _factory.getPair(tokenDeployments[deploymentId].eliteToken, tokenDeployments[deploymentId].rootToken);
        uint256 amountToRemove = IERC20(_elitePool).balanceOf(deployData[deploymentId].liquidityController);
