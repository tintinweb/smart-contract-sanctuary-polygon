...
interface IWorldContract {
  ...
  function tokenSupply(uint tokenId) external returns(uint supply);
...
contract BuyMiningClaims {
  ...
  function _beforeBuy(
    ...
    if(supply < 500) {
      ...
      require(buyAmount == 1, "can buy a max of 1 in the first 500");
    } else if(supply < 1000) {
      // Limit to 3 in the first 10 minutes
      require(buyAmount <= 3, "can buy a max of 3 in the first 1000");
    }
