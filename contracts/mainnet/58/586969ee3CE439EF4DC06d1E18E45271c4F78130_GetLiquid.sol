...
contract GetLiquid {
    ...
    function updateFactory(address _factory) public onlyOwner returns(address) {
    ...
    function addLiquidity(
        ...
        CpieV2Router(ROUTER).addLiquidity(
            _tokenA, 
            _tokenB, 
            _amountA, 
            _amountB, 
    ...
    function removeLiquidity(address _tokenA, address _tokenB) external {
        ...
        CpieV2Router(ROUTER).removeLiquidity(
            _tokenA,
            _tokenB,
            liquidity,
            1,
    ...
    function removeLiquidityETH(address _token) external payable {
        ...
        uint liquidity = IERC20(_token).balanceOf(address(this));
        IERC20(_token).approve(ROUTER, liquidity);

        (uint amount, uint eth) =
        CpieV2Router(ROUTER).removeLiquidityETH(
            _token,
            liquidity,
            1,
            1,
        ...
        // Transfer token and ether from contract to user
        payable(msg.sender).transfer(eth);
        IERC20(_token).transferFrom(address(this), msg.sender, amount);
