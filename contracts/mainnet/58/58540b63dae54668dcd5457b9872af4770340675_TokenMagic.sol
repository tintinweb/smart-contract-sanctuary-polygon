...
contract WrappedNonFungable is ERC721, Ownable {
    ...
    function burn(address _address, uint256 _tokenId) public onlyOwner {
    ...
    function setBaseURI(string memory _baseUri) public onlyOwner {
        require(bytes(baseURI).length == 0, "SET");
        baseURI = _baseUri;
    }
    ...
    function tokenType() public pure returns (uint) {
        return 1;
    }

}

contract WrappedFungable is ERC20, Ownable {
...
contract TokenMagic {
    ...
    function create(uint contractType , string memory name, string memory symbol) public returns(address) {

            if (contractType == 0) {
                    
                    WrappedFungable t = new WrappedFungable(name, symbol);
            ...
            if (contractType == 1) {
                    ...
                    WrappedNonFungable t = new WrappedNonFungable(name, symbol);
                    t.transferOwnership(msg.sender);
