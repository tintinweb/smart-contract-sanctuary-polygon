...
contract TokenDonate is ReentrancyGuard {
    ...
    modifier legitAddress(address payable _address){
        require(usdt.transfer(_address, 0),"address invalid");
    ...
    mapping(address => uint256) public recievers ; // 0-not registered 1-active 2-suspended 3-inactive
    ...
    constructor(IERC20 token)ReentrancyGuard() {
        ...
        function TotalDonation() external view returns (uint256) {
            return _donation + CurrentBalance();
        }
        ...
        function Release() nonReentrant() external {
            ...
            require (usdt.balanceOf(address(this)) > thresholdValue, "Wait for the minimum amount");
            ...
            if( _availablereciever != 0){
                netAmount = usdt.balanceOf(address(this))/ _availablereciever;
            }
            require (netAmount > 0 , "Value to small");


            address payable empty;
            _donation = usdt.balanceOf(address(this));
             for (uint256 i = 0; i < _reciever; i++){  
                if  (recievers[id[i]] == 1){
                    usdt.transfer(id[i], netAmount);
