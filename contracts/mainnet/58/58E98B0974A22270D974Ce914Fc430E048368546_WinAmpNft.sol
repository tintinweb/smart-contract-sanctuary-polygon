...
contract RoyaltiesV2Impl is AbstractRoyalties, IRoyaltiesV2, IERC2981Upgradeable {
    ...
    */
    function royaltyInfo(uint256 id, uint256 _salePrice) override external view returns (address receiver, uint256 royaltyAmount) {
        if (royalties[id].length == 0) return (address(0), 0);
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual returns (bool) {
        return interfaceId == type(IRoyaltiesV2).interfaceId
...
abstract contract ERC1155Base is ERC1155DefaultApproval, ERC1155BurnableUpgradeable, ERC1155PausableUpgradeable, ERC1155Lazy {
    string public name;
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155Upgradeable, ERC1155Lazy) returns (bool) {
    ...
    */
    ...
    function _tokenURI(uint256 tokenId) internal view virtual returns (string memory) {
        ...
        if (bytes(__tokenURI).length > 0) {
            return checkPrefix(base, __tokenURI);
        }
...
abstract contract ERC1155DefaultApproval {
    mapping(address => bool) internal defaultApprovals;
...
abstract contract ERC1155Lazy is IERC1155LazyMint, ERC1155BaseURI, Mint1155Validator, RoyaltiesV2Impl {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155Upgradeable, RoyaltiesV2Impl) returns (bool) {
        return interfaceId == LibERC1155LazyMint._INTERFACE_ID_MINT_AND_TRANSFER
        || RoyaltiesV2Impl.supportsInterface(interfaceId)
    ...
    function mintAndTransfer(LibERC1155LazyMint.Mint1155Data memory data, address to, uint256 _amount) public override virtual {
        ...
        if (supply[data.tokenId] == 0) {
            ...
            address creator = data.creators[0].account;
            if (creator != sender) {
                bytes32 hash = LibERC1155LazyMint.hash(data);
                validate(creator, hash, data.signatures[0]);
            }
...
contract Mint1155Validator is EIP712Upgradeable {
    ...
    function validate(address account, bytes32 hash, bytes memory signature) internal view {
        require(
            SignatureCheckerUpgradeable.isValidSignatureNow(account, _hashTypedDataV4(hash), signature),
...
import "./rarible/tokens/erc-1155/ERC1155Base.sol";
import "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/access/AccessControlUpgradeable.sol";

contract WinAmpNft is ERC1155Base, OwnableUpgradeable, AccessControlUpgradeable {

    bytes32 public constant OWNER_ROLE = keccak256("OWNER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");

    function initialize(string memory _name, string memory _symbol, string memory baseURI, address transferProxy, address lazyTransferProxy) public initializer {
        __Ownable_init_unchained();
    ...
    function mintAndTransfer(LibERC1155LazyMint.Mint1155Data memory data, address to, uint256 _amount) public override {
        require(hasRole(MINTER_ROLE, data.creators[0].account), "WinAmpNft: first creator must have minter role");
        super.mintAndTransfer(data, to, _amount);
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override(AccessControlUpgradeable, ERC1155Base) returns (bool) {
        return AccessControlUpgradeable.supportsInterface(interfaceId) || ERC1155Base.supportsInterface(interfaceId);
    }
