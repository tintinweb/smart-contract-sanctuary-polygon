...
// File: contracts/HypeSocietyPaymentSplitter.sol
...
// File: contracts/HypeSocietyCollection.sol
...
contract HypeSocietyCollection is
    ERC721Enumerable,
    HSPaymentSplitter,
    Ownable,
    RoyaltiesV2Impl
{
    ...
    uint256 public constant MAX_NFT_PRESALE = 2100;
    uint256 private constant MAX_NFT = 7777; // 100 giveway

    uint256 public NFTPricePresale = 160 ether;
    ...
    address[] private contributors = [
        0x66Cba4266fc822294C68729397dffb6558d479E9,
        0xc4b9abbB0D635e65a4555A4E40aa205410600925,
        0x4218a18c93C34E76718f8b057FdA6db200B89833,
        0x1a1bb1cC7bb76fbF07A5F3646F63e694155765e6
    ];

    uint256[] private sharesContributors = [
        800, 
        1840,
        1840,
        5520
    ...
    constructor(string memory _name, string memory _symbol, string memory _URI)
        ...
        HSPaymentSplitter(contributors, sharesContributors)
    {
        baseURI = _URI;
        isPresale = true;
    }
     ...
     */
    function setURIs(string memory _URI)
        external
        onlyOwner
    {
        baseURI = _URI;
    ...
    function mintNFT(uint256 _numOfTokens) public payable {
        ...
        require(
            _numOfTokens <= BUY_LIMIT_PER_TX,
            "HypeSociety : Cannot mint above limit"
        );
        require(
            (totalSupply() + _numOfTokens) - giveawayCount <= MAX_NFT,
            "HypeSociety : Purchase would exceed max public supply of NFTs"
        );
     ...
     * It is payable. Amount is calculated as per (NFTPresalePrice * _numOfTokens)
     */
    function mintNFTDuringPresale(uint256 _numOfTokens) public payable {
        require(isActive, "HypeSociety : Contract is not active");
        require(isPresale,"HypeSociety : Contract is not presale period");

        require(
            totalSupply() <= MAX_NFT_PRESALE,
            "HypeSociety : All private tokens have been minted"
        );
        require(
            NFTPricePresale * _numOfTokens <= msg.value,
            "HypeSociety : Ether value sent is not correct"
        );
        ...
        if(totalSupply() >= MAX_NFT_PRESALE){
            isPresale = false;
        }
    }
    ...
    function mintByOwner(address _to) public onlyOwner {
        require(
            totalSupply() >= MAX_NFT_PRESALE,
            "HypeSociety : Tokens number to mint must exceed number of public tokens"
        );
        require(
            totalSupply() < MAX_NFT,
            "HypeSociety : Tokens number to mint cannot exceed number of MAX tokens"
        );
    ...
    function mintMultipleByOwner(
        ...
        require(
            ...
            "HypeSociety : Should have same length"
        );
        for (uint256 i = 0; i < _to.length; i++) {
            require(
                _tokenId[i] >= MAX_NFT_PRESALE,
                "HypeSociety : Tokens number to mint must exceed number of public tokens"
            );
            require(
                _tokenId[i] < MAX_NFT,
        ...
        returns (string memory)
    {
        require(
            _exists(_tokenId),
            "HypeSociety : URI query for nonexistent token"
        );
