 ...
 * 
 * Defines `ADMIN_ROLE`, `BURNER_ROLE`
...
// File: contracts\tokens\Shaar.sol

pragma solidity ^0.8.7;

contract Shaar is TokenRecover, Blacklistable, ERC20, ILToken {

    constructor() TokenRecover() ERC20('Shaar', 'SHR') {
    ...
    function transfer(address recipient, uint256 amount) public override(ERC20, IERC20) nonPaused returns (bool) {
        require((!isBlacklisted(recipient) && !isBlacklisted(msg.sender)),"Shaar: receiver or sender blacklisted");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override(ERC20, IERC20) nonPaused returns (bool) {
        require((!isBlacklisted(recipient) && !isBlacklisted(msg.sender) && !isBlacklisted(sender)),"Shaar: receiver or sender blacklisted");
