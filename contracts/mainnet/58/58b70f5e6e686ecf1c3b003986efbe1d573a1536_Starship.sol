...
contract Starship is IStarship, ERC721EnumerableUpgradeable, WarpBase {
    ...
    function URIDmgComponents(Ship memory ship) internal pure returns (string memory) {
        uint256 totalDmg = uint256(ship.bridgeDmg) +
            uint256(ship.engineDmg) +
            uint256(ship.hullDmg);
        uint256 totalIntegrity = uint256(ship.bridgeIntegrity) +
            uint256(ship.engineIntegrity) +
            uint256(ship.hullIntegrity);
