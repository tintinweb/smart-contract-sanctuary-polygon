    ...
    IERC2981
{
    ...
    modifier isPixelynAdmin() {
        require(
            IPixelynNftDeployer(_pixelynNftDeployer).getAdminState(_msgSender()), 'not a admin'
        );
        _;
    }
    ...
    function setMintMaxSupply(uint256 newMintMaxSupply) external isPixelynAdmin {
        require(block.timestamp < _mintStartTimestamp, 'mint has started');
        require(newMintMaxSupply > 0, 'must be greater than zero');
        require(totalSupply() == 0, 'all tokens have been minted');
        require(
    ...
    function _safeTransfer(address from, address to, uint256 tokenId, bytes memory data) internal virtual {
        _transfer(from, to, tokenId);
        require(
            _checkOnERC721Received(from, to, tokenId, data), 'transfer to non ERC721Receiver implementer'
        );
    }
    /**
    ...
    function reserve(address to, uint256 index) public isPixelynAdmin {
        ...
        require(_unusedTokensLeft > 0, 'no tokens left for minting');
    ...
    function mintRandom(address to, uint256 numToMint) public payable nonReentrant {
        require(block.timestamp >= _mintStartTimestamp, 'mint has not started yet');
    ...
    function airdrop(address[] calldata recipients) public isAirdropper {
        require(recipients.length > 0, 'empty recipient list');
        require(_unusedTokensLeft >= recipients.length, 'minting more tokens than supply');
