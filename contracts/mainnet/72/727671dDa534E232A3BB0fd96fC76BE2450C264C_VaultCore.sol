...
contract VaultCore is VaultStorage, MiniCurveExchange {
    ...
    function _mint(
        ...
        address _assetDefaultStrategy = assetDefaultStrategies[_asset];
        require(_assetDefaultStrategy != address(0), "No default strategy for asset");
        ...
        IStrategy strategy = IStrategy(_assetDefaultStrategy);
        if (_asset == strategy.token0()) {
            _assetToken.safeTransfer(_assetDefaultStrategy, _toUseAsset);
            IStrategy(_assetDefaultStrategy).directDeposit();
            emit AssetAllocated(_asset, _assetDefaultStrategy, _toUseAsset);
        } else {
            _toUseAsset = _swapAsset(_asset, primaryStableAddress, _toUseAsset);
        }
