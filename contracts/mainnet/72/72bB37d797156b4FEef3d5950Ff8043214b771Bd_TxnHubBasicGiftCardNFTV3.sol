...
abstract contract TxnHubAbstractNFTV2 is Pausable, ERC1155Base, ERC165Base, Ownable, ITxnHubNftBase {
    mapping(address => bool) private authorizedContracts_;
    string private uri_;
    ...
    function _checkAuthorized() internal view virtual {
        require(_msgSender() == owner() ||
            authorizedContracts_[_msgSender()], "Unauthorized Agent");
    }

    function _checkIsAuthorized(address addr) internal view virtual returns (bool) {
        return addr == owner() || authorizedContracts_[addr];
    }
...
//        emit AgentMintBatch(operator, _msgSender(), to, ids, amounts);
    ...
    function _setAuthorizedContract(address contractAddr, bool enable) internal virtual {
        authorizedContracts_[contractAddr] = enable;
...
import "../TxnHubAbstractNFTV2.sol";
import "../../interfaces/ITxnHubSimpleNFT.sol";

contract TxnHubBasicGiftCardNFTV3 is TxnHubAbstractNFTV2, ITxnHubSimpleNFT {

    constructor(string memory name, string memory symbol, string memory _uri) TxnHubAbstractNFTV2(name, symbol, _uri){
    }
    ...
    function setAuthorizedContract(address contractAddr, bool enable) external onlyOwner {
        _setAuthorizedContract(contractAddr, enable);
    }

    function checkAuthorized(address addr) external view returns (bool) {
        return _checkIsAuthorized(addr);
    }
}