...
/// @author Nitro League.
contract RaceEvent is IRaceEvent, Context, Ownable, Gameable, TokenWithdrawer {
    ////////////
    // ACCESS //
    ////////////
    // See Ownable.
    ...
    event WinningPositions(
        uint indexed oldWinningPositions,
        uint indexed newWinningPositions
    );
     ...
     * @param raceEventType_ type of Race Event. Pass
     * 0 for PRACTICE.
     * 1 for DAILY.
     * 2 for SPECIAL.
     * 3 for TOURNAMENT.
    ...
    constructor(
        ...
        require(nitro_rewardFactory[0] != address(0), "Zero address");
        require(nitro_rewardFactory[1] != address(0), "Zero address");
    ...
    function endRaceEvent(address[] memory results_)
        external
        override
        onlyGame
    {
        require(
            results_.length <= winningPositions,
            "Winners exceed winning positions"
        );
    ...
    /// Cancel Race Event.
    function cancelRaceEvent() external override onlyGame {
        require(
            rewardManager.getRewardState() == 0,
    ...
    ) external override onlyGame returns (address) {
        require(
    ...
    function setWinningPositions(uint winningPositions_)
        ...
        onlyGame
    {
        uint oldWinningPositions = winningPositions;
        winningPositions = winningPositions_;
        emit WinningPositions(oldWinningPositions, winningPositions);
    }
