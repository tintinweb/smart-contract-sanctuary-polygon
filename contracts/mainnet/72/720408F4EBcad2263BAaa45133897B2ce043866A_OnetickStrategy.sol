...
import "./OnetickVault.sol";
import "./interfaces/IStrategy.sol";

/**
 * @title   Passive Strategy
 * @notice  Rebalancing strategy for Onetick Vault that maintains the two
 *          following range orders:
 *
 *          1. Base order is placed at current tick (X, X + TS).
 *          2. Limit order is placed in next tick above (X + TS, X + 2TS)
 *               or below (X - TS, X), depending on which token it holds more of.
 *
 *          where:
 *
 ...
 *
 */
contract OnetickStrategy is IStrategy {
    OnetickVault public immutable vault;
    ...
    /**
     * @param _vault Underlying Onetick Vault
    ...
    constructor(
        ...
        IUniswapV3Pool _pool = OnetickVault(_vault).pool();
        int24 _tickSpacing = _pool.tickSpacing();

        vault = OnetickVault(_vault);
        pool = _pool;
    ...
    function shouldRebalance() public view override returns (bool) {
        ...
        // check price is in different tick
        (, int24 tick, , , , , ) = pool.slot0();
        if (tick == lastTick) {
            return false;
        }
        ...
        if (tick < TickMath.MIN_TICK + 2 * tickSpacing || tick > TickMath.MAX_TICK - 2 * tickSpacing) {
            return false;
        }
...
library TickMath {
    ...
    uint256 internal constant UINT_MAX_TICK = 887272;
    ...
    function getSqrtRatioAtTick(int24 tick) internal pure returns (uint160 sqrtPriceX96) {
        ...
        require(absTick <= UINT_MAX_TICK, 'T');
 ...
 */
contract OnetickVault is IVault, IUniswapV3MintCallback, IUniswapV3SwapCallback, ERC20, ReentrancyGuard {
    ...
    function rebalance(
        ...
        if (swapAmount != 0) {
            pool.swap(address(this), swapAmount > 0, swapAmount > 0 ? swapAmount : -swapAmount, sqrtPriceLimitX96, "");
