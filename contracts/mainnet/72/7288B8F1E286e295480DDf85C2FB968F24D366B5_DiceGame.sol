...
contract DiceGame is BaseGame, ReentrancyGuard {
    ...
    IMinBetManager public betManager;

    constructor(
        uint64 _subId,
        address bankAddress,
        address feeRouterAddress,
        address minBetAddress
    ) BaseGame(_subId, bankAddress, feeRouterAddress) {
        betManager = IMinBetManager(minBetAddress);
    }
    ...
    function placeBet(
    ...
    ) external nonReentrant IsNotHalted {
        ...
        if (!betManager.IsMinBet(asset, amount)) {
            revert BetAmountBelowMinBet(asset, amount);
        }

        uint8 betSum = getSum(options);
        if ((betSum == 0) || (betSum >= modulo) || (options.length != modulo)) {
        ...
        uint256 requestId = _requestRandomValues(500000, 3, 1);
        ...
        override
    {
        ...
        bets[id].randomValue = randomValue;
        ...
        if (hasWon(bet.betOn, result) == true) {
            ...
            // Bankroll Share
            uint256 brshare = ((bet.amount - share) * settings.bankrollShare) / 10000;
    ...
    /* onlyOwner */
    function changeBetManager(address newManager) external onlyOwner {
        betManager = IMinBetManager(newManager);
    }
    ...
    function getWinAmount(uint256 betAmount, uint8 betSum)
        internal
        view
        returns (uint256)
    {
    ...
    function getSum(uint8[] memory options) internal view returns (uint8) {
        uint8 sum = 0;
    ...
    function hasWon(uint8[] memory options, uint8 winNumber)
        internal
        view
        returns (bool win)
    {
...
error BetAmountBelowMinBet(address token, uint256 amount);
...
abstract contract BaseGame is Ownable, VRFConsumerBaseV2 {
    ...
    function setChainlinkSettings(uint64 _subId, bytes32 _keyHash)
        external
        onlyOwner
    {
        chainlinkConfig.subscriptionId = _subId;
