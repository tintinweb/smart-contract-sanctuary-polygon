...
// hevm: flattened sources of src/tranche.sol
...
contract Tranche is Math, Auth, FixedPoint {
    ...
    function depend(bytes32 contractName, address addr) public auth {
        ...
        else if (contractName == "coordinator") {coordinator = CoordinatorLike_2(addr);}
        else revert();
    ...
    function _safeTransfer(ERC20Like_2 erc20, address usr, uint amount) internal returns(uint) {
    ...
    function authTransfer(address erc20, address usr, uint amount) public auth {
        ERC20Like_2(erc20).transfer(usr, amount);
