...
contract EstrategiaAave is Ownable {
     ...
     * @param _token token utilizado por la estrategia (USDC).
     * @param _dataProvider address del dataProvider de AAVE.
     * @param _lendingPool address de la lendingPool de AAVE.
     */
    constructor(
        address _vault,
        address _token,
    ...
    /**
     * @dev Deposita los tokens en la Lending Pool de AAVE.
     */
    function depositar() public {
        uint256 tokenBal = IERC20(token).balanceOf(address(this));

        if (tokenBal > 0) {
            ILendingPool(lendingPool).deposit(token, tokenBal, address(this), 0);
        }
    ...
    /**
     * @dev Reinvierte los beneficios. No utilizada en esta estrategia.
     */
    function reinvertir() external {}

    /**
     * @dev Retorna el estado de depósitos y préstamos.
     */
    function reservasUsuario() public view returns (uint256, uint256) {
        (uint256 balDepositado, , uint256 balPrestado, , , , , , ) = IDataProvider(
            dataProvider
        ).getUserReserveData(token, address(this));
        return (balDepositado, balPrestado);
    }

    /**
     * @dev Retorna los datos vinculados a la posición de la estrategia.
     */
    function datosDeUsuario()
        public
        view
        returns (
            uint256 totalColateralUSDC,
            uint256 totalDeudaUSDC,
            uint256 disponibleTomarPrestado,
            uint256 actualUmbralLiquidacion,
            uint256 ltv,
            uint256 factorSalud
        )
    {
    ...
    function balanceEstrategia() public view returns (uint256) {
        return balanceDeToken() + balanceDesplegado();
    }
    ...
    /**
     * @dev Calcula el balance total de token en AAVE.
     */
    function balanceDesplegado() public view returns (uint256) {
        (uint256 balDepositado, uint256 balPrestado) = reservasUsuario();
        return balDepositado - balPrestado;
    }
    ...
    function retirarEstrategia() external {
        ...
        ILendingPool(lendingPool).withdraw(token, MAX, address(this));
    ...
    function _darPermisos() internal {
        IERC20(token).safeApprove(lendingPool, MAX);
    }
    
    /**
     * @dev Remueve permisos a contratos para transferir tokens.
     */    
    function _removerPermisos() internal {
...
/**
 * @dev Interface of AAVE's IProtocolDataProvider.
 */
...
/**
 * @dev Interface of AAVE's ILendingPool.
 */
