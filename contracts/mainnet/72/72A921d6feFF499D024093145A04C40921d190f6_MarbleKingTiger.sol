...
contract MarbleKingTiger is ERC721 {
    uint256 public totalSupply;
    uint256 public cost = 0.07 ether;
    uint256 public maxMints = 45;
    ...
    constructor()
        ERC721("Marble King Tiger", "MKT")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 46) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeifoqhdkr3lcieqzbeazta7j6cdrprpk4gck53uqfnvit2o5qoanem/", Strings.toString(tokenId), ".json")
            );
    }
