...
import {IAdministratorRegistry} from "../AdministratorRegistry/IAdministratorRegistry.sol";
    ...
    IProduct
{
    ...
    IAdministratorRegistry private _administratorRegistry;

    // ============ Events ============

    /**
     * @notice Emitted when the pointer to the Administrator Registry is updated.
     */
    event AdministratorRegistryUpdated(address indexed registry);
    ...
    function initialize(
        ...
        IStorageV3 store,
        address admin,
        address administratorRegistry_
    ) public initializer {
        ...
        setAdministratorRegistry(administratorRegistry_);
    }

    // ============ Modifier Methods ============

    /**
     * @notice Checks whether sender is in the registry.
     * Throws if called by an address that has not been added to the registry.
     */
    modifier onlyAdmins() {
        if (!_administratorRegistry.isAdmin(_msgSender())) {
            revert Unauthorized();
        }
        _;
    }
    ...
    /**
     * @notice Get address of the current Administrator Registry.
     */
    function administratorRegistry() external view returns (address) {
        return address(_administratorRegistry);
    }
    ...
    /**
     * @notice Set pointer to the Administrator Registry contract.
     */
    function setAdministratorRegistry(
        address administratorRegistry_
    ) public onlyOwner {
        _administratorRegistry = IAdministratorRegistry(administratorRegistry_);
        emit AdministratorRegistryUpdated(administratorRegistry_);
    }

    /**
     * @notice RevokableOperatorFiltererUpgradeable and OwnableUpgradeable both contain owner() function.
     *         AccountV4 is inherited from them, so we need to override the owner() function to determine what it should return.
...
interface IAdministratorRegistry {
    /**
