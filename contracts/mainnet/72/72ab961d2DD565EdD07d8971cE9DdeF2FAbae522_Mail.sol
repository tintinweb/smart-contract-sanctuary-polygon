...
contract Mail is OwnableUpgradeable {
    ...
    function setUrBox(address addr) external onlyOwner {
        urBox = UrBox(addr);
    }
...
contract UrBox is OwnableUpgradeable{
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ICattle1155 public item;
    IBadge public badge;
    IERC20Upgradeable public bvt;
    ...
    mapping(uint => uint) public boxAmount;
    mapping(uint => BoxInfo) public boxInfo;
    address public superMinter;
    mapping(address => uint) public minters;
    mapping(address =>mapping(uint => uint)) _balance;
    event Mint(address indexed addr,uint indexed boxID);
    function initialize() public initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        superMinter = msg.sender;
        // newBoxInfo(80001,[6,9,12],[9,9,1],0,0,5000 ether);
        // newBoxInfo(80002,[6,9,11],[5,5,1],0,0,3500 ether);
        // newBoxInfo(80003,[5,8,10],[5,5,1],0,0,1500 ether);
        // newBoxInfo(80004,[5,8,10],[3,3,1],0,0,1200 ether);
        // newBoxInfo(80005,[4,7],[3,3],0,0,800 ether);
    }
    ...
    function newBoxInfo(uint boxID,uint[] calldata itemList,uint[] calldata itemAmount,uint badgeId,uint skinId,uint bvtAmount) public onlyOwner{
        boxInfo[boxID] = BoxInfo({
            boxID : boxID,
            itemList : itemList,
    ...
    function _processOpen(address addr,uint boxID) internal{

        BoxInfo memory info = boxInfo[boxID];
        if(info.badgeId != 0){
            badge.mint(addr,info.badgeId);
        }
        if(info.skinId != 0){
            skin.mint(addr,info.skinId);
        }
        
        for(uint i = 0; i < info.itemList.length; i++){
            item.mint(msg.sender,info.itemList[i],info.itemAmount[i]);
        }
        
        bvt.transfer(addr,info.bvtAmount);
    }
    
    function openBox(uint boxId,uint amount) external{
        require(amount > 0,'wrong input amount');
        require(boxId > 30000 && boxId < 40000,'wrong box id');
        item.burn(msg.sender,boxId,amount);
        for(uint i = 0; i< amount; i++){
            _processOpen(msg.sender,boxId);
        }
    }
}
