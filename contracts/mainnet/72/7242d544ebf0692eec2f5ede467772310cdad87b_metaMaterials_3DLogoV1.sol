...
// @notice contract influences: antonymnft.com, azuki.com, cerealclub.io, livesofasuna.com
// @notice reference Creator terms for token Terms of Use. This document can be found in the description metadata.
...
contract metaMaterials_3DLogoV1 is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 private constant MAX_MINTS_PER_TX = 1000;
  ...
  uint256 private _auctionMaxSupply = 0;
  uint256 private _marketingSupply = 1000;
  uint256 private _internalSalePrice = 0.5 ether;

  // FOR CONTRACT NATIVE ROYALTIES. 
  address private royaltyAddress;
  uint96 private royaltyBasisPoints = 550; //250 = 2.5% for us. Add artist fee. eg: if artist fee=3% total=550.
  ...
  constructor(address verifier,
              ...
              ERC721A("metaMaterials_3DLogoV1", "MATS")
             {
              _verifier = verifier;
  ...
  // Payments Splitter release - Triggers a transfer to `account` of the amount of Ether they are owed, according to their percentage of the total shares and their previous withdrawals.
  ...
  function presaleMint(bytes calldata sig) external payable nonReentrant onlyEOA {
    ...
    if (hasMintedPresale(msg.sender))    revert AmountExceedsAllowlistLimit();
  ...
  function hasMintedPresale(address WLaccount) public view returns (bool) {
    return _mintedAllowlist[WLaccount];
  }

  function isAllowlisted(address WLaccount, bytes calldata sig) internal view returns (bool) {
    return ECDSA.recover(keccak256(abi.encodePacked(WLaccount)).toEthSignedMessageHash(), sig) == _verifier;
  }
  ...
  struct MintSummary {
    ...
    uint256 auctionPrice; uint256 internalSalePrice; uint256 blockTime;
  ...
  function getMintSummary() external view returns (MintSummary memory summary) {
    ...
    // Compute next drop in price for auction
    ...
    summary = MintSummary({
      ...
      internalSalePrice: getSalePrice(),
  ...
  // Royalty Information. 
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        return (royaltyAddress, (_salePrice * royaltyBasisPoints) / 10000); //10000 = 100%
    }

    /** ROYALTIES INTERFACE FOR EIP2981. 
