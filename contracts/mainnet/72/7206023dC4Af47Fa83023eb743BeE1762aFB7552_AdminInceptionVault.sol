...
contract AdminInceptionVault is IAdminInceptionVault, BoringOwnable, Initializable {
  ...
  uint256 private _collateralCount = 0;
  mapping(uint256 => address) private _collaterals;
  mapping(address => uint256) private _collateralId;
  ...
  **/
  function depositAndBorrow(
    ...
    collateralType.safeIncreaseAllowance(address(a_.core()), _depositAmount);
    a_.core().depositAndBorrow(_collateralType, _depositAmount, _borrowAmount);
    if (_collateralId[_collateralType] == 0) {
      uint256 newId = ++_collateralCount;
  ...
  */
  function claimMimo() external override {
    ...
    uint256 collateralCount_ = _collateralCount;
    for (uint256 i = 1; i <= collateralCount_; i++) {
      IGenericMiner supplyMiner = IGenericMiner(address(debtNotifier_.collateralSupplyMinerMapping(_collaterals[i])));
  ...
  */
  ...
  function getCollateralCount() external view override returns (uint256) {
    return _collateralCount;
  }

  function getCollateral(uint8 _id) external view override returns (address) {
    return _collaterals[_id];
  }

  function getCollateralId(address _collateral) external view override returns (uint256) {
  ...
  **/
  function deposit(address _collateralType, uint256 _amount) public override onlyOwner {
    ...
    collateralType.safeIncreaseAllowance(address(a_.core()), _amount);
    a_.core().deposit(_collateralType, _amount);
  }
  ...
  **/
  function borrow(uint256 _vaultId, uint256 _amount) public override onlyOwner {
    IAddressProvider a_ = _a;
    a_.core().borrow(_vaultId, _amount);
    IVaultsDataProvider vaultsData = IVaultsDataProvider(a_.vaultsData());
...
interface IAdminInceptionVault {
  ...
  function getCollateral(uint8 _id) external view returns (address);

  function getCollateralId(address _collateral) external view returns (uint256);
}
