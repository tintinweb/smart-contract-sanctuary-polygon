...
contract PiecesOfEdenPass is ERC721AEnumerable, Payable, DefaultOperatorFilterer {
    ...
    constructor() ERC721AEnumerable("Assassin's Creed Pieces of Eden Pass", "ACPE") {
        placeholderURI = 'ipfs://.../';

        priceFeed = AggregatorV3Interface(0xF9680D99D6C9589e2a93a78A04A279e509205945); // 0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419
        ...
        correctValue(getPriceEth() * quantity)
    {
        if (contractState == ContractState.PRESALE1) {
            ...
            if (verify(merkleRootAllowlist, leaf, proofAllowlist) && !allowlistMint[to] && maxAllowlist > 0) {
                maxAllowlist--;
			...
			if (_totalMinted() + totalMints < maxSalePlusOne) {
				revert("Exceeds available tokens!");
			}
            ...
            uint256 totalMints;
            if (verify(merkleRootGuaranteed, leaf, proofGuaranteed) && !guaranteedMint[to] && _totalMinted() < maxSalePlusOne) {
				guaranteedMint[to] = true;
                totalMints++;         
            }
            if (verify(merkleRootAllowlist, leaf, proofAllowlist) && !allowlistMint[to] && _totalMinted() + totalMints < maxSalePlusOne) {
                allowlistMint[to] = true;
                totalMints++;
            }
            if (totalMints == 0) {
            ...
            require(_totalMinted() + quantity < maxSalePlusOne, "Exceeds available amount!");
