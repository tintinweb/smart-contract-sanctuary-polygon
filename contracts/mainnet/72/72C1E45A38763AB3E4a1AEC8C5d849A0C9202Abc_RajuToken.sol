...
contract RajuToken is Initializable {
    ...
    function _specialTransfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (block.timestamp >= _phase1starttime && block.timestamp < (_phase1starttime + _phase1period)) {
                require(sender == _owner || recipient == _owner || sender == _presaleContract || recipient == _presaleContract, "error: Not admin!");
                _tokenTransfer(sender, recipient, amount);
                emit Transfer(sender, recipient, amount);
                return;
        } else if (block.timestamp >= _phase2starttime && block.timestamp <= (_phase2starttime + _phase2period))
        {
            if (sender == _owner || recipient == _owner || sender == _presaleContract || recipient == _presaleContract) {
                _tokenTransfer(sender, recipient, amount);
                return;
            }
            uint burnAmount = amount.mul(_p2pFee).div(10000);
            _tokenTransfer(sender, _blackHole, burnAmount);
    ...
    function _buyBack(uint autoBurnUsdcAmount) internal {
        ...
        _swapUsdcForTokens(autoBurnUsdcAmount, _blackHole);
    }
    ...
    function setStartTime1(uint256 phase1starttime) external limited {
        require(phase1starttime >= block.timestamp, "Phase1time error");
        _phase1starttime = phase1starttime;
    }

    function setPeriod1(uint256 phase1period) external limited {
        require(_phase1starttime >= block.timestamp, "Phase1period error");
        _phase1period = phase1period;
    }

    function setStartTime2(uint256 phase2starttime) external limited {
        require(phase2starttime >= block.timestamp, "Phase2time error");
        _phase2starttime = phase2starttime;
    }

    function setPeriod2(uint256 phase2period) external limited {
        require(_phase2starttime >= block.timestamp, "Phase2period error");
        _phase2period = phase2period;
    }

    function setStartTime3(uint256 phase3starttime) external limited {
        require(phase3starttime >= _phase2starttime, "Phase3time error");
        _phase3starttime = phase3starttime;
    }

    function setPeriod3(uint256 phase3period) external limited {
        require(_phase3starttime >= block.timestamp, "Phase3period error");
        _phase3period = phase3period;
    }

    function setphase2rebaserate(uint256 phase2rebaserate) external limited {
        require(_phase2starttime >= block.timestamp, "Phase2rate error");
        _phase2rebaserate = phase2rebaserate;
    }

    function setphase3rebaserate(uint256 phase3rebaserate) external limited {
        require(_phase3starttime >= block.timestamp, "Phase3rate error");
        _phase3rebaserate = phase3rebaserate;
    }

    function setPresale(address presaleContract) external limited {
        require(presaleContract != address(0), "Error: Can not be address zero.");
