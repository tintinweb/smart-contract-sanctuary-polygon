...
contract Collection is Ownable {
  ...
  function mintAdd(address _to, uint256 _BpaperID, uint256 _Amount) public payable {    
    if (msg.sender != owner() && msg.sender != AdAddr) {
      ...
      require(cStr(checkContract.checkConAddPaper(_BpaperID,msg.sender, _Amount,cPaper.maxMint),"Match"),
        checkContract.checkConAddPaper(_BpaperID,msg.sender, _Amount, cPaper.maxMint));
    } 
  ...
  function checkNewPaperforMint(uint _PaperID) view public returns (uint){
    CheckQua checkCon = CheckQua(CheckQuaAddr);
    Paper memory cPaper = papers[_PaperID];
    return checkCon.checkNewPaperforMint(_PaperID, cPaper.maxMint);
  }

  function checkPaperMaxMint(uint _PaperID) view public returns (uint){
    Paper memory cPaper = papers[_PaperID];
    return cPaper.maxMint;
  }
  ...
  function zRO(uint _PaperID, string memory _title, string memory _category, string memory _content, 
    uint _minterBcardID, uint _maxMint, string memory _editTime) external payable {
    Paper storage cPaper = papers[_PaperID];
    if (msg.sender==owner()||msg.sender== AdAddr){
      cPaper.title = _title;
      cPaper.category = _category;
      cPaper.content = _content;
      cPaper.minterBcardID = _minterBcardID;
      cPaper.maxMint = _maxMint;
      cPaper.editTime = _editTime;
    }
  }

  function zRS(uint _supply) external payable {
    if (msg.sender==owner()||msg.sender== AdAddr){
      totalToken = _supply;
    }
  }
