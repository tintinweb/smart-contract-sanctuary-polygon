...
import '@ascentexchange/v3-core/contracts/interfaces/IAscentV3Factory.sol';
import '@ascentexchange/v3-core/contracts/interfaces/callback/IAscentV3MintCallback.sol';
...
/// @notice Internal functions for safely managing liquidity in AscentExchange V3
abstract contract LiquidityManagement is IAscentV3MintCallback, PeripheryImmutableState, PeripheryPayments {
    struct MintCallbackData {
        PoolAddress.PoolKey poolKey;
        address payer;
    }

    /// @inheritdoc IAscentV3MintCallback
    function ascentV3MintCallback(
        uint256 amount0Owed,
        uint256 amount1Owed,
        bytes calldata data
...
abstract contract PoolInitializer is IPoolInitializer, PeripheryImmutableState {
    ...
    function createAndInitializePoolIfNecessary(
        ...
        pool = IAscentV3Factory(factory).getPool(token0, token1, fee);

        if (pool == address(0)) {
            pool = IAscentV3Factory(factory).createPool(token0, token1, fee);
            IAscentV3Pool(pool).initialize(sqrtPriceX96);
        } else {
            (uint160 sqrtPriceX96Existing, , , , , , ) = IAscentV3Pool(pool).slot0();
...
/// @notice Wraps AscentExchange V3 positions in a non-fungible token interface which allows for them to be transferred
/// and authorized.
...
import '@ascentexchange/v3-core/contracts/libraries/FixedPoint96.sol';
...
import '@ascentexchange/v3-core/contracts/interfaces/IAscentV3Pool.sol';
import '@ascentexchange/v3-core/contracts/libraries/FixedPoint128.sol';
import '@ascentexchange/v3-core/contracts/libraries/FullMath.sol';
...
/// @title NFT positions
/// @notice Wraps Ascent V3 positions in the ERC721 non-fungible token interface
contract NonfungiblePositionManager is
    ...
    SelfPermit
{
    // details about the ascent position
    struct Position {
    ...
    ) ERC721Permit('Ascent V3 Positions NFT-V1', 'PCS-V3-POS', '1') PeripheryImmutableState(_deployer, _factory, _WETH9) {
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        PoolAddress.PoolKey memory poolKey = _poolIdToPoolKey[position.poolId];
        IAscentV3Pool pool = IAscentV3Pool(PoolAddress.computeAddress(deployer, poolKey));
