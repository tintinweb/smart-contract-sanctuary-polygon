...
contract SpoilsOfWar is Context, Ownable, IBEP20 {
    ...
    function signUp(address referral, uint8 fleet, uint8 quick) public payable {
        ...
        fleet_ratededuct[fleet-1] += SafeMath.div(msg.value, 50 ether);   
	...
	function startGame() external {
        Player storage player = players[msg.sender];		
		transferTokens(msg.sender, address(this), 30 ether);		
    ...
    function whosWinning() view external returns(uint8 winner) {
        ...
        percentages[0] += getPercentile(addDeductions(fleet_ships[0], fleet_shipdeduct[0]),mx2, 35); 
        percentages[1] += getPercentile(addDeductions(fleet_ships[1], fleet_shipdeduct[1]),mx2, 35); 
        ...
        percentages[0] += getPercentile(fleet_battles[0],mx3, 40); 
        percentages[1] += getPercentile(fleet_battles[1],mx3, 40);        
