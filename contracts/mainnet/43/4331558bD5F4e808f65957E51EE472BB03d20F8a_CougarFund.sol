...
contract CougarFund is Ownable {
    ...
    event Withdrawed(address indexed token, address indexed recipient, uint256 amount);

    function Withdraw(IBEP20 _token, address _recipient) public onlyOwner {
        require(_recipient != address(0), "Cannot withdraw: ZERO address.");

        uint256 amount = _token.balanceOf(address(this));
        _token.safeTransfer(_recipient, amount);
        emit Withdrawed(address(_token), _recipient, amount);
    }
}
