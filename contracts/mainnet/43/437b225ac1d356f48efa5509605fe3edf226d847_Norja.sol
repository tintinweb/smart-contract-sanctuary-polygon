...
contract Norja is ERC20, AccessControl, ERC721Holder {
    ...
    constructor(address owner, address _collectionAddress) ERC20("Norja Coin", "NORJA") AccessControl(){
        ...
        coinAmountPerRewardUnit = 12.5 * 10 ** 18; // 5 ERC20 coins per rewardUnit, may be changed later on
        numberOfBlocksPerRewardUnit = 20571; // 12 hours per reward unit , may be changed later on
        welcomeBonusAmount = 0 * 10 ** 18; // 170 tokens welcome bonus, only paid once per tokenId
    }
    ...
    function stake(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddress).ownerOf(tokenId) == address(this),
            "NORJA: Error while transferring token");
    ...
    function setCollectionAddress(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "NORJA: update to zero address not possible");
