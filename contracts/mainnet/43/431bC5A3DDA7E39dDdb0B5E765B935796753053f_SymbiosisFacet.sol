...
contract SymbiosisFacet is ReentrancyGuard, IBridge, ISymbiosis {
    ...
    constructor(address _symbiosis) {
    ...
    function bridgeToSymbiosis(BridgeData memory _bridgeData, bytes memory _symbiosis) external payable {
        ...
        symBiosis.call{value: msg.value}(_symbiosis);
    ...
    function swapAndBridgeToSymbiosis(SwapData calldata _swap, BridgeData memory _bridgeData, bytes calldata _symBiosisData) external payable {
        ...
        if (isNotNative) {
            IERC20(_bridgeData.srcToken).safeApprove(symBiosis, _bridgeData.amount);
        }
        (bool succ, ) = symBiosis.call{value: isNotNative ? 0 : _bridgeData.amount}(data);
    ...
    function encodeCalldataWithOverride(bytes calldata beforeSymData, uint256 amount) public view returns (bytes memory afterSymData) {
    ...
    function encodeCalldataWithOverrideParameter(bytes calldata beforeSymData, uint256 symAmount) external pure returns (bytes memory afterSymData) {
        bytes4 selector = bytes4(beforeSymData);
        if (selector == 0xa11b1198) {
            (
                bytes memory firstSwapCalldata,
                bytes memory secondSwapCalldata,
            ...
            ) = abi.decode((beforeSymData[4:]), (bytes, bytes, address[], address, address, uint256, bool, address, bytes));
            amount = symAmount;
            return
