...
contract Registry is Initializable, AccessControlEnumerableUpgradeable {
    ...
    event LogCityCreated(uint256 indexed cityIdx, bytes32 indexed cityHash);
    ...
    function getCity(uint256 cityIdx)
        public
        view
        returns(CityDetail memory)
    {
        CityDetail memory res = cities[cityIdx];
        return res;
    }
       ...
       returns(string memory)
   {
    ...
    function createCity(CityDetail calldata city)
        external
        onlyDefaultAdmin
    {
        cities.push(city);
        ...
        unchecked {
            emit LogCityCreated(cities.length - 1, city.cityHash);
        }
    }

    function createProject(ProjectDetail calldata projectDetail, ProjectMint calldata projectMint, address owner, bytes32 cityHash)
        public
        onlyDefaultAdmin
    {
        require(projectMint.baseTokenImplementation != address(0), "createProject: baseTokenImplementation not set");
        require(cityHash == cities[projectDetail.cityIdx].cityHash, "createProject: city mismatch");
