...
contract Solmit is IERC20, Auth {
    ...
    function sendAirdrop(address _address , uint256 amount) external onlyOwner{
        ...
        uint256 currentTime = block.timestamp;
        usersAirdrop[_address].push([currentTime + (MONTH * 6)  , amount.mul(25).div(100)]);
        usersAirdrop[_address].push([currentTime + (MONTH * 7) , amount.mul(25).div(100)]);
        usersAirdrop[_address].push([currentTime + (MONTH * 8) , amount.mul(25).div(100)]);
        usersAirdrop[_address].push([currentTime + (MONTH * 9) , amount.mul(25).div(100)]);
