...
contract Bridge is Pausable, AccessControl, SafeMath {
    ...
    mapping(uint64 => mapping(uint32 => bytes)) public _depositRecords;
     ...
     */
    constructor (uint32 chainID, address[] memory initialRelayers, uint initialRelayerThreshold, uint256 fee, uint256 expiry) public {
        _chainID = chainID;
     ...
     */
    function getProposal(uint32 originChainID, uint64 depositNonce, bytes32 dataHash) external view returns (Proposal memory) {
     ...
     */
    function cancelProposal(uint32 chainID, uint64 depositNonce, bytes32 dataHash) public onlyAdminOrRelayer {
        uint72 nonceAndID = (uint72(depositNonce) << 8) | uint72(chainID);
        Proposal storage proposal = _proposals[nonceAndID][dataHash];
