...
interface IActiveFarming {
    function startFarming(FarmingParams calldata _params, address _owner) external;

    function endFarming(uint16 _tokenId, address _owner) external;
}
...
contract World is Initializable, WorldContracts {
    ...
    onlyEOA
    {
        ...
        for(uint256 i = 0; i < _farmingParams.length; i++) {
            FarmingParams calldata _farmingParam = _farmingParams[i];
            _tokenIds[i] = _farmingParam.tokenId;

            _requireValidEntityAndLocation(msg.sender, _farmingParam.tokenId, Location.ACTIVE_FARMING);

            // From
            _transferFromLocation(msg.sender, _farmingParam.tokenId);

            // To
            activeFarming.startFarming(_farmingParam, msg.sender);

            // Finalize
            tokenIdToInfo[_farmingParam.tokenId].location = Location.ACTIVE_FARMING;
        }

        emit EntityLocationChanged(_tokenIds, msg.sender, Location.ACTIVE_FARMING);
    }

    // Called by the ERC721 contracts to rip entities out of farming.
    //
    ...
    {
        _requireValidEntityAndLocation(_originalOwner, _tokenId, Location.NOT_STAKED);

        // From
        _transferFromLocation(_originalOwner, _tokenId);

        // To
        // Unstake the entity.
        ownerToStakedTokens[_originalOwner].remove(_tokenId);
    ...
    onlyEOA
    {
        ...
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            _requireValidEntityAndLocation(msg.sender, _tokenId, Location.NOT_STAKED);

            // From
            _transferFromLocation(msg.sender, _tokenId);
    ...
    function _requireValidEntityAndLocation(address _owner, uint256 _tokenId, Location _newLocation) private view {
        Location _oldLocation = tokenIdToInfo[_tokenId].location;
        ...
        if(_oldLocation != Location.NOT_STAKED) {
            require(ownerToStakedTokens[_owner].contains(_tokenId), "World: User does not own entity");
        }

        // Typically cannot go back to the same location. Active Farming will treat the same location as a restart.
        //
        require(_oldLocation != _newLocation || _newLocation == Location.ACTIVE_FARMING, "World: Location must be different for entity");
    }

    function entitiesStakedForOwner(address _owner) external view returns(uint256[] memory) {
