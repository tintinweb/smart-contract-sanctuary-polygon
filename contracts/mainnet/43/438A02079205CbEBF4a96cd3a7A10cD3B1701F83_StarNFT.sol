...
interface IStarNFT {
    ...
    error IStarNFT__ZeroValue();
    error IStarNFT__ZeroAddress();
    error IStarNFT__LengthMismatch();
    error IStarNFT__AlreadyUsed();
    error IStarNFT__NotOwner();
    error IStarNFT__InvalidSignature();
    ...
    ERC721URIStorageUpgradeable
{
    ...
    function createStore(
        ...
        if (amount == 0) revert IStarNFT__ZeroValue();
        ...
        (address recoveredAddress, ) = ECDSAUpgradeable.tryRecover(digest, signature_);
        if (recoveredAddress == address(0) || !hasRole(MINTER_ROLE, recoveredAddress))
            revert IStarNFT__InvalidSignature();
    ...
    function setMetadata(uint256 tokenId_, Metadata calldata metadata_) external {
        ...
        if (_ownerOf(tokenId_) != user) revert IStarNFT__NotOwner();
    ...
    function _checkUnique(uint256 uid_) internal {
        if (_isUsed.get(uid_)) revert IStarNFT__AlreadyUsed();
        _isUsed.set(uid_);
    }
    ...
    function _setTreasury(address treasury_) internal {
        if (treasury_ == address(0) || treasury_ == address(this)) revert IStarNFT__ZeroAddress();
