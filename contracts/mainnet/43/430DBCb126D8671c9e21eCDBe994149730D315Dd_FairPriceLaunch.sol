...
contract FairPriceLaunch is OwnableBase {
    ...
    address public fundsRedeemer = 0x14683076786A9995D17C7e8444Ed9a36AbC43Cd6;
    // The token used for contributions
    address public investToken = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063;

    // The Non-transferable token used for sale, redeemable for LMLT
    ERC20 public redeemableToken;
    ...
    event IssuePresaleLMLT(address investor, uint256 amount);
    ...
    constructor(
        ...
        investToken = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063;
        //times
        launchStartTime = 1646413200; //March 4th 12pm EST
        launchEndTime = launchStartTime.add(7 days);
        saleDuration = 7 days;
    
        //limits
        maxInvestAllowed = 10000000000000000000000; //$10,000 or 10,000 DAI 
        minInvestAllowed = 0;
        maxGlobalInvestAllowed = 10000000000000000000000000; //$1,000,000 or 1,000,000 DAI

        maxRedeemableToIssue = 500000 * 1e18; //500,000 preLMLT
        startingPrice = 500000000000000000; //0.50 DAI
        //ERC20 is passed in as argument and this contract needs to be set as owner
        redeemableToken = ERC20(0xd95E4398f578fD677Be30B1132aC4EAEa7Fb3813);
        saleEnabled = false;
    
    }

    function startSaleNow() public {
    ...
    /**
    @dev Claims the ERC20 tokens equivalent to their contribution
     */
    function claimRedeemable() private {
        ...
        uint256 issueAmount = investor.totalInvested.mul(10**18).div(finalPrice);
 
        investor.totalRedeemed =  investor.totalRedeemed.add(issueAmount);
        totalGlobalIssued = totalGlobalIssued.add(issueAmount);
        redeemableToken.transfer(msg.sender, issueAmount);
        emit IssuePresaleLMLT(msg.sender, issueAmount);
    }
    ...
    function withdrawInvestablePool() public onlyOwner {
        ...
        //LmltTreasury(fundsRedeemer).deposit(amount, investToken, amount.div(1e9));
    }

       function withdrawRedeemablePool() public onlyOwner {
        uint256 amount = ERC20(redeemableToken).balanceOf(address(this));
        ERC20(redeemableToken).transfer(fundsRedeemer, amount);
        //ERC20(investToken).approve(fundsRedeemer, amount);
        //LmltTreasury(fundsRedeemer).deposit(amount, investToken, amount.div(1e9));
    }
