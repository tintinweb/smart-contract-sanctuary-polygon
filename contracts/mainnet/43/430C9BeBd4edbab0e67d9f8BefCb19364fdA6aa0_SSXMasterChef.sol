...
//    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_weeks_unpaid_subscription >= 1) {
            ...
            _amount = _amount > _subscription_fee ? _amount.sub(_subscription_fee) : _amount = 0;
        }
    ...
    function calcTaxRate(uint256 _depositTs) public view returns (uint256 _taxRate) {
        ...
        if (MAX_WITHDRAW_TAX_RATE >= _days_deposited.mul(WITHDRAW_RATE_STEP)) {
            _taxRate = MAX_WITHDRAW_TAX_RATE.sub(_days_deposited.mul(WITHDRAW_RATE_STEP)); // TODO fix if _taxRate < 0
        }

        if (_taxRate < MIN_WITHDRAW_TAX_RATE) _taxRate = MIN_WITHDRAW_TAX_RATE;
    }

    function getWithdrawTax(uint256 _pid) public view returns (uint256 _withdraw_tax) {
        UserInfo memory user = userInfo[_pid][msg.sender];
        _withdraw_tax = calcTaxRate(user.depositTs);
    }

    function getPeriodUnpaidSubscription(uint256 _pid) public view returns (uint256 _unpaid_weeks) {
        UserInfo memory user = userInfo[_pid][msg.sender];
