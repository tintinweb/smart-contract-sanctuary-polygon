...
/**
 * @title BotAttackNFT - Bots of COG Battle Game on Polygon
 */
contract BotAttackNFT is ERC1155LazyMint {

    event LevelUp(address indexed account, uint256 level);
    event Dead(address indexed attacker, address indexed victim, uint256 level);
    ...
    /** 
     * @notice Claim a bot to start playing, but only if you don't already own a bot
     */
    function claimBot() external {
        claim(msg.sender, 0, 1);
        // claiming a Bot enters the game at level 1
    ...
    function verifyClaim(
        ...
        require(_tokenId == 0, "Only Bots can be claimed");
        require(_quantity == 1, "Only one Worker Bot at a time!");
        require(balanceOf[_claimer][0] == 0, "Already got a Worker");
        require(balanceOf[_claimer][1] == 0, "Already got a Drone");
        require(balanceOf[_claimer][2] == 0, "Already got a Sentinal");
    }

    /** 
     * @notice Transfer Bots to level up
     */
    function safeTransferFrom(
        address from,
        address to,
        ...
        // can only transfer bots
        require(id == 0, "This bot is not transferable!");
        super.safeTransferFrom(from, to, id, amount, data);
        if(from != to && id == 0) {
    ...
    /** 
     * @notice Burn a Bot to either level up or attack another Bot
     */
    function burn(
        address account,
        uint256 id,
        ...
        // the owner can burn their NFT, but any Sentinal Bot owner can also burn anyone else's Bot
        require(msg.sender == account || balanceOf[msg.sender][2] > 0, "NOT_TOKEN_OWNER or Sentinal Bot");
    ...
    /**
     * @notice Lets a Sentinal Bot owner attack another user's to burn their Bots
     */
    function attack(address victim) external gameNotPaused {
        address attacker = msg.sender;
        // only a Sentinal Bot owner can attack
        require(balanceOf[attacker][2] > 0, "You Need a Sentinal Bot to Attack!");
        // find a "victim" bot
        ...
        if(balanceOf[victim][0] > 0) {
            ...
            revert("This Owner Doesn't Hold Anymore Bots!");
        }
        // burn it
        _burn(victim, tokenToBurn, 1);
        // mint a death COG token to the attacker
        _mint(attacker, 3, 1, "");
        emit Dead(msg.sender, victim, tokenToBurn + 1);
    }
    ...
    function startGame() external {
        require(msg.sender == owner(), "Only Owner can call Start");
        isGamePaused = false;
    }
    ...
    function stopGame() external {
        require(msg.sender == owner(), "Only Owner can call Stop");
        isGamePaused = true;
    }
