...
contract OscilloExchange is OwnableUpgradeable {
    ...
    function acceptance(LibTrade.Execution[] memory chunk, uint gasprice) public view returns (LibTrade.Acceptance[] memory) {
        ...
        for (uint i = 0; i < chunk.length; i++) {
            ...
            if (IERC20(e.p.base).available(e.p.askAccount, address(this)) < askTransfers[0]) accepts[i].code = accepts[i].code | (1 << LibTrade.CodeIdxAskBalance);
            if (IERC20(e.p.quote).available(e.p.bidAccount, address(this)) < bidTransfers[0]) accepts[i].code = accepts[i].code | (1 << LibTrade.CodeIdxBidBalance);
    ...
    function execute(LibTrade.Execution[] calldata chunk, uint gasLimit) external gasLimiter(chunk.length, gasLimit) {
        ...
        for (uint i = 0; i < chunk.length; i++) {
            ...
            if (IERC20(e.p.base).available(e.p.askAccount, address(this)) < e.p.amount) code = code | (1 << LibTrade.CodeIdxAskBalance);
            if (IERC20(e.p.quote).available(e.p.bidAccount, address(this)) < amountQ) code = code | (1 << LibTrade.CodeIdxBidBalance);
            if (code != 0) {
