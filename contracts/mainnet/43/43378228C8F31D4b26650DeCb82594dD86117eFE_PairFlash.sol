...
contract PairFlash is Ownable,IUniswapV3FlashCallback, PeripheryImmutableState {
    ...
    function uniswapV3FlashCallback(
        ...
        TransferHelper.safeApprove(decoded.token2, address(swapRouter), decoded.amount2);        
        ...
        // Input variables
        uint256 amount = decoded.amount3;
        uint16 referral = 0;

        address lendingCore = address(0x8dFf5E27EA6b7AC08EbFdf9eB090F32ee9a30fcf);

        // Approve LendingPool contract to move your DAI
        IERC20(decoded.token3).approve(lendingCore, amount);

        // Deposit 1000 DAI
        lendingPool.deposit(decoded.token3, amount,address(this), referral);        
        ...
        // Input variables
        uint256 amount = decoded.amount2;
        uint16 referral = 0;
        ...
        // Deposit 1000 DAI
        lendingPool.borrow(decoded.token2, amount,2,0, address(this));   
        ...
        if (amount0Owed > 0) TransferHelper.safeTransferFrom(token0, address(this), msg.sender, amount0Owed);
        if (amount1Owed > 0) TransferHelper.safeTransferFrom(token1, address(this), msg.sender, amount1Owed);
    ...
    //deposit amount3 of token1 to aave
    //borrow amount2 of token2 from aave
    struct FlashParams {
        address token0;
        address token1;
        address token2; 
    ...
    function initFlash(FlashParams memory params) external onlyOwner {
        ...
        IUniswapV3Pool pool = IUniswapV3Pool(address(0x86f1d8390222A3691C28938eC7404A1661E618e0));
