...
contract TokenCondensor is BaseSiloAction{
    ...
    function enter(address implementation, bytes memory configuration, bytes memory inputData) public override returns(uint[4] memory outputAmounts){
        ...
        for(uint i=0; i<4; i++){
            ...
            if(input[index] == output[index]){
                outputAmounts[index] = amount[index] + amountToSwap;//carry through the input amount
                ...
                if(index == 0){
                    ...
                    path[0] = input[index];
                    path[1] = input[index-1];
                }
                if(path[0] == path[1]){
                    amountToSwap += amount[index];
                }
                else{
                    amountToSwap += amount[index];
                    if(amountToSwap > 0){
                        bestQuote = oracle.findOptimalSwap(path[0], path[1], amountToSwap);
                        if(bestQuote.amountOut > 0){
                            SafeERC20.safeApprove(IERC20(path[0]), bestQuote.router, 0);
                            SafeERC20.safeApprove(IERC20(path[0]), bestQuote.router, amountToSwap);
                            ...
                            if(index == 0){//if on final index and a swap was made, set the output amount
    ...
    function createConfig(address[4] memory _inputs, address[4] memory _outputs, address _exchangeOracle) public pure returns(bytes memory configData){
    ...
    function validateConfig(bytes memory configData) public view override returns(bool){
        ...
        for (uint i=0; i<3;i++){
            index = 3-i;
            if(input[index] != output[index] && input[index] != input[index-1]){//then we are trying to make a swap!
