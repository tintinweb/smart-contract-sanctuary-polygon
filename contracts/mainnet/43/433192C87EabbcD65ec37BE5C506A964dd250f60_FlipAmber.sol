...
interface IERC20 {
    ...
    function approve(address spender, uint256 amount) external returns (bool); // Добавлено
}
...
contract FlipAmber {
    ...
    function depositFunds(uint256 amount) external onlyOwner gasLimit(200000) {
    ...
    function withdrawFunds(uint256 amount) external onlyOwner gasLimit(200000) {
        require(amount > 0, "Amount must be greater than zero");
    ...
    modifier gasLimit(uint256 limit) {
        require(gasleft() >= limit, "Not enough gas provided");
        _;
    }
