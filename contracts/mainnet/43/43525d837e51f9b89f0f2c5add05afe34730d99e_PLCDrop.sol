...
contract PLCDrop is ERC1155, ERC2771Context  {
    ...
    ) external onlyOwner
    {
        for(uint i = 0; i < addrArray.length; i++) {
            _mint(addrArray[i], _tokenId, amount, "");
            __setURI(_tokenId, _uri);
        }
    }

    function __setURI(uint256 _id, string memory _uri) internal {
        tokenURI[_id] = _uri;
    }
