    ...
    function getRoyalty(address _collection, uint256 _id, uint256 _price) internal returns (address, uint256) {
        if (_feeManager.getRoyaltiesEnabled()) {
            try IERC2981(_collection).royaltyInfo(_id, _price) returns (address receiver, uint256 amount) {
                if (receiver != address(0) && amount < _price) {
                ...
                uint256 _amount = _price.mul(_percentage).div(_divider);

                // if BAYC, return Yuga Labs address
                if (_collection == address(0xBC4CA0EdA7647A8aB7C2061c2E118A18a936f13D)) {
                    return (address(0xA858DDc0445d8131daC4d1DE01f834ffcbA52Ef1), _amount);
                }

                try ICollection(_collection).owner() returns (address _receiver) {
