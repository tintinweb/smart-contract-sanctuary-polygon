...
interface IHedgeManager {
    ...
    event VarSet(bytes32 indexed what, bytes value, address indexed sender);

    event NewHedge(uint256 indexed hedgeId, address indexed hedgeOwner);

    event HedgeClosed(uint256 indexed hedgeId, address indexed hedgeOwner);

    event HedgeLiquidated(uint256 indexed hedgeId, address indexed hedgeOwner);

    event CollateralAdded(
        uint256 indexed hedgeId,
        uint256 amount,
        address indexed hedgeOwner
    );
    ...
    event WithdrawFees(address token, uint256 amount, address indexed sender);
