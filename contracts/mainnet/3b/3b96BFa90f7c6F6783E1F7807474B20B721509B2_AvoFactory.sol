    ...
    IAvoAuthoritiesList
{
    ...
    function syncAvoAuthorityMappings(address avoSafe_, address[] calldata authorities_) external {
        ...
        address owner_ = IAvoWalletV3(avoSafe_).owner();

        bool isAuthority_;
        for (uint256 i; i < authoritiesLength_; ) {
            // skip owner
            if (authorities_[i] == owner_) {
                unchecked {
                    ++i;
                }
                continue;
    ...
    IERC1271
{
    ...
    function avoSafeNonce() public view returns (uint256) {
        return uint256(_avoSafeNonce);
    }
    ...
    function _executeCast(
        ...
        _transientAllowHash = RESET_BYTES31;
        _transientId = 0;
    ...
    function _executeActions(Action[] memory actions_, uint256 id_, bool isFlashloanCallback_) internal {
        ...
        uint256 storageSlot0Snapshot_; // AvoCore variables
        uint256 customStorageSlotSnapshot_; // custom variables between AvoWallet (slot 1) & AvoMultisig (slot 54)
        // delegate call = ids 1 and 21. for flashloan callback actions the storage checks are not triggered,
        // it should only happen at the highest execution frame level.
        if ((id_ == 1 || id_ == 21) && !isFlashloanCallback_) {
            ...
            assembly {
                storageSlot0Snapshot_ := sload(0x0) // avoImpl, nonce (& initialized vars for Multisig)
            }

            if (IS_MULTISIG) {
                assembly {
                    customStorageSlotSnapshot_ := sload(0x36) // signers related variables for AvoMultisig, slot 54
                }
            } else {
                assembly {
                    customStorageSlotSnapshot_ := sload(0x1) // owner & initialized vars for AvoWallet, slot 1
                }
            }
        }
        ...
        for (uint256 i; i < actionsLength_; ) {
            ...
            } else if (action_.operation == 1 && (id_ == 1 || id_ == 21)) {
        ...
        if (storageSlot0Snapshot_ > 0) {
            ...
            if (IS_MULTISIG) {
                assembly {
                    customStorageSlot_ := sload(0x36) // signers related variables for AvoMultisig, slot 54
                }
            } else {
                assembly {
                    customStorageSlot_ := sload(0x1) // owner & initialized vars for AvoWallet, slot 1
                }
            }

            if (!(storageSlot0_ == storageSlot0Snapshot_ && customStorageSlot_ == customStorageSlotSnapshot_)) {
    ...
    function _validateParams(
        ...
        if (!(actionsLength_ > 0 && (avoSafeNonce_ == -1 || uint256(avoSafeNonce_) == _avoSafeNonce))) {
...
abstract contract AvoCoreProtected is AvoCore {
    ...
    function occupyAvoSafeNonces(uint88[] calldata avoSafeNonces_) external onlySelf {
        ...
        uint256 nextAvoSafeNonce_ = _avoSafeNonce;
        ...
        _avoSafeNonce = uint80(nextAvoSafeNonce_);
    }
...
abstract contract AvoCoreConstants is AvoCoreErrors {
    ...
    /// @dev constants for _transientAllowHash functionality: default reset value and function selectors
...
abstract contract AvoCoreVariablesSlot0 {
    ...
    uint80 internal _avoSafeNonce;

    // 2 bytes empty, can be used customized in AvoWalletVariables / AvoMultisigVariables
}
...
abstract contract AvoCoreVariablesSlot2 {
    ...
    // AvoWallet BEFORE VERSION 2.0.0:
...
abstract contract AvoCoreSlotGaps {
    ...
    uint256[49] private __gaps;
}

abstract contract AvoCoreTransient {
...
abstract contract AvoFactoryConstants is AvoFactoryErrors, IAvoFactory {
    ...
    // reset slot value not to 0 to get smaller gas refund when resetting value, reducing gas cost,
    // because total refund amount is capped to 20% of tx gas cost (EIP-3529).
    address internal constant RESET_ADDRESS = 0x0000000000000000000000000000000000000001;
...
contract AvoFactory is AvoForwarderCore {
    ...
    /// @notice reinitializes the contract setting the transient storage values to default
    function reinitialize() public reinitializer(2) {
        _transientDeployOwner = RESET_ADDRESS;
    ...
    function deployMultisig(address owner_) external onlyEOA(owner_) returns (address deployedAvoMultiSafe_) {
        ...
        _transientDeployVersion = avoMultisigImpl;
        ...
        IAvoMultisigV3(deployedAvoMultiSafe_).initialize();
    ...
    function deployMultisigWithVersion(
        ...
        _transientDeployOwner = owner_;
        _transientDeployVersion = avoMultisigVersion_;
    ...
    function transientDeployDataMultisig() external view returns (address version_, uint256 data_) {
        return (_transientDeployVersion, uint256(uint160(_transientDeployOwner)));
    }
    ...
    /// @param owner_   AvoSafe owner (EOA)
    ...
    function _getSaltMultisig(address owner_) internal pure returns (bytes32) {
        // use owner + index of wallet nr per EOA (currently always 0)
        // Note CREATE2 deployments take into account the deployers address (i.e. this factory address)
        return keccak256(abi.encode(owner_, uint256(0)));
    }
}
    ...
    AvoForwarderErrors
{
    ...
    /// @dev gets the bytes32 salt used for deterministic deployment for `owner_`, same as on AvoFactory
    function _getSalt(address owner_) internal pure returns (bytes32) {
        return keccak256(abi.encode(owner_));
    }

    /// @dev gets the bytes32 salt used for deterministic Multisig deployment for `owner_`, same as on AvoFactory
    function _getSaltMultisig(address owner_) internal pure returns (bytes32) {
        return keccak256(abi.encode(owner_, uint256(0)));
    }
...
abstract contract AvoForwarderViews is AvoForwarderCore {
    ...
    function avoSafeNonce(address owner_) external view returns (uint256) {
...
abstract contract AvoForwarderViewsMultisig is AvoForwarderCore {
    ...
    function avoSafeNonceMultisig(address owner_) external view returns (uint256) {
...
//
// @dev        If this contract changes then the deployment addresses for new AvoMultiSafes through factory change too!!
...
contract AvoMultiSafe {
    ...
    /// next 12 bytes: used flexible for use-cases found in the future
    ...
    /// @notice   sets _avoMultisigImpl & immutable _data, fetching it from msg.sender.
    //
    // @dev      those values are not input params to not influence the deterministic Create2 address!
    constructor() {
        // "\x05\x12\xd5\xa4" is hardcoded bytes of function selector for transientDeployDataMultisig()
        (, bytes memory deployData_) = msg.sender.staticcall(bytes("\x05\x12\xd5\xa4"));

        address impl_;
        uint256 data_;
        assembly {
            // cast first 20 bytes to version address (_avoMultisigImpl)
        ...
        _data = data_;
        _avoMultisigImpl = impl_;
    }

    /// @notice Delegates the current call to `_avoMultisigImpl` unless _avoMultisigImpl() or _data() is called.
    ///         if _avoMultisigImpl() is called then the address for _avoMultisigImpl is returned, same for _data().
    //
    ...
    fallback() external payable {
        ...
        assembly {
            ...
            if eq(functionSelector_, 0xf3b1cd2100000000000000000000000000000000000000000000000000000000) {
...
import { AvoCoreConstants, AvoCoreConstantsOverride, AvoCoreVariablesSlot0, AvoCoreVariablesSlot2, AvoCoreVariablesSlot3, AvoCoreSlotGaps, AvoCoreTransient } from "../AvoCore/AvoCoreVariables.sol";
...
abstract contract AvoMultiSigVariablesSlot0 is AvoCoreVariablesSlot0 {
    // inherited from AvoCoreVariablesSlot0:
    // address internal _avoImplementation;
    // uint80 internal _avoSafeNonce;

    /// @dev AvoMultisigInitializable.sol variables (modified from OpenZeppelin), see ./lib folder
    /// @dev Indicates that the contract has been initialized.
...
abstract contract AvoMultisigVariablesSlot1 {
    // 32 bytes empty, just used to better align rest of storage layout with AvoWallet
    bytes32 private __slot1Placeholder;
}

/// @notice Defines storage variables for AvoMultisig
abstract contract AvoMultisigVariables is
    AvoMultisigConstants,
    AvoMultiSigVariablesSlot0,
    AvoMultisigVariablesSlot1,
    AvoCoreVariablesSlot2,
    AvoCoreVariablesSlot3,
    AvoCoreSlotGaps,
    AvoCoreTransient
{
    ...
    function _setSigners(address[] memory signers_) internal {
        ...
        if (signersCount_ < requiredSigners || signersCount_ > MAX_SIGNERS_COUNT || signersCount_ == 0) {
            revert AvoMultisig__InvalidParams();
        }

        signersCount = uint8(signersCount_);
...
/// @dev contract copied from OpenZeppelin Initializable but with storage vars moved to AvoMultisigVariables.sol
 ...
 * ====
 */
abstract contract AvoMultisigInitializable is AvoMultisigVariables {
    /**
     * @dev Indicates that the contract has been initialized.
     * @custom:oz-retyped-from bool
     */
    // uint8 private _initialized; // -> in AvoMultisigVariables

    /**
     * @dev Indicates that the contract is in the process of being initialized.
     */
    // bool private _initializing; // -> in AvoMultisigVariables
...
contract AvoVersionsRegistry is AvoVersionsRegistryCore, AvoFeeCollector {
    ...
    function setAvoWalletVersion(
        ...
        bool allowed_,
        bool setDefault_
    ) external onlyOwner validAddress(avoWallet_) isContract(avoWallet_) {
    ...
    function setAvoMultisigVersion(
        ...
        bool allowed_,
        bool setDefault_
    ) external onlyOwner validAddress(avoMultisig_) isContract(avoMultisig_) {
        if (!allowed_ && setDefault_) {
            // can't be not allowed but supposed to be set as default
            revert AvoVersionsRegistry__InvalidParams();
        }
...
import { AvoWalletInitializable } from "./lib/AvoWalletInitializable.sol";
import { IAvoVersionsRegistry } from "../interfaces/IAvoVersionsRegistry.sol";
import { IAvoAuthoritiesList } from "../interfaces/IAvoAuthoritiesList.sol";
...
abstract contract AvoWalletCore is
    AvoWalletErrors,
    AvoWalletVariables,
    AvoWalletInitializable,
    AvoCore,
    AvoWalletEvents,
    IAvoWalletV3Base
{
    ...
    function _verifySig(
        ...
        if (Address.isContract(signatureParams_.signer)) {
            // signer must be owner or allowed authority
            // but no need to check for owner as owner can only be EOA
            if (authorities[signatureParams_.signer] == 1) {
            ...
            return recoveredSigner_ == owner || authorities[recoveredSigner_] == 1;
        }
    }
}
...
abstract contract AvoWalletAuthorities is AvoWalletCore {
    ...
    function addAuthorities(address[] calldata authorities_) external onlySelf {
        ...
        for (uint256 i; i < authoritiesLength_; ) {
            ...
            if (!(authorities[authorities_[i]] == 1 || authorities_[i] == owner_)) {
        ...
        (bool success_, ) = address(avoAuthoritiesList).call(
            abi.encodeCall(IAvoAuthoritiesList.syncAvoAuthorityMappings, (address(this), authorities_))
        );
        if (!success_) {
            emit ListSyncFailed();
        }
...
abstract contract AvoWalletSelfUpgradeable is AvoCoreSelfUpgradeable, AvoWalletCore {
...
abstract contract AvoWalletCast is AvoWalletCore {
    ...
    function cast(
        ...
        {
            bool validSignature_ = _verifySig(digest_, signatureParams_, params_.avoSafeNonce == -1, address(0));
        ...
        address owner_ = owner;
        {
            ...
            if (
                !(params_.actions.length > 0 &&
                    (params_.avoSafeNonce == -1 || uint256(params_.avoSafeNonce) == _avoSafeNonce))
            ) {
                revert AvoWallet__InvalidParams();
            }
        }
    ...
    AvoWalletCastAuthorized
{
    ...
    function reinitialize() public reinitializer(2) {
        // clean up last 80 bit (10 bytes) in slot 1 set to 0
        ...
        // set _transientAllowHash so refund behaviour is active
        _transientAllowHash = RESET_BYTES31;
    }
...
abstract contract AvoWalletVariablesSlot0 is AvoCoreVariablesSlot0 {
    // inherited from AvoCoreVariablesSlot0:
    // address internal _avoImplementation;
    // uint80 internal _avoSafeNonce;
    bytes2 private __slot0Placeholder;
}

abstract contract AvoWalletVariablesSlot1 {
    address public owner;

    /// @dev AvoWalletInitializable.sol variables (modified from OpenZeppelin), see ./lib folder
    /// @dev Indicates that the contract has been initialized.
    uint8 internal _initialized;
    /// @dev Indicates that the contract is in the process of being initialized.
    bool internal _initializing;
...
/// @dev contract copied from OpenZeppelin Initializable but with storage vars moved to AvoWalletVariables.sol
/// from OpenZeppelin Contracts (last updated v4.8.1) (proxy/utils/Initializable.sol)
/// see https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.8.1/contracts/proxy/utils/Initializable.sol
 ...
 * ====
 */
abstract contract AvoWalletInitializable is AvoWalletVariables {
    /**
     * @dev Indicates that the contract has been initialized.
     * @custom:oz-retyped-from bool
     */
    // uint8 private _initialized; // -> in AvoWalletVariables

    /**
     * @dev Indicates that the contract is in the process of being initialized.
     */
    // bool private _initializing; // -> in AvoWalletVariables
...
contract AvoGasEstimationsHelper {
        ...
        )
    {
        ...
        bytes memory result_;
        (success_, result_) = address(avoWallet_).call{ value: msg.value }(
        ...
        )
    {
        ...
        bytes memory result_;
        (success_, result_) = address(avoMultisig_).call{ value: msg.value }(
...
interface IAvoMultisigV3 is IAvoMultisigV3Base {
    ...
    /// @notice incrementing nonce for each valid tx executed (to ensure uniqueness)
    function avoSafeNonce() external view returns (uint256);
...
contract MockDelegateCallTargetMultisig {
    ...
    mapping(bytes32 => uint256) public nonSequentialNonces;
    uint256[49] private __gaps;
    // transitory storage slot
    bytes31 internal _transientAllowHash;
    uint8 internal _transientId;
    // storage slot 54 (varying between Multisig / AvoWallet):
...
contract MockDelegateCallTargetWallet {
    // same storage layout as VariablesV1.sol
    address internal _avoWalletImpl;
    uint80 internal _avoSafeNonce;
    bytes2 private __slot0Placeholder;
