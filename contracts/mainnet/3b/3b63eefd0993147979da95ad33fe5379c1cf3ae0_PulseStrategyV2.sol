...
contract PulseStrategyV2 is IStrategy {
    ...
    struct Storage {
        ...
        State state;
        bool currentForceRebalanceFlag;
    }
    ...
    function currentForceRebalanceFlag() public view returns (bool) {
        return _contractStorage().currentForceRebalanceFlag;
    }
    ...
    function updateMutableParams(MutableParams memory newMutableParams) external onlyOwner {
        ...
        if (
            ...
            s.currentForceRebalanceFlag = true;
        }
    ...
    function canStartAuction() external view returns (bool) {
        ...
        console2.log("Here", s.currentForceRebalanceFlag);
        if (state.forceRebalanceFlag || s.currentForceRebalanceFlag) return true;
    ...
    function canStopAuction() external view returns (bool) {
        ...
        State memory state = s.state;
        if (state.forceRebalanceFlag || s.currentForceRebalanceFlag) return false;
    ...
    function setForceRebalanceFlag() external onlyOwner {
        console2.log("Setting value to true");
        _contractStorage().currentForceRebalanceFlag = true;
    }
    ...
    function saveState() external onlyVault {
        ...
        s.state = State({
            ...
            forceRebalanceFlag: s.currentForceRebalanceFlag
        });
        s.currentForceRebalanceFlag = false;
    }
