...
contract SSXMasterChef is Operator, ReentrancyGuard {
    ...
    uint256 public SUBSCRIPTION_PERIOD = 1 weeks;
    uint256 public constant ONE_DAY_PERIOD = 1 days;
    ...
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount, uint256 subScriptionFee);
    ...
    function setSubscriptionPeriod(uint256 _newPeriod) public onlyOperator {
        SUBSCRIPTION_PERIOD = _newPeriod;
    }
    ...
    function getUserInfo(uint256 _pid, address _user) public view returns (
        uint256 _withdraw_tax,
        uint256 _subscription_rate,
        uint256 _depositTs,
        uint256 _nextTsToPaySubscription
    ) {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo memory user = userInfo[_pid][_user];
        _withdraw_tax = calcTaxRate(user.depositTs);
        uint256 _weeks_unpaid_subscription = calcUnpaidSubscriptionPeriod(user.lastSubscriptionTs);
        _subscription_rate = _weeks_unpaid_subscription.mul(pool.subscriptionRate);
        _depositTs = user.depositTs;
        _nextTsToPaySubscription = user.lastSubscriptionTs.add(SUBSCRIPTION_PERIOD);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (user.amount > 0) user.lastSubscriptionTs = block.timestamp.add(ONE_DAY_PERIOD * 30); // Start paying subscription fee after 1 month
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        uint256 _weeks_unpaid_subscription = calcUnpaidSubscriptionPeriod(user.lastSubscriptionTs);
        uint256 _subscription_fee = paySubscriptionFee(_pid, _sender);
        ...
        if (_amount > 0) {
            _amount = _amount <= user.amount ? _amount : user.amount;
            uint256 _taxAmount = payTax(_pid, _sender, _amount);
            user.amount = user.amount.sub(_amount);
            pool.token.safeTransfer(_sender, _amount.sub(_taxAmount));
            // If user withdraw everything, will reset the subscription pay timestamp
            if (user.amount == 0) user.lastSubscriptionTs = 0;
        }
        user.rewardDebt = user.amount.mul(pool.accTokenPerShare).div(1e18);
        emit Withdraw(_sender, _pid, _amount, _subscription_fee);
    }
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        // Cacl Subscription fee
        uint256 _subscription_fee = paySubscriptionFee(_pid, msg.sender);
        uint256 _amount = user.amount;

        // Cacl Taxes
        uint256 _taxAmount = payTax(_pid, msg.sender, _amount);

        user.amount = 0;
        user.rewardDebt = 0;
        pool.token.safeTransfer(msg.sender, _amount.sub(_taxAmount));
        emit EmergencyWithdraw(msg.sender, _pid, _amount);
    }

    function paySubscriptionFee(uint256 _pid, address _user) internal returns (uint256 _subscription_fee) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        ...
        if (_weeks_unpaid_subscription >= 1) {
            ...
            if (_subscription_fee >= user.amount) _subscription_fee = user.amount;
    ...
    function payTax(uint256 _pid, address _user, uint256 _amount) internal returns (uint256 _tax_amount) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        ...
        if (pool.taxRate > 0) {
            ...
            _tax_amount = _amount.mul(_taxRate).div(10000);
        }

        if (_tax_amount > 0) {
            pool.token.safeTransfer(daoFund, _tax_amount);
        }
    }

    function calcUnpaidSubscriptionPeriod(uint256 _lastSubscriptionTs) public view returns (uint256 _weeks_unpaid) {
        require(block.timestamp >= _lastSubscriptionTs, "Not paying time");
    ...
    function calcTaxRate(uint256 _depositTs) public view returns (uint256 _taxRate) {
        ...
        uint256 _days_deposited = _deposited_period.div(ONE_DAY_PERIOD);
        _taxRate = 0;
        if (MAX_WITHDRAW_TAX_RATE >= _days_deposited.mul(WITHDRAW_RATE_STEP)) {
    ...
    function getWithdrawTax(uint256 _pid, address _user) public view returns (uint256 _withdraw_tax) {
        UserInfo memory user = userInfo[_pid][_user];
        _withdraw_tax = calcTaxRate(user.depositTs);
    }

    function getPeriodUnpaidSubscription(uint256 _pid, address _user) public view returns (uint256 _unpaid_weeks) {
        UserInfo memory user = userInfo[_pid][_user];
        _unpaid_weeks = calcUnpaidSubscriptionPeriod(user.lastSubscriptionTs);
    }
    ...
    function setPoolSubscriptionRate(uint256 _pid, uint256 _new_subscription_rate) public onlyOperator {
        require(_new_subscription_rate <= MAX_SUBSCRIPTION_FEE, "Out of range");
        PoolInfo storage pool = poolInfo[_pid];
        pool.subscriptionRate = _new_subscription_rate;
    }
