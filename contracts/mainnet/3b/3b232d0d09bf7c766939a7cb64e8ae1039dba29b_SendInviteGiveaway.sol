...
contract SendInviteGiveaway {
    ...
    function sendRandomGiveaway(uint256 _amount, uint256 _length) public {
        require(contractOwner==msg.sender, "Permission Denied");
        require(_amount > 0, "ERR. AMT.");
        uint256 winnerTokenId = random(_length);
        // usdcToken.transferFrom(msg.sender, address(this), _amount);
        // usdcToken.transfer(inviteGiveawayAddr[winnerTokenId], _amount);
        usdcToken.transferFrom(msg.sender, inviteGiveawayAddr[winnerTokenId], _amount);
    }

    function sendTop10Giveaway(uint256 _amount, uint256 _length) public {
        require(contractOwner==msg.sender, "Permission Denied");
        require(_amount > 0, "ERR. AMT.");
        usdcToken.transferFrom(msg.sender, address(this), _amount*_length);
        
        for (uint i = 0; i < _length; i++) {
            usdcToken.transfer(top10GiveawayAddr[i], _amount);
            // usdcToken.transferFrom(msg.sender, address(this), _amount);
        }
    }
