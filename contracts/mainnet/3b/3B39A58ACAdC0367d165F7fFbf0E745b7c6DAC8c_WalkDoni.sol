...
contract ERC721 is ERC165, IERC721 {
    ...
    bool public checkTransfer = true;
    ...
    Counters.Counter private _totalTokensCount;
    ...
    function totalSupply() public view returns (uint256 totalSupply) {
        return _totalTokensCount.current();
    }
}
...
contract WalkDoni is ERC721, ERC721Metadata, Ownable {
    string nft_name = "Test";
    string nft_symbol = "Test";
    string public _buri;
    ...
    constructor(uint256 _tokenPrice, uint256 _selectPrice) public ERC721() {
    ...
    function _queryERC20Balance(address _tokenAddress, address _addressToQuery) internal view returns (uint256) {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (revealed[tokenId]) {
            return bytes(_buri).length > 0 ? string(abi.encodePacked(_buri, Strings.fromUint256(tokenId), baseExtension)) : "";
        } else {
            return bytes(_notBuri).length > 0 ? string(abi.encodePacked(_notBuri, Strings.fromUint256(tokenId), baseExtension)) : "";
        }
    }
    ...
    function mint(address _user, uint256 tokenId) external payable {
        ...
        require(tokenId <= mintingTotalSupply + 10000);
        if (msg.sender != owner) {
            require(msg.value >= tokenPrice);
            _safeNativeTransfer(dev, tokenPrice);
            // TransferHelper.safeTransferFrom(WETH, msg.sender, owner, tokenPrice * amount);
        }
    ...
    function ownerSelectMint(address[] calldata _user, uint256[] calldata tokenId) external {
        require(msg.sender == dev || msg.sender == owner);
        uint256 mintCount = mintingSupplyCount;
        require(mintingTotalSupply >= mintCount + _user.length);
        for (uint256 i = 0; i < _user.length; i++) {
            require(tokenId[i] <= mintingTotalSupply + 10000);
            _safeMint(_user[i], tokenId[i]);
            mintCount += 1;
        }
        mintingSupplyCount = mintCount;
    }
