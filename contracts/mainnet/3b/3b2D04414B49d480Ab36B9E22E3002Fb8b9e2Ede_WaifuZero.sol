...
contract WaifuZero is Ownable, ERC721, NonblockingReceiver {
    using Strings for uint256;
    ...
    event Transfered(uint16 chainId, address receiver, uint256 tokenId);
    ...
    function mint(uint8 _amount) external payable {
        ...
        require(msg.value >= cost * _amount, "ERROR: Insufficient funds");
        require(
            nextTokenId + _amount <= endTokenId,
            "ERROR: Mint exceeds supply"
        );
        _safeMint(msg.sender, ++nextTokenId);
        _owners[nextTokenId] = msg.sender;
        if (_amount == 2) {
            _safeMint(msg.sender, ++nextTokenId);
            _owners[nextTokenId] = msg.sender;
        }
    ...
    function mintForAddress(uint8 _amount, address _receiver) external onlyOwner {
        ...
        _owners[nextTokenId] = _receiver;
        if (_amount == 2) {
            _safeMint(_receiver, ++nextTokenId);
            _owners[nextTokenId] = _receiver;
        }
        emit Minted(_amount, _receiver);
    }
        ...
        returns (uint256[] memory)
    {
        ...
        uint256 currentTokenId = startTokenId + 1;
        uint256 ownedTokenIndex = 0;
        while (
            ownedTokenIndex < ownerTokenCount && currentTokenId <= endTokenId
        ) {
            if (_owners[currentTokenId] == owner_) {
    ...
    function traverseChains(uint16 _chainId, uint256 tokenId) public payable {
        ...
        emit Transfered(_chainId, msg.sender, tokenId);
    }
    ...
    function setApprovalForAll(address, bool) public override onlyOwner {}
