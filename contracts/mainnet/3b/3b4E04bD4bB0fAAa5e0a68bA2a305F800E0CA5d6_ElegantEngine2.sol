...
contract ElegantEngine2 is Ownable, Concatenate {
    ...
    function fix2(address _from) public onlyOwner {
        EngineInterface _old = EngineInterface(_from);
        uint i = 0;
        for (i; i < 20; i ++){
            (uint _size,
            ...
            uint _check,
            uint _mat,
            uint _prl) = _old.rulesByRace(i);

            createEvent(i,_size,_con,_check,_mat,_prl);
        }
    }

    function fix3(address _from) public onlyOwner{
        EngineInterface _old = EngineInterface(_from);
        StakingInterface _staking = StakingInterface(stakingAddress);
        address[] memory _list = _staking.walletList();
        uint i = 0;
        for (i; i < _list.length; i++){
            address temp = _list[i];
            walletToVC[temp] = _old.walletToVC(temp);
            walletToCoach[temp] = _old.walletToCoach(temp);
            walletToAnchor[temp] = _old.walletToAnchor(temp);
        }
    }
