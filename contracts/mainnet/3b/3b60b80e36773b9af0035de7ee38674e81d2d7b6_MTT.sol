...
contract MTT is BEP20Detailed, Ownable {
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 2e2 * 10**DECIMALS;
    ...
    uint256 public maxReflectionFee = 100;
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(treasuryFee.add(reflectionFee))
        );
    ...
    event TreasuryReceiver(uint256 amount);
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndHIF.mul(treasuryFee).div(
                treasuryFee.add(reflectionFee)
            ),
            gas: 30000
        }("");

        try distributor.deposit{value: amountETHToTreasuryAndHIF.mul(reflectionFee).div(
                treasuryFee.add(reflectionFee)
            )}() {} catch {}

        if(success) { 
            TreasuryReceiver(amountETHToTreasuryAndHIF.mul(treasuryFee).div(
                treasuryFee.add(reflectionFee)
            ));
        }
    ...
    function totalFee() public view returns (uint256) {
        return liquidityFee.add(treasuryFee).add(reflectionFee).add(blackHoleFee);
    }
    ...
    function setReflectionFee(uint256 _reflectionFee) external onlyOwner {
        require(_reflectionFee <= maxReflectionFee, "Over max fee");
