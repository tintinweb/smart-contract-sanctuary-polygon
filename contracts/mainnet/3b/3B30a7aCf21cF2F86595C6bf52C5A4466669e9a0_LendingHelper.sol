...
contract LendingHelper {
    ...
    function triggerLend() public view returns (bool) {
        ...
        if (currentCollatRatio != 0 && currentCollatRatio < 5250) return true;
        if (currentCollatRatio > 5520) return true; // TODO: make it private
    ...
    function _rebalanceLoan() internal {
        ...
        if (currColRatio < targetCollatRatio) {
            if (minRatio < targetCollatRatio.sub(currColRatio)) {
                _borrow();
            }
        } else if (minRatio < currColRatio.sub(targetCollatRatio)) {
            (uint256 deposits, uint256 borrows) = getCurrentPositionInWant();
