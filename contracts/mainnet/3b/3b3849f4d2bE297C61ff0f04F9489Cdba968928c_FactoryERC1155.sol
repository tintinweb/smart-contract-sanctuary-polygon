...
contract FactoryERC1155 {
    ...
    */
    function deployERC1155(string memory _contractName, string memory _uri) public returns (address) {
        uint[504] memory _ids;
        string[504] memory _names;

        for (uint i = 1; i < 505; i++) {
            _ids[i] = i;
            _names[i] = "Hello Worlds NFT";
        }
...
contract ERC1155Token is ERC1155, Ownable {
    ...
    */
    constructor(string memory _contractName, string memory _uri, string[504] memory _names, uint[504] memory _ids) ERC1155(_uri) {
        names = _names;
        ids = _ids;
        createMapping();
        setURI(_uri);
