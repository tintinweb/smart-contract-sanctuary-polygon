...
contract Whitelist is Initializable, OwnableUpgradeable, IWhitelist {
    ...
    function initialize(address couponSigner_) external initializer {
        __Ownable_init();
    ...
    function setCouponSigner(address couponSigner_) external onlyOwner {
        require(couponSigner_ != address(0), "couponSigner is zero");
        couponSigner = couponSigner_;
    }
    ...
    function isVerifiedCoupon(address sender, Coupon memory coupon)
        ...
        returns (bool)
    {
        if (couponSigner == address(0)) {
            return true;
        }

        return _isVerifiedCoupon(_createMessageDigest(sender), coupon);
    }
