...
contract MerkleAirdrop is Ownable, Pausable, ReentrancyGuard, EIP712MetaTransaction {
    using SafeERC20 for IERC20;

    /// @dev address of project token holder from which transfer to recipient when claiming tokens
    address immutable public reserveAddress;

    /// @dev The root of the claim merkle balance tree
    bytes32 immutable public merkleRoot;

    /// @dev address of project token
     ...
     * @param _merkleRoot The root of the claims balance tree
     * @param _token Airdrop token address
     */
    constructor(
        ...
        require(_reserveAddress != address(0), "MerkleAirdrop: invalid reserve address");
        require(_token != address(0), "MerkleAirdrop: invalid token address");
        require(_merkleRoot != bytes32(0), "MerkleAirdrop: invalid _merkleRoot");
     ...
     * @param _index index of the claim in the balance tree
     * @param _recipient token recipient
     * @param _amount token amount
     * @param _merkleProof The proof of the existing of the claim in the balance tree
     */
    function claimTokens(
        ...
        require(_amount > 0, "MerkleAirdrop: invalid amount");
        require(!claimed[_recipient], "MerkleAirdrop: token already claimed to recipient");
        require(_recipient == msgSender(), "MerkleAirdrop: only recipient can initiate their own claim");

        claimed[_recipient] = true;

        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(_index, _recipient, _amount));
        require(MerkleProof.verify(_merkleProof, merkleRoot, node), 'MerkleAirdrop: Invalid proof provided');

        // transfer token from reserve address to recipient
        token.safeTransferFrom(reserveAddress, _recipient, _amount);

        emit TokenClaimed(msgSender(), _recipient, _amount);
    }
