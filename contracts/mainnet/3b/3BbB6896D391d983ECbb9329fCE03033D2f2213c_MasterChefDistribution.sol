...
contract MasterChefDistribution is Ownable, ReentrancyGuard {
	...
	modifier onlyVault() {
		require(approvedVaults[msg.sender], "ERR: Only approved eVAULTs");
		_;
	}
	...
	function setPoolId(address _vault, uint256 _id) public onlyOwner {
		require(approvedVaults[_vault], "ERR: Invalid vault");
		vaultToPoolId[_vault] = _id;
	}

	function updateVaultAddresses(address _vaultAddress, bool _status)
		public
		onlyOwner
	{
		require(_vaultAddress != address(0), "ERR: Invalid vault");
	...
	function getBoosts(address userAddress) public view returns (uint256) {
		VoteEscrow voteEscrow = VoteEscrow(veToken);
	...
	function updatePool(uint256 _pid) public {
		...
		// uint256 lpSupply = pool.vaultAddress.balanceOf(address(this));
		uint256 lpSupply = pool.vaultAddress.totalSupply();
	...
	function stake(address userAddress, uint256 _amount)
		public
		nonReentrant
		onlyVault
	{
		...
		if (user.amount > 0) {
			uint256 pendingAmount = (user.amount * (pool.accERC20PerShare)) /
				(1e12) -
				(user.rewardDebt);
			pendingAmount = (getBoosts(userAddress) * pendingAmount) / 1e12;
	...
	function withdraw(address userAddress, uint256 _amount)
		public
		nonReentrant
		onlyVault
	{
...
contract VoteEscrow is ERC20Votes, ReentrancyGuard, Ownable {
	...
	function withdraw() external nonReentrant {
		 ...
		 */
		IMultiFeeDistribution(multiFeeDistribution).stake(
	...
	function voting_power_locked_days(uint256 _value, uint256 _days) pure
		public
		returns (uint256)
	{
		if (_days >= MAXDAYS) {
