...
// Interface needed on the receiver contract
...
contract UserModule is Variables, Events {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    modifier nonReentrant() {
        require(_status != 2, "P2:M2:ReentrancyGuard: reentrant call");
        _status = 2;
        _;
        _status = 1;
    }
    ...
    /**
    * @dev Function to get flashloan.
    * @param tokens_ array of tokens addresses.
    * @param amts_ array of tokens amount.
    * @param data_ encoded bytes data.
    */
    function flashborrow(address[] memory tokens_, uint[] memory amts_, bytes calldata data_) nonReentrant public {
        require(tokens_.length == amts_.length, "P2:M2: not-equal-length");
        uint length_ = tokens_.length;
        IERC20[] memory tokenContracts = new IERC20[](length_);
        uint[] memory premiums_ = new uint[](length_);
        uint[] memory initialBals_ = new uint[](length_);
        uint[] memory finalBals_ = new uint[](length_);
        for (uint i = 0; i < length_; i++) {
            for (uint j = i + 1; j < length_; j++) {
                require(tokens_[i] != tokens_[j], "P2:M2: same-token-repeat");
            }
            tokenContracts[i] = IERC20(tokens_[i]);
            initialBals_[i] = tokenContracts[i].balanceOf(address(this));
            liquidity.borrow(tokens_[i], amts_[i]);
            tokenContracts[i].safeTransfer(msg.sender, amts_[i]);
            premiums_[i] = amts_[i].mul(_fee).div(1e4);
        }
        IFlashloanReceiver(msg.sender).executeOperation(tokens_, amts_, premiums_, msg.sender, data_);
        for (uint i = 0; i < length_; i++) {
            approve(TokenInterface(tokens_[i]), address(liquidity), amts_[i]);
            liquidity.payback(tokens_[i], amts_[i]);
            finalBals_[i] = tokenContracts[i].balanceOf(address(this));
            require(finalBals_[i] >= initialBals_[i].add(premiums_[i]), "P2:M2: fee-not-paid-fully");
            require(liquidity.protocolRawBorrow(address(this), tokens_[i]) == 0, "P2:M2: protocol-not-fully-paid");
        }
        emit flashBorrowLog(tokens_, amts_, premiums_, msg.sender);
    }
