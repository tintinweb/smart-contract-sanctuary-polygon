...
// File: contracts\interfaces\ISolidlyGauge.sol
...
// File: contracts\StakedLPTokenSolidlyBase1.sol

pragma solidity =0.5.16;
contract StakedLPTokenSolidlyBase1 is PoolToken {
	using SafeToken for address;
	
	bool public constant isStakedLPToken = true;
	string public constant stakedLPTokenType = "SolidlyBase1";
...
// File: contracts\StakedLPTokenFactorySolidlyBase1.sol

pragma solidity =0.5.16;
contract StakedLPTokenFactorySolidlyBase1 {
	address public router;
	...
	function createStakedLPToken(address pair) external returns (address stakedLPToken) {
		...
		bytes memory bytecode = type(StakedLPTokenSolidlyBase1).creationCode;
		assembly {
			stakedLPToken := create2(0, add(bytecode, 32), mload(bytecode), pair)
		}
		StakedLPTokenSolidlyBase1(stakedLPToken)._initialize(pair, token0, token1, router, voter, rewardsToken, bridgeTokens);
