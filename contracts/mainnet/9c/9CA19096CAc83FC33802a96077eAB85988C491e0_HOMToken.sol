        ...
        if (sender == owner() || recipient == owner() || isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            ...
            if(reserves >= 20)
            {
                ...
                if(automatedMarketMakerPairs[recipient] && sellFee > 0) {
                    ...
                    uint256 tokenamount = balanceOf(sender);  // token amount in sender
                    if(selltime[sender] == 0)
                    {
                        require(amount < (tokenamount / 20), "You can sell the 5% of your tokens only"); 
                        selltime[sender] = block.timestamp;
                    }else{
                        if(block.timestamp < selltime[sender] + (86400 * 30))  //in 30 days
                        {
                            if(sellamount[sender] == 0)
                            {
                                sellamount[sender] = amount ;
                                require( amount < ( tokenamount / 20 ), "You can sell the 5% of your tokens in 30 days" );
                            }else{
                                require(( sellamount[sender] + amount) < ( tokenamount / 20 ), "You can sell the 5% of your tokens in 30 days" );
                                sellamount[sender] = sellamount[sender] + amount ;
                            }
