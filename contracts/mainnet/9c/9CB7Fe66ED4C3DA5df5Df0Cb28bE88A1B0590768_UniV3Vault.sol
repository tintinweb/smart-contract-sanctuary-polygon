...
contract UniV3Vault is Vault, IUniV3Vault {
  ...
  function initialize(
    ...
    super.initialize(_vaultId, _duration, _adapter, _srCapacity, _jrCapacity, _jrAsset, _feeBps, _feeReceiver);
  }
  ...
  function deposit(
    ...
    if (tranche == JUNIOR) {
      ...
      // Add liquidty to Uniswap V3 and mint claim token
  ...
  function withdraw(uint256 tranche, bytes calldata removeLiquidityData) public override isInitialized nonReentrant {
    require(tranche == SENIOR || tranche == JUNIOR, "T");

    IUniV3Adapter adapter = IUniV3Adapter(address(adapter));

    if (!isStarted && tranche == SENIOR) {
      // Early withdrawal - Senior tranche
      ...
      (uint256 amount0, uint256 amount1) = adapter.earlyReturnCapital(msg.sender, tranche, removeLiquidityData);

      logFundsWithdrawn(SENIOR, amount0, amount1, true);
      return;
    }

    if (!isStarted && tranche == JUNIOR) {
      // Early withdrawal - Junior tranche
      ...
      logFundsWithdrawn(JUNIOR, amount, true);
      return;
    }
    ...
    if (tranche == SENIOR) {
      // Normal withdrawal - Senior tranche
      ...
      (amount0, amount1) = adapter.removeLiquidity(msg.sender, removeLiquidityData);
      adapter.returnCapital(msg.sender, amount0, amount1, tranche);
      srBearerToken.burn(address(msg.sender), bearerBalance);

      logFundsWithdrawn(SENIOR, amount0, amount1, false);
      return;
    }

    if (tranche == JUNIOR) {
      // Normal withdrawal - Junior tranche

      require(removeLiquidityData.length == 0, "OEI");

      // Caller must be a junior tranche depositor or feeReceiver
      ...
      logFundsWithdrawn(JUNIOR, amount0, amount1, false);
      return;
    }
  }

  /// @dev Helper function for logging an array with length 1
  function logFundsWithdrawn(uint256 tranche, uint256 amount0, bool isEarly) internal {
    uint256[] memory amounts = new uint256[](1);
    amounts[0] = amount0;
  ...
  function logFundsWithdrawn(uint256 tranche, uint256 amount0, uint256 amount1, bool isEarly) internal {
    uint256[] memory amounts = new uint256[](2);
    amounts[0] = amount0;
    amounts[1] = amount1;
    emit FundsWithdrawn(amounts, tranche, msg.sender, isEarly);
...
/// @author psykeeper, supafreq, everywherebagel, maze, rx
/// @notice Foundational contract for building vaults, which coordinate user deposits and tranche earnings
...
abstract contract Vault is IVault, ReentrancyGuard {
  ...
  /// @notice ERC20 bearer token that entitles owner to a portion of the senior tranche deposits
  ...
  /// @notice ERC20 bearer token that entitles owner to a portion of the senior tranche bearer tokens and the junior tranche premium payment
  /// @dev If the vault hasn't started, this is used to return the senior deposit
  ...
  function initialize(
    ...
    require(_jrCapacity != 0, "NEI");
    require(_srCapacity != 0, "NEI");
    require(_jrAsset != address(0), "NEI");
    ...
    srBearerToken = new TrancheBearerToken("Saffron Vault Senior Bearer Token", "SAFF-BTS");
    jrBearerToken = new TrancheBearerToken("Saffron Vault Junior Bearer Token", "SAFF-BTJ");
  ...
  function claim() public virtual isInitialized nonReentrant {
    ...
    // Send a proportional share of the total junior deposits (premium) to the senior depositor
...
/// @author Remco Bloemen - https://2π.com/21/muldiv/index.html - MIT License
