...
import {IGauge, IGUniRouterStaking} from "./interfaces/IGUniRouterStaking.sol";
    ...
    OwnableUpgradeable
{
    ...
    {
        return _addLiquidity(pool, amount0Max, amount1Max, amount0Min, amount1Min, receiver);
    }

    /// @notice addLiquidityAndStake stakes G-UNI 
    /// @param pool address of G-UNI pool to add liquidity to
    /// @param amount0Max the maximum amount of token0 msg.sender willing to input
    /// @param amount1Max the maximum amount of token1 msg.sender willing to input
    /// @param amount0Min the minimum amount of token0 actually input (slippage protection)
        ...
        )
    {
        (amount0, amount1, mintAmount) =
            _addLiquidity(pool, amount0Max, amount1Max, amount0Min, amount1Min, address(this));

        IERC20(address(pool)).safeIncreaseAllowance(address(gauge), mintAmount);
...
import {IGUniPool, IGUniRouterBase} from "./IGUniRouterBase.sol";
import {IGauge} from "./IGauge.sol";

interface IGUniRouterStaking is IGUniRouterBase {
    function addLiquidityAndStake(
        IGUniPool pool,
        IGauge gauge,
        uint256 amount0Max,
