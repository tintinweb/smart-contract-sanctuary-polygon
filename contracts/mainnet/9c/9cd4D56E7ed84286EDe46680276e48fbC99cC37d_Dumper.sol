...
contract Dumper is Ownable {
    ...
    function simulateUsdc(uint256 _slippage) external view returns (uint256) {
        require(_slippage >= 50, "Swapper: Slippage too low");
        ...
        // Swap Alchs for GHST
        uint256 fudGhst = _getAmountOutMin(fudBal, fud, ghst, _slippage);
        uint256 fomoGhst = _getAmountOutMin(fomoBal, fomo, ghst, _slippage);
        uint256 alphaGhst = _getAmountOutMin(alphaBal, alpha, ghst, _slippage);
        uint256 kekGhst = _getAmountOutMin(kekBal, kek, ghst, _slippage);

        uint256 ghstReceived = fudGhst + fomoGhst + alphaGhst + kekGhst;
