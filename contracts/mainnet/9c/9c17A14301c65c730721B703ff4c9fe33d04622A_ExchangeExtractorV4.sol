...
contract ExchangeExtractorV4 {
        ...
        )
    {
        ...
        address[3][] memory pairs_tokens = new address[3][](_qty);
        uint8[2][] memory pairs_digits = new uint8[2][](_qty);
    ...
    function extractPair(IUniswapV2Factory factory, uint256 index)
        private
        view
        returns (
            address[3] memory,
    ...
    function getDecimals(IERC20 token) private view returns (uint8) {
        ...
        if (success) {
            uint8 decimals;
            assembly {
                decimals := mload(add(res, add(0x20, 0)))
            }
            return decimals;
        } else {
            return 0;
    ...
    function estimateSwap(
        ...
        for (uint256 i = 0; i < routers.length; i++) {
            ...
            amounts = abi.decode(res, (uint256[]));
            currentAmountIn = amounts[paths[i].length - 1];
        }
        return currentAmountIn;
    }

    function estimateSwaps(
        address[][] calldata routers,
        address[][][] calldata paths,
        uint256[] calldata amountsIn
    ) external view returns (uint256[] memory) {
        uint256[] memory results = new uint256[](routers.length);
        for (uint256 i = 0; i < routers.length; i++) {
            results[i] = estimateSwap(routers[i], paths[i], amountsIn[i]);
        }
        return results;
    }
    ...
    function arbitrages(
        ...
        for (uint256 i = 0; i < routers.length; i++) {
            arbitrage(routers[i], paths[i], amountIn, deadline);
        }
        return true;
    }
}
