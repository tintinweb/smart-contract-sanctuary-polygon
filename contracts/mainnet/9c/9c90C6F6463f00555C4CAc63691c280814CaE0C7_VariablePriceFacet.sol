...
contract VariablePriceFacet is Modifiers {
    ...
    function _updatePrice() internal returns (uint256 _price, uint256 updatedPrice) {
        VariablePriceContract storage ds = s().variablePrices;
        (_price, updatedPrice) = ds._updatePrice();
    }
    function _currentPrice() internal view returns (uint256 _price) {
        VariablePriceContract storage ds = s().variablePrices;
        _price = ds._currentPrice();
    }
    function currentPrice() external view returns (uint256 _price) {
        _price = _currentPrice();
    }
}
...
struct Claim {
    // claim id.
    uint256 id;
    address feeRecipient; // pool address
    address mineAddress; // the amount of eth deposited
    uint256 depositAmount; // the gem quantity to mint to the user upon maturity
    uint256 mintQuantity; // the deposit length of time, in seconds
    uint256 depositLength; // the block number when this record was created.
    uint256 createdTime; // the block number when this record was created.
    uint256 createdBlock; // block number when the claim was submitted or 0 if unclaimed
    uint256 claimedBlock; // the fee that was paid
    uint256 feePaid; // whether this claim has been collected
    bool collected; // whether this claim must be mature before it can be collected
    bool requireMature; // whether this claim has been collected
    bool mature;
}
...
interface ITokenAttributeActor {

    function getContractAttributes() external view returns (string[] memory, string[] memory);

    function getTokenAttributes(
        uint256 tokenId
    ) external view returns (string[] memory, string[] memory);

    function getContractReplacements() external view returns (Replacement[] memory);

    function getTokenReplacements(
        uint256 tokenId
    ) external view returns (Replacement[] memory);

    function initTokenAttributes(
        uint256 tokenId
    ) external;
...
import "../interfaces/ITokenAttributeActor.sol";
...
library BitGemLib {
    ...
    function _mintClaim(
        ...
        tokenId = ERC721ALib.currentIndex(erc721a) - 1;
    ...
    function _mintBitGem(
        ...
        ERC721ALib._mint(erc721a, address(tokenAddress), to, 1, "", true);
        tokenId = ERC721ALib.currentIndex(erc721a) - 1;
...
library ClaimLibrary {
    ...
    // get the price of a bitgem claim for a given time period, given the bitgem's min and max claim lengths and the cost of a
    // minclaimlength claim. price and time are linearly inversely related to each other, with price denoting price at mintime.
    // For example, given a mintime of 1 day, a max time of 3 days, and a price of 3 ether, the price at 3 days is 1 ether, the
    // price at 1 days is 3 ether, and the price at 2 days is 2 ether.
    function getClaimPrice(
        ...
        uint256 price = (minLengthPrice * minLength) / targetLength;
        if (price < minLengthPrice / maxLength) {
            price = minLengthPrice / maxLength;
        } else if (price > minLengthPrice) {
            price = minLengthPrice;
        }
        return price;
    }
    ...
    function getClaimTime(
        ...
        uint256 time = (minLength * minLengthPrice) / targetPrice;
        if (time < minLength) {
            time = minLength;
    ...
    function createClaim(
        ...
        uint256 requireMatureTimeBonus = bg.settings.maxTime;
        ...
        require(
            claimTime <= bg.settings.maxTime + requireMatureTimeBonus,
            "Claim time is too long"
        );

        // compute the claim price given the claim time
        uint256 claimPrice = getClaimPrice(
            bg.settings.minTime,
            bg.settings.maxTime + requireMatureTimeBonus,
            vs.price,
            claimTime
        );
        ...
        uint256 matureRefundBonus = 0;
        if (requireMature) {
            matureRefundBonus = (claimPrice / 10);
        ...
        // refund oversend
        if (
            msg.value > claimPrice + matureRefundBonus &&
            msg.value > 1 ether / 100
        ) {
            (bool success, ) = payable(msg.sender).call{
                value: msg.value - (claimPrice + matureRefundBonus)
            }("");
            require(success, "Failed to refund extra payment");
        }
    ...
    function collectClaim(
        ...
        console.log("isMature", isMature);
        console.log("requireMature", claim.requireMature);
        ...
        _burn = handlePostClaim(vs, bg, erc721a, claim.mature, claim.id);
    }
    ...
    function refundClaim(
        ...
        // transfer the ETH principal to caller
        (bool sent, ) = payable(recipient).call{value: claim.depositAmount}("");
        require(sent, "Failed to send Ether");

        claim.feePaid = 0; // update the claim with the fee paid
        claim.mature = true; // update the claim with the maturity
        claim.collected = true;
    }
    ...
    function handlePostClaim(
        ...
        if (isMature) {
            ...
            LibAppStorage.diamondStorage().attributes._burn(claimId);
            _burn = true;
        }
    }
}
...
struct AppStorage {
    ...
    SaltStorage salts;
    ERC721AContract erc721Contracts;
    AttributeContract attributes;
    MetadataContract metadata;
    ERC721EnumerableContract enumerations;
    FlashLenderContract flashLenders;
    ClaimContract claims;
    VariablePriceContract variablePrices;
    FeeManagerContract fees;
    BitGemContract bitgems;
    MultiPartContract multiParts;
...
library LibAppStorage {
    bytes32 internal constant DIAMOND_STORAGE_POSITION = keccak256("diamond.bitgem.app.storage");
