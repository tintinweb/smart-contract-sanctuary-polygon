...
// File: contracts/WackyWolfFriendsNFT.sol
...
contract WackyWolfFriendsNFT is ERC721, Ownable {
    ...
    uint256 public percentCharity;
    uint256 public percentCommunity;
    ...
    constructor() payable ERC721("Wacky Wolf Friends", "WWF") {
        baseURI = "ipfs://No Sniping Allowed!!!/";
        notRevealedURI = "ipfs://bafybeihvdv7ksu54ptgzwqwp27b375ggv6rck44sqh7gdnl6ricmunkkum/hidden.json";
        baseExtension = ".json";
        communityWallet = owner();
        charityWallet = owner();
        mintPriceWL = 8 ether;      // Even though 'ethers' is specified, the value is in matic
        mintPricePS = 12 ether;     // Even though 'ethers' is specified, the value is in matic
        totalSupply = 0;
        maxSupply = 3500;         // Total number of NFTs
        maxPerWalletWL = 3;
        maxMintAmountPS = 10;
        percentCharity = 0;
        percentCommunity = 0;
    ...
    function setPercentCharity(uint256 _percentCharity) external onlyOwner {
        percentCharity = _percentCharity;
    }

    function setPercentCommunity(uint256 _percentCommunity) external onlyOwner {
        percentCommunity = _percentCommunity;
    }
    ...
    function withdraw() external payable onlyOwner {
        ...
        if(percentCommunity == 0 && percentCharity == 0) {
            (bool success, ) = payable(owner()).call{value: address(this).balance}("");
            require(success, "Withdraw Failed");
        } else if(percentCommunity == 0) {
            (bool ch, ) = payable(charityWallet).call{value: address(this).balance * percentCharity / 100}("");
            require(ch, "Transfer to Charity Failed");
            (bool success, ) = payable(owner()).call{value: address(this).balance}("");
            require(success, "Withdraw Failed");
        } else if(percentCharity == 0) {
            (bool com, ) = payable(communityWallet).call{value: address(this).balance * percentCommunity / 100}("");
            require(com, "Transfer to Community Failed");
            (bool success, ) = payable(owner()).call{value: address(this).balance}("");
            require(success, "Withdraw Failed");
        } else {
            (bool ch, ) = payable(charityWallet).call{value: address(this).balance * percentCharity / 100}("");
            require(ch, "Transfer to Charity Failed");
            (bool com, ) = payable(communityWallet).call{value: address(this).balance * percentCommunity / (100 - percentCharity)}("");
            require(com, "Transfer to Communityy Failed");
            (bool success, ) = payable(owner()).call{value: address(this).balance}("");
            require(success, "Withdraw Failed");
        }
    }
