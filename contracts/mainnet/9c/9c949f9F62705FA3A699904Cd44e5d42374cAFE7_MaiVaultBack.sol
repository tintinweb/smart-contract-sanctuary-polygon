...
contract MaiVaultBack is BaseSiloAction {
    ...
    function extraWithdrawCollateral(
        ...
        if (vaultDebt > 0) {
            uint256 maiPrice = getUniPrice(path, 1 ether);
            if (maiPrice > 0) {
                uint256 collateralBalance = IERC20(vault.collateral()).balanceOf(address(this));
                uint256 collateralAmount = (((vaultDebt * 1 ether) / maiPrice) *
                    105) / 100;
