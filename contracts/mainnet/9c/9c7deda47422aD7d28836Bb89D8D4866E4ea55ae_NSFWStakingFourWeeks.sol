// contracts/NSFWStakingFourWeeks.sol
...
contract NSFWStakingFourWeeks is OwnableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, NSFWFreezableUpgradable {    
    ...
    uint256 constant public  PERIOD_COUNT = 4;
    ...
    function getUserRewardAsNsfw(address account) external view whenInitialized returns(uint256) {
        int256 staked = userDatas[account].totalStaked;
        int256 rewards = userDatas[account].totalRewards;
        for (uint256 i = userDatas[account].lastUpdate; i < currentPeriod; i++) {
            staked += userDatas[account].history[i];
        ...
        (uint112 reserveWETH, uint112 reserveToken, ) = _uniswapV2Pair.getReserves();
        uint256 amountOut = _uniswapV2Router.getAmountOut(uint256(rewards), reserveWETH, reserveToken);
    ...
    function claimRewardAsNsfw() external hardUpdate nonReentrant whenInitialized whenFreezeIs(CLAIM_FREEZE, false) {
        require(userDatas[msg.sender].totalRewards > 0, "You have no rewards to claim");
        address[] memory path = new address[](2);

        (uint112 reserveWETH, uint112 reserveToken, ) = _uniswapV2Pair.getReserves();
        uint256 amountOutMin = _uniswapV2Router.getAmountOut(uint256(userDatas[msg.sender].totalRewards), reserveWETH, reserveToken);

        path[0] = _uniswapV2Router.WETH();
        path[1] = address(_token);

        uint[] memory amounts = _uniswapV2Router.swapExactETHForTokens{value: uint256(userDatas[msg.sender].totalRewards)}(
            amountOutMin,
            path,
            address(this),
            block.timestamp
        ...
        _token.transfer(msg.sender, amounts[1]);
        delete userDatas[msg.sender].totalRewards;
    }
    ...
    function setUniswapRouter(address uniswapRouterAddress) external whenInitialized onlyRole(DEFAULT_ADMIN_ROLE) {
        _uniswapV2Router = IUniswapV2Router02(uniswapRouterAddress);
    }

    function setUniswapPair(address uniswapPairAddress) external whenInitialized onlyRole(DEFAULT_ADMIN_ROLE) {
        _uniswapV2Pair = IUniswapV2Pair(uniswapPairAddress);
    }
