...
contract DataNFT is ERC721URIStorage {
	...
	modifier isOwner(){
        require(contractOwner == msg.sender, "Only owner can mint new NFTs.");
        _;			
	}

    function mint(address recipient, uint16 _level) public isOwner {
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        _safeMint(recipient, newItemId);
        tokenIdToAtr[newItemId] = Atrributes(_level, 10, 20, 100, "");
    ...
    function getSpeed(uint256 tokenId) public view returns (string memory) {
        uint256 speed = tokenIdToAtr[tokenId].Speed;
        return speed.toString();
    }

    function getStrength(uint256 tokenId) public view returns (string memory) {
        uint256 strength = tokenIdToAtr[tokenId].Strength;
        return strength.toString();
    }

    function getLife(uint256 tokenId) public view returns (string memory) {
        uint256 life = tokenIdToAtr[tokenId].Life;
        return life.toString();
    }

    function getMac(uint256 tokenId) public view returns (string memory) {
    ...
    function train(uint256 tokenId) public {
        ...
        require(atr1.Level < MAX_LEVEL, "You are already the strongest man!");
        atr1.Level += 1;
        atr1.Speed += uint16(random(10));
        atr1.Strength += uint16(random(20));
        atr1.Life += uint16(random(100));
        _setTokenURI(tokenId, getTokenURI(tokenId));
    }

    function mac(uint256 tokenId, string memory _macAddress) public {
        require(_exists(tokenId), "Please use an existing token");
        require(
            ownerOf(tokenId) == msg.sender,
            "You must own this token to train it"
