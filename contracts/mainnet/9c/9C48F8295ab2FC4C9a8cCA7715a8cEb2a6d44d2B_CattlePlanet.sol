...
contract CattlePlanet is OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    mapping(address => uint) public rentReward;
    ...
    function pullOutPlanetCard(uint tokenId) external onlyOriginalOwner(tokenId) {
        require(rentInfo[tokenId].rentOwner == address(0), 'this planet is renting');
    ...
    function setRentInfo(uint tokenId, address rentOwner, uint rentAmount) external {
        ...
        require(planetInfo[tokenId].owner != rentOwner, 'can not rent your own planet');
        require(rentId[rentOwner] == 0 && rentId[msg.sender] == 0, 'rented');
        require(msg.sender == planetInfo[tokenId].owner, 'not the owner');
        require(rentAmount > 0, 'must > 0');
        require(userInfo[rentOwner].planet == 0 || userInfo[rentOwner].planet == tokenId, 'renter in other planet');
        if (userInfo[rentOwner].planet == 0) {
            require(planetInfo[tokenId].population < planetInfo[tokenId].populationLimit, 'out of population limit');
            planetInfo[tokenId].population ++;
            userInfo[rentOwner].planet = tokenId;
            emit BondPlanet(rentOwner, tokenId);
        }
    ...
    function payRent() external {
        ...
        rentReward[rentInfo[tokenId].owner] += rentInfo[tokenId].rentAmount;
    ...
    function checkRentInfo(address addr) external view returns (uint tokenId, address owner, address rentOwner, uint rentAmount, uint rentTime, uint reward){
        tokenId = rentId[addr];
        if (tokenId == 0) {
            tokenId = ownerOfPlanet[addr];
        }
        owner = rentInfo[tokenId].owner;
        rentOwner = rentInfo[tokenId].rentOwner;
        rentAmount = rentInfo[tokenId].rentAmount;
        rentTime = rentInfo[tokenId].rentTime;
        reward = rentReward[addr];
    }
