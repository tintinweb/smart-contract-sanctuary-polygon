...
library Address {
    ...
    function verifyCallResultFromTarget(
        ...
        string memory errorMessage
    ) internal view returns (bytes memory _para) {
        if (success) {
    ...
    function verifyCallResult(
        ...
        bytes memory returndata,
        string memory errorMessage
    ) internal pure returns (bytes memory _para) {
        if (success) {
            return returndata;
        } else {
            _revert(returndata, errorMessage);
...
contract BJPC is ERC721, Ownable {
    ...
    uint256 public pCost = 569.08 ether;
    uint256 public wCost = 295.01 ether;
    ...
    constructor() ERC721("Bored Jeffrey President Club", "BJPC") {}
    ...
    function airDrop(address _adr, uint256 _mintAmount) public mintCompliance(_mintAmount) onlyOwner {
    ...
    function setPCost(uint256 _pCost) public onlyOwner {
        require(_pCost > 0);
        pCost = _pCost * 1 ether;
    }

    function setWCost(uint256 _wCost) public onlyOwner {
        require(_wCost > 0);
        wCost = _wCost * 1 ether;
    }
