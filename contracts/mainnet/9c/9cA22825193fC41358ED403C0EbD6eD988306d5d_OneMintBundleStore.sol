...
contract OneMintBundleStore is AccessControl {
  ...
  // Set of all registry ids that have claimed a bundle
  mapping(uint => mapping(uint => bool)) private claimants;
  mapping(uint => Bundle) bundles;

  error RegistryIdInvalid();
  error AccountAlreadyClaimed(address account);
  error PartnerAddressMismatch(address tokenPartnerAddress, address bundlePartnerAddress);
  error AlreadyExists(uint id);
  error DoesNotExist(uint id);

  event Claimed(uint indexed id, address indexed owner, uint indexed registryId);

  constructor(address _registry, address _item) {
  ...
  function getBundle(uint id) external view returns (Bundle memory) {
    return bundles[id];
  }

  function getBundles(uint[] calldata ids) external view returns (Bundle[] memory) {
    Bundle[] memory result = new Bundle[](ids.length);

    for (uint i = 0; i < ids.length; i++) {
      Bundle storage bundle = bundles[ids[i]];
      result[i] = bundle;
    }
  ...
  function hasClaimed(uint id, uint registryId) external view exists(id) returns (bool) {
    return claimants[id][registryId];
  }

  // ----------- Claim -------------

  function mint(uint id, uint registryId) external exists(id) {
    Bundle memory bundle = bundles[id];
    // Revert if user is not owner of registry
    if (msg.sender != registry.ownerOf(registryId)) {
      revert RegistryIdInvalid();
    }
    // Revert if owner of token has already claimed bundle
    if (claimants[id][registryId]) {
      revert AccountAlreadyClaimed(msg.sender);
    }
    // Revert if user is trying to claim a bundle with the incorrect partner token
    CozyRegistry.PartnerToken memory _partnerToken = registry.getPartnerToken(registryId);
    if (bundle.partnerAddress != _partnerToken.partnerAddress) {
      revert PartnerAddressMismatch(_partnerToken.partnerAddress, bundle.partnerAddress);
    }

    uint[] memory amounts = new uint[](bundle.tokenIds.length);
    for (uint i = 0; i < bundle.tokenIds.length; i++) {
      amounts[i] = 1;
    }
    claimants[id][registryId] = true;
    item.mintBatch(msg.sender, bundle.tokenIds, amounts);
    emit Claimed(id, msg.sender, registryId);
  }
