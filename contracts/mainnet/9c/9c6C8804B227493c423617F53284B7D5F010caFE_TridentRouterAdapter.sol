...
contract TridentRouterAdapter is IUniswapRouterETH {
    ...
    bool public constant UNWRAP_BENTO = true;
    ...
    constructor (
        ...
        // approve bento.transfer by trident router
        bento = router.bento();
        IBento(bento).setMasterContractApproval(
            address(this),
            address(router),
            true,
            0,
    ...
    function getAmountsOut(
        ...
        (address pool, address tokenA, uint amountOut) = _findBestPool(path, amountIn);
    ...
    function swapExactTokensForTokens(
        ...
        (address pool, address tokenA,) = _findBestPool(path, amountIn);
...
// INTERNAL FUNCTIONS
    ...
    function _findBestPool(address[] calldata path, uint amountIn) internal view returns (
        address pool,
        address tokenA,
        uint amountOut
    ) {
        ...
        uint count = factory.poolsCount(tokenA, tokenB);
        require(count > 0, 'route not found');

        for (uint i = 0; i < count; i++) {
            address _pool = factory.pools(tokenA, tokenB, i);
            uint _amount = IPool(_pool).getAmountOut(abi.encode(
                tokenA, amountIn
            ));
    ...
    function _prepareTokens(IERC20 token, uint amount) internal returns (uint before) {
        ...
        if (token.allowance(address(this), bento) < amount) {
            token.safeApprove(bento, type(uint256).max);
        }
