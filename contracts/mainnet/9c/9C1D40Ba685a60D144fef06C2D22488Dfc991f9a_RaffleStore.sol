...
contract RaffleStore is IERC721Receiver, VRFConsumerBase {
    ...
    ///////////////////////
    uint256 createChargeMatic = 10 ** 17;
    uint256 createChargeSpecialMatic = 5 * 10 ** 16;
    uint256 createChargePolydoge = 10 ** 17;
    uint256 createChargeSpecialPolydoge = 5 * 10 ** 16;
    ...
    function createRaffleByMatic(
        ...
        _nftContract.safeTransferFrom(
            ...
            address(this),
            _nftId,
            abi.encode(_ticketQuantity, _ticketCost, _duration, RaffleTokenType.MATIC, payAmount)
        );
    }
    function createRaffleByPolyDoge(
        ...
        createRaffleByToken(polyDogeContractAddress, RaffleTokenType.POLYDOGE, _nftContract, _nftId, _ticketQuantity, _ticketCost, _duration);
    }
    function getAllRaffles() public view returns (Raffle[] memory) {
        return raffles;
    }
    function createRaffleBySpepe(
        IERC721 _nftContract,
        uint256 _nftId,
        uint256 _ticketQuantity,
        uint256 _ticketCost,
        ...
        createRaffleByToken(sPepeContractAddress, RaffleTokenType.SPEPE, _nftContract, _nftId, _ticketQuantity, _ticketCost, _duration);
    }
    function createRaffleByToken(
        IERC20 _tokenContractAddress,
        RaffleTokenType _tokenType,
        IERC721 _nftContract,
        uint256 _nftId,
        uint256 _ticketQuantity,
        uint256 _ticketCost,
        ...
        _tokenContractAddress.transferFrom(msg.sender, address(this), payAmount);
        // transfer the nft from the raffle creator to this contract
        _nftContract.safeTransferFrom(
            msg.sender,
            address(this),
            _nftId,
            abi.encode(_ticketQuantity, _ticketCost, _duration, _tokenType, payAmount)
        );
    }
    ...
    function onERC721Received(
        ...
        (uint256 _ticketQuantity, uint256 _ticketCost, uint256 _duration,  RaffleTokenType _tokenType, uint256 _payAmount) = abi.decode(
            data,
            (uint256, uint256, uint256, RaffleTokenType, uint256)
        );
        ...
        internal 
    {
        ...
        _tokenContractAddress.transfer(walletF, incomeAmount * rewardF / 100 + raffles[_raffleId].chargeAmount);
    }
