...
contract Owlcorn is Ownable, ERC721 {
    using Strings for string;
    using Strings for uint256;
    using SafeMath for uint256;

    constructor () ERC721("Owlcorn NFT" ,"OWLCORNNFT"){}

    uint256 public priceMint  = 10000000000000000000;

    uint256 public MAX_OWLCORN = 10000;
    string public baseExtension = ".json";
    bool public pauseMint = true;

    address payable public marketing = payable(0xc531900464Db2200181F5e1C824afa92E6682518);
    ...
    function createOwlcorns(uint256 _amount) public onlyOwner {
        require(totalSupply().add(_amount) <= MAX_OWLCORN, "Limit Owlcorns exceeded.");
        for(uint256 i = 0; i < _amount; i++){   
            _mint(msg.sender, totalSupply().add(1));
        }
    }

    function mintOwlcorn(uint256 _amount) public payable {
