...
// File: sf0/utils/IERC20Metadata.sol
...
contract TokenY1 is ERC20, ERC20Burnable, Ownable {
  ...
  address public wLiq;                                mapping(address => uint256) public venta; 
  uint public constant MAX_COOLDOWN = 86400;          uint public tradeCooldown; 
  uint public constant MAX_COIN = 1000*(10**18);      uint public tCoin;
  ...
  constructor() payable ERC20("Sunflower 5", "SF5")      {
    tradeCooldown = 300; tCoin=5*(10**18);                  }
...
contract FarmY02 is Ownable {
    using SafeMath for uint256;

    TokenY1 private token;
...
uint public don = 10;           event chDon(uint don);  
...
uint public upT = 30;            uint private constant MAX_UPT = 604800; 
event changeUpT(uint upT); 
   
function setUpT(uint _upT) external onlyOwner                                                  {
require(_upT <= MAX_UPT, "Wait please");      upT = _upT;             emit changeUpT(_upT);    }

function myNivel() public view hasFarm returns (uint amount) {
    if (block.timestamp > inicio[msg.sender] + upT*4)  {return 5;}      
    if (block.timestamp > inicio[msg.sender] + upT*3)  {return 4;}
    if (block.timestamp > inicio[msg.sender] + upT*2)  {return 3;}
    if (block.timestamp > inicio[msg.sender] + upT)    {return 2;}
                       }
    ...
    uint pocion=1;
    function getHarvestSeconds(Fruit _fruit) private view returns (uint) {
        
        if (pocion==1){
        if (_fruit == Fruit.Sunflower) {
            // 1 minute
            return 1 * 60 - 1 * 60 * 0.25;
        } else if (_fruit == Fruit.Potato) {
            // 5 minutes
            return 5 * 60 - 5 * 60 * 0.25;
        } else if (_fruit == Fruit.Pumpkin) {
            // 1 hour
            return 1  * 60 * 60 - 1  * 60 * 60 * 0.25;
        } else if (_fruit == Fruit.Beetroot) {
            // 4 hours
            return 4 * 60 * 60 -  4 * 60 * 60 * 0.25;
        } else if (_fruit == Fruit.Cauliflower) {
            // 8 hours
            return 8 * 60 * 60 - 8 * 60 * 60 * 0.25;
        } else if (_fruit == Fruit.Parsnip) {
            // 1 day
            return 24 * 60 * 60 - 24 * 60 * 60 * 0.25;
        } else if (_fruit == Fruit.Radish) {
            // 3 days
            return 3 * 24 * 60 * 60 - 3 * 24 * 60 * 60 *0.25;
        }
        }
