...
contract FVSTAKE is Ownable {
    address public FVAddress;
    address public SWAPAddress;
    address public immutable USDC;

    uint96 public lockUpPeriod = 3 days;
    ...
    function changeFVContract(address newAddy) external onlyOwner {
        require(
            newAddy != address(0),
    ...
    function Stake(uint256 amount) external {
        ...
        ERC20(FVAddress).transferFrom(msg.sender, address(this), amount);
    ...
    function Unstake(uint256 idx) external {
        ...
        require(
            block.timestamp - _stakingInfo[msg.sender][idx].stakingDate >
                lockUpPeriod,
            "You can unstaked after 3 days from staking"
        );

        ERC20(FVAddress).transfer(
            msg.sender,
        ...
        returns (uint256, uint256)
    {
        ...
        if (block.timestamp - claimDate[_user] > claimPeriod) {
            for (uint8 i = 0; i < _stakingInfo[_user].length; i++) {
                if (
                    !_stakingInfo[_user][i].unstaked &&
                    block.timestamp - _stakingInfo[_user][i].stakingDate >
                    claimPeriod
                ) {
                    if (claimDate[_user] == 0)
                        passTime =
                            block.timestamp -
                            _stakingInfo[_user][i].stakingDate;
                    else {
                        passTime = block.timestamp - claimDate[_user];
                    }
                    rewardAmount1 +=
