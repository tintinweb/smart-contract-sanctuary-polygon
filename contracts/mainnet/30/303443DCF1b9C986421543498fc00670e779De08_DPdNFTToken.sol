...
// File: interfaces/IDPxNFTToken.sol
...
interface IDPxNFTToken is IERC721{
 ...
 * Box兑换凭证
 */
contract DPdNFTToken is ERC721Enumerable, Ownable{
    using SafeMath for uint256;

    string private baseURI;
    address private dpBoxAddr;

    constructor(string memory tokenBaseUri)ERC721("DP NFT", "dNFT") {
        baseURI = tokenBaseUri;
    }

    function mint(address _to) external onlyDpBox returns(uint256) {
    ...
    function setDpBox(address _dpBoxAddr) external onlyOwner{
        dpBoxAddr = _dpBoxAddr;
    }
    ...
    modifier onlyDpBox(){
        require(msg.sender == dpBoxAddr, "caller is not the dpBox");
        _;
    }
