...
import "../../v3/PotatoChip/interfaces/IPotatoChipBurnable.sol";

contract WheelOfFortuneV3 is
	IWheelOfFortune,
	...
	OwnableUpgradeable
{
	...
	struct History {
		// v2 :: struct for history of games
		uint256 date;
		bytes32 requestId;
		uint256 wheelId;
		uint256 bet;
	...
	mapping(address => History[]) public gameHistory; // v2 :: History of transactions for wheel of fortune
	uint256 public rewardTax; // v2 :: added for rewards tax
	address public taxTreasury; // v2 :: address that will receive rewards tax

	function initialize(address _registryFarmerV2) public initializer {
		__Ownable_init();

		_setRegistryFarmer(_registryFarmerV2);
		minWagerAmountPotato = 10**18; // 1 $POTATO
	}

	modifier hasSufficientFreeSpinCooldown(uint256 farmerId) {
		uint256 freeSpinCooldownBlocks = wheelByWheelId[0].cooldownBlocks;
	...
	modifier onlyValidPaidSpins(uint256 wheelId, uint256 wagerAmountPotato) {
		...
		if (lastSpinBlocktime > 0) {
			...
			(
				bool wagerOutcomeIsDetermined,
				uint256 wagerPayoff
			) = getWagerOutcome(msg.sender, wheelId);
			require(
	...
	modifier onlyFarmerHolder(uint256 farmerId) {
		...
		require(
			isHolder || isStaker,
			"Only holder or staker is allowed to call this function"
		);
		_;
	}
	...
	function _createWager(uint256 wheelId, uint256 wagerAmountPotato) private {
		...
		// v2 :: save transaction for history
		GameLibrary.Wheel memory wheel = wheelByWheelId[wheelId];

		gameHistory[msg.sender].push(
			History(
				block.timestamp,
				randomnessRequestId,
				wheelId,
	...
	function claimReward(uint256 wheelId) public onlyBettor(wheelId) {
		...
		// v2 :: reward tax computation and reward net
		uint256 wagerPayoffTax = (wagerPayoff * rewardTax) / 100;
		wagerPayoff -= wagerPayoffTax;
		...
		// v2 :: tax mint
		potato.mintAsDelegate(taxTreasury, wagerPayoffTax, "WheelOfFortune");
	...
	function getTransaction(uint256 _index)
		public
		view
		returns (History memory transaction)
	{
		transaction = gameHistory[msg.sender][_index];
		uint256 randomness = _getWagerRandomness(transaction.requestId);
		if (randomness == 0) return transaction;

		uint256 n = randomness % 100;
		bool isWon = n < transaction.winChanceBasisPoints;
		if (!isWon) return transaction;

		uint256 wagerPayoffPotato = (transaction.bet * transaction.multiplier) /
			100;
		uint256 rewardTaxTotal = (wagerPayoffPotato * rewardTax) / 100;

		if (transaction.wheelId == 0) {
			uint256 basicReward = 10 ether;
			rewardTaxTotal = (basicReward * rewardTax) / 100;
			transaction.reward = basicReward - rewardTaxTotal;
			transaction.burn = rewardTaxTotal;
			transaction.result = true;
			return transaction;
		}

		transaction.reward = wagerPayoffPotato - rewardTaxTotal;
		transaction.burn = rewardTaxTotal;
		transaction.result = true;
		return transaction;
	}

	function getTransactionHistory(uint256 _numberOfTransaction)
		external
		view
		returns (History[] memory)
	{
		uint256 gameHistoryLength = getGameHistoryLength();
		// require(!(gameHistoryLength == 0), "There aren't any transactions");

		History[] memory transactionCollection = new History[](
			_numberOfTransaction
		);

		if (gameHistoryLength == 0) return transactionCollection;

		if (
			gameHistoryLength == _numberOfTransaction ||
			gameHistoryLength < _numberOfTransaction
		) {
			for (uint256 index = 0; index < gameHistoryLength; index++) {
				transactionCollection[index] = getTransaction(index);
			}
			return transactionCollection;
		}

		for (uint256 index = 0; index < _numberOfTransaction; index++) {
			--gameHistoryLength;
			transactionCollection[index] = getTransaction(gameHistoryLength);
		}
		return transactionCollection;
	}

	function getGameHistoryLength() public view returns (uint256) {
		return gameHistory[msg.sender].length;
	}
