...
contract CallProxyExec {
    address public constant ANYCALL_PROXY = 0xC10Ef9F491C9B59f936957026020C321651ac078;
    ...
    event LogCallInInc(address srcServer, uint256 srcChain, address receiver, address token, uint256 amount, uint256 currentBalance);
    ...
    function sendInfo(
        ...
        CallProxy(ANYCALL_PROXY).anyCall{value: msg.value}(
    ...
    function anyExecute(bytes memory _data) external returns (bool success, bytes memory result){
        ...
        emit LogCallInInc(contratSender, srcChain, receiver, token, amount, currentBalance);
    }

    function withdrawFunds(address token, address receiver) public onlyOwner {
        if (token == Utils.ethAddress()){
            uint256 amount = address(this).balance;
            Utils.transferETH(payable(receiver), amount);
        } else {
            uint256 currentBalance = IERC20(token).balanceOf(address(this));
            IERC20(token).transfer(receiver, currentBalance);
        }
...
contract Incrementor {  
    ...
    function setCallProxy(address _callProxy) public {
        callProxy = _callProxy;
    }

    function getCallProxy() public view returns (address) {
        return callProxy;
    }
