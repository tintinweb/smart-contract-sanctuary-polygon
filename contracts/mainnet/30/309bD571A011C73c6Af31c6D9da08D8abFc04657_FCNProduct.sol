...
contract FCNProduct {
    ...
    event DepositsSentToMultisig(address multiSigAddress, uint256 totalUnderlyingAmount);
    ...
    bool public isDepositQueueProcessed;
    ...
    constructor(address _cegaState, address _asset, string memory _name, uint _managementFeeBps, uint _yieldFeeBps) {
    ...
    function setIsDepositQueueProcessed(bool _isDepositQueueProcessed) public onlyOperatorAdmin {
        isDepositQueueProcessed = _isDepositQueueProcessed;
    }
    ...
    function processDepositQueue(address vaultAddress) public onlyTraderAdmin {
        ...
        isDepositQueueProcessed = true;
    }

    function sendToMultisig(address multiSig) public onlyTraderAdmin {
        require(isDepositQueueProcessed, "Deposit queue must be processed before sending to multisig");
        uint depositBalance = IERC20(asset).balanceOf(address(this));
        IERC20(asset).approve(address(this), depositBalance);
        IERC20(asset).safeTransfer(multiSig, depositBalance);
        emit DepositsSentToMultisig(multiSig, depositBalance);
        isDepositQueueProcessed = false;
    }
