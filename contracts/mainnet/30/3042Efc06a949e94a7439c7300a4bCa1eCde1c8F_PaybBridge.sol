...
import "../interfaces/token/BEP20.sol";
import "../utils/TransferHelper.sol";
import "../utils/ECDSA.sol";
import "@openzeppelin/contracts/utils/ReentrancyGuard.sol";

contract PaybBridge is Ownable, ReentrancyGuard {
    /// @dev Variables
    ...
    function setTokens(address _ethTokensAddress, address _bscTokenAddress)
        public
        onlyOwner
    {
        ERCtoBEP[_ethTokensAddress] = _bscTokenAddress;
        BEPtoERC[_bscTokenAddress] = _ethTokensAddress;
    }
    ...
    function withdraw(
        ...
        require(
            ECDSA.recover(signature, _signature) == governance,
            "invalid signature"
        );
        ...
        if (ERCtoBEP[_ercToken] == address(0)) {
            ...
            bytes32 salt = keccak256(
                abi.encodePacked(_ercToken, _name, _symbol)
            );
            address newToken;
            assembly {
            ...
            emit AddedToken(newToken, _name, _symbol);
        }
        BEP20Token(ERCtoBEP[_ercToken]).mint(msg.sender, _amount);
        executed[_transitId] = true;
        emit Transit(_ercToken, msg.sender, _amount, _transitId);
    }

    function transit(address _bepToken, uint256 _amount) external nonReentrant {
        require(_amount > 0, "amount must be greater than 0");
        ...
        emit Payback(_bepToken, msg.sender, _amount);
    }

    function payback(address _bepToken, uint256 _amount) external payable nonReentrant {
        address ercToken = BEPtoERC[_bepToken];

        require(ercToken != address(0), "invalid token");
        require(_amount > 0, "amount must be greater than 0");

        BEP20Token(_bepToken).burn(msg.sender, _amount);
