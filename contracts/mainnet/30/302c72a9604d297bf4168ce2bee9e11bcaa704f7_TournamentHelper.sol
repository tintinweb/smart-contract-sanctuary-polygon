...
// File: contracts/libraries/TournamentHelper.sol
...
library TournamentHelper {
    ...
    function calculateEndTime(
        ...
        return
            _startTime.add(
                _numRounds.mul(_roundDuration).add(
                    (_numRounds.sub(1)).mul(_roundInterval)
                )
            );
    }
    ...
    function validatePercentage(
        ...
        if (minimum && percentage == 0) return 1;
    ...
    function encodeNft(
        ...
        return abi.encode(_nftContract, _tokenId);
    }

    function decodeNft(
        bytes memory nftData
    ) public pure returns (address nftContract, uint256 tokenId) {
        (nftContract, tokenId) = abi.decode(nftData, (address, uint256));
    }
    ...
    function fixVariables(
        ...
        return (_numRounds, _roundDuration, _roundInterval);
    }

    function checkWinner(
        bytes memory _compara,
        bytes memory _player1,
        bytes memory _player2
    ) public pure returns (uint8) {
        if (keccak256(_compara) == keccak256(_player1)) return 1;
        else if (keccak256(_compara) == keccak256(_player2)) return 2;
        else return 0;
    }
}
...
contract Tournament is ITournament {
    ...
    uint256 public immutable override numRounds; // number of rounds
    uint256 public immutable override roundDuration; // duration of each round
    uint256 public immutable override roundInterval; // interval between rounds
    uint256 public immutable minutesOnDraw; // how many minutes will be added on Draw
    uint256 public immutable maxPlayers;
    uint256 public immutable override jackpotPerc; //percentage of loosers that goes to jackpot each round
    uint256 public immutable override publicGoodsPerc; //percentage of jackpot that goes to publicGoods each round
    ...
    constructor(
        ...
        maxPlayers = TournamentHelper.calculateMaxPlayers(numRounds);

        endTime = TournamentHelper.calculateEndTime(
            _startTime,
            _numRounds,
            _roundDuration,
            _roundInterval
        ...
        jackpotPerc = TournamentHelper.validatePercentage(_jackpotPerc, false);
        publicGoodsPerc = TournamentHelper.validatePercentage(
            _publicGoodsPerc,
            true
        );
    ...
    modifier onlyAdministrator() {
        // Check Permissions
        require(tournamentHub.checkAdmin(msg.sender));
        _;
    }
    ...
    modifier onlyProject() {
        //Check authorization
        require(tournamentHub.checkProject(msg.sender));
        _;
    }
    ...
    function startTournament() public onlyAdministrator {
        require(tournamentStatus == StateTournament.Waiting);
        ...
        )
    {
        ...
        require(tournamentStatus == StateTournament.Waiting);
        require(getPlayer.length < maxPlayers);

        // adds the player to the list of players
        bytes memory encodedNft = TournamentHelper.encodeNft(
            _nftContract,
            _tokenId
        );
    ...
    function vote(
        ...
        uint8 _playerInMatch = TournamentHelper.checkWinner(
            encodedNft,
            MatchInterface.getPlayer1(),
            MatchInterface.getPlayer2()
        );

        if (_playerInMatch == 0) return 0;
    ...
    function claimPublicGoods() public onlyAdministrator {
        require(tournamentStatus == StateTournament.Finished);
        require(claimed == FeesClaimed.NotClaimed);
        IRound _actualRound = IRound(getRound[round - 1]);
...
contract TournamentGenerator is ITournamentGenerator {
    ...
    function changeTournamentHub(address _contract) public {
        // Check Permissions
        if (activated) require(tournamentHub.checkAdmin(msg.sender));
        else require(deployer == msg.sender);
