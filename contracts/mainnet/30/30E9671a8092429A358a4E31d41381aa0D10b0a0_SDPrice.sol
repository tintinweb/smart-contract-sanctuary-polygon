...
contract SDPrice is Ownable, IEACAggregatorProxy {
    ...
    IERC20Extended public sd;
    IERC20Extended public usdc;

    constructor(address _algebraPool, uint32 _twapDuration, address _sd, address _usdc) {
        require(_twapDuration > 0, "twap duration should be greater than 0");
        pool = IAlgebraPool(_algebraPool);
        twapDuration = _twapDuration;
        sd = IERC20Extended(_sd);
        usdc = IERC20Extended(_usdc);
        currentRound = 1;
    ...
    function latestAnswer() external view override returns (int256 res){
        ...
        res = int256(_sqrtPriceX96ToUint(sqrtPriceX96, sd.decimals()));
    }
    ...
    function getAnswer(uint256 roundId) external view override returns (int256 res){
        uint160 sqrtPriceX96 = _getSqrtTwapX96();
        res = int256(_sqrtPriceX96ToUint(sqrtPriceX96, sd.decimals()));
    }
