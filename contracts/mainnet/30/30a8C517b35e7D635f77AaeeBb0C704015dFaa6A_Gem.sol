...
contract Gem is ERC721URIStorage, GameRole {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    uint256 private _totalSupply;

    constructor() ERC721("Gem - Crystal.network", "GEM") {
        _tokenIds.increment();
    }
    ...
    function mintGem(address player, string memory tokenURI)
        ...
        returns (uint256)
    {
        uint256 newGemId = _tokenIds.current();
        _mint(player, newGemId);
        _setTokenURI(newGemId, tokenURI);
    ...
    function burnGem(uint256 gemId) public onlyGame returns (bool) {
        _burn(gemId);
        _totalSupply -= 1;
        return true;
    }
