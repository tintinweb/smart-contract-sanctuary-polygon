...
function getNFTData(uint amount, address _address) public view returns(uint _price, uint _totalSupply, uint maxSupply, uint _balanceOfUser,uint _startedAt) {
  
  return (getPrice(amount, _address),totalSupply(),getMaxMintAmount(), balanceOf(_address), startedAt);
}
