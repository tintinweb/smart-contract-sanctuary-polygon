...
import "./Calcs.sol";
import "./IFoundersControl.sol";
import "./IVaultInterestFounders.sol";
import "./IFounders.sol";


/// @title A Contract to claim Interests to be paid to Founders.
/// @author Panoram Finance / Voha.
/// @notice You can use this contract to claim the interest earning for any user in Founders.
contract InterestFounders is ReentrancyGuard, Calcs, IFounders {
    ...
    /// @dev Events Emitted
    event ClaimRewardsSuccess(address indexed wallet, uint256 indexed idFounders, uint256 amount, uint256 indexed idRequest);
    event ClaimRequestSuccess(address indexed wallet, uint256 indexed idRequest, uint256 amountRewards, uint256 foundersID);
    ...
    /// @param _FoundersControl - the address of the Founders Control contract.
    constructor(address _tokenInfo, address _FoundersControl) Calcs(_FoundersControl) {
        //Set Roles
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(DEV_ROLE, msg.sender);
        _setupRole(DEFAULT_ADMIN_ROLE, 0xb818dd90d667D6D269FA7a690114f98473aF7b95); // cambiar por el Multisign
    ...
    function SetClaimRewardsRequest(uint256 _id, address _token) public validToken(_token) isPaused nonReentrant{
        // VALIDAR MEJOR EL ISFULLYPAID Y EL ALLINTERESTPAY EN LUGAR DE ESTO PARA QUE NO REVIERTA LA FUNCION CUANDO TENGA REWARS PENDIENTES, PERO 
        // YA ESTE COMPLETAMENTE PAGADA La Deuda
        (uint256 amountRewards, uint256 timeOfCalc) = calcInterestAccumulated(msg.sender, _id);
        if(amountRewards == 0 && timeOfCalc == 0){
            revert("Founders interest was Fully Paid");
        }
        uint256 totalRewards = amountRewards + FoundersControl.getPendingRewards(msg.sender, _id);
        totalRewards /= 1e12;
        (,address FoundersRewards,) = tokenInfo.getFoundersVaults(_token);
        if(totalRewards == 0){ 
            revert("You have not enough rewards yet");
        }
        uint256 amountVault = IVaultInterestFounders(FoundersRewards).getTotalRewards();
        if(totalRewards > amountVault){
            revert("Not enough rewards in the vault");
        }
        uint96 date = FoundersControl.getLastSetClaimRewards(msg.sender, _id);
         if((block.timestamp - uint256(date)) < waitTimeForRewards){
            revert("You must wait at least 24 hours to Set a Rewards Claim Again");
        } 

        (uint256 amountClaimed, , , , uint256 totalToPaid) = FoundersControl.getRewardsClaimed(msg.sender, _id);
        ...
        IVaultInterestFounders(FoundersRewards).addRequest(totalRewards);
        uint256 _idRequest = FoundersControl.createRequest(msg.sender, 0, totalRewards,2,0);
        FoundersControl.updateLastTimeClaim(msg.sender, _id, timeOfCalc);
        // Actualizar el valor de los rewards pendientes
        FoundersControl.updatePendingRewards(msg.sender, _id, timeOfCalc);
        emit ClaimRequestSuccess(msg.sender, _idRequest, totalRewards, _id);
    }
    ...
    /// @param _id the user's Founders ID.
    /// @param _token - the allowed token address for withdrawals.
    function claimRewards(uint256 _idRequest, uint256 _id, address _token) public validToken(_token) isPaused nonReentrant{
        (,uint256 rewards, Status state, uint96 date,) = FoundersControl.getRequest(msg.sender, _idRequest);
        ...
        if(checkLastClaimTime <= waitTimeForRewards){
         ...
         (,address FoundersRewards,) = tokenInfo.getFoundersVaults(address(_token));
        if(rewards > IVaultInterestFounders(FoundersRewards).getTotalRewards()){
            revert("Not enough rewards in the vault");
        } 
        IVaultInterestFounders(FoundersRewards).claimRequest(rewards, _token);
        IERC20(_token).safeTransfer(msg.sender, rewards);

        FoundersControl.updateClaimed(_id, msg.sender, rewards, block.timestamp);
        FoundersControl.closeRequest(msg.sender, _idRequest, Status.complete, _id);

         (uint256 amountClaimed, , , , uint256 totalToPaid) = FoundersControl.getRewardsClaimed(msg.sender, _id);
        if(amountClaimed >= totalToPaid){
            FoundersControl.setAllInteresPaid(msg.sender, _id, true);
        } 

        emit ClaimRewardsSuccess(msg.sender, _id, rewards, _idRequest); 
    }
    ...
    /// @dev Function to update the Founders Control contract address if necessary.
    /// @param _FoundersControl - The new address of the Founders Control contract.
    function updateFoundControl(address _FoundersControl) external onlyDev checkAddress0(_FoundersControl) {
        FoundersControl = IFoundersControl(_FoundersControl);
    }
...
interface IVaultInterestFounders {
    ...
    /// @dev Function to add a withdrawal request to pay user's interests.
