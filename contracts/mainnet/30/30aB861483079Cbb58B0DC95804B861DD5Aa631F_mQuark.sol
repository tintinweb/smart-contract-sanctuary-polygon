...
contract mQuark is ImQuark, ERC721, IERC2981, DefaultOperatorFilterer, Ownable {
  ...
  function mintWithPreURI(
    ...
    _onlymQuarkControl();
    if (templateId == 0) revert InvalidTemplateId();
    if (!_verifySignature(signature, signer, projectId, templateId, collectionId, uri, salt))
   ...
   * @dev See ERC 165
   */
  function supportsInterface(bytes4 interfaceId) public pure override(ERC721, IERC165, ImQuark) returns (bool) {
    return
...
interface ImQuarkControl {
  ...
  // Emitted when verifier address is set
  event VerifierAddressSet(address verifier);
  /**
