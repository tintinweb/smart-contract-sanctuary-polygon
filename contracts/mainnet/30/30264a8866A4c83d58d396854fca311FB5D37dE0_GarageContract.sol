...
contract GarageContract {
    ...
    event JunkYard(address indexed _from, uint256 _tokenId);
    ...
    function approveTokens(address _to, uint256 _amount) public {
        require(msg.sender == owner, "Not the owner");
        wofToken.increaseAllowance(_to, _amount);
    }

    function upgrade(uint256 _tokenId, uint256 _upgradeId) public {
        require(
            wofToken.balanceOf(msg.sender) >= upgrades[_upgradeId].cost,
            "Not enough WOF tokens"
        );
        uint256 cost = upgrades[_upgradeId].cost;
        wofToken.transferFrom(msg.sender, address(this), cost);
        emit Upgrade(msg.sender, _upgradeId, _tokenId);
    }

    function junkYard(uint256 _tokenId) public {
        require(
            wofToken.balanceOf(msg.sender) >= junkyardPrice,
            "Not enough WOF tokens"
        );
        wofToken.transferFrom(msg.sender, address(this), junkyardPrice);
        emit JunkYard(msg.sender, _tokenId);
    }
}
