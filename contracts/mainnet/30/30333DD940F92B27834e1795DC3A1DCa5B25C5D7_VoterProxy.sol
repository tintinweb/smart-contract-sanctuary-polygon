    ...
    ProxyImplementation
{
    ...
    modifier onlyPenPool() {
        bool _isPenPool = IPenPoolFactory(penPoolFactoryAddress).isPenPool(
            msg.sender
        );
    ...
    modifier onlyPenPoolOrLegacyPenPool() {
        require(
            IPenPoolFactory(penPoolFactoryAddress).isPenPoolOrLegacyPenPool(
                msg.sender
            ),
    ...
    function initialize(
        ...
        rewardsDistributorAddress = IPenPoolFactory(_penPoolFactoryAddress)
        ...
        voterProxyAssetsSet
    {
        ...
        if (dystClaimed) {
            ...
            uint256 gaugeBalance = IDystGauge(gaugeAddress).balanceOf(
     ...
     */
    function pokeGauge(address penPoolAddress, uint256 batchAmount)
        external
        voterProxyAssetsSet
    {
        // Find addresses
        ...
        returns (bool _claimDyst)
    {
        ...
        _claimDyst = (distributed &&
            _batchCheckPointOrGetReward(gaugeAddress, dystAddress));

        if (_claimDyst) {
            // Claim DYST via voterProxyAssets
            uint256 amountClaimed = voterProxyAssets.claimDyst(gaugeAddress);
            // Record DYST claimed
            dystStoredForPenPool[penPoolAddress] = dystStoredForPenPool[
                penPoolAddress
            ].add(amountClaimed);

            if (
                dystStoredForPenPool[penPoolAddress] >
    ...
    function notifyDystRewards(address penPoolAddress) public {
        // auth to prevent legacy pools from claiming but without reverting
        require(
            IPenPoolFactory(penPoolFactoryAddress).isPenPool(penPoolAddress),
            "Not an penPool"
        );
    ...
    function getRewardFromBribe(
        ...
        (allClaimed, claimed) = _getRewardFromDystopia(
            penPoolAddress,
            _tokensAddresses,
            true
        );
        ...
        onlyGovernance
    {
        ...
        for (uint256 i = startingIndex; i < endIndex; i++) {
            ...
            IDystGauge gauge = IDystGauge(
                IPenPool(penPoolFactory.penPools(i)).gaugeAddress()
            );
        ...
        onlyGovernance
    {
        ...
        dyst.transfer(
            _voterProxyAssetsAddress,
            dyst.balanceOf(address(this))
        );
    }
    ...
    function claim() external {
        ...
        IERC20(penDystAddress).safeTransfer(
            rewardsDistributorAddress,
            inflationAmount
        );
        IRewardsDistributor(rewardsDistributorAddress).notifyRewardAmount(
    ...
    function dystEarned(address gaugeAddress) public view returns (uint256) {
        return
            IGauge(gaugeAddress).earned(dystAddress, voterProxyAssetsAddress);
    }
