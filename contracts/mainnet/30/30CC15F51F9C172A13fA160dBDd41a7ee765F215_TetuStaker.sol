...
interface IVeTetu {
  ...
  event GovActionAnnounced(uint256 _type, uint256 timeToExecute);
  event Initialized(uint8 version);
  event Merged(address indexed stakingToken, address indexed provider, uint256 from, uint256 to);
  event RevisionIncreased(uint256 value, address oldLogic);
  event Split(uint256 parentTokenId, uint256 newTokenId, uint256 percent);
  event StakingTokenAdded(address value, uint256 weight);
  event Transfer(address indexed from, address indexed to, uint256 indexed tokenId);
  event TransferWhitelisted(address value);
  event Withdraw(address indexed stakingToken, address indexed provider, uint256 tokenId, uint256 value, uint256 ts);

  function CONTROLLABLE_VERSION() external view returns (string memory);

  function GOV_ACTION_TIME_LOCK() external view returns (uint256);

  function MAX_ATTACHMENTS() external view returns (uint256);

  function VE_VERSION() external view returns (string memory);

  function abstain(uint256 _tokenId) external;

  function addToken(address token, uint256 weight) external;

  function announceAction(uint8 _type) external;
  ...
  function govActionTimeLock(uint8) external view returns (uint256);
  ...
  function increaseUnlockTime(uint256 _tokenId, uint256 _lockDuration)
    external
    returns (uint256 power, uint256 unlockDate);
  ...
  function isWhitelistedTransfer(address) external view returns (bool);

  function lockedAmounts(uint256, address) external view returns (uint256);

  function lockedDerivedAmount(uint256) external view returns (uint256);

  function lockedEnd(uint256) external view returns (uint256);
  ...
  function ownerToOperators(address, address) external view returns (bool);

  function ownershipChange(uint256) external view returns (uint256);

  function platformVoter() external view returns (address);

  function pointHistory(uint256 _loc) external view returns (IVeTetu.Point memory);
  ...
  function tokenToOwnerIndex(uint256) external view returns (uint256);
  ...
  function userPointHistory(uint256 _tokenId, uint256 _loc) external view returns (IVeTetu.Point memory);

  function userPointHistoryTs(uint256 _tokenId, uint256 _idx) external view returns (uint256);

  function voted(uint256) external view returns (uint256);
...
contract SpTetuManager is OwnableUpgradeable, PausableUpgradeable {
  /**
   * @dev Dyson Contracts:
  ...
  function __SpTetuManager_init(address _keeper, address _voter) public initializer {
    __SpTetuManager_init_unchained(_keeper, _voter);
  }

  function __SpTetuManager_init_unchained(address _keeper, address _voter) internal initializer {
    keeper = _keeper;
    voter = _voter;
  }
...
import "./interfaces/ITetuVoter.sol";
import "./interfaces/IVeTetu.sol";
import "./interfaces/ITetuStrategy.sol";
import "./SpTetuManager.sol";
 ...
 * @title   Tetu Staker
 * @dev     Simsala
 * @notice  Locks Tetu into this contract & provides a gauge for people to provide liquidity
 *          with a boost
 */
contract TetuStaker is SpTetuManager, ReentrancyGuardUpgradeable, ERC20Upgradeable {
  using SafeERC20Upgradeable for IERC20Upgradeable;

  // Addresses used
  uint256 public constant MAX_TIME = 16 * 7 * 86400;
  ITetuVoter public tetuVoter;
  IVeTetu public veToken;
  ...
  uint256 public constant MAX_LOCK = (16 weeks);
  ...
  /**
   * @notice  Initialize the Staker
   * @dev     Simsala
   * @param   _tetuVoter  Solidly voting contract
   * @param   _veDist  Claim $veTETU rewards
   * @param   _treasury  Treasury address
   * @param   _keeper  Keeper address
   * @param   _voter  Voter address
   * @param   _reserveRate  rate at how much $TETU should be reserved for exiting $spTETU
   */
  function __TetuStaker_init(
    address _tetuVoter,
    address _veDist,
    address _treasury,
    address _keeper,
    ...
    __SpTetuManager_init(_keeper, _voter);
    __ReentrancyGuard_init_unchained();
    __Ownable_init_unchained();
    __TetuStaker_init_unchained(_tetuVoter, _veDist, _treasury, _reserveRate, _veToken, _want);
    __ERC20_init_unchained(_name, _symbol);
  }

  function __TetuStaker_init_unchained(
    address _tetuVoter,
    address _veDist,
    address _treasury,
    uint256 _reserveRate,
  ...
  /**
   * @notice  Verifies if the gauge is whitelisted for deposits
  ...
  /**
   * @notice  Deposits $TETU for $spTETU and sends depositor minted token
   * @dev     Simsala
   * @param   _amount  amount to be deposited
   */
  function _deposit(uint256 _amount) internal nonReentrant whenNotPaused {
    lock();
    uint256 _pool = balanceOfTetu();
    want.safeTransferFrom(msg.sender, address(this), _amount);
    uint256 _after = balanceOfTetu();
    _amount = _after - (_pool);
    // Additional check for deflationary tokens

    if (_amount > 0) {
      _mint(msg.sender, _amount);
      emit DepositWant(totalTetu());
    }
  }

  /**
   * @notice  Deposits into Gauge on behalf of Strategy with Boost
   * @param   _gauge  Gauge of Solidly
   * @param   _amount  Amount of Solidly LP to stake
   */
  function deposit(address _gauge, uint256 _amount) external nonReentrant onlyWhitelist(_gauge) {
    // Grab needed info
    IERC20Upgradeable _underlying = ITetuStrategy(msg.sender).want();
  ...
  /**
   * @dev If the balance of want is greater than the required reserve, the excess tokens are locked.
   * @dev If the balance of want is less than the required reserve, the lock period is extended.
   */
  function lock() public {
    if (totalTetu() > 0) {
      (, , bool shouldIncreaseLock) = lockInfo();
      if (balanceOfTetu() > requiredReserve()) {
        uint256 availableBalance = balanceOfTetu() - requiredReserve();
        veToken.increaseAmount(address(want), veTokenId, availableBalance);
  ...
  /**
   * @notice  withdraws from Boosted Chef if whitelisted
   * @param   _gauge  gauge of solidly protocol
  ...
  /**
   * @notice  harvests reward and sends to the gauge
   *          if token is the 'want' token, remove snapshot balance and only send difference
   * @param   _gauge  gauge of solidly protocol
   * @param   tokens  which tokens to transfer to gauge contract
   */
  function harvestRewards(address _gauge, address[] calldata tokens) external onlyWhitelist(_gauge) {
    claimVeEmissions();
    uint256 tetuBalBefore = balanceOfTetu();
    IGauge(_gauge).getReward(address(this), tokens);
    for (uint256 i; i < tokens.length; ) {
      if (address(tokens[i]) == address(want)) {
        uint256 tetuBalAfter = balanceOfTetu();
        IERC20Upgradeable(tokens[i]).safeTransfer(msg.sender, (tetuBalAfter - tetuBalBefore));
      } else {
  ...
  /**
   * @notice  Calculates how much is required to keep the reserveRate parity
   * @dev     Simsala
   * @return  reqReserve  returns how much is required
   */
  function requiredReserve() public view returns (uint256 reqReserve) {
    reqReserve = (balanceOfTetuInVe() * (reserveRate)) / (MAX);
  }

  /**
   * @notice  Calculates how much of the $TETU is locked
   * @dev     Simsala
   * @return  tetu  returns the quantity of $TETU in VE
   */
  function balanceOfTetuInVe() public view returns (uint256 tetu) {
    return veToken.balanceOfNFT(veTokenId);
  }

  /**
   * @notice  dictates balance of want in contract
   * @return  uint256  quantity of $TETU in Contract
   */
  function balanceOfTetu() public view returns (uint256) {
  ...
  /**
   * @notice  calculates all $TETU in all contracts
   * @dev     Simsala
   * @return  uint256  returns the $TETU in all contracts
   */
  function totalTetu() public view returns (uint256) {
    return balanceOfTetu() + (balanceOfTetuInVe());
  }

  /**
   * @notice Whitelists a strategy address to interact with the Boosted Chef and gives approvals.
   * @param _strategy new strategy address.
   */
  function whitelistStrategy(address _strategy) external onlyManager {
    IERC20Upgradeable _want = ITetuStrategy(_strategy).want();
    address _gauge = ITetuStrategy(_strategy).gauge();
  ...
  /**
   * @dev Sets the TetuVoter contract address.
   * @param _voter The address of the TetuVoter contract.
   */
  function setTetuVoter(address _voter) external onlyOwner {
    tetuVoter = ITetuVoter(_voter);
  }

  /**
   * @notice Removes a strategy address from the whitelist and remove approvals.
  ...
  /**
   * @notice Prepare a strategy to be retired and replaced with another.
  ...
  function proposeStrategy(address _oldStrategy, address _newStrategy) external onlyManager {
    require(ITetuStrategy(_oldStrategy).gauge() == ITetuStrategy(_newStrategy).gauge(), "!gauge");
    replacementStrategy[_oldStrategy] = _newStrategy;
  }

  /**
   * @notice  Upgrades to announced strategy
   * @param   _gauge  Update the strategy with the new gauge
   */
  ...
  /**
   * @notice  Claims from VE and recursively locks up
   */
  ...
  /**
   * @notice  Claims multiple owner rewards such as trading fees and bribes from gauges, transferred to treasury
   * @param   _gauges  array of gauges to be claimed for
   * @param   _tokens  multi-dimensional array to claim which tokens should be claimed for respective gauge
   */
  ...
  /**
   * @notice  claim owner rewards such as trading fees and bribes from gauges, transferred to treasury
   * @param   _gauge  gauge to be claimed with
   * @param   _tokens  array of tokens to be claimed
   */
  ...
  /**
   * @notice  create a new veToken if none is assigned to this address
   * @param   _amount Amount of tokens to be locked into this address
   * @param   _lockDuration  period of tokens to be locked into this address (MAXTIME)
   */
  function createLock(uint256 _amount, uint256 _lockDuration) external onlyManager {
    require(veTokenId == 0, "veToken > 0");
    require(_amount > 0, "amount == 0");

    want.safeTransferFrom(address(msg.sender), address(this), _amount);
    veTokenId = veToken.createLock(address(want), _amount, _lockDuration);
    _mint(msg.sender, _amount);

    emit CreateLock(msg.sender, veTokenId, _amount, _lockDuration);
  }

  // Release expired lock of a veToken owned by this address
  function release() external onlyOwner {
    (uint256 endTime, , ) = lockInfo();
    require(endTime <= block.timestamp, "!Unlocked");
    veToken.withdraw(address(want), veTokenId);

    emit Release(msg.sender, veTokenId, balanceOfTetu());
  }

  /**
   * @notice  Increase the lock period
   */
  function increaseUnlockTime() external onlyManager {
    veToken.increaseUnlockTime(veTokenId, MAX_TIME);
    emit IncreaseTime(msg.sender, veTokenId, MAX_TIME);
  }

  /**
   * @notice  transfer veToken to another address, must detach from all gauges first
   * @param   _to  receipient of veToken
   */
  ...
  /**
   * @notice  Treasury address which receives operational funds
   * @param   _treasury  address which receives operational funds
   */
  ...
  /**
   * @notice  Adjust reserve rate
   * @dev     Simsala
   * @param   _rate  rate to adjust to
   */
  ...
  /**
   * @notice  Delegate voting power to a delegate
   * @dev     Simsala
   * @param   _delegateContract  Contract of Snapshot
   * @param   _delegate  Address of wallet delegated to
   */
  function setDelegate(address _delegateContract, address _delegate) external onlyOwner {
    IDelegation(_delegateContract).setDelegate("tetu.eth", _delegate);
  }

  /**
   * @notice  Recover tokens from the Staker that has been sent by mistake
   * @param   _token  token to recover
   * @param   _native  if true, sends gas token, if false, sends ERC20
   */
  ...
  /**
   * @notice  confirmation required for receiving veToken to smart contract
   */
  function onERC721Received(
    address operator,
    address from,
