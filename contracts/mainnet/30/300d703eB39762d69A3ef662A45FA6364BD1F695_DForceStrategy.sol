...
contract DForceStrategy is LogicUpgradeable {
    ...
    function __DForceStrategy_init(
    ...
    modifier onlyMultiLogicProxy() {
        require(msg.sender == multiLogicProxy, "DL1");
        _;
    }

    modifier isUsedToken(address token) {
        require(iTokens[token] != address(0), "DL2");
        _;
    }
    ...
    function setMultiLogicProxy(address _multiLogicProxy) external onlyOwner {
        require(multiLogicProxy == address(0), "DL5");
        ...
        onlyOwner
    {
        ...
        require(length >= 2, "DL6");
        require(path[0] == rewardsToken, "DL7");
        require(path[length - 1] == blid, "DL8");
     ...
     */
    function init(address token, address iToken) public onlyOwner {
        require(iTokens[token] == address(0), "DL10");

        address _logic = logic;
        iTokens[token] = iToken;

        // Add token/iToken to Logic
        IDForceLogic(_logic).addiTokens(token, iToken);

        // Entermarkets with token/iToken
        address[] memory tokens = new address[](1);
        tokens[0] = iToken;
        IDForceLogic(_logic).enterMarkets(tokens);

        // Approve rewards token
        if (!rewardsInit) {
            IDForceLogic(_logic).approveTokenForSwap(rewardsToken);
        ...
        isUsedToken(token)
    {
        ...
        IDForceLogic(_logic).takeTokenFromStorage(amount, token);
        ...
        isUsedToken(token)
    {
        ...
        if (amountStrategy < balanceToken)
            IDForceLogic(_logic).returnTokenToStorage(amountStrategy, token);
        else IDForceLogic(_logic).returnTokenToStorage(balanceToken, token);
    ...
    function claimRewards(address token, uint256 minimumBLIDPerRewardToken)
        ...
        isUsedToken(token)
    {
        require(pathToSwapRewardsToBLID.length >= 2, "DL9");

        address _logic = logic;

        // Claim Rewards token
        address[] memory tokens = new address[](1);
        tokens[0] = iTokens[token];
        IDForceLogic(_logic).claim(tokens);
        ...
        IDForceLogic(_logic).swapExactTokensForTokens(
            rewardsSwapRouter,
            amountIn,
            amountOutMin,
        ...
        IDForceLogic(_logic).addEarnToStorage(amountBLID);
        ...
        isUsedToken(token)
    {
        ...
        if (balance < amount) {
            ...
            IDForceLogic(_logic).returnETHToMultiLogicProxy(amount);
        }
    ...
    function createCircles(
        ...
        address iToken = iTokens[token];
        address _logic = logic;
        uint8 iterateCount = _iterateCount;
        uint256 amount = _amount;

        require(amount > 0, "DL12");
        ...
        require(collateralFactor > 0, "DL11");

        for (uint256 i = 0; i < iterateCount; ) {
            IDForceLogic(_logic).mint(iToken, amount);

            uint256 borrowAmount = (amount * collateralFactor) / 100;
            IDForceLogic(_logic).borrow(iToken, borrowAmount);
    ...
    function destructCircles(address token, uint8 _iterateCount) private {
        ...
        for (uint256 i = 0; i < iterateCount; ) {
            uint256 iTokenBalance; // balance of iToken
            ...
            iTokenBalance = IERC20Upgradeable(iToken).balanceOf(_logic);
            borrowBalance = IiToken(iToken).borrowBalanceCurrent(_logic);
            exchangeRateMantissa = IiToken(iToken).exchangeRateStored();

            // calculates of supplied balance, divided by 10^18 to safe digits correctly
            uint256 supplyBalance = (iTokenBalance * exchangeRateMantissa) /
                10**18;
            ...
            if (borrowBalance == 0) {
                if (iTokenBalance > 0) {
                    // redeem and exit
                    IDForceLogic(_logic).redeemUnderlying(
                        iToken,
                        supplyBalance
                    );
                    return;
            ...
            // if redeem tokens
            IDForceLogic(_logic).redeemUnderlying(iToken, withdrawBalance);
            ...
            if (repayAmount > 0) {
                ...
                if (borrowBalance > repayAmount) {
                    IDForceLogic(_logic).repayBorrow(iToken, repayAmount);
                } else {
                    IDForceLogic(_logic).repayBorrow(iToken, borrowBalance);
                }
            }
