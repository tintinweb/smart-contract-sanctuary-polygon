...
contract MultiSend {
    ...
    function withdraw(IBEP20 token) public onlyOwner{
        token.safeTransfer(owner, token.balanceOf(address(this)));
    }
    
    //batch send fixed token amount from sender, require approval of contract as spender
    function multiSendFixedToken(IBEP20 token, address[] memory recipients, uint256 amount) public onlyOwner {
        
        address from = msg.sender;
        
        require(recipients.length > 0);
        require(amount > 0);
        require(recipients.length * amount <= token.allowance(from, address(this)), "Approve MultiSend contract to spend your tokens");
    ...
    function multiSendDiffToken(IBEP20 token, address[] memory recipients, uint256[] memory amounts) public onlyOwner {
