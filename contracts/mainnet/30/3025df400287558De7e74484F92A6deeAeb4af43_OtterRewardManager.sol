...
// interfaces
import {IClamPond} from './interfaces/IClamPond.sol';
import {IERC20} from './interfaces/IERC20.sol';
import {IPortfolioManager} from './interfaces/IPortfolioManager.sol';
import {IPearlBank} from './interfaces/IPearlBank.sol';
import {IUSDPlusMarket} from './connectors/overnight/IUSDPlusMarket.sol';
import {IWrappedUsdPlusToken} from './connectors/overnight/IWrappedUsdPlusToken.sol';

interface IOtterWrappedUsdPlusToken is IERC20 {
    function mint(uint256 amount_) external returns (uint256);

    function mintFromUsdPlus(uint256 amount_) external returns (uint256);
}

contract OtterRewardManager is BaseContractV2, PausableUpgradeable {
    using WadRayMath for uint256;

    IERC20 public constant usdplus =
        IERC20(0x236eeC6359fb44CCe8f97E99387aa7F8cd5cdE1f);

    IPearlBank public pearlBank;
    IClamPond public clamPond;
    IPortfolioManager public portfolioManager;
    IERC20 public usdc;
    IOtterWrappedUsdPlusToken public ocUsdPlus;
    ...
    event Payout(uint256 totalUsdPlus);
    ...
    function initialize(
        ...
        clamPond = IClamPond(clamPond_);
        pearlBank = IPearlBank(pearlBank_);
        usdc = IERC20(usdc_);
        ocUsdPlus = IOtterWrappedUsdPlusToken(ocUsdPlus_);
        nextPayoutTime = nextPayoutTime_;
        payoutPeriod = 1 days;
        payoutTimeRange = 15 minutes;
    }

    // ---  setters
    function setPortfolioManager(address portfolioManager_) external onlyAdmin {
        require(portfolioManager_ != address(0), 'Zero address not allowed');
        portfolioManager = IPortfolioManager(portfolioManager_);
        emit PortfolioManagerUpdated(portfolioManager_);
    }

    function setPayoutTimes(
        uint256 nextPayoutTime_,
        uint256 payoutPeriod_,
        uint256 payoutTimeRange_
    ) external onlyAdmin {
        require(nextPayoutTime_ != 0, 'Zero _nextPayoutTime not allowed');
        require(payoutPeriod_ != 0, 'Zero _payoutPeriod not allowed');
        require(
            nextPayoutTime_ > payoutTimeRange_,
            '_nextPayoutTime shoud be more than _payoutTimeRange'
        );
        nextPayoutTime = nextPayoutTime_;
        payoutPeriod = payoutPeriod_;
        payoutTimeRange = payoutTimeRange_;
    ...
    function _payout() internal {
        ...
        // reward paid in USDC / USD+
        portfolioManager.claimAndBalance();

        // wrapped as ocUsdPlus
        uint256 rewardInUSDC = usdc.balanceOf(address(this));
        usdc.approve(address(ocUsdPlus), rewardInUSDC);
        uint256 ocUsdPlusAmount = ocUsdPlus.mint(rewardInUSDC);

        uint256 rewardInUsdPlus = usdplus.balanceOf(address(this));
        if (rewardInUsdPlus > 1e6) {
            usdplus.approve(address(ocUsdPlus), rewardInUsdPlus + 1e6);
            ocUsdPlusAmount += ocUsdPlus.mintFromUsdPlus(rewardInUsdPlus);
        }

        ocUsdPlus.approve(address(pearlBank), ocUsdPlusAmount);
        pearlBank.addReward(address(ocUsdPlus), ocUsdPlusAmount);
        clamPond.rebase();

        emit Payout(rewardInUSDC + rewardInUsdPlus);
    }
...
interface IPearlBank is IERC20 {
...
interface IWrappedUsdPlusToken is IERC20, IERC4626 {
