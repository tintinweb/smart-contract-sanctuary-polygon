...
contract GMODragons is ERC721 {
    ...
    constructor()
        ERC721("GMO Dragons", "GMOD")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 133) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeiaailomklry7uvvdr7wwpjopblwfgf4serkgjy5zwyijpwzcu7h5i/", Strings.toString(tokenId), ".json")
            );
    }
