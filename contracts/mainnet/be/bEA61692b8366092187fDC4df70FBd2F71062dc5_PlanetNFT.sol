...
contract PlanetNFT is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, EIP2981RoyaltyOverrideCore, OwnableUpgradeable {
    using CountersUpgradeable for CountersUpgradeable.Counter;

    CountersUpgradeable.Counter private _tokenIdCounter;

    function initialize() initializer public {
        __ERC721_init("PlanetNFT", "PNFT");
        __ERC721Enumerable_init();
        __ERC721URIStorage_init();
        __Ownable_init();
    }

    function safeMint(address to, string memory uri, address creator, uint16 bps) public onlyOwner {
        ...
        {
            TokenRoyaltyConfig memory royaltyConfig = TokenRoyaltyConfig(tokenId, creator, bps);
            TokenRoyaltyConfig[] memory royaltyConfigs = new TokenRoyaltyConfig[](1);
            royaltyConfigs[0] = royaltyConfig;
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC721Upgradeable, ERC721EnumerableUpgradeable, EIP2981RoyaltyOverrideCore)
        returns (bool)
    {
        return EIP2981RoyaltyOverrideCore.supportsInterface(interfaceId) || super.supportsInterface(interfaceId);
    }
