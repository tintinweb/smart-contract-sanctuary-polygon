	...
	ERC2771Context
{
	...
	struct ExecutePaidMetaTxVars {
		...
		uint256 gasLimit;
		address refundToken;
		uint256 refundTokenGasPrice;
		uint256 refundTokenMaxGasPrice;
		...
		virtual
	{
		...
		require(
			vars.refundTokenGasPrice <= vars.refundTokenMaxGasPrice,
		...
		uint256 gasUsed = gasAtStart - gasleft() + 100_000; // add some gas for refund transfer
		require(gasUsed <= vars.gasLimit, "gasUsed > gasLimit");
		IERC20(vars.refundToken).safeTransfer(
			vars.refundAddress,
			gasUsed * vars.refundTokenGasPrice
		);
	}
