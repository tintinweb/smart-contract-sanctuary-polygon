...
interface IMultiVesting {
    ...
    function currentDebtID(uint256 _vestingID)
        external
        view
        returns (uint256 currentDebt_);
    ...
    function debtDecayID(uint256 _vestingID)
        external
        view
        returns (uint256 decay_);

    function debtRatio(uint256 _vestingID)
        external
        view
    ...
    function getPayout(uint256 _amountIn, uint256 _vestingID)
        external
        view
    ...
    function pendingPayoutFor(address _depositor, uint256 _vestingID)
        external
        view
        returns (uint256 pendingPayout_);

    function percentVestedFor(address _depositor, uint256 _vestingID)
        external
        view
    ...
    function redeem(address _depositor, uint256 _vestingID)
        external
        returns (uint256);
    ...
    function setBondTermsByVestingId(
        uint256 _vestingID,
        uint256 _vesting,
        uint256 _bonus,
        uint256 _maxDebt
...
contract Registry is AccessControl {
    ...
    mapping(address => uint) public bondTypeFee; // use different fees for different factory
    mapping(address => uint) public feeDiscount; // discounts the bond fees for bond owners 100%=1000
    uint public constant MAX_FEE = 50000; // 5%
    uint public constant FEE_DECIMALS = 1000000; //100%

    function setBondTypeFees(address _factory, uint _fee) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not ADMIN role"); //ONLY ADMIN ROLE
        require(_fee <= MAX_FEE, "Fee too high");

        bool isFactory=false;
        for (uint i=0; i < factories.length; i++) { //check if factory is already in array
            if(factories[i] == _factory){
                isFactory = true;
                break;
            }
        }
        if(isFactory){ //add the factory if it doesnt exists already
    ...
    function addFactory(address _factory) external{
        ...
        if(!found){ //add the factory if it doesnt exists already
    ...
    constructor(address _mizuDao, address _mizuTreasury) {
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, address(this)); // used to automatically give factory role
        _setupRole(SETFEE_ROLE, _mizuDao);
        _setupRole(AFFILIATE_ROLE, _mizuDao);
        _setupRole(REFERRAL_ROLE, _mizuDao);
    }
    /* ======== FACTORY FUNCTIONS ======== */
     ...
     */
    function pushBond(address _payoutToken, address _principleToken, address _customTreasury, address _customBond, address _initialOwner, address _factory) external returns(address _treasury, address _bond) {
        require(hasRole(FACTORY_ROLE, msg.sender), "Not FACTORY role"); //ONLY FACTORY ROLE
    ...
    function setFeeDiscount(address _bond, uint _newFeeDiscount) external { //todo change to bond instead of owner
        require(hasRole(SETFEE_ROLE, msg.sender), "Not SETFEE role");
    ...
    function resetFeeDiscount(address _bond) external { // ONLY SETFEE ROLE Reset the value to the default value.
        require(hasRole(SETFEE_ROLE, msg.sender), "Not SETFEE role");
        delete feeDiscount[_bond];
    }

    /* ======== EXTERNAL VIEW FUNCTIONS ======== */

    function getFee(address _bond) external view returns(uint _checkedFee) {
        uint bondID = indexOfBond[_bond];
        uint typeFee = bondTypeFee[bondDetails[bondID]._factory];
        if (bondID == 0) {
            if (bondDetails[bondID]._bondAddress == _bond) { //avoid false false positive when bondID=0
                if (feeDiscount[_bond] == 0) { //check if feeDiscount = 0
                    _checkedFee = typeFee; //use typeFee if _feeDiscount = 0
                } else {
                    _checkedFee = typeFee-(typeFee*(feeDiscount[_bond])/1000);
                }
            }
            else{
                revert();
    ...
    function getAffiliate(address _affiliate) external view returns( uint _percentage){
            if (AffPercentage[_affiliate] != 0)
                {
                _percentage = AffPercentage[_affiliate];
                }
            else {_percentage = BaseAffPerc;}
        return( _percentage);
    }

    function getRefPerc(address _referrer) external view returns(uint _percentage){
        if (CustomRefPerc[_referrer] != 0 && 
        CustomRefPerc[_referrer] > BaseRefPerc)
        {
        _percentage = CustomRefPerc[_referrer];
        }
        else {_percentage = BaseRefPerc;}
        return _percentage;
    }
    ...
    function setAffWhitelist(address _affiliate, bool _bool) external{
        require(hasRole(AFFILIATE_ROLE, msg.sender), "Not FACTORY role"); //ONLY AFFILIATE ROLE
        AffWhitelist[_affiliate] = _bool;
    }

    //function only admin set baseAffPerc;
    function setBaseAffPerc(uint _percentage) external{
        require(_percentage <= 1000000, "over 100%");     
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not ADMIN role"); //ONLY ADMIN ROLE
        BaseAffPerc = _percentage;
    }

    function setAffPerc(address _affiliate, uint _percentage) external{
        require(_percentage <= 1000000, "over 100%");     
        require(hasRole(AFFILIATE_ROLE, msg.sender), "Not AFFILIATE role"); //ONLY ADMIN ROLE
    ...
    function claimAllAff(address _affiliate) external{
        for (uint256 i = 0; i < bondDetails.length; i++) {
            ...
            uint earnings = bond.AffiliateEarnings();
    ...
    function setRefPerc(address _referrer, uint _percentage) external {
        require(_percentage <= 1000000, "over 100%");          
        require(hasRole(REFERRAL_ROLE, msg.sender), "Not REFERRAL role"); //ONLY REFERRAL ROLE
        CustomRefPerc[_referrer] = _percentage;
    }

    function setBaseRefPerc(uint _percentage) external {
        require(_percentage <= 1000000, "over 100%");  
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not ADMIN role"); //ONLY ADMIN ROLE
    ...
    function claimAllRef(address _referrer) external{
        for (uint256 i = 0; i < bondDetails.length; i++) {
            IMultiVesting bond = IMultiVesting(bondDetails[i]._bondAddress);
            uint earnings = bond.ReferralEarnings(_referrer);
            if(earnings > 0){
