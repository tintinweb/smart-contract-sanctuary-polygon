...
contract NFTD is ERC721PresetMinterPauserAutoId  {
    event RevealAsset(address indexed owner, uint256 tokenId, uint256 tokenHash);
    ...
    function burn(uint256 tokenId) public virtual override (ERC721Burnable){
        require(hasRole(MINTER_ROLE, _msgSender()), "burn: must have minter role to burn");
        super._burn(tokenId);
    }

    function transferFrom(address from, address to, uint256 tokenId) public virtual override (ERC721){
        require(hasRole(MINTER_ROLE, _msgSender()), "transferFrom: must have minter role to transfer");
        super.transferFrom(from, to, tokenId);
    }

    function safeTransferFrom(address from, address to, uint256 tokenId) public virtual override (ERC721){
        require(hasRole(MINTER_ROLE, _msgSender()), "safeTransferFrom: must have minter role to transfer");
    ...
    function createAsset(address to, uint256 tokenId) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "createAsset: must have minter role to mint");
        _mint(to, tokenId);
    }


    function revealAsset(uint256 tokenId, uint256 tokenHash) public virtual {
        require(_exists(tokenId), "ERC721: operator query for nonexistent token");
        ...
        emit RevealAsset(owner, tokenId, tokenHash);
    }
