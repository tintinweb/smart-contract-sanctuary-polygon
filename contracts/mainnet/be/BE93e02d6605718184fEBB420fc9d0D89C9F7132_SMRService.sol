...
contract SMRService {
    ...
    string private constant _pendingStatus = "pending";
    string private constant _activeStatus = "active";     
   ...
   // mapping for all certificates collections
    ...
    function addCertificateContract(address contractAddress, bool activate) external onlyOwner {
        ...
        // last index
        certificates[contractAddress].index = addresses.length - 1;
    ...
    function removeCertificateContract(address contractAddress) external onlyOwner {
        ...
        uint256 index = certificates[contractAddress].index;
    ...
    function contractStatus(address contractAddress) view  virtual external returns (string memory) {
        if (certificates[contractAddress].ref == address(0)) 
            return _notExistStatus;
        else if  (certificates[contractAddress].active) 
            return _activeStatus;
        else return _pendingStatus;
    }     
