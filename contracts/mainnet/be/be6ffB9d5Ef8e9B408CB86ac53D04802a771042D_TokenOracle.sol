...
contract TokenOracle is Ownable, IOracle {
    ...
    function consult() external view override returns (uint256) {
        uint256 _priceTokenToStable = IPairOracle(oracleTokenStable).consult(token, (PRICE_PRECISION * (10**missingDecimals))); // 49999999
        uint256 _priceStableToUsd = IOracle(oracleStable).consult(); // 1000000
        return _priceStableToUsd * PRICE_PRECISION / _priceTokenToStable;
    }
