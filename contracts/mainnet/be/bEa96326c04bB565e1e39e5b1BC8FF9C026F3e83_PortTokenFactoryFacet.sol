...
contract PortToken is ERC20 {
    ...
    event EmergencyWithdrawn(address indexed user, uint256 amount, address[] _tokenAddrs, uint256[] _tokenAmounts);
    ...
    /// @notice using this function user can request emergency withdraw of internal tokens
    ///         token list should be prepared offchain, all tokens from list will be transfered to user
    ///         in amounts, proportional to provided amount of portfolio token
    ///         WARNING: any tokens not included into list will be effectively lost by user
    /// @param _amount amount of portfolio token to burn during withdraw process
    /// @param _tokenAddrs array of withdrawn token addresses
    function emergencyWithdraw(uint256 _amount, address[] memory _tokenAddrs) external {
        require(_tokenAddrs.length > 0, "No tokens provided for withdraw");
        uint256 totalSupl = totalSupply();
        _burn(_msgSender(), _amount); //burn itself will check balance

        uint256[] memory tokenAmounts = new uint256[](_tokenAddrs.length);

        for (uint256 i = 0; i < _tokenAddrs.length; i++) {
            uint256 userTokenAmount = (IERC20(_tokenAddrs[i]).balanceOf(address(this)) * _amount) / totalSupl;
            if (userTokenAmount == 0) continue;

            tokenAmounts[i] = userTokenAmount;
            IERC20(_tokenAddrs[i]).transfer(_msgSender(), userTokenAmount);
        }

        emit EmergencyWithdrawn(_msgSender(), _amount, _tokenAddrs, tokenAmounts);
    }
    ...
    function controllerMint(address _to, uint256 _amt) external onlyController {
        ITransferHookReceiver(controller).onTokenTransfer(address(this), address(0), _to, _amt);
        _mint(_to, _amt);
    }

    function controllerBurn(address _from, uint256 _amt) external onlyController {
        ITransferHookReceiver(controller).onTokenTransfer(address(this), _from, address(0), _amt);
        _burn(_from, _amt);
    }

    function burn(uint256 _amt) external {
        ITransferHookReceiver(controller).onTokenTransfer(address(this), _msgSender(), address(0), _amt);
        _burn(_msgSender(), _amt);
    }
    ...
    function externalCall(
        ...
        returndata = Address.functionCallWithValue(_target, _data, _value, _errorMessage);
        return returndata;
    }
}
...
contract PortTokenDeployerFacet is Modifiers {
    function deployPortToken(
        ...
        return address(new PortToken(address(this), manager, _name, _symbol));
    }
}
...
import "./PortTokenDeployerFacet.sol";

contract PortTokenFactoryFacet is Modifiers {
    event CreatedPortToken(address indexed token, address indexed manager, string name, string symbol);

    /// @notice create empty portfolio token, caller will be assigned as initial token manager
    /// @param _name token name
    /// @param _symbol token symbol
    function createPortToken(string calldata _name, string calldata _symbol) external returns (address portTokenAddr) {
        return LibPortTokenFactory.createPortToken(msg.sender, _name, _symbol);
    }

    /**
    @notice create portfolio token, configure fees and add tracked tokens with provided amounts per 1 portfolio token
    caller will be assigned as initial token manager
    passed fee values should be multiplied by FEE_DENOMINATOR
    @param _name token name
    @param _symbol token symbol
    @param _streamingFee streaming fee value
    @param _successFee success fee value
    @param _trackedTokens list of tracked token addresses
    @param _amounts corresponding tracked token amounts in base units
    */
    function createAndInitPortToken(
        string calldata _name,
        string calldata _symbol,
        uint16 _streamingFee,
        uint16 _successFee,
        address[] memory _trackedTokens,
        uint256[] memory _amounts
    ) external returns (address portTokenAddr) {
        portTokenAddr = LibPortTokenFactory.createPortToken(msg.sender, _name, _symbol);
        LibPortTokenFactory.initPortToken(msg.sender, portTokenAddr, _streamingFee, _successFee, _trackedTokens, _amounts);
    }

    /**
    @notice create portfolio token, configure fees and add tracked tokens 
    with amounts calculated from provided ratios of _targetPrice: amount will be calculated 
    using oracles to match value of amount of tracked token to targetPrice*ratio.
    Caller will be assigned as initial token manager.
    Passed fee values should be multiplied by FEE_DENOMINATOR.
    @param _name token name
    @param _symbol token symbol
    @param _streamingFee streaming fee value
    @param _successFee success fee value
    @param _targetPrice target price of one portfolio token at the creation moment
    @param _trackedTokens list of tracked token addresses
    @param _ratios corresponding tracked token value ratios multiplied by SHARE_DENOMINATOR 
    */
    function createAndInitPortTokenFromRatios(
        string calldata _name,
        string calldata _symbol,
        uint16 _streamingFee,
        uint16 _successFee,
        ...
        portTokenAddr = LibPortTokenFactory.createPortToken(msg.sender, _name, _symbol);
        uint256[] memory amounts = LibPriceInterface.estimateAmountsFromTotalPrice(_targetPrice, _trackedTokens, _ratios);
        LibPortTokenFactory.initPortToken(msg.sender, portTokenAddr, _streamingFee, _successFee, _trackedTokens, amounts);
    }

    /// @notice returns number of created portfolio toknes
    function portTokensLength() external view returns (uint256) {
        return LibPortTokenFactory.diamondStorage().portTokens.length;
    }

    /// @notice gets port token with provided id
    /// @param _id token index
    /// @return token address
    function portTokens(uint256 _id) external view returns (address) {
        LibPortTokenFactory.DiamondStorage storage ds = LibPortTokenFactory.diamondStorage();
        require(ds.portTokens.length > _id);

        return ds.portTokens[_id];
    }
}
...
library LibFee {
    bytes32 constant DIAMOND_STORAGE_POSITION = keccak256("portfolio.streamingfee.v1");
    ...
    function setStreamingFee(address _addr, uint16 _value) internal {
        DiamondStorage storage ds = diamondStorage();
        require(_value <= ds.feeLimits.maxStreamingFee && _value >= ds.feeLimits.minStreamingFee, "Invalid fee");

        StreamingFeeData storage d = streamingFeeDataForToken(_addr);
        d.value = _value;
    ...
    function setSuccessFee(address _addr, uint16 _value) internal {
        DiamondStorage storage ds = diamondStorage();
        require(_value <= ds.feeLimits.maxSuccessFee && _value >= ds.feeLimits.minSuccessFee, "Invalid fee");

        SuccessFeeData storage d = successFeeDataForToken(_addr);
        d.value = _value;
...
library LibPortTokenFactory {
    ...
    function initPortToken(
        ...
        LibFee.setStreamingFee(_portTokenAddr, _streamingFee);
        LibFee.setSuccessFee(_portTokenAddr, _successFee);

        for (uint256 i = 0; i < _trackedTokens.length; i++) {
            LibTrackedToken.addTrackedToken(_portTokenAddr, _trackedTokens[i], _amounts[i]);
        }

        require(LibPriceInterface.checkUtilityTokenShare(_portTokenOwner, _trackedTokens, _amounts), "The share of utility token is too small");
    }
}
...
library LibPriceInterface {
    ...
    function getTokenForPrice(address _addr, uint256 _amount) internal view returns (uint256 price) {
        ...
        if (os.feedType == 1) {
            ...
            price =
                (LibChainlinkUtils.getDerivedPrice(bs.feed, os.feed, IERC20Metadata(_addr).decimals()) * _amount) /
                (10**IERC20Metadata(ds.baseToken).decimals());
        } else if (os.feedType == 254) {
            price = (IOffchainOracle(os.feed).getRate(IERC20(ds.baseToken), IERC20(_addr), true) * _amount) / 10**IERC20Metadata(_addr).decimals();
    ...
    function checkUtilityTokenShare(
        ...
        if (portTokenOwner != address(0)  && ds.minUtilityTokenBalance>0 && IERC20(ds.utilityToken).balanceOf(portTokenOwner) >= ds.minUtilityTokenBalance) {
            return true;
        }
...
library LibTrackedToken {
    ...
    function getTargetAmount(address _addr, uint256 _amount) internal view returns (TrackedTokensList memory tl) {
        ...
        for (uint256 i; i < tl.length; i++) {
            ...
            tl.amounts[i] = (tc.targetAmounts[tc.trackedTokens.at(i)] * _amount) / (10**18);
        }
...
contract Modifiers {
    ...
    modifier onlyTokenManager(address _tokenAddr) {
        require(PortToken(_tokenAddr).manager() == msg.sender, "Only porfolio manager allowed");
