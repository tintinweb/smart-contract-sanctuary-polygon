...
contract UniswapV2QueryFacet {
    AppStore internal s;
    function getReservesByPairs(address[] calldata _pairs) external view returns (address[3][] memory, uint256[3][] memory) {
        address[3][] memory _addrs = new address[3][](_pairs.length);
        uint256[3][] memory _reserves = new uint256[3][](_pairs.length);
        for (uint i = 0; i < _pairs.length; i++) {
            (_reserves[i][0], _reserves[i][1], _reserves[i][2]) = IUniswapV2Pair( _pairs[i]).getReserves();
            IUniswapV2Pair _uniswapPair = IUniswapV2Pair(_pairs[i]);
            _addrs[i][0] = _uniswapPair.token0();
            _addrs[i][1] = _uniswapPair.token1();
            _addrs[i][2] = _pairs[i];
        }
        return (_addrs, _reserves);
    }
