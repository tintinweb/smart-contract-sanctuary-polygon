...
library WhitelistMintFacetStorage {
    using EnumerableSet for EnumerableSet.AddressSet;

    struct Layout {
        uint256 whitelistMintPrice;  // The mint price for a whitelisted address
        EnumerableSet.AddressSet whitelist;  // Set of whitelisted addresses
        uint256 whitelistMintStartTimestamp;
        uint256 whitelistMintEndTimestamp;
    }

    bytes32 internal constant STORAGE_SLOT =
        keccak256('storage.WhitelistMintFacetStorage');
    ...
    function isWhitelistMintPeriodNow() internal returns(bool) {
        Layout storage l = layout();
        return (l.whitelistMintStartTimestamp < block.timestamp) &&
            (block.timestamp < l.whitelistMintEndTimestamp);
    }

    function setWhitelistMintPrice(uint256 _whitelistMintPrice) internal {
        layout().whitelistMintPrice = _whitelistMintPrice;
    }

    function whitelistMintPrice() internal view returns (uint256) {
        return layout().whitelistMintPrice;
    }

    function whitelist() internal view returns (EnumerableSet.AddressSet storage) {
        return layout().whitelist;
    }

    function whitelistContains(address account) internal view returns(bool) {
        return whitelist().contains(account);
    }
}

abstract contract WhitelistMintStorage is Access {
    using EnumerableSet for EnumerableSet.AddressSet;

    /// @notice Emitted when whitelistMintStartTimestamp and whitelistMintEndTimestamp are set.
    /// @param start new value.
    /// @param end new value.
    event WhitelistMintPeriodSet(uint256 start, uint256 end);

    /// @notice Emitted when a whitelistMintPrice is set.
    /// @param whitelistMintPrice new value.
    event WhitelistMintPriceSet(uint256 whitelistMintPrice);
    ...
    function whitelistMintStartTimestamp() external view returns (uint256) {
        return WhitelistMintFacetStorage.layout().whitelistMintStartTimestamp;
    }

    function whitelistMintEndTimestamp() external view returns (uint256) {
        return WhitelistMintFacetStorage.layout().whitelistMintEndTimestamp;
    }

    /// @notice Set whitelistMintPrice
    function setWhitelistMintPeriod(uint256 start, uint256 end) external onlyOwner {
        require(
            (start < end) ||
            (start == 0 && end == 0),
            "invalid period"
        );
        WhitelistMintFacetStorage.Layout storage l = WhitelistMintFacetStorage.layout();
        l.whitelistMintStartTimestamp = start;
        l.whitelistMintEndTimestamp = end;
        emit WhitelistMintPeriodSet(start, end);
    }

    /// @notice Set whitelistMintPrice
    /// @param _whitelistMintPrice The new whitelistMintPrice
    function setWhitelistMintPrice(uint256 _whitelistMintPrice) external onlyOwner {
        WhitelistMintFacetStorage.setWhitelistMintPrice(_whitelistMintPrice);
        emit WhitelistMintPriceSet(_whitelistMintPrice);
    }

    /// @notice Get the whitelistMintPrice
    /// @return The current whitelistMintPrice
    function whitelistMintPrice() public view returns (uint256) {
        return WhitelistMintFacetStorage.whitelistMintPrice();
    }

    /// @notice Check if an account in the whitelist
    ...
    function whitelistContains(address account) external view returns(bool) {
        return WhitelistMintFacetStorage.whitelistContains(account);
    }
    ...
    function addToWhitelist(address[] memory accounts) external onlyOwner {
        EnumerableSet.AddressSet storage whitelist = WhitelistMintFacetStorage.whitelist();
        for (uint256 i = 0; i < accounts.length; i++) {
            if (whitelist.add(accounts[i])) {
    ...
    /// @param accounts An array of addresses to be removed from the whitelist
    function removeFromWhitelist(address[] memory accounts) external onlyOwner {
        EnumerableSet.AddressSet storage whitelist = WhitelistMintFacetStorage.whitelist();
        for (uint256 i = 0; i < accounts.length; i++) {
            if (whitelist.remove(accounts[i])) {
                emit RemovedFromWhitelist(accounts[i]);
            }
        }
    }

    function isWhitelistMintPeriodNow() external returns(bool) {
        return WhitelistMintFacetStorage.isWhitelistMintPeriodNow();
    }

    /// @notice Returns the number of addresses in the whitelist
    ...
    /// @notice Retrieves the address at the specified index in the whitelist
    /// @param index The index of the address in the whitelist
    /// @return The address at the specified index in the whitelist
    function getWhitelistAddressAt(uint256 index) external view returns (address) {
        EnumerableSet.AddressSet storage whitelist = WhitelistMintFacetStorage.whitelist();
        return whitelist.at(index);
    }

    /// @notice Returns all addresses in the whitelist
    /// @return An array containing all addresses in the whitelist
    function getAllWhitelistAddresses() external view returns (address[] memory) {
        EnumerableSet.AddressSet storage whitelist = WhitelistMintFacetStorage.whitelist();
...
contract NFTPassportMinter is
    Access,
    WhitelistMintStorage,
    CoinStorage,
    NFTPassportStorage
{
    using SafeERC20 for IERC20;

    event MintedToWhitelistedAddress(
    ...
    function initialize(address _nftPassport, address _coin, address ownerValue) external initializer {
        require(_nftPassport != address(0), "ZERO_ADDRESS");
        require(_coin != address(0), "ZERO_ADDRESS");

        __Context_init_unchained();
        __Ownable_init_unchained();
        __Access_init_unchained(ownerValue);

        __NFTPassportStorage_init_unchained(_nftPassport);
        __CoinStorage_init_unchained(_coin);
    }

    function mintByWhitelistedAddress() external {
        require(WhitelistMintFacetStorage.whitelistContains(msg.sender), "NOT_WHITELISTED");
        require(WhitelistMintFacetStorage.isWhitelistMintPeriodNow(), "NOT_WHITELIST_MINT_PERIOD");
        uint256 _whitelistMintPrice = WhitelistMintFacetStorage.whitelistMintPrice();
        IERC20(coin).safeTransferFrom(msg.sender, nftPassport.treasury(), _whitelistMintPrice);
        uint256 tokenId = nftPassport.mintRestricted(msg.sender);
        emit MintedToWhitelistedAddress(msg.sender, msg.sender, tokenId, _whitelistMintPrice);
    }
}
