...
import "../../interfaces/beefy/IBeefyUniV2Zap.sol";
import "../../interfaces/beefy/IBeefyV6.sol";
import "../../strategies/Common/StratManager.sol";
import "../../strategies/Common/FeeManager.sol";

contract StrategyPolygonQuickLPBeefyLaunchpool is StratManager, FeeManager {
    ...
    address constant public beefyZapper = address(0x540A9f99bB730631BF243a34B19fd00BA8CF315C);
    address constant public matic = address(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270);
    address constant public lpToken = address(0x65752C54D9102BDFD69d351E1838A1Be83C924C6);
    ...
    constructor(
        ...
        address _keeper,
        address _strategist,
        address _sphereFeeRecipient,
        address[] memory _outputToNativeRoute,
        address[] memory _outputToLp0Route,
        address[] memory _outputToLp1Route
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _sphereFeeRecipient) public {
        want = _want;
    ...
    // puts the funds to work (without calling the balanceOfWant function again)
    function _deposit(uint256 wantBal) internal {
    ...
    function addLiquidity() internal {
        ...
        IRewardPool(want).deposit(IERC20(lpToken).balanceOf(address(this)));
    }
    ...
    function callReward() public view returns (uint256) {
        ...
        return nativeOut.mul(1).div(1000).mul(callFee).div(MAX_FEE);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(want).safeApprove(beefyZapper, uint256(- 1));
        IERC20(output).safeApprove(beefyZapper, uint256(- 1));
        IERC20(matic).safeApprove(beefyZapper, uint256(- 1));
        IERC20(lpToken).safeApprove(address(want), uint256(- 1));
        IERC20(lpToken).safeApprove(unirouter, uint256(-1));
    }

    function _removeAllowances() internal {
        IERC20(want).safeApprove(rewardPool, 0);
        IERC20(output).safeApprove(unirouter, 0);
        IERC20(want).safeApprove(beefyZapper, 0);
        IERC20(output).safeApprove(beefyZapper, 0);
        IERC20(matic).safeApprove(beefyZapper, 0);
        IERC20(lpToken).safeApprove(want, 0);
...
contract StratManager is Ownable, Pausable {
    ...
    address public sphereFeeRecipient;
     ...
     * @param _sphereFeeRecipient address where to send Sphere's fees.
     */
    constructor(
        address _keeper,
        address _strategist,
        ...
        vault = _vault;
        sphereFeeRecipient = _sphereFeeRecipient;
    }
    ...
    /**
     * @dev Updates sphere fee recipient.
     * @param _sphereFeeRecipient new sphere fee recipient address.
     */
    function setSphereFeeRecipient(address _sphereFeeRecipient) external onlyOwner {
        sphereFeeRecipient = _sphereFeeRecipient;
    }
...
abstract contract FeeManager is StratManager {
    ...
    uint public sphereFee = MAX_FEE - STRATEGIST_FEE - callFee;

    function setCallFee(uint256 _fee) public onlyManager {
        require(_fee <= MAX_CALL_FEE, "!cap");
        
        callFee = _fee;
        sphereFee = MAX_FEE - STRATEGIST_FEE - callFee;
    }
