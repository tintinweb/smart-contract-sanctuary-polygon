...
contract SphereVesting is Ownable, ReentrancyGuard {
    ...
    function withdraw() public nonReentrant {
        ...
        uint256 monthsPassed = (block.timestamp -
            vestingData[msg.sender].startTimestamp) / (oneMonth);
        vestingData[msg.sender].startTimestamp += monthsPassed * (oneMonth);
        ...
        emit Release(msg.sender, releaseAmount, vestingData[msg.sender].pool, block.timestamp);
    }

    function releaseAmt(address _user) public view returns (uint) {
        uint256 monthsPassed = (block.timestamp -
            vestingData[_user].startTimestamp) / (oneMonth);
    ...
    function amountVestedInPool(Pool _pool) public view returns(uint256) {
        if(_pool == Pool.TEAM_A || _pool == Pool.TEAM_B || _pool == Pool.TEAM_C || _pool == Pool.TEAM_D){
            return teamVested;
        }
