...
library AmysStakingCo {
    ...
    function getMCPoolData(address chef) external view returns (IERC20[] memory lpTokens, uint256[] memory allocPoint) {
        uint len = IMasterHealer(chef).poolLength();

        lpTokens = new IERC20[](len);
        allocPoint = new uint256[](len);

        for (uint i; i < len; i++) {
            (bool success, bytes memory data) = chef.staticcall(abi.encodeWithSignature("poolInfo(uint256)", i));
            if (success) (lpTokens[i], allocPoint[i]) = abi.decode(data,(IERC20, uint256));
        }
    }
