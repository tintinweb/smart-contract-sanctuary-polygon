...
contract StarshipControl is IStarshipControl, WarpBase {
    ...
    function damage(
        ...
        if (_repair) {
            ...
            else shipInfo[_shipId].bridgeDmg -= _valueEngine;

            // Engine
            if (_valueBridge > shipInfo[_shipId].engineDmg) shipInfo[_shipId].engineDmg = 0;
            else shipInfo[_shipId].engineDmg -= _valueBridge;
        } else {
            shipInfo[_shipId].hullDmg += _valueHull;
            shipInfo[_shipId].bridgeDmg += _valueEngine;
            shipInfo[_shipId].engineDmg += _valueBridge;
