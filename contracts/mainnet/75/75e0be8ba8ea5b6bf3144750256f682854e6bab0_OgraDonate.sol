...
contract OgraDonate is Ownable {
    ...
    /**
     * @dev Checks if the donation process is finalized
     */
    modifier isntDonating() {
        require(!donating, "you can't use this function until the donation process is finished");
        _;
    }
     ...
     */
    function addOrRemoveRefugee(address refugee, bool add) public onlyOwner isntDonating returns (bool success){
        require(add != isStillRefugee[refugee], "refugee already with this bool");
     ...
     */
    function addRefugeeList(address[] memory refugees_) public onlyOwner isntDonating returns (bool success){
    ...
    function distributeDonations() public returns (bool success){
        if(donationArrayCounter == 0) {
            ...
            totalOGRADonated += donateCorrectArray(refugeesArrays[0]);
            ...
            totalOGRADonated += donateCorrectArray(refugeesArrays[donationArrayCounter]);
            donating = false;
            emit DistributionFinalized(donationsT);
            donationsT ++;
            donationArrayCounter = 0;
     ...
     */
    function donateCorrectArray(address[] memory refugees_) private returns(uint) {
        uint len = refugees_.length;
        uint donated;
        for (uint i = 0; i < len; i++) {
    ...
    function seeDonationCounters() public view returns(uint donationT_, uint arrayCounter_, uint donationArrayCounter_, uint singleAmount_,uint totalOGRADonated_) {
        return (donationsT, arrayCounter, donationArrayCounter, singleAmount, totalOGRADonated);
    }
}