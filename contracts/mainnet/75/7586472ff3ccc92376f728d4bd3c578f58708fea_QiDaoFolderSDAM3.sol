...
import "./interfaces/IAPool.sol";
import "./interfaces/IYVault.sol";
import "./QiDaoFolder.sol";
// import "forge-std/console.sol";

contract QiDaoFolderSDAM3 is QiDaoFolder {
  ...
  address internal constant SDAM3_VAULT_ADDRESS = 0x57Cbf36788113237D64E46f25A88855c3dff1691;
  address internal constant SDAM3_STAKEDAO_VAULT_ADDRESS = 0x7d60F21072b585351dFd5E8b17109458D97ec120;
  ...
  constructor() QiDaoFolder(MAI_ADDRESS, MAI_PAIR_ADDRESS, SDAM3_VAULT_ADDRESS, SDAM3_STAKEDAO_VAULT_ADDRESS, QI_ADDRESS, GELATO_OPS_ADDRESS) {
  }

  function _approveSwaps() internal override {
    IERC20(maiAddress).safeApprove(CURVE_MAI_3POOL_ADDRESS, type(uint).max);
    IERC20(AM3CRV_ADDRESS).safeApprove(CURVE_MAI_3POOL_ADDRESS, type(uint).max);
    IERC20(AM3CRV_ADDRESS).safeApprove(SDAM3_STAKEDAO_VAULT_ADDRESS, type(uint).max);
  }

  function _swapMaiForToken(uint _amountMai, uint _minAmountTokenReceived) internal override {
    uint am3crvBal = IERC20(AM3CRV_ADDRESS).balanceOf(address(this));
    uint tokenBal = IERC20(SDAM3_STAKEDAO_VAULT_ADDRESS).balanceOf(address(this));
    IAPool(CURVE_MAI_3POOL_ADDRESS).exchange(0, 1, _amountMai, 0);
    IYVault(SDAM3_STAKEDAO_VAULT_ADDRESS).deposit(IERC20(AM3CRV_ADDRESS).balanceOf(address(this)) - am3crvBal);
    uint gotAmount = (IERC20(SDAM3_STAKEDAO_VAULT_ADDRESS).balanceOf(address(this)) - tokenBal);
    require(
      gotAmount >= _minAmountTokenReceived,
      "too much slippage"
    );
    emit SwappedMaiForToken(_amountMai, gotAmount);
  }

  function _swapTokenForMai(uint _amountToken, uint _minAmountMaiReceived) internal override {
    uint am3crvBal = IERC20(AM3CRV_ADDRESS).balanceOf(address(this));
    uint maiBal = IERC20(maiAddress).balanceOf(address(this));
    IYVault(SDAM3_STAKEDAO_VAULT_ADDRESS).withdraw(_amountToken);
    IAPool(CURVE_MAI_3POOL_ADDRESS).exchange(1, 0, IERC20(AM3CRV_ADDRESS).balanceOf(address(this)) - am3crvBal, 0);
    uint gotAmount = (IERC20(maiAddress).balanceOf(address(this)) - maiBal);
    require(
      gotAmount >= _minAmountMaiReceived,
      "too much slippage"
    );
    emit SwappedTokenForMai(_amountToken, gotAmount);
  }
}
...
abstract contract QiDaoFolder is UUPSUpgradeable {
  ...
  function uniswapV2Call(address _sender, uint _amount0, uint _amount1, bytes calldata _params) external {
    ...
    IQiDaoVault(vaultAddress).payBackToken(vaultId, amtReceived);
    _unfold(type(uint).max, _minMaiReceivedPerToken);
  ...
  function vaultValue() view public returns (uint) {
    uint debt = vaultDebt();
    uint repaymentFees = debt * IQiDaoVault(vaultAddress).closingFee() / 10000;
  ...
  function _unfold(uint _maxAmountCollateralSold, uint _minMaiReceivedPerToken) internal {
    ...
    uint target = targetCdrPlusBuffer();

    for (uint i = 0; i < maxFoldsPerCall; i++) {
      if (vaultDebt() == 0) { break; }
      if (vaultCdr() > target) { break; }
      ...
      if (hypotheticalNewCdr > target) {
        uint currentCdr = vaultCdr();
        withdrawAmt = withdrawAmt * (target - currentCdr) / (hypotheticalNewCdr - currentCdr);
      }
  ...
  function _repayAvailableMaiDebt() internal {
    ...
    if (amt > 0) {
      IQiDaoVault(vaultAddress).payBackToken(vaultId, amt);
    }
  }
  ...
  function _getRebalanceType() view internal returns (RebalanceType rt) {
    ...
    } else if ((cdr > targetCdr + foldTriggerDeviation) && (availableBorrows() > minAmountToFold)) {
  ...
  function _collateralPrice() view internal returns (uint) {
    return IQiDaoVault(vaultAddress).getEthPriceSource();
  }
  ...
  function _availableBorrowsByTargetCdr() view internal returns (uint) {
    uint maxTotalBorrowsInCollateral = _vaultCollateralValue() * CDR_PRECISION / targetCdrPlusBuffer();
    return maxTotalBorrowsInCollateral - vaultDebt();
  }

  function targetCdrPlusBuffer() view public returns (uint) {
    if (targetCdr == type(uint).max) {
      return type(uint).max;
    }

    return targetCdr + (foldTriggerDeviation / 2);
  }
