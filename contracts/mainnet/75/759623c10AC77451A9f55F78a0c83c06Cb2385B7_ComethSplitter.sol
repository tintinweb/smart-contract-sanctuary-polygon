...
/**
 * @title OriumSplitter
 * @notice OriumSplitter is a contract that splits a value between addresses
 * according to a share.
 * @dev OriumSplitter is a Beacon deployed when a new Guild is created.
 * @author Orium Network Team - [email protected]
 */

contract OriumSplitter is Initializable, AccessControlUpgradeable {
    bytes32 public constant GUILD_ROLE = keccak256("GUILD_ROLE");
    bytes32 public constant VAULT_ROLE = keccak256("VAULT_ROLE");
    uint256 public constant EVENT_ID = 1;

    address public oriumTreasury;
    address public guildOwner;
    address public factory;
    address public scholarshipManager;
    address public vaultAddress;
    address public vaultOwner;

    uint256 public scholarshipProgramId;
    uint256 public platformId;
     ...
     * @param   _oriumTreasury address of the Orium Treasury where orium fees are sent.
     * @param   _guildOwner address of the Guild owner
     * @param   _scholarshipProgramId id of the scholarship program
     * @param   _factory address of the Orium Factory contract
     * @param   _platformId id of the platform where the Guild is deployed
    ...
    function initialize(
        ...
        guildOwner = _guildOwner;
        scholarshipProgramId = _scholarshipProgramId;
        factory = _factory;
        platformId = _platformId;
        oriumTreasury = _oriumTreasury;
        scholarshipManager = _scholarshipManager;
        vaultAddress = _vaultAddress;
        vaultOwner = _vaultOwner;

        uint256[] memory _shares = IScholarshipManager(_scholarshipManager).sharesOf(
            _scholarshipProgramId,
            EVENT_ID
        );

        shares = getSharesWithoutBorrower(_shares);

        beneficiaries = new address[](3);
        beneficiaries[0] = _vaultOwner;
        beneficiaries[1] = _guildOwner;
        beneficiaries[2] = _oriumTreasury;

        require(
            beneficiaries.length == shares.length,
            "OriumSplitter: beneficiaries and shares length mismatch"
        );

        __AccessControl_init();

        _setupRole(GUILD_ROLE, _guildOwner);
        _setupRole(VAULT_ROLE, _vaultOwner);
        _setupRole(DEFAULT_ADMIN_ROLE, _guildOwner);
    }

    /**
     * @notice  Split the balance of the contract between the guild and orium.
     * @dev     The guild share is calculated based on the scholarship program shares.
     */

    function split() public {
        address[] memory tokenAddresses = IOriumFactory(factory).getPlatformTokens(platformId);

        for (uint256 i = 0; i < tokenAddresses.length; i++) {
            uint256 _balance = IERC20(tokenAddresses[i]).balanceOf(address(this));
            if (_balance == 0) continue;
            for (uint256 j = 0; j < beneficiaries.length; j++) {
                uint256 _amount = (_balance * shares[j]) / 100 ether;
                if (_amount == 0) return;
                _releasePayment(tokenAddresses[i], beneficiaries[j], _amount);
            }
        }
    }

    function _releasePayment(address _token, address _beneficiary, uint256 _amount) internal {
        IERC20(_token).transfer(_beneficiary, _amount);
        emit Withdrawal(_token, _amount, _beneficiary);
    }

    /**
     * @notice returns Scholarship Program Shares recalculate with Orium Fee.
     * @dev    Orium fee share is added to the end of the array.
     * @param   _shares array of shares of the scholarship program.
     * @return  _sharesWithOriumFee array of shares of the scholarship program with orium fee.
     */
    function getSharesWithOriumFee(
        uint256[] memory _shares
    ) public view returns (uint256[] memory _sharesWithOriumFee) {
        uint256 _oriumFee = IOriumFactory(factory).oriumFee();
        uint256 _totalSharesWithoutOriumFee = 100 ether - _oriumFee;

        _sharesWithOriumFee = new uint256[](_shares.length + 1);

        for (uint256 i = 0; i < _shares.length; i++) {
            _sharesWithOriumFee[i] = (_shares[i] * _totalSharesWithoutOriumFee) / 100 ether;
        }

        _sharesWithOriumFee[_sharesWithOriumFee.length - 1] = _oriumFee;

        _sharesWithOriumFee[0] += 100 ether - sumShares(_sharesWithOriumFee);
    }

    /**
     * @notice  Sum shares of a scholarship program.
     * @dev     Sum all values of an array.
     * @param   _shares  array of shares of the scholarship program.
     * @return  _sum  sum of all values of the array.
     */
    function sumShares(uint256[] memory _shares) public pure returns (uint256 _sum) {
        for (uint256 i = 0; i < _shares.length; i++) {
            _sum += _shares[i];
        }
    }

    function getSharesWithoutBorrower(
        uint256[] memory _shares
    ) public view virtual returns (uint256[] memory _sharesWithoutBorrower) {
        uint256 _borrowerIndex = 1;

        uint256[] memory _sharesWithOriumFee = getSharesWithOriumFee(_shares);
        uint256 _sharesSumWithoutBorrower = 100 ether - _sharesWithOriumFee[_borrowerIndex];

        _sharesWithoutBorrower = _deleteValueFromArrayAndRecalculateShares(
            _borrowerIndex,
            _sharesWithOriumFee,
            _sharesSumWithoutBorrower
        );

        _sharesWithoutBorrower[0] += 100 ether - sumShares(_sharesWithoutBorrower);
    }

    function _deleteValueFromArrayAndRecalculateShares(
        uint256 _valueIndex,
        uint256[] memory _array,
        uint256 _sharesSumWithoutMissingPercentage
    ) internal pure returns (uint256[] memory _newArray) {
        _newArray = new uint256[](_array.length - 1);

        for (uint256 i = 0; i < _array.length; i++) {
            if (i == _valueIndex) continue;

            _newArray[i < _valueIndex ? i : i - 1] =
                (_array[i] * 100 ether) /
                _sharesSumWithoutMissingPercentage;
        }
    }
}
...
import { OriumSplitter } from "../base/OriumSplitter.sol";
import { IERC20 } from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
contract ComethSplitter is OriumSplitter {
    mapping(address => mapping(uint256 => uint256)) public nftAddressToTokenIdToNonce;
    mapping(uint256 => uint256) public nonceToDeadline;

    modifier onlyNftVault() {
        require(msg.sender == vaultAddress, "ComethSplitter: Only NFT vault");
        _;
    }
     ...
     * @param _duration uint64 duration of the rental
     * @param _nonce uint256 nonce of the rental offer
     * @param _feeAmount uint256 fee amount of the rental offer
     * @param _deadline uint256 deadline of the rental offer
     */
    function createRentalOffer(
        ...
        IRentalProtocol.RentalOffer memory rentalOffer = getRentalOfferStruct(
            _tokenId,
            _nftAddress,
            _duration,
            _feeAmount,
        ...
        nftAddressToTokenIdToNonce[_nftAddress][_tokenId] = _nonce;
        nonceToDeadline[_nonce] = _deadline;

        address rentalImplementation = IOriumFactory(factory).rentalImplementationOf(_nftAddress);
        IERC721(_nftAddress).approve(rentalImplementation, _tokenId);
        IRentalProtocol(rentalImplementation).preSignRentalOffer(rentalOffer);
    }
     ...
     * @param _nftAddress address of the NFT contract
     */
    function cancelRentalOffer(uint256 _tokenId, address _nftAddress) public onlyNftVault {
        address rentalImplementation = IOriumFactory(factory).rentalImplementationOf(_nftAddress);
        IRentalProtocol(rentalImplementation).cancelRentalOffer(
            nftAddressToTokenIdToNonce[_nftAddress][_tokenId]
        );
    }
     ...
     * @param _tokenId uint256 id of the NFT
     */
    function endRental(address _nftAddress, uint256 _tokenId) public onlyNftVault {
        address rentalImplementation = IOriumFactory(factory).rentalImplementationOf(_nftAddress);
        uint256 _end = IRentalProtocol(rentalImplementation).rentals(_nftAddress, _tokenId).end;
        if (_end < block.timestamp) {
            IRentalProtocol(rentalImplementation).endRental(_nftAddress, _tokenId);
        } else if (IRentalProtocol(rentalImplementation).endRentalPrematurelyRequests(_nftAddress, _tokenId) != address(0)){
            IRentalProtocol(rentalImplementation).endRentalPrematurely(_nftAddress, _tokenId);
        } else {
            revert("ComethSplitter: Rental not ended");
        }
    }
    ...
    function endRentalAndRelist(
        ...
        endRental(_nftAddress, _tokenId);
        createRentalOffer(_tokenId, _nftAddress, _duration, _nonce, _feeAmount, _deadline, _taker);
    }

    /**
     * @notice  Transfer the NFT back to the NFT vault
     * @dev     This function is called by the NFT vault when undelgating the NFT
     * @param   _tokenId  nft id
     * @param   _nftAddress  nft address
     */
    function unDelegateNft(address _nftAddress, uint256 _tokenId) external onlyNftVault {
        _recoverNftIfPossible(_nftAddress, _tokenId);
        IERC721(_nftAddress).transferFrom(address(this), vaultAddress, _tokenId);
    }

    /**
     * @notice  Get the Cometh rental offer struct
     * @dev     The guild share is calculated based on the scholarship program shares
     * @param   _tokenId nft id
     * @param   _nftAddress  nft address
     * @param   _duration  duration of the rental
     * @param   _feeAmount  fee amount
     * @param   _nonce  nonce using 0x protocol
     * @return  rentalOffer Cometh rental offer struct
     */
    function getRentalOfferStruct(
        uint256 _tokenId,
        address _nftAddress,
        uint64 _duration,
        uint256 _feeAmount,
        ...
        uint256 _deadLine,
        address _taker
    ) public view returns (IRentalProtocol.RentalOffer memory rentalOffer) {
        uint256[] memory programShares = IScholarshipManager(scholarshipManager).sharesOf(
            scholarshipProgramId,
            EVENT_ID
        );
        uint256[] memory sharesWithOriumFee = getSharesWithOriumFee(programShares);
        uint256 _sumSharesWithoutBorrower = 100 ether - sharesWithOriumFee[1];

        // convertion ether to basis points
        uint16 _basisPoints = uint16((_sumSharesWithoutBorrower * 100) / 1 ether);

        IRentalProtocol.NFT[] memory nfts = new IRentalProtocol.NFT[](1);

        nfts[0] = IRentalProtocol.NFT({
            token: _nftAddress,
            tokenId: _tokenId,
            duration: _duration,
        ...
        rentalOffer = IRentalProtocol.RentalOffer({
            maker: address(this),
            taker: _taker,
            nfts: nfts,
            feeToken: IOriumFactory(factory).getPlatformTokens(platformId)[0],
    ...
    function nonceOf(address _nftAddress, uint256 _tokenId) external view returns (uint256) {
        return nftAddressToTokenIdToNonce[_nftAddress][_tokenId];
    }

    function deadlineOf(uint256 _nonce) external view returns (uint256) {
        return nonceToDeadline[_nonce];
    }

    function _recoverNftIfPossible(address _nftAddress, uint256 _tokenId) internal {
        address rentalImplementation = IOriumFactory(factory).rentalImplementationOf(_nftAddress);
        NftState nftState = IOriumNftVault(vaultAddress).getNftState(_nftAddress, _tokenId);

        if (nftState == NftState.LISTED) {
            cancelRentalOffer(_tokenId, _nftAddress);
            /**
             * @dev carefully consider the impacts before deleting the line above!
             * @dev this event is needed for subgraph
             */
            IScholarshipManager(scholarshipManager).onRentalOfferCancelled(
                _nftAddress,
                _tokenId,
                vaultAddress,
                scholarshipProgramId
            );
        } else if (nftState == NftState.CLAIMABLE) {
            uint256 _end = IRentalProtocol(rentalImplementation).rentals(_nftAddress, _tokenId).end;
            if (_end < block.timestamp) {
                IRentalProtocol(rentalImplementation).endRental(_nftAddress, _tokenId);
            } else {
             ...
             * @dev this event is needed for subgraph
             */
            IScholarshipManager(scholarshipManager).onRentalEnded(
                _nftAddress,
                _tokenId,
                vaultAddress,
                scholarshipProgramId
            );
        } else if (nftState == NftState.IDLE) {
            bool _isInvalidNonce = IRentalProtocol(rentalImplementation).invalidNonce(
                address(this),
                nftAddressToTokenIdToNonce[_nftAddress][_tokenId]
            );

            if (_isInvalidNonce) return;
            if (nftAddressToTokenIdToNonce[_nftAddress][_tokenId] == 0) return; // do not cancel if the offer was never created

            /**
             * @dev this is needed to garantee that all the rental offers are cancelled
             *      before the NFT is recovered. This is important for the subgraph
             *      carefully consider the impacts before deleting the line above!
             */
        ...
        } else if (nftState == NftState.BORROWED) {
            revert("ComethSplitter: NFT is borrowed");
        }
    }
}
