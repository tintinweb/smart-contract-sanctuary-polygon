...
/// @notice This contract is use to save all the i  nformation related to Lending/Earn Section.
contract LendingControl is AccessControl, ReentrancyGuard, ILending {
    ///@dev developer role created
    bytes32 public constant DEV_ROLE = keccak256("DEV_ROLE");

    bool private close;
    uint16 public interesLenders = 1500; // interest to pay in lending 15% en base 10 mil
    uint32 private daysBeforePenalization = 30 days;
    uint32 private extendPenalization = 20 days;
    uint256 public requestId = 54;

    ICals private calc;

    /// @dev number of deposits in lending.
    uint256 public numberDeposits = 0;
    ...
    function getLenderInfo(address wallet, uint256 id)
        public
        view
        returns (
            uint256 _amount,
            uint256 _deposit,
            address _token,
            uint96 _lastCalcTime,
            uint256 _payPerSecond,
            uint96 _withdraw
        )
    {
        return (
            lenders[wallet][id].amount,
    ...
    function getRewardsClaimed(address wallet, uint256 id)
        public
        view
        returns (uint256 rewards, uint96 claimTime, uint96 lastTimeClaim)
    {
    ...
    function getLastSetClaimRewards(address _wallet, uint256 _idLending)
        public
        view
        returns (uint96 lastSetClaimRewards)
    {
    ...
    function addRegistry(uint256 id, address wallet, uint256 _amount, address _token, uint256 _payPerSecond)
        public
        onlyLending
        nonReentrant
    {
    ...
    /// @param _amount - the amount deposited by the user.
    /// @param _payPerSecond - The payment per second that corresponds to this user.
    function updateRegistry(uint256 id, address wallet, uint256 _amount, uint256 _payPerSecond)
        public
        onlyLending
        nonReentrant
    {
    ...
    function updateLastTimeClaim(address wallet, uint256 idLending, uint256 _timeClaim)
        public
        onlyLending
        nonReentrant
    {
    ...
    /// @param idLending - the Lending ID created for the user.
    /// @param _timeClaim - The date of the last time the user's interest payment was calculated.
    function updatePendingRewards(address wallet, uint256 idLending, uint256 _timeClaim)
        public
        onlyLending
        nonReentrant
    {
    ...
    function updateClaimed(uint256 id, address wallet, uint256 _rewards, uint256 _claimTime)
        public
        onlyLending
        nonReentrant
    {
    ...
    function updateOnReinvestmentRewards(
        address _wallet,
        uint256 _idLending,
        uint256 _amount,
        uint256 _claimed,
    ...
    function createRequest(address _wallet, uint256 _amount, uint256 _rewards, uint8 _flag)
        public
        onlyLending
        returns (uint256)
    {
    ...
    function getRequest(address _wallet, uint256 _id)
        public
        view
        returns (uint256 _amount, uint256 _rewards, Status _state, uint96 _date)
    {
        return (
    ...
    function UpdateInteresAccumulated(address _wallet, uint256 _id) public onlyLending nonReentrant {
    ...
    function SaveInteresAccumulated() private {
        ...
        for (uint256 i = 1; i <= numberDeposits;) {
            ...
            lenders[walletsRegister[i]][i].payPerSecond =
                calc.calcInterestForSecond(lenders[walletsRegister[i]][i].amount, interesLenders);
    ...
    function updatePosition(address wallet, uint256 id, Data calldata info, address _token)
        public
        onlyLending
        nonReentrant
    {
        ...
        lenders[wallet][id].amount = info.amount;
        lenders[wallet][id].pendRewards = info.pendRewards;
        lenders[wallet][id].claimed = info.claimed;
        lenders[wallet][id].depositTime = info.depositTime;
        lenders[wallet][id].token = _token;
        lenders[wallet][id].lastCalcTime = info.lastCalcTime;
        lenders[wallet][id].claimTime = info.claimTime;
        lenders[wallet][id].lastSetClaimRewards = info.lastSetClaimRewards;
        lenders[wallet][id].payPerSecond = info.payPerSecond;
        lenders[wallet][id].daysLeft = info.daysLeft;
        lenders[wallet][id].withdrawTime = info.withdrawTime;
        numberDeposits++;
        walletsRegister[numberDeposits] = wallet;
    }
    ...
    function updateRequest(
        ...
        uint96 _date,
        uint96 _lastCalcTime
    ) public onlyLending nonReentrant {
        if (close) {
            revert("update close");
        }
        uint256 _id = ++requestId;
        ...
        request[_wallet][_id]._date = _date;
        request[_wallet][_id].lastCalcTime = _lastCalcTime;
    }

    function updateClose() public onlyDev {
        close = true;
    }
}
