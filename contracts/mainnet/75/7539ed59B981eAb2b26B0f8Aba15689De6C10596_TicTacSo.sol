...
contract TicTacSo {
    ...
    mapping(address => uint256[]) public runningGames;
    mapping(address => uint256[]) public archivedGames;
    ...
    function startGame(address to, uint8 size) public returns (uint256) {
        ...
        for (uint i = index; i < invites[msg.sender].length - 1; i++) {
            invites[msg.sender][i] = invites[msg.sender][i + 1];
        }
        invites[msg.sender].pop();

        for (uint i = index2; i < invited[to].length - 1; i++) {
            invited[to][i] = invited[to][i + 1];
        }
        invited[to].pop();
        ...
        runningGames[msg.sender].push(lastId);
        runningGames[to].push(lastId);
    ...
    function makeMove(
        ...
        if (result != 0) {
            ...
            address from = gameData[gameId].from;
            address to = gameData[gameId].to;

            // Remove from active games
            bool destr = false;
            for (uint i = 0; i < runningGames[from].length - 1; i++) {
                if (runningGames[from][i] == gameId) {
                    destr = true;
                }
                if (destr)
                    runningGames[from][i] = runningGames[from][i + 1];
            }
            runningGames[from].pop();
            archivedGames[from].push(gameId);

            bool destr2 = false;
            for (uint i = 0; i < runningGames[to].length - 1; i++) {
                if (runningGames[to][i] == gameId) {
                    destr2 = true;
                }
                if (destr2)
                    runningGames[to][i] = runningGames[to][i + 1];
            }
            runningGames[to].pop();
            archivedGames[to].push(gameId);
            return true;
        }
        return false;
    }
