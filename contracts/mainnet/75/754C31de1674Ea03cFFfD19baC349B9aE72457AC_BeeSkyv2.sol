...
contract BeeSkyv2 is Ownable {
  ...
  address public USDT = 0x9619628Be369731983a043a6F39F4bc87612ff1F;
  address public GLOVERNANCE = 0xc8D52616421deab345db1f95F42a2FbFeC34c5d3;
  ...
  uint256 public period = 2400;
  uint256 public votedeley = 2400;
  ...
  constructor() Ownable(msg.sender) {
    adminaddress[0] = 0x2070214ceaCb0ED07fa013afAfa3C6b69B8c7311; //devfee wallet
    adminaddress[1] = 0xC81F4004D0eccd7291e619cd5a13333844621FdC; //marketing wallet
    adminaddress[2] = 0xcDc8D8190a0891133089B349b37CE25EFbB916c6; //caller wallet
    adminaddress[3] = 0x4F5bf7F3407c207eF9a25171412f3551D8DB7765; //staking wallet
    adminaddress[4] = 0x10fa09Ead659e5a8C53Aa7657A9Ec017304B8CDc; //**topshared wallet
    adminaddress[5] = 0x50C19654229CCaF0A523A832f360349534441db2; //**dividend wallet
    adminaddress[6] = 0x5A46C086E3845B59E05820516E8D3d61E9d8D31E; //ad fee
    adminaddress[7] = 0xf3F20bF087aEca9BDCa750dCb1491e199e9BeF46; //ad cid
    ...
    fee.refer[5] = 5;
    fee.withdraw = 0;
    Package memory newpack1 = Package( 50, 40, 0,"Jade");
    Package memory newpack2 = Package( 500, 450, 0,"Ruby");
    Package memory newpack3 = Package( 1000, 1000, 0,"Pearl");
    Package memory newpack4 = Package( 5000, 5500, 0,"Emerald");
    Package memory newpack5 = Package( 10000, 12000, 0,"Exclusive");
    Package memory newpack6 = Package( 30000, 39000, 0,"Millionaire");
  ...
  function setUpAdmin(address[] memory _admin) public onlyOwner {
    require(_admin.length == 8,"revert by arguments length");
    ...
    adminaddress[6] = _admin[6]; //dividend wallet
    adminaddress[7] = _admin[7]; //dividend wallet
  }
  ...
  function directBuy(uint256 _amountusd) external noReentrant() {
    ...
    if(!isAdmin[msg.sender]){
      usd.transfer(adminaddress[6],takefee(_amountusd,30,100));
  ...
  function directSell(uint256 _amounttoken) external noReentrant() {
    ...
    if(!isAdmin[msg.sender]){
      usd.transfer(adminaddress[6],takefee(receiveamount,fee.withdraw,fee.denominator));
  ...
  function claimTopShared(uint256 _month) external returns (bool) {
    ...
    if(msg.sender==owner){
      if(top_adr[0]==msg.sender){ spenderamount = takefee(spenderamount,100,100); }
      else if(top_adr[1]==msg.sender){ spenderamount = takefee(spenderamount,50,100); }
      else if(top_adr[2]==msg.sender){ spenderamount = takefee(spenderamount,30,100); }
      else if(top_adr[3]==msg.sender){ spenderamount = takefee(spenderamount,15,100); }
  ...
  function safeusdtransfer(address _recipient,uint256 _amount) internal {
    ...
    if(_recipient==address(0)){ _recipient = adminaddress[7]; }
  ...
  function remainingvotetimer() public view returns (uint256) {
    if(votestart.add(votedeley)>block.timestamp){
      return votestart.add(votedeley).sub(block.timestamp);
    }else{
      return 0;
    }
  }
