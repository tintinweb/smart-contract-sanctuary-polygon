...
contract PartsSeller is WarpBase {
    ...
    mapping(PartType => uint256) partTypeMultiplier;
    ...
    function buyMultipleParts(
        uint256[] calldata _amounts,
        address[] calldata _principles,
        PartType[] calldata _parts
    ) external whenNotPaused {
        for (uint256 i = 0; i < _parts.length; i++) {
            buyPart(_amounts[i], _principles[i], _parts[i]);
        }
    }
    ...
    function buyPart(
        ...
        if (partTypeMultiplier[_part] != 0)
            actualValue = (actualValue * partTypeMultiplier[_part]) / 100;
    ...
    /** @dev set part multiplier
        @param _part {address}
        @param _multiplier {uint256}
     */
    function setPartMultiplier(PartType _part, uint256 _multiplier) external onlyOwner {
        partTypeMultiplier[_part] = _multiplier;
    }

    /** @dev View function to get principles for dApp */
    function getPrinciples() public view returns (Principle[] memory) {
    ...
    /** @dev view function to get part multipliers */
    function getPartMultipliers() public view returns (uint256[4] memory info) {
        info[0] = partTypeMultiplier[PartType.BRIDGE];
        info[1] = partTypeMultiplier[PartType.HULL];
        info[2] = partTypeMultiplier[PartType.ENGINE];
        info[3] = partTypeMultiplier[PartType.FUEL];
    }

    /** @dev get both */
    function getMultipliers() external view returns (Principle[] memory, uint256[4] memory) {
        return (getPrinciples(), getPartMultipliers());
    }
}
