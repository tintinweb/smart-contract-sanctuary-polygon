...
interface INFTSaleFactory {
  ...
  function createProject(uint32, uint32, uint32, uint128[2] calldata, uint32, uint128[] calldata, uint128[] calldata, uint128[3] calldata, address) external returns (address);
...
contract NFTSale {
  ...
  uint32 public calculation;
  uint32 public buyLimit; // buy limit per booster
  uint8 public nftType;

  uint64 public whitelistLength;
  uint64 public nftTotalSale;
  ...
  mapping(uint16 => uint128) public minKom;
  mapping(uint8 => Detail) public nft;

  mapping(address => Invoice[]) public invoices;
  mapping(address => string) public recipient;

  mapping(uint8 => uint32) public nftSold;
  mapping(uint8 => mapping(uint16 => uint32)) public nftSoldPerRound;

  mapping(address => mapping(uint8 => mapping(uint16 => uint32))) public purchasePerNftPerRound;
  ...
  event NFTBought(uint16 indexed booster, uint8 indexed nftType, address indexed buyer, uint128 nftReceived, uint128 buyAmount, uint128 feeCharged);
  ...
  function initialize(
    ...
    for(uint16 i=1; i<=3; ++i){
      ...
      if(i < 3){
        booster[i].end = booster[i].start + _duration;
        minKom[i] = _minKom[i-1];
      }
    }

    uint32 total;
    for(uint8 i=0; i<nftType; ++i){
      nft[i].sale = _sale[i];
  ...
  /**
    ...
    */
  function getUserAllocation(address _user, uint16 _booster) external view returns(uint32){
    if(_user == factory.savior() || _user == owner){
      uint32 sold;
      for(uint8 i=0; i<nftType; ++i){
        sold += nftSold[i];
      }

      return uint32(nftTotalSale) - sold;
    }

    if((_booster < 3 && !whitelist[_user] && getUserTotalStaked(_user) < minKom[_booster]) || _booster == 0 || _booster > 3) return 0;

    uint32 userPurchasePerRound;
    for(uint8 i=0; i<nftType; ++i){
  ...
  /**
    ...
    */
  function getUserNftBought(address _user, uint8 _nftType) external view returns (uint128 total){
    for(uint16 i=1; i<=3; ++i){
  ...
  function amountInCalc(
    ...
    uint8 _nftType,
    uint16 _running
  ) private view returns(uint128 amountInFinal, uint32 nftReceivedFinal) {
    uint32 left = uint32(nft[_nftType].sale) - nftSold[_nftType];
    require(left > 0, "!sale");
    ...
    uint32 limit = buyLimit;
    if(userPurchasePerRound + _nftReceived > limit) amountInFinal = (uint128(limit) - userPurchasePerRound) * nftPrice;

    require(amountInFinal > 0, "max");
    nftReceivedFinal = uint32(amountInFinal / nftPrice);
  }
  ...
  /**
    ...
    */
  function buyToken(uint8 _nftType, uint32 _nftAmount) external isBoosterProgress isNotPaused {
    uint16 running = boosterProgress();
    
    if(running < 3) require(checkUserStaked(msg.sender, running), "!eligible");

    uint64 buyerId = setBuyer(msg.sender);
    
    (uint128 amountInFinal, uint32 nftReceivedFinal) = amountInCalc(_nftAmount, msg.sender, _nftType, running);
    
    uint128 feeCharged = (amountInFinal * booster[running].fee_d2) / 10000;

    invoices[msg.sender].push(Invoice(buyerId, running, _nftType, uint32(block.timestamp), nftReceivedFinal, amountInFinal, feeCharged));
  ...
  /**
    ...
    * @param _nftAmount NFT amount to buy
    */
  function teamBuy(uint8 _nftType, uint32 _nftAmount) external isBoosterProgress isNotPaused {
    uint16 running = boosterProgress();
    uint32 left = uint32(nft[_nftType].sale) - nftSold[_nftType];
    ...
    invoices[msg.sender].push(Invoice(buyerId, running, _nftType, uint32(block.timestamp), _nftAmount, 0, 0));
  ...
  /**
    * @dev Check buyer allocation
    * @param _user User address
    * @param _booster Booster number
    */
  function checkUserStaked(address _user, uint16 _booster) private returns(bool) {
    if(userStaked[_user] == 0) userStaked[_user] = getUserTotalStaked(_user);

		if(_booster == 1 && whitelist[_user]) return true;
		
    if(userStaked[_user] < minKom[_booster]) return false;
  ...
  function setRecipient(string calldata _recipient) external isNotPaused  {
    ...
    require(uint32(nftTotalSale) - sold > 0 && bytes(_recipient).length != 0, "bad");
  ...
  // **** ADMIN AREA ****
	...
	/**
		...
		* @param _user User address
		*/
	function setWhitelist(address[] calldata _user) external onlyOwner {
		uint64 length = whitelistLength;

		for(uint64 i=0; i<_user.length; ++i){
			if(whitelist[_user[i]]) continue;

			whitelist[_user[i]] = true;
			++length;
		}

		whitelistLength = length;
	}
	...
	/**
		...
		* @param _user User address
		*/
	function unsetWhitelist(address[] calldata _user) external onlyOwner {
		uint64 length = whitelistLength;

		for(uint64 i=0; i<_user.length; ++i){
			if(!whitelist[_user[i]]) continue;

			whitelist[_user[i]] = false;
			--length;
		}
  ...
  /**
    ...
    * @param _calculation Epoch date to start buy allocation calculation
    */
  function setCalculation(uint32 _calculation) external onlyOwner {
    require(uint32(block.timestamp) < calculation, "bad");
  ...
  /**
    ...
    * @param _minKom KOM staked minimum to join sale
    */
  function setMinKom(uint16 _booster, uint128 _minKom) external onlyOwner {
    require(uint64(block.timestamp) < booster[_booster].start, "bad");

    minKom[_booster] = _minKom;
  }
  ...
  /**
    ...
    * @param _buyLimit NFT Limit to buy
    */
  function setBuyLimit(uint32 _buyLimit) external onlyOwner isBoosterProgress {
    buyLimit = _buyLimit;
  }
  ...
  /**
    ...
    * @param _start Epoch date to start round 1
    */
  function setStart(uint32 _start, uint32 _duration) external onlyOwner {
  ...
  /**
    * @dev Set Sale
    * @param _sale Amount token project to sell
    */
  ...
  /**
    ...
    * @param _fee_d2 Fee project percent in 2 decimal
    */
  function setFee_d2(uint16 _booster, uint128 _fee_d2) external onlyOwner {
    require(uint64(block.timestamp) < booster[_booster].start && _booster > 0 && _booster <= 3, "bad");
...
contract NFTSaleFactory is INFTSaleFactory{
  ...
  function createProject(
    ...
    project = address(new NFTSale());

    allProjects.push(project);
      
    NFTSale(project).initialize(
      _calculation,
      _start,
      _duration,
      _minKom,
