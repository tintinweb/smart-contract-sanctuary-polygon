    ...
    OperatorFilterer
{
    ...
    /// @notice Claim mapping
    mapping(string => bool) private serialToActive;
    ...
    modifier canClaim(string memory _serial) {
        ...
        require(!serialToActive[_serial], "Already claimed.");
        _;
    }

    function initialize() public virtual initializer {
        __AccessControl_init();
        __ERC721B_init("Kandi", "KNDORB");
        __ERC2981_init();
    ...
    /**
     * @notice Claim a kandi collectible
     * @param _serial - The token ids
     */
    function claim(string calldata _serial) external canClaim(_serial) {
        // Set claimed
        serialToActive[_serial] = true;
        // Process mint
        _mint(_msgSender(), _owners.length);
        _totalMinted += 1;
    }

    /**
     * @notice Claim a kandi collectible as owner on behalf of a use, send to `to`
     * @param _serial - The token ids
     * @param _to - The transfer address
     */
    function claimTransfer(
        string calldata _serial,
        address _to
    ) external canClaim(_serial) onlyRole(OWNER_ROLE) {
        // Set claimed
        serialToActive[_serial] = true;
        // Process mint
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public override(ERC721BUpgradeable) onlyAllowedOperator(from) {
    ...
    /**
     * @notice Sets the valid serials to mint
     * @param _serials The serial numbers
     */
    function setSerialConfiguration(
        string[] calldata _serials
    ) external onlyRole(OWNER_ROLE) {
        unchecked {
            for (uint256 i; i < _serials.length; ++i) {
                serialToActive[_serials[i]] = false;
            }
        }
    }
