...
contract InterGalacticCoin is Context, IERC20, Ownable {
        ...
        )
    {
        ...
        tTransferAmount = tTransferAmount.sub(tMarketing).sub(tPrizePool);
        ...
        )
    {
        ...
        uint256 rPrizePool = tPrizePool.mul(currentRate);
    ...
    function calculatePrizePoolFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(_prizePoolFee).div(10**2);
    }
        ...
        lockTheSwap
    {
        ...
        uint256 forPrizePool = contractTokenBalance.sub(forLiquidity).sub(
            forMarketing
        );
    ...
    function sendToPrizePool(uint256 tPrizePool) private{
        uint256 currentRate =  _getRate();
        uint256 rPrizePool = tPrizePool.mul(currentRate);
        _rOwned[prizePoolWallet] = _rOwned[prizePoolWallet].add(rPrizePool);
        _rOwned[address(this)] = _rOwned[address(this)].sub(rPrizePool);
        if(_isExcluded[prizePoolWallet])
            _tOwned[prizePoolWallet] = _tOwned[prizePoolWallet].add(tPrizePool);
    }
