...
contract GotchiSitter is TokenRecover {
                ...
                for(uint256 j = 0; j < _pets.length; j++){
                    if (
                        ...
                        IAavegotchiFacet(aavegotchiFacetAddress).isPetOperatorForAll(
                            IAavegotchiFacet(aavegotchiFacetAddress).ownerOf(_pets[j]),
    ...
    function shouldChargeDailyFee() external view returns (bool canExec, bytes memory execPayload) {
        ...
        execPayload = abi.encodeWithSelector(this.chargeDailyFee.selector);

        return (canExec, execPayload);
    }

    function chargeDailyFee() external {
