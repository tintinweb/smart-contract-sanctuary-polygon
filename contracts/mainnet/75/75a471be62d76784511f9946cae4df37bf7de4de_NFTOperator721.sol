    ...
    ReentrancyGuardUpgradeable
{
    ...
    CountersUpgradeable.Counter private _itemsMelted;
    ...
    function createGameItem(
        ...
        uint256 tokenId = INFT721(nftContract).createToken(_tokenURI);
        ...
        _executorOnly
    {
        ...
        IERC20Upgradeable ERC20Interface = IERC20Upgradeable(XpndAddress);
        ERC20Interface.safeTransferFrom(msg.sender, TreasuryAddress, charge);
    ...
    function meltToken(address nftContract, uint256 _tokenId)
        public
        payable
        nonReentrant
        whenNotPaused
        _executorOnly
    {
        INFT721(nftContract).burnToken(_tokenId);

        uint256 _itemIdToMelt = tokenIdToItemId[_tokenId];
        idToGameItem[_itemIdToMelt].owner = address(0);
        if (idToGameItem[_itemIdToMelt].sold == true) {
            idToGameItem[_itemIdToMelt].sold = false;
            _itemsSold.decrement();
        }
        idToGameItem[_itemIdToMelt].melted = true;
        _itemsMelted.increment();
    }
    ...
    function fetchGameItems() public view returns (GameItem[] memory) {
        ...
        for (uint256 i = 0; i < itemCount; i++) {
            if (
                idToGameItem[i + 1].owner == address(0) &&
                idToGameItem[i + 1].melted == false
            ) {
    ...
    function fetchMyNFTs() public view returns (GameItem[] memory) {
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (
                idToGameItem[i + 1].owner == msg.sender &&
                idToGameItem[i + 1].melted == false
            ) {
                itemCount += 1;
            }
    ...
    function fetchItemsCreated() public view returns (GameItem[] memory) {
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (
                idToGameItem[i + 1].seller == msg.sender &&
                idToGameItem[i + 1].melted == false
            ) {
                itemCount += 1;
            }
