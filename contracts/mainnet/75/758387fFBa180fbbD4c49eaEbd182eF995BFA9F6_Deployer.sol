...
contract Deployer {
	...
	// after deploy
	function deployAddressProvider_1() public onlyOwner returns (AddressProvider) {
		// deploy address provider
		addressProvider = new AddressProvider();
		addressProvider.setDAO(address(this));
		addressProvider.setEmergencyAdmin(address(this));
		return addressProvider;

	}

	function deployOracleMaster_2() public onlyOwner returns (OracleMaster) {
	...
	function deployHedgeToken(
		...
		HedgeToken token = new HedgeToken(address(hedgeCore), name_, symbol_, decimals_);
		return token;
	}
