...
// File contracts/aggregators/Aggregator.sol
...
// File contracts/aggregators/Airnfts.sol
...
contract Airnfts is Aggregator, ERC721Holder {
  ...
  /**
   */
  function buyProxy(uint256 _id, address _receipt) external payable onlyFibswap returns (bool) {
    require(Address.isContract(target), "!contract");

    // Try to execute the callData
    // the low level call will return `false` if its execution reverts
    bool success;
    (success, ) = target.call{value: msg.value}(abi.encode("buy(uint)", _id));
