 ...
 */
contract AidiRaffle is AidiWithdrawable, VRFConsumerBase, AccessControl {
    ...
    )
  {
    ...
    fee = 5 * 10 ** 14; // 0.0005 LINK (Varies by network)
...
// Creating Rafflee 
  ...
  function drawWinners(bytes32 _id) external {
    ...
    require((_raffle.end > 0 && _entries.length >= _raffle.minEntriesForDraw) || (_raffle.end == 0 && _raffle.owner == msg.sender) || (_raffle.end > 0 && block.timestamp > _raffle.end), "Raffle's minimum entry requirement for drawing not met or the raffle entry period is not over yet.");
    ...
    if (_raffle.entryFeesCollected > 0) {
        ...
        if (_prizePerEachWinner > 0) {
                for(uint256 i = 0; i < _numberOfwinners; i++) {
                    uint256 _winnerIdx = _random(_entries.length);
                    address _winner = address(_entries[_winnerIdx]);
                    _raffle.winners.push(_winner);
                    _entryToken.safeTransfer(_winner, _prizePerEachWinner);
                }
        }
  ...
  function _random(uint256 length) private view returns (uint256) {
    return uint256((randomResult % length) + 1);
  }
