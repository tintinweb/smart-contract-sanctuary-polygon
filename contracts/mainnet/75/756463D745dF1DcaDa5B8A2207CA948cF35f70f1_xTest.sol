...
interface AntiSnipe {
    ...
    function withdrawToAddress(address destination, uint256 amount) external;
...
contract xTest is IERC20 {
    ...
    address constant public USDCE = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    IERC20 constant public IERC20_USDCE = IERC20(USDCE);
    ...
    TaxWallets public _taxWallets = TaxWallets({
        xtf: 0x2F0a436176f71908e2505c5F7d4663e24871ce73,
        treasury: 0xc3Bf4CC50bBA34af9e796F34472505495988E21D,
        liquidity: 0x1Ff5d1e5eC8102B330d34F6DD70364dE0952bE74
    });
    ...
    uint256 public totalTimesRebased;
...
//===============================================================================================================
    ...
    function setTaxes(uint16 buyFee, uint16 sellFee, uint16 transferFee, uint16 piTaxBonus, uint16 referrerBonus, uint16 circuitBreakerTax) external onlyOwner {
        ...
        _taxRates.circuitBreakerTax = circuitBreakerTax;
    }
    ...
    function setCircuitBreakerEnabled(bool enabled) external onlyOwner {
        ...
        if (enabled) {
            ...
            lastTradeBlockPrice = (IERC20_USDCE.balanceOf(lpPair) * 10**18) / balanceOf(lpPair);
        }
    }
    ...
    function getPendingRebases() public view returns (uint256) {
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        require(initialized, "Initialize the rest of the constructor first.");
        bool buy = false;
        bool sell = false;
        bool other = false;
        if (lpPairs[from]) {
    ...
    function runRebase(uint256 _maxRebasesPerTX) internal {
        if (autoRebaseEnabled
           && block.timestamp >= autoRebaseLastTriggered + rebaseTimeInSeconds
            ...
            totalTimesRebased += rebaseAmounts;
    ...
    function finalizeTransfer(address from, address to, uint256 amount, bool buy, bool sell, bool other) internal returns (bool) {
        ...
        if (_hasLimits(from, to)) {
            ...
            if (cbFeatureEnabled) {
                if (!circuitBreakerEnabled) {
                    uint256 price = (IERC20_USDCE.balanceOf(lpPair) * 10**18) / balanceOf(lpPair);
    ...
    function takeTaxes(address from, bool buy, bool sell, uint256 amount, uint256 aSupply) internal returns (uint256) {
        uint256 currentFee = getCurrentFee(buy, sell, amount, aSupply);
        ...
        uint256 burnAmount = (feeAmount / (burnRatio + _ratios.total)) * burnRatio;
    ...
    function getCurrentFee(bool buy, bool sell, uint256 amount, uint256 aSupply) internal view returns (uint256 currentFee) {
        if (buy) {
            if (circuitBreakerEnabled) {
                currentFee = 0;
            } else {
            ...
            if (circuitBreakerEnabled) {
                currentFee = _taxRates.circuitBreakerTax;
            } else {
    ...
    function getEstimatedFee(bool isBuy, bool isSell, uint256 amount) external view returns (uint256 currentFee) {
        return getCurrentFee(isBuy, isSell, amount, _aSupply);
    }
    ...
    function manualRebase(uint256 maxTimesToRebase) external {
        ...
        if (maxTimesToRebase == 999) {
            maxTimesToRebase = getPendingRebases();
        }
    ...
    function depositReferralTokens(uint256 amount) external {
        basicTransfer(msg.sender, address(refHolder), amount);
    }

    function withdrawUSDCEFromLiquidityHolder(uint256 amount) external onlyOwner {
        amount *= 10**IERC20_USDCE.decimals();
        antiSnipe.withdrawToAddress(_owner, amount);
    }
