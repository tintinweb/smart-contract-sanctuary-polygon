...
|    * @param _payoutTimeRange duration to honor payout time. Ex: 15 * 60;
    ...
    */
    function _payout() internal {
        ...
        uint256 _nav = IVaultCore(address(this)).checkBalance(false);
        console.log("I-TS: %s NAV: %s", _t, _nav);

        IHarvester(harvesterAddress).harvestAndDistribute();
        _t = cash.totalSupply();
        _nav = IVaultCore(address(this)).checkBalance(false);
...
interface IVaultCore {
    function checkBalance(bool _nav) external returns (uint256);
}
