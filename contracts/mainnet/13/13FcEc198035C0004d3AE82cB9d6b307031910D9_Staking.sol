...
contract Staking is Ownable, Cells, Pausable, ReentrancyGuard {
	...
	mapping(address => mapping(uint256 => uint256)) public usersLimit; // user => cellId => timestamp
	...
	function stake(uint256 _cellId) external whenNotPaused nonReentrant {
		require(usersLimit[msg.sender][_cellId] + 24 hours < block.timestamp, "Daily limit exceeded");
		...
		usersLimit[msg.sender][_cellId] = block.timestamp;
	...
	function withdraw() external whenNotPaused nonReentrant {
		...
		bool hasBonus = checkFortune(stakeInfo[msg.sender].bonusChance, stakeInfo[msg.sender].cellId);
		...
		for (uint256 bonusInd=0; bonusInd<stakeInfo[msg.sender].bonuses.length; bonusInd++){
			...
			uint256 bonusAmount = random(minAmount, maxAmount, stakeInfo[msg.sender].cellId);
	...
	function random(uint256 _min, uint256 _max, uint256 _index) public view	returns(uint256) {
		uint256 diff = _max - _min;
		uint256 seed = uint256(keccak256(abi.encodePacked(
			block.number, block.difficulty*(_index + activeStakesCount)
		))) % diff;
		return seed + _min;
	}

	function checkFortune(uint256 chancePercent, uint256 cellId) public view returns(bool){
		for (uint256 i = 0; i < cellId; i++) {
			uint256 num = random(0,100,i+1);
