...
contract CardTypeValueImplementer {
    ...
    function getValue(uint16 _tokenId) public pure returns (uint8) {
        if(_tokenId <= 100) {
            return 1;
        }
        else if(_tokenId > 100 && _tokenId <= 3000) {
            return 2;
        }
        else if(_tokenId > 3001 && _tokenId <= 10000) {
            return 3;
        }
        revert("Not found");
    }
