...
contract TokenWhitelistFacet is Modifiers {
    using EnumerableSet for EnumerableSet.AddressSet;

    event TokenEnabled(address indexed tokenAddr);
    event TokenDisabled(address indexed tokenAddr);

    /// @notice add token to tracked token whitelist
    /// @param _addr token address
    function enableToken(address _addr) external onlyOwner {
        LibTokenWhitelist.enableToken(_addr);
    }

    /// @notice remove token from tracked token whitelist
    /// @param _addr token address
    function disableToken(address _addr) external onlyOwner {
        LibTokenWhitelist.disableToken(_addr);
    }

    /// @notice check if token is whitelisted
    /// @param _addr token address
    function isWhitelisted(address _addr) external view returns (bool) {
        LibTokenWhitelist.DiamondStorage storage ds = LibTokenWhitelist.diamondStorage();
        return ds.whitelist.contains(_addr);
    }

    /// @notice check if token is allowed to be tracked:
    ///         token should be whitelisted an have configured price oracle
    /// @param _addr token address
    function isAllowed(address _addr) external view returns (bool) {
        return LibTokenWhitelist.isAllowed(_addr);
    }
}
