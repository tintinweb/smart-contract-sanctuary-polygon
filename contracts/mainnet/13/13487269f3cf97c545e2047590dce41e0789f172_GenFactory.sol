...
interface IGenFactory {
    ...
    function createGenTicket(
        ...
        // uint _totalTranches,//3
        uint[] memory _trancheWeightage,//[40,35,25]
        uint[] memory _trancheLength,//[0,30,55]
        string memory _uri
...
contract GenTickets is ERC1155, ERC1155Receiver {
    ...
    function redeemTicket(address _to, uint256 _id, uint256 _amount) public {
        ...
        for (uint i=0; i< trancheLength.length ; i++){
            ...
            if (i < tranche ){
                redeemedPercentage += (trancheWeightage[i].div(10**16));//to conider weightage in upto two decimals XX.YY %
                redeemedCounter += 1;
            }

            if (TGE.add(trancheLength[i]) <= getBlockTimestamp()){
                redeemablePercentage += (trancheWeightage[i].div(10**16));
                redeemableCounter += 1;
            } else{
                break;
            }
        ...
        uint256 tokens = (ticketType.ticketSize).mul(_amount).mul(redeemablePercentage - redeemedPercentage).div(10000);//0.XXYY
...
contract GenFactory is IGenFactory {
    ...
    function createGenTicket(
        ...
        require(_numTickets.length == _ticketSizes.length && _trancheWeightage.length == _trancheLength.length, 'GenFactory: ARRAY SIZE MISMATCH');
        ...
        GenTickets gt = new GenTickets(_underlyingToken, _numTickets, _ticketSizes, _trancheWeightage, _trancheLength, _uri, this, msg.sender);
        // Populate mapping
