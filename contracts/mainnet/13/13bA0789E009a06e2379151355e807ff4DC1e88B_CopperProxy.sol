...
contract CopperProxy is Ownable {
     ...
     * - isStandardFee value should be true unless there is an issue with saveTransfer, in which case it can be passed
    ...
    function exitPool(address pool, uint256 maxBPTTokenOut, bool isStandardFee) external onlyPoolOwner(pool) {
        ...
        if (isStandardFee == true) { // default
            ...
            saveFeeDistribution(pool, fundToken);
        }
     ...
     */
    function saveFeeDistribution(address pool, address fundToken) internal {
