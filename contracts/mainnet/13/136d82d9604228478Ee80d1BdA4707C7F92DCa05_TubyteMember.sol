...
contract TubyteMember is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    constructor(string memory baseURI) ERC721("Tubyte Member", "DSM") {
        setBaseURI(baseURI);
    }
...
contract TubytePost is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    TubyteMember memberContract;
    ...
    constructor(address _memberContractAddr, string memory baseURI)
        ERC721("Tubyte Post", "DSP")
    {
        memberContract = TubyteMember(_memberContractAddr);
        setBaseURI(baseURI);
    }
    ...
    function setMemberContract(address _t) public onlyOwner {
        memberContract = TubyteMember(_t);
    }

    function mintItem(string memory newTokenURI) public returns (uint256) {
        require(
            memberContract.balanceOf(msg.sender) > 0,
            "You must own a TubyteMembership"
        );
        _tokenIds.increment();
