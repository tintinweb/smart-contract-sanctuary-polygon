...
contract ZBCMutual is Setting,IMutual, RiskOrder{
    ...
    address public constant defaultRefer = 0xdD47A5a611FcB5F72AA64Bfd37107b7d1894909a;

    uint256 private constant feesPercents = 200;
    address[6] private feesAccount = [0xa81Beb53af6b28a8d0b1818a85Cb8e919bC0329a,
    0x590ba5C29C017Ee6c63aA6A9F61BC6846A30fcF6,
    0x6F0b144B0E4B3b4DF7E3320b75EeaA7eC269866a,
    0xFF97F9Ea81a173403E12fD843dde141834b60E0b,
    0x6b7FF2c1895BCc5f10fA64864e376FF44d0A9a05,
    0x94b0fC703626D4deDB3AC5E3ed92E3f8Ff293C91];

    uint256 private constant marketECOPercents = 50;
    address private constant marketECO = 0x319035C16F7a95604dCE1e8De385153ECBc15FD1;
    ...
    uint256[6] private levelTeam = [0, 0, 30, 50, 100, 200];
    ...
    function deposit(uint256 _amount) external onlyRegister {
        ...
        _deposit(msg.sender, _amount, false, false);
    ...
    function depositBySafe(address userAddr, uint256 _amount) external {
        ...
        _deposit(userAddr, _amount, false, true);
    ...
    function depositByBorrow(uint256 _amount, address lender_) external onlyRegister {
        ...
        _deposit(msg.sender, _amount, true, false);
    ...
    function _deposit(address _userAddr, uint256 _amount, bool isBorrow, bool isSafe) private {
        ...
        _addLuckUser(_userAddr,_amount);

        _distributeAmount(_amount);

        (bool isUnFreeze, uint256 newAmount) = _unfreezeCapitalOrReward(msg.sender,_amount,isBorrow);
    ...
    function _addLuckUser(address addr,uint256 amount) private {
    ...
    function getUser(address _user) public view returns(UserInfo memory,RewardInfo memory) {
        ...
        ri.totalFreeze = ri.freezeCapitals + ri.freezeTotalLevel515;
...
contract RiskOrder {
    ...
    function initRiskLevel() private {
        RiskLevel memory rl1 = RiskLevel(1_000_000e6,700_000e6,400_000e6,1_500_000e6);
        riskLevels.push(rl1);
        RiskLevel memory rl2 = RiskLevel(5_000_000e6,3_000_000e6,1_500_000e6,5_000_000e6);
        riskLevels.push(rl2);
        RiskLevel memory rl3 = RiskLevel(10_000_000e6,5_000_000e6,2_000_000e6,10_000_000e6);
        riskLevels.push(rl3);
    }
