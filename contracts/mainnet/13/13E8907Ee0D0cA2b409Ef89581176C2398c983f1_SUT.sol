...
contract SUT is ERC20Pausable, AccessController {
  ...
  bytes32 private constant LOCKER_ROLE = "LOCKER_ROLE";
  bytes32 private constant DEPOSITOR_ROLE = "DEPOSITOR_ROLE";

  event LockStatusUpdated(address target, bool status);
  ...
  function setPause(bool _pauseStatus)
  external
  onlyOwner {
    if(_pauseStatus) {
      _pause();
  ...
  function setAddressLock(address _target, bool _lock)
  external
  onlyRole(LOCKER_ROLE, msg.sender) {
    transferLocked[_target] = _lock;

    emit LockStatusUpdated(_target, _lock);
  }
  ...
  /**
   * @notice called when token is deposited on root chain via Polygon POS bridge
   ...
   */
  function deposit(address _to, bytes calldata _depositData)
  external
  onlyRole(DEPOSITOR_ROLE, msg.sender) {
    uint amount = abi.decode(_depositData, (uint));

    _mint(_to, amount);
  }

  /**
   * @notice called when user wants to withdraw tokens back to root chain via Polygon POS bridge
