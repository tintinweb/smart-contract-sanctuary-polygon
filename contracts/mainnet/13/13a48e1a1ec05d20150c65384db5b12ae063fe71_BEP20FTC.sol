...
contract BEP20FTC is Ownable, ReentrancyGuard {
    using SafeMath for uint256;

    uint256 public min_investment = 0.5 ether; 
    uint256[] public Fee = [1000, 0, 0, 0, 0, 0, 0, 0, 0, 0];
  ...
  //  uint256 public reinvestRate = 2;
    ...
    uint16[5] public ref_bonuses = [200, 50, 30, 10, 10]; 
    ...
    function withdrawReferral() public nonReentrant {
        ...
        require(
            players[msg.sender].total_match_bonus < (players[msg.sender].total_invested) * max_ref ,
            "you must deposit more to withdraw referral"
            );

        uint256 payed_amount = (players[msg.sender].match_bonus);
