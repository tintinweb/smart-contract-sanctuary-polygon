...
contract GreenTradeNFT is ERC721, Ownable, ERC721Holder {
    ...
    constructor (string memory _name,
        string memory _uri,
        address _adminAddress) ERC721(_name,_name) {        
        collection_uri = _uri;
...
// marketplace logic
    function list(uint256 token_id, uint256 _price) public {
		...
		pairs[currentPairId].token_id = token_id;
		pairs[currentPairId].creator = creatorOf(token_id);
        pairs[currentPairId].creatorFee = royalties(token_id);
    ...
    function delist(uint256 _id) external {        
        ...
        _transfer(address(this), pairs[_id].owner, pairs[_id].token_id);	
    ...
    function buy(uint256 _id) external payable {
        ...
        _transfer(address(this), msg.sender, pair.token_id);		
		
		pairs[_id].bValid = false;
        emit Swapped(msg.sender, pair);		
    }

    function buyFromCrossMint(uint256 _id, address _to) external payable {
		require(_id <= currentPairId && pairs[_id].pair_id == _id, "Could not find item");
        require(pairs[_id].bValid, "invalid Pair id");
		require(pairs[_id].owner != msg.sender, "owner can not buy");
        require(pairs[_id].owner != _to, "owner can not buy");
		...
		// transfer NFT token to buyer
        _transfer(address(this), _to, pair.token_id);		
