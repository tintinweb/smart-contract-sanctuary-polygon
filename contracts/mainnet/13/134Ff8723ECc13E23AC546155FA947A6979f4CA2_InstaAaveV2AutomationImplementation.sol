...
contract InstaAaveV2AutomationImplementation is InstaAutomationHelper {
    ...
    function submitAutomationRequestMock(
        ...
        uint32 userLatestId = _userLatestId[user_];
        require(
            userLatestId == 0 ||
                _userAutomationConfigs[userLatestId].status != Status.AUTOMATED,
            "position-already-in-protection"
        ...
        uint256 healthFactor_ = getHealthFactor(user_);
        require(
            healthFactor_ < type(uint128).max,
            "current-health-factor-too-large-for-automation-request"
        );
    ...
    function executeAutomation(
        ...
        Spell memory spells_ = _buildSpell(params);
        ...
        bool isSafe_ = finalHf_ >= (_userAutomationConfig.safeHF - _bufferHf);
        ...
        returns (Spell memory spells)
    {
        ...
        /**
            ...
            The unpacking will be as follows: 
                - route_ = params.route % (2**8);
                - flashloanFeeInBps_ = params.route >> 9
         */
        uint256 route_ = params.route % (2**8);
        uint256 flashloanFeeInBps_ = params.route >> 9;
        uint256 loanAmtWithFee_ = params.collateralAmount *
            (((params.collateralAmount * flashloanFeeInBps_) / 1e4) + 1);
        uint256 totalFee_ = params.collateralAmountWithTotalFee -
            loanAmtWithFee_;
