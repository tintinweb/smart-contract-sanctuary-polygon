...
abstract contract LibertiAggregationRouterV4 {
    ...
    function userSwap(
        ...
        if (bytes4(data[:4]) != SWAP_SELECTOR) {
            revert SelectorError();
        }
    ...
    ReentrancyGuardUpgradeable
{
    ...
    ) external onlyOwner nonReentrant returns (uint256 returnAmount) {
    ...
    function exit() external nonReentrant returns (uint256 amountToken0, uint256 amountToken1) {
