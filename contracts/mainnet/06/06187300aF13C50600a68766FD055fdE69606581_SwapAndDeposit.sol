...
contract SwapAndDeposit is MyPausableUpgradeable, ISwapAndDeposit {
  ...
  function buyWithERC20AndDeposit(
    ...
    uint256 outputAmount = router.tradeERC20(IERC20(inputTokenAddress), IERC20(outputTokenAddress), inputTokenAmount);

    // register approval for bridgeERC20 to spend output token
    if (IERC20(outputTokenAddress).allowance(address(this), address(bridgeERC20)) < outputAmount) {
      IERC20(outputTokenAddress).safeIncreaseAllowance(address(bridgeERC20), outputAmount);
    }

    // deposit tokens into bridgeERC20
    bridgeERC20.depositERC20(IERC20(outputTokenAddress), outputAmount, _msgSender(), targetChainId);

    // tokens successfully swapped and deposited => emit event
    emit BoughtAndDeposited(inputTokenAddress, inputTokenAmount, outputTokenAddress, outputAmount, targetChainId);
  }
    ...
    nonReentrant
  {
    ...
    uint256 outputAmount = router.tradeNativeTokenForERC20{value: msg.value}(IERC20(outputTokenAddress), msg.value);

    // register approval for bridgeERC20 to spend depositToken
    if (IERC20(outputTokenAddress).allowance(address(this), address(bridgeERC20)) < outputAmount) {
      IERC20(outputTokenAddress).safeIncreaseAllowance(address(bridgeERC20), outputAmount);
    }
    ...
    emit BoughtAndDeposited(address(0), msg.value, outputTokenAddress, outputAmount, targetChainId);
  }
  ...
  receive() external payable {
    buyWithNativeTokenAndDeposit(address(defaultOutputToken), 45000);
  }
}
