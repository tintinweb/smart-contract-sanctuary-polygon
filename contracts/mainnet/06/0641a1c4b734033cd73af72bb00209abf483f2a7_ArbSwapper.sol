...
contract ArbSwapper is Context, Ownable {
    ...
    /*
    function estimateProfitabilityOld(address buyDEXRouterAddress, address sellDEXRouterAddress, address baseToken, address arbToken, uint arbAmount) public view returns (uint totalProfit) {
        ...
        address[] memory buyPath = getPathFromTokenToToken(baseToken, arbToken);
        uint buyTokenOutput = buyDEXRouter.getAmountsOut(arbAmount, buyPath)[1];

        address[] memory sellPath = getPathFromTokenToToken(arbToken, baseToken);
        uint sellOutput = sellDEXRouter.getAmountsOut(buyTokenOutput, sellPath)[1];

        if (sellOutput < arbAmount) {
            return 0;
        } else {
            uint returnAmount = sellOutput - arbAmount;
            return returnAmount;
        }
    }
    ...
    function estimateProfitability(address buyDEXRouterAddress, address sellDEXRouterAddress, address[] memory arbPath, uint arbAmount) public view returns (uint totalProfit) {
        ...
        for(uint i = 1; i < (arbPath.length / 2) - 1; i++) {
            ...
            uint balanceOfToken = arbTokenContract.balanceOf(address(this));
        ...
        for(uint i = arbPath.length / 2; i < arbPath.length - 2; i++) {
            ...
            sellDEXRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(balanceOfToken, 0, swapPath, address(this), deadline);

        }

        address[] memory sellPath = getPathFromTokenToToken(arbPath[arbPath.length - 2], arbPath[arbPath.length - 1]);
        buyDEXRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(arbAmount, 0, sellPath, address(this), deadline);
        ...
        payable(msg.sender).transfer(searcherProfit);
    ...
    */
    ...
    function printCrypto(address buyDEXRouterAddress, address sellDEXRouterAddress, address[] memory arbPath, uint arbAmount) public payable {
        IUniswapV2Router buyDEXRouter = IUniswapV2Router(buyDEXRouterAddress);
        IUniswapV2Router sellDEXRouter = IUniswapV2Router(sellDEXRouterAddress);
