...
contract StratFeeManager is Ownable, Pausable {
    ...
    constructor(
        ...
        routerYel = _commonAddresses.routerYel;
...
contract Strat is StratFeeManager {
    ...
    uint public yelRewards; // counting last rewards in yel
    ...
    // Routes
    address[] public nativeToYelRoute;
    ...
    constructor(
        ...
        CommonAddresses memory _commonAddresses,
        address[] memory _nativeToYelRoute,
        ...
        native = _nativeToYelRoute[0];
        yel = _nativeToYelRoute[1];
        nativeToYelRoute = _nativeToYelRoute;

        if (_crvToNativePath.length > 0) addRewardV3(_crvToNativePath, 0);
    ...
    function beforeDeposit() external override {
        if (harvestOnDeposit) {
            require(msg.sender == vault || msg.sender == yeller, "!vault");
            _harvest();
        }
    }
    ...
    function _harvest() internal whenNotPaused {
        ...
        uint256 rewardsBal = IERC20(rewardToken).balanceOf(address(this));
        if(rewardsBal > 0) {
            swapRewardsToYel();
            chargeFees();
            transferToYeller();
    ...
    function swapRewardsToYel() internal {
        ...
        uint toYel = IERC20(native).balanceOf(address(this));
        if(toYel > 0) {
            IUniswapRouterETH(routerYel).swapExactTokensForTokens(
                toYel, 0, nativeToYelRoute, address(this), block.timestamp
            );
        }
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(native).approve(routerYel, type(uint).max);
    }

    function _removeAllowances() internal {
        IERC20(want).approve(address(booster), 0);
        IERC20(native).approve(routerYel, 0);
    }
