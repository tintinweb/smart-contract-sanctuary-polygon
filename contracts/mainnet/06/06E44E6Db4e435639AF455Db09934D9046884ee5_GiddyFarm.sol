...
contract GiddyFarm is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  function userRewards(address account) external view returns (uint amount) {
    uint totalGiddy = IERC20(GIDDY_TOKEN).balanceOf(address(this));
    ...
    if (block.timestamp > lastUpdate && user.amount > 0) {
      ...
      newRewards = accumulatedRewards + ((newRewards * 1e12) / totalGiddy);
  ...
  function deposit(ApproveWithAuthorization.ApprovalRequest calldata giddyAuth, bytes calldata giddySig, uint fap) external whenNotPaused nonReentrant {
    ...
    uint256 amount = deductFeeGiddy(giddyAuth.value, fap);
  ...
  function withdraw(uint256 amount, uint256 fap) external whenNotPaused nonReentrant {
    ...
    updateFarm();
    amount = deductFeeGiddy(amount, fap);
  ...
  function updateFarm() private {
    ...
    if (totalGiddy > 0) {
      ...
      if (newRewards > remainingRewards) {
        accumulatedRewards += ((remainingRewards * 1e12) / totalGiddy);
        remainingRewards = 0;
      }
      else {
        accumulatedRewards += ((newRewards * 1e12) / totalGiddy);
  ...
  function deductFeeGiddy(uint amount, uint fap) private returns (uint) {
    if (fap > 0) {
      require(fap < amount, "FEE_AMOUNT");
      SafeERC20Upgradeable.safeTransfer(IERC20Upgradeable(GIDDY_TOKEN), config.feeAccount(), fap);
    }
    return amount - fap;
  }
}
