...
contract TradingWarriorsContract is ERC721, ERC721URIStorage, Ownable {
    ...
    string private baseURI = "https://tradingwarriors.net/twarriors/";
    ...
    constructor() ERC721("Trading Warriors Game DAO", "TWG") {
        stakeholders.push();
        holders.push();
    }
    ...
    function mintPhase(uint256 amount, uint256 phase) public payable {
        ...
        require(phase == 1 || phase == 2 || phase == 3, "Not in phase.");
        require(
            _totalTradingWarriors.current() + amount <= totalSupply(),
            "Not enough tokens left to buy."
        );
    ...
    function toJail(
        ...
        for (uint256 ids = 0; ids < tokenIds.length; ids++) {
            ...
            for (uint256 i = 0; i < stakeholders.length; i++) {
                Stake[] storage current_stake = stakeholders[i].address_stakes;
                for (uint256 ig = 0; ig < current_stake.length; ig++) {
                    if (current_stake[ig].tokenId == tokenId) {
                        current_stake[ig].room = room;
                        current_stake[ig].since = block.timestamp;
                        current_stake[ig].team = 0;
                        current_stake[ig].lockedUntil = lockedUntil;
                    }
                }
            }
        }
    ...
    function _addHolder(address owner, uint256 tokenId) internal {
        ...
        if (index == 0) {
            ...
            holders[userIndex].address_warriors.push(
                Warrior(msg.sender, tokenId, true)
            );
        } else {
            holders[index].address_warriors.push(
                Warrior(msg.sender, tokenId, true)
            );
        }
    }
        ...
        )
    {
        ...
        uint256[] memory team = new uint256[](wn);
        for (uint256 i = 0; i < stakeholders.length; i++) {
            Stake[] memory cny = stakeholders[i].address_stakes;
            for (uint256 icny = 0; icny < cny.length; icny++) {
                if (room != 0) {
                    if (cny[icny].room == room && cny[icny].since != 0) {
                        ...
                        team[arr] = cny[icny].team;
                        arr++;
                    }
                }
            }
        }
        return (userAdd, userTokenId, team, wn);
    }
    ...
    {
        address[] memory userAdd = new address[](
            _totalTradingWarriors.current()
        );
        uint256[] memory userTokenId = new uint256[](
            _totalTradingWarriors.current()
        );
        for (uint256 i = 0; i < _totalTradingWarriors.current(); i++) {
            userAdd[i] = ownerOf(i);
            userTokenId[i] = i;
        }

        return (userAdd, userTokenId);
    }
