...
contract GatheringToken is Initializable, ERC20Upgradeable {
  ...
  function initialize() public initializer {
    __ERC20_init('Heystack DMO', 'DMO');
    // Mint 10,000 tokens to msg.sender to enable liquidity
    ...
    poolAddress = 0xb204BF10bc3a5435017D3db247f56dA601dFe08A;
    poolId = 0xb204bf10bc3a5435017d3db247f56da601dfe08a000200000000000000000264;
  ...
  function voteOnDoc(uint256 _docId, uint8 _vote) external returns (bool approved) {
    ...
    uint256 poolBalance = IERC20(poolAddress).balanceOf(msg.sender);
    require(balanceOf(msg.sender) > 0 || poolBalance > 0, 'Not part of this gathering.');
    ...
    // Calculate pool balance for the user
    uint256 userPoolBalance = 0;
    if (poolBalance > 0) {
      address gatheringAddress = address(this);

      uint256 poolTotalSupply = IERC20(poolAddress).totalSupply();
      (uint256 cash, , , ) = VaultContract(vaultAddress).getPoolTokenInfo(poolId, gatheringAddress);
      userPoolBalance = (poolBalance * cash) / poolTotalSupply;
    }

    // get token balance and pool balance
    uint256 voteBalance = balanceOf(msg.sender) + userPoolBalance;
    ...
    if (_vote == 1) {
      doc.totalApproveVotesWeight += voteBalance;
      uint256 threshold = totalSupply() / (100 / PASS_THRESHOLD);
      ...
      doc.totalRejectVotesWeight += voteBalance;
      // todo: check if passed threshold and delete doc if yes
      return false;
    } else if (_vote == 3) {
      doc.totalAbstainVotesWeight += voteBalance;
      return false;
    }
  }
  ...
  function tokenTransfer() external {
    ...
    IERC20(tokenAddress).transfer(msg.sender, usdcBalance); // transfer ERC20 token balance of gatheringToken contract to BG holder.
  }
  */
