...
import "../../libraries/RandomShuffle.sol";
 ...
 */
contract ERC721Properties is ERC721Upgradeable, OwnableUpgradeable, App, __with_semver(uint8(1), uint8(1), uint8(1)) {
    using Strings for uint256;
    using RandomShuffle for uint32[];
    ...
    function setRandomShuffleMint(bytes32 tierValue, uint256 tokenCount) external requirePermission(CONTROLLER) {
        ...
        _setAttribute(PROPERTY_TIER, tierValue, ATTRIBUTE_MAX_AMOUNT, bytes32(tokenCount)); //TODO check if max_amount == tokenCount
        ...
        ids.setRandomShuffle(tokenCount);
    }
    ...
    function _randomMint(
        ...
        if (randomMintByTier) {
            ...
            uint32[] storage ids = _randomShuffleUnusedArtworkIds[tierValue];
            artworkIdPropValue = bytes32(ids.getNextRandomShuffle(rand));
        }
...
import "../SalesController/ISalesController.sol";

abstract contract ERC721BaseSale is App {
    ...
    address internal salesController;
    uint256 internal start;
    uint256 internal end;
    mapping(bytes32 => bool) public tiersActive;
    ...
    event SetSalesController(address salesController);
    event SetTimeOfSale(uint256 start, uint256 end);
    event SetTiersActive(bytes32[] tiers, bool [] active);

    modifier onlySalesController() {
        require(_msgSender() == salesController, Exceptions.INVALID_AUTHORIZATION_ERROR);
        _;
    }

    modifier saleTime() {
        require(start != 0 || end != 0, Exceptions.SALE_TIME_ERROR);
        require(
            (block.timestamp >= start || start == 0) && (block.timestamp <= end || end == 0),
            Exceptions.SALE_TIME_ERROR
        );
        _;
    }
    ...
    function initialize(
        address _kernel,
        address _salesController,
        bytes calldata _options
    ) external virtual;

    function setTiersActive(bytes32 [] memory tierValues, bool [] memory activity) public onlySalesController{
        require(tierValues.length == activity.length, Exceptions.VALIDATION_ERROR);

        for(uint256 i; i!=tierValues.length; i++){
            tiersActive[tierValues[i]] = activity[i];
        }
        emit SetTiersActive(tierValues, activity);
    }

    //TODO requirePermission(0) -> onlySalesController
    function setSalesController(address newSales) public requirePermission(0){
        _setSalesController(newSales);
    }

    function _setSalesController(address newSalesController) internal {
        salesController = newSalesController;
        emit SetSalesController(salesController);
    }

    function setTimeOfSale(uint256 start_, uint256 end_) public onlySalesController{
        start = start_;
        end = end_;
        emit SetTimeOfSale(start, end);
    }

    function setPaymentPolicy(bytes32[] memory tierValues, uint256[] memory tierPrices_) public onlySalesController{
    ...
    function setClaimLimit(uint64 claimLimit_) public onlySalesController{
        claimLimit = claimLimit_;
        emit SetClaimLimit(claimLimit);
    }

    function setTierPerWalletLimits(bytes32[] memory tierValues, uint256[] memory tierLimits) public onlySalesController{
    ...
    function setTotalClaimsLimits(uint64 totalClaims) public onlySalesController{
    ...
    function _transferToTreasury() internal {
        address wallet = ISalesController(salesController).getTreasury();
        require(wallet != address(0x00), Exceptions.NO_TREASURY_ADDRESS);
        payable(wallet).sendValue(msg.value);
    }

    function setActive(bool isActive_) public onlySalesController{
        isActive = isActive_;
...
contract ERC721OpenSale is ERC721BaseSale, __with_semver(uint8(2), uint8(0), uint8(0)) {
    ...
    function initialize(
        ...
        __App_init(IKernel(_kernel));
        Options memory options = abi.decode(_options, (Options));

        _setSalesController(_salesController);
        setActive(options.isActive);
        setClaimLimit(options.claimLimit);
        setTotalClaimsLimits(options.totalClaimsLimit);
        setPaymentPolicy(options.tierValues, options.tierPrices);
        setTimeOfSale(options.start, options.end);
        setTierPerWalletLimits(options.tierValues, options.tierLimits);
        setTokenSaleAddress(options.tokenSaleAddress);
        setTiersActive(options.tierValues, options.tiersActive);
    }
    ...
    function buy(
    ...
    ) external payable saleTime {
        require(isActive && tiersActive[tierValue], Exceptions.UNAVAILABLE_ERROR);
        ...
        if (msg.value > 0) {
            ...
            _claim(tierValue);
            if(costInBuyTokens>0){
                require(msg.value >= costInBuyTokens, Exceptions.VALIDATION_ERROR);
                _transferToTreasury();
            }
            ISalesController(salesController).mint(to, tierValue);
            ...
            address erc721 = ISalesController(salesController).mint(to, tierValue);
            _afterBuy(to, erc721, tierValue, costInBuyTokens, tokenBuyAddress, "ERC20_SALE", __semver());
        }
    }

    function setTokenSaleAddress(address tokenSaleAddress_) public onlySalesController {
        _setTokenSaleAddress(tokenSaleAddress_);
    }

    function _setTokenSaleAddress(address tokenSaleAddress_) internal {
...
interface ISalesController {
    ...
    function deploySale(bytes calldata options, SaleType saleType) external;

    function mint(address to, bytes32 tierValue) external returns (address);

    function executeSingle(address app, bytes memory data) external returns (bytes memory);

    function executeBatch(address[] calldata apps, bytes[] calldata data) external returns (bytes[] memory);
...
library AppsIds {
    ...
    bytes32 constant SALES_CONTROLLER = keccak256(abi.encodePacked("SALES_CONTROLLER"));
}
...
library Exceptions {
    ...
    string constant ADMIN_SALES_CONTROLLER_ERROR = "ADMIN_SALES_CONTROLLER_ERROR";
    string constant SALE_TIME_ERROR = "SALE_TIME_ERROR";
}
 ...
 * @title RandomShuffle
 * @author SuperdaoTeam
 * @notice Modified Knith algoritm for random shuffling. Initially array to shuffle should be filled by zeros.
 */
library RandomShuffle {
    function setRandomShuffle(uint32[] storage shuffleArray, uint256 size) internal {
        assembly {
            sstore(shuffleArray.slot, size)
        }
    }

    function getNextRandomShuffle(uint32[] storage shuffleArray, uint256 rand) internal returns (uint256 nextRandom) {
        uint32 len = uint32(shuffleArray.length);
        uint32 ptr = uint32(rand % len);
        nextRandom = shuffleArray[ptr];
        uint32 last = shuffleArray[len - 1];
        if (nextRandom == 0) nextRandom = ptr;
        if (last == 0) {
            shuffleArray[ptr] = len - 1;
        } else {
            shuffleArray[ptr] = last;
        }
        shuffleArray.pop();
    }
}
