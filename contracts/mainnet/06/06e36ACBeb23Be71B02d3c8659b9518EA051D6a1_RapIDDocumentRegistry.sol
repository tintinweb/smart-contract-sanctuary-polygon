...
contract RapIDDocumentRegistry {
    ...
    mapping(address => bytes32[]) public documentByIssuer; // issuer address => list of docs
    ...
    function activateDocument(bytes32 docHash) external isNotIssued(docHash) {
        ...
        document[docHash].index = issuedDocuments[msg.sender]++;
        document[docHash].status = STATUS_ACTIVE;

        documentByIssuer[msg.sender].push(docHash);
