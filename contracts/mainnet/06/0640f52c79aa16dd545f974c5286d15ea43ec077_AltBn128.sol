...
interface IOxODexFactory {
    ...
    function getTokenFeeFreeLimit() external view returns (uint256);
...
contract OxODexPool {
    ...
    function deposit(uint _amount, uint256[4] memory _publicKey) external {
        ...
        if(ERC20(factoryContract.token()).balanceOf(msg.sender) < factoryContract.getTokenFeeFreeLimit()) {
            ...
            ERC20(token).transfer(IOxODexFactory(factory).treasurerAddress(), fee);   
        }else{
            ...
            if(fee > 0) {
                /// Transfer the fee to the treasurer
                ERC20(token).transfer(OxODexFactory(factory).treasurerAddress(), fee);  
            }
        }
    ...
    function getDiscountFeeForAmount(uint256 amount) public view returns(uint256){
        return (amount * OxODexFactory(factory).tokenFee()) / 10_000;
    }
    ...
    function swapOnWithdrawal(
        ...
        withdraw(payable(address(this)), amountToken, ringIndex, c0, keyImage, s);
...
contract OxODexFactory {
    ...
    function createPool(address _token) public returns (address vault) {
