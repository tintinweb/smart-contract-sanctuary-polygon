...
contract CalculatorFactory is Owned {
address factory;
//WETH MATIC MAINNET
...
function createCalculatorV1(RootedToken root) public onlyFactory() returns (EliteFloorCalculatorV1) {
    return new EliteFloorCalculatorV1(root, _pancakeFactory);
}
 function createCalculator(RootedToken root, EliteToken elite) public onlyFactory() returns (EliteFloorCalculator) {
    return new EliteFloorCalculator(root, elite, wrappedToken, _pancakeFactory, _router);
}

function createLazarusPit(RootedToken root) public onlyFactory() returns (LazarusPit) {
    return new LazarusPit(root);
}
function createTransferGate(RootedToken root) public onlyFactory() returns (RootedTransferGate) {
    return new RootedTransferGate(root, _router);
}
