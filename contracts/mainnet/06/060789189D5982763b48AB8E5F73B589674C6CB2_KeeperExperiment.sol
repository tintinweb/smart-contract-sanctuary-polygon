...
contract KeeperExperiment is AccessControlledAndUpgradeable {
  ...
  function isPriceChangeOverThresholdWithPercentageChange(uint32 marketIndex)
    public
    view
    returns (bool, int256)
  {
  ...
  function isPriceChangeOverThreshold(uint32 marketIndex)
    public
    view
    returns (bool isOverThreshold)
  {
    (isOverThreshold, ) = isPriceChangeOverThresholdWithPercentageChange(marketIndex);
  }
  ...
  function isPriceChangeOverThresholdNotView(uint32 marketIndex) public returns (bool, int256) {
    return isPriceChangeOverThresholdWithPercentageChange(marketIndex);
  }
  ...
  function getPendingNextPriceActionsPaymentTokenValue(uint32 marketIndex)
    public
    view
    returns (uint256)
  {
    uint256 currentUpdateIndex = ILongShort(longShortAddress).marketUpdateIndex(marketIndex);
    (uint256 longTokenPrice, uint256 shortTokenPrice) = ILongShort(longShortAddress)
      .get_syntheticToken_priceSnapshot(marketIndex, currentUpdateIndex);

    uint256 longSidePendingActionsValueInPaymentToken = ILongShort(longShortAddress)
      .batched_amountPaymentToken_deposit(marketIndex, true) +
    ...
    uint256 shortSidePendingActionsValueInPaymentToken = ILongShort(longShortAddress)
      .batched_amountPaymentToken_deposit(marketIndex, false) +
      (((ILongShort(longShortAddress).batched_amountSyntheticToken_redeem(marketIndex, false) +
        ILongShort(longShortAddress).batched_amountSyntheticToken_toShiftAwayFrom_marketSide(
          marketIndex,
          false
        )) * shortTokenPrice) / 1e18);
    return longSidePendingActionsValueInPaymentToken + shortSidePendingActionsValueInPaymentToken;
  }
  ...
  function shouldUpdateOracle(uint32 marketIndex) public view returns (bool) {
  ...
  function shouldUpdateOracleNotView(uint32 marketIndex) external returns (bool) {
    return shouldUpdateOracle(marketIndex);
  }

  function shouldUpdateOracleBreakdown(uint32 marketIndex)
    public
    view
    returns (
      bool,
    ...
    )
  {
    ...
    ) = isPriceChangeOverThresholdWithPercentageChange(marketIndex);
    return (
  ...
  {
    return shouldUpdateOracleBreakdown(marketIndex);
  }

  function shouldUpdateOracleBreakdownWithPendingActionsValue(uint32 marketIndex)
    public
    view
    returns (
      bool,
    ...
    )
  {
    ...
    ) = shouldUpdateOracleBreakdown(marketIndex);
    return (
      wasLastUpdateOlderThanThreshold,
      priceChangeIsOverThreshold,
      priceChangePercent_1e18,
      getPendingNextPriceActionsPaymentTokenValue(marketIndex)
    );
  }
  ...
  function shouldUpdateForMarketNotViewChecker(uint32 marketIndex)
    external
    returns (bool canExec, bytes memory execPayload)
  {
    ...
    canExec = ((wasLastUpdateOlderThanThreshold ||
      priceChangeIsOverThreshold ||
      areTherePendingNextPriceActions) &&
      (priceChangePercent_1e18 > 0 || priceChangePercent_1e18 < 0));
    execPayload = abi.encodeWithSelector(ILongShort.updateSystemState.selector, marketIndex);
  }

  // Used by Gelato resolver
  // Intended to restrict updates to only larger sums to conserve gas
  // https://docs.gelato.network/guides/writing-a-resolver/smart-contract-resolver
  function shouldUpdateForMarketWithValueThresholdNotViewChecker(
    uint32 marketIndex,
    ...
    (
      ...
      bool priceChangeIsOverThreshold,
      int256 priceChangePercent_1e18,
      uint256 pendingActionsValueInPaymentTokens
    ) = shouldUpdateOracleBreakdownWithPendingActionsValue(marketIndex);
    canExec =
      (wasLastUpdateOlderThanThreshold || priceChangeIsOverThreshold) &&
      pendingActionsValueInPaymentTokens >= valueThreshold &&
      priceChangePercent_1e18 != 0;
    execPayload = abi.encodeWithSelector(ILongShort.updateSystemState.selector, marketIndex);
  }
}
