...
contract MajrERC20 is Ownable, ERC20 {
   ...
   * @notice Constructor
   */
  constructor() ERC20(oldName, oldSymbol) {
  ...
  function addToWhitelist(address _account) external onlyOwner {
    require(
      !canTransferBeforeTransfersAreEnabled[_account],
      "MajrERC20: Address is already in the whitelist."
    );
  ...
  function _beforeTokenTransfer(
    ...
    if (!areTransfersEnabled) {
      require(
        canTransferBeforeTransfersAreEnabled[from],
        "MajrERC20: Transfers are not enabled yet and this address is not in the whitelist."
      );
    }
  ...
  function burn(uint256 _amount) external {
    ...
    require(
      ...
      "MajrERC20: Burn amount exceeds balance."
    );
