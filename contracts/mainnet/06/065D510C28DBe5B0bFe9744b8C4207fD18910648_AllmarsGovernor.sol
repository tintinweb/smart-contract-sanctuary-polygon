...
library InputHelpers {
    function ensureInputLengthMatch(uint256 a, uint256 b) internal pure {
        Throw._require(a == b, Errors.INPUT_LENGTH_MISMATCH);
    }
    ...
    function ensureInputLengthMatch(
        ...
        Throw._require(a == b && b == c, Errors.INPUT_LENGTH_MISMATCH);
    }
    ...
    function ensureArrayIsSorted(address[] memory array) internal pure {
        ...
        for (uint256 i = 1; i < array.length; ++i) {
            ...
            Throw._require(previous < current, Errors.UNSORTED_ARRAY);
            previous = current;
        }
    }
}
...
library Math {
    ...
    function add(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 c = a + b;
        Throw._require(c >= a, Errors.ADD_OVERFLOW);
        return c;
    }
    ...
    function add(int256 a, int256 b) internal pure returns (int256) {
        int256 c = a + b;
        Throw._require((b >= 0 && c >= a) || (b < 0 && c < a), Errors.ADD_OVERFLOW);
        return c;
    }
    ...
    function sub(uint256 a, uint256 b) internal pure returns (uint256) {
        Throw._require(b <= a, Errors.SUB_OVERFLOW);
        uint256 c = a - b;
        return c;
    }
    ...
    int256 c = a - b;
        Throw._require((b >= 0 && c <= a) || (b < 0 && c > a), Errors.SUB_OVERFLOW);
        return c;
    }
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 c = a * b;
        Throw._require(a == 0 || c / a == b, Errors.MUL_OVERFLOW);
        return c;
    }
    ...
    function divDown(uint256 a, uint256 b) internal pure returns (uint256) {
        Throw._require(b != 0, Errors.ZERO_DIVISION);
        return a / b;
    }
...
library LogExpMath {
    ...
    function pow(uint256 x, uint256 y) internal pure returns (uint256) {
        ...
        Throw._require(x < 2**255, Errors.X_OUT_OF_BOUNDS);
        ...
        Throw._require(y < MILD_EXPONENT_BOUND, Errors.Y_OUT_OF_BOUNDS);
    ...
    function exp(int256 x) internal pure returns (int256) {
        Throw._require(x >= MIN_NATURAL_EXPONENT && x <= MAX_NATURAL_EXPONENT, Errors.INVALID_EXPONENT);
    ...
    function ln(int256 a) internal pure returns (int256) {
        ...
        Throw._require(a > 0, Errors.OUT_OF_BOUNDS);
...
library FixedPoint {
    ...
    function mulDown(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 product = a * b;
        Throw._require(a == 0 || product / a == b, Errors.MUL_OVERFLOW);
    ...
    function divDown(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        if (a == 0) {
            ...
            Throw._require(aInflated / a == ONE, Errors.DIV_INTERNAL); // mul overflow
...
contract WeightedMath {
    ...
    returns (uint256 invariant)
    {
        ...
        **********************************************************************************************/
        ...
        Throw._require(invariant > 0, Errors.ZERO_INVARIANT);
    }
    ...
    function _calcOutGivenIn(
        ...
        **********************************************************************************************/
        ...
        Throw._require(amountIn <= balanceIn.mulDown(_MAX_IN_RATIO), Errors.MAX_IN_RATIO);
    ...
    function _calcInGivenOut(
        ...
        **********************************************************************************************/
        ...
        Throw._require(amountOut <= balanceOut.mulDown(_MAX_OUT_RATIO), Errors.MAX_OUT_RATIO);
    ...
    function _calcTokenInGivenExactBptOut(
        ...
        ******************************************************************************************/
        ...
        Throw._require(invariantRatio <= _MAX_INVARIANT_RATIO, Errors.MAX_OUT_BPT_FOR_TOKEN_IN);
    ...
    function _calcTokenOutGivenExactBptIn(
        ...
        *****************************************************************************************/
        ...
        Throw._require(invariantRatio >= _MIN_INVARIANT_RATIO, Errors.MIN_BPT_IN_FOR_TOKEN_OUT);
...
//    Testing in polygon values
    uint256 public wethRate = 1 * 10 ** 18 / 100000;
    uint256 public maxTerm = 3600; // 1 hour
    uint256 public minTerm = 600; // 10 minutes
...
//    uint256 public minTerm = 24 * 60 * 60;
    ...
    address public WETHContract = address(0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619);
...
// Events
    ...
    constructor (
        ...
        require(
            (erc20Address != address(0) && erc20Address.isContract()),
                "Invalid ERC20 contract address"
        );
        require(
            (nftAddress != address(0) && nftAddress.isContract()),
    ...
    function setSwapFee(uint256 _fee) external onlyOwner {
        LBP(POOL).setSwapFeePercentage(_fee);
        swapFee = _fee;
    }
    ...
    function depositNFT(uint256 _tokenId) public isInitiated {
        uint256 erc20Rate = getAmountIn();
        require(
            IERC20(ERC20Contract).balanceOf(address(this)) >= erc20Rate,
                "Insufficient ERC20 balance on contract"
        );
        ...
        IERC20(ERC20Contract).safeTransfer(_msgSender(), erc20Rate);
    }
    ...
    function withdrawWETHByOwner(uint256 amount) public onlyOwner isInitiated {
        ...
        require(
            amount <= IERC20(WETHContract).balanceOf(address(this)),
    ...
    function withdrawERC20ByOwner(uint256 amount) public onlyOwner isInitiated {
        require(amount > 0, "Zero amount prohibited");
        require(
            amount <= IERC20(ERC20Contract).balanceOf(address(this)),
                "Insufficient contract ERC20 balance"
        );
    ...
    function claimNFT(uint256 tokenId, address token) public isInitiated {
        ...
        if (_deposits[tokenId].timestamp + minTerm > block.timestamp) {
            IERC20(ERC20Contract).safeTransferFrom(
                _msgSender(), address(this), _deposits[tokenId].erc20Rate
            );
            ...
            if (token == WETHContract) {
                ...
                sendNFT(_msgSender(), tokenId);
                uint256 toBalancer = wethRate * BALANCER_SHARE / 100;
                IERC20(WETHContract).safeApprove(VAULT, toBalancer);
                address[] memory tokens = new address[](2);
                uint256[] memory amounts = new uint256[](2);
                if (WETHContract < ERC20Contract) {
                    tokens[0] = WETHContract;
                    tokens[1] = ERC20Contract;
                    amounts[0] = toBalancer;
                    amounts[1]  = 0;
                } else {
                    tokens[1] = WETHContract;
                    tokens[0] = ERC20Contract;
                    amounts[1] = toBalancer;
                    amounts[0]  = 0;
                }
                _joinPool(tokens, amounts, 1);
                IERC20(WETHContract).safeTransfer(owner(), wethRate - toBalancer);
            } else  {
                IERC20(ERC20Contract).safeTransferFrom(_msgSender(), address(this), getAmountIn());
                sendNFT(_msgSender(), tokenId);
            }
        }
    }
