...
contract NftMinter is ERC721, AccessControlEnumerable, ERC2981 {
    ...
    /**
     * @dev Minting function for buys on a whitelist. The price can be different from
     *  the regular minting function. In addition,
     */
    function whitelistMint(bytes32[] memory proof, string memory clientInfo)
        public
        payable
    {
        require(
            !whitelistMinted[_msgSender()],
            "Whitelist NFT already minted."
        );
        require(onWhitelist(_msgSender(), proof), "Invalid proof.");
        require(msg.value == whitelistPricePerNft, "Invalid payment value.");
     ...
     */
    function adminMint(uint256 numTokensToMint)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _mintCommon(numTokensToMint);
     ...
     */
    function onWhitelist(address claimant, bytes32[] memory proof)
        internal
        view
        returns (bool)
    {
        ...
        bytes32 computedHash = keccak256(abi.encodePacked(claimant));
