...
contract BoredApeYachtClubFANMADE is ERC721  {
    ...
    constructor()
        ERC721("Bored Ape Yacht Club FANMADE", "BAYCFM")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 20001) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeicp2lu6vzlb5h2z43lscyylveih6eqgnsgplftpg6v5uctxgkupfi/", Strings.toString(tokenId), ".json")
            );
    }
