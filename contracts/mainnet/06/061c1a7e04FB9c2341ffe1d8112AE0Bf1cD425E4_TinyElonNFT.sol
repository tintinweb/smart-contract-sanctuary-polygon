...
import "./token/onft/extension/TinyElonNFT721.sol";

contract TinyElonNFT is TinyElonNFT721 {
    constructor(address _layerZeroEndpoint, uint _startMintId, uint _endMintId, string memory _notRevealedUri) TinyElonNFT721("TinyElonNFT", "TinyElonNFT", _layerZeroEndpoint, _startMintId, _endMintId, _notRevealedUri) {}
}
...
contract TinyElonNFT721 is ONFT721 {
    uint public nextMintId;
    uint public maxMintId;

    uint public tokenPrice = 0 ether;
    ...
    constructor(string memory _name, string memory _symbol, address _layerZeroEndpoint, uint _startMintId, uint _endMintId, string memory _notRevealedUri) ONFT721(_name, _symbol, _layerZeroEndpoint) {
        nextMintId = _startMintId;
        maxMintId = _endMintId;
        setNotRevealedURI(_notRevealedUri);
    }

    function getMaxMintId() public view returns (uint) {
        return maxMintId;
    }

    function getCurrentMintId() public view returns (uint) {
        return nextMintId - 1;
    }
    ...
    function mint(uint8 numTokens) external payable {
        require(saleIsActive, "TinyElonNFT : Sale must be active to mint token");
        require(numTokens < 30, "TinyElonNFT : Max 30 NFTs per transaction");
        require(nextMintId + (numTokens - 1) <= maxMintId, "TinyElonNFT : Max Mint limit reached");
        uint _tokenPrice = tokenPrice;
        require(msg.value >= _tokenPrice * numTokens, "Ether value sent is not correct");

        if (presale) {
            require(isAddressWhitelisted(msg.sender), "TinyElonNFT : Not on the whitelist!");
        }
