...
contract pGGold is ERC20("pGGold", "pGGOLD"), Ownable {
    ...
    function mintMany(address[] calldata _to, uint[] calldata _amount) external {
        require(managers[msg.sender] == true, "This address is not allowed to interact with the contract");
    ...
    function burnMany(address[] calldata _from, uint[] calldata _amount) external {
        require(managers[msg.sender] == true, "This address is not allowed to interact with the contract");
        for(uint i = 0; i < _from.length; i++) {
            _burn(_from[i], _amount[i]);
        }
    }
}
