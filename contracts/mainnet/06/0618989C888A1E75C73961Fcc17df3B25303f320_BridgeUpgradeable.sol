    ...
    ReentrancyGuardUpgradeable
{
    ...
    IFeeManagerUpgradeable private _feeManager;
    ...
    */
    ...
    function fetch_feeManager() public view virtual returns (address) {
 ...
 */
    ...
    function adminSetFeeManager(address feeManager) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        _feeManager = IFeeManagerUpgradeable(feeManager);
    }
    ...
    */
    ...
    {
        _feeManager.setWidgetFeeInBps(widgetID, widgetFeeInBpsX1000);
    }

    function setTokenToUSDx1000(address token, uint256 priceInUSDx1000) external virtual onlyRole(FEE_SETTER_ROLE) {
        _feeManager.setFeeTokenToPriceX10e9(token, priceInUSDx1000);
    }
    ...
    function getBridgeFee(
        ...
        (uint256 totalFee, bytes memory data) = _feeManager.getFee(
            destChainID,
            srcToken,
            destToken,
            feeToken,
    ...
    function getBridgeFeeWithSwap(
        ...
        (uint256 totalFee, bytes memory data) = _feeManager.getFeeWithSwap(
            destChainID,
            srcToken,
            destToken,
            feeToken,
    ...
    function _deposit(
        ...
        depositHandler.deposit(resourceID, destinationChainID, swapDetails.depositNonce, swapDetails, widgetId, sender);

        emit Deposit(destinationChainID, resourceID, swapDetails.depositNonce, widgetId);
    }
...
interface IDepositExecute {
    ...
    /**
        ...
        @param swapDetails details of swap
    ...
    // function getAdminOfWidget(string memory widgetId) external view returns (address);

    // function getWidgetFeeAccumulated(address feeToken, string memory widgetId) external view returns (uint256);

    // function getFeeTokenToLpFeesAccumulated(address feeToken) external view returns (uint256);

    // function getFeeTokenToValidatorFeesAccumulated(address feeToken) external view returns (uint256);

    // function getFeeTokenToProtocolFeesAccumulated(address feeToken) external view returns (uint256);

    // function setWidgetAdmin(string memory widgetId, address _admin) external;

    // function withdrawWidgetFee(string memory widgetId, address feeToken) external;

    // function withdrawLpFee(address feeToken) external;

    // function withdrawProtocolFee(address feeToken) external;

    // function withdrawValidatorFee(address feeToken) external;
}
...
interface IERCHandler {
    ...
    function setTokenToUSDx1000(address token, uint256 priceInUSDx1000) external;

    function getTokenToUSDx1000(address token) external view returns (uint256);
