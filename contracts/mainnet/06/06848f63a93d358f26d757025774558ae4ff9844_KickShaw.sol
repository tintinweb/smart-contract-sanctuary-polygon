    ...
    ERC721Burnable
{
    ...
    address  public royaltyAddress = 0x0d5ccbC1C64581421C4B04A525980Ae40590c1fc;
    address  public router = 0x0d5ccbC1C64581421C4B04A525980Ae40590c1fc;

    string public goldLeagueURI = "https://2022-gold-membership.s3.filebase.com/";
    string public platLeagueURI = "https://2022-platinum-membership.s3.filebase.com/";


    uint256 public goldCounter = 0;
    uint256 public platCounter = 0;
    ...
    //traking gold buyers
    address[] public goldBuyers;

    //tracking platinum buyers
    address[] public platinumBuyers;
    ...
    ) ERC721("KickShaw Fantasy League", "KickShaw") {
        receiver = _receiver;
...
uint256 feeZero = 50000000000000000000;
uint256 feeOne =  200000000000000000000;

function setFee(uint256 _goldLeague, uint256 _platLeague) public onlyOwner {
    feeZero = _goldLeague;
    feeOne = _platLeague;
}
...
function getGoldArray() public view returns (address[] memory) {
    return goldBuyers;
}

function getPlatinumArray() public view returns (address[] memory) {
    return platinumBuyers;
}

function arrayRemover(uint256 _index, string memory _league) internal {

    if(compareStrings(_league, "gold")){

        require(_index < goldBuyers.length, "index is out of bounds");
        for(uint i = _index; i < goldBuyers.length - 1; i++){
            goldBuyers[i] = goldBuyers[i + 1];
        }

        goldBuyers.pop();
        
    } else if(compareStrings(_league, "platinum")) {

        require(_index < platinumBuyers.length, "index is out of bounds");

        for(uint i = _index; i < platinumBuyers.length - 1; i++){
            platinumBuyers[i] = platinumBuyers[i + 1];
        }
        platinumBuyers.pop();
...
function mintCard(uint256 league) public payable {
    require(!paused(),"Sale must be active to mint");
    require(league == 0 || league == 1, "Wrong mint league");
    uint256 fee;
    league == 0 ? fee = feeZero : fee = feeOne;
    ...
    if(league == 0) {
        uint256 counter = goldCounter + 1;
        require(counter <= 1000, "Cannot mint more than Gold League supply");
        goldCounter++;
        internalMint(msg.sender, string(abi.encodePacked(goldLeagueURI, counter.toString(), baseExtension)));
        goldBuyers.push(msg.sender);
        receiver.transfer(msg.value);
        emit mintMembership(msg.sender, "gold");
    }
    else if(league == 1){  
        uint256 counter = platCounter + 1;
        require(counter <= 500, "Cannot mint more than Platinum League supply");
        platCounter++;
        internalMint(msg.sender, string(abi.encodePacked(platLeagueURI, counter.toString(), baseExtension)));
        platinumBuyers.push(msg.sender);
        receiver.transfer(msg.value);
        emit mintMembership(msg.sender, "platinum");
...
function mintLeague(address _receiver, string[] memory _tokenURI, uint _index, string memory _league) public onlyOwner {

    arrayRemover(_index, _league);
    for (uint256 i = 0; i < _tokenURI.length; i++){
        internalMint(_receiver, _tokenURI[i]);
    }
...
function setURI(uint256 tokenId, string memory uri) external onlyOwner {
    ...
    //Overrides
     ...
     function internalMint(address to, string memory _uri) internal {
