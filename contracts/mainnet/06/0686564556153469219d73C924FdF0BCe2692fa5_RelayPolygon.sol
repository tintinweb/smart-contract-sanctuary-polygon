...
contract RelayPolygon is Ownable, ReentrancyGuard {
    ...
    function depositEthAndSwap(address tokenAddress)
        public
        payable
        nonReentrant
    {
    ...
    function quickSwap(address tokenAddress) public payable nonReentrant {
        require(msg.value > 0, "You must send some ETH");
        // require(ValidateSigner(_signature), "You must sign the message");
        userBalanceEth[msg.sender] += msg.value;
        takeFee(msg.value);
    ...
    function SwapETHtoTokens(address _swapTo, uint256 amount)
        public
        nonReentrant
    {
        require(
    ...
    function swapMaticToTokensLater(address _swapTo) public nonReentrant {
    ...
    function swapTokensForTokenLater(
        address tokenIn,
        address tokenOut,
        address to
    ) public nonReentrant {
        ...
        payable
    {
        ...
        uint256 fFees = (msg.value / 1000) * fFee;
    ...
    {
        return abi.decode(data, (uint256  , uint256, address[], address, uint));
    }
