...
contract MockDZap is Ownable {
    ...
    address public immutable INCH_ROUTER;
    ...
    function swap(bytes memory data_) external payable {
        (
        ...
        if (isNotNative) {
            ...
            IERC20(desc.srcToken).approve(INCH_ROUTER, desc.amount);
        }

        (uint256 returnAmount, uint256 spentAmount, ) = IAggregationRouterV4(
            INCH_ROUTER
        ).swap{value: msg.value}(executor, desc, routeData);

        if (isNotNative) {
            uint256 unspentAmount = IERC20(desc.srcToken).balanceOf(
                address(this)
            );
            if (unspentAmount > 0)
                IERC20(desc.srcToken).safeTransfer(_msgSender(), unspentAmount);
        } else {
    ...
    /*     
        ...
        // uint256 unspentAmount = _balanceOf(desc.srcToken, address(this));
        // _transfer(desc.srcToken, _msgSender(), unspentAmount);
        ...
        function _balanceOf(IERC20 token, address account)
            internal
            view
            returns (uint256)
        {
