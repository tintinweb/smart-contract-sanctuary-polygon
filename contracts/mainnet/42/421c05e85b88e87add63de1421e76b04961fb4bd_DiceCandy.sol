...
PullPayment
{
    ...
    uint256 internal constant BATCH_SIZE = 200;

    // Storage vars
    /// @dev Token Id counter
    Counters.Counter public currentTokenId;
    uint256 internal MAX_MINT = 4;
    uint256 internal MINT_LIMIT = 1;
    /// @dev Base token URI used as a prefix by tokenURI().
    string public baseTokenURI;
    /// @dev Contract metadata URI used by contractURI().
    string public contractURI;
    /// @dev _paused flag
    bool private _paused;

    constructor() ERC721("DiceCandy2", "DCCN2") {
        _paused = false;
        baseTokenURI = "";
        contractURI = "";
    }

    /// @dev Returns bool when queried with interfaceId.
    /// +
    ...
    /// @dev Sets the base contract URI.
    ...
    returns (uint256)
    {
        ...
        for (uint256 i = 1; i <= MINT_LIMIT; i++) {

            _safeMint(recipient, tokenId + i + 10);
    ...
    /// @dev approve for setApprovalForAll
    ...
    /// @dev approve for onlyAllowedOperatorApproval
    ...
    /// @dev safeTransferFrom
    function safeTransferFrom(address from_, address to_, uint256 tokenId_, bytes memory _data)
    public
    whenNotPaused
    override(ERC721, IERC721)
    onlyAllowedOperator(from_)
    {
        super.safeTransferFrom(from_, to_, tokenId_, _data);
    }
    ...
    /// @dev Safe minting of BATCH_SIZE of tokens.
    function safeBatchMultiMint()
    public
    onlyOwner
    whenNotPaused
    {
        uint256 _first = currentTokenId.current();
        uint256 _last = _first + BATCH_SIZE;
