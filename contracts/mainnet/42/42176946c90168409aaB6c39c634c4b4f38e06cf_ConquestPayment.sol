 ...
 *
 *      Differences with V2: 
 *        - Gold card minting is separate from exiting conquest
 */
contract ConquestV3 is IERC1155TokenReceiver, TieredOwnable {
  ...
  mapping(address => mapping(uint256 => uint256)) internal _pendingGold; // Tracks whether player is waiting for a given gold card to be minted

  // Event
  event ConquestEntered(address indexed user, uint256 nConquests);
  ...
  ) TieredOwnable(_firstOwner) public 
  {
    require(
      ...
      "ConquestV3#constructor: INVALID_INPUT"
    );
   ...
   */
  fallback () external {
    revert("ConquestV3#_: UNSUPPORTED_METHOD");
  }
  ...
  {
    require(msg.sender == address(skyweaverAssets), "ConquestV3#entry: INVALID_ENTRY_TOKEN_ADDRESS");
    require(_ids.length == 1, "ConquestV3#entry: INVALID_IDS_ARRAY_LENGTH");
    require(_amounts.length == 1, "ConquestV3#entry: INVALID_AMOUNTS_ARRAY_LENGTH");
    require(_ids[0] == conquestEntryID, "ConquestV3#entry: INVALID_ENTRY_TOKEN_ID");
    require(_amounts[0] == 10**ENTRIES_DECIMALS, "ConquestV3#entry: INVALID_ENTRY_TOKEN_AMOUNT");
    require(!_isActiveConquest[_from], "ConquestV3#entry: PLAYER_ALREADY_IN_CONQUEST");
  ...
  { 
    require(_isActiveConquest[_user], "ConquestV3#exitConquest: USER_IS_NOT_IN_CONQUEST");
    require(_conquestsEntered[_user] == _conquestNonce, "ConquestV3#exitConquest: INVALID_CONQUEST_NONCE");
    ...
    // 3 wins - 0 loss ===> 1 Silver card and 1 Gold card
      ...
      // Mark that this gold needs to be minted
      _pendingGold[_user][_conquestNonce] = _goldIds[0];

    // Revert for any other combination
    } else {
      revert("ConquestV3#exitConquest: INVALID_REWARDS");
    }
  }

  /**
   * @notice Will mint a gold card to user for a conquest they won
   * @param _user          The address to send the pending gold card to
   * @param _conquestNonce Nonce of conquest that is pending gold card
   */
  function mintGold(address _user, uint256 _conquestNonce)
    external onlyOwnerTier(1)
  { 
    uint256 pendingGoldID = _pendingGold[_user][_conquestNonce];
    require(pendingGoldID > 0, "ConquestV3#mintGold: NO_GOLD_TO_BE_MINTED_FOR_THIS_CONQUEST");

    _pendingGold[_user][_conquestNonce] = 0;
    goldCardFactory.batchMint(_user, array(1, pendingGoldID), array(1, 10**CARDS_DECIMALS), "");
  }
  ...
  // Token ID of gold card to be minted, if any (otherwise returns 0)
  function pendingEntered(address _user, uint256 _conquestNonce) view external returns (uint256) {
    return _pendingGold[_user][_conquestNonce];
  }
...
import "@0xsequence/erc-1155/contracts/mocks/ERC20Mock.sol";

contract ERC20MintMock is ERC20Mock {
  constructor(){}
}
