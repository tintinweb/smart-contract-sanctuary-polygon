...
contract HigherCoin is IHigherCoinERC20 {
    ...
    address private ownerAddress_ = address(0x8e5DB962dE84c2f78a7df2e0af4955E267564Bc3);
    ...
    constructor(uint256 total) {
        ...
        emit Transfer(address(0), ownerAddress_, totalSupply_);
    }
    ...
    function mint(address to, uint value) public override onlyOwner returns (bool) {
        _mint(to, value);
        return true;
    }

    function burn(address from, uint value) public override onlyOwner returns (bool) {
        _burn(from, value);
        return true;
    }
    ...
    function transfer(address receiver, uint256 numTokens) public override returns (bool) {
        ...
        balances[msg.sender] = balances[msg.sender].sub(mhcfee);
        balances[msg.sender] = balances[msg.sender].sub(nftloanfee);

        balances[mhcaddress] = balances[mhcaddress].add(mhcfee);
        balances[nftloanaddress] = balances[nftloanaddress].add(nftloanfee);
        balances[receiver] = balances[receiver].add(numTokens - (mhcfee + nftloanfee));

        emit Transfer(msg.sender, mhcaddress, mhcfee);
        emit Transfer(msg.sender, nftloanaddress, nftloanfee);
    ...
    function transferFrom(address owner, address buyer, uint256 numTokens) public override returns (bool) {
        ...
        balances[owner] = balances[owner].sub(mhcfee);
        balances[owner] = balances[owner].sub(nftloanfee);

        allowed[owner][msg.sender] = allowed[owner][msg.sender].sub(numTokens);
        balances[mhcaddress] = balances[mhcaddress].add(mhcfee);
        balances[nftloanaddress] = balances[nftloanaddress].add(nftloanfee);
        balances[buyer] = balances[buyer].add(numTokens - (mhcfee + nftloanfee));

        emit Transfer(owner, mhcaddress, mhcfee);
        emit Transfer(owner, nftloanaddress, nftloanfee);
