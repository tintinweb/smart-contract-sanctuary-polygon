...
contract ASMultiSigWallet {
    ...
    */
    ...
    event CreatePermission(address indexed owner, uint indexed txIndex);
    event AllowPermission(address  indexed owner,uint indexed txIndex);
    ...
    event ChangeOwnerShip(uint indexed indexNumber, address indexed oldOwner, address indexed newOwner);

    event SubmitTransaction(address indexed owner, uint indexed wtIndex, address indexed to, uint value, bytes data);
    ...
    function createPermission() public onlyOwner {
        ...
        emit CreatePermission(msg.sender, txIndex);
    }

    function allowPermission(uint _txIndex) public onlyOwner txExists(_txIndex) notExecuted(_txIndex) notConfirmed(_txIndex) {
    ...
    function executeOwnerChange(uint _txIndex,address newOwner) public onlyOwner txExists(_txIndex) notExecuted(_txIndex) {
        ...
        transaction.executed = true;
        NFT_Astro_Address.OwnershipChange(newOwner);
        emit ExecuteVIPSale(msg.sender, _txIndex);
    }
    function ChangeOwner(uint _txIndex, address oldOwner, address _newWalletAddress) public onlyOwner ownerExists(oldOwner) ownerDoesNotExist(_newWalletAddress) notExecuted(_txIndex) txExists(_txIndex) {
        ...
        emit ChangeOwnerShip(_txIndex, oldOwner, _newWalletAddress);
    }
    ...
    function getInternalTransactionCount() public view returns (uint) {
        return walletTransfers.length;
    }
    ...
    function getWalletTransferCount() public view returns (uint) {
        return walletTransfers.length;
    }
