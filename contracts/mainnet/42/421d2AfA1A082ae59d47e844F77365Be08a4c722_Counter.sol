...
contract Counter is NonblockingLzApp {
    ...
    function remoteAddOperation(
        uint16 dstChainId, // send a ping to this destination chainId
        uint256 amount
    ) external payable {
        bytes memory payload = abi.encode(OP_ADD, amount);
        _outbound(dstChainId, payload);
    }
    ...
    function _outbound(
        ...
        uint16 version = 1;
        uint gasForDestinationLzReceive = 1500000;
    ...
    function _nonblockingLzReceive(
        ...
        assembly {
            sendBackToAddress := mload(add(srcAddress, 20))
        }
