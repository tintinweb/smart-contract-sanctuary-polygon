        ...
        start();          
          if (External(WMATICToken).balanceOf(address(this)) > threshold && I > II) { 
              ...
              pushAddrs.push(msg.sender);
                ...
                payable (CreativezDeployerAddrs).transfer(depositCreativezAddrs); //3.1. Send the MATIC gained from rewards to the Creativez.sol contract for mass distribution
                a++;  
                if(buttonPushTimer < External(CreativezButtonPushRewardsAddrs).deployerTimeStamp()){
                delete pushAddrs;
              }
