    ...
    BaseRelayRecipient
{
    ...
    uint public productAvailable;
    Zingot2 _zingot = Zingot2(0x3D23038a15D2186A4467f9647482994792eb401c);

    string public name = "Ame72 ZCM";
    string public symbol = "zAme";

    address private _treasury = 0x3D23038a15D2186A4467f9647482994792eb401c;
    string private _uri;
    ...
    {
        return "2.2.5+opengsn.accepteverything.ipaymaster";
    }
    ...
    function uri(
        ...
        return
            bytes(_uri).length > 0
                ? string(abi.encodePacked(_uri, collectionId.toString()))
                : "";
    }
    ...
    function mintBatch(
        ...
        for (uint256 index = 0; index < ids.length; index++) {
            ...
            require(
                idToProduct[ids[index]].maxSupply >=
                    (idToProduct[ids[index]].currentSupply + amounts[index]),
                "No more token available"
            );
            require(
                idToProduct[ids[index]].start == true,
                "Sales hasn't started"
            );
            idToProduct[ids[index]].currentSupply += amounts[index];
        }
    ...
    function addProduct(
        ...
        bool start,
        address artist
    ) public isWhitelisted whenNotPaused {
...
contract Zingot2 is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, ERC20SnapshotUpgradeable, OwnableUpgradeable, UUPSUpgradeable, ERC2771ContextUpgradeable {
  ...
  function initialize(address oldZingot) initializer public {
