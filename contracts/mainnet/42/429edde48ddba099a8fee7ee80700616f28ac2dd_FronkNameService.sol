...
contract FronkNameService is Ownable, ERC721 {
    ...
    /// @dev Domain just for whitelist address
    ...
    constructor() ERC721("Fronk Name Service", "FNS") {}
    ...
    /// @dev Register a new name only for whitelist address. Check if the name is available, valid and if the sender has whitelisted
        ...
        returns (string memory)
    {
        ...
        string memory _name = string(abi.encodePacked(id[tokenId], ".fronk"));
        ...
        string memory avatars = Base64.encode(
            bytes(
                abi.encodePacked(
                    '<svg width="270" height="270" fill="none" xmlns="http://www.w3.org/2000/svg">',
                    '<path fill="url(#a)" d="M0 0h270v270H0z"/>',
                    '<defs><filter id="b" color-interpolation-filters="sRGB" filterUnits="userSpaceOnUse" height="270" width="270"><feDropShadow dx="0" dy="1" stdDeviation="2" flood-opacity=".225" width="200%" height="200%"/></filter></defs>',
                    '<image x="25" y="25" height="60" width="60" href="https://ipfs.io/ipfs/QmNwjc9XJDXjuzYKVbESK7dGWdhS2Ba8uTJFFrDoqojugV"/>',
                    '<defs><linearGradient id="a" x1="0" y1="0" x2="270" y2="270" gradientUnits="userSpaceOnUse"><stop stop-color="#ff0"/><stop offset="1" stop-color="red" stop-opacity=".99"/></linearGradient></defs>',
                    '<text text-anchor="middle" x="135" y="231" font-size="20" fill="#000" filter="url(#b)" font-family="Monospace,sans-serif" font-weight="bold">',_name,'</text>',
                    '</svg>'
                )
            )
        );

        /// @dev If using the default Brand
        if (uptodate(records[id[tokenId]].brand)) {
            brand = records[id[tokenId]].brand;
        } else {
            brand = string(abi.encodePacked("Fronk Name Service"));
        }

        /// @dev If using the default Avatar or Logo
        if (uptodate(records[id[tokenId]].avatar)) {
            avatar = records[id[tokenId]].avatar;
        } else {
            avatar = string(
                abi.encodePacked("data:image/svg+xml;base64,", avatars)
            );
        }

        /// @dev If using the default text Tagline
        if (uptodate(records[id[tokenId]].tagline)) {
            tagline = records[id[tokenId]].tagline;
        } else {
            tagline = string(
                abi.encodePacked(
        ...
        // @dev If using the default Content (ipfs)
        if (uptodate(records[id[tokenId]].content)) {
            content = records[id[tokenId]].content;
        } else {
            content = string(abi.encodePacked("none"));
        }
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '", "image": "',
                        avatar,
                        '", "attributes": [{"trait_type": "Digit Club","value": "#',
                        strLen,
                        'DigitClub"}, {"trait_type": "Brand","value": "',
                        brand,
                        '"}, {"trait_type": "Content","value": "',
                        content,
                        '"}]}'
                    )
                )
    ...
    function setRecord(
        ...
        if (recordType == RecordType.BRAND) {
            records[name].brand = record;
        } else if (recordType == RecordType.AVATAR) {
            records[name].avatar = record;
        } else if (recordType == RecordType.TAGLINE) {
            records[name].tagline = record;
        } else if (recordType == RecordType.CONTENT) {
            records[name].content = record;
        }
    }
    ...
    function setAllRecords(
        ...
        records[name].brand = _brand;
        records[name].avatar = _avatar;
        records[name].tagline = _tagline;
        records[name].content = _content;
    }
    ...
    {
        if (recordType == RecordType.BRAND) {
            return records[name].brand;
        } else if (recordType == RecordType.AVATAR) {
            return records[name].avatar;
        } else if (recordType == RecordType.TAGLINE) {
            return records[name].tagline;
        } else if (recordType == RecordType.CONTENT) {
        ...
        returns (string[] memory, address)
    {
        ...
        string[] memory allRecords = new string[](4);

        allRecords[0] = records[name].brand;
        allRecords[1] = records[name].avatar;
        allRecords[2] = records[name].tagline;
        allRecords[3] = records[name].content;
