...
import "../Project/IFilmioProjectV1.sol";

contract StakeV1 is ReentrancyGuardUpgradeable, ERC2771Recipient {
    ...
    event StakeTokens(address user, uint256 stakeId, uint256 amount, address to, uint256 timestamp);

    event UnstakeTokens(address user, uint256 stakeId, uint256 quantity, uint256 timestamp);
    ...
    function staketokens(
        ...
        require(keccak256(abi.encodePacked((stakeType))) == keccak256(abi.encodePacked(("PRJ"))), "Invalid stake type");
        require(projectId != 0, "Invalid stake project id");
        ...
        require(doesProjectExist, "Given Project Id does not exist");
        ...
        //check for balance
        require(tokenInterface.balanceOf(_msgSender()) >= quantity, "Insufficient balance in source account");
        //check for allowance
        require(
    ...
    function unstaketokens(uint256 stakeId, uint256 quantity) external nonReentrant returns (bool) {
        require(stakeId != 0, "Invalid stake id = 0");

        uint256 indexStakes = stakeId - 1;
        //check if stakeId exists
        require((stakes[indexStakes] == stakeId), "stakeId not found");

        //get details of this stake using stake id
        Stake memory stakedetails = stakeDetails[stakeId];
        require(_msgSender() == stakedetails.account, "Only staker can unstake");
        require(stakedetails.quantity >= quantity, "Unstake quantity should be less than or equal to staked quantity");
        require(
            tokenInterface.allowance(escrowAccount, address(this)) >= quantity,
            "Escrow account has not approved stake contract"
        );
        require(tokenInterface.balanceOf(escrowAccount) >= quantity, "Insufficient balance in escrow account");
