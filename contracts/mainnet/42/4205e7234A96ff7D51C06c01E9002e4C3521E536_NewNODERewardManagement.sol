...
contract NewNODERewardManagement is Ownable {
    ...
    function withdrawAmount(address account, uint256 index) external onlyManager returns(uint256) {
        ...
        require(_node.isStake == 1, "Invalid Node Index");
        require(_node.expireTime <= block.timestamp, "Not Expired Yet");

        uint256 returnValue = 0;

        if (_node.nodeType == 1) {
            returnValue = nodePriceLesser;
            lesserNodesStake[account] = lesserNodesStake[account] - 1;
        } else if (_node.nodeType == 2) {
            returnValue = nodePriceCommon;
            commonNodesStake[account] = commonNodesStake[account] - 1;
        } else if (_node.nodeType == 3) {
            returnValue = nodePriceLegendary;
            legendaryNodesStake[account] = legendaryNodesStake[account] - 1;
        }
