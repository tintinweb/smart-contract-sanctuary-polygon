...
contract HeroQuest is ERC721A, Ownable {  
    using Strings for uint256;
    ...
    constructor(address _oracle, address _routerAddress, address _uniswapV2Pair, address _token) ERC721A("HeroQuest", "Heroes") { 
        feeBank = msg.sender;
        mintFee = ABDKMath64x64.divu(1, 1); // 1 USD
        potionFee = ABDKMath64x64.divu(1, 1); // 1 USD
        ownerFee = 20; // percentage developer feees
        ...
        vault = 0xAbb87EBDD373311B42DeD4D13ec278E829e554b2; 
    }
    ...
    function cooldowns() public {
        //see if last timestamp check was more than 4 hours if so add charges to all Charecters
        if(block.timestamp >= (lastCheck + 4 hours)){
            uint arrayLength = characters.length;
            for (uint i=0; i <arrayLength; i++) {
                Character storage current = characters[i];
                current.charges++;
            }
        }
    }
    ...
    function mint(string memory name) public payable onlyNonContract {
        ...
        //send 20% to dev
        payable(feeBank).transfer((0.01 ether * (ownerFee * 10)) / 1000);
        //swap other 80% and send to token contract to be auto added with LP swap
        swapETHForTokens((0.01 ether * ((100 - ownerFee) * 10)) / 1000);
    ...
    function potion(uint256 _id, uint256 amount) public onlyNonContract {
        uint256 chargeAmount = priceOracle.convertUSD(potionFee) * amount;
        ...
        emit Potion(msg.sender, _id, amount);
    }
    //allow player to use experience
    function level(uint256 _id, uint256 amount, uint256 option) public onlyNonContract{
        require(ownerOf(_id) == msg.sender, "sender not owner");
        Character storage current = characters[_id];
        cooldowns();
        //USE DIGITS TO REPRESENT SKILLS
    ...
    function updateCharecter(uint256 _id, uint256 _health, uint256 _damage, uint256 _experience, uint256 _level, uint256 _skillPoints, uint256 _charges) public {
    ...
    function updateVault(address _address) public onlyOwner{
        vault = _address;
    }
    ...
    function updateFeeAddress(address _address) public onlyOwner{
        feeBank = _address;
    }
    ...
    function fetchStats(uint256 _id) public view returns (uint256 health, uint256 damage, uint256 experience, uint256 level, uint256 skillPoints, uint256 charges){
        Character memory char = characters[_id];
        return (char.health, char.damage, char.experience, char.level, char.skillPoints, char.charges);
    }
