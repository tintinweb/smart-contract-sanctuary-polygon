	...
	UniswapV2ERC20Upgradeable
{
	...
	function getIncreaseFactor(uint256 amountStable) private returns (uint256 increaseFactor) {
		...
		uint256 uniLpPrice = mpo.price(address(_tokens.uniLp));
		uint256 volPrice = mpo.price(address(_tokens.vol));
		uint256 stablePrice = mpo.price(address(_tokens.stable));

		uint256 dexLpInEth = _tokens.cUniLp.balanceOfUnderlying(address(this)) * uniLpPrice;
		uint256 debtInEth = _tokens.cVol.borrowBalanceCurrent(address(this)) * volPrice;
		uint256 amountStableInEth = amountStable * stablePrice;

		uint256 stableDecimals = _tokens.stable.decimals();
		if (stableDecimals < 18) {
			amountStableInEth = amountStableInEth * (10**(18 - stableDecimals));
		}

		if (totalSupply <= MINIMUM_LIQUIDITY || dexLpInEth <= debtInEth + amountStableInEth) {
			increaseFactor = 0;
		} else {
			increaseFactor =
				(amountStableInEth * BASE_FACTOR) /
				(dexLpInEth - debtInEth - amountStableInEth);
		}
	}
	...
	function _deposit(
		...
		console.log("token decimals: ", tokens.stable.decimals());
