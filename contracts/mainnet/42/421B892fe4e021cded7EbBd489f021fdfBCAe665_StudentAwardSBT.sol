...
// File: contracts\EQ8dex\Roles.sol
...
// File: contracts\EQ8dex\MinterRole.sol
...
// File: contracts\EQ8dex\StudentAwardSBT.sol

//
pragma solidity ^0.8.0;
contract StudentAwardSBT is ERC721URIStorage, MinterRole {
    ...
    struct issuedStruct {
        ...
        string IssuerName;
        string ProductName;
        string TechnologyProviderCompany;	
        uint256 SchoolAdmissionID;	
        uint256 SchoolSoulID;
        address walletaddress;
        string ImgUrl;
    }

    uint256 public SBT_ID = 1;
    mapping(uint256 => studentStruct) public studentSbtInfo;
    mapping(uint256 => issuedStruct) public issuedSbtInfo;
    mapping(uint256 => bool) public burnedSbtId;
    event SBT_mint(address minter, address receiver, uint256 SBT_ID);
    event SBT_burn(uint256 SBT_ID);

    constructor() ERC721("Student_SoulBoundToken", "SBT") {
        admin = _msgSender();
    }


    function mint(address _to, studentStruct memory _studentStruct, issuedStruct memory _issuedStruct)
        external
        onlyMinter(_msgSender())
    {
        _mint(_to, SBT_ID);
        _issuedStruct.ProductName = "Prasasti";
        _issuedStruct.TechnologyProviderCompany = "NeuralByte Digital Solution";
        _studentStruct.SBTId = SBT_ID;
        studentSbtInfo[SBT_ID] = _studentStruct;
        issuedSbtInfo[SBT_ID] = _issuedStruct;
        emit SBT_mint(admin, _to, SBT_ID);
        SBT_ID++;
    }
    function updateSbtdata(uint256 _sbtId, studentStruct memory _studentStruct, issuedStruct memory _issuedStruct) external onlyMinter(_msgSender()){
        require(!burnedSbtId[_sbtId],"Sbt is burned");
        studentSbtInfo[_sbtId] = _studentStruct;
        issuedSbtInfo[_sbtId] = _issuedStruct;
    }

    function burn(uint256 _sbtId) external onlyMinter(_msgSender()) {
        _burn(_sbtId);
        burnedSbtId[_sbtId] = true;
        studentStruct memory _studentStruct;
        issuedStruct memory _issuedStruct;
        studentSbtInfo[_sbtId] = _studentStruct;
        issuedSbtInfo[_sbtId] = _issuedStruct;
