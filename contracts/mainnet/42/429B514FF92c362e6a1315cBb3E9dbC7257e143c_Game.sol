...
contract Game is IGame, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable{
    ...
    mapping (address => bool) public managementAddrs;
    ...
    function checkManagement(
        ...
        condition = nftId != 0 &&
            managementAddrs[management] &&
            INFTManagement(management).getOwnerOf(nftId) == account && 
            INFTManagement(management).freeInfos(INFTManagement(management).freeTypes(nftId), typeMissile) > amount + INFTManagement(management).freeUsed(nftId, typeMissile);
    }
    ...
    function setManagement(address management, bool state) external onlyOwner {
        managementAddrs[management] = state;
    }
