...
// File contracts/interfaces/ISiloFactory.sol
...
// File contracts/Gov/TierManager.sol



pragma solidity ^0.8.0;
contract TierManager is OwnableUpgradeable, UUPSUpgradeable {
    ...
    address[] public supportedLPTokens;

    function _authorizeUpgrade(address) internal override onlyOwner {}

    function initialize(address _siloFactory) public initializer {
        __Ownable_init();
        ...
        Factory = ICompounderFactory(
            0xDc15F68E5F80ACD5966c84f518B1504A7E1772CA
        );
        siloFactory = _siloFactory;
    }

    function updateSupportedShareTokens(
        address shareToken,
        uint256 index
    ) external onlyOwner {
        if (index < supportedLPTokens.length) {
            supportedLPTokens[index] = shareToken;
        } else {
            supportedLPTokens.push(shareToken);
        }
    }

    function updateSupportedLPTokens(
        address lpToken,
        uint256 index
    ) external onlyOwner {
        if (index < supportedShareTokens.length) {
            supportedShareTokens[index] = lpToken;
        } else {
            supportedShareTokens.push(lpToken);
        }
    }

    function updateSiloFactory(address _siloFactory) external onlyOwner {
        require(_siloFactory != address(0), "wrong silo factory");
        siloFactory = _siloFactory;
    }

    function updateGravityFactory(address _factory) external onlyOwner {
        require(_factory != address(0), "wrong  factory");
        factory = _factory;
    }

    function takeSnapshotOfAllSupportedShareTokens() external onlyOwner {
        for (uint256 i = 0; i < supportedShareTokens.length; i++) {
    ...
    function checkTier(address caller) external view returns (uint256) {
        uint256 bigBal = GFI.balanceOf(caller);
        uint256 userLPBal;
        for (uint256 i = 0; i < supportedLPTokens.length; i++) {
            userLPBal = IERC20(supportedLPTokens[i]).balanceOf(caller);
            bigBal +=
                (userLPBal * GFI.balanceOf(supportedLPTokens[i])) /
                IERC20(supportedLPTokens[i]).totalSupply();
        }
        bigBal += userSilosGFIBalance(caller);
    ...
    function viewAllGFIBalances(
        address caller
    ) external view returns (uint256[] memory) {
        uint256[] memory balances = new uint256[](
            supportedShareTokens.length + 4
        );

        balances[1] = GFI.balanceOf(caller);
        balances[2] = userSilosGFIBalance(caller);
        balances[3] = userLPGFIBalance(caller);
        balances[0] += balances[2] + balances[1] + balances[3];
        return balances;
    }

    //for IDOs
    function checkTierIncludeSnapshot(
        address caller
    ) external view returns (uint256) {
        uint256 bigBal;
        for (uint256 i = 0; i < supportedShareTokens.length; i++) {
            bigBal += IShare(supportedShareTokens[i])
                .getSharesGFIWorthAtLastSnapshot(caller);
        }
        bigBal += userSilosGFIBalance(caller);
        for (uint256 i = 3; i > 0; i--) {
            if (bigBal >= Governor.Tiers(i - 1)) {
    ...
    function userSilosGFIBalance(
        address _owner
    ) public view returns (uint256 balance) {
        if (siloFactory == address(0)) {
            return 0;
        }
        uint256 tokenCount = ISiloFactory(siloFactory).balanceOf(_owner);
        uint256 index;
        uint256 id;
        address silo;
        for (index = 0; index < tokenCount; ) {
            id = ISiloFactory(siloFactory).tokenOfOwnerByIndex(_owner, index);
            silo = ISiloFactory(siloFactory).siloMap(id);
            balance += GFI.balanceOf(silo);
            unchecked {
                index++;
            }
        }
    ...
    function userLPGFIBalance(
        address caller
    ) public view returns (uint256 balance) {
        if (factory == address(0)) {
            return 0;
        }
        IUniswapV2Factory gravityFactory = IUniswapV2Factory(factory);
        uint256 lpCount = gravityFactory.allPairsLength();
        uint256 index;
        uint256 userLPBal;
        address pair;
        for (index = 0; index < lpCount; ) {
            pair = gravityFactory.allPairs(index);
            if (GFI.balanceOf(pair) > 0) {
                userLPBal = IERC20(pair).balanceOf(caller);
                if (userLPBal > 0) {
                    balance +=
                        (userLPBal * GFI.balanceOf(pair)) /
    ...
    function userSilos(
        address _owner
    ) public view returns (address[] memory silos) {
        uint256 tokenCount = ISiloFactory(siloFactory).balanceOf(_owner);
        if (tokenCount == 0) {
            ...
            for (index = 0; index < tokenCount; index++) {
                id = ISiloFactory(siloFactory).tokenOfOwnerByIndex(
                    _owner,
                    index
                );
                result[index] = ISiloFactory(siloFactory).siloMap(id);
            }
            return result;
        }
    }

    function viewIDOTier(address caller) external view returns (uint256) {
        uint256 bigBal;
        for (uint256 i = 0; i < supportedShareTokens.length; i++) {
            bigBal += IShare(supportedShareTokens[i]).getSharesGFICurrentWorth(
                caller
            );
        }
        bigBal += userSilosGFIBalance(caller);
