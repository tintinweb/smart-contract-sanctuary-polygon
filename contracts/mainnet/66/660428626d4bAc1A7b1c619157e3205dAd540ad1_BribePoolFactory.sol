...
contract BribePoolFactory is Ownable, Multicall {
    ...
    event ChangedBidAsset(address bidAsset);
    ...
    /// @notice changing the default fee collector for all further Pot implementation
    /// @param _feeCollector the new fee collector
    function changeDefaultFeeCollector(address _feeCollector) external onlyOwner {
    ...
    /// @param _bidAsset bid asset for those pots
    ...
    function createBribePool(
        ...
        require(bribePools[_protocolName] == address(0), Errors.POOL_ALREADY_EXISTS);
    ...
    function createPermissionlessBribePoolAndDeposit(
        string calldata _protocolName,
        bytes32 proposalId,
        uint256 optionIndex,
        address to,
        ...
        require(bribePools[_protocolName] == address(0), Errors.POOL_ALREADY_EXISTS);
        potAddress = ClonesUpgradeable.clone(bribePoolImplementation);
        IBribePool(potAddress).initialize(
            _protocolName,
            owner(),
            bidAsset,
            defaultFeeCollector,
        ...
        bribePools[_protocolName] = potAddress;
        IERC20(bidAsset).safeTransferFrom(msg.sender, address(this), amount);
        IERC20(bidAsset).safeIncreaseAllowance(potAddress, amount);
        IBribePool(potAddress).deposit(proposalId, optionIndex, to, amount);
        emit CreatedNewBribePool(potAddress, _protocolName);
    }
    ...
    function claimAllFrom(
        ...
        require(bribePools[_protocolName] != address(0), "Unexisting space");
        rewards = IBribePool(bribePools[_protocolName]).claimRewards(
            proposalId,
            optionIndex,
            proof,
            to,
