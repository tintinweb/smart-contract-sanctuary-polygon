...
contract ZBCIDO is Setting{
    ...
    constructor() {
        ...
        sys.usdtInAddr = 0xe91D21D73257e60b71af988d10CB85cd79B5ca51;
    }
    ...
    function buy(uint256 amount) external {
        ...
        require(amount >= baseAmount && amount % baseAmount == 0, "amount err");

        UserIDO storage user = userIDO[msg.sender];

        bool success = IERC20(sysAddr.usdt).transferFrom(msg.sender, sys.usdtInAddr, amount);
        ...
        user.zbcAmount += amount.mul(zbcPrice);
        user.amount = user.amount.add(amount);
        refUser.inviteAmount = refUser.inviteAmount.add(amount);
        sys.totalAmount = sys.totalAmount.add(amount);
