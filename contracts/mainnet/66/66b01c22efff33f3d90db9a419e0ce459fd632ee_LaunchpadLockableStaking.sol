    ...
    IStakingLockable
{
    ...
    function isLocked(address account) public view override returns (bool) {
        return
            ...
            levelManager.isLocked(address(this), account);
    }
    ...
    function getLockPeriod() public view returns (uint256) {
        return
            address(levelManager) == address(0)
                ? 0
                : levelManager.getLockPeriod(address(this));
    }
    ...
    function getUnlocksAt(address account) public view returns (uint256) {
        ...
        return levelManager.getUserUnlocksAt(address(this), account);
    }
    ...
    function withdraw(uint256 amount) external {
        UserInfo storage user = userInfo[msg.sender];
        bool tokensLocked = isLocked(msg.sender);
        uint256 fee;
        ...
        updateUserDebt();
        emit Withdraw(msg.sender, amount, fee, tokensLocked);
    }
    ...
    function updateUserPending() internal {
        UserInfo storage user = userInfo[msg.sender];
        uint256 accRewardPerShare = liquidityMining.accRewardPerShare;

        if (user.amount > 0) {
            uint256 pending = ((user.amount * accRewardPerShare) / divider) -
                user.rewardDebt;
            if (pending > 0) {
    ...
    function updateUserDebt() internal {
        UserInfo storage user = userInfo[msg.sender];
        uint256 accRewardPerShare = liquidityMining.accRewardPerShare;
    ...
    function updatePool() internal {
        ...
        liquidityMining.accRewardPerShare =
            liquidityMining.accRewardPerShare +
            ((tokensReward * divider) / stakingTokenSupply);
        liquidityMining.lastRewardBlock = block.number;
    }
    ...
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        if (
            ...
            accRewardPerShare =
                accRewardPerShare +
                ((reward * divider) / stakingTokenSupply);
        }
    ...
    function isUserBlacklisted(address account)
        external
        view
        returns (bool)
    {
    ...
    function batchBlacklist(bool status, address[] calldata addresses)
        external
        onlyOwnerOrAdmin
    {
