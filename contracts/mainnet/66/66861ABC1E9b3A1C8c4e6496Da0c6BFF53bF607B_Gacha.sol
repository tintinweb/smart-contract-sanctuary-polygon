    ...
    */
    ...
    function checkRewardConditions(address _challengerAddress) public view returns(bool){
        ...
        if(checkRequireBalanceNft(IChallenge(_challengerAddress).challenger())) {
            ...
            if(currentChallengeInfo.targetStepPerDay <= IChallenge(_challengerAddress).goal()){
                if(currentChallengeInfo.challengeDuration <= challengeDuration){
                    ...
                    if(isCorrectStepDataToSend) {
                        if(IChallenge(_challengerAddress).dayRequired() >= challengeDuration.sub(challengeDuration.div(currentChallengeInfo.toleranceAmount))) {
                            if(
                                currentChallengeInfo.amountBaseDeposit <= IChallenge(_challengerAddress).totalReward() && 
                                IChallenge(_challengerAddress).allowGiveUp(1) ||
                                currentChallengeInfo.amountTokenDeposit <= IChallenge(_challengerAddress).totalReward() &&
                                !IChallenge(_challengerAddress).allowGiveUp(1)
                            ) {
