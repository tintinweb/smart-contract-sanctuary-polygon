...
contract Base is Authorised {
    ...
    function transferOut(ERC20 asset, address to, uint256 amount) external onlyAuthorised() {
...
contract AaveActions is Base {
    ...
    uint256 public targetHealthFactor = 120 * 1e16;
    ...
    function healthFactorStatus() public view returns (bool needsRebalancing, uint256 factor, uint256 ethAmount) {
        ...
        needsRebalancing = factor > 1e18 + 5e16 || factor < 1e18 - 5e16; // ± 5%
        if (factor > 1e18) {
            ethAmount = (1e18 * totalCollateralETH * currentLiquidationThreshold / 1e4 - targetHealthFactor * totalDebtETH) / targetHealthFactor;
        } else {
            ethAmount = (targetHealthFactor * totalDebtETH - 1e18 * totalCollateralETH * currentLiquidationThreshold / 1e4) / targetHealthFactor;
        }
    }
...
contract UniswapActions is Base {
    ...
    function getAveragePriceTick() public view returns (int24 tick) {
        ...
        tick = int24((tickCumulatives[0] - tickCumulatives[1]) / 3600);
    }
    ...
    function getAssetsInV3() public view returns (uint256 amount0, uint256 amount1) {
        (uint160 sqrtPriceX96, , , , , , ) = pool.slot0();
        uint160 sqrtRatioAX96 = TickMath.getSqrtRatioAtTick(tickLower);
        uint160 sqrtRatioBX96 = TickMath.getSqrtRatioAtTick(tickUpper);
        ...
        //(uint256 amount0v2, uint256 amount1v2,) = assetsInUniV2();
        //amount0 += amount0v2;
        //amount1 += amount1v2;
    }
    ...
    function _collectFees() internal returns (uint128 amount0, uint128 amount1) {
        ...
        (,,, amount0, amount1) = pool.positions(PositionKey.compute(address(this), tickLower, tickUpper));
        pool.collect(address(this), tickLower, tickUpper, amount0, amount1);
    }
    ...
    function _getLiquidityAndAmounts(int24 a, int24 b, uint256 max0, uint256 max1) internal view returns (uint256 amount0, uint256 amount1, uint128 liq) {
        (uint160 sqrtPriceX96, , , , , , ) = pool.slot0();
        uint160 sqrtRatioAX96 = TickMath.getSqrtRatioAtTick(a);
        uint160 sqrtRatioBX96 = TickMath.getSqrtRatioAtTick(b);
...
contract AaveUniStrategy is UniswapActions, AaveActions {
    ...
    function deploy(uint256 usdcAmount, uint128 minLiquidityAddeed) external onlyAuthorised returns (uint256 amount0, uint256 amount1, uint128 liquidityAdded) {
        ...
        uint256 toAave = 1e18 * usdcAmount / (1e18 + 1e18 * ratio / collateralisationRatio);
        _deposit(usdc, toAave);
        usdcAmount -= toAave;
    ...
    function compound(uint128 minimumLiquidityAdded) external onlyAuthorised returns (uint256 liquidityAdded) {
        _collectFees();
        liquidityAdded = _addV2Liquidity(minimumLiquidityAdded);
        _depositAvailable(usdc);
        _repayAvailable(weth);
    }

    function maintainHedge() external onlyAuthorised returns (int256 liquidityChange, int256 collateralChange, int256 debtChange) {
        ...
        if (hedgeFactor > 1e18 && healthFactor > 1e18) { // borrow weth, sell to usdc, lp
            ...
            uint128 liquidity = _getLiquidityForAmount1(amount);
            ...
            uint128 liquidity = _getLiquidityForAmount1(amount * 1e18 / valueRatio);
    ...
    function maintainHealthFactor() external onlyAuthorised returns (int256 liquidityChange, int256 collateralChange, int256 debtChange) {
        (bool fixHealthFactor, uint256 healthFactor, uint256 ethAmount) = healthFactorStatus();
        if (!fixHealthFactor) return (0, 0, 0);
        ethAmount = 1e18 * ethAmount / (1e18 + 1e4 * getValueRatio() / 8500);
        if (healthFactor > 1e18) {
            ...
            uint128 liquidity = _getLiquidityForAmount1(ethAmount);
    ...
    function resetRange() external onlyAuthorised returns (uint256 amount0, uint256 amount1, uint128 liq) {
        return _resetRange();
    }
