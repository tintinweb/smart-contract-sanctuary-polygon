...
contract wav3sRaffleTweetUSDT is VRFConsumerBaseV2{
    ...
    function executeRaffle(string calldata pubId) external stopInEmergency onlyWav3sTrigger {
        ...
        require (
        s_PubIdToRaffleState[pubId] == RaffleState.OPEN, 
         "Errors.wav3sRaffleTweetUSDT__executeRaffle__RaffleNotOpen"
         );
        require (
        ...
        requestRandomWinners(budget,reward,pubId);
    }
    

    function requestRandomWinners(uint256 budget, uint256 reward, string calldata pubId) internal {
        // Request random numbers
         // Cast budget and reward to uint32
    uint32 budget32 = uint32(budget);
    uint32 reward32 = uint32(reward);

    // Calculate the number of words to request
    uint32 numWords = budget32 / reward32;
