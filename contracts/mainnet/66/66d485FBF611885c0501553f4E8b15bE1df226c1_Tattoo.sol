...
contract Tattoo is Context,
    ...
    constructor(
        ...
        uint256 limit
    ) ERC721("OYABUN Tattoo", "OYABUN_TATTOO") {
        _baseTokenURI = baseTokenURI;
        royaltyPercent = _royaltyPercent;
        royaltyReceipient = _royaltyReceipient;
        limitTokensOwn = limit;
    ...
    function setLimitOwnTokens(uint256 limit) public onlyOwner {
        limitTokensOwn = limit;
    }

    function setRoyaltyPercent(uint256 newPercent) public onlyOwner {
        require(newPercent < maxPercent, "Tattoo: percent must be less than max");
    ...
    function mint(address to) public {
        require(hasRole(MINTER_ROLE, _msgSender()), "Tattoo: must have minter role to mint");
    ...
    function mintBatch(address to, uint256 quantity) public {
        require(hasRole(MINTER_ROLE, _msgSender()), "Tattoo: must have minter role to mint");
        if(ownedTokensCount[to] + quantity <= limitTokensOwn) {
    ...
    function pause() public {
        require(hasRole(PAUSER_ROLE, _msgSender()), "Tattoo: must have pauser role to pause");
        _pause();
    }

    function unpause() public {
        require(hasRole(PAUSER_ROLE, _msgSender()), "Tattoo: must have pauser role to unpause");
        _unpause();
    }
