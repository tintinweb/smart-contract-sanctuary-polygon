...
contract BlockchainBillboard is ERC721, ReentrancyGuard, Ownable, Pausable {
  ...
  string public baseUri =
    "https://blockchainbillboard.io/api/metadata/";
  ...
  function mint(
    ...
    require(verifySignature(tokenId, fee, signature), "Invalid signature");
  ...
  function tokenURI(
    ...
    return
      bytes(baseUri).length != 0
        ? string(
          abi.encodePacked(baseUri, Strings.toString(tokenId), uriSuffix)
        )
        : "";
  }
  ...
  function getTokenRange(
    ...
    for (uint256 i = from; i <= to; i++) {
      if (_exists(i))
        tokens[i - from] = TokenData({tokenId: i, uri: tokenURI(i)});
    }
    return tokens;
  }
  ...
  function verifySignature(
    ...
    bytes32 payloadHash = keccak256(abi.encode(tokenId, fee));
