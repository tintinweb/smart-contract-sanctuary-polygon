...
import { BaseVault, IBhavishAdministrator, IBhavishPrediction, IERC20Metadata, SafeERC20 } from "./BaseVault.sol";
import { IBhavishERC20SDK } from "../Interface/IBhavishERC20SDK.sol";
import { AccessControl } from "@openzeppelin/contracts/access/AccessControl.sol";

/**
 * @title Bhavish Lossy Vault
 * This works on lossy currency only
 */
contract BhavishLossyVault is AccessControl, BaseVault {
    using SafeERC20 for IERC20Metadata;

    IBhavishERC20SDK public bhavishSDK;
    IERC20Metadata public bgToken;

    constructor(
        IBhavishAdministrator _bhavishAdmin, // Bhavish Admin Address
        IERC20Metadata _asset, // This is underlying/wrapped/BGN Token
        IBhavishERC20SDK _bhavishSDK, // Bhavish SDK Address
        string memory tokenName, // Token Name of the Bhavish Vault token
        string memory tokenSymbol, // Token Symbol of the Bhavish Vault token
        bytes32 _underlying,
        bytes32 _strike,
        string memory _supportedCurrency
    ) BaseVault(_asset, _bhavishAdmin, tokenName, tokenSymbol, _supportedCurrency, _underlying, _strike) {
    ...
    function _performPrediction(
        ...
        bhavishSDK.predict(
            IBhavishERC20SDK.PredictionStruct(assetPair.underlying, assetPair.strike, _roundId, _nextPredictionUp),
            address(this),
            address(this),
            _predictAmount
        );
    ...
    function _performClaim(uint256[] calldata _roundIds, uint256 _slippage) internal override {
        bhavishSDK.claim(
            IBhavishERC20SDK.PredictionStruct(assetPair.underlying, assetPair.strike, 0, false),
            _roundIds
        );
    }

    function _performRefund(uint256 _roundId) internal override {
    ...
    function _performPredictionAfter(uint256 _amount) internal override {}

    function _performClaimAfter(uint256 _amount) internal override {}

    function _performRefundAfter(uint256 _amount) internal override {}

    function _safeTransfer(address _to, uint256 _value) internal override {}

    function setBhavishAdmin (IBhavishAdministrator _admin) external onlyOperator(msg.sender) {
        bhavishAdmin = _admin;
    }
    
    function depositAsset(uint256 _amount) external whenNotPaused nonReentrant {
        require(_amount > 0, "Invalid Deposit Amount");
        _depositToVault(_amount);

        bgToken.safeTransferFrom(msg.sender, address(this), _amount);
    }

    function withdrawAsset(uint256 shares) external nonReentrant {
        _withdrawFromVault(shares);
    }
}
 ...
 * @title Base Vault
 */
abstract contract BaseVault is ERC4626, Pausable, AccessControl, ReentrancyGuard {
    ...
    IBhavishPrediction.AssetPair assetPair;

    uint256 public predictionPerc;
    ...
    uint256 public withdrawFeeRatio = 0; // 0%
    uint256 public performanceFeeRatio = 1000; // 10% of profit made by vault
    ...
    function _performClaim(uint256[] calldata _roundIds, uint256 _slippage) internal virtual;

    function _performRefund(uint256 _roundId) internal virtual;

    function _performPredictionAfter(uint256 _amount) internal virtual;

    function _performClaimAfter(uint256 _amount) internal virtual;

    function _performRefundAfter(uint256 _amount) internal virtual;
    ...
    constructor(
        IERC20Metadata _asset, // This is the underlying asset's erc20 metadata
    ...
    function _getPerformanceFeeRatio(uint256 _amount) internal view returns (uint256) {
        return (_amount * performanceFeeRatio) / DENOMINATOR;
    }

    function _getWithdrawFeeRatio(uint256 _amount) internal view returns (uint256) {
        return (_amount * withdrawFeeRatio) / DENOMINATOR;
    }
    ...
    function _getFee(uint256 _amount, address _userAddress) internal view returns (uint256) {
        uint256 _userDepositedAmount = vaultDeposit.ledger[_userAddress];
        uint256 _userCurrentAmount = maxWithdraw(_userAddress);
        (, uint256 _profit) = _userCurrentAmount.trySub(_userDepositedAmount);
        uint256 _fee = _getWithdrawFeeRatio(_amount);

        if (_profit > 0) _fee += _getPerformanceFeeRatio(_profit.min(_amount));
    ...
    function getUserDeposit(address _userAddress) public view returns (uint256) {
        return vaultDeposit.ledger[_userAddress];
    }

    function getUserCount() public view returns (uint256) {
        return vaultDeposit.addressLedger.length;
    }

    function getAllUsers() public view returns (address[] memory) {
        return vaultDeposit.addressLedger;
    }

    function setWithdrawFee(uint256 _fee) public onlyOperator(msg.sender) {
        require(_fee <= MAX_WITHDRAW_FEE, "Cannot be > 1%");
        withdrawFeeRatio = _fee;
    }

    function setPerformanceFee(uint256 _fee) public onlyOperator(msg.sender) {
        require(_fee <= MAX_PERFORMANCE_FEE, "Cannot be > 50%");
        performanceFeeRatio = _fee;
    }

    function setMaxCapacity(uint256 cap) public onlyOperator(msg.sender) {
        ...
        returns (uint256 shares)
    {
        ...
        shares = previewDeposit(_assetAmount);

        if (vaultDeposit.ledger[msg.sender] == 0) vaultDeposit.addressLedger.push(msg.sender);

        vaultDeposit.ledger[msg.sender] += shares;
    ...
    function _withdrawFromVault(uint256 shares) public returns (uint256 assetAmount) {
        uint256 _amount = convertToAssets(shares);

        require(vaultDeposit.ledger[msg.sender] > 0, "Zero Assets");
        require(balanceOf(msg.sender) >= shares, "Insufficient shares");
        require(_amount <= address(this).balance, "cannot withdraw > contract balance");

        uint256 fee = _getFee(_amount, msg.sender);
        assetAmount = _amount - fee;

        vaultDeposit.ledger[msg.sender] -= shares;
        vaultDeposit.totalDeposit -= _amount;
        treasuryAmount += fee;

        emit WithdrawProcessed(msg.sender, assetAmount, shares);
        emit FeeCollected(fee);

        // Since we have withdraw function exposed in ERC4626 openzeppelin framework, will make use of it.
        _withdraw(msg.sender, msg.sender, msg.sender, assetAmount, shares);
    }

    function performPrediction(uint256 roundId, bool nextPredictionUp) external onlyOperator(msg.sender) {
        // Predict only Percentage defined from the current balance
        uint256 predictAmount = (address(this).balance * predictionPerc) / DENOMINATOR;

        _performPrediction(roundId, nextPredictionUp, predictAmount);

        vaultDeposit.totalDeposit -= predictAmount;

        _performPredictionAfter(predictAmount);
    }

    function performClaim(uint256[] calldata roundIds, uint256 slippage) external onlyOperator(msg.sender) {
        uint256 beforeBalance = address(this).balance;

        _performClaim(roundIds, slippage);

        uint256 claimedAmount = address(this).balance - beforeBalance;
        vaultDeposit.totalDeposit += claimedAmount;

        _performClaimAfter(claimedAmount);
    }

    function performRefund(uint256[] calldata roundIds) external onlyOperator(msg.sender) {
        uint256 beforeBalance = address(this).balance;

        for (uint256 i = 0; i < roundIds.length; i++) _performRefund(roundIds[i]);

        uint256 totalRefundAmount = address(this).balance - beforeBalance;
        vaultDeposit.totalDeposit += totalRefundAmount;

        _performRefundAfter(totalRefundAmount);
    }
    ...
    function transferToAdmin(uint256 _amount) external payable nonReentrant onlyAdmin(msg.sender) {
        ...
        _safeTransfer(payable(address(bhavishAdmin)), _amount);

        emit TransferToAdmin(address(bhavishAdmin), _amount);
    }
}
