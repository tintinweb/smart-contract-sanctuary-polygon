...
library Payment {
    ...
    function calBuyerCommission(
        ...
        require(serviceTaxPercentBuyer <= 10000, "Invalid tax");
        ...
        uint256 commissionBuyer = (amount * commission) / (10 ** 4);
    ...
    function matchOrder(
        ...
        uint256 _serviceTaxPercentSeller,
        uint256 _commissionBuyer,
        uint256 _serviceTaxAmountBuyer,
        ...
        require(_serviceTaxPercentSeller <= 10000, "Invalid tax");

        _order.commissionSeller = (_price * _commissionSellerPercent) / 10000;

        uint256 _serviceTaxAmountSeller = (_price * _serviceTaxPercentSeller) /
            10000;

        _order.serviceTaxSeller = _serviceTaxAmountSeller;
        _order.serviceTaxBuyer = _serviceTaxAmountBuyer;
        uint256 totalServiceTax = _serviceTaxAmountBuyer +
            _serviceTaxAmountSeller;

        taxToFeeTo = totalServiceTax;
        uint256 totalCommission = _commissionBuyer + _order.commissionSeller;
        ...
        uint256 amountToSeller = _buyerPaidAmount -
            totalServiceTax -
            totalCommission;

        if (!_sellerGetVAT) {
            amountToFeeTo += _VATAmount;
            amountToSeller -= _VATAmount;
        }
        ...
        _order.commissionBuyer = _commissionBuyer;
    }
...
error InvalidPremiumPercent();

contract VerdantMarketplace is Manager, ReentrancyGuardUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    /**
     * @dev Function to set new feeTo
     * @param _feeTo new feeTo address to set
     * Emit FeeToSet event
     */
    ...
    function createBid(
        ...
        require(order.paymentToken == _paymentToken, "Invalid-payment-method");
        require(
    ...
    function acceptBid(
        ...
        emit AcceptBid(
            ...
            StructLib.PaymentData(
                order.paymentToken,
                bid.bidPrice,
                orders[bid.orderId].taxReceived,
                verdantCommissionReceived,
                verdantServiceTaxReceived
            )
        );
