...
contract SmartSteps{
    ...
    address payable public  depositReceiver;
    mapping (address => User)  public userdata;

    function initinalize(address payable _admin,address payable _depositReceiver) external{
    ...
    function invest (address _refferal_code,uint _index,string memory _side) public payable{
       ...
       if (user.refferal_code == address(0) && admin != msg.sender) {
        ...
        user.buyStatus = true;
        user.side = _side;
        depositReceiver.transfer(msg.value);
