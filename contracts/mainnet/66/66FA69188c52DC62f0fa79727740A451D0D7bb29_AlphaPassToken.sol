...
// File: contracts/AlphaPassToken.sol
...
contract AlphaPassToken is ERC721EnumerableSimple, AlphaPassTokenI, NativeMetaTransaction, ContextMixin {
    ...
    mapping(uint256 => PassType) public passType;
    mapping(PassType => uint256) public maxAmount;
    mapping(PassType => uint256) public createdAmount;

    mapping(address => uint256) public signedTransferNonce;

    event MetananoDataChanged(address indexed previousMetananoData, address indexed newMetananoData);
    event BaseURIChanged(string value);
    event PermanentURI(string _value, uint256 indexed _id);

    constructor(address _metananoDataAddress, string memory _newBaseURI, uint256[] memory maxAmounts)
    ERC721("Meta Nanos Alpha Pass", "ALPHAPASS")
    EIP712Base("Meta Nanos Alpha Pass", "1.0")
    {
        ...
        require(maxAmounts.length == MAX_PASS_TYPE_COUNT);
        for (uint256 i = 0; i < MAX_PASS_TYPE_COUNT; i++) {
            maxAmount[PassType(i)] = maxAmounts[i];
        }
    }

    modifier onlySystemControl() {
        require(_msgSender() == metananoData.getAddress("systemControl"), "systemControl key required for this function.");
        _;
    }

    modifier onlyCreateControl() {
        require(msg.sender == metananoData.getAddress("tokenCreateControl"), "Token createControl key required for this function.");
        _;
    }
    ...
    function setMetananoData(MetaNanoDataI _newMetananoData)
    external
    onlySystemControl
    {
        require(address(_newMetananoData) != address(0x0), "You need to provide an actual metanano data contract.");
        emit MetananoDataChanged(address(metananoData), address(_newMetananoData));
        metananoData = _newMetananoData;
    }
    ...
    // Batch-issue assets with adjacent IDs.
    function createMulti(uint256 _amount, address _owner, PassType _passType)
    public
    onlyCreateControl
    {
        uint256 tokenIdStart = totalSupply();
        require(maxAmount[_passType] >= createdAmount[_passType] + _amount, "pass limit reached");
        createdAmount[_passType] += _amount;
        for (uint256 i = 0; i < _amount; i++) {
            uint256 newTokenId = tokenIdStart + i;
            passType[newTokenId] = _passType;
            _safeMint(_owner, newTokenId);
        }
    }

    //batch move operation for series of tokens. this will check permissions of all tokens in the subsequent safeTransferFrom
    function moveMulti(uint256 fromTokenInclusive, uint256 toTokenInclusive, address _destination)
    public
    {
        require(toTokenInclusive >= fromTokenInclusive, "to must be > from");
        for (uint256 tokenId = fromTokenInclusive; tokenId <= toTokenInclusive; tokenId++) {
            address from = ownerOf(tokenId);
            safeTransferFrom(from, _destination, tokenId);
        }
    }

    // This is required for OpenZeppelin to work with this base URI.
    function _baseURI()
    ...
    function setURIPermanent(uint256 _tokenId)
    public
    onlyCreateControl
    {
