...
contract MaticMan {
	...
	address payable public			WALLET_PROJECT2;
	...
	uint constant public			PROJECT_FEE						= 150;					// Investment Wallet 15%, it will be used for Investments only, rewards from investment will be send back to the contract for sustainability
	uint constant public			PROJECT2_FEE					= 50;					// Dev fee 3% of deposit + 1.5% of deposit for Marketing & Audit + 0.5% of Deposit for Support Crew
	uint constant public			DEV_FEE							= 50;					// CEO fee 3% of deposit + 1.5% of deposit for Marketing & Audit + 0.5% of Deposit for Support Crew
	uint constant public			MAX_WITHDRAW_AMOUNT				= 1000 ether;			// claim 1000 MATIC max
	...
	uint constant public			REINVEST_PERCENT				= 50;					// auto reinvest 5% of claim
	...
	constructor(address payable _walletProject, address payable _walletProject2, address payable _walletDev) {

		INSURANCE_CONTRACT = payable(new INSURANCE());
		WALLET_PROJECT = _walletProject;
		WALLET_PROJECT2 = _walletProject2;
	...
	function invest(address _referrer, uint8 _planIdx) public payable {
		...
		uint pfee2 = msg.value * PROJECT2_FEE / PERCENTS_DIVIDER;
		WALLET_PROJECT2.transfer(pfee);
		emit ProjectFeePaid(pfee2 + pfee);
