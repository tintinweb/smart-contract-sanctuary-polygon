...
contract VaultRewards is ReentrancyGuard, AccessControl {
    ...
    uint256 private avaibleRewards;
    ...
    constructor(address _tokenInfo, address _token, uint256 _percentage) {
        ...
        setmultisig(0x30268390218B20226FC101cD5651A51b12C07470); //actualizar a la multisig
        percentage= _percentage;
        //totalSupply = 14756413;
    }
    ...
    function setStrategyContract(address _approvedContract, bool _state) public {
    ...
    function deposit(uint256 _amount,  address _token) external onlyStrategyContract nonReentrant validToken(_token){
        ...
        avaibleRewards = (_amount + avaibleRewards); 
    }
    ...
    function seeDaily() public onlyStrategyContract returns (uint256 tempRewards)
    { 
        uint256 time = lastCall;
        uint256 share;
        if(time == 0){
              lastCall = block.timestamp;
              share = calcPercentage(); //Calculate portion to distribute this day
              avaibleRewards -= share; //update avaible rewards
              return tempTotalRewards = share;    
        }else{
        uint256 today = block.timestamp;
        uint256 Verify;
        today <= time ? Verify = (time-today) : Verify = (today - time);
        if(Verify < 24 hours){ //86400
    ...
    function calcPercentage() internal view returns(uint256 _portion){
        uint256 adjusment = avaibleRewards * 10**12;
    ...
    function updatePercentage(uint256 _percentage) public {
        if (!hasRole(DEV_ROLE, msg.sender)) {
             revert("have no dev role");
        }
    ...
    function updateDivider(uint256 _divider) public {
        if (!hasRole(DEV_ROLE, msg.sender)) {
             revert("have no dev role");
        }
