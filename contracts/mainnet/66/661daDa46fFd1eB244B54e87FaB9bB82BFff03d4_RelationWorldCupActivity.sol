...
contract RelationWorldCupActivity is Ownable, Initializable, AccessControl, SemanticBaseStruct {
    ...
    // The predicate index , use to check mint same spo
    uint256 private _pIndex;


    // Mapping of address of (Mapping of object to true/false)
    mapping(address => mapping(string => bool)) _hasObject;
    ...
    function _buildStringPOSignData(StringPO[] memory stringPOList) internal  returns (string memory signData){
        for (uint256 i = 0; i < stringPOList.length; i++) {
            ...
            if(intPO.pIndex == _pIndex){
                require(!_hasObject[msg.sender][intPO.o], "already claim");
                _hasObject[msg.sender][intPO.o] = true;
            }
    ...
    function _buildBlankNodePOSignData(BlankNodePO[] memory blankNodePOList) internal  returns (string memory signData){
        for (uint256 i = 0; i < blankNodePOList.length; i++) {
            ...
            signData = string.concat(signData, stringSignData);
        }
    }
