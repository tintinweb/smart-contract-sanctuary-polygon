...
contract Marketplace {
    ...
    event AuctionStarted(uint256 indexed itemId, address indexed seller, bool isAuction);
    ...
    function listForSale(uint256 itemId, uint256 price) external onlyValidCollection {
        ...
        items[itemId] = Item(msg.sender, price, true, false, 0, address(0));
    ...
    function startAuction(uint256 itemId) external onlyValidCollection {
        require(items[itemId].seller == address(0), "Item already listed");
        require(collection.ownerOf(itemId) == msg.sender, "You don't own this item");
        ...
        items[itemId] = Item(msg.sender, 0, false, true, 0, address(0));
        emit AuctionStarted(itemId, msg.sender, true);
    }
