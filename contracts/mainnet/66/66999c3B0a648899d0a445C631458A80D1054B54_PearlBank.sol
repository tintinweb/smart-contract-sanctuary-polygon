...
// Libraries
import {BaseContractV2} from './libraries/BaseContractV2.sol';
import {ContractWhitelist} from './libraries/ContractWhitelist.sol';
import {ERC20Upgradeable} from '@openzeppelin/contracts-upgradeable/token/ERC20/ERC20Upgradeable.sol';
import {ReentrancyGuardUpgradeable} from '@openzeppelin/contracts-upgradeable/security/ReentrancyGuardUpgradeable.sol';

// Interfaces
import {IERC20, IERC20Burnable} from './interfaces/IERC20.sol';
import {IERC721} from '@openzeppelin/contracts8/token/ERC721/IERC721.sol';
import {IERC721Receiver} from '@openzeppelin/contracts8/token/ERC721/IERC721Receiver.sol';

contract PearlBank is
    BaseContractV2,
    ContractWhitelist,
    IERC721Receiver,
    ...
    ERC20Upgradeable
{
    ...
    /// @notice token => user => number of reward epochs paid to otter
    mapping(address => mapping(address => uint256))
        public paidTokenRewardsPerStake;

    /// @notice if the token is an approved reward token
    mapping(address => bool) public isRewardToken;

    /// @notice the otter info for a otter
    mapping(address => Otter) public otterInfo;

    /// @notice token => user => rewards due to be paid to otter
    ...
    /// @notice address of the CLAM token
    address public CLAM;

    /// @notice charge withdraw fee if (current - last staked time) < withdrawFeeDuration
    uint256 public withdrawFeeDuration;

    /// @notice rate of withdraw fee, 50 = (50 / 10000) = 0.5%
    uint256 public withdrawFeeRate;

    address public rewardManager;

    /// @notice token => latest reward amount
    mapping(address => uint256) public latestTokenRewards;

    /// @notice token => the last staked number when add reward
    mapping(address => uint256) public lastTotalMultipliedStaked;

    modifier onlyRewardManager() {
        require(
            rewardManager == msg.sender,
            'only reward manager can call this'
        );
        _;
    }
    ...
    event Stake(address indexed addr, uint256 amount, uint256 multipliedAmount);
    event StakeMultiplier(
    ...
    event WithdrawMultiplier(
        address indexed addr,
        address indexed nft,
        uint256 indexed tokenId,
        uint256 multipliedAmount
    );
    event RewardAdded(address indexed token, uint256 amount, uint256 rps);
    event RewardClaimed(
        address indexed token,
        address indexed staker,
        uint256 amount
    );
    event RewardManagerUpdated(address newRewardManager);
    ...
    /**
     * @notice stake CLAM and get PEARL
     * @param amount_ the amount of CLAM to stake
     */
    function stake(uint256 amount_) external isEligibleSender nonReentrant {
        ...
        Otter memory info = otterInfo[msg.sender];
        if (info.amount == 0) {
        ...
        IERC20(CLAM).transferFrom(msg.sender, address(this), amount_);
        _mint(msg.sender, amount_);

        uint256 multipliedAmount = _getMultipliedAmount(amount_, msg.sender);

        // store the sender's info
        otterInfo[msg.sender] = Otter({
            amount: info.amount + amount_,
        ...
        emit Stake(msg.sender, amount_, multipliedAmount);
    }

    /**
     * @notice withdraw CLAM
     */
        ...
        returns (uint256)
    {
        ...
        require(info.amount >= amount_, 'Not enough staked balance');

        _collectRewards(msg.sender);
        _payRewards(msg.sender);
        uint256 fee = withdrawFee(msg.sender, amount_);

        uint256 multipliedAmount = _getMultipliedAmount(amount_, msg.sender);
        totalMultipliedStaked -= multipliedAmount;
        totalStaked -= amount_;
        if (info.amount == amount_) {
            delete otterInfo[msg.sender];
        } else {
            otterInfo[msg.sender] = Otter({
                amount: info.amount - amount_,
                multipliedAmount: info.multipliedAmount - multipliedAmount,
        ...
        if (fee > 0) {
            IERC20Burnable(CLAM).burn(fee);
        }
        IERC20(CLAM).transfer(msg.sender, amount_ - fee);
        _burn(msg.sender, amount_);

        emit Withdraw(msg.sender, amount_, fee);
    ...
    function setRewardManager(address rewardManager_) public onlyAdmin {
        rewardManager = rewardManager_;
        emit RewardManagerUpdated(rewardManager);
    }
     ...
     */
    function addReward(address token, uint256 amount)
        external
        onlyRewardManager
    {
        require(isRewardToken[token], 'Token is not approved');
        ...
        // console.log(
        //     'add reward: %s, reward per stake %s',
        //     amount,
        //     rewardPerStake
        // );
        ...
        latestTokenRewards[token] = amount;
        lastTotalMultipliedStaked[token] = totalMultipliedStaked;
        emit RewardAdded(token, amount, rewardPerStake);
    }

    /**
     * @notice pay rewards to a otter
     */
    ...
    function _resetPaidRewards(address user) private {
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            address token = rewardTokens[i];
            paidTokenRewardsPerStake[token][user] = totalTokenRewardsPerStake[
                token
            ];
        }
    }
    ...
    /**
     * @notice collect rewards from a otter
     * @param user the amount of clam to stake
     */
    ...
    function _collectRewardsForToken(address token, address user) private {
        Otter memory info = otterInfo[user];
        if (info.multipliedAmount > 0) {
            uint256 owedPerUnitStake = totalTokenRewardsPerStake[token] -
                paidTokenRewardsPerStake[token][user];
            // console.log(
            //     'total rewards: %s, paid: %s',
            //     totalTokenRewardsPerStake[token],
            //     paidTokenRewardsPerStake[token][user]
            // );
            uint256 totalRewards = (info.multipliedAmount * owedPerUnitStake) /
                SCALE;
            paidTokenRewardsPerStake[token][user] = totalTokenRewardsPerStake[
                token
            ];
            ...
            // console.log(
            //     'Collecting rewards %s toBePaid %s',
            //     user,
            //     toBePaid[token][user]
            // );
        }
     ...
     */
    function addRewardToken(address token) external onlyAdmin {
        require(!isRewardToken[token], 'Reward token exists');
     ...
     */
    function removeRewardToken(address token) external onlyAdmin {
        require(isRewardToken[token], 'Reward token does not exist');
        for (uint256 i = 0; i < rewardTokens.length; i++) {
    ...
    function setWithdrawFeeDuration(uint256 duration) external onlyAdmin {
        withdrawFeeDuration = duration;
    }

    function setWithdrawFeeRate(uint256 rate) external onlyAdmin {
     ...
     */
    function _getMultipliedAmount(uint256 amount, address account)
        private
        view
        returns (uint256 multipliedAmount)
    {
        if (!isWhitelisted(account)) {
            return 0;
        }
        uint256 multiplier = BASE;
        // TODO: Enabled NFT multipliers in the future
        ...
        //     }
        // }
        multipliedAmount = (amount * multiplier) / BASE;
    }

    /**
     * @notice get the available token rewards
     * @param staker the otter
     * @param token the token to check for
     * @return totalRewards - the available rewards for that token and otter
     */
    function getAvailableTokenRewards(address staker, address token)
        external
        view
        returns (uint256 totalRewards)
    {
        Otter memory info = otterInfo[staker];
        uint256 owedPerUnitStake = totalTokenRewardsPerStake[token] -
            paidTokenRewardsPerStake[token][staker];
        uint256 pendingRewards = (info.multipliedAmount * owedPerUnitStake) /
            SCALE;
        totalRewards = pendingRewards + toBePaid[token][staker];
    }

    function withdrawFee(address otter_, uint256 amount_)
        public
        view
        returns (uint256)
    {
        if (isWhitelistedContract(otter_)) {
            return 0;
        }
        Otter memory info = otterInfo[otter_];
        if ((block.timestamp - info.timestamp) > withdrawFeeDuration) {
            return 0;
        }
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0) || to == address(0)) {
            ...
            Otter memory info = otterInfo[to];
            if (info.amount == 0) {
    ...
    function _afterTokenTransfer(
        ...
        if (from == address(0) || to == address(0)) {
            ...
            Otter memory otterFrom = otterInfo[from];
            Otter memory otterTo = otterInfo[to];

            // get new multiplied amounts
            uint256 multipliedFromAmount = _getMultipliedAmount(
                fromBalance,
                from
            );
            ...
            if (isWhitelisted(from) && isWhitelisted(to)) {
                totalMultipliedStaked -= (otterFrom.multipliedAmount +
                    otterTo.multipliedAmount);
                ...
                if (!isWhitelisted(from)) {
                    totalMultipliedStaked -= otterTo.multipliedAmount;
                    totalMultipliedStaked += multipliedToAmount;
                }
            }
            // update otter info
            otterInfo[from] = Otter({
                amount: fromBalance,
                multipliedAmount: multipliedFromAmount,
                timestamp: otterFrom.timestamp
            });
            otterInfo[to] = Otter({
                amount: toBalance,
                multipliedAmount: multipliedToAmount,
                timestamp: otterFrom.timestamp > otterTo.timestamp
                    ? otterFrom.timestamp
                    : otterTo.timestamp
            });
        }
    ...
    function onERC721Received(
        ...
        return PearlBank.onERC721Received.selector;
    }
}
...
abstract contract ContractWhitelist is BaseContractV2 {
    ...
    function isWhitelistedContract(address addr) public view returns (bool) {
