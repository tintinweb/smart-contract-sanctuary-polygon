...
contract ERC721PaymentOpenSale is App, __with_semver(uint8(1), uint8(0), uint8(0)) {
        ...
        returns (uint256 stablePrice, uint256 tokenPrice)
    {
        ...
        require(
            UniswapV3Oracle(oracleAddress).isWhitelistTokenAddress(tokenAddress), 
    ...
    function buy(
        ...
        require((++claims[tierValue]) <= claimLimitByTier[tierValue], Exceptions.BUY_LIMIT_ERROR);
        address oracleAddress = kernel.getOracleAddress();
        require(
            UniswapV3Oracle(oracleAddress).isWhitelistTokenAddress(tokenAddress), 
            Exceptions.UNSUPPORTED_TOKEN_ADDRESS
        ...
        require(balanceOfSender >= costInTokens, Exceptions.INSUFFICIENT_AMOUNTS_OF_FUNDS);
    ...
    function _setStableCoinAddress(address stableCoinAddress_) internal requirePermission(CONTROLLER) {
        require(
            stableCoinAddress_ != address(0) && stableCoinAddress_.code.length > 0, 
