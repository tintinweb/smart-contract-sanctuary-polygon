...
contract Crypton is ERC20 {
    string public constant symbol = "mCRTN";
    string public constant name = "MATICRYPTON";
    uint8 public constant decimals = 18;
    uint private constant __totalSupply = 1000000 * (10 ** 18);
    address public owner;
 
    uint public amountAllowed = 0;
    uint public amountMaticAllowed = 5 * (10 ** 16);
    uint public claimedAtFaucet = 0;
    uint public claimedMaticAtFaucet = 0;
    ...
    function requestMaticTokens(address payable _requestor) public payable {
        require(block.timestamp > lockTime[msg.sender], "Lock time has not expired. Please try again later");
        require(address(this).balance > amountMaticAllowed, "Not enough matic funds in the faucet. Please donate.");
        _requestor.transfer(amountMaticAllowed);
        claimedMaticAtFaucet += amountMaticAllowed;
