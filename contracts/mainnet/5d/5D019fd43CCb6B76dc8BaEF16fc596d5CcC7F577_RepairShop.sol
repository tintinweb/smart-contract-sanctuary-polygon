...
contract RepairShop is WarpBase, IRepairShop {
    ...
    function repairShip(
        ...
        uint256 totalAmount = (_hullAmount + _bridgeAmount + _engineAmount) * 1e18;
        IERC20Upgradeable(dai).safeTransferFrom(msg.sender, address(this), totalAmount); //pay the battle bid

        IERC20Upgradeable(dai).safeTransfer(growth, totalAmount / 2); //50% to growth
        IERC20Upgradeable(dai).safeTransfer(treasury, totalAmount - (totalAmount / 2)); //50% to treasury
