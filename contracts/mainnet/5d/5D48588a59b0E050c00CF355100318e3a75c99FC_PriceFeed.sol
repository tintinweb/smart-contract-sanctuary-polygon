...
/**
 * @title PriceFeed
 * @notice Allow to set property related stuff and fetch prices of different assets from Chainlink
 */
contract PriceFeed is IPriceFeed, StandardAccessControlEnumerable {
    ...
    event CurrencyToFeedUpdated(
        string indexed _pairName,
        address _currency,
        address _feed
    );

    event PropertyDetailsUpdated(
        string indexed _propertySymbol,
        IPriceFeed.Property _propertyDetails
    );
    ...
    /**
     * @param _pairName pair name to fetch the price of
     * @return returns latest price from chainlink of the pair
     */
    function getLatestPrice(
        ...
        _require(
            ...
            Errors.INVALID_PAIR_NAME
        );
        return uint256(_getLatestPrice(_pairName));
    }

    /**
     * @param _pairName pair names to fetch the prices of
     * @return returns latest prices from chainlink of the pairs
     */
    function getLatestPrices(
        string[] calldata _pairName
    ) external view returns (uint256[] memory) {
        uint256[] memory prices = new uint256[](_pairName.length);
    ...
    function _getLatestPrice(
        ...
        address _feedAddress = storageParams.nameToFeed[_pairName];
        return _fetchPriceFromChainlink(_feedAddress);
    }

    /**
     * @param _of address of feed to get price of
     * @return returns latest price from chainlink of the feed
     */
    function feedPriceChainlink(address _of) external view returns (uint256) {
        return _fetchPriceFromChainlink(_of);
    }

    /**
     * @param _feedAddress address of feed to get price of
     * @return latestPrice returns scaled(18 decimals) latest price from chainlink of the feed.
     */
    function _fetchPriceFromChainlink(
        address _feedAddress
    ) internal view returns (uint latestPrice) {
        (
            uint80 roundId,
            int price,
            ,
            uint256 updatedAt,
            uint80 answeredInRound
        ) = AggregatorV3Interface(_feedAddress).latestRoundData();
        require(updatedAt != 0, "Invalid updatedAt timestamp");
        // to ensure that price used is not older than 24 hours
     ...
     * @param currency baseCurreny of property
     * @return returns pershare price of property in base currency
     */
    function getSharePriceInBaseCurrency(
        string memory _propertySymbol,
        address currency
        ...
        if (
            ...
            _revert(Errors.INVALID_BASE_CURRENCY);
        }
        return storageParams.propertyDetails[_propertySymbol].price;
    }

    /**
     * @notice to set the property details e.g price, feedAddress, baseCurrency. Make sure pause buy/sell in Marketplace before calling this to avoid frontRunning
     * @param _propertySymbol symbol of property
     * @param _propertyDetails details of property . see IPriceFeed.Property
     */
    function setPropertyDetails(
        string memory _propertySymbol,
    ...
    /**
     * @param _propertySymbol symbol of property
     * @return property returns details of property
     */
    function getPropertyDetail(
        string memory _propertySymbol
    ...
    /**
     * @param _properties symbol of properties
     * @return returns details of multiple properties
     */
    function getPropertyDetails(
        string[] memory _properties
    ) external view returns (IPriceFeed.Property[] memory) {
        IPriceFeed.Property[] memory property = new IPriceFeed.Property[](
            _properties.length
        );
    ...
    /**
     * @notice sets the feed address against the currency address
     * @param _pairName name of pair
     * @param _currency currency of pair
     * @param _feed nchainlink feed address of currency
     */
    function setCurrencyToFeed(
        ...
        emit CurrencyToFeedUpdated(_pairName, _currency, _feed);
    }

    /**
     * @param _currency address of currency
     * @return returns address of chainlink feed against the currency
     */
    function getCurrencyToFeed(
        address _currency
    ) external view returns (address) {
