...
import "../interfaces/IPopStakingV2.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

contract PopStakingV2 is IPopStakingV2, Ownable, ReentrancyGuard {
    ...
    constructor (
        ...
        _popToken = popAdd;
        _stakeOut = stakeOut;
        _releasePerBlock = 1 ether;
        _oneDayBlocks = 42000;//42000
        _linearRate = 8;
        _startBlockNumber = startBlock;
        _endBlockNumber = startBlock + 50457600;
        _veType = [0.1 ether, 0.205 ether, 0.42 ether, 1.28 ether, 2.54 ether, 5 ether];
        _timestampType = [604800, 1209600, 2592000, 7776000, 15552000, 31536000];
        _stakingType = [294000, 588000, 1260000, 3780000, 7560000, 15330000];
    }
    ...
    function updatePool() public {
        ...
        require(curBlock >= _startBlockNumber, "NOT START");
    ...
    function deposit(uint256 lockAmount, uint256 lockType) public override nonReentrant {
        ...
        require(currentBlock >= _startBlockNumber, "NOT START");
        ...
        _userInfo[msg.sender].push(
            UserInfo({
                ...
                poolType: lockType,
                amount: lockAmount,
                veTokenAmount: _veType[lockType] * lockAmount / 1e18,
    ...
    function reedem(uint256 userIndex) public override nonReentrant {
        ...
        emit Reedem(msg.sender, reedemAmount, _userInfo[msg.sender][userIndex].veTokenAmount);
    }
    ...
    function getRewardPerBlock(uint256 blockNumber) public view override returns (uint256 perBlockReward) {
        if (blockNumber >= _startBlockNumber) {
            ...
            uint256 epic = throughBlock / _oneDayBlocks;
            perBlockReward = _releasePerBlock + (epic + 1) * _linearRate * _releasePerBlock / 10000;
        }
    }
            ...
            if (popPerInStart == popPerInEnd) {
                ...
                for (uint i; i < wholeEpic; i++) {
                    wholeBlockPopRelease += oneDayBlocks * popPerInStart 
                                            + oneDayBlocks * releasePerBlock * _linearRate * (i + 1) / 10000;
                }
    ...
    function setRelease(uint256 newRelease) public onlyOwner {
        _releasePerBlock = newRelease;
    }

    function setLinearRate(uint256 newRate) public onlyOwner {
        _linearRate = newRate;
    }

    function setVe(uint256 index, uint256 newVe) public onlyOwner {
        _veType[index] = newVe;
    }
...
interface IPopStakingV2 {
    event Deposit(address sender, uint256 amount, uint256 lockType);
    event Reedem(address recipient, uint256 amount, uint256 veAmount);
