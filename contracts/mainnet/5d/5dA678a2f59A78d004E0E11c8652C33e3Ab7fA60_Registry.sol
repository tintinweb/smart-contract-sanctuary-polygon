...
interface IMetaPipe {
    ...
    event PipeCreated(uint256 indexed tokenId, address indexed owner, address indexed backref);
    ...
    function verify(RelayMetaRequest calldata request, bytes calldata signature) external view returns (bool);

    function executePassThrough(PassThroughRequest calldata request) external returns (bool, bytes memory);

    function executeRelayMeta(RelayMetaRequest calldata request, bytes calldata signature) external returns (bool, bytes memory);
}

interface IMetaPipeFactory {
    function deploy(address backref, uint256 tokenId) external returns (address);
}
...
interface IRegistry {
    ...
    event CycleDepositMade(address indexed sender, uint256 indexed tokenId, uint256 nextDeadlineAt, uint256 gasLeft);

    event GasSupplyWithdrawn(address indexed sender, uint256 indexed tokenId, uint256 gasAmount);

    function makeCycleDepositFor(uint256 tokenId) external;

    function execute(uint256 tokenId, IMetaPipe.RelayMetaRequest calldata request, bytes calldata signature) external;
...
contract Registry is IRegistry, ERC721 {
    ...
    constructor(address protocolGovernance_) ERC721("MetaPipe Registry", "MetaPipe-REG") {
    ...
    function mint(address to) external{
        ...
        IMetaPipe metaPipe = IMetaPipe(factory.deploy(address(this), newTokenId));
    ...
    function makeCycleDepositFor(uint256 tokenId) external {
        ...
        params.underlyingToken.safeTransferFrom(msg.sender, params.treasury, params.cost);
        record.gasLeft += params.gasSupply;
        record.deadlineAt = (
            block.timestamp > record.deadlineAt ? block.timestamp : record.deadlineAt
        ) + uint256(params.duration);   
        recordOf[tokenOwner] = record;

        emit CycleDepositMade(msg.sender, tokenId, record.deadlineAt, record.gasLeft);    
    }
    ...
    function execute(uint256 tokenId, IMetaPipe.RelayMetaRequest calldata request, bytes calldata signature) external {
        ...
        (uint256 gasLeft, IMetaPipe pipe) = (record.gasLeft, record.metaPipeAddress);

        require(pipe.verify(request, signature), Exceptions.SIGNATURE_VERIFICATION_ERROR);
    ...
    function _beforeTokenTransfer(address from, address, uint256) internal view virtual override {
        require(from == address(0), Exceptions.PIPE_NO_BURN_NO_TRANSFER);
    }

    function _afterTokenTransfer(address, address to, uint256) internal view virtual override {
        require(balanceOf(to) == 1, Exceptions.PIPE_ALREADY_INITIALIZED);
    }

    function _requireProtocolGovernance() internal view {
        require(msg.sender == Ownable(address(protocolGovernance)).owner(), Exceptions.ONLY_PROTOCOL_GOVERNANCE);
    }
}
...
contract MetaPipe is IMetaPipe, EIP712 {
    ...
    bytes32 private constant _TYPEHASH = keccak256("RelayMetaRequest(address owner,address to,uint256 value,uint256 gas,uint256 deadline,uint256 nonce,bytes data)");
    ...
    constructor(address backref_, uint256 tokenId_) EIP712("MetaPipe Instance", "V1") {
        tokenId = tokenId_;
        backref = backref_;
    ...
    function executePassThrough(
        PassThroughRequest calldata request
    )
        external
        returns (bool success, bytes memory returndata) 
    {
        ...
        (success, returndata) = request.to.call{value: request.value}(request.data);
        if (!success) { _revert(); }
    ...
    function executeRelayMeta(
        ...
        returns (bool success, bytes memory returndata) 
    {
        require(verify(request, signature), Exceptions.SIGNATURE_VERIFICATION_ERROR);
