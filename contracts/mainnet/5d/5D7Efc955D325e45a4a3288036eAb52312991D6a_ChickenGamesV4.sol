	...
	OwnableUpgradeable
{
	...
	//Mega Release Update
	mapping(uint256 => uint256) public potatoChipBalanceByChickenId;
	mapping(uint256 => bytes32) public prevRequestIdByChickenIdV2;
		...
		onlyChickenOwner(chickenId)
	{
		...
		prevRequestIdByChickenIdV2[chickenId] = randomnessRequestId;
	...
	function claimChickenGamePotatoChipReward(uint256 chickenId)
		public
		onlyChickenOwner(chickenId)
	{
		(bool isDetermined, uint256 reward) = getGameRewardV2(chickenId);
		(chickenId);
		require(isDetermined, "Outcome not yet determined");
		if (reward > 0) potatoChipBalanceByChickenId[chickenId] += reward;
		prevRewardHasBeenClaimed[chickenId] = true;
	}
	...
	function withdrawPotatoBalanceV2(uint256 chickenId)
		public
		onlyChickenOwner(chickenId)
	{
		uint256 balance = potatoChipBalanceByChickenId[chickenId];
		require(balance > 0, "No rewards available");
	...
	function getGameRewardV2(uint256 chickenId)
		public
		view
		returns (bool _isDetermined, uint256 _rewardPotato)
	{
