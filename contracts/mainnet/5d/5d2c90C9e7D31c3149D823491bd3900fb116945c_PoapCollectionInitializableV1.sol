 ...
 * @author Csongor Barabasi & Philipp Klein
 * @dev this contract will not be deployed directly.
 * New clones shall be created through `PoapCollectionCloneFactoryV1`.
 ...
 * `creator` and `roleAdmin` have authority to grant and revoke roles
 * later on.
 *
 * Accounts:
 *
 ...
 * `MANAGER_ROLE`
 * - attached to `manager`
 * - can call all sensitive operations.
 */
contract PoapCollectionInitializableV1 is
    Initializable,
    ERC721Upgradeable,
    ...
    PausableUpgradeable
{
     ...
     * {managerMint} can only be called if this is set to true.
     */
    ...
    /**
     * @notice Struct for initializing the `PoapCollectionInitializableV1`.
     * @param _roleAdmin address of the account that will have `DEFAULT_ADMIN_ROLE`.
     * Can grant and revoke roles to other accounts on the new `PoapCollectionInitializableV1`.
     * Can NOT mint, pause, ect. on the new `PoapCollectionInitializableV1`.
     * @param _manager account that will have `MANAGER_ROLE` on the new `PoapCollectionInitializableV1`.
    ...
    /**
     * @notice initialize the `PoapCollectionInitializableV1`.
     * @dev clones of this contract are created by the `manager`
     * account of fuel-core through it's `PoapCollectionCloneFactoryV1` contract
    ...
    /**
     * @notice publish the `PoapCollectionInitializableV1` as active for sale.
    ...
    /**
     * @notice allows managers to recover tokens that have accidentally
    ...
    /**
     * @notice get the baseURI for NFT metadata
     * @notice every tokenid will have the same uri
     * @return the base uri of the metadata
     */
