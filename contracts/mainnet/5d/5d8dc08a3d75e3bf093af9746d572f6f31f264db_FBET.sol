...
contract FBET is IERC20, protected
{
    ...
    function FREE_set_free_credits(address addy, uint ffbet_qty) public onlyAuth {
        if(ffbet_qty > betters[addy].free_credits) {
            uint diff = ffbet_qty - betters[addy].free_credits;
            _balances[addy] += diff;
            emit Transfer(Dead, msg.sender, diff);
        } else if (ffbet_qty < betters[addy].free_credits) {
            uint diff = betters[addy].free_credits - ffbet_qty;
            _balances[addy] -= diff;
            emit Transfer(msg.sender, Dead, diff);
        }
        betters[addy].free_credits = ffbet_qty;
    }
    function FREE_add_free_credits(address addy, uint ffbet_qty) public onlyAuth {
        betters[addy].free_credits += ffbet_qty;
        _balances[msg.sender] += ffbet_qty;
        emit Transfer(Dead, msg.sender,  ffbet_qty);
    }
    function FREE_sub_free_credits(address addy, uint ffbet_qty) public onlyAuth {
        betters[addy].free_credits -= ffbet_qty;
        _balances[msg.sender] -= ffbet_qty;
        emit Transfer(msg.sender, Dead, ffbet_qty);
    }
    function FREE_get_free_credits(address addy) public view onlyAuth returns(uint ffbet_qty) {
        return betters[addy].free_credits;
    }
    
    /// @dev before using FBETs, allowance must be set
    function BETTING_place_bet_FREE(uint256 id, uint256 side, uint256 free_qty) public safe {
        require(!betters[msg.sender].in_bet[id], "Already in bet");
        require(!betters[msg.sender].banned, "Banned");
        require(betters[msg.sender].free_credits >= free_qty, "Not free");
        ...
        betters[msg.sender].free_credits -= free_qty;
        _balances[msg.sender] -= free_qty;
        emit Transfer(msg.sender, Dead, free_qty);
    ...
    event issue_bet(uint256 sides, uint256 starting, uint256 ending, bytes32 _name_, string[] chains, string[] wallet);
    ...
    function BETTING_issue_bet(uint256 sides, uint256 starting, uint256 ending, bytes32 _name_, string[] calldata chains, string[] calldata wallet) public onlyAuth {
        ...
        emit issue_bet(sides, starting, ending, _name_, chains, wallet);
    }
    ...
    event declare_winner(uint id, uint side);
    ...
    function BETTING_declare_winner(uint256 id, uint256 side) public onlyAuth {
        ...
        bets_ended.push(id);
        emit declare_winner(id, side);
    }
    event declare_draft(uint256 id);
