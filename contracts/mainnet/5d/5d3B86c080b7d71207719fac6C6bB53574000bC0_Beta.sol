...
contract Beta is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Distribution for launchpad and initial liquidity
    uint256 public constant LAUNCH_PAD_ALLOCATION = 300 ether;
    uint256 public constant INITIAL_LIQUIDITY_AMOUNT = 200000 ether;

    // Have the rewards been distributed to the pools
    bool public launchpadDistributed = false;
    ...
    /**
     * @notice Constructs the BETA ERC-20 contract.
     */
    constructor(uint256 _taxRate) public ERC20("BETA", "BETA") {
        // Mints 1 BETA to contract creator for initial pool setup
    ...
    function _getBetaPrice() internal view returns (uint256 _betaPrice) {
        try IOracle(oracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Beta: failed to fetch BETA price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _betaPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_betaPrice >= taxTiersTwaps[tierId]) {
    ...
    /**
     * @notice Operator mints BETA to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BETA to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBetaPrice = _getBetaPrice();
            currentTaxRate = _updateTaxRate(currentBetaPrice);
            if (currentBetaPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(!launchpadDistributed, "only can distribute once");
        require(_launchPadContract != address(0), "!_launchpad");
        require(_liquidityOwner != address(0), "!_owner");
        launchpadDistributed = true;
        _mint(_launchPadContract, LAUNCH_PAD_ALLOCATION);
        _mint(_liquidityOwner, INITIAL_LIQUIDITY_AMOUNT);
    }
