...
contract RentalSettingsFacet is Modifiers{
    ...
    function setRentalSettings(
        ...
        for(uint256 i = 0; i < _tokenIds.length; ){
            ...
            //we allow someone to set the gotchi rental settings if they are the depositor (the gotchi is in the vault) OR
            //they're the current owner and the gotchi is not lent out
            require(
                (s.deposits[im_diamondAddress][_tokenIds[i]] == msg.sender) ||
                (IAavegotchi(im_diamondAddress).ownerOf(_tokenIds[i]) == msg.sender && !(IAavegotchi(im_diamondAddress).isAavegotchiLent(uint32(_tokenIds[i])))),
                    "setRentalSettings: not gotchi depositor or owner");

            //we need to maintain our percentage cut for the vault
            require(_revenueSplit[2] == s.defaultSettings.revenueSplit[2],"setRentalSettings: cannot override minimum vault split");
    ...
    function resetToDefaultRentalSettings(uint256[] calldata _tokenIds) public {
        for(uint256 i = 0; i < _tokenIds.length; ){
            require(s.deposits[im_diamondAddress][_tokenIds[i]] == msg.sender, "setRentalSettings: not gotchi depositor");
...
struct AppStorage {
    ...
    mapping(address => address) receiptContracts;
}
