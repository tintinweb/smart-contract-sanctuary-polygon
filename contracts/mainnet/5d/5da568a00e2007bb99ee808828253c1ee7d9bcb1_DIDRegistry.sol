 ...
 */
contract DIDRegistry is ERC721, IERC2981, Ownable, IRegistry {
    ...
    IImage internal image;

    constructor(
        string memory _name,
        string memory _symbol,
        address _nameValidator,
        address _image
    ) ERC721(_name, _symbol) Ownable() {
        setNameValidator(_nameValidator);
        setImage(_image);
    }
    ...
    modifier onlyController() {
        require(controllers[msg.sender], "DID: only controller");
        _;
    }

    /**
     * IERC2981: Returns zero royalty fee.
     */
    function royaltyInfo(uint256 tokenId, uint256 salePrice)
        external
        view
        ...
        returns (string memory)
    {
        ...
        string memory name = reverseLookup(owner);
        return image.tokenURI(owner, name);
    }

    function imageURI(uint256 tokenId) public view returns (string memory) {
        address owner = ownerOf(tokenId);
        string memory name = reverseLookup(owner);
        return image.imageURI(owner, name);
    }
    ...
    function setNameValidator(address _validator) public override onlyOwner {
        emit NameValidator(address(validator), _validator);
        validator = INameValidator(_validator);
    }

    function setImage(address _image) public override onlyOwner {
        emit Image(address(image), _image);
        image = IImage(_image);
    }
        ...
        returns (address[] memory)
    {
        ...
        for (i = 0; i < _names.length; i++) {
            results[i] = _ownerOf(_toId(_names[i]));
        }
        return results;
    }
        ...
        returns (uint256, uint256)
    {
        ...
        require(len > 0 && len <= 50, "DID: invalid name length");
    ...
    function batchBind(
        ...
        require(ownerOf(tokenId) == registrant, "DID: not name owner");
        uint256 len = bindLabels.length;
        require(len == values.length, "DID: not same length");
    ...
    function _bind(
        ...
        require(len <= 255, "DID: value too long");
        uint256 labelId = _toId(label);

        if (len == 0) {
            require(labels[labelId] > 0, "DID: unsupported label"); // label is enabled or disabled
            delete records[tokenId][labelId];
        } else {
            require(labels[labelId] == LABEL_ENABLED, "DID: unsupported label"); // label is enabled
    ...
    function _beforeTokenTransfer(
        ...
        require(balanceOf(to) == 0, "DID: already registered");
    }
...
interface IImage {
    function tokenURI(address owner, string memory name)
        external
        view
        returns (string memory);

    function imageURI(address owner, string memory name)
        external
        view
        returns (string memory);
}
...
interface IRegistry is IERC721, IERC721Metadata {
    ...
    function setImage(address image) external;
    ...
    event Image(address oldImage, address newImage);
}