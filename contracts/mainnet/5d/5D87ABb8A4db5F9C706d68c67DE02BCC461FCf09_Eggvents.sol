...
contract Eggvents is ERC721A, EIP712, Ownable {
    ...
    constructor(
        ...
        // address givenOracle
        ERC721A("Eggvents", "EGGVNTS")
        EIP712("teh-raptor", "1.0")
    {
    ...
    function discover(
        ...
        _mint(receiver, 1);
        eggIdToType[_newTokenId] = tokenType;

        // owner to token types
        string[] storage collectedTypes = ownerToEggTypes[receiver];
        collectedTypes.push(tokenType);
        ownerToEggTypes[receiver] = collectedTypes;

        // owner to token ids
        uint256[] storage collectedIds = ownerToEggIds[receiver];
        collectedIds.push(_newTokenId);
        ownerToEggIds[receiver] = collectedIds;
