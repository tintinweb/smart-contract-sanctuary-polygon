...
contract Degen is Ownable {
    ...
    IUni public QuickSwap = IUni(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff); //main network address for Quick swap
    address public USDT = address(0xc2132D05D31c914a87C6611C10748AEb04B58e8F); // USDT token on Polygon main net
    ...
    function _swap( 
       ...
       if(dexId == 0){
            ...
            if(orderType == OrderType.EthForTokens) {
                path[0] = QuickSwap.WETH();
                swapResult = QuickSwap.swapExactETHForTokens{value:assetInOffered}(minExpectedAmount, path, to, block.timestamp);
            }
            else if (orderType == OrderType.TokensForEth) {
                path[path.length-1] = QuickSwap.WETH();
                TransferHelper.safeApprove(path[0], address(QuickSwap), assetInOffered);
                swapResult = QuickSwap.swapExactTokensForETH(assetInOffered, minExpectedAmount, path, to, block.timestamp);
            }
            else if (orderType == OrderType.TokensForTokens) {
                TransferHelper.safeApprove(path[0], address(QuickSwap), assetInOffered);
                swapResult = QuickSwap.swapExactTokensForTokens(assetInOffered, minExpectedAmount, path, to, block.timestamp);
            }   
