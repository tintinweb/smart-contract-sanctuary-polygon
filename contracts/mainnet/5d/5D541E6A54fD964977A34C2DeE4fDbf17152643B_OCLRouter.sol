...
library FinderLib {
    ...
    function getMaintainerRole() internal pure returns (bytes32) {
        return ZeroXInterfaces.MAINTAINER_ROLE;
    }

    function getDefaultAdminRole() internal pure returns (bytes32) {
        return ZeroXInterfaces.DEFAULT_ADMIN_ROLE;
    }
}
...
/**
 * @title Enables to swap on other DEXs
 */
    ...
    SelfPermitUpgradeable
{
    ...
    function sellProperty(
        ...
        require(usdc == USDC, "Invalid currency");
        require(tryB == TRYB, "Invalid currency");

        console.log("Before safetransfer from in mp sc");

        IERC20Upgradeable(propertyToken).safeTransferFrom(
            _msgSender(),
            address(this),
            numberOfProperties
        );
        ...
        IERC20Upgradeable(propertyToken).safeIncreaseAllowance(
            marketplace,
            numberOfProperties
        );
        ...
        swapArgs.recipient = address(this);

        uint tokensReceived = IMarketplace(marketplace).swap(swapArgs, false);

        console.log("Swap done");
        IERC20Upgradeable(tryB).safeIncreaseAllowance(
            address(DFX_ROUTER),
            tokensReceived
        );

        console.log("allowance after swap done", tokensReceived);
        console.log(
            IERC20Metadata(usdc).balanceOf(address(this)),
            "usdc before"
        );
        uint usdcAmount = DFX_ROUTER.originSwap(
            usdc,
            tryB,
            usdc,
            tokensReceived,
        ...
        console.log("orogin swap  done", usdcAmount);

        IERC20Upgradeable(usdc).safeTransfer(reciepient, usdcAmount);
    }
    ...
    function swap(
        ...
        IERC20Upgradeable(_origin).safeTransferFrom(
            _msgSender(),
            address(this),
            _originAmount
        );
        IERC20Upgradeable(_origin).safeIncreaseAllowance(
            address(DFX_ROUTER),
            _originAmount
        );
        swappedAmount = DFX_ROUTER.originSwap(
            _quoteCurrency,
            _origin,
            _target,
            _originAmount,
        ...
        IERC20Upgradeable(_target).safeTransfer(receipient, swappedAmount);
    }

    function swapWithMetaTx(
        IMetaTx.MetaTxArgsBuy memory _metaTxArgs,
        address _quoteCurrency,
        address _origin,
        address _target,
        uint256 _originAmount,
        ...
        IMetaTx(_origin).executeMetaTransaction(
    ...
    function approveAndSellProperty(
        IMetaTx.MetaTxArgsSell memory _metaTxArgsSell,
        ...
        // now buying property
        IMarketplace(_marketplace).swap(_swapArgs, false);
    }

    function approveAndBuyPropertyWithMetaTx(
        IMetaTx.MetaTxArgsBuy memory _metaTxArgsBuy,
        IMarketplace.swapArgs memory _swapArgs,
        address _marketplace
    ) external {
        ...
        uint tokensToTransferFrom = getPropertyPriceWithFees(
            _marketplace,
            _swapArgs.from,
            _swapArgs.to,
            true,
    ...
    function approveAndBuyPropertyWithPermit(
        IMetaTx.MetaTxArgsSell memory _metaTxArgs,
        IMarketplace.swapArgs memory _swapArgs,
        address _marketplace
    ) external {
        IMetaTx(_swapArgs.from).permit(
    ...
    function getPropertyPriceWithFees(
        ...
        bool isBuyProperty,
        uint amountOfTokens
    ) public view returns (uint priceWithFees) {
        IPriceFeed.Property memory property = IMarketplace(_marketplace)
            .getPropertyPrice(from, to);

        uint feePercentage;
        uint propertyPrice = amountOfTokens * property.price;

        if (isBuyProperty) {
            feePercentage = IMarketplace(_marketplace).getBuyFeePercentage();
            priceWithFees =
                propertyPrice +
                ((propertyPrice * feePercentage) / PERCENTAGE_BASED_POINT);
        } else {
            feePercentage = IMarketplace(_marketplace).getSellFeePercentage();
            priceWithFees =
                propertyPrice +
                ((propertyPrice * feePercentage) / PERCENTAGE_BASED_POINT);
        }
    ...
    function updateRouterAddress(IDFXRouter _newRouter) external onlyAdmin {
        DFX_ROUTER = _newRouter;
    }
    ...
    function updateUSDC(address _newAddr) external onlyAdmin {
        USDC = _newAddr;
    }

    function updateTRYB(address _newAddr) external onlyAdmin {
        TRYB = _newAddr;
    }
