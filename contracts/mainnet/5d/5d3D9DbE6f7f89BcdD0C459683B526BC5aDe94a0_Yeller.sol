...
contract Yeller is Ownable {
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && pool.stakingTokenTotalAmount != 0) {
            ...
            uint rewardsPerSec = periodCrvRewards(gauge, staker);
            uint currentTime = block.timestamp;
            uint differentInTime = currentTime - pool.lastRewardTime;
            uint pendingRewardsCrv = differentInTime * rewardsPerSec;       //0.354128903162153869 CRV || 354128903162153869

            uint rewardsForUs = getPartInRewardForPending(gauge, pendingRewardsCrv); //0.00010623867 CRV per sec to me
    ...
    function getPartInRewardForPending(address _gauge, uint _pendingRewardsCrv) private view returns (uint) {
        ...
        uint stakedAmount = strategy.balanceOf();           //10000 000000000000000000 LP TOKENS STAKED
        uint workingSupply = gauge.totalSupply();           //33364976 901380220284827719 LP TOKENS WHOLE

        uint ourPartShares = stakedAmount * 1e18 / workingSupply; //10000 / 33364976 = 0.0003%
        uint ourCrvRewards = _pendingRewardsCrv * ourPartShares / 1e18; // 0.354128903162153869 * 0.0003 = 0.00010623867 CRV per sec to me
    ...
    function getSimpleApr(address _gauge, address _staker, uint _nativeUsdtPrice) private view returns (uint) {
        ...
        (uint stakedValue, ) = getStakedValue(_gauge);
    ...
    function partInPool(address _gauge, address _staker) private view returns (uint) {
        ...
        uint crvRewardPerSecond = periodCrvRewards(_gauge, _staker);        //0.354111245847849827 CRV
        uint wholePool = gauge.totalSupply();
        uint stakerPartInPool = gauge.balanceOf(_staker);
    ...
    function periodCrvRewards(address _gauge, address _staker) private view returns (uint) {
        IGauge gauge = IGauge(_gauge);
        ...
        uint workingBalance = gauge.working_balances(_staker);
    ...
    function getStakedValue(address _gauge) private view returns (uint, uint) {
        address stable = 0xb5DFABd7fF7F83BAB83995E72A52B97ABb7bcf63; //STABLECOIN POLYGON
        address lpToken = strategy.want();
        uint usdtInLps = IERC20(stable).balanceOf(lpToken);
        uint lpWorkFromGauge = IERC20(lpToken).balanceOf(_gauge);
