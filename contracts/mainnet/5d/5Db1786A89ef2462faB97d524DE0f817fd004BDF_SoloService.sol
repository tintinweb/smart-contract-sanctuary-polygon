...
contract SoloService is Base, ITournamentService {
  // variables
  ...
  function cancelTournament(
    ...
    // abi.encode(betNFT, [challengerId, challengerStake, round, opponentId, opponentStake, startAt, expireTime], [fixedFee, cancelFee])
    (bool betNFT, uint256[] memory battleInfo, uint256[] memory fees) = abi.decode(_configs.data, (bool, uint256[], uint256[]));
    booking[battleInfo[5]] = false;
    ...
    for (uint256 i = 0; i < size; i++) {
      ...
      if (betNFT) {
        IZooKeeper(zooKeeper).transferERC721Out(IChampionUtils(championUtils).getTokenContract(_warriors[i].id), _warriors[i].account, _warriors[i].id);
      }
      IZooKeeper(zooKeeper).transferERC20Out(_configs.currency, _warriors[i].account, battleInfo[1] + fees[0] - fees[1]);
    }
  ...
  function joinTournament(
...

    {
      ...
      uint64 tournamentId;
      address joiner;
      (serviceId, tournamentId, joiner, championId, stance, nonce) = abi.decode(_params, (uint64, uint64, address, uint256, uint16, uint256));
      ...
      require(signer == joiner, "Signer mismatch");
    }

    {
      // abi.encode(betNFT, [challengerId, challengerStake, round, opponentId, opponentStake, startAt, expireTime], [fixedFee, cancelFee])
      (bool betNFT, uint256[] memory battleInfo, uint256[] memory fees) = abi.decode(_configs.data, (bool, uint256[], uint256[]));
      require(championId == battleInfo[0] || championId == battleInfo[3], "Not in battle");
      require(fees[0] >= fees[1], "Invalid fee");

      require(IChampionUtils(championUtils).isOriginalOwnerOf(signer, championId), "Require owner"); // require owner of token
      require(!IChampionUtils(championUtils).isOriginalOwnerOf(zooKeeper, battleInfo[3]), "Opponent already joined BN"); // require owner of token

      uint256 stake;
      if (championId == battleInfo[0]) {
        require(!booking[battleInfo[5]], "Already booked");
        stake = battleInfo[1];
        booking[battleInfo[5]] = true;
      } else {
        stake = battleInfo[4];
      }
      IZooKeeper(zooKeeper).transferERC20In(_configs.currency, signer, fees[0] + stake);

      require(battleInfo[6] > block.timestamp, "Invite expired");

      if (betNFT) {
        IZooKeeper(zooKeeper).transferERC721In(IChampionUtils(championUtils).getTokenContract(championId), signer, championId);
      }
    }

    {
      require(!ITournamentState(tournamentState).championNonce(_serviceId, championId, nonce), "Invalid nonce");
      _configs.size = 2;
      ...
      ITournamentState(tournamentState).setChampionNonce(_serviceId, championId, nonce);
    }
  }
  ...
  function completeTournament(
    ...
    // abi.encode(betNFT, [challengerId, challengerStake, round, opponentId, opponentStake, startAt, expireTime], [fixedFee, cancelFee])
    (bool betNFT, uint256[] memory battleInfo, ) = abi.decode(_configs.data, (bool, uint256[], uint256[]));

    require(!betNFT || _callbacks.length >= 2, "Invalid callbacks");

    uint256 prizePool = battleInfo[1] + battleInfo[4];
    ...
    for (uint256 i = 0; i < _callbacks.length; i++) {
      (bool success, ) = _callbacks[i].account.call(_callbacks[i].data);
