...
contract Swap is BaseContract {
  ...
  function swapMenForStToken(uint _amount) external {
    ...
    menToken.transfer(address(this), _amount);

    address[] memory path = new address[](2);
    path[0] = address(menToken);
    path[1] = address(stToken);
    uint currentStTokenBalance = stToken.balanceOf(address(this));
    uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(_amount, 0, path, address(this), block.timestamp + 10);
    uint stTokenAmount = stToken.balanceOf(address(this)) - currentStTokenBalance;
    stToken.transfer(msg.sender, stTokenAmount);
  ...
  function swapStTokenForMen(uint _amount) external {
    ...
    stToken.transfer(address(this), _amount);
    ...
    uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(_amount, 0, path, address(this), block.timestamp + 10);
    uint menAmount = menToken.balanceOf(address(this)) - currentMenBalance;
    menToken.transfer(msg.sender, menAmount);
