...
contract ERC721Mintable is ERC721, ERC721Enumerable, ERC721URIStorage, IERC2981, Ownable {
    ...
    modifier onlyMinter() virtual {
        require(_minters[_msgSender()] || owner() == _msgSender(), "ERC721Mintable: caller is not minter or owner");
        _;
    }
    ...
    function mint(address to, string memory tokenRelativeURI) public virtual onlyMinter {
        _mintInternal(to, tokenRelativeURI, bytes32(0));
    }


    function mintWithHash(address to, string memory tokenRelativeURI, bytes32 fileHash) public virtual onlyMinter {
        _mintInternal(to, tokenRelativeURI, fileHash);
    }


    function mintBatch(address to, string[] memory tokenRelativeURIs, bytes32[] memory fileHashes) public virtual onlyMinter {
        
        for(uint i = 0; i < tokenRelativeURIs.length; i++) {

            bytes32 fileHash = i < fileHashes.length? fileHashes[i]: bytes32(0);

            _mintInternal(to, tokenRelativeURIs[i], fileHash);
        }
    }


    function _mintInternal(address to, string memory tokenRelativeURI, bytes32 fileHash) internal virtual {
        require(mintingFrozen == false, 'ERC1155Mintable: No more minting allowed.');

        _mint(to, _tokenIdTracker.current());

        _setTokenURI(_tokenIdTracker.current(), tokenRelativeURI);
    ...
    function burn(uint256 tokenId) public virtual {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721Mintable: caller is not owner nor approved");
...
contract ERC721MintableTradeable is ERC721Mintable, Executable {
    ...
    constructor(
        ...
        address royaltyReceiver,
        address executor
    ) ERC721Mintable(name, symbol, baseTokenURI, contractURIValue, royaltyPercent, royaltyReceiver) Executable(executor) {}


    // executors, owner or minter can mint.
    modifier onlyMinter() virtual override {
        require(isExecutor(_msgSender()) || isMinter(_msgSender()) || owner() == _msgSender(), "ERC721MintableTradeable: caller is not executor/minter/owner");
        _;
    }
