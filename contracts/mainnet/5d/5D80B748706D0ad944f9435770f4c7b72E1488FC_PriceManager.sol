...
contract PriceManager is IPriceManager, AccessControl {
    using Address for address;

    string public assetPair; // BTCUSDT or ETHUSDT or BNBUSDT
    uint8 public roundIdChecks = 10;
    uint256 public roundIdTimestampDifference = 3600;
    ...
    function setRoundIdTimestampDifference(uint256 _difference) external onlyAdmin(msg.sender) {
    ...
    function getLatestRoundId(
        ...
        uint256 _expiration,
        uint80 _roundId
    ) internal view returns (uint80 latestRoundId) {
        address oracle = addressMap[_underlying][_strike];
        require(oracle != address(0), "Chainlink: No aggregator");
        uint256 timestamp = AggregatorV2V3Interface(oracle).getTimestamp(_roundId);
        require(timestamp + roundIdTimestampDifference >= _expiration && timestamp <= _expiration, "invalid round id");
        latestRoundId = _roundId;

        for (uint8 i = 1; i <= roundIdChecks; i++) {
            uint256 roundIdTimestamp = AggregatorV2V3Interface(oracle).getTimestamp(uint256(_roundId + i));
            if (roundIdTimestamp > _expiration) break;
            if (roundIdTimestamp > 0) latestRoundId = _roundId + i;
        }
        // revert if any valid round Id still exists before expiration
        uint256 nextRoundTimestamp = AggregatorV2V3Interface(oracle).getTimestamp(uint256(latestRoundId + 1));

        require(nextRoundTimestamp == 0 || nextRoundTimestamp > _expiration, "incorrect round id");
    }
    ...
    function getPriceByTimestamp(
        ...
        address oracle = addressMap[_underlying][_strike];
        require(oracle != address(0), "Chainlink: No aggregator");
        (, int256 answer, , , ) = AggregatorV2V3Interface(oracle).getRoundData(
            getLatestRoundId(_underlying, _strike, _expiration, uint80(_roundId))
        );
        price = uint256(answer);
        decimals = AggregatorV2V3Interface(oracle).decimals();
    }
    ...
    function getTimestampByRoundId(
        ...
        address oracle = addressMap[_underlying][_strike];
        require(oracle != address(0), "Chainlink: No aggregator");
        timestamp = AggregatorV2V3Interface(oracle).getTimestamp(_roundId);
    }
}
