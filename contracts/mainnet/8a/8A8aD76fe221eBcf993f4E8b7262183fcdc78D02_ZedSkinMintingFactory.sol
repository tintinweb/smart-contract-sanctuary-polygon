...
contract ZedSkinMintingFactory is 
    Initializable, 
    ...
    IMintableWithERC20
{
    ...
    uint256 public allowListSaleMax;

    /// @notice Maximum tokens per wallet for the public sale
    uint256 public maxTokens;

    /// @notice The number of skins minted from the alow list
    uint256 private _allowListMintCount;

    /// @dev The number of skins minted from the public sale
    uint256 private _publicSaleMintCount;

    /// @notice WC Skin name
    string private constant SKIN_NAME = "WC 2022";

    /// @dev Mapping from price to an ERC20 address
    ...
    function initialize(address skinContractAddress) initializer external {
        __EIP712MetaTransactionUpgradeable_init("ZED Skin Minting Factory", "1");
        __Ownable_init();

        _skin = ISkin(skinContractAddress);

        // Initialize defaults
        allowListSaleMax = 400;
        maxTokens = 1;
    }
    ...
    /**
     * @dev Sets the maximum for the allow list mint.
     */
    function setAllowListSaleMax(uint256 max) external onlyOwner {
        allowListSaleMax = max;
    }
    ...
    /**
     * @dev Sets the maximum number of tokens that can be minted in the public sale.
     */
    ...
    /**
     * @dev Gets the total number minted by this contract
     */
    function totalSupply() external view returns(uint256){
        return _allowListMintCount + _publicSaleMintCount;
    }
    ...
    */
    ...
    {   
        ...
        uint256 newCount = _allowListMintCount;
        require(newCount < allowListSaleMax, "Allow list sold out");
        
        uint256 addressMintCount = addressToAllowListMintCount[_msgSender()];
        require(addressMintCount < 1, "Allow list maximum reached");

        // Overflow unrealistic
        unchecked {
            // Set new values before minting
            newCount++;
            _allowListMintCount = newCount;
    ...
    */
        ...
        nonReentrant 
    {
        ...
        uint256 newCount = _publicSaleMintCount;
        require((newCount + _allowListMintCount) < MAX_SUPPLY, "Public sale sold out");

        uint256 mintCount = addressToPublicSaleMintCount[_msgSender()];
        require(mintCount < maxTokens, "Public sale maximum reached");

        // Overflow unrealistic
        unchecked {
            // Set new values before minting
            newCount++;
            _publicSaleMintCount = newCount;
    ...
    @param erc20Address The ERC20 contract address
    */
    function _mint(address erc20Address)
        internal
        virtual
    {
        ...
        _skin.mint(_msgSender(), SKIN_NAME);

        // Mint event - token ID unknown
        emit Mint(_msgSender(), 0, SKIN_NAME);
    }
