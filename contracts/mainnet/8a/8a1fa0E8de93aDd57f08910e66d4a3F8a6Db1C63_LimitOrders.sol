...
contract LimitOrders is EIP712("forbitspace Limit Order Protocol", "1.0.0") {
    ...
    function recoverSigner(Order memory ord, bytes memory _signature)
        ...
        returns (address)
    {
        bytes32 orderHash = getOrderHash(ord);
        (bytes32 r, bytes32 s, uint8 v) = splitSignature(_signature);
        return ECDSA.recover(orderHash, v, r, s);
    }
    function getOrderHash(Order memory ord)
        public
        view
        returns (bytes32)
    {
    ...
    function shouldFulfilOrder(Order memory ord, bytes memory _signature) public view returns (bool) {
        ...
        if(IERC20(ord.tokenIn).allowance(ord.owner,address(spaceRouter)) >= ord.amountIn) _allowance = true;
        ...
        return (_isEnoughToken && _isNotCancelledOrder && _isPendingOrder && _isSigner && _isFulfiledOrder && _allowance);
    }

    function isNotCancelledOrder(Order memory ord) public view returns (bool) {
        bytes32 ordHash = getOrderHash(ord);
        bool _isNotCancelledOrder = true;
        for(uint i=0; i < msgHashOrderCancel[ord.owner].length; i++){
            if(ordHash == msgHashOrderCancel[ord.owner][i]) {
    ...
    function ExecuteCustomOrder(Order memory ord, bytes memory _signature) public reentrancyGuard returns (bool result, string memory message){
        ...
        if(execute(ord)) {
            msgHashOrderCancel[ord.owner].push(ordHash);
    ...
    function execute(Order memory ord) internal returns (bool filled) {
        ...
        try spaceRouter.makeTokenTokenSwap(ord.owner, ord.tokenIn, ord.tokenOut, pair , ord.amountIn, ord.minAmountOut, ord.deadline) { return true; } catch { return false; }
    }
