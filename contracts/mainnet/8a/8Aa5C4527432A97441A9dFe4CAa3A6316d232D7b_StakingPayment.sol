...
/**
 * @dev Use to made payment for staking service.
 * Has a function to approve user's Staking request.
 * @notice Is pausable to prevent malicious behavior.
 */
contract StakingPayment is Ownable, AccessControl, Pausable {
    ...
    receive() external payable {
        revert("[StakingPayment] Revert receive function.");
    }

    fallback() external payable {
        revert("[StakingPayment] Revert fallback function.");
    }

    /**
     * @dev Approve staking service purchasing made by user.
     * Transfer fee using balance vault.
     * @param _chainId - Chain id of staking service.
     * @param _tokenAddress - Token address for staking pool.
     * @param _startStakeTime - Date to start staking (lock staked token).
     * @param _endStakeTime - Date to end staking (unlock staked token).
     ...
     * @param _fees - Staking service fees.
     */
    function approveStaking(
        ...
        uint256 _fees
    ) external onlyRole(WORKER) whenNotPaused {
        require(
            _stakingOwner != address(0),
            "[StakingPayment.approveStaking] Invalid staking owner address"
        );
        if (_fees != 0) {
            balanceVault.payWithUpo(_stakingOwner, _fees);
            totalFees += _fees;
        }
        require(
            bytes(_chainId).length > 0,
            "[StakingPayment.approveStaking] Invalid chain id"
        );
        require(
            _tokenAddress != address(0),
            "[StakingPayment.approveStaking] Invalid token address"
        );
        require(
            _startStakeTime < _endStakeTime &&
                (_startStakeTime > block.timestamp && _endStakeTime > block.timestamp),
            "[StakingPayment.approveStaking] Invalid date range"
        );
        require(
            _userStakeLimit > 0,
            "[StakingPayment.approveStaking] Invalid user stake limit"
        );
        require(
            _poolStakeLimit > 0,
            "[StakingPayment.approveStaking] Invalid pool stake limit"
        );
        require(
            _percentage > 0,
            "[StakingPayment.approveStaking] Invalid reward percentage"
        );

        emit StakingApproved(
            _chainId,
            _tokenAddress,
            _startStakeTime,
            _endStakeTime,
     ...
     */
    function adminFeesWithdraw() public onlyRole(WORKER) whenNotPaused {
        uint256 withdrawAmount = totalFees;
        require(
            withdrawAmount > 0,
            "[StakingPayment.adminFeesWithdraw] Insufficient fees to withdraw"
        );
        totalFees = 0;
    ...
    /**
     * @dev Set staking payment in to pause state (only setter allowed).
     */
    function pauseStakingPayment() external onlyOwner {
        _pause();
    }

    /**
     * @dev Set staking payment in to normal state.
     */
    function unpauseStakingPayment() external onlyOwner {
        _unpause();
    }
}
