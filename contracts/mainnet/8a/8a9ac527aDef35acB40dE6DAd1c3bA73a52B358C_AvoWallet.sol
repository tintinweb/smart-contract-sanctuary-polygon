    ...
    IERC1271
{
    ...
    function _executeCast(
        ...
        // @dev starting point for measuring reserve gas should be here right after execution
    ...
    /// catches out of gas errors (as much as possible), reverting with AvoCore__OutOfGas
    ...
    function _executeActions(Action[] memory actions_, uint256 id_, bool isFlashloanCallback_) internal {
        ...
        if (id_ == 1 || id_ == 21) {
            ...
            // this ensures the Avo does not end up in a corrupted state.
            ...
            if (IS_MULTISIG) {
                assembly {
                    storageSlot4Snapshot_ := sload(0x4) // storage slot 4 related variables such as signers for Multisig
                }
            }
        }
        ...
        if (storageSlot0Snapshot_ > 0) {
            ...
            uint256 storageSlot4_;
            if (IS_MULTISIG) {
                assembly {
                    storageSlot4_ := sload(0x4) // storage slot 4 related variables such as signers for Multisig
                }
            }
            ...
            if (
                !(storageSlot0_ == storageSlot0Snapshot_ &&
                    ...
                    storageSlot4_ == storageSlot4Snapshot_)
            ) {
...
abstract contract AvoCoreEIP1271 is AvoCore {
    /// @inheritdoc IERC1271
    /// @dev reverts with AvoCore__InvalidEIP1271Signature if signature is invalid
    ...
    /// Can only be self-called (meaning only onwer or authorized addresses can execute this)
...
/// @title      AvoSelfUpgradeable
/// @notice     Simple contract to upgrade the implementation address stored at storage slot 0x0
/// @dev        Mostly based on OpenZeppelin ERC1967Upgrade contract, adapted with onlySelf etc.
///             Note For any new implementation, the upgrade method MUST be in the implementation itself,
...
abstract contract AvoCoreSelfUpgradeable is AvoCore {
    ...
    ///                             can only be called by contract itself (in case of Avo through `cast`)
...
abstract contract AvoCoreProtected is AvoCore {
    ...
    /// @notice occupies the sequential `avoSafeNonces_` in storage (maximum 5). This can be used to
    /// cancel / invalidate a signed request because the nonce will be "used" up
    ...
    ///        submit [77, 78, 79].
    ///        Submitting input params with a gap from the current avoSafeNonce will revert (e.g. [79, 80])
    ...
    /// @notice occupies the `nonSequentialNonces_` in storage. This can be used to
    /// cancel / invalidate a signed request because the nonce will be "used" up
...
abstract contract AvoCoreEvents {
    ...
    /// @notice Emitted when a message is marked as allowed
    ...
    /// `occupyAvoSafeNonces()`, which can be used to cancel a request
    ...
    /// `useNonSequentialNonces()`, which can be used to cancel a request
...
abstract contract AvoCoreConstantsOverride {
    // @dev: MUST SET DOMAIN_SEPARATOR_NAME & DOMAIN_SEPARATOR_VERSION IN CONTRACTS USING AvoCore: constants for EIP712 values
    ...
    /// @dev amount of gas to keep in castAuthorized caller method as reserve for emitting event + collecting fee
    uint256 internal immutable CAST_AUTHORIZED_RESERVE_GAS;
    /// @dev amount of gas to keep in cast / castAuthorized caller method as reserve for emitting event
...
abstract contract AvoCoreConstants is AvoCoreErrors {
    ...
    /// @dev overwrite chain id for EIP712 is always set to 63400
    ...
    /// @dev EIP712 typehash for cast calls including structs
    bytes32 public constant CAST_TYPE_HASH =
        keccak256(
            "Cast(CastParams params,CastForwardParams forwardParams)Action(address target,bytes data,uint256 value,uint256 operation)CastForwardParams(uint256 gas,uint256 validUntil,uint256 validAfter,uint256 gasPrice)CastParams(Action[] actions,uint256 id,int256 avoSafeNonce,bytes32 salt,address source,bytes metadata)"
        );
    ...
    /// @dev EIP712 typehash for CastForwardParams struct
    bytes32 public constant CAST_FORWARD_PARAMS_TYPE_HASH =
        keccak256("CastForwardParams(uint256 gas,uint256 validUntil,uint256 validAfter,uint256 gasPrice)");

    /// @dev "magic value" according to EIP1271 https://eips.ethereum.org/EIPS/eip-1271#specification
    bytes4 internal constant EIP1271_MAGIC_VALUE = 0x1626ba7e;

    /// @dev EIP712 typehash for castAuthorized calls including structs
    bytes32 public constant CAST_AUTHORIZED_TYPE_HASH =
        keccak256(
            "CastAuthorized(CastParams params,CastAuthorizedParams authorizedParams)Action(address target,bytes data,uint256 value,uint256 operation)CastAuthorizedParams(uint256 maxFee,uint256 validUntil,uint256 validAfter,uint256 gasPrice)CastParams(Action[] actions,uint256 id,int256 avoSafeNonce,bytes32 salt,address source,bytes metadata)"
        );

    /// @dev EIP712 typehash for CastAuthorizedParams struct
    bytes32 public constant CAST_AUTHORIZED_PARAMS_TYPE_HASH =
        keccak256("CastAuthorizedParams(uint256 maxFee,uint256 validUntil,uint256 validAfter,uint256 gasPrice)");

    /***********************************|
    |             IMMUTABLES            |
    |__________________________________*/

    /// @notice  registry holding the valid versions (addresses) for AvoMultisig implementation contracts
...
abstract contract AvoCoreVariablesSlot0 {
    /// @notice address of the Avo Multisig logic / implementation contract. IMPORTANT: SAME STORAGE SLOT AS FOR PROXY
...
abstract contract AvoCoreVariablesSlot1 {
    /// @notice owner of the smart Multisig
...
abstract contract AvoCoreVariablesSlot2 {
    ...
    /// but considers the slot number in calculating the hash of the key to store
...
/// @dev    IMPORTANT: all storage variables go into AvoWalletVariables
abstract contract AvoWalletCore is AvoWalletErrors, AvoWalletVariables, AvoCore, AvoWalletEvents, IAvoWalletV3Base {
    ...
    ) AvoWalletVariables(avoVersionsRegistry_, avoForwarder_, avoAuthoritiesList_) {
    ...
    function _verifySig(
    ...
    ) internal view returns (bool isValid_, address recoveredSigner_) {
        ...
        bool isContract_;
        if (signatureParams_.signer == address(0)) {
            // signer_ is optional and can be the zero address, simply recover signer from signature and use that
            ...
            if (Address.isContract(signatureParams_.signer)) {
                ...
                // if signer_ is not a contract but is defined, then it must match the recovered signer from signature
                recoveredSigner_ = ECDSA.recover(digest_, signatureParams_.signature);
        ...
        if (isContract_) {
            // no need to check for owner as owner can only be EOA
            ...
            if (authorities[recoveredSigner_] == 1) {
                ...
                // signer_ is a contract but not an allowed authority -> signature is invalid
...
abstract contract AvoWalletEIP1271 is AvoCoreEIP1271, AvoWalletCore {
    /// @inheritdoc IERC1271
    /// @dev reverts with AvoWallet__InvalidEIP1271Signature if signature is invalid
    function isValidSignature(
        ...
        if (signature.length == 0) {
            ...
            (bool validSignature_, ) = _verifySig(
                hash,
                SignatureParams({ signature: signature, signer: address(0) }),
                ...
                // for sequential nonces it adds unneeded verification and gas cost, because the check will always pass
                true
            );
            if (!validSignature_) {
                revert AvoCore__InvalidEIP1271Signature();
...
/// @title      AvoWalletSelfUpgradeable
/// @notice     Simple contract to upgrade the implementation address stored at storage slot 0x0
/// @dev        Mostly based on OpenZeppelin ERC1967Upgrade contract, adapted with onlySelf etc.
///             Note For any new implementation, the upgrade method MUST be in the implementation itself,
///             otherwise it can not be upgraded anymore!
abstract contract AvoWalletSelfUpgradeable is AvoCoreSelfUpgradeable {
    ...
    function upgradeTo(address avoWalletImpl_) public override onlySelf {
        avoVersionsRegistry.requireValidAvoWalletVersion(avoWalletImpl_);

        _avoImplementation = avoWalletImpl_;
...
abstract contract AvoWalletCast is AvoWalletCore {
    ...
    function nonSequentialNonce(
        ...
        return getSigDigest(params_, forwardParams_);
    }
    ...
    function verify(
        ...
        // _verifySig returns actual signer, also if input param `signer_` is not set
        (bool validSignature_, ) = _verifySig(
    ...
    function cast(
    ...
    ) external payable returns (bool success_, string memory revertReason_) {
        {
            ...
            // if sender is forwarder, compare actual sent gas to user instructed gas
...
abstract contract AvoWalletCastAuthorized is AvoWalletCore {
    ...
    function castAuthorized(
        ...
        // @dev part below costs ~24k gas for if feeAmount_ and maxFee is set
        uint256 feeAmount_;
        address payable feeCollector_;
        {
            // this code below costs about 25_900 gas for a percentage fee mode where fee is > 0
            uint256 gasUsed_;
            unchecked {
                // gas can not underflow
                // gasUsed already includes everything at this point except for paying fee logic
                gasUsed_ = gasSnapshot_ - gasleft();
            }
            (feeAmount_, feeCollector_) = avoVersionsRegistry.calcFee(gasUsed_);
        }

        // pay fee, if any
        if (feeAmount_ > 0) {
            if (authorizedParams_.maxFee > 0 && feeAmount_ > authorizedParams_.maxFee) {
                revert AvoWallet__MaxFee(feeAmount_, authorizedParams_.maxFee);
            }

            Address.sendValue(feeCollector_, feeAmount_);
        }
...
///
///         The `castAuthorized` method allows the owner of the wallet to execute multiple arbitrary actions
///         The `cast` method allows the AvoForwarder (relayer) to execute multiple arbitrary actions authorized by signature
///         Relayers are expected to call the AvoForwarder contract `execute`, which deploys an AvoWallet if necessary first.
...
///         This contract validates all signatures for defaultChainId of 63400 instead of current block.chainid from opcode (EIP-1344)
...
///         otherwise it can not be upgraded anymore! also see AvoWalletSelfUpgradeable
    ...
    AvoWalletCastAuthorized
{
    ...
    /// @param avoAuthoritiesList_     address of the AvoAuthoritiesList contract
    constructor(
        IAvoVersionsRegistry avoVersionsRegistry_,
        address avoForwarder_,
        IAvoAuthoritiesList avoAuthoritiesList_
    ) AvoWalletCore(avoVersionsRegistry_, avoForwarder_, avoAuthoritiesList_) {}
    ...
    function reinitialize() public reinitializer(2) {
        // clean up storage slot 2 and 3, which included EIP712Upgradeable hashes in earlier versions. See VariablesV1.sol
        assembly {
            // load content from storage slot 1, except for last 80 bit. Loading: 176 bit (42 * 8)
...
abstract contract AvoWalletErrors {
    ...
    error AvoWallet__MaxFee(uint256 fee, uint256 maxFee);
}
...
abstract contract AvoWalletEvents {
    ...
    /// caller = owner / AvoForwarder address. signer = address that triggered this Cast (authority or owner)
...
import { IAvoAuthoritiesList } from "../interfaces/IAvoAuthoritiesList.sol";
import { AvoWalletErrors } from "./AvoWalletErrors.sol";
import { AvoCoreConstants, AvoCoreConstantsOverride, AvoCoreVariablesSlot0, AvoCoreVariablesSlot1, AvoCoreVariablesSlot2, AvoCoreVariablesSlot3 } from "../AvoCore/AvoCoreVariables.sol";

abstract contract AvoWalletConstants is AvoCoreConstantsOverride, AvoCoreConstants, AvoWalletErrors {
    ...
    constructor(
        ...
        AvoCoreConstantsOverride(DOMAIN_SEPARATOR_NAME, DOMAIN_SEPARATOR_VERSION, 39_000, 12_000, false)
    {
...
abstract contract AvoWalletVariables is
    ...
    AvoCoreVariablesSlot3
{
    // ----------- storage slot 0 to 3 through inheritance, see respective contracts -----------
    ...
    /// @notice mapping for allowed authorities on AvoWallet. Can trigger actions through signature & forwarder
    ...
    /// @notice sets some immutable Avocado contract addresses
    /// @param avoVersionsRegistry_    address of the avoVersionsRegistry contract
    /// @param avoForwarder_           address of the avoForwarder (proxy) contract to forward tx with valid signatures
    ///                                must be a valid version in AvoVersionsRegistry.
    /// @param avoAuthoritiesList_     address of the AvoAuthoritiesList contract
    constructor(
        IAvoVersionsRegistry avoVersionsRegistry_,
        address avoForwarder_,
        IAvoAuthoritiesList avoAuthoritiesList_
    ) AvoWalletConstants(avoVersionsRegistry_, avoForwarder_, avoAuthoritiesList_) {}
}
...
interface IAvoAuthoritiesList {
    /// @notice syncs mappings of `authorities_` to an AvoSafe `avoSafe_` based on the data present at the AvoWallet
