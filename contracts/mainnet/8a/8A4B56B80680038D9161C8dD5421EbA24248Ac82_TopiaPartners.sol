...
contract TopiaPartners is Ownable {
    ...
    function changeWeight(uint256 _weight) external onlyOwner{
        userData[msg.sender].weight = _weight;
        trancheData[1].weight = _weight;
    }

    function changeTresary(address _treasary) external onlyOwner {
        treasary = _treasary;
    }
    ...
    function earlyPartnerClaim() external returns (uint256) {
        ...
        require(address(VeTopia) != address(0) && address(TOPIA) != address(0), "Addresses not set");
        UserWeight storage u = userData[msg.sender];
