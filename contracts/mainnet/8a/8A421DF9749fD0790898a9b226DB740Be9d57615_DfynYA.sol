...
import "./interfaces/dfyn/IDfynChest.sol";
...
contract DfynYA is BaseYA, UniswapV2Giddy, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  uint256 constant private COMPOUND_THRESHOLD = 1e16;
  address constant private DFYN_ROUTER = 0xA102072A4C07F06EC3B4900FDC4C7B80b6c57429;
  address constant private DFYN_TOKEN = 0xC168E40227E4ebD8C1caE80F7a55a4F0e6D66C97;
  address constant private VDFYN_TOKEN = 0x75455c3DE45dD32CBE9a5aD5E518D3D50823c976;
  address constant private DFYN_CHEST = 0x75455c3DE45dD32CBE9a5aD5E518D3D50823c976;
  ...
  function getContractBalance() public view override returns (uint256) {
    return IDfynChest(DFYN_CHEST).vdfynForDfyn(IERC20(VDFYN_TOKEN).balanceOf(address(this)));
  }
  ...
  function depositCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = calcPriceSimple(DFYN_ROUTER, USDC_TOKEN, DFYN_TOKEN, value - fap);
  }
  ...
  function deposit(bytes calldata auth, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    uint256 valueDfyn = swapTokensSimple(DFYN_ROUTER, USDC_TOKEN, DFYN_TOKEN, address(this), value);

    uint256 shares = contractShares == 0 ? valueDfyn * INIT_SHARES : valueDfyn * contractShares / getContractBalance();
    userShares[owner] += shares;
    contractShares += shares;

    if (!IERC20(DFYN_TOKEN).approve(DFYN_CHEST, valueDfyn)) {
      revert();
    }
    IDfynChest(DFYN_CHEST).enter(valueDfyn);
  ...
  function depositGiddyCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    (outBefore, amountOut) = calcSwap(DFYN_ROUTER, USDC_TOKEN, DFYN_TOKEN, outBefore, amountOut);
  ...
  function depositGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    uint256 valueDfyn = swapTokensSimple(DFYN_ROUTER, USDC_TOKEN, DFYN_TOKEN, address(this), usdcValue);
  ...
  function withdrawCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = calcPriceSimple(DFYN_ROUTER, DFYN_TOKEN, USDC_TOKEN, sharesToValue(shares));
    amountOut -= fap;
  }
  ...
  function withdraw(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    uint256 valueVDfyn = IERC20(VDFYN_TOKEN).balanceOf(address(this)) * shares / contractShares;
    userShares[_msgSender()] -= shares;
    contractShares -= shares;

    uint256 valueDfyn = IDfynChest(DFYN_CHEST).vdfynForDfyn(valueVDfyn);
    IDfynChest(DFYN_CHEST).leave(valueVDfyn);
    uint256 valueUsdc = swapTokensSimple(DFYN_ROUTER, DFYN_TOKEN, USDC_TOKEN, address(this), valueDfyn);
  ...
  function withdrawGiddyCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    amountOut = outBefore;
    (outBefore, amountOut) = calcSwap(DFYN_ROUTER, DFYN_TOKEN, USDC_TOKEN, outBefore, amountOut);
  ...
  function withdrawGiddy(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    ...
    uint256 value = IDfynChest(DFYN_CHEST).vdfynForDfyn(valueVDfyn);
    IDfynChest(DFYN_CHEST).leave(valueVDfyn);

    value = swapTokensSimple(DFYN_ROUTER, DFYN_TOKEN, USDC_TOKEN, address(this), value);
  ...
  modifier deductEarningsFee() {
    ...
    if (earnings > 0) {
      uint256 feeDfyn = earnings * GIDDY_CONFIG.earningsFee() / BASE_PERCENT;
      if (feeDfyn > 0) {
        IDfynChest(DFYN_CHEST).leave(IDfynChest(DFYN_CHEST).dfynForVdfyn(feeDfyn));
      }
      if (IERC20(DFYN_TOKEN).balanceOf(address(this)) >= COMPOUND_THRESHOLD) {
        swapTokensSimple(DFYN_ROUTER, DFYN_TOKEN, USDC_TOKEN, GIDDY_CONFIG.feeAccount(), IERC20(DFYN_TOKEN).balanceOf(address(this)));
      }
      emit Compound(earnings - feeDfyn, lastCompoundTime, lastContractBalance);
      lastCompoundTime = block.timestamp;
    }
    _;
