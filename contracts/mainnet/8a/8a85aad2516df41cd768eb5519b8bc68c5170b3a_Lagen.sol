...
contract Lagen is ERC721A, Ownable, ReentrancyGuard {
    string private _collectionURI;
    string public baseURI;

    uint256 public _tokenId = 1;
    uint256 public _totalSupply = 500;
    uint256 public constant PUBLIC_SALE_PRICE = 1.0 ether;
    uint256 private maxMintsPerPublicSale = 10;
    uint256 private maxGiftMintId = 1;
    string private revealUri;
    bool public publicSaleEnabled = false;
    bool public revealed = true;
    // keep track of those on giftMint who have claimed their NFT
    ...
    constructor(string memory _hiddenURI)
        ERC721A("Lagen", "LGN", maxMintsPerPublicSale, _totalSupply)
    {
        sethiddenBaseURI(_hiddenURI);
        setCollectionURI("Lagen");
    }
    ...
    modifier canMint(uint256 numberOfTokens) {
        require(
            _tokenId + numberOfTokens <= _totalSupply,
    ...
    function mintGift() public nonReentrant {
        require(
            _tokenId <= maxGiftMintId,
            "minted the maximum # of giftlist tokens"
        );
        require(
            !claimedGift[msg.sender],
            "NFT is already claimed by this wallet"
        );
        _safeMint(msg.sender, 1);
        claimedGift[msg.sender] = true;
        _tokenId++;
    }
        ...
        nonReentrant
    {
        ...
        require((_tokenId + numberOfTokens) <= (_totalSupply), "No more NFTs");
        require(
            maxMintsPerPublicSale >=
                publicSaleMinted[msg.sender] + numberOfTokens,
            "You have no publicMint left"
        );
        require(
            maxMintsPerPublicSale >= numberOfTokens,
            "publicMint: 10 max per tx"
        );
    ...
    function ownerMint(uint256 numberOfTokens, address _address)
        public
        onlyOwner
    {
        require((_tokenId + numberOfTokens) <= (_totalSupply), "No more NFTs");
    ...
    function getGiftMinted() public view returns (bool) {
        return claimedGift[msg.sender];
    }

    function getPSMinted() public view returns (uint256) {
        return publicSaleMinted[msg.sender];
    }
