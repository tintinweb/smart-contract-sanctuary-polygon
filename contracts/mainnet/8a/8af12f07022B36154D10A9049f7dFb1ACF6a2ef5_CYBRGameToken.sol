...
/**
 * @title CYBRGameToken - Utility token for CyberCity game.
 */
contract CYBRGameToken is ERC20, RBAC {
    bool public senderCheck;
    bool public recipientCheck;

    IWhitelist public immutable senders;
    IWhitelist public immutable recipients;

    event SenderCheckEnabled();
    event SenderCheckDisabled();
    event RecipientCheckEnabled();
    event RecipientCheckDisabled();

    constructor(address _admin) ERC20("CYBR Game", "CYBRg") RBAC(_admin) {
        senderCheck = true;
        recipientCheck = true;

        senders = IWhitelist(address(new Whitelist(_admin)));
        recipients = IWhitelist(address(new Whitelist(_admin)));
    }
    ...
    function toggleSenderCheck(bool _state)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_state != senderCheck, "WRONG_STATE");

        senderCheck = _state;

        if (_state) {
            emit SenderCheckEnabled();
        } else {
            emit SenderCheckDisabled();
        }
    }

    function toggleRecipientCheck(bool _state)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_state != recipientCheck, "WRONG_STATE");

        recipientCheck = _state;

        if (_state) {
            emit RecipientCheckEnabled();
        } else {
            emit RecipientCheckDisabled();
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (senderCheck) {
            if (senders.isMember(from)) {
                return;
            }
        }

        if (recipientCheck) {
            require(recipients.isMember(to), "Cannot accept token");
        }
    }
}
