...
contract GBT is ERC20, ReentrancyGuard, Ownable, Initializable {
    ...
    uint256 public feeRate; // 10000 = 100%, 100 = 1%
    address payable public feeTo;

    // Events
    event Buy(address indexed user, uint256 gbtAmount, uint256 baseAmount, uint256 price);
    event Sell(address indexed user, uint256 gbtAmount, uint256 baseAmount, uint256 price);
    ...
    function setReserveVirtualBASE(uint256 reserveVirtualBASE_) external onlyOwner {
        reserveVirtualBASE = reserveVirtualBASE_;
    }
    ...
    function baseBal() external view returns (uint256) {
        if(BASE_TOKEN == address(0)) address(this).balance;
    ...
    */
    function buy(uint256 _amountBASE, uint256 _minGBT, uint256 expireTimestamp/*, address affiliate*/) 
    external payable nonReentrant {
        ...
        uint256 newReserveBASE = oldReserveBASE + _amountBASE;
        ...
        if(BASE_TOKEN == address(0)) {
            require(msg.value >= _amountBASE, "amount error");
            if(msg.value > _amountBASE)
                account.transfer(msg.value - _amountBASE);
        }
        else {
        ...
        emit Buy(account, outGBT, _amountBASE,  outGBT / _amountBASE); 
    }
    ...
    */
    function sell(uint256 _amountGBT, uint256 _minETH, uint256 expireTimestamp) external nonReentrant {
        ...
        uint256 feeAmountGBT = _amountGBT * feeRate / 10000;
        ...
        IERC20(address(this)).safeTransferFrom(account, feeTo, feeAmountGBT);
        
        emit Sell(account, _amountGBT, outBASE, (_amountGBT - feeAmountGBT)  / outBASE); 
        if(BASE_TOKEN == address(0)){
            (bool success, ) = msg.sender.call{value: outBASE}("");
    ...
    function calculateAmountBase(uint256 outGBT) public view returns (uint256 _amountBASE) {
        while(true) {
            uint256 oldReserveBASE = reserveVirtualBASE + reserveRealBASE;
            uint256 oldReserveGBT = reserveGBT;
            uint256 newReserveGBT = oldReserveGBT - outGBT;
            uint256 newReserveBASE = (oldReserveBASE * oldReserveGBT) / newReserveGBT;
            _amountBASE = newReserveBASE - oldReserveBASE;
            if(calculateOutGbt(_amountBASE) < outGBT)
                outGBT += 1;
            else
                break;
        }
    ...
    function calculateOutGbt(uint256 _amountBASE) public view returns (uint256 outGBT) {
        uint256 oldReserveBASE = reserveVirtualBASE + reserveRealBASE;
        uint256 newReserveBASE = oldReserveBASE + _amountBASE;
        uint256 oldReserveGBT = reserveGBT;
        uint256 newReserveGBT = oldReserveBASE * oldReserveGBT / newReserveBASE;
        outGBT = oldReserveGBT - newReserveGBT;
    }

    function retrieveToken(address payable to) public onlyOwner {
        if(BASE_TOKEN == address(0)) {
            to.call{value: (address(this)).balance}("");
        }
        else {
            IERC20(BASE_TOKEN).safeTransfer(to, IERC20(BASE_TOKEN).balanceOf(address(this)));
        }
    }
}
