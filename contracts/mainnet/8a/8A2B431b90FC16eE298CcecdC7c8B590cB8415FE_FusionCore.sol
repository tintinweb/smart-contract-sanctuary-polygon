...
contract FusionCore {
    ...
    modifier passedLiquidation(address _borrower) virtual {
        ...
        require((collatAssetPrice * collateralBalance[_borrower]) <= calculateLiquidationPoint(_borrower), "Position can't be liquidated!");
        _;
    }
    ...
    function getCollatAssetPrice() public view returns(uint collatAssetPrice){
        ...
        collatAssetPrice = uint(price) / 10**8;
    }
    ...
    function calculateBorrowLimit(address _borrower) public view returns(uint limit) {
        uint collatAssetPrice = getCollatAssetPrice();
        limit = ((((collatAssetPrice * collateralBalance[_borrower]) / 100) * 70)) - borrowBalance[_borrower];
    }
