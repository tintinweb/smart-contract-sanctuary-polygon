...
contract MultiHonor_V1 is Initializable, IMultiHonor, AccessControlUpgradeable {
    ...
    function balanceOf(address account, uint256 id) public view returns (uint256) {
        ...
        if (id == 0) {
            return uint256(this.TotalPoint(tokenId));
        }
        if (id == 1) {
            return uint256(this.POC(tokenId));
        }
        if (id == 2) {
            return uint256(this.VEPoint(tokenId));
        }
        if (id == 3) {
            return uint256(this.EventPoint(tokenId));
        }
        if (id == 4) {
            return uint256(this.Level(tokenId));
        }
        return 0;
    }
}