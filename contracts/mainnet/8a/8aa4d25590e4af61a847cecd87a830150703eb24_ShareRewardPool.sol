...
contract ShareRewardPool  is Initializable, OwnableUpgradeable  {
    ...
    uint256 public tSharePerSecond; // 60000 SHARE / (370 days * 24h * 60min * 60s)
    ...
    function changeStartRunningTime(uint256 _poolStartTime, uint256 _runningTime) public onlyOwner disable { 
            poolStartTime = _poolStartTime; 
            runningTime = _runningTime;
            poolEndTime = poolStartTime + runningTime;
        }

    function changePegTokenPerSecond(uint256 _tSharePerSecond) public onlyOwner disable { 
            tSharePerSecond = _tSharePerSecond;
        }

        function changeTOTAL_REWARDS(uint256 _TOTAL_REWARDS) public onlyOwner disable { 
    ...
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            accSharePerShare = accSharePerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accSharePerShare = pool.accSharePerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
