...
contract AirChrysalis {
    ...
    event giveawayFinished(address indexed account, uint256 giveawayId, address indexed winner, uint256[2][] tokenIds, address contractAddr);
    ...
    function createGiveaway(address _contractAddr, uint256[2][] memory _tokensTuple) public {
        for (uint256 idx = 0; idx < _tokensTuple.length; idx++) {
            ...
            require(token.balanceOf(msg.sender, _tokensTuple[idx][0]) >= _tokensTuple[idx][1], "caller must have this amount of token");
        }
        accounts.push(msg.sender);
        addressGiveaways[msg.sender].push(giveawayId);
        address[] memory participants;
        giveawaysMap[msg.sender][giveawayId] = Giveaway(_contractAddr, _tokensTuple, participants, false);
    ...
    function finishGiveaway(address account, uint256 _giveawayId) public {
        ...
        if (_giveaway.participants.length > 0) {
            ...
            emit giveawayFinished(account, _giveawayId, winner, _giveaway.tokensTuple, _giveaway.contractAddr);
        } else {
    ...
    function resetGiveaways() public {
        for (uint256 idx = 0; idx < accounts.length; idx++) {
            ...
            for (uint256 j = 0; j< ids.length; j++) {
                uint256[2][] memory _tokenTuples;
                address[] memory _participants;
                giveawaysMap[accounts[idx]][ids[j]] = Giveaway(address(0), _tokenTuples, _participants, false);
            }
