...
contract CurveExchange is Pausable, ReentrancyGuard, ICurveExchange {
    ...
    address public factory_registry;
    address public crypto_registry;
    address public crypto_factory;
    mapping(address => address[2]) public base_coins;
    mapping(address => mapping(address => bool)) public is_approved;

    constructor(address _address_provider) {
        address_provider = AddressProvider(_address_provider);
        registry = AddressProvider(_address_provider).get_registry();
        factory_registry = AddressProvider(_address_provider).get_address(3);
        crypto_registry = AddressProvider(_address_provider).get_address(5);
        crypto_registry = AddressProvider(_address_provider).get_address(6);
    }

    function _get_exchange_amount(
        address _registry,
        address _pool,
        address _from,
        address _to,
    ...
    */
        ...
        (i, j, is_underlying) = Registry(_registry).get_coin_indices(
            _pool,
            _from,
            _to
        );
        ...
        if (
            is_underlying &&
            (_registry == registry || Registry(_registry).is_meta(_pool))
        ) {
    ...
    function _get_crypto_exchange_amount(
        address _registry,
        address _pool,
        address _from,
        address _to,
    ...
    @return Quantity of `_to` to be received*/
        ...
        uint256 i = 0;
        uint256 j = 0;
        (i, j) = CryptoRegistry(_registry).get_coin_indices(_pool, _from, _to);

        return CryptoPool(_pool).get_dy(i, j, _amount);
    }

    function _exchange(ExchangeArgs memory exchangeArgs)
        internal
        whenNotPaused
        returns (uint256)
    {
        ...
        int128 j = 0;
        bool is_underlying = false;
        (i, j, is_underlying) = Registry(exchangeArgs._registry)
            .get_coin_indices(
                exchangeArgs._pool,
                exchangeArgs._from,
                exchangeArgs._to
            );
        if (is_underlying && exchangeArgs._registry == factory_registry) {
            if (Registry(exchangeArgs._registry).is_meta(exchangeArgs._pool)) {
                address[2] memory base_coin = base_coins[exchangeArgs._pool];
                if (base_coin.length == 0) {
                    base_coin = [
                        CurvePool(exchangeArgs._pool).coins(0),
                        CurvePool(exchangeArgs._pool).coins(1)
                    ];
                    base_coins[exchangeArgs._pool] = base_coin;
                }
            }
            //we only need to use exchange underlying if the input or output is not in the base coins
            // is_underlying = base_coins[_from].length>0 || base_coins[_to].length>0;
            else {
        ...
        if (exchangeArgs._from == ETH_ADDRESS) {
            eth_amount = exchangeArgs._amount;
        } else {
            (bool success, ) = exchangeArgs._from.call{
                value: msg.value,
                gas: 5000
            }(
                abi.encodeWithSignature(
                    "transferFrom(address,address,uint256)",
                    exchangeArgs._sender,
                    exchangeArgs._from,
                    exchangeArgs._amount
                )
            );
        ...
        // approve input token
        if (
            exchangeArgs._from != ETH_ADDRESS &&
            !is_approved[exchangeArgs._from][exchangeArgs._pool]
        ) {
            (bool success, ) = exchangeArgs._from.call(
                abi.encodeWithSignature(
            ...
            is_approved[exchangeArgs._from][exchangeArgs._pool] = true;
        }

        //perform coin exchange
        if (is_underlying) {
            CurvePool(exchangeArgs._pool).exchange_underlying{
                value: eth_amount
            }(i, j, exchangeArgs._amount, exchangeArgs._expected);
        } else {
            CurvePool(exchangeArgs._pool).exchange{value: eth_amount}(
                i,
                j,
                exchangeArgs._amount,
                exchangeArgs._expected
            );
        }

        // perform output transfer
        if (exchangeArgs._to == ETH_ADDRESS) {
            received_amount = exchangeArgs._to.getBalance();
            exchangeArgs._receiver.call{value: received_amount}(" ");
        } else {
            received_amount = exchangeArgs._to.getBalance();
            (bool success, ) = exchangeArgs._to.call(
                abi.encodeWithSignature(
                    "transfer(address,uint256)",
                    exchangeArgs._receiver,
                    received_amount
                )
            );
    ...
    function _crypto_exchange(CryptoExchangeArgs memory cryptoExchangeArgs)
        internal
        whenNotPaused
        returns (uint256)
    {
        address initial = cryptoExchangeArgs.from;
        address target = cryptoExchangeArgs.to;

        if (cryptoExchangeArgs.from == ETH_ADDRESS) {
            initial = WETH_ADDRESS;
        }

        if (cryptoExchangeArgs.to == ETH_ADDRESS) {
            target = WETH_ADDRESS;
        }
        ...
        uint256 i = 0;
        uint256 j = 0;
        (i, j) = CryptoRegistry(cryptoExchangeArgs.crypto_registry)
            .get_coin_indices(cryptoExchangeArgs.pool, initial, target);

        // perform / verify input transfer
        if (cryptoExchangeArgs.from == ETH_ADDRESS) {
            eth_amount = cryptoExchangeArgs.amount;
        } else {
            (bool success, ) = cryptoExchangeArgs.from.call(
                abi.encodeWithSignature(
                    "transferFrom(address,address,uint256)",
                    cryptoExchangeArgs.sender,
                    cryptoExchangeArgs.from,
                    cryptoExchangeArgs.amount
                )
            );
            require(success, "Error");
        }
        // approve input token
        if (!is_approved[cryptoExchangeArgs.from][cryptoExchangeArgs.pool]) {
            (bool success, ) = cryptoExchangeArgs.from.call(
                abi.encodeWithSignature(
                    "approve(address,uint256)",
                    cryptoExchangeArgs.pool,
                    type(uint256).max
                )
            );
            require(success, "Error");
            is_approved[cryptoExchangeArgs.from][
                cryptoExchangeArgs.pool
            ] = true;
        }
        ...
        // perform coin exchange
        if (
            ETH_ADDRESS == cryptoExchangeArgs.from ||
            ETH_ADDRESS == cryptoExchangeArgs.to
        ) {
            CryptoPoolETH(cryptoExchangeArgs.pool).exchange{value: eth_amount}(
                i,
                j,
                cryptoExchangeArgs.amount,
                cryptoExchangeArgs.expected,
                true
            );
        } else {
            CryptoPool(cryptoExchangeArgs.pool).exchange(
                i,
                j,
                cryptoExchangeArgs.amount,
                cryptoExchangeArgs.expected
        ...
        //perform output transfer
        if (cryptoExchangeArgs.to == ETH_ADDRESS) {
            received_amount = cryptoExchangeArgs.to.getBalance();
            cryptoExchangeArgs.receiver.call{value: received_amount}(" ");
        } else {
            received_amount = cryptoExchangeArgs.to.getBalance();
            (bool success, ) = cryptoExchangeArgs.to.call(
                abi.encodeWithSignature(
                    "transfer(address,uint256)",
                    cryptoExchangeArgs.receiver,
                    received_amount
                )
            );
            require(success, "Error");
    ...
    */
        ...
        if (Registry(crypto_registry).get_decimals(_pool)[0] > 0) {
            CryptoExchangeArgs memory cryptoExchangeArgs = CryptoExchangeArgs(
                _pool,
                _from,
                _to,
                _amount,
            ...
            return _crypto_exchange(cryptoExchangeArgs);
        }

        if (Registry(crypto_factory).get_decimals(_pool)[0] > 0) {
            CryptoExchangeArgs memory cryptoExchangeArgs = CryptoExchangeArgs(
                _pool,
                _from,
                _to,
        ...
        if (Registry(_registry).get_decimals(_pool)[0] == 0) {
            _registry = factory_registry;
        }
    ...
    @return Quantity of `_to` to be received
   */
        address _registry = crypto_registry;
        if (Registry(_registry).get_decimals(_pool)[0] > 0) {
            address initial = _from;
            address target = _to;
            if (_from == ETH_ADDRESS) {
                initial = WETH_ADDRESS;
            }
        ...
        return _get_exchange_amount(registry, _pool, _from, _to, _amount);
    }
}
