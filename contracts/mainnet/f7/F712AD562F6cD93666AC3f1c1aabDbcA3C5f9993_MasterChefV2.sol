...
contract MasterChefV2 is Ownable {
    ...
    event LogUpdatePool(uint indexed pid, uint lastRewardTime, uint lpSupply, uint accAxoPerShare);
    event LogInit();
    ...
    function pendingAXO(uint _pid, address _user) external view validatePid(_pid) returns (uint pending) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint booReward = totalAllocPoint == 0 ? 0 : ((multiplier * axoPerBlock() * pool.allocPoint) / totalAllocPoint);
            accAxoPerShare = accAxoPerShare + (booReward * ACC_AXO_PRECISION / lpSupply);
        }
    ...
    /// @notice Calculates and returns the `amount` of AXO per second allocated to this contract
    ...
    function _updatePool(uint pid) internal validatePid(pid) returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if (lpSupply > 0) {
                ...
                pool.accAxoPerShare = uint128(pool.accAxoPerShare + ((booReward * ACC_AXO_PRECISION) / lpSupply));
            }
            pool.lastRewardTime = uint64(block.timestamp);
            poolInfo[pid] = pool;
            emit LogUpdatePool(pid, pool.lastRewardTime, lpSupply, pool.accAxoPerShare);
        }
    }
