...
contract GammaLPFarmV1 is BaseSiloAction {
        ...
        for (; i < 2; ) {
            if (rewards[i] > 0) {
                ...
                if (rewardToken == dQUICK_ADDRESS) {
                    // uint256 pendingReward = IDragonLair(dQUICK_ADDRESS)
                    //     .dQUICKForQUICK(rewards[i]);

                    IDragonLair(dQUICK_ADDRESS).leave(rewards[i]);
                    rewards[i] = IERC20(QUICK_ADDRESS).balanceOf(address(this));
    ...
    function createConfig(
        ...
        configData = abi.encode(_inputs, _outputs, _trigger, _pid, rewarders);
    }
    ...
    function checkMaintain(
        ...
        (, , uint256 trigger, uint256 pid, address[2] memory rewarders) = abi
            .decode(
                configuration,
                (address[5], address[5], uint256, uint256, address[2])
            );
    ...
    function extraInfo(
        ...
        if (rewarders[0] != address(0)) {
            pendingReward = IRewarder(rewarders[0]).pendingToken(
                pid,
                msg.sender
            );
        }
