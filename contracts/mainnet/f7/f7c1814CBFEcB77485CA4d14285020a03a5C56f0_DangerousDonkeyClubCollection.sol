    ...
    OwnableUpgradeable
{
     ...
     */
    function initialize(address _admin,address _v1nftaddress) public initializerERC721A initializer {
        __ERC721A_init({
    ...
    function isactivatestaking(bool status) public returns(bool){
        require(admin == msg.sender,"is not admin call");
        isactivate = status;
        return true;
    }
    function changeadmin(address _admin) public returns(bool){
        require(owner() == msg.sender,"is not admin call");
        admin = _admin;
        return true;
    }
    function stakingv1(uint256[] memory _tokenid,bytes memory signature) public returns(bool){
        require(isactivate,"is end of the staking");
        require(_tokenid.length <= 100,"is not more than 100");
        for(uint256 i=0;i<_tokenid.length;i++){
            ERC721AUpgradeable(oldnftaddress).transferFrom(msg.sender,address(this),_tokenid[i]);
        }
        if(verify(admin,msg.sender,address(this),_tokenid.length,signature)){
            _mint(msg.sender, _tokenid.length);
            unchecked {
                ERC721AStorage.layout()._preSaleCurrentIndex += _tokenid.length;
                ERC721AStorage.layout()._tokensBoughtPreSale[msg.sender] += _tokenid.length;
            }
        }
        return true;
    }
    function Destroy() public onlyOwner returns(bool){
        require(address(this).balance == 0,"balance of this contract not zero");
    ...
    function verify(
        ...
        bytes32 messageHash = getMessageHash(from, to,amount);
    ...
    function preSaleMint(uint256 quantity) external payable {
        require(!AddressUpgradeable.isContract(msg.sender),"is not call contract address");
        require(
