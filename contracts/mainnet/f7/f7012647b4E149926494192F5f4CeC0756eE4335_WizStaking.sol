    ...
    OwnableUpgradeable
{
    ...
    uint256 public rentMinerFee;
    uint256 public feedMinerFee;
    uint256 public dailyPercentage;

    //mainnet address: 0xAB594600376Ec9fD91F8e885dADF0CE036862dE0;
    address private MATIC_TO_USD;
    uint256 public maticDeposit;
    uint256 public feedingMaticDeposit;
    ...
    struct History {
        uint256 time;
        uint256 amount;
        WithdrawalType typeOfWithdrawal;
    }
    ...
    function rentMiner(address referral) public payable nonReentrant {
        ...
        require(
            tokenAmount > 0 && _isDepositEnough(msg.value),
            "token > 0 and not enough matic"
        );
        require(
        ...
        //require(_isDepositEnough(maticAmount), "you need enough Matic");
        ...
        History memory rentHistory = History({
            time: block.timestamp,
            amount: tokenAmount,
            typeOfWithdrawal: WithdrawalType.Rent
        });

        withdrawalHistory[msg.sender].push(rentHistory);
    ...
    function feedMiner(uint256 _minerIndex) public payable nonReentrant {
        ...
        require(
            _isFeedingDepositEnough(msg.value) && canFeed,
            "Matic deposit not enough && can't feed"
        );
        ...
        History memory feedHistory = History({
            time: block.timestamp,
            amount: _amount,
            typeOfWithdrawal: WithdrawalType.Feeding
        });

        withdrawalHistory[msg.sender].push(feedHistory);
    ...
    function _canStillWithdrawFunds(uint256 _lastFeeding)
        private
        view
        returns (bool)
    {
        //check if feeding is is within 7 days
        uint256 feedingDays = _lastFeeding + (8 days);
        if (block.timestamp < feedingDays) {
            return true;
        } else {
            return false;
    ...
    function claimReward() public nonReentrant {
        ...
        for (uint256 i = 0; i < minersLength; i++) {
            ...
            bool canWithdraw;
            if (_canStillWithdrawFunds(miner.minerLastFed)) {
        ...
        History memory claimHistory = History({
            time: block.timestamp,
            amount: amountToTransfer,
            typeOfWithdrawal: WithdrawalType.Claiming
        });

        withdrawalHistory[msg.sender].push(claimHistory);
    ...
    function getTokenPrice() public view returns (uint256) {
        (uint256 matic, uint256 bitd, ) = UNISWAP_PAIR.getReserves();
        //get token price in wbnb
        //peform the multiplication bnb / 1e18 / token / 1e9
        uint256 reserves = ((10**18 * (10**9 * matic)) / (10**18 * bitd));

        int256 maticPrice = _getLatestPriceBNBTOUSD();

        uint256 priceInDollars = reserves * (uint256(maticPrice));
    ...
    function getUserDetails(address _address)
        public
        view
        returns (User memory, History[] memory)
    {
        return (users[_address], withdrawalHistory[_address]);
    }

    function withdrawTokenFromContract() public onlyOwner nonReentrant {
    ...
    function setMaticValueForFeeding(uint256 _value) public onlyOwner {
        feedingMaticDeposit = _value;
    }
    ...
    function _isFeedingDepositEnough(uint256 _amount)
        private
        view
        returns (bool)
    {
        return _amount == feedingMaticDeposit;
    }

    function configureAdminWallet(address payable _wallet, uint256 index)
        public
        onlyOwner
    {
        if (index == 1) {
        ...
        if (index == 4) {
            //4 is MarketingWallet
            MarketingWallet = _wallet;
        }

        if (index == 5) {
            //5 is ChimneyWallet
            ChimneyWallet = _wallet;
        }
    }

    function configureLeadersWallet(address payable _address, uint256 index)
        public
        onlyOwner
    {
        LeadersWallet[index] = _address;
    }
