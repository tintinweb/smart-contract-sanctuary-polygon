...
contract nanojobs {
        ...
        if (
            ...
            if (
                ...
                if (vstat.vote_ok > vstat.vote_nok) {
                    ...
                    usdToken.transfer(jobs[proposals[index].proposalTask]
                    .solutions[proposals[index].proposalValue].proponent, jobs[proposals[index].proposalTask].reward);
                    //pay
                } else {
                    jobs[proposals[index].proposalTask]
                    .solutions[proposals[index].proposalValue]
                    ...
                    jobs[proposals[index].proposalTask].jobstatus=uint256(JobStatusEnum.WAITING);
                    
                    unsolvedJobs.push();
                    unsolvedJobs[unsolvedJobs.length - 1] = proposals[index].proposalTask;
                    // no pay
                }
            }
        }
    ...
    function setToNanojobWhitelist(uint256 index, address[] memory whitelist,bool inUse) public {
    ...
    function setToNanojobConflictResolutor(uint256 index, address[] memory conflictResolutor,bool inUse) public {
        require(msg.sender == jobs[index].owner,"job owner only");
        require(conflictResolutor.length > 0);
        jobs[index].useConflictResolution=true;
        for (uint256 i = 0; i < conflictResolutor.length; i++) {
            _jobConflictResolutors[index].resolutors[conflictResolutor[i]]=inUse;
        }    
    }
    ...
    function addNanojob(
        ...
        jobs[_counter_jobs.current()].jobstatus = 0;
        unsolvedJobs.push();
        unsolvedJobs[unsolvedJobs.length - 1] = _counter_jobs.current();
    ...
    function getWaitingJobs() public view returns (uint256[] memory){
        return unsolvedJobs;
    }

    function addJobSolution(uint256 jobIndex, string memory solution)
        public
        returns (uint256)
    {
        require(
        ...
        if (jobs[jobIndex].paymenttype == 1) {
            usdToken.transfer(msg.sender, jobs[jobIndex].reward);
            solstat = 3;
        }
        ...
        if (jobs[jobIndex].solutions.length>=jobs[jobIndex].maxsolutions){
            jobs[jobIndex].jobstatus=uint256(JobStatusEnum.COMPLETED);
            for (uint256 _i=0;_i<unsolvedJobs.length;_i++){
                if (unsolvedJobs[_i]==_counter_jobs.current()){
                    unsolvedJobs[_i]=unsolvedJobs[unsolvedJobs.length-1];
                    unsolvedJobs.pop();
                }
            }
        }
        return jobs[jobIndex].solutions.length;
    }
    ...
    function setJobSolutionByIndex(
        ...
        if (jobs[jobIndex].useConflictResolution && msg.sender!=jobs[jobIndex].owner) {
            require(
