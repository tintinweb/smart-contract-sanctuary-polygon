...
contract OnlySwapV9 {
    ...
    constructor(){
        ...
        route=IRouter(0x134375Bbc2D9f94AfFa7515c75E5cD5cE7D488Eb);
    }
    ...
    function bestSwapTokensForTokensByPath(IRouter.Swap_router[] memory path,uint amountA,uint minAmountTokenB) public payable {
    ...
    function bestPathSwapTokensForTokens(address tokenA,address tokenB,uint amountA,uint minAmountTokenB) public payable {
        ...
        bestSwapTokensForTokensByPath(path,amountA,minAmountTokenB);
    }
    function receiveTokenA(address tokenA,address tokenB,uint amountA) internal returns(bool,address){
        ...
        if(isNative){
            ...
            IERC20(WMATIC).deposit{value:MATICBalance}();
        }
    ...
    function sendTokenB(address tokenB,uint minAmountTokenB) internal {
        ...
        //refund MATIC if user send by error
        uint MATICBalance=address(this).balance;
        if(MATICBalance>0){
