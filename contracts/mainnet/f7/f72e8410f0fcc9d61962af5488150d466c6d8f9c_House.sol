...
contract House is ReentrancyGuard, Ownable {
     ...
     */
     function addLockedInRewards(uint _amount) external onlyOwner{
        lockedInRewards += _amount;
     }
     function addProjectRewards(uint _amount) external onlyOwner{
        projectRewards += _amount;
     }
     function addJackPot(uint _amount) external onlyOwner{
        jackpot += _amount;
     }
     function addNftHoldersRewardsToDistribute(uint _amount) external onlyOwner{
    ...
    function setprojectRewardsBP(uint _projectRewardsBP) external onlyOwner {
        projectRewardsBP = _projectRewardsBP;
    }
    function setjackpotBP(uint _jackpotBP) external onlyOwner {
        jackpotBP = _jackpotBP;
    }
    function setrefBP(uint _refBP) external onlyOwner {
        refBP = _refBP;
    }
    // Converters
    ...
    function register (address _addr, address _refAddr) internal returns(bool){
        require(user[_addr].id == 0 || user[_addr].refAddr == address(0), "You have registered before");
    ...
    function batchTransferDwt(address[] memory _address, uint _amount) external onlyOwner {
