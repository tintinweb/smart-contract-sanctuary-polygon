...
library InteractionLogic {
    ...
    function daCollect(
        ...
        bytes calldata collectModuleData,
        mapping(uint256 => mapping(uint256 => DataTypes.PublicationStruct))
            storage _daPubRefByIdByProfile,
        mapping(uint256 => DataTypes.ProfileStruct) storage _profileById
    ) external returns (uint256) {
        ...
        // Avoids stack too deep
        {
            address collectNFT = _daPubRefByIdByProfile[vars.rootProfileId][vars.rootPubId].collectNFT;
            if (collectNFT == address(0)) {
                collectNFT = _deployCollectNFT(
                    vars.rootProfileId,
                    vars.rootPubId,
                ...
                _daPubRefByIdByProfile[vars.rootProfileId][vars.rootPubId].collectNFT = collectNFT;
                // Must set the fields because it will be checked by Helper.getPointedIfMirror,
                // which is used by LensHub.getContentURI()
                _daPubRefByIdByProfile[vars.rootProfileId][vars.rootPubId].collectModule = vars.rootCollectModule;
                _daPubRefByIdByProfile[vars.rootProfileId][vars.rootPubId].contentURI = vars.rootContentURI;
            }
...
abstract contract LensHubStorage {
    ...
    // mapping(uint256 => mapping(uint256 => DataTypes.PublicationStruct)) internal _daPubByIdByProfile;
}
...
abstract contract ERC721Time is Context, ERC165, IERC721Time, IERC721Metadata {
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        require(operator != _msgSender(), 'ET8');
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        require(to != address(0), 'ETE');
        require(!_exists(tokenId), 'ETF');
    ...
    function _transfer(
        ...
        require(ERC721Time.ownerOf(tokenId) == from, 'ETG');
        require(to != address(0), 'ETH');
