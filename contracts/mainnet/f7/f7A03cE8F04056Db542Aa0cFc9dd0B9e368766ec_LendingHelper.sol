...
contract LendingHelper {
    ...
    function getAaveAssets() public view returns (address[] memory assets) {
    ...
    function getBorrowFromSupply(uint256 supply, uint256 collatRatio)
        ...
        returns (uint256)
    {
        return supply.mul(collatRatio).div(basisOne.sub(collatRatio));
    }

    function getCurrentPositionInWant() public view returns (uint256 deposits, uint256 borrows) {
        deposits = balanceOfAToken();
        borrows = wethToWant(balanceOfDebtToken()); // Conversion needed to want
    }
    ...
    function triggerLend() public view returns (bool) {
        ...
        if (currentCollatRatio >= maxCollatRatio) return true; // 6500 currently
    }
    ...
    function initiateLending() external onlyAuthorized {
        (uint256 deposits, ) = getCurrentPositionInWant();
        // Deposit available want as collateral as much as possible but always below max for lend
        uint256 maxCollateralAmount = balanceOfWantMaxForLend();
        if (strategyWantBalance() > wantDust && deposits < maxCollateralAmount) {
            uint256 collToDeposit =
                Math.min(strategyWantBalance(), balanceOfWantMaxForLend().sub(deposits));
            // TODO: make this nicer
            _depositCollateral(collToDeposit);
        }
        // Adjust the deposits & borrows according to the collateral target
        // TODO: make this nicer
        _adjustToTargetRatio();
    }

    function adjustToTargetRatio() public onlyAuthorized {
        _adjustToTargetRatio();
    }

    function liquidateAllLend() public onlyAuthorized returns (uint256 _amountFree) {
        uint256 balanceOfDebtTokenInWant = wethToWant(balanceOfDebtToken());
    ...
    /// @notice Adjust position down according an amount that needs to be freed.
    function freeFunds(uint256 amountToFree) public onlyAuthorized {
        ...
        // Calculate the new total borrow according total lend & targetCollatRatio
        uint256 newBorrow = getBorrowFromSupply(newSupply, targetCollatRatio);

        // Repay required amount
        _adjustDownTo(newBorrow, borrows);
        _sendFundsToStrategy();
    }

    function repayWeth(uint256 amount) public onlyAuthorized returns (uint256) {
        _repayWeth(amount);
    }

    function unwindPosition(uint256 _repayAmount) public onlyAuthorized {
    ...
    function depositCollateral(uint256 amount) public onlyAuthorized {
    ...
    function _adjustToTargetRatio() internal {
        // Check current position
        uint256 currentCollatRatio = getCurrentCollatRatio();
        // If we are under target -> we want to be max borrowed
        if (currentCollatRatio < targetCollatRatio) {
            // We should borrow up
            if (targetCollatRatio.sub(currentCollatRatio) > minRatio) {
                // We only act on relevant differences (minRatio)
                _borrow();
            }
            // If we are over target -> we want to adjust down
        } else if (currentCollatRatio.sub(targetCollatRatio) > minRatio) {
            (uint256 deposits, uint256 borrows) = getCurrentPositionInWant();
    ...
    function _transferWethFromStrat(uint256 amount) internal {
        ...
        if (wethBal < amount) {
            IERC20(wETH).safeTransferFrom(address(strategy), address(this), amount.sub(wethBal));
        }
        _receiveFundsCheck(stratWethBal, strategy.balanceOfWeth(), balanceOfWeth(), wethBal);
    }
    ...
    function _transferWantFromStrat(uint256 amount) internal {
        ...
        if (wantBal < amount) {
            IERC20(want).safeTransferFrom(address(strategy), address(this), amount.sub(wantBal));
        }
        _receiveFundsCheck(stratWantBal, strategy.balanceOfWant(), balanceOfWant(), wantBal);
    }
    ...
    function _sendFundsCheck(
        ...
        require(stratDelta == contractDelta, '!TransferSanity');
        //TODO: add check to make sure this is the same as the transferred amount???
    }
    ...
    function _repayWeth(uint256 amount) public onlyAuthorized returns (uint256) {
    ...
    function _borrow() internal returns (uint256) {
        ...
        if (totalAmountToBorrow > wantDust) {
            uint256 canBorrow = getBorrowFromDepositInWant(deposits, maxBorrowCollatRatio);

            if (canBorrow <= borrows) return 0;
            canBorrow = canBorrow.sub(borrows);

            if (canBorrow < totalAmountToBorrow) {
                totalAmountToBorrow = canBorrow;
            }
            // Borrow desired amount checking that is below our maxBorrowCollatRatio
            _borrowWeth(wantToWeth(totalAmountToBorrow));
            return totalAmountToBorrow;
        }
    }

    function _unwindPartialPosition(uint256 _amount) internal {
        uint256 availableWeth = strategy.balanceOfWeth();
        uint256 toRepayInWeth = Math.min(_amount, availableWeth);
    ...
    function _increasePositionHealth(uint256 _amount) internal {
        (uint256 deposits, uint256 borrows) = getCurrentPositionInWant();
        uint256 targetDeposit = getDepositFromBorrow(borrows, targetCollatRatio).sub(deposits);
        uint256 toDeposit = Math.min(_amount, targetDeposit);

        if (toDeposit > wantDust) {
            uint256 collToDeposit = Math.min(toDeposit, strategy.balanceOfWant());
            _depositCollateral(collToDeposit);
        }
    }

    /**
     * Adjust position down from currentBorrowed to newAmountBorrowed in want. // TODO check this function
     */
    function _adjustDownTo(uint256 newBorrow, uint256 currBorrowed) internal returns (uint256) {
        if (currBorrowed > newBorrow) {
            uint256 deltaBorrow = wantToWeth(currBorrowed.sub(newBorrow));
            _unwindPartialPosition(deltaBorrow);
            return deltaBorrow;
        }
        if (currBorrowed < newBorrow) {
            uint256 deltaDeposit = wantToWeth(newBorrow.sub(currBorrowed));
            // Deposit back to get targetCollatRatio (we always need to leave this in this ratio)
            _increasePositionHealth(deltaDeposit);
            return deltaDeposit;
        }
    }
    ...
    function _withdrawCollateral(uint256 amount) internal returns (uint256) {
        if (amount == 0) return 0;
        lendingPool.withdraw(want, amount, address(this));
        return amount;
    }
    ...
    function wantToWeth(uint256 _wantAmount) internal view returns (uint256) {
        if (_wantAmount == 0) return 0;
        return _wantAmount.mul(1e18).div(_wethPrice());
    }

    function wethToWant(uint256 _wethAmount) internal view returns (uint256) {
    ...
    function deleverageWithCollateral(uint256 _collatRatio) external onlyAuthorized {
        ...
        uint256 amountWithdraw = _withdrawExcessCollateral(_collatRatio);
        _sendFundsToStrategy();
        strategy.swapToWeth(amountWithdraw);
        _repayWeth(wantToWeth(amountWithdraw));
    }
