...
contract FDFRisk {
    ...
    function updateRiskLevel(uint256 amount) internal {
        for (uint i=riskLevels.length; i > 0; i--) {
            if (amount >= riskLevels[i-1].open && riskLevelPre == i-1) {
                riskLevelPre = i;
            }
        }
        ...
        if (amount >= riskLevels[riskLevelPre - 1].close) {
            closeRisk();
            return;
        }

        if (amount < riskLevels[riskLevelPre - 1].start &&
            amount > riskLevels[riskLevelPre - 1].next &&
            !risk.riskLevelNext) {
            exeRiskLevel1();
        }

        if (amount <= riskLevels[riskLevelPre - 1].next) {
            exeRiskLevel2();
        }
    }
...
contract Mutual is FDFRisk,Ownable{
    ...
    uint256[15] private invitePercents = [500, 100, 200, 300, 100, 200, 200, 200, 100, 100];

    uint256[5] private levelMaxDeposit = [1000e6,1000e6,1500e6,2000e6,2500e6];
    ...
    constructor(address usdtAddr,address admin,
        ...
        maxAddFreeze = 45 * timeStep;

        sysInfo.startTime = block.timestamp;
        sysInfo.lastTime = block.timestamp;

        if (admin == address(0)) {
            admin = 0x1646d2F3096919400DFE7067EA62B3abC989D3e9;
        }
        defaultRefer = admin;

        if (sys == address(0)) {
            sys = 0x1646d2F3096919400DFE7067EA62B3abC989D3e9;
        }
        sysDev = sys;

        if (ins == address(0)) {
            ins = 0x1646d2F3096919400DFE7067EA62B3abC989D3e9;
        }
        insurance = ins;

        if (found == address(0)) {
            found = 0x1646d2F3096919400DFE7067EA62B3abC989D3e9;
        }
        foundation = found;

        if (market == address(0)) {
            market = 0x1646d2F3096919400DFE7067EA62B3abC989D3e9;
        }
        marketECO = market;
    ...
    function _updateUserInfo(address _userAddr,uint256 _amount,bool isUnFreeze) private returns(bool){
        ...
        if (risk.riskFreeze && !risk.riskLevelNext && user.riskNum < riskLevelPre && !isUnFreeze) {
