...
contract Auction is IERC721Receiver, Context, ReentrancyGuard {
    ...
    event LogStartAuction(address indexed erc721address, uint tokenId, uint startTimestamp, uint endTimestamp);
    event LogBid(address indexed auction, address indexed bidder, uint bid, uint endTimestamp);
    ...
    event LogBuy(address indexed buyer, uint price);
	...
	*/
	...
	function _calculateFee(uint _amount) internal view returns (uint256 _commission, uint256 _remain)
    {
        uint temp;
        bool b;
