...
contract Collection is Ownable, ERC721Enumerable {
    ...
    */
	function setURI(string memory _newURI) external onlyOwner {
		emit SetURI(baseURI(), _newURI);

		_baseURI_ = _newURI;
	}

	/**
       	@notice Mint NFT to `_to`
       	@dev  Caller must be Owner
		@param	_to				    Address of Beneficiary
		@param	_tokenIds			A list of minting tokenIDs
    */
	function mint(address _to, uint256[] calldata _tokenIds) external onlyOwner {
    ...
    function burn(uint256[] calldata _tokenIds) external {
        ...
        for (uint256 i; i < _len; i++) {
            ...
            require(
                ownerOf(_tokenId) == _operator || _operator == owner(),
                "Token not owned nor Authorizer"
            );
	...
	/**
       	@notice Query a list of NFTs that owned by `_account`
       	@dev  Caller can be ANY
		@param	_account		Querying account's address
    ...
    */
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override isPaused {
		super._beforeTokenTransfer(from, to, tokenId);
    }
