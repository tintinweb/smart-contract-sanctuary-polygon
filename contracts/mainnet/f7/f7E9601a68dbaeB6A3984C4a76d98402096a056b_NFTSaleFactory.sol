...
contract NFTSale {
  ...
  function initialize(
    ...
    require(!initialized && minKom > 0 && _sale.length == _price.length && msg.sender == address(factory) && _calculation < _start, "bad");
  ...
  function setPrice(uint128[] calldata _price) external onlyOwner {
     ...
     * @param _fee_d2 Fee project percent in 2 decimal
     */
    function setFee_d2(uint16[2] calldata _fee_d2) external onlyOwner {
      require(uint64(block.timestamp) < booster[1].start, "bad");
