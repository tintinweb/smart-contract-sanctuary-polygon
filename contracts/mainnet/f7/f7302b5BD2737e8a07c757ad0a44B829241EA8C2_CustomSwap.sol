...
contract CustomSwap is ISwap, Ownable {
    ...
    address quartersAddress;

    constructor(address wallet, address _quartersAddress) {
        masterWallet = wallet;
        quartersAddress = _quartersAddress;
    }

    receive() external payable {}

    modifier onlyQuarters() {
        require(
            msg.sender == quartersAddress,
            "Quarters: caller is not the quarters"
        );
        _;
    }
    ...
    function changeQuarters(address _quarters) external onlyOwner {
        quartersAddress = _quarters;
    }
