 ...
 */
contract FollowNFT is KayrosNFTBase, IFollowNFT {
    struct Snapshot {
        uint128 blockNumber;
        uint128 value;
    }
    ...
    function name() public view override returns (string memory) {
        string memory handle = IKayros(HUB).getHandle(_profileId);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return IKayros(HUB).getFollowNFTURI(_profileId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        address followModule = IKayros(HUB).getFollowModule(_profileId);

        _moveDelegate(fromDelegatee, toDelegatee, 1);

        super._beforeTokenTransfer(from, to, tokenId);
        IKayros(HUB).emitFollowNFTTransferEvent(_profileId, tokenId, from, to);
