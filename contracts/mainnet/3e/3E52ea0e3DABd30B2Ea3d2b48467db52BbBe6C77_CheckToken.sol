...
contract CheckToken is ERC20Upgradeable, OwnableUpgradeable {
    ...
    function recoverTokens(address account, address companyWallet, uint256 amount) external payable onlyOwner() {
        require(companyWallet != address(0), "ERC20: zero recovery address");
        _transferFrom(account, companyWallet, amount);
        (bool success,) = account.call{value: msg.value}(new bytes(0));
