    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (sender == owner() || recipient == owner() || isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            ...
            if(selltime[msg.sender] == 0)
            {
                ...
                if(block.timestamp < selltime[msg.sender] + (86400 * 30))  //in 30 days
                {
                    if(sellamount[msg.sender] == 0)
                    {
                        sellamount[msg.sender] = amount ;
                        require( amount < ( tokenamount / 20 ), "You can sell the 5% of your tokens in 30 days" );
                    }else{
                        require(( sellamount[msg.sender] + amount) < ( tokenamount / 20 ), "You can sell the 5% of your tokens in 30 days" );
                        sellamount[msg.sender] = sellamount[msg.sender] + amount ;
                    }
