...
contract RefractlyPenroseStrategy is StratFeeManager, GasFeeThrottler {
  ...
  address private constant penroseProxyAddress = 0xc9Ae7Dac956f82074437C6D40f67D6a5ABf3E34b;

  IPenLens private constant penLens = IPenLens(penLensAddress);
  IPenroseProxy private constant penroseProxy = IPenroseProxy(penroseProxyAddress);
  ...
  function deposit() public whenNotPaused {
    ...
    if (wantBal > 0) {
      penroseProxy.depositLpAndStake(want, wantBal);
  ...
  function withdraw(uint256 _amount) external {
    ...
    if (wantBal < _amount) {
      penroseProxy.unstakeLpAndWithdraw(want, _amount - wantBal);
  ...
  function _harvest(address callFeeRecipient) internal whenNotPaused {
    ...
    if (lpBalance > 0) {
      address stakingPool = penLens.stakingRewardsByDystPool(address(want));
      penroseProxy.claimStakingRewards(stakingPool);
    }
  ...
  function rewardAvailable(address rewardToken) public returns (uint256) {
    address stakingPool = penLens.stakingRewardsByDystPool(address(want));
    address proxyAddress = penLens.userProxyByAccount(address(this));
  ...
  function retireStrat() external {
    ...
    penroseProxy.unstakeLpAndWithdraw(want, balanceOfPool());
  ...
  function _giveAllowances() internal {
    IERC20(want).safeApprove(penroseProxyAddress, 0);
    IERC20(want).safeApprove(penroseProxyAddress, type(uint256).max);
