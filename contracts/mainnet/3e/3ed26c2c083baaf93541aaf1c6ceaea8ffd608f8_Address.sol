...
contract Cheetopia is ERC721Enumerable, Ownable {
  ...
  uint256 public constant TOKEN_PRICE = 0.16 ether;
  uint256 public constant MAX_TOKENS  = 200;
    
  constructor() ERC721("Cheetopia", "CHEETAH") {
  ...
  function allowlistMint(bytes32[] memory proof) external payable {
    ...
    require(
      ...
      "Your address is not on the allow list"
    );
    require(!hasClaimed[msg.sender], "Already claimed your CHEETAH");
    require(supply + 1 <= MAX_TOKENS, "Purchase would exceed max supply");
