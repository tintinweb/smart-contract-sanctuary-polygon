	...
	UniswapV2ERC20Upgradeable
{
	...
	function getIncreaseFactor(uint256 amountStable) private returns (uint256 increaseFactor) {
		...
		increaseFactor = dexLpInEth == 0
			? 0
			: (amountStableInEth * BASE_FACTOR) / (dexLpInEth - debtInEth);
	}
	...
	function _withdraw(
		...
		uint256 amountStableSnapshot = _tokens.stable.balanceOf(address(this));
		...
		amountStableToUser = amountFromPool + (amountStableSnapshot * liquidity) / totalSupply;
