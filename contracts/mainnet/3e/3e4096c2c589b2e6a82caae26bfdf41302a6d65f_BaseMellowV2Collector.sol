...
import "../vaults/LpTokenFacet.sol";
import "../oracles/Oracle.sol";

contract BaseMellowV2Collector is IBaseCollector {
    ...
    function securityParamsByToken(address token, VaultFacet facet) public view returns (bytes memory) {
        address[] memory tokens = facet.tokens();
        bytes[] memory securityParams = facet.securityParams();
        for (uint256 i = 0; i < tokens.length; i++) {
            if (token == tokens[i]) return securityParams[i];
        }
        return new bytes(0);
    }

    function convertInUnderlyings(
        ...
        {
            (IBaseOracle oracle_, ) = oracle.erc20Oracles(token);
        ...
        (address[] memory subtokens, uint256[] memory subtokenAmounts) = oracle.quote(
            token,
            amount,
            securityParamsByToken(token, facet)
        );

        for (uint256 i = 0; i < subtokens.length; i++) {
            uint256[] memory subunderlingAmounts = convertInUnderlyings(
                underlyingTokens,
                subtokens[i],
                subtokenAmounts[i],
                facet
            ...
            for (uint256 j = 0; j < underlyingTokens.length; j++) {
                amounts[j] += subunderlingAmounts[j];
            }
        }
    }
    ...
    ) public view returns (address[] memory underlyingTokens, uint256[] memory underlyingTokenAmounts) {
        bool[] memory isUnderlyingToken = new bool[](tokens.length);
        uint256 count = 0;
        for (uint256 i = 0; i < tokens.length; i++) {
            (IBaseOracle oracle_, ) = oracle.erc20Oracles(tokens[i]);
            if (address(oracle_) != address(0)) {
                isUnderlyingToken[i] = true;
                count++;
            }
        }

        underlyingTokens = new address[](count);
        underlyingTokenAmounts = new uint256[](count);
        count = 0;
        for (uint256 i = 0; i < tokens.length; i++) {
            if (isUnderlyingToken[i]) {
                underlyingTokens[count++] = tokens[i];
            }
        }

        for (uint256 i = 0; i < tokens.length; i++) {
            uint256[] memory subamounts = convertInUnderlyings(underlyingTokens, tokens[i], amounts[i], facet);
            for (uint256 j = 0; j < underlyingTokenAmounts.length; j++) {
                underlyingTokenAmounts[j] += subamounts[j];
            }
        }
    }
    ...
    function collect(
        ...
        VaultFacet facet = VaultFacet(vault);
        (address[] memory tokens, uint256[] memory amounts) = facet.getTokensAndAmounts();
        (underlyingTokens, response.tvl) = convert(facet, tokens, amounts);

        response.subvaultsTvl = new uint256[][](1);
        response.subvaultsTvl[0] = response.tvl;
        response.unclaimedFees = new uint256[](underlyingTokens.length);

        try LpTokenFacet(vault).lpToken() returns (LpToken lpToken) {
            if (address(lpToken) != address(0)) {
                response.totalSupply = lpToken.totalSupply();
                response.userBalance = lpToken.balanceOf(user);
            }
        } catch {}

        if (response.totalSupply == 0) {
            // handle most of edge cases
            response.totalSupply = 1e18;
        }
    }
}
