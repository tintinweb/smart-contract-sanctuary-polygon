...
import "contracts/utils/NFTPartyMembershipOwnershipPauserMintPriceTransferTaxBase.sol";
...
import "contracts/utils/NFTPartyMembershipStorage.sol";
import "contracts/utils/MinimalPartyMembersToMintStorage.sol";
...
import "contracts/utils/WaitingDelayBetweenMintsSecondsForPartyLeaderStorage.sol";


/**
 * @title ERC721 token confirming the release of party rules.
 */
contract NFTPartyDraftRules is
    Base,
    NFTPartyMembershipOwnershipPauserMintPriceTransferTaxBase,
    VotePowerPoints,
    MintPowerPointsStorage,
    MinPowerPointsStorage,
    MaxInProgressStorage,
    AcceptedExpirationSecondsStorage,
    VotingPeriodSecondsStorage,
    LevelAcceptedNumeratorStorage,
    QuorumNumeratorStorage,
    WaitingDelayBetweenMintsSecondsStorage,
    WaitingDelayBetweenMintsSecondsForPartyLeaderStorage,
    MintHashMixin,
    MinimalPartyMembershipToMintStorage,
    AddPowerPointsOnFinalize
{
    mapping(uint256 /*tokenId*/ => TokenInfo) internal tokenInfo;
    address public nftPartyRules;

    mapping (uint256 /*partyMembershipId*/ => uint256 /*timestamp*/) public partyMembershipLastMintTimestamp;
    mapping (uint256 /*partyMembershipId*/ => uint256 /*counter*/) public partyMembershipInProgressCounter;
    ...
    struct TokenInfo {
        ...
        uint256 votesYes;
        uint256 votesNo;
        mapping(uint256 /*nftPartyMembershipTokenId*/ => bool) partyMembershipIsVoted;
        uint256 rulesTokenId;
    }
    ...
    /// @return votesYes votes number for "yes"
    /// @return votesNo votes number for "no"
    /// @return rulesTokenId rules token id
    function getTokenInfo(uint256 tokenId) external view returns(
        ...
        TokenInfo storage o = tokenInfo[tokenId];
        mintedTimestamp = o.mintedTimestamp;
        status = o.status;
        votesYes = o.votesYes;
        votesNo = o.votesNo;
        rulesTokenId = o.rulesTokenId;
    }

    event Vote(uint256 indexed tokenId, uint256 indexed partyMembershipId, uint256 indexed passportId);
    event Accepted(uint256 indexed tokenId);
    event Declined(uint256 indexed tokenId);
    ...
    function initialize(
        ...
        __NFTPartyMembershipOwnershipPauserMintPriceTransferTaxBase_init({
            name_: nameValue,
            symbol_: symbolValue,
    ...
    function initialize2(
        ...
        __NFTPartyMembershipStorage_init_unchained();
    }

    /// @notice 3rd initialization
    /// @param votePowerPointsValue votePowerPoints value
    /// @param mintPowerPointsValue mintPowerPoints value
    /// @param votingPeriodSecondsValue votingPeriodSeconds value
    /// @param levelAcceptedNumeratorValue levelAcceptedNumerator value
    /// @param maxInProgressValue maxInProgress value
    /// @param minPowerPointsValue minPowerPoints value
    /// @param acceptedExpirationSecondsValue acceptedExpirationSeconds value
    /// @param quorumNumeratorValue quorumNumerator value
    /// @param waitingDelayBetweenMintsSecondsValue waitingDelayBetweenMintsSeconds value
    /// @param waitingDelayBetweenMintsSecondsForPartyLeaderValue waitingDelayBetweenMintsSecondsForPartyLeader value
    function initialize3(
        uint256 votePowerPointsValue,
        uint256 mintPowerPointsValue,
        uint256 votingPeriodSecondsValue,
        uint256 levelAcceptedNumeratorValue,
        uint256 maxInProgressValue,
        uint256 minPowerPointsValue,
        uint256 acceptedExpirationSecondsValue,
        uint256 quorumNumeratorValue,
        uint256 waitingDelayBetweenMintsSecondsValue,
        uint256 waitingDelayBetweenMintsSecondsForPartyLeaderValue
    ) external virtual initializer3 {
        // make contract code size smaller via embedding some trivial code
        ...
        // __VotingPeriodSecondsStorage_init_unchained(votingPeriodSecondsValue);
        votingPeriodSeconds = votingPeriodSecondsValue;

        __LevelAcceptedNumeratorStorage_init_unchained(levelAcceptedNumeratorValue);
        // require(levelAcceptedNumeratorValue <= 10000, "BAD_NUMERATOR");
        // levelAcceptedNumerator = levelAcceptedNumeratorValue;

        //__MaxInProgressStorage_init_unchained(maxInProgressValue);
        ...
        // __WaitingDelayBetweenMintsSecondsStorage_init_unchained(waitingDelayBetweenMintsSecondsValue);
        waitingDelayBetweenMintsSeconds = waitingDelayBetweenMintsSecondsValue;

        // __WaitingDelayBetweenMintsSecondsForPartyLeaderStorage_init_unchained(waitingDelayBetweenMintsSecondsForPartyLeaderValue);
        waitingDelayBetweenMintsSecondsForPartyLeader = waitingDelayBetweenMintsSecondsForPartyLeaderValue;

        // __AcceptedExpirationSecondsStorage_init_unchained(acceptedExpirationSecondsValue);
    ...
    /// @notice initialize with nft party rules
    /// @param nftPartyRulesAddress nftPartyRules address
    function initNFTPartyRules(address nftPartyRulesAddress) external onlyOwner {
        require(nftPartyRules == address(0), "ALREADY_SET");  // may be set only once
        require(nftPartyRulesAddress != address(0), "ZERO_ADDRESS");
        nftPartyRules = nftPartyRulesAddress;
    }

    /// @notice mint token
    /// @param hash uniq hash of minting
    function mint(bytes32 hash) external {
        require(IERC721Enumerable(nftPartyMembership).totalSupply() >= minimalPartyMembershipToMint || minimalPartyMembershipToMint == 0, "TOO_SMALL_MEMBERS");
        uint256 tokenId = ++_lastTokenId;

        uint256 passportId = getPassportId();
        uint256 partyMembershipId = getPartyMembershipId(_msgSender());

        // Msg.Sender in total can have NFT Status = InProgress up to maxInProgress.
        require(partyMembershipInProgressCounter[partyMembershipId] < maxInProgress, "TOO_MANY");

        require(
            block.timestamp - partyMembershipLastMintTimestamp[partyMembershipId] >= waitingDelayBetweenMintsSeconds
        , "TOO_OFTEN");
        partyMembershipLastMintTimestamp[partyMembershipId] = block.timestamp;
        ...
        tokenInfo[tokenId].mintedTimestamp = block.timestamp;
        tokenInfo[tokenId].status = Status.InProgress;
        partyMembershipInProgressCounter[partyMembershipId] += 1;

        _emitMintedWithHash({tokenId: tokenId, hash: hash});
    }

    /// @notice vote for the token
    /// @param tokenId token id
    /// @param yes yes or no (true/false)
    function vote(uint256 tokenId, bool yes) external {
        // require(tokenInfo[tokenId].mintedTimestamp > 0, "NOT_EXISTS"); // already checked below BAD_STATUS
        require(tokenInfo[tokenId].mintedTimestamp + votingPeriodSeconds > block.timestamp, "TOO_LATE");
        require(tokenInfo[tokenId].status == Status.InProgress, "BAD_STATUS");  // can happen only if votingPeriodSeconds was changed after finalize

        uint256 passportId = getPassportId();
        uint256 partyMembershipTokenId = getPartyMembershipId();
        checkPartyMembership();

        require(!tokenInfo[tokenId].partyMembershipIsVoted[partyMembershipTokenId], "ALREADY_VOTED");
        // note that the same wallet may vote multiple times if it changes the owned partyMembership
        tokenInfo[tokenId].partyMembershipIsVoted[partyMembershipTokenId] = true;
        if (yes){
            tokenInfo[tokenId].votesYes += 1;
        } else {
            tokenInfo[tokenId].votesNo += 1;
        }
        emit Vote(tokenId, partyMembershipTokenId, passportId);

        // For voting, a citizen receives VotePowerPoints
        _addPowerPointsOnVote(passportId);
    }

    function _burn(uint256 tokenId) internal virtual override {
        if (tokenInfo[tokenId].status == Status.InProgress) {
            uint256 partyMembershipId = getPartyMembershipId(ownerOf(tokenId));
            partyMembershipInProgressCounter[partyMembershipId] -= 1;
        }
        super._burn(tokenId);
    ...
    /// @notice finalize the token
    /// @param tokenId token id
    function finalize(uint256 tokenId) external {
        require(tokenInfo[tokenId].mintedTimestamp + votingPeriodSeconds < block.timestamp, "TOO_LATE");
        require(tokenInfo[tokenId].status == Status.InProgress, "BAD_STATUS");
        uint256 votesYea = tokenInfo[tokenId].votesYes;
        uint256 votesNo = tokenInfo[tokenId].votesNo;
        uint256 votesTotal = votesNo + votesYea;
        if (
            (votesTotal * DENOMINATOR / IERC721EnumerableUpgradeable(nftPartyMembership).totalSupply() >= quorumNumerator) &&
            (votesYea * DENOMINATOR / votesTotal >= levelAcceptedNumerator)
        ) {
            tokenInfo[tokenId].status = Status.Accepted;
            emit Accepted(tokenId);
        } else {
            tokenInfo[tokenId].status = Status.Declined;
            emit Declined(tokenId);
        }
        uint256 partyMembershipId = getPartyMembershipId(ownerOf(tokenId));
        partyMembershipInProgressCounter[partyMembershipId] -= 1;
    ...
    function _beforeTokenTransfer(
        ...
        // The recipient's address must have an NFT PartyMembership.
        if (to != address(0)) {
            require(IERC721EnumerableUpgradeable(nftPartyMembership).balanceOf(to) == 1, "NO_MEMBERSHIP");
            checkPartyMembership(to);
        }

        // Transfer / resale is only possible if AllowTransfers = True.
        super._beforeTokenTransfer(from, to, tokenId);  // in ERC721AllowedTransfer

        // Only NFTs with statuses Declined, Expired, Executed can be alienated. It is prohibited to transfer with the Accepted, InProgress status.
        if ((from != address(0)) && (to != address(0))) {
            Status status = tokenInfo[tokenId].status;
            require(
                (status == Status.Declined) ||
                (status == Status.Expired) ||
                (status == Status.Executed) ||
                (to == nftPassport.docsCustody()),
                "BAD_STATUS"
            );
        }

        // When transferring, a fixed amount of TransferTax is transferred to the Treasury Account.
        // handle inside TransferTax
    }

    /// @notice get actual status
    /// @param tokenId token id
    /// @return status
    /// @dev for front-end usage
    function getActualStatus(uint256 tokenId) external view returns(Status) {
        if (tokenInfo[tokenId].status == Status.Accepted) {
            if (_isAcceptedExpired(tokenInfo[tokenId].mintedTimestamp, votingPeriodSeconds)){
                return Status.Expired;
            }
        }
        return tokenInfo[tokenId].status;
    }

    /// @notice execute accepted token
    /// @param tokenId token id
    /// @param rulesTokenId rules token id
    /// @return status
    function executeAccepted(uint256 tokenId, uint256 rulesTokenId) external returns(Status) {
        require(msg.sender == nftPartyRules, "BAD_CALLER");
        require(tokenInfo[tokenId].status == Status.Accepted, "BAD_STATUS");  // double check, it's already checked inside NFTPartyRules
        if (_isAcceptedExpired(tokenInfo[tokenId].mintedTimestamp, votingPeriodSeconds)) {
            ...
            tokenInfo[tokenId].rulesTokenId = rulesTokenId;
    ...
    function setStatusExpired(uint256 tokenId) external {
        require(tokenInfo[tokenId].status == Status.Accepted, "BAD_STATUS");
        require(_isAcceptedExpired(tokenInfo[tokenId].mintedTimestamp, votingPeriodSeconds), "NOT_EXPIRED");
        tokenInfo[tokenId].status = Status.Expired;
        emit Expired(tokenId);
    }
}
...
import "contracts/utils/CoinStorage.sol";
import "contracts/utils/Access.sol";
import "contracts/utils/NFTPartyMembershipOwnershipERC721BaseEnumerablePausableUpgradeable.sol";
import "contracts/utils/TransferTaxStorage.sol";
import "contracts/utils/SaleTaxNumeratorStorage.sol";
import "contracts/utils/MintPriceStorage.sol";
import "contracts/interfaces/ISaleTax.sol";
...
/// @title ERC721 with PartyMembership-ownership with offchain marketplace
/// @dev use it as a base contract for party assets
abstract contract NFTPartyMembershipOwnershipPauserMintPriceTransferTaxBase is
    Initializable,
    EIP712Upgradeable,
    Access,
    TransferTaxStorage,
    MintPriceStorage,
    SaleTaxNumeratorStorage,
    NFTPartyMembershipOwnershipERC721BaseEnumerablePausableUpgradeable
{
    using SafeERC20 for IERC20;

    uint256 internal _lastTokenId;  // note: start from 1

    function __NFTPartyMembershipOwnershipPauserMintPriceTransferTaxBase_init(
        string memory name_,
        string memory symbol_,
        string memory version,
        address coinAddress,
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, ownerValue);
    ...
    function _mint(address to, uint256 tokenId) internal virtual override {
        if (mintPrice != 0) {
            IERC20(coin).safeTransferFrom(_msgSender(), treasury, mintPrice);
        }
        emit MintPricePaid(_msgSender(), tokenId, mintPrice);
        super._mint(to, tokenId);
    }
    ...
    function supportsInterface(bytes4 interfaceId) override(NFTPartyMembershipOwnershipERC721BaseEnumerablePausableUpgradeable, SaleTaxNumeratorStorage) virtual view public returns(bool) {
        return NFTPartyMembershipOwnershipERC721BaseEnumerablePausableUpgradeable.supportsInterface(interfaceId) ||
            (type(IERC721EnumerableUpgradeable).interfaceId == interfaceId) ||
            SaleTaxNumeratorStorage.supportsInterface(interfaceId)
        ;
    }
    ...
    /// @param tokenId token id
    /// @param docsCustody docs custody
    function transferToDocsCustody(
        uint256 tokenId,
        address docsCustody
    ) external {
        require(msg.sender == address(nftPartyMembership), "wrong caller");
...
import "contracts/utils/NFTPartyMembershipStorage.sol";
import "contracts/NFTPartyMembership.sol";


/// @title ERC721 but ownership is based on PartyMembership
abstract contract NFTPartyMembershipOwnershipERC721BaseEnumerablePausableUpgradeable is
    Initializable,
    ContextUpgradeable,
    ERC165Upgradeable,
    IERC721Upgradeable,
    ...
    NFTPartyMembershipStorage
{
    using AddressUpgradeable for address;
    using StringsUpgradeable for uint256;

    event TransferWithPartyMembership(
        address from,
        address to,
        uint256 indexed tokenId,
        uint256 indexed fromPartyMembershipId,
        uint256 indexed toPartyMembershipId
    );
    ...
    // Mapping from token ID to owner partyMembership id
    mapping(uint256 /*tokenId*/ => uint256 /*partyMembershipId*/) private _ownerPartyMembershipId;

    // Mapping owner partyMembership id to token count
    mapping(uint256 /*partyMembershipId*/ => uint256 /*tokensCount*/) private _balances;
    ...
    // Mapping from owner partyMembership to list of owned token IDs
    mapping(uint256 /*partyMembershipId*/ => mapping(uint256 => uint256)) private _ownedTokens;
    ...
    function balanceOf(address ownerValue) public view virtual override returns (uint256) {
        ...
        if(IERC721EnumerableUpgradeable(NFTPartyMembership(nftPartyMembership).nftPassport()).balanceOf(ownerValue) == 0) {
            return 0;
        }
        if(IERC721EnumerableUpgradeable(nftPartyMembership).balanceOf(ownerValue) == 0) {
            return 0;
        }
        uint256 partyMembershipId = IERC721EnumerableUpgradeable(nftPartyMembership).tokenOfOwnerByIndex(ownerValue, 0);
        return _balances[partyMembershipId];
    }

    /**
     * @dev See {IERC721-balanceOf}.
     */
    function balanceOfPartyMembership(uint256 partyMembershipId) public view virtual returns (uint256) {
        return _balances[partyMembershipId];
    }
     ...
     */
    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        uint256 ownerPartyMembershipId = _ownerPartyMembershipId[tokenId];
        require(ownerPartyMembershipId != 0, "ownerPartyMembershipId=0");
        address ownerValue = ownerAddressOfPartyMembershipId(ownerPartyMembershipId);
        return ownerValue;
    }
     ...
     * @dev See {IERC721-ownerOf}.
     */
    function ownerPartyMembershipOf(uint256 tokenId) public view virtual returns (uint256) {
        uint256 ownerPartyMembershipId = _ownerPartyMembershipId[tokenId];
        require(ownerPartyMembershipId != 0, "ownerPartyMembershipId=0");
        return ownerPartyMembershipId;
    }
     ...
     */
    function approve(address to, uint256 tokenId) public virtual override {
        address ownerValue = NFTPartyMembershipOwnershipERC721BaseEnumerablePausableUpgradeable.ownerOf(tokenId);
     ...
     */
    function _exists(uint256 tokenId) internal view virtual returns (bool) {
        return _ownerPartyMembershipId[tokenId] != 0;
    }
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        uint256 toPartyMembershipId = getPartyMembershipId(to);
        _balances[toPartyMembershipId] += 1;
        _ownerPartyMembershipId[tokenId] = toPartyMembershipId;

        emit Transfer(address(0), to, tokenId);
        emit TransferWithPartyMembership(address(0), to, tokenId, 0, toPartyMembershipId);
    }
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        _balances[getPartyMembershipId(ownerValue)] -= 1;
        delete _ownerPartyMembershipId[tokenId];

        emit Transfer(ownerValue, address(0), tokenId);
        emit TransferWithPartyMembership(ownerValue, address(0), tokenId, getPartyMembershipId(ownerValue), 0);
    }
    ...
    function _transfer(
        ...
        require(NFTPartyMembershipOwnershipERC721BaseEnumerablePausableUpgradeable.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own");
        require(to != address(0), "ERC721: transfer to the zero address");
        ...
        uint256 fromPartyMembershipId = getPartyMembershipId(from);
        uint256 toPartyMembershipId = getPartyMembershipId(to);

        _balances[fromPartyMembershipId] -= 1;
        _balances[toPartyMembershipId] += 1;
        _ownerPartyMembershipId[tokenId] = toPartyMembershipId;

        emit Transfer(from, to, tokenId);
        emit TransferWithPartyMembership(from, to, tokenId, fromPartyMembershipId, toPartyMembershipId);
    }
    ...
    function tokenOfOwnerByIndex(address ownerValue, uint256 index) public view virtual override returns (uint256) {
        require(index < NFTPartyMembershipOwnershipERC721BaseEnumerablePausableUpgradeable.balanceOf(ownerValue), "ERC721Enumerable: owner index out of bounds");
        return _ownedTokens[getPartyMembershipId(ownerValue)][index];
    }
     ...
     */
    function tokenByIndex(uint256 index) public view virtual override returns (uint256) {
        require(index < NFTPartyMembershipOwnershipERC721BaseEnumerablePausableUpgradeable.totalSupply(), "ERC721Enumerable: global index out of bounds");
        return _allTokens[index];
    }
     ...
     */
    function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
        uint256 length = NFTPartyMembershipOwnershipERC721BaseEnumerablePausableUpgradeable.balanceOf(to);
        _ownedTokens[getPartyMembershipId(to)][length] = tokenId;
        _ownedTokensIndex[tokenId] = length;
    }
    ...
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId) private {
        ...
        uint256 lastTokenIndex = NFTPartyMembershipOwnershipERC721BaseEnumerablePausableUpgradeable.balanceOf(from) - 1;
        uint256 tokenIndex = _ownedTokensIndex[tokenId];
        ...
        // When the token to delete is the last token, the swap operation is unnecessary
        if (tokenIndex != lastTokenIndex) {
            uint256 lastTokenId = _ownedTokens[fromPartyMembershipId][lastTokenIndex];

            _ownedTokens[fromPartyMembershipId][tokenIndex] = lastTokenId; // Move the last token to the slot of the to-delete token
            _ownedTokensIndex[lastTokenId] = tokenIndex; // Update the moved token's index
        }

        // This also deletes the contents at the last position of the array
        delete _ownedTokensIndex[tokenId];
        delete _ownedTokens[fromPartyMembershipId][lastTokenIndex];
    }
    ...
    uint256[10] private __gap_NFTPartyMembershipOwnershipERC721BaseEnumerablePausableUpgradeable;
}
...
/// @title NFTPartyMembershipStorage
abstract contract NFTPartyMembershipStorage is NFTPartyStorage {
    address public nftPartyMembership;

    function __NFTPartyMembershipStorage_init_unchained() internal {
        nftPartyMembership = INFTParty(nftParty).getNFTPartyMembership(partyId);
    }

    function getPartyMembershipId() internal view returns(uint256) {
        return getPartyMembershipId(_msgSender());
    }

    function getPartyMembershipId(address who) internal view returns(uint256) {
        require(IERC721EnumerableUpgradeable(nftPartyMembership).balanceOf(who) == 1, "NO_MEMBERSHIP");
        return IERC721EnumerableUpgradeable(nftPartyMembership).tokenOfOwnerByIndex(who, 0);
    }

    function checkPartyMembership() internal view {
        checkPartyMembership(_msgSender());
    }

    function checkPartyMembership(address who) internal view {
        require(IERC721EnumerableUpgradeable(nftPartyMembership).balanceOf(who) == 1, "NO_MEMBERSHIP");
    }

    function ownerAddressOfPartyMembershipId(uint256 partyMembershipId) internal view returns(address) {
        return IERC721EnumerableUpgradeable(nftPartyMembership).ownerOf(partyMembershipId);
    }
}
...
/// @title MinimalPartyMembershipToMintStorage
abstract contract MinimalPartyMembershipToMintStorage is
    NFTPartyStorage
{
    uint256 public minimalPartyMembershipToMint;
    event MinimalPartyMembershipToMintSet(uint256 value);

    function __MinimalPartyMembershipToMintToMintStorage_init_unchained(uint256 minimalPartyMembershipToMintValue) internal {
        minimalPartyMembershipToMint = minimalPartyMembershipToMintValue;
    }

    /// @notice Set new "minimalPartyMembershipToMint" setting value (only contract owner may call)
    /// @param minimalPartyMembershipToMintValue new setting value
    function setMinimalPartyMembershipToMint(uint256 minimalPartyMembershipToMintValue) external onlyOwner {
        minimalPartyMembershipToMint = minimalPartyMembershipToMintValue;
        emit MinimalPartyMembershipToMintSet(minimalPartyMembershipToMintValue);
    }
}
...
/// @title WaitingDelayBetweenMintsSecondsForPartyLeaderStorage
abstract contract WaitingDelayBetweenMintsSecondsForPartyLeaderStorage is NFTPassportStorage {
    uint256 public waitingDelayBetweenMintsSecondsForPartyLeader;
    event WaitingDelayBetweenMintsSecondsForPartyLeaderSet(uint256 indexed value);

    function __WaitingDelayBetweenMintsSecondsForPartyLeaderStorage_init_unchained(uint256 waitingDelayBetweenMintsSecondsForPartyLeaderValue) internal {
        waitingDelayBetweenMintsSecondsForPartyLeader = waitingDelayBetweenMintsSecondsForPartyLeaderValue;
    }

    /// @notice Set new "waitingDelayBetweenMintsSecondsForPartyLeader" setting value (only contract owner may call)
    /// @param newValue new setting value
    function setWaitingDelayBetweenMintsSecondsForPartyLeader(uint256 newValue) public onlyOwner {
        waitingDelayBetweenMintsSecondsForPartyLeader = newValue;
        emit WaitingDelayBetweenMintsSecondsForPartyLeaderSet(newValue);
    }
}