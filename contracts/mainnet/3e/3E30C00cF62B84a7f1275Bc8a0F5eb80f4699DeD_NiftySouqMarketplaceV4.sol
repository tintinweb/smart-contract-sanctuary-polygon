...
contract NiftySouqMarketplaceV4 is Initializable {
    ...
    function lazyMintSellNft(
        ...
        require(
            lazyMintSellData_.seller != _buyer,
    ...
    function _lazyMint(
        ...
        if (isERC1155 && contractType == ContractType.NIFTY_V2) {
            ...
            if(msg.sender != _buyer){
                NiftySouqIERC1155V2(tokenAddress).transferNft(msg.sender, _buyer, tokenId_,purchaseQuantity);
            }
            ...
            if(msg.sender != _buyer) {
                NiftySouqIERC721V2(tokenAddress).transferNft(msg.sender, _buyer, tokenId_);
            }
        ...
        );
        {
            ...
            uint256 price = lazyMintSellData_.minPrice;
            uint256 quantity = purchaseQuantity;
            
            for (i = 0; i < lazyMintSellData_.revenues.length; i++) {
                uint256 revenues_ = lazyMintSellData_.revenues[i];
        ...
        returns (uint256 tokenId_, uint256 offerId_)
    {
        ...
        if (
            ...
            require(
                msg.value == lazyData_.price.add(serviceFee),
...
interface NiftySouqIFixedPrice {
    ...
    function isSalesupportedTokens(string calldata name_)
        external
        view
