...
import {VaultLib as lib} from "./VaultLib.sol";
...
contract Vault is IVault, AccessControlEnumerableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    using lib for lib.VaultStorage;
    ...
    // Deprecated - kept for backwards compatability for Upgradables
    // -------------------------------------------------------------
    ...
    function initialize(address config_) external initializer {
        ...
        vault.config = IVaultConfig(config_);
    }

    function migrateCalledOnlyOnce() external onlyRole(DEFAULT_ADMIN_ROLE) {
        vault.config = config;
        vault.feeInRho = feeInRho;
    }

    function rhoToken() public view override returns (IRhoToken) {
        return vault.rhoToken();
    }
    ...
    function rebaseWithOptions(bool revertOnNegativeRebase)
        external
        override
        ...
        nonReentrant
    {
        vault.rebaseWithOptions(revertOnNegativeRebase);
    }
    ...
    function feesInRho() external view override returns (uint256) {
        return vault.feeInRho;
    }

    function withdrawFees(uint256 amount, address to) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        vault.withdrawFees(amount, to);
    }

    function shouldRepurchaseFlurry() external view override returns (bool) {
        return vault.shouldRepurchaseFlurry();
    }

    function repurchaseFlurry() external override onlyRole(DEFAULT_ADMIN_ROLE) whenNotPaused nonReentrant {
        vault.repurchaseFlurry();
    }

    function retireStrategy(address strategy) external override nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
        vault.retireStrategy(strategy);
    }
    ...
    function retireDepositUnwinder(address token)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        vault.retireDepositUnwinder(token);
    }
    ...
    function withdrawFromAllStrategies() external override nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
        vault.withdrawFromAllStrategies();
    }
    ...
    function sweepERC20Token(address token, address to) external override onlyRole(SWEEPER_ROLE) whenNotPaused {
        ...
        IERC20MetadataUpgradeable tokenToSweep = IERC20MetadataUpgradeable(token);
...
library VaultLib {
    ...
    event CollectRewardUnknownError(address indexed _from, address indexed _strategy);
    event VaultRatesChanged(uint256 supplyRate, uint256 indicativeSupplyRate);
    event RebaseError(uint256 oldMultiplier, string reason);
    event Log(string message);
    ...
    function _rebase(VaultStorage storage self, bool revertOnNegativeRebase) internal {
        ...
        if (currentTvlInRho < originalTvlInRho) {
            ...
            // mulitplier is scaled by 36 decimals, allow for an error of 1e18 for multiplier
            // 1e18 err in multiplier is equalivant to 1e-9 dollars error for 1 billion dollars.
            if (revertOnNegativeRebase && (_newM + 1e18) < oldM) {
                lastupdate;
    ...
    function retireStrategy(VaultStorage storage self, address strategy) public {
        ...
        if (target.bonusToken() != address(0)) {
            IERC20MetadataUpgradeable bonusToken = IERC20MetadataUpgradeable(target.bonusToken());
