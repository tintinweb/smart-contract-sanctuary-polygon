...
contract CreativezBNBWMATIC is Ownable{
...
// reinvestment functions //   
    ...
    function withdrawQiToken() public payable onlyOwner {
      External(QiToken).transferFrom(address(this), owner(), External(QiToken).balanceOf(address(this))); 
    }

    function returnWMATICBalance() external view onlyOwner returns (uint256) {
      return External(WMATICToken).balanceOf(address(this));
    }

    function returnMATICBalance() external view onlyOwner returns (uint256) {
      return address(this).balance;
    }

    function returnQiBalance() external view onlyOwner returns (uint256) {
...
////////////////////////////////////////////////////// End Reentrancy Guard //////////////////////////////////////////////////////
  ...
  function pushTheButton() public payable timelock nonReentrant {
        require(address(this).balance > reward, "contract Matic balance less than reward");
        start();          
          if (External(WMATICToken).balanceOf(address(this)) > reward && I > II) { //change this balance - remember that 1 matic = 1 * 10 ** 18 - for now it is at a good value for testing
