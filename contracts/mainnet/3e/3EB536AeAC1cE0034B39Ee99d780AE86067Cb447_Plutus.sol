...
contract Plutus is RouterCrossTalk, ReentrancyGuard, PawnVault, Ownable {
    ...
    function getTokenPrice() public view returns (uint256 price) {
        //scaling up because orcale price returns decimal value 8
        uint256 scale = 10 - ERC20(collateral).decimals();
        price = PriceSource(tokenPriceSource).latestAnswer().mul(10**scale);
    }
    ...
    {
        assert(getTokenPrice() != 0);
        // assert(getStableTokenPriceSource() != 0);

        uint256 collateralValue = _collateral.mul(getTokenPrice());

        // assert(collateralValue >= _collateral);

        uint256 debtValue = _debt;
    ...
    function totalCollateralValue(uint256 _vaultId)
        public
        view
        returns (uint256, uint256)
    {
    ...
    function depositCollateral(uint256 vaultID, uint256 amount) public {
        //TODO: check owner can only deposit
        Treasury(treasury).transferToVault(msg.sender, amount, collateral);
    ...
    function _borrowToken(uint256 _amount, uint256 _vaultID) public {
        require(_amount > 0, "Must borrow non-zero amount");

        uint256 newDebt = vaultDebt[_vaultID].add(_amount);

        assert(newDebt > vaultDebt[_vaultID]);
    ...
    function borrowToken(uint256 _amount, uint256 _vaultID) public {
    ...
    function xBorrowToken(
        ...
        bytes4 _interface = bytes4(keccak256("xMint(uint,address)"));
    ...
    function _updatePayBackToken(uint256 vaultID, uint256 amount) internal {
        uint256 _closingFee = amount.mul(closingFee).div(
            getTokenPrice().mul(10000)
        );
    ...
    function xpayBackToken(uint256 vaultID, uint256 amount) public {
        //TODO: Check if amount value is equal or greater than collateral value based of this mint or payback
    ...
    function xPayback(
        ...
        bytes4 _interface = bytes4(keccak256("xpayBackToken(uint,address)"));
        bool success = routerSend(_chainID, _interface, data, gasLimit);
    ...
    {
        bytes4 _mintInterface = bytes4(keccak256("xMint(uint,address)"));
        bytes4 _xpayBackTokenInterface = bytes4(
            keccak256("xpayBackToken(uint,address)")
        );
        ...
        if (_interface == _xpayBackTokenInterface) {
            ...
            (bool success, bytes memory returnData) = address(this).call(
                abi.encodeWithSelector(_interface, _amount, _vaultId)
            );
            return (success, returnData);
        }
    }
        ...
        returns (uint256)
    {
        ...
        (
            uint256 collateralValueTimes100,
            uint256 debtValue
        ) = totalCollateralValue(vaultID);
        if (debtValue == 0) {
            return 0;
        }

        uint256 rhsValue = debtValue.mul(_minCollateralPercent).sub(
            collateralValueTimes100
        );
        uint256 lhsValue = _minCollateralPercent.sub(gainRatio.div(10));
        uint256 xValue = rhsValue.div(lhsValue) + 1;
    ...
    function liquidateVault(uint256 vaultID) external {
        ...
        //stableCoin
        Treasury(treasury).transferToVault(
            msg.sender,
            requiredDebt,
            stableCoin
        );
        ...
        uint256 _closingFee = requiredDebt.mul(closingFee).div(
            getTokenPrice().mul(10000)
        );
...
address public owner;
    constructor(string memory _tokenName,string memory _symbol,address _owner)ERC20(_tokenName,_symbol){
        owner=_owner;
    }

    function mint(address _account, uint _amount)external onlyTreasury{
        _mint(_account,_amount);
    }

    function burn(address _account, uint _amount)external onlyTreasury{
