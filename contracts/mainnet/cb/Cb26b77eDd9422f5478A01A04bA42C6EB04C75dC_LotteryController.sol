...
contract LotteryController is TimelockController, Pausable {
  ...
  uint256 private _cumulatedTotalValue = 0;
  uint256 private _totalWithdrawn = 0;
  uint256 private _lastDrawBlock = block.number;
  ...
  function draw(uint64 vrfSubscriptionId, uint32 callbackGasLimit)
      public whenNotPaused onlyRole(DRAW_ROLE)
  {
    require(block.timestamp >= lastDraw + 6 days + 23 hours);
    lastDraw = block.timestamp;
    lottery.draw(vrfSubscriptionId, callbackGasLimit);
    _cumulatedTotalValue += _totalValue * (block.number - _lastDrawBlock);
    _totalValue = address(this).balance + _totalWithdrawn;
  ...
  function getUnclaimedRevenue() public view returns (uint256) {
    uint256 cumulatedShares = token.getCumulatedBalance(msg.sender);
    uint256 cumulatedTotalSupply = token.getCumulatedTotalSupply();
    return cumulatedShares * _cumulatedTotalValue / cumulatedTotalSupply - _withdrawn[msg.sender];
  }

  function withdraw(uint256 amount) public whenNotPaused {
    uint256 unclaimed = getUnclaimedRevenue();
...
// ChainLink VRF V2 key hash (500 gwei gas lane)
bytes32 constant VRF_KEY_HASH = 0xcc294a196eeeb44da2888d17c0625cc88d70d9760a69d58d853ba6581a9ab0cd;
    ...
    PullPaymentUpgradeable, ReentrancyGuardUpgradeable
{
  ...
  function draw(uint64 vrfSubscriptionId, uint32 callbackGasLimit) public onlyOwner {
