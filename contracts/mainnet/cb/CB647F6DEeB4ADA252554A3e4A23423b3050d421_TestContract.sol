...
abstract contract WhitelistMerkle is Ownable {
    ...
    modifier onlyPremiumWhitelisted(bytes32[] calldata _merkleProof) {
        require (!_isPremiumWhitelisted(msg.sender, _merkleProof), "Invalid MerkleProof");
        _;
    }

    modifier onlyStandardWhitelisted(bytes32[] calldata _merkleProof) {
        require (!_isStandardWhitelisted(msg.sender, _merkleProof), "Invalid MerkleProof");
        _;
    }
...
contract TestContract is ERC721A, WhitelistMerkle, Pausable, ReentrancyGuard {

    uint256 public constant MAX_MINT_PER_ADDRESS = 1;
    uint256 public constant MAX_SUPPLY = 250;
    uint256 public constant DEV_MAX_SUPPLY = 60;
    uint256 public constant PREMIUM_WHITELIST_MAX_SUPPLY = 140;
    uint256 public constant STANDARD_WHITELIST_MAX_SUPPLY = 50;
    ...
    function devMint(uint256 quantity) external onlyOwner {
        require(
            mintedDevSupply + quantity <= DEV_MAX_SUPPLY,
    ...
    function premiumWhitelistMint(bytes32[] calldata _merkleProof) external onlyPremiumWhitelisted(_merkleProof) whenNotPaused {
    ...
    function standardWhitelistMint(bytes32[] calldata _merkleProof) external onlyStandardWhitelisted(_merkleProof) whenNotPaused callerIsUser {
        uint256 _standardSaleStartTime = uint256(timeConfig.standardSaleStartTime);
        ...
        require(
            mintedStandardSupply + MAX_MINT_PER_ADDRESS <= STANDARD_WHITELIST_MAX_SUPPLY,
