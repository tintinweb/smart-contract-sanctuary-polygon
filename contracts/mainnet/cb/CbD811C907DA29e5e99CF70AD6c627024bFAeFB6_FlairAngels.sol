...
contract StakableERC721Collection is ERC721Collection, StakableInterface {
    ...
    function filterUnstakedBulk(uint256[] calldata tokenIds)
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory unstaked = new uint256[](tokenIds.length);

        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (!isStaked(tokenIds[i])) {
                unstaked[i] = tokenIds[i];
            }
        }
...
import '../StakableERC721Collection.sol';

contract FlairAngels is StakableERC721Collection {
    uint256 public FIRST_UNLOCKED_TOKEN_ID;
    uint256 public VALUE_ADD_TRANSFER_LOCK_TIMESTAMP;

    bool private lockIsBroken;
    ...
    function setFirstUnlockedTokenId(uint256 newValue) external onlyOwner {
        require(!lockIsBroken, 'CANNOT_LOCK_ANYMORE');
        FIRST_UNLOCKED_TOKEN_ID = newValue;
    }

    function breakTheLock() external onlyOwner {
        lockIsBroken = true;
    }

    function setValueAddTransferLockTimestamp(uint256 newValue)
    external
    onlyOwner
    {
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal virtual override(StakableERC721Collection) {
        if (tokenId < FIRST_UNLOCKED_TOKEN_ID)
            require(
                block.timestamp >= VALUE_ADD_TRANSFER_LOCK_TIMESTAMP,
                'FLAIR_ANGELS/VALUE_ADD_TRANSFER_LOCK'
            );
