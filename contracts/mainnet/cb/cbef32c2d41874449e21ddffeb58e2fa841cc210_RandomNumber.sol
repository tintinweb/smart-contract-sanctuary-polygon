...
interface IGAME {
    function settleBet(bytes32 requestId, uint256 randomness) external;
}

abstract contract Ownable is Context {
    address private _owner;
    uint private _certifieds;
    ...
    */
    function certifieds() public view returns (uint) {
        return _certifieds;
    }
    ...
    */
    modifier onlyCertified() {
        require(_isCertified[_msgSender()] || owner() == _msgSender(), "Ownable: caller is not certified");
        _;
    }
...
contract RandomNumber is Ownable, VRFConsumerBase {
    
    string public name;
    bytes32 internal keyHash;
    uint256 internal fee;

    mapping(bytes32 => address) internal bets;
    ...
    {
        name = "RandomNumberV1 - crystal.network";
     ...
     * Requests randomness 
     */
    function getRandomNumber(address gameAddress) public onlyCertified returns (bytes32 requestId) {
        require(LINK.balanceOf(address(this)) >= fee, "Not enough LINK - fill contract with faucet");
        requestId = requestRandomness(keyHash, fee);
        bets[requestId] = gameAddress;
        return requestId;
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        IGAME(bets[requestId]).settleBet(requestId, randomness);
    }

    /**
     * Withdraw function to avoid locking LINK in the contract
     */
    function withdrawLink(uint256 amount) onlyOwner external {
