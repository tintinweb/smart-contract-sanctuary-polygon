...
contract SwapV3Helper is SafeOwnable, Operatable, Initializable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    event NewRouter(ISwapRouter oldRouter, ISwapRouter newRouter);
    ...
    function initialize(address _vault, ISwapRouter _router, address _receiver) public initializer {
        _transferOwnership(msg.sender);
        vault = _vault;
        require(address(_router) != address(0), "router is zero");
        router = _router;
        receiver = _receiver;
    }

    function setRouter(ISwapRouter _router) external onlyOwner {
        require(address(_router) != address(0), "vault is zero");
    ...
    function setReceiver(address _receiver) external onlyOwner {
        require(address(_receiver) != address(0), "receiver is zero");
        emit NewReceiver(receiver, _receiver);
    }

    function exactOutputSingle(ISwapRouter.ExactOutputSingleParams calldata params) external onlyOperator returns (uint amountIn) {
        IERC20Upgradeable(params.tokenIn).safeTransferFrom(vault, address(this), params.amountInMaximum);
        IERC20Upgradeable(params.tokenIn).approve(address(router), params.amountInMaximum);
        amountIn = router.exactOutputSingle(params);
        uint remain = IERC20Upgradeable(params.tokenIn).balanceOf(address(this));
        if (remain > 0) {
            IERC20Upgradeable(params.tokenIn).safeTransfer(receiver, remain);
        }
    }

    function recoverWrongToken(IERC20Upgradeable token, address to) public onlyOwner {
