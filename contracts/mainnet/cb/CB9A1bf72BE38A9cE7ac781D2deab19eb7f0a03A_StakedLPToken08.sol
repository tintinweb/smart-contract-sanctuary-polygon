...
// File: contracts\interfaces\IStakingRewards08.sol
...
// File: contracts\StakedLPToken08.sol
...
contract StakedLPToken08 is IUniswapV2Pair, PoolToken {
    using SafeToken for address;
	
	bool public constant isStakedLPToken = true;
	string public constant stakedLPTokenType = "08";
	...
	function _initialize(
		...
		require(factory == address(0), "StakedLPToken08: FACTORY_ALREADY_SET"); // sufficient check
		factory = msg.sender;
	...
	function _convertSharesToAmount(uint amount) internal view returns (uint shares) {
		if (IStakingRewards08(stakingRewards).totalSupply() == 0) return amount;
		return amount
			.mul(IStakingRewards08(stakingRewards).underlyingBalanceWithInvestment())
			.div(IStakingRewards08(stakingRewards).totalSupply());
	}
	
	function _convertAmountToShares(uint shares) internal view returns (uint amount) {
		if (IStakingRewards08(stakingRewards).totalSupply() == 0) return shares;
		return shares
			.mul(IStakingRewards08(stakingRewards).totalSupply())
			.div(IStakingRewards08(stakingRewards).underlyingBalanceWithInvestment());
	}
	
	function _update() internal {
		totalBalance = _convertSharesToAmount(IStakingRewards08(stakingRewards).balanceOf(address(this)));
		emit Sync(totalBalance);
	}
	...
	function mint(address minter) external nonReentrant update returns (uint mintTokens) {
		...
		IStakingRewards08(stakingRewards).deposit(mintAmount);
		mintTokens = mintAmount.mul(1e18).div(exchangeRate());
		...
		require(mintTokens > 0, "StakedLPToken08: MINT_AMOUNT_ZERO");
		_mint(minter, mintTokens);
		emit Mint(msg.sender, minter, mintAmount, mintTokens);
	}
	...
	function redeem(address redeemer) external nonReentrant update returns (uint redeemAmount) {
		...
		require(redeemAmount > 0, "StakedLPToken08: REDEEM_AMOUNT_ZERO");
		require(redeemAmount <= totalBalance, "StakedLPToken08: INSUFFICIENT_CASH");
		_burn(address(this), redeemTokens);
		IStakingRewards08(stakingRewards).withdraw(_convertAmountToShares(redeemAmount));
		redeemAmount = underlying.myBalance();
		_safeTransfer(redeemer, redeemAmount);
		emit Redeem(msg.sender, redeemer, redeemAmount, redeemTokens);		
	}
	...
	function _getReward() internal returns (uint256) {
		IStakingRewards08(stakingRewards).getReward(rewardsToken);
	...
	function reinvest() external nonReentrant update {
		...
		if (token0 == rewardsToken || token1 == rewardsToken) {
			...
			swapExactTokensForTokens(rewardsToken, token0, reward.sub(bounty));
		...
		IStakingRewards08(stakingRewards).deposit(liquidity);
	...
	function getReserves() external view returns (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) {
		...
		require(reserve0 > 100 && reserve1 > 100, "StakedLPToken08: INSUFFICIENT_RESERVES");
	}
    ...
    function safe112(uint n) internal pure returns (uint112) {
        require(n < 2**112, "StakedLPToken08: SAFE112");
        return uint112(n);
    }
}

// File: contracts\StakedLPTokenFactory08.sol
...
contract StakedLPTokenFactory08 {
	address public router;
	address public WETH;
	...
	function createStakedLPToken(address stakingRewards) external returns (address payable stakedLPToken) {
		...
		address pair = IStakingRewards08(stakingRewards).underlying();
		address rewardsToken = IStakingRewards08(stakingRewards).rewardTokens()[0];
		address token0 = IUniswapV2Pair(pair).token0();
		address token1 = IUniswapV2Pair(pair).token1();
		bytes memory bytecode = type(StakedLPToken08).creationCode;
		assembly {
			stakedLPToken := create2(0, add(bytecode, 32), mload(bytecode), stakingRewards)
		}
		StakedLPToken08(stakedLPToken)._initialize(stakingRewards, pair, rewardsToken, token0, token1, router, WETH);
