...
contract StrategBlockRegistry is Ownable, IStrategyBlockRegistry {

    bool public immutable degenMode;
    uint256 public blocksLength;
    mapping(address => StrategBlockData) public blocks; 

    constructor(bool _degenMode) {
        degenMode = _degenMode;
    }
    ...
    function addBlocks(address[] memory _blocks, string[] memory _names, string[] memory _ipfsHash) external {
        ...
        for (uint i = 0; i < _blocks.length; i++) {
            ...
            blocks[_blocks[i]] = StrategBlockData({
                enabled: true,
                deployer: msg.sender,
                name: _names[i],
                ipfsHash: _ipfsHash[i]
            });
    ...
    function removeBlocks(address[] memory _blocks) external {
        ...
        for (uint i = 0; i < _blocks.length; i++) {
            ...
            if(dMode)
                if(msg.sender != b.deployer) revert NotDeployer();
    ...
    function getBlocks(address[] memory _blocks) external view returns (StrategBlockData[] memory) {
        StrategBlockData[] memory blocksArray = new StrategBlockData[](_blocks.length);
