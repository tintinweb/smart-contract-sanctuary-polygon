...
contract AkiPDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("AkiP_Dividend_Tracker", "AkiP_Dividend_Tracker", 9) {
    	claimWait = 14400;
        minimumTokenBalanceForDividends = 1000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "AkiP_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "AkiP_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AkiP contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "AkiP_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "AkiP_Dividend_Tracker: Cannot update claimWait to same value");
...
contract AkiP is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    AkiPDividendTracker public dividendTracker;
    ...
    // sells have fees of 8 and 4 (5 * 1.5 and 3 * 1.5)
    ...
    constructor() public BEP20("AnunnakiMATIC", "AKIP", 9) {
    	...
    	dividendTracker = new AkiPDividendTracker();

    	liquidityWallet = owner();
        
    	IPancakeswapV2Router02 _pancakeswapV2Router = IPancakeswapV2Router02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "AkiP: The dividend tracker already has that address");

        AkiPDividendTracker newDividendTracker = AkiPDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "AkiP: The new dividend tracker must be owned by the AkiP token contract");
    ...
    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "AkiP: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "AkiP: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "AkiP: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "AkiP: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "AkiP: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "AkiP: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "AkiP: Cannot update gasForProcessing to same value");
