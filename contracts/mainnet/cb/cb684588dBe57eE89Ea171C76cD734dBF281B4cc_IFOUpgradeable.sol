...
interface IWONE is IERC20Upgradeable {
	function deposit() external payable;

	function withdraw(uint256) external;
}

interface ITicketKeeper {
	function hasTicket(address buyer) external view returns (bool);
}
...
contract IFOUpgradeable is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
	using SafeMathUpgradeable for uint256;
	using SafeERC20Upgradeable for IWONE;
	...
	// address => whether claimed their share
	...
	// address of private pool record keeper
	ITicketKeeper public ticketKeeper;

	// wrapped native token address
	IWONE public WONE;

	function initialize(IWONE _WONE, ITicketKeeper _ticketKeeper, IWrapperToNative _nativeConvertor) public initializer {
		__Ownable_init();
		__ReentrancyGuard_init();

		WONE = _WONE;
		ticketKeeper = _ticketKeeper;
	...
	/// @dev To make the pool support NATIVE i.e. ONE deposit, specify the {_depositToken} as {WONE} address, make sure {WONE} address in contract is correct
	...
	/// @param _privatePool Boolean denoting if it is a private pool & require the buyer to own a ticket
	function updateDetails(
		...
		runningTime = 60 seconds;
		// runningTime = 10 minutes; // for testing
		refundThreshold = 8000; // 80%

		WONE.approve(address(nativeConvertor), type(uint256).max);
	}
	...
	/// @dev call this function after converting ONE to WONE for pools that supports native deposit
	function deposit(uint256 _amount) public nonReentrant {
		...
		require(!privatePool, "Not authorized for private sale");
	...
	function harvest() external nonReentrant {
		...
		require(!harvested[_msgSender()], "Already harvested");
		require(harvestEnabled, "Harvesting not enabled");

		uint256 offeringTokenAmount = getOfferingAmount(_msgSender());

		if (offeringTokenAmount > 0) {
			offeringToken.safeTransfer(_msgSender(), offeringTokenAmount);
		}

		harvested[_msgSender()] = true;
		emit Harvest(_msgSender(), offeringTokenAmount);
	}
	...
	function getGeneratedReward(uint256 _fromTime, uint256 _toTime) public view returns (uint256) {
		...
		return _toTime.sub(_fromTime).mul(_totalDepositAmount().div(runningTime));
	}
	...
	function timelyWithdraw() external {
		...
		if (!sentToEscrow) {
			...
			if (_offeringTokenDistributionAmount() < offeringAmount) {
				offeringToken.safeTransfer(receiverAddress, offeringAmount.sub(_offeringTokenDistributionAmount()));
			}
	...
	function setHarvestEnabled(bool _harvestEnabled) external onlyOwner {
		assert(harvestEnabled != _harvestEnabled);
		harvestEnabled = _harvestEnabled;
	}

	function setWONEAddress(IWONE _WONE) external onlyOwner {
		WONE = _WONE;
		_WONE.approve(address(nativeConvertor), type(uint256).max);
	}
	...
	function setTicketKeeper(ITicketKeeper _newTicketKeeper) external onlyOwner {
		ticketKeeper = _newTicketKeeper;
	}

	function setNativeConvertor(IWrapperToNative _nativeConvertor) external onlyOwner {
		nativeConvertor = _nativeConvertor;
		WONE.approve(address(_nativeConvertor), type(uint256).max);
	}
	...
	function haltIFO(address payable sendTo) external onlyOwner {
		uint256 woneBalance = WONE.balanceOf(address(this));
		if (woneBalance > 0) WONE.safeTransfer(sendTo, woneBalance);
		...
		require(success, "Failed sending ONE to given address");
	}

	// Internal functions & Fallback

	function _offeringTokenDistributionAmount() internal view returns (uint256 distributionAmount) {
		address[] memory _usersList = usersList;
		for (uint256 i; i < _usersList.length; i++) {
			distributionAmount = distributionAmount.add(getOfferingAmount(_usersList[i]));
		}
	}

	function _updateExcessAmount() internal {
		address[] memory _usersList = usersList;
	...
	receive() external payable {
		// accidentalSent[_msgSender()] = accidentalSent[_msgSender()].add(msg.value);

		require(msg.value > 0, "No amount sent");
		require(address(depositToken) == address(WONE), "Cannot send to a NON NATIVE pool");

		WONE.deposit{value: msg.value}();

		// send amount back to sender
		require(WONE.transfer(_msgSender(), msg.value), "Cannot send WONE to user");
		deposit(msg.value);
	}
}