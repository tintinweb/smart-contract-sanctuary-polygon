...
contract EmoteMinter is AccessControl, EIP712 {
    ...
    function mint(MintRequest calldata request) external {
        ...
        // ensure nonce wasn't used already
        Counters.Counter storage nonce = _nonces[signer];
        require(nonce.current() == request.nonce, "Nonce already used");
        nonce.increment();
        // mint Emotes
        emote.mint(request.to, request.tokenId, request.quantity, "");
    }

    function nonces(address signer) public view virtual returns (uint256) {
