...
import {GameNFT, IGameNFT} from "./tokens/gamenft/GameNFT.sol";
import {IGameItems} from "./tokens/gameitems/IGameItems.sol";

import {ILootSystem} from "./loot/ILootSystem.sol";
import {IHoldingSystem, ID} from "./interfaces/IHoldingSystem.sol";
...
contract HoldingSystem is GameRegistryConsumerUpgradeable, IHoldingSystem {
    ...
    function _claimMilestone(
        ...
        if (account != IGameNFT(tokenContract).ownerOf(tokenId)) {
    ...
    function _timeHeld(
        ...
        uint32 lastTransfer = GameNFT(tokenContract).lastTransfer(tokenId);
