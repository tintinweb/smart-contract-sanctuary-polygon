...
contract AOPaymaster is BasePaymaster {

    IAONFT public nftContract;
    ...
    returns (bytes memory context, bool revertOnRecipientRevert) {
        ...
        require(relayRequest.request.to == address(nftContract), "Target must be an assigned contract");

        address account = address(uint160(GsnUtils.getParam(relayRequest.request.data, 0)));
        uint256 id = GsnUtils.getParam(relayRequest.request.data, 1);
        require(nftContract.balanceOf(account, id) < 1, "User already owns an NFT");
