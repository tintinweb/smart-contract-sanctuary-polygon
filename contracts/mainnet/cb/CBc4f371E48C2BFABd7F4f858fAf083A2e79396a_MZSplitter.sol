...
contract MZSplitter is Admin {
     ...
     */
    constructor(address moderator, address metaMarkDeployed_) {
        // Sender is the owner
        _setOwner(tx.origin);
        // Allow creator to specify a moderator
        _setModerator(moderator);

        // Initialize Meta Mark address
    ...
    function supportToken(string memory tokenSymbol, address tokenDeployed) public onlyAdmins {
        require(_supportedTokens[tokenSymbol].deployed == address(0), "Token already supported");

        // Add new supported token with contract address
        ERC20Token storage newToken = _supportedTokens[tokenSymbol];
        newToken.symbol = tokenSymbol;
        newToken.deployed = tokenDeployed;
    ...
    function setAffiliateMax(uint8 max) public onlyAdmins {
        require(max >= 0, "Must be above or equal to 0");
        _affiliateCutMax = max;
    }

    function depositERC20(string memory tokenSymbol, uint256 value, string memory sku, address splitAddress) public {
        ...
        _splitAmount(tokenSymbol, value, splitAddress);
    }

    function splitAmount(string memory tokenSymbol, uint256 value, address splitAddress) public onlyAdmins {
      // Split the ERC20 payout
      _splitAmount(tokenSymbol, value, splitAddress);
    }
    function _splitAmount(string memory tokenSymbol, uint256 value, address splitAddress) internal {
        require(_supportedTokens[tokenSymbol].deployed != address(0), "Token not supported");
        require(value > 0, "Must provide value amount");

        // Retrieve MZSplit data contract
        MZSplit mzSplit = MZSplit(splitAddress);
        uint8 _sumCuts = uint8(mzSplit._sumCuts());

        // Retrieve MZMark contract
        IERC721 metaMarks = IERC721(_metaMarkDeployed);

        // Track used up value of cuts
        uint256 valueUsed = 0;
        // Loop through all cuts
        for(uint8 i=0; i<uint8(mzSplit._splitCount()); i++) {
            // Calculate recipient portion: value * (cut / 100)
            uint256 valueCut = value.mul(mzSplit.getCut(i)).div(_sumCuts);
            // Increment recipient balance
            _supportedTokens[tokenSymbol].balances[address(metaMarks.ownerOf(mzSplit.getRecipient(i)))] += valueCut;
            // Track used up value
            valueUsed += valueCut;
        }

        // Add any remaining unused value to first balance
        _supportedTokens[tokenSymbol].balances[address(metaMarks.ownerOf(mzSplit.getRecipient(0)))] += value - valueUsed;

        // Fire deposit event
        emit Deposit(tx.origin, tokenSymbol, value);

        // Send balances payouts
        withdrawBalances(tokenSymbol, splitAddress);
    }

    function withdrawBalances(string memory tokenSymbol, address splitAddress) public {
        require(_supportedTokens[tokenSymbol].deployed != address(0), "Token not supported");
        ...
        // Retrieve MZSplit data contract
        MZSplit mzSplit = MZSplit(splitAddress);
        uint8 _splitCount = uint8(mzSplit._splitCount());

        // Loop through all recipients
        for(uint8 i=0; i<_splitCount; i++) {
            // Retrieve recipient address
            address owner = address(metaMarks.ownerOf(mzSplit.getRecipient(i)));
            // Retrieve recipient balance
            uint256 balance = _supportedTokens[tokenSymbol].balances[owner];
            // Make sure has a balance
            if(balance > 0) {
                // Retrieve recipient payable address
                ...
                // Fire withdraw event
                emit Withdraw(recipient, tokenSymbol, balance);
                // Clear balance
                _supportedTokens[tokenSymbol].balances[owner] = 0;
            }
        }
    }

    function depositWithAffiliate(string memory tokenSymbol, uint256 value, string memory sku, address splitAddress, address payable affiliate, uint256 affiliateCut) public payable {
        require(_supportedTokens[tokenSymbol].deployed != address(0), "Token not supported");
        require(value > 0, "No amount provided");
        ...
        for(uint8 i=0; i<uint8(mzSplit._splitCount()); i++) {
            // Retrieve recipient address
            address recipient = address(metaMarks.ownerOf(mzSplit.getRecipient(i)));
            // Calculate recipient portion minus affiliate portion: value * (cut * 100 - (affiliateCut * 100 / splitCount)) / 100 * 100
            uint256 valueCut = value.mul(uint256(mzSplit.getCut(i)).mul(100).sub(affiliateCut.mul(100).div(uint8(mzSplit._splitCount())))).div(10000);

            // Increment recipient balance
            _supportedTokens[tokenSymbol].balances[recipient] += valueCut;

            // Fire deposit event
            emit Deposit(recipient, tokenSymbol, valueCut);
        ...
        // Increment recipient balance
        _supportedTokens[tokenSymbol].balances[affiliate] += affiliateBalance;
        // Fire deposit event
        emit Deposit(affiliate, tokenSymbol, affiliateBalance);
        ...
        for(uint8 i=0; i<uint8(mzSplit._splitCount()); i++) {
            ...
            address recipient = address(metaMarks.ownerOf(mzSplit.getRecipient(i)));
            // Retrieve recipient balance
            uint256 balance = _supportedTokens[tokenSymbol].balances[recipient];
            ...
            if(balance > 0) {
                ...
                // Clear balance
                _supportedTokens[tokenSymbol].balances[recipient] = 0;
            }
        }
        ...
        // Clear affiliate balance
        _supportedTokens[tokenSymbol].balances[affiliate] = 0;
    }

    function getBalance(string memory tokenSymbol, address recipient) public view returns(uint256) {
        return _supportedTokens[tokenSymbol].balances[recipient];
    }
    function getContractBalance(string memory tokenSymbol) public view returns(uint256) {
        require(_supportedTokens[tokenSymbol].deployed != address(0), "Token not supported");
     ...
     * @param recipient        Address of the receiving party
     * @param newBalance       Amount to replace the balance
     */
    function setBalance(string memory tokenSymbol, address recipient, uint256 newBalance) public onlyAdmins {
        require(_supportedTokens[tokenSymbol].deployed != address(0), "Token not supported");
        require(newBalance >= 0, "Invalid amount");

        // Update MetaMark token id to the new id
        _supportedTokens[tokenSymbol].balances[recipient] = newBalance;
    }
