...
contract NOVACREEDITEMSBASE is ERC1155Supply, Controllable {
  ...
  function burnFrom(address user, uint _id, uint _amount) external onlyController {
    _burn(user,_id,_amount);
  }
  function burnBatch(uint[] memory _ids, uint[] memory _amounts) external onlyController {
    _burnBatch(msg.sender, _ids, _amounts);
  }
  function burnFromBatch(address user, uint[] memory _ids, uint[] memory _amounts) external onlyController {
    _burnBatch(user, _ids, _amounts);
  }
