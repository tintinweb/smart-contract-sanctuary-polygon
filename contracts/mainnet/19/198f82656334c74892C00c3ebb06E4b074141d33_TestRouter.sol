...
contract TestRouter is Ownable {
    /// @dev constructs the inherited call structure.
    struct TestStruct {
        address inputToken;
        address bridge;
        bytes adapterData;
    ...
    function outBoundTransfer(TestStruct[] calldata userRequests)
        external
        virtual
    {   
        for (uint256 i = 0; i < userRequests.length; i++) {
            require(
                IERC20(userRequests[i].inputToken).allowance(msg.sender, address(this)) >= 
                userRequests[i].socketInfo.amount
            );
            IERC20(userRequests[i].inputToken).transferFrom(msg.sender, address(this),userRequests[i].socketInfo.amount);
            IERC20(userRequests[i].inputToken).approve(userRequests[i].bridge, userRequests[i].socketInfo.amount);
            socketsRegistry.outboundTransferTo(userRequests[i].socketInfo);
        }
    }
}
