...
contract GotchiSitter is TokenRecover {
    ...
    function whenNextDepositIsRequiredFor(address _parent) public view returns (uint256){
        uint256 totalChildren = countChildrenOf(_parent);

        require(totalChildren > 0, "No funds required cause you don't own any gotchi!");

        uint256 feePerDay = totalChildren * pricePerPetPerDay;
        uint256 enoughForDays = Math.ceilDiv( parent_balance[_parent], feePerDay ) * 1 days;

        return block.timestamp + enoughForDays;
    }
