...
contract Claim is AccessControl {
    ...
    function claim(
        ...
        if (claimSettingsMemory.permission == CLAIM_PERMISSION.TokenGated) {
            ...
            require(_adr == msg.sender, "Not Allowed");
        ...
        if (claimSettingsMemory.cooldownTime != 0) {
            require(PendingClaimDetails[msg.sender].length < 20, "Max Reached");
    ...
    function claimAllPending(address _receiver) external {
        ...
        for (uint256 i = 0; i < pendingClaims.length; ) {
        ...
        PendingClaimDetails[msg.sender] = pendingClaims;
    ...
    function depositTokens(uint256 _amount) external onlyRole(MODERATOR) {
        require(!claimSettings.hasAllowanceMechanism);
        IERC20(claimSettings.airdropToken).safeTransferFrom(
            msg.sender,
            address(this),
            _amount
