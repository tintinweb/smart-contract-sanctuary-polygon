...
contract MircaleMatic {
  ...
  function initialize(address payable _admin, address payable _admin2,address payable _creator) public {
        ...
        PROJECT_FEE = 10; 
        levelIncomeLength = 100;
        defaultPackages = [50 ether,200 ether,400 ether,800 ether,1600 ether];
    ...
    function changeLevellength(uint length) public {
        require(msg.sender==creatorAddress,"no access");
        levelIncomeLength = length;
    }

    function invest(address referrer,uint index) public payable {
        require(index < defaultPackages.length, "wrong package");
		require(msg.value>=defaultPackages[index],"wrong input value");
    ...
    function levelIncome(address _user, uint _amount) internal {
        ...
        for(uint i=0; i < levelIncomeLength; i++){
            if(upline != address(0)){
