...
contract AaveFlashloan is FlashLoanSimpleReceiverBase {
    ...
    constructor(IPoolAddressesProvider _provider,IUniswapV2Router02 _router, address[] memory _ethMarkets, address[] memory _baseTokens) FlashLoanSimpleReceiverBase(_provider){
        owner = msg.sender;
    ...
    function liquidate(address borrower, address collateralMarket, address borrowMarket, uint repayAmount) external {
        ...
        {
            IComptroller comptroller = IComptroller(IToken(collateralMarket).comptroller());
            (,,uint shortfall) = comptroller.getAccountLiquidity(borrower);
            require(shortfall > 0, "Cannot be liquidated");
        }

        if (repayAmount == 0) { // 传0进来的计算
        ...
        bytes memory data = abi.encode(
            ...
            collateralToken, 
            borrowToken, 
            _isEthMarket(collateralMarket), 
            _isEthMarket(borrowMarket)
        );
    ...
    function executeOperation(
        ...
        _do(amount, premium, params);
        
        _approveInternal(asset, address(POOL),  amount + premium);

        return true;
    }

    function _do(uint256 amount, uint256 fee, bytes calldata params) internal{
      ...
      (address borrower, 
        ...
        bool isEthBorrowMarket) = abi.decode(params, (address, address, address, address, address, bool, bool));

      if (isEthBorrowMarket) {
          IWETH(WETH).withdraw(amount);
          IEthToken(borrowMarket).liquidateBorrow{value : amount}(borrower, collateralMarket);
      }else{
          _approveInternal(IErcToken(borrowMarket).underlying(), borrowMarket, amount);    
          IErcToken(borrowMarket).liquidateBorrow(borrower, amount, collateralMarket);
      }
      ...
      (uint amountOut, address[] memory bestPath) = _getEstimateOut(collateralToken, borrowToken, seizedAmount);
      require(amountOut > amount+fee,"less amountOut");
    ...
    function _getEstimateOut(address tokenIn, address tokenOut, uint256 amountIn) internal view returns (uint256, address[] memory){
    ...
    function _accrueInterest(address borrower,address _borrowMarket) internal{
        ...
        for(uint i = 0; i < assertsIn.length; i++){
            IToken market = IToken(assertsIn[i]);
