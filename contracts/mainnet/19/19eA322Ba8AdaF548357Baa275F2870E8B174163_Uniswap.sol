...
contract Uniswap is Ownable, AccessControl, Pausable, ReentrancyGuard {
    ...
    mapping (address => mapping(address => uint256)) public swapPairs;
    ...
    function addSwapPair(uint256 tokenID, address token0, address token1, uint24 poolFee) public onlyOwner {
        swapPairs[token0][token1] = tokenID;
        swapPairs[token1][token0] = tokenID;
        pairs[tokenID] = swapPair(tokenID,token0,token1,poolFee);
    }

    function removeSwapPair(address token0, address token1) public onlyOwner {
        swapPairs[token0][token1] = 0;
        swapPairs[token1][token0] = 0;
    }

    function swapByIn(address tokenIn, address tokenOut ,uint256 amountIn) external whenNotPaused nonReentrant returns(uint256 amountOut) {
        require(hasRole(MANAGER,_msgSender()),"permission denied");
        require(swapPairs[tokenIn][tokenOut] > 0,"swap pair not support");
        ...
        ISwapRouter.ExactInputSingleParams memory params =
            ISwapRouter.ExactInputSingleParams({
                tokenIn: tokenIn,
                tokenOut: tokenOut,
                fee: pairs[swapPairs[tokenIn][tokenOut]].poolFee,
    ...
    function swapByOut(address tokenIn, address tokenOut ,uint256 amountOut, uint256 amountInMaximum) external whenNotPaused nonReentrant returns(uint256 amountIn) {
        require(hasRole(MANAGER,_msgSender()),"permission denied");
        require(swapPairs[tokenIn][tokenOut] > 0,"swap pair not support");
    ...
    function increaseLiquidityCurrentRange(
        ...
        require(hasRole(MANAGER,_msgSender()),"permission denied");
        require(swapPairs[token0][token1] > 0,"swap pair not support");
        uint256 tokenId = swapPairs[token0][token1];

        TransferHelper.safeTransferFrom(token0, _msgSender(), address(this), amountAdd0);
        TransferHelper.safeTransferFrom(token1, _msgSender(), address(this), amountAdd1);
    ...
    function _transferOut(address token) private {
