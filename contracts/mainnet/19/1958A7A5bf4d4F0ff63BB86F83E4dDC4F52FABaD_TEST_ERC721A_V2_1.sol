...
//Created By Sueun Cho (Profile NFT)
...
contract TEST_ERC721A_V2_1 is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public publicRoll = 0 ether;

    string private _baseTokenURI;

    bool public PublicIsSale = false;

    constructor(string memory baseTokenURI) ERC721A("ECO", "ECO_Profile") {
        _baseTokenURI = baseTokenURI;
    }

    function teamMint(uint256 quantity) external payable onlyOwner {
        require(totalSupply() + quantity <= TEAM_SUPPLY, "Team Supply Only 10");
        _safeMint(msg.sender, quantity);
    }

    function mintByETH(uint256 quantity) external payable nonReentrant{
        require(PublicIsSale, "Not Start");
        ...
        require(quantity <= maxPurchase, "Can only mint 1 amount  NFT at a time");
