...
import '../interfaces/IUniversePairVaultV2.sol';

contract UniversePairVaultConfig is IUniversePairVaultConfig, Ownable {

    // operate address
    address operator;
    /// @dev vaultAddress => pairVaultParam
    mapping(address => PairVaultParam) private pairVaultParamMap;
    /// @dev vaultAddress => Incentives
    mapping(address => IUniswapV3Staker.IncentiveKey[]) private incentiveListMap;

    PairVaultParam defaultParam = PairVaultParam({
        performanceFee : 10,
        diffTick : 300
    });
    ...
    /// @inheritdoc IUniversePairVaultConfig
    ...
    function changePairVaultParam(
        ...
        PairVaultParam memory param = pairVaultParamMap[_vaultAddress];
        param.performanceFee = _performanceFee;
        param.diffTick = _diffTick;

        pairVaultParamMap[_vaultAddress] = param;
    }
    ...
    function addIncentive(address vaultAddress, IUniswapV3Staker.IncentiveKey memory key) external override onlyManager{
        ...
        if(block.timestamp >= key.startTime){
            IUniversePairVaultV2(vaultAddress).nftStake(key);
        }
    ...
    function removeIncentive(address vaultAddress, IUniswapV3Staker.IncentiveKey memory key) external override onlyManager{
        ...
        for (uint i = 0; i < length; i++){
            ...
            if(incentiveId == IncentiveId.compute(key)){
                IUniversePairVaultV2(vaultAddress).nftUnStake(key);
    ...
    /// @inheritdoc IUniversePairVaultConfig
    function pairVaultParam(address vaultAddress) external view override returns(PairVaultParam memory param){
        param = _pairVaultParam(vaultAddress);
    }

    /// @inheritdoc IUniversePairVaultConfig
    function performanceFee() external view override returns(uint8){
        PairVaultParam memory param =  _pairVaultParam(msg.sender);
        return param.performanceFee;
    }

    /// @inheritdoc IUniversePairVaultConfig
    function diffTick() external view override returns(uint24){
        PairVaultParam memory param = _pairVaultParam(msg.sender);
        return param.diffTick;
    }
    ...
    function _pairVaultParam(address vaultAddress) internal view returns(PairVaultParam memory param){
        param =  pairVaultParamMap[vaultAddress];
        if(param.diffTick == 0){
            param = defaultParam;
        }
    }
...
import "./IPairVaultOperatorActionsV2.sol";
...
interface IUniversePairVaultV2 is IERC20, IVaultOwnerActions, IPairVaultOperatorActionsV2, IVaultEvents{
    ...
    /// Returns positionTick The last rebalance tick,
    /// Returns liquidity The NFT's liquidity
    ...
    function setUTokens(address _uToken) external;
