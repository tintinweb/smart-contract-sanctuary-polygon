...
// File: contracts/PolyGoblinTownWTFsol
...
contract PolyGoblinTownWTF is ERC721r, Ownable {
  using Strings for uint256;
  ...
  uint256 public nftPerFreeLimit = 10;
  uint256 public freeMax = 1000;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (supply >= freeMax) {
        ...
        require(ownerMintedCount + _mintAmount <= nftPerFreeLimit, "max NFT per address exceeded");
        require(supply + _mintAmount <= freeMax, "max NFT for free mint session");
    }
    }
  ...
  function setNftPerFreeLimit(uint256 _limit) public onlyOwner {
    nftPerFreeLimit = _limit;
  }

  function setFreeMax(uint256 _supply) public onlyOwner {
    freeMax = _supply;
  }
  ...
  function mintToAddress(uint256 _mintAmount, address _to) public onlyOwner {
    require(!paused, "the contract is paused");
    uint256 supply = totalSupply();
    require(_mintAmount > 0, "need to mint at least 1 NFT");
    require(supply + _mintAmount <= _maxSupply, "max NFT limit exceeded");
