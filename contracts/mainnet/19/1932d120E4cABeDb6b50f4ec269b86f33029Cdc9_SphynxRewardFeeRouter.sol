...
contract SphynxRewardFeeRouter is ISphynxRouter {
    ...
    uint256 swapFee = 15; // 0.15% swap fee default
    ...
    function swapExactTokensForTokens(
        ...
        amounts = SphynxLibrary.getAmountsOut(factory, amountIn * (10000 - swapFee) / 10000, path); // 0.15% swap fee
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, ISphynxFactory(factory).feeTo(), amounts[0] * swapFee / 10000); // 0.15% default swap fee(withdraw at once)
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] * (10000 + swapFee) / 10000 <= amountInMax, 'SphynxRouter: EXCESSIVE_INPUT_AMOUNT'); // 0.15% default swap fee check
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, SphynxLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        TransferHelper.safeTransferFrom(path[0], msg.sender, ISphynxFactory(factory).feeTo(), amounts[0] * swapFee / 10000); // 0.15% default swap fee
        _swap(amounts, path, to);
    }
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = SphynxLibrary.getAmountsOut(factory, msg.value * (10000 - swapFee) / 10000, path); // 0.15% swap fee default
        require(amounts[amounts.length - 1] >= amountOutMin, 'SphynxRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(SphynxLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        TransferHelper.safeTransferETH(ISphynxFactory(factory).feeTo(), msg.value * swapFee / 10000); // 0.15% swap fee default(withdraw at once)
    }
        ...
        returns (uint[] memory amounts)
    {
        ...
        TransferHelper.safeTransferETH(to, amounts[amounts.length - 1] * (10000 - swapFee) / 10000); // 0.15% swap fee default
        TransferHelper.safeTransferETH(ISphynxFactory(factory).feeTo(), amounts[amounts.length - 1] * swapFee / 10000); // 0.15% swap fee default(withdraw at once)
    }
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'SphynxRouter: INVALID_PATH');
        amounts = SphynxLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] * (10000 + swapFee) / 10000 <= msg.value, 'SphynxRouter: EXCESSIVE_INPUT_AMOUNT'); // swap fee check
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(SphynxLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        TransferHelper.safeTransferETH(ISphynxFactory(factory).feeTo(), amounts[0] * swapFee / 10000); // 0.15% swap fee(withdraw at once)
        // refund dust eth, if any
        if (msg.value > amounts[0] * (10000 + swapFee) / 10000) TransferHelper.safeTransferETH(msg.sender, msg.value - amounts[0] * (10000 + swapFee) / 10000); // 0.15% swap fee
    }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, SphynxLibrary.pairFor(factory, path[0], path[1]), amountIn * (10000 - swapFee) / 10000
        );
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ISphynxFactory(factory).feeTo(), amountIn * swapFee / 10000
        ); // 0.15% default swap fee(withdraw at once)
    }
    ...
    {
        require(path[0] == WETH, 'SphynxRouter: INVALID_PATH');
        uint amountIn = msg.value * (10000 - swapFee) / 10000; //0.15% swap fee default
        ...
        ensure(deadline)
    {
        ...
        TransferHelper.safeTransferETH(to, amountOut * (10000 - swapFee) / 10000); //0.15% swap fee default
        TransferHelper.safeTransferETH(ISphynxFactory(factory).feeTo(), amountOut * swapFee / 10000); // 0.15% swap fee default(withdraw at once)
    }
...
library SphynxLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'f2af0069c0bb54e0ac59b7e6bcad73e52f364682907e1b7046ae718d0edb7c2a'   // INIT_CODE_PAIR_HASH of Sphynx Factory
            ))));
    }
