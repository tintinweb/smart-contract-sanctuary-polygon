...
contract GameProps is SafeOwnable, ERC1155BurnableUpgradeable, Verifier {
    ...
    mapping(uint256 => bool) private nonceRecord;

    function initialize(uint256[] memory _tokenIds, uint256[] memory _mintTotalSupply, string memory url) public initializer {
        require(_tokenIds.length == _mintTotalSupply.length, "Deploy: param not enough");

        for (uint i = 0; i < _tokenIds.length; i++) {
            totalSupply[_tokenIds[i]] = _mintTotalSupply[i];
        }
    ...
    function setMintAmount(uint256 tokenId, uint256 amount) external onlyOwner {
        require(haveMinted[tokenId] < amount, "SetMintAmount: amount is too low");
    ...
    function verifyMintParam(address account, uint256 tokenId, uint256 amount, uint256 nonce, uint8 v, bytes32 r, bytes32 s) private view returns (bool) {
        bytes32 hash = keccak256(abi.encodePacked(account, "_", tokenId, "_", amount, "_", nonce, "", block.chainid, "_", address(this)));
        bytes32 cHash = convertHash(hash);
        return verifier == cHash.recover(v, r, s);
    }

    function mint(address to, uint256 id, uint256 amount, uint256 nonce, uint8 v, bytes32 r, bytes32 s) external {
        require(!nonceRecord[nonce], "Mint: nonce repeat");
        require(verifyMintParam(to, id, amount, nonce, v, r, s), "Mint: verify failed");

        nonceRecord[nonce] = true;
