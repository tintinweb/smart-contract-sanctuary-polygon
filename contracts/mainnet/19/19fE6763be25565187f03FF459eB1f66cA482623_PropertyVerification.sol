...
contract PropertyVerification is OwnableUpgradeable {
  using SafeERC20Upgradeable for IERC20Upgradeable;
  using CountersUpgradeable for CountersUpgradeable.Counter;
  CountersUpgradeable.Counter private _propertyId;
  ...
  event PropertyVerified(address propertyOwner, uint256 propertyId, bool propertyVerified);

  event verificationPaused(uint256 propertyId, bool propertyVerified);

  uint256 public disputeTimeLimit;
  // amount of funds to deposit if a property is unverified
  uint256 public verifyDepositAmount;

  modifier propertyOwnerOnly(uint256 propertyId) {
    require(msg.sender == properties[propertyId].propertyOwner, "Sender is not the owner");
    _;
  }
  ...
  function initialize(
    ...
    disputeTimeLimit = 86400; // 1 day
    verifyDepositAmount = 50 ether;
  }

  function changeCurrency(address _newCurrency) external onlyOwner {
    currency = IERC20Upgradeable(_newCurrency);
  }
  ...
  function pauseVerify(uint256 propertyId) external onlyOwner {
    require(properties[propertyId].verified, "This property was never verified");
    properties[propertyId].verified = false;

    emit verificationPaused(propertyId, false);
  }
  ...
  function isPropertyVerified(address propertyOwner, uint256 propertyId)
    ...
    returns (bool)
  {
    return properties[propertyId].propertyOwner == propertyOwner && properties[propertyId].verified;
  }
  ...
  function makePropertyPublic(uint256 propertyId) external propertyOwnerOnly(propertyId) {
    require(!properties[propertyId].isPublic, "property is already public");
    // user must have at least the deposit amount worth of currency
    require(
      currency.balanceOf(msg.sender) >= verifyDepositAmount,
      "User does not have enough of the currency"
    );

    properties[propertyId].isPublic = true;

    // if property is not verified, require user to make a deposit
    if (!properties[propertyId].verified) {
      // move the funds from user to this smart contract
      currency.safeTransferFrom(msg.sender, address(this), verifyDepositAmount);
      properties[propertyId].depositAmount = verifyDepositAmount;
    }
    ...
    listingIds[propertyId] = listingId;
  ...
  function makePropertyNotPublic(uint256 propertyId) external propertyOwnerOnly(propertyId) {
    require(properties[propertyId].isPublic, "Property is already not public");

    // if the property is verified, the funds should have been returned to property owner
    if (!properties[propertyId].verified) {
      // Transfer deposit back to user
      currency.safeTransfer(
        properties[propertyId].propertyOwner,
        properties[propertyId].depositAmount
      );

      // Since the funds were returned, the deposit amount should be back to 0
      properties[propertyId].depositAmount = 0;
    }
    properties[propertyId].isPublic = false;

    uint256 listingId = listingIds[propertyId];
    MarketplaceContract.cancelListing(listingIds[propertyId]);
    // remove listing id from property
    listingIds[propertyId] = 0;

    emit PropertyNotPublic(properties[propertyId].propertyOwner, propertyId, listingId, false);
  }
   ...
   */
  function verify(address propertyOwnerAddress, uint256 propertyId) external returns (uint256) {
    // only escrow contract should be verifying properties (after collect)
    require(msg.sender == EscrowContractAddress, "not the escrow contract");
    require(
    ...
    if (!properties[propertyId].verified && properties[propertyId].depositAmount > 0) {
      currency.safeTransfer(propertyOwnerAddress, properties[propertyId].depositAmount);
  ...
  function setDisputeTimeLimit(uint256 newDisputetimeLimit) external onlyOwner {
    disputeTimeLimit = newDisputetimeLimit;
  }

  function setVerifyDepositAmount(uint256 newVerifyDepositAmount) external onlyOwner {
    verifyDepositAmount = newVerifyDepositAmount;
  }

  function unlistProperty(uint256 propertyId) external {
    require(msg.sender == EscrowContractAddress, "User is not the escrow contract");

    // if the property was verified, the user already got their deposit back
    if (!properties[propertyId].verified) {
      currency.safeTransfer(
        MarketplaceContract.treasuryWallet(),
        properties[propertyId].depositAmount
      );
    }
    ...
    properties[propertyId].verified = false;
    properties[propertyId].isPublic = false;
    // set deposit amount to 0 after sending money back to user
    properties[propertyId].depositAmount = 0;
  }

  function setEscrowAddress(address newEscrowAddress) external onlyOwner {
    require(newEscrowAddress != address(0), "Escrow address cannot be 0 address");
    EscrowContractAddress = newEscrowAddress;
  }

  function setMarketplaceAddress(address newMarketplaceAddress) external onlyOwner {
    require(newMarketplaceAddress != address(0), "marketplace address cannot be 0 address");
    MarketplaceContract = IMarketplace(newMarketplaceAddress);
  }
}
