...
contract whitelistCheck is EIP712{
    ...
    string private constant SIGNING_DOMAIN = "EVIE";
...
contract moderntst3haiku is ERC721Enumerable, Ownable , whitelistCheck{
    using Strings for uint;
    using ECDSA for bytes32;

    uint public moderntst3haiku_PRESALE = 1; //presale
    uint public moderntst3haiku_WALLET_PRESALE_LIMIT = 10; //moderntst3haikuperwalletInWholeDuration
    uint public moderntst3haiku_WALLET_PUBLICSALE_LIMIT = 20; //moderntst3haikuperwalletInWholeDuration
    uint public moderntst3haiku_MAX_COUNT = 1400; //maxSupply
    uint public totalTokensSoldInPresale;
    uint public preSaleAmountMinted;

    uint public moderntst3haiku_MAINSALE_PRICE = 0.5 ether; //priceInMainSale
    uint public moderntst3haiku_PRESALE_PRICE = 0.3 ether; //priceInPreSale
    ...
    string private _tokenBaseURI = "https://gateway.pinata.cloud/ipfs/QmSn2wECGnivgsrzNgQsLsPSyJFib48pG7XDjyLeGCR4Vk/";
    string private constant Sig_WORD = "private";
    address private _signerAddress = 0x2aD20Ce88C05D64F57245aF0fb701f33Ec2F8CBa;
    ...
    constructor() ERC721("moderntst3 Haiku", "moderntst3haiku") {}

    function setmoderntst3haiku_MAX_COUNT(uint _maxCount) external onlyOwner{
        moderntst3haiku_MAX_COUNT = _maxCount;
    }

    function setmoderntst3haiku_PRESALE(uint _presale) external onlyOwner{
        moderntst3haiku_PRESALE = _presale;
    }

    function setmoderntst3haiku_WALLET_PRESALE_LIMIT(uint _presaleLimit) external onlyOwner{
        moderntst3haiku_WALLET_PRESALE_LIMIT = _presaleLimit;
    }

    function setmoderntst3haiku_WALLET_PUBLICSALE_LIMIT(uint _publicSaleLimit) external onlyOwner{
        moderntst3haiku_WALLET_PUBLICSALE_LIMIT = _publicSaleLimit;
    }

    function setmoderntst3haiku_MAINSALE_PRICE(uint _mainSalePrice) external onlyOwner{
        moderntst3haiku_MAINSALE_PRICE = _mainSalePrice;
    }

    function setmoderntst3haiku_PRESALE_PRICE(uint _presalePrice) external onlyOwner{
        moderntst3haiku_PRESALE_PRICE = _presalePrice;
    }
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= moderntst3haiku_MAX_COUNT, "EXCEED_MAX");
    ...
    function buy(uint tokenQuantity) external payable {
        ...
        require (tokenQuantity <= moderntst3haiku_WALLET_PUBLICSALE_LIMIT);
        require (saleListPurchase[msg.sender] + tokenQuantity <= moderntst3haiku_WALLET_PUBLICSALE_LIMIT, "MAX MINT COMPLETE");
        require (totalSupply() + tokenQuantity <= (moderntst3haiku_MAX_COUNT - preSaleAmountMinted), "EXCEED_MAX");
        require (moderntst3haiku_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
    ...
    function founderMint(uint tokenQuantity) external onlyOwner {
        require(totalSupply() + tokenQuantity <= moderntst3haiku_MAX_COUNT, "EXCEED_MAX");
        for(uint i = 0; i < tokenQuantity; i++) {
        _safeMint(msg.sender, totalSupply() + 1);
        }
    }
    ...
    function presaleBuy( Whitelist memory whitelist, uint tokenQuantity) external payable{
        require(presaleLive, "PRESALE_CLOSED");//moderntst3haiku_WALLET_PRESALE_LIMIT
        require(getWhitelist(whitelist), "DIRECT_MINT_DISALLOWED");
        require(whitelist.userAddress == msg.sender);
        require(preSaleAmountMinted + tokenQuantity <= moderntst3haiku_PRESALE, "EXCEED_PRIVATE");
        require(presalerListPurchases[msg.sender] + tokenQuantity <= moderntst3haiku_WALLET_PRESALE_LIMIT, "EXCEED_ALLOC");
        require(moderntst3haiku_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
