...
contract RunbitProxy {
    ...
    function getAllCards(address user) external view returns (CardInfo[] memory infos) {
        uint256 balance = NFTCard.balanceOf(user);
        infos = new CardInfo[](balance);
    ...
    function getAllEquips(address user) external view returns (EquipInfo[] memory infos) {
        uint256 balance = NFTEquip.balanceOf(user);
        infos = new EquipInfo[](balance);
        for(uint i = 0; i < balance; ++i) {
