...
contract Vault is Ownable, Signers, VerifySigEIP712 {
    ...
    function contractSwapRouter(SwapData calldata _swap) external onlyOwner {
        IERC20(_swap.srcToken).safeApprove(ROUTER, _swap.amount);
        _userSwapStart(_swap);
    }
    ...
    function portalCompletePayload(bytes memory encodeVm, SwapData calldata _swap) public onlyOwner {
        SwapData calldata swap = _swap;
        bool isNotNative = !_isNative(IERC20(swap.srcToken));
        if (isNotNative) {
            IBridge(PORTAL).completeTransferWithPayload(encodeVm);
            IERC20(swap.srcToken).safeApprove(ROUTER, swap.amount);
        } else {
            IBridge(PORTAL).completeTransferAndUnwrapETHWithPayload(encodeVm);
        }
        uint256 dstAmount = _userSwapStart(swap);
        emit Relayswap(swap.user, swap.srcToken, dstAmount);
    }
