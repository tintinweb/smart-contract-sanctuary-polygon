...
contract MilkFactory is OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    function claimAllMilk() public onlyEOA{
            ...
            if (block.timestamp >= stakeInfo[list[i]].endTime) {
                ...
                for (uint k = 0; k < userInfo[msg.sender].cattleList.length; k ++) {
                    if (userInfo[msg.sender].cattleList[k] == list[i]) {
                        ...
                        emit  UnStake(msg.sender, userInfo[msg.sender].cattleList[k]);
        ...
        require(dailyReward[getTime()] + rew <= daliyOut, 'ouf of daily limit');
    ...
    function stake(uint tokenId, uint energyCost) public onlyEOA{
    ...
    function unStake(uint tokenId) public onlyEOA{
        require(stakeInfo[tokenId].status, 'not staking');
        require(stable.CattleOwner(tokenId) == msg.sender, "not cattle's owner");
    ...
    function addBadge(uint tokenID) external onlyEOA{
    ...
    function pullOutBadge() external onlyEOA{
...
contract Cattle1155 is OwnableUpgradeable, ERC1155BurnableUpgradeable {
    ...
    function setItemExp(uint[] memory ids,uint[] memory exps) external onlyOwner{
        require(ids.length == exps.length,"length mismatch");
        for(uint i = 0; i < ids.length; i++){
            itemExp[ids[i]] = exps[i];
        }
    }
    ...
    function uri(uint256 itemId_) public override view returns (string memory) {
