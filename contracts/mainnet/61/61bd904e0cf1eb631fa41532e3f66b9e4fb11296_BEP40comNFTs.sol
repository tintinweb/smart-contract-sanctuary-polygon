...
---------------------- [ bep40.com ] ----------------------
    ...
    |  Flavors
    |
    |  >  BEP40.COM NFTs v.2: Standard ERC-721 Token with Marketplace
...
contract BEP40NFTs is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControlEnumerable, ERC721Burnable {
...
/**
 * @title BEP40.COM NFTs
 * ERC-721 Marketplace with tokens and royalties support
 */

contract BEP40comNFTs is BEP40NFTs, ReentrancyGuard {
    ...
    constructor(
        ...
        BEP40NFTs(_owner, name, symbol, _anyoneCanMint) 
    {
        admin = _admin;
        contract_owner = _owner;
        require(_commissionRate<=100, "BEP40NFTs: Commission rate has to be between 0 and 100");
    ...
    function sell(uint256 tokenId, uint256 price, address payable wallet) public {
        ...
        // onlyOwner
        require(ownerOf(tokenId)==msg.sender, "BEP40NFTs: Only owner can sell this item");

        // cannot set a price if auction is activated
        require(!auctions[tokenId].open, "BEP40NFTs: Cannot sell an item which has an active auction");
    ...
    function buy(uint256 tokenId) public nonReentrant {
        ...
        // is on sale
        require(!auctions[tokenId].open && sellBidPrice[tokenId]>0, "BEP40NFTs: The collectible is not for sale");

        // transfer ownership
        address owner = ownerOf(tokenId);

        require(msg.sender!=owner, "BEP40NFTs: The seller cannot buy his own collectible");
    ...
    function createAuction(uint256 tokenId, uint _closingTime, address payable _beneficiary, uint256 _reservePrice) public {
        ...
        require(sellBidPrice[tokenId]==0, "BEP40NFTs: The selected NFT is open for sale, cannot be auctioned");
        require(!auctions[tokenId].open, "BEP40NFTs: The selected NFT already has an auction");
        require(ownerOf(tokenId)==msg.sender, "BEP40NFTs: Only owner can auction this item");
    ...
    function bid(uint256 tokenId, uint256 bid_value) public nonReentrant {
        ...
        address owner = ownerOf(tokenId);
        require(msg.sender!=owner, "BEP40NFTs: The owner cannot bid his own collectible");
