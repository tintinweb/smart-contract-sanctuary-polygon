    ...
    SelfPermitUpgradeable
{
    ...
    address public bbVault; // buyBack vault
    ...
    function stake(
        ...
        IERC20Upgradeable(_stakeToken).safeTransferFrom(
            msg.sender,
            address(this),
            assets
        );
        IERC20Upgradeable(_stakeToken).safeIncreaseAllowance(
            _customVaultAddress,
            assets
        );
        console.log("insode the vault router before", assets);
        // tx below will result in receiving xTokens to this contract from custom vault
        uint256 xTokensReceived = IERC4626StakingPool(_customVaultAddress)
            .stake(assets);
        console.log("insode the vault router after", xTokensReceived);

        // approving
        IERC20Upgradeable(_xToken).safeIncreaseAllowance(
            _bbVault,
            xTokensReceived
        );
        console.log("After token appeove");
        // now staking to the main vault using xToken
        shares = IERC4626StakingPool(_bbVault).stake(
            xTokensReceived,
            address(this)
        );
        console.log("After main vault stake", shares);

        if (depositSTokensToGauge) {
            IERC20Upgradeable(_bbVault).safeIncreaseAllowance(_gauge, shares);
            console.log("After gauge appprove");
            console.log(_gauge, "This is gagueg address ");
            IGauge(_gauge).depositFor(shares, 0, msg.sender);
        } else {
            // now transferring sTokens to user
            IERC20Upgradeable(_bbVault).safeTransfer(msg.sender, shares);
        }
    }
    ...
    function updateBBVault(address _bbVault) external {
        bbVault = _bbVault;
    }
    ...
    function unstakeBuyBackToken(
        ...
        IERC20Upgradeable(_bbToken).safeTransferFrom(
            msg.sender,
            address(this),
            _amount
        );

        IERC20Upgradeable(_bbToken).safeIncreaseAllowance(_bbToken, _amount);

        uint256 receiveTokens = IERC4626StakingPool(_bbToken).withdraw(
            _amount,
            address(this),
            address(this)
        );
        ...
        IERC20Upgradeable(_tokenOut).safeTransfer(msg.sender, receivedTOkenOut);
    }
