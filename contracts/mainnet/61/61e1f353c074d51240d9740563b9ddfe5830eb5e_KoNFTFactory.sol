    ...
    ContextMixin
{
    ...
    mapping(uint256 => string) private _tokenURIs;
    mapping(uint256 => string) private _depositTokenURIs;
    ...
    constructor(
        ...
        _setupContractId(contractId_);
        _setupRole(DEPOSITOR_ROLE, childChainManagerProxy_);
        _setupRole(MINT_ROLE, minter_);
    ...
    function _setDepositTokenURI(uint256 tokenId, string memory _tokenURI) internal virtual {
        require(_exists(tokenId), "ERC721Metadata: URI set of nonexistent token");
        _depositTokenURIs[tokenId] = _tokenURI;
    }
        ...
        only(DEPOSITOR_ROLE)
    {
        ...
        if (depositData.length == 32) {
            ...
            _mint(user, tokenId);
            _setTokenURI(tokenId, _depositTokenURIs[tokenId]);
        ...
        // deposit batch
            ...
            for (uint256 i; i < length; i++) {
                ...
                _setTokenURI(tokenIds[i], _depositTokenURIs[tokenIds[i]]);
            }
        }
    ...
    * This implementation all user to mint tokens but it can be changed as per requirement
    * @param recipient user for whom tokens are being minted
    * @param uri is origin file address for NFT Token
    */
    function mint(address recipient, string memory uri) public only(MINT_ROLE) returns (uint256) {
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        _mint(recipient, newItemId);
        _setTokenURI(newItemId, uri);
        _setDepositTokenURI(newItemId, uri);
...
contract KoNFTFactory is NativeMetaTransaction, ContextMixin {

  /**
    * @dev https://docs.polygon.technology/docs/develop/ethereum-polygon/submit-mapping-request/
    */
  address public constant CHILD_CHAIN_MANAGER_PROXY = 0xA6FA4fB5f76172d178d61B04b0ecd319C5d1C0aa;
