...
contract PriceInterfaceFacet is Modifiers {
    event PriceInterfaceConfigChanged(
        address baseTokenAddr,
        address utilityTokenAddr,
        uint16 minUtilityShare,
        uint256 minUtilityTokenBalance,
        uint256 minTokenValuation
    );
    event PriceSourceConfigChanged(address indexed tokenAddr, address feedAddr, uint8 feedType);
    event PriceInterfaceAllowUnreliableSourcesChanged(bool state);

    /**
    @notice configure price interface global settings
    @param _baseToken address of token, in which all prices will be denominated, probably stablecoin
    @param _utilityToken address of utility token
    @param _minUtilityTokenShare minimum allowed ratio of utility token in portfolio token, in fraction of 1 multiplied by share denuminator
    @param _minUtilityTokenBalance minimum utility token balanace of portfolio manager to allow to bypass minimum utility token ratio requirement
    @param _minTokenValuation minimum value of tracked token locked in portfolio token to be considered significant
     */
    function setPriceInterfaceConfig(
        address _baseToken,
        address _utilityToken,
        uint16 _minUtilityTokenShare,
        uint256 _minUtilityTokenBalance,
        uint256 _minTokenValuation
    ) external onlyOwner {
        LibPriceInterface.DiamondStorage storage ds = LibPriceInterface.diamondStorage();
        require(_baseToken != address(0));
        require(_utilityToken != address(0));
        require(_minUtilityTokenShare <= LibPriceInterface.SHARE_DENOMINATOR);

        ds.baseToken = _baseToken;
        ds.utilityToken = _utilityToken;
        ds.minUtilityTokenShare = _minUtilityTokenShare;
        ds.minTokenValuation = _minTokenValuation;
        ds.minUtilityTokenBalance = _minUtilityTokenBalance;

        emit PriceInterfaceConfigChanged(_baseToken, _utilityToken, _minUtilityTokenShare, _minUtilityTokenBalance, _minTokenValuation);
    }

    /// @notice allow usage of unreliable oracles, to be used for testing purposes
    function setAllowUnreliablePriceSources(bool _state) external onlyOwner {
        LibPriceInterface.DiamondStorage storage ds = LibPriceInterface.diamondStorage();
        ds.allowUnreliablePriceSources = _state;
        emit PriceInterfaceAllowUnreliableSourcesChanged(_state);
    }

    /**
    @notice get price interface global settings
    @return baseToken address of token, in which all prices will be denominated, probably stablecoin;
            utilityToken address of utility token;
            minUtilityTokenShare minimum allowed ratio of utility token in portfolio token, in fraction of 1 multiplied by share denuminator;
            minUtilityTokenBalance minimum utility token balanace of portfolio manager to allow to bypass minimum utility token ratio requirement;
            minTokenValuation minimum value of tracked token locked in portfolio token to be considered significant;
    */
    function getPriceInterfaceConfig()
        external
        view
        returns (
            address baseToken,
            address utilityToken,
            uint16 minUtilityTokenShare,
            uint256 minUtilityTokenBalance,
            uint256 minTokenValuation
        )
    {
        ...
        baseToken = ds.baseToken;
        utilityToken = ds.utilityToken;
        minUtilityTokenShare = ds.minUtilityTokenShare;
        minTokenValuation = ds.minTokenValuation;
        minUtilityTokenBalance = ds.minUtilityTokenBalance;
    }

    /**
    @notice Set price source config for token
    @param _addr token address
    @param _feed price feed address
    @param _feedType feed type id, 1: chainlink usd feed, 254: 1inch offchain oracle
     */
    function setPriceSourceConfig(
        address _addr,
        address _feed,
        uint8 _feedType
    ) external onlyOwner {
        LibPriceInterface.DiamondStorage storage ds = LibPriceInterface.diamondStorage();
        ds.priceSources[_addr].feed = _feed;
        ds.priceSources[_addr].feedType = _feedType;

        emit PriceSourceConfigChanged(_addr, _feed, _feedType);
    }

    /**
    @notice Get price source config for token
    @param _addr token address
    @return PriceSourceSettings struct
     */
    function getPriceSourceConfig(address _addr) external view returns (LibPriceInterface.PriceSourceSettings memory) {
        LibPriceInterface.DiamondStorage storage ds = LibPriceInterface.diamondStorage();
        return ds.priceSources[_addr];
    }

    /**
    @notice return value of provided amount of token in base token
    @param _addr token address
    @param _amount amount of token
    @return price in base token base units
    */
    function getTokenPrice(address _addr, uint256 _amount) external view returns (uint256 price) {
        return LibPriceInterface.getTokenPrice(_addr, _amount);
    }

    /**
    @notice return amount of provided token with provided value in base token
    @param _addr token address
    @param _price amount of base token
    @return amount corresonding amount of token
    */
    function getTokenForPrice(address _addr, uint256 _price) external view returns (uint256 amount) {
        return LibPriceInterface.getTokenForPrice(_addr, _price);
    }

    /**
    @notice calculate total price of provided tokens
    @param _tokens array of tokens
    @param _amounts array of corresponding amounts
    @return total value in base token
     */
    function getTokensTotalPrice(address[] memory _tokens, uint256[] memory _amounts) external view returns (uint256) {
        return LibPriceInterface.getTokensTotalPrice(_tokens, _amounts);
    }

    /**
    @notice calculate share of utility token in provided token amounts
    @param _tokens array of tokens
    @param _amounts array of corresponding amounts
    @return ratio of utility token (fraction of 1 multiplied by SHARE_DENOMINATOR)
     */
    function getUtilityTokenShare(address[] memory _tokens, uint256[] memory _amounts) external view returns (uint256) {
        return LibPriceInterface.getUtilityTokenShare(_tokens, _amounts);
    }

    /**
    @notice check if there is enough utility token in provided token amounts, owner dependend 
            since owners utility token balance can bypass utility token share check
    @param _portTokenOwner address of token owner, zero address can be provided to skip owner check
    @param _tokens array of tokens
    @param _amounts array of corresponding amounts
    @return true if utiltity token share is not less than required 
     */
    function checkUtilityTokenShare(
        ...
        return LibPriceInterface.checkUtilityTokenShare(_portTokenOwner, _tokens, _amounts);
    }

    /**
    @notice calculate minimum amount of utility token for given amounts of tokens to pass minimum utility token share check
    @param _tokens array of tokens
    @param _amounts array of corresponding amounts
    @return amount of utility token
     */
    function estimateMinUtilityTokenAmount(address[] memory _tokens, uint256[] memory _amounts) external view returns (uint256) {
        return LibPriceInterface.estimateMinUtilityTokenAmount(_tokens, _amounts);
    }

    /**
    @notice estimates amounts of tokens from total price and ratios
    @param _totalPrice total price in base token
    @param _tokens array of token addresses
    @param _ratios corresponding ratios (fraction of 1 multiplied by SHARE_DENOMINATOR)
    ...
    function estimateAmountsFromTotalPrice(
        ...
        return LibPriceInterface.estimateAmountsFromTotalPrice(_totalPrice, _tokens, _ratios);
    }

    /**
    @notice check if provided amount of token is significant, checks that provided amount of token has value more than set threshold
    @param _token token address
    @param _amount amout of token to check value
     */
    function amountIsSignificant(address _token, uint256 _amount) external view returns(bool) {
        return LibPriceInterface.amountIsSignificant(_token, _amount);
    }
}
