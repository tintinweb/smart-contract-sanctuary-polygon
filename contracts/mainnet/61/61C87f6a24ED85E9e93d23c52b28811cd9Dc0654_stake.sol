...
//        uint256 public StakeTime = 5 * 86400;
        StakeTime = 5 *3600;
	...
	if(amount > StakeWal[addr].frozen)
	{
	    a = a.sub(StakeWal[addr].frozen);
	    StakeWal[addr].frozen = 0;
	    StakeWal[addr].unlock = StakeWal[addr].unlock.sub(a);
	    a = 0;
	}
	if(amount == StakeWal[addr].frozen)
	{
	...
	{
	    a = a.sub(StakeWal[addr].frozen);
	    StakeWal[addr].frozen = StakeWal[addr].frozen.sub(a);
	    a = 0;
	}
