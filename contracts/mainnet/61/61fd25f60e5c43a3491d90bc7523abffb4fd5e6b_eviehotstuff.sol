...
contract eviehotstuff is ERC721Enumerable, Ownable , whitelistCheck{
    using Strings for uint;
    using ECDSA for bytes32;

    uint public eviehotstuff_PRESALE = 1; //presale
    uint public eviehotstuff_WALLET_PRESALE_LIMIT = 10; //eviehotstuffperwalletInWholeDuration
    uint public eviehotstuff_WALLET_PUBLICSALE_LIMIT = 20; //eviehotstuffperwalletInWholeDuration
    uint public eviehotstuff_MAX_COUNT = 88; //maxSupply
    uint public totalTokensSoldInPresale;
    uint public preSaleAmountMinted;

    uint public eviehotstuff_MAINSALE_PRICE = 0.5 ether; //priceInMainSale
    uint public eviehotstuff_PRESALE_PRICE = 0.3 ether; //priceInPreSale
    ...
    string private _tokenBaseURI = "https://gateway.pinata.cloud/ipfs/QmW9dg7QkZzn9WWzUEiieWtgRNFSjrtmbgWF18Ujy4tYsV/";
    ...
    constructor() ERC721("EVIE Hot Stuff", "eviehotstuff") {}

    function seteviehotstuff_MAX_COUNT(uint _maxCount) external onlyOwner{
        eviehotstuff_MAX_COUNT = _maxCount;
    }

    function seteviehotstuff_PRESALE(uint _presale) external onlyOwner{
        eviehotstuff_PRESALE = _presale;
    }

    function seteviehotstuff_WALLET_PRESALE_LIMIT(uint _presaleLimit) external onlyOwner{
        eviehotstuff_WALLET_PRESALE_LIMIT = _presaleLimit;
    }

    function seteviehotstuff_WALLET_PUBLICSALE_LIMIT(uint _publicSaleLimit) external onlyOwner{
        eviehotstuff_WALLET_PUBLICSALE_LIMIT = _publicSaleLimit;
    }

    function seteviehotstuff_MAINSALE_PRICE(uint _mainSalePrice) external onlyOwner{
        eviehotstuff_MAINSALE_PRICE = _mainSalePrice;
    }

    function seteviehotstuff_PRESALE_PRICE(uint _presalePrice) external onlyOwner{
        eviehotstuff_PRESALE_PRICE = _presalePrice;
    }
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= eviehotstuff_MAX_COUNT, "EXCEED_MAX");
    ...
    function buy(uint tokenQuantity) external payable {
        ...
        require (tokenQuantity <= eviehotstuff_WALLET_PUBLICSALE_LIMIT);
        require (saleListPurchase[msg.sender] + tokenQuantity <= eviehotstuff_WALLET_PUBLICSALE_LIMIT, "MAX MINT COMPLETE");
        require (totalSupply() + tokenQuantity <= (eviehotstuff_MAX_COUNT - preSaleAmountMinted), "EXCEED_MAX");
        require (eviehotstuff_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
    ...
    function founderMint(uint tokenQuantity) external onlyOwner {
        require(totalSupply() + tokenQuantity <= eviehotstuff_MAX_COUNT, "EXCEED_MAX");
        for(uint i = 0; i < tokenQuantity; i++) {
        _safeMint(msg.sender, totalSupply() + 1);
        }
    }
    ...
    function presaleBuy( Whitelist memory whitelist, uint tokenQuantity) external payable{
        require(presaleLive, "PRESALE_CLOSED");//eviehotstuff_WALLET_PRESALE_LIMIT
        require(getWhitelist(whitelist), "DIRECT_MINT_DISALLOWED");
        require(whitelist.userAddress == msg.sender);
        require(preSaleAmountMinted + tokenQuantity <= eviehotstuff_PRESALE, "EXCEED_PRIVATE");
        require(presalerListPurchases[msg.sender] + tokenQuantity <= eviehotstuff_WALLET_PRESALE_LIMIT, "EXCEED_ALLOC");
        require(eviehotstuff_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
