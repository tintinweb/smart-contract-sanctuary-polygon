...
// File: VotePowerProxy/interfaces/IVotingPower.sol

//SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

interface IVotingPower {
    function getBarBalance(address _user) external view returns (uint256);

    function getBarVaultBalance(address _user) external pure returns (uint256);

    function getIFOPoolBalancee(address _user) external view returns (uint256);

    function getBarPoolBalance(address _user) external view returns (uint256);

    function getBarMaticLpBalance(address _user) external view returns (uint256);
...
// File: VotePowerProxy/contracts/utils/Context.sol
...
// File: VotePowerProxy/contracts/access/Ownable.sol
...
// File: VotePowerProxy/VotePowerProxy.sol
...
contract VotePowerProxy is Ownable {
    ...
    function getBarBalance(address _user) external view returns (uint256) {
        return IVotingPower(VotingPowerContract).getBarBalance(_user);
    }

    function getBarVaultBalance(address _user) external view returns (uint256) {
        return IVotingPower(VotingPowerContract).getBarVaultBalance(_user);
    }
    ...
    function getBarPoolBalance(address _user) external view returns (uint256) {
        return IVotingPower(VotingPowerContract).getBarPoolBalance(_user);
    }

    function getBarMaticLpBalance(address _user) external view returns (uint256) {
        return IVotingPower(VotingPowerContract).getBarMaticLpBalance(_user);
    }
