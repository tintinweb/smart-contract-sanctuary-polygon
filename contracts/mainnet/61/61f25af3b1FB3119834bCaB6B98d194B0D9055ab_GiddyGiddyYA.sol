...
contract GiddyGiddyYA is BaseYA, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  function calcV2Swap(address router, address src, address dst, uint256 value) public view returns (uint256 beforeSwap, uint256 afterSwap) {
    ...
    beforeSwap = (value * dstReserve) / srcReserve;
    afterSwap = dstReserve - ((srcReserve * dstReserve) / (srcReserve + value));
  }

  function calcV2PriceSimple(address router, address src, address dst, uint256 value) public view returns (uint256 amountOut, uint256 priceImpact) {
    (uint256 beforeSwap, uint256 afterSwap) = calcV2Swap(router, src, dst, value);
    amountOut = afterSwap;
  ...
  function depositCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = calcV2PriceSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, value - fap);
  }
  ...
  function withdrawCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = calcV2PriceSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, sharesToValue(shares));
    amountOut -= fap;
  }
