...
contract Orders is Ownable, IOrders {
    ...
    function createOrder(bytes32 _hash, address _recipient, address _collection, uint256 _nftId, uint256 _timestamp, address _token, uint256 _amount, uint _destination) public  {
        ...
        uint256 orderPrice = deposit(_token, _nftId, _amount, _destination);
    ...
    function deposit(address _token, uint256 _nftId, uint256 _amount, uint _destination) private returns(uint256 orderPrice) {
        IERC20(_token).transferFrom(msg.sender, address(this), _amount);

        prices[_nftId] = _amount - shippingFee[_destination];
    ...
    function getTotalPrice(uint256 _nftId, uint _destination) public view returns (uint256 total, uint256) {
        uint256 order_price = prices[_nftId] + (prices[_nftId] *  1000 / 10000);
    
        if (order_price == 0) {
            order_price = initialPrice;
        }

        total = order_price + shippingFee[_destination];
        
        return (total, shippingFee[_destination]);
    }
