...
contract TreasuryDAO is ReentrancyGuard, AccessControl {
    ...
    uint256 public distributionTimeFrame = 3600 * 24;
    ...
    function autoLiquidity() external nonReentrant onlyRole(OPERATOR_ROLE) {
     ...
     * Can only be called by the current owner.
     */
    function getUSDDrip(uint256 totalUsdLockup) external returns (uint256) {
    ...
    function _removeLiquidity(address token) internal {
        ...
        require(IERC20(token).approve(address(sandManSwapRouter), tokenAmount), '!approved');

        IUniswapV2Pair lpToken = IUniswapV2Pair(token);

        address token0 = lpToken.token0();
        address token1 = lpToken.token1();
        address WETH = sandManSwapRouter.WETH();
        ...
        if (token0 == WETH || token1 == WETH){
            ...
            // IWETH(sandManSwapRouter.WETH()).withdraw(tokenAmount/2);

            sandManSwapRouter.removeLiquidityETHSupportingFeeOnTransferTokens(
                token1,
                tokenAmount,
                0,
                0,
    ...
    function swapDustToETH(address token) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 balanceToken = IERC20(address(token)).balanceOf(address(this));
        if (balanceToken > 0){
            _swapTokensForEth(token, balanceToken);
        }
    }

    function extractToken(address token) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 balanceToken = IERC20(address(token)).balanceOf(address(this));
        if (balanceToken > 0){
    ...
    function extractTokenETH(address token) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 balanceToken = IERC20(address(token)).balanceOf(address(this));
        if (balanceToken > 0){
            IERC20(address(token)).transfer(msg.sender, balanceToken);
        }
...
contract SandManToken is ERC20, Ownable {
    ...
    uint256 public minSandManAmountToLiquify = 1 * (10 ** 18); // para las prueba solo le doy importante a este
    // Min amount to liquify. (default 100 MATIC)
    uint256 public minMaticAmountToLiquify = 100 *  (10 ** 18); 
    IUniswapV2Router02 public sandManSwapRouter;
    // The trading pair
    address public sandManSwapPair;
    // In swap and liquify
    ...
    function swapAndLiquify() private lockTheSwap transferTaxFree {
        ...
        if (contractTokenBalance >= minSandManAmountToLiquify) {
            try
                treasuryDAO.autoLiquidity()
            { /* suceeded */ } catch { /* failed, but we avoided reverting */ }
        }
    ...
    function _isNotHolderExcluded(address _userAddress) internal view returns(bool) {
        return(!_excludedHolders[_userAddress]);
    }
    ...
    function updateOperators(address _operatorAddress, bool _status) external onlyOwner {
    ...
    ///@dev Update operator address status
    function updateHolders(address _holderAddress, bool _status) external onlyOwner {
        require(_holderAddress != address(0), "ERR9");

        emit ExcludedHoldersUpdated(_holderAddress, _excludedHolders[_holderAddress], _status);

        _excludedHolders[_holderAddress] = _status;
    }
    ...
    ///@dev setup limits of liquidity
    function setLiquidityBounds(uint256 _minMaticAmountToLiquify, uint256 _minSandManAmountToLiquify) external onlyOperator {
        minMaticAmountToLiquify = _minMaticAmountToLiquify;
        minSandManAmountToLiquify = _minSandManAmountToLiquify;
    }

    function setAutoLiquidityStatus(bool _swapAndLiquifyEnabled) external onlyOperator {
    ...
    ///@dev Check if the address is excluded from antibot system.
    function isExcludedOperator(address _userAddress) external view returns(bool) {
        return(_excludedOperators[_userAddress]);
    }

    ///@dev Check if the address is excluded from antibot system.
    function isExcludedHolder(address _userAddress) external view returns(bool) {
        return(_excludedHolders[_userAddress]);
    }
...
contract MasterChefNFT is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    function _payPendingTheEndlessSandMan(uint256 _pid) internal {
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pendingSandManToken > 0 || user.sandManRewardLockup > 0) {
                ...
                // send rewards
                _safeTokenTransfer(address(sandManToken), msg.sender, sandmanRewards);

                if (user.hasNFT) {
                    _payNFTBoost(_pid, sandmanRewards);
                    user.experience = user.experience + ((sandmanRewards * experienceRate) / 10000);
            ...
            if (_pid == theEndlessPID) {
                ...
                if (pendingTheEndlessToken > 0 || user.theEndlessRewardLockup > 0) {
                    ...
                    _safeTokenTransfer(address(theEndlessToken), msg.sender, theEndlessRewards);
                }
            }
        } else if (pendingSandManToken > 0 || pendingTheEndlessToken > 0 || pendingUSDToken > 0) {
    ...
    function _updateHarvestLockup(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (user.hasNFT && newHarvestInverval > 0) {
            ...
            if(user.experience > 100){
                extraBoosted = (user.experience / 10) / 1e24; 
            }
    ...
    // Pay NFT Boost
    function _payNFTBoost(uint256 _pid, uint256 _pending) internal {
        UserInfo storage user = userInfo[_pid][msg.sender];

        uint256 extraBoosted = 0;
        if(user.experience > 100){
            extraBoosted = (user.experience / 100) / 1e24; 
        }


        uint256 rewardBoosted = (_pending * (user.powerStaking + extraBoosted)) / 10000;
        if (rewardBoosted > 0) {
            sandManToken.mint(msg.sender, rewardBoosted);
        }
    }
    ///
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (_pid != theEndlessPID && totalAllocPoint > poolInfo[theEndlessPID].allocPoint) { 
    ...
    function pendingUSD(address _user) external view returns (uint256) {
        ...
        return (((user.amount * accDepositUSDRewardPerShare) / 1e24) - user.usdRewardDebt + user.usdRewardLockup);
    }
    ...
    function withdrawNFT(uint256 _pid) external nonReentrant {
        ...
        if (user.sandManRewardLockup > 0) {
            _payNFTBoost(_pid, user.sandManRewardLockup);
