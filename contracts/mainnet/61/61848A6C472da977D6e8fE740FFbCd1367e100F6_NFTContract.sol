...
contract NFTContract is ERC721URIStorage, Ownable {
    ...
    constructor(address _server) ERC721("NFTNGINE", "NFTEGN") {
        paused = false;
        server = _server;
    }
                    ...
                    Base64.encode(
                        bytes(
                            string(
                                abi.encodePacked(
                                    '{"name": "NFTNGINE",',
                                    '"description": "AI Generated NFTs on Polygon",',
                                    '"image": "",',
                                    '"external_link": ""',
                                    "}"
                                )
    ...
    function mintNFT(
        ...
        address recipient
    ) public isNotPaused returns (uint256) {

        require(msg.sender == server, "Only server can mint");
        uint256 newItemId = _tokenIds.current();

        string memory finalTokenUri = payload;
    ...
    // need to make a map to get encryption
