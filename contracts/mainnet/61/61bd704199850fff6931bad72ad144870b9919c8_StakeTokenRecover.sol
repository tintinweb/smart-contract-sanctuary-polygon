...
import '../stake/StakeToken.sol';

contract StakeTokenRecover is StakeToken {
    /** Initialize ------------------------------------------------------------------------ */
    function initialize(
        address _manager,
        string memory name,
        ...
        enabled = false; // Initially Enabled
    ...
    function init(
        ...
        ventureCapital = IVentureCapital(_ventureCapital);
...
contract StakeToken is IStakeToken, ExternallyCallable, Pausable, ERC721EnumerableUpgradeable {
    ...
    function postTransfer(
        ...
        StakeData1e18 memory stake = stakeManager.getStake(tokenId);
        require(
            stake.status != StakeStatus.Withdrawn,
            'TOKEN: Stake is withdrawn and can not be transferred'
        );

        // if(_msgSender() == opensea)
        stakeManager.stakeTax(tokenId, transferTax);
        StakeData1e18 memory taxedStake = stakeManager.getStake(tokenId);

        ventureCapital.transferSharesAndRebalance(from, to, stake.shares, taxedStake.shares);
    }
    ...
    /** @dev setTransferTax
        @param _tax {uint256}
     */
    function setTransferTax(uint256 _tax) external onlyManager {
        transferTax = _tax;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory directory = getDirectory(stake);
        return
            string(
                abi.encodePacked(
                    base64Uri,
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                directory,
                                '.mp4", ',
                                '"external_url": "https://axion.network/", ',
                                '"description": "',
                                getDescription(stake),
                                '", ',
                                getAttributes(stake),
                                '}'
                            )
    ...
    function getDescription(StakeData1e18 memory stake) internal pure returns (string memory) {
        return
            string(
                bytes(
                    abi.encodePacked(
                        'This is no normal NFT. This is an Axion Stake NFT, which is earning the holder 8% interest in AXN plus liquid Bitcoin dividends which can be withdrawn at any point from our staking portal. This can be traded, and the buyer will start earning Bitcoin from it instead of the previous owner. Once the stake is matured, the AXN principal + interest can be withdrawn to your wallet. This particular stake started with **',
                        uint256((stake.shares / uint64(1e18))).toString(),
                        ' AXN**, staked for **',
                        uint256(stake.stakingDays).toString(),
                        ' days**, giving the holder **',
                        uint256((stake.shares / uint64(1e18))).toString(),
                        ' shares** in our ecosystem. For more information, please visit [Axion Network](https://axion.network).'
                    )
                )
            );
    }

    function getAttributes(StakeData1e18 memory stake) internal pure returns (string memory) {
        return
            string(
                bytes(
                    abi.encodePacked(
                        ...
                        '" },'
                        '{ "display_type": "number", "trait_type": "Shares", "value": "',
                        uint256((stake.shares / uint64(1e18))).toString(),
                        '"}, { "display_type": "number", "trait_type": "Amount of AXN", "value": "',
                        uint256((stake.amount / uint64(1e18))).toString(),
                        '"}, ',
                        getDates(stake),
                        ']'
                    )
    ...
    function getDates(StakeData1e18 memory stake) internal pure returns (string memory) {
        return
            string(
                bytes(
                    abi.encodePacked(
    ...
    function getDirectory(StakeData1e18 memory stake) internal pure returns (string memory) {
        string memory folder = 'A';
        string memory number = '1';

        if (stake.stakingDays == 5555) {
            folder = 'H';
        } else if (stake.stakingDays >= 4745) {
            folder = 'G';
        } else if (stake.stakingDays >= 3650) {
            folder = 'F';
        } else if (stake.stakingDays >= 2920) {
            folder = 'E';
        } else if (stake.stakingDays >= 1825) {
            folder = 'D';
        } else if (stake.stakingDays >= 1095) {
            folder = 'C';
        } else if (stake.stakingDays >= 365) {
            folder = 'B';
        }

        if (stake.amount == 1000 * 1e6 * 1e18) {
            number = '6';
        } else if (stake.amount >= 500 * 1e6 * 1e18) {
            number = '5';
        } else if (stake.amount >= 100 * 1e6 * 1e18) {
            number = '4';
        } else if (stake.amount >= 50 * 1e6 * 1e18) {
            number = '3';
        } else if (stake.amount >= 10 * 1e6 * 1e18) {
            number = '2';
        }

        return string(abi.encodePacked(folder, '/', folder, number)); // A/A1.mp4 etc.
    }
}
