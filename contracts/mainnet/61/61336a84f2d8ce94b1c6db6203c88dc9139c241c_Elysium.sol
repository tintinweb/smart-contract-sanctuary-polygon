...
contract Elysium is ERC721Enumerable, Payable, DefaultOperatorFilterer {
    using Strings for uint256;
    ...
    constructor() ERC721("Elysium", "ELS") {
    ...
    function mintReserved(address[] calldata to, uint256[] calldata quantity) external onlyOwner {
        ...
        for (uint256 i; i < to.length; i++) {
            require(totalSupply() + quantity[i] < maxSalePlusOne, "Exceeds Available Tokens!");

            for (uint256 j = 0; j < quantity[i]; j++) {
                uint256 tid = tokenId();
                mintsPerAddress[to[i]] += 1;
                _safeMint(to[i], tid);
            }
