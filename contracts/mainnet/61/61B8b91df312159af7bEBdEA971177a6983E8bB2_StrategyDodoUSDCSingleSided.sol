...
contract StrategyDodoUSDCSingleSided is StrategyBase {
    ...
    address[] public dodoUsdcRoute = [dodo_usdt_usdc_lp, dodo_dodo_usdt_lp];
    ...
    )
    {
        ...
        IERC20(usdc_dodo_lp_token).approve(dodo_approve, uint256(- 1));
    ...
    function _harvest(address callFeeRecipient) internal {
        ...
        // check for usdc LP
        _usdc = IERC20(usdc_dodo_lp_token).balanceOf(address(this));

        if (_usdc > 0) {
            IDodoMine(rewards).deposit(_usdc);
        }
    ...
    function getName() external pure override returns (string memory) {
        return "StrategyDodoUSDCSingleSided";
    }
}