...
interface INXUSD {
    ...
    function mintToBentoBox(address clone, uint256 amount, IBentoBoxV1 bentoBox) external;
    function burn(uint256 amount) external;
}

interface IPermissionManager {
    function info(address sender) external returns(uint index, bool isAllowed);
}
...
contract CauldronV2 is BoringOwnable, IMasterContract {
    ...
    IPermissionManager public immutable liquidatorManager;
    ...
    /// @notice The constructor is only used for the initial master contract. Subsequent clones are initialised via `init`.
    constructor(IBentoBoxV1 bentoBox_, IERC20 nxusd_, IPermissionManager liquidatorManager_) public {
        bentoBox = bentoBox_;
        nxusd = nxusd_;
        masterContract = this;
    ...
    function reduceSupply(uint256 amount) public virtual {
        ...
        INXUSD(address(nxusd)).burn(amount);
    }
}