...
contract WishingWell is EIP712 {
  ...
  event WishFulfilled(uint256 indexed id, address wisher, uint256 expiresAt, string metadata, ERC20Cost[] erc20);
  ...
  bytes32 constant public WISH_SIG_HASH = keccak256("Wish(address wisher,uint256 id,uint256 expiresAt,string metadata,ERC20Cost[] erc20)ERC20Cost(address token,string tokenId,uint256 amountWei,string usdTarget)");
  function __verifyWishSigner(address wisher, uint256 id, uint256 expiresAt, string calldata metadata, ERC20Cost[] calldata erc20, bytes calldata signature) private view {
    ...
    require(settings.isPermittedTo(signer, "WishingWell", "Wish"), "403");
  }
  function wish(uint256 id, uint256 expiresAt, string calldata metadata, ERC20Cost[] calldata erc20, bytes calldata signature) external {

    require(expiresAt >= block.timestamp, "410");
    require(wishes[id].fulfilled == false, "400");

    __verifyWishSigner(msg.sender, id, expiresAt, metadata, erc20, signature);

    wishes[id].fulfilled = true;
    wishes[id].id = id;
    wishes[id].expiresAt = expiresAt;
    wishes[id].purchaser = msg.sender;
    wishes[id].metadata = metadata;
    ...
    for(uint8 i = 0; i < erc20.length; i++) {
      ...
      if(treasury.acceptedTokens(address(token))) {
        ...
        token.transfer(address(treasury), erc20[i].amountWei);
      }
    }

    emit WishFulfilled(id, msg.sender, expiresAt, metadata, erc20);
  }
}
