...
contract TokenAddressInitializerFacet {

    function setWETHTokenAddress(address _wethAddress) public {
        LibDiamond.enforceIsContractOwner();
        LibDiamond.setWethTokenAddress(_wethAddress);
    }

    function getWETHTokenAddress() external view returns (address) {
        return LibDiamond.diamondStorage().WethTokenAddress;
    }

    function setRBWTokenAddress(address _rbwAddress) public {
        LibDiamond.enforceIsContractOwner();
        LibDiamond.setRbwTokenAddress(_rbwAddress);
    }

    function getRBWTokenAddress() external view returns(address) {
        return LibDiamond.diamondStorage().rbwTokenAddress;
    }

    function setUNIMTokenAddress(address _unimAddress) public {
        LibDiamond.enforceIsContractOwner();
        LibDiamond.setUnimTokenAddress(_unimAddress);
    }

    function getUNIMTokenAddress() external view returns(address) {
        return LibDiamond.diamondStorage().unimTokenAddress;
    }
}
