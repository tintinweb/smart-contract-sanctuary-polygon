...
contract TokenErrorReporter {
    ...
    enum FailureInfo {
        ...
        SET_BORROW_SHARE_ADMIN_CHECK,
        SET_BORROW_SHARE_BOUNDS_CHECK,
...
contract Comptroller is ComptrollerV3Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
     ...
     * @notice Determine what the account liquidity would be if the given amounts were redeemed/borrowed
     * @param cTokenModify The market to hypothetically redeem/borrow in
     * @param account The account to determine liquidity for 借款人账户
     * @param redeemTokens The number of tokens to hypothetically redeem 假设要赎回的代币数量
     * @param borrowAmount The amount of underlying to hypothetically borrow 假设借入的标的资产金额
     * @dev Note that we calculate the exchangeRateStored for each collateral cToken using stored data,
     *  without calculating accumulated interest. 请注意，我们使用存储的数据计算每个抵押品cToken存储的兑换率，而不计算累计利息
     * @return (possible error code,
                hypothetical account liquidity in excess of collateral requirements,
     *          hypothetical account shortfall below collateral requirements)
     */
    function getHypotheticalAccountLiquidityInternal(
        ...
        AccountLiquidityLocalVars memory vars; // Holds all our calculation results 保存我们的计算结果
        uint oErr;
        MathError mErr;
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            // sumCollateral += tokensToDenom * cTokenBalance 存款可抵押价格= 存款的cToken数量xcToken的抵押价格
            (mErr, vars.sumCollateral) = mulScalarTruncateAddUInt(vars.tokensToDenom, vars.cTokenBalance, vars.sumCollateral);
            if (mErr != MathError.NO_ERROR) {
                return (Error.MATH_ERROR, 0, 0);
            }

            // sumBorrowPlusEffects += oraclePrice * borrowBalance 抵押品价格=市场价格x借款数量
            (mErr, vars.sumBorrowPlusEffects) = mulScalarTruncateAddUInt(vars.oraclePrice, vars.borrowBalance, vars.sumBorrowPlusEffects);
            if (mErr != MathError.NO_ERROR) {
                return (Error.MATH_ERROR, 0, 0);
            }

            // Calculate effects of interacting with cTokenModify //如果清算的资产是当前资产
            if (asset == cTokenModify) {
                // redeem effect 偿还
                // sumBorrowPlusEffects += tokensToDenom * redeemTokens
                // 抵押品价格 = 标准转化系数 * 赎回的cToken数量 + 抵押品价格
