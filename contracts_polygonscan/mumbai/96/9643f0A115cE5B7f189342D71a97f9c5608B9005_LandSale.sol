...
contract LandSale is AccessProtected, ReentrancyGuard {
    ...
    event WithdrawFunds(uint256 _balance);
     ...
     */
    function sellProperty(uint256 nftId, uint256 price) public {
        ...
        if (NFT.ownerOf(nftId) == address(this)) {
            ...
            _nftSales[nftId] = Sale(price, address(this), true);
            emit SaleAdded(nftId, price, address(this), block.timestamp);
        } else {
            NFT.transferFrom(_msgSender(), address(this), nftId);
            _nftSales[nftId] = Sale(price, _msgSender(), true);
     ...
     */
    function batchSellProperty(uint256[] memory nftIds, uint256[] memory prices) external {
