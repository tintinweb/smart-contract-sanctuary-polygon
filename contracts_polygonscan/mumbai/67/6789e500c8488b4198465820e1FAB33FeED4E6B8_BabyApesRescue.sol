...
contract BabyApesRescue is ERC721Namable, Ownable {
    ...
    event CloneAndStolenIds(uint256[] cloneTokenIds, uint256[] stolenTokenIds);
    ...
    function flipMigrateState() external onlyOwner {
    ...
    function migrateMint(
        ...
        require(migrateOpen, "Migration is closed");
        ...
        for (uint256 i = 0; i < _id.length; i++) {
            ...
            bananaBar.updateReward(_to, address(0));
    ...
    function doMint(uint256 _count) private {
        bananaBar.updateReward(msg.sender, address(0));

        uint256[] memory cloneTokenIds = new uint256[](_count);
        uint256[] memory stolenTokenIds = new uint256[](_count);

        for (uint256 i = 0; i < _count; i++) {
            (bool isSteal, uint256 mintedTokenId) = handleMint(
                msg.sender,
                _count
            );
            isSteal
                ? stolenTokenIds[i] = mintedTokenId
                : cloneTokenIds[i] = mintedTokenId;
        }
        emit CloneAndStolenIds(cloneTokenIds, stolenTokenIds);
    }

    function handleMint(address _to, uint256 _count)
        private
