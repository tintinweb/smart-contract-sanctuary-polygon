...
contract MarketPlace is Ownable {
    ...
    function exchange( OrderStruct memory makerOrder, OrderStruct memory takerOrder, address royaltyAddress, uint royaltyFee, uint8 executionType ) private returns (bool) {
        ...
        bool exeType = (getPriceByExecutionType(executionType) == 0) ? true/* fixed */ : false/* auction */;
        ...
        if(((makerOrder.asset.tokenType == 0) && (takerOrder.asset.tokenType == 1)) || ((makerOrder.asset.tokenType == 1) && (takerOrder.asset.tokenType == 0))) {
            ...
            if(makerOrder.asset.tokenType == 0) {
                ...
                (platformFee, royaltyFees, price) = calculateFees(
                    (((exeType != makerOrder.isSell) ? makerOrder.asset.quantity : takerOrder.exchangeFor.price) * makerOrder.exchangeFor.price /*no of copies*/),
                    beneficiaryFee,
                    royaltyFee
                );
    ...
    function validateOrder( OrderStruct memory order) private pure returns ( address signer) {
    ...
    function createLazyMintHash( LazyMintStruct memory lazyMint) private pure returns (bytes32) {
        return keccak256(
            abi.encodePacked(
                lazyMint.expiry,
                lazyMint.supply,
    ...
    function getPriceByExecutionType( uint8 executionType) private pure returns ( uint) {
        uint takePriceFrom; 
        if(executionType == 0) { // fixed price
            takePriceFrom = 0; // seller
        }
        else if(executionType == 1){ // auction
            takePriceFrom = 1; // buyer
        }
        return takePriceFrom;
    }
