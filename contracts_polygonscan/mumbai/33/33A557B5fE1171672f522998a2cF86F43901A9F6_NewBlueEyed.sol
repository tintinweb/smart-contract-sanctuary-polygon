...
contract NewBlueEyed is ERC721Enumerable, Ownable {
    ...
    function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable {
        require(!paused, "the contract is paused");
        require(onlyWhitelisted, "Whitelist is false");
        uint256 supply = totalSupply();
        require(_mintAmount > 0, "min mint is 1 NFT");
        require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
    ...
    function OGListMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable {
        require(!paused, "the contract is paused");
        require(onlyOGListed, "OGList is false");
        uint256 supply = totalSupply();
        require(_mintAmount > 0, "min mint is 1 NFT");
        require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
