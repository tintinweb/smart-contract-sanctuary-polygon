...
contract OfferDex is Ownable, DexStorage {
  ...
  function clearOffers(uint256 tokenId, address collection, uint256 index) private {
      ...
      for(uint256 i = 0; i < temp.count; i++){
        offerId = temp.offerIds[i];
        price = temp._offer[offerId].price;
        if(offerId != index){
              token = ALIA;
           if(temp._offer[offerId].currencyType == 1){
               price = price / 1000000000000;
               token = BUSD;
           }else if(temp._offer[offerId].currencyType == 2) {
              token = ETH;
           }
         if(temp._offer[offerId].currencyType == 3){
           chainTransfer(temp._offer[offerId]._address, 1000, price);
         } else {
            token.transfer(temp._offer[offerId]._address, price);
         }
        }
          delete _offers[tokenId][collection]._offer[offerId];
      }
