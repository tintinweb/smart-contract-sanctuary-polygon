...
contract SpaceshipCollection is ERC721, ERC721URIStorage, AccessControl, Initializable {
    using Counters for Counters.Counter;

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    Counters.Counter private _tokenIdCounter;

    constructor(address actualDeployer) ERC721("MyToken", "MTK") {
        _grantRole(MINTER_ROLE, msg.sender);
        _grantRole(DEFAULT_ADMIN_ROLE, actualDeployer);
        _grantRole(MINTER_ROLE, actualDeployer);
    }
    function initialize(string[] memory metadataArray, address to) public initializer returns (bool)
    {
...
contract CollectionFactory is AccessControl {
    ...
    constructor () {
        ...
        deployedContracts[0] = instance;
        _tokenIdCounter.increment();
    }
    function create(string[] memory metadataArray) public onlyRole(MINTER_ROLE) returns (address){
        address cloneAddress = Clones.clone(implementationAddress);
        uint256 pos = _tokenIdCounter.current();
        deployedContracts[pos] = cloneAddress;
        _tokenIdCounter.increment();
