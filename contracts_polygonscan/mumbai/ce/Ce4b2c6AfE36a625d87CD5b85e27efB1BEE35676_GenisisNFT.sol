...
contract GenisisNFT is Ownable, ERC721URIStorage, IERC721Enumerable {
    ...
    constructor() ERC721("Artifest-1010-Genisis-NFT", "GenisisNFT") {

    }

    function addNftBulk(Nft[] memory _array) public onlyOwner() {
        //nfts = nft;
    ...
    function getNFTLength() public view returns (uint){
        return nfts.length;
    }

   // Add new NFTs

    function addNft(string memory uri,  bool mintable, uint256 mintCap, address allowedAddress) external onlyOwner() {
    ...
    function updateNft(uint256 nftID, bool mintable, address allowedAddress) external onlyOwner() {
        Nft storage nft = nfts[nftID];
        nft.mintable = mintable;
        nft.allowedAddress = allowedAddress;
    }

    // Purchase NFT including minting.  Ensure Approval / Allowance to use tokens to purchase NFTs.

    function purchaseNft(uint256 nftID) external returns (bool){
        ...
        require(nft.allowedAddress == msg.sender, "PurchaseNft:: NFT is not allowed to be purchased by this address");
