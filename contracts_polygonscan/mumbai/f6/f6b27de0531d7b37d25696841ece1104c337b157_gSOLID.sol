...
contract gSOLID is IgSOLID, ERC20 {
    ...
    /* ========== EVENTS ========== */
    event LogApprovedContract(address approved);
    ...
    function initialize(address _approvedContract) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");

        require(_approvedContract != address(0), "Approved");
        approved = _approvedContract;

        emit LogApprovedContract(approved);
