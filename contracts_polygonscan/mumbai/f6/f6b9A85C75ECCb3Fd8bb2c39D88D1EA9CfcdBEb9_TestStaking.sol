...
contract TestStaking is IStaking, RewardsAdministrator, ReentrancyGuard, Pausable {
    ...
    function rewardPerToken() public view override returns (uint256) {
        ...
        return rewardPerTokenGlobal.add(eTime.sub(sTime).mul(rate).mul(1e18).div(_totalStakes));
    }
    ...
    function openCampaign(uint256 _periodStart, uint256 _periodFinish, uint256 _rate) external override onlyRewardsAdministrator updateReward(address(0)) {
        require((_periodStart >= block.timestamp || (_periodStart == periodStart && _rate == rate)) && _periodFinish > _periodStart, "Stake: Invalid time!");
        // uint256 rewardsDuration = _periodFinish.sub(_periodStart);
...
abstract contract RewardsAdministrator is Ownable {
    ...
    function openCampaign(uint256 _periodStart, uint256 _periodFinish, uint256 _rate) external virtual;
