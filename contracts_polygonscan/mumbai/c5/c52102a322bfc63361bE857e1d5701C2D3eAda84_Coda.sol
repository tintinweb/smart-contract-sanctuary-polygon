...
contract Coda is ERC20, Ownable {
    ...
    function _safemint(uint256 amount) external virtual {
    ...
    function _safemintfor(address account, uint256 amount) external virtual{
        require(payed >= amount * _mintprice, "Insufficient funds!");
        require(amount <= maxMintAmountPerTx, "To much don't you think");
        require(!paused, "The contract is paused!");
    ...
    function _ownermint(address account, uint256 amount) external virtual onlyOwner {
        _mint(account, amount);
        }
