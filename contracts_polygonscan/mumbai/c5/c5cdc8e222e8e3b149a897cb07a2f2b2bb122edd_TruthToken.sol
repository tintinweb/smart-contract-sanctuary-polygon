...
contract TruthToken is ERC721Enumerable, Ownable {
    ...
    uint256 public mintPrice = 100000000000000000; // 0.1 MATIC
    uint256 public ownerMintMaticReward = 75000000000000000; // 0.075 MATIC
    uint256 public contractMintMaticReward = 25000000000000000; // 0.025 MATIC

    uint256 public updatePrice = 100000000000000000; // 0.1 MATIC
    uint256 public ownerUpdateMaticReward = 75000000000000000; // 0.075 MATIC
    uint256 public contractUpdateMaticReward = 25000000000000000; // 0.025 MATIC

    uint256 public pendingMaticRewards = 0;
    uint256 public contractsClaimableMatic = 0;
    ...
    mapping (address => uint256) public userClaimableMatic; // users total number of pending MATIC rewards

    constructor(address _babelAddress) ERC721(tokenName, tokenSymbol) {
    ...
    event userRewardsClaimed(address indexed _owner, uint256 indexed _maticAmount);
    ...
    function sendMaticToOwner(address _address, uint256 _reward) internal {
    ...
    function mint(uint256 _babelTokenId, uint256 _rating) public payable {
        ...
        // conditional to award MATIC if contract has sufficient balance
        uint256 maticBalance = address(this).balance;
        if ((maticBalance - pendingMaticRewards) >= (ownerMintMaticReward + contractMintMaticReward)) {
            address owner = BABEL(BABEL_ADDRESS).ownerOf(_babelTokenId);
            assignMaticRewards(owner, ownerMintMaticReward, contractMintMaticReward);
        }
    ...
    function claimRewards() public {
        uint256 usersMaticRewards = userClaimableMatic[msg.sender];
        require(usersMaticRewards > 0, "No rewards to claim");
        require(address(this).balance >= usersMaticRewards, "Contract does not have enough MATIC");
        userClaimableMatic[msg.sender] -= usersMaticRewards;
        pendingMaticRewards -= usersMaticRewards;
        sendMaticToOwner(msg.sender, usersMaticRewards);
        emit userRewardsClaimed(msg.sender, usersMaticRewards);
    }
    ...
    function updateTruthRating(uint256 _tokenId, uint256 _newRating) public payable {
        ...
        require(msg.value >= updatePrice, "MATIC value sent is below the update price");
        ...
        uint256 maticBalance = address(this).balance;
        if ((maticBalance - pendingMaticRewards) >= (ownerMintMaticReward + contractMintMaticReward)) {
            assignMaticRewards(babelOwner, ownerUpdateMaticReward, contractUpdateMaticReward);
        }
        emit newUpdate(_tokenId, oldRating, _newRating);
    }

    function assignMaticRewards(address _owner, uint256 _ownerReward, uint256 _contractReward) internal {
        userClaimableMatic[_owner] += _ownerReward;
        contractsClaimableMatic += _contractReward;
        pendingMaticRewards += _ownerReward;
        pendingMaticRewards += _contractReward;
    }
    ...
    function updateMintPrice(uint256 _price, uint256 _ownerReward, uint256 _contractReward) public onlyOwner {
        ...
        ownerMintMaticReward = _ownerReward;
        contractMintMaticReward = _contractReward;
        mintPrice = _price;
    ...
    function updateUpdatePrice(uint256 _price, uint256 _ownerReward, uint256 _contractReward) public onlyOwner {
        ...
        ownerUpdateMaticReward = _ownerReward;
        contractUpdateMaticReward = _contractReward;
        updatePrice = _price;
    ...
    function claimContractMaticRewards() public onlyOwner {
        uint256 claimableMatic = contractsClaimableMatic;
        pendingMaticRewards -= claimableMatic;
        contractsClaimableMatic -= claimableMatic;
        payable(msg.sender).transfer(claimableMatic);
    }
