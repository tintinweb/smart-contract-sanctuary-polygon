...
using SafeMath for uint;
    ...
    function createTournament(string memory _name, uint _pricePerPlayer, uint _playersPerTeam, uint _numberOfTeams,address _prizeToken, uint _daysTillRevert) public onlyOwner returns (uint){
            ...
            uint[] memory tempIds;
            uint[] memory tempJoinedIds;
            uint[] memory tempDr;
            uint[] memory tempPlacements;
            //IERC20 testToken= IERC20(0xfe4F5145f6e09952a5ba9e956ED0C25e3Fa4c7F1);
            Tournament memory t = Tournament(tournamentId,_name, msg.sender,_pricePerPlayer,_playersPerTeam,_numberOfTeams,tempIds,tempJoinedIds,0,0,tempPlacements,IERC20(_prizeToken),tempDr,addMinutes(block.timestamp,_daysTillRevert));
            tournaments.push(t);
    ...
    function createTeam(address _teamOwner, string memory _name, address[] memory _memberAddress) public {
        
        Team memory t = Team(teamId,_teamOwner,_name, _memberAddress);
        teams.push(t);
        teamId++;
    }
    ...
    //only joins no payments ...
    function joinTournament(uint _teamId, uint _tournamentId) public { //, uint _amount
        ...
        //this check is not good it should check if joined and payed completely
    ...
    function payTournament(address _forAddress, uint _teamId, uint _tournamentId, uint _amount) public {
        require(tournaments[_tournamentId].status==1,"Tournament not in registration status");
        require(_amount>=tournaments[_tournamentId].pricePerPlayer, "Insufficient funds");
        require(tournaments[_tournamentId].joinedIds.length<tournaments[_tournamentId].numberOfTeams, "Tournament full");
        bytes memory h=abi.encodePacked(_forAddress,_tournamentId);
        paid[h]+=_amount;
        uint teamPaid=0;
        for(uint i=0;i<teams[_teamId].memberAddress.length;i++){
            h=abi.encodePacked(teams[_teamId].memberAddress[i],_tournamentId);
            teamPaid+=paid[h];
        }

        if (teamPaid>=tournaments[_tournamentId].pricePerPlayer*tournaments[_tournamentId].playersPerTeam){
            tournaments[_tournamentId].joinedIds.push(_teamId); //team paid enough .. add it to joined
        }

        //TOURNAMENT FULL
        if (tournaments[_tournamentId].joinedIds.length<tournaments[_tournamentId].numberOfTeams){
            tournaments[_tournamentId].teamIds=tournaments[_tournamentId].joinedIds;
    ...
    function distributeRewards(uint _tournamentId) public {
        ...
        for (uint i=0;i<tour.distributionRules.length;i++){
            ...
            if (i==0)  //creator payment
            {
                ...
                uint numPlayers=teams[tour.placements[i-1]].memberAddress.length;
                for (uint j=0;j<numPlayers;j++){
                    sendToken(_tournamentId,teams[tour.placements[i-1]].memberAddress[j],amount.div(numPlayers));
                }
        ...
        tournaments[_tournamentId].status=3;
