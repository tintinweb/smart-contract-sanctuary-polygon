...
abstract contract NFTOneofExtention is ERC1155, Pausable, Destroyable, Ownable {
    ...
    modifier mintersOnly(uint256 _id) {
        require(
            minters[_id][msg.sender] == true ||
                (_msgSender() == owner() && minters[_id][address(0x0)] == true), // only approve owner if type exist
    ...
    modifier onlyCreator() {
        require(
            isCreator(_msgSender()) || _msgSender() == owner(),
            "onlyCreator: caller is not an approved creator"
        );
        _;
    }
    ...
    function typeExists(uint256 id)
        public
        view
        virtual
        whenNotDestroyed
    ...
    function setCreator(address operator, bool approved)
        public
        virtual
        onlyOwner
        whenNotDestroyed
    ...
    function isCreator(address account)
        public
        view
        virtual
        whenNotDestroyed
    ...
    function safeTransferFrom(
        ...
        if (
            isPowerMinted(id) &&
            !exists(id) &&
            bytes(NFTOneofExtention._tokenURIs[id]).length == 0
        ) {
            ...
            require(
                getNonFungibleIndex(id) <= power_mint.amount,
                "ERC1155: insufficient balance for transfer"
            );
            require(
    ...
    function _setURI(uint256 tokenId, string memory _tokenURI)
        internal
        virtual
        whenNotDestroyed
        whenNotPaused
...
contract ERC1155AllowanceWrapper is ERC165, IERC1155
{
    ...
    */
    function safeTransferFrom(address _from, address _to, uint256 _id, uint256 _value, bytes calldata _data) external override{
        if (msg.sender != _from) {
            allowances[_from][msg.sender][_id] = allowances[_from][msg.sender][_id] - _value;
        }
    ...
    function safeBatchTransferFrom(address _from, address _to, uint256[] calldata _ids, uint256[] calldata _values, bytes calldata _data) external override {
        if (msg.sender != _from) {
            for (uint256 i = 0; i < _ids.length; ++i) {
                allowances[_from][msg.sender][_ids[i]] = allowances[_from][msg.sender][_ids[i]] - _values[i];
            }
        }
    ...
    */
    function setApprovalForAll(address /*_operator*/, bool /*_approved*/) external override pure {
...
contract NFTOneof is NFTOneofExtention {
    ...
    constructor(string memory _uri) ERC1155(_uri) NFTOneofExtention() {}

    function mintBatch(
        address _to,
        Mint[] calldata requests,
        bytes calldata _data
    ) public onlyCreator whenNotDestroyed whenNotPaused {
        uint256 count = 0;
        ...
        for (uint256 i = 0; i < requests.length; i++) {
            ...
            if (mint._type == 0) {
                ...
                require(
                    typeExists(mint._type),
                    "mintBatch: token type does not exist"
                );
                require(
                    !(isFungible(mint._type) &&
                        bytes(mint.metadata).length == 0),
            ...
            if (isFungible(_type)) {
                ...
                uint256[] memory mintIds = getNonFungibleItems(
                    _type,
                    mint.amount
                );
                for (uint256 j = 0; j < mintIds.length; j++) {
                    ...
                    amounts[index] = 1;
                    ++index;
                    if (mint._type != 0 && bytes(mint.metadata).length > 0) {
    ...
    function powerMint(uint256[] memory counts, string[] memory metadataURIs)
        ...
        returns (uint256[] memory)
    {
        // check thath counts and metadataURIs same length
        require(
            counts.length == metadataURIs.length,
            "PowerMint: counts and metadataURIs length mismatch here"
        );
        uint256[] memory _types = new uint256[](counts.length);
        for (uint256 i = 0; i < counts.length; i++) {
            uint256 _type = initPowerMintingEnabledType(
                counts[i],
                metadataURIs[i]
            );
            _types[i] = _type;
    ...
    function select(PowerMintSelect[] memory requests)
        public
        onlyCreator
        whenNotDestroyed
        whenNotPaused
    {
        for (uint256 i = 0; i < requests.length; i++) {
            ...
            require(
                isNonFungibleItem(request.token_id),
                "Select: token must be non fungible"
            );
            require(
                isPowerMinted(request.token_id),
                "Select: token must be a powerminted type"
            );
            require(
                !exists(request.token_id) &&
                    bytes(NFTOneofExtention._tokenURIs[request.token_id])
                        .length ==
                    0,
            ...
            PowerMintValue memory power_mint = power_mints[_type];
            require(
                getNonFungibleIndex(request.token_id) <= power_mint.amount,
                "Select: Token is out of bounds"
            );
            require(
                !(power_mint.owner == address(0) &&
                    power_mint.amount == 0 &&
    ...
    function transfer(Transfer[] memory requests)
        public
        whenNotDestroyed
        whenNotPaused
    {
    ...
    function mintNonFungible(
    ...
    ) public whenNotDestroyed whenNotPaused mintersOnly(_type) {
        // No need to check this is a nf type rather than an id since
        // creatorOnly() will only let a type pass through.
        require(isNonFungible(_type));
        ...
        for (uint256 i = 0; i < _to.length; ++i) {
            ...
            if (bytes(metadata).length > 0) {
                NFTOneofExtention._setURI(id, metadata);
            }
            _mint(dst, id, 1, "");
    ...
    ) public whenNotDestroyed whenNotPaused mintersOnly(_id) {
