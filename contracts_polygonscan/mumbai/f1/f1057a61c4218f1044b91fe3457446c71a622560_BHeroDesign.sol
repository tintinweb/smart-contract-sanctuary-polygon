    ...
    IBHeroDesign
{
        ...
        returns (uint256 nextSeed, uint256 encodedDetails)
    {
        ...
        (seed, details.color) = Utils.randomRangeInclusive(
            seed,
            1,
            COLOR_COUNT
        );
        (seed, details.skin) = Utils.randomRangeInclusive(seed, 1, SKIN_COUNT);
        (seed, details.speed) = Utils.randomRangeInclusive(
            seed,
            stats.speed.min,
            stats.speed.max
        );
...
library BHeroDetails {
    ...
    function encode(Details memory details) internal pure returns (uint256) {
        ...
        value |= details.clickSkin << 65;
        value |= details.Power << 70;
        value |= details.work << 75;
        ...
        returns (Details memory result)
    {
        ...
        result.clickSkin = (details >> 65) & 31;
        result.Power = (details >> 70) & 31;
        result.work = (details >> 75) & 31;
    ...
    function decodeWork(uint256 details) internal pure returns (uint256) {
        return (details >> 75) & 31;
    }
    ...
    function setWork(uint256 details) internal pure returns (uint256) {
        uint256 work = decodeWork(details);
        details &= ~(uint256(31) << 75);
        details |= (work + 1) << 75;
        return details;
    }

    function setUnWork(uint256 details) internal pure returns (uint256) {
        uint256 work = decodeWork(details);
        details &= ~(uint256(31) << 75);
        details |= (work - 1) << 75;
        return details;
