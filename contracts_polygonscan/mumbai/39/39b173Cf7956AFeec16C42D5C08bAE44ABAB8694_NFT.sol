...
contract NFT is ERC721MintMore, Ownable {
    ...
    function mintWithoutEvent(address _recipient, string memory _name) public onlyOwner {
        require(getUTFStrLen(_name) <= 16, 'setTokenName: Name is too long. Maximum: 16 characters');
        require(charMatch(_name), 'setTokenName: Name can contain only a-z, A-Z, 0-9, space and dot');
    ...
    function mintMore(address _recipient, string memory _name, uint _count) public returns (uint) {
        for (uint i = 0; i < _count; i++) mintWithoutEvent(_recipient, string(abi.encodePacked(_name, ' ', Strings.toString(i))));
        return tokenCount - 1;
    }
