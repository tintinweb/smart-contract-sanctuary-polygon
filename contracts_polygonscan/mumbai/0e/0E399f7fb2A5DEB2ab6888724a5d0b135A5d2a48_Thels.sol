...
contract Thels is Ownable {
    ...
    uint256 public constant FEE = 50; // 5.0%
    ...
    function withdraw(address token, uint256 amount) public {
        ...
        require(
            getCollateralValue(msg.sender) -
                getTokenPrice(allowedTokens[token]) *
                amount >
    ...
    function repayAndClose(address receiver) public {
        repay(debtAmounts[msg.sender]);
        stopStream(receiver);
    }
    ...
    function distributeRewards(uint256 amount) private {
        for (uint256 i = 0; i < deposited.length; i++) {
            depositAmounts[deposited[i]] *= (1 + amount / totalUSDCx);
        }
    }
