...
contract PolygonStakBank is ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function earned(address _account) public view returns (uint256) {
        return balanceOf(_account) * (rewardPerToken() - userInfo[_account].userRewardPerTokenPaid) / (1e18 + userInfo[_account].rewards);
    ...
    function getMultiplier(address _account) public view returns (uint256) {
        ...
        uint256 integerMonthsElapsed = secondsElapsed - ((secondsElapsed % (30 days)) / 30 days);
  ...
  function stake(uint256 _amount)
    ...
    function updateBaseDate(uint256 _amount, address _account) internal {
        ...
        uint256 newBaseDate = (oldBaseDate * oldStake) + (block.timestamp * _amount) / (oldStake + _amount);
    ...
    for (uint i = 0; i < 20; i++) {
        bytes1 b = bytes1(uint8(uint(uint160(acc)) / (2**(8*(19 - i)))));
     ...
     */
    function setRewardTokenContract(IERC20Upgradeable _token) external onlyOwner {
        rewardToken = _token;
    }
     ...
     */
    function setStakingTokenContract(IERC20Upgradeable _token) external onlyOwner {
        stakingToken = _token;
    }
