...
contract CityToken is ERC721URIStorage, Ownable{
   ...
   using Strings for uint256;
    ...
    mapping (uint256 => TokenInfo) private _tokenMeta;
    ...
    function mintNFT(uint256 numberOfNfts, uint256 price, string memory _tokenURI, string memory _name) public payable returns (uint256) {
        ...
        for (uint i = 0; i < numberOfNfts; i++) {
            ...
            _tokenMeta[_tokenIdTracker.current()].uri = _tokenURI;
            _tokenMeta[_tokenIdTracker.current()].sale = false;
            _tokenMeta[_tokenIdTracker.current()].price = price;
            _tokenMeta[_tokenIdTracker.current()].name = _name;
        }
    ...
    */
    ...
    //             result[i] = super.tokenOfOwnerByIndex(_owner, i);
    //         }
    //         return result;
    //     }
    // }

    function tokenMeta(uint256 _tokenId) public view returns (TokenInfo memory) {
        require(_exists(_tokenId));
        return _tokenMeta[_tokenId];
    }

    function _setTokenMeta(uint256 _tokenId, TokenInfo memory _meta) private {
        require(_exists(_tokenId));
    ...
    function getAllOnSale () public view virtual returns( TokenInfo[] memory ) {
        TokenInfo[] memory tokensOnSale = new TokenInfo[](_tokenIdTracker.current());
        uint256 counter = 0;

        for(uint i = 1; i < _tokenIdTracker.current() + 1; i++) {
    ...
    function purchaseToken(uint256 _tokenId) public payable {
        ...
        require(payable(owner()).send((msg.value).mul(loyaltyFee).div(100)));
        address tokenSeller = ownerOf(_tokenId);

        payable(tokenSeller).transfer((msg.value).mul(100-loyaltyFee).div(100));
    ...
    // function getOwnSaleToken(address _owner) external view returns(TokenInfo[] memory) {
    //     uint256 tokenCount = balanceOf(_owner);
    //     require(tokenCount > 0, "Nothing tokens");

    //     uint256 counter = 0;

    //     TokenInfo[] memory saleTokens = new TokenInfo[](tokenCount);
    //     for (uint256 i = 0; i < tokenCount; i++) {
    //         uint256 tokenId = super.tokenOfOwnerByIndex(_owner, i);
    //         if(_tokenMeta[tokenId].sale == true) {
    //             saleTokens[counter] = _tokenMeta[tokenId];
    //             counter++;
    //         }
    //     }
    //     return saleTokens;
