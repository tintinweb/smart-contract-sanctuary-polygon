 ...
 * Post deployment note: contract deployed @ 0xa0d482393727149c789d3325BBB0dcaa5C1710d8
 */ 
contract ColoredTiles_Test_Mumbai is TokenDropBase {
    ...
    constructor(
        ...
        "Colored Tiles | Test (Mumbai)",
        0.01 * 10 ** 18,                                                    // 0.01 ERC20 token
        0xfe4F5145f6e09952a5ba9e956ED0C25e3Fa4c7F1,                         // There is no official ETH token on Mumbai Testnet, this is a Dummy ERC20 token on the Mumbai PoS chain
        0xab900F603A1cA0ed4f452b80F7f9c00206005dc9,                         // One of Paulin's Test wallets
 ...
 */ 
abstract contract TokenDropBase is Context, ReentrancyGuard, Ownable, ERC20FundsHolder, TokenMintingPool {

    string private _name;                           // the name of this drop

    uint256 private _maxPerTx = 5;                  // maximum of tokens that can be minted per transaction
    uint256 private _maxPerAddress = 25;            // maximum of tokens that can be minted by a specific address.
    mapping(address => uint256) _mintsPerAddress;   // keeps track of how many tokens were minted per address.

    bytes32 private _presaleMerkleRoot;             // used to prove that an address has access to the presale.
     ...
     *  - Sets the presale merkle tree root
     */
    constructor(
    ...
    /**
     * @dev Get the presale status.
     */
    function getPresaleStatus() public view returns (string memory) {
        if (_presaleIsOpen && remainingSupplyCount() > 0) {
            return "OPEN";
        }
        return "CLOSED";
    }
    ...
    /**
     * @dev Get the public sale status.
     */
    function getPublicSaleStatus() public view returns (string memory) {
        if (_publicSaleIsOpen && remainingSupplyCount() > 0) {
            return "OPEN";
        }
        return "CLOSED";
    }
    ...
    /**
     * @dev Claim presale tokens.
     * 
     * Requirements:
     *  - sender is allowed to access the presale
     *  - sender hasn't minted the presale yet
     *  - payment is adequate
     *  - see {_sendMintRequest}
     */
    function claimPresaleTokens(bytes32[] memory addressProof, uint256 amount) external nonReentrant onlyPresale {
        address caller = _msgSender();
        bytes32 addressHash = keccak256(abi.encodePacked(caller));
        require(MerkleProof.verify(addressProof, _presaleMerkleRoot, addressHash), "TokenDropBase: Invalid presale access proof.");
        require(!_presaleMinters[caller], "TokenDropBase: Caller already minted during the presale.");
        _presaleMinters[caller] = true;
        _processMintRequest(caller, amount);
    }

    /**
     * @dev Claim public sale tokens.
     * 
     * Requirements:
     *  - payment is adequate
     *  - see {_sendMintRequest}
     */
    function claimTokens(uint256 amount) external nonReentrant onlyPublicSale {
        _processMintRequest(_msgSender(), amount);
    }
