...
contract VaultService is PriceModel, Ownable {
    ...
    function removeLiquidity(uint256 amount) external {
        uint profit = calculateYeild(amount);
        ERC20(_underlyingTokenAddress).transfer(msg.sender, amount.add(profit));
        _lpToken.burn(msg.sender, amount);
        _totalLiquidity = _totalLiquidity.sub(amount);
        _paidInterest = _paidInterest.sub(profit);
        _availableLiquidity = _availableLiquidity.sub(amount.add(profit));
        _updateCumIndexByLiquidity(_totalLiquidity, _availableLiquidity);
    ...
    function updateOnLeverageClose(
        ...
        _paidInterest = _paidInterest.add(interest);
    ...
    function calculateYeild(uint amount) public view returns (uint interestAmount) {
        interestAmount = _paidInterest.mul(amount).div(_lpToken.totalSupply());
    }
    ...
    function getPaidInterest() external view returns (uint256) {
...
contract PositionRepository is Ownable, PoolACL {
   ...
   modifier activePoolPositionOnly(address pool, address trader) {
    ...
    // Returns qu0antity of leverages holders
...
contract PoolService is Ownable {
    ...
    constructor(address addressRepository, address vault, bool isHighRisk) {
        ...
        // Tokens init
        _lToken = LToken(_vaultService.getLPToken());
    ...
    function setLeverageCut( uint cut) external onlyOwner {
        LEVERAGE_CUT = cut;
    }
    ...
    function openPosition(uint256 amount, uint leverage) external {
        require(amount < MAX_DEPOSIT, "NOT MORE in TESTS");
        // move tokens to vault
        uint256 leveragedAmount = amount.mul(leverage);
    ...
    function _closePosition(address holder, address liquidator) internal {
        uint[9] memory closePosition_;
        closePosition_[0] /*balanceBeforeSale*/ =
            _vaultService.getBalance(_underlyingTokenAddress);

        closePosition_[1] /*underlyingAssetAmount*/ = _saleAllTokensExceptVaultToken(holder);

        (closePosition_[2] /*amount*/, closePosition_[3]/*leveragedAmount*/, ) =
            _PositionRepository.getPositionDetails(address(this), holder);

        closePosition_[4] /*totalBalanceInVaultTokens*/ =
            _vaultService.getBalance(_underlyingTokenAddress).add(closePosition_[1]).sub(closePosition_[0]);

        closePosition_[5] /*amountInterested*/ = calcAmountInterested(holder);

        // Amount which should be pushed back to pool
        closePosition_[6] /*backToVault*/ = closePosition_[3].add(closePosition_[5]).sub(closePosition_[2]);
        closePosition_[7] /*amountToReturn*/ =  closePosition_[4].sub(closePosition_[6]);

        closePosition_[8] /*liquidationPremium*/;
        
        if (liquidator == address(0))
            closePosition_[8] = 0;
        else {
            closePosition_[8] = closePosition_[3].mul(LEVERAGE_CUT).div(100);
            if (closePosition_[8] > closePosition_[7])
                closePosition_[8] = closePosition_[7];
        }

        closePosition_[7] = closePosition_[7].sub(closePosition_[8]);

        // Vault move tokens to holder
        _vaultService.updateOnLeverageClose( holder, closePosition_[7], closePosition_[6], closePosition_[5], closePosition_[8], liquidator);
