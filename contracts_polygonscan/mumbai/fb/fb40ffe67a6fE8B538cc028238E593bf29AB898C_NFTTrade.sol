...
contract NFTTrade is Ownable, IERC721Receiver, ReentrancyGuard, EIP712Base, BaseRelayRecipient, Pausable {
    ...
    mapping(address => mapping(uint256 => uint256)) public _nftToBundleId; // NFT contract Address -> TokenID -> BundleId
    ...
    constructor(
        ...
        treasuryAmount = _treasuryAmount * (1e18); // expected to be passed as a normal number, and converted to decimal-adjusted value
    }
    ...
    function setAdmin(address admin, bool enabled) external onlyOwner {
        require(admin != address(0), "Null Address cannot be used");
    ...
    function setTreasuryAddress(address _treasuryAddress) external onlyAdmin {
        require(_treasuryAddress != address(0), "Null Address cannot be used");
        require(!_treasuryAddress.isContract(), "Treasury Address must not be a contract");
        treasuryAddress = _treasuryAddress;
    }

    function setTreasuryAmount(uint256 _treasuryAmount) external onlyAdmin whenNotPaused {
        require(treasuryAmount >= 0, "treasuryAmount has to be greater than or equal to 0");
        treasuryAmount = _treasuryAmount * (1e18);
    }

    function transferToTreasury(address _traderOne, address _traderTwo) internal {
        uint256 perHeadFee = treasuryAmount / (2);
        voxel.safeTransferFrom(_traderOne, treasuryAddress, perHeadFee);
        voxel.safeTransferFrom(_traderTwo, treasuryAddress, perHeadFee);
    }
     ...
     */
    function createNFTBundle(address[] calldata _nftAddresses, uint256[] calldata _nftIds)
        external
        nonReentrant
        whenNotPaused
    {
     ...
     * @param _bundleId - id of the NFT Bundle
     */
    function cancelNFTBundle(uint256 _bundleId) external nonReentrant whenNotPaused {
        ...
        return
            keccak256(
                abi.encode(
                    OFFER_TYPEHASH,
                    offer.offerMaker,
                    offer.bundleIdWanted,
                    offer.bundleIdOffered,
                    offer.timestamp,
                    offer.expiryTime
                )
            );
    ...
    function acceptOffer(
        ...
        require(verify(offer.offerMaker, offer, sigR, sigS, sigV), "Signature data and Offer data do not match");
        ...
        require(
            bundles[offer.bundleIdOffered].owner == offer.offerMaker,
            "The offer maker does not own the bundle offered"
        );
        require(!bundles[offer.bundleIdWanted].isTraded, "One of the Bundles has been already traded");
        require(!bundles[offer.bundleIdOffered].isTraded, "One of the Bundles has been already traded");
        require(bundles[offer.bundleIdWanted].isActive, "One of the Bundles is inactive");
        require(bundles[offer.bundleIdOffered].isActive, "One of the Bundles is inactive");
        address bundle1Owner = bundles[offer.bundleIdWanted].owner;
        require(_msgSender() == bundle1Owner, "Offer can only be accepted by the bundle owner");

        bundles[offer.bundleIdWanted].isTraded = true;
        bundles[offer.bundleIdOffered].isTraded = true;
        bundles[offer.bundleIdWanted].isActive = false;
        bundles[offer.bundleIdOffered].isActive = false;

        uint256[] memory _nftIdsWanted = bundles[offer.bundleIdWanted].tokenIDs;
        address[] memory _nftAddressesWanted = bundles[offer.bundleIdWanted].nftAddresses;
        uint256[] memory _nftIdsOffered = bundles[offer.bundleIdOffered].tokenIDs;
        address[] memory _nftAddressesOffered = bundles[offer.bundleIdOffered].nftAddresses;

        // Transfer NFTs to be traded
        for (uint256 i = 0; i < _nftIdsOffered.length; i++) {
            _nftToBundleId[_nftAddressesOffered[i]][_nftIdsOffered[i]] = 0;
            IERC721(_nftAddressesOffered[i]).transferFrom(address(this), bundle1Owner, _nftIdsOffered[i]);
        }
        for (uint256 i = 0; i < _nftIdsWanted.length; i++) {
            _nftToBundleId[_nftAddressesWanted[i]][_nftIdsWanted[i]] = 0;
            IERC721(_nftAddressesWanted[i]).transferFrom(address(this), offer.offerMaker, _nftIdsWanted[i]);
        }

        // Transfer the Voxel Tokens as fee
        transferToTreasury(bundle1Owner, offer.offerMaker);

        emit Traded(offer.bundleIdWanted, offer.bundleIdOffered, bundle1Owner, offer.offerMaker, block.timestamp);
    }
    ...
    function cancelOffer(
        ...
        Offer memory offer = Offer({
            ...
            bundleIdWanted: bundleIdWanted,
            bundleIdOffered: bundleIdOffered,
            timestamp: timestamp,
            expiryTime: expiryTime
        });
        require(verify(offerMaker, offer, sigR, sigS, sigV), "Signature data and Offer data do not match");
        require(offerMaker == _msgSender(), "Only offer owner can cancel it");
