...
/**
 @title MingHuaYuan NFT 
 @author Jeffrey Lin, Justa Liang, Greg Shen
 */
contract MingHwaYuanArtNFT is ERC721Enumerable, Ownable, EIP712 {
    ...
    uint8 constant MAX_MINT_SUPPLY = 100;
    ...
    constructor(StageInfo memory _initStageInfo, string memory _initBaseURI)
        ERC721("MingHwaYuan", "MHY")
        EIP712("MingHwaYuan", "1")
    {
    ...
    function whitelistMint(bytes calldata signature) external payable {
        require(!whitelistMinted[_msgSender()], "Minted already");
        _verify(signature);
        // check time
        ...
        // check if exceed
        require(
            totalSupply() + 1 <= stageInfo.maxSupply,
            "Whitelist minted out"
        );
        // check fund
    ...
    function publicMint() external payable {
        ...
        require(stageInfo.stageId == 2, "Public mint not started");
        // check time
    ...
    function nextStage(StageInfo memory _stageInfo) external onlyOwner {
        ...
        require(
            _stageInfo.maxSupply <= MAX_MINT_SUPPLY,
