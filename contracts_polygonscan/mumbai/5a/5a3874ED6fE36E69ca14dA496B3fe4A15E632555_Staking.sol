...
contract Staking is Ownable, AccessControl, ReentrancyGuard, Pausable {
    ...
    event StakeVaultPaused();
    event StakeVaultUnpaused();
    ...
    modifier beforeStakeStarted() {
        require(block.timestamp < startStakeTime, "[StakeVault.beforeStakeStarted] Staking period started");
        _;
    }

    modifier afterStakeStarted() {
        require(block.timestamp >= startStakeTime, "[StakeVault.afterStakeStarted] Staking period not yet started");
        _;
    }

    modifier afterStakeEnded() {
        require(block.timestamp >= endStakeTime, "[StakeVault.afterStakeEnded] Staking period not yet ended");
        _;
    }
    ...
    function stakeNaka(uint256 _nakaAmount) external beforeStakeStarted {
        require(_nakaAmount + userInfo[msg.sender].stakeAmount <= userStakeLimit, "[StakeVault.stakeNaka] total stake amount should be less than user limit");
        require(_nakaAmount + poolStakeTotal <= poolStakeLimit, "[StakeVault.stakeNaka] total stake amount should be less than pool limit");
        IERC20 token = IERC20(nakaTokenAddress);
    ...
    function withdrawNaka() external afterStakeEnded nonReentrant {
        ...
        require(0 == unclaimAmount, "[StakeVault.withdrawNaka] unclaim amount should be zero before user can withdraw");
    ...
    function claimReward(uint256 _nakaAmount) external afterStakeStarted nonReentrant {
        uint256 unclaimAmount = getUserUnclaimAmount(msg.sender);
        require(_nakaAmount <= unclaimAmount, "[StakeVault.claimReward] claim amount should be less than user unclaim amount");
    ...
    function getUserStakeAmount(address _userAddress) external view returns (uint256) {
        return userInfo[_userAddress].stakeAmount;
    }

    function getUserClaimedAmount(address _userAddress) external view returns (uint256) {
        return userInfo[_userAddress].claimedAmount;
    }

    function getUserUnclaimAmount(address _userAddress) public view returns (uint256) {
        ...
        uint256 currentDay = (block.timestamp - (block.timestamp % 86400)) / 86400;
        uint256 duration = endDay - startDay;
        uint256 dayPassed = (currentDay - startDay) > duration ? duration : (currentDay - startDay);

        uint256 totalRewardUntilToday = poolStakeLimit * dayPassed * multiplier * user.stakeAmount / duration / poolStakeTotal;
