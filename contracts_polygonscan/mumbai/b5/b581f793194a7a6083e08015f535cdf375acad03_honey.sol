...
contract honey is Context, IERC20, Ownable,Pausable{
  ...
  address public immutable uniswapV2Pair;
    ...
    function _approve(address owner, address spender, uint256 amount) private {
       ...
       function SetHoneyTeamTaxPercent(uint256 Hives) external onlyOwner() {
        _HoneyTeamTax =Hives;
    }
    
    function SetTeamTaxPercent(uint256 TeamAddress) external onlyOwner() {
        _TreasuryTax = TeamAddress;
    }
  ...
  function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
    ...
    function _getCalculatedFees(uint256 amount) internal view returns(  uint256, uint256, uint256) {
        ...
        fee.HiveFee = amount.mul(_HoneyTeamTax).div(100);
    ...
    function SetTreasuryAddress( address Treasure) external onlyOwner{
        Treasury=Treasure;
    }
   function SetTeamAddress (address TeamAcc) external onlyOwner{
       HiveAcc=TeamAcc;
   }
}