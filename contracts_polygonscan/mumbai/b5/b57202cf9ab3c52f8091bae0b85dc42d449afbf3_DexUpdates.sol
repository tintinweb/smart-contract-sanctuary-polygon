...
contract DexUpdates is Ownable, DexStorage {
  ...
  function makeOfferPayable(uint256 tokenId, address collection, uint256 amount, string memory ownerId) isValid(collection) public payable {
   require(amount > 0, "129");
    require(INFT(collection).ownerOf(tokenId) != address(0x0), "131");
    uint256 offerCount = ++_offers[tokenId][collection].count;
    offer memory tempOffer =  offer(msg.sender, ownerId, 3, amount, block.timestamp);
  ...
  // function getLists(uint256 tokenId, address collection) isValid(collection) public returns(address[10] memory, uint256[10] memory){
