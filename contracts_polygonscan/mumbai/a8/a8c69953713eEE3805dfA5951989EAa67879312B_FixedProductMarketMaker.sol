...
contract FixedProductMarketMaker is ERC20, ERC1155Receiver, Initializable {
    ...
    function buy(uint investmentAmount, uint outcomeIndex, uint minOutcomeTokensToBuy, bool isStake) external {
        ...
        if (isStake) {
            ...
            emit FPMMStake(msg.sender, investmentAmount, feeAmount, outcomeIndex, outcomeTokensToBuy);
        } else {
            emit FPMMBuy(msg.sender, investmentAmount, feeAmount, outcomeIndex, outcomeTokensToBuy);
        }
    }

    function sell(uint returnAmount, uint outcomeIndex, uint maxOutcomeTokensToSell, bool isUnstake) external {
        ...
        _conductFeeTransfer(feeAmount);
        if (isUnstake) {
            emit FPMMUnstake(msg.sender, returnAmount, feeAmount, outcomeIndex, outcomeTokensToSell);
        } else {
