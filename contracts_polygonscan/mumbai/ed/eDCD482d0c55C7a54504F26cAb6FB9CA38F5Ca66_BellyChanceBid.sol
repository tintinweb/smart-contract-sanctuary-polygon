...
contract BellyChanceBid is VRFConsumerBase {
    ...
    function createChanceBid(uint256 _totalAmount) public {
        require(
    ...
    function enterChanceBid(uint256 itemId, uint256 _amount) public {
        require(
            msg.sender != address(0),
            "The addres must be diferent than addres zero"
        );
        ...
        BidFor[] storage _bids = bidsForItems[itemId];

        if (_bids.length > 0) {
            _bids[_bids.length] = BidFor(payable(msg.sender), itemId, _amount);
        } else {
            bidsForItems[itemId] = _bids;
        }
    }
        ...
        override
    {
        ...
        //Calcular segun los porcentajes el ganador
    }
}
