...
contract MetaSoccerStaking is Context, Pausable, AccessControl, IERC721Receiver, ERC721Enumerable, ReentrancyGuard
{
    ...
    function withdrawWithoutRewards(uint256 _tokenId) external nonReentrant {
        ...
        require(!isRewardTime(_tokenId), "Reward Time");
    ...
    // TODO - add test
    function hasStakedIds(uint256[] calldata _tokenIds) public view returns (bool[] memory) {
        bool[] memory ret = new bool[](_tokenIds.length);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ret[i] = hasStaked(_tokenIds[i]);
        }
        return ret;
    }
    ...
    function _distributeRewards(address owner, uint256 _tokenId) internal {
        ...
        string memory attributeValue = ScoutEntropyReader(entropyReader).getAttributeFromSeed(rewardsAttribute, _tokenId);
        for (uint256 i = 0; i < rewardsTokens.length; i++) {
            uint256 reward = rewardsByAttribute[rewardsTokens[i]][attributeValue];
            if (reward > 0) {
                IERC20(rewardsTokens[i]).safeTransferFrom(rewardsPool, owner, reward);
            }
        }
    }
  ...
  function getAttributeFromSeed(string memory _attr, uint256 _tokenId) public view returns(string memory) {
    if (keccak256(abi.encodePacked((_attr))) == keccak256(abi.encodePacked(("StartingLevel")))) {
      ...
      if (scoutGenerator[0] == uint(RedeemersTypes.TicketsV2)) {
        string memory lootbox = ticketsV2.voucherTypes(scoutGenerator[1]);
