...
contract DexInterfaceFacet is Modifiers {
    ...
    event DexEnabled(address indexed);
    event DexDisabled(address indexed);
    ...
    function issueFromDexCalldata(
        ...
        require(tl.length > 0, "NO TRACKED TOKENS TO ISSUE");
        ...
        for (uint256 i = 0; i < tl.length; i++) {
            ...
            if (tl.tokens[i] == _fromAddr) {
                require(_fromAmount >= tl.amounts[i], "From amount is not enough.");
        ...
        for (uint256 i = 0; i < tl.length; i++) {
            ...
            require(newBalance >= requiredBalances[i], "Not enough tracked token received");
            //@TODO do something with skims
            //skims[i] = newBalance-requiredBalances[i];
        }

        // return from token leftovers if any
        uint256 fromLeftover = _fromAmount - (fromBalance - fromToken.balanceOf(address(this)));
        if (fromLeftover > 0) {
    ...
    function redeemToDexCalldata(
        ...
        require(portToken.balanceOf(msg.sender) >= _amount, "INSUFFICIENT BALANCE");

        //get tracked token info
        LibTrackedToken.TrackedTokensList memory tl = LibTrackedToken.getActualAmount(_addr, _amount);
        require(tl.length > 0, "NO TRACKED TOKENS TO REDEEM");

        uint256 userBalance = IERC20(_toAddr).balanceOf(msg.sender);
        ...
        for (uint256 i = 0; i < tl.length; i++) {
            ...
            // transfer or allow tracked tokens
            {
                ...
                if (tl.tokens[i] == _toAddr) {
                    ...
                    if (res.length > 0) {
                        require(abi.decode(res, (bool)), "TRACKED TRANSFER FAILED");
                    }
                } else {
                    res = portToken.externalCall(tl.tokens[i], abi.encodeWithSelector(IERC20.approve.selector, _dexAddr, tl.amounts[i]), 0);
                    if (res.length > 0) {
                        require(abi.decode(res, (bool)), "TRACKED ALLOWANCE CHANGE FAILED");
                    }
                }
            }
        }
        ...
        require(userBalance + _toAmount <= IERC20(_toAddr).balanceOf(msg.sender), "Received amount is too small.");
    ...
    function approveTrackedToken(
        ...
        if (res.length > 0) {
            require(abi.decode(res, (bool)), "TRACKED ALLOWANCE RESET FAILED");
        }
        if (_amount > 0) {
    ...
    function swapUsingDexCalldata(
        ...
        require(isDexAllowed(_dexAddr), "Unknown dex address");
        require(_fromAmount > 0, "Zero amount provided");
        require(_fromAddr != _toAddr, "Source and target tokens are the same");

        uint256 minFromBalance = IERC20(_fromAddr).balanceOf(_addr);
        require(minFromBalance >= _fromAmount, "From balance smaller than from amount");
        minFromBalance -= _fromAmount;

        uint256 minToBalance = IERC20(_toAddr).balanceOf(_addr) + _toAmount;

        PortToken token = PortToken(_addr);
        token.externalCall(_dexAddr, _calldata, 0, "Dex call failed.");

        require(IERC20(_fromAddr).balanceOf(_addr) >= minFromBalance, "Too many tokens sent");
        require(IERC20(_toAddr).balanceOf(_addr) >= minToBalance, "Not enough tokens received");
    }
}
...
contract TrackedTokenFacet is Modifiers {
    ...
    event TrackedTokenAdded(address indexed, address, uint256);
    event TrackedTokenRemoved(address indexed, address);
    event TrackedTokenTargetRatioChanged(address indexed addr, address[] trackedTokens, uint256[] targetRatio);
    event IssuedFromTrackedTokens(address indexed sender, address indexed addr, uint256 amount, address[] trackedTokens, uint256[] trackedAmounts);
    event RedeemedToTrackedTokens(address indexed sender, address indexed addr, uint256 amount, address[] trackedTokens, uint256[] trackedAmounts);
    ...
    function removeTrackedToken(address _addr, address _trackedAddr) external onlyTokenManager(_addr) {
        ...
        require(trackedToken.balanceOf(_addr) <= tc.minBalance[_trackedAddr], "WITHDRAW TOKEN FIRST");

        tc.trackedTokens.remove(_trackedAddr);
        tc.targetRatio[_trackedAddr] = 0;
        tc.minBalance[_trackedAddr] = 0;
    ...
    function setTargetRatio(
        ...
        require(_values.length == _l, "ARGUMENT MISMATCH");

        for (uint256 i = 0; i < _l; i++) {
            require(tc.trackedTokens.contains(_trackedTokens[i]), "TOKEN NOT TRACKED");
    ...
    function redeemToTrackedTokens(address _addr, uint256 _amount) external {
        ...
        for (uint256 i; i < tl.length; i++) {
            ...
            if (res.length > 0) {
                require(abi.decode(res, (bool)), "TRACKED TOKEN TRANSFER FAILED");
            }
        }
...
contract Modifiers {
    ...
    modifier onlyTokenManager(address _tokenAddr) {
        require(PortToken(_tokenAddr).manager() == msg.sender, "ONLY MANAGER ALLOWED");
        _;
    }
}
...
library LibPriceInterface {
    ...
    // get amount of base token units per 1 token (10**decimals())
...
library LibTokenWhitelist {
    ...
    event TokenEnabled(address indexed);
    event TokenDisabled(address indexed);
    event TokenConfigChanged(address indexed);
...
library LibTrackedToken {
    ...
    struct TrackedTokensConfig {
        ...
        mapping(address => uint256) minBalance; //todo remove it from here
    ...
    function addTrackedToken(
        ...
        require(tc.trackedTokens.length() <= LibTrackedToken.MAX_TRACKED_TOKENS, "TRACKED TOKEN LIMIT REACHED");
    ...
    function hasTrackedTokenBalance(address _addr) internal view returns (bool) {
        ...
        for (uint256 i = 0; i < _l; i++) {
            if (IERC20(tc.trackedTokens.at(i)).balanceOf(_addr) > tc.minBalance[tc.trackedTokens.at(i)]) {
                return true;
            }
        }
    ...
    function getRealAmount(address _addr, uint256 _amount) internal view returns (TrackedTokensList memory tl) {
        ...
        for (uint256 i; i < tl.length; i++) {
            ...
            if (portTokenSupply > 0) {
                tl.amounts[i] = (IERC20(tl.tokens[i]).balanceOf(_addr) * _amount) / portTokenSupply;
            } else {
                tl.amounts[i] = 0;
            }
        }
