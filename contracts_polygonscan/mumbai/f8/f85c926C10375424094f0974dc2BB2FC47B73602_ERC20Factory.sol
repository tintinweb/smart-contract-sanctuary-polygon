...
contract ERC20Factory is Ownable {
    ...
    uint256 public nextId = uint256(1);
    ...
    function createERC20(uint256 _id, bytes memory _implementationData)
        external
        payable
    {
...
contract ERC20Lib is IERC20, Ownable {
    ...
    function init(bytes calldata _implementationData) external {
        require(initialized == false, "Contract already initialized");

        (name, symbol, owner, totalSupply, decimals, tokenURL) = abi.decode(
            _implementationData,
            (string, string, address, uint256, uint8, string)
        );
    ...
    function updateTokenURL(string memory _tokenURL) onlyOwner external  {
        tokenURL = _tokenURL;
    }
