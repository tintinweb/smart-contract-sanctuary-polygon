...
contract ChessBettor is Ownable {
  ...
  function createGame(string memory _gameId, uint256 _amount) internal {
    ...
    newGame.voteList[newGame.aI].push(Vote(address(0), 0, 0, block.timestamp, true));
  ...
  function placeBet(string memory _gameId, uint8 _vote, uint256 _amount) public payable {
    ...
    require(game.numberBets < maxBettorsPerGame, "Maximum number of bettors have placed bets for this game.");
    ...
    game.voteList[game.aI].push(Vote(msg.sender, _vote, _amount, block.timestamp, false));
  ...
  function formatData(
    ...
    for (uint256 i = minIndex; i <= maxIndex; i++) {
      ...
      fullPlayerData.allGameData[index] = ReturnableGameSpecificUserData(returnableGameData, Vote(vote.player, vote.voteFor, vote.amount, vote.voteTimestamp, vote.claimed), result);
      index += 1;
    }
  ...
  function getGameDetails(string memory _gameId) public view returns (ReturnableGameData memory, Vote[] memory) {
    ...
    ReturnableGameData memory returnableGameData = ReturnableGameData(
      ...
      _game.whiteBets,
      _game.blackBets,
      _game.validTotalAmount,
      _game.validAmountWhite,
      _game.validAmountBlack
    );
