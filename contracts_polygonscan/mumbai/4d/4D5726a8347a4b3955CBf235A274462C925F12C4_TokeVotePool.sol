...
interface IManager {
    ...
    function updateBalance(address account, address token, uint256 amount, bool stateSync) external;
...
contract TokeVotePool is ILiquidityPool, Initializable, ERC20, Ownable, Pausable {
    ...
    function triggerBalanceUpdateEvent(address[] memory _addresses) public {
        ...
        for (uint256 i = 0; i < _addresses.length; i++) {
            ...
            sendBalanceUpdate(_addresses[i]);
        }
    ...
    function _deposit(
        ...
        sendBalanceUpdate(toAccount);
    }


    function sendBalanceUpdate(address _user) private {
        uint256 userBalance = balanceOf(_user).sub(requestedWithdrawals[_user].amount);
        manager.updateBalance(_user, address(this), userBalance, true);
    }
