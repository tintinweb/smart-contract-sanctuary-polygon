...
import {IMarketProxy} from "./interfaces/IMarketProxy.sol";
...
import {ICanvasManagerV1} from "./interfaces/ICanvasManagerV1.sol";
...
/**
 * @title ZoraMarketProxy
 * @author Matrix
 */
contract ZoraMarketProxy is IMarketProxy, AccessControl {
    ...
    address internal immutable _nft; // address of nft contract
    IAuctionHouseV2 internal _defaultMarket; // default address of zora market
    IAuctionHouseV1 internal _oldMarket;
    ICanvasManagerV1 internal _oldCanvasManager; // old CanvasManager contract

    address internal _defaultCurrency; // default auction currency
    uint256 internal _defaultPrice; // default auction price
    uint256 internal _defaultDuration = 604800; // default auction duration, 604800 is 7 days

    uint256 internal _minNftId;
    uint256 internal _maxNftId;

    mapping(uint256 => AuctionInfo) internal _auctions; // nft token id => auction info
    uint256[] internal _tokenIdsInMarket;
    ...
    constructor(
        ...
        address market,
        address oldMarket,
        address oldCanvasManager,
        address currency,
        uint256 defaultPrice,
        uint256 minNftId
    ) {
        require(nft != address(0), "zero nft");
        ...
        _nft = nft;
        _defaultMarket = IAuctionHouseV2(market);
        _oldMarket = IAuctionHouseV1(oldMarket);
        _oldCanvasManager = ICanvasManagerV1(oldCanvasManager);

        _defaultCurrency = currency;
        _defaultPrice = defaultPrice;

        _minNftId = minNftId;
        _maxNftId = minNftId + 1000;

        emit NewMarketProxy(nft, market, oldMarket, oldCanvasManager, minNftId);
    }
        ...
        )
    {
        return (
            _nft, // nft
            address(_defaultMarket), // market
            address(_oldMarket), // oldMarket
            address(_oldCanvasManager), // oldCanvasManager
    ...
    function setMinMaxTokenId(uint256 minId, uint256 maxId) external onlyAdmin {
        require(minId < maxId, "minId <= maxId");

        _minNftId = minId;
        _maxNftId = maxId;

        emit SetMinMaxTokenId(minId, maxId);
    }

    function setOldCanvasManager(address newCanvasManager) external onlyAdmin {
        require(newCanvasManager != address(0), "zero newCanvasManager");

        address oldCanvasManager = address(_oldCanvasManager);
        _oldCanvasManager = ICanvasManagerV1(newCanvasManager);

        emit SetOldCanvasManager(oldCanvasManager, newCanvasManager);
    }

    function setOldMarket(address newMarket) external onlyAdmin {
        require((address(_oldCanvasManager) == address(0)) || (newMarket != address(0)), "zero newMarket");

        address oldMarket = address(_oldMarket);
        _oldMarket = IAuctionHouseV1(newMarket);

        emit SetOldMarket(oldMarket, newMarket);
    }

    function setDefaultMarket(address newDefaultMarket) external onlyAdmin {
        require(newDefaultMarket != address(0), "zero default market");

        address oldDefaultMarket = address(_defaultMarket);
        _defaultMarket = IAuctionHouseV2(newDefaultMarket);

        emit SetDefaultMarket(oldDefaultMarket, newDefaultMarket);
    }

    function setDefaultCurrency(address newDefaultCurrency) external onlyAdmin {
        require(newDefaultCurrency != address(0), "zero default currency");

        address oldDefaultCurrency = _defaultCurrency;
        _defaultCurrency = newDefaultCurrency;

        emit SetDefaultCurrency(oldDefaultCurrency, newDefaultCurrency);
    }

    function setDefaultPrice(uint256 newDefaultPrice) external onlyAdmin {
        require(newDefaultPrice != 0, "zero default price");

        uint256 oldDefaultPrice = _defaultPrice;
        _defaultPrice = newDefaultPrice;

        emit SetDefaultPrice(oldDefaultPrice, newDefaultPrice);
    }

    function setDefaultDuration(uint256 newDefaultDuration) external onlyAdmin {
        require(newDefaultDuration != 0, "zero default duration");

        uint256 oldDefaultDuration = _defaultDuration;
        _defaultDuration = newDefaultDuration;

        emit SetDefaultDuration(oldDefaultDuration, newDefaultDuration);
    }

    function getOwnerTokens() external view returns (uint256[] memory tokenIds) {
        tokenIds = ERC721Tradable(_nft).ownerTokens(address(this));
    }

    function getTokenIdsInMarket() external view returns (uint256[] memory tokenIds) {
        tokenIds = _tokenIdsInMarket;
    }

    function getNftByOwner(address owner) external view returns (uint256[] memory tokenIds, uint256[] memory auctionIds) {
        (uint256[] memory tokenIds1, uint256[] memory auctionIds1) = _defaultMarket.getNftByOwner(_nft, owner);
        uint256[] memory tokenIds2 = ERC721Tradable(_nft).ownerTokens(owner);

        tokenIds = tokenIds1.merge(tokenIds2);

        auctionIds = new uint256[](tokenIds.length);
        for (uint256 i = 0; i < tokenIds1.length; i++) {
            auctionIds[i] = auctionIds1[i];
        }
    }

    function getAuctionInfo(uint256 nftId) external view returns (AuctionInfo memory info) {
        if (_contain(nftId)) {
            info = _auctions[nftId];

            info.owner = IERC721(_nft).ownerOf(nftId);
            if (info.owner == address(this)) {
                info.state = 0;
            } else if (info.owner == info.market) {
                info.state = 1;
            } else {
                info.state = 2;
            }
        } else if (address(_oldCanvasManager) != address(0)) {
            (info.auctionId, info.reservePrice, info.beginTime, info.duration, info.owner, info.currency, info.state, info.approved) = _oldCanvasManager
                .getArtAuctionInfo(nftId);
        }
    }
    ...
    {
        IBidDbV2 bidDbV2 = IBidDbV2(_defaultMarket.getBidDb());
        auctionId = bidDbV2.getAuctionId(_nft, tokenId);

        if (auctionId != 0) {
            // get result from new market at first
            market = address(_defaultMarket);
            (auction, status, bids) = bidDbV2.getAuction(auctionId);
        } else if (address(_oldCanvasManager) != address(0)) {
            // try to get auctionId from CanvasManager
            address artAuction = _oldCanvasManager.getArtAuctionAddress(tokenId);
            auctionId = IArtAuction(artAuction).getArtAuctionId();

            if (auctionId != type(uint256).max) {
                // get result from old market
                market = address(_oldMarket);
                address bidDbV1 = _oldMarket.bidDb();
                ...
                if (state == 0) {
                    ...
                    revert("no auctionId when state is 1 or 2");
                }
            }
        }
    }

    function getOpenAuctionsInOldMarket() external view returns (uint256[] memory auctionIds, IAuctionHouse.Auction[] memory auctions) {
        if (address(_oldCanvasManager) != address(0)) {
            uint256 count = 0;
            uint256[] memory endArts = _oldCanvasManager.getEndArts();

            for (uint256 i = 0; i < endArts.length; i++) {
                uint256 tokenId = endArts[i];
                (, , , , , , uint8 state, ) = _oldCanvasManager.getArtAuctionInfo(tokenId);
                if (state == 1) {
                    count++;
                }
            }
            ...
            if (count > 0) {
                ...
                auctions = new IAuctionHouse.Auction[](count);
                for (uint256 i = 0; i < endArts.length; i++) {
                    uint256 tokenId = endArts[i];
                    (uint256 auctionId, , , , , , uint8 state, ) = _oldCanvasManager.getArtAuctionInfo(tokenId);
                    if (state == 1) {
                        auctionIds[index] = auctionId;
                        auctions[index] = _oldMarket.auctions(auctionId);
                        index++;
                    }
                }
            }
    ...
    function createAuction(
        ...
        address owner = IERC721(_nft).ownerOf(nftId);

        if (owner == address(this)) {
            _onlyAdmin();
        } else {
            require((msg.sender == owner) || (msg.sender == IERC721(_nft).getApproved(nftId)), "Caller must be owner or approver");
        }

        address market = address(_defaultMarket);
        IERC721(_nft).approve(market, nftId);

        currency = (currency == address(0)) ? _defaultCurrency : currency;
        reservePrice = (reservePrice == 0) ? _defaultPrice : reservePrice;
        duration = (duration == 0) ? _defaultDuration : duration;

        uint256 auctionId = IAuctionHouseV2(market).createAuction(
            nftId, // tokenId
            _nft, // tokenContract
            duration, // duration
            reservePrice, // reservePrice
            payable(address(this)), // curator
            curatorFeePercentages, // curatorFeePercentages
            currency // auctionCurrency
        );

        {
            AuctionInfo storage info = _auctions[nftId];
            info.auctionId = auctionId;
            info.beginTime = block.timestamp;
            info.market = market;
            info.currency = currency;
            info.reservePrice = reservePrice;
            info.duration = duration;
            info.owner = owner;
            info.curatorFeePercentages = curatorFeePercentages;
            info.approved = true;
            info.state = 1;
        }
        ...
        emit CreateAuction(nftId, market, currency, reservePrice, duration, curatorFeePercentages, auctionId);

        return auctionId;
    }

    function setAuctionApproval(uint256 nftId, bool newApproved) external {
        if (_contain(nftId)) {
            AuctionInfo memory info = _auctions[nftId];
            _onlyAdminOrOwner(info.owner);
            require(info.state == 1, "SAA: not in auction");

            IZoraAuctionHouse(info.market).setAuctionApproval(info.auctionId, newApproved);
            _auctions[nftId].approved = newApproved;

            emit SetAuctionApproval(nftId, info.auctionId, info.approved, newApproved);
        } else if (address(_oldCanvasManager) != address(0)) {
            _onlyAdmin();
            _oldCanvasManager.setArtAuctionApproval(nftId, newApproved);
        }
    }

    function setAuctionReservePrice(uint256 nftId, uint256 newReservePrice) external {
        require(newReservePrice > 0, "zero reserve price");

        if (_contain(nftId)) {
            AuctionInfo memory info = _auctions[nftId];
            _onlyAdminOrOwner(info.owner);
            require(info.state == 1, "SARR: not in auction");

            IZoraAuctionHouse(info.market).setAuctionReservePrice(info.auctionId, newReservePrice);
            _auctions[nftId].reservePrice = newReservePrice;

            emit SetAuctionReservePrice(nftId, info.auctionId, info.reservePrice, newReservePrice);
        } else if (address(_oldCanvasManager) != address(0)) {
            _onlyAdmin();
            _oldCanvasManager.setArtAuctionReservePrice(nftId, newReservePrice);
        }
    }
    ...
    function cancelAuction(uint256 nftId) external {
        if (_contain(nftId)) {
            AuctionInfo memory info = _auctions[nftId];
            _onlyAdminOrOwner(info.owner);
            require(info.state == 1, "CA: not in auction");

            address tokenOwner = IERC721(_nft).ownerOf(nftId);
            if (tokenOwner == info.market) {
                IZoraAuctionHouse(info.market).cancelAuction(info.auctionId);
                tokenOwner = IERC721(_nft).ownerOf(nftId);
            }

            if (tokenOwner != info.market) {
                _removeTokenId(nftId);

                if (tokenOwner == address(this)) {
                    // action is canceled
                    delete _auctions[nftId];

                    emit CancelAuction(nftId, info.auctionId);
                } else {
                    // action is end
                    _auctions[nftId].state = 2;

                    emit EndAuction(nftId, info.auctionId);
                }
            }
        } else if (address(_oldCanvasManager) != address(0)) {
            _onlyAdmin();
            _oldCanvasManager.cancelArtAuction(nftId);
        }
    }

    /**
     * @notice Anyone can call this function to end auction, emits CancelAuction or EndAuction event.
     */
    function endAuction(uint256 nftId) external {
        if (_contain(nftId)) {
            AuctionInfo memory info = _auctions[nftId];
            _onlyAdminOrOwner(info.owner);
            require(info.state == 1, "EA: not in auction");

            address tokenOwner = IERC721(_nft).ownerOf(nftId);
            if (tokenOwner == info.market) {
                IZoraAuctionHouse(info.market).endAuction(info.auctionId);
                tokenOwner = IERC721(_nft).ownerOf(nftId);
            }
            ...
            _onlyAdmin();
            _oldCanvasManager.endArtAuction(nftId);
        }
    }

    function getMaxClaimAmount(address asset) external view returns (uint256) {
        address token = (asset == address(0)) ? _defaultCurrency : asset;
    ...
    function claim(
        ...
        if (amount == 0) {
            ...
            require(amount <= oldBalance, "claim too much");
        }

        IERC20(asset).safeTransfer(to, amount);
        uint256 newBalance = IERC20(asset).balanceOf(address(this));

        emit Claim(asset, to, amount, oldBalance, newBalance);
    }
    ...
    function onERC721Received(
        ...
        emit OnERC721Receive(operator, from, tokenId);
    ...
    function _onlyAdminOrOwner(address owner) internal view {
        if (owner == address(this)) {
            _onlyAdmin();
        } else {
    ...
    function _contain(uint256 tokenId) internal view returns (bool) {
        if (tokenId > _minNftId && tokenId <= _maxNftId) {
            return true;
        }

        for (uint256 i = 0; i < _tokenIdsInMarket.length; i++) {
            if (_tokenIdsInMarket[i] == tokenId) {
                return true;
            }
        }
    ...
    function _addTokenId(uint256 tokenId) internal {
        ...
        _tokenIdsInMarket.push(tokenId);
    }

    function _removeTokenId(uint256 tokenId) internal {
        for (uint256 i = 0; i < _tokenIdsInMarket.length; i++) {
            if (_tokenIdsInMarket[i] == tokenId) {
                _tokenIdsInMarket[i] = _tokenIdsInMarket[_tokenIdsInMarket.length - 1];
                _tokenIdsInMarket.pop();
                return;
            }
        }
    }
 ...
 * @author Matrix
 */
interface IMarketProxy is IERC721Receiver {
    // ==================== Structs ====================
    ...
    struct AuctionInfo {
        ...
        bool approved;
        uint8 curatorFeePercentages;
        uint8 state; // 0: nft in this contract; 1: nft in market; 2: other
    }

    // ==================== Events ====================

    event NewMarketProxy(address indexed nft, address indexed market, address oldMarket, address oldCanvasManager, uint256 minNftId);

    event SetMinMaxTokenId(uint256 minId, uint256 maxId);
    event SetOldCanvasManager(address indexed oldCanvasManager, address indexed newCanvasManager);
    event SetOldMarket(address indexed oldMarket, address indexed newMarket);
    event SetDefaultMarket(address indexed oldMarket, address indexed newMarket);
    event SetDefaultCurrency(address indexed oldCurrency, address indexed newCurrency);
    event SetDefaultPrice(uint256 oldPrice, uint256 newPrice);
    event SetDefaultDuration(uint256 oldDuration, uint256 newDuration);
    ...
    event SetAuctionCurrency(uint256 indexed tokenId, address indexed oldCurrency, address indexed newCurrency);
    event SetAuctionApproval(uint256 indexed tokenId, uint256 indexed auctionId, bool oldApproved, bool newApproved);
    event SetAuctionReservePrice(uint256 indexed tokenId, uint256 indexed auctionId, uint256 oldReservePrice, uint256 newReservePrice);
    event CancelAuction(uint256 indexed tokenId, uint256 indexed auctionId);
    event EndAuction(uint256 indexed tokenId, uint256 indexed auctionId);
    event Claim(address indexed asset, address indexed to, uint256 amount, uint256 oldBalance, uint256 newBalance);

    event OnERC721Receive(address indexed operator, address indexed from, uint256 indexed tokenId);
    ...
    function setMinMaxTokenId(uint256 minId, uint256 maxId) external;

    function setDefaultMarket(address newDefaultMarket) external;

    function setDefaultCurrency(address newDefaultCurrency) external;

    function setDefaultPrice(uint256 newDefaultPrice) external;

    function setDefaultDuration(uint256 newDefaultDuration) external;
    ...
    function claim(address asset, address to, uint256 amount) external; // prettier-ignore
}
...
/**
 * @title ICanvasManagerV1
 * @author Matrix
 */
interface ICanvasManagerV1 {
    // ========== External functions ==========
