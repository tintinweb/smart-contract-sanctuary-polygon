...
contract Moments is ERC1155PresetMinterPauser, VRFConsumer {
    ...
    event MomentsCollectionMinted(uint256 id);
    ...
    mapping(uint256 => CollectionSupply) _collectionSupplies;

    /**
     * Mint a moments NFT collection with a chance to win an epic
     * version of the NFT.
     */
    function mintMomentsCollection(
        address to,
        uint256 supply,
        uint256 epicSupply
        ...
        CollectionSupply memory collectionSupply = CollectionSupply(
            supply,
            epicSupply
        );

        _collectionSupplies[collectionId] = collectionSupply;

        mint(to, collectionId, supply, "");
        mint(to, epicCollectionId, epicSupply, "");

        emit MomentsCollectionMinted(collectionId);
        return collectionId;
    }

    bytes32 public constant SELLER_ROLE = keccak256("SELLER_ROLE");

    struct RandomTransferRequest {
        address from;
        address to;
        uint256 collectionId;
    }

    event RandomTransferRequested(
        address from,
        address to,
        uint256 collectionId,
        uint256 requestId
    ...
    mapping(uint256 => RandomTransferRequest) _randomTransferRequests;
    mapping(uint256 => uint256) _activeRequestsForCollection;

    function transferRandomNftFromCollection(uint256 collectionId, address to)
        public
    {
        require(
            hasRole(MINTER_ROLE, _msgSender()),
            "Moments: must have seller role to random transfer"
        );

        // If no epic NFT is left, transfer a regular one and return
        uint256 epicsLeft = balanceOf(_msgSender(), collectionId + 1);
        if (epicsLeft == 0) {
            return safeTransferFrom(_msgSender(), to, collectionId, 1, "");
        }
        ...
        require(
            _activeRequestsForCollection[collectionId] < supplyLeft + epicsLeft,
            "Moments: no more NFTs available for random transfer"
        );

        _activeRequestsForCollection[collectionId] =
            _activeRequestsForCollection[collectionId] +
            1;

        uint256 requestId = requestRandomness();

        RandomTransferRequest memory request = RandomTransferRequest(
            _msgSender(),
            to,
            collectionId
        );

        _randomTransferRequests[requestId] = request;

        emit RandomTransferRequested(_msgSender(), to, collectionId, requestId);
    }

    event RandomTransferRequestFulfilled(
        address from,
        address to,
        uint256 collectionId,
        uint256 requestId
    ...
    function fulfillRandomness(uint256 requestId, uint256 randomness)
        ...
        override
    {
        RandomTransferRequest memory request = _randomTransferRequests[
            requestId
        ];

        emit RandomTransferRequestFulfilled(
            request.from,
            request.to,
        ...
        _activeRequestsForCollection[request.collectionId] =
            _activeRequestsForCollection[request.collectionId] -
            1;
        ...
        CollectionSupply memory collectionSupply = _collectionSupplies[
            request.collectionId
        ];

        bool shouldTransferEpic = randomness % collectionSupply.supply <
            collectionSupply.epicSupply;

        if (shouldTransferEpic) {
            return
                _safeTransferFrom(
                    request.from,
                    request.to,
