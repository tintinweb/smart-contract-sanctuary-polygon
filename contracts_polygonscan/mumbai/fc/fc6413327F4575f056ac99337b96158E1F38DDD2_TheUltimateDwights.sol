...
import "./../Central-OpenZeppelin/open/contracts/token/ERC721/ERC721.sol";
import "./../Central-OpenZeppelin/open/contracts/access/Ownable.sol";
import "./../Central-OpenZeppelin/open/contracts/utils/Counters.sol";
import "./../Central-OpenZeppelin/open/contracts/token/ERC721/extensions/ERC721Enumerable.sol";
import "./../Central-OpenZeppelin/open/contracts/token/ERC721/extensions/ERC721URIStorage.sol";


pragma solidity ^0.8.0;

contract TheUltimateDwights is ERC721Enumerable, Ownable {
    
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant MAX_THUDs = 7500;
    ...
    Counters.Counter private _publicTHUD;

    constructor() ERC721("TheUltimateDwights", "THUD") {}
...
// $$\   $$ |$$ |         $$ |      $$ |   $$ |      $$ |  $$ | \____$$\ 
// \$$$$$$  |$$$$$$$$\    $$ |      $$ |   $$$$$$$$\ $$ |  $$ |$$$$$$$  |
//  \______/ \________|   \__|      \__|   \________|\__|  \__|\_______/ 
                                                                      
                                                                                                                                 
    function makeContractActive(bool isActive) external onlyOwner {
        _isActive = isActive;
    }
    ...
    function mint(uint256 numberOfTokens) external payable {
        ...
        require(
            _publicTHUD.current() < MAX_THUDs,
            "Purchase would exceed MHC_MAX"
        );
        require(
            minPrice * numberOfTokens <= msg.value,
            "ETH amount is not sufficient"
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicTHUD.current();
            if (_publicTHUD.current() < MAX_THUDs) {
                _publicTHUD.increment();
...
//  \______/ \________|   \__|      \__|   \________|\__|  \__|\_______/ 
    ...
    function getTotalMintedTokens() external view returns(uint){
        return getTotalMintSupply();
    }

    function getMintedTokensForUser(address _user) external view returns(uint256 [] memory){
        uint balance = balanceOf(_user);
        uint256[] memory response = new uint256[](1);
        for(uint i=0; i<balance; i++){
            response[i]=tokenOfOwnerByIndex(_user, i);
        }
        return response;
    }
    ...
    function getTotalMintSupply() public view returns(uint){
        return _publicTHUD.current();
    }
    ...
    function withdraw(address someAddress) external onlyOwner {
        uint256 balance = address(this).balance;
        if (someAddress != address(0)){
            payable(someAddress).transfer(balance);
        }
