...
contract Child {
    ...
    //矿山总储备量
    uint private ore_total_reserves;

    //矿山保有量
    uint private ore_inventory;
    ...
    constructor(uint _data, uint _index){
       ...
       for(uint i=0;i<uruOres.length; i++){
         ore_total_reserves += uruOres[i];
       }
    }
    ...
    function getOreTotalReserves() external view returns (uint){
      return ore_total_reserves;
    }

    function getOreInventory() external view returns (uint){
