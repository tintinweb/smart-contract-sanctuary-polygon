...
contract XNToken is Initializable, ERC20PresetMinterPauserUpgradeable, UUPSUpgradeable, OwnableUpgradeable {
  bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");

  modifier validDestination(address to) {
    require(to != address(0x0), "Cannot mint to 0 address");
  ...
  function transferMultiple(address[] memory dests, uint256[] memory values) public onlyOwner
  returns (uint256) {
      uint256 i = 0;
    ...
    validDestination(to)
  {
    require(
      ...
      "Minter role is required to mint"
    );
    _mint(to, amount);
  }
}