...
contract OfferDex is Ownable, DexStorage {
  ...
  function rejectOffer(uint256 tokenId, address collection, uint256 index) isValid(collection) public {
    ...
    require(INFT(collection).ownerOf(tokenId) == msg.sender || _saleTokens[tokenId][collection].seller == msg.sender || _auctionTokens[tokenId][collection].seller == msg.sender, "not owner of token");
    for(uint256 i=0; i< tempCount; i++){
