...
contract Staking is Ownable {
    ...
    */
    function addUserStake(uint256 amount, uint256 months) internal {
        ...
        if (months == 1) {
            stakesOfUser[msg.sender][_currentId].withdrawalTime = currentTime + 60;
        } else if (months == 3) {
            stakesOfUser[msg.sender][_currentId].withdrawalTime = currentTime + 120;
        } else if(months == 6) {
            stakesOfUser[msg.sender][_currentId].withdrawalTime = currentTime + 180;
        } else if(months == 12) {
            stakesOfUser[msg.sender][_currentId].withdrawalTime = currentTime + 240;
        } else {
