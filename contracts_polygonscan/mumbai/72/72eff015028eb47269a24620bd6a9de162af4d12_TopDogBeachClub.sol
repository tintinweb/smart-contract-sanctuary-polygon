...
contract TopDogBeachClub is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function mintPublic(uint256 amount) external payable nonReentrant {
        ...
        require(
            amount > 0 && amount <= DOGS_PER_TX,
            "You can't mint that many doggos"
        );
        require(
            totalSupply() + amount <= MAX_DOGS,
            "Mint would exceed max supply of doggos"
        );
    ...
    function mintFoundingMember() external payable nonReentrant {
        ...
        require(
            _foundingMemberClaims[msg.sender] !=
                FounderMemberClaimStatus.Claimed,
            "You've already claimed your doggo"
        );
        require(
            _foundingMemberClaims[msg.sender] ==
                FounderMemberClaimStatus.Unclaimed,
            "You are not a founding member"
        );
        require(
            totalSupply() + 1 < PRESALE_MAX_DOGGOS,
            "Mint would exceed max pre-sale doggos"
        ); // should never happen but a saftey net
        require(
            totalSupply() + 1 <= MAX_DOGS,
            "Mint would exceed max supply of doggos"
        );
    ...
    function addFoundingMembers(address[] memory members) external onlyOwner {
        for (uint256 i = 0; i < members.length; i++) {
            _foundingMemberClaims[members[i]] = FounderMemberClaimStatus
                .Unclaimed;
        }
    }
    ...
    function getBirthday(uint256 tokenId) external view returns (uint256) {
        require(
            tokenId < totalSupply(),
            "That doggo has not been born yet :-("
        );
