...
import "SWUtils.sol";
interface IAuth { function canDo(bytes32 ContractName, bytes32 FuncName, address Sender) external view returns (bool); }
contract AddrMgr {
    using SWUtils for *;
    ...
    function _canDo(bytes32 ContractName, bytes32 FuncName, address Sender) internal view returns (bool) {
        try IAuth(get("AuthMgr")).canDo(ContractName, FuncName, Sender) returns (bool res) { return res; }
        catch {
            try IAuth(get("PANICAuthMgr")).canDo(SMName, FuncName, msg.sender) returns (bool res) { return res; }
