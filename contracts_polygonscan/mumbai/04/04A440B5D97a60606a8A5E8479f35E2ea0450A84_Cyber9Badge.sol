...
contract Cyber9Badge is ERC721Enumerable, Ownable, ReentrancyGuard{
    ...
    mapping(uint256 => uint256) private freeMintsPerBadge;
    mapping(uint256 => string) private badgeURI;
    ...
    function mint(uint256 _amount) external payable maxMintPerAddress(_amount) {
        ...
        for(uint256 i=1; i<_amount+1;i++) {        
            ...
            freeMintsPerBadge[amountMinted + 1] = 1;
            badgeURI[amountMinted + 1] = baseURI;
            amountMinted++;
        }
    }
    ...
    returns (string memory) {
        ...
        return bytes(badgeURI[_tokenId]).length >0 ? string(abi.encodePacked(badgeURI[_tokenId], "1.json")) : "";
    }
    
    function freeMintsLeft(uint256 _tokenId) external view returns (uint256){
        require(_exists(_tokenId),"ERC721Metadata: URI query for nonexistent token");
        return freeMintsPerBadge[_tokenId];
    }
    ...
    function useFreeMint(uint256 tokenId, address _from) external nonReentrant {
        ...
        require(freeMintsPerBadge[tokenId]>0,"This token has used all the free mints available");
            
        freeMintsPerBadge[tokenId]--;
        
        if(freeMintsPerBadge[tokenId] == 0 && revealed==true) {
            switchUsedURI(tokenId);
        }else if(freeMintsPerBadge[tokenId] == 0){
   ...
   function setDepletedURI(string memory _newDepletedURI) public onlyOwner {
    ...
    function switchBaseURI(uint256 tokenId) private {
        badgeURI[tokenId] = depletedURI;
    }
    
    function switchUsedURI(uint256 tokenId) private {
        badgeURI[tokenId] = specialURI;
    }
