...
contract BellyChanceBid is VRFConsumerBase {
    ...
    mapping(uint256 => mapping(address => BidFor)) public bidsForItems;
    ...
    function enterChanceBid(uint256 itemId, uint256 _amount) public {
        ...
        bidsForItems[itemId][msg.sender] = BidFor(_amount, msg.sender);
    ...
    {
        return bidsForItems[itemId][msg.sender];
    }
        ...
        override
    {
        ...
        ChanceBidItem memory finishedItem = allChanceBids[itemFinished];
        uint256 len = finishHolders.length;
        uint256 totalValue = finishedItem.valueAcumulated;
        for (uint256 index = 0; index < len; index++) {
            address holder = holders[index];
            BidFor memory bid = bidsForItems[itemFinished][holder];
            uint256 percentatge = (bid.amount / totalValue) * 100;

            debugProbs.push(bid.amount);
        }
        randomness = _randomness;
    }
}
