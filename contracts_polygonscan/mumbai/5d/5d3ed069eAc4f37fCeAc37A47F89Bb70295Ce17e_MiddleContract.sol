...
// File contracts/MiddleContract.sol
...
contract MiddleContract is IERC721Receiver{

    address LuchamaskContractAddress;
    address TVKMockToken;
    bytes4 constant ERC721_RECEIVED = 0xf0b9e5ba;


    constructor(address _LuchamaskContractAddress, address _TVKMockToken){
        LuchamaskContractAddress = _LuchamaskContractAddress;
        TVKMockToken = _TVKMockToken;
    }

    function getTVKMockTokens () public {
        MockToken(TVKMockToken).requestMockTokens();
    }


    function mintTokensFromContract() public payable {
        (bool success, bytes memory data) = LuchamaskContractAddress.call{value: msg.value}(abi.encodeWithSignature("mintMask()"));
        require(success == true, "we get an error executing the mintMask method from the middle contract");
    }
