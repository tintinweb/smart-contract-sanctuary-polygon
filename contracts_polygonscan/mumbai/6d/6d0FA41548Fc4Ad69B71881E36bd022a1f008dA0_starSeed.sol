...
contract starSeed is Ownable{
    ...
    function depositStar(uint256 _amount) public returns(uint256,uint256){
        bool success;
        require (IERC20(starAddress).balanceOf(msg.sender) >= _amount, "Deposit amount is greater then your balance."); //check the user has atleast the amount of Star they are trying to deposit.
        (success) = IERC20(starAddress).transferFrom(msg.sender, address(this), _amount); //transfer the funds from the user to the game.
    ...
    function withdrawStar(uint256 _amount) public returns(uint256){
        ...
        (success) = IERC20(starAddress).transferFrom(address(this),msg.sender,_amount); //transfer the funds from the game to the user.
        require(success, "Transfer failed."); //ensure transfer completed successfully 
    ...
    function approveWithdraw(address user) external onlyOwner returns(uint256){
        ...
        (success) = IERC20(starAddress).transferFrom(address(this),user,pendingWithdraw[user]); //transfer the funds from the game to the user.
        require(success, "Transfer failed."); //ensure transfer completed successfully 
