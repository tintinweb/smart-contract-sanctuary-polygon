...
contract LimelightIDO is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public CLOUT;
    address public WMATIC;
    address public addressToSendWMATIC;
    address public wmaticCloutLP;
    ...
    mapping(address => bool) public boughtCLOUT;
    ...
    constructor(
        ...
        address _treasury,
        address _staking,
        address _wmaticCloutLP
    ) {
        require(_CLOUT != address(0));
        require(_WMATIC != address(0));
        require(_treasury != address(0));
        require(_staking != address(0));
        require(_wmaticCloutLP != address(0));

        CLOUT = _CLOUT;
        WMATIC = _WMATIC;
        treasury = _treasury;
        wmaticCloutLP = _wmaticCloutLP;
        staking = _staking;
        cancelled = false;
        finalized = false;
    }
    ...
    function purchaseCLOUT(uint256 _amountWMATIC) external returns (bool) {
        require(saleStarted() == true, 'Not started');
        require(
            !whiteListEnabled || whiteListed[msg.sender] == true,
            'Not whitelisted'
        );
        require(boughtCLOUT[msg.sender] == false, 'Already participated');

        boughtCLOUT[msg.sender] = true;

        uint256 _purchaseAmount = _calculateSaleQuote(_amountWMATIC);
        ...
        IERC20(WMATIC).safeTransferFrom(msg.sender, address(this), _amountWMATIC);
    ...
    function withdraw() external {
        ...
        IERC20(WMATIC).transfer(msg.sender, (amount / 1e9) * salePrice);
    }
    ...
    function finalize(address _receipt) external onlyOwner {
        require(totalAmount == 0, 'need all clouts to be sold');

        uint256 maiInTreasure = 250000 * 1e18;

        IERC20(WMATIC).approve(treasury, maiInTreasure);
        uint256 cloutMinted = ITreasury(treasury).deposit(maiInTreasure, WMATIC, 0);

        require(cloutMinted == 250000 * 1e9);

        // dev: create lp with 15 WMATIC per CLOUT
        IERC20(WMATIC).transfer(wmaticCloutLP, 750000 * 1e18);
        IERC20(CLOUT).transfer(wmaticCloutLP, 50000 * 1e9);
        uint256 lpBalance = IUniswapV2Pair(wmaticCloutLP).mint(address(this));
        uint256 valueOfToken = ITreasury(treasury).valueOfToken(
            wmaticCloutLP,
            lpBalance
        );

        IUniswapV2Pair(wmaticCloutLP).approve(treasury, lpBalance);
        uint256 zeroMinted = ITreasury(treasury).deposit(
            lpBalance,
            wmaticCloutLP,
            valueOfToken
        );
        require(zeroMinted == 0, 'should not mint any CLOUT');
        IERC20(CLOUT).approve(staking, cloutMinted);
