...
contract PrizeLinkedSavings is Ownable, SuperAppBase {
    /// @dev Entrance fee for the game (hardcoded to $1)
    uint256 private constant _ENTRANCE_FEE = 1e18;
    /// @dev Minimum flow rate to participate (hardcoded to $10 / mo)
    int96 private constant _MINIMUM_FLOW_RATE =
        int96(uint256(10e18) / uint256(3600 * 24 * 30));

    string private constant _ERR_STR_NO_TICKET =
        "LotterySuperApp: need ticket to play";
    string private constant _ERR_STR_LOW_FLOW_RATE =
        "LotterySuperApp: flow rate too low";
        ...
        )
    {
        if (_winner != address(0)) {
            (drawingTime, flowRate, , ) = _cfa.getFlow(
                _acceptedToken,
                address(this),
                _winner
            );
    ...
    function _beforePlay(bytes calldata ctx)
        private
        view
    ...
    function _play(
        ...
        (, int96 flowRate, , ) = IConstantFlowAgreementV1(agreementClass)
            .getFlowByID(_acceptedToken, agreementId);
    ...
    /// @dev Play the game
    function _quit(address player, bytes calldata ctx)
        private
        returns (bytes memory newCtx)
    {
    ...
    // @dev Make a new draw
    function _draw(address player, bytes calldata ctx)
        private
        returns (bytes memory newCtx)
    {
    ...
    modifier onlyHost() {
        require(
            ...
            "LotterySuperApp: support only one host"
        );
        _;
    }
    ...
    modifier onlyExpected(ISuperToken superToken, address agreementClass) {
        require(
            ...
            "LotterySuperApp: not accepted token"
        );
        require(
            _isCFAv1(agreementClass),
            "LotterySuperApp: only CFAv1 supported"
        );
        _;
    }
}
