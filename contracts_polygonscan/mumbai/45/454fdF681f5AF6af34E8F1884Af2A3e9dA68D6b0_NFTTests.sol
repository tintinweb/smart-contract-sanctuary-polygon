...
contract NFTTests is Ownable, ERC721URIStorage {
    ...
    constructor() ERC721("NFTTests", "NFTTests") {
        for (uint i = 0; i < 100; i++) {
            nfts.push(Nft({
                    index: i,
                    uri: "https://gateway.pinata.cloud/ipfs/QmYJtSRQUNhXGuKEwyPVY4eZJ6nUzJea59Gf868Miq6eZL",
                    mintable: true,
                    mintCap: 50,
                    numberMinted: 0
                }));
    ...
    function createBlankNodes(uint256 total) public onlyOwner() {
        for(uint i=0; i< total; i++){
            nfts.push(Nft({index:0, uri: "", mintable: true, mintCap: 0, numberMinted: 0}));
        }
    }
    ...
    function addNftBulk(Nft[] memory _array) public onlyOwner() {
        ...
        for(uint i=0; i<_array.length; i++){
            nfts[_array[i].index] = _array[i];
        }
    }

   // Add new NFTs
    function addNft(string memory uri,  bool mintable, uint256 mintCap) external onlyOwner() {
    ...
    function updateNft(uint256 nftID, bool mintable) external onlyOwner() {
