...
//https://medium.com/aisthisi/aisthisi-technical-deep-dive-part-2-5250b0d71ee
//https://ethereum.stackexchange.com/questions/113793/royalties-for-erc-1155
...
/// @custom:security-contact sds
contract Presale is ERC1155, AccessControl, Pausable, ERC1155Burnable, ERC1155Supply, RoyaltiesV2Impl, ERC2981Base {
    // Setup the roles and owner
    ...
    // Meta data url parts
    string private baseUri = "https://d3cyw651c4t2lt.cloudfront.net/surfdev/presale/";
    string private suffix = ".json";

    // ERC2981 Royalties
    RoyaltyData private _royalties;
    address public royaltyAddress;
    uint256 royaltyBase = 400;

    // Supply and Sale Data    
    bool public presaleEnabled = false;
    uint256 public salePrice = 0.2 ether;
    uint256 public supplyAmount = 10000;
    uint256 public maxBuy = 1;
    uint256 public maxPerWallet = 3;



    constructor() ERC1155(baseUri) {       
        // This line has to come first or the deploy will fail and revert
        // Set roles and owner
        ...
        // Mint the intial supply for presale
        _mint(address(this), 0, supplyAmount, "");
        // Mint team and studio
        _mint(msg.sender, 0, 520, "");
        // 2981 Royalties
    ...
    function setURI(string memory newuri) public onlyRole(CLEVEL) {
        _setURI(newuri);
    }
    ...
    function mint(address account, uint256 id, uint256 amount, bytes memory data) public onlyRole(CEO) {
    ...
    function destory() public onlyRole(CEO) { 
    ...
    function burn(
        ...
        require(
            account == _msgSender() || isApprovedForAll(account, _msgSender()) || hasRole(CLEVEL, msg.sender),
            "ERC1155: caller is not owner nor approved"
        );
    ...
    function updateSalePrice(uint256 _amount) public onlyRole(CLEVEL) {
        salePrice = _amount;
    }
    ...
    function buyNft(uint _amount) public payable {        
        require(balanceOf(address(this), 0) >= _amount, 'Contract balance insufficient');
        if(!hasRole(CEO, msg.sender)) {
            require(presaleEnabled, 'Presale not enabled');
            require(_amount > 0 && _amount <= maxBuy, 'Invalid amount or Exceeds max purchase');
            require((balanceOf(msg.sender, 0) + _amount) <= maxPerWallet, 'Exceeds max wallet amount');
            require(msg.value == (salePrice * _amount));    
        }
        
        ERC1155(address(this)).safeTransferFrom(address(this), msg.sender, 0, _amount, "");
    }
    ...
    function updatePresaleEnabled(bool _status) public onlyRole(CLEVEL) {
