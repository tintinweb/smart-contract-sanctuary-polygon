...
contract DevZeuDistributor is AccessControl {
    
    DevZeuI devZeuToken;

    uint256 public tokenPrice = uint256(7 * 10**16); // = 0.07 eth
    address public withdrawWallet;

    bytes32 public constant TOGGLE_MINTING_ROLE = keccak256("TOGGLE_MINTING_ROLE");
    bool public _mintingPaused = false;

    constructor(DevZeuI _devZeuToken) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(TOGGLE_MINTING_ROLE, msg.sender);

        withdrawWallet = msg.sender;

        devZeuToken = _devZeuToken;
    }

    function mint(uint256 num) public payable returns (bool) {

        require(num <= 5, "You can mint a maximum of 5");
        require(msg.value >= tokenPrice * num, "Insufficient amount provided");
        require(!_mintingPaused, "Minting paused");


        uint256 tokenId = devZeuToken.getCurrentTokenId();
        require(tokenId + num <= 7777, "Maximum cap of 7777 mints reached");

        for(uint256 i; i < num; i++){
            devZeuToken.mint(msg.sender, tokenId + i + 1);
        }
        return true;
    }
