...
contract Roulette is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getStatus() public view returns(uint, uint, uint) {
        return (
        bets[msg.sender].length,             // number of active bets
    ...
    function bet(uint8 number, uint8 betType, uint256 betAmount) payable public whenNotPaused nonReentrant(){
        ...
        require(provisionalBalance < address(this).balance, "There is not enough balance in the bank");
        ...
        beryl.safeTransferFrom(msgSender, address(this), betAmount);
    ...
    function spinWheel() public whenNotPaused nonReentrant() {
    ...
    function cashOut() public whenNotPaused nonReentrant() {
        ...
        require(amount <= address(this).balance, "There is not enough balance in the bank");
        winnings[player] = 0;

        beryl.safeTransferFrom(address(this), player, amount);
    }


    function addToPool(uint256 amount) public onlyOwner {

        beryl.safeTransferFrom(msg.sender, address(this), amount);
