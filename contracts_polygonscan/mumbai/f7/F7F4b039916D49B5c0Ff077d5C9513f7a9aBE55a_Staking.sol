...
contract Staking is Ownable, AccessControl, ReentrancyGuard, Pausable {
    ...
    modifier beforeStakeStarted() {
        require(
            ...
            "[StakeVault.beforeStakeStarted] Staking period started"
        );
        _;
    }
    ...
    modifier afterStakeStarted() {
        require(
            block.timestamp <= startStakeTime,
            "[StakeVault.afterStakeStarted] Staking period not yet started"
        );
        _;
    }
    ...
    modifier afterStakeEnded() {
        require(
            ...
            "[StakeVault.afterStakeEnded] Staking period not yet ended"
        );
        _;
    }
    ...
    function stakeNaka(uint256 _nakaAmount) external beforeStakeStarted {
        require(
            ...
            "[StakeVault.stakeNaka] total stake amount should be less than user limit"
        );
        require(
            _nakaAmount + poolStakeTotal <= poolStakeLimit,
            "[StakeVault.stakeNaka] total stake amount should be less than pool limit"
        );
        IERC20 token = IERC20(nakaTokenAddress);
    ...
    function withdrawNaka() external afterStakeEnded nonReentrant {
        ...
        require(
            0 == unclaimAmount,
            "[StakeVault.withdrawNaka] unclaim amount should be zero before user can withdraw"
        );
        ...
        nonReentrant
    {
        ...
        require(
            ...
            "[StakeVault.claimReward] claim amount should be less than user unclaim amount"
        );
     ...
     * Calculated by claimedAmount - (poolRatio * dayRatio * percentage * stakeAmount).
     * poolRatio is poolStakeLimit / poolStakeTotal.
     * dayRatio is passedDay / duration.
     * @param _userAddress - user address to be retrieve.
     */
        ...
        returns (uint256)
    {
        ...
        uint256 totalRewardUntilToday = (poolStakeLimit *
            dayPassed *
            percentage *
            user.stakeAmount) /
            duration /
