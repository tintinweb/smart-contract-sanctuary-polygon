...
// File: contracts/interfaces/IPTokenTest.sol
...
interface IPTokenTest is IERC20 {
...
// File: contracts/GenSupply.sol


pragma solidity ^0.8.0;


/// @author tosbot overriding 101.digital
...
abstract contract GenSupply {
    ...
    uint256 private constant SUP_GEN2_ID  = 6666;
    ...
    Counters.Counter private _gen2TokenCount;
    ...
    uint256 private _gen2MaxSupply;
    ...
    function tokenCount(bytes2 _gen) public view returns (uint256) {
        if (_gen == GEN0) {
            ...
            return _gen2TokenCount.current();
        } else {
            return 0;
        }
    }
    ...
    function nextToken(bytes2 _gen) internal virtual returns (uint256) {
        ...
        if (_gen == GEN0) {
            ...
            token = _gen2TokenCount.current();
            _gen2TokenCount.increment();
        }  
        return token;
    }
...
// File: contracts/GenCounter.sol
    ...
    abstract contract GenCounter is GenSupply {
        ...
        mapping(uint256 => uint256) private gen2TokenMatrix;
        ...
        /// @return the next token ID
        function nextToken(bytes2 _gen) internal override ensureAvailability(_gen) returns (uint256) {
        ...
        function getStartFrom(bytes2 _gen) private pure returns (uint256) {
            ...
            if (_gen == GEN0) {
                ...
                startFrom = START_GEN2_ID;
            }
            return startFrom;
        }

        function getMatrixValue(bytes2 _gen, uint256 _index) internal view returns (uint256) {
            if (_gen == GEN0) {
                ...
                return gen2TokenMatrix[_index];
            } else {
                return 0;
            }
        }

        function setMatrixValue(bytes2 _gen, uint256 _index, uint256 _value) private {
            if (_gen == GEN0) {
...
contract RBTest is ERC721Enumerable, Ownable, GenCounter {
    using SafeMath for uint256;

    IPTokenTest public PlaToken;

    uint256 private constant MAX_SALE_MINT = 1000; //2;
    uint256 private constant INK_TIME = 300; //86400;
    ...
    uint256 public presalePrice = 0.06 ether; //60 ether;
    uint256 public salePrice = 0.012 ether; //120 ether

    uint256 public breedPlaPrice = 10 ether; //777 ether;
    ...
    function mintPresale(uint256 _rawb0tsToMint) external payable presaleIsActive {
        uint256 amount = presaleWhitelist[msg.sender];
        require(amount > 0, "No tokens available for this address");
        require(_rawb0tsToMint > 0 && _rawb0tsToMint <= amount, "Invalid purchase amount");
    ...
    function mintSale(uint256 _rawb0tsToMint) external payable saleIsActive {
    ...
    function breed(uint256 _parent1, uint256 _parent2) external rawb0tOwner(_parent1) rawb0tOwner(_parent2) {        
    ...
    function getPendingInk(uint256 _id) public view rawb0tOwner(_id) returns (uint256) {
        uint256 inkPercentage = (block.timestamp - inkLastUpdate[_id]) * 100 / INK_TIME;
    ...
    function getChildGen(bytes2 _genParent1, bytes2 _genParent2) private pure returns (bytes2) {
        ...
        if (_genParent1 == GEN01 && _genParent2 == GEN02
            ...
            gen = GEN13;
        } else if (_genParent1 == GEN1 && _genParent2 == GEN1) {
            gen = GEN2;
        }
        return gen;
    }
    ...
    function setPlaToken(address _plaTokenAddress) external onlyOwner {
        PlaToken = IPTokenTest(_plaTokenAddress);
    }
