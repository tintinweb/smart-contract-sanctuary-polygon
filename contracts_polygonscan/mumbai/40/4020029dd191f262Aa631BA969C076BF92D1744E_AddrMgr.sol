...
contract AddrMgr {
    ...
    function set(bytes32 ContractName, address ContractAddress) external virtual modcanDo("set") returns (bool Res) {
    ...
    function Remove(bytes32 ContractName) external virtual modcanDo("Remove") returns (bool Res) {
        if(DBKeys.length > 1) {
            uint Ix = DBKeys.length;
            for(uint i = 0; i < DBKeys.length; i++) {
                if(DBKeys[i] == ContractName) { Ix = i; break; }
            }
            if(Ix == DBKeys.length) { return true; }
