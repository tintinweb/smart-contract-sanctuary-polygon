...
contract Buddy is 
    ...
    /**
     * @notice Withdraws the amount to the foundation address
     */

    function withdraw(address _tokenAddress) public onlyAdmin {
        if (tokenAddress != address(0)) {
            uint256 amount = IERC20(_tokenAddress).balanceOf(address(this));
            IERC20(_tokenAddress).transfer(getFoundationTreasury(), amount);
        } else {
