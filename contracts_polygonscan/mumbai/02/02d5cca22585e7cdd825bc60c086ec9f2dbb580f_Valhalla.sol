...
// File: contracts/MEGVDividendPayingToken.sol
...
contract Valhalla is ERC20, Ownable {
    ...
    //MEGVDividendTracker public dividendTracker;

    address public burnAddress;
    
    uint256 public maxBuyTransactionAmount = 50000000 * (10**8);
    uint256 public maxSellTransactionAmount = 10000000 * (10**8);
    uint256 public swapTokensAtAmount = 200000 * (10**8);
    uint256 public _maxWalletToken = 200000000 * (10**8);
    ...
    // sell fees
    uint256 public sellMarketFee = 3;
    ...
    function updateMaxWalletAmount(uint256 newAmountNoDecimials) external onlyOwner {
        _maxWalletToken = newAmountNoDecimials * (10**8);
    }     

    function updateSwapAtAmount(uint256 newAmountNoDecimials) external onlyOwner {
        swapTokensAtAmount = newAmountNoDecimials * (10**8);
    } 
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        //require(newAddress != address(dividendTracker), "MEGV: The dividend tracker already has that address");

        //MEGVDividendTracker newDividendTracker = MEGVDividendTracker(payable(newAddress));

       // require(newDividendTracker.owner() == address(this), "MEGV: The new dividend tracker must be owned by the MEGV token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "MEGV: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "MEGV: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        //require(pair != uniswapV2Pair, "MEGV: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

       // _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MEGV: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MEGV: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MEGV: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            revert("MEGV: This account cannot send tokens until trading is enabled");
        }
        if(amount == 0) {
