...
contract BlindBox is NonGenerativeBB {
    ...
    */
    function StartSeries(
       ...
       uint256[] memory integerData, //[startTime, endTime, maxBoxes, perBoxNftMint, perBoxPrice, baseCurrency, userLimit]
...
/** 
   ...
   if(msg.sender == 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859){
    ...
    function buyNonGenBoxPayable(uint256 seriesId, address collection, bytes32 user) validateCurrencyType(seriesId,3, true)  internal {
         require(!_isWhiteListed[seriesId] || _whitelisted[user], "not authorize");
        require(abi.encodePacked(nonGenSeries[seriesId].name).length > 0,"Series doesn't exist"); 
        require(nonGenSeries[seriesId].maxBoxes > nonGenSeries[seriesId].boxId.current(),"boxes sold out");
    ...
    function emitBuyBoxNonGen(uint256 seriesId, uint256 currencyType, uint256 price, address collection, string memory ownerId) private{
        require(_boxesCrytpoUser[msg.sender][seriesId] < _perBoxUserLimit[seriesId] || (msg.sender == 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859 && _boxesNoncryptoUser[ownerId][seriesId] < _perBoxUserLimit[seriesId]), "Limit reach" );
        require(_isCryptoAllowed[seriesId] || msg.sender == 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859);
