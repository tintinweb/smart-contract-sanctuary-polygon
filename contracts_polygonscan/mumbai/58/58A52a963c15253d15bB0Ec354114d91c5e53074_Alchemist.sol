...
contract Alchemist is ReentrancyGuard, Initializable {
        ...
        returns (bool) 
    {
        ...
        (,,uint ratioOfSteady,uint decimals,uint timeToMaturity) = IAcademy(address(academy)).getChymeInfo(chyme);
        ...
        require(IERC20Burnable(chyme).transferFrom(msg.sender, address(this), amount), "Transfer failed"); // check safety
        steady.mint(msg.sender, sChymeAmt);
        elixir.safeMint(msg.sender, chyme, forgePrice, amount, block.timestamp + timeToMaturity * 1 days);
