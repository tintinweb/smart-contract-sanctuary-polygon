...
contract NFT is ERC721MintMore, Ownable {
    ...
    function mintMore(address _recipient, string memory _name, uint _count) public returns (uint) {
        ...
        for (uint i = 0; i < _count; i++) {
            _safeMintWithoutEvent(_recipient, tokenCount);
...
contract ERC721MintMore is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _safeMintWithoutEvent(address to, uint tokenId) internal virtual {
        _safeMintWithoutEvent(to, tokenId, '');
    }

    function _safeMintWithoutEvent(address to, uint tokenId, bytes memory _data) internal  {
        _mintWithoutEvent(to, tokenId);
        require(_checkOnERC721Received(address(0), to, tokenId, _data), 'ERC721: transfer to non ERC721Receiver implementer');
    }

    function _mintWithoutEvent(address to, uint tokenId) internal  {
        require(to != address(0), 'ERC721: mint to the zero address');
        require(!_exists(tokenId), 'ERC721: token already minted');
        _beforeTokenTransfer(address(0), to, tokenId);
        _balances[to] += 1;
