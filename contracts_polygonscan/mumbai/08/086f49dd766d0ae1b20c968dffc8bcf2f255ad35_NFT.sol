...
// File: github.com/bcnmy/metatx-standard/blob/master/src/contracts/lib/EIP712Base.sol
...
// File: github.com/bcnmy/metatx-standard/blob/master/src/contracts/EIP712MetaTransaction.sol
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    */
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        revert("ERC721: transferring not allowed");
...
contract NFT is ERC721, EIP712MetaTransaction("INDIGG ERC721 NFT MINTER", "1"), Ownable {
    
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    mapping(address => bool) public isWhitelisted;
    mapping(address => bool) public isMinted;
    mapping(bytes32 => bool) public isUserAdded;
    mapping(bytes32 => address) public emailHash_address;

    event User(uint256 timestamp, address user);
    event Mint(uint256 timestamp, address user, uint256 tokenId);

    constructor() public ERC721("INDIGG Badge NFT", "IGG") {}

    function mintToCaller(address caller) 
        public
        returns (uint256)
    {
        require(isWhitelisted[caller], "INDIGG: caller not authorised"); // Makes sure that only the discord members can mint the badge
        require(isMinted[caller] == false, "INDIGG: NFT already minted"); // 
        _tokenIds.increment();

        uint256 newItemId = _tokenIds.current(); 

        emit Mint(block.timestamp, caller, newItemId);
        isMinted[caller] = true;
        _mint(caller, newItemId);
        return newItemId;
    }

    function batchMint(address[] memory caller) external onlyOwner {
        for (uint256 i = 0; i<caller.length; i++) {
            mintToCaller(caller[i]);
        }
    }

    function setWhitelist(bytes32 emailHash, address user) public onlyOwner {
        require(!isUserAdded[emailHash], "INDIGG: user already added");
        isUserAdded[emailHash] = true;
        isWhitelisted[user] = true;
        emailHash_address[emailHash] = user;
        emit User(block.timestamp, user);
    }
