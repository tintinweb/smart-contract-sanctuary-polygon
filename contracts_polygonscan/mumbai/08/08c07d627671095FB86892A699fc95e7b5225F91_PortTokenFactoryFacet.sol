...
contract PortTokenFactoryFacet {
    ...
    //TODO include check for minimum amount of port
    function createAndInitPortToken(
        ...
        require(_trackedTokens.length == _ratios.length);
        require(_trackedTokens.length > 0);

        address portTokenAddr = _createPortToken(msg.sender,_name, _symbol);
        ...
        for(uint256 i=0;i<_trackedTokens.length;i++) {
            LibTrackedToken.addTrackedToken(portTokenAddr,_trackedTokens[i], _ratios[i]);
        }

        require (LibPriceInterface.checkUtilityTokenShare(_trackedTokens, _ratios), "The share of utility token is too small");
...
library LibFee {
    ...
    event StreamingFeeChanged(address indexed addr, uint32 value);
    event SuccessFeeChanged(address indexed addr, uint32 value);
