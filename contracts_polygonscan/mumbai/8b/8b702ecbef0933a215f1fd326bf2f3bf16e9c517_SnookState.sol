...
contract SnookState is ISnookState, Initializable {
  event DescriptorSet(uint stars, uint traitCount, uint lives); // DEBUG ONLY
  ...
  // DEBUG ONLY: no modifier
  function getDescriptor(uint tokenId) external override view returns(Descriptor memory) {
    return _descriptors[tokenId];
  }
  ...
  function _setDescriptor(uint tokenId, Descriptor calldata descriptor) internal {
    emit DescriptorSet(descriptor.stars, descriptor.traitCount, descriptor.lives); // DEBUG ONLY
