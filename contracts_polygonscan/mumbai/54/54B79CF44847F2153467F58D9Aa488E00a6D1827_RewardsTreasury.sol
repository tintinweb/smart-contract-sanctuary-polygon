  ...
  AccessControlUpgradeable
{
  ...
  function initRewardsTreasury(
    ...
    initFundsWrapperCustody(_fundsOwner, _acceptedToken);
...
contract FundsWrapperCustody is Initializable {
  ...
  function initFundsWrapperCustody(
    address fundsOwner,
    ISuperToken _acceptedToken
  ) public initializer {
  ...
  function deposit(uint256 _amt) external onlyFundsOwner {
    ...
    acceptedFundToken.upgrade(_amt);
  ...
  function _onFundsOwnershipTransferred(address from, address to) internal {
    uint256 oldAllowance = underlyingToken.allowance(address(this), from);

    // Deplete the allowance from the old funds owner
    bool decreased = underlyingToken.decreaseAllowance(from, oldAllowance);
    require(decreased, "failed to decreaseAllowance");
