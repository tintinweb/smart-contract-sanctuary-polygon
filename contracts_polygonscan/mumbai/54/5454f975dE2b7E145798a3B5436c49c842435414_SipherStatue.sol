...
contract SipherStatue is ERC1155, Pausable, Ownable {
	...
	event mintRecord(uint256 _tokenId, string _tokenname, uint256 amount);
	...
	function mint(uint256 _tokenId, uint256 token_amount) external onlyOwner{
		...
		emit mintRecord(_tokenId, tokenIdToName[_tokenId], token_amount);
	}
     ...
     */
	function claimStatue(address from, uint256 _tokenId, uint256 amount) external notBlacklist{
		require(msg.sender == from, "Only owner of token can claim");
		require(balanceOf(from,_tokenId) - claimAmount[from][_tokenId] >= amount, "not enought balance to claim");
		claimAmount[from][_tokenId] = claimAmount[from][_tokenId] + amount;
		emit claimRecord(from, _tokenId, tokenIdToName[_tokenId], amount);
	}

	/**
     * @dev get remain balance after clamming
     */
	function getBalanceAfterClaim(address from, uint256 _tokenId) external view returns(uint256){
