...
/**
 * Chibi Citizen, ERC-721 token
 */
contract ChibiCitizen is IChibiCitizen, ERC721Enumerable, Base {
    using Counters for Counters.Counter;
    /**
     * role for minting citizens
     */
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    /**
     * role for managing level & exp
     */
    bytes32 public constant LEVEL_MANAGER_ROLE =
        keccak256("LEVEL_MANAGER_ROLE");
    /**
     * base token uri
     */
    string public baseTokenURI;
    /**
     * to determine if the contract use default fixed metadata or dynamic metadata (including level, experience)
     */
    bool public useFixedMetadata;
    /**
     * metadata of citizens
     */
    ...
    constructor() ERC721("Chibi Citizen", "CHIBICITIZEN") {}

    /**
     * see {IChibiCitizen-safeMint}
     */
    function safeMint(
        address to,
        uint8 generation,
        ...
        // generate metadata for citizen
        _citizens[tokenId] = Citizen({
            tokenId: tokenId,
            class: citizenClass,
            classId: classId,
        ...
        emit Minted(tokenId, generation, citizenClass, classId, mintMethod);
    }

    /**
     * see {IChibiCitizen-setLevel}
     */
    function setLevel(
        uint256 tokenId,
        uint8 level,
        uint16 exp
    ) external override onlyRole(LEVEL_MANAGER_ROLE) {
        require(_citizens[tokenId].minted, "nonexistent token");
        _citizens[tokenId].level = level;
        _citizens[tokenId].exp = exp;
        emit LevelChanged(tokenId, level, exp);
    }

    /**
     * see {IChibiCitizen-setBaseTokenURI}
     */
    function setBaseTokenURI(
        string calldata newBaseTokenURI,
        bool newUseFixedMetadata
    ) external override onlyOwner {
        baseTokenURI = newBaseTokenURI;
        useFixedMetadata = newUseFixedMetadata;
    }

    /**
     * see {IChibiCitizen-getToken}
     */
        ...
        returns (Citizen memory)
    {
        ...
        return _citizens[tokenId];
    }

    /**
     * see {IChibiCitizen-getTokens}
     */
        ...
        returns (Citizen[] memory tokens)
    {
        uint256 tokenCount = Math.min(
            totalSupply() - startIndex,
            numberOfTokens
        );
        tokens = new Citizen[](tokenCount);
        for (uint256 i = startIndex; i < startIndex + tokenCount; i++) {
            uint256 tokenId = tokenByIndex(i);
            tokens[i - startIndex] = _citizens[tokenId];
        }
    }

    /**
     * see {IChibiCitizen-tokenURI}
     */
            ...
            return
                string(
                    abi.encodePacked(
                        baseURI,
                        "gen-",
                        Strings.toString(_citizens[tokenId].generation),
                        "/",
                        types[uint256(_citizens[tokenId].class)],
                        "-",
                        Strings.toString(_citizens[tokenId].classId),
                        ".json"
                    )
                );
        }
 ...
 * Base contract
 */
abstract contract Base is IBase, AccessControl {
    /**
