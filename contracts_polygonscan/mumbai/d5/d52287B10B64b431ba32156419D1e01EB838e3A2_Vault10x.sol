...
contract Vault10x is Ownable {
    ...
    function tokenBuy(
        ...
        uint256 isaPerUSD = isaOracle.price();
        uint256 tokensToMint = isaPerUSD.mul(amount);
        require(tokensToMint >= MIN_BUY);
        require(isatkn.allowance(msg.sender, address(this)) >= amount);
        require(isatkn.balanceOf(msg.sender) >= amount);
    ...
    function updatePrice() public {
        ...
        uint256 currentPrice = priceOracle.price();
        (uint256 changeInPrice, bool isIncreased) = priceChange(currentPrice);
        if(changeInPrice < 20) {
          if(isIncreased) {
