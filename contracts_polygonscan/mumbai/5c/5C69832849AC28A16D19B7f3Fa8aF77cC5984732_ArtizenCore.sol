...
contract ArtizenCore is IArtizenCore, Ownable {
    struct Grant {
        ...
        uint256 totalDonations; //in USDC (18 decimals)
    ...
    /**
        ...
        @param _amountDonated the amount of USDC donated
    ...
    */
    ...
    // Gifts voting power to an account without any USDC deposits
    function giftVotes(
    ...
    */
    ...
    /**
        @notice Refunds specified donor of cancelled grant if USDC was donated
    ...
    */
    function setProjectInGrant(
        ...
        uint256 _projectID,
        bool _inGrant
    ) external override notShutdown grantNotCancelled(_grantID) {
        ...
        onlyOwner
    {
        ...
        // Infinite approve Treasury for USDC withdraws
        USDC.approve(_newTreasury, type(uint256).max);
    ...
    /**
        @notice internal core donation logic, only processes USDC donations 
        @param _grantID the ID of grant donated to
        @param _amountUSDCDonated the amount of USDC donated
        @param _delegateVotesTo an address to delegate votes to
    */
    function _donate(
        uint256 _grantID,
        uint256 _amountUSDCDonated,
        address _delegateVotesTo
    ) internal grantNotCancelled(_grantID) {
        require(_amountUSDCDonated > 0, "ART_CORE: CANT DONATE ZERO");
        require(
        ...
        // Pulls USDC from donor (msg.sender)
        require(
            USDC.transferFrom(msg.sender, address(this), _amountUSDCDonated)
        );

        // Sends donated funds to Treasury to earn yield
        if (block.chainid == 137) {
            IArtizenTreasury(treasuryAddress).deposit(_amountUSDCDonated);
        }

        // add donated amount to specified grant
        grants[_grantID].totalDonations += _amountUSDCDonated;
        grants[_grantID].totalProtocolFees +=
            (_amountUSDCDonated * protocolFee) /
            SCALE;

        // Allocating votes. y USDC = square root of 100y votes
        voteBalances[_delegateVotesTo][_grantID] += FixedPointMathLib.sqrt(
            _amountUSDCDonated / 1e16
        );

        // Account for individual donation amounts per grant per user
        grantDonorsArray[_grantID].push(msg.sender); // in array
        grantDonationsMap[_grantID][msg.sender] += _amountUSDCDonated; // in mapping

        emit Donate(msg.sender, _grantID, _amountUSDCDonated);
    }
    ...
    */
        ...
        grantNotCancelled(_grantID)
    {
        ...
        IArtizenTreasury(treasuryAddress).moveEnoughUSDCFromAaveToTreasury(
            _totalGrantAdminFee
        );
    ...
    function _refundDonation(address _donor, uint256 _amount) internal {
        // Sends [_amount] USDC from Treasury to [_donor] as a refund
    ...
    // Calcs the share of total funds in grant based on votes (in USDC)
...
interface IArtizenTreasury {
    ...
    function grantUSDCInTreasury() external view returns (uint256);
    ...
    function moveUSDCFromTreasuryToAave(uint256 _amountUSDC) external;

    function moveUSDCFromAaveToTreasury(uint256 _amountUSDC) external;

    function moveEnoughUSDCFromAaveToTreasury(uint256 _amountUSDC) external;
    ...
    function setTokenAddresses(address _USDC, address _aUSDC) external;
    ...
    function getTotalUSDCOwnedByArtizen() external view returns (uint256);
}
