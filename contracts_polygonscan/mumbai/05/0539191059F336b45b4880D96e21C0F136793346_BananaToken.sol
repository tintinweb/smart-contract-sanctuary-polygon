...
contract BananaToken is ERC20("Banana", "BANANA"), Ownable {
	...
	function setMinters(address[] calldata _adds, bool[] calldata _values) external onlyOwner {
		for(uint256 i = 0 ; i < _adds.length; i++)
			minters[_adds[i]] = _values[i];
	}
	...
	function burn(uint256 _amount) external {
		_transfer(_msgSender(), burnAddress, _amount);
	}
	...
	function burnFrom(address _user, uint256 _amount) external {
		...
		transferFrom(_user, burnAddress, _amount);
	}
