    ...
    ReentrancyGuardUpgradeable
{
    ...
    function setAutocompound(address _autoCompound) external onlyOwner {
        require(_autoCompound != address(0), "Wrong autocompound address");
     ...
     */
    function addLocker(address _locker) external onlyOwner {
        locker = _locker;
    }
    ...
    function enterStaking(uint256 _amount) external onlyAutocompound {
        ...
        if (user.amount > 0) {
            uint256 pending = ((user.amount * pool.accSheeshaPerShare) / 1e12) -
                user.rewardDebt;
            if (pending > 0) {
                safeSheeshaTransfer(_msgSender(), pending);
            }
        }
        if (_amount > 0) {
            IERC20Upgradeable(address(sheesha)).safeTransferFrom(
        ...
        returns (uint256)
    {
        ...
        if (pool.token.balanceOf(address(this)) >= tokenRewards) {
            tokenSupply = pool.token.balanceOf(address(this)) - tokenRewards;
        }
