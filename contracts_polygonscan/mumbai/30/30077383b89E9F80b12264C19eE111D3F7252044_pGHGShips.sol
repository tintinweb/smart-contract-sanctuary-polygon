...
contract pGHGShips is ERC721, Ownable {
    ...
    function mintMany(uint16[] calldata ids) external onlyOwner {
        for(uint16 i = 0; i < ids.length; i++) {
            _safeMint(msg.sender, ids[i]);
        }
    }

    function mintRange(uint16 inclStartId, uint16 inclEndId) external onlyOwner {
