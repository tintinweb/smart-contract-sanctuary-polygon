...
contract Staker is Ownable {
    ...
    uint256 public immutable minTokensPerminute;
    uint256 public immutable maxTokensPerminute;
    uint256 public tokensPerminute;
    ...
    constructor() {
        stakedToken = IBEP20(0xc73f4dD5EbdC34FC80cC6de4658dFe675DD7325c); // AQAR LP
        rewardToken = IBEP20(0x1Ba64dBC14e5F4e894ea2D97018a769Fdb15f664); // AQAR 
        
        minTokensPerminute = 1000000000;
        maxTokensPerminute = 100000000000000000000; 
        tokensPerminute = 100000000000000;
        ...
        mintingRewardTokensEnabled = false;
    ...
    function setRewardTokensPerminute(uint256 _tokensPerminute) external onlyOwner onlyStarted {
        require(pausedTimestamp == 0, "Staker setTokensPerminute: you can't set while paused!");        
        require(_tokensPerminute >= minTokensPerminute, "Staker setTokensPerminute: too low tokens, see minTokensPerminute!");
		require(_tokensPerminute <= maxTokensPerminute, "Staker setTokensPerminute: too many tokens, see maxTokensPerminute!");
    ...
    function setMinDepositAmount(uint256 _minDepositAmount) external onlyOwner {
        require(_minDepositAmount >= 1000000000000000, "Staker setMinDepositAmount: 1000000000000000 wei is minimum!");
    ...
    function pendingRewardsOfUser(address _user) public view returns (uint256 real, uint256 reflection) { // reflection - legasy (
		...
		if (stakedTokenDeposied != 0 && user.amount != 0 && blockTimestamp() > lastRewardTimestamp) {
            _accRewardTokensPerShare = accRewardTokensPerShare + ((getMultiplierNow() * tokensPerminute) * 1e12 / stakedTokenDeposied);
        }
    ...
    function _updatePool() internal {        
        ...
        uint256 tokensReward = getMultiplierNow() * tokensPerminute;
    ...
    function deposit(uint256 _amount) public onlyStarted {
        ...
        require(_amount >= minDepositAmount, "Staker deposit: you can't deposit less than minDepositAmount of wei!");
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal {        
		require(_amount <= balanceOfRewardToken(), 'Staker: not enough reward tokens. Notify contract owner to supply tokens !!!');
