   ...
   (uint256 mainPerecentage, uint256 authorPercentage, address blindRAddress) = getPercentages(tokenId, collection);
      ...
      if(blindRAddress == address(0x0)){
        ...
        if( collection == address(XNFT)  || collectionsWithRoyalties[collection]) {
          ...
          //   token.mint(nonCryptoNFTVault, SafeMath.mul(SafeMath.div(price,1000), authorPercentage));
          //   _nonCryptoWallet[_tokenAuthors[tokenId][collection].ownerId].Alia += SafeMath.mul(SafeMath.div(price,1000), authorPercentage);
          // }
        }
