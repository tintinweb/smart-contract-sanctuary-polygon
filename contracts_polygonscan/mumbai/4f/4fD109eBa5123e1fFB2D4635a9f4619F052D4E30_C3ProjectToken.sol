    ...
    OwnableUpgradeable
{
    ...
    mapping(address => uint256) public nftFromId;
    ...
    function initialize(
        ...
        __ERC20_init_unchained(_name, string(abi.encodePacked(_prefix, "-", _identifier)));
    ...
    function onERC721Received(
        ...
        VCUSData memory data = IContractNFT(nftAddress).getData(tokenId);

        require(
            data.status == ConfirmationStatus.Approved,
            'The retirement is not yet approved'
        );

        string memory projectId = string(abi.encodePacked(data.registry, "-", data.project, "-", Strings.toString(data.vintage)));

        require(
            strcmp(projectId, identifier),
            'NFT Token wrong'
        );

        uint256 quantity = data.quantity * (10 ** 18);

        _mint(from, quantity);

        nftFromId[from] = tokenId;

        return this.onERC721Received.selector;

    }

    // https://medium.com/@maltabba/tokens-smart-contracts-best-practices-prevent-transferring-to-the-0x0-and-the-contract-address-cdfc11af4937
