...
contract Comic is ERC721, Pausable, Ownable {
    ...
    modifier notBlacklist() {
      if (isBlacklist[msg.sender]) {
        revert("You are block by admin, Please contact owner of contract for more information");
      }    
      _;
    }
    ...
    function ownerOf(uint256 tokenId)
        ...
        {
            require(tokenId <= currentId && tokenId > 0, 'ComicERC721: invalid token id');
    ...
    function burn(address from, uint256 _tokenId) external {
        ...
        require(msg.sender == from, "Only owner of token can call burn function");
        _burn(_tokenId);
     ...
     */
    function ownerTransfer(address from, address to, uint _id) external onlyOwner{
        approve(to, _id);
