...
contract SolidTreasury is OlympusAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    */
    ...
    ISOLID public immutable SOLID;
    IsSOLID public sSOLID;
    ...
    /**
     * @notice allow approved address to deposit an asset for SOLID
    ...
    function deposit(
        ...
        // mint SOLID needed and store amount of rewards for distribution
        // TODO: question -> why the depositor would inform a value for profit to lose his solid returns?
        send_ = value.sub(_profit);
        SOLID.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn SOLID for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        SOLID.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new SOLID using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        SOLID.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or SOLID from the treasury, using sSOLID as collateral. This might allow an
     * sSOLID holder to provide SOLID liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(SOLID)) {
            require(permissions[STATUS.SOLIDDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sSOLID.changeDebt(value, msg.sender, true);
        require(sSOLID.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(SOLID)) {
            SOLID.mint(msg.sender, value);
            solidDebt = solidDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sSOLID.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with SOLID
     * @param _amount uint256
     */
    function repayDebtWithSOLID(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.SOLIDDEBTOR][msg.sender], notApproved);
        SOLID.burnFrom(msg.sender, _amount);
        sSOLID.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        solidDebt = solidDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(SOLID), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SSOLID) {
            sSOLID = IsSOLID(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SSOLID) {
            // 9
            sSOLID = IsSOLID(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(SOLID.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns SOLID valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(SOLID)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return SOLID.totalSupply() - solidDebt;
    }
}