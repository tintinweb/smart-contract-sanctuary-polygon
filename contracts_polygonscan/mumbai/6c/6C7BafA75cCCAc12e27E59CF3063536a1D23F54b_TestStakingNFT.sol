...
contract TestStakingNFT is IStakingNFT, RewardsAdministratorNFT, ReentrancyGuard, Pausable, IERC721Receiver {
    ...
    function earned(address account, uint256 stakeId) public view override returns (uint256) {
        return userStakes[account][stakeId].multiplier.mul(rewardPerNft(stakeId, userStakes[account][stakeId].requestedTime)).sub(userStakes[account][stakeId].rewardPaid);
    }
    ...
    function batchStake(uint256[] memory tokenIds) external nonReentrant override {
        ...
        userStakes[msg.sender][stakeId] = Stake({tokenIds: tokenIds, multiplier: tokenIds.length, stakedTime: stakeId, requestedTime: 0, unstakedTime: 0, rewardPaid: 0 });
