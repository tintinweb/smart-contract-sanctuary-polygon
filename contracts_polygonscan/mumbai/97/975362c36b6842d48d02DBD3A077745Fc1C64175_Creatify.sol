    ...
    RoyaltiesV2Impl
{
    ...
    bytes32 public constant CREATIFY_ADMIN_ROLE = keccak256("CREATIFY_ADMIN_ROLE");
    bytes32 public constant CREATIFY_OPERATOR_ROLE = keccak256("CREATIFY_OPERATOR_ROLE");
    bytes32 public constant CREATIFY_CREATOR_ROLE = keccak256("CREATIFY_CREATOR_ROLE");
    ...
    /**
     * @dev Grants `CREATIFY_ADMIN_ROLE`, `CREATIFY_CREATOR_ROLE` and `CREATIFY_OPERATOR_ROLE` to the
    ...
    constructor(
        ...
        _setRoleAdmin(CREATIFY_ADMIN_ROLE, CREATIFY_ADMIN_ROLE);
     ...
     * Requirements:
     *
     * - the caller must have the `CREATIFY_CREATOR_ROLE`.
     */
    function createArtifact(string memory metadataHash)
        public
        onlyRole(CREATIFY_CREATOR_ROLE)
        returns (uint256)
    {
    ...
    ) public onlyRole(CREATIFY_OPERATOR_ROLE) returns (uint256) {
        // We cannot just use balanceOf to create the new tokenId because tokens
        // can be burned (destroyed), so we need a separate counter.
        _tokenIdTracker.increment();
        uint256 currentTokenID = _tokenIdTracker.current();
     ...
     * Requirements:
     *
     * - the caller must have the `CREATIFY_OPERATOR_ROLE`.
     */
    function pause() public onlyRole(CREATIFY_OPERATOR_ROLE) {
        _pause();
    }
    ...
    ) public onlyRole(CREATIFY_ADMIN_ROLE) {
     ...
     * - the caller must have the `CREATIFY_OPERATOR_ROLE`.
     */
    function unpause() public onlyRole(CREATIFY_OPERATOR_ROLE) {
        _unpause();
    }
