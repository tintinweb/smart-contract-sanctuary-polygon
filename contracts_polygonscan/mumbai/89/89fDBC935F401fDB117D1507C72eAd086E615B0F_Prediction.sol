...
contract Prediction is AccessProtected, Feed, KeeperCompatible {
    ...
    // requestId to matchId mapping
    mapping(bytes32 => RequestFor) public requestToMatch;
    ...
    PendingResult[] public PendingResults;

    mapping(uint8 => mapping(uint256 => uint256)) public _pendingResultIndex;
    ...
    event LeagueAdded(uint8 indexed sportId, string name, string sport);
    ...
    event MatchResultAdded(uint256 indexed matchId, MatchResult matchResult);
    ...
    constructor(
        ...
        bytes32 _boolJobId,
        uint256 _fee
    ) Feed(_linkToken, _oracle, _boolJobId, _fee) {
        bund = _bund;
    }
    ...
    function addLeague(League calldata _league) external onlyAdmin {
        require(
            bytes(_league.name).length != 0 &&
                bytes(_league.sport).length != 0 &&
                bytes(_league.matchResultUrl).length != 0
        );
    ...
    function _updateLeague(uint8 _id, League calldata _league)
        external
        onlyAdmin
    {
    ...
    function addPool(Pool calldata _pool) external onlyAdmin {
        ...
        for (uint256 i = 0; i < _pool.matchIds.length; i++) {
            ...
            require(matches[matchId].endTime != 0, "Match Not present");
        }
        pools.push(_pool);
        emit PoolAdded(
            pools.length - 1,
    ...
    function _addMatch(Match memory matchData) private {
        ...
        uint256 matchId = matchIndex[leagueId][espnMatchId];
        require(
            matchId == 0 &&
        ...
        _addMatchesToEnumeration(leagueId, matches.length - 1);
        emit MatchAdded(
            leagueId,
            matchData.season,
            matches.length - 1,
    ...
    function addMatch(Match calldata _matchData) external onlyAdmin {
        _addMatch(_matchData);
    }

    function addMatches(Match[] calldata _matchData) external onlyAdmin {
        for (uint256 i = 0; i < _matchData.length; i++) {
            _addMatch(_matchData[i]);
        }
    }

    function _addMatchesToEnumeration(uint8 leagueId, uint256 matchId) private {
        PendingResults.push(PendingResult(matchId, leagueId, false, false));
        _pendingResultIndex[leagueId][matchId] = PendingResults.length - 1;
    }

    function _removeMatchFromEnumeration(uint8 leagueId, uint256 matchId)
        private
    {
        uint256 lastResultIndex = PendingResults.length - 1;
        uint256 resultIndex = _pendingResultIndex[leagueId][matchId];

        PendingResult memory lastResult = PendingResults[lastResultIndex];

        PendingResults[resultIndex] = lastResult;

        _pendingResultIndex[lastResult.leagueId][
            lastResult.matchId
        ] = resultIndex;

        delete _pendingResultIndex[leagueId][matchId];
        PendingResults.pop();
    }
    ...
    {
        for (uint256 i = 0; i < PendingResults.length; i++) {
            uint256 matchId = PendingResults[i].matchId;
            if (
                block.timestamp > matches[matchId].endTime &&
                block.timestamp - lastTimeStamp > 120
            ) {
                if (!(matches[matchId].matchResult == MatchResult.Cancelled)) {
                    if (!PendingResults[i].updatedTeamA) {
                        upkeepNeeded = true;
                        performData = abi.encode(matchId, 0);
                    } else if (!PendingResults[i].updatedTeamB) {
                        upkeepNeeded = true;
                        performData = abi.encode(matchId, 1);
                    }
                }
    ...
    function performUpkeep(bytes calldata performData) external override {
        (uint256 matchId, uint8 path) = abi.decode(
            performData,
            (uint256, uint8)
        );
        uint8 leagueId = matches[matchId].leagueId;
        string memory url = matchResultUrl(
            leagues[leagueId].matchResultUrl,
            matches[matchId].espnMatchId
        );
        lastTimeStamp = block.timestamp;
        // if (path == 3) {
        //     bytes32 requestId = requestMatchResult(
        //         url,
        //         path,
        //         this.fulfillBytes.selector
        //     );
        //     requestToMatch[requestId] = RequestFor(matchId, path);
        // } else {
        bytes32 requestId = requestMatchResult(
            url,
            path,
        ...
        if (path == 1) {
            _removeMatchFromEnumeration(leagueId, matchId);
        } else {
            uint256 index = _pendingResultIndex[leagueId][matchId];
            PendingResults[index].updatedTeamA = true;
        }
     ...
     */
    function fulfillBool(bytes32 _requestId, bool _winner)
        public
        recordChainlinkFulfillment(_requestId)
    {
        uint256 matchId = requestToMatch[_requestId].matchId;
        uint8 path = requestToMatch[_requestId].path;
        if (path == 0) {
            if (_winner) {
                matches[matchId].matchResult = MatchResult.TeamA;
                emit MatchResultAdded(matchId, MatchResult.TeamA);
            }
        } else {
            if (_winner) {
                matches[matchId].matchResult = MatchResult.TeamB;
                emit MatchResultAdded(matchId, MatchResult.TeamB);
            } else if (matches[matchId].matchResult != MatchResult.TeamA) {
                matches[matchId].matchResult = MatchResult.Drawn;
                emit MatchResultAdded(matchId, MatchResult.Drawn);
            }
        }
    }

    // function fulfillBytes(bytes32 _requestId, bytes memory _status)
    //     public
    //     recordChainlinkFulfillment(_requestId)
    // {
    //     uint256 matchId = requestToMatch[_requestId].matchId;
    //     if (keccak256(bytes("STATUS_CANCELLED")) == keccak256(bytes(_status))) {
...
contract Feed is ChainlinkClient {
    ...
    bytes32 internal boolJobId;
    bytes32 internal bytesJobId;
    ...
    // function matchStatusUrl(string memory _baseUrl)
    //     public
    //     pure
    //     returns (string memory)
    // {
    ...
    function requestMatchResult(
        ...
        //     );
        //     request.add("get", matchStatusUrl(_url));
        // } else {
        request = buildChainlinkRequest(
            boolJobId,
            address(this),
        ...
        if (_path == 0) {
            ...
            request.add("path", "competitors.1.winner");
        }
