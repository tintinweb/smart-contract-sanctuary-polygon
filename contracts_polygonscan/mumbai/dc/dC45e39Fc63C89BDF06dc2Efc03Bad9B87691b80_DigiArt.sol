...
contract DigiArt is ERC1155, ERC1155Holder, Ownable {
  ...
  function buySingleToken(uint256 _tokenId) public payable {
    ...
    if(design.isCustomToken) {
      uint tokenAmount = (design.price * 99)/100;
      ...
      token.transferFrom(msg.sender, address(this), design.price-tokenAmount);
      tokenSpent[msg.sender] +=design.price;
    }
    else {
      uint tokenAmount = (design.price * 95)/100;
      // User can't send more tokens than they have
      require(wEth.balanceOf(msg.sender) >=tokenAmount);

      wEth.transferFrom(msg.sender, sendTo, tokenAmount);
      wEth.transferFrom(msg.sender, address(this), design.price-tokenAmount);
      wethSpent[msg.sender] +=design.price;
    } 
    // Transfer
