...
contract NFT is ERC721MintMore, Ownable {
    ...
    constructor(string memory _tokenName, string memory _tokenSymbol, address _productAddress) ERC721MintMore(_tokenName, _tokenSymbol) {
    ...
    function mint(address _recipient, string memory _name) public onlyOwner returns (uint) {
        ...
        require(charMatch(_name), 'mint: Name can contain only a-z, A-Z, 0-9, space and dot');
        _safeMint(_recipient, tokenCount);
        mintAddDetails(_name);
        return tokenCount - 1;
    }

    function mintAddDetails(string memory _name) private onlyOwner {
    ...
    function mintMore(address _recipient, string memory _name, uint _count) public onlyOwner returns (uint) {
        require(getUTFStrLen(_name) <= 16, 'mintMore: Name is too long. Maximum: 16 characters');
        require(charMatch(_name), 'mintMore: Name can contain only a-z, A-Z, 0-9, space and dot');
        _mintMore(_recipient, tokenCount, _count);
        for (uint i = 0; i < _count; i++) {
            //mintAddDetails(string(abi.encodePacked(_name, ' ', Strings.toString(i))));
            tokenDetails[tokenCount] = Details(
                _name,
                getRandomNumber(2) == 1 ? true : false,
                getRandomNumber(3),
    ...
    function test(address _recipient, uint _count) public onlyOwner {
        _mintMore(_recipient, tokenCount, _count);
    }
...
contract ERC721MintMore is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _mintMore(address to, uint startTokenID, uint count) internal {
        require(to != address(0), 'ERC721: mint to the zero address');
