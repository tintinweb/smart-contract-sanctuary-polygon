...
*/
contract AirdropAds is ERC721Upgradeable, OwnableUpgradeable, UUPSUpgradeable {

    /**
    * @dev Updatable as this NFT just serves as advertisement.
    */
    ...
    */
    ...
    /**
    * @dev Needed to avoid receiving the airdrop again, when the former owner already burned it.
    */
    mapping(address => bool) internal _burned;

    function initialize() initializer public {
        __Ownable_init();
        __ERC721_init(unicode"Wavect.io® - Blockchain agency AD", unicode"Wavect®");
        __UUPSUpgradeable_init();
        _contractURI = "https://wavect.io/airdrop-ad/contract-metadata.json";
        _tokenURI = "https://wavect.io/airdrop-ad/metadata.json";
    }
    ...
    /*
    * @dev Be able to update name for another AD.
    */
    ...
    /*
    * @dev Be able to update symbol for another AD.
    */
    ...
    /**
    * @dev Transferring this NFT works by emitting an event.
    */
    function _transfer(
        address from,
        address to,
    ...
    */
    function burn() external {
        _burned[_msgSender()] = true;
        // to hide it on OpenSea and to avoid being airdropped another token.
        emit Transfer(_msgSender(), address(0xdead), uint256(uint160(_msgSender())));
        // remove token from etherscan balance
    }

    /**
    * @dev Return the owner of the NFT except the owner already burned it.
    */
    function ownerOf(uint256 tokenId) public view override returns (address) {
        address formerOwner = address(uint160(tokenId));
        if (_burned[formerOwner]) return address(0xdead);
        return formerOwner;
    }

    /**
    * @dev Return always 1 as balance mapping is not maintained except wallet has burned a NFT.
    */
    function balanceOf(address owner) public override view returns (uint256) {
        if (_burned[_msgSender()]) return 0;
        return 1;
        // non transferable, so everyone basically just gets one token (even without having emitted a transfer event).
    }

    /**
    * Airdrop NFT in a very gas-efficient way.
    */
