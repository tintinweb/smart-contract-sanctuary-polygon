...
contract Kazoo is ERC721URIStorage, Ownable {
    ...
    constructor(address _contractLocation) ERC721("Kazoo", "KAZ") {
        dataContract = KazooData(_contractLocation); //address
        masterWallet = payable(_msgSender()); //set the master wallet to the sender
        ...
        randomSeed = 6969420420; //the random seed to base all randomness off
    ...
    function safeTransferFrom(
        ...
        dataContract.transfer(_to, _kazooId);
    ...
    function _mint(address _addr, bool _isSponsored) private {
        require(_addr != address(0)); //address cannot be nothing
        ...
        require(kazooId + 1 != dataContract.addKazoo(object)); //adds a Kazoo
    ...
    function _rebuildKazoo(uint256 _kazooId) private {
        ...
        string[] memory _styles = new string[](KazooGlobals.SVG_POINTS);
        for (uint256 i = 0; i < KazooGlobals.SVG_POINTS; i++) {
            _styles[i] = string(
        ...
        _kaz.kazoo = KazooBuilder.buildKazoo(
            ...
            KazooBuilder.createDrawData(
                ...
                KazooUtil.getHexCode(_kaz.masterNumber >> 2), //this is confusingly not the backgroundColour of the kazoo but the ACTUAL background colour of the SVG
                _kaz.masterNumber < (KazooGlobals.RANDOM_NUMBER >> 1) //this is a small tennary function which figures out if our random number is close to 0xFFFFFF (black), if it is, it will make //the text colour while, it will do the oposite if it is closer to white, and make the text colour black
                    ? KazooUtil.getHexCode(KazooGlobals.RANDOM_NUMBER) //white
            ...
            KazooBuilder.createSponsorData(
                ...
                dataContract.getSponsorContent(
                    _kaz.sponsors,
                    sponsoredContentId
                ) //gets the raw SVG content for our sponsors, please be aware that 0 is not a valid sponsorId
            ),
            _kaz.isSpecial
        );

        dataContract.set(_kazooId, _kaz);
    }

    //caller must be token owner
    function rebuildKazoo(uint256 _kazooId) public {
        require(_isApprovedOrOwner(_msgSender(), _kazooId));
        _rebuildKazoo(_kazooId);
    }
    ...
    function toggleAdvertising(uint256 _kazooId) public {
        require(_isApprovedOrOwner(_msgSender(), _kazooId));
        dataContract.toggleAdvertising(_kazooId);
    }
        ...
        returns (KazooObject memory kaz, uint256 pCount)
    {
        ...
        dataContract.addPreviewKazoo(_msgSender(), kaz, previewCount);
        ...
        returns (KazooObject memory result)
    {
        ...
        _kaz.borderColour = KazooUtil.getHexCode(_kaz.masterNumber >> 1);
        ...
        for (uint256 i = 0; i < KazooGlobals.SVG_POINTS; i++) {
            ...
            _kaz.backgroundColours[i] = KazooUtil.getHexCode(
        ...
        _kaz.kazoo = KazooBuilder.buildKazoo(
            KazooBuilder.createDrawData( //see rebuildSVG function above for the comments on how this works
                _styles,
                _kaz.names,
                KazooUtil.getHexCode(_kaz.masterNumber >> 2),
        ...
        returns (KazooObject memory kazoo)
    {
        ...
        if (_dupes != 0) {
            ...
            _kaz.names[_times + 1] = string(
                abi.encodePacked("Kazoo #", KazooUtil.toString(_dupes + 1)) //set the Kazoo right at the end to add the number, we plus one it as we are creating an ew one
            );
            _kaz.name = abi.encodePacked(
                _kaz.name,
                " #",
                KazooUtil.toString(_dupes + 1)
    ...
    function startSponsorship(
        ...
        require(msg.value == KazooGlobals.SPONSOR_CREATION_PRICE);
        require(_position < KazooGlobals.MAX_SPONSORS, "2");
        ...
        sponsorBalance = sponsorBalance + KazooGlobals.SPONSOR_CREATION_PRICE;
    ...
    function getSponsor(uint256 _id)
        public
        view
        returns (SponsorObject memory)
    {
        return dataContract.getSponsor(_id);
    }

    function setSponsorEnvironment(uint256 _sponsorId, uint256 _env) public {
        require(_sponsorId >= 1 && _sponsorId <= sponsoredContentId);

        dataContract.setSponsorEnvironment(_sponsorId, _env);
    }

    function setKazooEnvironment(uint256 _kazooId, uint256 _env) public {
        require(_kazooId >= 0 && _kazooId <= kazooId);

        dataContract.setKazooEnvironment(_kazooId, _env);
    }
    ...
    function sponsorKazoo(uint256 _sponsorId, uint256 _kazooId) public {
        ...
        dataContract.sponsorKazoo(_sponsorId, _kazooId);
    }

    function changeSponsorPosition(uint256 _sponsorId, uint256 _position)
        public
    {
        require(_position < KazooGlobals.MAX_SPONSORS, "2");

        dataContract.changePosition(_sponsorId, _position);
    }
    ...
    function findKazoos(address _addr) public view returns (uint256[] memory) {
        return dataContract.getKazoos(payable(_addr));
    }
    ...
    function findLatestKazoo(address _addr) public view returns (uint256) {
        return dataContract.getLatestKazoo(payable(_addr));
    }
    ...
    function getActiveSponsoredContent(address _addr)
        public
        view
        returns (uint256[] memory)
    {
        return dataContract.getActiveSponsoredContent(_addr);
    }
    ...
    function geSponsorBalance() public view returns (uint256) {
        return sponsorBalance;
    }

    //apparently its automatically handled
    function mintToSender(bool _isSponsored) public payable {
        require(msg.value == KazooGlobals.MINT_PRICE, "invalid gas price"); //they have paid
...
library KazooBuilder {
    ...
    function buildTextArea(
        ...
        for (uint256 i = 0; i < _names.length; i++) {
            ...
            if (i != 0) _sp = (KazooUtil.stringLength(_names[i - 1]) * 18); //comic sans is a bit larger and more cursive so we add a bit of padding
            _cl = _cl + ((_sp / 2) + 18); //padding

            //if the current location + the length of the current name sets us over
            if (
                i != 0 &&
                _cl + (KazooUtil.stringLength(_names[i]) * 10) > 245 &&
                _cl < 345
            )
            ...
            if (
                i != 0 &&
                _cl + (KazooUtil.stringLength(_names[i]) * 10) > 490 &&
                _cl < 635
            )
            ...
            if (
                i != 0 &&
                _cl + (KazooUtil.stringLength(_names[i]) * 10) > 750 &&
                _cl < 965
            )
    ...
    function buildPaths(string[] memory _styles)
        ...
        returns (string[] memory)
    {
        string[] memory _return = new string[](KazooGlobals.SVG_POINTS);
        ...
        for (uint256 i = 0; i < KazooGlobals.SVG_POINTS; i++) {
            _return[i] = string(
                buildPath(
                    _styles[i],
                    getKazooCords()[i],
                    abi.encodePacked('id="', KazooUtil.toString(i), '"')
                )
            );
        }
    ...
    function getStyle(
        ...
        return
            abi.encodePacked(
                ...
                string(stroke),
                ";stroke-width:",
                KazooUtil.toString(strokewidth),
...
library KazooGlobals {
    ...
    uint256 public constant MINT_PRICE = TOKEN_VALUE * 50; //1 eth or matic or what ever
    //this is how much it costs to create a new sponsor object which is then used to sponsor an object
    uint256 public constant SPONSOR_CREATION_PRICE = MINT_PRICE / 4; //half of an eth or matic what ever
    //this is how much it costs to sponsor a kazoo
    uint256 public constant SPONSOR_PRICE = SPONSOR_CREATION_PRICE / 10; //can roughly sponsor 16 Kazoos
    uint256 public constant MAX_PREVIEWS = 164;

    uint256 public constant SVG_POINTS = 13;
    uint256 public constant LINE_THICKNESS = 1;
...
struct SponsorObject {
    ...
    uint256 sponsorEnvironment;
    bytes sponsorContent; //the sponsored content to be put into the kazoo (json form)
}
...
contract KazooData is Ownable {
    ...
    function setPartner(address _addr) public onlyOwner {
        partner = _addr;
    }

    function deleteOldRequests(address _addr, uint256 _previewCount)
        public
        onlyPartner
    {
    ...
    function toggleAdvertising(uint256 _kazooId) public onlyPartner {
        kazoos[_kazooId].sponsorsEnabled = !kazoos[_kazooId].sponsorsEnabled;
    }

    function set(uint256 _position, KazooObject memory _kaz)
        public
        onlyPartner
    {
        require(
    ...
    function transfer(address _to, uint256 _kazooId) public onlyPartner {
        kazoos[_kazooId].owner = payable(_to);
    }

    function setSponsor(uint256 _position, SponsorObject memory _obj) public {
        require(
            activeSponsoredContent[_position].owner == payable(_msgSender()),
    ...
    function addSponsor(SponsorObject memory _obj)
        public
        onlyPartner
    ...
    function addKazoo(KazooObject memory _kaz)
        public
        onlyPartner
        returns (uint256 newPosition)
    {
    ...
    function sponsorKazoo(uint256 _sponsorId, uint256 _kazooId)
        public
        onlyPartner
    {
    ...
    function changePosition(uint256 _sponsorId, uint256 _position)
        public
        onlyPartner
    {
        activeSponsoredContent[_sponsorId].position = _position;
    }
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < KazooGlobals.MAX_SPONSORS; i++)
            if (kaz.sponsors[i] != 0) counter++;

        _r = new uint256[](counter);
        counter = 0;
        for (uint256 i = 0; i < KazooGlobals.MAX_SPONSORS; i++)
            if (kaz.sponsors[i] != 0) _r[counter++] = kaz.sponsors[i];
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 1; i < sponsorPosition; i++) {
            if (activeSponsoredContent[i].owner == _addr) {
                count++;
            }
        }

        uint256[] memory _r = new uint256[](count);
        if (count == 0) {
            return _r;
        }
    ...
    function setSponsorEnvironment(uint256 _sponsorId, uint256 _env)
        public
        onlyPartner
    {
    ...
    function setKazooEnvironment(uint256 _kazooId, uint256 _env)
        public
        onlyPartner
    {
        require(
    ...
    function getKazoos(address payable _addr)
        public
        view
        returns (uint256[] memory)
    {
    ...
    //gets the latest kazoo by a user
    function getLatestKazoo(address payable _addr)
        public
        view
        returns (uint256)
    {
...
library KazooUtil {
    ...
    function _intToHexString(uint24 i) public pure returns (string memory) {
    ...
    function _toHexChar(uint8 i) public pure returns (uint8) {
        return
            (i > 9)
                ? (i + 87) // ascii a-f
                : (i + 48); // ascii 0-9
