...
contract TestStakingNFT is IStakingNFT, RewardsAdministratorNFT, ReentrancyGuard, Pausable, IERC721Receiver {
    ...
    event NftRequestUnstake(address indexed user, uint256[] tokenIds, uint256 requestedTime);
    ...
    event SetUnstakePeriod(uint256 newPeriod);
    event SetMaxLength(uint256 newMaxLength);
    ...
    uint256 public rewardCycle = 5 minutes;
    ...
    function earned(address account, uint256 stakeId) public view override returns (uint256) {
        return userStakes[account][stakeId].multiplier.mul(rewardPerNft(stakeId, userStakes[account][stakeId].requestedTime).sub(userStakes[account][stakeId].rewardGlobal)).sub(userStakes[account][stakeId].rewardPaid);
    }

    function batchStake(uint256[] memory tokenIds) external nonReentrant override {
        uint256 stakeId = block.timestamp;
        require(stakeTimes[msg.sender].length < maxLength, "Stake: Overflow packages!");
        require(tokenIds.length < maxLength, "Stake: Overflow NFT length!");
        ...
        userStakes[msg.sender][stakeId] = Stake({tokenIds: tokenIds, multiplier: tokenIds.length, stakedTime: stakeId, requestedTime: 0, unstakedTime: 0, rewardPaid: 0, rewardGlobal: rewardPerNftGlobal});
    ...
    function requestUnstake(uint stakeIndex) public override {
        ...
        require(userStakes[msg.sender][stakeId].requestedTime == 0, "Stake: Already requested!");
        userStakes[msg.sender][stakeId].requestedTime = block.timestamp;
        userStakes[msg.sender][stakeId].unstakedTime = block.timestamp + unstakePeriod; // not really need checking overflow here~!
        emit NftRequestUnstake(msg.sender, userStakes[msg.sender][stakeId].tokenIds, userStakes[msg.sender][stakeId].requestedTime);
    }

    function unstake(uint256 stakeIndex) external nonReentrant override {
        uint256 stakeId = stakeTimes[msg.sender][stakeIndex];
        require(userStakes[msg.sender][stakeId].requestedTime != 0 && block.timestamp >= userStakes[msg.sender][stakeId].unstakedTime, "Stake: Not enough time yet!");
    ...
    function getStake(address account, uint256 stakeId) public view override returns (uint256[] memory tokenIds, uint256 multiplier, uint256 stakedTime, uint256 requestedTime, uint256 unstakedTime, uint256 rewardPaid) {
        return (
            userStakes[account][stakeId].tokenIds,
            userStakes[account][stakeId].multiplier,
            userStakes[account][stakeId].stakedTime,
            userStakes[account][stakeId].requestedTime,
    ...
    function claimReward(uint256 stakeId) public nonReentrant override {
        uint256 reward = earned(msg.sender, stakeId);
        if (reward > 0) {
    ...
    function openCampaign(uint256 _periodStart, uint256 _periodFinish, uint256 _rate) external onlyRewardsAdministrator updateReward() override {
        require((_periodStart >= block.timestamp || _periodStart == periodStart) && _periodFinish > block.timestamp, "Stake: Invalid time!");
    ...
    function setUnstakePeriod(uint256 _unstakePeriod) external onlyOwner {
        ...
        emit SetUnstakePeriod(_unstakePeriod);
    }
...
interface IStakingNFT {
    ...
    function getStake(address account, uint256 stakeId) external view returns (uint256[] memory, uint256, uint256, uint256, uint256, uint256);
