...
contract TestTestTest is ERC721A, Ownable, ReentrancyGuard {
     enum Status {
        Pending,
        PreSale,
        PublicSale,
    ...
    uint256 public saleMaxPerWallet = 20;
    ...
    function presaleMint(uint256 _amount, bytes32[] calldata _merkleProof) external payable {
        require(status == Status.PreSale, "Presale is not active");
        require(
            tx.origin == msg.sender,
            "Contract is not allowed to mint"
        );
        require(_amount <= maxBatchSize, "Exceeded Max Batch Size");
        require(presaleMintPrice * _amount == msg.value, "Invalid value");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Not on the list");
        require(
            presaleMaxPerWallet >= presaleRedeemedCount[msg.sender] + _amount,
            "Max mint amount per wallet exceeded"
        );

        presaleRedeemedCount[msg.sender] = presaleRedeemedCount[msg.sender] + _amount;
        require(
    ...
    function mint(uint256 _amount) external payable {
        ...
        require(_amount <= maxBatchSize, "Exceeded Max Batch Size");
        require(publicMintPrice * _amount == msg.value, "Invalid value");
        require(
            saleMaxPerWallet >= saleRedeemedCount[msg.sender] + _amount,
            "Max mint amount per wallet exceeded"
        );

        saleRedeemedCount[msg.sender] = saleRedeemedCount[msg.sender] + _amount;
        require(
            totalSupply() + _amount + reserveAmount - tokensReserved <=
                maxSupply,
            "Max supply exceeded"
    ...
    function setPresaleMintPrice(uint256 newPrice) external onlyOwner {
        presaleMintPrice = newPrice;
    }
    ...
    function withdraw() external onlyOwner {
        require(status == Status.Finished, "Invalid status for withdraw");
