...
contract LootBox is Ownable, Pausable, ReentrancyGuard, VRFConsumerBase {
  ...
  function estimateCost(
    ...
    uint256 globalDiscount = _amount;
    if (discountCountMap[tierDiscountSettings][_toAddress] < settings.maxDiscountTime
      && discountWhitelistMap[tierDiscountSettings][_toAddress] > 0) {
      ...
      globalDiscount -= tierDiscount;
    } 
    
    if (globalDiscount > 0 && discountWhitelistMap[globalDiscountsettings][_toAddress] > 0) {
      // global whitelist discount
      cost = cost - globalDiscount * settings.price * discountWhitelistMap[globalDiscountsettings][_toAddress] / PERCENT_BASIS_POINT;
    }
