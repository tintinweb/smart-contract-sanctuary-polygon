...
contract NftToken is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    event mintForMinerEvent(address _to,string _tokenURI,uint256 tokenId);
    ...
    function mintForMiner(address _to,string memory _tokenURI) public nonReentrant returns (bool, uint256) {
        ...
        emit mintForMinerEvent(_to,_tokenURI,tokenIdNow);
        return (true, tokenIdNow);
    }
	
	struct mintForMinerItem {
	   address _to;
	   string _tokenURI;
	   uint256 tokenId;
	}
	
	event MassMintForMinerEvent(mintForMinerItem[] tokenIdList);
	
	 function MassMintForMiner(address _to,string[] memory _tokenURI_List) public nonReentrant returns (bool, mintForMinerItem[] memory tokenIdList) {
        require(MinerList[msg.sender] == true, "e002");
        require(_to != address(0), "e003");
		uint256 num = _tokenURI_List.length;
		tokenIdList = new mintForMinerItem[](num);
		for (uint256 i=0;i<num;i++) {
		tokenIdNow = tokenIdNow + 1;
        _safeMint(_to, tokenIdNow, _tokenURI_List[i]);
        tokenIdList[i] = mintForMinerItem(_to,_tokenURI_List[i],tokenIdNow);
		}
		emit MassMintForMinerEvent(tokenIdList);
        return (true, tokenIdList);
    }
