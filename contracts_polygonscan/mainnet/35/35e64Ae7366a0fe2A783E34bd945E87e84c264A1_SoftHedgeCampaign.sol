...
contract SoftHedgeCampaign {
	...
	modifier onlyDAO() {
		require(msg.sender == DAO, "SHC: DAO ONLY");
		_;
	}

	constructor(address DAO_, address rewardToken_) {
		require(DAO_ != address(0), "SHC: DAO ADDR ZERO!");
		require(rewardToken_ != address(0), "SHC: TOKEN ADDR ZERO!");
		DAO = DAO_;
	...
	function claim() external {
		require(winners[msg.sender] != 0, "SHC: NO BONUS");
		require(isWinner[msg.sender] == true, "SHC: WINNERS ONLY");
		...
		SafeERC20.safeTransfer(IERC20(rewardToken), msg.sender, bonus);
	}

	// ---------------- ADMIN OR DAO ONLY ------------------ //
	function setWinner(address winner, uint256 amount) external onlyDAO {
		require(winner != address(0), "SHC: DAO");
		require(amount != 0, "SHC: ZERO AMOUNT");
  ...
  function resetWinner(address player) external onlyDAO {
    require(isWinner[player] == true, "SHC: SHOULD BE WINNER");
    
    isWinner[player] = false;
    winners[player] = 0;
	...
	function setDAO(address _newDAO) external onlyDAO {
		require(_newDAO != address(0), "SHC: set _DAO to the zero address");
		DAO = _newDAO;
	}
