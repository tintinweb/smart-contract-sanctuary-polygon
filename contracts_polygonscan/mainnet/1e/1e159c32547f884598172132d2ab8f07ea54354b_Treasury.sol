...
contract Treasury is ContractGuard {
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of COCA price
    ...
    uint256 public mintingFactorForPayingDebt; // print extra COCA during debt phase
    ...
    function getTombPrice() public view returns (uint256 tombPrice) {
        try IOracle(tombOracle).consult(tomb, 1e18) returns (uint144 price) {
            ...
            revert("Treasury: failed to consult COCA price from the oracle");
        }
    }
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (_tombPrice <= tombPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = tombPriceOne.mul(1e18).div(_tombPrice); // to burn 1 COCA
    ...
    function initialize(
        ...
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn COCA and mint COPS)
        maxDebtRatioPercent = 3500; // Upto 35% supply of COPS to purchase
    ...
    function buyBonds(uint256 _tombAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        require(tombPrice == targetPrice, "Treasury: COCA price moved");
        require(
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            if (previousEpochTombPrice > tombPriceCeiling) {
                // Expansion ($COCA Price > 1 $FTM): there is some seigniorage to be allocated
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(tbond), "cops");
        require(address(_token) != address(tshare), "gringo");
