...
contract GreedyBots is OwnableUpgradeable, ERC721EnumerableUpgradeable{
    ...
    */
    ...
    function mint(address _to,uint256 _qty) external payable {
        ...
        if( block.number < preSaleEnd ){
           ...
           require((minted[msg.sender] + _qty) < 10,"Maximum reached");
           yourPrice = cost * _qty;
    ...
    function tokenURI(uint256 _id) public view virtual override returns (string memory) {
        return string(abi.encodePacked('https://ipfs.io/ipfs/', unRevealedURI,'/', _id.toString() ));
    } 
    ...
    function updateBotURI(uint256 version, string memory _ipfshash) external onlyOwner{
        botUri[version] = string(abi.encodePacked(_ipfshash));

    }

    function extendPresale(uint256 _block) external onlyOwner{
        preSaleEnd = _block;
    }

    /// @notice For Opensea Royalties
    function botURI(uint _version) public view returns (string memory) {
        if(balanceOf(msg.sender) > 0){
            return string(abi.encodePacked('https://ipfs.io/ipfs/',botUri[_version]));
        }else{
            return string(abi.encodePacked('You do not own a Greedy Bot'));
        }
