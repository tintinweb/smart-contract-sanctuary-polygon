...
contract OxCATS is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  // public
  function oxcost() public view returns (uint256 _cost) {
    uint256 supply = totalSupply();

    if(dynamicCost) { 
        _cost = needToUpdateCost(supply);       
        return _cost;
    }
    return cost;      
  }
  ...
  function setDynamicCost(bool _dCostState) public onlyOwner {
    dynamicCost = _dCostState;
  }
