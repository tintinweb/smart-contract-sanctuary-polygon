...
contract USMART is Context, IERC20, Ownable {
    ...
    address private _MarWalletAddress = 0x990a281468E31e03a5B526D21D289e9B21f590C5;
    ...
    function setMarWalletAddress(address newAdd) external onlyOwner() {
        _MarWalletAddress = newAdd;
    }
    ...
    function _takedev(uint256 tdev) private {
        uint256 currentRate =  _getRate();
        uint256 rdev = tdev.mul(currentRate);
        _rOwned[_MarWalletAddress] = _rOwned[_MarWalletAddress].add(rdev);
        if(_isExcluded[_MarWalletAddress])
            _tOwned[_MarWalletAddress] = _tOwned[_MarWalletAddress].add(tdev);
    }
