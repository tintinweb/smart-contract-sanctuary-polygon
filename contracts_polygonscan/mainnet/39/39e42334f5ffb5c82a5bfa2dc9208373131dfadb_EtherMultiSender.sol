...
contract EtherMultiSender {
    ...
    function AirDropForNFT(address token, address payable[] memory _recipients, uint256 balances) public payable {
        ...
        for(i; i < _recipients.length; i++) {
            erc20.transferFrom(msg.sender, _recipients[i], balances);
        }
    }
}