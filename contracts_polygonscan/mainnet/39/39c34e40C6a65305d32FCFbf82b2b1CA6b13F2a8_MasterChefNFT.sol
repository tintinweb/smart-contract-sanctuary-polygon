...
contract MasterChefNFT is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    function pendingTheEndless(address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[theEndlessPID];
...
contract TreasuryDAO is ReentrancyGuard, AccessControl {
     ...
     */
    constructor(address _wnativeToken, address _usdCurrency, IUniswapV2Router02 _sandManSwapRouter) {
     ...
     * Can only be called by the current owner.
     */
    function getUSDDrip() external onlyRole(OPERATOR_ROLE) returns (uint256) {
    ...
    function convertDepositFeesToUSD(address token, uint256 tokenType) public {
        ...
        // uint256 usdValue = getTokenUSDValue(totalTokenBalance, token, tokenType, false, address(usdCurrency));
        ...
        // uint256 usdPriorBalance = usdCurrency.balanceOf(address(this));
        ...
        // uint256 usdProfit = usdCurrency.balanceOf(address(this)) - usdPriorBalance;
