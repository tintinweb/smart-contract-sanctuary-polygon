...
contract Token is Context, IERC20, Ownable {
     ...
     * Token configuration
     */
    string private _name = "ProtocolV8";
    string private _symbol = "POL8";
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        _transferPrep(_msgSender(), recipient, amount);
        return true;
    }
    ...
    function transferFrom(
        ...
        _transferPrep(sender, recipient, amount);
        return true;
    }
    ...
    {
        uint256 tTreasury = calculateTreasuryFee(amount);
        uint256 tLiquidity = calculateLiquidityFee(amount);
        uint256 tBurn = calculateBurnFee(amount);
        uint256 tTransferAmount = amount.sub(tLiquidity).sub(tTreasury).sub(
            tBurn
        );
        return (tTransferAmount, tTreasury, tLiquidity, tBurn);
    }
    ...
    /**
     * @dev Similar as transferOwnership, but for treasury
     * Can only be called by the current owner.
     */
    function transferTreasury(address newTreasury) public virtual onlyOwner {
        require(
            newTreasury != address(0),
            "Ownable: new treasury is the zero address"
        );
    ...
    function removeAllFee() private {
        if (_liquidityFee == 0 && _treasuryFee == 0 && _burnFee == 0) return;
    ...
    function _transferPrep(
        ...
        _initTransfer(from, to, amount, takeFee);
    }
    ...
    function _initTransfer(
        ...
        _transfer(sender, recipient, tTransferAmount); // transfer to recipient
        _transfer(sender, address(this), tLiquidity); // liquidity fee
        _transfer(sender, _treasury, tTreasury); // treasury fee
        _burn(sender, tBurn);
