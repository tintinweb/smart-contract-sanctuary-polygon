...
contract MasterChefNFT is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    uint256  constant sandManMaximumSupply = 500 * (10 ** 3) * (10 ** 18); // 500,000 sandManToken
    uint256 constant MAX_EMISSION_RATE = 10 * (10 ** 18); // 10
    uint256 constant MAXIMUM_HARVEST_INTERVAL = 4 hours;
    
    // The SANDMAN TOKEN!
    SandManToken  immutable sandManToken;
    // SandMan Treasury
    TreasuryDAO  immutable treasuryDAO;
    // The THE ENDLESS TOKEN!
    TheEndlessToken  immutable theEndlessToken;    
    // Interface NFT FACTORY
    IFactoryNFT  immutable iFactoryNFT;
    // Interface Merge API
    ...
    function _payPendingTheEndlessSandMan(uint256 _pid) internal {
        ...
        emit Log("pay pending start", _pid);
        ...
        if (canHarvest(_pid, msg.sender)) {
            ...
            emit Log("pay sandman", pendingSandManToken);
            if (_pid == theEndlessPID) {
                emit Log("starting pay usd", pendingSandManToken);
                _payPendingUSDReward();
                emit Log("finish pay usd", pendingSandManToken);
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        emit Log("starting", _pid);
        updatePool(_pid);
        emit Log("update pool", _pid);
        _payPendingTheEndlessSandMan(_pid);
        emit Log("pay pending", _pid);
