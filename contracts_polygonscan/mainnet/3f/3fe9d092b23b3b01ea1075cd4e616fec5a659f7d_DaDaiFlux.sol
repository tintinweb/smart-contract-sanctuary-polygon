...
contract Stakeable {
    ...
    * @notice Staked event is triggered whenever a user stakes tokens, address is indexed to make it filterable
     ...
     */
    uint256 internal rewardPerHour = 1000; // 1000 predstavlja 0,1% po satu po za svaki stejkovan token
...
*/
contract DaDaiFlux is Ownable, Stakeable{
  ...
  */
  function _mint(address account, uint256 amount) internal {
    require(account != address(0), "Flux: cannot mint to zero address");
  ...
  */
  function _burn(address account, uint256 amount) internal {
    require(account != address(0), "Flux: cannot burn from zero address");
    require(_balances[account] >= amount, "Flux: Cannot burn more than the account owns");
  ...
  *  - sender balance most be = or bigger than amount
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    require(sender != address(0), "Flux: transfer from zero address");
    require(recipient != address(0), "Flux: transfer to zero address");
    require(_balances[sender] >= amount, "Flux: cant transfer more than your account holds");
   ...
   *   - owner and spender cannot be zero address
    */
    function _approve(address owner, address spender, uint256 amount) internal {
      require(owner != address(0), "Flux: approve cannot be done from zero address");
      require(spender != address(0), "Flux: approve cannot be to zero address");
    ...
    *   - The caller must have a allowance = or bigger than the amount spending
    ...
    function transferFrom(address spender, address recipient, uint256 amount) external returns(bool){
      ...
      require(_allowances[spender][msg.sender] >= amount, "Flux: You cannot spend that much on this account");
      // Transfer first
  ...
  * Decrease the allowance on the account inputted from the caller address
   */
    ...
    function set_rewardPerHour(uint256 _newrewardPerHour) public onlyOwner {
    rewardPerHour = _newrewardPerHour;
  }

 
  function get_rewardPerHour() external view returns (uint256) {
    ...
    *
    ...
    function stake(uint256 _amount) public {
      ...
      require(_amount < _balances[msg.sender], "Flux: Cannot stake more than you own");
