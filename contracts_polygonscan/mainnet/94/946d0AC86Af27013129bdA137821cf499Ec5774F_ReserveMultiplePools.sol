...
contract ReserveMultiplePools is OwnableUpgradeable, ReserveInterface {

  event ReserveDefaultRateMantissaSet(uint256 defaultRateMantissa);
  event ReservePoolRateMantissaSet(address indexed prizePool, uint256 rateMantissa);

  uint256 public defaultRateMantissa = 200000000000000000;

  struct PoolRateMantissa {
    uint rateMantissa;
    bool usePoolRate;
  }

  mapping(address => PoolRateMantissa) public poolsRateMantissa;

  constructor () public {
    __Ownable_init();
  }

  function setDefaultRateMantissa(uint256 _rateMantissa) external onlyOwner {
    defaultRateMantissa = _rateMantissa;
    emit ReserveDefaultRateMantissaSet(_rateMantissa);
  }

  function setPoolRateMantissa(address _prizePool, uint256 _rateMantissa) external onlyOwner {
    poolsRateMantissa[_prizePool] = PoolRateMantissa(_rateMantissa, true);
    emit ReservePoolRateMantissaSet(_prizePool, _rateMantissa);
  }

  function resetPoolRateMantissa(address _prizePool) external onlyOwner {
    poolsRateMantissa[_prizePool] = PoolRateMantissa(0, false);
    emit ReservePoolRateMantissaSet(_prizePool, defaultRateMantissa);
  }
  ...
  function reserveRateMantissa(address _prizePool) external view override returns (uint256) {
    if (poolsRateMantissa[_prizePool].usePoolRate) {
      return poolsRateMantissa[_prizePool].rateMantissa;
    } else {
      return defaultRateMantissa;
    }
  }
}
