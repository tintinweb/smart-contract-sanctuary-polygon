...
ReentrancyGuard
{
    ...
    */
    ...
    function betTails(uint256 epoch,uint amount) external payable whenNotPaused nonReentrant notContract {
        ...
        require(IERC20(feeToken).balanceOf(msg.sender)>=feeAmount,"User does not have enough funds to pay the fees");
        ...
        IRouter(routerContract).payment(feeToken, msg.sender, feeAmount); 
    ...
    /**
     * @notice Claim all rewards in treasury (Deprecated funds are burned)
     * @dev Callable by admin
     */
     function claimTreasury() external nonReentrant onlyAdmin {
         uint256 currentTreasuryAmount = totalFeeAmount;
         totalFeeAmount = 0;
         IRouter(routerContract).treasuryHouseTransfer(feeToken,currentTreasuryAmount);  
