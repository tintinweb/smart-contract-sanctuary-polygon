...
contract DropmintsRelayer is Ownable, Pausable, EIP712 {
    using ECDSA for bytes32;

    struct ForwardRequest {
        address from;       // Externally-owned account (EOA) making the request.
        address to;         // Destination address (FreshMint.sol || DropmintsMarket.sol)
    ...
    function execute(ForwardRequest calldata req, bytes calldata signature) public payable whenNotPaused() returns (bool, bytes memory) {
        require(_senderWhitelist[msg.sender], "DropmintRelayer: sender of meta-transaction is not whitelisted");
        require(verify(req, signature), "DropmintRelayer: signature does not match request");
         ...
         * ## This should never happen, as the EOA should continue to be funded ##
         */
    ...
    function addSenderToWhitelist(address sender) public onlyOwner() {
        require(!isWhitelisted(sender), "DropmintRelayer: sender address is already whitelisted"); // This requirement prevents registry duplication.
    ...
    function killDropmintRelayer(address payable payoutAddress) public onlyOwner() {
