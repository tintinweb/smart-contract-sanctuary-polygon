...
import "./interfaces/external/IUniswapRouterETH.sol";
import "./interfaces/external/IPegSwap.sol";
import "./interfaces/external/IKeeperRegistry.sol";
...
contract BeefyHarvester is ManageableUpgradeable, PausableUpgradeable, IBeefyHarvester {
        ...
        )
    {
        ...
        (bool isAnyVaultHarvestable, address vault, uint256 newStartIndex, uint256 estimatedTxCost, uint256 estimatedCallRewards) = _findVaultToHarvest(vaults);
    ...
    function _findVaultToHarvest(address[] memory vaults_) internal view returns (bool isAnyVaultHarvestable_, address vault_, uint256 newStartIndex_, uint256 estimatedTxCost_, uint256 estimatedCallRewards_) {
    ...
    function performUpkeep(bytes calldata performData_) external override whenNotPaused {
        (
            address vault,
            uint256 newStartIndex,
            uint256 estimatedTxCost,
            uint256 estimatedCallRewards
        ) = abi.decode(performData_, (address, uint256, uint256, uint256));
    ...
    function _runUpkeep(
        ...
        uint256 calculatedTxCost = _calculateTxCostWithOverheadWithPremium(gasUsedByPerformUpkeep);
        uint256 calculatedProfit = UpkeepLibrary._calculateProfit(calculatedCallRewards, calculatedTxCost);

        _reportHarvestSummary(oldStartIndex, newStartIndex_, gasUsedByPerformUpkeep, estimatedTxCost_, estimatedCallRewards_, estimatedProfit, calculatedTxCost, calculatedCallRewards, calculatedProfit);
    ...
    function _reportHarvestSummary(
        ...
        uint256 calculatedTxCost_,
        uint256 calculatedCallRewards_,
        uint256 calculatedProfit_) internal {
        emit HarvestSummary(
            block.number,
            // state variables
            oldStartIndex_,
    ...
    function _harvestVault(address vault_) internal returns (bool didHarvest_, uint256 callRewards_) {
        ...
        address callFeeRecipient = address(upkeepRefunder);
    ...
    function setStartIndex(uint256 startIndex_) external onlyManager {
        address[] memory vaults = vaultRegistry.allVaultAddresses();
        startIndex = UpkeepLibrary._getCircularIndex(startIndex_, 0, vaults.length);
    }
    ...
    function _calculateTxCostWithPremium(uint256 gasOverhead_) internal view returns (uint256 txCost_) {
        return UpkeepLibrary._calculateUpkeepTxCost(tx.gasprice, gasOverhead_, chainlinkUpkeepTxPremiumFactor);
    }
    ...
    {
        totalGasOverhead_ =
            vaultHarvestFunctionGasOverhead_ + KEEPER_REGISTRY_GAS_OVERHEAD;
    }
