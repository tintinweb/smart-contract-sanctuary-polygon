...
interface IRaiderRandomness {
  function getBigRandomNum(uint _seed) external view returns (uint);
}
...
contract Recruiting is Ownable, Pausable, ReentrancyGuard {
    ...
    enum Race {
      Mike, John, Tammy, Old, White, Blue, Orc, Skeleton, Cyborg, Dark, Fairy, Tigress
    }
    ...
    Race[] public uncommonChars = 
      ...
      Race.White, Race.Blue, Race.Tigress]; 
    ...
    constructor(
      ...
      raceRarity[Race.Tigress] = Rarity.Uncommon;
      ...
      rarityThresholds[Rarity.Common] = Thresholds(100, 25, 93, 98, 100); 
      rarityThresholds[Rarity.Uncommon] = Thresholds(100, 25, 40, 95, 100);
      rarityThresholds[Rarity.Rare] = Thresholds(100, 25, 30, 45, 100);
    }
    ...
    function nextUncommon() internal returns (Race) {
      Race race = uncommonChars[rarityCount[Rarity.Uncommon].current().mod(6)];
    ...
    function nextRecruitTime(uint _raiderId) public view returns(uint) {
      ...
      if (lastRecruited > 0) {
        uint ourNextRecruitTime = lastRecruited.add(recruitingFrequency);
        return ourNextRecruitTime.sub(block.timestamp);
      } else {
        uint ourNextRecruitTime = history().whenRaiderWasRecruited(_raiderId).add(recruitingFrequency);
        return ourNextRecruitTime.sub(block.timestamp);
      }
    }
    ...
    function getBigRandomNum(uint _seed) public view returns(uint) {
      return IRaiderRandomness(randomnessAddress).getBigRandomNum(_seed);
    }
    ...
    function currentRaceCounts() public view returns(uint[12] memory) {
      uint[12] memory raceCounts;
      ...
      raceCount[Race.Tigress].current()];
      return raceCounts;
    }
...
contract Raiders is ERC721Enumerable, Ownable, Pausable {
    string _baseTokenURI;

    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override(ERC721Enumerable) whenNotPaused {
        super._beforeTokenTransfer(from, to, tokenId);
    }

    constructor(string memory baseURI) ERC721("CryptoRaiders", "RAIDER") {
        setBaseURI(baseURI);
    }

    function mintRaiders(address _to, uint256 _count) public onlyOwner whenNotPaused {
        for(uint i = 0; i < _count; i++) {
            _safeMint(_to, totalSupply());
        }
    }

    function burnRaider(uint256 tokenId) public onlyOwner {
        _burn(tokenId);
    }
    ...
    function raidersOwnedBy(address _owner) external view returns (uint256[] memory) {
        uint256 raiderCount = balanceOf(_owner);

        uint256[] memory tokensId = new uint256[](raiderCount);
        for (uint256 i = 0; i < raiderCount; i++) {
