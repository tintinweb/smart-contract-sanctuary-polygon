...
// File contracts/GravisStaking.sol
...
contract GravisStaking is Ownable {
    ...
    IMintableToken public matrix;

    uint256 public matrixPerGrvxPerSecond;
    ...
    constructor(
        IERC20 grvx_,
        IMintableToken matrix_,
        uint256 rewardStart_,
        uint256 matrixPerGrvxPerYear_
    ) {
        if (rewardStart_ != 0) {
        ...
        matrixPerGrvxPerSecond = matrixPerGrvxPerYear_ / (365 days);
    }
    ...
    function claimReward() external {
        ...
        matrix.mint(msg.sender, reward);
    }

    // RESTRICTED FUNCTIONS

    function setMatrixPerGrvxPerYear(uint256 matrixPerGrvxPerYear_)
        external
        onlyOwner
    {
        _distributeRewards();
