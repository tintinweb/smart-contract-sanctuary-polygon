...
contract MasterChefNFT is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    function _payPendingUSDReward() internal {
        ...
        emit LogValue("usdPending", usdPending);

        if (usdPending > 0) {
            emit Log("4");
            treasuryDAO.transferUSDToOwner(msg.sender, usdPending);
            user.usdRewardLockup = 0;
            emit LogValue("totalUsdLockup first", totalUsdLockup);
            if (totalUsdLockup > usdPending) {
                totalUsdLockup = usdPending;
            } else {
                totalUsdLockup = totalUsdLockup - usdPending;
                emit LogValue("totalUsdLockup after", totalUsdLockup);
            }
