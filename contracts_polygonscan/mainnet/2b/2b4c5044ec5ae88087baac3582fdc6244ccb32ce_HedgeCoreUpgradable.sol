 ...
 */
contract HedgeCoreUpgradable is IHedgeCore, Initializable, UUPSUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, HedgeCoreStorage {
	...
	function initGnesisHedge(
		...
		// init intial status
		// 1 price fetch
		// 1.1 set price impact
		priceImpact = 10000; // default is 10%
		// 1.2 fetch inital prices for vsq and hegeTarget token
	 ...
	 */
	function withdrawAllWhenPaused() public override nonReentrant onlyInitialized whenPaused {
	 ...
	 *	@param newImpact_ no upper limit
	 */
	function updatePriceImpact(uint256 newImpact_) public onlyDAO {
		priceImpact = newImpact_;
	}
	...
	function _startNewHedge() private {
		...
		bool isLong;
		uint256 lastPrice = (hedgeInfo.hedgeTokenPrice * PRECISION) / hedgeInfo.hedgeTargetPrice;
		// multiply price by price impact
		lastPrice += (lastPrice * priceImpact) / RATIO_PRECISION;
		if (((currHedgeTokenPrice * PRECISION) / curHedgeTargetPrice) >= lastPrice) {
			isLong = true;
		}
	...
	/**
	 * @dev log store only. do not touch any core logic
	 ...
	 * @dev for user view only. might not be accurate if user transfer their hge/lev tokens
	...
	function priceAfterImpact() external view returns (uint256) {
		uint256 price = (hedgeInfo.hedgeTokenPrice * PRECISION) / hedgeInfo.hedgeTargetPrice;
		// multiply price by price impact
		price += (price * priceImpact) / RATIO_PRECISION;
...
contract HedgeCoreStorage {
	...
	// game related
	uint256 public priceImpact; // 1e5 pricision  (no upper limit) lev winning pric will be price*(1 + impact)
	uint256 public constant RESTRICTED_PERIOD = 14400; // period of time when user can/cannot deposit withdraw and swap
