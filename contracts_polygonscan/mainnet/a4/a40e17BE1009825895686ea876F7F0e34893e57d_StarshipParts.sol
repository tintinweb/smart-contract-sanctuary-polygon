...
contract StarshipParts is IStarshipParts, ERC721EnumerableUpgradeable, WarpBase {
    ...
    /** @notice set starship parts control */
    ...
    function mint(address _to, uint256 _tokenId) external override {
        ...
        require(msg.sender == starshipPartsControl, 'Not control');
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return 'ipfs://QmPX1bq8xbbWoBuVPBYBG5rd861wSD5ViF7KHG8EN8UJ2j/';
    }
    ...
    function _beforeTokenTransfer(
        ...
        IMarketPlace(marketPlace).delistPart(tokenId);
    }
    ...
    function tokenURI(uint256 tokenId)
        ...
        view
        virtual
        override(ERC721Upgradeable, IStarshipParts)
        returns (string memory)
    {
        //** Token burned? */
        uint256 counter = IStarshipPartsControl(starshipPartsControl).getCounter();
    ...
    function getUri(uint256 tokenId, bool burned) internal view returns (string memory) {
        ...
        PartInfo memory part = IStarshipPartsControl(starshipPartsControl).getPartInfo(tokenId);
        return
            string(
                abi.encodePacked(
                    base64Uri,
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                burned ? '{ "name": "Expended ' : '{ "name": "',
                                IStarshipPartsControl(starshipPartsControl).getStringPartType(
                                    part.typeOf
                                ),
                                ' #',
                                tokenId.toString(),
    ...
    /** Get attributes */
    function getAttributes(PartInfo memory partInfo) internal view returns (string memory) {
        return
            string(
                bytes(
                    abi.encodePacked(
                        '"attributes": [{ "trait_type": "Strength", "value": "',
                        partInfo.strength.toString(),
                        '", "display_type": "number"},',
