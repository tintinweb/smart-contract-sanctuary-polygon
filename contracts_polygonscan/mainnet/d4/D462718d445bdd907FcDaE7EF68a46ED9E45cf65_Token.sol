...
contract Token is TokenERC20, Control, ReentrancyGuard {
	 ...
	 */
	function rebase() external initialized isBackendRole {
		...
		)
	{
		...
		if (currentPrice > lastRebasePrice * 99 / 100 ) {
			return (0, false);
		} else {
			uint256 supplyDelta = (lastRebasePrice * _totalSupply) / currentPrice;
			return (supplyDelta, true);
		}
...
library BokkyPooBahsDateTimeLibrary {
	uint256 constant SECONDS_PER_DAY = 15 * 60;//24 * 60 * 60;
