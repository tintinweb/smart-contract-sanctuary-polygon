...
contract HamperBlockManagerGame {
    ...
    function getTokensTarjeta(uint id) private view returns(int){
        ...
        if(id==2 || id==17 || id==33){
            ...
            else if(tirada>30){tokens=-2*tokens;}
            else{tokens=-3*tokens;}
        }
        if(id==4 || id==38 || id==28 || id==12){tokens=int(100+tirada)*tokens/100;}
        if(id==7 || id==22 || id==36){
            if(tirada>95){tokens=-11*tokens;}
            else if(tirada>60){tokens=-5*tokens;}
            else{tokens=-1*tokens;}
        }
        return tokens;
    }
