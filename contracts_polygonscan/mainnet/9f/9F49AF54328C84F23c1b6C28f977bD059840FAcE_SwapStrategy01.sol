...
import "./../LUniswap/interfaces/IUniswapV2Router02.sol";
...
contract SwapStrategy01 is ISwapStrategy01 {    
    ...
    function  addCoin(string memory _name, address _token) public {
        coin[_name] = _token;
    }
    ...
    function swapByIndex03( uint256 _amountIn, address _router, address[] memory path ) public override returns(uint256 amountOut) {
        ...
        amountOut = ToToken.balanceOf(address(this));
    ...
    function swapByIndex( uint256 _amountIn, address _router, address[] memory path ) public returns(uint256 amountOut) {
        IERC20 SToken = IERC20(path[0]);
        IUniswapV2Router02 router = IUniswapV2Router02(_router);    
        if (SToken.allowance(address(this), address(router)) < _amountIn) {
            require(SToken.approve( address(router) , type(uint256).max), "approve error");
    ...
    function swapTo( string memory Coin, uint256 _amountIn ) public override returns(uint256 amountOut) {
        ...
        path[2] = coin[Coin]; // DAI

        address router = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506;
        ( amountOut ) = swapByIndex03( _amountIn, router, path );
        
        /*
        if (Coin.toSlice().equals("WBTC".toSlice())) {
            ( amountOut ) = swapToWBTC( _amountIn );
        }
        if (Coin.toSlice().equals("DAI".toSlice())) {
            ( amountOut ) = swapToDAI( _amountIn );
        }
        if (Coin.toSlice().equals("USDC".toSlice())) {
            address[] memory pathUSDC = new address[](3);
            pathUSDC[0] = coin["WMATIC"];
            pathUSDC[1] = coin["WETH"];
            pathUSDC[2] = coin["USDC"];
            ( amountOut ) = swapByIndex03( _amountIn, 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506, pathUSDC );
        } 
        
        if (Coin.toSlice().equals("WETH".toSlice())) {
            ( amountOut ) = swapToWETH( _amountIn );
        }
        */
    }

    // swapETHForExactTokens
    function swapToWBTC( uint256 _amountIn ) public returns(uint256 amountOut) {
        // WMATIC -> WETH -> WBTC
        address[] memory path = new address[](3);
        path[0] = coin["WMATIC"];
        path[1] = coin["WETH"];
    ...
    function swapToDAI( uint256 _amountIn ) public returns(uint256 amountOut) {
        // WMATIC -> WETH -> DAI
        address[] memory path = new address[](3);
        path[0] = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; // WMATIC
        path[1] = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619; // WETH
        path[2] = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063; // DAI

        address router = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506;
        ( amountOut ) = swapByIndex03( _amountIn, router, path );
    }

    function swapToUSDC( uint256 _amountIn ) public returns(uint256 amountOut) {
        // WMATIC -> WETH -> USDC
        address[] memory path = new address[](3);
        path[0] = coin["WMATIC"];
        path[1] = coin["WETH"];
    ...
    function swapToWETH( uint256 _amountIn ) public returns(uint256 amountOut) {
        // WMATIC -> WETH -> USDC
