...
contract MadApeStaking is Ownable {
    ...
    mapping(uint256 => bool) private _isTokenStakedMapping;
    mapping(address => mapping(uint256 => uint256)) private _ownerStakedTokenTimeSinceLastRewardMapping;

    bool public _isActive;
    uint256 _deactivationTime;
    uint256 _baseReward;
    ...
    */
    event OwnerHasChanged(address oldOwner, address newOwner, uint256 tokenId, uint256 timestamp);
    ...
    */
    ...
    constructor(address _rewardTokenAddress, address _stakeTokenAddress, address _rarityCheckerAddress){
        _changeRewardToken(_rewardTokenAddress);
        _changeStakeToken(_stakeTokenAddress);
        _changeRarityChecker(_rarityCheckerAddress);
        _changeBaseReward(1 ether);
    ...
    function stake(uint256 tokenId) public returns (bool){
        require(_isActive, "MadApeStaking: Staking is currently not active");
    ...
    function unstake(uint256 tokenId) public returns (bool){
        require(_stakeToken.ownerOf(tokenId) == _msgSender(), "MadApeStaking: Message sender not currently the owner of this token");
    ...
    function ownerHasChanged(address oldOwner, address newOwner, uint256 tokenId) public onlyOperator {
        _unstake(oldOwner, tokenId);
        emit OwnerHasChanged(oldOwner, newOwner, tokenId, block.timestamp);
    }
    ...
    function _calculateReward(address owner, uint256 tokenId) internal view returns (uint256 rewardTokens){
        ...
        uint256 daysStaking = SafeMath.div(timeStakingSinceLastRewardInSeconds, 86400);
        ...
        uint256 baseReward = _getBaseReward();
     ...
     */
    function _changeRarityChecker(address newRarityChecker) public onlyOwner {
        emit RarityCheckerChanged(_getRarityChecker(), newRarityChecker);
        _setRarityChecker(newRarityChecker);
    }

    /**
    * @dev Stores a new address for rarity checker.
    */
    function _setRarityChecker(address newRarityChecker) private {
    ...
    */
    function _getRarityChecker() public view virtual returns (address) {
     ...
     */
    function _changeBaseReward(uint256 newBaseReward) public onlyOwner {
        emit BaseRewardChanged(_getBaseReward(), newBaseReward);
        _setBaseReward(newBaseReward);
    }

    /**
    * @dev Stores a new value for base reward.
    */
    function _setBaseReward(uint256 newBaseReward) private {
    ...
    */
    function _getBaseReward() public view virtual returns (uint256) {
        return _baseReward;
    }
     ...
     */
    function _changeRewardToken(address newRewardToken) public onlyOwner {
        emit RewardTokenChanged(_getRewardToken(), newRewardToken);
    ...
    */
    function _setRewardToken(address newRewardToken) private {
    ...
    */
    function _getRewardToken() public view virtual returns (address) {
     ...
     */
    function _changeStakeToken(address newStakeToken) public onlyOwner {
        emit StakeTokenChanged(_getStakeToken(), newStakeToken);
        _setStakeToken(newStakeToken);
    }

    /**
    * @dev Stores a new address for stake token.
    */
    function _setStakeToken(address newStakeToken) private {
    ...
    */
    function _getStakeToken() public view returns (address) {
     ...
     */
    function _changeIsActive(bool isActive) public onlyOwner {
        emit IsActiveChanged(_getIsActive(), isActive);
    ...
    * @dev Sets isActive.
    */
    function _setIsActive(bool isActive) private {
        _isActive = isActive;
        if (!isActive) {
    ...
    */
    function _getIsActive() public view returns (bool) {
        return _isActive;
    }
