...
contract AngelTokenCrowdsale is Pausable, Crowdsale, Ownable {
    ...
    uint256 public _teamClaimAmount; //token available for the team --update
    uint256 public _tokenClaimedByTeam; //tokens claimed by team --update
    ...
    //fallback function for receiving eth  --tested
    ...
    //allow admin to manually send tokens to someone  --tested
        ...
        onlyOwner
    {
        ...
        //calculate 5% to send to the beneficiary
        uint256 tokensToSend = tokenAmount.mul(5).div(100);
        ...
        //update user info
        claimInfo[msg.sender].amountClaimed += tokensToSend;
    }

    //calculate the tokens with the discount   --tested
    ...
    function buyTokens(address beneficiary) public payable nonReentrant {
        ...
        claimInfo[beneficiary].amountBought += tokens;
        claimInfo[beneficiary].buyingDate = block.timestamp;
        claimInfo[beneficiary].amountClaimed += tokensToSend;
    }

    //get the number of discounts
    ...
    function get_vestingStartTime() public view returns (uint256) {
    ...
    function tokensToClaim(address beneficiary) public view returns (uint256) {
        return
            claimInfo[beneficiary].amountBought.sub(
                claimInfo[beneficiary].amountClaimed
            );
    }

    //tokens left for the team to claim
    function teamTokensToClaim() public view returns (uint256) {
        return _teamClaimAmount.sub(_tokenClaimedByTeam);
    }
    ...
    //delete a discount
    function delete_discount(uint256 index) public onlyOwner {
        require(discountRates.length > 0, "There are no discounts!");
        discountRates[index] = discountRates[discountRates.length - 1];
        discountRates.pop();
    }
    ...
    //allow admin to withdraw all ETH in the contract  --tested
    function withdraw() public onlyOwner {
        address payable to = payable(msg.sender);
        to.transfer(address(this).balance);
    }

    //allow admin to send tokens to the contract   --tested
    ...
    //allow admin to send tokens fot the team withdraw
    function addTeamSupply() public onlyOwner {
        require(_teamClaimAmount == 0, "Alredy sent tokens to the contract.");
        _teamClaimAmount = 650000000 * (10**18);
        _token.transferFrom(msg.sender, address(this), _teamClaimAmount);
    }
    ...
    //returns how many tokens a user can claim
        ...
        returns (uint256)
    {
        ...
        if (currentTime < _vestingStartTime) {
            return 0;
        }
        ...
        uint256 amountOfTokensWithdrawable = (
            claimInfo[beneficiary].amountBought.mul(monthsPassed.mul(7916))
        ).div(100000);
    ...
    //let a user take his claimable tokens
    function claim() public {
        ...
        uint256 amountOfTokensClaimable = getClaimableTokens(msg.sender);

        //update buyer info
        claimInfo[msg.sender].amountClaimed += amountOfTokensClaimable;

        //transfer tokens to users
        _token.transfer(msg.sender, amountOfTokensClaimable);
    }

    //get the amount of tokens the team can claim
    function getTeamClaimableTokens() public view returns (uint256) {
        require(_teamClaimAmount > 0, "No tokens for the team in contract");
        uint256 currentTime = block.timestamp;
        ...
        monthsPassed = monthsPassed.div(6);
        if (monthsPassed > 10) {
            monthsPassed = 10;
        }
        ...
        //calculate how many tokens the buyer should receive
        uint256 amountOfTokensWithdrawable = (
            _teamClaimAmount.mul(monthsPassed)
        ).div(10);

        //how many token he can withdraw now
        uint256 amountOfTokensClaimable = amountOfTokensWithdrawable.sub(
            _tokenClaimedByTeam
        );
    ...
    //let the team get their claimable tokens
    function teamClaim() public onlyOwner {
        require(_teamClaimAmount > 0, "No tokens for the team in contract");
        require(teamTokensToClaim() > 0, "Nothing more to claim!");
        uint256 currentTime = block.timestamp;
        require(
            currentTime >= _vestingStartTime,
            "Vesting has not started yet"
        ...
        //get claimable tokens
        uint256 amountOfTokensClaimableForTeam = getTeamClaimableTokens();

        //update buyer info
        _tokenClaimedByTeam += amountOfTokensClaimableForTeam;

        //transfer tokens to team
        _token.transfer(msg.sender, amountOfTokensClaimableForTeam);
    }
}
