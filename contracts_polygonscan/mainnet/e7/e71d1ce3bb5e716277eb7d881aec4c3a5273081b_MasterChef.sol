...
contract MasterChef is Ownable {
    ...
    function pendingSushi(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 appleReward = (multiplier * applePerBlock * pool.allocPoint) / totalAllocPoint;
            accSushiPerShare = accSushiPerShare + (appleReward * 1e12) / lpSupply;
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accSushiPerShare = pool.accSushiPerShare + (appleReward * 1e12) / lpSupply;
        pool.lastRewardBlock = block.number;
    }
