...
contract BondToken is IBondToken, ERC721EnumerableUpgradeable, WarpBase {
    ...
    //* ======== Variables ======== */
    uint256 counter;
    mapping(address => bool) internal bonds;
    mapping(uint256 => BondInfo) internal token;
    address starshipPartsControl;

    //* ======== Modifiers ======== */
    modifier onlyBond() {
        require(bonds[msg.sender], 'BondToken: Oops not a bond');
        _;
    }
     ...
     *  @param to address
     */
    function safeMint(address to, uint256 paid)
        external
        override
        whenNotPaused
        onlyBond
        returns (uint256)
    {
        ...
        token[tokenId] = BondInfo({bond: msg.sender, repaid: false, value: paid});

        //** mint user starship part */
        if (paid >= IStarshipPartsControl(starshipPartsControl).getMinimumValue()) {
            IStarshipPartsControl(starshipPartsControl).buildPart(to, paid);
        }
    ...
    /**
     *  @notice once bond is fully vested repaid is set to true
     *  @param tokenId uint
     */
    function bondRepaid(uint256 tokenId) external override whenNotPaused onlyBond {
        _burn(tokenId);
        token[tokenId].repaid = true;
        emit PermanentURI(getUri(tokenId, true), tokenId);
    }

    /** @notice add an allowed bond contract to bond token */
    function setBond(address _bond, bool _set) external onlyOwner {
        bonds[_bond] = _set;
    }
    ...
    function getBondInfo(uint256 _tokenId) external view override returns (BondInfo memory) {
        return token[_tokenId];
    }
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return 'ipfs://QmbbZCEP9WVmE4kpeJCUw3X3xn5CjgXPJNBh4TyvWQiHo5/bond.png';
    }
     ...
     * by default, can be overriden in child contracts.
     */
    function repaidURI() internal view virtual returns (string memory) {
        return 'ipfs://QmbbZCEP9WVmE4kpeJCUw3X3xn5CjgXPJNBh4TyvWQiHo5/bond_redeemed.png';
    }
    ...
    function tokenURI(uint256 tokenId)
        ...
        view
        virtual
        override(ERC721Upgradeable, IBondToken)
        returns (string memory)
    {
        /** Token burned? */
        if (_exists(tokenId) == false && tokenId < counter) {
            return getUri(tokenId, true);
        }
        ...
        /** Get uri */
        return getUri(tokenId, false);
    }

    function getUri(uint256 tokenId, bool burned) internal view returns (string memory) {
        BondInfo memory tokenInfo = token[tokenId];
        // uint256 currentValue = IBond(tokenInfo.bond).getBond(tokenId).payout;
        string memory base64Uri = 'data:application/json;base64,';
        return
            string(
                abi.encodePacked(
                    base64Uri,
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                burned ? '{ "name": "Redeemed Bond #' : '{ "name": "Bond #',
                                tokenId.toString(),
                                '", "image": "',
                                burned ? repaidURI() : _baseURI(),
                                '", ',
                                getAttributes(tokenInfo),
                                '}'
                            )
                        )
                    )
    ...
    /** Get attributes */
    function getAttributes(BondInfo memory tokenInfo) internal pure returns (string memory) {
        return
            string(
                bytes(
                    abi.encodePacked(
                        '"attributes": [{"trait_type": "USD Paid", "value": "',
                        (tokenInfo.value / 1e9).toString(),
                        '"},',
                        '{ "trait_type": "Repaid", "value": "',
                        tokenInfo.repaid ? 'Yes' : 'No',
                        '"}]'
                    )
                )
            );
