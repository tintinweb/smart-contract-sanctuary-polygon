...
// File: contracts/gnome-it-all.sol



// Amended by HashLips, krahS457
...
contract GnomeItAllNFT is ERC721Royalty, Ownable, RandomlyAssigned {
  using Strings for uint256;
  using Counters for Counters.Counter;
  ...
  constructor() 
    ERC721("Gnome It All", "GIA")
    RandomlyAssigned(maxSupply, 1)
  {
    setHiddenMetadataUri("ipfs://QmYFq8dWhrUrRzudNShtRm8RDGgfSuF5yfGUJeggrMcG51/hidden.json");
    ...
    returns (uint256[] memory)
  {
    ...
    uint256 numberOfMintedTokens = tokenCount();
    uint256 currentIndex = 0;
    uint256 ownedTokenIndex = 0;
    uint256[] memory ownedTokenIds = new uint256[](ownerTokenCount);

    while (ownedTokenIndex < ownerTokenCount && currentIndex < numberOfMintedTokens) {
        uint256 currentTokenId = mintedTokenIds[currentIndex];
        if (_owner == ownerOf(currentTokenId)) {
