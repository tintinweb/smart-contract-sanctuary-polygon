...
// Note that this pool has no minter key of GRINGO (rewards).
// Instead, the governance will call GRINGO distributeReward method and send reward to this pool at the beginning.
contract GRINGORewardPool {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. GRINGO to distribute per block.
        uint256 lastRewardTime; // Last time that GRINGOs distribution occurs.
        uint256 accTSharePerShare; // Accumulated GRINGOs per share, times 1e18. See below.
    ...
    // The time when GRINGO mining starts.
    uint256 public poolStartTime;

    // The time when GRINGO mining ends.
    uint256 public poolEndTime;

    uint256 public tSharePerSecond = 0.00186122 ether; // 59500 GRINGO / (370 days * 24h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "GringoRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "GringoRewardPool: existing pool?");
        }
    }
    ...
    // Update the given pool's Gringo allocation point. Can only be called by the owner.
    ...
    // View function to see pending GRINGOs on frontend.
    ...
    // Safe tshare transfer function, just in case if rounding error causes pool to not have enough GRINGOs.
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (GRINGO or lps) if less than 90 days after pool ends
