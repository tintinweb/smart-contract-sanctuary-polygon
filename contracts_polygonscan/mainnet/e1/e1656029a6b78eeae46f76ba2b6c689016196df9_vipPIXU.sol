...
abstract contract ERC20Token is IERC20, Context {
...
abstract contract Divine is ERC20Token, AccessControl {
   constructor(
       string memory name_,
       string memory symbol_,
       uint8 decimals_
   ) public ERC20Token(name_, symbol_, decimals_) {}
}

contract vipPIXU is Divine {
   using SafeMath for uint256;

   bytes32 public constant ADMIN_ROLE = keccak256("");
   bytes32 public constant VIP_PIXU_OWNER_ROLE = keccak256("VIP_PIXU_OWNER_ROLE");
   bytes32 public constant VIP_PIXU_MINTER_ROLE = keccak256("VIP_PIXU_MINTER_ROLE");
   ...
   modifier hasOwnerRole(address account) {
       require(hasRole(VIP_PIXU_OWNER_ROLE, account), "!account_owner_role");
       _;
   }

   modifier hasMinterRole(address account) {
       require(hasRole(VIP_PIXU_MINTER_ROLE, account), "!account_minter_role");
       _;
   }

   constructor(address initialMinter, address initialOwner, uint256 initialSupply) public Divine("Vip PIXU", "vipPIXU", 18) {
       ...
       _setRoleAdmin(VIP_PIXU_MINTER_ROLE, ADMIN_ROLE);
       _setRoleAdmin(VIP_PIXU_OWNER_ROLE, ADMIN_ROLE);
       _setupRole(VIP_PIXU_MINTER_ROLE, initialMinter);
       _setupRole(VIP_PIXU_OWNER_ROLE, initialOwner);
   }

   function allowOpenTrading() external hasOwnerRole(msg.sender) returns (bool) {
       requireSellerApproval = false;
       return requireSellerApproval;
   }

   function disableMinting() external hasOwnerRole(msg.sender) returns (bool) {
       allowMinting = false;
       return allowMinting;
   }

   function enableMinting() external hasOwnerRole(msg.sender) returns (bool) {
       allowMinting = true;
       return allowMinting;
   }
   ...
   function _beforeTokenTransfer(
       ...
       require(
           (balanceOf(to_) > 0 || isApprovedSeller[from_] == true),
           "!account_approved_transfer_tokens"
       );
   }

   function mint(address recipient_, uint256 amount_) public hasMinterRole(msg.sender) {
       require(allowMinting, "minting_disabled");
