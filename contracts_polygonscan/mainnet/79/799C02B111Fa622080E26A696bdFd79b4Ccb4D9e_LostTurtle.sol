...
contract LostTurtle is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string baseURI;
  string public baseExtension = ".json";
  uint256 public cost = 33 ether;
  uint256 public preSaleCost = 33 ether;
  uint256 public MainSaleCostt = 33 ether;
  uint256 public maxSupply = 540;
  uint256 public maxMintAmount = 30;
  uint256 public total = 5400;

  uint256 public revealmin = 60;
  uint256 public revealmax = 540;
  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        ...
        require(supply + _mintAmount <= total);
        uint256 ownerTokenCount = balanceOf(msg.sender);
        require(ownerTokenCount + _mintAmount <= maxMintAmount);
    ...
    returns (string memory)
  {
    ...
    if(revealed == false) {
        if(tokenId <= revealmin)
        {
            return notRevealedUri;
        }
        if(tokenId>revealmax)
        {
            return notRevealedUri;
        }
    }
  ...
  function setRevealmin(uint256 _revealmin) public onlyOwner() {
    revealmin = _revealmin;
  }
  function setRevealmax(uint256 _revealmax) public onlyOwner() {
    revealmax = _revealmax;
  }
