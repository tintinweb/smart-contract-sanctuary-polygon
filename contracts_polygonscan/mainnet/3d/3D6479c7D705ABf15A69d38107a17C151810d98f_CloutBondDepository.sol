...
// File: interfaces/ICloutBondingCalculator.sol


pragma solidity 0.7.5;

interface ICloutBondingCalculator {
...
// File: interfaces/ICloutTreasury.sol


pragma solidity 0.7.5;

interface ICloutTreasury {
...
contract CloutBondDepository is Ownable {
    ...
    function deposit(
        ...
        uint256 value = ICloutTreasury(treasury).valueOfToken(
            principle,
            _amount
        );
        ...
        ICloutTreasury(treasury).deposit(_amount, principle, profit);
    ...
    function bondPriceInUSD() public view returns (uint256 price_) {
        if (isLiquidityBond) {
            price_ = bondPrice()
                .mul(
                    ICloutBondingCalculator(bondCalculator).markdown(principle)
                )
                .div(100);
        } else {
    ...
    function standardizedDebtRatio() external view returns (uint256) {
        if (isLiquidityBond) {
            return
                debtRatio()
                    .mul(
                        ICloutBondingCalculator(bondCalculator).markdown(
                            principle
                        )
                    )
                    .div(1e9);
