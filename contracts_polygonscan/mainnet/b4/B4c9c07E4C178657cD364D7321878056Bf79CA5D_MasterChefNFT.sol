...
contract MasterChefNFT is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    function _payPendingUSDReward(address userAddress) internal {
        UserInfo storage user = userInfo[theEndlessPID][userAddress];

        uint256 usdPending = ((user.amount * accDepositUSDRewardPerShare) / 1e24) - user.usdRewardDebt;

        if (usdPending > 0) {
            treasuryDAO.transferUSDToOwner(userAddress, usdPending);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (_pid == theEndlessPID)
            _payPendingUSDReward(msg.sender);
