...
contract NuchainAraToken is ERC20, HasRoot {
    ...
    // Max cap of the token.
    function cap() public view returns (uint256) {
        return 1000_000_000 * 10 ** decimals(); // MAX 1000 million ARA
    }

    constructor(address minter) ERC20("Nuchain ARA Token", "ARA") {
        _setRoot(msg.sender);
        _setMinter(minter);

        // inital coin supply 3 million
        _mint(msg.sender, 3000_000 * 10**decimals());
    }

    modifier onlyMinter() {
        require(isMinter(msg.sender), "ARAToken: caller isn't minter");
        _;
    }
    ...
    function _mint(address account, uint256 value) internal override {
        require(totalSupply().add(value) <= cap(), "ARAToken: cap exceeded");
    ...
    function changeMinter(address newMinter) external onlyRoot {
    ...
    /**
     * Mint new tokens (transfer from native token to ERC-20 token).
     */
    ...
    /**
     * Burn tokens (transfer from ERC-20 token to native token).
     */
    function transferToNative(bytes32 to, uint256 value) public returns (bool) {
