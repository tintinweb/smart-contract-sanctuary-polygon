...
// INCLUDED IS THE SOURCE CODE FOR THE WEB3 FASHION MANIFESTO PAGES ON THE MATIC NETWORK
// Not all of the text I edited correctly, but it's ok, so much to do really so I just forked this contract, one day we'll do a clean one ;)
 ...
 * @dev see https://eips.ethereum.org/EIPS/eip-721
 */
contract ManifestoPages is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    */
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = ManifestoPages.ownerOf(tokenId);
        require(to != owner, "ERC721: approval to current owner");

        require(_msgSender() == owner || ManifestoPages.isApprovedForAll(owner, _msgSender()),
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view virtual returns (bool) {
        ...
        address owner = ManifestoPages.ownerOf(tokenId);
        return (spender == owner || getApproved(tokenId) == spender || ManifestoPages.isApprovedForAll(owner, spender));
    }
    ...
    function _burn(uint256 tokenId) internal virtual {
        address owner = ManifestoPages.ownerOf(tokenId); // internal owner
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        require(ManifestoPages.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own"); // internal owner
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(ManifestoPages.ownerOf(tokenId), to, tokenId); // internal owner
    }
...
/**
 * @notice Access Controls contract for f3manifesto
 * @author f3manifesto.eth
 */
contract f3manifestoAccessControls is AccessControl {
...
contract DigitalaxMaterials is ERC1155Burnable, BaseRelayRecipient {
    ...
    f3manifestoAccessControls public accessControls;
    ...
    constructor(
        ...
        f3manifestoAccessControls _accessControls,
        address _childChain,
    ...
    function updateAccessControls(f3manifestoAccessControls _accessControls) external {
        require(
            accessControls.hasAdminRole(_msgSender()),
...
/**
 * @title f3manifesto Manifesto Pages 
 * @dev Issues ERC-721 tokens as well as being able to hold child 1155 tokens
 */
contract f3manifesto is ManifestoPages("ManifestoPages", "MFP"), ERC1155Receiver, IERC998ERC1155TopDown, BaseChildTunnel, BaseRelayRecipient, Initializable {
    ...
    /**
     @param _accessControls Address of the f3manifesto access control contract
     @param _childContract ERC1155 the Digitalax child NFT contract
     0xb5505a6d998549090530911180f38aC5130101c6
     */
    function initialize(f3manifestoAccessControls _accessControls, DigitalaxMaterials _childContract, address _childChain, address _trustedForwarder) public initializer {
        accessControls = _accessControls;
