...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.4.0/contracts/token/ERC1155/utils/ERC1155Receiver.sol
...
// File: contracts/library/AdminControl.sol
...
// File: contracts/Attention2476.sol
...
contract AttentionArtchick is AdminControl, ERC1155Receiver{
    ...
    /**    
    * Set 2476 block emergency hatch..
    ...
    */
     ...
     */
    function migrateProject(address staker, uint256 total,string memory project ) external adminRequired{
        ...
        require(keccak256(abi.encodePacked(stakesByAddress[staker])) == (keccak256(abi.encodePacked(("")))),"Attention2476 : Wallet is already promoting a project.");
        require(IERC1155(contractAddress2476).balanceOf(msg.sender,2476)>=total,"Attention2476 : Wallet has insuficient tokens.");

        stakesByAddress[staker]=project;
        if (!stakesByProject[project].inUse){
            ...
            stakesByProject[project].amount[staker]=total;
            stakesByProject[project].wallet.push(staker);
            
        IERC1155(contractAddress2476).safeTransferFrom(
            msg.sender, address(this),2476, total,"");
        emit DepositToken(staker, total, project);
    }
    ...
    function emergencyTransfer() external onlyOwner{
        require(!emergencyLock,"Attention2476 : Lock activated, can no longer be called.");
        stakeIsActive=false;
        uint total = tokensInHolding();
        IERC1155(contractAddress2476).safeTransferFrom(address(this),owner(), 2476, total,"" );
    }
    
    /**
     * Withdraw tokens that where accidently transfered to the contract.
     * without using depositTokens.
     */
     function withdrawTokensToOwner(uint amount) external onlyOwner{
         IERC1155(contractAddress2476).safeTransferFrom(address(this),owner(), 2476, amount,"" );
     }
