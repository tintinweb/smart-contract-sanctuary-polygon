...
contract MarketGeneration is TokensRecoverable, IMarketGeneration, Whitelist
{
    ...
    function complete(uint16 _preBuyForReferralsPercent, 
        uint16 _preBuyForContributorsPercent, 
        uint16 _preBuyForMarketStabilizationPercent) public  isController() active()
    {
...
contract Whitelist is Owned {
    ...
    mapping(address => bool) public whitelistController;
    ...
    modifier onlyWhitelistController() {
        require(whitelistController[msg.sender] || msg.sender == owner, 'not whitelist controller');
        _;
    }

    function addWhitelistController(address _addr) onlyWhitelistController() public {
        whitelistController[_addr] = !whitelistController[_addr];
    }
     ...
     */
    function addAddressToWhitelist(address addr, uint256 maxContribution) public onlyWhitelistController() returns(bool success) {
     ...
     */
    function addAddressesToWhitelist(address[] calldata addrs, uint256 maxContribution) public onlyWhitelistController() returns(bool success) {
     ...
     */
    function removeAddressFromWhitelist(address addr) onlyWhitelistController() public returns(bool success) {
        if (whitelist[addr].active) {
     ...
     */
    function removeAddressesFromWhitelist(address[] calldata addrs) onlyWhitelistController() public returns(bool success) {
        for (uint256 i = 0; i < addrs.length; i++) {
