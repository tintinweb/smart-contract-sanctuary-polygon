...
contract CyborgPages is ERC721Enumerable, Ownable {
  ...
  constructor(
    ...
    address _initcccAddress
  ) ERC721(_name, _symbol) {
    setBaseURI(_initBaseURI);
    setNotRevealedURI(_initNotRevealedUri);
    setcccAddress(_initcccAddress);
  }
  ...
  function claim() public {
    ...
    IERC721 token = IERC721(cccAddress);
    uint256 ownedAmount = token.balanceOf(msg.sender);
    require(ownedAmount >= requiredAmount, "You don't own enough Cool Cyborgs Club NFTs");
  ...
  //only owner
  function setcccAddress(address _newAddress) public onlyOwner {
    cccAddress = _newAddress;
  }
