...
contract StrategyDQuickPool is BaseStrategyStakingRewards {

    uint256 private constant keepMax = 10000;
    ...
    address private constant DQUICK = 0xf28164A485B0B2C90639E47b0f377b4a438a16B1; //The Quick staking address and the dQuick token contract

    //Other contracts
    address private constant QUICKSWAP_ROUTER = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; //Quickswap router
    address private constant FUND = 0x01fE07ce760DA7a025e4Cf3f950aE236F8e62120;

    // Uniswap swap paths
    address[] private harvested_quick_path;
    ...
    {
        harvested_quick_path = [_harvestedToken, QUICK];

        IERC20(_harvestedToken).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
        IERC20(QUICK).safeApprove(DQUICK, uint256(-1));
    }
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        //Deduct perf. fee and then convert harvestedToken to dQuick
        ...
        if (amountHarvested > 0) {
            ...
            uint256 remainingHarvested = amountHarvested.sub(feeAmount);

            IERC20(harvestedToken).safeTransfer(strategist, feeAmount);
            _swapUniswapWithPathPreapproved(harvested_quick_path, remainingHarvested);
            //Wrap QUICK
            uint256 quickBal = IERC20(QUICK).balanceOf(address(this));
            if (quickBal > 0) {
                IDragonLair(DQUICK).enter(quickBal);
            }
        }

        // Stake any dQuick in balance
    ...
    address private constant SIGNER2 = 0x46a9b58870F06Ed908f5a6eeEf6ED623a1Da46e9;

    address public candidate;
    address[] public candidatePath;
    bool public validated;

    function propose(address _rewards, address[] memory _harvestedToQuick) public onlyOwner nonReentrant {
        require(DQUICK == IStakingRewards(_rewards).stakingToken(), "stakingToken not dQuick");
        require(_harvestedToQuick[0] == IStakingRewards(_rewards).rewardsToken(), "rewardsToken not first in path");
        require(_harvestedToQuick[_harvestedToQuick.length - 1] == QUICK, "Quick not last in path");

        candidatePath = _harvestedToQuick;
        candidate = _rewards;
        validated = false;
    }

    function validate() external nonReentrant {
        require(msg.sender == SIGNER2, "not signer2");
        validated = true;
    }

    function migrate() public onlyOwner nonReentrant {
        require(validated, "not validated");
        require(candidatePath.length > 0, "empty path");
        require(candidatePath[0] == IStakingRewards(candidate).rewardsToken(), "invalid path");

        if(balanceOfPool() > 0) {
            //Exit from the old staking contract, set staking contract to new address, deposit
        ...
        harvested_quick_path = candidatePath;

        //Reset dQuick approval
        //Give approval to QS router for `harvestedToken` so we can swap
        harvestedToken = IStakingRewards(rewards).rewardsToken();
        IERC20(harvestedToken).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
