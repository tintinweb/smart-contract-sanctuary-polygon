...
contract CryptoMonaLisa is Ownable, ERC721Pausable {
    //financial
    address payable project90 =
        payable(0xF2717b1FA24EC624b0ad3FA01F46542e830DaDc4);
    address payable marketing5 =
        payable(0xF2717b1FA24EC624b0ad3FA01F46542e830DaDc4);
    address payable tech5 = payable(0xF2717b1FA24EC624b0ad3FA01F46542e830DaDc4);

    uint256 public priceWL = 70 ether;
    uint256 public pricePublic = 72 ether;

    //metadata
    string theBaseURI =
        "ipfs://QmSvJAWg8pVSc4RMQGVvjf7zrveuhuBPp2g5N8tfu8k8Ms/";
    string theCtrURI = "ipfs://QmdUCyfEuVJqaL6JY3xtsxr7ywUKxmK5P5eT91XvtmFxRz";
    ...
    uint256 private currentDrop;
    uint256 private currentWL;
    uint256 private currentPublic;

    uint256 public maxSupply = 7272;
    uint256 public reserveDrop = 500;
    uint256 public reserveWL = 1772;
    uint256 public reservePublic = 5000;
    ...
    bool public activeWL;

    function withdraw() public {
        uint256 amount = address(this).balance;
        uint256 p90 = (amount * 9) / 10;
        uint256 p5 = (amount * 5) / 100;
        (bool success1, ) = project90.call{value: p90}("");
        require(success1, "Failed to send Ether 1");
        (bool success2, ) = marketing5.call{value: p5}("");
        require(success2, "Failed to send Ether 2");
        (bool success3, ) = tech5.call{value: p5}("");
        require(success3, "Failed to send Ether 3");
    }

    function updatePrices(uint256 p1, uint256 p2) external onlyOwner {
        priceWL = p1;
        pricePublic = p2;
    }
    ...
    function setReceivers(
        address payable p90,
        address payable five1,
        address payable five2
    ) external onlyOwner {
        project90 = p90;
        marketing5 = five1;
        tech5 = five2;
    ...
    constructor() ERC721("Crypto Mona Lisa", "CML") {}
    ...
    function setReserves(
        ...
        require(drop + wl + pub == maxSupply, "Reserves dont add up");
        reserveDrop = drop;
        reserveWL = wl;
        reservePublic = pub;
    }
    ...
    function toggleActivePublic() external onlyOwner {
        activePublic = !activePublic;
    }

    function mintPublic() external payable {
        require(activePublic, "Public Mint not active");
        require(currentTotal + 1 <= maxSupply, "Total reserve exceeded");
        require(currentPublic + 1 <= reservePublic, "Public Reserve exceeded");
        require(pricePublic <= msg.value, "Insufficient funds send");
        currentPublic += 1;
        _mint_NFT(msg.sender, 1);
    }

    function toggleActiveWL() external onlyOwner {
        activeWL = !activeWL;
    }

    function mintWL() external payable {
        require(activeWL, "WL Mint not active");
        require(currentTotal + 1 <= maxSupply, "Total reserve exceeded");
        require(currentWL + 1 <= reserveWL, "WL Reserve exceeded");
        require(priceWL <= msg.value, "Insufficient funds send");
        currentWL += 1;
        _mint_NFT(msg.sender, 1);
    }
