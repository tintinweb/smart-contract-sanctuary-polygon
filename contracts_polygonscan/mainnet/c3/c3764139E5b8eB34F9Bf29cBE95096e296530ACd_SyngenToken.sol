...
0xfa2c0Df8037b5Ffdd1a04103EFA23DEa04E6eb93 // Development 
0xc8EDe0a0DB231a4Fa912C256026200346961724b // charity 
*/

contract SyngenToken is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 600000000000000 *  10**18; 

    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "syngen";
    string private _symbol = "syg";
    ...
    uint public _DevelopmentFee = 1;
    address public DevelopmentWallet = 0xfa2c0Df8037b5Ffdd1a04103EFA23DEa04E6eb93;

    uint256 public _CharityFee = 1;
    address public CharityWallet = 0xc8EDe0a0DB231a4Fa912C256026200346961724b;
    ...
    uint256 private numTokensSellToAddToLiquidity = 3000000 * 10**18;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 treasuryAmt = amount.mul(_DevelopmentFee).div(100);
        uint256 marketingAmt = amount.mul(_CharityFee).div(100);
        uint256 BurnAmt = amount.mul(_BurnFee).div(100);


        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(treasuryAmt).sub(marketingAmt).sub(BurnAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(treasuryAmt).sub(marketingAmt).sub(BurnAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(treasuryAmt).sub(marketingAmt).sub(BurnAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(treasuryAmt).sub(marketingAmt).sub(BurnAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(treasuryAmt).sub(marketingAmt).sub(BurnAmt)));
        }
        ...
        _transferStandard(sender, DevelopmentWallet, treasuryAmt);
        _transferStandard(sender, CharityWallet, marketingAmt);
