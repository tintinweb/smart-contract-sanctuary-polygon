...
contract HRHighScores is Ownable {
  ...
  uint256[] public hens;
  uint256 public racersCount;
  uint256 public hensCount;
  mapping(address => bool) public registeredRacer;
  mapping(uint256 => bool) public registeredHen;

  HenRace henRace;

  constructor(address _henRace) {
    henRace = HenRace(_henRace);
  }

  function registerRacer(address addy) public {
    require(!registeredRacer[addy], "Racer already registered");
    racers.push(addy);
    racersCount++;
    registeredRacer[addy] = true;
  }

  function registerHen(uint256 hen) public {
    require(!registeredHen[hen], "Hen already registered");
    hens.push(hen);
    hensCount++;
    registeredHen[hen] = true;
  }

  function racersByPage(uint8 page) public view returns (Racer[] memory) {
    uint256 length = racers.length >= page * 100 ? 100 : racers.length - ((page - 1) * 100);
    Racer[] memory list = new Racer[](length);
    uint256 base = page == 1 ? 0 : ((page - 1) * 100);
    for (uint8 i = 0; i < list.length; i++) {
      address racer = racers[i + base];
      list[i] = Racer(racer, henRace.scoreByUser(racer));
    }
    return list;
  }

  function hensByPage(uint8 page) public view returns (Hen[] memory) {
    uint256 length = hens.length >= page * 100 ? 100 : hens.length - ((page - 1) * 100);
    Hen[] memory list = new Hen[](length);
    uint256 base = page == 1 ? 0 : ((page - 1) * 100);
    for (uint8 i = 0; i < list.length; i++) {
      uint256 hen = hens[i + base];
      list[i] = Hen(hen, henRace.scoreByTokenId(hen));
    }
    return list;
  }

  function addRacers (address[] memory addresses) public onlyOwner {
    uint256 length = addresses.length;
    for (uint256 i = 0; i < length; i++) {
      address addy = addresses[i];
      if (!registeredRacer[addy]) {
        racers.push(addy);
        racersCount++;
        registeredRacer[addy] = true;
      }
  ...
  function addHens (uint256[] memory hensArray) public onlyOwner {
    uint256 length = hensArray.length;
    for (uint256 i = 0; i < length; i++) {
      uint256 tokenId = hensArray[i];
      if (!registeredHen[tokenId]) {
        hens.push(tokenId);
        hensCount++;
        registeredHen[tokenId] = true;
      }
    }
  }
}
...
contract HenRace is Ownable, VRFConsumerBase {
  ...
  uint8 public maxRacersPerRace = 10;
  uint256 public entryCost = 50 ether;
  ...
  mapping(address => uint256) public scoreByUser;
  ...
  function enterRace(uint256 tokenId) public {
    require(foxHen.ownerOf(tokenId) == msg.sender && !foxHen.isFox(tokenId), "You must own that fox or hen. You can't race with a Fox.");
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    ...
    for (uint8 i = 0; i < positions.length; i++) {
      ...
      scoreByUser[race.racers[i].account] += 10 - positions[i];
      scoreByTokenId[race.racers[i].tokenId] += 10 - positions[i];
      if (positions[i] < 3) {
