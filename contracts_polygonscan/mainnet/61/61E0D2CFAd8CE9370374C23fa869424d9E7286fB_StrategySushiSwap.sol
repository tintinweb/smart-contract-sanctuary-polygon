        ...
        returns (uint256)
    {
        if (rewardRate > 0) {
            ...
            uint256 usdcAfter = IERC20(usdcAddress)
                ...
                .sub(usdcBefore);
            (bool success) = IStrategyMush(rewardAddress).depositReward(usdcAfter);
            require(
                success,
                'USD transfer failed'
            );
...
abstract contract BaseStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    address public constant mushAddress =
        0xa77944a08f2e127aC8392eC19C094DfEDDdC604C; // LOCAL TEST TOKEN ADDRESS!! Must change before deploying to production
    address public constant rewardAddress =
        0x81A7c58C66462d338f9c79A0b2Ec84C48D8e4675; // LOCAL TEST DIVIDEND CONTRACT ADDRESS!! Must change before deploying to production
