...
contract WalletMonstersApusV2 is ERC1155, Ownable, IERC1155Receiver{
    ...
    mapping(address => mapping(uint256 => int256)) public health;
   
 
    //Dapp Events
    event bornDate(address from, string name, uint256 enjimonID,bool parthenogenesis, uint256 date);
    ...
    event battleApproved(uint256 battleId, address opponent,uint256 opEnjimonId, address challenger, uint256 chalEnjimonId, uint256 date);
    event battleCancelled(address challenger, address opponent,uint256 battleId, uint256 date);
    event battleRetreat(address flight, address fight,uint256 battleId, uint256 date);
    event battleStale(address playerDraw, address playerQuit,uint256 battleId, uint256 date);
    ...
    constructor() ERC1155("https://jovzrmggro7w.usemoralis.com/{id}.json") {
   ...
   //Utility functions
    ...
    //interactions
    function mintMonster(string memory enjimonName, int256 healthPoints, int256 defense, int256 attack, uint256 level, string memory enjimonType,bool parthenogenesis) public onlyOwner {
 
        _tokenDetails[nextId] = Enjimon(enjimonName, healthPoints, defense, attack, level, block.timestamp, enjimonType, 0,"Apus Sector", parthenogenesis);
       
        _mint(msg.sender, nextId, 1, "");
       
        emit bornDate(msg.sender, enjimonName, nextId, parthenogenesis, block.timestamp);
    ...
    function approveBattle(uint _battleId, uint defendingEnjimon) public  
            {
               ...
               require(hasPendingBattle[msg.sender] == true);
                ...
                emit battleApproved(_battleId, msg.sender,trainerEnjimon[msg.sender], _pendingBattle[_battleId].challenger,trainerEnjimon[_pendingBattle[_battleId].challenger], block.timestamp); 
    ...
    function attack(uint256 _battleId) public{
      ...
      Enjimon storage enjimon2 =_tokenDetails[trainerEnjimon[ _opponent]]; //opponent
        ...
        if(msg.sender ==  _challenger){
           ...
           int256 damage = enjimon2.defense - enjimon1.attack;
           ...
           if(enjimon2.healthPoints - damage <= 0){
               ...
               uint index = Randomness();
                ...
                enjimon1.defense+=2;
                enjimon1.attack+=1;
                enjimon1.healthPoints = health[msg.sender][trainerEnjimon[msg.sender]] + 5;
            ...
            turnTime[msg.sender] = block.timestamp;
            int256 damage = enjimon1.defense - enjimon2.attack;
            ...
            if(enjimon1.healthPoints - damage <= 0){
               ...
               _safeTransferFrom(address(this), _opponent, 0 , reward,'' );
                ...
                enjimon2.defense+=2;
                enjimon2.attack+=1;
                enjimon2.healthPoints = health[msg.sender][trainerEnjimon[msg.sender]] + 5;
    ...
    function cancelBattle(uint256 _battleId) public returns(bool){
        ...
        emit battleCancelled(msg.sender, _pendingBattle[_battleId].opponent, _battleId, block.timestamp);
    ...
    if(msg.sender == _pendingBattle[_battleId].challenger){
        safeTransferFrom(msg.sender, _pendingBattle[_battleId].opponent, 0, 25, '');
        _burn(msg.sender, 0, 25);
        amountBurned+= 25;

         emit battleRetreat(msg.sender,_pendingBattle[_battleId].opponent ,_battleId, block.timestamp);

    }else if(msg.sender == _pendingBattle[_battleId].opponent){
         safeTransferFrom(msg.sender, _pendingBattle[_battleId].challenger, 0, 25, '');
        _burn(msg.sender, 0, 25);
        amountBurned+= 25;

        emit battleRetreat(msg.sender,_pendingBattle[_battleId].challenger ,_battleId, block.timestamp);
    }
    ...
    function stalePlayer(uint256 _battleId) public returns(bool){
        ...
        delete _pendingBattle[_battleId];
       if(msg.sender == _pendingBattle[_battleId].opponent){

           emit battleStale(msg.sender, _pendingBattle[_battleId].challenger,_battleId, block.timestamp);
       }else if(msg.sender == _pendingBattle[_battleId].challenger){
       
            emit battleStale(msg.sender, _pendingBattle[_battleId].opponent,_battleId, block.timestamp);
       }
