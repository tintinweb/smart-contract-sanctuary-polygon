...
contract FloorTokenChild is IERC721, Ownable, ERC721Checkpointable, FxBaseChildTunnel {

    /**
     * @notice Require that the sender is the childChainManager.
     */
    modifier onlyChildChainManager() {
        require(msg.sender == childChainManager, 'Sender is not the childChainManager');
        _;
    }
    ...
    event FloorCreated(uint256 indexed tokenId, uint8 size);
    ...
    function _processMessageFromRoot(
        ...
        (uint256 floorId, uint8 size, string memory meta) = abi.decode(data, (uint256, uint8, string));
        sizes[floorId] = size;
        metadata[floorId] = meta;
    }

    function sendMessageToRoot(bytes memory message) private {
