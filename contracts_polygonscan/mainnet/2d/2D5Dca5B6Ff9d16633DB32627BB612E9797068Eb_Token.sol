...
contract Token is Context, IERC20, Ownable {
  ...
  uint256 private _diffRatio = 1; // for comparison during mint & burn

  string private _name = 'ProtocolV6';
  string private _symbol = 'POL6';
  ...
  function balanceOf(address account) public view override returns (uint256) {
    ...
    return tokenFromReflection(_rOwned[account]).mul(_diffRatio);
  }
    ...
    )
  {
    ...
    return (tTransferAmount, tFee, tTreasury, tLiquidity);
  }
  ...
  function _takeTreasury(uint256 tTreasury) private {
    ...
    _rOwned[_treasury] = _rOwned[_treasury].add(rTreasury);
    if (_isExcluded[_treasury])
      _tOwned[_treasury] = _tOwned[_treasury].add(tTreasury);
  }
  ...
  function _mint(address account, uint256 amount) public virtual {
    ...
    _tTotal += amount;
    uint256 _rTotalNew = (MAX - (MAX % _tTotal));
    _diffRatio = _rTotalNew.div(_rTotal);
    uint256 rAmount = amount.mul(_getRate());
    _rOwned[account] = _rOwned[account].add(rAmount);
    if (_isExcluded[account]) _tOwned[account] = _tOwned[account].add(amount);
  ...
  function _burn(address account, uint256 amount) public virtual {
    ...
    unchecked {
      ...
      if (_isExcluded[account]) _tOwned[account] = _tOwned[account].sub(amount);
    }
